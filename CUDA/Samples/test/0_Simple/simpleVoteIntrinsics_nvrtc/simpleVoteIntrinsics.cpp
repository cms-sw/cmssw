...
int main(int argc, char **argv)
{
    ...
    checkCudaErrors(cuMemAlloc(&d_input, VOTE_DATA_GROUP*warp_size * sizeof(unsigned int)));
    checkCudaErrors(cuMemAlloc(&d_result, VOTE_DATA_GROUP*warp_size * sizeof(unsigned int)));

    genVoteTestPattern(h_input, VOTE_DATA_GROUP*warp_size);

    checkCudaErrors(cuMemcpyHtoD(d_input, h_input, VOTE_DATA_GROUP*warp_size * sizeof(unsigned int)));
    ...
    printf("\tRunning <<Vote.Any>> kernel1 ...\n");
    {
        ...
        checkCudaErrors(cuModuleGetFunction(&kernel_addr, module, "VoteAnyKernel1"));

        int size = VOTE_DATA_GROUP *warp_size;
        void *arr[] = { (void *)&d_input, (void *)&d_result, (void *)&size };

        checkCudaErrors(cuLaunchKernel(kernel_addr,
                                            gridBlock.x, gridBlock.y, gridBlock.z, /* grid dim */
    ...
    checkCudaErrors(cuMemcpyDtoH(h_result, d_result,VOTE_DATA_GROUP*warp_size * sizeof(unsigned int)));
    ...
    printf("\tRunning <<Vote.All>> kernel2 ...\n");
    {
        ...
        CUfunction kernel_addr;
        checkCudaErrors(cuModuleGetFunction(&kernel_addr, module, "VoteAllKernel2"));
    ...
    printf("\tRunning <<Vote.Any>> kernel3 ...\n");
    {
        ...
        checkCudaErrors(cuModuleGetFunction(&kernel_addr, module, "VoteAnyKernel3"));
