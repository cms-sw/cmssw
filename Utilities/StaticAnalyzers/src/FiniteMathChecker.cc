  ...
  void FiniteMathChecker::checkPreStmt(const clang::CallExpr *CE, clang::ento::CheckerContext &ctx) const {
    ...
    if (!BT)
      BT.reset(new clang::ento::BugType(this,
                                        ...
                                        "fastmath plugin"));
    std::unique_ptr<clang::ento::PathSensitiveBugReport> PSBR = std::make_unique<clang::ento::PathSensitiveBugReport>(*BT, BT->getCheckerName(), N);
    std::unique_ptr<clang::ento::BasicBugReport> report = std::make_unique<clang::ento::BasicBugReport>(*BT, BT->getCheckerName(), PSBR->getLocation());
