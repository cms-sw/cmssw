...
Source::~Source()
{
  new DelayedClose(get_underlying(m_fh), m_id, m_site);
}

std::shared_ptr<XrdCl::File>
Source::getFileHandle()
{
    return get_underlying(m_fh);
}
...
Source::handle(std::shared_ptr<ClientRequest> c)
{
    ...
    if (m_stats)
    {
        std::shared_ptr<XrdReadStatistics> readStats = XrdSiteStatistics::startRead(get_underlying(m_stats), c);
