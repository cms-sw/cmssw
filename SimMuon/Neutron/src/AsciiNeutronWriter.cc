...
void AsciiNeutronWriter::writeCluster(int chamberType,
  ...
  for (size_t i = 0; i < hits.size(); ++i) {
    ...
    os << h.entryPoint().x() << " " << h.entryPoint().y() << " "
       ...
       << " "
       << " " << h.timeOfFlight() << " " << h.energyLoss() << " "
       << h.particleType() << " " << h.detUnitId() << " " << h.trackId() << " "
       << h.momentumAtEntry().theta() << " " << h.momentumAtEntry().phi()
       << endl;
  }
}
