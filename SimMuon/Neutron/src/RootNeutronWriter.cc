...
RootNeutronWriter::~RootNeutronWriter() {
  for (std::map<int, RootChamberWriter>::iterator mapItr =
           theChamberWriters.begin();
...
RootChamberWriter &RootNeutronWriter::chamberWriter(int chamberType) {
  std::map<int, RootChamberWriter>::iterator mapItr =
      theChamberWriters.find(chamberType);
  if (mapItr != theChamberWriters.end()) {
    return mapItr->second;
  } else {
    throw cms::Exception("NeutronWriter")
        << "It's dangerous to create ROOT chamber "
...
void RootNeutronWriter::writeCluster(int chamberType,
                                     const edm::PSimHitContainer &hits) {
  std::cout << "ROOTNEUTRONWRITER " << chamberType << " HITS SIZE "
            << hits.size() << std::endl;
