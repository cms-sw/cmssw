...
SubsystemNeutronWriter::SubsystemNeutronWriter(edm::ParameterSet const &pset)
    : theHitWriter(nullptr), useRandFlat(false),
      ...
      theT0(pset.getParameter<double>("t0")), theNEvents(0), initialized(false),
      useLocalDetId_(true) {
  ...
  if (writer == "ASCII") {
    ...
    if (!rng.isAvailable()) {
      throw cms::Exception("Configuration")
          << "SubsystemNeutronWriter requires the "
...
void SubsystemNeutronWriter::printStats() {
  ...
  for (map<int, int>::iterator mapItr = theCountPerChamberType.begin();
    ...
    edm::LogInfo("SubsystemNeutronWriter")
        << "   theEventOccupancy[" << mapItr->first << "] = " << mapItr->second
        << " / " << theNEvents << " / NCT \n";
  }
}
...
void SubsystemNeutronWriter::produce(edm::Event &e, edm::EventSetup const &c) {
  ...
  for (map<int, edm::PSimHitContainer>::iterator hitsByChamberItr =
           hitsByChamber.begin();
...
void SubsystemNeutronWriter::writeHits(int chamberType,
  ...
  for (size_t i = 0; i < chamberHits.size(); ++i) {
    ...
    LogDebug("SubsystemNeutronWriter")
        << "found hit from part type " << hit.particleType() << " at tof "
        << tof << " p " << hit.pabs() << " on det " << hit.detUnitId()
...
void SubsystemNeutronWriter::writeCluster(
    int chamberType, const edm::PSimHitContainer &cluster) {
  if (accept(cluster)) {
...
void SubsystemNeutronWriter::adjust(PSimHit &h, float timeOffset,
                                    float smearing) {
  unsigned int detId =
      useLocalDetId_ ? localDetId(h.detUnitId()) : h.detUnitId();
  float htime = h.timeOfFlight() + timeOffset + smearing;
  // prevent float precision loss
  if (h.timeOfFlight() > 1.E+6) {
    htime = smearing;
  }
  h = PSimHit(h.entryPoint(), h.exitPoint(), h.pabs(), htime, h.energyLoss(),
              h.particleType(), detId, h.trackId(), h.momentumAtEntry().theta(),
