...
SubsystemNeutronReader::SubsystemNeutronReader(const edm::ParameterSet &pset)
    : theHitReader(nullptr), theLuminosity(pset.getParameter<double>(
                                 "luminosity")), // in units of 10^34
      theStartTime(pset.getParameter<double>("startTime")),
      theEndTime(pset.getParameter<double>("endTime")),
      theEventOccupancy(
          pset.getParameter<vector<double>>("eventOccupancy")) // TODO make map
{
...
void SubsystemNeutronReader::generateChamberNoise(
    int chamberType, int chamberIndex, edm::PSimHitContainer &result,
    CLHEP::HepRandomEngine *engine) {
  // make sure this chamber hasn't been done before
  if (find(theChambersDone.begin(), theChambersDone.end(), chamberIndex) ==
      theChambersDone.end()) {
    float meanNumberOfEvents =
        theEventOccupancy[chamberType - 1] * theEventsInWindow;
    ...
    // std::cout << "Number of neutron events to add for chamber type " <<
    // chamberType << " : "
    ...
    for (int i = 0; i < nEventsToAdd; ++i) {
      ...
      float timeOffset =
          CLHEP::RandFlat::shoot(engine, theStartTime, theEndTime);
      ...
      for (vector<PSimHit>::const_iterator neutronHitItr = neutronHits.begin();
        ...
        PSimHit hit(rawHit.entryPoint(), rawHit.exitPoint(), rawHit.pabs(),
                    rawHit.tof() + timeOffset, rawHit.energyLoss(),
                    rawHit.particleType(), det, rawHit.trackId(),
                    rawHit.thetaAtEntry(), rawHit.phiAtEntry(),
                    rawHit.processType());
