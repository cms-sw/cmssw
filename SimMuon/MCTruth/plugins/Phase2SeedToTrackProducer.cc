...
#include "Phase2SeedToTrackProducer.h"
...
//
Phase2SeedToTrackProducer::Phase2SeedToTrackProducer(const edm::ParameterSet &iConfig)
...
void Phase2SeedToTrackProducer::produce(edm::StreamID, edm::Event &iEvent, const edm::EventSetup &iSetup) const {
  using namespace edm;
  using namespace std;
  ...
  if (L2seedsCollection.isValid())
    ...
    edm::LogError("Phase2SeedToTrackProducer") << "L2 seeds collection not found !! " << endl;
  ...
  for (unsigned int i = 0; i < L2seeds->size(); i++) {
    ...
    edm::LogVerbatim("Phase2SeedToTrackProducer")
    ...
    edm::LogVerbatim("Phase2SeedToTrackProducer") <<  "seedPtError=" <<
...
TrajectoryStateOnSurface Phase2SeedToTrackProducer::seedTransientState(
    const L2MuonTrajectorySeed &tmpSeed,
