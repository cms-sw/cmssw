...
CSCStripElectronicsSim::CSCStripElectronicsSim(const edm::ParameterSet &p)
    : CSCBaseElectronicsSim(p),
      ...
      theComparatorThreshold(20.), theComparatorNoise(0.),
      theComparatorRMSOffset(2.), theComparatorSaturation(1057.),
      theComparatorWait(50.), theComparatorDeadTime(100.), theDaqDeadTime(200.),
      theTimingOffset(0.), nScaBins_(p.getParameter<int>("nScaBins")),
      doSuppression_(p.getParameter<bool>("doSuppression")),
      doCrosstalk_(p.getParameter<bool>("doCrosstalk")),
      theStripConditions(nullptr), theCrosstalkGenerator(nullptr),
      theComparatorClockJump(2), sca_time_bin_size(50.),
      sca_peak_bin(p.getParameter<int>("scaPeakBin")),
      theComparatorTimeBinOffset(
          p.getParameter<double>("comparatorTimeBinOffset")),
      theComparatorTimeOffset(p.getParameter<double>("comparatorTimeOffset")),
      theComparatorSamplingTime(
          p.getParameter<double>("comparatorSamplingTime")),
      theSCATimingOffsets(
          p.getParameter<std::vector<double>>("scaTimingOffsets")) {
...
void CSCStripElectronicsSim::initParameters() {
  ...
  theAverageTimeOfFlight =
      averageDistance * cm / c_light; // Units of c_light: mm/ns
  ...
  theNumberOfSamples =
      nScaBins_ * static_cast<int>(sca_time_bin_size / theSamplingTime);
}
...
CSCAnalogSignal
CSCStripElectronicsSim::makeNoiseSignal(int element,
  ...
  CSCAnalogSignal finalSignal(element, theSamplingTime, binValues, 0.,
                              theSignalStartTime);
  return finalSignal;
}

float CSCStripElectronicsSim::comparatorReading(
    const CSCAnalogSignal &signal, float time,
...
void CSCStripElectronicsSim::runComparator(
    std::vector<CSCComparatorDigi> &result, CLHEP::HepRandomEngine *engine) {
  // first, make a list of all the comparators we actually
  // need to run
  std::list<int> comparatorsWithSignal;
  CSCSignalMap::iterator signalMapItr;
  for (signalMapItr = theSignalMap.begin(); signalMapItr != theSignalMap.end();
       ++signalMapItr) {
  ...
  for (std::list<int>::iterator listItr = comparatorsWithSignal.begin();
    ...
    const CSCAnalogSignal &signal1 =
        find(readoutElement(iComparator * 2 + 1), engine);
    const CSCAnalogSignal &signal2 =
        find(readoutElement(iComparator * 2 + 2), engine);
    for (float time = theSignalStartTime + theComparatorTimeOffset;
      ...
      if (comparatorReading(signal1, time, engine) > theComparatorThreshold ||
        ...
        if (height1 > height2) {
          ...
          float leftStrip = 0.;
          if (iComparator > 0) {
            leftStrip = comparatorReading(
                find(readoutElement(iComparator * 2), engine), time, engine);
          }
          ...
          if (iComparator * 2 + 3 <= nElements) {
            rightStrip = comparatorReading(
                find(readoutElement(iComparator * 2 + 3), engine), time,
                engine);
          }
...
std::list<int> CSCStripElectronicsSim::getKeyStrips(
    const std::vector<CSCComparatorDigi> &comparators) const {
  std::list<int> result;
  for (std::vector<CSCComparatorDigi>::const_iterator compItr =
           comparators.begin();
...
std::list<int> CSCStripElectronicsSim::getKeyStripsFromMC() const {
  ...
  std::list<int> result;
  transform(
      theDetectorHitMap.begin(), theDetectorHitMap.end(),
...
std::list<int>
CSCStripElectronicsSim::channelsToRead(const std::list<int> &keyStrips,
                                       int window) const {
  std::list<int> result;
  ...
  if (doSuppression_) {
    for (; keyStripItr != keyStrips.end(); ++keyStripItr) {
      ...
      for (int istrip = (*keyStripItr) - window;
           istrip <= (*keyStripItr) + window; ++istrip) {
...
void CSCStripElectronicsSim::fillStripDigis(const std::list<int> &keyStrips,
  ...
  CSCStripDigiCollection::Range stripRange(stripDigis.begin(),
                                           stripDigis.end());
...
void CSCStripElectronicsSim::addCrosstalk(CLHEP::HepRandomEngine *engine) {
  ...
  CSCSignalMap::iterator mapI = theSignalMap.begin(),
                         mapEnd = theSignalMap.end();
...
void CSCStripElectronicsSim::addCrosstalk(const CSCAnalogSignal &signal,
  ...
  theCrosstalkGenerator->setParameters(capacitiveCrosstalk, 0.,
                                       resistiveCrosstalk);
...
void CSCStripElectronicsSim::createDigi(int channel,
  ...
  int chamberType = theSpecs->chamberType();
  float timeSmearing =
      CLHEP::RandGaussQ::shoot(engine) * theTimingCalibrationError[chamberType];
  // undo the correction for TOF, instead, using some nominal
  // value from ME2/1
  float t0 = theSignalStartTime + theSCATimingOffsets[chamberType] +
...
void CSCStripElectronicsSim::fillMissingLayer(
    const CSCLayer *layer, const CSCComparatorDigiCollection &comparators,
    CSCStripDigiCollection &digis, CLHEP::HepRandomEngine *engine) {
  theSignalMap.clear();
  setLayer(layer);
  CSCDetId chamberId(theLayerId.chamberId());
  // find all comparator key strips in this chamber
  std::list<int> chamberKeyStrips;
  for (CSCComparatorDigiCollection::DigiRangeIterator comparatorItr =
           comparators.begin();
       comparatorItr != comparators.end(); ++comparatorItr) {
    // could be more efficient
    if (CSCDetId((*comparatorItr).first).chamberId() == chamberId) {
      std::vector<CSCComparatorDigi> layerComparators(
          (*comparatorItr).second.first, (*comparatorItr).second.second);
      std::list<int> layerKeyStrips = getKeyStrips(layerComparators);
      chamberKeyStrips.insert(chamberKeyStrips.end(), layerKeyStrips.begin(),
                              layerKeyStrips.end());
    }
  }
