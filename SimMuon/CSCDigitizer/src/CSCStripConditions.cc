...
template class CorrelatedNoisifier<
    CSCStripConditions::CSCCorrelatedNoiseMatrix>;

template void
CorrelatedNoisifier<CSCStripConditions::CSCCorrelatedNoiseMatrix>::noisify(
    CSCAnalogSignal &, CLHEP::HepRandomEngine *,
...
float CSCStripConditions::smearedGain(const CSCDetId &detId, int channel,
                                      CLHEP::HepRandomEngine *engine) const {
  return CLHEP::RandGaussQ::shoot(engine, gain(detId, channel),
...
void CSCStripConditions::noisify(const CSCDetId &detId, CSCAnalogSignal &signal,
  ...
  CSCAnalogSignal tmpSignal(channel, scaBinSize, binValues, 0.,
                            signal.getTimeOffset());
