...
void CSCNeutronReader::addHits(std::map<int, edm::PSimHitContainer> &hitMap,
  ...
  for (std::map<int, edm::PSimHitContainer>::const_iterator signalHitItr =
           signalHits.begin();
    ...
    if (find(chambersDone.begin(), chambersDone.end(), chamberIndex) ==
        chambersDone.end()) {
      ...
      for (edm::PSimHitContainer::const_iterator neutronHitItr =
               neutronHits.begin();
