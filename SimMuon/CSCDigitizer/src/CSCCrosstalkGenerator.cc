...
CSCAnalogSignal
CSCCrosstalkGenerator::getCrosstalk(const CSCAnalogSignal &inputSignal) const {
  ...
  for (int outputBin = 0; outputBin < nBins; ++outputBin) {
    ...
    float slope =
        inputSignal.getValue(aTime) - inputSignal.getValue(aTime - 1.);
    binValues[outputBin] = slope * theCrosstalk +
                           theResistiveFraction * inputSignal.getValue(aTime);
  }

  return CSCAnalogSignal(0, binSize, binValues, 0.,
                         inputSignal.getTimeOffset());
}
