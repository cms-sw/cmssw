...
int main(int argc, char *argv[])
{
  ...
  try {
    ...
    const DDCompactView::GraphType& gt = cpv.graph();
    ...
    for (; git != gend; ++git) {
      ...
      if (!git->empty()) { 
	DDCompactView::GraphType::edge_list::const_iterator cit  = git->begin();
	DDCompactView::GraphType::edge_list::const_iterator cend = git->end();
	for (; cit != cend; ++cit) {
