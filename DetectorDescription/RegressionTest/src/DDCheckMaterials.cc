...
// internal usage
bool
DDCheckMaterial( DDMaterial& mip, std::pair< std::string, std::string > & result, int rlevel = 0 )
{
  ...
  DDMaterial & mp = mip;
  result.second=mp.ddname().fullname();	 
...
bool
DDCheckMaterials( std::ostream & os, std::vector< std::pair< std::string, std::string >> * res )
{
  bool result = false;
