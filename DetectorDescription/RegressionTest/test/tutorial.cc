...
void dump_nodes(DDNodes & nodes, int max=100)
{
   ...
   for (; it != nodes.end(); ++it) {
     ...
     //std::cout << it->absRotation() << " " << std::endl;	   
...
void tutorial()
{
 ...
 const graph_type & cpvGraph = cpv.graph();
 ...
 typedef graph_type::const_edge_range NR; // type of range of neightbours 
  ...
  if (ans=="y") {
    ...
    */
    ...
    while (nav!="e") {
      ...
      std::cin >> nav;
      char c = nav[0];
      typedef std::vector<std::pair<DDPartSelection*,DDsvalues_type *> > spectype;
