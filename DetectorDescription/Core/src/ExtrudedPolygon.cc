...
DDI::ExtrudedPolygon::stream( std::ostream & os ) const
{
  ...
  for( unsigned int k = 1; k <= xysize; ++k )
    os << CMS_CONVERT_TO( p_[k], cm ) << ", " <<  CMS_CONVERT_TO( p_[k + xysize], cm ) << "; ";
  os << " with " << p_[0] << " Z sections:";
  unsigned int m0 = p_.size() - 4*p_[0];
  for( unsigned int m = m0; m < m0 + p_[0]; ++m )
  {
    os << " z[cm]=" << CMS_CONVERT_TO( p_[m], cm );
    os << ", x[cm]=" << CMS_CONVERT_TO( p_[m+p_[0]], cm );
    os << ", y[cm]="  << CMS_CONVERT_TO( p_[m+2*p_[0]], cm );
    os << ", scale[cm]="  << CMS_CONVERT_TO( p_[m+3*p_[0]], cm ) << ";";
  }
}
