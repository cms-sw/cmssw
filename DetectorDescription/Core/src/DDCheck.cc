...
//:void DDCheckPD(const DDLogicalPart & lp, GraphType::neighbour_type & nb, std::ostream & os)
bool DDCheckPD(const DDLogicalPart & lp, DDCompactView::GraphType::edge_range nb, const DDCompactView::GraphType & g, std::ostream & os)
{
   bool result = false;
...
bool DDCheckConnect(const DDCompactView & cpv, std::ostream & os)
{
  ...
  //  WalkerType wkr = DDCompactView().walker();
  WalkerType wkr = cpv.walker();
  ...
  // Pass 2:
  DDCompactView::GraphType & g = const_cast<DDCompactView::GraphType&>(cpv.graph());

  int uc = 0;
  DDCompactView::GraphType::adj_list::size_type it = 0;
...
{
   bool result = false;
   // const_cast because GraphType does not provide const_iterators!
   DDCompactView::GraphType & g = const_cast<DDCompactView::GraphType&>(cpv.graph());
