...
namespace {
  ...
  std::pair<bool, std::string> oldDDIsValid(const std::string& ns,
                                            const std::string& nm,
...
void testDDIsValid::checkAgaistOld() {
  ...
  while (std::getline(in, line)) {
    ...
    std::vector<DDLogicalPart> result;
    std::pair<bool, std::string> oldRes = oldDDIsValid(ns, line.substr(p, e - p), oldResult, true);
    std::pair<bool, std::string> res = DDIsValid(ns, line.substr(p, e - p), result, true);
    if (oldRes.first != res.first || oldRes.second != res.second || oldResult.size() != result.size()) {
      ++bad;
      continue;
    }
    for (int i = 0; i != int(result.size()); ++i) {
      if (result[i].name() == oldResult[i].name())
        continue;
      ++bad;
      break;
    }
  ...
  if (bad)
    std::cerr << bad << " queries not the same!" << std::endl;
