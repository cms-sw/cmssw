...
static long  algorithm(dd4hep::Detector& /* description */,
...
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("HGCalGeom") << childNames.size() << " children: " 
				<< childNames[0] << "; " << childNames[1]
				<< " positioned in " << nCellsRow.size()
				<< " rows and " << nColumns 
				<< " columns with lowest column at "
				<< nBottomY << " in mother " << parentName
				<< " of size " << waferSize;
  for (unsigned int k=0; k<nCellsRow.size(); ++k)
    edm::LogVerbatim("HGCalGeom") << "[" << k << "] Ncells " << nCellsRow[k]
				  << " Edge rotations " << angleEdges[2*k] 
				  << ":" << angleEdges[2*k+1]
				  << " Type of edge cells " 
				  << detectorType[2*k] << ":"
				  << detectorType[2*k+1];
  ...
  for (unsigned int ir=0; ir<nCellsRow.size(); ++ir) {
	...
	rotation = cms::makeRotation3D(90._deg, phix, 90._deg, phiy, 0., 0.);
      ...
      ++kount;
#ifdef EDM_ML_DEBUG
      edm::LogVerbatim("HGCalGeom") << "DDHGCalWafer: " << name << " number "
				    << copy << " positioned in " << parentName
