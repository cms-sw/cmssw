...
static long  algorithm(dd4hep::Detector& /* description */,
...
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("HGCalGeom") << "DDHGCalWafer8: Wafer 2r " << waferSize
				<< " T " << waferT << " Half Separation "
				<< waferSepar << " Cells/Wafer " << nCells
				<< " Cell Type " << cellType
				<< " Material " << material << " Names "
				<< motherName << " NameSpace "
				<< ns.name() << " # of cells "
				<< cellNames.size();
  ...
  for (int u=0; u<2*nCells; ++u) {
	...
	int    cell(0);
	if      ((u == 0)          && (v == 0))          cell = 7;
	else if ((u == 0)          && (v == nCells-1))   cell = 8;
	else if ((u == nCells)     && (v == 2*nCells-1)) cell = 9;
	else if ((u == 2*nCells-1) && (v == 2*nCells-1)) cell = 10;
	else if ((u == 2*nCells-1) && (v == nCells-1))   cell = 11;
	else if ((u == nCells)     && (v == 0))          cell = 12;
	else if (u == 0)                                 cell = 1;
	else if ((v-u) == (nCells-1))                    cell = 4;
	else if (v == (2*nCells-1))                      cell = 2;
	else if (u == (2*nCells-1))                      cell = 5;
	else if ((u-v) == nCells)                        cell = 3;
	else if (v == 0)                                 cell = 6;
...
#ifdef EDM_ML_DEBUG
	edm::LogVerbatim("HGCalGeom") << "DDHGCalWafer8: " << cellNames[cell]
