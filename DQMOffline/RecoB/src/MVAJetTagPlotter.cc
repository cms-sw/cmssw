...
MVAJetTagPlotter::MVAJetTagPlotter(const std::string &tagName,
  ...
  for (unsigned int i = 0; i != pSets.size(); ++i) {
    std::string ss = "CAT";
    ss += std::to_string(i);
    categoryPlotters.push_back(std::make_unique<TaggingVariablePlotter>(
        folderName, etaPtBin, pSets[i], mc, willFinalize, ibook, i ? ss : std::string()));
  }
}
...
void MVAJetTagPlotter::setEventSetup(const edm::EventSetup &setup) {
  ...
  if (!computer)
    throw cms::Exception("Configuration") << "JetTagComputer passed to "
