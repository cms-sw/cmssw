							 ...
							 const bool& wf, DQMStore::IBooker & ibook)
{
  if (dataFormatType == "TrackCounting") {
    return new TrackCountingTagPlotter(folderName, etaPtBin, pSet, update, mc, wf, ibook);
  } else if (dataFormatType == "TrackProbability") {
    return new TrackProbabilityTagPlotter(folderName, etaPtBin, pSet, update, mc, wf, ibook);
  } else if (dataFormatType == "SoftLepton") {
    return new SoftLeptonTagPlotter(folderName, etaPtBin, pSet, mc, update, ibook);
  } else if (dataFormatType == "TrackIP") {
    return new TrackIPTagPlotter(folderName, etaPtBin, pSet, update, mc, wf, ibook);
  } else if (dataFormatType == "TaggingVariable") {
    return new TaggingVariablePlotter(folderName, etaPtBin, pSet, update, mc, ibook);
  } else if (dataFormatType == "GenericMVA") {
    return new MVAJetTagPlotter(tagName, etaPtBin, pSet, folderName, update, mc, ibook);
  }
