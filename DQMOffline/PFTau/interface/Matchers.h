...
void match(const C &candCollection, const M &matchedCandCollection,
           std::vector<int> &matchIndices, bool matchCharge = false,
           float dRMax = -1) {
...
void match(const C &candCollection, const M &matchedCandCollection,
           ...
           bool matchCharge = false, float dRMax = -1) {
  ...
  for (unsigned i = 0; i < candCollection.size(); ++i) {
    ...
    for (unsigned jm = 0; jm < matchedCandCollection.size(); ++jm) {
      ...
      if (parameterSet.getParameter<bool>("slimmedLikeSelection")) {
        if (!(muonMatchedCandCollection[jm].pt() >
                  parameterSet.getParameter<double>("ptBase") ||
              muonMatchedCandCollection[jm].isPFMuon() ||
              (muonMatchedCandCollection[jm].pt() >
                   parameterSet.getParameter<double>("ptNotPF") &&
               (muonMatchedCandCollection[jm].isGlobalMuon() ||
                muonMatchedCandCollection[jm].isStandAloneMuon() ||
