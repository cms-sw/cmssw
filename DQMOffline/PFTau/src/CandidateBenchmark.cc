...
void CandidateBenchmark::setup(DQMStore::IBooker &b,
  ...
  if (!histogramBooked_) {
    ...
    edm::ParameterSet chPS =
        parameterSet.getParameter<edm::ParameterSet>("ChargeHistoParameter");

    if (ptPS.getParameter<bool>("switchOn")) {
      pt_ = book1D(
          b, "pt_", "p_{T};p_{T} (GeV)", ptPS.getParameter<int32_t>("nBin"),
          ptPS.getParameter<double>("xMin"), ptPS.getParameter<double>("xMax"));
    }
    ...
    if (chPS.getParameter<bool>("switchOn")) {
      charge_ = book1D(
          b, "charge_", "charge;charge", chPS.getParameter<int32_t>("nBin"),
          chPS.getParameter<double>("xMin"), chPS.getParameter<double>("xMax"));
    }
    histogramBooked_ = true;
  }
}
