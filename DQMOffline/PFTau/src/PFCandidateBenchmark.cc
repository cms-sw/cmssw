...
void PFCandidateBenchmark::setup(DQMStore::IBooker &b) {
  ...
  ecalEnergy_ =
      book1D(b, "ecalEnergy_", "ECAL energy, corrected;E_{ECAL} (GeV)",
             ecalEnergyPS.n, ecalEnergyPS.m, ecalEnergyPS.M);
  hcalEnergy_ =
      book1D(b, "hcalEnergy_", "HCAL energy, corrected;E_{HCAL} (GeV)",
             ecalEnergyPS.n, ecalEnergyPS.m, ecalEnergyPS.M);
  mva_e_pi_ = book1D(b, "mva_e_pi_", "e VS #pi MVA output;MVA", mva_e_piPS.n,
                     mva_e_piPS.m, mva_e_piPS.M);
  elementsInBlocksSize_ =
      book1D(b, "elementsInBlocksSize_", "number of elements used", 10, 0, 10);
}
