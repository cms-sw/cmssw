...
void MatchMETBenchmark::setup(DQMStore::IBooker &b) {
  ...
  delta_et_Over_et_VS_et_ = book2D(
      b, "delta_et_Over_et_VS_et_", ";ME_{T, true} (GeV);#DeltaME_{T}/ME_{T}",
      ptPS.n, ptPS.m, ptPS.M, dptOvptPS.n, dptOvptPS.m, dptOvptPS.M);

  delta_et_VS_et_ =
      book2D(b, "delta_et_VS_et_", ";ME_{T, true} (GeV);#DeltaME_{T}", ptPS.n,
             ptPS.m, ptPS.M, dptPS.n, dptPS.m, dptPS.M);

  delta_phi_VS_et_ =
      book2D(b, "delta_phi_VS_et_", ";ME_{T, true} (GeV);#Delta#phi", ptPS.n,
             ptPS.m, ptPS.M, dphiPS.n, dphiPS.m, dphiPS.M);

  delta_ex_ = book1D(b, "delta_ex_", "#DeltaME_{X}", dptPS.n, dptPS.m, dptPS.M);

  RecEt_VS_TrueEt_ = book2D(b, "RecEt_VS_TrueEt_", ";ME_{T, true} (GeV);ME_{T}",
                            ptPS.n, ptPS.m, ptPS.M, ptPS.n, ptPS.m, ptPS.M);

  delta_set_VS_set_ =
      book2D(b, "delta_set_VS_set_", ";SE_{T, true} (GeV);#DeltaSE_{T}",
             setPS.n, setPS.m, setPS.M, dsetPS.n, dsetPS.m, dsetPS.M);

  delta_set_Over_set_VS_set_ =
      book2D(b, "delta_set_Over_set_VS_set_",
             ";SE_{T, true} (GeV);#DeltaSE_{T}/SE_{T}", setPS.n, setPS.m,
             setPS.M, dptOvptPS.n, dptOvptPS.m, dptOvptPS.M);

  delta_ex_VS_set_ =
      book2D(b, "delta_ex_VS_set_", ";SE_{T, true} (GeV);#DeltaE_{X}", setPS.n,
             setPS.m, setPS.M, ptPS.n, -ptPS.M, ptPS.M);

  RecSet_Over_TrueSet_VS_TrueSet_ = book2D(
      b, "RecSet_Over_TrueSet_VS_TrueSet_", ";SE_{T, true} (GeV);SE_{T}/SE_{T}",
      setPS.n, setPS.m, setPS.M, setOvsetPS.n, setOvsetPS.m, setOvsetPS.M);
}
...
void MatchMETBenchmark::fillOne(const reco::MET &cand,
  ...
  delta_set_VS_set_->Fill(matchedCand.sumEt(),
                          cand.sumEt() - matchedCand.sumEt());
  if (matchedCand.sumEt() > 0.001)
    delta_set_Over_set_VS_set_->Fill(matchedCand.sumEt(),
                                     (cand.sumEt() - matchedCand.sumEt()) /
                                         matchedCand.sumEt());
  else
  ...
  if (matchedCand.sumEt() > 0.001)
    RecSet_Over_TrueSet_VS_TrueSet_->Fill(matchedCand.sumEt(),
                                          cand.sumEt() / matchedCand.sumEt());
}
