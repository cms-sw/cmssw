...
int elePublishHistos() {
  ...
  if (gSystem->AccessPathName(pub_output_dir.Data()) == kFALSE) {
    ...
    std::cerr << "Failed to create " << pub_output_dir << std::endl;
    return 1;
  } else {
  ...
  if (gSystem->CopyFile(pub_input_file.Data(), (pub_output_dir + "/" + pub_input_file).Data(), kTRUE) < 0) {
    std::cerr << "Failed to copy " << pub_input_file << std::endl;
    return 2;
  } else {
  ...
  if (file != 0) {
    ...
    if (file->cd(pub_input_folder) != kTRUE) {
      std::cerr << "Do not find " << pub_input_folder << std::endl;
      return 4;
    } else {
      std::cout << "Input folder is " << pub_input_folder << std::endl;
    }
  } else {
    std::cerr << "Failed to open " << pub_input_file << std::endl;
    return 3;
  }
  ...
  web_page << " They were made using configurations "
  ...
  web_page << " One can download the full <a href=\"" << pub_input_file << "\"> histograms file</a>.";
  web_page << "</p>\n";
  ...
  while (key1 = (TKey *)(*nextKey1)()) {
    ...
    if (obj1->IsA()->InheritsFrom("TDirectory") == kFALSE) {
      std::cout << "Ignoring object " << obj1->GetName() << std::endl;
      continue;
    } else {
    ...
    while (key2 = (TKey *)(*nextKey2)()) {
      ...
      if (obj2->IsA()->InheritsFrom("TH1") == kFALSE) {
        std::cout << "Ignoring object " << obj2->GetName() << std::endl;
        continue;
      }
  ...
  while (key1 = (TKey *)(*nextKey1)()) {
    ...
    while (key2 = (TKey *)(*nextKey2)()) {
      ...
      std::cout << dir->GetName() << "/" << histo->GetName() << ";" << key2->GetCycle() << " has "
                << histo->GetEntries() << " entries"
                << " (~" << histo->GetEffectiveEntries() << ")"
                << " of mean value " << histo->GetMean() << std::endl;
      ...
      web_page << "<a id=\"" << anchor_name << "\" name=\"" << anchor_name << "\"></a>"
               << "<a href=\"" << gif_name << "\"><img border=\"0\" class=\"image\" width=\"500\" src=\"" << gif_name
               << "\"></a><br>\n";
    }
  }
