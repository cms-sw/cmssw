...
void ElectronTagProbeAnalyzer::bookHistograms( DQMStore::IBooker & iBooker, edm::Run const &, edm::EventSetup const & )
 {
  ...
  // basic quantities
  h1_vertexPt_barrel = ElectronDqmAnalyzerBase::bookH1(iBooker, "vertexPt_barrel","ele transverse momentum in barrel",nbinpt,0.,ptmax,"p_{T vertex} (GeV/c)");
  h1_vertexPt_endcaps = ElectronDqmAnalyzerBase::bookH1(iBooker, "vertexPt_endcaps","ele transverse momentum in endcaps",nbinpt,0.,ptmax,"p_{T vertex} (GeV/c)");
  h1_vertexEta = ElectronDqmAnalyzerBase::bookH1(iBooker, "vertexEta","ele momentum #eta",nbineta,etamin,etamax,"#eta");
  h2_vertexEtaVsPhi = ElectronDqmAnalyzerBase::bookH2(iBooker, "vertexEtaVsPhi","ele momentum #eta vs #phi",nbineta2D,etamin,etamax,nbinphi2D,phimin,phimax,"#eta","#phi (rad)");
  h2_vertexXvsY = ElectronDqmAnalyzerBase::bookH2(iBooker, "vertexXvsY","ele vertex x vs y",nbinxyz2D,-0.1,0.1,nbinxyz2D,-0.1,0.1,"x (cm)","y (cm)" );
  h1_vertexZ = ElectronDqmAnalyzerBase::bookH1(iBooker, "vertexZ","ele vertex z",nbinxyz,-25, 25,"z (cm)" );

  // super-clusters
//  h1_sclPhi = ElectronDqmAnalyzerBase::bookH1(iBooker, "sclPhi","ele supercluster phi",nbinphi,phimin,phimax);
  h1_sclEt = ElectronDqmAnalyzerBase::bookH1(iBooker, "sclEt","ele supercluster transverse energy",nbinpt,0.,ptmax);

  // electron track
  h1_chi2 = ElectronDqmAnalyzerBase::bookH1(iBooker, "chi2","ele track #chi^{2}",100,0.,15.,"#Chi^{2}");
  h1_foundHits = ElectronDqmAnalyzerBase::bookH1(iBooker, "foundHits","ele track # found hits",nbinfhits,0.,fhitsmax,"N_{hits}");
  h1_lostHits = ElectronDqmAnalyzerBase::bookH1(iBooker, "lostHits","ele track # lost hits",5,0.,5.,"N_{lost hits}");

  // electron matching and ID
  h1_Eop_barrel = ElectronDqmAnalyzerBase::bookH1(iBooker,  "Eop_barrel","ele E/P_{vertex} in barrel",nbineop,0.,eopmax,"E/P_{vertex}");
  h1_Eop_endcaps = ElectronDqmAnalyzerBase::bookH1(iBooker,  "Eop_endcaps","ele E/P_{vertex} in endcaps",nbineop,0.,eopmax,"E/P_{vertex}");
  h1_EeleOPout_barrel = ElectronDqmAnalyzerBase::bookH1(iBooker,  "EeleOPout_barrel","ele E_{ele}/P_{out} in barrel",nbineop,0.,eopmax,"E_{ele}/P_{out}");
  h1_EeleOPout_endcaps = ElectronDqmAnalyzerBase::bookH1(iBooker,  "EeleOPout_endcaps","ele E_{ele}/P_{out} in endcaps",nbineop,0.,eopmax,"E_{ele}/P_{out}");
  h1_dEtaSc_propVtx_barrel = ElectronDqmAnalyzerBase::bookH1(iBooker,  "dEtaSc_propVtx_barrel","ele #eta_{sc} - #eta_{tr}, prop from vertex, in barrel",nbindetamatch,detamatchmin,detamatchmax,"#eta_{sc} - #eta_{tr}");
  h1_dEtaSc_propVtx_endcaps = ElectronDqmAnalyzerBase::bookH1(iBooker,  "dEtaSc_propVtx_endcaps","ele #eta_{sc} - #eta_{tr}, prop from vertex, in endcaps",nbindetamatch,detamatchmin,detamatchmax,"#eta_{sc} - #eta_{tr}");
  h1_dEtaEleCl_propOut_barrel = ElectronDqmAnalyzerBase::bookH1(iBooker,  "dEtaEleCl_propOut_barrel","ele #eta_{EleCl} - #eta_{tr}, prop from outermost, in barrel",nbindetamatch,detamatchmin,detamatchmax,"#eta_{elecl} - #eta_{tr}");
  h1_dEtaEleCl_propOut_endcaps = ElectronDqmAnalyzerBase::bookH1(iBooker,  "dEtaEleCl_propOut_endcaps","ele #eta_{EleCl} - #eta_{tr}, prop from outermost, in endcaps",nbindetamatch,detamatchmin,detamatchmax,"#eta_{elecl} - #eta_{tr}");
  h1_dPhiSc_propVtx_barrel = ElectronDqmAnalyzerBase::bookH1(iBooker,  "dPhiSc_propVtx_barrel","ele #phi_{sc} - #phi_{tr}, prop from vertex, in barrel",nbindphimatch,dphimatchmin,dphimatchmax,"#phi_{sc} - #phi_{tr} (rad)");
  h1_dPhiSc_propVtx_endcaps = ElectronDqmAnalyzerBase::bookH1(iBooker,  "dPhiSc_propVtx_endcaps","ele #phi_{sc} - #phi_{tr}, prop from vertex, in endcaps",nbindphimatch,dphimatchmin,dphimatchmax,"#phi_{sc} - #phi_{tr} (rad)");
  h1_dPhiEleCl_propOut_barrel = ElectronDqmAnalyzerBase::bookH1(iBooker,  "dPhiEleCl_propOut_barrel","ele #phi_{EleCl} - #phi_{tr}, prop from outermost, in barrel",nbindphimatch,dphimatchmin,dphimatchmax,"#phi_{elecl} - #phi_{tr} (rad)");
  h1_dPhiEleCl_propOut_endcaps = ElectronDqmAnalyzerBase::bookH1(iBooker,  "dPhiEleCl_propOut_endcaps","ele #phi_{EleCl} - #phi_{tr}, prop from outermost, in endcaps",nbindphimatch,dphimatchmin,dphimatchmax,"#phi_{elecl} - #phi_{tr} (rad)");
  h1_Hoe_barrel = ElectronDqmAnalyzerBase::bookH1(iBooker, "Hoe_barrel","ele hadronic energy / em energy, in barrel", nbinhoe, hoemin, hoemax,"H/E","Events","ELE_LOGY E1 P") ;
  h1_Hoe_endcaps = ElectronDqmAnalyzerBase::bookH1(iBooker, "Hoe_endcaps","ele hadronic energy / em energy, in endcaps", nbinhoe, hoemin, hoemax,"H/E","Events","ELE_LOGY E1 P") ;
  h1_sclSigEtaEta_barrel = ElectronDqmAnalyzerBase::bookH1(iBooker, "sclSigEtaEta_barrel","ele supercluster sigma eta eta in barrel",100,0.,0.05);
  h1_sclSigEtaEta_endcaps = ElectronDqmAnalyzerBase::bookH1(iBooker, "sclSigEtaEta_endcaps","ele supercluster sigma eta eta in endcaps",100,0.,0.05);

  // fbrem
  h1_fbrem = ElectronDqmAnalyzerBase::bookH1(iBooker, "fbrem","ele brem fraction",100,0.,1.,"P_{in} - P_{out} / P_{in}") ;
  h1_classes = ElectronDqmAnalyzerBase::bookH1(iBooker, "classes","ele electron classes",10,0.0,10.);

  // pflow
  h1_mva = ElectronDqmAnalyzerBase::bookH1(iBooker,  "mva","ele identification mva",100,-1.,1.);
  h1_provenance = ElectronDqmAnalyzerBase::bookH1(iBooker,  "provenance","ele provenance",5,-2.,3.);

  // isolation
  h1_tkSumPt_dr03 = ElectronDqmAnalyzerBase::bookH1(iBooker, "tkSumPt_dr03","tk isolation sum, dR=0.3",100,0.0,20.,"TkIsoSum, cone 0.3 (GeV/c)","Events","ELE_LOGY E1 P");
  h1_ecalRecHitSumEt_dr03 = ElectronDqmAnalyzerBase::bookH1(iBooker, "ecalRecHitSumEt_dr03","ecal isolation sum, dR=0.3",100,0.0,20.,"EcalIsoSum, cone 0.3 (GeV)","Events","ELE_LOGY E1 P");
  h1_hcalTowerSumEt_dr03 = ElectronDqmAnalyzerBase::bookH1(iBooker, "hcalTowerSumEt_dr03","hcal isolation sum, dR=0.3",100,0.0,20.,"HcalIsoSum, cone 0.3 (GeV)","Events","ELE_LOGY E1 P");

  // di-electron mass
  setBookIndex(200) ;
  h1_mee = ElectronDqmAnalyzerBase::bookH1(iBooker, "mesc","Tag ele Probe SC invariant mass", nbinmee, meemin, meemax,"m_{eSC} (GeV/c^{2})");
  h1_mee_os = ElectronDqmAnalyzerBase::bookH1(iBooker, "mee_os","ele pairs invariant mass, opposite sign", nbinmee, meemin, meemax,"m_{e^{+}e^{-}} (GeV/c^{2})");
