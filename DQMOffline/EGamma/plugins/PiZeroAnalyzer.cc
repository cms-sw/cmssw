...
void PiZeroAnalyzer::makePizero(const edm::EventSetup& es, const edm::Handle<EcalRecHitCollection> rhEB,  const edm::Handle<EcalRecHitCollection> rhEE ) {
  ...
  for (auto & seed : seeds) {
    ...
    float s4s9_[4];
    for(float & i : s4s9_)i= seed.energy();
    for(auto & j : RecHitsInWindow){
      ...
      if((((EBDetId)j.id()).ieta() == seed_id.ieta()-1 && seed_id.ieta()!=1 ) || ( seed_id.ieta()==1 && (((EBDetId)j.id()).ieta() == seed_id.ieta()-2))){
	...
	if(((EBDetId)j.id()).ieta() == seed_id.ieta()){
	  ...
	  if((((EBDetId)j.id()).ieta() == seed_id.ieta()+1 && seed_id.ieta()!=-1 ) || ( seed_id.ieta()==-1 && (((EBDetId)j.id()).ieta() == seed_id.ieta()+2))){
	    ...
	    cout<<" (EBDetId)RecHitsInWindow[j].id()).ieta() "<<((EBDetId)j.id()).ieta()<<" seed_id.ieta() "<<seed_id.ieta()<<endl;
