...
void ElectronGeneralAnalyzer::bookHistograms( DQMStore::IBooker & iBooker, edm::Run const &, edm::EventSetup const & )
 {
  h2_ele_beamSpotXvsY = ElectronDqmAnalyzerBase::bookH2(iBooker, "beamSpotXvsY","beam spot x vs y",100,-0.2,0.2,100,-0.2,0.2,"x (cm)","y (cm)") ;
  py_ele_nElectronsVsLs = ElectronDqmAnalyzerBase::bookP1(iBooker, "nElectronsVsLs","# gsf electrons vs LS",150,0.,1500.,0.,20.,"LS","<N_{ele}>") ;
  py_ele_nClustersVsLs = ElectronDqmAnalyzerBase::bookP1(iBooker, "nClustersVsLs","# clusters vs LS",150,0.,1500.,0.,100.,"LS","<N_{SC}>") ;
  py_ele_nGsfTracksVsLs = ElectronDqmAnalyzerBase::bookP1(iBooker, "nGsfTracksVsLs","# gsf tracks vs LS",150,0.,1500.,0.,20.,"LS","<N_{GSF tk}>") ;
  py_ele_nTracksVsLs = ElectronDqmAnalyzerBase::bookP1(iBooker, "nTracksVsLs","# tracks vs LS",150,0.,1500.,0.,100.,"LS","<N_{gen tk}>") ;
  py_ele_nVerticesVsLs = ElectronDqmAnalyzerBase::bookP1(iBooker, "nVerticesVsLs","# vertices vs LS",150,0.,1500.,0.,10.,"LS","<N_{vert}>") ;
  h1_ele_triggers = ElectronDqmAnalyzerBase::bookH1(iBooker, "triggers","hlt triggers",256,0.,256.,"HLT bit") ;
 }
