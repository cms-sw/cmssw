...
bool ZToMuMuGammaAnalyzer::basicMuonSelection ( const reco::Muon & mu) {
  ...
  if ( mu.innerTrack().isNonnull() ) {
    pixHits=mu.innerTrack()->getHitPattern().numberOfValidPixelHits(reco::HitPattern::TRACK_HITS);
    tkHits=mu.innerTrack()->getHitPattern().numberOfValidStripHits(reco::HitPattern::TRACK_HITS);
  }
...
bool ZToMuMuGammaAnalyzer::muonSelection ( const reco::Muon & mu,  const reco::BeamSpot& beamSpot) {
  ...
  if ( mu.track()->getHitPattern().numberOfValidPixelHits(reco::HitPattern::TRACK_HITS) <  validPixHits_ )     result=false;
  if ( mu.globalTrack()->getHitPattern().numberOfValidMuonHits(reco::HitPattern::TRACK_HITS) < validMuonHits_ ) result=false;
