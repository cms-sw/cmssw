...
//   int    chi2Bin = parameters_.getParameter<int>("chi2Bin");
  ...
  if (dbe_) {
    ...
    if(makeProfiles_){
      ...
      p_hOverEVsEt_[1]   = dbe_->bookProfile("hOverEVsEtBarrel","Avg H/E vs Et;E_{T} (GeV);H/E",etBin,etMin,etMax,hOverEBin,hOverEMin,hOverEMax);
      p_hOverEVsEt_[2]   = dbe_->bookProfile("hOverEVsEtEndcap","Avg H/E vs Et;E_{T} (GeV);H/E",etBin,etMin,etMax,hOverEBin,hOverEMin,hOverEMax);
      p_hOverEVsEta_[0]  = dbe_->bookProfile("hOverEVsEta","Avg H/E vs #eta;#eta;H/E",etaBin,etaMin,etaMax,hOverEBin,hOverEMin,hOverEMax);
      histname="pHoverEVsEta";
      p_hOverEVsEta_[0] = dbe_->bookProfile(histname+"All"," All photons H/E vs #eta: all Ecal ",  etaBin,etaMin, etaMax,100, 0.,0.1);
...
void ZToMuMuGammaAnalyzer::analyze( const edm::Event& e, const edm::EventSetup& esup )
{
  ...
  for( reco::MuonCollection::const_iterator  iMu = muonCollection.begin(); iMu != muonCollection.end(); iMu++) {
	...
	// double dr1 = deltaR(*iMu, aPho);
        //double dr2 = deltaR(*iMu2,aPho);
        ...
        nPho++;
        int iDet=0;
        if ( aPho->isEB() && splitHistosEBEE_ ) {
	  iDet=1; 
	  nPhoBarrel++;
	}
        if ( aPho->isEE() && splitHistosEBEE_ ) {
	  iDet=2;       
	  nPhoEndcap++;           
	}
	...
	if ( makeProfiles_ ) {
	  ...
	  p_sigmaIetaIetaVsEta_[iDet]  ->Fill(aPho->eta(),aPho->sigmaIetaIeta());
...
bool ZToMuMuGammaAnalyzer::photonSelection ( const reco::PhotonRef & pho) {
  ...
  if (pho->r9() <=0.9) {
    if (pho->isEB() && (pho->hadTowOverEm()>0.075 || pho->sigmaIetaIeta() > 0.014)) return false;
    if (pho->isEE() && (pho->hadTowOverEm()>0.075 || pho->sigmaIetaIeta() > 0.034)) return false;
    ///  remove after moriond    if (EtCorrEcalIso>4.0) return false;
    if (EtCorrHcalIso>4.0) return false;
    if (EtCorrTrkIso>4.0) return false;
    if ( pho->chargedHadronIso()  > 4 )  return false;
    return true;
  } else {
    if (pho->isEB() && (pho->hadTowOverEm()>0.082 || pho->sigmaIetaIeta() > 0.014)) return false;
    if (pho->isEE() && (pho->hadTowOverEm()>0.075 || pho->sigmaIetaIeta() > 0.034)) return false;
    /// remove after moriond if (EtCorrEcalIso>50.0) return false;
