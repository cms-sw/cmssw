#ifndef DQMOFFLINE_TRIGGER_MONELEMCONTAINER
#define DQMOFFLINE_TRIGGER_MONELEMCONTAINER
...
template<class T> void MonElemContainer<T>::fill(const T& obj,const EgHLTOffData& evtData,float weight)
{
  if(cut_==NULL || cut_->pass(obj,evtData)){
    for(size_t i=0;i<monElems_.size();i++) monElems_[i]->fill(obj,weight);
    for(size_t i=0;i<cutMonElems_.size();i++) cutMonElems_[i]->fill(obj,evtData,weight);
  }
}
