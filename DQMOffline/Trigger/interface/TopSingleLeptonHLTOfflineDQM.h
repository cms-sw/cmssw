...
namespace HLTOfflineDQMTopSingleLepton {
  ...
  inline void MonitorSingleLepton::triggerBinLabels(const std::string& channel,
  ...
  inline void MonitorSingleLepton::fill(const edm::Event& event,
                                        const edm::TriggerResults& triggerTable,
                                        const std::string& channel,
                                        const std::vector<std::string>& labels) const {
    for (unsigned int idx = 0; idx < labels.size(); ++idx) {
...
private:
  ...
  std::map<std::string, std::pair<edm::ParameterSet, std::unique_ptr<HLTOfflineDQMTopSingleLepton::MonitorSingleLepton>>>
      selection_;
