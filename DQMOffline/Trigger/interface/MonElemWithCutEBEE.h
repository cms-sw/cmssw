#ifndef DQMOFFLINE_TRIGGER_MONELEMWITHCUTEBEE
#define DQMOFFLINE_TRIGGER_MONELEMWITHCUTEBEE
...
template<class T,typename varType> void MonElemWithCutEBEE<T,varType>::fill(const T& obj,const EgHLTOffData& evtData,float weight)
{
  if(fabs(obj.detEta())<1.5) barrel_.fill(obj,evtData,weight);
  else endcap_.fill(obj,evtData,weight);
}
