#ifndef DQMOFFLINE_TRIGGER_MONELEMWITHCUT
#define DQMOFFLINE_TRIGGER_MONELEMWITHCUT
...
template<class T,typename varTypeX,typename varTypeY> 
void MonElemWithCut<T,varTypeX,varTypeY>::fill(const T& obj,const EgHLTOffData& evtData,float weight)
{
  if(cut_==NULL || cut_->pass(obj,evtData)) monElemMgr_->fill(obj,weight);
}
