...
template<class T> bool EgHLTDQMVarCut<T>::pass(const T& obj,const EgHLTOffData& evtData)const
{
...
public:
  ...
  bool pass(const T& obj,const EgHLTOffData& evtData)const{return (obj.*varFunc_)()>cutValue_;}
...
template<class T> bool EgMultiCut<T>::pass(const T& obj,const EgHLTOffData& evtData)const
{
  for(size_t i=0;i<cuts_.size();i++){
    if(!cuts_[i]->pass(obj,evtData)) return false;
  }
  return true;
...
template<class T> struct EgEleTrigCut : public EgHLTDQMCut<T> {
public:
...
public:
  ...
  bool pass(const T& obj,const EgHLTOffData& evtData)const;
  EgHLTDQMCut<T>* clone()const{return new EgEleTrigCut(*this);}
};

template<class T> bool EgEleTrigCut<T>::pass(const T& obj,const EgHLTOffData& evtData)const
{
...
template<class T> bool EgEvtTrigCut<T>::pass(const T& obj,const EgHLTOffData& evtData)const
{
  TrigCodes::TrigBitSet passMasked = bitsToPass_&evtData.evtTrigBits();
...
public:
  ...
  bool pass(const T& obj,const EgHLTOffData& evtData)const{return !cut_->pass(obj,evtData);}
...
public:
  ...
  bool pass(const T& obj,const EgHLTOffData& evtData)const{return obj.charge()==charge_;}
...
private:
  ...
  int tagCutCode_;
  int (EgHLTOffEle::*tagCutCodeFunc_)()const;
  float minMass_;
  float maxMass_;
public:
  EgTagProbeCut(int probeCutCode,int (T::*probeCutCodeFunc)()const,int tagCutCode,int (EgHLTOffEle::*tagCutCodeFunc)()const,float minMass=81.,float maxMass=101.):probeCutCode_(probeCutCode),probeCutCodeFunc_(probeCutCodeFunc),tagCutCode_(tagCutCode),tagCutCodeFunc_(tagCutCodeFunc),minMass_(minMass),maxMass_(maxMass){}
  ~EgTagProbeCut(){}
...
template<class T> bool EgTagProbeCut<T>::pass(const T& obj,const EgHLTOffData& evtData)const
{
  int nrTags=0;
...
template<class T> bool EgJetTagProbeCut<T>::pass(const T& obj,const EgHLTOffData& evtData)const
{
  int nrProbes=0;
  const std::vector<EgHLTOffEle>& eles = *evtData.eles;
  for(size_t eleNr=0;eleNr<eles.size();eleNr++){
  ...
  bool b2bJet=false;
  const std::vector<reco::CaloJet>& jets =*evtData.jets;
...
struct EgTrigTagProbeCut : public EgHLTDQMCut<EgHLTOffEle> {
private:
  TrigCodes::TrigBitSet bitsToPass_;
  int cutCode_;
  int (EgHLTOffEle::*cutCodeFunc_)()const;
  float minMass_;
  float maxMass_;
public:
  EgTrigTagProbeCut(TrigCodes::TrigBitSet bitsToPass,int cutCode,int (EgHLTOffEle::*cutCodeFunc)()const,float minMass=81.,float maxMass=101.):bitsToPass_(bitsToPass),cutCode_(cutCode),cutCodeFunc_(cutCodeFunc),minMass_(minMass),maxMass_(maxMass){}
  ~EgTrigTagProbeCut(){}
  
  bool pass(const EgHLTOffEle& ele,const EgHLTOffData& evtData)const;
  EgHLTDQMCut<EgHLTOffEle>* clone()const{return new EgTrigTagProbeCut(*this);} 
