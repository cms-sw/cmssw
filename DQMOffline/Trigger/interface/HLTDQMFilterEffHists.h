...
edm::ParameterSetDescription HLTDQMFilterEffHists<ObjType>::makePSetDescriptionHistConfigs()
{
  ...
  auto histDescCases = 
    "1D" >> 
    (edm::ParameterDescription<std::vector<double> >("binLowEdges",std::vector<double>(),true) and
     edm::ParameterDescription<std::string>("nameSuffex","",true) and
     edm::ParameterDescription<std::string>("vsVar","",true)) or
    "2D" >>
    (edm::ParameterDescription<std::vector<double> >("xBinLowEdges",std::vector<double>(),true) and
...
template <typename ObjType>
void HLTDQMFilterEffHists<ObjType>::bookHists(DQMStore::IBooker& iBooker,const std::vector<edm::ParameterSet>& histConfigs)
{
...
void HLTDQMFilterEffHists<ObjType>::book1D(DQMStore::IBooker& iBooker,const edm::ParameterSet& histConfig)
{
  ...
  if(!vsVarFunc) {
    throw cms::Exception("ConfigError")<<" vsVar "<<vsVar<<" is giving null ptr (likely empty) in "<<__FILE__<<","<<__LINE__<<std::endl;
  }
...
void HLTDQMFilterEffHists<ObjType>::book2D(DQMStore::IBooker& iBooker,const edm::ParameterSet& histConfig)
{
  ...
  if(!xVarFunc || !yVarFunc) {
    throw cms::Exception("ConfigError")<<" xVar "<<xVar<<" or yVar "<<yVar<<" is giving null ptr (likely empty str passed)"<<std::endl;
  }
  VarRangeCutColl<ObjType> rangeCuts(histConfig.getParameter<std::vector<edm::ParameterSet> >("rangeCuts"));
  

  //really? really no MonitorElement::getTH2...sigh
  hist = std::make_unique<HLTDQMHist2D<ObjType,float> >(static_cast<TH2*>(mePass->getTH1()),xVar,yVar,xVarFunc,yVarFunc,rangeCuts); 
  histsPass_.emplace_back(std::move(hist));
  ...
  hist = std::make_unique<HLTDQMHist2D<ObjType,float> >(static_cast<TH2*>(meTot->getTH1()),xVar,yVar,xVarFunc,yVarFunc,rangeCuts); 
  histsTot_.emplace_back(std::move(hist));
}
