...
namespace egHLT {
  ...
  template <typename PROD>
  int debug::listAllProducts(const edm::Event& iEvent, const char* moduleName) {
    ...
    for (size_t i = 0; i < products.size(); i++) {
      ...
      std::cout << "for product " << i + 1 << "/" << products.size() << " " << products[i].provenance()->moduleLabel()
                << " " << products[i].provenance()->moduleName() << std::endl;
    }
