...
void LepHTMonitor::dqmBeginRun(const edm::Run &run, const edm::EventSetup &e){
  ...
  if(!pathFound){
    LogDebug("LepHTMonitor") << "Path not found: " << triggerPath_ << '\n';
    return;
  }
...
void LepHTMonitor::analyze(const edm::Event &e, const edm::EventSetup &eSetup){
  ...
  if(hltresults.isValid()){
	...
	//edm::LogWarning("LepHTMonitor")<<" Trig name is "<<name<<"\n";
  ...
  if(Contains(triggerPath_,"DoubleMu")) {lep_pt = min_mu_pt; lep_eta = trailing_ele_eta;}
  if(Contains(triggerPath_,"DoubleEle")) {lep_pt = min_ele_pt; lep_eta = trailing_mu_eta;}
  ...
  if(hasFiredLeptonAuxiliary || triggerPathLeptonAuxiliary_=="" || !e.isRealData()){
    ...
    if(nleps_cut && ht_plateau && met_plateau && csv_plateau
  ...
  if(hasFiredAuxiliary || triggerPathAuxiliary_=="" || !e.isRealData()){
    ...
    if(nleps_cut && lep_plateau && met_plateau && csv_plateau){
    ...
    if(nleps_cut && lep_plateau && ht_plateau && csv_plateau){
    ...
    if(nleps_cut && lep_plateau && ht_plateau && met_plateau){
