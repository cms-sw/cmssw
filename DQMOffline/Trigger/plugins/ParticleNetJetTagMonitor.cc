...
ParticleNetJetTagMonitor::ParticleNetJetTagMonitor(const edm::ParameterSet& iConfig):
  ...
  jetPNETScoreForHTandBTagToken_(mayConsume<reco::JetTagCollection>(iConfig.getParameter<edm::InputTag>("jetPNETScoreForHTandBTag"))),
  ...
  numGenericTriggerEvent_(new GenericTriggerEventFlag(iConfig.getParameter<edm::ParameterSet>("numGenericTriggerEvent"), consumesCollector(), *this)),
  denGenericTriggerEvent_(new GenericTriggerEventFlag(iConfig.getParameter<edm::ParameterSet>("denGenericTriggerEvent"), consumesCollector(), *this)),
  jetSelection_(iConfig.getParameter<std::string>("jetSelection")),
  jetSelectionForHTandBTag_(iConfig.existsAs<std::string>("jetSelectionForHTandBTag") ? iConfig.getParameter<std::string>("jetSelectionForHTandBTag") : ""),
...
void ParticleNetJetTagMonitor::bookHistograms(DQMStore::IBooker& ibooker, edm::Run const& iRun, edm::EventSetup const& iSetup) {
  ...
  denHLTPathsAreValid_ = (denGenericTriggerEvent_ && denGenericTriggerEvent_->on() && denGenericTriggerEvent_->allHLTPathsAreValid());
  numHLTPathsAreValid_ = (numGenericTriggerEvent_ && numGenericTriggerEvent_->on() && numGenericTriggerEvent_->allHLTPathsAreValid());

  if (requireValidHLTPaths_ && (!denHLTPathsAreValid_)) return;
  if (requireValidHLTPaths_ && (!numHLTPathsAreValid_)) return;
  ...
  if(!jet1PNETscoreTransBinning.empty()){
    bookME(ibooker,jet1_pnetscore_trans,"jet1_pnetscore_trans","jet lead PNET-score transformed", jet1PNETscoreTransBinning);
    setMETitle(jet1_pnetscore_trans, "Lead atanh(PNET-score)", "Entries");
  }
  if(!jet2PNETscoreTransBinning.empty()){
    bookME(ibooker,jet2_pnetscore_trans,"jet2_pnetscore_trans","jet trail PNET-score transformed", jet2PNETscoreTransBinning);
  ...
  if(!jet1PNETscoreTransBinning.empty() and !jet2PNETscoreTransBinning.empty()){
    bookME(ibooker,mean_2j_pnetscore_trans,"mean_2j_pnetscore_trans","mean 2jet PNET-score transformed", jet1PNETscoreTransBinning);
  ...
  if(!jet1PtBinning2d.empty() and !jet1PNETscoreBinning2d.empty()){
    bookME(ibooker,jet1_pt_pnetscore1,"jet1_pt_pnetscore1","jet1 p{T} vs lead PNET-score",jet1PtBinning2d,jet1PNETscoreBinning2d);
    setMETitle(jet1_pt_pnetscore1, "p_{T}(j1)", "Lead PNET-score");
  }
  if(!jet1PtBinning2d.empty() and !jet2PNETscoreBinning2d.empty()){
    bookME(ibooker,jet1_pt_pnetscore2,"jet1_pt_pnetscore2","jet1 p_{T} vs trail PNET-score",jet1PtBinning2d,jet2PNETscoreBinning2d);
  ...
  if(!jet1PtBinning2d.empty() and !jet1PNETscoreBinning2d.empty() and !jet2PNETscoreBinning2d.empty()){
    bookME(ibooker,jet1_pt_mean2pnetscore,"jet1_pt_mean2pnetscore","jet1 p_{T} vs mean 2jet PNET-score",jet1PtBinning2d,jet1PNETscoreBinning2d);
    setMETitle(jet1_pt_mean2pnetscore, "p_{T}(j1)", "Mean(PNET-score)");
  }

  if(!jet2PtBinning2d.empty() and !jet1PNETscoreBinning2d.empty()){
    bookME(ibooker,jet2_pt_pnetscore1,"jet2_pt_pnetscore1","jet2 p_{T} vs lead PNET-score",jet2PtBinning2d,jet1PNETscoreBinning2d);
    setMETitle(jet2_pt_pnetscore1, "p_{T}(j2)", "Lead PNET-score");
  }
  if(!jet2PtBinning2d.empty() and !jet2PNETscoreBinning2d.empty()){
    bookME(ibooker,jet2_pt_pnetscore2,"jet2_pt_pnetscore2","jet2 p_{T} vs trail PNET-score",jet2PtBinning2d,jet2PNETscoreBinning2d);
  ...
  if(!jet2PtBinning2d.empty() and !jet1PNETscoreBinning2d.empty() and !jet2PNETscoreBinning2d.empty()){
    bookME(ibooker,jet2_pt_mean2pnetscore,"jet2_pt_mean2pnetscore","jet2 p_{T} vs mean 2jet PNET-score",jet2PtBinning2d,jet1PNETscoreBinning2d);
    setMETitle(jet2_pt_mean2pnetscore, "p_{T}(j2)", "Mean(PNET-score)");
  }

  if(!jet1PtBinning2d.empty() and !jet1PNETscoreTransBinning2d.empty()){
    bookME(ibooker,jet1_pt_pnetscore1_trans,"jet1_pt_pnetscore1_trans","jet1 p_{T} vs lead PNET-score transformed",jet1PtBinning2d,jet1PNETscoreTransBinning2d);
  ...
  if(!jet1PtBinning2d.empty() and !jet2PNETscoreTransBinning2d.empty()){
    bookME(ibooker,jet1_pt_pnetscore2_trans,"jet1_pt_pnetscore2_trans","jet1 p_{T} vs trail PNET-score transformed",jet1PtBinning2d,jet2PNETscoreTransBinning2d);
  ...
  if(!jet1PtBinning2d.empty() and !jet1PNETscoreTransBinning2d.empty() and !jet2PNETscoreTransBinning2d.empty()){
    bookME(ibooker,jet1_pt_mean2pnetscore_trans,"jet1_pt_mean2pnetscore_trans","jet1 p_{T} vs mean 2jet PNET-score transformed",jet1PtBinning2d,jet1PNETscoreBinning2d);
  ...
  if(!jet2PtBinning2d.empty() and !jet1PNETscoreTransBinning2d.empty()){
    bookME(ibooker,jet2_pt_pnetscore1_trans,"jet2_pt_pnetscore1_trans","jet2 p_{T} vs lead PNET-score transformed",jet2PtBinning2d,jet1PNETscoreTransBinning2d);
  ...
  if(!jet2PtBinning2d.empty() and !jet2PNETscoreTransBinning2d.empty()){
    bookME(ibooker,jet2_pt_pnetscore2_trans,"jet2_pt_pnetscore2_trans","jet2 p_{T} vs trail PNET-score transformed",jet2PtBinning2d,jet2PNETscoreTransBinning2d);
  ...
  if(!jet2PtBinning2d.empty() and !jet1PNETscoreTransBinning2d.empty() and !jet2PNETscoreTransBinning2d.empty()){
    bookME(ibooker,jet2_pt_mean2pnetscore_trans,"jet2_pt_mean2pnetscore_trans","jet2 p_{T} vs mean 2jet PNET-score transformed",jet2PtBinning2d,jet1PNETscoreBinning2d);
...
void ParticleNetJetTagMonitor::analyze(edm::Event const& iEvent, edm::EventSetup const& iSetup) {
  ...
  for (auto const & v : *primaryVertices) {                                                                                                                                                           
    if (not vertexSelection_(v)) continue;
    pv = &v;
    break;
  } 
  ...
  for (auto const & m : *muonHandle) { 
    if(tagMuonSelection_(m) and
       std::fabs(m.muonBestTrack()->dxy(pv->position())) <= maxLeptonDxyCut_ and
       std::fabs(m.muonBestTrack()->dz(pv->position()))  <= maxLeptonDzCut_)
      tagMuons.push_back(m);
    if(vetoMuonSelection_(m) and 
       std::fabs(m.muonBestTrack()->dxy(pv->position())) <= maxLeptonDxyCut_ and
       std::fabs(m.muonBestTrack()->dz(pv->position()))  <= maxLeptonDzCut_)
      vetoMuons.push_back(m);
  }

  if(ntagmuons_>=0 and int(tagMuons.size()) != ntagmuons_) return;
  selectionFlowStatus++;                                                                                                                                                                             
  selectionFlow->Fill(selectionFlowStatus);

  if(nvetomuons_>= 0 and int(vetoMuons.size()) != nvetomuons_) return;
  selectionFlowStatus++;                                                                                                                                                                             
  selectionFlow->Fill(selectionFlowStatus);
  ...
  for (size_t index = 0; index < electronHandle->size(); index++) {
    ...
    if(tagElectronSelection_(e) and
       (*tagEleIDHandle)[reco::GsfElectronRef(electronHandle,index)] and
    ...
    if(vetoElectronSelection_(e) and 
       (*vetoEleIDHandle)[reco::GsfElectronRef(electronHandle,index)] and
       std::fabs(e.gsfTrack()->dxy(pv->position())) <= maxLeptonDxyCut_ and
       std::fabs(e.gsfTrack()->dz(pv->position()))  <= maxLeptonDzCut_)
      vetoElectrons.push_back(e);
  }
  if(ntagelectrons_ >= 0 and int(tagElectrons.size()) != ntagelectrons_) return;
  selectionFlowStatus++;       
  selectionFlow->Fill(selectionFlowStatus);

  if(nvetoelectrons_ >= 0 and int(vetoElectrons.size()) != nvetoelectrons_) return;
  selectionFlowStatus++;                                                                                                                                                                             
  selectionFlow->Fill(selectionFlowStatus);

  // Overall number of leptons for flavor composition
  if(ntagleptons_ >= 0 and int(tagElectrons.size()+tagMuons.size()) != ntagleptons_) return;
  selectionFlowStatus++;                                                                                                                                                                            
  selectionFlow->Fill(selectionFlowStatus);

  if(nvetoleptons_ >= 0 and int(vetoElectrons.size()+vetoMuons.size()) != nvetoleptons_) return;
  selectionFlowStatus++;                                                                                                                                                                            
  selectionFlow->Fill(selectionFlowStatus);
  ...
  if(nemupairs_ >= 0 and int(emuPairs.size()) != nemupairs_) return;
  selectionFlowStatus++;                                                                                                                                                                            
  selectionFlow->Fill(selectionFlowStatus);
  ...
  for (auto const & j : *jetHandle) {     
    ...
    if(jetSoftDropMassHandle.isValid()){
      ...
      if(massSD < minSoftDropMassCut_ or massSD > maxSoftDropMassCut_) continue;
    }
    ...
    if(minDR_jm < lepJetDeltaRmin_) continue;
    float minDR_je = 1000;
    for(size_t iel = 0; iel < vetoElectrons.size(); iel++){
      float dR = reco::deltaR(vetoElectrons.at(iel).p4(),jet.p4());
      if(dR < minDR_je) minDR_je = dR;
    }
    if(minDR_je < lepJetDeltaRmin_) continue;
    // Compute HT
  ...
  if(njets_ >= 0 and int(selectedJets.size()) < njets_) return;
  selectionFlowStatus++;                                                                                                                                                                            
  selectionFlow->Fill(selectionFlowStatus);

  // count number of jets over minPNETScoreCut
  if(std::count_if(jetPNETScoreValues.begin(),jetPNETScoreValues.end(),[&](float score){ return score > minPNETScoreCut_;}) < njets_) return;
  selectionFlowStatus++;                                                                                                                                                                            
  selectionFlow->Fill(selectionFlowStatus);
  ...
  std::sort(jetPNETScoreSortedIndices.begin(), jetPNETScoreSortedIndices.end(), [&](const size_t & i1, const size_t & i2) {
      return jetPNETScoreValues[i1] > jetPNETScoreValues[i2];
    });
  ...
  if(requireHLTOfflineJetMatching_){
    ...
    std::sort(jetPNETScoreSortedIndicesHLT.begin(), jetPNETScoreSortedIndicesHLT.end(), [&](const size_t& i1, const size_t& i2) {
	return jetPNETScoreValuesHLT[i1] > jetPNETScoreValuesHLT[i2];
      });
	...
	if(std::find(matched_obj.begin(),matched_obj.end(),jhlt) != matched_obj.end()) continue;
	float dR = reco::deltaR(selectedJets[jetPNETScoreSortedIndices.at(jreco)].p4(),jetHLTRefs.at(jetPNETScoreSortedIndicesHLT.at(jhlt))->p4());
    ...
    if(matched_obj.size() != ntrigobjecttomatch_) return;
  }
  selectionFlowStatus++;                                                                                                                                                                           
  selectionFlow->Fill(selectionFlowStatus);
  ...
  if(muon_pt.numerator!=nullptr) 
    muon_pt.fill(trg_passed,(tagMuons.size()>0) ? tagMuons.front().pt() : 0);
  if(muon_eta.numerator!=nullptr) 
    muon_eta.fill(trg_passed,(tagMuons.size()>0) ? tagMuons.front().eta() : 0);
  if(electron_pt.numerator!=nullptr) 
    electron_pt.fill(trg_passed,(tagElectrons.size()>0) ? tagElectrons.front().pt() : -100);
  if(electron_eta.numerator!=nullptr) 
    electron_eta.fill(trg_passed,(tagElectrons.size()>0) ? tagElectrons.front().eta() : -100);
  if(dilepton_pt.numerator!=nullptr) 
    dilepton_pt.fill(trg_passed,(emuPairs.size()>0) ? emuPairs.front().pt() : 0);
  if(dilepton_mass.numerator!=nullptr) 
    dilepton_mass.fill(trg_passed,(emuPairs.size()>0) ? emuPairs.front().mass() : 0);

  if(jet1_pt.numerator!=nullptr) 
    jet1_pt.fill(trg_passed,(selectedJets.size()>0) ? selectedJets.at(jetPtSortedIndices.at(0)).pt() : 0);
  ...
  if(jet1_eta.numerator!=nullptr) 
    jet1_eta.fill(trg_passed,(selectedJets.size()>0) ? selectedJets.at(jetPtSortedIndices.at(0)).eta() : -100);
  ...
  if(jet1_pnetscore.numerator!=nullptr) 
    jet1_pnetscore.fill(trg_passed,(selectedJets.size()>0) ? jetPNETScoreValues.at(jetPNETScoreSortedIndices.at(0)) : -1);
  if(jet2_pnetscore.numerator!=nullptr) 
    jet2_pnetscore.fill(trg_passed,(selectedJets.size()>1) ? jetPNETScoreValues.at(jetPNETScoreSortedIndices.at(1)) : -1);
  if(jet1_pnetscore_trans.numerator!=nullptr) 
    jet1_pnetscore_trans.fill(trg_passed,(selectedJets.size()>0) ? atanh(jetPNETScoreValues.at(jetPNETScoreSortedIndices.at(0))) : -1);
  if(jet2_pnetscore_trans.numerator!=nullptr) 
    jet2_pnetscore_trans.fill(trg_passed,(selectedJets.size()>1) ? atanh(jetPNETScoreValues.at(jetPNETScoreSortedIndices.at(1))) : -1);
  if(mean_2j_pnetscore.numerator!=nullptr) 
    mean_2j_pnetscore.fill(trg_passed,(selectedJets.size()>1) ? (jetPNETScoreValues.at(jetPNETScoreSortedIndices.at(0))+jetPNETScoreValues.at(jetPNETScoreSortedIndices.at(1)))/2. : -1);
  if(mean_2j_pnetscore_trans.numerator!=nullptr) 
    mean_2j_pnetscore_trans.fill(trg_passed,(selectedJets.size()>1) ? atanh((jetPNETScoreValues.at(jetPNETScoreSortedIndices.at(0))+jetPNETScoreValues.at(jetPNETScoreSortedIndices.at(1)))/2.) : -1);

  if(jet1_pt_eta.numerator!=nullptr) 
    jet1_pt_eta.fill(trg_passed,(selectedJets.size()>0) ? selectedJets.at(jetPtSortedIndices.at(0)).pt() : 0, (selectedJets.size()>0) ? selectedJets.at(jetPtSortedIndices.at(0)).eta() : -1);
  if(jet2_pt_eta.numerator!=nullptr) 
    jet2_pt_eta.fill(trg_passed,(selectedJets.size()>1) ? selectedJets.at(jetPtSortedIndices.at(1)).pt() : 0, (selectedJets.size()>1) ? selectedJets.at(jetPtSortedIndices.at(1)).eta() : -1);

  if(jet1_pt_pnetscore1.numerator!=nullptr) 
    jet1_pt_pnetscore1.fill(trg_passed,(selectedJets.size()>0) ? selectedJets.at(jetPtSortedIndices.at(0)).pt() : 0, (selectedJets.size()>0) ? jetPNETScoreValues.at(jetPNETScoreSortedIndices.at(0)) : -1);
  if(jet2_pt_pnetscore1.numerator!=nullptr) 
    jet2_pt_pnetscore1.fill(trg_passed,(selectedJets.size()>1) ? selectedJets.at(jetPtSortedIndices.at(1)).pt() : 0, (selectedJets.size()>1) ? jetPNETScoreValues.at(jetPNETScoreSortedIndices.at(0)) : -1);
  if(jet1_pt_pnetscore2.numerator!=nullptr) 
    jet1_pt_pnetscore2.fill(trg_passed,(selectedJets.size()>1) ? selectedJets.at(jetPtSortedIndices.at(0)).pt() : 0, (selectedJets.size()>1) ? jetPNETScoreValues.at(jetPNETScoreSortedIndices.at(1)) : -1);
  if(jet2_pt_pnetscore2.numerator!=nullptr) 
    jet2_pt_pnetscore2.fill(trg_passed,(selectedJets.size()>1) ? selectedJets.at(jetPtSortedIndices.at(1)).pt() : 0, (selectedJets.size()>1) ? jetPNETScoreValues.at(jetPNETScoreSortedIndices.at(1)) : -1);
  
  if(jet1_pt_pnetscore1_trans.numerator!=nullptr) 
    jet1_pt_pnetscore1_trans.fill(trg_passed,(selectedJets.size()>0) ? selectedJets.at(jetPtSortedIndices.at(0)).pt() : 0, (selectedJets.size()>0) ? atanh(jetPNETScoreValues.at(jetPNETScoreSortedIndices.at(0))) : -1);
  if(jet2_pt_pnetscore1_trans.numerator!=nullptr) 
    jet2_pt_pnetscore1_trans.fill(trg_passed,(selectedJets.size()>1) ? selectedJets.at(jetPtSortedIndices.at(1)).pt() : 0, (selectedJets.size()>1) ? atanh(jetPNETScoreValues.at(jetPNETScoreSortedIndices.at(0))) : -1);
  if(jet1_pt_pnetscore2_trans.numerator!=nullptr) 
    jet1_pt_pnetscore2_trans.fill(trg_passed,(selectedJets.size()>1) ? selectedJets.at(jetPtSortedIndices.at(0)).pt() : 0, (selectedJets.size()>1) ? atanh(jetPNETScoreValues.at(jetPNETScoreSortedIndices.at(1))) : -1);
  if(jet2_pt_pnetscore2_trans.numerator!=nullptr) 
    jet2_pt_pnetscore2_trans.fill(trg_passed,(selectedJets.size()>1) ? selectedJets.at(jetPtSortedIndices.at(1)).pt() : 0, (selectedJets.size()>1) ? atanh(jetPNETScoreValues.at(jetPNETScoreSortedIndices.at(1))) : -1);

  if(jet1_pt_mean2pnetscore.numerator!=nullptr) 
    jet1_pt_mean2pnetscore.fill(trg_passed,(selectedJets.size()>1) ? selectedJets.at(jetPtSortedIndices.at(0)).pt() : 0, (selectedJets.size()>1) ? (jetPNETScoreValues.at(jetPNETScoreSortedIndices.at(0))+jetPNETScoreValues.at(jetPNETScoreSortedIndices.at(1)))/2 : -1);
  if(jet2_pt_mean2pnetscore.numerator!=nullptr) 
    jet2_pt_mean2pnetscore.fill(trg_passed,(selectedJets.size()>1) ? selectedJets.at(jetPtSortedIndices.at(1)).pt() : 0, (selectedJets.size()>1) ? atanh((jetPNETScoreValues.at(jetPNETScoreSortedIndices.at(0))+jetPNETScoreValues.at(jetPNETScoreSortedIndices.at(1)))/2) : -1);

  if(jet1_pt_mean2pnetscore_trans.numerator!=nullptr)
    jet1_pt_mean2pnetscore_trans.fill(trg_passed,(selectedJets.size()>1) ? selectedJets.at(jetPtSortedIndices.at(0)).pt() : 0, (selectedJets.size()>1) ? atanh((jetPNETScoreValues.at(jetPNETScoreSortedIndices.at(0))+jetPNETScoreValues.at(jetPNETScoreSortedIndices.at(1)))/2) : -1);
  if(jet2_pt_mean2pnetscore_trans.numerator!=nullptr)
    jet2_pt_mean2pnetscore_trans.fill(trg_passed,(selectedJets.size()>1) ? selectedJets.at(jetPtSortedIndices.at(1)).pt() : 0, (selectedJets.size()>1) ? atanh((jetPNETScoreValues.at(jetPNETScoreSortedIndices.at(0))+jetPNETScoreValues.at(jetPNETScoreSortedIndices.at(1)))/2) : -1);  
}
...
void ParticleNetJetTagMonitor::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  desc.add<edm::InputTag>("tagElectronID", edm::InputTag("egmGsfElectronIDsForDQM:cutBasedElectronID-Fall17-94X-V2-tight"));
  desc.add<edm::InputTag>("vetoElectronID", edm::InputTag("egmGsfElectronIDsForDQM:cutBasedElectronID-Fall17-94X-V2-loose"));
  ...
  desc.add<std::string>("tagMuonSelection", "pt > 25 && abs(eta) < 2.4 && passed(CutBasedIdTight) && passed(PFIsoTight)");
  desc.add<std::string>("tagElectronSelection", "pt > 20 && abs(eta) < 2.5");
  desc.add<std::string>("vetoMuonSelection", "pt > 10 && abs(eta) < 2.4 && passed(CutBasedIdLoose) && passed(PFIsoLoose)");
