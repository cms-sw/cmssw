    ...
    for (auto name : triggerNames.triggerNames()) {
      ...
      for (auto const& triggerPath : triggerPaths) {
        if (TString(name.c_str()).Contains(TString(triggerPath), TString::kIgnoreCase) &&
            TString(name.c_str()).Contains(TString("ele"), TString::kIgnoreCase) &&
            TString(name.c_str()).Contains(TString("mu"), TString::kIgnoreCase))
          elecmu = true;
        else {
          if (TString(name.c_str()).Contains(TString(triggerPath), TString::kIgnoreCase) &&
              TString(name.c_str()).Contains(TString("ele"), TString::kIgnoreCase))
      ...
      if (dielec) {
        ...
        if (dielec && isoElecs.size() > 1) {
          ...
          if ((lowerEdge_ == -1. && upperEdge_ == -1.) || (lowerEdge_ < mass && mass < upperEdge_)) {
            // fill plots for trigger monitoring
            if (!triggerTable_.isUninitialized())
              fill(event, *triggerTable, "diElec", diElecPaths_);
    ...
    for (unsigned int i = 0; i < triggerNames.triggerNames().size(); ++i) {
      ...
      if (kElec > 0 && kMuon < 1 && !isoElecs.empty()) {
        ...
        for (unsigned int inde = 0; inde < isoElecs.size(); inde++) {
          if (nElectrons > 0)
            deltar1 = deltaR(*electronRefs_[0], *isoElecs[inde]);
          if (nElectrons > 1)
            deltar2 = deltaR(*electronRefs_[1], *isoElecs[inde]);
        ...
        if (nElectrons > 0 && l1IndMatched < 500)
          fill("leptDeltaREta_", isoElecs[l1IndMatched]->eta(), l1DeltaRMin);
        if (nElectrons > 1 && l2IndMatched < 500)
          fill("leptDeltaREta_", isoElecs[l2IndMatched]->eta(), l2DeltaRMin);
        if (l1DeltaRMin < DRMIN) {
          l1Matched = true;
          fill("matchingMon_", 0.5);
          fill("leptResolution_",
               fabs(isoElecs[l1IndMatched]->pt() - electronRefs_[0]->pt()) / isoElecs[l1IndMatched]->pt());
        }
        if (l2DeltaRMin < DRMIN) {
          l2Matched = true;
          fill("matchingMon_", 1.5);
          fill("leptResolution_",
               fabs(isoElecs[l2IndMatched]->pt() - electronRefs_[1]->pt()) / isoElecs[l2IndMatched]->pt());
        }
      }
      ...
      if (kMuon > 0 && kElec < 1 && !isoMuons.empty()) {
        ...
        for (unsigned int l = 0; l < nMuons; l++) {
          ...
          for (auto& myMuonRef : myMuonRefs) {
            if (fabs((myMuonRef->pt() - muonRefs_[l]->pt()) / muonRefs_[l]->pt()) < 1e-5)
              isNew = false;
          }
          if (isNew)
            myMuonRefs.push_back(muonRefs_[l]);
        }
        ...
        for (unsigned int indm = 0; indm < isoMuons.size(); indm++) {
          if (nMyMuons > 0)
            deltar1 = deltaR(*myMuonRefs[0], *isoMuons[indm]);
          if (nMyMuons > 1)
            deltar2 = deltaR(*myMuonRefs[1], *isoMuons[indm]);
          if (nMyMuons > 2)
            deltar3 = deltaR(*myMuonRefs[2], *isoMuons[indm]);
          if (nMyMuons > 0 && (nMyMuons < 1 || deltar1 < deltar2) && (nMyMuons < 2 || deltar1 < deltar3) &&
              deltar1 < l1DeltaRMin) {
            l1DeltaRMin = deltar1;
            l1IndMatched = indm;
          }
        ...
        if (nMyMuons > 0 && l1IndMatched < 500)
          fill("leptDeltaREta_", isoMuons[l1IndMatched]->eta(), l1DeltaRMin);
        if (nMyMuons > 1 && l2IndMatched < 500)
          fill("leptDeltaREta_", isoMuons[l2IndMatched]->eta(), l2DeltaRMin);
        if (nMyMuons > 2 && l3IndMatched < 500)
          fill("leptDeltaREta_", isoMuons[l3IndMatched]->eta(), l3DeltaRMin);
        if (l1DeltaRMin < DRMIN) {
          l1Matched = true;
          fill("matchingMon_", 0.5);
          fill("leptResolution_",
               fabs(isoMuons[l1IndMatched]->pt() - myMuonRefs[0]->pt()) / isoMuons[l1IndMatched]->pt());
          if (l2DeltaRMin < DRMIN) {
            l2Matched = true;
            fill("matchingMon_", 1.5);
            fill("leptResolution_",
                 fabs(isoMuons[l2IndMatched]->pt() - myMuonRefs[1]->pt()) / isoMuons[l2IndMatched]->pt());
          } else if (l3DeltaRMin < DRMIN) {
            l2Matched = true;
            fill("matchingMon_", 1.5);
            fill("leptResolution_",
                 fabs(isoMuons[l3IndMatched]->pt() - myMuonRefs[2]->pt()) / isoMuons[l3IndMatched]->pt());
          }
        } else {
          if (l2DeltaRMin < DRMIN) {
            l1Matched = true;
      ...
      if (kElec > 0 && kMuon > 0 && !isoElecs.empty()) {
        ...
        for (unsigned int inde = 0; inde < isoElecs.size(); inde++) {
          if (nElectrons > 0)
            deltar = deltaR(*electronRefs_[0], *isoElecs[inde]);
      ...
      if (kElec > 0 && kMuon > 0 && !isoMuons.empty()) {
        ...
        for (unsigned int indm = 0; indm < isoMuons.size(); indm++) {
          if (nMuons > 0)
            deltar = deltaR(*muonRefs_[0], *isoMuons[indm]);
          if (deltar < l2DeltaRMin) {
            l2DeltaRMin = deltar;
            l2IndMatched = indm;
          }
        }
        if (nMuons > 0 && l2IndMatched < 500)
          fill("leptDeltaREta_", isoMuons[l2IndMatched]->eta(), l2DeltaRMin);
        if (l2DeltaRMin < DRMIN) {
          l2Matched = true;
          fill("matchingMon_", 1.5);
          fill("leptResolution_",
               fabs(isoMuons[l2IndMatched]->pt() - muonRefs_[0]->pt()) / isoMuons[l2IndMatched]->pt());
        }
      }
...
TopDiLeptonHLTOfflineDQM::TopDiLeptonHLTOfflineDQM(const edm::ParameterSet& cfg)
  ...
  for (auto& i : sel) {
    ...
    selection_[selectionStep(selectionOrder_.back())] = std::make_pair(
        i,
        std::make_unique<HLTOfflineDQMTopDiLepton::MonitorDiLepton>(selectionStep(selectionOrder_.back()).c_str(),
...
void TopDiLeptonHLTOfflineDQM::dqmBeginRun(const edm::Run& iRun, const edm::EventSetup& iSetup) {
  using namespace std;
  using namespace edm;
...
void TopDiLeptonHLTOfflineDQM::analyze(const edm::Event& event, const edm::EventSetup& setup) {
  if (!triggerTable_.isUninitialized()) {
    edm::Handle<edm::TriggerResults> triggerTable;
    if (!event.getByToken(triggerTable_, triggerTable))
      return;
...
void TopDiLeptonHLTOfflineDQM::bookHistograms(DQMStore::IBooker& i, edm::Run const&, edm::EventSetup const&) {
