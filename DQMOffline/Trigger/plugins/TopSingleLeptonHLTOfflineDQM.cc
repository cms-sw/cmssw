...
namespace HLTOfflineDQMTopSingleLepton {
  ...
  MonitorSingleLepton::MonitorSingleLepton(const char* label, const edm::ParameterSet& cfg, edm::ConsumesCollector&& iC)
    ...
    if (cfg.existsAs<edm::ParameterSet>("elecExtras")) {
      ...
      if (elecExtras.existsAs<std::string>("isolation")) {
        elecIso_ = std::make_unique<StringCutObjectSelector<reco::GsfElectron>>(
    ...
    if (cfg.existsAs<edm::ParameterSet>("jetExtras")) {
      ...
      if (includeBTag_) {
        edm::ParameterSet btagEff = jetExtras.getParameter<edm::ParameterSet>("jetBTaggers")
                                        .getParameter<edm::ParameterSet>("trackCountingEff");
        btagEff_ = iC.consumes<reco::JetTagCollection>(btagEff.getParameter<edm::InputTag>("label"));
        ...
        edm::ParameterSet btagVtx =
            jetExtras.getParameter<edm::ParameterSet>("jetBTaggers").getParameter<edm::ParameterSet>("secondaryVertex");
        btagVtx_ = iC.consumes<reco::JetTagCollection>(btagVtx.getParameter<edm::InputTag>("label"));
    ...
    triggerSummaryTokenRAW =
        iC.consumes<trigger::TriggerEventWithRefs>(edm::InputTag("hltTriggerSummaryRAW", "", processName_));
    triggerSummaryTokenAOD =
        iC.consumes<trigger::TriggerEventWithRefs>(edm::InputTag("hltTriggerSummaryAOD", "", processName_));
  }

  void MonitorSingleLepton::book(DQMStore::IBooker& store_) {
    //set up the current directory path
    std::string current(folder_);
    current += label_;
    ...
    hists_["leptDeltaREta_"] =
        store_.book2D("DeltaRMinEtaLepton", "#Delta R_{min}(leptons) wrt #eta", 30, -3, 3, 10, 0., 0.1);
  ...
  void MonitorSingleLepton::fill(const edm::Event& event,
    ...
    if (!event.getByToken(pvs_, pvs)) {
      edm::LogWarning("TopSingleLeptonHLTOfflineDQM") << "Vertex collection not found \n";
      return;
    }
    ...
    if (!event.getByToken(elecs_, elecs)) {
      edm::LogWarning("TopSingleLeptonHLTOfflineDQM") << "Electron collection not found \n";
      return;
    }
    ...
    if (!event.getByToken(muons_, muons)) {
      edm::LogWarning("TopSingleLeptonHLTOfflineDQM") << "Muon collection not found \n";
      return;
    }
    reco::Muon mu;
    ...
    if (includeBTag_) {
      if (!event.getByToken(btagEff_, btagEff))
        return;
      //        if( !event.getByToken(btagPur_, btagPur) ) return;
      if (!event.getByToken(btagVtx_, btagVtx))
        return;
    }
    ...
    if (!event.getByToken(jets_, jets)) {
      edm::LogWarning("TopSingleLeptonHLTOfflineDQM") << "Jet collection not found \n";
      return;
    }
    ...
    if (multBVtx != 0 && mMultIso == 1) {
      double mtW = eventKinematics.tmassWBoson(&mu, mET, bJetCand);
      if (mtW == mtW)
        fill("MTWm_", mtW);
      double Mlb = eventKinematics.masslb(&mu, mET, bJetCand);
      if (Mlb == Mlb)
        fill("mMub_", Mlb);
      double MTT = eventKinematics.tmassTopQuark(&mu, mET, bJetCand);
      if (MTT == MTT)
        fill("mMTT_", MTT);
    }
    ...
    for (unsigned int i = 0; i < triggerNames.triggerNames().size(); ++i) {
      ...
      for (auto const& triggerPath : triggerPaths) {
        if (TString(name.c_str()).Contains(TString(triggerPath), TString::kIgnoreCase))
          isInteresting = true;
      }
      if (!isInteresting)
        continue;
      ...
      if (kElec > 0) {
        ...
        if (hasRawTriggerSummary) {
          const unsigned int filterIndexElec(
              rawTriggerEvent->filterIndex(edm::InputTag(moduleLabelElec, "", processName_)));
          rawTriggerEvent->getObjects(filterIndexElec, electronIds_, electronRefs_);
        } else {
          const unsigned int filterIndexElec(
              aodTriggerEvent->filterIndex(edm::InputTag(moduleLabelElec, "", processName_)));
      ...
      if (kMuon > 0) {
        ...
        if (hasRawTriggerSummary) {
          const unsigned int filterIndexMuon(
              rawTriggerEvent->filterIndex(edm::InputTag(moduleLabelMuon, "", processName_)));
          rawTriggerEvent->getObjects(filterIndexMuon, muonIds_, muonRefs_);
        } else {
          const unsigned int filterIndexMuon(
              aodTriggerEvent->filterIndex(edm::InputTag(moduleLabelMuon, "", processName_)));
      ...
      if (kJet > 0) {
        ...
        if (hasRawTriggerSummary) {
          const unsigned int filterIndexJet(
              rawTriggerEvent->filterIndex(edm::InputTag(moduleLabelJet, "", processName_)));
          rawTriggerEvent->getObjects(filterIndexJet, pfjetIds_, pfjetRefs_);
        } else {
          const unsigned int filterIndexJet(
              aodTriggerEvent->filterIndex(edm::InputTag(moduleLabelJet, "", processName_)));
        ...
        if (nPFJets > 0 && j1DeltaRMin < DRMIN)
          j1Matched = true;
        if (nPFJets > 1 && j2DeltaRMin < DRMIN)
          j2Matched = true;
        if (nPFJets > 2 && j3DeltaRMin < DRMIN)
          j3Matched = true;
      }
      ...
      if (eIndMatched < 500) {
        ...
        if (lMatched)
          fill("leptResolution_",
               fabs(isoElecs[eIndMatched]->pt() - electronRefs_[0]->pt()) / isoElecs[eIndMatched]->pt());
      }

      if (mIndMatched < 500) {
        fill("leptDeltaREta_", isoMuons[mIndMatched]->eta(), mDeltaRMin);
        if (lMatched)
          fill("leptResolution_", fabs(isoMuons[mIndMatched]->pt() - muonRefs_[0]->pt()) / isoMuons[mIndMatched]->pt());
      }
      ...
      if (j1IndMatched < 500) {
        ...
        if (j1Matched) {
          fill("jetResolution_",
               fabs(correctedJets[j1IndMatched].pt() - pfjetRefs_[0]->pt()) / correctedJets[j1IndMatched].pt());
        ...
        if (j2IndMatched < 500) {
          ...
          if (j2Matched) {
            fill("jetResolution_",
                 fabs(correctedJets[j2IndMatched].pt() - pfjetRefs_[1]->pt()) / correctedJets[j2IndMatched].pt());
          ...
          if (j3IndMatched < 500) {
            ...
            if (j3Matched) {
              fill("jetResolution_",
                   fabs(correctedJets[j3IndMatched].pt() - pfjetRefs_[2]->pt()) / correctedJets[j3IndMatched].pt());
...
TopSingleLeptonHLTOfflineDQM::TopSingleLeptonHLTOfflineDQM(const edm::ParameterSet& cfg)
  ...
  for (auto& i : sel) {
    ...
    selection_[selectionStep(selectionOrder_.back())] =
        std::make_pair(i,
                       std::make_unique<HLTOfflineDQMTopSingleLepton::MonitorSingleLepton>(
  ...
  for (const std::string& s : selectionOrder_) {
    ...
    if (type == "elecs") {
      selectmap_[type] =
          std::make_unique<SelectionStepHLT<reco::GsfElectron>>(selection_[key].first, consumesCollector());
    }
    if (type == "jets") {
...
void TopSingleLeptonHLTOfflineDQM::dqmBeginRun(const edm::Run& iRun, const edm::EventSetup& iSetup) {
  using namespace std;
  using namespace edm;
...
void TopSingleLeptonHLTOfflineDQM::analyze(const edm::Event& event, const edm::EventSetup& setup) {
  if (!triggerTable_.isUninitialized()) {
    ...
    if (!event.getByToken(triggerTable_, triggerTable))
      return;
    if (!acceptHLT(event, *triggerTable, triggerPaths_))
      return;
  }
  ...
  for (auto selIt = selectionOrder_.begin(); selIt != selectionOrder_.end(); ++selIt) {
    ...
    if (selection_.find(key) != selection_.end()) {
      ...
      for (auto selIt2 = selectionOrder_.begin(); selIt2 <= selIt; ++selIt2) {
        ...
        if (selection_.find(key2) == selection_.end())
          continue;

        if (type2 == "Hlt" || type2 == "empty")
          continue;
        if (!selectmap_[type2]->select(event))
          passSel = false;
