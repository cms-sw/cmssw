...
protected:
  ...
  void bookME(DQMStore::IBooker&,
              ...
              double xmax);
  void bookME(
      DQMStore::IBooker&, JetME& me, std::string& histname, std::string& histtitle, std::vector<double> binningX);
  void bookME(DQMStore::IBooker&,
              JetME& me,
              std::string& histname,
              std::string& histtitle,
  ...
  void bookMESub(DQMStore::IBooker&,
                 ...
                 bool doEta = true,
                 bool doEtaPhi = true,
                 bool doVsLS = true);  //Fill Histograms
  void FillME(JetME* a_me,
              double pt_,
              double phi_,
              double eta_,
              int ls_,
...
private:
  ...
  MEbinning jet_phi_binning_{Jet_N_PHI, -Jet_MAX_PHI, Jet_MAX_PHI};
  // Define Eta Bin //
  double Jet_MAX_ETA = 5;
  //  unsigned int Jet_N_ETA = 50;
  unsigned int Jet_N_ETA =
      20;  // (mia) not optimal, we should make use of variable binning which reflects the detector !
  MEbinning jet_eta_binning_{Jet_N_ETA, -Jet_MAX_ETA, Jet_MAX_ETA};

  // Define HEP17 for PHI
  double Jet_MAX_PHI_HEP17 = -0.52;
  double Jet_MIN_PHI_HEP17 = -0.87;
  unsigned int N_PHI_HEP17 = 7;
  MEbinning phi_binning_hep17_{N_PHI_HEP17, Jet_MIN_PHI_HEP17, Jet_MAX_PHI_HEP17};

  // Define HEP18 for PHI
  double Jet_MAX_PHI_HEP18 = -0.17;
  double Jet_MIN_PHI_HEP18 = -0.52;
  unsigned int N_PHI_HEP18 = 7;
  MEbinning phi_binning_hep18_{N_PHI_HEP18, Jet_MIN_PHI_HEP18, Jet_MAX_PHI_HEP18};
  // Define HEP17 for ETA
  double Jet_MAX_ETA_HEP17 = 3.0;
  double Jet_MIN_ETA_HEP17 = 1.3;
  unsigned int N_ETA_HEP17 = 9;
  MEbinning eta_binning_hep17_{N_ETA_HEP17, Jet_MIN_ETA_HEP17, Jet_MAX_ETA_HEP17};

  MEbinning eta_binning_hem17_{N_ETA_HEP17, -Jet_MAX_ETA_HEP17, -Jet_MIN_ETA_HEP17};
};
