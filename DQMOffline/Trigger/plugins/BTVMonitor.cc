...
class BTVMonitor : public DQMEDAnalyzer, public TriggerDQMBase {
public:
  typedef dqm::reco::MonitorElement MonitorElement;
  typedef dqm::reco::DQMStore DQMStore;

  BTVMonitor(const edm::ParameterSet&);
  ~BTVMonitor() throw() override;
...
protected:
  ...
  struct JetRefCompare {
    inline bool operator()(const edm::RefToBase<pat::Jet>& j1, const edm::RefToBase<pat::Jet>& j2) const {
...
private:
  ...
  StringCutObjectSelector<pat::Jet, true> bjetSelection_;
  ...
  bool opsign_;
  StringCutObjectSelector<pat::Jet> MHTdefinition_;
  double MHTcut_;
...
BTVMonitor::BTVMonitor(const edm::ParameterSet& iConfig)
...
void BTVMonitor::bookHistograms(DQMStore::IBooker& ibooker, edm::Run const& iRun, edm::EventSetup const& iSetup) {
...
void BTVMonitor::analyze(edm::Event const& iEvent, edm::EventSetup const& iSetup) {
  // if valid HLT paths are required,
  ...
  if (applyLeptonPVcuts_ && (pv == nullptr)) {
    edm::LogWarning("BTVMonitor") << "Invalid handle to reco::VertexCollection, event will be skipped";
    return;
  }
  ...
  if (!jetHandle.isValid() && njets_ > 0) {
    edm::LogWarning("BTVMonitor") << "Jet handle not valid \n";
    return;
  }
...
void BTVMonitor::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  edm::ParameterSetDescription desc;
  desc.add<std::string>("FolderName", "HLT/BTV/default");
  ...
  desc.add<std::string>("HTdefinition", "pt > 0 & abs(eta)<2.5");
  desc.add<std::string>("vertexSelection", "!isFake");
  desc.add<std::string>("bjetSelection", "pt > 30 & abs(eta)<2.4");
  ...
  desc.add<unsigned int>("nbjets", 1);
  desc.add<double>("workingpoint", -1);  // no cut applied
  ...
  descriptions.add("BTVMonitoring", desc);
}

// Define this as a plug-in
DEFINE_FWK_MODULE(BTVMonitor);
