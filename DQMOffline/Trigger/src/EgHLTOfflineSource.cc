...
void EgHLTOfflineSource::bookHistograms(DQMStore::IBooker &iBooker, edm::Run const &run, edm::EventSetup const &c)
{
  ...
  for(const auto & eleHLTFilterName : eleHLTFilterNames_){iBooker.setCurrentFolder(dirName_+"/Source_Histos/"+eleHLTFilterName);  addEleTrigPath(monElemFuncs,eleHLTFilterName);}
  for(const auto & phoHLTFilterName : phoHLTFilterNames_){iBooker.setCurrentFolder(dirName_+"/Source_Histos/"+phoHLTFilterName);  addPhoTrigPath(monElemFuncs,phoHLTFilterName);}
...
{ 
  ...
  for(const auto & eleTightLooseTrigName : eleTightLooseTrigNames_){
  ...
  for(const auto & diEleTightLooseTrigName : diEleTightLooseTrigNames_){
    std::vector<std::string> trigNames;
  ...
  for(const auto & phoTightLooseTrigName : phoTightLooseTrigNames_){
    std::vector<std::string> trigNames;
  ...
  for(const auto & diPhoTightLooseTrigName : diPhoTightLooseTrigNames_){
    std::vector<std::string> trigNames;
