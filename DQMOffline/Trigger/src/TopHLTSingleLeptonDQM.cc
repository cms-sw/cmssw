...
#include "DQMOffline/Trigger/interface/TopHLTSingleLeptonDQM.h"
    ...
    MonitorEnsemble::fill(const edm::Event& event, const edm::EventSetup& setup, const HLTConfigProvider& hltConfig, const std::vector<std::string> triggerPaths)
    {
      ...
      for(unsigned int i=0; i<triggerNames.triggerNames().size(); ++i){
        ...
        if (kElec > 0) {
          ...
          //          cout << "reco electron n° " << eIndMatched << " - deltaR = " << eDeltaRMin << " - isMatched = " << lMatched << endl;
        }
        // access to hlt muons
        muonIds_.clear(); muonRefs_.clear();
        ...
        if (kMuon > 0) {
          ...
          //          cout << "reco muon n° " << mIndMatched << " - deltaR = " << mDeltaRMin << " - isMatched = " << lMatched << endl;
        }
        ...
        if (kJet > 0) {
          ...
          /*          cout << " Filter in slot " << kJet << " - label/type " << moduleLabelJet << "/" << moduleTypeJet << endl;
                      cout << " Filter packed up at: " << filterIndexJet << endl;
                      cout << "  Accepted objects:" << endl; */
          triggerEventWithRefsHandle->getObjects(filterIndexJet,pfjetIds_,pfjetRefs_);
          /*const unsigned int nPFJets(pfjetIds_.size());
            cout << "   PFJets: " << nPFJets << "  - the objects: # id pt" << endl;
            for (unsigned int l=0; l!=nPFJets; ++l) {
            cout << "   " << l << " " << pfjetIds_[l]
            << " " << pfjetRefs_[l]->pt()
            << endl;
            }*/
...
TopHLTSingleLeptonDQM::TopHLTSingleLeptonDQM(const edm::ParameterSet& cfg): vertexSelect_(0), beamspotSelect_(0)
{
  ...
  for(unsigned int i=0; i<sel.size(); ++i){
    ...
    selection_[selectionStep(selectionOrder_.back())] = std::make_pair(sel.at(i), new TopHLTSingleLepton::MonitorEnsemble(selectionStep(selectionOrder_.back()).c_str(), cfg.getParameter<edm::ParameterSet>("setup"), consumesCollector()));
  }
  ...
  void
TopHLTSingleLeptonDQM::beginRun(edm::Run const & iRun, edm::EventSetup const& iSetup)
{
  using namespace std;
  using namespace edm;
