...
PhoHLTFilterMon::PhoHLTFilterMon(DQMStore::IBooker &iBooker, const std::string& filterName,TrigCodes::TrigBitSet filterBit,const BinData& bins,const CutMasks& masks):
  ...
  filterBit_(filterBit)
{
  ...
  for(size_t i=0;i<phoMonElems_.size();i++){
    MonElemFuncs::initStdPhoHists(iBooker,phoMonElems_[i]->monElems(),filterName_,filterName_+"_pho_passFilter"+phoMonElems_[i]->name(),bins);
  }
  
  if(monHLTFailedPho) phoFailMonElems_.push_back(new MonElemContainer<OffPho>());
  for(size_t i=0;i<phoFailMonElems_.size();i++){
    MonElemFuncs::initStdPhoHists(iBooker,phoFailMonElems_[i]->monElems(),filterName_,filterName_+"_pho_failFilter"+phoMonElems_[i]->name(),bins);
  }
  ...
  if(doN1andSingleEffsPho){
    for(size_t i=0;i<phoEffHists_.size();i++){ 
      MonElemFuncs::initStdEffHists(iBooker,phoEffHists_[i]->cutMonElems(),filterName_,
  ...
  diPhoMassBothME_ = new DiPhoMon(iBooker, filterName_+"_diPho_bothPassFilter_mass",
  ...
  diPhoMassOnlyOneME_ = new DiPhoMon(iBooker, filterName_+"_diPho_onlyOnePassFilter_mass",
				     filterName_+"_diPho_onlyOnePassFilter Mass;M_{#gamma#gamma} (GeV/c^{2})",
				     bins.mass.nr,bins.mass.min,bins.mass.max,&ParticlePair<OffPho>::mass);
  diPhoMassBothHighME_ = new DiPhoMon(iBooker, filterName_+"_diPho_bothPassFilter_massHigh",
				  filterName_+"_diPho_bothPassFilter Mass;M_{#gamma#gamma} (GeV/c^{2})",
				  bins.massHigh.nr,bins.massHigh.min,bins.massHigh.max,&ParticlePair<OffPho>::mass);
  diPhoMassOnlyOneHighME_ = new DiPhoMon(iBooker, filterName_+"_diPho_onlyOnePassFilter_massHigh",
				     filterName_+"_diPho_onlyOnePassFilter Mass;M_{#gamma#gamma} (GeV/c^{2})",
				     bins.massHigh.nr,bins.massHigh.min,bins.massHigh.max,&ParticlePair<OffPho>::mass);
