...
double CalculateHLT::tmassWBoson(reco::RecoCandidate* mu, const reco::MET& met, const reco::Jet& b) {
  if (!failed_ && tmassWBoson_ < 0)
    operator()(b, mu, met);
  return tmassWBoson_;
}

double CalculateHLT::masslb(reco::RecoCandidate* mu, const reco::MET& met, const reco::Jet& b) {
  if (!failed_ && mlb_ < 0)
    operator()(b, mu, met);
  return mlb_;
}

double CalculateHLT::tmassTopQuark(reco::RecoCandidate* lepton, const reco::MET& met, const reco::Jet& b) {
  if (!failed_ && tmassTopQuark_ < 0)
