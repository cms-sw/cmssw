...
DQMHcalPhiSymAlCaReco::DQMHcalPhiSymAlCaReco(const edm::ParameterSet &ps)
  ...
  //
  folderName_ =
      ps.getUntrackedParameter<string>("FolderName", "ALCAStreamHcalPhiSym");

  hbherecoMB = consumes<HBHERecHitCollection>(
      ps.getParameter<edm::InputTag>("hbheInputMB"));
  horecoMB = ps.getParameter<edm::InputTag>("hoInputMB");
  hfrecoMB =
      consumes<HFRecHitCollection>(ps.getParameter<edm::InputTag>("hfInputMB"));

  hbherecoNoise = consumes<HBHERecHitCollection>(
      ps.getParameter<edm::InputTag>("hbheInputNoise"));
  horecoNoise = ps.getParameter<edm::InputTag>("hoInputNoise");
  hfrecoNoise = consumes<HFRecHitCollection>(
      ps.getParameter<edm::InputTag>("hfInputNoise"));

  rawInLabel_ = consumes<FEDRawDataCollection>(
      ps.getParameter<edm::InputTag>("rawInputLabel"));
...
void DQMHcalPhiSymAlCaReco::bookHistograms(DQMStore::IBooker &ibooker,
  ...
  double xmin = 0.1;
  double xmax = 1.1;
  hiDistrHBHEsize1D_ = ibooker.book1D(
      "DistrHBHEsize", "Size of HBHE Collection", hiDistr_r_nbin_, xmin, xmax);
  hiDistrHFsize1D_ = ibooker.book1D("DistrHFsize", "Size of HF Collection",
                                    hiDistr_r_nbin_, xmin, xmax);

  // First moment
  hiDistrMBPl2D_ =
      ibooker.book2D("MBdepthPl1", "iphi- +ieta signal distribution at depth1",
                     hiDistr_x_nbin_, hiDistr_x_min_, hiDistr_x_max_,
                     hiDistr_y_nbin_, hiDistr_y_min_, hiDistr_y_max_);
  ...
  hiDistrNoisePl2D_ =
      ibooker.book2D("NoisedepthPl1", "iphi-ieta noise distribution at depth1",
                     hiDistr_x_nbin_ + 1, hiDistr_x_min_ - 1., hiDistr_x_max_,
                     hiDistr_y_nbin_ + 1, hiDistr_y_min_ - 1., hiDistr_y_max_);
  ...
  // Variance
  hiDistrVarMBPl2D_ = ibooker.book2D(
      "VarMBdepthPl1", "iphi- +ieta signal distribution at depth1",
      hiDistr_x_nbin_, hiDistr_x_min_, hiDistr_x_max_, hiDistr_y_nbin_,
      hiDistr_y_min_, hiDistr_y_max_);

  hiDistrVarMBPl2D_->setAxisTitle("i#phi ", 2);
  hiDistrVarMBPl2D_->setAxisTitle("i#eta ", 1);

  hiDistrVarNoisePl2D_ = ibooker.book2D(
      "VarNoisedepthPl1", "iphi-ieta noise distribution at depth1",
      hiDistr_x_nbin_, hiDistr_x_min_, hiDistr_x_max_, hiDistr_y_nbin_,
      hiDistr_y_min_, hiDistr_y_max_);
  ...
  // Second moment
  hiDistrMB2Min2D_ = ibooker.book2D(
      "MB2depthMin1", "iphi- +ieta signal distribution at depth1",
      hiDistr_x_nbin_, hiDistr_x_min_, hiDistr_x_max_, hiDistr_y_nbin_,
      hiDistr_y_min_, hiDistr_y_max_);
  ...
  hiDistrNoise2Min2D_ = ibooker.book2D(
      "Noise2depthMin1", "iphi-ieta noise distribution at depth1",
      hiDistr_x_nbin_, hiDistr_x_min_, hiDistr_x_max_, hiDistr_y_nbin_,
      hiDistr_y_min_, hiDistr_y_max_);
  ...
  // Variance
  hiDistrVarMBMin2D_ = ibooker.book2D(
      "VarMBdepthMin1", "iphi- +ieta signal distribution at depth1",
      hiDistr_x_nbin_, hiDistr_x_min_, hiDistr_x_max_, hiDistr_y_nbin_,
      hiDistr_y_min_, hiDistr_y_max_);
  ...
  hiDistrVarNoiseMin2D_ = ibooker.book2D(
      "VarNoisedepthMin1", "iphi-ieta noise distribution at depth1",
      hiDistr_x_nbin_, hiDistr_x_min_, hiDistr_x_max_, hiDistr_y_nbin_,
      hiDistr_y_min_, hiDistr_y_max_);
...
void DQMHcalPhiSymAlCaReco::analyze(const Event &iEvent,
  ...
  if (!rawIn.isValid()) {
    LogDebug("") << "HcalCalibAlgos: Error! can't get hbhe product!"
                 << std::endl;
    return;
  }
  ...
  for (HBHERecHitCollection::const_iterator hbheItr = HithbheNS.begin();
    ...
    if (hid.depth() == 1) {
      if (hid.ieta() > 0) {
        ...
        hiDistrNoise2Pl2D_->Fill(hid.ieta(), hid.iphi(),
                                 hbheItr->energy() * hbheItr->energy());
      } else {
        hiDistrNoiseMin2D_->Fill(fabs(hid.ieta()), hid.iphi(),
                                 hbheItr->energy());
        hiDistrNoise2Min2D_->Fill(fabs(hid.ieta()), hid.iphi(),
                                  hbheItr->energy() * hbheItr->energy());
      }
    }
  }
  ...
  for (HBHERecHitCollection::const_iterator hbheItr = HithbheMB.begin();
    ...
    if (hid.depth() == 1) {
      if (hid.ieta() > 0) {
        ...
        hiDistrMB2Pl2D_->Fill(hid.ieta(), hid.iphi(),
                              hbheItr->energy() * hbheItr->energy());
      } else {
        hiDistrMBMin2D_->Fill(fabs(hid.ieta()), hid.iphi(), hbheItr->energy());
        hiDistrMB2Min2D_->Fill(fabs(hid.ieta()), hid.iphi(),
                               hbheItr->energy() * hbheItr->energy());
      }
    }
  }
