...
DQMHcalDiJetsAlCaReco::DQMHcalDiJetsAlCaReco(const edm::ParameterSet &iConfig)
  ...
  jets_ = consumes<CaloJetCollection>(
      iConfig.getParameter<edm::InputTag>("jetsInput"));
  ec_ = consumes<EcalRecHitCollection>(
      iConfig.getParameter<edm::InputTag>("ecInput"));
  hbhe_ = consumes<HBHERecHitCollection>(
      iConfig.getParameter<edm::InputTag>("hbheInput"));
  ho_ = consumes<HORecHitCollection>(
      iConfig.getParameter<edm::InputTag>("hoInput"));
  hf_ = consumes<HFRecHitCollection>(
      iConfig.getParameter<edm::InputTag>("hfInput"));

  saveToFile_ = iConfig.getUntrackedParameter<bool>("SaveToFile", false);
  fileName_ = iConfig.getUntrackedParameter<string>(
      "FileName", "MonitorAlCaHcalDiJets.root");
}

DQMHcalDiJetsAlCaReco::~DQMHcalDiJetsAlCaReco() {}

//--------------------------------------------------------
void DQMHcalDiJetsAlCaReco::bookHistograms(
...
void DQMHcalDiJetsAlCaReco::analyze(const Event &iEvent,
  ...
  if (!jets.isValid()) {
    LogDebug("") << "DQMHcalDiJetsAlCaReco: Error! can't getjet product!"
                 << std::endl;
    return;
  }
  ...
  if (!ec.isValid()) {
    LogDebug("") << "DQMHcalDiJetsAlCaReco: Error! can't get ec product!"
                 << std::endl;
    return;
  }
  ...
  if (!hbhe.isValid()) {
    LogDebug("") << "DQMHcalDiJetsAlCaReco: Error! can't get hbhe product!"
                 << std::endl;
    return;
  }
  ...
  if (!ho.isValid()) {
    LogDebug("") << "DQMHcalDiJetsAlCaReco: Error! can't get ho product!"
                 << std::endl;
    return;
  }
  ...
  if (!hf.isValid()) {
    LogDebug("") << "DQMHcalDiJetsAlCaReco: Error! can't get hf product!"
                 << std::endl;
    return;
  }
