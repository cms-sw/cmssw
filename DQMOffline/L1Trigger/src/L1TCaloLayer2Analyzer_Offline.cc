#include "DQMOffline/L1Trigger/interface/L1TStage2CaloLayer2Analyzer_Offline.h"
...
// -------------------------------------- Constructor --------------------------------------------
//
L1TStage2CaloLayer2Analyzer_Offline::L1TStage2CaloLayer2Analyzer_Offline(const edm::ParameterSet& ps) :
				...
				thePVCollection_(
						consumes < reco::VertexCollection > (ps.getParameter < edm::InputTag > ("PVCollection"))),
				theBSCollection_(
						consumes < reco::BeamSpot > (ps.getParameter < edm::InputTag > ("beamSpotCollection"))),
				triggerEvent_(consumes < trigger::TriggerEvent > (ps.getParameter < edm::InputTag > ("TriggerEvent"))),
				triggerResults_(
						consumes < edm::TriggerResults > (ps.getParameter < edm::InputTag > ("TriggerResults"))),
				triggerFilter_(ps.getParameter < edm::InputTag > ("TriggerFilter")),
				triggerPath_(ps.getParameter < std::string > ("TriggerPath")),
				histFolder_(ps.getParameter < std::string > ("histFolder")),
				stage2CaloLayer2JetToken_(
						consumes < l1t::JetBxCollection
								> (ps.getParameter < edm::InputTag > ("stage2CaloLayer2JetSource"))),
				stage2CaloLayer2EtSumToken_(
						consumes < l1t::EtSumBxCollection
								> (ps.getParameter < edm::InputTag > ("stage2CaloLayer2EtSumSource"))) {
	edm::LogInfo("L1TStage2CaloLayer2Analyzer_Offline") << "Constructor "
			<< "L1TStage2CaloLayer2Analyzer_Offline::L1TStage2CaloLayer2Analyzer_Offline " << std::endl;
}

//
// -- Destructor
//
L1TStage2CaloLayer2Analyzer_Offline::~L1TStage2CaloLayer2Analyzer_Offline() {
	edm::LogInfo("L1TStage2CaloLayer2Analyzer_Offline")
			<< "Destructor L1TStage2CaloLayer2Analyzer_Offline::~L1TStage2CaloLayer2Analyzer_Offline " << std::endl;
}

//
// -------------------------------------- beginRun --------------------------------------------
//
void L1TStage2CaloLayer2Analyzer_Offline::dqmBeginRun(edm::Run const &, edm::EventSetup const &) {
	edm::LogInfo("L1TStage2CaloLayer2Analyzer_Offline") << "L1TStage2CaloLayer2Analyzer_Offline::beginRun" << std::endl;
}
//
// -------------------------------------- bookHistos --------------------------------------------
//
void L1TStage2CaloLayer2Analyzer_Offline::bookHistograms(DQMStore::IBooker & ibooker_, edm::Run const &,
		edm::EventSetup const &) {
	edm::LogInfo("L1TStage2CaloLayer2Analyzer_Offline") << "L1TStage2CaloLayer2Analyzer_Offline::bookHistograms"
			<< std::endl;
...
//
void L1TStage2CaloLayer2Analyzer_Offline::beginLuminosityBlock(edm::LuminosityBlock const& lumiSeg,
		edm::EventSetup const& context) {
	edm::LogInfo("L1TStage2CaloLayer2Analyzer_Offline") << "L1TStage2CaloLayer2Analyzer_Offline::beginLuminosityBlock"
			<< std::endl;
}
...
// -------------------------------------- Analyze --------------------------------------------
//
void L1TStage2CaloLayer2Analyzer_Offline::analyze(edm::Event const& e, edm::EventSetup const& eSetup) {
	edm::LogInfo("L1TStage2CaloLayer2Analyzer_Offline") << "L1TStage2CaloLayer2Analyzer_Offline::analyze" << std::endl;

	edm::Handle<reco::VertexCollection> vertexHandle;
	e.getByToken(thePVCollection_, vertexHandle);
	if (!vertexHandle.isValid()) {
		edm::LogError("L1TStage2CaloLayer2Analyzer_Offline") << "invalid collection: vertex " << std::endl;
		return;
	}
...
void L1TStage2CaloLayer2Analyzer_Offline::fillEnergySums(edm::Event const& e, const unsigned int nVertex) {
	...
	if (!caloJets.isValid()) {
		edm::LogError("L1TStage2CaloLayer2Analyzer_Offline") << "invalid collection: calo jets " << std::endl;
		return;
	}
	if (!caloMETs.isValid()) {
		edm::LogError("L1TStage2CaloLayer2Analyzer_Offline") << "invalid collection: calo MET " << std::endl;
		return;
	}
	if (!l1EtSums.isValid()) {
		edm::LogError("L1TStage2CaloLayer2Analyzer_Offline") << "invalid collection: L1 ET sums " << std::endl;
		return;
	}
	...
	h_turnOnMET_total_->Fill(recoMET);
	h_turnOnMHT_total_->Fill(recoMHT);
	h_turnOnETT_total_->Fill(recoETT);
	h_turnOnHTT_total_->Fill(recoHTT);

	if (l1MET > 40) { // TODO: add 60, 80, 100, 120
		h_turnOnMET_pass_->Fill(recoMET);
	}

	if (l1MHT > 40) { // TODO: add 60, 80, 100, 120
		h_turnOnMHT_pass_->Fill(recoMHT);
	}

	if (l1HTT > 120) { // TODO: add 160, 200, 240, 280
		h_turnOnHTT_pass_->Fill(recoHTT);
	}

}

void L1TStage2CaloLayer2Analyzer_Offline::fillJets(edm::Event const& e, const unsigned int nVertex) {
	...
	if (!l1Jets.isValid()) {
		edm::LogError("L1TStage2CaloLayer2Analyzer_Offline") << "invalid collection: L1 jets " << std::endl;
		return;
	}

	if (caloJets->size() == 0) {
		edm::LogError("L1TStage2CaloLayer2Analyzer_Offline") << "no calo jets found" << std::endl;
		return;
	}

	if (l1Jets->size() == 0) {
		edm::LogError("L1TStage2CaloLayer2Analyzer_Offline") << "no L1 jets found" << std::endl;
		return;
	}
...
//	}
	...
	if (!foundMatch) {
		edm::LogError("L1TStage2CaloLayer2Analyzer_Offline") << "Could not find a matching L1 jet " << std::endl;
		return;
	}
...
// -------------------------------------- endLuminosityBlock --------------------------------------------
//
void L1TStage2CaloLayer2Analyzer_Offline::endLuminosityBlock(edm::LuminosityBlock const& lumiSeg,
		edm::EventSetup const& eSetup) {
	edm::LogInfo("L1TStage2CaloLayer2Analyzer_Offline") << "L1TStage2CaloLayer2Analyzer_Offline::endLuminosityBlock"
			<< std::endl;
}
...
// -------------------------------------- endRun --------------------------------------------
//
void L1TStage2CaloLayer2Analyzer_Offline::endRun(edm::Run const& run, edm::EventSetup const& eSetup) {
	edm::LogInfo("L1TStage2CaloLayer2Analyzer_Offline") << "L1TStage2CaloLayer2Analyzer_Offline::endRun" << std::endl;
}

//
// -------------------------------------- book histograms --------------------------------------------
//
void L1TStage2CaloLayer2Analyzer_Offline::bookHistos(DQMStore::IBooker & ibooker) {
	bookEnergySumHistos(ibooker);
	bookJetHistos(ibooker);
}

void L1TStage2CaloLayer2Analyzer_Offline::bookEnergySumHistos(DQMStore::IBooker & ibooker) {
	ibooker.cd();
...
void L1TStage2CaloLayer2Analyzer_Offline::bookJetHistos(DQMStore::IBooker & ibooker) {
	ibooker.cd();
	ibooker.setCurrentFolder(histFolder_.c_str());
	// jet reco vs L1
	...
	// jet turn-ons
	h_efficiencyJetEt_HB_pass_ = ibooker.book1D("efficiencyJetEt_HB_Num", "jet turn-on (HB); reco jet ET; events", 300, 0, 300);
	h_efficiencyJetEt_HE_pass_ = ibooker.book1D("efficiencyJetEt_HE_Num", "jet turn-on (HE); reco jet ET; events", 300, 0, 300);
	h_efficiencyJetEt_HF_pass_ = ibooker.book1D("efficiencyJetEt_HF_Num", "jet turn-on (HF); reco jet ET; events", 300, 0, 300);
	h_efficiencyJetEt_HB_HE_pass_ = ibooker.book1D("efficiencyJetEt_HB_HE_Num", "jet turn-on (HB+HE); reco jet ET; events", 300, 0,
			300);

	h_efficiencyJetEt_HB_total_ = ibooker.book1D("efficiencyJetEt_HB_Den", "jet turn-on (HB); reco jet ET; events", 300, 0, 300);
	h_efficiencyJetEt_HE_total_ = ibooker.book1D("efficiencyJetEt_HE_Den", "jet turn-on (HE); reco jet ET; events", 300, 0, 300);
	h_efficiencyJetEt_HF_total_ = ibooker.book1D("efficiencyJetEt_HF_Den", "jet turn-on (HF); reco jet ET; events", 300, 0, 300);
	h_efficiencyJetEt_HB_HE_total_ = ibooker.book1D("efficiencyJetEt_HB_HE_Den", "jet turn-on (HB+HE); reco jet ET; events", 300, 0,
			300);
...
// -------------------------------------- functions --------------------------------------------
//
double L1TStage2CaloLayer2Analyzer_Offline::Distance(const reco::Candidate & c1, const reco::Candidate & c2) {
	return deltaR(c1, c2);
}

double L1TStage2CaloLayer2Analyzer_Offline::DistancePhi(const reco::Candidate & c1, const reco::Candidate & c2) {
	return deltaPhi(c1.p4().phi(), c2.p4().phi());
}

// This always returns only a positive deltaPhi
double L1TStage2CaloLayer2Analyzer_Offline::calcDeltaPhi(double phi1, double phi2) {
...
DEFINE_FWK_MODULE (L1TStage2CaloLayer2Analyzer_Offline);
