...
HcalCollapseAnalyzer::HcalCollapseAnalyzer(const edm::ParameterSet &iConfig)
    : topFolderName_(iConfig.getParameter<std::string>("topFolderName")),
      ...
      recHitHBHE_(iConfig.getUntrackedParameter<edm::InputTag>(
          "recHitHBHE", edm::InputTag("hbhereco"))),
      preRecHitHBHE_(iConfig.getUntrackedParameter<edm::InputTag>(
          "preRecHitHBHE", edm::InputTag("hbheprereco"))),
  ...
  edm::LogVerbatim("Collapse")
      << "Verbosity " << verbosity_ << " with tags " << recHitHBHE_ << " and "
      << preRecHitHBHE_ << " and Do " << doHB_ << ":" << doHE_;
}

void HcalCollapseAnalyzer::fillDescriptions(
  ...
  desc.addUntracked<edm::InputTag>("preRecHitHBHE",
                                   edm::InputTag("hbheprereco"));
...
void HcalCollapseAnalyzer::analyze(edm::Event const &iEvent,
  ...
  if (verbosity_ > 0) {
    edm::LogVerbatim("Collapse")
        << "Handle Reco " << hbhereco << " Size " << hbhereco->size();
    edm::LogVerbatim("Collapse")
        << "Handle PreReco " << hbheprereco << " Size " << hbheprereco->size();
  }
  if (hbhereco.isValid() && hbheprereco.isValid()) {
    ...
    if (verbosity_ > 0)
      edm::LogVerbatim("Collapse")
          << "Size of hbhereco: " << recohbhe->size()
          << " and hbheprereco: " << prerecohbhe->size();
    double sfrac = (prerecohbhe->empty()) ? 1
                                          : ((double)(recohbhe->size())) /
                                                ((double)(prerecohbhe->size()));
    h_sfrac->Fill(sfrac);
    ...
    for (const auto &hit : (*recohbhe)) {
      ...
      if (((id.subdet() == HcalEndcap) && doHE_) ||
        ...
        if (verbosity_ > 0) {
          edm::LogVerbatim("Collapse")
              << id << " is derived from " << ids.size() << " components";
...
void HcalCollapseAnalyzer::bookHistograms(DQMStore::IBooker &ibooker,
  ...
  h_size = ibooker.book1D("h_size", "Size of the RecHit collection", 100, 500.0,
                          1500.0);
  h_depth = ibooker.book1D("h_depth", "Depth of the Id's used", 10, 0.0, 10.0);
  h_sfrac = ibooker.book1D("h_sfrac",
                           "Ratio of sizes of preRecHit and RecHit collections",
                           150, 0.0, 1.5);
  h_frac = ibooker.book1D("h_frac", "Fraction of energy before collapse", 150,
                          0.0, 1.5);
  h_balance = ibooker.book1D("h_balance",
                             "Balance of energy between pre- and post-collapse",
                             100, 0.5, 1.5);
}
