...
void ESRecoSummary::analyze(const edm::Event& ev, const edm::EventSetup&)
{
  ...
  for (const auto & thePreShowerRecHit : *thePreShowerRecHits) 
    {
      ...
      if (thePreShowerRecHit. -> energy() > maxRecHitEnergyES ) maxRecHitEnergyES = thePreShowerRecHit. -> energy() ;
  ...
  for (const auto & theEndcapSuperCluster : *theEndcapSuperClusters) {
    ...
    if ( fabs(theEndcapSuperCluster.eta()) < 1.65 || fabs(theEndcapSuperCluster.eta()) > 2.6 ) continue;
    ...
    for (reco::CaloCluster_iterator ecalBasicCluster = theEndcapSuperCluster.clustersBegin(); ecalBasicCluster!= theEndcapSuperCluster.clustersEnd(); 
	 ecalBasicCluster++) {
