...
ECALMultifitAnalyzer_HI::ECALMultifitAnalyzer_HI(
    ...
    :
      ...
      recoPhotonsCollection_(consumes<std::vector<reco::Photon>>(
          iConfig.getParameter<edm::InputTag>("recoPhotonSrc"))),
      caloJetToken_(consumes<reco::CaloJetCollection>(
          iConfig.getParameter<edm::InputTag>("recoJetSrc"))),
      RecHitCollection_EB_(consumes<EcalRecHitCollection>(
          iConfig.getParameter<edm::InputTag>("RecHitCollection_EB"))),
      RecHitCollection_EE_(consumes<EcalRecHitCollection>(
          iConfig.getParameter<edm::InputTag>("RecHitCollection_EE"))),
      mRechitEnergyThreshold(
          iConfig.getParameter<double>("rechitEnergyThreshold")),
      mRecoPhotonPtThreshold(
          iConfig.getParameter<double>("recoPhotonPtThreshold")),
      mRecoJetPtThreshold(iConfig.getParameter<double>("recoJetPtThreshold")),
      mDeltaRPhotonThreshold(
          iConfig.getParameter<double>("deltaRPhotonThreshold")),
...
void ECALMultifitAnalyzer_HI::analyze(const edm::Event &iEvent,
  ...
  for (EcalRecHitCollection::const_iterator hit = ebHandle->begin();
       hit != ebHandle->end(); ++hit) {
    ...
    for (std::vector<reco::Photon>::const_iterator pho =
             recoPhotonsHandle->begin();
         pho != recoPhotonsHandle->end(); ++pho) {
    ...
    for (std::vector<reco::CaloJet>::const_iterator jet =
             recoJetHandle->begin();
         jet != recoJetHandle->end(); ++jet) {
  ...
  for (EcalRecHitCollection::const_iterator hit = eeHandle->begin();
       hit != eeHandle->end(); ++hit) {
...
void ECALMultifitAnalyzer_HI::bookHistograms(DQMStore::IBooker &iBooker,
  ...
  TH2F *hProfile_Chi2 =
      new TH2F("hProfile_Chi2", "", nBins, -5, 5, nBins, 0, maxChi2);
  TH2F *hProfile_Err =
      new TH2F("hProfile_Err", "", nBins, -5, 5, nBins, 0, maxError);

  eb_chi2 = iBooker.book1D("rechit_eb_chi2", "Rechit eb_chi2;chi2 fit value;",
                           nBins, 0, maxChi2);
  eb_chi2_eta = iBooker.book2D("rechit_eb_eta_Vs_mean_Chi2", hProfile_Chi2);
  eb_chi2_e5 =
      iBooker.book1D(Form("rechit_eb_chi2_e%2.0f", mRechitEnergyThreshold),
                     Form("Rechit eb_chi2, e>%2.0fGeV;chi2 fit value;",
                          mRechitEnergyThreshold),
                     nBins, 0, maxChi2);
  eb_chi2_e5_eta = iBooker.book2D(
      Form("rechit_eb_chi2_e%2.0f_eta", mRechitEnergyThreshold), hProfile_Chi2);
  eb_errors = iBooker.book1D("rechit_eb_errors",
                             "Rechit eb_errors;error on the energy;", nBins, 0,
                             maxError);
  eb_errors_eta = iBooker.book2D("rechit_eb_errors_eta", hProfile_Err);
  eb_errors_e5 = iBooker.book1D(
      Form("rechit_eb_errors_e%2.0f", mRechitEnergyThreshold),
      "Rechit eb_errors, e>5GeV;error on the energy;", nBins, 0, maxError);
  eb_errors_e5_eta = iBooker.book2D(
      Form("rechit_eb_errors_e%2.0f_eta", mRechitEnergyThreshold),
      hProfile_Err);
  eb_chi2_photon15 = iBooker.book1D(
      Form("rechit_eb_chi2_photon%2.0f", mRecoPhotonPtThreshold),
      "Rechit eb_chi2 near photons;chi2 fit value;", nBins, 0, maxChi2);
  eb_errors_photon15 = iBooker.book1D(
      Form("rechit_eb_errors_photon%2.0f", mRecoPhotonPtThreshold),
      "Rechit eb_errors near photons;error on the energy;", nBins, 0, maxError);
  eb_chi2_jet30 = iBooker.book1D(
      Form("rechit_eb_chi2_jet%2.0f", mRecoJetPtThreshold),
      "Rechit eb_chi2 near jets;chi2 fit value;", nBins, 0, maxChi2);
  eb_errors_jet30 = iBooker.book1D(
      Form("rechit_eb_errors_jet%2.0f", mRecoJetPtThreshold),
      "Rechit eb_errors near jets;error on the energy;", nBins, 0, maxError);

  ee_chi2 = iBooker.book1D("rechit_ee_chi2", "Rechit ee_chi2;chi2 fit value;",
                           nBins, 0, maxChi2);
  ee_chi2_eta = iBooker.book2D("rechit_ee_chi2_eta", hProfile_Chi2);
  ee_chi2_e5 = iBooker.book1D(
      Form("rechit_ee_chi2_e%2.0f", mRechitEnergyThreshold),
      "Rechit ee_chi2, e>5GeV;chi2 fit value;", nBins, 0, maxChi2);
  ee_chi2_e5_eta = iBooker.book2D(
      Form("rechit_ee_chi2_e%2.0f_eta", mRechitEnergyThreshold), hProfile_Chi2);
  ee_errors = iBooker.book1D("rechit_ee_errors",
                             "Rechit ee_errors;error on the energy;", nBins, 0,
                             maxError);
  ee_errors_eta = iBooker.book2D("rechit_ee_errors_eta", hProfile_Err);
  ee_errors_e5 = iBooker.book1D(
      Form("rechit_ee_errors_e%2.0f", mRechitEnergyThreshold),
      "Rechit ee_errors, e>5GeV;error on the energy;", nBins, 0, maxError);
  ee_errors_e5_eta = iBooker.book2D(
      Form("rechit_ee_errors_e%2.0f_eta", mRechitEnergyThreshold),
      hProfile_Err);
  ee_chi2_photon15 = iBooker.book1D(
      Form("rechit_ee_chi2_photon%2.0f", mRecoPhotonPtThreshold),
      "Rechit ee_chi2 near photons;chi2 fit value;", nBins, 0, maxChi2);
  ee_errors_photon15 = iBooker.book1D(
      Form("rechit_ee_errors_photon%2.0f", mRecoPhotonPtThreshold),
      "Rechit ee_errors near photons;error on the energy;", nBins, 0, maxError);
  ee_chi2_jet30 = iBooker.book1D(
      Form("rechit_ee_chi2_jet%2.0f", mRecoJetPtThreshold),
      "Rechit ee_chi2 near jets;chi2 fit value;", nBins, 0, maxChi2);
  ee_errors_jet30 = iBooker.book1D(
      Form("rechit_ee_errors_jet%2.0f", mRecoJetPtThreshold),
      "Rechit ee_errors near jets;error on the energy;", nBins, 0, maxError);
