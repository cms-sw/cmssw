...
void DTnoiseDBValidation::endRun(edm::Run const& run, edm::EventSetup const& setup) {
  ...
  if(theDiffQReport) {
	...
	LogWarning("NoiseDBValidation") << " Bad partial difference of noisy channels! Contents : " << badChannel.getContents();
      }
  }
  ...
  if(theDiffQReport2) {
	...
	LogWarning("NoiseDBValidation") << " Bad percentual occupancy for wheel : " << wheel << "  Contents : " << badChannel.getContents();
      }
  }
  ...
  if(theDiffQReport3) {
      ...
      for (auto & badChannel : badChannels) {
	LogWarning("NoiseDBValidation") << " Bad percentual occupancy for station : " << badChannel.getBin() << "  Contents : " << badChannel.getContents();
      }
  }
  ...
  if(theDiffQReport4) {
      ...
      for (auto & badChannel : badChannels) {
	LogWarning("NoiseDBValidation") << " Bad percentual occupancy for sector : " << badChannel.getBin() << "  Contents : " << badChannel.getContents();
      }
  }
  ...
  if(theDiffQReport5) {
	...
	if(badChannel.getBin()==1)
	  LogWarning("NoiseDBValidation") << " Bad percentual occupancy for the first 10 wires! Contents : " << badChannel.getContents();
	if(badChannel.getBin()==2)
	  LogWarning("NoiseDBValidation") << " Bad percentual occupancy for the middle wires! Contents : " << badChannel.getContents();
	if(badChannel.getBin()==3)
	  LogWarning("NoiseDBValidation") << " Bad percentual occupancy for the last 10 wires! Contents : "<<badChannel.getContents();
      }
  }
