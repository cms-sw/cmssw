...
void DQMPFCandidateAnalyzer::bookHistograms(DQMStore::IBooker & ibooker,
  ...
  if(!isMiniAOD_){
    ...
    for (std::vector<edm::ParameterSet>::const_iterator v = diagnosticsParameters_.begin(); v!=diagnosticsParameters_.end(); v++) {
      ...
      multiplicityPFCand_nameRECO_.emplace_back(std::string(v->getParameter<std::string>("name")).append("_multiplicity_").c_str());
      ...
      occupancyPFCand_nameRECO_.emplace_back(std::string(v->getParameter<std::string>("name")).append("_occupancy_").c_str());
      
      ptPFCand_nameRECO_.emplace_back(std::string(v->getParameter<std::string>("name")).append("_pt_").c_str());
    ...
    for (std::vector<edm::ParameterSet>::const_iterator v = diagnosticsParameters_.begin(); v!=diagnosticsParameters_.end(); v++) {
      ...
      multiplicityPFCand_name_.emplace_back(std::string(v->getParameter<std::string>("name")).append("_multiplicity_").c_str());
      ...
      //push back names first, we need to create histograms with the name and fill it for endcap plots later
      occupancyPFCand_name_.emplace_back(std::string(v->getParameter<std::string>("name")).append("_occupancy_puppiWeight_").c_str());
      ptPFCand_name_.emplace_back(std::string(v->getParameter<std::string>("name")).append("_pt_puppiWeight_").c_str());
      //push back names first, we need to create histograms with the name and fill it for endcap plots later
      occupancyPFCand_name_puppiNolepWeight_.emplace_back(std::string(v->getParameter<std::string>("name")).append("_occupancy_puppiNolepWeight_").c_str());	  
      ptPFCand_name_puppiNolepWeight_.emplace_back(std::string(v->getParameter<std::string>("name")).append("_pt_puppiNolepWeight_").c_str());
      //special booking for endcap plots, merge plots for eminus and eplus into one plot, using variable binning
      //barrel plots have eta-boundaries on minus and plus side
      //parameters start on minus side
      if(etaMinPFCand*etaMaxPFCand<0){//barrel plots, plot only in barrel region
