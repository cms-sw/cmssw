...
void MuonMiniAOD::bookHistograms(DQMStore::IBooker& ibooker,
  ...
  for (auto& workingPoint : workingPoints) {
    workingPoint->setBinLabel(1, "Pass", 1);
    workingPoint->setBinLabel(2, "No Pass", 1);
    workingPoint->setBinLabel(1, "Pass", 2);
    workingPoint->setBinLabel(2, "No Pass", 2);
  }
}
...
void MuonMiniAOD::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (edm::View<pat::Muon>::const_iterator muon1 = muons->begin(); muon1 != muons->end(); ++muon1) {
    for (auto& workingPoint : workingPoints) {
      int Pass_A = 0;
      int Pass_B = 0;
      if (PassesCut_A(muon1, thePrimaryVertex, workingPoint->getName()))
        Pass_A = 1;
      else
        Pass_A = 2;
      if (PassesCut_B(muon1, thePrimaryVertex, workingPoint->getName()))
        Pass_B = 1;
      else
        Pass_B = 2;

      workingPoint->Fill(Pass_A, Pass_B);
    }
  }
}
