...
void SegmentTrackAnalyzer::beginJob(edm::EventSetup const& iSetup,DQMStore * dbe) {
  ...
  trackHitPercentualVsEta = dbe->book2D("trackHitDivtrackSegmHitVsEta_"+trackCollection, "(track recHits)/(associated segments recHits) versus #eta - " +trackCollection, etaBin, etaMin, etaMax, 20, 0, 1);
  dtTrackHitPercentualVsEta = dbe->book2D("dtTrackHitDivtrackSegmHitVsEta_"+trackCollection, "(track recHits from DTs)/(associated segments recHits) versus #eta - " +trackCollection, etaBin, etaMin, etaMax, 20, 0, 1);
  cscTrackHitPercentualVsEta = dbe->book2D("cscTrackHitDivtrackSegmHitVsEta_"+trackCollection, "(track recHits from CSCs)/(associated segments recHits) versus #eta - " +trackCollection, etaBin, etaMin, etaMax, 20, 0, 1);
  ...
  trackHitPercentualVsPhi = dbe->book2D("trackHitDivtrackSegmHitVsPhi_"+trackCollection, "(track recHits)/(associated segments recHits) versus #phi - " +trackCollection, phiBin, phiMin, phiMax, 20, 0, 1);
  trackHitPercentualVsPhi->setAxisTitle("rad",2);
  dtTrackHitPercentualVsPhi = dbe->book2D("dtTrackHitDivtrackSegmHitVsPhi_"+trackCollection, "(track recHits from DTs)/(associated segments recHits) versus #phi - " +trackCollection, phiBin, phiMin, phiMax, 20, 0, 1);
  dtTrackHitPercentualVsPhi->setAxisTitle("rad",2);
  cscTrackHitPercentualVsPhi = dbe->book2D("cscTrackHitDivtrackSegmHitVsPhi_"+trackCollection, "(track recHits from CSCs)/(associated segments recHits) versus #phi - " +trackCollection, phiBin, phiMin, phiMax, 20, 0, 1);
  ...
  trackHitPercentualVsPt = dbe->book2D("trackHitDivtrackSegmHitVsPt_"+trackCollection, "(track recHits)/(associated segments recHits) versus 1/p_{t} - " +trackCollection, ptBin, ptMin, ptMax, 20, 0, 1);
  trackHitPercentualVsPt->setAxisTitle("GeV",2);
  dtTrackHitPercentualVsPt = dbe->book2D("dtTrackHitDivtrackSegmHitVsPt_"+trackCollection, "(track recHits from DTs)/(associated segments recHits) versus 1/p_{t} - " +trackCollection, ptBin, ptMin, ptMax, 20, 0, 1);
  dtTrackHitPercentualVsPt->setAxisTitle("GeV",2);
  cscTrackHitPercentualVsPt = dbe->book2D("cscTrackHitDivtrackSegmHitVsPt_"+trackCollection, "(track recHits from CSCs)/(associated segments recHits) versus 1/p_{t} - " +trackCollection, ptBin, ptMin, ptMax, 20, 0, 1);
