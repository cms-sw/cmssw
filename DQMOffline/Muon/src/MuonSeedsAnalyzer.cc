...
void MuonSeedsAnalyzer::beginJob(edm::EventSetup const& iSetup, DQMStore * dbe) {
  ...
  NumberOfRecHitsPerSeed = dbe->book1D(histname, "Number of RecHits of each seed", seedHitBin, seedHitMin, seedHitMax);
 
  PhiBin = parameters.getParameter<int>("PhiBin");
  PhiMin = parameters.getParameter<double>("PhiMin");
  PhiMax = parameters.getParameter<double>("PhiMax");
  histname = "seedPhi_";
  seedPhi = dbe->book1D(histname, "Seed azimuthal angle", PhiBin, PhiMin, PhiMax);
  ...
  histname = "seedEta_";
  seedEta = dbe->book1D(histname, "Seed pseudorapidity", EtaBin, EtaMin, EtaMax);
  ...
  histname = "seedTheta_";
  seedTheta = dbe->book1D(histname, "Seed polar angle", ThetaBin, ThetaMin, ThetaMax);
  ...
  histname = "seedPt_";
  seedPt = dbe->book1D(histname, "Transverse seed momentum", seedPtBin, seedPtMin, seedPtMax);
  ...
  histname = "seedPx_";
  seedPx = dbe->book1D(histname, "x component of seed momentum", seedPxyzBin, seedPxyzMin, seedPxyzMax);
  seedPx->setAxisTitle("GeV");
  histname = "seedPy_";
  seedPy = dbe->book1D(histname, "y component of seed momentum", seedPxyzBin, seedPxyzMin, seedPxyzMax);
  seedPy->setAxisTitle("GeV");
  histname = "seedPz_";
  seedPz = dbe->book1D(histname, "z component of seed momentum", seedPxyzBin, seedPxyzMin, seedPxyzMax);
  ...
  seedPtErrVsPhi = dbe->book2D(histname, "Seed p_{t}Err/p_{t} versus #phi", PhiBin, PhiMin, PhiMax, pErrBin, pErrMin, pErrMax);
  seedPtErrVsPhi->setAxisTitle("rad",2);
  histname = "seedPtErrOverPtVsEta_";
  seedPtErrVsEta = dbe->book2D(histname, "Seed p_{t}Err/p_{t} versus #eta", EtaBin, EtaMin, EtaMax, pErrBin, pErrMin, pErrMax);
  histname = "seedPtErrOverPtVsPt_";
  seedPtErrVsPt = dbe->book2D(histname, "Seed p_{t}Err/p_{t} versus p_{t}", seedPtBin/5, seedPtMin, seedPtMax, pErrBin, pErrMin, pErrMax);
  ...
  seedPErrVsPhi = dbe->book2D(histname, "Seed pErr/p versus #phi", PhiBin, PhiMin, PhiMax, pErrBin, pErrMin, pErrMax);
  seedPErrVsPhi->setAxisTitle("rad",2);
  histname = "seedPErrOverPVsEta_";
  seedPErrVsEta = dbe->book2D(histname, "Seed pErr/p versus #eta", EtaBin, EtaMin, EtaMax, pErrBin, pErrMin, pErrMax);
  histname = "seedPErrOverPVsPt_";
  seedPErrVsPt = dbe->book2D(histname, "Seed pErr/p versus p_{t}", seedPtBin/5, seedPtMin, seedPtMax, pErrBin, pErrMin, pErrMax);
