...
protected:
  ...
  void ResidualCheck(DQMStore::IGetter &, const std::string& muType, const std::vector<std::string>& resHistos, int &numPlot, double &Mean, double &Mean_err, double &Sigma, double &Sigma_err);
  void GaussFit(const std::string& type, const std::string& parameter, MonitorElement *  Histo, float &mean, float &mean_err, float &sigma, float &sigma_err);
