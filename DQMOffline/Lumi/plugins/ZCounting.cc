...
ZCounting::ZCounting(const edm::ParameterSet& iConfig)
    : triggerResultsInputTag_(iConfig.getParameter<edm::InputTag>("TriggerResults")),
      ...
      fStandaloneRegName_token(consumes<reco::TrackCollection>(iConfig.getUntrackedParameter<std::string>("StandaloneReg", "standAloneMuons"))),
      fStandaloneUpdName_token(consumes<reco::TrackCollection>(iConfig.getUntrackedParameter<std::string>("StandaloneUpd", "standAloneMuons:UpdatedAtVtx"))),
      fTrackName_token(consumes<reco::TrackCollection>(iConfig.getUntrackedParameter<std::string>("edmTrackName", "generalTracks"))),
...
void ZCounting::bookHistograms(DQMStore::IBooker& ibooker_, edm::Run const&, edm::EventSetup const&) {
  ...
  h_mass_ID_fail_BB = ibooker_.book2D("h_mass_ID_fail_BB",
                                       "Muon ID failing barrel-barrel",
                                       LumiBin_,
                                       LumiMin_,
                                       LumiMax_,
                                       MassBin_,
                                       MassMin_,
                                       MassMax_);
  h_mass_ID_fail_BE = ibooker_.book2D("h_mass_ID_fail_BE",
                                       "Muon ID failing barrel-endcap",
                                       LumiBin_,
                                       LumiMin_,
                                       LumiMax_,
                                       MassBin_,
  ...
  h_mass_ID_fail_EE = ibooker_.book2D("h_mass_ID_fail_EE",
                                       "Muon ID failing endcap-endcap",
                                       LumiBin_,
                                       LumiMin_,
                                       LumiMax_,
                                       MassBin_,
  ...
  for (auto const &standAlone : *tracksStandAlone) {
    ...
    for (auto const &itMu2 : *hMuonProduct) {
      ...
      if (track->extra().key() == muonStandAlone.extra().key()) {
        ...
        if (muonStandAlone.pt() == track->pt() && muonStandAlone.eta() == track->eta() && muonStandAlone.phi() == track->phi()) {
...
bool ZCounting::passGlobalMuon(const reco::Muon& muon) {
  ...
  return muon.isGlobalMuon()
    && muon.outerTrack()->numberOfValidHits() >= N_STA_HITS
    && muon.innerTrack()->pt() > MIN_PT_TRK
    && std::abs(muon.innerTrack()->eta()) < MAX_ETA_TRK
    && muon.outerTrack()->pt() > MIN_PT_STA
    && std::abs(muon.outerTrack()->eta()) < MAX_ETA_STA
    && reco::deltaR(muon.outerTrack()->eta(), muon.outerTrack()->phi(), muon.innerTrack()->eta(), muon.innerTrack()->phi()) < DRMAX_IO;
}

//--------------------------------------------------------------------------------------------------
bool ZCounting::passTrack(const reco::Track& track) {
  return track.hitPattern().trackerLayersWithMeasurement() >= 6 
