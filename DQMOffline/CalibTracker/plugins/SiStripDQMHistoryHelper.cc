...
void SiStripDQMHistoryHelper::scanTreeAndFillSummary(const std::vector<MonitorElement*>& MEs,
  ...
  try {
    ...
    std::cout << "Error: " << keyName << " is not a valid regular expression: \"" << e.what() << "\"" << std::endl;
...
bool SiStripDQMHistoryHelper::setDBLabelsForLandau(const std::string& keyName,
                                                   std::vector<std::string>& userDBContent) const {
...
bool SiStripDQMHistoryHelper::setDBValuesForLandau(const MonitorElement* me, HDQMSummary::InputVector& values) const {
  ...
  if (m_fitME.getFitnDof() != 0)
    values.push_back(m_fitME.getFitChi() / m_fitME.getFitnDof());
  else
    values.push_back(-99.);
  return true;
}
