...
public:
  explicit GenericHistoryDQM(const edm::ParameterSet& iConfig)
      : SiStripPopConHistoryDQMBase(iConfig), m_detectorID{iConfig.getParameter<uint32_t>("DetectorId")} {}
...
private:
  ...
  bool setDBLabelsForUser(const std::string& keyName,
                          std::vector<std::string>& userDBContent,
                          const std::string& quantity) const override;
  bool setDBValuesForUser(const MonitorElement* me,
...
uint32_t GenericHistoryDQM::returnDetComponent(const MonitorElement* ME) const {
  LogTrace("GenericHistoryDQM")
      << "[GenericHistoryDQM::returnDetComponent] returning value defined in the configuration Pset \"DetectorId\"";
  return m_detectorID;
}
...
bool GenericHistoryDQM::setDBLabelsForUser(const std::string& keyName,
  ...
  if (quantity == "userExample_XMax") {
    ...
    edm::LogError("DQMHistoryServiceBase")
        << "Quantity " << quantity << " cannot be handled\nAllowed quantities are"
        ...
        << "\n or a specific user quantity that should be implemented in the user functions "
           "GenericHistoryDQM::setDBLabelsForUser"
        << std::endl;
    return false;
  }
  return true;
