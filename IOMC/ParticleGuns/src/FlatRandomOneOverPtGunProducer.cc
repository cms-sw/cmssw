...
namespace edm {
  ...
  FlatRandomOneOverPtGunProducer::FlatRandomOneOverPtGunProducer(const edm::ParameterSet& pset)
    ...
    edm::LogInfo("ParticleGun") << "FlatRandomOneOverPtGunProducer: initialized with minimum and maximum 1/pt "
                                << fMinOneOverPt << ":" << fMaxOneOverPt;
  }
  ...
  void FlatRandomOneOverPtGunProducer::fillDescriptions(ConfigurationDescriptions& descriptions) {
    ParameterSetDescription desc;
    ParameterSetDescription pgunParams;
    pgunParams.add<double>("MinOneOverPt");
    pgunParams.add<double>("MaxOneOverPt");
  ...
  void FlatRandomOneOverPtGunProducer::produce(Event& e, const EventSetup& es) {
    ...
    for (unsigned int ip = 0; ip < fPartIDs.size(); ++ip) {
      ...
      LogDebug("ParticleGun") << "FlatRandomOneOverPtGunProducer: Event generated with pt:eta:phi " << pt << " " << eta
                              << " " << phi << " (" << theta / CLHEP::deg << ":" << phi / CLHEP::deg << ")";
