...
BetafuncEvtVtxGenerator::BetafuncEvtVtxGenerator(edm::ParameterSet const& p, edm::ConsumesCollector&) :
    BaseEvtVtxGenerator(), fVertex(new HepMC::FourVector()), readDB_(p.getParameter<bool>("readDB")), boost_() {
  if (!readDB_) {
...
void BetafuncEvtVtxGenerator::beginLuminosityBlock_(edm::LuminosityBlock const&, edm::EventSetup const& iEventSetup) {
  update(iEventSetup);
}

void BetafuncEvtVtxGenerator::beginRun_(edm::Run const&, edm::EventSetup const& iEventSetup) {
  update(iEventSetup);
}

void BetafuncEvtVtxGenerator::update(edm::EventSetup const& iEventSetup) {
...
void BetafuncEvtVtxGenerator::generateNewVertex_(edm::HepMCProduct& product, CLHEP::HepRandomEngine& engine) {
...
HepMC::FourVector* BetafuncEvtVtxGenerator::newVertex(CLHEP::HepRandomEngine& engine) {
	...
	tmp_sigx /= sqrt(2.0);
	double X = CLHEP::RandGaussQ::shoot(&engine, 0., tmp_sigx) + fX0; // + Z*fdxdz ;

	double tmp_sigy = BetaFunction(Z,fZ0);
	// need sqrt(2) for beamspot width relative to single beam width
	tmp_sigy /= sqrt(2.0);
	double Y = CLHEP::RandGaussQ::shoot(&engine, 0., tmp_sigy) + fY0; // + Z*fdydz;
...
TMatrixD* BetafuncEvtVtxGenerator::GetInvLorentzBoost() {
	...
	//boost_.ResizeTo(4,4);
	boost_.reset(new TMatrixD(4,4));
	TMatrixD& boost = *boost_;
	
	//if ((alpha_ == 0) && (phi_==0)) { boost_->Zero(); return boost_.get(); }
	...
	boost(0,0) = 1./cos(phi_);
	boost(0,1) = - cos(alpha_)*sin(phi_);
	boost(0,2) = - tan(phi_)*sin(phi_);
	boost(0,3) = - sin(alpha_)*sin(phi_);
	boost(1,0) = - cos(alpha_)*tan(phi_);
	boost(1,1) = 1.;
	boost(1,2) = cos(alpha_)*tan(phi_);
	boost(1,3) = 0.;
	boost(2,0) = 0.;
	boost(2,1) = - cos(alpha_)*sin(phi_);
	boost(2,2) = cos(phi_);
	boost(2,3) = - sin(alpha_)*sin(phi_);
	boost(3,0) = - sin(alpha_)*tan(phi_);
	boost(3,1) = 0.;
	boost(3,2) = sin(alpha_)*tan(phi_);
	boost(3,3) = 1.;
