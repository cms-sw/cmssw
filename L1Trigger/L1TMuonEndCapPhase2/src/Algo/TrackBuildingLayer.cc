...
// Static
seg_theta_t TrackBuildingLayer::calc_theta_median(
...
void TrackBuildingLayer::attach_segments(
        const segment_collection_t& segments,
        const road_t& road,
        const bool& displaced_en,
        track_t& track
    ...
    seg_phi_t trk_abs_phi = (static_cast<seg_phi_t>(trk_col) << v3::kHitmapColFactorLog2) + (1 << (v3::kHitmapColFactorLog2 - 1));
    seg_phi_t sector_abs_phi = (static_cast<seg_phi_t>(sector_col) << v3::kHitmapColFactorLog2) + (1 << (v3::kHitmapColFactorLog2 - 1));
    ...
    for (int i_row = 0; i_row < v3::kHitmapNRows; ++i_row) {
        ...
        trk_pat_phi[i_row] = (static_cast<seg_phi_t>(temp_trk_pat_center) << v3::kHitmapColFactorLog2) + (1 << (v3::kHitmapColFactorLog2 - 1));
    }
    ...
    for (int site_id = 0; site_id < v3::kNumTrackSites; ++site_id) {
        ...
        if (diff_1 <= diff_2 && diff_1 < theta_window){
            ...
            if (CONFIG.verbosity_ > 4) {
                std::cout
                    << "Segment outside of theta window; detatched:"
                    << " site_id "    << site_id
                    << " seg_id "     << site_seg_id
