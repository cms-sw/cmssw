...
void ParameterAssignmentLayer::apply(
        const bool& displaced_en,
        std::vector<track_t>& tracks
) const {
    std::vector<int> feature_sites = {
    ...
    for (auto& track : tracks) { // Begin loop tracks
        ...
        if (CONFIG.verbosity_ > 1) {
            std::cout
                ...
                << " theta "        << track.theta
                << " beta "         << track.beta
                << " pt_address "   << track.pt_address
                << " rels_address " << track.rels_address
                << " dxy_address "  << track.dxy_address
                << " valid "        << track.valid
                << std::endl;
        }
    } // End loop tracks
