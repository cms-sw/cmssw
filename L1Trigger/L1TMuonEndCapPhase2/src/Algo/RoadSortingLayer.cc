...
void RoadSortingLayer::apply(
    ...
    for (unsigned int i_zone = 0; i_zone < zone_roads.size(); ++i_zone) { // Loop Zones
...

        {
            for (int i_col = 0; i_col < keep_n_roads; ++i_col) {
                ...
                if (CONFIG.verbosity_ > 2 && roads_kept[i_col].quality > 0) {
                    std::cout
                        << "Road Kept" 
                        << " zone " << roads_kept[i_col].zone
                        << " col "  << roads_kept[i_col].col
                        << " pat "  << roads_kept[i_col].pattern
                        << " qual " << roads_kept[i_col].quality
                        << std::endl;
                }
            }
        }
        ...
        data::mergesort(roads_kept, 32, 16, [](const road_t& lhs, const road_t& rhs) -> int {
    ...
    data::mergesort(&top_roads[0], 8, 4, [](const road_t& lhs, const road_t& rhs) -> int {
            return lhs.quality < rhs.quality;
    });
