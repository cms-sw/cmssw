...
TrackFinder::TrackFinder(
        const edm::ParameterSet& i_config,
        edm::ConsumesCollector&& i_consumes_collector
):
    context_(i_config, i_consumes_collector),
    tp_collectors_(),
    sector_processors_()
{
    // ===========================================================================
    ...
    for (int endcap = kMinEndcap; endcap <= kMaxEndcap; ++endcap) {
        for (int sector = kMinTrigSector; sector <= kMaxTrigSector; ++sector) {
            sector_processors_.push_back(
                std::make_unique<SectorProcessor>(context_, endcap, sector)
            );
        }
    }
}
