...
void CSCTPSelector::select(
        const TriggerPrimitive& tp,
        TPInfo tp_info,
        ILinkTPCMap& ilink_tpc_map
) const {
...
// Utils
// ===========================================================================
int CSCTPSelector::get_input_link(
        const TriggerPrimitive& tp, 
        TPInfo& tp_info
) const {
    int ilink = -1;
    ...
    if (
            csc::is_in_sector(endcap_, sector_, tp_endcap, tp_sector)
    ) {
        tp_selection = TPSelection::kNative;
    } else if (
            CONFIG.include_neighbor_en_
    ...
    ilink = calculate_input_link(
            tp_subsector, tp_station, 
            tp_ring, tp_csc_id,
            tp_selection
    );
...
int CSCTPSelector::calculate_input_link(
        const int& tp_subsector, const int& tp_station, 
        const int& tp_ring, const int& tp_csc_id,
        const TPSelection& tp_selection
) const {
    int ilink = -1;
