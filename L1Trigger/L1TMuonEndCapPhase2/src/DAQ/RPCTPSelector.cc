...
RPCTPSelector::RPCTPSelector(const EMTFContext& context, const int& endcap, const int& sector)
    : context_(context), endcap_(endcap), sector_(sector) {
  // Do Nothing
}

RPCTPSelector::~RPCTPSelector() {
  // Do Nothing
}

void RPCTPSelector::select(const TriggerPrimitive& tp, TPInfo tp_info, ILinkTPCMap& ilink_tpc_map) const {
  emtf_assert(tp.subsystem() == L1TMuon::kRPC);

  // Map RPC trigger primitives to input links
  int ilink = get_input_link(tp, tp_info);  // Returns RPC "link" index
...
// Utils
// ===========================================================================
int RPCTPSelector::get_input_link(const TriggerPrimitive& tp, TPInfo& tp_info) const {
  int ilink = -1;
  ...
  const int tp_emtf_sect = tp_data.emtf_sector;
  const bool tp_is_CPPF = tp_data.isCPPF;

  // Short-Circuit: In neighbor chambers, have two separate CPPFDigis for the two EMTF sectors
  if (tp_is_CPPF && (tp_emtf_sect != sector_))
    return ilink;
...
int RPCTPSelector::calculate_input_link(const int& tp_subsector,
