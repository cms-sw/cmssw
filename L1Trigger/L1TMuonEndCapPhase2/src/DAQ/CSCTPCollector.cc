...
void CSCTPCollector::collect(const edm::Event& i_event, BXTPCMap& bx_tpc_map) const {
  ...
  for (auto& tp_entry : tpc) {
    ...
    if (!(tp_data.strip < max_strip)) {
      edm::LogWarning("L1T") << "Found error in LCT strip: " << tp_data.strip << " (allowed range: 0-" << max_strip - 1
                             << ").";
    ...
    if (!(tp_data.keywire < max_wire)) {
      edm::LogWarning("L1T") << "Found error in LCT wire: " << tp_data.keywire << " (allowed range: 0-" << max_wire - 1
                             << ").";
