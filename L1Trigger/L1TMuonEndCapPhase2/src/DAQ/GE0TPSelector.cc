...
void GE0TPSelector::select(const TriggerPrimitive& tp, TPInfo tp_info, ILinkTPCMap& ilink_tpc_map) const {
  ...
  int ilink = getInputLink(tp, tp_info);  // Returns GE0 "link" index
...
// Utils
// ===========================================================================
int GE0TPSelector::getInputLink(const TriggerPrimitive& tp, TPInfo& tp_info) const {
  int ilink = -1;
...
int GE0TPSelector::calcInputLink(const int& tp_subsector, const int& tp_csc_id, const TPSelection& tp_selection) const {
  int ilink = -1;
