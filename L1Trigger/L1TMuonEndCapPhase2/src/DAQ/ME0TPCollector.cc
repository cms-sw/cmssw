...
ME0TPCollector::ME0TPCollector(const EMTFContext& context, edm::ConsumesCollector& i_consumes_collector)
    : context_(context),
      input_token_(i_consumes_collector.consumes<ME0Tag::collection_type>(
          context.pset_.getParameter<edm::InputTag>("ME0Input"))) {
  // Do Nothing
}

ME0TPCollector::~ME0TPCollector() {
  // Do Nothing
}

void ME0TPCollector::collect(const edm::Event& i_event, BXTPCMap& bx_tpc_map) const {
  // Constants
  ...
  edm::Handle<ME0Tag::collection_type> me0_digis;
  ...
  for (auto& tp_entry : tpc) {
    const auto& tp_det_id = tp_entry.tp_.detId<ME0DetId>();
    ...
    if (tp_endcap == 1) {
      ...
      if (tp_phiposition < phiposition_q1) {
        tp_chamber = csc::next_10deg_chamber(tp_chamber);
      } else if (tp_phiposition < phiposition_q3) {
        // Do nothing
      } else {
        tp_chamber = csc::prev_10deg_chamber(tp_chamber);
      }
    } else {
      // negative endcap
