...
void SectorProcessor::select(
    ...
    if (tp_selectors_it == tp_selectors_.end()) {
        edm::LogWarning("L1T") 
            << "TPCollector has been implemented, "
            << "but there is no TPSelector for "
            << tp_subsystem;
        return;
    }
...
void SectorProcessor::copy_tp(
        const ILinkTPCMap& source,
        ILinkTPCMap& target
) const {
...
void SectorProcessor::convert_tp(
        const int& initial_hit_id, 
        const ILinkTPCMap& ilink_tpc_map,
        ...
        for (const auto& tp_entry : ilink_tpc) { // loop trigger primitives
            ...
            if (tp_converters_it == tp_converters_.end()) {
                edm::LogWarning("L1T") << "TPCollector & TPSelector have been implemented, "
                    << "but there is no TPConverter for "
                    << tp_subsystem;
                continue;
            }
    ...
    hits.insert(
            hits.end(), 
            std::make_move_iterator(substitutes.begin()), 
            std::make_move_iterator(substitutes.end())
    );
...
void SectorProcessor::populate_segments(
        const std::vector<EMTFHitCollection>& bx_window_hits,
        std::map<int, int>& seg_to_hit,
        segment_collection_t& segments
) {
    // Initialize
    ...
    for (; bx_window_hits_rit != bx_window_hits_rend; ++bx_window_hits_rit) { // Begin loop from latest BX Collection to oldest BX Hit Collection
        ...
        for (const auto& hit : bx_hits) { // Begin loop hits in BX
            ...
            if (CONFIG.verbosity_ > 1) {
                std::cout
                    ...
                    << " Sector: "          <<  sector_
                    << " BX: "              <<  (*bx_)
                    << " Hit iLink: "       <<  hit_chamber
                    << " Hit iSeg: "        <<  ch_seg
                    << " Hit Host "         <<  hit_host
                    << " Hit Rel BX "       <<  (hit_bx - *bx_)
                    << " Hit Timezones "    <<  hit_timezones
                    << std::endl;

                std::cout
                    << " id "               <<  seg_id
                    << " phi "              <<  segments[seg_id].phi     
                    << " bend "             <<  segments[seg_id].bend    
                    << " theta1 "           <<  segments[seg_id].theta1  
                    << " theta2 "           <<  segments[seg_id].theta2  
                    << " qual1 "            <<  segments[seg_id].qual1   
                    << " qual2 "            <<  segments[seg_id].qual2   
                    << " time "             <<  segments[seg_id].time    
                    << " zones "            <<  segments[seg_id].zones   
                    << " timezones "        <<  segments[seg_id].tzones  
                    << " cscfr "            <<  segments[seg_id].cscfr   
                    << " layer "            <<  segments[seg_id].layer   
                    << " bx "               <<  segments[seg_id].bx      
                    << " valid "            <<  segments[seg_id].valid   
                    << std::endl;
            }
...
void SectorProcessor::build_tracks(
    ...
    context_.output_layer_.apply(
            endcap_, sector_, *bx_,
            seg_to_hit, tracks, displaced_en,
            bx_tracks
    );
