...
void EMTFContext::update(
    ...
    prompt_graph_ptr_ = tensorflow::loadGraphDef(
            edm::FileInPath(config_.prompt_graph_path_).fullPath()
    );

    prompt_session_ptr_ = tensorflow::createSession(
            prompt_graph_ptr_
    );
    ...
    disp_graph_ptr_ = tensorflow::loadGraphDef(
            edm::FileInPath(config_.displ_graph_path_).fullPath()
    );

    disp_session_ptr_ = tensorflow::createSession(
            disp_graph_ptr_
    );
