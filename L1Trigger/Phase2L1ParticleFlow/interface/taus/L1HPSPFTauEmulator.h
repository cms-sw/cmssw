...
namespace L1HPSPFTauEmu {
  ...
  typedef ap_int<10> dz_t; //type for z0 cut
    ...
    public:
      ...
      Particle( pt_t pt, etaphi_t eta, etaphi_t phi, type_t id, dz_t z0 ) : hwPt(pt), hwEta(eta), hwPhi(phi), hwId(id), hwZ0(z0) {}
...
ap_uint<12> int_abs(ap_int<12> x){
   ap_uint<12> result;
   if (x < 0){ result = -x;}
   else{ result = x;}
   return result;
...
ap_uint<10> dz_abs(ap_int<10> x){
  ...
  ap_uint<20> setSConeSize2(pt_t total_pt) {
  ...
  bool inSignalCone(Particle part, Particle seed, const int track_count, ap_uint<20> cone2, pt_t& iso_pt, bool& isLead) {
      //finds the signal cone candidates (including strip pt check 
        ...
        bool isPotentialLead = 0;
 
        isPotentialLead = is_charged(part) && part.id() != 4 && part.pt() > min_leadChargedPfCand_pt && int_abs(part.eta()) < etaCutoff;
        ...
        if(part.pt() !=0){
          ...
          if( (int_abs(deltaEta) < strip_eta) && (int_abs(deltaPhi) < strip_phi) && 
              ( part.id() == 3 || (part.id() == 1)) ){
              if(isPotentialLead){isLead = true;} 
              inCone = true;
            
            }
          else if( ((deltaEta2 + deltaPhi2) < cone2) && !( (part.id() == 0) && ( track_count >3)) && !(is_charged(part) && dz_abs(deltaZ) > dzCut)){
              if(isPotentialLead){isLead = true;}
              inCone = true;
          }
          else{
            
            if( is_charged(part) && dz_abs(deltaZ) > dzCut){
               iso_pt += part.pt();
               inCone = false;
            }                
  ...
  Tau makeHPSTauHW(const std::vector<Particle>& parts, const Particle seed, const pt_t total_pt/*, Config config*/){
...
std::vector<Tau> emulateEvent(std::vector<Particle>& parts, std::vector<Particle>& jets, bool jEnable){  
   ...
   //sorting by pt
   std::sort(parts_copy.begin(), parts_copy.end(), [](Particle i, Particle j){
         return( i.pt() > j.pt());
   });
  ...
  std::sort(jets_copy.begin(), jets_copy.end(), [](Particle i, Particle j){
      return(i.pt() > j.pt());
  });
...
//add in the jet taus 
  ...
  for(std::vector<int>::size_type i = 0; i != jseeds.size(); i++){
   ...
   while(iso_index < parts_copy.size()){
     ...
     if(inIsolationCone(isoCand, jseed)  ){
       if(is_charged(isoCand) && isoCand.pt() > max_pt_j){
         if(isoCand.z0()){jseed.z0() = isoCand.z0();}
         max_pt_j = isoCand.pt();
       }
   ...
   std::sort(taus.begin(), taus.end(), [](Tau i, Tau j){
      return(i.pt() > j.pt());
   } );      
