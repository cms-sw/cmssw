...
private:
  ...
  struct JetInput {
    ...
    JetInput() {}
    JetInput(const std::string &n,
             const edm::EDGetTokenT<edm::View<reco::LeafCandidate>> &t,
             const l1tpf::SimpleCorrEm &c)
        : name(n), token(t), jec(c) {}
  };
  ...
  struct JetBranch {
    ...
    void fill(const edm::View<reco::LeafCandidate> &jets,
              const l1tpf::SimpleCorrEm &corr,
              const StringCutObjectSelector<reco::LeafCandidate> &sel) {
      count_raw = 0;
      ht_raw = 0;
      count_corr = 0;
      ht_corr = 0;
      ...
      for (reco::LeafCandidate jet : jets) {
        if (sel(jet)) {
          count_raw++;
          ht_raw += std::min(jet.pt(), 500.);
          sum_raw += jet.p4();
        }
        jet.setP4(
            reco::Particle::PolarLorentzVector(corr(jet.pt(), std::abs(jet.eta())), jet.eta(), jet.phi(), jet.mass()));
        if (sel(jet)) {
          count_corr++;
          ht_corr += std::min(jet.pt(), 500.);
          sum_corr += jet.p4();
        }
      }
      mht_raw = sum_raw.Pt();
  ...
  struct MetBranch {
    ...
    void fill(const edm::View<reco::LeafCandidate> &recoMets) {
      met = 0;
      met_phi = 0;
      for (reco::LeafCandidate recoMet : recoMets) {
        met = recoMet.et();
        met_phi = recoMet.phi();
      }
    }
  ...
  struct SpecialBranch {
    ...
    SpecialBranch(const std::string &n,
                  const edm::EDGetTokenT<edm::View<reco::Candidate>> &t,
                  const std::string &cut,
                  const std::string &expr)
        : name(n), token(t), sel(cut, true), func(expr, true) {}
    void makeBranches(TTree *tree) { tree->Branch((name).c_str(), &val, (name + "/F").c_str()); }
...
L1TPFJetMetTreeProducer::L1TPFJetMetTreeProducer(const edm::ParameterSet &iConfig) {
  ...
  for (const std::string &name : jets.getParameterNamesForType<edm::InputTag>()) {
    jets_.emplace_back(name,
                       consumes<edm::View<reco::LeafCandidate>>(jets.getParameter<edm::InputTag>(name)),
                       l1tpf::SimpleCorrEm(jecs, name));
  }
  ...
  for (auto &b : jetbranches_)
    b.makeBranches(tree_);

  jetCollbranches_.reserve(jets_.size());
  for (const auto &j : jets_)
    jetCollbranches_.emplace_back(j.name);
  for (auto &b : jetCollbranches_)
    b.makeBranches(tree_);
  ...
  for (const auto &m : mets_)
    metbranches_.emplace_back(m.name);
  for (auto &b : metbranches_)
    b.makeBranches(tree_);
...
void L1TPFJetMetTreeProducer::analyze(const edm::Event &iEvent, const edm::EventSetup &iSetup) {
  run_ = iEvent.id().run();
