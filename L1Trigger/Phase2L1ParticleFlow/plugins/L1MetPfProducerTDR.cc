...
class L1MetPfProducerTDR : public edm::global::EDProducer<> {
public:
  explicit L1MetPfProducerTDR(const edm::ParameterSet&);
  ~L1MetPfProducerTDR() override;
...
L1MetPfProducerTDR::L1MetPfProducerTDR(const edm::ParameterSet& cfg)
...
void L1MetPfProducerTDR::produce(edm::StreamID, 
                              edm::Event& iEvent, 
...
void L1MetPfProducerTDR::CalcMetHLS(std::vector<float> pt, std::vector<float> phi,
                                    reco::Candidate::PolarLorentzVector &metVector) const{
    pxy_t hw_px = 0;
    pxy_t hw_py = 0;
    pxy_t hw_sumx = 0;
...
void L1MetPfProducerTDR::CalcMetFast(std::vector<float> pt_vec, std::vector<float> phi_vec,
                                    reco::Candidate::PolarLorentzVector &metVector) const{
    float sumx = 0;
    float sumy = 0;
...
void L1MetPfProducerTDR::init_projx_table(std::vector<pt_t> table_out) {
...
void L1MetPfProducerTDR::ProjX(pt_t pt, phi_t phi, pxy_t &x, bool debug) const{
...
void L1MetPfProducerTDR::init_projy_table(std::vector<pt_t> table_out) {
    for (uint i = 0; i < sinCosTableSize_; i++) {
...
void L1MetPfProducerTDR::ProjY(pt_t pt, phi_t phi, pxy_t &y, bool debug) const{
...
void L1MetPfProducerTDR::init_inv_table(std::vector<pt_t> table_out) {
...
void L1MetPfProducerTDR::init_atan_table(std::vector<pt_t> table_out) {
    // multiply result by 1=2^(PT-SIZE)
    table_out[0]=int(0);
...
void L1MetPfProducerTDR::PhiFromXY(pxy_t px, pxy_t py, phi_t &phi) const{
...
L1MetPfProducerTDR::~L1MetPfProducerTDR() {}

#include "FWCore/Framework/interface/MakerMacros.h"
DEFINE_FWK_MODULE(L1MetPfProducerTDR);
