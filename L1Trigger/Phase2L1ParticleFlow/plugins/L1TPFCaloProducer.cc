...
void L1TPFCaloProducer::produce(edm::Event &iEvent, const edm::EventSetup &iSetup) {
  ...
  for (const auto &token : ecalCands_) {
    ...
    for (const reco::Candidate &it : *ecals) {
      if (debug_)
        std::cout << "L1TPFCaloProducer: adding ECal input pt " << it.pt() << ", eta " << it.eta() << ", phi "
                  << it.phi() << std::endl;
  ...
  if (!ecalOnly_) {
    ...
    for (const auto &token : hcalCands_) {
      ...
      for (const reco::Candidate &it : *hcals) {
        if (debug_)
          std::cout << "L1TPFCaloProducer: adding HCal cand input pt " << it.pt() << ", eta " << it.eta() << ", phi "
                    << it.phi() << std::endl;
  ...
  if (debug_) {
    for (const l1t::PFCluster &it : *corrEcal) {
      std::cout << "L1TPFCaloProducer: corrected ECal cluster pt " << it.pt() << ", eta " << it.eta() << ", phi "
                << it.phi() << std::endl;
    }
  }
  ...
  if (hadCorrector_.valid()) {
    caloLinker_->correct([&](const l1tpf_calo::CombinedCluster &c) -> float {
      if (debug_)
        std::cout << "L1TPFCaloProducer: raw linked cluster pt " << c.et << ", eta " << c.eta << ", phi " << c.phi
                  << ", emPt " << c.ecal_et << std::endl;
  ...
  for (l1t::PFCluster &c : *clusters) {
    ...
    if (debug_)
      std::cout << "L1TPFCaloProducer: calibrated linked cluster pt " << c.pt() << ", eta " << c.eta() << ", phi "
                << c.phi() << ", emPt " << c.emEt() << std::endl;
  }
  ...
  for (const auto &token : hcalDigis_) {
    ...
    for (const auto &itr : *hcalTPs) {
      ...
      if (!hcalDigisHF_ && std::abs(towerEta) > 2)
        continue;
      if (debug_)
        std::cout << "L1TPFCaloProducer: adding HCal digi input pt " << et << ", eta " << towerEta << ", phi "
                  << towerPhi << std::endl;
...
void L1TPFCaloProducer::readHcalHGCTowers_(edm::Event &iEvent, const edm::EventSetup &iSetup) {
  ...
  for (const auto &token : hcalHGCTowers_) {
    ...
    for (auto it = hgcTowers->begin(0), ed = hgcTowers->end(0); it != ed; ++it) {
      if (debug_)
        std::cout << "L1TPFCaloProducer: adding HGC Tower hadEt " << it->etHad() << ", emEt " << it->etEm() << ", pt "
                  << it->pt() << ", eta " << it->eta() << ", phi " << it->phi() << std::endl;
