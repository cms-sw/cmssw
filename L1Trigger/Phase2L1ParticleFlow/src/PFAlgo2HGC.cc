...
void PFAlgo2HGC::link_tk2mu(Region &r, std::vector<int> &tk2mu, std::vector<int> &mu2tk) const {
  ...
  for (int imu = 0, nmu = r.muon.size(); imu < nmu; ++imu) {
    ...
    if (debug_)
      printf("PFAlgo2HGC \t muon  %3d (pt %7.2f, eta %+5.2f, phi %+5.2f) \n",
             imu,
             mu.floatPt(),
             mu.floatEta(),
             mu.floatPhi());
    ...
    if (debug_ && imatch > -1)
      printf("PFAlgo2HGC \t muon  %3d (pt %7.2f) linked to track %3d (pt %7.2f)\n",
             imu,
             mu.floatPt(),
             imatch,
             r.track[imatch].floatPt());
    if (debug_ && imatch == -1)
      printf("PFAlgo2HGC \t muon  %3d (pt %7.2f) not linked to any track\n", imu, mu.floatPt());
    mu2tk[imu] = imatch;
    if (imatch > -1) {
      tk2mu[imatch] = imu;
...
void PFAlgo2HGC::link_tk2calo(Region &r, std::vector<int> &tk2calo) const {
  ...
  for (int itk = 0, ntk = r.track.size(); itk < ntk; ++itk) {
    ...
    if (debug_)
      printf("PFAlgo2HGC \t track %3d (pt %7.2f) to be matched to calo, min pT %7.2f\n", itk, tk.floatPt(), minCaloPt);
    for (int ic = 0, nc = r.calo.size(); ic < nc; ++ic) {
      auto &calo = r.calo[ic];
    ...
    if (debug_ && tk2calo[itk] != -1)
      printf("PFAlgo2HGC \t track %3d (pt %7.2f) matches to calo %3d (pt %7.2f) with dist %.3f (dr %.3f)\n",
             itk,
             tk.floatPt(),
             tk2calo[itk],
...
void PFAlgo2HGC::unlinkedtk_algo(Region &r, const std::vector<int> &tk2calo) const {
  ...
  for (int itk = 0, ntk = r.track.size(); itk < ntk; ++itk) {
    ...
    if (tk.floatPt() < maxPt) {
      if (debug_)
        printf("PFAlgo2HGC \t track %3d (pt %7.2f) not matched to calo, kept as charged hadron\n", itk, tk.floatPt());
...
void PFAlgo2HGC::calo_relink(Region &r,
  ...
  for (int ic = 0, nc = r.calo.size(); ic < nc; ++ic) {
    if (addtopt[ic]) {
      auto &calo = r.calo[ic];
      if (debug_)
        printf("PFAlgo2HGC \t calo  %3d (pt %7.2f, sum tk pt %7.2f) is increased to pt %7.2f after merging\n",
               ic,
               calo.floatPt(),
               calo2sumtkpt[ic],
  ...
  for (int ic = 0, nc = r.calo.size(); ic < nc; ++ic) {
    ...
    if (ptdiff > +ptMatchHigh_ * pterr) {
      if (ecalPriority_) {
        ...
        if (ptdiff > 2) {
          if (debug_)
            printf("PFAlgo2HGC \t calo  %3d (pt %7.2f, empt %7.2f)    ---> make also neutral hadron with pt %7.2f\n",
                   ic,
                   calo.floatPt(),
                   calo.floatEmPt(),
                   ptdiff);
        ...
        if (debug_)
          printf("PFAlgo2HGC \t calo  %3d (pt %7.2f)    ---> promoted to neutral with pt %7.2f\n",
                 ic,
                 calo.floatPt(),
                 ptdiff);
        auto &p = addCaloToPF(r, calo);
      ...
      if (debug_ && rescaleTracks_)
        printf("PFAlgo2HGC \t calo  %3d (pt %7.2f)    ---> tracks overshoot and will be scaled down by %.4f\n",
               ic,
               calo.floatPt(),
               calo2alpha[ic]);
      if (debug_ && !rescaleTracks_)
        printf("PFAlgo2HGC \t calo  %3d (pt %7.2f)    ---> tracks overshoot by %.4f\n",
               ic,
               calo.floatPt(),
  ...
  for (int itk = 0, ntk = r.track.size(); itk < ntk; ++itk) {
    ...
    if (calo.hwFlags == 1) {
      ...
      if (calo2ntk[tk2calo[itk]] == 1 && caloTrkWeightedAverage_) {
        ...
        p.hwStatus = GoodTK_Calo_TkPt;
        if (debug_)
          printf("PFAlgo2HGC \t track %3d (pt %7.2f) linked to calo %3d promoted to %s\n",
                 itk,
                 tk.floatPt(),
                 tk2calo[itk],
...
void PFAlgo2HGC::unlinkedcalo_algo(Region &r) const {
  ...
  for (int ic = 0, nc = r.calo.size(); ic < nc; ++ic) {
    if (!r.calo[ic].used) {
      ...
      if (debug_)
        printf("PFAlgo2HGC \t calo  %3d (pt %7.2f) not linked, promoted to neutral\n", ic, r.calo[ic].floatPt());
    }
  }
}
...
void PFAlgo2HGC::save_muons(Region &r, const std::vector<int> &tk2mu) const {
  ...
  for (int itk = 0, ntk = r.track.size(); itk < ntk; ++itk) {
    if (r.track[itk].muonLink) {
      ...
      if (debug_)
        printf("PFAlgo2HGC \t track %3d (pt %7.2f) promoted to muon.\n", itk, r.track[itk].floatPt());
    }
  }
}
