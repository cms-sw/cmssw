...
unsigned int l1tpf_impl::Region::nOutput(OutputType type, bool usePuppi) const {
    ...
    for (const auto &p : (usePuppi ? puppi : pf)) {
        ...
        switch(type) {
            ...
            case electron_type: if (p.hwId == l1t::PFCandidate::Electron) ret++; break;
            case pfmuon_type: if (p.hwId == l1t::PFCandidate::Muon) ret++; break;
            case charged_hadron_type: if (p.hwId == l1t::PFCandidate::ChargedHadron) ret++; break;
            case neutral_hadron_type: if (p.hwId == l1t::PFCandidate::NeutralHadron) ret++; break;
            case photon_type: if (p.hwId == l1t::PFCandidate::Photon) ret++; break;
            case n_output_types: assert(false);
        }
    }
    return ret;
