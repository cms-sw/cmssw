...
void l1ct::PFAlgo3Emulator::toFirmware(const PFInputRegion & in, PFRegion & region, HadCaloObj calo[/*nCALO*/], EmCaloObj emcalo[/*nEMCALO*/], TkObj track[/*nTRACK*/], MuObj mu[/*nMU*/]) const {
    region = in.region;
...
void l1ct::PFAlgo3Emulator::toFirmware(const OutputRegion & out, PFChargedObj outch[/*nTRACK*/], PFNeutralObj outpho[/*nPHOTON*/], PFNeutralObj outne[/*nSELCALO*/], PFChargedObj outmu[/*nMU*/]) const {
...
int l1ct::PFAlgo3Emulator::tk_best_match_ref(unsigned int dR2MAX, const std::vector<l1ct::EmCaloObjEmu> & calo, const l1ct::TkObjEmu & track) const {
...
int l1ct::PFAlgo3Emulator::em_best_match_ref(unsigned int dR2MAX, const std::vector<l1ct::HadCaloObjEmu> & calo, const l1ct::EmCaloObjEmu & em) const {
...
void l1ct::PFAlgo3Emulator::pfalgo3_em_ref(const PFInputRegion & in, const std::vector<int> & iMu/*[nTRACK]*/, std::vector<int> & iEle/*[nTRACK]*/, OutputRegion & out, std::vector<HadCaloObjEmu> & hadcalo_out/*[nCALO]*/) const {
    // constants
    ...
    for (unsigned int ic = 0; ic < nEMCALO; ++ic) {  calo_sumtk[ic] = 0; }
    ...
    for (unsigned int ic = 0; ic < nEMCALO; ++ic) {
        ...
        if (calo_sumtk[ic] > 0) {
            ...
            if ((ptdiff >= 0 && ptdiff2 <= sigma2Hi) || (ptdiff < 0 && ptdiff2 < sigma2Lo)) {
                ...
                photonPt = 0; 
                isEM[ic] = true;
                if (debug_) printf("FW  \t emcalo %3d pt %8.2f ptdiff %8.2f [match window: -%.2f / +%.2f] flagged as electron\n", ic, in.emcalo[ic].floatPt(), Scales::floatPt(ptdiff), std::sqrt(Scales::floatPt(sigma2Lo)), std::sqrt(float(sigma2Hi)));
            } else if (ptdiff > 0) {
                // electron + photon
                photonPt = ptdiff; 
                isEM[ic] = true;
                if (debug_) printf("FW  \t emcalo %3d pt %8.2f ptdiff %8.2f [match window: -%.2f / +%.2f] flagged as electron + photon of pt %8.2f\n", ic, in.emcalo[ic].floatPt(), Scales::floatPt(ptdiff), std::sqrt(Scales::floatPt(sigma2Lo)), std::sqrt(float(sigma2Hi)), Scales::floatPt(photonPt));
            } else {
                // pion
                photonPt = 0;
                isEM[ic] = false;
                if (debug_) printf("FW  \t emcalo %3d pt %8.2f ptdiff %8.2f [match window: -%.2f / +%.2f] flagged as pion\n", ic, in.emcalo[ic].floatPt(), Scales::floatPt(ptdiff), std::sqrt(Scales::floatPt(sigma2Lo)), std::sqrt(Scales::floatPt(sigma2Hi)));
            }
        } else {
            // photon
            isEM[ic] = true;
            photonPt = in.emcalo[ic].hwPt;
            if (debug_ && in.emcalo[ic].hwPt > 0) printf("FW  \t emcalo %3d pt %8.2f flagged as photon\n", ic, in.emcalo[ic].floatPt());
        }
        if (photonPt) {
    ...
    for (unsigned int it = 0; it < nTRACK; ++it) {
        ...
        if (debug_ && (iEle[it]!=-1)) printf("FW  \t track  %3d pt %8.2f flagged as electron (emcluster %d).\n", it, in.track[it].floatPt(), iEle[it]);
    }
    ...
    for (unsigned int ic = 0; ic < nEMCALO; ++ic) {
        ...
        if (debug_ && (in.emcalo[ic].hwPt > 0)) {
             printf("FW  \t emcalo %3d pt %8.2f isEM %d matched to hadcalo %3d pt %8.2f emPt %8.2f isEM %d\n", 
                                ic, in.emcalo[ic].floatPt(), int(isEM[ic]), em2calo[ic], (em2calo[ic] >= 0 ? in.hadcalo[em2calo[ic]].floatPt() : -1), 
                                (em2calo[ic] >= 0 ? in.hadcalo[em2calo[ic]].floatEmPt() : -1), (em2calo[ic] >= 0 ? int(in.hadcalo[em2calo[ic]].hwIsEM) : 0));
        }
    }
    ...
    for (unsigned int ih = 0; ih < nCALO; ++ih) {
        ...
        for (unsigned int ic = 0; ic < nEMCALO; ++ic) {
            if (em2calo[ic] == int(ih)) {
                if (isEM[ic]) sub += in.emcalo[ic].hwPt;
                else keep = true;
            }
        }
        ...
        if (debug_ && (in.hadcalo[ih].hwPt > 0)) {
            printf("FW  \t calo   %3d pt %8.2f has a subtracted pt of %8.2f, empt %8.2f -> %8.2f   isem %d mustkeep %d \n",
                        ih, in.hadcalo[ih].floatPt(), Scales::floatPt(alldiff), in.hadcalo[ih].floatEmPt(), Scales::floatPt(emdiff), int(in.hadcalo[ih].hwIsEM), keep);
                    
        }
        if (alldiff <= ( in.hadcalo[ih].hwPt >>  4 ) ) {
            hadcalo_out[ih].hwPt = 0;   // kill
            hadcalo_out[ih].hwEmPt = 0; // kill
            if (debug_ && (in.hadcalo[ih].hwPt > 0)) printf("FW  \t calo   %3d pt %8.2f --> discarded (zero pt)\n", ih, in.hadcalo[ih].floatPt());
        } else if ((in.hadcalo[ih].hwIsEM && emdiff <= ( in.hadcalo[ih].hwEmPt >> 3 )) && !keep) {
            hadcalo_out[ih].hwPt = 0;   // kill
            hadcalo_out[ih].hwEmPt = 0; // kill
            if (debug_ && (in.hadcalo[ih].hwPt > 0)) printf("FW  \t calo   %3d pt %8.2f --> discarded (zero em)\n", ih, in.hadcalo[ih].floatPt());
        } else {
...
void l1ct::PFAlgo3Emulator::run(const PFInputRegion & in, OutputRegion & out) const {
    ...
    for (unsigned int it = 0; it < nTRACK; ++it) {
        if (track_good[it]) {
            ...
            if (iEle[it] != -1) out.pfcharged[it].srcCluster = in.emcalo[iEle[it]].src;
