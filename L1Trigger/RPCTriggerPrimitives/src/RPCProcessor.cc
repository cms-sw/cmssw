    ...
    for (auto& own : recHits) {
      ...
      if (ApplyLinkBoardCut_ == true) {
        ...
        if (region_ == 0) {  //For Barrel
          ...
          if (PassLinkCutBarrel)
            recHit_output.push_back(own);
          ...
          if (PassLinkCutEndCap)
            recHit_output.push_back(own);
        }

      } else
        recHit_output = recHits;
...
edm::OwnVector<RPCRecHit> RPCProcessor::ApplyClusterSizeCut(const edm::OwnVector<RPCRecHit> recHits_,
                                                            int ClusterSizeCut_) {
...
bool RPCProcessor::ApplyLinkBoardCut(int NClusters, int LinkboardCut) {
  ...
  if (NClusters > LinkboardCut)
    passCutsize = false;

  return passCutsize;
}

std::string RPCProcessor::GetStringBarrel(const int ring_,
                                          const int station_,
                                          const int sector_,
                                          const int layer_,
