...
OmtfName::OmtfName(unsigned int iProcesor, const OMTFConfiguration* omtfConfig) {
  int board = 0;
  if (iProcesor < omtfConfig->nProcessors())  //OMTFn
    board = -iProcesor - 1;
  else  //OMTFp
    board = iProcesor - (omtfConfig->nProcessors() - 1);
