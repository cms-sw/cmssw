...
void PseudoBayesGrouping::run(Event& iEvent,
  ...
  if (debug && prelimMatches->size() > 0) {
    ...
    for (std::vector<CandidateGroup*>::iterator cand_it = prelimMatches->begin(); cand_it != prelimMatches->end();
         cand_it++) {
      std::cout << (*cand_it)->getNLayerhits() << ", " << (*cand_it)->getNisGood() << ", " << (*cand_it)->getNhits()
                << ", " << (*cand_it)->getQuality() << ", " << (*cand_it)->getCandId() << std::endl;
    }
  }
...
void PseudoBayesGrouping::FillMuonPaths(std::vector<MuonPath*>* mpaths) {
  ...
  for (std::vector<CandidateGroup*>::iterator itCand = finalMatches->begin(); itCand != finalMatches->end(); itCand++) {
    ...
    for (std::vector<DTPrimitive>::iterator itDTP = (*itCand)->getcandHits().begin();
         itDTP != (*itCand)->getcandHits().end();
         itDTP++) {
      if (debug)
  ...
  for (std::vector<DTPrimitive>::iterator dtPD_it = digisinLDown.begin(); dtPD_it != digisinLDown.end(); dtPD_it++) {
    ...
    for (std::vector<DTPrimitive>::iterator dtPU_it = digisinLUp.begin(); dtPU_it != digisinLUp.end(); dtPU_it++) {
      ...
      for (std::vector<Pattern*>::iterator pat_it = patterns.begin(); pat_it != patterns.end(); pat_it++) {
        ...
        if ((*pat_it)->GetSL1() != (LDown) || (*pat_it)->GetSL2() != (LUp) || (*pat_it)->GetDiff() != diff)
          continue;
        ...
        for (std::vector<DTPrimitive>::iterator dtTest_it = alldigis.begin(); dtTest_it != alldigis.end();
          ...
          if (((*pat_it)->LatHitIn(dtTest_it->getLayerId(), dtTest_it->getChannelId(), allowedVariance)) != -999) {
            if (((*pat_it)->LatHitIn(dtTest_it->getLayerId(), dtTest_it->getChannelId(), allowedVariance)) == -10)
              cand->AddHit((*dtTest_it), dtTest_it->getLayerId(), false);
            else
              cand->AddHit((*dtTest_it), dtTest_it->getLayerId(), true);
          }
        }
        if ((cand->getNhits() >= minNLayerHits &&
             (cand->getNLayerUp() >= minSingleSLHitsMax || cand->getNLayerDown() >= minSingleSLHitsMax) &&
             (cand->getNLayerUp() >= minSingleSLHitsMin && cand->getNLayerDown() >= minSingleSLHitsMin)) ||
            (allowUncorrelatedPatterns &&
             ((cand->getNLayerUp() >= minUncorrelatedHits && cand->getNLayerDown() == 0) ||
              (cand->getNLayerDown() >= minUncorrelatedHits && cand->getNLayerUp() == 0)))) {
          if (debug) {
...
void PseudoBayesGrouping::ReCleanPatternsAndDigis() {
  ...
  while (
      (prelimMatches->at(0)->getNLayerhits() >= minNLayerHits &&
       (prelimMatches->at(0)->getNLayerUp() >= minSingleSLHitsMax ||
        prelimMatches->at(0)->getNLayerDown() >= minSingleSLHitsMax) &&
       (prelimMatches->at(0)->getNLayerUp() >= minSingleSLHitsMin &&
        prelimMatches->at(0)->getNLayerDown() >= minSingleSLHitsMin)) ||
      (allowUncorrelatedPatterns &&
       ((prelimMatches->at(0)->getNLayerUp() >= minUncorrelatedHits && prelimMatches->at(0)->getNLayerDown() == 0) ||
        (prelimMatches->at(0)->getNLayerDown() >= minUncorrelatedHits && prelimMatches->at(0)->getNLayerUp() == 0)))) {
    ...
    for (std::vector<CandidateGroup*>::iterator cand_it = prelimMatches->begin(); cand_it != prelimMatches->end();
      ...
      if (*(*cand_it) == *(*itSel) && allowDuplicates)
        continue;
      for (std::vector<DTPrimitive>::iterator dt_it = (*itSel)->getcandHits().begin();
           dt_it != (*itSel)->getcandHits().end();
           dt_it++) {
