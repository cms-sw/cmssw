    ...
    if (this->getPrimitive(i)->isValidTime() && ptr->getPrimitive(i)->isValidTime()) {
      if (ptr->getPrimitive(i)->getSuperLayerId() != this->getPrimitive(i)->getSuperLayerId() ||
          ...
          /* Esta condición no debería cumplirse nunca ya que en un 'Segment'
		     ...
		     */
          ptr->getPrimitive(i)->getOrbit() != this->getPrimitive(i)->getOrbit() ||
          (ptr->getLateralComb())[i] != (this->getLateralComb())[i])
        return false;
    } else {
...
bool MuonPath::completeMP(void) {
  return (prim[0]->isValidTime() && prim[1]->isValidTime() && prim[2]->isValidTime() && prim[3]->isValidTime());
}
...
const LATERAL_CASES *MuonPath::getLateralComb(void) { return (lateralComb); }
...
void MuonPath::setDriftDistance(float dx, int cell) { xDriftDistance[cell] = dx; }
