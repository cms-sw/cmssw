    ...
    if (bestMatch_rpcRecHit) {
      ...
      if ((*dt_metaprimitive).quality < m_max_quality_to_overwrite_t0) {
        (*dt_metaprimitive).t0 =
            ...
            25 *
                shift_back;  // Overwriting t0 will propagate to BX since it is defined by round((*metaPrimitiveIt).t0/25.)-shift_back
...
void RPCIntegrator::makeRPCOnlySegments() {
  ...
  for (auto rpc_mp_it_layer1 = rpc_metaprimitives.begin(); rpc_mp_it_layer1 != rpc_metaprimitives.end();
       rpc_mp_it_layer1++) {
    ...
    if (rpc_id_l1.station() > 2 || rpc_id_l1.layer() != 1 || (rpc_mp_it_layer1->rpcFlag == 5 && !m_storeAllRPCHits))
    ...
    for (auto rpc_mp_it_layer2 = rpc_metaprimitives.begin(); rpc_mp_it_layer2 != rpc_metaprimitives.end();
         rpc_mp_it_layer2++) {
      ...
      if (rpc_id_l2.station() == rpc_id_l1.station() && rpc_id_l2.ring() == rpc_id_l1.ring() &&
          ...
          (rpc_mp_it_layer2->rpcFlag != 5 ||
           m_storeAllRPCHits)) {  // avoid building RPC only segment with a hit already matched to DT, except if one aske to store all RPC info
...
rpc_metaprimitive* RPCIntegrator::matchDTwithRPC(metaPrimitive* dt_metaprimitive) {
  ...
  int dt_bx =
      ...
      shift_back;  // because at this stage the BX of metaprimitive is not yet computed... will also have to subtract 20*25 ns because of the recent change
  ...
  for (auto rpc_mp_it = rpc_metaprimitives.begin(); rpc_mp_it != rpc_metaprimitives.end(); rpc_mp_it++) {
    ...
    if (rpc_det_id.ring() == dt_chId.wheel()  // ring() in barrel RPC corresponds to the wheel
      ...
      int delta_phi = (int)round(
          (getPhi_DT_MP_conv(rpc_mp_it->global_position.phi(), rpc_det_id.sector()) - dt_metaprimitive->phi) *
          m_dt_phiB_granularity);  // just a trick to apply the phi window cut on what could be accessed to fine tune it
...
double RPCIntegrator::getPhiBending(rpc_metaprimitive* rpc_hit_1, rpc_metaprimitive* rpc_hit_2) {
  ...
  GlobalPoint seg_middle_global =
      m_dtGeo->chamber(DT_chamber)->toGlobal(LocalPoint(average_x, 0., 0.));  // for station 1 and 2, z = 0
