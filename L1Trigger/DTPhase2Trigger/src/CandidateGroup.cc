...
void CandidateGroup::AddHit(DTPrimitive dthit, int lay, bool isGood) {
  ...
  if (quality != (quality | std::bitset<8>(power(2, lay))))
    nLayerhits++;
  if (isGood)
    nisGood++;
...
void CandidateGroup::RemoveHit(DTPrimitive dthit) {
  ...
  for (std::vector<DTPrimitive>::iterator dt_it = candHits.begin(); dt_it != candHits.end(); dt_it++) {
    if (dthit.getLayerId() == dt_it->getLayerId() && dthit.getChannelId() == dt_it->getChannelId()) {
      ...
      if (pattern->LatHitIn(dt_it->getLayerId(), dt_it->getChannelId(), 0) > -5)
        nisGood++;
      if (quality != (quality | std::bitset<8>(power(2, dt_it->getLayerId()))))
        nLayerhits++;
      quality = quality | std::bitset<8>(power(2, dt_it->getLayerId()));
      nhits++;
      if (dt_it->getLayerId() <= 3)
        nLayerDown++;
      else if (dt_it->getLayerId() >= 4)
        nLayerUp++;
...
bool CandidateGroup::operator>(const CandidateGroup& cOther) const {
  ...
  if (nisGood > cOther.getNisGood())
    return true;
  else if (nisGood < cOther.getNisGood())
    return false;
  //Tehn quality is number of layers fired
  else if (nLayerhits > cOther.getNLayerhits())
    return true;
  else if (nLayerhits < cOther.getNLayerhits())
    return false;
  //Then number of matched hits (if multiple hits in a layer is better)
  else if (nhits > cOther.getNhits())
    return true;
  else if (nhits < cOther.getNhits())
    return false;
  //Balanced quality, prefer 3+2 over 4+1 for example
  else if ((nLayerUp - nLayerDown) > (cOther.getNLayerUp() - cOther.getNLayerDown()))
    return true;
  else if ((nLayerUp - nLayerDown) < (cOther.getNLayerUp() - cOther.getNLayerDown()))
    return false;
  //Last, patterns with less gen hits are better matched
  else if (pattern->GetGenHits().size() < (cOther.getPattern()->GetGenHits().size()))
    return true;
  else if (pattern->GetGenHits().size() > (cOther.getPattern()->GetGenHits().size()))
    return false;
  //For a total order relation we need this additional dummy
  else if (candId < cOther.getCandId())
    return true;
  else if (candId > cOther.getCandId())
    return false;
