...
void MuonPathAnalyzerInChamber::setChiSquareThreshold(float ch2Thr) { chiSquareThreshold = ch2Thr; }
...
void MuonPathAnalyzerInChamber::analyze(MuonPath *inMPath, std::vector<MuonPath *> &outMPath) {
  ...
  if (debug)
    cout << inMPath->getNPrimitives() << endl;
  MuonPath *mPath = new MuonPath(*inMPath);

  if (debug) {
    std::cout << "DTp2::analyze, looking at mPath: " << std::endl;
    for (int i = 0; i < mPath->getNPrimitives(); i++)
      std::cout << mPath->getPrimitive(i)->getLayerId() << " , " << mPath->getPrimitive(i)->getSuperLayerId() << " , "
                << mPath->getPrimitive(i)->getChannelId() << " , " << mPath->getPrimitive(i)->getLaterality()
                << std::endl;
  }
  ...
  for (int i = 0; i < totalNumValLateralities; i++) {  // LOOP for all lateralities:
    ...
    if (mPath->getChiSq() > chiSquareThreshold)
      continue;
...
void MuonPathAnalyzerInChamber::buildLateralities(MuonPath *mpath) {
  ...
  for (int ilat = 0; ilat < NLayers; ilat++) {
    ...
    if (debug)
      std::cout << "[DEBUG] Input[" << ilat << "]: " << lr << std::endl;

    // If left/right fill number
    if (lr != NONE) {
      if (debug)
        std::cout << "[DEBUG]   - Adding it to " << lateralities.size() << " lists..." << std::endl;
...
void MuonPathAnalyzerInChamber::setWirePosAndTimeInMP(MuonPath *mpath) {
  ...
  if (debug)
    cout << "shift1=" << shiftinfo[wireId1.rawId()] << " shift3=" << shiftinfo[wireId3.rawId()] << endl;
  ...
  for (int i = 0; i < mpath->getNPrimitives(); i++) {
    if (mpath->getPrimitive(i)->isValidTime()) {
      if (i < 4)
        mpath->setXWirePos(10000 * shiftinfo[wireId1.rawId()] +
                               (mpath->getPrimitive(i)->getChannelId() + 0.5 * (double)((i + 1) % 2)) * delta,
                           i);
      if (i >= 4)
        mpath->setXWirePos(10000 * shiftinfo[wireId3.rawId()] +
                               (mpath->getPrimitive(i)->getChannelId() + 0.5 * (double)((i + 1) % 2)) * delta,
                           i);
    ...
    if (debug)
      cout << mpath->getPrimitive(i)->getTDCTime() << " ";
  }
  if (debug)
    cout << endl;
}
void MuonPathAnalyzerInChamber::calculateFitParameters(MuonPath *mpath,
  ...
  if (!(sign_tdriftvdrift == -1) && !(incell_tdriftvdrift == -1) && !(physical_slope == -1)) {
    ...
    if (debug)
      cout << "In fitPerLat "
           << "t0 " << mpath->getBxTimeValue() << " slope " << mpath->getTanPhi() << " pos " << mpath->getHorizPos()
           << " chi2 " << mpath->getChiSq() << " rawId " << mpath->getRawId() << endl;
  }
