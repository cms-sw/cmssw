...
private:
  ...
  void fillModule(const std::vector<HGCalDataFrame>&,
                  const std::vector<std::pair<DetId, uint32_t>>&,
                  const HGCalTriggerCellThresholdDataPayload&,
                  const HGCalTriggerCellThresholdDataPayload&,
                  const HGCalTriggerCellThresholdDataPayload&,
                  const std::map<HGCalDetId, double>&,
                  const std::unordered_map<uint32_t, double>&);
...
HGCalTriggerThresholdTriggerCellTester::HGCalTriggerThresholdTriggerCellTester(const edm::ParameterSet& conf)
    : inputee_(consumes<HGCalDigiCollection>(conf.getParameter<edm::InputTag>("eeDigis"))),
      ...
      inputbeselect_(
          consumes<l1t::HGCalTriggerCellBxCollection>(conf.getParameter<edm::InputTag>("beTriggerCellsSelect"))),
...
// SimHits_inputbh_(consumes<edm::PCaloHitContainer>(conf.getParameter<edm::InputTag>("bhSimHits")))
{
  ...
  if (is_Simhit_comp_) {
    hgcCellData_SimHitasso_ =
        fs_->make<TH1F>("hgcCellData_SimHitasso_", "Cell values with an associated SimHit", 1000, 0, 2000.);
    hgcCellSimHits_ = fs_->make<TH1F>("hgcCellSimHits_", "Cell simhit energies", 500, 0, 0.16);
    hgcCellData_vsSimHits_ =
        fs_->make<TH2F>("hgcCellData_vsSimHits_", "Cell values vs simhit energies", 500, 0, 0.16, 1000, 0., 2000.);
  }
  ...
  //
  hgcCellData_linampl_ =
      fs_->make<TH1F>("hgcCellData_linampl_", "Cell linearized amplitudes values All", 1000, 0, 70000);
  if (is_Simhit_comp_) {
    hgcCellData_linampl_vsSimHits_ = fs_->make<TH2F>("hgcCellData_linampl_vsSimHits_",
                                                     "Cell linearized amplitudes vs  simhit energies",
                                                     500,
                                                     0,
                                                     0.16,
                                                     1000,
                                                     0,
                                                     70000);
    hgcCellData_linampl_vsSimHits_zoom_ = fs_->make<TH2F>("hgcCellData_linampl_vsSimHits_zoom_",
                                                          "Cell linearized amplitudes vssimhit energies, zoomed",
                                                          1000,
                                                          0,
                                                          0.002,
                                                          1000,
  ...
  // HGC Trigger cells
  triggerCellData_noThreshold_ =
      fs_->make<TH1F>("triggerCellData_noThreshold_", "Trigger cell values, no threshold", 1000, 0., 70000.);
  if (is_Simhit_comp_) {
    triggerCellData_noThreshold_vsSimHits_ = fs_->make<TH2F>("triggerCellData_noThreshold_vsSimHits_",
                                                             "Trigger cell values vs simhit energies, no threshold",
                                                             500,
                                                             0,
                                                             0.16,
                                                             1000,
                                                             0.,
                                                             70000.);
    triggerCellSimHits_noThreshold_ =
        fs_->make<TH1F>("triggerCellSimHits_noThreshold", "Trigger cell simhit energies, no threshold", 500, 0, 0.16);
  }
  triggerCellData_Threshold_ =
      fs_->make<TH1F>("triggerCellData_Threshold_", "Trigger cell values, threshold", 1000, 0., 70000.);
  if (is_Simhit_comp_)
    triggerCellData_Threshold_vsSimHits_ = fs_->make<TH2F>("triggerCellData_Threshold_vsSimHits_",
                                                           "Trigger cell values vs simhit energies, threshold",
                                                           500,
                                                           0,
                                                           0.16,
                                                           1000,
  ...
  if (is_Simhit_comp_) {
    triggerCellData_vsSimHits_ = fs_->make<TH2F>(
        "triggerCellData_vsSimHits_", "Trigger cell values vs simhit energies", 500, 0, 0.16, 1100, 0., 1100.);
  }
  ...
  //
  selectedCellsVsAllCells_ee_ = fs_->make<TH2F>(
      "selectedCellsVsAllCells_ee", "Number of selected cells vs number of cell", 128, 0, 128, 128, 0., 128.);
  energyLossVsNCells_ee_ = fs_->make<TH2F>(
      "energyLossVsNCells_ee", "Relative energy loss after selection vs number of cell", 128, 0., 128., 101, 0, 1.01);
  selectedCellsVsAllCells_fh_ = fs_->make<TH2F>(
      "selectedCellsVsAllCells_fh", "Number of selected cells vs number of cell", 128, 0, 128, 128, 0., 128.);
  energyLossVsNCells_fh_ = fs_->make<TH2F>(
      "energyLossVsNCells_fh", "Relative energy loss after selection vs number of cell", 128, 0., 128., 101, 0, 1.01);
}

HGCalTriggerThresholdTriggerCellTester::~HGCalTriggerThresholdTriggerCellTester() {}

void HGCalTriggerThresholdTriggerCellTester::beginRun(const edm::Run& /*run*/, const edm::EventSetup& es) {
  es.get<CaloGeometryRecord>().get(triggerGeometry_);
}

void HGCalTriggerThresholdTriggerCellTester::analyze(const edm::Event& e, const edm::EventSetup& es) {
  checkSelectedCells(e, es);
  rerunThresholdFragments(e, es);
}

void HGCalTriggerThresholdTriggerCellTester::checkSelectedCells(const edm::Event& e, const edm::EventSetup& es) {
  ...
  for (auto cl_itr = be_triggercells_all.begin(0); cl_itr != be_triggercells_all.end(0); cl_itr++) {
    ...
    auto itr_insert =
        module_triggercells_all.emplace(std::make_tuple(zside, subdet, layer, module), std::vector<uint32_t>());
  ...
  for (auto cl_itr = be_triggercells_select.begin(0); cl_itr != be_triggercells_select.end(0); cl_itr++) {
    ...
    uint32_t layer = HGCalDetId(triggercell.detId()).layer();
    auto itr_insert =
        module_triggercells_select.emplace(std::make_tuple(zside, subdet, layer, module), std::vector<uint32_t>());
    itr_insert.first->second.emplace_back(triggercell.hwPt());
  }
  ...
  for (const auto& module_cells : module_triggercells_all) {
    ...
    for (const auto& energy : module_cells.second)
      energy_all += energy;
    for (const auto& energy : module_cells_select_itr->second)
    ...
    if (std::get<1>(module_cells.first) == ForwardSubdetector::HGCEE) {
      ...
      if (energy_all > 0)
        energyLossVsNCells_ee_->Fill(ncells_all, (double)energy_select / (double)energy_all);

    } else if (std::get<1>(module_cells.first) == ForwardSubdetector::HGCHEF) {
      selectedCellsVsAllCells_fh_->Fill(ncells_all, ncells_select);
      if (energy_all > 0)
        energyLossVsNCells_fh_->Fill(ncells_all, (double)energy_select / (double)energy_all);
    }
  }
}

void HGCalTriggerThresholdTriggerCellTester::rerunThresholdFragments(const edm::Event& e, const edm::EventSetup& es) {
  ...
  if (is_Simhit_comp_) {
    ...
    for (const auto& simhit : ee_simhits) {
      ...
      std::pair<int, int> recoLayerCell = triggerGeometry_->eeTopology().dddConstants().simToReco(
          cell, layer, sec, triggerGeometry_->eeTopology().detectorType());
    ...
    for (const auto& simhit : fh_simhits) {
      ...
      HGCalTestNumbering::unpackHexagonIndex(simid, subdet, zp, layer, sec, subsec, cell);
      mysubdet = (ForwardSubdetector)(subdet);
      std::pair<int, int> recoLayerCell = triggerGeometry_->fhTopology().dddConstants().simToReco(
          cell, layer, sec, triggerGeometry_->fhTopology().detectorType());
      cell = recoLayerCell.first;
      layer = recoLayerCell.second;
      if (layer < 0 || cell < 0) {
        continue;
  ...
  for (const auto& module_hits : hit_modules_ee) {
    ...
    fillModule(dataframes,
               linearized_dataframes,
               data_TCsums_woThreshold,
               data_TCsums_Threshold,
               datadecoded,
               simhit_energies,
               TC_simhit_energies);
...
void HGCalTriggerThresholdTriggerCellTester::fillModule(
    const std::vector<HGCalDataFrame>& dataframes,
    const std::vector<std::pair<DetId, uint32_t>>& linearized_dataframes,
    const HGCalTriggerCellThresholdDataPayload& fe_payload_TCsums_woThreshold,
    const HGCalTriggerCellThresholdDataPayload& fe_payload_TCsums_Threshold,
    const HGCalTriggerCellThresholdDataPayload& fe_payload,
  ...
  for (const auto& tc : fe_payload_TCsums_Threshold.payload) {
    if (tc.hwPt() > 0) {
      ...
      if (is_Simhit_comp_) {
        if (TC_simhit_energies.at(tc.detId()) > 0)
          triggerCellData_Threshold_vsSimHits_->Fill(TC_simhit_energies.at(tc.detId()), tc.hwPt());
      }
    }
  }
  ...
  for (const auto& tc : fe_payload.payload) {
    ...
    if (tc.hwPt() > 0) {
      ...
      if (is_Simhit_comp_) {
        if (TC_simhit_energies.at(tc.detId()) > 0)
          triggerCellData_vsSimHits_->Fill(TC_simhit_energies.at(tc.detId()), tc.hwPt());
      }
    }
  }
