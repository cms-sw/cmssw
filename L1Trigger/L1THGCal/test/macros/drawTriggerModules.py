            ...
            for boxline in thisboxlines:
                ...
                for line in triggercelllines:
                    if compare_lines(boxline, line):
                        ...
                        break
                if existingline:
                    triggercelllines.remove(existingline)
                else:
                ...
                for line in modulelines:
                    if compare_lines(boxline, line):
                        ...
                        break
                if existingline:
                    modulelines.remove(existingline)
                    #if module.module==2: print "Remove line in trigger cell", triggercell.triggercell
                else:
                    modulelines.append(boxline)
                    #if module.module==2: print "Adding line in trigger cell", triggercell.triggercell
            box.SetFillColor(0)
            box.SetLineColor(ROOT.kGray)
            box.Draw(drawstyle)
            boxes.append(box)
            if not "same" in drawstyle: drawstyle += " same"
        linesTrigger.extend(triggercelllines)
    linesModule.extend(modulelines)
for line in linesTrigger:
    line.Draw()
for line in linesModule:
...
canvas.Write()
canvas.Print("triggerCellMap.png")
