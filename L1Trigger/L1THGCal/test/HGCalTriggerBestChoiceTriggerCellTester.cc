...
private:
  ...
  void fillModule(const std::vector<HGCalDataFrame>&,
                  ...
                  const HGCalTriggerCellBestChoiceDataPayload&,
                  const HGCalTriggerCellBestChoiceDataPayload&,
                  const HGCalTriggerCellBestChoiceDataPayload&,
...
// SimHits_inputbh_(consumes<edm::PCaloHitContainer>(conf.getParameter<edm::InputTag>("bhSimHits")))
{
  ...
  // HGC Trigger cells
  triggerCellData_noBestChoice_ =
      fs_->make<TH1F>("triggerCellData_noBestChoice_", "Trigger cell values, no best choice", 1000, 0., 70000.);
  if (is_Simhit_comp_) {
    triggerCellData_noBestChoice_vsSimHits_ = fs_->make<TH2F>("triggerCellData_noBestChoice_vsSimHits_",
                                                              "Trigger cell values vs simhit energies, no best choice",
                                                              500,
                                                              0,
                                                              0.16,
                                                              1000,
                                                              0.,
                                                              70000.);
    triggerCellSimHits_noBestChoice_ = fs_->make<TH1F>(
        "triggerCellSimHits_noBestChoice", "Trigger cell simhit energies, no best choice", 500, 0, 0.16);
  }
  triggerCellData_BestChoice_ =
      fs_->make<TH1F>("triggerCellData_BestChoice_", "Trigger cell values, best choice", 1000, 0., 70000.);
  if (is_Simhit_comp_)
    triggerCellData_BestChoice_vsSimHits_ = fs_->make<TH2F>("triggerCellData_BestChoice_vsSimHits_",
                                                            "Trigger cell values vs simhit energies, best choice",
                                                            500,
                                                            0,
                                                            0.16,
                                                            1000,
...
void HGCalTriggerBestChoiceTriggerCellTester::beginRun(const edm::Run& /*run*/, const edm::EventSetup& es) {
  es.get<CaloGeometryRecord>().get(triggerGeometry_);
}

void HGCalTriggerBestChoiceTriggerCellTester::analyze(const edm::Event& e, const edm::EventSetup& es) {
  checkSelectedCells(e, es);
  rerunBestChoiceFragments(e, es);
}

void HGCalTriggerBestChoiceTriggerCellTester::checkSelectedCells(const edm::Event& e, const edm::EventSetup& es) {
...
void HGCalTriggerBestChoiceTriggerCellTester::rerunBestChoiceFragments(const edm::Event& e, const edm::EventSetup& es) {
...
void HGCalTriggerBestChoiceTriggerCellTester::fillModule(
    const std::vector<HGCalDataFrame>& dataframes,
    const std::vector<std::pair<DetId, uint32_t>>& linearized_dataframes,
    const HGCalTriggerCellBestChoiceDataPayload& fe_payload_TCsums_woBestChoice,
    const HGCalTriggerCellBestChoiceDataPayload& fe_payload_TCsums_BestChoice,
    const HGCalTriggerCellBestChoiceDataPayload& fe_payload,
  ...
  for (const auto& tc : fe_payload_TCsums_BestChoice.payload) {
    if (tc.hwPt() > 0) {
      ...
      if (is_Simhit_comp_) {
        if (TC_simhit_energies.at(tc.detId()) > 0)
          triggerCellData_BestChoice_vsSimHits_->Fill(TC_simhit_energies.at(tc.detId()), tc.hwPt());
      }
    }
  }
