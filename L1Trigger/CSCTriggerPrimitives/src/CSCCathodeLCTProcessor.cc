...
CSCCathodeLCTProcessor::CSCCathodeLCTProcessor() : CSCBaseboard() {
  ...
  numStrips_ = CSCConstants::NUM_STRIPS_5CFEBS;
...
void CSCCathodeLCTProcessor::checkConfigParameters() {
  ...
  static const unsigned int max_min_separation = CSCConstants::MAX_NUM_HALF_STRIPS_7CFEBS;
...
std::vector<CSCCLCTDigi> CSCCathodeLCTProcessor::run(const CSCComparatorDigiCollection* compdc) {
  ...
  if (numStrips_ <= 0 or numStrips_ > CSCConstants::NUM_STRIPS_7CFEBS) {
    if (cscChamber_) {
      ...
      if (isME11_) {
        ...
        if (!disableME1a_ && theRing == 1 && !gangedME1a_)
          numStrips_ = CSCConstants::NUM_STRIPS_7CFEBS;
        if (!disableME1a_ && theRing == 1 && gangedME1a_)
          numStrips_ = CSCConstants::NUM_STRIPS_5CFEBS;
...
void CSCCathodeLCTProcessor::pulseExtension(
    const std::vector<int> time[CSCConstants::NUM_LAYERS][CSCConstants::MAX_NUM_HALF_STRIPS_7CFEBS], PulseArray pulse) {
...
void CSCCathodeLCTProcessor::markBusyKeys(const int best_hstrip,
  ...
  for (int hstrip = best_hstrip - nspan; hstrip <= best_hstrip + pspan; hstrip++) {
    if (hstrip >= 0 && hstrip < CSCConstants::MAX_NUM_HALF_STRIPS_7CFEBS) {
      quality[hstrip] = 0;
    }
  }
...
void CSCCathodeLCTProcessor::dumpDigis(
    const std::vector<int> strip[CSCConstants::NUM_LAYERS][CSCConstants::MAX_NUM_HALF_STRIPS_7CFEBS]) const {
