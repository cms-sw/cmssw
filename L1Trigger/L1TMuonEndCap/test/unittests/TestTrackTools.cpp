...
void TestTrackTools::test_theta() {
  for (int i = 0; i < 1801; ++i) {
    ...
    CPPUNIT_ASSERT_DOUBLES_EQUAL(
        (endcap == -1 ? (180. - theta) : theta), calc_theta_deg_from_int(calc_theta_int(theta, endcap)), eps);
  }
}
...
void TestTrackTools::test_phi() {
  for (int i = 0; i < 3600; ++i) {
    ...
    int neigh_sector =
        (phi - 15. + 22. < 0.) ? int((phi - 15. + 22. + 360.) / 60.) + 1 : int((phi - 15. + 22.) / 60.) + 1;
    double eps = 360. / 576;
    ...
    CPPUNIT_ASSERT_DOUBLES_EQUAL(
        phi, calc_phi_glob_deg(calc_phi_loc_deg(calc_phi_loc_int(phi + 360., sector)), sector), eps);
    CPPUNIT_ASSERT_DOUBLES_EQUAL(
        phi, calc_phi_glob_deg(calc_phi_loc_deg(calc_phi_loc_int(phi - 360., sector)), sector), eps);
    if ((sector == 6 && neigh_sector == 1) || (sector != 6 && neigh_sector == sector + 1)) {
      CPPUNIT_ASSERT_DOUBLES_EQUAL(
          phi, calc_phi_glob_deg(calc_phi_loc_deg(calc_phi_loc_int(phi, neigh_sector)), neigh_sector), eps);
      CPPUNIT_ASSERT_DOUBLES_EQUAL(
          phi, calc_phi_glob_deg(calc_phi_loc_deg(calc_phi_loc_int(phi + 360., neigh_sector)), neigh_sector), eps);
      CPPUNIT_ASSERT_DOUBLES_EQUAL(
          phi, calc_phi_glob_deg(calc_phi_loc_deg(calc_phi_loc_int(phi - 360., neigh_sector)), neigh_sector), eps);
    }
  }
