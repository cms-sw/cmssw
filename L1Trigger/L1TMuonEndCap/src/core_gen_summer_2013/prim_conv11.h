...
#ifndef __prim_conv11_h_file__
#define __prim_conv11_h_file__
#include "vppc_sim_lib.h"

class prim_conv11
{
 public:
	prim_conv11(){built = false; glbl_gsr = true; defparam();}
	void defparam();
	void build();
	bool built;
	bool glbl_gsr;
...
// [4] = th_init 
// [5] = th_displacement
signal_storage params__storage;  signal_ params;
			// initial ph for this chamber scaled to 0.1333 deg/strip
signal_storage fph__storage;  signal_ fph;
		signal_storage th_corr__storage;  signal_ th_corr;
		signal_storage index__storage;  signal_ index;
		signal_storage th_orig__storage;  signal_ th_orig;
			// theta correction lut, for me1/1 only
// takes index = {wiregroup(2 MS bits), dblstrip}, returns theta correction for tilted wires
signal_storage th_corr_mem__storage;  signal_ th_corr_mem;
		signal_storage factor__storage;  signal_ factor; // strip width factors for each segment
		...
		signal_storage ph_init_ix__storage;  signal_ ph_init_ix; // parameter index for ph_init 
