...
void AngleCalculation::calculate_angles(EMTFTrack& track, const int izone) const {
  ...
  for (int istation = 0; istation < emtf::NUM_STATIONS; ++istation) {
    ...
    if (bugME11Dupes_) {
      ...
      {
        edm::LogError("L1T") << "st_conv_hits.at(istation).size() = " << st_conv_hits.at(istation).size();
        return;
      }
    } else {
  ...
  if (not(st_conv_hits.size() == emtf::NUM_STATIONS)) {
    edm::LogError("L1T") << "st_conv_hits.size() = " << st_conv_hits.size()
                         << ", emtf::NUM_STATIONS = " << emtf::NUM_STATIONS;
    return;
  }
    ...
    for (int ist2 = ist1 + 1; ist2 < emtf::NUM_STATIONS; ++ist2) {  // station B
      ...
      for (const auto& conv_hitA : conv_hitsA) {
        for (const auto& conv_hitB : conv_hitsB) {
          ...
          if (not(thA != 0 && thB != 0)) {
            edm::LogError("L1T") << "thA = " << thA << ", thB = " << thB;
            return;
          }
          if (not(dth < invalid_dtheta)) {
            edm::LogError("L1T") << "dth = " << dth << ", invalid_dtheta = " << invalid_dtheta;
            return;
          }
  ...
  if (not(ipair == emtf::NUM_STATION_PAIRS)) {
    edm::LogError("L1T") << "ipair = " << ipair << ", emtf::NUM_STATION_PAIRS = " << emtf::NUM_STATION_PAIRS;
    return;
  }
  ...
  if (best_pair != -1) {
    ...
    if (not(theta_fp != 0)) {
      edm::LogError("L1T") << "theta_fp = " << theta_fp;
      return;
    }
  ...
  for (int i = 0; i < emtf::NUM_STATIONS; ++i) {
    ...
    if (!v.empty()) {
      ...
      if (not(0 <= bt_station && bt_station <= 4)) {
        edm::LogError("L1T") << "bt_station = " << bt_station;
        return;
      }
...
void AngleCalculation::calculate_bx(EMTFTrack& track) const {
  ...
  if (not(delayBX >= 0)) {
    edm::LogError("L1T") << "delayBX = " << delayBX;
    return;
  }
  ...
  if (not(second_bx != 99)) {
    edm::LogError("L1T") << "second_bx = " << second_bx;
    return;
  }
...
void AngleCalculation::erase_tracks(EMTFTrackCollection& tracks) const {
  ...
  for (const auto& track : tracks) {
    ...
    if (not(track.Hits().size() <= emtf::NUM_STATIONS)) {
      edm::LogError("L1T") << "track.Hits().size() = " << track.Hits().size()
                           << ", emtf::NUM_STATIONS= " << emtf::NUM_STATIONS;
      return;
    }
  }
