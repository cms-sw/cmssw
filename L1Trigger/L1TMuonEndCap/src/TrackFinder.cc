...
TrackFinder::TrackFinder(const edm::ParameterSet& iConfig, edm::ConsumesCollector&& iConsumes) :
    ...
    primConvData_(iConfig.getParameter<edm::ParameterSet>("spPCParams16").getParameter<bool>("PrimConvData")),
...
void TrackFinder::process(
  ...
  sector_processor_lut_.read(primConvData_, fwConfig_ ? condition_helper_.get_pc_lut_version() : primConvLUT_);
