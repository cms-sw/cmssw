...
void AnalyzerSimHitMaps::beginJob() {
  ...
  hSimHit_Barrel_XY_Zoom =
      fs->make<TH2D>("hSimHit_Barrel_XY_Zoom", "PSimHit Barrel y vs. x", 960, 30, 60, 960, -15, 15);
  hSimHit_Endcap_Fw_XY =
      fs->make<TH2D>("hSimHit_Endcap_Fw_XY", "PSimHit Forward Endcap y vs. x", 960, -120, 120, 960, -120, 120);
  hSimHit_Endcap_Bw_XY =
      fs->make<TH2D>("hSimHit_Endcap_Bw_XY", "PSimHit Backward Endcap y vs. x", 960, -120, 120, 960, -120, 120);
  hSimHit_RZ = fs->make<TH2D>("hSimHit_RZ", "PSimHit #rho vs. z", 900, -300, 300, 480, 0, 120);
  hSimHit_Endcap_Fw_RZ_Zoom =
      fs->make<TH2D>("hSimHit_Endcap_Fw_RZ_Zoom", "PSimHit Forward Endcap #rho vs. z", 960, 140, 170, 960, 30, 60);
  hSimHit_Endcap_Bw_RZ_Zoom =
      fs->make<TH2D>("hSimHit_Endcap_Bw_RZ_Zoom", "PSimHit Backward Endcap #rho vs. z", 960, -170, -140, 960, 70, 100);
  ...
  for (int ix = 0; ix < 11; ix++) {
    for (int iy = 10; iy > -1; iy--) {
      histoName.str("");
      histoTitle.str("");
      histoName << "hSimHit_Barrel_XY_Survey_" << -110 + ix * 20 << "x" << -110 + (1 + ix) * 20 << "_" << -110 + iy * 20
                << "y" << -110 + (1 + iy) * 20;
      histoTitle << "PSimHit Barrel y (" << -110 + iy * 20 << ", " << -110 + (1 + iy) * 20 << ") cm vs. x ("
                 << -110 + ix * 20 << ", " << -110 + (1 + ix) * 20 << ") cm";
      TH2D* h = fs->make<TH2D>(histoName.str().c_str(),
  ...
  for (int iz = 0; iz < 27; iz++) {
    for (int ir = 5; ir >= 0; ir--) {
      histoName.str("");
      histoTitle.str("");
      histoName << "hSimHit_RZ_Survey_" << -10 + ir * 20 << "r" << -10 + (1 + ir) * 20 << "_" << -270 + iz * 20 << "z"
                << -270 + (1 + iz) * 20;
      histoTitle << "PSimHit #rho (" << -10 + ir * 20 << ", " << -10 + (1 + ir) * 20 << ") cm vs. z (" << -270 + iz * 20
                 << ", " << -270 + (1 + iz) * 20 << ") cm";
      TH2D* h = fs->make<TH2D>(histoName.str().c_str(),
                               histoTitle.str().c_str(),
                               800,
...
void AnalyzerSimHitMaps::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (iterSimHit = simHitHandlePXBH->begin(); iterSimHit != simHitHandlePXBH->end(); iterSimHit++) {
    ...
    histoNameXY << "hSimHit_Barrel_XY_Survey_" << (20 * (floor((shPos.x() + 10) / 20)) - 10) << "x"
                << (20 * (floor((shPos.x() + 10) / 20)) + 10) << "_" << (20 * (floor((shPos.y() + 10) / 20)) - 10)
                << "y" << (20 * (floor((shPos.y() + 10) / 20)) + 10);
    histoNameRZ << "hSimHit_RZ_Survey_" << (20 * (floor((shPos.perp() + 10) / 20)) - 10) << "r"
                << (20 * (floor((shPos.perp() + 10) / 20)) + 10) << "_" << (20 * (floor((shPos.z() + 10) / 20)) - 10)
                << "z" << (20 * (floor((shPos.z() + 10) / 20)) + 10);
  ...
  for (iterSimHit = simHitHandlePXBL->begin(); iterSimHit != simHitHandlePXBL->end(); iterSimHit++) {
    const GeomDetUnit* gDetUnit = theGeometry->idToDetUnit(iterSimHit->detUnitId());
    GlobalPoint shPos = gDetUnit->surface().toGlobal(iterSimHit->localPosition());
    std::ostringstream histoNameXY;
    std::ostringstream histoNameRZ;
  ...
  for (iterSimHit = simHitHandlePXFH->begin(); iterSimHit != simHitHandlePXFH->end(); iterSimHit++) {
    const GeomDetUnit* gDetUnit = theGeometry->idToDetUnit(iterSimHit->detUnitId());
    GlobalPoint shPos = gDetUnit->surface().toGlobal(iterSimHit->localPosition());
    std::ostringstream histoNameXY;
    std::ostringstream histoNameRZ;
  ...
  for (iterSimHit = simHitHandlePXFL->begin(); iterSimHit != simHitHandlePXFL->end(); iterSimHit++) {
    const GeomDetUnit* gDetUnit = theGeometry->idToDetUnit(iterSimHit->detUnitId());
    GlobalPoint shPos = gDetUnit->surface().toGlobal(iterSimHit->localPosition());
    std::ostringstream histoNameXY;
    std::ostringstream histoNameRZ;
