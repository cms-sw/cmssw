...
//   name only
GlobalAlgorithm::GlobalAlgorithm(const std::string& algoNameValue) : m_algoName(algoNameValue) {
  // default values for private members not set
  // the other private members are C++ initialized
  m_algoBitNumber = -1;
  m_algoChipNumber = -1;
...
GlobalAlgorithm::GlobalAlgorithm(const std::string& algoNameValue, const std::string& algoLogicalExpressionValue)
