...
void l1t::TriggerMenuParser::setVecZdcEnergySumTemplate(
...
void l1t::TriggerMenuParser::parseCondFormats(const L1TUtmTriggerMenu* utmMenu) {
  ...
  m_vecZdcEnergySumTemplate.resize(m_numberConditionChips);
    ...
    for (size_t ii = 0; ii < rpn_vec.size(); ii++) {
      ...
      if ((m_conditionMap[chipNr]).count(condition.getName()) == 0) {
        ...
        if (condition.getType() == esConditionType::SingleEgamma ||
          ...
          parseZdcEnergySum(condition, chipNr, false);
...
/**
 * parseZdcEnergySum Parse an "energy sum" condition from the ZDC subsystem and insert an entry to the conditions map
 *
 * @param node The corresponding node.
 * @param name The name of the condition.
 * @param chipNr The number of the chip this condition is located.
...
bool l1t::TriggerMenuParser::parseZdcEnergySum(L1TUtmCondition condEnergySum,
  ...
  // temporary storage of the parameters
  std::vector<ZdcEnergySumTemplate::ObjectParameter> objParameter(nrObj);
  ...
  ZdcEnergySumTemplate energySumCond(name);
  ...
  if (!insertConditionIntoMap(energySumCond, chipNr)) {
    ...
    (m_vecZdcEnergySumTemplate[chipNr]).push_back(energySumCond);
  }
  //
  return true;
}
