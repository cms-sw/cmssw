...
class extrapolateBit {
	...
	Float_t getscalefrom7TeV() { 
		Float_t scalefrom7TeV = 1;
		if ((ratePU28_ > 0.0001) && (ratePU34_ > 0.0001)) {
			scalefrom7TeV = ratePU34_/ratePU28_;
		}
		return scalefrom7TeV;	
	}
	
	Float_t getPU28number() {
		return extrapFunction_->Eval(MAX_EXTRAP_PU);
	}
...
class extrapolateMenu {
	...
	void MyInit() {
		...
		FitFunction["L1_MuOpen_EG12"] = "pol0";
		FitFunction["L1_Mu12_EG7"] = "pol2";
		FitFunction["L1_SingleJet16"] = "pol0";
		...
		FitFunction["L1_SingleEG20"] = "pol0";
		...
		FitFunction["L1_EG18er_JetC_Cen36_Tau28_dPhi1"] = "pol0";
		FitFunction["L1_SingleMu16er"] = "pol1";
		FitFunction["L1_EG18er_JetC_Cen28_Tau20_dPhi1"] = "pol0";
		...
		FitFunction["L1_DoubleJetC56"] = "pol0";
		...
		FitFunction["L1_TripleJet_64_44_24_VBF"] = "pol0";
		FitFunction["L1_TripleJet_64_48_28_VBF"] = "pol0";
...
extrapolateMenu::extrapolateMenu() {
	...
	TFile *outFile = new TFile("fits_to_L1seeds_7e33_menu_PU34.root","RECREATE");

	readScaleFile("L1Menu_70_70_28_0.437_rates_mod.txt",28);
	readScaleFile("L1Menu_70_70_34_0.529_rates_mod.txt",34);

// CB here are files for 2012 rates and their PU
	std::map<float,TString> dataFiles;
	dataFiles[24] = "70_24_47.2923_rates_mod.txt";
	dataFiles[21] = "70_21_44.2939_rates_mod.txt";
	dataFiles[19] = "70_19_41.0814_rates_mod.txt";
	...
	txtos << "rates_at_8TeV_for_7e33_PU34.txt";
	TString TXTOutPutFileName = txtos.str();
	std::ofstream TXTOutfile(TXTOutPutFileName);

	std::ostringstream csvos;
	csvos << "rates_at_8TeV_for_7e33_PU34.csv";
	TString CSVOutPutFileName = csvos.str();
	std::ofstream CSVOutfile(CSVOutPutFileName);

	std::cout << "L1Bit" << "\t" << "L1SeedName" << "\t" << "rate@7e33_PU32" << "\t" << "rate@PU28" << "\t" << "34/28_factor_from_7TeV" << std::endl;

	TXTOutfile << "L1Bit" << "\t" << "L1SeedName" << "\t" << "rate@7e33_PU32" << "\t" << "rate@PU28" << "\t" << "34/28_factor_from_7TeV" << std::endl;
	CSVOutfile << "L1Bit" << ";" << "L1SeedName" << ";" << "rate@7e33_PU32" << ";" << "rate@PU28" << ";" << "34/28_factor_from_7TeV" << std::endl;
	...
	for(;bitToNameIt!=bitToNameEnd;++bitToNameIt) {
		...
		Float_t factoratPU28 = triggersToExtrapolate[bit]->getPU28number();
		Float_t factor7TeV   = triggersToExtrapolate[bit]->getscalefrom7TeV();
		
		if (rate > 0.001) {
			sumrate +=rate;
		}
		
		std::cout << bit << "\t" << triggerName.Data() << "\t" << rate << "\t" << factoratPU28 << "\t" << factor7TeV << endl;

        TXTOutfile << bit << "\t" << triggerName.Data() << "\t" << rate << "\t" << factoratPU28 << "\t" << factor7TeV << endl;
		CSVOutfile << bit << ";" << triggerName.Data() << ";" << rate << ";" << factoratPU28 << ";" << factor7TeV << endl;

		outFile->WriteObject(triggersToExtrapolate[bit]->getGraph(),("g_"+triggerName).Data());
	}

	std::cout << std::endl << std::endl << "Combined rate: " << sumrate << std::endl << std::endl;
