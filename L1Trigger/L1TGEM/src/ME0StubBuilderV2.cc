...
ME0StubBuilderV2::ME0StubBuilderV2(const edm::ParameterSet& ps) {
...
void ME0StubBuilderV2::fillDescription(edm::ParameterSetDescription& desc) {
...
void ME0StubBuilderV2::build(const GEMPadDigiCollection* padDigis, GE0TriggerDigiCollection& oc) {
  ...
  if (config.numOutputs <= 0)
    throw cms::Exception("ME0StubBuilderV2") << "numOutputs must be greater than 0";
  if (config.numOr <= 0)
    throw cms::Exception("ME0StubBuilderV2") << "numOr must be greater than 0";
  if (config.BXWindow <= 0)
    throw cms::Exception("ME0StubBuilderV2") << "BXWindow must be greater than 0";
  if (config.BXWindow % 2 == 0)
    throw cms::Exception("ME0StubBuilderV2") << "BXWindow must be odd";
  ...
  for (const auto& dataPair : dataMap) {
    ...
    for (size_t idxBX = 0; idxBX < BXOffsetList.size(); ++idxBX) {
      ...
      for (ME0StubPrimitive& seg : segList) {
        ...
        // debug
        if (debug_) {
          LogTrace("ME0StubBuilderV2") << "    Eta Partition = " << seg.etaPartition()
        ...
        segmentVector.push_back(segFinal);
      }
    }
    oc.put(std::make_pair(segmentVector.begin(), segmentVector.end()), detid);
  }
}
