...
Mask l1t::me0::get_ly_mask(const patdef_t& ly_pat, int max_span = 37) {
  ...
  m_vals.reserve(ly_pat.layers.size());
  for (hi_lo_t ly : ly_pat.layers) {
    m_vals.push_back(shift_center(ly, max_span));
  }

  // use the high and low indices to determine where the high bits must go for
  // each layer
  m_vec.reserve(m_vals.size());
  for (const std::vector<int>& x : m_vals) {
    m_vec.push_back(set_high_bits(x));
  }
