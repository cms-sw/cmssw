 ...
 * It should containe file for this m_PAC, defined by m_tower, logSector, logSegment,
 * named pacPat_t<m_tower>sc<logSector>sg<logSegment>.vhd
 ...
 * 
 */
RPCPac::RPCPac(std::string patFilesDir, int m_tower, int logSector, int logSegment):
  RPCPacBase(m_tower, logSector, logSegment) { 
...
//  if(patFilesDir.find("pat") != std::string::npos) {
    patFileName = patFilesDir 
        + "pacPat_t" + rpcconst.intToString(m_ConeCrdnts.m_Tower) 
        + "sc" + rpcconst.intToString(m_ConeCrdnts.m_LogSector) 
        + "sg" + rpcconst.intToString(m_ConeCrdnts.m_LogSegment) 
        + ".xml";
 ...
 *
 */
int RPCPac::getPatternsGroupCount () {
 ...
 *
 */
RPCPattern RPCPac::getPattern(int patNum) const {
 ...
 *
*/
bool RPCPac::getTPatternsGroupShape(int logPlane, int logStripNum) {
 ...
 * 
*/
bool RPCPac::getEPatternsGroupShape(int groupNum, int logPlane, int bitNum) {
...
std::string RPCPac::getPatternsGroupDescription(int patternGroupNum) {
...
void RPCPac::insertQualityRecord(unsigned int qualityTabNumber,
...
void RPCPac::insertPatterns(const L1RpcPatternsVec& patternsVec) {
...
void RPCPac::init(const RPCPatternsParser& parser) {
  ...
  m_PatternsVec = parser.getPatternsVec(m_ConeCrdnts);
