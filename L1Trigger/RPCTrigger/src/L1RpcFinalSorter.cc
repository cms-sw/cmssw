...
L1RpcTBMuonsVec2 L1RpcFinalSorter::RunHalf(L1RpcTBMuonsVec2 &tcsMuonsVec2) {
  ...
  for(int iTC = 0; iTC < tcsMuonsVec2.size()-1; iTC++) {
    for(int iMu = 0; iMu < tcsMuonsVec2[iTC].size(); iMu++) {
      if(tcsMuonsVec2[iTC][iMu].GetCode() == 0)
        continue;
      if(tcsMuonsVec2[iTC][iMu].GBDataKilledLast() ) {
        for(int iMuN = 0; iMuN < tcsMuonsVec2[iTC+1].size(); iMuN++) {
  ...
  //---------sorting-----------------------------------------
/*  multiset<L1RpcTBMuon, L1RpcTBMuon::TMuonMore> barellLiveMuonsSet;
  multiset<L1RpcTBMuon, L1RpcTBMuon::TMuonMore> endcapLiveMuonsSet;
  for(int iTC = 1; iTC < tcsMuonsVec2.size()-1; iTC++)
    for(int iMu = 0; iMu < tcsMuonsVec2[iTC].size(); iMu++)
      if(tcsMuonsVec2[iTC][iMu].IsLive() ) {
        if(abs(16 - tcsMuonsVec2[iTC][iMu].GetEtaAddr()) <=7 )
          barellLiveMuonsSet.insert(tcsMuonsVec2[iTC][iMu]);
        else
          endcapLiveMuonsSet.insert(tcsMuonsVec2[iTC][iMu]);
      }
  L1RpcTBMuonsVec outputBarrelMuons(barellLiveMuonsSet.begin(), barellLiveMuonsSet.end() );
  L1RpcTBMuonsVec outputEndcapMuons(endcapLiveMuonsSet.begin(), endcapLiveMuonsSet.end() ); */
...
L1RpcTBMuonsVec2 L1RpcFinalSorter::Run(L1RpcTBMuonsVec2 &tcsMuonsVec2) {
  ...
  //#ifdef _GRAB_MUONS  //<<<<<<<<<<<<<<<<<<<<<  L1RpcMuonsGrabber  <<<<<<<<<<<<<<<<<
  //  L1RpcMuonsGrabber::Instance()->StoreFinalGBMuons(firstHalfTcsMuonsVec2);
  //#endif

  RunHalf(firstHalfTcsMuonsVec2);

  //#ifdef _GRAB_MUONS //<<<<<<<<<<<<<<<<<<<<<  L1RpcMuonsGrabber  <<<<<<<<<<<<<<<<<
  //  L1RpcMuonsGrabber::Instance()->StoreAnswers(GBOutputMuons);
  //#endif
...
//-----setting 2'complement EtaAddr
  for(int iMu = 0; iMu < GBOutputMuons[0].size(); iMu++) {
