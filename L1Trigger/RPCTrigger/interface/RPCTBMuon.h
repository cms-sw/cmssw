...
public:
  ...
  struct TMuonMore : public std::less<RPCTBMuon> {
    bool operator()(const RPCTBMuon& muonL, const RPCTBMuon& muonR) const { return muonL.getCode() > muonR.getCode(); }
  };

  unsigned int toBits(MuonBitsType muonBitsType) const;

  unsigned int toBits() const { return toBits(m_muonBitsType); }
...
public:
  ...
  public:
    ...
    static const int getMuonBitsCnt() { return m_qualBitsCnt + m_ptBitsCnt + m_signBitsCnt; }
  };
  friend class PACOut;
