...
namespace csctf_analysis
{
  class RefTrack : public Track
  {
	...
	double getPt() const override {return mom.Pt();};
	double getEta() const override {return mom.PseudoRapidity();};
	double getPhi() const override {return (mom.Phi() > 0) ? mom.Phi() : mom.Phi() + 2*M_PI;};
	double getP() const override {return mom.P();};
	double getPz() const override {return mom.Pz();};
