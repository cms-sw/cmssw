...
namespace csctf_analysis {
  ...
  RefTrack::RefTrack(const SimTrack& track) : Track() {
    mom.SetPxPyPzE(track.momentum().x(), track.momentum().y(), track.momentum().z(), track.momentum().t());
    Quality = -1;
    type = track.type();
  }
  RefTrack::RefTrack(const reco::Muon& muon) : Track() {
    mom.SetPxPyPzE(muon.p4().x(), muon.p4().y(), muon.p4().z(), muon.p4().t());
    Quality = -1;
    type = 13;
  }
  ...
  void RefTrack::ghostMatchedTo(const TFTrack& track, int i, double newR) {
    ...
    ghostQ->push_back(track.quality());
  ...
  void RefTrack::setMatch(TFTrack& trackToMatch) { matchedTrack = &trackToMatch; }
