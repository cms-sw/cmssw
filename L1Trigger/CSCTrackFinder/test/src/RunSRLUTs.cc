...
namespace csctf_analysis {
  ...
  void RunSRLUTs::run(std::vector<csctf::TrackStub> *stub_list) {
    ...
    for (std::vector<csctf::TrackStub>::iterator itr = stub_list->begin(); itr != stub_list->end(); itr++) {
      if (itr->station() != 5) {
        ...
        lclphidat lclPhi;
        try {
          lclPhi = srLUTs_[fpga]->localPhi(itr->strip(), itr->pattern(), itr->quality(), itr->bend());
        ...
        gblphidat gblPhi;
        try {
          gblPhi = srLUTs_[fpga]->globalPhiME(lclPhi.phi_local, itr->keyWireGroup(), itr->cscid());
        } catch (cms::Exception &e) {
        ...
        try {
          gblEta =
              srLUTs_[fpga]->globalEtaME(lclPhi.phi_bend_local, lclPhi.phi_local, itr->keyWireGroup(), itr->cscid());
        } catch (cms::Exception &e) {
  ...
  void RunSRLUTs::makeTrackStubs(const CSCCorrelatedLCTDigiCollection *inClcts,
                                 std::vector<csctf::TrackStub> *outStubVec) {
  ...
  void RunSRLUTs::makeAssociatedTrackStubs(const L1CSCTrackCollection *inTrackColl,
                                           TrackAndAssociatedStubsCollection *outTrkStubCol) {
