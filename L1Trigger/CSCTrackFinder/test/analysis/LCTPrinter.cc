...
class LCTPrinter : public edm::EDAnalyzer {
public:
...
void LCTPrinter::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (lctDigiColl = lctDigiColls->begin(); lctDigiColl != lctDigiColls->end(); lctDigiColl++) {
    ...
    for (; lctDigi != lctDigiEnd; lctDigi++) {
      ...
      try {
        lclPhi = srLUTs_[fpga]->localPhi(lctDigi->strip(), lctDigi->pattern(), lctDigi->quality(), lctDigi->bend());
      } catch (...) {
        bzero(&lclPhi, sizeof(lclPhi));
        std::cout << "Bad local phi!" << std::endl;
      }
      try {
        gblPhi = srLUTs_[fpga]->globalPhiME(lclPhi.phi_local, lctDigi->keyWireGroup(), cscId + 1);
      } catch (...) {
      ...
      try {
        gblEta =
            srLUTs_[fpga]->globalEtaME(lclPhi.phi_bend_local, lclPhi.phi_local, lctDigi->keyWireGroup(), cscId + 1);
      } catch (...) {
