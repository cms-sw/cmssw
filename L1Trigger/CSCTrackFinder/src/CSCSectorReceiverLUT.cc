...
lclphidat CSCSectorReceiverLUT::calcLocalPhi(const lclphiadd& theadd) const {
  ...
  double binPhiL = static_cast<double>(maxPhiL) / (2. * CSCConstants::NUM_STRIPS_5CFEBS);
  ...
  if (theadd.strip < 2 * CSCConstants::NUM_STRIPS_5CFEBS)
...
gblphidat CSCSectorReceiverLUT::calcGlobalPhiME(const gblphiadd& address) const {
  ...
  try {
    ...
    if (thechamber) {
      ...
      // per chamber, NUM_STRIPS_5CFEBS=80.  Now, since we know the chamber id
      // and how many strips the chamber has, we can re-adjust the scale.
      //const double scale = static_cast<double>(CSCConstants::NUM_STRIPS_5CFEBS)/nStrips;
  ...
  try {
    ...
    if (thechamber) {
      ...
      if (me1ir_only && (_station != 1 || CSCTriggerNumbering::ringFromTriggerLabels(_station, cscid) != 1)) {
        ...
        const unsigned nStripsPerBin = CSCConstants::NUM_STRIPS_5CFEBS / numBins;
        /**
	   * Calculate Eta correction
	   */

        // Check that no strips will be left out.
        if (nStrips % numBins != 0 || CSCConstants::NUM_STRIPS_5CFEBS % numBins != 0)
          edm::LogWarning("CSCSectorReceiverLUT")
              << "getGlobalEtaValue warning: number of strips " << nStrips << " (" << CSCConstants::NUM_STRIPS_5CFEBS
