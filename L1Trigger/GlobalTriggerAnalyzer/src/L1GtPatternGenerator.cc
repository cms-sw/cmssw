...
static void extractRecordData(const edm::Event& iEvent,
  ...
  if (!handle.isValid()) {
    throw cms::Exception(__func__) << "Failed to extract record of type " << typeid(TRecord).name() << " labeled "
...
void L1GtPatternGenerator::beginJob() {
  ...
  m_writer = std::unique_ptr<L1GtPatternWriter>(new L1GtPatternWriter(
