...
void MuTimingModuleWithStat::process(AlgoMuonBase* algoMuon) {
  if(algoMuon->getSimBeta() == 0) {
    LogTrace("omtfEventPrintout")<<__FUNCTION__<<":"<<__LINE__<<" algoMuon->getSimBeta() == 0 "<<std::endl;
    return;
  }
  ...
  LogTrace("omtfEventPrintout")<<__FUNCTION__<<":"<<__LINE__<<" algoMuon SimBeta "<<algoMuon->getSimBeta()<<" one_beta "<<one_beta<<std::endl;
  ...
  for(auto& stubResult : algoMuon->getStubResults() ) {
    ...
    LogTrace("omtfEventPrintout")<<__FUNCTION__<<":"<<__LINE__<<" layer "<<layer
    ...
    for(unsigned int iRoll = 0; iRoll < timigTo1_Beta[iLayer].size(); ++iRoll) {
      for(unsigned int iEtaBin = 0; iEtaBin < timigTo1_Beta[iLayer][iRoll].size(); ++iEtaBin) {
        ...
        for(int iTimingBin = 1; iTimingBin <= timigVs1_BetaHist->GetXaxis()->GetNbins(); iTimingBin++) {
          ...
          edm::LogVerbatim("omtfEventPrintout")<<__FUNCTION__<<":"<<__LINE__<<" layer "<<iLayer<<" roll "<<iRoll<<" etaBin "<<iEtaBin<<" timing "<<timing
