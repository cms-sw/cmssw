...
void bitwise_compare(const char* tag,
                     TTree* tree1,
                     TTree* tree2,
                     const char* var,
                     const char* var2,
  ...
  if (!TString(var).CompareTo("sumEt[0]")) {
    var = "ETT";
  } else if (!TString(var).CompareTo("sumEt[2]")) {
    var = "HTT";
  } else if (!TString(var).CompareTo("sumEt[4]")) {
    var = "ETM";
  } else if (!TString(var).CompareTo("sumEt[6]")) {
    var = "HTM";
  }
...
void bitwise_compare(
    const char* tag, TTree* tree1, TTree* tree2, const char* var, const char* cut, int nbins, double max, double min) {
  bitwise_compare(tag, tree1, tree2, var, var, cut, nbins, max, min);
}

void NtupleDiff(const char* tag,
                const char* file1,
                const char* file2,
                const char* treepath1 = "l1UpgradeEmuTree/L1UpgradeTree",
                const char* treepath2 = "l1UpgradeEmuTree/L1UpgradeTree") {
