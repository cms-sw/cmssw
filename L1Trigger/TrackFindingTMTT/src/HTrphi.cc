...
pair<unsigned int, unsigned int> HTrphi::trueCell( const TP* tp ) const {
  ...
  if (iQoverPt >= 0 && iQoverPt < int(nBinsQoverPtAxis_) && iPhiTrk >= 0 && iPhiTrk < int(nBinsPhiTrkAxis_)) {
    ...
    */
    ...
    if (iQoverPt < 0)                        iQoverPt = 0;
    if (iQoverPt >= int(nBinsQoverPtAxis_))  iQoverPt = nBinsQoverPtAxis_ - 1;
    if (iPhiTrk  < 0)                        iPhiTrk  = 0;
    if (iPhiTrk  >= int(nBinsPhiTrkAxis_))   iPhiTrk  = nBinsPhiTrkAxis_ - 1;
  }
