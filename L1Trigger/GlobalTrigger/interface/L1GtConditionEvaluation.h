...
public:
    ...
    inline const bool evaluateConditionStoreResult()
    {
        return m_condLastResult = evaluateCondition();
    }
    ...
    const Type1& threshold, const Type2& value, const bool condGEqValue) const
{
    ...
    if ( value > 0 ) {
        ...
        << "  threshold check for condGEqValue = " << condGEqValue
        << "\n    hex: " << std::hex
...
// check if a bit with a given number is set in a mask
template<class Type1>
const bool L1GtConditionEvaluation::checkBit(
    const Type1& mask, const unsigned int bitNumber) const
{
    ...
    << "\n     mask address = " << &mask
    << std::dec << "\n     dec: "
    << "mask = " << mask << " oneBit = " << oneBit << " bitNumber = " << bitNumber
    << std::hex << "\n     hex: "
    << "mask = " << mask << " oneBit = " << oneBit << " bitNumber = " << bitNumber
    << std::dec
