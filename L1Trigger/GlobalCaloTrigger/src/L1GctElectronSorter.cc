  ...
  m_outputCands(4)
{
  ...
  for(unsigned i=0;i!=sourceCards.size();i++){
    if(sourceCards.at(i)!=0){
      m_theSCs.at(i) = sourceCards.at(i);
    }else{
      throw cms::Exception("L1GctSetupError")
...
void L1GctElectronSorter::fetchInput() {
  ...
  for (unsigned int i=0; i<m_theSCs.size(); i++) {
    ...
    for (unsigned int j=0; j<4; j++) {
      ...
      if (m_isolation) {
	setInputEmCand((j+i*4),m_theSCs.at(i)->getIsoElectrons().at(j));
      }else {
	setInputEmCand((j+i*4),m_theSCs.at(i)->getNonIsoElectrons().at(j));
      }
    }
  }
}
...
std::ostream& operator<<(std::ostream& s, const L1GctElectronSorter& ems) {
  ...
  for (unsigned i=0; i<ems.m_theSCs.size(); i++) {
    s << "SourceCard* " << i << " = " << ems.m_theSCs.at(i)<<std::endl;
  }
