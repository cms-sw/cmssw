...
{
 public:
  static L1GctLutFromFile<NAddressBits, NDataBits>* setupLut(std::string filename);
...
template <int NAddressBits, int NDataBits>
L1GctLutFromFile<NAddressBits, NDataBits>* L1GctLutFromFile<NAddressBits, NDataBits>::setupLut(const std::string filename)
{
...
void L1GctLutFromFile<NAddressBits, NDataBits>::readFromFile(const std::string filename)
{
  ...
  while (std::getline(inFile,strFromFile).good()) {
    if (strFromFile.length()>0 && strFromFile.find_first_not_of("-+")==std::string::npos) break;
  }
