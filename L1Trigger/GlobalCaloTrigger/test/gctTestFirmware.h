...
public:
  ...
  void fillJetsFromFirmware(const std::string &fileName, int bxStart, int numOfBx);
  ...
  bool checkEnergySumsFromFirmware(const L1GlobalCaloTrigger* gct, const std::string &fileName, int numOfBx);

private:

  // FUNCTION PROTOTYPES FOR JET FINDER CHECKING
  /// Read one event's worth of jets from the file
  std::vector<JetsVector> getJetsFromFile(int bxStart, int numOfBx);
  /// Read a single jet
  L1GctJet nextJetFromFile (unsigned jf, int bx);
  //=========================================================================
