...
enum SignalMode { mnone = 0, mreg, mwire, minput, moutput, minout, mnum, mtemp };
...
public:
  ...
  void setbrackets(const char* l, const char* r) {
    lb = l;
    rb = r;
  };
#endif
  ...
  Signal operator++(int) {
    return *this = *this + 1;
  };  // postfix operators require some work (different names for operations and assignment printing?)
...
public:
  globcontrol();
#ifdef VGEN
  void AddParameter(string ln) {
    pars[npar] = ln;
    npar++;
  };
  void AddDeclarator(string ln) {
    decls[ndecl] = ln;
    ndecl++;
  };
  void AddComment(string ln) {
    ln += "\n";
    decls[ndecl] = ln;
    ndecl++;
  };
