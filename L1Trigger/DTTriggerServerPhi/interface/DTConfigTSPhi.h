...
class DTConfigTSPhi : public DTConfig {
  ...
  //! Constructor
  DTConfigTSPhi(edm:: ParameterSet& ps);
  ...
  inline int  TsmGetCarryFlag() const { return m_tsmhsp; }

  //! Enabled TRACOs in TS
  inline bool usedTraco(int i) const { return m_tstren[i-1]; }
  ...
  bool m_tstren[24];     // Enabled TRACOs
  BitArray<8> m_tsmword; // TSM backup mode word
  int m_ntsstsmd;        // nb tss to one of the tsmd (only if back-up mode)
