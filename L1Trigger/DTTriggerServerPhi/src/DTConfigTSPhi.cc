...
// Constructors --
//----------------
DTConfigTSPhi::DTConfigTSPhi(edm::ParameterSet& ps) : m_ps(&ps) { 
...
DTConfigTSPhi::setDefaults() {
  ...
  int mymsk = m_ps->getParameter<int>("TSSMSK1");
  if (checkMask(mymsk)) 
    m_tssmsk[0] = mymsk;
  else {
  ...
  mymsk= m_ps->getParameter<int>("TSSMSK2");  
  if (checkMask(mymsk)) 
    m_tssmsk[1] = mymsk;
  else {
  ...
  m_tsshte[0] = m_ps->getParameter<bool>("TSSHTE1");
  
  // Htrig checking in TSS for sort2
  m_tsshte[1] = m_ps->getParameter<bool>("TSSHTE2");

  // Htrig checking in TSS for carry
  m_tsshte[2] = m_ps->getParameter<bool>("TSSHTEC");

  // Inner SL checking in TSS for sort1
  m_tssnoe[0] = m_ps->getParameter<bool>("TSSNOE1");
  
  // Inner SL checking in TSS for sort2
  m_tssnoe[1] = m_ps->getParameter<bool>("TSSNOE2");
 
  // Inner SL checking in TSS for carry
  m_tssnoe[2] = m_ps->getParameter<bool>("TSSNOEC");

  // Correlation checking in TSS for sort1
  m_tsscce[0] = m_ps->getParameter<bool>("TSSCCE1");

  // Correlation checking in TSS for sort2
  m_tsscce[1] = m_ps->getParameter<bool>("TSSCCE2");
 
  // Correlation checking in TSS for carry
  m_tsscce[2] = m_ps->getParameter<bool>("TSSCCEC");

  // Ghost 1 supperssion option in TSS
  int mygs = m_ps->getParameter<int>("TSSGS1");
  if (mygs>=0 && mygs<3)
    m_tssgs1 = mygs;
  else {
  ...
  mygs= m_ps->getParameter<int>("TSSGS2");
  if (mygs>=0 && mygs<5)
    m_tssgs2 = mygs;
  else {
  ...
  m_tsscgs1 = m_ps->getParameter<bool>("TSSCGS1");
  
  // Correlated ghost 2 supperssion option in TSS
  m_tsscgs2 = m_ps->getParameter<bool>("TSSCGS2");

  // Order of quaity bits in TSM for sort1
  mymsk = m_ps->getParameter<int>("TSMMSK1");
  if (checkMask(mymsk)) 
    m_tsmmsk[0] = mymsk;
  else {
  ...
  mymsk= m_ps->getParameter<int>("TSMMSK2");
  if (checkMask(mymsk)) 
    m_tsmmsk[1] = mymsk;
  else {
  ...
  m_tsmhte[0] = m_ps->getParameter<bool>("TSMHTE1");
  
  // Htrig checking in TSM for sort2
  m_tsmhte[1] = m_ps->getParameter<bool>("TSMHTE2");
 
  // Htrig checking in TSM for carry
  m_tsmhte[2] = m_ps->getParameter<bool>("TSMHTEC");
  
  // Inner SL checking in TSM for sort1
  m_tsmnoe[0] = m_ps->getParameter<bool>("TSMNOE1");
  
  // Inner SL checking in TSM for sort2
  m_tsmnoe[1] = m_ps->getParameter<bool>("TSMNOE2");
 
  // Inner SL checking in TSM for carry
  m_tsmnoe[2] = m_ps->getParameter<bool>("TSMNOEC");

  // Correlation checking in TSM for sort1
  m_tsmcce[0] = m_ps->getParameter<bool>("TSMCCE1");
  
  // Correlation checking in TSM for sort2
  m_tsmcce[1] = m_ps->getParameter<bool>("TSMCCE2");
 
  // Correlation checking in TSM for carry
  m_tsmcce[2] = m_ps->getParameter<bool>("TSMCCEC");

  // Ghost 1 supperssion option in TSM
  mygs = m_ps->getParameter<int>("TSMGS1");
  if (mygs>=0 && mygs<3)
    m_tsmgs1 = mygs;
  else {
  ...
  mygs= m_ps->getParameter<int>("TSMGS2");
  if (mygs>=0 && mygs<5)
    m_tsmgs2 = mygs;
  else {
  ...
  m_tsmcgs1 = m_ps->getParameter<bool>("TSMCGS1");
  
  // Correlated ghost 2 supperssion option in TSM
  m_tsmcgs2 = m_ps->getParameter<bool>("TSMCGS2");

  // Handling carry in case of pile-up
  int myhsp = m_ps->getParameter<int>("TSMHSP");
  if (myhsp>=0 && myhsp<3)
    m_tsmhsp = myhsp;
  else {
  ...
  m_tsmword.one();
  int word = m_ps->getParameter<int>("TSMWORD");
  if (word<0 || word>255){
  ...
  for (int i=0;i<24;i++){
    std::stringstream os;
    os << "TSTREN" << i;
    m_tstren[i] = m_ps->getParameter<bool>(os.str());
  }
