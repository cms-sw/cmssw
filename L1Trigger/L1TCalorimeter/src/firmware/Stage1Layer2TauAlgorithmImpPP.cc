...
void l1t::Stage1Layer2TauAlgorithmImpPP::processEvent(const std::vector<l1t::CaloEmCand>& EMCands,
  ...
  for (CaloRegionBxCollection::const_iterator region = subRegions.begin(); region != subRegions.end(); region++) {
    ...
    for (const auto& subRegion : subRegions) {
      ...
      if (deltaPhi + deltaEta > 0 && deltaPhi + deltaEta < 2) {  //nondiagonal neighbors
        if (subRegion.hwPt() > highestNeighborEt) {
          ...
          int neighborTauVeto = subRegion.hwQual() & 0x1;  // tauVeto should be the first bit of quality integer
    ...
    if ((tauEt > highestNeighborEt && (NESW == "isEast" || NESW == "isNorth")) ||
        (tauEt >= highestNeighborEt && (NESW == "isSouth" || NESW == "isWest")) || highestNeighborEt == 0) {
      if (highestNeighborEt >= tauNeighbourThreshold)
        tauEt += highestNeighborEt;
      ...
      if (region->hwEta() >= isoTauEtaMin && region->hwEta() <= isoTauEtaMax) {
        if ((highestNeighborTauVeto == 0 && regionTauVeto == 0) || tauEt > tauMaxPtTauVeto) {
          ...
          if (useIsolLut) {
            ...
            if (jetIsolation < tauMaxJetIsolationA ||
                (tauEt >= tauMinPtJetIsolationB && jetIsolation < tauMaxJetIsolationB) ||
                (std::abs(jetIsolation - 999.) < 0.1))
              isoFlag = 1;
          }
        }
      }
...
double l1t::Stage1Layer2TauAlgorithmImpPP::JetIsolation(int et,
                                                        int ieta,
                                                        int iphi,
