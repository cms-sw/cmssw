...
Stage1Layer2TauAlgorithmImpHW::Stage1Layer2TauAlgorithmImpHW(CaloParamsHelper const* params)
    : params_(params), isoTauLut{make_unique<Stage1TauIsolationLUT>(params_)} {}
...
void l1t::Stage1Layer2TauAlgorithmImpHW::processEvent(const std::vector<l1t::CaloEmCand>& EMCands,
  ...
  for (auto iTau = isoTaus->begin(); iTau != isoTaus->end(); ++iTau)
    iTau->setHwIso(1);
}

//  Compute jet isolation.
double l1t::Stage1Layer2TauAlgorithmImpHW::JetIsolation(int et,
                                                        int ieta,
                                                        int iphi,
