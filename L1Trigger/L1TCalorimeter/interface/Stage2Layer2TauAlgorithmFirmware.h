...
namespace l1t {
  ...
  private:
    void merging(const std::vector<l1t::CaloCluster>& clusters,
    ...
    static bool compareTowers(const l1t::CaloTower& TT1, const l1t::CaloTower& TT2);  // implements operator < for TT
    bool is3x3Maximum(const l1t::CaloTower& tower,
                      const std::vector<CaloTower>& towers,
                      l1t::CaloStage2Nav& caloNav);  // is maximum in the 3x3 window? (recompute jet flag)
    std::vector<std::unique_ptr<l1t::CaloCluster>> makeSecClusters(
        const std::vector<l1t::CaloTower>& towers,
        std::vector<int>& sites,
        const l1t::CaloCluster& mainCluster,
        l1t::CaloStage2Nav& caloNav);  // make the secondary clusters fr merging (need to be deleted later)
