// L1TCaloUpgrateToGCTConverter.cc
//
// Author: Ivan Cali
//
// This takes the outputs of the Stage 1 Calo trigger emulator and
// converts them into the old GT format. Input collections are assumed
// to be sorted but not truncated to proper length, which we do here.
...
l1t::L1TCaloUpgrateToGCTConverter::produce(Event& e, const EventSetup& es)
{
  ...
  // create the new em and jet collections
  ...
  // BX range is assumed to be the same for all collections.
  ...
  for(int itBX=firstBX; itBX!=lastBX+1; ++itBX){
    int nISOEGammas = 0; //max 4
    int nNoISOEGammas = 0; //max 4
	...
	itEGamma != EGamma->end(itBX); ++itEGamma){
      ...
      if(iso){
	if (nISOEGammas == 4) continue;
	isoEmResult->push_back(EmCand);
	nISOEGammas++;
      }
      else {
	if (nNoISOEGammas == 4) continue;
  ...
  //for(int itBX=firstBX; itBX!=lastBX+1; ++itBX){
    int nForJets = 0; //max 4
    int nCenJets = 0; //max 4
	...
	itJet != Jet->end(itBX); ++itJet){
      ...
      //             bool isTau, bool isFor, uint16_t block, uint16_t index, int16_t bx);
      if(forward){
	if(nForJets == 4) continue;
	forJetResult->push_back(JetCand);
	nForJets++;
      }
      else{
	if(nCenJets == 4) continue;
