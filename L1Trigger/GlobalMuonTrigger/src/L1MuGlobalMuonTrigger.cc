...
L1MuGlobalMuonTrigger::L1MuGlobalMuonTrigger(const edm::ParameterSet& ps) {
  ...
  if (L1MuGMTConfig::Debug(1))
    edm::LogVerbatim("GMT_info") << "**** L1GlobalMuonTrigger building ****";
  if (L1MuGMTConfig::Debug(1))
    edm::LogVerbatim("GMT_info");

  // create new PSB
  if (L1MuGMTConfig::Debug(2))
    edm::LogVerbatim("GMT_info") << "creating GMT PSB";
  m_PSB = new L1MuGMTPSB(*this, consumesCollector());

  // create new matcher
  if (L1MuGMTConfig::Debug(2))
    edm::LogVerbatim("GMT_info") << "creating GMT Matcher (0,1)";
  m_Matcher[0] = new L1MuGMTMatcher(*this, 0);  // barrel
  m_Matcher[1] = new L1MuGMTMatcher(*this, 1);  // endcap

  // create new cancel-out units
  if (L1MuGMTConfig::Debug(2))
    edm::LogVerbatim("GMT_info") << "creating GMT Cancel Out Unit (0,1,2,3)";
  ...
  if (L1MuGMTConfig::Debug(2))
    edm::LogVerbatim("GMT_info") << "creating GMT MIP & ISO bit Assigment Unit (0,1)";
  m_MipIsoAU[0] = new L1MuGMTMipIsoAU(*this, 0);  // barrel
  m_MipIsoAU[1] = new L1MuGMTMipIsoAU(*this, 1);  // endcap

  // create new Merger
  if (L1MuGMTConfig::Debug(2))
    edm::LogVerbatim("GMT_info") << "creating GMT Merger (0,1)";
...
void L1MuGlobalMuonTrigger::produce(edm::Event& e, const edm::EventSetup& es) {
  ...
  if (L1MuGMTConfig::Debug(2))
    edm::LogVerbatim("GMT_info");
  if (L1MuGMTConfig::Debug(2))
    edm::LogVerbatim("GMT_info") << "**** L1GlobalMuonTrigger processing ****";
  if (L1MuGMTConfig::Debug(2))
    edm::LogVerbatim("GMT_info");
  ...
  for (int bx = bx_min; bx <= bx_max; bx++) {
    ...
    if (L1MuGMTConfig::Debug(2))
      edm::LogVerbatim("GMT_info") << "L1GlobalMuonTrigger processing bunch-crossing : " << bx;

    // get data into the data buffer
    if (L1MuGMTConfig::Debug(2))
      edm::LogVerbatim("GMT_info") << "running GMT PSB";
    if (m_PSB) {
    ...
    if (m_PSB && !m_PSB->empty()) {
      // run matcher
      if (L1MuGMTConfig::Debug(2))
        edm::LogVerbatim("GMT_info") << "running GMT barrel Matcher";
      if (m_Matcher[0])
        m_Matcher[0]->run();
      if (L1MuGMTConfig::Debug(3) && m_Matcher[0])
        m_Matcher[0]->print();
      if (L1MuGMTConfig::Debug(2))
        edm::LogVerbatim("GMT_info") << "running GMT endcap Matcher";
      if (m_Matcher[1])
        m_Matcher[1]->run();
      if (L1MuGMTConfig::Debug(3) && m_Matcher[1])
        m_Matcher[1]->print();

      // run cancel-out units
      if (L1MuGMTConfig::Debug(2))
        edm::LogVerbatim("GMT_info") << "running GMT barrel Cancel Out Unit";
      if (m_CancelOutUnit[0])
        m_CancelOutUnit[0]->run();
      if (L1MuGMTConfig::Debug(3) && m_CancelOutUnit[0])
        m_CancelOutUnit[0]->print();

      if (L1MuGMTConfig::Debug(2))
        edm::LogVerbatim("GMT_info") << "running GMT endcap Cancel Out Unit";
      if (m_CancelOutUnit[1])
        m_CancelOutUnit[1]->run();
      if (L1MuGMTConfig::Debug(3) && m_CancelOutUnit[1])
        m_CancelOutUnit[1]->print();

      if (L1MuGMTConfig::Debug(2))
        edm::LogVerbatim("GMT_info") << "running GMT CSC/fRPC Cancel Out Unit";
      if (m_CancelOutUnit[2])
        m_CancelOutUnit[2]->run();
      if (L1MuGMTConfig::Debug(3) && m_CancelOutUnit[2])
        m_CancelOutUnit[2]->print();

      if (L1MuGMTConfig::Debug(2))
        edm::LogVerbatim("GMT_info") << "running GMT DT/bRPC Cancel Out Unit";
      if (m_CancelOutUnit[3])
        m_CancelOutUnit[3]->run();
      if (L1MuGMTConfig::Debug(3) && m_CancelOutUnit[3])
        m_CancelOutUnit[3]->print();

      // run MIP & ISO bit assignment units
      if (L1MuGMTConfig::Debug(2))
        edm::LogVerbatim("GMT_info") << "running GMT barrel MIP & ISO bit Assignment Unit";
      if (m_MipIsoAU[0])
        m_MipIsoAU[0]->run();
      if (L1MuGMTConfig::Debug(3) && m_MipIsoAU[0])
        m_MipIsoAU[0]->print();
      if (L1MuGMTConfig::Debug(2))
        edm::LogVerbatim("GMT_info") << "running GMT endcap MIP & ISO bit Assignment Unit";
      if (m_MipIsoAU[1])
        m_MipIsoAU[1]->run();
      if (L1MuGMTConfig::Debug(3) && m_MipIsoAU[1])
        m_MipIsoAU[1]->print();

      // run Merger
      if (L1MuGMTConfig::Debug(2))
        edm::LogVerbatim("GMT_info") << "running GMT barrel Merger";
      if (m_Merger[0])
        m_Merger[0]->run();
      if (L1MuGMTConfig::Debug(3) && m_Merger[0])
        m_Merger[0]->print();
      if (L1MuGMTConfig::Debug(2))
        edm::LogVerbatim("GMT_info") << "running GMT endcap Merger";
      if (m_Merger[1])
        m_Merger[1]->run();
      if (L1MuGMTConfig::Debug(3) && m_Merger[1])
        m_Merger[1]->print();

      // run sorter
      if (L1MuGMTConfig::Debug(2))
        edm::LogVerbatim("GMT_info") << "running GMT Sorter";
      if (m_Sorter)
        m_Sorter->run();
      if (L1MuGMTConfig::Debug(1) && m_Sorter)
        m_Sorter->print();
      ...
      if (m_Sorter->numberOfCands() > 0) {
        ...
        for (iexc = gmt_cont.begin(); iexc != gmt_cont.end(); iexc++) {
          if (*iexc)
            m_ExtendedCands.push_back(**iexc);
        }
      }
...
void L1MuGlobalMuonTrigger::reset() {
  ...
  if (m_Matcher[1])
    m_Matcher[1]->reset();
  if (m_CancelOutUnit[0])
    m_CancelOutUnit[0]->reset();
  if (m_CancelOutUnit[1])
    m_CancelOutUnit[1]->reset();
  if (m_CancelOutUnit[2])
    m_CancelOutUnit[2]->reset();
  if (m_CancelOutUnit[3])
    m_CancelOutUnit[3]->reset();
  if (m_MipIsoAU[0])
    m_MipIsoAU[0]->reset();
  if (m_MipIsoAU[1])
    m_MipIsoAU[1]->reset();
