		      ...
		      unsigned vme_addr_width, bool distrRAM) 
{
  ...
  m_Inputs = in_widths;
  m_TotalInWidth = 0; 
  for (const auto & in_width : in_widths) m_TotalInWidth += in_width.second;

  m_Outputs = out_widths;
  m_TotalOutWidth = 0; 
  for (const auto & out_width : out_widths) m_TotalOutWidth += out_width.second;
...
void L1MuGMTLUT::Save(const char* path) {
  ...
  of << "NAME = " << m_name << endl;
  of << "INSTANCES =" ;
  for (const auto & m_InstName : m_InstNames) of << " " << m_InstName;
  of << endl;
