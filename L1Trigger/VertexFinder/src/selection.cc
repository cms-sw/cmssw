...
const l1t::Vertex& getPrimaryVertex(const std::vector<l1t::Vertex>& aVertexCollection)
{
  ...
  return getPrimaryVertex(aVertexCollection, [](const Tracks_t& tracks) -> float {
    float sumPt = 0.0;
    for (auto t : tracks)
      sumPt += t->getMomentum().transverse();
    return sumPt;
  });
}
...
const l1t::Vertex& getPrimaryVertex(const std::vector<l1t::Vertex>& aVertexCollection, const std::function<float(const std::vector<edm::Ptr<l1t::Vertex::Track_t>>&)>& aFunction)
{
  ...
  return *std::max_element(
    aVertexCollection.begin(), aVertexCollection.end(), [aFunction](const l1t::Vertex& v1, const l1t::Vertex& v2) -> bool { return (aFunction(v1.tracks()) < aFunction(v2.tracks())); });
}
