  ...
  settings_(iConfig)
{
  ...
  hisRecoVertexZ0Resolution_ = inputDir.make<TH1F>("hisRecoVertexZ0Resolution", "Reconstructed primary vertex z_{0} resolution; z_{0} Resolution [cm]; Counts", 200, -1., 1.);
  hisRecoVertexZ0Separation_ = inputDir.make<TH1F>("hisRecoVertexZ0Separation", "Separation between reconstructed and generator primary vertex in z_{0}; z_{0} Separation (|z_{0}^{gen} - z_{0}^{reco}| [cm]; Counts", 100, 0., 1.);
...
void VertexAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for (unsigned int i = 0; i < numVertices; ++i) {
    ...
    if (settings_.vx_algo() == Algorithm::Kmeans || settings_.vx_algo() == Algorithm::HPV || settings_.vx_algo() == Algorithm::FastHisto )
...
void VertexAnalyzer::endJob()
{
  ...
  cout << "Averate ratio of fake tracks in primary vertex " << hisFakeTracksRateInPV_->GetMean() * 100
       << " % " << endl;
  cout << "Average PV z0 separation " << hisRecoVertexZ0Separation_->GetMean()
       << " cm " << endl;
  cout << "PV z0 resolution " << hisRecoVertexZ0Resolution_->GetStdDev()
       << " cm " << endl;
  ...
  cout << "PrimaryVertex Finding Efficiency = " << recoPVeff << " +/- " << recoPVeff_err
       << endl;
