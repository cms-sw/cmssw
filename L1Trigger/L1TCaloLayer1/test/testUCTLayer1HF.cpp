...
double gaussianRandom(double mean, double standardDeviation) { return 0; }
...
int main(int argc, char** argv) {
  ...
  for (int event = 0; event < nEvents; event++) {
    ...
    for (uint32_t i = 0; i < nHitTowers; i++) {
      ...
      while (caloEta < 30 || caloEta > 41)
        caloEta = (30 + random() % 12);
      int caloPhi;
    ...
    if (expectedTotalET < 32)
      fraction = 0.20;
    if (diff > (fraction * expectedTotalET)) {
      std::cout << std::dec << std::endl;
      print(uctLayer1);
      std::cout << std::endl
                << "Expected " << std::showbase << std::internal << std::setfill('0') << std::setw(10) << std::hex
