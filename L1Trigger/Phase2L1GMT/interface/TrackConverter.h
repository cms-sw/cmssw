...
namespace Phase2L1GMT {
  ...
  private:
    ...
    uint generateQuality(const edm::Ptr<TTTrack<Ref_Phase2TrackerDigi_> >& track) const {
      uint chi2Cut = 0xf;
    ...
    uint ptLookup(const uint absCurv) const {
      for (auto i : ptShifts) {
    ...
    uint etaLookup(const uint absTanL) const {
