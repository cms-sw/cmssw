...
ap_uint<BITSSIGMACOORD + 1> TPSAlgorithm::deltaCoord(const ap_int<BITSSTUBCOORD>& phi1, const ap_int<BITSSTUBCOORD>& phi2) {
...
match_t TPSAlgorithm::match(const propagation_t prop, const l1t::MuonStubRef& stub,uint trackID) {
  if (verbose_ == 1) {
    printf("Matching to coord1=%d coord2=%d eta1=%d eta2=%d\n",stub->coord1(),stub->coord2(),stub->eta1(),stub->eta2());
    stub->print();
  }
  //Matching of Coord1
...
PreTrackMatchedMuon TPSAlgorithm::processTrack(const ConvertedTTTrack& track, const l1t::MuonStubRefVector& stubs) {
  ...
  if (muon.stubs().size()>0) {//change the ID for now
    muon.setValid(true);
