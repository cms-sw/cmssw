...
std::pair<l1t::KMTFTrack, l1t::KMTFTrack> KMTFCore::chain(const l1t::MuonStubRef& seed,
  ...
  for (const auto& mask : combinatorics) {
    ...
    covariance(2, 0) = 0;
    covariance(2, 1) = 0;
    if (!(mask==1||mask==2||mask == 3 ||mask==4||mask == 5 || mask == 9 || mask == 6 || mask == 10 || mask == 12))
  ...
  if (verbose_){
    ...
    for (const auto& track: pretracks)
      printf("Kalman Track charge=%d pt=%f hit pattern = %d eta=%f phi=%f curvature=%d curvature STA =%d stubs=%d chi2=%d,%d rank=%d,%d pts=%f %f   ID=%d\n",
	     track.charge(),
	     track.pt(),
	     track.hitPattern(),
	     track.eta(),
...
int KMTFCore::correctedPhiB(const l1t::MuonStubRef& stub) {
  return ap_fixed<BITSPHIB,BITSPHIB,AP_TRN_ZERO,AP_SAT>(ap_ufixed<PHIBSCALE, PHIBSCALE_INT>(28.5205658) * stub->coord2());
}
...
void KMTFCore::propagate(l1t::KMTFTrack& track) {
  ...
  if (step == 1) {
    ...
    if (verbose_) {
      printf("Vertex phiB prop = %d(=%d +%d) - %d = %d\n", DXY.to_int(),aK.to_int(),(eK*Kint).to_int(), ap_fixed<BITSPHIB, BITSPHIB>(phiB).to_int(), phiBNew);
    }
  }
  ///////////////////////////////////////////////////////
...
bool KMTFCore::updateLUT(l1t::KMTFTrack& track,
...
bool KMTFCore::getBit(int bitmask, int pos) { 
...
bool KMTFCore::estimateChiSquare(l1t::KMTFTrack& track,bool vertex) {
  ...
  for (unsigned int i=0;i<(track.stubs().size()-1);i++) {
	...
	diffPhiB=0;
      ...
      if (verbose_)
	printf("Error propagation coefficients A=%d B=%f BK = %d\n",propErrA[stub->depthRegion()-innerStub->depthRegion()-1], propErrB[stub->depthRegion()-innerStub->depthRegion()-1],uint(ap_fixed<8,2>(propErrB[stub->depthRegion()-innerStub->depthRegion()-1])*(absK>>4)));
	...
	positionError=positionError*2;
      ...
      uint err =positionError+uint(ap_fixed<8,2>(propErrB[stub->depthRegion()-innerStub->depthRegion()-1])*absK);      
      ...
      chiErr=chiErr+err;
      if (verbose_){
	printf("Chi Square stub for track with pattern=%d   inner stub depth=%d-> AK=%d stubDepth=%d diff1=%d diff2=%d delta=%d absK=%d err=%d\n", track.hitPattern(),innerStub->depthRegion(),  int(AK),stub->depthRegion(), diffPhi, diffPhiB, absDelta,uint(absK),err);
      }
  }
      if (verbose_){
