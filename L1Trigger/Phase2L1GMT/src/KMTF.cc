...
KMTF::KMTF(int verbose,const edm::ParameterSet& iConfig):
  verbose_(verbose),
  trackMaker_(new KMTFCore(iConfig))
{
}

KMTF::~KMTF() {}

std::pair<std::vector<l1t::KMTFTrack>,std::vector<l1t::KMTFTrack> > 
KMTF::process(const l1t::MuonStubRefVector& stubsAll,
	      int bx,unsigned int MAXN) {
  ...
  for (unsigned int i=0;i<32;++i) {
    ...
    if (patterns) {
      printf("KMTFPattern ");
      if(i<Nstubs4)
	printf("%d %d %d 1 %d 0 ",stubs4[0]->coord1(),stubs4[0]->coord2(),stubs4[0]->quality(),stubs4[0]->kmtf_address());
      else 
	...
	if (tracks.second.id()&0x2)     
      ...
      if (patterns) {
	if (tracks.first.id()&0x1)
	  printf("1 %d %d %d %d %d %d ",tracks.first.curvatureAtVertex()<0 ? 1: 0,
		 tracks.first.ptPrompt(),tracks.first.phiAtMuon()/(1<<5),tracks.first.coarseEta(),int(tracks.first.dxy()*ap_ufixed<8,1>(1.606)),tracks.first.rankPrompt());
	else 
	  printf("0 0 0 0 0 0 0 ");
	if (tracks.second.id()&0x2)
	  printf("1 %d %d %d %d %d %d ",tracks.second.curvatureAtVertex()<0 ? 1: 0,
		 tracks.second.ptDisplaced(),tracks.second.phiAtMuon()/(1<<5),tracks.second.coarseEta(),int(tracks.second.dxy()*ap_ufixed<8,1>(1.606)),tracks.second.rankDisp());
	else 
	  printf("0 0 0 0 0 0 0 ");
      }
    }
...
std::vector<l1t::KMTFTrack> KMTF::cleanRegion(const std::vector<l1t::KMTFTrack>& tracks2,
					      ...
					      const std::vector<l1t::KMTFTrack>& tracks4,bool vertex) {
  ...
  for (unsigned int i=0;i<tracks2.size();++i) {
    ...
    for (unsigned int j=0;j<tracks2.size();++j) {
      if (i==j)
	continue;
      overlapCleanTrack(source ,tracks2[j],0,vertex);      
    }
    for (unsigned int j=0;j<tracks3.size();++j) {
      overlapCleanTrack(source ,tracks3[j],1,vertex);      
    }
    for (unsigned int j=0;j<tracks4.size();++j) {
      overlapCleanTrack(source ,tracks4[j],1,vertex);      
    }
  ...
  for (unsigned int i=0;i<tracks3.size();++i) {
    ...
    for (unsigned int j=0;j<tracks3.size();++j) {
      if (i==j)
	continue;
      overlapCleanTrack(source ,tracks3[j],0,vertex);      
    }
    for (unsigned int j=0;j<tracks2.size();++j) {
      overlapCleanTrack(source ,tracks2[j],0,vertex);      
    }
  ...
  for (unsigned int i=0;i<tracks4.size();++i) {
    ...
    for (unsigned int j=0;j<tracks4.size();++j) {
      if (i==j)
	continue;
      overlapCleanTrack(source ,tracks4[j],0,vertex);      
    }
    for (unsigned int j=0;j<tracks3.size();++j) {
      overlapCleanTrack(source ,tracks3[j],0,vertex);      
    }
