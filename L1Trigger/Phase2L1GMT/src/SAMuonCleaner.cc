...
std::vector<l1t::SAMuon> SAMuonCleaner::cleanTF(const std::vector<l1t::SAMuon>& tfMuons) {
  ...
  for(unsigned int i=0;i<tfMuons.size();++i) {
    ...
    for(unsigned int j=0;j<tfMuons.size();++j) {
      if(i==j)
	continue;
      overlapCleanTrack(source,tfMuons[j],0);
    }
...
std::vector<l1t::SAMuon> SAMuonCleaner::interTFClean(const std::vector<l1t::SAMuon>& bmtf,const std::vector<l1t::SAMuon>& omtf,const std::vector<l1t::SAMuon>& emtf) {
...
void SAMuonCleaner::sort(std::vector<l1t::SAMuon>& in) {
  ...
  for (const auto& track: in) {
    if (track.stubs().size()>0&&out.size()<12)
      out.push_back(track);
  }
  in=out;
}
