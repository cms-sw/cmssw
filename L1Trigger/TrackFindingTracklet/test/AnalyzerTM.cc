...
namespace trklet {
   ...
   *  \date   2023, Jan
   */
  class AnalyzerTM : public one::EDAnalyzer<one::WatchRuns, one::SharedResources> {
  public:
    AnalyzerTM(const ParameterSet& iConfig);
  ...
  AnalyzerTM::AnalyzerTM(const ParameterSet& iConfig) : useMCTruth_(iConfig.getParameter<bool>("UseMCTruth")) {
    usesResource("TFileService");
    // book in- and output ED products
    const string& label = iConfig.getParameter<string>("OutputLabelTM");
  ...
  void AnalyzerTM::beginRun(const Run& iEvent, const EventSetup& iSetup) {
  ...
  void AnalyzerTM::analyze(const Event& iEvent, const EventSetup& iSetup) {
    // read in ht products
  ...
  //
  void AnalyzerTM::formTracks(const StreamsTrack& streamsTrack,
                              const StreamsStub& streamsStubs,
                              vector<vector<TTStubRef>>& tracks,
                              int channel) const {
  ...
  //
  void AnalyzerTM::associate(const vector<vector<TTStubRef>>& tracks,
                             const StubAssociation* ass,
                             set<TPPtr>& tps,
                             int& sum,
                             bool perfect) const {
