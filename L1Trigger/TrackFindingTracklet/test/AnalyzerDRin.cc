...
namespace trklet {
  ...
  /*! \class  trklet::AnalyzerDRin
   *  \brief  Class to analyze hardware like structured TTStub Collection generated by DRin module
   *  \author Thomas Schuh
   *  \date   2023, Jan
   */
  class AnalyzerDRin : public one::EDAnalyzer<one::WatchRuns, one::SharedResources> {
  public:
    AnalyzerDRin(const ParameterSet& iConfig);
  ...
  AnalyzerDRin::AnalyzerDRin(const ParameterSet& iConfig) : useMCTruth_(iConfig.getParameter<bool>("UseMCTruth")) {
  ...
  void AnalyzerDRin::beginRun(const Run& iEvent, const EventSetup& iSetup) {
  ...
  void AnalyzerDRin::analyze(const Event& iEvent, const EventSetup& iSetup) {
    // read in ht products
  ...
  //
  void AnalyzerDRin::formTracks(const StreamsTrack& streamsTrack,
                                const StreamsStub& streamsStubs,
                                vector<vector<TTStubRef>>& tracks,
                                int channel) const {
  ...
  //
  void AnalyzerDRin::associate(const vector<vector<TTStubRef>>& tracks,
                               const StubAssociation* ass,
                               set<TPPtr>& tps,
                               int& sum,
                               bool perfect) const {
