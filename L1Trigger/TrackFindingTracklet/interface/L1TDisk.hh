  ...
  void findMatches(L1TDisk* D, double phiSF, double rphicut1, double rcut1, 
	...
	for (int offset=-1;offset<2;offset++) {
	  ...
	  for (unsigned int j=0;j<D->stubs_[jSector].size();j++) {
	    ...
	    if (Delta!=Delta) {
	      cout << "why is this check here??? should never pass???" <<endl;
	      continue;
	    }
	    ...
	    if (0) {
	      if (fabs(Delta)<10.0 && fabs(deltar)<15.0) {
		static ofstream out("diskmatch.txt");
		out << aTracklet.r()<<" "<<aTracklet.z()<<" "<<r<<" "<<z<<" "<<Delta<<" "<<deltar<<endl;
	      }
	    }
	    ...
	    /*
	    // we currently don't use stub pt in matching stubs to tracklet
	...
	if (bestdist<1e30) tracklets_[iSector][i].addStub(tmp);
  ...
  void findBarrelMatches(L1TGeomBase* L, double phiSF){
	...
	for (int offset=-1;offset<2;offset++) {
	  ...
	  for (unsigned int j=0;j<L->stubs_[jSector].size();j++) {
	    ...
	    if (fabs(deltaz)>5.0) continue;

	    double dist=hypot(rdeltaphi/(0.1*phiSF),deltaz/5.0);
