...
public:
  ...
  FPGAMatchCalculator(string name, unsigned int iSector):
    ...
    if (abs(disk_)==3){
      ...
      rcut2S_[10]=5.0/krprojshiftdisk;      
  ...
  void execute() {
    ...
    std::vector<std::pair<std::pair<FPGATracklet*,int>,std::pair<FPGAStub*,L1TStub*> > > mergedMatches=mergeMatches(matches_);
    ...
    for(unsigned int j=0;j<mergedMatches.size();j++){
      ...
      FPGATracklet* tracklet=mergedMatches[j].first.first;
  ...
  std::vector<std::pair<std::pair<FPGATracklet*,int>,std::pair<FPGAStub*,L1TStub*> > > mergeMatches(vector<FPGACandidateMatch*>& candmatch) {
    
    std::vector<std::pair<std::pair<FPGATracklet*,int>,std::pair<FPGAStub*,L1TStub*> > >  tmp;
