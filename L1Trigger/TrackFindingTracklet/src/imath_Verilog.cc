...
void var_cut::print(std::map<const var_base *,std::set<std::string> > &cut_strings, const int step, Verilog, const std::map<const var_base *,std::set<std::string> > * const previous_cut_strings) const {
...
void var_base::print_cuts(std::map<const var_base *,std::set<std::string> > &cut_strings, const int step, Verilog, const std::map<const var_base *,std::set<std::string> > * const previous_cut_strings) const {
  if (p1_) p1_->print_cuts(cut_strings,step,verilog,previous_cut_strings);
  if (p2_) p2_->print_cuts(cut_strings,step,verilog,previous_cut_strings);
  if (p3_) p3_->print_cuts(cut_strings,step,verilog,previous_cut_strings);
...
void var_flag::print(std::ofstream& fs, Verilog, int l1, int l2, int l3){
  ...
  for (const auto &cut : cuts_){
    if (cut->get_op() != "cut")
      ...
      if (cut->get_cut_var()->get_p1()) cut->get_cut_var()->get_p1()->print_cuts(cut_strings1,step_,verilog,&cut_strings1);
      if (cut->get_cut_var()->get_p2()) cut->get_cut_var()->get_p2()->print_cuts(cut_strings1,step_,verilog,&cut_strings1);
      if (cut->get_cut_var()->get_p3()) cut->get_cut_var()->get_p3()->print_cuts(cut_strings1,step_,verilog,&cut_strings1);
    }
  }
