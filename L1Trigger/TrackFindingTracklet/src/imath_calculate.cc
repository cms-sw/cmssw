...
bool VarBase::calculate(int debug_level) {
  ...
  itest = Trk_bitShift::bitShift(itest, ns);
  itest = itest >> ns;
  if (itest != ival_) {
...
void VarAdjustK::local_calculate() {
  ...
  if (lr_ > 0)
    ...
    ival_ = Trk_bitShift::bitShift(ival_, (-lr_));
}
...
void VarAdd::local_calculate() {
  ...
  if (shift1 > 0)
    i1 = Trk_bitShift::bitShift(i1, shift1);
  if (shift2 > 0)
    i2 = Trk_bitShift::bitShift(i2, shift2);
  ival_ = i1 + i2;
  if (ps_ > 0)
    ival_ = ival_ >> ps_;
}
...
void VarShift::local_calculate() {
  ...
  if (shift_ > 0)
    ival_ = ival_ >> shift_;
  if (shift_ < 0)
    ival_ = Trk_bitShift::bitShift(ival_, (-shift_));
}
...
void VarDSPPostadd::local_calculate() {
  ...
  ival_ = p3_->ival();
  if (shift3_ > 0)
    ival_ = Trk_bitShift::bitShift(ival_, shift3_);
  if (shift3_ < 0)
    ival_ = ival_ >> (-shift3_);
