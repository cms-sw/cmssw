...
namespace trklet {
  ...
  void DR::consume(const StreamsTrack& streamsTrack, const StreamsStub& streamsStub) {
    ...
    auto nonNullTrack = [](const int& sum, const FrameTrack& frame) { return sum + (frame.first.isNonnull() ? 1 : 0); };
    auto nonNullStub = [](const int& sum, const FrameStub& frame) { return sum + (frame.first.isNonnull() ? 1 : 0); };
