...
public:
  ...
  DEcompare(typeT dt, typeT em) : data_(dt), emul_(em), t_match(false) {
    ...
    if (debug_)
      LogDebug("DEcompare") << "DEcompare"
                            << ": creating instance of type: " << GetName(0) << ", data size:" << data_->size()
                            << " ncand:" << ncand_[0] << ", emul size:" << emul_->size() << " ncand:" << ncand_[1]
                            << ".\n"
                            << std::flush;
  };
  ...
  */
  ...
  int get_ncand(int i) const {
    if (debug_ && (i < 0 || i > 2))
      LogDebug("DEcompare") << "DEcompare illegal number of candidates request flag:" << i << "\n";
    return ncand_[i];
  }
...
bool DEcompare<T>::do_compare(std::ofstream& os, int dump) {
  ...
  if (dump == -1 || (dump == 1 && !t_match))
    os << "  ..." << GetName() << " data and emulator comparison: " << ok.c_str() << std::endl;
  return t_match;
}
...
bool DEcompare<T>::CompareCollections(std::ofstream& os, int dump) {
  ...
  if (debug_)
    std::cout << "\tDEcompare stats2:"
              << " data_bad:" << data_bad.size() << " emul_bad:" << emul_bad.size() << " data_good:" << data_good.size()
              << " emul_good:" << emul_good.size() << " data_:" << data_->size() << " emul_:" << emul_->size() << ".\n"
              << std::flush;
  ...
  for (col_sz i = 0; i < nde; i++) {
    ...
    if (abs(dump) == 1)
      ...
      if ((dump == 2 && !de_utils.de_equal_loc(*itd, *itm)) || (dump == 3 && de_utils.de_equal_loc(*itd, *itm)))
        DumpCandidate(itd, itm, os, prtmode);
    }
...
template <typename T>
bool DEcompare<T>::SortCollections(cand_vec& data_good, cand_vec& emul_good, cand_vec& data_bad, cand_vec& emul_bad) {
  if (debug_)
  ...
  if (debug_)
    std::cout << "\tDEcompare stats1:"
              << " data_bad:" << data_bad.size() << " emul_bad:" << emul_bad.size() << " data_good:" << data_good.size()
              << " emul_good:" << emul_good.size() << " data: " << get_ncand(data_) << " emul: " << get_ncand(emul_)
              << "\n"
              << std::flush;
  if (debug_)
