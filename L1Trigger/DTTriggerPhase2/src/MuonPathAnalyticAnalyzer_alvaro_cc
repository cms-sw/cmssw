...
void MuonPathAnalyticAnalyzer::initialise(const edm::EventSetup &iEventSetup) {
  if (debug_)
    cout << "MuonPathAnalyticAnalyzer::initialiase" << endl;
...
void MuonPathAnalyticAnalyzer::run(edm::Event &iEvent,
  ...
  if (debug_)
    cout << "MuonPathAnalyticAnalyzer: run" << endl;
...
const int MuonPathAnalyticAnalyzer::LAYER_ARRANGEMENTS_[4][3] = {
    {0, 1, 2},
...
void MuonPathAnalyticAnalyzer::analyze(MuonPath *inMPath, std::vector<metaPrimitive> &metaPrimitives) {
  if (debug_)
  ...
  if (debug_) cout << "mPath cloned" << endl; 
  mPath->setQuality(NOPATH);
  if (debug_) cout << "Quality set to NOPATH" << endl; 
  
  buildLateralities();
  /*for (int i = 0; i < 16; i++) {
  //for (int i = 0; i < totalNumValLateralities_; i++) {
    latQuality_[i].valid = false;
    latQuality_[i].bxValue = 0;
    latQuality_[i].quality = NOPATH;
    latQuality_[i].invalidateHitIdx = -1;
  } */
 
  if (debug_) cout << "Arriving before superlayer_datapath " << endl; 
  ...
  if (mPath->quality() >= minQuality_) {
    ...
    for (int i = 0; i < totalNumValLateralities_; i++) {  //here
      ...
      if (latQuality_[i].valid and
        ...
        if (debug_) cout << "Lateralities -> " << lat[0] <<  lat[1] <<  lat[2] << lat[3] << endl;
...
void MuonPathAnalyticAnalyzer::setCellLayout(const int layout[4]) {
 ...
 * de lateralidad, involucrando 4 celdas de las que conforman el MuonPathAnalyticAnalyzer,
 ...
 */
bool MuonPathAnalyticAnalyzer::isStraightPath(LATERAL_CASES sideComb[4]) {
 ...
 * se está trabajando.
 */
void MuonPathAnalyticAnalyzer::evaluatePathQuality(MuonPath *mPath) {
  // here
...
void MuonPathAnalyticAnalyzer::evaluateLateralQuality(int latIdx, MuonPath *mPath, LATQ_TYPE *latQuality) {
  int layerGroup[3];
 ...
 */
int MuonPathAnalyticAnalyzer::eqMainBXTerm(LATERAL_CASES sideComb[2], int layerIdx[2], MuonPath *mPath) {
 ...
 *    layerIdx[1] -> Capa más alta
 */
int MuonPathAnalyticAnalyzer::eqMainTerm(LATERAL_CASES sideComb[2], int layerIdx[2], MuonPath *mPath, int bxValue) {
  int eqTerm = 0, coefs[2];
 ...
 * de las capas 0 y 2.
 */
void MuonPathAnalyticAnalyzer::calculatePathParameters(MuonPath *mPath) {
...
void MuonPathAnalyticAnalyzer::calcTanPhiXPosChamber(MuonPath *mPath) {
  /*
 ...
 */
void MuonPathAnalyticAnalyzer::calcTanPhiXPosChamber4Hits(MuonPath *mPath) {
 ...
 */
void MuonPathAnalyticAnalyzer::calcTanPhiXPosChamber3Hits(MuonPath *mPath) {
  int layerIdx[2];
  int x_prec_inv = (int)(1. / (10. * x_precision_));
  int numberOfBits = (int)(round(std::log(x_prec_inv) / std::log(2.)));
 ...
 */
void MuonPathAnalyticAnalyzer::calcCellDriftAndXcoor(MuonPath *mPath) {
 ...
 */
void MuonPathAnalyticAnalyzer::calcChiSquare(MuonPath *mPath) {
  /*
    float xi, zi, factor;
...
void MuonPathAnalyticAnalyzer::superlayer_datapath(MuonPath *mPath) {
  int validCells = 0;
  if (debug_) {
    cout << ">>>>DEBUG: SUPERLAYER_DATAPATH " << endl;
    for (int j = 0; j < 4; j++)
        cout << " " <<mPath->primitive(j)->channelId()<< " " <<mPath->primitive(j)->tdcTimeStamp()<< " " <<mPath->primitive(j)->isValidTime() << endl; 
  }
  ...
  if (debug_) cout << ">>>>DEBUG: SUPERLAYER_DATAPATH" << endl
  ...
  if (debug_) cout << "Starting segment composer with cell_horiz_layout " << cell_horiz_layout[0] << cell_horiz_layout[1] <<cell_horiz_layout[2] <<cell_horiz_layout[3] << endl;
...
/*  cell_valid my_cell_valid; 
  for (int i = 0; i<4; i++){
    my_cell_valid.cell_horiz_layout[i] = cell_horiz_layout[i];
    my_cell_valid.valid[i] = valids[i];
  } */
  //std::pair<int*,int*> my_cell_valid = std::make_pair({cell_horiz_layout[0],cell_horiz_layout[1],cell_horiz_layout[2],cell_horiz_layout[3]},
   //                                     {valids[0], valids[1], valids[2], valids[3]});
  //nt* my_cell_valid = e_pair(cell_horiz_layout,valids);
  //latcomb_consts NULL_ENCODER_LIST = {0,0.0,{0,0,0,0},0};
  
  
  //std::map<cell_valid, std::vector<latcomb_consts>>::iterator layout_iterator;
  //std::map<cell_valid, std::vector<latcomb_consts>>::iterator layout_iterator;
  //layout_iterator = LAYOUT_VALID_TO_LATCOMB_CONSTS_ENCODER.find(my_cell_valid);
  for (auto & elem : LAYOUT_VALID_TO_LATCOMB_CONSTS_ENCODER) 
    ...
    //if ( LAYOUT_VALID_TO_LATCOMB_CONSTS_ENCODER.find(my_cell_valid) == LAYOUT_VALID_TO_LATCOMB_CONSTS_ENCODER.end()) {
  //    my_latcomb_consts.push_back(NULL_ENCODER_LIST);
  //    if (debug_) cout << "my_cell_valid not found!" << endl; 
  //  } else {
      for (auto & ind_latcomb_consts : elem.second)
      //for (auto & ind_latcomb_consts : layout_iterator->second)
        my_latcomb_consts.push_back(ind_latcomb_consts);
    } 
  for (auto & ind_latcomb_consts : my_latcomb_consts ){
    if (debug_) cout << "Computing t0 with latcomb_consts"  << ind_latcomb_consts.latcomb <<","<<  ind_latcomb_consts.numer_const << ",{" << ind_latcomb_consts.numer_coeff[0] << "," <<ind_latcomb_consts.numer_coeff[1] << "," <<ind_latcomb_consts.numer_coeff[2] << "," <<ind_latcomb_consts.numer_coeff[3] << "}," << ind_latcomb_consts.denom << endl; 
    ...
    if (prim_t0 != 0) {
      if (debug_) cout << "Will fill Alvaro's laterality " <<  ind_latcomb_consts.latcomb << "= Emu lat " << lat << endl; 
...
int MuonPathAnalyticAnalyzer::validate_4hits_one_latcomb_reduced(int valids[4], int t0s[4], int cell_horiz_layout[4], latcomb_consts my_latcomb_consts) {
...
int MuonPathAnalyticAnalyzer::tuck_t0(int t, int t0s[4], int valids[4]){
  ...
  if (debug_) cout << "min_t:" << min_t << " t:" << t << " max_t:" << max_t << endl;
...
/*for (auto &elem : LAYOUT_VALID_TO_LATCOMB_CONSTS_ENCODER) {
  cout << elem.first.cell_horiz_layout[0] << " "
       << elem.first.cell_horiz_layout[1] << " "
       << elem.first.cell_horiz_layout[2] << " "
       << elem.first.cell_horiz_layout[3] << " " 
       << elem.second[0].latcomb << endl; 
}
cout << "********************************" << endl; 
