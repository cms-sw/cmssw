...
std::vector<metaPrimitive> MPThetaMatching::getBestThetaMPInChamber(std::vector<metaPrimitive> thetaMPs) {
  ...
  for (const auto &mp1 : thetaMPs) {
    ...
    if(mp1.quality>1) { bestThetaMPs.push_back(mp1); continue;}

    // if q=1 and there is no q=3, save it
    for (const auto &mp2: thetaMPs) {
      ...
      if (sector1 == sector2 && wheel1 == wheel2 && station1 == station2) {
        if (mp1.quality > mp2.quality) {
            continue; //there is a q=3 and it was alreacy saved
        } else if (mp2.quality > mp1.quality) { //not likely case unless topological TPs
           bestThetaMPs.push_back(mp2);
        }
