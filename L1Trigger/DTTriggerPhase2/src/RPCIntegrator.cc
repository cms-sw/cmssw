...
RPCIntegrator::~RPCIntegrator() {
  if (m_debug_)
    std::cout << "RPCIntegrator destructor" << std::endl;
}

void RPCIntegrator::initialise(const edm::EventSetup& iEventSetup, double shift_back_fromDT) {
  if (m_debug_)
    std::cout << "RPCIntegrator initialisation" << std::endl;
...
void RPCIntegrator::removeRPCHitsUsed() {
  if (m_debug_)
    std::cout << "RPCIntegrator removeRPCHitsUsed method" << std::endl;
  if (!m_storeAllRPCHits_) {  // Remove RPC hit attached to a DT or RPC segment if required by user (avoid having two TP's corresponding to the same physical hit)
...
RPCMetaprimitive* RPCIntegrator::matchDTwithRPC(metaPrimitive* dt_metaprimitive) {
  ...
  int dt_bx =
      ...
      shift_back_;  // because at this stage the BX of metaprimitive is not yet computed... will also have to subtract 20*25 ns because of the recent change
...
L1Phase2MuDTPhDigi RPCIntegrator::createL1Phase2MuDTPhDigi(
  ...
  if (m_debug_)
    std::cout << "Creating DT TP out of RPC recHits" << std::endl;
...
double RPCIntegrator::phi_DT_MP_conv(double rpc_global_phi, int rpcSector) {
  ...
  if (rpcSector == 1)
    ...
    if (rpc_global_phi >= 0)
      return rpc_global_phi - (rpcSector - 1) * Geom::pi() / 6.;
    else
      return rpc_global_phi + (13 - rpcSector) * Geom::pi() / 6.;
  }
}
...
bool RPCIntegrator::hasPosRF_rpc(int wh, int sec) { return (wh > 0 || (wh == 0 && sec % 4 > 1)); }
