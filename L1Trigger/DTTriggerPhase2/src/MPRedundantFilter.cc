...
// ============================================================================
MPRedundantFilter::MPRedundantFilter(const ParameterSet& pset) : MPFilter(pset), MaxBufferSize(8) {
  // Obtention of parameters
  debug = pset.getUntrackedParameter<bool>("debug");
  if (debug)
    cout << "MPRedundantFilter: constructor" << endl;
}

MPRedundantFilter::~MPRedundantFilter() {
  if (debug)
    cout << "MPRedundantFilter: destructor" << endl;
}
...
void MPRedundantFilter::initialise(const edm::EventSetup& iEventSetup) {
  if (debug)
    cout << "MPRedundantFilter::initialiase" << endl;
  buffer.clear();
...
void MPRedundantFilter::run(edm::Event& iEvent,
  ...
  if (debug)
    cout << "MPRedundantFilter: done" << endl;
