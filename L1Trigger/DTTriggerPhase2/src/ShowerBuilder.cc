...
void ShowerBuilder::run(Event& iEvent,
    ...
    if  (nHits != 0) {
        if (debug_) LogDebug("ShowerBuilder") << "        - Going to study " << nHits << " hits";
        ...
        if (debug_) LogDebug("ShowerBuilder") << "        - No hits to study.";
    }
...
void ShowerBuilder::setInChannels(const DTDigiCollection *digis) {
    for (const auto &dtLayerId_It : *digis) {
        ...
        if (id.superlayer() == 2) continue; // Skip SL2 digis
...
void ShowerBuilder::processHits_standAlone(std::map<int, ShowerCandidatePtr> &showerCands) {
    ...
    for (auto &hit : all_hits) {
        ...
        if (hit.superLayerId() == 1) bmtl1_sl1_buffer.push_back(_hitpbx);
        else if (hit.superLayerId() == 3) bmtl1_sl3_buffer.push_back(_hitpbx);   
    }
...
void ShowerBuilder::processHitsFirmwareEmulation(std::map<int, ShowerCandidatePtr> &showerCands) {
    ...
    if (debug_) LogDebug("ShowerBuilder") << "        - bx range: " << min_bx << " - " << max_bx 
                                          << ", size: "<< (max_bx-min_bx);
    
    for (int bx = min_bx; bx <= max_bx + 17; bx++) {
        fill_obdt(bx);
        
        if (debug_) LogDebug("ShowerBuilder") << "          ^ " << obdt_buffer.size() 
                                              << " hits in obdt_buffer at BX " << bx;
        fill_bmtl1_buffers();
        
        nHits_sl1 = bmtl1_sl1_buffer.size();
        if (debug_) LogDebug("ShowerBuilder") << "          ^ " << nHits_sl1 
                                              << " hits in bmtl1_sl1_buffer at BX " << bx;
        nHits_sl3 = bmtl1_sl3_buffer.size();
        if (debug_) LogDebug("ShowerBuilder") << "          ^ " << nHits_sl3 
                                              << " hits in bmtl1_sl3_buffer at BX " << bx;    
        if (triggerShower(bmtl1_sl1_buffer)) {
...
void ShowerBuilder::set_shower_properties(
...
void ShowerBuilder::fill_obdt(const int bx) {
    ...
    if (all_hits_perBx.find(bx) != all_hits_perBx.end()) {
        for (auto &hit : all_hits_perBx[bx]) {
            ...
            if (debug_) LogDebug("ShowerBuilder") << "          ^ Trying to add hit with wire " 
                                                  << hit.channelId() << " in OBDT at BX " << bx;
            if (!showerb::buffer_contains(hot_wires_buffer, hit)) {
                if (debug_) LogDebug("ShowerBuilder") << "          ^ added";
...
void ShowerBuilder::fill_bmtl1_buffers(){
    ...
    if (obdt_buffer.empty()) return;
    if (debug_) LogDebug("ShowerBuilder") << "          ^ Getting hits from OBDT";
    for (int i = 0; i < nHits_per_bx_; i++) {
        if (obdt_buffer.empty()) break;
