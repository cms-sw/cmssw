...
Phase1L1TJetProducer::Phase1L1TJetProducer(const edm::ParameterSet& iConfig) {
  ...
  this->etaBinning_ = iConfig.getParameter<std::vector<double>>("etaBinning");
  this->nBinsEta_ = this->etaBinning_.size() - 1;
  this->nBinsPhi_ = iConfig.getParameter<unsigned int>("nBinsPhi");
  this->phiLow_ = iConfig.getParameter<double>("phiLow");
  this->phiUp_ = iConfig.getParameter<double>("phiUp");
  this->jetIEtaSize_ = iConfig.getParameter<unsigned int>("jetIEtaSize");
  this->jetIPhiSize_ = iConfig.getParameter<unsigned int>("jetIPhiSize");
  this->seedPtThreshold_ = iConfig.getParameter<double>("seedPtThreshold");
  this->puSubtraction_ = iConfig.getParameter<bool>("puSubtraction");
  this->outputCollectionName_ = iConfig.getParameter<std::string>("outputCollectionName");
  this->vetoZeroPt_ = iConfig.getParameter<bool>("vetoZeroPt");

  this->_inputCollectionTag = new edm::EDGetTokenT<edm::View<reco::Candidate>>(
      consumes<edm::View<reco::Candidate>>(iConfig.getParameter<edm::InputTag>("inputCollectionTag")));
  this->_caloGrid = new TH2F("caloGrid",
                             "Calorimeter grid",
                             this->nBinsEta_,
                             this->etaBinning_.data(),
                             this->nBinsPhi_,
                             this->phiLow_,
                             this->phiUp_);
...
void Phase1L1TJetProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  if (this->_inputCollectionTag) {
    ...
    const auto seedsVector = this->_findSeeds(*(this->_caloGrid), this->seedPtThreshold_);  // seedPtThreshold = 6
    std::vector<reco::CaloJet> l1jetVector;
    if (this->puSubtraction_) {
      l1jetVector = this->_buildJetsFromSeedsWithPUSubtraction(*(this->_caloGrid), seedsVector, this->vetoZeroPt_);
    } else {
    ...
    iEvent.put(std::move(l1jetVectorPtr), this->outputCollectionName_);
  }
        ...
        for (int phiIndex = -phiHalfSize; phiIndex <= phiHalfSize; phiIndex++) {
          ...
          if (phiIndex > 0) {
            if (phiIndex > -etaIndex) {
              isLocalMaximum =
                  ((isLocalMaximum) && (centralPt > this->_getTowerEnergy(caloGrid, iEta + etaIndex, iPhi + phiIndex)));
            } else {
              isLocalMaximum = ((isLocalMaximum) &&
                                (centralPt >= this->_getTowerEnergy(caloGrid, iEta + etaIndex, iPhi + phiIndex)));
            }
          } else {
            if (phiIndex >= -etaIndex) {
              isLocalMaximum =
