      ...
      for (int station = 1; station <= 4; station++) {
        ...
        for (int bx = 3; bx >= -3; bx--) {
          ...
          for (int rpcbx = bx - 1; rpcbx <= bx + 1; rpcbx++) {
            ...
            for (int hit = 0; hit < nhits; hit++) {
              ...
              if (rpcts1 && dtts && dtts->code() < m_QualityLimit &&
                  deltaPhi(dtts->phi(), rpcts1->phi()) < m_DphiWindow) {
          ...
          if (!delta.empty()) {
            ...
            if (((delta_0.size() <= min_index) && (min_index < (delta_0.size() + delta_p.size())) &&
                 !delta_p.empty())) {
              init_bx = ibx_dtp;
              final_bx = fbx_dtp;
            ...
            if (dtts && dtts->code() < m_QualityLimit && dttsnew && !shift_1 &&
                !m_phiDTDigis_tm.chPhiSegm(wheel, station, sector, final_bx, flipBit(track_seg))) {
              ...
              if (sign(dtts->phi()) != sign(dttsnew->phi()) && deltaPhi(-(dtts->phi()), dttsnew->phi()) > 100) {
                dtts_sh2nd = new L1MuDTChambPhDigi(final_bx,
                                                   dtts->whNum(),
                                                   dtts->scNum(),
                                                   dtts->stNum(),
                                                   dtts->phi(),
...
int DTRPCBxCorrection::nRPCHits(L1MuTMChambPhContainer inCon, int bx, int wh, int sec, int st) {
  ...
  for (auto& i : *vInCon) {
    if (bx == i.bxNum() && i.code() != 7 && i.whNum() == wh && i.scNum() == sec && i.stNum() == st)
      size++;
  }
