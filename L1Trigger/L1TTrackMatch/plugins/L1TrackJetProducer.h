...
std::vector<EtaPhiBin> L2_clustering(std::vector<std::vector<EtaPhiBin>> &L1clusters, int phiBins_, float phiStep_, float etaStep_ ) {
  ...
  for (int phibin = 0; phibin < phiBins_; ++phibin) {  //Find eta-phibin with highest pT
   if (L1clusters[phibin].size()==0) continue;
      
   // sort L1 clusters max -> min
   sort(L1clusters[phibin].begin(),L1clusters[phibin].end(),[] ( struct EtaPhiBin &a, struct EtaPhiBin &b){ return a.pTtot>b.pTtot;  } );
   for (unsigned int imax = 0; imax<L1clusters[phibin].size(); ++imax){
     if (L1clusters[phibin][imax].used ) continue;
     ...
     for (unsigned int icluster = 0; icluster<L1clusters[phibin + 1].size(); ++icluster) {
       if (L1clusters[phibin + 1][icluster].used)
          continue;
       if (fabs(L1clusters[phibin + 1][icluster].eta - L1clusters[phibin][imax].eta) > 1.5 * etaStep_) 
          continue;
    ...
    for (unsigned int icluster = 0; icluster<L1clusters[phibin + 2].size(); ++icluster) {
      if (L1clusters[phibin + 2][icluster].used)
          continue;
      if (fabs(L1clusters[phibin + 2][icluster].eta - L1clusters[phibin][imax].eta) > 1.5 * etaStep_) 
          continue;
  ...
  for (int m = 0; m < nclust - 1; ++m) {
    for (int n = m + 1; n < nclust; ++n){
      if (clusters[n].eta != clusters[m].eta) 
          continue;
      if ( fabs(DPhi(clusters[n].phi, clusters[m].phi))>1.5 * phiStep_)
          continue;
