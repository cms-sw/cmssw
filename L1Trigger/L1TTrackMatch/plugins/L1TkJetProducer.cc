...
private:
  ...
  const edm::EDGetTokenT< JetBxCollection > jetToken;
...
void L1TkJetProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  // ----------------------------------------------------------------------------------------------
    ...
    // L1 jets
  edm::Handle< JetBxCollection> CentralJetHandle;
  iEvent.getByToken(jetToken,CentralJetHandle);
  std::vector<Jet>::const_iterator jetIter;
  ...
  if ( !CentralJetHandle.isValid() ) {
	...
	if (deltaR < max_deltaR) { //use tracks within deltaR < max_deltaR of the jet
	  ...
	  edm::Ptr< L1TTTrackType > trkPtr( L1TTTrackHandle, itrk) ;
  ...
  iEvent.put( std::move(cenTkJets), "Central");
