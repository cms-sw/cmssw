...
void  L1TrackTripletEmulatorProducer::produce(Event &iEvent, const EventSetup &iSetup) {
  ...
  TkTriplet trkTriplet(math::XYZTLorentzVector(tripletPx,tripletPy,tripletPz,tripletP), 
                        triplet_charge, pair_masses[0], pair_masses[2], 
                        pair_dzs[0], pair_dzs[2],
  ...
  l1t::TkTripletWord::ditrack_maxmass_t bitDiTrackMaxMass = DoubleToBit(pair_masses[0],
  ...
  l1t::TkTripletWord::ditrack_minz0_t bitDiTrackMinZ0 = DoubleToBit(pair_dzs[2],
  ...
  l1t::TkTripletWord::ditrack_maxz0_t bitDiTrackMaxZ0 = DoubleToBit(pair_dzs[0],
  ...
  l1t::TkTripletWord bitTriplet(val, bitPt, bitEta, bitPhi, bitMass, bitCharge, bitDiTrackMinZ0, bitDiTrackMaxMass, bitDiTrackMinZ0, bitDiTrackMaxZ0, unassigned);
...
bool L1TrackTripletEmulatorProducer::TrackSelector(L1TrackTripletEmulatorProducer::L1track &track, double PVz, double pt_min_cut, double eta_max_cut, double mva_min_cut, double dz_max_cut, int nstub_min_cut){
    return (track.Pt>=pt_min_cut) && (fabs(track.Eta)<=eta_max_cut) && (track.MVA>=mva_min_cut) && (fabs(track.Z0-PVz)<=dz_max_cut) && (track.Nstubs>= nstub_min_cut); 
}
...
double L1TrackTripletEmulatorProducer::FloatPhiFromBits(const L1TTTrackType &track){
   ...
   return BitToDouble(digiphi, TTTrack_TrackWord::TrackBitWidths::kPhiSize + kExtraGlobalPhiBit, TTTrack_TrackWord::stepPhi0);
}
