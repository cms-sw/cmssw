    ...
    if (withSystematics && !toysNoSystematics_ && (readToysFromHere == 0)) {
      ...
      if (toysFrequentist_) {
          ...
          w->saveSnapshot("reallyClean", w->allVars());
          {
              ...
              mc_bonly->GetPdf()->fitTo(*dobs, RooFit::Save(1), RooFit::Minimizer("Minuit2","minimize"), RooFit::Strategy(0), RooFit::Hesse(0), RooFit::Constrain(*mc_bonly->GetNuisanceParameters()));
          }
          w->saveSnapshot("clean", w->allVars());
