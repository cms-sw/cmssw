...
void runExternal(const char *file, const char *algo, double tol, const char *wsp, const char *datan, const char *mcn) {
    ...
    if (w->var("MH")) w->var("MH")->setVal(115.);
    //w->Print("V");
...
#else
    double xminR = 1.0, xminS = 1.0, yminR, yminS, y1R, y1S, q0R, q0S;
    nuis = clean;
...
int main(int argc, char **argv) {
    ...
    if (argc >= 2) {
        if (strstr(argv[1],"root")) {
            runExternal(argv[1], 
                        argc >= 3 ? argv[2] : "Minuit2",  
                        argc >= 4 ? atof(argv[2]) : 0.0001,  
                        "w",  
                        "data_obs", 
                        "ModelConfig");
        } else if (argc >= 4){
