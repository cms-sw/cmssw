class Datacard():
    ...
    def path_to_shape(self, bin, proc) :
        """
        ...
        with the exception that the keywords $CHANNEL correp. to bin and $PROCESS corresp. to proc are replaced by their
        corresponding real values. Other potential keywords remain unchanged.
        """
        return self.barcode(bin, proc, 1).replace('$CHANNEL',bin).replace('$PROCESS',proc)

    def path_to_uncert(self, bin, proc) :
        """
        ...
        the datacard with the exception that the keywords $CHANNEL correp. to bin and $PROCESS corresp. to proc are
        replaced by their corresponding real values. Other potential keywords remain unchanged.
        """
        return self.barcode(bin, proc, 2).replace('$CHANNEL',bin).replace('$PROCESS',proc)
