...
PhotonFix::PhotonFix(double e, double eta, double phi, double r9) : _e(e), _eta(eta), _phi(phi), _r9(r9) { setup(); }
...
double PhotonFix::fixedEnergy() const {
  ...
  if (_hl == 0) {
    f += _meanR9[_be][_hl][1] * (_r9 - _meanR9[_be][_hl][0]) * (_r9 - _meanR9[_be][_hl][0]) +
         _meanR9[_be][_hl][2] * (_r9 - _meanR9[_be][_hl][0]) * (_r9 - _meanR9[_be][_hl][0]) *
             (_r9 - _meanR9[_be][_hl][0]);
  } else {
...
double PhotonFix::sigmaEnergy() const {
  ...
  if (_hl == 0) {
    f += _sigmaR9[_be][_hl][1] * (_r9 - _sigmaR9[_be][_hl][0]) * (_r9 - _sigmaR9[_be][_hl][0]) +
         _sigmaR9[_be][_hl][2] * (_r9 - _sigmaR9[_be][_hl][0]) * (_r9 - _sigmaR9[_be][_hl][0]) *
             (_r9 - _sigmaR9[_be][_hl][0]);
...
double PhotonFix::rawEnergy() const { return _e; }

double PhotonFix::eta() const { return _eta; }

double PhotonFix::phi() const { return _phi; }

double PhotonFix::r9() const { return _r9; }
...
double PhotonFix::GetaPhi(double f0, double f1) { return aPhi(f0, f1); }

void PhotonFix::barrelCGap(unsigned i, unsigned j, unsigned k, double c) { _barrelCGap[i][j][k] = c; }
void PhotonFix::barrelSGap(unsigned i, unsigned j, unsigned k, double c) { _barrelSGap[i][j][k] = c; }
void PhotonFix::barrelMGap(unsigned i, unsigned j, unsigned k, double c) { _barrelMGap[i][j][k] = c; }
void PhotonFix::endcapCrystal(unsigned i, unsigned j, bool c) { _endcapCrystal[i][j] = c; }
void PhotonFix::endcapCGap(unsigned i, unsigned j, unsigned k, double c) { _endcapCGap[i][j][k] = c; }
void PhotonFix::endcapSGap(unsigned i, unsigned j, unsigned k, double c) { _endcapSGap[i][j][k] = c; }
void PhotonFix::endcapMGap(unsigned i, unsigned j, unsigned k, double c) { _endcapMGap[i][j][k] = c; }

void PhotonFix::print() const {
  std::cout << "PhotonFix:  e,eta,phi,r9 = " << _e << ", " << _eta << ", " << _phi << ", " << _r9 << ", gaps " << _aC
            << ", " << _aS << ", " << _aM << ", " << _bC << ", " << _bS << ", " << _bM << std::endl;
}
...
void PhotonFix::printParameters(std::ostream &o) {
  ...
  for (unsigned be(0); be < 2; be++) {
    for (unsigned hl(0); hl < 2; hl++) {
      o << " Parameters for " << (be == 0 ? "barrel" : "endcap") << ", " << (hl == 0 ? "high" : "low") << " R9"
        << std::endl;

      o << "  Mean  scaling        ";
      for (unsigned i(0); i < 4; i++)
        o << std::setw(14) << _meanScale[be][hl][i];
      o << std::endl;
      o << "  Mean  " << (be == 0 ? "Eta  " : "ZetaX") << " total    ";
      for (unsigned i(0); i < 4; i++)
        o << std::setw(14) << _meanAT[be][hl][i];
      o << std::endl;
      o << "  Mean  " << (be == 0 ? "Eta  " : "ZetaX") << " crystal  ";
      for (unsigned i(0); i < 4; i++)
        o << std::setw(14) << _meanAC[be][hl][i];
      o << std::endl;
      o << "  Mean  " << (be == 0 ? "Eta  " : "ZetaX") << " submodule";
      for (unsigned i(0); i < 4; i++)
        o << std::setw(14) << _meanAS[be][hl][i];
      o << std::endl;
      o << "  Mean  " << (be == 0 ? "Eta  " : "ZetaX") << " module   ";
      for (unsigned i(0); i < 4; i++)
        o << std::setw(14) << _meanAM[be][hl][i];
      o << std::endl;
      o << "  Mean  " << (be == 0 ? "Eta   zero     " : "ZetaY total    ");
      for (unsigned i(0); i < 4; i++)
        o << std::setw(14) << _meanBT[be][hl][i];
      o << std::endl;
      o << "  Mean  " << (be == 0 ? "Phi  " : "ZetaY") << " crystal  ";
      for (unsigned i(0); i < 4; i++)
        o << std::setw(14) << _meanBC[be][hl][i];
      o << std::endl;
      o << "  Mean  " << (be == 0 ? "Phi  " : "ZetaY") << " submodule";
      for (unsigned i(0); i < 4; i++)
        o << std::setw(14) << _meanBS[be][hl][i];
      o << std::endl;
      o << "  Mean  " << (be == 0 ? "Phi  " : "ZetaY") << " module   ";
      for (unsigned i(0); i < 4; i++)
        o << std::setw(14) << _meanBM[be][hl][i];
      o << std::endl;
      o << "  Mean  R9             ";
      for (unsigned i(0); i < 4; i++)
        o << std::setw(14) << _meanR9[be][hl][i];
      o << std::endl;

      o << "  Sigma scaling        ";
      for (unsigned i(0); i < 4; i++)
        o << std::setw(14) << _sigmaScale[be][hl][i];
      o << std::endl;
      o << "  Sigma " << (be == 0 ? "Eta  " : "ZetaX") << " total    ";
      for (unsigned i(0); i < 4; i++)
        o << std::setw(14) << _sigmaAT[be][hl][i];
      o << std::endl;
      o << "  Sigma " << (be == 0 ? "Eta  " : "ZetaX") << " crystal  ";
      for (unsigned i(0); i < 4; i++)
        o << std::setw(14) << _sigmaAC[be][hl][i];
      o << std::endl;
      o << "  Sigma " << (be == 0 ? "Eta  " : "ZetaX") << " submodule";
      for (unsigned i(0); i < 4; i++)
        o << std::setw(14) << _sigmaAS[be][hl][i];
      o << std::endl;
      o << "  Sigma " << (be == 0 ? "Eta  " : "ZetaX") << " module   ";
      for (unsigned i(0); i < 4; i++)
        o << std::setw(14) << _sigmaAM[be][hl][i];
      o << std::endl;
      o << "  Sigma " << (be == 0 ? "Eta  " : "ZetaY") << " total    ";
      for (unsigned i(0); i < 4; i++)
        o << std::setw(14) << _sigmaBT[be][hl][i];
      o << std::endl;
      o << "  Sigma " << (be == 0 ? "Eta  " : "ZetaY") << " crystal  ";
      for (unsigned i(0); i < 4; i++)
        o << std::setw(14) << _sigmaBC[be][hl][i];
      o << std::endl;
      o << "  Sigma " << (be == 0 ? "Phi  " : "ZetaY") << " submodule";
      for (unsigned i(0); i < 4; i++)
        o << std::setw(14) << _sigmaBS[be][hl][i];
      o << std::endl;
      o << "  Sigma " << (be == 0 ? "Phi  " : "ZetaY") << " module   ";
      for (unsigned i(0); i < 4; i++)
        o << std::setw(14) << _sigmaBM[be][hl][i];
      o << std::endl;
      o << "  Sigma R9             ";
      for (unsigned i(0); i < 4; i++)
        o << std::setw(14) << _sigmaR9[be][hl][i];
      o << std::endl;
    }
  }
}
...
double PhotonFix::expCorrection(double a, const double *p) {
  ...
  double b(a - p[0]);
  if (b >= 0.0)
    return p[1] * exp(-fabs(p[2]) * b);
  else
    return p[1] * exp(fabs(p[3]) * b);
}
...
double PhotonFix::gausCorrection(double a, const double *p) {
  ...
  double b(a - p[0]);
  if (b >= 0.0)
    return p[1] * exp(-0.5 * p[2] * p[2] * b * b);
  else
    return p[1] * exp(-0.5 * p[3] * p[3] * b * b);
}
bool PhotonFix::initialised() { return _initialised; }
