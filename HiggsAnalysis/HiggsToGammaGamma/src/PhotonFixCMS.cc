...
#include "HiggsAnalysis/HiggsToGammaGamma/interface/PhotonFixCMS.h"
...
bool PhotonFixCMS::initialise(const edm::EventSetup &iSetup, const std::string &s) {
  ...
  for (unsigned i(0); i < 169; i++) {
    for (unsigned j(0); j < 360; j++) {
      ...
      double eta = 0.25 * (bc[i][j][0] + bc[i + 1][j][0] + bc[i][k][0] + bc[i + 1][k][0]);
      ...
      if ((j % 20) == 19) {
        if (i == 19) {
          PhotonFix::barrelMGap(0, j / 20, 0, eta);
          PhotonFix::barrelMGap(0, j / 20, 1, phi);
        }
        if (i == 39) {
          PhotonFix::barrelMGap(1, j / 20, 0, eta);
          PhotonFix::barrelMGap(1, j / 20, 1, phi);
        }
        if (i == 59) {
          PhotonFix::barrelMGap(2, j / 20, 0, eta);
          PhotonFix::barrelMGap(2, j / 20, 1, phi);
        }
        if (i == 84) {
          PhotonFix::barrelMGap(3, j / 20, 0, eta);
          PhotonFix::barrelMGap(3, j / 20, 1, phi);
        }
        if (i == 109) {
          PhotonFix::barrelMGap(4, j / 20, 0, eta);
          PhotonFix::barrelMGap(4, j / 20, 1, phi);
        }
        if (i == 129) {
          PhotonFix::barrelMGap(5, j / 20, 0, eta);
          PhotonFix::barrelMGap(5, j / 20, 1, phi);
        }
        if (i == 149) {
          PhotonFix::barrelMGap(6, j / 20, 0, eta);
          PhotonFix::barrelMGap(6, j / 20, 1, phi);
        }
      }
    }
  }
  ...
  for (int iz(0); iz < 2; iz++) {
    for (int ix(0); ix < 100; ix++) {
      for (int iy(0); iy < 100; iy++) {
        ...
        if (iz == 0)
          PhotonFix::endcapCrystal(ix, iy, valid[ix][iy]);
        if (valid[ix][iy]) {
  ...
  for (unsigned iz(0); iz < 2; iz++) {
    ...
    std::cout << "Endcap number of crystal, submodule boundaries = " << nC << ", " << nS << std::endl;
  }
...
double PhotonFixCMS::fixedEnergy() const { return pf.fixedEnergy(); }

double PhotonFixCMS::sigmaEnergy() const { return pf.sigmaEnergy(); }

const PhotonFix &PhotonFixCMS::photonFix() const { return pf; }
