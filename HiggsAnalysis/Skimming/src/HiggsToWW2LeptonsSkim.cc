...
HiggsToWW2LeptonsSkim::HiggsToWW2LeptonsSkim(const edm::ParameterSet& iConfig) : nEvents_(0), nAccepted_(0) {
  ...
  if (muTracks.isValid()) {
    ...
    for (muons = muTracks->begin(); muons != muTracks->end(); ++muons) {
      if (muons->eta() > etaMin_ && muons->eta() < etaMax_) {
        if (muons->pt() > singleTrackPtMin_)
          accepted1 = true;
        if (muons->pt() > diTrackPtMin_)
          nTrackOver2ndCut++;
      }
    }
  }
  ...
  if (pTracks.isValid()) {
    ...
    for (electrons = eTracks->begin(); electrons != eTracks->end(); ++electrons) {
      if (electrons->eta() > etaMin_ && electrons->eta() < etaMax_) {
        if (electrons->pt() > singleTrackPtMin_)
          accepted1 = true;
        if (electrons->pt() > diTrackPtMin_)
          nTrackOver2ndCut++;
      }
    }
  }

  if (accepted1 && nTrackOver2ndCut >= 2)
    accepted = true;
