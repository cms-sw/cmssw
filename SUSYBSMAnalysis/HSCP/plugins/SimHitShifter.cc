...
SimHitShifter::SimHitShifter(const edm::ParameterSet& iConfig) {
  ...
  ShiftFileName =
      iConfig.getUntrackedParameter<std::string>("ShiftFileName",
                                                 ...
                                                 "simhitshifter/SimHitShifter/Merged_Muon_RawId_Shift.txt");
    ...
    if (simdetid.det() == DetId::Muon && simdetid.subdetId() == MuonSubdetId::RPC) {  //Only RPCs
      ...
      if (shiftinfo.find(TheChamberDetId.rawId()) == shiftinfo.end()) {
        ...
        if (TheChamberDetId.station() == 1 && TheChamberDetId.ring() == 4) {
          CSCDetId TheChamberDetIdNoring4 = CSCDetId(TheChamberDetId.endcap(),
                                                     TheChamberDetId.station(),
                                                     1  //1 instead of 4
                                                     ,
                                                     TheChamberDetId.chamber(),
                                                     TheChamberDetId.layer());
...
void SimHitShifter::beginRun(const edm::Run& run, const edm::EventSetup& iSetup) {}

// ------------ method called once each job just before starting event loop  ------------
void SimHitShifter::beginJob() {}

// ------------ method called once each job just after ending the event loop  ------------
void SimHitShifter::endJob() {}
