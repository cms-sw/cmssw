...
// Class:      BetaFromSTA
// 
/**\class BetaFromSTA BetaFromSTA.cc SUSYBSMAnalysis/BetaFromSTA/src/BetaFromSTA.cc
...
// $Id: BetaFromSTA.cc,v 1.12 2008/01/16 17:54:12 ptraczyk Exp $
//
//
...
class BetaFromSTA : public edm::EDProducer {
  public:
      explicit BetaFromSTA(const edm::ParameterSet&);
      ~BetaFromSTA();
...
//
BetaFromSTA::BetaFromSTA(const edm::ParameterSet& iConfig)
  :
...
void
BetaFromSTA::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
...
//    textplot(x,y,left);
  ...
  if (debug) 
    cout << " *** Beta from STA End ***" << endl;

}


void BetaFromSTA::processSegment( const DTRecSegment2D* si, vector<double> &dstnc, vector<double> &dsegm, 
                                  vector<double> &dtraj, vector<double> &hitWeight, vector<double> &left ) {
...
double
BetaFromSTA::fitT0(double &a, double &b, vector<double> xl, vector<double> yl, vector<double> xr, vector<double> yr ) {
...
void 
BetaFromSTA::rawFit(double &a, double &da, double &b, double &db, const vector<double> hitsx, const vector<double> hitsy) {
...
void 
BetaFromSTA::textplot(vector<double> x, vector <double> y, vector <double> side)
{
...
void 
BetaFromSTA::beginJob(const edm::EventSetup&)
{
}
