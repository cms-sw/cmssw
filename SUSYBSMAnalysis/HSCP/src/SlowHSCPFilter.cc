...
class SlowHSCPFilter : public edm::EDFilter {
   ...
   private:
      ...
      std::vector<SimTrack*> FindPrimaryHSCP   ();
      int                    ComesFromWhichHSCP(unsigned int TrackId, std::vector<SimTrack*> HSCPs);


      std::vector< SimTrack >  TrackColl;
      std::vector< SimVertex > VertexColl;
...
*/
  ...
  TrackColl = *h_Tracks.product();

  Handle<std::vector< SimVertex > > h_Vertex;
  iEvent.getByLabel("g4SimHits", h_Vertex);
  VertexColl = *h_Vertex.product();

  std::vector<SimTrack*> PrimaryHSCPTracks = FindPrimaryHSCP();
  ...
  for(k=0 ; k<CSC_Hits.size();k++)
  {
     if(abs(CSC_Hits[k].particleType()) >10000){
        ...
        unsigned int HSCP_Id      = ComesFromWhichHSCP(CSC_Hits[k].trackId(), PrimaryHSCPTracks);
...
//        printf(" : tof (if beta=1) =  %6.2fns --> DeltaT = %6.2f\n", T0, CSC_Hits[k].timeOfFlight() - T0);
  ...
  for(k=0 ; k<DT_Hits.size();k++)
  {
     if(abs(DT_Hits[k].particleType()) >10000){
        unsigned int HSCP_Id      = ComesFromWhichHSCP(DT_Hits[k].trackId(), PrimaryHSCPTracks);
...
*/
  ...
  int recoL1Muon[2];
  for(unsigned int i=0;(i<PrimaryHSCPTracks.size()&&i<2);i++){
...
int
SlowHSCPFilter::ClosestL1Muon(double phi, double eta, double dRMax, const l1extra::L1MuonParticleCollection L1_Muons)
{
...
int
SlowHSCPFilter::ComesFromWhichHSCP(unsigned int TrackId, std::vector<SimTrack*> HSCPs)
{
   SimTrack* Track = NULL;
