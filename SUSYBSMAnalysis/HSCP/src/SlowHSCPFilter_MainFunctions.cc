...
void
GetTrueL1MuonsAndTime(const edm::Event& iEvent, const edm::EventSetup& iSetup, int* recoL1Muon, double* MinDt)
{
...
//  iSetup.get<MuonGeometryRecord>().get(RPC_Geom);
   ...
   for(k=0 ; k<CSC_Hits.size();k++)
   {
      if(abs(CSC_Hits[k].particleType()) >10000){
         ...
         unsigned int HSCP_Id      = SlowHSCPFilter_ComesFromWhichHSCP(CSC_Hits[k].trackId(), PrimaryHSCPTracks, TrackColl, VertexColl);
...
//        printf(" : tof (if beta=1) =  %6.2fns --> DeltaT = %6.2f\n", T0, CSC_Hits[k].timeOfFlight() - T0);
   ...
   for(k=0 ; k<DT_Hits.size();k++)
   {
      if(abs(DT_Hits[k].particleType()) >10000){
         unsigned int HSCP_Id      = SlowHSCPFilter_ComesFromWhichHSCP(DT_Hits[k].trackId(), PrimaryHSCPTracks, TrackColl, VertexColl);
...
//  } 
  ...
  for(unsigned int i=0;i<PrimaryHSCPTracks.size();i++){
     ...
     recoL1Muon[i] = SlowHSCPFilter_ClosestL1Muon(PrimaryHSCPTracks[i]->momentum().phi(), PrimaryHSCPTracks[i]->momentum().eta(), 0.3, L1_Muons);
...
int
SlowHSCPFilter_ClosestL1Muon(double phi, double eta, double dRMax, const l1extra::L1MuonParticleCollection L1_Muons)
{
       double dR = 99999; int J=-1;

       for(unsigned int j=0;j<L1_Muons.size();j++){
          if(dR > SlowHSCPFilter_DeltaR(phi,eta, L1_Muons[j].phi(), L1_Muons[j].eta()) ){
             dR = SlowHSCPFilter_DeltaR(phi,eta, L1_Muons[j].phi(), L1_Muons[j].eta());
             J  = j;
          }
       }
...
int
SlowHSCPFilter_ComesFromWhichHSCP(unsigned int TrackId, std::vector<SimTrack*> HSCPs, std::vector<SimTrack>  TrackColl, std::vector<SimVertex> VertexColl)
{
   SimTrack* Track = NULL;
   ...
   for(unsigned int i=0;i<HSCPs.size();i++){
      double DR = SlowHSCPFilter_DeltaR(Track->momentum().phi(), Track->momentum().eta(), HSCPs[i]->momentum().phi(), HSCPs[i]->momentum().eta() );
...
double
SlowHSCPFilter_DeltaR(double phi1, double eta1, double phi2, double eta2)
{
