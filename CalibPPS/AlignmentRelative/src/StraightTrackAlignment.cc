...
void StraightTrackAlignment::finish()
{
    ...
    for (const auto &dit : task.geometry.getSensorMap())
    {
      ...
      for (unsigned int q = 0; q < task.quantityClasses.size(); q++)
      {
        for (unsigned int a = 0; a < results.size(); a++)
        {
          ...
          {
            case AlignmentTask::qcShR1: v = ac.getShR1(); e = ac.getShR1Unc(); break;
            case AlignmentTask::qcShR2: v = ac.getShR2(); e = ac.getShR2Unc(); break;
            case AlignmentTask::qcShZ: v = ac.getShZ(); e = ac.getShZUnc(); break;
            case AlignmentTask::qcRotZ: v = ac.getRotZ(); e = ac.getRotZUnc(); break;
          }
  ...
  for (unsigned int a = 0; a < results.size(); a++)
  {
    ...
    for (const auto &p : results[a])
    {
      ...
      const double sh_x_e = sqrt( pow(dir2.dy/det * p.second.getShR1Unc(), 2) + pow(dir1.dy/det * p.second.getShR2Unc(), 2) );
      const double sh_y_e = sqrt( pow(dir2.dx/det * p.second.getShR1Unc(), 2) + pow(dir1.dx/det * p.second.getShR2Unc(), 2) );

      const CTPPSRPAlignmentCorrectionData corr(
        sh_x, sh_x_e,
        sh_y, sh_y_e,
        p.second.getShZ(), p.second.getShZUnc(),
        0., 0.,
        0., 0.,
        p.second.getRotZ(), p.second.getRotZUnc()
      );
    ...
    {
      CTPPSRPAlignmentCorrectionsMethods::writeToXML(convertedAlignments, fileNamePrefix + algorithms[a]->getName() + ".xml",
        preciseXMLFormat, saveXMLUncertainties, true, true, true, true);
    }
    ...
    if (!expandedFileNamePrefix.empty() || !factoredFileNamePrefix.empty())
    {
      ...
      factorRPFromSensorCorrections(cumulativeAlignments, expandedAlignments, factoredAlignments,
        task.geometry, equalWeights, verbosity);

      if (!expandedFileNamePrefix.empty())
      {
        CTPPSRPAlignmentCorrectionsMethods::writeToXML(expandedAlignments, expandedFileNamePrefix + algorithms[a]->getName() + ".xml",
          preciseXMLFormat, saveXMLUncertainties, true, true, true, true);
      }

      if (!factoredFileNamePrefix.empty())
      {
        CTPPSRPAlignmentCorrectionsMethods::writeToXML(factoredAlignments, factoredFileNamePrefix + algorithms[a]->getName() + ".xml",
          preciseXMLFormat, saveXMLUncertainties, true, true, true, true);
      }
    }
  }
