  ...
  std::map<unsigned int, AlignmentResult> &results, TDirectory *dir)
{
  ...
  */
  ...
  if (task->resolveShR)
  {
    TVectorD fm_ShX_gl(dim); fm_ShX_gl.Zero();
    TVectorD fm_ShX_lp(dim); fm_ShX_lp.Zero();
    TVectorD fm_ShY_gl(dim); fm_ShY_gl.Zero();
    TVectorD fm_ShY_lp(dim); fm_ShY_lp.Zero();
  ...
  if (task->resolveRotZ)
  {
    TVectorD fm_RotZ_gl(dim); fm_RotZ_gl.Zero();
    TVectorD fm_RotZ_lp(dim); fm_RotZ_lp.Zero();
    ...
    for (const auto &sp : task->geometry.getSensorMap())
    {
      ...
      for (int m = 0; m < 2; ++m)
      {
        double rho = 0.;
        TVectorD *fmp = NULL;
        if (m == 0) { rho = 1.; fmp = &fm_RotZ_gl; }
        if (m == 1) { rho = geom.z; fmp = &fm_RotZ_lp; }
        TVectorD &fm = *fmp;
  ...
  for (const auto &dit : task->geometry.getSensorMap())
  {
    ...
    for (unsigned int qci = 0; qci < task->quantityClasses.size(); ++qci)
    {
      ...
      switch (qc)
      {
        case AlignmentTask::qcShR1: r.setShR1(v); break;
        case AlignmentTask::qcShR2: r.setShR2(v); break;
        case AlignmentTask::qcShZ: r.setShZ(v); break;
        case AlignmentTask::qcRotZ: r.setRotZ(v); break;
      }
    }
