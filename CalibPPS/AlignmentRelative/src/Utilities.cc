...
void factorRPFromSensorCorrections(const CTPPSRPAlignmentCorrectionsData &inputAlignments,
  CTPPSRPAlignmentCorrectionsData &expandedAlignments, CTPPSRPAlignmentCorrectionsData &factoredAlignments,
  ...
  for (auto rpit : sensorsPerRP)
  {
    ...
    for (const auto &senId : sensors)
    {
      ...
      if (rpId.subdetId() == CTPPSDetId::sdTrackingStrip)
      {
        ...
        double unc = sqrt( pow(d2.dx * fullAlignments[senId].getShXUnc(), 2) + pow(d2.dy * fullAlignments[senId].getShYUnc(), 2) );
        if (unc <= 0.)
          unc = 1.;
    ...
    if (verbosity)
    {
      printf("    m_sh_x = (%.1f +- %.1f) um, m_sh_y = (%.1f +- %.1f) um, m_sh_z = (%.1f +- %.1f) mm\n",
        m_sh_x*1E3, m_sh_x_unc*1E3, m_sh_y*1E3, m_sh_y_unc*1E3, m_sh_z, m_sh_z_unc);
      printf("    m_rot_x = (%.1f +- %.1f) mrad, m_rot_y = (%.1f +- %.1f)  mrad, m_rot_z = (%.1f +- %.1f) mrad\n",
        m_rot_x*1E3, m_rot_x_unc*1E3, m_rot_y*1E3, m_rot_y_unc*1E3, m_rot_z*1E3, m_rot_z_unc*1E3);
    }

    factoredAlignments.addRPCorrection(rpId, CTPPSRPAlignmentCorrectionData(
      m_sh_x, m_sh_x_unc, m_sh_y, m_sh_y_unc, m_sh_z, m_sh_z_unc,
      m_rot_x, m_rot_x_unc, m_rot_y, m_rot_y_unc, m_rot_z, m_rot_z_unc
    ));
    ...
    for (const auto &senId : sensors)
    {
      ...
      if (rpId.subdetId() == CTPPSDetId::sdTrackingStrip)
      {
        ...
        const double de_s_unc = std::abs(d2.dx * fullAlignments[senId].getShXUnc() + d2.dy * fullAlignments[senId].getShYUnc()); // the x and y components are fully correlated

        rc = CTPPSRPAlignmentCorrectionData(
          d2.dx * de_s, d2.dx * de_s_unc,
          d2.dy * de_s, d2.dy * de_s_unc,
          fullAlignments[senId].getShZ() - m_sh_z, fullAlignments[senId].getShZUnc(),
          fullAlignments[senId].getRotX() - m_rot_x, fullAlignments[senId].getRotXUnc(),
          fullAlignments[senId].getRotY() - m_rot_y, fullAlignments[senId].getRotYUnc(),
          fullAlignments[senId].getRotZ() - m_rot_z, fullAlignments[senId].getRotZUnc()
        );
      }
      ...
      {
        rc = CTPPSRPAlignmentCorrectionData(
          fullAlignments[senId].getShX() - m_sh_x, fullAlignments[senId].getShXUnc(),
          fullAlignments[senId].getShY() - m_sh_y, fullAlignments[senId].getShYUnc(),
          fullAlignments[senId].getShZ() - m_sh_z, fullAlignments[senId].getShZUnc(),
          fullAlignments[senId].getRotX() - m_rot_x, fullAlignments[senId].getRotXUnc(),
          fullAlignments[senId].getRotY() - m_rot_y, fullAlignments[senId].getRotYUnc(),
          fullAlignments[senId].getRotZ() - m_rot_z, fullAlignments[senId].getRotZUnc()
