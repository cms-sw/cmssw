  ...
  map<unsigned int, AlignmentResult> &results, TDirectory *dir)
{
  ...
  for (const auto &dit : task->geometry.getSensorMap())
  {
    ...
    for (unsigned int i = 0; i < task->quantityClasses.size(); i++)
    {
      ...
      switch (task->quantityClasses[i])
      {
        case AlignmentTask::qcShR1: r.setShR1(v); r.setShR1Unc(e); break;
        case AlignmentTask::qcShR2: r.setShR2(v); r.setShR2Unc(e); break;
        case AlignmentTask::qcShZ: r.setShZ(v); r.setShZUnc(e); break;
        case AlignmentTask::qcRotZ: r.setRotZ(v); r.setRotZUnc(e); break;
      }
    }
