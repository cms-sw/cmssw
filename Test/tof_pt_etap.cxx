...
{
    Double_t value=42.0;Double_t etap=par[0];Double_t zcoord=par[1];
    Double_t natcharge=par[2];Double_t invmass=par[3]; Double_t theta=2*ATan(Exp(-etap));
    Double_t relr=(*pt)*(1.0E9)/(C()*natcharge*B);
    ...
    Double_t Tz1=(Zhalflength-zcoord)*natcharge*1.60217657*B*Tan(theta)/((*pt)*5.344286);
    Double_t deltaeta=-Log(Tan(0.5*ATan(BarrelR/(relr*Tfinal/Tan(theta)+zcoord))))-eta_n;
    if(etap>=0.0 && Abs(Tfinal)>Abs(Tz1)) {Tfinal=Tz1;
...
void tof_pt_etap()
{
    ...
    TF1 *f1 = new TF1( "p1",tof_pt_etap, 0.8,50.,5);
    f1->SetParameters(0.0,0.1,1.0,9.1E-31,0.0);f1->SetLineColor(42);
    f1->Draw();f1->GetHistogram()->GetXaxis()->SetTitle("p_t [GeV]");f1->GetHistogram()->GetYaxis()->SetTitle("#Delta t [s]");
    f1->SetMinimum(1.0E-15);
    TF1 *f2 = new TF1( "p2",tof_pt_etap, 0.8,50.,5);
    f2->SetParameters(1.0,0.1,1.0,9.1E-31,0.0);f2->SetLineColor(13);
    f2->Draw("SAME");
    TF1 *f3 = new TF1( "p3",tof_pt_etap, 0.8,50.,5);
    f3->SetParameters(1.57,0.1,1.0,9.1E-31,0.0);f3->SetLineColor(33);
    f3->Draw("SAME");
    TF1 *f4 = new TF1( "p4",tof_pt_etap, 0.8,50.,5);
    f4->SetParameters(2.5,0.1,1.0,9.1E-31,0.0);f4->SetLineColor(66);
    f4->Draw("SAME");
    gPad->SetLogx();
    c1->Draw();
    legend->SetHeader("z=0.10m");
    legend->AddEntry(f1,"#eta_p=0.00","l");
    legend->AddEntry(f2,"#eta_p=1.00","l");
    legend->AddEntry(f3,"#eta_p=1.57","l");
    legend->AddEntry(f4,"#eta_p=2.50","l");
    legend->Draw();
}
