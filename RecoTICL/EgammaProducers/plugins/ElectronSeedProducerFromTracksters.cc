// Author: Riccardo Salvatico <riccardo.salvatico@cern.ch>
// Date: 05/2025
...
class ElectronSeedProducerFromTracksters : public edm::stream::EDProducer<> {
public:
  explicit ElectronSeedProducerFromTracksters(const edm::ParameterSet&);
...
private:
    //This will be a pain in the ass to replace
...
ElectronSeedProducerFromTracksters::ElectronSeedProducerFromTracksters(const edm::ParameterSet& conf)
    :
...
void ElectronSeedProducerFromTracksters::produce(edm::Event& e, const edm::EventSetup& iSetup) {
  LogDebug("ElectronSeedProducerFromTracksters") << "[ElectronSeedProducerFromTracksters::produce] entering ";
  ...
  for (auto const& seed : *seeds) {
    ...
    LogDebug("ElectronSeedProducerFromTracksters") << "new seed with " << seed.nHits() << " hits"
...
SuperClusterRefVector ElectronSeedProducerFromTracksters::filterClusters(
  ...
  LogDebug("ElectronSeedProducerFromTracksters") << "Filtered out " << sclRefs.size() << " superclusters from "
                                   << superClusters->size();

  return sclRefs;
}

void ElectronSeedProducerFromTracksters::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  desc.add<edm::InputTag>("endcapSuperClusters",
                          {"ticlSuperClusters", "ticlTracksterLinksSuperclusteringDNN"});

  descriptions.add("HGCalDrivenElectronSeedsDefault", desc);
}

#include "FWCore/Framework/interface/MakerMacros.h"
DEFINE_FWK_MODULE(ElectronSeedProducerFromTracksters);
