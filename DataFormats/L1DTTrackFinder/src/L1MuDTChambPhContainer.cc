...
bool L1MuDTChambPhContainer::bxEmpty(int step) const {
  ...
  for (const auto & phiSegment : phiSegments) {
    if  (step == phiSegment.bxNum()) empty = false;
  }
...
int L1MuDTChambPhContainer::bxSize(int step1, int step2) const {
  ...
  for (const auto & phiSegment : phiSegments) {
    if  (step1 <= phiSegment.bxNum() && step2 >= phiSegment.bxNum() 
      && phiSegment.Ts2Tag() == 0 && phiSegment.code() != 7) size++;
    if  (step1 <= phiSegment.bxNum()-1 && step2 >= phiSegment.bxNum()-1 
      && phiSegment.Ts2Tag() == 1 && phiSegment.code() != 7) size++;
  }
