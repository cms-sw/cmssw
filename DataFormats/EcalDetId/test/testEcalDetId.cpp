      ...
      try {
        {
          ...
          CPPUNIT_ASSERT(
              !(EBDetId::isNextToEtaBoundary(aPositiveId) ^
                (ieta == 1 ||
                 std::binary_search(EBDetId::kModuleBoundaries,
                                    EBDetId::kModuleBoundaries + 4, ieta))));
          ...
          CPPUNIT_ASSERT(EBDetId::unhashIndex(aPositiveId.hashedIndex()) ==
                         aPositiveId);
        ...
        // EBDetId Zside -1
        {
          ...
          CPPUNIT_ASSERT(EBDetId::unhashIndex(aNegativeId.hashedIndex()) ==
                         aNegativeId);
          detIds.at(aNegativeId.hashedIndex()) = aNegativeId;
        }
      } catch (cms::Exception &e) {
        std::cout << e.what() << " failed for " << ieta << "," << iphi
                  << std::endl;
    ...
    for (int iy = EEDetId::IY_MIN; iy <= EEDetId::IY_MAX; iy++)
      try {
        ...
        if (EEDetId::slowValidDetId(ix, iy)) {
          ...
          CPPUNIT_ASSERT(EEDetId::unhashIndex(aPositiveId.hashedIndex()) ==
                         aPositiveId);
          detIds.at(aPositiveId.hashedIndex()) = aPositiveId;
        } else {
        ...
        if (EEDetId::slowValidDetId(ix, iy)) {
          ...
          CPPUNIT_ASSERT(EEDetId::unhashIndex(aNegativeId.hashedIndex()) ==
                         aNegativeId);
          detIds.at(aNegativeId.hashedIndex()) = aNegativeId;
        }
      } catch (cms::Exception &e) {
...
void testEcalDetId::testESDetId() {
  ...
  for (int istrip = ESDetId::ISTRIP_MIN; istrip <= ESDetId::ISTRIP_MAX;
       istrip++)
...
void testEcalDetId::testEcalTrigTowerDetId() {
  ...
  for (int ieta = EcalTrigTowerDetId::MIN_I; ieta <= EcalTrigTowerDetId::MAX_I;
       ieta++)
    for (int iphi = EcalTrigTowerDetId::MIN_I;
         iphi <= EcalTrigTowerDetId::MAX_I; iphi++) {
      try {
...
void testEcalDetId::testEcalElectronicsId() {
  for (int
idcc=EcalElectronicsId::MIN_DCCID;idcc<=EcalElectronicsId::MAX_DCCID;idcc++) for
(int
itower=EcalElectronicsId::MIN_TOWERID;itower<EcalElectronicsId::MAX_TOWERID;itower++)
      for (int
ichannel=EcalElectronicsId::MIN_CHANNELID;ichannel<EcalElectronicsId::MAX_CHANNELID;ichannel++)
        {
          try
            {
