...
void testEcalContainer::testContainer() {
      ...
      try {
        {
          ...
          EcalContainer<EBDetId, float>::const_iterator citer =
              ec.find(aPositiveId.rawId());
          if (citer != ec.end()) {
            CPPUNIT_ASSERT(*(ec.find(aPositiveId.rawId())) ==
                           aPositiveId.hashedIndex());
          }
        }
        ...
        // EBDetId Zside -1
        {
          ...
          EcalContainer<EBDetId, float>::const_iterator citer =
              ec.find(aNegativeId.rawId());
          if (citer != ec.end()) {
            CPPUNIT_ASSERT(*(ec.find(aNegativeId.rawId())) ==
                           aNegativeId.hashedIndex());
          }
        }
  ...
  float i = 0.;
  for (EcalContainer<EBDetId, float>::const_iterator citer = ec.begin();
       citer != ec.end(); citer++) {
