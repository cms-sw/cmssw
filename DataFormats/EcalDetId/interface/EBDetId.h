...
public:
  ...
  EBDetId(int crystal_ieta, int crystal_iphi) : DetId(Ecal, EcalBarrel) {
    id_ |= ((crystal_ieta > 0) ? (0x10000 | (crystal_ieta << 9))
                               : ((-crystal_ieta) << 9)) |
           (crystal_iphi & 0x1FF);
  }
  // slow
  ...
  EcalTrigTowerDetId tower() const {
    return EcalTrigTowerDetId(zside(), EcalBarrel, abs(tower_ieta()),
                              tower_iphi());
  }
  /// get the ECAL/SM id
  int ism() const {
  ...
  int numberByEtaPhi() const {
    return (MAX_IETA + (positiveZ() ? ietaAbs() - 1 : -ietaAbs())) * MAX_IPHI +
           iphi() - 1;
  }
  ...
  /** returns a new EBDetId offset by nrStepsEta and nrStepsPhi (can be
   * negative), returns EBDetId(0) if invalid */
  ...
  static EBDetId unhashIndex(int hi) {
    ...
    return (validHashIndex(hi)
                ? EBDetId(pseudo_eta < 0 ? pseudo_eta : pseudo_eta + 1,
                          hi % MAX_IPHI + 1)
                : EBDetId());
  }
  ...
  static bool validDetId(int i, int j) {
    return i != 0 && (std::abs(i) <= MAX_IETA) && (j >= MIN_IPHI) &&
           (j <= MAX_IPHI);
  }
