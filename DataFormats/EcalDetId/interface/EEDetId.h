...
public:
   ...
   * @param mode pass XYMODE if i j refer to ix, iy, SCCRYSTALMODE if thery
   * refer to isc, ic
   */
  // fast
  EEDetId(int crystal_ix, int crystal_iy, int iz) : DetId(Ecal, EcalEndcap) {
    id_ |= (crystal_iy & 0x7f) | ((crystal_ix & 0x7f) << 7) |
           ((iz > 0) ? (0x4000) : (0));
  }
  // slow
  ...
  /** Gets the SuperCrystal number within the endcap. This number runs from 1 to
   * 316, numbers 70 149 228 307 are not used.
   *
   * BEWARE: This number is not consistent with indices used in constructor: see
   * details below.
   *
   ...
   * \endverbatim
   *
   * Quadrant 2 indices are deduced by a symetry about y-axis and by adding an
   * offset of 79.<br> Quadrant 3 and 4 indices are deduced from quadrant 1 and
   * 2 by a symetry about x-axis and adding an offset. Quadrant N starts with
   * index 1 + (N-1)*79.
   *
   * <p>EE- indices are deduced from EE+ by a symetry about (x,y)-plane
   * (mirrored view). <b>It is inconsistent with indices used in constructor
   * EEDetId(int, int,int) in SCCRYSTALMODE</b>. Indices of constructor uses a
   * symetry along y-axis: in principal it considers the isc as a local index.
   * The discrepancy is most probably due to a bug in the implementation of this
   * isc() method.
   */
  int isc() const;
  ...
  /** Gets a DetId from a compact index for arrays. Converse of hashedIndex()
   * method.
  ...
  static bool validDetId(int crystal_ix, int crystal_iy, int iz) {
    return crystal_ix >= IX_MIN && crystal_ix <= IX_MAX &&
           crystal_iy >= IY_MIN && crystal_iy <= IY_MAX && std::abs(iz) == 1 &&
