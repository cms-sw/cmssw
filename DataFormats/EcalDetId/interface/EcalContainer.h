...
template <typename DetId, typename T> class EcalContainer {
...
public:
  ...
  void checkAndResize() {
    if (m_items.empty()) {
      ...
      // DetId::kSizeForDenseIndexing << std::endl;
      m_items.resize(DetId::kSizeForDenseIndexing);
    }
  }

  void checkAndResize(size_t priv_size) {
    // this method allows to resize the vector to a specific size forcing a
    // specific value
    if (m_items.empty()) {
      //		    std::cout << "resizing to " << priv_size <<
      // std::endl;
  ...
  inline Item const &operator[](uint32_t rawId) const {
    //                        if (m_items.size()==0) {
    //	  std::cout << "resizing to " << DetId::kSizeForDenseIndexing <<
    // std::endl;
