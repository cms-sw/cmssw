...
EcalPnDiodeDetId::EcalPnDiodeDetId(int EcalSubDetectorId, int DCCId, int PnId)
    : DetId(Ecal, EcalLaserPnDiode) {
  if ((DCCId < MIN_DCCID) || (DCCId > MAX_DCCID) || (PnId < MIN_PNID) ||
      (PnId > MAX_PNID) ||
  ...
  id_ |= ((((EcalSubDetectorId == EcalBarrel) ? (0) : (1)) << 11) |
          ((DCCId & 0x7F) << 4) | (PnId & 0xF));
}

EcalPnDiodeDetId::EcalPnDiodeDetId(const DetId &gen) {
  if (!gen.null() &&
      (gen.det() != Ecal || gen.subdetId() != EcalLaserPnDiode)) {
    throw cms::Exception("InvalidDetId");
  }
  id_ = gen.rawId();
}
...
std::ostream &operator<<(std::ostream &s, const EcalPnDiodeDetId &id) {
  return s << "(EcalPnDiode " << id.iEcalSubDetectorId() << ',' << id.iDCCId()
           << ',' << id.iPnId() << ')';
}
