...
EcalTrigTowerDetId::EcalTrigTowerDetId(int zside, EcalSubdetector subDet, int i,
                                       int j, int mode)
  ...
  if (mode == SUBDETIJMODE) {
    ...
    throw cms::Exception("InvalidDetId")
        << "EcalTriggerTowerDetId:  Cannot create object. SUBDETDCCTTMODE not "
           "yet implemented.";
  } else
    throw cms::Exception("InvalidDetId")
        << "EcalTriggerTowerDetId:  Cannot create object.  Unknown mode for "
           "(int, EcalSubdetector, int, int) constructor.";

  if (tower_i > MAX_I || tower_i < MIN_I || tower_j > MAX_J || tower_j < MIN_J)
    throw cms::Exception("InvalidDetId")
        << "EcalTriggerTowerDetId:  Cannot create object.  Indexes out of "
           "bounds.";

  id_ |= ((zside > 0) ? (0x8000) : (0x0)) |
         ((subDet == EcalBarrel) ? (0x4000) : (0x0)) | (tower_i << 7) |
         (tower_j & 0x7F);
}

EcalTrigTowerDetId::EcalTrigTowerDetId(const DetId &gen) {
  if (!gen.null() &&
      (gen.det() != Ecal || gen.subdetId() != EcalTriggerTower)) {
    throw cms::Exception("InvalidDetId");
  }
  id_ = gen.rawId();
}
...
int EcalTrigTowerDetId::iquadrant() const {
  if (subDet() == EcalEndcap)
    ...
    throw cms::Exception("MethodNotApplicable")
        << "EcalTriggerTowerDetId: iquadrant not applicable";
}

bool EcalTrigTowerDetId::validDetId(int iz, EcalSubdetector sd, int i, int j) {
  return (1 == abs(iz) && 0 < i && 0 < j && kEETowersInPhiPerEndcap >= j &&
          ((EcalBarrel == sd && kEBTowersInEta >= i) ||
           (EcalEndcap == sd && kEEOuterEta <= i && kEEInnerEta >= i &&
  ...
  return (
      subDet() == EcalBarrel
          ...
          : kEBTotalTowers + ((zside() + 1) / 2) * kEETowersPerEndcap +
                ((iea < 27 ? iea : 27) - kEEOuterEta) *
                    kEETowersInPhiPerEndcap +
                (iea < 27 ? iph : // for iphi=27,28 only half TT present, odd
                                  // for EE-, even EE+
...
EcalTrigTowerDetId EcalTrigTowerDetId::detIdFromDenseIndex(uint32_t di) {
  ...
  if (di < kEBTotalTowers) // barrel
  {
    ...
    j = (idc % 18) * kEBTowersInPhi + ((1 + iz) / 2) * kEBTowersInPhi -
...
std::ostream &operator<<(std::ostream &s, const EcalTrigTowerDetId &id) {
  return s << "(EcalTT subDet "
           << ((id.subDet() == EcalBarrel) ? ("Barrel") : ("Endcap")) << " iz "
           << ((id.zside() > 0) ? ("+ ") : ("- ")) << " ieta " << id.ietaAbs()
           << " iphi " << id.iphi() << ')';
}
