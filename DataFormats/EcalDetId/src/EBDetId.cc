...
EBDetId::EBDetId(int index1, int index2, int mode) : DetId(Ecal, EcalBarrel) {
  ...
  if (mode == ETAPHIMODE) {
    ...
    throw cms::Exception("InvalidDetId")
        << "EBDetId:  Cannot create object.  Unknown mode for (int, int) "
           "constructor.";
  }

  if (!validDetId(crystal_ieta, crystal_iphi)) {
    //    std::cout << "crystal_eta " << crystal_ieta << "crystal_phi " <<
    //    crystal_iphi << std::endl;
    throw cms::Exception("InvalidDetId")
...
int EBDetId::ic() const {
  ...
  return (ie * kCrystalsInPhi) +
         (positiveZ() ? (kCrystalsInPhi - ((iphi() - 1) % kCrystalsInPhi))
...
bool EBDetId::isNextToEtaBoundary(EBDetId id) {
  ...
  return ieta == 1 ||
         (kModuleBoundaries + 4) !=
             std::find(kModuleBoundaries, kModuleBoundaries + 4, ieta);
}
...
std::ostream &operator<<(std::ostream &s, const EBDetId &id) {
  return s << "(EB ieta " << id.ieta() << ", iphi " << id.iphi() << " ; ism "
           << id.ism() << " , ic " << id.ic() << ')';
}
