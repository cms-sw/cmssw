...
short EcalScDetId::xyz2HashedIndex[EcalScDetId::IX_MAX][EcalScDetId::IY_MAX]
...
EcalScDetId::EcalScDetId(int ix, int iy, int iz) : DetId(Ecal, EcalEndcap) {
  ...
  const int scBit =
      1 << 15; // bit set to 1 to distinguish from crystal id (EEDetId)
  //                         and for a reasonale behaviour of DetId ccomparison
  //                         operators.
  id_ |=
      (iy & 0x7f) | ((ix & 0x7f) << 7) | ((iz > 0) ? (1 << 14) : (0)) | scBit;
}
...
bool EcalScDetId::validDetId(int iX, int iY, int iZ) {
  static const char endcapMap[401] = {"       XXXXXX       "
                                      "    XXXXXXXXXXXX    "
                                      "   XXXXXXXXXXXXXX   "
                                      "  XXXXXXXXXXXXXXXX  "
                                      " XXXXXXXXXXXXXXXXXX "
    ...
    for (int iZ = -1; iZ <= +1; iZ += 2) {
      for (int iY = IY_MIN; iY <= IY_MAX; ++iY) {
        for (int iX = IX_MIN; iX <= IX_MAX; ++iX) {
          if (validDetId(iX, iY, iZ)) {
            xyz2HashedIndex[iX - IX_MIN][iY - IY_MIN][iZ > 0 ? 1 : 0] =
                ++hashedIndex;
            assert((unsigned)hashedIndex <
                   sizeof(hashedIndex2DetId) / sizeof(hashedIndex2DetId[0]));
