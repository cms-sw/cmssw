...
public:
  ...
  public:
    ...
    constexpr Pixel(uint32_t pix_x, uint32_t pix_y, uint32_t pix_adc) : x(pix_x), y(pix_y), adc(pix_adc) {}
    uint32_t x;
    uint32_t y;
    uint32_t adc;
  };
  ...
  Phase2ITPixelCluster(unsigned int isize,
                       uint32_t const* adcs,
                       uint32_t const* xpos,
                       uint32_t const* ypos,
                       uint32_t const xmin,
  ...
  inline uint32_t maxPixelRow() const {
    verifyVersion();
    return minPixelRow() + rowSpan();
  }                                                                     // The max x index.
  inline uint32_t minPixelCol() const { return thePixelCol & MAXPOS; }  // The min y index.
  inline uint32_t maxPixelCol() const {
    verifyVersion();
    return minPixelCol() + colSpan();
  }  // The max y index.
...
public:
  ...
  void packCol(uint32_t ymin, uint32_t yspan) { thePixelCol = pack_(ymin, yspan); }
  void packRow(uint32_t xmin, uint32_t xspan) { thePixelRow = pack_(xmin, xspan); }

  /// mostly to be compatible for <610
  void verifyVersion() const {
    if
      UNLIKELY(thePixelRow < MAXPOS && thePixelCol < MAXPOS)
