...
HcalCalibDataFrame::HcalCalibDataFrame(const HcalCalibDetId& id) : id_(id), size_(0), hcalPresamples_(0) {
...
void HcalCalibDataFrame::setPresamples(int ps) { hcalPresamples_ |= ps & 0xF; }
void HcalCalibDataFrame::setReadoutIds(const HcalElectronicsId& eid) { electronicsId_ = eid; }
...
void HcalCalibDataFrame::setZSInfo(bool unsuppressed, bool markAndPass, uint32_t crossingMask) {
