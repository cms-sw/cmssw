...
int  RPCRawDataCounts::fedFormatErrors(int fedId) const
{
  for (const auto & theReadoutError : theReadoutErrors) {
    if (theReadoutError.first.first != fedId) continue;
    if (theReadoutError.first.second > ReadoutError::NoProblem && theReadoutError.first.second <= ReadoutError::InconsistentDataSize) return 1; 
  }
  return 0;
}
...
int  RPCRawDataCounts::fedErrorRecords(int fedId) const
{
  for (const auto & theRecordType : theRecordTypes) {
    if (theRecordType.first.first != fedId) continue;
    if (theRecordType.first.second > DataRecord::Empty) return 1; 
  }
  return 0;
}
...
std::string RPCRawDataCounts::print() const 
{
  ...
  for (const auto & theReadoutError : theReadoutErrors) {
    str <<"ERROR("<<theReadoutError.first.first<<","<<theReadoutError.first.second<<")="<<theReadoutError.second<<endl;
  } 
  return str.str();
}
