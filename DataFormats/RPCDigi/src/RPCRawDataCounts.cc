...
int RPCRawDataCounts::fedFormatErrors(int fedId) const {
  for (auto im = theReadoutErrors.begin(); im != theReadoutErrors.end(); ++im) {
...
int RPCRawDataCounts::fedErrorRecords(int fedId) const {
  for (auto im = theRecordTypes.begin(); im != theRecordTypes.end(); ++im) {
    if (im->first.first != fedId)
      continue;
...
void RPCRawDataCounts::operator+=(const RPCRawDataCounts& o) {
  for (auto irt = o.theRecordTypes.begin(); irt != o.theRecordTypes.end(); ++irt) {
    theRecordTypes[make_pair(irt->first.first, irt->first.second)] += irt->second;
  }

  for (auto ire = o.theReadoutErrors.begin(); ire != o.theReadoutErrors.end(); ++ire) {
    theReadoutErrors[make_pair(ire->first.first, ire->first.second)] += ire->second;
  }

  for (auto ire = o.theGoodEvents.begin(); ire != o.theGoodEvents.end(); ++ire) {
    theGoodEvents[make_pair(ire->first.first, ire->first.second)] += ire->second;
  }

  for (auto ire = o.theBadEvents.begin(); ire != o.theBadEvents.end(); ++ire) {
...
std::string RPCRawDataCounts::print() const {
  ...
  for (auto irt = theRecordTypes.begin(); irt != theRecordTypes.end(); ++irt) {
    str << "RECORD (" << irt->first.first << "," << irt->first.second << ")" << irt->second;
  }
  for (auto ire = theReadoutErrors.begin(); ire != theReadoutErrors.end(); ++ire) {
