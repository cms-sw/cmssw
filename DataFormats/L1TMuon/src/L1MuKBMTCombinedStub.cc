...
L1MuKBMTCombinedStub::L1MuKBMTCombinedStub() :
  whNum_(0),scNum_(0),stNum_(0),phi_(0), phiB_(0),tag_(false), quality_(-1), bxNum_(17),coarseEta_(0),
  eta1_(0),eta2_(0),qeta1_(-1),qeta2_(-1) {}


L1MuKBMTCombinedStub::L1MuKBMTCombinedStub(int wheel,int sector,int station,int phi,int phiB,bool tag,int bx,int quality,int coarseEta,int eta1,int eta2, int qeta1,int qeta2):
  whNum_(wheel),
  scNum_(sector),
  stNum_(station),
  phi_(phi),
...
bool L1MuKBMTCombinedStub::operator==(const L1MuKBMTCombinedStub& id) const {
  ...
  if ( coarseEta_  != id.coarseEta_ ) return false;
...
ostream& operator<<(ostream& s, const L1MuKBMTCombinedStub& id) {
  ...
  << "coarse eta:" << setw(4) <<id.coarseEta_ << " "
