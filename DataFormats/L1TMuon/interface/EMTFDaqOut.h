...
namespace l1t {
  ...
  /*   {}; */
    ...
    void set_Counters(emtf::Counters bits)           { Counters = bits;
                                                       hasCounters = true;            }
    void set_MECollection(emtf::MECollection bits)   { MECollection = bits;
                                                       numME = MECollection.size();   }
    void push_ME(emtf::ME bits)                      { MECollection.push_back(bits);
                                                       numME += 1;                    }
    void set_RPCCollection(emtf::RPCCollection bits) { RPCCollection = bits;
                                                       numRPC = RPCCollection.size(); }
    void push_RPC(emtf::RPC bits)                    { RPCCollection.push_back(bits);
                                                       numRPC += 1;                   }
    void set_SPCollection(emtf::SPCollection bits)   { SPCollection = bits;
