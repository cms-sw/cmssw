...
namespace l1t {
  ...
  /*   {}; */
    ...
    void set_AMC13Header(const emtf::AMC13Header& bits)     { AMC13Header = bits;
                                                       hasAMC13Header = true;         }
    void set_MTF7Header(const emtf::MTF7Header& bits)       { MTF7Header = bits;
                                                       hasMTF7Header = true;          }
    void set_EventHeader(const emtf::EventHeader& bits)     { EventHeader = bits;
                                                       hasEventHeader = true;         }
    void set_Counters(const emtf::Counters& bits)           { Counters = bits;
                                                       hasCounters = true;            }
    void set_MECollection(emtf::MECollection bits)   { MECollection = std::move(bits);
                                                       numME = MECollection.size();   }
    void push_ME(const emtf::ME& bits)                      { MECollection.push_back(bits);
                                                       numME += 1;                    }
    void set_RPCCollection(emtf::RPCCollection bits) { RPCCollection = std::move(bits);
                                                       numRPC = RPCCollection.size(); }
    void push_RPC(const emtf::RPC& bits)                    { RPCCollection.push_back(bits);
                                                       numRPC += 1;                   }
    void set_SPCollection(emtf::SPCollection bits)   { SPCollection = std::move(bits);
                                                       numSP = SPCollection.size();   }
    void push_SP(const emtf::SP& bits)                      { SPCollection.push_back(bits);
                                                       numSP += 1;                    }
    void set_EventTrailer(const emtf::EventTrailer& bits)   { EventTrailer = bits;
                                                       hasEventTrailer = true;        }
    void set_MTF7Trailer(const emtf::MTF7Trailer& bits)     { MTF7Trailer = bits;
                                                       hasMTF7Trailer  = true;        }
    void set_AMC13Trailer(const emtf::AMC13Trailer& bits)   { AMC13Trailer = bits;
