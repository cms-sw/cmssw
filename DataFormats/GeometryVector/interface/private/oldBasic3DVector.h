...
public:
  ...
  Basic3DVector(const Basic3DVector& p) : theX(p.x()), theY(p.y()), theZ(p.z()), theW(p.w()) {}

  /// Copy constructor and implicit conversion from Basic3DVector of different precision
  template <class U>
  Basic3DVector(const Basic3DVector<U>& p) : theX(p.x()), theY(p.y()), theZ(p.z()), theW(p.w()) {}
  ...
  Basic3DVector(const T& x, const T& y, const T& z, const T& w = 0) : theX(x), theY(y), theZ(z), theW(w) {}
#else
  /// construct from cartesian coordinates
  Basic3DVector(const T& x, const T& y, const T& z) : theX(x), theY(y), theZ(z), theW(0) {}
  Basic3DVector(const T& x, const T& y, const T& z, const T& w) : theX(x), theY(y), theZ(z), theW(w) {}
#endif
