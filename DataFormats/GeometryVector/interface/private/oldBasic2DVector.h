...
public:
  ...
  Basic2DVector(const Basic2DVector& p) : theX(p.x()), theY(p.y()) {}
  ...
  template <typename U>
  Basic2DVector(Vec2<U> const& iv) : theX(iv[0]), theY(iv[1]) {}
  template <typename U>
  Basic2DVector(Vec4<U> const& iv) : theX(iv.arr[0]), theY(iv.arr[1]) {}
#elif defined(USE_SSEVECT)
  // constructor from Vec2 or vec4
  template <typename U>
  Basic2DVector(mathSSE::Vec2<U> const& iv) : theX(iv.arr[0]), theY(iv.arr[1]) {}
  template <typename U>
  Basic2DVector(mathSSE::Vec4<U> const& iv) : theX(iv.arr[0]), theY(iv.arr[1]) {}
#endif
