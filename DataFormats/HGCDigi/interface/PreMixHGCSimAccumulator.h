#ifndef DataFormats_HGCDigi_PreMixHGCSimAccumulator_h
#define DataFormats_HGCDigi_PreMixHGCSimAccumulator_h
...
public:
  ...
  PreMixHGCSimAccumulator() = default;
  ~PreMixHGCSimAccumulator() = default;
  ...
  public:
    // begin
    const_iterator(const PreMixHGCSimAccumulator* ncc)
        : ncc_(ncc), iDet_(0), iData_(0), endData_(ncc->detIdSize_.empty() ? 0 : ncc->detIdSize_.front().size()) {}
    // end
    const_iterator(const PreMixHGCSimAccumulator* ncc, unsigned int detSize, unsigned int dataSize)
  ...
  private:
    const PreMixHGCSimAccumulator* ncc_;
    unsigned int iDet_;
    unsigned int iData_;
