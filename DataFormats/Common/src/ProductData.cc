...
namespace edm {
  ...
  ProductData::ProductData(const std::shared_ptr<BranchDescription const>& bd) : wrapper_(), prov_(bd, ProductID()) {}

  // For use by FWLite
  ProductData::ProductData(WrapperBase* product, Provenance const& prov)
      : wrapper_(product, do_nothing_deleter()), prov_(prov) {}

  void ProductData::resetBranchDescription(const std::shared_ptr<BranchDescription const>& bd) {
