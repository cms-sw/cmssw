...
public:
  ...
  void test(const MultiRef &);
  void test(const MultiVal &);
  void test2(const MultiRef &);
#ifdef private
  void dump(const MultiRef::Indices &);
  template <typename T> void dump(const RefVector<T> &);
  template <typename T> void dump(const std::vector<T> &);
  void dump(const MultiRef &, const char *when);
  void dump(const MultiVal &, const char *when);
#else
  template <typename T> void dump(const RefVector<T> &) {}
  template <typename T> void dump(const std::vector<T> &) {}
  void dump(const MultiRef &, const char *when) {}
  ...
  std::vector<int> w1, w2;
    ...
    void fastFillRefs(const edm::TestHandle<Key> &handle, MultiRef &map, const UnaryFunc &u, const BinaryFunc &f) {
    ...
    template<typename Key, typename UnaryFunc, typename BinaryFunc> 
    void lazyFillRefs(const edm::TestHandle<Key> &handle, MultiRef &map, const UnaryFunc &u, const BinaryFunc &f, bool swap) {
    ...
    template<typename Key, typename UnaryFunc, typename BinaryFunc> 
    void fastFillVals(const edm::TestHandle<Key> &handle, MultiVal &map, const UnaryFunc &u, const BinaryFunc &f) {
    ...
    template<typename Key, typename UnaryFunc, typename BinaryFunc> 
    void lazyFillVals(const edm::TestHandle<Key> &handle, MultiVal &map, const UnaryFunc &u, const BinaryFunc &f, bool swap) {
...
#ifdef private
void testMultiAssociation::dump(const MultiRef::Indices &indices) {
    using namespace std;
...
template<typename T>
void testMultiAssociation::dump(const edm::RefVector<T> &data) {
    using namespace std;
...
template<typename T>
void testMultiAssociation::dump(const std::vector<T> &data) {
    using namespace std;
    cerr <<  "  Dumping " << typeid(data).name() << " at " << &data << endl;
    cerr <<  "    Values (size = " << data.size() << ")" << endl;
    for (size_t i = 0; i < data.size(); ++i) {
...
void testMultiAssociation::dump(const MultiRef &assoc, const char *what) {
    using namespace std;
...
void testMultiAssociation::dump(const MultiVal &assoc, const char *what) {
    using namespace std;
...
void testMultiAssociation::test(const MultiRef & assoc) {
  // TEST contains
...
void testMultiAssociation::test2(const MultiRef & assoc) {
  // TEST contains
  CPPUNIT_ASSERT(!assoc.contains(ProductID(1)));
  CPPUNIT_ASSERT(assoc.contains(ProductID(2)));
  CPPUNIT_ASSERT(assoc.contains(ProductID(3)));
...
void testMultiAssociation::test(const MultiVal & assoc) {
#if 1
  // TEST Vector access
