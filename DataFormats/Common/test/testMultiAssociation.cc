...
namespace {
  ...
  void TestData::initialize() {
    ...
    for (size_t j = 0; j < 7; ++j) {
      ...
      REQUIRE(ptrs[j]->id() == 1);
      REQUIRE(ptrs[j]->addr() == &der1s[k]);
    }
  }

}  // namespace

TEST_CASE("MultiAssociation", "[MultiAssociation]") {
  TestData data;
  data.initialize();

  SECTION("checkAll") { data.checkAll(); }

  SECTION("checkVals") { data.checkVals(); }

  SECTION("checkTwoFillers") { data.checkTwoFillers(); }

  SECTION("checkUnsortedKeys") { data.checkUnsortedKeys(); }

  SECTION("checkBadFill") { data.checkBadFill(); }

  SECTION("checkBadRead") { data.checkBadRead(); }

  SECTION("checkWithPtr") { data.checkWithPtr(); }

  SECTION("checkWithOwn") { data.checkWithOwn(); }

  SECTION("checkWritableMap") { data.checkWritableMap(); }
}
...
namespace {
...
#ifdef private
  void TestData::dump(MultiRef::Indices const& indices) {
    using namespace std;
  ...
  template <typename T>
  void TestData::dump(RefVector<T> const& data) {
    using namespace std;
  ...
  template <typename T>
  void TestData::dump(std::vector<T> const& data) {
    using namespace std;
    cerr << "  Dumping " << typeid(data).name() << " at " << &data << endl;
    cerr << "    Values (size = " << data.size() << ")" << endl;
    for (size_t i = 0; i < data.size(); ++i) {
  ...
  void TestData::dump(MultiRef const& assoc, char const* what) {
    using namespace std;
  ...
  void TestData::dump(MultiVal const& assoc, char const* what) {
    using namespace std;
  ...
  void TestData::test(MultiRef const& assoc) {
    // TEST contains
    REQUIRE(!assoc.contains(ProductID(1)));
    REQUIRE(assoc.contains(ProductID(2)));
    REQUIRE(assoc.contains(ProductID(3)));
    REQUIRE(!assoc.contains(ProductID(4)));
    ...
    REQUIRE(br3.size() == 2);
    REQUIRE(br4.size() == 3);
    REQUIRE(br2.begin()->id() == ProductID(1));
    REQUIRE(br2.begin()->key() == 0);
    REQUIRE(**br2.begin() == k.front());
    REQUIRE(*br2.front() == k.front());
    REQUIRE(*br2[0] == k[0]);
    REQUIRE(br4.back().id() == ProductID(1));
    REQUIRE(br4.back().key() == 2);
    REQUIRE(br4[2].key() == 2);

    // TEST const_ranges
    // Check that ranges are consecutive
    br1 = assoc[edm::Ref<CKey1>(handleK1, 0)];
    REQUIRE(br1.end() == br1.begin());
    br2 = assoc[edm::Ref<CKey1>(handleK1, 1)];
    REQUIRE(br2.end() == br2.begin() + 1);
    REQUIRE(br2.begin() == br1.end());
    br3 = assoc[edm::Ref<CKey1>(handleK1, 2)];
    REQUIRE(br3.end() == br3.begin() + 2);
    REQUIRE(br3.begin() == br2.end());
    ...
    REQUIRE(br1.end() == br1.begin() + 3);
    REQUIRE(br1.end() == br2.begin() + 0);
    REQUIRE(br2.end() == br2.begin() + 0);
    REQUIRE(br2.end() == br3.begin() + 0);
    REQUIRE(br3.end() == br3.begin() + 1);
    ...
    REQUIRE(r3.size() == 2);
    REQUIRE(r4.size() == 3);
    REQUIRE(r2.begin()->id() == ProductID(1));
    REQUIRE(r2.begin()->key() == 0);
    REQUIRE(**r2.begin() == k.front());
    REQUIRE(**r2.begin() == k.front());
    REQUIRE((r4.end() - 1)->id() == ProductID(1));
    REQUIRE((r4.end() - 1)->key() == 2);
  }

  void TestData::test2(MultiRef const& assoc) {
    // TEST contains
    REQUIRE(!assoc.contains(ProductID(1)));
    REQUIRE(assoc.contains(ProductID(2)));
    REQUIRE(assoc.contains(ProductID(3)));
    ...
    br2 = assoc[edm::Ref<CKey2>(handleK2, 0)];
    br3 = assoc[edm::Ref<CKey2>(handleK2, 1)];
    REQUIRE(br1.end() == br1.begin() + 0);
    REQUIRE(br1.end() == br2.begin() + 0);
    REQUIRE(br2.end() == br2.begin() + 4);
    REQUIRE(br2.end() == br3.begin() + 0);
    REQUIRE(br3.end() == br3.begin() + 0);
  }

  void TestData::test(MultiVal const& assoc) {
#if 1
    // TEST Vector access
  ...
  template <typename Map>
  bool TestData::tryTwoFillers(bool lazyfiller) {
    Map map;
    if (lazyfiller) {
  ...
  void TestData::checkTwoFillers() {
    REQUIRE(tryTwoFillers<MultiRef>(true));
    REQUIRE_THROWS_AS(tryTwoFillers<MultiRef>(false), cms::Exception);
    REQUIRE(tryTwoFillers<MultiVal>(true));
    REQUIRE_THROWS_AS(tryTwoFillers<MultiVal>(false), cms::Exception);
  }
  template <typename Map>
  bool TestData::tryUnsortedKeys(bool lazy) {
    Map map;
  ...
  void TestData::checkUnsortedKeys() {
    REQUIRE(tryUnsortedKeys<MultiRef>(true));
    REQUIRE_THROWS_AS(tryUnsortedKeys<MultiRef>(false), cms::Exception);
    REQUIRE(tryUnsortedKeys<MultiVal>(true));
    REQUIRE_THROWS_AS(tryUnsortedKeys<MultiVal>(false), cms::Exception);
  }

  // i = even: succeed; i = odd: fail
  bool TestData::tryBadFill(int i) {
    MultiRef m;
  ...
  void TestData::checkBadFill() {
    for (int i = 0; i < 100; ++i) {
      if (i % 2 == 0)
        REQUIRE(tryBadFill(i));
      else
        REQUIRE_THROWS_AS(tryBadFill(i), cms::Exception);
    }
  }

  bool TestData::tryBadRead(int i) {
    using boost::lambda::_1;
    using boost::lambda::_2;
    MultiRef m;
  ...
  void TestData::checkWritableMap() {
    using boost::lambda::_1;
    using boost::lambda::_2;
    MultiVal tryRW;
  ...
  void TestData::checkBadRead() {
    for (int i = 0; i < 100; ++i) {
      if (i % 2 == 0)
        REQUIRE(tryBadRead(i));
      else
        REQUIRE_THROWS_AS(tryBadRead(i), cms::Exception);
    }
  }

  void TestData::checkWithPtr() {
    MultiPtr map;
    {  // Fill the map
    ...
    {  // Read the map
      for (size_t i = 0; i < handleK1->size(); ++i) {
        ...
        REQUIRE(static_cast<size_t>(r.size()) == ((i + 2) % 3));
        for (size_t j = 0; j < ((i + 2) % 3); ++j) {
          REQUIRE(r[j].key() == (3 * i + 4 * j) % 10);
          REQUIRE(r[j]->addr() == &der1s[(3 * i + 4 * j) % 10]);
          //REQUIRE(  (r.begin()+j)->key()   ==        (3*i+4*j)%10 );
          //REQUIRE(  (*(r.begin()+j))->addr() == &der1s[(3*i+4*j)%10]);
        }
      }
    }
  }
  void TestData::checkWithOwn() {
    MultiOwn map;
    {  // Fill the map
    ...
    {  // Read the map
      for (size_t i = 0; i < handleK1->size(); ++i) {
        ...
        REQUIRE(static_cast<size_t>(r.size()) == ((i + 2) % 3));
        for (size_t j = 0; j < ((i + 2) % 3); ++j) {
          REQUIRE((r.begin() + j)->id() == bases[(i + j) % 3].id());
        }
      }
    }
  }
