...
TEST_CASE("Ref", "[Ref]") {
  SECTION("construction") {
    ...
    REQUIRE(dummyRef.id() == pid);
    REQUIRE(dummyRefProd.id() == pid);
    REQUIRE(dummyRef.key() == key);
    REQUIRE(&(*dummyRef) == &dummyCollection[key]);
    REQUIRE((dummyRef.operator->()) == &dummyCollection[key]);
    REQUIRE(dummyRef->address() == dummyCollection[key].address());
    REQUIRE(&(*dummyRefProd) == &dummyCollection);
    REQUIRE((dummyRefProd.operator->()) == &dummyCollection);

    Ref<DummyCollection> testRef1(pid, &dummyCollection[key], key, nullptr);
    REQUIRE(testRef1.get() == &dummyCollection[key]);
    REQUIRE(testRef1.key() == key);
    REQUIRE(testRef1.isTransient() == false);
    REQUIRE(testRef1.id() == pid);

    Ref<DummyCollection> testRef2(pid, &dummyCollection[key], key);
    REQUIRE(testRef2.get() == &dummyCollection[key]);
    REQUIRE(testRef2.key() == key);
    REQUIRE(testRef2.isTransient() == false);
    REQUIRE(testRef2.id() == pid);

    Ref<DummyCollection> testRef3(pid, &dummyCollection[key], key, false);
    REQUIRE(testRef3.get() == &dummyCollection[key]);
    REQUIRE(testRef3.key() == key);
    REQUIRE(testRef3.isTransient() == false);
    REQUIRE(testRef3.id() == pid);

    Ref<DummyCollection> testRef4(pid, &dummyCollection[key], key, true);
    REQUIRE(testRef4.get() == &dummyCollection[key]);
    REQUIRE(testRef4.key() == key);
    REQUIRE(testRef4.isTransient() == true);
    REQUIRE(testRef4.id() == pid);

    Ref<DummyCollection> testRef5(&dummyCollection, key);
    REQUIRE(testRef5.get() == &dummyCollection[key]);
    REQUIRE(testRef5.key() == key);
    REQUIRE(testRef5.isTransient() == true);
    REQUIRE(testRef5.id() == edm::ProductID());
  }
  ...
  SECTION("comparison") {
    {
      ...
      REQUIRE(dummyRef1 == dummyRef2);
      REQUIRE(!(dummyRef1 != dummyRef2));
      REQUIRE(!(dummyRef1 < dummyRef2));
      REQUIRE(!(dummyRef2 < dummyRef1));
      REQUIRE(!(dummyRef1 < dummyRef1));
      REQUIRE(!(dummyRef2 < dummyRef2));

      REQUIRE(dummyRefProd1 == dummyRefProd2);
      REQUIRE(!(dummyRefProd1 != dummyRefProd2));
      REQUIRE(!(dummyRefProd1 < dummyRefProd2));
      REQUIRE(!(dummyRefProd2 < dummyRefProd1));
      REQUIRE(!(dummyRefProd1 < dummyRefProd1));
      REQUIRE(!(dummyRefProd2 < dummyRefProd2));

      Ref<DummyCollection> dummyRefNewKey(handle, key + 1);
      REQUIRE(!(dummyRef1 == dummyRefNewKey));
      REQUIRE(dummyRef1 != dummyRefNewKey);
      REQUIRE(dummyRef1 < dummyRefNewKey);
      REQUIRE(!(dummyRefNewKey < dummyRef1));
      ...
      REQUIRE(!(dummyRef1 == dummyRefNewPID));
      REQUIRE(dummyRef1 != dummyRefNewPID);
      REQUIRE(!(dummyRefProd1 == dummyRefProdNewPID));
      REQUIRE(dummyRefProd1 != dummyRefProdNewPID);
      REQUIRE(dummyRefProd1 < dummyRefProdNewPID);
      REQUIRE(!(dummyRefProdNewPID < dummyRefProd1));
    }
    {
      ...
      REQUIRE(dummyRef21 != dummyRef22);
      REQUIRE(dummyRef21 < dummyRef22);
      REQUIRE(!(dummyRef22 < dummyRef21));
      ...
      REQUIRE(dummyRef31 != dummyRef32);
      REQUIRE(!(dummyRef31 < dummyRef32));
      REQUIRE(dummyRef32 < dummyRef31);
    }
  }
  ...
  SECTION("get") {
    ...
    REQUIRE(0 == ref0FromHandle->value_);
    Ref<IntCollection> ref1FromHandle(handle, 1);
    REQUIRE(1 == ref1FromHandle->value_);
    ...
    REQUIRE(0 == (*refProd0)[0].value_);
    REQUIRE(1 == (*refProd0)[1].value_);
    REQUIRE(1 == (*refProd2)[1].value_);
    ...
    REQUIRE(vw[0].value_ == ref0->value_);
    //std::cerr << ">>> checking View element #1" << std::endl;
    REQUIRE(vw[1].value_ == ref1->value_);
