...
namespace io_v1 {
  ...
  inline unsigned L1MuGMTCand::readDataField(unsigned start, unsigned count) const {
    unsigned mask = ((1 << count) - 1) << start;
    return (m_dataWord & mask) >> start;
  }

  inline void L1MuGMTCand::writeDataField(unsigned start, unsigned count, unsigned value) {
    unsigned mask = ((1 << count) - 1) << start;
    m_dataWord &= ~mask;  // clear
    m_dataWord |= (value << start) & mask;
  }
}  // namespace io_v1
using L1MuGMTCand = io_v1::L1MuGMTCand;
#endif
