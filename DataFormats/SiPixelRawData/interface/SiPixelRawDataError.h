...
public:
  ...
  /// Constructor with 64-bit error word and type included (header or trailer
  /// word)
  ...
  void setWord32(
      cms_uint32_t errorWord32); // function to allow user to input the error
                                 // word (if 32-bit) after instantiation
  void setWord64(
      cms_uint64_t errorWord64); // function to allow user to input the error
                                 // word (if 64-bit) after instantiation
  void setType(int errorType); // function to allow user to input the error type
                               // after instantiation
  void setFedId(int fedId); // function to allow user to input the fedID after
                            // instantiation
  void setMessage(); // function to create an error message based on errorType

  inline cms_uint32_t getWord32() const {
    return errorWord32_;
  } // the 32-bit word that contains the error information
  inline cms_uint64_t getWord64() const {
    return errorWord64_;
  } // the 64-bit word that contains the error information
  inline int getType() const {
    return errorType_;
  } // the number associated with the error type (26-31 for ROC number errors,
    // 32-33 for calibration errors)
  inline int getFedId() const {
    return fedId_;
...
inline bool operator<(const SiPixelRawDataError &one,
