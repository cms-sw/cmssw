...
class SiPixelRawDataError {
  public:
    ...
    /// 32-bit word contructor that also includes detId information
    SiPixelRawDataError(const unsigned int errorWord32, const int errorType, uint32_t detId);
    ...
    void setDetId(uint32_t detId);			// function to allow user to input the error type after instantiation
    ...
    inline uint32_t getDetId() const {return detId_;} 	 // the plaquette that the error occured in
