...
/**
 * \class DigiContainerIteratorAdaptor MuonDigiCollection.h
...
template <typename IndexType, typename DigiType> class DigiContainerIterator {
public:
  ...
  typedef std::pair<IndexType, std::pair<DigiRangeIterator, DigiRangeIterator>>
      value_type;
  ...
  value_type operator*(void)const {
    return std::make_pair(base_->first, std::make_pair(base_->second.begin(),
                                                       base_->second.end()));
  }
...
template <typename IndexType, typename DigiType> class MuonDigiCollection {
...
#ifndef CMS_NOCXX11
  /// insert a range of digis for a  given DetUnit
  template <typename IRange> void move(IRange range, const IndexType &index) {
    std::vector<DigiType> &digis = data_[index];
    digis.reserve(digis.size() + (range.second - range.first));
    digis.insert(digis.end(), std::make_move_iterator(range.first),
                 std::make_move_iterator(range.second));
  }
#endif
...
inline void swap(MuonDigiCollection<IndexType, DigiType> &rh,
