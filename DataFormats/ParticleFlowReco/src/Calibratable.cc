...
std::ostream& pftools::operator<<(std::ostream& s, const Calibratable& calib_) {
  s << "Calibratable summary:\n\tsim:\t\t(" << calib_.sim_energyEvent_ << "),\t[" << calib_.sim_etaEcal_ << ", "
    << calib_.sim_phiEcal_ << "]\n";
  s << "\ttestbeam:\t(" << calib_.tb_energyEvent_ << ", " << calib_.tb_energyEcal_ << ", " << calib_.tb_energyHcal_
    << "), [" << calib_.tb_eta_ << ", " << calib_.tb_phi_ << "]\n";
  s << "\trechits:\t(" << calib_.rechits_energyEvent_ << ", " << calib_.rechits_energyEcal_ << ", "
    << calib_.rechits_energyHcal_ << ")\n";
  s << "\tcluster:\t(" << calib_.cluster_energyEvent_ << ", " << calib_.cluster_energyEcal_ << ", "
    << calib_.cluster_energyHcal_ << ")\n";
  s << "\tcands:\t\t(" << calib_.cand_energyEvent_ << ", " << calib_.cand_energyEcal_ << ", " << calib_.cand_energyHcal_
    << "), ";
  s << "\t[" << calib_.cand_eta_ << ", " << calib_.cand_phi_ << "], " << calib_.cands_num_ << " of them\n";
  for (std::vector<CandidateWrapper>::const_iterator c = calib_.cands_.begin(); c != calib_.cands_.end(); ++c) {
    const CandidateWrapper& cw = *c;
    s << "\t\t\t\tType: " << cw.type_ << ", (" << cw.energy_ << ", " << cw.energyEcal_ << ", " << cw.energyHcal_
      << ") at [ " << cw.eta_ << ", " << cw.phi_ << "]\n";
  }
...
std::ostream& pftools::operator<<(std::ostream& s, const CalibratableElement& ce) {
  s << "CalibratableElement: (energy, eta, phi) = (" << ce.energy_ << ", " << ce.eta_ << ", " << ce.phi_ << ")";
...
void Calibratable::recompute() {
  ...
  cluster_energyEvent_ =
      cluster_meanEcal_.energy_ * cluster_ecal_.size() + cluster_meanHcal_.energy_ * cluster_hcal_.size();
  ...
  rechits_energyEvent_ =
      rechits_meanEcal_.energy_ * rechits_ecal_.size() + rechits_meanHcal_.energy_ * rechits_hcal_.size();
  ...
  tb_energyEvent_ = tb_meanEcal_.energy_ * tb_ecal_.size() + tb_meanHcal_.energy_ * tb_hcal_.size();
  ...
  for (std::vector<CandidateWrapper>::iterator it = cands_.begin(); it != cands_.end(); ++it) {
...
CandidateWrapper Calibratable::computeMean(const std::vector<CandidateWrapper>& wrappers) {
  ...
  if (wrappers.empty())
    return cw;
  for (std::vector<CandidateWrapper>::const_iterator it = wrappers.begin(); it != wrappers.end(); ++it) {
...
CalibratableElement Calibratable::computeMean(const std::vector<CalibratableElement>& diets) {
  ...
  if (diets.empty())
    return dmean;
  for (std::vector<CalibratableElement>::const_iterator cit = diets.begin(); cit != diets.end(); ++cit) {
...
void Calibratable::fillCaloWindow(const std::vector<CalibratableElement>& source, CaloWindow& destination) const {
  ...
  for (; cit != source.end(); ++cit) {
    ...
    if (!ok)
      std::cout << __PRETTY_FUNCTION__ << ": couldn't fill CaloWindow with " << ce << "\n";
  }
}
