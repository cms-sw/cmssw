...
CaloRing::CaloRing() : panes_(1) { myPanes_.push_back(0); }

CaloRing::CaloRing(unsigned nPanes) : panes_(nPanes) {
...
double CaloRing::totalE() const {
  double ans(0.0);
  for (std::vector<double>::const_iterator cit = myPanes_.begin(); cit != myPanes_.end(); ++cit) {
    ans += *cit;
  }
  return ans;
}
...
void CaloRing::reset() {
  for (std::vector<double>::iterator cit = myPanes_.begin(); cit != myPanes_.end(); ++cit)
    *cit = 0.0;
}
...
std::ostream& pftools::operator<<(std::ostream& s, const pftools::CaloRing& caloRing) {
...
std::ostream& pftools::operator<<(std::ostream& s, const pftools::CaloWindow& caloWindow) {
  s << "CaloWindow at (" << caloWindow.baryEta() << ", " << caloWindow.baryPhi() << "):\n";
  double totalE(0.0);
  for (std::map<unsigned, pftools::CaloRing>::const_iterator cit = caloWindow.getRingDepositions().begin();
       cit != caloWindow.getRingDepositions().end();
       ++cit) {
...
vector<double> CaloWindow::stream(double normalisation) const {
  ...
  for (map<unsigned, pftools::CaloRing>::const_iterator cit = energies_.begin(); cit != energies_.end(); ++cit) {
...
void CaloWindow::printEnergies(std::ostream& s, double range) {
  for (std::map<unsigned, pftools::CaloRing>::const_iterator cit = energies_.begin(); cit != energies_.end(); ++cit) {
    CaloRing c = (*cit).second;
...
CaloWindow::CaloWindow() : baryEta_(0.0), baryPhi_(0.0), nRings_(1), deltaR_(0.1), nPanes_(1), axis_(0.0) {}
...
void CaloWindow::reset() { energies_.clear(); }
...
std::pair<unsigned, unsigned> CaloWindow::relativePosition(double eta, double phi) const {
  ...
  unsigned division =
      static_cast<unsigned>(floor((*energies_.find(ring)).second.getNPanes() * dTheta / (TMath::Pi() * 2)));
...
std::map<unsigned, double> CaloWindow::getRingEnergySummations() const {
  ...
  for (std::map<unsigned, CaloRing>::const_iterator cit = energies_.begin(); cit != energies_.end(); ++cit) {
