...
Point CaloEllipse::getMajorMinorAxes(double sigma) const {
  ...
  for (; it != dataPoints_.end(); ++it) {
    const Point& p = *it;
    double X = cos(theta) * (p.first - mean.first) - sin(theta) * (p.second - mean.second);
    double Y = sin(theta) * (p.first - mean.first) + cos(theta) * (p.second - mean.second);
    sum_xx += X * X;
    sum_yy += Y * Y;
  }
...
std::ostream& pftools::operator<<(std::ostream& s, const pftools::CaloEllipse& em) {
  s << "CaloEllipse at position = " << toString(em.getPosition()) << ", theta = " << em.getTheta()
    << ", major/minor axes = " << toString(em.getMajorMinorAxes()) << ", eccentricity = " << em.getEccentricity()
    << "\n";
