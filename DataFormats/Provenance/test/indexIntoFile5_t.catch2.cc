...
public:
  ...
  void testDuplicateCheckerFunctions() {
    ...
    REQUIRE(relevantPreviousEvents.empty());
    ...
    for (int j = 0; j < 2; ++j) {
      ...
      REQUIRE(!indexIntoFile11.containsDuplicateEvents());
      REQUIRE(indexIntoFile12.containsDuplicateEvents());
      REQUIRE(indexIntoFile22.containsDuplicateEvents());

      relevantPreviousEvents.clear();
      indexIntoFile11.set_intersection(indexIntoFile12, relevantPreviousEvents);
      REQUIRE(relevantPreviousEvents.size() == 3);
      std::set<IndexIntoFile::IndexRunLumiEventKey>::const_iterator iter = relevantPreviousEvents.begin();
      REQUIRE(iter->event() == 1);
      REQUIRE(iter->processHistoryIDIndex() == 0);
      REQUIRE(iter->run() == 7);
      REQUIRE(iter->lumi() == 1);
      ++iter;
      REQUIRE(iter->event() == 3);
      ++iter;
      REQUIRE(iter->event() == 4);
...
};  // TestIndexIntoFile5Fixture

TEST_CASE("IndexIntoFile5", "[indexIntoFile5_t]") {
  TestIndexIntoFile5 fixture;
  fixture.setUp();

  SECTION("testDuplicateCheckerFunctions") { fixture.testDuplicateCheckerFunctions(); }
}
