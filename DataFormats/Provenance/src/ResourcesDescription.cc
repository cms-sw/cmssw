...
namespace edm {
  ResourcesDescription::ResourcesDescription(std::string_view serialized) {
    ...
    if (not serialized.empty()) {
      ...
      if (ret == 0) {
        throw Exception(errors::EventCorruption) << "Failed to deserialize ResourcesDescription string format";
      }
    }
  }

  std::string ResourcesDescription::serialize() const {
