...
namespace edm {
  void RunAuxiliary::write(std::ostream& os) const {
    os << "Process History ID = " << processHistoryID_ << std::endl;
    os << id_ << std::endl;
  }

  void RunAuxiliary::mergeAuxiliary(RunAuxiliary const& newAux) {
    assert(id_ == newAux.id_);
    mergeNewTimestampsIntoThis_(newAux);
  }

  void RunAuxiliary::mergeNewTimestampsIntoThis_(RunAuxiliary const& newAux) {
