...
private:
    ...
    boost::uint16_t m_boardId;        // 16: board identifier
    //
    ...
    //
    boost::uint16_t m_bxNr;           // 12: bunch cross number of the actual bx
    boost::uint32_t m_eventNr;        // 24: event number since last L1 reset generated in PSB
    //
    boost::uint16_t m_aData[NumberAData];    // 16: A_Data_ChX
    //
    boost::uint16_t m_bData[NumberBData];    // 16: B_Data_ChX
    //
    boost::uint16_t m_localBxNr;      // 12: local bunch cross number of the actual bx
