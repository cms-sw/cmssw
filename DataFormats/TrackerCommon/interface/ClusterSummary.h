...
//
/**\class ClusterSummary ClusterSummary.cc
 msegala/ClusterSummary/src/ClusterSummary.cc
...
ClusterSummary provides summary inforation for a number of cluster dependent
variables. All the variables are stored within variables_ The modules selected
are stored within modules_ The number of variables for each module is stored
within iterator_
...
public:
  ...
  // Enum which describes the ordering of the summary variables inside vector
  // variables_
  ...
  // These functions are broken into two categories. The standard versions take
  // the enums as input and find the locations in the vector. The ones labeled
  // "byIndex" take the vector location as input
public:
  ...
  void addClusChargeByIndex(const int mod, const float val) {
    clusCharge.at(mod) += val;
  }

  void addNClus(const CMSTracker mod, const int val) {
    nClus.at(getModuleLocation(mod)) += val;
  }
  void addClusSize(const CMSTracker mod, const int val) {
    clusSize.at(getModuleLocation(mod)) += val;
  }
  void addClusCharge(const CMSTracker mod, const float val) {
    clusCharge.at(getModuleLocation(mod)) += val;
  }

  const std::vector<int> &getModules() const { return modules; }
  // Return the location of desired module within modules_. If warn is set to
  // true, a warnign will be outputed in case no module was found
