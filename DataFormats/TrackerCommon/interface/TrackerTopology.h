...
public:
  ...
  public:
    ...
    bool operator()(DetId i1, DetId i2) const {
      if (i1.det() == i2.det() && i1.subdetId() == i2.subdetId() &&
  ...
  unsigned int tobSide(const DetId &id) const {
    return ((id.rawId() >> tobVals_.rod_fw_bwStartBit_) &
            tobVals_.rod_fw_bwMask_);
  }
  ...
  unsigned int tibSide(const DetId &id) const {
    return ((id.rawId() >> tibVals_.str_fw_bwStartBit_) &
            tibVals_.str_fw_bwMask_);
  }
  ...
  unsigned int tecOrder(const DetId &id) const {
    return ((id.rawId() >> tecVals_.petal_fw_bwStartBit_) &
            tecVals_.petal_fw_bwMask_);
  }
  unsigned int tibOrder(const DetId &id) const {
    return ((id.rawId() >> tibVals_.str_int_extStartBit_) &
            tibVals_.str_int_extMask_);
  }
  unsigned int tidOrder(const DetId &id) const {
    return ((id.rawId() >> tidVals_.module_fw_bwStartBit_) &
            tidVals_.module_fw_bwMask_);
  }
  ...
  bool tobIsDoubleSide(const DetId &id) const {
    return tobGlued(id) == 0 && (tobLayer(id) == 1 || tobLayer(id) == 2);
  }
  bool tecIsDoubleSide(const DetId &id) const {
    return tecGlued(id) == 0 &&
           (tecRing(id) == 1 || tecRing(id) == 2 || tecRing(id) == 5);
  }
  bool tibIsDoubleSide(const DetId &id) const {
    return tibGlued(id) == 0 && (tibLayer(id) == 1 || tibLayer(id) == 2);
  }
  bool tidIsDoubleSide(const DetId &id) const {
    return tidGlued(id) == 0 && (tidRing(id) == 1 || tidRing(id) == 2);
  }
  ...
  bool tobIsRPhi(const DetId &id) const {
    return tobRPhi(id) != 0 && !tobIsDoubleSide(id);
  }
  bool tecIsRPhi(const DetId &id) const {
    return tecRPhi(id) != 0 && !tecIsDoubleSide(id);
  }
  bool tibIsRPhi(const DetId &id) const {
    return tibRPhi(id) != 0 && !tibIsDoubleSide(id);
  }
  bool tidIsRPhi(const DetId &id) const {
    return tidRPhi(id) != 0 && !tidIsDoubleSide(id);
  }

  // phase0 stereo
  uint32_t tobStereo(const DetId &id) const {
    return (((id.rawId() >> tobVals_.sterStartBit_) & tobVals_.sterMask_) == 1)
               ? 1
               : 0;
  }

  uint32_t tibStereo(const DetId &id) const {
    return (((id.rawId() >> tibVals_.sterStartBit_) & tibVals_.sterMask_) == 1)
               ? 1
               : 0;
  }

  uint32_t tidStereo(const DetId &id) const {
    return (((id.rawId() >> tidVals_.sterStartBit_) & tidVals_.sterMask_) == 1)
               ? 1
               : 0;
  }

  uint32_t tecStereo(const DetId &id) const {
    return (((id.rawId() >> tecVals_.sterStartBit_) & tecVals_.sterMask_) == 1)
               ? 1
               : 0;
  }
  ...
  uint32_t tibGlued(const DetId &id) const {
    uint32_t testId =
        (id.rawId() >> tibVals_.sterStartBit_) & tibVals_.sterMask_;
    return (testId == 0) ? 0 : (id.rawId() - testId);
  }

  uint32_t tecGlued(const DetId &id) const {
    uint32_t testId =
        (id.rawId() >> tecVals_.sterStartBit_) & tecVals_.sterMask_;
    return (testId == 0) ? 0 : (id.rawId() - testId);
  }

  uint32_t tobGlued(const DetId &id) const {
    uint32_t testId =
        (id.rawId() >> tobVals_.sterStartBit_) & tobVals_.sterMask_;
    return (testId == 0) ? 0 : (id.rawId() - testId);
  }
  ...
  DetId tibPartnerDetId(const DetId &id) const {
    ...
    } else if (((id.rawId() >> tibVals_.sterStartBit_) & tibVals_.sterMask_) ==
               2) {
      return DetId(id.rawId() - 1);
    } else {
      return DetId();
  ...
  DetId tobPartnerDetId(const DetId &id) const {
    if (((id.rawId() >> tobVals_.sterStartBit_) & tobVals_.sterMask_) == 1) {
      return DetId(id.rawId() + 1);
    } else if (((id.rawId() >> tobVals_.sterStartBit_) & tobVals_.sterMask_) ==
               2) {
      return DetId(id.rawId() - 1);
    } else {
      return DetId();
  ...
  DetId tidPartnerDetId(const DetId &id) const {
    if (((id.rawId() >> tidVals_.sterStartBit_) & tidVals_.sterMask_) == 1) {
      return DetId(id.rawId() + 1);
    } else if (((id.rawId() >> tidVals_.sterStartBit_) & tidVals_.sterMask_) ==
               2) {
      return DetId(id.rawId() - 1);
    } else {
      return DetId();
  ...
  uint32_t tecPartnerDetId(const DetId &id) const {
    if (((id.rawId() >> tecVals_.sterStartBit_) & tecVals_.sterMask_) == 1) {
      return DetId(id.rawId() + 1);
    } else if (((id.rawId() >> tecVals_.sterStartBit_) & tecVals_.sterMask_) ==
               2) {
      return DetId(id.rawId() - 1);
    } else {
      return DetId();
  ...
  DetId pxfDetId(uint32_t side, uint32_t disk, uint32_t blade, uint32_t panel,
  ...
  DetId tecDetId(uint32_t side, uint32_t wheel, uint32_t petal_fw_bw,
                 uint32_t petal, uint32_t ring, uint32_t module,
                 uint32_t ster) const {
    ...
    rawid |= (side & tecVals_.sideMask_) << tecVals_.sideStartBit_ |
             ...
             (petal_fw_bw & tecVals_.petal_fw_bwMask_)
                 << tecVals_.petal_fw_bwStartBit_ |
  ...
  DetId tibDetId(uint32_t layer, uint32_t str_fw_bw, uint32_t str_int_ext,
                 uint32_t str, uint32_t module, uint32_t ster) const {
    DetId id = SiStripDetId(DetId::Tracker, StripSubdetector::TIB);
    uint32_t rawid = id.rawId();
    rawid |= (layer & tibVals_.layerMask_) << tibVals_.layerStartBit_ |
             (str_fw_bw & tibVals_.str_fw_bwMask_)
                 << tibVals_.str_fw_bwStartBit_ |
             (str_int_ext & tibVals_.str_int_extMask_)
                 << tibVals_.str_int_extStartBit_ |
  ...
  DetId tidDetId(uint32_t side, uint32_t wheel, uint32_t ring,
                 uint32_t module_fw_bw, uint32_t module, uint32_t ster) const {
    ...
    rawid |= (side & tidVals_.sideMask_) << tidVals_.sideStartBit_ |
             ...
             (module_fw_bw & tidVals_.module_fw_bwMask_)
  ...
  DetId tobDetId(uint32_t layer, uint32_t rod_fw_bw, uint32_t rod,
                 uint32_t module, uint32_t ster) const {
    DetId id = SiStripDetId(DetId::Tracker, StripSubdetector::TOB);
    uint32_t rawid = id.rawId();
    rawid |= (layer & tobVals_.layerMask_) << tobVals_.layerStartBit_ |
             (rod_fw_bw & tobVals_.rod_fw_bwMask_)
                 << tobVals_.rod_fw_bwStartBit_ |
  ...
  std::pair<DetId, SameLayerComparator>
  ...
  pxfDetIdDiskComparator(uint32_t side, uint32_t disk) const {
    return std::make_pair(pxfDetId(side, disk, 1, 1, 1),
                          SameLayerComparator(this));
  }

  std::pair<DetId, SameLayerComparator>
  tecDetIdWheelComparator(uint32_t side, uint32_t wheel) const {
    return std::make_pair(tecDetId(side, wheel, 1, 1, 1, 1, 1),
                          SameLayerComparator(this));
  }

  std::pair<DetId, SameLayerComparator>
  tibDetIdLayerComparator(uint32_t layer) const {
    return std::make_pair(tibDetId(layer, 1, 1, 1, 1, 1),
                          SameLayerComparator(this));
  }

  std::pair<DetId, SameLayerComparator>
  tidDetIdWheelComparator(uint32_t side, uint32_t wheel) const {
    return std::make_pair(tidDetId(side, wheel, 1, 1, 1, 1),
                          SameLayerComparator(this));
  }

  std::pair<DetId, SameLayerComparator>
  tobDetIdLayerComparator(uint32_t layer) const {
    return std::make_pair(tobDetId(layer, 1, 1, 1, 1),
                          SameLayerComparator(this));
  }
  ...
  unsigned int getField(const DetId &id, DetIdFields idx) const {
    return ((id.rawId() >> bits_per_field[idx].startBit) &
            bits_per_field[idx].mask);
  }
