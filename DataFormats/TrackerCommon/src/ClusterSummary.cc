...
const std::vector<std::vector<std::string>> ClusterSummary::subDetSelections{
    {"0x1e000000-0x1A000000", "0x1e000000-0x16000000", "0x1e000000-0x18000000",
...
const std::vector<std::string> ClusterSummary::variableNames{
    "NCLUSTERS", "CLUSTERSIZE", "CLUSTERCHARGE"};

ClusterSummary::ClusterSummary() : ClusterSummary(NVALIDENUMS) {}

ClusterSummary::ClusterSummary(const int nSelections)
    : modules(nSelections), nClus(nSelections), clusSize(nSelections),
      clusCharge(nSelections) {
...
ClusterSummary::ClusterSummary(const ClusterSummary &src)
    : modules(src.getModules()), nClus(src.getNClusVector()),
      clusSize(src.getClusSizeVector()), clusCharge(src.getClusChargeVector()) {
}
