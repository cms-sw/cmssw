...
PixelBarrelName::PixelBarrelName(const DetId &id, const TrackerTopology *tt,
                                 bool phase)
    : PixelModuleName(true), thePart(mO), theLayer(0), theModule(0),
      theLadder(0), phase1(phase) {
...
PixelBarrelName::PixelBarrelName(std::string name, bool phase)
  ...
  if ((name.substr(0, 5) != "BPix_") || (name.find("_B") == string::npos) ||
  ...
  // find shell
  string shellString =
      name.substr(name.find("_B") + 2, name.find("_SEC") - name.find("_B") - 2);
  if (shellString == "mO")
    thePart = mO;
    ...
    edm::LogError("BadNameString|SiPixel")
        << "Unable to determine shell in "
           "PixelBarrelName::PixelBarrelName(std::string): "
        << name;
  }

  // find the layer
  string layerString = name.substr(name.find("_LYR") + 4,
                                   name.find("_LDR") - name.find("_LYR") - 4);
  if (layerString == "1")
    theLayer = 1;
    ...
    edm::LogError("BadNameString|SiPixel")
        << "Unable to determine layer in "
           "PixelBarrelName::PixelBarrelName(std::string): "
        << name;
  }

  // find the ladder
  string ladderString = name.substr(name.find("_LDR") + 4,
                                    name.find("_MOD") - name.find("_LDR") - 4);

  if (phase1) { // phase 1 ladders
    // do we want to kee the "F" for phase1?
    if (ladderString.substr(ladderString.size() - 1, 1) == "F") {
      int ladderNum =
          atoi(ladderString.substr(0, ladderString.size() - 1).c_str());
      if (theLayer == 1 && ladderNum >= 1 && ladderNum <= 6)
        ...
        edm::LogError("BadNameString|SiPixel")
            << "Unable to determine ladder in "
               "PixelBarrelNameUpgrade::PixelBarrelName(std::string): "
            << name;
      }
    } // full ladders
    else {
  ...
  // find the module
  string moduleString =
      name.substr(name.find("_MOD") + 4, name.size() - name.find("_MOD") - 4);
  if (moduleString == "1")
    theModule = 1;
...
int PixelBarrelName::sectorName() const {
  ...
  if (theLayer == 1) {
    ...
    if (phase1) { // phase 1
      // Layer 1 for phase1 is special, modules on a ladder are shared in
      // different sectors Ladder   Module    Sector
      //   1      2,3,4       1
      //   1        1         2
      //   2       3,4        2
      //   2       1,2        3
...
bool PixelBarrelName::operator==(const PixelModuleName &o) const {
  if (o.isBarrel()) {
    ...
    return (other && thePart == other->thePart && theLayer == other->theLayer &&
            theModule == other->theModule && theLadder == other->theLadder);
  } else
    return false;
}
...
string PixelBarrelName::name() const {
  ...
  stm << "BPix_B" << thePart << "_SEC" << sectorName() << "_LYR" << theLayer
      << "_LDR" << theLadder;
  if (isHalfModule())
    stm << "H";
  else
...
DetId PixelBarrelName::getDetId(const TrackerTopology *tt) {
  ...
  // translate the ladder number from the naming convention to the cmssw
  // convention
  bool outer = false;
  Shell shell = thePart;
  int ind = 0;
