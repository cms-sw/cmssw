...
PixelEndcapNameUpgrade::PixelEndcapNameUpgrade(const DetId &id)
  ...
  */
  //    iasonas2-with this we determine inner-outer x-position (x>0-x<0), NOT
  //    ring m/pI:1-6,18-31,49-56. m/pO:7-17,32-48.
...
PixelEndcapNameUpgrade::PixelEndcapNameUpgrade(std::string name)
    : PixelModuleName(false), thePart(mO), theDisk(0), theBlade(0),
      thePannel(0), thePlaquette(0) {
  ...
  if ((name.substr(0, 5) != "FPix_") || (name.find("_B") == string::npos) ||
    ...
    edm::LogError("BadNameString|SiPixel")
        << "Bad name string in "
           "PixelEndcapNameUpgrade::PixelEndcapNameUpgrade(std::string): "
        << name;
    return;
  }
  ...
  string hcString =
      name.substr(name.find("_B") + 2, name.find("_D") - name.find("_B") - 2);
  if (hcString == "mO")
    thePart = mO;
  else if (hcString == "mI")
    thePart = mI;
    ...
    edm::LogError("BadNameString|SiPixel")
        << "Unable to determine half cylinder in "
           "PixelEndcapNameUpgrade::PixelEndcapNameUpgrade(std::string): "
        << name;
  }
  // get the disk
  string diskString =
      name.substr(name.find("_D") + 2, name.find("_BLD") - name.find("_D") - 2);
  if (diskString == "1")
    theDisk = 1;
    ...
    edm::LogError("BadNameString|SiPixel")
        << "Unable to determine disk number in "
           "PixelEndcapNameUpgrade::PixelEndcapNameUpgrade(std::string): "
        << name;
  }
  // get the blade
  string bladeString = name.substr(name.find("_BLD") + 4,
                                   name.find("_PNL") - name.find("_BLD") - 4);
  ...
  if (bladeString == "1")
    ...
    edm::LogError("BadNameString|SiPixel")
        << "Unable to determine blade number in "
           "PixelEndcapNameUpgrade::PixelEndcapNameUpgrade(std::string): "
        << name;
  }
  // find the panel
  string panelString = name.substr(name.find("_PNL") + 4,
                                   name.find("_PLQ") - name.find("_PNL") - 4);
  if (panelString == "1")
    thePannel = 1;
    ...
    edm::LogError("BadNameString|SiPixel")
        << "Unable to determine panel number in "
           "PixelEndcapNameUpgrade::PixelEndcapNameUpgrade(std::string): "
        << name;
  }
  // find the plaquette
  string plaquetteString =
      name.substr(name.find("_PLQ") + 4, name.size() - name.find("_PLQ") - 4);
  if (plaquetteString == "1")
    thePlaquette = 1;
  else {
    edm::LogError("BadNameString|SiPixel")
        << "Unable to determine plaquette number in "
           "PixelEndcapNameUpgrade::PixelEndcapNameUpgrade(std::string): "
        << name;
  }
...
bool PixelEndcapNameUpgrade::operator==(const PixelModuleName &o) const {
  if (!o.isBarrel()) {
    const PixelEndcapNameUpgrade *other =
        dynamic_cast<const PixelEndcapNameUpgrade *>(&o);
    return (other && thePart == other->thePart && theDisk == other->theDisk &&
            theBlade == other->theBlade && thePannel == other->thePannel &&
...
string PixelEndcapNameUpgrade::name() const {
  ...
  stm << "FPix_B" << thePart << "_D" << theDisk << "_BLD" << theBlade << "_PNL"
      << thePannel << "_PLQ" << thePlaquette;
  return stm.str();
}
