...
PixelBarrelNameUpgrade::PixelBarrelNameUpgrade(const DetId &id)
  ...
  if (oldModule < 0 && oldLadder < 0)
    ...
    thePart = pI;
  // std::cout << "(oldModule, oldLadder)============(" << oldModule << ", "<<
  // oldLadder<<" )" << std::endl;
...
PixelModuleName::ModuleType PixelBarrelNameUpgrade::moduleType() const {
  return isHalfModule() ? PixelBarrelNameUpgrade::v1x8
                        : PixelBarrelNameUpgrade::v2x8;
}

bool PixelBarrelNameUpgrade::operator==(const PixelModuleName &o) const {
  if (o.isBarrel()) {
    const PixelBarrelNameUpgrade *other =
        dynamic_cast<const PixelBarrelNameUpgrade *>(&o);
