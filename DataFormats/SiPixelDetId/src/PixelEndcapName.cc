...
PixelEndcapName::PixelEndcapName(const DetId &id, const TrackerTopology *tt,
    ...
    : PixelModuleName(false), thePart(mO), theDisk(0), theBlade(0),
      thePannel(0), thePlaquette(0), phase1(phase) {
  ...
  bool outer =
      false; // outer means with respect to the LHC ring (x - axis) Bm(p)I/O
  ...
  if (phase1) { // phase 1
    ...
    if (pilot_blade && theDisk == 3) { // do only for disk 3
      ...
      if (tmpBlade >= 1 && tmpBlade <= 4) {
        // convert the sequential counting of pilot blades to std. cmssw
        // convention
        if (tmpBlade < 3)
          tmpBlade += 3;
        else
    ...
    thePlaquette = tt->pxfModule(
        id); // ring number is coded as plaqutte (plaqs are unused)
...
PixelEndcapName::PixelEndcapName(std::string name, bool phase)
  ...
  if (hcString == "mO")
    ...
    edm::LogError("BadNameString|SiPixel")
        << "Unable to determine half cylinder in "
           "PixelEndcapName::PixelEndcapName(std::string): "
        << name;
  }
  ...
  string panelString;
  if (phase1)
    panelString = name.substr(name.find("_PNL") + 4,
                              name.find("_RNG") - name.find("_PNL") - 4);
  else
    panelString = name.substr(name.find("_PNL") + 4,
  ...
  if (phase1) { // phase1
    ...
    string ringString =
        name.substr(name.find("_RNG") + 4, name.size() - name.find("_RNG") - 4);
    if (ringString == "1")
      ...
      edm::LogError("BadNameString|SiPixel")
          << "Unable to determine ring number in "
             "PixelEndcapName::PixelEndcapName(std::string): "
          << name;
    }
...
string PixelEndcapName::name() const {
  ...
  if (phase1)
    stm << "FPix_B" << thePart << "_D" << theDisk << "_BLD" << theBlade
        << "_PNL" << thePannel << "_RNG" << thePlaquette;
  else
    stm << "FPix_B" << thePart << "_D" << theDisk << "_BLD" << theBlade
        << "_PNL" << thePannel << "_PLQ" << thePlaquette;
