...
TEST_CASE("SoAConstMultiViewCUDA") {
  ...
  CUDA_CHECK(cudaMallocHost(&h_buf1, bufferSize1));
  ...
  CUDA_CHECK(cudaMallocHost(&h_buf2, bufferSize2));
  ...
  CUDA_CHECK(cudaMalloc(&d_buf1, bufferSize1));
  SoA d_soahdLayout1(d_buf1, sizes1);
  SoAConstView d_Constview(d_soahdLayout1);

  std::byte* d_buf2 = nullptr;
  CUDA_CHECK(cudaMalloc(&d_buf2, bufferSize2));
  SoA d_soahdLayout2(d_buf2, sizes2);
  SoAConstView d_Constview2(d_soahdLayout2);

  CUDA_CHECK(cudaMemcpy(d_buf1, h_buf1, bufferSize1, cudaMemcpyHostToDevice));
  CUDA_CHECK(cudaMemcpy(d_buf2, h_buf2, bufferSize2, cudaMemcpyHostToDevice));
  ...
  CUDA_CHECK(cudaMalloc(&d_outputPosition, outputSizePosition));
  ...
  CUDA_CHECK(cudaMemcpy(h_outputPosition.data(), d_outputPosition, outputSizePosition, cudaMemcpyDeviceToHost));
  ...
  CUDA_CHECK(cudaMalloc(&d_outputPCA, outputSizePCA));
  checkPCAMultiView<<<(pcaMultiView.size() + 255) / 256, 256>>>(pcaMultiView, d_outputPCA);
  CUDA_CHECK(cudaDeviceSynchronize());
  std::vector<float> h_outputPCA(pcaMultiView.size());
  CUDA_CHECK(cudaMemcpy(h_outputPCA.data(), d_outputPCA, outputSizePCA, cudaMemcpyDeviceToHost));
  ...
  CUDA_CHECK(cudaFreeHost(h_buf1));
  CUDA_CHECK(cudaFreeHost(h_buf2));
  CUDA_CHECK(cudaFree(d_buf1));
  CUDA_CHECK(cudaFree(d_buf2));
  CUDA_CHECK(cudaFree(d_outputPosition));
}
