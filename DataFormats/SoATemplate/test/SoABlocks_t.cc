...
TEST_CASE("SoABlocks") {
  ...
  // TODO: Implement soaToStreamInternal() for SoABlocks
  // blocks.position().soaToStreamInternal(std::cout);
  // blocks.pca().soaToStreamInternal(std::cout);
  // blocks.scalars().soaToStreamInternal(std::cout);
  ...
  SECTION("SoABlocks View") {
    ...
    for (int i = 0; i < blocksView.pca().metadata().size(); ++i) {
      ...
      REQUIRE(pca.eigenvector_1() == 0.0f);
      REQUIRE(pca.eigenvector_2() == 0.0f);
      REQUIRE(pca.eigenvector_3() == 1.0f);
  ...
  SECTION("Range checking ConstView") {
    ...
    // Check for under-and overflow in the element accessors
    REQUIRE_THROWS_AS(blocksConstView.pca().eigenvector_1(underflow), std::out_of_range);
    REQUIRE_THROWS_AS(blocksConstView.pca().eigenvector_1(overflow), std::out_of_range);
  }
}
