...
// a SoAGenericView, but this behavious is not tested here.
...
TEST_CASE("SoAGenericBlocks") {
  ...
  for (size_t i = 0; i < elemsPCA; i++) {
    pcaView[i].eigenvector_1() = (i * 1.0f) / time;
    pcaView[i].eigenvector_2() = (i * 2.0f) / time;
    pcaView[i].eigenvector_3() = (i * 3.0f) / time;
  ...
  SECTION("GenericBlocks View") {
    ...
    for (int i = 0; i < genericBlocksView.pca().metadata().size(); ++i) {
      ...
      REQUIRE(pca.eigenvector_1() == static_cast<float>(i) / time);
      REQUIRE(pca.eigenvector_2() == static_cast<float>(i * 2.0f) / time);
      REQUIRE(pca.eigenvector_3() == static_cast<float>(i * 3.0f) / time);
      REQUIRE(pca.candidateDirection()(0) == static_cast<double>(i) / time);
      REQUIRE(pca.candidateDirection()(1) == static_cast<double>(i * 2.0) / time);
      REQUIRE(pca.candidateDirection()(2) == static_cast<double>(i * 3.0) / time);
    }
  }
  ...
  SECTION("Deep copy the GeenericBlocks View") {
    ...
    REQUIRE(reinterpret_cast<std::byte *>(genericSoABlocksView.position().metadata().addressOf_detectorType()) +
            ...
            reinterpret_cast<std::byte *>(genericSoABlocksView.pca().metadata().addressOf_eigenvector_1()));
    REQUIRE(reinterpret_cast<std::byte *>(genericSoABlocksView.pca().metadata().addressOf_eigenvector_1()) +
                cms::soa::alignSize(elemsPCA * sizeof(float), SoAGenericBlocks::alignment) ==
            reinterpret_cast<std::byte *>(genericSoABlocksView.pca().metadata().addressOf_eigenvector_2()));
    REQUIRE(reinterpret_cast<std::byte *>(genericSoABlocksView.pca().metadata().addressOf_eigenvector_2()) +
                cms::soa::alignSize(elemsPCA * sizeof(float), SoAGenericBlocks::alignment) ==
            reinterpret_cast<std::byte *>(genericSoABlocksView.pca().metadata().addressOf_eigenvector_3()));
    REQUIRE(reinterpret_cast<std::byte *>(genericSoABlocksView.pca().metadata().addressOf_eigenvector_3()) +
                cms::soa::alignSize(elemsPCA * sizeof(float), SoAGenericBlocks::alignment) ==
    ...
    for (size_t i = 0; i < elemsPCA; i++) {
      REQUIRE(genericSoABlocksView.pca()[i].eigenvector_1() == pcaConstView[i].eigenvector_1());
      REQUIRE(genericSoABlocksView.pca()[i].eigenvector_2() == pcaConstView[i].eigenvector_2());
      REQUIRE(genericSoABlocksView.pca()[i].eigenvector_3() == pcaConstView[i].eigenvector_3());
