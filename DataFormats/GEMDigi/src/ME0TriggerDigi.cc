...
bool ME0TriggerDigi::operator==(const ME0TriggerDigi &rhs) const {
  return ((chamberid_ == rhs.chamberid_) && (quality_ == rhs.quality_) &&
	  (phiposition_ == rhs.phiposition_)   && (partition_ == rhs.partition_)   && 
	  (deltaphi_ == rhs.deltaphi_) && 
	  (bend_ == rhs.bend_)     && (bx_ == rhs.bx_) );
}

std::ostream & operator<<(std::ostream & o,
			  const ME0TriggerDigi& digi) {
  return o << "ME0 chamber id #" << digi.getChamberid()
           << " Partition = "     << digi.getPartition()
           << ": Quality = "  << digi.getQuality()
           << " Phiposition = "     << digi.getPhiposition()
           << " Strip = "     << digi.getStrip()
	   << " deltaPhi = "   << digi.getDeltaphi()
