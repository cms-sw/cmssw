  ...
  for (int re = GEMDetId::minRegionId; re <= GEMDetId::maxRegionId; ++re) {
	...
	for (int st = GEMDetId::minStationId; st <= GEMDetId::maxStationId; ++st) {
	  for (int la = GEMDetId::minLayerId; la <= GEMDetId::maxLayerId; ++la) {
		...
		if ((id.region() != re) || (id.ring() != ri) ||
		    (id.station() != st) || (id.layer() != la) ||
		    (id.chamber() != ch) || (id.roll() != ro)) {
		  std::cout << id << " should have been (" << re << ", " << ri
			    << ", " << st << ", " << la << ", " << ch << ", "
			    << ro << ") " << std::hex << id.rawId() << std::dec
			    << " ***** ERROR *****" << std::endl;
		}
	      }
	    }
...
void testFail() {
  ...
  GEMDetId id1(raw1);
  GEMDetId id2(raw2);
  std::cout << " ID0: " << std::hex << id0.rawId() << std::dec << ":" 
	    << id0.rawId() << " " << id0 << "\n ID1: " << std::hex 
	    << id1.rawId() << ":" << raw1 << std::dec << ":" << raw1 << " "
	    << id1 << "\n ID2: " << std::hex << id2.rawId() << ":" 
	    << raw2 << std::dec << ":" << raw2 << " " << id2 << std::endl;
}
