// Last commit: $Id: test_SiStripEnumsAndStrings.cc,v 1.3 2007/03/21 08:23:00 bainbrid Exp $
...
void test_SiStripEnumsAndStrings::beginJob( const edm::EventSetup& setup ) {
  ...
  std::vector<sistrip::ApvReadoutMode> apv_modes; 
  {
	...
	ss << "[test_SiStripEnumsAndStrings::" << __func__ << "]" 
	   ...
	   << "  str: " << str;
	LogTrace(mlDqmCommon_) << ss.str();
	if ( out != sistrip::UNKNOWN_APV_READOUT_MODE ) { apv_modes.push_back(in); }
      }
    }
  }
  ...
  std::vector<sistrip::FedReadoutMode> fed_modes; 
  {
	...
	ss << "[test_SiStripEnumsAndStrings::" << __func__ << "]" 
	   ...
	   << "  str: " << str;
	LogTrace(mlDqmCommon_) << ss.str();
	if ( out != sistrip::UNKNOWN_FED_READOUT_MODE ) { fed_modes.push_back(in); }
      }
    }
  }
  ...
  // sistrip::Monitorable
  {
    ...
    for ( uint32_t cntr = 0; cntr <= sistrip::invalid_; cntr++ ) {
      ...
      if ( out != sistrip::UNKNOWN_MONITORABLE ||
  ...
  // sistrip::Presentation
  {
    ...
    for ( uint32_t cntr = 0; cntr <= sistrip::invalid_; cntr++ ) {
      ...
      if ( out != sistrip::UNKNOWN_PRESENTATION ||
