...
namespace reco {
  ...
  public:
    ...
    //  hit pattern
    HitPattern hitPattern() const { return hitPattern_; }
    /// position-momentum error matrix ( 6x6, degenerate )
    PosMomError posMomError() const;
    /// number of hits found 
    unsigned short found() const { return hitPattern_.numberOfValidHits(); }
    /// number of hits lost
    unsigned short lost() const { return hitPattern_.numberOfLostHits(); }
    /// number of invalid hits
    unsigned short invalid() const { return hitPattern_.numberOfLostHits(); }
