...
namespace reco {
  ...
  IsolatedPixelTrackCandidate(const reco::TrackRef & tr, const l1t::JetRef & tauRef, double max, double sum): 
    RecoCandidate( 0, LorentzVector((tr.get()->px()),(tr.get())->py(),(tr.get())->pz(),(tr.get())->p()) ),
    ...
    IsolatedPixelTrackCandidate(const l1t::JetRef & tauRef, double enIn, double enOut, int nhitIn, int nhitOut):
    RecoCandidate( 0, LorentzVector(tauRef->px(),tauRef->py(),tauRef->pz(),tauRef->p()) ), 
    ...
    virtual l1t::JetRef l1ttau() const;
    void    setL1TTauJet( const l1t::JetRef & tauRef ) { l1ttauJet_ = tauRef; }
