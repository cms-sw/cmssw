...
// Catch2 tests for MemoryCopyTraits
//
// This test file defines various types, and specializes MemoryCopyTraits for them.
//
// The following tests are performed:
//
// - MemoryCopyTraits specializations for int and double are correct. In
// particular, their memory regions are correct.
// - MemoryCopyTraits specialization for std::vector<float> is correct. In
// particular, one can successfully initialize a vector from the properties of
// another.
// - A memcpy-able struct "S" can be copied correctly using its MemoryCopyTraits
// specialization, and the method ngt::MemoryCopyTraits<S>::finalize() works
// correctly.
// - It can be checked that there is no MemoryCopyTraits specialization for
// std::map.
// - A type "S2" whose specialization requires initialization but has no
// properties can be copied correctly through the MemoryCopyTraits interface.
// - Several bad MemoryCopyTraits specializations can be detected correctly at
// compile time.

// --------------------------------------------------------------------------
// Definitions of various types, and their MemoryCopyTraits specializations

// A trivially copyable type and its MemoryCopyTraits specialization
struct S {
  S() = default;
...
// MemoryCopyTraits specialization
struct S2 {
  S2() { vec.resize(size); }
...
template <>
struct ngt::MemoryCopyTraits<S2> {
...
// A bad MemoryCopyTraits specialization, that is missing regions()
struct S3 {
  std::vector<int> vec;
};

template <>
struct ngt::MemoryCopyTraits<S3> {
  static void initialize(S3& object) { object.vec.resize(10); }
};

// A bad MemoryCopyTraits specialization, with a non-valid initialize()
struct S4 {
  std::vector<int> data;
};

template <>
struct ngt::MemoryCopyTraits<S4> {
...
// A bad MemoryCopyTraits specialization, missing only the const regions
struct S5 {
  int value;
};

template <>
struct ngt::MemoryCopyTraits<S5> {
...
TEST_CASE("test MemoryCopyTraits", "[MemoryCopyTraits]") {
  SECTION("int") {
    REQUIRE(ngt::HasMemoryCopyTraits<int>);

    auto checkInt = [](int v) {
      // test non-const regions
      auto regions = ngt::MemoryCopyTraits<int>::regions(v);
      ...
      // test const regions
      const int const_v = v;
      auto const_regions = ngt::MemoryCopyTraits<int>::regions(const_v);
  ...
  SECTION("double") {
    REQUIRE(ngt::HasMemoryCopyTraits<double>);

    auto checkDouble = [](double v) {
      // test non-const regions
      auto regions = ngt::MemoryCopyTraits<double>::regions(v);
      REQUIRE(regions.size() == 1);
      ...
      auto const_regions = ngt::MemoryCopyTraits<double>::regions(const_v);
      REQUIRE(const_regions.size() == 1);
  ...
  SECTION("std::vector<float>") {
    ...
    REQUIRE(ngt::HasMemoryCopyTraits<VectorType>);
    REQUIRE(ngt::HasTrivialCopyProperties<VectorType>);
    REQUIRE(ngt::HasValidInitialize<VectorType>);

    VectorType vec = {-5.5f, -3.3f, -1.1f, 4.4f, 8.8f};

    // Test properties
    auto vec_size = ngt::MemoryCopyTraits<VectorType>::properties(vec);
    REQUIRE(vec_size == 5);

    // Test initialize
    VectorType new_vec;
    ngt::MemoryCopyTraits<VectorType>::initialize(new_vec, vec_size);
    REQUIRE(new_vec.size() == 5);

    // Test non-const regions
    auto regions = ngt::MemoryCopyTraits<VectorType>::regions(vec);
    REQUIRE(regions.size() == 1);
    ...
    auto const_regions = ngt::MemoryCopyTraits<VectorType>::regions(const_vec);
    REQUIRE(const_regions.size() == 1);
  ...
  SECTION("memcpy-able struct") {
    ...
    REQUIRE(ngt::HasMemoryCopyTraits<S>);
    REQUIRE(ngt::HasTrivialCopyProperties<S>);
    REQUIRE(ngt::HasValidInitialize<S>);
    S s{test_msg, test_vec};

    // initialize a clone of s
    S s_clone;
    ngt::MemoryCopyTraits<S>::initialize(s_clone, ngt::MemoryCopyTraits<S>::properties(s));

    // Get memory regions
    auto const s_regions = ngt::MemoryCopyTraits<S>::regions(s);
    auto s_clone_regions = ngt::MemoryCopyTraits<S>::regions(s_clone);
    ...
    ngt::MemoryCopyTraits<S>::finalize(s_clone);
  ...
  SECTION("std::map<int,int>") {
    ...
    // there is no MemoryCopyTraits specialization for std::map (and there shouldn't be, since std::map is not trivially copyable)
    static_assert(not ngt::HasMemoryCopyTraits<MapType>);
  }

  SECTION("A valid specialization with initialize() but without properties()") {
    REQUIRE(ngt::HasMemoryCopyTraits<S2>);
    REQUIRE(not ngt::HasTrivialCopyProperties<S2>);
    REQUIRE(ngt::HasValidInitialize<S2>);
    ...
    ngt::MemoryCopyTraits<S2>::initialize(s2_clone);

    // get memory regions
    auto const s2_regions = ngt::MemoryCopyTraits<S2>::regions(s2);
    auto s2_clone_regions = ngt::MemoryCopyTraits<S2>::regions(s2_clone);
  ...
  SECTION("Invalid specializations") {
    ...
    static_assert(not ngt::HasMemoryCopyTraits<S3>);
    static_assert(not ngt::HasRegions<S3>);

    // S4: Has properties, but the initialize() declaration takes only one
    // argument.
    // static_assert(not ngt::HasMemoryCopyTraits<S4>); //FIXME
    static_assert(ngt::HasTrivialCopyProperties<S4>);
    // static_assert(not ngt::HasValidInitialize<S4>); //FIXME

    // S5: Missing just the const regions() overload
    static_assert(not ngt::HasMemoryCopyTraits<S5>);
    static_assert(not ngt::HasRegions<S5>);
  }
}
