...
public:
 KMTFTrack() : reco::LeafCandidate(-1, reco::LeafCandidate::PolarLorentzVector(0.1, 0.0, 0.0, 0.105)),
 ...
 KMTFTrack(const l1t::MuonStubRef& seed, int phi, int phiB):
  ...
  bool overlapTrack(const KMTFTrack& other) const {
	...
	if (s1->phiRegion() == s2->phiRegion() && s1->etaRegion() == s2->etaRegion() && s1->depthRegion() == s2->depthRegion() &&
	    s1->id() == s2->id())
	  return true;
      }
    }
  ...
  bool operator==(const KMTFTrack& t2) const {
    ...
    for (unsigned int i = 0; i < this->stubs().size(); ++i) {
      ...
      if (s1->phiRegion() != s2->phiRegion() || s1->etaRegion() != s2->etaRegion() || s1->depthRegion() != s2->depthRegion() ||
          s1->id() != s2->id() || s1->tfLayer() != s2->tfLayer())
        return false;
    }
    return true;
  }
