...
the ECAL End-Cap (EE)). 
The ECAL raw data format is shown in Figure~\ref{fig:ECAL}. 
...
\item {\bf RUN TYPE}: Field (Figure~\ref{}) encoded by the DCC supervisor software in local data taking mode and written into the DCC Run Type register.
\begin{description}
...
\item[-] {\bf CYCLE SETTINGS}: The cycle operation settings applied during the local run. The CYCLE SETTINGS encoding depends on the SEQUENCE and is shown in Figure 2. The POWER, FILTER, DELAY, VINJ, MGPA CONTENT and OFFSET fields are binary representations of configuration parameters for each SEQUENCE type. The RT\_WL field encodes the Laser/LED wavelength according to Table~\ref{tab:RT_WL}.
\end{description}
...
\end{tabular}
\caption{\sf DCC error description. }
\label{tab:DCCerror}
\end{table}
\end{center}
...
\end{tabular}
\caption{\sf RT\_HALF description.}
\label{tab:RT_HALF}
\end{table}
\end{center}
...
100 &	LED \\ \hline
\end{tabular}
\caption{\sf RT\_TYPE description.}
\label{tab:RT_TYPE}
\end{table}
\end{center}
...
\end{tabular}
\caption{\sf SEQUENCE description.}
\label{tab:SEQUENCE}
\end{table}
\end{center}
...
\end{tabular}
\caption{\sf  MGPA gain description.}
\label{tab:MGPAG}
\end{table}
\end{center}
...
\end{tabular}
\caption{\sf  MEM gain description.}
\label{tab:MGPAM}
\end{table}
\end{center}
...
\end{tabular}
\caption{\sf  The RT\_WL description.}
\label{tab:RT_WL}
\end{table}
\end{center}
...
\end{tabular}
\caption{\sf DCC Channel status description. If multiple errors are
...
\end{tabular}
\caption{\sf Trigger tower flags description.}
\label{tab:TTFlag}
\end{table}
\end{center}
...
1xx &	xx action has been forced by an erroneous condition or by configuration  \\ \hline
- &	TT flagged in forced readout mode (from TCC)  \\ \hline
- &	Data transmission errors (TCC and Algorithm Board(AB) link errors)  \\ \hline
\end{tabular}
\caption{\sf Selective Readout flags description.}
\label{tab:SRPFlag}
\end{table}
\end{center}
...
The CMS DAQ Header and trailer~\cite{TRIDAS} are respectively the first and last word of the ECAL event represented in Figure~\ref{fig:ECAL}.
...
1111 &	Error \\ \hline
\end{tabular}
\caption{\sf The TRIGGER TYPE description.}
...
\end{tabular}
\caption{\sf Error checks performed during parsing.}
