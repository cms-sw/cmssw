...
HGCalTriggerDetId::HGCalTriggerDetId(int subdet, int zp, int type, int layer,
  ...
  id_ |= (((cellU     & kHGCalCellUMask) << kHGCalCellUOffset) |
	  ...
	  ((subdet    & kHGCalSubdetMask)<< kHGCalSubdetOffset));
}

HGCalTriggerDetId::HGCalTriggerDetId(const DetId& gen) {
  if (!gen.null()) {
    if ((gen.det()!=HGCalTrigger) ||
	(((gen.subdetId()&kHGCalSubdetMask)!=HGCalEETrigger) && 
	 ((gen.subdetId()&kHGCalSubdetMask)!=HGCalHSiTrigger))) {
      throw cms::Exception("Invalid DetId") << "Cannot initialize HGCalTriggerDetId from " << std::hex << gen.rawId() << std::dec; 
    }  
  }
  id_ = gen.rawId();
}
...
HGCalTriggerDetId& HGCalTriggerDetId::operator=(const DetId& gen) {
  if (!gen.null()) {
	...
	(((gen.subdetId()&kHGCalSubdetMask)!=HGCalEETrigger) && 
	 ((gen.subdetId()&kHGCalSubdetMask)!=HGCalHSiTrigger))) {
      throw cms::Exception("Invalid DetId") << "Cannot assign HGCalTriggerDetId from " << std::hex << gen.rawId() << std::dec; 
    }  
  }
  id_ = gen.rawId();
  return (*this);
...
int HGCalTriggerDetId::triggerCellX() const {
  int nT = (type() == HGCSiliconDetId::HGCalFine) ? 
    HGCSiliconDetId::HGCalFineTrigger : HGCSiliconDetId::HGCalCoarseTrigger;
