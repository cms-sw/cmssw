...
{
    constexpr inline void setField(uint32_t* u, const unsigned mask,
                         const unsigned offset, const unsigned value)
        {*u &= ~(mask << offset); *u |= ((value & mask) << offset);}

    constexpr inline unsigned getField(const uint32_t u, const unsigned mask,
    ...
    constexpr inline bool getBit(const uint32_t u, const unsigned bitnum)
        {return u & (1U << bitnum);}
}
