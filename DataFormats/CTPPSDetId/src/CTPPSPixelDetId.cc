...
const uint32_t CTPPSPixelDetId::startPlaneBit = 16,
               CTPPSPixelDetId::maskPlane = 0x7, CTPPSPixelDetId::maxPlane = 5;

CTPPSPixelDetId::CTPPSPixelDetId(uint32_t id) : CTPPSDetId(id) {

  if (!check(id)) {
    throw cms::Exception("InvalidDetId")
        << "CTPPSPixelDetId ctor:"
        << " det: " << det() << " subdet: " << subdetId()
        << " is not a valid CTPPS Pixel id";
  }
}

CTPPSPixelDetId::CTPPSPixelDetId(unsigned int Arm, unsigned int Station,
                                 unsigned int RP, unsigned int Plane)
    : CTPPSDetId(sdTrackingPixel, Arm, Station, RP) {
  this->init(Arm, Station, RP, Plane);
}

void CTPPSPixelDetId::init(unsigned int Arm, unsigned int Station,
                           unsigned int RP, unsigned int Plane) {
...
std::ostream &operator<<(std::ostream &os, const CTPPSPixelDetId &id) {
  os << " Arm " << id.arm() << " Station " << id.station() << " RP " << id.rp()
     << " Plane " << id.plane();
