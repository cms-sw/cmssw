...
const uint32_t TotemRPDetId::startPlaneBit = 15, TotemRPDetId::maskPlane = 0xF,
               TotemRPDetId::maxPlane = 9, TotemRPDetId::lowMaskPlane = 0x7FFF;
const uint32_t TotemRPDetId::startChipBit = 13, TotemRPDetId::maskChip = 0x3,
               TotemRPDetId::maxChip = 3, TotemRPDetId::lowMaskChip = 0x1FFF;

const string TotemRPDetId::planeNames[] = {"01", "02", "03", "04", "05",
                                           "06", "07", "08", "09", "10"};
...
TotemRPDetId::TotemRPDetId(uint32_t Arm, uint32_t Station, uint32_t RomanPot,
                           uint32_t Plane, uint32_t Chip)
    : CTPPSDetId(sdTrackingStrip, Arm, Station, RomanPot) {
  if (Arm > maxArm || Station > maxStation || RomanPot > maxRP ||
      Plane > maxPlane || Chip > maxChip) {
    throw cms::Exception("InvalidDetId")
        << "TotemRPDetId ctor:"
        << " Invalid parameters:"
