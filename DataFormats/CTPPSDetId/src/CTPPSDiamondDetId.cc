...
const uint32_t CTPPSDiamondDetId::startPlaneBit = 17,
               CTPPSDiamondDetId::maskPlane = 0x3,
               CTPPSDiamondDetId::maxPlane = 3,
               CTPPSDiamondDetId::lowMaskPlane = 0x1FFFF;
const uint32_t CTPPSDiamondDetId::startDetBit = 12,
               CTPPSDiamondDetId::maskChannel = 0x1F,
               CTPPSDiamondDetId::maxChannel = 31,
               CTPPSDiamondDetId::lowMaskChannel = 0xFFF;
...
CTPPSDiamondDetId::CTPPSDiamondDetId(uint32_t Arm, uint32_t Station,
                                     uint32_t RomanPot, uint32_t Plane,
                                     uint32_t Channel)
    : CTPPSDetId(sdTimingDiamond, Arm, Station, RomanPot) {
  if (Arm > maxArm || Station > maxStation || RomanPot > maxRP ||
      Plane > maxPlane || Channel > maxChannel) {
    throw cms::Exception("InvalidDetId")
        << "CTPPSDiamondDetId ctor:"
        << " Invalid parameters:"
...
std::ostream &operator<<(std::ostream &os, const CTPPSDiamondDetId &id) {
  os << "arm=" << id.arm() << " station=" << id.station() << " rp=" << id.rp()
     << " plane=" << id.plane() << " Detector=" << id.channel();
