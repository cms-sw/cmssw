 ...
 *
 * This class is very similar to CTPPSDiamondDetId; however the detector is
 *completely separated, therefore it is useful to keep them separate and
 *independent.
 **/
...
public:
  ...
  TotemTimingDetId(uint32_t arm, uint32_t station, uint32_t romanPot = 0,
                   uint32_t plane = 0, uint32_t channel = 0);

  static constexpr uint32_t startPlaneBit = 17, maskPlane = 0x3, maxPlane = 3,
                            lowMaskPlane = 0x1FFFF;
  static constexpr uint32_t startDetBit = 12, maskChannel = 0x1F,
                            maxChannel = 31, lowMaskChannel = 0xFFF;
