 ...
 *   bits [19:21] => Roman Pot: 0 (near top), 1 (near bottom), 2 (near
 *horizontal), 3 (far horizontal), 4 (far top), 5 (far bottom) bits [0:18] =>
 *available for derived classes
 *
 * The ...Name() methods implement the official naming scheme based on EDMS
 *906715.
 **/
...
public:
  ...
  CTPPSDetId(uint32_t SubDet, uint32_t Arm, uint32_t Station,
             uint32_t RomanPot = 0);

  //-------------------- bit assignment --------------------

  static const uint32_t startArmBit, maskArm, maxArm, lowMaskArm;
  static const uint32_t startStationBit, maskStation, maxStation,
      lowMaskStation;
