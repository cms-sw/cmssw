...
unsigned char* RawDataBuffer::addSource(uint32_t sourceId, unsigned char const* buf, uint32_t size) {
  ...
  if ((uint64_t)usedSize_ + (uint64_t)size > maxSize)
      throw cms::Exception("RawDataBuffer") << "RawDataBuffer size overflow adding ID " << sourceId << ": " << usedSize_ << " + " << size << " > " << maxSize;
  ...
  if (phase1Range_) {
    ...
    if (size % 16)
      throw cms::Exception("RawDataBuffer") << "source " << sourceId << " data not multiple of 16 bytes (" << size << ")";
    if (size < 32)
...
const RawFragmentWrapper RawDataBuffer::fragmentData(uint32_t sourceId) const {
  ...
  std::span<const unsigned char> && byte_span{reinterpret_cast<const unsigned char*>(this->data_.data() + desc.first), desc.second};
  return RawFragmentWrapper(sourceId, std::move(byte_span));

}

const RawFragmentWrapper RawDataBuffer::fragmentData(std::map<uint32_t, std::pair<uint32_t, uint32_t>>::const_iterator const& it) const {
  if (it == map_.end()) {
    return RawFragmentWrapper();
  }
  const auto& desc = it->second;  
