...
public:
  ...
  TkRotation() : R11(1), R12(0), R13(0), R21(0), R22(1), R23(0), R31(0), R32(0), R33(1) {}
  ...
  TkRotation(const Basic3DVector<T>& axis, T phi)
      : R11(cos(phi)), R12(sin(phi)), R13(0), R21(-sin(phi)), R22(cos(phi)), R23(0), R31(0), R32(0), R33(1) {
  ...
  */
  ...
  TkRotation transposed() const { return TkRotation(R11, R21, R31, R12, R22, R32, R13, R23, R33); }

  Basic3DVector<T> operator*(const Basic3DVector<T>& v) const { return rotate(v); }
  ...
  Basic3DVector<T> multiplyInverse(const Basic3DVector<T>& v) const { return rotateBack(v); }
  ...
  Basic3DVector<T> operator*(const Basic2DVector<T>& v) const {
    return Basic3DVector<T>(R11 * v.x() + R12 * v.y(), R21 * v.x() + R22 * v.y(), R31 * v.x() + R32 * v.y());
  }
  Basic3DVector<T> multiplyInverse(const Basic2DVector<T>& v) const {
    return Basic3DVector<T>(R11 * v.x() + R21 * v.y(), R12 * v.x() + R22 * v.y(), R13 * v.x() + R23 * v.y());
  }
...
public:
  ...
  TkRotation2D transposed() const { return TkRotation2D(axis[0][0], axis[1][0], axis[0][1], axis[1][1]); }

  BasicVector rotate(const BasicVector& v) const { return transposed().rotateBack(v); }

  BasicVector rotateBack(const BasicVector& v) const { return v[0] * axis[0] + v[1] * axis[1]; }
