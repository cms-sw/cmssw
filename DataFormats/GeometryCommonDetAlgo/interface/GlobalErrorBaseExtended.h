...
template <class T, class ErrorWeightType> class GlobalErrorBaseExtended {
...
public:
   ...
   */
  GlobalErrorBaseExtended(T c11, T c21, T c31, T c41, T c51, T c61, T c22,
                          T c32, T c42, T c52, T c62, T c33, T c43, T c53,
                          T c63, T c44, T c54, T c64, T c55, T c65, T c66) {
  ...
  T rerr(const GlobalPoint &aPoint) const {
    ...
    if (r2 != 0)
      return std::max<T>(0, (1. / r2) *
                                (x2 * cxx() + 2. * xy * cyx() + y2 * cyy()));
    else
      return 0.5 * (cxx() + cyy());
  }
  ...
  T phierr(const GlobalPoint &aPoint) const {
    ...
    T xy = aPoint.x() * aPoint.y();
    if (r2 != 0)
      return std::max<T>(0, (1. / (r2 * r2)) *
                                (y2 * cxx() - 2. * xy * cyx() + x2 * cyy()));
    else
      return 0;
  }
