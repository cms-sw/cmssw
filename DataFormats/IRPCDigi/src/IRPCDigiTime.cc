...
float IRPCDigiTime::time(){
  return (timeLR()+timeHR())/2.;
}
...
float IRPCDigiTime::timeLR(){
  return TDC2Time(theDigi.bxLR(),theDigi.sbxLR(),theDigi.fineLR());
}

float IRPCDigiTime::timeHR(){
  return TDC2Time(theDigi.bxHR(),theDigi.sbxHR(),theDigi.fineHR());
}
