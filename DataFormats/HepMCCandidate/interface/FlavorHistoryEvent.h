...
namespace reco {
  ...
  namespace helpers {
    ...
    struct FlavorHistoryEventHelper{
      ...
      FlavorHistoryEventHelper( int iflavor,
	...
	if ( flavor       > right.flavor       ) return false;
	if ( flavorSource > right.flavorSource ) return false;
	if ( dR           > right.dR           ) return false;
	return true;
      }

      bool operator== ( FlavorHistoryEventHelper const & right) const {
	return flavor == right.flavor &&
	  flavorSource == right.flavorSource &&
	  dR == right.dR;
      }
	...
	char buff[1000];
	sprintf(buff, "Flavor = %2d, type = %2d, dR = %6f",
		helper.flavor, helper.flavorSource, helper.dR );
	out << buff << std::endl;
	return out;
      }
    };
...
public:
  ...
  unsigned int         nb() const { if ( isCached() ) return nb_; else return 0;}
  unsigned int         nc() const { if ( isCached() ) return nc_; else return 0;}

  // Accessor to maximum delta R between highest flavor constituents
  double               deltaR() const { if ( isCached() ) return dR_; else return -1.0; }
  unsigned int         highestFlavor() const { if ( isCached() ) return highestFlavor_; else return 0; }
  flavor_type          flavorSource() const { if ( isCached() ) return flavorSource_; else return FlavorHistory::FLAVOR_NULL; }
