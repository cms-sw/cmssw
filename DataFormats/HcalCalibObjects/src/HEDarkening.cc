...
HEDarkening::HEDarkening() {
  ...
  float _lumiscale[nEtaBins_HEDarkening][nScintLayers_HEDarkening] = {
  ...
  for(int j = 0; j < nEtaBins_HEDarkening; j++){
    for(int i = 0; i < nScintLayers_HEDarkening; i++){
...
float HEDarkening::degradation(float intlumi, int ieta, int lay) {
  ...
  if(ieta < 0 || ieta >= nEtaBins_HEDarkening) return 1.;
  
  //shift layer index by 1 to act as array index
  lay += 1;
  
  //if outside layer range, no darkening
  if(lay < 0 || lay >= nScintLayers_HEDarkening) return 1.;
