...
TEST_CASE("AoS testcase for PortableCollection", "[PortableCollectionAOS]") {
  ...
  for (auto const& device : cms::alpakatools::devices<Platform>()) {
    ...
    for (size_t i = 0; i < elems; i++) {
      REQUIRE(aosCollectionHostView[i].x() == Approx(static_cast<float>(i) + 0.0f * static_cast<float>(elems)));
      REQUIRE(aosCollectionHostView[i].y() == Approx(static_cast<float>(i) + 1.0f * static_cast<float>(elems)));
      REQUIRE(aosCollectionHostView[i].z() == Approx(static_cast<float>(i) + 2.0f * static_cast<float>(elems)));

      REQUIRE(aosCollectionHostView[i].exampleVector()(0) ==
              Approx(static_cast<float>(i) + 3.0f * static_cast<float>(elems)));
      REQUIRE(aosCollectionHostView[i].exampleVector()(1) ==
              Approx(static_cast<float>(i) + 4.0f * static_cast<float>(elems)));
      REQUIRE(aosCollectionHostView[i].exampleVector()(2) ==
              Approx(static_cast<float>(i) + 5.0f * static_cast<float>(elems)));
    }
    REQUIRE(aosCollectionHostView.s1() == 1);
    REQUIRE(aosCollectionHostView.s2() == Approx(2.0f));
    REQUIRE(aosCollectionHostView.s3() == Approx(3.0f));
    ...
    for (size_t i = 0; i < elems; i++) {
      REQUIRE(soaCollectionHost2View[i].x() == Approx(static_cast<float>(i) + 0.0f * static_cast<float>(elems)));
      REQUIRE(soaCollectionHost2View[i].y() == Approx(static_cast<float>(i) + 1.0f * static_cast<float>(elems)));
      REQUIRE(soaCollectionHost2View[i].z() == Approx(static_cast<float>(i) + 2.0f * static_cast<float>(elems)));

      REQUIRE(soaCollectionHost2View[i].exampleVector()(0) ==
              Approx(static_cast<float>(i) + 3.0f * static_cast<float>(elems)));
      REQUIRE(soaCollectionHost2View[i].exampleVector()(1) ==
              Approx(static_cast<float>(i) + 4.0f * static_cast<float>(elems)));
      REQUIRE(soaCollectionHost2View[i].exampleVector()(2) ==
              Approx(static_cast<float>(i) + 5.0f * static_cast<float>(elems)));
    }
    REQUIRE(soaCollectionHost2View.s1() == 1);
    REQUIRE(soaCollectionHost2View.s2() == Approx(2.0f));
    REQUIRE(soaCollectionHost2View.s3() == Approx(3.0f));
  }
}
