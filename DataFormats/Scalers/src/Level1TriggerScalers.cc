...
namespace io_v1 {
  ...
  std::ostream& operator<<(std::ostream& s, Level1TriggerScalers const& c) {
    ...
    snprintf(
        line, kLineBufferSize, " GtEvents:                        %20llu %22.3f Hz", c.gtEvents(), c.gtEventsRate());
    s << line << std::endl;
    ...
    if (c.version() >= 5) {
      snprintf(
          line, kLineBufferSize, " LastOrbitCounter0:  %10u  0x%8.8X", c.lastOrbitCounter0(), c.lastOrbitCounter0());
      s << line << std::endl;
      ...
      snprintf(
          line, kLineBufferSize, " LastEventCounter0:  %10u  0x%8.8X", c.lastEventCounter0(), c.lastEventCounter0());
      s << line << std::endl;
