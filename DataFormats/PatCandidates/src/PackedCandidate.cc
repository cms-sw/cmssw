...
void pat::PackedCandidate::packVtx(bool unpackAfterwards) {
  ...
  float s = std::sin(float(p4_.load()->Phi()) + dphi_),
        c = std::cos(float(
            p4_.load()->Phi() +
            dphi_));  // not the fastest option, but we're in reduced precision already, so let's avoid more roundoffs
  dxy_ = -dxPV * s + dyPV * c;
...
void pat::PackedCandidate::unpackVtx() const {
  ...
  auto vertex = std::make_unique<Point>(
      ...
      pv.Y() + dxy_ * c,
      pv.Z() +
          dz_);  // for our choice of using the PCA to the PV, by definition the remaining term -(dx*cos(phi) + dy*sin(phi))*(pz/pt) is zero
