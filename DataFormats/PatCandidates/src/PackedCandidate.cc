...
namespace pat::io_v1 {
  CovarianceParameterization PackedCandidate::covarianceParameterization_;
  std::once_flag PackedCandidate::covariance_load_flag;

  void PackedCandidate::pack(bool unpackAfterwards) {
  ...
  void PackedCandidate::packVtx(bool unpackAfterwards) {
  ...
  void PackedCandidate::unpack() const {
  ...
  void PackedCandidate::packCovariance(const reco::TrackBase::CovarianceMatrix &m, bool unpackAfterwards) {
  ...
  void PackedCandidate::unpackCovariance() const {
  ...
  void PackedCandidate::unpackVtx() const {
    reco::VertexRef pvRef = vertexRef();
    ...
    auto vertex =
        std::make_unique<Point>(pv.X() - dxy_ * s,
                                pv.Y() + dxy_ * c,
  ...
  const reco::Track PackedCandidate::pseudoPosDefTrack() const {
  ...
  void PackedCandidate::unpackTrk() const {
    maybeUnpackBoth();
  ...
  const reco::CandidateBaseRef &PackedCandidate::masterClone() const {
    throw cms::Exception("Invalid Reference") << "this Candidate has no master clone reference."
                                              << "Can't call masterClone() method.\n";
  }

  bool PackedCandidate::hasMasterClone() const { return false; }

  bool PackedCandidate::hasMasterClonePtr() const { return false; }

  const reco::CandidatePtr &PackedCandidate::masterClonePtr() const {
    throw cms::Exception("Invalid Reference") << "this Candidate has no master clone ptr."
                                              << "Can't call masterClonePtr() method.\n";
  }

  size_t PackedCandidate::numberOfDaughters() const { return 0; }

  size_t PackedCandidate::numberOfMothers() const { return 0; }

  bool PackedCandidate::overlap(const reco::Candidate &o) const {
    return p4() == o.p4() && vertex() == o.vertex() && charge() == o.charge();
    //  return  p4() == o.p4() && charge() == o.charge();
  }

  const reco::Candidate *PackedCandidate::daughter(size_type) const { return nullptr; }

  const reco::Candidate *PackedCandidate::mother(size_type) const { return nullptr; }

  const reco::Candidate *PackedCandidate::daughter(const std::string &) const {
    throw edm::Exception(edm::errors::UnimplementedFeature)
        << "This Candidate type does not implement daughter(std::string). "
        << "Please use CompositeCandidate or NamedCompositeCandidate.\n";
  }

  reco::Candidate *PackedCandidate::daughter(const std::string &) {
    throw edm::Exception(edm::errors::UnimplementedFeature)
        << "This Candidate type does not implement daughter(std::string). "
        << "Please use CompositeCandidate or NamedCompositeCandidate.\n";
  }

  reco::Candidate *PackedCandidate::daughter(size_type) { return nullptr; }

  double PackedCandidate::vertexChi2() const { return 0; }

  double PackedCandidate::vertexNdof() const { return 0; }

  double PackedCandidate::vertexNormalizedChi2() const { return 0; }

  double PackedCandidate::vertexCovariance(int i, int j) const {
    throw edm::Exception(edm::errors::UnimplementedFeature)
        << "reco::ConcreteCandidate does not implement vertex covariant "
           "matrix.\n";
  }

  void PackedCandidate::fillVertexCovariance(CovarianceMatrix &err) const {
    throw edm::Exception(edm::errors::UnimplementedFeature)
        << "reco::ConcreteCandidate does not implement vertex covariant "
           "matrix.\n";
  }

  bool PackedCandidate::longLived() const { return false; }

  bool PackedCandidate::massConstraint() const { return false; }

  // puppiweight
  void PackedCandidate::setPuppiWeight(float p, float p_nolep) {
  ...
  float PackedCandidate::puppiWeight() const { return 1.f * packedPuppiweight_ / std::numeric_limits<uint8_t>::max(); }

  float PackedCandidate::puppiWeightNoLep() const {
  ...
  void PackedCandidate::setRawCaloFraction(float p) {
  ...
  void PackedCandidate::setRawHcalFraction(float p) { rawHcalFraction_ = 100 * p; }

  void PackedCandidate::setCaloFraction(float p) { caloFraction_ = 100 * p; }

  void PackedCandidate::setHcalFraction(float p) { hcalFraction_ = 100 * p; }

  void PackedCandidate::setIsIsolatedChargedHadron(bool p) { isIsolatedChargedHadron_ = p; }

  void PackedCandidate::setDTimeAssociatedPV(float aTime, float aTimeError) {
  ...
  uint8_t PackedCandidate::packTimeError(float timeError) {
    if (timeError <= 0)
      return 0;
    // log-scale packing.
  ...
  float PackedCandidate::unpackTimeWithError(int16_t time, uint8_t timeError) {
    if (time % 2 == 0) {
      ...
      return PackedCandidate::unpackTimeNoError(time / 2);
    }
  }
  int16_t PackedCandidate::packTimeWithError(float time, float timeError) {
