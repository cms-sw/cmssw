//
// $Id: GenericParticle.cc,v 1.3.2.2 2008/04/03 18:57:18 lowette Exp $
//
    ...
    PATObject<reco::RecoCandidate>()
{
    fillInFrom(*cand);
    refToOrig_ = cand;
#if 0 // As it took me quite some time to cast a edm::RefToBase<Candidate> to a edm::RefToBase<reco::RecoCandidate>
      // i won't delete this code which might be useful in the future
    const reco::RecoCandidate  *rc = dynamic_cast<const reco::RecoCandidate *>(cand.get());
    if (rc != 0) {
        // Now we have to convert edm::RefToBase<Candidate> to edm::RefToBase<reco::RecoCandidate>
        // which is far from trivial. 
        // a) we need a boost::shared_ptr< reftobase::RefHolderBase >, that we can get from the RefToBase<Candidate>
        boost::shared_ptr< edm::reftobase::RefHolderBase > holder(
             boost::shared_ptr< edm::reftobase::RefHolderBase>(cand.holder().release())
        );
        // b) and then we can make the RefToBase from it
        refToOrig_ = edm::RefToBase<reco::RecoCandidate>(holder); 
        // c) this will make the Ref go through IndirectHolder<reco::RecoCandidate>, and work fine
