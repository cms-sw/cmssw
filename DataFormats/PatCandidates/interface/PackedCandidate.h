...
namespace pat {
  ...
  public:
    ...
    const PVAssoc fromPV(size_t ipv = 0) const {
      ...
      if (pvAssociationQuality() == CompatibilityBTag and std::abs(dzAssociatedPV()) > std::abs(dz(ipv)))
        return PVTight;  // it is not closest, but at least prevents the B assignment stealing
    ...
    void setRawCaloFraction(
        float p);  /// Set the raw ECAL+HCAL energy over candidate energy for isolated charged hadrons
    ...
    void setIsIsolatedChargedHadron(
        bool p);  /// Set isolation (as in particle flow, i.e. at calorimeter surface rather than at PV) flat for charged hadrons
    bool isIsolatedChargedHadron() const {
      return isIsolatedChargedHadron_;
    }  /// Flag isolation (as in particle flow, i.e. at calorimeter surface rather than at PV) flag for charged hadrons
  ...
  protected:
    ...
    void packBoth() {
      ...
      unpack();
      unpackVtx();
    }  // do it this way, so that we don't loose precision on the angles before computing dxy,dz
    ...
    uint8_t packedHits_,
        packedLayers_;  // packedLayers_ -> layers with valid hits; packedHits_ -> extra hits beyond the one-per-layer implied by packedLayers_
    ...
    static constexpr float MIN_TIME_NOERROR =
        0.0002f;                                 // 0.2 ps, smallest non-zero time that can be stored by packTimeNoError
