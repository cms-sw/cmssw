...
public:
  ...
  CSCStripDigi(const int &istrip, const std::vector<int> &vADCCounts,
      ...
      : strip(istrip), ADCCounts(vADCCounts), ADCOverflow(vADCOverflow),
        OverlappedSample(vOverlap), Errorstat(vErrorstat) {}

  // Construct from the strip number and the ADC readings.
  CSCStripDigi(const int &istrip, const std::vector<int> &vADCCounts)
      : strip(istrip), ADCCounts(vADCCounts), ADCOverflow(8, 0),
        OverlappedSample(8, 0), Errorstat(8, 0) {}

  CSCStripDigi()
      : strip(0), ADCCounts(8, 0), ADCOverflow(8, 0), OverlappedSample(8, 0),
        Errorstat(8, 0) {}
