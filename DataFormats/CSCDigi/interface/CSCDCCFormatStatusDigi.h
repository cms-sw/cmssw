 ...
 *
 *    Description:  CSC DCC Format error, status and payload flags for a single
 * DCC
 *
 *        Version:  1.0
...
public:
  ...
  CSCDCCFormatStatusDigi(
      const DCCIdType DCCId_, const ExaminerMaskType fDCC_MASK_,
      ...
      : DCCId(DCCId_) {
    init();
    setDCCExaminerInfo(fDCC_MASK_, fCSC_MASK_, fDDU_SUMMARY_ERRORS_,
                       mDDU_ERRORS_, mCSC_ERRORS_, mCSC_PAYLOADS_,
                       mCSC_STATUS_);
  }
  ...
  void setDCCExaminerInfo(
      const ExaminerMaskType fDCC_MASK_, const ExaminerMaskType fCSC_MASK_,
      const ExaminerStatusType fDDU_SUMMARY_ERRORS_,
      const std::map<DDUIdType, ExaminerStatusType> &mDDU_ERRORS_,
      const std::map<CSCIdType, ExaminerStatusType> &mCSC_ERRORS_,
      const std::map<CSCIdType, ExaminerStatusType> &mCSC_PAYLOADS_,
  ...
  void setDDUSummaryErrors(ExaminerStatusType status) {
    fDDU_SUMMARY_ERRORS = status;
  }
  void setDDUErrors(DDUIdType DDUId, ExaminerStatusType status) {
    std::map<DDUIdType, ExaminerStatusType>::const_iterator item =
  ...
  void setCSCErrors(CSCIdType CSCId, ExaminerStatusType status) {
    std::map<CSCIdType, ExaminerStatusType>::const_iterator item =
  ...
  ExaminerStatusType getCSCErrors(const uint16_t crateId,
                                  const uint16_t dmbId) const {
  ...
  ExaminerStatusType getCSCPayload(const uint16_t crateId,
                                   const uint16_t dmbId) const {
  ...
  ExaminerStatusType getCSCStatus(const uint16_t crateId,
                                  const uint16_t dmbId) const {
  ...
  bool getDDUErrorFlag(const DDUIdType DDUId,
                       const FormatErrorFlag flag) const {
    return ((getDDUErrors(DDUId) & ExaminerStatusType(0x1 << flag)) != 0);
  }

  bool getCSCErrorFlag(const CSCIdType CSCId,
                       const FormatErrorFlag flag) const {
    return ((getCSCErrors(CSCId) & ExaminerStatusType(0x1 << flag)) != 0);
  }

  bool getCSCErrorFlag(const uint16_t crateId, const uint16_t dmbId,
                       const FormatErrorFlag flag) const {
    return ((getCSCErrors(crateId, dmbId) & ExaminerStatusType(0x1 << flag)) !=
            0);
  }

  bool getCSCPayloadFlag(const CSCIdType CSCId,
                         const CSCPayloadFlag flag) const {
    return ((getCSCPayload(CSCId) & ExaminerStatusType(0x1 << flag)) != 0);
  }

  bool getCSCPayloadFlag(const uint16_t crateId, const uint16_t dmbId,
                         const CSCPayloadFlag flag) const {
    return ((getCSCPayload(crateId, dmbId) & ExaminerStatusType(0x1 << flag)) !=
            0);
  }
  ...
  bool getCSCStatusFlag(const uint16_t crateId, const uint16_t dmbId,
                        const CSCStatusFlag flag) const {
    return ((getCSCStatus(crateId, dmbId) & ExaminerStatusType(0x1 << flag)) !=
            0);
  }
