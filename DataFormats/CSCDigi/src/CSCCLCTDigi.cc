...
CSCCLCTDigi::CSCCLCTDigi(const int valid, const int quality, const int pattern,
                         ...
                         const int fullbx)
    : valid_(valid), quality_(quality), pattern_(pattern),
      striptype_(striptype), bend_(bend), strip_(strip), cfeb_(cfeb), bx_(bx),
      trknmb_(trknmb),
      // fullbx_(0)
      fullbx_(fullbx) {
  // valid_     = valid;
...
CSCCLCTDigi::CSCCLCTDigi()
    : valid_(0), quality_(0), pattern_(0), striptype_(0), bend_(0), strip_(0),
      cfeb_(0), bx_(0), trknmb_(0), fullbx_(0) {
...
void CSCCLCTDigi::print() const {
  if (isValid()) {
    ...
    edm::LogVerbatim("CSCDigi")
        ...
        << " BX = " << std::setw(1) << getBX() << " Full BX= " << std::setw(1)
        << getFullBX();
  } else {
...
std::ostream &operator<<(std::ostream &o, const CSCCLCTDigi &digi) {
  return o << "CSC CLCT #" << digi.getTrknmb() << ": Valid = " << digi.isValid()
           << " Quality = " << digi.getQuality()
           << " Pattern = " << digi.getPattern()
           << " StripType = " << digi.getStripType()
           << " Bend = " << digi.getBend() << " Strip = " << digi.getStrip()
