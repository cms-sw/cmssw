...
bool CSCALCTDigi::operator==(const CSCALCTDigi &rhs) const {
  ...
  if (isValid() == rhs.isValid() && getQuality() == rhs.getQuality() &&
      ...
      getCollisionB() == rhs.getCollisionB() && getKeyWG() == rhs.getKeyWG() &&
      getBX() == rhs.getBX()) {
    returnValue = true;
  }
  return returnValue;
...
void CSCALCTDigi::print() const {
  if (isValid()) {
    edm::LogVerbatim("CSCDigi")
        << "CSC ALCT #" << setw(1) << getTrknmb() << ": Valid = " << setw(1)
        << isValid() << " Quality = " << setw(2) << getQuality()
        << " Accel. = " << setw(1) << getAccelerator()
        << " PatternB = " << setw(1) << getCollisionB()
        << " Key wire group = " << setw(3) << getKeyWG() << " BX = " << setw(2)
        << getBX() << " Full BX= " << std::setw(1) << getFullBX();
  } else {
...
std::ostream &operator<<(std::ostream &o, const CSCALCTDigi &digi) {
  return o << "CSC ALCT #" << digi.getTrknmb() << ": Valid = " << digi.isValid()
