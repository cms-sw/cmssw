      ...
      for (int rng = 1; rng < 4; rng++)
        for (int csc = 1; csc < 37; csc++)
          for (int pln = 1; pln < 7; pln++) {
            ...
            collection.put(std::make_pair(digivec.begin(), digivec.end()),
                           detid);

          } // end of for(int endcp=1 ...for(int pln=1 ...)
}

void testCSCDigis::fillCSCComparatorDigi(
    CSCComparatorDigiCollection &collection) {
        ...
        for (int csc = 1; csc < 37; csc++)
          for (int pln = 1; pln < 7; pln++) {
            ...
            for (int i = 10; i < 11; ++i) {
              ...
              CSCStripDigi digi(aStrip, someADCCounts, someVec, someVec,
                                someVec);
              digivec.push_back(digi);
            }
  ...
  for (int endcp = 1; endcp < 3; endcp++)
    for (int stn = 1; stn < 5; stn++)
      for (int rng = 1; rng < 4; rng++)
        for (int csc = 1; csc < 37; csc++)
          for (int pln = 3; pln < 4;
               pln++) { // ALCT primitives are for layer 3 only
        ...
        for (int csc = 1; csc < 37; csc++)
          for (int pln = 3; pln < 4;
            ...
            for (int i = 1; i < 3; ++i) {
              ...
              CSCALCTDigi digi(valid, quality, accel, patternb, keywire, bx,
                               trknmb);
              digivec.push_back(digi);
            }
  ...
  for (int endcp = 1; endcp < 3; endcp++)
    for (int stn = 1; stn < 5; stn++)
      for (int rng = 1; rng < 4; rng++)
        for (int csc = 1; csc < 37; csc++)
          for (int pln = 3; pln < 4;
               pln++) { // CLCT primitives are for layer 3 only
            ...
            for (int i = 1; i < 3; ++i) {
              ...
              CSCCLCTDigi digi(valid, quality, patshape, striptype, bend, strip,
                               cfeb, bx, trknmb);
              digivec.push_back(digi);
            }
...
void testCSCDigis::fillCSCCorrLCTDigi(
    CSCCorrelatedLCTDigiCollection &collection) {
  for (int endcap = 1; endcap <= 2; ++endcap)
    for (int station = 1; station <= 4; ++station)
      for (int ring = 1; ring <= ((station == 1) ? 3 : 2); ++ring)
        for (int chamber = 1;
             chamber <= ((ring == 1 && station != 1) ? 18 : 36); ++chamber)
          for (int layer = 3; layer <= 3; ++layer) {
            ...
            for (int trknmb = 1; trknmb <= 2; ++trknmb) {
              ...
              CSCCorrelatedLCTDigi digi(trknmb, valid, quality, keywire, strip,
                                        pattern, bend, bx, mpclink);
              digivec.push_back(digi);
            }
            collection.put(std::make_pair(digivec.begin(), digivec.end()),
                           detid);
...
void testCSCDigis::fillCSCCFEBStatusDigi(
    CSCCFEBStatusDigiCollection &collection) {
...
void testCSCDigis::fillCSCTMBStatusDigi(
    CSCTMBStatusDigiCollection &collection) {
  for (int endcp = 1; endcp < 3; endcp++)
    for (int stn = 1; stn < 5; stn++)
      for (int rng = 1; rng < 4; rng++)
        for (int csc = 1; csc < 37; csc++) {
...
/*
void testCSCDigis::fillCSCDCCFormatStatusDigi(CSCDCCFormatStatusDigiCollection &
collection){ std::vector<CSCDCCFormatStatusDigi> digivec; CSCDetId
detid(0,0,0,0,0);
...
*/
void testCSCDigis::fillCSCDCCFormatStatusDigi(
    CSCDCCFormatStatusDigiCollection &collection) {
  std::vector<CSCDCCFormatStatusDigi> digivec;
  CSCDetId detid(0, 0, 0, 0, 0);
  for (int dcc = FEDNumbering::MINCSCFEDID; dcc < FEDNumbering::MAXCSCFEDID;
       dcc++) {
...
void testCSCDigis::readCSCCorrLCTDigi(
    CSCCorrelatedLCTDigiCollection &collection) {
  int count = 0;
  ...
  for (detUnitIt = collection.begin(); detUnitIt != collection.end();
    ...
    for (CSCCorrelatedLCTDigiCollection::const_iterator digiIt = range.first;
      ...
      std::cout
          << "CSC Correlated LCT - endcap station ring csc layer LCT# Quality: "
          << id.endcap() << " " << id.station() << " " << id.ring() << " "
          << id.chamber() << " " << id.layer() << " " << digiIt->getTrknmb()
...
void testCSCDigis::readCSCWireDigi(CSCWireDigiCollection &collection) {
  ...
  for (detUnitIt = collection.begin(); detUnitIt != collection.end();
    ...
    for (CSCWireDigiCollection::const_iterator digiIt = range.first;
      ...
      printf("CSC Wire - endcap station ring csc layer wire tbin: %3d %3d %3d "
             "%3d %3d %3d  %3d\n",
...
void testCSCDigis::readCSCComparatorDigi(
    CSCComparatorDigiCollection &collection) {
  ...
  for (detUnitIt = collection.begin(); detUnitIt != collection.end();
    ...
    for (CSCComparatorDigiCollection::const_iterator digiIt = range.first;
      ...
      CPPUNIT_ASSERT((*digiIt).getTimeBin() ==
                     1); // time bin word=6 means bit 1 should be first!
      printf("CSCComparatorDigi - endcap station ring csc layer strip "
             "comparator time: %3d %3d %3d %3d %3d %3d %3d %3d\n",
             id.endcap(), id.station(), id.ring(), id.chamber(), id.layer(),
             (*digiIt).getStrip(), (*digiIt).getComparator(),
             (*digiIt).getTimeBin());
...
void testCSCDigis::readCSCStripDigi(CSCStripDigiCollection &collection) {
  ...
  for (detUnitIt = collection.begin(); detUnitIt != collection.end();
    ...
    for (CSCStripDigiCollection::const_iterator digiIt = range.first;
      ...
      count++;
      CPPUNIT_ASSERT((*digiIt).getStrip() == 10);
      printf("CSC Strip - endcap station ring csc layer: %3d %3d %3d %3d %3d "
             "strip: %3d  ADC: %4d %4d %4d %4d %4d %4d %4d %4d\n",
             id.endcap(), id.station(), id.ring(), id.chamber(), id.layer(),
             (*digiIt).getStrip(), (*digiIt).getADCCounts()[0],
             (*digiIt).getADCCounts()[1], (*digiIt).getADCCounts()[2],
             (*digiIt).getADCCounts()[3], (*digiIt).getADCCounts()[4],
             (*digiIt).getADCCounts()[5], (*digiIt).getADCCounts()[6],
...
void testCSCDigis::readCSCRPCDigi(CSCRPCDigiCollection &collection) {
  ...
  for (detUnitIt = collection.begin(); detUnitIt != collection.end();
    ...
    for (CSCRPCDigiCollection::const_iterator digiIt = range.first;
      ...
      printf("RPC digi - endcap station ring csc layer: %3d %3d %3d %3d %3d "
             "%3d  %4d \n",
             id.endcap(), id.station(), id.ring(), id.chamber(), id.layer(),
             (*digiIt).getRpc(), (*digiIt).getBXN());
...
void testCSCDigis::readCSCALCTDigi(CSCALCTDigiCollection &collection) {
  ...
  for (detUnitIt = collection.begin(); detUnitIt != collection.end();
    ...
    for (CSCALCTDigiCollection::const_iterator digiIt = range.first;
      ...
      printf("CSC ALCT - endcap station ring csc layer valid quality accel "
             "pattern wire bx track: %3d %3d %3d %3d %3d %3d  %3d %3d %3d %3d "
             "%3d %3d\n",
             id.endcap(), id.station(), id.ring(), id.chamber(), id.layer(),
             (*digiIt).isValid(), (*digiIt).getQuality(),
             (*digiIt).getAccelerator(), (*digiIt).getCollisionB(),
...
void testCSCDigis::readCSCCLCTDigi(CSCCLCTDigiCollection &collection) {
  ...
  for (detUnitIt = collection.begin(); detUnitIt != collection.end();
    ...
    for (CSCCLCTDigiCollection::const_iterator digiIt = range.first;
      ...
      printf("CSC CLCT - endcap station ring csc layer valid quality pattern "
             "striptype bend strip cfeb bx tracknmb %3d %3d %3d %3d %3d %3d  "
             "%3d %3d %3d %3d %3d %3d %3d %3d\n",
             id.endcap(), id.station(), id.ring(), id.chamber(), id.layer(),
             (*digiIt).isValid(), (*digiIt).getQuality(),
             (*digiIt).getPattern(), (*digiIt).getStripType(),
             (*digiIt).getBend(), (*digiIt).getStrip(), (*digiIt).getCFEB(),
             (*digiIt).getBX(), (*digiIt).getTrknmb());
...
void testCSCDigis::readCSCCFEBStatusDigi(
    CSCCFEBStatusDigiCollection &collection) {
  ...
  for (detUnitIt = collection.begin(); detUnitIt != collection.end();
    ...
    int cfebcount = 0;
    for (CSCCFEBStatusDigiCollection::const_iterator digiIt = range.first;
         digiIt != range.second; ++digiIt) {
      cfebcount++;
      CPPUNIT_ASSERT((*digiIt).getCFEBNmb() == cfebcount);
      printf("CSC CFEBStatus - endcap station ring csc cfeb: %3d %3d %3d %3d "
             "%3d \n",
             id.endcap(), id.station(), id.ring(), id.chamber(),
             (*digiIt).getCFEBNmb());
      /*  Commented because the latest changes in CFEBStatusdDigi are not
         reflected std::cout<<"CSC CFEBStatus - SCA Full Condition:"; for(int
         i=0;i<4;i++) std::cout<<" "<<(*digiIt).getSCAFullCond()[i];
            std::cout<<std::endl;
...
void testCSCDigis::readCSCTMBStatusDigi(
    CSCTMBStatusDigiCollection &collection) {
  ...
  for (detUnitIt = collection.begin(); detUnitIt != collection.end();
    ...
    for (CSCTMBStatusDigiCollection::const_iterator digiIt = range.first;
         digiIt != range.second; ++digiIt) {
      ...
      printf("CSC TMBStatus - endcap station ring csc TMB_DAV: %3d %3d %3d
      %3d%3d
...
/*
void testCSCDigis::readCSCDCCFormatStatusDigi(CSCDCCFormatStatusDigiCollection &
collection) { CSCDCCFormatStatusDigiCollection::DigiRangeIterator detUnitIt; for
(detUnitIt=collection.begin(); detUnitIt!=collection.end();
       ++detUnitIt){
    ...
    int count=0;
    for (CSCDCCFormatStatusDigiCollection::const_iterator digiIt = range.first;
digiIt!=range.second; ++digiIt){
...
*/
void testCSCDigis::readCSCDCCFormatStatusDigi(
    CSCDCCFormatStatusDigiCollection &collection) {
