...
class BXVector  {
 ...
 public:
  ...
  unsigned size( ) const { return size(0);}
  ...
  // array subscript operator, always referring to objects for BX=0
  T& operator[](std::size_t i) { return at(0,i); }
  const T& operator[](std::size_t i) const { return at(0,i); }
