...
HOTPDigiTwinMux::HOTPDigiTwinMux(int ieta, int iphi, int bx, int mip, int validbit, int wheel, int sector, int index, int link) {

  if((ieta > 15) || (ieta < -15)) printf("HOTwinMuxDigi:  ieta out of range");
  if ((mip < 0) || (mip > 1)) printf("HOTwinMuxDigi: mip value out of range");
 
  theTP_HO=(std::abs(ieta)&0xF) | ((ieta<0)?(0x10) : (0x00)) |
    ((iphi&0x7F)<<5) |  
