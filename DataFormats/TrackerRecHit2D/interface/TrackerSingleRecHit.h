...
public:
  ...
  TrackerSingleRecHit(const LocalPoint& p, const LocalError& e,
		      DetId id,
		      OmniClusterRef const&  clus) : Base(p,e,id, Base::single), cluster_(clus){}

  TrackerSingleRecHit(const LocalPoint& p, const LocalError& e,
		      DetId id,
		      ClusterPixelRef const&  clus) : Base(p,e,id, Base::single), cluster_(clus){}

  TrackerSingleRecHit(const LocalPoint& p, const LocalError& e,
		      DetId id,
		      ClusterStripRef const&  clus) : Base(p,e,id, Base::single), cluster_(clus){}

  TrackerSingleRecHit(const LocalPoint& p, const LocalError& e,
		      DetId id,  
		      ClusterRegionalRef const& clus) :  Base(p,e,id, Base::single), cluster_(clus){}
