...
class FastTrackerRecHit : public BaseTrackerRecHit 
{
    ...
    FastTrackerRecHit( const LocalPoint& p, const LocalError&e, std::shared_ptr<const GeomDet> idet,fastTrackerRecHitType::HitType hitType) 
    ...
    virtual void setRecHitCombinationIndex(int32_t recHitCombinationIndex) {recHitCombinationIndex_ = recHitCombinationIndex;}
    ...
    void store() { myPos_=pos_;  myErr_=err_;}  ///< helps making the hit postion and error persistent
