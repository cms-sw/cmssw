...
public:
  ...
  unsigned int getBeamId( ) const { return beamId; }

  /// access the collection of hits
  const std::vector<SiStripLaserRecHit2D>& getData( ) const { return data; }

  /// access iterator to the collection of hits
  std::vector<SiStripLaserRecHit2D>::const_iterator begin( ) const { return data.begin(); }

  /// access iterator to the collection of hits
  std::vector<SiStripLaserRecHit2D>::const_iterator end( ) const { return data.end(); }
  ...
  unsigned int getBeamNumber( ) const { return beamId%100/10; }
  ...
  bool isAlignmentTube( ) const { return ( beamId%1000/100 ) == 2; }

  /// true if this beam hits TEC R6 (last digit of beamId)
  bool isRing6( ) const { return (beamId%10) == 1; }
