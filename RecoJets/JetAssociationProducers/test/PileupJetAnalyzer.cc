...
PileupJetAnalyzer::PileupJetAnalyzer(const edm::ParameterSet &params)
  ...
  jetTagToken = consumes<JetFloatAssociation::Container>(
      params.getParameter<edm::InputTag>("jetTagLabel"));
  tokenSimTrack =
      consumes<edm::SimTrackContainer>(edm::InputTag("fastSimProducer"));
  tokenSimVertex =
      consumes<edm::SimVertexContainer>(edm::InputTag("fastSimProducer"));
...
void PileupJetAnalyzer::analyze(const edm::Event &event,
  ...
  for (JetTracksAssociationCollection::const_iterator iter =
    ...
    if (jet->energy() < jetMinE || jet->et() < jetMinEt ||
