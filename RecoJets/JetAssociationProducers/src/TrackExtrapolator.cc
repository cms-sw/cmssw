...
TrackExtrapolator::TrackExtrapolator(const edm::ParameterSet &iConfig) {
  tracksSrc_ = consumes<reco::TrackCollection>(
...
void TrackExtrapolator::produce(edm::Event &iEvent,
  ...
  iSetup.get<DetIdAssociatorRecord>().get("EcalDetIdAssociator",
  ...
  auto extrapolations =
      std::make_unique<std::vector<reco::TrackExtrapolation>>();
  ...
  for (std::vector<reco::TrackRef>::const_iterator
           trkBegin = goodTracks.begin(),
    ...
    if (propagateTrackToVolume(**itrk, *field_h, *propagator_h, ecalvolume,
      ...
      extrapolations->push_back(
          reco::TrackExtrapolation(*itrk, vresultPos, vresultMom));
    }
  }
...
// -----------------------------------------------------------------------------
//
bool TrackExtrapolator::propagateTrackToVolume(
    const reco::Track &fTrack, const MagneticField &fField,
    const Propagator &fPropagator, const FiducialVolume &volume,
    reco::TrackBase::Point &resultPos, reco::TrackBase::Vector &resultMom) {
  GlobalPoint trackPosition(fTrack.vx(), fTrack.vy(),
                            fTrack.vz()); // reference point
  GlobalVector trackMomentum(fTrack.px(), fTrack.py(),
                             fTrack.pz()); // reference momentum
  if (fTrack.extra()
          .isAvailable()) { // use outer point information, if available
    trackPosition =
        GlobalPoint(fTrack.outerX(), fTrack.outerY(), fTrack.outerZ());
    trackMomentum =
        GlobalVector(fTrack.outerPx(), fTrack.outerPy(), fTrack.outerPz());
  }

  GlobalTrajectoryParameters trackParams(trackPosition, trackMomentum,
                                         fTrack.charge(), &fField);
  ...
  if (propagatedInfo.isValid() && propagatedInfo.globalPosition().z() > minz) {
    propagatedInfo = fPropagator.propagate(
        trackState, *Plane::build(Surface::PositionType(0, 0, minz),
                                  Surface::RotationType()));

  } else if (propagatedInfo.isValid() &&
             propagatedInfo.globalPosition().z() < -minz) {
    propagatedInfo = fPropagator.propagate(
        trackState, *Plane::build(Surface::PositionType(0, 0, -minz),
                                  Surface::RotationType()));
  }
