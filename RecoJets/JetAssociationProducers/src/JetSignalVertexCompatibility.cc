...
JetSignalVertexCompatibility::JetSignalVertexCompatibility(
  ...
  jetTracksAssocToken = consumes<JetTracksAssociationCollection>(
      params.getParameter<edm::InputTag>("jetTracksAssoc"));
  primaryVerticesToken = consumes<VertexCollection>(
...
void JetSignalVertexCompatibility::produce(edm::Event &event,
  ...
  auto result = std::make_unique<JetFloatAssociation::Container>(
      jetTracksAssoc->keyProduct());
