...
JetExtender::JetExtender(const edm::ParameterSet &fConfig)
    : mJets(fConfig.getParameter<edm::InputTag>("jets")),
      ...
      mJet2TracksAtCALO(
          fConfig.getParameter<edm::InputTag>("jet2TracksAtCALO")) {
  token_mJets = consumes<edm::View<reco::Jet>>(mJets);
  if (!(mJet2TracksAtVX.label().empty()))
    token_mJet2TracksAtVX =
        consumes<reco::JetTracksAssociation::Container>(mJet2TracksAtVX);
  if (!(mJet2TracksAtCALO.label().empty()))
    token_mJet2TracksAtCALO =
        consumes<reco::JetTracksAssociation::Container>(mJet2TracksAtCALO);
...
void JetExtender::produce(edm::Event &fEvent, const edm::EventSetup &fSetup) {
  ...
  auto jetExtender = std::make_unique<reco::JetExtendedAssociation::Container>(
      reco::JetRefBaseProd(jets_h));

  // loop over jets (make sure jets in associations are the same as in
  // collection
  ...
  for (unsigned j = 0; j < jets_h->size(); ++j) {
    ...
    if (j2tVX_h.isValid()) { // fill tracks@VX  summary
      try {
        extendedData.mTracksAtVertexNumber =
            reco::JetTracksAssociation::tracksNumber(*j2tVX_h, jet);
        extendedData.mTracksAtVertexP4 =
            reco::JetTracksAssociation::tracksP4(*j2tVX_h, jet);
      } catch (cms::Exception const &) {
        edm::LogError("MismatchedJets")
            << "Jets in original collection " << mJets
    ...
    if (j2tCALO_h.isValid()) { // fill tracks@CALO  summary
      try {
        extendedData.mTracksAtCaloNumber =
            reco::JetTracksAssociation::tracksNumber(*j2tCALO_h, jet);
        extendedData.mTracksAtCaloP4 =
            reco::JetTracksAssociation::tracksP4(*j2tCALO_h, jet);
      } catch (cms::Exception const &) {
        edm::LogError("MismatchedJets")
            << "Jets in original collection " << mJets
