...
void CompoundJetProducer::writeCompoundJets(edm::Event& iEvent, edm::EventSetup const& iSetup) {
  ...
  auto it = fjCompoundJets_.begin(), iEnd = fjCompoundJets_.end(), iBegin = fjCompoundJets_.begin();
  ...
  for (; it != iEnd; ++it) {
    ...
    auto itSubJetBegin = it->subjets().begin(), itSubJet = itSubJetBegin, itSubJetEnd = it->subjets().end();
    for (; itSubJet != itSubJetEnd; ++itSubJet) {
      ...
      auto fastSubIt = subjetFastjetConstituentIndices.begin(), transConstEnd = subjetFastjetConstituentIndices.end();
