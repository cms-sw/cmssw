...
const char *const OldVirtualJetProducer::JetType::names[] = {
  "BasicJet","GenJet","CaloJet","PFJet","TrackJet","PFClusterJet"
};


//______________________________________________________________________________
OldVirtualJetProducer::JetType::Type
OldVirtualJetProducer::JetType::byName(const string &name)
{
...
void OldVirtualJetProducer::makeProduces( std::string alias, std::string tag )
{
...
//______________________________________________________________________________
OldVirtualJetProducer::OldVirtualJetProducer(const edm::ParameterSet& iConfig)
  ...
  , fromHTTTopJetProducer_(0)
{
  ...
  if (jetAlgorithm_=="SISCone") {
    ...
    throw cms::Exception("Invalid jetAlgorithm")
      <<"Jet algorithm for OldVirtualJetProducer is invalid, Abort!\n";
  ...
  if ( doPUOffsetCorr_ ) {
     ...
     if(puSubtractorName_.empty()){
       edm::LogWarning("OldVirtualJetProducer") << "Pile Up correction on; however, pile up type is not specified. Using default... \n";
...
//______________________________________________________________________________
OldVirtualJetProducer::~OldVirtualJetProducer()
{
} 
...
//______________________________________________________________________________
void OldVirtualJetProducer::produce(edm::Event& iEvent,const edm::EventSetup& iSetup)
{
  ...
  // clear data
  LogDebug("OldVirtualJetProducer") << "Clear data\n";
  fjInputs_.clear();
  fjJets_.clear();
  inputs_.clear();  
  ...
  LogDebug("OldVirtualJetProducer") << "Got inputs\n";
  ...
  inputTowers();
  LogDebug("OldVirtualJetProducer") << "Inputted towers\n";
  ...
  if ( doPUOffsetCorr_ ) {
     ...
     LogDebug("OldVirtualJetProducer") << "Subtracted pedestal\n";
  }
  ...
  LogDebug("OldVirtualJetProducer") << "Ran algorithm\n";
  // For Pileup subtraction using offset correction:
  ...
  if ( doPUOffsetCorr_ ) {
    LogDebug("OldVirtualJetProducer") << "Do PUOffsetCorr\n";
  ...
  output( iEvent, iSetup );
  LogDebug("OldVirtualJetProducer") << "Wrote jets\n";
...
void OldVirtualJetProducer::inputTowers( )
{
...
//______________________________________________________________________________
bool OldVirtualJetProducer::isAnomalousTower(reco::CandidatePtr input)
{
  if (!makeCaloJet(jetTypeE)) 
      return false;
  else
...
//______________________________________________________________________________
// void OldVirtualJetProducer::runAlgorithm( edm::Event & iEvent, edm::EventSetup const& iSetup,
//                                        std::vector<edm::Ptr<reco::Candidate> > const & inputs_);

//______________________________________________________________________________
void OldVirtualJetProducer::copyConstituents(const vector<fastjet::PseudoJet>& fjConstituents,
                                          reco::Jet* jet)
{
...
OldVirtualJetProducer::getConstituents(const vector<fastjet::PseudoJet>&fjConstituents)
{
...
void OldVirtualJetProducer::output(edm::Event & iEvent, edm::EventSetup const& iSetup)
{
  ...
  if ( !writeCompound_ ) {
    ...
    case JetType::BasicJet :
      ...
      break;
    default:
      throw cms::Exception("InvalidInput") << "invalid jet type in OldVirtualJetProducer\n";
      break;
    };
  } else {
...
template< typename T >
void OldVirtualJetProducer::writeJets( edm::Event & iEvent, edm::EventSetup const& iSetup )
{
...
template< class T>
void OldVirtualJetProducer::writeCompoundJets(  edm::Event & iEvent, edm::EventSetup const& iSetup)
{
  if ( verbosity_ >= 1 ) { 
    std::cout << "<OldVirtualJetProducer::writeCompoundJets (moduleLabel = " << moduleLabel_ << ")>:" << std::endl;
  }
