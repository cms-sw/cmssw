...
// Class:      FastjetJetProducer
// 
/**\class FastjetJetProducer FastjetJetProducer.cc RecoJets/JetProducers/plugins/FastjetJetProducer.cc
...
void FastjetJetProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
	...
	if (!makeTrackJet(jetTypeE)) VirtualJetProducer::produce( iEvent, iSetup );  	// use the default production from one collection
	else produceTrackJets(iEvent, iSetup);    					// produce trackjets from tracks grouped per primary vertex

	// fjClusterSeq_ retains quite a lot of memory - about 1 to 7Mb at 200 pileup
	// depending on the exact configuration; and there are 24 OldFastjetJetProducers in the
...
void FastjetJetProducer::runAlgorithm( edm::Event & iEvent, edm::EventSetup const& iSetup) {
	...
	*/
	...
	else fjClusterSeq_ = ClusterSequencePtr( new fastjet::ClusterSequenceVoronoiArea( fjInputs_, *fjJetDefinition_ , fastjet::VoronoiAreaSpec(voronoiRfact_) ) );
	...
	for ( std::vector<fastjet::PseudoJet>::const_iterator ijet = tempJets.begin(),
		...
		for ( transformer_coll::const_iterator itransf = transformers.begin(),
			...
			if ( transformedJet != 0 ) transformedJet = (**itransf)(transformedJet);
			else passed=false;
		}

		if ( correctShape_ ) transformedJet = (*subtractor)(transformedJet);
		if ( passed ) fjJets_.push_back( transformedJet );
	}
}
