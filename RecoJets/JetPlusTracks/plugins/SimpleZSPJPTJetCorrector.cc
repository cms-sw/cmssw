...
typedef ROOT::Math::LorentzVector<ROOT::Math::PtEtaPhiE4D<double>>
...
SimpleZSPJPTJetCorrector::SimpleZSPJPTJetCorrector() { debug_ = false; }

SimpleZSPJPTJetCorrector::SimpleZSPJPTJetCorrector(
    const std::string &fDataFile) {
  debug_ = false;
  mParameters = new JetCorrectorParameters(fDataFile, "");

  // std::cout<<" Formula
  // "<<((mParameters->definitions()).formula()).c_str()<<std::endl;

  mFunc = new TFormula("function",
                       ((mParameters->definitions()).formula()).c_str());

  // Read parameters
  if (debug_) {
    std::cout << " Size of parameters as read by SimpleJetCorrectorParameters "
              << mParameters->size() << std::endl;
    for (unsigned int i = 0; i < mParameters->size(); i++) {
      ...
      for (std::vector<float>::const_iterator j = p.begin(); j < p.end(); j++) {
        std::cout << " Parameter number " << mParameters->record(i).xMin(0)
                  << " " << mParameters->record(i).xMax(0) << " " << (*j)
                  << std::endl;
      }
    }
  } // debug
...
double SimpleZSPJPTJetCorrector::correctionPtEtaPhiE(double fPt, double fEta,
                                                     double fPhi,
                                                     double fE) const {
...
double SimpleZSPJPTJetCorrector::correctionEtEtaPhiP(double fEt, double fEta,
                                                     double fPhi,
                                                     double fP) const {
  ...
  if (debug_) {
    cout << " Et and eta of jet " << et << " " << eta << " bin " << band << " "
         << p[1] << " " << p[2] << " " << p[3] << " " << p[4] << " " << p[5]
         << endl;
  }
  ...
  if (et < p[0])
    koef = 1. - mFunc->Eval(p[0]);

  //
  if (koef <= 0.000001) {
    if (debug_)
      std::cout << "SimpleZSPJPTJetCorrector::Problem with ZSP corrections "
                << koef << std::endl;
    koef = 1.;
  }
  ...
  if (debug_)
    cout << " The new energy found " << etnew << " " << et << " " << koef
         << endl;

  return etnew / et;
}

double SimpleZSPJPTJetCorrector::correctionPUEtEtaPhiP(double fEt, double fEta,
                                                       double fPhi,
                                                       double fP) const {
