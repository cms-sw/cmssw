...
JetPlusTrackCorrector::JetPlusTrackCorrector(const edm::ParameterSet &pset,
    ...
    : verbose_(pset.getParameter<bool>("Verbose")),
      ...
      jetTracksAtVertex_(
          pset.getParameter<edm::InputTag>("JetTracksAssociationAtVertex")),
      jetTracksAtCalo_(
          pset.getParameter<edm::InputTag>("JetTracksAssociationAtCaloFace")),
      ...
      trackQuality_(reco::TrackBase::qualityByName(
          pset.getParameter<std::string>("TrackQuality"))),
      response_(Map(pset.getParameter<std::string>("ResponseMap"), verbose_)),
      efficiency_(
          Map(pset.getParameter<std::string>("EfficiencyMap"), verbose_)),
      leakage_(Map(pset.getParameter<std::string>("LeakageMap"), verbose_)),
      pionMass_(0.140), muonMass_(0.105), elecMass_(0.000511),
  ...
  if (verbose_) {
    ...
    ss << "[JetPlusTrackCorrector::" << __func__
       ...
       << " Particles" << std::endl
       << "  UsePions             : " << (usePions_ ? "true" : "false")
       << std::endl
       << "  UseMuons             : " << (useMuons_ ? "true" : "false")
       << std::endl
       << "  UseElecs             : " << (useElecs_ ? "true" : "false")
       << std::endl
       << " Corrections" << std::endl
       << "  UseInConeTracks      : " << (useInConeTracks_ ? "true" : "false")
       << std::endl
       << "  UseOutOfConeTracks   : "
       << (useOutOfConeTracks_ ? "true" : "false") << std::endl
       << "  UseOutOfVertexTracks : "
       << (useOutOfVertexTracks_ ? "true" : "false") << std::endl
       << "  ResponseMap          : "
       << pset.getParameter<std::string>("ResponseMap") << std::endl
       << " Efficiency" << std::endl
       << "  UsePionEfficiency    : " << (useEff_ ? "true" : "false")
       << std::endl
       << "  EfficiencyMap        : "
       << pset.getParameter<std::string>("EfficiencyMap") << std::endl
       << "  LeakageMap           : "
       << pset.getParameter<std::string>("LeakageMap") << std::endl
       << " Tracks" << std::endl
       ...
       << "  UseTrackQuality      : " << (useTrackQuality_ ? "true" : "false")
       << std::endl
       << " Collections" << std::endl
       << "  Muons                : " << muons_ << std::endl
       << "  Electrons            : " << electrons_ << std::endl
       << " Vectorial" << std::endl
       << "  UseTracksAndResponse : "
       << ((vectorial_ && vecResponse_) ? "true" : "false") << std::endl
       << "  UseTracksOnly        : "
       << ((vectorial_ && !vecResponse_) ? "true" : "false");
  ...
  if (!useInConeTracks_ || !useOutOfConeTracks_ || !useOutOfVertexTracks_) {
    ...
    ss << "[JetPlusTrackCorrector::" << __func__ << "]"
       ...
       << " UseInConeTracks      : " << (useInConeTracks_ ? "true" : "false")
       << std::endl
       << " UseOutOfConeTracks   : " << (useOutOfConeTracks_ ? "true" : "false")
       << std::endl
       << " UseOutOfVertexTracks : "
       << (useOutOfVertexTracks_ ? "true" : "false");
    edm::LogWarning("JetPlusTrackCorrector") << ss.str();
  }

  input_jetTracksAtVertex_token_ =
      iC.consumes<reco::JetTracksAssociation::Container>(jetTracksAtVertex_);
  input_jetTracksAtCalo_token_ =
      iC.consumes<reco::JetTracksAssociation::Container>(jetTracksAtCalo_);
...
// -----------------------------------------------------------------------------
//
double JetPlusTrackCorrector::correction(
    const reco::Jet &fJet, const reco::Jet &fJetcalo, const edm::Event &event,
    const edm::EventSetup &setup, P4 &corrected, MatchedTracks &pions,
    MatchedTracks &muons, MatchedTracks &elecs, bool &validMatches) {
  ...
  if (verbose_) {
    ...
    ss << "Total correction:" << std::endl
       ...
       << "(" << fJet.px() << "," << fJet.py() << "," << fJet.pz() << ","
       << fJet.energy() << ")" << std::endl
       << " Corrected (Px,Py,Pz,E)     : "
       << "(" << corrected.px() << "," << corrected.py() << ","
       << corrected.pz() << "," << corrected.energy() << ")" << std::endl
       << " Uncorrected (Pt,Eta,Phi,M) : "
       << "(" << fJet.pt() << "," << fJet.eta() << "," << fJet.phi() << ","
       << fJet.mass() << ")" << std::endl
       << " Corrected (Pt,Eta,Phi,M)   : "
       << "(" << corrected.pt() << "," << corrected.eta() << ","
       << corrected.phi() << "," << corrected.mass() << ")" << std::endl
       << " Scalar correction to E     : " << scale << std::endl
       << " Scalar correction to Et    : "
       << (fJet.et() > 0. ? corrected.Et() / fJet.et() : 1.); // << std::endl
  ...
  /*
    ...
    <<" "<< theResponseOfChargedWithoutEff <<" "<< theSumPtWithEff <<" "<<
    theSumPtWithoutEff <<" "<<
    theSumEnergyWithEff <<" "<<
...
//
double JetPlusTrackCorrector::correction(
    const reco::Particle::LorentzVector &jet) const {
  edm::LogError("JetPlusTrackCorrector")
      << "JetPlusTrackCorrector can be run on entire event only";
  return 1.;
}
...
// -----------------------------------------------------------------------------
//
bool JetPlusTrackCorrector::matchTracks(
    const reco::Jet &fJet, const edm::Event &event,
    const edm::EventSetup &setup, //@@ required by method in derived class
    jpt::MatchedTracks &pions, jpt::MatchedTracks &muons,
  ...
  // std::cout<<"JetPlusTrackCorrector::matchTracks, JetTrackAssociation
  // ok="<<ok<<std::endl;
  ...
  // Track collections propagated to Vertex and CaloFace for "pions", muons and
  // electrons
...
bool JetPlusTrackCorrector::jtaUsingEventData(const reco::Jet &fJet,
  ...
  if (!jetTracksAtVertex.isValid() || jetTracksAtVertex.failedToGet()) {
    if (verbose_ && edm::isDebugEnabled()) {
      edm::LogWarning("JetPlusTrackCorrector")
          << "[JetPlusTrackCorrector::" << __func__ << "]"
          << " Invalid handle to reco::JetTracksAssociation::Container (for "
             "Vertex)"
  ...
  // std::cout<<" JetPlusTrackCorrector::jtaUsingEventData::here
  // "<<jetSplitMerge_<<std::endl;

  // Retrieve jet-tracks association for given jet
  const reco::JetTracksAssociation::Container jtV =
      *(jetTracksAtVertex.product());
  TrackRefs excluded;
  if (jetSplitMerge_ < 0) {
  ...
  // Retrieve jet-tracks association for given jet
  const reco::JetTracksAssociation::Container jtC =
      *(jetTracksAtCalo.product());
  if (jetSplitMerge_ < 0) {
  ...
  //   std::cout<<" JTA:Tracks in vertex "<<trks.vertex_.size()<<" in calo
  //   "<<trks.caloFace_.size()<<std::endl;
...
bool JetPlusTrackCorrector::getMuons(const edm::Event &event,
                                     edm::Handle<RecoMuons> &reco_muons) const {
...
void JetPlusTrackCorrector::matchTracks(const JetTracks &jet_tracks,
  ...
  // Identify pions/muons/electrons that are "in/in" and "in/out"
  {
    ...
    for (; itrk != jtrk; ++itrk) {
      ...
      for (reco::VertexCollection::const_iterator iv = pvCollection->begin();
           iv != pvCollection->end(); iv++) {
        numpv++;
      ...
      //       std::cout<<"JetPlusTrackCorrector::matchTracks:
      //       jet_tracks.caloFace_.size()="<<jet_tracks.caloFace_.size()
      //                <<" found = "<<found<<std::endl;

      bool is_muon = useMuons_ && matchMuons(itrk, reco_muons);
      bool is_ele =
          useElecs_ && matchElectrons(itrk, reco_elecs, reco_elec_ids);
      ...
      if (found) {
        ...
        if (is_muon) {
          ...
          //         std::cout<<"JetPlusTrackCorrector::matchTracks:
          //         pions.inVertexOutOfCalo_.push_back(*itrk), size="
...
// -----------------------------------------------------------------------------
//
bool JetPlusTrackCorrector::getElectrons(
    const edm::Event &event, edm::Handle<RecoElectrons> &reco_elecs,
...
// -----------------------------------------------------------------------------
//
bool JetPlusTrackCorrector::failTrackQuality(
...
bool JetPlusTrackCorrector::findTrack(const MatchedTracks &pions,
                                      ...
                                      TrackRefs::const_iterator &itrk) const {
  TrackRefs::iterator ip =
      find(pions.inVertexInCalo_.begin(), pions.inVertexInCalo_.end(), *itrk);
  TrackRefs::iterator im =
      find(muons.inVertexInCalo_.begin(), muons.inVertexInCalo_.end(), *itrk);
  TrackRefs::iterator ie =
      find(elecs.inVertexInCalo_.begin(), elecs.inVertexInCalo_.end(), *itrk);
  if (ip == pions.inVertexInCalo_.end() && im == muons.inVertexInCalo_.end() &&
      ie == elecs.inVertexInCalo_.end()) {
    return false;
  } else {
    return true;
  }
...
bool JetPlusTrackCorrector::tracksInCalo(const MatchedTracks &pions,
  ...
  if (!pions.inVertexInCalo_.empty() || !muons.inVertexInCalo_.empty() ||
      !elecs.inVertexInCalo_.empty()) {
    return true;
  } else {
    return false;
  }
...
JetPlusTrackCorrector::pionCorrection(const P4 &jet,
                                      const MatchedTracks &pions) {

  P4 corr_pions;
  /*
    std::cout<<" pionCorrection::Corrections for jet "<<
    theResponseOfChargedWithEff <<" "<< theResponseOfChargedWithoutEff <<" "<<
    theSumPtWithEff <<" "<<
    theSumPtWithoutEff <<" "<<
    theSumEnergyWithEff <<" "<<
    theSumEnergyWithoutEff <<std::endl;
  */
  ...
  if (useInConeTracks_) {
    corr_pions_in_cone =
        pionCorrection(jet, pions.inVertexInCalo_, in_cone, true, true);
  ...
  if (useOutOfConeTracks_) {
    corr_pions_out_of_cone =
        pionCorrection(jet, pions.inVertexOutOfCalo_, out_of_cone, true, false);
  ...
  if (useOutOfVertexTracks_) {
    corr_pions_out_of_vertex =
        pionCorrection(jet, pions.outOfVertexInCalo_, not_used, false, true);
  ...
  if (verbose_) {
    ...
    ss << " Pion corrections:" << std::endl
       << "  In/In      : "
       << "(" << pions.inVertexInCalo_.size() << ") "
       << corr_pions_in_cone.energy() << std::endl
       << "  In/Out     : "
       << "(" << pions.inVertexOutOfCalo_.size() << ") "
       << corr_pions_out_of_cone.energy() << std::endl
       << "  Out/In     : "
       << "(" << pions.outOfVertexInCalo_.size() << ") "
    ...
    if (useEff_) {
      ss << " Pion efficiency corrections:" << std::endl
         ...
         << "  In/Out     : "
         << "(" << pions.inVertexOutOfCalo_.size() << ") "
         << corr_pions_eff_out_of_cone.energy();
    }
    edm::LogVerbatim("JetPlusTrackCorrector") << ss.str();
  }
...
//
JetPlusTrackCorrector::P4
JetPlusTrackCorrector::muonCorrection(const P4 &jet,
                                      const MatchedTracks &muons) {
  ...
  if (useOutOfConeTracks_) {
    corr_muons_out_of_cone =
        muonCorrection(jet, muons.inVertexOutOfCalo_, true, false);
    corr_muons += corr_muons_out_of_cone;
  }

  if (useOutOfVertexTracks_) {
    corr_muons_out_of_vertex =
        muonCorrection(jet, muons.outOfVertexInCalo_, false, true);
  ...
  if (verbose_) {
    ...
    ss << " Muon corrections:" << std::endl
       << "  In/In      : "
       << "(" << muons.inVertexInCalo_.size() << ") "
       << corr_muons_in_cone.energy() << std::endl
       << "  In/Out     : "
       << "(" << muons.inVertexOutOfCalo_.size() << ") "
       << corr_muons_out_of_cone.energy() << std::endl
       << "  Out/In     : "
       << "(" << muons.outOfVertexInCalo_.size() << ") "
       << corr_muons_out_of_vertex.energy();
    edm::LogVerbatim("JetPlusTrackCorrector") << ss.str();
  }
...
//
JetPlusTrackCorrector::P4
JetPlusTrackCorrector::elecCorrection(const P4 &jet,
                                      const MatchedTracks &elecs) const {
  ...
  if (verbose_) {
    ...
    ss << " Electron corrections:" << std::endl
       << "  In/In      : "
       << "(" << elecs.inVertexInCalo_.size() << ") " << null.energy()
       << std::endl
       << "  In/Out     : "
       << "(" << elecs.inVertexOutOfCalo_.size() << ") " << null.energy()
       << std::endl
       << "  Out/In     : "
...
// -----------------------------------------------------------------------------
//
JetPlusTrackCorrector::P4 JetPlusTrackCorrector::jetDirFromTracks(
    const P4 &corrected, const MatchedTracks &pions, const MatchedTracks &muons,
    const MatchedTracks &elecs) const {
...
// -----------------------------------------------------------------------------
//
JetPlusTrackCorrector::P4 JetPlusTrackCorrector::calculateCorr(
    const P4 &jet, const TrackRefs &tracks, jpt::Efficiency &eff,
    bool in_cone_at_vertex, bool in_cone_at_calo_face, double mass,
    bool is_pion, double mip) {

  // Correction to be applied to jet 4-momentum
  P4 correction;

  /*
    std::cout<<" >>>>> Jet "<<jet.pt()<<" "<<jet.eta()<<" "<<jet.phi()<<" Number
    of tracks "<<tracks.size()<<" Pions? "<<is_pion<<" InVert
    "<<in_cone_at_vertex<< " InCalo "<<in_cone_at_calo_face<<std::endl;

    std::cout<<" calculateCorr Start::Corrections for jet "<<
    theResponseOfChargedWithEff <<" "<< theResponseOfChargedWithoutEff <<" "<<
    theSumPtWithEff <<" "<<
    theSumPtWithoutEff <<" "<<
    theSumEnergyWithEff <<" "<<
  ...
  */
  ...
  if (!tracks.empty()) {
    ...
    for (; itrk != jtrk; ++itrk) {
      ...
      if (vectorial_ && vecResponse_) {
        ...
        double energy =
            sqrt((*itrk)->px() * (*itrk)->px() + (*itrk)->py() * (*itrk)->py() +
      ...
      if (is_pion &&
          (ieta == response_.nEtaBins() || ipt == response_.nPtBins())) {
        continue;
      }
      ...
      if (in_cone_at_calo_face) {
        if (vectorial_ && vecResponse_) {
          ...
          if ((*itrk)->extra().isNonnull()) {
            outer_pt = (*itrk)->pt();
            outer_eta =
                (*itrk)->outerPosition().eta(); //@@ outerMomentum().eta()
            outer_phi =
                (*itrk)->outerPosition().phi(); //@@ outerMomentum().phi()
          }
          ...
          if (!is_pion) {
            ...
            outer *=
                (outer.energy() > 0. ? inner.energy() / outer.energy() : 1.);
  ...
  /*
    std::cout<<" calculateCorr End::Corrections for jet "<<
    theResponseOfChargedWithEff <<" "<< theResponseOfChargedWithoutEff <<" "<<
    theSumPtWithEff <<" "<<
    theSumPtWithoutEff <<" "<<
    theSumEnergyWithEff <<" "<<
...
//
JetPlusTrackCorrector::P4
JetPlusTrackCorrector::pionEfficiency(const P4 &jet, const Efficiency &eff,
    ...
    for (uint32_t ipt = 0; ipt < response_.nPtBins() - 1; ++ipt) {
      ...
      for (uint16_t ii = 0; ii < 2; ++ii) {
        ...
        if (vectorial_ && vecResponse_) {
          ...
          corr_p4 = PtEtaPhiM(
              corr_pt, corr_eta, corr_phi,
              pionMass_); //@@ E^2 = p^2 + m_pion^2, |p| calc'ed from pt bin
          corr_p4 *= (corr_p4.energy() > 0.
                          ? corr / corr_p4.energy()
                          : 1.); //@@ p4 scaled up by mean energy for bin
        } else {
  ...
  theSumEnergyWithEff += theSumEnergy;
  /*
        std::cout<<" Efficiency correction End::Corrections for jet "<<
     theResponseOfChargedWithEff <<" "<< theResponseOfChargedWithoutEff <<" "<<
        theSumPtWithEff <<" "<<
        theSumPtWithoutEff <<" "<<
        theSumEnergyWithEff <<" "<<
...
// -----------------------------------------------------------------------------
//
bool JetPlusTrackCorrector::matchMuons(
    TrackRefs::const_iterator &itrk,
  ...
  for (; imuon != jmuon; ++imuon) {
    ...
    if (itrk->id() != imuon->innerTrack().id()) {
      edm::LogError("JetPlusTrackCorrector")
          ...
          << " is different from the product id of the inner track used for "
             "muons "
          << imuon->innerTrack().id() << "!" << std::endl
          << "Cannot compare tracks from different collection. Configuration "
             "Error!";
      return false;
    }
...
// -----------------------------------------------------------------------------
//
bool JetPlusTrackCorrector::matchElectrons(
    TrackRefs::const_iterator &itrk, const edm::Handle<RecoElectrons> &elecs,
...
void JetPlusTrackCorrector::rebuildJta(const reco::Jet &fJet,
  ...
  std::vector<reco::JetBaseRef> theJets =
      reco::JetTracksAssociation::allJets(jtV0);

  TrackRefs tracks = tracksthis;
  tracksthis.clear();

  //  std::cout<<" Size of initial vector "<<tracks.size()<<" "<<fJet.et()<<"
  //  "<<fJet.eta()<<" "<<fJet.phi()<<std::endl;
  ...
  for (TrackRefs::iterator it = tracks.begin(); it != tracks.end(); it++) {
    ...
    for (JetBaseRefIterator ii = theJets.begin(); ii != theJets.end(); ii++) {
      ...
      if (flag == 0) {
        ...
        //(*ii)->et()<<" "<<(*ii)->eta()<<" "<< (*ii)->phi()<<" Track
        //"<<(**it).eta()<<" "<<(**it).phi()<<" "<<scalethis<<" "<<scale<<"
        //"<<flag<<std::endl;
        break;
      }
    }

    // std::cout<<" Track "<<tr<<" "<<flag<<" "<<dR2this<<" "<<dR2check<<" Jet
    // "<<fJet.eta()<<" "<< fJet.phi()<<" Track "<<(**it).eta()<<"
    // "<<(**it).phi()<<std::endl;
    if (flag == 1) {
  ...
  // std::cout<<" The new size of tracks "<<tracksthis.size()<<" Excludede
  // "<<Excl.size()<<std::endl;
  return;
}
...
double JetPlusTrackCorrector::correctAA(
    const reco::Jet &fJet, const reco::TrackRefVector &trBgOutOfVertex,
    double &mConeSize, const reco::TrackRefVector &pioninin,
    const reco::TrackRefVector &pioninout, double ja,
    const reco::TrackRefVector &trBgOutOfCalo) const {
  double mScale = 1.;
  double NewResponse = fJet.energy();

  // std::cout<<"---JetPlusTrackCorrector::correctAA, Jet initial:
  // NewResponse="<<NewResponse <<" et = "<<fJet.et()
  //         <<" pt= "<<fJet.pt()<<" eta="<<fJet.eta()<<" phi="<<fJet.phi() <<"
  //         ja="<<ja
  //        <<" trBgOutOfVertex="<<trBgOutOfVertex.size()<< "
  ...
  for (reco::TrackRefVector::iterator iBgtV = trBgOutOfVertex.begin();
       iBgtV != trBgOutOfVertex.end(); iBgtV++) {
    ...
    double echarBg =
        sqrt((**iBgtV).px() * (**iBgtV).px() + (**iBgtV).py() * (**iBgtV).py() +
             (**iBgtV).pz() * (**iBgtV).pz() + 0.14 * 0.14);

    //	 ResponseOfBackgroundCharged +=
    // echarBg*response_.value(ieta,ipt)/efficiency_.value(ieta,ipt);

    //	 std::cout<<" Efficiency of bg tracks
    //"<<efficiency_.value(ieta,ipt)<<std::endl;
  ...
  for (reco::TrackRefVector::iterator iBgtC = trBgOutOfCalo.begin();
       iBgtC != trBgOutOfCalo.end(); iBgtC++) {
    // Temporary solution>>>>>> Remove tracks with pt>50 GeV
    ...
    double echarBg =
        sqrt((**iBgtC).px() * (**iBgtC).px() + (**iBgtC).py() * (**iBgtC).py() +
  ...
  for (reco::TrackRefVector::const_iterator it = pioninout.begin();
    ...
    //          std::cout<<" Track in out, size= "<<pioninout.size()<<" p=
    //          "<<(*it)->p()<<" pt= "<<(*it)->pt()
    //                   <<" eta=  "<<(*it)->eta()<<" phi=
    //                   "<<(*it)->phi()<<std::endl;
  ...
  for (reco::TrackRefVector::const_iterator it = pioninin.begin();
    ...
    //          std::cout<<" Track in in, size= "<<pioninin.size()<<" p=
    //          "<<(*it)->p()<<" pt= "<<(*it)->pt()
    //                   <<" eta= "<<(*it)->eta()<<" phi=
    //                   "<<(*it)->phi()<<std::endl;
  ...
  //=>
  //    double SquareEtaRingWithoutJets = 4*(M_PI*mConeSize -
  //    mConeSize*mConeSize);
  ...
  EnergyOfBackgroundCharged =
      EnergyOfBackgroundCharged / SquareEtaRingWithoutJets;
  ResponseOfBackgroundCharged =
      ResponseOfBackgroundCharged / SquareEtaRingWithoutJets;
  //    NumberOfBackgroundCharged   =
  //    NumberOfBackgroundCharged/SquareEtaRingWithoutJets;
  ...
  //    NumberOfBackgroundCharged   =
  //    M_PI*mConeSize*mConeSize*NumberOfBackgroundCharged;

  NewResponse =
      NewResponse - EnergyOfBackgroundCharged + ResponseOfBackgroundCharged;

  //      std::cout<<"====JetPlusTrackCorrector, Old
  //      response=fJet.energy()"<<fJet.energy()
...
void Map::print(std::stringstream &ss) const {
  ss << " Number of bins in eta : " << data_.size() << std::endl
     << " Number of bins in pt  : " << (data_.empty() ? 0 : data_[0].size())
     << std::endl;
  ...
  for (; ieta != jeta; ++ieta) {
    ...
    for (; ipt != jpt; ++ipt) {
      ...
      ss << " EtaBinNumber: " << eta_bin << " PtBinNumber: " << pt_bin
         << " EtaValue: " << eta_[eta_bin] << " PtValue: " << pt_[pt_bin]
         << " Value: " << data_[eta_bin][pt_bin] << std::endl;
    }
  }
}
...
MatchedTracks::MatchedTracks()
    : inVertexInCalo_(), outOfVertexInCalo_(), inVertexOutOfCalo_() {
  clear();
}
...
// -----------------------------------------------------------------------------
//
Efficiency::Efficiency(const jpt::Map &response, const jpt::Map &efficiency,
...
double Efficiency::inConeCorr(uint32_t eta_bin, uint32_t pt_bin) const {
  if (check(eta_bin, pt_bin, __func__)) {
    return (outOfConeCorr(eta_bin, pt_bin) * leakage_.value(eta_bin, pt_bin) *
...
double Efficiency::outOfConeCorr(uint32_t eta_bin, uint32_t pt_bin) const {
  if (check(eta_bin, pt_bin, __func__)) {
    ...
    double eff = (1. - efficiency_.value(eta_bin, pt_bin)) /
                 efficiency_.value(eta_bin, pt_bin);
    if (!ntrks) {
      return 0.;
    }
...
// -----------------------------------------------------------------------------
//
bool Efficiency::check(uint32_t eta_bin, uint32_t pt_bin,
