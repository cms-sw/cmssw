...
//  std::cout<<" Size of initial vector "<<tracks.size()<<" "<<fJet.et()<<" "<<fJet.eta()<<" "<<fJet.phi()<<std::endl;
  ...
  for(auto && track : tracks)
	...
	{
	  if(&(*theJet) == &fJet ) {continue;}
	  double dR2 = deltaR2( theJet->eta(), theJet->phi(), trkEta, trkPhi );
	  double scale = dR2;
	  if(jetSplitMerge_ == 0) scale = 1./(*theJet).et();
	  if(jetSplitMerge_ == 2) scale = dR2/(*theJet).et();
      ...
      if(flag == 1) {tracksthis.push_back (track);}else{Excl.push_back (track);}
    }
