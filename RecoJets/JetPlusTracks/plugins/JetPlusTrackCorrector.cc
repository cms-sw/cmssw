...
void JetPlusTrackCorrector::matchTracks( const JetTracks& jet_tracks, 
  ...
  if ( useMuons_ ) {if(usereco_) {getMuons( event, reco_muons );} 
                                else {getMuons( event, pat_muons ); 
  ...
  if ( useElecs_ ) { if(usereco_) {getElectrons( event, reco_elecs, reco_elec_ids );} 
                                       else {getElectrons(event, pat_elecs);
  ...
  if ( !found_reco_muons || !found_reco_elecs ) {
    if(!found_pat_muons || !found_pat_elecs) {edm::LogError("JetPlusTrackCorrector")
      << "[JetPlusTrackCorrector::" << __func__ << "]"
    ...
    // std::cout<<" Track accepted "<<std::endl;
     ...
     // std::cout<<"JetPlusTrackCorrector::matchTracks:jet_tracks.caloFace_.size()="<<jet_tracks.caloFace_.size()
      ...
      bool eleaccept = false;
       if(found_reco_muons) muaccept=matchMuons( itrk, reco_muons ); 
       ...
       if(found_reco_elecs) eleaccept=matchElectrons( itrk, reco_elecs, reco_elec_ids ); else 
               if( found_pat_elecs) {eleaccept=matchElectrons( itrk, pat_elecs);
...
bool JetPlusTrackCorrector::getElectrons( 
                                          const edm::Event& event,
                                          edm::Handle<pat::ElectronCollection>& reco_elecs
                                        ) const {
...
bool JetPlusTrackCorrector::matchMuons( TrackRefs::const_iterator& itrk,
  ...
  for ( ; imuon != jmuon; ++imuon ) {
    if(imuon->innerTrack().isNull() == 1 ) continue;
 ...
 // std::cout<<"JetPlusTrackCorrector::matchElectrons "<<std::endl;
  ...
  for ( ; ielec != jelec; ++ielec ) {
   ...
   //   std::cout<<" matchElectrons:DeltaR "<<deltaRMIN<<std::endl; 
     if ( deltaRMIN < 0.02 ) {/*std::cout<<"Electron is accepted "<<std::endl;*/ return true;}
     else return false;
}
