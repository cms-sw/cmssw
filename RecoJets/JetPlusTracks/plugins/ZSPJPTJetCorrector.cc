...
ZSPJPTJetCorrector::ZSPJPTJetCorrector(const edm::ParameterSet &fConfig) {
  ...
  if (iPU >= 0 || fixedPU > 0) {
    ...
    for (vector<string>::iterator it = theFilesL1Offset.begin();
         it != theFilesL1Offset.end(); it++) {
      std::string file = "CondFormats/JetMETObjects/data/" + (*it) + ".txt";
      edm::FileInPath f2(file);
      mSimpleCorrectorOffset.push_back(
          new SimpleZSPJPTJetCorrector(f2.fullPath()));
    }
  }

  theFilesZSP = fConfig.getParameter<vector<string>>("tagName");
  for (vector<string>::iterator it = theFilesZSP.begin();
       it != theFilesZSP.end(); it++) {
    std::string file = "CondFormats/JetMETObjects/data/" + (*it) + ".txt";
    edm::FileInPath f1(file);
    mSimpleCorrector.push_back(new SimpleZSPJPTJetCorrector(f1.fullPath()));
  }
  //   cout<<" Size of correctors "<<mSimpleCorrector.size()<<"
  //   "<<mSimpleCorrectorOffset.size()<<endl;
}
...
double ZSPJPTJetCorrector::correction(const reco::Jet &fJet,
  ...
  double a = mSimpleCorrector[nPU]->correctionPtEtaPhiE(
      fJet.p4().Pt(), fJet.p4().Eta(), fJet.p4().Phi(), fJet.p4().E());

  if (iPU >= 0) {
    if (!mSimpleCorrectorOffset.empty()) {
      b = mSimpleCorrectorOffset[nPU]->correctionPUEtEtaPhiP(
          fJet.p4().Pt(), fJet.p4().Eta(), fJet.p4().Phi(), fJet.p4().E());
    }
  }
  double c = a * b;
