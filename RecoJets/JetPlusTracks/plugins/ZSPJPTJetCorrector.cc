...
ZSPJPTJetCorrector::ZSPJPTJetCorrector(const edm::ParameterSet& fConfig) {
  ...
  if (iPU >= 0 || fixedPU > 0) {
    ...
    for (auto it = theFilesL1Offset.begin(); it != theFilesL1Offset.end(); it++) {
  ...
  for (auto it = theFilesZSP.begin(); it != theFilesZSP.end(); it++) {
    std::string file = "CondFormats/JetMETObjects/data/" + (*it) + ".txt";
...
double ZSPJPTJetCorrector::correction(const reco::Jet& fJet,
  ...
  double a =
      mSimpleCorrector[nPU]->correctionPtEtaPhiE(fJet.p4().Pt(), fJet.p4().Eta(), fJet.p4().Phi(), fJet.p4().E());
