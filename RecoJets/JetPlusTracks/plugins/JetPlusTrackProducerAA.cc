...
JetPlusTrackProducerAA::JetPlusTrackProducerAA(
  ...
  input_extrapolations_token_ =
      consumes<std::vector<reco::TrackExtrapolation>>(mExtrapolations);
}
...
void JetPlusTrackProducerAA::produce(edm::Event &iEvent,
  ...
  //  std::cout<<"JetPlusTrackProducerAA::produce,
  //  extrapolations_h="<<extrapolations_h->size()<<std::endl;
  //=>
  ...
  for (unsigned i = 0; i < jets_h->size(); ++i) {
    ...
    if (ok) {
      //    std::cout<<" Size of Pion in-in "<<pions.inVertexInCalo_.size()<<"
      //    in-out "<<pions.inVertexOutOfCalo_.size()
      //             <<" out-in "<<pions.outOfVertexInCalo_.size()<<" Oldjet
      //             "<<oldjet->et()<<" factorZSP "<<factorZSP
      //             <<"  "<<corrected.et()<<" scaleJPT "<<scaleJPT<<" after JPT
      //             "<<p4.pt()<<std::endl;
  ...
  reco::TrackRefVector trBgOutOfVertex =
      calculateBGtracksJet(tmpColl, fTracks, extrapolations_h, trBgOutOfCalo);

  //===> Area without Jets
  std::map<reco::JPTJetCollection::iterator, double> AreaNonJet;

  for (reco::JPTJetCollection::iterator ij1 = tmpColl.begin();
       ij1 != tmpColl.end(); ij1++) {
    int nj1 = 1;
    for (reco::JPTJetCollection::iterator ij2 = tmpColl.begin();
         ij2 != tmpColl.end(); ij2++) {
      if (ij2 == ij1)
        continue;
    ...
    //      std::cout<<"+++AreaNonJet[ij1]="<<AreaNonJet[ij1]<<"
    //      nj1="<<nj1<<std::endl;
  }

  //===>

  //  std::cout<<" The size of BG tracks: trBgOutOfVertex=
  //  "<<trBgOutOfVertex.size()
  //           <<" trBgOutOfCalo= "<<trBgOutOfCalo.size()<<std::endl;
  //
  //  std::cout<<" The size of JPT jet collection "<<tmpColl.size()<<std::endl;

  for (reco::JPTJetCollection::iterator ij = tmpColl.begin();
       ij != tmpColl.end(); ij++) {
    ...
    double factorPU =
        mJPTalgo->correctAA(*ij, trBgOutOfVertex, mConeSize, pioninin,
                            pioninout, ja, trBgOutOfCalo);
    ...
    //    std::cout<<" New JPT energy "<<(*ij).et()<<" "<<(*ij).pt()<<"
    //    "<<(*ij).eta()<<" "<<(*ij).phi()<<std::endl;
  }
...
reco::TrackRefVector JetPlusTrackProducerAA::calculateBGtracksJet(
    reco::JPTJetCollection &fJets, std::vector<reco::TrackRef> &fTracks,
    edm::Handle<std::vector<reco::TrackExtrapolation>> &extrapolations_h,
    reco::TrackRefVector &trBgOutOfCalo) {
  ...
  for (unsigned t = 0; t < fTracks.size(); ++t) {
    ...
    // {
    // cout<<"BG, BAD trackQuality, ptBgV="<<fTracks[t]->pt()<<" etaBgV =
    // "<<fTracks[t]->eta()<<" phiBgV = "<<fTracks[t]->phi()<<endl; continue;
    // }
    ...
    //  std::cout<<"++++++++++++++++>  track="<<t<<" trackEta="<<trackEta<<"
    //  trackPhi="<<trackPhi
    ...
    for (unsigned j = 0; j < fJets.size(); ++j) {
      ...
      //  <<" jetE="<<jet->energy()<<" jetEta="<<jetEta<<"
      //  jetPhi="<<jetPhi<<std::endl;
      ...
      if (fabs(jetEta - trackEta) < mConeSize) {
        ...
        if (dphiTrackJet < mConeSize) {
          track_bg = 1;
          //    std::cout<<"===>>>> Track inside jet at vertex, track_bg="<<
          //    track_bg <<" track="<<t<<" jet="<<j
    ...
    if (track_bg == 0) {
      ...
      //       std::cout<<"------Track outside jet at vertex, track_bg="<<
      //       track_bg<<" track="<<t
      //               <<" trackEta="<<trackEta<<" trackPhi="<<trackPhi
      //               <<std::endl;
    }
  ...
  int nValid = 0;
  for (std::vector<reco::TrackExtrapolation>::const_iterator
           xtrpBegin = extrapolations_h->begin(),
           xtrpEnd = extrapolations_h->end(), ixtrp = xtrpBegin;
       ixtrp != xtrpEnd; ++ixtrp) {

    //    std::cout<<"JetPlusTrackProducerAA::calculateBGtracksJet: initial
    //    track pt= "<<ixtrp->track()->pt()
    //             <<" eta= "<<ixtrp->track()->eta()<<"
    //             phi="<<ixtrp->track()->phi()
    ...
    reco::TrackRefVector::iterator it =
        find(trBgOutOfVertex.begin(), trBgOutOfVertex.end(), (*ixtrp).track());

    if (it != trBgOutOfVertex.end()) {
      trBgOutOfCalo.push_back(*it);

      //          std::cout<<"+++trBgOutOfCalo, pt= "<<ixtrp->track()->pt()<<"
