...
public:
  Efficiency(const jpt::Map &response, const jpt::Map &efficiency,
...
private:
  ...
  bool check(uint32_t eta_bin, uint32_t pt_bin,
             std::string name = "check") const;
...
/// Particles matched to tracks that are in/in, in/out, out/in at Vertex and
/// CaloFace
class MatchedTracks {
public:
  MatchedTracks();
...
//  void set( double a, double b) const
//  {theResponseOfChargedWithEff=a;theResponseOfChargedFull=b;} double
//  theResponseOfChargedWithEff; double theResponseOfChargedFull;
//};
...
public:
  /// Constructor
  JetPlusTrackCorrector(const edm::ParameterSet &iPS,
  ...
  double correction(const reco::Jet &, const reco::Jet &, const edm::Event &,
                    const edm::EventSetup &, P4 &, jpt::MatchedTracks &pions,
                    jpt::MatchedTracks &muons, jpt::MatchedTracks &elecs,
                    bool &validMatches);

  /// Scalar correction method
  double correction(const reco::Jet &, const reco::Jet &, const edm::Event &,
                    const edm::EventSetup &, jpt::MatchedTracks &pions,
                    jpt::MatchedTracks &muons, jpt::MatchedTracks &elecs,
                    bool &validMatches);
  ...
  //  double correctAA(  const reco::Jet&, const reco::TrackRefVector&, double&,
  //  const reco::TrackRefVector&,const reco::TrackRefVector&, double&) const;
  double correctAA(const reco::Jet &, const reco::TrackRefVector &, double &,
                   const reco::TrackRefVector &, const reco::TrackRefVector &,
                   double, const reco::TrackRefVector &) const;
  ...
  bool matchTracks(const reco::Jet &, const edm::Event &,
                   const edm::EventSetup &, jpt::MatchedTracks &pions,
                   jpt::MatchedTracks &muons, jpt::MatchedTracks &elecs);
...
protected:
  ...
  virtual bool jetTrackAssociation(const reco::Jet &, const edm::Event &,
                                   const edm::EventSetup &,
                                   jpt::JetTracks &) const;

  /// JTA using collections from event
  bool jtaUsingEventData(const reco::Jet &, const edm::Event &,
                         jpt::JetTracks &) const;

  /// Matches tracks to different particle types (overriden in derived class)
  virtual void matchTracks(const jpt::JetTracks &, const edm::Event &,
                           jpt::MatchedTracks &pions, jpt::MatchedTracks &muons,
                           jpt::MatchedTracks &elecs);

  /// Calculates individual pion corrections
  P4 pionCorrection(const P4 &jet, const TrackRefs &pions, jpt::Efficiency &,
                    bool in_cone_at_vertex, bool in_cone_at_calo_face);

  /// Calculates individual muons corrections
  P4 muonCorrection(const P4 &jet, const TrackRefs &muons,
                    bool in_cone_at_vertex, bool in_cone_at_calo_face);

  /// Calculates individual electron corrections
  P4 elecCorrection(const P4 &jet, const TrackRefs &elecs,
                    bool in_cone_at_vertex, bool in_cone_at_calo_face);

  /// Calculates vectorial correction using total track 3-momentum
  P4 jetDirFromTracks(const P4 &jet, const jpt::MatchedTracks &pions,
  ...
  P4 calculateCorr(const P4 &jet, const TrackRefs &, jpt::Efficiency &,
                   bool in_cone_at_vertex, bool in_cone_at_calo_face,
                   double mass, bool is_pion, double mip);

  /// Correction to be applied using tracking efficiency
  P4 pionEfficiency(const P4 &jet, const jpt::Efficiency &,
  ...
  /// Get RECO electrons
  bool getElectrons(const edm::Event &, edm::Handle<RecoElectrons> &,
  ...
  bool findTrack(const jpt::JetTracks &, TrackRefs::const_iterator &,
  ...
  void rebuildJta(const reco::Jet &, const JetTracksAssociations &,
                  TrackRefs &included, TrackRefs &excluded) const;

  /// Exclude jet-track association
  void excludeJta(const reco::Jet &, const JetTracksAssociations &,
                  TrackRefs &included, const TrackRefs &excluded) const;
...
inline double JetPlusTrackCorrector::correction(
    const reco::Jet &fJet, const reco::Jet &fJetcalo, const edm::Event &event,
    const edm::EventSetup &setup, jpt::MatchedTracks &pions,
    jpt::MatchedTracks &muons, jpt::MatchedTracks &elecs, bool &validMatches) {
  P4 not_used_for_scalar_correction;
  return correction(fJet, fJetcalo, event, setup,
                    not_used_for_scalar_correction, pions, muons, elecs,
                    validMatches);
}

inline bool JetPlusTrackCorrector::eventRequired() const { return true; }
inline bool JetPlusTrackCorrector::vectorialCorrection() const {
  return vectorial_;
}
inline bool JetPlusTrackCorrector::canCorrect(const reco::Jet &jet) const {
  return (fabs(jet.eta()) <= maxEta_);
}

inline JetPlusTrackCorrector::P4 JetPlusTrackCorrector::pionCorrection(
    const P4 &jet, const TrackRefs &pions, jpt::Efficiency &eff,
    bool in_cone_at_vertex, bool in_cone_at_calo_face) {
  return calculateCorr(jet, pions, eff, in_cone_at_vertex, in_cone_at_calo_face,
                       pionMass_, true, -1.);
}

inline JetPlusTrackCorrector::P4
JetPlusTrackCorrector::muonCorrection(const P4 &jet, const TrackRefs &muons,
                                      bool in_cone_at_vertex,
                                      bool in_cone_at_calo_face) {
  return calculateCorr(jet, muons, not_used, in_cone_at_vertex,
                       in_cone_at_calo_face, muonMass_, false, 2.);
}

inline JetPlusTrackCorrector::P4
JetPlusTrackCorrector::elecCorrection(const P4 &jet, const TrackRefs &elecs,
                                      bool in_cone_at_vertex,
                                      bool in_cone_at_calo_face) {
  return calculateCorr(jet, elecs, not_used, in_cone_at_vertex,
                       in_cone_at_calo_face, elecMass_, false, 0.);
}

inline double JetPlusTrackCorrector::checkScale(const P4 &jet,
                                                P4 &corrected) const {
...
inline const jpt::Map &JetPlusTrackCorrector::responseMap() const {
  return response_;
}
inline const jpt::Map &JetPlusTrackCorrector::efficiencyMap() const {
  return efficiency_;
}
inline const jpt::Map &JetPlusTrackCorrector::leakageMap() const {
  return leakage_;
}
