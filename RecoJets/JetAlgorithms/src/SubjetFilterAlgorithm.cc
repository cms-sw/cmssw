				...
				const edm::EventSetup& iSetup)
{
  ...
  unsigned nFat = (int)std::min((int)fjFatJets.size(),2);
  for (unsigned iFat=0;iFat<nFat;iFat++) {
    ...
    fjFilterJets = fastjet::sorted_by_pt(cs.exclusive_subjets(fjFatJet,dcut));
    ...
    fjSubJets.insert(fjSubJets.end(),fjFilterJets.begin(),fjFilterJets.end());
