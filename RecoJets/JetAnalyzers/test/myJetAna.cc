...
void myJetAna::analyze(const edm::Event &evt, const edm::EventSetup &es) {
  ...
  ***/
  ...
  if ((evt.bunchCrossing() == 51) || (evt.bunchCrossing() == 151) || (evt.bunchCrossing() == 2596) ||
      (evt.bunchCrossing() == 2724) || (evt.bunchCrossing() == 2824) || (evt.bunchCrossing() == 3487)) {
  ...
  if (evt.id().run() == 1) {
    ...
    if ((Pass_BunchCrossing) && (Pass_HFTime) && (Pass_Vertex)) {
      Pass = true;
    } else {
      Pass = false;
    }
  ...
  ***/
    ...
    if ((nHPD == 1) && (nTowers > 6)) {
      evtType = 2;
      edm::LogInfo("category") << " nHPD = " << nHPD << " Towers = " << nTowers << " Type = " << evtType;
    }
    ...
    for (i = colls.begin(); i != colls.end(); i++) {
      for (HBHERecHitCollection::const_iterator j = (*i)->begin(); j != (*i)->end(); j++) {
        ...
        if (j->id().subdet() == HcalBarrel) {
          ...
          HBTime->Fill(j->time());
          if (!Pass_NoiseSummary)
            HBTimeFlagged2->Fill(j->time());
          if (j->flagField(HcalCaloFlagLabels::HBHETimingShapedCutsBits) != 0)
            HBTimeFlagged->Fill(j->time());
          HBTvsE->Fill(j->energy(), j->time());

          if (j->time() > 20.)
            HBEneTThr->Fill(j->energy());

          if ((j->time() < -25.) || (j->time() > 75.)) {
            HBEneOOT->Fill(j->energy());
            if (j->energy() > HBHEThreshold)
              HBEneOOTTh->Fill(j->energy());
            if (j->energy() > HBHEThreshold1)
              HBEneOOTTh1->Fill(j->energy());
          }
          if (j->energy() > HBHEThreshold) {
            HBEneTh->Fill(j->energy());
            HBTimeTh->Fill(j->time());
            if (!Pass_NoiseSummary)
              HBTimeThFlagged2->Fill(j->time());
            if (j->flagField(HcalCaloFlagLabels::HBHETimingShapedCutsBits) != 0)
              HBTimeThFlagged->Fill(j->time());

            if (evt.id().run() >= StableRun)
              HBTimeThR->Fill(j->time());
            HBTotalE += j->energy();
          ...
          if (j->energy() > HBHEThreshold1) {
            ...
            if (!Pass_NoiseSummary)
              HBTimeTh1Flagged2->Fill(j->time());
            if (j->flagField(HcalCaloFlagLabels::HBHETimingShapedCutsBits) != 0)
              HBTimeTh1Flagged->Fill(j->time());

            if (evt.id().run() >= StableRun)
              HBTimeTh1R->Fill(j->time());
            if ((j->time() < -25.) || (j->time() > 75.)) {
          ...
          if (j->energy() > HBHEThreshold2) {
            ...
            if (!Pass_NoiseSummary)
              HBTimeTh2Flagged2->Fill(j->time());
            if (j->flagField(HcalCaloFlagLabels::HBHETimingShapedCutsBits) != 0)
              HBTimeTh2Flagged->Fill(j->time());

            if (evt.id().run() >= StableRun)
              HBTimeTh2R->Fill(j->time());
          }
          if (j->energy() > HBHEThreshold3) {
            HBTimeTh3->Fill(j->time());
            if (evt.id().run() >= StableRun)
              HBTimeTh3R->Fill(j->time());
          }
          if ((evt.id().run() == 120020) && (evt.id().event() == 453)) {
        ...
        if (j->id().subdet() == HcalEndcap) {
          ...
          HETime->Fill(j->time());
          if (!Pass_NoiseSummary)
            HETimeFlagged2->Fill(j->time());
          if (j->flagField(HcalCaloFlagLabels::HBHETimingShapedCutsBits) != 0)
            HETimeFlagged->Fill(j->time());
          HETvsE->Fill(j->energy(), j->time());

          if (j->time() > 20.)
            HEEneTThr->Fill(j->energy());

          if ((j->time() < -25.) || (j->time() > 75.)) {
            HEEneOOT->Fill(j->energy());
            if (j->energy() > HBHEThreshold)
              HEEneOOTTh->Fill(j->energy());
            if (j->energy() > HBHEThreshold1)
              HEEneOOTTh1->Fill(j->energy());
          }
          ...
          if (j->energy() > HBHEThreshold) {
            ...
            if (!Pass_NoiseSummary)
              HETimeThFlagged2->Fill(j->time());
            if (j->flagField(HcalCaloFlagLabels::HBHETimingShapedCutsBits) != 0)
              HETimeThFlagged->Fill(j->time());

            if (evt.id().run() >= StableRun)
              HETimeThR->Fill(j->time());
            HETotalE += j->energy();
          ...
          if (j->energy() > HBHEThreshold1) {
            ...
            if (!Pass_NoiseSummary)
              HETimeTh1Flagged2->Fill(j->time());
            if (j->flagField(HcalCaloFlagLabels::HBHETimingShapedCutsBits) != 0)
              HETimeTh1Flagged->Fill(j->time());
            if (evt.id().run() >= StableRun)
              HETimeTh1R->Fill(j->time());
            if ((j->time() < -25.) || (j->time() > 75.)) {
          ...
          if (j->energy() > HBHEThreshold2) {
            ...
            if (!Pass_NoiseSummary)
              HETimeTh2Flagged2->Fill(j->time());
            if (j->flagField(HcalCaloFlagLabels::HBHETimingShapedCutsBits) != 0)
              HETimeTh2Flagged->Fill(j->time());
            if (evt.id().run() >= StableRun)
              HETimeTh2R->Fill(j->time());
          }
          if (j->energy() > HBHEThreshold3) {
            HETimeTh3->Fill(j->time());
            if (evt.id().run() >= StableRun)
              HETimeTh3R->Fill(j->time());
          }
        ...
        if (j->id().subdet() == HcalForward) {
          if (NPMTHits == 1) {
            if ((j->flagField(HcalCaloFlagLabels::HFLongShort) == 1) ||
              ...
              if (j->id().depth() == 1)
                HFEtaFlaggedL->Fill(j->id().ieta());
              if (j->id().depth() == 2)
                HFEtaFlaggedS->Fill(j->id().ieta());
            } else {
          ...
          if (j->time() > 20.)
            HFEneTThr->Fill(j->energy());

          if (j->energy() > 10.)
            HFTvsEThr->Fill(j->energy(), j->time());
          ...
          if (j->flagField(HcalCaloFlagLabels::HFLongShort) == 1) {
            ...
            if (j->energy() > 10.)
              HFTvsEFlagged2Thr->Fill(j->energy(), j->time());
          }
          ...
          if (j->energy() > HFThreshold) {
            ...
            if (j->flagField(HcalCaloFlagLabels::HFLongShort) == 1)
              HFTimeThFlagged2->Fill(j->time());
            if (j->flagField(HcalCaloFlagLabels::HFDigiTime) == 1)
              HFTimeThFlagged3->Fill(j->time());

            if (evt.id().run() >= StableRun)
              HFTimeThR->Fill(j->time());
            if ((j->flagField(HcalCaloFlagLabels::HFLongShort) == 1) ||
                (j->flagField(HcalCaloFlagLabels::HFDigiTime) == 1)) {
              HFTimeThFlagged->Fill(j->time());

              if (j->energy() > HFThreshold2)
                HFTimeTh2Flagged->Fill(j->time());
              if (j->energy() > HFThreshold3)
                HFTimeTh3Flagged->Fill(j->time());

              if (evt.id().run() >= StableRun) {
                HFTimeThFlaggedR->Fill(j->time());
                if (NPMTHits == 1)
                  HFTimeThFlaggedR1->Fill(j->time());
                if (NPMTHits == 2)
                  HFTimeThFlaggedR2->Fill(j->time());
                if (NPMTHits == 3)
                  HFTimeThFlaggedR3->Fill(j->time());
                if (NPMTHits == 4)
                  HFTimeThFlaggedR4->Fill(j->time());
                if (NPMTHits > 1)
                  HFTimeThFlaggedRM->Fill(j->time());
              }
            }
            HFTotalE += j->energy();
          ...
          if (j->energy() > HFThreshold1) {
            ...
            if (j->flagField(HcalCaloFlagLabels::HFLongShort) == 1)
              HFTimeTh1Flagged2->Fill(j->time());
            if (j->flagField(HcalCaloFlagLabels::HFDigiTime) == 1)
              HFTimeTh1Flagged3->Fill(j->time());
            if (evt.id().run() >= StableRun)
              HFTimeTh1R->Fill(j->time());
          ...
          if (j->energy() > HFThreshold2) {
            ...
            if (j->flagField(HcalCaloFlagLabels::HFLongShort) == 1)
              HFTimeTh2Flagged2->Fill(j->time());
            if (j->flagField(HcalCaloFlagLabels::HFDigiTime) == 1)
              HFTimeTh2Flagged3->Fill(j->time());
            if (evt.id().run() >= StableRun)
              HFTimeTh2R->Fill(j->time());
          }
          if (j->energy() > HFThreshold3) {
            HFTimeTh3->Fill(j->time());
            if (j->flagField(HcalCaloFlagLabels::HFLongShort) == 1)
              HFTimeTh3Flagged2->Fill(j->time());
            if (j->flagField(HcalCaloFlagLabels::HFDigiTime) == 1)
              HFTimeTh3Flagged3->Fill(j->time());
            if (evt.id().run() >= StableRun)
              HFTimeTh3R->Fill(j->time());
          }
    ...
    for (int ieta = 0; ieta < 100; ieta++) {
       ...
       ***/
        ...
        if ((HFRecHit[ieta][iphi][1] == -10.) && (HFRecHit[ieta][iphi][0] != -10.)) {
          ...
          if (HFRecHitFlag[ieta][iphi][0] != 0)
            HFLEneNoSFlagged->Fill(HFRecHit[ieta][iphi][0]);
        }
        if ((HFRecHit[ieta][iphi][0] == -10.) && (HFRecHit[ieta][iphi][1] != -10.)) {
          HFSEneNoL->Fill(HFRecHit[ieta][iphi][1]);
          if (HFRecHitFlag[ieta][iphi][1] != 0)
            HFSEneNoLFlagged->Fill(HFRecHit[ieta][iphi][1]);
        }
      }
    }
    ...
    for (iHORecHit = collsHORecHit.begin(); iHORecHit != collsHORecHit.end(); iHORecHit++) {
      for (HORecHitCollection::const_iterator j = (*iHORecHit)->begin(); j != (*iHORecHit)->end(); j++) {
        if (j->id().subdet() == HcalOuter) {
          ...
          if (((j->id().iphi() >= 59 && j->id().iphi() <= 70 && j->id().ieta() >= 11 && j->id().ieta() <= 15) ||
               (j->id().iphi() >= 47 && j->id().iphi() <= 58 && j->id().ieta() >= 5 && j->id().ieta() <= 10))) {
            HOSEne->Fill(j->energy());
            if (j->energy() > HOThreshold)
              HOSTime->Fill(j->time());
          } else if ((j->id().iphi() < 59 || j->id().iphi() > 70 || j->id().ieta() < 11 || j->id().ieta() > 15) &&
                     (j->id().iphi() < 47 || j->id().iphi() > 58 || j->id().ieta() < 5 || j->id().ieta() > 10)) {
  ...
  ******/
    ...
    if ((EBTotalE > 320000) && (EBTotalE < 330000) && (HBTotalE > 2700000) && (HBTotalE < 2800000)) {
  ...
  *****/
    ...
    if (NPMTHits == 0)
      TrkMultFlagged0->Fill(tC.size());
    if (NPMTHits == 1)
      TrkMultFlagged1->Fill(tC.size());
    if (NPMTHits == 2)
      TrkMultFlagged2->Fill(tC.size());
    if (NPMTHits == 3)
      TrkMultFlagged3->Fill(tC.size());
    if (NPMTHits == 4)
      TrkMultFlagged4->Fill(tC.size());
    if (NPMTHits > 1)
      TrkMultFlaggedM->Fill(tC.size());
...
void myJetAna::endJob() {
  for (int i = 0; i < 4000; i++) {
    if (((float)nBNC[i] / (float)totBNC) > 0.05) {
      edm::LogInfo("myJetAna") << "+++ " << i << " " << ((float)nBNC[i] / (float)totBNC) << " " << nBNC[i] << " "
                               << totBNC;
    }
  }
}
