...
public:
  ...
  public:
    ...
    Arrow(Float_t x, Float_t y, Float_t z, Float_t xo, Float_t yo, Float_t zo = 0)
        : TEveArrow(x, y, z, xo, yo, zo), m_et(0), m_energy(0), m_vc(nullptr) {}
...
// member functions
//
void FWMET3DProxyBuilder::scaleProduct(TEveElementList* parent, FWViewType::EType type, const FWViewContext* vc) {
  // printf("scale prod \n");
  FWViewEnergyScale* caloScale = vc->getEnergyScale();

  for (auto& m_arrow : m_arrows) {
    if (vc == m_arrow->m_vc) {
      m_arrow->setScale(caloScale);
    }
  }
}

void FWMET3DProxyBuilder::build(const reco::MET& met,
                                unsigned int iIndex,
  ...
  Arrow* arrow = new Arrow(sin(theta) * cos(phi),
                           sin(theta) * sin(phi),
                           cos(theta),
