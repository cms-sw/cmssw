...
void FWPFTauProxyBuilder::buildViewType(const FWEventItem* iItem,
  ...
  for (const auto& pfTau : *pfTaus) {
    ...
    if (viewType == FWViewType::kLego) {
      ...
      try {
        const auto* tauTagInfo = dynamic_cast<const reco::PFTauTagInfo*>(pfTau.pfTauTagInfoRef().get());
...
REGISTER_FWPROXYBUILDER(FWPFTauProxyBuilder,
                        reco::PFTauCollection,
                        "PFTau",
                        FWViewType::kAll3DBits | FWViewType::kAllRPZBits | FWViewType::kAllLegoBits);
