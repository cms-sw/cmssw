...
FWECALCaloDataDetailViewBuilder::showSuperClusters( Color_t color1, Color_t color2 )
{
   ...
   if( collection.isValid() )
   {
      ...
      for(auto & i : sorted)
      {
	 if( !(fabs(i.eta() - m_eta) < (m_size*0.0172)
	       && fabs(i.phi() - m_phi) < (m_size*0.0172)) )
	   continue;
      ...
      if( hit.id().subdetId() == EcalBarrel || xyEE == false )
      {
         ...
         if( points != 0 )
         {
            ...
            for( unsigned int i = 0; i < 8; ++i )
            {
               ...
               if ( ((hit.id().subdetId() == EcalBarrel)  && (crystal.Perp() > 135) )||  ((hit.id().subdetId() == EcalEndcap) && (crystal.Perp() > 155))) continue;
