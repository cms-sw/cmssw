...
FWTabularWidget::FWTabularWidget(FWTableManagerBase* iTable, const TGWindow* p, GContext_t context)
  ...
  for (auto it = m_widthOfTextInColumns.begin(), itEnd = m_widthOfTextInColumns.end(); it != itEnd; ++it) {
    m_tableWidth += *it;
  }
  Resize();
...
void FWTabularWidget::DoRedraw() {
  ...
  x += kSeperatorWidth;
  for (auto itTextWidth = m_widthOfTextInColumns.begin(); itTextWidth != m_widthOfTextInColumns.end(); ++itTextWidth) {
