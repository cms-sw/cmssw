...
void FWTrackHitsDetailView::build(const FWModelId& id, const reco::Track* track) {
  {
  ...
  for (TEveElement::List_i i = m_modules->BeginChildren(), end = m_modules->EndChildren(); i != end; ++i) {
    ...
    if (gs == nullptr && (*i != nullptr)) {
      std::cerr << "Got a " << typeid(rhs).name() << ", expecting TEveGeoShape. ignoring (it must be the clusters)."
                << std::endl;
      continue;
    }
...
void FWTrackHitsDetailView::setTextInfo(const FWModelId& id, const reco::Track* track) {
  m_infoCanvas->cd();
...
void FWTrackHitsDetailView::addModules(const reco::Track& track,
  ...
  for (trackingRecHit_iterator recIt = track.recHitsBegin(), recItEnd = track.recHitsEnd(); recIt != recItEnd;
    ...
    if (detid.rawId() != 0) {
      ...
      if (!ids.insert(detid.rawId()).second)
        continue;
      if (iItem->getGeom()) {
