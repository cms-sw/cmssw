...
TGeoMgrFromDdd::ReturnType TGeoMgrFromDdd::produce(const DisplayGeomRecord& iRecord) {
  ...
  edm::LogWarning("TGeoMgrFromDdd") << "about to initialize the DDCompactView walker with a root node " << viewH->root();
  ...
  do {
    ...
    if (m_verbose) {
      ...
      edm::LogVerbatim("TGeoMgrFromDdd") << info.first.name() << " " << info.second->copyno() << " " << DDSolidShapesName::name(info.first.solid().shape());
    }
