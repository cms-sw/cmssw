...
void FWCSCStripDigiProxyBuilder::build(const FWEventItem* iItem, TEveElementList* product, const FWViewContext*) {
  ...
  for (CSCStripDigiCollection::DigiRangeIterator dri = digis->begin(), driEnd = digis->end(); dri != driEnd; ++dri) {
    ...
    for (CSCStripDigiCollection::const_iterator dit = range.first; dit != range.second; ++dit) {
      ...
      if (std::find_if(adcCounts.begin(), adcCounts.end(), [&](auto c) { return c > signalThreshold; }) !=
        ...
        float localPointTop[3] = {yAxisOrientation * (yOrigin + length) * tanStripAngle, length, 0.0};

        float localPointBottom[3] = {yAxisOrientation * (yOrigin - length) * tanStripAngle, -length, 0.0};
        ...
        stripDigiSet->AddLine(globalPointBottom[0],
                              globalPointBottom[1],
                              globalPointBottom[2],
                              globalPointTop[0],
                              globalPointTop[1],
...
REGISTER_FWPROXYBUILDER(FWCSCStripDigiProxyBuilder,
