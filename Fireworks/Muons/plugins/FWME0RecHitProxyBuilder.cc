...
void
FWME0RecHitProxyBuilder::buildViewType(const ME0RecHit& iData,
                                       unsigned int iIndex, 
                                       TEveElement& oItemHolder, 
                                       FWViewType::EType type,
                                       const FWViewContext*)
{
  ...
  {
    fwLog( fwlog::kError ) << "failed to get geometry of ME0 roll with detid: " 
			   << rawid <<std::endl;
    return;
  }
...
REGISTER_FWPROXYBUILDER( FWME0RecHitProxyBuilder, ME0RecHit, "ME0 RecHits", 
