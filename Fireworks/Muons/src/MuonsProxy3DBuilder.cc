				...
				TEveElementList** product)
{
   ...
   for ( reco::MuonCollection::const_iterator muon = muons->begin(); muon != muons->end(); ++muon, ++index )
	...
	if ( useStandAloneFit )
	  {
	     innerPropagator->SetMaxR( muon->standAloneMuon()->innerPosition().Rho() );
	     innerPropagator->SetMaxZ( fabs(muon->standAloneMuon()->innerPosition().z()) );
	...
	if ( muon->isTrackerMuon() && ! useStandAloneFit ) {
	   ...
	   // add muon segments
	   addMatchInformation( &(*muon), m_item->getGeom(), outerTrack, muonList );
	   outerTrack->MakeTrack();
	   muonList->AddElement( outerTrack );
	}
...
void MuonsProxy3DBuilder::addMatchInformation( const reco::Muon* muon,
