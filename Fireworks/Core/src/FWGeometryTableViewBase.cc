  ...
  if (event->fType == kButtonPress) {
    ...
    if (map) {
      ...
      gVirtualX->TranslateCoordinates(GetId(),
                                      ...
                                      event->fX,
                                      event->fY,  //0,0 in local coordinates
                                      ax,
  ...
  public:
    FWGeometryVF(const TGWindow* p, FWGeometryTableViewBase* tv) : TGVerticalFrame(p), m_tv(tv) {
      ...
      gVirtualX->SelectInput(GetId(),
                             kKeyPressMask | kKeyReleaseMask | kExposureMask | kPointerMotionMask |
...
void FWGeometryTableViewBase::checkExpandLevel() {
  ...
  for (auto i = getTableManager()->refEntries().begin(); i != getTableManager()->refEntries().end(); ++i) {
    if (i->m_level < ae)
  ...
  if (m_viewersConfig) {
    ...
    if (nullptr != keyVals) {
      for (auto it = keyVals->begin(); it != keyVals->end(); ++it) {
        ...
        if (m_marker)
          getMarkerScene(v)->AddElement(m_marker);
...
void FWGeometryTableViewBase::selectView(int idx) {
  ...
  for (auto eit = v->BeginChildren(); eit != v->EndChildren(); ++eit) {
    if ((((TEveSceneInfo*)(*eit))->GetScene()) == m_eveScene) {
      v->RemoveElement(*eit);
      if (m_marker)
        getMarkerScene(v)->RemoveElement(m_marker);
      gEve->Redraw3D();
      return;
    }
  }
...
void FWGeometryTableViewBase::cellClicked(Int_t iRow, Int_t iColumn, Int_t iButton, Int_t iKeyMod, Int_t x, Int_t y) {
  int idx = getTableManager()->rowToIndex()[iRow];
  FWGeometryTableManagerBase::NodeInfo& ni = getTableManager()->refEntries()[idx];

  if (iColumn != 2)
    getTableManager()->cancelEditor(false);
  ...
  if (iButton == kButton1) {
    if (iColumn == 0) {
      ...
      gVirtualX->TranslateCoordinates(
          gClient->GetDefaultRoot()->GetId(), m_tableWidget->GetId(), x, y, xLoc, yLoc, wdummy);
  ...
  if (gv) {
    switch (menuIdx) {
      ...
      case FWGeoTopNode::kCamera: {
        ...
        v->CurrentCamera().SetCenterVec(FWGeoTopNode::s_pickedCamera3DCenter.X(),
                                        FWGeoTopNode::s_pickedCamera3DCenter.Y(),
                                        FWGeoTopNode::s_pickedCamera3DCenter.Z());
...
void FWGeometryTableViewBase::setTopNodePathFromConfig(const FWConfiguration& iFrom) {
  ...
  if (tn >= lastIdx) {
    fwLog(fwlog::kWarning) << Form(
        "Ignoring node path from confugration file -- %s value larger than number of nodes \n",
