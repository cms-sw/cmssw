...
FWGUIValidatingTextEntry::FWGUIValidatingTextEntry(const TGWindow* parent, const char* text, Int_t id)
    : TGTextEntry(parent, text, id), m_popup(nullptr), m_list(nullptr), m_validator(nullptr), m_listHeight(100) {
...
// member functions
//
void FWGUIValidatingTextEntry::setValidator(FWValidatorBase* iValidator) { m_validator = iValidator; }

Bool_t FWGUIValidatingTextEntry::ProcessMessage(Long_t msg, Long_t parm1, Long_t parm2) {
...
void FWGUIValidatingTextEntry::keyPressedInPopup(TGFrame* f, UInt_t keysym, UInt_t mask) {
  switch (keysym) {
    case kKey_Tab:
    case kKey_Escape:
      RequestFocus();
...
namespace {
  ...
  public:
    ChangeFocusTimer(TGWindow* iWindow) : TTimer(100), m_window(iWindow) {}
...
void FWGUIValidatingTextEntry::showOptions() {
  if (nullptr != m_validator) {
    ...
    {
      unsigned int h = m_list->GetNumberOfEntries() * m_list->GetItemVsize();
      if (h && (h < m_listHeight)) {
    ...
    int ax, ay;
    Window_t wdummy;
    gVirtualX->TranslateCoordinates(GetId(), m_popup->GetParent()->GetId(), 0, GetHeight(), ax, ay, wdummy);
    ...
    m_popup->PlacePopup(ax, ay, GetWidth() - 2, m_popup->GetDefaultHeight());
  }
}
