...
// $Id: MuonDetailView.cc,v 1.9 2008/12/05 01:49:20 chrjones Exp $
//
...
//
MuonDetailView::MuonDetailView()
{

}

// MuonDetailView::MuonDetailView(const MuonDetailView& rhs)
// {
...
void MuonDetailView::build (TEveElementList **product, const FWModelId &id)
{
  ...
  if(0 == tList) {
    // First mod: replacing string here
    //    tList =  new TEveElementList(m_item->name().c_str(),"Supercluster RhoZ",true);
    tList =  new TEveElementList(m_item->name().c_str(),"trackerMuons",true);
    *product = tList;
