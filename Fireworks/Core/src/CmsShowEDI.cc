...
CmsShowEDI::CmsShowEDI(const TGWindow* p, UInt_t w, UInt_t h, FWSelectionManager* selMgr, FWColorManager* colorMgr)
  ...
  builder.indent(0)
      ...
      .indent(3)
      .addLabel("Color", 8)
      .addColorPicker(colorMgr, &m_colorSelectWidget)
      .expand(false)
      .addHSeparator()
      .addLabel("Opacity", 8)
      ...
      .addTextButton("To back", &m_backButton)
      .floatLeft()
      .expand(false)
      .addNumberEntry(
          0.0, 4, TGNumberFormat::kNESInteger, FWEventItem::minLayerValue(), FWEventItem::maxLayerValue(), &m_layerEntry)
      .expand(false)
      .floatLeft()
      .addTextButton("To front", &m_frontButton)
      .expand(false)
      .vSpacer()
      .addHSeparator()
      .endTab()
      ...
      .addValidatingTextEntry(nullptr, &m_filterExpressionEntry)
      .floatLeft()
      .addTextButton("Filter", &m_filterButton)
      .expand(false)
      ...
      .addTextButton("Select", &m_selectButton)
      .floatLeft()
      .expand(false)
      .addTextButton("Select all", &m_selectAllButton)
      .expand(false)
      .floatLeft()
      .addTextButton("Unselect all", &m_deselectAllButton)
      .expand(false)
      .indent(3)
      .addLabel("Color Selection", 8)
      .addColorPicker(colorMgr, &m_cw)
      .expand(false)
      ...
      .addHSeparator()
      .addTextButton("Remove collection", &m_removeButton)
      .expand(false)
      .vSpacer()
      .endTab()
      .untabs();
...
void CmsShowEDI::updateFilter() { m_filterExpressionEntry->SetText(m_item->filterExpression().c_str()); }
...
void CmsShowEDI::toggleItemVisible(Bool_t on) {
  FWDisplayProperties changeProperties = m_item->defaultDisplayProperties();
...
void CmsShowEDI::changeSelectionColor(Color_t c) {
  FWChangeSentry sentry(*(m_item->changeManager()));
  const std::set<FWModelId>& ss = m_item->selectionManager()->selected();
  FWDisplayProperties dp = m_item->defaultDisplayProperties();
  dp.setColor(c);
  for (auto s : ss) {
    m_item->setDisplayProperties(s.index(), dp);
  }
}

void CmsShowEDI::show(FWDataCategories iToView) { m_tabs->SetTab(iToView); }
