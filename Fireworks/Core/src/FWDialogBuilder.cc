...
FWLayoutBuilder::FWLayoutBuilder(TGCompositeFrame *window, bool expandY)
  ...
  TGLayoutHints *hints = new TGLayoutHints(expandY ? kLHintsExpandX | kLHintsExpandY : kLHintsExpandX, 0, 0, 0, 0);
...
FWLayoutBuilder &FWLayoutBuilder::indent(int left /*= 2*/, int right /* = -1*/) {
  if (right < 0)
    right = left;

  TGVerticalFrame *parent = m_framesStack.back();
  TGLayoutHints *hints = new TGLayoutHints(kLHintsExpandX | kLHintsExpandY, left, right, 0, 0);
  m_currentHints = nullptr;
...
/** Set whether or not the previous layout element should expand and
    in which direction.
  */
FWLayoutBuilder &FWLayoutBuilder::expand(bool expandX /*= true*/, bool expandY /*= false*/) {
  UInt_t style = 0;
...
TGLayoutHints *FWLayoutBuilder::nextHints() {
  if (m_floatLeft) {
    ...
    m_currentHints = new TGLayoutHints(kLHintsExpandX, left, 0, m_currentHints->GetPadTop(), 0);
  } else {
...
/** Helper class to construct dialogs in a more readable ways.
  ...
  */
FWDialogBuilder::FWDialogBuilder(TGCompositeFrame *window, FWDialogBuilder *parent /*= 0*/, bool expandY)
    : FWLayoutBuilder(window, expandY), m_parent(parent), m_tabs(nullptr) {}

FWDialogBuilder &FWDialogBuilder::newRow() {
  FWLayoutBuilder::newRow();
  return *this;
}

FWDialogBuilder &FWDialogBuilder::indent(int left /* = 2*/, int right /* = -1*/) {
...
FWDialogBuilder &FWDialogBuilder::addTextView(const char *defaultText /*= 0*/, TGTextView **out /*= 0*/) {
...
FWDialogBuilder &FWDialogBuilder::addTextEdit(const char *defaultText /*= 0*/, TGTextEdit **out /*= 0 */) {
...
FWDialogBuilder &FWDialogBuilder::addColorPicker(const FWColorManager *manager, FWColorSelect **out /*= 0*/) {
  const char *graphicsLabel = " ";
  FWColorSelect *widget = new FWColorSelect(nextFrame(), graphicsLabel, 0, manager, -1);
...
FWDialogBuilder &FWDialogBuilder::addHSlider(size_t size, TGHSlider **out /*= 0*/) {
...
FWDialogBuilder &FWDialogBuilder::addTextButton(const char *text, TGTextButton **out /*= 0*/) {
...
FWDialogBuilder &FWDialogBuilder::addTextEntry(const char *defaultText, TGTextEntry **out) {
...
FWDialogBuilder &FWDialogBuilder::addNumberEntry(
    float defaultValue, size_t digits, TGNumberFormat::EStyle style, int min, int max, TGNumberEntry **out /*= 0*/) {
...
FWDialogBuilder &FWDialogBuilder::addCheckbox(const char *text, TGCheckButton **out /*= 0*/) {
...
FWDialogBuilder &FWDialogBuilder::addTable(FWTableManagerBase *manager, FWTableWidget **out /*= 0*/) {
  expand(true, true);
...
FWDialogBuilder &FWDialogBuilder::addHSeparator(size_t horizontalPadding /*= 4*/, size_t verticalPadding /*= 3*/) {
  TGLayoutHints *hints =
...
FWDialogBuilder &FWDialogBuilder::spaceLeft(size_t spacing) {
  FWLayoutBuilder::spaceLeft(spacing);
  return *this;
}

FWDialogBuilder &FWDialogBuilder::spaceRight(size_t spacing) {
  FWLayoutBuilder::spaceRight(spacing);
  return *this;
}

FWDialogBuilder &FWDialogBuilder::spaceLeftRight(size_t spacing) {
  FWLayoutBuilder::spaceLeft(spacing);
  FWLayoutBuilder::spaceRight(spacing);
  return *this;
}
...
FWDialogBuilder &FWDialogBuilder::frameSpaceLeft(size_t spacing) {
  FWLayoutBuilder::frameSpaceLeft(spacing);
  return *this;
}

FWDialogBuilder &FWDialogBuilder::frameSpaceRight(size_t spacing) {
  FWLayoutBuilder::frameSpaceRight(spacing);
  return *this;
}

FWDialogBuilder &FWDialogBuilder::frameSpaceLeftRight(size_t spacing) {
  FWLayoutBuilder::frameSpaceLeft(spacing);
  FWLayoutBuilder::frameSpaceRight(spacing);
  return *this;
}

FWDialogBuilder &FWDialogBuilder::expand(size_t expandX /*= true*/, size_t expandY /*= false*/) {
