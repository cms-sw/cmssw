...
FWProxyBuilderBase::Product::Product(FWViewType::EType t, const FWViewContext* c)
    : m_viewType(t), m_viewContext(c), m_elements(nullptr) {
...
FWProxyBuilderBase::Product::~Product() {
  ...
  for (auto i = pable->BeginProjecteds(); i != pable->EndProjecteds(); ++i) {
...
FWProxyBuilderBase::~FWProxyBuilderBase() { m_products.clear(); }

//
// member functions
//

void FWProxyBuilderBase::setItem(const FWEventItem* iItem) { m_item = iItem; }
      ...
      for (auto i = m_products.begin(); i != m_products.end(); ++i) {
        ...
        if (pable->HasProjecteds()) {
          ...
          for (auto i = pable->BeginProjecteds(); i != pable->EndProjecteds(); ++i) {
            ...
            auto parentIt = projectedAsElement->BeginChildren();
            for (auto prodIt = elms->BeginChildren(); prodIt != elms->EndChildren(); ++prodIt, ++cnt) {
              // reused projected holder
              if (cnt < oldSize) {
                if ((*parentIt)->NumChildren()) {
                  // update projected (mislleading name)
                  for (auto pci = (*parentIt)->BeginChildren(); pci != (*parentIt)->EndChildren(); pci++)
...
//______________________________________________________________________________
void FWProxyBuilderBase::modelChanges(const FWModelIds& iIds, Product* p) {
  TEveElementList* elms = p->m_elements;
  assert(m_item && static_cast<int>(m_item->size()) <= elms->NumChildren() &&
         "can not use default modelChanges implementation");

  auto itElement = elms->BeginChildren();
  int index = 0;
  for (auto it = iIds.begin(), itEnd = iIds.end(); it != itEnd; ++it, ++itElement, ++index) {
...
TEveElementList* FWProxyBuilderBase::createProduct(const FWViewType::EType viewType, const FWViewContext* viewContext) {
...
void FWProxyBuilderBase::removePerViewProduct(FWViewType::EType type, const FWViewContext* vc) {
  for (auto i = m_products.begin(); i != m_products.end(); ++i) {
...
void FWProxyBuilderBase::setInteractionList(FWInteractionList* l, const std::string& /*purpose*/) {
...
void FWProxyBuilderBase::localModelChanges(const FWModelId&, TEveElement*, FWViewType::EType, const FWViewContext*) {
...
void FWProxyBuilderBase::build(const FWEventItem*, TEveElementList*, const FWViewContext*) {
  assert(
      "virtual build(const FWEventItem*, TEveElementList*, const FWViewContext*) not implemented by inherited class");
}

void FWProxyBuilderBase::buildViewType(const FWEventItem*, TEveElementList*, FWViewType::EType, const FWViewContext*) {
  assert(
      "virtual buildViewType(const FWEventItem*, TEveElementList*, FWViewType::EType, const FWViewContext*) not "
      "implemented by inherited class");
}

void FWProxyBuilderBase::setProjectionLayer(float layer) {
  m_layer = layer;
  for (auto pIt = m_products.begin(); pIt != m_products.end(); ++pIt) {
...
void FWProxyBuilderBase::setupAddElement(TEveElement* el, TEveElement* parent, bool color) const {
...
/** This method is invoked to setup the per element properties of the various
    objects being drawn.
  */
void FWProxyBuilderBase::setupElement(TEveElement* el, bool color) const {
...
TEveCompound* FWProxyBuilderBase::createCompound(bool set_color, bool propagate_color_to_all_children) const {
...
const fireworks::Context& FWProxyBuilderBase::context() const { return m_item->context(); }

int FWProxyBuilderBase::layer() const { return m_item->layer(); }

//
// static member functions
//

std::string FWProxyBuilderBase::typeOfBuilder() { return std::string(); }

bool FWProxyBuilderBase::representsSubPart() { return false; }
