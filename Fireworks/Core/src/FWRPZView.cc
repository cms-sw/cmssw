...
TEveCaloViz* FWRPZView::getEveCalo() const { return static_cast<TEveCaloViz*>(m_calo); }
...
void FWRPZView::importElements(TEveElement* iChildren, float iLayer, TEveElement* iProjectedParent) {
...
void FWRPZView::setEtaRng() {
  if (typeId() != FWViewType::kRhoZ) {
    ...
    if (!m_showHF->value())
      eta_range = 3.0;
    if (!m_showEndcaps->value())
      eta_range = context().caloTransEta();
...
void FWRPZView::voteCaloMaxVal() {
  if (!m_calo->GetData()->Empty()) {
    ...
    for (auto it = cellLists.begin(); it != cellLists.end(); it++) {
      ...
      if (binCells) {
        ...
        for (auto k = binCells->begin(); k != a; ++k) {
...
void FWRPZView::populateController(ViewerParameterGUI& gui) const {
  ...
  gui.requestTab("Projection").addParam(&m_showProjectionAxes).addParam(&m_projectionAxesLabelSize).separator();

  TGCompositeFrame* f = gui.getTabContainer();

  f->AddFrame(new TGLabel(f, "FishEye:"));
  gui.addParam(&m_fishEyeDistortion).addParam(&m_fishEyeR).separator();

  f->AddFrame(new TGLabel(f, "PreScales:"));

  gui.requestTab("Projection").addParam(&m_compressMuon).addParam(&m_muonDistortion).addParam(&m_caloDistortion);

  if (typeId() == FWViewType::kRhoPhi || typeId() == FWViewType::kRhoPhiPF) {
    gui.requestTab("Calo").addParam(m_showHF).addParam(m_showEndcaps);
  }
}
