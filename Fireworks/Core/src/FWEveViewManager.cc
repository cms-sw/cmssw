...
public:
  EveSelectionSentry() { m_blocked = gEve->GetSelection()->BlockSignals(true); }
  ~EveSelectionSentry() { gEve->GetSelection()->BlockSignals(m_blocked); }
...
//
FWEveViewManager::FWEveViewManager(FWGUIManager* iGUIMgr) : FWViewManagerBase() {
  // builders
  ...
  if (edmplugin::PluginManager::get()->categoryToInfos().end() !=
    ...
    available =
        edmplugin::PluginManager::get()->categoryToInfos().find(FWProxyBuilderFactory::get()->category())->second;
    std::transform(available.begin(),
                   available.end(),
                   std::inserter(builders, builders.begin()),
                   boost::bind(&edmplugin::PluginInfo::name_, _1));
  ...
  for (auto it = builders.begin(), itEnd = builders.end(); it != itEnd; ++it) {
  ...
  gEve->GetHighlight()->Connect(
      "SelectionAdded(TEveElement*)", "FWEveViewManager", this, "highlightAdded(TEveElement*)");
  gEve->GetHighlight()->Connect(
      "SelectionRepeated(TEveElement*)", "FWEveViewManager", this, "highlightAdded(TEveElement*)");
...
void FWEveViewManager::newItem(const FWEventItem* iItem) {
  ...
  for (size_t bii = 0, bie = blist.size(); bii != bie; ++bii) {
    ...
    if (builder->willHandleInteraction() == false) {
      ...
      std::pair<Iterator, bool> t = m_interactionLists.insert(std::make_pair(iItem, (FWInteractionList*)nullptr));
    ...
    for (size_t viewType = 0; viewType < FWViewType::kTypeSize; ++viewType) {
      ...
      if (builder->havePerViewProduct((FWViewType::EType)viewType)) {
        for (size_t i = 0, e = m_views[viewType].size(); i != e; ++i) {
          ...
          TEveElementList* product = builder->createProduct(type, view->viewContext());
...
//______________________________________________________________________________
FWViewBase* FWEveViewManager::buildView(TEveWindowSlot* iParent, const std::string& viewName) {
...
void FWEveViewManager::modelChangesComing() { gEve->DisableRedraw(); }

void FWEveViewManager::modelChangesDone() { gEve->EnableRedraw(); }
...
void FWEveViewManager::globalEnergyScaleChanged() {
  for (int t = 0; t < FWViewType::kTypeSize; ++t) {
    for (auto i = m_views[t].begin(); i != m_views[t].end(); ++i) {
...
void FWEveViewManager::selectionCleared() { context().selectionManager()->clearSelection(); }
...
FWTypeToRepresentations FWEveViewManager::supportedTypesAndRepresentations() const {
  ...
  for (auto it = m_typeToBuilder.begin(), itEnd = m_typeToBuilder.end(); it != itEnd; ++it) {
...
void FWEveViewManager::BuilderInfo::classType(std::string& typeName, bool& simple) const {
...
void FWEveViewManager::highlightAdded(TEveElement* iElement) {
  ...
  if (iElement == context().getCaloData()) {
    ...
    for (auto i = hlist.begin(); i != hlist.end(); ++i) {
