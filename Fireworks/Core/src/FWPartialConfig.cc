...
namespace {
  struct MyNameMap {
    std::vector<std::pair<std::string, std::string> > names = {{"GUI", "Windows"},
                                                               {"CommonPreferences", "Colors"},
                                                               {"EventNavigator", "EventFilters"},
...
FWPartialConfigSaveGUI::FWPartialConfigSaveGUI(const char* path_out, const char* path_in, FWConfigurationManager* iCfg)
  ...
  AddFrame(new TGLabel(this, Form("Output file: %s", gSystem->BaseName(path_out))),
