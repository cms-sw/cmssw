...
//
FWCandidateTowerProxyBuilder::FWCandidateTowerProxyBuilder() : m_helper(typeid(reco::Candidate)) {}
...
void FWCandidateTowerProxyBuilder::build(const FWEventItem* iItem, TEveElementList* el, const FWViewContext* ctx) {
  //  m_towers=0;
  if (iItem) {
    //      iItem->get(m_towers);
...
REGISTER_FWPROXYBUILDER(FWCandidateTowerProxyBuilder,
                        reco::Candidate,
                        "CaloTower",
