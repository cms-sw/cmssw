...
void Phase2TrackerValidateDigi::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  const DetSetVector<PixelDigi>* pixDigis = digiHandle.product();


  // DigiSimLink
  edm::Handle< edm::DetSetVector<PixelDigiSimLink> > pixelSimLinks;
  iEvent.getByLabel(digiSrc_,   pixelSimLinks);
  ...
  for(std::vector<PSimHit>::const_iterator isim = simHits->begin(); isim != simHits->end(); ++isim){
    ...
    bool matched = false;
    edm::DetSetVector<PixelDigi>::const_iterator DSVIter = pixDigis->find(rawid);
    if (DSVIter != pixDigis->end()) { 
...
// -- Get SimTrack Id
//
unsigned int Phase2TrackerValidateDigi::getSimTrackId(edm::Handle<edm::DetSetVector<PixelDigiSimLink> >& pixelSimLinks, const DetId& detId, unsigned int& channel) {
