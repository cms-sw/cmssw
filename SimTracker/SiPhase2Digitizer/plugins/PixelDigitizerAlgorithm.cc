              ...
              .getParameter<double>("Even_column_interchannelCoupling_next_column")) 
{
  ...
  LogInfo("PixelDigitizerAlgorithm") << "Algorithm constructed "
				     ...
				     << " pix-inefficiency " << addPixelInefficiency_;
}
                                                ...
                                                const GlobalVector& bfield) 
{
  ...
  std::copy_if(inputBegin, inputEnd, std::back_inserter(matchedSimHits), 
                                     [detId] (auto const& hit) -> bool { return hit.detUnitId() == detId; }
              );
...
void PixelDigitizerAlgorithm::add_cross_talk(const Phase2TrackerGeomDetUnit* pixdet) {
  ...
  for (auto& s : theSignal) {
    ...
    s.second.set(signalInElectrons 
               - signalInElectrons_odd_row_Xtalk_next_row 
               - signalInElectrons_even_row_Xtalk_next_row 
               - signalInElectrons_odd_column_Xtalk_next_column 
