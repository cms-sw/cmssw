// -*- C++ -*-
//
// Package:    SiPhase2Digitizer
// Class:      SiPhase2Digitizer
// 
/**\class SiPhase2Digitizer SiPhase2Digitizer.cc SimTracker/SiPhase2Digitizer/src/SiPhase2Digitizer.cc
...
// user include files
#include "SimTracker/SiPhase2Digitizer/plugins/SiPhase2Digitizer.h"
...
namespace cms
{
  SiPhase2Digitizer::SiPhase2Digitizer(const edm::ParameterSet& iConfig, edm::EDProducer& mixMod):
    first(true),
    _pixeldigialgo(),
    ...
    geometryType(iConfig.getParameter<std::string>("GeometryType"))
  {
    ...
    if ( ! rng.isAvailable()) {
      throw cms::Exception("Configuration")
        << "SiPhase2Digitizer requires the RandomNumberGeneratorService\n"
        "which is not present in the configuration file.  You must add the service\n"
        "in the configuration file or remove the modules that require it.";
    }
  
    rndEngine       = &(rng->getEngine());
    _pixeldigialgo.reset(new SiPhase2DigitizerAlgorithm(iConfig,(*rndEngine)));

  }
  
  SiPhase2Digitizer::~SiPhase2Digitizer(){  
  ...
  void
  SiPhase2Digitizer::initializeEvent(edm::Event const& e, edm::EventSetup const& iSetup) {
    if(first){
  ...
  void
  SiPhase2Digitizer::accumulate(edm::Event const& iEvent, edm::EventSetup const& iSetup) {
    // Step A: Get Inputs
  ...
  void
  SiPhase2Digitizer::accumulate(PileUpEventPrincipal const& iEvent, edm::EventSetup const& iSetup) {
    // Step A: Get Inputs
    for(vstring::const_iterator i = trackerContainers.begin(), iEnd = trackerContainers.end(); i != iEnd; ++i) {
      edm::Handle<std::vector<PSimHit> > simHits;
      edm::InputTag tag(hitsProducer, *i);
  ...
  void
  SiPhase2Digitizer::finalizeEvent(edm::Event& iEvent, const edm::EventSetup& iSetup) {
...
using cms::SiPhase2Digitizer;
DEFINE_DIGI_ACCUMULATOR(SiPhase2Digitizer);
