...
const bool Pixel3DDigitizerAlgorithm::_is_inside_ohmic_column(const LocalPoint & p, const std::pair<float,float> & half_pitch) const
{
        ...
        const float & thickness )
{
    ...
    std::function<std::vector<float>(int) > do_step = [&pos_moving,&u_drift,_diffusion_step] (int i) -> std::vector<float>
                { 
                    auto dd = u_drift(pos_moving[0],pos_moving[1]);
                    return std::vector<float>({
                        i*_diffusion_step*dd.x(),
                        i*_diffusion_step*dd.y(),
                        i*_diffusion_step*dd.z()
                        }); 
                };
    ...
    do
    {
        ...
        LogDebug("(super-)charge diffusion") 
            << "step-" << i << ", Initial Ne= " << ncarriers << ", "
            << "r=(" << pos_moving[0]/unit_um 
            << ", " << pos_moving[1]/unit_um 
        ...
        bool diffusion_activated)
{
    ...
    const float max_radial_distance = std::sqrt(half_pitch.first*half_pitch.first+half_pitch.second*half_pitch.second);
    ...
    for(const auto & super_charge: ionization_points)
    {
        ...
        const std::pair<int,int> current_pixel_int = std::make_pair<int,int>(std::floor(current_pixel.first),
                    std::floor(current_pixel.second));
        ...
        const auto relative_position_at_pc = std::make_pair<float,float>((current_pixel.first-current_pixel_int.first)*pitch.first,
        ...
        LogDebug("Pixel3DDigitizerAlgorithm::drift") 
            << "(super-)Charge\nlocal position: (" << super_charge.x()/unit_um 
                << ", " << super_charge.y()/unit_um << ", " << super_charge.z()/unit_um << ") [um]"
            << "\nMeasurement Point (row,column) (" << current_pixel.first << ", " << current_pixel.second << ")" 
            << "\nProxy pixel-cell frame (centered at  left-down corner): (" 
                << relative_position_at_pc.first/unit_um << ", " << relative_position_at_pc.second/unit_um << ") [um]" 
            << "\nProxy pixel-cell frame (centered at n-column): (" 
                << position_at_pc.x()/unit_um << ", " << position_at_pc.y()/unit_um << ") [um] " 
            << "\nNe=" << super_charge.energy() << " electrons";

        // Check if the point is inside any of the column --> no charge was actually created then
        if( _is_inside_n_column(position_at_pc) 
                || _is_inside_ohmic_column(position_at_pc,half_pitch) )
        {
            LogDebug("Pixel3DDigitizerAlgorithm::drift") 
        ...
        LogDebug("Pixel3DDigitizerAlgorithm::drift") 
            ...
            << "Initial electrons = " << super_charge.energy() << " [electrons], Electrons after loss/diff= " << energyOnCollector 
            << " [electrons] ";
        ...
        collection_points.push_back( DigitizerUtility::SignalPoint(current_pixel_int.first,current_pixel_int.second,0.0,0.0,hit.tof(),energyOnCollector) ); 
    }
