...
TrackingMaterialAnalyser::TrackingMaterialAnalyser(
  ...
  if (m_saveSummaryPlot)
    m_plotter =
        new TrackingMaterialPlotter(300., 120., 10); // 10x10 points per cm2
  else
    m_plotter = nullptr;
}
...
void TrackingMaterialAnalyser::saveParameters(const char *name) {
  ...
  for (unsigned int i = 0; i < m_groups.size(); ++i) {
    ...
    std::cout << boost::format("\tnumber of hits:               %9d") %
                     layer.tracks()
              << std::endl;
    ...
    std::cout
        ...
        << std::endl;
    parameters << boost::format("%-20s\t%7d\t%5.1f ± %5.1f cm\t%6.4f ± %6.4f "
                                "\t%6.4fe-03 ± %6.4fe-03 GeV") %
                      layer.name() % layer.tracks() % layer.averageLength() %
                      layer.sigmaLength() % layer.averageRadiationLengths() %
                      layer.sigmaRadiationLengths() %
                      layer.averageEnergyLoss() % layer.sigmaEnergyLoss()
               << std::endl;
  }
...
void TrackingMaterialAnalyser::analyze(const edm::Event &event,
  ...
  if (m_groups.empty()) {
    ...
    LogInfo("TrackingMaterialAnalyser")
        << "TrackingMaterialAnalyser: List of the tracker groups: "
        << std::endl;
    for (unsigned int i = 0; i < m_groups.size(); ++i)
      LogInfo("TrackingMaterialAnalyser") << i << " TrackingMaterialAnalyser:\t"
                                          << m_groups[i]->info() << std::endl;
  }
  Handle<std::vector<MaterialAccountingTrack>> h_tracks;
  event.getByToken(m_materialToken, h_tracks);

  for (std::vector<MaterialAccountingTrack>::const_iterator
           t = h_tracks->begin(),
           end = h_tracks->end();
       t != end; ++t) {
...
void TrackingMaterialAnalyser::split(MaterialAccountingTrack &track) {
  ...
  if (detectors == 0) {
    ...
    if (m_skipAfterLastDetector)
      limits[detectors] =
          track.detectors()[detectors - 1].m_curvilinearOut + TOLERANCE;
    else
      limits[detectors] = track.summary().length() + TOLERANCE;
    limits[detectors + 1] =
        INFINITY; // this is probably no more needed, but doesn't harm...

    // pick the algorithm to define the non-trivial limits
    switch (m_splitMode) {
    // assign each segment to the the nearest layer
    // e.g. the material between pixel barrel 3 and TIB 1 will be split among
    // the two
    case NEAREST_LAYER:
      for (unsigned int i = 1; i < detectors; ++i) {
        limits[i] = (track.detectors()[i - 1].m_curvilinearOut +
                     track.detectors()[i].m_curvilinearIn) /
                    2.;
      }
      break;

    // assign each segment to the the inner layer
    // e.g. all material between pixel barrel 3 and TIB 1 will go into the pixel
    // barrel
    case INNER_LAYER:
      for (unsigned int i = 1; i < detectors; ++i)
    ...
    while (i < track.steps().size()) {
      ...
      // from here onwards we should be in the accountable region, either
      // completely in a single layer:
      ...
      if (begin < limits[index] or end > limits[index + 2]) {
        // sanity check
        std::cerr << "MaterialAccountingTrack::split(): ERROR: internal logic "
                     "error, expected "
                  << limits[index] << " < " << begin << " < "
      ...
      if (limits[index] <= begin and end <= limits[index + 1]) {
        ...
        assert(fraction < 1.);
        std::pair<MaterialAccountingStep, MaterialAccountingStep> parts =
            step.split(fraction);
        ...
        ++index; // next layer
        if (index < detectors)
          track.detectors()[index].account(parts.second, limits[index + 1],
                                           end);
      }
      begin = end;
    }
  ...
  // add the material from each detector to its layer (if there is one and only
  // one)
  for (unsigned int i = 0; i < track.detectors().size(); ++i)
    if (group[i] != 0)
      m_groups[group[i] - 1]->addDetector(track.detectors()[i]);

  // end of track: commit internal buffers and reset the m_groups internal state
  // for a new track
  for (unsigned int i = 0; i < m_groups.size(); ++i)
...
int TrackingMaterialAnalyser::findLayer(
  ...
  if (inside == 0) {
    index = 0;
    std::cerr << "TrackingMaterialAnalyser::findLayer(...): ERROR: detector "
                 "does not belong to any DetLayer"
              << std::endl;
    std::cerr << "TrackingMaterialAnalyser::findLayer(...): detector position: "
              << std::fixed << " (r: " << std::setprecision(1) << std::setw(5)
              << detector.position().perp() << ", z: " << std::setprecision(1)
              << std::setw(6) << detector.position().z()
              << ", phi: " << std::setprecision(3) << std::setw(6)
              << detector.position().phi() << ")" << std::endl;
  }
  if (inside > 1) {
    index = 0;
    std::cerr << "TrackingMaterialAnalyser::findLayer(...): ERROR: detector "
