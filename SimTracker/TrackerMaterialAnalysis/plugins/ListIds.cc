...
ListIds::ListIds(const edm::ParameterSet &pset)
    : printMaterial_(pset.getUntrackedParameter<bool>("printMaterial")),
      materials_(
          pset.getUntrackedParameter<std::vector<std::string>>("materials")) {}

ListIds::~ListIds() {}

void ListIds::analyze(const edm::Event &evt, const edm::EventSetup &setup) {
  std::cout
      << "______________________________ DDD ______________________________"
      << std::endl;
  ...
  if (theCmsTrackerStringToEnum.type(dddGetString(attribute, fv)) !=
  ...
  bool printAnyMaterial = (std::find(materials_.begin(), materials_.end(),
                                     "ANY") != materials_.end());
  do {
    ...
    if (printAnyMaterial ||
        (std::find(materials_.begin(), materials_.end(),
                   fv.logicalPart().material().name().fullname()) !=
         materials_.end())) {
      ...
      for (unsigned int h = 2; h < history.size(); ++h) {
        std::cout << '/' << history[h].logicalPart().name().ns() << ":"
                  << history[h].logicalPart().name().name() << '['
                  << history[h].copyno() << ']';
      }
      if (printMaterial_)
  ...
  std::cout << "______________________________ std::vector<GeomDet*> from "
               "TrackerGeometry::dets() ______________________________"
            << std::endl;
  ...
  for (unsigned int i = 0; i < dets.size(); ++i) {
    ...
    // Surface::PositionType is a typedef for Point3DBase<float,GlobalTag>
    // a.k.a. GlobalPoint
    const Surface::PositionType &p = det.position();
    math::XYZVector position(p.x(), p.y(), p.z());

    std::cout << det.subDetector() << '\t' << det.geographicalId().det() << '\t'
