...
double const MaterialAccountingGroup::s_tolerance =
    0.01; // 100um should be small enough that no elements from different
          // layers/groups are so close

MaterialAccountingGroup::MaterialAccountingGroup(const std::string &name,
                                                 const DDCompactView &geometry)
    : m_name(name), m_elements(), m_boundingbox(), m_accounting(), m_errors(),
      m_tracks(0), m_counted(false), m_file(nullptr) {
  ...
  while (fv.next()) {
    ...
    LogTrace("MaterialAccountingGroup")
        ...
        << std::endl;
    LogTrace("MaterialAccountingGroup")
        << "Name of added element: " << fv.logicalPart().toString()
        << std::endl;
  ...
  LogTrace("MaterialAccountingGroup")
      << "Final BBox r_range: " << m_boundingbox.range_r().first << ", "
  ...
  m_dedx_spectrum = new TH1F((m_name + "_dedx_spectrum").c_str(),
                             "Energy loss spectrum", 1000, 0, 1);
  m_radlen_spectrum = new TH1F((m_name + "_radlen_spectrum").c_str(),
                               "Radiation lengths spectrum", 1000, 0, 1);
  m_dedx_vs_eta = new TProfile((m_name + "_dedx_vs_eta").c_str(),
                               "Energy loss vs. eta", 600, -3, 3);
  m_dedx_vs_z = new TProfile((m_name + "_dedx_vs_z").c_str(),
                             "Energy loss vs. Z", 6000, -300, 300);
  m_dedx_vs_r = new TProfile((m_name + "_dedx_vs_r").c_str(),
                             "Energy loss vs. R", 1200, 0, 120);
  m_radlen_vs_eta = new TProfile((m_name + "_radlen_vs_eta").c_str(),
                                 "Radiation lengths vs. eta", 600, -3, 3);
  m_radlen_vs_z = new TProfile((m_name + "_radlen_vs_z").c_str(),
                               "Radiation lengths vs. Z", 6000, -300, 300);
  m_radlen_vs_r = new TProfile((m_name + "_radlen_vs_r").c_str(),
...
bool MaterialAccountingGroup::inside(
    const MaterialAccountingDetector &detector) const {
  const GlobalPoint &position = detector.position();
  // first check to see if the point is inside the bounding box
  LogTrace("MaterialAccountingGroup")
      << "Testing position: (x, y, z, r) = " << position.x() << ", "
      << position.y() << ", " << position.z() << ", " << position.perp()
      << std::endl;
  if (not m_boundingbox.inside(position.perp(), position.z())) {
    ...
    for (unsigned int i = 0; i < m_elements.size(); ++i) {
      LogTrace("MaterialAccountingGroup")
          << "Closest testing agains(x, y, z, r): (" << m_elements[i].x()
          << ", " << m_elements[i].y() << ", " << m_elements[i].z() << ", "
          << m_elements[i].perp() << ") --> "
...
bool MaterialAccountingGroup::addDetector(
    const MaterialAccountingDetector &detector) {
  if (not inside(detector))
    return false;

  // multiple hits in the same layer (overlaps, etc.) from a single track still
  // count as one for averaging, since the energy deposits from the track have
  // been already split between the different detectors
...
void MaterialAccountingGroup::savePlot(TProfile *plot, float average,
...
std::string MaterialAccountingGroup::info(void) const {
  ...
  out << "BBox: " << std::setprecision(1) << std::setw(6)
      << m_boundingbox.range_z().first << " < Z < " << std::setprecision(1)
      << std::setw(6) << m_boundingbox.range_z().second;
  out << ", " << std::setprecision(1) << std::setw(5)
      << m_boundingbox.range_r().first << " < R < " << std::setprecision(1)
...
void MaterialAccountingGroup::savePlots(void) {
  ...
  savePlot(m_radlen_vs_eta, averageRadiationLengths(),
           m_name + "_radlen_vs_eta");
