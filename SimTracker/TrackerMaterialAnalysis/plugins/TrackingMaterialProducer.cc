...
static const G4LogicalVolume *GetVolume(const std::string &name) {
  ...
  for (G4LogicalVolumeStore::const_iterator volume = lvs->begin();
       volume != lvs->end(); ++volume)
    LogInfo("TrackingMaterialProducer")
...
static inline const G4AffineTransform &
GetTransform(const G4TouchableHistory *touchable, int depth) {
  return touchable->GetHistory()->GetTransform(
      touchable->GetHistory()->GetDepth() - depth);
}

// navigate up the hierarchy of volumes until one with an attached sensitive
// detector is found return a tuple holding
...
std::tuple<const G4VPhysicalVolume *, int>
...
TrackingMaterialProducer::TrackingMaterialProducer(
  ...
  edm::ParameterSet config =
      iPSet.getParameter<edm::ParameterSet>("TrackingMaterialProducer");
  m_selectedNames =
      config.getParameter<std::vector<std::string>>("SelectedVolumes");
...
void TrackingMaterialProducer::update(const BeginOfJob *event) {
  // INFO
  LogInfo("TrackingMaterialProducer")
      << "TrackingMaterialProducer: List of the selected volumes: "
      << std::endl;
  for (std::vector<std::string>::const_iterator volume_name =
    ...
    if (volume) {
      ...
      std::cerr << "TrackingMaterialProducer::update(const BeginOfJob*): "
                   "WARNING: selected volume \""
                << *volume_name << "\" not found in geometry " << std::endl;
    }
  }
}
...
bool TrackingMaterialProducer::isSelectedFast(
    const G4TouchableHistory *touchable) {
  for (int d = touchable->GetHistoryDepth() - 1; d >= 0; --d) {
    if (std::find(m_selectedNames.begin(), m_selectedNames.end(),
                  touchable->GetVolume(d)->GetName()) != m_selectedNames.end())
      return true;
  }
  return false;
}
...
void TrackingMaterialProducer::update(const G4Step *step) {
  const G4TouchableHistory *touchable =
      static_cast<const G4TouchableHistory *>(step->GetTrack()->GetTouchable());
  ...
  const G4Material *material =
      touchable->GetVolume()->GetLogicalVolume()->GetMaterial();
  ...
  GlobalPoint globalPositionIn(globalPosPre.x() / cm, globalPosPre.y() / cm,
                               globalPosPre.z() / cm); // mm -> cm
  GlobalPoint globalPositionOut(globalPosPost.x() / cm, globalPosPost.y() / cm,
                                globalPosPost.z() / cm); // mm -> cm
  ...
  if (sensitive) {
    ...
    position =
        GlobalPoint(pos.x() / cm, pos.y() / cm, pos.z() / cm); // mm -> cm
  ...
  m_track.step(MaterialAccountingStep(length, radiationLengths, energyLoss,
                                      globalPositionIn, globalPositionOut));
  if (leave_sensitive)
    m_track.leaveDetector(sensitive, cosThetaPost);

  if (sensitive)
    LogInfo("TrackingMaterialProducer")
        << "Track was near sensitive     volume " << sensitive->GetName()
        << std::endl;
  else
    LogInfo("TrackingMaterialProducer")
  ...
  LogInfo("TrackingMaterialProducer")
      ...
      << "position(r,z): (" << position.perp() << ", " << position.z()
      << ") cm\n"
...
void TrackingMaterialProducer::update(const EndOfTrack *event) {
  ...
  radLen_vs_eta_->Fill(track->GetMomentum().eta(),
                       m_track.summary().radiationLengths());
  ...
  LogInfo("TrackingMaterialProducer")
      ...
      << m_track.summary().length() << " cm" << std::endl;
  LogInfo("TrackingMaterialProducer")
