...
std::vector<XHistogram::position> XHistogram::splitSegment(Range rangeX,
                                                           Range rangeY) const {
  ...
  // filter away the fragments shorter than m_minDl, and save the center of each
  // fragment along with its fractionary length
...
/// fill one point
void XHistogram::fill(double x, double y, const std::vector<double> &weight,
                      double norm) {
  check_weight(weight);
...
void XHistogram::fill(double x, double y, const std::vector<double> &weight,
                      double norm, unsigned int colour) {
  check_weight(weight);
...
/// fill one segment, normalizing each bin's weight to the fraction of the
/// segment it contains
void XHistogram::fill(const Range &x, const Range &y,
                      const std::vector<double> &weight, double norm) {
  check_weight(weight);
...
/// fill one segment and set its color, normalizing each bin's weight to the
/// fraction of the segment it contains
void XHistogram::fill(const Range &x, const Range &y,
                      const std::vector<double> &weight, double norm,
                      unsigned int colour) {
  check_weight(weight);
  ...
  for (size_t i = 0, s = v.size(); i < s; ++i) {
    for (size_t h = 0; h < m_size; ++h)
      m_histograms[h]->Fill(v[i].x, v[i].y, v[i].f * weight[h]);
    m_normalization->Fill(v[i].x, v[i].y, v[i].f * norm);
    m_colormap->SetBinContent(m_colormap->FindBin(v[i].x, v[i].y),
                              (float)colour);
  }
}
