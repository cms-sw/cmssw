...
public:
  ...
  MaterialAccountingGroup(const std::string &name,
...
private:
  ...
  operator=(const MaterialAccountingGroup &layer) = delete;

public:
  /// buffer material from a detector, if the detector is inside the DetLayer
  /// bounds
  ...
  double sigmaLength(void) const {
    return m_tracks ? std::sqrt(m_errors.length() / m_tracks -
  ...
  /// return the sigma of the normalized energy loss density factor for
  /// Bethe-Bloch
  double sigmaEnergyLoss(void) const {
    return m_tracks ? std::sqrt(m_errors.energyLoss() / m_tracks -
                                averageEnergyLoss() * averageEnergyLoss())
                    : 0.;
  }
...
private:
  ...
  // 100um should be small enough that no elements from different layers/groups
  // are so close
