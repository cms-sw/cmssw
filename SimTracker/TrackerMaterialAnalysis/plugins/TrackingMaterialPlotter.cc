...
unsigned int
TrackingMaterialPlotter::fill_gradient(const TColor &first, const TColor &last,
  ...
  for (unsigned int i = 0; i < steps; ++i) {
    new TColor(static_cast<Int_t>(index + i), r1 + delta_r * i,
...
unsigned int
TrackingMaterialPlotter::fill_gradient(unsigned int first, unsigned int last,
                                       unsigned int steps /*= 100*/,
                                       unsigned int index /* = 0*/) {
  return fill_gradient(*(TColor *)gROOT->GetListOfColors()->At(first),
                       *(TColor *)gROOT->GetListOfColors()->At(last), steps,
                       index);
}

TrackingMaterialPlotter::TrackingMaterialPlotter(float maxZ, float maxR,
                                                 float resolution) {
  ...
  m_tracker = XHistogram(2, rzBinsZ, rzBinsR, std::make_pair(rzMinZ, rzMaxZ),
                         std::make_pair(rzMinR, rzMaxR), m_color.size(), max);
...
void TrackingMaterialPlotter::plotSegmentUnassigned(
    const MaterialAccountingStep &step) {
  std::vector<double> w(2);
  w[0] = step.radiationLengths();
  w[1] = step.energyLoss();
  m_tracker.fill(std::make_pair(step.in().z(), step.out().z()),
                 std::make_pair(step.in().perp(), step.out().perp()), w,
                 step.length(), 1); // 0 is empty, 1 is unassigned
}

void TrackingMaterialPlotter::plotSegmentInLayer(
    const MaterialAccountingStep &step, int layer) {
  std::vector<double> w(2);
  w[0] = step.radiationLengths();
  w[1] = step.energyLoss();
  m_tracker.fill(
      std::make_pair(step.in().z(), step.out().z()),
      std::make_pair(step.in().perp(), step.out().perp()), w, step.length(),
      layer +
          1); // layer is 1-based, but plot uses: 0 is empty, 1 is unassigned
}
...
void TrackingMaterialPlotter::draw(void) {
  ...
  canvas = new TCanvas("radlen_rz", "RadiationLengths - RZ view",
  ...
  int white_slots = 1;
  int MyPalette[100];
  double stops[9] = {0.0000, 0.1250, 0.2500, 0.3750, 0.5000,
                     0.6250, 0.7500, 0.8750, 1.0000};
  double red[9] = {0. / 255.,   5. / 255.,   15. / 255.,
                   35. / 255.,  102. / 255., 196. / 255.,
                   208. / 255., 199. / 255., 110. / 255.};
  double green[9] = {0. / 255.,   48. / 255.,  124. / 255.,
                     192. / 255., 206. / 255., 226. / 255.,
                     97. / 255.,  16. / 255.,  0. / 255.};
  double blue[9] = {99. / 255.,  142. / 255., 198. / 255.,
                    201. / 255., 90. / 255.,  22. / 255.,
                    13. / 255.,  8. / 255.,   2. / 255.};
  int palette_index = TColor::CreateGradientColorTable(9, stops, red, green,
  ...
  canvas = new TCanvas("dedx_rz", "-dE/dx term - RZ view",
                       (int)(600 * scale * 1.25), (int)(120 * scale * 1.50));
  canvas->GetFrame()->SetFillColor(kWhite);
  gStyle->SetOptStat(0);
  gStyle->SetPalette(m_gradient.size(), &m_gradient.front());
  ...
  canvas = new TCanvas("layer_rz", "Layers - RZ view",
                       (int)(600 * scale * 1.25), (int)(120 * scale * 1.50));
  canvas->GetFrame()->SetFillColor(kWhite);
  gStyle->SetOptStat(0);
