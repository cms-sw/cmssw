...
public:
  /// default CTOR
  XHistogram(void)
      : m_minDl(0.000001), m_xRange(), m_yRange(), m_xBins(), m_yBins(),
        m_size(), m_histograms(), m_normalization(), m_colormap(), m_dummy() {}

  // explicit CTOR
  XHistogram(size_t size, size_t bins_x, size_t bins_y, Range x, Range y,
             size_t zones, const std::vector<double> &max)
      : m_minDl(0.000001), m_xRange(x), m_yRange(y), m_xBins(bins_x),
        m_yBins(bins_y), m_size(size), m_histograms(m_size), m_normalization(),
        m_colormap() {
    // setup unnamed ROOT histograms
    for (size_t i = 0; i < m_size; ++i) {
      m_histograms[i].reset(new Histogram(nullptr, nullptr, bins_x, x.first,
                                          x.second, bins_y, y.first, y.second));
      m_histograms[i]->SetMinimum(0.);
      m_histograms[i]->SetMaximum(max[i]);
    }
    m_normalization.reset(new Histogram(nullptr, nullptr, bins_x, x.first,
                                        x.second, bins_y, y.first, y.second));
    m_colormap.reset(new ColorMap(nullptr, nullptr, bins_x, x.first, x.second,
                                  bins_y, y.first, y.second));
    m_colormap->SetMinimum(0);
    m_colormap->SetMaximum(zones);
    Histogram(nullptr, nullptr, 0, 0., 0., 0, 0.,
              0.); // make ROOT "forget" about unnamed histograms
  }
  ...
  /// segment it contains
  void fill(const Range &x, const Range &y, const std::vector<double> &weight,
            double norm);
  ...
  /// when splitting across bin boundaries with splitSegment(...), sub-segments
