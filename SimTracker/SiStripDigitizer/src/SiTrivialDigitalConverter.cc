...
SiTrivialDigitalConverter::convert(const std::vector<float> &analogSignal,
  ...
  if (PreMixing_) {
    ...
    for (size_t i = 0; i < analogSignal.size(); i++) {
      ...
      int adc = convert((gainHandle->getStripGain(i, detGainRange)) *
                        (analogSignal[i]));
      if (adc > 0)
        _temp.push_back(SiStripDigi(i, adc));
    }
...
SiTrivialDigitalConverter::convertRaw(const std::vector<float> &analogSignal,
                                      edm::ESHandle<SiStripGain> &gainHandle,
                                      unsigned int detid) {
  ...
  if (gainHandle.isValid()) {
    ...
    for (size_t i = 0; i < analogSignal.size(); i++) {
      ...
      // convert analog amplitude to digital
      int adc = convertRaw((gainHandle->getStripGain(i, detGainRange)) *
                           (analogSignal[i]));
