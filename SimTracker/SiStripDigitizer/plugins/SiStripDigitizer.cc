...
SiStripDigitizer::SiStripDigitizer(const edm::ParameterSet &conf,
    ...
    : gainLabel(conf.getParameter<std::string>("Gain")),
      ...
      ZSDigi(conf.getParameter<edm::ParameterSet>("DigiModeList")
                 .getParameter<std::string>("ZSDigi")),
      SCDigi(conf.getParameter<edm::ParameterSet>("DigiModeList")
                 .getParameter<std::string>("SCDigi")),
      VRDigi(conf.getParameter<edm::ParameterSet>("DigiModeList")
                 .getParameter<std::string>("VRDigi")),
      PRDigi(conf.getParameter<edm::ParameterSet>("DigiModeList")
                 .getParameter<std::string>("PRDigi")),
  ...
  mixMod.produces<edm::DetSetVector<SiStripDigi>>(ZSDigi).setBranchAlias(
      ZSDigi);
  mixMod.produces<edm::DetSetVector<SiStripRawDigi>>(SCDigi).setBranchAlias(
      alias + SCDigi);
  mixMod.produces<edm::DetSetVector<SiStripRawDigi>>(VRDigi).setBranchAlias(
      alias + VRDigi);
  mixMod.produces<edm::DetSetVector<SiStripRawDigi>>(PRDigi).setBranchAlias(
      alias + PRDigi);
  mixMod.produces<edm::DetSetVector<StripDigiSimLink>>().setBranchAlias(
      alias + "siStripDigiSimLink");
  mixMod
      .produces<std::vector<std::pair<int, std::bitset<6>>>>("AffectedAPVList")
      .setBranchAlias(alias + "AffectedAPV");
  for (auto const &trackerContainer : trackerContainers) {
    edm::InputTag tag(hitsProducer, trackerContainer);
    iC.consumes<std::vector<PSimHit>>(
        edm::InputTag(hitsProducer, trackerContainer));
  }
...
void SiStripDigitizer::accumulateStripHits(
    edm::Handle<std::vector<PSimHit>> hSimHits, const TrackerTopology *tTopo,
    size_t globalSimHitIndex, const unsigned int tofBin) {
  // globalSimHitIndex is the index the sim hit will have when it is put in a
  // collection of sim hits for all crossings. This is only used when creating
  // digi-sim links if configured to do so.

  if (hSimHits.isValid()) {
    std::set<unsigned int> detIds;
    std::vector<PSimHit> const &simHits = *hSimHits.product();
    for (std::vector<PSimHit>::const_iterator it = simHits.begin(),
                                              itEnd = simHits.end();
         it != itEnd; ++it, ++globalSimHitIndex) {
      unsigned int detId = (*it).detUnitId();
      if (detIds.insert(detId).second) {
        // The insert succeeded, so this detector element has not yet been
        // processed.
        assert(detectorUnits[detId]);
        if (detectorUnits[detId]
                ->type()
                .isTrackerStrip()) { // this test can be removed and replaced by
                                     // stripdet!=0
          ...
          theDigiAlgo->accumulateSimHits(it, itEnd, globalSimHitIndex, tofBin,
                                         stripdet, bfield, tTopo,
                                         randomEngine_);
        }
      }
...
void SiStripDigitizer::accumulate(edm::Event const &iEvent,
  ...
  for (auto const &trackerContainer : trackerContainers) {
    ...
    // Now that the hits have been processed, I'll add the amount of hits in
    // this crossing on to the global counter. Next time accumulateStripHits()
    // is called it will count the sim hits as though they were on the end of
    // this collection. Note that this is only used for creating digi-sim links
...
void SiStripDigitizer::initializeEvent(edm::Event const &iEvent,
                                       edm::EventSetup const &iSetup) {
  // Make sure that the first crossing processed starts indexing the sim hits
  // from zero. This variable is used so that the sim hits from all crossing
  // frames have sequential indices used to create the digi-sim link (if
  // configured to do so) rather than starting from zero for each crossing.
  ...
  // FIX THIS! We only need to clear and (re)fill detectorUnits when the
  // geometry type IOV changes.  Use ESWatcher to determine this.
  bool changes = true;
...
void SiStripDigitizer::finalizeEvent(edm::Event &iEvent,
                                     edm::EventSetup const &iSetup) {
  ...
  std::unique_ptr<edm::DetSetVector<StripDigiSimLink>> pOutputDigiSimLink(
      new edm::DetSetVector<StripDigiSimLink>);
  ...
  for (const auto &iu : pDD->detUnits()) {
    if (useConfFromDB) {
      // apply the cable map _before_ digitization: consider only the detis that
      // are connected
    ...
    if (sgd != nullptr) {
      ...
      theDigiAlgo->digitize(collectorZS, collectorRaw, collectorLink, sgd,
                            gainHandle, thresholdHandle, noiseHandle,
                            pedestalHandle, theAffectedAPVvector,
                            randomEngine_);
      if (zeroSuppression) {
  ...
  if (zeroSuppression) {
    ...
    std::unique_ptr<edm::DetSetVector<SiStripDigi>> output(
        new edm::DetSetVector<SiStripDigi>(theDigiVector));
    std::unique_ptr<std::vector<std::pair<int, std::bitset<6>>>>
    AffectedAPVList(
    ...
    if (makeDigiSimLinks_)
      iEvent.put(
          std::move(pOutputDigiSimLink)); // The previous EDProducer didn't name
                                          // this collection so I won't either
  } else {
    // Step C: create output collection
    std::unique_ptr<edm::DetSetVector<SiStripRawDigi>> output_virginraw(
    ...
    std::unique_ptr<edm::DetSetVector<SiStripRawDigi>> output_processedraw(
        new edm::DetSetVector<SiStripRawDigi>());
    std::unique_ptr<edm::DetSetVector<SiStripDigi>> output(
        new edm::DetSetVector<SiStripDigi>());
    // Step D: write output to file
    iEvent.put(std::move(output), ZSDigi);
    iEvent.put(std::move(output_scopemode), SCDigi);
    iEvent.put(std::move(output_virginraw), VRDigi);
