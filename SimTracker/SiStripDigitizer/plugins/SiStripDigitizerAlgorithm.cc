...
// Modified 15/May/2013 mark.grimes@bristol.ac.uk - Modified so that the
// digi-sim link has the correct index for the sim hits stored. It was
// previously always set to zero (I won't mention that it was me who originally
// wrote that). Modified on Feb 11, 2015: prolay.kumar.mal@cern.ch &
// Jean-Laurent.Agram@cern.ch
//                           Added/Modified the individual strip noise in zero
//                           suppression mode from the conditions DB;
//                           previously, the digitizer used to consider the
//                           noise value for individual strips inside a module
//                           from the central strip noise value.
//////////////////////////////////////////////////////////////////////////////////////////////////////////
...
SiStripDigitizerAlgorithm::SiStripDigitizerAlgorithm(
    ...
    : lorentzAngleName(conf.getParameter<std::string>("LorentzAngle")),
      ...
      APVSaturationProbScaling_(
          conf.getParameter<double>("APVSaturationProbScaling")),
      makeDigiSimLinks_(
          conf.getUntrackedParameter<bool>("makeDigiSimLinks", false)),
      ...
      theElectronPerADC(conf.getParameter<double>(
      ...
      theTOFCutForDeconvolution(
          conf.getParameter<double>("TOFCutForDeconvolution")),
      ...
      APVProbabilityFile(
          conf.getParameter<edm::FileInPath>("APVProbabilityFile")) {
...
void SiStripDigitizerAlgorithm::initializeDetUnit(
    StripGeomDetUnit const *det, const edm::EventSetup &iSetup) {
  ...
  // storing the bad strip of the the module. the module is not removed but just
  // signal put to 0
...
void SiStripDigitizerAlgorithm::initializeEvent(const edm::EventSetup &iSetup) {
  ...
  // This should be clear by after all calls to digitize(), but I might as well
  // make sure
  ...
  iSetup.get<SiStripLorentzAngleSimRcd>().get(lorentzAngleName,
                                              lorentzAngleHandle);
}
...
void SiStripDigitizerAlgorithm::accumulateSimHits(
    std::vector<PSimHit>::const_iterator inputBegin,
    std::vector<PSimHit>::const_iterator inputEnd, size_t inputBeginGlobalIndex,
    unsigned int tofBin, const StripGeomDetUnit *det,
    const GlobalVector &bfield, const TrackerTopology *tTopo,
  ...
  float langle = (lorentzAngleHandle.isValid())
                     ? lorentzAngleHandle->getLorentzAngle(detID)
                     : 0.;
  ...
  if (CLHEP::RandFlat::shoot(engine) > inefficiency) {
    AssociationInfoForChannel
        *pDetIDAssociationInfo; // I only need this if makeDigiSimLinks_ is
                                // true...
    if (makeDigiSimLinks_)
      pDetIDAssociationInfo =
          &(associationInfoForDetId_[detId]); // ...so only search the map if
                                              // that is the case
    std::vector<float>
        previousLocalAmplitude; // Only used if makeDigiSimLinks_ is true.
                                // Needed to work out the change in amplitude.

    size_t simHitGlobalIndex =
        inputBeginGlobalIndex; // This needs to stored to create the digi-sim
                               // link later
    for (std::vector<PSimHit>::const_iterator simHitIter = inputBegin;
         simHitIter != inputEnd; ++simHitIter, ++simHitGlobalIndex) {
      ...
      if (std::fabs(simHitIter->tof() - cosmicShift -
                    det->surface().toGlobal(simHitIter->localPosition()).mag() /
                        30.) < tofCut &&
          simHitIter->energyLoss() > 0) {
        if (makeDigiSimLinks_)
          previousLocalAmplitude =
              locAmpl; // Not needed except to make the sim link association.
        size_t localFirstChannel = numStrips;
        size_t localLastChannel = 0;
        // process the hit
        theSiHitDigitizer->processHit(&*simHitIter, *det, bfield, langle,
                                      locAmpl, localFirstChannel,
        ...
        if (makeDigiSimLinks_) { // No need to do any of this if truth
                                 // association was turned off in the
                                 // configuration
          for (size_t stripIndex = 0; stripIndex < locAmpl.size();
               ++stripIndex) {
            // Work out the amplitude from this SimHit from the difference of
            // what it was before and what it is now
            float signalFromThisSimHit =
                locAmpl[stripIndex] - previousLocalAmplitude[stripIndex];
            if (signalFromThisSimHit !=
                0) { // If this SimHit had any contribution I need to record it.
              auto &associationVector = (*pDetIDAssociationInfo)[stripIndex];
              bool addNewEntry = true;
              // Make sure the hit isn't in already. I've seen this a few times,
              // it always seems to happen in pairs so I think it's something to
              // do with the stereo strips.
              for (auto &associationInfo : associationVector) {
                if (associationInfo.trackID == simHitIter->trackId() &&
                  ...
                  // The hit is already in, so add this second contribution and
                  // move on
              ...
              // If the hit wasn't already in create a new association info
              // structure.
              if (addNewEntry)
                associationVector.push_back(AssociationInfo{
                    simHitIter->trackId(), simHitIter->eventId(),
                    signalFromThisSimHit, simHitGlobalIndex, tofBin});
  ...
  theSiPileUpSignals->add(detID, locAmpl, thisFirstChannelWithSignal,
                          thisLastChannelWithSignal);
...
void SiStripDigitizerAlgorithm::calculateInstlumiScale(
  ...
  if (puInfo && FirstLumiCalc_) {
    ...
    const std::vector<float> &TrueInteractionList =
        puInfo->getMix_TrueInteractions();
...
void SiStripDigitizerAlgorithm::digitize(
    edm::DetSet<SiStripDigi> &outdigi, edm::DetSet<SiStripRawDigi> &outrawdigi,
    edm::DetSet<StripDigiSimLink> &outLink, const StripGeomDetUnit *det,
  ...
  const SiPileUpSignals::SignalMapType *theSignal(
      theSiPileUpSignals->getSignal(detID));
  ...
  if (APVSaturationFromHIP) {
    // Implementation of the proper charge scaling function. Need consider
    // resaturation effect: The probability map gives  the probability that at
    // least one HIP happened during the last N bunch crossings (cfr APV
    // recovery time). The impact on the charge depends on the clostest HIP
    // occurance (in terms of bunch crossing). The function discribing the APV
    // recovery is therefore the weighted average function which takes into
    // account all possibilities of HIP occurances across the last bx's.

    // do this step here because we now have access to luminosity information
    if (FirstDigitize_) {

      for (std::map<int, float>::iterator iter = mapOfAPVprobabilities.begin();
           iter != mapOfAPVprobabilities.end(); ++iter) {
        std::bitset<6> bs;
        for (int Napv = 0; Napv < 6; Napv++) {
          float cursor = CLHEP::RandFlat::shoot(engine);
          bs[Napv] =
              cursor < iter->second * APVSaturationProb_
                  ? true
      ...
      while (!HasAtleastOneAffectedAPV) {
        for (int bx = floor(300.0 / 25.0); bx > 0;
             bx--) { // Reminder: make these numbers not hard coded!!
    ...
    if (bs.any()) {
      ...
      if (!PreMixing_) {
        ...
        // Here below is the scaling function which describes the evolution of
        // the baseline (i.e. how the charge is suppressed). This must be
        // replaced as soon as we have a proper modeling of the baseline
        // evolution from VR runs
        float Shift =
            1 - NumberOfBxBetweenHIPandEvent /
                    floor(300.0 /
                          25.0); // Reminder: make these numbers not hardcoded!!
  ...
  auto iAssociationInfoByChannel = associationInfoForDetId_.find(
      detID); // Use an iterator so that I can easily remove it once finished
  ...
  if (zeroSuppression) {
      ...
      if (SingleStripNoise) {
        ...
        for (int strip = 0; strip < numStrips; ++strip) {
          if (!badChannels[strip]) {
            ...
            // std::cout<<"<SiStripDigitizerAlgorithm::digitize>: gainValue:
            // "<<gainValue<<"\tnoiseRMSv["<<strip<<"]:
            // "<<noiseRMSv[strip]<<std::endl;
          }
        }
        ...
        if (RefStrip < numStrips) {
          ...
          float RefnoiseRMS = noiseHandle->getNoise(RefStrip, detNoiseRange) *
                              theElectronPerADC / RefgainValue;

          theSiNoiseAdder->addNoise(detAmpl, firstChannelWithSignal,
                                    lastChannelWithSignal, numStrips,
                                    RefnoiseRMS, engine);
          // std::cout<<"<SiStripDigitizerAlgorithm::digitize>: RefgainValue:
          // "<<RefgainValue<<"\tRefnoiseRMS: "<<RefnoiseRMS<<std::endl;
        }
      }
    } // if noise
    ...
    // turned off in the configuration this map will be empty and the iterator
    // will always equal associationInfoForDetId_.end().
    if (iAssociationInfoByChannel !=
        associationInfoForDetId_.end()) { // make sure the readings for this
                                          // DetID aren't completely from noise
      for (const auto &iDigi : digis) {
        auto &associationInfoByChannel = iAssociationInfoByChannel->second;
        const std::vector<AssociationInfo> &associationInfo =
            associationInfoByChannel[iDigi.channel()];

        // Need to find the total from all sim hits, because this might not be
        // the same as the total digitised due to noise or whatever.
        float totalSimADC = 0;
        ...
        for (const auto &iAssociationInfo : associationInfo) {
          // Note simHitGlobalIndex used to have +1 because TrackerHitAssociator
          // (the only place I can find this value being used) expected counting
          // to start at 1, not 0.  Now changed.
          outLink.push_back(StripDigiSimLink(
              iDigi.channel(), iAssociationInfo.trackID,
              iAssociationInfo.simHitGlobalIndex, iAssociationInfo.tofBin,
  ...
  if (!zeroSuppression) {
    ...
    //  edm::LogWarning("SiStripDigitizer")<<"You are running the digitizer
    //  without Noise generation and without applying Zero Suppression. ARE YOU
    //  SURE???";
    //}else{

    DigitalRawVecType rawdigis =
        theSiDigitalConverter->convertRaw(detAmpl, gainHandle, detID);
    ...
    if (iAssociationInfoByChannel !=
        associationInfoForDetId_.end()) { // make sure the readings for this
                                          // DetID aren't completely from noise
      // N.B. For the raw digis the channel is inferred from the position in the
      // vector. I'VE NOT TESTED THIS YET!!!!! ToDo Test this properly.
      for (size_t channel = 0; channel < rawdigis.size(); ++channel) {
        ...
        if (iAssociationInfo == associationInfoByChannel.end())
          continue; // Skip if there is no sim information for this channel
                    // (i.e. it's all noise)
        const std::vector<AssociationInfo> &associationInfo =
            iAssociationInfo->second;
        ...
        for (const auto &iAssociationInfo : associationInfo) {
          ...
          // to start at 1, not 0.  Now changed.
          outLink.push_back(StripDigiSimLink(
              channel, iAssociationInfo.trackID,
              iAssociationInfo.simHitGlobalIndex, iAssociationInfo.tofBin,
              iAssociationInfo.eventID,
              iAssociationInfo.contributionToADC / totalSimADC));
        } // end of loop over associationInfo
  ...
  // Now that I've finished with this entry in the map of associations, I can
  // remove it. Note that there might not be an association if the ADC reading
  // is from noise in which case associationIsValid will be false.
