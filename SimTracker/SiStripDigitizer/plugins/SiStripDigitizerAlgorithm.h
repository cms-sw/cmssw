...
public:
  ...
  void initializeDetUnit(StripGeomDetUnit const *det,
  ...
  void accumulateSimHits(const std::vector<PSimHit>::const_iterator inputBegin,
                         ...
                         size_t inputBeginGlobalIndex, unsigned int tofBin,
  ...
  void
  digitize(edm::DetSet<SiStripDigi> &outDigis,
           edm::DetSet<SiStripRawDigi> &outRawDigis,
           edm::DetSet<StripDigiSimLink> &outLink,
           const StripGeomDetUnit *stripdet, edm::ESHandle<SiStripGain> &,
           edm::ESHandle<SiStripThreshold> &, edm::ESHandle<SiStripNoises> &,
...
private:
  ...
  const bool makeDigiSimLinks_; //< Whether or not to create the association to
                                // sim truth collection. Set in configuration.
  const bool peakMode;
  const bool noise;
  ...
  /** This structure is used to keep track of the SimTrack that contributed to
     each digi so that the truth association can be created.*/
  struct AssociationInfo {
    unsigned int trackID;
    EncodedEventId eventID;
    float contributionToADC;
    size_t simHitGlobalIndex; ///< The array index of the sim hit, but in the
                              ///< array for all crossings
    unsigned int tofBin; // Needed along with subDet to determine which PSimHit
                         // collection simHitGlobalIndex indexes
  };
  ...
  /// Structure that holds the information on the SimTrack contributions. Only
  /// filled if makeDigiSimLinks_ is true.
