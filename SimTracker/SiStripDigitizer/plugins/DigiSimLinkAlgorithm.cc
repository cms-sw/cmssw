...
DigiSimLinkAlgorithm::DigiSimLinkAlgorithm(const edm::ParameterSet &conf)
  ...
  theElectronPerADC = conf_.getParameter<double>(
      peakMode ? "electronPerAdcPeak" : "electronPerAdcDec");
  ...
  theTOFCutForDeconvolution =
      conf_.getParameter<double>("TOFCutForDeconvolution");
  ...
  theSiDigitalConverter =
      new SiTrivialDigitalConverter(theElectronPerADC, PreMixing_);
...
void DigiSimLinkAlgorithm::run(
    edm::DetSet<SiStripDigi> &outdigi, edm::DetSet<SiStripRawDigi> &outrawdigi,
    const std::vector<std::pair<const PSimHit *, int>> &input,
    StripGeomDetUnit const *det, GlobalVector bfield, float langle,
  ...
  // stroing the bad stip of the the module. the module is not removed but just
  // signal put to 0
  std::vector<bool> badChannels;
  badChannels.clear();
  badChannels.insert(badChannels.begin(), numStrips, false);
  SiStripBadStrip::data fs;
  for (SiStripBadStrip::ContainerIterator it = detBadStripRange.first;
       it != detBadStripRange.second; ++it) {
  ...
  std::vector<std::pair<const PSimHit *, int>>::const_iterator simHitIter =
      input.begin();
  std::vector<std::pair<const PSimHit *, int>>::const_iterator simHitIterEnd =
      input.end();
  if (CLHEP::RandFlat::shoot(engine) > inefficiency) {
    for (; simHitIter != simHitIterEnd; ++simHitIter) {
      ...
      if (std::fabs(((*simHitIter).first)->tof() - cosmicShift -
        ...
        localLastChannel = 0;
        // process the hit
        theSiHitDigitizer->processHit(((*simHitIter).first), *det, bfield,
                                      langle, locAmpl, localFirstChannel,
                                      localLastChannel, tTopo, engine);
          ...
          if (particle != nullptr) {
            ...
            if (charge != 0 && isHadron) {
              if (CLHEP::RandFlat::shoot(engine) < APVSaturationProb) {
                ...
                std::cout << "-------------------HIP--------------"
                          << std::endl;
                std::cout << "Killing APVs " << FirstAPV << " - " << LastAPV
                          << " " << detID << std::endl;
                for (int strip = FirstAPV * 128; strip < LastAPV * 128 + 128;
                     ++strip)
                  badChannels[strip] =
                      true; // doing like that I remove the signal information
                            // only after the stip that got the HIP but it
                            // remains the signal of the previous one. I'll make
                            // a further loop to remove all signal
              }
            }
          }
        }
        ...
        theDigiSimLinkPileUpSignals->add(
            locAmpl, localFirstChannel, localLastChannel, ((*simHitIter).first),
            (*simHitIter).second);

        // sum signal on strips
        for (size_t iChannel = localFirstChannel; iChannel < localLastChannel;
             iChannel++) {
  ...
  const DigiSimLinkPileUpSignals::HitToDigisMapType &theLink(
      theDigiSimLinkPileUpSignals->dumpLink());
  ...
  if (zeroSuppression) {
    if (noise) {
      ...
      while (RefStrip < numStrips &&
             badChannels[RefStrip]) { // if the refstrip is bad, I move up to
      ...
      if (RefStrip < numStrips) {
        ...
        theSiNoiseAdder->addNoise(
            detAmpl, firstChannelWithSignal, lastChannelWithSignal, numStrips,
            noiseRMS * theElectronPerADC / gainValue, engine);
      }
    }
    digis.clear();
    theSiZeroSuppress->suppress(
        theSiDigitalConverter->convert(detAmpl, gainHandle, detID), digis,
  ...
  if (!zeroSuppression) {
    ...
    if (noise) {
      ...
      if (SingleStripNoise) {
        for (int strip = 0; strip < numStrips; ++strip) {
          if (!badChannels[strip])
            noiseRMSv[strip] = (noiseHandle->getNoise(strip, detNoiseRange)) *
                               theElectronPerADC;
        }
    ...
    if (RealPedestals) {
      for (int strip = 0; strip < numStrips; ++strip) {
        if (!badChannels[strip])
          vPeds[strip] =
              (pedestalHandle->getPed(strip, detPedestalRange) + pedOffset) *
              theElectronPerADC;
      }
    } else {
      for (int strip = 0; strip < numStrips; ++strip) {
    ...
    //  edm::LogWarning("DigiSimLinkAlgorithm")<<"You are running the digitizer
    //  without Noise generation and without applying Zero Suppression. ARE YOU
    //  SURE???";
    //}else{
...
void DigiSimLinkAlgorithm::push_link(const DigitalVecType &digis,
                                     ...
                                     unsigned int detID) {
  link_coll.clear();
  for (DigitalVecType::const_iterator i = digis.begin(); i != digis.end();
       i++) {
    ...
    for (std::map<const PSimHit *, Amplitude>::const_iterator iter =
      ...
      if (fraction >= threshold) {
        ...
        for (std::vector<std::pair<const PSimHit *, int>>::const_iterator
                 simcount = (*cmi).second.begin();
             simcount != (*cmi).second.end(); ++simcount) {
          if ((*iter).first == (*simcount).first)
            sim_counter = (*simcount).second;
        }
        link_coll.push_back(
            StripDigiSimLink((*mi).first,                // channel
...
void DigiSimLinkAlgorithm::push_link_raw(const DigitalRawVecType &digis,
  ...
  link_coll.clear();
  int nstrip = -1;
  for (DigitalRawVecType::const_iterator i = digis.begin(); i != digis.end();
       i++) {
    nstrip++;
    // Instead of checking the validity of the links against the digis,
    //  let's loop over digis and push the corresponding link
