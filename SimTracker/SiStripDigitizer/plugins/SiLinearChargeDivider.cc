...
SiLinearChargeDivider::SiLinearChargeDivider(const edm::ParameterSet &conf)
    : // Run APV in peak instead of deconvolution mode, which degrades the time
      ...
      chargedivisionsPerStrip(
          conf.getParameter<int>("chargeDivisionsPerStrip")),
      // delta cutoff in MeV, has to be same as in Geant (0.120425 MeV
      // corresponding to 100um range for electrons)
      deltaCut(conf.getParameter<double>("DeltaProductionCut")),
      // Offset for digitization during the MTCC and in general for taking
      // cosmic particle The value to be used it must be evaluated and depend on
      // the volume defnition used for the cosimc generation (Considering only
      // the tracker the value is 11 ns)
...
{
  readPulseShape(conf.getParameter<edm::FileInPath>(
                         peakMode ? "APVShapePeakFile" : "APVShapeDecoFile")
                     .fullPath());
}

SiLinearChargeDivider::~SiLinearChargeDivider() {}

void SiLinearChargeDivider::readPulseShape(
    const std::string &pulseShapeFileName) {
  // Pulse shape file format: empty lines and comments (lines starting with '#')
  // are ignored one line "resolution: value" is interpreted as the resolution
  ...
  if (std::abs((*maxIt) - 1.) > std::numeric_limits<double>::epsilon()) {
    throw cms::Exception("WrongAPVPulseShape")
        << "The max value of the APV pulse shape stored in the text file used "
           "in SimGeneral/MixingModule/python/SiStripSimParameters_cfi.py is "
           "not equal to 1. Need to be fixed.";
  }
  pulset0Idx = std::distance(pulseValues.begin(), maxIt);
}

SiChargeDivider::ionization_type SiLinearChargeDivider::divide(
    const PSimHit *hit, const LocalVector &driftdir, double moduleThickness,
    const StripGeomDetUnit &det, CLHEP::HepRandomEngine *engine) {
  ...
  ParticleData const *particle =
      theParticleDataTable->particle(hit->particleType());
  double const particleMass = particle ? particle->mass() * 1000 : 139.57;
  double const particleCharge = particle ? particle->charge() : 1.;

  if (!particle) {
    LogDebug("SiLinearChargeDivider")
        << "Cannot find particle of type " << hit->particleType()
        << " in the PDT we assign to this particle the mass and charge of the "
           "Pion";
  }
  ...
  int NumberOfSegmentation =
      ...
      (fabs(particleMass) < 1.e-6 || particleCharge == 0)
          ? 1
          :
          // computes the number of segments from number of segments per strip
          // times number of strips.
          (int)(1 +
                chargedivisionsPerStrip *
                    fabs(
                        driftXPos(hit->exitPoint(), driftdir, moduleThickness) -
                        driftXPos(hit->entryPoint(), driftdir,
                                  moduleThickness)) /
  ...
  if (NumberOfSegmentation <= 1) {
    ...
    _ionization_points[0] = EnergyDepositUnit(
        eLoss * decSignal / eLoss, hit->entryPoint() + 0.5f * direction);
  } else {
    float eLossVector[NumberOfSegmentation];
    if (fluctuateCharge) {
      fluctuateEloss(particleMass, hit->pabs(), eLoss, direction.mag(),
                     NumberOfSegmentation, eLossVector, engine);
      // Save the energy of each segment
      for (int i = 0; i != NumberOfSegmentation; i++) {
        // take energy value from vector eLossVector,
        _ionization_points[i] = EnergyDepositUnit(
            eLossVector[i] * decSignal / eLoss,
            hit->entryPoint() +
                float((i + 0.5) / NumberOfSegmentation) * direction);
      }
    } else {
      // Save the energy of each segment
      for (int i = 0; i != NumberOfSegmentation; i++) {
        ...
        _ionization_points[i] = EnergyDepositUnit(
            decSignal / float(NumberOfSegmentation),
            hit->entryPoint() +
                float((i + 0.5) / NumberOfSegmentation) * direction);
      }
    }
...
void SiLinearChargeDivider::fluctuateEloss(double particleMass,
                                           float particleMomentum, float eloss,
                                           float length, int NumberOfSegs,
                                           float elossVector[],
  ...
  for (int i = 0; i < NumberOfSegs; i++) {
    ...
    sum += (elossVector[i] =
                fluctuate->SampleFluctuations(mom, particleMass, deltaCutoff,
                                              seglen, segeloss, engine) /
                1000.);
  }
...
float SiLinearChargeDivider::TimeResponse(const PSimHit *hit,
  ...
  const auto dTOF = det.surface().toGlobal(hit->localPosition()).mag() / 30. +
                    cosmicShift - hit->tof();
