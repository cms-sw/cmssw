...
SiHitDigitizer::SiHitDigitizer(const edm::ParameterSet &conf)
    : depletionVoltage(conf.getParameter<double>("DepletionVoltage")),
      ...
      theSiChargeCollectionDrifter(new SiLinearChargeCollectionDrifter(
          CBOLTZ_over_e_SI * chargeMobility *
              conf.getParameter<double>("Temperature") *
              (conf.getParameter<bool>("noDiffusion") ? noDiffusionMultiplier
                                                      : 1.0),
          conf.getParameter<double>("ChargeDistributionRMS"), depletionVoltage,
          conf.getParameter<double>("AppliedVoltage"))),
      theSiInduceChargeOnStrips(new SiTrivialInduceChargeOnStrips(
          conf, conf.getParameter<double>("GevPerElectron"))) {}

SiHitDigitizer::~SiHitDigitizer() {}

void SiHitDigitizer::processHit(const PSimHit *hit, const StripGeomDetUnit &det,
  ...
  double moduleThickness =
      det.specificSurface().bounds().thickness(); // active detector thicness
  double timeNormalisation = (moduleThickness * moduleThickness) /
                             (2. * depletionVoltage * chargeMobility);
