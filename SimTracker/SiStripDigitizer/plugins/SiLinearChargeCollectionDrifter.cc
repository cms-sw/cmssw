...
SiLinearChargeCollectionDrifter::SiLinearChargeCollectionDrifter(double dc,
    ...
    : // Everything which does not depend on the specific det
      diffusionConstant(dc), chargeDistributionRMS(cdr), depletionVoltage(dv),
      appliedVoltage(av) {}

SiChargeCollectionDrifter::collection_type
SiLinearChargeCollectionDrifter::drift(
    const SiChargeCollectionDrifter::ionization_type &ion,
...
SignalPoint SiLinearChargeCollectionDrifter::drift(const EnergyDepositUnit &edu,
  ...
  return SignalPoint(edu.x() + depth * drift.x() / drift.z(),
                     ...
                     sqrt(2. * diffusionConstant * driftTime), edu.energy());
}
