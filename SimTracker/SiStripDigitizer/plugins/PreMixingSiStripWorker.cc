...
public:
  PreMixingSiStripWorker(const edm::ParameterSet &ps,
...
private:
  void DMinitializeDetUnit(StripGeomDetUnit const *det,
  ...
  typedef std::pair<uint16_t, Amplitude>
      RawDigi; // Replacement for SiStripDigi with pulse height instead of
               // integer ADC
  typedef std::vector<SiStripDigi>
      OneDetectorMap; // maps by strip ID for later combination - can have
                      // duplicate strips
  typedef std::vector<RawDigi>
      OneDetectorRawMap; // maps by strip ID for later combination - can have
                         // duplicate strips
  typedef std::map<uint32_t, OneDetectorMap>
      SiGlobalIndex; // map to all data for each detector ID
  typedef std::map<uint32_t, OneDetectorRawMap>
      SiGlobalRawIndex; // map to all data for each detector ID
...
PreMixingSiStripWorker::PreMixingSiStripWorker(const edm::ParameterSet &ps,
    ...
    : gainLabel(ps.getParameter<std::string>("Gain")),
      ...
      theElectronPerADC(ps.getParameter<double>(
      ...
      theSiDigitalConverter(new SiTrivialDigitalConverter(
          theElectronPerADC, false)) // no premixing

{
  ...
  if (APVSaturationFromHIP_) {
    ...
    SiStripAPVPileInputTag_ =
        ps.getParameter<edm::InputTag>("SistripAPVPileInputTag");
    iC.consumes<std::vector<std::pair<int, std::bitset<6>>>>(
        SistripAPVLabelSig_);
  }
  ...
  if (!rng.isAvailable()) {
    throw cms::Exception("Psiguration")
        << "SiStripDigitizer requires the RandomNumberGeneratorService\n"
           "which is not present in the psiguration file.  You must add the "
           "service\n"
...
void PreMixingSiStripWorker::initializeEvent(const edm::Event &e,
  ...
  for (auto iu = pDD->detUnits().begin(); iu != pDD->detUnits().end(); ++iu) {
    ...
    if ((isub == StripSubdetector::TIB) || (isub == StripSubdetector::TID) ||
        (isub == StripSubdetector::TOB) || (isub == StripSubdetector::TEC)) {
...
void PreMixingSiStripWorker::DMinitializeDetUnit(
...
void PreMixingSiStripWorker::addSignals(const edm::Event &e,
  ...
  // keep here for future reference.  In current implementation, HIP killing is
  // done once in PU file
  /*  if(APVSaturationFromHIP_) {
      edm::Handle<std::vector<std::pair<int,std::bitset<6>> > >  APVinput;

      if( e.getByLabel(SistripAPVLabelSig_,APVinput) ) {

      std::vector<std::pair<int,std::bitset<6>> >::const_iterator entry =
     APVinput->begin(); for( ; entry != APVinput->end(); entry++) {
      theAffectedAPVmap_.insert(APVMap::value_type(entry->first,
     entry->second));
      }
      }
      } */
    ...
    if (APVSaturationFromHIP_) {
      ...
      if (inputAPVHandle.isValid()) {
        ...
        for (; entry != APVinput->end(); entry++) {
          theAffectedAPVmap_.insert(
              APVMap::value_type(entry->first, entry->second));
        }
      }
    }
  }
...
void PreMixingSiStripWorker::put(edm::Event &e, edm::EventSetup const &iSetup,
  ...
  // First, have to convert all ADC counts to raw pulse heights so that values
  // can be added properly In PreMixing, pulse heights are saved with ADC =
  // sqrt(9.0*PulseHeight) - have to undo.
  ...
  // Signal Digis are in the list first, have to merge lists of hit strips on
  // the fly, add signals on duplicates later

  OneDetectorRawMap LocalRawMap;

  // Now, loop over hits and add them to the map in the proper sorted order
  // Note: We are assuming that the hits from the Signal events have been
  // created in "PreMix" mode, rather than in the standard ADC conversion
  // routines.  If not, this doesn't work at all.

  // At the moment, both Signal and Reconstituted PU hits have the same
  // compression algorithm. If this were different, and one needed gains, the
  // conversion back to pulse height can only be done in this routine.  So, yes,
  // there is an extra loop over hits here in the current code, because, in
  // principle, one could convert to pulse height during the read/store phase.
  ...
  for (SiGlobalIndex::const_iterator IDet = SiHitStorage_.begin();
    ...
    // loop over hit strips for this DetId, do conversion to pulse height,
    // store.
    ...
    for (; iLocal != LocalMap.end(); ++iLocal) {
      ...
      float ReSignal =
          signal * signal /
          9.0; // The PreMixing conversion is adc = sqrt(9.0*pulseHeight)
  ...
  if (APVSaturationFromHIP_) {
    ...
    for (APVMap::const_iterator iAPV = theAffectedAPVmap_.begin();
      ...
      iAPVchk = iAPV;
      if ((++iAPVchk) ==
          theAffectedAPVmap_.end()) { // make sure not to lose the last one
        DeadAPVList[currentID] = NewAPVBits;
      }
    }
  }
  ...
  //  Ok, done with merging raw signals and APVs - now add signals on duplicate
  //  strips

  // collection of Digis to put in the event
  std::vector<edm::DetSet<SiStripDigi>> vSiStripDigi;

  // loop through our collection of detectors, merging hits and making a new
  // list of "signal" digis
  ...
  // must loop over all detIds in the tracker to get all of the noise added
  // properly.
  for (const auto &iu : pDD->detUnits()) {
    ...
    if (sgd != nullptr) {
      ...
      edm::DetSet<SiStripDigi> SSD(
          detID); // Make empty collection with this detector ID
      ...
      // convert our signals back to raw counts so that we can add noise
      // properly:
      ...
      */
      ...
      // SiStripPedestals::Range detPedestalRange =
      // pedestalHandle->getRange(detID);
  ...
  edm::LogInfo("PreMixingSiStripWorker")
