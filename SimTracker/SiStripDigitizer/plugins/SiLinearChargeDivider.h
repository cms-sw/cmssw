 ...
 * It divides the charge on the line connecting entry and exit point of the
 * SimTrack in the Silicon. Effects that are considered here are:
...
public:
  ...
  // main method: divide the charge (from the PSimHit) into several energy
  // deposits in the bulk
  SiChargeDivider::ionization_type divide(const PSimHit *, const LocalVector &,
                                          double, const StripGeomDetUnit &det,
...
private:
  ...
  // utility: drifts the charge to the surface to estimate the number of
  // relevant strips
  inline float driftXPos(const Local3DPoint &pos, const LocalVector &drift,
                         double thickness) {
    return pos.x() + (thickness / 2. - pos.z()) * drift.x() / drift.z();
  }
  // fluctuate the Eloss
  void fluctuateEloss(double const particleMass, float momentum, float eloss,
                      float length, int NumberOfSegmentation,
