...
class SimHit;
/**
 * Class which takes the responses from each SimHit and piles-up them, within a
 * given module. More precisely, it keeps for each strip the link to each
...
public:
  ...
  // That allows later to comput the fraction of the contribution of each simhit
  // to the ADC value
  typedef std::map<int, std::vector<std::pair<const PSimHit *, Amplitude>>,
                   std::less<int>>
      HitToDigisMapType;
  // associates to each strip a vector of pairs
  // {simhit,index_in_the_allhit_collection} That allows to build the links
  // properly
  typedef std::map<int, std::vector<std::pair<const PSimHit *, int>>,
                   std::less<int>>
  ...
  virtual void add(const std::vector<float> &locAmpl,
                   ...
                   const size_t &lastChannelWithSignal, const PSimHit *hit,
                   const int &counter);
