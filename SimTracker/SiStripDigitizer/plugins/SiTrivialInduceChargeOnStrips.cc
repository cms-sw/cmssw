...
struct Count {
  Count() {}
#ifdef SISTRIP_COUNT
  // note: this code is not thread safe, counts will be inaccurate if run with
  // multiple threads
  double ncall = 0;
  ...
  ~Count() {
    std::cout << "deposits " << ncall << " " << maxdep << " " << ndep / ncall
              << " " << std::sqrt(ndep2 * ncall - ndep * ndep) / ncall
              << std::endl;
    std::cout << "zeros " << dzero << std::endl;
    std::cout << "strips  " << nstr / ndep << " "
...
//                                   0     1      2     3     4    5      6 7
const std::string type[Ntypes] = {"IB1", "IB2", "OB1", "OB2", "W1a",
                                  "W2a", "W3a", "W1b", "W2b", "W3b",
                                  "W4",  "W5",  "W6",  "W7"};
enum {
  indexOfIB1 = 0,
...
fillSignalCoupling(const edm::ParameterSet &conf, int nTypes,
  ...
  for (int i = 0; i < nTypes; ++i) {
    ...
    if (typeArray[i].find("W") != std::string::npos && mode == "Dec")
      version =
          conf.getParameter<bool>("CouplingConstantsRunIIDecW") ? "RunII" : "";
    else if (typeArray[i].find("B") != std::string::npos && mode == "Dec")
      version =
          conf.getParameter<bool>("CouplingConstantsRunIIDecB") ? "RunII" : "";

    auto dc = conf.getParameter<std::vector<double>>(
        "CouplingConstant" + version + mode + typeArray[i]);
    signalCoupling.emplace_back(dc.begin(), dc.end());
  }
  return signalCoupling;
}

inline unsigned int typeOf(const StripGeomDetUnit &det,
...
inline double chargeDeposited(size_t strip, size_t Nstrips, double amplitude,
                              double chargeSpread, double chargePosition) {
  double integralUpToStrip =
      (strip == 0) ? 0.
                   : (approx_erf((strip - chargePosition) / chargeSpread /
                                 1.41421356237309515));
  double integralUpToNext =
      (strip + 1 == Nstrips) ? 1.
                             : (approx_erf((strip + 1 - chargePosition) /
                                           chargeSpread / 1.41421356237309515));
...
SiTrivialInduceChargeOnStrips::SiTrivialInduceChargeOnStrips(
    const edm::ParameterSet &conf, double g)
    : signalCoupling(fillSignalCoupling(conf, Ntypes, type)), Nsigma(3.),
      geVperElectron(g) {}

void SiTrivialInduceChargeOnStrips::induce(
  ...
  float diff=0;
  for (size_t i=0; i!=ampl.size(); ++i) { diff = std::max(diff,ampl[i]>0 ?
std::abs(ampl[i]-oampl[i])/ampl[i] : 0);} if (diff> 1.e-4) { std::cout << diff
<< std::endl; std::cout << "orig " << ominA << " " << omaxA << " ";
//    for (auto a : oampl) std::cout << a << ",";
    std::cout << std::endl;
    std::cout << "vect " << minA << " " << maxA << " ";
//    for (auto a : ampl) std::cout << a << ",";
...
void SiTrivialInduceChargeOnStrips::induceVector(
    const SiChargeCollectionDrifter::collection_type &collection_points,
    const StripGeomDetUnit &det, std::vector<float> &localAmplitudes,
    size_t &recordMinAffectedStrip, size_t &recordMaxAffectedStrip,
    const TrackerTopology *tTopo) const {
  ...
  const StripTopology &topology =
      dynamic_cast<const StripTopology &>(det.specificTopology());
  ...
  for (int ip = 0; ip < NP; ip += MaxN) {
    ...
    for (int i = 0; i != N; ++i) {
      ...
      chargeSpread[i] = collection_points[j].sigma() /
                        topology.localPitch(collection_points[j].position());
    ...
    for (int i = 0; i != N; ++i) {
      fromStrip[i] = std::max(
          0, int(std::floor(chargePosition[i] - Nsigma * chargeSpread[i])));
      nStrip[i] = std::min(Nstrips, int(std::ceil(chargePosition[i] +
                                                  Nsigma * chargeSpread[i]))) -
                  fromStrip[i];
    }
    int tot = 0;
    for (int i = 0; i != N; ++i)
    ...
    for (int i = 0; i != Nstrips; ++i) {
      ...
      for (auto affectedStrip = affectedFromStrip;
           affectedStrip < affectedUntilStrip; ++affectedStrip)
        localAmplitudes[affectedStrip] +=
...
void SiTrivialInduceChargeOnStrips::induceOriginal(
    const SiChargeCollectionDrifter::collection_type &collection_points,
    const StripGeomDetUnit &det, std::vector<float> &localAmplitudes,
    size_t &recordMinAffectedStrip, size_t &recordMaxAffectedStrip,
    const TrackerTopology *tTopo) const {
  ...
  for (auto signalpoint = collection_points.begin();
       signalpoint != collection_points.end(); signalpoint++) {

    // In strip coordinates:
    double chargePosition = topology.strip(signalpoint->position());
    double chargeSpread =
        signalpoint->sigma() / topology.localPitch(signalpoint->position());

    size_t fromStrip = size_t(
        std::max(0, int(std::floor(chargePosition - Nsigma * chargeSpread))));
    size_t untilStrip = size_t(std::min(
        Nstrips, size_t(std::ceil(chargePosition + Nsigma * chargeSpread))));

    count.str(std::max(0, int(untilStrip) - int(fromStrip)));
    for (size_t strip = fromStrip; strip < untilStrip; strip++) {

      double chargeDepositedOnStrip = chargeDeposited(
          strip, Nstrips, signalpoint->amplitude() / geVperElectron,
          chargeSpread, chargePosition);

      size_t affectedFromStrip =
          size_t(std::max(0, int(strip - coupling.size() + 1)));
      size_t affectedUntilStrip =
          size_t(std::min(Nstrips, strip + coupling.size()));
      for (size_t affectedStrip = affectedFromStrip;
           affectedStrip < affectedUntilStrip; affectedStrip++) {
        localAmplitudes.at(affectedStrip) +=
            chargeDepositedOnStrip *
            coupling.at((size_t)std::abs((int)affectedStrip - (int)strip));
      }
