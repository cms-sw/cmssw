...
VertexAssociatorByPositionAndTracks::VertexAssociatorByPositionAndTracks(
    ...
    : productGetter_(productGetter), absZ_(absZ), sigmaZ_(sigmaZ),
      maxRecoZ_(maxRecoZ), absT_(absT), sigmaT_(sigmaT), maxRecoT_(maxRecoT),
...
VertexAssociatorByPositionAndTracks::VertexAssociatorByPositionAndTracks(
    ...
    const reco::SimToRecoCollection *trackSimToRecoAssociation)
    : productGetter_(productGetter), absZ_(absZ), sigmaZ_(sigmaZ),
      maxRecoZ_(maxRecoZ), absT_(std::numeric_limits<double>::max()),
...
VertexAssociatorByPositionAndTracks::associateRecoToSim(
  ...
  LogDebug("VertexAssociation") << "VertexAssociatorByPositionAndTracks::"
                                ...
                                << recoVertices.size() << " reco::Vertices to"
                                << simVertices.size() << " TrackingVertices";
  ...
  for (size_t iReco = 0; iReco != recoVertices.size(); ++iReco) {
    ...
    // skip fake vertices
    if (std::abs(recoVertex.z()) > maxRecoZ_ || recoVertex.isFake() ||
        !recoVertex.isValid() || recoVertex.ndof() < 0.)
      continue;
    ...
    for (const size_t iSim : simPVindices) {
      ...
      LogTrace("VertexAssociation")
          << "  Considering TrackingVertex at Z " << simVertex.position().z();

      //  recoVertex.t() == 0.  is a special value
      // need to change this to std::numeric_limits<double>::max() or something
      // more clear
      const bool useTiming =
          (absT_ != std::numeric_limits<double>::max() && recoVertex.t() != 0.);
      if (useTiming) {
        LogTrace("VertexAssociation")
            << " and T " << recoVertex.t() * CLHEP::second << std::endl;
      }

      const double tdiff =
          std::abs(recoVertex.t() - simVertex.position().t() * CLHEP::second);
      const double zdiff = std::abs(recoVertex.z() - simVertex.position().z());
      if (zdiff < absZ_ && zdiff / recoVertex.zError() < sigmaZ_ &&
          (!useTiming ||
           (tdiff < absT_ && tdiff / recoVertex.tError() < sigmaT_))) {
        auto sharedTracks = calculateVertexSharedTracks(
            recoVertex, simVertex, *trackRecoToSimAssociation_);
        auto fraction = double(sharedTracks) / recoVertex.tracksSize();
        if (sharedTrackFraction_ < 0 || fraction > sharedTrackFraction_) {
          LogTrace("VertexAssociation")
              << "   Matched with significance " << zdiff / recoVertex.zError()
              << " " << tdiff / recoVertex.tError() << " shared tracks "
              << sharedTracks << " reco Tracks " << recoVertex.tracksSize()
              << " TrackingParticles " << simVertex.nDaughterTracks();

          ret.insert(
              reco::VertexBaseRef(vCH, iReco),
              std::make_pair(TrackingVertexRef(tVCH, iSim), sharedTracks));
        }
      }
    }
  }
...
VertexAssociatorByPositionAndTracks::associateSimToReco(
  ...
  LogDebug("VertexAssociation") << "VertexAssociatorByPositionAndTracks::"
                                ...
                                << simVertices.size() << " TrackingVertices to "
                                << recoVertices.size() << " reco::Vertices";
  ...
  for (size_t iSim = 0; iSim != simVertices.size(); ++iSim) {
    ...
    LogTrace("VertexAssociation")
        << " TrackingVertex at Z " << simVertex.position().z();
    ...
    for (size_t iReco = 0; iReco != recoVertices.size(); ++iReco) {
      ...
      LogTrace("VertexAssociation")
          << "  Considering reco::Vertex at Z " << recoVertex.z();
      const bool useTiming =
          (absT_ != std::numeric_limits<double>::max() && recoVertex.t() != 0.);
      if (useTiming) {
        LogTrace("VertexAssociation")
      ...
      if (zdiff < absZ_ && zdiff / recoVertex.zError() < sigmaZ_ &&
          (!useTiming ||
           (tdiff < absT_ && tdiff / recoVertex.tError() < sigmaT_))) {
        auto sharedTracks = calculateVertexSharedTracks(
            simVertex, recoVertex, *trackSimToRecoAssociation_);
        auto fraction = double(sharedTracks) / recoVertex.tracksSize();
        if (sharedTrackFraction_ < 0 || fraction > sharedTrackFraction_) {
          LogTrace("VertexAssociation")
              << "   Matched with significance " << zdiff / recoVertex.zError()
          ...
          ret.insert(
              TrackingVertexRef(tVCH, iSim),
              std::make_pair(reco::VertexBaseRef(vCH, iReco), sharedTracks));
        }
      }
    }
  }
