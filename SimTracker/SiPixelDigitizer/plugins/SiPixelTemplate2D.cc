//
//  SiPixelTemplate2D.cc  Version 2.20
//
...
#ifndef SI_PIXEL_TEMPLATE_STANDALONE
#include "SimTracker/SiPixelDigitizer/plugins/SiPixelTemplate2D.h"
...
bool SiPixelTemplate2D::pushfile(const SiPixel2DTemplateDBObject& dbobject, std::vector< SiPixelTemplateStore2D > & thePixelTemp_)
{
   ...
   for(int m=0; m<db.numOfTempl(); ++m)
   {
      ...
      for (iy=0; iy < theCurrentTemp.head.NTyx; ++iy) {
	    ...
	    //std::cout << "runnr: " << theCurrentTemp.entry[iy][jx].runnum << std::endl;
...
bool SiPixelTemplate2D::interpolate(int id, float cotalpha, float cotbeta, float locBz, float locBx)
{
   ...
   std::cout << "cotalpha=" << cotalpha << "  and cotbeta=" << cotbeta << std::endl;
...
void SiPixelTemplate2D::sideload(SiPixelTemplateEntry2D* entry, int iDtype, float locBx, float locBz, float lorwdx, float lorwdy, float q50, float fbin[3], float xsize, float ysize, float zsize)
{
   ...
   if(flip_y_) lorywidth_ = -lorywidth_;
   lorydrift_ = lorywidth_/2.;
   if(flip_x_) lorxwidth_ = -lorxwidth_;
   lorxdrift_ = lorxwidth_/2.;
...
bool SiPixelTemplate2D::xytempl(float xhit, float yhit, bool ydouble[BYM2], bool xdouble[BXM2], float template2d[BXM2][BYM2], bool derivatives, float dpdx2d[2][BXM2][BYM2], float& QTemplate)
{
   // Interpolate for a new set of track angles
...
bool SiPixelTemplate2D::xytempl(float xhit, float yhit, bool ydouble[BYM2], bool xdouble[BXM2], float template2d[BXM2][BYM2])
{
   // Interpolate for a new set of track angles
   ...
   return SiPixelTemplate2D::xytempl(xhit, yhit, ydouble, xdouble, template2d, derivatives, dpdx2d, QTemplate);
...
bool SiPixelTemplate2D::xytempl(int id, float cotalpha, float cotbeta, float xhit, float yhit, std::vector<bool>& ydouble, std::vector<bool>& xdouble, float template2d[BXM2][BYM2])
{
   ...
   if(SiPixelTemplate2D::interpolate(id, cotalpha, cotbeta, locBz, locBx)) {
     std::cout << "Entering xytempl again" << std::endl;
      
      return SiPixelTemplate2D::xytempl(xhit, yhit, yd, xd, template2d, derivatives, dpdx2d, QTemplate);
   } else {
      return false;
   }
