...
bool SiPixelTemplate2D::pushfile(int filenum)
{
	...
	if(in_file.is_open()) {
		...
		if(theCurrentTemp.head.templ_version != code_version) {LOGERROR("SiPixelTemplate2D") << "code expects version " << code_version << ", but gets version " << theCurrentTemp.head.templ_version <<  ", no template load" << ENDL; return false;}
...
bool SiPixelTemplate2D::pushfile(const SiPixel2DTemplateDBObject& dbobject)
{
	...
	int i, j, k, l, iy, jx;
	//	const char *tempfile;
	const int code_version={21}; // was 16
	...
	std::cout << "Templates pushed: " << thePixelTemp_.size() << " (attempted " << db.numOfTempl() << ")" << std::endl;
// Add this template to the store
...
bool SiPixelTemplate2D::xytempl(int id, float cotalpha, float cotbeta, float locBz, float xhit, float yhit, std::vector<bool>& ydouble, std::vector<bool>& xdouble, float template2d[BXM2][BYM2])
{
	...
	if(id != id_current_ || cotalpha != cota_current_ || cotbeta != cotb_current_) {
		...
		if(id != id_current_) {
			...
			for(i=0; i<(int)thePixelTemp_.size(); ++i) {
			  std::cout << "Found ID " << thePixelTemp_[i].head.ID << ", although that might not be the one you're looking for (which is " << id << ")." << std::endl;
...
// Determine the size of this template    
...
#ifndef SI_PIXEL_TEMPLATE_STANDALONE
					std::cout << "Template has " << Nyx_ << " x " << Nxx_ << " entries." << std::endl;
					if(Nyx_ < 2 || Nxx_ < 2) {
					...
					/*
					std::cout << "cotbetas: " << std::endl;
					for(int iy=0; iy<Nyx_; iy++){
					  for(int ix=0; ix<Nxx_; ix++){
					    std::cout << ix << " " << iy << " " << thePixelTemp_[index_id_].entry[iy][ix].cotbeta << "  " << std::endl;
					  }
					}
					std::cout << "cb0: " << cotbeta0_ << "  cb1: " << cotbeta1_ << "  delta: " << deltacotb_ << std::endl;
					*/
	...
	if(index_id_ < 0 || index_id_ >= (int)thePixelTemp_.size()) {
	  ...
	  throw cms::Exception("DataCorrupt") << "SiPixelTemplate2D::interpolate can't find (BUH!) the needed template ID = " << id 
...
// Interpolate the absolute value of cot(beta)     
	...
	//std::cout << "dcotb " << dcotb << std::endl;
	//std::cout << "iy0 " << iy0_ << std::endl;
...
bool SiPixelTemplate2D::xytempl(int id, float cotalpha, float cotbeta, float xhit, float yhit, std::vector<bool>& ydouble, std::vector<bool>& xdouble, float template2d[BXM2][BYM2])
{
	// Interpolate for a new set of track angles 
	// Local variables 
	float locBz = -1;
	...
	return SiPixelTemplate2D::xytempl(id, cotalpha, cotbeta, locBz, xhit, yhit, ydouble, xdouble, template2d);
