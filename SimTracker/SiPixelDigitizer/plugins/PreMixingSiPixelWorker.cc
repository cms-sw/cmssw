...
PreMixingSiPixelWorker::PixelEfficiencies::PixelEfficiencies(const edm::ParameterSet& conf, bool AddPixelInefficiency, int NumberOfBarrelLayers, int NumberOfEndcapDisks) {
  ...
  if (AddPixelInefficiency){
    ...
    if (FromConfig) {
      edm::LogInfo ("PixelDigitizer ") <<"The PixelDigitizer inefficiency configuration is read from the config file.\n";
    ...
    else edm::LogInfo ("PixelDigitizer ") <<"The PixelDigitizer inefficiency configuration is read from the database.\n";
...
void PreMixingSiPixelWorker::put(edm::Event &e, edm::EventSetup const& iSetup, std::vector<PileupSummaryInfo> const& ps, int bs) {
  ...
  edm::LogInfo("PreMixingSiPixelWorker") << "total # Merged Pixels: " << _signal.size() ;
      ...
      if(!theSignal.empty()) {
        ...
        if (pixelEff_.FromConfig) {
          ...
          if    (pixdet->subDetector()==GeomDetEnumerators::SubDetector::PixelBarrel ||
            ...
            if (NumberOfBarrelLayers==3){
              if(numColumns>416)  edm::LogWarning ("Pixel Geometry") <<" wrong columns in barrel "<<numColumns;
              if(numRows>160)  edm::LogWarning ("Pixel Geometry") <<" wrong rows in barrel "<<numRows;
            ...
            if (NumberOfBarrelLayers==3){  // whether it is the present or the phase 1 detector can be checked using GeomDetEnumerators::SubDetector
              if(numColumns>260 || numRows>160) {
                if(numColumns>260)  edm::LogWarning ("Pixel Geometry") <<" wrong columns in endcaps "<<numColumns;
                if(numRows>160)  edm::LogWarning ("Pixel Geometry") <<" wrong rows in endcaps "<<numRows;
                return;
              }
