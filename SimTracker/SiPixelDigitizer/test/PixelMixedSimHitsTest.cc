// -*- C++ -*-
//
// Package:    PixelMixedSimHitsTest
// Class:      PixelMixedSimHitsTest
//
/**\class PixelMixedSimHitsTest PixelMixedSimHitsTest.cc 

 Description: Test Barrel pixel simhits and fills root histograms. 
 SimHits are loaded from CrossingFrame.
...
PixelMixedSimHitsTest::~PixelMixedSimHitsTest() {
...
void PixelMixedSimHitsTest::beginJob() {
  ...
  htof1m = new TH1F("htof1m", "TOF BC=-1", 250, -50., 75.);
  htof0 = new TH1F("htof0", "TOF BC=0", 250, -50., 75.);
  htof1p = new TH1F("htof1p", "TOF BC=+1", 250, -50., 75.);
  htof2p = new TH1F("htof2p", "TOF BC=+2", 250, -50., 75.);
...
void PixelMixedSimHitsTest::analyze(const edm::Event &iEvent, const edm::EventSetup &iSetup) {
  ...
  std::unique_ptr<MixCollection<PSimHit> > allPixelTrackerHits(
      new MixCollection<PSimHit>(cf.product(), trackerContainers));
  ...
  for (isim = allPixelTrackerHits->begin(); isim != allPixelTrackerHits->end(); isim++) {
    ...
    if (PRINT)
      cout << "det z/r " << detZ << " " << detR << " " << detThick << " " << detLength << " " << detWidth << " " << cols
           << " " << rows << endl;
    ...
    if (PRINT)
      cout << "det id " << detId.rawId() << " " << detid << " " << subid << " " << layer << " " << ladder << " "
           << zindex << endl;
#ifdef CHECK_GEOM
    // To get the module position
    ...
    hBunchCrossing->Fill(float(bcrossing));
    if (bcrossing == -1)
      htof1m->Fill(tof);
    else if (bcrossing == 0)
      htof0->Fill(tof);
    ...
    if (layer == 1) {
      ...
      if (pid != 11 && moduleDirectionUp)
        hladder2idUp->Fill(float(ladder));
      ...
      if (pid != 11 && moduleDirectionUp)
        hladder3idUp->Fill(float(ladder));
    }
  }
...
void PixelMixedSimHitsTest::endJob() {
