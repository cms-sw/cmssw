...
//
PixelMixedSimHitsTest::PixelMixedSimHitsTest(const edm::ParameterSet& iConfig):
  conf_(iConfig) {
...
//    iEvent.getByLabel("SimG4Object","TrackerHitsPixelBarrelHighTof",PixelBarrelHitsHighTof);
    ...
    for (isim=allPixelTrackerHits->begin(); isim!= allPixelTrackerHits->end();isim++) {
     ...
     if (bcrossing == -1) htof1m->Fill(tof);
     else if(bcrossing == 0) htof0->Fill(tof);
     else if(bcrossing == 1) htof1p->Fill(tof);
     else if(bcrossing == 2) htof2p->Fill(tof);
