...
public:
  ...
  TrackingParticle::Vector momentum(const edm::Event &iEvent,
                                    const edm::EventSetup &iSetup,
                                    const Charge ch, const Point &vertex,
                                    const LorentzVector &lv) const override {
    return TrackingParticle::Vector();
  }

  TrackingParticle::Point vertex(const edm::Event &iEvent,
                                 const edm::EventSetup &iSetup, const Charge ch,
                                 const Point &vertex,
                                 const LorentzVector &lv) const override {
  ...
  initEvent(edm::Handle<SimHitTPAssociationProducer::SimHitTPAssociationList>
                simHitsTPAssocToSet) override {
    simHitsTPAssoc = simHitsTPAssocToSet;
  }

  std::unique_ptr<ParametersDefinerForTP> clone() const override {
    return std::unique_ptr<CosmicParametersDefinerForTP>(
        new CosmicParametersDefinerForTP(*this));
  }
