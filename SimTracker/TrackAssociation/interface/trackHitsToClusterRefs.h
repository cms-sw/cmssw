...
getHitFromIter(TrackingRecHitCollection::const_iterator iter) {
  return &(*iter);
}
    ...
    if (trackerHitRTTI::isFromDet(*rhit)) {
      ...
      if (subdetid == PixelSubdetector::PixelBarrel ||
        ...
        if (tid == typeid(SiStripMatchedRecHit2D)) {
          const SiStripMatchedRecHit2D *sMatchedRHit =
              dynamic_cast<const SiStripMatchedRecHit2D *>(rhit);
          if (!sMatchedRHit->monoHit().cluster().isNonnull() ||
              !sMatchedRHit->stereoHit().cluster().isNonnull())
            edm::LogError("TrackAssociator")
                << ">>> RecHit does not have an associated cluster!"
                << " file: " << __FILE__ << " line: " << __LINE__;
          ...
          const SiStripRecHit2D *sRHit =
              dynamic_cast<const SiStripRecHit2D *>(rhit);
          ...
          const SiStripRecHit1D *sRHit =
              dynamic_cast<const SiStripRecHit1D *>(rhit);
          if (!sRHit->cluster().isNonnull())
            edm::LogError("TrackAssociator")
                << ">>> RecHit does not have an associated cluster!"
                << " file: " << __FILE__ << " line: " << __LINE__;
          ...
          const Phase2TrackerRecHit1D *ph2Hit =
              dynamic_cast<const Phase2TrackerRecHit1D *>(rhit);
