...
void TrackerHitAssociator::makeMaps(const edm::Event& theEvent, const TrackerHitAssociator::Config& config) {
  ...
  if (assocHitbySimTrack_) {
	...
	LogDebug("TrkHitAssocTrace") << "simHits from crossing frames; map size = " << SimHitMap.size() << ", Hit count = " << Nhits << std::endl;
      }
    }
    for(auto const& simHitToken : config.simHitTokens_) {
      ...
      if(theEvent.getByToken(simHitToken, simHits)) {
        ...
        ++Nhits;
      	...
      	LogDebug("TrkHitAssocTrace") << "simHits from prompt collections; map size = " << SimHitMap.size() << ", Hit count = " << Nhits << std::endl;
      }
    }
	...
	LogDebug("TrkHitAssocTrace") << "simHits from crossing frames " << collectionIndex << ":  " << Nhits << std::endl;
      }
    }
    collectionIndex = 0;
    for(auto const& simHitToken : config.simHitTokens_) {
	...
	LogDebug("TrkHitAssocTrace") << "simHits from prompt collection " << collectionIndex << ":  " << Nhits << std::endl;
      }
    }
  }
}
...
std::vector<PSimHit> TrackerHitAssociator::associateHit(const TrackingRecHit & thit) const
{
  ...
  if(doTrackAssoc_) return result;  // We don't want the SimHits for this RecHit
  ...
  if (!assocHitbySimTrack_ && !simhitCFPos.empty()) {
    ...
    if(dynamic_cast<const SiStripMatchedRecHit2D *>(&thit)) {
      for(auto const& theSimHitAddr : simhitCFPos) {
	    ...
	    LogDebug("TrkHitAssocTrace")
	      << "by CFpos, simHit detId =  " << theSimHit.detUnitId()
      ...
      for(auto const& theSimHitAddr : simhitCFPos) {
        ...
        if (it!= SimHitMap.end()) {
	     ...
	     LogDebug("TrkHitAssocTrace")
	       ...
	       << "), process = " << (it->second)[theSimHitIndex].processType() << " (" << (it->second)[theSimHitIndex].eventId().bunchCrossing()
	       << ", " << (it->second)[theSimHitIndex].eventId().event() << ", " << (it->second)[theSimHitIndex].trackId() << ")" << std::endl;
          }
        }
      }
    }
...
void TrackerHitAssociator::associateSimpleRecHitCluster(const SiStripCluster* clust,
  ...
  if(isearch != stripdigisimlink->end()) {  //if it is not empty
    ...
    if(clust!=nullptr){//the cluster is valid
      ...
      LogDebug("TrkHitAssocDbg") << "Cluster size " << clusiz << " first strip = " << first << " last strip = " << last-1 << std::endl
							...
							std::vector<simhitAddr>* simhitCFPos) const
{
  ...
  if(isearch !=  ph2trackerdigisimlink->end()) {  //if it is not empty
    ...
    if(!(cluster.isNull())){//if the cluster is valid
      ...
      LogDebug("TrkHitAssocDbg") << "    Cluster minRow " << minRow << " maxRow " << maxRow << " column " << Col << std::endl;
      int dsl = 0;
      for(auto const& linkiter : link_detset.data) {
	++dsl;
	std::pair<int,int> coord = Phase2TrackerDigi::channelToPixel(linkiter.channel());
	LogDebug("TrkHitAssocDbg") << "    " << dsl << ") Digi link: row " << coord.first << " col " << coord.second << std::endl;    
						 ...
						 std::vector<simhitAddr>* simhitCFPos) const
{
  ...
  if(isearch != pixeldigisimlink->end()) {  //if it is not empty
	...
	LogDebug("TrkHitAssocDbg") << "    " << dsl << ") Digi link: row " << pixel_coord.first << " col " << pixel_coord.second << std::endl;    
...
inline std::string TrackerHitAssociator::printDetBnchEvtTrk(const DetId& detid, const uint32_t& detID, std::vector<SimHitIdpr>& simtrackid) const
{ 
  std::stringstream message;
  message << "recHit subdet, detID = " << detid.subdetId() << ", " << detID << ", (bnch, evt, trk) = ";
  for (size_t i=0; i<simtrackid.size(); ++i)
    message << ", (" << simtrackid[i].second.bunchCrossing() << ", "
