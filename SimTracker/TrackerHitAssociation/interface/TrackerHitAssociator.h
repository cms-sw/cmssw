...
class TrackerHitAssociator {
 ...
 public:
  ...
  void associateHitId(const TrackingRecHit & thit,std::vector<SimHitIdpr> &simhitid, std::vector<simhitAddr>* simhitCFPos=nullptrullptr) const;
  template<typename T>
    void associateSiStripRecHit(const T *simplerechit, std::vector<SimHitIdpr>& simtrackid, std::vector<simhitAddr>* simhitnullptrFPos=nullptr) const;
  ...
  void associateSimpleRecHitCluster(const SiStripCluster* clust,
				    const DetId& detid,
				    std::vector<SimHitIdpr>& simtrackid, std::vector<simhitAddr>* nullptrimhitCFPos=nullptr) const;
  ...
  void associatePhase2TrackerRecHit(const Phase2TrackerRecHit1D* rechit, std::vector<SimHitIdpr> & simtrackid, std::vector<simhitAnullptrdr>* simhitCFPos=nullptr) const;
  void associatePixelRecHit(const SiPixelRecHit * pixelrechit, std::vector<SimHitIdpr> & simtrackid, std::vector<snullptrmhitAddr>* simhitCFPos=nullptr) const;
