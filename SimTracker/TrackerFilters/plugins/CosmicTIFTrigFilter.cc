...
CosmicTIFTrigFilter::CosmicTIFTrigFilter(const edm::ParameterSet &conf)
    : m_Token(consumes<edm::HepMCProduct>(
          conf.getParameter<edm::ParameterSet>("Generator")
              .getParameter<std::string>("HepMCProductLabel"))) {
...
bool CosmicTIFTrigFilter::filter(edm::Event &iEvent,
  ...
  for (HepMC::GenEvent::particle_const_iterator i = MCEvt->particles_begin();
       i != MCEvt->particles_end(); ++i) {
    int myId = (*i)->pdg_id();
    if (abs(myId) == 13) {
      ...
      if (trigconf == 1) {
        ...
        */
        ...
        // trigger conditions
        if ((hit1 && hit2) || (hit3 && hit2) || (hit1 && hit4) ||
            (hit3 && hit4)) {
