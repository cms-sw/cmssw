...
public:
  ...
  void analyze(const edm::Event& event, const edm::EventSetup& setup) override {
    ...
    for (unsigned int track = 0; track < tkCollection->size(); ++track) {
      ...
      edm::LogInfo("TrackInfoAnalyzerExample")
          << "Starting state position" << seed.startingState().parameters().position();
      edm::LogInfo("TrackInfoAnalyzerExample")
          << "Starting state direction" << seed.startingState().parameters().momentum();
      ...
      if (rechitref->isValid()) {
        ...
        edm::LogInfo("TrackInfoAnalyzerExample")
            << "Local x-z plane angle of 3rd hit:" << atan2(localdir.x(), localdir.z());
      }
      ...
      for (iter = trackinforef->trajStateMap().begin(); iter != trackinforef->trajStateMap().end(); ++iter) {
        ...
        if (trackinforef->type((*iter).first) == Matched) {  // get the direction for the components
          edm::LogInfo("TrackInfoAnalyzerExample")
              << "LocalMomentum (mono): " << trackinforef->localTrackMomentumOnMono(Combined, (*iter).first);
          edm::LogInfo("TrackInfoAnalyzerExample")
              << "LocalMomentum (stereo): " << trackinforef->localTrackMomentumOnStereo(Combined, (*iter).first);
...
TrackInfoAnalyzerExample::TrackInfoAnalyzerExample(const edm::ParameterSet& pset)
    : TItkassociatorCollectionToken(
          consumes<reco::TrackInfoTrackAssociationCollection>(pset.getParameter<edm::InputTag>("TrackInfo"))),
