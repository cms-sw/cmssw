...
int main(int argc, char* argv[]) {
  ...
  desc.add_options()(kHelpCommandOpt, "produce help message")(
      ...
      kOutputFormatCommandOpt, value<string>(), "output file format as text or json (default: text)");
  // clang-format on
  ...
  try {
    perftools::EdmEventSize me(fileName, treeName, perftools::EdmEventSize::Mode::leaves);
    ...
    if (verbose) {
      ...
      if (vm.count(kOutputFormatOpt) && vm[kOutputFormatOpt].as<std::string>() == "json")
        me.dump(std::cout, false, perftools::EdmEventSize::Format::json);
      else
        me.dump(std::cout);
      std::cout << std::endl;
    }
    ...
    if (vm.count(kOutputOpt)) {
      ...
      if (vm.count(kOutputFormatOpt) && vm[kOutputFormatOpt].as<std::string>() == "json")
        me.dump(of, false, perftools::EdmEventSize::Format::json);
      else
        me.dump(of);
      of << std::endl;
    }
