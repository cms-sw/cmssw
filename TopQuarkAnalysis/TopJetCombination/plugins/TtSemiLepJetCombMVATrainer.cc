...
void TtSemiLepJetCombMVATrainer::beginJob() {
  for (unsigned int i = 0; i < 5; i++)
    nEvents[i] = 0;
}

void TtSemiLepJetCombMVATrainer::analyze(const edm::Event& evt, const edm::EventSetup& setup) {
  ...
  if (genEvt->semiLeptonicChannel() == leptonType_) {
    ...
    if (matchingHandle->empty())
      throw cms::Exception("EmptyProduct") << "Empty vector from jet-parton matching. This should not happen! \n";
    matching = *(matchingHandle->begin());
    if (matching.size() < nPartons)
      return;
    // skip events that were affected by the outlier
    // rejection in the jet-parton matching
    for (unsigned int i = 0; i < matching.size(); ++i) {
      if (matching[i] == -3)
        continue;  // -3: parton was chosen to be excluded from jet-parton matching
  ...
  for (unsigned int i = 0; i < jets->size(); ++i) {
    if (maxNJets_ >= (int)nPartons && i == (unsigned int)maxNJets_)
      break;
...
void TtSemiLepJetCombMVATrainer::endJob() {
...
WDecay::LeptonType TtSemiLepJetCombMVATrainer::readLeptonType(const std::string& str) {
  if (str == "kElec")
    ...
    throw cms::Exception("Configuration") << "Chosen leptonType is not supported: " << str << "\n";
}
