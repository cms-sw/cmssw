...
void TopJetAnalyzer::analyze(const edm::Event& evt, const edm::EventSetup& setup) {
  ...
  std::cout << std::setw(5) << "jet :" << std::setw(11) << "pt :" << std::setw(9) << "eta :" << std::setw(9)
            << "phi :" << std::setw(11) << "TCHE :" << std::setw(11) << "TCHP :" << std::setw(9)
            << "SSVHE :" << std::setw(9) << "SSVHP :";
  if (jets->begin()->isCaloJet()) {
    std::cout << std::setw(8) << "emf :" << std::setw(10) << "n90Hits :" << std::setw(7) << "fHPD";
  }
  if (jets->begin()->isPFJet()) {
    std::cout << std::setw(9) << "chf : " << std::setw(8) << "nhf : " << std::setw(8) << "cef : " << std::setw(8)
              << "nef : " << std::setw(6) << "nCh : " << std::setw(6) << "nConst";
  }
  ...
  for (std::vector<pat::Jet>::const_iterator jet = jets->begin(); jet != jets->end(); ++jet) {
    std::cout << std::setw(3) << i << " : " << std::setprecision(3) << std::fixed << std::setw(8) << jet->pt() << " : "
              ...
              << jet->bDiscriminator("trackCountingHighPurBJetTags") << " : " << std::setw(6)
              << jet->bDiscriminator("simpleSecondaryVertexHighEffBJetTags") << " : " << std::setw(6)
              << jet->bDiscriminator("simpleSecondaryVertexHighPurBJetTags") << " : ";
    if (jet->isCaloJet()) {
      std::cout << std::setw(5) << jet->emEnergyFraction() << " : " << std::setw(7) << jet->jetID().n90Hits << " : "
                << std::setw(6) << jet->jetID().fHPD;
    }
    if (jet->isPFJet()) {
      std::cout << std::setw(5) << jet->chargedHadronEnergyFraction() << " : " << std::setw(5)
                << jet->neutralHadronEnergyFraction() << " : " << std::setw(5) << jet->chargedEmEnergyFraction()
                << " : " << std::setw(5) << jet->neutralEmEnergyFraction() << " : " << std::setw(3)
                << jet->chargedMultiplicity() << " : " << std::setw(6) << jet->nConstituents();
    }
    std::cout << std::endl;
    i++;
  }
