...
void StKinFitter::setupFitter() {
  ...
  for(int constraint : constraints_){
    if(constraint == 1) std::cout<<"    - hadronic W-mass"<<std::endl;
    if(constraint == 2) std::cout<<"    - leptonic W-mass"<<std::endl;
    if(constraint == 3) std::cout<<"    - hadronic top mass"<<std::endl;
    if(constraint == 4) std::cout<<"    - leptonic top mass"<<std::endl;
    if(constraint == 5) std::cout<<"    - neutrino mass"<<std::endl;
  }
  ...
  for (int constraint : constraints_) {
    if (constraint == 1) fitter_->addConstraint(cons1_);
    if (constraint == 2) fitter_->addConstraint(cons2_);
    if (constraint == 3) fitter_->addConstraint(cons3_);
  }
