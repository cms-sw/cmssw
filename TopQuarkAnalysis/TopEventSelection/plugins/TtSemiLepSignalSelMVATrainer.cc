...
void TtSemiLepSignalSelMVATrainer::analyze(const edm::Event& evt, const edm::EventSetup& setup) {
  ...
  if (whatData_ == -1) {  //your training-file contains both, signal and background events
    ...
    if (whatData_ == 1) {  //your tree contains only signal events
      ...
      std::cout << "Config File Error!! Please check <whatData> in TtSemiLepSignalSelMVATrainer.cfi";
  }
}
