...
TtDecayChannelSelector::TtDecayChannelSelector(const edm::ParameterSet& cfg)
  ...
  if (restrictTauDecays_) {
    ...
    restrictTauDecays_ = (allowedTauDecays.existsAs<bool>("electron") || allowedTauDecays.existsAs<bool>("muon") ||
                          allowedTauDecays.existsAs<bool>("oneProng") || allowedTauDecays.existsAs<bool>("threeProng"));
    // specify the different possible restrictions of the tau decay channels
    allowElectron_ =
        (allowedTauDecays.existsAs<bool>("electron") ? allowedTauDecays.getParameter<bool>("electron") : false);
    allowMuon_ = (allowedTauDecays.existsAs<bool>("muon") ? allowedTauDecays.getParameter<bool>("muon") : false);
    allow1Prong_ =
        (allowedTauDecays.existsAs<bool>("oneProng") ? allowedTauDecays.getParameter<bool>("oneProng") : false);
    allow3Prong_ =
        (allowedTauDecays.existsAs<bool>("threeProng") ? allowedTauDecays.getParameter<bool>("threeProng") : false);
  }
    ...
    if (channel == iLep) {
      ...
      if (channel == 2) {
        if (checkSum(allowedDecays_) == channel) {
          // no redundancy
          accept = (allowedDecays_[Elec] == (int)iElec) && (allowedDecays_[Muon] == (int)iMuon) &&
          ...
          if (iElec + iMuon + iTau != channel) {
            ...
            if ((iElec == 2) || (iMuon == 2) || (iTau == 2)) {
              ...
              accept = (((iElec && decayBranchA_[Elec]) &&
                         ((iMuon && decayBranchB_[Muon]) || (iTau && decayBranchB_[Tau]))) ||
                        ((iMuon && decayBranchA_[Muon]) &&
                         ((iElec && decayBranchB_[Elec]) || (iTau && decayBranchB_[Tau]))) ||
                        ((iTau && decayBranchA_[Tau]) &&
                         ((iElec && decayBranchB_[Elec]) || (iMuon && decayBranchB_[Muon]))));
            }
          }
        }
      }
