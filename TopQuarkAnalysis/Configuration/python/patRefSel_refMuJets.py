...
signalMuonCut += ' && track.getHitPattern.trackerLayersWithMeasurement(\'TRACK_HITS\') > 5'                              # muon ID: 'isGlobalMuonPromptTight'
signalMuonCut += ' && globalTrack.getHitPattern.numberOfValidMuonHits(\'TRACK_HITS\')> 0'                               # muon ID: 'isGlobalMuonPromptTight'
signalMuonCut += ' && abs(dB) < 0.2'                                                                  # 2-dim impact parameter with respect to beam spot (s. "PAT muon configuration" above)
signalMuonCut += ' && innerTrack.getHitPattern.numberOfValidPixelHits(\'TRACK_HITS\') > 0'                               # tracker reconstruction
