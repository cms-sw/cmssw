...
*/
//
// $Id: BooLowMAnalyzer.cc,v 1.1.2.18.2.1 2009/07/29 22:05:36 jengbou Exp $
//
//
...
BooLowMAnalyzer::BooLowMAnalyzer(const edm::ParameterSet& iConfig)
{
  ...
  fApplyFlavorJEC = iConfig.getParameter<edm::ParameterSet>("jetCuts").getParameter<bool>  ("ApplyFlavorJEC");
  fJES              = iConfig.getParameter<edm::ParameterSet>("jetCuts").getParameter<double>("JES");
  SigmasTypef = 0;
  //if (fJES==1.1) SigmasTypef = 1;
  //if (fJES==0.9) SigmasTypef = -1;
...
bool
BooLowMAnalyzer::IsTruthMatch( Combo acombo, const edm::View<pat::Jet> jets, TtGenEvent genEvt, bool MatchFlavor, bool fourJets ) {
...
Combo
BooLowMAnalyzer::ApplyFlavorCorrections( Combo acombo, const edm::View<pat::Jet> jets ) {
  ...
  pat::Jet corrPatWp = patWp.correctedJet("had","uds");
  pat::Jet corrPatWq = patWp.correctedJet("had","uds");
  pat::Jet corrPatHadb = patWp.correctedJet("had","b");
  pat::Jet corrPatLepb = patWp.correctedJet("had","b");

  
  Combo tmpcombo;
  /*
  TLorentzVector tmpP4;
  tmpP4.SetPxPyPzE(corrPatWp.px(),corrPatWp.py(),corrPatWp.pz(),corrPatWp.energy());
  tmpcombo.SetWp(tmpP4);
  tmpP4.SetPxPyPzE(corrPatWq.px(),corrPatWq.py(),corrPatWq.pz(),corrPatWq.energy());
  tmpcombo.SetWq(tmpP4);
  tmpP4.SetPxPyPzE(corrPatHadb.px(),corrPatHadb.py(),corrPatHadb.pz(),corrPatHadb.energy());
  tmpcombo.SetHadb(tmpP4);
  tmpP4.SetPxPyPzE(corrPatLepb.px(),corrPatLepb.py(),corrPatLepb.pz(),corrPatLepb.energy());
  tmpcombo.SetLepb(tmpP4);

  tmpcombo.SetLepW(acombo.GetLepW());
  
  tmpcombo.SetSigmas(SigmasTypef);
  tmpcombo.analyze();
  */
  return tmpcombo;
...
//(tmpgenWp.P()+tmpgenWq.P())*TMath::Sin(tmptheta1+tmptheta2)/(2.*tmpgentop.M() ));
   ...
   if (fwriteAscii) {
	   ...
	   for( size_t ijet=0; ijet != jets.size(); ++ijet) {
		   ...
		   fasciiFile << jets[ijet].energy() <<" "<< jets[ijet].px() <<" "<<jets[ijet].py()<<" "<<jets[ijet].pz()<< " " << jets[ijet].bDiscriminator("simpleSecondaryVertexBJetTags") << std::endl;
   ...
   if (NgoodJets >=4 ) {
	   ...
	   int nbtags =0;
	   double mediumOP = 2.05;
	   
	   if (jets[ bestCombo.GetIdHadb() ].bDiscriminator("simpleSecondaryVertexBJetTags") > mediumOP ) nbtags++;
	   if (jets[ bestCombo.GetIdLepb() ].bDiscriminator("simpleSecondaryVertexBJetTags") > mediumOP ) nbtags++;
	   if (jets[ bestCombo.GetIdWq() ].bDiscriminator("simpleSecondaryVertexBJetTags") > mediumOP ) nbtags++;
	   if (jets[ bestCombo.GetIdWp() ].bDiscriminator("simpleSecondaryVertexBJetTags") > mediumOP ) nbtags++;
	   ...
	   if (fIsMCTop) {
	     ...
	     if ( IsTruthMatch(bestCombo, jets, *genEvent, false, false ) ) {
   ...
   if ( found_goodMET) {
	   ...
	   if (NgoodJets >= 4 ) {
		   ...
		   // choose sigmas
		   myCombi_.SetSigmas(SigmasTypef);
		   ...
		   // choose sigmas
		   myCombi2_.SetSigmas(SigmasTypef);
		   ...
		   hjets_->Fill1d("jet_Hadb_disc_cut2", jets[ bestCombo.GetIdHadb() ].bDiscriminator("simpleSecondaryVertexBJetTags") );
		   hjets_->Fill1d("jet_Lepb_disc_cut2", jets[ bestCombo.GetIdLepb() ].bDiscriminator("simpleSecondaryVertexBJetTags") );
		   ...
		   // apply flavor corrections
		   if (fApplyFlavorJEC) {
		     Combo flavorCombo = ApplyFlavorCorrections(bestCombo,jets);
		     hmass_->Fill1d("FlvLeptonicTop_mass_cut2", flavorCombo.GetLepTop().M());
		     hmass_->Fill1d("FlvHadronicTop_mass_cut2", flavorCombo.GetHadTop().M());
		     hmass_->Fill1d("FlvHadronicW_mass_cut2", flavorCombo.GetHadW().M());
		   }
		   ...
		   if ( fIsMCTop ) {
		     if ( IsTruthMatch(Combo3, jets, *genEvent,false,true) ) {
