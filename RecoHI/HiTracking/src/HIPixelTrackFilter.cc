...
bool HIPixelTrackFilter::operator()(const reco::Track* track, const PixelTrackFilterBase::Hits& recHits) const {
  ...
  if (theTIPMax > 0 && fabs(d0) > theTIPMax)
    return false;
  if (theNSigmaTipMaxTolerance > 0 && (fabs(d0) / d0sigma) > theNSigmaTipMaxTolerance)
    return false;
  if (theLIPMax > 0 && fabs(dz) > theLIPMax)
    return false;
  if (theNSigmaLipMaxTolerance > 0 && (fabs(dz) / dzsigma) > theNSigmaLipMaxTolerance)
    return false;

  bool ok = true;
  if (useClusterShape)
    ok = ClusterShapeTrackFilter::operator()(track, recHits);
