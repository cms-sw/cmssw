/*
 ...
 - The code always keeps all tracks in the input collection  
...
void HICaloCompatibleTrackSelector::produce( edm::Event& evt, const edm::EventSetup& es ) 
{
  ...
  bool isPFThere = evt.getByLabel(srcPFCands_, pfCandidates);
  ...
  for (TI ti = hSrcTrack->begin(), ed = hSrcTrack->end(); ti != ed; ++ti, ++current) {
	...
	if(isPFThere){
	  ...
	  matchPt = matchPFCandToTrack(pfCandidates, trackKey);      
	}
      }
      else if(isTowerThere){      
	double matchDr;
	...
	if (matchPt>0 && matchDr>matchConeRadius_pt) matchPt=0.;      
      }
      
      
      float caloComp_pt = (fCaloComp->Eval(trk.pt())!=fCaloComp->Eval(trk.pt())) ? 0 : fCaloComp->Eval(trk.pt()); // protect agains NaN
      
      // Set quality bit based on calo matching
      
      if(matchPt/trk.pt()>caloComp_pt){
	if(reco::TrackBase::qualityByName(qualityToSet_.c_str()) != TrackBase::undefQuality){
	  selTracks_->back().setQuality(reco::TrackBase::qualityByName(qualityToSet_.c_str()));
	}
      }
    }     //if(trk.pt() > trkPtMin_)
  }  // close track loop
...
double
HICaloCompatibleTrackSelector::matchPFCandToTrack( const edm::Handle<reco::PFCandidateCollection> & pfCandidates, unsigned it)
{
  ...
  if(isCandFound){
	...
	break;
      ...
      // We could also add in the PS, for example
