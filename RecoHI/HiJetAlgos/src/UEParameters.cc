...
void UEParameters::get_fourier(double &re, double &im, size_t n, size_t eta) const {
  re = 0;
  im = 0;
...
double UEParameters::get_sum_pt(int eta) const {
  double re;
  double im;
...
double UEParameters::get_vn(int n, int eta) const {
  if (n < 0) {
    return NAN;
  }
  else if (n == 0) {
...
double UEParameters::get_psin(int n, int eta) const {
  if (n <= 0) {
    return 0;
  }
