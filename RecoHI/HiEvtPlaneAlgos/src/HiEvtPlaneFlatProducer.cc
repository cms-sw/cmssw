...
HiEvtPlaneFlatProducer::HiEvtPlaneFlatProducer(const edm::ParameterSet &iConfig)
    : centralityVariable_(
          iConfig.getParameter<std::string>("centralityVariable")),
      centralityBinTag_(
          iConfig.getParameter<edm::InputTag>("centralityBinTag")),
...
void HiEvtPlaneFlatProducer::produce(edm::Event &iEvent,
  ...
  if (hiWatcher.check(iSetup)) {
    ...
    for (int i = 0; i < NumEPNames; i++) {
      ...
      if (caloCentRef_ > 0) {
        int minbin =
            (caloCentRef_ - caloCentRefWidth_ / 2.) * nCentBins_ / 100.;
        int maxbin =
            (caloCentRef_ + caloCentRefWidth_ / 2.) * nCentBins_ / 100.;
  ...
  int indx = 0;
  for (EvtPlaneCollection::const_iterator rp = evtPlanes_->begin();
       rp != evtPlanes_->end(); rp++) {
    ...
    ep[indx] = new EvtPlane(indx, 2, psiFlat, soff, coff, rp->sumw(),
                            rp->sumw2(), rp->sumPtOrEt(), rp->sumPtOrEt2(), m);
