...
//
/**\class EvtPlaneProducer EvtPlaneProducer.cc
RecoHI/EvtPlaneProducer/src/EvtPlaneProducer.cc
...
class GenPlane {
public:
  GenPlane(string name, double etaminval1, double etamaxval1, double etaminval2,
           double etamaxval2, int orderval) {
    epname = name;
    etamin1 = etaminval1;
    etamax1 = etamaxval1;
    etamin2 = etaminval2;
  ...
  double getAngle(double &ang, double &sv, double &cv, double &svNoWgt,
                  double &cvNoWgt, double &w, double &w2, double &PtOrEt,
                  double &PtOrEt2, uint &epmult) {
    ang = -10;
    sv = 0;
    cv = 0;
    sv = sumsin;
...
EvtPlaneProducer::EvtPlaneProducer(const edm::ParameterSet &iConfig)
  ...
  for (int i = 0; i < NumEPNames; i++) {
    rp[i] = new GenPlane(EPNames[i], EPEtaMin1[i], EPEtaMax1[i], EPEtaMin2[i],
                         EPEtaMax2[i], EPOrder[i]);
  }
  for (int i = 0; i < NumEPNames; i++) {
...
void EvtPlaneProducer::produce(edm::Event &iEvent,
    ...
    for (CaloTowerCollection::const_iterator j = caloCollection_->begin();
      ...
      for (int i = 0; i < NumEPNames; i++) {
        ...
        if (EPDet[i] == HF) {
          ...
          rp[i]->addParticle(w, tower_energyet, sin(EPOrder[i] * tower_phi),
                             cos(EPOrder[i] * tower_phi), tower_eta);
        }
      }
    }
  }
  ...
  if (castorCollection_.isValid()) {
    for (std::vector<reco::CastorTower>::const_iterator j =
             castorCollection_->begin();
         j != castorCollection_->end(); j++) {
      tower_eta = j->eta();
      tower_phi = j->phi();
      tower_energyet = j->et();
      double minet = minet_;
  ...
  if (trackCollection_.isValid()) {
    for (reco::TrackCollection::const_iterator j = trackCollection_->begin();
         j != trackCollection_->end(); j++) {
      bool accepted = true;
      bool isPixel = false;
      ...
      if (accepted) {
        ...
        for (int i = 0; i < NumEPNames; i++) {
          ...
          if (EPDet[i] == Tracker) {
            ...
            rp[i]->addParticle(w, track_pt, sin(EPOrder[i] * track_phi),
                               cos(EPOrder[i] * track_phi), track_eta);
          }
        }
      }
    } // end for
  ...
  for (int i = 0; i < NumEPNames; i++) {
    ...
    evtplaneOutput->push_back(
        EvtPlane(i, 0, ang, sv, cv, wv, wv2, pe, pe2, epmult));
