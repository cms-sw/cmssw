...
class LoadEPDB {
public:
  explicit LoadEPDB(const edm::ESHandle<RPFlatParams> flatparmsDB_,
    ...
    if (flatTableSize < flat[0]->getHBins() + 2 * flat[0]->getOBins()) {
      ...
      while (flatTableSize > Hbins + 2 * Obins + cbins) {
        const RPFlatParams::EP *thisBin =
            &(flatparmsDB_->m_table[Hbins + 2 * Obins + cbins]);
        ...
        for (int j = 0; j < ncentbins; j++) {
          const RPFlatParams::EP *thisBin =
              &(flatparmsDB_->m_table[Hbins + 2 * Obins + cbins + j + 1]);
          ...
          if (fabs(centbinning - 2.) < 0.01) {
            for (int i = 0; i < hi::NumEPNames; i++)
              flat[i]->setCentRes2(j, thisBin->x[i], thisBin->y[i]);
          }
          if (fabs(centbinning - 5.) < 0.01) {
            for (int i = 0; i < hi::NumEPNames; i++)
              flat[i]->setCentRes5(j, thisBin->x[i], thisBin->y[i]);
          }
          if (fabs(centbinning - 10.) < 0.01) {
            for (int i = 0; i < hi::NumEPNames; i++)
              flat[i]->setCentRes10(j, thisBin->x[i], thisBin->y[i]);
          }
          if (fabs(centbinning - 20.) < 0.01) {
            for (int i = 0; i < hi::NumEPNames; i++)
              flat[i]->setCentRes20(j, thisBin->x[i], thisBin->y[i]);
          }
          if (fabs(centbinning - 25.) < 0.01) {
            for (int i = 0; i < hi::NumEPNames; i++)
              flat[i]->setCentRes25(j, thisBin->x[i], thisBin->y[i]);
          }
          if (fabs(centbinning - 30.) < 0.01) {
            for (int i = 0; i < hi::NumEPNames; i++)
              flat[i]->setCentRes30(j, thisBin->x[i], thisBin->y[i]);
          }
          if (fabs(centbinning - 40.) < 0.01) {
            for (int i = 0; i < hi::NumEPNames; i++)
              flat[i]->setCentRes40(j, thisBin->x[i], thisBin->y[i]);
          }
        }
