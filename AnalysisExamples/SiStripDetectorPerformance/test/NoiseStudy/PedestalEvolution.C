...
char outFile[128]="PedestalEvolution";
...
void book(){
  ...
  for (int i=1;i<iovDim;i++){
    ...
    for (int j=0;j<vHistoNames.size();j++){
      ...
      sprintf(newName,"PedestalVariationProfile_%s_%s",SubNamePtr,dirName);
      //cout << "newName " << newName << endl;
      Hlist.Add(new TH1F(newName,newName,vHistoNBinsX[j],-0.5,vHistoNBinsX[j]-0.5));
      sprintf(newName,"PedestalVariation_%s_%s",SubNamePtr,dirName);
    ...
    for (int j=0;j<vLayerName.size();j++){
      char newName[128];
      sprintf(newName,"PedestalVariation_%s_%s",vLayerName[j].c_str(),dirName);
      Hlist.Add(new TH1F(newName,newName,histoBins,histoMin,histoMax));      
      sprintf(newName,"PedestalComparison_%s_%s",vLayerName[j].c_str(),dirName);
      Hlist.Add(new TH2F(newName,newName,histoBins,0,512,histoBins,0,512));      
    }

    gDirectory->cd("../SubDet");
    //Make Histos hor each SubDet
    Hlist.Add(new TH1F("PedestalVariation_TIB_"+TString(dirName),"PedestalVariation_TIB_"+TString(dirName),histoBins,histoMin,histoMax));      
    Hlist.Add(new TH1F("PedestalVariation_TOB_"+TString(dirName),"PedestalVariation_TOB_"+TString(dirName),histoBins,histoMin,histoMax));      
    Hlist.Add(new TH1F("PedestalVariation_TEC_"+TString(dirName),"PedestalVariation_TEC_"+TString(dirName),histoBins,histoMin,histoMax));      
    Hlist.Add(new TH1F("PedestalVariation_TID_"+TString(dirName),"PedestalVariation_TID_"+TString(dirName),histoBins,histoMin,histoMax));      
  }
  ...
  for (int j=0;j<vLayerName.size();j++){
    char newName[128];
    sprintf(newName,"pPedestalVariation_%s",vLayerName[j].c_str());
    Hlist.Add(new TProfile(newName,newName,iovDim,minIov,maxIov,histoMin,histoMax));
  }
  
  Hlist.Add(new TProfile("pPedestalVariation_TIB","pPedestalVariation_TIB",iovDim,minIov,maxIov,histoMin,histoMax));
  Hlist.Add(new TProfile("pPedestalVariation_TID","pPedestalVariation_TID",iovDim,minIov,maxIov,histoMin,histoMax));
  Hlist.Add(new TProfile("pPedestalVariation_TOB","pPedestalVariation_TOB",iovDim,minIov,maxIov,histoMin,histoMax));
  Hlist.Add(new TProfile("pPedestalVariation_TEC","pPedestalVariation_TEC",iovDim,minIov,maxIov,histoMin,histoMax));
...
void save(){
  ...
  for (int i=1;i<iovDim;i++){
    ...
    for (int j=0;j<vLayerName.size();j++){
      char newName[128];
      myCanvas.SetLogy(1);;
      sprintf(newName,"IOV_%d/Layer/PedestalVariation_%s_IOV_%d",iov[i],vLayerName[j].c_str(),iov[i]);
	...
	ps.NewPage();
      ...
      myCanvas.SetLogy(0);
      sprintf(newName,"IOV_%d/Layer/PedestalComparison_%s_IOV_%d",iov[i],vLayerName[j].c_str(),iov[i]);
    ...
    myCanvas.SetLogy(1);
    for (int j=0;j<4;j++){
      sprintf(newName,"IOV_%d/SubDet/PedestalVariation_%s_IOV_%d",iov[i],SubDet[j],iov[i]);
      if (((TH1F*) outFile_->Get(newName))->GetEntries()){
	((TH1F*) outFile_->Get(newName))->Draw();
	myCanvas.Update();
	ps.NewPage();
  ...
  for (int j=0;j<4;j++){
    char newName[128];
    sprintf(newName,"pPedestalVariation_%s",SubDet[j]);
    float Max = ((TProfile*) outFile_->Get(newName))->GetMaximum();
    float Min = ((TProfile*) outFile_->Get(newName))->GetMinimum();
    ((TProfile*) outFile_->Get(newName))->SetMaximum((1+profileAlpha)*Max-profileAlpha*Min);
    ((TProfile*) outFile_->Get(newName))->SetMinimum((1+profileAlpha)*Min-profileAlpha*Max);
...
void variation(TH1F* in, TH1F* ref, int iov){
  ...
  sprintf(outName,"IOV_%d/DetId/PedestalVariationProfile_%s_IOV_%d",iov,SubNamePtr,iov);
  outH1[0] = (TH1F*)  outFile_->Get(outName);

  sprintf(outName,"IOV_%d/DetId/PedestalVariation_%s_IOV_%d",iov,SubNamePtr,iov);
  outH1[1] = (TH1F*)  outFile_->Get(outName);

  char tmp[128];
  LayerName(inName,tmp);
  sprintf(outName,"IOV_%d/Layer/PedestalVariation_%s_IOV_%d",iov,tmp,iov);
  outH1[2] = (TH1F*)  outFile_->Get(outName);

  sprintf(outName,"IOV_%d/Layer/PedestalComparison_%s_IOV_%d",iov,tmp,iov);
  outH2 = (TH2F*)  outFile_->Get(outName);

  sprintf(outName,"pPedestalVariation_%s",tmp);
  ...
  sprintf(outName,"IOV_%d/SubDet/PedestalVariation_%s_IOV_%d",iov,det,iov);
  //  cout << outName << endl;
  outH1[3] = (TH1F*)  outFile_->Get(outName);
  
  sprintf(outName,"pPedestalVariation_%s",det);
...
void loop(int iov){
  ...
  for (int j=0;j<vHistoNames.size();j++){
    ...
    char hTitle[128];
    sprintf(hTitle,"Pedestals/%s",vHistoNames[j].c_str());
    //cout << hTitle << endl;
...
PedestalEvolution(char* path_, char* RunsList_){
  ...
  //Get Histo Names
  refFile_->cd("Pedestals");
  TIter nextkey(gDirectory->GetListOfKeys());
  TKey *key;
  while (key = (TKey*)nextkey()) {    
    const char * title;
    title=key->GetTitle();
    if (strncmp(title,"Pedestals_",6)==0 && strstr(title,"Cumulative")== NULL){
