...
using namespace std;
TrackLocalAngleNew::TrackLocalAngleNew(edm::ParameterSet const& conf) : 
  conf_(conf)
{
}
void TrackLocalAngleNew::init(const edm::Event& e, const edm::EventSetup& es){
  ...
  if(!(conf_.getParameter<bool>("MTCCtrack"))){
    ...
    LogDebug("TrackLocalAngleNew") << "get the fitter from the ES" << "\n";
  ...
  //
    ...
    LogDebug("TrackLocalAngleNew") << "get also the propagator" << "\n";
  ...
  LogDebug("TrackLocalAngleNew") << "get also the TransientTrackingRecHitBuilder" << "\n";
...
TrackLocalAngleNew::~TrackLocalAngleNew() {  
}  

std::vector<std::pair<const TrackingRecHit*,float> > TrackLocalAngleNew::findtrackangle(const reco::Track& theT)
{
  //  int cont = 0;
    //convert the TrackingRecHit vector to a TransientTrackingRecHit vector
    //meanwhile computes the number of degrees of freedom
 LogDebug("TrackLocalAngleNew") << "Start\n";
  ...
  for (trackingRecHit_iterator i=theT.recHitsBegin();
   ...
   LogDebug("TrackLocalAngleNew") << "Transient rechit filled" << "\n";
  ...
  LogDebug("TrackLocalAngleNew") << "Initial TSOS\n" << theTSOS << "\n";
...
std::vector< std::pair<const TrackingRecHit*,float> > TrackLocalAngleNew::buildTrack(
										  TransientTrackingRecHit::RecHitContainer& hits,
					 ...
					 const TrajectorySeed& seed)
{
  ...
  if (trajVec.size() != 0){
    ...
    LogDebug("TrackLocalAngleNew") <<"track done";
    std::vector<TrajectoryMeasurement> TMeas=theTraj->measurements();

    std::vector<TrajectoryMeasurement>::iterator itm;
    int i=0;
    LogDebug("TrackLocalAngleNew::findtrackangle")<<"Loop on rechit and TSOS";
	...
	if(trackdirection.z()!=0){
      ...
      LogDebug("TrackLocalAngleNew")<<"I found "<<i<<" hits.";
    }
  }
...
std::vector<std::pair<const TrackingRecHit*,float> > TrackLocalAngleNew::findtrackangle(const TrajectorySeed& seed,
...
TrajectoryStateOnSurface   TrackLocalAngleNew::startingTSOS(const TrajectorySeed& seed)const
{
