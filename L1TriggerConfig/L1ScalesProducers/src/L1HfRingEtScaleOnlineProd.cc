...
class L1HfRingEtScaleOnlineProd : public L1ConfigOnlineProdBase<L1HfRingEtScaleRcd, L1CaloEtScale> {
public:
...
std::unique_ptr<L1CaloEtScale> L1HfRingEtScaleOnlineProd::newObject(const std::string& objectKey) {
  ...
  l1t::OMDSReader::QueryResults hfRingScaleKeyResults =
      m_omdsReader.basicQuery("SC_HF_ET_SUM_FK", "CMS_GT", "L1T_SCALES", "L1T_SCALES.ID", scalesKeyResults);

  std::string hfRingScaleKey;

  if (hfRingScaleKeyResults.queryFailed()) {
    edm::LogError("L1-O2O") << "Problem with key for L1HfRingEtScaleRcd : HF ring Et scale key query failed ";
  } else if (hfRingScaleKeyResults.numberRows() != 1) {
    edm::LogError("L1-O2O") << "Problem with key for L1HfRingEtScaleRcd : " << (hfRingScaleKeyResults.numberRows())
