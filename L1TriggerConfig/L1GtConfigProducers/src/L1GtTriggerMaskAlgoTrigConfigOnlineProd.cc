...
std::unique_ptr<L1GtTriggerMask> L1GtTriggerMaskAlgoTrigConfigOnlineProd::newObject(const std::string& objectKey) {
  ...
  const std::vector<std::string>& columns = m_omdsReader.columnNames(gtSchema, "GT_PARTITION_FINOR_ALGO");

  if (edm::isDebugEnabled()) {
    for (const auto& column : columns) {
      LogTrace("L1GtTriggerMaskAlgoTrigConfigOnlineProd") << column << std::endl;
    }
  }
