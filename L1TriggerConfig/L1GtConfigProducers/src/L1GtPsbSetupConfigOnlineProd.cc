...
std::unique_ptr<L1GtPsbSetup> L1GtPsbSetupConfigOnlineProd::newObject(const std::string& objectKey) {
  ...
  l1t::OMDSReader::QueryResults psbKeys =
      m_omdsReader.basicQuery(psbColumns, gtSchema, "GT_SETUP", "GT_SETUP.ID", m_omdsReader.singleAttribute(objectKey));
  ...
  if (!checkOneLineResult(psbKeys, "GT_SETUP query for PSB keys with ID = " + objectKey)) {
    edm::LogError("L1-O2O") << "Problem to get content of CMS_GT.GT_SETUP for GT_SETUP.ID key: " << objectKey;
...
void L1GtPsbSetupConfigOnlineProd::addPsbFromDb(const std::string& psbKey, std::vector<L1GtPsbConfig>& psbSetup) {
  ...
  l1t::OMDSReader::QueryResults psbQuery = m_omdsReader.basicQuery(
      columnNames, gtSchema, "GT_PSB_SETUP", "GT_PSB_SETUP.ID", m_omdsReader.singleAttribute(psbKey));
  ...
  if (!checkOneLineResult(psbQuery, "GT_PSB_SETUP query for PSB keys with ID = " + psbKey)) {
    edm::LogError("L1-O2O") << "Problem to get setup for PSB keys with ID = " << psbKey;
  ...
  const std::vector<bool>& enableRecLvds =
      extractBoolVector(psbQuery, lvdPrefix, lvdSuffix, L1GtPsbConfig::PsbNumberLvdsGroups);
  const std::vector<bool>& serLinkRecEnable =
      extractBoolVector(psbQuery, serPrefix, serSuffix, L1GtPsbConfig::PsbSerLinkNumberChannels);
...
void L1GtPsbSetupConfigOnlineProd::addDefaultPsb(const std::string& psbColumn,
                                                 std::vector<L1GtPsbConfig>& psbSetup) const {
...
bool L1GtPsbSetupConfigOnlineProd::checkOneLineResult(const l1t::OMDSReader::QueryResults& result,
                                                      const std::string& queryDescription) const {
...
std::string L1GtPsbSetupConfigOnlineProd::numberedColumnName(const std::string& prefix, unsigned number) const {
