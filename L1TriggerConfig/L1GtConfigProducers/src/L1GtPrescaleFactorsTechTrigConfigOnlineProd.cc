...
// constructor
L1GtPrescaleFactorsTechTrigConfigOnlineProd::L1GtPrescaleFactorsTechTrigConfigOnlineProd(const edm::ParameterSet& parSet)
...
std::unique_ptr<L1GtPrescaleFactors> L1GtPrescaleFactorsTechTrigConfigOnlineProd::newObject(
  ...
  l1t::OMDSReader::QueryResults resultsView = m_omdsReader.basicQueryView(columnsView,
                                                                          ...
                                                                          "GT_RUN_SETTINGS_PRESC_VIEW.ID",
                                                                          m_omdsReader.singleAttribute(objectKey));

  // check if query was successful
  if (resultsView.queryFailed()) {
    edm::LogError("L1-O2O") << "Problem to get content of GT_RUN_SETTINGS_PRESC_VIEW "
  ...
  for (int iRow = 0; iRow < resultsViewRows; ++iRow) {
    ...
    const std::vector<std::string>& columns = m_omdsReader.columnNames(gtSchema, "GT_FDL_PRESCALE_FACTORS_TT");
    ...
    l1t::OMDSReader::QueryResults results =
        m_omdsReader.basicQuery(columns,
                                gtSchema,
                                "GT_FDL_PRESCALE_FACTORS_TT",
                                "GT_FDL_PRESCALE_FACTORS_TT.ID",
                                m_omdsReader.singleAttribute(objectKeyPrescaleFactorsSet));

    // check if query was successful
    if (results.queryFailed()) {
      edm::LogError("L1-O2O") << "Problem with L1GtPrescaleFactorsTechTrigRcd key:" << objectKeyPrescaleFactorsSet;
      return pL1GtPrescaleFactors;
    }
    ...
    if (prescaleFactorsSetIndex != countSet) {
      edm::LogError("L1-O2O") << "L1GtPrescaleFactorsTechTrig has unordered sets PRESCALE_INDEX in DB for\n"
                              << " GT_RUN_SETTINGS_PRESC_VIEW.ID = " << objectKey << "\n"
                              << std::endl;
      return pL1GtPrescaleFactors;
    }
