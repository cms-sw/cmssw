...
std::unique_ptr<L1GtPrescaleFactors> L1GtPrescaleFactorsAlgoTrigConfigOnlineProd::newObject(
  ...
  columnsView.emplace_back("PRESCALE_FACTORS_ALGO_FK");
  ...
  for (int iRow = 0; iRow < resultsViewRows; ++iRow) {
    ...
    const std::vector<std::string>& columns = m_omdsReader.columnNames(gtSchema, "GT_FDL_PRESCALE_FACTORS_ALGO");
    ...
    if (results.queryFailed()) {
      edm::LogError("L1-O2O") << "Problem with L1GtPrescaleFactorsAlgoTrigRcd key:" << objectKeyPrescaleFactorsSet;
      return pL1GtPrescaleFactors;
    }
    ...
    if (prescaleFactorsSetIndex != countSet) {
      edm::LogError("L1-O2O") << "L1GtPrescaleFactorsAlgoTrig has unordered sets PRESCALE_INDEX in DB for\n"
                              << " GT_RUN_SETTINGS_PRESC_VIEW.ID = " << objectKey << "\n"
                              << std::endl;
      return pL1GtPrescaleFactors;
    }
