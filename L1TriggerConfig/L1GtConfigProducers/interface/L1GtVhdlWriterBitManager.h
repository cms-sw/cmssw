...
public:
  ...
  std::string readMapInverse(const std::map<std::string, std::string> &map, const std::string &value);
  std::string shiftLeft(std::string hexString);
  std::string buildEtaMuon(const std::vector<L1GtMuonTemplate::ObjectParameter> *op,
                           const unsigned int &num,
                           const unsigned int &counter);
  std::string buildEtaCalo(const std::vector<L1GtCaloTemplate::ObjectParameter> *op,
                           const unsigned int &num,
                           const unsigned int &counter);
  /// \param high is true in order to build phiHigh and false in order to build phiLow
  std::string buildPhiMuon(const std::vector<L1GtMuonTemplate::ObjectParameter> *op,
                           const unsigned int &num,
                           const unsigned int &counter,
                           bool high);
  std::string buildPhiCalo(const std::vector<L1GtCaloTemplate::ObjectParameter> *op,
                           const unsigned int &num,
                           const unsigned int &counter);
  std::string buildPhiEnergySum(const std::vector<L1GtEnergySumTemplate::ObjectParameter> *op,
                                const unsigned int &num,
                                const unsigned int &counter);
