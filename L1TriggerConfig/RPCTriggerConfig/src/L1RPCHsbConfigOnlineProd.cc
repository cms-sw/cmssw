...
class L1RPCHsbConfigOnlineProd : public L1ConfigOnlineProdBase< L1RPCHsbConfigRcd,
							   L1RPCHsbConfig > {
   public:
...
L1RPCHsbConfigOnlineProd::newObject( const std::string& objectKey )
{
  ...
  int mask=3;
  for (int i=0; i<9; i++) hsbconf.push_back(mask);
  pHsbConfig->setHsb0Mask(hsbconf);
  pHsbConfig->setHsb1Mask(hsbconf);
  return pHsbConfig;
