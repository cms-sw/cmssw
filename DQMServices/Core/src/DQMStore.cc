...
MonitorElement* DQMStore::IBooker::book1DD(TString const& name, TH1D* object) { return owner_->book1DD(name, object); }
...
MonitorElement* DQMStore::IBooker::book2DD(TString const& name, TH2D* object) { return owner_->book2DD(name, object); }
...
bool DQMStore::IGetter::containsAnyMonitorable(std::string const& path) { return owner_->containsAnyMonitorable(path); }
...
bool DQMStore::checkBinningMatches(MonitorElement* me, TH1* h, unsigned const verbose) {
  if (me->getTH1()->GetNbinsX() != h->GetNbinsX() || me->getTH1()->GetNbinsY() != h->GetNbinsY() ||
      me->getTH1()->GetNbinsZ() != h->GetNbinsZ() || me->getTH1()->GetXaxis()->GetXmin() != h->GetXaxis()->GetXmin() ||
...
void DQMStore::cloneRunHistograms(uint32_t const run, uint32_t const moduleId) {
  if (verbose_ > 1) {
    std::cout << "DQMStore::cloneRunHistograms - Preparing run histograms for run: " << run << ", module: " << moduleId
              << std::endl;
  }
  ...
  for (auto const& me : boost::make_iterator_range(begin, end)) {
    ...
    if (verbose_ > 1)
      std::cout << "DQMStore::save:"
                << " run: " << me.run() << " lumi: " << me.lumi() << " lumiFlag: " << me.getLumiFlag()
                << " moduleId: " << me.moduleId() << " fullpathname: " << me.getFullname() << " flags: " << std::hex
    ...
    if (isSubdirectory(refpath, *me.data_.dirname)) {
      if (ref == SaveWithoutReference)
        ...
        if (not master or status < minStatus) {
          if (verbose_ > 1)
            std::cout << "DQMStore::save: skipping monitor element '" << me.data_.objname
                      << "' while saving, status is " << status << ", required minimum status is " << minStatus
                      << std::endl;
          continue;
        }
      }
...
void DQMStore::save(std::string const& filename,
  ...
  if (verbose_) {
    std::cout << "DQMStore::save: Opening TFile '" << filename << "' with option '" << fileupdate << "'" << std::endl;
  }
  ...
  for (auto const& dir : dirs_) {
    ...
    if (not path.empty() and not isSubdirectory(refpath, dir) and not isSubdirectory(path, dir))
      continue;
  ...
  if (verbose_) {
    std::cout << "DQMStore::save: successfully wrote " << nme << " objects from path '" << path << "/"
...
void DQMStore::savePB(std::string const& filename,
  ...
  if (verbose_) {
    std::cout << "DQMStore::savePB: Opening PBFile '" << filename << "'" << std::endl;
  }
  ...
  for (auto const& dir : dirs_) {
    // Check if we should process this directory.  We process the
    // requested part of the object tree, including references.
    if (not path.empty() and not isSubdirectory(path, dir))
      continue;
  ...
  // Maybe make some noise.
  if (verbose_) {
    std::cout << "DQMStore::savePB: successfully wrote " << nme << " objects from path '" << path << "/"
              << "' into DQM file '" << filename << "'\n";
  }
}
...
unsigned int DQMStore::readDirectory(TFile* file,
  ...
  if (prepend == s_collateDirName || prepend == s_referenceDirName) {
    ...
    if (slash != std::string::npos && slash + 10 == dirpart.size() && dirpart.compare(slash + 1, 9, "EventInfo") == 0) {
      if (verbose_)
