...
void BinLogLikelihoodRatio(long Nentries, long Nfailures, double epsilon_max, double* S_fail_obs, double* S_pass_obs) {
 ...
 +--------------------------------------------------------------------------*/
  ...
  if (n == 0)
    n = 1;  //protect against no failures: approx. 0 by 1
  if (n == N)
    n -= 1;  //protect against all failures: approx. n by (n - 1)
  double epsilon_meas = (double)n / (double)N;

  double LogQ = ((double)n) * (Log(epsilon_meas) - Log(epsilon_max)) +
...
void PoissionLogLikelihoodRatio(
    double data, double hypothesis, double epsilon_max, double epsilon_min, double* S_fail_obs, double* S_pass_obs) {
  /*--------------------------------------------------------------------------+
  ...
  *S_fail_obs = 0.0;
  if (data > tolerance_max) {
    ...
    double LogQ = (double)(Nsig + Nbak) * Log(1.0 + (double)Nsig / (double)Nbak) - (double)Nsig;
    *S_fail_obs = sqrt(2.0 * LogQ);
