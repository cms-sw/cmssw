...
DQMStoreQTestsExample::DQMStoreQTestsExample(const edm::ParameterSet& iConfig) : counter(0) {
  ...
  poMPLandauH1_ = dbe->book1D("landau_hist", "Example Landau Histogram", NBINS, XMIN, XMAX);

  int1 = dbe->bookInt("int1");
  intRef1 = dbe->bookInt("int1Ref");

  testh2f = dbe->book2D("testh2f", "testh2f histo", NBINS / 10, XMIN, XMAX, NBINS / 10, XMIN, XMAX);
  // profile histogram
  testprof = dbe->bookProfile("testprof", "testprof histo", NBINS / 10, XMIN, XMAX, NBINS / 10, XMIN, XMAX);
  // 2D profile histogram
  testprof2d = dbe->bookProfile2D(
      "testprof2d", "testprof2d histo", NBINS / 10, XMIN, XMAX, NBINS / 10, XMIN, XMAX, NBINS / 10, XMIN, XMAX);
...
DQMStoreQTestsExample::~DQMStoreQTestsExample() {
  ...
  if (yrange_test)
    delete yrange_test;
  if (deadChan_test)
    delete deadChan_test;
  if (noisyChan_test)
    delete noisyChan_test;
  if (contentSigma_test)
    delete contentSigma_test;
  if (equalH_test)
    delete equalH_test;
  if (meanNear_test)
    delete meanNear_test;
...
// (see Core/interface/QTestStatus.h)
// test_type: info message on what kind of tests are run
void DQMStoreQTestsExample::runTests(int expected_status, const string& test_type) {
  cout << " ========================================================== " << endl;
  ...
  if (expected_status && status != expected_status)
    cout << "ERROR: Comp2RefChi2 test expected status " << expected_status << ", got " << status << endl;
  status = ks_test->getStatus();
  if (expected_status && status != expected_status)
    cout << "ERROR: Comp2RefKolmogorov test expected status " << expected_status << ", got " << status << endl;

  status = xrange_test->getStatus();
  // there is no "INVALID" result when running "contents within x-range" test
  if (expected_status && expected_status != dqm::qstatus::INVALID && expected_status != status)
    cout << "ERROR: ContentsXRange test expected status " << expected_status << ", got " << status << endl;

  status = yrange_test->getStatus();
  // there is no "INVALID" result when running "contents within y-range" test
  if (expected_status && expected_status != dqm::qstatus::INVALID && expected_status != status)
    cout << "ERROR: ContentsYRange test expected status " << expected_status << ", got " << status << endl;

  status = deadChan_test->getStatus();
  // there is no "INVALID" result when running "dead channel" test
  if (expected_status && expected_status != dqm::qstatus::INVALID && expected_status != status)
    cout << "ERROR: DeadChannel test expected status " << expected_status << ", got " << status << endl;

  status = noisyChan_test->getStatus();
  // there is no "INVALID" result when running "noisy channel" test
  if (expected_status && expected_status != dqm::qstatus::INVALID && expected_status != status)
    cout << "ERROR: NoisyChannel test expected status " << expected_status << ", got " << status << endl;

  status = contentSigma_test->getStatus();
  // there is no "INVALID" result when running "content sigma" test
  if (expected_status && expected_status != dqm::qstatus::INVALID && expected_status != status)
    cout << "ERROR: ContentSigma test expected status " << expected_status << ", got " << status << endl;

  status = meanNear_test->getStatus();
  if (expected_status && expected_status != status)
    cout << "ERROR: MeanWithinExpected test expected status " << expected_status << ", got " << status << endl;
  ...
  if (expected_status && expected_status != status)
    cout << "ERROR: Comp2RefEqualH test expected status " << expected_status << ", got " << status << endl;
...
int DQMStoreQTestsExample::checkTest(QCriterion* qc) {
  ...
  cout << " Test name: " << qc->getName() << " (Algorithm: " << qc->algoName() << "), Result:";
...
void DQMStoreQTestsExample::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  testh2f->Fill(gRandom->Gaus(mean_, sigma_), gRandom->Gaus(mean_, sigma_));

  testprof->Fill(gRandom->Gaus(mean_, sigma_), TMath::Abs(gRandom->Gaus(mean_, sigma_)));

  testprof2d->Fill(
      gRandom->Gaus(mean_, sigma_), gRandom->Gaus(mean_, sigma_), TMath::Abs(gRandom->Gaus(mean_, sigma_)));
...
void DQMStoreQTestsExample::showBadChannels(QCriterion* qc) {
  ...
  while (it != badChannels.end()) {
    cout << " Channel (" << it->getBinX() << "," << it->getBinY() << "," << it->getBinZ()
         << ") Contents: " << it->getContents() << " +- " << it->getRMS() << endl;
