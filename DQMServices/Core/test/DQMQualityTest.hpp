...
public:
  ...
  void runTests(float* prob_chi2,
                ...
                float* probH_equal,
                float* prob_mean) {
    std::cout << " Running test " << chi2_test_->getName() << " (Algorithm: " << chi2_test_->getAlgoName() << ") "
              << std::endl;
    *prob_chi2 = chi2_test_->runTest(my_test);
    std::cout << " Chi2 Probability = " << *prob_chi2 << std::endl;
    //
    std::cout << " Running test " << ks_test_->getName() << " (Algorithm: " << ks_test_->getAlgoName() << ") "
              << std::endl;
    *prob_ks = ks_test_->runTest(my_test);
    std::cout << " Kolmogorov Probability = " << *prob_ks << std::endl;
    //
    std::cout << " Running test " << xrange_test_->getName() << " (Algorithm: " << xrange_test_->getAlgoName() << ") "
              << std::endl;
    *prob_xrange = xrange_test_->runTest(my_test);
    std::cout << " Entry fraction within allowed x-range = " << *prob_xrange << std::endl;
    //
    std::cout << " Running test " << yrange_test_->getName() << " (Algorithm: " << yrange_test_->getAlgoName() << ") "
              << std::endl;
    *prob_yrange = yrange_test_->runTest(my_test);
    std::cout << " Fraction of bin within allowed y-range = " << *prob_yrange << std::endl;
    showBadChannels(yrange_test_);
    //
    std::cout << " Running test " << deadChan_test_->getName() << " (Algorithm: " << deadChan_test_->getAlgoName()
              << ") " << std::endl;
    *prob_deadChan = deadChan_test_->runTest(my_test);
    std::cout << " Fraction of alive channels = " << *prob_deadChan << std::endl;
    showBadChannels(deadChan_test_);
    //
    std::cout << " Running test " << noisyChan_test_->getName() << " (Algorithm: " << noisyChan_test_->getAlgoName()
              << ") " << std::endl;
    *prob_noisyChan = noisyChan_test_->runTest(my_test);
    std::cout << " Fraction of quiet channels = " << *prob_noisyChan << std::endl;
    showBadChannels(noisyChan_test_);
    //
    std::cout << " Running test " << meanNear_test_->getName() << " (Algorithm: " << meanNear_test_->getAlgoName()
              << ") " << std::endl;
    *prob_mean = meanNear_test_->runTest(my_test);
    std::cout << " Probability that mean deviation is statistical fluctuation = " << *prob_mean << std::endl;
    //
    ...
    //	      << emu_result << std::endl;
    //
    std::cout << " Running test " << equalH_test_->getName() << " (Algorithm: " << equalH_test_->getAlgoName() << ") "
              << std::endl;
...
protected:
  DQMStore* dbe_;
  float xmin_;
  float xmax_;  // range for histograms
