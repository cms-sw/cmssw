...
static void getMEInfo(DQMStore &store, MonitorElement &me, MEInfo &info) {
  info.style = 'U';
  info.runnr = -1;
  info.system.clear();
  info.category.clear();
  ...
  if (name.compare(0, 4, "Run ") == 0 && (slash = name.find('/')) < name.size() - 1 &&
      (sys = name.find('/', slash + 1)) < name.size() - 1 && (cat = name.find('/', sys + 1)) < name.size() - 1) {
...
std::string hexlify(const std::string &x) {
  std::string result;
...
int main(int argc, char **argv) {
  ...
  if (bad) {
    std::cerr << "Usage: " << argv[0] << " [--dataset NAME]"
              << " [--step NAME]"
              << " FILE...\n";
    return 1;
  }
