...
void DQMExample_Step2::dqmEndJob(DQMStore::IBooker &ibooker_,
  ...
  // book new histogram
  h_ptRatio =
      ibooker_.book1D("ptRatio", "pt ratio pf matched objects", 50, 0., 100.);
  ...
  for (int iBin = 1; iBin < numerator->getNbinsX(); ++iBin) {
    if (denominator->getBinContent(iBin) == 0)
      ...
      h_ptRatio->setBinContent(iBin, numerator->getBinContent(iBin) /
                                         denominator->getBinContent(iBin));
  }
}
