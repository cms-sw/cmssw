...
#  def trigval<subsys>(i, p, *rows): i["HLT/<subsys>/Preselection" + p] = rows
# """
###

###---- EGAMMA selection goes here: ----

def trigvalegammaZ(i, p, *rows): i["HLT/HLTEgammaValidation/Zee Preselection/" + p] = rows
...
def hltlayoutW(i, p, *rows): i["HLT/HLTEgammaValidation/Wenu Preselection/" + p] = rows
...
def hltLayoutGammaJet(i, p, *rows): i["HLT/HLTEgammaValidation/Photon Summary" + p] = rows
...
def trigvalmuon(i, p, *rows): i["HLT/Muon/Efficiency_Layouts/" + p] = rows
...
def trigvaltau(i, p, *rows): i["HLT/TauRelVal/Summary For " + p] = rows
...
def trigvaljetmet(i, p, *rows): i["HLT/HLTJETMET/ValidationReport/" + p] = rows
...
#def trigvalbjet(i, p, *rows): i["HLT//Preselection" + p] = rows

###---- ALCA selection goes here: ----
def trigvalalca(i, p, *rows): i["HLT/AlCaEcalPi0/Preselection" + p] = rows

###---- TOP selection goes here: ----
def trigvaltopmuon(i, p, *rows): i["HLT/Top/TopValidationReport/Semileptonic_muon/" + p] = rows
...
def trigvaltopelectron(i, p, *rows): i["HLT/Top/TopValidationReport/Semileptonic_electron/" + p] = rows
...
def trigvalsusybsm(i, p, *rows): i["HLT/SusyExo/00 SusyExoValidationReport/00 Global Efficiencies/" + p] = rows
...
############# Reco Distributions
def trigvalsusybsm(i, p, *rows): i["HLT/SusyExo/00 SusyExoValidationReport/01 Reco Distributions/" + p] = rows
...
def trigvalhiggsHWW(i, p, *rows): i["HLT/Higgs/HiggsValidationReport/HWW/" + p] = rows
...
def trigvalhiggsHgg(i, p, *rows): i["HLT/Higgs/HiggsValidationReport/Hgg/" + p] = rows
...
def trigvalhiggsH2tau(i, p, *rows): i["HLT/Higgs/HiggsValidationReport/H2tau/" + p] = rows
...
def trigvalhiggsHZZ(i, p, *rows): i["HLT/Higgs/HiggsValidationReport/HZZ/" + p] = rows
...
def trigvalhiggsHtaunu(i, p, *rows): i["HLT/Higgs/HiggsValidationReport/Htaunu/" + p] = rows
