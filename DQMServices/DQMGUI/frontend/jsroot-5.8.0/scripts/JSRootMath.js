...
(function (factory) {
  ...
  JSROOT.Math.lgam = function (x) {
    ...
    if (x < -34.0) {
      ...
      if (z < 1e-300) return Number.POSITIVE_INFINITY;
    ...
    if (x < 13.0) {
      ...
      while (u < 2.0) {
        if (u < 1e-300) return Number.POSITIVE_INFINITY;
        z /= u;
        p += 1.0;
        u = x + p;
      }
    ...
    if (x > kMAXLGM) return sgngam * Number.POSITIVE_INFINITY;
    ...
    if (x >= 1000.0)
      q +=
        ...
        x;
    else q += this.Polynomialeval(p, A, 4) / x;
    return q;
  };
  ...
  JSROOT.Math.Polynomialeval = function (x, a, N) {
    if (N == 0) return a[0];
      ...
      var pom = a[0];
      for (var i = 1; i <= N; ++i) pom = pom * x + a[i];
      return pom;
    }
  };
  ...
  JSROOT.Math.ndtri = function (y0) {
    if (y0 <= 0.0) return Number.NEGATIVE_INFINITY;
    if (y0 >= 1.0) return Number.POSITIVE_INFINITY;
    ...
    if (y > 0.13533528323661269189) {
      ...
      x =
        y +
        y *
          ((y2 * this.Polynomialeval(y2, P0, 4)) /
            this.Polynomial1eval(y2, Q0, 8));
      x = x * s2pi;
      return x;
    }
  ...
  JSROOT.Math.igamc = function (a, x) {
    ...
    if (x < 1.0 || x < a) return 1.0 - JSROOT.Math.igam(a, x);
  ...
  JSROOT.Math.igami = function (a, y0) {
    ...
    for (i = 0; i < 10; ++i) {
      ...
      d = (y - y0) / d;
      if (Math.abs(d / x) < kMACHEP) return x;
      x = x - d;
    }
  ...
  JSROOT.Math.landau_pdf = function (x, xi, x0) {
    ...
    var a2 = new Array(-1.84556867, -4.284640743);
  ...
  JSROOT.Math.chisquared_pdf = function (x, r, x0) {
    ...
    return (
      Math.exp(
        (r / 2 - 1) * Math.log((x - x0) / 2) -
          (x - x0) / 2 -
          JSROOT.Math.lgamma(r / 2)
      ) / 2
    );
  };
  ...
  JSROOT.Math.BreitWigner = function (x, mean, gamma) {
    return (
      gamma / ((x - mean) * (x - mean) + (gamma * gamma) / 4) / 2 / Math.PI
    );
  };
  ...
  JSROOT.Math.gaus = function (f, x, i) {
    ...
    return (
      f.GetParValue(i + 0) *
      Math.exp(
        -0.5 * Math.pow((x - f.GetParValue(i + 1)) / f.GetParValue(i + 2), 2)
      )
    );
  };
  ...
  JSROOT.Math.gausn = function (f, x, i) {
    return (
      JSROOT.Math.gaus(f, x, i) /
      (Math.sqrt(2 * Math.PI) * f.GetParValue(i + 2))
    );
  };
  ...
  JSROOT.Math.gausxy = function (f, x, y, i) {
    ...
    return (
      ...
      Math.exp(
        -0.5 * Math.pow((y - f.GetParValue(i + 3)) / f.GetParValue(i + 4), 2)
      )
    );
  };
