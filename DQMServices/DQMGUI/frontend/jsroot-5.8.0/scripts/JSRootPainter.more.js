...
(function (factory) {
  ...
  function drawText() {
    ...
    if (text._typename == 'TLatex') {
      arg.latex = 1;
      fact = 0.9;
    } else if (text._typename == 'TMathText') {
      arg.latex = 2;
      fact = 0.8;
    }
    ...
    this.StartTextDrawing(
      text.fTextFont,
      Math.round(textsize > 1 ? textsize : textsize * Math.min(w, h) * fact)
    );
    ...
    if (!this.moveDrag)
      this.moveDrag = function (dx, dy) {
        ...
        this.draw_g.attr(
          'transform',
          'translate(' + this.pos_dx + ',' + this.pos_dy + ')'
        );
      };
    ...
    if (ellipse._typename == 'TCrown') {
      if (ellipse.fR1 <= 0) {
        ...
        if (ellipse.fPhimin == 0 && ellipse.fPhimax == 360) {
          ...
          var a1 = (ellipse.fPhimin * Math.PI) / 180,
            a2 = (ellipse.fPhimax * Math.PI) / 180,
            dx1 = Math.round(rx1 * Math.cos(a1)),
            dy1 = Math.round(ry1 * Math.sin(a1)),
            dx2 = Math.round(rx1 * Math.cos(a2)),
            dy2 = Math.round(ry1 * Math.sin(a2)),
            dx3 = Math.round(rx2 * Math.cos(a1)),
            dy3 = Math.round(ry2 * Math.sin(a1)),
            dx4 = Math.round(rx2 * Math.cos(a2)),
            dy4 = Math.round(ry2 * Math.sin(a2));
  ...
  function drawPie() {
    ...
    for (var n = 0; n < nb; n++) total += pie.fPieSlices[n].fValue;
    ...
    for (var n = 0; n < nb; n++) {
      ...
      var x2 = Math.round(rx * Math.cos(af)),
  ...
  function drawMarker() {
    ...
    if (path) this.draw_g.append('svg:path').attr('d', path).call(att.func);
  }
  ...
  function drawArrow() {
    ...
    this.wsize = Math.max(
      3,
      Math.round(
        Math.max(this.pad_width(), this.pad_height()) * arrow.fArrowSize * 0.8
      )
    );
    ...
    if (oo.indexOf('->-') >= 0) this.mid = 1;
    else if (oo.indexOf('-|>-') >= 0) this.mid = 11;
    else if (oo.indexOf('-<-') >= 0) this.mid = 2;
    else if (oo.indexOf('-<|-') >= 0) this.mid = 12;
    if (oo.lastIndexOf('>') == oo.length - 1)
      this.end =
        oo.lastIndexOf('|>') == oo.length - 2 && oo.length > 1 ? 11 : 1;
    ...
    this.rotate = function (angle, x0, y0) {
      var dx = this.wsize * Math.cos(angle),
    ...
    this.createPath = function () {
      ...
      if (this.beg)
        path +=
          this.rotate(angle - Math.PI - this.angle2, this.x1, this.y1) +
          this.rotate(angle - Math.PI + this.angle2, this.beg > 10);

      if (this.mid % 10 == 2)
        path +=
          this.rotate(
            angle - Math.PI - this.angle2,
      ...
      if (this.end)
        path +=
          this.rotate(angle - this.angle2, this.x2, this.y2) +
          this.rotate(angle + this.angle2, this.end > 10);

      return (
        'M' +
        Math.round(this.x1 + (this.beg > 10 ? dx : 0)) +
        ',' +
        Math.round(this.y1 + (this.beg > 10 ? dy : 0)) +
        'L' +
        Math.round(this.x2 - (this.end > 10 ? dx : 0)) +
        ',' +
      ...
      this.moveStart = function (x, y) {
        var fullsize = Math.sqrt(
          ...
          sz1 =
            ...
            fullsize,
          sz2 =
            Math.sqrt(Math.pow(x - this.x2, 2) + Math.pow(y - this.y2, 2)) /
            fullsize;
        if (sz1 > 0.9) this.side = 1;
    ...
    if (!this.moveEnd)
      this.moveEnd = function (not_changed) {
        ...
        if (this.side != 1)
          exec += 'SetX1(' + arrow.fX1 + ');;SetY1(' + arrow.fY1 + ');;';
        if (this.side != -1)
          exec += 'SetX2(' + arrow.fX2 + ');;SetY2(' + arrow.fY2 + ');;';
  ...
  function drawRooPlot(divid, plot, opt) {
    var painter = new JSROOT.TObjectPainter(plot),
      cnt = -1;
  ...
  TF1Painter.prototype.CreateBins = function (ignore_zoom) {
    ...
    if (tf1.fSave.length > 0 && !this.nosave) {
      ...
      for (var n = 0; n < np; ++n) {
        ...
        if (gxmin !== gxmax && (xx + 2 * dx < gxmin || xx - 2 * dx > gxmax))
          continue;
        var yy = tf1.fSave[n];
  ...
  TF1Painter.prototype.ProcessTooltip = function (pnt) {
    ...
    if (pnt === null || this.bins === null) {
      ...
      this.bins.length == 0 ||
      pnt.x < this.bins[0].grx ||
      pnt.x > this.bins[this.bins.length - 1].grx
    ) {
      cleanup = true;
    }
    ...
    var res = {
      ...
      lines: [],
      exact:
        Math.abs(bin.grx - pnt.x) < radius &&
        Math.abs(bin.gry - pnt.y) < radius,
    };

    res.changed = gbin.property('current_bin') !== best;
    res.menu = res.exact;
    res.menu_dist = Math.sqrt(
      (bin.grx - pnt.x) * (bin.grx - pnt.x) +
        (bin.gry - pnt.y) * (bin.gry - pnt.y)
    );
    ...
    if (pmain)
      res.lines.push(
        'x = ' +
          pmain.AxisAsText('x', bin.x) +
          ' y = ' +
          pmain.AxisAsText('y', bin.y)
      );
  ...
  function drawFunction(divid, tf1, opt) {
    ...
    if (JSROOT.Math !== undefined) return painter.PerformDraw();
  ...
  function TGraphPainter(graph) {
    ...
    this.has_errors =
      graph._typename == 'TGraphErrors' ||
      graph._typename == 'TGraphAsymmErrors' ||
      this.is_bent ||
      graph._typename.match(/^RooHist/);
  }
  ...
  TGraphPainter.prototype.DecodeOptions = function (opt) {
    ...
    if (res.Errors === undefined) res.Errors = this.has_errors ? 1 : 0;
    ...
    if (!res.Axis) {
      ...
      if (!pad || (pad.fPrimitives && pad.fPrimitives.arr[0] === graph))
        res.Axis = 'AXIS';
  ...
  TGraphPainter.prototype.CreateBins = function () {
    ...
    if (gr._typename == 'TGraphErrors') kind = 1;
    else if (
      gr._typename == 'TGraphAsymmErrors' ||
      gr._typename == 'TGraphBentErrors' ||
      gr._typename.match(/^RooHist/)
    )
      kind = 2;
  ...
  TGraphPainter.prototype.CreateHistogram = function (only_set_ranges) {
    ...
    this.options.HOptions =
      this.options.Axis + ';ymin:' + minimum + ';ymax:' + maximum;
  ...
  TGraphPainter.prototype.UnzoomUserRange = function (dox, doy, doz) {
    ...
    var arg = '';
    if (
      (dox && histo.fXaxis.fXmin > this.xmin) ||
      histo.fXaxis.fXmax < this.xmax
    )
      arg += 'x';
    if (
      (doy && histo.fYaxis.fXmin > this.ymin) ||
      histo.fYaxis.fXmax < this.ymax
    )
      arg += 'y';
    if (!arg) return false;
  ...
  TGraphPainter.prototype.OptimizeBins = function (maxpnt, filter_func) {
    ...
    if (step < 2) step = 2;
    var optbins = [];
    for (var n = 0; n < selbins.length; n += step) optbins.push(selbins[n]);
  ...
  TGraphPainter.prototype.TooltipText = function (d) {
    ...
    if (d && pmain) {
      ...
      if (
        ...
        pmain.x_kind == 'normal' &&
        'exlow' in d &&
        (d.exlow != 0 || d.exhigh != 0)
      )
        lines.push(
          'error x = -' +
            pmain.AxisAsText('x', d.exlow) +
            '/+' +
            pmain.AxisAsText('x', d.exhigh)
        );

      if (
        (this.options.Errors || this.options.EF > 0) &&
        pmain.y_kind == 'normal' &&
        'eylow' in d &&
        (d.eylow != 0 || d.eyhigh != 0)
      )
        lines.push(
          'error y = -' +
            pmain.AxisAsText('y', d.eylow) +
            '/+' +
  ...
  TGraphPainter.prototype.get_main = function () {
    ...
    pmain = {
      ...
      grx: function (value) {
        ...
        else value = (value - this.pad.fX1) / (this.pad.fX2 - this.pad.fX1);
        return value * this.pw;
      },
      gry: function (value) {
        if (this.pad.fLogy)
          value = value > 0 ? JSROOT.log10(value) : this.pad.fUymin;
        else value = (value - this.pad.fY1) / (this.pad.fY2 - this.pad.fY1);
  ...
  TGraphPainter.prototype.DrawBins = function () {
    ...
    if (this.options.EF) {
      ...
      var path1 = JSROOT.Painter.BuildSvgPath(
          this.options.EF > 1 ? 'bezier' : 'line',
          drawbins
        ),
        bins2 = [];
      ...
      var path2 = JSROOT.Painter.BuildSvgPath(
        this.options.EF > 1 ? 'Lbezier' : 'Lline',
        bins2
      );
    ...
    if (this.options.Line == 1 || this.options.Fill == 1 || excl_width !== 0) {
      ...
      if (!drawbins)
        drawbins = this.OptimizeBins(this.options.Curve ? 5000 : 0);
      ...
      if (this.options.Curve === 1) kind = 'bezier';
      else if (excl_width !== 0) kind += 'calc'; // we need to calculated deltas to build exclusion points
      ...
      if (excl_width !== 0) {
        ...
        var path2 = JSROOT.Painter.BuildSvgPath(
          'L' + (this.options.Curve === 1 ? 'bezier' : 'line'),
          extrabins
        );
      ...
      if (this.options.Line || this.options.Fill) {
        ...
        if (this.options.Line) elem.call(this.lineatt.func);
        else elem.style('stroke', 'none');

        if (this.options.Fill) elem.call(this.fillatt.func);
        else elem.style('fill', 'none');
      }
    ...
    if (this.options.Errors || this.options.Rect || this.options.Bar) {
      ...
      nodes = this.draw_g
        ...
        .attr('transform', function (d) {
          return 'translate(' + d.grx1 + ',' + d.gry1 + ')';
        });
    }
    ...
    if (this.options.Bar) {
      ...
      for (var i = 1; i < drawbins.length - 1; ++i)
        drawbins[i].width = Math.max(
          2,
          (drawbins[i + 1].grx1 - drawbins[i - 1].grx1) / 2 - 2
        );
      ...
      switch (drawbins.length) {
        case 0:
          break;
        case 1:
          drawbins[0].width = w / 4;
          break; // pathologic case of single bin
        case 2:
          drawbins[0].width = drawbins[1].width =
            (drawbins[1].grx1 - drawbins[0].grx1) / 2;
          break;
        default:
          drawbins[0].width = drawbins[1].width;
          drawbins[drawbins.length - 1].width =
            drawbins[drawbins.length - 2].width;
      }
    ...
    if (this.options.Errors) {
      ...
      nodes
        ...
        .attr('d', function (d) {
          d.error = true;
          return (
            (d.exlow > 0 ? mm + (d.grx0 + lw) + ',' + d.grdx0 + vleft : '') +
    ...
    if (this.options.Mark) {
      ...
      var maxnummarker = 1000000 / (this.markeratt.MarkerLength() + 7),
        step = 1;

      if (!drawbins) drawbins = this.OptimizeBins(maxnummarker);
  ...
  TGraphPainter.prototype.ExtractTooltip = function (pnt) {
    ...
    this.draw_g.selectAll('.grpoint').each(function () {
      ...
      if (d.error || d.rect || d.marker) {
        rect = {
          x1: Math.min(-painter.error_size, d.grx0, -msize),
    ...
    if (this.fillatt && this.fillatt.used && !this.fillatt.empty())
      res.color2 = this.fillatt.fillcolor();
  ...
  TGraphPainter.prototype.FindBestBin = function (pnt) {
    ...
    if (bestbin)
      bestdist = Math.sqrt(
        Math.pow(pnt.x - pmain.grx(bestbin.x), 2) +
          Math.pow(pnt.y - pmain.gry(bestbin.y), 2)
      );
      ...
      for (n = 1; n < this.bins.length; ++n) {
        ...
        if (IsInside(pnt.x, grx0, grx)) {
          ...
          if (Math.abs(grx - grx0) < 1) {
            ...
            posy = pnt.y;
            dist = IsInside(pnt.y, gry0, gry)
              ? 0
              : Math.min(Math.abs(pnt.y - gry0), Math.abs(pnt.y - gry));
          } else {
  ...
  TGraphPainter.prototype.ExtractTooltipForPath = function (pnt) {
    ...
    if (best.closeline) {
      ...
      res.exact =
        Math.abs(pnt.x - res.x) <= best.radius &&
        (Math.abs(pnt.y - res.gry1) <= best.radius ||
          Math.abs(pnt.y - res.gry2) <= best.radius);

      res.menu = res.exact;
      res.menu_dist = Math.sqrt(
        (pnt.x - res.x) * (pnt.x - res.x) +
          Math.pow(
            Math.min(Math.abs(pnt.y - res.gry1), Math.abs(pnt.y - res.gry2)),
            2
          )
      );
    }
  ...
  TGraphPainter.prototype.ShowTooltipForPath = function (hint) {
    ...
    if (hint.changed) {
      ...
      if (hint.ismark) {
        ...
        if (!hint.islines) {
          elem
            .style('stroke', hint.color1 == 'black' ? 'green' : 'black')
            .style('fill', 'none');
        } else {
          if (this.options.Line) elem.call(this.lineatt.func);
  ...
  TGraphPainter.prototype.moveEnd = function (not_changed) {
    ...
    if (exec && !not_changed) this.WebCanvasExec(exec);
  };
  ...
  TGraphPainter.prototype.FillContextMenu = function (menu) {
    ...
    if (!this.snapid)
      menu.addchk(
        this.TestEditable(),
        'Editable',
        this.TestEditable.bind(this, true)
      );
  ...
  TGraphPainter.prototype.ExecuteMenuCommand = function (method, args) {
    if (
      JSROOT.TObjectPainter.prototype.ExecuteMenuCommand.call(
        this,
        method,
        args
      )
  ...
  function drawGraph(divid, graph, opt) {
    ...
    if (!painter.main_painter() && painter.options.HOptions) {
      JSROOT.draw(
        divid,
        painter.CreateHistogram(),
        painter.options.HOptions,
  ...
  TGraphPolargramPainter.prototype = Object.create(
    JSROOT.TooltipHandler.prototype
  );

  TGraphPolargramPainter.prototype.translate = function (
    angle,
    radius,
    keep_float
  ) {
    var _rx = this.r(radius),
      _ry = (_rx / this.szx) * this.szy,
      pos = {
  ...
  TGraphPolargramPainter.prototype.GetFrameRect = function () {
    ...
    rect.szx = Math.round(
      Math.max(0.1, 0.5 - Math.max(pad.fLeftMargin, pad.fRightMargin)) * w
    );
    rect.szy = Math.round(
      Math.max(0.1, 0.5 - Math.max(pad.fBottomMargin, pad.fTopMargin)) * h
    );
  ...
  TGraphPolargramPainter.prototype.MouseWheel = function () {
    ...
    var delta = d3.event.wheelDelta
      ? -d3.event.wheelDelta
      : d3.event.deltaY || d3.event.detail;
    if (!delta) return;

    delta = delta < 0 ? -0.2 : 0.2;

    var rmin = this.scale_rmin,
      rmax = this.scale_rmax,
      range = rmax - rmin;
  ...
  TGraphPolargramPainter.prototype.Redraw = function () {
    ...
    this.r = d3
      .scaleLinear()
      .domain([this.scale_rmin, this.scale_rmax])
      .range([0, this.szx]);
    ...
    if (!this.gridatt)
      this.gridatt = new JSROOT.TAttLineHandler({
        color: polar.fLineColor,
        style: 2,
        width: 1,
      });
    ...
    this.ndig =
      range <= 0 ? -3 : Math.round(JSROOT.log10(ticks.length / range));
    ...
    if (
      ticks[ticks.length - 1] < polar.fRwrmax &&
    ...
    this.StartTextDrawing(
      polar.fRadialLabelFont,
      Math.round(polar.fRadialTextSize * this.szy * 2)
    );
    ...
    for (var n = 0; n < ticks.length; ++n) {
      ...
      if (n < ticks.length - 1 || !exclude_last)
        this.DrawText({
          align: 23,
          x: Math.round(rx),
          y: Math.round(polar.fRadialTextSize * this.szy * 0.5),
          text: this.format(ticks[n]),
          color: this.get_color[polar.fRadialLabelColor],
          latex: 0,
        });
    ...
    for (var n = 0; n < nmajor; ++n) {
      ...
      this.DrawText({
        ...
        y: Math.round(
          (this.szy + (fontsize / this.szx) * this.szy) * Math.sin(angle)
        ),
        text: lbls[n],
        color: this.get_color[polar.fPolarLabelColor],
        latex: 1,
      });
    }
    ...
    d3.select(interactive.node().parentNode).attr(
      'transform',
      this.draw_g.attr('transform')
    );
  ...
  TGraphPolarPainter.prototype.DrawBins = function () {
    ...
    if (mpath)
      this.draw_g.append('svg:path').attr('d', mpath).call(this.markeratt.func);
  };
  ...
  TGraphPolarPainter.prototype.ExtractTooltip = function (pnt) {
    ...
    for (var n = 0; n < graph.fNpoints; ++n) {
      ...
      var dist2 =
        (pos.x - pnt.x) * (pos.x - pnt.x) + (pos.y - pnt.y) * (pos.y - pnt.y);
      if (dist2 < best_dist2) {
        best_dist2 = dist2;
        bestindx = n;
        bestpos = pos;
    ...
    var res = {
      ...
      y: bestpos.y,
      color1:
        this.markeratt && this.markeratt.used
    ...
    if (graph.fEX && graph.fEX[bestindx])
      res.lines.push(
        'error phi = ' + main.AxisAsText('phi', graph.fEX[bestindx])
      );
  ...
  TSplinePainter.prototype.Eval = function (knot, x) {
    ...
    if (knot._typename == 'TSplinePoly5')
      return (
        ...
        dx *
          (knot.fB +
            dx * (knot.fC + dx * (knot.fD + dx * (knot.fE + dx * knot.fF))))
      );
  ...
  TSplinePainter.prototype.ProcessTooltip = function (pnt) {
    ...
    if (pnt === null || !spline || !main) {
      ...
      if (
        indx < spline.fN - 1 &&
        Math.abs(spline.fPoly[indx + 1].fX - xx) < Math.abs(xx - knot.fX)
      )
        knot = spline.fPoly[++indx];
    ...
    res.menu = res.exact;
    res.menu_dist = Math.sqrt(
      (res.x - pnt.x) * (res.x - pnt.x) + (res.y - pnt.y) * (res.y - pnt.y)
    );

    if (res.changed)
      gbin
        .attr('cx', Math.round(res.x))
  ...
  TGraphTimePainter.prototype.DrawPrimitives = function (
    indx,
    callback,
    ppainter
  ) {
  ...
  TGraphTimePainter.prototype.ContineDrawing = function () {
    ...
    if (this.wait_animation_frame) {
      ...
      this.running_timeout = setTimeout(
        this.ContineDrawing.bind(this),
        sleeptime
      );
    }
  };
...
*/
  ...
  TEfficiencyPainter.prototype.Normal = function (
    total,
    passed,
    level,
    bUpper
    ...
    if (bUpper) return average + delta > 1 ? 1.0 : average + delta;
  ...
  TEfficiencyPainter.prototype.FillGraph = function (gr, opt) {
    ...
    for (var n = 0; n < npoints; ++n) {
      ...
      gr.fEXlow[j] =
        eff.fTotalHistogram.fXaxis.GetBinCenter(n + 1) -
        eff.fTotalHistogram.fXaxis.GetBinLowEdge(n + 1);
      gr.fEXhigh[j] =
        eff.fTotalHistogram.fXaxis.GetBinLowEdge(n + 2) -
        eff.fTotalHistogram.fXaxis.GetBinCenter(n + 1);
  ...
  JSROOT.Painter.drawEfficiency = function (divid, eff, opt) {
    if (
      !eff ||
      !eff.fTotalHistogram ||
      eff.fTotalHistogram._typename.indexOf('TH1') != 0
    )
      return null;
  ...
  TMultiGraphPainter.prototype.UpdateObject = function (obj) {
    ...
    if (this.firstpainter) {
      ...
      if (this.autorange && !histo) histo = this.ScanGraphsRange(graphs);
  ...
  TMultiGraphPainter.prototype.ScanGraphsRange = function (graphs, histo, pad) {
    ...
    if (histo) {
      ...
      if (
        graphs.arr[0] &&
        graphs.arr[0].fHistogram &&
        graphs.arr[0].fHistogram.fXaxis.fTimeDisplay
      ) {
        time_display = true;
      ...
      if (minimum < 0 && rw.ymin >= 0) minimum = 0;
      if (maximum > 0 && rw.ymax <= 0) maximum = 0;
    }

    if (uxmin < 0 && rw.xmin >= 0) uxmin = logx ? 0.9 * rw.xmin : 0;
    if (uxmax > 0 && rw.xmax <= 0) uxmax = logx ? 1.1 * rw.xmax : 0;

    if (mgraph.fMinimum != -1111) rw.ymin = minimum = mgraph.fMinimum;
    if (mgraph.fMaximum != -1111) rw.ymax = maximum = mgraph.fMaximum;
    ...
    if (uxmin <= 0 && logx) uxmin = uxmax > 1000 ? 1 : 0.001 * uxmax;
  ...
  TMultiGraphPainter.prototype.DrawNextFunction = function (indx, callback) {
    ...
    JSROOT.draw(
      this.divid,
      mgraph.fFunctions.arr[indx],
      mgraph.fFunctions.opt[indx],
      this.DrawNextFunction.bind(this, indx + 1, callback)
    );
  };

  TMultiGraphPainter.prototype.DrawNextGraph = function (
    indx,
    opt,
    subp,
    used_timeout
    ...
    if (indx % 500 === 499 && !used_timeout)
      return setTimeout(
        this.DrawNextGraph.bind(this, indx, opt, null, true),
        0
      );
  ...
  JSROOT.Painter.drawMultiGraph = function (divid, mgraph, opt) {
    ...
    var d = new JSROOT.DrawOptions(opt);
    d.check('3D');
    d.check('FB'); // no 3D supported, FB not clear
  ...
  function drawWebPainting(divid, obj, opt) {
    ...
    painter.ReadAttr = function (str, names) {
      ...
      for (var k = 0; k < names.length; ++k) {
        ...
        obj[names[k]] = parseInt(
          str.substr(lastp + 1, p > lastp ? p - lastp - 1 : undefined)
        );
        lastp = p;
      }
      return obj;
    ...
    painter.Redraw = function () {
      ...
      function check_attributes(painter, kind) {
        ...
        switch (kind) {
          case 'f':
            lastpath.call(painter.fillatt.func).attr('stroke', 'none');
            break;
          case 'l':
            lastpath.call(painter.lineatt.func).attr('fill', 'none');
            break;
          case 'm':
            lastpath.call(painter.markeratt.func);
            break;
        }
      }
      ...
      for (k = 0; k < arr.length; ++k) {
        ...
        switch (oper) {
          ...
          case 'y':
            this.createAttFill({
              attr: this.ReadAttr(arr[k], ['fFillColor', 'fFillStyle']),
              force: true,
            });
            check_attributes();
            continue;
          case 'x':
            this.createAttMarker({
              attr: this.ReadAttr(arr[k], [
                'fMarkerColor',
                'fMarkerStyle',
                'fMarkerSize',
          ...
          case 't': {
            if (attr.fTextSize) {
              ...
              this.DrawText({
                ...
                rotate: -angle,
                text: txt,
                color: JSROOT.Painter.root_colors[attr.fTextColor],
                latex: 0,
                draw_g: group,
              });
  ...
  JSROOT.Painter.drawJSImage = function (divid, obj, opt) {
    ...
    var img = main
      .append('img')
      .attr('src', obj.fName)
      .attr('title', obj.fTitle || obj.fName);
  ...
  JSROOT.Painter.CreateBranchItem = function (
    node,
    branch,
    tree,
    parent_branch
    ...
    if (branch._typename === 'TBranchElement')
      subitem._title +=
        ' from ' + branch.fClassName + ';' + branch.fClassVersion;
    ...
    if (nb_branches > 0) {
      ...
      subitem._expand = function (bnode, bobj) {
        ...
        if (
          bobj.fLeaves &&
          bobj.fLeaves.arr.length === 1 &&
          (bobj.fType === JSROOT.BranchType.kClonesNode ||
            bobj.fType === JSROOT.BranchType.kSTLNode)
        ) {
        ...
        for (var i = 0; i < bobj.fBranches.arr.length; ++i)
          JSROOT.Painter.CreateBranchItem(
            bnode,
            bobj.fBranches.arr[i],
            bobj.$tree,
            bobj
          );

        var object_class = JSROOT.IO.GetBranchObjectClass(
            bobj,
            bobj.$tree,
            true
          ),
  ...
  JSROOT.Painter.TreeHierarchy = function (node, obj) {
    if (
      obj._typename != 'TTree' &&
      obj._typename != 'TNtuple' &&
      obj._typename != 'TNtupleD'
    )
      return false;
  ...
  JSROOT.Painter.drawTree = function (divid, obj, opt) {
    ...
    if (obj._typename == 'TBranchFunc') {
      ...
      if (opt && opt.indexOf('dump') == 0) args.expr += '>>' + opt;
      else if (opt) args.expr += opt;
      tree = obj.branch.$tree;
    } else if (obj.$branch) {
      // this is drawing of the single leaf from the branch
      args = { expr: '.' + obj.fName + (opt || ''), branch: obj.$branch };
      if (
        args.branch.fType === JSROOT.BranchType.kClonesNode ||
        args.branch.fType === JSROOT.BranchType.kSTLNode
      ) {
      ...
      if (
        !opt &&
        obj.fStreamerType &&
        obj.fStreamerType !== JSROOT.IO.kTString &&
        obj.fStreamerType >= JSROOT.IO.kObject &&
        obj.fStreamerType <= JSROOT.IO.kAnyP
      )
        opt = 'dump';
    ...
    tree.Draw(args, function (histo, hopt, intermediate) {
      ...
      if (!args.player) drawid = divid;
      else if (args.create_player === 2) drawid = painter.drawid;

      if (drawid)
        return JSROOT.redraw(
          drawid,
          histo,
          hopt,
          intermediate ? null : callback
        );

      if (args.create_player === 1) {
      ...
      JSROOT.AssertPrerequisites('jq2d', function () {
        ...
        JSROOT.redraw(
          ...
          histo,
          hopt,
          args.player_intermediate ? null : callback
        );
