...
(function (factory) {
  ...
  JSROOT.TObjectPainter.prototype.v7EvalAttr = function (name, dflt) {
    ...
    if (this.rstyle && this.rstyle.fBlocks) {
      ...
      for (var k = 0; k < blks.length; ++k) {
        ...
        var match =
          (this.csstype && block.selector == this.csstype) ||
  ...
  JSROOT.TObjectPainter.prototype.v7EvalColor = function (name, dflt) {
    ...
    if (rgb) return '#' + rgb + this.v7EvalAttr(name + '_a', '');
  ...
  TAxisPainter.prototype.SetAxisConfig = function (
    name,
    kind,
    func,
    min,
  ...
  TAxisPainter.prototype.format10Exp = function (order, value) {
    ...
    if (value) {
      ...
      if (value != 0 && value != 1)
        res = value.toString() + (JSROOT.gStyle.Latex ? '#times' : 'x');
    }
    res += '10';
  ...
  TAxisPainter.prototype.CreateFormatFuncs = function () {
    ...
    var ndiv = 508;
    if (is_gaxis) ndiv = axis.fNdiv;
    else if (axis) ndiv = Math.max(axis.fNdivisions, 4);
    ...
    if (this.kind == 'time') {
      ...
      this.moreloglabels = axis
        ? axis.TestBit(JSROOT.EAxisBits.kMoreLogLabels)
        : false;
      ...
      this.format = function (d, asticks, notickexp_fmt) {
        var val = parseFloat(d),
          rnd = Math.round(val);
        if (!asticks)
          return rnd === val && Math.abs(rnd) < 1e9
            ? rnd.toString()
            : JSROOT.FFormat(val, notickexp_fmt || JSROOT.gStyle.fStatFormat);
        ...
        if (this.moreloglabels || Math.abs(vlog - Math.round(vlog)) < 0.001) {
          ...
          return vlog < 0
            ? val.toFixed(Math.round(-vlog + 0.5))
            : val.toFixed(0);
        }
        return null;
      };
      ...
      var scale_range = this.scale_max - this.scale_min;
      if (this.nticks > scale_range) this.nticks = Math.round(scale_range);
      this.nticks2 = 1;
      ...
      if (axis && axis.fNbins && axis.fLabels) {
        if (
          axis.fNbins != Math.round(axis.fXmax - axis.fXmin) ||
          axis.fXmin != 0 ||
          axis.fXmax != axis.fNbins
        ) {
      ...
      this.format = function (d) {
        ...
        if (!this.regular_labels)
          indx =
            ((indx - this.axis.fXmin) / (this.axis.fXmax - this.axis.fXmin)) *
            this.axis.fNbins;
      ...
      this.format = function (d, asticks, fmt) {
        ...
        if (asticks)
          return this.ndig > 10
            ? val.toExponential(this.ndig - 11)
            : val.toFixed(this.ndig);
  ...
  TAxisPainter.prototype.CreateTicks = function (
    only_major_as_array,
    optionNoexp,
    optionNoopt,
    optionInt
    ...
    handle.minor = handle.middle = handle.major = this.ProduceTicks(
      this.nticks
    );

    if (only_major_as_array) {
      var res = handle.major,
        delta = (this.scale_max - this.scale_min) * 1e-5;
      if (res[0] > this.scale_min + delta) res.unshift(this.scale_min);
      if (res[res.length - 1] < this.scale_max - delta)
    ...
    if (this.kind == 'labels' && !this.regular_labels) {
      ...
      for (var n = 0; n < this.axis.fNbins; ++n) {
        var x =
          this.axis.fXmin +
          (n / this.axis.fNbins) * (this.axis.fXmax - this.axis.fXmin);
        if (x >= this.scale_min && x < this.scale_max) handle.lbl_pos.push(x);
      }
    }

    if (this.nticks2 > 1) {
      handle.minor = handle.middle = this.ProduceTicks(
        handle.major.length,
        this.nticks2
      );
      ...
      if (
        handle.middle.length <= handle.major.length ||
        handle.middle.length > gr_range / 3.5
      ) {
        handle.minor = handle.middle = handle.major;
      } else if (this.nticks3 > 1 && this.kind !== 'log') {
        handle.minor = this.ProduceTicks(handle.middle.length, this.nticks3);
        if (
          handle.minor.length <= handle.middle.length ||
          handle.minor.length > gr_range / 1.7
        )
    ...
    handle.next = function (doround) {
      ...
      if (
        this.nmiddle < this.middle.length &&
        Math.abs(this.grpos - this.func(this.middle[this.nmiddle])) < 1
      ) {
        this.nmiddle++;
        this.kind = 2;
      }
      ...
      if (
        this.nmajor < this.major.length &&
        Math.abs(this.grpos - this.func(this.major[this.nmajor])) < 1
      ) {
        this.nmajor++;
        this.kind = 1;
      }
    ...
    if (this.kind == 'normal' && handle.major.length > 0) {
      ...
      if (!optionNoexp) {
        var maxtick = Math.max(
            Math.abs(handle.major[0]),
            Math.abs(handle.major[handle.major.length - 1])
          ),
          mintick = Math.min(
            Math.abs(handle.major[0]),
            Math.abs(handle.major[handle.major.length - 1])
      ...
      if (optionInt) {
        if (this.order)
          console.warn(
            'Axis painter - integer labels are configured, but axis order ' +
              this.order +
              ' is preferable'
          );
        if (this.ndig)
          console.warn(
            'Axis painter - integer labels are configured, but ' +
              this.ndig +
  ...
  TAxisPainter.prototype.AddTitleDrag = function (
    title_g,
    vertical,
    offset_k,
    reverse,
    ...
    drag_move
      .on(prefix + 'start', function () {
        ...
        else alt_pos = Math.round(axis_length / 2);
      ...
      .on('drag', function () {
        ...
        if (vertical) {
          set_x = acc_x;
          if (Math.abs(acc_y - set_y) > Math.abs(acc_y - alt_pos))
            set_y = alt_pos;
        } else {
          set_y = acc_y;
          if (Math.abs(acc_x - set_x) > Math.abs(acc_x - alt_pos))
            set_x = alt_pos;
        }
      ...
      .on(prefix + 'end', function () {
        ...
        title_g.property('shift_x', new_x).property('shift_y', new_y);
  ...
  TAxisPainter.prototype.DrawAxis = function (
    vertical,
    layer,
    w,
    h,
    ...
    else if (this.invert_side) second_shift = -second_shift;
    ...
    if (is_gaxis && axis.TestBit(JSROOT.EAxisBits.kTickMinus))
      optionMinus = true;
    ...
    if (optionPlus && optionMinus) {
      ...
      side = myXor(reverse, vertical) ? 1 : -1;
    } else if (optionPlus) {
      side = myXor(reverse, vertical) ? -1 : 1;
    }
    ...
    if (this.max_tick_size && tickSize > this.max_tick_size)
      tickSize = this.max_tick_size;
    ...
    while (handle.next(true)) {
      var h1 = Math.round(tickSize / 4),
        h2 = 0;

      if (handle.kind < 3) h1 = Math.round(tickSize / 2);

      if (handle.kind == 1) {
        // if not showing labels, not show large tick
        if (!('format' in this) || this.format(handle.tick, true) !== null)
          h1 = tickSize;
        this.ticks.push(handle.grpos); // keep graphical positions of major ticks
      }

      if (ticks_plusminus > 0) h2 = -h1;
      else if (side < 0) {
        h2 = -h1;
        h1 = 0;
      } else {
      ...
      if (res.length == 0) {
        res = vertical
          ...
          : 'M' + handle.grpos + ',' + -h1;
        res2 = vertical
          ? 'M' + (second_shift - h1) + ',' + handle.grpos
          : 'M' + handle.grpos + ',' + (second_shift + h1);
      } else {
        res += vertical
          ? 'm' + (h1 - lasth) + ',' + (handle.grpos - lastpos)
          : 'm' + (handle.grpos - lastpos) + ',' + (lasth - h1);
        res2 += vertical
          ? 'm' + (lasth - h1) + ',' + (handle.grpos - lastpos)
          : 'm' + (handle.grpos - lastpos) + ',' + (h1 - lasth);
      }
    ...
    var labelsize = Math.round(
      axis.fLabelSize < 1
        ? axis.fLabelSize * text_scaling_size
        : axis.fLabelSize
    );
    ...
    if (!disable_axis_drawing && !optionUnlab) {
      var label_color = this.get_color(axis.fLabelColor),
        labeloffset = Math.round(
          axis.fLabelOffset * text_scaling_size /*+ 0.5*labelsize*/
        ),
      ...
      if (this.lbls_both_sides)
        label_g.push(
          axis_g
            ...
            .attr(
              'transform',
              vertical ? 'translate(' + w + ',0)' : 'translate(0,' + -h + ')'
            )
        );
      ...
      for (var lcnt = 0; lcnt < label_g.length; ++lcnt) {
        ...
        var lastpos = 0,
          fix_coord = vertical
            ? -labeloffset * side
            : (labeloffset + 2) * side + ticks_plusminus * tickSize;
        ...
        for (var nmajor = 0; nmajor < lbl_pos.length; ++nmajor) {
          ...
          var pos = Math.round(this.func(lbl_pos[nmajor])),
            gap_before =
              nmajor > 0
                ? Math.abs(Math.round(pos - this.func(lbl_pos[nmajor - 1])))
                : 0,
            gap_after =
              nmajor < lbl_pos.length - 1
                ? Math.abs(Math.round(this.func(lbl_pos[nmajor + 1]) - pos))
                : 0;
          ...
          if (
            textwidth &&
            ((!vertical && !rotate_lbls) || (vertical && rotate_lbls)) &&
            this.kind != 'log'
          ) {
            var maxwidth = gap_before * 0.45 + gap_after * 0.45;
            if (!gap_before) maxwidth = 0.9 * gap_after;
            else if (!gap_after) maxwidth = 0.9 * gap_before;
            textscale = Math.min(textscale, maxwidth / textwidth);
          } else if (
            vertical &&
            max_text_width &&
            !lcnt &&
            max_text_width - labeloffset > 20 &&
            textwidth > max_text_width - labeloffset
          ) {
            textscale = Math.min(
              textscale,
              (max_text_width - labeloffset) / textwidth
            );
          }
          ...
          this.DrawText({
            ...
            align: vertical
              ? side < 0
                ? 30
                : 10
              : myXor(this.has_obstacle, side < 0)
              ? 13
              : 10,
            latex: 1,
        ...
        if (lbls_tilt)
          label_g[lcnt].selectAll('text').each(function () {
            var txt = d3.select(this),
              tr = txt.attr('transform');
            txt
              .attr('transform', tr + ' rotate(25)')
    ...
    if (JSROOT.gStyle.Zooming && !this.disable_zooming) {
      ...
      if (vertical)
        ...
        r.attr('x', 0)
          .attr('y', side > 0 ? 0 : -labelsize - 3)
          .attr('width', w)
          .attr('height', labelsize + 3);
    }
    ...
    if (axis.fTitle.length > 0 && !disable_axis_drawing) {
      var title_g = axis_g.append('svg:g').attr('class', 'axis_title'),
        title_fontsize =
          axis.fTitleSize >= 1
            ? axis.fTitleSize
            : Math.round(axis.fTitleSize * text_scaling_size),
        title_offest_k =
          1.6 *
          (axis.fTitleSize < 1
            ? axis.fTitleSize
            : axis.fTitleSize / (this.pad_height('') || 10)),
      ...
      if (vertical) {
        ...
        if (
          ...
          if (shift_x < rect.width - tickSize)
            shift_x = Math.round(rect.width - tickSize);
        }

        shift_y = Math.round(center ? h / 2 : reverse ? h : 0);

        this.DrawText({
          align: (center ? 'middle' : myxor ? 'begin' : 'end') + ';middle',
      ...
      this.FinishTextDrawing(title_g, function () {
        if (axis_rect) {
          ...
          shift_x =
            side > 0
              ? Math.round(
                  axis_rect.left - title_rect.right - title_fontsize * 0.3
                )
              : Math.round(
                  axis_rect.right - title_rect.left + title_fontsize * 0.3
                );
        }
  ...
  TFramePainter.prototype.DrawGrids = function () {
    ...
    var h = this.frame_height(),
      ...
      grid_color =
        JSROOT.gStyle.fGridColor > 0
          ? this.get_color(JSROOT.gStyle.fGridColor)
          : 'black';

    if (grid_style < 0 || grid_style >= JSROOT.Painter.root_line_styles.length)
      grid_style = 11;
    ...
    if (this.x_handle) {
      ...
      for (var n = 0; n < this.x_handle.ticks.length; ++n)
        if (this.swap_xy) grid += 'M0,' + this.x_handle.ticks[n] + 'h' + w;
        else grid += 'M' + this.x_handle.ticks[n] + ',0v' + h;
      ...
      if (grid.length > 0)
        layer
          ...
          .style(
            'stroke-dasharray',
            JSROOT.Painter.root_line_styles[grid_style]
          );
    }
    ...
    if (this.y_handle) {
      ...
      for (var n = 0; n < this.y_handle.ticks.length; ++n)
        if (this.swap_xy) grid += 'M' + this.y_handle.ticks[n] + ',0v' + h;
        else grid += 'M0,' + this.y_handle.ticks[n] + 'h' + w;
  ...
  TFramePainter.prototype.DrawAxes = function (shrink_forbidden) {
    ...
    if (!disable_axis_draw) {
      draw_horiz.DrawAxis(
        ...
        false,
        show_second_ticks ? -h : 0,
        disable_axis_draw
      );

      draw_vertical.DrawAxis(
        true,
        layer,
        w,
        h,
    ...
    if (
      !shrink_forbidden &&
      JSROOT.gStyle.CanAdjustFrame &&
      !disable_axis_draw
    ) {
      var shrink = 0,
        ypos = draw_vertical.position;
  ...
  TFramePainter.prototype.CleanXY = function () {
    ...
    delete this.x;
    delete this.grx;
    delete this.ConvertX;
    delete this.RevertX;
    delete this.y;
    delete this.gry;
    delete this.ConvertY;
    delete this.RevertY;
    delete this.z;
    delete this.grz;
  };
  ...
  TFramePainter.prototype.CleanFrameDrawings = function () {
    ...
    if (typeof this.Create3DScene === 'function') this.Create3DScene(-1);
  ...
  TFramePainter.prototype.Redraw = function () {
    ...
    this.draw_g.attr('x', lm).attr('y', tm).attr('width', w).attr('height', h);
    ...
    if (tooltip_rect.property('handlers_set') != handlers_set) {
      var close_handler = handlers_set
          ? this.ProcessTooltipEvent.bind(this, null)
          : null,
        mouse_handler = handlers_set
          ? this.ProcessTooltipEvent.bind(this, { handler: true, touch: false })
          : null;

      tooltip_rect
        .property('handlers_set', handlers_set)
        .on('mouseenter', mouse_handler)
        .on('mousemove', mouse_handler)
        .on('mouseleave', close_handler);

      if (JSROOT.touches) {
        var touch_handler = handlers_set
          ? this.ProcessTooltipEvent.bind(this, { handler: true, touch: true })
          : null;
    ...
    tooltip_rect.attr('x', 0).attr('y', 0).attr('width', w).attr('height', h);
    ...
    if (
      !hintsg.empty() &&
      this.IsTooltipAllowed() &&
      hintsg.property('hints_pad') == this.pad_name
    )
      setTimeout(
        this.ProcessTooltipEvent.bind(this, hintsg.property('last_point')),
        10
      );
  };
  ...
  TFramePainter.prototype.ProcessFrameClick = function (pnt, dblckick) {
    ...
    if (!dblckick)
      pp.SelectObjectPainter(exact ? exact.painter : this, {
        x: pnt.x + (this._frame_x || 0),
        y: pnt.y + (this._frame_y || 0),
      });
  ...
  TFramePainter.prototype.Zoom = function (xmin, xmax, ymin, ymax, zmin, zmax) {
    ...
    if (xmin === 'x') {
      ...
      zmin = xmax;
      zmax = ymin;
      xmin = xmax = ymin = undefined;
    }

    var zoom_x = xmin !== xmax,
      zoom_y = ymin !== ymax,
      zoom_z = zmin !== zmax,
      unzoom_x = false,
      unzoom_y = false,
  ...
  TFramePainter.prototype.mouseDoubleClick = function () {
    ...
    var kind = 'xyz';
    if (!valid_x) kind = this.swap_xy ? 'x' : 'y';
    else if (!valid_y) kind = this.swap_xy ? 'y' : 'x';
  ...
  TFramePainter.prototype.startRectSel = function () {
    ...
    this.zoom_curr = [
      Math.max(0, Math.min(w, this.zoom_origin[0])),
    ...
    d3.select(window)
      .on('mousemove.zoomRect', this.moveRectSel.bind(this))
  ...
  TFramePainter.prototype.moveRectSel = function () {
    ...
    this.zoom_rect
      .attr('x', Math.min(this.zoom_origin[0], this.zoom_curr[0]))
  ...
  TFramePainter.prototype.endRectSel = function () {
    ...
    var xmin,
      ...
      ymax,
      isany = false,
      idx = this.swap_xy ? 1 : 0,
      idy = 1 - idx;

    if (
      changed[idx] &&
      Math.abs(this.zoom_curr[idx] - this.zoom_origin[idx]) > 10
    ) {
      xmin = Math.min(
        this.RevertX(this.zoom_origin[idx]),
        this.RevertX(this.zoom_curr[idx])
      );
      xmax = Math.max(
        this.RevertX(this.zoom_origin[idx]),
        this.RevertX(this.zoom_curr[idx])
    ...
    if (
      changed[idy] &&
      Math.abs(this.zoom_curr[idy] - this.zoom_origin[idy]) > 10
    ) {
      ymin = Math.min(
        this.RevertY(this.zoom_origin[idy]),
        this.RevertY(this.zoom_curr[idy])
      );
      ymax = Math.max(
        this.RevertY(this.zoom_origin[idy]),
        this.RevertY(this.zoom_curr[idy])
    ...
    var kind = this.zoom_kind,
      pnt =
        kind === 1 ? { x: this.zoom_origin[0], y: this.zoom_origin[1] } : null;
  ...
  TFramePainter.prototype.startTouchZoom = function () {
    ...
    if (arr.length == 1) {
      ...
      var now = new Date(),
        diff = now.getTime() - this.last_touch.getTime();
      this.last_touch = now;
      ...
      if (
        ...
        this.svg_frame().on('touchcancel', null).on('touchend', null, true);
      } else if (JSROOT.gStyle.ContextMenu) {
        this.zoom_curr = arr[0];
        this.svg_frame()
          .on('touchcancel', this.endTouchSel.bind(this))
    ...
    if (arr.length != 2 || !JSROOT.gStyle.Zooming || !JSROOT.gStyle.ZoomTouch)
      return;
    ...
    this.svg_frame().on('touchcancel', null).on('touchend', null);
    ...
    d3.select(window)
      .on('touchmove.zoomRect', this.moveTouchSel.bind(this))
  ...
  TFramePainter.prototype.moveTouchSel = function () {
    ...
    if (arr.length != 2) return this.clearInteractiveElements();
    ...
    if (
      this.zoom_origin[0] - this.zoom_curr[0] > 10 ||
  ...
  TFramePainter.prototype.endTouchSel = function () {
    ...
    if (this.zoom_kind === 0) {
      ...
      if (diff > 500 && diff < 2000 && !this.frame_painter().IsTooltipShown()) {
        this.ShowContextMenu('main', {
          clientX: this.zoom_curr[0],
          clientY: this.zoom_curr[1],
        });
        this.last_touch = new Date(0);
      } else {
        this.clearInteractiveElements();
    ...
    if (
      changed[xid] &&
      Math.abs(this.zoom_curr[xid] - this.zoom_origin[xid]) > 10
    ) {
      xmin = Math.min(
        this.RevertX(this.zoom_origin[xid]),
        this.RevertX(this.zoom_curr[xid])
      );
      xmax = Math.max(
        this.RevertX(this.zoom_origin[xid]),
        this.RevertX(this.zoom_curr[xid])
    ...
    if (
      changed[yid] &&
      Math.abs(this.zoom_curr[yid] - this.zoom_origin[yid]) > 10
    ) {
      ymin = Math.min(
        this.RevertY(this.zoom_origin[yid]),
        this.RevertY(this.zoom_curr[yid])
      );
      ymax = Math.max(
        this.RevertY(this.zoom_origin[yid]),
        this.RevertY(this.zoom_curr[yid])
  ...
  TFramePainter.prototype.ShowContextMenu = function (kind, evnt, obj) {
    ...
    var menu_painter = this,
      frame_corner = false,
      fp = this; // object used to show context menu
    ...
    if (!evnt) {
      ...
      if (kind === undefined) {
        ...
        if (tch.length === 1) pnt = { x: tch[0][0], y: tch[0][1], touch: true };
        else if (ms.length === 2) pnt = { x: ms[0], y: ms[1], touch: false };
    ...
    JSROOT.Painter.createMenu(menu_painter, function (menu) {
      ...
      if (
        fp &&
        (!domenu || (frame_corner && kind !== 'frame' && fp != menu.painter))
      )
      ...
      if (domenu)
        menu.painter.FillObjectExecMenu(menu, kind, function () {
          ...
          menu.show(
            menu.painter.ctx_menu_evnt,
            menu.painter.SwitchTooltip.bind(menu.painter, true)
          );
        });
    }); // end menu creation
  };
  ...
  TFramePainter.prototype.FillContextMenu = function (menu, kind, obj) {
    ...
    if (kind == 'x' || kind == 'y') {
      ...
      if (faxis) {
        menu.addchk(
          faxis.TestBit(JSROOT.EAxisBits.kMoreLogLabels),
          'More log',
          function () {
            faxis.InvertBit(JSROOT.EAxisBits.kMoreLogLabels);
            this.RedrawPad();
          }
        );
        menu.addchk(
          faxis.TestBit(JSROOT.EAxisBits.kNoExponent),
          'No exponent',
          function () {
            faxis.InvertBit(JSROOT.EAxisBits.kNoExponent);
            this.RedrawPad();
          }
        );
        menu.add('sub:Labels');
        menu.addchk(
          faxis.TestBit(JSROOT.EAxisBits.kCenterLabels),
          'Center',
          function () {
            faxis.InvertBit(JSROOT.EAxisBits.kCenterLabels);
            this.RedrawPad();
          }
        );
        menu.addchk(
          faxis.TestBit(JSROOT.EAxisBits.kLabelsVert),
          'Rotate',
          function () {
            faxis.InvertBit(JSROOT.EAxisBits.kLabelsVert);
            this.RedrawPad();
          }
        );
        this.AddColorMenuEntry(menu, 'Color', faxis.fLabelColor, function (
          arg
        ) {
          faxis.fLabelColor = parseInt(arg);
          this.RedrawPad();
        });
        this.AddSizeMenuEntry(
          menu,
          'Offset',
          0,
          0.1,
          ...
          faxis.fLabelOffset,
          function (arg) {
            faxis.fLabelOffset = parseFloat(arg);
            this.RedrawPad();
          }
        );
        this.AddSizeMenuEntry(
          ...
          faxis.fLabelSize,
          function (arg) {
            faxis.fLabelSize = parseFloat(arg);
            this.RedrawPad();
          }
        );
        menu.add('endsub:');
        ...
        menu.addchk(
          faxis.TestBit(JSROOT.EAxisBits.kCenterTitle),
          'Center',
          function () {
            faxis.InvertBit(JSROOT.EAxisBits.kCenterTitle);
            this.RedrawPad();
          }
        );
        menu.addchk(
          faxis.TestBit(JSROOT.EAxisBits.kRotateTitle),
          'Rotate',
          function () {
            faxis.InvertBit(JSROOT.EAxisBits.kRotateTitle);
            this.RedrawPad();
          }
        );
        this.AddColorMenuEntry(menu, 'Color', faxis.fTitleColor, function (
          arg
        ) {
          faxis.fTitleColor = parseInt(arg);
          this.RedrawPad();
        });
        this.AddSizeMenuEntry(
          menu,
          ...
          faxis.fTitleOffset,
          function (arg) {
            faxis.fTitleOffset = parseFloat(arg);
            this.RedrawPad();
          }
        );
        this.AddSizeMenuEntry(
          ...
          faxis.fTitleSize,
          function (arg) {
            faxis.fTitleSize = parseFloat(arg);
            this.RedrawPad();
          }
        );
        menu.add('endsub:');
        menu.add('sub:Ticks');
        this.AddColorMenuEntry(menu, 'Color', faxis.fAxisColor, function (arg) {
          faxis.fAxisColor = parseInt(arg);
          this.RedrawPad();
        });
        this.AddSizeMenuEntry(
          menu,
          ...
          faxis.fTickLength,
          function (arg) {
            faxis.fTickLength = parseFloat(arg);
            this.RedrawPad();
          }
        );
        menu.add('endsub:');
    ...
    if (alone) menu.add('header:Frame');
    else menu.add('separator');
    ...
    menu.add('Save as frame.png', function () {
      this.pad_painter().SaveAs('png', 'frame', 'frame.png');
    });
    menu.add('Save as frame.svg', function () {
      this.pad_painter().SaveAs('svg', 'frame', 'frame.svg');
    });
  ...
  TFramePainter.prototype.ShowAxisStatus = function (axis_name) {
    ...
    if (taxis) {
      ...
      hint_title = taxis.fTitle || 'histogram TAxis object';
    }

    if (this.swap_xy) id = 1 - id;

    var axis_value =
      axis_name == 'x' ? this.RevertX(m[id]) : this.RevertY(m[id]);

    status_func(
      hint_name,
      hint_title,
      axis_name + ' : ' + this.AxisAsText(axis_name, axis_value),
      m[0] + ',' + m[1]
    );
  };
  ...
  TFramePainter.prototype.AddInteractive = function () {
    ...
    if (
      JSROOT.BatchMode ||
      (!JSROOT.gStyle.Zooming && !JSROOT.gStyle.ContextMenu)
    )
      return;
    ...
    if (
      JSROOT.touches &&
      ((JSROOT.gStyle.Zooming && JSROOT.gStyle.ZoomTouch) ||
        JSROOT.gStyle.ContextMenu)
    )
  ...
  TFramePainter.prototype.AnalyzeMouseWheelEvent = function (
    event,
    item,
    dmin,
    ignore
  ...
  TFramePainter.prototype.AddKeysHandler = function () {
    if (this.keys_handler || JSROOT.BatchMode || typeof window == 'undefined')
      return;
  ...
  TFramePainter.prototype.CreateXY = function () {
    ...
    this.x
      .domain([this.ConvertX(this.scale_xmin), this.ConvertX(this.scale_xmax)])
      .range(this.swap_xy ? gr_range_y : gr_range_x);

    if (this.x_kind == 'time') {
      // we emulate scale functionality
      this.grx = function (val) {
        return this.x(this.ConvertX(val));
      };
    } else if (this.logx) {
      this.grx = function (val) {
        return val < this.scale_xmin
          ? this.swap_xy
            ? this.x.range()[0] + 5
            : -5
          : this.x(val);
    ...
    this.y
      .domain([this.ConvertY(this.scale_ymin), this.ConvertY(this.scale_ymax)])
      .range(this.swap_xy ? gr_range_x : gr_range_y);

    if (this.y_kind == 'time') {
      // we emulate scale functionality
      this.gry = function (val) {
        return this.y(this.ConvertY(val));
      };
    } else if (this.logy) {
      // make protection for log
      this.gry = function (val) {
        return val < this.scale_ymin
          ? this.swap_xy
            ? -5
            : this.y.range()[0] + 5
          : this.y(val);
  ...
  TFramePainter.prototype.ToggleLog = function (axis) {
    ...
    if (!curr) {
      ...
      if (this.swap_xy && axis === 'x') kind = this['y_kind'];
      else if (this.swap_xy && axis === 'y') kind = this['x_kind'];
      if (kind === 'labels') return;
    }

    var pp = this.pad_painter(),
      canp = this.canv_painter();
  ...
  function TPadPainter(pad, iscan) {
    ...
    if (!this.iscan && pad !== null) {
      if (pad.fObjectID) this.this_pad_name = 'pad' + pad.fObjectID;
      // use objectid as padname
      else this.this_pad_name = 'ppp' + JSROOT.id_counter++; // artificical name
    }
  ...
  TPadPainter.prototype.Cleanup = function () {
    ...
    for (var k = 0; k < this.painters.length; ++k) this.painters[k].Cleanup();
  ...
  TPadPainter.prototype.ButtonSize = function (fact) {
    return Math.round(
      (!fact ? 1 : fact) * (this.iscan || !this.has_canvas ? 16 : 12)
    );
  };
  ...
  TPadPainter.prototype.CreateCanvasSvg = function (check_resize, new_size) {
    ...
    if (rect.width <= lmt || rect.height <= lmt) {
      ...
      console.warn(
        'Hide canvas while geometry too small w=',
        rect.width,
        ' h=',
        rect.height
      );
      rect.width = 200;
      rect.height = 100; // just to complete drawing
    } else {
    ...
    this._fast_drawing =
      JSROOT.gStyle.SmallPad &&
      (rect.width < JSROOT.gStyle.SmallPad.width ||
        rect.height < JSROOT.gStyle.SmallPad.height);
  ...
  TPadPainter.prototype.EnlargePad = function () {
    ...
    if (this._fast_drawing != was_fast) this.ShowButtons();
  };
  ...
  TPadPainter.prototype.CreatePadSvg = function (only_resize) {
    ...
    if (only_resize) {
      ...
      if (!JSROOT.BatchMode)
        svg_rect
          .attr('pointer-events', 'visibleFill') // get events also for not visible rect
          .on('dblclick', this.EnlargePad.bind(this))
          .on('click', this.SelectObjectPainter.bind(this, this))
          .on('mouseenter', this.ShowObjectStatus.bind(this));
    }
    ...
    this.createAttLine({
      attr: this.pad,
      color0: this.pad.fBorderMode == 0 ? 'none' : '',
    });
    ...
    this._fast_drawing =
      JSROOT.gStyle.SmallPad &&
      (w < JSROOT.gStyle.SmallPad.width || h < JSROOT.gStyle.SmallPad.height);
  ...
  TPadPainter.prototype.FindPrimitive = function (exact_obj, classname, name) {
    ...
    for (var i = 0; i < this.pad.fPrimitives.arr.length; i++) {
      ...
      if (name !== undefined && name !== null) if (obj.fName !== name) continue;
  ...
  TPadPainter.prototype.HasObjectsToDraw = function () {
    ...
    if (arr)
      for (var n = 0; n < arr.length; ++n)
        if (arr[n] && arr[n]._typename != 'ROOT::Experimental::RPadDisplayItem')
          return true;
  ...
  TPadPainter.prototype.DrawPrimitives = function (indx, callback, ppainter) {
    if (indx === 0) {
      ...
      if (this.iscan) this._start_tm = this._lasttm_tm = new Date().getTime();

      // set number of primitves
      this._num_primitives =
        this.pad && this.pad.fPrimitives ? this.pad.fPrimitives.length : 0;
    }
    ...
    while (true) {
      ...
      if (!this.pad || indx >= this.pad.fPrimitives.length) {
        ...
        if (this._start_tm) {
          ...
          if (spenttm > 3000)
            console.log(
              'Canvas drawing took ' + (spenttm * 1e-3).toFixed(2) + 's'
            );
      ...
      ppainter = JSROOT.draw(
        this.divid,
        this.pad.fPrimitives[indx],
        '',
        handle
      );
  ...
  TPadPainter.prototype.FillContextMenu = function (menu) {
    ...
    else menu.add('header: Canvas');

    menu.addchk(
      this.IsTooltipAllowed(),
      'Show tooltips',
      this.SetTooltipAllowed.bind(this, 'toggle')
    );
    ...
    if (this.ToggleEventStatus)
      menu.addchk(
        this.HasEventStatus(),
        'Event status',
        this.ToggleEventStatus.bind(this)
      );

    if (this.enlarge_main() || (this.has_canvas && this.HasObjectsToDraw()))
      menu.addchk(
        this.enlarge_main('state') == 'on',
        'Enlarge ' + (this.iscan ? 'canvas' : 'pad'),
        this.EnlargePad.bind(this)
      );
    ...
    menu.add(
      'Save as ' + fname + '.png',
      fname + '.png',
      this.SaveAs.bind(this, 'png', false)
    );
    menu.add(
      'Save as ' + fname + '.svg',
      fname + '.svg',
      this.SaveAs.bind(this, 'svg', false)
    );
  ...
  TPadPainter.prototype.DrawNextSnap = function (
    lst,
    indx,
    call_back,
    objpainter
    ...
    while (true) {
      ...
      if (objpainter) {
        if (snap._typename == 'ROOT::Experimental::RPadDisplayItem')
          // subpad
          return objpainter.RedrawPadSnap(snap, draw_callback);

        if (
          objpainter.UpdateObject(
            snap.fDrawable || snap.fObject,
            snap.fOption || ''
          )
        )
          objpainter.Redraw();
      ...
      if (snap._typename == 'ROOT::Experimental::RPadDisplayItem') {
        ...
        if (snap.fPrimitives && snap.fPrimitives.length > 0) {
          padpainter.AddButton(
            JSROOT.ToolbarIcons.camera,
            'Create PNG',
            'PadSnapShot'
          );
          padpainter.AddButton(
  ...
  TPadPainter.prototype.RedrawPadSnap = function (snap, call_back) {
    ...
    if (this.batch_mode && this.iscan && (!padattr.fCw || !padattr.fCh)) {
      padattr.fCw = 900;
      padattr.fCh = 700;
    }

    if (this.iscan && snap.fTitle && document) document.title = snap.fTitle;
    ...
    if (this.snapid === undefined) {
      ...
      if (this.batch_mode && this.iscan) this._fixed_size = true;
    ...
    for (var k = 0; k < this.painters.length; ++k) {
      ...
      for (var i = 0; i < snap.fPrimitives.length; ++i)
        if (snap.fPrimitives[i].fObjectID === sub.snapid) {
          sub = null;
          isanyfound = true;
          break;
        }
    ...
    if (!isanyfound) {
      ...
      if (svg_p && !svg_p.empty()) svg_p.property('mainpainter', null);
      for (var k = 0; k < this.painters.length; ++k)
        if (fp !== this.painters[k]) this.painters[k].Cleanup();
      this.painters = [];
      if (fp) {
  ...
  TPadPainter.prototype.CreateImage = function (format, call_back) {
    if (format == 'pdf') {
      ...
      this.ProduceImage(true, format, function (res) {
        if (format == 'svg' || !res) return JSROOT.CallBack(call_back, res);
  ...
  TPadPainter.prototype.ProduceImage = function (
    full_canvas,
    file_format,
    call_back
  ) {
    ...
    if (!use_frame)
      ...
      painter.ForEachPainterInPad(function (pp) {
        ...
        // add svg image
        item.img = item.prnt
          .insert('image', '.primitives_layer') // create image object
          .attr('x', sz2.x)
          .attr('y', sz2.y)
    ...
    function reconstruct(res) {
      for (var k = 0; k < items.length; ++k) {
        ...
        if (item.img) item.img.remove(); // delete embed image

        var prim = item.prnt.select('.primitives_layer');

        if (item.foreign)
          // reinsert foreign object
          item.prnt.node().insertBefore(item.foreign.node(), prim.node());

        if (item.frame_node)
          // reinsert frame as first in list of primitives
    ...
    var width = elem.property('draw_width'),
    ...
    if (file_format == 'svg') return reconstruct(svg); // return SVG file as is
  ...
  TPadPainter.prototype.PadButtonClick = function (funcname) {
    ...
    if (funcname == 'PadContextMenus') {
      ...
      JSROOT.Painter.createMenu(pthis, function (menu) {
        ...
        if (pthis.iscan) menu.add('Canvas', 'pad', pthis.ItemContextMenu);
        else menu.add('Pad', 'pad', pthis.ItemContextMenu);
    ...
    for (var i = 0; i < this.painters.length; ++i) {
      ...
      if (typeof pp.PadButtonClick == 'function') pp.PadButtonClick(funcname);
    ...
    switch (action) {
      ...
      case 'leavebtn':
        if (!state)
          btn.property(
            'timout_handler',
            setTimeout(this.toggleButtonsVisibility.bind(this, 'timeout'), 500)
          );
        return;
    }
  ...
  TPadPainter.prototype.AddButton = function (
    _btn,
    _tooltip,
    _funcname,
    _keyname
  ...
  TPadPainter.prototype.ShowButtons = function () {
    ...
    if (this._fast_drawing) {
      ctrl = JSROOT.ToolbarIcons.CreateSVG(
        group,
        JSROOT.ToolbarIcons.circle,
        this.ButtonSize(),
        'EnlargePad'
      ...
      for (var k = 0; k < this._buttons.length; ++k) {
        ...
        var svg = JSROOT.ToolbarIcons.CreateSVG(
          group,
          item.btn,
          this.ButtonSize(),
          item.tooltip +
            (iscan ? '' : ' on pad ' + this.this_pad_name) +
            (item.keyname ? ' (keyshortcut ' + item.keyname + ')' : '')
        );

        if (group.property('vertical')) svg.attr('x', y).attr('y', x);
        else svg.attr('x', x).attr('y', y);

        svg
          .attr('name', item.funcname)
          .style('display', ctrl.property('buttons_state') ? '' : 'none')
          .on('mouseenter', this.toggleButtonsVisibility.bind(this, 'enterbtn'))
          .on(
            'mouseleave',
            this.toggleButtonsVisibility.bind(this, 'leavebtn')
          );
    ...
    this.AlignBtns(
      group,
      this.pad_width(this.this_pad_name),
      this.pad_height(this.this_pad_name)
    );
  ...
  TPadPainter.prototype.AlignBtns = function (btns, width, height, svg) {
    var sz0 = this.ButtonSize(1.25),
      nextx = (btns.property('nextx') || 0) + sz0,
      btns_x,
      btns_y;
  ...
  TPadPainter.prototype.DecodeOptions = function (opt) {
    ...
    if (d.check('NOCOLORS') || d.check('NOCOL'))
      this.options.GlobalColors = this.options.LocalColors = false;
    if (d.check('LCOLORS') || d.check('LCOL')) {
      this.options.GlobalColors = false;
      this.options.LocalColors = true;
    }
    if (d.check('NOPALETTE') || d.check('NOPAL'))
      this.options.IgnorePalette = true;
  ...
  function drawPad(divid, pad, opt) {
    ...
    if (
      painter.MatchObjectType('TPad') &&
      (!painter.has_canvas || painter.HasObjectsToDraw())
    ) {
      painter.AddButton(
        JSROOT.ToolbarIcons.camera,
        'Create PNG',
    ...
    // we select current pad, where all drawing is performed
    var prev_name = painter.has_canvas
      ? painter.CurrentPadName(painter.this_pad_name)
      : undefined;
  ...
  TCanvasPainter.prototype.ChangeLayout = function (layout_kind, call_back) {
    ...
    if (layout_kind == 'simple') {
      main = origin;
      for (var k = 0; k < lst.length; ++k) main.node().appendChild(lst[k]);
  ...
  TCanvasPainter.prototype.ToggleProjection = function (kind, call_back) {
    ...
    if (kind == 'X') layout = 'vert2_31';
    else if (kind == 'Y') layout = 'horiz2_13';
  ...
  TCanvasPainter.prototype.DrawProjection = function (kind, hist) {
    ...
    if (this.proj_painter === 1) {
      ...
      if (kind == 'X') {
        ...
        canv.fUxmin = main.logx
          ? JSROOT.log10(main.scale_xmin)
          : main.scale_xmin;
        canv.fUxmax = main.logx
          ? JSROOT.log10(main.scale_xmax)
          : main.scale_xmax;
        drawopt = 'fixframe';
      } else {
        ...
        canv.fUxmin = main.logy
          ? JSROOT.log10(main.scale_ymin)
          : main.scale_ymin;
        canv.fUxmax = main.logy
          ? JSROOT.log10(main.scale_ymax)
          : main.scale_ymax;
        drawopt = 'rotate';
      }
      ...
      if (this.DrawInUI5ProjectionArea) {
        ...
        this.DrawInUI5ProjectionArea(canv, drawopt, function (painter) {
          pthis.proj_painter = painter;
        });
      } else {
        this.DrawInSidePanel(canv, drawopt, function (painter) {
          pthis.proj_painter = painter;
        });
      }
    } else {
  ...
  TCanvasPainter.prototype.SendWebsocket = function (msg, chid) {
    if (this._websocket) this._websocket.Send(msg, chid);
  };
  ...
  TCanvasPainter.prototype.OnWebsocketMsg = function (handle, msg) {
    ...
    if (msg == 'CLOSE') {
        ...
        if (!this.ShowUI5Panel) {
          ...
          conn.SetReceiver({
            ...
            OnWebsocketMsg: function (panel_handle, msg) {
              var panel_name =
                msg.indexOf('SHOWPANEL:') == 0 ? msg.substr(10) : '';
              console.log('Panel get message ' + msg + ' show ' + panel_name);

              this.cpainter.ShowUI5Panel(panel_name, panel_handle, function (
                res
              ) {
      ...
      console.log(
        'unrecognized msg len:' + msg.length + ' msg:' + msg.substr(0, 20)
      );
    }
  };
  ...
  TCanvasPainter.prototype.CompeteCanvasSnapDrawing = function () {
    ...
    this.ShowSection(
      'Menu',
      this.pad.TestBit(JSROOT.TCanvasStatusBits.kMenuBar)
    );
    this.ShowSection(
      'StatusBar',
      this.pad.TestBit(JSROOT.TCanvasStatusBits.kShowEventStatus)
    );
    this.ShowSection(
      'ToolBar',
      this.pad.TestBit(JSROOT.TCanvasStatusBits.kShowToolBar)
    );
    this.ShowSection(
      'Editor',
      this.pad.TestBit(JSROOT.TCanvasStatusBits.kShowEditor)
    );
    this.ShowSection(
      'ToolTips',
      this.pad.TestBit(JSROOT.TCanvasStatusBits.kShowToolTips)
    );
  };
  ...
  JSROOT.addDrawFunc({
    name: 'ROOT::Experimental::RHistDrawable<1>',
    icon: 'img_histo1d',
    prereq: 'v7hist',
    func: 'JSROOT.v7.drawHist1',
    opt: '',
  });
  JSROOT.addDrawFunc({
    name: 'ROOT::Experimental::RHistDrawable<2>',
    icon: 'img_histo2d',
    prereq: 'v7hist',
    func: 'JSROOT.v7.drawHist2',
    opt: '',
  });
  JSROOT.addDrawFunc({
    name: 'ROOT::Experimental::RText',
    icon: 'img_text',
    prereq: 'v7more',
    func: 'JSROOT.v7.drawText',
    opt: '',
    direct: true,
    csstype: 'text',
  });
  JSROOT.addDrawFunc({
    name: 'ROOT::Experimental::RLine',
    icon: 'img_graph',
    prereq: 'v7more',
    func: 'JSROOT.v7.drawLine',
    opt: '',
    direct: true,
    csstype: 'line',
  });
  JSROOT.addDrawFunc({
    name: 'ROOT::Experimental::RBox',
    icon: 'img_graph',
    prereq: 'v7more',
    func: 'JSROOT.v7.drawBox',
    opt: '',
    direct: true,
    csstype: 'box',
  });
  JSROOT.addDrawFunc({
    name: 'ROOT::Experimental::RMarker',
    icon: 'img_graph',
    prereq: 'v7more',
    func: 'JSROOT.v7.drawMarker',
    opt: '',
    direct: true,
    csstype: 'marker',
  });
  JSROOT.addDrawFunc({
    name: 'ROOT::Experimental::RLegend',
    icon: 'img_graph',
    prereq: 'v7more',
    func: 'JSROOT.v7.drawLegend',
    opt: '',
    direct: true,
    csstype: 'legend',
  });
