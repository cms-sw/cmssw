...
(function (factory) {
  ...
  JSROOT.Painter.createMenu = function (painter, maincallback) {
    ...
    if (!maincallback && typeof painter === 'function') {
      maincallback = painter;
      painter = null;
    }
    ...
    menu.add = function (name, arg, func, title) {
      ...
      if (name.indexOf('header:') == 0) {
        this.code +=
          "<li class='ui-widget-header' style='padding:3px; padding-left:5px;'>" +
          name.substr(7) +
          '</li>';
        return;
      }
      ...
      if (name.indexOf('chk:') == 0) {
        item = "<span class='ui-icon ui-icon-check' style='margin:1px'></span>";
        name = name.substr(4);
      } else if (name.indexOf('unk:') == 0) {
        item = "<span class='ui-icon ui-icon-blank' style='margin:1px'></span>";
        name = name.substr(4);
      }
      ...
      else if ($.ui.version.indexOf('1.11') == 0) item += name;
      else item = '<div' + title + '>' + item + name + '</div>';
    ...
    menu.addDrawMenu = function (menu_name, opts, call_back) {
      ...
      if (opts.length === 1) {
        if (opts[0] === 'inspect')
          menu_name = menu_name.replace('Draw', 'Inspect');
    ...
    menu.show = function (event, close_callback) {
      ...
      $(document.body).append(
        '<ul class="jsroot_ctxmenu">' + this.code + '</ul>'
      );
        ...
        .menu({
          ...
          select: function (event, ui) {
            ...
            if (typeof func == 'function') {
              if ('painter' in menu) func.bind(pthis.painter)(arg);
              // if 'painter' field set, returned as this to callback
              else func(arg);
            }
          },
        });
      ...
      if (event.clientX + this.element.width() > $(window).width())
        newx = $(window).width() - this.element.width() - 20;
      if (event.clientY + this.element.height() > $(window).height())
        newy = $(window).height() - this.element.height() - 20;

      if (newx !== null)
        this.element.css('left', (newx > 0 ? newx : 0) + window.pageXOffset);
      if (newy !== null)
        this.element.css('top', (newy > 0 ? newy : 0) + window.pageYOffset);
    };
  ...
  BrowserLayout.prototype.SetBrowserTitle = function (title) {
    ...
    if (!main.empty()) main.select('.jsroot_browser_title').text(title);
  };
  ...
  BrowserLayout.prototype.ToggleBrowserKind = function (kind) {
    ...
    if (kind === 'float') {
      ...
      $(vsepar.node()).draggable({
        ...
        containment: 'parent',
        helper: function () {
          return $(this).clone().css('background-color', 'grey');
        },
        drag: function (event, ui) {
          pthis.SetButtonsPosition();
  ...
  BrowserLayout.prototype.ToggleBrowserVisisbility = function (fast_close) {
    ...
    if (!this.browser_visible) {
      ...
      area
        .property('last_left', null)
        .property('last_vsepar', null)
        .property('last_drawing', null);
    } else {
      ...
      if (vsepar.empty() && $(area.node()).offset().left > mainw / 2)
        tgt = mainw + 10 + 'px';
    ...
    var pthis = this,
      visible_at_the_end = !this.browser_visible,
      _duration = fast_close ? 0 : 700;
    ...
    if (!vsepar.empty()) {
      ...
      drawing
        .transition()
        .style('left', tgt_drawing)
        .duration(_duration)
        .on('end', this.CheckResize.bind(this));
    }

    if (this.status_layout && this.browser_kind == 'fix') {
      main
        .select('.jsroot_h_separator')
        .transition()
        .style('left', tgt_drawing)
        .duration(_duration);
      main
        .select('.jsroot_status_area')
        .transition()
        .style('left', tgt_drawing)
        .duration(_duration);
    }
  ...
  BrowserLayout.prototype.DeleteContent = function () {
    ...
    var vsepar = main.select('.jsroot_v_separator');
    if (!vsepar.empty()) $(vsepar.node()).draggable('destroy');
  ...
  BrowserLayout.prototype.CreateStatusLine = function (height, mode) {
    ...
    var id = this.gui_div + '_status',
      line = d3.select('#' + id),
      skip_height_check = false,
      is_visible = !line.empty();

    if (mode === 'toggle') {
      mode = !is_visible;
      skip_height_check = height === this.last_hsepar_height;
    } else if (height === 'delete') {
      mode = false;
      height = 0;
    ...
    line = main
      .insert('div', '.jsroot_browser_area')
      .attr('id', id)
    ...
    for (var k = 0; k < 4; ++k)
      d3.select(this.status_layout.GetFrame(k))
        .attr('title', frame_titles[k])
        .style('overflow', 'hidden')
        .append('label')
        .attr('class', 'jsroot_status_label');
  ...
  BrowserLayout.prototype.AdjustSeparator = function (
    vsepar,
    hsepar,
    redraw,
    first_time
    ...
    if (hsepar !== null) {
      ...
      var elem = main.select('.jsroot_h_separator'),
        hlimit = 0;
      ...
      if (!elem.empty()) {
        ...
        d3.select('#' + this.gui_div + '_status').style(
          'height',
          hsepar + 'px'
        );
        hlimit = hsepar + w + 'px';
    ...
    if (vsepar !== null) {
      ...
      d3.select('#' + this.gui_div + '_drawing').style(
        'left',
        vsepar + w + 'px'
      );
  ...
  BrowserLayout.prototype.ShowStatus = function (
    name,
    title,
    info,
    coordinates
    ...
    $(this.status_layout.GetFrame(0))
      .children('label')
      .text(name || '');
    $(this.status_layout.GetFrame(1))
      .children('label')
      .text(title || '');
    $(this.status_layout.GetFrame(2))
      .children('label')
      .text(coordinates || '');
    $(this.status_layout.GetFrame(3))
      .children('label')
      .text(info || '');
    ...
    if (!this.status_layout.first_check) {
      ...
      for (var n = 0; n < 4; ++n)
        maxh = Math.max(
          maxh,
          $(this.status_layout.GetFrame(n)).children('label').outerHeight()
        );
      if (
        maxh > 5 &&
        (maxh > this.last_hsepar_height || maxh < this.last_hsepar_height + 5)
      )
  ...
  HierarchyPainter.prototype.isLastSibling = function (hitem) {
    ...
    var chlds = hitem._parent._childs,
      indx = chlds.indexOf(hitem);
    if (indx < 0) return false;
    while (++indx < chlds.length) if (!('_hidden' in chlds[indx])) return false;
    return true;
  };
  ...
  HierarchyPainter.prototype.addItemHtml = function (hitem, d3prnt, arg) {
    ...
    if (handle !== null) {
      ...
      if (
        ...
        'execute' in handle ||
        'aslink' in handle ||
        ('expand' in handle && hitem._more !== false)
      )
        can_click = true;
    }
    ...
    if (img1.length == 0 && isroot) hitem._icon = img1 = 'img_base';
    ...
    if (
      !can_menu &&
      typeof hitem._kind == 'string' &&
      hitem._kind.indexOf('ROOT.') == 0
    )
    ...
    if (arg === 'update') {
      ...
      if (
        arg &&
        arg >= (hitem._parent._show_limit || JSROOT.gStyle.HierarchyLimit)
      )
        break_list = true;
    }
    ...
    if (icon_class.length > 0) {
      ...
      if (plusminus)
        d3icon.style('cursor', 'pointer').on('click', function () {
          h.tree_click(this, 'plusminus');
        });
    }
    ...
    if (this.with_icons && !break_list) {
      ...
      if ('_icon_click' in hitem || (handle && 'icon_click' in handle))
        d3img.on('click', function () {
          h.tree_click(this, 'icon');
        });
    }

    var d3a = d3line.append('a');
    if (can_click || has_childs || break_list)
      d3a.attr('class', 'h_item').on('click', function () {
        h.tree_click(this);
      });
    ...
    if (break_list) {
      ...
      d3a
        .attr(
          'title',
          'there are ' + (hitem._parent._childs.length - arg) + ' more items'
        )
        .text('...more...');
      return false;
    }
    ...
    if ('disp_kind' in h) {
      ...
      if (JSROOT.gStyle.ContextMenu && can_menu)
        d3a.on('contextmenu', function () {
          h.tree_contextmenu(this);
        });

      d3a
        .on('mouseover', function () {
          h.tree_mouseover(true, this);
        })
        .on('mouseleave', function () {
          h.tree_mouseover(false, this);
        });
    } else if (hitem._direct_context && JSROOT.gStyle.ContextMenu)
      d3a.on('contextmenu', function () {
        h.direct_contextmenu(this);
      });

    var element_name = hitem._name,
      element_title = '';

    if ('_realname' in hitem) element_name = hitem._realname;

    if ('_title' in hitem) element_title = hitem._title;

    if ('_fullname' in hitem) element_title += '  fullname: ' + hitem._fullname;

    if (!element_title) element_title = element_name;
  ...
  HierarchyPainter.prototype.toggleOpenState = function (isopen, h) {
    ...
    if (!('_childs' in hitem)) {
      if (
        !isopen ||
        this.with_icons ||
        (!hitem._expand && hitem._more !== true)
      )
        return false;
  ...
  HierarchyPainter.prototype.RefreshHtml = function (callback) {
    ...
    if (!this.h || d3elem.empty()) return JSROOT.CallBack(callback);
    ...
    if (factcmds.length) {
      ...
      for (var n = 0; n < factcmds.length; ++n) {
        var btn = fastbtns
          ...
          .on('click', function () {
            h.ExecuteCommand(d3.select(this).attr('item'), this);
          });
    ...
    d3btns
      ...
      .attr('class', 'h_button')
      .text('open all')
      .attr('title', 'open all items in the browser')
      .on('click', h.toggleOpenState.bind(h, true));
    d3btns.append('text').text(' | ');
    d3btns
      .append('a')
      .attr('class', 'h_button')
      .text('close all')
      .attr('title', 'close all items in the browser')
      .on('click', h.toggleOpenState.bind(h, false));
    ...
    if (typeof h.removeInspector == 'function') {
      ...
      d3btns
        ...
        .on('click', h.removeInspector.bind(h));
    }
    ...
    if ('_online' in this.h) {
      ...
      d3btns
        ...
        .attr('class', 'h_button')
        .text('reload')
        .attr('title', 'reload object list from the server')
        .on('click', h.reload.bind(h));
    }
    ...
    if ('disp_kind' in this) {
      ...
      d3btns
        ...
        .attr('class', 'h_button')
        .text('clear')
        .attr('title', 'clear all drawn objects')
        .on('click', h.clear.bind(h, false));
    }
    ...
    if (
      ...
      JSROOT.GetUrlOption('nostatus') === null
    ) {
  ...
  HierarchyPainter.prototype.UpdateTreeNode = function (hitem, d3cont) {
    if (d3cont === undefined || d3cont.empty()) {
      ...
      if (d3cont.empty() && '_cycle' in hitem)
        d3cont = this.select_main().select(
          "[item='" + name + ';' + hitem._cycle + "']"
        );
  ...
  HierarchyPainter.prototype.UpdateBackground = function (
    hitem,
    scroll_into_view
  ) {
    ...
    if (scroll_into_view && hitem._background) d3a.node().scrollIntoView(false);
  };
  ...
  HierarchyPainter.prototype.tree_click = function (node, place) {
    ...
    if (hitem._break_point) {
      ...
      var prnt = hitem._parent,
        indx = prnt._childs.indexOf(hitem),
    ...
    if (place == 'icon') {
      var func = null;
      if (typeof hitem._icon_click == 'function') func = hitem._icon_click;
      else if (handle && typeof handle.icon_click == 'function')
        func = handle.icon_click;
      if (func && func(hitem, this)) this.UpdateTreeNode(hitem, d3cont);
      return;
    }
    ...
    if (
      (place == 'plusminus' && !('_childs' in hitem) && hitem._more) ||
    ...
    if (place == 'item') {
      if ('_player' in hitem) return this.player(itemname);

      if (handle && handle.aslink) return window.open(itemname + '/');
      ...
      if (!drawopt) {
        for (var pitem = hitem._parent; !!pitem; pitem = pitem._parent) {
          if (pitem._painter) {
            can_draw = false;
            if (can_expand === undefined) can_expand = false;
            break;
          }
        }
      }
      ...
      if (can_draw && can_expand && !drawopt) {
        ...
        if (dflt_expand || (handle && handle.dflt === 'expand'))
          can_draw = false;
        else if (this.isItemDisplayed(itemname)) can_draw = false; // if already displayed, try to expand
      }

      if (can_draw) return this.display(itemname, drawopt);

      if (can_expand || dflt_expand) return this.expand(itemname, null, d3cont);
    ...
    if (hitem._isopen) delete hitem._isopen;
    else hitem._isopen = true;
  ...
  HierarchyPainter.prototype.tree_contextmenu = function (elem) {
    ...
    JSROOT.Painter.createMenu(painter, function (menu) {
      if (itemname == '' && !('_jsonfile' in hitem)) {
        ...
        var files = [];
        painter.ForEachRootFile(function (item) {
          files.push(item._file.fFullURL);
        });

        if (!painter.GetTopOnlineItem()) addr = JSROOT.source_dir + 'index.htm';

        if (painter.IsMonitoring())
          addr += separ() + 'monitoring=' + painter.MonitoringInterval();

        if (files.length == 1) addr += separ() + 'file=' + files[0];
        ...
        if (painter.disp)
          painter.disp.ForEachPainter(function (p) {
            if (p.GetItemName() != null) items.push(p.GetItemName());
          });
        ...
        menu.add('Direct link', function () {
          window.open(addr);
        });
        menu.add('Only items', function () {
          window.open(addr + '&nobrowser');
        });
      } else if (onlineprop) {
        ...
        if (hitem._kind === 'ROOT.TStyle')
          menu.add('Apply', function () {
            painter.ApplyStyle(itemname);
          });
      }

      if (typeof hitem._menu == 'function') hitem._menu(menu, hitem, painter);
  ...
  HierarchyPainter.prototype.CreateDisplay = function (callback) {
    ...
    else this.disp = new JSROOT.GridDisplay(this.disp_frameid, this.disp_kind);
  ...
  HierarchyPainter.prototype.CreateBrowser = function (
    ...
    var guiCode =
      "<p class='jsroot_browser_version'><a href='https://root.cern/js/'>JSROOT</a> version <span style='color:green'><b>" +
      JSROOT.version +
      '</b></span></p>';
    ...
    this.brlayout.SetBrowserTitle(
      this.is_online ? 'ROOT online server' : 'Read a ROOT file'
    );
    ...
    if (!this.is_online && !this.no_select) {
      this.ReadSelectedFile = function () {
        ...
        else this.OpenRootFile(filename);
      };

      jmain
        .find('.gui_selectFileName')
        .val('')
        .change(function () {
    ...
    if (jlayout.length) {
      ...
      jlayout.change(function () {
        hpainter.SetDisplay(
          $(this).val() || 'collapsible',
          hpainter.gui_div + '_drawing'
        );
      });
    }
  ...
  HierarchyPainter.prototype.EnableMonitoring = function (on) {
    ...
    var chkbox = d3.select(
      '#' + this.gui_div + ' .jsroot_browser .gui_monitoring'
    );
  ...
  JSROOT.BuildGUI = function () {
    ...
    if (
      JSROOT.GetUrlOption('nobrowser') !== null ||
      (myDiv.attr('nobrowser') && myDiv.attr('nobrowser') !== 'false')
    )
  ...
  CollapsibleDisplay.prototype.ForEachFrame = function (
    userfunc,
    only_visible
  ) {
  ...
  TabsDisplay.prototype.ForEachFrame = function (userfunc, only_visible) {
    ...
    $('#' + topid + '> .tabs_draw').each(function () {
      cnt++;
      if (!only_visible || cnt == active) userfunc($(this).get(0));
    });
  };
  ...
  FlexibleDisplay.prototype.CreateFrame = function (title) {
    ...
    function PopupWindow(div) {
      if (div === 'first') {
        ...
        $('#' + topid + ' .flex_frame').each(function () {
          if (!$(this).is(':hidden') && $(this).prop('state') != 'minimal')
            div = $(this);
        });
        if (!div) return;
      }
      ...
      JSROOT.Painter.SelectActivePad({
        pp: dummy.canv_painter(),
        active: true,
      });
    ...
    function ChangeWindowState(main, state) {
      ...
      switch (state) {
        ...
        default:
          ...
          main
            .height(main.prop('original_height'))
            .width(main.prop('original_width'));
          if (curr != 'minimal')
            main
              .css('left', main.prop('original_left'))
              .css('top', main.prop('original_top'));
      }

      if (state !== 'minimal') PopupWindow(main);
      else PopupWindow('first');
    }
    ...
    $('#' + subid)
      ...
      .resizable({
        ...
        stop: function (event, ui) {
          var rect = {
            ...
            height: ui.size.height - $(this).find('.flex_header').height() - 1,
          };
      ...
      .draggable({
        ...
        start: function (event, ui) {
          ...
          $(elementMouseIsOver)
            .parents()
            .map(function () {
              if ($(this).get(0) === ddd.get(0)) isparent = true;
            });
      ...
      .dblclick(function () {
        ...
        if (main.prop('state') == 'normal') ChangeWindowState(main, 'maximal');
        else ChangeWindowState(main, 'normal');
      })
      .find('button')
      .first()
  ...
  JSROOT.GridDisplay.prototype.CreateSeparator = function (
    handle,
    main,
    group
  ) {
    ...
    separ
      ...
      .css(
        handle.vertical ? 'top' : 'left',
        'calc(' + group.position + '% - 2px)'
      )
    ...
    separ.bind('changePosition', function (e, drag_ui) {
      ...
      } else if (drag_ui && drag_ui.offset) {
        if (handle.vertical)
          pos = Math.round(
            ((drag_ui.offset.top + 2 - $(this).parent().offset().top) /
              $(this).parent().innerHeight()) *
              100
          );
        else
          pos = Math.round(
            ((drag_ui.offset.left + 2 - $(this).parent().offset().left) /
      ...
      function SetGroupSize(prnt, grid) {
        ...
        prnt
          .children("[groupid='" + grid + "']")
          .css(name, size)
          .children('.jsroot_separator')
          .css(name, size);
      }
    ...
    separ.draggable({
      ...
      stop: function (event, ui) {
        ...
        var handle = $(this).prop('handle'),
          id = parseInt($(this).attr('separator-id'));
        if (
          Math.abs(handle.groups[id].startpos - handle.groups[id].position) <
          0.5
        )
          return;
  ...
  JSROOT.CreateTreePlayer = function (player) {
    ...
    player.ConfigureOnline = function (
      itemname,
      url,
      askey,
      root_version,
    ...
    player.ShowExtraButtons = function (args) {
      ...
      main
        .find('.treedraw_cut')
        .val(args && args.parse_cut ? args.parse_cut : '')
        .keyup(this.keyup);
      main
        .find('.treedraw_opt')
        .val(args && args.drawopt ? args.drawopt : '')
        .keyup(this.keyup);
      main
        .find('.treedraw_number')
        .val(args && args.numentries ? args.numentries : '')
        .spinner({ numberFormat: 'n', min: 0, page: 1000, max: numentries })
        .keyup(this.keyup);
      main
        .find('.treedraw_first')
        .val(args && args.firstentry ? args.firstentry : '')
        .spinner({ numberFormat: 'n', min: 0, page: 1000, max: numentries })
        .keyup(this.keyup);
      main
        .find('.treedraw_clear')
    ...
    player.Show = function (divid, args) {
      ...
      var show_extra =
        args && (args.parse_cut || args.numentries || args.firstentry);
      ...
      main
        .find('.treedraw_varexp')
        .val(
          args && args.parse_expr ? args.parse_expr : this.dflt_expr || 'px:py'
        )
        .keyup(this.keyup);
    ...
    player.PerformDraw = function () {
      ...
      if (frame.find('.treedraw_more').length == 0) {
        ...
        url +=
          '&prototype="const char*,const char*,Option_t*,Long64_t,Long64_t"&varexp="' +
          expr +
          '"&selection="' +
          cut +
          '"';
        ...
        if (nentries == '')
          nentries =
            this.root_version >= 394499 ? 'TTree::kMaxEntries' : '1000000000'; // kMaxEntries available since ROOT 6.05/03
    ...
    if (item._childs && !asleaf)
      for (var n = 0; n < item._childs.length; ++n) {
        ...
        if (
          leaf &&
          leaf._kind &&
          leaf._kind.indexOf('ROOT.TLeaf') == 0 &&
          leaf_cnt < 2
        ) {
