...
*/
  ...
  THistPainter.prototype.GetAxis = function (name) {
    ...
    if (axis._typename == 'ROOT::Experimental::RAxisEquidistant') {
      ...
      axis.GetBinCoord = function (bin) {
        ...
        return (
          this.fBinBorders[indx] * Math.abs(bin - indx2) +
          this.fBinBorders[indx2] * Math.abs(bin - indx)
        );
      };
      axis.FindBin = function (x, add) {
  ...
  THistPainter.prototype.GetSelectIndex = function (axis, size, add) {
    ...
    if (min !== max && taxis) {
      if (size == 'left') indx = taxis.FindBin(min, add || 0);
  ...
  THistPainter.prototype.ButtonClick = function (funcname) {
    ...
    switch (funcname) {
      case 'ToggleZoom':
        ...
        break;
      case 'ToggleLogX':
        this.frame_painter().ToggleLog('x');
        break;
      case 'ToggleLogY':
        this.frame_painter().ToggleLog('y');
        break;
      case 'ToggleLogZ':
        this.frame_painter().ToggleLog('z');
        break;
      case 'ToggleStatBox':
        this.ToggleStat();
        return true;
  ...
  TH1Painter.prototype.ScanContent = function (when_axis_changed) {
    ...
    for (var i = 0; i < this.nbinsx; ++i) {
      ...
      if (value > 0) if (hmin_nz == 0 || value < hmin_nz) hmin_nz = value;
      if (first) {
        hmin = hmax = value;
        first = false;
      }
  ...
  TH1Painter.prototype.FillStatistic = function (stat, dostat, dofit) {
    ...
    if (this.IsTProfile()) {
      ...
      if (print_under > 0)
        stat.AddText(
          'Underflow = ' + stat.Format(histo.getBinContent(0), 'entries')
        );

      if (print_over > 0)
        stat.AddText(
          'Overflow = ' +
            stat.Format(histo.getBinContent(this.nbinsx + 1), 'entries')
        );
  ...
  TH1Painter.prototype.DrawBins = function () {
    ...
    if (options.Bar) return this.DrawBars(width, height);
    ...
    } else if (options.Error) path_err = '';
          ...
          if (my >= -yerr1 && my <= height + yerr2) {
            ...
            if (path_err !== null) {
              if (pthis.options.errorX > 0) {
                var mmx1 = Math.round(
                    midx - (mx2 - mx1) * pthis.options.errorX
                  ),
    ...
    for (i = left; i <= right; ++i) {
      ...
      if (res.length === 0) {
        ...
        if (grx === currx && !lastbin) {
          ...
          if (
            !draw_markers &&
            (curry_min !== curry_max || prevy !== curry_min)
          ) {
  ...
  TH1Painter.prototype.AutoZoom = function () {
    ...
    if (right - left < dist && left < right)
      this.frame_painter().Zoom(
        xaxis.GetBinCoord(left),
        xaxis.GetBinCoord(right)
      );
  };
  ...
  TH1Painter.prototype.CanZoomIn = function (axis, min, max) {
    ...
    if (axis == 'x' && xaxis.FindBin(max, 0.5) - xaxis.FindBin(min, 0) > 1)
      return true;
  ...
  TH1Painter.prototype.Draw2D = function (call_back) {
    ...
    if (this.DrawAxes()) this.DrawBins();
    else console.log('FAIL DARWING AXES');
  ...
  TH2Painter.prototype.DrawBinsColor = function (w, h) {
    ...
    for (i = handle.i1; i < handle.i2; ++i) {
      for (j = handle.j1; j < handle.j2; ++j) {
        ...
        colPaths[colindx] +=
          'v' +
          (handle.gry[j] - handle.gry[j + 1]) +
          'h' +
          (handle.grx[i + 1] - handle.grx[i]) +
          'v' +
          (handle.gry[j + 1] - handle.gry[j]) +
          'z';
      }
    }
  ...
  TH2Painter.prototype.DrawBinsText = function (w, h, handle) {
    ...
    if (this.options.fBarOffset !== 0)
      text_offset = this.options.fBarOffset * 1e-3;
  ...
  TH2Painter.prototype.DrawBinsBox = function (w, h) {
    ...
    if (main.maxbin === main.minbin) main.minbin = Math.min(0, main.maxbin - 1);
  ...
  TH2Painter.prototype.DrawCandle = function (w, h) {
    ...
    for (i = handle.i1; i < handle.i2; ++i) {
      ...
      w *= 0.66;
      center =
        (handle.grx[i + 1] + handle.grx[i]) / 2 +
        (this.options.fBarOffset / 1000) * w;
  ...
  TH2Painter.prototype.DrawBins = function () {
    ...
    if (this.IsTH2Poly()) {
      ...
      else if (this.options.Contour > 0) handle = this.DrawBinsContour(w, h);
  ...
  TH2Painter.prototype.GetBinTips = function (i, j) {
    ...
    if (pmain.x_kind == 'labels')
      ...
      lines.push(
        'x = [' +
          pmain.AxisAsText('x', xaxis.GetBinCoord(i)) +
          ', ' +
          pmain.AxisAsText('x', xaxis.GetBinCoord(i + 1)) +
          ')'
      );
    ...
    if (pmain.y_kind == 'labels')
      ...
      lines.push(
        'y = [' +
          pmain.AxisAsText('y', yaxis.GetBinCoord(j)) +
          ', ' +
          pmain.AxisAsText('y', yaxis.GetBinCoord(j + 1)) +
          ')'
      );
