(function (factory) {
  ...
  ThreeBSP.Geometry = function (
    ...
    if (geometry instanceof THREE.Geometry) {
      ...
      geometry.polygons &&
      geometry.polygons[0] instanceof ThreeBSP.Polygon
    ) {
  ...
  ThreeBSP.Polygon.prototype.splitPolygon = function (
    ...
    if (classification === COPLANAR) {
      (this.nsign * polygon.nsign * this.normal.dot(polygon.normal) > 0
        ? coplanar_front
        : coplanar_back
      ).push(polygon);
      ...
      for (i = 0; i < vertice_count; ++i) {
        ...
        if ((ti | tj) === SPANNING) {
          ...
          t =
            (this.w - (nnx * vi.x + nny * vi.y + nnz * vi.z)) /
            (nnx * (vj.x - vi.x) + nny * (vj.y - vi.y) + nnz * (vj.z - vi.z));
      ...
      if (f.length >= 3)
        front.push(new ThreeBSP.Polygon(f).copyProperties(polygon, true));
      if (b.length >= 3)
        back.push(new ThreeBSP.Polygon(b).copyProperties(polygon, true));
    }
  };
...
*/
  ...
  ThreeBSP.Node.prototype.invert = function () {
    ...
    for (var i = 0; i < polygon_count; ++i) this.polygons[i].flip();
