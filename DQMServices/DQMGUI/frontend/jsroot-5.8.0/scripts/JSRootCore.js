...
(function (factory) {
  if (typeof define === 'function' && define.amd) {
    var jsroot = factory({}),
      ...
      paths = {
        ...
        MathJax:
          'https://root.cern/js/mathjax/latest/MathJax.js?config=TeX-AMS-MML_SVG&amp;delayStartupUntil=configured',
    ...
    if (norjs) {
      ...
      var cfg_paths;
      if (
        requirejs.s !== undefined &&
        requirejs.s.contexts !== undefined &&
        requirejs.s.contexts._ !== undefined &&
        requirejs.s.contexts._.config !== undefined
      )
        cfg_paths = requirejs.s.contexts._.config.paths;
      else
        console.warn(
    ...
    if (norjs || !require.specified('jsroot')) define('jsroot', [], jsroot);
  } else if (typeof exports === 'object' /*&& typeof module !== 'undefined'*/) {
    // processing with Node.js or CommonJS

    //  mark JSROOT as used with Node.js
    exports.BatchMode = exports.nodejs =
  ...
  if (JSROOT.BatchMode === undefined) JSROOT.BatchMode = false; // when true, disables all kind of interactive features
  ...
  if (typeof document !== 'undefined' && typeof window !== 'undefined') {
    ...
    for (var n = 0; n < scripts.length; ++n) {
      ...
      JSROOT.source_min =
        scripts[n].src.indexOf('scripts/JSRootCore.min.js') >= 0;
      JSROOT.source_fullpath = scripts[n].src;

      if (console !== undefined && typeof console.log == 'function')
        console.log(
          'Set JSROOT.source_dir to ' +
            JSROOT.source_dir +
            ', ' +
            JSROOT.version
        );
    ...
    JSROOT.browser.isChromeHeadless =
      navigator.userAgent.indexOf('HeadlessChrome') >= 0;
  }

  JSROOT.browser.isWebKit =
    JSROOT.browser.isChrome ||
    JSROOT.browser.isSafari ||
    JSROOT.browser.isOpera;
  ...
  JSROOT.seed = function (i) {
    i = Math.abs(i);
    if (i > 1e8) i = Math.abs(1e8 * Math.sin(i));
    else if (i < 1) i *= 1e8;
  ...
  JSROOT.JSONR_unref = function (obj) {
    ...
    function unref_value(value) {
      ...
      var i,
        k,
        res,
        proto = Object.prototype.toString.apply(value);
      ...
      if (newfmt !== false && len > 1 && ks[0] === '$arr' && ks[1] === 'len') {
        ...
        if (value.b !== undefined) {
          ...
          if (arr.buffer) {
            ...
            for (var k = 0; k < len; ++k) dv.setUint8(k, buf.charCodeAt(k));
          } else {
            throw new Error(
              'base64 coding supported only for native arrays with binary data'
            );
          }
        } else {
          // compressed coding
          ...
          while (nkey < len) {
            ...
            if (ks[nkey][0] !== 'v')
              throw new Error(
                'Unexpected member ' + ks[nkey] + ' in array decoding'
              );
  ...
  JSROOT.clone = function (src, map, nofunc) {
    ...
    if (proto === '[object Array]') {
      ...
      for (var i = 0; i < src.length; ++i)
        if (typeof src[i] === 'object') tgt.push(JSROOT.clone(src[i], map));
        else tgt.push(src[i]);
    ...
    // process typed array
    if (
      proto.indexOf('[object ') == 0 &&
      proto.indexOf('Array]') == proto.length - 6
    ) {
      var tgt = [];
      map.obj.push(src);
      map.clones.push(tgt);
    ...
    for (var k in src) {
      if (typeof src[k] === 'object') tgt[k] = JSROOT.clone(src[k], map);
      else if (!map.nofunc || typeof src[k] !== 'function') tgt[k] = src[k];
    }
  ...
  JSROOT.clear_func = function (src, map) {
    ...
    if (proto === '[object Array]') {
      for (var n = 0; n < src.length; n++)
        if (typeof src[n] === 'object') JSROOT.clear_func(src[n], map);
      return src;
    }
    ...
    for (var k in src) {
      if (typeof src[k] === 'object') JSROOT.clear_func(src[k], map);
      else if (typeof src[k] === 'function') delete src[k];
    }

    if (nomap)
      for (var n = 0; n < map.length; ++n) delete map[n]['__clean_func__'];
  ...
  JSROOT.parse_multi = function (json) {
    ...
    if (arr && arr.length)
      for (var i = 0; i < arr.length; ++i) arr[i] = this.JSONR_unref(arr[i]);
    return arr;
  };
  ...
  JSROOT.toJSON = function (obj) {
    ...
    function copy_value(value) {
      ...
      if (
        ...
        for (var i = 0; i < value.length; ++i) arr[i] = copy_value(value[i]);
        return arr;
      }
  ...
  JSROOT.GetUrlOption = function (opt, url, dflt) {
    ...
    if (!url) {
      if (JSROOT.gStyle.IgnoreUrlOptions || typeof document === 'undefined')
        return dflt;
      url = document.URL;
    }
  ...
  JSROOT.ParseAsArray = function (val) {
    ...
    if (res.length === 0) res.push(val.substr(1, val.length - 2).trim());
  ...
  JSROOT.GetUrlOptionAsArray = function (opt, url) {
    ...
    while (opt.length > 0) {
      ...
      if (separ > 0) opt = opt.substr(separ + 1);
      else opt = '';
  ...
  JSROOT.findFunction = function (name) {
    ...
    for (var n = 0; elem && n < names.length; ++n) elem = elem[names[n]];
  ...
  JSROOT.CallBack = function (func, arg1, arg2) {
    ...
    if (
      'obj' in func &&
      'func' in func &&
      typeof func.obj == 'object' &&
      typeof func.func == 'string' &&
  ...
  JSROOT.NewHttpRequest = function (url, kind, user_call_back) {
    ...
    xhr.http_callback =
      typeof user_call_back == 'function'
        ? user_call_back.bind(xhr)
        : function () {};
    ...
    var method = 'GET',
      ...
      p = kind.indexOf(';sync');
    if (p > 0) {
      kind = kind.substr(0, p);
      async = false;
    }
    if (kind === 'head') method = 'HEAD';
    else if (kind === 'post' || kind === 'multi' || kind === 'posttext')
      method = 'POST';

    xhr.kind = kind;

    if (
      JSROOT.wrong_http_response_handling &&
      method == 'GET' &&
      ...
      xhr.addEventListener(
        ...
        function (oEvent) {
          if (
            ...
            this.expected_size &&
            oEvent.loaded > this.expected_size
          ) {
            this.did_abort = true;
            this.abort();
            console.warn(
              'Server sends more bytes ' +
                oEvent.loaded +
                ' than expected ' +
                this.expected_size +
                '. Abort I/O operation'
    ...
    xhr.onreadystatechange = function () {
      ...
      if (
        ...
        !(
          this.status == 0 &&
          (url.indexOf('file://') == 0 || url.indexOf('blob:') == 0)
        )
      ) {
        return this.http_callback(null);
      }
      ...
      if (
        this.nodejs_checkzip &&
        this.getResponseHeader('content-encoding') == 'gzip'
      ) {
      ...
      switch (this.kind) {
        case 'xml':
          return this.http_callback(this.responseXML);
        case 'posttext':
        case 'text':
          return this.http_callback(this.responseText);
        case 'object':
          return this.http_callback(JSROOT.parse(this.responseText));
        case 'multi':
          return this.http_callback(JSROOT.parse_multi(this.responseText));
        case 'head':
          return this.http_callback(this);
      }
      ...
      if (this.kind == 'bin' && 'byteLength' in this.response) {
        ...
        var filecontent = '',
          u8Arr = new Uint8Array(this.response);
  ...
  JSROOT.loadScript = function (urllist, callback, debugout, from_previous) {
    ...
    if (from_previous) {
      if (debugout) document.getElementById(debugout).innerHTML = '';
      else JSROOT.progress();
    ...
    var completeLoad = JSROOT.loadScript.bind(
      JSROOT,
      urllist,
      callback,
      debugout,
    ...
    if (filename.indexOf('&&&scripts/') === 0) {
      ...
      if (JSROOT.use_full_libs)
        filename = 'libs/' + filename.slice(8, filename.length - 7) + '.js';
    } else if (filename.indexOf('$$$') === 0) {
      isrootjs = true;
      filename = filename.slice(3);
      if (
        filename.indexOf('style/') == 0 &&
        JSROOT.source_min &&
    ...
    if (isstyle) {
      ...
      for (var n = 0; n < styles.length; ++n) {
        if (
          !styles[n].href ||
          styles[n].type !== 'text/css' ||
          styles[n].rel !== 'stylesheet'
        )
          continue;
    ...
    else if (isbower && JSROOT.bower_dir !== null)
      filename = JSROOT.bower_dir + filename;
    ...
    if (debugout)
      document.getElementById(debugout).innerHTML =
        'loading ' + filename + ' ...';
    else JSROOT.progress('loading ' + filename + ' ...');
  ...
  JSROOT.AssertPrerequisites = function (kind, callback, debugout) {
    ...
    if (!kind || typeof kind !== 'string') return jsroot.CallBack(callback);
    ...
    if (
      (kind.indexOf('hist;') >= 0 || kind.indexOf('hist3d;') >= 0) &&
      jsroot.sources.indexOf('hist') < 0
    ) {
    ...
    if (
      (kind.indexOf('hierarchy;') >= 0 || kind.indexOf('jq2d;') >= 0) &&
      jsroot.sources.indexOf('hierarchy') < 0
    ) {
    ...
    if (
      kind.indexOf('openui5;') >= 0 &&
      jsroot.sources.indexOf('openui5') < 0
    ) {
    ...
    if (
      ...
      jsroot.sources.indexOf('3d') < 0
    ) {
      mainfiles +=
        (use_bower ? '###threejs/build/' : '&&&scripts/') +
        'three.min.js;' +
    ...
    if (kind.indexOf('datgui;') >= 0) {
      if (!JSROOT.nodejs && typeof window != 'undefined')
        mainfiles +=
          (use_bower ? '###dat.gui' : '&&&scripts') + '/dat.gui.min.js;';
    ...
    if (kind.indexOf('mathjax;') >= 0) {
      if (typeof MathJax == 'undefined') {
        mainfiles +=
          (use_bower ? '###MathJax' : 'https://root.cern/js/mathjax/latest') +
    ...
    if (need_jquery && !jsroot.load_jquery) {
      var has_jq = typeof jQuery != 'undefined',
        lst_jq = '';

      if (has_jq)
        jsroot.console(
          'Reuse existing jQuery ' + jQuery.fn.jquery + ', required 3.1.1',
          debugout
        );
      else
        lst_jq +=
          (use_bower ? '###jquery/dist' : '&&&scripts') + '/jquery.min.js;';
      if (has_jq && typeof $.ui != 'undefined') {
        jsroot.console(
          'Reuse existing jQuery-ui ' + $.ui.version + ', required 1.12.1',
          debugout
        );
      } else {
        lst_jq +=
          (use_bower ? '###jquery-ui' : '&&&scripts') + '/jquery-ui.min.js;';
    ...
    for (var n = modules.length - 1; n >= 0; --n)
      if (jsroot.ready_modules.indexOf(modules[n]) >= 0) modules.splice(n, 1);
    ...
    if (modules.length > 0 && typeof define === 'function' && define.amd) {
      jsroot.console(
        'loading ' + JSON.stringify(modules) + ' with require.js',
        debugout
      );
  ...
  JSROOT.BuildSimpleGUI = function (user_scripts, andThen) {
    ...
    if (simplegui) {
      debugout = 'simpleGUI';
      if (JSROOT.GetUrlOption('file') || JSROOT.GetUrlOption('files'))
        requirements += 'io;';
      if (
        simplegui.getAttribute('nobrowser') &&
        simplegui.getAttribute('nobrowser') != 'false'
      )
        nobrowser = true;
    ...
    if (!user_scripts)
      user_scripts =
        JSROOT.GetUrlOption('autoload') || JSROOT.GetUrlOption('load');

    if (user_scripts) requirements += 'load:' + user_scripts + ';';

    JSROOT.AssertPrerequisites(
      requirements,
      function () {
        JSROOT.CallBack(
          JSROOT.findFunction(
            nobrowser ? 'JSROOT.BuildNobrowserGUI' : 'JSROOT.BuildGUI'
          )
        );
    ...
    switch (typename) {
      ...
      case 'TLegend':
        ...
        JSROOT.extend(obj, {
          ...
          fMargin: 0.25,
          fNColumns: 1,
          fPrimitives: JSROOT.Create('TList'),
        });
        break;
      case 'TLegendEntry':
        JSROOT.Create('TObject', obj);
      ...
      case 'TAttPad':
        JSROOT.extend(obj, {
          fLeftMargin: JSROOT.gStyle.fPadLeftMargin,
      ...
      case 'TPad':
        ...
        JSROOT.extend(obj, {
          ...
          fAspectRatio: 0,
          fNumber: 0,
          fLogx: JSROOT.gStyle.fOptLogx,
          fLogy: JSROOT.gStyle.fOptLogy,
          fLogz: JSROOT.gStyle.fOptLogz,
      ...
      case 'TCanvas':
        ...
        JSROOT.extend(obj, {
          ...
          fCw: 500,
          fCh: 300,
          fCatt: JSROOT.Create('TAttCanvas'),
          kMoveOpaque: true,
          kResizeOpaque: true,
          fHighLightColor: 5,
          fBatch: true,
  ...
  JSROOT.getMethods = function (typename, obj) {
    ...
    if (typename.indexOf('TF1') == 0 || typename === 'TF2') {
      ...
      m.evalPar = function (x, y) {
        if (!('_func' in this) || this._title !== this.fTitle) {
            ...
            if (this.fFormula.fClingParameters && this.fFormula.fParams) {
              for (var i = 0; i < this.fFormula.fParams.length; ++i) {
                var regex = new RegExp(
                    '(\\[' + this.fFormula.fParams[i].first + '\\])',
                    'g'
                  ),
                  parvalue = this.fFormula.fClingParameters[
                    this.fFormula.fParams[i].second
                  ];
                _func = _func.replace(
                  regex,
                  parvalue < 0 ? '(' + parvalue + ')' : parvalue
                );
              }
            }
          }
          ...
          else this._func = new Function('x', 'return ' + _func).bind(this);
      ...
      m.GetParName = function (n) {
        if (this.fParams && this.fParams.fParNames)
      ...
      m.GetParValue = function (n) {
        if (this.fParams && this.fParams.fParameters)
          return this.fParams.fParameters[n];
        if (this.fFormula && this.fFormula.fClingParameters)
    ...
    if (
      (typename.indexOf('TGraph') == 0 || typename == 'TCutG') &&
      typename != 'TGraphPolargram' &&
      typename != 'TGraphTime'
    ) {
    ...
    if (
      ...
      m.getBinError = function (bin) {
        ...
        if (bin < 0) bin = 0;
        if (bin < this.fSumw2.length) return Math.sqrt(this.fSumw2[bin]);
      ...
      m.setBinContent = function (bin, content) {
        ...
        this.fEntries++;
        this.fTsumw = 0;
        if (bin >= 0 && bin < this.fArray.length) this.fArray[bin] = content;
      };
    }
      ...
      m.Fill = function (x, weight) {
        var axis = this.fXaxis,
          bin =
            1 +
            Math.floor(
              ((x - axis.fXmin) / (axis.fXmax - axis.fXmin)) * axis.fNbins
            );
        if (bin < 0) bin = 0;
        else if (bin > axis.fNbins + 1) bin = axis.fNbins + 1;
    ...
    if (typename.indexOf('TH2') == 0) {
      ...
      m.Fill = function (x, y, weight) {
        var axis1 = this.fXaxis,
          axis2 = this.fYaxis,
          bin1 =
            1 +
            Math.floor(
              ((x - axis1.fXmin) / (axis1.fXmax - axis1.fXmin)) * axis1.fNbins
            ),
          bin2 =
            1 +
            Math.floor(
              ((y - axis2.fXmin) / (axis2.fXmax - axis2.fXmin)) * axis2.fNbins
            );
        if (bin1 < 0) bin1 = 0;
        else if (bin1 > axis1.fNbins + 1) bin1 = axis1.fNbins + 1;
        if (bin2 < 0) bin2 = 0;
        else if (bin2 > axis2.fNbins + 1) bin2 = axis2.fNbins + 1;
        this.fArray[bin1 + (axis1.fNbins + 2) * bin2] +=
    ...
    if (typename.indexOf('TH3') == 0) {
      m.getBin = function (x, y, z) {
        return (
          x + (this.fXaxis.fNbins + 2) * (y + (this.fYaxis.fNbins + 2) * z)
        );
      };
      ...
      m.Fill = function (x, y, z, weight) {
        var axis1 = this.fXaxis,
          ...
          bin3 =
            1 +
            Math.floor(
              ((z - axis3.fXmin) / (axis3.fXmax - axis3.fXmin)) * axis3.fNbins
            );
        if (bin1 < 0) bin1 = 0;
        else if (bin1 > axis1.fNbins + 1) bin1 = axis1.fNbins + 1;
        if (bin2 < 0) bin2 = 0;
        ...
        if (bin3 < 0) bin3 = 0;
        else if (bin3 > axis3.fNbins + 1) bin3 = axis3.fNbins + 1;
        this.fArray[
          bin1 + (axis1.fNbins + 2) * (bin2 + (axis2.fNbins + 2) * bin3)
        ] += weight === undefined ? 1 : weight;
      };
    }
    ...
    if (typename.indexOf('TProfile') == 0) {
      ...
      m.getBinEffectiveEntries = function (bin) {
        ...
        return sumOfWeightsSquare > 0
          ? (sumOfWeights * sumOfWeights) / sumOfWeightsSquare
          : 0;
      };
      m.getBinError = function (bin) {
        if (bin < 0 || bin >= this.fNcells) return 0;
        ...
        var contsum = cont / sum,
          eprim = Math.sqrt(Math.abs(err2 / sum - contsum * contsum));
    ...
    if (typename == 'TAxis') {
      ...
      m.GetBinCenter = function (bin) {
        ...
        return (
          this.fXmin + ((bin - 0.5) * (this.fXmax - this.fXmin)) / this.fNbins
        );
      };
    }
  ...
  JSROOT.Initialize = function () {
    ...
    if (JSROOT.GetUrlOption('nocache', src) != null)
      JSROOT.nocache = new Date().getTime(); // use timestamp to overcome cache limitation
    if (
      JSROOT.GetUrlOption('wrong_http_response', src) != null ||
      JSROOT.GetUrlOption('wrong_http_response') != null
    )
    ...
    if (JSROOT.GetUrlOption('gui', src) !== null)
      return window_on_load(function () {
        JSROOT.BuildSimpleGUI();
      });

    if (typeof define === 'function' && define.amd)
      return window_on_load(function () {
        JSROOT.BuildSimpleGUI('check_existing_elements');
      });
    ...
    if (JSROOT.GetUrlOption('hierarchy', src) != null)
      prereq += '2d;hierarchy;';
    if (JSROOT.GetUrlOption('jq2d', src) != null)
      prereq += '2d;hierarchy;jq2d;';
    ...
    if (
      bower === null &&
      JSROOT.source_dir.indexOf('bower_components/jsroot/') >= 0
    )
      bower = '';
    if (bower !== null) {
      if (bower.length > 0) JSROOT.bower_dir = bower;
      else if (
        JSROOT.source_dir.indexOf('jsroot/') ==
        JSROOT.source_dir.length - 7
      )
        JSROOT.bower_dir = JSROOT.source_dir.substr(
          0,
          JSROOT.source_dir.length - 7
        );
      if (JSROOT.bower_dir !== null)
        console.log('Set JSROOT.bower_dir to ' + JSROOT.bower_dir);
    }

    if (prereq.length > 0 || onload != null)
      window_on_load(function () {
        if (prereq.length > 0) JSROOT.AssertPrerequisites(prereq, onload);
        else if (onload != null) {
