...
(function (factory) {
  ...
  Toolbar.prototype.changeBrightness = function (bright) {
    ...
    this.element
      .selectAll(bright ? '.toolbar-btn' : '.toolbar-btn-bright')
  ...
  TGeoPainter.prototype.CreateToolbar = function (args) {
    if (this._toolbar || this._usesvg || this._usesvgimg || this.ctrl.notoolbar)
      return;
    var painter = this;
    var buttonList = [
      {
      ...
      },
      {
        ...
        click: function () {
          painter.showControlOptions('toggle');
        },
      },
      {
        name: 'enlarge',
  ...
  TGeoPainter.prototype.InitVRMode = function () {
    ...
    navigator.getVRDisplays().then(function (displays) {
      ...
      if (vrDisplay.stageParameters) {
        pthis._standingMatrix.fromArray(
  ...
  TGeoPainter.prototype.UpdateVRControllersList = function () {
    ...
    // Hide meshes.
    this._controllersMeshes.forEach(function (mesh) {
      mesh.visible = false;
    });
    this._vrControllers = [];
  ...
  TGeoPainter.prototype.ProcessVRControllerIntersections = function () {
    ...
    for (var i = 0; i < this._vrControllers.length; ++i) {
      ...
      intersects = intersects.concat(
        this._controls.GetOriginDirectionIntersects(origin, end)
      );
    }
    // Remove duplicates.
    intersects = intersects.filter(function (item, pos) {
  ...
  TGeoPainter.prototype.toggleVRMode = function () {
    ...
    this._vrDisplay
      ...
      .then(function () {
        ...
        pthis._dolly.position.set(
          pthis._camera.position.x / 4,
          -pthis._camera.position.y / 8,
          -pthis._camera.position.z / 4
        );
  ...
  TGeoPainter.prototype.decodeOptions = function (opt) {
    ...
    if (_opt !== null && _opt == 'true') {
      res._debug = true;
      res._grid = true;
    }
    if (_opt !== null && _opt == 'bound') {
      res._debug = true;
      res._grid = true;
      res._bound = true;
    }
    if (_opt !== null && _opt == 'full') {
      res._debug = true;
      res._grid = true;
      res._full = true;
      res._bound = true;
    ...
    if (d.check('ORTHO_CAMERA_ROTATE')) {
      res.ortho_camera = true;
      res.can_rotate = true;
    }
    if (d.check('ORTHO_CAMERA')) {
      res.ortho_camera = true;
      res.can_rotate = false;
    }
    ...
    if (d.check('BKGR_', true)) {
      var bckgr = null;
      if (d.partAsInt(1) > 0) bckgr = JSROOT.Painter.root_colors[d.partAsInt()];
      else
        for (var col = 0; col < 8; ++col)
          if (JSROOT.Painter.root_colors[col].toUpperCase() === d.part)
            bckgr = JSROOT.Painter.root_colors[col];
    ...
    if (d.check('PROJX', true)) {
      res.project = 'x';
      if (d.partAsInt(1) > 0) res.projectPos = d.partAsInt();
      res.can_rotate = false;
    }
    ...
    if (d.check('NOHIGHLIGHT') || d.check('NOHIGH'))
      res.highlight_scene = res.highlight = 0;
    ...
    if (d.check('TRANSP', true)) res.transparency = d.partAsInt(0, 100) / 100;
    ...
    if (res._yup === undefined) res._yup = this.svg_canvas().empty();
  ...
  TGeoPainter.prototype.TestMatrixes = function () {
    ...
    var arg = {
      ...
      func: function (node) {
        ...
        var mesh = painter._clones.CreateObject3D(
          entry.stack,
          painter._toplevel,
          'mesh'
        );
        ...
        console.log(
          painter._clones.ResolveStack(entry.stack).name,
          'maxdiff',
          max,
          'determ',
          m1.determinant(),
  ...
  TGeoPainter.prototype.changedGlobalTransparency = function (
    transparency,
    skip_render
  ) {
    ...
    this._toplevel.traverse(function (node) {
      if (
        node &&
        node.material &&
        node.material.inherentOpacity !== undefined
      ) {
        var t = func ? func(node) : undefined;
        if (t !== undefined) node.material.opacity = 1 - t;
        else
          node.material.opacity = Math.min(
            1 - (transparency || 0),
            node.material.inherentOpacity
          );
  ...
  TGeoPainter.prototype.changedTransformation = function (arg) {
    ...
    if (arg == 'reset') ctrl.trans_z = ctrl.trans_radial = 0;
    ...
    this._toplevel.traverse(function (mesh) {
      ...
      if (node.vect0 === undefined) {
        ...
        node.vect0 = new THREE.Vector3(
          (box3.max.x + box3.min.x) / 2,
          (box3.max.y + box3.min.y) / 2,
          (signz * (box3.max.z + box3.min.z)) / 2
      ...
      vect2
        .set(
          ctrl.trans_radial * node.vect0.x,
          ctrl.trans_radial * node.vect0.y,
          ctrl.trans_z * node.vect0.z
        )
        .applyMatrix4(node.minvert)
        .sub(node.vect1);

      node.matrix.multiplyMatrices(
        node.matrix0,
        translation.makeTranslation(vect2.x, vect2.y, vect2.z)
      );
      node.matrix.decompose(node.position, node.quaternion, node.scale);
      node.matrixWorldNeedsUpdate = true;
    });
    ...
    if (arg != 'norender') this.drawSimpleAxis();
  };
  ...
  TGeoPainter.prototype.showControlOptions = function (on) {
    var usedat = _dat;
    if (!usedat && typeof dat == 'object') usedat = dat;
    ...
    if (typeof usedat == 'undefined')
      return JSROOT.AssertPrerequisites(
        'datgui',
        this.showControlOptions.bind(this, 'load')
      );

    var painter = this;

    this._datgui = new usedat.GUI({
      autoPlace: false,
      width: Math.min(650, painter._renderer.domElement.width / 2),
    });
    ...
    if (this.ctrl.project) {
      ...
      this._datgui
        .add(this.ctrl, 'projectPos', bound.min[axis], bound.max[axis])
      ...
      for (var naxis = 0; naxis < 3; ++naxis) {
        ...
        clipFolder
          .add(cc, 'value', cc.min, cc.max)
          .name(axisC + ' position')
          .onChange(this.changedClipping.bind(this, naxis));
      }

      clipFolder
        .add(this.ctrl, 'clipIntersect')
    ...
    appearance
      .add(this.ctrl, 'highlight')
      .name('Highlight Selection')
      .listen()
      .onChange(this.changedHighlight.bind(this));

    appearance
      .add(this.ctrl, 'transparency', 0.0, 1.0, 0.001)
      .listen()
      .onChange(this.changedGlobalTransparency.bind(this));

    appearance
      .addColor(this.ctrl, 'background')
      .name('Background')
      .onChange(this.changedBackground.bind(this));

    appearance
      .add(this.ctrl, 'wireframe')
      .name('Wireframe')
      .listen()
      .onChange(this.changedWireFrame.bind(this));

    this.ctrl._axis_cfg = 0;
    appearance
      .add(this.ctrl, '_axis', { none: 0, show: 1, center: 2 })
      .name('Axes')
      .onChange(this.changedAxes.bind(this));

    if (!this.ctrl.project)
      appearance
        .add(this.ctrl, 'rotate')
        .name('Autorotate')
        .listen()
        .onChange(this.changedAutoRotate.bind(this));
    ...
    if (this._webgl) {
      var advanced = this._datgui.addFolder('Advanced'),
        depthcfg = {};
      this.ctrl.depthMethodItems.forEach(function (i) {
        depthcfg[i.name] = i.value;
      });

      advanced
        .add(this.ctrl, 'depthTest')
        .name('Depth test')
        .listen()
        .onChange(this.changedDepthTest.bind(this));

      advanced
        .add(this.ctrl, 'depthMethod', depthcfg)
    ...
    if (!this.ctrl.project) {
      ...
      transform
        .add(this.ctrl, 'trans_z', 0, 3, 0.01)
        .name('Z axis')
        .listen()
        .onChange(this.changedTransformation.bind(this));
      transform
        .add(this.ctrl, 'trans_radial', 0, 3, 0.01)
        .name('Radial')
        .listen()
        .onChange(this.changedTransformation.bind(this));
    ...
    var ssaofolder = this._datgui.addFolder('Smooth Lighting (SSAO)'),
      ssao_handler = this.changedSSAO.bind(this),
      ssaocfg = {};

    this.ctrl.ssao.outputItems.forEach(function (i) {
      ssaocfg[i.name] = i.value;
    });

    ssaofolder
      .add(this.ctrl.ssao, 'enabled')
      .name('Enable SSAO')
      .listen()
      .onChange(ssao_handler);

    ssaofolder
      .add(this.ctrl.ssao, 'output', ssaocfg)
      .listen()
      .onChange(ssao_handler);

    ssaofolder
      .add(this.ctrl.ssao, 'kernelRadius', 0, 32)
      .listen()
      .onChange(ssao_handler);

    ssaofolder
      .add(this.ctrl.ssao, 'minDistance', 0.001, 0.02)
      .listen()
      .onChange(ssao_handler);

    ssaofolder
      .add(this.ctrl.ssao, 'maxDistance', 0.01, 0.3)
      .listen()
      .onChange(ssao_handler);
  };
  ...
  TGeoPainter.prototype.createSSAO = function () {
    ...
    if (!this._ssaoPass) {
      if (!this._effectComposer) {
        ...
        this._effectComposer.addPass(
          new THREE.RenderPass(this._scene, this._camera)
        );
      }

      this._ssaoPass = new THREE.SSAOPass(
        this._scene,
        this._camera,
        this._scene_width,
        this._scene_height
        ...
        for (var n = 0; n < intersects.length; ++n) {
          ...
          if (obj.geo_name) {
            ...
            if (itemname.indexOf('<prnt>') == 0)
              itemname =
                (menu.painter.GetItemName() || 'top') + itemname.substr(6);
            ...
            if (name.indexOf('Nodes/') === 0) hdr = name.substr(6);
            else if (name.length > 0) hdr = name;
            else if (!hdr) hdr = 'header';
          } else continue;

          menu.add((many ? 'sub:' : 'header:') + hdr, itemname, function (arg) {
            this.ActivateInBrowser([arg], true);
          });

          menu.add('Browse', itemname, function (arg) {
            this.ActivateInBrowser([arg], true);
          });

          if (menu.painter._hpainter)
            menu.add('Inspect', itemname, function (arg) {
              this._hpainter.display(itemname, 'inspect');
            });
          ...
          if (++cnt > 1)
            menu.add('Manifest', n, function (indx) {
              ...
              if (this._last_hidden)
                this._last_hidden.forEach(function (obj) {
                  obj.visible = true;
                });
          ...
          if (!menu.painter._geom_viewer)
            menu.add('Hide', n, function (indx) {
              var resolve = menu.painter._clones.ResolveStack(
                intersects[indx].object.stack
              );
              ...
              if (
                ...
                JSROOT.GEO.updateBrowserIcons(
                  resolve.obj.fVolume,
                  this._hpainter
                );
              } else if (resolve.obj && resolve.node.kind === 1) {
    ...
    if (clip[0].enabled || clip[1].enabled || clip[2].enabled) {
      ...
      for (var i = 0; i < intersects.length; ++i) {
        var point = intersects[i].point,
          special = intersects[i].object.type == 'Points',
          clipped = true;

        if (
          clip[0].enabled &&
          myXor(
            this._clipPlanes[0].normal.dot(point) >
        ...
        if (
          clip[1].enabled &&
          myXor(
            this._clipPlanes[1].normal.dot(point) >
              this._clipPlanes[1].constant,
            special
          )
        )
          clipped = false;
        if (
          clip[2].enabled &&
          this._clipPlanes[2].normal.dot(point) > this._clipPlanes[2].constant
        )
          clipped = false;
  ...
  GeoDrawingControl.prototype = Object.create(
    JSROOT.Painter.InteractiveControl.prototype
  );
  ...
  GeoDrawingControl.prototype.drawSpecial = function (col, indx) {
    ...
    if (col) {
      ...
      if (c.highlightScale) c.material.size = c.origin.size * c.highlightScale;
      return true;
    } else if (c.origin) {
      c.material.color = c.origin.color;
      c.material.opacity = c.origin.opacity;
      if (c.hightlightWidthScale) c.material.linewidth = c.origin.width;
      if (c.highlightScale) c.material.size = c.origin.size;
      return true;
    }
  };
  ...
  TGeoPainter.prototype.HighlightMesh = function (
    active_mesh,
    color,
    geo_object,
    geo_index,
    ...
    if (geo_object) {
      ...
      this._toplevel.traverse(function (mesh) {
        if (
          ...
          JSROOT.GEO.IsSameStack(mesh.stack, geo_stack)
        )
    ...
    if (!no_recursive) {
      ...
      var lst =
        ...
        (!this._main_painter
          ...
          : this._main_painter._slave_painters.concat([this._main_painter]));
    ...
    if (curr_mesh && active_mesh && curr_mesh.length == active_mesh.length) {
      ...
      for (var k = 0; k < curr_mesh.length && same; ++k) {
        if (
          curr_mesh[k] !== active_mesh[k] ||
          get_ctrl(curr_mesh[k]).checkHighlightIndex(geo_index)
        )
          same = false;
      }
    }
  ...
  TGeoPainter.prototype.ProcessMouseClick = function (pnt, intersects, evnt) {
    ...
    if (ctrl.setSelected('blue', click_indx)) this.Render3D();
  ...
  TGeoPainter.prototype.setMouseTmout = function (val) {
    if (this.ctrl) this.ctrl.mouse_tmout = val;

    if (this._controls) this._controls.mouse_tmout = val;
  };
  ...
  TGeoPainter.prototype.setDepthMethod = function (val) {
    if (this.ctrl) this.ctrl.depthMethod = val;
  };
  ...
  TGeoPainter.prototype.addOrbitControls = function () {
    ...
    this._controls = JSROOT.Painter.CreateOrbitControl(
      this,
      this._camera,
      this._scene,
      this._renderer,
    ...
    this._controls.ProcessMouseMove = function (intersects) {
      ...
      for (var k = 0; k < intersects.length; ++k) {
        var obj = intersects[k].object,
          info = null;
        if (!obj) continue;
        if (obj.geo_object) info = obj.geo_name;
        else if (obj.stack) info = painter.GetStackFullName(obj.stack);
        if (!info) continue;
      ...
      return {
        name: resolve.obj.fName,
        title: resolve.obj.fTitle || resolve.obj._typename,
        lines: lines,
      };
    };
    ...
    this._controls.ProcessDblClick = function () {
      if (painter._last_manifest) {
        ...
        if (painter._last_hidden)
          painter._last_hidden.forEach(function (obj) {
            obj.visible = true;
          });
  ...
  TGeoPainter.prototype.addTransformControl = function () {
    ...
    this._tcontrols = new THREE.TransformControls(
      this._camera,
    ...
    window.addEventListener('keydown', function (event) {
      switch (event.keyCode) {
        case 81: // Q
          painter._tcontrols.setSpace(
            painter._tcontrols.space === 'local' ? 'world' : 'local'
          );
          break;
        case 17: // Ctrl
          painter._tcontrols.setTranslationSnap(
            Math.ceil(painter._overall_size) / 50
          );
        ...
        case 189:
        case 109: // -, _, num-
          painter._tcontrols.setSize(
            Math.max(painter._tcontrols.size - 0.1, 0.1)
          );
          break;
      }
    });
    ...
    this._tcontrols.addEventListener('change', function () {
      painter.Render3D(0);
    });
  };
  ...
  TGeoPainter.prototype.nextDrawAction = function () {
    ...
    if (this.drawing_stage == 1) {
      ...
      var need_worker =
        !JSROOT.BatchMode &&
        (numvis > 10000 || (matrix && this._clones.ScanVisible() > 1e5));
      ...
      if (!need_worker || !this._worker_ready) {
        ...
        var res = this._clones.CollectVisibles(
          this._current_face_limit,
          frustum
        );
    ...
    if (this.drawing_stage == 3) {
      ...
      if (this._new_append_nodes) {
        ...
        var del;
        if (this._geom_viewer) del = this._draw_nodes;
        else
          del = this._clones.MergeVisibles(
    ...
    if (this.drawing_stage === 4) {
      ...
      this._build_shapes = this._clones.MergeShapesLists(
        this._build_shapes,
        shapes
      );
    ...
    if (this.drawing_stage === 5) {
      ...
      if (this.canSubmitToWorker()) {
        var job = { limit: this._current_face_limit, shapes: [] },
          cnt = 0;
        for (var n = 0; n < this._build_shapes.length; ++n) {
          var clone = null,
            item = this._build_shapes[n];
          ...
          if (item.ready || item.geom) {
            ...
            clone = {
              id: item.id,
              ready: true,
              nfaces: JSROOT.GEO.numGeometryFaces(item.geom),
              refcnt: item.refcnt,
            };
          } else {
    ...
    if (this.drawing_stage === 7 || this.drawing_stage === 8) {
      if (this.drawing_stage === 7) {
        // building shapes
        var res = this._clones.BuildShapes(
          this._build_shapes,
          this._current_face_limit,
          500
        );
  ...
  TGeoPainter.prototype.createEntryMesh = function (entry, shape, toplevel) {
    ...
    if (this._splitColors && entry.stack) {
      if (entry.stack[0] === 0) entry.custom_color = 'green';
      else if (entry.stack[0] === 1) entry.custom_color = 'blue';
    }
    ...
    prop.material.side = this.ctrl.bothSides
      ? THREE.DoubleSide
      : THREE.FrontSide;
    ...
    if (this.ctrl._debug || this.ctrl._full) {
      var wfg = new THREE.WireframeGeometry(mesh.geometry),
        wfm = new THREE.LineBasicMaterial({
  ...
  TGeoPainter.prototype.appendMoreNodes = function (nodes, from_drawing) {
    ...
    if (this._more_nodes)
      for (var n = 0; n < this._more_nodes.length; ++n) {
        ...
        var obj3d = this._clones.CreateObject3D(
          entry.stack,
          this._toplevel,
          'delete_mesh'
        );
  ...
  TGeoPainter.prototype.getProjectionSource = function () {
    if (this._clones_owner) return this._full_geom;
    if (!this._main_painter) {
      console.warn('MAIN PAINTER DISAPPER');
      return null;
    }
  ...
  TGeoPainter.prototype.getGeomBoundingBox = function (topitem, scalar) {
    ...
    topitem.traverse(function (mesh) {
      if (
        check_any ||
        (mesh.stack && mesh instanceof THREE.Mesh) ||
    ...
    if (scalar !== undefined)
      box3.expandByVector(
        box3.getSize(new THREE.Vector3()).multiplyScalar(scalar)
      );
  ...
  TGeoPainter.prototype.doProjection = function () {
    var toplevel = this.getProjectionSource(),
      pthis = this;
    ...
    if (this.ctrl.projectPos === undefined) {
      var bound = this.getGeomBoundingBox(toplevel),
        min = bound.min[this.ctrl.project],
        max = bound.max[this.ctrl.project],
        mean = (min + max) / 2;
    ...
    toplevel.traverse(function (mesh) {
      ...
      var geom2 = JSROOT.GEO.projectGeometry(
        mesh.geometry,
        mesh.parent.matrixWorld,
        pthis.ctrl.project,
        pthis.ctrl.projectPos,
        mesh._flippedMesh
      );
  ...
  TGeoPainter.prototype.SameMaterial = function (node1, node2) {
    ...
    var m1 =
      node1.fVolume.fMedium !== null ? node1.fVolume.fMedium.fMaterial : null;
    var m2 =
      node2.fVolume.fMedium !== null ? node2.fVolume.fMedium.fMaterial : null;
  ...
  TGeoPainter.prototype.changedLight = function (box) {
    ...
    for (var k = 0; k < this._camera.children.length; ++k) {
      var light = this._camera.children[k],
        enabled = false;
    ...
    lights.forEach(function (light) {
      light.power = (4 * Math.PI) / lights.length;
    });
  ...
  TGeoPainter.prototype.createScene = function (w, h) {
    ...
    this._scene.overrideMaterial = new THREE.MeshLambertMaterial({
      color: 0x7000ff,
      transparent: true,
      opacity: 0.2,
      depthTest: false,
    ...
    if (this.ctrl.ortho_camera) {
      ...
      this._camera.up = this.ctrl._yup
    ...
    var rrr = JSROOT.Painter.Create3DRenderer(
      w,
      h,
      this._usesvg,
      this._usesvgimg,
  ...
  TGeoPainter.prototype.adjustCameraPosition = function (
    first_time,
    keep_zoom
  ) {
    if (!this._toplevel) return;
    ...
    if (first_time)
      for (var naxis = 0; naxis < 3; ++naxis) {
        ...
        cc.max += sz * 0.01;
        cc.min -= sz * 0.01;
        if (!cc.value) cc.value = (cc.min + cc.max) / 2;
        else if (cc.value < cc.min) cc.value = cc.min;
        else if (cc.value > cc.max) cc.value = cc.max;
      }
    ...
    if ((this.ctrl.rotatey || this.ctrl.rotatez) && this.ctrl.can_rotate) {
      ...
      switch (this.ctrl.project) {
        case 'x':
          this._camera.position.set(k * 1.5 * Math.max(sizey, sizez), 0, 0);
          break;
        case 'y':
          this._camera.position.set(0, k * 1.5 * Math.max(sizex, sizez), 0);
          break;
        case 'z':
          this._camera.position.set(0, 0, k * 1.5 * Math.max(sizex, sizey));
          break;
      }
    } else if (this.ctrl._yup) {
      this._camera.position.set(
        midx - k * Math.max(sizex, sizez),
        midy + k * sizey,
        midz - k * Math.max(sizex, sizez)
      );
    } else {
      this._camera.position.set(
        midx - k * Math.max(sizex, sizey),
        midy - k * Math.max(sizex, sizey),
        midz + k * sizez
      );
    }
  ...
  TGeoPainter.prototype.focusCamera = function (focus, autoClip) {
    if (this.ctrl.project) return this.adjustCameraPosition();
    ...
    if (this.ctrl._yup)
      position = new THREE.Vector3(
        midx - 2 * Math.max(sizex, sizez),
        midy + 2 * sizey,
        midz - 2 * Math.max(sizex, sizez)
      );
    else
      position = new THREE.Vector3(
        midx - 2 * Math.max(sizex, sizey),
        midy - 2 * Math.max(sizex, sizey),
        midz + 2 * sizez
      );
    ...
    function animate() {
      ...
      if (painter._animating) {
        ...
        if (!painter._geom_viewer) painter.startDrawGeometry();
      }
      var smoothFactor = -Math.cos((2.0 * Math.PI * step) / frames) + 1.0;
      painter._camera.position.add(
        posIncrement.clone().multiplyScalar(smoothFactor)
      );
      ...
      if (autoClip) {
        for (var axis = 0; axis < 3; ++axis)
          painter.ctrl.clip[axis].value +=
            painter.ctrl.clip[axis].inc * smoothFactor;
  ...
  TGeoPainter.prototype.autorotate = function (speed) {
    ...
    function animate() {
      ...
      if (painter._controls) {
        ...
        painter._controls.autoRotateSpeed =
          (rotSpeed * (current.getTime() - last.getTime())) / 16.6666;
    ...
    if (this.ctrl._debug || this.ctrl._grid) {
      ...
      this._scene.add(
        new THREE.GridHelper(
          Math.ceil(this._overall_size),
          Math.ceil(this._overall_size) / 50
        )
      );
      this.helpText(
  ...
  TGeoPainter.prototype.drawCount = function (unqievis, clonetm) {
    ...
    var arg = {
      cnt: [],
      func: function (node) {
        if (this.cnt[this.last] === undefined) this.cnt[this.last] = 1;
        else this.cnt[this.last]++;

        nshapes += JSROOT.GEO.CountNumShapes(
          painter._clones.GetNodeShape(node.id)
        );
  ...
  TGeoPainter.prototype.PerformDrop = function (
    obj,
    itemname,
    hitem,
    opt,
  ...
  TGeoPainter.prototype.ExtraObjectVisible = function (
    hpainter,
    hitem,
    toggle
  ) {
  ...
  TGeoPainter.prototype.drawGeoTrack = function (track, itemname) {
    ...
    var track_width = track.fLineWidth || 1,
      track_color =
        JSROOT.Painter.root_colors[track.fLineColor] || 'rgb(255,0,255)';
  ...
  TGeoPainter.prototype.drawEveTrack = function (track, itemname) {
    ...
    var buf = new Float32Array((track.fN - 1) * 6),
      pos = 0,
      projv = this.ctrl.projectPos,
      projx = this.ctrl.project === 'x',
      projy = this.ctrl.project === 'y',
  ...
  TGeoPainter.prototype.drawHit = function (hit, itemname) {
    ...
    for (var i = 0; i < size; i++)
      pnts.AddPoint(
        projx ? projv : hit.fP[i * 3],
        projy ? projv : hit.fP[i * 3 + 1],
        projz ? projv : hit.fP[i * 3 + 2]
      );

    var mesh = pnts.CreatePoints({
      color: JSROOT.Painter.root_colors[hit.fMarkerColor] || 'rgb(0,0,255)',
      style: hit.fMarkerStyle,
      callback: function (delayed) {
        if (delayed) this.Render3D(100);
      }.bind(this),
    });
  ...
  TGeoPainter.prototype.FindNodeWithVolume = function (
    name,
    action,
    prnt,
    itemname,
    ...
    if (prnt.fVolume.fNodes)
      for (var n = 0; n < prnt.fVolume.fNodes.arr.length; ++n) {
        res = this.FindNodeWithVolume(
          name,
          action,
          prnt.fVolume.fNodes.arr[n],
          itemname,
          volumes
        );
        if (res) break;
  ...
  TGeoPainter.prototype.checkScript = function (script_name, call_back) {
    ...
    if (
      !script_name ||
      script_name.length < 3 ||
      JSROOT.GEO.NodeKind(draw_obj) !== 0
    )
    ...
    var mgr = {
      GetVolume: function (name) {
        ...
        var currnode = painter.FindNodeWithVolume(regexp, function (arg) {
          return arg;
        });
  ...
  TGeoPainter.prototype.prepareObjectDraw = function (draw_obj, name_prefix) {
    ...
    if (name_prefix == '__geom_viewer_append__') {
      ...
      var lvl = this.ctrl.vislevel,
        maxnodes = this.ctrl.maxnodes;
      if (this.geo_manager) {
        if (!lvl && this.geo_manager.fVisLevel)
          lvl = this.geo_manager.fVisLevel;
        if (!maxnodes) maxnodes = this.geo_manager.fMaxVisNodes;
      }
      ...
      var uniquevis = this.ctrl.no_screen
        ? 0
        : this._clones.MarkVisibles(true, false, hide_top_volume);

      if (uniquevis <= 0)
        uniquevis = this._clones.MarkVisibles(false, false, hide_top_volume);
      else uniquevis = this._clones.MarkVisibles(true, true, hide_top_volume); // copy bits once and use normal visibility bits
      ...
      if (this.options._count) return this.drawCount(uniquevis, spent);
    }
  ...
  TGeoPainter.prototype.showDrawInfo = function (msg) {
    ...
    if (!msg) {
      ...
      if (info.empty())
        info = d3.select(main).append('p').attr('class', 'geo_info');
    ...
    while (true) {
      ...
      if (now - tm0 > interval || res === 1 || res === 2) {
        ...
        if (
          this._first_drawing &&
          this._webgl &&
          this._num_meshes - this._last_render_meshes > 100 &&
          now - this._last_render_tm > 2.5 * interval
        ) {
        ...
        if (res !== 2)
          setTimeout(this.continueDraw.bind(this), res === 1 ? 100 : 1);
  ...
  TGeoPainter.prototype.TestCameraPosition = function (force) {
    ...
    if (!this.ctrl.project && this._webgl)
      JSROOT.GEO.produceRenderOrder(
        this._toplevel,
        origin,
        this.ctrl.depthMethod,
        this._clones
      );
  };
  ...
  TGeoPainter.prototype.Render3D = function (tmout, measure) {
    ...
    if (tmout <= 0 || this._usesvg) {
      ...
      if (
        ...
        this._effectComposer.passes.length > 0
      ) {
    ...
    if (!this.render_tmout)
      this.render_tmout = setTimeout(
        this.Render3D.bind(this, 0, measure),
        tmout
      );
  };
  ...
  TGeoPainter.prototype.startWorker = function () {
    ...
    this._worker.onmessage = function (e) {
      ...
      if ('log' in e.data) return JSROOT.console('geo: ' + e.data.log);

      if ('progress' in e.data) return JSROOT.progress(e.data.progress);
        ...
        if (item.buf_pos && item.buf_norm) {
          if (item.buf_pos.length === 0) {
            ...
            console.error(
              ...
              item.buf_pos.length,
              'item.buf_norm',
              item.buf_norm.length
            );
            origin.geom = null;
          } else {
            origin.geom = new THREE.BufferGeometry();

            origin.geom.addAttribute(
              'position',
  ...
  TGeoPainter.prototype.drawSimpleAxis = function (norender) {
    ...
    if (!this.ctrl._axis) return norender ? null : this.Render3D();
    ...
    for (var naxis = 0; naxis < numaxis; ++naxis) {
      ...
      switch (naxis) {
        case 0:
          buf[3] = box.max.x;
          if (yup[0] && !ortho) lbl = labels[0] + ' ' + lbl;
          else lbl += ' ' + labels[0];
          break;
        case 1:
          buf[4] = box.max.y;
          if (yup[1]) lbl += ' ' + labels[1];
          else lbl = labels[1] + ' ' + lbl;
          break;
        case 2:
          buf[5] = box.max.z;
          lbl += ' ' + labels[2];
      ...
      if (this.ctrl._axis == 2)
        for (var k = 0; k < 6; ++k) if (k % 3 !== naxis) buf[k] = center[k % 3];
      ...
      if (
        center[naxis] === 0 &&
        center[naxis] >= box.min[name] &&
        center[naxis] <= box.max[name]
      )
      ...
      var text3d = new THREE.TextGeometry(lbl, {
        font: JSROOT.threejs_font_helvetiker_regular,
        size: text_size,
        height: 0,
        curveSegments: 5,
      });
      ...
      text3d = new THREE.TextGeometry(Convert(box.min[name]), {
        font: JSROOT.threejs_font_helvetiker_regular,
        size: text_size,
        height: 0,
        curveSegments: 5,
  ...
  TGeoPainter.prototype.setAxesDraw = function (on) {
    if (on === 'toggle') this.ctrl._axis = this.ctrl._axis ? 0 : 1;
    else this.ctrl._axis = typeof on == 'number' ? on : on ? 1 : 0;
    this.drawSimpleAxis();
  };
  ...
  TGeoPainter.prototype.changedHighlight = function () {
    if (!this.ctrl.highlight) this.HighlightMesh(null);
  };

  /** Assign clipping attributes to the meshes - supported only for webgl @private */
  TGeoPainter.prototype.updateClipping = function (
    without_render,
    force_traverse
  ) {
    if (!this._webgl) return;
    ...
    var clip = this.ctrl.clip,
      ...
      constants = [
        clip[0].value,
        -1 * clip[1].value,
        (this.ctrl._yup ? -1 : 1) * clip[2].value,
      ],
  ...
  TGeoPainter.prototype.completeDraw = function (close_progress) {
    ...
    if (!this._clones) {
      ...
      var extras =
        (this._main_painter ? this._main_painter._extraObjects : null) ||
        this._extraObjects;
    ...
    if (first_time) {
      ...
      if (this._webgl && this.ctrl.rotate && !this.ctrl.project)
        this.autorotate(2.5);
      if (!this._usesvg && this.ctrl.show_controls && !JSROOT.BatchMode)
        this.showControlOptions(true);
    }
    ...
    if (this._draw_nodes_again) return this.startDrawGeometry(); // relaunch drawing
  ...
  TGeoPainter.prototype.RemoveDrawnNode = function (nodeid) {
    ...
    for (var n = 0; n < this._draw_nodes.length; ++n) {
      ...
      if (
        entry.nodeid === nodeid ||
        this._clones.IsNodeInStack(nodeid, entry.stack)
      ) {
  ...
  TGeoPainter.prototype.Cleanup = function (first_time) {
    if (!first_time) {
      ...
      if (this._tcontrols) this._tcontrols.dispose();

      if (this._controls) this._controls.Cleanup();
      ...
      if (this._datgui) this._datgui.destroy();
      ...
      if (obj && this.ctrl.is_main) {
        if (obj.$geo_painter === this) delete obj.$geo_painter;
        else if (obj.fVolume && obj.fVolume.$geo_painter === this)
  ...
  TGeoPainter.prototype.CheckResize = function (arg) {
    ...
    if (pad_painter) if (!pad_painter.CheckCanvasResize(arg)) return false;

    var sz = this.size_for_3d();

    if (this._scene_width === sz.width && this._scene_height === sz.height)
      return false;
  ...
  TGeoPainter.prototype.toggleEnlarge = function () {
    ...
    if (this.enlarge_main('toggle')) this.CheckResize();
  };
  ...
  TGeoPainter.prototype.accessObjectWireFrame = function (obj, on) {
    ...
    if (!obj.hasOwnProperty('material') || obj instanceof THREE.GridHelper)
      return;

    if (this.ownedByTransformControls(obj)) return;

    if (on !== undefined && obj.stack) obj.material.wireframe = on;
  ...
  TGeoPainter.prototype.changedWireFrame = function () {
    ...
    var painter = this,
      on = this.ctrl.wireframe;

    this._scene.traverse(function (obj) {
      painter.accessObjectWireFrame(obj, on);
    });
  ...
  TGeoPainter.prototype.UpdateObject = function (obj) {
    ...
    if (this.geo_manager && obj._typename == 'TGeoManager') {
      ...
      this.AssignObject({
        ...
        fName: obj.fMasterVolume.fName,
        $geoh: obj.fMasterVolume.$geoh,
        _proxy: true,
      });
      return true;
    }
  ...
  JSROOT.Painter.drawGeoObject = function (divid, obj, opt) {
    ...
    if (painter.ctrl.is_main && !obj.$geo_painter) obj.$geo_painter = painter;
  ...
  JSROOT.GEO.buildCompositeVolume = function (comp, maxlvl, side) {
    ...
    node1.fVolume = JSROOT.GEO.buildCompositeVolume(
      comp.fNode.fLeft,
      maxlvl - 1,
      side + 'Left'
    );
    ...
    node2.fVolume = JSROOT.GEO.buildCompositeVolume(
      comp.fNode.fRight,
      maxlvl - 1,
      side + 'Right'
    );
  ...
  JSROOT.GEO.provideVisStyle = function (obj) {
    ...
    var vis =
        !JSROOT.GEO.TestBit(obj, JSROOT.GEO.BITS.kVisNone) &&
  ...
  JSROOT.GEO.createItem = function (node, obj, name) {
    ...
    if (obj._typename == 'TGeoMaterial') sub._icon = 'img_geomaterial';
    else if (obj._typename == 'TGeoMedium') sub._icon = 'img_geomedium';
    else if (obj._typename == 'TGeoMixture') sub._icon = 'img_geomixture';
    else if (obj._typename.indexOf('TGeoNode') === 0 && obj.fVolume) {
    ...
    if (volume || shape || subnodes) {
      ...
      if (!sub._title && obj._typename != 'TGeoVolume')
        sub._title = obj._typename;

      if (shape) {
        if (sub._title == '') sub._title = shape._typename;
      ...
      if (volume) sub._icon += JSROOT.GEO.provideVisStyle(volume);
      else if (iseve) sub._icon += JSROOT.GEO.provideVisStyle(obj);
  ...
  JSROOT.GEO.createList = function (parent, lst, name, title) {
    ...
    item._expand = function (node, lst) {
      ...
      if ('fVolume' in lst) lst = lst.fVolume.fNodes;
      ...
      for (var n in lst.arr) JSROOT.GEO.createItem(node, lst.arr[n]);
  ...
  JSROOT.GEO.provideMenu = function (menu, item, hpainter) {
    ...
    function ScanEveVisible(obj, arg, skip_this) {
      ...
      if (!skip_this) {
        if (arg.assign !== undefined) obj.fRnrSelf = arg.assign;
        else if (obj.fRnrSelf) arg.vis++;
        else arg.hidden++;
      }
      ...
      if (arg === 'self') {
        ...
        hpainter.ForEach(function (m) {
          ...
          if (m._geoobj && m._icon) {
            m._icon =
              item._icon.split(' ')[0] + JSROOT.GEO.provideVisStyle(m._geoobj);
    ...
    if (
      item._geoobj._typename.indexOf('TGeoNode') === 0 &&
      JSROOT.GEO.findItemWithPainter(item)
    )
      menu.add('Focus', function () {
        ...
        if (
          drawitem._painter &&
          typeof drawitem._painter.focusOnItem == 'function'
        )
    ...
    if (iseve) {
      ...
      if (res.hidden + res.visible > 0)
        menu.addchk(
          res.hidden == 0,
          'Daughters',
          res.hidden != 0 ? 'true' : 'false',
          ToggleEveVisibility
        );
    } else {
      menu.addchk(
        JSROOT.GEO.TestBit(vol, JSROOT.GEO.BITS.kVisNone),
        'Invisible',
        JSROOT.GEO.BITS.kVisNone,
        ToggleMenuBit
      );
      menu.addchk(
        JSROOT.GEO.TestBit(vol, JSROOT.GEO.BITS.kVisThis),
        'Visible',
        JSROOT.GEO.BITS.kVisThis,
        ToggleMenuBit
      );
      menu.addchk(
        JSROOT.GEO.TestBit(vol, JSROOT.GEO.BITS.kVisDaughters),
        'Daughters',
  ...
  JSROOT.GEO.browserIconClick = function (hitem, hpainter) {
    if (hitem._volume) {
      if (
        hitem._more &&
        hitem._volume.fNodes &&
        hitem._volume.fNodes.arr.length > 0
      )
        JSROOT.GEO.ToggleBit(hitem._volume, JSROOT.GEO.BITS.kVisDaughters);
      else JSROOT.GEO.ToggleBit(hitem._volume, JSROOT.GEO.BITS.kVisThis);
    ...
    if (
      hitem._geoobj &&
      (hitem._geoobj._typename == 'TEveGeoShapeExtract' ||
        hitem._geoobj._typename == 'ROOT::Experimental::TEveGeoShapeExtract')
    ) {
  ...
  JSROOT.GEO.getBrowserIcon = function (hitem, hpainter) {
    var icon = '';
    if (hitem._kind == 'ROOT.TEveTrack') icon = 'img_evetrack';
    else if (hitem._kind == 'ROOT.TEvePointSet') icon = 'img_evepoints';
    else if (hitem._kind == 'ROOT.TPolyMarker3D') icon = 'img_evepoints';
    if (icon.length > 0) {
      var drawitem = JSROOT.GEO.findItemWithPainter(hitem);
      if (drawitem)
  ...
  JSROOT.addDrawFunc({
    name: 'TGeoVolumeAssembly',
    icon: 'img_geoassembly',
  ...
  JSROOT.addDrawFunc({
    name: 'TEvePointSet',
    icon_get: JSROOT.GEO.getBrowserIcon,
    icon_click: JSROOT.GEO.browserIconClick,
  });
  JSROOT.addDrawFunc({
    name: 'TEveTrack',
    icon_get: JSROOT.GEO.getBrowserIcon,
