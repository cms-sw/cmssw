...
struct EmuChamberPart {
  ...
  EmuChamberPart() { side = station = ring = chamber = chIndex = ipart = partIndex = 0; }
...
public:
  void addWgHit(int layer, int wg) { wgs[layer].push_back(wg + layer); }

  void addHsHit(int layer, int hs) { hss[layer].push_back(hs + layer); }
