...
private:
  ...
  void postDrawTH2F(TCanvas *, const VisDQMObject &o) {
    ...
    if (name.find("rctIsoEmEff1") != std::string::npos && name.find("rctIsoEmEff1") != name.find("rctIsoEmEff1Occ")) {
      dummybox->Draw("box,same");
      return;
    }

    if (name.find("rctIsoEmEff2") != std::string::npos && name.find("rctIsoEmEff2") != name.find("rctIsoEmEff2Occ")) {
      dummybox->Draw("box,same");
      return;
    }
