...
private:
  void preDrawTH1F(TCanvas *, const VisDQMObject &o) {
    ...
    // - MET
    else if (!(REMATCH("D0", o.name) || REMATCH("Dz", o.name) || REMATCH("Dxy", o.name) || REMATCH("Phi", o.name) ||
               REMATCH("Eta", o.name) || REMATCH("_LS", o.name) || REMATCH("Flag", o.name) || REMATCH("Chi2", o.name) ||
               REMATCH("Flag", o.name) || REMATCH("Frac", o.name) ||
               //REMATCH("_logx", o.name)       ||
               REMATCH("NHits", o.name) || REMATCH("NLayers", o.name)  //||
