...
public:
  ...
  void postDraw(TCanvas* canvas, const VisDQMObject& o, const VisDQMImgInfo& ri) override {
    ...
    if (lower < 1 || lower > obj->GetNbinsY())
      lower = 1;
    if (upper < 1 || upper > obj->GetNbinsY())
      upper = obj->GetNbinsY();

    for (int i = lower; i <= upper; i++) {
      auto name = std::string(obj->GetName()) + "_" + std::to_string(i);
      TH1* h = new TH1F(name.c_str(), "", obj->GetNbinsX(), obj->GetXaxis()->GetXmin(), obj->GetXaxis()->GetXmax());
      h->SetBit(kCanDelete);
      double entries = 0;
      int nonzerobins = 0;
      for (int x = 1; x < obj->GetNbinsX(); x++) {
        if (obj->GetBinContent(x, i) > 0)
          nonzerobins++;
      ...
      n_color++;
      if (n_color == 10)
        n_color = 15;  // skip low saturation stuff in between.
