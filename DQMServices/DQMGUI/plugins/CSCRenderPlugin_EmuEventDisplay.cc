...
EmuEventDisplay::EmuEventDisplay() {
  ...
  while (chPart.next(detector)) {
    double rd =
        (detector.NumberOfStrips(chPart.station, chPart.ring) * detector.stripDPhiRad(chPart.station, chPart.ring)) / 2;
    ...
    {  // Z-R plane elements
      ...
      double r2 = detector.R_mm(chPart.side,
                                chPart.station,
                                chPart.ring,
                                chPart.part,
                                N_LAYERS,
                                detector.NumberOfHalfstrips(chPart.station, chPart.ring, chPart.part),
                                detector.NumberOfWiregroups(chPart.station, chPart.ring));
    ...
    if (readHistogramHits(data, chPart, hits)) {
      ...
      if (hits.getLayerHits(chPart, layerHits)) {
        for (unsigned int i = 0; i < layerHits.size(); i++) {
          ...
          double r = detector.R_mm(
              chPart.side, chPart.station, chPart.ring, layerHit.part, layerHit.layer + 1, layerHit.hs, layerHit.wg);
      ...
      if (layerBitset > 0) {
        for (int layer = 0; layer < N_LAYERS; layer++) {
          if (layerBitset & (1 << layer)) {
            ...
            double pd =
                detector.LocalPhiRadStripToChamberCenter(chPart.side, chPart.station, chPart.ring, layer + 1, strip);
  ...
  while (chPart.next(detector)) {
    ...
    if (readHistogramHits(data, chPart, hits)) {
      ...
      if (hits.getLayerHits(chPart, layerHits)) {
        for (unsigned int i = 0; i < layerHits.size(); i++) {
          EmuLayerHit layerHit = layerHits[i];
          double cx = detector.X_mm(chPart.side,
                                    chPart.station,
                                    chPart.ring,
                                    layerHit.part,
                                    chPart.chamber,
                                    ...
                                    layerHit.hs,
                                    layerHit.wg);
          double cy = detector.Y_mm(chPart.side,
                                    chPart.station,
                                    chPart.ring,
                                    layerHit.part,
                                    chPart.chamber,
