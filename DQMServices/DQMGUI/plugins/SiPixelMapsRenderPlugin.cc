...
public:
  bool applies(const VisDQMObject& o, const VisDQMImgInfo&) override {
    if ((o.name.find("PixelPhase1/Phase1_MechanicalView") != std::string::npos ||
         ...
         o.name.find("PixelPhase1Timing/") != std::string::npos ||
         o.name.find("PixelPhase1/SiPixelQualityPCL") != std::string::npos) &&
  ...
  void postDraw(TCanvas* canvas, const VisDQMObject& o, const VisDQMImgInfo& /*ri*/) override {
    ...
    // Phase1, Layer 1-4
    if (w == 72 && h == 26)
      dress_occup_plot(obj, 1, 0, 1);
    if (w == 72 && h == 58)
      dress_occup_plot(obj, 2, 0, 1);
    if (w == 72 && h == 90)
      dress_occup_plot(obj, 3, 0, 1);
    if (w == 72 && h == 130)
      dress_occup_plot(obj, 4, 0, 1);

    // Phase1 FPIX Ring 1-2
    if (w == 56 && h == 92)
      dress_occup_plot(obj, 0, 1, 1);
    if (w == 56 && h == 140)
      dress_occup_plot(obj, 0, 2, 1);

    // Phase1 full FPIX
    if (w == 112 && h == 140)
      dress_occup_plot(obj, 0, 0, 1);

    // Phase0 BPIX  Layer 1-3
    if (w == 72 && h == 40)
      dress_occup_plot(obj, 1, 0, 0);
    if (w == 72 && h == 64)
      dress_occup_plot(obj, 2, 0, 0);
    if (w == 72 && h == 88)
      dress_occup_plot(obj, 3, 0, 0);
