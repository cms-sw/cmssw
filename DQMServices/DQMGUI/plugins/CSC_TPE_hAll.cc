...
void CSC_TPE_hAll::draw(TH3*& me) {
  ...
  sprintf(tmp_str,
          "L = %.2f%% (%.2f%%)",
          den_cnt1[2] != 0 ? 100.0 * num_cnt1[2] / den_cnt1[2] : 0,
          denb_cnt1[2] != 0 ? 100.0 * numb_cnt1[2] / denb_cnt1[2] : 0);
  pave_total[5]->AddText(tmp_str);
  sprintf(tmp_str,
          "C = %.2f%% (%.2f%%)",
          den_cnt1[1] != 0 ? 100.0 * num_cnt1[1] / den_cnt1[1] : 0,
          denb_cnt1[1] != 0 ? 100.0 * numb_cnt1[1] / denb_cnt1[1] : 0);
  pave_total[5]->AddText(tmp_str);
  sprintf(tmp_str,
          "A = %.2f%% (%.2f%%)",
          den_cnt1[0] != 0 ? 100.0 * num_cnt1[0] / den_cnt1[0] : 0,
          denb_cnt1[0] != 0 ? 100.0 * numb_cnt1[0] / denb_cnt1[0] : 0);
  pave_total[5]->AddText(tmp_str);

  pave_total[7]->Clear();
  sprintf(tmp_str,
          "L = %.2f%% (%.2f%%)",
          den_cnt2[2] != 0 ? 100.0 * num_cnt2[2] / den_cnt2[2] : 0,
          denb_cnt2[2] != 0 ? 100.0 * numb_cnt2[2] / denb_cnt2[2] : 0);
  pave_total[7]->AddText(tmp_str);
  sprintf(tmp_str,
          "C = %.2f%% (%.2f%%)",
          den_cnt2[1] != 0 ? 100.0 * num_cnt2[1] / den_cnt2[1] : 0,
          denb_cnt2[1] != 0 ? 100.0 * numb_cnt2[1] / denb_cnt2[1] : 0);
  pave_total[7]->AddText(tmp_str);
  sprintf(tmp_str,
          "A = %.2f%% (%.2f%%)",
          den_cnt2[0] != 0 ? 100.0 * num_cnt2[0] / den_cnt2[0] : 0,
          denb_cnt2[0] != 0 ? 100.0 * numb_cnt2[0] / denb_cnt2[0] : 0);
  pave_total[7]->AddText(tmp_str);
