...
private:
  ...
  void drawExclusionBox(double x1, double y1, double x2, double y2) {
    ...
    if (x1 < gPad->GetFrame()->GetX1())
      x1 = gPad->GetFrame()->GetX1();
    if (x2 > gPad->GetFrame()->GetX2())
      x2 = gPad->GetFrame()->GetX2();
    if (y1 < gPad->GetFrame()->GetY1())
      y1 = gPad->GetFrame()->GetY1();
    if (y2 > gPad->GetFrame()->GetY2())
      y2 = gPad->GetFrame()->GetY2();
    if (y1 < y2 && x1 < x2)
  ...
  void preDrawTH2F(TCanvas *c, const VisDQMObject &o, VisDQMRenderInfo &r) {
    ...
    if (name.find("Occ") != std::string::npos || name.find("calDiscrepancy") != std::string::npos ||
    ...
    if (name.find("ecal") != std::string::npos && checkAndRemove(drawOptions, "tlabels")) {
      ...
      for (int ieta = -28; ieta < 29; ++ieta) {
        ...
        for (int iphi = 1; iphi <= 72; ++iphi) {
          if (ieta < c->GetFrame()->GetX1() || ieta > c->GetFrame()->GetX2() || iphi < c->GetFrame()->GetY1() ||
              iphi > c->GetFrame()->GetY2())
            continue;
          sprintf(text, "% d,%d", ieta, iphi);
          tlabels_.DrawText(ieta, iphi, text);
        }
      }
    } else if ((name.find("hcal") != std::string::npos or
                o.name.find("L1TdeStage2CaloLayer1/Occupancies") != std::string::npos) &&
               checkAndRemove(drawOptions, "tlabels")) {
      char text[20];
      for (int ieta = -42; ieta < 42; ++ieta) {
        if (ieta == 0 || abs(ieta) == 29)
          continue;
        for (int iphi = 1; iphi <= 72; ++iphi) {
          if (abs(ieta) > 29 && iphi % 2 == 0)
            continue;
