...
(function (factory) {
  ...
  JSROOT.CheckArrayPrototype = function (arr, check_content) {
    ...
    for (var k = 0; k < arr.length; ++k) {
      ...
      if (sub == 'object' && JSROOT.CheckArrayPrototype(arr[k])) {
        plain = false;
        break;
      }
    }
  ...
  function ArrayIterator(arr, select, tgtobj) {
    ...
    if (typeof select === 'object') this.select = select;
    // remember indexes for selection
    else this.select = []; // empty array, undefined for each dimension means iterate over all indexes
  }
  ...
  ArrayIterator.prototype.next = function () {
    ...
    if (cnt >= 0) {
      ...
      while (--cnt >= 0) {
        if (
          this.select[cnt] === undefined &&
          ++this.indx[cnt] < this.arr[cnt].length
        )
          break;
      }
    ...
    while (true) {
      ...
      if (
        typ === 'array' &&
        (obj.length > 0 || this.select[cnt + 1] === '$size$')
      ) {
        this.arr[++cnt] = obj;
  ...
  TDrawVariable.prototype.Parse = function (
    tree,
    selector,
    code,
    only_branch,
    ...
    while (pos < code.length || only_branch) {
      ...
      if (only_branch) {
        ...
        while (
          pos2 < code.length &&
          (is_next_symbol(code[pos2]) || code[pos2] === '.')
        )
          pos2++;
        if (code[pos2] == '$') {
          var repl = '';
          switch (code.substr(pos, pos2 - pos)) {
            case 'LocalEntry':
            case 'Entry':
              repl = 'arg.$globals.entry';
              break;
            case 'Entries':
              repl = 'arg.$globals.entries';
              break;
          }
          if (repl) {
      ...
      while (pos2 < code.length) {
        ...
        if (code[pos2] === '.') {
          ...
          if (arriter.length === 0 && br) {
            ...
            if (
              br.fType === JSROOT.BranchType.kClonesNode ||
              br.fType === JSROOT.BranchType.kSTLNode
            ) {
              arriter.push(undefined);
            } else {
              var objclass = JSROOT.IO.GetBranchObjectClass(
                br,
                tree,
                false,
                true
              );
              if (objclass && JSROOT.IsRootCollection(null, objclass))
                arriter.push(undefined);
            }
          }
      ...
      if (arriter.length === 0) arriter = undefined;
      else if (arriter.length === 1 && arriter[0] === undefined) arriter = true;
  ...
  TDrawVariable.prototype.Produce = function (obj) {
    ...
    var arg = { $globals: this.globals, $math: JSROOT.Math },
      usearrlen = -1,
      arrs = [];
    for (var n = 0; n < this.branches.length; ++n) {
      ...
      if (this.brarray[n] === undefined)
        this.brarray[n] =
          JSROOT.CheckArrayPrototype(arg[name]) > 0 ||
          JSROOT.IsRootCollection(arg[name]);
      ...
      if (
        this.brarray[n] === true &&
        JSROOT.CheckArrayPrototype(arg[name], true) === 2
      ) {
  ...
  TDrawSelector.prototype.ParseParameters = function (tree, args, expr) {
    ...
    while (pos >= 0) {
      ...
      if (separ > 0) {
        parvalue = parname.substr(separ + 1);
        parname = parname.substr(0, separ);
      }
      ...
      switch (parname) {
        ...
        case 'entries':
        case 'numentries':
          if (parvalue === 'all') args.numentries = tree.fEntries;
          else if (parvalue === 'half')
            args.numentries = Math.round(tree.fEntries / 2);
          else if (intvalue !== undefined) args.numentries = intvalue;
          break;
        case 'first':
        ...
        case 'hbins':
          ...
          if (isNaN(this.hist_nbins) || this.hist_nbins <= 3)
  ...
  TDrawSelector.prototype.ParseDrawExpression = function (tree, args) {
      ...
      switch (expr[pos]) {
        ...
        case ':':
          ...
          if (!nbr1 && !nbr2 && pos > prev)
            names.push(expr.substr(prev, pos - prev));
          prev = pos + 1;
          break;
      }
    }
    ...
    if (cut) if (!this.cut.Parse(tree, this, cut)) return false;
    ...
    if (args.drawopt !== undefined) this.histo_drawopt = args.drawopt;
    else this.histo_drawopt = this.ndim === 2 ? 'col' : '';
  ...
  TDrawSelector.prototype.DrawOnlyBranch = function (tree, branch, expr, args) {
    ...
    if (this.dump_values) {
      ...
      this.copy_fields =
        ((args.branch.fLeaves && args.branch.fLeaves.arr.length > 1) ||
          (args.branch.fBranches && args.branch.fBranches.arr.length > 0)) &&
        !args.leaf;
  ...
  TDrawSelector.prototype.Begin = function (tree) {
    ...
    if (this.monitoring) this.lasttm = new Date().getTime();
  };
  ...
  TDrawSelector.prototype.ShowProgress = function (value) {
    ...
    var ndig = 0;
    if (this.aver_diff <= 0) ndig = 0;
    else if (this.aver_diff < 0.0001) ndig = 3;
    else if (this.aver_diff < 0.001) ndig = 2;
    else if (this.aver_diff < 0.01) ndig = 1;
  ...
  TDrawSelector.prototype.GetMinMaxBins = function (axisid, nbins) {
    ...
    if (this.vars[axisid].kind === 'string') {
      ...
      for (var k = 0; k < arr.length; ++k)
        if (res.lbls.indexOf(arr[k]) < 0) res.lbls.push(arr[k]);
      ...
      if (this.hist_nbins) nbins = res.nbins = this.hist_nbins;

      res.isinteger =
        Math.round(res.min) === res.min && Math.round(res.max) === res.max;
      if (res.isinteger)
        for (var k = 0; k < arr.length; ++k)
      ...
      if (res.isinteger) {
        ...
        if (res.max - res.min < nbins * 5) {
          ...
          var range = res.max - res.min + 2,
            step = Math.floor(range / nbins);
    ...
    res.GetBin = function (value) {
      var bin = this.lbls
        ? this.lbls.indexOf(value)
        : Math.floor((value - this.min) * this.k);
  ...
  TDrawSelector.prototype.CreateHistogram = function () {
    ...
    if (this.dump_values) {
      ...
      switch (this.ndim) {
        case 1:
          this.hist = JSROOT.CreateHistogram('TH1' + this.htype, this.x.nbins);
          break;
        case 2:
          this.hist = JSROOT.CreateHistogram(
            'TH2' + this.htype,
            this.x.nbins,
            this.y.nbins
          );
  ...
  TDrawSelector.prototype.FillTBitsHistogram = function (xvalue, weight) {
    ...
    for (var bit = 0, mask = 1, b = 0; bit < sz; ++bit) {
      if (xvalue.fAllBits[b] && mask) {
        if (bit <= this.x.nbins) this.hist.fArray[bit + 1] += weight;
        else this.hist.fArray[this.x.nbins + 1] += weight;
      }
  ...
  TDrawSelector.prototype.Fill3DHistogram = function (
    xvalue,
    yvalue,
    zvalue,
    weight
    ...
    this.hist.fArray[
      xbin + (this.x.nbins + 2) * (ybin + (this.y.nbins + 2) * zbin)
    ] += weight;
  ...
  TDrawSelector.prototype.DumpValue = function (v1, v2, v3, v4) {
    ...
    switch (this.ndim) {
      case 1:
        ...
        break;
      case 2:
        obj = { x: v1, y: v2, weight: v3 };
        break;
      case 3:
        obj = { x: v1, y: v2, z: v3, weight: v4 };
        break;
    }

    if (this.cut.is_dummy()) {
      if (this.ndim === 1) obj = v1;
  ...
  TDrawSelector.prototype.ProcessArraysFunc = function (entry) {
    ...
    if (this.arr_limit || this.graph) {
      ...
      var br0 = this.tgtarr.br0,
        len = br0.length;
      switch (this.ndim) {
        case 1:
          for (var k = 0; k < len; ++k) this.Fill1DHistogram(br0[k], 1);
          break;
        case 2:
          var br1 = this.tgtarr.br1;
          for (var k = 0; k < len; ++k) this.Fill2DHistogram(br0[k], br1[k], 1);
          break;
        case 3:
          var br1 = this.tgtarr.br1,
            br2 = this.tgtarr.br2;
  ...
  TDrawSelector.prototype.Process = function (entry) {
    ...
    for (var n = 0; n < this.ndim; ++n) this.vars[n].Produce(this.tgtobj);
  ...
  TDrawSelector.prototype.Terminate = function (res) {
    ...
    return JSROOT.CallBack(
      this.histo_callback,
      this.hist,
      this.dump_values ? 'inspect' : this.histo_drawopt
    );
  };
  ...
  JSROOT.IO.FindBrachStreamerElement = function (branch, file) {
    ...
    if (
      !branch ||
      !file ||
      branch._typename !== 'TBranchElement' ||
      branch.fID < 0 ||
      branch.fStreamerType < 0
    )
      return null;

    var s_i = file.FindStreamerInfo(
        branch.fClassName,
    ...
    function match_elem(elem) {
      ...
      if (
        elem.fType === JSROOT.IO.kBool &&
        branch.fStreamerType === JSROOT.IO.kUChar
      )
        return true;
      if (
        (branch.fStreamerType === JSROOT.IO.kSTL ||
          branch.fStreamerType === JSROOT.IO.kSTL + JSROOT.IO.kOffsetL ||
          branch.fStreamerType === JSROOT.IO.kSTLp ||
          branch.fStreamerType === JSROOT.IO.kSTLp + JSROOT.IO.kOffsetL) &&
  ...
  JSROOT.IO.DefineMemberTypeName = function (file, parent_class, member_name) {
    ...
    for (var k = 0; k < arr.length; ++k) {
      if (arr[k].fTypeName === 'BASE') {
        var res = JSROOT.IO.DefineMemberTypeName(
          file,
          arr[k].fName,
          member_name
        );
  ...
  JSROOT.IO.GetBranchObjectClass = function (
    branch,
    tree,
    with_clones,
    with_leafs
    ...
    if (
      branch.fType === JSROOT.BranchType.kLeafNode &&
      branch.fID === -2 &&
      branch.fStreamerType === -1
    ) {
    ...
    if (
      with_clones &&
      branch.fClonesName &&
      (branch.fType === JSROOT.BranchType.kClonesNode ||
        branch.fType === JSROOT.BranchType.kSTLNode)
    )
    ...
    if (
      branch.fType === JSROOT.BranchType.kBaseClassNode &&
      s_elem &&
      s_elem.fTypeName === 'BASE'
    )
      return s_elem.fName;
    ...
    if (branch.fType === JSROOT.BranchType.kObjectNode) {
      if (
        s_elem &&
        (s_elem.fType === JSROOT.IO.kObject || s_elem.fType === JSROOT.IO.kAny)
      )
    ...
    if (branch.fType === JSROOT.BranchType.kLeafNode && s_elem && with_leafs) {
      if (s_elem.fType === JSROOT.IO.kObject || s_elem.fType === JSROOT.IO.kAny)
        return s_elem.fTypeName;
      if (s_elem.fType === JSROOT.IO.kObjectp)
  ...
  JSROOT.TreeMethods.Process = function (selector, args) {
    ...
    function CreateLeafElem(leaf, name) {
      ...
      switch (leaf._typename) {
        case 'TLeafF':
          datakind = JSROOT.IO.kFloat;
          break;
        case 'TLeafD':
          datakind = JSROOT.IO.kDouble;
          break;
        case 'TLeafO':
          datakind = JSROOT.IO.kBool;
          break;
        case 'TLeafB':
          datakind = leaf.fIsUnsigned ? JSROOT.IO.kUChar : JSROOT.IO.kChar;
          break;
        case 'TLeafS':
          datakind = leaf.fIsUnsigned ? JSROOT.IO.kUShort : JSROOT.IO.kShort;
          break;
        case 'TLeafI':
          datakind = leaf.fIsUnsigned ? JSROOT.IO.kUInt : JSROOT.IO.kInt;
          break;
        case 'TLeafL':
          datakind = leaf.fIsUnsigned ? JSROOT.IO.kULong64 : JSROOT.IO.kLong64;
          break;
        case 'TLeafC':
          datakind = JSROOT.IO.kTString;
          break; // datakind = leaf.fIsUnsigned ? JSROOT.IO.kUChar : JSROOT.IO.kChar; break;
        default:
          return null;
      }
    ...
    function AddBranchForReading(
      ...
      if (typeof branch === 'string') branch = handle.tree.FindBranch(branch);
      ...
      item = {
        ...
        GetBasketEntry: function (k) {
          ...
          var res =
            k < this.branch.fMaxBaskets ? this.branch.fBasketEntry[k] : 0;
          if (res) return res;
        ...
        GetEntry: function (entry) {
          ...
          var shift = entry - this.first_entry,
            off;
      ...
      if (branch.fBranchCount) {
        ...
        if (
          !BranchCount2 &&
          branch.fBranchCount.fStreamerType === JSROOT.IO.kSTL &&
          (branch.fStreamerType === JSROOT.IO.kStreamLoop ||
            branch.fStreamerType === JSROOT.IO.kOffsetL + JSROOT.IO.kStreamLoop)
        ) {
          ...
          if (elemd && elemd.fCountName && arrd)
            for (var k = 0; k < arrd.length; ++k)
              if (
                arrd[k].fName ===
                branch.fBranchCount.fName + '.' + elemd.fCountName
              ) {
                BranchCount2 = arrd[k];
                break;
              }
      ...
      function ScanBranches(lst, master_target, chld_kind) {
        ...
        for (var k = 0; k < lst.arr.length; ++k) {
          ...
          if (elem && elem.fTypeName === 'BASE') {
            ...
            if (
              br.fTotBytes &&
              !AddBranchForReading(br, target_object, target_name, read_mode)
            )
              return false;
            if (!ScanBranches(br.fBranches, master_target, chld_kind))
              return false;
          ...
          if (chld_kind > 0) {
            ...
            if (pp > 0)
              chld_direct =
                JSROOT.IO.DetectBranchMemberClass(
                  lst,
                  branch.fName + '.' + subname.substr(0, pp + 1),
                  k
                ) || 'TObject';
          }
      ...
      if (branch._typename === 'TBranchObject') {
        ...
        if (
          !read_mode ||
          (typeof read_mode === 'string' && read_mode[0] === '.') ||
          read_mode === 1
        ) {
          ...
          if (typeof read_mode === 'string' && read_mode[0] === '.') {
            member.conttype = JSROOT.IO.DetectBranchMemberClass(
              branch.fBranches,
              branch.fName + read_mode
            );
            if (!member.conttype) {
              console.error(
          ...
          child_scan =
            branch.fType === JSROOT.BranchType.kClonesNode
              ? JSROOT.BranchType.kClonesMemberNode
              : JSROOT.BranchType.kSTLMemberNode;
        }
      } else if (
        (object_class = JSROOT.IO.GetBranchObjectClass(branch, handle.tree))
      ) {
        if (read_mode === true) {
          console.warn(
            'Object branch ' +
        ...
        for (var l = 0; l < newtgt.length - 1; ++l)
          newtgt[l] = target_object[l];
        newtgt[newtgt.length - 1] = {
          name: target_name,
          lst: JSROOT.IO.MakeMethodsList(object_class),
        };
        ...
        is_brelem &&
        nb_leaves === 1 &&
        leaf.fName === branch.fName &&
        branch.fID == -1
      ) {
        elem = JSROOT.IO.CreateStreamerElement(target_name, branch.fClassName);

        if (elem.fType === JSROOT.IO.kAny) {
          var streamer = handle.file.GetStreamer(branch.fClassName, {
            val: branch.fClassVersion,
            checksum: branch.fCheckSum,
          });
          if (!streamer) {
            elem = null;
        ...
        if (!elem && branch.fStreamerType && branch.fStreamerType < 20)
          elem = JSROOT.IO.CreateStreamerElement(
            target_name,
            branch.fStreamerType
          );
      } else if (nb_leaves === 1) {
        ...
        if (isok)
          member = {
            ...
            func: function (buf, obj) {
              ...
              while (l < this.leaves.length) this.leaves[l++].func(buf, tgt);
            },
          };
      }
      ...
      if (item_cnt && typeof read_mode === 'string') {
        ...
        if (snames.length === 1) {
          ...
          if (!branch.fParentName) {
            console.error(
              'Not possible to provide more than 2 parts in the target name',
              target_name
            );
            return null;
          }
          ...
          for (var k = 0; k < snames.length; ++k) {
            var chld_class = JSROOT.IO.DefineMemberTypeName(
              handle.file,
              parent_class,
              snames[k]
            );
            member.smethods[k] = JSROOT.IO.MakeMethodsList(
              chld_class || 'AbstractClass'
            );
        ...
        if (member.objs_branch_func) {
          ...
          member.func = function (buf, obj) {
            var arr = obj[this.name0],
              n = 0; // objects array where reading is done
            while (n < arr.length) this.func0(buf, this.get(arr, n++)); // read all individual object with standard functions
          };
        }
      } else if (item_cnt) {
        handle.process_arrays = false;
        ...
        if (
          elem.fType === JSROOT.IO.kDouble32 ||
          elem.fType === JSROOT.IO.kFloat16
        ) {
          ...
          (elem.fType === JSROOT.IO.kOffsetP + JSROOT.IO.kDouble32 ||
            elem.fType === JSROOT.IO.kOffsetP + JSROOT.IO.kFloat16) &&
          branch.fBranchCount2
        ) {
          ...
          member.func = function (buf, obj) {
            var sz0 = obj[this.stl_size],
              sz1 = obj[this.arr_size],
              arr = new Array(sz0);
            for (var n = 0; n < sz0; ++n)
        ...
        // special handling of simple arrays
          ...
          (elem.fType > 0 && elem.fType < JSROOT.IO.kOffsetL) ||
          elem.fType === JSROOT.IO.kTString ||
          (elem.fType > JSROOT.IO.kOffsetP &&
            elem.fType < JSROOT.IO.kOffsetP + JSROOT.IO.kOffsetL &&
            branch.fBranchCount2)
        ) {
          member = {
            name: target_name,
          ...
          if (branch.fBranchCount2) {
            ...
            member.func = function (buf, obj) {
              ...
              for (var n = 0; n < sz0; ++n)
                arr[n] =
                  buf.ntou1() === 1 ? buf.ReadFastArray(sz1[n], this.type) : [];
              obj[this.name] = arr;
            };
          }
        } else if (
        ...
        } else if (elem.fType == JSROOT.IO.kStreamer) {
          ...
          elem.fType === JSROOT.IO.kStreamLoop ||
          elem.fType === JSROOT.IO.kOffsetL + JSROOT.IO.kStreamLoop
        ) {
          if (item_cnt2) {
      ...
      if (child_scan)
        if (!ScanBranches(branch.fBranches, target_object, child_scan))
          return null;
    ...
    for (var nn = 0; nn < selector.branches.length; ++nn) {
      var item = AddBranchForReading(
        selector.branches[nn],
        undefined,
        selector.names[nn],
        selector.directs[nn]
      );
    ...
    for (var h = 1; h < handle.arr.length && handle.simple_read; ++h) {
      ...
      if (
        ...
        item.numbaskets !== item0.numbaskets
      )
    ...
    for (var nn = 0; nn < handle.arr.length; ++nn) {
      var branch = handle.arr[nn].branch,
        e1 = branch.fFirstEntry;
      if (e1 === undefined)
        e1 = branch.fBasketBytes[0] ? branch.fBasketEntry[0] : 0;
      handle.firstentry = Math.max(handle.firstentry, e1);
      handle.lastentry =
        nn === 0
          ? e1 + branch.fEntries
          : Math.min(handle.lastentry, e1 + branch.fEntries);
    }
    ...
    if (
      !isNaN(args.firstentry) &&
      args.firstentry > handle.firstentry &&
      args.firstentry < handle.lastentry
    )
    ...
    if (typeof selector.ProcessArrays === 'function' && handle.simple_read) {
      ...
      for (var k = 0; k < handle.arr.length; ++k) {
        ...
        if (
          elem.type <= 0 ||
          elem.type >= JSROOT.IO.kOffsetL ||
          elem.type === JSROOT.IO.kCharStar
        )
          handle.process_arrays = false;
      }
    ...
    function ReadBaskets(bitems, baskets_call_back) {
      ...
      function ExtractPlaces() {
        ...
        for (var n = 0; n < bitems.length; ++n) {
          ...
          if (places.length === 0) filename = branch.fFileName;
          else if (filename !== branch.fFileName) continue;

          bitems[n].selected = true; // mark which item was selected for reading

          places.push(
            branch.fBasketSeek[bitems[n].basket],
            branch.fBasketBytes[bitems[n].basket]
          );
        }
      ...
      function ReadProgress(value) {
        ...
        var portion =
          (handle.staged_prev +
            value * (handle.staged_now - handle.staged_prev)) /
          (handle.process_max - handle.process_min);
      ...
      function ProcessBlobs(blobs) {
        ...
        for (var k = 0; k < bitems.length; ++k) {
          ...
          if (
            basket.fNbytes !== bitems[k].branch.fBasketBytes[bitems[k].basket]
          )
            console.error(
              'mismatch in read basket sizes',
              bitems[k].branch.fBasketBytes[bitems[k].basket]
            );
        ...
        else JSROOT.CallBack(baskets_call_back, bitems);
      }

      // extract places where to read
      if (ExtractPlaces())
        handle.file.ReadBuffer(places, ProcessBlobs, filename, ReadProgress);
      else JSROOT.CallBack(baskets_call_back, null);
    }
    ...
    function ReadNextBaskets() {
        ...
        for (var n = handle.arr.length - 1; n >= 0; --n) {
          ...
          while (elem.staged_basket < elem.numbaskets) {
            ...
            if (bskt) {
              ...
              if (bitem.raw) bitem.raw.locate(0);
              // reset pointer - same branch may be read several times
              else bitem.raw = JSROOT.CreateTBuffer(null, 0, handle.file); // create dummy buffer - basket has no data
      ...
      if (totalsz === 0 && !is_direct) return handle.selector.Terminate(true);
      ...
      if (handle.process_max > handle.process_min)
        portion =
          (handle.staged_prev - handle.process_min) /
          (handle.process_max - handle.process_min);
    ...
    function ProcessBaskets(bitems) {
      ...
      while (true) {
        var loopentries = 100000000,
          min_curr = handle.process_max,
          n,
          elem;
        ...
        for (n = 0; n < handle.arr.length; ++n) {
          ...
          if (
            !elem.raw ||
            !elem.basket ||
            elem.first_entry + elem.basket.fNevBuf <= handle.current_entry
          ) {
            delete elem.raw;
            delete elem.basket;
        ...
        // main processing loop
          ...
          while (loopentries--) {
            for (n = 0; n < handle.arr.length; ++n) {
              ...
              elem.member.func(
                elem.raw,
                elem.GetTarget(handle.selector.tgtobj)
              );
            }
  ...
  JSROOT.TreeMethods.Draw = function (args, result_callback) {
    ...
    if (args.expr === 'testio') return this.IOTest(args, result_callback);

    var selector = new TDrawSelector(result_callback);

    if (args.branch) {
      if (!selector.DrawOnlyBranch(this, args.branch, args.expr, args))
        selector = null;
    } else {
      if (!selector.ParseDrawExpression(this, args)) selector = null;
    }

    if (!selector) return JSROOT.CallBack(result_callback, null);
  ...
  JSROOT.TreeMethods.IOTest = function (args, result_callback) {
    ...
    function TestNextBranch() {
      ...
      selector.Process = function () {
        if (this.tgtobj.br0 === undefined) this.fail = true;
      };

      selector.Terminate = function (res) {
        if (typeof res !== 'string') res = !res || this.fails ? 'FAIL' : 'ok';
        ...
        if (now - args.lasttm > 5000 || args.nbr - args.lastnbr > 50)
          setTimeout(tree.IOTest.bind(tree, args, result_callback), 100);
        // use timeout to avoid deep recursion
        else TestNextBranch();
      };
      ...
      if (skip_branch || num <= 0) {
        ...
        if (num < drawargs.numentries) {
          ...
          drawargs.firstentry =
            first +
            Math.round((last - first - drawargs.numentries) * Math.random());
        }
