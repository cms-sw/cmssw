(function (factory) {
  if (typeof define === 'function' && define.amd) {
    ...
    if (jsroot.nodejs)
      jsroot.Painter.readStyleFromURL(
        '?interactive=0&tooltip=0&nomenu&noprogress&notouch&toolbar=0&webgl=0'
      );
  } else {
  ...
  if (!JSROOT._test_d3_) {
    if (typeof d3 == 'object' && d3.version && d3.version[0] === '5') {
      ...
      if (d3.version !== '4.4.4')
        console.warn(
          'Try to use older d3.js ' + d3.version + ', expected 5.7.0'
        );
      JSROOT._test_d3_ = 4;
  ...
  JSROOT.ToolbarIcons = {
    ...
    circle: {
      path: 'M256,256 m-150,0 a150,150 0 1,0 300,0 a150,150 0 1,0 -300,0',
    },
    three_circles: {
      path:
  ...
  Painter.readStyleFromURL = function (url) {
    ...
    if (optimize == '') JSROOT.gStyle.OptimizeDraw = 2;
    ...
    if (inter === 'nomenu') JSROOT.gStyle.ContextMenu = false;
      ...
      if (!inter || inter == '1') inter = '111111';
      else if (inter == '0') inter = '000000';
      if (inter.length === 6) {
        if (inter[0] == '0') JSROOT.gStyle.ToolBar = false;
        else if (inter[0] == '1') JSROOT.gStyle.ToolBar = 'popup';
        else if (inter[0] == '2') JSROOT.gStyle.ToolBar = true;
    ...
    if (JSROOT.GetUrlOption('nomenu', url) !== null)
      JSROOT.gStyle.ContextMenu = false;
    if (JSROOT.GetUrlOption('noprogress', url) !== null)
      JSROOT.gStyle.ProgressBox = false;
    if (JSROOT.GetUrlOption('notouch', url) !== null) JSROOT.touches = false;
    if (JSROOT.GetUrlOption('adjframe', url) !== null)
      JSROOT.gStyle.CanAdjustFrame = true;
    ...
    JSROOT.gStyle.fStatFormat = JSROOT.GetUrlOption(
      'statfmt',
      url,
      JSROOT.gStyle.fStatFormat
    );
    JSROOT.gStyle.fFitFormat = JSROOT.GetUrlOption(
      'fitfmt',
      url,
      JSROOT.gStyle.fFitFormat
    );
    ...
    if (toolbar !== null) {
      ...
      if (toolbar.indexOf('left') >= 0) {
        JSROOT.gStyle.ToolBarSide = 'left';
        val = 'popup';
      }
      ...
      if (toolbar.indexOf('vert') >= 0) {
        JSROOT.gStyle.ToolBarVert = true;
        val = 'popup';
      }
      if (toolbar.indexOf('show') >= 0) val = true;
      JSROOT.gStyle.ToolBar =
        val ||
        (toolbar.indexOf('0') < 0 &&
          toolbar.indexOf('false') < 0 &&
          toolbar.indexOf('off') < 0);
    }

    var palette = JSROOT.GetUrlOption('palette', url);
    if (palette !== null) {
      palette = parseInt(palette);
      if (!isNaN(palette) && palette > 0 && palette < 113)
        JSROOT.gStyle.Palette = palette;
    }
    ...
    if (webgl === '0' || webgl === 'false') JSROOT.gStyle.NoWebGL = true;
    else if (webgl === 'ie') JSROOT.gStyle.NoWebGL = !JSROOT.browser.isIE;

    var geosegm = JSROOT.GetUrlOption('geosegm', url);
    if (geosegm !== null)
      JSROOT.gStyle.GeoGradPerSegm = Math.max(2, parseInt(geosegm));
    var geocomp = JSROOT.GetUrlOption('geocomp', url);
    if (geocomp !== null)
      JSROOT.gStyle.GeoCompressComp = geocomp !== '0' && geocomp !== 'false';
  };
  ...
  Painter.createRootColors = function () {
    ...
    for (var indx = 0; indx < moreCol.length; ++indx) {
      ...
      for (var n = 0; n < entry.str.length; n += 6) {
        ...
        colorMap[num] =
          'rgb(' +
          parseInt('0x' + entry.str.slice(n, n + 2)) +
          ',' +
          parseInt('0x' + entry.str.slice(n + 2, n + 4)) +
          ',' +
          parseInt('0x' + entry.str.slice(n + 4, n + 6)) +
          ')';
      }
    }
  ...
  Painter.MakeColorRGB = function (col) {
    ...
    var rgb =
      Math.round(col.fRed * 255) +
      ',' +
      Math.round(col.fGreen * 255) +
      ',' +
      Math.round(col.fBlue * 255);
    if (col.fAlpha === undefined || col.fAlpha == 1) rgb = 'rgb(' + rgb + ')';
    else rgb = 'rgba(' + rgb + ',' + col.fAlpha.toFixed(3) + ')';
  ...
  Painter.extendRootColors = function (jsarr, objarr) {
    ...
    for (var n = 0; n < rgb_array.length; ++n)
      if (rgb_array[n] && jsarr[n] != rgb_array[n]) jsarr[n] = rgb_array[n];
  ...
  TAttMarkerHandler.prototype.SetArgs = function (args) {
    if (typeof args == 'object' && typeof args.fMarkerStyle == 'number')
      args = { attr: args };

    if (args.attr) {
      if (args.color === undefined)
        args.color = Painter.root_colors[args.attr.fMarkerColor];
  ...
  TAttMarkerHandler.prototype.create = function (x, y) {
    if (!this.optimized)
      return (
        'M' +
        (x + this.x0).toFixed(this.ndig) +
        ',' +
        (y + this.y0).toFixed(this.ndig) +
        this.marker
      );
    ...
    var xx = Math.round(x),
      ...
      m2 =
        this.lastx === null
          ? m1
          : 'm' + (xx - this.lastx) + ',' + (yy - this.lasty) + 'h1';
    this.lastx = xx + 1;
    this.lasty = yy;
  ...
  TAttMarkerHandler.prototype.Change = function (color, style, size) {
    ...
    var half = (size / 2).toFixed(this.ndig),
      full = size.toFixed(this.ndig);
    ...
    switch (shape) {
      ...
      case 6: // star
        ...
        this.marker =
          'l' +
          (size / 3).toFixed(this.ndig) +
          ',' +
          full +
          'l-' +
          ((5 / 6) * size).toFixed(this.ndig) +
          ',-' +
      ...
      case 9: // mult
        this.x0 = this.y0 = -size / 2;
        this.marker =
          'l' + full + ',' + full + 'm0,-' + full + 'l-' + full + ',' + full;
        break;
      default:
        // diamand
        this.x0 = -size / 2;
  ...
  TAttLineHandler.prototype.SetArgs = function (args) {
    ...
    if (!this.color && this.width > 0) this.color = 'lightgrey';
  };
  ...
  TAttLineHandler.prototype.Apply = function (selection) {
    ...
    if (this.empty())
      ...
      selection
        ...
        .style(
          'stroke-dasharray',
          Painter.root_line_styles[this.style] || null
        );
  };
  ...
  TAttFillHandler.prototype.SetArgs = function (args) {
    if (args.attr && typeof args.attr == 'object') {
      if (args.pattern === undefined && args.attr.fFillStyle !== undefined)
        args.pattern = args.attr.fFillStyle;
      if (args.color === undefined && args.attr.fFillColor !== undefined)
  ...
  TAttFillHandler.prototype.Apply = function (selection) {
    ...
    if ('opacity' in this) selection.style('opacity', this.opacity);

    if ('antialias' in this) selection.style('antialias', this.antialias);
  };
  ...
  TAttFillHandler.prototype.verifyDirectChange = function (painter) {
    ...
    this.Change(
      this.color,
      this.pattern,
      painter ? painter.svg_canvas() : null,
      true
    );
  };
   ...
   */
  TAttFillHandler.prototype.Change = function (
    color,
    pattern,
    svg,
    color_as_svg
    ...
    switch (this.pattern) {
      ...
      case 3011:
        ...
        h = 18;
        lines =
          'M5,0v8M2,1l6,6M8,1l-6,6M9,9v8M6,10l3,3l-3,3M0,9v8M3,10l-3,3l3,3';
        lfill = 'none';
        break;
      case 3012:
        w = 10;
        h = 20;
        lines =
          'M5,1A4,4,0,0,0,5,9A4,4,0,0,0,5,1M0,11A4,4,0,0,1,0,19M10,11A4,4,0,0,0,10,19';
        lfill = 'none';
        break;
      case 3013:
        w = h = 7;
      ...
      case 3015:
        ...
        h = 12;
        lines =
          'M2,1A2,2,0,0,0,2,5A2,2,0,0,0,2,1M0,7A2,2,0,0,1,0,11M6,7A2,2,0,0,0,6,11';
        lfill = 'none';
        break;
      case 3016:
        w = 12;
   ...
   * @private */
  TAttFillHandler.prototype.CreateSample = function (
    sample_svg,
    width,
    height
  ) {
  ...
  Painter.getFontDetails = function (fontIndex, size) {
    ...
    while (fontName.length > 0) {
      if (fontName[0] === 'b') res.weight = 'bold';
      else if (fontName[0] === 'i') res.style = 'italic';
      else if (fontName[0] === 'o') res.style = 'oblique';
      else break;
      fontName = fontName.substr(1);
    }

    if (fontName == 'Symbol') res.weight = res.style = null;
    ...
    res.setFont = function (selection, arg) {
      ...
      if (arg != 'without-size')
        selection.attr('font-size', this.size).attr('xml:space', 'preserve');
      if (this.weight) selection.attr('font-weight', this.weight);
      if (this.style) selection.attr('font-style', this.style);
    };
  ...
  Painter.chooseTimeFormat = function (awidth, ticks) {
    ...
    awidth /= 60;
    if (awidth < 30) return ticks ? '%Hh%M' : '%d/%m %H:%M';
    awidth /= 60;
    if (awidth < 12) return ticks ? '%d-%Hh' : '%d/%m/%y %Hh';
    awidth /= 24;
    if (awidth < 15.218425) return ticks ? '%d/%m' : '%d/%m/%y';
    awidth /= 30.43685;
    if (awidth < 6) return '%d/%m/%y';
    awidth /= 12;
    if (awidth < 2) return ticks ? '%m/%y' : '%d/%m/%y';
    return '%Y';
  };
  ...
  Painter.getTimeOffset = function (axis) {
    ...
    if (sof.indexOf('GMT') == 0) {
      ...
      if (sof.length > 3) {
        ...
        offset -=
          sign *
          (parseInt(sof.substr(p, 2)) * 3600 +
            parseInt(sof.substr(p + 2, 2)) * 60);
      }
    }
  ...
  Painter.translateLaTeX = function (str) {
    ...
    if (!Painter.symbolsRegexCache) {
      ...
      Painter.symbolsRegexCache = new RegExp(
        '(' +
          Object.keys(Painter.symbols_map)
            .join('|')
            .replace(/\{/g, '{')
            .replace(/\\}/g, '\\}') +
          ')',
  ...
  Painter.BuildSvgPath = function (kind, bins, height, ndig) {
    ...
    function jsroot_d3_svg_lineFiniteDifferences(points) {
      var i = 0,
        ...
        p0 = points[0],
        p1 = points[1],
        d = (m[0] = jsroot_d3_svg_lineSlope(p0, p1));
      while (++i < j) {
        m[i] =
          (d + (d = jsroot_d3_svg_lineSlope((p0 = p1), (p1 = points[i + 1])))) /
          2;
      }
      m[i] = d;
      return m;
    ...
    function jsroot_d3_svg_lineMonotoneTangents(points) {
      var d,
        ...
        b,
        s,
        m = jsroot_d3_svg_lineFiniteDifferences(points),
        i = -1,
        j = points.length - 1;
      while (++i < j) {
      ...
      while (++i <= j) {
        s =
          (points[Math.min(j, i + 1)].grx - points[Math.max(0, i - 1)].grx) /
          (6 * (1 + m[i] * m[i]));
        points[i].dgrx = s || 0;
    ...
    var res = { path: '', close: '' },
      bin = bins[0],
      prev,
      maxy = Math.max(bin.gry, height + 5),
      currx = Math.round(bin.grx),
      curry = Math.round(bin.gry),
      dx,
      dy,
      npnts = bins.length;
    ...
    res.path =
      (kind[0] == 'L' ? 'L' : 'M') + conv(bin.grx) + ',' + conv(bin.gry);
    ...
    if (smooth) {
      ...
      for (var n = 1; n < npnts; ++n) {
        ...
        if (n > 1) res.path += 's';
        res.path +=
          conv(bin.grx - bin.dgrx - prev.grx) +
          ',' +
  ...
  LongPollSocket.prototype.nextrequest = function (data, kind) {
    ...
    var req = JSROOT.NewHttpRequest(url, reqmode, function (res) {
      ...
      if (this.handle.req === this) this.handle.req = null; // get response for existing dummy request

      if (res === null) return this.handle.processreq(null);
      ...
      if (this.handle.raw) {
        ...
        if (offset < u8Arr.length) this.handle.processreq(res, offset);
      } else if (
        this.getResponseHeader('Content-Type') == 'application/x-binary'
      ) {
  ...
  FileDumpSocket.prototype.get_file = function (fname, res) {
    ...
    if (!res) return;
    if (this.receiver.ProvideData) this.receiver.ProvideData(res, 0);
    setTimeout(this.next_operation.bind(this), 10);
  };
  ...
  WebWindowHandle.prototype.GetUserArgs = function (field) {
    if (field && typeof field == 'string') {
      return this.user_args && typeof this.user_args == 'object'
        ? this.user_args[field]
        : undefined;
    }
  ...
  WebWindowHandle.prototype.DoneItem = function (item, _msg, _len) {
    ...
    if (this.msgqueue.length == 0) delete this.msgqueue;
  ...
  WebWindowHandle.prototype.Connect = function (href) {
    ...
    function retry_open(first_time) {
      ...
      if (!first_time)
        console.log('try connect window again' + new Date().getTime());
      ...
      if (pthis.kind == 'file') {
        ...
        path =
          path.replace('http://', 'ws://').replace('https://', 'wss://') +
          'root.websocket';
        if (args) path += '?' + args;
      ...
      conn.onmessage = function (e) {
        ...
        if (pthis.ackn > 7) pthis.Send('READY', 0); // send dummy message to server
      };
      ...
      if (!JSROOT.BatchMode) setTimeout(retry_open, 3000); // after 3 seconds try again
    } // retry_open
  ...
  JSROOT.ConnectWebWindow = function (arg) {
    ...
    else if (!arg || typeof arg != 'object') arg = {};
    ...
    if (
      JSROOT.GetUrlOption('batch_mode') !== null &&
      JSROOT.GetUrlOption('key') &&
      (JSROOT.browser.isChromeHeadless || JSROOT.browser.isChrome)
    )
      JSROOT.loadScript(
        'root_batch_holder.js?key=' + JSROOT.GetUrlOption('key')
      );

    if (!arg.platform) arg.platform = JSROOT.GetUrlOption('platform');

    if (arg.platform == 'qt5') JSROOT.browser.qt5 = true;
    else if (arg.platform == 'cef3') JSROOT.browser.cef3 = true;
    ...
    if (!arg.socket_kind) arg.socket_kind = JSROOT.GetUrlOption('ws');

    if (!arg.socket_kind) {
      if (JSROOT.browser.qt5) arg.socket_kind = 'rawlongpoll';
      else if (JSROOT.browser.cef3) arg.socket_kind = 'longpoll';
    ...
    if (arg.first_recv) {
      arg.receiver = {
        ...
        OnWebsocketMsg: function (handle, msg) {
          ...
          if (msg.indexOf(arg.first_recv) != 0) return handle.Close();
    ...
    if (!arg.receiver) return JSROOT.CallBack(arg.callback, handle, arg);
    ...
    if (arg.prereq2) {
      JSROOT.AssertPrerequisites(arg.prereq2, function () {
        ...
        if (!arg.first_recv || arg.first_msg)
  ...
  TBasePainter.prototype.AccessTopPainter = function (on) {
    ...
    if (!chld) return null;
    if (on === true) chld.painter = this;
    else if (on === false) delete chld.painter;
    return chld.painter;
  };
  ...
  TBasePainter.prototype.Cleanup = function (keep_origin) {
    ...
    if (this._changed_layout) this.set_layout_kind('simple');
    this.AccessTopPainter(false);
    this.divid = null;
    delete this._selected_main;

    if (this._hpainter && typeof this._hpainter.ClearPainter === 'function')
      this._hpainter.ClearPainter(this);
  ...
  TBasePainter.prototype.DrawingReady = function (res_painter) {
    ...
    if (this._ready_callback_ !== undefined) {
      ...
      while (callbacks.length) JSROOT.CallBack(callbacks.shift(), res_painter);
    }
    return this;
  };
  ...
  TBasePainter.prototype.set_layout_kind = function (kind, main_selector) {
    ...
    if (!origin.empty()) {
      ...
      origin.property(
        'layout_selector',
        kind != 'simple' && main_selector ? main_selector : null
      );
   ...
   * @private
   */
  TBasePainter.prototype.check_main_resize = function (
    check_level,
    new_size,
    height_factor
  ) {
    ...
    if (can_resize == 'height')
      if (
        height_factor &&
        Math.abs(rect_origin.width * height_factor - rect_origin.height) >
          0.1 * rect_origin.width
      )
        do_resize = true;

    if (
      (rect_origin.height <= lmt || rect_origin.width <= lmt) &&
      can_resize &&
      can_resize !== 'false'
    )
      do_resize = true;
    ...
    if (do_resize && enlarge !== 'on') {
      ...
      if (rect_origin.width > lmt) {
        ...
        main_origin.style(
          'height',
          Math.round(rect_origin.width * height_factor) + 'px'
        );
    ...
    if (old_h && old_w && old_h > 0 && old_w > 0) {
      if (old_h !== rect.height || old_w !== rect.width)
        if (
          check_level > 1 ||
          rect.width / old_w < 0.66 ||
          rect.width / old_w > 1.5 ||
          (rect.height / old_h < 0.66 && rect.height / old_h > 1.5)
        )
          rect.changed = true;
    } else {
      rect.changed = true;
  ...
  TBasePainter.prototype.enlarge_main = function (action, skip_warning) {
    ...
    if (
      main.empty() ||
      !JSROOT.gStyle.CanEnlarge ||
      origin.property('can_enlarge') === false
    )
      return false;
  ...
  TBasePainter.prototype.get_visible_rect = function (elem, fullsize) {
    ...
    if (!fullsize) {
      ...
      res.width -=
        this.GetStyleValue(elem, 'padding-left') +
        this.GetStyleValue(elem, 'padding-right');
      res.height -=
        this.GetStyleValue(elem, 'padding-top') -
        this.GetStyleValue(elem, 'padding-bottom');
    }
  ...
  TObjectPainter.prototype.OptionsAsString = function () {
    ...
    if (!this.OptionesChanged()) return this.options.original || '';
  ...
  TObjectPainter.prototype.get_color = function (indx) {
    ...
    if (!jsarr) {
      ...
      jsarr = this.root_colors =
        pp && pp.root_colors ? pp.root_colors : JSROOT.Painter.root_colors;
    }
   ...
   * @abstract
   * @private */
  TObjectPainter.prototype.AttributeChange = function (
    class_name,
    member_name,
    new_value
  ) {
  ...
  TObjectPainter.prototype.CreateG = function (frame_layer) {
    if (this.draw_g) {
      ...
      var up = [],
        chlds = layer.node().childNodes;
      for (var n = 0; n < chlds.length; ++n)
        if (d3.select(chlds[n]).classed('most_upper_primitives'))
          up.push(chlds[n]);

      up.forEach(function (top) {
        d3.select(top).raise();
      });
    }
    ...
    if (this.draw_object) {
      ...
      this.draw_g.attr(
        'objtype',
        encodeURI(this.draw_object._typename || 'type')
      );
    }
  ...
  TObjectPainter.prototype.AxisToSvg = function (axis, value, ndc, noround) {
    ...
    if (use_frame && main && main['gr' + axis]) {
      value =
        axis == 'y'
          ? main.gry(value) + (use_frame ? 0 : main.frame_y())
      ...
      if (pad) {
        if (axis == 'y') {
          if (pad.fLogy) value = value > 0 ? JSROOT.log10(value) : pad.fUymin;
          value = (value - pad.fY1) / (pad.fY2 - pad.fY1);
        } else {
          if (pad.fLogx) value = value > 0 ? JSROOT.log10(value) : pad.fUxmin;
          value = (value - pad.fX1) / (pad.fX2 - pad.fX1);
        }
      }
      value =
        axis == 'y'
          ? (1 - value) * this.pad_height()
          : value * this.pad_width();
    }
  ...
  TObjectPainter.prototype.SvgToAxis = function (axis, coord, ndc) {
    ...
    if (use_frame && main) {
      return axis == 'y'
        ? main.RevertY(coord - (use_frame ? 0 : main.frame_y()))
    ...
    var value =
      axis == 'y' ? 1 - coord / this.pad_height() : coord / this.pad_width();
    var pad = ndc ? null : this.root_pad();
  ...
  TObjectPainter.prototype.size_for_3d = function (can3d) {
    ...
    if ((!fp || fp.mode3d) && can3d > 0) {
      ...
      size.width = Math.round(
        size.width *
          (1 - JSROOT.gStyle.fPadLeftMargin - JSROOT.gStyle.fPadRightMargin)
      );
      size.height = Math.round(
        size.height *
          (1 - JSROOT.gStyle.fPadTopMargin - JSROOT.gStyle.fPadBottomMargin)
      );
    }

    var pw = this.pad_width(this.this_pad_name),
      x2 = pw - size.x - size.width,
      ph = this.pad_height(this.this_pad_name),
      y2 = ph - size.y - size.height;
  ...
  TObjectPainter.prototype.apply_3d_size = function (size, onlyget) {
    ...
    if (size.can3d > 1) {
      ...
      if (size.can3d === 3 || size.can3d === 4) {
        ...
        if (elem.empty())
          elem = svg
            .insert('g', '.primitives_layer')
            .attr('class', size.clname);

        elem.attr('transform', 'translate(' + size.x + ',' + size.y + ')');
      } else {
        if (elem.empty())
          elem = svg
            .insert('foreignObject', '.primitives_layer')
            .attr('class', size.clname);
      ...
      if (elem.empty())
        elem = d3
          .select(prnt)
          .append('div')
          .attr('class', size.clname + ' jsroot_noselect');
  ...
  TObjectPainter.prototype.SetDivId = function (divid, is_main, pad_name) {
    ...
    if (is_main >= 0 && this.select_main(true).empty()) {
      if (typeof divid == 'string')
        console.error('element with id ' + divid + ' not exists');
      else
        console.error(
    ...
    if (this.pad_name === undefined) this.pad_name = this.CurrentPadName();

    if (is_main < 0) return;

    // create TFrame element if not exists
    if (
      this.svg_frame().select('.main_layer').empty() &&
      (is_main == 1 || is_main == 3 || is_main == 4)
    ) {
      if (typeof JSROOT.Painter.drawFrame == 'function')
        JSROOT.Painter.drawFrame(divid, null, is_main == 4 ? '3d' : '');
      if (is_main != 4 && this.svg_frame().empty())
    ...
    if (pp && pp !== this) pp.painters.push(this);

    if (
      (is_main === 1 || is_main === 4 || is_main === 5) &&
      !svg_p.property('mainpainter')
    )
  ...
  TObjectPainter.prototype.CalcAbsolutePosition = function (sel, pos) {
    while (!sel.empty() && !sel.classed('root_canvas')) {
      ...
      if (
        cl &&
        (cl.indexOf('root_frame') >= 0 || cl.indexOf('__root_pad_') >= 0)
      ) {
  ...
  TObjectPainter.prototype.createAttMarker = function (args) {
    if (!args || typeof args !== 'object') args = { std: true };
    else if (
      args.fMarkerColor !== undefined &&
      args.fMarkerStyle !== undefined &&
      args.fMarkerSize !== undefined
    )
  ...
  TObjectPainter.prototype.createAttLine = function (args) {
    if (!args || typeof args !== 'object') args = { std: true };
      ...
      args.fLineColor !== undefined &&
      args.fLineStyle !== undefined &&
      args.fLineWidth !== undefined
    )
      args = { attr: args, std: false };
  ...
  TObjectPainter.prototype.createAttFill = function (args) {
    if (!args || typeof args !== 'object') args = { std: true };
      ...
      args._typename &&
      args.fFillColor !== undefined &&
      args.fFillStyle !== undefined
    )
      args = { attr: args, std: false };
  ...
  TObjectPainter.prototype.AddMove = function () {
    if (
      !JSROOT.gStyle.MoveResize ||
      JSROOT.BatchMode ||
      !this.draw_g ||
      this.draw_g.property('assigned_move')
    )
      return;
    ...
    drag_move
      .on(
        ...
        function () {
          ...
          not_changed = true;
          if (this.moveStart) this.moveStart(pos[0], pos[1]);
        }.bind(this)
      )
      .on(
        'drag',
        function () {
          ...
          d3.event.sourceEvent.stopPropagation();
          not_changed = false;
          if (this.moveDrag) this.moveDrag(d3.event.dx, d3.event.dy);
        }.bind(this)
      )
      .on(
        prefix + 'end',
        function () {
          d3.event.sourceEvent.preventDefault();
          d3.event.sourceEvent.stopPropagation();
          if (this.moveEnd) this.moveEnd(not_changed);
          var cp = this.canv_painter();
  ...
  TObjectPainter.prototype.AddDrag = function (callback) {
    ...
    function rect_width() {
      return Number(pthis.draw_g.attr('width'));
    }
    function rect_height() {
      return Number(pthis.draw_g.attr('height'));
    }
    ...
    function MakeResizeElements(group, width, height, handler) {
      ...
      make(
        'se-resize',
        'M' + (width - 2) + ',' + (height - 2) + 'h-15v5h20v-20h-5Z'
      );

      make('w-resize', 'M-3,18h5v' + Math.max(0, height - 2 * 18) + 'h-5Z');
      make(
        'e-resize',
        'M' + (width + 3) + ',18h-5v' + Math.max(0, height - 2 * 18) + 'h5Z'
      );
      make('n-resize', 'M18,-3v5h' + Math.max(0, width - 2 * 18) + 'v-5Z');
      make(
        's-resize',
        'M18,' + (height + 3) + 'v-5h' + Math.max(0, width - 2 * 18) + 'v5Z'
      );
    }
    ...
    function complete_drag() {
      ...
      if (callback.minwidth && newwidth < callback.minwidth)
        newwidth = callback.minwidth;
      if (callback.minheight && newheight < callback.minheight)
        newheight = callback.minheight;

      var change_size =
          newwidth !== rect_width() || newheight !== rect_height(),
      ...
      if (change_size || change_pos) {
        if (change_size && 'resize' in callback)
          callback.resize(newwidth, newheight);
        if (change_pos && 'move' in callback)
          callback.move(newx, newy, newx - oldxx, newy - oldy);
    ...
    drag_move
      .on(prefix + 'start', function () {
        ...
        drag_rect = d3
          .select(pthis.draw_g.node().parentNode)
          .append('rect')
          .classed('zoom', true)
      ...
      .on('drag', function () {
        ...
        drag_rect
          .attr('x', Math.min(Math.max(handle.acc_x1, 0), handle.pad_w))
    ...
    drag_resize
      ...
      .on('drag', function () {
        ...
        var x1 = Math.max(0, handle.acc_x1),
          x2 = Math.min(handle.acc_x2, handle.pad_w),
          y1 = Math.max(0, handle.acc_y1),
          y2 = Math.min(handle.acc_y2, handle.pad_h);
  ...
  TObjectPainter.prototype.startTouchMenu = function (kind) {
    ...
    this.svg_frame()
      .on('touchcancel', this.endTouchMenu.bind(this, kind))
  ...
  TObjectPainter.prototype.endTouchMenu = function (kind) {
    ...
    if (diff > 500) {
      ...
      this.ShowContextMenu(kind, {
        clientX: rect.left + this[fld].pos[0],
   ...
   * @private */
  TObjectPainter.prototype.AddColorMenuEntry = function (
    menu,
    name,
    value,
    set_func,
    ...
    menu.add('sub:' + name, function () {
      ...
      var col = prompt(
        'Enter color ' + (useid ? '(only id number)' : '(name or id)'),
        value
      );
      if (col == null) return;
      var id = parseInt(col);
    ...
    for (var n = -1; n < 11; ++n) {
      ...
      var svg =
        "<svg width='100' height='18' style='margin:0px;background-color:" +
        col +
        "'><text x='4' y='12' style='font-size:12px' fill='" +
        (n == 1 ? 'white' : 'black') +
        "'>" +
        col +
        '</text></svg>';
   ...
   * @private */
  TObjectPainter.prototype.AddSizeMenuEntry = function (
    menu,
    name,
    min,
    max,
   ...
   * @private */
  TObjectPainter.prototype.FillObjectExecMenu = function (
    menu,
    kind,
    call_back
  ) {
    ...
    function DoExecMenu(arg) {
      ...
      if (item.fExec == 'Show:Editor') {
        if (cp && typeof cp.ActivateGed == 'function') cp.ActivateGed(execp);
        return;
      }
    ...
    function DoFillMenu(_menu, _reqid, _call_back, reply) {
      ...
      if (items && items.length) {
        ...
        for (var n = 0; n < items.length; ++n) {
          ...
          else _menu.addchk(item.fChecked, item.fName, n, DoExecMenu);
        }
  ...
  TObjectPainter.prototype.GetColorExec = function (col, method) {
    var id = -1,
      arr = JSROOT.Painter.root_colors;
    if (typeof col == 'string') {
      if (!col || col == 'none') id = 0;
      else
  ...
  TObjectPainter.prototype.FillAttContextMenu = function (menu, preffix) {
    ...
    if (this.lineatt && this.lineatt.used) {
      ...
      this.AddSizeMenuEntry(
        ...
        this.lineatt.width,
        function (arg) {
          this.lineatt.Change(undefined, parseInt(arg));
          this.InteractiveRedraw(true, 'exec:SetLineWidth(' + arg + ')');
        }.bind(this)
      );
      this.AddColorMenuEntry(
        menu,
        'color',
        this.lineatt.color,
        function (arg) {
          this.lineatt.Change(arg);
          this.InteractiveRedraw(true, this.GetColorExec(arg, 'SetLineColor'));
        }.bind(this)
      );
      menu.add(
        'sub:style',
      ...
      for (var n = 1; n < 11; ++n) {
        ...
        var svg =
          "<svg width='100' height='18'><text x='1' y='12' style='font-size:12px'>" +
          n +
          "</text><line x1='30' y1='8' x2='100' y2='8' stroke='black' stroke-width='3' stroke-dasharray='" +
          dash +
          "'></line></svg>";

        menu.addchk(
          this.lineatt.style == n,
          svg,
          n,
          function (arg) {
            this.lineatt.Change(undefined, undefined, parseInt(arg));
            this.InteractiveRedraw(true, 'exec:SetLineStyle(' + arg + ')');
          }.bind(this)
        );
      }
      menu.add('endsub:');
      ...
      if ('excl_side' in this.lineatt && this.lineatt.excl_side !== 0) {
        ...
        this.AddSizeMenuEntry(
          ...
          100,
          10,
          this.lineatt.excl_width,
          function (arg) {
            this.lineatt.ChangeExcl(undefined, parseInt(arg));
            this.InteractiveRedraw();
          }.bind(this)
        );
    ...
    if (this.fillatt && this.fillatt.used) {
      ...
      this.AddColorMenuEntry(
        menu,
        'color',
        this.fillatt.colorindx,
        function (arg) {
          this.fillatt.Change(parseInt(arg), undefined, this.svg_canvas());
          this.InteractiveRedraw(
            true,
            this.GetColorExec(parseInt(arg), 'SetFillColor')
          );
        }.bind(this),
        this.fillatt.kind
      );
      menu.add(
        ...
        function () {
          var id = prompt(
            'Enter fill style id (1001-solid, 3000..3010)',
            this.fillatt.pattern
          );
          if (id == null) return;
          id = parseInt(id);
          if (isNaN(id)) return;
      ...
      for (var n = 0; n < supported.length; ++n) {
        var sample = this.createAttFill({
            std: false,
            pattern: supported[n],
            color: this.fillatt.colorindx || 1,
          }),
          svg =
            "<svg width='100' height='18'><text x='1' y='12' style='font-size:12px'>" +
            supported[n].toString() +
            "</text><rect x='40' y='0' width='60' height='18' stroke='none' fill='" +
            sample.fillcolor() +
            "'></rect></svg>";

        menu.addchk(
          this.fillatt.pattern == supported[n],
          svg,
          supported[n],
          function (arg) {
    ...
    if (this.markeratt && this.markeratt.used) {
      ...
      this.AddColorMenuEntry(
        menu,
        'color',
        this.markeratt.color,
        function (arg) {
          this.markeratt.Change(arg);
          this.InteractiveRedraw(
            true,
            this.GetColorExec(arg, 'SetMarkerColor')
          );
        }.bind(this)
      );
      this.AddSizeMenuEntry(
        ...
        this.markeratt.size,
        function (arg) {
          this.markeratt.Change(undefined, undefined, parseFloat(arg));
          this.InteractiveRedraw(
            true,
      ...
      for (var n = 0; n < supported.length; ++n) {
        var clone = new TAttMarkerHandler({
            style: supported[n],
            color: this.markeratt.color,
            size: 1.7,
          }),
          svg =
            "<svg width='60' height='18'><text x='1' y='12' style='font-size:12px'>" +
            supported[n].toString() +
            "</text><path stroke='black' fill='" +
            (clone.fill ? 'black' : 'none') +
            "' d='" +
            clone.create(40, 8) +
            "'></path></svg>";

        menu.addchk(
          this.markeratt.style == supported[n],
          svg,
          supported[n],
          function (arg) {
            this.markeratt.Change(undefined, parseInt(arg));
            this.InteractiveRedraw(true, 'exec:SetMarkerStyle(' + arg + ')');
          }.bind(this)
        );
      }
      menu.add('endsub:');
  ...
  TObjectPainter.prototype.TextAttContextMenu = function (menu, prefix) {
    ...
    this.AddColorMenuEntry(
      ...
      obj.fTextColor,
      function (arg) {
        this.GetObject().fTextColor = parseInt(arg);
        this.InteractiveRedraw(
          true,
          this.GetColorExec(parseInt(arg), 'SetTextColor')
        );
      }.bind(this)
    );
    ...
    for (var n = 0; n < align.length; ++n) {
      menu.addchk(
        align[n] == obj.fTextAlign,
        align[n],
        align[n],
        // align[n].toString() + "_h:" + hnames[Math.floor(align[n]/10) - 1] + "_v:" + vnames[align[n]%10-1], align[n],
        function (arg) {
          this.GetObject().fTextAlign = parseInt(arg);
          this.InteractiveRedraw(true, 'exec:SetTextAlign(' + arg + ')');
        }.bind(this)
      );
    }
    menu.add('endsub:');
    ...
    for (var n = 1; n < 16; ++n) {
      menu.addchk(
        n == Math.floor(obj.fTextFont / 10),
        n,
        n,
        function (arg) {
          this.GetObject().fTextFont = parseInt(arg) * 10 + 2;
          this.InteractiveRedraw(
            true,
  ...
  TObjectPainter.prototype.GetShowStatusFunc = function () {
    ...
    var pp = this.canv_painter(),
      res = JSROOT.Painter.ShowStatus;

    if (pp && typeof pp.ShowCanvasStatus === 'function')
      res = pp.ShowCanvasStatus.bind(pp);
  ...
  TObjectPainter.prototype.ShowObjectStatus = function () {
    ...
    if (obj && status_func)
      status_func(
        this.GetItemName() || obj.fName,
        obj.fTitle || obj._typename,
        obj._typename
      );
  };
   ...
   * histogram functions
   * @private */
  TObjectPainter.prototype.FindPainterFor = function (
    selobj,
    selname,
    seltype
  ) {
  ...
  TObjectPainter.prototype.ConfigureUserTooltipCallback = function (
    call_back,
    user_timeout
  ) {
  ...
  TObjectPainter.prototype.ProvideUserTooltip = function (data) {
    ...
    if (this.UserTooltipTimeout <= 0) return this.UserTooltipCallback(data);
    ...
    if (data == null) return this.UserTooltipCallback(data);
   ...
   */
  TObjectPainter.prototype.StartTextDrawing = function (
    font_face,
    font_size,
    draw_g,
    max_font_size
    ...
    var font =
      font_size === 'font'
        ? font_face
        : JSROOT.Painter.getFontDetails(font_face, font_size);
    ...
    draw_g
      .property('draw_text_completed', false)
    ...
    if (draw_g.property('_fast_drawing'))
      draw_g.property(
        '_font_too_small',
        (max_font_size && max_font_size < 5) || font.size < 4
      );
  };
  ...
  TObjectPainter.prototype.TextScaleFactor = function (value, draw_g) {
    if (!draw_g) draw_g = this.draw_g;
    if (value && value > draw_g.property('text_factor'))
      draw_g.property('text_factor', value);
  };
  ...
  TObjectPainter.prototype.GetBoundarySizes = function (elem) {
    if (elem === null) {
      console.warn('empty node in GetBoundarySizes');
  ...
  TObjectPainter.prototype.FinishTextDrawing = function (draw_g, call_ready) {
    ...
    if (f > 0 && (f < 0.9 || f > 1)) font.size = Math.floor(font.size / f);

    if (max_sz && font.size > max_sz) font.size = max_sz;
    ...
    if (svgs)
      svgs.each(function () {
        ...
        if (
          valign &&
          valign.indexOf('vertical-align:') == 0 &&
          valign.indexOf('ex;') == valign.length - 3
        ) {
        ...
        if (!JSROOT.nodejs) {
          ...
          width = 1.05 * box.width;
          height = 1.05 * box.height;
        }
    ...
    if (svgs)
      svgs.each(function () {
        ...
        else if (arg.align[0] == 'end') arg[nx] += sign.x * (arg.width - mw);
    ...
    draw_g
      ...
      .each(function () {
        ...
        if (JSROOT.nodejs) {
          if (arg.scale && f > 0) {
            arg.box.width = arg.box.width / f;
            arg.box.height = arg.box.height / f;
          }
        ...
        if (arg.width) {
          ...
          if (arg.align[0] == 'middle') arg.x += arg.width / 2;
          else if (arg.align[0] == 'end') arg.x += arg.width;
        }
        ...
        if (arg.plain) {
          ...
          arg.dx =
            (arg.align[0] == 'middle' ? -0.5 : arg.align[0] == 'end' ? -1 : 0) *
            arg.box.width;
        }

        if (arg.height) {
          if (arg.align[1].indexOf('bottom') === 0) arg.y += arg.height;
          else if (arg.align[1] == 'middle') arg.y += arg.height / 2;
        }

        if (arg.plain) {
          if (arg.align[1] == 'top') txt.attr('dy', '.8em');
          else if (arg.align[1] == 'middle') {
            if (JSROOT.browser.isIE || JSROOT.nodejs) txt.attr('dy', '.4em');
        ...
        var trans =
          arg.x || arg.y
            ? 'translate(' + Math.round(arg.x) + ',' + Math.round(arg.y) + ')'
            : '';
        if (arg.rotate) trans += ' rotate(' + Math.round(arg.rotate) + ')';
        if (arg.dx || arg.dy)
          trans +=
            ' translate(' + Math.round(arg.dx) + ',' + Math.round(arg.dy) + ')';
        if (trans) txt.attr('transform', trans);

        if (
          JSROOT.browser.isWebKit &&
          draw_g.node().insertAdjacentHTML &&
          arg.large_latex
        ) {
  ...
  TObjectPainter.prototype.produceLatex = function (node, label, arg, curr) {
    ...
    function makeem(value) {
      ...
      if (res.indexOf('0.') == 0) res = res.substr(1);
      else if (res.indexOf('-0.') == 0) res = '-.' + res.substr(3);
    ...
    function get_boundary(painter, element, approx_rect) {
      ...
      element.selectAll('tspan').each(function () {
        important.push(this);
      });
      ...
      tspans.each(function () {
        if (important.indexOf(this) < 0)
          d3.select(this).attr('display', 'none');
      });
      var box = painter.GetBoundarySizes(arg.mainnode);

      tspans.each(function () {
        if (important.indexOf(this) < 0) d3.select(this).attr('display', null);
      });
    ...
    while (label) {
      ...
      if (found.arg) {
        ...
        if (found.arg == 'int') {
          ...
          if (isNaN(foundarg)) {
            console.log('wrong int argument', label.substr(0, pos));
            return false;
          }
        } else if (found.arg == 'float') {
          foundarg = parseFloat(foundarg);
          if (isNaN(foundarg)) {
            console.log('wrong float argument', label.substr(0, pos));
            return false;
          }
        }
      ...
      var nextdy = curr.dy,
        ...
        scale = 1,
        left_brace = '{',
        right_brace = '}'; // this will be applied to the next element

      curr.dy = curr.dx = 0; // relative shift for elements

      if (found.special) {
        subnode
          .attr('dx', makeem(nextdx))
          .attr('dy', makeem(nextdy))
          .text(found.special);
        nextdx = nextdy = 0;
        curr.special = found;
      ...
      if (found.braces) {
        ...
        subpos.left = subpos.left_cont
          .append('svg:tspan')
          .text(found.braces[0]);
        subnode1 = subnode.append('svg:tspan');
        subpos.left_rect = {
          y: curr.y - curr.fsize * 1.1,
          height: curr.fsize * 1.2,
          x: curr.x,
          width: curr.fsize * 0.6,
        };
        ...
        switch (found.name) {
          ...
          case 'scale[':
            ...
            break;
          case '#font[':
            JSROOT.Painter.getFontDetails(foundarg).setFont(
              subnode,
              'without-size'
            );
            break;
          ...
          case '#sqrt[':
            ...
            subpos.square_root
              .append('svg:tspan')
              .text(
                foundarg == 3 ? '\u221B' : foundarg == 4 ? '\u221C' : '\u221A'
              ); // unicode square, cubic and fourth root
            subnode1 = subnode.append('svg:tspan');
            subpos.sqrt_rect = {
              y: curr.y - curr.fsize * 1.1,
              height: curr.fsize * 1.2,
              x: 0,
              width: curr.fsize * 0.7,
            };
      ...
      while (true) {
        ...
        while (n != 0 && ++pos < label.length) {
          if (label.indexOf(left_brace, pos) === pos) n++;
          else if (label.indexOf(right_brace, pos) === pos) n--;
        }
        ...
        if (subpos.square_root) {
          ...
          if (be.height > bs.height && bs.height > 0) {
            ...
            sqrt_dy =
              (be.y + be.height - (bs.y + bs.height)) / curr.fsize / yscale;
            subpos.square_root
              .style('font-size', Math.round(100 * yscale) + '%')
              .attr('dy', makeem(sqrt_dy));
          }
          ...
          subpos.square_root
            .append('svg:tspan')
            .attr('dy', makeem(0.25 - sqrt_dy))
            .attr('dx', makeem(-a.length / 3 - 0.2))
            .text('\u2009'); // unicode tiny space
        ...
        if (subpos.braces) {
          ...
          if (yscale != 1)
            right_cont
              .append('svg:tspan')
              .attr('dx', makeem(-0.2))
              .text('\u2009'); // unicode tiny space if larger brace is used

          var right = right_cont
            .append('svg:tspan')
            .text(subpos.braces.braces[1]);
  ...
  TObjectPainter.prototype.DrawText = function (arg) {
    ...
    if (typeof arg.align == 'string') {
      ...
      if (arg.align / 10 >= 3) align[0] = 'end';
      else if (arg.align / 10 >= 2) align[0] = 'middle';
      if (arg.align % 10 == 0) align[1] = 'bottom';
      else if (arg.align % 10 == 1) align[1] = 'bottom-base';
      else if (arg.align % 10 == 3) align[1] = 'top';
    }
    ...
    if (JSROOT.gStyle.MathJax !== undefined) {
      switch (JSROOT.gStyle.MathJax) {
        case 0:
          JSROOT.gStyle.Latex = 2;
          break;
        case 2:
          JSROOT.gStyle.Latex = 4;
          break;
        default:
          JSROOT.gStyle.Latex = 3;
      }
    ...
    if (typeof JSROOT.gStyle.Latex == 'string') {
      switch (JSROOT.gStyle.Latex) {
        case 'off':
          JSROOT.gStyle.Latex = 0;
          break;
        case 'symbols':
          JSROOT.gStyle.Latex = 1;
          break;
        case 'MathJax':
        case 'mathjax':
        case 'math':
        ...
        default:
          ...
          JSROOT.gStyle.Latex =
            !isNaN(code) && code >= 0 && code <= 4 ? code : 2;
      }
    }
    ...
    if (arg.latex === 1)
      use_mathjax =
        JSROOT.gStyle.Latex > 3 ||
        (JSROOT.gStyle.Latex == 3 && JSROOT.Painter.isAnyLatex(label));
    ...
    if (!use_mathjax || arg.nomathjax) {
      ...
      arg.plain =
        !arg.latex ||
        JSROOT.gStyle.Latex < 2 ||
        this.produceLatex(txt, label, arg) === 0;

      if (arg.plain) {
        if (arg.latex && JSROOT.gStyle.Latex == 1)
          label = Painter.translateLaTeX(label); // replace latex symbols
        txt.text(label);
      }

      // complete rectangle with very rougth size estimations
      arg.box =
        !JSROOT.nodejs && !JSROOT.gStyle.ApproxTextSize && !arg.fast
          ? this.GetBoundarySizes(txt.node())
          : arg.text_rect || {
              height: arg.font_size * 1.2,
              width: JSROOT.Painter.approxTextWidth(font, label),
            };
      ...
      if (arg.scale)
        this.TextScaleFactor((1.05 * arg.box.width) / arg.width, arg.draw_g);
      if (arg.scale)
        this.TextScaleFactor((1 * arg.box.height) / arg.height, arg.draw_g);
    ...
    if (JSROOT.nodejs) {
      ...
      if (
        mtext.indexOf('\\(') == 0 &&
        mtext.lastIndexOf('\\)') == mtext.length - 2
      )
  ...
  TObjectPainter.prototype.FinishMathjax = function (draw_g, fo_g, id) {
    ...
    if (vvv.empty()) {
      ...
      if (arg && arg.latex != 2) {
        ...
        fo_g
          .append('svg')
          .attr('width', Math.min(20, merr.text().length + 5) + 'ex')
          .attr('height', '3ex')
  ...
  TooltipHandler.prototype.ProcessTooltipEvent = function (pnt, enabled) {
    ...
    if (pnt && pnt.handler) {
      ...
      if (!rect || rect.empty()) {
        ...
        pnt =
          pos && pos.length == 1
            ? { touch: true, x: pos[0][0], y: pos[0][1] }
            : null;
      } else {
    ...
    for (var n = 0; n < hints.length; ++n) {
      ...
      hint.height = Math.round(
        hint.lines.length * textheight * hstep +
          2 * hmargin -
          textheight * (hstep - 1)
      );
    ...
    if (status_func) {
      ...
      for (var k = 0; k < (hints ? hints.length : 0); ++k) {
        ...
        if (hints[k].exact && (!hint || !hint.exact)) {
          hint = hints[k];
          break;
        }
        ...
        var dist2 =
          (pnt.x - hints[k].x) * (pnt.x - hints[k].x) +
          (pnt.y - hints[k].y) * (pnt.y - hints[k].y);
        if (dist2 < best_dist2) {
          best_dist2 = dist2;
          best_hint = hints[k];
        }
    ...
    var frame_shift = { x: 0, y: 0 },
      trans = frame_rect.transform || '';
    if (!pp.iscan) {
    ...
    var viewmode = hintsg.property('viewmode') || '',
      actualw = 0,
      posx = pnt.x + frame_rect.hint_delta_x;
    ...
    if (nhints > 1) {
      ...
      if (viewmode == 'left') bright = 0.7;
      else if (viewmode == 'right') bleft = 0.3;
    ...
    for (var n = 0; n < hints.length; ++n) {
      ...
      if (viewmode == 'single') {
        curry = pnt.touch
          ? pnt.y - hint.height - 5
          : Math.min(pnt.y + 15, maxhinty - hint.height - 3) +
            frame_rect.hint_delta_y;
      } else {
      ...
      if (!was_empty) group.selectAll('*').remove();

      group.attr('width', 60).attr('height', hint.height);
      ...
      if (was_empty)
        if (JSROOT.gStyle.TooltipAnimation > 0)
          group
            .transition()
            .duration(JSROOT.gStyle.TooltipAnimation)
            .attrTween('opacity', translateFn());
    ...
    if (
      viewmode == 'single' &&
      posx + actualw > pad_width - frame_rect.x &&
      posx > actualw + 20
    ) {
      posx -= actualw + 20;
      svgs.attr('x', posx);
    ...
    if (viewmode !== 'single' && gapy < maxhinty && gapy !== curry) {
      if (gapminx <= posx + actualw + 5 && gapmaxx >= posx - 5)
        svgs.attr('y', function () {
          return d3.select(this).property('gapy');
        });
    } else if (viewmode !== 'single' && curry > maxhinty) {
      var shift = Math.max(maxhinty - curry - 10, minhinty);
      if (shift < 0)
        svgs.attr('y', function () {
          return d3.select(this).property('curry') + shift;
        });
    }
  ...
  Painter.drawRawText = function (divid, txt, opt) {
    ...
    painter.Draw = function () {
      var txt =
        this.txt._typename && this.txt._typename == 'TObjString'
          ? this.txt.fString
          : this.txt.value;
  ...
  JSROOT.addDrawFunc({
    ...
    icon: 'img_canvas',
    prereq: 'v6',
    func: 'JSROOT.Painter.drawCanvas',
    opt: ';grid;gridx;gridy;tick;tickx;ticky;log;logx;logy;logz',
    expand_item: 'fPrimitives',
  });
  JSROOT.addDrawFunc({
    name: 'TPad',
    icon: 'img_canvas',
    prereq: 'v6',
    func: 'JSROOT.Painter.drawPad',
    opt: ';grid;gridx;gridy;tick;tickx;ticky;log;logx;logy;logz',
    expand_item: 'fPrimitives',
  });
  JSROOT.addDrawFunc({
  ...
  JSROOT.addDrawFunc({
    ...
    icon: 'img_frame',
    prereq: 'v6',
    func: 'JSROOT.Painter.drawFrame',
  });
  JSROOT.addDrawFunc({
    name: 'TPave',
    icon: 'img_pavetext',
    prereq: 'v6;hist',
    func: 'JSROOT.Painter.drawPave',
  });
  JSROOT.addDrawFunc({
    name: 'TPaveText',
    icon: 'img_pavetext',
  ...
  JSROOT.addDrawFunc({
    ...
    icon: 'img_histo1d',
    prereq: 'v6;hist',
    func: 'JSROOT.Painter.drawHistogram1D',
    opt: ';hist;P;P0;E;E1;E2;E3;E4;E1X0;L;LF2;B;B1;A;TEXT;LEGO;same',
    ctrl: 'l',
  });
  JSROOT.addDrawFunc({
    name: 'TProfile',
    ...
    prereq: 'v6;hist',
    func: 'JSROOT.Painter.drawHistogram1D',
    opt: ';E0;E1;E2;p;AH;hist',
  });
  JSROOT.addDrawFunc({
    name: 'TH2Poly',
    icon: 'img_histo2d',
    prereq: 'v6;hist',
    func: 'JSROOT.Painter.drawHistogram2D',
    opt: ';COL;COL0;COLZ;LCOL;LCOL0;LCOLZ;LEGO;TEXT;same',
    expand_item: 'fBins',
    theonly: true,
  });
  ...
  JSROOT.addDrawFunc({
    ...
    func: 'JSROOT.Painter.drawHistogram2D',
    opt:
      ';COL;COLZ;COL0;COL1;COL0Z;COL1Z;COLA;BOX;BOX1;PROJ;PROJX1;PROJX2;PROJX3;PROJY1;PROJY2;PROJY3;SCAT;TEXT;TEXTE;TEXTE0;CONT;CONT1;CONT2;CONT3;CONT4;ARR;SURF;SURF1;SURF2;SURF4;SURF6;E;A;LEGO;LEGO0;LEGO1;LEGO2;LEGO3;LEGO4;same',
    ctrl: 'colz',
  });
  ...
  JSROOT.addDrawFunc({
    ...
    icon: 'img_histo3d',
    prereq: 'v6;hist3d',
    func: 'JSROOT.Painter.drawHistogram3D',
    opt: ';SCAT;BOX;BOX2;BOX3;GLBOX1;GLBOX2;GLCOL',
  });
  JSROOT.addDrawFunc({
    name: 'THStack',
    icon: 'img_histo1d',
    prereq: 'v6;hist',
    func: 'JSROOT.Painter.drawHStack',
    expand_item: 'fHists',
    opt: 'NOSTACK;HIST;E;PFC;PLC',
  });
  JSROOT.addDrawFunc({
    name: 'TPolyMarker3D',
    icon: 'img_histo3d',
    prereq: 'v6;hist3d',
    func: 'JSROOT.Painter.drawPolyMarker3D',
  });
  JSROOT.addDrawFunc({
    name: 'TPolyLine3D',
    icon: 'img_graph',
    prereq: '3d',
    func: 'JSROOT.Painter.drawPolyLine3D',
    direct: true,
  });
  ...
  JSROOT.addDrawFunc({
    ...
    icon: 'img_graph',
    prereq: 'more2d',
    func: 'JSROOT.Painter.drawGraphTime',
    opt: 'once;repeat;first',
    theonly: true,
  });
  JSROOT.addDrawFunc({
    name: 'TGraph2D',
    icon: 'img_graph',
    prereq: 'v6;hist3d',
    func: 'JSROOT.Painter.drawGraph2D',
    opt: ';P;PCOL',
    theonly: true,
  });
  JSROOT.addDrawFunc({
  ...
  JSROOT.addDrawFunc({
    name: 'TGraphPolargram',
    icon: 'img_graph',
    prereq: 'more2d',
    func: 'JSROOT.Painter.drawGraphPolargram',
    theonly: true,
  });
  JSROOT.addDrawFunc({
    name: 'TGraphPolar',
    icon: 'img_graph',
    prereq: 'more2d',
    func: 'JSROOT.Painter.drawGraphPolar',
    opt: ';F;L;P;PE',
    theonly: true,
  });
  JSROOT.addDrawFunc({
  ...
  JSROOT.addDrawFunc({
    ...
    icon: 'img_graph',
    prereq: 'more2d',
    func: 'JSROOT.Painter.drawEfficiency',
    opt: ';AP',
  });
  ...
  JSROOT.addDrawFunc({
    ...
    icon: 'img_canvas',
    prereq: 'more2d',
    func: 'JSROOT.Painter.drawRooPlot',
  });
  JSROOT.addDrawFunc({
    name: 'TMultiGraph',
    icon: 'img_mgraph',
    prereq: 'more2d',
    func: 'JSROOT.Painter.drawMultiGraph',
    expand_item: 'fGraphs',
  });
  JSROOT.addDrawFunc({
    name: 'TStreamerInfoList',
    icon: 'img_question',
    prereq: 'hierarchy',
    func: 'JSROOT.Painter.drawStreamerInfo',
  });
  JSROOT.addDrawFunc({
    name: 'TPaletteAxis',
    icon: 'img_colz',
  ...
  JSROOT.addDrawFunc({
    ...
    icon: 'img_graph',
    prereq: 'more2d',
    func: 'JSROOT.Painter.drawWebPainting',
  });
  JSROOT.addDrawFunc({
    name: 'TCanvasWebSnapshot',
    icon: 'img_canvas',
    prereq: 'v6',
    func: 'JSROOT.Painter.drawPadSnapshot',
  });
  JSROOT.addDrawFunc({ name: 'TPadWebSnapshot', sameas: 'TCanvasWebSnapshot' });
  JSROOT.addDrawFunc({
    name: 'kind:Text',
    icon: 'img_text',
    func: JSROOT.Painter.drawRawText,
  });
  JSROOT.addDrawFunc({
    name: 'TObjString',
    icon: 'img_text',
  ...
  JSROOT.addDrawFunc({
    ...
    icon: 'img_tf1',
    prereq: 'math;more2d',
    func: 'JSROOT.Painter.drawFunction',
  });
  JSROOT.addDrawFunc({
    name: 'TF2',
    icon: 'img_tf2',
    prereq: 'math;hist',
    func: 'JSROOT.Painter.drawTF2',
  });
  JSROOT.addDrawFunc({
    name: 'TSpline3',
    icon: 'img_tf1',
    prereq: 'more2d',
    func: 'JSROOT.Painter.drawSpline',
  });
  JSROOT.addDrawFunc({
    name: 'TSpline5',
    icon: 'img_tf1',
  ...
  JSROOT.addDrawFunc({
    ...
    icon: 'img_graph',
    prereq: 'more2d',
    func: 'JSROOT.Painter.drawEllipse',
    direct: true,
  });
  ...
  JSROOT.addDrawFunc({
    ...
    icon: 'img_graph',
    prereq: 'more2d',
    func: 'JSROOT.Painter.drawPolyLine',
    direct: true,
  });
  ...
  JSROOT.addDrawFunc({
    name: 'TAxis3D',
    prereq: 'v6;hist3d',
    func: 'JSROOT.Painter.drawAxis3D',
  });
  JSROOT.addDrawFunc({
    name: 'TMarker',
    icon: 'img_graph',
    prereq: 'more2d',
    func: 'JSROOT.Painter.drawMarker',
    direct: true,
  });
  JSROOT.addDrawFunc({
    name: 'TPolyMarker',
    icon: 'img_graph',
    prereq: 'more2d',
    func: 'JSROOT.Painter.drawPolyMarker',
    direct: true,
  });
  JSROOT.addDrawFunc({
    name: 'TASImage',
    icon: 'img_mgraph',
    prereq: 'more2d',
    func: 'JSROOT.Painter.drawASImage',
  });
  JSROOT.addDrawFunc({
    name: 'TJSImage',
    icon: 'img_mgraph',
    prereq: 'more2d',
    func: 'JSROOT.Painter.drawJSImage',
    opt: ';scale;center',
  });
  JSROOT.addDrawFunc({
    name: 'TGeoVolume',
    icon: 'img_histo3d',
    prereq: 'geom',
    func: 'JSROOT.Painter.drawGeoObject',
    expand: 'JSROOT.GEO.expandObject',
    opt: ';more;all;count;projx;projz;wire;dflt',
    ctrl: 'dflt',
  });
  JSROOT.addDrawFunc({
    name: 'TEveGeoShapeExtract',
    icon: 'img_histo3d',
    prereq: 'geom',
    func: 'JSROOT.Painter.drawGeoObject',
    expand: 'JSROOT.GEO.expandObject',
  ...
  JSROOT.addDrawFunc({
    name: 'ROOT::Experimental::TEveGeoShapeExtract',
    icon: 'img_histo3d',
    prereq: 'geom',
    func: 'JSROOT.Painter.drawGeoObject',
    expand: 'JSROOT.GEO.expandObject',
  ...
  JSROOT.addDrawFunc({
    ...
    expand: 'JSROOT.GEO.expandObject',
    func: 'JSROOT.Painter.drawGeoObject',
    opt: ';more;all;count;projx;projz;wire;tracks;dflt',
    dflt: 'expand',
    ctrl: 'dflt',
  });
  JSROOT.addDrawFunc({
    ...
    prereq: 'geom',
    func: 'JSROOT.Painter.drawGeoObject',
    opt: ';more;all;axis;compa;count;projx;projz;wire;dflt',
    ctrl: 'dflt',
  });
  ...
  JSROOT.addDrawFunc({
    name: 'TFolder',
    icon: 'img_folder',
    icon2: 'img_folderopen',
    noinspect: true,
    prereq: 'hierarchy',
    expand: 'JSROOT.Painter.FolderHierarchy',
  });
  JSROOT.addDrawFunc({
    name: 'TTask',
    icon: 'img_task',
    prereq: 'hierarchy',
    expand: 'JSROOT.Painter.TaskHierarchy',
    for_derived: true,
  });
  JSROOT.addDrawFunc({
    name: 'TTree',
  ...
  JSROOT.addDrawFunc({
    ...
    icon: 'img_list',
    prereq: 'hierarchy',
    func: 'JSROOT.Painter.drawList',
    expand: 'JSROOT.Painter.ListHierarchy',
    dflt: 'expand',
  });
  ...
  JSROOT.addDrawFunc({
    name: 'ROOT::Experimental::TCanvas',
    icon: 'img_canvas',
    prereq: 'v7',
    func: 'JSROOT.v7.drawCanvas',
    opt: '',
    expand_item: 'fPrimitives',
  });
  ...
  JSROOT.getDrawSettings = function (kind, selector) {
    ...
    for (var cnt = 0; cnt < 1000; ++cnt) {
      ...
      for (var i = 0; i < opts.length; ++i) {
        ...
        if (selector.indexOf('nosame') >= 0 && opts[i].indexOf('same') == 0)
          continue;
    ...
    if (!noinspect && res.opts) res.opts.push('inspect');
  ...
  JSROOT.draw = function (divid, obj, opt, drawcallback) {
    ...
    function completeDraw(painter) {
      var callbackfunc = null,
        ishandle = false;
      if (typeof drawcallback == 'function') callbackfunc = drawcallback;
      else if (
        drawcallback &&
        typeof drawcallback == 'object' &&
        typeof drawcallback.func == 'function'
      ) {
      ...
      if (ishandle && isdirectdraw) {
        ...
        if (!painter || painter._ready_called_) {
          drawcallback.completed = true;
          return painter;
        }
      }
      ...
      else JSROOT.CallBack(callbackfunc, painter);
      return painter;
    }
    ...
    if (typeof handle.func == 'object') {
      ...
      if ('prereq' in handle && typeof handle.prereq == 'string')
        prereq = handle.prereq;
      if ('script' in handle && typeof handle.script == 'string')
        prereq += ';user:' + handle.script;
    }
  ...
  JSROOT.MakeSVG = function (args, callback) {
    ...
    function build(main) {
      ...
      JSROOT.draw(main.node(), args.object, args.option || '', function (
        painter
      ) {
        var has_workarounds =
          JSROOT.Painter.ProcessSVGWorkarounds && JSROOT.svg_workaround;
        ...
        if (has_workarounds) svg = JSROOT.Painter.ProcessSVGWorkarounds(svg);

        svg = svg
          .replace(/url\(\&quot\;\#(\w+)\&quot\;\)/g, 'url(#$1)') // decode all URL
    ...
    if (!JSROOT.nodejs) {
      build(
        d3.select(window.document).append('div').style('visible', 'hidden')
      );
      ...
      eval(
        'const { JSDOM } = require("jsdom"); JSROOT.nodejs_window = (new JSDOM("<!DOCTYPE html>hello")).window;'
      );
