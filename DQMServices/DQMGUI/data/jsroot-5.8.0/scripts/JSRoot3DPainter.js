...
(function (factory) {
  ...
  JSROOT.Painter.UseSVGFor3D = function () {
    ...
    if (this._Detect_UseSVGFor3D !== undefined) return this._Detect_UseSVGFor3D;
  ...
  JSROOT.Painter.Create3DRenderer = function (
    width,
    height,
    usesvg,
    makeimage,
    ...
    if (usesvg) {
      ...
      if (res.usesvgimg || res.renderer.makeOuterHTML !== undefined) {
        ...
        res.dom = document.createElementNS(
          'http://www.w3.org/2000/svg',
          'path'
        );
        res.dom.setAttribute(
          'jsroot_svg_workaround',
          res.renderer.workaround_id
        );
      }
    } else {
      res.renderer = res.usewebgl
    ...
    if (!res.dom) {
      ...
      if (!usesvg && makeimage) {
        res.dom = res.renderer.svgImage = document.createElementNS(
          'http://www.w3.org/2000/svg',
          'image'
        );
        d3.select(res.dom).attr('width', width).attr('height', height);
      }
    }
    ...
    if (renderer.workaround_id !== undefined) {
      if (typeof renderer.makeOuterHTML == 'function') {
        JSROOT.svg_workaround[
          renderer.workaround_id
        ] = renderer.makeOuterHTML();
      } else {
  ...
  JSROOT.Painter.ProcessSVGWorkarounds = function (svg) {
    ...
    for (var k = 0; k < JSROOT.svg_workaround.length; ++k)
      svg = svg.replace(
        '<path jsroot_svg_workaround="' + k + '"></path>',
        JSROOT.svg_workaround[k]
      );
  ...
  JSROOT.Painter.TooltipFor3D = function (prnt, canvas) {
    ...
    this.extract_pos = function (e) {
      if (typeof e == 'object' && e.u !== undefined && e.l !== undefined)
        return e;
      var res = { u: 0, l: 0 };
      if (this.abspos) {
        res.l = JSROOT.browser.isIE
          ? e.clientX + document.documentElement.scrollLeft
          : e.pageX;
        res.u = JSROOT.browser.isIE
    ...
    this.show = function (v, mouse_pos, status_func) {
      ...
      if (this.lastlbl !== v) {
        ...
        if (JSROOT.browser.isIE) this.tt.style.width = this.tt.offsetWidth;
      }
    };

    this.hide = function () {
      if (this.tt !== null) this.parent.removeChild(this.tt);
  ...
  JSROOT.Painter.CreateOrbitControl = function (
    painter,
    camera,
    scene,
    renderer,
    ...
    control.tooltip = new JSROOT.Painter.TooltipFor3D(
      painter.select_main().node(),
      renderer.domElement
    );
    ...
    control.GetMouseIntersects = function (mouse) {
      ...
      var pnt = {
        x: (mouse.x / sz.width) * 2 - 1,
        y: (-mouse.y / sz.height) * 2 + 1,
      };
    ...
    control.DetectZoomMesh = function (evnt) {
      ...
      if (intersects)
        for (var n = 0; n < intersects.length; ++n)
          if (intersects[n].object.zoom) return intersects[n];
    ...
    control.ProcessDblClick = function (evnt) {
      ...
      if (intersect && this.painter) {
        this.painter.Unzoom(
          intersect.object.use_y_for_z ? 'y' : intersect.object.zoom
        );
      } else {
        this.reset();
      }
    ...
    control.EndEvent = function () {
      ...
      if (this.mouse_ctxt.on) {
        ...
        this.ContextMenu(
          this.mouse_ctxt,
          this.GetMouseIntersects(this.mouse_ctxt)
        );
    ...
    control.MainProcessContextMenu = function (evnt) {
      ...
      if (this.control_active) this.mouse_ctxt.on = true;
      else if (this.block_ctxt) this.block_ctxt = false;
      else
        this.ContextMenu(
          this.mouse_ctxt,
          this.GetMouseIntersects(this.mouse_ctxt)
    ...
    control.MainProcessMouseMove = function (evnt) {
      ...
      if (this.control_active || this.block_mousemove || !this.ProcessMouseMove)
        return;

      if (this.mouse_zoom_mesh) {
        // when working with zoom mesh, need special handling

        var zoom2 = this.DetectZoomMesh(evnt),
          pnt2 = null;
        ...
        if (pnt2 && this.painter.enable_highlight)
          if (
            this.mouse_zoom_mesh.object.ShowSelection(
              this.mouse_zoom_mesh.point,
              pnt2
            )
          )
            this.painter.Render3D(0);
      ...
      if (!this.mouse_tmout) this.DelayedProcessMouseMove();
      else
        this.tmout_handle = setTimeout(
          this.DelayedProcessMouseMove.bind(this),
          this.mouse_tmout
        );
    };
    ...
    function control_mousewheel(evnt) {
      ...
      if (
        JSROOT.Painter.IsRender3DFired(control.painter) ||
        control.mouse_zoom_mesh
      ) {
        evnt.preventDefault();
      ...
      if (
        control.painter &&
        typeof control.painter.AnalyzeMouseWheelEvent == 'function'
      ) {
        ...
        if (kind !== 'z')
          position =
            (position + control.painter.size_xy3d) /
            2 /
            control.painter.size_xy3d;
    ...
    function control_mouseup(evnt) {
      if (
        control.mouse_zoom_mesh &&
        control.mouse_zoom_mesh.point2 &&
        control.painter.Get3DZoomCoord
      ) {
        var kind = control.mouse_zoom_mesh.object.zoom,
          pos1 = control.painter.Get3DZoomCoord(
            control.mouse_zoom_mesh.point,
            kind
          ),
          pos2 = control.painter.Get3DZoomCoord(
            control.mouse_zoom_mesh.point2,
            kind
          );
      ...
      if (control.enable_zoom) control.RemoveZoomMesh();
      ...
      if (control.enable_select && control.mouse_select_pnt) {
        ...
        var same_pnt =
          pnt.x == control.mouse_select_pnt.x &&
          pnt.y == control.mouse_select_pnt.y;
  ...
  JSROOT.Painter.createLineSegments = function (
    arr,
    material,
    index,
    only_geometry
  ...
  InteractiveControl.prototype.extractIndex = function (intersect) {
    return undefined;
  };
  ...
  InteractiveControl.prototype.checkHighlightIndex = function (indx) {
    return undefined;
  };
  ...
  PointsControl.prototype.extractIndex = function (intersect) {
    return intersect && intersect.index !== undefined
      ? intersect.index
      : undefined;
  };
  ...
  PointsControl.prototype.setHighlight = function (col, indx) {
    var m = this.mesh;
    m.h_index = indx;
    if (col) this.createSpecial(col, indx);
    else this.createSpecial(m.select_col, m.select_indx);
    return true;
  };
  ...
  PointsCreator.prototype.Complete = function (arg) {
    ...
    if (this.texture) {
      if (arg == 'loaded' && this.texture.onUpdate)
        this.texture.onUpdate(this.texture);
      if (this._did_create) return;
      material = new THREE.PointsMaterial({
        size: (this.webgl ? 3 : 1) * this.scale,
        map: this.texture,
        transparent: true,
      });
    } else {
      if (this._did_create) return;
      material = new THREE.PointsMaterial({
        size: (this.webgl ? 3 : 1) * this.scale * this.k,
        color: this.color,
      });
    }
  ...
  PointsCreator.prototype.CreatePoints = function (args) {
    ...
    // special dots
    if (args.style === 1) this.k = 0.3;
    else if (args.style === 6) this.k = 0.5;
    else if (args.style === 7) this.k = 0.7;

    // this is plain creation of points, no texture loading
    if (!args.style || this.k !== 1 || JSROOT.BatchMode) return this.Complete();

    var handler = new JSROOT.TAttMarkerHandler({
      style: args.style,
      color: args.color,
      size: 8,
    });
    ...
    var plainSVG =
      ...
      '<path d="' +
      handler.create(32, 32) +
      '" stroke="' +
      handler.getStrokeColor() +
      '" fill="' +
      handler.getFillColor() +
      '"/></svg>';
    ...
    if (!this.callback) return this.Complete();
  };
  ...
  function Create3DLineMaterial(painter, obj) {
    ...
    var lcolor = painter.get_color(obj.fLineColor),
      material = null,
      style = obj.fLineStyle
        ? JSROOT.Painter.root_line_styles[obj.fLineStyle]
        : '',
    ...
    if (dash && dash.length >= 2)
      material = new THREE.LineDashedMaterial({
        color: lcolor,
        dashSize: parseInt(dash[0]),
        gapSize: parseInt(dash[1]),
      });
    else material = new THREE.LineBasicMaterial({ color: lcolor });

    if (obj.fLineWidth && obj.fLineWidth > 1 && !JSROOT.browser.isIE)
      material.linewidth = obj.fLineWidth;
  ...
  function drawPolyLine3D() {
    ...
    var lines = JSROOT.Painter.createLineSegments(
      pnts,
      Create3DLineMaterial(this, line)
    );
