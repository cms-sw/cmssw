...
(function (factory) {
  if (typeof define === 'function' && define.amd) {
    ...
    factory(
      jsroot,
      require('d3'),
      require('./JSRootPainter.hist.js'),
      require('three'),
      require('./three.extra.min.js'),
  ...
  JSROOT.TFramePainter.prototype.SetCameraPosition = function (
    pad,
    first_time
  ) {
    ...
    if (pad && (first_time || !this.zoom_changed_interactive))
      if (
        !isNaN(pad.fTheta) &&
        !isNaN(pad.fPhi) &&
        (pad.fTheta !== this.camera_Theta || pad.fPhi !== this.camera_Phi)
      ) {
        max3d = 3 * Math.max(this.size_xy3d, this.size_z3d);
        var phi = ((-pad.fPhi - 90) / 180) * Math.PI,
          theta = (pad.fTheta / 180) * Math.PI;

        this.camera_Phi = pad.fPhi;
        this.camera_Theta = pad.fTheta;

        this.camera.position.set(
          max3d * Math.cos(phi) * Math.cos(theta),
    ...
    if (first_time) this.camera.lookAt(this.lookat);
  };
  ...
  JSROOT.TFramePainter.prototype.Create3DScene = function (arg) {
    ...
    this.size_z3d = 100;
    this.size_xy3d =
      sz.height > 10 && sz.width > 10
        ? Math.round((sz.width / sz.height) * this.size_z3d)
        : this.size_z3d;
    ...
    var res = JSROOT.Painter.Create3DRenderer(
      this.scene_width,
      this.scene_height,
      this.usesvg,
      sz.can3d == 4
    ...
    this.control = JSROOT.Painter.CreateOrbitControl(
      this,
      this.camera,
      this.scene,
      this.renderer,
    ...
    var axis_painter = this,
      obj_painter = this.main_painter();
    ...
    this.control.ProcessMouseMove = function (intersects) {
      ...
      if (tip && !tip.use_itself) {
        var delta_xy = 1e-4 * axis_painter.size_xy3d,
      ...
      if (!tip && zoom_mesh && axis_painter.Get3DZoomCoord) {
        ...
        if (taxis) {
          hint.name = taxis.fName;
          hint.title = taxis.fTitle || 'histogram TAxis object';
        }

        hint.line =
          axis_name + ' : ' + axis_painter.AxisAsText(axis_name, axis_value);
    ...
    this.control.ContextMenu = function (pos, intersects) {
      ...
      if (intersects)
        for (var n = 0; n < intersects.length; ++n) {
          ...
          if (
            mesh.painter &&
            typeof mesh.painter.ShowContextMenu === 'function'
          ) {
            p = mesh.painter;
            break;
          }
  ...
  JSROOT.TFramePainter.prototype.SetActive = function (on) {
    if (this.control) this.control.enableKeys = on;
  };
  ...
  JSROOT.TFramePainter.prototype.Render3D = function (tmout) {
    ...
    if (tmout <= 0 || this.usesvg || JSROOT.BatchMode) {
      ...
      if (
        this.first_render_tm === 0 &&
        this.renderer instanceof THREE.SoftwareRenderer
      )
        this.renderer.render(this.scene, this.camera);
      ...
      if (this.first_render_tm === 0) {
        ...
        this.enable_highlight =
          this.first_render_tm < 1200 && this.IsTooltipAllowed();
  ...
  JSROOT.TFramePainter.prototype.BinHighlight3D = function (tip, selfmesh) {
    ...
    if (
      changed &&
      tip.$painter &&
      typeof tip.$painter.RedrawProjection == 'function'
    )
      tip.$painter.RedrawProjection(tip.ix - 1, tip.ix, tip.iy - 1, tip.iy);

    if (this.GetObject())
      this.ProvideUserTooltip({
        obj: this.GetObject(),
  ...
  JSROOT.TFramePainter.prototype.TestAxisVisibility = function (
    camera,
    toplevel,
    fb,
    bb
    ...
    for (var n = 0; n < top.children.length; ++n) {
      ...
      if (chld.grid) chld.visible = bb && testvisible(chld.grid, 3);
      else if (chld.zid) chld.visible = testvisible(chld.zid, 2);
      else if (chld.xyid) chld.visible = testvisible(chld.xyid, 3);
        ...
        if (bb && !fb) {
          range = 3;
          shift = -2;
        } else if (fb && !bb) range = 3;
        else if (!fb && !bb) range = chld.bottom ? 3 : 0;
  ...
  JSROOT.TFramePainter.prototype.DrawXYZ = function (toplevel, opts) {
    ...
    var grminx = -this.size_xy3d,
      grmaxx = this.size_xy3d,
      grminy = -this.size_xy3d,
      grmaxy = this.size_xy3d,
      grminz = 0,
      grmaxz = 2 * this.size_z3d,
    ...
    this.scale_ymin = ymin;
    this.scale_ymax = ymax;
    ...
    if (pad && pad.fLogz) {
      ...
      this.z_kind = 'normal';
      if (this.zaxis && this.zaxis.fLabels && opts.ndim === 3)
        this.z_kind = 'labels';
    }
    ...
    if (xaxis && xaxis.fTitle) {
      var text3d = new THREE.TextGeometry(xaxis.fTitle, {
        font: JSROOT.threejs_font_helvetiker_regular,
        size: textsize,
        height: 0,
        curveSegments: 5,
    ...
    this.Get3DZoomCoord = function (point, kind) {
      ...
      var pos = point[kind],
        min = this['scale_' + kind + 'min'],
        max = this['scale_' + kind + 'max'];
    ...
    function CreateZoomMesh(kind, size_3d, use_y_for_z) {
      ...
      mesh.GlobalIntersect = function (raycaster) {
        ...
        if (pnt[this.zoom] < min) pnt[this.zoom] = min;
        else if (pnt[this.zoom] > max) pnt[this.zoom] = max;
      ...
      mesh.ShowSelection = function (pnt1, pnt2) {
        ...
        var tgtmesh = this.children ? this.children[0] : null,
          gg,
          kind = this.zoom;
        if (!pnt1 || !pnt2) {
          if (tgtmesh) {
    ...
    lbls.forEach(function (lbl) {
      ...
      m.set(
        ...
        text_scale,
        0,
        (-maxtextheight * text_scale - 1.5 * ticklen) * (lbl.gry || 1),
        0,
        0,
        1,
        0,
    ...
    if (yaxis && yaxis.fTitle) {
      var text3d = new THREE.TextGeometry(yaxis.fTitle, {
        font: JSROOT.threejs_font_helvetiker_regular,
        size: textsize,
        height: 0,
        curveSegments: 5,
    ...
    if (!opts.use_y_for_z) {
      ...
      lbls.forEach(function (lbl) {
        ...
        m.set(
          ...
          text_scale,
          0,
          (-maxtextheight * text_scale - 1.5 * ticklen) * (lbl.grx || 1),
          -text_scale,
          0,
          0,
          posy,
    ...
    while (zticks.next()) {
      ...
      if (is_major && lbl) {
        ...
        if (lastmajorz !== null && draw_height > 0)
          text_scale = Math.min(
            text_scale,
            (0.9 * (grz - lastmajorz)) / draw_height
          );
      ...
      // create grid
      if (zgridx && is_major) zgridx.push(grminx, 0, grz, grmaxx, 0, grz);

      if (zgridy && is_major) zgridy.push(0, grminy, grz, 0, grmaxy, grz);
    ...
    var zcont = [],
      zticksline = JSROOT.Painter.createLineSegments(ticks, lineMaterial);
    for (var n = 0; n < 4; ++n) {
      ...
      if (zaxis && zaxis.fTitle) {
        var text3d = new THREE.TextGeometry(zaxis.fTitle, {
          font: JSROOT.threejs_font_helvetiker_regular,
          size: textsize,
          height: 0,
          curveSegments: 5,
        ...
        var draw_width = text3d.boundingBox.max.x - text3d.boundingBox.min.x,
          draw_height = text3d.boundingBox.max.y - text3d.boundingBox.min.y,
          posz = zaxis.TestBit(JSROOT.EAxisBits.kCenterTitle)
            ? (grmaxz + grminz - draw_width) / 2
            : grmaxz - draw_width;
    ...
    var linex_geom = JSROOT.Painter.createLineSegments(
      [grminx, 0, 0, grmaxx, 0, 0],
      lineMaterial,
      null,
      true
    );
    ...
    var liney_geom = JSROOT.Painter.createLineSegments(
      [0, grminy, 0, 0, grmaxy, 0],
      lineMaterial,
      null,
      true
    );
    ...
    var linez_geom = JSROOT.Painter.createLineSegments(
      [0, 0, grminz, 0, 0, grmaxz],
      lineMaterial,
      null,
      true
    );
  ...
  JSROOT.THistPainter.prototype.Draw3DBins = function () {
    ...
    if (this.IsTH2Poly() && this.DrawPolyLego) return this.DrawPolyLego();
    ...
    function GetBinContent(ii, jj, level) {
      ...
      if (basehisto) binz1 = basehisto.getBinContent(ii + 1, jj + 1);
      else if (pthis.options.BaseLine !== false) binz1 = pthis.options.BaseLine;
      else binz1 = pthis.options.Zero ? axis_zmin : 0;
      if (binz2 < binz1) {
        var d = binz1;
        binz1 = binz2;
        binz2 = d;
    ...
    if (this.options.Lego === 12 || this.options.Lego === 14) {
      ...
      levels = this.CreateContour(
        histo.fContour ? histo.fContour.length : 20,
        main.lego_zmin,
        main.lego_zmax
      );
    ...
    for (var nlevel = 0; nlevel < levels.length - 1; ++nlevel) {
      ...
      var grzmin = main.grz(zmin),
        grzmax = main.grz(zmax);
      ...
      var positions = new Float32Array(numvertices * 3),
        ...
        face_to_bins_index = use16indx
          ? new Uint16Array(numvertices / 3)
          : new Uint32Array(numvertices / 3),
        pos2 = null,
        norm2 = null,
        face_to_bins_indx2 = null,
        v = 0,
        v2 = 0,
        vert,
        bin,
      ...
      if (num2vertices > 0) {
        ...
        face_to_bins_indx2 = use16indx
          ? new Uint16Array(num2vertices / 3)
          : new Uint32Array(num2vertices / 3);
      }
      ...
      for (i = i1; i < i2; ++i) {
        ...
        for (j = j1; j < j2; ++j) {
          ...
          y1 =
            handle.gry[j] + handle.ybar1 * (handle.gry[j + 1] - handle.gry[j]);
          y2 =
            handle.gry[j] + handle.ybar2 * (handle.gry[j + 1] - handle.gry[j]);
          ...
          var size = indicies.length,
            bin_index = histo.getBin(i + 1, j + 1);
          if (nobottom) size -= 6;
      ...
      mesh.tooltip = function (intersect) {
        ...
        if (
          ...
          intersect.faceIndex >= this.face_to_bins_index.length
        )
          return null;
        ...
        tip.x1 = Math.max(
          -main.size_xy3d,
          handle.grx[tip.ix - 1] +
            handle.xbar1 * (handle.grx[tip.ix] - handle.grx[tip.ix - 1])
        );
        tip.x2 = Math.min(
          main.size_xy3d,
          handle.grx[tip.ix - 1] +
            handle.xbar2 * (handle.grx[tip.ix] - handle.grx[tip.ix - 1])
        );

        tip.y1 = Math.max(
          -main.size_xy3d,
          handle.gry[tip.iy - 1] +
            handle.ybar1 * (handle.gry[tip.iy] - handle.gry[tip.iy - 1])
        );
        tip.y2 = Math.min(
          main.size_xy3d,
          handle.gry[tip.iy - 1] +
            handle.ybar2 * (handle.gry[tip.iy] - handle.gry[tip.iy - 1])
        );

        var binz1 = this.baseline,
          binz2 = tip.value;
        if (histo['$baseh'])
    ...
    var line = JSROOT.Painter.createLineSegments(
      lpositions,
      material,
      uselineindx ? lindicies : null
    );
  ...
  JSROOT.Painter.drawAxis3D = function (divid, axis, opt) {
    ...
    if (!('_main' in axis)) painter.SetDivId(divid);
  ...
  JSROOT.TH1Painter.prototype.Draw3D = function (call_back, resize) {
    ...
    if (resize) {
      ...
      if (is_main) {
        ...
        main.DrawXYZ(main.toplevel, {
          use_y_for_z: true,
          zmult: 1.1,
          zoom: JSROOT.gStyle.Zooming,
          ndim: 1,
        });
      }
    ...
    if (is_main) {
      ...
      this.DrawColorPalette(
        this.options.Zscale &&
          (this.options.Lego === 12 || this.options.Lego === 14)
      );
  ...
  JSROOT.TH2Painter.prototype.DrawContour3D = function (realz) {
    ...
    var lines = JSROOT.Painter.createLineSegments(
      pnts,
      JSROOT.Painter.Create3DLineMaterial(this, histo)
    );
  ...
  JSROOT.TH2Painter.prototype.DrawSurf = function () {
    ...
    switch (this.options.Surf) {
      ...
      case 16:
        ...
        break;
      default:
        ilevels = main.z_handle.CreateTicks(true);
        dogrid = true;
        break;
    }
    ...
    var loop,
      nfaces = [],
      pos = [],
      indx = [], // buffers for faces
      nsegments = 0,
      lpos = null,
      lindx = 0, // buffer for lines
      ngridsegments = 0,
      grid = null,
      gindx = 0, // buffer for grid lines segments
    ...
    var pntbuf = new Float32Array(6 * 3),
      k = 0,
      lastpart = 0; // maximal 6 points
    var gridpnts = new Float32Array(2 * 3),
      gridcnt = 0;
    ...
    function RememberVertex(indx, ii, jj) {
      var bin =
        ((ii - handle.i1) * (handle.j2 - handle.j1) + (jj - handle.j1)) * 8;
    ...
    function AddMainTriangle(x1, y1, z1, x2, y2, z2, x3, y3, z3, is_first) {
      for (var lvl = 1; lvl < levels.length; ++lvl) {
        ...
        if (!loop) {
          var npnts =
            Math.abs(side2 - side1) +
            Math.abs(side3 - side2) +
            Math.abs(side1 - side3);
          if (side1 === 0) ++npnts;
          if (side2 === 0) ++npnts;
          if (side3 === 0) ++npnts;
        ...
        if (k === 0) continue;
        if (k < 9) {
          console.log('found less than 3 points', k / 3);
          continue;
        }

        if (grid && gridcnt === 6) {
          for (var jj = 0; jj < 6; ++jj) grid[gindx + jj] = gridpnts[jj];
          gindx += 6;
        }
    ...
    if (donormals) {
      ...
      normindx = new Int32Array(
        (handle.i2 - handle.i1) * (handle.j2 - handle.j1) * 8
      );
    ...
    for (loop = 0; loop < 2; ++loop) {
      if (loop) {
        ...
        if (dolines && nsegments > 0) lpos = new Float32Array(nsegments * 6);
    ...
    for (var lvl = 1; lvl < levels.length; ++lvl)
      if (pos[lvl]) {
        ...
        geometry.computeVertexNormals();
        if (donormals && lvl === 1)
          RecalculateNormals(geometry.getAttribute('normal').array);
        ...
        if (palette) {
          ...
          fcolor =
            histo.fFillColor > 1 ? this.get_color(histo.fFillColor) : 'white';
          if (this.options.Surf === 14 && histo.fFillColor < 2)
            fcolor = this.get_color(48);
        }
    ...
    if (grid) {
      ...
      if (this.options.Surf === 1)
        ...
        material = new THREE.LineBasicMaterial({
          color: new THREE.Color(this.get_color(histo.fLineColor)),
        });
    ...
    if (this.options.Surf === 17) this.DrawContour3D();
  ...
  JSROOT.TH2Painter.prototype.DrawError = function () {
    ...
    line.tooltip = function (intersect) {
      ...
      tip.x1 = Math.max(
        -main.size_xy3d,
        main.grx(histo.fXaxis.GetBinLowEdge(tip.ix))
      );
      tip.x2 = Math.min(
        main.size_xy3d,
        main.grx(histo.fXaxis.GetBinLowEdge(tip.ix + 1))
      );
      tip.y1 = Math.max(
        -main.size_xy3d,
        main.gry(histo.fYaxis.GetBinLowEdge(tip.iy))
      );
      tip.y2 = Math.min(
        main.size_xy3d,
        main.gry(histo.fXaxis.GetBinLowEdge(tip.iy + 1))
      );

      tip.z1 = main.grz(
        tip.value - tip.error < this.zmin ? this.zmin : tip.value - tip.error
      );
      tip.z2 = main.grz(
        tip.value + tip.error > this.zmax ? this.zmax : tip.value + tip.error
      );
    ...
    for (i = 0; i < len; ++i) {
      ...
      for (var ngr = 0; ngr < ngraphs; ++ngr) {
        ...
        var npnts = gr.fNpoints,
          x = gr.fX,
          y = gr.fY;
        while (npnts > 2 && x[0] === x[npnts - 1] && y[0] === y[npnts - 1])
          --npnts;
        ...
        for (var ntry = 0; ntry < 2; ++ntry) {
          ...
          for (var vert = 0; vert < npnts; ++vert) {
            ...
            if (vert > 0)
              dist2 =
                (currx - lastx) * (currx - lastx) +
                (curry - lasty) * (curry - lasty);
            if (dist2 > dist2limit) {
  ...
  TH3Painter.prototype.CountStat = function () {
    ...
    if (stat_sum0 > 0) {
      ...
      res.rmsz = Math.sqrt(
        Math.abs(stat_sumz2 / stat_sum0 - res.meanz * res.meanz)
      );
    }
  ...
  TH3Painter.prototype.GetBinTips = function (ix, iy, iz) {
    ...
    if (pmain.x_kind == 'labels')
      lines.push(
        'x = ' +
          pmain.AxisAsText('x', histo.fXaxis.GetBinLowEdge(ix + 1)) +
          '  xbin=' +
          (ix + 1)
      );
    else
      lines.push(
        'x = [' +
          pmain.AxisAsText('x', histo.fXaxis.GetBinLowEdge(ix + 1)) +
          ', ' +
          pmain.AxisAsText('x', histo.fXaxis.GetBinLowEdge(ix + 2)) +
          ')   xbin=' +
          (ix + 1)
      );
    ...
    if (pmain.y_kind == 'labels')
      lines.push(
        'y = ' +
          pmain.AxisAsText('y', histo.fYaxis.GetBinLowEdge(iy + 1)) +
          '  ybin=' +
          (iy + 1)
      );
    else
      lines.push(
        'y = [' +
          pmain.AxisAsText('y', histo.fYaxis.GetBinLowEdge(iy + 1)) +
          ', ' +
          pmain.AxisAsText('y', histo.fYaxis.GetBinLowEdge(iy + 2)) +
          ')  ybin=' +
          (iy + 1)
      );
    ...
    if (pmain.z_kind == 'labels')
      lines.push(
        'z = ' +
          pmain.AxisAsText('z', histo.fZaxis.GetBinLowEdge(iz + 1)) +
          '  zbin=' +
          (iz + 1)
      );
    else
      lines.push(
        'z = [' +
          pmain.AxisAsText('z', histo.fZaxis.GetBinLowEdge(iz + 1)) +
          ', ' +
          pmain.AxisAsText('z', histo.fZaxis.GetBinLowEdge(iz + 2)) +
          ')  zbin=' +
          (iz + 1)
      );

    var binz = histo.getBinContent(ix + 1, iy + 1, iz + 1);
    if (binz === Math.round(binz)) lines.push('entries = ' + binz);
    else
      lines.push(
  ...
  TH3Painter.prototype.Draw3DScatter = function () {
    ...
    var pnts = new JSROOT.Painter.PointsCreator(
        numpixels,
        main.webgl,
        main.size_xy3d / 200
      ),
  ...
  TH3Painter.prototype.Draw3DBins = function () {
    ...
    if (this.options.GLBox === 11 || this.options.GLBox === 12) {
      ...
      var geom = JSROOT.Painter.TestWebGL()
    ...
    if (use_scale)
      use_scale =
        this.gminbin || this.gmaxbin
          ? 1 / Math.max(Math.abs(this.gminbin), Math.abs(this.gmaxbin))
          : 1;
    ...
    var scalex =
        (main.grx(histo.fXaxis.GetBinLowEdge(i2 + 1)) -
          main.grx(histo.fXaxis.GetBinLowEdge(i1 + 1))) /
        (i2 - i1),
      scaley =
        (main.gry(histo.fYaxis.GetBinLowEdge(j2 + 1)) -
          main.gry(histo.fYaxis.GetBinLowEdge(j1 + 1))) /
        (j2 - j1),
      scalez =
        (main.grz(histo.fZaxis.GetBinLowEdge(k2 + 1)) -
          main.grz(histo.fZaxis.GetBinLowEdge(k1 + 1))) /
        (k2 - k1);
    ...
    for (i = i1; i < i2; ++i) {
      for (j = j1; j < j2; ++j) {
        for (k = k1; k < k2; ++k) {
          ...
          wei = use_scale
            ? Math.pow(Math.abs(bin_content * use_scale), 0.3333)
            : 1;
    ...
    for (var ncol = 0; ncol < cols_size.length; ++ncol) {
      ...
      if (helper_kind[nseq] === 1)
        helper_indexes[nseq] = new Uint16Array(
          nbins * JSROOT.Painter.Box3D.MeshSegments.length
        );

      if (helper_kind[nseq] === 2)
        helper_positions[nseq] = new Float32Array(
          nbins * JSROOT.Painter.Box3D.Segments.length * 3
        );
    }
    ...
    for (i = i1; i < i2; ++i) {
      binx = histo.fXaxis.GetBinCenter(i + 1);
      grx = main.grx(binx);
      for (j = j1; j < j2; ++j) {
        biny = histo.fYaxis.GetBinCenter(j + 1);
        gry = main.gry(biny);
        for (k = k1; k < k2; ++k) {
          bin_content = histo.getBinContent(i + 1, j + 1, k + 1);
          if (bin_content === 0 || bin_content < this.gminbin) continue;
          ...
          binz = histo.fZaxis.GetBinCenter(k + 1);
          grz = main.grz(binz);
          ...
          var vvv = nbins * buffer_size,
            bin_v = bin_verts[nseq],
            bin_n = bin_norms[nseq];
    ...
    for (var ncol = 0; ncol < cols_size.length; ++ncol) {
      ...
      all_bins_buffgeom.addAttribute(
        'position',
      ...
      var material = use_lambert
        ? new THREE.MeshLambertMaterial({
            color: fillcolor,
            opacity: use_opacity,
            transparent: use_opacity < 1,
          })
        : new THREE.MeshBasicMaterial({
            color: fillcolor,
            opacity: use_opacity,
      ...
      combined_bins.tooltip = function (intersect) {
        ...
        var p = this.painter,
          ...
          wei = this.use_scale
            ? Math.pow(Math.abs(tip.value * this.use_scale), 0.3333)
            : 1;

        tip.x1 = grx - this.scalex * wei;
        tip.x2 = grx + this.scalex * wei;
        tip.y1 = gry - this.scaley * wei;
        tip.y2 = gry + this.scaley * wei;
        tip.z1 = grz - this.scalez * wei;
        tip.z2 = grz + this.scalez * wei;
      ...
      if (helper_kind[nseq] > 0) {
        ...
        if (helper_kind[nseq] === 1) {
          ...
          lines = JSROOT.Painter.createLineSegments(
            bin_verts[nseq],
            helper_material,
            helper_indexes[nseq]
          );
  ...
  TGraph2DPainter.prototype.Graph2DTooltip = function (intersect) {
    ...
    if (this.check_next && indx + 1 < this.graph.fX.length) {
      ...
      if (
        sqr(d.x - grx1) + sqr(d.y - gry1) + sqr(d.z - grz1) <
        sqr(d.x - grx) + sqr(d.y - gry) + sqr(d.z - grz)
      ) {
        grx = grx1;
        gry = gry1;
        grz = grz1;
  ...
  TGraph2DPainter.prototype.Redraw = function () {
    ...
    function CountSelected(zmin, zmax) {
      ...
      for (var i = 0; i < graph.fNpoints; ++i) {
        if (
          graph.fX[i] < fp.scale_xmin ||
          graph.fX[i] > fp.scale_xmax ||
          graph.fY[i] < fp.scale_ymin ||
    ...
    for (var lvl = 0; lvl < levels.length - 1; ++lvl) {
      ...
      if (this.options.Error) err = new Float32Array(size * 6 * 3);

      if (this.options.Line) line = new Float32Array((size - 1) * 6);
      ...
      if (line && iline > 3 && line.length == iline) {
        ...
        if (!JSROOT.browser.isIE)
          material.linewidth = this.GetObject().fLineWidth;
      ...
      if (!pnts) {
        ...
        if (!this.options.Circles)
          fcolor = palette
            ? palette.calcColor(lvl, levels.length)
            : this.get_color(graph.fMarkerColor);

        pnts.AssignCallback(
          this.PointsCallback.bind(this, {
            graph: graph,
            index: index,
            painter: fp,
            scale0: 0.3 * scale,
        ...
        pnts.CreatePoints({
          color: fcolor,
          style: this.options.Circles ? 4 : graph.fMarkerStyle,
        });
      }
    }
  ...
  JSROOT.Painter.drawGraph2D = function (divid, gr, opt) {
    ...
    if (!gr.fHistogram) gr.fHistogram = painter.CreateHistogram();
  ...
  JSROOT.Painter.drawPolyMarker3D = function (divid, poly, opt) {
    ...
    painter.Redraw = function () {
      ...
      for (var i = 0; i < poly.fP.length; i += 3) {
        if (
          poly.fP[i] < fp.scale_xmin ||
          poly.fP[i] > fp.scale_xmax ||
          poly.fP[i + 1] < fp.scale_ymin ||
          poly.fP[i + 1] > fp.scale_ymax ||
          poly.fP[i + 2] < fp.scale_zmin ||
          poly.fP[i + 2] > fp.scale_zmax
        )
          continue;
        ++numselect;
      }
      ...
      var size = Math.floor(numselect / step),
        pnts = new JSROOT.Painter.PointsCreator(
          size,
          fp.webgl,
          fp.size_xy3d / 100
        ),
      ...
      pnts.CreatePoints({
        color: this.get_color(poly.fMarkerColor),
