...
(function (factory) {
  ...
  TAxisPainter.prototype.DrawAxis = function (
    ...
    if (!disable_axis_drawing && !optionUnlab) {
      var label_color = this.get_color(axis.fLabelColor),
        labeloffset = Math.round(
          Math.abs(axis.fLabelOffset) * text_scaling_size
        ),
    ...
    if (axis.fTitle.length > 0 && !disable_axis_drawing) {
      var title_g = axis_g.append('svg:g').attr('class', 'axis_title'),
        ...
        title_color = this.get_color(
          is_gaxis ? axis.fTextColor : axis.fTitleColor
        ),
        shift_x = 0,
        shift_y = 0;
  ...
  TFramePainter.prototype.SetAxesRanges = function (
    xaxis,
    xmin,
    xmax,
    yaxis,
  ...
  TFramePainter.prototype.CheckAxisZoom = function (name) {
    ...
    if (axis && axis.TestBit(JSROOT.EAxisBits.kAxisRange)) {
      if (
        axis.fFirst !== axis.fLast &&
        (axis.fFirst > 1 || axis.fLast < axis.fNbins)
      ) {
        this['zoom_' + name + 'min'] =
          axis.fFirst > 1 ? axis.GetBinLowEdge(axis.fFirst) : axis.fXmin;
        this['zoom_' + name + 'max'] =
          axis.fLast < axis.fNbins
            ? axis.GetBinLowEdge(axis.fLast + 1)
            : axis.fXmax;
  ...
  TFramePainter.prototype.CheckPadUserRange = function (pad, name) {
    ...
    if (
      Math.abs(umin - this[smin]) > eps ||
      Math.abs(umax - this[smax]) > eps
    ) {
  ...
  TFramePainter.prototype.CreateXY = function (opts) {
    ...
    if (opts.extra_y_space) {
      ...
      else this.scale_ymax += (this.scale_ymax - this.scale_ymin) * 0.1;
    }
    ...
    if (this.x_kind == 'time') {
      ...
      if (this.scale_xmin <= 0 && this.xaxis && !this.swap_xy)
        for (var i = 0; i < this.xaxis.fNbins; ++i) {
          this.scale_xmin = Math.max(
            this.scale_xmin,
            this.xaxis.GetBinLowEdge(i + 1)
          );
    ...
    if (this.swap_xy ? pad.fLogx : pad.fLogy) {
      ...
      if (this.scale_ymin <= 0 && this.yaxis && opts.ndim > 1 && !this.swap_xy)
        for (var i = 0; i < this.yaxis.fNbins; ++i) {
          this.scale_ymin = Math.max(
            this.scale_ymin,
            this.yaxis.GetBinLowEdge(i + 1)
          );
  ...
  TFramePainter.prototype.DrawGrids = function () {
    ...
    if (pad && pad.fGridx && this.x_handle) {
      ...
      for (var n = 0; n < this.x_handle.ticks.length; ++n)
        if (this.swap_xy) gridx += 'M0,' + this.x_handle.ticks[n] + 'h' + w;
        else gridx += 'M' + this.x_handle.ticks[n] + ',0v' + h;

      var colid =
          JSROOT.gStyle.fGridColor > 0
            ? JSROOT.gStyle.fGridColor
            : this.GetAxis('x')
            ? this.GetAxis('x').fAxisColor
            : 1,
    ...
    if (pad && pad.fGridy && this.y_handle) {
      ...
      for (var n = 0; n < this.y_handle.ticks.length; ++n)
        if (this.swap_xy) gridy += 'M' + this.y_handle.ticks[n] + ',0v' + h;
        else gridy += 'M0,' + this.y_handle.ticks[n] + 'h' + w;

      var colid =
          JSROOT.gStyle.fGridColor > 0
            ? JSROOT.gStyle.fGridColor
            : this.GetAxis('y')
            ? this.GetAxis('y').fAxisColor
            : 1,
        grid_color = this.get_color(colid) || 'black';
  ...
  TFramePainter.prototype.AxisAsText = function (axis, value) {
    if (axis == 'x') {
      ...
      if (this.y_handle && 'format' in this.y_handle)
        return this.y_handle.format(
          value,
          false,
          false,
          JSROOT.gStyle.YValuesFormat
        );
    } else {
      if (this.z_handle && 'format' in this.z_handle)
        return this.z_handle.format(
          value,
          false,
          false,
          JSROOT.gStyle.ZValuesFormat
        );
    }
  ...
  TFramePainter.prototype.DrawAxes = function (
    shrink_forbidden,
    disable_axis_draw,
    AxisPos,
    has_x_obstacle
    ...
    this.x_handle.lbls_both_sides =
      !this.x_handle.invert_side && pad.fTickx > 1; // labels on both sides
    ...
    this.y_handle.lbls_both_sides =
      !this.y_handle.invert_side && pad.fTicky > 1; // labels on both sides
  ...
  TFramePainter.prototype.UpdateAttributes = function (force) {
    ...
    if (this.fillatt === undefined) {
      if (tframe) this.createAttFill({ attr: tframe });
        ...
        this.createAttFill({
          pattern: pad.fFrameFillStyle,
          color: pad.fFrameFillColor,
        });
    ...
    if (!tframe && pad && pad.fFrameLineColor !== undefined)
      this.createAttLine({
        color: pad.fFrameLineColor,
        width: pad.fFrameLineWidth,
        style: pad.fFrameLineStyle,
      });
      ...
      if (faxis) {
        ...
        this.AddColorMenuEntry(
          ...
          function (arg) {
            ...
            this.InteractiveRedraw(
              'pad',
              this.GetColorExec(parseInt(arg), 'SetLabelColor'),
              kind
            );
          }.bind(main || this)
        );
        ...
        this.AddColorMenuEntry(
          ...
          function (arg) {
            ...
            this.InteractiveRedraw(
              'pad',
              this.GetColorExec(parseInt(arg), 'SetTitleColor'),
              kind
            );
          }.bind(main || this)
        );
        ...
        if (faxis._typename == 'TGaxis') {
          this.AddColorMenuEntry(menu, 'Color', faxis.fLineColor, function (
            arg
          ) {
            faxis.fLineColor = parseInt(arg);
            this.RedrawPad();
          });
          this.AddSizeMenuEntry(
            menu,
            ...
            faxis.fTickSize,
            function (arg) {
              faxis.fTickSize = parseFloat(arg);
              this.RedrawPad();
            }
          );
        } else {
          this.AddColorMenuEntry(
            ...
            function (arg) {
              ...
              this.InteractiveRedraw(
                'pad',
                this.GetColorExec(parseInt(arg), 'SetAxisColor'),
                kind
              );
            }.bind(main || this)
          );
  ...
  TFramePainter.prototype.Zoom = function (xmin, xmax, ymin, ymax, zmin, zmax) {
    ...
    if (changed) this.InteractiveRedraw('pad', 'zoom');
  ...
  TFramePainter.prototype.ShowAxisStatus = function (axis_name) {
    ...
    var taxis = this.GetAxis(axis_name),
      hint_name = axis_name,
      hint_title = 'TAxis',
  ...
  TFramePainter.prototype.AddInteractive = function () {
    ...
    if (
      JSROOT.touches &&
      ((JSROOT.gStyle.Zooming && JSROOT.gStyle.ZoomTouch && !this.projection) ||
  ...
  function TPadPainter(pad, iscan) {
    ...
    if (!this.iscan && pad !== null && 'fName' in pad) {
      ...
      if (!regexp.test(this.this_pad_name))
        this.this_pad_name = 'jsroot_pad_' + JSROOT.id_counter++;
    }
  ...
  TPadPainter.prototype.CreateAutoColor = function () {
    ...
    if (pal) {
      ...
      var palindx = Math.round(
        (indx * (pal.getLength() - 3)) / (numprimitives - 1)
      );
  ...
  TPadPainter.prototype.DrawActiveBorder = function (svg_rect, is_active) {
    ...
    if (!svg_rect)
      svg_rect = this.iscan
        ? this.svg_canvas().select('.canvas_fillrect')
        : this.svg_pad(this.this_pad_name).select('.root_pad_border');

    var lineatt = this.is_active_pad
      ? new JSROOT.TAttLineHandler({ style: 1, width: 1, color: 'red' })
      : this.lineatt;
  ...
  TPadPainter.prototype.CheckSpecial = function (obj) {
    ...
    if (obj.name == 'ListOfColors') {
      if (this.options && this.options.CreatePalette) {
        var arr = [];
        for (
          var n = obj.arr.length - this.options.CreatePalette;
          n < obj.arr.length;
          ++n
        ) {
  ...
  TPadPainter.prototype.HasObjectsToDraw = function () {
    ...
    for (var n = 0; n < this.pad.fPrimitives.arr.length; ++n)
      if (
        this.pad.fPrimitives.arr[n] &&
        this.pad.fPrimitives.arr[n]._typename != 'TPad'
      )
        return true;
  ...
  TPadPainter.prototype.DrawPrimitives = function (indx, callback, ppainter) {
    if (indx === 0) {
      ...
      this._num_primitives =
        this.pad && this.pad.fPrimitives ? this.pad.fPrimitives.arr.length : 0;
    }
  ...
  TPadPainter.prototype.FillContextMenu = function (menu) {
    ...
    if (!this._websocket) {
      ...
      menu.addchk(
        this.pad.fGridx,
        'Grid x',
        (this.pad.fGridx ? '0' : '1') + 'fGridx',
        SetPadField
      );
      menu.addchk(
        this.pad.fGridy,
        'Grid y',
        (this.pad.fGridy ? '0' : '1') + 'fGridy',
        SetPadField
      );
    ...
    if (this.ActivateStatusBar)
      menu.addchk(
        this.HasEventStatus(),
        'Event status',
        this.ActivateStatusBar.bind(this, 'toggle')
      );
  ...
  TPadPainter.prototype.DrawNextSnap = function (
    ...
    while (true) {
      ...
      if (objpainter) {
        ...
        if (snap.fKind === JSROOT.WebSnapIds.kSubPad) {
          // subpad
      ...
      if (snap.fKind === JSROOT.WebSnapIds.kColors) {
        var ListOfColors = [],
          arr = snap.fSnapshot.fOper.split(';');
        for (var n = 0; n < arr.length; ++n) {
          var name = arr[n],
            p = name.indexOf(':');
          if (p > 0) {
            ListOfColors[parseInt(name.substr(0, p))] =
              'rgb(' + name.substr(p + 1) + ')';
          } else {
            p = name.indexOf('=');
            ListOfColors[parseInt(name.substr(0, p))] =
        ...
        // copy existing colors and extend with new values
        if (this.options && this.options.LocalColors)
          this.root_colors = JSROOT.Painter.extendRootColors(
            null,
            ListOfColors
          );
        ...
        if (
          snap.fSnapshot.fBuf &&
          (!this.options || !this.options.IgnorePalette)
        ) {
          var palette = [];
  ...
  TPadPainter.prototype.RedrawPadSnap = function (snap, call_back) {
    ...
    if (this.snapid === undefined) {
      ...
      if (this.batch_mode && (!first.fCw || !first.fCh)) {
        first.fCw = 900;
        first.fCh = 700;
      }
      ...
      if (!this.batch_mode) this.AddOnlineButtons();

      if (snap.fScripts && typeof snap.fScripts == 'string') {
        var arg = '';

        if (snap.fScripts.indexOf('load:') == 0) arg = snap.fScripts;
    ...
    function MatchPrimitive(painters, primitives, class_name, obj_name) {
      ...
      for (var k = 0; k < painters.length; ++k) {
        ...
        if (
          obj_name &&
          (!painters[k].GetObject() ||
            painters[k].GetObject().fName !== obj_name)
        )
          continue;
        painter = painters[k];
        break;
      ...
      for (var k = 0; k < primitives.length; ++k) {
        if (
          primitives[k].fKind !== 1 ||
          !primitives[k].fSnapshot ||
          primitives[k].fSnapshot._typename !== class_name
        )
          continue;
  ...
  TPadPainter.prototype.GetWebPadOptions = function (arg) {
    ...
    if (this.snapid) {
      elem = {
        ...
        logy: this.pad.fLogy,
        logz: this.pad.fLogz,
        gridx: this.pad.fGridx,
        gridy: this.pad.fGridy,
        tickx: this.pad.fTickx,
        ticky: this.pad.fTicky,
        mleft: this.pad.fLeftMargin,
        mright: this.pad.fRightMargin,
        mtop: this.pad.fTopMargin,
        mbottom: this.pad.fBottomMargin,
        zx1: 0,
        zx2: 0,
        zy1: 0,
        zy2: 0,
      ...
      if (this.GetPadRanges(elem)) arg.push(elem);
      else console.log('fail to get ranges for pad ' + this.pad.fName);
    }
    ...
    for (var k = 0; k < this.painters.length; ++k) {
      ...
      if (typeof sub.GetWebPadOptions == 'function') {
        ...
        var opt = {
          ...
          snapid: sub.snapid.toString(),
          opt: sub.OptionsAsString(),
          fcust: '',
          fopt: [],
        };
  ...
  TPadPainter.prototype.GetPadRanges = function (r) {
    ...
    if (main) {
      if (main.zoom_xmin !== main.zoom_xmax) {
        r.zx1 = main.zoom_xmin;
        r.zx2 = main.zoom_xmax;
      }

      if (main.zoom_ymin !== main.zoom_ymax) {
        r.zy1 = main.zoom_ymin;
        r.zy2 = main.zoom_ymax;
      }

      if (main.zoom_zmin !== main.zoom_zmax) {
        r.zz1 = main.zoom_zmin;
        r.zz2 = main.zoom_zmax;
      }
    }
    ...
    var same = function (x) {
      ...
      k =
        (func(main.scale_xmax) - func(main.scale_xmin)) /
        p.property('draw_width'),
    ...
    k =
      (func(main.scale_ymax) - func(main.scale_ymin)) /
      p.property('draw_height');
  ...
  TPadPainter.prototype.DrawingReady = function (res_painter) {
    ...
    if (main && main.mode3d && typeof main.Render3D == 'function')
      main.Render3D(-2222);
  ...
  TCanvasPainter.prototype.SendWebsocket = function (msg) {
    ...
    if (this._websocket.CanSend()) this._websocket.Send(msg);
    else console.warn('DROP SEND: ' + msg);
  };
  ...
  TCanvasPainter.prototype.ActivateStatusBar = function (state) {
    if (this.testUI5()) return;
    if (this.brlayout) this.brlayout.CreateStatusLine(23, state);
  ...
  TCanvasPainter.prototype.RemoveGed = function () {
    ...
    if (this.brlayout) this.brlayout.DeleteContent();

    this.ProcessChanges('sbits', this);
  };

  /** Function used to activate GED @private */
  TCanvasPainter.prototype.ActivateGed = function (
    objpainter,
    kind,
    mode,
    callback
  ) {
    if (this.testUI5() || !this.brlayout) return JSROOT.CallBack(callback);

    if (this.brlayout.HasContent()) {
      if (mode === 'toggle' || mode === false) this.RemoveGed();
      else this.SelectObjectPainter(objpainter);

      JSROOT.CallBack(callback, true);
    }

    if (mode === false) return JSROOT.CallBack(callback);
    ...
    JSROOT.ToolbarIcons.CreateSVG(
      ...
      .style('margin', '3px')
      .on('click', this.brlayout.Toggle.bind(this.brlayout, 'fix'));
    ...
    JSROOT.ToolbarIcons.CreateSVG(
      ...
      .style('margin', '3px')
      .on('click', this.brlayout.Toggle.bind(this.brlayout, 'float'));

    JSROOT.ToolbarIcons.CreateSVG(
      btns,
      JSROOT.ToolbarIcons.cross,
      15,
      'delete GED'
    )
      .style('margin', '3px')
      .on('click', this.RemoveGed.bind(this));
      ...
      sap.ui.define(
        ...
        function (JSONModel, XMLView) {
          ...
          XMLView.create({
            ...
            pthis.RegisterForPadEvents(
              oGed.getController().padEventsReceiver.bind(oGed.getController())
            );
  ...
  TCanvasPainter.prototype.ShowSection = function (that, on, callback) {
    if (this.testUI5()) return JSROOT.CallBack(callback);
    ...
    switch (that) {
      case 'Menu':
        break;
      case 'StatusBar':
        this.ActivateStatusBar(on);
        break;
      case 'Editor':
        this.ActivateGed(this, null, !!on, callback);
        callback = null;
        break;
      case 'ToolBar':
        break;
  ...
  TCanvasPainter.prototype.ProcessChanges = function (kind, painter, subelem) {
    ...
    switch (kind) {
      ...
      case 'zoom': // when changing zoom inside frame
        if (!painter.GetWebPadOptions) painter = painter.pad_painter();
        ...
        if (kind.substr(0, 5) == 'exec:' && painter && painter.snapid) {
          msg =
            ...
            JSROOT.toJSON({
              _typename: 'TWebObjectOptions',
              snapid:
                painter.snapid.toString() + (subelem ? '#' + subelem : ''),
              opt: kind.substr(5),
              fcust: 'exec',
              fopt: [],
            });
  ...
  TCanvasPainter.prototype.SelectActivePad = function (
    pad_painter,
    obj_painter,
    click_pos
  ) {
    ...
    if (pad_painter.snapid !== undefined && this._websocket)
      arg = {
        _typename: 'TWebPadClick',
        padid: pad_painter.snapid.toString(),
        objid: '',
        x: -1,
        y: -1,
        dbl: false,
  ...
  TCanvasPainter.prototype.GetStatusBits = function () {
    ...
    if (this.HasEventStatus())
      bits |= JSROOT.TCanvasStatusBits.kShowEventStatus;
  ...
  TCanvasPainter.prototype.ProduceJSON = function () {
    ...
    if (!this.normal_canvas) canv.fPrimitives.Clear();
  ...
  function drawCanvas(divid, can, opt) {
    ...
    if (!nocanvas && can.fCw && can.fCh && !JSROOT.BatchMode) {
      ...
      if (!rect0.height && rect0.width > 0.1 * can.fCw) {
        painter
          .select_main()
          .style('width', can.fCw + 'px')
          .style('height', can.fCh + 'px');
  ...
  function drawPadSnapshot(divid, snap, opt) {
    ...
    painter.RedrawPadSnap(snap, function () {
      painter.ShowButtons();
      painter.DrawingReady();
    });
