...
(function (factory) {
      ...
      draw_next: function () {
        while (++this.indx < this.lst.arr.length) {
          var handle = { func: this.draw_bind },
            ...
            opt =
              this.lst.opt && this.lst.opt[this.indx]
                ? this.lst.opt[this.indx]
                : this.opt;
          if (!item) continue;
    ...
    for (var i = 0; i < lst.arr.length; ++i) {
      ...
      if (!obj || !obj._typename) {
        ...
        switch (obj._typename) {
          case 'TColor':
            item._value = JSROOT.Painter.MakeColorRGB(obj);
            break;
          case 'TText':
        ...
        if (do_context && JSROOT.canDraw(obj._typename))
  ...
  function KeysHierarchy(folder, keys, file, dirname) {
    ...
    for (var i = 0; i < keys.length; ++i) {
      ...
      if ('fRealName' in key) item._realname = key.fRealName + ';' + key.fCycle;

      if (
        key.fClassName == 'TDirectory' ||
        key.fClassName == 'TDirectoryFile'
      ) {
        var dir = null;
        if (dirname != null && file != null)
          dir = file.GetDir(dirname + key.fName);
        if (dir == null) {
          item._more = true;
  ...
  function ObjectHierarchy(top, obj, args) {
    ...
    if (proto === '[object DataView]') {
      ...
      for (var k = 0; k < obj.byteLength; ++k) {
        ...
        if (item._value.length > 0) item._value += k % 4 === 0 ? ' | ' : ' ';
    ...
    var isarray =
        proto.lastIndexOf('Array]') == proto.length - 6 &&
        proto.indexOf('[object') == 0 &&
        !isNaN(obj.length),
      compress = isarray && obj.length > JSROOT.gStyle.HierarchyLimit,
      arrcompress = false;
    ...
    if (!('_obj' in top)) top._obj = obj;
    else if (top._obj !== obj) alert('object missmatch');

    if (!top._title) {
      if (obj._typename) top._title = 'ROOT.' + obj._typename;
      else if (isarray) top._title = 'Array len: ' + obj.length;
    }

    if (arrcompress) {
      for (var k = 0; k < obj.length; ) {
        var nextk = Math.min(k + 10, obj.length),
          allsame = true,
          prevk = k;
    ...
    for (var key in obj) {
      ...
      if (typeof fld == 'object') {
        ...
        if (
          ...
          if (inparent) {
            ...
            switch (fld._typename) {
              case 'TColor':
                item._value = JSROOT.Painter.MakeColorRGB(fld);
                break;
              case 'TText':
              ...
              default:
                if (
                  JSROOT.IsRootCollection(fld) &&
                  typeof fld.arr === 'object'
                ) {
        ...
        simple = true;
        if (key == 'fBits') item._value = '0x' + fld.toString(16);
        else item._value = fld.toString();
        item._vclass = 'h_value_num';
      ...
      if (!simple || !nosimple) top._childs.push(item);
    }
  ...
  BrowserLayout.prototype.CheckResize = function () {
    if (this.hpainter && typeof this.hpainter.CheckResize == 'function')
      ...
      this.objpainter &&
      typeof this.objpainter.CheckResize == 'function'
    ) {
  ...
  BrowserLayout.prototype.CreateBrowserBtns = function () {
    ...
    var btns = br
      .append('div')
      .classed('jsroot_browser_btns', true)
      .classed('jsroot', true);
  ...
  BrowserLayout.prototype.SetBrowserContent = function (guiCode) {
    ...
    main
      .insert('div', '.jsroot_browser_btns')
      ...
      .style('display', 'flex')
      .style('flex-direction', 'column') /* use the flex model */
  ...
  function HierarchyPainter(name, frameid, backgr) {
    ...
    if (!JSROOT.hpainter) JSROOT.hpainter = this;
  }
  ...
  HierarchyPainter.prototype.Cleanup = function () {
    ...
    if (JSROOT.hpainter === this) JSROOT.hpainter = null;
  };
  ...
  HierarchyPainter.prototype.FileHierarchy = function (file) {
    ...
    var folder = {
      ...
      _get: function (item, itemname, callback) {
        ...
        if (fff._file) ReadFileObject(fff._file);
        else if (fff._localfile)
          new JSROOT.TLocalFile(fff._localfile, ReadFileObject);
        else if (fff._fullurl) new JSROOT.TFile(fff._fullurl, ReadFileObject);
      },
    };
  ...
  HierarchyPainter.prototype.Find = function (arg) {
    function find_in_hierarchy(top, fullname) {
      ...
      while (pos > 0) {
        ...
        var localname =
          pos >= fullname.length ? fullname : fullname.substr(0, pos);
  ...
  HierarchyPainter.prototype.itemFullName = function (
    node,
    uptoparent,
    compact
  ) {
    ...
    if ('_numargs' in hitem)
      for (var n = 0; n < hitem._numargs; ++n) {
        ...
        if (!argvalue && typeof callback == 'object')
          argvalue = prompt(
            'Input argument ' + argname + ' for command ' + hitem._name,
            ''
          );
        if (!argvalue) return;
  ...
  HierarchyPainter.prototype.get = function (arg, call_back, options) {
    ...
    if (typeof arg === 'string') {
      ...
      if (
        arg._parent !== undefined &&
        arg._name !== undefined &&
        arg._kind !== undefined
      )
        item = arg;
      else if (arg.name !== undefined) itemname = arg.name;
      else if (arg.arg !== undefined) itemname = arg.arg;
      else if (arg.item !== undefined) item = arg.item;
    }
    ...
    if (item) itemname = this.itemFullName(item);
      ...
      item = this.Find({ name: itemname, allow_index: true, check_keys: true });
    ...
    if (d !== null && 'last' in d && d.last !== null) {
      ...
      return this.expand(
        ...
        function (res) {
          ...
          hpainter.get(
            { name: newparentname + d.rest, rest: d.rest },
            call_back,
            options
          );
        },
  ...
  HierarchyPainter.prototype.display = function (itemname, drawopt, call_back) {
    ...
    function display_callback(respainter) {
      ...
      if (
        respainter &&
        typeof respainter === 'object' &&
        typeof respainter.SetItemName === 'function'
      ) {
    ...
    h.CreateDisplay(function (mdi) {
      ...
      h.get(
        ...
        function (resitem, obj) {
          ...
          if (divid.length > 0)
            return (updating ? JSROOT.redraw : JSROOT.draw)(
              divid,
              obj,
              drawopt,
              display_callback
          ...
          mdi.ForEachPainter(function (p, frame) {
            ...
            if (!updating && drawopt != null && p.GetItemDrawOpt() != drawopt)
              return;
          ...
          if (updating) {
            JSROOT.console(
              'something went wrong - did not found painter when doing update of ' +
                display_itemname
            );
  ...
  HierarchyPainter.prototype.dropitem = function (
    itemname,
    divid,
    opt,
    call_back
    ...
    function drop_callback(drop_painter) {
      if (drop_painter && typeof drop_painter === 'object')
        drop_painter.SetItemName(itemname, null, h);
    ...
    h.get(itemname, function (item, obj) {
      ...
      if (main_painter && typeof main_painter.PerformDrop === 'function')
        return main_painter.PerformDrop(
          obj,
          itemname,
          item,
          opt,
  ...
  HierarchyPainter.prototype.updateItems = function (items) {
    ...
    if (draw_items.length > 0) this.displayAll(draw_items, draw_options);
  };
  ...
  HierarchyPainter.prototype.updateAll = function (
    only_auto_items,
    only_items
  ) {
    if (!this.disp) return;

    if (only_auto_items === 'monitoring')
    ...
    this.disp.ForEachPainter(function (p) {
      ...
      if (typeof itemname != 'string' || allitems.indexOf(itemname) >= 0)
        return;
    ...
    if (this.files_monitoring && !only_auto_items)
      this.ForEachRootFile(function (item) {
        painter.ForEach(function (fitem) {
          delete fitem._readobj;
        }, item);
        delete item._file;
      });

    if (allitems.length > 0) this.displayAll(allitems, options);
  };
  ...
  HierarchyPainter.prototype.displayAll = function (items, options, call_back) {
    ...
    if (options.length == 1 && options[0] == 'iotest') {
      ...
      return h.get(items[0], function (item, obj) {
        var tm1 = new Date();
        d3.select('#' + h.disp_frameid)
          .append('h2')
          .html(
            'Item ' +
              items[0] +
              ' reading time = ' +
              (tm1.getTime() - tm0.getTime()) +
              'ms'
          );
        return JSROOT.CallBack(call_back);
      });
      ...
      if (dropitems[i] && dropitems[i].length > 0) {
        ...
        for (var j = 0; j < dropitems[i].length; ++j) {
          ...
          if (pos > 0 && h.Find(dropitems[i][j]) === null)
            dropitems[i][j] =
              dropitems[i][j].substr(0, pos) + items[i].substr(pos);
    ...
    var frame_names = new Array(items.length),
      items_wait = new Array(items.length);
    for (var n = 0; n < items.length; ++n) {
      ...
      if (p >= 0) {
        ...
        while (frame_names.indexOf(fname) >= 0) fname = items[n] + '_' + k++;
      }
      frame_names[n] = fname;
    }
    ...
    h.CreateDisplay(function (mdi) {
      ...
      function DropNextItem(indx, painter) {
        if (painter && dropitems[indx] && dropitems[indx].length > 0)
          return h.dropitem(
            dropitems[indx].shift(),
            painter.divid,
            dropopts[indx].shift(),
            DropNextItem.bind(h, indx, painter)
          );
  ...
  HierarchyPainter.prototype.activate = function (items, force) {
    ...
    if (force && this.brlayout) {
      if (!this.brlayout.browser_kind)
        return this.CreateBrowser('float', true, find_next);
  ...
  HierarchyPainter.prototype.expand = function (
    itemname,
    call_back,
    d3cont,
    silent
    ...
    function DoExpandItem(_item, _obj, _name) {
      ...
      var handle = _item._expand
        ? null
        : JSROOT.getDrawHandle(_item._kind, '::expand');
  ...
  HierarchyPainter.prototype.GetTopOnlineItem = function (item) {
    ...
    if (this.h._childs && '_online' in this.h._childs[0])
  ...
  HierarchyPainter.prototype.ForEachJsonFile = function (call_back) {
    if (!this.h) return;
    if ('_jsonfile' in this.h) return JSROOT.CallBack(call_back, this.h);
  ...
  HierarchyPainter.prototype.OpenJsonFile = function (filepath, call_back) {
    ...
    this.ForEachJsonFile(function (item) {
      if (item._jsonfile == filepath) isfileopened = true;
    });
    ...
    JSROOT.NewHttpRequest(filepath, 'object', function (res) {
      ...
      var h1 = {
        _jsonfile: filepath,
        _kind: 'ROOT.' + res._typename,
        _jsontmp: res,
        _name: filepath.split('/').pop(),
      };
      ...
      if (pthis.h == null) pthis.h = h1;
      else if (pthis.h._kind == 'TopFolder') pthis.h._childs.push(h1);
      else {
        var h0 = pthis.h,
          topname = '_jsonfile' in h0 ? 'Files' : 'Items';
  ...
  HierarchyPainter.prototype.OpenRootFile = function (filepath, call_back) {
    ...
    var isfileopened = false;
    this.ForEachRootFile(function (item) {
      if (item._fullurl === filepath) isfileopened = true;
    });
    if (isfileopened) return JSROOT.CallBack(call_back);
    ...
    JSROOT.OpenFile(filepath, function (file) {
      ...
      if (!file) {
        ...
        if (!d3.select('#gui_fileCORS').style('background', 'red').empty())
          setTimeout(function () {
            d3.select('#gui_fileCORS').style('background', '');
          }, 5000);
      ...
      if (pthis.h == null) {
        ...
        var h0 = pthis.h,
          topname = h0._kind == 'ROOT.TFile' ? 'Files' : 'Items';
        pthis.h = {
          _name: topname,
          _kind: 'TopFolder',
          _childs: [h0, h1],
  ...
  HierarchyPainter.prototype.ApplyStyle = function (style, call_back) {
    if (!style) return JSROOT.CallBack(call_back);

    if (typeof style === 'object') {
      if (style._typename === 'TStyle') JSROOT.extend(JSROOT.gStyle, style);
      return JSROOT.CallBack(call_back);
    }
    ...
    if (typeof style === 'string') {
      ...
      if (item !== null)
        return this.get(item, function (item2, obj) {
          hpainter.ApplyStyle(obj, call_back);
        });
    ...
    while (item._parent) {
      ...
      if ('_file' in item)
        return {
          ...
          itemname: subname,
          localfile: !!item._file.fLocalFile,
        };
  ...
  JSROOT.MarkAsStreamerInfo = function (h, item, obj) {
    ...
    if (obj && obj._typename == 'TList') obj._typename = 'TStreamerInfoList';
  };
  ...
  HierarchyPainter.prototype.GetOnlineItem = function (
    item,
    itemname,
    callback,
    option
    ...
    if (item) {
      ...
      if (typeof func == 'function') {
        ...
        if (dreq != null)
          if (typeof dreq == 'string') req = dreq;
          else {
    ...
    var itemreq = JSROOT.NewHttpRequest(url, req_kind, function (obj) {
      ...
      } else if (draw_handle && 'after_request' in draw_handle) func = draw_handle.after_request;
  ...
  HierarchyPainter.prototype.OpenOnline = function (
    server_address,
    user_callback
  ) {
    var painter = this;
    ...
    function AdoptHierarchy(result) {
      ...
      if ('_title' in painter.h && painter.h._title != '')
    ...
    JSROOT.NewHttpRequest(
      server_address + 'h.json?compact=3',
      'object',
      AdoptHierarchy
    ).send(null);
  };
  ...
  HierarchyPainter.prototype.FillOnlineMenu = function (
    menu,
    onlineprop,
    itemname
  ) {
    ...
    if (
      !node._childs &&
      node._more !== false &&
      (node._more || root_type || sett.expand)
    )
      menu.add('Expand', function () {
        painter.expand(itemname);
      });

    if (handle && 'execute' in handle)
      menu.add('Execute', function () {
        painter.ExecuteCommand(itemname, menu.tree_node);
      });
    ...
    if (sett.opts && node._can_draw !== false)
      menu.addDrawMenu('Draw in new window', sett.opts, function (arg) {
        window.open(drawurl + separ + 'opt=' + arg);
      });
  ...
  HierarchyPainter.prototype.SetMonitoring = function (interval, monitor_on) {
    ...
    if (this.IsMonitoring()) this._runMonitoring();
  };
  ...
  HierarchyPainter.prototype._runMonitoring = function (arg) {
    ...
    if (arg == 'frame') {
      ...
      delete this._monitoring_handle;
      this._monitoring_frame = requestAnimationFrame(
        this._runMonitoring.bind(this, 'draw')
      );
      return;
    }
    ...
    this._monitoring_handle = setTimeout(
      this._runMonitoring.bind(this, 'frame'),
  ...
  HierarchyPainter.prototype.CreateDisplay = function (callback) {
    ...
    if (
      this.disp_kind == 'simple' ||
      (this.disp_kind.indexOf('grid') == 0 &&
        this.disp_kind.indexOf('gridi') < 0)
    )
      this.disp = new GridDisplay(this.disp_frameid, this.disp_kind);
    else
      return JSROOT.AssertPrerequisites(
        'jq2d',
        this.CreateDisplay.bind(this, callback)
      );

    if (this.disp) this.disp.CleanupFrame = this.CleanupFrame.bind(this);
   ...
   */
  HierarchyPainter.prototype.CreateCustomDisplay = function (
    itemname,
    custom_kind,
    callback
  ) {
    if (this.disp_kind != 'simple') return this.CreateDisplay(callback);
  ...
  HierarchyPainter.prototype.StartGUI = function (gui_div, gui_call_back, url) {
    ...
    if (jsonarr.length == 1 && itemsarr.length == 0 && expanditems.length == 0)
      itemsarr.push('');
      ...
      if (typeof layout == 'string' && layout.length > 0)
        ...
        switch (itemsarr.length) {
          ...
          case 1:
            ...
            break;
          case 2:
            this.disp_kind = 'vert2';
            break;
          case 3:
            this.disp_kind = 'vert21';
            break;
          case 4:
            this.disp_kind = 'vert22';
            break;
          case 5:
            this.disp_kind = 'vert32';
            break;
          case 6:
            this.disp_kind = 'vert222';
            break;
          case 7:
            this.disp_kind = 'vert322';
            break;
          case 8:
            this.disp_kind = 'vert332';
            break;
          case 9:
            this.disp_kind = 'vert333';
            break;
          default:
            this.disp_kind = 'flex';
        }
    ...
    if (!browser_kind) browser_kind = 'fix';
    else if (browser_kind === 'no') browser_kind = '';
    ...
    function OpenAllFiles(res) {
      if (browser_kind) {
        hpainter.CreateBrowser(browser_kind);
        browser_kind = '';
      }
      if (status !== null) {
        hpainter.CreateStatusLine(statush, status);
        status = null;
      }
      if (jsonarr.length > 0)
        ...
        localfile !== null &&
        typeof hpainter.SelectLocalFile == 'function'
      ) {
        localfile = null;
        hpainter.SelectLocalFile(OpenAllFiles);
    ...
    function AfterOnlineOpened() {
      ...
      if (
        !hpainter.exclude_browser &&
        !browser_configured &&
        '_browser' in hpainter.h
      ) {
        browser_kind = hpainter.h._browser;
        if (browser_kind === 'no') browser_kind = '';
        else if (browser_kind === 'off') {
          browser_kind = '';
          status = null;
      ...
      if (
        '_layout' in hpainter.h &&
        !layout &&
        (hpainter.is_online != 'draw' || itemsarr.length > 1)
      )
      ...
      if (gui_div) hpainter.PrepareGuiDiv(gui_div, hpainter.disp_kind);
    ...
    if (h0 !== null) return this.OpenOnline(h0, AfterOnlineOpened);

    if (gui_div) this.PrepareGuiDiv(gui_div, this.disp_kind);
  ...
  HierarchyPainter.prototype.PrepareGuiDiv = function (myDiv, layout) {
    ...
    if (!this.exclude_browser) {
      ...
      JSROOT.ToolbarIcons.CreateSVG(
        btns,
        JSROOT.ToolbarIcons.diamand,
        15,
        'toggle fix-pos browser'
      )
        .style('margin', '3px')
        .on('click', this.CreateBrowser.bind(this, 'fix', true));

      if (!this.float_browser_disabled)
        JSROOT.ToolbarIcons.CreateSVG(
          btns,
          JSROOT.ToolbarIcons.circle,
          15,
          'toggle float browser'
        )
          .style('margin', '3px')
          .on('click', this.CreateBrowser.bind(this, 'float', true));

      if (!this.status_disabled)
        JSROOT.ToolbarIcons.CreateSVG(
          btns,
          JSROOT.ToolbarIcons.three_circles,
          15,
          'toggle status line'
        )
          .style('margin', '3px')
          .on('click', this.CreateStatusLine.bind(this, 0, 'toggle'));
    }
  ...
  HierarchyPainter.prototype.CreateBrowser = function (
    browser_kind,
    update_html,
    call_back
  ) {
  ...
  JSROOT.Painter.drawStreamerInfo = function (divid, lst) {
    ...
    for (var i = 0; i < lst.arr.length; ++i) {
      ...
      for (var l = 0; l < entry.fElements.arr.length; ++l) {
        ...
        if (elem.fArrayDim === 1) info += '[' + elem.fArrayLength + ']';
        else
          for (var dim = 0; dim < elem.fArrayDim; ++dim)
            info += '[' + elem.fMaxIndex[dim] + ']';
        if (elem.fBaseVersion === 4294967295) info += ':-1';
        ...
        item._childs.push({
          _name: info,
          _title: title,
          _kind: elem.fTypeName,
          _icon: elem.fTypeName == 'BASE' ? 'img_class' : 'img_member',
        });
      }
  ...
  JSROOT.Painter.drawInspector = function (divid, obj) {
    ...
    if (obj._typename) painter.h._title += '  type:' + obj._typename;
      ...
      if (sett.opts)
        menu.addDrawMenu('nosub:Draw', sett.opts, function (arg) {
          ...
          if (this.removeInspector) {
            ...
            if (arg == 'inspect') return this.ShowInspector(obj);
          }
          JSROOT.cleanup(divid);
  ...
  MDIDisplay.prototype.CheckMDIResize = function (only_frame_id, size) {
    ...
    this.ForEachPainter(function (painter, frame) {
      ...
      if (
        painter.GetItemName() !== null &&
        typeof painter.CheckResize == 'function'
      ) {
  ...
  CustomDisplay.prototype.ForEachFrame = function (userfunc, only_visible) {
    ...
    for (var k = 0; k < ks.length; ++k) {
      ...
      if (!node.empty()) JSROOT.CallBack(userfunc, node.node());
    }
  };
  ...
  CustomDisplay.prototype.CreateFrame = function (title) {
    ...
    for (var k = 0; k < ks.length; ++k) {
      ...
      if (items.indexOf(title + ';') >= 0) return d3.select('#' + ks[k]).node();
    }
    return null;
  };
  ...
  GridDisplay.prototype.CreateGroup = function (
    ...
    for (var cnt = 0; cnt < num; ++cnt) {
      ...
      if (cnt > 0)
        group.position =
          handle.groups[cnt - 1].position + handle.groups[cnt - 1].size;
      ...
      if (handle.vertical)
        elem
          .style('float', 'bottom')
          .style('height', group.size + '%')
          .style('width', '100%');
      else
        elem
          .style('float', 'left')
          .style('width', group.size + '%')
          .style('height', '100%');
      ...
      if (group.drawid >= 0) {
        ...
        elem
          .style('display', 'flex')
          .style('flex-direction', handle.vertical ? 'row' : 'column');
      }
  ...
  GridDisplay.prototype.ForEachFrame = function (userfunc, only_visible) {
    if (this.simple_layout) userfunc(this.GetFrame());
    else
      this.select_main()
  ...
  GridDisplay.prototype.GetFrame = function (id) {
    if (this.simple_layout) return this.select_main('origin').node();
    var res = null;
    this.select_main()
      .selectAll('.jsroot_newgrid')
      .each(function () {
