    ...
    GetTypeId: function (typname, norecursion) {
      ...
      if (!norecursion) {
        ...
        if (typeof replace === 'string')
          return JSROOT.IO.GetTypeId(replace, true);
      }
  ...
  JSROOT.R__unzip = function (arr, tgtsize, noalert, src_shift) {
    ...
    while (fullres < tgtsize) {
      ...
      if (curr + HDRSIZE >= totallen) {
        if (!noalert)
          JSROOT.alert('Error R__unzip: header size exceeds buffer size');
        return null;
      }
      ...
      if (
        ...
        fmt = 'old';
        off = 0;
      } else if (getChar(curr) == 'X' && getChar(curr + 1) == 'Z') fmt = 'LZMA';
      else if (getChar(curr) == 'L' && getChar(curr + 1) == '4') {
        fmt = 'LZ4';
        off = 0;
        CHKSUM = 8;
      }
...
*/
      ...
      if (fmt !== 'new' && fmt !== 'old' && fmt !== 'LZ4') {
        if (!noalert)
          JSROOT.alert('R__unzip: ' + fmt + ' format is not supported!');
        return null;
      }
      ...
      var srcsize =
        HDRSIZE +
        ((getCode(curr + 3) & 0xff) |
          ((getCode(curr + 4) & 0xff) << 8) |
          ((getCode(curr + 5) & 0xff) << 16));

      var uint8arr = new Uint8Array(
        arr.buffer,
        arr.byteOffset + curr + HDRSIZE + off + CHKSUM,
        Math.min(
          arr.byteLength - curr - HDRSIZE - off - CHKSUM,
          srcsize - HDRSIZE - CHKSUM
        )
      );
      ...
      var reslen =
        fmt === 'LZ4'
          ? JSROOT.LZ4.uncompress(uint8arr, tgt8arr)
    ...
    if (fullres !== tgtsize) {
      if (!noalert)
        JSROOT.alert(
          'R__unzip: fail to unzip data expects ' +
            tgtsize +
            ' , got ' +
            fullres
        );
  ...
  TBuffer.prototype.MapObject = function (tag, obj) {
    if (obj !== null) this.fObjectMap[tag] = obj;
  };
  ...
  TBuffer.prototype.ReadVersion = function () {
    ...
    // one can check between Read version and end of streamer
    else this.o -= 4; // rollback read bytes, this is old buffer without byte count
  ...
  TBuffer.prototype.ntou8 = function () {
    var high = this.arr.getUint32(this.o);
    this.o += 4;
    var low = this.arr.getUint32(this.o);
    this.o += 4;
  ...
  TBuffer.prototype.ReadFastArray = function (n, array_type) {
    ...
    switch (array_type) {
      case JSROOT.IO.kDouble:
        ...
        for (; i < n; ++i, o += 8) array[i] = view.getFloat64(o);
        break;
      case JSROOT.IO.kFloat:
        array = new Float32Array(n);
        for (; i < n; ++i, o += 4) array[i] = view.getFloat32(o);
        break;
      case JSROOT.IO.kLong:
      case JSROOT.IO.kLong64:
        array = new Float64Array(n);
        for (; i < n; ++i) array[i] = this.ntoi8();
        return array; // exit here to avoid conflicts
      case JSROOT.IO.kULong:
      case JSROOT.IO.kULong64:
        array = new Float64Array(n);
        for (; i < n; ++i) array[i] = this.ntou8();
        return array; // exit here to avoid conflicts
      case JSROOT.IO.kInt:
      case JSROOT.IO.kCounter:
        array = new Int32Array(n);
        for (; i < n; ++i, o += 4) array[i] = view.getInt32(o);
        break;
      case JSROOT.IO.kBits:
      case JSROOT.IO.kUInt:
        array = new Uint32Array(n);
        for (; i < n; ++i, o += 4) array[i] = view.getUint32(o);
        break;
      case JSROOT.IO.kShort:
        array = new Int16Array(n);
        for (; i < n; ++i, o += 2) array[i] = view.getInt16(o);
        break;
      case JSROOT.IO.kUShort:
        array = new Uint16Array(n);
        for (; i < n; ++i, o += 2) array[i] = view.getUint16(o);
        break;
      case JSROOT.IO.kChar:
        array = new Int8Array(n);
        for (; i < n; ++i) array[i] = view.getInt8(o++);
        break;
      case JSROOT.IO.kBool:
      case JSROOT.IO.kUChar:
        array = new Uint8Array(n);
        for (; i < n; ++i) array[i] = view.getUint8(o++);
        break;
      case JSROOT.IO.kTString:
        array = new Array(n);
        for (; i < n; ++i) array[i] = this.ReadTString();
        return array; // exit here to avoid conflicts
  ...
  TBuffer.prototype.extract = function (place) {
    ...
    for (var n = 0; n < place.length; n += 2)
      res[n / 2] = new DataView(
        this.arr.buffer,
        this.arr.byteOffset + place[n],
        place[n + 1]
      );
  ...
  JSROOT.IO.GetArrayKind = function (type_name) {
    ...
    if (
      type_name === 'TString' ||
      type_name === 'string' ||
      JSROOT.IO.CustomStreamers[type_name] === 'TString'
    )
      return 0;
  ...
  TBuffer.prototype.ReadNdimArray = function (handle, func) {
    var ndim = handle.fArrayDim,
      maxindx = handle.fMaxIndex,
      res;
    ...
    if (ndim === 1) {
      ...
      for (var n = 0; n < maxindx[0]; ++n) res[n] = func(this, handle);
    } else if (ndim === 2) {
      res = new Array(maxindx[0]);
      for (var n = 0; n < maxindx[0]; ++n) {
        var res2 = new Array(maxindx[1]);
        for (var k = 0; k < maxindx[1]; ++k) res2[k] = func(this, handle);
        res[n] = res2;
      }
    } else {
      var indx = [],
  ...
  TBuffer.prototype.ReadBasketEntryOffset = function (basket, offset) {
    ...
    if (this.remain() <= 0) {
      ...
      if (!basket.fEntryOffset)
        console.warn(
          'No fEntryOffset when expected for basket with',
          basket.fNevBuf,
          'entries'
        );
      return;
    ...
    else basket.fDisplacement = undefined;
  };
  ...
  TBuffer.prototype.ReadClass = function () {
    ...
    if (tag == JSROOT.IO.kNewClassTag) {
      ...
      if (
        this.GetMappedClass(
          this.fTagOffset + startpos + JSROOT.IO.kMapOffset
        ) === -1
      )
        this.MapClass(
          this.fTagOffset + startpos + JSROOT.IO.kMapOffset,
  ...
  TBuffer.prototype.ReadObjectAny = function () {
    ...
    if ('objtag' in clRef) return this.GetMappedObject(clRef.objtag);
  ...
  TBuffer.prototype.ClassStreamer = function (obj, classname) {
    ...
    if (streamer !== null) {
      for (var n = 0; n < streamer.length; ++n) streamer[n].func(this, obj);
    } else {
  ...
  TDirectory.prototype.GetKey = function (keyname, cycle, call_back) {
    ...
    while (pos > 0) {
      ...
      if (
        dirkey !== null &&
        typeof call_back == 'function' &&
        dirkey.fClassName.indexOf('TDirectory') == 0
      ) {
        this.fFile.ReadObject(this.dir_name + '/' + dirname, 1, function (
          newdir
        ) {
  ...
  TDirectory.prototype.ReadObject = function (obj_name, cycle, user_call_back) {
    this.fFile.ReadObject(
      this.dir_name + '/' + obj_name,
      cycle,
      user_call_back
    );
  };
  ...
  TDirectory.prototype.ReadKeys = function (objbuf, readkeys_callback) {
    ...
    file.ReadBuffer([this.fSeekKeys, this.fNbytesKeys], function (blob) {
      ...
      for (var i = 0; i < nkeys; ++i) dir.fKeys.push(buf.ReadTKey());
  ...
  function TFile(url, newfile_callback) {
    ...
    if (!this.fAcceptRanges) {
      ...
      JSROOT.NewHttpRequest(this.fURL, 'head', function (res) {
        if (!res) return JSROOT.CallBack(newfile_callback, null);
   ...
   * @private */
  TFile.prototype.ReadBuffer = function (
    place,
    result_callback,
    filename,
    progress_callback
    ...
    if (
      ...
      !filename &&
      (!this.fAcceptRanges || this.fFileContent.can_extract(place))
    )
    ...
    var file = this,
      ...
      last = 0,
      blobs = [],
      read_callback; // array of requested segments
    ...
    function send_new_request(increment) {
      ...
      if (progress_callback && typeof xhr.addEventListener === 'function') {
        ...
        var progress_offest = sum1 / sum_total,
          progress_this = (sum2 - sum1) / sum_total;
        xhr.addEventListener('progress', function (oEvent) {
          if (oEvent.lengthComputable)
            progress_callback(
              progress_offest + (progress_this * oEvent.loaded) / oEvent.total
            );
        });
      }
    ...
    read_callback = function (res) {
      ...
      if (res && place[0] === 0 && place.length === 2 && !file.fFileContent) {
        ...
        file.fFileContent = JSROOT.CreateTBuffer(
          typeof res == 'string' ? res : new DataView(res)
        );

        if (!file.fAcceptRanges) file.fEND = file.fFileContent.length;
      ...
      if (!res) {
        if (first === 0 && last > 2 && file.fMaxRanges > 1) {
          ...
          if (last / 2 > 200) file.fMaxRanges = 200;
          else if (last / 2 > 50) file.fMaxRanges = 50;
          else if (last / 2 > 20) file.fMaxRanges = 20;
          else if (last / 2 > 5) file.fMaxRanges = 5;
      ...
      if (!ismulti) {
        ...
        var hdr_range = this.getResponseHeader('Content-Range'),
          segm_start = 0,
          segm_last = -1;

        if (hdr_range && hdr_range.indexOf('bytes') >= 0) {
          var parts = hdr_range
            .substr(hdr_range.indexOf('bytes') + 6)
            .split(/[\s-\/]+/);
          if (parts.length === 3) {
        ...
        while (o < view.byteLength - 1 && !finish_header && nline < 5) {
          ...
          if (code1 == 13 && code2 == 10) {
            if (
              ...
              console.error(
                'Decode multipart message, expect boundary ',
                boundary,
                'got ',
                line
              );
            ...
            if (
              line.indexOf('content-range') >= 0 &&
              line.indexOf('bytes') > 0
            ) {
              var parts = line
                .substr(line.indexOf('bytes') + 6)
                .split(/[\s-\/]+/);
              if (parts.length === 3) {
                segm_start = parseInt(parts[0]);
                segm_last = parseInt(parts[1]);
  ...
  TFile.prototype.GetDir = function (dirname, cycle) {
    if (cycle === undefined && typeof dirname == 'string') {
      ...
      if (pos > 0) {
        cycle = parseInt(dirname.substr(pos + 1));
  ...
  TFile.prototype.ReadObjBuffer = function (key, callback) {
    ...
    this.ReadBuffer(
      [key.fSeekKey + key.fKeylen, key.fNbytes - key.fKeylen],
      function (blob1) {
  ...
  TFile.prototype.AddReadTree = function (obj) {
    if (JSROOT.TreeMethods) return JSROOT.extend(obj, JSROOT.TreeMethods);
   ...
   */
  TFile.prototype.ReadObject = function (
    obj_name,
    cycle,
    user_call_back,
    only_dir
    ...
    this.GetKey(obj_name, cycle, function (key) {
      if (!key) return JSROOT.CallBack(user_call_back, null);
      ...
      if (!isdir && only_dir) return JSROOT.CallBack(user_call_back, null);
      ...
      file.ReadObjBuffer(key, function (buf) {
        ...
        if (file.readTrees)
          return JSROOT.AssertPrerequisites('tree', function () {
            if (file.readTrees) {
              file.readTrees.forEach(function (t) {
                JSROOT.extend(t, JSROOT.TreeMethods);
              });
  ...
  TFile.prototype.ReadFormulas = function (tf1, user_call_back, cnt) {
    ...
    if (indx >= this.fKeys.length) return JSROOT.CallBack(user_call_back, tf1);

    var file = this;

    this.ReadObject(this.fKeys[indx].fName, this.fKeys[indx].fCycle, function (
      formula
    ) {
  ...
  TFile.prototype.ExtractStreamerInfos = function (buf) {
    ...
    for (var k = 0; k < lst.arr.length; ++k) {
      ...
      for (var l = 0; l < si.fElements.arr.length; ++l) {
        ...
        var typ = elem.fType,
          typname = elem.fTypeName;

        if (typ >= 60) {
          if (
            typ === JSROOT.IO.kStreamer &&
            ...
            if (
              typname.indexOf(prefix) === 0 &&
              typname[typname.length - 1] == '>'
            ) {
              typ = elem.fCtype;
              typname = typname
                .substr(prefix.length, typname.length - prefix.length - 1)
                .trim();

              if (
                elem.fSTLtype === JSROOT.IO.kSTLmap ||
                elem.fSTLtype === JSROOT.IO.kSTLmultimap
              )
                if (typname.indexOf(',') > 0)
                  typname = typname.substr(0, typname.indexOf(',')).trim();
                else continue;
            }
          }
          if (typ >= 60) continue;
        } else {
          if (typ > 20 && typname[typname.length - 1] == '*')
  ...
  TFile.prototype.ReadKeys = function (readkeys_callback) {
    ...
    this.ReadBuffer([0, 1024], function (blob) {
      ...
      if (file.fVersion < 1000000) {
        ...
        // new format to support large files
        file.fEND = buf.ntou8();
      ...
      file.ReadBuffer([file.fBEGIN, Math.max(300, nbytes)], function (blob3) {
        ...
        file.ReadBuffer(
          [file.fSeekKeys, file.fNbytesKeys, file.fSeekInfo, file.fNbytesInfo],
          function (blobs) {
            ...
            for (var i = 0; i < nkeys; ++i) file.fKeys.push(buf4.ReadTKey());
  ...
  JSROOT.IO.GetPairStreamer = function (si, typname, file) {
    if (!si) {
      ...
      if (!si) {
        ...
        si.fElements.Add(
          JSROOT.IO.CreateStreamerElement('first', GetNextName(), file)
        );
        si.fElements.Add(
          JSROOT.IO.CreateStreamerElement('second', GetNextName(), file)
        );
      }
    }
  ...
  JSROOT.IO.CreateMember = function (element, file) {
    ...
    switch (member.type) {
      ...
      case JSROOT.IO.kShort:
        member.func = function (buf, obj) {
          obj[this.name] = buf.ntoi2();
        };
        break;
      case JSROOT.IO.kInt:
      case JSROOT.IO.kCounter:
        member.func = function (buf, obj) {
          obj[this.name] = buf.ntoi4();
        };
        break;
      case JSROOT.IO.kLong:
      case JSROOT.IO.kLong64:
        member.func = function (buf, obj) {
          obj[this.name] = buf.ntoi8();
        };
        break;
      case JSROOT.IO.kDouble:
        member.func = function (buf, obj) {
          obj[this.name] = buf.ntod();
        };
        break;
      case JSROOT.IO.kFloat:
        member.func = function (buf, obj) {
          obj[this.name] = buf.ntof();
        };
        break;
      case JSROOT.IO.kLegacyChar:
      case JSROOT.IO.kUChar:
        member.func = function (buf, obj) {
          obj[this.name] = buf.ntou1();
        };
        break;
      case JSROOT.IO.kUShort:
        member.func = function (buf, obj) {
          obj[this.name] = buf.ntou2();
        };
        break;
      case JSROOT.IO.kBits:
      case JSROOT.IO.kUInt:
      ...
      case JSROOT.IO.kULong:
        member.func = function (buf, obj) {
          obj[this.name] = buf.ntou8();
        };
        break;
      case JSROOT.IO.kBool:
        member.func = function (buf, obj) {
          obj[this.name] = buf.ntou1() != 0;
        };
        break;
      ...
      case JSROOT.IO.kOffsetL + JSROOT.IO.kFloat:
        if (element.fArrayDim < 2) {
          ...
          member.func = function (buf, obj) {
            obj[this.name] = buf.ReadFastArray(
              this.arrlength,
          ...
          member.func = function (buf, obj) {
            obj[this.name] = buf.ReadNdimArray(this, function (buf, handle) {
              return buf.ReadFastArray(
                handle.arrlength,
                handle.type - JSROOT.IO.kOffsetL
              );
            });
          };
        }
      ...
      case JSROOT.IO.kOffsetP + JSROOT.IO.kFloat16:
        if (element.fFactor !== 0) {
          ...
          member.read = function (buf) {
            ...
            this.dv.setUint32(
              0,
              (theExp << 23) |
                ((theMan & ((1 << (this.nbits + 1)) - 1)) << (23 - this.nbits))
            );
            return (
              ((1 << (this.nbits + 1)) & theMan ? -1 : 1) *
              this.dv.getFloat32(0)
            );
          };
        }

        member.readarr = function (buf, len) {
          var arr = this.double32
        ...
        if (member.type < JSROOT.IO.kOffsetL) {
          member.func = function (buf, obj) {
            obj[this.name] = this.read(buf);
          };
        } else if (member.type > JSROOT.IO.kOffsetP) {
          member.cntname = element.fCountName;
          member.func = function (buf, obj) {
            if (buf.ntou1() === 1) {
          ...
          member.arrlength = element.fArrayLength;
          member.func = function (buf, obj) {
            obj[this.name] = this.readarr(buf, this.arrlength);
          };
        } else {
          member.arrlength = element.fMaxIndex[element.fArrayDim - 1];
          member.minus1 = true;
      ...
      case JSROOT.IO.kObject:
        var classname =
          element.fTypeName === 'BASE' ? element.fName : element.fTypeName;
      ...
      case JSROOT.IO.kTNamed:
        ...
        member.func = function (buf, obj) {
          obj[this.name] = buf.ClassStreamer({}, this.typename);
        };
        break;
      ...
      case JSROOT.IO.kOffsetL + JSROOT.IO.kStreamLoop:
        ...
        if (member.typename.lastIndexOf('**') > 0) {
          member.typename = member.typename.substr(
            0,
            member.typename.lastIndexOf('**')
          );
        ...
        if (member.isptrptr) {
          ...
          if (member.arrkind > 0)
            ...
            member.readitem = function (buf) {
              return buf.ClassStreamer({}, this.typename);
            };
        }
        ...
        if (member.readitem !== undefined) {
          member.read_loop = function (buf, cnt) {
            return buf.ReadNdimArray(this, function (buf2, member2) {
              ...
              for (var i = 0; i < cnt; ++i) itemarr[i] = member2.readitem(buf2);
              return itemarr;
            });
          };
          ...
          member.objs_branch_func = function (buf, obj) {
            ...
            for (var loop0 = 0; loop0 < arr.length; ++loop0) {
              var obj1 = this.get(arr, loop0),
                cnt = obj1[this.cntname];
      ...
      case JSROOT.IO.kStreamer:
        ...
        if (
          element._typename === 'TStreamerSTLstring' ||
          ...
          stl === JSROOT.IO.kSTLvector ||
          stl === JSROOT.IO.kSTLlist ||
          stl === JSROOT.IO.kSTLdeque ||
          stl === JSROOT.IO.kSTLset ||
          ...
          if (
            element.fCtype &&
            element.fCtype < 20 &&
            element.fCtype !== member.typeid
          ) {
            console.warn(
              'Contained type',
              member.conttype,
              'not recognized as basic type',
              element.fCtype,
              'FORCE'
            );
          ...
          if (member.typeid > 0) {
            ...
            if (
              member.conttype.lastIndexOf('*') ===
              member.conttype.length - 1
            ) {
              member.isptr = true;
              member.conttype = member.conttype.substr(
                0,
                member.conttype.length - 1
              );
            }
            ...
            if (!member.isptr && member.arrkind < 0) {
              var subelem = JSROOT.IO.CreateStreamerElement(
                'temp',
                member.conttype
              );
          ...
          stl === JSROOT.IO.kSTLmap ||
          stl === JSROOT.IO.kSTLmultimap
        ) {
          var p1 = member.typename.indexOf('<'),
            p2 = member.typename.lastIndexOf('>');
          ...
          member.streamer = JSROOT.IO.GetPairStreamer(
            member.si,
            member.pairtype,
            file
          );
        ...
        } else if (stl === JSROOT.IO.kSTLbitset) {
        ...
        if (!member.readelem) {
          ...
          member.branch_func = function (buf, obj) {
            ...
            for (var n = 0; n < cnt; ++n)
              arr[n] = buf.ReadNdimArray(this, function (buf2, member2) {
                return member2.readelem(buf2);
              });
          ...
          member.split_func = function (buf, arr, n) {
            ...
            var ver = this.read_version(buf);
            for (var i = 0; i < n; ++i)
              arr[i][this.name] = buf.ReadNdimArray(this, function (
                buf2,
                member2
              ) {
                return member2.readelem(buf2);
          ...
          member.objs_branch_func = function (buf, obj) {
            ...
            for (var n = 0; n < arr.length; ++n) {
              ...
              obj1[this.name] = buf.ReadNdimArray(this, function (
                buf2,
                member2
              ) {
                return member2.readelem(buf2);
  ...
  TFile.prototype.GetStreamer = function (clname, ver, s_i) {
    ...
    if (typeof custom === 'string') return this.GetStreamer(custom, ver, s_i);
  ...
  TLocalFile.prototype.ReadBuffer = function (
    place,
    result_callback,
    filename,
    progress_callback
  ) {
    if (filename) throw new Error('Cannot access other local file ' + filename);

    var reader = new FileReader(),
      cnt = 0,
      blobs = [],
      file = this.fLocalFile;
    ...
    reader.onload = function (evnt) {
      ...
      reader.readAsArrayBuffer(
        file.slice(place[cnt], place[cnt] + place[cnt + 1])
      );
    };
  ...
  TNodejsFile.prototype.ReadBuffer = function (
    place,
    result_callback,
    filename,
    progress_callback
  ...
  JSROOT.IO.ProduceCustomStreamers = function () {
    ...
    cs['TClonesArray'] = function (buf, list) {
      ...
      if (!streamer) {
        ...
        // create objects
        for (var n = 0; n < nobjects; ++n) list.arr[n] = { _typename: classv };

        // call streamer for all objects member-wise
        for (var k = 0; k < streamer.length; ++k)
          for (var n = 0; n < nobjects; ++n) streamer[k].func(buf, list.arr[n]);
      }
    };
    ...
    cs['TTreeIndex'] = function (buf, obj) {
      ...
      if (ver > 1)
        obj.fIndexValuesMinor = buf.ReadFastArray(obj.fN, JSROOT.IO.kLong64);
    ...
    cs['TObjArray'] = function (buf, list) {
      ...
      list.fLast = nobjects - 1;
      list.fLowerBound = buf.ntou4();
      while (i < nobjects) list.arr[i++] = buf.ReadObjectAny();
    };
    ...
    cs['TStreamerElement'] = function (buf, element) {
      ...
      element.fMaxIndex = buf.ReadFastArray(
        ver == 1 ? buf.ntou4() : 5,
        JSROOT.IO.kUInt
      );
      ...
      if (
        element.fType === JSROOT.IO.kUChar &&
        (element.fTypeName == 'Bool_t' || element.fTypeName == 'bool')
      )
      ...
      } else if (ver > 3 && element.fBits & JSROOT.BIT(6)) {
        // kHasRange

        var p1 = element.fTitle.indexOf('[');
        if (p1 >= 0 && element.fType > JSROOT.IO.kOffsetP)
          p1 = element.fTitle.indexOf('[', p1 + 1);
        var p2 = element.fTitle.indexOf(']', p1 + 1);

        if (p1 >= 0 && p2 >= p1 + 2) {
          var arr = JSROOT.ParseAsArray(element.fTitle.substr(p1, p2 - p1 + 1)),
            nbits = 32;
    ...
    cs['TStreamerSTL'] = function (buf, elem) {
      ...
      if (
        ...
        (elem.fTypeName.indexOf('std::set') === 0 ||
          elem.fTypeName.indexOf('set') == 0)
      )
        elem.fSTLtype = JSROOT.IO.kSTLset;

      if (
        elem.fSTLtype === JSROOT.IO.kSTLset &&
        (elem.fTypeName.indexOf('std::multimap') === 0 ||
          elem.fTypeName.indexOf('multimap') === 0)
      )
        elem.fSTLtype = JSROOT.IO.kSTLmultimap;
    };

    cs['TStreamerSTLstring'] = function (buf, elem) {
      if (buf.last_read_version > 0) buf.ClassStreamer(elem, 'TStreamerSTL');
    };

    cs['TStreamerObject'] = cs['TStreamerBasicType'] = cs[
      'TStreamerObjectAny'
    ] = cs['TStreamerString'] = cs['TStreamerObjectPointer'] = function (
      buf,
      elem
    ) {
      if (buf.last_read_version > 1)
    ...
    cs['RooCategory'] = function (buf, obj) {
      ...
      obj._sharedProp =
        v === 1
          ? buf.ReadObjectAny()
          : buf.ClassStreamer({}, 'RooCategorySharedProperties');
    };
    ...
    cs['RooLinkedList'] = function (buf, obj) {
      ...
      while (size--) obj.arr.Add(buf.ReadObjectAny());
      if (v > 1) obj._name = buf.ReadTString();
    };

    cs['TASImage'] = function (buf, obj) {
      if (
        buf.last_read_version == 1 &&
        buf.fFile.fVersion > 0 &&
        buf.fFile.fVersion < 50000
      ) {
      ...
      if (buf.ntou1() != 0) {
        ...
        obj.fImgBuf = buf.ReadFastArray(
          obj.fWidth * obj.fHeight,
          JSROOT.IO.kDouble
        );
      }
    };
    ...
    ds['TQObject'] = ds['TGraphStruct'] = ds['TGraphNode'] = ds[
      'TGraphEdge'
    ] = function (buf, obj) {
      // do nothing
    };
    ...
    ds['TBasket'] = function (buf, obj) {
      ...
      if (flag % 10 != 2) {
        if (obj.fNevBuf) {
          ...
          if (20 < flag && flag < 40)
            for (
              var i = 0, kDisplacementMask = 0xff000000;
              i < obj.fNevBuf;
              ++i
            )
      ...
      if (flag === 1 || flag > 10) {
        ...
        if (sz > obj.fKeylen) {
          ...
          obj.fBufferRef = JSROOT.CreateTBuffer(
            blob,
            0,
            buf.fFile,
            sz - obj.fKeylen
  ...
  JSROOT.IO.ReadVectorElement = function (buf) {
    if (this.member_wise) {
      ...
      if (
        ver.val === this.member_ver &&
        ver.checksum === this.member_checksum
      ) {
        streamer = this.member_streamer;
      } else {
        streamer = buf.fFile.GetStreamer(this.conttype, ver);

        this.member_streamer = streamer = buf.fFile.GetSplittedStreamer(
          streamer
        );
      ...
      for (i = 0; i < n; ++i) res[i] = { _typename: this.conttype }; // create objects
      if (!streamer) {
        console.error(
        ...
        for (k = 0; k < streamer.length; ++k) {
          ...
          if (member.split_func) {
            ...
            for (i = 0; i < n; ++i) member.func(buf, res[i]);
          }
        }
      }
      return res;
    ...
    if (n > 200000) {
      console.error('vector streaming for of', this.conttype, n);
      return res;
    }
  ...
  JSROOT.IO.ReadMapElement = function (buf) {
    ...
    if (this.member_wise) {
      ...
      if (this.si) {
        var si = buf.fFile.FindStreamerInfo(
          this.pairtype,
          ver.val,
          ver.checksum
        );
    ...
    if (this.member_wise) for (i = 0; i < n; ++i) streamer[1].func(buf, res[i]);
