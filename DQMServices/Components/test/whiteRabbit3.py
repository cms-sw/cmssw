    ...
    def run(self):
        ...
        if self.fakeRun == 0:
            ...
            for s in range(0,len(self.sequence)):
                ...
                l.write((time.ctime(time.time()) + ' cp -p %s %s ' % (self.mainDir+'/'+test, wdir+'/'+test)).encode())
                l.write(('\n').encode())
                (status,output) = subprocess.getstatusoutput('cp -p %s %s ' % (self.mainDir+'/'+test, wdir+'/'+test))
                if self.numev != 0:
                    print('Customizing Event to process/generate %d' % self.numev)
                    print("sed -i -e '#^numev.*#numev=%d#' %s/%s " % (self.numev, wdir, test))
                    (status,output) = subprocess.getstatusoutput("sed -i -e 's#^numev.*#numev=%d#' %s/%s " % (self.numev, wdir, test))
                if self.source and s==0:
                    ...
                    (status,output) = subprocess.getstatusoutput("sed -i -e 's#^DQMSEQUENCE.*#DQMSEQUENCE=%s#' %s/%s " % (self.source, wdir, test))
                if self.client and (s==1 or s==2):
                    print('Customizing client sequence with %s' % self.client)
                    (status,output) = subprocess.getstatusoutput("sed -i -e 's#^DQMSEQUENCE.*#DQMSEQUENCE=%s#\' %s/%s " % (self.client, wdir, test))
                l.write((time.ctime(time.time()) + ' cd %s && source %s/%s' % (wdir, wdir, test)).encode())
                (status,output) = subprocess.getstatusoutput('cd %s && source %s/%s' % (wdir, wdir, test) )
                l.write((time.ctime(time.time()) + "After running CMSSW, this is the exit code " + str(status)).encode())
                l.write((time.ctime(time.time()) + '\n ' + output).encode())
                l.write(('\n').encode())
                l.flush()
                if not status == 0:
                    l.write(self.inspect().encode())
                    l.write(('%s: Exit status different from 0, quitting. Status: %d' % (time.ctime(time.time()), status)).encode() )
                    l.write(('\n').encode())
                    l.close()
                    f = open(self.cwd+'/'+self.testNumber+'_Failed.log', 'wb')
                    f.write(('\n').encode())
                    f.close()
                    return
            l.close()
        f = open(self.cwd+'/'+self.testNumber+'_OK.log', 'wb')
        xmls = glob.glob('%s/*.xml' % wdir)
        #xmls = subprocess.getoutput('ls -1tr %s/*.xml' % wdir ).split('\n')
        output = subprocess.getoutput('eval `scramv1 runtime -sh` && showtags -r | grep "^V"').split('\n')
        testPackages = ""
        for line in output:
        ...
        for single in xmls:
            ...
            if resRun:
                for k in resRun.keys():
                    f.write(("db.results.save({'ts': '%s',  'IB': '%s', 'test': '%s', 'type': 'runProduct', 'logfile':'%s', 'packages': [%s], 'results': %s})\n " % (self.timestamp,cmssw_version,self.testNumber, filename.replace('FrameworkJobReport_',''),testPackages, resRun[k])).encode())
            resLS = fwjr.getReport(FWJRParser.LS)
            if resLS:
                for k in resLS.keys():
                    f.write(("db.results.save({'ts': '%s',  'IB': '%s', 'test': '%s', 'type': 'lsProduct', 'logfile':'%s', 'packages': [%s], 'results': %s})\n " % (self.timestamp,cmssw_version,self.testNumber, filename.replace('FrameworkJobReport_',''),testPackages, resLS[k])).encode())
...
if __name__ == '__main__':
    try:
        ...
        if os.path.exists('%s' % cwd ):
            ...
            logs = subprocess.getoutput(command).split('\n')
            for l in logs:
