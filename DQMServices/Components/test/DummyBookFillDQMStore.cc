...
void DummyBookFillDQMStore::analyze(edm::Event const& iEvent, edm::EventSetup const& iSetup) {
...
void DummyBookFillDQMStore::beginLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&) {
...
void DummyBookFillDQMStore::endLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&) {
  auto it = m_lumiFillers.begin();
  auto ite = m_lumiFillers.end();
  for (; it != ite; ++it)
    (*it)->fill();
...
void DummyBookFillDQMStore::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
