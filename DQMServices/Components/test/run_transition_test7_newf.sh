...
do
  ...
  echo -e "   ***   ${COLOR_WARN}RUNNING TEST 7 on ${bookIn} with OPTION $postfix${COLOR_NORMAL}   ***   "
  ...
  echo -e "${COLOR_WARN}${testConfig}${COLOR_NORMAL}"
  cmsRun ${LOCAL_TEST_DIR}/${testConfig} Collate &> /dev/null || die "cmsRun ${testConfig}" $?
  ...
  compare_using_files.py DQM_V0001_R000000001__Test__{,Merged_}File1_File3__DQM.root -C -s b2b -t 0.999999 2> /dev/null | grep 'Successes: 100.00%'
  compare_using_files.py DQM_V0001_R000000002__Test__{,Merged_}File1_File3__DQM.root -C -s b2b -t 0.999999 2> /dev/null | grep 'Successes: 100.00%'
