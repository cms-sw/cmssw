...
namespace {
  ...
  public:
    TH1FReader(const edm::ParameterSet& iPSet, DQMStore& iStore, std::string folder, bool iSetLumiFlag)
    ...
    void read() override {
      ...
      if (hist->GetEntries() != m_entries[m_indexToCheck]) {
        throw cms::Exception("WrongEntries")
            << "The element: " << m_name << " for index " << m_indexToCheck << " was expected to have "
            << m_entries[m_indexToCheck] << " entries but instead has " << hist->GetEntries();
      }

      if (hist->GetMean() != m_means[m_indexToCheck]) {
        throw cms::Exception("WrongEntries")
            << "The element: " << m_name << " for index " << m_indexToCheck << " was expected to have "
            << m_means[m_indexToCheck] << " mean but instead has " << hist->GetMean();
      }
      ++m_indexToCheck;
    }
  ...
  public:
    ...
    void read() override {
      ...
      if (hist->GetMean() != m_means[m_indexToCheck]) {
        throw cms::Exception("WrongMeans")
            << "The element: " << m_name << " for index " << m_indexToCheck << " was expected to have "
            << m_means[m_indexToCheck] << " mean but instead has " << hist->GetMean();
      }
      ++m_indexToCheck;
...
void DummyTestReadDQMStore::analyze(edm::Event const& iEvent, edm::EventSetup const& iSetup) {
  using namespace edm;
...
void DummyTestReadDQMStore::beginJob() {}

// ------------ method called once each job just after ending the event loop  ------------
void DummyTestReadDQMStore::endJob() {}

// ------------ method called when starting to processes a run  ------------
void DummyTestReadDQMStore::beginRun(edm::Run const& iRun, edm::EventSetup const&) {
  if (iRun.run() != runToCheck_)
    return;
...
void DummyTestReadDQMStore::endRun(edm::Run const& iRun, edm::EventSetup const&) {
  if (iRun.run() != runToCheck_)
    return;
...
void DummyTestReadDQMStore::endLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&) {
...
void DummyTestReadDQMStore::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
