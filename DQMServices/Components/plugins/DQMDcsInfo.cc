...
DQMDcsInfo::DQMDcsInfo(const edm::ParameterSet& ps) {
  ...
  gtCollection_ =
      consumes<L1GlobalTriggerReadoutRecord>(parameters_.getUntrackedParameter<std::string>("gtCollection", "gtDigis"));
  dcsStatusCollection_ = consumes<DcsStatusCollection>(
      parameters_.getUntrackedParameter<std::string>("dcsStatusCollection", "scalersRawToDigi"));
...
void DQMDcsInfo::makeDcsInfo(const edm::Event& e) {
  ...
  if (!dcsStatus.isValid()) {
    ...
    for (int i = 0; i < 24; i++)
      dcs[i] = false;  // info not available: set to false
    return;
  }
  ...
  for (auto const& dcsStatusItr : *dcsStatus) {
    ...
    if (!dcsStatusItr.ready(DcsStatus::TECm))
      dcs[22] = false;
    if (!dcsStatusItr.ready(DcsStatus::CASTOR))
      dcs[23] = false;
  }
