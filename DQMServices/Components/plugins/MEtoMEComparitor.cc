...
void MEtoMEComparitor::endLuminosityBlock(const edm::LuminosityBlock& iLumi, const edm::EventSetup&) {
...
void MEtoMEComparitor::beginRun(const edm::Run& iRun, const edm::EventSetup& iSetup) {
  if (_autoProcess) {
...
void MEtoMEComparitor::endRun(const edm::Run& iRun, const edm::EventSetup& iSetup) {
...
void MEtoMEComparitor::keepBadHistograms(const std::string& directory, const T* h_new, const T* h_ref) {
  ...
  d_n += "_diff";
  auto* difference = new T(
      d_n.c_str(), h_new->GetTitle(), h_new->GetNbinsX(), h_new->GetXaxis()->GetXmin(), h_new->GetXaxis()->GetXmax());
...
void MEtoMEComparitor::compare(const W& where, const std::string& instance) {
  ...
  where.getByLabel(edm::InputTag(_moduleLabel, instance, _process_ref), metoedm_ref);
  where.getByLabel(edm::InputTag(_moduleLabel, instance, _process_new), metoedm_new);
  ...
  typedef
      typename std::map<std::string, std::pair<const MEtoEDMObject*, const MEtoEDMObject*> >::iterator Mapping_iterator;

  Mapping mapping;

  LogDebug("MEtoMEComparitor") << "going to do the mapping from " << metoedmobject_ref.size() << " x "
                               << metoedmobject_new.size();
  ...
  for (unsigned int i_new = 0; i_new != metoedmobject_new.size(); ++i_new) {
    ...
    if (metoedmobject_new[i_new].object.GetEntries() == 0 || metoedmobject_new[i_new].object.Integral() == 0) {
      countMe--;
      continue;
    }
  ...
  for (auto it = mapping.begin(); it != mapping.end(); ++it) {
    ...
    if (dir.size() >= _dirDepth)
      for (unsigned int iD = 1; iD != _dirDepth; ++iD)
        subsystem += "/" + dir[iD];
    ...
    if (h_ref->GetEntries() != 0 && h_ref->Integral() != 0) {
      ...
      if (KS < _KSgoodness) {
        ...
        for (unsigned int iBin = 0; iBin != (unsigned int)h_new->GetNbinsX() + 1; ++iBin) {
        ...
        if (relativediff > _diffgoodness) {
          edm::LogWarning("MEtoMEComparitor")
              << "for " << h_new->GetName() << " in " << it->first << " the KS is " << KS * 100. << " %"
...
void MEtoMEComparitor::beginJob() {}

// ------------ method called once each job just after ending the event loop  ------------
void MEtoMEComparitor::endJob() {}
