...
boost::property_tree::ptree DQMFileSaver::fillJson(int run,
  ...
  return dqmfilesaver::fillJson(run, lumi, dataFilePathName, transferDestinationStr, mergeTypeStr, fms);
}
  ...
  if (fakeFilterUnitMode_) {
    ...
    if (fileFormat == ROOT) {
      ...
      openHistoFilePathName =
          edm::Service<evf::EvFDaqDirector>()->getOpenProtocolBufferHistogramFilePath(lumi, stream_label_);
...
DQMFileSaver::DQMFileSaver(const edm::ParameterSet &ps)
  ...
  if (convention == "Offline")
    ...
    throw cms::Exception("DQMFileSaver") << "Invalid 'convention' parameter '" << convention << "'."
  ...
  if (fileFormat == "ROOT")
    ...
    throw cms::Exception("DQMFileSaver") << "Invalid 'fileFormat' parameter '" << fileFormat << "'."
  ...
  if ((convention_ == Online || convention_ == FilterUnit) && producer_ != "DQM" && producer_ != "HLTDQM" &&
      producer_ != "Playback") {
    throw cms::Exception("DQMFileSaver") << "Invalid 'producer' parameter '" << producer_
                                         << "'.  Expected 'DQM', 'HLTDQM' or 'Playback'.";
  } else if (convention_ != Online && convention != FilterUnit && producer_ != "DQM") {
    throw cms::Exception("DQMFileSaver") << "Invalid 'producer' parameter '" << producer_ << "'.  Expected 'DQM'.";
  }
  ...
  if (refsave == "default")
    ...
    throw cms::Exception("DQMFileSaver") << "Invalid 'referenceHandling' parameter '" << refsave
  ...
  if (convention_ != FilterUnit) {
    ...
    edm::LogInfo("DQMFileSaver") << "The base dir provided in the configuration '" << dirName_ << "'\n"
    ...
    if (!(fms_ && daqDirector))
      throw cms::Exception("DQMFileSaver") << "Internal error, cannot initialize DAQ services.";
  }
...
void DQMFileSaver::globalEndLuminosityBlock(const edm::LuminosityBlock &iLS, const edm::EventSetup &) const {
  ...
  if (ilumi > 0 && saveByLumiSection_ > 0) {
    if (convention_ != Online && convention_ != FilterUnit && convention_ != Offline)
      throw cms::Exception("DQMFileSaver") << "Internal error, can save files at end of lumi block"
...
void DQMFileSaver::globalEndRun(const edm::Run &iRun, const edm::EventSetup &) const {
  ...
  if (irun > 0 && saveByRun_ > 0 && (nrun_ % saveByRun_) == 0) {
    if (convention_ == Online) {
      ...
      throw cms::Exception("DQMFileSaver") << "Internal error.  Can only save files in endRun()"
                                           << " in Online and Offline modes.";
  }

  // create a fake EoR file for testing purposes.
  if (fakeFilterUnitMode_) {
    edm::LogInfo("DQMFileSaver") << "Producing fake EoR file.\n";
