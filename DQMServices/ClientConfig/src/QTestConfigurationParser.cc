...
bool QTestConfigurationParser::qtestsConfig() {
  ...
  unsigned int qtestTagsNum = doc()->getElementsByTagName(qtxml::_toDOMS("QTEST"))->getLength();

  for (unsigned int i = 0; i < qtestTagsNum; ++i) {
    /// Get Node
    DOMNode *qtestNode = doc()->getElementsByTagName(qtxml::_toDOMS("QTEST"))->item(i);
    ...
    if (!std::strcmp(activate.c_str(), testActivationOFF.c_str())) {
      ...
      ///Get Qtest TYPE
      DOMNodeList *typeNodePrefix = qtestElement->getElementsByTagName(qtxml::_toDOMS("TYPE"));
      ...
      if (this->checkParameters(qtestName, qtestType))
        return true;
    }
...
std::map<std::string, std::string> QTestConfigurationParser::getParams(DOMElement *qtestElement,
...
bool QTestConfigurationParser::monitorElementTestsMap() {
  ...
  unsigned int linkTagsNum = doc()->getElementsByTagName(qtxml::_toDOMS("LINK"))->getLength();

  for (unsigned int i = 0; i < linkTagsNum; ++i) {
    DOMNode *linkNode = doc()->getElementsByTagName(qtxml::_toDOMS("LINK"))->item(i);
    ///Get ME name
    if (!linkNode) {
      return true;
    }
    ...
    if (!qualityTestList.empty())
      mapMonitorElementTests[linkName] = qualityTestList;
  }  ///Loop on linkTagsNum
