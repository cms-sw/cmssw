...
HcalTrigTowerGeometry::towerIds(const HcalDetId & cellId) const {
  ...
  if(cellId.subdet() == HcalForward) {
	...
	++ieta;
      ...
      results.emplace_back(ieta, iphi );
    } 
    if (use1x1_) {
      int hfRing = cellId.ietaAbs();
    ...
    if(cellId.ietaAbs() < theTopology->firstHEDoublePhiRing()) {    
      results.emplace_back(cellId.ieta(), cellId.iphi() );
    } else {
      ...
      results.emplace_back(ieta, iphi1 );
      results.emplace_back(ieta, iphi1+1 );
    }
  }
...
HcalTrigTowerGeometry::detIds(const HcalTrigTowerDetId & hcalTrigTowerDetId) const {
  ...
  if (abs(cell_ieta) <= theTopology->lastHBRing()){
    ...
    for (int cell_depth = min_depth; cell_depth <= min_depth + n_depths - 1; cell_depth++)
      results.emplace_back(HcalBarrel,cell_ieta,cell_iphi,cell_depth);
  }
  ...
  if (abs(cell_ieta) <= theTopology->lastHORing()){ 
    ...
    for (int ho_depth = min_depth; ho_depth <= min_depth + n_depths - 1; ho_depth++)
      results.emplace_back(HcalOuter, cell_ieta,cell_iphi,ho_depth);
  }
  ...
  if (abs(cell_ieta) >= theTopology->firstHERing() && 
    ...
    if (use2017_) {
         if (abs(tower_ieta) == 26)
            --n_depths;
         if (tower_ieta == 27)
            results.emplace_back(HcalEndcap, cell_ieta - 1, cell_iphi, 7);
         if (tower_ieta == -27)
            results.emplace_back(HcalEndcap, cell_ieta + 1, cell_iphi, 7);
    }
    
    for (int cell_depth = min_depth; cell_depth <= min_depth + n_depths - 1; cell_depth++)
      results.emplace_back(HcalEndcap, cell_ieta, cell_iphi, cell_depth);
    ...
    if (abs(tower_ieta) == 28){
      ...
      for (int cell_depth = min_depth; cell_depth <= min_depth + n_depths - 1; cell_depth++){
	if (tower_ieta < 0) results.emplace_back(HcalEndcap, tower_ieta - 1, cell_iphi, cell_depth);
	if (tower_ieta > 0) results.emplace_back(HcalEndcap, tower_ieta + 1, cell_iphi, cell_depth);
      }
    }
  ...
  if (abs(cell_ieta) >= theTopology->firstHFRing()){  
	...
	for (int iHFTowerEtaSegment = 0; iHFTowerEtaSegment < HfTowerEtaSize; iHFTowerEtaSegment++){
	  ...
	  for (int cell_depth = min_depth; cell_depth <= min_depth + n_depths - 1; cell_depth++)
	    results.emplace_back(HcalForward, cell_ieta, cell_iphi, cell_depth);
	  ...
	  if ( zside * cell_ieta == 30 ) {
	    ...
	    for (int cell_depth = min_depth; cell_depth <= min_depth + n_depths - 1; cell_depth++) 
	      results.emplace_back(HcalForward, 29 * zside , cell_iphi, cell_depth);
	  }
	}
      }  
      ...
      for (int cell_depth = min_depth; cell_depth <= min_depth + n_depths - 1; cell_depth++)
	results.emplace_back(HcalForward, tower_ieta, tower_iphi, cell_depth);      
	...
	int i29 = 29;
	  ...
	  for (int cell_depth = min_depth; cell_depth <= min_depth + n_depths - 1; cell_depth++)
	    results.emplace_back(HcalForward, i29, tower_iphi, cell_depth);      
      }
    }
  }
