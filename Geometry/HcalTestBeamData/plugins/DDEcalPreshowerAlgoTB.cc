...
DDEcalPreshowerAlgoTB::DDEcalPreshowerAlgoTB() : DDAlgorithm() {
  edm::LogInfo("HCalGeom") << "DDEcalPreshowerAlgoTB info: Creating an instance" ;
...
void DDEcalPreshowerAlgoTB::initialize(const DDNumericArguments & nArgs,
  ...
  edm::LogInfo("HCalGeom") << "DDEcalPreshowerAlgoTB info: Initialize" ;
  quadMin_ = vArgs["IQUAD_MIN"];
  edm::LogInfo("HCalGeom") << "DDEcalPreshowerAlgoTB IQUAD_MIN";
  quadMax_ = vArgs["IQUAD_MAX"];  
  edm::LogInfo("HCalGeom") << "DDEcalPreshowerAlgoTB IQUAD_MAX";
  thickLayers_ = vArgs["Layers"];
  edm::LogInfo("HCalGeom") << "DDEcalPreshowerAlgoTB Layers";
  thickness_ = double(nArgs["PRESH_Z_TOTAL"]);
  edm::LogInfo("HCalGeom") << "DDEcalPreshowerAlgoTB PRESH_Z_TOTAL";
  materials_ = vsArgs["LayMat"];
  edm::LogInfo("HCalGeom") << "DDEcalPreshowerAlgoTB material";
  rmaxVec = vArgs["R_MAX"]; // inner radii
  edm::LogInfo("HCalGeom") << "DDEcalPreshowerAlgoTB R_MAX";
  rminVec = vArgs["R_MIN"]; // outer radii
  edm::LogInfo("HCalGeom") << "DDEcalPreshowerAlgoTB R_MIN";
  waf_intra_col_sep = double(nArgs["waf_intra_col_sep"]);
  edm::LogInfo("HCalGeom") << "DDEcalPreshowerAlgoTB waf_intra_col_sep";
  waf_inter_col_sep = double(nArgs["waf_inter_col_sep"]);
  edm::LogInfo("HCalGeom") << "DDEcalPreshowerAlgoTB waf_intra_col_sep = "<<waf_inter_col_sep;
  waf_active = double(nArgs["waf_active"]);
  edm::LogInfo("HCalGeom") << "DDEcalPreshowerAlgoTB waf_active = " << waf_active;
  wedge_length = double(nArgs["wedge_length"]);
  edm::LogInfo("HCalGeom") << "DDEcalPreshowerAlgoTB wedge_length = " 
			   << wedge_length;
  ...
  edm::LogInfo("HCalGeom") << "DDEcalPreshowerAlgoTB Dummy Material = "
			   << dummyMaterial;

  idNameSpace = DDCurrentNamespace::ns();

  edm::LogInfo("HCalGeom") << "DDEcalPreshowerAlgoTB info: end initialize" ;
}

void DDEcalPreshowerAlgoTB::execute(DDCompactView& cpv) {

  edm::LogInfo("HCalGeom") << "******** DDEcalPreshowerAlgoTB execute!";
...
void DDEcalPreshowerAlgoTB::doLayers(DDCompactView& cpv) {
  ...
  for(size_t i = 0; i<rminVec.size(); ++i) {
    ...
    if (getMaterial(i) != dummyMaterial ) {
      ...
      edm::LogInfo("HCalGeom") <<"DDEcalPreshowerAlgoTB debug : Child "
...
void DDEcalPreshowerAlgoTB::doWedges(DDCompactView& cpv) {
  ...
  edm::LogInfo("HCalGeom") << "DDEcalPreshowerAlgoTB::debug : doWedges()";
  ...
  for(int I=1; I<=sz;++I) {
    for(int J=int(quadMax_[I-1]); J>=int(quadMin_[I-1]); --J) {
      //edm::LogInfo("HCalGeom") <<"DDEcalPreshowerAlgoTB::I=" << I << " J="  << J;
      nx += 1;
      icopy += 1;
      go=0; 
  ...
  for(int I=1; I<=sz;++I) {
	...
	edm::LogInfo("HCalGeom") << "DDEcalPreshowerAlgoTB::debug : Child " 
				 << name1 << " copy = " << icopy << " (" 
				 << icopyt+nxt << ") in Mother " 
				 << parent().name() << " translation " << tran
				 << " rotation " << rot1;
      }
