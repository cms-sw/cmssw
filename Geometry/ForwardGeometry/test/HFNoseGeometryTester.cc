...
void HFNoseGeometryTester::doTestWafer(const HGCalGeometry* geom) { 
  ...
  for (int zside : zsides) {
	...
	for (int waferV : wafers) {
	  ...
	  std::cout << "zside " << zside << " layer " << layer << " wafer " 
		    << waferU << ":" << waferV << " type " << type 
		    << std::endl;
	  for (int cellU : cells) {
	    for (int cellV : cells) {
		...
		std::cout << "DetId (" << zside << ":" << type << ":" << layer
			  << ":" << waferU << ":" << waferV << ":" << cellU 
			  << ":" << cellV << ") Geom " << icell1 
			  << " position ("  << global1.x() << ", " 
			  << global1.y() << ", " << global1.z() << ") ids " 
			  << std::hex << id1.rawId() << ":" << idc1.rawId() 
			  << std::dec << ":" << HFNoseDetId(id1) << ":" 
			  << HFNoseDetId(idc1) << " parameter[3] = " 
