///////////////////////////////////////////////////////////////////////////////
// File: DDHGCalTBSiliconRotatedCassette.cc
...
public:
  DDHGCalTBSiliconRotatedCassette();
...
DDHGCalTBSiliconRotatedCassette::DDHGCalTBSiliconRotatedCassette() {
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("HGCalGeom") << "DDHGCalTBSiliconRotatedCassette: Creating an instance";
#endif
}

void DDHGCalTBSiliconRotatedCassette::initialize(const DDNumericArguments& nArgs,
                                                 const DDVectorArguments& vArgs,
...
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("HGCalGeom") << "DDHGCalTBSiliconRotatedCassette: " << waferFull_.size() << " full and "
  ...
  edm::LogVerbatim("HGCalGeom") << "DDHGCalTBSiliconRotatedCassette: Partial Modules:";
  ...
  if ((passiveCool_.size() == 1) && (passiveCool_[0] == "NULL"))
    passiveCool_.clear();
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("HGCalGeom") << "DDHGCalTBSiliconRotatedCassette: " << passiveAbsorb_.size() << " full and "
...
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("HGCalGeom") << "DDHGCalTBSiliconRotatedCassette: " << materials_.size() << " types of volumes";
  ...
  edm::LogVerbatim("HGCalGeom") << "DDHGCalTBSiliconRotatedCassette: " << cassetteShift_.size()
...
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("HGCalGeom") << "DDHGCalTBSiliconRotatedCassette: NameSpace " << nameSpace_ << ":";
#endif
  cassette_.setParameter(cassettes_, cassetteShift_);
}

////////////////////////////////////////////////////////////////////
// DDHGCalTBSiliconRotatedCassette methods...
////////////////////////////////////////////////////////////////////
void DDHGCalTBSiliconRotatedCassette::execute(DDCompactView& cpv) {
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("HGCalGeom") << "==>> Constructing DDHGCalTBSiliconRotatedCassette...";
  copies_.clear();
#endif
  constructLayers(parent(), cpv);
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("HGCalGeom") << "DDHGCalTBSiliconRotatedCassette: " << copies_.size()
  ...
  copies_.clear();
  edm::LogVerbatim("HGCalGeom") << "<<== End of DDHGCalTBSiliconRotatedCassette construction...";
#endif
}

void DDHGCalTBSiliconRotatedCassette::constructLayers(const DDLogicalPart& module, DDCompactView& cpv) {
  double zi(zMinBlock_);
  int laymin(0);
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("HGCalGeom") << "DDHGCalTBSiliconRotatedCassette: Enters constructLayers with " << layers_.size()
                                << " layers";
#endif
  for (unsigned int i = 0; i < layers_.size(); i++) {
    ...
    double thickTot(0);
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCalGeom") << "DDHGCalTBSiliconRotatedCassette: Section " << i << " Layers " << laymin << ":"
                                  << laymax << " zi " << zi;
#endif
    for (int ly = laymin; ly < laymax; ++ly) {
...
#ifdef EDM_ML_DEBUG
      edm::LogVerbatim("HGCalGeom") << "DDHGCalTBSiliconRotatedCassette: Layer " << ly << ":" << ii << " Front " << zi
      ...
      if (layerSense_[ly] == 0) {
...
#ifdef EDM_ML_DEBUG
        edm::LogVerbatim("HGCalGeom") << "DDHGCalTBSiliconRotatedCassette: " << solid.name() << " polyhedra of "
        ...
        glog = DDLogicalPart(solid.ddname(), matter, solid);
#ifdef EDM_ML_DEBUG
        edm::LogVerbatim("HGCalGeom") << "DDHGCalTBSiliconRotatedCassette: " << solid.name() << " Tubs made of "
        ...
        if (layerSense_[ly] > 0) {
          ...
          unsigned int num = (-layerSense_[ly] <= passiveTypes_) ? passiveAbsorb_.size() : passiveCool_.size();
          if (num > 0)
            positionPassive(glog, i, -layerSense_[ly], cpv);
        }
      }
...
#ifdef EDM_ML_DEBUG
      if (layerSense_[ly] > 0)
	edm::LogVerbatim("HGCalGeom") << " LayerIndex " << i << ":" << ii << ":" << ly;
      edm::LogVerbatim("HGCalGeom") << "DDHGCalTBSiliconRotatedCassette: " << glog.name() << " number " << copy << ":" << inc << ":" << layerSense_[ly] << ":" << facingTypes_ << " positioned in " << module.name() << " at " << r1 << " with " << rotName  << " rotation";
#endif
	...
	edm::LogError("HGCalGeom") << "DHGCalSiliconRotatedCassette:Thickness of the partition " << layerThick_[i] << " is smaller than " << thickTot << ": thickness of all its components **** ERROR ****";
      } else {
	edm::LogWarning("HGCalGeom") << "DHGCalSiliconRotatedCassette:Thickness of the partition " << layerThick_[i] << " does not match with " << thickTot << " of the components";
      }
    }
...
void DDHGCalTBSiliconRotatedCassette::positionSensitive(const DDLogicalPart& glog, int layer, DDCompactView& cpv) {
  ...
  edm::LogVerbatim("HGCalGeom") << "DDHGCalTBSiliconRotatedCassette: PositionSensitive layer " << layer << "  r "
                                ...
                                << layertype;
  edm::LogVerbatim("HGCalGeom") << "DDHGCalTBSiliconRotatedCassette: " << glog.ddname() << "  r " << delx << " R "
  ...
  for (int k = firstWafer; k < lastWafer; ++k) {
    ...
    edm::LogVerbatim("HGCalGeom") << "DDHGCalTBSiliconRotatedCassette::Wafer: layer " << layer + 1 << " cassette "
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCalGeom") << " DDHGCalTBSiliconRotatedCassette: Layer "
...
#ifdef EDM_ML_DEBUG
    ++ntype[type];
    edm::LogVerbatim("HGCalGeom") << " DDHGCalTBSiliconRotatedCassette: " << name << " number " << copy << " type "
                                  << layertype << ":" << type << " positioned in " << glog.ddname() << " at " << tran
                                  << " with no rotation";
#endif
  } 
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("HGCalGeom") << "DDHGCalTBSiliconRotatedCassette: Maximum # of u " << ium << " # of v " << ivm
...
void DDHGCalTBSiliconRotatedCassette::positionPassive(const DDLogicalPart& glog,
                                                      int layer,
                                                      int absType,
                                                      DDCompactView& cpv) {
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("HGCalGeom") << "DDHGCalTBSiliconRotatedCassette: positionPassive is called for layer " << layer
...
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("HGCalGeom") << "DDHGCalTBSiliconRotatedCassette: Type " << type << ":" << absnum << " number per cassette " << num;
#endif
  for (int k = 0; k < cassettes_; ++k) {
    double xpos(0), ypos(0), zpos(0);
    int i1 = k + absnum;
    int i2 = layer * cassettes_ + i1;
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCalGeom") << "DDHGCalTBSiliconRotatedCassette::Passive2: layer " << layer + 1 << " cassette "
                                  << " PassiveIndex " << i1 << ":" << i2 << ":" << passiveAbsorb_.size() << ":"
                                  << passiveCool_.size();
#endif
    std::string passive = (type) ? passiveAbsorb_[i2] : passiveCool_[i2];
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCalGeom") << " DDHGCalTBSiliconRotatedCassette: Passive2 " << passive << " number " << i2
                                  << " pos " << xpos << ":" << ypos << ":" << zpos;
    kount++;
#endif
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCalGeom") << " DDHGCalTBSiliconRotatedCassette: " << name << " number " << i2
                                  << " positioned in " << glog.ddname() << " at " << tran << " with no rotation";
#endif
  }
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("HGCalGeom") << "DDHGCalTBSiliconRotatedCassette: " << kount << " passives of type " << absType
                                << " for " << glog.ddname();
#endif
}

DEFINE_EDM_PLUGIN(DDAlgorithmFactory, DDHGCalTBSiliconRotatedCassette, "hgcal:DDHGCalTBSiliconRotatedCassette");
