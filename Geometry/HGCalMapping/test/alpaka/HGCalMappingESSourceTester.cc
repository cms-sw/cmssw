  ...
  void HGCalMappingESSourceTester::produce(device::Event& iEvent, device::EventSetup const& iSetup) {
    ...
    for (size_t idx = 0; idx < cellIdx.di_.size(); idx++) {
      ...
      printf("[HGCalMappingIndexESSourceTester][produce][%s] has index(internal)=%ld #eRx=%d #cells=%d offset=%d\n",
             typecode.c_str(), idx, cellIdx.maxErx_[idx], cellIdx.di_[idx].getMaxIndex(), cellIdx.offsets_[idx]);
    }
    ...
    for (uint32_t i = 0; i < ncells; i++) {
      ...
      if(!cells.view()[i].valid()) continue;
      validCells ++;
      printf("\t idx=%d isHD=%d iscalib=%d isSiPM=%d typeidx=%d chip=%d half=%d seq=%d rocpin=%d cellidx=%d triglink=%d trigcell=%d i1=%d i2=%d t=%d trace=%f eleid=0x%x detid=0x%x\n",
             i,icell.isHD(),icell.iscalib(),icell.isSiPM(),icell.typeidx(),icell.chip(),icell.half(),icell.seq(),
             icell.rocpin(),icell.cellidx(),icell.triglink(),icell.trigcell(),icell.i1(),icell.i2(),
             icell.t(),icell.trace(),icell.eleid(),icell.detid());
    }
    ...
    printf("[HGCalMappingIndexESSourceTester][produce] Module indexer has FEDs=%d Types in sequences=%ld max idx=%d\n",
           modulesIdx.nfeds_,modulesIdx.globalTypesCounter_.size(),modulesIdx.maxModulesIdx_);
    ...
    for (int i = 0; i < nmodules; i++) {
      ...
      if(!modules.view()[i].valid()) continue;
      validModules++;
      printf("\t idx=%d zside=%d isSiPM=%d plane=%d i1=%d i2=%d celltype=%d typeidx=%d fedid=%d localfedid=%d captureblock=%d capturesblockidx=%d econdidx=%d eleid=0x%x detid=0x%d\n",
             i,imod.zside(),imod.isSiPM(),imod.plane(),imod.i1(),imod.i2(),imod.celltype(),imod.typeidx(),imod.fedid(),imod.slinkidx(),
             imod.captureblock(),imod.captureblockidx(),imod.econdidx(),imod.eleid(),imod.detid());
    }

    printf("[HGCalMappingIndexESSourceTester][produce] Module and cells maps retrieved for HGCAL %d/%d valid modules %d/%d valid cells",
           validModules,modules.view().metadata().size(),validCells,cells.view().metadata().size());
    ...
    printf("[HGCalMappingIndexESSourceTester][produce]  Creating %d number of raw ElectronicsIds from SoAs module idx: %d, cell idx: %d\n",n_i,modidx,cellidx);
    elecid=0;
    start = now();
    for (int i = 0; i < n_i; i++) {
    ...
    printf("[HGCalMappingIndexESSourceTester][produce] Creating %d number of raw HGCSiliconDetId from SoAs module idx: %d, cell idx: %d\n",n_i,modidx,cellidx);
    uint32_t detid(0);
    start = now();
    for (int i = 0; i < n_i; i++) {
  ...
  std::map<uint32_t, uint32_t> HGCalMappingESSourceTester::mapGeoToElectronics(
    ...
    for (int i = 0; i < modules.view().metadata().size(); i++) {
      ...
      for (int j = 0; j < cells.view().metadata().size(); j++) {
        ...
        if (sipm) {
          geoid = ::hgcal::mappingtools::getSiPMDetId(imod.zside(),
                                                      imod.plane(),
                                                      imod.i2(),
                                                      imod.celltype(),
                                                      jcell.i1(),
        ...
        if (geo2ele) {
          ...
          if (!sipm && it != idmap.end() && imod.plane() <= 26) {
            ...
            printf("WARNING duplicate found for plane=%d u=%d v=%d cellU=%d cellV=%d valid=%d -> detid=0x%x\n",
                   imod.plane(), imod.i1(), imod.i2(), jcell.i1(), jcell.i2(),
                   jcell.valid(),detid.rawId());
          }
        }
        if (!geo2ele) {
