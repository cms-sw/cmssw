...
def get_sequence_for_channel(channel, isHD):
  # for LD tileboards
  if not isHD:
...
def get_global_ring_number(local_ring, ring_max):
  ring = ring_max - local_ring
  return ring

def get_iphi_number(base_iphi, sector, isHD):
  niPhis = 8 if not isHD else 12
  iphi = base_iphi + (8 * (sector - 1)) # 0, 1, 2
  return iphi

def get_ring_max_and_n_rings(mod_n_rings, mod_ring_max, localROC, localHalfROC, hd_tiles):
...
if __name__ == '__main__':
  ...
  excluded_indices = {
    8, 17, 18, 27, 44, 53, 54, 63, 80, 89, 90, 99
  }
