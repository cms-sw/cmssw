...
void DDHGCalPassiveFull::execute(DDCompactView& cpv) {
  ...
  if ((std::abs(thickTot - thick_) >= tol) && (!layerType_.empty())) {
    if (thickTot > thick_) {
      edm::LogError("HGCalGeom") << "DDHGCalPassiveFull::Thickness of the partition " << thick_ << " is smaller than " << thickTot << ": thickness of all its components **** ERROR ****";
    } else {
      edm::LogWarning("HGCalGeom") << "DDHGCalPassiveFull::Thickness of the partition " << thick_ << " does not match with " << thickTot << " of the components";
    }
  }
}
