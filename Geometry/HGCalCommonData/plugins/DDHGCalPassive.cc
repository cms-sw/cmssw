      ...
      for (unsigned i3 = 0; i3 < absNames_.size(); ++i3) {
        ...
        if ((std::abs(thickTot - moduleThick_) >= tol) && (!layerType_.empty())) {
          if (thickTot > moduleThick_) {
            edm::LogError("HGCalGeom") << "DDHGCalPassive::Thickness of the partition " << moduleThick_ << " is smaller than " << thickTot << ": thickness of all its components **** ERROR ****";
          } else {
            edm::LogWarning("HGCalGeom") << "DDHGCalPassive::Thickness of the partition " << moduleThick_ << " does not match with " << thickTot << " of the components";
          }
        }
      }
    }
