...
#ifdef EDM_ML_DEBUG
  std::cout << childNames.size() << " children: " << childNames[0] << "; " << childNames[1] << " positioned "
            << positionX.size() << " times with cell size " << cellSize << std::endl;
  for (unsigned int k = 0; k < positionX.size(); ++k)
    std::cout << "[" << k << "] x " << positionX[k] << " y " << positionY[k] << " angle " << angles[k] << " detector "
              << detectorType[k] << std::endl;
#endif
  rotns = sArgs["RotNameSpace"];
  idNameSpace = DDCurrentNamespace::ns();
  parentName = parent().name();
#ifdef EDM_ML_DEBUG
  std::cout << "DDHGCalWaferAlgo debug: Parent " << parentName << " NameSpace " << idNameSpace << " for Rotation "
            << rotns << std::endl;
#endif
}
...
void DDHGCalWaferAlgo::execute(DDCompactView& cpv) {
  ...
  for (unsigned int k = 0; k < positionX.size(); ++k) {
    ...
    if (angles[k] != 0) {
      ...
      if (!rotation) {
#ifdef EDM_ML_DEBUG
        std::cout << "DDHGCalWaferAlgo: Creating new rotation " << DDName(rotstr, rotns) << "\t90, " << angles[k]
                  << ", 90, " << (angles[k] + 90) << ", 0, 0" << std::endl;
#endif
        rotation = DDrot(DDName(rotstr, rotns),
                         90 * CLHEP::deg,
                         angles[k] * CLHEP::deg,
                         90 * CLHEP::deg,
                         (90 + angles[k]) * CLHEP::deg,
                         0 * CLHEP::deg,
                         0 * CLHEP::deg);
      }
    }
...
#ifdef EDM_ML_DEBUG
    std::cout << "DDHGCalWaferAlgo: " << DDName(name, idNameSpace) << " number " << copy << " positioned in "
              << parentName << " at " << tran << " with " << rotation << std::endl;
#endif
  }
}
