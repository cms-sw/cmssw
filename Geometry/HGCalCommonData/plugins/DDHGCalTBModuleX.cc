...
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("HGCalGeom") << "DDHGCalTBModuleX: Material " << genMat_
				<< " with " << wafer_.size() << " wafers";
  unsigned int i(0);
  for (auto wafer : wafer_) {
    edm::LogVerbatim("HGCalGeom") << "Wafer[" << i << "] " << wafer; ++i;}
  edm::LogVerbatim("HGCalGeom") << "DDHGCalTBModuleX: " << covers_.size() 
				<< " covers";
  i = 0;
  for (auto cover : covers_) {
    edm::LogVerbatim("HGCalGeom") << "Cover[" << i << "] " << cover; ++i;}
#endif
...
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("HGCalGeom") << "DDHGCalTBModuleX: " << materials_.size()
				<< " types of volumes";
  for (unsigned int i=0; i<names_.size(); ++i)
    edm::LogVerbatim("HGCalGeom") << "Volume [" << i << "] " << names_[i] 
				  << " of thickness " << layerThick_[i] 
...
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("HGCalGeom") << "DDHGCalTBModuleX: " << blockThick_.size()
				<< " blocks with in/out " << inOut_;
  unsigned int lastF = ((inOut_ > 1) ? ((unsigned int)(layerFrontOut_[0])) :
			blockThick_.size());
  for (unsigned int i=0; i<lastF; ++i)
    edm::LogVerbatim("HGCalGeom") << "Block [" << i << "] of thickness "  
				  << blockThick_[i] << " with inner layers " 
				  << layerFrontIn_[i] << ":" << layerBackIn_[i];
  if (inOut_ > 1) {
    for (unsigned int i=lastF; i<blockThick_.size(); ++i)
      edm::LogVerbatim("HGCalGeom") << "Block [" << i << "] with outer layers " 
				    << layerFrontOut_[i] << ":" 
				    << layerBackOut_[i];
  }
#endif
...
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("HGCalGeom") << "DDHGCalTBModuleX: " << layerType_.size()
				<< " layers";
  for (unsigned int i=0; i<layerType_.size(); ++i)
    edm::LogVerbatim("HGCalGeom") << "Layer [" << i << "] with material type " 
				  << layerType_[i] << " sensitive class " 
				  << layerSense_[i] << " and " << maxModule_[i]
				  << " maximum row/columns";
#endif
...
void DDHGCalTBModuleX::constructBlocks(const DDLogicalPart& parent, 
  ...
  for (unsigned int i=0; i<blockThick_.size(); i++) {
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCalGeom") << "DDHGCalTBModuleX: " << glog.name()
    ...
    if (inOut_ > 1)
      constructLayers(i,layerFrontOut_[i],layerBackOut_[i],-0.5*blockThick_[i],
		      blockThick_[i],true,glog,cpv);
    zi     = zo;
  }
#ifdef EDM_ML_DEBUG
...
void DDHGCalTBModuleX::constructLayers(int block, int firstLayer, 
...
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("HGCalGeom") << "DDHGCalTBModuleX: \t\tInside Block "
				<< block << " Layers " << firstLayer << ":"
				<< lastLayer << " zFront " << zFront
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCalGeom") << "DDHGCalTBModuleX: " << name << " Layer "
				  << ly << ":" << ii << " Z " << zi << ":"
				  << zo << " Thick " << layerThick_[ii]
  ...
  if (fabs(thickTot-totalWidth) < tolerance_) {
    ...
    edm::LogError("HGCalGeom") << "Thickness of the partition " 
			       ...
			       << module.name() << " Layers " << firstLayer
			       << ":" << lastLayer << ":" << ignoreCenter
			       << "**** ERROR ****";
  } else if (thickTot < totalWidth) {
    edm::LogWarning("HGCalGeom") << "Thickness of the partition " 
				 << totalWidth << " does not match with " 
				 << thickTot << " of the components in "
				 << module.name() << " Layers " << firstLayer
				 << ":" << lastLayer << ":" << ignoreCenter;
  }
}
...
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("HGCalGeom") << "DDHGCalTBModuleX: # of columns " << incm 
