...
void DDHGCalNoTaperEndcap::initialize(const DDNumericArguments& nArgs,
  ...
  edm::LogInfo("HGCalGeom") << "DDHGCalNoTaperEndcap: NameSpace " << m_idNameSpace << "\tParent " << parent().name();
}
  ...
  while (std::abs(offsetX) < m_rMax) {
    ...
    while (std::abs(offsetY) < m_rMax) {
      ...
      if (limit2 > m_rMin && limit1 < m_rMax) {
#ifdef EDM_ML_DEBUG
        std::cout << m_childName << " copyNo = " << copyNo << " (" << column << "," << row
                  << "): offsetX,Y = " << offsetX << "," << offsetY << " limit=" << limit1 << ":" << limit2
        ...
        if (!rotation) {
          rotation =
              DDrot(DDName(rotstr, m_idNameSpace),
        ...
        edm::LogInfo("HGCalGeom") << "Module " << copyNo << ": location = " << tran << " Rotation " << rotation;
...
#ifdef EDM_ML_DEBUG
        std::cout << " (" << column << "," << row << "): offsetX,Y = " << offsetX << "," << offsetY
                  << " is out of limit=" << limit1 << ":" << limit2 << " rMin, rMax = " << m_rMin << "," << m_rMax
                  << std::endl;
#endif
      }
...
#ifdef EDM_ML_DEBUG
  std::cout << rowmax << " rows and " << column << " columns in quadrant " << xQuadrant << ":" << yQuadrant
            << std::endl;
#endif
  return copyNo;
}
