...
static long algorithm(dd4hep::Detector& /* description */, cms::DDParsingContext& ctxt, xml_h e) {
  ...
  for (unsigned int k = 0; k < tags.size(); ++k) {
    ...
    if (std::abs(thickTot - thick) >= tol) {
      if (thickTot > thick) {
        edm::LogError("HGCalGeom") << "DDHGCalWaferP::Thickness of the partition " << cms::convert2mm(thick)
                                   << " is smaller than " << cms::convert2mm(thickTot)
                                   << ": thickness of all its components **** ERROR ****";
      } else {
        edm::LogWarning("HGCalGeom") << "DDHGCalWaferP::Thickness of the partition " << cms::convert2mm(thick)
