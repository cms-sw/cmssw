...
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("HGCalGeom") << "DDHGCalModuleAlgo: zStart " << (f2mm * zMinBlock) << " rFineCoarse " << (f2mm * rMaxFine)
                                << " wafer width " << (f2mm * waferW) << " gap among wafers " << (f2mm * waferGap) << " sectors "
                                << sectors;
#endif
  ...
  for (unsigned int i = 0; i < layers.size(); i++) {
    ...
    for (int ly = laymin; ly < laymax; ++ly) {
      ...
      edm::LogVerbatim("HGCalGeom") << "DDHGCalModuleAlgo: Layer " << ly << ":" << ii << " Front " << (f2mm * zi) << ", "
                                    << (f2mm * routF) << " Back " << (f2mm * zo) << ", " << (f2mm * rinB) << " superlayer thickness "
                                    << (f2mm * layerThick[i]);
#endif
      ...
      if (layerSense[ly] == 0) {
        ...
        for (unsigned int k = 0; k < pgonZ.size(); ++k)
          edm::LogVerbatim("HGCalGeom") << "[" << k << "] z " << (f2mm * pgonZ[k]) << " R " << (f2mm * pgonRin[k]) << ":" << (f2mm * pgonRout[k]);
#endif
      } else {
