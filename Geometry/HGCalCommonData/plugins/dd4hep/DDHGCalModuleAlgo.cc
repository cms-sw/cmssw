...
static long algorithm(dd4hep::Detector& /* description */, cms::DDParsingContext& ctxt, xml_h e) {
  ...
  for (unsigned int i = 0; i < names.size(); ++i)
    edm::LogVerbatim("HGCalGeom") << "Volume [" << i << "] " << names[i] << " of thickness " << HGCalDD4HepHelper::convert2mm(thick[i])
...
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("HGCalGeom") << "DDHGCalModuleAlgo: zStart " << HGCalDD4HepHelper::convert2mm(zMinBlock) << " rFineCoarse " << HGCalDD4HepHelper::convert2mm(rMaxFine) << " wafer width " << HGCalDD4HepHelper::convert2mm(waferW) << " gap among wafers " << HGCalDD4HepHelper::convert2mm(waferGap) << " sectors " << sectors;
#endif
  ...
  for (unsigned int i = 0; i < layers.size(); i++) {
    ...
    for (int ly = laymin; ly < laymax; ++ly) {
...
#ifdef EDM_ML_DEBUG
      edm::LogVerbatim("HGCalGeom") << "DDHGCalModuleAlgo: Layer " << ly << ":" << ii << " Front " << HGCalDD4HepHelper::convert2mm(zi) << ", " << HGCalDD4HepHelper::convert2mm(routF) << " Back " << HGCalDD4HepHelper::convert2mm(zo) << ", " << HGCalDD4HepHelper::convert2mm(rinB) << " superlayer thickness " << HGCalDD4HepHelper::convert2mm(layerThick[i]);
#endif
      ...
      if (layerSense[ly] == 0) {
        ...
        glog = dd4hep::Volume(solid.name(), solid, matter);
#ifdef EDM_ML_DEBUG
        edm::LogVerbatim("HGCalGeom") << "DDHGCalModuleAlgo: " << solid.name() << " Tubs made of " << materials[ii] << " of dimensions " << HGCalDD4HepHelper::convert2mm(rinB) << ", " << HGCalDD4HepHelper::convert2mm(routF) << ", " << HGCalDD4HepHelper::convert2mm(0.5 * thick[ii]) << ", 0.0, 360.0";
              ...
              if (corner.first > 0) {
                ...
                if (corner.first == (int)(HGCalParameters::k_CornerSize)) {
                  ...
                  if (rpos < rMaxFine)
                    ...
                    ++ncoarse;
                  edm::LogVerbatim("HGCalGeom") << "DDHGCalModuleAlgo: " << glog1.name() << " number " << copyL << " positioned in " << glog.name() << " at (" << HGCalDD4HepHelper::convert2mm(xpos) << "," << HGCalDD4HepHelper::convert2mm(ypos) << ",0) with " << rotation;
#endif
                }
              }
            }
...
#ifdef EDM_ML_DEBUG
        edm::LogVerbatim("HGCalGeom") << "DDHGCalModuleAlgo: # of columns " << incm << " # of rows " << inrm << " and " << nin << ":" << kount << ":" << ntot << " wafers (" << nfine << ":" << ncoarse << ") for " << glog.name() << " R " << HGCalDD4HepHelper::convert2mm(rinB) << ":" << HGCalDD4HepHelper::convert2mm(routF);
#endif
      }
      ...
      ++copyNumber[ii];
#ifdef EDM_ML_DEBUG
      edm::LogVerbatim("HGCalGeom") << "DDHGCalModuleAlgo: " << glog.name() << " number " << copy << " positioned in " << module.name() << " at (0,0," << HGCalDD4HepHelper::convert2mm(zz) << ") with " << rot;
#endif
      zz += (0.5 * thick[ii]);
    ...
    if (fabs(thickTot - layerThick[i]) > tol2) {
      if (thickTot > layerThick[i]) {
        ...
        edm::LogWarning("HGCalGeom") << "Thickness of the partition " << HGCalDD4HepHelper::convert2mm(layerThick[i]) << " does not match with " << HGCalDD4HepHelper::convert2mm(thickTot) << " of the components\n";
      }
    }
