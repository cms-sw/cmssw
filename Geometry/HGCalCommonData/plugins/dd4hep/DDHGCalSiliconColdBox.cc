...
struct HGCalSiliconColdBox {
  ...
  HGCalSiliconColdBox(cms::DDParsingContext& ctxt, xml_h e) {
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCalGeom") << "DDHGCalSiliconColdBox: Creating an instance";
#endif
    ...
    rotmats_ = args.value<std::vector<std::string>>("LayerRotm");
#ifdef EDM_ML_DEBUG
...
:" << convertRadToDeg(alpha_) << ":"
    ...
    edm::LogVerbatim("HGCalGeom") << "DDHGCalSiliconColdBox: Partial Modules:";
    ...
    if ((passiveCool_.size() == 1) && (passiveCool_[0] == "NULL"))
      passiveCool_.clear();
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCalGeom") << "DDHGCalSiliconColdBox: " << passiveAbsorb_.size() << " full and "
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCalGeom") << "DDHGCalSiliconColdBox: " << materials_.size() << " types of volumes";
    ...
    edm::LogVerbatim("HGCalGeom") << "DDHGCalSiliconColdBox: " << cassetteShift_.size()
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCalGeom") << "==>> Constructing DDHGCalSiliconColdBox...";
    copies_.clear();
#endif
    ...
    int laymin(0);
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCalGeom") << "DDHGCalSiliconColdBox: Starts constructLayers with " << layers_.size()
                                  << " layers";
#endif
    for (unsigned int i = 0; i < layers_.size(); i++) {
...
#ifdef EDM_ML_DEBUG
        edm::LogVerbatim("HGCalGeom") << "DDHGCalSiliconColdBox: Layer " << ly << ":" << ii << " Front "
      ...
      if (std::abs(thickTot - layerThick_[i]) >= tol2) {
        if (thickTot > layerThick_[i]) {
          edm::LogError("HGCalGeom") << "DHGCalSiliconColdBox:Thickness of the partition "
                                     << cms::convert2mm(layerThick_[i]) << " is smaller than "
                                     << cms::convert2mm(thickTot)
                                     << ": thickness of all its components **** ERROR ****";
        } else {
          edm::LogWarning("HGCalGeom") << "DHGCalSiliconColdBox:Thickness of the partition "
    ...
    copies_.clear();
    edm::LogVerbatim("HGCalGeom") << "<<== End of DDHGCalSiliconColdBox construction...";
#endif
  }
...
#ifdef EDM_ML_DEBUG
      ++ntype[type];
      edm::LogVerbatim("HGCalGeom") << " DDHGCalSiliconColdBox: " << wafer << " number " << copy << " type "
...
#ifdef EDM_ML_DEBUG
    int ium(0), ivm(0), kount(0);
    edm::LogVerbatim("HGCalGeom") << "DDHGCalSiliconColdBox: " << glog.name() << "  r " << cms::convert2mm(delx)
...
#ifdef EDM_ML_DEBUG
      edm::LogVerbatim("HGCalGeom") << " DDHGCalSiliconColdBox: " << passive << " number " << copy << " type "
                                    << layertype << ":" << type << " positioned in " << glog.name() << " at ("
                                    << cms::convert2mm(xpos) << "," << cms::convert2mm(ypos) << ",0) with no rotation";
#endif
    }
  ...
  void positionPassiveNew(cms::DDParsingContext& ctxt, xml_h e, const dd4hep::Volume& glog, int layer, int absType) {
    cms::DDNamespace ns(ctxt, e, true);
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCalGeom") << "DDHGCalSiliconColdBox: positionPassiveNew is called for layer " << layer
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCalGeom") << "DDHGCalSiliconColdBox: Type " << type << " number per cassette " << num;
#endif
...
#ifdef EDM_ML_DEBUG
        edm::LogVerbatim("HGCalGeom") << "DDHGCalSiliconColdBox::Passive2: layer " << layer + 1 << " cassette "
  ...
  std::vector<std::string> rotmats_;        // Rotation matrices (Cold Box test stand)
...
static long algorithm(dd4hep::Detector& /* description */, cms::DDParsingContext& ctxt, xml_h e) {
  HGCalSiliconColdBox siliconColdBoxAlgo(ctxt, e);
  return cms::s_executed;
}

DECLARE_DDCMS_DETELEMENT(DDCMS_hgcal_DDHGCalSiliconColdBox, algorithm)
