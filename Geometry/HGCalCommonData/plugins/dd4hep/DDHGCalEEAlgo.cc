...
struct HGCalEEAlgo {
  ...
  HGCalEEAlgo(cms::DDParsingContext& ctxt, xml_h e) {
    ...
    for (unsigned int i = 0; i < names_.size(); ++i)
      edm::LogVerbatim("HGCalGeom") << "Volume [" << i << "] " << names_[i] << " of thickness " << HGCalDD4HepHelper::convert2mm(thick_[i]) << " filled with " << materials_[i] << " first copy number " << copyNumber_[i];
#endif
    ...
    for (unsigned int i = 0; i < layers_.size(); ++i)
      edm::LogVerbatim("HGCalGeom") << "Block [" << i << "] of thickness " << HGCalDD4HepHelper::convert2mm(layerThick_[i]) << " with " << layers_[i] << " layers";
#endif
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCalGeom") << "zStart " << HGCalDD4HepHelper::convert2mm(zMinBlock_) << " radius for wafer type separation uses " << rad100to200_.size() << " parameters; zmin " << HGCalDD4HepHelper::convert2mm(zMinRadPar_) << " cutoff " << choiceType_ << ":" << nCutRadPar_ << ":" << fracAreaMin_ << " wafer width " << HGCalDD4HepHelper::convert2mm(waferSize_) << " separations " << HGCalDD4HepHelper::convert2mm(waferSepar_) << " sectors " << sectors_ << ":" << convertRadToDeg(alpha_) << ":" << cosAlpha_;
...
#ifdef EDM_ML_DEBUG
        edm::LogVerbatim("HGCalGeom") << "DDHGCalEEAlgo: Layer " << ly << ":" << ii << " Front " << HGCalDD4HepHelper::convert2mm(zi) << ", " << HGCalDD4HepHelper::convert2mm(routF) << " Back " << HGCalDD4HepHelper::convert2mm(zo) << ", " << HGCalDD4HepHelper::convert2mm(rinB) << " superlayer thickness " << HGCalDD4HepHelper::convert2mm(layerThick_[i]);
#endif
        ...
        if (layerSense_[ly] < 1) {
          ...
          if (layerSense_[ly] == 0 || absorbMode_ == 0) {
            ...
            HGCalGeomTools::radius(zz - hthick,
                                   ...
                                   pgonRout);
#ifdef EDM_ML_DEBUG
            edm::LogVerbatim("HGCalGeom") << "DDHGCalEEAlgo: z " << HGCalDD4HepHelper::convert2mm((zz - hthick)) << ":" << HGCalDD4HepHelper::convert2mm((zz + hthick)) << " with " << pgonZ.size() << " palnes";
            for (unsigned int isec = 0; isec < pgonZ.size(); ++isec)
              edm::LogVerbatim("HGCalGeom") << "[" << isec << "] z " << HGCalDD4HepHelper::convert2mm(pgonZ[isec]) << " R " << HGCalDD4HepHelper::convert2mm(pgonRin[isec]) << ":" << HGCalDD4HepHelper::convert2mm(pgonRout[isec]);
#endif
            for (unsigned int isec = 0; isec < pgonZ.size(); ++isec) {
              pgonZ[isec] -= zz;
...
#ifdef EDM_ML_DEBUG
          edm::LogVerbatim("HGCalGeom") << "DDHGCalEEFileAlgo: " << solid.name() << " Tubs made of " << matter.name() << " of dimensions " << HGCalDD4HepHelper::convert2mm(rinB) << ":" << HGCalDD4HepHelper::convert2mm(rins) << ", " << HGCalDD4HepHelper::convert2mm(routF) << ":" << HGCalDD4HepHelper::convert2mm(routs) << ", " << HGCalDD4HepHelper::convert2mm(hthick) << ", 0.0, 360.0 and position " << glog.name() << " number " << copy << ":" << layerCenter_[copy - firstLayer_];
#endif
          PositionSensitive(
...
#ifdef EDM_ML_DEBUG
        edm::LogVerbatim("HGCalGeom") << "DDHGCalEEAlgo: " << glog.name() << " number " << copy << " positioned in " << module.name() << " at (0,0," << HGCalDD4HepHelper::convert2mm(zz) << ") with no rotation";
#endif
        zz += hthick;
    ...
    for (int u = -N; u <= N; ++u) {
      for (int v = -N; v <= N; ++v) {
        ...
        if (((corner.first <= 0) && std::abs(u) < 5 && std::abs(v) < 5) || (std::abs(u) < 2 && std::abs(v) < 2)) {
          edm::LogVerbatim("HGCalGeom") << "DDHGCalEEAlgo: " << glog.name() << " R " << HGCalDD4HepHelper::convert2mm(rin) << ":" << HGCalDD4HepHelper::convert2mm(rout) << "\n Z " << HGCalDD4HepHelper::convert2mm(zpos) << " LayerType " << layertype << " u " << u << " v " << v << " with " << corner.first << " corners";
        }
#endif
        if (corner.first > 0) {
          int type = waferType_->getType(HGCalDD4HepHelper::convert2mm(xpos), HGCalDD4HepHelper::convert2mm(ypos), HGCalDD4HepHelper::convert2mm(zpos));
...
#ifdef EDM_ML_DEBUG
            ++ntype[type];
            edm::LogVerbatim("HGCalGeom") << " DDHGCalEEAlgo: " << wafers_[type] << " number " << copy << " positioned in " << glog.name() << " at (" << HGCalDD4HepHelper::convert2mm(xpos) << ", " << HGCalDD4HepHelper::convert2mm(ypos) << ",0) with no rotation";
#endif
          }
        }
      }
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCalGeom") << " DDHGCalEEAlgo: Maximum # of u " << ium << ":" << iumAll << " # of v " << ivm << ":" << ivmAll << " and " << nin << ":" << kount << ":" << ntot << " wafers (" << ntype[0] << ":" << ntype[1] << ":" << ntype[2] << ":" << ntype[3] << ":" << ntype[4] << ":" << ntype[5] << ") for " << glog.name() << " R " << HGCalDD4HepHelper::convert2mm(rin) << ":" << HGCalDD4HepHelper::convert2mm(rout);
#endif
  }
};
