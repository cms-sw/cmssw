...
struct HGCalHEAlgo {
  ...
  HGCalHEAlgo(cms::DDParsingContext& ctxt, xml_h e) {
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCalGeom") << "DDHGCalHEFileAlgo: " << waferNames.size() << " wafers";
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCalGeom") << "DDHGCalHEFileAlgo: " << materials.size() << " types of volumes";
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCalGeom") << "DDHGCalHEFileAlgo: " << materialsTop.size()
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCalGeom") << "DDHGCalHEFileAlgo: " << materialsBot.size()
    ...
    cosAlpha = cos(alpha);
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCalGeom") << "DDHGCalHEFileAlgo: zStart " << zMinBlock
                                  << " radius for wafer type separation uses " << rad100to200.size()
                                  << " parameters; zmin " << zMinRadPar << " cutoff " << choiceType << ":" << nCutRadPar
                                  << ":" << fracAreaMin << " wafer width " << waferSize << " separations " << waferSepar
                                  << " sectors " << sectors << ":" << convertRadToDeg(alpha) << ":" << cosAlpha;
    ...
    for (unsigned int i = 0; i < layerNumbers.size(); i++) {
      ...
      for (int ly = laymin; ly < laymax; ++ly) {
        ...
        if (layerSense[ly] < 1) {
          ...
          ns.addVolumeNS(glog);
#ifdef EDM_ML_DEBUG
          edm::LogVerbatim("HGCalGeom") << "DDHGCalHEFileAlgo: " << solid.name() << " polyhedra of " << sectors
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCalGeom") << "DDHGCalHEFileAlgo: " << copies.size() << " different wafer copy numbers";
    int k(0);
...
#endif
        positionSensitive(ctxt, e, glog2, rin, rmid, zz + zpos, layerSenseBot[ly], (copy - firstLayer));
      }
      zpos += hthickl;
      ++copyNumberBot[ii];
    }
