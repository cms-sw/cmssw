...
struct HGCalHEFileAlgo {
    ...
    for (unsigned int i = 0; i < layerNumbers_.size(); i++) {
      ...
      if (std::abs(thickTot - layerThick_[i]) > tol2_) {
        if (thickTot > layerThick_[i]) {
          edm::LogError("HGCalGeom") << "DDHGCalHEFileAlgo::Thickness of the partition " << cms::convert2mm(layerThick_[i]) << " is smaller than " << cms::convert2mm(thickTot) << ": thickness of all its components **** ERROR ****";
        } else {
          edm::LogWarning("HGCalGeom") << "DDHGCalHEFileAlgo::Thickness of the partition " << cms::convert2mm(layerThick_[i]) << " does not match with " << cms::convert2mm(thickTot) << " of the components";
        }
      }
  ...
  void positionMix(cms::DDParsingContext& ctxt,
    ...
    if (std::abs(thickTot - thick) > tol2_) {
      if (thickTot > thick) {
        edm::LogError("HGCalGeom") << "DDHGCalHEFileAlgo::Thickness of the partition " << cms::convert2mm(thick) << " is smaller than " << cms::convert2mm(thickTot) << ": thickness of all its components in the top part **** ERROR ****";
      } else {
        edm::LogWarning("HGCalGeom") << "DDHGCalHEFileAlgo::Thickness of the partition " << cms::convert2mm(thick) << " does not match with " << cms::convert2mm(thickTot) << " of the components in top part";
      }
    }
...
static long algorithm(dd4hep::Detector& /* description */, cms::DDParsingContext& ctxt, xml_h e) {
  HGCalHEFileAlgo heFileAlgo(ctxt, e);
