...
struct HGCalEEFileAlgo {
  ...
  HGCalEEFileAlgo(cms::DDParsingContext& ctxt, xml_h e) {
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCalGeom") << "zStart " << HGCalDD4HepHelper::convert2mm(zMinBlock_) << " wafer width " << HGCalDD4HepHelper::convert2mm(waferSize_) << " separations " << HGCalDD4HepHelper::convert2mm(waferSepar_) << " sectors " << sectors_ << ":" << convertRadToDeg(alpha_) << ":" << cosAlpha_;
#endif
    ...
    for (unsigned int i = 0; i < slopeB_.size(); ++i)
      edm::LogVerbatim("HGCalGeom") << "Bottom Block [" << i << "] Zmin " << HGCalDD4HepHelper::convert2mm(zFrontB_[i]) << " Rmin " << HGCalDD4HepHelper::convert2mm(rMinFront_[i]) << " Slope " << slopeB_[i];
    for (unsigned int i = 0; i < slopeT_.size(); ++i)
      edm::LogVerbatim("HGCalGeom") << "Top Block [" << i << "] Zmin " << HGCalDD4HepHelper::convert2mm(zFrontT_[i]) << " Rmax " << HGCalDD4HepHelper::convert2mm(rMaxFront_[i]) << " Slope " << slopeT_[i];
    ...
    for (unsigned int i = 0; i < layers_.size(); ++i) {
      ...
      for (int ly = laymin; ly < laymax; ++ly) {
...
#ifdef EDM_ML_DEBUG
        edm::LogVerbatim("HGCalGeom") << "DDHGCalEEFileAlgo: Layer " << ly << ":" << ii << " Front " << HGCalDD4HepHelper::convert2mm(zi) << ", " << HGCalDD4HepHelper::convert2mm(routF) << " Back " << HGCalDD4HepHelper::convert2mm(zo) << ", " << HGCalDD4HepHelper::convert2mm(rinB) << " superlayer thickness " << HGCalDD4HepHelper::convert2mm(layerThick_[i]);
#endif
        ...
        if (layerSense_[ly] < 1) {
          ...
          if (layerSense_[ly] == 0 || absorbMode_ == 0) {
            ...
            HGCalGeomTools::radius(zz - hthick,
                                   ...
                                   pgonRout);
#ifdef EDM_ML_DEBUG
            edm::LogVerbatim("HGCalGeom") << "DDHGCalEEFileAlgo: z " << HGCalDD4HepHelper::convert2mm((zz - hthick)) << ":" << HGCalDD4HepHelper::convert2mm((zz + hthick)) << " with " << pgonZ.size() << " palnes";
        ...
        ++copyNumber_[ii];
#ifdef EDM_ML_DEBUG
        edm::LogVerbatim("HGCalGeom") << "DDHGCalEEFileAlgo: " << glog.name() << " number " << copy << " positioned in " << mother.name() << " at (0,0," << HGCalDD4HepHelper::convert2mm(zz) << ") with no rotation";
#endif
        zz += hthick;
    ...
    for (int u = -N; u <= N; ++u) {
      for (int v = -N; v <= N; ++v) {
        ...
        if (((corner.first <= 0) && std::abs(u) < 5 && std::abs(v) < 5) || (std::abs(u) < 2 && std::abs(v) < 2)) {
          edm::LogVerbatim("HGCalGeom") << "DDHGCalEEFileAlgo: " << glog.name() << " R " << HGCalDD4HepHelper::convert2mm(rin) << ":" << HGCalDD4HepHelper::convert2mm(rout) << "\n Z " << HGCalDD4HepHelper::convert2mm(zpos) << " LayerType " << layertype << " u " << u << " v " << v << " with " << corner.first << " corners";
        }
#endif
...
#ifdef EDM_ML_DEBUG
            ++ntype[type];
            edm::LogVerbatim("HGCalGeom") << " DDHGCalEEFileAlgo: " << wafers_[type] << " number " << copy << " type " << layertype << ":" << type << " positioned in " << glog.name() << " at (" << HGCalDD4HepHelper::convert2mm(xpos) << ", " << HGCalDD4HepHelper::convert2mm(ypos) << ", 0) with no rotation";
#endif
          }
        }
      }
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCalGeom") << "DDHGCalEEFileAlgo: Maximum # of u " << ium << ":" << iumAll << " # of v " << ivm << ":" << ivmAll << " and " << nin << ":" << kount << ":" << ntot << " wafers (" << ntype[0] << ":" << ntype[1] << ":" << ntype[2] << ":" << ntype[3] << ":" << ntype[4] << ":" << ntype[5] << ") for " << glog.name() << " R " << HGCalDD4HepHelper::convert2mm(rin) << ":" << HGCalDD4HepHelper::convert2mm(rout);
#endif
  }
