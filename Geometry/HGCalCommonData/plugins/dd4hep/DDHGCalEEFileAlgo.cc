...
struct HGCalEEFileAlgo {
  ...
  HGCalEEFileAlgo(cms::DDParsingContext& ctxt, xml_h e) {
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCalGeom") << "zStart " << HGCalDD4HepHelper::convert2mm(zMinBlock_) << " wafer width "
    ...
    for (unsigned int i = 0; i < slopeB_.size(); ++i)
      edm::LogVerbatim("HGCalGeom") << "Bottom Block [" << i << "] Zmin " << HGCalDD4HepHelper::convert2mm(zFrontB_[i])
                                    << " Rmin " << HGCalDD4HepHelper::convert2mm(rMinFront_[i]) << " Slope "
                                    << slopeB_[i];
    for (unsigned int i = 0; i < slopeT_.size(); ++i)
      edm::LogVerbatim("HGCalGeom") << "Top Block [" << i << "] Zmin " << HGCalDD4HepHelper::convert2mm(zFrontT_[i])
        ...
        if (layerSense_[ly] < 1) {
          ...
          if (layerSense_[ly] == 0 || absorbMode_ == 0) {
            ...
            edm::LogVerbatim("HGCalGeom")
                << "DDHGCalEEFileAlgo: z " << HGCalDD4HepHelper::convert2mm((zz - hthick)) << ":"
  ...
  void positionSensitive(cms::DDParsingContext& ctxt,
    ...
    edm::LogVerbatim("HGCalGeom") << "DDHGCalEEFileAlgo: " << glog.name() << " rin:rout "
                                  << HGCalDD4HepHelper::convert2mm(rin) << ":" << HGCalDD4HepHelper::convert2mm(rout)
                                  << " zpos " << HGCalDD4HepHelper::convert2mm(zpos) << " N " << N
                                  << " for maximum u, v;  r " << HGCalDD4HepHelper::convert2mm(r) << " R "
    ...
    for (int u = -N; u <= N; ++u) {
      for (int v = -N; v <= N; ++v) {
        ...
        if (((corner.first <= 0) && std::abs(u) < 5 && std::abs(v) < 5) || (std::abs(u) < 2 && std::abs(v) < 2)) {
          edm::LogVerbatim("HGCalGeom") << "DDHGCalEEFileAlgo: " << glog.name() << " R "
        ...
        if (corner.first > 0 && type >= 0) {
          ...
          if (corner.first == (int)(HGCalParameters::k_CornerSize)) {
            ...
            edm::LogVerbatim("HGCalGeom")
                ...
                << " positioned in " << glog.name() << " at (" << HGCalDD4HepHelper::convert2mm(xpos) << ", "
