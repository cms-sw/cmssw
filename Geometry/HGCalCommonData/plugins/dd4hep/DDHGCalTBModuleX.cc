...
namespace DDHGCalGeom {
  void constructLayers(const cms::DDNamespace& ns,
    ...
    edm::LogVerbatim("HGCalGeom") << "DDHGCalTBModuleX: \t\tInside Block " << block << " Layers " << firstLayer << ":"
				  << lastLayer << " zFront " << convertCmToMm(zFront) << " thickness " << convertCmToMm(totalWidth) << " ignore Center "
				  << ignoreCenter;
#endif
...
#ifdef EDM_ML_DEBUG
      edm::LogVerbatim("HGCalGeom") << "DDHGCalTBModuleX: " << name << " Layer " << ly << ":" << ii << " Z " << convertCmToMm(zi) << ":"
				    << convertCmToMm(zo) << " Thick " << convertCmToMm(layerThick[ii]) << " Sense " << layerSense[ly];
#endif
...
#ifdef EDM_ML_DEBUG
	edm::LogVerbatim("HGCalGeom") << "DDHGCalTBModuleX: " << solid.name() << " box of dimension " << convertCmToMm(absorbW) << ":"
				      << convertCmToMm(absorbH) << ":" << convertCmToMm(0.5 * layerThick[ii]);
#endif
...
#ifdef EDM_ML_DEBUG
	int incm(0), inrm(0);
	edm::LogVerbatim("HGCalGeom") << module.name() << " Copy " << copy << " Type " << layerSense[ly] << " rout " << convertCmToMm(rMax)
				      << " Row " << nrow << " column " << ncol << " ncrMax " << maxModule[ly] << " Z " << convertCmToMm(zz)
	  ...
	  for (int nc = -ncol; nc <= ncol; ++nc) {
		...
		if (layerSense[ly] == 1) {
		  ...
		  edm::LogVerbatim("HGCalGeom")
		    << "DDHGCalTBModuleX: " << glog1.name() << " number " << copy << " positioned in " << module.name()
...
#ifdef EDM_ML_DEBUG
		  edm::LogVerbatim("HGCalGeom") << "DDHGCalTBModuleX: " << glog2.name() << " number " << copyx << " positioned in "
...
static long algorithm(dd4hep::Detector& /* description */,
  ...
  for (unsigned int i = 0; i < names.size(); ++i)
    edm::LogVerbatim("HGCalGeom") << "Volume [" << i << "] " << names[i] << " of thickness " << convertCmToMm(layerThick[i])
  ...
  for (unsigned int i = 0; i < blockThick.size(); ++i) {
    if (inOut > 1)
      edm::LogVerbatim("HGCalGeom") << "Block [" << i << "] of thickness " << convertCmToMm(blockThick[i]) << " with inner layers "
...
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("HGCalGeom") << "DDHGCalTBModuleX: zStart " << convertCmToMm(zMinBlock) << " rFineCoarse " << convertCmToMm(rMaxFine)
                                << " wafer width " << convertCmToMm(waferW) << " gap among wafers " << convertCmToMm(waferGap) << " absorber width "
                                << convertCmToMm(absorbW) << " absorber height " << convertCmToMm(absorbH) << " rMax " << convertCmToMm(rMax) << ":" << convertCmToMm(rMaxB);
  edm::LogVerbatim("HGCalGeom") << "DDHGCalTBModuleX: NameSpace " << ns.name() << " Parent Name "
                                << idName;
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCalGeom") << "DDHGCalTBModuleX: Block " << i << ":" << name << " z " << convertCmToMm(zi) << ":" << convertCmToMm(zo) << " R "
                                  << convertCmToMm(rMaxB) << " T " << convertCmToMm(blockThick[i]);
#endif
...
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("HGCalGeom") << "DDHGCalTBModuleX: All blocks are placed in " << convertCmToMm(zMinBlock) << ":" << convertCmToMm(zi);
#endif
