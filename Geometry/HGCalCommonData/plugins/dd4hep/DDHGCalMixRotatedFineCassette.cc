...
struct HGCalMixRotatedFineCassette {
  ...
  HGCalMixRotatedFineCassette(cms::DDParsingContext& ctxt, xml_h e) {
    ...
    for (unsigned int i = 0; i < slopeB_.size(); ++i)
      edm::LogVerbatim("HGCalGeom") << "Bottom Block [" << i << "] Zmin " << zFrontB_[i] << " Rmin " << rMinFront_[i] << " Slope " << slopeB_[i];
    for (unsigned int i = 0; i < slopeT_.size(); ++i)
      edm::LogVerbatim("HGCalGeom") << "Top Block [" << i << "] Zmin " << zFrontT_[i] << " Rmax " << rMaxFront_[i] << " Slope " << slopeT_[i];
#endif
    waferFull_ = args.value<std::vector<std::string>>("WaferNamesFull");
    waferPart_ = args.value<std::vector<std::string>>("WaferNamesPartial");
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCalGeom") << "DDHGCalMixRotatedFineCassette: " << waferFull_.size() << " full and " << waferPart_.size() << " partial modules\nDDHGCalMixRotatedFineCassette:Full Modules:";
    ...
    for (unsigned int i = 0; i < names_.size(); ++i)
      edm::LogVerbatim("HGCalGeom") << "Volume [" << i << "] " << names_[i] << " of thickness " << thick_[i] << " filled with " << materials_[i] << " first copy number " << copyNumber_[i];
#endif
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCalGeom") << "DDHGCalMixRotatedFineCassette: " << materialTop_.size() << " types of volumes in the top part; cover Type " << coverTypeTop_ << " with initial copy number " << copyNumberCoverTop_;
    for (unsigned int i = 0; i < materialTop_.size(); ++i)
      edm::LogVerbatim("HGCalGeom") << "Volume [" << i << "] " << namesTop_[i] << " of thickness " << layerThickTop_[i] << " filled with " << materialTop_[i] << " first copy number " << copyNumberTop_[i];
    ...
    edm::LogVerbatim("HGCalGeom") << "DDHGCalMixRotatedFineCassette: " << cassetteShift_.size() << " elements for cassette shifts";
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCalGeom") << "DDHGCalMixRotatedFineCassette:: with " << tileFineRMin_.size() << ":" << tileCoarseRMin_.size() << " rings";
    ...
    edm::LogVerbatim("HGCalGeom") << "TileProperties with " << tileFineIndex_.size() << ":" << tileCoarseIndex_.size() << " entries in " << tileFineLayerStart_.size() << ":" << tileCoarseLayerStart_.size() << " layers";
...
#ifdef EDM_ML_DEBUG
	edm::LogVerbatim("HGCalGeom") << "DDHGCalMixRotatedFineCassette: Layer " << ly << ":" << ii << " Front " << cms::convert2mm(zi)  << ", " << cms::convert2mm(routF) << " Back " << cms::convert2mm(zo) << ", " << cms::convert2mm(rinB) << " superlayer thickness " << layerThick_[i];
#endif
	...
	if (layerSense_[ly] == 0) {
	  ...
	  HGCalGeomTools::radius(zz - hthick, zz + hthick, zFrontB_, rMinFront_, slopeB_, zFrontT_, rMaxFront_, slopeT_, -layerSense_[ly], pgonZ, pgonRin, pgonRout);
	  for (unsigned int isec = 0; isec < pgonZ.size(); ++isec) {
	    ...
	    if (layerSense_[ly] == 0 || absorbMode_ == 0)
          ...
          ns.addVolumeNS(glog);
#ifdef EDM_ML_DEBUG
	  edm::LogVerbatim("HGCalGeom") << "DDHGCalMixRotatedFineCassette: " << solid.name() << " polyhedra of " << sectors_ << " sectors covering " << convertRadToDeg(-alpha_) << ":" << convertRadToDeg(-alpha_ + 2._pi) << " with " << pgonZ.size() << " sections";
	  ...
	  double routs = (mode < 1) ? routF : HGCalGeomTools::radius(zz - hthick, zFrontT_, rMaxFront_, slopeT_);
          ...
          ns.addVolumeNS(glog);
#ifdef EDM_ML_DEBUG
	  edm::LogVerbatim("HGCalGeom") << "DDHGCalMixRotatedFineCassette: " << solid.name() << " Tubs made of " << matter.name() << " of dimensions " << cms::convert2mm(rinB) << ":" << cms::convert2mm(rins) << ", " << cms::convert2mm(routF) << ":" << cms::convert2mm(routs) << ", " << cms::convert2mm(hthick) << ", 0.0, 360.0 and positioned in: " << glog.name()  << " number " << copy;
#endif
	...
	++copyNumber_[ii];
#ifdef EDM_ML_DEBUG
	edm::LogVerbatim("HGCalGeom") << "DDHGCalMixRotatedFineCassette: " << glog.name() << " number " << copy << " positioned in " << mother.name() << " at (0,0," << cms::convert2mm(zz) << ") with no rotation";
#endif
	zz += hthick;
	...
	if (thickTot > layerThick_[i]) {
	  edm::LogError("HGCalGeom") << "Thickness of the partition " << cms::convert2mm(layerThick_[i]) << " is smaller than " << cms::convert2mm(thickTot) << ": thickness of all its components **** ERROR ****";
	} else {
	  edm::LogWarning("HGCalGeom") << "Thickness of the partition " << cms::convert2mm(layerThick_[i]) << " does not match with " << cms::convert2mm(thickTot) << " of the components";
	}
      }
  ...
  void positionMix(cms::DDParsingContext& ctxt, xml_h e, const dd4hep::Volume& glog, const std::string& nameM, int copyM, double thick, const dd4hep::Material& matter, int absType, bool fine) {
	...
	int lastTile = (fine) ? ((layer + 1 < static_cast<int>(tileFineLayerStart_.size())) ? tileFineLayerStart_[layer + 1] : static_cast<int>(tileFineIndex_.size())) : ((layer + 1 < static_cast<int>(tileCoarseLayerStart_.size())) ? tileCoarseLayerStart_[layer + 1] : static_cast<int>(tileCoarseIndex_.size()));
#ifdef EDM_ML_DEBUG
	edm::LogVerbatim("HGCalGeom") << "DDHGCalMixRotatedFineCassette: Layer " << ly << ":" << ii << " Copy " << copy << " Tiles " << firstTile << ":" << lastTile;
#endif
	for (int ti = firstTile; ti < lastTile; ++ti) {
	  ...
	  int ir1 = (fine) ? std::get<1>(HGCalTileIndex::tileUnpack(tileFineIndex_[ti])) : std::get<1>(HGCalTileIndex::tileUnpack(tileCoarseIndex_[ti]));
	  int ir2 = (fine) ? std::get<2>(HGCalTileIndex::tileUnpack(tileFineIndex_[ti])) : std::get<2>(HGCalTileIndex::tileUnpack(tileCoarseIndex_[ti]));
	  edm::LogVerbatim("HGCalGeom") << "DDHGCalMixRotatedFineCassette: Layer " << copy << ":" << (layer + 1) << " iR " << ir1 << ":" << ir2 << " R " << cms::convert2mm(r1) << ":" << cms::convert2mm(r2) << " Thick " << cms::convert2mm(2.0 * hthickl) << " phi " << fimin << ":" << fimax << ":" << convertRadToDeg(phi1) << ":" << convertRadToDeg(phi2) << " cassette " << cassette << ":" << cassette0 << " Shift " << cms::convert2mm(cshift.first) << ":" << cms::convert2mm(cshift.second);
#endif
...
#ifdef EDM_ML_DEBUG
	  edm::LogVerbatim("HGCalGeom") << "DDHGCalMixRotatedFineCassette: " << glog1.name() << " Tubs made of " << matter1.name() << " of dimensions " << cms::convert2mm(r1) << ", " << cms::convert2mm(r2) << ", " << cms::convert2mm(hthickl) << ", " << convertRadToDeg(phi1) << ", " << convertRadToDeg(phi2);
#endif
          dd4hep::Position tran(-cshift.first, cshift.second, zpos);
          glog.placeVolume(glog1, copy, tran);
#ifdef EDM_ML_DEBUG
	  edm::LogVerbatim("HGCalGeom") << "DDHGCalMixRotatedFineCassette: Position " << glog1.name() << " number " << copy << " in " << glog.name() << " at ("<< cms::convert2mm(cshift.first) << ", " << cms::convert2mm(cshift.second) << ", " << cms::convert2mm(zpos) << ") with no rotation";
#endif
	}
	++copyNumberTop_[ii];
	zpos += hthickl;
	...
	if (thickTot > thick) {
	  edm::LogError("HGCalGeom") << "DDHGCalMixRotatedFineCassette: Thickness of the partition " << cms::convert2mm(thick) << " is smaller than " << cms::convert2mm(thickTot) << ": thickness of all its components in the top part **** ERROR ****";
	} else {
	  edm::LogWarning("HGCalGeom") << "DDHGCalMixRotatedFineCassette: Thickness of the partition " << cms::convert2mm(thick) << " does not match with " << cms::convert2mm(thickTot) << " of the components in top part";
	}
      }
    } else {
...
#ifdef EDM_ML_DEBUG
	edm::LogVerbatim("HGCalGeom") << "DDHGCalMixRotatedFineCassette::Passive: layer " << layer + 1 << " cassette " << cassette << " Shift " << cms::convert2mm(-cshift.first) << ":" << cms::convert2mm(cshift.second) << " PassiveIndex " << i << ":" << passiveFull_.size() << ":" << passivePart_.size();
#endif
	std::string passive = (absType <= waferTypes_) ? passiveFull_[i] : passivePart_[i];
#ifdef EDM_ML_DEBUG
	edm::LogVerbatim("HGCalGeom") << "DDHGCalMixRotatedFineCassette: Passive " << passive << " number " << cassette << " pos " << cms::convert2mm(xpos) << ":" << cms::convert2mm(ypos);
	kount++;
#endif
	dd4hep::Position tran(xpos, ypos, 0.0);
	glog.placeVolume(ns.volume(passive), cassette, tran);
#ifdef EDM_ML_DEBUG
	edm::LogVerbatim("HGCalGeom") << "DDHGCalMixRotatedFineCassette: " << passive << " number " << cassette << " positioned in " << glog.name() << " at (" << cms::convert2mm(xpos) << "," << cms::convert2mm(ypos) << ",0) with no rotation";
#endif
      }
#ifdef EDM_ML_DEBUG
      edm::LogVerbatim("HGCalGeom") << "DDHGCalMixRotatedFineCassette: " << kount << " passives of type " << absType << " for " << glog.name();
#endif
    } else {
      ...
      int lastWafer = (((layer + 1) < static_cast<int>(waferLayerStart_.size())) ? waferLayerStart_[layer + 1] : static_cast<int>(waferIndex_.size()));
      ...
      edm::LogVerbatim("HGCalGeom") << "DDHGCalMixRotatedFineCassette::Bottom: " << glog.name() << "  r " << cms::convert2mm(delx) << " R " << cms::convert2mm(dely) << " dy " << cms::convert2mm(dy) << " Shift " << cms::convert2mm(xyoff.first) << ":" << cms::convert2mm(xyoff.second) << " WaferSize " << cms::convert2mm(waferSize_ + waferSepar_) << " index " << firstWafer << ":" << (lastWafer - 1) << " Copy " << copyM << ":" << layer;
#endif
...
#ifdef EDM_ML_DEBUG
	edm::LogVerbatim("HGCalGeom") << "DDHGCalMixRotatedFineCassette::index:Property:layertype:type:part:orien:cassette:place:offsets:ind " << k << waferProperty_[k] << ":" << layertype << ":" << type << ":" << part << ":" << orien << ":" << cassette << ":" << place;
#endif
	...
	edm::LogVerbatim("HGCalGeom") << "DDHGCalMixRotatedFineCassette::Wafer: layer " << layer + 1 << " cassette " << cassette << " Shift " << cms::convert2mm(cshift.first) << ":" << cms::convert2mm(cshift.second) << " Original " << cms::convert2mm(xorig) << ":" << cms::convert2mm(yorig) << ":" << convertRadToDeg(angle) << " Final " << cms::convert2mm(xpos) << ":" << cms::convert2mm(ypos);
#endif
	std::string wafer;
	int i(999);
...
#ifdef EDM_ML_DEBUG
	++ntype[type];
	edm::LogVerbatim("HGCalGeom") << " DDHGCalMixRotatedFineCassette: " << wafer << " number " << copy << " type " << layertype << ":" << type << " positioned in " << glog.name() << " at (" << cms::convert2mm(xpos) << "," << cms::convert2mm(ypos) << ",0) with no rotation";
#endif
      }
#ifdef EDM_ML_DEBUG
