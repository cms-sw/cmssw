...
static long algorithm(dd4hep::Detector& /* description */, cms::DDParsingContext& ctxt, xml_h e) {
  ...
  for (unsigned int i = 0; i < layers.size(); ++i)
    edm::LogVerbatim("HGCalGeom") << "Block [" << i << "] of thickness " << (f2mm * layerThick[i]) << " with " << layers[i]
                                  << " layers";
#endif
...
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("HGCalGeom") << "DDHGCalTBModule: zStart " << (f2mm * zMinBlock) << " rFineCoarse " << (f2mm * rMaxFine)
                                << " wafer width " << (f2mm * waferW) << " gap among wafers " << (f2mm * waferGap) << " absorber width "
                                << (f2mm * absorbW) << " absorber height " << (f2mm * absorbH);
#endif
  ...
  for (unsigned int i = 0; i < slopeT.size(); ++i)
    edm::LogVerbatim("HGCalGeom") << "Block [" << i << "] Zmin " << (f2mm * zFront[i]) << " Rmax " << (f2mm * rMaxFront[i]) << " Slope "
                                  << slopeT[i];
#endif
  ...
  for (unsigned int i = 0; i < layers.size(); i++) {
    ...
    for (int ly = laymin; ly < laymax; ++ly) {
...
#ifdef EDM_ML_DEBUG
      edm::LogVerbatim("HGCalGeom") << "DDHGCalTBModule: Layer " << ly << ":" << ii << " Front " << (f2mm * zi) << ", " << (f2mm * routF)
                                    << " Back " << (f2mm * zo) << ", " << (f2mm * rinB) << " superlayer thickness " << (f2mm * layerThick[i]);
#endif
      ...
      if (layerSense[ly] == 0) {
...
#ifdef EDM_ML_DEBUG
        edm::LogVerbatim("HGCalGeom") << "DDHGCalTBModule test: " << solid.name() << " box of dimension " << (f2mm * absorbW)
                                      << ":" << (f2mm * absorbH) << ":" << (f2mm * 0.5 * thick[ii]);
#endif
      } else {
        ...
        edm::LogVerbatim("HGCalGeom") << "DDHGCalTBModule: " << solid.name() << " Tubs made of " << materials[ii]
                                      << " of dimensions " << (f2mm * rinB) << ", " << (f2mm * routF) << ", " << (f2mm * 0.5 * thick[ii])
                                      << ", 0.0, 360.0";
#endif
        ...
        edm::LogVerbatim("HGCalGeom") << glog.name() << " rout " << (f2mm * routF) << " Row " << nrow << " Column " << ncol;
#endif
        double xc[6], yc[6];
            ...
            if (inr % 2 == inc % 2) {
              ...
              if (cornerAll) {
                ...
                edm::LogVerbatim("HGCalGeom") << "DDHGCalModule: " << glog1.name() << " number " << copyL
                                              << " positioned in " << glog.name() << " at (" << (f2mm * xpos) << ", " << (f2mm * ypos) << ", 0)";
#endif
              }
            }
          }
        ...
        edm::LogVerbatim("HGCalGeom") << "DDHGCalModule: # of columns " << incm << " # of rows " << inrm << " and "
                                      ...
                                      << ") for " << glog.name() << " R " << (f2mm * rinB) << ":" << (f2mm * routF);
#endif
      }
      ...
      edm::LogVerbatim("HGCalGeom") << "DDHGCalTBModule test: " << glog.name() << " number " << copy
                                    << " positioned in " << module.name() << " at (0, 0, " << (f2mm * zz) << ")";
#endif
      zz += (0.5 * thick[ii]);
    ...
    if (fabs(thickTot - layerThick[i]) > tol2) {
      if (thickTot > layerThick[i]) {
        edm::LogError("HGCalGeom") << "Thickness of the partition " << (f2mm * layerThick[i]) << " is smaller than thickness "
                                   << (f2mm * thickTot) << " of all its components **** ERROR ****\n";
      } else {
        edm::LogWarning("HGCalGeom") << "Thickness of the partition " << (f2mm * layerThick[i]) << " does not match with "
                                     << (f2mm * thickTot) << " of the components\n";
      }
    }
