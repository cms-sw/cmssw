  ...
  for (unsigned int i = 0; i < layers.size(); i++) {
    ...
    if (fabs(thickTot - layerThick[i]) > tol2) {
      if (thickTot > layerThick[i]) {
        edm::LogError("HGCalGeom") << "DDHGCalModule::Thickness of the partition " << cms::convert2mm(layerThick[i])
                                   << " is smaller than thickness " << cms::convert2mm(thickTot)
                                   << " of all its components **** ERROR ****\n";
      } else {
        edm::LogWarning("HGCalGeom") << "DDHGCalModule::Thickness of the partition " << cms::convert2mm(layerThick[i])
