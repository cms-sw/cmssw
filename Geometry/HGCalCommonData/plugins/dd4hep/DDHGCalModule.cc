...
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("HGCalGeom") << "DDHGCalModule: zStart " << (f2mm * zMinBlock) << " rFineCoarse "
                                << (f2mm * rMaxFine) << " wafer width " << (f2mm * waferW) << " sectors " << sectors;
#endif
...
#ifdef EDM_ML_DEBUG
      edm::LogVerbatim("HGCalGeom") << "DDHGCalModule: Layer " << ly << ":" << ii << " Front " << (f2mm * zi) << ", "
