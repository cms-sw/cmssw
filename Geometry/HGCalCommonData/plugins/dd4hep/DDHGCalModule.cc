...
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("HGCalGeom") << "DDHGCalModule: zStart " << cms::convert2mm(zMinBlock) << " rFineCoarse "
  ...
  for (unsigned int i = 0; i < layers.size(); i++) {
...
#ifdef EDM_ML_DEBUG
      edm::LogVerbatim("HGCalGeom") << "DDHGCalModule: Layer " << ly << ":" << ii << " Front " << cms::convert2mm(zi)
              ...
              if (corner.first > 0) {
                ...
                if (corner.first == (int)(HGCalParameters::k_CornerSize)) {
                  ...
                  edm::LogVerbatim("HGCalGeom")
                      ...
                      << " at (" << cms::convert2mm(xpos) << "," << cms::convert2mm(ypos) << ",0) with " << rotation;
#endif
                }
              }
            }
    ...
    if (fabs(thickTot - layerThick[i]) > tol2) {
      if (thickTot > layerThick[i]) {
        edm::LogError("HGCalGeom") << "DDHGCalModule::Thickness of the partition " << cms::convert2mm(layerThick[i]) << " is smaller than thickness " << cms::convert2mm(thickTot) << " of all its components **** ERROR ****\n";
      } else {
        edm::LogWarning("HGCalGeom") << "DDHGCalModule::Thickness of the partition " << cms::convert2mm(layerThick[i]) << " does not match with " << cms::convert2mm(thickTot) << " of the components\n";
      }
    }
