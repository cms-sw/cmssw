...
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("HGCalGeom") << "DDHGCalModule: zStart " << (f2mm * zMinBlock) << " rFineCoarse " << (f2mm * rMaxFine)
                                << " wafer width " << (f2mm * waferW) << " sectors " << sectors;
#endif
  ...
  for (unsigned int i = 0; i < layers.size(); i++) {
    ...
    for (int ly = laymin; ly < laymax; ++ly) {
...
#ifdef EDM_ML_DEBUG
      edm::LogVerbatim("HGCalGeom") << "DDHGCalModule: Layer " << ly << ":" << ii << " Front " << (f2mm * zi) << ", " << (f2mm * routF)
      ...
      if (layerSense[ly] == 0) {
        ...
        glog = dd4hep::Volume(solid.name(), solid, matter);
#ifdef EDM_ML_DEBUG
        edm::LogVerbatim("HGCalGeom") << "DDHGCalModule: " << solid.name() << " Tubs made of " << materials[ii] << " of dimensions " << (f2mm * rinB) << ", " << (f2mm * routF) << ", " << (f2mm * 0.5 * thick[ii]) << ", 0.0, 360.0";
