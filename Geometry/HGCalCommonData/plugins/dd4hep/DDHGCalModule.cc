...
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("HGCalGeom") << "DDHGCalModule: zStart " << HGCalDD4HepHelper::convert2mm(zMinBlock) << " rFineCoarse " << HGCalDD4HepHelper::convert2mm(rMaxFine) << " wafer width " << HGCalDD4HepHelper::convert2mm(waferW) << " sectors " << sectors;
#endif
  ...
  for (unsigned int i = 0; i < layers.size(); i++) {
    ...
    for (int ly = laymin; ly < laymax; ++ly) {
...
#ifdef EDM_ML_DEBUG
      edm::LogVerbatim("HGCalGeom") << "DDHGCalModule: Layer " << ly << ":" << ii << " Front " << HGCalDD4HepHelper::convert2mm(zi) << ", " << HGCalDD4HepHelper::convert2mm(routF) << " Back " << HGCalDD4HepHelper::convert2mm(zo) << ", " << HGCalDD4HepHelper::convert2mm(rinB) << " superlayer thickness " << HGCalDD4HepHelper::convert2mm(layerThick[i]);
#endif
      ...
      if (layerSense[ly] == 0) {
        ...
        glog = dd4hep::Volume(solid.name(), solid, matter);
#ifdef EDM_ML_DEBUG
        edm::LogVerbatim("HGCalGeom") << "DDHGCalModule: " << solid.name() << " Tubs made of " << materials[ii] << " of dimensions " << HGCalDD4HepHelper::convert2mm(rinB) << ", " << HGCalDD4HepHelper::convert2mm(routF) << ", " << HGCalDD4HepHelper::convert2mm(0.5 * thick[ii]) << ", 0.0, 360.0";
              ...
              if (corner.first > 0) {
                ...
                if (corner.first == (int)(HGCalParameters::k_CornerSize)) {
                  ...
                  if (rpos < rMaxFine)
                    ...
                    ++ncoarse;
                  edm::LogVerbatim("HGCalGeom") << "DDHGCalModule: " << glog1.name() << " number " << copyL << " positioned in " << glog.name() << " at (" << HGCalDD4HepHelper::convert2mm(xpos) << "," << HGCalDD4HepHelper::convert2mm(ypos) << ",0) with " << rotation;
#endif
                }
              }
            }
      ...
      ++copyNumber[ii];
#ifdef EDM_ML_DEBUG
      edm::LogVerbatim("HGCalGeom") << "DDHGCalModule: " << glog.name() << " number " << copy << " positioned in " << module.name() << " at (0,0," << HGCalDD4HepHelper::convert2mm(zz) << ") with no rotation";
#endif
      zz += (0.5 * thick[ii]);
