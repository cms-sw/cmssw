...
struct HGCalPassive {
  ...
  HGCalPassive(cms::DDParsingContext& ctxt, xml_h e) {
    ...
    std::vector<std::string> tagLayer = args.value<std::vector<std::string>>("TagLayer");   // Tag of the layer (to be added to name)
    std::vector<std::string> tagSector = args.value<std::vector<std::string>>("TagSector"); // Tag of the sector (to be added to name)
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCalGeom") << "DDHGCalPassive: " << tagLayer.size() << " Modules with base name "
				  << parentName << " made of " << material << " T " << thick << " Sectors "
				  << tagSector.size() << " Parts " << parts << " phi0 " << convertRadToDeg(phi0);
    ...
    std::vector<std::string> materials = args.value<std::vector<std::string>>("LayerMaterials");   // Materials of the layers
...
#ifdef EDM_ML_DEBUG
	edm::LogVerbatim("HGCalGeom") << "DDHGCalPassive: " << solid.name() << " extruded polygon made of " << matter.name()
