...
struct HGCalPassive {
  ...
  HGCalPassive(cms::DDParsingContext& ctxt, xml_h e) {
    ...
    edm::LogVerbatim("HGCalGeom") << "DDHGCalPassive: " << tagpos.size() << " Modules with base name " << parentName << " made of " << moduleMaterial << " T " << moduleThick << " having " << sectors << " sectors" << st0.str() << " phi0 " << convertRadToDeg(phi0) << " dphi " << convertRadToDeg(dphi) << " Tags:" << st1.str();
#endif
    ...
    std::vector<std::string> absNames = args.value<std::vector<std::string>>("AbsorberName");  // Names of the absorber layers
	...
	for (unsigned i3 = 0; i3 < absNames.size(); ++i3) {
...
#ifdef EDM_ML_DEBUG
	  edm::LogVerbatim("HGCalGeom") << "DDHGCalPassive: " << solid.name() << " extruded polygon made of " << matter.name() << " z|x|y|s (0) " << zw[0] << ":" << zx[0] << ":" << zy[0] << ":" << scale[0] << " z|x|y|s (1) " << zw[1] << ":" << zx[1] << ":" << zy[1] << ":" << scale[1] << " and " << xM.size() << " edges";
...
#ifdef EDM_ML_DEBUG
	      edm::LogVerbatim("HGCalGeom") << "DDHGCalPassive: Layer " << i << ":" << l << ":" << solid.name() << " extruded polygon made of " << matter.name() << " z|x|y|s (0) " << zw[0] << ":" << zx[0] << ":" << zy[0] << ":" << scale[0]  << " z|x|y|s (1) " << zw[1] << ":" << zx[1] << ":" << zy[1] << ":" << scale[1] << " and " << xM.size() << " edges";
	      for (unsigned int kk = 0; kk < xM.size(); ++kk)
		edm::LogVerbatim("HGCalGeom") << "[" << kk << "] " << xM[kk] << ":" << yM[kk];
#endif
	    }
...
#ifdef EDM_ML_DEBUG
	    edm::LogVerbatim("HGCalGeom") << "DDHGCalPassive: " << glogs[i].name() << " number " << copyNumber[i] << " positioned in " << glogM.name() << " at (0, 0, " << cms::convert2mm(zi + 0.5 * layerThick[i]) << ") with no rotation";
#endif
	    ++copyNumber[i];
	    zi += layerThick[i];
	  ...
	  if ((std::abs(thickTot - moduleThick) >= tol) && (!layerType.empty())) {
	    if (thickTot > moduleThick) {
	      edm::LogError("HGCalGeom") << "Thickness of the partition " << moduleThick << " is smaller than " << thickTot << ": thickness of all its components **** ERROR ****";
	    } else {
	      edm::LogWarning("HGCalGeom") << "Thickness of the partition " << moduleThick << " does not match with " << thickTot << " of the components";
	    }
	  }
	}
      }
