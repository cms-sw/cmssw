...
struct HGCalHEAlgo {
  ...
  HGCalHEAlgo(cms::DDParsingContext& ctxt, xml_h e) {
    ...
    for (unsigned int i = 0; i < volumeNames_.size(); ++i)
      edm::LogVerbatim("HGCalGeom") << "Volume [" << i << "] " << volumeNames_[i] << " of thickness " << HGCalDD4HepHelper::convert2mm(thickness_[i]) << " filled with " << materials_[i] << " first copy number " << copyNumber_[i];
#endif
    ...
    for (unsigned int i = 0; i < layerNumbers_.size(); ++i)
      edm::LogVerbatim("HGCalGeom") << "Block [" << i << "] of thickness " << HGCalDD4HepHelper::convert2mm(layerThick_[i]) << " Rmid " << HGCalDD4HepHelper::convert2mm(rMixLayer_[i]) << " with " << layerNumbers_[i] << " layers";
#endif
    ...
    for (unsigned int i = 0; i < materialsTop_.size(); ++i)
      edm::LogVerbatim("HGCalGeom") << "Volume [" << i << "] " << namesTop_[i] << " of thickness " << HGCalDD4HepHelper::convert2mm(layerThickTop_[i]) << " filled with " << materialsTop_[i] << " first copy number " << copyNumberTop_[i];
    ...
    for (unsigned int i = 0; i < materialsBot_.size(); ++i)
      edm::LogVerbatim("HGCalGeom") << "Volume [" << i << "] " << namesBot_[i] << " of thickness " << HGCalDD4HepHelper::convert2mm(layerThickBot_[i]) << " filled with " << materialsBot_[i] << " first copy number " << copyNumberBot_[i];
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCalGeom") << "DDHGCalHEAlgo: zStart " << HGCalDD4HepHelper::convert2mm(zMinBlock_) << " radius for wafer type separation uses " << rad100to200_.size() << " parameters; zmin " << HGCalDD4HepHelper::convert2mm(zMinRadPar_) << " cutoff " << choiceType_ << ":"  << nCutRadPar_ << ":" << fracAreaMin_ << " wafer width " << HGCalDD4HepHelper::convert2mm(waferSize_) << " separations " << HGCalDD4HepHelper::convert2mm(waferSepar_) << " sectors " << sectors_ << ":" << convertRadToDeg(alpha_) << ":" << cosAlpha_;
    ...
    for (unsigned int i = 0; i < slopeB_.size(); ++i)
      edm::LogVerbatim("HGCalGeom") << "Block [" << i << "] Zmin " << HGCalDD4HepHelper::convert2mm(zFrontB_[i]) << " Rmin " << HGCalDD4HepHelper::convert2mm(rMinFront_[i]) << " Slope " << slopeB_[i];
    for (unsigned int i = 0; i < slopeT_.size(); ++i)
      edm::LogVerbatim("HGCalGeom") << "Block [" << i << "] Zmin " << HGCalDD4HepHelper::convert2mm(zFrontT_[i]) << " Rmax " << HGCalDD4HepHelper::convert2mm(rMaxFront_[i]) << " Slope " << slopeT_[i];
    ...
    for (unsigned int i = 0; i < layerNumbers_.size(); i++) {
      ...
      for (int ly = laymin; ly < laymax; ++ly) {
...
#ifdef EDM_ML_DEBUG
        edm::LogVerbatim("HGCalGeom") << "DDHGCalHEAlgo: Layer " << ly << ":" << ii << " Front " << HGCalDD4HepHelper::convert2mm(zi) << ", " << HGCalDD4HepHelper::convert2mm(routF) << " Back " << HGCalDD4HepHelper::convert2mm(zo) << ", " << HGCalDD4HepHelper::convert2mm(rinB) << " superlayer thickness " << HGCalDD4HepHelper::convert2mm(layerThick_[i]);
#endif
        ...
        if (layerSense_[ly] < 1) {
          ...
          for (unsigned int k = 0; k < pgonZ.size(); ++k)
            edm::LogVerbatim("HGCalGeom") << "[" << k << "] z " << HGCalDD4HepHelper::convert2mm(pgonZ[k]) << " R " << HGCalDD4HepHelper::convert2mm(pgonRin[k]) << ":" << HGCalDD4HepHelper::convert2mm(pgonRout[k]);
#endif
        } else {
          double rins =
...
#ifdef EDM_ML_DEBUG
          edm::LogVerbatim("HGCalGeom") << "DDHGCalHEAlgo: " << solid.name() << " Tubs made of " << matter.name() << " of dimensions " << HGCalDD4HepHelper::convert2mm(rinB) << ":" << HGCalDD4HepHelper::convert2mm(rins) << ", " << HGCalDD4HepHelper::convert2mm(routF) << ":" << HGCalDD4HepHelper::convert2mm(routs) << ", " << HGCalDD4HepHelper::convert2mm(hthick) << ", 0.0, 360.0 and positioned in: " << glog.name() << " number " << copy;
#endif
        ...
        ++copyNumber_[ii];
#ifdef EDM_ML_DEBUG
        edm::LogVerbatim("HGCalGeom") << "DDHGCalHEAlgo: " << glog.name() << " number " << copy << " positioned in " << mother.name() << " at (0,0," << HGCalDD4HepHelper::convert2mm(zz) << ") with no rotation";
#endif
        zz += hthick;
      ...
      if (std::abs(thickTot - layerThick_[i]) >= tol2_) {
        if (thickTot > layerThick_[i]) {
          edm::LogError("HGCalGeom") << "Thickness of the partition " << HGCalDD4HepHelper::convert2mm(layerThick_[i]) << " is smaller than "
                                     << HGCalDD4HepHelper::convert2mm(thickTot) << ": thickness of all its components **** ERROR ****";
        } else {
          edm::LogWarning("HGCalGeom") << "Thickness of the partition " << HGCalDD4HepHelper::convert2mm(layerThick_[i])
                                       << " does not match with " << HGCalDD4HepHelper::convert2mm(thickTot) << " of the components";
        }
      }
  ...
  void positionMix(cms::DDParsingContext& ctxt,
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCalGeom") << "DDHGCalHEAlgo: " << solid.name() << " Tubs made of " << matter.name() << " of dimensions " << HGCalDD4HepHelper::convert2mm(rmid) << ", " << HGCalDD4HepHelper::convert2mm(rout) << ", " << HGCalDD4HepHelper::convert2mm(hthick) << ", 0.0, 360.0";
#endif
    ...
    for (unsigned int ly = 0; ly < layerTypeTop_.size(); ++ly) {
...
#ifdef EDM_ML_DEBUG
      edm::LogVerbatim("HGCalGeom") << "DDHGCalHEAlgo: Layer " << ly << ":" << ii << " R " << HGCalDD4HepHelper::convert2mm(rmid) << ":" << HGCalDD4HepHelper::convert2mm(rout) << " Thick " << HGCalDD4HepHelper::convert2mm(layerThickTop_[ii]);
#endif
      ...
      edm::LogVerbatim("HGCalGeom") << name << " z|rin|rout " << HGCalDD4HepHelper::convert2mm(zz) << ":" << HGCalDD4HepHelper::convert2mm(rmid) << ":" << HGCalDD4HepHelper::convert2mm(rout) << " eta " << eta1 << ":" << eta2;
      edm::LogVerbatim("HGCalGeom") << "DDHGCalHEAlgo: " << solid.name() << " Tubs made of " << matter1.name() << " of dimensions " << HGCalDD4HepHelper::convert2mm(rmid) << ", " << HGCalDD4HepHelper::convert2mm(rout) << ", " << HGCalDD4HepHelper::convert2mm(hthickl) << ", 0.0, 360.0";
#endif
      zpos += hthickl;
...
#ifdef EDM_ML_DEBUG
      edm::LogVerbatim("HGCalGeom") << "DDHGCalHEAlgo: Position " << glog2.name() << " number " << copy << " in " << glog1.name() << " at (0,0," << HGCalDD4HepHelper::convert2mm(zpos) << ") with no rotation";
#endif
      ++copyNumberTop_[ii];
      zpos += hthickl;
    }
    if (std::abs(thickTot - thick) >= tol2_) {
      if (thickTot > thick) {
        edm::LogError("HGCalGeom") << "Thickness of the partition " << HGCalDD4HepHelper::convert2mm(thick) << " is smaller than "
                                   << HGCalDD4HepHelper::convert2mm(thickTot)
                                   << ": thickness of all its components in the top part **** ERROR ****";
      } else {
        edm::LogWarning("HGCalGeom") << "Thickness of the partition " << HGCalDD4HepHelper::convert2mm(thick) << " does not match with "
                                     << HGCalDD4HepHelper::convert2mm(thickTot) << " of the components in top part";
      }
    }
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCalGeom") << "DDHGCalHEAlgo: " << solid.name() << " Tubs made of " << matter.name() << " of dimensions " << HGCalDD4HepHelper::convert2mm(rin) << ", " << HGCalDD4HepHelper::convert2mm(rmid) << ", " << HGCalDD4HepHelper::convert2mm(hthick) << ", 0.0, 360.0";
#endif
    ...
    for (unsigned int ly = 0; ly < layerTypeBot_.size(); ++ly) {
...
#ifdef EDM_ML_DEBUG
      edm::LogVerbatim("HGCalGeom") << "DDHGCalHEAlgo: Layer " << ly << ":" << ii << " R " << HGCalDD4HepHelper::convert2mm(rin) << ":" << HGCalDD4HepHelper::convert2mm(rmid) << " Thick " << HGCalDD4HepHelper::convert2mm(layerThickBot_[ii]);
#endif
      ...
      edm::LogVerbatim("HGCalGeom") << name << " z|rin|rout " << HGCalDD4HepHelper::convert2mm(zz) << ":" << HGCalDD4HepHelper::convert2mm(rin) << ":" << HGCalDD4HepHelper::convert2mm(rmid) << " eta " << eta1 << ":" << eta2;
      edm::LogVerbatim("HGCalGeom") << "DDHGCalHEAlgo: " << solid.name() << " Tubs made of " << matter1.name() << " of dimensions " << HGCalDD4HepHelper::convert2mm(rin) << ", " << HGCalDD4HepHelper::convert2mm(rmid) << ", " << HGCalDD4HepHelper::convert2mm(hthickl) << ", 0.0, 360.0";
#endif
      zpos += hthickl;
...
#ifdef EDM_ML_DEBUG
        edm::LogVerbatim("HGCalGeom") << "DDHGCalHEAlgo: z " << HGCalDD4HepHelper::convert2mm((zz + zpos)) << " Center " << copy << ":" << (copy - firstLayer_) << ":" << layerCenter_[copy - firstLayer_];
#endif
  ...
  void positionSensitive(cms::DDParsingContext& ctxt,
    ...
    edm::LogVerbatim("HGCalGeom") << "DDHGCalHEAlgo: " << glog.name() << " rout " << HGCalDD4HepHelper::convert2mm(rout) << " N " << N << " for maximum u, v Offset; Shift " << HGCalDD4HepHelper::convert2mm(xyoff.first) << ":" << HGCalDD4HepHelper::convert2mm(xyoff.second) << " WaferSize " << HGCalDD4HepHelper::convert2mm((waferSize_ + waferSepar_));
#endif
    for (int u = -N; u <= N; ++u) {
      for (int v = -N; v <= N; ++v) {
        int nr = 2 * v;
...
#ifdef EDM_ML_DEBUG
            ++ntype[type];
            edm::LogVerbatim("HGCalGeom") << "DDHGCalHEAlgo: " << glog.name() << " number " << copy << " positioned in " << glog.name() << " at (" << HGCalDD4HepHelper::convert2mm(xpos) << "," << HGCalDD4HepHelper::convert2mm(ypos) << ",0) with no rotation";
#endif
          }
        }
      }
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCalGeom") << "DDHGCalHEAlgo: Maximum # of u " << ium << ":" << iumAll << " # of v " << ivm << ":" << ivmAll << " and " << nin << ":" << kount << ":" << ntot << " wafers (" << ntype[0] << ":" << ntype[1] << ":" << ntype[2] << ":" << ntype[3] << ":" << ntype[4] << ":" << ntype[5] << ") for " << glog.name() << " R " << HGCalDD4HepHelper::convert2mm(rin) << ":" << HGCalDD4HepHelper::convert2mm(rout);
#endif
  }
