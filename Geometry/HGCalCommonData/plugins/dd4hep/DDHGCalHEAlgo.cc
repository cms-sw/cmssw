...
struct HGCalHEAlgo {
  ...
  HGCalHEAlgo(cms::DDParsingContext& ctxt, xml_h e) {
    ...
    for (unsigned int i = 0; i < volumeNames_.size(); ++i)
      edm::LogVerbatim("HGCalGeom") << "Volume [" << i << "] " << volumeNames_[i] << " of thickness " << (f2mm_ * thickness_[i])
    ...
    for (unsigned int i = 0; i < layerNumbers_.size(); ++i)
      edm::LogVerbatim("HGCalGeom") << "Block [" << i << "] of thickness " << (f2mm_ * layerThick_[i]) << " Rmid "
                                    << (f2mm_ * rMixLayer_[i]) << " with " << layerNumbers_[i] << " layers";
#endif
    ...
    for (unsigned int i = 0; i < materialsTop_.size(); ++i)
      edm::LogVerbatim("HGCalGeom") << "Volume [" << i << "] " << namesTop_[i] << " of thickness " << (f2mm_ * layerThickTop_[i])
    ...
    for (unsigned int i = 0; i < materialsBot_.size(); ++i)
      edm::LogVerbatim("HGCalGeom") << "Volume [" << i << "] " << namesBot_[i] << " of thickness " << (f2mm_ * layerThickBot_[i])
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCalGeom") << "DDHGCalHEAlgo: zStart " << (f2mm_ * zMinBlock_) << " radius for wafer type separation uses "
                                  << rad100to200_.size() << " parameters; zmin " << (f2mm_ * zMinRadPar_) << " cutoff "
                                  << choiceType_ << ":" << nCutRadPar_ << ":" << fracAreaMin_ << " wafer width "
                                  << (f2mm_ * waferSize_) << " separations " << (f2mm_ * waferSepar_) << " sectors " << sectors_ << ":"
    ...
    for (unsigned int i = 0; i < slopeB_.size(); ++i)
      edm::LogVerbatim("HGCalGeom") << "Block [" << i << "] Zmin " << (f2mm_ * zFrontB_[i]) << " Rmin " << (f2mm_ * rMinFront_[i])
                                    << " Slope " << slopeB_[i];
    for (unsigned int i = 0; i < slopeT_.size(); ++i)
      edm::LogVerbatim("HGCalGeom") << "Block [" << i << "] Zmin " << (f2mm_ * zFrontT_[i]) << " Rmax " << (f2mm_ * rMaxFront_[i])
                                    << " Slope " << slopeT_[i];
      ...
      for (int ly = laymin; ly < laymax; ++ly) {
...
#ifdef EDM_ML_DEBUG
        edm::LogVerbatim("HGCalGeom") << "DDHGCalHEAlgo: Layer " << ly << ":" << ii << " Front " << (f2mm_ * zi) << ", " << (f2mm_ * routF)
                                      << " Back " << (f2mm_ * zo) << ", " << (f2mm_ * rinB) << " superlayer thickness " << (f2mm_ * layerThick_[i]);
#endif
        ...
        if (layerSense_[ly] < 1) {
          ...
          edm::LogVerbatim("HGCalGeom") << "DDHGCalHEAlgo: " << solid.name() << " Tubs made of " << matter.name()
                                        ...
                                        << ", " << (f2mm_ * hthick) << ", 0.0, 360.0 and positioned in: " << glog.name()
                                        << " number " << copy;
#endif
  ...
  void positionMix(cms::DDParsingContext& ctxt,
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCalGeom") << "DDHGCalHEAlgo: " << solid.name() << " Tubs made of " << matter.name()
                                  << " of dimensions " << (f2mm_ * rmid) << ", " << (f2mm_ * rout) << ", " << (f2mm_ * hthick) << ", 0.0, 360.0";
#endif
    ...
    for (unsigned int ly = 0; ly < layerTypeTop_.size(); ++ly) {
...
#ifdef EDM_ML_DEBUG
      edm::LogVerbatim("HGCalGeom") << "DDHGCalHEAlgo: Layer " << ly << ":" << ii << " R " << (f2mm_ * rmid) << ":" << (f2mm_ * rout)
                                    << " Thick " << (f2mm_ * layerThickTop_[ii]);
#endif
      ...
      edm::LogVerbatim("HGCalGeom") << name << " z|rin|rout " << (f2mm_ * zz) << ":" << (f2mm_ * rmid) << ":" << (f2mm_ * rout) << " eta " << eta1
                                    << ":" << eta2;
      edm::LogVerbatim("HGCalGeom") << "DDHGCalHEAlgo: " << solid.name() << " Tubs made of " << matter1.name()
                                    << " of dimensions " << (f2mm_ * rmid) << ", " << (f2mm_ * rout) << ", " << (f2mm_ * hthickl) << ", 0.0, 360.0";
#endif
      zpos += hthickl;
      ...
      edm::LogVerbatim("HGCalGeom") << "DDHGCalHEAlgo: Position " << glog2.name() << " number " << copy << " in "
                                    << glog1.name() << " at (0, 0, " << (f2mm_ * zpos) << ") with no rotation";
#endif
      ++copyNumberTop_[ii];
      zpos += hthickl;
    }
    if (std::abs(thickTot - thick) >= tol2_) {
      if (thickTot > thick) {
        edm::LogError("HGCalGeom") << "Thickness of the partition " << (f2mm_ * thick) << " is smaller than " << (f2mm_ * thickTot)
                                   << ": thickness of all its components in the top part **** ERROR ****";
      } else {
        edm::LogWarning("HGCalGeom") << "Thickness of the partition " << (f2mm_ * thick) << " does not match with " << (f2mm_ * thickTot)
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCalGeom") << "DDHGCalHEAlgo: " << solid.name() << " Tubs made of " << matter.name()
                                  << " of dimensions " << (f2mm_ * rin) << ", " << (f2mm_ * rmid) << ", " << (f2mm_ * hthick) << ", 0.0, 360.0";
#endif
    ...
    for (unsigned int ly = 0; ly < layerTypeBot_.size(); ++ly) {
...
#ifdef EDM_ML_DEBUG
      edm::LogVerbatim("HGCalGeom") << "DDHGCalHEAlgo: Layer " << ly << ":" << ii << " R " << (f2mm_ * rin) << ":" << (f2mm_ * rmid)
                                    << " Thick " << (f2mm_ * layerThickBot_[ii]);
#endif
      ...
      edm::LogVerbatim("HGCalGeom") << name << " z|rin|rout " << (f2mm_ * zz) << ":" << (f2mm_ * rin) << ":" << (f2mm_ * rmid) << " eta " << eta1
                                    << ":" << eta2;
      edm::LogVerbatim("HGCalGeom") << "DDHGCalHEAlgo: " << solid.name() << " Tubs made of " << matter1.name()
                                    << " of dimensions " << (f2mm_ * rin) << ", " << (f2mm_ * rmid) << ", " << (f2mm_ * hthickl) << ", 0.0, 360.0";
#endif
      zpos += hthickl;
...
#ifdef EDM_ML_DEBUG
        edm::LogVerbatim("HGCalGeom") << "DDHGCalHEAlgo: z " << (f2mm_ * (zz + zpos)) << " Center " << copy << ":"
  ...
  void positionSensitive(cms::DDParsingContext& ctxt,
    ...
    edm::LogVerbatim("HGCalGeom") << "DDHGCalHEAlgo: " << glog.name() << " rout " << (f2mm_ * rout) << " N " << N
                                  << " for maximum u, v Offset; Shift " << (f2mm_ * xyoff.first) << ":" << (f2mm_ * xyoff.second)
      ...
      for (int v = -N; v <= N; ++v) {
        ...
        if (corner.first > 0) {
          ...
          if (corner.first == (int)(HGCalParameters::k_CornerSize)) {
            ...
            ++ntype[type];
            edm::LogVerbatim("HGCalGeom") << "DDHGCalHEAlgo: " << glog.name() << " number " << copy << " positioned in "
                                          << glog.name() << " at (" << (f2mm_ * xpos) << ", " << (f2mm_ * ypos) << ", 0) with no rotation";
#endif
          }
        }
      }
