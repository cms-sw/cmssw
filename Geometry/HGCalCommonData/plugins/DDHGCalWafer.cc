...
#ifdef EDM_ML_DEBUG
  std::cout << childNames_.size() << " children: " << childNames_[0] << "; " << childNames_[1] << " in namespace "
            << idNameSpace_ << " positioned in " << nCellsRow_.size() << " rows and " << nColumns_
            << " columns with lowest column at " << nBottomY_ << " in mother " << parentName_ << " of size "
            << waferSize_ << std::endl;
  for (unsigned int k = 0; k < nCellsRow_.size(); ++k)
    std::cout << "[" << k << "] Ncells " << nCellsRow_[k] << " Edge rotations " << angleEdges_[2 * k] << ":"
              << angleEdges_[2 * k + 1] << " Type of edge cells " << detectorType_[2 * k] << ":"
  ...
  for (unsigned int ir = 0; ir < nCellsRow_.size(); ++ir) {
    ...
    for (int ic = 0; ic < nCellsRow_[ir]; ++ic) {
      ...
      if (irot != 0) {
        ...
        if (!rot) {
#ifdef EDM_ML_DEBUG
          std::cout << "DDHGCalWaferAlgo: Creating new rotation " << DDName(rotstr, idNameSpace_) << "\t90, " << irot
                    << ", 90, " << (irot + 90) << ", 0, 0" << std::endl;
#endif
          rot = DDrot(DDName(rotstr, idNameSpace_),
                      90 * CLHEP::deg,
                      irot * CLHEP::deg,
                      90 * CLHEP::deg,
                      (90 + irot) * CLHEP::deg,
      ...
      ++kount;
#ifdef EDM_ML_DEBUG
      std::cout << "DDHGCalWafer: " << DDName(name, idNameSpace_) << " number " << copy << " positioned in "
                << parentName_ << " at " << tran << " with " << rot << std::endl;
#endif
    }
    ny += 6;
  }
