  ...
  for (unsigned int i = 0; i < layers_.size(); i++) {
    ...
    if (fabs(thickTot - layerThick_[i]) > tol_) {
      if (thickTot > layerThick_[i]) {
        edm::LogError("HGCalGeom") << "DDHGCalModule::Thickness of the partition " << layerThick_[i]
                                   << " is smaller than thickness " << thickTot
                                   << " of all its components **** ERROR ****\n";
      } else {
        edm::LogWarning("HGCalGeom") << "DDHGCalModule::Thickness of the partition " << layerThick_[i]
