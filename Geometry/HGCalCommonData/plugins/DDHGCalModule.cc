...
#ifdef EDM_ML_DEBUG
  std::cout << "DDHGCalModule: " << materials.size() << " types of volumes" << std::endl;
...
#ifdef EDM_ML_DEBUG
  std::cout << "DDHGCalModule: " << layerType.size() << " layers" << std::endl;
...
#ifdef EDM_ML_DEBUG
  std::cout << "DDHGCalModule: zStart " << zMinBlock << " rFineCoarse " << rMaxFine << " wafer width " << waferW
            << " sectors " << sectors << std::endl;
#endif
...
#ifdef EDM_ML_DEBUG
  std::cout << "DDHGCalModule: Bottom slopes " << slopeB[0] << ":" << slopeB[1] << " and " << slopeT.size()
            << " slopes for top" << std::endl;
  for (unsigned int i = 0; i < slopeT.size(); ++i)
    std::cout << "Block [" << i << "] Zmin " << zFront[i] << " Rmax " << rMaxFront[i] << " Slope " << slopeT[i]
              << std::endl;
#endif
    ...
    for (int ly = laymin; ly < laymax; ++ly) {
...
#ifdef EDM_ML_DEBUG
      std::cout << "DDHGCalModule test: Layer " << ly << ":" << ii << " Front " << zi << ", " << routF << " Back " << zo
                << ", " << rinB << " superlayer thickness " << layerThick[i] << std::endl;
#endif
      ...
      if (layerSense[ly] == 0) {
        ...
        DDSolid solid = DDSolidFactory::polyhedra(
            DDName(name, idNameSpace), sectors, -alpha, CLHEP::twopi, pgonZ, pgonRin, pgonRout);
        glog = DDLogicalPart(solid.ddname(), matter, solid);
#ifdef EDM_ML_DEBUG
        std::cout << "DDHGCalModule test: " << solid.name() << " polyhedra of " << sectors << " sectors covering "
                  << -alpha / CLHEP::deg << ":" << (-alpha + CLHEP::twopi) / CLHEP::deg << " with " << pgonZ.size()
                  << " sections" << std::endl;
        for (unsigned int k = 0; k < pgonZ.size(); ++k)
          std::cout << "[" << k << "] z " << pgonZ[k] << " R " << pgonRin[k] << ":" << pgonRout[k] << std::endl;
#endif
      } else {
        DDSolid solid =
            DDSolidFactory::tubs(DDName(name, idNameSpace), 0.5 * thick[ii], rinB, routF, 0.0, CLHEP::twopi);
        glog = DDLogicalPart(solid.ddname(), matter, solid);
#ifdef EDM_ML_DEBUG
        std::cout << "DDHGCalModule test: " << solid.name() << " Tubs made of " << matName << " of dimensions " << rinB
                  << ", " << routF << ", " << 0.5 * thick[ii] << ", 0.0, " << CLHEP::twopi / CLHEP::deg << std::endl;
        std::cout << "DDHGCalModule test position in: " << glog.name() << " number " << copy << std::endl;
#endif
      ...
      ++copyNumber[ii];
#ifdef EDM_ML_DEBUG
      std::cout << "DDHGCalModule test: " << glog.name() << " number " << copy << " positioned in " << module.name()
        ...
        if (corner.first > 0) {
          ...
          if (corner.first == (int)(HGCalParameters::k_CornerSize)) {
            ...
            if (rpos < rMaxFine)
              ...
              ++ncoarse;
#ifdef EDM_ML_DEBUG
            std::cout << "DDHGCalModule: " << name << " number " << copy << " positioned in " << glog.ddname() << " at "
...
#ifdef EDM_ML_DEBUG
  std::cout << "DDHGCalModule: # of columns " << incm << " # of rows " << inrm << " and " << nin << ":" << kount << ":"
            << ntot << " wafers (" << nfine << ":" << ncoarse << ") for " << glog.ddname() << " R " << rin << ":"
            << rout << std::endl;
#endif
}
