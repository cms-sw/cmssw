...
void DDHGCalEEAlgo::constructLayers(const DDLogicalPart& module, DDCompactView& cpv) {
  ...
  for (unsigned int i = 0; i < layers_.size(); i++) {
    ...
    for (int ly = laymin; ly < laymax; ++ly) {
      ...
      std::pair<double, double> zr = HGCalGeomTools::zradius(zz, zz + thick_[ii], zFrontT_, rMaxFront_);
      zz += hthick;
      thickTot += thick_[ii];
      ...
      DDLogicalPart glog;
      if (layerSense_[ly] < 1) {
        int nsec = (layerSense_[ly] == 0 || absorbMode_ == 0 || zr.first < 0) ? 2 : 3;
        ...
        if (layerSense_[ly] == 0 || absorbMode_ == 0) {
          ...
          for (int isec = 0; isec < nsec; ++isec) {
            double zs = ((isec == 0) ? (zz - hthick) : (isec == nsec - 1) ? (zz + hthick) : zr.first);
            double rm = (((isec == 0) || (isec == nsec - 1)) ? HGCalGeomTools::radius(zs, zFrontT_, rMaxFront_, slopeT_)
                                                             : zr.second) *
                            cosAlpha_ -
                        tol;
            pgonZ[isec] = zs - zz;
...
void DDHGCalEEAlgo::positionSensitive(
  ...
  edm::LogVerbatim("HGCalGeom") << "DDHGCalEEAlgo: " << glog.ddname() << " rout " << rout << " N " << N
