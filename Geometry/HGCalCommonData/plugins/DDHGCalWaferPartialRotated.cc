  ...
  for (unsigned int k = 0; k < tags_.size(); ++k) {
    for (unsigned int m = 0; m < placementIndex_.size(); ++m) {
      ...
      if (std::abs(thickTot - thick_) >= tol) {
        if (thickTot > thick_) {
          edm::LogError("HGCalGeom") << "DDHGCalWaferPartialRotated::Thickness of the partition " << thick_ << " is smaller than " << thickTot << ": thickness of all its components **** ERROR ****";
        } else {
          edm::LogWarning("HGCalGeom") << "DDHGCalWaferPartialRotated::Thickness of the partition " << thick_ << " does not match with " << thickTot << " of the components";
        }
      }
    }
  }
