  ...
  for (unsigned int i = 0; i < layers_.size(); i++) {
    ...
    if (std::abs(thickTot - layerThick_[i]) >= tol2_) {
      if (thickTot > layerThick_[i]) {
        edm::LogError("HGCalGeom") << "DDHGCalSiliconModule::Thickness of the partition " << layerThick_[i]
                                   << " is smaller than " << thickTot
                                   << ": thickness of all its components **** ERROR ****";
      } else {
        edm::LogWarning("HGCalGeom") << "DDHGCalSiliconModule::Thickness of the partition " << layerThick_[i]
