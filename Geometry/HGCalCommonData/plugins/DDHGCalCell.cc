...
void DDHGCalCell::initialize(const DDNumericArguments& nArgs,
  ...
  if ((truncCN_.size() != truncSensN_.size()) || (extenCN_.size() != extenSensN_.size()) ||
    ...
    edm::LogWarning("HGCalGeom") << "DDHGCalCell:The number of cells & sensitive differ:" << " Truncated " << truncCN_.size() << ":" << truncSensN_.size() << " Extended " << extenCN_.size() << ":" << extenSensN_.size() << " Corners " << cornrCN_.size() << ":" << cornrSensN_.size();
#ifdef EDM_ML_DEBUG
