...
FastTimeParametersESModule::FastTimeParametersESModule(
  ...
  edm::LogInfo("HGCalGeom")
      << "FastTimeParametersESModule for " << name_.size() << " types";
#ifdef EDM_ML_DEBUG
...
FastTimeParametersESModule::ReturnType FastTimeParametersESModule::produce(
    const IdealGeometryRecord& iRecord) {
  edm::LogInfo("HGCalGeom") << "FastTimeParametersESModule::produce(const "
                               "IdealGeometryRecord& iRecord)";
