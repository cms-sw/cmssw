...
void DDHGCalWaferFullRotated::execute(DDCompactView& cpv) {
  ...
  for (unsigned int k = 0; k < tag_.size(); ++k) {
    ...
    if (std::abs(thickTot - thick_) >= tol) {
      if (thickTot > thick_) {
        edm::LogError("HGCalGeom") << "DDHGCalWaferFullRoatated::Thickness of the partition " << thick_
                                   << " is smaller than " << thickTot
                                   << ": thickness of all its components **** ERROR ****";
      } else {
        edm::LogWarning("HGCalGeom") << "DDHGCalWaferFullRoatated::Thickness of the partition " << thick_
