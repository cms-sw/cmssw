...
void DDHGCalTBModule::initialize(const DDNumericArguments& nArgs,
...
#ifdef EDM_ML_DEBUG
  std::cout << "DDHGCalTBModule: " << materials_.size() << " types of volumes" << std::endl;
  for (unsigned int i = 0; i < names_.size(); ++i)
    std::cout << "Volume [" << i << "] " << names_[i] << " of thickness " << thick_[i] << " filled with "
              << materials_[i] << " first copy number " << copyNumber_[i] << std::endl;
#endif
  ...
  for (unsigned int i = 0; i < layers_.size(); ++i)
    std::cout << "Block [" << i << "] of thickness " << layerThick_[i] << " with " << layers_[i] << " layers"
              << std::endl;
#endif
  layerType_ = dbl_to_int(vArgs["LayerType"]);
  layerSense_ = dbl_to_int(vArgs["LayerSense"]);
#ifdef EDM_ML_DEBUG
  std::cout << "DDHGCalTBModule: " << layerType_.size() << " layers" << std::endl;
  for (unsigned int i = 0; i < layerType_.size(); ++i)
    std::cout << "Layer [" << i << "] with material type " << layerType_[i] << " sensitive class " << layerSense_[i]
              << std::endl;
#endif
...
#ifdef EDM_ML_DEBUG
  std::cout << "DDHGCalTBModule: zStart " << zMinBlock_ << " rFineCoarse " << rMaxFine_ << " wafer width " << waferW_
            << " gap among wafers " << waferGap_ << " absorber width " << absorbW_ << " absorber height " << absorbH_
            << " sectors " << sectors_ << std::endl;
#endif
...
#ifdef EDM_ML_DEBUG
  std::cout << "DDHGCalTBModule: Bottom slopes " << slopeB_[0] << ":" << slopeB_[1] << " and " << slopeT_.size()
            << " slopes for top" << std::endl;
  for (unsigned int i = 0; i < slopeT_.size(); ++i)
    std::cout << "Block [" << i << "] Zmin " << zFront_[i] << " Rmax " << rMaxFront_[i] << " Slope " << slopeT_[i]
              << std::endl;
#endif
...
void DDHGCalTBModule::constructLayers(const DDLogicalPart& module, DDCompactView& cpv) {
  ...
  for (unsigned int i = 0; i < layers_.size(); i++) {
    ...
    for (int ly = laymin; ly < laymax; ++ly) {
...
#ifdef EDM_ML_DEBUG
      std::cout << "DDHGCalTBModule test: Layer " << ly << ":" << ii << " Front " << zi << ", " << routF << " Back "
                << zo << ", " << rinB << " superlayer thickness " << layerThick_[i] << std::endl;
#endif
...
#ifdef EDM_ML_DEBUG
        std::cout << "DDHGCalTBModule test: " << solid.name() << " box of dimension " << absorbW_ << ":" << absorbH_
                  << ":" << 0.5 * thick_[ii] << std::endl;
#endif
      } else {
        DDSolid solid =
            DDSolidFactory::tubs(DDName(name, idNameSpace_), 0.5 * thick_[ii], rinB, routF, 0.0, CLHEP::twopi);
        glog = DDLogicalPart(solid.ddname(), matter, solid);
#ifdef EDM_ML_DEBUG
        std::cout << "DDHGCalTBModule test: " << solid.name() << " Tubs made of " << matName << " of dimensions "
                  << rinB << ", " << routF << ", " << 0.5 * thick_[ii] << ", 0.0, " << CLHEP::twopi / CLHEP::deg
                  << std::endl;
#endif
      ...
      ++copyNumber_[ii];
#ifdef EDM_ML_DEBUG
      std::cout << "DDHGCalTBModule test: " << glog.name() << " number " << copy << " positioned in " << module.name()
...
void DDHGCalTBModule::positionSensitive(DDLogicalPart& glog, int type, double rin, double rout, DDCompactView& cpv) {
  ...
  double xc[6], yc[6];
#ifdef EDM_ML_DEBUG
  std::cout << glog.ddname() << " rout " << rout << " Row " << nrow << " Column " << ncol << std::endl;
#endif
      ...
      if (inr % 2 == inc % 2) {
        ...
        if (cornerAll) {
          ...
          if (copies_.count(copy) == 0 && type == 1)
            copies_.insert(copy);
#ifdef EDM_ML_DEBUG
          std::cout << "DDHGCalTBModule: " << name << " number " << copy << " positioned in " << glog.ddname() << " at "
...
#ifdef EDM_ML_DEBUG
  std::cout << "DDHGCalTBModule: # of columns " << incm << " # of rows " << inrm << " and " << kount << " wafers for "
