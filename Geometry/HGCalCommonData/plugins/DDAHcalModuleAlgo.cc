...
void DDAHcalModuleAlgo::initialize(const DDNumericArguments& nArgs,
...
#ifdef EDM_ML_DEBUG
  std::cout << "DDAHcalModuleAlgo: " << materials.size() << " types of volumes" << std::endl;
  for (unsigned int i = 0; i < names.size(); ++i)
    std::cout << "Volume [" << i << "] " << names[i] << " of thickness " << thick[i] << " filled with " << materials[i]
  ...
  for (unsigned int i = 0; i < layers.size(); ++i)
    std::cout << "Block [" << i << "] of thickness " << layerThick[i] << " with " << layers[i] << " layers"
              << std::endl;
#endif
  layerType = dbl_to_int(vArgs["LayerType"]);
  layerSense = dbl_to_int(vArgs["LayerSense"]);
#ifdef EDM_ML_DEBUG
  std::cout << "DDAHcalModuleAlgo: " << layerType.size() << " layers" << std::endl;
  for (unsigned int i = 0; i < layerType.size(); ++i)
    std::cout << "Layer [" << i << "] with material type " << layerType[i] << " sensitive class " << layerSense[i]
              << std::endl;
#endif
  widths = vArgs["Widths"];
...
void DDAHcalModuleAlgo::constructLayers(const DDLogicalPart& module, DDCompactView& cpv) {
  ...
  for (unsigned int i = 0; i < layers.size(); i++) {
    ...
    for (int ly = laymin; ly < laymax; ++ly) {
...
#ifdef EDM_ML_DEBUG
      std::cout << "DDAHcalModuleAlgo test: Layer " << ly << ":" << ii << " Front " << zi << " Back " << zo
...
#ifdef EDM_ML_DEBUG
        std::cout << "DDAHcalModuleAlgo test: " << solid.name() << " Box made of " << matName << " of dimensions "
                  << 0.5 * widths[0] << ", " << 0.5 * heights[0] << ", " << 0.5 * thick[ii] << std::endl;
#endif
      } else {
        DDSolid solid =
...
#ifdef EDM_ML_DEBUG
        std::cout << "DDAHcalModuleAlgo test: " << solid.name() << " Box made of " << matName << " of dimensions "
                  << 0.5 * widths[1] << ", " << 0.5 * heights[1] << ", " << 0.5 * thick[ii] << std::endl;
#endif
      ...
      ++copyNumber[ii];
#ifdef EDM_ML_DEBUG
      std::cout << "DDAHcalModuleAlgo test: " << glog.name() << " number " << copy << " positioned in " << module.name()
                << " at " << r1 << " with " << rot << std::endl;
#endif
      zz += (0.5 * thick[ii]);
...
#ifdef EDM_ML_DEBUG
  int kount(0);
  std::cout << glog.ddname() << " Row " << nrow << " Column " << ncol << std::endl;
#endif
...
#ifdef EDM_ML_DEBUG
        kount++;
        std::cout << "DDAHcalModuleAlgo: " << name << " number " << copy << " positioned in " << glog.ddname() << " at "
                  << tran << " with " << rotation << std::endl;
#endif
      }
    }
  }
#ifdef EDM_ML_DEBUG
  std::cout << "DDAHcalModuleAlgo: " << kount << " tiles for " << glog.ddname() << std::endl;
#endif
}
