  ...
  for (unsigned int i = 0; i < layers_.size(); i++) {
    ...
    if (fabs(thickTot - layerThick_[i]) > tol_) {
      if (thickTot > layerThick_[i]) {
        edm::LogError("HGCalGeom") << "DDAHcalModuleAlgo::Thickness of the partition " << layerThick_[i] << " is smaller than thickness " << thickTot << " of all its components **** ERROR ****\n";
      } else {
        edm::LogWarning("HGCalGeom") << "DDAHcalModuleAlgo::Thickness of the partition " << layerThick_[i] << " does not match with " << thickTot << " of the components\n";
      }
    }
