...
#ifdef EDM_ML_DEBUG
  std::cout << "DDHGCalModuleAlgo: " << materials.size() << " types of volumes" << std::endl;
...
#ifdef EDM_ML_DEBUG
  std::cout << "DDHGCalModuleAlgo: " << layerType.size() << " layers" << std::endl;
...
#ifdef EDM_ML_DEBUG
  std::cout << "DDHGCalModuleAlgo: zStart " << zMinBlock << " rFineCoarse " << rMaxFine << " wafer width " << waferW
            << " gap among wafers " << waferGap << " sectors " << sectors << std::endl;
#endif
...
#ifdef EDM_ML_DEBUG
  std::cout << "DDHGCalModuleAlgo: Bottom slopes " << slopeB[0] << ":" << slopeB[1] << " and " << slopeT.size()
  ...
  for (unsigned int i = 0; i < layers.size(); i++) {
    ...
    for (int ly = laymin; ly < laymax; ++ly) {
...
#ifdef EDM_ML_DEBUG
      std::cout << "DDHGCalModuleAlgo test: Layer " << ly << ":" << ii << " Front " << zi << ", " << routF << " Back "
                << zo << ", " << rinB << " superlayer thickness " << layerThick[i] << std::endl;
#endif
      ...
      if (layerSense[ly] == 0) {
...
#ifdef EDM_ML_DEBUG
        std::cout << "DDHGCalModuleAlgo test: " << solid.name() << " polyhedra of " << sectors << " sectors covering "
        ...
        glog = DDLogicalPart(solid.ddname(), matter, solid);
#ifdef EDM_ML_DEBUG
        std::cout << "DDHGCalModuleAlgo test: " << solid.name() << " Tubs made of " << matName << " of dimensions "
                  << rinB << ", " << routF << ", " << 0.5 * thick[ii] << ", 0.0, " << CLHEP::twopi / CLHEP::deg
                  << std::endl;
#endif
      ...
      ++copyNumber[ii];
#ifdef EDM_ML_DEBUG
      std::cout << "DDHGCalModuleAlgo test: " << glog.name() << " number " << copy << " positioned in " << module.name()
      ...
      if (inr % 2 == inc % 2) {
        ...
        if (corner.first == (int)(HGCalParameters::k_CornerSize)) {
          ...
          if (copies.count(copy) == 0)
            copies.insert(copy);
#ifdef EDM_ML_DEBUG
          std::cout << "DDHGCalModuleAlgo: " << name << " number " << copy << " positioned in " << glog.ddname()
...
#ifdef EDM_ML_DEBUG
  std::cout << "DDHGCalModuleAlgo: # of columns " << incm << " # of rows " << inrm << " and " << kount << " wafers for "
