  ...
  for (unsigned int i = 0; i < layers_.size(); i++) {
    ...
    if (std::abs(thickTot - layerThick_[i]) >= tol2_) {
      if (thickTot > layerThick_[i]) {
        edm::LogError("HGCalGeom") << "DDHGCalHEAlgo::Thickness of the partition " << layerThick_[i]
                                   << " is smaller than " << thickTot
                                   << ": thickness of all its components **** ERROR ****";
      } else {
        edm::LogWarning("HGCalGeom") << "DDHGCalHEAlgo::Thickness of the partition " << layerThick_[i]
...
void DDHGCalHEAlgo::positionMix(const DDLogicalPart& glog,
  ...
  if (std::abs(thickTot - thick) >= tol2_) {
    if (thickTot > thick) {
      edm::LogError("HGCalGeom") << "DDHGCalHEAlgo::Thickness of the partition " << thick << " is smaller than "
                                 << thickTot << ": thickness of all its components in the top part **** ERROR ****";
    } else {
      edm::LogWarning("HGCalGeom") << "DDHGCalHEAlgo::Thickness of the partition " << thick << " does not match with "
