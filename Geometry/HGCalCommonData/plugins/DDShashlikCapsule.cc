...
#include "Geometry/HGCalCommonData/plugins/DDShashlikCapsule.h"
#include "CLHEP/Units/GlobalPhysicalConstants.h"
#include "CLHEP/Units/GlobalSystemOfUnits.h"

DDShashlikCapsule::DDShashlikCapsule() {
  edm::LogInfo("HGCalGeom") << "DDShashlikCapsule test: Creating an instance";
}

DDShashlikCapsule::~DDShashlikCapsule() {}

void DDShashlikCapsule::initialize(const DDNumericArguments & nArgs,
  ...
  for (unsigned int i=0; i<holeX.size(); ++i) {
    edm::LogInfo("HGCalGeom") << "DDShashlikCapsule: Hole[" << i << "] at ("
			      << holeX[i] << ", " << holeY[i] << ")";
  }
  idNameSpace = DDCurrentNamespace::ns();
  edm::LogInfo("HGCalGeom") << "DDShashlikCapsule: NameSpace " << idNameSpace 
			    << "\tParent " << parent().name();
}

void DDShashlikCapsule::execute(DDCompactView& cpv) {
  ...
  DDSolid      solidHole  = DDSolidFactory::tubs(name,0.5*activeThick,0,holeR,
						 0,CLHEP::twopi);
  edm::LogInfo("HGCalGeom") << "DDShashlikCapsule:: " << name <<" tube of dim "
  ...
  for (int ii=0; ii<activeLayers; ii++) {
    ...
    edm::LogInfo("HGCalGeom") << "DDShashlikCapsule:: "<< name<<" trap of dim "
    ...
    for (unsigned int k=0; k<holeX.size(); ++k) {
      ...
      edm::LogInfo("HGCalGeom") << "DDShashlikCapsule:: Construct " << solx.name() << " by putting the hole in " << solid.name() << " at " << tran;
      solid = solx;
    }
    ...
    edm::LogInfo("HGCalGeom") << "DDShashlikCapsule:: Construct " << solx.name() << " by putting the hole in " << solid.name() << " at " << tranc;
    solid = solx;
    ...
    edm::LogInfo("HGCalGeom") << "DDShashlikCapsule:" << active.name() <<" no "
  ...
  if (holeX.size() > 0) {
    ...
    edm::LogInfo("HGCalGeom") << "DDShashlikCapsule:: " << fibre.name() 
    ...
    for (unsigned int k=0; k<holeX.size(); ++k) {
      ...
      edm::LogInfo("HGCalGeom") << "DDShashlikCapsule:" << fibre.name()<<" no "
  ...
  edm::LogInfo("HGCalGeom") << "DDShashlikCapsule:" << fibre.name() <<" no 1"
