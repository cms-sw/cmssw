...
void HGCalWaferIDTester::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (unsigned int k = 0; k < detIds_.size(); ++k) {
    if (detIds_[k].det() == det) {
      ...
      st1 << " Part:Orient:Cassette:Shift " << std::get<1>(hgdc.waferFileInfo(indx)) << ":" << std::get<2>(hgdc.waferFileInfo(indx)) << ":" << std::get<3>(hgdc.waferFileInfo(indx)) << ":" << hgdc.cassetteShiftSilicon(layer, detIds_[k].waferU(), detIds_[k].waferV());
      ...
      st1 << " Part:Orient:Cassette:Shift " << std::get<1>(hgdc.waferFileInfo(indx)) << ":" << std::get<2>(hgdc.waferFileInfo(indx)) << ":" << std::get<3>(hgdc.waferFileInfo(indx)) << ":" << hgdc.cassetteShiftSilicon(layer, id.waferU(), id.waferV());
      ...
      st1 << " input position: (" << posXY_[k].first / CLHEP::cm << ", " << yy / CLHEP::cm << "); position from ID (" << xx0 << ", "
          << yy0 << ") distance " << diff;
