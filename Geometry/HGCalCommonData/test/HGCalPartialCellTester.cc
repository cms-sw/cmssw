...
HGCalPartialCellTester::HGCalPartialCellTester(const edm::ParameterSet& iC)
  ...
  edm::LogVerbatim("HGCalGeom") << "Test positions for wafer of size " << waferSize_ << " Type " << waferType_
                                << " Placement Index " << placeIndex_ << " mode " << modeUV_ << " with " << nTrials_ << " trials";
}
...
void HGCalPartialCellTester::analyze(const edm::Event&, const edm::EventSetup&) {
  ...
  if (modeUV_ <= 0) {
    for (int i = 0; i < nTrials_; i++) {
	  ...
	  (c2 > -1 * R2) && (xi > 0)) {  //Only allowing (x, y) inside a partial wafer 11, placement index 2
	...
	edm::LogVerbatim("HGCalGeom") << cellType << " x = " << xi << " y = " << yi << " type = " << waferType_ << " placement index " << placeIndex_ << " u " << uv1.first << ":" << uv5.first << ":" << uv5.first << " v " << uv1.second << ":" << uv5.second << ":" << uv5.second << ":" << comment;
      }
    }
  } else {
    for (int i = 0; i < nTrials_; i++) {
	...
	std::pair<int32_t, int32_t> uv1 = wafer.cellUVFromXY1(xy1.first, xy1.second, placeIndex_, waferType_, true, false);
	std::pair<int32_t, int32_t> uv5 = wafer.cellUVFromXY1(xy1.first, xy1.second, placeIndex_, waferType_, partialType_, true, false);
	std::string comment = ((uv1.first != ui)|| (uv1.second != vi) || (uv5.first != ui)|| (uv5.second != vi)) ? " ***** ERROR *****" : "";
	edm::LogVerbatim("HGCalGeom") << "u = " << ui << " v = " << vi << " type = " << waferType_ << " placement index " << placeIndex_ << " x " << xy1.first << " ,y " << xy1.second << " u " << uv5.first << " v " << uv5.second << comment;
      }
    }
  }
  auto end_t = std::chrono::high_resolution_clock::now();
  auto diff_t = end_t - start_t;
  edm::LogVerbatim("HGCalGeom") << "Execution time for " << nTrials_ << " events = "
