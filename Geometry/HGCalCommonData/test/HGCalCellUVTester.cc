...
void HGCalCellUVTester::analyze(const edm::Event&, const edm::EventSetup&) {
  ...
  for(int i = 0; i<100000; i++){
    ...
    if ((xi < r2) && (xi > -1*r2) && (c1 < R2) && (c1 > -1*R2) && (c2 < R2) && (c2 > -1*R2)){            //Only allowing (x, y) inside a wafer
      std::pair<int32_t, int32_t> uv0 = wafer.HGCalCellUVFromXY0(xi, yi, placeIndex_, waferType_, 0, 0);
      std::pair<int32_t, int32_t> uv1 = wafer.HGCalCellUVFromXY1(xi, yi, placeIndex_, waferType_, 0, 0);
      std::pair<int32_t, int32_t> uv3 = wafer.HGCalCellUVFromXY3(xi, yi, placeIndex_, waferType_, 1, 0);
      //std::pair<int32_t, int32_t> uv2 = wafer.HGCalCellUVFromXY2(xi, yi, placeIndex_, waferType_, 1, 0);
      std::string comment = ((uv1.first != uv3.first) || (uv0.first != uv3.first) || (uv1.second != uv3.second) || (uv0.second != uv3.second)) ? " ***** ERROR *****" : "";
      edm::LogVerbatim("HGCalGeom") << "x = " << xi << " y = " << yi << " type = " << waferType_
                                    << " placement index " << placeIndex_ << " u " << uv0.first << ":" << uv1.first << ":" << uv3.first
      			            << " v " << uv0.second << ":"  << uv1.second << ":" << uv3.second << ":" << comment;
