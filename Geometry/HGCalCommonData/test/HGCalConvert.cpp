...
struct tile {
  ...
  tile(double s = 0, int h1 = 0, int h2 = 0, int h3 = 0, int h4 = 0,
       int t = 0) : sipm(s), hex1(h1), hex2(h2), hex3(h3), hex4(h4), type(t) {};
};
...
public:
  ...
  void convert(const char* infile, const char* outfile1, const char* outfile2,
	       const char* outfile3, int modeGlobal=0, int debug=0);
private:
  void writeSilicon(const char*, const std::map<int,wafer>&, const std::string&, 
...
ConvertSilicon::ConvertSilicon(int layMax1, int layMax2) :
  layMax1_(layMax1), layMax2_(layMax2) {}

void ConvertSilicon::convert(const char* infile, const char* outfile1, const char* outfile2, 
			     const char* outfile3, int modeGlobal, int debug) {
  ...
  if (!fInput.good()) {
    ...
    fInput.close();
    std::cout << "Read " << all << " records with " << comments << " comments "
	      << others << " non-comment records out of which " << good << ":"
	      << module1.size() << ":" << module2.size() << ":" << module3.size()
...
void ConvertSilicon::writeSilicon(const char* outfile, const std::map<int,wafer>& module,
				  const std::string& tag, const bool& mode, const bool& debug) {
  char apost('"');
  ...
  if (mode) {
    blank = "  ";
    fOut << blank << "<Vector name=" << apost << "WaferIndex" << tag << apost 
	 << " type=" << apost << "numeric" << apost << " nEntries=" << apost
	 << module.size() << apost << ">";
  } else {
    blank = "    ";
    fOut << blank << "<Vector name=" << apost << "WaferIndex" << apost 
	 << " type=" << apost << "numeric" << apost << " nEntries=" << apost
	 << module.size() << apost << ">";
  }
  for (itr = module.begin(); itr != module.end(); ++itr) {
    ...
    ++k1;
    if (debug) 
      std::cout << "Wafer " << waferLayer(itr->first) << ":" << waferU(itr->first)
		<< ":" << waferV(itr->first) << " T " << (itr->second).thick
		<< " P " << (itr->second).partial << " O "
		<< (itr->second).orient << std::endl;
  }
  fOut << "\n" << blank << "</Vector>\n";
  if (mode)
    fOut << blank << "<Vector name=" << apost << "WaferTypes" << tag << apost 
	 << " type=" << apost << "numeric" << apost << " nEntries=" << apost
	 << module.size() << apost << ">";
  else
    fOut << blank << "<Vector name=" << apost << "WaferTypes" << apost 
	 << " type=" << apost << "numeric" << apost << " nEntries=" << apost
	 << module.size() << apost << ">";
  for (itr = module.begin(); itr != module.end(); ++itr) {
    if (k2 % 20 == 0) 
  ...
  fOut << "\n" << blank << "</Vector>\n";
  if (mode) {
    fOut << blank << "<Vector name=" << apost << "WaferPartial" << tag << apost 
	 << " type=" << apost << "numeric" << apost << " nEntries=" << apost
	 << module.size() << apost << ">";
    for (itr = module.begin(); itr != module.end(); ++itr) {
      if (k3 % 20 == 0) 
    ...
    fOut << "\n" << blank << "</Vector>\n";
    fOut << blank << "<Vector name=" << apost << "WaferOrient" << tag << apost 
	 << " type=" << apost << "numeric" << apost << " nEntries=" << apost
	 << module.size() << apost << ">";
    for (itr = module.begin(); itr != module.end(); ++itr) {
      if (k4 % 20 == 0) 
...
int ConvertSilicon::waferIndex(const int& layer, const int& waferU,
			       const int& waferV) {
  int id(0);
...
void ConvertScintillator::convert(const char* infile, const char* outfile, int debug) {
  ...
  if (!fInput.good()) {
	...
	if (items.size() != 10) {
	  ...
	  sscanf (buffer, "%d %d %f %f %f %X %X %X %X", &layer, &ring, &rstart,
		  &rend, &sipm, &hex1, &hex2, &hex3, &hex4);
    ...
    std::cout << "Read " << all << " records with " << comments << " comments "
	      << others << " non-comment records out of which " << good << ":"
	      << module.size() << ":" << ringR.size() << ":" << layerRing.size()
	      << " are good and " << bad << " are bad\n" << std::endl;
    ...
    fOut << "  <Vector name=" << apost << "TileRMin" << apost << " type="
	 << apost << "numeric" << apost << " nEntries=" << apost
	 << ringR.size() << apost << ">";
    ...
    fOut << "\n  </Vector>\n";
    fOut << "  <Vector name=" << apost << "TileRMax" << apost << " type="
	 << apost << "numeric" << apost << " nEntries=" << apost
	 << ringR.size() << apost << ">";
    for (it1 = ringR.begin(); it1 != ringR.end(); ++it1) {
      if (l2 % 7 == 0) 
    ...
    fOut << "  <Vector name=" << apost << "TileRingMin" << apost << " type="
	 << apost << "numeric" << apost << " nEntries=" << apost
	 << layerRing.size() << apost << ">";
    ...
    fOut << "\n  </Vector>\n";
    fOut << "  <Vector name=" << apost << "TileRingMax" << apost << " type="
	 << apost << "numeric" << apost << " nEntries=" << apost
	 << layerRing.size() << apost << ">";
    for (it2 = layerRing.begin(); it2 != layerRing.end(); ++it2) {
      if (l4 % 14 == 0) 
    ...
    fOut << "  <Vector name=" << apost << "TileIndex" << apost << " type="
	 << apost << "numeric" << apost << " nEntries=" << apost
	 << module.size() << apost << ">";
    for (itr = module.begin(); itr != module.end(); ++itr) {
      if (k1 % 7 == 0) 
	...
	fOut << std::setw(8) << itr->first << ",";
      ++k1;
      if (debug > 0) 
	std::cout << "Tile " << tileLayer(itr->first) << ":" 
		  << tileRing(itr->first) << " Type " << (itr->second).type 
		  << " Area " << (itr->second).sipm << std::hex << " HEX "
		  << (itr->second).hex1 << " " << (itr->second).hex2 << " "
		  << (itr->second).hex3 << " " << (itr->second).hex4 
		  << std::dec << "\n";
    }
    fOut << "\n  </Vector>\n";
    fOut << "  <Vector name=" << apost << "TileType" << apost << " type="
	 << apost << "numeric" << apost << " nEntries=" << apost
	 << module.size() << apost << ">";
    for (itr = module.begin(); itr != module.end(); ++itr) {
      if (k2 % 20 == 0) 
    ...
    fOut << "\n  </Vector>\n";
    fOut << "  <Vector name=" << apost << "TileSiPM" << apost << " type="
	 << apost << "numeric" << apost << " nEntries=" << apost
	 << module.size() << apost << ">";
    for (itr = module.begin(); itr != module.end(); ++itr) {
      if (k3 % 20 == 0) 
    ...
    fOut << "\n  </Vector>\n";
    fOut << "  <Vector name=" << apost << "TileHEX1" << apost << " type="
	 << apost << "numeric" << apost << " nEntries=" << apost
	 << module.size() << apost << ">" << std::hex;
    for (itr = module.begin(); itr != module.end(); ++itr) {
      if (k4 % 6 == 0) 
	fOut << "\n     0x" << (itr->second).hex1 << ",";
      else
    ...
    fOut << "\n  </Vector>\n" << std::dec;
    fOut << "  <Vector name=" << apost << "TileHEX2" << apost << " type="
	 << apost << "numeric" << apost << " nEntries=" << apost
	 << module.size() << apost << ">" << std::hex;
    for (itr = module.begin(); itr != module.end(); ++itr) {
      if (k5 % 6 == 0) 
    ...
    fOut << "\n  </Vector>\n" << std::dec;
    fOut << "  <Vector name=" << apost << "TileHEX3" << apost << " type="
	 << apost << "numeric" << apost << " nEntries=" << apost
	 << module.size() << apost << ">" << std::hex;
    for (itr = module.begin(); itr != module.end(); ++itr) {
      if (k6 % 6 == 0) 
    ...
    fOut << "\n  </Vector>\n" << std::dec;
    fOut << "  <Vector name=" << apost << "TileHEX4" << apost << " type="
	 << apost << "numeric" << apost << " nEntries=" << apost
	 << module.size() << apost << ">" << std::hex;
    for (itr = module.begin(); itr != module.end(); ++itr) {
      if (k7 % 6 == 0) 
...
int ConvertScintillator::tileIndex(const int& layer, const int& ring, 
				   const int& iphi) {
  int id(0);
  id |= (((iphi & kHGCalPhiMask) << kHGCalPhiOffset) |
...
int ConvertScintillator::tileRing(const int& id) { 
  return (id >> kHGCalRingOffset) & kHGCalRingMask; 
}

int ConvertScintillator::tilePhi(const int& id) { 
  return (id >> kHGCalPhiOffset) & kHGCalPhiMask; 
}
