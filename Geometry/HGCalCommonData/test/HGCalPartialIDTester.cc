...
HGCalPartialIDTester::HGCalPartialIDTester(const edm::ParameterSet &iC)
  ...
  for (unsigned int k = 0; k < layer_.size(); ++k) {
    edm::LogVerbatim("HGCalGeom") << "[" << k << "]  Layer " << layer_[k] << " Wafer " << waferU_[k] << ":" << waferV_[k] << " zside " << zside_[k] << " position " << xpos_[k] << ":" << ypos_[k] << ":" << zpos_[k];
  }
}
...
void HGCalPartialIDTester::beginRun(edm::Run const &iRun, edm::EventSetup const &iSetup) {
  ...
  if (!hgcCons.isValid()) {
    ...
    for (uint32_t i = 0; i < layer_.size(); i++) {
      ...
      double wt(0);
      edm::LogVerbatim("HGCalGeom") << "Input " << xpos_[i] << ":" << ypos_[i] << ":" << zside_[i] << ":" << layer_[i] << ":" << waferU << ":" << waferV;
	...
	std::pair<float, float> xy = hgcCons_->locateCell(zside_[i], layer_[i], waferU, waferV, cellU, cellV, false, true, false, false);
	...
	dR = std::sqrt(dx*dx + dy*dy);
      ...
      edm::LogVerbatim("HGCalGeom") << "Input " << dets << ":" << zside_[i] << ":" << layer_[i] << ":"
                                    ...
                                    << " WaferType " << waferType << " Wafer " << waferU << ":" << waferU_[i] << ":" << waferV << ":" << waferV_[i] << " Cell "
