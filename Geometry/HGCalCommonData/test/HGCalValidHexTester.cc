...
HGCalValidHexTester::HGCalValidHexTester(const edm::ParameterSet& iC) :
  ...
  edm::LogVerbatim("HGCalGeom") << "Test valid cells for " << nameDetector_ << " using constants of " << nameSense_ << " for " << layers_.size() << " modules";
  for (unsigned int k = 0; k < layers_.size(); ++k)
    edm::LogVerbatim("HGCalGeom") << "Wafer[" << k << "] Layer " << layers_[k] << " Type " << types_[k] << " U:V " << moduleU_[k] << ":" << moduleV_[k];
}
    ...
    for (int u = 0; u < 2 * nCells; ++u) {
      for (int v = 0; v < 2 * nCells; ++v) {
        if (((v - u) < nCells) && (u - v) <= nCells) {
          ...
          edm::LogVerbatim("HGCalGeom") << "Cell[" << k << "," << ncell << "] " << HGCSiliconDetId(det, 1, types_[k], layers_[k], moduleU_[k], moduleV_[k], u, v) << " is " << state << " fiducial volume";
          ++ncell;
        }
      }
    }
