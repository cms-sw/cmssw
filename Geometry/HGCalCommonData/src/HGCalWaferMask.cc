...
bool HGCalWaferMask::maskCell(int u, int v, int N, int ncor, int fcor, 
			      int corners) {
/*
...
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("HGCalGeom") << "u|v " << u << ":" << v << " N " << N << " type "
				<< type << " rot " << rotn << " good " << good;
#endif
  return good;
}
...
std::pair<int, int> HGCalWaferMask::getTypeMode(const double& xpos, 
  ...
  if (ncor == 6) {
    ...
    rotn = fcor;
    double rpos = sqrt((xpos + dx1[fcor]) * (xpos + dx1[fcor]) +
		       (ypos + dy1[fcor]) * (ypos + dy1[fcor]));
    if (rpos <= rout && rpos >= rin) {
      rpos = sqrt((xpos + dx2[fcor]) * (xpos + dx2[fcor]) +
		  (ypos + dy2[fcor]) * (ypos + dy2[fcor]));
      if (rpos <= rout && rpos >= rin) type = HGCalGeomTools::WaferChopTwo;
    }
    if (type == HGCalGeomTools::WaferHalf) {
      rpos = sqrt((xpos + dx3[fcor]) * (xpos + dx3[fcor]) +
		  (ypos + dy3[fcor]) * (ypos + dy3[fcor]));
      if (rpos <= rout && rpos >= rin) {
	rpos = sqrt((xpos + dx4[fcor]) * (xpos + dx4[fcor]) +
		    (ypos + dy4[fcor]) * (ypos + dy4[fcor]));
	if (rpos <= rout && rpos >= rin) type = HGCalGeomTools::WaferChopTwoM;
      }
    }
  } else if (ncor == 3) {
    ...
    rotn = fcor - 1;
    if (rotn < 0) rotn = HGCalGeomTools::WaferCorner5;
    double rpos = sqrt((xpos + dx5[fcor]) * (xpos + dx5[fcor]) +
		       (ypos + dy5[fcor]) * (ypos + dy5[fcor]));
    if (rpos <= rout && rpos >= rin) {
      rpos = sqrt((xpos + dx6[fcor]) * (xpos + dx6[fcor]) +
		  (ypos + dy6[fcor]) * (ypos + dy6[fcor]));
      if (rpos <= rout && rpos >= rin) type = HGCalGeomTools::WaferSemi;
    }
  } else {
...
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("HGCalGeom") << "Corners " << ncor << ":" << fcor << " Type "
				<< type << ":" << rotn;
#endif
