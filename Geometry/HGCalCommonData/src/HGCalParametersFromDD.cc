...
bool HGCalParametersFromDD::build(const DDCompactView* cpv,
  ...
  if (ok) {
    ...
    edm::LogVerbatim("HGCalGeom") << "Volume " << name << " GeometryMode " << php.mode_ << ":"
                                  ...
                                  << HGCalGeometryMode::Hexagon8CalibCell << ":" <<  HGCalGeometryMode::TrapezoidFineCell;
#endif
    ...
    if (php.mode_ == HGCalGeometryMode::Hexagon) {
      ...
      // Load wafer positions
      geom->loadWaferHexagon8(php);
    } else if ((php.mode_ == HGCalGeometryMode::Trapezoid) || (php.mode_ == HGCalGeometryMode::TrapezoidFile) || (php.mode_ == HGCalGeometryMode::TrapezoidModule) || (php.mode_ == HGCalGeometryMode::TrapezoidCassette) || (php.mode_ == HGCalGeometryMode::TrapezoidFineCell)) {
      ...
      php.waferMaskMode_ = static_cast<int>(getDDDValue("WaferMaskMode", sv));
      php.waferZSide_ = static_cast<int>(getDDDValue("WaferZside", sv));
      if ((php.mode_ == HGCalGeometryMode::TrapezoidModule) || (php.mode_ == HGCalGeometryMode::TrapezoidCassette) || (php.mode_ == HGCalGeometryMode::TrapezoidFineCell))
        php.useSimWt_ = static_cast<int>(getDDDValue("UseSimWt", sv));
      if ((php.waferMaskMode_ == HGCalGeomParameters::scintillatorCassette) || (php.waferMaskMode_ == HGCalGeomParameters::scintillatorFineCell))
        php.cassettes_ = getDDDValue("Cassettes", sv);
#ifdef EDM_ML_DEBUG
