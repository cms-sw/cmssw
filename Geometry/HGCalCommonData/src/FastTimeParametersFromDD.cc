...
bool FastTimeParametersFromDD::build(const DDCompactView* cpv,
                                     ...
                                     const int type) {
#ifdef EDM_ML_DEBUG
  std::cout << "FastTimeParametersFromDD::build called with names " << name << " and type " << type << std::endl;
#endif
...
#ifdef EDM_ML_DEBUG
      std::cout << "Barrel Parameters: " << php.nZBarrel_ << ":" << php.nPhiBarrel_ << ":" << php.geomParBarrel_[0]
...
#ifdef EDM_ML_DEBUG
      std::cout << "Endcap Parameters: " << php.nEtaEndcap_ << ":" << php.nPhiEndcap_ << ":" << php.geomParEndcap_[0]
                << ":" << php.geomParEndcap_[1] << ":" << php.geomParEndcap_[2] << std::endl;
#endif
    } else {
...
#ifdef EDM_ML_DEBUG
  std::cout << "FastTimeParametersFromDD::Returns with flag " << ok << " for " << name << " and type " << type
            << std::endl;
#endif
  return ok;
}
