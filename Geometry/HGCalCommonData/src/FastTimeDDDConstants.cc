...
std::pair<int, int> FastTimeDDDConstants::getZPhi(double z, double phi) const {
  ...
  if (iphi > ftpar_->nPhiBarrel_)
    iphi = 1;
#ifdef EDM_ML_DEBUG
  std::cout << "FastTimeDDDConstants:Barrel z|phi " << z << " " << phi / CLHEP::deg << " iz|iphi " << iz << " " << iphi
            << std::endl;
#endif
...
std::pair<int, int> FastTimeDDDConstants::getEtaPhi(double r, double phi) const {
  ...
  if (iphi > ftpar_->nPhiEndcap_)
    iphi = 1;
#ifdef EDM_ML_DEBUG
  std::cout << "FastTimeDDDConstants:Endcap r|phi " << r << " " << phi / CLHEP::deg << " ir|iphi " << ir << " " << iphi
            << std::endl;
#endif
...
#ifdef EDM_ML_DEBUG
  std::cout << "Theta range " << thmin / CLHEP::deg << ":" << thmax / CLHEP::deg << " Eta range " << etaMin_ << ":"
            << etaMax_ << ":" << dEta_ << std::endl;
#endif
...
#ifdef EDM_ML_DEBUG
  std::cout << "FastTimeDDDConstants initialized with " << ftpar_->nZBarrel_ << ":" << ftpar_->nPhiBarrel_ << ":"
            << getCells(1) << " cells for barrel; dz|dphi " << dZBarrel_ << "|" << dPhiBarrel_ << " and "
            << ftpar_->nEtaEndcap_ << ":" << ftpar_->nPhiEndcap_ << ":" << getCells(2) << " cells for endcap; dphi "
            << dPhiEndcap_ << " The Limits in R are" << std::endl;
  for (unsigned int k = 0; k < rLimits_.size(); ++k) {
    std::cout << "[" << k << "] " << rLimits_[k] << " ";
    if (k % 8 == 7)
      std::cout << std::endl;
  }
  if ((rLimits_.size() - 1) % 8 != 7)
    std::cout << std::endl;
#endif
}
