...
int32_t HGCalWaferIndex::waferIndex(int32_t layer, int32_t waferU,
                                    int32_t waferV, bool old) {
  int32_t id(0);
  if (old) {
    id |=
        (((waferU & kHGCalWaferCopyMask) << kHGCalWaferCopyOffset) |
...
int HGCalWaferIndex::waferCopy(const int32_t id) {
  return (id >> kHGCalWaferCopyOffset) & kHGCalWaferCopyMask;
}

bool HGCalWaferIndex::waferFormat(const int32_t id) {
  return ((id & kHGCalLayerOldMask) == 0);
}
