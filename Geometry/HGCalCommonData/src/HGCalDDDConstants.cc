...
void HGCalDDDConstants::waferFromPosition(const double x,
  ...
  if (debug)
    edm::LogVerbatim("HGCalGeom") << "waferFromPosition:: Layer " << layer << ":" << ll << " Rot " << rotx << " X " << x
                                  << ":" << xx << " Y " << y << ":" << yy << " side " << zside << " extend " << extend << " initial wafer index " << waferU  << ":" << waferV;;
  ...
  if (waferin) {
	...
	if (ktr != hgpar_->waferInfoMap_.end()) {
      ...
      if (debug) {
	edm::LogVerbatim("HGCalGeom") << "Wafer " << waferU << ":" << waferV << " position " << xx << ":" << yy << " Distance " << dx << ":" << dy << " diff0 " << (dx - rmax) << ":" << (dy - hexside) << " diff1 " << (dy - 0.5 * hexside) << ":" << (dx * tan30deg_ - (hexside - dy));
	...
	if (((dy - 0.5 * hexside) <= tolc) || ((dx * tan30deg_ - (hexside - dy)) <= tolc)) {
	  ...
	  if (debug)
	    edm::LogVerbatim("HGCalGeom") << "WaferFromPosition:: Input " << layer << ":" << ll << ":" << hgpar_->firstLayer_ << ":" << rotx << ":" << x << ":" << y << ":" << hgpar_->xLayerHex_[ll] << ":" << hgpar_->yLayerHex_[ll] << ":" << xx << ":" << yy << " compared with " << hgpar_->waferPosX_[k] << ":" << hgpar_->waferPosY_[k] << " difference " << dx << ":" << dy << ":" << dx * tan30deg_ << ":" << (hexside_ - dy) << " comparator " << rmax_ << ":" << rmaxT_ << ":" << hexside_ << ":" << hexsideT_ << " wafer " << waferU << ":" << waferV << ":" << celltype;
	...
	if (waferHexagon8File()) {
	  ...
	  celltype = HGCalWaferType::getType(index, hgpar_->waferInfoMap_);
	  if (debug)
	    edm::LogVerbatim("HGCalGeom") << "Position (" << x << ", " << y << ") Wafer type:partial:orient:cassette " << celltype << ":" << HGCalWaferType::getPartial(index, hgpar_->waferInfoMap_)  << ":" << HGCalWaferType::getOrient(index, hgpar_->waferInfoMap_) << ":"  << HGCalWaferType::getCassette(index, hgpar_->waferInfoMap_);
	} else {
	  auto itr = hgpar_->typesInLayers_.find(HGCalWaferIndex::waferIndex(layer, waferU, waferV));
	  celltype = ((itr == hgpar_->typesInLayers_.end()) ? HGCSiliconDetId::HGCalCoarseThick	: hgpar_->waferTypeL_[itr->second]);
	}
	xx -= (dx0 + hgpar_->waferPosX_[k]);
	yy -= (dy0 + hgpar_->waferPosY_[k]);
	break;
