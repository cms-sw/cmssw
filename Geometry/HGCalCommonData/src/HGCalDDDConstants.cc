...
HGCalDDDConstants::HGCalDDDConstants(const HGCalParameters* hp, const std::string& name)
  ...
  if (waferHexagon6() || waferHexagon8()) {
    ...
    cellOffset_ = std::make_unique<HGCalCellOffset>(hgpar_->waferSize_, hgpar_->nCellsFine_, hgpar_->nCellsCoarse_, hgpar_->guardRingOffset_, hgpar_->mouseBite_, hgpar_->sensorSizeOffset_);
#ifdef EDM_ML_DEBUG
