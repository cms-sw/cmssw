...
public:
  ...
  //tray
  unsigned int btlTray(const DetId &id) const { return ((id.rawId() >> btlVals_.trayStartBit_) & btlVals_.trayMask_); }

  // ring id
  unsigned int etlRing(const DetId &id) const { return ((id.rawId() >> etlVals_.ringStartBit_) & etlVals_.ringMask_); }
  ...
  //side
  unsigned int btlSide(const DetId &id) const { return ((id.rawId() >> btlVals_.sideStartBit_) & btlVals_.sideMask_); }

  unsigned int etlSide(const DetId &id) const { return ((id.rawId() >> etlVals_.sideStartBit_) & etlVals_.sideMask_); }
  ...
  DetId btlDetId(uint32_t side, uint32_t layer, uint32_t tray, uint32_t module) const {
    ...
    rawid |= MTDDetId::BTL << MTDDetId::kMTDsubdOffset | (side & btlVals_.sideMask_) << btlVals_.sideStartBit_ |
  ...
  DetId etlDetId(uint32_t side, uint32_t layer, uint32_t ring, uint32_t module) const {
    DetId id(DetId::Forward, ForwardSubdetector::FastTime);
    uint32_t rawid = id.rawId();
    rawid |= MTDDetId::ETL << MTDDetId::kMTDsubdOffset | (side & etlVals_.sideMask_) << etlVals_.sideStartBit_ |
