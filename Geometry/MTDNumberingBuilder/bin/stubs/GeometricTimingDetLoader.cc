...
GeometricTimingDetLoader::GeometricTimingDetLoader(const edm::ParameterSet& iConfig) {
  LogDebug("GeometricTimingDetLoader") << "GeometricTimingDetLoader::GeometricTimingDetLoader" << std::endl;
}
...
void GeometricTimingDetLoader::beginRun(edm::Run const& /* iEvent */, edm::EventSetup const& es) {
  std::cout << "GeometricTimingDetLoader::beginJob" << std::endl;
  auto* pgd = new PGeometricTimingDet;
  ...
  if (mydbservice->isNewTagRequest("PGeometricTimingDetRcd")) {
    mydbservice->createNewIOV<PGeometricTimingDet>(
        pgd, mydbservice->beginOfTime(), mydbservice->endOfTime(), "PGeometricTimingDetRcd");
  } else {
