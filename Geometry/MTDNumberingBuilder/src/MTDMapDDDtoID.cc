...
void MTDMapDDDtoID::buildAllStep2(const GeometricTimingDet* theTracker) {
  ...
  edm::LogInfo("MTDMapDDDtoID") << "Created MTDMapDDDtoID; results in " << allDetectors.size()
                                << " detectors numbered.";
}
...
std::vector<MTDMapDDDtoID::nav_type> const& MTDMapDDDtoID::allNavTypes() const { return navVec; }
