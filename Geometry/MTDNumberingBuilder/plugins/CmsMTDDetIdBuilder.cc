...
CmsMTDDetIdBuilder::CmsMTDDetIdBuilder(std::vector<int> detidShifts) : m_detidshifts() {
  if (detidShifts.size() != nSubDet * maxLevels)
    edm::LogError("WrongConfiguration") << "Wrong configuration of TrackerGeometricTimingDetESModule. Vector of "
...
GeometricTimingDet* CmsMTDDetIdBuilder::buildId(GeometricTimingDet* mtd) {
  LogDebug("BuildingTrackerDetId") << "Starting to build MTD DetIds";

  DetId t(DetId::Forward, 0);
  mtd->setGeographicalID(t);
  iterate(mtd, 0, mtd->geographicalID().rawId());

  return mtd;
}

void CmsMTDDetIdBuilder::iterate(GeometricTimingDet* in, int level, unsigned int ID) {
  ...
  switch (level) {
    ...
    case 0: {
      for (uint32_t i = 0; i < (in)->components().size(); i++) {
        GeometricTimingDet* component = in->component(i);
