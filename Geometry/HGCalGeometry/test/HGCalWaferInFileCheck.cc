...
void HGCalWaferInFileCheck::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  if ((hgdc.geomMode() == HGCalGeometryMode::Hexagon8) || (hgdc.geomMode() == HGCalGeometryMode::Hexagon8Full)) { 
	...
	std::cout << "ID[" << k << "]: (" << (hgdc.getLayerOffset() + layer) << ", " << waferU << ", " << waferV << ", " << typex << ") at (" << std::setprecision(4) << xy.first << ", " << xy.second << ", " << hgdc.waferZ(layer, true) << ") not valid" << std::endl;
	++bad1;
      }
    }
	...
	if (!hgdc.waferFileInfoExist(indx)) {
	  ...
	  if (part != HGCalTypes::WaferOut) {
	    ...
	    std::cout << "ID[" << k << "]: (" << (hgdc.getLayerOffset() + id.layer()) << ", " << id.waferU() << ", " << id.waferV() << ", " << typex << ")  at (" << std::setprecision(4) << xy.first << ", " << xy.second << ", " << hgdc.waferZ(id.layer(), true) << ") not in wafer-list" << std::endl;
	    ++bad2;
	  } else {
	    ++xtra;
	  }
    ...
    std::cout << "\n\nFinds " << bad2 << " missing wafers among " << all << " valid wafers and " << xtra << " extra ones" << std::endl << std::endl;
	...
	if ((!typeOK) || (!partOK) || (!rotnOK)) {
	  ...
	  const auto& xy = hgdc.waferPosition(layer, waferU, waferV, true, false);
	  std::cout << "ID[" << k << "]: (" << (hgdc.getLayerOffset() + layer) << ", " << waferU << ", " << waferV << ", " << type1 << ":" << type2 << ", " << partx1 << ":" << partx2 << ", " << rotn1 << ":" << rotn2 << ") at (" << std::setprecision(4) << xy.first << ", " << xy.second << ", " << hgdc.waferZ(layer, true) << ") failure flag " << typeOK << ":" << partOK << ":" << rotnOK << std::endl;
	}
      }
    }
    std::cout << "\n\nFinds " << badG << " (" << badT << "[" << badT1 << ":" << badT2 << "]:" << badP << ":" << badR << ") mismatch among " << allG << " wafers with the same indices" << std::endl << std::endl;
  }
}
