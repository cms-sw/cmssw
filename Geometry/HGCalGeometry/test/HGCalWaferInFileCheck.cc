...
void HGCalWaferInFileCheck::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  if (hgdc.waferHexagon8()) {
    ...
    edm::LogVerbatim("HGCalGeom") << "Old Partial types: 1|Five, 2|ChopTwo, 3|ChopTwoM, 4|Half, 5|Semi, 6|Semi2, 7|Three, 8|Half2, 9|Five2\nNew Partial types: 11|LDTop, 12|LDBottom, 13|LDLeft, 14|LDRight, 15|LDFive, 16|LDThree, 21|HDTop, 22|HDBottom, 23|HDLeft, 24|HDRight, 25|HDFive\nCommon Partial types: 0|Full, 99|Out";
    ...
    for (unsigned int k = 0; k < hgdc.waferFileSize(); ++k) {
      ...
      if (geom->topology().validModule(id, 3)) {
        ...
        bool partOK = ((part1 == part2) || ((part1 == HGCalTypes::WaferFull) && (part2 == HGCalTypes::WaferOut)) || ((part2 == HGCalTypes::WaferFull) && (part1 != HGCalTypes::WaferOut)));
