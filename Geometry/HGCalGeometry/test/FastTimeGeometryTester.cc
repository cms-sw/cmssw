...
public:
  ...
  ~FastTimeGeometryTester() override = default;
...
private:
  ...
  const edm::ESGetToken<FastTimeGeometry, IdealGeometryRecord> geomToken_;
  int type_;
};
...
void FastTimeGeometryTester::analyze(const edm::Event&, const edm::EventSetup& iSetup) {
...
void FastTimeGeometryTester::doTest(const FastTimeGeometry* geom, ForwardSubdetector subdet) {
  const std::vector<DetId>& ids = geom->getValidDetIds();
  edm::LogVerbatim("FastTimeGeom") << ids.size() << " valid ids for " << geom->cellElement() << std::endl;
