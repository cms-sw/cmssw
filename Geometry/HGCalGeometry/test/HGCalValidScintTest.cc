...
private:
  ...
  struct layerInfo {
    int ringMin, ringMax;
    double rMin, rMax;
    layerInfo (int minR = 100, double rMn = 0, int maxR = 0, double rMx = 0) : ringMin(minR), ringMax(maxR), rMin(rMn), rMax(rMx) {};
  };
};
...
void HGCalValidScintTest::analyze(const edm::Event&, const edm::EventSetup& iSetup) {
  ...
  edm::LogVerbatim("HGCalGeom") << "Perform test for HGCalHEScintillatorSensitive Detector " << det << " Mode " << geom->topology().dddConstants().geomMode();
  ...
  for (auto itr = layerMap.begin(); itr != layerMap.end(); ++itr)
    edm::LogVerbatim("HGCalGeom") << "Layer " << itr->first << " lowest Ring " << (itr->second).ringMin << ":" << (itr->second).rMin << " largest Ring " << (itr->second).ringMax << ":" << (itr->second).rMax;
}
