...
private:
  std::vector<std::string> getPoints(double xpos, double ypos, double delX,
				     double delY, double rin, double rout);
...
HGCalWaferInFileTest::HGCalWaferInFileTest(const edm::ParameterSet& iC)
  ...
  std::cout << "Test wafer characteristics for " << nameDetector_ << " using constants of " << nameSense_ << " with verbosity " << verbosity_ << std::endl;
}
...
void HGCalWaferInFileTest::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  if (hgdc.waferHexagon8()) {
    ...
    std::cout << "\n\nFinds " << bad1 << " invalid wafers among " << hgdc.waferFileSize() << " wafers in the list" << std::endl << std::endl;
      ...
      if (geom->topology().validModule(id, 3)) {
        ...
        if (!typeOK) {
          ...
          if (type1 == 0)
            ...
            ++badT2;
          const auto& xy = hgdc.waferPosition(layer, waferU, waferV, true, false);
          std::cout << "ID[" << k << "]: (" << (hgdc.getLayerOffset() + layer) << ", " << waferU << ", " << waferV << ", " << type1 << ":" << type2 << ") at (" << std::setprecision(4) << xy.first << ", " << xy.second << ", " << hgdc.waferZ(layer, true) << ") failure flag " << typeOK << std::endl;
        }
      }
    }
    std::cout << "\n\nFinds " << badT << "[" << badT1 << ":" << badT2 << "] mismatch in type among " << allG << " wafers with the same indices" << std::endl << std::endl;
    ...
    for (unsigned int k = 0; k < hgdc.waferFileSize(); ++k) {
	  ...
	  const auto& rr = hgdc.rangeRLayer(layer, true);
	  auto points = getPoints(xy.first, xy.second, delX, delY, rr.first, rr.second);
          std::cout << "ID[" << k << "]: (" << (hgdc.getLayerOffset() + layer) << ", " << waferU << ", " << waferV << "," << type2 << ", " << partx1 << ":" << partx2 << ", " << rotn1 << ":" << rotn2 << ") at (" << std::setprecision(4) << xy.first << ", " << xy.second << ", " << hgdc.waferZ(layer, true) << ") failure flag " << partOK << ":" << rotnOK << " with " << points.size() << " points:";
	  for (auto point : points)
	    std::cout << " " << point;
	  std::cout << " in the region " << rr.first << ":" << rr.second << std::endl;
        }
    ...
    std::cout << "\n\nFinds " << badG << " ("  << badP << ":" << badR << ") mismatch in partial|orientation among " << allX << " wafers with the same indices" << std::endl << std::endl;
  }
}

std::vector<std::string> HGCalWaferInFileTest::getPoints(double xpos, double ypos, double delX,
							 double delY, double rin, double rout) {
  ...
  if (verbosity_ > 0)
    std::cout << "I/p " << xpos << ":" << ypos << ":" << delX << ":" << delY << ":" << rin << ":" << rout << " Corners " << ncor << " iok " << iok << std::endl;
