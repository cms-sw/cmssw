...
HGCalMouseBite::HGCalMouseBite(const HGCalDDDConstants& hgc, const bool rot) {
  ...
  if (modeUV) {
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCalGeom") << "Creating HGCMosueBite with cut at " << cut << " along " << angle.size() << " axes in wafers of size " << wafersize;
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCalGeom") << "HGCalMouseBite:: " << rejectFine_.size() << " masked u, v's among the fine grain wafers:";
	...
	if (((v - u) < nc) && ((u - v) <= nc)) {
	  ...
	  for (auto proj : projXY) {
	    double dist = (rot ? (yp * proj.first + xp * proj.second) : (xp * proj.first + yp * proj.second));
	    if (dist > cut) rejectCoarse_.emplace_back(100 * u + v);
	    break;
	  }
	}
      }
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCalGeom") << "HGCalMouseBite:: " << rejectCoarse_.size() << " masked u, v's among the coarse grain wafers:";
