...
CaloGeometryBuilder::produceAligned( const CaloGeometryRecord& iRecord )
{
  ...
  for ( std::vector<std::string>::const_iterator ite ( theCaloList.begin() ) ;
    ...
    {
      std::cout << "CaloGeometryBuilder::produceAligned-> producing geometry for " << *ite << std::endl;
      // look for HCAL parts
      ...
      else if ( (*ite) == EcalEndcapGeometry::producerTag() ) 
	{
      ...
      // look for Ecal Shashlik
      else if ( (*ite) == EcalShashlikGeometry::producerTag() ) 
	{
	  edm::LogInfo("CaloGeometryBuilder") << "Building EcalShashlik reconstruction geometry";
	  iRecord.getRecord<EcalShashlikGeometryRecord>().get( EcalShashlikGeometry::producerTag(), pG ); 
