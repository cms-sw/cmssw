...
private:
  // ----------member data ---------------------------
  void build( CaloGeometry* cg , const HcalTopology& ht,
	      DetId::Detector     det, 
	      int                 subdetn, 
	      const char*         name,
	      unsigned int        histi   );

  void buildHcal( CaloGeometry* cg , const HcalTopology& ht,
		  DetId::Detector     det, 
		  int                 subdetn, 
		  const char*         name,
		  unsigned int        histi   );
  ...
  void cmpset( CaloSubdetectorGeometry*       geom ,
	       const GlobalPoint&             gp   ,
	       const double                   dR     ) ;

  void ovrTst( CaloGeometry* cg      , 
...
void 
CaloGeometryAnalyzer::cmpset( CaloSubdetectorGeometry* geom ,
			      const GlobalPoint&             gp   ,
			      const double                   dR     ) 
{
...
void 
CaloGeometryAnalyzer::ovrTst( CaloGeometry* cg      , 
			      CaloSubdetectorGeometry* geom ,
			      const EEDetId&   id   , 
			      std::fstream&    fOvr   )
{
   ...
   if( iphi != 0 )
   {
      ...
      EcalEndcapGeometry* eeG ( dynamic_cast<EcalEndcapGeometry*>( geom ) );
			      ...
			      std::fstream&    fOvr   )
{
   ...
   if( 85 == std::abs( ieta ) )
   {
      EcalBarrelGeometry* ebG ( dynamic_cast<EcalBarrelGeometry*>( geom ) );
      auto cell ( geom->getGeometry(id) ) ;
...
void 
CaloGeometryAnalyzer::buildHcal( CaloGeometry*       cg      , 
...
void 
CaloGeometryAnalyzer::build( CaloGeometry*       cg      , 
			     const HcalTopology& ht      ,
			     DetId::Detector     det     , 
			     int                 subdetn , 
			     const char*         name    ,
			     unsigned int        histi     ) 
{
   ...
   for(const auto & i : ids) 
   {
	 ...
	 if (subdetn == EcalPreshower) 
	 {
	    ...
	    ESDetId closestCell=ESDetId((dynamic_cast<EcalPreshowerGeometry*>(geom))->getClosestCellInPlane(cell->getPosition(),esid.plane()));
	    f << "  // Return position is " << closestCell << std::endl;
	    //sanity checks
...
CaloGeometryAnalyzer::analyze( const edm::Event& /*iEvent*/, const edm::EventSetup& iSetup )
{
   ...
   if (pass_==0) 
   {
      ...
      std::cout<<"**Ecal Barrel avg Radius = "
	       << ((EcalBarrelGeometry*)(pG->getSubdetectorGeometry(DetId::Ecal, EcalBarrel)))->avgRadiusXYFrontFaceCenter()
	       << std::endl ;

      std::cout<<"**Ecal Endcap avg Zabs = "
	       << ((EcalEndcapGeometry*)(pG->getSubdetectorGeometry(DetId::Ecal, EcalEndcap )))->avgAbsZFrontFaceCenter()
	       << std::endl ;
