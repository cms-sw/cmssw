...
namespace {
  static long algorithm(dd4hep::Detector& description,
    ...
    LogVerbatim("TIDGeom") << "DDTIDAxialCableAlgo debug: Parameters for creating "
    ...
    LogVerbatim("TIDGeom") << "DDTIDAxialCableAlgo debug: Parent " << mother.name()
                        << "\tChild " << childName
                        << " NameSpace " << ns.name()
    ...
    for (int k=0; k<(int)(zposWheel.size()); k++) {
      ...
      logs.emplace_back(Volume(solid.name(),
			       solid,
			       ns.material("materials:Air")));
    }
    ...
    r = thktot*rMax/rTop;
    z = zBend - thktot;
    LogVerbatim("TIDGeom") << "DDTIDAxialCableAlgo test: Thk " << thk
	...
	LogVerbatim("TIDGeom") << "DDTIDAxialCableAlgo test " << logs[k].name()
			       << " number " << i+1
			       << " positioned in " << mother.name() << " at " << tran
			       << " with " << rotation;
      }
    } 
    return s_executed;
