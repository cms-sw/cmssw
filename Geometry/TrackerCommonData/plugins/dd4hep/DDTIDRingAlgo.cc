...
static long algorithm(Detector& /* description */, cms::DDParsingContext& ctxt, xml_h e) {
  ...
  edm::LogVerbatim("TIDGeom") << "Parent " << mother.name() << "\tModule " << moduleName[0] << ", " << moduleName[1] << "\tICC "
  ...
  for (int i = 0; i < number; i++) {
    ...
    edm::LogVerbatim("TIDGeom") << module.name() << " number " << i + 1 << " positioned in " << mother.name() << " at " << trmod
                        << " with " << rotation;
    //Now the ICC
    if (i % 2 == 0) {
      zpos = zICC[0];
    ...
    edm::LogVerbatim("TIDGeom") << icc.name() << " number " << i + 1 << " positioned in " << mother.name() << " at " << tricc
                        << " with " << rotation;
  }
  return 1;
}
