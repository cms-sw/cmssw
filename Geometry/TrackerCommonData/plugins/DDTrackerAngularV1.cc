...
DDTrackerAngularV1::DDTrackerAngularV1() { LogDebug("TrackerGeom") << "DDTrackerAngularV1 info: Creating an instance"; }
...
void DDTrackerAngularV1::initialize(const DDNumericArguments& nArgs,
  ...
  LogDebug("TrackerGeom") << "DDTrackerAngularV1 debug: Parent " << parentName << "\tChild " << childName
...
void DDTrackerAngularV1::execute(DDCompactView& cpv) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    LogDebug("TrackerGeom") << "DDTrackerAngularV1 test " << child << " number " << copy << " positioned in " << mother
