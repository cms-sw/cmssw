...
DDTECAxialCableAlgo::DDTECAxialCableAlgo() { LogDebug("TECGeom") << "DDTECAxialCableAlgo info: Creating an instance"; }
...
void DDTECAxialCableAlgo::initialize(const DDNumericArguments& nArgs,
  ...
  LogDebug("TECGeom") << "DDTECAxialCableAlgo debug: Parameters for creating " << startAngle.size()
                      << " axial cables and positioning " << n << " copies in Service volume\n"
                      << "                            zStart " << zStart << " zEnd " << zEnd << " rMin " << rMin
                      << " rMax " << rMax << " Cable width " << width / CLHEP::deg << " thickness " << thickR << ", "
                      << thickZ << " dZ " << dZ << "\n                            Range, Delta "
                      << rangeAngle / CLHEP::deg << ", " << delta / CLHEP::deg;
  for (int i = 0; i < (int)(startAngle.size()); i++)
    LogDebug("TECGeom") << "                          Cable " << i << " from Z " << zPos[i] << " startAngle "
  ...
  LogDebug("TECGeom") << "DDTECAxialCableAlgo debug: Parent " << parentName << "\tChild " << childName << " NameSpace "
...
void DDTECAxialCableAlgo::execute(DDCompactView& cpv) {
  ...
  for (int k = 0; k < (int)(startAngle.size()); k++) {
    ...
    LogDebug("TECGeom") << "DDTECAxialCableAlgo test: " << DDName(name, idNameSpace) << " Polycone made of " << matName
                        << " from " << -0.5 * width / CLHEP::deg << " to " << 0.5 * width / CLHEP::deg << " and with "
                        << pconZ.size() << " sections ";
    for (int ii = 0; ii < (int)(pconZ.size()); ii++)
      LogDebug("TECGeom") << "\t"
                          << "\tZ[" << ii << "] = " << pconZ[ii] << "\tRmin[" << ii << "] = " << pconRmin[ii]
                          << "\tRmax[" << ii << "] = " << pconRmax[ii];
    ...
    for (i = 0; i < n; i++) {
      ...
      LogDebug("TECGeom") << "DDTECAxialCableAlgo test " << DDName(name, idNameSpace) << " number " << i + 1
