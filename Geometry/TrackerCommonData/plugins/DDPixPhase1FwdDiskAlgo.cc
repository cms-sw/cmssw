...
void DDPixPhase1FwdDiskAlgo::execute(DDCompactView& cpv) {
  ...
  for (int iBlade = 0; iBlade < nBlades; ++iBlade) {
    if (flagString[iBlade] == flagSelector[0]) {
      ...
      double phix =
          std::atan2(std::cos(bladeAngle) * std::sin(phi) + std::cos(phi) * std::sin(bladeTilt) * std::sin(bladeAngle),
                     std::cos(phi) * std::cos(bladeAngle) - std::sin(phi) * std::sin(bladeTilt) * std::sin(bladeAngle));
      double thetx = std::acos(-std::cos(bladeTilt) * std::sin(bladeAngle));
      double phiz =
          std::atan2(std::sin(phi) * std::sin(bladeAngle) - std::cos(phi) * std::cos(bladeAngle) * std::sin(bladeTilt),
                     std::cos(phi) * std::sin(bladeAngle) + std::cos(bladeAngle) * std::sin(phi) * std::sin(bladeTilt));
