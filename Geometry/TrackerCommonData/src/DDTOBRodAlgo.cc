...
DDTOBRodAlgo::DDTOBRodAlgo():
  ...
  edm::LogInfo("TrackerGeom") << "DDTOBRodAlgo info: Creating an instance";
}
...
void DDTOBRodAlgo::initialize(const DDNumericArguments & nArgs,
  ...
  LogDebug("TrackerGeom") << "DDTOBRodAlgo debug: Parent " << parentName 
  ...
  LogDebug("TrackerGeom") << "DDTOBRodAlgo debug:\t" << module <<" positioned "
			  << moduleRot.size() << " times";
  for (i=0; i<moduleRot.size(); i++) 
    LogDebug("TrackerGeom") << "\tRotation " << moduleRot[i] << "\ty = " 
			    << moduleY[i] << "\tz = " << moduleZ[i];

  connect      = vsArgs["ICCName"];
  connectY     = vArgs["ICCY"];    
  connectZ     = vArgs["ICCZ"];    
  LogDebug("TrackerGeom") << "DDTOBRodAlgo debug:\t" << connect.size() 
			  << " ICC positioned with no rotation";
  for (i=0; i<connect.size(); i++) 
    LogDebug("TrackerGeom") << "\t" << connect[i] << "\ty = " << connectY[i] 
			    << "\tz = " << connectZ[i];
}

void DDTOBRodAlgo::execute() {
  
  LogDebug("TrackerGeom") << "==>> Constructing DDTOBRodAlgo...";
  ...
  for (i=0; i<sideRod.size(); i++) {
    for (j=0; j<sideRodX.size(); j++) {
      ...
      LogDebug("TrackerGeom") << "DDTOBRodAlgo test: "  << child << " number " 
  ...
  LogDebug("TrackerGeom") << "DDTOBRodAlgo test: " << child1 << " number 1 "
  ...
  LogDebug("TrackerGeom") << "DDTOBRodAlgo test: " << child2 << " number 1 "
  ...
  LogDebug("TrackerGeom") << "DDTOBRodAlgo test: " << child3 << " number 1 "
  ...
  LogDebug("TrackerGeom") << "<<== End of DDTOBRodAlgo construction ...";
}
