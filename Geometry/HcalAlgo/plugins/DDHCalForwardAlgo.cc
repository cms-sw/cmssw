...
void DDHCalForwardAlgo::initialize(const DDNumericArguments & nArgs,
  ...
  edm::LogInfo("HCalGeom") << "DDHCalForwardAlgo debug: Cell material " << cellMat
  ...
  for (unsigned int i = 0; i < number.size(); i++)
    edm::LogInfo("HCalGeom") << "\t" << i << " Number of children " << size[i] 
			     << " occurence " << number[i] << " first child index "
			     << type[i];

  idNameSpace = DDCurrentNamespace::ns();
  DDName parentName = parent().name(); 
  edm::LogInfo("HCalGeom") << "DDHCalForwardAlgo debug: Parent " << parentName
			   << " NameSpace " << idNameSpace;
}

void DDHCalForwardAlgo::execute(DDCompactView& cpv) {
  
  edm::LogInfo("HCalGeom") << "==>> Constructing DDHCalForwardAlgo...";
  ...
  for (unsigned int i=0; i<number.size(); i++) {
    ...
    for (int j=0; j<number[i]; j++) {
      ...
      DDSolid solid = DDSolidFactory::box(DDName(name, idNameSpace),
					  dx, cellDy, cellDz);
      edm::LogInfo("HCalGeom") << "DDHCalForwardAlgo test: " 
      ...
      edm::LogInfo("HCalGeom") << "DDHCalForwardAlgo test: " << solid.ddname() 
	...
	edm::LogInfo("HCalGeom") << "DDHCalForwardAlgo test: " << child 
  ...
  edm::LogInfo("HCalGeom") << "<<== End of DDHCalForwardAlgo construction ...";
}
