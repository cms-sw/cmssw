...
void DDHCalTBCableAlgo::initialize(const DDNumericArguments & nArgs,
  ...
  edm::LogInfo("HCalGeom") << "DDHCalTBCableAlgo debug: General material " 
			   << genMat << "\tSectors "  << nsectors << ", " 
			   << nsectortot << "\tHalves " << nhalf << "\tRin " <<rin;
  for (unsigned int i = 0; i < theta.size(); i++)
    edm::LogInfo("HCalGeom") << "\t" << i << " Theta " << theta[i] << " rmax " 
			     << rmax[i] << " zoff " << zoff[i];
  edm::LogInfo("HCalGeom") << "\tCable mockup made of " << absMat << "\tThick " 
  ...
  edm::LogInfo("HCalGeom") << "DDHCalTBCableAlgo debug: Parent " << parentName
...
void DDHCalTBCableAlgo::execute(DDCompactView& cpv) {
  ...
  edm::LogInfo("HCalGeom") << "==>> Constructing DDHCalTBCableAlgo...";
  unsigned int i=0;
  ...
  solid = DDSolidFactory::polyhedra(DDName(idName, idNameSpace),
				    ...
				    pgonRmin, pgonRmax);
  edm::LogInfo("HCalGeom") << "DDHCalTBCableAlgo test: " 
  ...
  for (int ii=0; ii<nsectortot; ii++) {
    ...
    if (phideg != 0) {
      ...
      if (!rotation) {
	edm::LogInfo("HCalGeom") << "DDHCalTBCableAlgo test: Creating a new "
   ...
   cpv.position(seclogic, genlogic, ii+1, DDTranslation(0.0, 0.0, 0.0), rotation);
    edm::LogInfo("HCalGeom") << "DDHCalTBCableAlgo test: " << seclogic.name() 
  ...
  solid = DDSolidFactory::trap(DDName(name, idNameSpace), dz, 0, 0, dy, dx1, 
			       dx1, 0, dy, dx2, dx2, 0);
  edm::LogInfo("HCalGeom") << "DDHCalTBCableAlgo test: " << solid.name() 
  ...
  string rotstr = name;
  edm::LogInfo("HCalGeom") << "DDHCalTBCableAlgo test: Creating a new rotation: " 
  ...
  edm::LogInfo("HCalGeom") << "DDHCalTBCableAlgo test: " << glog.name() 
  ...
  edm::LogInfo("HCalGeom") << "DDHCalTBCableAlgo test: " << cablog1.name() 
 ...
 cpv.position(cablog2, glog, 1, DDTranslation(xpos, 0.0, 0.0), DDRotation());
  edm::LogInfo("HCalGeom") << "DDHCalTBCableAlgo test: " << cablog2.name() 
 ...
 cpv.position(cablog2, glog, 2, DDTranslation(-xpos, 0.0, 0.0), DDRotation());
  ...
  edm::LogInfo("HCalGeom") << "<<== End of DDHCalTBCableAlgo construction ...";
}
