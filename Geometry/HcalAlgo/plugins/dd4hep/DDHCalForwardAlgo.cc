...
#ifdef EDM_ML_DEBUG
      edm::LogVerbatim("HCalGeom") << "DDHCalForwardAlgo: " << solid.name() << " Box made of " << cellMat
...
#ifdef EDM_ML_DEBUG
        edm::LogVerbatim("HCalGeom") << "DDHCalForwardAlgo: " << childName[indx] << " number " << k + 1 << " positioned in "
                                     << glog.name() << " at " << r1 << " with " << rot;
#endif
        xpos += 2 * cellDx;
      }
    }
