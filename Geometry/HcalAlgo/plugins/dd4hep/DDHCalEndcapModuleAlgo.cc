...
struct HCalEndcapModuleAlgo {
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HCalGeom") << "DDHCalEndcapModuleAlgo: Zmin " << convertCmToMm(zMinBlock) << "\tZmax " << convertCmToMm(zMaxBlock)
                                 << "\tZ1Beam " << convertCmToMm(z1Beam) << "\tZiDip " << convertCmToMm(ziDip) << "\tDzStep " << convertCmToMm(dzStep)
                                 << "\tModuleThick " << convertCmToMm(moduleThick) << "\tLayerThick " << convertCmToMm(layerThick) << "\tScintThick "
                                 << convertCmToMm(scintThick);
#endif
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HCalGeom") << "DDHCalEndcapModuleAlgo: RMaxFront " << convertCmToMm(rMaxFront) << "\tRmaxBack " << convertCmToMm(rMaxBack)
                                 << "\tTrims " << convertCmToMm(trimLeft) << ":" << convertCmToMm(trimRight) << "\tTolAbs " << convertCmToMm(tolAbs);
#endif
  ...
  void constructInsideModule0(cms::DDParsingContext& ctxt, xml_h e, dd4hep::Volume& module) {
    ...
    for (unsigned int iphi = 0; iphi < phiName.size(); iphi++) {
      ...
      solid = dd4hep::Trap(
          ns.prepend(name), 0.5 * layerThick, 0, 0, parm.yh1, parm.bl1, parm.tl1, parm.alp, parm.yh2, parm.bl1, parm.tl2, parm.alp);
#ifdef EDM_ML_DEBUG
      edm::LogVerbatim("HCalGeom") << "DDHCalEndcapModuleAlgo: " << solid.name() << " Trap made of " << plasticMat
                                   << " of dimensions " << convertCmToMm(0.5 * layerThick) << ", 0, 0, " << convertCmToMm(parm.yh1) << ", "
                                   << convertCmToMm(parm.bl1) << ", " << convertCmToMm(parm.tl1) << ", " << convertRadToDeg(parm.alp) << ", "
                                   << convertCmToMm(parm.yh2) << ", " << convertCmToMm(parm.bl2) << ", " << convertCmToMm(parm.tl2) << ", "
      ...
      edm::LogVerbatim("HCalGeom") << "DDHCalEndcapModuleAlgo: " << glog.name() << " number " << (idOffset + layer + 1)
                                   << " positioned in " << module.name() << " at (" << convertCmToMm(parm.xpos) << ", " << convertCmToMm(parm.ypos) << ", " << convertCmToMm(parm.zpos) << ") with rotation: " << rot;
#endif
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HCalGeom") << "DDHCalEndcapModuleAlgo: Front " << convertCmToMm(zi) << ", " << convertCmToMm(rinF) << ", " << convertCmToMm(routF) << " Back "
                                 << convertCmToMm(zo) << ", " << convertCmToMm(rinB) << ", " << convertCmToMm(routB);
#endif
    HCalEndcapModuleAlgo::HcalEndcapPar parm = parameterLayer(0, rinF, routF, rinB, routB, zi, zo);
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HCalGeom") << "DDHCalEndcapModuleAlgo: Trim " << convertCmToMm(tolAbs) << " Param " << convertCmToMm(parm.yh1) << ", "
                                 << convertCmToMm(parm.bl1) << ", " << convertCmToMm(parm.tl1) << ", " << convertCmToMm(parm.yh2) << ", " << convertCmToMm(parm.bl2) << ", "
                                 << convertCmToMm(parm.tl2);
#endif
    parm.bl1 -= tolAbs;
    parm.tl1 -= tolAbs;
    parm.bl2 -= tolAbs;
    ...
    edm::LogVerbatim("HCalGeom") << "DDHCalEndcapModuleAlgo: " << solid.name() << " Trap made of " << matabsorbr.name()
                                 << " of dimensions " << convertCmToMm(0.5 * moduleThick) << ", " << convertRadToDeg(parm.theta)
                                 << ", " << convertRadToDeg(parm.phi) << ", " << convertCmToMm(parm.yh1) << ", " << convertCmToMm(parm.bl1) << ", "
                                 << convertCmToMm(parm.tl1) << ", " << convertRadToDeg(parm.alp) << ", " << convertCmToMm(parm.yh2) << ", "
                                 << convertCmToMm(parm.bl2) << ", " << convertCmToMm(parm.tl2) << ", " << convertRadToDeg(parm.alp);
#endif
    ...
    edm::LogVerbatim("HCalGeom") << "DDHCalEndcapModuleAlgo: " << glog.name() << " number 1 positioned in "
                                 << module.name() << " at (" << convertCmToMm(parm.xpos) << ", " << convertCmToMm(parm.ypos) << ", " << convertCmToMm(parm.zpos) << ") with rotation: " << rot;
#endif
  }
  ...
  void constructInsideModule(cms::DDParsingContext& ctxt, xml_h e, dd4hep::Volume& module) {
    ...
    for (unsigned int i = 0; i < layerName.size(); i++) {
      ...
      for (unsigned int iphi = 0; iphi < phiName.size(); iphi++) {
...
#ifdef EDM_ML_DEBUG
        edm::LogVerbatim("HCalGeom") << "DDHCalEndcapModuleAlgo: Layer " << i << " Phi " << iphi << " Front " << convertCmToMm(ziAir)
                                     << ", " << convertCmToMm(rinF) << ", " << convertCmToMm(routF) << " Back " << convertCmToMm(zo) << ", " << convertCmToMm(rinB) << ", "
                                     << convertCmToMm(routB);
#endif
        ...
        solid = dd4hep::Trap(
            ns.prepend(name), 0.5 * layerThick, 0, 0, parm.yh1, parm.bl1, parm.tl1, parm.alp, parm.yh1, parm.bl1, parm.tl1, parm.alp);
#ifdef EDM_ML_DEBUG
        edm::LogVerbatim("HCalGeom") << "DDHCalEndcapModuleAlgo: " << solid.name() << " Trap made of "
                                     << matplastic.name() << " of dimensions " << convertCmToMm(0.5 * layerThick) << ", 0, 0, "
                                     << convertCmToMm(parm.yh1) << ", " << convertCmToMm(parm.bl1) << ", " << convertCmToMm(parm.tl1) << ", "
                                     << convertRadToDeg(parm.alp) << ", " << convertCmToMm(parm.yh1) << ", " << convertCmToMm(parm.bl1) << ", "
        ...
        edm::LogVerbatim("HCalGeom") << "DDHCalEndcapModuleAlgo: " << plog.name() << " number " << (idOffset + layer + 1)
                                     << " positioned in " << glog.name() << " at (0, " << convertCmToMm(ypos) << ", 0) with no rotation";
#endif
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HCalGeom") << "ModNumber " << modNumber << " " << convertCmToMm(zi) << " " << convertCmToMm(zo) << " " << slopeTopF << " "
                                 << slopeTop << " " << slopeBot << " " << convertCmToMm(rin) << " " << convertCmToMm(rout) << " " << convertCmToMm(getTrim(iphi));
#endif
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HCalGeom") << "Output Dimensions " << convertCmToMm(parm.yh1) << " " << convertCmToMm(parm.bl1) << " " << convertCmToMm(parm.tl1) << " "
                                 << convertRadToDeg(parm.alp) << " Position " << convertCmToMm(parm.xpos) << " " << convertCmToMm(parm.ypos) << " "
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HCalGeom") << "dx|dy|r " << convertCmToMm(dx) << ":" << convertCmToMm(dy) << ":" << convertCmToMm(r);
#endif
    if (r > 1.0e-8) {
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HCalGeom") << "Output Dimensions " << convertCmToMm(parm.yh1) << " " << convertCmToMm(parm.bl1) << " " << convertCmToMm(parm.tl1) << " "
                                 << convertCmToMm(parm.yh2) << " " << convertCmToMm(parm.bl2) << " " << convertCmToMm(parm.tl2) << " " << convertRadToDeg(parm.alp)
                                 << " " << convertRadToDeg(parm.theta) << " " << convertRadToDeg(parm.phi)
                                 << " Position " << convertCmToMm(parm.xpos) << " " << convertCmToMm(parm.ypos) << " " << convertCmToMm(parm.zpos);
#endif
    return parm;
  }
  ...
  void constructScintLayer(dd4hep::Volume& detector,
    ...
    dd4hep::Solid solid =
      dd4hep::Trap(ns.prepend(name), 0.5 * dz, 0, 0, parm.yh1, parm.bl1, parm.tl1, parm.alp, parm.yh1, parm.bl1, parm.tl1, parm.alp);
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HCalGeom") << "DDHCalEndcapModuleAlgo: " << solid.name() << " Trap made of " << scintMat
                                 << " of dimensions " << convertCmToMm(0.5 * dz) << ", 0, 0, " << convertCmToMm(parm.yh1) << ", " << convertCmToMm(parm.bl1) << ", "
                                 << convertCmToMm(parm.tl1) << ", " << convertRadToDeg(parm.alp) << ", " << convertCmToMm(parm.yh1) << ", "
                                 << convertCmToMm(parm.bl1) << ", " << convertCmToMm(parm.tl1) << ", " << convertRadToDeg(parm.alp);
#endif
