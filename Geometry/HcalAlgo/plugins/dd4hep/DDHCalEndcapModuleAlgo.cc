...
struct HCalEndcapModuleAlgo {
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HCalGeom") << "DDHCalEndcapModuleAlgo: Parent " << args.parentName() << "   " << modName << " idName "
  ...
  void constructInsideModule0(cms::DDParsingContext& ctxt, xml_h e, dd4hep::Volume& module) {
    ...
    for (unsigned int iphi = 0; iphi < phiName.size(); iphi++) {
      ...
      solid = dd4hep::Trap(0.5 * layerThick,
			   0,
			   0,
			   parm.yh1,
			   parm.bl1,
  ...
  void constructInsideModule(cms::DDParsingContext& ctxt, xml_h e, dd4hep::Volume& module) {
	...
	ns.addSolidNS(ns.prepend(name), solid);
#ifdef EDM_ML_DEBUG
	edm::LogVerbatim("HCalGeom") << "DDHCalEndcapModuleAlgo: " << solid.name() << " Trap made of " << matplastic.name()
  ...
  HcalEndcapPar parameterLayer(unsigned int iphi, 
			       double rinF, 
			       double routF, 
			       double rinB, 
			       double routB, 
  ...
  void constructScintLayer(dd4hep::Volume& detector,
    ...
    dd4hep::Solid solid = dd4hep::Trap(0.5 * dz,
                                       0,
                                       0,
                                       parm.yh1,
                                       parm.bl1,
  ...
  dd4hep::Rotation3D getRotation(const std::string& rotstr) {
    return ((rotstr == "hcalrotations::YXZ4") ?
	    cms::makeRotation3D(90._deg, -90._deg, 90._deg, 0., 0., 0.) :
