...
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("HCalGeom") << "DDHCalTestBeamAlgo: Parameters for positioning-- Eta " << eta << "\tPhi " << convertRadToDeg(phi) << "\tTheta " << convertRadToDeg(theta) << "\tDistance " << HcalDD4HepHelper::convert2mm(distance) << "/" << HcalDD4HepHelper::convert2mm(distanceZ) << "/" << HcalDD4HepHelper::convert2mm(dist) << "\tDz " << HcalDD4HepHelper::convert2mm(dz) << "\tcopyNumber " << copyNumber;
...
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("HCalGeom") << "DDHCalTestBeamAlgo: " << child.name() << " number " << copyNumber << " positioned in " << parent.name() << " at (" << HcalDD4HepHelper::convert2mm(xpos) << ", " << HcalDD4HepHelper::convert2mm(ypos) << ", " << HcalDD4HepHelper::convert2mm(zpos) << ") with rotation: " << rotation;
#endif
  xpos = (dist - dz) * sin(theta) * cos(phi);
  ypos = (dist - dz) * sin(theta) * sin(phi);
  zpos = (dist - dz) * cos(theta);

  edm::LogInfo("HCalGeom") << "DDHCalTestBeamAlgo: Suggested Beam position (" << HcalDD4HepHelper::convert2mm(xpos) << ", " << HcalDD4HepHelper::convert2mm(ypos) << ", " << HcalDD4HepHelper::convert2mm(zpos) << ") and (dist, eta, phi) = (" << HcalDD4HepHelper::convert2mm(dist - dz) << ", " << eta << ", " << phi << ")";
