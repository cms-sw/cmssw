...
GeometricDet::GeometricDet( DDnav_type const & navtype, GeometricEnumType type )
  :_ddd(navtype.begin(),navtype.end()),
   _type(type)
{ 
  //
...
GeometricDet::deepComponents() const
{
  //
...
void
GeometricDet::deepComponents( GeometricDetContainer & cont ) const
{
  if (isLeaf())
...
void
GeometricDet::addComponents( GeometricDetContainer const & cont )
{
  if (_container.empty()) {
    _container=cont;
    return;
...
void
GeometricDet::addComponent( GeometricDet* det )
{
...
GeometricDet::positionBounds() const
{
...
GeometricDet::rotationBounds() const
{
  DD3Vector x, y, z;
