...
void CmsTrackerSubStrctBuilder<FilteredView>::buildComponent(FilteredView& fv, GeometricDet* g, const std::string& s) {
  ...
  edm::LogVerbatim("TrackerNumberingBuilder") << "CmsTrackerSubStrctBuilder::buildComponent " << s << std::endl;
  edm::LogVerbatim("TrackerNumberingBuilder") << "\t" << fv.geoHistory() << std::endl;
  switch (CmsTrackerLevelBuilder<FilteredView>::theCmsTrackerStringToEnum.type(
    ...
    case GeometricDet::layer:
      ...
      edm::LogVerbatim("TrackerNumberingBuilder") << "SubStrct Just broke Layer " << fv.geoHistory() << std::endl;
      break;
    case GeometricDet::OTPhase2Layer:
      theCmsTrackerOTLayerBuilder.build(fv, subdet, s);
      edm::LogVerbatim("TrackerNumberingBuilder") << "SubStrct Just broke OTLayer " << fv.geoHistory() << std::endl;
      break;
    case GeometricDet::wheel:
      theCmsTrackerWheelBuilder.build(fv, subdet, s);
      edm::LogVerbatim("TrackerNumberingBuilder") << "SubStrct Just broke Wheel " << fv.geoHistory() << std::endl;
      break;
    case GeometricDet::disk:
      theCmsTrackerDiskBuilder.build(fv, subdet, s);
      edm::LogVerbatim("TrackerNumberingBuilder") << "SubStrct Just broke Disk " << fv.geoHistory() << std::endl;
      break;
