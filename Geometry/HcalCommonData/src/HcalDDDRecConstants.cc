...
std::vector<HcalDDDRecConstants::HcalActiveLength> HcalDDDRecConstants::getThickActive(const int& type) const {
  ...
  for (auto& bin : bins) {
    ...
    for (auto phi : bin.phis)
      edm::LogVerbatim("HcalGeom") << "Phi " << phi.first << ":" << phi.second / CLHEP::deg;
#endif
...
void HcalDDDRecConstants::getOneEtaBin(HcalSubdetector subdet,
  ...
  if (!layers.empty()) {
    ...
    for (unsigned int l = 0; l < phis.size(); ++l)
      edm::LogVerbatim("HcalGeom") << "[" << l << "] " << phis[l].first << ":" << phis[l].second / CLHEP::deg;
#endif
...
void HcalDDDRecConstants::initialize(void) {
  ...
  for (int i = 0; i < nEta; ++i) {
    ...
    if (ieta >= (int)(hpar->etaTable.size())) {
      edm::LogError("HCalGeom") << "Going beyond the array boundary " << hpar->etaTable.size() << " at index " << i
                                << " of etaTable from SimConstant";
      throw cms::Exception("DDException") << "Going beyond the array boundary " << hpar->etaTable.size() << " at index "
                                          << i << " of etaTable from SimConstant";
    } else {
      etaTable.emplace_back(hpar->etaTable[ieta]);
...
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("HcalGeom") << "Modified eta/deltaphi table for " << nEta << " bins";
  for (int i = 0; i < nEta; ++i)
...
#ifdef EDM_ML_DEBUG
      edm::LogVerbatim("HcalGeom") << "HB " << i << " " << imx << " " << laymax << " " << laymax0;
#endif
...
#ifdef EDM_ML_DEBUG
  for (int i = 0; i < 4; ++i)
    edm::LogVerbatim("HcalGeom") << "Detector Type[" << i << "] iEta " << iEtaMin[i] << ":" << iEtaMax[i]
                                 << " MaxDepth " << maxDepth[i];
#endif
...
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("HcalGeom") << "HB with " << nModule[0] << " modules and " << nHalves[0] << " halves and "
                               << gconsHB.size() << " layers";
...
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("HcalGeom") << "HE with " << nModule[1] << " modules and " << nHalves[1] << " halves and "
                               << gconsHE.size() << " layers";
  ...
  if ((subdet == HcalBarrel) || (subdet == HcalEndcap)) {
...
#ifdef EDM_ML_DEBUG
      edm::LogVerbatim("HcalGeom") << "Eta|Phi|Zside " << eta << ":" << phi << ":" << zside << " with " << oldDep.size()
                                   << " old Depths";
      unsigned int kk(0);
...
#ifdef EDM_ML_DEBUG
          edm::LogVerbatim("HcalGeom") << "New Depth " << ndepth << " old Depth " << odepth << " max " << maxlay;
#endif
          for (int k : phis) {
            zside = (k > 0) ? 1 : -1;
            phi = (k > 0) ? k : -k;
    ...
    edm::LogVerbatim("HcalGeom") << "Reverse Map for mapping old to new IDs with " << detIdSpR_.size() << " entries";
    l = 0;
