...
void HcalGeomParameters::loadGeometry(const DDFilteredView& _fv, HcalParameters& php) {
  ...
  while (dodet) {
    ...
    for (unsigned int k = 0; k < copy.size(); ++k)
      st1 << " " << copy[k];
    edm::LogVerbatim("HCalGeom") << "Name " << fv.logicalPart().solid().name() << " Copy " << copy.size()  << ":" << st1.str();
#endif
...
#ifdef EDM_ML_DEBUG
	edm::LogVerbatim("HCalGeom") << "Sets for Detector " << idet << " for " << sol.name() << " Flag " << hf << " DZ " << dzVcal_;
#endif
      }
#ifdef EDM_ML_DEBUG
    } else {
...
#ifdef EDM_ML_DEBUG
	edm::LogVerbatim("HCalGeom") << "Sets for Detector " << idet << " for " << fv.name() << " Flag " << hf << " DZ " << dzVcal_;
#endif
      }
#ifdef EDM_ML_DEBUG
    } else {
