...
void HcalRecNumberingTester::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (int type = 0; type < 2; ++type) {
    ...
    edm::LogVerbatim("HcalGeom") << "Detector type: " << type << " with eta ranges " << etar.first << ":" << etar.second;
    for (int eta = etar.first; eta <= etar.second; ++eta) {
      std::vector<std::pair<int, double>> phis = hdc.getPhis(type + 1, eta);
      for (auto& phi : phis) {
	edm::LogVerbatim("HcalGeom") << "Type:Eta:phi " << type << ":" << eta << ":" << phi.first << " Depth range (+z) " << hdc.getMinDepth(type, eta, phi.first, 1) << ":" << hdc.getMaxDepth(type, eta, phi.first, 1) << " (-z) " << hdc.getMinDepth(type, eta, phi.first, -1) << ":" << hdc.getMaxDepth(type, eta, phi.first, -1);
      }
    }
  }
  ...
  edm::LogVerbatim("HcalGeom") << "Topology Mode " << hdc.getTopoMode() << " HB with " << hbar.size() << " eta sectors and HE with " << hcap.size() << " eta sectors";
  ...
  for (const auto& cell : hbcell) {
    edm::LogVerbatim("HcalGeom") << "HB[" << i1 << "] det " << cell.detType() << " zside " << cell.zside() << ":" << cell.halfSize() << " RO " << cell.actualReadoutDirection() << " eta " << cell.etaBin() << ":" << cell.etaMin() << ":" << cell.etaMax() << " phi " << cell.nPhiBins() << ":" << cell.nPhiModule() << ":" << cell.phiOffset() << ":" << cell.phiBinWidth() << ":" << cell.unitPhi() << " depth " << cell.depthSegment() << ":" << cell.depth() << ":" << cell.depthMin() << ":" << cell.depthMax() << ":" << cell.depthType();
    ++i1;
  ...
  for (const auto& cell : hecell) {
    edm::LogVerbatim("HcalGeom") << "HE[" << i2 << "] det " << cell.detType() << " zside " << cell.zside() << ":" << cell.halfSize() << " RO " << cell.actualReadoutDirection() << " eta " << cell.etaBin() << ":" << cell.etaMin() << ":" << cell.etaMax() << " phi " << cell.nPhiBins() << ":" << cell.nPhiModule() << ":" << cell.phiOffset() << ":" << cell.phiBinWidth() << ":" << cell.unitPhi() << " depth " << cell.depthSegment() << ":" << cell.depth() << ":" << cell.depthMin() << ":" << cell.depthMax() << ":" << cell.depthType();
    ++i2;
    std::vector<std::pair<int, double>> phis = cell.phis();
    edm::LogVerbatim("HcalGeom") << "Phis (" << phis.size() << ") :";
    for (const auto& phi : phis)
  ...
  for (const auto& cell : hfcell) {
    edm::LogVerbatim("HcalGeom") << "HF[" << i3 << "] det " << cell.detType() << " zside " << cell.zside() << ":" << cell.halfSize() << " RO " << cell.actualReadoutDirection() << " eta " << cell.etaBin() << ":" << cell.etaMin() << ":" << cell.etaMax() << " phi " << cell.nPhiBins() << ":" << cell.nPhiModule() << ":" << cell.phiOffset() << ":" << cell.phiBinWidth() << ":" << cell.unitPhi() << " depth " << cell.depthSegment() << ":" << cell.depth() << ":" << cell.depthMin() << ":" << cell.depthMax() << ":" << cell.depthType();
    ++i3;
  }
  ...
  for (const auto& cell : hocell) {
    edm::LogVerbatim("HcalGeom") << "HO[" << i4 << "] det " << cell.detType() << " zside " << cell.zside() << ":" << cell.halfSize() << " RO " << cell.actualReadoutDirection() << " eta " << cell.etaBin() << ":" << cell.etaMin() << ":" << cell.etaMax() << " phi " << cell.nPhiBins() << ":" << cell.nPhiModule() << ":" << cell.phiOffset() << ":" << cell.phiBinWidth() << ":" << cell.unitPhi() << " depth " << cell.depthSegment() << ":" << cell.depth() << ":" << cell.depthMin() << ":" << cell.depthMax() << ":" << cell.depthType();
    ++i4;
  }
  for (int type = 0; type <= 1; ++type) {
    ...
    for (const auto& active : act) {
      edm::LogVerbatim("HcalGeom") << "zside " << active.zside << " ieta " << active.ieta << " depth " << active.depth << " type " << active.stype << " eta " << active.eta << " active thickness " << active.thick;
    }
  }
  ...
  if (subdet == HcalBarrel || subdet == HcalEndcap) {
	...
	int iphi = (k > 0) ? k : -k;
#ifdef EDM_ML_DEBUG
	edm::LogVerbatim("HcalGeom") << "Look for Subdet " << subdet << " Zside " << zside << " Eta " << eta << " Phi " << iphi << " depths " << hdc.getMinDepth(type, eta, iphi, zside) << ":" << hdc.getMaxDepth(type, eta, iphi, zside);
#endif
	std::vector<HcalDetId> ids;
	for (int depth = hdc.getMinDepth(type, eta, iphi, zside); depth <= hdc.getMaxDepth(type, eta, iphi, zside); ++depth) {
	  HcalDetId id(subdet, zside * eta, iphi, depth);
	  HcalDetId hid = hdc.mergedDepthDetId(id);
	  hdc.unmergeDepthDetId(hid, ids);
	  edm::LogVerbatim("HcalGeom") << "Input ID " << id << " Merged ID " << hid << " containing " << ids.size() << " IDS:";
	  for (auto id : ids)
  ...
  for (const auto& cell : hbcell) {
    ...
    edm::LogVerbatim("HcalGeom") << "HB (eta=" << ieta << ", phi=" << cell.phis()[0].first << ", depth=" << cell.depthSegment() << ") r/z = " << rz;
  }
  for (const auto& cell : hecell) {
    int ieta = cell.etaBin() * cell.zside();
    double rz = hdc.getRZ(HcalEndcap, ieta, cell.phis()[0].first, cell.depthSegment());
    edm::LogVerbatim("HcalGeom") << "HE (eta=" << ieta << ", phi=" << cell.phis()[0].first << ", depth=" << cell.depthSegment() << ") r/z = " << rz;
  }
}
