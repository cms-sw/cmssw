...
void HcalScintillatorTester::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  if (auto pHSNDC = iSetup.getHandle(token_)) {
	...
	for (int depth = hdc.getMinDepth(1, eta, 1, zside, false);
	     depth <= hdc.getMaxDepth(1, eta, 1, zside, false); ++depth) {
	  ...
	  for (int lay = (layL - 1); lay < layH; ++lay) {
	    ...
	    if (kk > 1) {
	      edm::LogVerbatim("HCalGeom") << "Layer " << (lay + 1) 
					   << " Length " << length[0] << ":" 
					   << length[1] << " width " << width[0]
					   << ":" << width[1] << " Area " 
					   << area[0] << ":" << area[1];
	    } else if (kk > 0) {
	      edm::LogVerbatim("HCalGeom") << "Layer " << (lay + 1) 
	...
	for (int depth = hdc.getMinDepth(2, eta, 1, zside, false);
	     depth <= hdc.getMaxDepth(2, eta, 1, zside, false); ++depth) {
	  ...
	  for (int lay = (layL - 1); lay < layH; ++lay) {
		...
		if ((lay != 0 || eta == 18) &&
		    (lay != 1 || (eta == 18 && 
				  (hdc.parameter()->rhoxHE[k] - 
				   hdc.parameter()->dyHE[k] > 1000)) ||
		     (eta != 18 && (hdc.parameter()->rhoxHE[k] - 
				    hdc.parameter()->dyHE[k] < 1000))) &&
		    ((rmin + 30) < (hdc.parameter()->rhoxHE[k] + 
				    hdc.parameter()->dyHE[k])) && 
		    (rmax > (hdc.parameter()->rhoxHE[k] - 
			     hdc.parameter()->dyHE[k]))) {
		  rmin = std::max(rmin, (hdc.parameter()->rhoxHE[k] - 
					 hdc.parameter()->dyHE[k]));
		  rmax = std::min(rmax, (hdc.parameter()->rhoxHE[k] + 
					 hdc.parameter()->dyHE[k]));
		  double dr = rmax - rmin;
		  if (dr > 0) {
		    ...
		    if (nphi == 1) {
		      ...
		      dely[kk] = dy1;
		      area[kk] = dr * dy1;
		      double dy2 = 0.5 * ((rmax + rmin) * tan(10._deg) - 
					  4 * hdc.parameter()->dx1HE[k]) - dy1;
		      delr[kk+2] = dr;
		      dely[kk+2] = dy2;
		      area[kk+2] = dr * dy2;
		      ++kk;
