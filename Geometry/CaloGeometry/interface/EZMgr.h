#ifndef GEOMETRY_CALOGEOMETRY_EZMGR_H
#define GEOMETRY_CALOGEOMETRY_EZMGR_H 1
...
class EZMgr
{
   public:
      ...
      EZMgr< T >( size_type vecSize ) : m_vecSize ( vecSize ) ,
					m_counter ( 0 )
      {
	 m_vec.resize(0); 
	 assert( m_vec.capacity() == 0 ) ;
      ...
      virtual ~EZMgr< T >() { assert( 0 == m_counter ) ; } 
   ...
   private:
      ...
      EZMgr() ; //stop
      EZMgr( const EZMgr& ) ; //stop
      EZMgr& operator=( const EZMgr& ) ; //stop
