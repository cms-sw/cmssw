...
CaloSubdetectorGeometry::getClosestCell( const GlobalPoint& r ) {
  ...
  for( uint32_t i ( 0 ); i != m_validIds.size() ; ++i ) {
    const std::shared_ptr<CaloCellGeometry> cell ( getGeometry( m_validIds[ i ] ) ) ;
    if( nullptr != cell ) {
...
CaloSubdetectorGeometry::getSummary( CaloSubdetectorGeometry::TrVec&  tVec ,
  ...
  for( uint32_t i ( 0 ) ; i != m_validIds.size() ; ++i ) {
    Tr3D tr ;
    const std::shared_ptr<CaloCellGeometry> ptr ( cellGeomPtr( i ) ) ;
    assert( nullptr != ptr ) ;
...
CCGFloat CaloSubdetectorGeometry::deltaPhi( const DetId& detId ) {
  ...
  if(!m_deltaPhi.load(std::memory_order_acquire))  {
    ...
    for( uint32_t i ( 0 ) ; i != kSize ; ++i ) {
      const std::shared_ptr<CaloCellGeometry> cellPtr ( cellGeomPtr( i ) ) ;
      if( nullptr != cellPtr ) {
	CCGFloat dPhi1 (fabs(
