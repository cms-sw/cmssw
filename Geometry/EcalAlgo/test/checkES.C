...
struct recordLess {
  bool operator()(const record &a, const record &b) {
    return ((a.z_ < b.z_) || ((a.z_ == b.z_) && (a.y_ < b.y_)) ||
            ((a.z_ == b.z_) && (a.y_ == b.y_) && (a.x_ < b.x_)));
  }
};
...
void checkDuplicate(std::string fname, std::string outFile) {
  ...
  if (!infile.is_open()) {
	...
	if ((fabs(records[k].x_ - records[k1].x_) < 0.0001) &&
	    (fabs(records[k].y_ - records[k1].y_) < 0.0001) &&
      ...
      fileout << "[" << records[k].serial_ << "] ID " << std::hex << records[k].id_ << std::dec << std::setprecision(6) << " (" << records[k].x_ << ", " << records[k].y_ << ", " << records[k].z_ << ") with " << dup << " Duplicates:";
      if (dup > 0) {
...
void plotHist(const char* fname, bool save=false) {
  ...
  std::string titl[4] = {"z = -1, layer = 1", "z = -1, layer = 2",
			 "z = 1, layer = 1", "z = 1, layer = 2"};
