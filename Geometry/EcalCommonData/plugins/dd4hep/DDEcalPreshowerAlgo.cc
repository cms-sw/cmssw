...
static long algorithm(dd4hep::Detector& /* description */,
  ...
  for (size_t i = 0; i < es.thickLayers_.size(); ++i) {
	...
	absz = int(es.abs2ndx.size());
      ...
      Solid Out_Altmp3 = SubtractionSolid(
          dd_Alname_j, Out_Al, OutAltmp, Position(outalbx / 2, outalby / 2 + shiftR, 0));
      ...
      Solid Out_Altmp4 = SubtractionSolid(
          dd_Alname_k, Out_Altmp3, OutAltmp2, Position(outalbx2 / 2 + shiftR2, outalby2 / 2, 0));
      Solid Out_Altmp5 = UnionSolid(
          dd_Alname_l, Out_Altmp4, Out_Altmp4, ns.rotation("esalgo:RABS90"));
      Solid Out_Altmp6 = UnionSolid(
          dd_Alname_m, Out_Altmp5, Out_Altmp4, ns.rotation("esalgo:RABS180B"));
      /*Solid Out_Al2 = */
  ...
  for (int M = 0; M < int(es.types_l5_.size() + es.types_l4_.size()); M++) {
    ...
    Solid solid_lfhtrunc = Trap("esalgo:LDRFHTR",
    ...
    if (M < int(es.types_l5_.size())) {
	  ...
	  Solid solid_5b = UnionSolid(dd_tmp_name_5b,
				      solid_5a,
				      solid_lfhalf,
				      Transform3D(ns.rotation("esalgo:RM1299"),
	  ...
	  Solid solid_5c = UnionSolid(dd_tmp_name_5c,
				      solid_5b,
				      solid_lfhtrunc,
				      Transform3D(ns.rotation("esalgo:RM1299"),
	...
	Solid solid_5pb = UnionSolid(
				     dd_tmp_name_5pb, solid_5pa, solid_lfront,
				     Transform3D(ns.rotation("esalgo:RM1299"),
	...
	Solid solid = UnionSolid(ddname, solid_5pb, solid_lbck,
				 Transform3D(ns.rotation("esalgo:RM1299"),
					     Position(sdx, sdy, sdz)));
	/* Volume ladder = */
	ns.addVolumeNS(Volume(ddname, solid, ns.material(es.LaddMaterial_)));
    ...
    if (M >= int(es.types_l5_.size())) {
	  ...
	  Solid solid_a = Box(dd_tmp_name_a, boxax / 2, boxay / 2, boxaz / 2.);
          ...
          sdye[enb] = -boxay / 2 - es.LdrFrnt_Length / 2;
          sdze[enb] = -es.ladder_thick / 2. + es.LdrFrnt_Offset;
          Solid solid_b = UnionSolid(dd_tmp_name_b,
				     solid_a,
				     solid_lfront,
				     Transform3D(ns.rotation("esalgo:RM1299"),
						 Position(sdxe[enb], sdye[enb], sdze[enb])));
	  ...
	  ns.addVolumeNS(Volume(ddname2, solid, ns.material(es.LaddMaterial_)));
	    ...
	    Solid solid_b = UnionSolid(dd_tmp_name_b,
            ...
            Solid solid_c = UnionSolid(dd_tmp_name_c,
				       solid_b,
				       solid_lfhtrunc,
				       Transform3D(ns.rotation("esalgo:RM1299"),
						   Position(sdxe2[enb], sdye2[enb], sdze2[enb])));
	...
	Solid solid_pb = UnionSolid(
				    dd_tmp_name_pb, solid_pa, solid_lfront,
				    Transform3D(ns.rotation("esalgo:RM1299"),
						Position(sdx, sdy, sdz)));
        ...
        sdy = boxay / 2 + es.LdrBck_Length / 2;
        sdz = -es.ladder_thick / 2. + es.LdrBck_Offset;
        Solid solid = UnionSolid(ddname, solid_pb, solid_lbck,
				 Transform3D(ns.rotation("esalgo:RM1299"),
					     Position(sdx, sdy, sdz)));
        /* Volume ladder = */
	ns.addVolumeNS(Volume(ddname, solid, ns.material(es.LaddMaterial_)));
    ...
    if (M < int(es.types_l5_.size())) {
      ...
      for (int i = 0; i <= 1; i++) {
        for (int j = 0; j <= 4; j++) {
          ...
          if (es.ladd_l5_map_[(i + j * 2 + M * 10)] == 1) {
            scopy++;
	    ns.volume(ddname).placeVolume(swedLog, scopy + 1000 * swed_scopy_glob, Transform3D(ns.rotation("esalgo:RM1299"),
										    Position(xpos, ypos, zpos)));	    
	    ns.volume(ddname2).placeVolume(swedLog, scopy + 1000 * swed_scopy_glob + 100, Transform3D(ns.rotation("esalgo:RM1299"),
											   Position(xpos, ypos, zpos)));

            ypos = ypos + es.ywedge_ceramic_diff;
            zpos = -es.ladder_thick / 2. + 0.005_mm + es.zwedge_ceramic_diff;
	    ns.volume(ddname).placeVolume(sfbxLog, scopy + 1000 * swed_scopy_glob,
			       Transform3D(ns.rotation("esalgo:RM1298"),
					   Position(xpos, ypos, zpos)));

	    ns.volume(ddname2).placeVolume(sfbyLog, scopy + 1000 * swed_scopy_glob,
				Transform3D(ns.rotation("esalgo:RM1300A"),
					    Position(xpos, ypos, zpos)));
	  }
	}
      }
      ...
      for (int i = 0; i <= 1; i++) {
        for (int j = 0; j <= 3; j++) {
          ...
          if (es.ladd_l4_map_[(i + j * 2 + (M - es.types_l5_.size()) * 8)] == 1) {
            scopy++;
	    ns.volume(ddname).placeVolume(swedLog, scopy + 1000 * swed_scopy_glob,
			       Transform3D(ns.rotation("esalgo:RM1299"),
					   Position(xpos, ypos, zpos)));
	    ns.volume(ddname2).placeVolume(swedLog, scopy + 1000 * swed_scopy_glob + 100,
				Transform3D(ns.rotation("esalgo:RM1299"),
					    Position(xpos, ypos, zpos)));
  ...
  for (int I = -9; I <= 9; ++I) {
    ...
    for (int K = 0; K < es.noLaddInCol_[J]; K++) {
      ...
      xpos = I * (2 * es.waf_intra_col_sep + es.waf_inter_col_sep);
      sfLog.placeVolume(ns.volume(ddname2), icopy[j], Transform3D(ns.rotation("esalgo:R270"),
								  Position(ypos, -xpos, zpos - es.zlead1_ + es.zlead2_)));
      ...
      sfLog.placeVolume(ns.volume(ddname3), icopy[j], Transform3D(ns.rotation("esalgo:R180"),
								  Position(xpos, -ypos, zpos)));
      ...
      sfLog.placeVolume(ns.volume(ddname3), icopy[j], Transform3D(ns.rotation("esalgo:R090"),
								  Position(-ypos, -xpos, zpos - es.zlead1_ + es.zlead2_)));
    }
  }
  }
