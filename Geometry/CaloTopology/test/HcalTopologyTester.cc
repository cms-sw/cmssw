...
void HcalTopologyTester::doTest(const HcalTopology& topology) {
  ...
  edm::LogVerbatim("HCalGeom") << "Total number of cells in HB:" << topology.getHBSize() << " HE: " << topology.getHESize() << " HF: " << topology.getHFSize() << " HO: " << topology.getHOSize() << " HT: " << topology.getHTSize() << " Calib: " << topology.getCALIBSize() << " Overall: " << topology.ncells();
