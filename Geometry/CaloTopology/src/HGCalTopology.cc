...
DetId HGCalTopology::denseId2detId(uint32_t hi) const {
  ...
  if (validHashIndex(hi)) {
    HGCalTopology::idCont id_;
    id_.zside  = ((int)(hi)<kEKhalf_ ? -1 : 1);
...
DetId HGCalTopology::switchZSide(const DetId startId) const {
  ...
  if (startId.det() == DetId::Forward && startId.subdetId() == (int)(subdet_)){
    HGCalTopology::idCont id_ = decode(startId);
    id_.zside  =-id_.zside;
    DetId id   = encode(id_);
    if (valid(id)) return id.rawId();
  }
...
HGCalTopology::idCont HGCalTopology::decode(const DetId& startId) const {
...
DetId HGCalTopology::encode(const HGCalTopology::idCont& id_) const {
