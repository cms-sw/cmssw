...
// $Id: RPCGEO_POSTLS1.cc,v 1.1 2013/03/25 17:13:45 carrillo Exp $
//
//
...
class RPCGEO_UPSCOPE : public edm::EDAnalyzer {
   public:
      explicit RPCGEO_UPSCOPE(const edm::ParameterSet&);
      ~RPCGEO_UPSCOPE();
...
//
RPCGEO_UPSCOPE::RPCGEO_UPSCOPE(const edm::ParameterSet& /*iConfig*/){
   //now do what ever initialization is needed
}


RPCGEO_UPSCOPE::~RPCGEO_UPSCOPE()
{
...
void
RPCGEO_UPSCOPE::analyze(const edm::Event& /*iEvent*/, const edm::EventSetup& iSetup)
{
   using namespace edm;
   ...
   for (TrackingGeometry::DetContainer::const_iterator it=rpcGeo->dets().begin();it<rpcGeo->dets().end();it++){
	 ...
	 if(rpcId.region()!=0 && (rpcId.station()==4 || rpcId.ring()==1)){
...
void 
RPCGEO_UPSCOPE::beginJob()
{
}

// ------------ method called once each job just after ending the event loop  ------------
void 
RPCGEO_UPSCOPE::endJob() {
}

//define this as a plug-in
DEFINE_FWK_MODULE(RPCGEO_UPSCOPE);
