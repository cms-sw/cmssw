...
DDBHMAngular::DDBHMAngular() {
  LogDebug("BHMGeom") << "DDBHMAngular test: Creating an instance";
}
...
void DDBHMAngular::initialize(const DDNumericArguments & nArgs,
  ...
  rr     = nArgs["radius"];
  dphi   = nArgs["deltaPhi"];
  LogDebug("BHMGeom") << "DDBHMAngular debug: Parameters for positioning-- "
		       << units << " copies at radius " << rr/CLHEP::cm 
		       << " cm with delta(phi) " << dphi/CLHEP::deg;

  rotMat      = sArgs["Rotation"];
  childName   = sArgs["ChildName"]; 
  LogDebug("BHMGeom") << "DDBHMAngular debug: Parent " << parent().name()
...
void DDBHMAngular::execute(DDCompactView& cpv) {
  ...
  for (int jj=0; jj<units; jj++) {
    ...
    LogDebug("BHMGeom") << "DDBHMAngular test: " << child << " number " <<jj+1
