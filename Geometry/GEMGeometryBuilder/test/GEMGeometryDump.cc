...
void GEMGeometryDump::analyze(const edm::Event& event, const edm::EventSetup& eventSetup) {
  ...
  if (!gemGeometry_.isValid()) {
    ...
    for (unsigned int k1 = 0; k1 < regions.size(); ++k1) {
      edm::LogVerbatim("GEMGeometry") << "\nRegion " << k1 << ":" << regions[k1]->region() << " with " << regions[k1]->nStations() << " staions";
      auto const& stations = regions[k1]->stations();

      for (unsigned int k2 = 0; k2 < stations.size(); ++k2) {
	edm::LogVerbatim("GEMGeometry") << "\nStation " << k2 << ":" << stations[k2]->station() << ":" << stations[k2]->getName() << " with " << stations[k2]->nRings() << " rings";
	auto const& rings = stations[k2]->rings();

	for (unsigned int k3 = 0; k3 < rings.size(); ++k3) {
	  edm::LogVerbatim("GEMGeometry") << "\nRing " << k3 << ":" << rings[k3]->ring() << " with " << rings[k3]->nSuperChambers() << " superChambers";
	  auto const& superChambers = rings[k3]->superChambers();

	  for (unsigned int k4 = 0; k4 < superChambers.size(); ++k4) {
	    edm::LogVerbatim("GEMGeometry") << "\nSuperChamber " << k4 << ":" << superChambers[k4]->id() << " with " << superChambers[k4]->nChambers() << " chambers";
	    auto const& chambers = superChambers[k4]->chambers();

	    for (unsigned int k5 = 0; k5 < chambers.size(); ++k5) {
	      edm::LogVerbatim("GEMGeometry") << "\nChamber " << k5 << ":" << chambers[k5]->id() << " with " << chambers[k5]->nEtaPartitions() << " etaPartitions";
	      auto const& etaPartitions = chambers[k5]->etaPartitions();
	      
	      for (unsigned int k6 = 0; k6 < etaPartitions.size(); ++k6) {
		edm::LogVerbatim("GEMGeometry") << "\nEtaPartition " << k6 << ":" << etaPartitions[k6]->id() << etaPartitions[k6]->type().name() << " with " << etaPartitions[k6]->nstrips() << " strips of pitch " << etaPartitions[k6]->pitch() << " and " << etaPartitions[k6]->npads() << " pads of pitch " << etaPartitions[k6]->padPitch();
		if (verbose_) {
