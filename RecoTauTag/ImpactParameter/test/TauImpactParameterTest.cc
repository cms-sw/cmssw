...
void TauImpactParameterTest::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (iJet = tauIpInfo.begin(); iJet != tauIpInfo.end(); iJet++) {
    try {
      double Rmatch = 0.1, Rsignal = 0.07, Riso = 0.4, pT_LT = 6, pT_min = 1;
      ...
      if (discriminator == 0)
        continue;
      if (theJet->et() < 0 || theJet->et() > 150)
        continue;
      if (fabs(theJet->eta()) > 2.2)
        continue;
      ...
      if ((leadingTrack)->numberOfValidHits() < 8 || leadingTrack->normalizedChi2() > 10)
        continue;
      ...
      for (iTrack = tracks.begin(); iTrack != tracks.end(); iTrack++) {
        LogInfo("TauImpactParameterTest")
            << "    track pt, eta " << (*iTrack)->pt() << " " << (*iTrack)->eta() << " " << (*iTrack)->d0() << " "
            << (*iTrack)->d0Error() << " " << (*iTrack)->dz() << " " << (*iTrack)->dzError() << " "
            << (*iTrack)->recHitsSize() << " " << (*iTrack)->normalizedChi2();
      ...
      if (tauTracks.size() == 1 || tauTracks.size() == 3) {
        ...
        if (trackData != nullptr) {
          ...
          LogInfo("TauImpactParameterTest")
              << "          ip,sip,err " << tip.value() << " " << tip.significance() << " " << tip.error();
          LogInfo("TauImpactParameterTest")
              << "       3D ip,sip,err " << tzip.value() << " " << tzip.significance() << " " << tzip.error();
          ...
          if (tauTracks.size() == 3) {
            ...
            for (std::vector<TrackRef>::const_iterator iTr = tauTracks.begin(); iTr != tauTracks.end(); iTr++) {
