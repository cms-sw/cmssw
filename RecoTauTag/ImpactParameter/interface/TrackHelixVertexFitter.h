...
namespace tauImpactParameter {
  ...
  public:
    ...
    static void computedxydz(const TVectorT<double>& inpar,
                             int particle,
                             double& kappa,
                             double& lam,
                             double& phi,
  ...
  private:
    ...
    static int measuredValueIndex(int TrackPar, int Particle) { return TrackPar + Particle * TrackParticle::NHelixPar; }
    static int freeParIndex(int Par, int Particle) {
      if (Par == x0 || Par == y0 || Par == z0)
        return Par;
