...
namespace tauImpactParameter {
  ...
  public:
    ...
    LorentzVectorParticle(
        const TVectorT<double>& par, const TMatrixTSym<double>& cov, int pdgid, double charge, double b);
    ...
    double parameter(int i) const override {
      ...
      if (i == E)
        return sqrt(particle_px * particle_px + particle_py * particle_py + particle_pz * particle_pz +
                    particle_m * particle_m);
      if (i == p)
        return sqrt(particle_px * particle_px + particle_py * particle_py + particle_pz * particle_pz);
      if (i == pt)
        return sqrt(particle_px * particle_px + particle_py * particle_py);
