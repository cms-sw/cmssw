...
float
GeneratorTau::getVisNuAngle() const
{
...
GeneratorTau::getLeadTrack() const
{
...
float
GeneratorTau::getChargedOpeningAngle() const
{
   return getOpeningAngle(genChargedPions_);
}

float
GeneratorTau::getGammaOpeningAngle() const
{
   return getOpeningAngle(genGammas_);
}

GeneratorTau::tauDecayModeEnum
GeneratorTau::computeDecayMode(const reco::GenParticle* theTau)
{
   ...
   //edm::LogInfo("GeneratorTau") << "Compute decay mode: Followed decay chain to pions/leptons - found " << pdgDecayProductTypes.size() << " daughters..";
