...
/** \class PFRecoTauDiscriminationAgainstMuon3
 *
 ...
 *
 * $Id: PFRecoTauDiscriminationAgainstMuon3.cc,v 1.3 2012/12/20 10:50:00 veelken Exp $
 *
 */
...
class PFRecoTauDiscriminationAgainstMuon3 : public PFTauDiscriminationProducerBase 
{
  enum { kLoose, kMedium, kTight };
 public:
  explicit PFRecoTauDiscriminationAgainstMuon3(const edm::ParameterSet& cfg)
  ...
  ~PFRecoTauDiscriminationAgainstMuon3() {} 
...
double PFRecoTauDiscriminationAgainstMuon3::discriminate(const reco::PFTauRef& pfTau)
{
  int numMatches = 0;
  ...
  if      ( discriminatorOption_ == kLoose  && numMatches <= 1                                                        ) discriminatorValue = 1.;
  else if ( discriminatorOption_ == kMedium && numMatches <= 1 && numLast2StationsWithHits == 0                       ) discriminatorValue = 1.;
  else if ( discriminatorOption_ == kTight  && numMatches <= 1 && numLast2StationsWithHits == 0 && passesCaloMuonVeto ) discriminatorValue = 1.;

  return discriminatorValue;
} 

DEFINE_FWK_MODULE(PFRecoTauDiscriminationAgainstMuon3);
