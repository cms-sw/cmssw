...
namespace {
  ...
  {
    public:
      bool operator()(const reco::RecoTauPiZero& a, const reco::RecoTauPiZero& b)
      {
...
class RecoTauPiZeroProducer : public edm::EDProducer 
{
  ...
  private:
    typedef boost::ptr_vector<reco::tau::RecoTauPiZeroBuilderPlugin> builderList;
    typedef boost::ptr_vector<reco::tau::RecoTauPiZeroQualityPlugin> rankerList;
    typedef reco::tau::RecoTauLexicographicalRanking<rankerList, RecoTauPiZero> PiZeroPredicate;
...
RecoTauPiZeroProducer::RecoTauPiZeroProducer(const edm::ParameterSet& pset)
{
  ...
  for(VPSet::const_iterator builderPSet = builders.begin(); builderPSet != builders.end(); ++builderPSet)
  {
    ...
    // Build the plugin
    builders_.push_back(RecoTauPiZeroBuilderPluginFactory::get()->create(pluginType, *builderPSet));
  }
...
void RecoTauPiZeroProducer::produce(edm::Event& evt, const edm::EventSetup& es)
{
  ...
  std::auto_ptr<JetPiZeroAssociation> association(new JetPiZeroAssociation(PFJetRefProd(pfJets)));

  size_t iJet = 0;
  // Loop over our jets
  for(PFJetCollection::const_iterator jet = pfJets->begin(); jet != pfJets->end(); ++jet, ++iJet)
  {
    ...
    for(builderList::const_iterator builder = builders_.begin(); builder != builders_.end(); ++builder)
    {
    ...
    PiZeroVector cleanPiZeros = 
      reco::tau::cleanOverlaps<PiZeroVector, PiZeroOverlapChecker>(dirtyPiZeros);
...
void RecoTauPiZeroProducer::print(const std::vector<reco::RecoTauPiZero>& piZeros, std::ostream& out)
{
  ...
  for(std::vector<reco::RecoTauPiZero>::const_iterator piZero = piZeros.begin(); piZero != piZeros.end(); ++piZero)
  {
    out << *piZero;
    ...
    for(rankerList::const_iterator ranker = rankers_.begin(); ranker != rankers_.end(); ++ranker)
    {
      ...
      out << " " << std::resetiosflags(std::ios::left) << std::setprecision(3) << (*ranker)(*piZero);
      out << std::endl;
    }
  }
}
