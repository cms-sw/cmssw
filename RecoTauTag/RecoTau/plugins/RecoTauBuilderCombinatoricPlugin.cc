  ...
  namespace tau {
    class RecoTauBuilderCombinatoricPlugin : public RecoTauBuilderPlugin
    {
      public:
        ...
        virtual std::vector<reco::PFTau> operator()(const reco::PFJetRef& jet, const std::vector<RecoTauPiZero>& piZeros) const;
      ...
      RecoTauBuilderPlugin(pset)
    {
      ...
      for(VPSet::const_iterator dm = decayModes.begin(); dm != decayModes.end(); ++dm)
      {
        decayModeInfo info;
        ...
        const reco::PFJetRef& jet, const std::vector<RecoTauPiZero>& piZeros) const
    {
          ...
          decayMode != decayModesToBuild_.end(); ++decayMode)
      {
        ...
        for(PFCombo::iterator trackCombo = trackCombos.begin(); trackCombo != trackCombos.end(); ++trackCombo)
        {
          // FIX ME HERE
          // Loop over the different combinations of PiZeros
          for(PiZeroCombo::iterator piZeroCombo = piZeroCombos.begin(); piZeroCombo != piZeroCombos.end(); ++piZeroCombo)
          {
            // Output tau
            ...
            // Set signal and isolation components for charged hadrons, after converting
            // them to a PFCandidateRefVector
            ...
            // Add all the candidates that weren't included in the combinatoric generation
            tau.addPFCands(RecoTauConstructor::kIsolation, 
                RecoTauConstructor::kChargedHadron, pfch_end, pfchs.end());
...
DEFINE_EDM_PLUGIN(RecoTauBuilderPluginFactory, reco::tau::RecoTauBuilderCombinatoricPlugin, "RecoTauBuilderCombinatoricPlugin");
