...
class RecoTauBuilderConePlugin : public RecoTauBuilderPlugin {
  ...
  template<class CandType> class DeltaRFilter {
    public:
  ...
  template<class CandType> class DeltaRPtrFitler {
    public:
  ...
  public:
    ...
    std::vector<reco::PFTau> operator()(const reco::PFJetRef& jet, 
...
RecoTauBuilderConePlugin::RecoTauBuilderConePlugin(const edm::ParameterSet& pset):
  ...
  leadObjecPtThreshold_(
      pset.getParameter<double>("leadObjectPt")),
  matchingCone_(
      pset.getParameter<std::string>("matchingCone")),
  signalConeChargedHadrons_(
      pset.getParameter<std::string>("signalConeChargedHadrons")),
    ...
    const reco::PFJetRef& jet, const std::vector<RecoTauPiZero>& piZeros) const
{
  ...
  // PFCandidates always a sorted list, so we can just take the first if it if
  // it exists.
  PFCandidatePtr leadPFCH;
  PFCandPtrs::iterator leadPFCH_iter = std::find_if(
      pfchs.begin(), pfchs.end(), matchingConeFilter);
  ...
  PFCandPtrs::iterator leadPFGamma_iter = std::find_if(pfGammas.begin(), 
  ...
  if (leadPFCH.isNull() && leadPFGamma.isNull()) { return output; }
  ...
  PFCandPtrDRFilter signalConePFCHFilter(coneAxis, 0, 
      signalConeChargedHadrons_(*jet));
  PFCandPtrDRFilter signalConePFNHFilter(coneAxis, 0, 
      signalConeNeutralHadrons_(*jet));
  PiZeroDRFilter signalConePiZeroFilter(coneAxis, 0, 
      signalConePiZeros_(*jet));

  PFCandPtrDRFilter isoConePFCHFilter(coneAxis, 
      signalConeChargedHadrons_(*jet), isoConeChargedHadrons_(*jet));
  PFCandPtrDRFilter isoConePFNHFilter(coneAxis, 
      signalConeNeutralHadrons_(*jet), isoConeNeutralHadrons_(*jet));
  PiZeroDRFilter isoConePiZeroFilter(coneAxis, 
      signalConePiZeros_(*jet), isoConePiZeros_(*jet));
