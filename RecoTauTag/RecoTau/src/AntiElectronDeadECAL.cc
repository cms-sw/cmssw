...
bool AntiElectronDeadECAL::operator()(const reco::Candidate* tau) const
{
  ...
  if (extrapolateToECalEntrance_) {
    ...
    if (pfTau != nullptr) {
      leadChargedHadron = pfTau->leadChargedHadrCand().isNonnull() ?
	pfTau->leadChargedHadrCand().get() : nullptr;
    }
    else {
      const pat::Tau *patTau = dynamic_cast<const pat::Tau*>(tau);
      if (patTau != nullptr) {
	leadChargedHadron = patTau->leadChargedHadrCand().isNonnull() ?
	  patTau->leadChargedHadrCand().get() : nullptr;
      }
    }
  }
  ...
  if ( verbosity_ ) {
	...
	<< ", phi at ECAL (vtx) = " << tau_phi << " (" << leadChargedHadron->phi() << ")";
      ...
      edm::LogPrint("TauAgainstEleDeadECAL") << " tau: Pt = " << tau->pt() << ", eta at vtx = " << tau_eta << ", phi at vtx = " << tau_phi;
    }
  }
  for ( std::vector<towerInfo>::const_iterator badTower = badTowers_.begin();
    ...
    if ( deltaR(badTower->eta_, badTower->phi_, tau_eta, tau_phi) < dR_ ) {
      if ( verbosity_ ) {
	edm::LogPrint("TauAgainstEleDeadECAL") << " matches badTower: eta = " << badTower->eta_ << ", phi = " << badTower->phi_ ;
      }
      isNearBadTower = true;
      break;
    }
