...
PFRecoTauDiscriminationAgainstMuon2Helper::PFRecoTauDiscriminationAgainstMuon2Helper(
    ...
    const std::vector<int>& maskHitsRPC, const edm::Handle<reco::MuonCollection>& muons, const reco::PFTauRef& pfTau) : pfLeadChargedHadron_{pfTau->leadPFChargedHadrCand()}{
    if (verbosity) {
...
bool PFRecoTauDiscriminationAgainstMuon2Helper::eval(const PFRecoTauDiscriminationAgainstMuonConfigSet& config, const reco::PFTauRef& pfTau) const {
      ...
      if (config.discriminatorOption == PFRecoTauDiscriminationAgainstMuonConfigSet::kLoose && numStationsWithMatches_ <= config.maxNumberOfMatches)
        discriminatorValue = true;
      else if (config.discriminatorOption == PFRecoTauDiscriminationAgainstMuonConfigSet::kMedium && numStationsWithMatches_ <= config.maxNumberOfMatches &&
               numLast2StationsWithHits_ <= config.maxNumberOfHitsLast2Stations)
        discriminatorValue = true;
      else if (config.discriminatorOption == PFRecoTauDiscriminationAgainstMuonConfigSet::kTight && numStationsWithMatches_ <= config.maxNumberOfMatches &&
