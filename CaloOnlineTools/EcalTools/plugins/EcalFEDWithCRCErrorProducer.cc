...
//
/**\class EcalFEDWithCRCErrorProducer EcalFEDWithCRCErrorProducer.cc
filter/EcalFEDWithCRCErrorProducer/src/EcalFEDWithCRCErrorProducer.cc
...
EcalFEDWithCRCErrorProducer::EcalFEDWithCRCErrorProducer(
  ...
  fedUnpackList_ = iConfig.getUntrackedParameter<std::vector<int>>(
      "FEDs", std::vector<int>());
  writeAllEcalFEDs_ =
      iConfig.getUntrackedParameter<bool>("writeAllEcalFED", false);
  if (fedUnpackList_.empty())
    for (int i = FEDNumbering::MINECALFEDID; i <= FEDNumbering::MAXECALFEDID;
         i++)
    ...
    if (length >= 1) {
      ...
      if (writeAllEcalFEDs_ || crcError) {
        ...
        if (fedDataProd.size() != 0) {
          //                std::cout << " More than one FEDRawDataCollection
          //                with data in FED "; std::cout << j << " Skipping the
          //                2nd\n";
          continue;
        }
