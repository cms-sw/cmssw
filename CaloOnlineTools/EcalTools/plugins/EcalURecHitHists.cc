...
EcalURecHitHists::EcalURecHitHists(const edm::ParameterSet &iConfig)
    : EBUncalibratedRecHitCollection_(iConfig.getParameter<edm::InputTag>(
          "EBUncalibratedRecHitCollection")),
      EEUncalibratedRecHitCollection_(iConfig.getParameter<edm::InputTag>(
          "EEUncalibratedRecHitCollection")),
      runNum_(-1), histRangeMax_(iConfig.getUntrackedParameter<double>(
                       "histogramMaxRange", 200.0)),
      histRangeMin_(
          iConfig.getUntrackedParameter<double>("histogramMinRange", -10.0)),
      fileName_(iConfig.getUntrackedParameter<std::string>(
          "fileName", std::string("ecalURechHitHists"))) {
  ...
  allFedsHist_ = new TH1F(name1.c_str(), title1.c_str(), numBins, histRangeMin_,
                          histRangeMax_);
...
void EcalURecHitHists::analyze(edm::Event const &iEvent,
  ...
  LogDebug("EcalURecHitHists")
      << "event " << ievt << " hits collection size " << EBhits->size();

  iEvent.getByLabel(EEUncalibratedRecHitCollection_, EEhits);
  LogDebug("EcalURecHitHists")
      << "event " << ievt << " hits collection size " << EEhits->size();

  for (EcalUncalibratedRecHitCollection::const_iterator hitItr =
           EBhits->begin();
    ...
    if (result != maskedFEDs_.end()) {
      LogWarning("EcalURecHitHists") << "skipping uncalRecHit for FED " << FEDid
                                     << " ; amplitude " << ampli;
      continue;
    }
    ...
    if (result != maskedChannels_.end()) {
      LogWarning("EcalURecHitHists")
          << "skipping uncalRecHit for channel: " << ic << " with amplitude "
          << ampli;
      continue;
    }
...
void EcalURecHitHists::initHists(int FED) {
  ...
  int numBins = (int)round(histRangeMax_ - histRangeMin_) + 1;
  TH1F *hist = new TH1F(name1.c_str(), title1.c_str(), numBins, histRangeMin_,
                        histRangeMax_);
...
void EcalURecHitHists::endJob() {
  ...
  LogWarning("EcalMipGraphs")
      << "Masked channels are: " << channels << " and that is all!";
}
