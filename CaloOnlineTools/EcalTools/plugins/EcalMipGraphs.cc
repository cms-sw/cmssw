...
EcalMipGraphs::EcalMipGraphs(const edm::ParameterSet &iConfig)
    : EBRecHitCollection_(
          iConfig.getParameter<edm::InputTag>("EcalRecHitCollectionEB")),
      EERecHitCollection_(
          iConfig.getParameter<edm::InputTag>("EcalRecHitCollectionEE")),
      ...
      runNum_(-1), side_(iConfig.getUntrackedParameter<int>("side", 3)),
      threshold_(
          iConfig.getUntrackedParameter<double>("amplitudeThreshold", 12.0)),
      minTimingAmp_(iConfig.getUntrackedParameter<double>(
          "minimumTimingAmplitude", 0.100)) {
  vector<int> listDefaults;
  listDefaults.push_back(-1);

  maskedChannels_ = iConfig.getUntrackedParameter<vector<int>>("maskedChannels",
                                                               listDefaults);
  maskedFEDs_ =
      iConfig.getUntrackedParameter<vector<int>>("maskedFEDs", listDefaults);
  seedCrys_ =
      iConfig.getUntrackedParameter<vector<int>>("seedCrys", vector<int>());

  vector<string> defaultMaskedEBs;
  defaultMaskedEBs.push_back("none");
  maskedEBs_ = iConfig.getUntrackedParameter<vector<string>>("maskedEBs",
                                                             defaultMaskedEBs);
  ...
  allFedsTimingHist_ =
      fileService->make<TH1F>(name1.c_str(), title1.c_str(), 150, -7, 7);
  ...
  if (maskedFEDs_[0] == -1) {
    ...
    if (maskedEBs_[0] != "none") {
      maskedFEDs_.clear();
      for (vector<string>::const_iterator ebItr = maskedEBs_.begin();
           ebItr != maskedEBs_.end(); ++ebItr) {
...
void EcalMipGraphs::analyze(edm::Event const &iEvent,
  ...
  if (runNum_ == -1) {
    ...
    canvasNames_ =
        fileService->make<TTree>("canvasNames", "Names of written canvases");
  ...
  // debug
  // LogWarning("EcalMipGraphs") << "event " << ievt << " EBhits collection size
  // " << EBhits->size(); LogWarning("EcalMipGraphs") << "event " << ievt << "
  // EEhits collection size " << EEhits->size();
...
TGraph *EcalMipGraphs::selectDigi(DetId thisDet, int ievt) {
  ...
  // find digi we need  -- can't get find() to work; need DataFrame(DetId det)
  // to work?
  ...
  if (FEDid < 610 || FEDid > 645)
    isBarrel = false;
  int cryIndex =
      isBarrel ? ((EBDetId)thisDet).hashedIndex() : getEEIndex(elecId);
  ...
  if (isBarrel) {
    ...
    while (digiItr != EBdigisHandle->end() &&
           ((*digiItr).id() != (EBDetId)thisDet)) {
      ++digiItr;
    }
    ...
    while (digiItr != EEdigisHandle->end() &&
           ((*digiItr).id() != (EEDetId)thisDet)) {
      ++digiItr;
    }
  ...
  for (int i = 0; i < df.size(); ++i) {
    if (df.sample(i).gainId() != 0)
      ordinate[i] = (int)(pedestal + (df.sample(i).adc() - pedestal) *
                                         gainRatio[df.sample(i).gainId() - 1]);
    else
      ordinate[i] = 49152; // Saturation of gain1
  }

  TGraph *oneGraph = fileService->make<TGraph>(10, abscissa, ordinate);
  string name = "Graph_ev" + intToString(naiveEvtNum_) + "_ic" +
                intToString(ic) + "_FED" + intToString(FEDid);
  string gainString = (gainId == 1) ? "Free" : intToString(gainHuman);
  string title = "Event" + intToString(naiveEvtNum_) + "_lv1a" +
                 intToString(ievt) + "_ic" + intToString(ic) + "_" + sliceName +
                 "_gain" + gainString;
...
void EcalMipGraphs::selectHits(Handle<EcalRecHitCollection> hits, int ievt,
                               ESHandle<CaloTopology> caloTopo) {
  for (EcalRecHitCollection::const_iterator hitItr = hits->begin();
    ...
    if (FEDid < 610 || FEDid > 645)
      isBarrel = false;
    int cryIndex =
        isBarrel ? ((EBDetId)det).hashedIndex() : ((EEDetId)det).hashedIndex();
    ...
    if (result != maskedFEDs_.end()) {
      // LogWarning("EcalMipGraphs") << "skipping uncalRecHit for FED " << FEDid
      // << " ; amplitude " << ampli;
      continue;
    }
    result = find(maskedChannels_.begin(), maskedChannels_.end(), cryIndex);
    if (result != maskedChannels_.end()) {
      // LogWarning("EcalMipGraphs") << "skipping uncalRecHit for channel: " <<
      // cryIndex << " in fed: " << FEDid << " with amplitude " << ampli ;
      continue;
    }
    ...
    // Either we must have a user-requested cry (in which case there is no
    // amplitude selection) Or we pick all crys that pass the amplitude cut (in
    // which case there is no fixed crystal selection)
    if (cryIsInList || (seedCrys_.empty() && ampli > threshold_)) {
      // We have a winner!
      crysAndAmplitudesMap_[cryIndex] = ampli;
      string name = "Event" + intToString(naiveEvtNum_) + "_ic" +
                    intToString(ic) + "_FED" + intToString(FEDid);
      string title = "Digis";
      string seed = "ic" + intToString(ic) + "_FED" + intToString(FEDid);
      int freq = 1;
      pair<map<string, int>::iterator, bool> pair =
      ...
      CaloNavigator<DetId> cursor =
          CaloNavigator<DetId>(det, caloTopo->getSubdetectorTopology(det));
...
int EcalMipGraphs::getEEIndex(EcalElectronicsId elecId) {
  int FEDid = 600 + elecId.dccId();
  return 10000 * FEDid + 100 * elecId.towerId() + 5 * (elecId.stripId() - 1) +
         elecId.xtalId();
}
...
void EcalMipGraphs::endJob() {
  ...
  LogWarning("EcalMipGraphs")
      << "Masked FEDs are: " << feds << " and that is all!";
}
