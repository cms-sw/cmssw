...
EcalDisplaysByEvent::EcalDisplaysByEvent(const edm::ParameterSet &iConfig)
    : EBRecHitCollection_(
      ...
      threshold_(
          iConfig.getUntrackedParameter<double>("amplitudeThreshold", 0.5)),
      minTimingAmp_(iConfig.getUntrackedParameter<double>(
          "minimumTimingAmplitude", 0.100)),
      makeDigiGraphs_(
          iConfig.getUntrackedParameter<bool>("makeDigiGraphs", false)),
      makeTimingHistos_(
          iConfig.getUntrackedParameter<bool>("makeTimingHistos", true)),
      makeEnergyHistos_(
          iConfig.getUntrackedParameter<bool>("makeEnergyHistos", true)),
      makeOccupancyHistos_(
          iConfig.getUntrackedParameter<bool>("makeOccupancyHistos", true)),
      histRangeMin_(
          iConfig.getUntrackedParameter<double>("histogramMinRange", 0.0)),
      histRangeMax_(
          iConfig.getUntrackedParameter<double>("histogramMaxRange", 1.8)),
      minTimingEnergyEB_(
          iConfig.getUntrackedParameter<double>("minTimingEnergyEB", 0.0)),
      minTimingEnergyEE_(
          iConfig.getUntrackedParameter<double>("minTimingEnergyEE", 0.0)) {
  ...
  seedCrys_ =
      iConfig.getUntrackedParameter<vector<int>>("seedCrys", listDefaults);
...
void EcalDisplaysByEvent::analyze(edm::Event const &iEvent,
  ...
  if (runNum_ == -1) {
    ...
    histoCanvasNames_ = fileService->make<TTree>(
        "histoCanvasNames", "Names of written canvases with histos");
    histoCanvasNamesVector = new std::vector<string>();
    histoCanvasNames_->Branch("histoCanvasNamesVector", "vector<string>",
...
void EcalDisplaysByEvent::selectHits(Handle<EcalRecHitCollection> hits,
  ...
  for (EcalRecHitCollection::const_iterator hitItr = hits->begin();
    ...
    if (result != maskedFEDs_.end()) {
      // LogWarning("EcalDisplaysByEvent") << "skipping uncalRecHit for FED " <<
      // FEDid << " ; amplitude " << ampli;
      continue;
    }
    result = find(maskedChannels_.begin(), maskedChannels_.end(), cryIndex);
    if (result != maskedChannels_.end()) {
      // LogWarning("EcalDisplaysByEvent") << "skipping uncalRecHit for channel:
    ...
    if (cryIsInList || (seedCrys_.empty() && ampli > threshold_)) {
      ...
      string name = "Digis_Event" + intToString(naiveEvtNum_) + "_ic" +
...
TGraph *EcalDisplaysByEvent::selectDigi(DetId thisDet, int ievt) {
  ...
  if (itr != crysAndAmplitudesMap_.end()) {
    // edm::LogWarning("EcalDisplaysByEvent")<< "itr->second(ampli)="<<
    // itr->second;
    energy = (float)itr->second;
  }
  // else
  // edm::LogWarning("EcalDisplaysByEvent") << "cry " << ic << "not found in
  // ampMap";
...
void EcalDisplaysByEvent::makeHistos(Handle<EEDigiCollection> eeDigiHandle) {
  ...
  for (EEDigiCollection::const_iterator digiItr = eeDigis->begin();
       digiItr != eeDigis->end(); ++digiItr) {
...
void EcalDisplaysByEvent::endJob() {
  ...
  drawCanvas(timingMapCanvasAll_, timingMapEEMAll_, timingMapEBAll_,
             timingMapEEPAll_);
  drawCanvas(energyCanvasAll_, energyEEMAll_, energyEBAll_, energyEEPAll_);
  drawCanvas(energyMapCanvasAll_, energyMapEEMAll_, energyMapEBAll_,
             energyMapEEPAll_);
  drawCanvas(energyMapCoarseCanvasAll_, energyMapEEMcoarseAll_,
             energyMapEBcoarseAll_, energyMapEEPcoarseAll_);
  drawCanvas(recHitOccupancyCanvasAll_, recHitOccupancyEEMAll_,
             recHitOccupancyEBAll_, recHitOccupancyEEPAll_);
  drawCanvas(recHitOccupancyCoarseCanvasAll_, recHitOccupancyEEMcoarseAll_,
             recHitOccupancyEBcoarseAll_, recHitOccupancyEEPcoarseAll_);
  drawCanvas(digiOccupancyCanvasAll_, digiOccupancyEEMAll_, digiOccupancyEBAll_,
             digiOccupancyEEPAll_);
  drawCanvas(digiOccupancyCoarseCanvasAll_, digiOccupancyEEMcoarseAll_,
             digiOccupancyEBcoarseAll_, digiOccupancyEEPcoarseAll_);
  ...
  LogWarning("EcalDisplaysByEvent")
...
void EcalDisplaysByEvent::initEvtByEvtHists(int naiveEvtNum_, int ievt) {
  ...
  if (makeTimingHistos_) {
    ...
    timingEB_ =
        new TH1F("timeForAllFedsEB",
                 "timeForAllFeds;Relative Time (1 clock = 25ns)", 78, -7, 7);
    timingEEM_ = new TH1F("timeForAllFedsEEM",
                          "timeForAllFeds_EEM;Relative Time (1 clock = 25ns)",
                          78, -7, 7);
    timingEEP_ = new TH1F("timeForAllFedsEEP",
                          "timeForAllFeds_EEP;Relative Time (1 clock = 25ns)",
                          78, -7, 7);
    timingCanvas_ = new TCanvas(canvasTitle.c_str(), canvasTitle.c_str(), 300,
                                60, 500, 200);
    ...
    // init2DEcalCanvas("TimingMapCoarse_Event"+lastPart);
  }
  if (makeEnergyHistos_) {
    energyEB_ = new TH1F("energyEB", "Energy for EB Feds (GeV)", 200,
                         histRangeMin_, histRangeMax_);
    energyEEM_ = new TH1F("energyEEM", "Energy for EEM Feds (GeV)", 200,
                          histRangeMin_, 10.0);
    energyEEP_ = new TH1F("energyEEP", "Energy for EEP Feds (GeV)", 200,
                          histRangeMin_, 10.0);
    string canvasTitle = "Energy_Event" + lastPart;
    energyCanvas_ = new TCanvas(canvasTitle.c_str(), canvasTitle.c_str(), 300,
                                60, 500, 200);
    ...
    energyMapCoarseCanvas_ =
        init2DEcalCanvas("EnergyMapCoarse_Event" + lastPart);
  }
  if (makeOccupancyHistos_) {
    // RecHit Occupancy
    ...
    recHitOccupancyCanvas_ =
        init2DEcalCanvas("RecHitOccupancy_Event" + lastPart);
    recHitOccupancyCoarseCanvas_ =
        init2DEcalCanvas("RecHitOccupancyCoarse_Event" + lastPart);
    ...
    digiOccupancyCoarseCanvas_ =
        init2DEcalCanvas("DigiOccupancyCoarse_Event" + lastPart);
  }
}
...
void EcalDisplaysByEvent::initAllEventHistos() {
  ...
  timingEBAll_ =
      new TH1F("timeForAllFedsEBAll",
               "timeForAllFeds;Relative Time (1 clock = 25ns)", 78, -7, 7);
  timingEEMAll_ =
      new TH1F("timeForAllFedsEEMAll",
               "timeForAllFeds_EEM;Relative Time (1 clock = 25ns)", 78, -7, 7);
  timingEEPAll_ =
      new TH1F("timeForAllFedsEEPAll",
  ...
  energyEBAll_ = new TH1F("energyEBAllEvents", "Energy for EB Feds (GeV)", 200,
                          histRangeMin_, histRangeMax_);
  energyEEMAll_ = new TH1F("energyEEMAllEvents", "Energy for EEM Feds (GeV)",
                           200, histRangeMin_, 10.0);
  energyEEPAll_ = new TH1F("energyEEPAllEvents", "Energy for EEP Feds (GeV)",
                           200, histRangeMin_, 10.0);
  ...
  recHitOccupancyCoarseCanvasAll_ =
      init2DEcalCanvas("RecHitOccupancyCoarse_AllEvents");
  ...
  digiOccupancyCoarseCanvasAll_ =
      init2DEcalCanvas("DigiOccupancyCoarse_AllEvents");
}
...
TH3F *EcalDisplaysByEvent::init3DEcalHist(std::string histTypeName,
  ...
  if (isBarrel) {
    ...
    if (isCoarse) {
      ...
      hist = new TH3F(histName.c_str(), histTitle.c_str(), 18, modPhiBins, 9,
                      modEtaBins, 78, timingBins);
    } else {
      histTitle = histTitle + " by TT Nominal value = 6;iphi;ieta";
      hist = new TH3F(histName.c_str(), histTitle.c_str(), 360 / 5, ttPhiBins,
                      35, ttEtaBins, 78, timingBins);
    }
  } else {
    double ttXBins[21];
    double ttYBins[21];
    ...
    if (isCoarse) {
      ...
      hist = new TH3F(histName.c_str(), histTitle.c_str(), 20, ttXBins, 20,
                      ttYBins, 78, timingBins);
    } else {
  ...
  if (isBarrel) {
    ...
    if (isCoarse) {
      ...
      double ttEtaBins[36] = {-85, -80, -75, -70, -65, -60, -55, -50, -45,
                              ...
                              1,   6,   11,  16,  21,  26,  31,  36,  41,
                              46,  51,  56,  61,  66,  71,  76,  81,  86};
      hist = new TH2F(histName.c_str(), histTitle.c_str(), 360 / 5, 1, 361., 35,
                      ttEtaBins);
    } else {
      histTitle = histTitle + ";iphi;ieta";
      hist = new TH2F(histName.c_str(), histTitle.c_str(), 360, 1, 361., 172,
                      -86, 86);
    }
  } else {
    if (isEEM) {
    ...
    if (isCoarse) {
      ...
      hist = new TH2F(histName.c_str(), histTitle.c_str(), 100, 1, 101, 100, 1,
                      101);
    }
  }
  return hist;
...
void EcalDisplaysByEvent::drawHistos() {
  ...
  if (makeEnergyHistos_) {
    ...
    drawCanvas(energyMapCoarseCanvas_, energyMapEEMcoarse_, energyMapEBcoarse_,
               energyMapEEPcoarse_);
  }
  if (makeOccupancyHistos_) {
    // Put the occupancy canvas together
    drawCanvas(recHitOccupancyCanvas_, recHitOccupancyEEM_, recHitOccupancyEB_,
               recHitOccupancyEEP_);
    drawCanvas(recHitOccupancyCoarseCanvas_, recHitOccupancyEEMcoarse_,
               recHitOccupancyEBcoarse_, recHitOccupancyEEPcoarse_);
    // And the same for the digis
    drawCanvas(digiOccupancyCanvas_, digiOccupancyEEM_, digiOccupancyEB_,
               digiOccupancyEEP_);
    drawCanvas(digiOccupancyCoarseCanvas_, digiOccupancyEEMcoarse_,
               digiOccupancyEBcoarse_, digiOccupancyEEPcoarse_);
  }
}

void EcalDisplaysByEvent::drawCanvas(TCanvas *canvas, TH1F *hist1, TH1F *hist2,
                                     TH1F *hist3) {
  canvas->Divide(1, 2);
...
void EcalDisplaysByEvent::drawCanvas(TCanvas *canvas, TH2F *hist1, TH2F *hist2,
                                     TH2F *hist3) {
  canvas->Divide(1, 2);
  canvas->cd(1)->Divide(2, 1);
  // EEM
...
void EcalDisplaysByEvent::drawCanvas(TCanvas *canvas, TH3F *hist1, TH3F *hist2,
                                     TH3F *hist3) {
...
void EcalDisplaysByEvent::drawEELines() {
  ...
  int ixSectorsEE[202] = {
      61, 61, 60, 60, 59,  59,  58, 58, 57, 57, 55, 55, 45, 45,  43,  43, 42,
      42, 41, 41, 40, 40,  39,  39, 40, 40, 41, 41, 42, 42, 43,  43,  45, 45,
      55, 55, 57, 57, 58,  58,  59, 59, 60, 60, 61, 61, 0,  100, 100, 97, 97,
      95, 95, 92, 92, 87,  87,  85, 85, 80, 80, 75, 75, 65, 65,  60,  60, 40,
      40, 35, 35, 25, 25,  20,  20, 15, 15, 13, 13, 8,  8,  5,   5,   3,  3,
      0,  0,  3,  3,  5,   5,   8,  8,  13, 13, 15, 15, 20, 20,  25,  25, 35,
      35, 40, 40, 60, 60,  65,  65, 75, 75, 80, 80, 85, 85, 87,  87,  92, 92,
      95, 95, 97, 97, 100, 100, 0,  61, 65, 65, 70, 70, 80, 80,  90,  90, 92,
      0,  61, 65, 65, 90,  90,  97, 0,  57, 60, 60, 65, 65, 70,  70,  75, 75,
      80, 80, 0,  50, 50,  0,   43, 40, 40, 35, 35, 30, 30, 25,  25,  20, 20,
      0,  39, 35, 35, 10,  10,  3,  0,  39, 35, 35, 30, 30, 20,  20,  10, 10,
      8,  0,  45, 45, 40,  40,  35, 35, 0,  55, 55, 60, 60, 65,  65};

  int iySectorsEE[202] = {
      50, 55, 55, 57, 57,  58, 58, 59, 59, 60, 60, 61, 61, 60, 60, 59,  59,
      58, 58, 57, 57, 55,  55, 45, 45, 43, 43, 42, 42, 41, 41, 40, 40,  39,
      39, 40, 40, 41, 41,  42, 42, 43, 43, 45, 45, 50, 0,  50, 60, 60,  65,
      65, 75, 75, 80, 80,  85, 85, 87, 87, 92, 92, 95, 95, 97, 97, 100, 100,
      97, 97, 95, 95, 92,  92, 87, 87, 85, 85, 80, 80, 75, 75, 65, 65,  60,
      60, 40, 40, 35, 35,  25, 25, 20, 20, 15, 15, 13, 13, 8,  8,  5,   5,
      3,  3,  0,  0,  3,   3,  5,  5,  8,  8,  13, 13, 15, 15, 20, 20,  25,
      25, 35, 35, 40, 40,  50, 0,  45, 45, 40, 40, 35, 35, 30, 30, 25,  25,
      0,  50, 50, 55, 55,  60, 60, 0,  60, 60, 65, 65, 70, 70, 75, 75,  85,
      85, 87, 0,  61, 100, 0,  60, 60, 65, 65, 70, 70, 75, 75, 85, 85,  87,
      0,  50, 50, 55, 55,  60, 60, 0,  45, 45, 40, 40, 35, 35, 30, 30,  25,
      25, 0,  39, 30, 30,  15, 15, 5,  0,  39, 30, 30, 15, 15, 5};

  for (int i = 0; i < 202; i++) {
    ixSectorsEE[i] += 1;
    iySectorsEE[i] += 1;
    //   std::cout << i << " " << ixSectorsEE[i] << " " << iySectorsEE[i] <<
    //   std::endl;
  }
  ...
  for (int i = 0; i < 201; i = i + 1) {
    if ((ixSectorsEE[i] != 1 || iySectorsEE[i] != 1) &&
      ...
      l.DrawLine(ixSectorsEE[i], iySectorsEE[i], ixSectorsEE[i + 1],
                 iySectorsEE[i + 1]);
    }
  }
}
