...
EcalHexDisplay::EcalHexDisplay(const edm::ParameterSet &ps)
    : verbosity_(ps.getUntrackedParameter<int>("verbosity", 1)),
      ...
      event_(0), writeDcc_(ps.getUntrackedParameter<bool>("writeDCC", false)),
      filename_(ps.getUntrackedParameter<std::string>("filename", "dump.bin")),
      fedRawDataCollectionTag_(
          ps.getParameter<edm::InputTag>("fedRawDataCollectionTag")) {}
...
void EcalHexDisplay::analyze(const edm::Event &e, const edm::EventSetup &c) {
  ...
  for (int id = 0; id <= FEDNumbering::MAXFEDID; ++id) {
    ...
    if (data.size() > 4) {
      ...
      std::cout << "\n\n\n[EcalHexDumperModule] Event: " << std::dec << event_
                << " fed_id: " << id << " size_fed: " << data.size() << "\n"
                << std::endl;

      if ((data.size() % 16) != 0) {
        std::cout << "***********************************************"
                  << std::endl;
        std::cout << "Fed size in bits not multiple of 64, strange."
                  << std::endl;
        std::cout << "***********************************************"
                  << std::endl;
      }
      ...
      const unsigned long *pData = (reinterpret_cast<unsigned long *>(
