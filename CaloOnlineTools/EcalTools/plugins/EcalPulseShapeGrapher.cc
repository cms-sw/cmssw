...
//
/**\class EcalPulseShapeGrapher EcalPulseShapeGrapher.cc
 Analyzers/EcalPulseShapeGrapher/src/EcalPulseShapeGrapher.cc
...
EcalPulseShapeGrapher::EcalPulseShapeGrapher(const edm::ParameterSet &iConfig)
    : EBUncalibratedRecHitCollection_(iConfig.getParameter<edm::InputTag>(
      ...
      rootFilename_(iConfig.getUntrackedParameter<std::string>(
          "rootFilename", "pulseShapeGrapher")) {
  // now do what ever initialization is needed

  std::vector<int> listDefaults;
  listDefaults.push_back(-1);
  listChannels_ = iConfig.getUntrackedParameter<std::vector<int>>(
      "listChannels", listDefaults);

  for (std::vector<int>::const_iterator itr = listChannels_.begin();
    ...
    rawPulseShapeHistMap_[*itr] =
        new TH2F(name.c_str(), title.c_str(), 10, 0, 10, 500, 0, 500);
    ...
    firstSampleHistMap_[*itr] =
        new TH1F(name.c_str(), title.c_str(), 300, 100, 400);
...
void EcalPulseShapeGrapher::analyze(const edm::Event &iEvent,
  ...
  // cout << "event: " << eventNum << " sample hits collection size: " <<
  // sampleHits->size() << endl; Handle<EcalUncalibratedRecHitCollection>
  // fittedHits; iEvent.getByLabel(FittedUncalibratedRecHitCollection_,
  // fittedHits);
  ...
  // cout << "event: " << eventNum << " digi collection size: " << digis->size()
  // << endl;

  unique_ptr<EcalElectronicsMapping> ecalElectronicsMap(
      new EcalElectronicsMapping);
  ...
  for (EcalUncalibratedRecHitCollection::const_iterator hitItr =
    ...
    // cout << "Cry hash:" << hitDetId.hashedIndex() << " amplitude: " <<
    // amplitude << endl;
    ...
    // cout << "1) maxsample amp:" << maxSampleAmp << " maxSampleIndex:" <<
    // maxSampleIndex << endl;
    for (int i = 0; i < 10; ++i) {
      // cout << "Filling hist for:" << hitDetId.hashedIndex() << " with
      // sample:" << i
      //<< " amp:"
      //<<(float)(df.sample(i).adc()-baseline)/(maxSampleAmp-baseline) << endl;
      // cout << "ADC of sample:" << df.sample(i).adc() << " baseline:" <<
      // baseline << " maxSampleAmp:"
      //<< maxSampleAmp << endl << endl;
      pulseShapeHistMap_[hitDetId.hashedIndex()]->Fill(
          i, (float)(sampleADC[i] - pedestal) / amplitude);
      rawPulseShapeHistMap_[hitDetId.hashedIndex()]->Fill(
          i, (float)(sampleADC[i]));
    }
