...
EcalPedHists::EcalPedHists(const edm::ParameterSet &ps)
    : runNum_(-1), fileName_(ps.getUntrackedParameter<std::string>(
                       "fileName", std::string("ecalPedDigiDump"))),
  ...
  if (listFEDs_.empty()) {
    ...
    if (!listEBs_.empty()) {
      listFEDs_.clear();
      for (vector<string>::const_iterator itr = listEBs_.begin();
           itr != listEBs_.end(); ++itr) {
  ...
  if (!allFEDsSelected_) {
    ...
    for (vector<int>::const_iterator intIter = listFEDs_.begin();
         intIter != listFEDs_.end(); intIter++) {
      if (((*intIter) < 601) || (654 < (*intIter))) {
        cout << "[EcalPedHists] FED value: " << (*intIter)
             << " found in listFEDs. "
  ...
  listChannels_ =
      ps.getUntrackedParameter<vector<int>>("listChannels", listDefaults);
  listDefaults.clear();
  ...
  listSamples_ =
      ps.getUntrackedParameter<vector<int>>("listSamples", listDefaults);

  inputIsOk_ = true;
  vector<int>::iterator intIter;

  // Verify crystal numbers are valid
  for (intIter = listChannels_.begin(); intIter != listChannels_.end();
       ++intIter) {
    // TODO: Fix crystal index checking?
    // if ( ((*intIter) < 1)||(1700 < (*intIter)) )
    //{
    //  cout << "[EcalPedHists] ic value: " << (*intIter) << " found in
    //  listChannels. "
  ...
  for (intIter = listSamples_.begin(); intIter != listSamples_.end();
       intIter++) {
    if (((*intIter) < 1) || (10 < (*intIter))) {
      cout << "[EcalPedHists] sample number: " << (*intIter)
...
void EcalPedHists::endJob(void) {
  ...
  if (inputIsOk_) {
    ...
    for (set<int>::const_iterator FEDitr = theRealFedSet_.begin();
         FEDitr != theRealFedSet_.end(); ++FEDitr) {
      if (!histsFilled_)
        break;
      ...
      for (vector<int>::const_iterator itr = listChannels_.begin();
           itr != listChannels_.end(); itr++) {
        // debug
...
void EcalPedHists::analyze(const edm::Event &e, const edm::EventSetup &c) {
  ...
  if (allFEDsSelected_) {
    ...
    try {
      ...
      edm::LogError("EcalPedHists")
          << "Error! can't get the product " << headerProducer_;
      return;
    }
  ...
  try {
    ...
    edm::LogError("EcalPedOffset")
        << "Error! can't get the product " << barrelDigiCollection_;
  ...
  try {
    ...
    edm::LogError("EcalPedOffset")
        << "Error! can't get the product " << endcapDigiCollection_;
...
void EcalPedHists::initHists(int FED) {
  ...
  for (vector<int>::const_iterator intIter = listChannels_.begin();
       intIter != listChannels_.end(); ++intIter) {
    ...
    string name3 = "Cry";
    name3.append(chnl + "Gain12");
    histMap.insert(make_pair(
        name1, new TH1F(name1.c_str(), title1.c_str(), 75, 175.0, 250.0)));
    histMap[name1]->SetDirectory(nullptr);
    histMap.insert(make_pair(
        name2, new TH1F(name2.c_str(), title2.c_str(), 75, 175.0, 250.0)));
    histMap[name2]->SetDirectory(nullptr);
    histMap.insert(make_pair(
        name3, new TH1F(name3.c_str(), title3.c_str(), 75, 175.0, 250.0)));
...
void EcalPedHists::readEBdigis(edm::Handle<EBDigiCollection> digis) {
  ...
  for (EBDigiCollection::const_iterator digiItr = digis->begin();
    ...
    set<int>::const_iterator fedIter =
        find(theRealFedSet_.begin(), theRealFedSet_.end(), FEDid);
    ...
    // Get the adc counts from the selected samples and fill the corresponding
    // histogram Must subtract 1 from user-given sample list (e.g., user's
    // sample 1 -> sample 0)
    for (vector<int>::iterator itr = listSamples_.begin();
         itr != listSamples_.end(); itr++) {
      histsFilled_ = true;
...
void EcalPedHists::readEEdigis(edm::Handle<EEDigiCollection> digis) {
  ...
  // Loop over digis
  for (EEDigiCollection::const_iterator digiItr = digis->begin();
       digiItr != digis->end(); ++digiItr) {
    EEDetId detId = EEDetId(digiItr->id());
    EcalElectronicsId elecId = ecalElectronicsMap_->getElectronicsId(detId);
    int FEDid = 600 + elecId.dccId();
    int crystalId = 10000 * FEDid + 100 * elecId.towerId() +
                    5 * (elecId.stripId() - 1) + elecId.xtalId();
