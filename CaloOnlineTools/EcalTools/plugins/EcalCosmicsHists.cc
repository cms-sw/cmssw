...
EcalCosmicsHists::EcalCosmicsHists(const edm::ParameterSet &iConfig)
    : ecalRawDataColl_(iConfig.getParameter<edm::InputTag>("ecalRawDataColl")),
      ecalRecHitCollectionEB_(
          iConfig.getParameter<edm::InputTag>("ecalRecHitCollectionEB")),
      ecalRecHitCollectionEE_(
          iConfig.getParameter<edm::InputTag>("ecalRecHitCollectionEE")),
      barrelClusterCollection_(
          iConfig.getParameter<edm::InputTag>("barrelClusterCollection")),
      endcapClusterCollection_(
          iConfig.getParameter<edm::InputTag>("endcapClusterCollection")),
      l1GTReadoutRecTag_(iConfig.getUntrackedParameter<std::string>(
          "L1GlobalReadoutRecord", "gtDigis")),
      l1GMTReadoutRecTag_(iConfig.getUntrackedParameter<std::string>(
          "L1GlobalMuonReadoutRecord", "gtDigis")),
      ...
      minTimingAmpEB_(
          iConfig.getUntrackedParameter<double>("MinTimingAmpEB", 0.100)),
      minTimingAmpEE_(
          iConfig.getUntrackedParameter<double>("MinTimingAmpEE", 0.100)),
      minRecHitAmpEB_(
          iConfig.getUntrackedParameter<double>("MinRecHitAmpEB", 0.027)),
      minRecHitAmpEE_(
          iConfig.getUntrackedParameter<double>("MinRecHitAmpEE", 0.18)),
      minHighEnergy_(
          iConfig.getUntrackedParameter<double>("MinHighEnergy", 2.0)),
      fileName_(iConfig.getUntrackedParameter<std::string>(
          "fileName", std::string("ecalCosmicHists"))),
      runInFileName_(
          iConfig.getUntrackedParameter<bool>("runInFileName", true)),
      startTime_(
          iConfig.getUntrackedParameter<double>("TimeStampStart", 1215107133.)),
      runTimeLength_(
          iConfig.getUntrackedParameter<double>("TimeStampLength", 3.)),
      numTimingBins_(
          iConfig.getUntrackedParameter<int>("TimeStampBins", 1800)) {
  naiveEvtNum_ = 0;
  cosmicCounter_ = 0;
  cosmicCounterEB_ = 0;
  cosmicCounterEEM_ = 0;
  ...
  edm::ParameterSet trkParameters =
...
void EcalCosmicsHists::analyze(edm::Event const &iEvent,
  ...
  // LogDebug("EcalCosmicsHists")<< "  My Event: " << naiveEvtNum_ << " " <<
  // iEvent.id().run() << " " << iEvent.id().event() << " " <<
  // iEvent.time().value(); LogDebug("EcalCosmicsHists")<< "Timestamp: " <<
  // iEvent.id().run() << " " << iEvent.id().event() << " " <<
  // iEvent.time().value();
  ...
  // std::cout << "Event Time " << eventTime << " High " <<timeStampHigh<< "
  // low"<<timeStampLow <<" value " <<iEvent.time().value() << std::endl;
  //========================================================================

  iEvent.getByLabel(barrelClusterCollection_, bscHandle);
  if (!(bscHandle.isValid())) {
    LogWarning("EcalCosmicsHists")
        << barrelClusterCollection_ << " not available";
    return;
  }
  ...
  if (!(escHandle.isValid())) {
    LogWarning("EcalCosmicsHists")
        << endcapClusterCollection_ << " not available";
  ...
  if (!(hits.isValid())) {
    LogWarning("EcalCosmicsHists")
        << ecalRecHitCollectionEB_ << " not available";
    return;
  }
  ...
  if (!(hitsEE.isValid())) {
    LogWarning("EcalCosmicsHists")
        << ecalRecHitCollectionEE_ << " not available";
    return;
  }
  ...
  for (EcalRawDataCollection::const_iterator headerItr = DCCHeaders->begin();
    ...
    if (bx == -100) {
      ...
      LogWarning("EcalCosmicsHists")
          << "This header has a conflicting bx OTHERS were " << bx << " here "
          << mybx;
  ...
  for (reco::SuperClusterCollection::const_iterator clus =
           clusterCollection_p->begin();
    ...
    for (reco::CaloCluster_iterator bclus = (clus->clustersBegin());
         bclus != (clus->clustersEnd()); ++bclus) {
    ...
    std::vector<std::pair<DetId, float>> clusterDetIds =
        clus->hitsAndFractions(); // get these from the cluster
    for (std::vector<std::pair<DetId, float>>::const_iterator detitr =
             clusterDetIds.begin();
      ...
      EcalRecHitCollection::const_iterator thishit =
          hits->find((*detitr).first);
      if (thishit == hits->end())
        continue;
      // The checking above should no longer be needed...... as only those in
      // the cluster would already have rechits..
    ...
    EcalElectronicsId elecId =
        ecalElectronicsMap_->getElectronicsId((EBDetId)maxDet);
    ...
    if (energy > minHighEnergy_) {
      ...
      LogInfo("EcalCosmicsHists")
          << "High energy event " << iEvent.id().run() << " : "
          << iEvent.id().event() << " " << naiveEvtNum_ << " : " << energy
          << " " << numXtalsinCluster << " : " << iphi << " " << ieta << " : "
      ...
      if (energy > 100.0) {
        LogInfo("EcalCosmicsHists")
            << "Very high energy event " << iEvent.id().run() << " : "
            << iEvent.id().event() << " " << naiveEvtNum_ << " : " << energy
            << " " << numXtalsinCluster << " : " << iphi << " " << ieta << " : "
            << isEcalL1 << isHCALL1 << isDTL1 << isRPCL1 << isCSCL1;
  ...
  if (hasEndcapClusters) {
    ...
    for (reco::SuperClusterCollection::const_iterator clus =
      ...
      EEDetId maxDet;
      EEDetId secDet;
      // LogInfo("EcalCosmicsHists") << "Here is what we initialized the maxDet
      // to: " << maxDet;

      //      for (reco::basicCluster_iterator bclus = (clus->clustersBegin());
      //      bclus != (clus->clustersEnd()); ++bclus) {
      // 	//double cphi = (*bclus)->phi();
      // 	//double ceta = (*bclus)->eta();
      ...
      for (std::vector<std::pair<DetId, float>>::const_iterator detitr =
        ...
        // LogInfo("EcalCosmicsHists") << " Here is the DetId inside the
        // cluster: " << (EEDetId)(*detitr); Here I use the "find" on a digi
        // collection... I have been warned...
        ...
        if ((*detitr).first.subdetId() != EcalEndcap) {
          LogError("EcalCosmicsHists")
              << " subdet is " << (*detitr).first.subdetId();
          continue;
        }

        EcalRecHitCollection::const_iterator thishit =
            hitsEE->find((*detitr).first);
        ...
        // LogInfo("EcalCosmicsHists") << " Found hit for DetId: " <<
        // (EEDetId)(*detitr);
        double thisamp = myhit.energy();
        ...
        // LogInfo("EcalCosmicsHists") << "maxDetId is now: " <<
        // (EEDetId)(maxDet);
      }
      ...
      // Set some more values
      // TODO: need to fix the seeds vector to be DetId or have another one for
      // EE seeds.push_back(maxDet);
      ...
      if (energy > minHighEnergy_) {
        ...
        LogInfo("EcalCosmicsHists")
            << "High energy event in EE " << iEvent.id().run() << " : "
            << iEvent.id().event() << " " << naiveEvtNum_ << " : " << energy
            << " " << numXtalsinCluster << " : " << ix << " " << iy << " : "
            << isEcalL1 << isHCALL1 << isDTL1 << isRPCL1 << isCSCL1;
        ...
        if (energy > 100.0) {
          LogInfo("EcalCosmicsHists")
              << "Very high energy event in EE " << iEvent.id().run() << " : "
              << iEvent.id().event() << " " << naiveEvtNum_ << " : " << energy
              << " " << numXtalsinCluster << " : " << ix << " " << iy << " : "
              << isEcalL1 << isHCALL1 << isDTL1 << isRPCL1 << isCSCL1;
          //    HighEnergy_100GeV_occuCoarse->Fill(iphi,ieta);
  ...
  HighEnergy_ratioClusters->Fill((double(numberOfHighEClusters)) /
                                 (double(numberOfCosmics)));
  ...
  if (numberOfCosmics > 0) {
    ...
    // std::cout << " Orbit " <<   iEvent.orbitNumber() << " BX " <<
    // iEvent.bunchCrossing()<< std::endl; std::cout << " BX " <<
    // iEvent.experimentType() << std::endl;
  }
  ...
  if (numberOfCosmicsTop && numberOfCosmicsBottom) {
    ...
    numberofCosmicsTopBottomHist_->Fill(numberOfCosmicsTop +
                                        numberOfCosmicsBottom);
  }
  ...
  if (recoTracks.isValid()) {
    //    LogWarning("EcalCosmicsHists") << "... Valid TrackAssociator
    //    recoTracks !!! " << recoTracks.product()->size();
    ...
    for (reco::TrackCollection::const_iterator recoTrack = recoTracks->begin();
      ...
      TrackDetMatchInfo info = trackAssociator_.associate(
          iEvent, iSetup, *recoTrack, trackParameters_);

      //       edm::LogVerbatim("TrackAssociator") <<
      //       "\n-------------------------------------------------------\n
      //       Track (pt,eta,phi): " <<
      //    recoTrack->pt() << " , " << recoTrack->eta() << " , " <<
      //    recoTrack->phi() ;
      //       edm::LogVerbatim("TrackAssociator") << "Ecal energy in crossed
      //       crystals based on RecHits: " <<
      //    info.crossedEnergy(TrackDetMatchInfo::EcalRecHits);
      //       edm::LogVerbatim("TrackAssociator") << "Ecal energy in 3x3
      //       crystals based on RecHits: " <<
      //    info.nXnEnergy(TrackDetMatchInfo::EcalRecHits, 1);
      //       edm::LogVerbatim("TrackAssociator") << "Hcal energy in crossed
      //       towers based on RecHits: " <<
      //    info.crossedEnergy(TrackDetMatchInfo::HcalRecHits);
      //       edm::LogVerbatim("TrackAssociator") << "Hcal energy in 3x3 towers
      //       based on RecHits: " <<
      //    info.nXnEnergy(TrackDetMatchInfo::HcalRecHits, 1);
      //       edm::LogVerbatim("TrackAssociator") << "Number of muon segment
      //       matches: " << info.numberOfSegments();

      //       std::cout <<
      //       "\n-------------------------------------------------------\n
      //       Track (pt,eta,phi): " <<
      //    recoTrack->pt() << " , " << recoTrack->eta() << " , " <<
      //    recoTrack->phi() << std::endl;
      ...
      for (unsigned int i = 0; i < info.crossedEcalIds.size(); i++) {
        ...
        if (info.crossedEcalIds[i].det() == DetId::Ecal &&
            info.crossedEcalIds[i].subdetId() == 1) {
          ...
          //    << " ieta: " << ebDetId.ieta() << " : nCross " <<
          //    info.crossedEcalIds.size() << std::endl;
          ...
          // EcalRecHit myhit = (*thishit);
          // double thisamp = myhit.energy();
          // std::cout << " Crossed energy: " << thisamp << " : nCross " <<
          // info.crossedEcalIds.size() << std::endl;
        }
      }

      // edm::LogVerbatim("TrackAssociator") << " crossedEcalIds size: " <<
      // info.crossedEcalIds.size()
      //				  << " crossedEcalRecHits size: " <<
      // info.crossedEcalRecHits.size();
      numberofCrossedEcalIdsHist_->Fill(info.crossedEcalIds.size());
      tracks++;
      if (!info.crossedEcalIds.empty())
        trackDetIdMap.insert(
            std::pair<int, std::vector<DetId>>(tracks, info.crossedEcalIds));
    }
    ...
    while (!seeds.empty() && !trackDetIdMap.empty()) {
      ...
      // edm::LogVerbatim("TrackAssociator") << "NumTracks:" <<
      // trackDetIdMap.size() << " numClusters:" << seeds.size();

      for (std::vector<EBDetId>::const_iterator seedItr = seeds.begin();
           seedItr != seeds.end(); ++seedItr) {
        for (std::map<int, std::vector<DetId>>::const_iterator mapItr =
                 trackDetIdMap.begin();
             mapItr != trackDetIdMap.end(); ++mapItr) {
          for (unsigned int i = 0; i < mapItr->second.size(); i++) {
            // only checks for barrel
            if (mapItr->second[i].det() == DetId::Ecal &&
                mapItr->second[i].subdetId() == 1) {
      ...
      if (bestDr < 1000) {
        // edm::LogVerbatim("TrackAssociator") << "Best deltaR from matched
        // DetId's to cluster:" << bestDr;
        ...
        if (bestDPhi < 1.5 && bestDEta < 1.8) {
          ...
          // edm::LogVerbatim("TrackAssociator") << "Best seed energy: " <<
          // EcalhitEnergy ;
  ...
  if (!recoTracksBarrel.isValid()) {
    ...
    // edm::LogWarning("EcalCosmicsHists") << "Number of barrel reco tracks
    // found in the event: " << recoTracksBarrel->size() ;
    HighEnergy_numRecoTrackBarrel->Fill(recoTracksBarrel->size());
    for (reco::TrackCollection::const_iterator recoTrack =
             recoTracksBarrel->begin();
         recoTrack != recoTracksBarrel->end(); ++recoTrack) {
      // edm::LogWarning("EcalCosmicsHists") << "Track (pt,eta,phi): " <<
      // recoTrack->pt() << " , " << recoTrack->eta() << " , " <<
      // recoTrack->phi() ; edm::LogWarning("EcalCosmicsHists") << "Track
      // innermost hit: " << recoTrack->innerPosition().phi() ;
    }
    ...
    for (reco::SuperClusterCollection::const_iterator clus =
      ...
      for (std::vector<std::pair<DetId, float>>::const_iterator detitr =
        ...
        if (rechitenergy > minRecHitAmpEB_) {
          ...
          if (rechitenergy > 10)
            edm::LogWarning("EcalCosmicsHists")
                << "!!!!! Crystal with energy " << rechitenergy
                << " at (ieta,iphi) (" << ieta << " ," << iphi
                << "); Id: " << (*detitr).first.det();
        }
      }
    }
    ...
    if (recoTracksBarrel->size() == 2) {
      ...
      reco::TrackCollection::const_iterator Track2 =
          (recoTracksBarrel->begin()) + 1;
      float angle = (acos(sin(Track1->theta()) * cos(Track1->phi()) *
                              sin(Track2->theta()) * cos(Track2->phi()) +
                          sin(Track1->theta()) * sin(Track1->phi()) *
                              sin(Track2->theta()) * sin(Track2->phi()) +
                          cos(Track1->theta()) * cos(Track2->theta()))) /
                    0.017453292519943;
      // edm::LogWarning("EcalCosmicsHists") << "Tracks angle: " << angle;
      HighEnergy_TracksAngle->Fill(angle);
      if ((Track1->innerPosition().phi()) > 0 &&
          (Track2->innerPosition().phi()) < 0) {
  ...
  if (hbhe.isValid()) {
    //    LogInfo("EcalCosmicHists") << "event " << ievt << " HBHE RecHits
    //    collection size " << hbhe->size();
    const HBHERecHitCollection hbheHit = *(hbhe.product());
    for (HBHERecHitCollection::const_iterator hhit = hbheHit.begin();
         hhit != hbheHit.end(); hhit++) {
      //      if (hhit->energy() > 0.6){
  ...
  if (hfrh.isValid()) {
    //    LogInfo("EcalCosmicHists") << "event " << ievt << " HF RecHits
    //    collection size " << hfrh->size();
    const HFRecHitCollection hfHit = *(hfrh.product());
    for (HFRecHitCollection::const_iterator hhit = hfHit.begin();
         hhit != hfHit.end(); hhit++) {
  ...
  if (horh.isValid()) {
    //    LogInfo("EcalCosmicHists") << "event " << ievt << " HO RecHits
    //    collection size " << horh->size();
    const HORecHitCollection hoHit = *(horh.product());
    for (HORecHitCollection::const_iterator hhit = hoHit.begin();
         hhit != hoHit.end(); hhit++) {
      //     if (hhit->energy() > 0.6){
...
std::vector<bool>
EcalCosmicsHists::determineTriggers(const edm::Event &iEvent,
  ...
  bool l1DoubleNoIsoEGBTBtight =
      menu->gtAlgorithmResult("L1_DoubleNoIsoEG_BTB_tight", dWord);
  bool l1DoubleNoIsoEGBTBloose =
      menu->gtAlgorithmResult("L1_DoubleNoIsoEG_BTB_loose ", dWord);
  bool l1DoubleNoIsoEGTopBottom =
      menu->gtAlgorithmResult("L1_DoubleNoIsoEGTopBottom", dWord);
  bool l1DoubleNoIsoEGTopBottomCen =
      menu->gtAlgorithmResult("L1_DoubleNoIsoEGTopBottomCen", dWord);
  bool l1DoubleNoIsoEGTopBottomCen2 =
      menu->gtAlgorithmResult("L1_DoubleNoIsoEGTopBottomCen2", dWord);
  bool l1DoubleNoIsoEGTopBottomCenVert =
      menu->gtAlgorithmResult("L1_DoubleNoIsoEGTopBottomCenVert", dWord);

  l1Triggers[4] = l1SingleEG1 || l1SingleEG5 || l1SingleEG8 || l1SingleEG10 ||
                  l1SingleEG12 || l1SingleEG15 || l1SingleEG20 ||
                  l1SingleEG25 || l1DoubleNoIsoEGBTBtight ||
                  l1DoubleNoIsoEGBTBloose || l1DoubleNoIsoEGTopBottom ||
                  l1DoubleNoIsoEGTopBottomCen || l1DoubleNoIsoEGTopBottomCen2 ||
  ...
  for (igmtrr = gmt_records.begin(); igmtrr != gmt_records.end(); igmtrr++) {
    ...
    // if(idt>0) std::cout << "Found " << idt << " valid DT candidates in bx
    // wrt. L1A = "
    ...
    // if(irpcb>0) std::cout << "Found " << irpcb << " valid RPC candidates in
    // bx wrt. L1A = "
    //  << igmtrr->getBxInEvent() << std::endl;
    ...
    // if(icsc>0) std::cout << "Found " << icsc << " valid CSC candidates in bx
    // wrt. L1A = "
    //  << igmtrr->getBxInEvent() << std::endl;
  ...
  for (int ibx = -1; ibx <= 1; ibx++) {
    ...
    for (iphi = valid_phi.begin(); iphi != valid_phi.end(); iphi++) {
      // std::cout << "Found HCAL mip with phi=" << *iphi << " in bx wrt. L1A =
      // " << ibx << std::endl;
      if (*iphi < 9)
        hcal_top = true;
      if (*iphi > 8)
...
void EcalCosmicsHists::initHists(int FED) {
  ...
  TH1F *E2hist = new TH1F(Form("E2_FED_%d", FED), Form("E2_FED_%d", FED),
                          numBins, histRangeMin_, histRangeMax_);
  FEDsAndE2Hists_[FED] = E2hist;
  FEDsAndE2Hists_[FED]->SetDirectory(nullptr);

  TH1F *energyhist =
      new TH1F(Form("Energy_FED_%d", FED), Form("Energy_FED_%d", FED), numBins,
               histRangeMin_, histRangeMax_);
  FEDsAndenergyHists_[FED] = energyhist;
  FEDsAndenergyHists_[FED]->SetDirectory(nullptr);

  TH2F *E2vsE1hist = new TH2F(
      Form("E2vsE1_FED_%d", FED), Form("E2vsE1_FED_%d", FED), numBins,
      histRangeMin_, histRangeMax_, numBins, histRangeMin_, histRangeMax_);
  FEDsAndE2vsE1Hists_[FED] = E2vsE1hist;
  FEDsAndE2vsE1Hists_[FED]->SetDirectory(nullptr);

  TH2F *energyvsE1hist = new TH2F(
      Form("EnergyvsE1_FED_%d", FED), Form("EnergyvsE1_FED_%d", FED), numBins,
      histRangeMin_, histRangeMax_, numBins, histRangeMin_, histRangeMax_);
  ...
  TH1F *freqHist =
      new TH1F(Form("Frequency_FED_%d", FED),
               Form("Frequency for FED %d;Event Number", FED), 100, 0., 100000);
  FEDsAndFrequencyHists_[FED] = freqHist;
  FEDsAndFrequencyHists_[FED]->SetDirectory(nullptr);

  TH1F *iphiProfileHist =
      new TH1F(Form("iPhi_Profile_FED_%d", FED),
               Form("iPhi Profile for FED %d", FED), 360, 1., 361);
  FEDsAndiPhiProfileHists_[FED] = iphiProfileHist;
  FEDsAndiPhiProfileHists_[FED]->SetDirectory(nullptr);

  TH1F *ietaProfileHist =
      new TH1F(Form("iEta_Profile_FED_%d", FED),
               Form("iEta Profile for FED %d", FED), 172, -86, 86);
  FEDsAndiEtaProfileHists_[FED] = ietaProfileHist;
  FEDsAndiEtaProfileHists_[FED]->SetDirectory(nullptr);

  TH2F *timingHistVsFreq =
      new TH2F(Form("timeVsFreqFED_%d", FED), Form("time Vs Freq FED %d", FED),
               78, -7, 7, 100, 0., 100000);
  FEDsAndTimingVsFreqHists_[FED] = timingHistVsFreq;
  FEDsAndTimingVsFreqHists_[FED]->SetDirectory(nullptr);

  TH2F *timingHistVsAmp =
      new TH2F(Form("timeVsAmpFED_%d", FED), Form("time Vs Amp FED %d", FED),
               78, -7, 7, numBins, histRangeMin_, histRangeMax_);
  FEDsAndTimingVsAmpHists_[FED] = timingHistVsAmp;
  FEDsAndTimingVsAmpHists_[FED]->SetDirectory(nullptr);

  TH1F *numXtalInClusterHist = new TH1F(
      Form("NumXtalsInCluster_FED_%d", FED),
      Form("Num active Xtals In Cluster for FED %d;Num Active Xtals", FED), 25,
      0, 25);
  FEDsAndNumXtalsInClusterHists_[FED] = numXtalInClusterHist;
  FEDsAndNumXtalsInClusterHists_[FED]->SetDirectory(nullptr);

  TH2F *OccupHist =
      new TH2F(Form("occupFED_%d", FED),
               Form("Occupancy FED %d;i#eta;i#phi", FED), 85, 1, 86, 20, 1, 21);
  FEDsAndOccupancyHists_[FED] = OccupHist;
  FEDsAndOccupancyHists_[FED]->SetDirectory(nullptr);

  TH2F *timingHistVsPhi = new TH2F(
      Form("timeVsPhiFED_%d", FED),
      Form("time Vs Phi FED %d;Relative Time (1 clock = 25ns);i#phi", FED), 78,
      -7, 7, 20, 1, 21);
  FEDsAndTimingVsPhiHists_[FED] = timingHistVsPhi;
  FEDsAndTimingVsPhiHists_[FED]->SetDirectory(nullptr);

  TH2F *timingHistVsModule = new TH2F(
      Form("timeVsModuleFED_%d", FED),
  ...
  TH2F *dccRuntypeVsBxFED = new TH2F(Form("DCCRuntypeVsBxFED_%d", FED),
                                     Form("DCC Runtype vs. BX FED %d", FED),
                                     3600, 0, 3600, 24, 0, 24);
...
void EcalCosmicsHists::beginRun(edm::Run const &,
  ...
  //=============Special Bins for TT and Modules
  // borders=============================
  ...
  //=============END Special Bins for TT and Modules
  // borders===========================

  //====================Frequency for Event timing
  // information=========================
  double timingEndInSeconds = runTimeLength_ * 3600.;
  double timingBinWidth = timingEndInSeconds / double(numTimingBins_);
  //====================END Frequency for Event timing
  // information=====================

  allFedsenergyHist_ =
      new TH1F("energy_AllClusters", "energy_AllClusters;Cluster Energy (GeV)",
               numBins, histRangeMin_, histRangeMax_);
  allFedsenergyHighHist_ = new TH1F(
      "energyHigh_AllClusters", "energyHigh_AllClusters;Cluster Energy (GeV)",
      numBins, histRangeMin_, 200.0);
  allFedsenergyOnlyHighHist_ =
      new TH1F("energyHigh_HighEnergyClusters",
               "energy of High Energy Clusters;Cluster Energy (GeV)", numBins,
               histRangeMin_, 200.0);
  allFedsE2Hist_ = new TH1F("E2_AllClusters",
                            "E2_AllClusters;Seed+highest neighbor energy (GeV)",
                            numBins, histRangeMin_, histRangeMax_);
  allFedsE2vsE1Hist_ = new TH2F(
      "E2vsE1_AllClusters",
      "E2vsE1_AllClusters;Seed Energy (GeV);Seed+highest neighbor energy (GeV)",
      numBins, histRangeMin_, histRangeMax_, numBins, histRangeMin_,
      histRangeMax_);
  allFedsenergyvsE1Hist_ = new TH2F(
      "energyvsE1_AllClusters",
      "energyvsE1_AllClusters;Seed Energy (GeV);Energy(GeV)", numBins,
      histRangeMin_, histRangeMax_, numBins, histRangeMin_, histRangeMax_);
  allFedsTimingHist_ =
      new TH1F("timeForAllFeds",
               "timeForAllFeds;Relative Time (1 clock = 25ns)", 78, -7, 7);
  allFedsTimingVsFreqHist_ = new TH2F(
      "timeVsFreqAllEvent",
      "time Vs Freq All events;Relative Time (1 clock = 25ns);Event Number", 78,
      -7, 7, 2000, 0., 200000);
  allFedsTimingVsAmpHist_ = new TH2F(
      "timeVsAmpAllEvents",
      "time Vs Amp All Events;Relative Time (1 clock = 25ns);Amplitude (GeV)",
      78, -7, 7, numBins, histRangeMin_, histRangeMax_);
  allFedsFrequencyHist_ =
      new TH1F("FrequencyAllEvent", "Frequency for All events;Event Number",
               2000, 0., 200000);

  //--------Special Hists for times stamp
  // info----------------------------------------------
  allFedsFreqTimeHist_ =
      new TH1F("FrequencyAllEventsInTime",
               Form("Time of Cosmic Events; Time (s);Passing Event rate/%5g s",
                    timingBinWidth),
               numTimingBins_, 0., timingEndInSeconds);
  allFedsFreqTimeVsPhiHist_ =
      new TH2F("FrequencyAllEventsInTimeVsPhi",
               Form("Time of Cosmic Events vs iPhi; iPhi;Time (s)/%5g s",
                    timingBinWidth * 360.),
               360, 1., 361., numTimingBins_ / 360, 0., timingEndInSeconds);
  allFedsFreqTimeVsPhiTTHist_ = new TH2F(
      "FrequencyAllEventsInTimeVsTTPhi",
      Form("Time of Cosmic Events vs iPhi (TT bins); iPhi;Time (s)/%5g s",
           timingBinWidth * 72.),
      72, 1., 361., numTimingBins_ / 72, 0., timingEndInSeconds);
  allFedsFreqTimeVsEtaHist_ =
      new TH2F("FrequencyAllEventsInTimeVsEta",
               Form("Time of Cosmic Events vs iEta; Time (s)/%5g s; iEta",
                    timingBinWidth * 172.),
               numTimingBins_ / 172, 0., timingEndInSeconds, 172, -86., 86.);
  allFedsFreqTimeVsEtaTTHist_ = new TH2F(
      "FrequencyAllEventsInTimeVsTTEta",
      Form("Time of Cosmic Events vs Eta (TT bins);Time (s)/%5g s; iEta",
           timingBinWidth * 35.),
      numTimingBins_ / 35, 0., timingEndInSeconds, 35, ttEtaBins);
  //--------END Special Hists for times stamp
  // info------------------------------------------

  allFedsiPhiProfileHist_ = new TH1F(
      "iPhiProfileAllEvents", "iPhi Profile all events;i#phi", 360, 1., 361.);
  allFedsiEtaProfileHist_ = new TH1F(
      "iEtaProfileAllEvents", "iEta Profile all events;i#eta", 172, -86, 86);

  allOccupancy_ =
      new TH2F("OccupancyAllEvents", "Occupancy all events;i#phi;i#eta", 360,
               1., 361., 172, -86, 86);
  TrueOccupancy_ =
      new TH2F("TrueOccupancyAllEvents", "True Occupancy all events;#phi;#eta",
               360, -3.14159, 3.14159, 172, -1.5, 1.5);
  allOccupancyCoarse_ = new TH2F("OccupancyAllEventsCoarse",
                                 "Occupancy all events Coarse;i#phi;i#eta",
                                 360 / 5, 1, 361., 35, ttEtaBins);
  TrueOccupancyCoarse_ = new TH2F("TrueOccupancyAllEventsCoarse",
                                  "True Occupancy all events Coarse;#phi;#eta",
                                  360 / 5, -3.14159, 3.14159, 34, -1.5, 1.5);

  // single xtal cluster occupancy
  allOccupancySingleXtal_ = new TH2F(
      "OccupancySingleXtal", "Occupancy single xtal clusters;i#phi;i#eta", 360,
      1., 361., 172, -86, 86);
  energySingleXtalHist_ =
      new TH1F("energy_SingleXtalClusters",
               "Energy single xtal clusters;Cluster Energy (GeV)", numBins,
               histRangeMin_, 200.0);

  allFedsTimingPhiHist_ =
      new TH2F("timePhiAllFEDs",
               "time vs Phi for all FEDs (TT binning);i#phi;Relative Time (1 "
               "clock = 25ns)",
               72, 1, 361, 78, -7, 7);
  allFedsTimingPhiEbpHist_ =
      new TH2F("timePhiEBP",
               "time vs Phi for FEDs in EB+ (TT binning) ;i#phi;Relative Time "
               "(1 clock = 25ns)",
               72, 1, 361, 78, -7, 7);
  ...
  allFedsTimingPhiEtaHist_ =
      new TH3F("timePhiEtaAllFEDs",
               "(Phi,Eta,time) for all FEDs (SM,M "
               "binning);i#phi;i#eta;Relative Time (1 clock = 25ns)",
               18, modPhiBins, 9, modEtaBins, 78, timingBins);
  allFedsTimingTTHist_ =
      new TH3F("timeTTAllFEDs",
               "(Phi,Eta,time) for all FEDs (SM,TT "
               "binning);i#phi;i#eta;Relative Time (1 clock = 25ns)",
               72, ttPhiBins, 35, ttEtaBins, 78, timingBins);
  allFedsTimingLMHist_ = new TH2F("timeLMAllFEDs",
                                  "(LM,time) for all FEDs (SM,LM "
                                  "binning);LM;Relative Time (1 clock = 25ns)",
                                  92, 1, 92, 78, -7, 7);

  allFedsTimingEbpHist_ =
      new TH1F("timeEBP", "time for FEDs in EB+;Relative Time (1 clock = 25ns)",
               78, -7, 7);
  allFedsTimingEbmHist_ =
      new TH1F("timeEBM", "time for FEDs in EB-;Relative Time (1 clock = 25ns)",
               78, -7, 7);
  allFedsTimingEbpTopHist_ = new TH1F(
      "timeEBPTop", "time for FEDs in EB+ Top;Relative Time (1 clock = 25ns)",
      78, -7, 7);
  allFedsTimingEbmTopHist_ = new TH1F(
      "timeEBMTop", "time for FEDs in EB- Top;Relative Time (1 clock = 25ns)",
      78, -7, 7);
  allFedsTimingEbpBottomHist_ = new TH1F(
      "timeEBPBottom",
      "time for FEDs in EB+ Bottom;Relative Time (1 clock = 25ns)", 78, -7, 7);
  allFedsTimingEbmBottomHist_ = new TH1F(
      "timeEBMBottom",
      "time for FEDs in EB- Bottom;Relative Time (1 clock = 25ns)", 78, -7, 7);

  numberofCosmicsHist_ =
      new TH1F("numberofCosmicsPerEvent",
               "Number of cosmics per event;Number of Cosmics", 30, 0, 30);
  numberofCosmicsHistEB_ =
      new TH1F("numberofCosmicsPerEvent_EB",
               "Number of cosmics per event EB;Number of Cosmics", 30, 0, 30);

  numberofCosmicsWTrackHist_ =
      new TH1F("numberofCosmicsWTrackPerEvent",
  ...
  numberofGoodEvtFreq_ = new TH1F("frequencyOfGoodEvents",
                                  "Number of events with cosmic vs Event;Event "
                                  "Number;Number of Good Events/100 Events",
                                  2000, 0, 200000);

  numberofCrossedEcalIdsHist_ = new TH1F(
      "numberofCrossedEcalCosmicsPerEvent",
      "Number of crossed ECAL cosmics per event;Number of Crossed Cosmics", 10,
      0, 10);

  allOccupancyExclusiveECAL_ =
      new TH2F("OccupancyAllEvents_ExclusiveECAL",
               "Occupancy all events Exclusive ECAL ;i#phi;i#eta", 360, 1.,
               361., 172, -86, 86);
  allOccupancyCoarseExclusiveECAL_ =
      new TH2F("OccupancyAllEventsCoarse_ExclusiveECAL",
               "Occupancy all events Coarse Exclusive ECAL;i#phi;i#eta",
               360 / 5, ttPhiBins, 35, ttEtaBins);
  allOccupancyECAL_ = new TH2F("OccupancyAllEvents_ECAL",
                               "Occupancy all events ECAL;i#phi;i#eta", 360, 1.,
                               361., 172, -86, 86);
  allOccupancyCoarseECAL_ =
      new TH2F("OccupancyAllEventsCoarse_ECAL",
               "Occupancy all events Coarse ECAL;i#phi;i#eta", 360 / 5,
               ttPhiBins, 35, ttEtaBins);
  allFedsTimingHistECAL_ =
      new TH1F("timeForAllFeds_ECAL",
               "timeForAllFeds ECAL;Relative Time (1 clock = 25ns)", 78, -7, 7);
  allFedsTimingPhiEtaHistECAL_ =
      new TH3F("timePhiEtaAllFEDs_ECAL",
               "ECAL (Phi,Eta,time) for all FEDs (SM,M "
               "binning);i#phi;i#eta;Relative Time (1 clock = 25ns)",
               18, modPhiBins, 9, modEtaBins, 78, timingBins);
  allFedsTimingTTHistECAL_ =
      new TH3F("timeTTAllFEDs_ECAL",
               "ECAL (Phi,Eta,time) for all FEDs (SM,TT "
               "binning);i#phi;i#eta;Relative Time (1 clock = 25ns)",
               72, ttPhiBins, 35, ttEtaBins, 78, timingBins);
  allFedsTimingLMHistECAL_ =
      new TH2F("timeLMAllFEDs_ECAL",
               "ECAL (LM,time) for all FEDs (SM,LM binning);LM;Relative Time "
               "(1 clock = 25ns)",
               92, 1, 92, 78, -7, 7);

  allOccupancyExclusiveDT_ =
      new TH2F("OccupancyAllEvents_ExclusiveDT",
               "Occupancy all events Exclusive DT;i#phi;i#eta", 360, 1., 361.,
               172, -86, 86);
  allOccupancyCoarseExclusiveDT_ =
      new TH2F("OccupancyAllEventsCoarse_ExclusiveDT",
               "Occupancy all events Coarse Exclusive DT;i#phi;i#eta", 360 / 5,
               1, 361., 35, ttEtaBins);
  allOccupancyDT_ =
      new TH2F("OccupancyAllEvents_DT", "Occupancy all events DT;i#phi;i#eta",
               360, 1., 361., 172, -86, 86);
  allOccupancyCoarseDT_ = new TH2F("OccupancyAllEventsCoarse_DT",
                                   "Occupancy all events Coarse DT;i#phi;i#eta",
                                   360 / 5, 1, 361., 35, ttEtaBins);
  allFedsTimingHistDT_ =
      new TH1F("timeForAllFeds_DT",
               "timeForAllFeds DT;Relative Time (1 clock = 25ns)", 78, -7, 7);
  allFedsTimingPhiEtaHistDT_ =
      new TH3F("timePhiEtaAllFEDs_DT",
               "DT (Phi,Eta,time) for all FEDs (SM,M "
               "binning);i#phi;i#eta;Relative Time (1 clock = 25ns)",
               18, modPhiBins, 9, modEtaBins, 78, timingBins);
  allFedsTimingTTHistDT_ =
      new TH3F("timeTTAllFEDs_DT",
               "DT (Phi,Eta,time) for all FEDs (SM,TT "
               "binning);i#phi;i#eta;Relative Time (1 clock = 25ns)",
               72, ttPhiBins, 35, ttEtaBins, 78, timingBins);
  allFedsTimingLMHistDT_ =
      new TH2F("timeLMAllFEDs_DT",
               "DT (LM,time) for all FEDs (SM,LM binning);LM;Relative Time (1 "
               "clock = 25ns)",
               92, 1, 92, 78, -7, 7);

  allOccupancyExclusiveRPC_ =
      new TH2F("OccupancyAllEvents_ExclusiveRPC",
               "Occupancy all events Exclusive RPC;i#phi;i#eta", 360, 1., 361.,
               172, -86, 86);
  allOccupancyCoarseExclusiveRPC_ =
      new TH2F("OccupancyAllEventsCoarse_ExclusiveRPC",
               "Occupancy all events Coarse Exclusive RPC;i#phi;i#eta", 360 / 5,
               1, 361., 35, ttEtaBins);
  allOccupancyRPC_ =
      new TH2F("OccupancyAllEvents_RPC", "Occupancy all events RPC;i#phi;i#eta",
               360, 1., 361., 172, -86, 86);
  allOccupancyCoarseRPC_ =
      new TH2F("OccupancyAllEventsCoarse_RPC",
               "Occupancy all events Coarse RPC;i#phi;i#eta", 360 / 5, 1, 361.,
               35, ttEtaBins);
  allFedsTimingHistRPC_ =
      new TH1F("timeForAllFeds_RPC",
               "timeForAllFeds RPC;Relative Time (1 clock = 25ns)", 78, -7, 7);
  allFedsTimingPhiEtaHistRPC_ =
      new TH3F("timePhiEtaAllFEDs_RPC",
               "RPC (Phi,Eta,time) for all FEDs (SM,M "
               "binning);i#phi;i#eta;Relative Time (1 clock = 25ns)",
               18, modPhiBins, 9, modEtaBins, 78, timingBins);
  allFedsTimingTTHistRPC_ =
      new TH3F("timeTTAllFEDs_RPC",
               "RPC (Phi,Eta,time) for all FEDs (SM,TT "
               "binning);i#phi;i#eta;Relative Time (1 clock = 25ns)",
               72, ttPhiBins, 35, ttEtaBins, 78, timingBins);
  allFedsTimingLMHistRPC_ =
      new TH2F("timeLMAllFEDs_RPC",
               "RPC (LM,time) for all FEDs (SM,LM binning);LM;Relative Time (1 "
               "clock = 25ns)",
               92, 1, 92, 78, -7, 7);

  allOccupancyExclusiveCSC_ =
      new TH2F("OccupancyAllEvents_ExclusiveCSC",
               "Occupancy all events Exclusive CSC;i#phi;i#eta", 360, 1., 361.,
               172, -86, 86);
  allOccupancyCoarseExclusiveCSC_ =
      new TH2F("OccupancyAllEventsCoarse_ExclusiveCSC",
               "Occupancy all events Coarse Exclusive CSC;i#phi;i#eta", 360 / 5,
               1, 361., 35, ttEtaBins);
  allOccupancyCSC_ =
      new TH2F("OccupancyAllEvents_CSC", "Occupancy all events CSC;i#phi;i#eta",
               360, 1., 361., 172, -86, 86);
  allOccupancyCoarseCSC_ =
      new TH2F("OccupancyAllEventsCoarse_CSC",
               "Occupancy all events Coarse CSC;i#phi;i#eta", 360 / 5, 1, 361.,
               35, ttEtaBins);
  allFedsTimingHistCSC_ =
      new TH1F("timeForAllFeds_CSC",
               "timeForAllFeds CSC;Relative Time (1 clock = 25ns)", 78, -7, 7);
  allFedsTimingPhiEtaHistCSC_ =
      new TH3F("timePhiEtaAllFEDs_CSC",
               "CSC (Phi,Eta,time) for all FEDs (SM,M "
               "binning);i#phi;i#eta;Relative Time (1 clock = 25ns)",
               18, modPhiBins, 9, modEtaBins, 78, timingBins);
  allFedsTimingTTHistCSC_ =
      new TH3F("timeTTAllFEDs_CSC",
               "CSC (Phi,Eta,time) for all FEDs (SM,TT "
               "binning);i#phi;i#eta;Relative Time (1 clock = 25ns)",
               72, ttPhiBins, 35, ttEtaBins, 78, timingBins);
  allFedsTimingLMHistCSC_ =
      new TH2F("timeLMAllFEDs_CSC",
               "CSC (LM,time) for all FEDs (SM,LM binning);LM;Relative Time (1 "
               "clock = 25ns)",
               92, 1, 62, 78, -7, 7);

  allOccupancyExclusiveHCAL_ =
      new TH2F("OccupancyAllEvents_ExclusiveHCAL",
               "Occupancy all events Exclusive HCAL;i#phi;i#eta", 360, 1., 361.,
               172, -86, 86);
  allOccupancyCoarseExclusiveHCAL_ =
      new TH2F("OccupancyAllEventsCoarse_ExclusiveHCAL",
               "Occupancy all events Coarse Exclusive HCAL;i#phi;i#eta",
               360 / 5, 1, 361., 35, ttEtaBins);
  allOccupancyHCAL_ = new TH2F("OccupancyAllEvents_HCAL",
                               "Occupancy all events HCAL;i#phi;i#eta", 360, 1.,
                               361., 172, -86, 86);
  allOccupancyCoarseHCAL_ =
      new TH2F("OccupancyAllEventsCoarse_HCAL",
               "Occupancy all events Coarse HCAL;i#phi;i#eta", 360 / 5, 1, 361.,
               35, ttEtaBins);
  allFedsTimingHistHCAL_ =
      new TH1F("timeForAllFeds_HCAL",
               "timeForAllFeds HCAL;Relative Time (1 clock = 25ns)", 78, -7, 7);
  allFedsTimingPhiEtaHistHCAL_ =
      new TH3F("timePhiEtaAllFEDs_HCAL",
               "HCAL (Phi,Eta,time) for all FEDs (SM,M "
               "binning);i#phi;i#eta;Relative Time (1 clock = 25ns)",
               18, modPhiBins, 9, modEtaBins, 78, timingBins);
  allFedsTimingTTHistHCAL_ =
      new TH3F("timeTTAllFEDs_HCAL",
               "HCAL (Phi,Eta,time) for all FEDs (SM,TT "
               "binning);i#phi;i#eta;Relative Time (1 clock = 25ns)",
               72, ttPhiBins, 35, ttEtaBins, 78, timingBins);
  allFedsTimingLMHistHCAL_ =
      new TH2F("timeLMAllFEDs_HCAL",
               "HCAL (LM,time) for all FEDs (SM,LM binning);LM;Relative Time "
               "(1 clock = 25ns)",
               92, 1, 92, 78, -7, 7);

  TrueBCOccupancy_ = new TH2F("BCTrueOccupancyAllEvents",
                              "True SB Occupancy all events;#phi;#eta", 360,
                              -3.14159, 3.14159, 172, -1.5, 1.5);
  TrueBCOccupancyCoarse_ =
      new TH2F("BCTrueOccupancyAllEventsCoarse",
               "True BC Occupancy all events Coarse;#phi;#eta", 360 / 5,
               -3.14159, 3.14159, 34, -1.5, 1.5);

  numberofBCinSC_ =
      new TH1F("numberofBCinSC",
               "Number of Basic Clusters in Super Cluster;Num Basic Clusters",
               20, 0, 20); // SC
  numberofBCinSCphi_ = new TH2F(
      "numberofBCinSCphi",
      "Number of Basic Clusters in Super Cluster;phi;Num Basic Clusters",
      360 / 5, -3.14159, 3.14159, 20, 0, 20); // SC

  allFedsTimingHistEcalMuon_ = new TH1F(
  ...
  triggerExclusiveHist_ = new TH1F(
      "triggerExclusiveHist", "Trigger Number (Mutually Exclusive)", 5, 0, 5);
  ...
  ratioAssocTracksHist_ = new TH1F(
      "ratioAssocTracks", "num assoc. tracks/tracks through Ecal", 11, 0, 1.1);
  ratioAssocClustersHist_ = new TH1F(
      "ratioAssocClusters", "num assoc. clusters/total clusters", 11, 0, 1.1);
  trackAssoc_muonsEcal_ =
      new TH2F("trackAssoc_muonsEcal", "Map of muon hits in Ecal", 360, 1.,
               361., 172, -86, 86); // 360, 0 , 360, 170,-85 ,85);
  deltaEtaDeltaPhiHist_ =
      new TH2F("deltaEtaDeltaPhi", "Delta ieta vs. delta iphi", 170, -85.5,
               84.5, 720, -360.5, 359.5);
  seedTrackEtaHist_ = new TH2F("seedTrackEta", "track ieta vs. seed ieta", 170,
                               -85.5, 84.5, 170, -85.5, 84.5);
  seedTrackPhiHist_ = new TH2F("seedTrackPhi", "track iphi vs. seed iphi", 720,
                               -360.5, 359.5, 720, -360.5, 359.5);

  dccEventVsBxHist_ = new TH2F("dccEventVsBx", "DCC Runtype vs. bunch crossing",
                               3600, 0, 3600, 24, 0, 24);
  dccBXErrorByFEDHist_ =
      new TH1F("dccBXErrorByFED", "Incorrect BX number by FED", 54, 601, 655);
  dccOrbitErrorByFEDHist_ = new TH1F(
      "dccOrbitErrorByFED", "Incorrect orbit number by FED", 54, 601, 655);
  dccRuntypeErrorByFEDHist_ = new TH1F(
      "dccRuntypeErrorByFED", "Incorrect DCC Runtype by FED", 54, 601, 655);
  dccRuntypeHist_ = new TH1F("dccRuntype", "DCC Runtype frequency", 24, 0, 24);
  dccErrorVsBxHist_ =
      new TH2F("dccErrorVsBX", "DCC Errors vs. BX", 3600, 0, 3600, 3, 0, 3);

  hcalEnergy_HBHE_ =
      new TH1F("hcalEnergy_HBHE", "RecHit Energy HBHE", 440, -10, 100);
  ...
  hcalHEHBecalEB_ =
      new TH2F("hcalHEHBecalEB", "HCAL HBHE RecHit energy vs ECAL EB energy",
               numBins, histRangeMin_, 300.0, 40, -5, 5);

  NumXtalsInClusterHist_ =
      new TH1F("NumXtalsInClusterAllHist",
               "Number of Xtals in Cluster;NumXtals", 150, 0, 150);
  numxtalsVsEnergy_ =
      new TH2F("NumXtalsVsEnergy",
               "Number of Xtals in Cluster vs Energy;Energy (GeV);Number of "
               "Xtals in Cluster",
               numBins, histRangeMin_, histRangeMax_, 150, 0, 150);
  numxtalsVsHighEnergy_ =
      new TH2F("NumXtalsVsHighEnergy",
               "Number of Xtals in Cluster vs Energy;Energy (GeV);Number of "
               "Xtals in Cluster",
               numBins, histRangeMin_, 200., 150, 0, 150);

  // high energy analysis
  allOccupancyHighEnergy_ = new TH2F("OccupancyHighEnergyEvents",
                                     "Occupancy high energy events;i#phi;i#eta",
                                     360, 1., 361., 172, -86, 86);
  allOccupancyHighEnergyCoarse_ =
      new TH2F("OccupancyHighEnergyEventsCoarse",
               "Occupancy high energy events Coarse;i#phi;i#eta", 72, ttPhiBins,
               35, ttEtaBins);
  allFedsOccupancyHighEnergyHist_ = new TH3F(
      "OccupancyHighEnergyEvents3D",
      "(Phi,Eta,energy) for all high energy events;i#phi;i#eta;energy (GeV)",
      18, modPhiBins, 9, modEtaBins, 10, highEBins);
  allFedsNumXtalsInClusterHist_ =
      new TH1F("NumActiveXtalsInClusterAllHist",
               "Number of active Xtals in Cluster;NumXtals", 100, 0, 100);

  HighEnergy_NumXtal =
      new TH1F("HighEnergy_NumXtal",
               "Num crystals in high E clusters;num crystals", 150, 0, 150);
  HighEnergy_NumXtalFedId =
      new TH2F("HighEnergy_NumXtalFedId",
               "Num crystals in cluster vs FedId;FedId;num crystals", 36, 610.,
               645., 150, 0, 150);
  HighEnergy_NumXtaliphi =
      new TH2F("HighEnergy_NumXtaliphi",
               "Num crystals in cluster vs iphi;i#phi;num crystals", 360, 1.,
               361., 150, 0, 150);
  HighEnergy_energy3D = new TH3F(
      "HighEnergy_energy3D",
      "(Phi,Eta,energy) for all high energy events;i#phi;i#eta;energy (GeV)",
      72, ttPhiBins, 35, ttEtaBins, 10, highEBins);

  HighEnergy_energyNumXtal = new TH2F(
      "HighEnergy_energyNumXtal",
      "Energy in cluster vs Num crystals in cluster;num crystals;energy", 150,
      0, 150, 200, 0., 200.);

  HighEnergy_bestSeed =
      new TH1F("HighEnergy_bestSeedEnergy", "BestSeed Energy from TrackAss",
               200, 0., 200.);
  HighEnergy_bestSeedOccupancy =
      new TH2F("HighEnergy_bestSeedOccupancy",
               "Occupancy HighEn events from TrackAss;i#phi;i#eta", 360, 1.,
               361., 172, -86, 86);

  HighEnergy_numClusHighEn =
      new TH1F("HighEnergy_numClusHighEn", "Num High Energy Clusters", 7, 0, 7);
  HighEnergy_ratioClusters =
      new TH1F("HighEnergy_ratioClusters",
               "Num High Energy Clusters/Num tot Clusters", 100, 0., 1.1);

  HighEnergy_numRecoTrackBarrel = new TH1F("HighEnergy_numRecoTracksBarrel",
                                           "Num BarrelRecoTracks", 10, 0, 10);
  HighEnergy_TracksAngle =
      new TH1F("HighEnergy_TracksAngle", "Angle between tracks", 720, 0., 180.);
  HighEnergy_TracksAngleTopBottom =
      new TH1F("HighEnergy_TopBottomTracksAngle",
               "Angle between top-bottom tracks", 720, 0., 180.);

  HighEnergy_2GeV_occuCoarse =
      new TH2F("HighEnergy_occu2GeV_Coarse",
               "Occupancy high energy events with more than 2 GeV;i#phi;i#eta",
               72, ttPhiBins, 35, ttEtaBins);
  HighEnergy_2GeV_occu3D =
      new TH3F("HighEnergy_2GeV_energy3D",
               "(iphi,ieta,energy) for all high energy events w > 10 "
               "GeV;i#phi;i#eta;energy (GeV)",
               72, ttPhiBins, 35, ttEtaBins, 10, highEBins);
  HighEnergy_100GeV_occuCoarse = new TH2F(
      "HighEnergy_occu100GeV_Coarse",
      "Occupancy high energy events with more than 100 GeV;i#phi;i#eta", 72,
      ttPhiBins, 35, ttEtaBins);
  HighEnergy_100GeV_occu3D =
      new TH3F("HighEnergy_100GeV_energy3D",
               "(iphi,ieta,energy) for all high energy events more than 100 "
               "GeV;i#phi;i#eta;energy (GeV)",
               72, ttPhiBins, 35, ttEtaBins, 10, highEBins);
  HighEnergy_0tracks_occu3D =
      new TH3F("HighEnergy_0Tracks_energy3D",
               "(iphi,ieta,energy) for all events with 0 "
               "tracks;i#phi;i#eta;energy (GeV)",
               72, ttPhiBins, 35, ttEtaBins, 10, highEBins);
  HighEnergy_1tracks_occu3D =
      new TH3F("HighEnergy_1Tracks_energy3D",
               "(iphi,ieta,energy) for all events with 1 "
               "tracks;i#phi;i#eta;energy (GeV)",
               72, ttPhiBins, 35, ttEtaBins, 10, highEBins);
  HighEnergy_2tracks_occu3D =
      new TH3F("HighEnergy_2Tracks_energy3D",
               "(iphi,ieta,energy) for all events with 2 "
               "tracks;i#phi;i#eta;energy (GeV)",
               72, ttPhiBins, 35, ttEtaBins, 10, highEBins);
  HighEnergy_0tracks_occu3DXtal =
      new TH3F("HighEnergy_0Tracks_energy3DXtal",
               "(iphi,ieta,energy) for all events with 0 "
               "tracks;i#phi;i#eta;energy (GeV)",
               360, 1., 361., 172, -86, 86, 200, 0., 200.);
  HighEnergy_1tracks_occu3DXtal =
      new TH3F("HighEnergy_1Tracks_energy3DXtal",
               "(iphi,ieta,energy) for all events with 1 "
               "tracks;i#phi;i#eta;energy (GeV)",
               360, 1., 361., 172, -86, 86, 200, 0., 200.);
  HighEnergy_2tracks_occu3DXtal =
      new TH3F("HighEnergy_2Tracks_energy3DXtal",
               "(iphi,ieta,energy) for all events with 2 "
               "tracks;i#phi;i#eta;energy (GeV)",
               360, 1., 361., 172, -86, 86, 200, 0., 200.);
  ...
  // EE-
  EEM_FedsSeedEnergyHist_ = new TH1F(
      "SeedEnergyAllFEDs", "Seed Energy for EEM Feds; Seed Energy (GeV)", 200,
      histRangeMin_, 10.0);

  EEM_AllOccupancyCoarse_ =
      new TH2F("OccupancyAllEventsCoarse",
               "Occupancy all events Coarse EEM;ix;iy", 20, 0, 100, 20, 0, 100);
  EEM_AllOccupancy_ =
      new TH2F("OccupancyAllEvents", "Occupancy all events EEM;ix;iy", 100, 0,
               100, 100, 0, 100);
  EEM_FedsenergyHist_ = new TH1F("energy_AllClusters",
                                 "energy_AllClusters_EEM;Cluster Energy (GeV)",
                                 numBins, histRangeMin_, 10.0);
  EEM_FedsenergyHighHist_ =
      new TH1F("energyHigh_AllClusters",
               "energyHigh_AllClusters in EEM;Cluster Energy (GeV)", numBins,
               histRangeMin_, 200.0);
  EEM_FedsenergyOnlyHighHist_ =
      new TH1F("energyHigh_HighEnergyClusters",
               "energy of High Energy Clusters in EEM;Cluster Energy (GeV)",
               numBins, histRangeMin_, 200.0);
  EEM_FedsE2Hist_ = new TH1F(
      "E2_AllClusters", "E2_AllClusters_EEM;Seed+highest neighbor energy (GeV)",
      numBins, histRangeMin_, 10.0);
  EEM_FedsE2vsE1Hist_ =
      new TH2F("E2vsE1_AllClusters",
               "E2vsE1_AllClusters_EEM;Seed Energy (GeV);Seed+highest neighbor "
               "energy (GeV)",
               numBins, histRangeMin_, 10.0, numBins, histRangeMin_, 10.0);
  EEM_FedsenergyvsE1Hist_ =
      new TH2F("energyvsE1_AllClusters",
  ...
  EEM_FedsTimingVsAmpHist_ = new TH2F("timeVsAmpAllEvents",
                                      "time Vs Amp All Events EEM;Relative "
                                      "Time (1 clock = 25ns);Amplitude (GeV)",
                                      78, -7, 7, numBins, histRangeMin_, 10.0);
  EEM_FedsTimingTTHist_ = new TH3F("timeTTAllFEDs",
                                   "(ix,iy,time) for all FEDs (SM,TT binning) "
                                   "EEM;ix;iy;Relative Time (1 clock = 25ns)",
                                   20, 0, 100, 20, 0, 100, 78, -7, 7);

  EEM_OccupancySingleXtal_ = new TH2F(
      "OccupancySingleXtal", "Occupancy single xtal clusters EEM;ix;iy", 100, 0,
      100, 100, 0, 100);
  EEM_energySingleXtalHist_ =
      new TH1F("energy_SingleXtalClusters",
               "Energy single xtal clusters EEM;Cluster Energy (GeV)", numBins,
               histRangeMin_, 200.0);

  EEM_OccupancyExclusiveECAL_ =
      new TH2F("OccupancyAllEvents_ExclusiveECAL",
               "Occupancy all events Exclusive ECAL  EEM;ix;iy", 100, 0, 100,
               100, 0, 100);
  EEM_OccupancyCoarseExclusiveECAL_ =
      new TH2F("OccupancyAllEventsCoarse_ExclusiveECAL",
               "Occupancy all events Coarse Exclusive ECAL EEM;ix;iy", 20, 0,
               100, 20, 0, 100);
  EEM_OccupancyECAL_ =
      new TH2F("OccupancyAllEvents_ECAL", "Occupancy all events ECAL EEM;ix;iy",
               100, 0, 100, 100, 0, 100);
  EEM_OccupancyCoarseECAL_ = new TH2F(
      "OccupancyAllEventsCoarse_ECAL",
      "Occupancy all events Coarse ECAL EEM;ix;iy", 20, 0, 100, 20, 0, 100);
  EEM_FedsTimingHistECAL_ = new TH1F(
      "timeForAllFeds_ECAL",
      "timeForAllFeds ECAL EEM;Relative Time (1 clock = 25ns)", 78, -7, 7);
  EEM_FedsTimingTTHistECAL_ =
      new TH3F("timeTTAllFEDs_ECAL",
               "(ix,iy,time) for all FEDs (SM,TT binning) ECAL "
               "EEM;ix;iy;Relative Time (1 clock = 25ns)",
               20, 0, 100, 20, 0, 100, 78, -7, 7);
  ...
  EEM_OccupancyCoarseExclusiveDT_ =
      new TH2F("OccupancyAllEventsCoarse_ExclusiveDT",
               "Occupancy all events Coarse Exclusive DT EEM;ix;iy", 20, 0, 100,
               20, 0, 100);
  EEM_OccupancyDT_ =
      new TH2F("OccupancyAllEvents_DT", "Occupancy all events DT EEM;ix;iy",
               100, 0, 100, 100, 0, 100);
  EEM_OccupancyCoarseDT_ = new TH2F("OccupancyAllEventsCoarse_DT",
                                    "Occupancy all events Coarse DT EEM;ix;iy",
                                    20, 0, 100, 20, 0, 100);
  EEM_FedsTimingHistDT_ = new TH1F(
      "timeForAllFeds_DT",
      "timeForAllFeds DT EEM;Relative Time (1 clock = 25ns)", 78, -7, 7);
  EEM_FedsTimingTTHistDT_ =
      new TH3F("timeTTAllFEDs_DT",
               "(ix,iy,time) for all FEDs (SM,TT binning) DT "
               "EEM;ix;iy;Relative Time (1 clock = 25ns)",
               20, 0, 100, 20, 0, 100, 78, -7, 7);
  ...
  EEM_OccupancyCoarseExclusiveRPC_ =
      new TH2F("OccupancyAllEventsCoarse_ExclusiveRPC",
               "Occupancy all events Coarse Exclusive RPC EEM;ix;iy", 20, 0,
               100, 20, 0, 100);
  EEM_OccupancyRPC_ =
      new TH2F("OccupancyAllEvents_RPC", "Occupancy all events RPC EEM;ix;iy",
               100, 0, 100, 100, 0, 100);
  EEM_OccupancyCoarseRPC_ = new TH2F(
      "OccupancyAllEventsCoarse_RPC",
      "Occupancy all events Coarse RPC EEM;ix;iy", 20, 0, 100, 20, 0, 100);
  EEM_FedsTimingHistRPC_ = new TH1F(
      "timeForAllFeds_RPC",
      "timeForAllFeds RPC EEM;Relative Time (1 clock = 25ns)", 78, -7, 7);
  EEM_FedsTimingTTHistRPC_ =
      new TH3F("timeTTAllFEDs_RPC",
               "(ix,iy,time) for all FEDs (SM,TT binning) RPC "
               "EEM;ix;iy;Relative Time (1 clock = 25ns)",
               20, 0, 100, 20, 0, 100, 78, -7, 7);
  ...
  EEM_OccupancyCoarseExclusiveCSC_ =
      new TH2F("OccupancyAllEventsCoarse_ExclusiveCSC",
               "Occupancy all events Coarse Exclusive CSC EEM;ix;iy", 20, 0,
               100, 20, 0, 100);
  EEM_OccupancyCSC_ =
      new TH2F("OccupancyAllEvents_CSC", "Occupancy all events CSC EEM;ix;iy",
               100, 0, 100, 100, 0, 100);
  EEM_OccupancyCoarseCSC_ = new TH2F(
      "OccupancyAllEventsCoarse_CSC",
      "Occupancy all events Coarse CSC EEM;ix;iy", 20, 0, 100, 20, 0, 100);
  EEM_FedsTimingHistCSC_ = new TH1F(
      "timeForAllFeds_CSC",
      "timeForAllFeds CSC EEM;Relative Time (1 clock = 25ns)", 78, -7, 7);
  EEM_FedsTimingTTHistCSC_ =
      new TH3F("timeTTAllFEDs_CSC",
               "(ix,iy,time) for all FEDs (SM,TT binning) CSC "
               "EEM;ix;iy;Relative Time (1 clock = 25ns)",
               20, 0, 100, 20, 0, 100, 78, -7, 7);

  EEM_OccupancyExclusiveHCAL_ =
      new TH2F("OccupancyAllEvents_ExclusiveHCAL",
               "Occupancy all events Exclusive HCAL EEM;ix;iy", 100, 0, 100,
               100, 0, 100);
  EEM_OccupancyCoarseExclusiveHCAL_ =
      new TH2F("OccupancyAllEventsCoarse_ExclusiveHCAL",
               "Occupancy all events Coarse Exclusive HCAL EEM;ix;iy", 20, 0,
               100, 20, 0, 100);
  EEM_OccupancyHCAL_ =
      new TH2F("OccupancyAllEvents_HCAL", "Occupancy all events HCAL EEM;ix;iy",
               100, 0, 100, 100, 0, 100);
  EEM_OccupancyCoarseHCAL_ = new TH2F(
      "OccupancyAllEventsCoarse_HCAL",
      "Occupancy all events Coarse HCAL EEM;ix;iy", 20, 0, 100, 20, 0, 100);
  EEM_FedsTimingHistHCAL_ = new TH1F(
      "timeForAllFeds_HCAL",
      "timeForAllFeds HCAL EEM;Relative Time (1 clock = 25ns)", 78, -7, 7);
  EEM_FedsTimingTTHistHCAL_ =
      new TH3F("timeTTAllFEDs_HCAL",
               "(ix,iy,time) for all FEDs (SM,TT binning) HCAL "
               "EEM;ix;iy;Relative Time (1 clock = 25ns)",
               20, 0, 100, 20, 0, 100, 78, -7, 7);

  EEM_numberofBCinSC_ = new TH1F(
      "numberofBCinSC",
      "Number of Basic Clusters in Super Cluster EEM;Num Basic Clusters", 20, 0,
      20);
  ...
  EEM_triggerExclusiveHist_ =
      new TH1F("triggerExclusiveHist",
               "Trigger Number (Mutually Exclusive) EEM", 5, 0, 5);
  triggerExclusiveHist_->GetXaxis()->SetBinLabel(1, "ECAL");
  ...
  EEM_numxtalsVsEnergy_ =
      new TH2F("NumXtalsVsEnergy",
               "Number of Xtals in Cluster vs Energy EEM;Energy (GeV);Number "
               "of Xtals in Cluster",
               numBins, histRangeMin_, 10.0, 150, 0, 150);
  ...
  EEM_OccupancyHighEnergy_ = new TH2F("OccupancyHighEnergyEvents",
                                      "Occupancy high energy events EEM;ix;iy",
                                      100, 0, 100, 100, 0, 100);
  ...
  // EE+
  EEP_FedsSeedEnergyHist_ = new TH1F(
      "SeedEnergyAllFEDs", "Seed Energy for EEP Feds; Seed Energy (GeV)", 200,
      histRangeMin_, 10.0);
  ...
  EEP_AllOccupancy_ =
      new TH2F("OccupancyAllEvents", "Occupancy all events EEP;ix;iy", 100, 0,
               100, 100, 0, 100);
  EEP_FedsenergyHist_ = new TH1F("energy_AllClusters",
                                 "energy_AllClusters_EEP;Cluster Energy (GeV)",
                                 numBins, histRangeMin_, 10.0);
  EEP_FedsenergyHighHist_ =
      new TH1F("energyHigh_AllClusters",
               "energyHigh_AllClusters in EEP;Cluster Energy (GeV)", numBins,
               histRangeMin_, 200.0);
  EEP_FedsenergyOnlyHighHist_ =
      new TH1F("energyHigh_HighEnergyClusters",
               "energy of High Energy Clusters in EEP;Cluster Energy (GeV)",
               numBins, histRangeMin_, 200.0);
  EEP_FedsE2Hist_ = new TH1F(
      "E2_AllClusters", "E2_AllClusters_EEP;Seed+highest neighbor energy (GeV)",
      numBins, histRangeMin_, 10.0);
  EEP_FedsE2vsE1Hist_ =
      new TH2F("E2vsE1_AllClusters",
               "E2vsE1_AllClusters_EEP;Seed Energy (GeV);Seed+highest neighbor "
               "energy (GeV)",
               numBins, histRangeMin_, 10.0, numBins, histRangeMin_, 10.0);
  EEP_FedsenergyvsE1Hist_ =
      new TH2F("energyvsE1_AllClusters",
  ...
  EEP_FedsTimingVsAmpHist_ = new TH2F("timeVsAmpAllEvents",
                                      "time Vs Amp All Events EEP;Relative "
                                      "Time (1 clock = 25ns);Amplitude (GeV)",
                                      78, -7, 7, numBins, histRangeMin_, 10.0);
  EEP_FedsTimingTTHist_ = new TH3F("timeTTAllFEDs",
                                   "(ix,iy,time) for all FEDs (SM,TT binning) "
                                   "EEP;ix;iy;Relative Time (1 clock = 25ns)",
                                   20, 0, 100, 20, 0, 100, 78, -7, 7);

  EEP_OccupancySingleXtal_ = new TH2F(
      "OccupancySingleXtal", "Occupancy single xtal clusters EEP;ix;iy", 100, 0,
      100, 100, 0, 100);
  EEP_energySingleXtalHist_ =
      new TH1F("energy_SingleXtalClusters",
               "Energy single xtal clusters EEP;Cluster Energy (GeV)", numBins,
               histRangeMin_, 200.0);

  EEP_OccupancyExclusiveECAL_ =
      new TH2F("OccupancyAllEvents_ExclusiveECAL",
               "Occupancy all events Exclusive ECAL  EEP;ix;iy", 100, 0, 100,
               100, 0, 100);
  EEP_OccupancyCoarseExclusiveECAL_ =
      new TH2F("OccupancyAllEventsCoarse_ExclusiveECAL",
               "Occupancy all events Coarse Exclusive ECAL EEP;ix;iy", 20, 0,
               100, 20, 0, 100);
  EEP_OccupancyECAL_ =
      new TH2F("OccupancyAllEvents_ECAL", "Occupancy all events ECAL EEP;ix;iy",
               100, 0, 100, 100, 0, 100);
  EEP_OccupancyCoarseECAL_ = new TH2F(
      "OccupancyAllEventsCoarse_ECAL",
      "Occupancy all events Coarse ECAL EEP;ix;iy", 20, 0, 100, 20, 0, 100);
  EEP_FedsTimingHistECAL_ = new TH1F(
      "timeForAllFeds_ECAL",
  ...
  EEP_OccupancyCoarseExclusiveDT_ =
      new TH2F("OccupancyAllEventsCoarse_ExclusiveDT",
               "Occupancy all events Coarse Exclusive DT EEP;ix;iy", 20, 0, 100,
               20, 0, 100);
  EEP_OccupancyDT_ =
      new TH2F("OccupancyAllEvents_DT", "Occupancy all events DT EEP;ix;iy",
               100, 0, 100, 100, 0, 100);
  EEP_OccupancyCoarseDT_ = new TH2F("OccupancyAllEventsCoarse_DT",
                                    "Occupancy all events Coarse DT EEP;ix;iy",
                                    20, 0, 100, 20, 0, 100);
  EEP_FedsTimingHistDT_ = new TH1F(
      "timeForAllFeds_DT",
  ...
  EEP_OccupancyCoarseExclusiveRPC_ =
      new TH2F("OccupancyAllEventsCoarse_ExclusiveRPC",
               "Occupancy all events Coarse Exclusive RPC EEP;ix;iy", 20, 0,
               100, 20, 0, 100);
  EEP_OccupancyRPC_ =
      new TH2F("OccupancyAllEvents_RPC", "Occupancy all events RPC EEP;ix;iy",
               100, 0, 100, 100, 0, 100);
  EEP_OccupancyCoarseRPC_ = new TH2F(
      "OccupancyAllEventsCoarse_RPC",
      "Occupancy all events Coarse RPC EEP;ix;iy", 20, 0, 100, 20, 0, 100);
  EEP_FedsTimingHistRPC_ = new TH1F(
      "timeForAllFeds_RPC",
  ...
  EEP_OccupancyCoarseExclusiveCSC_ =
      new TH2F("OccupancyAllEventsCoarse_ExclusiveCSC",
               "Occupancy all events Coarse Exclusive CSC EEP;ix;iy", 20, 0,
               100, 20, 0, 100);
  EEP_OccupancyCSC_ =
      new TH2F("OccupancyAllEvents_CSC", "Occupancy all events CSC EEP;ix;iy",
               100, 0, 100, 100, 0, 100);
  EEP_OccupancyCoarseCSC_ = new TH2F(
      "OccupancyAllEventsCoarse_CSC",
      "Occupancy all events Coarse CSC EEP;ix;iy", 20, 0, 100, 20, 0, 100);
  EEP_FedsTimingHistCSC_ = new TH1F(
      "timeForAllFeds_CSC",
  ...
  EEP_OccupancyExclusiveHCAL_ =
      new TH2F("OccupancyAllEvents_ExclusiveHCAL",
               "Occupancy all events Exclusive HCAL EEP;ix;iy", 100, 0, 100,
               100, 0, 100);
  EEP_OccupancyCoarseExclusiveHCAL_ =
      new TH2F("OccupancyAllEventsCoarse_ExclusiveHCAL",
               "Occupancy all events Coarse Exclusive HCAL EEP;ix;iy", 20, 0,
               100, 20, 0, 100);
  EEP_OccupancyHCAL_ =
      new TH2F("OccupancyAllEvents_HCAL", "Occupancy all events HCAL EEP;ix;iy",
               100, 0, 100, 100, 0, 100);
  EEP_OccupancyCoarseHCAL_ = new TH2F(
      "OccupancyAllEventsCoarse_HCAL",
      "Occupancy all events Coarse HCAL EEP;ix;iy", 20, 0, 100, 20, 0, 100);
  EEP_FedsTimingHistHCAL_ = new TH1F(
      "timeForAllFeds_HCAL",
  ...
  EEP_numberofBCinSC_ = new TH1F(
      "numberofBCinSC",
      "Number of Basic Clusters in Super Cluster EEP;Num Basic Clusters", 20, 0,
      20);
  ...
  EEP_triggerExclusiveHist_ =
      new TH1F("triggerExclusiveHist",
               "Trigger Number (Mutually Exclusive) EEP", 5, 0, 5);
  ...
  EEP_numxtalsVsEnergy_ =
      new TH2F("NumXtalsVsEnergy",
               "Number of Xtals in Cluster vs Energy EEP;Energy (GeV);Number "
               "of Xtals in Cluster",
               numBins, histRangeMin_, 10.0, 150, 0, 150);
  ...
  EEP_OccupancyHighEnergy_ = new TH2F("OccupancyHighEnergyEvents",
                                      "Occupancy high energy events EEP;ix;iy",
                                      100, 0, 100, 100, 0, 100);
...
void EcalCosmicsHists::endJob() {
  ...
  LogWarning("EcalCosmicsHists")
      << "---> Number of cosmic events: " << cosmicCounter_ << " in "
      << naiveEvtNum_ << " events.";
  LogWarning("EcalCosmicsHists")
      << "---> Number of EB cosmic events: " << cosmicCounterEB_ << " in "
      << naiveEvtNum_ << " events.";
  LogWarning("EcalCosmicsHists")
      << "---> Number of EE- cosmic events: " << cosmicCounterEEM_ << " in "
      << naiveEvtNum_ << " events.";
  LogWarning("EcalCosmicsHists")
      << "---> Number of EE+ cosmic events: " << cosmicCounterEEP_ << " in "
      << naiveEvtNum_ << " events.";

  //  LogWarning("EcalCosmicsHists") << "---> Number of top+bottom cosmic
  //  events: " << cosmicCounterTopBottom_ << " in " << cosmicCounter_ << "
  //  cosmics in " << naiveEvtNum_ << " events.";
}
