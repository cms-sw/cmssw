...
EcalDigiDisplay::EcalDigiDisplay(const edm::ParameterSet &ps) {
  ...
  requestedEbs_ =
      ps.getUntrackedParameter<std::vector<std::string>>("requestedEbs");
  ...
  requestedEbs_ = ps.getUntrackedParameter<std::vector<std::string>>(
      "requestedEbs", ebDefaults);
  // FEDs and EBs
  if (requestedFeds_[0] != -1) {
    ...
    if (requestedEbs_[0] != "none") {
      ...
      for (std::vector<std::string>::const_iterator ebItr =
               requestedEbs_.begin();
  ...
  // Channel list
  listChannels =
      ps.getUntrackedParameter<std::vector<int>>("listChannels", listDefaults);
  // Tower list
  listTowers =
      ps.getUntrackedParameter<std::vector<int>>("listTowers", listDefaults);
  ...
  if (fedIsGiven) {
    for (fedIter = requestedFeds_.begin(); fedIter != requestedFeds_.end();
         ++fedIter) {
      if ((*fedIter) < 601 || (*fedIter) > 655) {
        edm::LogError("EcalDigiDisplay")
            << " FED value: " << (*fedIter) << " found in requetsedFeds. "
  ...
  for (fedIter = requestedFeds_.begin(); fedIter != requestedFeds_.end();
       ++fedIter) {
    if ((*fedIter) > 609 && (*fedIter) < 646 &&
        inputIsOk) // if EB SM is being picked up
      barrelSM = true;
  }
  ...
  if (barrelSM) {
    if (cryDigi) {
      ...
      for (intIter = listChannels.begin(); intIter != listChannels.end();
           intIter++) {
        if (((*intIter) < 1) || (1700 < (*intIter))) {
          edm::LogError("EcalDigiDisplay")
              << " ic value: " << (*intIter) << " found in listChannels. "
    ...
    if (ttDigi) {
      for (intIter = listTowers.begin(); intIter != listTowers.end();
           intIter++) {

        if (((*intIter) < 1) || (70 < (*intIter))) {
          edm::LogError("EcalDigiDisplay")
              << " TT value: " << (*intIter) << " found in listTowers. "
  ...
  if (pnDigi) {
    for (intIter = listPns.begin(); intIter != listPns.end(); intIter++) {
      if (((*intIter) < 1) || (10 < (*intIter))) {
        edm::LogError("EcalDigiDisplay")
            << " Pn number : " << (*intIter) << " found in listPns. "
...
void EcalDigiDisplay::analyze(edm::Event const &e, edm::EventSetup const &c) {
  ...
  if (cryDigi || ttDigi) {
    ...
    if (!(ebDigisFound || eeDigisFound)) {
      edm::LogWarning("EcalDigiUnpackerModule")
...
void EcalDigiDisplay::readEBDigis(edm::Handle<EBDigiCollection> digis,
  ...
  for (EBDigiCollection::const_iterator digiItr = digis->begin();
    ...
    std::vector<int>::iterator fedIter =
        find(requestedFeds_.begin(), requestedFeds_.end(), FEDid);
    ...
    if (Mode == 1) {
      edm::LogInfo("EcalDigiDisplay")
          << "\n\n^^^^^^^^^^^^^^^^^^ [EcalDigiDisplay]  digi cry collection "
             "size "
          << digis->size();
      edm::LogInfo("EcalDigiDisplay")
          << "                       [EcalDigiDisplay]  dumping first "
          << listChannels[0] << " crystals\n";
    ...
    std::cout << "Tower: " << EBDetId((*digiItr).id()).tower().iTT()
              << " ic-cry: " << EBDetId((*digiItr).id()).ic()
              << " i-phi: " << EBDetId((*digiItr).id()).iphi()
              << " j-eta: " << EBDetId((*digiItr).id()).ieta() << std::endl;
    // Get Samples
...
void EcalDigiDisplay::readEEDigis(edm::Handle<EEDigiCollection> digis,
  ...
  for (EEDigiCollection::const_iterator digiItr = digis->begin();
    ...
    int chId = elecId.towerId(); // this is a channel in Endcap DCC, sometimes
                                 // also called as Super Crystal
    ...
    std::cout << "Tower: " << elecId.towerId() << "crystalId: " << crystalId
              << " i-x: " << EEDetId((*digiItr).id()).ix()
              << " j-y: " << EEDetId((*digiItr).id()).iy() << std::endl;
...
void EcalDigiDisplay::readPNDigis(edm::Handle<EcalPnDiodeDigiCollection> PNs,
  ...
  for (EcalPnDiodeDigiCollection::const_iterator pnItr = PNs->begin();
    ...
    if (Mode == 1) {
      edm::LogInfo("EcalDigiDisplay")
          << "\n\n^^^^^^^^^^^^^^^^^^ EcalDigiDisplay  digi PN collection.  "
             "Size: "
          << PNs->size();
      edm::LogInfo("EcalDigiDisplay")
          << "                       [EcalDigiDisplay]  dumping first "
