...
EcalPnGraphs::EcalPnGraphs(const edm::ParameterSet &ps) {
  ...
  fileName =
      ps.getUntrackedParameter<std::string>("fileName", std::string("toto"));

  first_Pn = 0;

  listPns =
      ps.getUntrackedParameter<std::vector<int>>("listPns", std::vector<int>());
  numPn = ps.getUntrackedParameter<int>("numPn");

  std::vector<int> listDefaults;
  listDefaults.push_back(-1);
  feds_ =
      ps.getUntrackedParameter<std::vector<int>>("requestedFeds", listDefaults);
  bool fedIsGiven = false;

  std::vector<std::string> ebDefaults;
  ebDefaults.push_back("none");
  ebs_ = ps.getUntrackedParameter<std::vector<std::string>>("requestedEbs",
                                                            ebDefaults);
  ...
  if (fedIsGiven) {
    ...
    for (fedIter = feds_.begin(); fedIter != feds_.end(); ++fedIter) {
      if ((*fedIter) < 601 || (*fedIter) > 654) {
        std::cout << "[EcalPnGraphs] pn number : " << (*fedIter)
                  << " found in listFeds. "
  ...
  if (listPns[0] != -1) {
    ...
    for (intIter = listPns.begin(); intIter != listPns.end(); intIter++) {
      if (((*intIter) < 1) || (10 < (*intIter))) {
        std::cout << "[EcalPnGraphs] pn number : " << (*intIter)
                  << " found in listPns. "
...
void EcalPnGraphs::analyze(const edm::Event &e, const edm::EventSetup &c) {
  ...
  // -numPn is the number of Pns (centered at Pn from listPns) for which graphs
  // are required
  ...
  for (pn_it = listPns.begin(); pn_it != listPns.end(); pn_it++) {
    ...
    for (int u = (-hpn); u <= hpn; u++) {
      ...
      if (ipn_c < 1 || ipn_c > 10)
        continue;
      std::vector<int>::iterator notInList =
          find(listAllPns.begin(), listAllPns.end(), ipn_c);
  ...
  // Loop over PN digis
  for (EcalPnDiodeDigiCollection::const_iterator pnItr = pn_digis->begin();
       pnItr != pn_digis->end(); ++pnItr) {
    // Get PNid of a digi
    ...
    std::vector<int>::iterator fedIter =
        find(feds_.begin(), feds_.end(), FEDid);

    if (fedIter == feds_.end()) {
      edm::LogWarning("EcalPnGraphs")
          << "For Event " << eventCounter
    ...
    std::string title;
    title = "Graph_ev" + intToString(eventCounter) + "_FED" +
            intToString(FEDid) + "_ipn" + intToString(ipn);
