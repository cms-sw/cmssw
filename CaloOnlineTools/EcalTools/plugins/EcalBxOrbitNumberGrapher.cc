...
EcalBxOrbitNumberGrapher::EcalBxOrbitNumberGrapher(
    ...
    : digiProducer_(iConfig.getParameter<std::string>("RawDigis")), runNum_(-1),
...
void EcalBxOrbitNumberGrapher::analyze(const edm::Event &iEvent,
  ...
  for (EcalRawDataCollection::const_iterator headerItr = DCCHeaders->begin();
    ...
    if (orbit == -100) {
      ...
      std::cout << " NOOOO This header has a conflicting orbit OTHER " << orbit
    ...
    if (bx == -100) {
      ...
      std::cout << " NOOOO This header has a conflicting bx OTHER " << bx
                << " new " << mybx << std::endl;
    }
    // LogDebug("EcalTimingCosmic") << " Lambda " << lambda; //hmm... this isn't
    // good, I should keep a record of the wavelength in the headers as an
    // inactive SM might have a different wavelength for this field and make
    // this not go through.
  }

  if ((bx != -100) & (orbit != -100)) {
    std::cout << " Interesting event Orbit " << orbit << " BX " << bx
              << std::endl;
...
void EcalBxOrbitNumberGrapher::beginJob() {
  bxnumberPlot_ =
      new TH1F("bxnumber", "BX number of interexting events", 3600, 0., 3600.);
  orbitErrorPlot_ = new TH1F(
      "bxOfOrbitDiffs", "BX number of interexting events with orbit changes",
      3600, 0., 3600.);
  orbitErrorBxDiffPlot_ =
      new TH1F("orbitErrorDiffPlot",
               "Orbit Difference of those HEADERS that have a difference", 20,
               -10., 10.);
  numberofOrbitDiffPlot_ = new TH1F("numberOfOrbitDiffsPlot",
                                    "Number of Orbit Differences", 54, 0., 54.);
}
