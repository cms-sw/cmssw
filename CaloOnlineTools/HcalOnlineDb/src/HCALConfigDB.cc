...
void HCALConfigDB::setAccessor(std::string _accessor) { accessor = std::move(_accessor); }
...
void HCALConfigDB::disconnect(void) {
  if (database != nullptr)
    database->disconnect();
  if (database2 != nullptr)
    database2->disconnect();
}

std::vector<unsigned int> HCALConfigDB::getOnlineLUT(
  ...
  if (topbottom == 0)
    _fpga = hcal::ConfigurationDatabase::Bottom;
  else if (topbottom == 1)
    _fpga = hcal::ConfigurationDatabase::Top;
  else {
  ...
  if (luttype == 1)
    _lt = hcal::ConfigurationDatabase::LinearizerLUT;
  else if (luttype == 2)
    _lt = hcal::ConfigurationDatabase::CompressionLUT;
  else {
...
std::vector<unsigned int> HCALConfigDB::getOnlineLUT(const std::string& tag,
                                                     uint32_t _rawid,
                                                     hcal::ConfigurationDatabase::LUTType _lt) {
  std::vector<unsigned int> result;
  ...
  if (_id.subdet() == HcalBarrel)
    subdetector = "HB";
  else if (_id.subdet() == HcalEndcap)
    subdetector = "HE";
  else if (_id.subdet() == HcalOuter)
    subdetector = "HO";
  else if (_id.subdet() == HcalForward)
    subdetector = "HF";
  ...
  try {
    ...
    query += toolbox::toString(" WHERE SIDE=%d AND ETA=%d AND PHI=%d AND DEPTH=%d AND SUBDETECTOR='%s'",
                               side,
                               etaAbs,
                               phi,
                               depth,
    ...
    while (rs->next()) {
      ...
      if (_condition_data_set_id < _cdsi) {
        ...
        if (_fpga == hcal::ConfigurationDatabase::Top)
          topbottom = 1;
        else
          topbottom = 0;
...
std::vector<unsigned int> HCALConfigDB::getOnlineLUTFromXML(const std::string& tag,
                                                            uint32_t _rawid,
                                                            hcal::ConfigurationDatabase::LUTType _lt) {
  std::vector<unsigned int> result;
...
oracle::occi::Connection* HCALConfigDB::getConnection(void) { return database->getConnection(); }

oracle::occi::Environment* HCALConfigDB::getEnvironment(void) { return database->getEnvironment(); }
