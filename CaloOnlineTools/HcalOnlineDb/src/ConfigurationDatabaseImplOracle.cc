	...
	while (rs->next()) {
		...
		m_parser.parseMultiple(buffer,items);
			...
			//if (islot == slot) {
				...
				int strtol_base=0;
				if (item.encoding=="hex") strtol_base=16;
				else if (item.encoding=="dec") strtol_base=10;

				// convert the data
				for (unsigned int j=0; j<item.items.size(); j++)
					lut.push_back(strtol(item.items[j].c_str(),0,strtol_base));
        ...
        while (rs->next()) {
                ...
                for (auto & item : items) {
					...
					(hcal::ConfigurationDatabase::FPGASelection)atoi(item.parameters["TOPBOTTOM"].c_str());
                                ...
                                // convert the data
                                for (unsigned int j=0; j<item.items.size(); j++)
                                        pat.push_back(strtol(item.items[j].c_str(),0,strtol_base));
                        //}
                }
        }
        //Always terminate statement
