...
class HcalLutManager{
 public:
  ...
  std::map<int, shared_ptr<LutXml> > getLutXmlFromAsciiMaster( string _filename,
							       string _tag,
							       int _crate = -1,
							       bool split_by_crate = true );

  std::map<int, shared_ptr<LutXml> > getLinearizationLutXmlFromAsciiMasterEmap( string _filename,
										string _tag,
										int _crate,
										bool split_by_crate = true );

  std::map<int, shared_ptr<LutXml> > getLinearizationLutXmlFromAsciiMasterEmap_new( string _filename,
										    string _tag,
										    int _crate,
										    bool split_by_crate = true );

  std::map<int, shared_ptr<LutXml> > getCompressionLutXmlFromAsciiMaster( string _filename,
									  string _tag,
									  int _crate = -1,
									  bool split_by_crate = true );

  std::map<int, shared_ptr<LutXml> > getLinearizationLutXmlFromCoder( const HcalTPGCoder & _coder,
								      string _tag,
								      bool split_by_crate = true );

  std::map<int, shared_ptr<LutXml> > getLinearizationLutXmlFromCoderEmap( const HcalTPGCoder & _coder,
									  string _tag,
									  bool split_by_crate = true );

  std::map<int, shared_ptr<LutXml> > getCompressionLutXmlFromCoder( string _tag, bool split_by_crate = true );

  std::map<int, shared_ptr<LutXml> > getCompressionLutXmlFromCoder( const CaloTPGTranscoderULUT & _coder,
								    string _tag,
								    bool split_by_crate = true );

  std::map<int, shared_ptr<LutXml> > getZdcLutXml( string _tag,
						   bool split_by_crate = true );

  // add two std::maps with LUTs. Designed mainly for joining compression LUTs to linearization ones.
  void addLutMap(std::map<int,
		 shared_ptr<LutXml> > & result,
		 const std::map<int,
		 shared_ptr<LutXml> > & other);
  ...
  int createLutXmlFiles_HBEFFromCoder_HOFromAscii_ZDC( string _tag, const HcalTPGCoder & _coder, const CaloTPGTranscoderULUT & _transcoder, string _lin_file, bool split_by_crate = true );
