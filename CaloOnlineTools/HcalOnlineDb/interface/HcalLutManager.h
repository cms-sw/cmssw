...
class HcalLutManager{
 public:
  ...
  std::map<int, boost::shared_ptr<LutXml> > getCompressionLutXmlFromCoder( const std::string& _tag, bool split_by_crate = true );

  std::map<int, boost::shared_ptr<LutXml> > getCompressionLutXmlFromCoder( const CaloTPGTranscoderULUT & _coder,
								    const std::string& _tag,
								    bool split_by_crate = true );

  std::map<int, boost::shared_ptr<LutXml> > getZdcLutXml( const std::string& _tag,
						   bool split_by_crate = true );
  ...
  HcalLutSet getLutSetFromFile( const std::string& _filename, int _type = 1 ); // _type = 1 - linearization, 2 - compression

  int writeLutXmlFiles( std::map<int, boost::shared_ptr<LutXml> > & _xml, const std::string& _tag = "default_tag", bool split_by_crate = true );
  ...
  int createAllLutXmlFiles( const std::string& _tag, const std::string& _lin_file, const std::string& _comp_file, bool split_by_crate = true );
  int createAllLutXmlFilesFromCoder( const HcalTPGCoder & _coder, const std::string& _tag, bool split_by_crate = true );
  int createLutXmlFiles_HBEFFromCoder_HOFromAscii( const std::string& _tag, const HcalTPGCoder & _coder, const std::string& _lin_file, bool split_by_crate = true );
  int createLutXmlFiles_HBEFFromCoder_HOFromAscii( const std::string& _tag, const HcalTPGCoder & _coder, const CaloTPGTranscoderULUT & _transcoder, const std::string& _lin_file, bool split_by_crate = true );

  int createLutXmlFiles_HBEFFromCoder_HOFromAscii_ZDC( const std::string& _tag, const HcalTPGCoder & _coder, const CaloTPGTranscoderULUT & _transcoder, const std::string& _lin_file, bool split_by_crate = true );

  int createAllLutXmlFilesLinAsciiCompCoder( const std::string& _tag, const std::string& _lin_file, bool split_by_crate = true );
  ...
  std::vector<unsigned int> getLutFromXml( const std::string& tag, uint32_t _rawid, hcal::ConfigurationDatabase::LUTType _lt );

  std::map<int, boost::shared_ptr<LutXml> > get_brickSet_from_oracle( const std::string& tag, const std::string& _accessor = "occi://CMS_HCL_PRTTYPE_HCAL_READER@anyhost/int2r?PASSWORD=HCAL_Reader_88,LHWM_VERSION=22" );

  int get_xml_files_from_db( const std::string& tag, const std::string& db_accessor = "occi://CMS_HCL_PRTTYPE_HCAL_READER@anyhost/int2r?PASSWORD=HCAL_Reader_88,LHWM_VERSION=22", bool split_by_crate = true );

  int create_lut_loader( const std::string& file_list, const std::string& _prefix, const std::string& tag_name, std::string comment="default comment", std::string version="V00-01-01", int subversion=1 );
