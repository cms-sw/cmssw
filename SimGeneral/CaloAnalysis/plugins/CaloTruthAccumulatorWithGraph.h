...
/* GRAPH DEFINITIONS
   ...
   its children. Only simHits within the HGCAL detector are taken into account.
...
class CaloTruthAccumulatorWithGraph : public DigiAccumulatorMixMod {
 public:
  explicit CaloTruthAccumulatorWithGraph( const edm::ParameterSet& config, edm::stream::EDProducerBase& mixMod, edm::ConsumesCollector& iC);
 private:
  ...
  template<class T> void fillSimHits(
