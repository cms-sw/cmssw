...
private:
  ...
  void fillSimHits(std::map<int, float> &, const edm::Event &,
...
CaloParticleDebugger::CaloParticleDebugger(const edm::ParameterSet &iConfig)
    : simTracks_(iConfig.getParameter<edm::InputTag>("simTracks")),
      ...
      collectionTags_(
          iConfig.getParameter<std::vector<edm::InputTag>>("collectionTags")) {
  edm::ConsumesCollector &&iC = consumesCollector();
  simTracksToken_ = iC.consumes<std::vector<SimTrack>>(simTracks_);
  genParticlesToken_ =
      iC.consumes<std::vector<reco::GenParticle>>(genParticles_);
  simVerticesToken_ = iC.consumes<std::vector<SimVertex>>(simVertices_);
  trackingParticlesToken_ =
      iC.consumes<std::vector<TrackingParticle>>(trackingParticles_);
  ...
  for (auto const &collectionTag : collectionTags_) {
    collectionTagsToken_.push_back(
...
void CaloParticleDebugger::analyze(const edm::Event &iEvent,
  ...
  sort(begin(sorted_tracks_idx), end(sorted_tracks_idx),
       [&tracks](int i, int j) {
  ...
  sort(begin(sorted_genParticles_idx), end(sorted_genParticles_idx),
       [&genParticles](int i, int j) {
         return genParticles[i].momentum().eta() <
                genParticles[j].momentum().eta();
       });
  ...
  sort(begin(sorted_vertices_idx), end(sorted_vertices_idx),
       [&vertices](int i, int j) {
  ...
  sort(begin(sorted_simcl_idx), end(sorted_simcl_idx),
       [&simclusters](int i, int j) {
  ...
  std::cout << "IDX\tTrackId\tPDGID\tMOMENTUM(x,y,z,E)\tVertexIdx\tGenPartIdx"
            << std::endl;
  ...
  for (auto i : sorted_genParticles_idx) {
    ...
    std::cout << i << "\t" << gp.pdgId() << "\t" << gp.momentum() << "\t"
              << gp.vertex() << std::endl;
  }
  ...
  for (auto i : sorted_cp_idx) {
    ...
    std::cout << "\n\n"
              ...
              << "\tType: " << cp.pdgId() << "\tEnergy: " << cp.energy()
              << "\tIdx: " << cp.g4Tracks()[0].trackId()
              << std::endl; // << cp << std::endl;
    double total_sim_energy = 0.;
    double total_cp_energy = 0.;
    std::cout << "--> Overall simclusters's size: " << cp.simClusters().size()
              << std::endl;
    ...
    std::cout << "--> Overall SC energy (sum using sim energies): "
              << total_sim_energy << std::endl;
    std::cout << "--> Overall SC energy (sum using CaloP energies): "
              << total_cp_energy << std::endl;
  }
  ...
  for (auto i : sorted_simcl_idx) {
    ...
    std::cout << "\n\n"
              ...
              << "\tType: " << simcl.pdgId() << "\tEnergy: " << simcl.energy()
              << "\tKey: " << i << std::endl; // << simcl << std::endl;
    double total_sim_energy = 0.;
    std::cout << "--> Overall simclusters's size: " << simcl.numberOfRecHits()
              << std::endl;
    for (auto const &cl : simcl.hits_and_fractions()) {
      total_sim_energy += detIdToTotalSimEnergy[cl.first] * cl.second;
    }
    std::cout << simcl << std::endl;
    std::cout << "--> Overall SimCluster energy (sum using sim energies): "
              << total_sim_energy << std::endl;
  }
}
...
void CaloParticleDebugger::fillSimHits(
    std::map<int, float> &detIdToTotalSimEnergy, const edm::Event &iEvent,
    const edm::EventSetup &iSetup) {
    ...
    for (auto const &simHit : *hSimHits) {
      ...
      if (geometryType_ == 1) {
        ...
        std::pair<int, int> recoLayerCell = ddd->simToReco(
            cell, layer, sec, hgtopo[subdet - 3]->detectorType());
...
// module  ------------
void CaloParticleDebugger::fillDescriptions(
  ...
  desc.add<std::vector<edm::InputTag>>(
      "collectionTags", {edm::InputTag("g4SimHits", "HGCHitsEE"),
