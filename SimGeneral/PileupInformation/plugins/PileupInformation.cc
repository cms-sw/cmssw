...
PileupInformation::PileupInformation(const edm::ParameterSet &config) {
  ...
  if (!isPreMixed_) {
    ...
    PileupInfoLabel_ = consumes<PileupMixingContent>(
        config.getParameter<edm::InputTag>("PileupMixingLabel"));

    PileupVtxLabel_ = consumes<PileupVertexContent>(
        config.getParameter<edm::InputTag>("PileupMixingLabel"));

    LookAtTrackingTruth_ = config.getUntrackedParameter<bool>("doTrackTruth");

    trackingTruthT_ = mayConsume<TrackingParticleCollection>(
        config.getParameter<edm::InputTag>("TrackingParticlesLabel"));
    trackingTruthV_ = mayConsume<TrackingVertexCollection>(
        config.getParameter<edm::InputTag>("TrackingParticlesLabel"));
    ...
    edm::LogInfo(MessageCategory_)
        << "Vertex distance cut set to " << distanceCut_ << " mm";
    edm::LogInfo(MessageCategory_)
        << "Volume radius set to " << volumeRadius_ << " mm";
    edm::LogInfo(MessageCategory_)
        << "Volume Z      set to " << volumeZ_ << " mm";
    edm::LogInfo(MessageCategory_)
        << "Lower pT Threshold set to " << pTcut_1_ << " GeV";
    edm::LogInfo(MessageCategory_)
        << "Upper pT Threshold set to " << pTcut_2_ << " GeV";
  } else {
    pileupSummaryToken_ = consumes<std::vector<PileupSummaryInfo>>(
        config.getParameter<edm::InputTag>("PileupSummaryInfoInputTag"));
    bunchSpacingToken_ = consumes<int>(
        config.getParameter<edm::InputTag>("BunchSpacingInputTag"));
  }
...
void PileupInformation::produce(edm::Event &event,
  ...
  if (isPreMixed_) {
    ...
    for (PSiter = psiInput.product()->begin();
         PSiter != psiInput.product()->end(); PSiter++) {
    ...
    // add bunch spacing to the event as a seperate integer for use by
    // downstream modules
  ...
  edm::Handle<PileupMixingContent>
      MixingPileup; // Get True pileup information from MixingModule
  ...
  const std::vector<float> &TrueInteractions =
      MixInfo->getMix_TrueInteractions();
  ...
  for (int ib = 0; ib < (int)bunchCrossing.size(); ++ib) {
    //      std::cout << " bcr, nint " << bunchCrossing[ib] << " " <<
    //      interactions[ib] << std::endl;
  ...
  // store information from pileup vertices, if it's in the event. Have to loop
  // on interactions again.
  ...
  edm::Handle<PileupVertexContent>
      MixingPileupVtx; // Get True pileup information from MixingModule
  if (event.getByToken(PileupVtxLabel_,
                       MixingPileupVtx)) { // extract information - way easier
                                           // than counting vertices
  ...
  // Now, get information on valid particles that look like they could be in the
  // tracking volume
  ...
  // int lastBunchCrossing = 0; // 0 is the true bunch crossing, should always
  // come first.
  ...
  if (LookAtTrackingTruth_) {
    ...
    if (event.getByToken(trackingTruthT_, mergedPH) &&
        event.getByToken(trackingTruthV_, mergedVH)) {
  ...
  std::vector<float>::iterator TInteractionsIter =
      TrueInteractions_Xing.begin();
  std::vector<std::vector<edm::EventID>>::iterator TEventInfoIter =
      eventInfoList_Xing.begin();
  ...
  for (BXIter = BunchCrossings.begin(); BXIter != BunchCrossings.end();
    ...
    if (HaveTrackingParticles) { // leave open the option of not producing
                                 // TrackingParticles and just keeping #
                                 // interactions

      for (iVtx = iVtxTest; iVtx != mergedVH->end(); ++iVtx) {

        if (iVtx->eventId().bunchCrossing() ==
            (*BXIter)) { // found first vertex in this bunch crossing

          if (iVtx->eventId().event() != lastEvent) {

            // std::cout << "BX,event " << iVtx->eventId().bunchCrossing() << "
            // " << iVtx->eventId().event() << std::endl;
      ...
      for (TrackingParticleCollection::const_iterator iTrack = iTrackTest;
           iTrack != mergedPH->end(); ++iTrack) {
        bool FoundTrk = false;

        float zpos = 0.;

        if (iTrack->eventId().bunchCrossing() == (*BXIter) &&
            iTrack->eventId().event() > 0) {
          FoundTrk = true;
          int correct_index = event_index_[iTrack->eventId().event()];

          // std::cout << " track index, correct index " <<
          // iTrack->eventId().event() << " " << correct_index << std::endl;
    ...
    //	  std::cout << "Making PSI for bunch " << lastBunchCrossing <<
    // std::endl;

    if (!HaveTrackingParticles) { // stick in one value so we don't have empty
                                  // vectors
    ...
    if (Have_pThats) {
      ...
      PileupSummaryInfo PSI_bunch = PileupSummaryInfo(
          (*InteractionsIter), (*zPosIter), (*tPosIter), sumpT_lowpT,
          sumpT_highpT, ntrks_lowpT, ntrks_highpT, (*TEventInfoIter),
          (*pThatIter), (*BXIter), (*TInteractionsIter), bunchSpacing);
      ...
      PileupSummaryInfo PSI_bunch = PileupSummaryInfo(
          (*InteractionsIter), zposZeros, tposZeros, sumpT_lowpT, sumpT_highpT,
          ntrks_lowpT, ntrks_highpT, (*TEventInfoIter), pThatZeros, (*BXIter),
          (*TInteractionsIter), bunchSpacing);

      PSIVector->push_back(PSI_bunch);
    }
    // std::cout << " " << std::endl;
    // std::cout << "Adding Bunch Crossing, nint " << (*BXIter) << " " <<
    // (*InteractionsIter) << std::endl;
    ...
    // if(HaveTrackingParticles) lastBunchCrossing =
    // iVtx->eventId().bunchCrossing();
  ...
  // add bunch spacing to the event as a seperate integer for use by downstream
  // modules
  std::unique_ptr<int> bunchSpacingP(new int(bunchSpacing));
  event.put(std::move(bunchSpacingP), "bunchSpacing");
}
