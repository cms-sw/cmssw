 ...
 * Saves final state protons from HepMC events in Crossing Frame, in the
 * generator-particle format.
 *
 * Note: Use the option USER_CXXFLAGS=-DEDM_ML_DEBUG with SCRAM in order to
 * enable debug messages.
 *
 *    March  9, 2017   : Initial version.
 *    March 14, 2017   : Updated debug messages.
 *     July 27, 2017   : Removed extra loop initially inherited from
 * GenParticleProducer. August 17, 2017   : Replaced std::auto_ptr with
 * std::unique_ptr. September 6, 2017   : Updated module to
 * edm::global::EDProducer with ConvertParticle as RunCache following
 * GenParticleProducer.
 *
 */
...
public:
  ...
  ConvertParticle()
      : abortOnUnknownPDGCode_(true), initialized_(false),
        chargeP_(PDGCacheMax, 0), chargeM_(PDGCacheMax, 0){};

  ConvertParticle(bool abortOnUnknownPDGCode)
      : abortOnUnknownPDGCode_(abortOnUnknownPDGCode), initialized_(false),
        chargeP_(PDGCacheMax, 0), chargeM_(PDGCacheMax, 0){};
  ...
  bool operator()(reco::GenParticle &cand,
                  HepMC::GenParticle const *part) const {
    ...
    if (v != nullptr) {
      ...
      reco::Candidate::Point vertex(vtx.x() * mmToCm, vtx.y() * mmToCm,
                                    vtx.z() * mmToCm);
...
class GenPUProtonProducer
    : public edm::global::EDProducer<edm::RunCache<ConvertParticle>> {
public:
  GenPUProtonProducer(const edm::ParameterSet &);
  ~GenPUProtonProducer() override;

  void produce(edm::StreamID, edm::Event &e,
               const edm::EventSetup &) const override;
  std::shared_ptr<ConvertParticle>
  globalBeginRun(const edm::Run &, const edm::EventSetup &) const override;
...
GenPUProtonProducer::GenPUProtonProducer(const ParameterSet &cfg)
    : abortOnUnknownPDGCode_(
          cfg.getUntrackedParameter<bool>("abortOnUnknownPDGCode", true)),
      bunchList_(cfg.getParameter<vector<int>>("bunchCrossingList")),
      minPz_(cfg.getParameter<double>("minPz")) {
  produces<GenParticleCollection>();
  mixToken_ = consumes<CrossingFrame<HepMCProduct>>(
      InputTag(cfg.getParameter<std::string>("mix"), "generatorSmeared"));
}

GenPUProtonProducer::~GenPUProtonProducer() {}

std::shared_ptr<ConvertParticle>
GenPUProtonProducer::globalBeginRun(const Run &, const EventSetup &es) const {
...
void GenPUProtonProducer::produce(StreamID, Event &evt,
  ...
  std::unique_ptr<MixCollection<HepMCProduct>> cfhepmcprod(
      new MixCollection<HepMCProduct>(cf.product()));
  npiles = cfhepmcprod->size();

  LogDebug("GenPUProtonProducer")
      << " Number of pile-up events : " << npiles << endl;

  for (size_t icf = 0; icf < npiles; ++icf) {
    LogDebug("GenPUProtonProducer")
        << "CF " << icf << " size : "
        << cfhepmcprod->getObject(icf).GetEvent()->particles_size() << endl;
  ...
  size_t idx_mix = 0;
  // Fill collection
  for (mixHepMC_itr = cfhepmcprod->begin(); mixHepMC_itr != cfhepmcprod->end();
       ++mixHepMC_itr, ++idx_mix) {
    ...
    if (find(bunchList_.begin(), bunchList_.end(), bunch) != bunchList_.end()) {
      ...
      for (auto p = event->particles_begin(); p != event->particles_end();
           ++p) {
