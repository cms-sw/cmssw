...
public:
  ...
  explicit PdtEntry(const std::string &name)
      : pdgId_(0), name_(name), data_(nullptr) {}
  /// PDG id
  int pdgId() const;
  /// particle name
...
template <>
inline PdtEntry
ParameterSet::getParameter<PdtEntry>(std::string const &name) const {
...
template <>
inline PdtEntry
ParameterSet::getUntrackedParameter<PdtEntry>(std::string const &name) const {
  Entry const *e = getEntryPointerOrThrow_(name);
  return pdtentry::getPdtEntry(*e, name.c_str());
}

template <>
inline PdtEntry ParameterSet::getUntrackedParameter<PdtEntry>(
    std::string const &name, PdtEntry const &defaultValue) const {
...
template <>
inline PdtEntry
ParameterSet::getUntrackedParameter<PdtEntry>(char const *name) const {
  Entry const *e = getEntryPointerOrThrow_(name);
  return pdtentry::getPdtEntry(*e, name);
}
...
template <>
inline PdtEntry ParameterSet::getUntrackedParameter<PdtEntry>(
    char const *name, PdtEntry const &defaultValue) const {
  Entry const *e = retrieveUntracked(name);
  if (e == nullptr)
    return defaultValue;
  return pdtentry::getPdtEntry(*e, name);
...
template <>
inline std::vector<PdtEntry> ParameterSet::getParameter<std::vector<PdtEntry>>(
...
template <>
inline std::vector<PdtEntry>
ParameterSet::getUntrackedParameter<std::vector<PdtEntry>>(
    std::string const &name) const {
  Entry const *e = getEntryPointerOrThrow_(name);
  return pdtentry::getPdtEntryVector(*e, name.c_str());
}
...
template <>
inline std::vector<PdtEntry>
ParameterSet::getParameter<std::vector<PdtEntry>>(char const *name) const {
  Entry const &e = retrieve(name);
...
ParameterSet::getParameterNamesForType<PdtEntry>(bool trackiness) const {
  std::vector<std::string> ints = getParameterNamesForType<int>(trackiness);
  std::vector<std::string> strings =
      getParameterNamesForType<std::string>(trackiness);
  std::copy(strings.begin(), strings.end(),
            std::back_insert_iterator<std::vector<std::string>>(ints));
  return ints;
}
...
ParameterSet::getParameterNamesForType<std::vector<PdtEntry>>(
  ...
  std::vector<std::string> ints =
      getParameterNamesForType<std::vector<int>>(trackiness);
  std::vector<std::string> strings =
      getParameterNamesForType<std::vector<std::string>>(trackiness);
  std::copy(strings.begin(), strings.end(),
            std::back_insert_iterator<std::vector<std::string>>(ints));
  return ints;
}
