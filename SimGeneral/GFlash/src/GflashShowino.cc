...
GflashShowino::GflashShowino()
    : theShowerType(-1), theEnergy(0), thePositionAtShower(0, 0, 0),
      thePathLengthAtShower(0), thePathLengthOnEcal(0), theStepLengthToHcal(0),
      theStepLengthToOut(0), thePathLength(0), theGlobalTime(0),
      thePosition(0, 0, 0), theEnergyDeposited(0) {
  theHelix = new GflashTrajectory;
}

GflashShowino::~GflashShowino() { delete theHelix; }

void GflashShowino::initialize(int showerType, double energy, double globalTime,
...
void GflashShowino::updateShowino(double deltaStep) {
  ...
  theGlobalTime += (theEnergy / 100.0) * deltaStep /
                   30.0; //@@@calculate exact time change in nsec
}
...
void GflashShowino::evaluateLengths() {
  ...
  if (std::fabs(eta) < Gflash::EtaMax[Gflash::kESPM]) {
    thePathLengthOnEcal =
        theHelix->getPathLengthAtRhoEquals(Gflash::RFrontCrystalEB);
    thePathLengthAtShower =
        theHelix->getPathLengthAtRhoEquals(thePosition.getRho());
    double pathLengthAtHcalBack =
        theHelix->getPathLengthAtRhoEquals(Gflash::Rmax[Gflash::kHB]);
    if (pathLengthAtHcalBack > 0) {
      theStepLengthToOut =
          std::min(300., pathLengthAtHcalBack - thePathLengthAtShower);
    } else {
      theStepLengthToOut = 200.;
    }
    theStepLengthToHcal =
        theHelix->getPathLengthAtRhoEquals(Gflash::Rmin[Gflash::kHB]) -
        thePathLengthAtShower;

  } else if (std::fabs(eta) < Gflash::EtaMax[Gflash::kENCA]) {
    double zsign = (eta > 0) ? 1.0 : -1.0;
    thePathLengthOnEcal =
        theHelix->getPathLengthAtZ(zsign * Gflash::ZFrontCrystalEE);
    thePathLengthAtShower = theHelix->getPathLengthAtZ(thePosition.getZ());
    theStepLengthToOut = std::min(
        300., theHelix->getPathLengthAtZ(zsign * Gflash::Zmax[Gflash::kHE]) -
                  thePathLengthAtShower);
    theStepLengthToHcal =
        theHelix->getPathLengthAtZ(zsign * Gflash::Zmin[Gflash::kHE]) -
        thePathLengthAtShower;
  } else {
    //@@@extend for HF later
...
Gflash3Vector &
GflashShowino::simulateFirstInteractionPoint(int fastSimShowerType,
  ...
  if (theEnergy > 0.0 && theEnergy < 15) {
    effectiveLambda =
        24.6 + 2.6 * std::tanh(3.0 * (std::log(theEnergy) - 1.43));
  } else {
    effectiveLambda =
        28.4 + 1.20 * std::tanh(1.5 * (std::log(theEnergy) - 4.3));
  }
  ...
  double frac_ssp2 =
      2.8310e+00 +
      2.6766e+00 * tanh(-4.8068e-01 * (std::log(theEnergy) + 3.4857e+00));
  ...
  if (fastSimShowerType == 100) { // fastTrack.onEcal() == 1
    ...
    thePathLengthOnEcal =
        theHelix->getPathLengthAtRhoEquals(Gflash::RFrontCrystalEB);
    double pathLengthAt2 =
        theHelix->getPathLengthAtRhoEquals(Gflash::RFrontCrystalEB + rhoTemp);
    double pathLengthAt3 =
        theHelix->getPathLengthAtRhoEquals(Gflash::Rmin[Gflash::kHB]);
    ...
    */
    ...
    if (depthAtShower > (pathLengthAt2 - thePathLengthOnEcal)) {
      ...
      if (CLHEP::HepUniformRand() < frac_ssp2) {
        depthAtShower =
            (pathLengthAt2 - thePathLengthOnEcal) +
            (pathLengthAt3 - pathLengthAt2) * CLHEP::HepUniformRand();
      }
      // inside Hcal
      else {
        depthAtShower = (pathLengthAt3 - thePathLengthOnEcal) -
                        effectiveLambda * log(CLHEP::HepUniformRand());
        // check whether the shower starts beyond HB
        double pathLengthAt4 =
            theHelix->getPathLengthAtRhoEquals(Gflash::Rmax[Gflash::kHB]);
        if (depthAtShower > (pathLengthAt4 - thePathLengthOnEcal)) {
          depthAtShower =
              (pathLengthAt4 - pathLengthAt3) * CLHEP::HepUniformRand();
        }
      }
    }
    //    }
    ...
    thePathLengthOnEcal =
        theHelix->getPathLengthAtZ(zsign * Gflash::ZFrontCrystalEE);
    double pathLengthAt2 =
        theHelix->getPathLengthAtZ(zsign * (Gflash::ZFrontCrystalEE + zTemp));
    double pathLengthAt3 =
        theHelix->getPathLengthAtZ(zsign * Gflash::Zmin[Gflash::kHE]);
    ...
    */
    ...
    if (depthAtShower > (pathLengthAt2 - thePathLengthOnEcal)) {
      if (CLHEP::HepUniformRand() < frac_ssp2) {
        ...
        depthAtShower = (pathLengthAt3 - thePathLengthOnEcal) -
                        effectiveLambda * std::log(CLHEP::HepUniformRand());
        // shower starts beyond HE
        double pathLengthAt4 =
            theHelix->getPathLengthAtZ(zsign * Gflash::Zmax[Gflash::kHE]);
        if (depthAtShower > (pathLengthAt4 - thePathLengthOnEcal)) {
          depthAtShower =
              (pathLengthAt4 - pathLengthAt3) * CLHEP::HepUniformRand();
        }
