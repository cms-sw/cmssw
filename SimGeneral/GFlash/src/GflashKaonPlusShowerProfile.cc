...
void GflashKaonPlusShowerProfile::loadParameters() {
    ...
    if (showerType == 0 || showerType == 1) {
      do {
        ...
        energyScale[Gflash::kESPM] =
            einc * (fTanh(einc, Gflash::kplus_emscale[0]) +
                    fTanh(einc, Gflash::kplus_emscale[1]) * r1);

        // LogNormal mean and sigma of Hcal energy
        energyMeanHcal =
            (fTanh(einc, Gflash::kplus_hadscale[0]) +
             fTanh(einc, Gflash::kplus_hadscale[1]) *
                 depthScale(position.getRho(), Gflash::RFrontCrystalEB,
        ...
        energyScale[Gflash::kHB] =
            exp(energyMeanHcal +
                energySigmaHcal *
                    (energyRho * r1 + sqrt(1.0 - energyRho * energyRho) * r2));
      } while (energyScale[Gflash::kESPM] < 0 ||
               energyScale[Gflash::kHB] > einc * 1.5);
    } else {
      do {
        r1 = CLHEP::RandGaussQ::shoot();
        ...
        energyMeanHcal =
            (fTanh(einc, Gflash::kplus_hadscale[0]) +
             fTanh(einc, Gflash::kplus_hadscale[1]) *
                 depthScale(std::fabs(position.getZ()), Gflash::ZFrontCrystalEE,
        ...
        energyScale[Gflash::kHE] =
            exp(energyMeanHcal +
                energySigmaHcal *
                    (energyRho * r1 + sqrt(1.0 - energyRho * energyRho) * r2));
      } while (energyScale[Gflash::kENCA] < 0 ||
               energyScale[Gflash::kHE] > einc * 1.5);
    }
    ...
    if (showerType == 2 || showerType == 3) {
      ...
      if (showerType == 2) {
        energyScale[Gflash::kHE] = std::max(
            0.0, energyScale[Gflash::kHB] -
                     gap_corr * depthScale(position.getRho(),
                                           Gflash::Rmin[Gflash::kHB], 28.));
      }
      ...
      do {
        r1 = CLHEP::RandGaussQ::shoot();
        energyMeanHcal +=
            std::log(1.0 - fTanh(einc, Gflash::kplus_hadscale[7]));
        energyScale[Gflash::kHE] = exp(energyMeanHcal + energySigmaHcal * r1);
      } while (energyScale[Gflash::kHE] > einc * 1.5);

      if (showerType == 6) {
        energyScale[Gflash::kHE] = std::max(
            0.0, energyScale[Gflash::kHE] -
                     gap_corr * depthScale(std::fabs(position.getZ()),
                                           Gflash::Zmin[Gflash::kHE], 66.));
      }
    }
  }
  ...
  for (int i = 0; i < 2 * Gflash::NPar; i++) {
    rhoHcal[i] =
        fTanh(einc, Gflash::kplus_rho[i + showerType * 2 * Gflash::NPar]);
  }
  ...
  for (int i = 0; i < Gflash::NPar; i++) {
    ...
    longHcal[i] =
        fTanh(einc, Gflash::kplus_par[i + showerType * Gflash::NPar]) +
        fTanh(einc, Gflash::kplus_par[i + (4 + showerType) * Gflash::NPar]) *
            correlationSum;
  }
  delete[] rhoHcal;
  ...
  for (int i = 0; i < Gflash::Nrpar; i++) {
    lateralPar[Gflash::kHB][i] =
        fTanh(einc, Gflash::kplus_rpar[i + showerType * Gflash::Nrpar]);
  ...
  if (showerType == 1) {
    ...
    for (int i = 0; i < Gflash::NPar; i++) {
      ...
      for (int j = 0; j < i + 1; j++) {
        correlationSum +=
            correlationVectorEcal[i * (i + 1) / 2 + j] * normalZ[j];
      }
      longEcal[i] = fTanh(einc, Gflash::kplus_par[i]) +
                    0.5 * fTanh(einc, Gflash::kplus_par[i + 4 * Gflash::NPar]) *
                        correlationSum;
    }
