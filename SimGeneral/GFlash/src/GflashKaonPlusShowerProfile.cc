...
void GflashKaonPlusShowerProfile::loadParameters()
{
  ...
  if(showerType == 0 || showerType == 1 || showerType == 4 || showerType == 5) {
    ...
    if(showerType == 0 || showerType == 1) {
      energyScale[Gflash::kESPM] = einc*(fTanh(einc,Gflash::emscale[0]) + fTanh(einc,Gflash::emcorr_pid[4])
	...
	einc*fTanh(einc,Gflash::hadcorr_pid[4]) +
	exp(energyMeanHcal+energySigmaHcal*(energyRho*r1 + sqrt(1.0- energyRho*energyRho)*r2 ))-0.05*einc);
    }
    else {
      energyScale[Gflash::kENCA] = einc*(fTanh(einc,Gflash::emscale[0]) + fTanh(einc,Gflash::emcorr_pid[5])
	...
	einc*fTanh(einc,Gflash::hadcorr_pid[5]) +
        exp(energyMeanHcal+energySigmaHcal*(energyRho*r1 + sqrt(1.0- energyRho*energyRho)*r2 ))-0.05*einc);
    }
  }
    ...
    if(showerType == 2 || showerType == 3) {
      energyScale[Gflash::kHB] = std::max(0.0, einc*fTanh(einc,Gflash::mipcorr_pid[4]) +
	...
	energyScale[Gflash::kHB] = std::max(0.0,energyScale[Gflash::kHB]
      ...
      energyScale[Gflash::kHE] = std::max(0.0, einc*fTanh(einc,Gflash::mipcorr_pid[5]) +
				 exp(energyMeanHcal+energySigmaHcal*CLHEP::RandGaussQ::shoot())-2.0);
      if(showerType == 6) {
