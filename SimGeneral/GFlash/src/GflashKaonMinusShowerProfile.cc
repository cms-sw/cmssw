...
void GflashKaonMinusShowerProfile::loadParameters()
{
  ...
  if(showerType == 0 || showerType == 1 || showerType == 4 || showerType == 5) {
	...
	energyScale[Gflash::kESPM] = einc*(1.15*fTanh(einc,Gflash::kminus_emscale[0]) + 0.8*fTanh(einc,Gflash::kminus_emscale[1])*r1);

	//LogNormal mean and sigma of Hcal energy
	energyMeanHcal  = (fTanh(einc,Gflash::kminus_hadscale[0]) +
			   1.5*fTanh(einc,Gflash::kminus_hadscale[1])*depthScale(position.getRho(),Gflash::RFrontCrystalEB,Gflash::LengthCrystalEB));
    ...
    energyMeanHcal  = fTanh(einc,Gflash::kminus_hadscale[4]) + 0.05;
