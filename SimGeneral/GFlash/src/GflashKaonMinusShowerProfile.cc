...
void GflashKaonMinusShowerProfile::loadParameters() {
  ...
  if (showerType == 0 || showerType == 1 || showerType == 4 ||
    ...
    if (showerType == 0 || showerType == 1) {
      do {
        ...
        energyScale[Gflash::kESPM] =
            einc * (fTanh(einc, Gflash::kminus_emscale[0]) +
                    fTanh(einc, Gflash::kminus_emscale[1]) * r1);

        // LogNormal mean and sigma of Hcal energy
        energyMeanHcal =
            (fTanh(einc, Gflash::kminus_hadscale[0]) +
    ...
    if (showerType == 2 || showerType == 3) {
      ...
      do {
        r1 = CLHEP::RandGaussQ::shoot();
        energyMeanHcal +=
            std::log(1.0 - fTanh(einc, Gflash::kminus_hadscale[7]));
  ...
  for (int i = 0; i < 2 * Gflash::NPar; i++) {
    rhoHcal[i] =
        fTanh(einc, Gflash::kminus_rho[i + showerType * 2 * Gflash::NPar]);
  }
  ...
  for (int i = 0; i < Gflash::NPar; i++) {
    ...
    longHcal[i] =
        fTanh(einc, Gflash::kminus_par[i + showerType * Gflash::NPar]) +
        fTanh(einc, Gflash::kminus_par[i + (4 + showerType) * Gflash::NPar]) *
            correlationSum;
  }
  delete[] rhoHcal;
  ...
  for (int i = 0; i < Gflash::Nrpar; i++) {
    lateralPar[Gflash::kHB][i] =
        fTanh(einc, Gflash::kminus_rpar[i + showerType * Gflash::Nrpar]);
  ...
  if (showerType == 1) {
    ...
    for (int i = 0; i < Gflash::NPar; i++) {
      ...
      longEcal[i] = fTanh(einc, Gflash::kminus_par[i]) +
                    0.5 *
                        fTanh(einc, Gflash::kminus_par[i + 4 * Gflash::NPar]) *
                        correlationSum;
    }
