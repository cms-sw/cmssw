...
void GflashHadronShowerProfile::initialize(int showerType, double energy,
...
void GflashHadronShowerProfile::hadronicParameterization() {
  ...
  while (stepLengthLeft > 0.0) {
    ...
    deltaEnergy =
        heightProfile * Gflash::divisionStep * energyScale[whichCalor];
    ...
    int nSpotsInStep =
        std::max(1, static_cast<int>(getNumberOfSpots(whichCalor) *
                                     (deltaEnergy / energyScale[whichCalor])));
    double sampleSpotEnergy =
        hadronicFraction * fluctuatedEnergy / nSpotsInStep;
    ...
    for (int ispot = 0; ispot < nSpotsInStep; ispot++) {
      ...
      theShowino->getHelix()->getGflashTrajectoryPoint(trajectoryPoint,
                                                       incrementPath);
  ...
  if (theShowino->getEnergy() > Gflash::MinEnergyCutOffForHO &&
      fabs(theShowino->getPositionAtShower().pseudoRapidity()) <
          Gflash::EtaMax[Gflash::kHO] &&
      theGflashHcalOuter) {

    // non zero ho fraction to simulate based on geant4
    double nonzeroProb =
        0.7 * fTanh(theShowino->getEnergy(), Gflash::ho_nonzero);
    double r0 = CLHEP::HepUniformRand();
    double leftoverE =
        theShowino->getEnergy() - theShowino->getEnergyDeposited();
    ...
    if (r0 < nonzeroProb && leftoverE > 0.0) {
      ...
      double pathLength = theShowino->getHelix()->getPathLengthAtRhoEquals(
          Gflash::Rmin[Gflash::kHO] - 10);
      stepLengthLeft = theShowino->getHelix()->getPathLengthAtRhoEquals(
                           Gflash::Rmax[Gflash::kHO] + 10) -
                       pathLength;
      showerDepth = pathLength - theShowino->getPathLengthAtShower();

      theShowino->setPathLength(pathLength);

      double pathLengthx = theShowino->getHelix()->getPathLengthAtRhoEquals(
          Gflash::Rmax[Gflash::kHO]);
      double pathLengthy = theShowino->getHelix()->getPathLengthAtRhoEquals(
          Gflash::Rmax[Gflash::kHB]);
      ...
      while (stepLengthLeft > 0.0) {
        ...
        double hoScale = leftoverE * (pathLengthx - pathLengthy) /
                         (pathLengthx - theShowino->getPathLengthAtShower());
        double refDepth = theShowino->getPathLength() -
                          theShowino->getHelix()->getPathLengthAtRhoEquals(
                              Gflash::Rmax[Gflash::kHB]);
        ...
        int nSpotsInStep = std::max(
            50, static_cast<int>((160. + 40 * CLHEP::RandGaussQ::shoot()) *
                                     std::log(theShowino->getEnergy()) +
                                 50.));
        ...
        for (int ispot = 0; ispot < nSpotsInStep; ispot++) {
          ...
          if (std::fabs(hitPosition.getZ() / CLHEP::cm) >
...
double
GflashHadronShowerProfile::medianLateralArm(double showerDepthR50,
                                            Gflash::CalorimeterNumber kCalor) {
  double lateralArm = 0.0;
  if (kCalor != Gflash::kNULL) {

    double showerDepthR50X =
        std::min(showerDepthR50 / 22.4, Gflash::maxShowerDepthforR50);
    double R50 = lateralPar[kCalor][0] +
                 std::max(0.0, lateralPar[kCalor][1]) * showerDepthR50X;
    double varinanceR50 = std::pow(
        (lateralPar[kCalor][2] + lateralPar[kCalor][3] * showerDepthR50X) * R50,
        2);
...
Gflash3Vector
GflashHadronShowerProfile::locateHitPosition(GflashTrajectoryPoint &point,
                                             double lateralArm) {
...
double GflashHadronShowerProfile::longitudinalProfile() {
  ...
  if (showerType == 0 || showerType == 4) {
    double shiftDepth =
        theShowino->getPathLengthOnEcal() - theShowino->getPathLengthAtShower();
    if (shiftDepth > 0) {
      heightProfile =
          twoGammaProfile(longEcal, showerDepth - shiftDepth, whichCalor);
    } else {
      heightProfile = 0.;
    ...
    if (whichCalor == Gflash::kESPM || whichCalor == Gflash::kENCA) {
      ...
      heightProfile =
          twoGammaProfile(longHcal, showerDepth - transDepth, whichCalor);
    } else
      heightProfile = 0.;
    ...
    if ((showerDepth - transDepth) > 0.0) {
      heightProfile =
          twoGammaProfile(longHcal, showerDepth - transDepth, Gflash::kHB);
    } else
      heightProfile = 0.;
...
double GflashHadronShowerProfile::hoProfile(double pathLength,
  ...
  double dint = 1.4 * Gflash::intLength[Gflash::kHO] *
                std::sin(tempPoint.getPosition().getTheta());
...
void GflashHadronShowerProfile::getFluctuationVector(
    double *lowTriangle, double *correlationVector) {
...
void GflashHadronShowerProfile::doCholeskyReduction(double **vv, double **cc,
                                                    const int ndim) {
...
int GflashHadronShowerProfile::getNumberOfSpots(
    Gflash::CalorimeterNumber kCalor) {
  ...
  if (showerType == 0 || showerType == 1 || showerType == 4 ||
      showerType == 5) {
  ...
  } else if (showerType == 2 || showerType == 3 || showerType == 6 ||
             showerType == 7) {
    if (kCalor == Gflash::kHB || kCalor == Gflash::kHE) {
      nmean = 5000 + 2500 * log(einc);
      nsigma = 500;
  ...
  numberOfSpots = std::max(
      500, static_cast<int>(nmean + nsigma * CLHEP::RandGaussQ::shoot()));
...
double GflashHadronShowerProfile::fTanh(double einc, const double *par) {
  double func = 0.0;
  if (einc > 0.0)
    func = par[0] + par[1] * std::tanh(par[2] * (std::log(einc) - par[3])) +
           par[4] * std::log(einc);
  return func;
}
...
double GflashHadronShowerProfile::depthScale(double ssp, double ssp0,
                                             double length) {
  double func = 0.0;
  if (length > 0.0)
...
double GflashHadronShowerProfile::gammaProfile(double alpha, double beta,
                                               double showerDepth,
                                               double lengthUnit) {
  double gamma = 0.0;
  ...
  if (showerDepth > 0.0) {
    ...
    gamma = (beta / lengthUnit) * std::pow(x, alpha - 1.0) * std::exp(-x) /
            std::exp(lgam(alpha));
  }
  return gamma;
}
...
double
GflashHadronShowerProfile::twoGammaProfile(double *longPar, double depth,
  ...
  twoGamma = longPar[0] * gammaProfile(exp(longPar[1]), exp(longPar[2]), depth,
                                       Gflash::radLength[kIndex]) +
             (1 - longPar[0]) * gammaProfile(exp(longPar[3]), exp(longPar[4]),
                                             depth, Gflash::intLength[kIndex]);
  return twoGamma;
}
