...
void GflashPiKShowerProfile::loadParameters() {
    ...
    if (showerType == 0 || showerType == 1) {
      do {
        ...
        energyScale[Gflash::kESPM] =
            einc * (fTanh(einc, Gflash::pion_emscale[0]) +
                    fTanh(einc, Gflash::pion_emscale[1]) * r1);

        // LogNormal mean and sigma of Hcal energy
        energyMeanHcal =
            (fTanh(einc, Gflash::pion_hadscale[0]) +
  ...
  for (int i = 0; i < 2 * Gflash::NPar; i++) {
    rhoHcal[i] =
        fTanh(einc, Gflash::pion_rho[i + showerType * 2 * Gflash::NPar]);
  }
  ...
  for (int i = 0; i < Gflash::NPar; i++) {
    ...
    longHcal[i] =
        fTanh(einc, Gflash::pion_par[i + showerType * Gflash::NPar]) +
        fTanh(einc, Gflash::pion_par[i + (4 + showerType) * Gflash::NPar]) *
            correlationSum;
  }
  delete[] rhoHcal;
  ...
  for (int i = 0; i < Gflash::Nrpar; i++) {
    lateralPar[Gflash::kHB][i] =
        fTanh(einc, Gflash::pion_rpar[i + showerType * Gflash::Nrpar]);
  ...
  if (showerType == 1) {
    ...
    for (int i = 0; i < Gflash::NPar; i++) {
      ...
      longEcal[i] = fTanh(einc, Gflash::pion_par[i]) +
                    0.5 * fTanh(einc, Gflash::pion_par[i + 4 * Gflash::NPar]) *
                        correlationSum;
    }
