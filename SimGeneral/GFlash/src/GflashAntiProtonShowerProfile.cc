...
void GflashAntiProtonShowerProfile::loadParameters() {
  ...
  for (int i = 0; i < 2 * Gflash::NPar; i++) {
    rhoHcal[i] =
        fTanh(einc, Gflash::pbar_rho[i + showerType * 2 * Gflash::NPar]);
  }
  ...
  for (int i = 0; i < Gflash::NPar; i++) {
    ...
    longHcal[i] =
        fTanh(einc, Gflash::pbar_par[i + showerType * Gflash::NPar]) +
        fTanh(einc, Gflash::pbar_par[i + (4 + showerType) * Gflash::NPar]) *
            correlationSum;
  }
  delete[] rhoHcal;
  ...
  for (int i = 0; i < Gflash::Nrpar; i++) {
    lateralPar[Gflash::kHB][i] =
        fTanh(einc, Gflash::pbar_rpar[i + showerType * Gflash::Nrpar]);
  ...
  if (showerType == 1) {
    ...
    for (int i = 0; i < Gflash::NPar; i++) {
      ...
      longEcal[i] = fTanh(einc, Gflash::pbar_par[i]) +
                    0.5 * fTanh(einc, Gflash::pbar_par[i + 4 * Gflash::NPar]) *
                        correlationSum;
    }
