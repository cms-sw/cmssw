...

{
  ...
  pixeldigi_collectionSig_ =
      ps.getParameter<edm::InputTag>("pixeldigiCollectionSig");
  pixeldigi_collectionPile_ =
      ps.getParameter<edm::InputTag>("pixeldigiCollectionPile");
  PixelDigiCollectionDM_ =
      ps.getParameter<std::string>("PixelDigiCollectionDM");

  PixelDigiToken_ =
      iC.consumes<edm::DetSetVector<PixelDigi>>(pixeldigi_collectionSig_);
  PixelDigiPToken_ =
      iC.consumes<edm::DetSetVector<PixelDigi>>(pixeldigi_collectionPile_);
...
void DataMixingSiPixelWorker::addSiPixelPileups(
  ...
  std::shared_ptr<Wrapper<edm::DetSetVector<PixelDigi>> const> inputPTR =
      getProductByTag<edm::DetSetVector<PixelDigi>>(
          *ep, pixeldigi_collectionPile_, mcc);

  if (inputPTR) {

    const edm::DetSetVector<PixelDigi> *input =
        const_cast<edm::DetSetVector<PixelDigi> *>(inputPTR->product());
    ...
    for (; DSViter != input->end(); DSViter++) {
      ...
      if (itest !=
        ...
        for (icopy = begin; icopy != end; icopy++) {
          LocalMap.insert(
              OneDetectorMap::value_type((icopy->channel()), *icopy));
        }
...
void DataMixingSiPixelWorker::putSiPixel(edm::Event &e) {
  ...
  LogInfo("DataMixingSiPixelWorker")
      << "total # Merged Pixels: " << vPixelDigi.size();

  // make new digi collection

  std::unique_ptr<edm::DetSetVector<PixelDigi>> MyPixelDigis(
      new edm::DetSetVector<PixelDigi>(vPixelDigi));
