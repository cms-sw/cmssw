...

{
  ...
  EBrechitCollectionSig_ =
      ps.getParameter<edm::InputTag>("EBrechitCollectionSig");
  EErechitCollectionSig_ =
      ps.getParameter<edm::InputTag>("EErechitCollectionSig");
  ESrechitCollectionSig_ =
      ps.getParameter<edm::InputTag>("ESrechitCollectionSig");

  EBPileRecHitInputTag_ =
      ps.getParameter<edm::InputTag>("EBPileRecHitInputTag");
  EEPileRecHitInputTag_ =
      ps.getParameter<edm::InputTag>("EEPileRecHitInputTag");
  ESPileRecHitInputTag_ =
      ps.getParameter<edm::InputTag>("ESPileRecHitInputTag");
...
void DataMixingEMWorker::addEMSignals(const edm::Event &e) {
  ...
  if (e.getByToken(EBRecHitToken_, pEBRecHits)) {
    ...
    LogDebug("DataMixingEMWorker")
        << "total # EB rechits: " << EBRecHits->size();
  }
  ...
  if (e.getByToken(EERecHitToken_, pEERecHits)) {
    ...
    LogDebug("DataMixingEMWorker")
        << "total # EE rechits: " << EERecHits->size();
  }
...
#ifdef DEBUG
    LogDebug("DataMixingEMWorker")
        << "total # ES rechits: " << ESRecHits->size();
#endif
  }
...
void DataMixingEMWorker::addEMPileups(const int bcr, const EventPrincipal *ep,
  ...
  if (EBRecHitsPTR) {
    ...
    const EBRecHitCollection *EBRecHits =
        const_cast<EBRecHitCollection *>(EBRecHitsPTR->product());
  ...
  if (EERecHitsPTR) {
    ...
    const EERecHitCollection *EERecHits =
        const_cast<EERecHitCollection *>(EERecHitsPTR->product());
  ...
  if (ESRecHitsPTR) {
    ...
    const ESRecHitCollection *ESRecHits =
        const_cast<ESRecHitCollection *>(ESRecHitsPTR->product());
...
void DataMixingEMWorker::putEM(edm::Event &e) {
  ...
  for (EBRecHitMap::const_iterator iEB = EBRecHitStorage_.begin();
    ...
    if (currentID == formerID) { // we have to add these rechits together
      ...
      ESum = (iEB->second).energy();
      EBTime = (iEB->second)
                   .time(); // take time of first hit in sequence - is this ok?
    }

    iEBchk = iEB;
    if ((++iEBchk) ==
        EBRecHitStorage_.end()) { // make sure not to lose the last one
      EcalRecHit aHit(formerID, ESum, EBTime);
      EBrechits->push_back(aHit);
    }
  }
  ...
  for (EERecHitMap::const_iterator iEE = EERecHitStorage_.begin();
    ...
    iEEchk = iEE;
    if ((++iEEchk) ==
        EERecHitStorage_.end()) { // make sure not to lose the last one
      EcalRecHit aHit(formerID, ESum, EETime);
      EErechits->push_back(aHit);
    }
  }
  ...
  for (ESRecHitMap::const_iterator iES = ESRecHitStorage_.begin();
    ...
    iESchk = iES;
    if ((++iESchk) ==
        ESRecHitStorage_.end()) { // make sure not to lose the last one
      EcalRecHit aHit(formerID, ESum, ESTime);
      ESrechits->push_back(aHit);
    }
  }
