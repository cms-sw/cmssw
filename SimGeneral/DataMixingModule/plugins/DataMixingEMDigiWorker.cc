...
void DataMixingEMDigiWorker::addEMSignals(const edm::Event &e,
  ...
  if (e.getByToken(EBDigiToken_, pEBDigis)) {
    ...
    LogDebug("DataMixingEMDigiWorker")
        << "total # EB digis: " << EBDigis->size();
  }
...
void DataMixingEMDigiWorker::addEMPileups(const int bcr,
  ...
  if (EBDigisPTR) {
    ...
    const EBDigiCollection *EBDigis =
        const_cast<EBDigiCollection *>(EBDigisPTR->product());
  ...
  if (EEDigisPTR) {
    ...
    const EEDigiCollection *EEDigis =
        const_cast<EEDigiCollection *>(EEDigisPTR->product());
  ...
  if (ESDigisPTR) {
    ...
    const ESDigiCollection *ESDigis =
        const_cast<ESDigiCollection *>(ESDigisPTR->product());
...
void DataMixingEMDigiWorker::putEM(edm::Event &e, const edm::EventSetup &ES) {
  ...
  for (EBDigiMap::const_iterator iEB = EBDigiStorage_.begin();
      ...
      for (unsigned int isamp = 0; isamp < max_samp; isamp++) {
        ...
        if (adc_new > 0 && adc_old > 0) {
          if (gain_old == gain_new) { // we're happy - easy case
            ...
            if (gain_old <
                gain_new) { // old has higher gain than new, scale to lower gain

              float ratio = gainRatios[gain_new - 1] / gainRatios[gain_old - 1];
              adc_old = (int)round((adc_old - pedeStals[gain_old - 1]) / ratio +
                                   pedeStals[gain_new - 1]);
              gain_consensus = gain_new;
            } else { // scale to old (lower) gain
              float ratio = gainRatios[gain_old - 1] / gainRatios[gain_new - 1];
              adc_new = (int)round((adc_new - pedeStals[gain_new - 1]) / ratio +
                                   pedeStals[gain_old - 1]);
              gain_consensus = gain_old;
            }
          }
        }
        ...
        if (adc_sum > 4096) {
          if (gain_consensus < 3) {
            ...
            double ratio =
                gainRatios[gain_consensus] / gainRatios[gain_consensus - 1];
            adc_sum =
                (int)round((adc_sum - pedeStals[gain_consensus - 1]) / ratio +
                           pedeStals[gain_consensus]);
        ...
        if (gain_consensus < sw_gain_consensus) {
          ...
          double ratio = gainRatios[sw_gain_consensus - 1] /
                         gainRatios[gain_consensus - 1];
          adc_sum =
              (int)round((adc_sum - pedeStals[gain_consensus - 1]) / ratio +
                         pedeStals[sw_gain_consensus - 1]);
    ...
    iEBchk = iEB;
    if ((++iEBchk) ==
        EBDigiStorage_.end()) { // make sure not to lose the last one
  ...
  for (EEDigiMap::const_iterator iEE = EEDigiStorage_.begin();
    ...
    iEEchk = iEE;
    if ((++iEEchk) ==
        EEDigiStorage_.end()) { // make sure not to lose the last one
  ...
  for (ESDigiMap::const_iterator iES = ESDigiStorage_.begin();
    ...
    if (currentID == formerID) { // we have to add these digis together
      ...
      for (unsigned int isamp = 0; isamp < max_samp; isamp++) {
        ...
        data = adc_sum +
               (rawdat & 0xF000); // data is 14 bit word with gain as MSBs
    ...
    iESchk = iES;
    if ((++iESchk) ==
        ESDigiStorage_.end()) { // make sure not to lose the last one
...
DataMixingEMDigiWorker::GetPedestals(const edm::EventSetup &ES,
                                     const DetId &detid) {
...
const std::vector<float>
DataMixingEMDigiWorker::GetGainRatios(const edm::EventSetup &ES,
                                      const DetId &detid) {
