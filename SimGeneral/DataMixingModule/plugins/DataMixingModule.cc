...
DataMixingModule::DataMixingModule(const edm::ParameterSet &ps,
  ...
  tok_qie10_ =
      consumes<HcalQIE10DigitizerTraits::DigiCollection>(QIE10PileInputTag_);
  tok_qie11_ =
      consumes<HcalQIE11DigitizerTraits::DigiCollection>(QIE11PileInputTag_);
  ...
  MergeTrackerDigis_ =
      (ps.getParameter<std::string>("TrackerMergeType")) == "Digis";
  ...
  if (MergeHcalDigis_)
    MergeHcalDigisProd_ =
        (ps.getParameter<std::string>("HcalDigiMerge") == "FullProd");
  ...
  if (MergeEMDigis_) {
    ...
    EBRecHitCollectionDM_ =
        ps.getParameter<std::string>("EBRecHitCollectionDM");
    EERecHitCollectionDM_ =
        ps.getParameter<std::string>("EERecHitCollectionDM");
    ESRecHitCollectionDM_ =
        ps.getParameter<std::string>("ESRecHitCollectionDM");
    //   nMaxPrintout_            =
    //   ps.getUntrackedParameter<int>("nMaxPrintout",10);
  ...
  if (MergeHcalDigis_) {
    HBHEDigiCollectionDM_ =
        ps.getParameter<std::string>("HBHEDigiCollectionDM");
    ...
    if (MergeHcalDigisProd_)
      HcalDigiWorkerProd_ =
          new DataMixingHcalDigiWorkerProd(ps, consumesCollector());
    else
    ...
    HORecHitCollectionDM_ =
        ps.getParameter<std::string>("HORecHitCollectionDM");
    HFRecHitCollectionDM_ =
        ps.getParameter<std::string>("HFRecHitCollectionDM");
  ...
  useSiStripRawDigi_ =
      ps.exists("SiStripRawDigiSource")
...
void DataMixingModule::initializeEvent(const edm::Event &e,
...
bool DataMixingModule::pileWorker(const EventPrincipal &ep, int bcr,
                                  int eventNr, const edm::EventSetup &ES,
  ...
  // SiStrips
  if (useSiStripRawDigi_)
    SiStripRawWorker_->addSiStripPileups(bcr, &ep, eventNr,
                                         &moduleCallingContext);
  else
...
void DataMixingModule::doPileUp(edm::Event &e, const edm::EventSetup &ES) {
  ...
  for (int bunchCrossing = minBunch_; bunchCrossing <= maxBunch_;
       ++bunchCrossing) {
    for (unsigned int isource = 0; isource < maxNbSources_; ++isource) {
      ...
      if (isource == 0)
        source->CalculatePileup(minBunch_, maxBunch_, PileupList,
                                TrueNumInteractions_, e.streamID());
      ...
      source->readPileUp(e.id(), recordEventID,
                         std::bind(&DataMixingModule::pileWorker,
                                   std::ref(*this), _1, bunchCrossing, _2,
                                   std::cref(ES), mcc),
                         NumPU_Events, e.streamID());
    }
  }
}
