...

{
  ...
  BunchSpacingInputTag_ =
      ps.getParameter<edm::InputTag>("BunchSpacingInputTag");
  CFPlaybackInputTag_ = ps.getParameter<edm::InputTag>("CFPlaybackInputTag");

  GenPUProtonsInputTags_ =
      ps.getParameter<std::vector<edm::InputTag>>("GenPUProtonsInputTags");
...
void DataMixingPileupCopy::addPileupInfo(const EventPrincipal *ep,
  ...
  if (PileupInfoPTR) {
    ...
    LogDebug("DataMixingEMWorker")
        << "PileupInfo Size: " << PileupSummaryStorage_.size();
  }
  ...
  // Gen. PU protons
  std::shared_ptr<edm::Wrapper<std::vector<reco::GenParticle>> const>
      GenPUProtonsPTR;
  for (std::vector<edm::InputTag>::const_iterator it_InputTag =
           GenPUProtonsInputTags_.begin();
       it_InputTag != GenPUProtonsInputTags_.end(); ++it_InputTag) {
    GenPUProtonsPTR =
        getProductByTag<std::vector<reco::GenParticle>>(*ep, *it_InputTag, mcc);
...
void DataMixingPileupCopy::putPileupInfo(edm::Event &e) {
  std::unique_ptr<std::vector<PileupSummaryInfo>> PSIVector(
      new std::vector<PileupSummaryInfo>);
  std::unique_ptr<int> bsInt(new int);

  std::vector<PileupSummaryInfo>::const_iterator PSiter;
  for (PSiter = PileupSummaryStorage_.begin();
       PSiter != PileupSummaryStorage_.end(); PSiter++) {
  ...
  *bsInt = bsStorage_;
  ...
  for (size_t idx = 0; idx < GenPUProtons_.size(); ++idx) {
    std::unique_ptr<std::vector<reco::GenParticle>> GenPUProtons_ptr(
        new std::vector<reco::GenParticle>());
    std::vector<reco::GenParticle>::const_iterator it_GenParticle =
        GenPUProtons_.at(idx).begin();
    std::vector<reco::GenParticle>::const_iterator it_GenParticle_end =
        GenPUProtons_.at(idx).end();
