...

{
  ...
  CSCstripdigi_collectionSig_ =
      ps.getParameter<edm::InputTag>("CSCstripdigiCollectionSig");
  CSCwiredigi_collectionSig_ =
      ps.getParameter<edm::InputTag>("CSCwiredigiCollectionSig");
  CSCCompdigi_collectionSig_ =
      ps.getParameter<edm::InputTag>("CSCCompdigiCollectionSig");

  DTDigiToken_ = iC.consumes<DTDigiCollection>(DTDigiTagSig_);
  CSCStripDigiToken_ =
      iC.consumes<CSCStripDigiCollection>(CSCstripdigi_collectionSig_);
  CSCWireDigiToken_ =
      iC.consumes<CSCWireDigiCollection>(CSCwiredigi_collectionSig_);
  CSCCompDigiToken_ =
      iC.consumes<CSCComparatorDigiCollection>(CSCCompdigi_collectionSig_);
  ...
  CSCStripPileInputTag_ =
      ps.getParameter<edm::InputTag>("CSCStripPileInputTag");
  ...
  CSCStripDigiPToken_ =
      iC.consumes<CSCStripDigiCollection>(CSCStripPileInputTag_);
  CSCWireDigiPToken_ = iC.consumes<CSCWireDigiCollection>(CSCWirePileInputTag_);
  CSCCompDigiPToken_ =
      iC.consumes<CSCComparatorDigiCollection>(CSCCompPileInputTag_);
  ...
  CSCStripDigiCollectionDM_ =
      ps.getParameter<std::string>("CSCStripDigiCollectionDM");
  CSCWireDigiCollectionDM_ =
      ps.getParameter<std::string>("CSCWireDigiCollectionDM");
  CSCComparatorDigiCollectionDM_ =
      ps.getParameter<std::string>("CSCComparatorDigiCollectionDM");
}
...
void DataMixingMuonWorker::addMuonSignals(const edm::Event &e) {
  ...
  if (e.getByToken(RPCDigiToken_, pRPCdigis)) {
    ...
    for (RLayerIt = RPCdigis->begin(); RLayerIt != RPCdigis->end();
         ++RLayerIt) {
      // The layerId
  ...
  if (e.getByToken(CSCStripDigiToken_, pCSCStripdigis)) {
    ...
    for (CSLayerIt = CSCStripdigis->begin(); CSLayerIt != CSCStripdigis->end();
         ++CSLayerIt) {
      // The layerId
  ...
  if (e.getByToken(CSCWireDigiToken_, pCSCWiredigis)) {
    ...
    for (CWLayerIt = CSCWiredigis->begin(); CWLayerIt != CSCWiredigis->end();
         ++CWLayerIt) {
      // The layerId
  ...
  if (e.getByToken(CSCCompDigiToken_, pCSCComparatordigis)) {
    ...
    const CSCComparatorDigiCollection *CSCComparatordigis =
        pCSCComparatordigis.product();
    CSCComparatorDigiCollection::DigiRangeIterator CWLayerIt;
    for (CWLayerIt = CSCComparatordigis->begin();
         CWLayerIt != CSCComparatordigis->end(); ++CWLayerIt) {
      // The layerId
      const CSCDetId &layerId = (*CWLayerIt).first;
...
void DataMixingMuonWorker::addMuonPileups(const int bcr,
  ...
  if (DTDigisPTR) {
    ...
    const DTDigiCollection *DTDigis =
        const_cast<DTDigiCollection *>(DTDigisPTR->product());

    DTDigiCollection::DigiRangeIterator DTLayerIt;
    for (DTLayerIt = DTDigis->begin(); DTLayerIt != DTDigis->end();
         ++DTLayerIt) {
      // The layerId
  ...
  if (RPCDigisPTR) {
    ...
    const RPCDigiCollection *RPCDigis =
        const_cast<RPCDigiCollection *>(RPCDigisPTR->product());

    RPCDigiCollection::DigiRangeIterator RPCLayerIt;
    for (RPCLayerIt = RPCDigis->begin(); RPCLayerIt != RPCDigis->end();
         ++RPCLayerIt) {
      // The layerId
  ...
  if (CSCStripDigisPTR) {
    ...
    const CSCStripDigiCollection *CSCStripDigis =
        const_cast<CSCStripDigiCollection *>(CSCStripDigisPTR->product());

    CSCStripDigiCollection::DigiRangeIterator CSCStripLayerIt;
    for (CSCStripLayerIt = CSCStripDigis->begin();
         CSCStripLayerIt != CSCStripDigis->end(); ++CSCStripLayerIt) {
      // The layerId
  ...
  if (CSCWireDigisPTR) {
    ...
    const CSCWireDigiCollection *CSCWireDigis =
        const_cast<CSCWireDigiCollection *>(CSCWireDigisPTR->product());

    CSCWireDigiCollection::DigiRangeIterator CSCWireLayerIt;
    for (CSCWireLayerIt = CSCWireDigis->begin();
         CSCWireLayerIt != CSCWireDigis->end(); ++CSCWireLayerIt) {
      // The layerId
  ...
  std::shared_ptr<Wrapper<CSCComparatorDigiCollection> const>
      CSCComparatorDigisPTR = getProductByTag<CSCComparatorDigiCollection>(
          *ep, CSCCompPileInputTag_, mcc);

  if (CSCComparatorDigisPTR) {

    const CSCComparatorDigiCollection *CSCComparatorDigis =
        const_cast<CSCComparatorDigiCollection *>(
            CSCComparatorDigisPTR->product());

    CSCComparatorDigiCollection::DigiRangeIterator CSCComparatorLayerIt;
    for (CSCComparatorLayerIt = CSCComparatorDigis->begin();
         CSCComparatorLayerIt != CSCComparatorDigis->end();
...
void DataMixingMuonWorker::putMuon(edm::Event &e) {
  ...
  std::unique_ptr<CSCStripDigiCollection> CSCStripDigiMerge(
      new CSCStripDigiCollection);
  std::unique_ptr<CSCWireDigiCollection> CSCWireDigiMerge(
      new CSCWireDigiCollection);
  std::unique_ptr<CSCComparatorDigiCollection> CSCComparatorDigiMerge(
      new CSCComparatorDigiCollection);

  // Loop over DT digis, copying them from our own local storage

  DTDigiCollection::DigiRangeIterator DLayerIt;
  for (DLayerIt = OurDTDigis_->begin(); DLayerIt != OurDTDigis_->end();
       ++DLayerIt) {
    // The layerId
    const DTLayerId &layerId = (*DLayerIt).first;
  ...
  RPCDigiCollection::DigiRangeIterator RLayerIt;
  for (RLayerIt = OurRPCDigis_->begin(); RLayerIt != OurRPCDigis_->end();
       ++RLayerIt) {
    // The layerId
    const RPCDetId &layerId = (*RLayerIt).first;
  ...
  CSCStripDigiCollection::DigiRangeIterator CSLayerIt;
  for (CSLayerIt = OurCSCStripDigis_->begin();
       CSLayerIt != OurCSCStripDigis_->end(); ++CSLayerIt) {
    // The layerId
    const CSCDetId &layerId = (*CSLayerIt).first;
  ...
  CSCWireDigiCollection::DigiRangeIterator CWLayerIt;
  for (CWLayerIt = OurCSCWireDigis_->begin();
       CWLayerIt != OurCSCWireDigis_->end(); ++CWLayerIt) {
    // The layerId
    const CSCDetId &layerId = (*CWLayerIt).first;
  ...
  for (CCLayerIt = OurCSCComparatorDigis_->begin();
       CCLayerIt != OurCSCComparatorDigis_->end(); ++CCLayerIt) {
    // The layerId
