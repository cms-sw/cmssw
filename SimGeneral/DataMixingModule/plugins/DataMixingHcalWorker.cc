...

{
  ...
  ZDCrechitCollectionSig_ =
      ps.getParameter<edm::InputTag>("ZDCrechitCollectionSig");

  HBHEPileRecHitInputTag_ =
      ps.getParameter<edm::InputTag>("HBHEPileRecHitInputTag");
  HOPileRecHitInputTag_ =
      ps.getParameter<edm::InputTag>("HOPileRecHitInputTag");
  HFPileRecHitInputTag_ =
      ps.getParameter<edm::InputTag>("HFPileRecHitInputTag");
  ZDCPileRecHitInputTag_ =
      ps.getParameter<edm::InputTag>("ZDCPileRecHitInputTag");

  HBHERecHitCollectionDM_ =
      ps.getParameter<std::string>("HBHERecHitCollectionDM");
  ...
  ZDCRecHitCollectionDM_ =
      ps.getParameter<std::string>("ZDCRecHitCollectionDM");

  HBHERecHitToken_ =
      iC.consumes<HBHERecHitCollection>(HBHErechitCollectionSig_);
  ...
  HBHERecHitPToken_ =
      iC.consumes<HBHERecHitCollection>(HBHEPileRecHitInputTag_);
...
void DataMixingHcalWorker::addHcalSignals(const edm::Event &e) {
  ...
  if (e.getByToken(HBHERecHitToken_, pHBHERecHits)) {
    ...
    LogDebug("DataMixingHcalWorker")
        << "total # HBHE rechits: " << HBHERecHits->size();
  }
...
#ifdef DEBUG
    LogDebug("DataMixingHcalWorker")
        << "total # HO rechits: " << HORecHits->size();
#endif
  }
...
#ifdef DEBUG
    LogDebug("DataMixingHcalWorker")
        << "total # HF rechits: " << HFRecHits->size();
#endif
  }
...
#ifdef DEBUG
    LogDebug("DataMixingHcalWorker")
        << "total # ZDC rechits: " << ZDCRecHits->size();
#endif
  }
...
void DataMixingHcalWorker::addHcalPileups(const int bcr,
  ...
  if (HBHERecHitsPTR) {
    ...
    const HBHERecHitCollection *HBHERecHits =
        const_cast<HBHERecHitCollection *>(HBHERecHitsPTR->product());
  ...
  if (HORecHitsPTR) {
    ...
    const HORecHitCollection *HORecHits =
        const_cast<HORecHitCollection *>(HORecHitsPTR->product());
  ...
  if (HFRecHitsPTR) {
    ...
    const HFRecHitCollection *HFRecHits =
        const_cast<HFRecHitCollection *>(HFRecHitsPTR->product());
  ...
  if (ZDCRecHitsPTR) {
    ...
    const ZDCRecHitCollection *ZDCRecHits =
        const_cast<ZDCRecHitCollection *>(ZDCRecHitsPTR->product());
...
void DataMixingHcalWorker::putHcal(edm::Event &e) {
  ...
  for (HBHERecHitMap::const_iterator iHB = HBHERecHitStorage_.begin();
    ...
    iHBchk = iHB;
    if ((++iHBchk) ==
        HBHERecHitStorage_.end()) { // make sure not to lose the last one
      HBHERecHit aHit(formerID, ESum, HBTime);
      HBHErechits->push_back(aHit);
    }
  }
  ...
  for (HORecHitMap::const_iterator iHO = HORecHitStorage_.begin();
    ...
    iHOchk = iHO;
    if ((++iHOchk) ==
        HORecHitStorage_.end()) { // make sure not to lose the last one
      HORecHit aHit(formerID, ESum, HOTime);
      HOrechits->push_back(aHit);
    }
  }
  ...
  for (HFRecHitMap::const_iterator iHF = HFRecHitStorage_.begin();
    ...
    iHFchk = iHF;
    if ((++iHFchk) ==
        HFRecHitStorage_.end()) { // make sure not to lose the last one
  ...
  for (ZDCRecHitMap::const_iterator iZDC = ZDCRecHitStorage_.begin();
    ...
    iZDCchk = iZDC;
    if ((++iZDCchk) ==
        ZDCRecHitStorage_.end()) { // make sure not to lose the last one
