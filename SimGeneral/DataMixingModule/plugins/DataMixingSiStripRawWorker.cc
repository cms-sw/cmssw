...

{
  ...
  Sistripdigi_collectionSig_ =
      ps.getParameter<edm::InputTag>("SistripdigiCollectionSig");
  ...
  edm::InputTag tag = edm::InputTag(Sistripdigi_collectionSig_.label(),
                                    SistripLabelSig_.label());

  SiStripInputTok_ = iC.consumes<edm::DetSetVector<SiStripDigi>>(tag);
  SiStripRawInputTok_ =
      iC.consumes<edm::DetSetVector<SiStripRawDigi>>(SiStripRawInputTag_);
}
...
void DataMixingSiStripRawWorker::addSiStripPileups(
  ...
  if (SiStripRawDigiSource_ == "SIGNAL") {
    pSSD = getProductByTag<edm::DetSetVector<SiStripDigi>>(
        *ep, SiStripPileInputTag_, mcc);
    digicollection_ =
        const_cast<edm::DetSetVector<SiStripDigi> *>(pSSD->product());
  } else if (SiStripRawDigiSource_ == "PILEUP") {
    pSSRD = getProductByTag<edm::DetSetVector<SiStripRawDigi>>(
        *ep, SiStripRawInputTag_, mcc);
    rawdigicollection_ =
        const_cast<edm::DetSetVector<SiStripRawDigi> *>(pSSRD->product());
  } else {
    std::cout << "you shouldn't be here" << std::endl;
  }
...
void DataMixingSiStripRawWorker::putSiStrip(edm::Event &e) {
  ...
  edm::DetSetVector<SiStripRawDigi>::const_iterator rawDSViter =
      rawdigicollection_->begin();
  for (; rawDSViter != rawdigicollection_->end(); rawDSViter++) {
    ...
    if (itest != SiHitStorage_.end()) {
      ...
      int currentstrip = 0;
      edm::DetSet<SiStripRawDigi>::const_iterator iRawDigi =
          rawDSViter->begin();
