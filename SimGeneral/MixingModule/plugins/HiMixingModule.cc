...
namespace edm {
  ...
  public:
    ...
    HiMixingWorkerBase(std::string& object, std::vector<InputTag>& tags, std::string& label)
        : object_(object), tags_(tags), label_(label) {
      ;
    }
  ...
  public:
    ...
    void addSignals(edm::Event& e) override {
      ...
      for (size_t itag = 0; itag < tags_.size(); ++itag) {
        ...
        if (!get)
          LogWarning("Product inconsistency")
              << "One of the sub-events is missing the product with type " << object_ << ", instance "
              << tags_[itag].instance() << " whereas the other one is fine.";
      }
    ...
    for (std::vector<string>::iterator it = names.begin(); it != names.end(); ++it) {
      ...
      for (size_t itag = 0; itag < tags.size(); ++itag) {
        ...
        if (object == "HepMCProduct") {
          ...
          LogInfo("Error") << "What the hell is this object?!";

        LogInfo("HiMixingModule") << "Will mix " << object << "s with InputTag= " << tag.encode() << ", label will be "
                                  << label;
      }
    }
