...
namespace edm {
  ...
  void MixingWorker<PSimHit>::addPileups(const int bcr, Event* e,unsigned int eventNr,int vertexoffset)
  {
	...
	crFrame_->addPileups(bcr,const_cast<std::vector<PSimHit>*> (result_t.product()),eventNr,vertexoffset);
      }
    }else {
      // Tracker treatment
	...
	if ( !checktof_ || ((limHighLowTof +tof ) <= highTrackTof)) { 
	  crFrame_->addPileups(bcr,const_cast<std::vector<PSimHit>*>(simhitshigh.product()),eventNr,0,checktof_,trackerHigh_);
	...
	if (  !checktof_ || ((tof+limHighLowTof) >= lowTrackTof && tof <= highTrackTof)) {
	  crFrame_->addPileups(bcr,const_cast<std::vector<PSimHit>*>(simhitslow.product()),eventNr,0,checktof_,trackerHigh_);
  ...
  void MixingWorker<SimTrack>::addPileups(const int bcr, Event* e,unsigned int eventNr,int vertexoffset)
  {
    ...
    if (got) {
      ...
      crFrame_->addPileups(bcr,const_cast<std::vector<SimTrack>*>(result_t.product()),eventNr,vertexoffset);
    }
  }
  ...
  void MixingWorker<SimVertex>::addPileups(const int bcr, Event* e,unsigned int eventNr,int vertexoffset)
  {
    ...
    if (got) {
      ...
      crFrame_->addPileups(bcr,const_cast<std::vector<SimVertex>*>(result.product()),eventNr);
  ...
  void MixingWorker<HepMCProduct>::addPileups(const int bcr, Event* e,unsigned int eventNr,int vertexoffset)
  {
    ...
    // we accept that since its more efficient than creating vectors for all other types
    Handle<HepMCProduct>  result_mc;
    bool got = e->getByLabel(tag_,result_mc);
    if (got){
      LogDebug("MixingModule") <<"  pileup HepMCProduct added";
      crFrame_->addHepMCPileups(bcr,result_mc.product(),eventNr);
    }
  }
  ...
  void MixingWorker<HepMCProduct>::addSignals(const Event &e)
  {
    ...
    if (got) {
      ...
      crFrame_->addHepMCSignals(result_t.product(),e.id());  
    }
