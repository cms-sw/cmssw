...
EcalTBH2TDCRecInfoProducer::EcalTBH2TDCRecInfoProducer(
  ...
  triggerDataCollection_ =
      ps.getParameter<std::string>("triggerDataCollection");
...
void EcalTBH2TDCRecInfoProducer::produce(edm::Event &e,
  ...
  if (!h2TriggerData) {
    edm::LogError("EcalTBTDCRecInfoError")
        << "Error! can't get the product " << triggerDataCollection_.c_str();
    return;
  }
  ...
  if (!h2TriggerData->wasBeamTrigger()) {
    ...
    e.put(std::make_unique<EcalTBTDCRecInfo>(
              algo_->reconstruct(runNumber, *ecalRawTDC)),
          recInfoCollection_);
  }
}
