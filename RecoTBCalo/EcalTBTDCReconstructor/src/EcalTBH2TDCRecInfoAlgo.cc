...
EcalTBH2TDCRecInfoAlgo::EcalTBH2TDCRecInfoAlgo(
    const std::vector<EcalTBH2TDCRanges> &tdcZeros)
    : tdcZeros_(tdcZeros), actualRun_(-1), actualRange_(-1) {}

EcalTBTDCRecInfo
EcalTBH2TDCRecInfoAlgo::reconstruct(const int &runNumber,
                                    const HcalTBTiming &TDCRawInfo) const {
  if (actualRun_ != runNumber) {
    actualRun_ = runNumber;
    actualRange_ = -1;
    for (unsigned int i = 0; i < tdcZeros_.size(); i++)
      if (runNumber <= tdcZeros_[i].runRanges.second &&
          runNumber >= tdcZeros_[i].runRanges.first)
        actualRange_ = i;
  ...
  edm::LogInfo("") << "EcalTBH2TDCRecInfoAlgo::reconstruct # tdc hits: " << ntdc
                   << std::endl;
  ;
  if (ntdc > 1) {
    for (int i = 0; i < ntdc; ++i) {
      edm::LogInfo("") << "hit i: " << i
                       << " tdc: " << TDCRawInfo.BeamCoincidenceHits(i)
                       << std::endl;
    }
  }

  if (ntdc == 0) {
    edm::LogError("NoTDCHits")
  ...
  if ( //! tdcRangeErrorMessageAlreadyDisplayed_  &&
      (tdcd < tdcZeros_[actualRange_].tdcZero - 1 ||
       tdcd > tdcZeros_[actualRange_].tdcZero + 26)) {
  ...
  double offset =
      ((double)tdcd - (double)tdcZeros_[actualRange_].tdcZero) / 25.; //
