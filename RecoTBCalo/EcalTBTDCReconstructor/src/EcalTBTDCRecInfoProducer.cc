...
EcalTBTDCRecInfoProducer::EcalTBTDCRecInfoProducer(
  ...
  eventHeaderCollection_ =
      ps.getParameter<std::string>("eventHeaderCollection");
  ...
  use2004OffsetConvention_ =
      ps.getUntrackedParameter<bool>("use2004OffsetConvention", false);
...
void EcalTBTDCRecInfoProducer::produce(edm::Event &e,
  ...
  if ((*ecalTDCRawInfo).size() < 1) {
    edm::LogError("EcalTBTDcRecInfoError")
        << "Less than one TDC good channel found. Aborting"
  ...
  if (!ecalEventHeader) {
    edm::LogError("EcalTBTDCRecInfoError")
        << "Error! can't get the product " << eventHeaderCollection_.c_str();
    return;
  }

  // Create empty output

  e.put(std::make_unique<EcalTBTDCRecInfo>(algo_->reconstruct(
            *ecalRawTDC, *tbEventHeader, use2004OffsetConvention_)),
        recInfoCollection_);
}
