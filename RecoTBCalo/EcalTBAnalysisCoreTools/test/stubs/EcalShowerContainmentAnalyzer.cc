...
protected:
  ...
  struct Ntuple {
    ...
    void zero() {
      run = event = 0;
      energy1x1 = energy3x3 = energy5x5 = corrected3x3 = corrected5x5 = posx =
          posy = hodo_posx = hodo_posy = 0;
    }
...
EcalShowerContainmentAnalyzer::EcalShowerContainmentAnalyzer(
  ...
  std::string outfilename = ps.getUntrackedParameter<std::string>(
      "OutputFile", "testContCorrect.root");
...
void EcalShowerContainmentAnalyzer::analyze(edm::Event const &iEvent,
  ...
  iEvent.getByLabel("ecal2006TBHodoscopeReconstructor",
                    "EcalTBHodoscopeRecInfo", pHodoscope);
...
double EcalShowerContainmentAnalyzer::getAdcToGevConstant(
...
double EcalShowerContainmentAnalyzer::energy3x3(const edm::Event &iEvent,
                                                EBDetId centerXtal) {
  using namespace edm;
  using namespace std;
