...
CLHEP::Hep3Vector
TBPositionCalc::CalculateTBPos(const std::vector<EBDetId> &upassedDetIds,
                               int myCrystal,
  ...
  CLHEP::Hep3Vector cmsPos =
      CalculateCMSPos(passedDetIds, myCrystal, passedRecHitsMap);
...
CLHEP::Hep3Vector
TBPositionCalc::CalculateCMSPos(const std::vector<EBDetId> &passedDetIds,
                                int myCrystal,
                                EcalRecHitCollection const *passedRecHitsMap) {
  ...
  float depth = 0.;
  if (eTot <= 0.) {
    edm::LogError("NegativeClusterEnergy")
        << "cluster with negative energy: " << eTot << ", setting depth to 0.";
  } else {
  ...
  double radius =
      sqrt(center_pos.x() * center_pos.x() + center_pos.y() * center_pos.y() +
