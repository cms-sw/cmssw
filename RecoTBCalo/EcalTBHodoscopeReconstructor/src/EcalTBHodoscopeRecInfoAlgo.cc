...
EcalTBHodoscopeRecInfoAlgo::EcalTBHodoscopeRecInfoAlgo(
    int fitMethod, const std::vector<double> &planeShift,
...
void EcalTBHodoscopeRecInfoAlgo::clusterPos(float &x, float &xQuality,
                                            const int &ipl, const int &xclus,
                                            const int &width) const {
  if (width == 1) {
    // Single fiber
    x = (myGeometry_->getFibreLp(ipl, xclus) +
         myGeometry_->getFibreRp(ipl, xclus)) /
            2.0 -
        planeShift_[ipl];
    xQuality = (myGeometry_->getFibreRp(ipl, xclus) -
                myGeometry_->getFibreLp(ipl, xclus));
  } else if (width == 2) {
    // Two half overlapped fibers
    x = (myGeometry_->getFibreLp(ipl, xclus + 1) +
         myGeometry_->getFibreRp(ipl, xclus)) /
            2.0 -
        planeShift_[ipl];
    xQuality = (myGeometry_->getFibreRp(ipl, xclus) -
                myGeometry_->getFibreLp(ipl, xclus + 1));
  } else {
    // More then two fibers case
    x = (myGeometry_->getFibreLp(ipl, xclus) +
         myGeometry_->getFibreRp(ipl, xclus + width - 1)) /
            2.0 -
        planeShift_[ipl];
    xQuality = (myGeometry_->getFibreRp(ipl, xclus + width - 1) -
                myGeometry_->getFibreLp(ipl, xclus));
  }
}

void EcalTBHodoscopeRecInfoAlgo::fitHodo(float &x, float &xQuality,
                                         const int &ipl, const int &nclus,
...
void EcalTBHodoscopeRecInfoAlgo::fitLine(
    float &x, float &xSlope, float &xQuality, const int &ipl1,
    const int &nclus1, const std::vector<int> &xclus1,
    const std::vector<int> &wclus1, const int &ipl2, const int &nclus2,
    const std::vector<int> &xclus2, const std::vector<int> &wclus2) const {
  if (nclus1 == 0) { // Fit with one plane
    fitHodo(x, xQuality, ipl2, nclus2, xclus2, wclus2);
    xSlope =
        0.0; //?? Should we put another number indicating that is not fitted
    return;
  }
...
EcalTBHodoscopeRecInfo EcalTBHodoscopeRecInfoAlgo::reconstruct(
    const EcalTBHodoscopeRawInfo &hodoscopeRawInfo) const {
  // Reset Hodo data
  float x, y = -100.0;
  ...
  for (int ipl = 0; ipl < myGeometry_->getNPlanes(); ipl++) {
    ...
    if (nhits > 0) {
      ...
      while (nh > 0) {
        ...
        nh--;
        do {
          while (last < myGeometry_->getNFibres() &&
                 hodoscopeRawInfo[ipl][last]) {
            last++;
            nh--;
          } // end
          if (last + 1 < myGeometry_->getNFibres() &&
              hodoscopeRawInfo[ipl][last + 1]) { // Skip 1 fibre hole
            last += 2;
            nh--;
            // std::cout << "Skip fibre " << ipl << " " << first << " "<< last
            // << std::endl;
          } else {
            break;
          }
  ...
  if (fitMethod_ == 0) {
    fitLine(x, xSlope, xQuality, 0, nclus[0], xclus[0], wclus[0], // X1
            2, nclus[2], xclus[2], wclus[2]);                     // X2
    fitLine(y, ySlope, yQuality, 1, nclus[1], xclus[1], wclus[1], // Y1
