...
public:
  ...
  reconstruct(const EcalTBHodoscopeRawInfo &hodoscopeRawInfo) const;
...
private:
  ...
  void clusterPos(float &x, float &xQuality, const int &ipl, const int &xclus,
                  const int &wclus) const;

  void fitHodo(float &x, float &xQuality, const int &ipl, const int &nclus,
               const std::vector<int> &xclus,
               const std::vector<int> &wclus) const;

  void fitLine(float &x, float &xSlope, float &xQuality, const int &ipl1,
               const int &nclus1, const std::vector<int> &xclus1,
               const std::vector<int> &wclus1, const int &ipl2,
               const int &nclus2, const std::vector<int> &xclus2,
