...
void HcalTBSlowDataUnpacker::unpackMaps(
    const FEDRawData &raw, std::map<std::string, std::string> &strings,
...
void HcalTBSlowDataUnpacker::unpack(const FEDRawData &raw, HcalTBRunData &htbrd,
  ...
  htbrd.setRunData(sd_strmap["RunType"].c_str(), sd_strmap["Beam.Mode"].c_str(),
                   sd_dblmap["Beam.Energy"]);

  htbep.setHFtableCoords(sd_dblmap["HFTable.X"], sd_dblmap["HFTable.Y"],
                         sd_dblmap["HFTable.V"]);
