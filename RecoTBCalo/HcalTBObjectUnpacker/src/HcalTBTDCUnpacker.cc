...
void HcalTBTDCUnpacker::setCalib(
    const std::vector<std::vector<std::string>> &calibLines_) {
  for (int i = 0; i < 161; i++) {
    tdc_ped[i] = 0.;
    tdc_convers[i] = 1.;
  }
  for (unsigned int ii = 0; ii < calibLines_.size(); ii++) {
    ...
    if (calibLines_[ii][0] == "TDC") {
      if (calibLines_[ii].size() == 4) {
        ...
        //        tdc_ped[channel],tdc_convers[channel]);
      } else {
        throw cms::Exception("Incomplete configuration")
            << "Wrong TDC configuration format : expected 3 parameters, got "
            << calibLines_[ii].size() - 1;
      }
    } // End of the TDCs
    ...
    if (calibLines_[ii][0] == "WC") {
      if (calibLines_[ii].size() == 6) {
        ...
        //         printf("Got WC plane %i b0 %f, b1 %f, mean %f, sigma
        //         %f\n",plane,
        //		 wc_[plane].b0,wc_[plane].b1,wc_[plane].mean,wc_[plane].sigma);
      } else {
        throw cms::Exception("Incomplete configuration")
            << "Wrong Wire Chamber configuration format : expected 5 "
               "parameters, got "
            << calibLines_[ii].size() - 1;
      }
...
void HcalTBTDCUnpacker::unpack(const FEDRawData &raw, HcalTBEventPosition &pos,
...
void HcalTBTDCUnpacker::unpackHits(const FEDRawData &raw,
  ...
  if (tdc->n_max_hits != 192) {
    const CombinedTDCQDCDataFormat *qdctdc =
        (const CombinedTDCQDCDataFormat *)raw.data();
...
void HcalTBTDCUnpacker::reconstructTiming(const std::vector<Hit> &hits,
  ...
  timing.setTimes(trigger_time, ttc_l1a_time, laser_flash, qie_phase,
                  TOF1S_time, TOF1J_time, TOF2S_time, TOF2J_time);
...
void HcalTBTDCUnpacker::reconstructWC(const std::vector<Hit> &hits,
                                      HcalTBEventPosition &ep) const {
  ...
  for (int plane = 0; plane < PLANECOUNT; plane++) {
    ...
    for (std::vector<Hit>::const_iterator j = hits.begin(); j != hits.end();
         j++) {
    ...
    for (int ii = 0; ii < n1; ii++) {
      ...
      for (int jj = 0; jj < n2; jj++) {
        for (int ll = 0; ll < nA; ll++) {
          float dsum =
              fabs(wc_[plane].mean - hits1[ii] - hits2[jj] + 2.0 * hitsA[ll]);
    ...
    if (includeUnmatchedHits_ ||
        plane > 9) // unmatched hits (all pairs get in here)
      for (int ii = 0; ii < n1; ii++) {
        if (hits1[ii] < -99990)
          continue;
        for (int jj = 0; jj < n2; jj++) {
