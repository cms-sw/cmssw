...
void HcalTBTriggerDataUnpacker::unpack(const FEDRawData &raw,
                                       HcalTBTriggerData &htbtd) const {

  // Use the size to determine which format we have received:
  //
  if (raw.size() == 80) { // "old" test beam trigger format

    const oldTriggerDataFormat *oldtrgblk =
        (const oldTriggerDataFormat *)(raw.data());
    htbtd.setStandardData(oldtrgblk->orbitNumber, oldtrgblk->triggerNumber,
    ...
    htbtd.setExtendedData(oldtrgblk->triggerWord, oldtrgblk->triggerTime_usec,
                          oldtrgblk->triggerTime_base, oldtrgblk->spillNumber,
                          oldtrgblk->runNumber, oldtrgblk->runNumberSequenceId);
  } else {
    ...
    htbtd.setStandardData(
        newtrgblk->stdBlock.orbitNumber, newtrgblk->stdBlock.eventNumber,
        bunch_id, newtrgblk->stdBlock.flags_daq_ttype,
        newtrgblk->stdBlock.algo_bits_3, newtrgblk->stdBlock.algo_bits_2,
        newtrgblk->stdBlock.algo_bits_1, newtrgblk->stdBlock.algo_bits_0,
        newtrgblk->stdBlock.tech_bits, newtrgblk->stdBlock.gps_1234,
        newtrgblk->stdBlock.gps_5678);

    htbtd.setExtendedData(newtrgblk->triggerWord, newtrgblk->triggerTime_usec,
                          newtrgblk->triggerTime_base, newtrgblk->spillNumber,
                          newtrgblk->runNumber, newtrgblk->runNumberSequenceId);
  }
}
} // namespace hcaltb
