...
void HcalTBSourcePositionDataUnpacker::unpack(
    const FEDRawData &raw, HcalSourcePositionData &hspd) const {

  if (raw.size() < 3 * 8) {
    throw cms::Exception("Missing Data")
        << "No data in the source position data block";
  }
  ...
  hspd.set(sp_dblmap["MESSAGE"],       // double message_counter
           ...
           sp_strmap["INDEX_DESCRIPTION"], sp_strmap["LAST_COMMAND"],
           sp_strmap["MESSAGE"]);
}
} // namespace hcaltb
