...
void HcalTBQADCUnpacker::setCalib(
  ...
  for (unsigned int ii = 0; ii < calibLines_.size(); ii++) {
    if (calibLines_[ii][0] == "QDC") {
      if (calibLines_[ii].size() == 4) {
        ...
        // qdc_ped[channel],qdc_gain[channel]);
      } else {
        throw cms::Exception("Incomplete configuration")
            << "Wrong QADC configuration format: expected 3 parameters, got "
...
void HcalTBQADCUnpacker::unpack(const FEDRawData &raw,
  ...
  if (is04) { /// this is TB04
    const ClassicQADCDataFormat *qadc =
        (const ClassicQADCDataFormat *)raw.data();
    ...
    beamadc.setADCs04(qdc_calib_hits[aMuonV], qdc_calib_hits[aMuonV3],
                      qdc_calib_hits[aMuonV6], qdc_calib_hits[aMuonVH1],
                      qdc_calib_hits[aMuonVH2], qdc_calib_hits[aMuonVH3],
                      qdc_calib_hits[aMuonVH4], qdc_calib_hits[aCerenkov2],
                      qdc_calib_hits[aCerenkov3], qdc_calib_hits[aSCI_VLE],
                      qdc_calib_hits[aSCI_521], qdc_calib_hits[aSCI_528],
                      qdc_calib_hits[aScint1], qdc_calib_hits[aScint2],
                      qdc_calib_hits[aScint3], qdc_calib_hits[aScint4],
                      Ecal7x7);
  } else { /// this is TB06
    ...
    for (unsigned int i = 0; i < qdctdc->n_qdc_hits; i++)
      qdc_calib_hits[i] =
          ((qdctdc->qdc_values[i] & 0xFFF) - qdc_ped[i]) / qdc_gain[i];

    beamadc.setADCs06(qdc_calib_hits[bMuonVF], qdc_calib_hits[bMuonVB],
                      qdc_calib_hits[bMuonV1], qdc_calib_hits[bMuonV2],
                      qdc_calib_hits[bMuonV3], qdc_calib_hits[bMuonV4],
                      qdc_calib_hits[bMuonV5], qdc_calib_hits[bMuonV6],
                      qdc_calib_hits[bMuonV7], qdc_calib_hits[bMuonV8],
                      qdc_calib_hits[bCerenkov1], qdc_calib_hits[bCerenkov2],
                      qdc_calib_hits[bCerenkov3], qdc_calib_hits[bScint1],
                      qdc_calib_hits[bScint2], qdc_calib_hits[bScint3],
                      qdc_calib_hits[bScint4], qdc_calib_hits[bTOF1S],
                      qdc_calib_hits[bTOF1J], qdc_calib_hits[bTOF2S],
                      qdc_calib_hits[bTOF2J], qdc_calib_hits[bSCI_521],
                      qdc_calib_hits[bSCI_528], qdc_calib_hits[bVH1],
                      qdc_calib_hits[bVH2], qdc_calib_hits[bVH3],
                      qdc_calib_hits[bVH4]);
  }
}
