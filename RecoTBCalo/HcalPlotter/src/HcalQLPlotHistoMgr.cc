...
TH1 *HcalQLPlotHistoMgr::GetAHistogram(const HcalDetId &id,
  ...
  sprintf(name, "%s_%s_%d_%d_%d_eid=%d_%d_%d_%d_HTR_%d:%d%c", flavor.c_str(),
          subdetStr.c_str(), id.ieta(), id.iphi(), id.depth(), eid.dccid(),
          eid.spigot(), eid.fiberIndex(), eid.fiberChanId(),
          eid.readoutVMECrateId(), eid.htrSlot(),
          (eid.htrTopBottom() == 1) ? ('t') : ('b'));
...
TH1 *HcalQLPlotHistoMgr::GetAHistogram(const HcalCalibDetId &id,
  ...
  if (chanstring.empty()) {
    chanstring = "Unknown";
    edm::LogInfo("HcalQLPlotHistoMgr::GetAHistogram")
        << "Unknown calibration channel " << id.cboxChannel();
  }

  sprintf(name, "%s_CALIB_%s_%d_%d_chan=%s_eid=%d_%d_%d_%d_HTR_%d:%d%c",
          flavor.c_str(), subdetStr.c_str(), id.ieta(), id.iphi(),
          chanstring.c_str(), eid.dccid(), eid.spigot(), eid.fiberIndex(),
          eid.fiberChanId(), eid.readoutVMECrateId(), eid.htrSlot(),
          (eid.htrTopBottom() == 1) ? ('t') : ('b'));

  return GetAHistogramImpl(name, ht, et);
}

TH1 *HcalQLPlotHistoMgr::GetAHistogramImpl(const char *name, HistType ht,
                                           EventType et) {
  TDirectory *td;
    ...
    case (ENERGY): {
      ...
      case (PEDESTAL):
        ...
        try {
          ...
          edm::LogError("HcalQLPlotHistoMgr::GetAHistogram")
