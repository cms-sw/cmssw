...
//
/**\class HcalQLPlotAnal HcalQLPlotAnal.cc
 RecoTBCalo/HcalQLPlotAnal/src/HcalQLPlotAnal.cc
...
HcalQLPlotAnal::HcalQLPlotAnal(const edm::ParameterSet &iConfig)
    : hcalDigiLabel_(
          iConfig.getUntrackedParameter<edm::InputTag>("hcalDigiTag")),
  ...
  algo_ = new HcalQLPlotAnalAlgos(
      iConfig.getUntrackedParameter<std::string>("outputFilename").c_str(),
      iConfig.getParameter<edm::ParameterSet>("HistoParameters"));

  tok_hbherec_ = consumes<HBHERecHitCollection>(
      iConfig.getUntrackedParameter<edm::InputTag>("hbheRHtag"));
  tok_horec_ = consumes<HORecHitCollection>(
      iConfig.getUntrackedParameter<edm::InputTag>("hoRHtag"));
  tok_hfrec_ = consumes<HFRecHitCollection>(
      iConfig.getUntrackedParameter<edm::InputTag>("hfRHtag"));
  ...
  tok_tb_ = consumes<HcalTBTriggerData>(
      iConfig.getUntrackedParameter<edm::InputTag>("hcalTrigTag"));
}
...
void HcalQLPlotAnal::analyze(const edm::Event &iEvent,
  ...
  if (!trig.isValid()) {
    edm::LogError("HcalQLPlotAnal::analyze")
  ...
  if (!hbhedg.isValid()) {
    edm::LogWarning("HcalQLPlotAnal::analyze")
