...
/**\class HcalQLPlotAnal HcalQLPlotAnal.cc
 MyEDProducts/HcalPlotter/src/HcalQLPlotAnal.cc
...
void HcalQLPlotAnalAlgos::processRH(const HBHERecHitCollection &hbherhc,
  ...
  for (it = hbherhc.begin(); it != hbherhc.end(); it++) {
    ...
    TH1 *ehist =
        histos_->GetAHistogram(id, eid, HcalQLPlotHistoMgr::ENERGY, triggerID_);
    if (ehist) {
      ehist->Fill(it->energy());
    }

    TH1 *thist =
        histos_->GetAHistogram(id, eid, HcalQLPlotHistoMgr::TIME, triggerID_);
    if (thist) {
      thist->Fill(it->time());
    }
  }
...
void HcalQLPlotAnalAlgos::processDigi(const HBHEDigiCollection &hbhedigic) {
  ...
  for (it = hbhedigic.begin(); it != hbhedigic.end(); it++) {
    ...
    TH1 *phist =
        histos_->GetAHistogram(id, eid, HcalQLPlotHistoMgr::PULSE, triggerID_);
    if (phist) {
    ...
    if (triggerID_ == HcalQLPlotHistoMgr::PEDESTAL) {
      phist =
          histos_->GetAHistogram(id, eid, HcalQLPlotHistoMgr::ADC, triggerID_);
      if (phist) {
        for (int bin = 0; bin < it->size(); bin++)
...
HcalCalibRecHit HcalQLPlotAnalAlgos::recoCalib(const HcalCalibDataFrame &cdigi,
  ...
  float t0 =
      (maxI > 0)
          ? (fabs((cdigi[maxI - 1].nominal_fC() - nominal_ped)) * calibFC2GeV)
          : 0.0;
