...
//========================================================================
{
  ...
  rootfile_ = iConfig.getUntrackedParameter<std::string>(
      "rootfile", "ecalSimpleTBanalysis.root");
  ...
  hodoRecInfoCollection_ =
      iConfig.getParameter<std::string>("hodoRecInfoCollection");
  hodoRecInfoProducer_ =
      iConfig.getParameter<std::string>("hodoRecInfoProducer");
  tdcRecInfoCollection_ =
      iConfig.getParameter<std::string>("tdcRecInfoCollection");
  ...
  std::cout << "EcalSimpleTBAnalyzer: fetching hitCollection: "
            << hitCollection_.c_str() << " produced by " << hitProducer_.c_str()
            << std::endl;
}
...
void EcalSimpleTBAnalyzer::beginJob() {
  ...
  // Amplitude vs TDC offset
  h_ampltdc = new TH2F("h_ampltdc", "Max Amplitude vs TDC offset", 100, 0., 1.,
                       1000, 0., 4000.);

  // Reconstructed energies
  h_tableIsMoving =
      new TH1F("h_tableIsMoving", "TableIsMoving", 100000, 0., 100000.);
  ...
  h_e1e9_mapx =
      new TH2F("h_e1e9_mapx", "E1/E9 vs X", 80, -20, 20, 600, 0., 1.2);
  h_e1e9_mapy =
      new TH2F("h_e1e9_mapy", "E1/E9 vs Y", 80, -20, 20, 600, 0., 1.2);

  h_e1e25_mapx =
      new TH2F("h_e1e25_mapx", "E1/E25 vs X", 80, -20, 20, 600, 0., 1.2);
  h_e1e25_mapy =
      new TH2F("h_e1e25_mapy", "E1/E25 vs Y", 80, -20, 20, 600, 0., 1.2);

  h_e9e25_mapx =
      new TH2F("h_e9e25_mapx", "E9/E25 vs X", 80, -20, 20, 600, 0., 1.2);
  h_e9e25_mapy =
      new TH2F("h_e9e25_mapy", "E9/E25 vs Y", 80, -20, 20, 600, 0., 1.2);

  h_Shape_ =
      new TH2F("h_Shape_", "Xtal in Beam Shape", 250, 0, 10, 350, 0, 3500);
}
...
void EcalSimpleTBAnalyzer::analyze(const edm::Event &iEvent,
  ...
  // std::cout << "EcalSimpleTBAnalyzer::analyze getting product with label: "
  // << digiProducer_.c_str()<< " prodname: " << digiCollection_.c_str() <<
  // endl;
  ...
  if (pdigis.isValid()) {
    ...
    edm::LogError("EcalSimpleTBAnalyzerError")
        << "Error! can't get the product " << digiCollection_;
  }
  ...
  if (phits.isValid()) {
    ...
    edm::LogError("EcalSimpleTBAnalyzerError")
        << "Error! can't get the product " << hitCollection_;
  }
  ...
  if (pHodo.isValid()) {
    ...
    edm::LogError("EcalSimpleTBAnalyzerError")
        << "Error! can't get the product " << hodoRecInfoCollection_;
  }
  ...
  if (pTDC.isValid()) {
    ...
    edm::LogError("EcalSimpleTBAnalyzerError")
        << "Error! can't get the product " << tdcRecInfoCollection_;
  }
  ...
  if (pEventHeader.isValid()) {
    ...
    edm::LogError("EcalSimpleTBAnalyzerError")
        << "Error! can't get the product " << eventHeaderProducer_;
  }
  ...
  if (xtalInBeam_.null()) {
    xtalInBeam_ =
        EBDetId(1, evtHeader->crystalInBeam(), EBDetId::SMCRYSTALMODE);
  ...
  //    for(EBUncalibratedRecHitCollection::const_iterator ithit =
  //    hits->begin(); ithit != hits->end(); ++ithit)
  //      {
  ...
  // Something like
  // EBFixedWindowSelector<EcalUncalibratedRecHit>
  // Simple5x5Matrix(hits,maxHitId,5,5); std::vector<EcalUncalibratedRecHit>
  // Energies5x5 = Simple5x5Matrix.getHits();
  ...
  for (unsigned int icry = 0; icry < 25; icry++) {
    ...
    ///       std::cout << "** Xtal in the matrix **** row " << row  << ",
    ///       column " << column << ", xtal " << Xtals5x5[icry].ic() <<
    ///       std::endl;
  }
