...
//========================================================================
{
  ...
  std::cout << "EcalSimple2007H4TBAnalyzer: fetching hitCollection: "
...
void EcalSimple2007H4TBAnalyzer::beginRun(edm::Run const &,
  ...
  //  validIds=theTBGeometry_->getValidDetIds(DetId::Ecal,EcalEndcap);
  //   std::cout << "Found " << validIds.size() << " channels in the geometry"
  //   << std::endl; for (unsigned int i=0;i<validIds.size();++i)
...
void EcalSimple2007H4TBAnalyzer::analyze(edm::Event const &iEvent,
  ...
  // std::cout << "EcalSimple2007H4TBAnalyzer::analyze getting product with
  // label: " << digiProducer_.c_str()<< " prodname: " <<
  // digiCollection_.c_str()
  // << endl;
  ...
  if (pdigis.isValid()) {
    ...
    edm::LogError("EcalSimple2007H4TBAnalyzerError")
  ...
  if (xtalInBeamTmp.null()) {
    ...
    xtalInBeam_ = EEDetId(35 - ((xtalInBeamTmp.ic() - 1) % 20),
                          int(int(xtalInBeamTmp.ic()) / int(20)) + 51, -1);
    std::cout << "Xtal In Beam is " << xtalInBeam_.ic() << xtalInBeam_
              << std::endl;
    for (unsigned int icry = 0; icry < 25; icry++) {
      ...
      if (tempId.ix() < 16 || tempId.ix() > 35 || tempId.iy() < 51 ||
          tempId.iy() > 75)
        ...
        if (!cell)
          continue;
        std::cout << "** Xtal in the matrix **** row " << row << ", column "
                  << column << ", xtal " << Xtals5x5[icry] << " Position "
                  << cell->getPosition(0.) << std::endl;
      }
    }
  } else if (xtalInBeamTmp !=
             EBDetId(1, evtHeader->crystalInBeam(),
                     EBDetId::SMCRYSTALMODE)) // run analysis only on first xtal
                                              // in beam
    return;
  ...
  EEDetId maxHitId(0);
  float maxHit = -999999.;
  for (EEUncalibratedRecHitCollection::const_iterator ithit = hits->begin();
       ithit != hits->end(); ++ithit) {
