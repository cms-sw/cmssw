...
class ROCmTestKernelAdditionModule : public edm::global::EDAnalyzer<> {
public:
  explicit ROCmTestKernelAdditionModule(edm::ParameterSet const& config);
  ~ROCmTestKernelAdditionModule() override = default;
...
ROCmTestKernelAdditionModule::ROCmTestKernelAdditionModule(edm::ParameterSet const& config)
    : size_(config.getParameter<uint32_t>("size")) {}

void ROCmTestKernelAdditionModule::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(ROCmTestKernelAdditionModule);
