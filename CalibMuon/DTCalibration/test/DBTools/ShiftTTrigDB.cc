  ...
  for (vector<DTSuperLayer*>::const_iterator sl = dtSupLylist.begin();
	...
	if(((*sl)->id().sector() == (*ch)[1]) ||  (*ch)[1] == 999){
	  if(((*sl)->id().station() == (*ch)[2]) ||  (*ch)[2] == 999){
	    ...
	    tTrigNewMap->setSLTtrig((*sl)->id(), newTTrigMean, ttrigSigma, DTTimeUnits::ns);
	    ttrigShifted = true;
	    if(debug){
    ...
    if(!ttrigShifted){
      ...
      tTrigNewMap->setSLTtrig((*sl)->id(), ttrigMean, ttrigSigma, DTTimeUnits::ns);
      if(debug){
