...
int main() {
  ...
  while (!olddata.eof()) {
    olddata >> old_index >> old_elem33 >> old_elem34 >> old_elem44 >> old_elem35 >> old_elem45 >> old_elem55 >>
  ...
  while (!newdata.eof()) {
    newdata >> new_index >> new_elem33 >> new_elem34 >> new_elem44 >> new_elem35 >> new_elem45 >> new_elem55 >>
  ...
  for (int i = 0; i < MAX_SIZE; ++i) {
    ...
    for (int k = 0; k < new_index_id.size() - 1; k++) {
      ...
      if (counter == counter1) {
        ...
        myXtalkFile << counter << "  " << diff_el33[k] << "  " << diff_el34[k] << "  " << diff_el35[k] << "  "
