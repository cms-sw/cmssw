...
int main() {
  ...
  while (!dbdata.eof()) {
    dbdata >> index >> elem33 >> elem34 >> elem44 >> elem35 >> elem45 >> elem55 >> elem46 >> elem56 >> elem66 >>
        elem57 >> elem67 >> elem77 >> flag >> flag1;
                                  ...
                                  if (Elem46[i] * Elem46[i] < Elem44[i] && Elem46[i] * Elem46[i] < Elem66[i]) {
                                    if (Elem56[i] * Elem56[i] < Elem55[i] && Elem56[i] * Elem56[i] < Elem66[i]) {
                                      if (Elem57[i] * Elem57[i] < Elem55[i] && Elem57[i] * Elem57[i] < Elem77[i]) {
                                        myMatrixFile << index_id[i] << "  " << Elem33[i] << "  " << Elem34[i] << "  "
                                                     << Elem44[i] << "  " << Elem35[i] << "  " << Elem45[i] << "  "
                                                     << Elem55[i] << "  " << Elem46[i] << "  " << Elem56[i] << "  "
                                                     << Elem66[i] << "  " << Elem57[i] << "  " << Elem67[i] << "  "
                                                     << Elem77[i] << std::endl;
                                      }
                                    }
                                  }
