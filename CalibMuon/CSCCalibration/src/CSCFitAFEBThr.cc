...
bool CSCFitAFEBThr::ThresholdNoise(const std::vector<float> & inputx, 
  ...
  /// do not fit input[y]==npulses and input[y]==0.0; calculate binom. error;
  std::vector<float> x;
  std::vector<float> y;
  ...
  int isum=0;
  for(unsigned int i=0;i<inputx.size();i++) if(inputy[i]>0.0) isum++;
  if(isum==0) {
    ndf=-4;
    return status;
  }
  ...
  for(unsigned int i=inputx.size()-1;i>=0;i--) {
    if(inputy[i]<(float)npulses) nbeg--;
    if(inputy[i]==(float)npulses) break;
  }

  for(unsigned int i=nbeg;i<inputx.size();i++) {
    if(inputy[i]>0.0) {
      x.push_back(inputx[i]); 
      y.push_back(inputy[i]);

      float p=inputy[i]/(float)npulses;
      float s=(float)npulses * p * (1.0-p);
      s=sqrt(s);
      ery.push_back(s);
    }			       
  }
  ...
  theOBJfun->setNorm((float)npulses);
