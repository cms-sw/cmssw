...
DTTTrigSyncTOFCorr::DTTTrigSyncTOFCorr(const edm::ParameterSet &config)
    : // The fixed t0 (or t_trig) to be subtracted to digi time (ns)
      theTTrig(
          config.getParameter<double>("tTrig")), // FIXME: Default was 500 ns
      // Velocity of signal propagation along the wire (cm/ns)
      theVPropWire(config.getParameter<double>(
          "vPropWire")), // FIXME: Default was 24.4 cm/ns
      ...
      theTOFCorrType(
          config.getParameter<int>("tofCorrType")), // FIXME: Default was 1
      debug(config.getUntrackedParameter<bool>("debug")),
      theBXspace(config.getUntrackedParameter<double>(
          "bxSpace", 25.)) // spacing of BX in ns
{}

DTTTrigSyncTOFCorr::~DTTTrigSyncTOFCorr() {}

double DTTTrigSyncTOFCorr::offset(const DTLayer *layer, const DTWireId &wireId,
  ...
  if (debug) {
    cout << "[DTTTrigSyncTOFCorr] Offset (ns): "
...
double DTTTrigSyncTOFCorr::offset(const DTWireId &wireId) const {
  ...
  // FIXME: This should be considered only if the Digi is constructed from a
  // float....
...
double DTTTrigSyncTOFCorr::emulatorOffset(const DTWireId &wireId, double &tTrig,
                                          double &t0cell) const {
  tTrig = theTTrig;
  t0cell = 0.;
