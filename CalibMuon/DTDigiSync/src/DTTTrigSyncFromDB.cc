...
void DTTTrigSyncFromDB::setES(const EventSetup &setup) {
  ...
  if (debug) {
    cout << "[DTTTrigSyncFromDB] ttrig version: " << tTrigMap->version()
         << endl;
  }
}

double DTTTrigSyncFromDB::offset(const DTLayer *layer, const DTWireId &wireId,
                                 const GlobalPoint &globPos, double &tTrig,
                                 double &wirePropCorr, double &tofCorr) const {
  // Correction for the float to int conversion while writeing the ttrig in ns
  // into an int variable (half a bin on average)
  // FIXME: this should disappear as soon as the ttrig object will become a
  // float
  //   static const float f2i_convCorr = (25./64.); // ns //FIXME: check how the
  ...
  if (doWirePropCorrection) {
    switch (theWirePropCorrType) {
      // The ttrig computed from the timebox accounts on average for the signal
      // propagation time from the center of the wire to the frontend. Here we
      // just have to correct for the distance of the hit from the wire center.
    case 0: {
      float wireCoord = layer->toLocal(globPos).y();
      wirePropCorr = -wireCoord / theVPropWire;
      break;
      // FIXME: What if hits used for the time box are not distributed uniformly
      // along the wire?
    }
    ...
    default: {
      throw cms::Exception("[DTTTrigSyncFromDB]")
          << " Invalid parameter: wirePropCorrType = " << theWirePropCorrType
          << std::endl;
      break;
    }
    }
  ...
  if (doTOFCorrection) {
    ...
    case 0: {
      // The ttrig computed from the real data accounts on average for the TOF
      // correction Depending on the granularity used for the ttrig computation
      // we just have to correct for the TOF from the center of the chamber, SL,
      // layer or wire to the hit position. At the moment only SL granularity is
      // considered Correction for TOF from the center of the SL to hit position
    ...
    case 1: {
      // On simulated data you need to consider only the TOF from 3D center of
      // the wire to hit position (because the TOF from the IP to the wire has
      // been already subtracted in the digitization:
      // SimMuon/DTDigitizer/DTDigiSyncTOFCorr.cc corrType=2)
      float flightToWire =
          layer
              ->toGlobal(LocalPoint(
                  layer->specificTopology().wirePosition(wireId.wire()), 0.,
                  0.))
              .mag();
    ...
    default: {
      throw cms::Exception("[DTTTrigSyncFromDB]")
          << " Invalid parameter: tofCorrType = " << theTOFCorrType
          << std::endl;
      break;
    }
    }
...
double DTTTrigSyncFromDB::offset(const DTWireId &wireId) const {
  ...
  if (tTrigMap->get(wireId.superlayerId(), ttrigMean, ttrigSigma, kFactor,
                    DTTimeUnits::ns) != 0) {
    cout << "[DTTTrigSyncFromDB]*Error: ttrig not found for SL: "
         << wireId.superlayerId() << endl;
    //     FIXME: LogError.....
  }
...
double DTTTrigSyncFromDB::emulatorOffset(const DTWireId &wireId, double &tTrig,
                                         double &t0cell) const {
  float t0 = 0;
  float t0rms = 0;
  if (doT0Correction) {
