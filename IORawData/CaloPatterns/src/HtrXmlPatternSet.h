...
public:
  ...
  // this is one larger than it 'needs' to be so that the array index can match
  ...
  void Fill(HtrXmlPatternToolParameters *params,
...
public:
  ...
  ChannelPattern *getPattern(int chan) {
    return (chan >= 1 && chan <= 24) ? (&m_patterns[chan - 1]) : (nullptr);
  }
  int getCrate() const { return m_crate; }
  int getSlot() const { return m_slot; }
  int getTB() const { return m_tb; }
...
public:
  HtrXmlPatternSet(int cratesActive[ChannelPattern::NUM_CRATES],
