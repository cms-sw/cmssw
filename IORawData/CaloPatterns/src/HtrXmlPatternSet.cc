...
void ChannelPattern::Fill_by_hand(const HcalElectronicsMap *emap,
  ...
  // if (heid.readoutVMECrateId()!=m_crate) std::cout << "crate: " <<
  // heid.readoutVMECrateId() << "; " << m_crate << std::endl; if
  // (heid.htrSlot()!=m_slot) std::cout << "slot:  " << heid.htrSlot() << "; "
  // << m_slot << std::endl; if (heid.htrTopBottom()!=m_tb) std::cout << "tb: "
  // << heid.htrTopBottom() << "; " << m_tb << std::endl; if
  // (heid.htrChanId()!=m_chan) std::cout << "chan:  " << heid.htrChanId() << ";
  // " << m_chan << std::endl;
  //
  // if (heid.readoutVMECrateId()==14 && heid.htrTopBottom()==0 &&
  // heid.htrChanId()==3) {
...
void ChannelPattern::Fill(HtrXmlPatternToolParameters *params,
  ...
  for (int samples = 0; samples < data->size(); samples++) {
    int index = m_sample_pos + params->m_presamples_per_event -
                data->presamples() + samples;
