...
HtrXmlPatternTool::HtrXmlPatternTool(HtrXmlPatternToolParameters *params) {
  ...
  // slotsOn array index corresponds to physical number (e.g., slotsOn[1] for
  // slot 1)
  ...
  if (cd) {
    ...
    if (hd) {
      ...
      if (cp)
        ...
        std::cerr << "Bad (crate,slot,channel): (" << HEID.readoutVMECrateId()
                  << "," << HEID.htrSlot() << "," << HEID.htrChanId() << ")"
                  << std::endl;
    } else if (m_params->m_show_errors)
      std::cerr << "Bad (crate,slot): (" << HEID.readoutVMECrateId() << ","
                << HEID.htrSlot() << ")" << std::endl;
  } else if (m_params->m_show_errors)
    std::cerr << "Bad (crate): (" << HEID.readoutVMECrateId() << ")"
              << std::endl;
}
...
void HtrXmlPatternTool::writeXML() {
  ...
  if (m_params->m_XML_file_mode == 1) {
    std::string name =
        m_params->m_output_directory + (m_params->m_file_tag) + "_all.xml";
    of = new std::ofstream(name.c_str(),
  ...
  for (int crate = 0; crate < ChannelPattern::NUM_CRATES; crate++) {
    ...
    if (m_params->m_XML_file_mode == 2) {
      ...
      (*of) << "<?xml version='1.0' encoding='UTF-8'?>" << std::endl;
      (*of) << "<CFGBrickSet name='" << m_params->m_file_tag << "'>"
            << std::endl;
    }
      ...
      for (int tb = 0; tb <= 1; tb++) {
        ...
        for (int fiber = 1; fiber <= 8; fiber++) {
          ...
          if (m_params->m_XML_file_mode == 3) {
            std::string name =
                m_params->m_output_directory + (m_params->m_file_tag);
            char cr_name[256];
            snprintf(cr_name, 256, "_crate_%d_slot_%d_tb_%d_fiber_%d.xml",
                     crate, slot, tb, fiber);
            name += cr_name;
            of = new std::ofstream(name.c_str(),
                                   std::ios_base::out | std::ios_base::trunc);
            if (!of->good()) {
              std::cerr << "XML output file " << name << " is bad."
                        << std::endl;
              delete of;
              return;
            }
...
void HtrXmlPatternTool::createHists() {
  ...
  std::string name =
      m_params->m_output_directory + "/" + (m_params->m_file_tag) + ".root";
      ...
      for (int tb = 0; tb <= 1; tb++) {
        ...
        for (int chan = 1; chan <= 24; chan++) {
          ...
          sprintf(hname, "Exact fC Cr%d,%d%s-%d", crate, slot,
                  ((tb == 1) ? ("t") : ("b")), chan);
          TH1 *hp = new TH1F(hname, hname, ChannelPattern::SAMPLES, -0.5,
                             ChannelPattern::SAMPLES - 0.5);
          hp->SetDirectory(nullptr);
          sprintf(hname, "Quantized fC Cr%d,%d%s-%d", crate, slot,
                  ((tb == 1) ? ("t") : ("b")), chan);
          TH1 *hq = new TH1F(hname, hname, ChannelPattern::SAMPLES, -0.5,
                             ChannelPattern::SAMPLES - 0.5);
          hp->SetDirectory(nullptr);
          sprintf(hname, "Encoded fC Cr%d,%d%s-%d", crate, slot,
                  ((tb == 1) ? ("t") : ("b")), chan);
          TH1 *ha = new TH1F(hname, hname, ChannelPattern::SAMPLES, -0.5,
                             ChannelPattern::SAMPLES - 0.5);
