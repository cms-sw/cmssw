...
static inline int setIf(const std::string &name,
...
std::vector<HcalQIESample> HcalFiberPattern::getSamples(int bunch,
  ...
  if (bunch < npresamples)
    throw cms::Exception("InvalidArgument")
        << "Asked for " << npresamples << " presamples with event at bunch "
        << bunch;
  if (nsamples - npresamples + bunch >= (int)(pattern_.size() / 2))
    throw cms::Exception("InvalidArgument")
        << "Asked for " << nsamples << " with event at " << bunch << " and "
        << npresamples << " presamples, but only " << pattern_.size() / 2
