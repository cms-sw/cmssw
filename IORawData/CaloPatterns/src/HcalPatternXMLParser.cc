...
/*
  ...
  <CFGBrick>
    ...
    <Parameter name='CREATIONSTAMP' type='string'>2005-03-08
  11:44:34</Parameter> <Parameter name='FORMATREVISION'
  type='string'>1</Parameter> <Parameter name='TARGETFIRMWARE'
  type='string'>0</Parameter> <Parameter name='GENERALIZEDINDEX'
  type='int'>140211</Parameter> <Data elements='128' encoding='hex'> .. </Data>
  </CFGBrick>
  </pre>
*/
...
void ConfigurationDBHandler::characters(const XMLCh *chars,
                                        const XMLSize_t length) {
  if (m_mode == md_Idle)
    return;
...
void HcalPatternXMLParser::parse(const std::string &xmlDocument,
  ...
  try {
    if (m_parser == nullptr) {
      ...
      m_parser->parser = std::unique_ptr<xercesc::SAX2XMLReader>(
          xercesc::XMLReaderFactory::createXMLReader());
    }

    MemBufInputSource src((const unsigned char *)xmlDocument.c_str(),
                          xmlDocument.length(), "hcal::PatternReader");
