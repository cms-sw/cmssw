...
HtrXmlPattern::HtrXmlPattern(const edm::ParameterSet &iConfig) {
  ...
  m_hand_pattern_number =
      iConfig.getUntrackedParameter<int>("hand_pattern_number");
  ...
  m_toolparameters->m_show_errors =
      iConfig.getUntrackedParameter<bool>("show_errors");
  m_toolparameters->m_presamples_per_event =
      iConfig.getUntrackedParameter<int>("presamples_per_event");
  m_toolparameters->m_samples_per_event =
      iConfig.getUntrackedParameter<int>("samples_per_event");

  m_toolparameters->m_XML_file_mode =
      iConfig.getUntrackedParameter<int>("XML_file_mode");
  m_toolparameters->m_file_tag =
      iConfig.getUntrackedParameter<std::string>("file_tag");
  m_toolparameters->m_user_output_directory =
      iConfig.getUntrackedParameter<std::string>("user_output_directory");
  ...
  m_toolparameters->m_output_directory =
      out_dir + "/" + (m_toolparameters->m_file_tag) + "/";
...
void HtrXmlPattern::analyze(const edm::Event &iEvent,
  ...
  if (hbhe.empty()) {
    ...
    for (i = hbhe.begin(); i != hbhe.end(); i++) {
      ...
      int count = 0;
      for (HBHEDigiCollection::const_iterator j = c.begin(); j != c.end();
           j++) {
