...
void HcalTBWriter::buildTree(const FEDRawDataCollection& raw) {
  ...
  for (int i=0; i<2048; i++) {
    ...
    std::string name;
    if (blockToName_.find(i)!=blockToName_.end()) name=blockToName_[i];
    else {
      char sname[64];
    ...
    if (name=="HCAL_Trigger" || name=="SliceTest_Trigger") trigChunk_=j;
...
void HcalTBWriter::extractEventInfo(const FEDRawDataCollection& raw, const edm::EventID& id) {
  ...
  if (trigChunk_>=0) {
    const newExtendedTrgMsgBlk* tinfo=(const newExtendedTrgMsgBlk*)(chunkList_[trigChunk_]->getData()+2); // assume 2 64-bit words for the CDF header
