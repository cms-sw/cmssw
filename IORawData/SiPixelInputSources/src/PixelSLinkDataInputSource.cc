...
int PixelSLinkDataInputSource::getEventNumberFromFillWords(
    const std::vector<uint64_t> &buffer, uint32_t &totword) {
...
PixelSLinkDataInputSource::PixelSLinkDataInputSource(
    const edm::ParameterSet &pset, const edm::InputSourceDescription &desc)
    : ProducerSourceFromFiles(pset, desc, true),
      m_fedid(pset.getUntrackedParameter<int>("fedid")), m_fileindex(0),
      m_runnumber(pset.getUntrackedParameter<int>("runNumber", -1)),
      m_currenteventnumber(0), m_currenttriggernumber(0),
...
bool PixelSLinkDataInputSource::setRunAndEventInfo(
    edm::EventID &id, edm::TimeValue_t &time,
  ...
  do {
    ...
    edm::LogInfo("PixelSLinkDataInputSource::produce()")
        << "**** event number = " << eventnumber << " global event number "
        << m_currenteventnumber << " data " << std::hex << m_data << std::dec
        << std::endl;
    while ((m_data >> 60) != 0x5) {
      ...
      edm::LogWarning("") << "next data " << std::hex << m_data << std::dec
                          << std::endl;
    ...
    if (count > 0) {
      edm::LogWarning("") << "Had to read " << count
    ...
    if (nfillwords > 0) {
      LogInfo("") << "n fill words = " << nfillwords
    ...
    if (m_currenteventnumber < eventnumber)
      LogError("PixelSLinkDataInputSource")
          ...
          << ") is LARGER than the next event number (" << m_currenteventnumber
          << ")" << std::endl;
