...
namespace vertexTools {
	...
	unsigned int
        ...
        unsigned int    computeSharedTracks(const VertexCompositePtrCandidate &sv2, const std::vector<CandidatePtr> &svTracks, double , float )
                {
                        unsigned int count = 0;
                        for(std::vector<CandidatePtr>::const_iterator iter = svTracks.begin();
                                        iter != svTracks.end(); iter++)
	...
	unsigned int computeSharedTracks(const reco::Vertex &pv, const reco::VertexCompositePtrCandidate &sv, double minTrackWeight,float mindist)
	{
		return computeSharedTracks(pv,sv.daughterPtrVector(),minTrackWeight,mindist);
	}
	unsigned int computeSharedTracks(const reco::Vertex &pv, const reco::Vertex &sv, double minTrackWeight,float )
	{
	...
	unsigned int computeSharedTracks(const reco::VertexCompositePtrCandidate &sv, const reco::VertexCompositePtrCandidate &sv2, double ,float )
	{
