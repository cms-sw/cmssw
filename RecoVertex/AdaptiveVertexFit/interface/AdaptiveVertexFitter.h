...
public:
   ...
   */
  CachingVertex<5>
  vertex(const std::vector<reco::TransientTrack> &) const override;
   ...
   */
  CachingVertex<5>
  vertex(const std::vector<RefCountedVertexTrack> &) const override;
  ...
  /** Fit vertex out of a std::vector of reco::TransientTracks. Uses the
   * specified linearization point.
   */
...
private:
   ...
   * reco::TransientTracks will be extracted. \param vertex The seed to use for
   * the VertexTracks. This position will also be used as the new linearization
   * point. \return The container of VertexTracks which are to be used in the
   * next fit.
   */
  std::vector<RefCountedVertexTrack>
  reLinearizeTracks(const std::vector<RefCountedVertexTrack> &tracks,
   ...
   */
  std::vector<RefCountedVertexTrack>
  reWeightTracks(const std::vector<RefCountedLinearizedTrackState> &,
   ...
   */
  std::vector<RefCountedVertexTrack>
  reWeightTracks(const std::vector<RefCountedVertexTrack> &,
                 const CachingVertex<5> &seed) const;
  ...
  std::vector<RefCountedVertexTrack>
  weightTracks(const std::vector<RefCountedLinearizedTrackState> &,
   ...
   */
  std::vector<RefCountedVertexTrack>
  linearizeTracks(const std::vector<reco::TransientTrack> &,
