...
void sortTracksByPt(std::vector<reco::TransientTrack> &cont) {
  ...
  auto *p_pt2 = pt2;
  std::sort(ind, ind + s,
            [p_pt2](int i, int j) { return p_pt2[i] > p_pt2[j]; });
...
void sortByDistanceToRefPoint(std::vector<RefCountedVertexTrack> &cont,
  ...
  for (auto const &tk : cont) {
    ind[i] = i;
    d2[i++] =
        (tk->linearizedTrack()->track().initialFreeState().position() - ref)
            .mag2();
  }
  // clang can not handle lambdas with variable length arrays
  auto *p_d2 = d2;
...
AdaptiveVertexFitter::AdaptiveVertexFitter(
    const AnnealingSchedule &ann, const LinearizationPointFinder &linP,
    const VertexUpdator<5> &updator,
    const VertexTrackCompatibilityEstimator<5> &crit,
    const VertexSmoother<5> &smoother, const AbstractLTSFactory<5> &ltsf)
    : theNr(0), theLinP(linP.clone()), theUpdator(updator.clone()),
...
AdaptiveVertexFitter::AdaptiveVertexFitter(const AdaptiveVertexFitter &o)
    : theMaxShift(o.theMaxShift), theMaxLPShift(o.theMaxLPShift),
      ...
      theNr(o.theNr), theLinP(o.theLinP->clone()),
      theUpdator(o.theUpdator->clone()), theSmoother(o.theSmoother->clone()),
...
CachingVertex<5> AdaptiveVertexFitter::vertex(
    const vector<reco::TransientTrack> &unstracks) const {
  if (unstracks.size() < 2) {
...
CachingVertex<5>
AdaptiveVertexFitter::vertex(const vector<RefCountedVertexTrack> &tracks,
 ...
 *  The specified LinearizationPointFinder will be used to find the
 * linearization point.
 */
CachingVertex<5>
...
AdaptiveVertexFitter::reLinearizeTracks(
  ...
  for (vector<RefCountedVertexTrack>::const_iterator i = tracks.begin();
    ...
    try {
      RefCountedLinearizedTrackState lTrData =
          theLinTrkFactory->linearizedTrackState(
              linP, (**i).linearizedTrack()->track());
      /*
      RefCountedLinearizedTrackState lTrData =
...
double AdaptiveVertexFitter::getWeight(float chi2) const {
  ...
  if (weight < 1e-20) {
    // LogInfo("RecoVertex/AdaptiveVertexFitter") << "Weight " << weight << " <
    // 0.0!";
    weight = 1e-20;
  };
  return weight;
...
AdaptiveVertexFitter::reWeightTracks(
    ...
    const CachingVertex<5> &vertex) const {
  const VertexState &seed = vertex.vertexState();
  // cout << "[AdaptiveVertexFitter] now reweight around " << seed.position() <<
  // endl;
  theNr++;
  ...
  for (vector<RefCountedLinearizedTrackState>::const_iterator i =
    ...
    try {
      chi2Res =
          theComp->estimate(vertex, *i, std::distance(lTracks.begin(), i));
    } catch (exception const &e) {
    };
    // cout << "[AdaptiveVertexFitter] /estimate " << endl;
    if (!chi2Res.first) {
      // cout << "[AdaptiveVertexFitter] aie... vertex candidate is at  " <<
      // vertex.position() << endl;
...
AdaptiveVertexFitter::weightTracks(
  ...
  for (vector<RefCountedLinearizedTrackState>::const_iterator i =
    ...
    double weight = 0.;
    pair<bool, double> chi2Res =
        theComp->estimate(seedvtx, *i, std::distance(lTracks.begin(), i));
    if (!chi2Res.first) {
...
AdaptiveVertexFitter::fit(const vector<RefCountedVertexTrack> &tracks,
  ...
  if (withPrior) {
    returnVertex = CachingVertex<5>(priorVertexPosition, priorVertexError,
                                    priorVertexPosition, priorVertexError,
                                    initialTracks, 0);
  }
  ...
  {
    cout << "  " <<
  (**i).linearizedTrack()->track().initialFreeState().momentum() << endl;
  }*/
  do {
    ns_trks = 0;
    CachingVertex<5> fVertex = initialVertex;
    // cout << "[AdaptiveVertexFit] step " << step << " at " <<
    // fVertex.position() << endl;
    if ((previousPosition - newPosition).transverse() > theMaxLPShift) {
      // relinearize and reweight.
      // (reLinearizeTracks also reweights tracks)
      // cout << "[AdaptiveVertexFit] relinearize at " <<
      // returnVertex.position() << endl;
      ...
      // cout << "[AdaptiveVertexFit] reweight at " << returnVertex.position()
      // << endl;
      if (gsfIntermediarySmoothing_)
        returnVertex = theSmoother->smooth(returnVertex);
    ...
    for (vector<RefCountedVertexTrack>::const_iterator i =
      ...
      if (nVertex.isValid()) {
        ...
        if (fabs(nVertex.position().z()) > 10000. ||
          ...
          LogInfo("AdaptiveVertexFitter")
              << "Vertex candidate just took off to " << nVertex.position()
              << "! Will discard this update!";
          // 	    //<< "track pt was " <<
          // (**i).linearizedTrack()->track().pt()
          // 					     << "track momentum was " <<
          // (**i).linearizedTrack()->track().initialFreeState().momentum()
          // 					     << "track position was " <<
          // (**i).linearizedTrack()->track().initialFreeState().position()
          // 					     << "track chi2 was " <<
          // (**i).linearizedTrack()->track().chi2()
          // 					     << "track ndof was " <<
          // (**i).linearizedTrack()->track().ndof()
          // 					     << "track w was " <<
          // (**i).weight()
          // 					     << "track schi2 was " <<
          // (**i).smoothedChi2();
