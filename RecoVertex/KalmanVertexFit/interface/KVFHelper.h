...
template <unsigned int N> class KVFHelper {
...
public:
  ...
  typedef typename VertexTrack<N>::RefCountedRefittedTrackState
   ...
   */
  BDpair trackParameterChi2(
      const RefCountedLinearizedTrackState linTrack,
      const RefCountedRefittedTrackState refittedTrackState) const;

  /**
   *  Methode which calculates the chi**2 between the prior and the fitted
   *   track parameters. The track must have been smoothed before calling this
   * method. If not, an Exception will be thrown.
   */
