...
CachingVertex<5> TrimmedVertexFitter::vertex(const std::vector<reco::TransientTrack>& tracks) const {
  ...
  if (!vtces.empty()) {
    ...
    for (auto rt = mytrks.begin(); rt != mytrks.end(); ++rt) {
...
TrimmedVertexFitter* TrimmedVertexFitter::clone() const { return new TrimmedVertexFitter(*this); }
...
void TrimmedVertexFitter::setTrackCompatibilityCut(float cut) { theRector.setTrackCompatibilityCut(cut); }

void TrimmedVertexFitter::setVertexFitProbabilityCut(float cut) { theRector.setVertexFitProbabilityCut(cut); }
