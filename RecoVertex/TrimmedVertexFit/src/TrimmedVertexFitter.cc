...
CachingVertex<5> TrimmedVertexFitter::vertex(
  ...
  if (!vtces.empty()) {
    ...
    for (std::vector<reco::TransientTrack>::const_iterator rt = mytrks.begin();
         rt != mytrks.end(); ++rt) {
      RefCountedLinearizedTrackState lstate =
          lfac.linearizedTrackState(rv.position(), *rt);

      RefCountedVertexTrack vtrk = vfac.vertexTrack(lstate, state, 1.0);
      vtrks.push_back(vtrk);
    };
    return CachingVertex<5>(rv.position(), rv.positionError(), vtrks,
                            rv.totalChiSquared());
  };
...
CachingVertex<5>
TrimmedVertexFitter::vertex(const std::vector<RefCountedVertexTrack> &tracks,
