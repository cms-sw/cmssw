...
public:
   ...
   */
  KinematicState operator()(const reco::TransientTrack &track,
   ...
   */
  KinematicState operator()(const reco::TransientTrack &track,
                            const GlobalPoint &point, const ParticleMass &m,
                            float m_sigma) const;

  /**
   * Operator to create a particle state at point
   * using the FreeTrajectoryState, charge and mass guess for the particle. The
   * state will be created with the reference point taken from the FTS
   */
  KinematicState operator()(const FreeTrajectoryState &state,
                            const ParticleMass &mass, float m_sigma) const;

  /**
   * Operator to create a particle state at point
   * using the FreeTrajectoryState, charge and mass guess for the particle. The
   * state will be created by propagating FTS to the transvese point of closest
  ...
  PerigeeKinematicState operator()(const KinematicState &state,
                                   const GlobalPoint &point) const;

private:
  KinematicState buildState(const FreeTrajectoryState &state,
                            const ParticleMass &mass, float m_sigma) const;
