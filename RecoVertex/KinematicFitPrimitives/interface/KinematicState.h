...
public:
  ...
  KinematicState(const FreeTrajectoryState &state, const ParticleMass &mass,
                 float m_sigma)
      : fts(state), param(state.position().x(), state.position().y(),
                          state.position().z(), state.momentum().x(),
                          state.momentum().y(), state.momentum().z(), mass),
  ...
  GlobalVector magneticFieldInInverseGeV() const {
    return trajectoryParameters().magneticFieldInInverseGeV();
  }

  const MagneticField *magneticField() const {
    return &trajectoryParameters().magneticField();
  }
