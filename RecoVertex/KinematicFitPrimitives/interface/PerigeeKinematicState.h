...
public:
  ...
  const KinematicState &theState() const {
    if (!isValid())
      throw VertexException("PerigeeKinematicState::initial state is requested "
  ...
  const ExtendedPerigeeTrajectoryError &perigeeError() const {
    ...
    if (!(hasError()))
      throw VertexException("PerigeeKinematicState::requesting perigee error "
                            "when none available");
    return cov;
  }
  ...
  const ExtendedPerigeeTrajectoryParameters &perigeeParameters() const {
    if (!(isValid()))
      throw VertexException("PerigeeKinematicState::requesting perigee "
                            "parameters for invalid state");
    return par;
  }
...
private:
  ...
  /*
   ...
   AlgebraicMatrix jacobianExPerigeeToKinematic(const
   ExtendedPerigeeTrajectoryParameters& state, const GlobalPoint& point)const;
  */
  AlgebraicMatrix
  jacobianCurvilinear2Perigee(const FreeTrajectoryState &fts) const;
