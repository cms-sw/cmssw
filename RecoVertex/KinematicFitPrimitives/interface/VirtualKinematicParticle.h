...
public:
  ...
  bool operator==(
      const ReferenceCountingPointer<KinematicParticle> &other) const override;
  ...
  refittedParticle(const KinematicState &state, float chi2, float ndf,
                   KinematicConstraint *cons = nullptr) const override;

  /**
   * Method returning LinearizedTrackState of the particle needed for
   * Kalman flter vertex fit. This implementation uses the
   * ParticleLinearizedTrackStateFactory class.
   */
  RefCountedLinearizedTrackState
  particleLinearizedTrackState(const GlobalPoint &point) const override;
