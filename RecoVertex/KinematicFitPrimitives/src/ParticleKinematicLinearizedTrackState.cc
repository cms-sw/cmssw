...
const AlgebraicVector6 &
ParticleKinematicLinearizedTrackState::constantTerm() const {
  if (!jacobiansAvailable)
    computeJacobians();
  return theConstantTerm;
}

const AlgebraicMatrix63 &
ParticleKinematicLinearizedTrackState::positionJacobian() const {
  if (!jacobiansAvailable)
    computeJacobians();
  return thePositionJacobian;
}

const AlgebraicMatrix64 &
ParticleKinematicLinearizedTrackState::momentumJacobian() const {
  if (!jacobiansAvailable)
    computeJacobians();
...
const AlgebraicVector6 &
ParticleKinematicLinearizedTrackState::parametersFromExpansion() const {
  if (!jacobiansAvailable)
    computeJacobians();
...
AlgebraicVector6
ParticleKinematicLinearizedTrackState::predictedStateParameters() const {
  if (!jacobiansAvailable)
    computeJacobians();
...
ParticleKinematicLinearizedTrackState::predictedStateWeight(int &error) const {
  if (!jacobiansAvailable)
    computeJacobians();
  int i = 0;
...
AlgebraicSymMatrix66
ParticleKinematicLinearizedTrackState::predictedStateError() const {
  if (!jacobiansAvailable)
    computeJacobians();
  return thePredState.perigeeError().covarianceMatrix();
}

// ImpactPointMeasurement
// ParticleKinematicLinearizedTrackState::impactPointMeasurement() const { throw
// VertexException(" ParticleKinematicLinearizedTrackState::impact point
// measurement is not implemented for kinematic classes!");}
...
ParticleKinematicLinearizedTrackState::particle() const {
  return part;
}

bool ParticleKinematicLinearizedTrackState::
operator==(LinearizedTrackState<6> &other) const {
  const ParticleKinematicLinearizedTrackState *otherP =
      dynamic_cast<const ParticleKinematicLinearizedTrackState *>(&other);
  if (otherP == nullptr) {
    throw VertexException(" ParticleKinematicLinearizedTrackState:: don't know "
                          "how to compare myself to non-kinematic track state");
  }
...
ParticleKinematicLinearizedTrackState::createRefittedTrackState(
  ...
  RefCountedRefittedTrackState rst = RefCountedRefittedTrackState(
      new KinematicRefittedTrackState(lst, vectorParameters));
  return rst;
}
...
ParticleKinematicLinearizedTrackState::predictedStateMomentumError() const {
  int error;
  if (!jacobiansAvailable)
    computeJacobians();
  AlgebraicSymMatrix44 res;
  AlgebraicSymMatrix33 m3 =
      thePredState.perigeeError().weightMatrix(error).Sub<AlgebraicSymMatrix33>(
          0, 0);
  res.Place_at(m3, 0, 0);
...
ParticleKinematicLinearizedTrackState::refittedParamFromEquation(
  ...
  if ((momentum(2) * predictedStateMomentumParameters()(2) < 0) &&
      (std::fabs(momentum(2)) > M_PI / 2)) {
...
void ParticleKinematicLinearizedTrackState::checkParameters(
...
void ParticleKinematicLinearizedTrackState::computeChargedJacobians() const {
  ...
  double field = part->currentState()
                     .magneticField()
                     ->inInverseGeV(thePredState.theState().globalPosition())
                     .z();
  ...
  theExpandedParams[4] = z_v - paramPt.z() -
                         (phiAtEP - theExpandedParams[2]) / tan(thetaAtEP) /
                             transverseCurvatureAtEP;
  ...
  thePositionJacobian(4, 0) =
      thePositionJacobian(2, 0) / tan(thetaAtEP) / transverseCurvatureAtEP;
  thePositionJacobian(4, 1) =
      thePositionJacobian(2, 1) / tan(thetaAtEP) / transverseCurvatureAtEP;
  ...
  //   cout<<"parameters for momentum jacobian: curv
  //   "<<transverseCurvatureAtEP<<endl; cout<<"sin phi Atep
  //   "<<sin(phiAtEP)<<endl; cout<<"cos phi at EP "<<cos(phiAtEP)<<endl;
  //   cout<<"upper  part is "<<X*cos(phiAtEP) + Y*sin(phiAtEP) <<endl;
  //   cout<<"lower part
  //   is"<<SS*transverseCurvatureAtEP*transverseCurvatureAtEP<<endl;
  ...
  theMomentumJacobian(2, 2) =
      (Y * cos(phiAtEP) - X * sin(phiAtEP)) / (SS * transverseCurvatureAtEP);
  ...
  theMomentumJacobian(4, 2) = (theMomentumJacobian(2, 2) - 1) / tan(thetaAtEP) /
                              transverseCurvatureAtEP;
  ...
  theConstantTerm = AlgebraicVector6(
      theExpandedParams - thePositionJacobian * expansionPoint -
...
void ParticleKinematicLinearizedTrackState::computeNeutralJacobians() const {
  ...
  theMomentumJacobian(4, 1) =
      theMomentumJacobian(3, 2) * (1 + 1 / (tan(thetaAtEP) * tan(thetaAtEP)));

  theMomentumJacobian(4, 2) =
      (X * sin(phiAtEP) - Y * cos(phiAtEP)) / tan(thetaAtEP);
  theMomentumJacobian(5, 3) = 1;
