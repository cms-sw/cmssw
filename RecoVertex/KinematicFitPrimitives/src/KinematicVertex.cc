...
KinematicVertex::KinematicVertex(
    ...
    const ReferenceCountingPointer<KinematicVertex> prVertex, float totalChiSq,
    float degreesOfFr)
    : theState(state), theChiSquared(totalChiSq), theNDF(degreesOfFr),
      pVertex(prVertex) {
  vl = true;
...
bool KinematicVertex::
operator==(const ReferenceCountingPointer<KinematicVertex> other) const {
  bool res = false;
  if (*this == *other)
    res = true;
  return res;
...
KinematicVertex::vertexBeforeConstraint() const {
  return pVertex;
}
...
KinematicVertex::operator reco::Vertex() {
  ...
  reco::Vertex vertex(
      reco::Vertex::Point(theState.position()),
      // 	RecoVertex::convertError(theVertexState.error()),
      theState.error().matrix(), chiSquared(), degreesOfFreedom(),
      daughters.size());

  for (std::vector<RefCountedKinematicParticle>::const_iterator i =
           daughters.begin();
       i != daughters.end(); ++i) {

    const TransientTrackKinematicParticle *ttkp =
        dynamic_cast<const TransientTrackKinematicParticle *>(&(**i));
    if (ttkp != nullptr) {
      const reco::TrackTransientTrack *ttt =
          dynamic_cast<const reco::TrackTransientTrack *>(
              ttkp->initialTransientTrack()->basicTransientTrack());
      if ((ttt != nullptr) && (ttt->persistentTrackRef().isNonnull())) {
        reco::TrackRef tr = ttt->persistentTrackRef();
        vertex.add(reco::TrackBaseRef(tr),
                   ttkp->refittedTransientTrack().track(), 1.);
      } else {
        const reco::GsfTransientTrack *ttt =
            dynamic_cast<const reco::GsfTransientTrack *>(
                ttkp->initialTransientTrack()->basicTransientTrack());
        if ((ttt != nullptr) && (ttt->persistentTrackRef().isNonnull())) {
