...
KinematicTree::finalStateParticles() const {
  if (isEmpty() || !(isConsistent())) {
    ...
    if (!(leftFinalParticle())) {
      std::cout << "top particle has no daughters, empty vector returned"
                << std::endl;
    } else {
    ...
    if (!back)
      throw VertexException("KinematicTree::FinalStateParticles; error occured "
                            "while getting back");
    return rs;
  }
}
...
std::pair<bool, RefCountedKinematicParticle>
KinematicTree::motherParticle() const {
  if (isEmpty())
...
std::vector<RefCountedKinematicParticle>
KinematicTree::daughterParticles() const {
  if (isEmpty())
...
bool KinematicTree::findDecayVertex(
    const RefCountedKinematicVertex vert) const {
  if (isEmpty() || !(isConsistent())) {
    throw VertexException(
        "KinematicTree::findParticle; tree is empty or not consistent");
  } else {
...
bool KinematicTree::leftBranchVertexSearch(
    RefCountedKinematicVertex vtx) const {
  bool found = false;
...
void KinematicTree::replaceCurrentParticle(
    RefCountedKinematicParticle newPart) const {
...
void KinematicTree::replaceCurrentVertex(
    RefCountedKinematicVertex newVert) const {
...
void KinematicTree::addTree(RefCountedKinematicVertex vtx, KinematicTree *tr) {
  ...
  if (!fnd)
    throw VertexException("KinematicTree::addTree; Current tree does not "
                          "contain the vertex passed");
