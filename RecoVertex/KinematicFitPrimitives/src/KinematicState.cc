...
KinematicState::KinematicState(const KinematicParameters &parameters,
    ...
    : fts(GlobalTrajectoryParameters(parameters.position(),
                                     parameters.momentum(), charge, field),
          CartesianTrajectoryError(
              error.matrix().Sub<AlgebraicSymMatrix66>(0, 0))),
      param(parameters), err(error), vl(true) {}

bool KinematicState::operator==(const KinematicState &other) const {
  return (kinematicParameters().vector() ==
          other.kinematicParameters().vector()) &&
         (kinematicParametersError().matrix() ==
          other.kinematicParametersError().matrix());
}
