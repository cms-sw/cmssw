...
PerigeeKinematicState::PerigeeKinematicState(const KinematicState &state,
  ...
  cu2pe.Place_at(PerigeeConversions::jacobianCurvilinear2Perigee(
                     state.freeTrajectoryState()),
                 0, 0);
  cu2pe(5, 5) = 1.;
...
/*
AlgebraicMatrix PerigeeKinematicState::jacobianKinematicToExPerigee(const
KinematicState& state, const GlobalPoint& pt)const
{
 ...
 double field =
TrackingTools::FakeField::Field::inGeVPerCentimeter(state.globalPosition()).z();
...
//  jac(1,4) = -(field*signTC/(transverseCurvature*transverseCurvature))*
cos(phi);
//  jac(1,5) = -(field*signTC/(transverseCurvature*transverseCurvature))*
sin(phi);
//  jac(1,6) =
-(field*signTC/(transverseCurvature*transverseCurvature))*tan(theta);
...
AlgebraicMatrix PerigeeKinematicState::jacobianExPerigeeToKinematic(const
ExtendedPerigeeTrajectoryParameters& state, const GlobalPoint& point)const
{
