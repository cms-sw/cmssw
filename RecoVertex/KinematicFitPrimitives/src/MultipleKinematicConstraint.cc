...
void MultipleKinematicConstraint::addConstraint(
    KinematicConstraint *newConst) const {
  if (newConst == nullptr)
...
MultipleKinematicConstraint::value(const AlgebraicVector &exPoint) const {
  if (cts.empty())
    throw VertexException(
        "MultipleKinematicConstraint::value requested for empty constraint");
  if (exPoint.num_row() == 0)
    throw VertexException("MultipleKinematicConstraint::value requested for "
                          "zero Linearization point");
  ...
  if ((inSize % 7) != 0)
    throw VertexException("MultipleKinematicConstraint::linearization point "
                          "has a wrong dimension");

  int total = 0;
  for (std::vector<KinematicConstraint *>::const_iterator i = cts.begin();
       i != cts.end(); i++) {
...
MultipleKinematicConstraint::derivative(const AlgebraicVector &exPoint) const {
  if (cts.empty())
    throw VertexException("MultipleKinematicConstraint::derivative requested "
...
int MultipleKinematicConstraint::numberOfEquations() const {
  int ne = 0;
  if (cts.empty())
    throw VertexException("MultipleKinematicConstraint::number of equations "
                          "requested for empty constraint");
  for (std::vector<KinematicConstraint *>::const_iterator i = cts.begin();
       i != cts.end(); i++) {
...
std::pair<AlgebraicVector, AlgebraicVector> MultipleKinematicConstraint::value(
  ...
  int count = 1;
  for (std::vector<RefCountedKinematicParticle>::const_iterator i = par.begin();
       i != par.end(); i++) {
    for (int j = 1; j < 8; j++) {
      param((count - 1) * 7 + j) =
          (*i)->currentState().kinematicParameters().vector()(j - 1);
    }
    count++;
  }
...
AlgebraicVector MultipleKinematicConstraint::deviations(int nStates) const {
  ...
  if (cts.empty())
    throw VertexException("MultipleKinematicConstraint::deviations requested "
                          "for empty constraint");
  for (std::vector<KinematicConstraint *>::const_iterator i = cts.begin();
       i != cts.end(); i++) {
