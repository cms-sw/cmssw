...
KinematicState TrackKinematicStatePropagator::propagateToTheTransversePCA(
...
namespace {
inline pair<HelixBarrelPlaneCrossingByCircle, BoundPlane::BoundPlanePointer>
planeCrossing(const FreeTrajectoryState &state, const GlobalPoint &point) {
  ...
  double fac =
      state.charge() / state.parameters().magneticFieldInInverseGeV(point).z();
  ...
  HelixBarrelPlaneCrossingByCircle planeCrossing(
      HelixPlaneCrossing::PositionType(inPos.x(), inPos.y(), inPos.z()),
      HelixPlaneCrossing::DirectionType(inMom.x(), inMom.y(), inMom.z()), kappa,
      direction);
  return std::pair<HelixBarrelPlaneCrossingByCircle, Plane::PlanePointer>(
      planeCrossing, plane);
}
} // namespace

bool TrackKinematicStatePropagator::willPropagateToTheTransversePCA(
  ...
  std::pair<HelixBarrelPlaneCrossingByCircle, BoundPlane::BoundPlanePointer>
      cros = planeCrossing(fState, point);
...
TrackKinematicStatePropagator::propagateToTheTransversePCACharged(
  ...
  // mass - momentum projections corellations do change under such a
  // transformation: special Jacobian needed
  ...
  if (thereIsNoCorr) {
    ...
    AlgebraicSymMatrix55 cov2 = ROOT::Math::Similarity(
        prop.jacobian(), fState.curvilinearError().matrix());
    FreeTrajectoryState fts(fPar, CurvilinearTrajectoryError(cov2));

    return KinematicState(
        fts, state.mass(),
        std::sqrt(state.kinematicParametersError().matrix()(6, 6)));

    // KinematicState kRes(fts, state.mass(),
    // std::sqrt(state.kinematicParametersError().matrix()(6,6))); std::cout <<
    // "\n\ncart from final Kstate\n" <<
    // kRes.kinematicParametersError().matrix()
    // << std::endl;
    ...
    // now both transformation jacobians: cartesian to curvilinear and back are
    // done We transform matrix to curv frame, then propagate it and translate
    // it back to cartesian frame.
    AlgebraicSymMatrix66 cov1 = ROOT::Math::Similarity(
        ca2cu, state.kinematicParametersError().matrix());

    /*
      std::cout << "\n\ncurv from Kstate\n" << cov1 << std::endl;
      std::cout << "curv from fts\n" << fState.curvilinearError().matrix() <<
      std::endl;
    */
    ...
    */
    ...
    /*
      KinematicState resK(resPar,resEr,state.particleCharge(),
      state.magneticField()); std::cout << "\ncurv from K prop\n" <<
...
TrackKinematicStatePropagator::propagateToTheTransversePCANeutral(
  ...
  // now both transformation jacobians: cartesian to curvilinear and back are
  // done We transform matrix to curv frame, then propagate it and translate it
  // back to cartesian frame.
  AlgebraicSymMatrix66 cov1 =
      ROOT::Math::Similarity(ca2cu, state.kinematicParametersError().matrix());
