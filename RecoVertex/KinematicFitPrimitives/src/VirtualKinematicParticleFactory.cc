...
RefCountedKinematicParticle VirtualKinematicParticleFactory::particle(
  ...
  if (previousParticle.get() != nullptr) {
    ...
    VirtualKinematicParticle *pr =
        dynamic_cast<VirtualKinematicParticle *>(prp);
    if (pr == nullptr) {
      throw VertexException(
          "KinematicParticleFactoryFromTransientTrack::Previous particle "
          "passed is not TransientTrack based!");
    }
  }
  return ReferenceCountingPointer<KinematicParticle>(
      new VirtualKinematicParticle(kineState, chiSquared, degreesOfFr,
