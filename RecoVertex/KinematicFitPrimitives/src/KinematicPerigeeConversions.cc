...
KinematicPerigeeConversions::extendedPerigeeFromKinematicParameters(
  ...
  //  double field =
  //  MagneticField::inGeVPerCentimeter(state.globalPosition()).z();
  ...
  double epsilon = signEpsilon * sqrt(impactDistance.x() * impactDistance.x() +
...
KinematicState KinematicPerigeeConversions::kinematicState(
    const AlgebraicVector4 &momentum, const GlobalPoint &referencePoint,
    const TrackCharge &charge, const AlgebraicSymMatrix77 &theCovarianceMatrix,
    const MagneticField *field) const {
  AlgebraicMatrix77 param2cart =
      jacobianParameters2Kinematic(momentum, referencePoint, charge, field);
  AlgebraicSymMatrix77 kinematicErrorMatrix =
      ROOT::Math::Similarity(param2cart, theCovarianceMatrix);
  ...
  AlgebraicVector7 par;
  AlgebraicVector4 mm =
      momentumFromPerigee(momentum, referencePoint, charge, field);
...
AlgebraicMatrix77 KinematicPerigeeConversions::jacobianParameters2Kinematic(
  ...
  //   if (charge != 0){
  //    double field =
  //    TrackingTools::FakeField::Field::inGeVPerCentimeter(referencePoint).z();
  //    factor =  -field*charge;
  //    }
  ...
  //   frameTransJ[2][2] = 1;
  //   frameTransJ[6][6] = 1;
  //   frameTransJ[3][3] = - factor * cos(momentum[2])  /
  //   (momentum[0]*momentum[0]); frameTransJ[4][3] = - factor *
  //   sin(momentum[2])  / (momentum[0]*momentum[0]); frameTransJ[5][3] = -
  //   factor / tan(momentum[1])  / (momentum[0]*momentum[0]);
  //
  //   frameTransJ[3][5] = - factor * sin(momentum[1]) / (momentum[0]);
  //   frameTransJ[4][5] =   factor * cos(momentum[1])  / (momentum[0]);
  //   frameTransJ[5][4] = -factor/
  //   (momentum[0]*sin(momentum[1])*sin(momentum[1]));

  return frameTransJ;
}

// Cartesian (px,py,px,m) from extended perigee
AlgebraicVector4 KinematicPerigeeConversions::momentumFromPerigee(
    const AlgebraicVector4 &momentum, const GlobalPoint &referencePoint,
    const TrackCharge &ch, const MagneticField *field) const {
  AlgebraicVector4 mm;
  double pt;
  if (ch != 0) {
    //   pt = abs(MagneticField::inGeVPerCentimeter(referencePoint).z() /
    //   momentum[0]);
