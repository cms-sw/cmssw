...
KinematicRefittedTrackState::KinematicRefittedTrackState(
    const KinematicState &st, const AlgebraicVector4 &mv) {
  state = st;
  momentumAtVertex = mv;
}
...
AlgebraicVector6 KinematicRefittedTrackState::parameters() const {
  ...
  return conv
      .extendedPerigeeFromKinematicParameters(state, state.globalPosition())
      .vector();
}
...
KinematicRefittedTrackState::kinematicParametersCovariance() const {
...
TrajectoryStateOnSurface KinematicRefittedTrackState::trajectoryStateOnSurface(
...
KinematicRefittedTrackState::stateWithNewWeight(const double newWeight) const {
  std::cout << "WARNING: Change weight for Kinematic state called, weigt will "
               "stay to be equal 1."
            << std::endl;
...
KinematicRefittedTrackState::components() const {
