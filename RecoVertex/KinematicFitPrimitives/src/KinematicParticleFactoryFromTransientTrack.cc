...
KinematicParticleFactoryFromTransientTrack::particle(
    const reco::TransientTrack &initialTrack, const ParticleMass &massGuess,
    float chiSquared, float degreesOfFr, float &m_sigma) const {
  ...
  return ReferenceCountingPointer<KinematicParticle>(
      new TransientTrackKinematicParticle(initState, chiSquared, degreesOfFr,
                                          lastConstraint, previousParticle,
                                          propagator, track));
}
...
KinematicParticleFactoryFromTransientTrack::particle(
    const reco::TransientTrack &initialTrack, const ParticleMass &massGuess,
    float chiSquared, float degreesOfFr, float &m_sigma,
...
KinematicParticleFactoryFromTransientTrack::particle(
    const reco::TransientTrack &initialTrack, const ParticleMass &massGuess,
    float chiSquared, float degreesOfFr, const GlobalPoint &expPoint,
    float m_sigma) const {
  //  FreeTrajectoryState st(initialTrack.impactPointTSCP().theState());
  KinematicState initState = builder(initialTrack.impactPointTSCP().theState(),
                                     massGuess, m_sigma, expPoint);
  const reco::TransientTrack *track = &initialTrack;
  KinematicConstraint *lastConstraint = nullptr;
  ReferenceCountingPointer<KinematicParticle> previousParticle = nullptr;
  return ReferenceCountingPointer<KinematicParticle>(
...
KinematicParticleFactoryFromTransientTrack::particle(
  ...
  TransientTrackKinematicParticle *pr =
      dynamic_cast<TransientTrackKinematicParticle *>(prp);
  if (pr == nullptr) {
    throw VertexException(
        "KinematicParticleFactoryFromTransientTrack::Previous particle passed "
        "is not TransientTrack based!");
  } else {
    track = pr->initialTransientTrack();
  }
  //  }else{track = 0;}
  return ReferenceCountingPointer<KinematicParticle>(
      new TransientTrackKinematicParticle(kineState, chiSquared, degreesOfFr,
                                          lastConstraint, previousParticle,
                                          propagator, track));
}
