...
bool VirtualKinematicParticle::
operator==(const KinematicParticle &other) const {
  bool dc = false;

  // first looking if this is an object of the same type
  const KinematicParticle *lp = &other;
  const VirtualKinematicParticle *lPart =
      dynamic_cast<const VirtualKinematicParticle *>(lp);
...
bool VirtualKinematicParticle::
operator==(const ReferenceCountingPointer<KinematicParticle> &other) const {
  bool res = false;
  if (*this == *other)
    res = true;
  return res;
...
KinematicState
VirtualKinematicParticle::stateAtPoint(const GlobalPoint &point) const {
...
VirtualKinematicParticle::refittedParticle(const KinematicState &state,
                                           float chi2, float ndf,
                                           KinematicConstraint *cons) const {
  VirtualKinematicParticle *ncp = const_cast<VirtualKinematicParticle *>(this);
  ReferenceCountingPointer<KinematicParticle> current =
      ReferenceCountingPointer<KinematicParticle>(ncp);
  return ReferenceCountingPointer<KinematicParticle>(
      new VirtualKinematicParticle(state, chi2, ndf, cons, current,
                                   propagator));
}
...
VirtualKinematicParticle::particleLinearizedTrackState(
  ...
  RefCountedKinematicParticle lp =
      ReferenceCountingPointer<KinematicParticle>(cr);
