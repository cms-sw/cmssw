...
int main() {
  ...
  double v[15] = {0.01, -0.01, 0., 0., 0., 0.01, 0., 0.,
                  0.,   0.01,  0., 0., 1., 0.,   1.};
  ...
  cout << "ts inversePtErr "
       << TrajectoryStateAccessor(*ts.freeState()).inversePtError()
       << std::endl;
  ...
  KinematicState ks2(ks.kinematicParameters(), kpe, ks.particleCharge(),
                     ks.magneticField());

  std::cout << "ks2 Par" << ks2.kinematicParameters().vector() << std::endl;
  std::cout << "ks2 Err" << ks2.kinematicParametersError().matrix()
            << std::endl;
  ...
  std::cout << "kst Err" << kst.kinematicParametersError().matrix()
            << std::endl;
  ...
  std::cout << "kst2 Err" << kst2.kinematicParametersError().matrix()
            << std::endl;
