...
convert(const vector<const reco::TransientTrack *> &ptrs) {
...
computeJetTrajectory(const vector<reco::TransientTrack> &trks) {
  /**
  ...
  GlobalTrajectoryParameters ret(
      pos, mom, 0,
...
vector<Cluster1D<reco::TransientTrack>>
...
/* TransientVertex kalmanVertexFit ( const Cluster1D < reco::TransientTrack > &
src )
{
...
MultiVertexBSeeder::vertices(const vector<reco::TransientTrack> &trks,
                             const reco::BeamSpot &s) const {
  return vertices(trks);
}

vector<TransientVertex>
MultiVertexBSeeder::vertices(const vector<reco::TransientTrack> &trks) const {
  ...
  */
  ...
  pair<vector<Cluster1D<reco::TransientTrack>>,
       vector<const reco::TransientTrack *>>
      res;
  res = finder(ips);
#ifdef MVBS_DEBUG
  ...
  for (vector<Cluster1D<reco::TransientTrack>>::const_iterator i =
           res.first.begin();
       i != res.first.end(); ++i) {
