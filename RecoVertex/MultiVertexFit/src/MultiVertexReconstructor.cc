...
vector<vector<TrackAndWeight>>
recover(const vector<TransientVertex> &vtces,
        const vector<reco::TransientTrack> &trks) {
  ...
  for (set<reco::TransientTrack>::const_iterator i = st.begin(); i != st.end();
       ++i) {
...
MultiVertexReconstructor::vertices(const vector<reco::TransientTrack> &trks,
...
vector<TransientVertex> MultiVertexReconstructor::vertices(
  ...
  if (verbose()) {
    cout << "[MultiVertexReconstructor] non-freezing seeder found "
         << tmp.size() << " vertices from " << trks.size() << " tracks."
         << endl;
  }
  ...
  if (verbose()) {
    cout << "[MultiVertexReconstructor] input " << tmp.size()
         << " vertices, output " << ret.size() << " vertices." << endl;
  };
  return ret;
}
...
vector<TransientVertex> MultiVertexReconstructor::vertices(
  ...
  // cout << "[MultiVertexReconstructor] FIXME dont just add up tracks.
  // superpose" << endl;
