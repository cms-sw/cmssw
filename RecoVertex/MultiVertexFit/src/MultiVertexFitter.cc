...
CachingVertex<5> createSeedFromLinPt(const GlobalPoint &gp) {
  return CachingVertex<5>(gp, GlobalError(), vector<RefCountedVertexTrack>(),
                          0.0);
}
...
void MultiVertexFitter::createSeed(const vector<TransientTrack> &tracks) {
  if (tracks.size() > 1) {
    CachingVertex<5> vtx =
        createSeedFromLinPt(theSeeder->getLinearizationPoint(tracks));
    int snr = seedNr();
...
void MultiVertexFitter::createSeed(const vector<TrackAndWeight> &tracks) {
  ...
  for (vector<TrackAndWeight>::const_iterator track = tracks.begin();
    ...
    RefCountedVertexTrack vTrData =
        vTrackFactory.vertexTrack(lTrData, realseed, weight);
...
vector<CachingVertex<5>>
MultiVertexFitter::vertices(const vector<TransientVertex> &vtces,
...
vector<CachingVertex<5>>
MultiVertexFitter::vertices(const vector<CachingVertex<5>> &initials,
                            const vector<TransientTrack> &primaries) {
  clear();
  ...
  for (vector<CachingVertex<5>>::const_iterator vtx = initials.begin();
    ...
    for (vector<TransientTrack>::const_iterator trk = trks.begin();
      ...
      if (!(thePrimaries.count(*trk))) {
        ...
        // cout << "[MultiVertexFitter " << trk->id() << " is not free." <<
        // endl;
      }
...
vector<CachingVertex<5>>
MultiVertexFitter::vertices(const vector<vector<TransientTrack>> &tracks,
                            const vector<TransientTrack> &primaries) {
  clear();
  createPrimaries(primaries);

  for (vector<vector<TransientTrack>>::const_iterator cluster = tracks.begin();
       cluster != tracks.end(); ++cluster) {
    createSeed(*cluster);
  };
  if (verbose()) {
    printSeeds();
...
vector<CachingVertex<5>>
MultiVertexFitter::vertices(const vector<vector<TrackAndWeight>> &tracks,
                            const vector<TransientTrack> &primaries) {
  clear();
  createPrimaries(primaries);

  for (vector<vector<TrackAndWeight>>::const_iterator cluster = tracks.begin();
       cluster != tracks.end(); ++cluster) {
    createSeed(*cluster);
  };
  if (verbose()) {
...
void MultiVertexFitter::updateWeights() {
  ...
  for (set<TransientTrack>::const_iterator trk = thePrimaries.begin();
    ...
    pair<bool, double> result =
        theComp.estimate(seed, theCache.linTrack(seed.position(), *trk));
    double weight = 0.;
    if (result.first)
      weight = theAssComp->phi(result.second);
    theWeights[*trk][seednr] =
        weight; // FIXME maybe "hard" 1.0 or "soft" weight?
  }
  ...
  for (vector<TransientTrack>::const_iterator trk = theTracks.begin();
    ...
    double tot_weight =
        theAssComp->phi(theAssComp->cutoff() * theAssComp->cutoff());

    for (vector<pair<int, CachingVertex<5>>>::const_iterator seed =
             theVertexStates.begin();
         seed != theVertexStates.end(); ++seed) {

      pair<bool, double> result = theComp.estimate(
          seed->second, theCache.linTrack(seed->second.position(), *trk));
      double weight = 0.;
      if (result.first)
        weight = theAssComp->phi(result.second);
      tot_weight += weight;
...
vector<CachingVertex<5>> MultiVertexFitter::fit() {
  ...
  while (updateSeeds() || !(theAssComp->isAnnealed())) {
    if (++ctr >= ctr_max)
      break;
    theAssComp->anneal();
    // lostVertexClaimer(); // was a silly(?) idea to "revive" vertex
    // candidates.
    resetSeedNr();
  };
  ...
  if (verbose()) {
    ...
    cout << "[MultiVertexFitter] remaining seeds: " << theVertexStates.size()
         << endl;
    printWeights();
  };

  vector<CachingVertex<5>> ret;
  for (vector<pair<int, CachingVertex<5>>>::const_iterator i =
           theVertexStates.begin();
       i != theVertexStates.end(); ++i) {
...
void MultiVertexFitter::lostVertexClaimer() {
  ...
  for (vector<pair<int, CachingVertex<5>>>::const_iterator i =
    ...
    /*
    cout << "[MultiVertexFitter] vertex seed " <<
    TransientVertexNamer().name(*i)
