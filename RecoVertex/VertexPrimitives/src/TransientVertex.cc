...
//       TrackBaseRef tbr(tr);
	...
	vertex.add(theOriginalTrack.trackBaseRef(), refittedTrack(theOriginalTrack).track(), trackWeight ( theOriginalTrack ) );
      } else { 
...
using namespace reco;
	...
	for(const auto & theOriginalTrack : theOriginalTracks)
	{
		...
		const CandidatePtrTransientTrack* cptt = dynamic_cast<const CandidatePtrTransientTrack*>(theOriginalTrack.basicTransientTrack());
		if ( cptt==0 )
