...
TransientVertex::TransientVertex()
    : theVertexState(), theOriginalTracks(), theChi2(0), theNDF(0),
      vertexValid(false), withPrior(false), theWeightMapIsAvailable(false),
...
TransientVertex::TransientVertex(const GlobalPoint &pos,
                                 ...
                                 float chi2)
    : theVertexState(pos, posError), theOriginalTracks(tracks), theChi2(chi2),
      theNDF(0), vertexValid(true), withPrior(false),
...
TransientVertex::TransientVertex(const GlobalPoint &priorPos,
                                 ...
                                 const GlobalError &priorErr,
                                 const GlobalPoint &pos, const double time,
                                 const GlobalError &posError,
                                 const std::vector<TransientTrack> &tracks,
                                 float chi2)
    : thePriorVertexState(priorPos, priorTime, priorErr),
      theVertexState(pos, time, posError), theOriginalTracks(tracks),
      theChi2(chi2), theNDF(0), vertexValid(true), withPrior(true),
      theWeightMapIsAvailable(false), theCovMapAvailable(false),
      withRefittedTracks(false) {
  theNDF = 2. * theOriginalTracks.size();
}
...
TransientVertex::TransientVertex(const GlobalPoint &priorPos,
    ...
    : thePriorVertexState(priorPos, priorTime, priorErr),
      theVertexState(pos, time, posError), theOriginalTracks(tracks),
      theChi2(chi2), theNDF(ndf), vertexValid(true), withPrior(true),
      theWeightMapIsAvailable(false), theCovMapAvailable(false),
      withRefittedTracks(false) {}
...
void TransientVertex::refittedTracks(
  ...
  if (refittedTracks.empty())
    throw VertexException("TransientVertex::refittedTracks: No refitted tracks "
                          "stored in input container");
...
AlgebraicMatrix33
TransientVertex::tkToTkCovariance(const TransientTrack &t1,
                                  const TransientTrack &t2) const {
  ...
  if (it != theCovMap.end()) {
    ...
    if (nit != tm.end()) {
      ...
      throw VertexException("TransientVertex::requested Track-to-track "
...
TransientTrack
TransientVertex::originalTrack(const TransientTrack &refTrack) const {
...
TransientTrack
TransientVertex::refittedTrack(const TransientTrack &track) const {
  if (theRefittedTracks.empty())
    throw VertexException("No refitted tracks stored in vertex");
  std::vector<TransientTrack>::const_iterator it =
      find(theOriginalTracks.begin(), theOriginalTracks.end(), track);
...
TransientVertex::operator reco::Vertex() const {
  ...
  Vertex vertex(Vertex::Point(theVertexState.position()),
                ...
                theVertexState.error4D().matrix4D(), theVertexState.time(),
                totalChiSquared(), degreesOfFreedom(),
                theOriginalTracks.size());
  for (std::vector<TransientTrack>::const_iterator i =
           theOriginalTracks.begin();
       i != theOriginalTracks.end(); ++i) {
    //     const TrackTransientTrack* ttt = dynamic_cast<const
    //     TrackTransientTrack*>((*i).basicTransientTrack()); if ((ttt!=0) &&
    //     (ttt->persistentTrackRef().isNonnull()))
    //     {
    //       TrackRef tr = ttt->persistentTrackRef();
    //       TrackBaseRef tbr(tr);
    if (withRefittedTracks) {

      vertex.add((*i).trackBaseRef(), refittedTrack(*i).track(),
                 trackWeight(*i));
    } else {
...
TransientVertex::operator reco::VertexCompositePtrCandidate() const {
  ...
  vtxCompPtrCand.setVertex(
      Candidate::Point(position().x(), position().y(), position().z()));

  Candidate::LorentzVector p4;
  for (std::vector<reco::TransientTrack>::const_iterator tt =
           theOriginalTracks.begin();
       tt != theOriginalTracks.end(); ++tt) {
  ...
  // TODO: if has refitted tracks we should scale the candidate p4 to the
  // refitted one
