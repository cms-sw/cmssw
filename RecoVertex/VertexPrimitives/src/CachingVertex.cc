...
CachingVertex<N>::CachingVertex(const GlobalPoint &pos,
                                ...
                                float totalChiSq)
    : theVertexState(pos, posErr), theChiSquared(totalChiSq), theNDF(0),
      theNDFAvailable(false), theTracks(tks), theCovMapAvailable(false),
      withPrior(false), theValid(true), vertexIs4D(false)

{}

// to be removed
template <unsigned int N>
CachingVertex<N>::CachingVertex(const GlobalPoint &pos, const double time,
                                const GlobalError &posTimeErr,
                                const std::vector<RefCountedVertexTrack> &tks,
                                float totalChiSq)
    : theVertexState(pos, time, posTimeErr), theChiSquared(totalChiSq),
      theNDF(0), theNDFAvailable(false), theTracks(tks),
      theCovMapAvailable(false), withPrior(false), theValid(true),
      vertexIs4D(true)
...
CachingVertex<N>::CachingVertex(const GlobalPoint &pos,
                                ...
                                const std::vector<RefCountedVertexTrack> &tks,
                                float totalChiSq)
    : theVertexState(pos, posWeight), theChiSquared(totalChiSq), theNDF(0),
      theNDFAvailable(false), theTracks(tks), theCovMapAvailable(false),
      withPrior(false), theValid(true), vertexIs4D(false) {}
...
CachingVertex<N>::CachingVertex(const GlobalPoint &pos, const double time,
                                ...
                                const std::vector<RefCountedVertexTrack> &tks,
                                float totalChiSq)
    : theVertexState(pos, time, posTimeWeight), theChiSquared(totalChiSq),
      theNDF(0), theNDFAvailable(false), theTracks(tks),
      theCovMapAvailable(false), withPrior(false), theValid(true),
      vertexIs4D(true) {}
...
CachingVertex<N>::CachingVertex(const AlgebraicVector3 &weightTimesPosition,
                                ...
                                const std::vector<RefCountedVertexTrack> &tks,
                                float totalChiSq)
    : theVertexState(weightTimesPosition, posWeight), theChiSquared(totalChiSq),
      theNDF(0), theNDFAvailable(false), theTracks(tks),
      theCovMapAvailable(false), withPrior(false), theValid(true),
      vertexIs4D(false) {}
...
CachingVertex<N>::CachingVertex(const VertexState &aVertexState,
                                const std::vector<RefCountedVertexTrack> &tks,
                                float totalChiSq)
    : theVertexState(aVertexState), theChiSquared(totalChiSq), theNDF(0),
      theNDFAvailable(false), theTracks(tks), theCovMapAvailable(false),
      withPrior(false), theValid(true), vertexIs4D(aVertexState.is4D()) {}
...
CachingVertex<N>::CachingVertex(const VertexState &aVertexState,
    ...
    : theVertexState(aVertexState), theChiSquared(totalChiSq), theNDF(0),
      theNDFAvailable(false), theTracks(tks), theCovMap(covMap),
      theCovMapAvailable(true), withPrior(false), theValid(true),
...
CachingVertex<N>::CachingVertex(const GlobalPoint &priorPos,
                                ...
                                const std::vector<RefCountedVertexTrack> &tks,
                                float totalChiSq)
    : theVertexState(pos, time, posErr), theChiSquared(totalChiSq), theNDF(0),
      theNDFAvailable(false), theTracks(tks), theCovMapAvailable(false),
...
CachingVertex<N>::CachingVertex(const GlobalPoint &priorPos,
                                ...
                                const std::vector<RefCountedVertexTrack> &tks,
                                float totalChiSq)
    : theVertexState(pos, priorTime, posWeight), theChiSquared(totalChiSq),
      theNDF(0), theNDFAvailable(false), theTracks(tks),
      theCovMapAvailable(false),
      thePriorVertexState(priorPos, priorTime, priorErr), withPrior(true),
      theValid(true), vertexIs4D(true) {}
...
CachingVertex<N>::CachingVertex(const GlobalPoint &priorPos,
    ...
    : theVertexState(weightTimesPosition, posWeight), theChiSquared(totalChiSq),
      theNDF(0), theNDFAvailable(false), theTracks(tks),
      theCovMapAvailable(false), thePriorVertexState(priorPos, priorErr),
...
CachingVertex<N>::CachingVertex(const VertexState &priorVertexState,
    ...
    : theVertexState(aVertexState), theChiSquared(totalChiSq), theNDF(0),
      theNDFAvailable(false), theTracks(tks), theCovMap(covMap),
      theCovMapAvailable(true), thePriorVertexState(priorVertexState),
      withPrior(true), theValid(true),
      vertexIs4D(priorVertexState.is4D() && aVertexState.is4D()) {
  if (theCovMap.empty())
    theCovMapAvailable = false;
...
CachingVertex<N>::CachingVertex()
    : theChiSquared(-1), theNDF(0), theNDFAvailable(false), theTracks(),
      theCovMapAvailable(false), withPrior(false), theValid(false),
      vertexIs4D(false) {}

template <unsigned int N> GlobalPoint CachingVertex<N>::position() const {
  return theVertexState.position();
}

template <unsigned int N> double CachingVertex<N>::time() const {
  return theVertexState.time();
}

template <unsigned int N> GlobalError CachingVertex<N>::error() const {
  return theVertexState.error();
}

template <unsigned int N> GlobalError CachingVertex<N>::error4D() const {
  return theVertexState.error4D();
}

template <unsigned int N> GlobalWeight CachingVertex<N>::weight() const {
  return theVertexState.weight();
}

template <unsigned int N> GlobalWeight CachingVertex<N>::weight4D() const {
...
template <unsigned int N> float CachingVertex<N>::degreesOfFreedom() const {
  if (!theNDFAvailable)
    computeNDF();
  return theNDF;
}

template <unsigned int N> void CachingVertex<N>::computeNDF() const {
  theNDF = 0;
  for (typename std::vector<RefCountedVertexTrack>::const_iterator itk =
           theTracks.begin();
...
template <unsigned int N> CachingVertex<N>::operator TransientVertex() const {
  ...
  for (typename std::vector<RefCountedVertexTrack>::const_iterator i =
    ...
    if (theCovMapAvailable) {
      for (typename std::vector<RefCountedVertexTrack>::const_iterator j =
               (i + 1);
           j != theTracks.end(); ++j) {
  ...
  TransientVertex tv;
  if (withPrior) {
    tv = TransientVertex(priorVertexState(), vertexState(), ttVect,
                         totalChiSquared(), degreesOfFreedom());
  } else {
    tv = TransientVertex(vertexState(), ttVect, totalChiSquared(),
                         degreesOfFreedom());
  }
