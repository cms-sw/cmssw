...
BasicSingleVertexState::BasicSingleVertexState()
    : thePos(GlobalPoint(0, 0, 0)), theTime(dNaN),
      theErr(AlgebraicSymMatrix44()), theWeight(AlgebraicSymMatrix44()),
      theWeightTimesPos(AlgebraicVector4()), theWeightInMix(0.),
      thePosAvailable(false), theTimeAvailable(false), theErrAvailable(false),
      theWeightAvailable(false), theWeightTimesPosAvailable(false),
...
BasicSingleVertexState::BasicSingleVertexState(const GlobalPoint &pos,
    ...
    : thePos(pos), theTime(dNaN), theErr(posErr),
      theWeight(AlgebraicSymMatrix44()), theWeightTimesPos(AlgebraicVector4()),
      theWeightInMix(weightInMix), thePosAvailable(true),
      theTimeAvailable(false), theErrAvailable(true), theWeightAvailable(false),
      theWeightTimesPosAvailable(false), valid(true), vertexIs4D(false) {}

BasicSingleVertexState::BasicSingleVertexState(const GlobalPoint &pos,
                                               const GlobalWeight &posWeight,
                                               const double &weightInMix)
    : thePos(pos), theTime(dNaN), theErr(AlgebraicSymMatrix44()),
      theWeight(posWeight), theWeightTimesPos(AlgebraicVector4()),
      theWeightInMix(weightInMix), thePosAvailable(true),
      theTimeAvailable(false), theErrAvailable(false), theWeightAvailable(true),
      theWeightTimesPosAvailable(false), valid(true), vertexIs4D(false) {}

BasicSingleVertexState::BasicSingleVertexState(
    const AlgebraicVector3 &weightTimesPosition, const GlobalWeight &posWeight,
    const double &weightInMix)
    : thePos(GlobalPoint(0, 0, 0)), theTime(dNaN),
      theErr(AlgebraicSymMatrix44()), theWeight(posWeight),
      theWeightTimesPos(weightTimesPosition[0], weightTimesPosition[1],
                        weightTimesPosition[2], 0),
      theWeightInMix(weightInMix), thePosAvailable(false),
      theTimeAvailable(false), theErrAvailable(false), theWeightAvailable(true),
      theWeightTimesPosAvailable(true), valid(true), vertexIs4D(false) {}
...
BasicSingleVertexState::BasicSingleVertexState(const GlobalPoint &pos,
                                               ...
                                               const double &weightInMix)
    : thePos(pos), theTime(time), theErr(posErr),
      theWeight(AlgebraicSymMatrix44()), theWeightTimesPos(AlgebraicVector4()),
      theWeightInMix(weightInMix), thePosAvailable(true),
      theTimeAvailable(true), theErrAvailable(true), theWeightAvailable(false),
      theWeightTimesPosAvailable(false), valid(true), vertexIs4D(true) {
...
BasicSingleVertexState::BasicSingleVertexState(const GlobalPoint &pos,
                                               ...
                                               const double &weightInMix)
    : thePos(pos), theTime(time), theErr(AlgebraicSymMatrix44()),
      theWeight(posWeight), theWeightTimesPos(AlgebraicVector4()),
      theWeightInMix(weightInMix), thePosAvailable(true),
      theTimeAvailable(true), theErrAvailable(false), theWeightAvailable(true),
      theWeightTimesPosAvailable(false), valid(true), vertexIs4D(true) {
...
BasicSingleVertexState::BasicSingleVertexState(
    ...
    : thePos(GlobalPoint(0, 0, 0)), theTime(dNaN),
      theErr(AlgebraicSymMatrix44()), theWeight(posWeight),
      theWeightTimesPos(weightTimesPosition[0], weightTimesPosition[1],
                        weightTimesPosition[2], weightTimesTime),
      theWeightInMix(weightInMix), thePosAvailable(false),
      theTimeAvailable(false), theErrAvailable(false), theWeightAvailable(true),
      theWeightTimesPosAvailable(true), valid(true), vertexIs4D(true) {
  GlobalWeight timeWeightMat(0., 0., 0., 0., 0., 0., 0., 0., 0., timeWeight);
  theWeight = theWeight + timeWeightMat;
}
...
BasicSingleVertexState::BasicSingleVertexState(
    ...
    const double &weightInMix)
    : thePos(pos), theTime(time), theErr(posTimeErr),
      theWeight(AlgebraicSymMatrix44()), theWeightTimesPos(AlgebraicVector4()),
      theWeightInMix(weightInMix), thePosAvailable(true),
      theTimeAvailable(true), theErrAvailable(true), theWeightAvailable(false),
      theWeightTimesPosAvailable(false), valid(true), vertexIs4D(true) {}
...
BasicSingleVertexState::BasicSingleVertexState(
    const GlobalPoint &pos, const double time,
    const GlobalWeight &posTimeWeight, const double &weightInMix)
    : thePos(pos), theTime(time), theErr(AlgebraicSymMatrix44()),
      theWeight(posTimeWeight), theWeightTimesPos(AlgebraicVector4()),
      theWeightInMix(weightInMix), thePosAvailable(true),
      theTimeAvailable(true), theErrAvailable(false), theWeightAvailable(true),
      theWeightTimesPosAvailable(false), valid(true), vertexIs4D(true) {}

BasicSingleVertexState::BasicSingleVertexState(
    const AlgebraicVector4 &weightTimesPosition, const GlobalWeight &posWeight,
    const double &weightInMix)
    : thePos(GlobalPoint(0, 0, 0)), theTime(dNaN),
      theErr(AlgebraicSymMatrix44()), theWeight(posWeight),
      theWeightTimesPos(weightTimesPosition), theWeightInMix(weightInMix),
      thePosAvailable(false), theTimeAvailable(false), theErrAvailable(false),
      theWeightAvailable(true), theWeightTimesPosAvailable(true), valid(true),
      vertexIs4D(true) {}
...
AlgebraicVector3 BasicSingleVertexState::weightTimesPosition() const {
  ...
  return AlgebraicVector3(theWeightTimesPos[0], theWeightTimesPos[1],
                          theWeightTimesPos[2]);
}
