...
public:
  ...
  TransientVertex(const GlobalPoint &priorPos, const GlobalError &priorErr,
                  ...
                  const std::vector<reco::TransientTrack> &tracks, float chi2);
  TransientVertex(const GlobalPoint &priorPos, const double priorTime,
                  const GlobalError &priorErr, const GlobalPoint &pos,
                  const double time, const GlobalError &posError,
                  const std::vector<reco::TransientTrack> &tracks, float chi2);
  ...
  /** Constructor defining the RecVertex by the prior,
   *  the vertex 3D position and uncertainty, time and uncertainty, the
   * associated tracks, the chi-squared and the number of degrees of freedom.
   *  The ndf can be a float.
   */
  TransientVertex(const GlobalPoint &priorPos, const GlobalError &priorErr,
   ...
   *  2*nb of tracks.
   */
  TransientVertex(const VertexState &prior, const VertexState &state,
                  const std::vector<reco::TransientTrack> &tracks, float chi2);
  ...
  //    */
  //   TransientVertex(const VertexState & state,
  // 		    const std::vector<reco::TransientTrack> & tracks, float
  // chi2, float ndf, 		    const reco::TransientTrackToFloatMap &
  // weightMap);
  ...
  GlobalError positionError() const {
    return theVertexState.is4D() ? theVertexState.error4D()
                                 : theVertexState.error();
  }
  GlobalPoint priorPosition() const { return thePriorVertexState.position(); }
  GlobalError priorError() const {
    return thePriorVertexState.is4D() ? thePriorVertexState.error4D()
                                      : thePriorVertexState.error();
  }
   ...
   * Throws an exception if now refitted tracks are stored ot the track is not
   * found in the list
   */

  reco::TransientTrack
  originalTrack(const reco::TransientTrack &refTrack) const;
   ...
   *   1.0 is assumed for all tracks used and present in the originalTracks()
   * std::vector.
   */
  ...
  /**
   * Returns true if the Track-to-track covariance matrices have been
   * calculated.
   */
