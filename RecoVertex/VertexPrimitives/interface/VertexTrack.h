...
template <unsigned int N> class VertexTrack final : public ReferenceCounted {

public:
  typedef ROOT::Math::SVector<double, N> AlgebraicVectorN;
  typedef ROOT::Math::SMatrix<double, N - 2, N - 2,
                              ROOT::Math::MatRepStd<double, N - 2, N - 2>>
      AlgebraicMatrixMM;
  typedef ROOT::Math::SMatrix<double, 3, N - 2,
                              ROOT::Math::MatRepStd<double, 3, N - 2>>
      AlgebraicMatrix3M;
  ...
  typedef ReferenceCountingPointer<LinearizedTrackState<N>>
  ...
  /**
   * The smoother track-chi2 (can be used to test the track-vertex
   * compatibility). Its value has a meaning only if the smoother has been run
   * after the vertex fit (track-refit) . Otherwise, the value returned is -1.
   */
...
template <unsigned int N> class VertexTrackEqual {
public:
  typedef ReferenceCountingPointer<VertexTrack<N>> RefCountedVertexTrack;
