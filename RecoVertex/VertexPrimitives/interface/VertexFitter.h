...
template <unsigned int N> class VertexFitter {
...
public:
  ...
  /** Fit vertex out of a set of VertexTracks. For the first iteration, the
   * already linearized track will be used.
   */
  virtual CachingVertex<N>
  vertex(const std::vector<typename CachingVertex<N>::RefCountedVertexTrack>
             &tracks) const = 0;
  ...
  /** Fit vertex out of a set of TransientTracks.
   *  The specified point will be used as linearization point, but will NOT be
   * used as prior.
   */
  virtual CachingVertex<N>
  ...
  virtual CachingVertex<N>
  vertex(const std::vector<reco::TransientTrack> &tracks,
         const GlobalPoint &priorPos, const GlobalError &priorError) const = 0;
