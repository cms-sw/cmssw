...
template <unsigned int N> class RefittedTrackState : public ReferenceCounted {
...
public:
  ...
  //   > AlgebraicMatrixN3; typedef
  //   ROOT::Math::SMatrix<double,N-2,3,ROOT::Math::MatRepStd<double,N-2,3> >
  //   AlgebraicMatrixM3;
  typedef ROOT::Math::SMatrix<double, N, N, ROOT::Math::MatRepSym<double, N>>
  ...
  trajectoryStateOnSurface(const Surface &surface) const = 0;
  ...
  virtual TrajectoryStateOnSurface
  trajectoryStateOnSurface(const Surface &surface,
  ...
  virtual ReferenceCountingPointer<RefittedTrackState>
  stateWithNewWeight(const double newWeight) const = 0;

  virtual std::vector<ReferenceCountingPointer<RefittedTrackState>>
  components() const = 0;
