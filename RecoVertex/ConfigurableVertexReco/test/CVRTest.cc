...
void printVertex(const TransientVertex &vtx) {
  cout << " `- pos=(" << vtx.position().x() << ", " << vtx.position().y()
       << ", " << vtx.position().z() << ") chi2=" << vtx.totalChiSquared()
       << " ndf=" << vtx.degreesOfFreedom() << " hr=" << vtx.hasRefittedTracks()
       << endl;
...
CVRTest::CVRTest(const edm::ParameterSet &iconfig)
  ...
  edm::ParameterSet vtxconfig =
      iconfig.getParameter<edm::ParameterSet>("vertexreco");
