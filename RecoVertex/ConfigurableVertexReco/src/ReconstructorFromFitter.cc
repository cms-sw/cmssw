...
ReconstructorFromFitter::ReconstructorFromFitter(
    std::unique_ptr<AbstractConfFitter> &&f)
    : theFitter(f.release()) {}

vector<TransientVertex>
ReconstructorFromFitter::vertices(const vector<reco::TransientTrack> &t) const {
  ...
  try {
    ...
    edm::LogWarning("ReconstructorFromFitter")
...
vector<TransientVertex>
ReconstructorFromFitter::vertices(const vector<reco::TransientTrack> &t,
...
ReconstructorFromFitter::ReconstructorFromFitter(
    const ReconstructorFromFitter &o)
...
void ReconstructorFromFitter::configure(const edm::ParameterSet &s) {
  // this looks better than changing the data member to be non-const ptr and
  // allow changes in all calls
