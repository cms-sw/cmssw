...
CachingVertex<5>
AbstractConfFitter::vertex(const std::vector<reco::TransientTrack> &t) const {
...
CachingVertex<5>
AbstractConfFitter::vertex(const std::vector<RefCountedVertexTrack> &tracks,
                           const reco::BeamSpot &spot) const {
  return theFitter->vertex(tracks, spot);
}

CachingVertex<5>
AbstractConfFitter::vertex(const std::vector<reco::TransientTrack> &tracks,
