...
// #include
// "RecoVertex/ConfigurableVertexReco/interface/ConfigurableTrimmedKalmanFinder.h"
...
const AnnealingSchedule *schedule(const edm::ParameterSet &m) {
  return new GeometricAnnealing(m.getParameter<double>("sigmacut"),
...
void ConfigurableMultiVertexFitter::configure(const edm::ParameterSet &n) {
  ...
  const VertexReconstructor *ini =
      initialiser(m.getParameter<edm::ParameterSet>("ini"));
  if (theRector)
    delete theRector;
...
namespace {
const ConfRecoBuilder<ConfigurableMultiVertexFitter> t("mvf",
                                                       "Multi Vertex Fitter");
}
