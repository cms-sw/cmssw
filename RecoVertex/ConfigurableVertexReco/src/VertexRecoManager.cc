...
void VertexRecoManager::registerReconstructor(
    const string &name, std::function<AbstractConfReconstructor *()> o,
    const string &d) {
...
VertexRecoManager::VertexRecoManager(const VertexRecoManager &o) {
  ...
  exit(0);
  /*
  for ( map < string, AbstractConfReconstructor * >::const_iterator
  i=o.theAbstractConfReconstructors.begin();
...
std::unique_ptr<AbstractConfReconstructor>
VertexRecoManager::get(const string &s) const {
