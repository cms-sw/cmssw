...
ConfigurableAnnealing::ConfigurableAnnealing(const edm::ParameterSet &m)
  ...
  // edm::LogWarning("ConfigurableAnnealing") << "below one code ist still
  // here.";
  if (type == "below") {
    ...
    theImpl =
        new DeterministicAnnealing(sched, m.getParameter<double>("sigmacut"));
  } else if (type == "geom") {
    theImpl = new GeometricAnnealing(m.getParameter<double>("sigmacut"),
                                     m.getParameter<double>("Tini"),
                                     m.getParameter<double>("ratio"));
  } else {
    edm::LogError("ConfigurableAnnealing")
        << "annealing type " << type << " is not known.";
    exit(-1);
  }
}
