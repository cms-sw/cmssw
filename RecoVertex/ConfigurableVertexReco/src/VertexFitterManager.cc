...
void VertexFitterManager::registerFitter(
    const string &name, std::function<AbstractConfFitter *()> o,
    const string &d) {
...
VertexFitterManager::VertexFitterManager(const VertexFitterManager &o) {
  ...
  exit(0);
  /*
  for ( map < string, AbstractConfFitter * >::const_iterator
  i=o.theAbstractConfFitters.begin(); i!=o.theAbstractConfFitters.end() ; ++i )
  {
...
VertexFitterManager::get(const string &s) const {
