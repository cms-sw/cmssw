...
void errorNoFitter(const string &finder) {
  cout << "[ConfigurableVertexFitter] got no fitter for \"" << finder << "\""
       << endl;
...
CachingVertex<5>
ConfigurableVertexFitter::vertex(const vector<RefCountedVertexTrack> &tracks,
                                 const reco::BeamSpot &spot) const {
  return theFitter->vertex(tracks, spot);
}

CachingVertex<5>
ConfigurableVertexFitter::vertex(const vector<reco::TransientTrack> &tracks,
