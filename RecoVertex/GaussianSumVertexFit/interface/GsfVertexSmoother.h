...
private:
  ...
  TrackChi2Pair vertexAndTrackUpdate(const VertexState & oldVertex,
	RefCountedVertexTrack track, const GlobalPoint & referencePosition) const;

  RefittedTrackComponent createNewComponent(const VertexState & oldVertex,
	 RefCountedLinearizedTrackState linTrack, float weight) const;
   ...
   */
  double priorVertexChi2(VertexState priorVertex, 
	VertexState fittedVertex) const;
