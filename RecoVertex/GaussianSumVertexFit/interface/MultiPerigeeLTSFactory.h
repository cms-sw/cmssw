...
/**
 *  Concrete class to encapsulate the creation of
 * RefCountedLinearizedTrackState, which is a reference-counting pointer. This
 * class handles the possibility that the input track is described by not one
 * state, but a Gaussian Mixture of state (see BasicMultiTrajectoryState).
 *  Should always be used in order to create a new
 * RefCountedLinearizedTrackState, so that the reference-counting mechanism
 * works well.
 */
