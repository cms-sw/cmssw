...
public:
  typedef ReferenceCountingPointer<RefittedTrackState<5>>
  ...
  /**
   *   Constructor with a reference position. The surface which is going to be
   * usedto assemble the TSOS components will be the surface perpendicular to
   * the PCA of the state with the highest weight to the reference point.
   */
  MultiRefittedTS(const std::vector<RefCountedRefittedTrackState> &prtsComp,
  ...
  trajectoryStateOnSurface(const Surface &surface) const override;
  ...
  /**
   *   Returns a new reco::Track, which can then be made persistent. The
   * parameters are taken from FTS described above.
   */
  ...
  std::vector<ReferenceCountingPointer<RefittedTrackState<5>>>
