...
// struct vT_find
// {
//   typedef typename CachingVertex<N>::RefCountedVertexTrack
//   RefCountedVertexTrack; bool operator()(const CachingVertex<5> & v, const
//   RefCountedVertexTrack t)
//   {
// //initial tracks
...
std::pair<bool, double> GsfVertexTrackCompatibilityEstimator::estimate(
    const CachingVertex<5> &vertex, const RefCountedLinearizedTrackState track,
    unsigned int hint) const {
  RefCountedVertexTrack vertexTrack =
      vTrackFactory.vertexTrack(track, vertex.vertexState());
  return estimate(vertex, vertexTrack);
}

std::pair<bool, double> GsfVertexTrackCompatibilityEstimator::estimate(
    const reco::Vertex &vertex, const reco::TransientTrack &track) const {
  //   GlobalPoint linP(vertex.position().x(),
  ...
  RefCountedVertexTrack vertexTrack =
      vTrackFactory.vertexTrack(linTrack, vState);
  ...
  //   TrackTransientTrack*>(track.basicTransientTrack()); if ((ttt!=0) &&
  if (find(vertex.tracks_begin(), vertex.tracks_end(), track.trackBaseRef()) !=
      vertex.tracks_end()) {
...
GsfVertexTrackCompatibilityEstimator::estimateFittedTrack(
  ...
  // which is different than then one which the vertex track has been defined
  // with.
