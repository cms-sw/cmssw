  ...
  for (auto & prevVtxComponent : prevVtxComponents) {
    ...
    for (auto & ltComponent : ltComponents) {
      newTrackComponents.push_back
        (createNewComponent(prevVtxComponent, ltComponent, trackWeight));
    }
  }
	...
	const
{
  ...
  for (const auto & trackComponent : trackComponents) {
    if (trackComponent.second.first!=0) {
      reWeightedRTSC.push_back(trackComponent.first.first->stateWithNewWeight(trackComponent.second.first/totalWeight));
    }
  }
