...
GsfVertexSmoother::smooth(const CachingVertex<5> &vertex) const {
  ...
  GlobalPoint priorVertexPosition =
  ...
  CachingVertex<5> fitVertex(priorVertexPosition, priorVertexError,
                             initialTracks, 0);
  ...
  // descending fit
  for (std::vector<RefCountedVertexTrack>::const_iterator i =
           (tracks.end() - 1);
       i != (tracks.begin()); --i) {
    fitVertex = theUpdator.add(fitVertex, *i);
    if (limitComponents)
      fitVertex = theMerger->merge(fitVertex);
    descendingFittedVertices.insert(descendingFittedVertices.begin(),
                                    fitVertex);
  }
  ...
  for (std::vector<RefCountedVertexTrack>::const_iterator i = tracks.begin();
    ...
    TrackChi2Pair thePair =
        vertexAndTrackUpdate(meanedVertex, *i, vertex.position());
    smoothedChi2 += thePair.second.second;
    newTracks.push_back(theVTFactory.vertexTrack(
        (**i).linearizedTrack(), vertex.vertexState(), thePair.first,
        thePair.second.second, AlgebraicSymMatrixOO(), (**i).weight()));
  }

  if (vertex.hasPrior()) {
    smoothedChi2 +=
        priorVertexChi2(vertex.priorVertexState(), vertex.vertexState());
...
GsfVertexSmoother::TrackChi2Pair GsfVertexSmoother::vertexAndTrackUpdate(
    const VertexState &oldVertex, const RefCountedVertexTrack track,
...
/**
 * This method assembles all the components of the refitted track into one
 * refitted track state, normalizing the components. Also, it adds the chi2
 * track-components increments.
 */

GsfVertexSmoother::TrackChi2Pair GsfVertexSmoother::assembleTrackComponents(
    const std::vector<GsfVertexSmoother::RefittedTrackComponent>
        &trackComponents,
    const GlobalPoint &referencePosition) const {
  ...
  for (std::vector<RefittedTrackComponent>::const_iterator iter =
           trackComponents.begin();
  ...
  for (std::vector<RefittedTrackComponent>::const_iterator iter =
    ...
    if (iter->second.first != 0) {
      reWeightedRTSC.push_back(iter->first.first->stateWithNewWeight(
          iter->second.first / totalWeight));
    }
  }

  RefCountedRefittedTrackState finalRTS = RefCountedRefittedTrackState(
      new MultiRefittedTS(reWeightedRTSC, referencePosition));
  return TrackChi2Pair(finalRTS, VtxTrkChi2Pair(totalVtxChi2, totalTrkChi2));
}

/**
 * This method does the smoothing of one track component with one vertex
 * component. And the track-component-chi2 increment and weight of new component
 * in mixture.
 */

GsfVertexSmoother::RefittedTrackComponent GsfVertexSmoother::createNewComponent(
    const VertexState &oldVertex, const RefCountedLinearizedTrackState linTrack,
    float trackWeight) const {

  int sign = +1;

  // Weight of the component in the mixture (non-normalized)
  double weightInMixture =
      theWeightCalculator.calculate(oldVertex, linTrack, 1000000000.);

  // position estimate of the component
  VertexState newVertex = kalmanVertexUpdator.positionUpdate(
      oldVertex, linTrack, trackWeight, sign);
...
VertexState GsfVertexSmoother::meanVertex(const VertexState &vertexA,
  ...
  for (std::vector<VertexState>::iterator iA = vsCompA.begin();
       iA != vsCompA.end(); ++iA) {
    for (std::vector<VertexState>::iterator iB = vsCompB.begin();
         iB != vsCompB.end(); ++iB) {
      AlgebraicSymMatrix33 newWeight =
          iA->weight().matrix() + iB->weight().matrix();
      AlgebraicVector3 newWtP =
          iA->weightTimesPosition() + iB->weightTimesPosition();
...
double
GsfVertexSmoother::priorVertexChi2(const VertexState priorVertex,
