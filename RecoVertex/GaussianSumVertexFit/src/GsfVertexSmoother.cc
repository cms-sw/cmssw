...
CachingVertex<5> GsfVertexSmoother::smooth(const CachingVertex<5>& vertex) const {
  ...
  // ascending fit
  for (auto i = tracks.begin(); i != (tracks.end() - 1); ++i) {
  ...
  // descending fit
  for (auto i = (tracks.end() - 1); i != (tracks.begin()); --i) {
    fitVertex = theUpdator.add(fitVertex, *i);
    if (limitComponents)
      fitVertex = theMerger->merge(fitVertex);
...
GsfVertexSmoother::TrackChi2Pair GsfVertexSmoother::vertexAndTrackUpdate(const VertexState& oldVertex,
  ...
  for (auto vertexCompIter = prevVtxComponents.begin(); vertexCompIter != prevVtxComponents.end(); vertexCompIter++) {
    for (auto trackCompIter = ltComponents.begin(); trackCompIter != ltComponents.end(); trackCompIter++) {
...
GsfVertexSmoother::TrackChi2Pair GsfVertexSmoother::assembleTrackComponents(
  ...
  for (auto iter = trackComponents.begin(); iter != trackComponents.end(); ++iter) {
...
VertexState GsfVertexSmoother::meanVertex(const VertexState& vertexA, const VertexState& vertexB) const {
  ...
  for (auto iA = vsCompA.begin(); iA != vsCompA.end(); ++iA) {
    for (auto iB = vsCompB.begin(); iB != vsCompB.end(); ++iB) {
...
double GsfVertexSmoother::priorVertexChi2(const VertexState priorVertex, const VertexState fittedVertex) const {
  ...
  double vetexChi2 = 0.;
  for (auto pvI = priorVertexComp.begin(); pvI != priorVertexComp.end(); ++pvI) {
    for (auto fvI = fittedVertexComp.begin(); fvI != fittedVertexComp.end(); ++fvI) {
