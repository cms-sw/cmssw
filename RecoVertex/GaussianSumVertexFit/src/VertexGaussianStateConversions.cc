...
MultiGaussianState<3> multiGaussianStateFromVertex(const VertexState aState) {
  ...
  for (std::vector<VertexState>::const_iterator ic = components.begin();
    ...
    if (ic->isValid()) {
      ...
      parameters(1) = pos.y();
      parameters(2) = pos.z();
      SingleStatePtr sgs(new SingleGaussianState<3>(
          parameters, ic->error().matrix(), ic->weightInMixture()));
...
VertexState
vertexFromMultiGaussianState(const MultiGaussianState<3> &multiState) {
  ...
  for (MultiGaussianState<3>::SingleStateContainer::const_iterator ic =
           singleStates.begin();
    ...
    GlobalError error(cov(0, 0), cov(1, 0), cov(2, 0), cov(1, 1), cov(2, 1),
                      cov(2, 2));
