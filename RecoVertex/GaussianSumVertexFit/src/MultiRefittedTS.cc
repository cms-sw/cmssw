...
MultiRefittedTS::MultiRefittedTS(
    ...
    : theComponents(prtsComp), ftsAvailable(false),
      refSurface(&referenceSurface), surf(true) {}

MultiRefittedTS::MultiRefittedTS(
    const std::vector<RefCountedRefittedTrackState> &prtsComp,
    const GlobalPoint &referencePosition)
    : theComponents(prtsComp), ftsAvailable(false),
      refPosition(referencePosition), surf(false) {}
...
void MultiRefittedTS::computeFreeTrajectoryState() const {
  if (surf) {
    ...
    TransverseImpactPointExtrapolator tipe(
        &((**maxIt).freeTrajectoryState().parameters().magneticField()));
    TrajectoryStateOnSurface initialTSOS =
        tipe.extrapolate((**maxIt).freeTrajectoryState(), refPosition);

    fts = *(
        trajectoryStateOnSurface(initialTSOS.surface()).freeTrajectoryState());
  }
  ftsAvailable = true;
}
...
MultiRefittedTS::AlgebraicVectorN MultiRefittedTS::parameters() const {
  throw VertexException("MultiRefittedTS::freeTrajectoryState(): Don't know "
                        "how to do that yet...");
}
...
double MultiRefittedTS::weight() const {
  if (!totalWeightAvailable) {
    ...
    if (theComponents.empty()) {
      cout << "Asking for weight of empty MultiRefittedTS, returning zero!"
           << endl;
    }
    for (RTSvector::const_iterator it = theComponents.begin();
         it != theComponents.end(); it++) {
...
MultiRefittedTS::stateWithNewWeight(const double newWeight) const {
  if (weight() < DBL_MIN) {
    throw VertexException("MultiRefittedTS::stateWithNewWeight(): Can not "
                          "reweight multi-state with total weight < DBL_MIN");
  }
  ...
  for (RTSvector::const_iterator it = theComponents.begin();
    ...
    reWeightedRTSC.push_back(
        (**it).stateWithNewWeight((**it).weight() * factor));
  }
  if (surf) {
    return RefCountedRefittedTrackState(
        new MultiRefittedTS(reWeightedRTSC, *refSurface));
  } else {
    return RefCountedRefittedTrackState(
...
MultiRefittedTS::trajectoryStateOnSurface(const Surface &surface) const {
  ...
  return TrajectoryStateOnSurface(
      (BasicTrajectoryState *)new BasicMultiTrajectoryState(tsosComponents));
}

TrajectoryStateOnSurface
MultiRefittedTS::trajectoryStateOnSurface(const Surface &surface,
                                          const Propagator &propagator)
    const { // fixme... is the propagation done correctly? Is there a gsf
            // propagator?
  vector<TrajectoryStateOnSurface> tsosComponents;
  tsosComponents.reserve(theComponents.size());
  for (RTSvector::const_iterator it = theComponents.begin();
       it != theComponents.end(); it++) {
    tsosComponents.push_back(
        (**it).trajectoryStateOnSurface(surface, propagator));
  }
  return TrajectoryStateOnSurface(
      (BasicTrajectoryState *)new BasicMultiTrajectoryState(tsosComponents));
}
