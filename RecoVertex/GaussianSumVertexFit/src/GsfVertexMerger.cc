...
CachingVertex<5>
GsfVertexMerger::merge(const CachingVertex<5> &oldVertex) const {
  ...
  if (oldVertex.hasPrior()) {
    return CachingVertex<5>(oldVertex.priorPosition(), oldVertex.priorError(),
                            ...
                            newVertexState.weight(), oldVertex.tracks(),
...
VertexState GsfVertexMerger::merge(const VertexState &oldVertex) const {
  ...
  MultiGaussianState<3> multiGaussianState(
      multiGaussianStateFromVertex(oldVertex));
