...
PerigeeMultiLTS::PerigeeMultiLTS(const GlobalPoint &linP,
  ...
  for (vector<TrajectoryStateOnSurface>::iterator it = tsosComp.begin();
    ...
    ltComp.push_back(
        theLTSfactory.linearizedTrackState(theLinPoint, theTrack, *it));
  }
}
...
void PerigeeMultiLTS::prepareCollapsedState() const {
  if (!collapsedStateAvailable) {
    collapsedStateLT =
        theLTSfactory.linearizedTrackState(theLinPoint, theTrack, theTSOS);
  }
...
bool PerigeeMultiLTS::operator==(LinearizedTrackState<5> &other) const {
  ...
  if (otherP == nullptr) {
    throw VertexException("PerigeeMultiLTS: don't know how to compare myself "
                          "to non-perigee track state");
  }
...
PerigeeMultiLTS::createRefittedTrackState(
    const GlobalPoint &vertexPosition, const AlgebraicVectorM &vectorParameters,
  ...
  return RefCountedRefittedTrackState(
      new PerigeeRefittedTrackState(refittedTSCP, vectorParameters));
}
...
AlgebraicVector5 PerigeeMultiLTS::refittedParamFromEquation(
  ...
  AlgebraicVectorN param =
      constantTerm() + positionJacobian() * vertexPosition +
