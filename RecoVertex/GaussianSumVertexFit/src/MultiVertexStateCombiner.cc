...
VertexState MultiVertexStateCombiner::combine(const VSC &theMixture) const {
  ...
  if (theMixture.empty()) {
    throw VertexException("MultiVertexStateCombiner:: VertexState container to "
                          "collapse is empty");
  }

  if (theMixture.size() == 1) {
    //     #ifndef CMS_NO_COMPLEX_RETURNS
    return VertexState(theMixture.front().position(),
                       theMixture.front().error(), 1.0);
    //     #else
    //       VertexState theFinalVM(theMixture.front().position(),
    //       theMixture.front().error(), 1.0); return theFinalVM;
    //     #endif
  }
  ...
  for (VSC::const_iterator mixtureIter1 = theMixture.begin();
    ...
    for (VSC::const_iterator mixtureIter2 = mixtureIter1 + 1;
      ...
      measCovar2 += vtxWeight * mixtureIter2->weightInMixture() *
                    ROOT::Math::SimilarityT(tmp, s);
    }
  }
  meanPosition /= weightSum;
  AlgebraicSymMatrix33 measCovar =
      measCovar1 / weightSum + measCovar2 / weightSum / weightSum;
