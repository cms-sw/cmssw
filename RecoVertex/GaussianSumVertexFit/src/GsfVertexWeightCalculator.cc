...
double
GsfVertexWeightCalculator::calculate(const VertexState &oldVertex,
                                     const RefCountedLinearizedTrackState track,
                                     double cov) const {
  double previousWeight =
      oldVertex.weightInMixture() * track->weightInMixture();
  // Jacobians
  ...
  double weight =
      pow(2. * M_PI, -0.5 * 5) * sqrt(1. / sigmaDet) * exp(-0.5 * chi);
