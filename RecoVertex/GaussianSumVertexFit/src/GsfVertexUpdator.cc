...
CachingVertex<5>
GsfVertexUpdator::add(const CachingVertex<5> &oldVertex,
  ...
  //       << track->linearizedTrack()->components().size() << " components to
  //       vertex of "
  ...
  for (VSC::iterator vertexCompIter = prevVtxComponents.begin();
    ...
    for (LTC::iterator trackCompIter = ltComponents.begin();
      ...
      // return invalid vertex in case one of the updated vertex-components is
      // invalid
...
CachingVertex<5>
GsfVertexUpdator::remove(const CachingVertex<5> &oldVertex,
                         const RefCountedVertexTrack track) const {
...
GsfVertexUpdator::VertexComponent GsfVertexUpdator::createNewComponent(
  ...
  double weightInMixture =
      theWeightCalculator.calculate(oldVertex, linTrack, 1.E9);
  if (weightInMixture < 0.)
    return VertexComponent(VertexState(), WeightChi2Pair(0., 0.));

  // position estimate of the component
  VertexState newVertex =
      kalmanVertexUpdator.positionUpdate(oldVertex, linTrack, weight, sign);
  ...
  std::pair<bool, double> chi2P =
      kalmanVertexUpdator.chi2Increment(oldVertex, newVertex, linTrack, weight);
  if (!chi2P.first)
    return VertexComponent(VertexState(), WeightChi2Pair(0., 0.));
  //         cout << "Update: "<<oldVertex.position()<<"
  //         "<<newVertex.position()<<" "<<chi2
  // 	     <<" "<<linTrack->weightInMixture()<<" "<<weightInMixture<<endl;

  return VertexComponent(newVertex,
                         WeightChi2Pair(weightInMixture, chi2P.second));
}

GsfVertexUpdator::VertexChi2Pair GsfVertexUpdator::assembleVertexComponents(
    const std::vector<GsfVertexUpdator::VertexComponent> &newVertexComponents)
    const {
  ...
  for (std::vector<VertexComponent>::const_iterator iter =
           newVertexComponents.begin();
       iter != newVertexComponents.end(); iter++) {
    totalWeight += iter->second.first;
  }
  // cout << "totalWeight "<<totalWeight<<endl;
  if (totalWeight < DBL_MIN) {
    edm::LogWarning("GsfVertexUpdator")
  ...
  return VertexChi2Pair(
      VertexState(new BasicMultiVertexState(vertexComponents)), totalChi2);
}
