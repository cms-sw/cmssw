...
TrackMix::TrackMix(const edm::ParameterSet &iConfig) : theConfig(iConfig) {
  token_gsf = consumes<edm::View<reco::Track>>(
      iConfig.getParameter<string>("gsfTrackLabel"));
  token_ckf = consumes<edm::View<reco::Track>>(
      iConfig.getParameter<string>("ckfTrackLabel"));
}
...
void TrackMix::analyze(const edm::Event &iEvent,
  ...
  try {
    ...
    if (t_tks.size() > 1) {
      ...
      for (v1TrackIter = v1TrackBegin; v1TrackIter != v1TrackEnd;
           v1TrackIter++) {
