...
GsfTest::GsfTest(const edm::ParameterSet &iConfig) : theConfig(iConfig) {
  token_tracks =
      consumes<TrackCollection>(iConfig.getParameter<string>("TrackLabel"));
  ...
  edm::LogInfo("RecoVertex/GsfTest")
      ...
      << "\n";
  //   token_TrackTruth =
  //   consumes<TrackingParticleCollection>(edm::InputTag("trackingtruth",
  //   "TrackTruth"));
  token_VertexTruth = consumes<TrackingVertexCollection>(
      edm::InputTag("trackingtruth", "VertexTruth"));
}
...
void GsfTest::analyze(const edm::Event &iEvent, const edm::EventSetup &iSetup) {
  ...
  try {
    ...
    if (t_tks.size() > 1) {
      ...
      std::cout << "KVF Position:  " << Vertex::Point(tv2.position())
                << tv2.normalisedChiSquared() << " " << tv2.degreesOfFreedom()
                << "\n";
      ...
      //       recSimColl=associatorForParamAtPca->associateRecoToSim(tks,
      // 									      TPCollectionH,
      // 									      &iEvent);
