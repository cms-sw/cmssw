...
int main() {
  ...
  for (int nav = 30; nav < 80; nav += 20) {
    ...
    for (int i = 8; i < 20; ++i) {
      ...
      cudaCheck(
          cudaMemcpy(ws_d.view().zt().data(), ev.ztrack.data(), sizeof(float) * ev.ztrack.size(), cudaMemcpyHostToDevice));
      cudaCheck(
          cudaMemcpy(ws_d.view().ezt2().data(), ev.eztrack.data(), sizeof(float) * ev.eztrack.size(), cudaMemcpyHostToDevice));
      cudaCheck(
          cudaMemcpy(ws_d.view().ptt2().data(), ev.pttrack.data(), sizeof(float) * ev.eztrack.size(), cudaMemcpyHostToDevice));
#else
