                                      ...
                                      const GlobalPoint& point) const
{
  ...
  res(1,1) =   a_1*(pt1 - p1vx)/(p1vy*p1vy);
  res(1,8) = - a_2*(pt2 - p2vx)/(p2vy*p2vy);
  ...
  res(1,4)  = -(pt1 - p1vx)/(pt1*p1vy);
  res(1,11) =  (pt2 - p2vx)/(pt2*p2vy);

  //py1 and py2 components: 5th and 12 elements:
  res(1,5)  =  (pt1*p1vx - pt1*pt1 + p1(4)*p1vy)/(pt1*p1vy*p1vy);
  res(1,12) = -(pt2*p2vx - pt2*pt2 + p2(4)*p2vy)/(pt2*p2vy*p2vy);
                                    ...
                                    const GlobalPoint& point) const
{
  ...
  // xv component
  res(1,1) = -a_1 *(pt1 - p1vx) / (p1vy*p1vy) + a_2 *(pt2 - p2vx) / (p2vy*p2vy);

  //yv component
  res(1,2) = a_1/p1vy - a_2/p2vy;
