...
DAClusterizerInZ_vect::DAClusterizerInZ_vect(const edm::ParameterSet& conf) {
  ...
  if (convergence_mode_ > 1) {
    edm::LogWarning("DAClusterizerinZ_vect") << "DAClusterizerInZ_vect: invalid convergece_mode" << convergence_mode_
					     << "  reset do default " << 0;
    convergence_mode_ = 0;
  }

  if (Tmin == 0) {
    edm::LogWarning("DAClusterizerinZ_vect") << "DAClusterizerInZ_vect: invalid Tmin" << Tmin
  ...
  if ((Tpurge > Tmin) || (Tpurge == 0)) {
    edm::LogWarning("DAClusterizerinZ_vect") << "DAClusterizerInZ_vect: invalid Tpurge" << Tpurge
						   << "  set to " << Tmin;
    Tpurge =  Tmin;
  }
  betapurge_ = 1./Tpurge;
  ...
  if ((Tstop > Tpurge) || (Tstop == 0)) {
    edm::LogWarning("DAClusterizerinZ_vect") << "DAClusterizerInZ_vect: invalid Tstop" << Tstop
						   << "  set to  " << max(1., Tpurge);
    Tstop = max(1., Tpurge) ;
  }
  betastop_ = 1./Tstop;
...
void DAClusterizerInZ_vect::verify(const vertex_t &v, const track_t &tks, unsigned int nv, unsigned int nt)const
{
  ...
  assert(v._swE == &v.swE.front());

  for(unsigned int k=0; k< nv-1; k++){
    if ( v.z[k] <= v.z[k+1]) continue;
    cout << " Z, cluster z-ordering assertion failure   z["<<k<< "] =" << v.z[k] << "    z["<<k+1<< "] =" << v.z[k+1] << endl;
  }
  //for(unsigned int k=0; k< nv-1; k++){
...
double DAClusterizerInZ_vect::update(double beta, track_t & gtracks,
				     vertex_t & gvertices, const double rho0) const {
  ...
  // now update z and pk
  auto kernel_calc_z = [  sumpi, nv, this ] (vertex_t & vertices ) -> double {
...
double DAClusterizerInZ_vect::updateTc(double beta, track_t & gtracks,
				     vertex_t & gvertices, const double rho0) const {
...
bool DAClusterizerInZ_vect::merge(vertex_t & y, track_t &tks, double & beta)const{
  ...
  for (unsigned int ik=0; ik < critical.size(); ik++){
    ...
    if(Tc*beta < 1){
      ...
      if(DEBUGLEVEL > 1)
	{
	  std::cout << "merging " << fixed << setprecision(4) << y._z[k + 1]
		    << " and " <<  y._z[k] << "  Tc = " << Tc <<  "  sw = "  << y._sw[k]+y._sw[k+1]  <<std::endl;
	}
#endif
...
DAClusterizerInZ_vect::purge(vertex_t & y, track_t & tks, double & rho0, const double beta) const {
  ...
  for (unsigned int k = 0; k < nv; k++) {
	...
	//double p = tks._pi[i] * y._pk[k] * local_exp(-beta * Eik(tks._z[i], y._z[k], tks._dz2[i])) / tks._Z_sum[i];
	sump += p;
