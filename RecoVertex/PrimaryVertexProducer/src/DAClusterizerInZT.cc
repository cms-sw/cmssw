...
double DAClusterizerInZT::Eik(const track_t & t, const vertex_t &k )const{
...
double DAClusterizerInZT::update(
					   double beta,
					   vector<track_t> & tks,
					   vector<vertex_t> & y
					   )const{
...
double DAClusterizerInZT::beta0(
			       double betamax,
			       vector<track_t> & tks,
			       vector<vertex_t> & y
			       )const{
...
void DAClusterizerInZT::splitAll(
			 vector<vertex_t> & y
			 )const{
...
DAClusterizerInZT::DAClusterizerInZT(const edm::ParameterSet& conf) 
{
  ...
  maxIterations_=100;
  if (Tmin==0){
    cout << "DAClusterizerInZT: invalid Tmin" << Tmin << "  reset do default " << 1./betamax_ << endl;
  }else{
    betamax_ = 1./Tmin;
  }
