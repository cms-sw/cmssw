...
public:
  ...
  struct track_t {
    ...
    void addItemSorted( double new_z, double new_dz2, const reco::TransientTrack* new_tt, double new_pi   )
    {
      ...
      unsigned int i =0;
      for(i = 0; i < z.size(); i++){ if (new_dz2 > dz2[i]) break; }
  ...
  void set_vtx_range(double beta, track_t & gtracks,
  ...
  double update(double beta, track_t & gtracks,
		vertex_t & gvertices, const double rho0 = 0) const;
  double updateTc(double beta, track_t & gtracks,
		vertex_t & gvertices, const double rho0 = 0) const;
