...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
  ...
  //////////////////////
   ...
   template <bool debug = false, typename TAcc, typename = std::enable_if_t<alpaka::isAccelerator<TAcc>>> ALPAKA_FN_ACC static void set_vtx_range(const TAcc& acc, portablevertex::TrackDeviceCollection::View tracks, portablevertex::VertexDeviceCollection::View vertices, const portablevertex::ClusterParamsHostCollection::ConstView cParams, double& osumtkwt, double& _beta, int trackBlockSize){
    ...
    int maxVerticesPerBlock = (int) 512/alpaka::getWorkDiv<alpaka::Grid, alpaka::Blocks>(acc)[0u]; // Max vertices size is 512 over number of blocks in grid
    ...
    for (int itrack = threadIdx+blockIdx*trackBlockSize; itrack < threadIdx+(blockIdx+1)*trackBlockSize ; itrack += blockSize){
      ...
      int kmax = std::max(0,std::min(maxVerticesPerBlock * blockIdx + (int) (vertices[blockIdx].nV()) - 1, (int) (tracks[itrack].kmax()) - 1));
  ...
  template <bool debug = false, typename TAcc, typename = std::enable_if_t<alpaka::isAccelerator<TAcc>>> ALPAKA_FN_ACC static void merge(const TAcc& acc, portablevertex::TrackDeviceCollection::View tracks, portablevertex::VertexDeviceCollection::View vertices, const portablevertex::ClusterParamsHostCollection::ConstView cParams, double& osumtkwt, double& _beta, int trackBlockSize){
    ...
    int nprev = vertices[blockIdx].nV();
      ...
      if (once_per_block(acc)){ printf("[ClusterizerAlgo::merge()] BlockIdx %i, start merging \n",blockIdx);}
    #endif
    if (nprev < 2) return;
    for (int ivertexO = maxVerticesPerBlock * blockIdx + threadIdx; ivertexO < maxVerticesPerBlock * blockIdx + vertices[blockIdx].nV()-1 ; ivertexO += blockSize){
    ...
    #ifdef DEBUG_RECOVERTEX_PRIMARYVERTEXPRODUCER_ALPAKA_CLUSTERIZERALGO
      if (once_per_block(acc)){ printf("[ClusterizerAlgo::merge()] BlockIdx %i, %i vertices to be merged\n",blockIdx, ncritical);}
    #endif
    for (int sortO = 0; sortO < ncritical ; ++sortO){ // All threads are running the same code, to know when to exit
      ...
      alpaka::syncBlockThreads(acc);
      #ifdef DEBUG_RECOVERTEX_PRIMARYVERTEXPRODUCER_ALPAKA_CLUSTERIZERALGO
        if (once_per_block(acc)){ printf("[ClusterizerAlgo::merge()] BlockIdx %i, merge vertex %i into vertex %i\n",blockIdx, ivertex, ivertexnext);}
      #endif
      ...
      if (once_per_block(acc)){
        ...
        #ifdef DEBUG_RECOVERTEX_PRIMARYVERTEXPRODUCER_ALPAKA_CLUSTERIZERALGO
          if (once_per_block(acc)){ printf("[ClusterizerAlgo::merge()] BlockIdx %i, merged vertex %i with z=%1.3f,rho=%1.3f\n",blockIdx,ivertexnext, vertices[ivertexnext].z(), vertices[ivertexnext].rho());}
        #endif
      } // end once_per_block
      alpaka::syncBlockThreads(acc); 
      ...
      for (int itrack = threadIdx+blockIdx*trackBlockSize; itrack < threadIdx+(blockIdx+1)*trackBlockSize; itrack += blockSize){
        if (tracks[itrack].kmax() > ivertexO+1) tracks[itrack].kmax()--;
  ...
  template <bool debug = false, typename TAcc, typename = std::enable_if_t<alpaka::isAccelerator<TAcc>>> ALPAKA_FN_ACC static void split(const TAcc& acc, portablevertex::TrackDeviceCollection::View tracks, portablevertex::VertexDeviceCollection::View vertices, const portablevertex::ClusterParamsHostCollection::ConstView cParams, double& osumtkwt, double& _beta, double threshold, int trackBlockSize){
    ...
    alpaka::syncBlockThreads(acc);
    if (ncritical == 0 || maxVerticesPerBlock == nprev) return; // I.e. either we don't want to or we can't split more
    #ifdef DEBUG_RECOVERTEX_PRIMARYVERTEXPRODUCER_ALPAKA_CLUSTERIZERALGO
      if (once_per_block(acc)){ printf("[ClusterizerAlgo::split()] BlockIdx %i, split %i vertices\n",blockIdx, ncritical);}
    #endif
    for (int sortO = 0; sortO < ncritical ; ++sortO){ // All threads are running the same code, to know when to exit
      if (ncritical == 0 || maxVerticesPerBlock == nprev) return;
      int ikO = 0;
      ...
      int ivertexnext = blockIdx * maxVerticesPerBlock + nprev -1; 
      #ifdef DEBUG_RECOVERTEX_PRIMARYVERTEXPRODUCER_ALPAKA_CLUSTERIZERALGO
        if (once_per_block(acc)){ printf("[ClusterizerAlgo::split()] BlockIdx %i, splitting vertex %i\n", blockIdx, ivertex);}
      #endif
	...
	w2 = 0.;
      ...
      alpaka::syncBlockThreads(acc);
      #ifdef DEBUG_RECOVERTEX_PRIMARYVERTEXPRODUCER_ALPAKA_CLUSTERIZERALGO
        if (once_per_block(acc)){ printf("[ClusterizerAlgo::split()] BlockIdx %i, for vertex %i, p1=%1.3f, p2=%1.3f, w1=%1.3f, w2=%1.3f, z1=%1.3f, z2=%1.3f\n",blockIdx, ivertex, p1, p2, w1, w2, z1, z2);}
      #endif
	...
	if ((ivertexO > maxVerticesPerBlock*blockIdx) && (z1 < (0.6 * vertices[ivertex].z() + 0.4 * vertices[ivertexprev].z()))) { // First in the if is ivertexO, as we care on whether the vertex is the leftmost or rightmost
      ...
      alpaka::syncBlockThreads(acc);
      #ifdef DEBUG_RECOVERTEX_PRIMARYVERTEXPRODUCER_ALPAKA_CLUSTERIZERALGO
        if (once_per_block(acc)){ printf("[ClusterizerAlgo::split()] BlockIdx %i, vertex %i will split into z1=%1.3f, z2=%1.3f\n",blockIdx, ivertex, z1, z2);}
      #endif
      int nnew = 999999;
      ...
      if (nnew == 999999) break; // Need to compute in all threads so all exist properly
      printf("%i => %i and %i\n",ivertex, ivertex, nnew);
      if (once_per_block(acc)){
	...
	vertices[blockIdx].nV() += 1;
        #ifdef DEBUG_RECOVERTEX_PRIMARYVERTEXPRODUCER_ALPAKA_CLUSTERIZERALGO
          if (once_per_block(acc)){ printf("[ClusterizerAlgo::split()] BlockIdx %i, vertex %i did split into indexes %i and %i\n",blockIdx, ivertex, ivertex, nnew);}
        #endif
      }
      alpaka::syncBlockThreads(acc);
  ...
  template <bool debug = false, typename TAcc, typename = std::enable_if_t<alpaka::isAccelerator<TAcc>>> ALPAKA_FN_ACC static void purge(const TAcc& acc, portablevertex::TrackDeviceCollection::View tracks, portablevertex::VertexDeviceCollection::View vertices, const portablevertex::ClusterParamsHostCollection::ConstView cParams, double& osumtkwt, double& _beta, double rho0, int trackBlockSize){
    ...
    if (once_per_block(acc)){
      ...
      if (k0 != (int) (maxVerticesPerBlock * blockIdx + nprev)){
        #ifdef DEBUG_RECOVERTEX_PRIMARYVERTEXPRODUCER_ALPAKA_CLUSTERIZERALGO
          if (once_per_block(acc)){ printf("[ClusterizerAlgo::purge()] BlockIdx %i, some vertices need purging. Will start purging \n",blockIdx);}
        #endif
	...
	vertices[k0].isGood() = false;
        #ifdef DEBUG_RECOVERTEX_PRIMARYVERTEXPRODUCER_ALPAKA_CLUSTERIZERALGO
          if (once_per_block(acc)){ printf("[ClusterizerAlgo::purge()] BlockIdx %i, vertex %i purged\n",blockIdx, k0);}
        #endif
      }
    }// end once_per_block
    alpaka::syncBlockThreads(acc); 
  ...
  template <bool debug = false, typename TAcc, typename = std::enable_if_t<alpaka::isAccelerator<TAcc>>> ALPAKA_FN_ACC static void initialize(const TAcc& acc, portablevertex::TrackDeviceCollection::View tracks, portablevertex::VertexDeviceCollection::View vertices, const portablevertex::ClusterParamsHostCollection::ConstView cParams, int trackBlockSize){
    ...
    for (int ivertex = threadIdx+maxVerticesPerBlock*blockIdx; ivertex < maxVerticesPerBlock*(blockIdx+1); ivertex+=blockSize){ // Initialize vertices in parallel in the block. Note that a block of threads should always be restricted to oeprations within maxVerticesPerBlock*blockIdx and maxVerticesPerBlock*(blockIdx+1)-1 to avoid running conditions
      ...
      if (ivertex == maxVerticesPerBlock*blockIdx){ // Set up the initial single vetex containing everything which should only happen for the first vertex 
    ...
    for (int itrack = threadIdx+blockIdx*trackBlockSize; itrack < threadIdx+(blockIdx+1)*trackBlockSize; itrack += blockSize){ // Technically not a loop as each thread will have one track in the per block approach, but in the more general case this can be extended to BlockSize in Alpaka != BlockSize in algorithm
      tracks[itrack].kmin() = maxVerticesPerBlock*blockIdx; // Tracks are associated to vertex in list kmin, kmin+1,... kmax-1, so this just assign all tracks to the vertex we just created
      tracks[itrack].kmax() = maxVerticesPerBlock*blockIdx + 1;
    }
    alpaka::syncBlockThreads(acc);
  }
  
  template <bool debug = false, typename TAcc, typename = std::enable_if_t<alpaka::isAccelerator<TAcc>>> ALPAKA_FN_ACC static void getBeta0(const TAcc& acc, portablevertex::TrackDeviceCollection::View tracks, portablevertex::VertexDeviceCollection::View vertices, const portablevertex::ClusterParamsHostCollection::ConstView cParams, double& _beta, int trackBlockSize){
  ...
  template <bool debug = false, typename TAcc, typename = std::enable_if_t<alpaka::isAccelerator<TAcc>>> ALPAKA_FN_ACC static void thermalize(const TAcc& acc, portablevertex::TrackDeviceCollection::View tracks, portablevertex::VertexDeviceCollection::View vertices, const portablevertex::ClusterParamsHostCollection::ConstView cParams, double& osumtkwt, double& _beta, double delta_highT, double rho0, int trackBlockSize){
    ...
    while (niter++ < maxIterations){ // Loop until vertex position change is small
      #ifdef DEBUG_RECOVERTEX_PRIMARYVERTEXPRODUCER_ALPAKA_CLUSTERIZERALGO
        if (once_per_block(acc)){ printf("[ClusterizerAlgo::thermalize()] BlockIdx %i, thermalize at _beta=%1.3f, iteration %i\n",blockIdx, _beta, niter);}
      #endif
      ...
      double dmax = 0.;
      for (int ivertexO = maxVerticesPerBlock*blockIdx ; ivertexO < maxVerticesPerBlock*blockIdx + vertices[blockIdx].nV(); ivertexO++){ // Each thread looks for the max on its own
        int ivertex = vertices[ivertexO].order();
      ...
      if (delta_sum_range > zrange_min_ && dmax > zrange_min_) {  // I.e., if a vertex moved too much we reassign
        #ifdef DEBUG_RECOVERTEX_PRIMARYVERTEXPRODUCER_ALPAKA_CLUSTERIZERALGO
          if (once_per_block(acc)){ printf("[ClusterizerAlgo::thermalize()] BlockIdx %i, thermalize at _beta=%1.3f, iteration %i. Found delta_sum_range=%1.3f, dmax=%1.3f, will redo track-vertex assignament\n",blockIdx, _beta, niter, delta_sum_range, dmax);}
        #endif
        set_vtx_range(acc, tracks, vertices, cParams, osumtkwt, _beta, trackBlockSize);
	delta_sum_range = 0.;
      }
      if (dmax < delta_max){ // If everything moved too little, we stop update
        #ifdef DEBUG_RECOVERTEX_PRIMARYVERTEXPRODUCER_ALPAKA_CLUSTERIZERALGO
          if (once_per_block(acc)){ 
            update(acc, tracks, vertices, cParams, osumtkwt, _beta, 0.0, true, trackBlockSize); // Update also swE to get proper TCs
            printf("[ClusterizerAlgo::thermalize()] BlockIdx %i, thermalize at _beta=%1.3f, iteration %i. Found delta_sum_range=%1.3f, dmax=%1.3f, all vertices stable enough to stop thermalizing\n",blockIdx, _beta, niter, delta_sum_range, dmax);
  ...
  template <bool debug = false, typename TAcc, typename = std::enable_if_t<alpaka::isAccelerator<TAcc>>> ALPAKA_FN_ACC static void coolingWhileSplitting(const TAcc& acc, portablevertex::TrackDeviceCollection::View tracks, portablevertex::VertexDeviceCollection::View vertices, const portablevertex::ClusterParamsHostCollection::ConstView cParams, double& osumtkwt, double& _beta, int trackBlockSize){
    ...
    while (_beta < betafreeze){ // The cooling loop
      #ifdef DEBUG_RECOVERTEX_PRIMARYVERTEXPRODUCER_ALPAKA_CLUSTERIZERALGO
        if (once_per_block(acc)){ printf("[ClusterizerAlgo::coolingWhileSplitting()] BlockIdx %i, current _beta=%1.3f\n",blockIdx, _beta);}
      #endif
      int nprev = vertices[blockIdx].nV();
      ...
      while (nprev !=  vertices[blockIdx].nV() ) { // If we are here, we merged before, keep merging until stable
        nprev = vertices[blockIdx].nV();
	update(acc, tracks, vertices, cParams, osumtkwt, _beta, 0.0, false, trackBlockSize); // Update positions after merge
	merge(acc, tracks, vertices, cParams, osumtkwt, _beta, trackBlockSize);
      } // end while after merging
      split(acc, tracks, vertices, cParams, osumtkwt, _beta, 1.0, trackBlockSize); // As we are close to a critical temperature, check if we need to split and if so, do it
      if (once_per_block(acc)){ // Cool down
	_beta = _beta/cParams.coolingFactor();
      }
      alpaka::syncBlockThreads(acc);
      thermalize(acc, tracks, vertices, cParams, osumtkwt, _beta, cParams.delta_highT(), 0.0, trackBlockSize); // Stabilize positions after cooling
    }
    set_vtx_range(acc, tracks, vertices, cParams, osumtkwt, _beta, trackBlockSize); // Reassign tracks to vertex
    update(acc, tracks, vertices, cParams, osumtkwt, _beta, 0.0, false, trackBlockSize); // Last, update positions again
  ...
  template <bool debug = false, typename TAcc, typename = std::enable_if_t<alpaka::isAccelerator<TAcc>>> ALPAKA_FN_ACC static void reMergeTracks(const TAcc& acc, portablevertex::TrackDeviceCollection::View tracks, portablevertex::VertexDeviceCollection::View vertices, const portablevertex::ClusterParamsHostCollection::ConstView cParams, double& osumtkwt, double& _beta, int trackBlockSize){
    ...
    while (nprev !=  vertices[blockIdx].nV() ) { // If we are here, we merged before, keep merging until stable
      set_vtx_range(acc, tracks, vertices, cParams, osumtkwt, _beta, trackBlockSize); // Reassign tracks to vertex
      update(acc, tracks, vertices, cParams, osumtkwt, _beta, 0.0, false, trackBlockSize); // Update before any final merge
      nprev = vertices[blockIdx].nV();
      merge(acc, tracks, vertices, cParams, osumtkwt, _beta, trackBlockSize);
    } // end while
  } // end reMergeTracks
  ...
  template <bool debug = false, typename TAcc, typename = std::enable_if_t<alpaka::isAccelerator<TAcc>>> ALPAKA_FN_ACC static void rejectOutliers(const TAcc& acc, portablevertex::TrackDeviceCollection::View tracks, portablevertex::VertexDeviceCollection::View vertices, const portablevertex::ClusterParamsHostCollection::ConstView cParams, double& osumtkwt, double& _beta, int trackBlockSize){
    ...
    while (nprev !=  vertices[blockIdx].nV()) {
      set_vtx_range(acc, tracks, vertices, cParams, osumtkwt, _beta, trackBlockSize); // Reassign tracks to vertex
      update(acc, tracks, vertices, cParams, osumtkwt, _beta, rho0, false, trackBlockSize); // At rho0 it changes the initial value of the partition function
      nprev = vertices[blockIdx].nV();
      merge(acc, tracks, vertices, cParams, osumtkwt, _beta, trackBlockSize);
    }
