...
void graphicsDebug(const std::vector<PointAndDistance> &input) {
  bool sve =
      SimpleConfigurable<bool>(false, "CrossingPointStudy:Harvest").value();
...
CrossingPtBasedLinearizationPointFinder::
    CrossingPtBasedLinearizationPointFinder(const RecTracksDistanceMatrix *m,
                                            ...
                                            const signed int n_pairs)
    : useMatrix(m->hasCrossingPoints()), theNPairs(n_pairs), theMatrix(m),
      theAlgo(algo.clone()) {}
...
CrossingPtBasedLinearizationPointFinder::getBestTracks(
  ...
  unsigned int n_tracks = (2 * (unsigned int)(theNPairs)) < tracks.size()
                              ? 2 * theNPairs
                              : tracks.size();

  std::vector<reco::TransientTrack> newtracks = tracks;
  partial_sort(newtracks.begin(), newtracks.begin() + n_tracks, newtracks.end(),
               CompareTwoTracks());
...
GlobalPoint CrossingPtBasedLinearizationPointFinder::useAllTracks(
  ...
  for (std::vector<reco::TransientTrack>::const_iterator x = tracks.begin();
       x != endm1; ++x) {
    for (std::vector<reco::TransientTrack>::const_iterator y = x + 1; y != end;
         ++y) {
...
GlobalPoint CrossingPtBasedLinearizationPointFinder::getLinearizationPoint(
  ...
  if (sum(tracks.size() - 1) < ((unsigned int)(theNPairs))) {
    /*
    std::cout << "[CrossingPtBasedLinearizationPointFinder] we exploit all track
    pairs"
         << std::endl;*/
  ...
  /*
  std::cout << "[CrossingPtBasedLinearizationPointFinder] we start: npairs=" <<
  theNPairs
       << std::endl;
  std::cout << "[CrossingPtBasedLinearizationPointFinder] t_interval=" <<
  t_interval << std::endl; std::cout <<
  "[CrossingPtBasedLinearizationPointFinder goodtracks.size=" <<
  goodtracks.size()
       << std::endl;*/
  ...
  while (vgp.size() < ((unsigned int)(theNPairs))) {
    ...
    // std::cout << "Considering now: " << t_first << ", " << t_first+t_interval
    // << std::endl;
    if (useMatrix) {
      ...
      TwoTrackMinimumDistance ttmd;
      bool status =
          ttmd.calculate(rt1.impactPointState(), rt2.impactPointState());
      if (status) {
        std::pair<GlobalPoint, GlobalPoint> pts = ttmd.points();
    ...
    if ((t_first + t_interval) < lim) {
      ...
      t_interval--;
      if (t_interval == 0) {
        /* std::cout << "[CrossingPtBasedLinearizationPointFinder] t_interval=0.
           break."
             << std::endl;*/
        break;
      }
