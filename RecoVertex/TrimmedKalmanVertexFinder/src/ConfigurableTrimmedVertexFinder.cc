...
void ConfigurableTrimmedVertexFinder::setParameters(
    const edm::ParameterSet &s) {
  theFilter.setPtCut(s.getParameter<double>("ptCut"));
  theTrackCompatibilityToPV =
      s.getParameter<double>("trackCompatibilityToPVcut");
  theTrackCompatibilityToSV =
      s.getParameter<double>("trackCompatibilityToSVcut");
...
std::vector<TransientVertex> ConfigurableTrimmedVertexFinder::vertices(
    const std::vector<TransientTrack> &tracks,
    std::vector<TransientTrack> &unused, const reco::BeamSpot &spot,
    bool use_spot) const {
  resetEvent(tracks);
  ...
  std::vector<TransientVertex> all =
      vertexCandidates(filtered, unused, spot, use_spot);
...
std::vector<TransientVertex> ConfigurableTrimmedVertexFinder::vertexCandidates(
  ...
  while (true) {
    ...
    float tkCompCut =
        (cand.empty() ? theTrackCompatibilityToPV : theTrackCompatibilityToSV);
