...
TrimmedVertexFinder::TrimmedVertexFinder(const TrimmedVertexFinder &other)
    : theFitter(other.theFitter->clone()),
      ...
      theEstimator(other.theEstimator->clone()), theMinProb(other.theMinProb) {}
...
TrimmedVertexFinder::vertices(std::vector<TransientTrack> &tks,
  ...
  while (!found && selected.size() >= 2) {
    ...
    std::vector<RefCountedVertexTrack>::iterator iWorst =
        theWorst(vtx, selected, theMinProb);
    ...
    if (iWorst != selected.end()) {
      ...
      for (std::vector<RefCountedVertexTrack>::const_iterator t =
               selected.begin();
           t != selected.end(); ++t) {
        if ((**t).weight() < 0.5) {
          /*
