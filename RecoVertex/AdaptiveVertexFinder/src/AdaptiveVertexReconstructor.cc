...
TransientVertex
AdaptiveVertexReconstructor::cleanUp(const TransientVertex &old) const {
  ...
  if (old.hasPrior()) {
    ...
    ret = TransientVertex(old.vertexState(), newtrks, old.totalChiSquared(),
                          old.degreesOfFreedom());
  }
  ...
  if (old.hasRefittedTracks()) {
    ...
    for (vector<reco::TransientTrack>::const_iterator i = oldrfs.begin();
         i != oldrfs.end(); ++i) {
...
void AdaptiveVertexReconstructor::erase(
    const TransientVertex &newvtx, set<reco::TransientTrack> &remainingtrks,
    float w) const {
  /*
...
AdaptiveVertexReconstructor::AdaptiveVertexReconstructor(float primcut,
                                                         float seccut,
                                                         float min_weight,
                                                         bool smoothing)
...
void AdaptiveVertexReconstructor::setupFitters(float primcut, float primT,
                                               float primr, float seccut,
                                               float secT, float secr,
                                               bool smoothing) {
  ...
  edm::LogError ("AdaptiveVertexReconstructor" )
    ...
    */
  thePrimaryFitter = new AdaptiveVertexFitter(
      GeometricAnnealing(primcut, primT, primr),
  ...
  // if the primary fails, sth is wrong, so here we set a threshold on the
  // weight.
  theSecondaryFitter = new AdaptiveVertexFitter(
      GeometricAnnealing(seccut, secT, secr), DefaultLinearizationPointFinder(),
      KalmanVertexUpdator<5>(), KalmanVertexTrackCompatibilityEstimator<5>(),
      *smoother);
...
AdaptiveVertexReconstructor::AdaptiveVertexReconstructor(
    const edm::ParameterSet &m)
    : thePrimaryFitter(nullptr), theSecondaryFitter(nullptr), theMinWeight(0.5),
...
vector<TransientVertex> AdaptiveVertexReconstructor::vertices(
    const vector<reco::TransientTrack> &primaries,
    const vector<reco::TransientTrack> &tracks, const reco::BeamSpot &s,
    bool has_primaries, bool usespot) const {
      ...
      if (n_tracks == remainingtrks.size()) {
        if (usespot) {
          ...
          LogDebug("AdaptiveVertexReconstructor")
              << "no tracks in vertex. trying again without beamspot "
                 "constraint!";
          continue;
        }
        LogDebug("AdaptiveVertexReconstructor")
...
vector<TransientVertex> AdaptiveVertexReconstructor::cleanUpVertices(
  ...
  vector<TransientVertex> ret;
  for (vector<TransientVertex>::const_iterator i = old.begin(); i != old.end();
       ++i) {
    if (!(i->hasTrackWeight())) { // if we dont have track weights, we take the
                                  // vtx
      ret.push_back(*i);
      continue;
    }
    ...
    for (TransientVertex::TransientTrackToFloatMap::const_iterator w =
             wm.begin();
         w != wm.end(); ++w) {
