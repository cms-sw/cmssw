...
VertexMerging::mergeVertex(reco::VertexCollection &secondaryVertices) {
  ...
  for (std::vector<reco::Vertex>::iterator sv = recoVertices.begin();
    ...
    for (std::vector<reco::Vertex>::iterator sv2 = recoVertices.begin();
      ...
      //        std::cout << sv2-recoVertices->begin() << " vs " <<
      //        sv-recoVertices->begin() << " : " << fr << " "  <<
      //        computeSharedTracks(*sv, *sv2) << " sig " <<
      //        dist.distance(*sv,*sv2).significance() << std::endl;
      //      std::cout << (fr > maxFraction) << " && " <<
      //      (dist.distance(*sv,*sv2).significance() < 2)  <<  " && " <<
      //      (sv-sv2!=0)  << " && " <<  (fr >= computeSharedTracks(*sv2, *sv))
      //      << std::endl;
      if (fr > maxFraction &&
          dist.distance(*sv, *sv2).significance() < minSignificance &&
          sv - sv2 != 0 && fr >= computeSharedTracks(*sv, *sv2)) {
        shared = true;
        // std::cout << "shared " << sv-recoVertices->begin() << " and "  <<
        // sv2-recoVertices->begin() << " fractions: " << fr << " , "  <<
        // computeSharedTracks(*sv2, *sv) << " sig: " <<
        // dist.distance(*sv,*sv2).significance() <<  std::endl;
      }
    }
    if (shared) {
    ...
    //    std::cout << "it = " <<  sv-recoVertices->begin() << " new size is: "
    //    << recoVertices->size() <<   std::endl;
  }
