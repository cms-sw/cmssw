...
public:
  ...
  static void fillDescriptions(edm::ConfigurationDescriptions &cdesc) {
    ...
    pdesc.add<edm::InputTag>("primaryVertices",
...
TemplatedInclusiveVertexFinder<InputContainer, VTX>::
    ...
    : minHits(params.getParameter<unsigned int>("minHits")),
      ...
      vertexMinAngleCosine(
          params.getParameter<double>("vertexMinAngleCosine")), // 0.98
      vertexMinDLen2DSig(
          params.getParameter<double>("vertexMinDLen2DSig")),            // 2.5
      ...
      vtxReco(new ConfigurableVertexReconstructor(
          params.getParameter<edm::ParameterSet>("vertexReco"))),
      clusterizer(new TracksClusteringFromDisplacedSeed(
          params.getParameter<edm::ParameterSet>("clusterizer")))

{
  token_beamSpot =
      consumes<reco::BeamSpot>(params.getParameter<edm::InputTag>("beamSpot"));
  token_primaryVertex = consumes<reco::VertexCollection>(
      params.getParameter<edm::InputTag>("primaryVertices"));
  token_tracks =
      consumes<InputContainer>(params.getParameter<edm::InputTag>("tracks"));
  produces<Product>();
  // produces<reco::VertexCollection>("multi");
}
template <class InputContainer, class VTX>
bool TemplatedInclusiveVertexFinder<InputContainer, VTX>::trackFilter(
...
template <class InputContainer, class VTX>
void TemplatedInclusiveVertexFinder<InputContainer, VTX>::produce(
  ...
  if (primaryVertices->size() != 0) {
    ...
    for (typename InputContainer::const_iterator track = tracks->begin();
      ...
      if (edm::isFinite(tt.timeExt()) &&
          pv.covariance(3, 3) > 0.) { // only apply if time available
        auto tError =
            std::sqrt(std::pow(tt.dtErrorExt(), 2) + pv.covariance(3, 3));
    ...
    for (std::vector<TracksClusteringFromDisplacedSeed::Cluster>::iterator
      ...
      if (useVertexReco) {
        vertices = vtxReco->vertices(
            cluster->tracks, bs); // attempt with config given reconstructor
      }
      TransientVertex singleFitVertex;
      if (useVertexFitter) {
        singleFitVertex = theAdaptiveFitter.vertex(
            cluster->tracks, cluster->seedPoint); // attempt with direct fitting
        if (singleFitVertex.isValid())
          vertices.push_back(singleFitVertex);
      }

      // for each transient vertex state determine if a time can be measured and
      // fill covariance
...
#ifdef VTXDEBUG
        VTX vv(*v);
        std::cout << "V chi2/n: " << v->normalisedChiSquared()
                  << " ndof: " << v->degreesOfFreedom();
        std::cout << " dlen: " << dlen.value() << " error: " << dlen.error()
                  << " signif: " << dlen.significance();
        std::cout << " dlen2: " << dlen2.value() << " error2: " << dlen2.error()
                  << " signif2: " << dlen2.significance();
        std::cout << " pos: " << vv.position() << " error: " << vv.xError()
                  << " " << vv.yError() << " " << vv.zError() << std::endl;
        std::cout << " time: " << vv.time() << " error: " << vv.tError()
                  << std::endl;
#endif
        GlobalVector dir;
...
#ifdef VTXDEBUG
          std::cout << "\t[" << (*i).track().pt() << ": " << (*i).track().eta()
                    << ", " << (*i).track().phi() << "], " << w << std::endl;
#endif
        }
        GlobalPoint sv((*v).position().x(), (*v).position().y(),
                       (*v).position().z());
        float vscal = dir.unit().dot((sv - ppv).unit());
        if (dlen.significance() > vertexMinDLenSig &&
            ((vertexMinAngleCosine > 0) ? (vscal > vertexMinAngleCosine)
                                        : (vscal < vertexMinAngleCosine)) &&
            v->normalisedChiSquared() < 10 &&
            dlen2.significance() > vertexMinDLen2DSig) {
