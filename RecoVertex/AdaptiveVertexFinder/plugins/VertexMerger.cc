...
void TemplatedVertexMerger<VTX>::produce(edm::Event &event,
  ...
  for (typename Product::iterator sv = recoVertices->begin();
    ...
    bool shared = false;
    VertexState s1(RecoVertex::convertPos(sv->position()),
                   RecoVertex::convertError(sv->error()));
    for (typename Product::iterator sv2 = recoVertices->begin();
         sv2 != recoVertices->end(); ++sv2) {
      VertexState s2(RecoVertex::convertPos(sv2->position()),
                     RecoVertex::convertError(sv2->error()));
      ...
      //      << std::endl;
      if (fr > maxFraction &&
          dist.distance(s1, s2).significance() < minSignificance &&
          sv - sv2 != 0 && fr >= vertexTools::computeSharedTracks(*sv, *sv2)) {
        shared = true;
...
typedef TemplatedVertexMerger<reco::VertexCompositePtrCandidate>
