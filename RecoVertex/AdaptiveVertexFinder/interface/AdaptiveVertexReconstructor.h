...
public:
  ...
  vertices(const std::vector<reco::TransientTrack> &v) const override;
...
private:
   ...
   */
  std::vector<TransientVertex>
  cleanUpVertices(const std::vector<TransientVertex> &) const;

  /** setup the vertex fitters.
   */
  void setupFitters(float primcut, float primT, float primr, float seccut,
                    float secT, float secr, bool smoothing);
