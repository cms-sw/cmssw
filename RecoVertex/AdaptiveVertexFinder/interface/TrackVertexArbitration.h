...
inline double cov33(const reco::VertexCompositePtrCandidate &sv) {
  return sv.vertexCovariance(3, 3);
}
} // namespace svhelper

template <class VTX> class TrackVertexArbitration {
public:
  TrackVertexArbitration(const edm::ParameterSet &params);

  std::vector<VTX>
  trackVertexArbitrator(edm::Handle<reco::BeamSpot> &beamSpot,
...
template <class VTX>
TrackVertexArbitration<VTX>::TrackVertexArbitration(
...
template <class VTX>
bool TrackVertexArbitration<VTX>::trackFilterArbitrator(
    const reco::TransientTrack &track) const {
  if (!track.isValid())
    return false;
  if (track.track().hitPattern().trackerLayersWithMeasurement() <
      trackMinLayers)
    return false;
...
float trackWeight(const reco::VertexCompositePtrCandidate &sv,
  ...
  const reco::CandidatePtrTransientTrack *cptt =
      dynamic_cast<const reco::CandidatePtrTransientTrack *>(
  ...
  if (cptt == nullptr)
    ...
    if (std::find(sv.daughterPtrVector().begin(), sv.daughterPtrVector().end(),
                  c) != sv.daughterPtrVector().end())
      return 1.0;
    else
      return 0;
  }
...
std::vector<VTX> TrackVertexArbitration<VTX>::trackVertexArbitrator(
    edm::Handle<reco::BeamSpot> &beamSpot, const reco::Vertex &pv,
    std::vector<reco::TransientTrack> &selectedTracks,
    std::vector<VTX> &secondaryVertices) {
  ...
  AdaptiveVertexFitter theAdaptiveFitter(
      ...
      DefaultLinearizationPointFinder(), KalmanVertexUpdator<5>(),
      KalmanVertexTrackCompatibilityEstimator<5>(), KalmanVertexSmoother());
  ...
  for (typename std::vector<VTX>::const_iterator sv = secondaryVertices.begin();
    ...
    //            std::cout << "Vertex : " << sv-secondaryVertices->begin() << "
    //            " << sv->position() << std::endl;
    Measurement1D dlen =
        vdist.distance(pv, VertexState(RecoVertex::convertPos(sv->position()),
    ...
    for (unsigned int itrack = 0; itrack < selectedTracks.size(); itrack++) {
      ...
      if (cachedIP.count(itrack)) {
        ...
        std::pair<bool, Measurement1D> ipvp =
            IPTools::absoluteImpactParameter3D(tt, pv);
      ...
      TrajectoryStateOnSurface tsos = extrapolator.extrapolate(
          tt.impactPointState(), RecoVertex::convertPos(sv->position()));
      if (!tsos.isValid())
        continue;
      GlobalPoint refPoint = tsos.globalPosition();
      GlobalError refPointErr = tsos.cartesianError().position();
      Measurement1D isv =
          dist.distance(VertexState(RecoVertex::convertPos(sv->position()),
                                    RecoVertex::convertError(sv->error())),
                        VertexState(refPoint, refPointErr));

      float dR =
          Geom::deltaR2(((sign > 0) ? flightDir : -flightDir),
                        tt.track()); //.eta(), flightDir.phi(),
                                     // tt.track().eta(), tt.track().phi());
      ...
      if (w > 0 || (isv.significance() < sigCut && isv.value() < distCut &&
                    isv.value() < dlen.value() * dLenFraction &&
                    timeSig < maxTimeSignificance)) {

        if ((isv.value() < ipv.value()) && isv.value() < distCut &&
            isv.value() < dlen.value() * dLenFraction && dR < dRCut &&
            timeSig < maxTimeSignificance) {
#ifdef VTXDEBUG
          if (w > 0.5)
            std::cout << " = ";
          ...
          if (w > 0.5 && isv.value() > ipv.value())
            ...
            std::cout << "   ";
#endif
          // add also the tracks used in previous fitting that are still closer
          // to Sv than Pv
        ...
        std::cout << "t : " << itrack << " ref " << ref.key() << " w: " << w
                  ...
                  << " pvip: " << ipv.significance() << " " << ipv.value()
                  << " res " << tt.track().residualX(0) << ","
                  << tt.track().residualY(0)
//                  << " tpvip: " << itpv.second.significance() << " " <<
//                  itpv.second.value()  << " dr: "   << dR << std::endl;
#endif
        ...
        std::cout << " . t : " << itrack << " ref " << ref.key() << " w: " << w
                  ...
                  << isv.second.value() << " pvip: " << ipv.significance()
                  << " " << ipv.value() << " dr: " << dR << std::endl;
#endif
      }
    }
    ...
    if (selTracks.size() >= 2) {
      ...
      if (singleFitVertex.isValid()) {
        ...
        GlobalVector vtxDir = GlobalVector(
            extVertex.p4().X(), extVertex.p4().Y(), extVertex.p4().Z());

        std::cout << " pointing : "
                  << Geom::deltaR(extVertex.position() - pv.position(), vtxDir)
                  << std::endl;
#endif
      }
    }
