...
void EcalCompactTrigPrimProducerTest::analyze(edm::Event const & event, edm::EventSetup const & c){
  ...
  if(!err_) std::cout << "Validation of compact trigger primitive collection "
  ...
  if(err_){
	...
	if((trigPrim->sample(trigPrim->sampleOfInterest()).raw() & 0x1FFF) //masked unused bits (see flaw in unpacker that modifies them)
	   != (trigPrim2->sample(0).raw() & 0x1FFF)) err("Trig prim differs: ") << *trigPrim
										<< " (" << std::hex
  ...
  std::cout << "Validation of skimmed trigger primitive collection "
	    << (err_ ? "failed" : "succeeded" ) << "\n";
}
