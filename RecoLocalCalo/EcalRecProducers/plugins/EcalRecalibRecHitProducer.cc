...
EcalRecalibRecHitProducer::EcalRecalibRecHitProducer(const edm::ParameterSet& ps) {
   ...
   EBRecHitTag_ = ps.getParameter<edm::InputTag>("EBRecHitCollection");
   EERecHitTag_ = ps.getParameter<edm::InputTag>("EERecHitCollection");

   EBRecHitToken_ = consumes<EBRecHitCollection>(EBRecHitTag_);
   EERecHitToken_ = consumes<EBRecHitCollection>(EERecHitTag_);
