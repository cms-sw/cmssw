                ...
                for(const auto & EBRecHit : *EBRecHits) {
                        ...
                        if (doIntercalib_) {
                                ...
                                if( icalit!=icalMap.end() ){
                                        ...
                                        edm::LogError("EcalRecHitError") << "No intercalib const found for xtal " << EBDetId(EBRecHit.id()) << "! something wrong with EcalIntercalibConstants in your DB? "
                                                ;
                                }
                        }
                ...
                for(const auto & EERecHit : *EERecHits) {
                        ...
                        if (doIntercalib_) {
                                ...
                                if( icalit!=icalMap.end() ) {
                                        ...
                                        edm::LogError("EcalRecHitError") << "No intercalib const found for xtal " << EEDetId(EERecHit.id()) << "! something wrong with EcalIntercalibConstants in your DB? ";
                                }
                        }
                        // get laser coefficient
                        float lasercalib = 1;
