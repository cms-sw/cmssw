...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
  ...
  void EcalUncalibRecHitProducerPortable::produce(device::Event& event, device::EventSetup const& setup) {
    ...
    auto uncalibRecHitSizeViewEB = cms::alpakatools::make_device_view<uint32_t>(queue),
         uncalibRecHitsDevEB.view().size();
    auto uncalibRecHitSizeViewEE = cms::alpakatools::make_device_view<uint32_t>(queue),
         uncalibRecHitsDevEE.view().size();
