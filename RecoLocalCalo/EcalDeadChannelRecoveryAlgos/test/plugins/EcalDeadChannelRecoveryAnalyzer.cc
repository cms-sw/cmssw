...
EcalDeadChannelRecoveryAnalyzer::EcalDeadChannelRecoveryAnalyzer(
    const edm::ParameterSet &iConfig) {
  originalRecHitCollectionT_ = consumes<EcalRecHitCollection>(
      iConfig.getParameter<edm::InputTag>("originalRecHitCollection"));

  recoveredRecHitCollectionT_ = consumes<EcalRecHitCollection>(
      iConfig.getParameter<edm::InputTag>("recoveredRecHitCollection"));
  ...
  histoEnergies_ =
      fs->make<TH1D>("originalEnergies", title.c_str(), 256, 0, 10);

  title = titlePrefix + "Recovered energies.";
  histoRecovery_ =
      fs->make<TH1D>("recoveryEnergies", title.c_str(), 256, 0, 10);
}
