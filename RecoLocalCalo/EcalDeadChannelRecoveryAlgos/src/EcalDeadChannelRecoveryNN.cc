...
template <>
void EcalDeadChannelRecoveryNN<EBDetId>::setCaloTopology(
    const CaloTopology *topo) {
  topology_ = topo->getSubdetectorTopology(DetId::Ecal, EcalBarrel);
}

template <>
void EcalDeadChannelRecoveryNN<EEDetId>::setCaloTopology(
    const CaloTopology *topo) {
...
void EcalDeadChannelRecoveryNN<T>::load_file(MultiLayerPerceptronContext &ctx,
  ...
  // crashing.
  {
    ...
    std::unique_ptr<TDirectory, decltype(resetGDirectory)> guard(
        gDirectory, resetGDirectory);
    gDirectory = nullptr;
    ctx.mlp = std::make_unique<TMultiLayerPerceptron>(
        "@z1,@z2,@z3,@z4,@z5,@z6,@z7,@z8:10:5:zf", ctx.tree.get());
  }
  ctx.mlp->LoadWeights(path.c_str());
}

template <> void EcalDeadChannelRecoveryNN<EBDetId>::load() {
...
template <> void EcalDeadChannelRecoveryNN<EEDetId>::load() {
  std::string p = "RecoLocalCalo/EcalDeadChannelRecoveryAlgos/data/NNWeights/";
...
template <typename T>
double EcalDeadChannelRecoveryNN<T>::recover(
    const T id, const EcalRecHitCollection &hit_collection, double Sum8Cut,
    bool *AcceptFlag) {
  ...
  double sum8_RelMC =
      makeNxNMatrice_RelMC(id, hit_collection, MNxN_RelMC, AcceptFlag);
  double sum8_RelDC =
      makeNxNMatrice_RelDC(id, hit_collection, MNxN_RelDC, AcceptFlag);
  ...
  if (*AcceptFlag) {
    if (sum8_RelDC > Sum8Cut && sum8_RelMC > Sum8Cut) {
      ...
      //  Matrices "MNxN_RelMC" and "MNxN_RelDC" have now the full set of
      //  energies, the original ones plus whatever "estimates" by the ANN for
      //  the "dead" xtal. Use those full matrices and calculate probabilities.
      //
      ...
      double frMNxN_RelMC[9];
      for (int i = 0; i < 9; i++) {
        frMNxN_RelMC[i] = MNxN_RelMC[i] / SumMNxN_RelMC;
      }

      double prMNxN_RelMC =
          P::Diagonal(frMNxN_RelMC[LU]) * P::UpDown(frMNxN_RelMC[UU]) *
          P::Diagonal(frMNxN_RelMC[RU]) * P::ReftRight(frMNxN_RelMC[LL]) *
          P::Central(frMNxN_RelMC[CC]) * P::ReftRight(frMNxN_RelMC[RR]) *
          P::Diagonal(frMNxN_RelMC[LD]) * P::UpDown(frMNxN_RelMC[DD]) *
          P::Diagonal(frMNxN_RelMC[RD]);
      ...
      double frMNxN_RelDC[9];
      for (int i = 0; i < 9; i++) {
        frMNxN_RelDC[i] = MNxN_RelDC[i] / SumMNxN_RelDC;
      }

      double prMNxN_RelDC =
          P::Diagonal(frMNxN_RelDC[LU]) * P::UpDown(frMNxN_RelDC[UU]) *
          P::Diagonal(frMNxN_RelDC[RU]) * P::ReftRight(frMNxN_RelDC[LL]) *
          P::Central(frMNxN_RelDC[CC]) * P::ReftRight(frMNxN_RelDC[RR]) *
          P::Diagonal(frMNxN_RelDC[LD]) * P::UpDown(frMNxN_RelDC[DD]) *
          P::Diagonal(frMNxN_RelDC[RD]);

      if (prMNxN_RelDC > prMNxN_RelMC) {
        NewEnergy = NewEnergy_RelDC;
        sum8 = sum8_RelDC;
      }
      if (prMNxN_RelDC <= prMNxN_RelMC) {
        NewEnergy = NewEnergy_RelMC;
        sum8 = sum8_RelMC;
      }

      //  If the return value of "CorrectDeadChannelsNN" is one of the followin
      //  negative values then it corresponds to an error condition. See
      //  "CorrectDeadChannelsNN.cc" for possible values.
      if (NewEnergy == -1000000.0 || NewEnergy == -1000001.0 ||
          NewEnergy == -2000000.0 || NewEnergy == -3000000.0 ||
          NewEnergy == -3000001.0) {
        *AcceptFlag = false;
        NewEnergy = 0.0;
      }
  ...
  // From the distribution of (max.cont.xtal / Sum8) we get as limit 5 (hard)
  // and 10 (softer) Choose 10 as highest possible energy to be assigned to the
  // dead channel under any scenario.
  if (NewEnergy > 10.0 * sum8) {
    *AcceptFlag = false;
    NewEnergy = 0.0;
  }
...
double EcalDeadChannelRecoveryNN<T>::estimateEnergy(double *M3x3Input,
  ...
  for (int i = 0; i < 9; i++) {
    if (TMath::Abs(M3x3Input[i]) < epsilon) {
      ...
      //  Generally the "dead" cells are allowed to have negative energies
      //  (since they will be estimated by the ANN anyway). But all the
      //  remaining "live" ones must have positive values otherwise the
      //  logarithm fails.
  ...
  //  Currently EXACTLY ONE AND ONLY ONE dead cell is corrected. Return
  //  -1000000.0 if zero DC's detected and -101.0 if more than one DC's exist.
  int idxDC = -1;
  if (missing_index == 0) {
    return -1000000.0;
  } //  Zero DC's were detected
  if (missing_index > 1) {
    return -1000001.0;
  } //  More than one DC's detected.
  if (missing_index == 1) {
    idxDC = missing[0];
  }
...
double EcalDeadChannelRecoveryNN<DetIdT>::makeNxNMatrice_RelMC(
    DetIdT itID, const EcalRecHitCollection &hit_collection, double *MNxN_RelMC,
    bool *AcceptFlag) {
  //  Since ANN corrects within a 3x3 window, the possible candidate 3x3 windows
  //  that contain the "dead" crystal form a 5x5 window around it (totaly eight
  //  3x3 windows overlapping). Get this 5x5 and locate the Max.Contain.Crystal
  //  within.
  ...
  //  Loop over all cells in the vector "NxNaroundDC", and for each cell find
  //  it's energy (from the EcalRecHits collection). Use this energy to detect
  //  the Max.Cont.Crystal.
  std::vector<DetId>::const_iterator theCells;

  for (theCells = NxNaroundDC.begin(); theCells != NxNaroundDC.end();
       ++theCells) {
...
#if 1
  // Not a smart check, because why not just get 4x4 matrix and have a
  // guaranteed hit?
  ...
  for (testCell = NxNaroundMaxCont.begin(); testCell != NxNaroundMaxCont.end();
       ++testCell) {
    if (itID == DetIdT(*testCell)) {
      dcIn3x3 = true;
    }
  }
...
template <>
double EcalDeadChannelRecoveryNN<EBDetId>::makeNxNMatrice_RelDC(
    EBDetId id, const EcalRecHitCollection &hit_collection, double *MNxN,
    bool *AcceptFlag) {
  //  Make an ANN 3x3 energy matrix around the crystal.
  //  If the "dead" crystal is at the EB boundary (+/- 85) do nothing since we
  //  need a full 3x3 around it.
  if (id.ieta() == 85 || id.ieta() == -85) {
    *AcceptFlag = false;
    return 0.0;
  }
  ...
  for (itCells = window.begin(); itCells != window.end(); ++itCells) {
    ...
    if (!cell.null()) {
      ...
      if (goS_it != hit_collection.end()) {
        ...
        if (cell.ieta() == ietaP && cell.iphi() == iphiP) {
          MNxN[RU] = energy;
        } else if (cell.ieta() == ietaP && cell.iphi() == iphiZ) {
          MNxN[RR] = energy;
        } else if (cell.ieta() == ietaP && cell.iphi() == iphiN) {
          MNxN[RD] = energy;
        }

        else if (cell.ieta() == ietaZ && cell.iphi() == iphiP) {
          MNxN[UU] = energy;
        } else if (cell.ieta() == ietaZ && cell.iphi() == iphiZ) {
          MNxN[CC] = energy;
        } else if (cell.ieta() == ietaZ && cell.iphi() == iphiN) {
          MNxN[DD] = energy;
        }

        else if (cell.ieta() == ietaN && cell.iphi() == iphiP) {
          MNxN[LU] = energy;
        } else if (cell.ieta() == ietaN && cell.iphi() == iphiZ) {
          MNxN[LL] = energy;
        } else if (cell.ieta() == ietaN && cell.iphi() == iphiN) {
          MNxN[LD] = energy;
        }
...
template <>
double EcalDeadChannelRecoveryNN<EEDetId>::makeNxNMatrice_RelDC(
    EEDetId itID, const EcalRecHitCollection &hit_collection, double *MNxN,
    bool *AcceptFlag) {
  //  Make an ANN 3x3 energy matrix around the crystal.
  //  If the "dead" crystal is at the EB boundary (+/- 85) do nothing since we
  //  need a full 3x3 around it.

  //  If the "dead" crystal is at the EE boundary (inner or outer ring) do
  //  nothing since we need a full 3x3 around it.
  if (EEDetId::isNextToRingBoundary(itID)) {
    *AcceptFlag = false;
    return 0.0;
  }
  ...
  //  Loop over all cells in the vector "NxNaroundRefXtal", and fill the MNxN
  //  matrix to be passed to the ANN for prediction.
  std::vector<DetId>::const_iterator itCells;

  for (itCells = NxNaroundRefXtal.begin(); itCells != NxNaroundRefXtal.end();
       ++itCells) {
    ...
    if (!cell.null()) {
      ...
      if (goS_it != hit_collection.end()) {
        ...
        if (cell.ix() == ixP && cell.iy() == iyP) {
          MNxN[RU] = energy;
        } else if (cell.ix() == ixP && cell.iy() == iyZ) {
          MNxN[RR] = energy;
        } else if (cell.ix() == ixP && cell.iy() == iyN) {
          MNxN[RD] = energy;
        }

        else if (cell.ix() == ixZ && cell.iy() == iyP) {
          MNxN[UU] = energy;
        } else if (cell.ix() == ixZ && cell.iy() == iyZ) {
          MNxN[CC] = energy;
        } else if (cell.ix() == ixZ && cell.iy() == iyN) {
          MNxN[DD] = energy;
        }

        else if (cell.ix() == ixN && cell.iy() == iyP) {
          MNxN[LU] = energy;
        } else if (cell.ix() == ixN && cell.iy() == iyZ) {
          MNxN[LL] = energy;
        } else if (cell.ix() == ixN && cell.iy() == iyN) {
          MNxN[LD] = energy;
        }
