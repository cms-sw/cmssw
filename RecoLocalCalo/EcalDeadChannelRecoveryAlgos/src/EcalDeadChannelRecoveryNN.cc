...
void EcalDeadChannelRecoveryNN<T>::load_file(MultiLayerPerceptronContext& ctx, std::string fn) {
  ...
  // crashing.
  {
    ...
    gDirectory = nullptr;
    ctx.mlp = std::make_unique<TMultiLayerPerceptron>("@z1,@z2,@z3,@z4,@z5,@z6,@z7,@z8:10:5:zf", ctx.tree.get());
  }
...
double EcalDeadChannelRecoveryNN<T>::recover(const T id,
  ...
  if (*AcceptFlag) {
    if (sum8_RelDC > Sum8Cut && sum8_RelMC > Sum8Cut) {
      ...
      //
      double SumMNxN_RelMC = MNxN_RelMC[LU] + MNxN_RelMC[UU] + MNxN_RelMC[RU] + MNxN_RelMC[LL] + MNxN_RelMC[CC] +
                             MNxN_RelMC[RR] + MNxN_RelMC[LD] + MNxN_RelMC[DD] + MNxN_RelMC[RD];
      ...
      double prMNxN_RelMC = P::Diagonal(frMNxN_RelMC[LU]) * P::UpDown(frMNxN_RelMC[UU]) *
      ...
      double SumMNxN_RelDC = MNxN_RelDC[LU] + MNxN_RelDC[UU] + MNxN_RelDC[RU] + MNxN_RelDC[LL] + MNxN_RelDC[CC] +
                             MNxN_RelDC[RR] + MNxN_RelDC[LD] + MNxN_RelDC[DD] + MNxN_RelDC[RD];
      ...
      double prMNxN_RelDC = P::Diagonal(frMNxN_RelDC[LU]) * P::UpDown(frMNxN_RelDC[UU]) *
      ...
      if (NewEnergy == -1000000.0 || NewEnergy == -1000001.0 || NewEnergy == -2000000.0 || NewEnergy == -3000000.0 ||
          NewEnergy == -3000001.0) {
        *AcceptFlag = false;
        NewEnergy = 0.0;
      }
