...
/**\class CastorTowerProducer CastorTowerProducer.cc
 RecoLocalCalo/Castor/src/CastorTowerProducer.cc

 Description: CastorTower Reconstruction Producer. Produce CastorTowers from
 CastorCells. Implementation:
*/
...
private:
  ...
  virtual void ComputeTowerVariable(
      const edm::RefVector<edm::SortedCollection<CastorRecHit>> &usedRecHits,
      double &Ehot, double &depth);
...
CastorTowerProducer::CastorTowerProducer(const edm::ParameterSet &iConfig)
  ...
  tok_input_ = consumes<CastorRecHitCollection>(
      iConfig.getParameter<std::string>("inputprocess"));
...
void CastorTowerProducer::produce(edm::Event &iEvent,
  ...
  if (nRecHits == 0)
    LogDebug("CastorTowerProducer") << "Warning: You are trying to run the "
                                       "Tower algorithm with 0 input rechits.";

  // declare castor array
  // (0,x): Energies - (1,x): emEnergies - (2,x): hadEnergies - (3,x): phi
  // position
  ...
  // loop over rechits to build castortowerarray[4][16] and
  // castorusedrechits[16]
  for (unsigned int i = 0; i < InputRecHits->size(); i++) {

    edm::Ref<CastorRecHitCollection> rechit_p =
        edm::Ref<CastorRecHitCollection>(InputRecHits, i);
  ...
  } // end loop over rechits to build castortowerarray[4][16] and
    // castorusedrechits[16]
  ...
  for (int k = 0; k < 16; k++) {
    ...
    if (poscastortowerarray[0][k] >
        sqrt(poscastorusedrechits[k].size()) * towercut_) {
      ...
      LogDebug("CastorTowerProducer")
          << "tower " << k + 1 << ": fem = " << fem << " ,depth = " << depth
          << " ,Ehot = " << Ehot << std::endl;

      TowerPoint temptowerposition(rhoTower, 5.9, poscastortowerarray[3][k]);
      Point towerposition(temptowerposition);

      CastorTower newtower(poscastortowerarray[0][k], towerposition,
                           poscastortowerarray[1][k], poscastortowerarray[2][k],
                           fem, depth, Ehot, poscastorusedrechits[k]);
    ...
    if (negcastortowerarray[0][k] >
        sqrt(negcastorusedrechits[k].size()) * towercut_) {
      ...
      LogDebug("CastorTowerProducer")
          << "tower " << k + 1 << " energy = " << negcastortowerarray[0][k]
          << "EM = " << negcastortowerarray[1][k]
          << "HAD = " << negcastortowerarray[2][k]
      ...
      CastorTower newtower(negcastortowerarray[0][k], towerposition,
                           negcastortowerarray[1][k], negcastortowerarray[2][k],
                           fem, depth, Ehot, negcastorusedrechits[k]);
      OutputTowers->push_back(newtower);
...
void CastorTowerProducer::ComputeTowerVariable(
    const edm::RefVector<edm::SortedCollection<CastorRecHit>> &usedRecHits,
    double &Ehot, double &depth) {
  ...
  for (CastorRecHitRefVector::iterator it = usedRecHits.begin();
       it != usedRecHits.end(); it++) {
