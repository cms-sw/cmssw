...
//
/**\class CastorInvalidDataFilter CastorInvalidDataFilter.cc
 RecoLocalCalo/CastorInvalidDataFilter/src/CastorInvalidDataFilter.cc
...
CastorInvalidDataFilter::CastorInvalidDataFilter(
  ...
  tok_summary_ = consumes<std::vector<edm::ErrorSummaryEntry>>(
      edm::InputTag("logErrorHarvester"));
}
...
bool CastorInvalidDataFilter::filter(edm::Event &iEvent,
  ...
  for (size_t i = 0; i < summary->size(); i++) {
    ...
    // std::cout << " category = " << error.category << " module = " <<
    // error.module << " severity = "
    //        << error.severity.getName() << " count = " << error.count <<
    //        std::endl;
    if (error.category == "Invalid Data" &&
        error.module == "CastorRawToDigi:castorDigis")
      invalid = true;
  }
...
// module  ------------
void CastorInvalidDataFilter::fillDescriptions(
