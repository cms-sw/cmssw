...
//
/**\class CastorClusterProducer CastorClusterProducer.cc
 RecoLocalCalo/Castor/src/CastorClusterProducer.cc

 Description: CastorCluster Reconstruction Producer. Produces Clusters from
...
CastorClusterProducer::CastorClusterProducer(const edm::ParameterSet &iConfig)
  ...
  tok_tower_ =
      consumes<CastorTowerCollection>(edm::InputTag("CastorTowerReco"));
...
void CastorClusterProducer::produce(edm::Event &iEvent,
  ...
  if (!input_.empty()) {
    ...
    auto OutputClustersfromClusterAlgo =
        std::make_unique<CastorClusterCollection>();
    ...
    if (nTowers == 0)
      LogDebug("CastorClusterProducer")
          << "Warning: You are trying to run the Cluster algorithm with 0 "
             "input towers.";
  ...
  if (!basicjets_.empty()) {
    ...
    auto OutputClustersfromBasicJets =
        std::make_unique<CastorClusterCollection>();
    ...
    for (unsigned i = 0; i < bjCollection->size(); i++) {
      ...
      for (itParticle = ccp.begin(); itParticle != ccp.end(); ++itParticle) {
        const CastorTower *castorcand =
            dynamic_cast<const CastorTower *>(itParticle->get());
        // cout << " castortowercandidate reference energy = " <<
        // castorcand->castorTower()->energy() << endl; cout << "
        // castortowercandidate reference eta = " <<
        // castorcand->castorTower()->eta() << endl; cout << "
        // castortowercandidate reference phi = " <<
        // castorcand->castorTower()->phi() << endl; cout << "
        // castortowercandidate reference depth = " <<
        // castorcand->castorTower()->depth() << endl;
        ...
        width += pow(phiangle(castorcand->phi() - bj->phi()), 2) *
                 castorcand->energy();
        fhot += castorcand->fhot() * castorcand->energy();

        // loop over rechits
        for (edm::RefVector<edm::SortedCollection<CastorRecHit>>::iterator it =
                 castorcand->rechitsBegin();
             it != castorcand->rechitsEnd(); it++) {
      ...
      CastorCluster cc(energy, position, emEnergy, hadEnergy, emEnergy / energy,
                       width, depth, fhot, sigmaz, usedTowers);
