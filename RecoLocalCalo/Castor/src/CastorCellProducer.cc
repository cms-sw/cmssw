...
/**\class CastorCellProducer CastorCellProducer.cc
 RecoLocalCalo/Castor/src/CastorCellProducer.cc

 Description: CastorCell Reconstruction Producer. Produce CastorCells from
 CastorRecHits. Implementation:
*/
...
CastorCellProducer::CastorCellProducer(const edm::ParameterSet &iConfig) {
  tok_input_ = consumes<CastorRecHitCollection>(
      edm::InputTag(iConfig.getUntrackedParameter<std::string>("inputprocess",
                                                               "castorreco")));
...
void CastorCellProducer::beginJob() {
  LogDebug("CastorCellProducer") << "Starting CastorCellProducer";
}

// ------------ method called once each job just after ending the event loop
// ------------
void CastorCellProducer::endJob() {
  LogDebug("CastorCellProducer") << "Ending CastorCellProducer";
}
