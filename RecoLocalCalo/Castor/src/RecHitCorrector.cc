...
//
/**\class RecHitCorrector RecHitCorrector.cc
...
void RecHitCorrector::produce(edm::Event &iEvent,
  ...
  if (!rechits.isValid())
    edm::LogWarning("CastorRecHitCorrector")
        << "No valid CastorRecHitCollection found, please check the "
           "InputLabel...";
  ...
  for (unsigned int i = 0; i < rechits->size(); i++) {
    ...
    if (doInterCalib_) {
      ...
      const CastorCalibrations &calibrations =
          conditions->getCastorCalibrations(rechit.id());
