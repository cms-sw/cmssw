...
//
/**\class CaloTowersMerger CaloTowersMerger.cc
 RecoLocalCalo/CaloTowersMerger/src/CaloTowersMerger.cc
...
void CaloTowersMerger::produce(edm::Event &iEvent,
  ...
  if (!regTower.isValid() && !extraTower.isValid()) {
    edm::LogError("CaloTowersMerger")
        << "both input tag:" << regularTowerTag << " and " << extraTowerTag
    ...
    for (; et_it != et_end; ++et_it) {
      if (std::find(overlappingTowers.begin(), overlappingTowers.end(),
                    et_it) == overlappingTowers.end())
...
CaloTower CaloTowersMerger::mergedTower(const CaloTower &rt,
                                        const CaloTower &et) {
  ...
  // crude test if one has HO energy and the other not (possibly due to bad hit
  // cleanup) However: for |iEta|>15 E_outer has different meening: it holds
  // either the energy in the outer depths in HCAL, etc

  if (rt.ietaAbs() < 16 &&
      (fabs(rt.outerEnergy() - et.outerEnergy()) < 0.00001)) {
  ...
  for (ec_it = ec_begin; ec_it != ec_end; ++ec_it) {
    // if properly resconstructed, the only possible overlap should be for HO
    // hits that are always listed as constituents if above thereshold (old
    // JetMET request)
    if (std::find(combinedConstituents.begin(), combinedConstituents.end(),
                  *ec_it) == combinedConstituents.end())
  ...
  if (rt_hasEcalConstit && et_hasEcalConstit) {
    ...
    if (rt.emEnergy() > 0 && et.emEnergy() > 0) {
      ...
      double x = rt.emEnergy() * rt.emPosition().x() +
                 et.emEnergy() * et.emPosition().x();
      double y = rt.emEnergy() * rt.emPosition().y() +
                 et.emEnergy() * et.emPosition().y();
      double z = rt.emEnergy() * rt.emPosition().z() +
                 et.emEnergy() * et.emPosition().z();
  ...
  CaloTower mergedTower(rt.id(), rt.emEnergy() + et.emEnergy(),
                        rt.hadEnergy() + et.hadEnergy(), newOuterE, -1, -1,
  ...
  unsigned int numBadHcalChan = rt.numBadHcalCells() -
                                et.numProblematicHcalCells() -
                                rt.numRecoveredHcalCells();
  unsigned int numBadEcalChan = rt.numBadEcalCells() -
                                et.numProblematicEcalCells() -
                                rt.numRecoveredEcalCells();

  unsigned int numProbHcalChan =
      rt.numProblematicHcalCells() + et.numProblematicHcalCells();
  unsigned int numProbEcalChan =
      rt.numProblematicEcalCells() + et.numProblematicEcalCells();

  unsigned int numRecHcalChan =
      rt.numRecoveredHcalCells() + et.numRecoveredHcalCells();
  unsigned int numRecEcalChan =
      rt.numRecoveredEcalCells() + et.numRecoveredEcalCells();

  mergedTower.setCaloTowerStatus(numBadHcalChan, numBadEcalChan, numRecHcalChan,
                                 numRecEcalChan, numProbHcalChan,
                                 numProbEcalChan);

  // use timing from the good tower only (for now, in default reco we use
  // information from good hits only) time is saved as integer but returned as
  // float in (ns)
