...
CaloTowerCandidateCreator::CaloTowerCandidateCreator(const ParameterSet &p)
  ...
  tok_src_ =
      consumes<CaloTowerCollection>(p.getParameter<edm::InputTag>("src"));
...
void CaloTowerCandidateCreator::produce(Event &evt, const EventSetup &) {
  ...
  for (; idx < caloTowers->size(); idx++) {
    ...
    if (mVerbose >= 2) {
      std::cout << "CaloTowerCandidateCreator::produce-> " << idx
    ...
    if (cal->et() >= mEtThreshold && cal->energy() >= mEThreshold) {
      ...
      cands->push_back(c);
      if (mVerbose >= 2)
        std::cout << "accepted: pT/eta/phi:" << c->pt() << '/' << c->eta()
                  << '/' << c->phi() << std::endl;
    } else {
      if (mVerbose >= 2)
  ...
  if (mVerbose >= 1) {
    std::cout << "CaloTowerCandidateCreator::produce-> " << cands->size()
