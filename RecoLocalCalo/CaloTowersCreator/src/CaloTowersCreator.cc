...
CaloTowersCreator::CaloTowersCreator(const edm::ParameterSet &conf)
    : algo_(conf.getParameter<double>("EBThreshold"),
      ...
      theHcalAcceptSeverityLevel_(
          conf.getParameter<unsigned int>("HcalAcceptSeverityLevel")),

      theRecoveredHcalHitsAreUsed_(
          conf.getParameter<bool>("UseHcalRecoveredHits")),
      theRecoveredEcalHitsAreUsed_(
          conf.getParameter<bool>("UseEcalRecoveredHits")),

      // paramaters controlling the use of rejected hits

      useRejectedHitsOnly_(conf.getParameter<bool>("UseRejectedHitsOnly")),

      theHcalAcceptSeverityLevelForRejectedHit_(conf.getParameter<unsigned int>(
          "HcalAcceptSeverityLevelForRejectedHit")),

      useRejectedRecoveredHcalHits_(
          conf.getParameter<bool>("UseRejectedRecoveredHcalHits")),
      useRejectedRecoveredEcalHits_(
          conf.getParameter<bool>("UseRejectedRecoveredEcalHits"))

{
  algo_.setMissingHcalRescaleFactorForEcal(
      conf.getParameter<double>("missingHcalRescaleFactorForEcal"));

  // register for data access
  tok_hbhe_ = consumes<HBHERecHitCollection>(
      conf.getParameter<edm::InputTag>("hbheInput"));
  tok_ho_ =
      consumes<HORecHitCollection>(conf.getParameter<edm::InputTag>("hoInput"));
  tok_hf_ =
      consumes<HFRecHitCollection>(conf.getParameter<edm::InputTag>("hfInput"));
  ...
  const std::vector<std::string> severitynames =
      conf.getParameter<std::vector<std::string>>(
  ...
  std::cout << "VI Producer "
            ...
            << (allowMissingInputs_ ? "allowMissing " : " ") << nLabels << ' '
...
void CaloTowersCreator::produce(edm::Event &e, const edm::EventSetup &c) {
  ...
  const HcalSeverityLevelComputer *hcalSevLvlComputer =
      hcalSevLvlComputerHndl.product();
  ...
  algo_.setGeometry(cttopo.product(), ctmap.product(), htopo.product(),
                    pG.product());
  ...
  algo_.setHcalAcceptSeverityLevelForRejectedHit(
  ...
  std::cout << "VI Produce: "
            ...
            << toks_ecal_.size() << ' ' << theEcalSeveritiesToBeExcluded_.size()
            << ' ' << theEcalSeveritiesToBeUsedInBadTowers_.size() << std::endl;
#endif
  ...
  for (std::vector<edm::EDGetTokenT<EcalRecHitCollection>>::const_iterator i =
           toks_ecal_.begin();
       i != toks_ecal_.end(); i++) {
  ...
  std::cout << "VI " << (*prod).size() << " " << totc << " " << totE << " "
            << totP4 << std::endl;
#endif
...
void CaloTowersCreator::fillDescriptions(
