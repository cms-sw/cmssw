...
//
// Modify MetaTower to save energy of rechits for use in tower 4-momentum
// assignment, added containers for timing assignment and for holding status
// information. Anton Anastassov (Northwestern)
//
...
public:
  ...
  CaloTowersCreationAlgo(
      ...
      double HOthresholdPlus1, double HOthresholdMinus1,
      double HOthresholdPlus2, double HOthresholdMinus2, double HF1threshold,
      double HF2threshold, double EBweight, double EEweight, double HBweight,
      double HESweight, double HEDweight, double HOweight, double HF1weight,
      double HF2weight, double EcutTower, double EBSumThreshold,
      double EESumThreshold, bool useHO,
      // (for momentum reconstruction algorithm)
      int momConstrMethod, double momHBDepth, double momHEDepth,
      double momEBDepth, double momEEDepth, int hcalPhase = 0);
  ...
  CaloTowersCreationAlgo(
      ...
      double HOthresholdPlus1, double HOthresholdMinus1,
      double HOthresholdPlus2, double HOthresholdMinus2, double HF1threshold,
      double HF2threshold, const std::vector<double> &EBGrid,
      const std::vector<double> &EBWeights, const std::vector<double> &EEGrid,
      const std::vector<double> &EEWeights, const std::vector<double> &HBGrid,
      const std::vector<double> &HBWeights, const std::vector<double> &HESGrid,
      const std::vector<double> &HESWeights, const std::vector<double> &HEDGrid,
      const std::vector<double> &HEDWeights, const std::vector<double> &HOGrid,
      const std::vector<double> &HOWeights, const std::vector<double> &HF1Grid,
      const std::vector<double> &HF1Weights, const std::vector<double> &HF2Grid,
      const std::vector<double> &HF2Weights, double EBweight, double EEweight,
      double HBweight, double HESweight, double HEDweight, double HOweight,
      double HF1weight, double HF2weight, double EcutTower,
      double EBSumThreshold, double EESumThreshold, bool useHO,
      // (for momentum reconstruction algorithm)
      int momConstrMethod, double momHBDepth, double momHEDepth,
      double momEBDepth, double momEEDepth, int hcalPhase = 0);
  ...
  void setGeometry(const CaloTowerTopology *cttopo,
                   ...
                   const HcalTopology *htopo, const CaloGeometry *geo);
  ...
  void rescaleTowers(const CaloTowerCollection &ctInput,
  ...
  ecalChanStatusForCaloTower(const EcalRecHit *hit);
  ...
  //  {theEcalAcceptSeverityLevelForRejectedHit = level;}
  ...
  GlobalPoint
  hadShwrPos(const std::vector<std::pair<DetId, float>> &metaContains,
             float fracDepth, double hadE);
  GlobalPoint
  emShwrPos(const std::vector<std::pair<DetId, float>> &metaContains,
  ...
  GlobalPoint hadShwPosFromCells(DetId frontCell, DetId backCell,
                                 float fracDepth);

  // for Chris
  GlobalPoint
  emShwrLogWeightPos(const std::vector<std::pair<DetId, float>> &metaContains,
                     float fracDepth, double totEmE);
...
private:
  struct MetaTower {
    ...
    int numBadEcalCells = 0, numRecEcalCells = 0, numProbEcalCells = 0,
        numBadHcalCells = 0, numRecHcalCells = 0, numProbHcalCells = 0;
  };
  ...
  /// looks for a given tower in the internal cache.  If it can't find it, it
  /// makes it.
  MetaTower &find(const CaloTowerDetId &id);

  /// helper method to look up the appropriate threshold & weight
  void getThresholdAndWeight(const DetId &detId, double &threshold,
                             double &weight) const;
  ...
  double theHOthresholdPlus2, theHOthresholdMinus2, theHF1threshold,
      theHF2threshold;
  ...
  double theHBweight, theHESweight, theHEDweight, theHOweight, theHF1weight,
      theHF2weight;
  ...
  // fields that hold the information passed from the CaloTowersCreator
  // configuration file: controll what is considered bad/recovered/problematic
  // channel for CaloTower purposes
  //
  ...
  void convert(const CaloTowerDetId &id, const MetaTower &mt,
  ...
  // Number of channels in the tower that were not used in RecHit production
  // (dead/off,...). These channels are added to the other "bad" channels found
