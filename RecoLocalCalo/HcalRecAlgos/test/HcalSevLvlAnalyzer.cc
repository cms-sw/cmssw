...
void HcalSevLvlAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (unsigned i = 0; i < 4; i++)    //loop over rechitflag
    for (unsigned k = 0; k < 5; k++)  //loop over channelstatus
    {
      ...
      std::cout << "Status for " << myIdHB << " with RHFlag " << sampleHBRHFlag[i] << " (" << std::hex
                << sampleHBRHFlag[i] << ") and ChStFlag " << std::dec << sampleHBChSt[k] << " (" << std::hex
                << sampleHBChSt[k] << std::dec << ") is: " << theLevel;
      std::cout << std::endl;

      bool dropchannel = myProd->dropChannel(sampleHBChSt[k]);
      bool recovered = myProd->recoveredRecHit(myIdHB, sampleHBRHFlag[i]);

      std::cout << "DropChannel status for " << myIdHB << " with RHFlag " << sampleHBRHFlag[i] << " (" << std::hex
                << sampleHBRHFlag[i] << ") and ChStFlag " << std::dec << sampleHBChSt[k] << " (" << std::hex
                << sampleHBChSt[k] << std::dec << ") is: " << dropchannel << ", recovered status is: " << recovered
                << std::endl;
      std::cout << std::endl;
    }
  std::cout << std::endl;
  ...
  for (unsigned i = 0; i < 4; i++)    //loop over rechitflag
    for (unsigned k = 0; k < 5; k++)  //loop over channelstatus
    {
      ...
      std::cout << "Status for " << myIdHE << " with RHFlag " << sampleHERHFlag[i] << " (" << std::hex
                << sampleHERHFlag[i] << ") and ChStFlag " << std::dec << sampleHEChSt[k] << " (" << std::hex
                << sampleHEChSt[k] << std::dec << ") is: " << theLevel;
      std::cout << std::endl;
    }
  std::cout << std::endl;
  ...
  for (unsigned i = 0; i < 4; i++)    //loop over rechitflag
    for (unsigned k = 0; k < 5; k++)  //loop over channelstatus
    {
      ...
      std::cout << "Status for " << myIdHF << " with RHFlag " << sampleHFRHFlag[i] << " (" << std::hex
                << sampleHFRHFlag[i] << ") and ChStFlag " << std::dec << sampleHFChSt[k] << " (" << std::hex
                << sampleHFChSt[k] << std::dec << ") is: " << theLevel;
      std::cout << std::endl;
    }
  std::cout << std::endl;
  ...
  for (unsigned i = 0; i < 3; i++)    //loop over rechitflag
    for (unsigned k = 0; k < 5; k++)  //loop over channelstatus
    {
      ...
      std::cout << "Status for " << myIdHO << " with RHFlag " << sampleHORHFlag[i] << " (" << std::hex
                << sampleHORHFlag[i] << ") and ChStFlag " << std::dec << sampleHOChSt[k] << " (" << std::hex
                << sampleHOChSt[k] << std::dec << ") is: " << theLevel;
      std::cout << std::endl;
    }
  std::cout << std::endl;
...
void HcalSevLvlAnalyzer::beginJob() {}

// ------------ method called once each job just after ending the event loop  ------------
void HcalSevLvlAnalyzer::endJob() {}
