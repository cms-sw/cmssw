...
namespace {
  class IntOrWild {
  public:
    inline IntOrWild(const std::string& spec) : value_(0), wild_(false) {
      if (spec == "*")
        wild_ = true;
      else
    ...
    inline bool matches(const int i) const { return wild_ || value_ == i; }
...
static LabeledShape parseShapeDescription(const edm::ParameterSet& pset,
  ...
  int to = static_cast<int>(pset.getParameter<unsigned>("t0")) + (label == "default" ? dtDefault : timeShift);
...
inline static bool operator<(const HcalPulseShape&, const HcalPulseShape&) { return false; }
...
static void updateChannelMap(const edm::ParameterSet& pset,
  ...
  for (const DetId& id : ids) {
    ...
    if (ieta.matches(hcalId.ieta()) && iphi.matches(hcalId.iphi()) && depth.matches(hcalId.depth())) {
...
static void dumpPulseShape(std::ostream& of, const std::string& label, const HcalPulseShapeLookup::Shape& pulseShape) {
...
//
HcalPulseShapesEP::HcalPulseShapesEP(const edm::ParameterSet& iConfig) : config_(iConfig) {
...
HcalPulseShapesEP::ReturnType HcalPulseShapesEP::produce(const HcalPulseShapeLookupRcd& rcd) {
  ...
  for (unsigned i = 1; i < nShapes; ++i)
    if (shapes[i].first == shapes[i - 1].first)
      throw cms::Exception("HcalPulseShapesEPBadConfig")
          << "Duplicate pulse shape label \"" << shapes[i].first << "\" encountered" << std::endl;
  ...
  for (unsigned isub = 0; isub < nSubDet; ++isub) {
    ...
    for (const edm::ParameterSet& cmap : channelMaps) {
      ...
      if (it == indexLookup.end())
        throw cms::Exception("HcalPulseShapesEPBadConfig")
            << "Previously undefined pulse shape label \"" << label << "\" encountered" << std::endl;
  ...
  if (!mapDumpFile.empty()) {
    ...
    for (unsigned isub = 0; isub < nSubDet; ++isub) {
      ...
      for (const DetId& id : ids) {
        const HcalDetId hcalId(id);
        of << subDetNames[isub] << std::setw(4) << hcalId.ieta() << std::setw(4) << hcalId.iphi() << std::setw(3)
           << hcalId.depth() << "  " << product->getChannelLabel(id) << std::endl;
      }
    }
  }
