...
MahiFit::MahiFit() : fullTSSize_(19), fullTSofInterest_(8) {}
...
void MahiFit::phase1Apply(const HBHEChannelInfo& channelData,
  ...
  if (channelData.hasTimeInfo())
    nnlsWork_.dt = timeSigmaSiPM_;
  else
    nnlsWork_.dt = timeSigmaHPD_;
...
void MahiFit::doFit(std::array<float, 3>& correctedOutput, int nbx) const {
  ...
  for (unsigned int iBX = 0; iBX < nnlsWork_.nPulseTot; ++iBX) {
    ...
    nnlsWork_.pulseCovArray[iBX].setZero(nnlsWork_.tsSize + nnlsWork_.maxoffset + nnlsWork_.bxOffset,
  ...
  if (foundintime) {
    ...
    if (correctedOutput.at(0) != 0) {
      ...
      correctedOutput.at(1) = -9999;  //time
...
void MahiFit::updateCov() const {
  ...
  for (unsigned int iBX = 0; iBX < nnlsWork_.nPulseTot; ++iBX) {
    if (nnlsWork_.ampVec.coeff(iBX) == 0)
      continue;
    ...
    if (offset == pedestalBX_)
      ...
      invCovMat +=
          nnlsWork_.ampVec.coeff(iBX) * nnlsWork_.ampVec.coeff(iBX) *
          nnlsWork_.pulseCovArray.at(offset + nnlsWork_.bxOffset)
              .block(nnlsWork_.maxoffset - offset, nnlsWork_.maxoffset - offset, nnlsWork_.tsSize, nnlsWork_.tsSize);
    }
  }
