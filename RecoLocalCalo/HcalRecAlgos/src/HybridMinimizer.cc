...
//    std::cout << " add parameter " << name << "  " <<  val << " step " << step << std::endl;
   ...
   if (step <= 0) { 
      ...
      if( !suppressMessages_ ) edm::LogWarning("HybridMinimizer") << "HybridMinimizer::SetVariable : " << txtmsg;
   ...
   if ( minuit2Index != ivar) {
      ...
      if( !suppressMessages_ )  edm::LogWarning("HybridMinimizer") << "HybridMinimizer::SetVariable : " << txtmsg << "(minuit2Index=" << minuit2Index << ")";
      ivar = minuit2Index;
      return false;
   }
...
void HybridMinimizer::SetFunction(const  ROOT::Math::IMultiGenFunction & func) { 
   ...
   if (!fUseFumili) {
      ...
      if (!fcnfunc) {
         if( !suppressMessages_ )  edm::LogError("HybridMinimizer") << "Wrong Fit method function for Fumili";
         return;
      }
...
bool HybridMinimizer::Minimize() { 
   ...
   if (!fMinuitFCN) { 
      if( !suppressMessages_ )  edm::LogError("HybridMinimizer") << "HybridMinimizer::Minimize : FCN function has not been set";
      return false; 
  }
...
*/
   ...
   if (validMinimum) { 
      ...
      if (fStatus != 0 && debugLevel > 0) if( !suppressMessages_ )  edm::LogWarning("HybridMinimizer") << "HybridMinimizer::Minimize : " << txt;
   }
   else { 
      ...
      if( !suppressMessages_ )  edm::LogWarning("HybridMinimizer") << "HybridMinimizer::Minimize : " << msg;
   }
...
//    fState = min.UserState();
   if (fMinimum == 0) { 
      if( !suppressMessages_ )  edm::LogError("HybridMinimizer") << "HybridMinimizer::GetMinosErrors:  failed - no function minimum existing";
      return false;
   }
   
   if (!fMinimum->IsValid() ) { 
      if( !suppressMessages_ )  edm::LogError("HybridMinimizer") << "HybridMinimizer::MINOS failed due to invalid function minimum";
      return false;
   }
...
bool HybridMinimizer::Scan(unsigned int ipar, unsigned int & nstep, double * x, double * y, double xmin, double xmax) { 
   ...
   if (!fMinuitFCN) { 
      if( !suppressMessages_ )  edm::LogError("HybridMinimizer") << "HybridMinimizer::Scan : Function must be set before using Scan";
      return false;
   }
   
   if ( ipar > fState.MinuitParameters().size() ) { 
      if( !suppressMessages_ )  edm::LogError("HybridMinimizer") << "HybridMinimizer::Scan : Invalid number. Minimizer variables must be set before using Scan";
      return false;
   }
   ...
   if (result.size() != nstep) { 
      if( !suppressMessages_ )  edm::LogError("HybridMinimizer") << "HybridMinimizer::Scan : Invalid result from MnParameterScan";
      return false; 
   }
   ...
   if (scan.Fval() < amin ) { 
      if (PrintLevel() > 0) if( !suppressMessages_ )  edm::LogInfo("HybridMinimizer") << "HybridMinimizer::Scan : A new minimum has been found";
...
bool HybridMinimizer::Contour(unsigned int ipar, unsigned int jpar, unsigned int & npoints, double * x, double * y) {
   ...
   if (fMinimum == 0) { 
      if( !suppressMessages_ )  edm::LogError("HybridMinimizer") << "HybridMinimizer::Contour : no function minimum existing. Must minimize function before";
      return false;
   }

   if (!fMinimum->IsValid() ) { 
      if( !suppressMessages_ )  edm::LogError("HybridMinimizer") << "HybridMinimizer::Contour : Invalid function minimum";
      return false;
   }
   assert(fMinuitFCN); 
   ...
   if (result.size() != npoints) { 
      if( !suppressMessages_ )  edm::LogError("HybridMinimizer") << "HybridMinimizer::Contour : Invalid result from MnContours";
      return false; 
   }
...
bool HybridMinimizer::Hesse( ) { 
   ...
   if (!fMinuitFCN) { 
      if( !suppressMessages_ )  edm::LogError("HybridMinimizer") << "HybridMinimizer::Hesse : FCN function has not been set";
      return false; 
   }
...
//      std::cout << fState << std::endl; 
   ...
   if (!fState.HasCovariance() ) { 
      ...
      if (PrintLevel() > 0) if( !suppressMessages_ )  edm::LogWarning("HybridMinimizer") << "HybridMinimizer::Hesse : Hesse failed";
