...
void PhysicsTowerOrganizer::findNeighbors(const CaloTowerDetId& tempid, std::set<const PhysicsTower*>& neighbors) const
{
  ...
  if(id.ietaAbs()<=20) {
    if(id.iphi()==72) ids.emplace_back(id.ieta(), 1);
    else              ids.emplace_back(id.ieta(), id.iphi()+1);
  } else {
    if(id.iphi()==71) ids.emplace_back(id.ieta(), 1);
    else              ids.emplace_back(id.ieta(), id.iphi()+2);
  }

  // get the neighbor with the lower iphi
  if(id.ietaAbs()<=20) {
    if(id.iphi()==1) ids.emplace_back(id.ieta(), 72);
    else             ids.emplace_back(id.ieta(), id.iphi()-1);
  } else {
    if(id.iphi()==1) ids.emplace_back(id.ieta(), 71);
    else             ids.emplace_back(id.ieta(), id.iphi()-2);
  }
  ...
  if(id.ietaAbs()<=19 || (id.ietaAbs()==20 && (id.iphi()%2)==0)) {
    if(id.iphi()==72) ids.emplace_back((id.ietaAbs()+1)*id.zside(), 1);
    else              ids.emplace_back((id.ietaAbs()+1)*id.zside(), id.iphi()+1);
  } else if(id.ietaAbs()>=21) {
    if(id.iphi()==71) ids.emplace_back((id.ietaAbs()+1)*id.zside(), 1);
    else              ids.emplace_back((id.ietaAbs()+1)*id.zside(), id.iphi()+2);
  }

  // get the neighbor with higher ieta and lower iphi
  if(id.ietaAbs()<=19) {
    if(id.iphi()==1) ids.emplace_back((id.ietaAbs()+1)*id.zside(), 72);
    else             ids.emplace_back((id.ietaAbs()+1)*id.zside(), id.iphi()-1);
  } else if(id.ietaAbs()>=21 || (id.ietaAbs()==20 && (id.iphi()%2)==1)) {
    if(id.iphi()==1) ids.emplace_back((id.ietaAbs()+1)*id.zside(), 71);
    else             ids.emplace_back((id.ietaAbs()+1)*id.zside(), id.iphi()-2);
  }
