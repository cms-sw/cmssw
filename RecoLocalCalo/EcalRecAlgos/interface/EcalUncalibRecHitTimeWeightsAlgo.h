 ...
 *
 *  The chi2 computation with matrix is replaced by the chi2express which is
 * moved outside the weight algo (need to clean up the interface in next
 * iteration so that we do not pass-by useless arrays)
 *
 */
...
public:
  ...
  /// Compute time
  double time(const C &dataFrame, const std::vector<double> &amplitudes,
    ...
    for (amplit = amplitudes.begin(); amplit < amplitudes.end(); ++amplit) {
      ...
      for (unsigned int isample = firstsamplet; isample < nsample; ++isample) {
        ...
        pedSubSamples(isample) =
            std::max(0., pedSubSamples(isample) -
                             amplitudes[ipulse] * pulse(isample) / pulsenorm);
      }
    }

    // Compute parameters
    double amplitude_(-1.), jitter_(-1.);
    ROOT::Math::SVector<double, 3> param =
