...
public:
  ...
  EcalRecHit makeRecHit(const EcalUncalibratedRecHit &uncalibRH,
    ...
    if (!adcToGeVConstantIsSet_) {
      std::cout << "EcalRecHitSimpleAlgo::makeRecHit: adcToGeVConstant_ not "
    ...
    float energy =
        uncalibRH.amplitude() * adcToGeVConstant_ * intercalibConstant;
    ...
    rh.setEnergyError(uncalibRH.amplitudeError() * adcToGeVConstant_ *
                      intercalibConstant);
    /* rh.setOutOfTimeEnergy( uncalibRH.outOfTimeEnergy() * adcToGeVConstant_ *
     * intercalibConstant ); */
