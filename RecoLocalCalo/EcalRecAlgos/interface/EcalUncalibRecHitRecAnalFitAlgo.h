  ...
  makeRecHit(const C &dataFrame, const double *pedestals,
    ...
    for (int iSample = 0; iSample < C::MAXSAMPLES; iSample++) {
      ...
      if (!iGainSwitch)
        ...
        frame[iSample] = double(((double)(dataFrame.sample(iSample).adc()) -
                                 pedestals[gainId - 1]) *
                                gainRatios[gainId - 1]);
    ...
    // Compute parameters
    // std::cout << "EcalUncalibRecHitRecAnalFitAlgo::makeRecHit() not yey
    // implemented. returning dummy rechit" << std::endl;
    ...
    return EcalUncalibratedRecHit(dataFrame.id(), amplitude_, pedestal_,
                                  jitter_ - 6, chi2_, flag);
  }
};
#endif
