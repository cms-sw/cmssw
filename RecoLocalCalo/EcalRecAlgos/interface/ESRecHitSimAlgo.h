...
private:
  EcalRecHit::ESFlags evalAmplitude(float *result, const ESDataFrame &digi,
                                    float ped) const;

  double *oldEvalAmplitude(const ESDataFrame &digi, const double &ped,
                           const double &w0, const double &w1,
                           const double &w2) const;
