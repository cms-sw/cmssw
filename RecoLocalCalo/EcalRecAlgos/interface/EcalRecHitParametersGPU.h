...
public:
    ...
    std::tuple<intvec, intvec, uint32vec, uint32vec> getValues() const {
        return {ChannelStatusToBeExcluded_, expanded_v_DB_reco_flags_, expanded_Sizes_v_DB_reco_flags_, expanded_flagbit_v_DB_reco_flags_};
    }
...
private:
    ...
    std::vector<uint32_t, cms::cuda::HostAllocator<uint32_t>> expanded_Sizes_v_DB_reco_flags_, expanded_flagbit_v_DB_reco_flags_;
