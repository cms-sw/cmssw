 ...
 *  using an analytical function fit, with the pulse parameters alpha and beta
 * fixed. It follows a fast fit algorithms devolped on test beam data by P.
 * Jarry If the pedestal is not given, it is calculated from the first 2
 * pre-samples;
...
template <class C>
class EcalUncalibRecHitFixedAlphaBetaAlgo
    : public EcalUncalibRecHitRecAbsAlgo<C> {
...
public:
  EcalUncalibRecHitFixedAlphaBetaAlgo<C>()
      : fAlpha_(0.), fBeta_(0.), fAmp_max_(-1.), fTim_max_(-1), fPed_max_(0),
        alfabeta_(0), fNb_iter_(4), fNum_samp_bef_max_(1),
        fNum_samp_after_max_(3), fSigma_ped(1.1), DM1_(3), temp_(3) {
  ...
  EcalUncalibRecHitFixedAlphaBetaAlgo<C>(int n_iter, int n_bef_max = 1,
                                         int n_aft_max = 3,
                                         float sigma_ped = 1.1)
      : fAlpha_(0.), fBeta_(0.), fAmp_max_(-1.), fTim_max_(-1), fPed_max_(0),
        alfabeta_(0), DM1_(3), temp_(3) {
...
EcalUncalibratedRecHit EcalUncalibRecHitFixedAlphaBetaAlgo<C>::makeRecHit(
    const C &dataFrame, const double *pedestals, const double *gainRatios,
    const EcalWeightSet::EcalWeightMatrix **weights,
  ...
  if (pedestals) {
    external_pede = true;
    if (dyn_pedestal) {
      pedestal = (double(dataFrame.sample(0).adc()) +
                  double(dataFrame.sample(1).adc())) /
                 2.;
    } else {
    ...
    for (int iSample = 0; iSample < C::MAXSAMPLES; iSample++) {
      ...
      // FIX-ME: warning: the vector pedestal is supposed to have in the order
      // G12, G6 and G1 if GainId is zero treat it as 3 temporarily to protect
      // against undefined frame will be set to ~max of gain1
      if (GainId == 0) {
        GainId = 3;
        isSaturated = true;
      }
    ...
    for (int iSample = 0; iSample < C::MAXSAMPLES; iSample++) {
      ...
      frame[iSample] = double(dataFrame.sample(iSample).adc()) - pedestal;
      // if gain has switched but no pedestals are available, no much good you
      // can do...
      if (GainId > gainId0)
        iGainSwitch = 1;
      if (frame[iSample] > maxsample) {
  ...
  if ((iGainSwitch == 1 &&
       external_pede == false) || // ... thus you return dummy rechit
  ...
  /*    std::cout << "separate fits\nA: " << fAmp_max_  << ", ResidualPed: " <<
     fPed_max_
              <<", pedestal: "<<pedestal << ", tPeak " << fTim_max_ <<
     std::endl;
  */
  return EcalUncalibratedRecHit(dataFrame.id(), fAmp_max_, pedestal + fPed_max_,
                                fTim_max_ - 5, chi2_, flags);
}
...
void EcalUncalibRecHitFixedAlphaBetaAlgo<C>::InitFitParameters(double *samples,
  ...
  // if timing very badly off, that just use max sample
    ...
    // y=a*(x-xM)^2+b*(x-xM)+c
    doFit_ = true;
    float a = float(samples[max_sample - 1] + samples[max_sample + 1] -
                    2 * samples[max_sample]) /
              2.;
    if (a == 0) {
      doFit_ = false;
      return;
...
template <class C>
float EcalUncalibRecHitFixedAlphaBetaAlgo<C>::PerformAnalyticFit(
    double *samples, int max_sample) {

  // int fValue_tim_max = max_sample;
  //! fit electronic function from simulation
  //! parameters fAlpha_ and fBeta_ are fixed and fit is providing the 3
  //! following parameters the maximum amplitude ( fAmp_max_ ) the time of the
  //! maximum  ( fTim_max_)
  ...
  if (!doFit_) {
    LogDebug("EcalUncalibRecHitFixedAlphaBetaAlgo")
  ...
  for (int iter = 0; iter < fNb_iter_; iter++) {
    ...
    for (int i = num_fit_min; i <= num_fit_max; i++) {
      // if(i>fsamp_edge_fit && i<num_fit_min) continue ; // remove front edge
      // samples
    ...
    DM1_.invert(fail);
    if (fail != 0.) {
      // just a guess from the value of the parameters in the previous
      // interaction; printf("wH4PulseFitWithFunction =====> determinant error
      // --> No Fit Provided !\n") ;
  ...
  // ampli mismatched to MaxSample, ampli largely negative, time off preselected
  // range
  if (fAmp_max_ > 2 * samples[max_sample] || fAmp_max_ < -100 ||
      fTim_max_ < 0 || 9 < fTim_max_) {
    InitFitParameters(samples, max_sample);
    return -104;
  }

  // std::cout <<"chi2: "<<chi2<<" ampl: "<<fAmp_max_<<" time: "<<fTim_max_<<"
  // pede: "<<fPed_max_<<std::endl;
  return chi2;
}
