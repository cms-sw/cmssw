 ...
 *
 *  Template used to compute the chi2 of an MGPA pulse for in-time and
 * out-of-time signals, algorithm based on the chi2express. The in-time chi2 is
 * calculated against the time intercalibrations from the DB while the
 * out-of-time chi2 is calculated against the Tmax measurement on event by event
 * basis.
 *
...
public:
  ...
  EcalUncalibRecHitRecChi2Algo<C>(const C &dataFrame, const double amplitude,
                                  const EcalTimeCalibConstant &timeIC,
                                  const double amplitudeOutOfTime,
                                  const double jitter, const double *pedestals,
...
EcalUncalibRecHitRecChi2Algo<C>::EcalUncalibRecHitRecChi2Algo(
    const C &dataFrame, const double amplitude,
    const EcalTimeCalibConstant &timeIC, const double amplitudeOutOfTime,
    const double jitter, const double *pedestals, const double *pedestalsRMS,
    const double *gainRatios, const EcalShapeBase &testbeamPulseShape,
    const std::vector<double> &chi2Parameters) {
  ...
  for (int iSample = 0; iSample < C::MAXSAMPLES;
       iSample++) // if gain switch use later the pedestal RMS, otherwise we use
                  // the pedestal from the DB
  {
    ...
    if (gainId == 1 && iSample == 0)
      S_0 = dataFrame.sample(iSample)
                .adc(); // take only first presample to estimate the pedestal
    if (gainId == 1 && iSample < 3)
      ped_ave += (1 / 3.0) *
                 dataFrame.sample(iSample)
                     .adc(); // take first 3 presamples to estimate the pedestal
  }
  ...
  for (int iSample = 0; iSample < C::MAXSAMPLES; iSample++) {
    ...
    double ped = !iGainSwitch
                     ? ped_ave
                     : pedestals[gainId - 1]; // use dynamic pedestal for G12
                                              // and average pedestal for G6,G1
    ...
    double R_iErrorSquare =
        noise_A * noise_A + const_A * const_A * amplitude * amplitude;
    ...
    double g_i =
        (testbeamPulseShape)(tzero + shiftTimeOutOfTime +
                             iSample * ADC_clock); // calculate out of time chi2

    double R_iOutOfTime =
        (S_i - S_0) * gainRatios[gainId - 1] - g_i * amplitudeOutOfTime;
    double R_iOutOfTimeErrorSquare =
        noise_B * noise_B +
        const_B * const_B * amplitudeOutOfTime * amplitudeOutOfTime;
  ...
  if (!isSaturated && !iGainSwitch && chi2_ > 0 && chi2OutOfTime_ > 0) {
    chi2_ = 7 * (3 + log(chi2_));
    chi2_ = chi2_ < 0 ? 0 : chi2_; // this is just a convinient mapping for
                                   // storing in the calibRecHit bit map
  ...
  {
    chi2_ = 99.0; // chi2 is very large in these cases, put a code value to
                  // discriminate against normal noise
    chi2OutOfTime_ = 99.0;
  }
}
