...
float EcalCleaningAlgo::recHitE(const DetId id,
  ...
  if (it != recHits.end()) {
    ...
    if (useTimingInfo) {
      ...
      if (id.subdetId() == EcalBarrel &&
          it->checkFlag(EcalRecHit::kOutOfTime) && ene > ignoreOutOfTimeThresh_)
        return 0;
    }
