...
EcalRecHitParametersGPU::EcalRecHitParametersGPU(edm::ParameterSet const& ps) {
    ...
    std::copy(ChannelStatusToBeExcluded.begin(), ChannelStatusToBeExcluded.end(),
        ChannelStatusToBeExcluded_.begin());
...
EcalRecHitParametersGPU::Product const& EcalRecHitParametersGPU::getProduct(cudaStream_t cudaStream) const { 
    auto const& product = 
        product_.dataForCurrentDeviceAsync(cudaStream, [this](EcalRecHitParametersGPU::Product& product, cudaStream_t cudaStream) {
            // malloc
            cudaCheck(cudaMalloc((void**)&product.ChannelStatusToBeExcluded, this->ChannelStatusToBeExcluded_.size() * sizeof(int)));
            cudaCheck(cudaMalloc((void**)&product.expanded_v_DB_reco_flags, this->expanded_v_DB_reco_flags_.size() * sizeof(int)));
            cudaCheck(cudaMalloc((void**)&product.expanded_Sizes_v_DB_reco_flags, this->expanded_Sizes_v_DB_reco_flags_.size() * sizeof(uint32_t)));
            cudaCheck(cudaMalloc((void**)&product.expanded_flagbit_v_DB_reco_flags, this->expanded_flagbit_v_DB_reco_flags_.size() * sizeof(uint32_t)));
