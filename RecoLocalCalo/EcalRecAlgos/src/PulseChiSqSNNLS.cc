...
void eigen_solve_submatrix(PulseMatrix &mat, PulseVector &invec,
                           PulseVector &outvec, unsigned NP) {
  ...
  default:
    throw cms::Exception("MultFitWeirdState")
        << "Weird number of pulses encountered in multifit, module is "
           "configured incorrectly!";
  }
}

PulseChiSqSNNLS::PulseChiSqSNNLS()
    : _chisq(0.), _computeErrors(true), _maxiters(50), _maxiterwarnings(true) {}

PulseChiSqSNNLS::~PulseChiSqSNNLS() {}

bool PulseChiSqSNNLS::DoFit(const SampleVector &samples,
                            const SampleMatrix &samplecov, const BXVector &bxs,
  ...
  for (int gainidx = 0; gainidx < ngains; ++gainidx) {
    ...
    SampleVector pedestal =
        (gains.array() == mask.array()).cast<SampleVector::value_type>();
    if (pedestal.maxCoeff() > 0.) {
      ++nPedestals;
      _bxs.resize(npulse + nPedestals);
      _bxs[npulse + nPedestals - 1] =
          100 + gainidx; // bx values >=100 indicate dynamic pedestals
      _pulsemat.resize(Eigen::NoChange, npulse + nPedestals);
      _pulsemat.col(npulse + nPedestals - 1) = pedestal;
    }
  }

  // construct negative step functions for saturated or potentially
  // slew-rate-limited samples
  for (int isample = 0; isample < SampleVector::RowsAtCompileTime; ++isample) {
    if (badSamples.coeff(isample) > 0) {
      ...
      // step correction has negative sign for saturated or slew-limited samples
      // which have been forced to zero
      step[isample] = -1.;

      ++nPedestals;
      _bxs.resize(npulse + nPedestals);
      _bxs[npulse + nPedestals - 1] =
          -100 - isample; // bx values <=-100 indicate step corrections for
                          // saturated or slew-limited samples
      _pulsemat.resize(Eigen::NoChange, npulse + nPedestals);
  ...
  for (int ipulse = 0; ipulse < npulse; ++ipulse) {
    ...
    int offset = 7 - 3 - bx;
    _pulsemat.col(ipulse) =
        fullpulse.segment<SampleVector::RowsAtCompileTime>(offset);
  }

  // unconstrain pedestals already for first iteration since they should always
  // be non-zero
  if (nPedestals > 0) {
...
bool PulseChiSqSNNLS::Minimize(const SampleMatrix &samplecov,
  ...
  while (true) {
    ...
    if (iter >= _maxiters) {
      if (_maxiterwarnings) {
        LogDebug("PulseChiSqSNNLS::Minimize")
...
bool PulseChiSqSNNLS::updateCov(const SampleMatrix &samplecov,
  ...
  for (unsigned int ipulse = 0; ipulse < npulse; ++ipulse) {
    ...
    int bx = _bxs.coeff(ipulse);
    if (std::abs(bx) >= 100)
      continue; // no contribution to covariance from pedestal or
                // saturation/slew step correction
...
bool PulseChiSqSNNLS::NNLS() {
  ...
  aTbvec.noalias() =
      invcovp.transpose().lazyProduct(_covdecomp.matrixL().solve(_sampvec));
    ...
    while (true) {
      ...
      for (unsigned int ipulse = 0; ipulse < _nP; ++ipulse) {
        if (ampvecpermtest.coeff(ipulse) <= 0.) {
          ...
          const double ratio =
              c_ampvec / (c_ampvec - ampvecpermtest.coeff(ipulse));
          if (ratio < minratio) {
            minratio = ratio;
            minratioidx = ipulse;
          }
