...
EcalSeverityLevelAlgo::EcalSeverityLevelAlgo(const edm::ParameterSet &p) {
  ...
  for (unsigned int is = 0; is != severities.size(); ++is) {
    ...
    EcalSeverityLevel::SeverityLevel snum = (EcalSeverityLevel::SeverityLevel)
        StringToEnumValue<EcalSeverityLevel::SeverityLevel>(severities[is]);
  ...
  for (unsigned int is = 0; is != dbseverities.size(); ++is) {
    ...
    for (unsigned int ifi = 0; ifi != dbflags.size(); ++ifi) {
      EcalChannelStatusCode::Code f = (EcalChannelStatusCode::Code)
          StringToEnumValue<EcalChannelStatusCode::Code>(dbflags[ifi]);
