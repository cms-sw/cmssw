...
EcalRecHit::ESFlags ESRecHitSimAlgo::evalAmplitude(float *results,
  ...
  for (int i = 0; i < digi.size(); i++) {
    ...
    LogDebug("ESRecHitSimAlgo")
        << "ESRecHitSimAlgo : Digi " << i << " ADC counts "
        << digi.sample(i).adc() << " Ped " << ped;
  ...
  auto aaa =
      (A2 > 0 && A1 > 0) ? std::log(A2 / A1) / n : 20.f; // if A1=0, t0=20
...
#else
  constexpr float A_1 =
      1. / (std::pow(w / n * (t1), n) * std::exp(n - w * (t1)));
#endif
  auto AA1 = A1 * A_1;
...
EcalRecHit ESRecHitSimAlgo::reconstruct(const ESDataFrame &digi) const {
  ...
  auto mipCalib =
      (mip != 0.f) ? MIPGeV_ * std::abs(vdt::fast_cosf(ang)) / (mip) : 0.f;
  energy *= mipCalib;
  otenergy *= mipCalib;

  LogDebug("ESRecHitSimAlgo")
      << "ESRecHitSimAlgo : reconstructed energy " << energy;
...
/*
  ...
  std::cout <<  "ESd " << digi.id() <<" : "
            ...
            << rechit.outOfTimeEnergy()<<"," << oldHit.outOfTimeEnergy() <<
std::endl;
...
double *ESRecHitSimAlgo::oldEvalAmplitude(const ESDataFrame &digi,
                                          const double &ped, const double &w0,
                                          const double &w1,
...
EcalRecHit ESRecHitSimAlgo::oldreconstruct(const ESDataFrame &digi) const {
  ...
  ESIntercalibConstantMap::const_iterator it_mip =
      mips_->getMap().find(digi.id());
  ESAngleCorrectionFactors::const_iterator it_ang =
      ang_->getMap().find(digi.id());

  ESChannelStatusMap::const_iterator it_status =
      channelStatus_->getMap().find(digi.id());
  ...
  double mipCalib =
      (fabs(cos(*it_ang)) != 0.) ? (*it_mip) / fabs(cos(*it_ang)) : 0.;
