...
HBHEPlan1Combiner::HBHEPlan1Combiner(const edm::ParameterSet& conf)
  ...
  if (!combiner_.get())
    throw cms::Exception("HBHEPlan1BadConfig") << "Invalid Plan1RechitCombiner algorithm configuration" << std::endl;
...
void HBHEPlan1Combiner::produce(edm::Event& iEvent, const edm::EventSetup& eventSetup) {
  using namespace edm;
  ...
  combiner_->clear();
  for (auto it = inputRechits->begin(); it != inputRechits->end(); ++it) {
