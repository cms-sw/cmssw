      ...
      for (unsigned int j = i + 1; j < es.size(); ++j) {
        if (vused[j] == 0) {
          ...
          if (layer <= lastLayerEE)
            radius2 = radii[0] * radii[0];
          else if (layer <= lastLayerFH)
            radius2 = radii[1] * radii[1];
          else if (layer <= lastLayerBH)
            radius2 = radii[2] * radii[2];
          else
            assert(radius2 < 100. && "nonsense layer value - cannot assign multicluster radius");
