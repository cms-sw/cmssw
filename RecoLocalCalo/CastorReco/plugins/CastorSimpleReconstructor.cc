...
CastorSimpleReconstructor::CastorSimpleReconstructor(
    ...
    : reco_(conf.getParameter<int>("firstSample"),
      ...
      det_(DetId::Hcal), firstSample_(conf.getParameter<int>("firstSample")),
  ...
  tok_input_ = consumes<CastorDigiCollection>(
      conf.getParameter<edm::InputTag>("digiLabel"));
  ...
  if (!strcasecmp(subd.c_str(), "CASTOR")) {
    ...
    edm::LogWarning("CastorSimpleReconstructor")
        << "CastorSimpleReconstructor is not associated with CASTOR "
           "subdetector!"
        << std::endl;
  }
}
...
void CastorSimpleReconstructor::produce(edm::Event &e,
  ...
  if (tsFromDB_) {
    ...
    if (!recoparams.isValid()) {
      tsFromDB_ = false;
      edm::LogWarning("CastorSimpleReconstructor")
          << "Could not handle the CastorRecoParamsRcd correctly, using "
             "parameters from cfg file from this event onwards... These "
             "parameters could be wrong for this run... please check"
          << std::endl;
    }
  }
  ...
  if (det_ == DetId::Calo && subdet_ == HcalCastorDetId::SubdetectorId) {
    ...
    for (i = digi->begin(); i != digi->end(); i++) {
      ...
      if (tsFromDB_) {
        const CastorRecoParam *param_ts =
            recoparams->getValues(detcell.rawId());
        reco_.resetTimeSamples(param_ts->firstSample(),
                               param_ts->samplesToAdd());
      }
      ...
      if (setSaturationFlag_) {
        ...
        if (doSaturationCorr_ &&
            rec->back().flagField(HcalCaloFlagLabels::ADCSaturationBit)) {
          // get saturation correction value
          const CastorSaturationCorr *saturationCorr =
              satcorr->getValues(detcell.rawId());
          double satCorrConst = 1.;
          satCorrConst = saturationCorr->getValue();
          reco_.recoverADCSaturation(rec->back(), coder, calibrations, *i,
                                     maxADCvalue_, satCorrConst);
        }
      }
    }
    // return result
