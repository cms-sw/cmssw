...
constexpr double MaximumFractionalError =
    0.0005; // 0.05% error allowed from this source

CastorSimpleRecAlgo::CastorSimpleRecAlgo(int firstSample, int samplesToAdd,
                                         bool correctForTimeslew,
                                         bool correctForPulse, float phaseNS)
    : firstSample_(firstSample), samplesToAdd_(samplesToAdd),
      correctForTimeslew_(correctForTimeslew) {
  if (correctForPulse)
    pulseCorr_ = std::make_unique<CastorPulseContainmentCorrection>(
        samplesToAdd_, phaseNS, MaximumFractionalError);
}
...
/// Timeshift correction for HPDs based on the position of the peak ADC
/// measurement.
///  Allows for an accurate determination of the relative phase of the pulse
///  shape from the HPD.  Calculated based on a weighted sum of the -1,0,+1
///  samples relative to the peak as follows:  wpksamp = (0*sample[0] +
///  1*sample[1] + 2*sample[2]) / (sample[0] + sample[1] + sample[2]) where
///  sample[1] is the maximum ADC sample value.
...
inline RecHit reco(const Digi &digi, const CastorCoder &coder,
                   const CastorCalibrations &calibs, int ifirst, int n,
                   bool slewCorrect,
  ...
  float time = -9999;
  ////Cannot calculate time value with max ADC sample at first or last position
  /// in window....
  ...
  */
    ...
    float t0 =
        fabs((tool[maxI - 1] - calibs.pedestal(capid)) * calibs.gain(capid));
    ...
    if (corr != nullptr) {
      ...
      //      std::cout << fc_ampl << " --> " << corr->getCorrection(fc_ampl) <<
      //      std::endl;
    }
...
template <class Digi>
inline bool isSaturated(const Digi &digi, const int &maxADCvalue, int ifirst,
                        int n) {
...
template <class Digi, class RecHit>
inline bool corrSaturation(RecHit &rechit, const CastorCoder &coder,
                           const CastorCalibrations &calibs, const Digi &digi,
                           const int &maxADCvalue, const double &satCorrConst,
                           int ifirst, int n) {
  ...
  if (digi[ifirst].adc() >= maxADCvalue &&
      digi[ifirst + 1].adc() < maxADCvalue) {
    float ampl = 0;
...
CastorRecHit
CastorSimpleRecAlgo::reconstruct(const CastorDataFrame &digi,
  ...
  return CastorSimpleRecAlgoImpl::reco<CastorDataFrame, CastorRecHit>(
      digi, coder, calibs, firstSample_, samplesToAdd_, false, nullptr,
      CastorTimeSlew::Fast);
}
...
void CastorSimpleRecAlgo::checkADCSaturation(CastorRecHit &rechit,
  ...
  if (CastorSimpleRecAlgoImpl::isSaturated<CastorDataFrame>(
          digi, maxADCvalue, firstSample_, samplesToAdd_))
    rechit.setFlagField(1, HcalCaloFlagLabels::ADCSaturationBit);
}

//++++ Saturation Correction +++++
void CastorSimpleRecAlgo::recoverADCSaturation(
    CastorRecHit &rechit, const CastorCoder &coder,
    const CastorCalibrations &calibs, const CastorDataFrame &digi,
    const int &maxADCvalue, const double &satCorrConst) const {
  if (CastorSimpleRecAlgoImpl::corrSaturation<CastorDataFrame, CastorRecHit>(
          rechit, coder, calibs, digi, maxADCvalue, satCorrConst, firstSample_,
          samplesToAdd_))
