...
public:
  ...
  CastorSimpleRecAlgo(int firstSample, int samplesToAdd,
                      bool correctForTimeslew, bool correctForContainment,
                      float fixedPhaseNs);
  ...
  void checkADCSaturation(CastorRecHit &rechit, const CastorDataFrame &digi,
                          const int &maxADCvalue) const;

  //++++ Saturation Correction +++++
  // recover pulse shape if ADC count is >= masADCvalue
  void recoverADCSaturation(CastorRecHit &rechit, const CastorCoder &coder,
                            const CastorCalibrations &calibs,
                            const CastorDataFrame &digi, const int &maxADCvalue,
