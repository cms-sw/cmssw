...
void ElectronTestAnalyzer::myVar(const reco::GsfElectron& ele,
  ...
  myMVAVar_kfhits          =  (validKF) ? myTrackRef->getHitPattern().trackerLayersWithMeasurement(HitPattern::TRACK_HITS) : -1.;
...
bool ElectronTestAnalyzer::trainTrigPresel(const reco::GsfElectron& ele) {
  ...
  if(fabs(ele.superCluster()->eta()) < 1.479) {
    if(ele.sigmaIetaIeta() < 0.014 &&
       ...
       ele.gsfTrack()->getHitPattern().numberOfLostHits(HitPattern::MISSING_INNER_HITS) == 0)
      myTrigPresel = true;
  }
  else {
