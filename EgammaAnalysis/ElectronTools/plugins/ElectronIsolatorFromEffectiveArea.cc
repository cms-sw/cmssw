...
ElectronIsolatorFromEffectiveArea::ElectronIsolatorFromEffectiveArea(const edm::ParameterSet& config)
  ...
  if (config.existsAs<edm::InputTag>("gsfElectrons"))
    gsfElectronTag = config.getParameter<edm::InputTag>("gsfElectrons");
  if (config.existsAs<edm::InputTag>("pfElectrons"))
    pfElectronTag = config.getParameter<edm::InputTag>("pfElectrons");
  if (config.existsAs<edm::InputTag>("patElectrons"))
    patElectronTag = config.getParameter<edm::InputTag>("patElectrons");
  if (!gsfElectronTag.label().empty())
    gsfElectronToken = consumes<reco::GsfElectronCollection>(gsfElectronTag);
  if (!pfElectronTag.label().empty())
    pfElectronToken = consumes<reco::PFCandidateCollection>(pfElectronTag);
  if (!patElectronTag.label().empty())
    patElectronToken = consumes<pat::ElectronCollection>(patElectronTag);
  produces<CandDoubleMap>();
}

bool ElectronIsolatorFromEffectiveArea::filter(edm::Event& event, const edm::EventSetup&) {
  std::unique_ptr<CandDoubleMap> product(new CandDoubleMap());
  CandDoubleMap::Filler filler(*product);

  edm::Handle<double> rho;
  event.getByToken(rhoIsoToken, rho);
