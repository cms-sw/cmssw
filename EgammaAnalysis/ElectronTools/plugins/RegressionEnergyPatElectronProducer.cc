...
RegressionEnergyPatElectronProducer::RegressionEnergyPatElectronProducer(const edm::ParameterSet& cfg) {
  inputGsfElectronsToken_ =
      mayConsume<reco::GsfElectronCollection>(cfg.getParameter<edm::InputTag>("inputElectronsTag"));
  ...
  if (inputCollectionType_ == 0 && !useReducedRecHits_) {
    throw cms::Exception("InconsistentParameters")
        << " *** Inconsistent configuration : if you read GsfElectrons, you should set useRecHitCollections to true "
           "and provide the correcte values to recHitCollectionEB and recHitCollectionEE (most probably "
           "reducedEcalRecHitsEB and reducedEcalRecHitsEE )"
        << std::endl;
  }
  ...
  if (energyRegressionType_ == 1)
    ...
    type = ElectronEnergyRegressionEvaluate::kWithTrkVar;
  ...
  for (unsigned iele = 0; iele < nElectrons_; ++iele) {
    ...
    if (energyRegressionType_ == 1) {
      RegressionMomentum = regressionEvaluator_->regressionValueNoTrkVar(mySCHelper->rawEnergy(),
                                                                         ...
                                                                         debug_);
      RegressionMomentumError =
          regressionEvaluator_->regressionUncertaintyNoTrkVar(mySCHelper->rawEnergy(),
                                                              mySCHelper->eta(),
                                                              mySCHelper->phi(),
                                                              mySCHelper->r9(),
                                                              mySCHelper->etaWidth(),
      ...
      RegressionMomentum = regressionEvaluator_->regressionValueWithSubClusters(mySCHelper->rawEnergy(),
                                                                                mySCHelper->eta(),
                                                                                mySCHelper->phi(),
                                                                                mySCHelper->r9(),
                                                                                mySCHelper->etaWidth(),
                                                                                ...
                                                                                debug_);
      RegressionMomentumError =
          regressionEvaluator_->regressionUncertaintyWithSubClusters(mySCHelper->rawEnergy(),
                                                                     mySCHelper->eta(),
                                                                     mySCHelper->phi(),
                                                                     mySCHelper->r9(),
                                                                     mySCHelper->etaWidth(),
      ...
      RegressionMomentum = regressionEvaluator_->regressionValueWithTrkVar(ele->p(),
                                                                           ...
                                                                           debug_);
      RegressionMomentumError = regressionEvaluator_->regressionUncertaintyWithTrkVar(ele->p(),
                                                                                      mySCHelper->rawEnergy(),
                                                                                      mySCHelper->eta(),
                                                                                      mySCHelper->phi(),
                                                                                      mySCHelper->etaWidth(),
