...
ElectronEnergyCalibratorRun2::ElectronEnergyCalibratorRun2(EpCombinationTool &combinator, 
														   ...
														   std::vector<double> smearings, //to be removed
														   std::vector<double> scales, //to be removed
...
void ElectronEnergyCalibratorRun2::calibrate(SimpleElectron &electron, edm::StreamID const & id) const 
{
	...
	smear = _correctionRetriever.getSmearingSigma(electron.getRunNumber(), electron.getNewEnergy(), electron.isEB(), electron.getR9(), aeta); 
