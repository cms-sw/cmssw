...
/**
 * The AlgorithmCalibration class is the interface between user code and
 * calibration framework developed for BReco subsytem. Both the analysis
 * programs and the calibration programs should only access calibration
 * information via that class.
 *
 * An algorithm is supposed to keep a single instance of that class to avoid the
 * reload of XML for every  event (during analysis) and because you want to
 * accumulate over all events the data to perform a new calibration in the
 * calibration program.
 *
...
public:
  ...
  template <class CI>
  void updateCalibration(const typename T::Input &calibrationInputForCategory,
