...
public:
  ...
  const CalibDataT *
  getCalibData(const typename CategoryT::Input &calibrationInput) const {
  ...
  const std::vector<std::pair<CategoryT, CalibDataT>> &
  categoriesWithData() const {
    return m_categoriesWithData;
  }
...
template <class CategoryT, class CalibDataT>
int CalibrationInterface<CategoryT, CalibDataT>::getIndex(
    const typename CategoryT::Input &calibrationInput) const {
  int i = 0;
  int found = -1;
  for (typename std::vector<std::pair<CategoryT, CalibDataT>>::const_iterator
           it = m_categoriesWithData.begin();
       it != m_categoriesWithData.end(); it++) {

    if ((*it).first.match(calibrationInput)) {
      if (found >= 0) {
        edm::LogWarning("BTagCalibration")
...
template <class CategoryT, class CalibDataT>
const CalibDataT *
CalibrationInterface<CategoryT, CalibDataT>::getCalibData(int i) const {
  size_t ii = i;
...
template <class CategoryT, class CalibDataT>
int CalibrationInterface<CategoryT, CalibDataT>::addCategoryDefinition(
...
template <class CategoryT, class CalibDataT>
int CalibrationInterface<CategoryT, CalibDataT>::addEntry(
    const CategoryT &categoryDefinition, const CalibDataT &data) {
...
template <class CategoryT, class CalibDataT>
void CalibrationInterface<CategoryT, CalibDataT>::setCalibData(
    int index, const CalibDataT &data) {
