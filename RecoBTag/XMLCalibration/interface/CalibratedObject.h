 ...
 * This is the abstract class of any object that the calibration framework
 * returns to the algorithm. Actually in the simplified framework it is not
 * needed to inherit from it, it is enough to mimic the same interface. Example
 * of "CalibratedObjects" are PDFs,Neural Network set of parameters, ...
 */
...
public:
  ...
  /**
   * You have to impelement a different updateCalibration(CalibrationInput) in
   * the derived class for each CalibrationInput you want to be able to
   * calibrate on. So for example you may want to have an
   * updateCalibration(RecTrack) but also a updateCalibration(JetWithTracks)
   * that only loops on jet rectracks and calls updateCalibration(RecTrack).
   *
   * This implementation do nothing.
   */
  virtual void updateCalibration() {}

  /** Return a name for your calibrated object. It is used as XML tag name in
   * reading and writing.
   */
