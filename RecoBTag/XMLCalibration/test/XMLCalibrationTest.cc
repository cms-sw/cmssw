...
// In the example .xml files I put two categories one with range 0..2.5 the
// other with
// rang 2.5...5
class TestCategory : public CalibrationCategory<float> {
public:
  bool match(const float &input)
      const override // const reference  for float is stupid but input object
...
XMLCalibrationTest::XMLCalibrationTest(const edm::ParameterSet &iConfig) {
  m_calib = new AlgorithmCalibration<TestCategory, CalibratedHistogramXML>(
      "test.xml");
}

XMLCalibrationTest::~XMLCalibrationTest() { delete m_calib; }

void XMLCalibrationTest::analyze(const edm::Event &iEvent,
                                 const edm::EventSetup &iSetup) {
  using namespace edm;
  // ask the algorithm the calibrated object for an input value
  // of 1.2
  // this will search for the matching category and return the associated
  // calibrated object.
