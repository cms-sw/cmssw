...
void CalibratedHistogramXML::read(XERCES_CPP_NAMESPACE::DOMElement *dom) {
  ...
  for (bin = 0; bin < size; bin++) {
    ...
    if (n1) {
      ...
      binValues.push_back(
          CalibrationXML::readAttribute<double>(binElement, "value"));

      binULimits.push_back(
          CalibrationXML::readAttribute<double>(binElement, "uLimit"));
      n1 = n1->getNextSibling();
    }
  }
  if (bin > 0)
    binValues.push_back(
        CalibrationXML::readAttribute<int>(dom, "overFlowValue"));

  limits = Range(binULimits.front(), binULimits.back());
  totalValid = false;
}

void CalibratedHistogramXML::write(
