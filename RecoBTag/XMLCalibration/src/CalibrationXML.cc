...
void CalibrationXML::openFile(const std::string &xmlFileName) {
  ...
  try {
    ...
    abort();
    // FIXME		throw GenTerminate("Error during Xerces-c
    // Initialization.");
  }
  ...
  //  Parse the XML file, catching any XML exceptions that might propogate out
  //  of it.
  bool errorsOccured = false;
  try {
    edm::LogInfo("XMLCalibration")
        << "Calibration XML: parsing " << m_xmlFileName.c_str() << std::endl;
  ...
  if (n1 == nullptr ||
      strcmp("Calibration", XMLString::transcode(n1->getNodeName())))
    abort();
  // FIXME		throw GenTerminate("The root element in the XML
  // Calibration file is not a Calibration element.\n This should be forbidden
  // at the DTD level.");
  else {
...
void CalibrationXML::saveFile(const std::string &xmlFileName) {
  DOMImplementation *theImpl = DOMImplementationRegistry::getDOMImplementation(
      XMLString::transcode("Core"));
  DOMLSSerializer *theSerializer =
      ((DOMImplementation *)theImpl)->createLSSerializer();
...
DOMElement *CalibrationXML::addChild(DOMNode *dom, const std::string &name) {
  ...
  if (dom->getFirstChild() == nullptr)
    dom->appendChild(dom->getOwnerDocument()->createTextNode(
        XMLString::transcode(indent.c_str())));

  DOMElement *child =
      (DOMElement *)dom->appendChild(dom->getOwnerDocument()->createElement(
