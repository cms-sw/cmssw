...
pair<bool, double> HistogramProbabilityEstimator::probability(
    bool quality, int ipType, float significance, const Track& track, const Jet& jet, const Vertex& vertex) const {
  ...
  if (ipType == 0) {
    it = m_calibration3D->data.begin();
    it_end = m_calibration3D->data.end();
  } else if (ipType == 1) {
    it = m_calibration2D->data.begin();
    it_end = m_calibration2D->data.end();
  } else
  ...
  if (found != it_end)
    probabilityHistogram = &found->histogram;
