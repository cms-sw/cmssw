...
float MuonTaggerNoIP::discriminator(const TagInfoHelper& tagInfo) const {
  ...
  for (unsigned int i = 0; i < info.leptons(); i++) {
    ...
    if (m_selector(properties)) {
      float tag =
          theNet.value(
              0, properties.ptRel, properties.ratioRel, properties.deltaR, info.jet()->energy(), info.jet()->eta()) +
          theNet.value(
              1, properties.ptRel, properties.ratioRel, properties.deltaR, info.jet()->energy(), info.jet()->eta());
      if (tag > bestTag)
        bestTag = tag;
    }
  }
