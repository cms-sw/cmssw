...
bool BTagSkimLeptonJet::filter(edm::Event &iEvent,
  ...
  // Jet cuts
  int nJet = 0;
  for (CaloJetCollection::const_iterator ajet = TheCaloJets.begin();
       ajet != TheCaloJets.end(); ++ajet) {

    if ((fabs(ajet->eta()) < MaxCaloJetEta_) && (ajet->pt() > MinCaloJetPt_)) {

      if (LeptonType_ == "muon") {
        for (MuonCollection::const_iterator amuon = TheMuons.begin();
             amuon != TheMuons.end(); ++amuon) {
          // select good muon
          if ((amuon->pt() > MinLeptonPt_) &&
              (fabs(amuon->eta()) < MaxLeptonEta_)) {

            double deltar = ROOT::Math::VectorUtil::DeltaR(ajet->p4().Vect(),
                                                           amuon->momentum());

            TVector3 jetvec(ajet->p4().Vect().X(), ajet->p4().Vect().Y(),
                            ajet->p4().Vect().Z());
            TVector3 muvec(amuon->momentum().X(), amuon->momentum().Y(),
                           amuon->momentum().Z());
            jetvec += muvec;
      ...
      if (LeptonType_ == "electron") {
        for (GsfElectronCollection::const_iterator anelectron =
                 TheElectrons.begin();
             anelectron != TheElectrons.end(); anelectron++) {
          if ((anelectron->pt() > MinLeptonPt_) &&
              (fabs(anelectron->eta()) < MaxLeptonEta_)) {

            double deltar = ROOT::Math::VectorUtil::DeltaR(
                ajet->p4().Vect(), anelectron->momentum());
...
void BTagSkimLeptonJet::endJob() {
  edm::LogVerbatim("BTagSkimLeptonJet")
      ...
      << LeptonType_ << ") BTagSkimLeptonJet: " << nAccepted_
