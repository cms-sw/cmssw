...
int main(int argc, char** argv) {
  ...
  if (plHandle.isValid() && wpHandle.isValid()) {
    ...
    std::cout << " nbeff/nberr ?" << perf.isResultOk(PerformanceResult::BTAGNBEFF, p) << "/"
              << perf.isResultOk(PerformanceResult::BTAGNBERR, p) << std::endl;
    ...
    std::cout << " now I ask for a calibration but I do not set eta in the binning point ---> should return all not "
                 "available "
              << std::endl;
    p.reset();
    ...
    std::cout << "invalid handle: workingPoint " << wpHandle.isValid() << " payload " << plHandle.isValid()
              << std::endl;
    try {
      *wpHandle;
      *plHandle;
