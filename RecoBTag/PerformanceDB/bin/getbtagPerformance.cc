...
int main(int argc, char** argv) {
  ...
  parser.addOption("rootFile", CommandLineParser::kString, "root filename");
  parser.addOption("payload", CommandLineParser::kString, "for example MISTAGSSVHEM, MCPfTCHEMb", "MISTAGSSVHEM");
  parser.addOption("flavor", CommandLineParser::kString, "for example b, c, or l", "b");
  parser.addOption("type", CommandLineParser::kString, "for example eff or SF", "eff");
  parser.addOption("pt", CommandLineParser::kDouble, "jet pt");
  parser.addOption("eta", CommandLineParser::kDouble, "jet eta");
  ...
  if (plHandle.isValid() && wpHandle.isValid()) {
    ...
    if (type == "eff") {
      if (flavor == "b")
        cout << "eff/err = " << perf.getResult(PerformanceResult::BTAGBEFF, p) << " / "
             << perf.getResult(PerformanceResult::BTAGBERR, p) << endl;
      if (flavor == "c")
        cout << "eff/err = " << perf.getResult(PerformanceResult::BTAGCEFF, p) << " / "
             << perf.getResult(PerformanceResult::BTAGCERR, p) << endl;
      if (flavor == "l")
        cout << "eff/err = " << perf.getResult(PerformanceResult::BTAGLEFF, p) << " / "
             << perf.getResult(PerformanceResult::BTAGLERR, p) << endl;
    } else if (type == "SF") {
      if (flavor == "b")
        cout << "SF/err = " << perf.getResult(PerformanceResult::BTAGBEFFCORR, p) << " / "
             << perf.getResult(PerformanceResult::BTAGBERRCORR, p) << endl;
      if (flavor == "c")
        cout << "SF/err = " << perf.getResult(PerformanceResult::BTAGCEFFCORR, p) << " / "
             << perf.getResult(PerformanceResult::BTAGCERRCORR, p) << endl;
      if (flavor == "l")
        cout << "SF/err = " << perf.getResult(PerformanceResult::BTAGLEFFCORR, p) << " / "
             << perf.getResult(PerformanceResult::BTAGLERRCORR, p) << endl;
    }
