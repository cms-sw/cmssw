...
void DeepFlavourJetTagProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for (std::size_t i=0; i < input_sizes.size(); i++) {
    ...
    auto & t = dnn_inputs_.at(i);
    t.init(input_shape.size(), &input_shape[0], TF_FLOAT);
