...
struct DeepDoubleCvBTFCache {
  DeepDoubleCvBTFCache() : graphDef(nullptr) {
  }

  std::atomic<tensorflow::GraphDef*> graphDef;
};

class DeepDoubleCvBTFJetTagsProducer : public edm::stream::EDProducer<edm::GlobalCache<DeepDoubleCvBTFCache>> {

  public:
    explicit DeepDoubleCvBTFJetTagsProducer(const edm::ParameterSet&, const DeepDoubleCvBTFCache*);
    ~DeepDoubleCvBTFJetTagsProducer() override;

    static void fillDescriptions(edm::ConfigurationDescriptions&);

    static std::unique_ptr<DeepDoubleCvBTFCache> initializeGlobalCache(const edm::ParameterSet&);
    static void globalEndJob(const DeepDoubleCvBTFCache*);
...
DeepDoubleCvBTFJetTagsProducer::DeepDoubleCvBTFJetTagsProducer(const edm::ParameterSet& iConfig,
  const DeepDoubleCvBTFCache* cache) :
...
DeepDoubleCvBTFJetTagsProducer::~DeepDoubleCvBTFJetTagsProducer()
{
...
void DeepDoubleCvBTFJetTagsProducer::fillDescriptions(edm::ConfigurationDescriptions& descriptions)
{
  ...
  desc.add<edm::FileInPath>("graph_path",
    edm::FileInPath("RecoBTag/Combined/data/DeepDoubleC/constantgraphlasthopefully.pb"));
    //edm::FileInPath("RecoBTag/Combined/data/DeepDoubleC/DDCvB.pb"));
  ...
  descriptions.add("pfDeepDoubleCvBJetTags", desc);
}

std::unique_ptr<DeepDoubleCvBTFCache> DeepDoubleCvBTFJetTagsProducer::initializeGlobalCache(
  const edm::ParameterSet& iConfig)
{
  ...
  DeepDoubleCvBTFCache* cache = new DeepDoubleCvBTFCache();
  cache->graphDef = tensorflow::loadGraphDef(pbFile);

  return std::unique_ptr<DeepDoubleCvBTFCache>(cache);
}

void DeepDoubleCvBTFJetTagsProducer::globalEndJob(const DeepDoubleCvBTFCache* cache)
{
...
void DeepDoubleCvBTFJetTagsProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
...
DEFINE_FWK_MODULE(DeepDoubleCvBTFJetTagsProducer);
