...
// In TensorFlow, the computational graph is stored 1in a stateless graph object which can be shared
...
struct DeepDoubleCvLTFCache {
  DeepDoubleCvLTFCache() : graphDef(nullptr) {
  }

  std::atomic<tensorflow::GraphDef*> graphDef;
};

class DeepDoubleCvLTFJetTagsProducer : public edm::stream::EDProducer<edm::GlobalCache<DeepDoubleCvLTFCache>> {

  public:
    explicit DeepDoubleCvLTFJetTagsProducer(const edm::ParameterSet&, const DeepDoubleCvLTFCache*);
    ~DeepDoubleCvLTFJetTagsProducer() override;

    static void fillDescriptions(edm::ConfigurationDescriptions&);

    static std::unique_ptr<DeepDoubleCvLTFCache> initializeGlobalCache(const edm::ParameterSet&);
    static void globalEndJob(const DeepDoubleCvLTFCache*);
  ...
  private:
    typedef std::vector<reco::DeepDoubleCvLTagInfo> TagInfoCollection;
...
DeepDoubleCvLTFJetTagsProducer::DeepDoubleCvLTFJetTagsProducer(const edm::ParameterSet& iConfig,
  const DeepDoubleCvLTFCache* cache) :
...
DeepDoubleCvLTFJetTagsProducer::~DeepDoubleCvLTFJetTagsProducer()
{
...
void DeepDoubleCvLTFJetTagsProducer::fillDescriptions(edm::ConfigurationDescriptions& descriptions)
{

  // pfDeepDoubleCvLJetTags
  edm::ParameterSetDescription desc;
  desc.add<edm::InputTag>("src", edm::InputTag("pfDeepDoubleCvLTagInfos"));
  ...
  descriptions.add("pfDeepDoubleCvLJetTags", desc);
}

std::unique_ptr<DeepDoubleCvLTFCache> DeepDoubleCvLTFJetTagsProducer::initializeGlobalCache(
  const edm::ParameterSet& iConfig)
{
  ...
  DeepDoubleCvLTFCache* cache = new DeepDoubleCvLTFCache();
  cache->graphDef = tensorflow::loadGraphDef(pbFile);

  return std::unique_ptr<DeepDoubleCvLTFCache>(cache);
}

void DeepDoubleCvLTFJetTagsProducer::globalEndJob(const DeepDoubleCvLTFCache* cache)
{
...
void DeepDoubleCvLTFJetTagsProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
...
DEFINE_FWK_MODULE(DeepDoubleCvLTFJetTagsProducer);
