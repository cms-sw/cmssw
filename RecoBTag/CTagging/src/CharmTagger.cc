...
CharmTagger::CharmTagger(const edm::ParameterSet &configuration)
    : sl_computer_(
          configuration.getParameter<edm::ParameterSet>("slComputerCfg")),
      ...
      defaultValueNoTracks_(
          configuration.getParameter<bool>("defaultValueNoTracks")) {
...
void CharmTagger::initialize(const JetTagComputerRecord &record) {
  ...
  if (use_condDB_) {
    ...
    mvaID_->initializeGBRForest(gbrForestHandle.product(), variable_names,
                                spectators, use_adaBoost_);
  } else {
    mvaID_->initialize("Color:Silent:Error", mva_name_, weight_file_.fullPath(),
                       variable_names, spectators, use_GBRForest_,
                       use_adaBoost_);
  }
}
...
float CharmTagger::discriminator(const TagInfoHelper &tagInfo) const {
  ...
  const reco::CandSecondaryVertexTagInfo &sv_info =
      tagInfo.get<reco::CandSecondaryVertexTagInfo>(1);
  const reco::CandSoftLeptonTagInfo &softmu_info =
      tagInfo.get<reco::CandSoftLeptonTagInfo>(2);
  const reco::CandSoftLeptonTagInfo &softel_info =
      tagInfo.get<reco::CandSoftLeptonTagInfo>(3);
  reco::TaggingVariableList vars =
      sl_computer_(ip_info, sv_info, softmu_info, softel_info);
  ...
  for (auto &mva_var : variables_) {
    ...
    if (mva_var.has_index) {
      ...
      inputs[mva_var.name] = (vals.size() > mva_var.index)
                                 ? vals[mva_var.index]
                                 : mva_var.default_value;
    }
