...
pair<bool, Measurement1D>
SignedImpactParameter3D::apply(const TransientTrack &transientTrack,
  ...
  if (!TSOS.isValid()) {
    cout << "====>>>> SignedImpactParameter3D::apply : TSOS not valid = "
         << TSOS.isValid() << endl;
  ...
  TrajectoryStateOnSurface theTSOS =
      closestApproachToJet(*FTS, vertex, JetDirection, transientTrack.field());
  ...
  if (theIsValid) {
    ...
    double theDistanceAlongJetAxis =
        J.dot(theTSOS.globalPosition() - vertexPosition);
    theValue =
        D.mag() * (theDistanceAlongJetAxis / abs(theDistanceAlongJetAxis));
    ...
    GlobalVector Xi(T0.x() - vertex.position().x(),
                    T0.y() - vertex.position().y(),
                    T0.z() - vertex.position().z());
    ...
    double E1 =
        ROOT::Math::Similarity(deriv, theTSOS.cartesianError().matrix());
    double E2 = ROOT::Math::Similarity(deriv_v, vertex.covariance());
    //    double E2 =
    //    RecoVertex::convertError(vertex.covariance()).matrix().similarity(deriv_v);
    //    double E2 = 0.; // no vertex error because of stupid use of hundreds
    //    of different types for same thing
    theError = sqrt(E1 + E2);
...
TrajectoryStateOnSurface SignedImpactParameter3D::closestApproachToJet(
    const FreeTrajectoryState &aFTS, const Vertex &vertex,
...
GlobalVector
SignedImpactParameter3D::distance(const TrajectoryStateOnSurface &aTSOS,
                                  const Vertex &vertex,
  ...
  GlobalPoint X =
      GlobalPoint(vertex.x(), vertex.y(), vertex.z()); // aVertex.position();
...
pair<double, Measurement1D>
SignedImpactParameter3D::distanceWithJetAxis(const TransientTrack &track,
                                             const GlobalVector &direction,
                                             const Vertex &vertex) {
  ...
  if (!TSOS.isValid()) {
    cout << "====>>>> SignedImpactParameter3D::distanceWithJetAxis : TSOS not "
            "valid = "
         << TSOS.isValid() << endl;
    return pair<double, Measurement1D>(
  ...
  TrajectoryStateOnSurface aTSOS =
      closestApproachToJet(*FTS, vertex, jetDirection, track.field());
  ...
  if (isValid) {
    ...
    // If the track has been used to refit the Primary vertex then sign it
    // positively, otherwise negative
    ...
    //
    // get the covariance matrix of the vertex and compute the error on
    // theDistanceToJetAxis
    //
    ...
    //    cout << "distance to jet axis : "<< theDistanceToJetAxis <<" and error
    //    : "<< theLDist_err<<endl;
