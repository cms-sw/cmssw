...
SignedTransverseImpactParameter::apply(const TransientTrack &track,
  ...
  const FreeTrajectoryState *FTS =
      stateOnSurface
          .freeState(); // aRecTrack.stateAtFirstPoint().freeTrajectoryState());
  ...
  if (!TSOS.isValid()) {
    ...
    theError = sqrt(E1 + E2);
    LogDebug("BTagTools") << "Tk error : " << E1 << " , Vtx error : " << E2
                          << "  tot : " << theError;
...
pair<bool, Measurement1D>
SignedTransverseImpactParameter::zImpactParameter(const TransientTrack &track,
                                                  const GlobalVector &direction,
                                                  const Vertex &vertex) const {
  ...
  if (!TSOS.isValid()) {
    LogDebug("BTagTools")
        << "====>>>> SignedTransverseImpactParameter::zImpactParameter : TSOS "
           "not valid";
  ...
  // CW  cout << "CW number or rows and columns : " <<
  // statePCA.cartesianError().matrix().num_row() << " , " CW <<
  // statePCA.cartesianError().matrix().num_col() << endl ;
