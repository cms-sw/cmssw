...
TrackSelector::TrackSelector(const edm::ParameterSet &params)
    : minPixelHits(params.getParameter<unsigned int>("pixelHitsMin")),
      ...
      useVariableJTA_(params.existsAs<bool>("useVariableJTA")
                          ? params.getParameter<bool>("useVariableJTA")
                          : false) {
  std::string qualityClass = params.getParameter<std::string>("qualityClass");
  if (qualityClass == "any" || qualityClass == "Any" || qualityClass == "ANY" ||
      qualityClass.empty()) {
    selectQuality = false;
...
bool TrackSelector::operator()(const Track &track,
                               const btag::TrackIPData &ipData, const Jet &jet,
                               const GlobalPoint &pv) const {

  bool jtaPassed = false;
  if (useVariableJTA_) {
    jtaPassed = TrackIPTagInfo::passVariableJTA(
        varJTApars, jet.pt(), track.pt(),
        reco::deltaR(jet.momentum(), track.momentum()));
  } else
    jtaPassed = true;

  return track.pt() >= minPt &&
         reco::deltaR2(jet.momentum(), track.momentum()) <
             maxJetDeltaR * maxJetDeltaR &&
