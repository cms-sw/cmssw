...
CandidateBoostedDoubleSecondaryVertexComputer::
    ...
    : useCondDB_(parameters.getParameter<bool>("useCondDB")),
      ...
      weightFile_(parameters.existsAs<edm::FileInPath>("weightFile")
                      ...
                      : edm::FileInPath()),
      useGBRForest_(parameters.existsAs<bool>("useGBRForest")
                        ? parameters.getParameter<bool>("useGBRForest")
                        : false),
      useAdaBoost_(parameters.existsAs<bool>("useAdaBoost")
                       ? parameters.getParameter<bool>("useAdaBoost")
                       : false) {
  uses(0, "svTagInfos");
...
void CandidateBoostedDoubleSecondaryVertexComputer::initialize(
    const JetTagComputerRecord &record) {
  ...
  if (useCondDB_) {
    ...
    mvaID->initializeGBRForest(gbrForestHandle.product(), variables, spectators,
                               useAdaBoost_);
  } else
    mvaID->initialize("Color:Silent:Error", "BDT", weightFile_.fullPath(),
                      variables, spectators, useGBRForest_, useAdaBoost_);
}

float CandidateBoostedDoubleSecondaryVertexComputer::discriminator(
    const TagInfoHelper &tagInfo) const {
  // get the TagInfo
  const reco::BoostedDoubleSVTagInfo &bdsvTagInfo =
      tagInfo.get<reco::BoostedDoubleSVTagInfo>(0);
  ...
  inputs["trackSip2dSigAboveCharm_0"] =
      vars.get(reco::btau::trackSip2dSigAboveCharm);
  inputs["trackSip2dSigAboveBottom_0"] =
      vars.get(reco::btau::trackSip2dSigAboveBottom_0);
  inputs["trackSip2dSigAboveBottom_1"] =
