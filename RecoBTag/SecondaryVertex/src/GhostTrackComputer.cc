...
GhostTrackComputer::GhostTrackComputer(const edm::ParameterSet &params)
    : charmCut(params.getParameter<double>("charmCut")),
      ...
      trackNoDeltaRSelector(
          dropDeltaR(params.getParameter<edm::ParameterSet>("trackSelection"))),
      minTrackWeight(params.getParameter<double>("minimumTrackWeight")),
      trackPairV0Filter(
          params.getParameter<edm::ParameterSet>("trackPairV0Filter")) {}

const btag::TrackIPData &GhostTrackComputer::threshTrack(
    const TrackIPTagInfo &trackIPTagInfo, const btag::SortCriteria sort,
...
const btag::TrackIPData &GhostTrackComputer::threshTrack(
    const CandIPTagInfo &trackIPTagInfo, const btag::SortCriteria sort,
    const reco::Jet &jet, const GlobalPoint &pv) const {
...
TaggingVariableList GhostTrackComputer::
operator()(const TrackIPTagInfo &ipInfo,
  ...
  GlobalPoint pv;
  if (havePv)
    pv = GlobalPoint(ipInfo.primaryVertex()->x(), ipInfo.primaryVertex()->y(),
  ...
  for (unsigned int i = 0; i < svInfo.nVertices(); i++) {
    ...
    for (TrackRefVector::const_iterator track = tracks.begin();
      ...
      if (hasRefittedTracks) {
        ...
        vars.insert(btau::trackEtaRel,
                    reco::btau::etaRel(jetDir, actualTrack.momentum()), true);
      } else {
        kin.add(**track, w);
        vars.insert(btau::trackEtaRel,
                    reco::btau::etaRel(jetDir, (*track)->momentum()), true);
      }
      if (!isTrackVertex)
        nVertexTracks++;
    }
  ...
  if (nVertices) {
    ...
    dist2D = Measurement1D(vertexDist2D.first / vertexDist2D.second,
                           std::sqrt(1. / vertexDist2D.second));
    dist3D = Measurement1D(vertexDist3D.first / vertexDist3D.second,
    ...
    dist2D = Measurement1D(tracksDist2D.first / tracksDist2D.second,
                           std::sqrt(1. / tracksDist2D.second));
    dist3D = Measurement1D(tracksDist3D.first / tracksDist3D.second,
  ...
  for (unsigned int i = 0; i < indices.size(); i++) {
    ...
    vars.insert(btau::trackGhostTrackDistVal, data.distanceToGhostTrack.value(),
                true);
    vars.insert(btau::trackGhostTrackDistSig,
                data.distanceToGhostTrack.significance(), true);
    vars.insert(btau::trackGhostTrackWeight, data.ghostTrackWeight, true);
    vars.insert(btau::trackDecayLenVal,
                havePv ? (data.closestToGhostTrack - pv).mag() : -1.0, true);
    ...
    vars.insert(btau::trackNPixelHits,
                track.hitPattern().pixelLayersWithMeasurement(), true);
    vars.insert(btau::trackNTotalHits,
                track.hitPattern().trackerLayersWithMeasurement(), true);
  ...
  vars.insert(
      ...
      threshTrack(ipInfo, reco::btag::IP3DSig, *jet, pv).ip3d.significance(),
      true);
  vars.insert(
      btau::trackSip2dSigAboveCharm,
      threshTrack(ipInfo, reco::btag::IP2DSig, *jet, pv).ip2d.significance(),
      true);
...
TaggingVariableList GhostTrackComputer::
operator()(const CandIPTagInfo &ipInfo,
