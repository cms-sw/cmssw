                                 ...
                                 const SecondaryVertexTagInfo &svInfo) const
{
	...
	for(unsigned int i = 0; i < indices.size(); i++) {
		...
		vars.insert(btau::trackNPixelHits, track.getHitPattern().pixelLayersWithMeasurement(HitPattern::TRACK_HITS), true);
		vars.insert(btau::trackNTotalHits, track.getHitPattern().trackerLayersWithMeasurement(HitPattern::TRACK_HITS), true);
