...
CombinedSVComputer::dropDeltaR(const edm::ParameterSet &pset) const {
...
CombinedSVComputer::CombinedSVComputer(const edm::ParameterSet &params)
    : trackFlip(params.getParameter<bool>("trackFlip")),
      ...
      trackPseudoSelector(
          params.getParameter<edm::ParameterSet>("trackPseudoSelection")),
      pseudoMultiplicityMin(
          params.getParameter<unsigned int>("pseudoMultiplicityMin")),
      trackMultiplicityMin(
          params.getParameter<unsigned int>("trackMultiplicityMin")),
      ...
      pseudoVertexV0Filter(
          params.getParameter<edm::ParameterSet>("pseudoVertexV0Filter")),
      trackPairV0Filter(
...
inline CombinedSVComputer::IterationRange
CombinedSVComputer::flipIterate(int size, bool vertex) const {
...
const btag::TrackIPData &CombinedSVComputer::threshTrack(
...
TaggingVariableList CombinedSVComputer::
operator()(const TrackIPTagInfo &ipInfo,
  ...
  range_for(i, range) {
    ...
    numberofvertextracks =
        numberofvertextracks + (svInfo.secondaryVertex(i)).nTracks();
    ...
    for (reco::Vertex::trackRef_iterator track = vertex.tracks_begin();
      ...
      if (hasRefittedTracks) {
        ...
        if (vtx < 0) // calculate this only for the first vertex
        {
          ...
          vtx_track_ESum +=
              std::sqrt(actualTrack.momentum().Mag2() +
  ...
  if (vtx >= 0) {
    ...
    vars.insert(btau::vertexFitProb,
                (svInfo.secondaryVertex(vtx)).normalizedChi2(), true);
  }

  // after we collected vertex information we let the common code complete the
  // job
  fillCommonVariables(vars, vertexKinematics, ipInfo, svInfo, vtx_track_ptSum,
                      vtx_track_ESum);
...
operator()(const CandIPTagInfo &ipInfo,
  ...
  range_for(i, range) {
    ...
    numberofvertextracks =
        numberofvertextracks +
        (svInfo.secondaryVertex(i)).numberOfSourceCandidatePtrs();
    ...
    for (std::vector<CandidatePtr>::const_iterator track = tracks.begin();
      ...
      {
        vtx_track_ptSum += std::sqrt((*track)->momentum().Perp2());
        vtx_track_ESum += std::sqrt((*track)->momentum().Mag2() +
                                    ROOT::Math::Square(ParticleMasses::piPlus));
      }
    }
  ...
  if (vtx >= 0) {
    vars.insert(btau::vertexNTracks, numberofvertextracks, true);
    vars.insert(btau::vertexFitProb,
                (svInfo.secondaryVertex(vtx)).vertexNormalizedChi2(), true);
  }
