...
//
/**\class BoostedDoubleSVProducer BoostedDoubleSVProducer.cc
 * ​RecoBTag/​SecondaryVertex/plugins/BoostedDoubleSVProducer.cc
 *
 * Description: EDProducer that produces collection of BoostedDoubleSVTagInfos
 *
 * Implementation:
 *    A collection of SecondaryVertexTagInfos is taken as input and a collection
 * of BoostedDoubleSVTagInfos is produced as output.
 */
//
...
private:
  ...
  void calcNsubjettiness(const reco::JetBaseRef &jet, float &tau1, float &tau2,
                         std::vector<fastjet::PseudoJet> &currentAxes) const;
  void setTracksPVBase(const reco::TrackRef &trackRef,
                       const reco::VertexRef &vertexRef, float &PVweight) const;
  void setTracksPV(const reco::CandidatePtr &trackRef,
                   const reco::VertexRef &vertexRef, float &PVweight) const;
  ...
  // ----------member data ---------------------------
  const edm::EDGetTokenT<std::vector<reco::CandSecondaryVertexTagInfo>>
      svTagInfos_;
...
BoostedDoubleSVProducer::BoostedDoubleSVProducer(
    ...
    : svTagInfos_(consumes<std::vector<reco::CandSecondaryVertexTagInfo>>(
          iConfig.getParameter<edm::InputTag>("svTagInfos"))),
      beta_(iConfig.getParameter<double>("beta")),
      R0_(iConfig.getParameter<double>("R0")),
      maxSVDeltaRToJet_(iConfig.getParameter<double>("maxSVDeltaRToJet")),
      maxDistToAxis_(iConfig.getParameter<edm::ParameterSet>("trackSelection")
                         .getParameter<double>("maxDistToAxis")),
      maxDecayLen_(iConfig.getParameter<edm::ParameterSet>("trackSelection")
                       .getParameter<double>("maxDecayLen")),
      trackPairV0Filter(
          iConfig.getParameter<edm::ParameterSet>("trackPairV0Filter")),
...
void BoostedDoubleSVProducer::produce(edm::Event &iEvent,
  ...
  // loop over TagInfos
  for (std::vector<reco::CandSecondaryVertexTagInfo>::const_iterator iterTI =
           svTagInfos->begin();
       iterTI != svTagInfos->end(); ++iterTI) {
    // get TagInfos
    ...
    float trackSip3dSig_3 = dummyTrackSip3dSig,
          trackSip3dSig_2 = dummyTrackSip3dSig,
          trackSip3dSig_1 = dummyTrackSip3dSig,
          trackSip3dSig_0 = dummyTrackSip3dSig;
    float tau2_trackSip3dSig_0 = dummyTrackSip3dSig,
          tau1_trackSip3dSig_0 = dummyTrackSip3dSig,
          tau2_trackSip3dSig_1 = dummyTrackSip3dSig,
          tau1_trackSip3dSig_1 = dummyTrackSip3dSig;
    float trackSip2dSigAboveCharm_0 = dummyTrackSip2dSigAbove,
          trackSip2dSigAboveBottom_0 = dummyTrackSip2dSigAbove,
          trackSip2dSigAboveBottom_1 = dummyTrackSip2dSigAbove;
    float tau1_trackEtaRel_0 = dummyTrackEtaRel,
          tau1_trackEtaRel_1 = dummyTrackEtaRel,
          tau1_trackEtaRel_2 = dummyTrackEtaRel;
    float tau2_trackEtaRel_0 = dummyTrackEtaRel,
          tau2_trackEtaRel_1 = dummyTrackEtaRel,
          tau2_trackEtaRel_2 = dummyTrackEtaRel;
    float tau1_vertexMass = dummyVertexMass,
          tau1_vertexEnergyRatio = dummyVertexEnergyRatio,
          tau1_vertexDeltaR = dummyVertexDeltaR,
          tau1_flightDistance2dSig = dummyFlightDistance2dSig;
    float tau2_vertexMass = dummyVertexMass,
          tau2_vertexEnergyRatio = dummyVertexEnergyRatio,
          tau2_vertexDeltaR = dummyVertexDeltaR,
          tau2_flightDistance2dSig = dummyFlightDistance2dSig;
    float jetNTracks = 0, nSV = 0, tau1_nSecondaryVertices = 0,
    ...
    const std::vector<reco::CandidatePtr> &selectedTracks =
        ipTagInfo.selectedTracks();
    const std::vector<reco::btag::TrackIPData> &ipData =
        ipTagInfo.impactParameterData();
    ...
    for (size_t itt = 0; itt < trackSize; ++itt) {
      ...
      if (currentAxes.size() > 1 &&
        ...
        index = 1;
      direction = GlobalVector(currentAxes[index].px(), currentAxes[index].py(),
                               currentAxes[index].pz());
      ...
      TrajectoryStateOnSurface closest = IPTools::closestApproachToJet(
          transientTrack.impactPointState(), *vertexRef, direction,
          transientTrack.field());
      if (closest.isValid())
        decayLengthTau = (closest.globalPosition() -
                          RecoVertex::convertPos(vertexRef->position()))
                             .mag();

      distTauAxis = std::abs(
          IPTools::jetTrackDistance(transientTrack, direction, *vertexRef)
              .second.value());

      float IP3Dsig = ipTagInfo.impactParameterData()[itt].ip3d.significance();

      if (!isfromV0 && decayLengthTau < maxDecayLen_ &&
          distTauAxis < maxDistToAxis_) {
        IP3Ds.push_back(IP3Dsig < -50. ? -50. : IP3Dsig);
        ++contTrk;
        if (currentAxes.size() > 1) {
          if (reco::deltaR2(trackRef->momentum(), currentAxes[0]) <
              reco::deltaR2(trackRef->momentum(), currentAxes[1]))
      ...
      if (kin.vectorSum().M() > bottomThreshold) // bottom cut
      {
        ...
        if ((i + 1) < indices.size())
          trackSip2dSigAboveBottom_1 =
              (ipData[indices[i + 1]]).ip2d.significance();
    ...
    for (size_t vtx = 0; vtx < svTagInfo.nVertices(); ++vtx) {
      const reco::VertexCompositePtrCandidate &vertex =
          svTagInfo.secondaryVertex(vtx);
        ...
        if (reco::deltaR2(svTagInfo.flightDirection(vtx), currentAxes[1]) <
          ...
          if (tau2_flightDistance2dSig < 0) {
            tau2_flightDistance2dSig =
                svTagInfo.flightDistance(vtx, true).significance();
            tau2_vertexDeltaR =
      ...
      if (reco::deltaR2(flightDir, jetDir) <
          (maxSVDeltaRToJet_ * maxSVDeltaRToJet_))
    ...
    if (tau1_nSecondaryVertices > 0.) {
      const math::XYZTLorentzVector &tau1_vertexSum =
          tau1Kinematics.weightedVectorSum();
      if (allSum.E() > 0.)
    ...
    if (tau2_nSecondaryVertices > 0.) {
      const math::XYZTLorentzVector &tau2_vertexSum =
          tau2Kinematics.weightedVectorSum();
      if (allSum.E() > 0.)
    ...
    for (std::map<double, size_t>::iterator iVtx = VTXmap.begin();
         iVtx != VTXmap.end(); ++iVtx) {
      ++cont;
      const reco::VertexCompositePtrCandidate &vertex =
          svTagInfo.secondaryVertex(iVtx->second);
      if (cont == 1) {
        ...
        if (vtxMass > 0.)
          z_ratio = reco::deltaR(currentAxes[1], currentAxes[0]) *
                    SV_p4_0.pt() / vtxMass;
      }
      if (cont == 2) {
    ...
    // when only one tau axis has SVs assigned, they are all assigned to the 1st
    // tau axis in the special case below need to swap values
    ...
    vars.insert(reco::btau::trackSip2dSigAboveCharm, trackSip2dSigAboveCharm_0,
                true);
    vars.insert(reco::btau::trackSip2dSigAboveBottom_0,
                trackSip2dSigAboveBottom_0, true);
    vars.insert(reco::btau::trackSip2dSigAboveBottom_1,
                trackSip2dSigAboveBottom_1, true);
    ...
    vars.insert(reco::btau::tau1_vertexEnergyRatio, tau1_vertexEnergyRatio,
                true);
    vars.insert(reco::btau::tau1_flightDistance2dSig, tau1_flightDistance2dSig,
                true);
    vars.insert(reco::btau::tau1_vertexDeltaR, tau1_vertexDeltaR, true);
    vars.insert(reco::btau::tau2_vertexMass, tau2_vertexMass, true);
    vars.insert(reco::btau::tau2_vertexEnergyRatio, tau2_vertexEnergyRatio,
                true);
    vars.insert(reco::btau::tau2_flightDistance2dSig, tau2_flightDistance2dSig,
                true);
    ...
    tagInfos->push_back(reco::BoostedDoubleSVTagInfo(
        vars, edm::Ref<std::vector<reco::CandSecondaryVertexTagInfo>>(
                  svTagInfos, iterTI - svTagInfos->begin())));
  }
...
void BoostedDoubleSVProducer::calcNsubjettiness(
    const reco::JetBaseRef &jet, float &tau1, float &tau2,
    std::vector<fastjet::PseudoJet> &currentAxes) const {
  std::vector<fastjet::PseudoJet> fjParticles;

  // loop over jet constituents and push them in the vector of FastJet
  // constituents
  for (const reco::CandidatePtr &daughter : jet->daughterPtrVector()) {
    if (daughter.isNonnull() && daughter.isAvailable()) {
      ...
      if (subjet && daughter->numberOfDaughters() > 1) {
        // loop over subjet constituents and push them in the vector of FastJet
        // constituents
        for (size_t i = 0; i < daughter->numberOfDaughters(); ++i) {
          ...
          if (constit)
            fjParticles.push_back(
                fastjet::PseudoJet(constit->px(), constit->py(), constit->pz(),
                                   constit->energy()));
          else
            edm::LogWarning("MissingJetConstituent")
                << "Jet constituent required for N-subjettiness computation is "
                   "missing!";
        }
      } else
        fjParticles.push_back(fastjet::PseudoJet(daughter->px(), daughter->py(),
                                                 daughter->pz(),
                                                 daughter->energy()));
    } else
      edm::LogWarning("MissingJetConstituent")
...
void BoostedDoubleSVProducer::setTracksPVBase(const reco::TrackRef &trackRef,
  ...
  for (IT it = vtx.tracks_begin(); it != vtx.tracks_end(); ++it) {
    ...
    // one of the tracks in the vertex is the same as the track considered in
    // the function
...
void BoostedDoubleSVProducer::setTracksPV(const reco::CandidatePtr &trackRef,
  ...
  const pat::PackedCandidate *pcand =
      dynamic_cast<const pat::PackedCandidate *>(trackRef.get());
  ...
  if (pcand) // MiniAOD case
  {
    ...
    const reco::PFCandidate *pfcand =
        dynamic_cast<const reco::PFCandidate *>(trackRef.get());

    setTracksPVBase(pfcand->trackRef(), vertexRef, PVweight);
  }
}

void BoostedDoubleSVProducer::etaRelToTauAxis(
    const reco::VertexCompositePtrCandidate &vertex,
    const fastjet::PseudoJet &tauAxis,
    std::vector<float> &tau_trackEtaRel) const {
  ...
  for (std::vector<reco::CandidatePtr>::const_iterator track = tracks.begin();
    ...
    tau_trackEtaRel.push_back(
        std::abs(reco::btau::etaRel(direction.Unit(), (*track)->momentum())));
}
...
// module  ------------
void BoostedDoubleSVProducer::fillDescriptions(
