...
vtxP4(const reco::VertexCompositePtrCandidate &vtx) {
  reco::Candidate::LorentzVector sum;
  const std::vector<reco::CandidatePtr> &tracks = vtx.daughterPtrVector();

  for (std::vector<reco::CandidatePtr>::const_iterator track = tracks.begin();
...
private:
  ...
  double maxDRForUnique, maxvecSumIMCUTForUnique, minCosPAtomerge,
      maxPtreltomerge;
  ...
  void resolveBtoDchain(std::vector<VertexProxy> &coll, unsigned int i,
                        unsigned int k);
  GlobalVector flightDirection(const reco::Vertex &v1, const reco::Vertex &v2);
  GlobalVector flightDirection(const reco::Vertex &v1,
...
BtoCharmDecayVertexMergerT<VTX>::BtoCharmDecayVertexMergerT(
    ...
    : token_primaryVertex(consumes<reco::VertexCollection>(
          params.getParameter<edm::InputTag>("primaryVertices"))),
      token_secondaryVertex(consumes<edm::View<VTX>>(
          params.getParameter<edm::InputTag>("secondaryVertices"))),
      maxDRForUnique(params.getParameter<double>("maxDRUnique")),
      maxvecSumIMCUTForUnique(
          params.getParameter<double>("maxvecSumIMifsmallDRUnique")),
...
//------------------------------------
template <typename VTX>
void BtoCharmDecayVertexMergerT<VTX>::produce(edm::Event &iEvent,
  ...
  if (!PVcoll->empty()) {
    ...
    for (typename edm::View<VTX>::const_iterator sv =
             secondaryVertices->begin();
    ...
    for (unsigned int iVtx = 0; iVtx < vertexProxyColl.size(); ++iVtx) {
      ...
      for (unsigned int kVtx = vertexProxyColl.size() - 1; kVtx > iVtx;
           --kVtx) {
        // remove D vertices from the collection and add the tracks to the
        // original one
    ...
    for (typename std::vector<VertexProxy>::const_iterator it =
             vertexProxyColl.begin();
         it != vertexProxyColl.end(); ++it)
...
template <typename VTX>
GlobalVector
BtoCharmDecayVertexMergerT<VTX>::flightDirection(const reco::Vertex &v1,
                                                 const reco::Vertex &v2) {
  return GlobalVector(v2.x() - v1.x(), v2.y() - v1.y(), v2.z() - v1.z());
}

template <typename VTX>
GlobalVector BtoCharmDecayVertexMergerT<VTX>::flightDirection(
    const reco::Vertex &v1, const reco::VertexCompositePtrCandidate &v2) {
  return GlobalVector(v2.vertex().x() - v1.x(), v2.vertex().y() - v1.y(),
                      v2.vertex().z() - v1.z());
}

template <typename VTX>
GlobalVector BtoCharmDecayVertexMergerT<VTX>::flightDirection(
    const reco::VertexCompositePtrCandidate &v1,
    const reco::VertexCompositePtrCandidate &v2) {
...
BtoCharmDecayVertexMergerT<reco::Vertex>::buildVertexProxy(
...
BtoCharmDecayVertexMergerT<reco::VertexCompositePtrCandidate>::buildVertexProxy(
  ...
  VertexProxy vtxProxy = {vtx, vtxP4(vtx).M(),
                          vtx.numberOfSourceCandidatePtrs()};
  return vtxProxy;
}

// -------------- resolveBtoDchain ----------------
template <>
void BtoCharmDecayVertexMergerT<reco::Vertex>::resolveBtoDchain(
    std::vector<VertexProxy> &coll, unsigned int i, unsigned int k) {
  using namespace reco;

  GlobalVector momentum1 = GlobalVector(
      coll[i].vert.p4().X(), coll[i].vert.p4().Y(), coll[i].vert.p4().Z());
  GlobalVector momentum2 = GlobalVector(
      coll[k].vert.p4().X(), coll[k].vert.p4().Y(), coll[k].vert.p4().Z());
  ...
  double cosPA =
      nearToFar.dot(momentumFar) / momentumFar.mag() / nearToFar.mag();
  ...
  // create a set of all tracks from both vertices, avoid double counting by
  // using a std::set<>
  std::set<reco::TrackRef> trackrefs;
  // first vertex
  for (reco::Vertex::trackRef_iterator ti = sv1.tracks_begin();
       ti != sv1.tracks_end(); ++ti) {
  ...
  // second vertex
  for (reco::Vertex::trackRef_iterator ti = sv2.tracks_begin();
       ti != sv2.tracks_end(); ++ti) {
  ...
  for (std::set<reco::TrackRef>::const_iterator it = trackrefs.begin();
       it != trackrefs.end(); ++it) {
  ...
  if (vertexDeltaR < maxDRForUnique && mother.M() < maxvecSumIMCUTForUnique &&
      cosPA > minCosPAtomerge && std::abs(ptrel) < maxPtreltomerge) {
    ...
    for (reco::Vertex::trackRef_iterator ti = sv2.tracks_begin();
      ...
      reco::Vertex::trackRef_iterator it =
          find(sv1.tracks_begin(), sv1.tracks_end(), *ti);
      if (it == sv1.tracks_end())
        coll[i].vert.add(*ti, sv2.refittedTrack(*ti), sv2.trackWeight(*ti));
      else
        bFoundDuplicate = true;
    }
    // in case a duplicate track is found, need to create the full track list in
    // the vertex from scratch because we need to modify the weight. the weight
    // must be the larger one, otherwise we may have outliers which are not real
    // outliers
...
template <>
void BtoCharmDecayVertexMergerT<reco::VertexCompositePtrCandidate>::
    resolveBtoDchain(std::vector<VertexProxy> &coll, unsigned int i,
                     unsigned int k) {
  using namespace reco;
  ...
  for (std::set<reco::CandidatePtr>::const_iterator it = trackrefs.begin();
       it != trackrefs.end(); ++it) {
    Candidate::LorentzVector temp((*it)->px(), (*it)->py(), (*it)->pz(),
  ...
  if (vertexDeltaR < maxDRForUnique && mother.M() < maxvecSumIMCUTForUnique &&
    ...
    for (std::vector<reco::CandidatePtr>::const_iterator ti = tracks2.begin();
         ti != tracks2.end(); ++ti) {
      std::vector<reco::CandidatePtr>::const_iterator it =
...
typedef BtoCharmDecayVertexMergerT<reco::VertexCompositePtrCandidate>
