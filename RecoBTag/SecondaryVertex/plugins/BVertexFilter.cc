...
//
/**\class BVertexFilterT BVertexFilter.cc
...
template <typename VTX> class BVertexFilterT : public edm::stream::EDFilter<> {
public:
...
BVertexFilterT<VTX>::BVertexFilterT(const edm::ParameterSet &params)
    : svFilter(params.getParameter<edm::ParameterSet>("vertexFilter")),
      useVertexKinematicAsJetAxis(
          params.getParameter<bool>("useVertexKinematicAsJetAxis")),
...
template <typename VTX> BVertexFilterT<VTX>::~BVertexFilterT() {}
...
bool BVertexFilterT<VTX>::filter(edm::Event &iEvent,
  ...
  if (!pvHandle->empty()) {
    ...
    if (!primary.isFake()) {
      for (typename edm::View<VTX>::const_iterator it = vertices.begin();
           it != vertices.end(); ++it) {
        ...
        if (svFilter(
                primary,
                reco::TemplatedSecondaryVertex<VTX>(primary, *it, axis, true),
                axis)) {
          count++;
...
typedef BVertexFilterT<reco::VertexCompositePtrCandidate>
