...
template <typename T> struct RefToBaseLess {
...
GlobalVector flightDirection(const reco::Vertex &pv,
  ...
  return GlobalVector(sv.vertex().x() - pv.x(), sv.vertex().y() - pv.y(),
                      sv.vertex().z() - pv.z());
}
...
private:
  ...
  void markUsedTracks(TrackDataVector &trackData,
                      ...
                      const SecondaryVertex &sv, size_t idx);

  struct SVBuilder {

    SVBuilder(const reco::Vertex &pv, const GlobalVector &direction,
              bool withPVError, double minTrackWeight)
        : pv(pv), direction(direction), withPVError(withPVError),
...
TemplatedSecondaryVertexProducer<IPTI, VTX>::getConstraintType(
...
TemplatedSecondaryVertexProducer<IPTI, VTX>::TemplatedSecondaryVertexProducer(
    ...
    : sortCriterium(TrackSorting::getCriterium(
          params.getParameter<std::string>("trackSort"))),
      trackSelector(params.getParameter<edm::ParameterSet>("trackSelection")),
      constraint(
          getConstraintType(params.getParameter<std::string>("constraint"))),
  ...
  token_trackIPTagInfo = consumes<std::vector<IPTI>>(
      params.getParameter<edm::InputTag>("trackIPTagInfos"));
  ...
  if (constraint == CONSTRAINT_PV_BEAMSPOT_SIZE ||
    ...
    token_BeamSpot = consumes<reco::BeamSpot>(
        params.getParameter<edm::InputTag>("beamSpotTag"));
  useExternalSV = false;
  if (params.existsAs<bool>("useExternalSV"))
    useExternalSV = params.getParameter<bool>("useExternalSV");
  if (useExternalSV) {
    token_extSVCollection = consumes<edm::View<VTX>>(
        params.getParameter<edm::InputTag>("extSVCollection"));
    extSVDeltaRToJet = params.getParameter<double>("extSVDeltaRToJet");
  }
  useSVClustering = (params.existsAs<bool>("useSVClustering")
                         ? params.getParameter<bool>("useSVClustering")
                         : false);
  useSVMomentum = (params.existsAs<bool>("useSVMomentum")
                       ? params.getParameter<bool>("useSVMomentum")
                       : false);
  ...
  if (useSVClustering) {
    ...
    jetPtMin = 0.; // hardcoded to 0. since we simply want to recluster all
                   // input jets which already had some PtMin applied
    ghostRescaling = (params.existsAs<double>("ghostRescaling")
                          ? params.getParameter<double>("ghostRescaling")
                          : 1e-18);
    relPtTolerance =
        (params.existsAs<double>("relPtTolerance")
             ? params.getParameter<double>("relPtTolerance")
             : 1e-03); // 0.1% relative difference in Pt should be sufficient to
                       // detect possible misconfigurations

    // set jet algorithm
    if (jetAlgorithm == "Kt")
      fjJetDefinition =
          JetDefPtr(new fastjet::JetDefinition(fastjet::kt_algorithm, rParam));
    else if (jetAlgorithm == "CambridgeAachen")
      fjJetDefinition = JetDefPtr(
          new fastjet::JetDefinition(fastjet::cambridge_algorithm, rParam));
    else if (jetAlgorithm == "AntiKt")
      fjJetDefinition = JetDefPtr(
          new fastjet::JetDefinition(fastjet::antikt_algorithm, rParam));
    else
      throw cms::Exception("InvalidJetAlgorithm")
          << "Jet clustering algorithm is invalid: " << jetAlgorithm
          << ", use CambridgeAachen | Kt | AntiKt" << std::endl;
  }
  if (useFatJets)
    token_fatJets = consumes<edm::View<reco::Jet>>(
        params.getParameter<edm::InputTag>("fatJets"));
  if (useGroomedFatJets)
    token_groomedFatJets = consumes<edm::View<reco::Jet>>(
        params.getParameter<edm::InputTag>("groomedFatJets"));
  if (useFatJets && !useSVClustering)
    rParam =
        params.getParameter<double>("rParam"); // will be used later as a dR cut

  produces<Product>();
}
template <class IPTI, class VTX>
TemplatedSecondaryVertexProducer<IPTI,
                                 VTX>::~TemplatedSecondaryVertexProducer() {}

template <class IPTI, class VTX>
void TemplatedSecondaryVertexProducer<IPTI, VTX>::produce(
    edm::Event &event, const edm::EventSetup &es) {
    ...
    if (useGroomedFatJets) {
      ...
      if (groomedFatJetsHandle->size() > fatJetsHandle->size())
        edm::LogError("TooManyGroomedJets")
            ...
            << "). Please check that the two jet collections belong to each "
               "other.";
    }
  }
  ...
  // ------------------------------------ SV clustering START
  // --------------------------------------------
  std::vector<std::vector<int>> clusteredSVs(trackIPTagInfos->size(),
                                             std::vector<int>());
  if (useExternalSV && useSVClustering && !trackIPTagInfos->empty()) {
    ...
    if (useFatJets) {
      for (edm::View<reco::Jet>::const_iterator it = fatJetsHandle->begin();
           it != fatJetsHandle->end(); ++it) {
        std::vector<edm::Ptr<reco::Candidate>> constituents =
            it->getJetConstituents();
        ...
        for (m = constituents.begin(); m != constituents.end(); ++m) {
          ...
          fjInputs.push_back(fastjet::PseudoJet(
              constit->px(), constit->py(), constit->pz(), constit->energy()));
        }
      }
    } else {
      for (typename std::vector<IPTI>::const_iterator it =
               trackIPTagInfos->begin();
           it != trackIPTagInfos->end(); ++it) {
        std::vector<edm::Ptr<reco::Candidate>> constituents =
            it->jet()->getJetConstituents();
        std::vector<edm::Ptr<reco::Candidate>>::const_iterator m;
        for (m = constituents.begin(); m != constituents.end(); ++m) {
          reco::CandidatePtr constit = *m;
          if (constit->pt() == 0) {
    ...
    for (typename edm::View<VTX>::const_iterator it = extSecVertex->begin();
         it != extSecVertex->end(); ++it) {
      const reco::Vertex &pv = *(trackIPTagInfos->front().primaryVertex());
      GlobalVector dir = flightDirection(pv, *it);
      dir = dir.unit();
      fastjet::PseudoJet p(
          dir.x(), dir.y(), dir.z(),
          dir.mag()); // using SV flight direction so treating SV as massless
      if (useSVMomentum)
        p = fastjet::PseudoJet(it->p4().px(), it->p4().py(), it->p4().pz(),
                               it->p4().energy());
    ...
    fjClusterSeq = ClusterSequencePtr(
    ...
    std::vector<fastjet::PseudoJet> inclusiveJets =
        fastjet::sorted_by_pt(fjClusterSeq->inclusive_jets(jetPtMin));
    ...
    if (useFatJets) {
      if (inclusiveJets.size() < fatJetsHandle->size())
        edm::LogError("TooFewReclusteredJets")
            ...
            << ") than original fat jets (" << fatJetsHandle->size()
            << "). Please check that the jet algorithm and jet size match "
               "those used for the original jet collection.";

      // match reclustered and original fat jets
      std::vector<int> reclusteredIndices;
      matchReclusteredJets<edm::View<reco::Jet>>(fatJetsHandle, inclusiveJets,
                                                 reclusteredIndices, "fat");
      ...
      if (useGroomedFatJets)
        matchSubjets(groomedIndices, groomedFatJetsHandle, trackIPTagInfos,
                     subjetIndices);
      else
      ...
      for (size_t i = 0; i < fatJetsHandle->size(); ++i) {
        ...
        if (fatJetsHandle->at(i).pt() ==
            0) // continue if the original jet has Pt=0
        {
          edm::LogWarning("NullTransverseMomentum")
              << "The original fat jet " << i
              << " has Pt=0. This is not expected so the jet will be skipped.";
          continue;
        }

        if (subjetIndices.at(i).empty())
          continue; // continue if the original jet does not have subjets
                    // assigned

        // since the "ghosts" are extremely soft, the configuration and ordering
        // of the reclustered and original fat jets should in principle stay the
        // same
        if ((std::abs(inclusiveJets.at(reclusteredIndices.at(i)).pt() -
                      fatJetsHandle->at(i).pt()) /
             fatJetsHandle->at(i).pt()) > relPtTolerance) {
          if (fatJetsHandle->at(i).pt() <
              10.) // special handling for low-Pt jets (Pt<10 GeV)
            edm::LogWarning("JetPtMismatchAtLowPt")
                << "The reclustered and original fat jet " << i
                << " have different Pt's ("
                << inclusiveJets.at(reclusteredIndices.at(i)).pt() << " vs "
                << fatJetsHandle->at(i).pt() << " GeV, respectively).\n"
                << "Please check that the jet algorithm and jet size match "
                   "those used for the original fat jet collection and also "
                   "make sure the original fat jets are uncorrected. In "
                   "addition, make sure you are not using CaloJets which are "
                   "presently not supported.\n"
                << "Since the mismatch is at low Pt, it is ignored and only a "
                   "warning is issued.\n"
                << "\nIn extremely rare instances the mismatch could be caused "
                   "by a difference in the machine precision in which case "
                   "make sure the original jet collection is produced and "
                   "reclustering is performed in the same job.";
          else
        ...
        std::vector<fastjet::PseudoJet> constituents =
            inclusiveJets.at(reclusteredIndices.at(i)).constituents();

        std::vector<int> svIndices;
        // loop over jet constituents and try to find "ghosts"
        for (std::vector<fastjet::PseudoJet>::const_iterator it =
                 constituents.begin();
        ...
        // loop over clustered SVs and assign them to different subjets based on
        // smallest dR
        for (size_t sv = 0; sv < svIndices.size(); ++sv) {
          ...
          dir = dir.unit();
          fastjet::PseudoJet p(dir.x(), dir.y(), dir.z(),
                               dir.mag()); // using SV flight direction so
                                           // treating SV as massless
          if (useSVMomentum)
            p = fastjet::PseudoJet(extSV.p4().px(), extSV.p4().py(),
                                   extSV.p4().pz(), extSV.p4().energy());

          std::vector<double> dR2toSubjets;

          for (size_t sj = 0; sj < subjetIndices.at(i).size(); ++sj)
            dR2toSubjets.push_back(Geom::deltaR2(
                p.rapidity(), p.phi_std(),
                trackIPTagInfos->at(subjetIndices.at(i).at(sj))
                    .jet()
                    ->rapidity(),
                trackIPTagInfos->at(subjetIndices.at(i).at(sj)).jet()->phi()));

          // find the closest subjet
          int closestSubjetIdx = std::distance(
              dR2toSubjets.begin(),
              std::min_element(dR2toSubjets.begin(), dR2toSubjets.end()));

          clusteredSVs.at(subjetIndices.at(i).at(closestSubjetIdx))
              .push_back(svIndices.at(sv));
        }
      }
    } else {
      ...
      std::vector<int> reclusteredIndices;
      matchReclusteredJets<std::vector<IPTI>>(trackIPTagInfos, inclusiveJets,
                                              reclusteredIndices);
      ...
      for (size_t i = 0; i < trackIPTagInfos->size(); ++i) {
        ...
        if (trackIPTagInfos->at(i).jet()->pt() ==
            0) // continue if the original jet has Pt=0
        {
          edm::LogWarning("NullTransverseMomentum")
              << "The original jet " << i
        ...
        // since the "ghosts" are extremely soft, the configuration and ordering
        // of the reclustered and original jets should in principle stay the
        // same
        if ((std::abs(inclusiveJets.at(reclusteredIndices.at(i)).pt() -
                      trackIPTagInfos->at(i).jet()->pt()) /
             trackIPTagInfos->at(i).jet()->pt()) > relPtTolerance) {
          if (trackIPTagInfos->at(i).jet()->pt() <
              10.) // special handling for low-Pt jets (Pt<10 GeV)
            edm::LogWarning("JetPtMismatchAtLowPt")
                << "The reclustered and original jet " << i
                << " have different Pt's ("
                << inclusiveJets.at(reclusteredIndices.at(i)).pt() << " vs "
                << trackIPTagInfos->at(i).jet()->pt()
                << " GeV, respectively).\n"
                << "Please check that the jet algorithm and jet size match "
                   "those used for the original jet collection and also make "
                   "sure the original jets are uncorrected. In addition, make "
                   "sure you are not using CaloJets which are presently not "
                   "supported.\n"
                << "Since the mismatch is at low Pt, it is ignored and only a "
                   "warning is issued.\n"
                << "\nIn extremely rare instances the mismatch could be caused "
        ...
        for (std::vector<fastjet::PseudoJet>::const_iterator it =
          ...
          clusteredSVs.at(i).push_back(
              it->user_info<VertexInfo>().vertexIndex());
        }
      }
    }
  }
  // case where fat jets are used to associate SVs to subjets but no SV
  // clustering is performed
  else if (useExternalSV && !useSVClustering && !trackIPTagInfos->empty() &&
           useFatJets) {
    // match groomed and original fat jets
    std::vector<int> groomedIndices;
    if (useGroomedFatJets)
    ...
    for (size_t i = 0; i < fatJetsHandle->size(); ++i) {
      ...
      for (typename edm::View<VTX>::const_iterator it = extSecVertex->begin();
        ...
        if (Geom::deltaR2(dir, jetDir) >
            rParam * rParam) // here using the jet clustering rParam as a dR cut
          continue;
  ...
  if (useGhostTrack)
    vertexRecoGT.reset(new GhostTrackVertexFinder(
        ...
        getGhostTrackFitType(
            vtxRecoPSet.getParameter<std::string>("fitType"))));
  else
  ...
  for (typename std::vector<IPTI>::const_iterator iterJets =
           trackIPTagInfos->begin();
       iterJets != trackIPTagInfos->end(); ++iterJets) {
    TrackDataVector trackData;
    //		      std::cout << "Jet " << iterJets-trackIPTagInfos->begin()
    //<< std::endl;
    ...
    GlobalVector jetDir(jetRef->momentum().x(), jetRef->momentum().y(),
                        jetRef->momentum().z());
    ...
    for (unsigned int i = 0; i < indices.size(); i++) {
      typedef
          typename TemplatedSecondaryVertexTagInfo<IPTI, VTX>::IndexedTrackData
              IndexedTrackData;
      ...
      if (!trackSelector(*reco::btag::toTrack(trackRef), ipData[indices[i]],
                         *jetRef, RecoVertex::convertPos(pv.position()))) {
        trackData.back().second.svStatus =
            TemplatedSecondaryVertexTagInfo<IPTI,
                                            VTX>::TrackData::trackSelected;
        continue;
      }
      ...
      trackData.back().second.svStatus = TemplatedSecondaryVertexTagInfo<
          IPTI, VTX>::TrackData::trackUsedForVertexFit;
    ...
    if (useGhostTrack)
      ghostTrack.reset(new GhostTrack(
          GhostTrackPrediction(RecoVertex::convertPos(pv.position()),
                               RecoVertex::convertError(pv.error()),
                               GlobalVector(iterJets->ghostTrack()->px(),
                                            iterJets->ghostTrack()->py(),
                                            iterJets->ghostTrack()->pz()),
                               0.05),
          *gtPred, gtStates, iterJets->ghostTrack()->chi2(),
    ...
    if (!useExternalSV) {
      ...
      case CONSTRAINT_PV_ERROR_SCALED: {
        ...
        BeamSpot bs(pv.position(), sigmaZ,
                    ...
                    beamSpot.isValid() ? beamSpot->dydz() : 0., beamWidth, cov,
                    BeamSpot::Unknown);
      ...
      case CONSTRAINT_PV_PRIMARIES_IN_FIT: {
        std::vector<TransientTrack> primaries_(primaries.begin(),
                                               primaries.end());
        if (useGhostTrack)
          fittedSVs =
              vertexRecoGT->vertices(pv, *beamSpot, primaries_, *ghostTrack);
        else
      ...
      std::remove_copy_if(
          boost::make_transform_iterator(fittedSVs.begin(), svBuilder),
          boost::make_transform_iterator(fittedSVs.end(), svBuilder),
          std::back_inserter(SVs), SVFilter(vertexFilter, pv, jetDir));

    } else {
      if (useSVClustering || useFatJets) {
        size_t jetIdx = (iterJets - trackIPTagInfos->begin());

        for (size_t iExtSv = 0; iExtSv < clusteredSVs.at(jetIdx).size();
             iExtSv++) {
          const VTX &extVertex =
              (*extSecVertex)[clusteredSVs.at(jetIdx).at(iExtSv)];
        ...
        for (size_t iExtSv = 0; iExtSv < extSecVertex->size(); iExtSv++) {
          ...
          if (Geom::deltaR2((position(extVertex) - pv.position()),
                            (extSVDeltaRToJet > 0) ? jetDir : -jetDir) >
      ...
      SVBuilder svBuilder(pv, jetDir, withPVError, minTrackWeight);
      std::remove_copy_if(
          boost::make_transform_iterator(extAssoCollection.begin(), svBuilder),
    ...
    std::vector<typename TemplatedSecondaryVertexTagInfo<IPTI, VTX>::VertexData>
        svData;
...
// Need specialized template because reco::Vertex iterators are TrackBase and it
// is a mess to make general
template <>
void TemplatedSecondaryVertexProducer<TrackIPTagInfo, reco::Vertex>::
    markUsedTracks(TrackDataVector &trackData, const input_container &trackRefs,
                   const SecondaryVertex &sv, size_t idx) {
  for (Vertex::trackRef_iterator iter = sv.tracks_begin();
    ...
    typename input_container::const_iterator pos = std::find(
        trackRefs.begin(), trackRefs.end(), iter->castTo<input_item>());
    ...
    if (pos == trackRefs.end()) {
      ...
      trackData[index].second.svStatus = (btag::TrackData::Status)(
          (unsigned int)btag::TrackData::trackAssociatedToVertex + idx);
    }
  }
}
template <>
void TemplatedSecondaryVertexProducer<CandIPTagInfo,
                                      reco::VertexCompositePtrCandidate>::
    markUsedTracks(TrackDataVector &trackData, const input_container &trackRefs,
                   const SecondaryVertex &sv, size_t idx) {
  for (typename input_container::const_iterator iter =
           sv.daughterPtrVector().begin();
       iter != sv.daughterPtrVector().end(); ++iter) {
...
template <>
typename TemplatedSecondaryVertexProducer<TrackIPTagInfo,
                                          reco::Vertex>::SecondaryVertex
TemplatedSecondaryVertexProducer<TrackIPTagInfo, reco::Vertex>::SVBuilder::
operator()(const TransientVertex &sv) const {
  if (!sv.originalTracks().empty() &&
      sv.originalTracks()[0].trackBaseRef().isNonnull())
    return SecondaryVertex(pv, sv, direction, withPVError);
  else {
    edm::LogError("UnexpectedInputs")
...
template <>
typename TemplatedSecondaryVertexProducer<
    CandIPTagInfo, reco::VertexCompositePtrCandidate>::SecondaryVertex
TemplatedSecondaryVertexProducer<CandIPTagInfo,
                                 reco::VertexCompositePtrCandidate>::SVBuilder::
operator()(const TransientVertex &sv) const {
  if (!sv.originalTracks().empty() &&
      sv.originalTracks()[0].trackBaseRef().isNonnull()) {
    edm::LogError("UnexpectedInputs")
    ...
    vtxCompPtrCand.setVertex(Candidate::Point(
    ...
    for (std::vector<reco::TransientTrack>::const_iterator tt =
             sv.originalTracks().begin();
         tt != sv.originalTracks().end(); ++tt) {
      if (sv.trackWeight(*tt) < minTrackWeight)
        continue;

      const CandidatePtrTransientTrack *cptt =
          dynamic_cast<const CandidatePtrTransientTrack *>(
              tt->basicTransientTrack());
      if (cptt == nullptr)
        edm::LogError("DynamicCastingFailed")
            << "Casting of TransientTrack to CandidatePtrTransientTrack "
               "failed!";
      else {
...
// ------------ method that matches reclustered and original jets based on
// minimum dR ------------
template <class IPTI, class VTX>
template <class CONTAINER>
void TemplatedSecondaryVertexProducer<IPTI, VTX>::matchReclusteredJets(
    ...
    std::vector<int> &matchedIndices, const std::string &jetType) {
  ...
  for (size_t j = 0; j < jets->size(); ++j) {
    ...
    for (size_t rj = 0; rj < reclusteredJets.size(); ++rj) {
      ...
      double tempDR2 = Geom::deltaR2(
          toJet(jets->at(j))->rapidity(), toJet(jets->at(j))->phi(),
          reclusteredJets.at(rj).rapidity(), reclusteredJets.at(rj).phi_std());
    ...
    if (matchedIdx >= 0) {
      if (matchedDR2 > rParam * rParam) {
        edm::LogError("JetMatchingFailed")
            << "Matched reclustered jet " << matchedIdx << " and original "
            << type << "jet " << j
            << " are separated by dR=" << sqrt(matchedDR2)
            << " which is greater than the jet size R=" << rParam << ".\n"
            << "This is not expected so please check that the jet algorithm "
               "and jet size match those used for the original "
            << type << "jet collection.";
      } else
      ...
      edm::LogError("JetMatchingFailed")
          ...
          << "jets failed. Please check that the jet algorithm and jet size "
             "match those used for the original "
          << type << "jet collection.";
...
// ------------ method that matches groomed and original jets based on minimum
// dR ------------
template <class IPTI, class VTX>
void TemplatedSecondaryVertexProducer<IPTI, VTX>::matchGroomedJets(
  ...
  for (size_t gj = 0; gj < groomedJets->size(); ++gj) {
    ...
    if (groomedJets->at(gj).pt() >
        0.) // skip pathological cases of groomed jets with Pt=0
    {
      for (size_t j = 0; j < jets->size(); ++j) {
        if (jetLocks.at(j))
          continue; // skip jets that have already been matched
        ...
        double tempDR2 = Geom::deltaR2(
            jets->at(j).rapidity(), jets->at(j).phi(),
            groomedJets->at(gj).rapidity(), groomedJets->at(gj).phi());
        if (tempDR2 < matchedDR2) {
          matchedDR2 = tempDR2;
          matchedIdx = j;
        }
    ...
    if (matchedIdx >= 0) {
      if (matchedDR2 > rParam * rParam) {
        edm::LogWarning("MatchedJetsFarApart")
            << "Matched groomed jet " << gj << " and original jet "
            << matchedIdx << " are separated by dR=" << sqrt(matchedDR2)
            << " which is greater than the jet size R=" << rParam << ".\n"
            << "This is not expected so the matching of these two jets has "
               "been discarded. Please check that the two jet collections "
  ...
  for (size_t j = 0; j < jets->size(); ++j) {
    std::vector<int>::iterator matchedIndex =
        std::find(jetIndices.begin(), jetIndices.end(), j);

    matchedIndices.push_back(
        matchedIndex != jetIndices.end()
            ? std::distance(jetIndices.begin(), matchedIndex)
            : -1);
  }
}
...
template <class IPTI, class VTX>
void TemplatedSecondaryVertexProducer<IPTI, VTX>::matchSubjets(
  ...
  for (size_t g = 0; g < groomedIndices.size(); ++g) {
    ...
    if (groomedIndices.at(g) >= 0) {
      for (size_t s = 0;
           s < groomedJets->at(groomedIndices.at(g)).numberOfDaughters(); ++s) {
        const edm::Ptr<reco::Candidate> &subjet =
            groomedJets->at(groomedIndices.at(g)).daughterPtr(s);

        for (size_t sj = 0; sj < subjets->size(); ++sj) {
          const edm::RefToBase<reco::Jet> &subjetRef = subjets->at(sj).jet();
          if (subjet == edm::Ptr<reco::Candidate>(
                            subjetRef.id(), subjetRef.get(), subjetRef.key())) {
      ...
      if (subjetIndices.empty())
        edm::LogError("SubjetMatchingFailed")
            << "Matching subjets to original fat jets failed. Please check "
               "that the groomed fat jet and subjet collections belong to each "
               "other.";
...
template <class IPTI, class VTX>
void TemplatedSecondaryVertexProducer<IPTI, VTX>::matchSubjets(
    const edm::Handle<edm::View<reco::Jet>> &fatJets,
    const edm::Handle<std::vector<IPTI>> &subjets,
    std::vector<std::vector<int>> &matchedIndices) {
  for (size_t fj = 0; fj < fatJets->size(); ++fj) {
    ...
    const pat::Jet *fatJet =
        dynamic_cast<const pat::Jet *>(fatJets->ptrAt(fj).get());

    if (!fatJet) {
      if (fj == 0)
        edm::LogError("WrongJetType")
            << "Wrong jet type for input fat jets. Please check that the input "
               "fat jets are of the pat::Jet type.";
      ...
      if (nSubjetCollections > 0) {
        for (size_t coll = 0; coll < nSubjetCollections; ++coll) {
          ...
          for (size_t fjsj = 0; fjsj < fatJetSubjets.size(); ++fjsj) {
            ...
            for (size_t sj = 0; sj < subjets->size(); ++sj) {
              const pat::Jet *subJet =
                  dynamic_cast<const pat::Jet *>(subjets->at(sj).jet().get());

              if (!subJet) {
                if (fj == 0 && coll == 0 && fjsj == 0 && sj == 0)
                  edm::LogError("WrongJetType")
                      << "Wrong jet type for input subjets. Please check that "
                         "the input subjets are of the pat::Jet type.";

                break;
              } else {
                if (subJet->originalObjectRef() ==
                    fatJetSubjets.at(fjsj)->originalObjectRef()) {
                  subjetIndices.push_back(sj);
                  break;
                }
              }
        ...
        if (subjetIndices.empty() && nSubjets > 0)
          edm::LogError("SubjetMatchingFailed")
              << "Matching subjets to fat jets failed. Please check that the "
                 "fat jet and subjet collections belong to each other.";
...
// module ------------
template <class IPTI, class VTX>
void TemplatedSecondaryVertexProducer<IPTI, VTX>::fillDescriptions(
  ...
  desc.add<edm::InputTag>("beamSpotTag", edm::InputTag("offlineBeamSpot"));
  {
    ...
    vertexReco.addOptionalNode(
        edm::ParameterDescription<double>("minweight", 0.5, true) and
            edm::ParameterDescription<double>("weightthreshold", 0.001,
                                              true) and
    ...
    vertexReco.addOptionalNode(
        edm::ParameterDescription<double>("maxFitChi2", 10.0, true) and
            ...
            edm::ParameterDescription<std::string>(
                "fitType", "RefitGhostTrackWithVertices", true),
        true);
  ...
  desc.add<edm::InputTag>("trackIPTagInfos",
                          edm::InputTag("impactParameterTagInfos"));
  {
  ...
  desc.add<edm::InputTag>("extSVCollection",
                          edm::InputTag("secondaryVertices"));
  desc.addOptionalNode(
      edm::ParameterDescription<bool>("useSVClustering", false, true) and
...
typedef TemplatedSecondaryVertexProducer<TrackIPTagInfo, reco::Vertex>
    SecondaryVertexProducer;
typedef TemplatedSecondaryVertexProducer<CandIPTagInfo,
                                         reco::VertexCompositePtrCandidate>
