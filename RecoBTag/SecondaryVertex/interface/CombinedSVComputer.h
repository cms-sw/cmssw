...
public:
  ...
  threshTrack(const reco::CandIPTagInfo &trackIPTagInfo,
              const reco::btag::SortCriteria sort, const reco::Jet &jet,
...
template <class SVTI, class IPTI>
void CombinedSVComputer::fillCommonVariables(
    reco::TaggingVariableList &vars, reco::TrackKinematics &vertexKinematics,
    const IPTI &ipInfo, const SVTI &svInfo, double &vtx_track_ptSum,
    double &vtx_track_ESum) const {
  ...
  if (vtx >= 0) {
    ...
    vars.insert(btau::flightDistance1dVal,
                flipValue(svInfo.flightDistance(vtx, 1).value(), true), true);
    vars.insert(btau::flightDistance1dSig,
                flipValue(svInfo.flightDistance(vtx, 1).significance(), true),
                true);
    vars.insert(btau::flightDistance2dVal,
                flipValue(svInfo.flightDistance(vtx, 2).value(), true), true);
    vars.insert(btau::flightDistance2dSig,
                flipValue(svInfo.flightDistance(vtx, 2).significance(), true),
                true);
    vars.insert(btau::flightDistance3dVal,
                flipValue(svInfo.flightDistance(vtx, 3).value(), true), true);
    vars.insert(btau::flightDistance3dSig,
                flipValue(svInfo.flightDistance(vtx, 3).significance(), true),
                true);
    vars.insert(btau::vertexJetDeltaR,
                Geom::deltaR(svInfo.flightDirection(vtx),
                             vertexFlip ? -jetDir : jetDir),
                true);
  ...
  range_for(i, range) {
    ...
    jet_track_ESum += std::sqrt(track->momentum().Mag2() +
                                ROOT::Math::Square(ParticleMasses::piPlus));

    // add track to kinematics for all tracks in jet
    // allKinematics.add(track); // would make more sense for some variables,
    // e.g. vertexEnergyRatio nicely between 0 and 1, but not necessarily the
    // best option for the discriminating power...

    // filter track -> this track selection can be tighter than the vertex track
    // selection (used to fill the track related variables...)
    ...
    vars.insert(btau::trackSip3dSig, flipValue(data.ip3d.significance(), false),
                true);
    vars.insert(btau::trackSip2dVal, flipValue(data.ip2d.value(), false), true);
    vars.insert(btau::trackSip2dSig, flipValue(data.ip2d.significance(), false),
                true);
    vars.insert(btau::trackJetDistVal, data.distanceToJetAxis.value(), true);
    //              vars.insert(btau::trackJetDistSig,
    //              data.distanceToJetAxis.significance(), true);
    //              vars.insert(btau::trackFirstTrackDist,
    //              data.distanceToFirstTrack, true);
    //              vars.insert(btau::trackGhostTrackVal,
    //              data.distanceToGhostTrack.value(), true);
    //              vars.insert(btau::trackGhostTrackSig,
    //              data.distanceToGhostTrack.significance(), true);
    vars.insert(btau::trackDecayLenVal,
                havePv ? (data.closestToJetAxis - pv).mag() : -1.0, true);

    vars.insert(btau::trackMomentum, trackMag, true);
    vars.insert(btau::trackEta, trackMom.Eta(), true);

    // check for erroneous Perp^2 values before evaluating Perp (fix for DeepCSV
    // NaN outputs)
    double perp_trackMom_jetDir = VectorUtil::Perp2(trackMom, jetDir);
    perp_trackMom_jetDir =
        (perp_trackMom_jetDir > 0. ? std::sqrt(perp_trackMom_jetDir) : 0.);
  ...
  if (vtxType == btag::Vertices::NoVertex &&
    ...
    for (typename std::vector<TrackRef>::const_iterator trkIt =
             pseudoVertexTracks.begin();
         trkIt != pseudoVertexTracks.end(); ++trkIt) {
      vars.insert(btau::trackEtaRel,
                  reco::btau::etaRel(jetDir, (*trkIt)->momentum()), true);
      vtx_track_ptSum += std::sqrt((*trkIt)->momentum().Perp2());
      vtx_track_ESum += std::sqrt((*trkIt)->momentum().Mag2() +
                                  ROOT::Math::Square(ParticleMasses::piPlus));
    }
  }
  ...
  vars.insert(btau::trackSip3dSigAboveCharm,
              flipValue(threshTrack(ipInfo, reco::btag::IP3DSig, *jet, pv)
                            .ip3d.significance(),
                        false),
              true);
  vars.insert(
      btau::trackSip3dValAboveCharm,
      flipValue(threshTrack(ipInfo, reco::btag::IP3DSig, *jet, pv).ip3d.value(),
                false),
      true);
  vars.insert(btau::trackSip2dSigAboveCharm,
              flipValue(threshTrack(ipInfo, reco::btag::IP2DSig, *jet, pv)
                            .ip2d.significance(),
                        false),
              true);
  vars.insert(
      btau::trackSip2dValAboveCharm,
      flipValue(threshTrack(ipInfo, reco::btag::IP2DSig, *jet, pv).ip2d.value(),
                false),
      true);
  ...
  if (vtxType != btag::Vertices::NoVertex) {
    ...
    math::XYZTLorentzVector vertexSum =
        useTrackWeights ? vertexKinematics.weightedVectorSum()
    ...
    if (vtxType != btag::Vertices::RecoVertex) {
      ...
      vars.insert(btau::vertexJetDeltaR, VectorUtil::DeltaR(vertexSum, jetDir),
                  true);
    }
    ...
    if (vtxType == btag::Vertices::RecoVertex && vertexMassCorrection) {
      ...
      double vertexPt2 =
          math::XYZVector(dir.x(), dir.y(), dir.z()).Cross(vertexSum).Mag2() /
          dir.mag2();
      vertexMass =
          std::sqrt(vertexMass * vertexMass + vertexPt2) + std::sqrt(vertexPt2);
    }
    vars.insert(btau::vertexMass, vertexMass, true);

    double varPi =
        (vertexMass / 5.2794) *
        (vtx_track_ESum / jet_track_ESum); // 5.2794 should be the average B
                                           // meson mass of PDG! CHECK!!!
    vars.insert(btau::massVertexEnergyFraction, varPi / (varPi + 0.04), true);
    double varB = (std::sqrt(5.2794) * vtx_track_ptSum) /
                  (vertexMass * std::sqrt(jet->pt()));
    vars.insert(btau::vertexBoostOverSqrtJetPt,
                varB * varB / (varB * varB + 10.), true);
  ...
  if (pfJet != nullptr) {
    vars.insert(btau::chargedHadronEnergyFraction,
                pfJet->chargedHadronEnergyFraction(), true);
    vars.insert(btau::neutralHadronEnergyFraction,
                pfJet->neutralHadronEnergyFraction(), true);
    vars.insert(btau::photonEnergyFraction, pfJet->photonEnergyFraction(),
                true);
    vars.insert(btau::electronEnergyFraction, pfJet->electronEnergyFraction(),
                true);
    vars.insert(btau::muonEnergyFraction, pfJet->muonEnergyFraction(), true);
    vars.insert(btau::chargedHadronMultiplicity,
                pfJet->chargedHadronMultiplicity(), true);
    vars.insert(btau::neutralHadronMultiplicity,
                pfJet->neutralHadronMultiplicity(), true);
    vars.insert(btau::photonMultiplicity, pfJet->photonMultiplicity(), true);
    vars.insert(btau::electronMultiplicity, pfJet->electronMultiplicity(),
                true);
    vars.insert(btau::muonMultiplicity, pfJet->muonMultiplicity(), true);
    vars.insert(btau::hadronMultiplicity,
                pfJet->chargedHadronMultiplicity() +
                    pfJet->neutralHadronMultiplicity(),
                true);
    vars.insert(btau::hadronPhotonMultiplicity,
                pfJet->chargedHadronMultiplicity() +
                    pfJet->neutralHadronMultiplicity() +
                    pfJet->photonMultiplicity(),
                true);
    ...
    vars.insert(btau::chargedHadronEnergyFraction,
                patJet->chargedHadronEnergyFraction(), true);
    vars.insert(btau::neutralHadronEnergyFraction,
                patJet->neutralHadronEnergyFraction(), true);
    vars.insert(btau::photonEnergyFraction, patJet->photonEnergyFraction(),
                true);
    vars.insert(btau::electronEnergyFraction, patJet->electronEnergyFraction(),
                true);
    vars.insert(btau::muonEnergyFraction, patJet->muonEnergyFraction(), true);
    vars.insert(btau::chargedHadronMultiplicity,
                patJet->chargedHadronMultiplicity(), true);
    vars.insert(btau::neutralHadronMultiplicity,
                patJet->neutralHadronMultiplicity(), true);
    vars.insert(btau::photonMultiplicity, patJet->photonMultiplicity(), true);
    vars.insert(btau::electronMultiplicity, patJet->electronMultiplicity(),
                true);
    vars.insert(btau::muonMultiplicity, patJet->muonMultiplicity(), true);
    vars.insert(btau::hadronMultiplicity,
                patJet->chargedHadronMultiplicity() +
                    patJet->neutralHadronMultiplicity(),
                true);
    vars.insert(btau::hadronPhotonMultiplicity,
                patJet->chargedHadronMultiplicity() +
                    patJet->neutralHadronMultiplicity() +
                    patJet->photonMultiplicity(),
                true);
    vars.insert(btau::totalMultiplicity,
                patJet->chargedHadronMultiplicity() +
                    patJet->neutralHadronMultiplicity() +
                    patJet->photonMultiplicity() +
                    patJet->electronMultiplicity() + patJet->muonMultiplicity(),
                true);
