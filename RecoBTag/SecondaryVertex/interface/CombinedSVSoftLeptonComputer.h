...
template <class IPTI, class SVTI>
reco::TaggingVariableList CombinedSVSoftLeptonComputer::
operator()(const IPTI &ipInfo, const SVTI &svInfo,
           const reco::CandSoftLeptonTagInfo &muonInfo,
           const reco::CandSoftLeptonTagInfo &elecInfo) const {
  ...
  unsigned int vtxType =
      (vars.checkTag(reco::btau::vertexCategory)
           ? (unsigned int)(vars.get(reco::btau::vertexCategory))
           : 99);
  if (vtxType == 99)
    return vars;
  ...
  for (unsigned int i = 0; i < muonInfo.leptons();
       ++i) // loop over all muons, not optimal -> find the best or use ranking
            // from best to worst
  {
    ...
    vars.insert(btau::leptonSip3d, flipSoftLeptonValue(propertiesMuon.sip3d),
                true);
  ...
  if (leptonCategory != 1) // no soft muon found, try soft electron
  {
    for (unsigned int i = 0; i < elecInfo.leptons();
         ++i) // loop over all electrons, not optimal -> find the best or use
              // ranking from best to worst
    {
      ...
      vars.insert(btau::leptonSip3d, flipSoftLeptonValue(propertiesElec.sip3d),
                  true);
  ...
  // set the default value for vertexLeptonCategory to 2 (=
  // NoVertexNoSoftLepton)
  int vertexLepCat = 2;
