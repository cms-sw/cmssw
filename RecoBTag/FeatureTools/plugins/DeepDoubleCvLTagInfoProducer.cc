...
class DeepDoubleCvLTagInfoProducer : public edm::stream::EDProducer<>
{

public:
    explicit DeepDoubleCvLTagInfoProducer(const edm::ParameterSet&);
    ~DeepDoubleCvLTagInfoProducer() override;

    static void fillDescriptions(edm::ConfigurationDescriptions& descriptions);

private:
    typedef std::vector<reco::DeepDoubleCvLTagInfo> DeepDoubleCvLTagInfoCollection;
...
DeepDoubleCvLTagInfoProducer::DeepDoubleCvLTagInfoProducer(
...
{
    produces<DeepDoubleCvLTagInfoCollection>();
}
...
void DeepDoubleCvLTagInfoProducer::fillDescriptions(
    edm::ConfigurationDescriptions& descriptions)
{
    ...
    descriptions.add("pfDeepDoubleCvLTagInfos", desc);
}

void DeepDoubleCvLTagInfoProducer::produce(edm::Event& iEvent,
    const edm::EventSetup& iSetup)
{

    auto output_tag_infos = std::make_unique<DeepDoubleCvLTagInfoCollection>();
    ...
    for (std::size_t jet_n = 0; jet_n < jets->size(); jet_n++)
    {
        ...
        btagbtvdeep::DeepDoubleCvLFeatures features;
...
DEFINE_FWK_MODULE(DeepDoubleCvLTagInfoProducer);
