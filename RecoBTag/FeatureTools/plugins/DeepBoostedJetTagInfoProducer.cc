...
const std::vector<std::string> DeepBoostedJetTagInfoProducer::particle_features_hlt_{
    ...
    "jet_pfcand_nhits",       "jet_pfcand_npixhits",     "jet_pfcand_nstriphits",      "jet_pfcand_trackjet_decayL",
  ...
  for (const auto &cand : daughters) {
	...
	if (track) {
	  candidate = pat::PackedCandidate(cand->polarP4(),
					   ...
					   PVRefProd,
					   pv_ass.key());
	  candidate.setAssociationQuality(pat::PackedCandidate::PVAssociationQuality(btagbtvdeep::vtx_ass_from_pfcand(*reco_cand, pv_ass_quality, pv_ass)));
