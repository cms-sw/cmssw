...
void HcalDigisValidation::book2D(DQMStore::IBooker &ib, const std::string& name, const HistLim& limX, const HistLim& limY) {
...
void HcalDigisValidation::bookPf(DQMStore::IBooker &ib, const std::string& name, const HistLim& limX, const HistLim& limY) {
    if (!msm_->count(name)) (*msm_)[name] = ib.bookProfile(name.c_str(), name.c_str(), limX.n, limX.min, limX.max, limY.n, limY.min, limY.max);
}

void HcalDigisValidation::bookPf(DQMStore::IBooker &ib, const std::string& name, const HistLim& limX, const HistLim& limY, const char *option) {
...
void HcalDigisValidation::fillPf(const std::string& name, double X, double Y) {
    msm_->find(name)->second->Fill(X, Y);
}

MonitorElement* HcalDigisValidation::monitor(const std::string& name) {
