...
int main(int argc, char *argv[]) {
  ...
  PlotCompareUtility *pc = new PlotCompareUtility(argv[1],argv[2],argv[3],"METTask_CT_");
  HistoData *hd;
  ...
  hd = pc->AddHistoData("Occ_ieta_iphi"); hd->SetType(1); hd->SetValueX("Occupancy");
  hd = pc->AddHistoData("Occvsieta"); hd->SetType(2); hd->SetValueX("CaloTower Occupancy");
  hd = pc->AddHistoData("SETvsieta"); hd->SetType(2); hd->SetValueX("SumET (GeV)");
  hd = pc->AddHistoData("METvsieta"); hd->SetType(2); hd->SetValueX("MET (GeV)");
  hd = pc->AddHistoData("MExvsieta"); hd->SetType(2); hd->SetValueX("MEx (GeV)");
  hd = pc->AddHistoData("MEyvsieta"); hd->SetType(2); hd->SetValueX("MEy (GeV)");
  hd = pc->AddHistoData("METPhivsieta"); hd->SetType(2); hd->SetValueX("MET-phi (radians)");
...
//      href->SetNormFactor(hnew->GetEntries());
	...
	//doing the statistical  tests between hnew and href
...
//      href->SetNormFactor(hnew->GetEntries());
      ...
      if (TMath::Abs(XaxisMin_ref - XaxisMax_ref)<1E-6)
	{
      ...
      hdumb->GetXaxis()->SetTitleSize(0.033);
      hdumb->GetXaxis()->SetLabelSize(0.03);
      hdumb->GetXaxis()->SetTitleOffset(1);
  ...
  for (int i = 0; i < num_histos; i++) {
    ...
    if (TMath::Abs(box_min - box_max) < 0.01) {
