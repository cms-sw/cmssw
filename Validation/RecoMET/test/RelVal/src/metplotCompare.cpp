...
int main(int argc, char *argv[]) {
	...
	hd = pc->AddHistoData("CaloMET_Nvtx30toInf"); hd->SetType(2);
	hd = pc->AddHistoData("CaloMET_Nvtx21to30"); hd->SetType(2);
	hd = pc->AddHistoData("CaloMET_Nvtx16to20"); hd->SetType(2);
	hd = pc->AddHistoData("CaloMET_Nvtx11to15"); hd->SetType(2);
	hd = pc->AddHistoData("CaloMET_Nvtx6to10"); hd->SetType(2);
	hd = pc->AddHistoData("CaloMET_Nvtx0to5"); hd->SetType(2);

	hd = pc->AddHistoData("CaloSumET_Nvtx30toInf"); hd->SetType(2);
	hd = pc->AddHistoData("CaloSumET_Nvtx21to30"); hd->SetType(2);
	hd = pc->AddHistoData("CaloSumET_Nvtx16to20"); hd->SetType(2);
	hd = pc->AddHistoData("CaloSumET_Nvtx11to15"); hd->SetType(2);
	hd = pc->AddHistoData("CaloSumET_Nvtx6to10"); hd->SetType(2);
	hd = pc->AddHistoData("CaloSumET_Nvtx0to5"); hd->SetType(2);
	hd = pc->AddHistoData("Nvertex"); hd->SetType(0);

	hd = pc->AddHistoData("CaloMETPhiResolution_GenMETCalo"); hd->SetType(4);
	hd = pc->AddHistoData("CaloMETResolution_GenMETCalo"); hd->SetType(3);
	hd = pc->AddHistoData("CaloMETPhiResolution_GenMETTrue"); hd->SetType(4);
	hd = pc->AddHistoData("CaloMETResolution_GenMETTrue"); hd->SetType(3);

	hd = pc->AddHistoData("CaloEmEtFraction"); hd->SetType(2);
	hd = pc->AddHistoData("CaloEmEtInEB"); hd->SetType(2);
	hd = pc->AddHistoData("CaloEmEtInEE"); hd->SetType(2);
	hd = pc->AddHistoData("CaloEmEtInHF"); hd->SetType(2);
	hd = pc->AddHistoData("CaloEtFractionHadronic"); hd->SetType(2);

	hd = pc->AddHistoData("CaloHadEtInHB"); hd->SetType(2);
	hd = pc->AddHistoData("CaloHadEtInHE"); hd->SetType(2);
	hd = pc->AddHistoData("CaloHadEtInHF"); hd->SetType(2);
	hd = pc->AddHistoData("CaloHadEtInHO"); hd->SetType(2);
	hd = pc->AddHistoData("CaloSETInpHF"); hd->SetType(2);
	hd = pc->AddHistoData("CaloSETInmHF"); hd->SetType(2);

	hd = pc->AddHistoData("CaloMET"); hd->SetType(2);

	hd = pc->AddHistoData("CaloMETPhi"); hd->SetType(3);
	hd = pc->AddHistoData("CaloMETSig"); hd->SetType(2);
	hd = pc->AddHistoData("CaloMEx"); hd->SetType(2);
	hd = pc->AddHistoData("CaloMEy"); hd->SetType(2);
	hd = pc->AddHistoData("CaloMaxEtInEmTowers"); hd->SetType(1);

	hd = pc->AddHistoData("CaloMaxEtInHadTowers"); hd->SetType(1);
	hd = pc->AddHistoData("CaloSumET"); hd->SetType(2);


	if (pc->GetStatus() != 0) { cerr << "error encountered, exiting.\n"; return pc->GetStatus(); }

	int num_histos = pc->GetNumHistos(); //changed 7/9 from pc->GetNumHistos()-1
	bool combinedFailed = false;
	const float threshold = KS_TEST ?  pc->GetKSThreshold() : pc->GetChi2Threshold(); 
	...
	for (int index = 0; index < pc->GetNumHistos(); index++) {
		...
		if (hnew->GetEntries() <= 1 || href->GetEntries() <= 1) {
			...
			//to get all plots even the ones without any entries -- sam - 3/19/12
			cerr << name << " error: no entries found for " << name  << endl;
		}
		...
		const double ks_score = hnew->KolmogorovTest(href,"D");
		const double chi2_score = hnew->Chi2Test(href, "p");
		...
		href->SetStats(0);
		href->SetLineWidth(1);
		href->SetLineColor(13);
		href->SetMarkerColor(13);
		...
		//    hdumb->GetXaxis()->SetTitleSize(0.038);
		hdumb->GetXaxis()->SetLabelSize(0.033);
		hdumb->GetYaxis()->SetTitle("Entries");
		hdumb->GetYaxis()->SetTitleOffset(1.2);
	...
	main_c.Print("AllResults-1dHistoCheck.eps","eps");
