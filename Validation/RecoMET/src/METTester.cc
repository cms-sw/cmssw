...
METTester::METTester(const edm::ParameterSet& iConfig)
{
  ...
  inputMETToken_           = consumes<reco::METCollection> (
      iConfig.getParameter<edm::InputTag>("InputMETLabel"));
  inputMETToken2_           = consumes<reco::CaloMETCollection> (
      iConfig.getParameter<edm::InputTag>("InputMETLabel"));
  inputCaloMETToken_           = consumes<reco::CaloMETCollection> (
      iConfig.getParameter<edm::InputTag>("InputMETLabel"));
  inputGenMETToken_        = consumes<reco::GenMETCollection> (
      iConfig.getParameter<edm::InputTag>("InputMETLabel"));
  inputPFMETToken_        = consumes<reco::PFMETCollection> (
      iConfig.getParameter<edm::InputTag>("InputMETLabel"));
  if(METType_ == "TCMET") {
    inputCaloMETToken_       = consumes<reco::CaloMETCollection> (
        iConfig.getParameter<edm::InputTag>("InputCaloMETLabel"));
    inputTrackToken_         = consumes<reco::TrackCollection> (
        iConfig.getParameter<edm::InputTag>("InputTrackLabel"));
    inputMuonToken_          = consumes<reco::MuonCollection> (
        iConfig.getParameter<edm::InputTag>("InputMuonLabel"));
    inputMuonFixedToken_     = consumes<reco::MuonCollection> (
        edm::InputTag("muons"));
    inputElectronToken_      = consumes<edm::View<reco::GsfElectron> > (
        iConfig.getParameter<edm::InputTag>("InputElectronLabel"));
    inputBeamSpotToken_      = consumes<reco::BeamSpot> (
        iConfig.getParameter<edm::InputTag>("InputBeamSpotLabel"));
  ...
  genTrue_Token_ = consumes<reco::GenMETCollection> (edm::InputTag("genMetTrue"));
  genCalo_Token_ = consumes<reco::GenMETCollection> (edm::InputTag("genMetCalo"));
  tcMet_ValueMap_Token_ = consumes<edm::ValueMap<reco::MuonMETCorrectionData> > (
      edm::InputTag("muonTCMETValueMapProducer" , "muCorrData"));
  muon_ValueMap_Token_  = consumes<edm::ValueMap<reco::MuonMETCorrectionData> > (
      edm::InputTag("muonMETValueMapProducer" , "muCorrData"));
}
...
void METTester::beginRun(const edm::Run& iRun, const edm::EventSetup& iSetup)
{
  ...
  if (dbe_) {
    ...
    Labels l;
    labelsForToken(inputMETToken_, l);
    TString label(l.module);
    dirName += label;
    ...
    } else if (METType_ == "TCMET" || std::string(l.module) == "corMetGlobalMuons")
    {
      ...
      if(!finebinning_)
      {
        ...
        else if( std::string(l.module) == "corMetGlobalMuons" ) {
          me["hmuPt"] = dbe_->book1D("METTask_muonPt", "METTask_muonPt", 50, 0, 500);
          me["hmuEta"] = dbe_->book1D("METTask_muonEta", "METTask_muonEta", 50, -2.5, 2.5);
          me["hmuNhits"] = dbe_->book1D("METTask_muonNhits", "METTask_muonNhits", 50, 0, 50);
          me["hmuChi2"] = dbe_->book1D("METTask_muonNormalizedChi2", "METTask_muonNormalizedChi2", 20, 0, 20);
...
void METTester::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  if (METType_ == "CaloMET")
  {
    ...
    iEvent.getByToken(inputMETToken2_, calo);
    if (!calo.isValid()) {
    ...
    iEvent.getByToken(genTrue_Token_, genTrue);
    if (genTrue.isValid()) {
    ...
    iEvent.getByToken(genCalo_Token_, genCalo);
    if (genCalo.isValid()) {
  ...
  }	else if (METType_ == "GenMET")
  {
    ...
    iEvent.getByToken(inputGenMETToken_, gen);
    if (!gen.isValid()) {
  ...
  } else if( METType_ == "PFMET")
  {
    ...
    iEvent.getByToken(inputPFMETToken_,hpfmetcol);
  ...
  } else if (METType_ == "MET")
  {
    ...
    iEvent.getByToken(inputMETToken_, hmetcol);
    if (!hmetcol.isValid()) {
      edm::LogInfo("OutputInfo") << " failed to retrieve data required by MET Task";
      edm::LogInfo("OutputInfo") << " MET Task cannot continue...!";
      return;
  ...
  } else if( METType_ == "TCMET" )
  {
    ...
    iEvent.getByToken(inputMETToken_, htcMetcol);

    const CaloMET *caloMet;
    edm::Handle<CaloMETCollection> hcaloMetcol;
    iEvent.getByToken(inputCaloMETToken_, hcaloMetcol);

    edm::Handle< reco::MuonCollection > muon_h;
    iEvent.getByToken(inputMuonToken_, muon_h);

    //      edm::Handle< edm::View<reco::Track> > track_h;
    edm::Handle<reco::TrackCollection> track_h;
    iEvent.getByToken(inputTrackToken_, track_h);

    edm::Handle< edm::View<reco::GsfElectron > > electron_h;
    iEvent.getByToken(inputElectronToken_, electron_h);

    edm::Handle< reco::BeamSpot > beamSpot_h;
    iEvent.getByToken(inputBeamSpotToken_, beamSpot_h);
    ...
    iEvent.getByToken(muon_ValueMap_Token_, muon_ValueMap_Handle);
  ...
  else if( std::string(l.module) == "corMetGlobalMuons" )
  {
    ...
    iEvent.getByToken(inputMETToken_, hcorMetGlobalMuonscol );
    ...
    iEvent.getByToken(muon_ValueMap_Token_, corMetGlobalMuons_ValueMap_Handle);

    edm::Handle< reco::MuonCollection > muon_Handle;
    iEvent.getByToken(inputMuonFixedToken_, muon_Handle);
