...
// 
/**\class TauTagVal_EMIso_BKG TauTagVal_EMIso_BKG.cc RecoTauTag/ConeIsolation/test/TauTagVal_EMIso_BKG.cc
...
TauTagVal_EMIso_BKG::TauTagVal_EMIso_BKG(const edm::ParameterSet& iConfig)
{
  nEvent = 0;
...
void TauTagVal_EMIso_BKG::beginJob(){ 

}

void TauTagVal_EMIso_BKG::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  using namespace edm;
  using namespace reco;
  ...
  if(jetItr != genJets->end() )
    {
      ...
      if(jj <2) new((*TauJets)[jjj])TLorentzVector(TauJetMC);
      jjj++;
    }
...
void TauTagVal_EMIso_BKG::endJob(){
  int ibin;
