...
/**\class TauTagMCValidation TauTagMCValidation.cc
 ...
 Class used to do the MC (data related code is not separated) Validation of the TauTag
...
//
//Modified by: Atanu Modak to include extra plots and clean up
//This code is now deals with MC validation only and has been 
//duplicated from the data certification code in DQMOffline/PFTau area. 
//Difference between the two is small. 
//In future can be merged together using inheritance if needed.
// user include files

#include "Validation/RecoTau/interface/TauTagMCValidation.h"
...
TauTagMCValidation::TauTagMCValidation(const edm::ParameterSet& iConfig):
...
TauTagMCValidation::~TauTagMCValidation() {
  if (genericTriggerEventFlag_) delete genericTriggerEventFlag_;
}

void TauTagMCValidation::bookHistograms(DQMStore::IBooker & ibooker, edm::Run const & iRun, edm::EventSetup const & /* iSetup */)
{
...
void TauTagMCValidation::dqmBeginRun(const edm::Run& iRun, const edm::EventSetup& iSetup) {
      ...
      for ( std::vector< edm::ParameterSet >::iterator it = discriminators_.begin(); it!= discriminators_.end();  it++, j++)
      {
        ...
        if ((*currentDiscriminator)[thePFTau] >= it->getParameter<double>("selectionCut")){
          ...
          if(tauGenJet){
            ...
            LogInfo("TauTagMCValidation") << " Failed to cast the MC candidate.";
	  }
...
double TauTagMCValidation::getSumPt(const std::vector<edm::Ptr<reco::PFCandidate> > & candidates ){
  double sumPt = 0.;
...
bool TauTagMCValidation::stripDiscriminatorLabel(const std::string& discriminatorLabel, std::string & newLabel) {
