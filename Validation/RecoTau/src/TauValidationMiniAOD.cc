...
{
  MonitorElement *ptTemp, *etaTemp, *phiTemp, *massTemp, *decayModeFindingTemp, *decayModeTemp, *byDeepTau2017v2VSerawTemp, *byDeepTau2017v2VSjetrawTemp, *byDeepTau2017v2VSmurawTemp, *summaryTemp;
  ibooker.setCurrentFolder("RecoTauV/miniAODValidation"+extensionName_);

  //summary plots
  histoInfo summaryHinfo = (histoSettings_.exists("summary")) ? histoInfo(histoSettings_.getParameter<edm::ParameterSet>("summary")) : histoInfo(21, -0.5, 20.5);
  ...
  histoInfo decayModeHinfo = (histoSettings_.exists("decayMode")) ? histoInfo(histoSettings_.getParameter<edm::ParameterSet>("decayMode")) : histoInfo(11,-0.5,10.5);
  ...
  decayModeTemp = ibooker.book1D("tau_decayMode", "tau_decayMode", decayModeHinfo.nbins, decayModeHinfo.min, decayModeHinfo.max);
...
void TauValidationMiniAOD::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (refCandidateCollection::const_iterator RefJet= ReferenceCollection->begin() ; RefJet != ReferenceCollection->end(); RefJet++ ){
	...
	pat::TauRef matchedTau(taus, matchedTauIndex);
      ...
      if(matchedTau->isTauIDAvailable("decayModeFinding"))       decayModeFindingMap.find("")    ->second->Fill(matchedTau->tauID("decayModeFinding"));
      if(matchedTau->isTauIDAvailable("byDeepTau2017v2VSeraw"))  byDeepTau2017v2VSerawMap.find("")  ->second->Fill(matchedTau->tauID("byDeepTau2017v2VSeraw"));
      if(matchedTau->isTauIDAvailable("byDeepTau2017v2VSjetraw"))byDeepTau2017v2VSjetrawMap.find("")->second->Fill(matchedTau->tauID("byDeepTau2017v2VSjetraw"));
      if(matchedTau->isTauIDAvailable("byDeepTau2017v2VSmuraw")) byDeepTau2017v2VSmurawMap.find("") ->second->Fill(matchedTau->tauID("byDeepTau2017v2VSmuraw"));
      int j = 0;
      for(const auto& it : discriminators_){
	...
	double selectionCut         = it.getParameter<double>("selectionCut");
        ...
        if(matchedTau->tauID(currentDiscriminator) >= selectionCut) summaryMap.find("Num")->second->Fill(j);
	j = j + 1;
      }
    }
  }
