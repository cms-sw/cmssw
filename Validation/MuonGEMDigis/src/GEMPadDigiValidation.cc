...
GEMPadDigiValidation::GEMPadDigiValidation(const edm::ParameterSet &cfg)
  ...
  InputTagToken_ = consumes<GEMPadDigiCollection>(
      cfg.getParameter<edm::InputTag>("PadLabel"));
...
void GEMPadDigiValidation::bookHistograms(DQMStore::IBooker &ibooker,
  ...
  for (auto &region : GEMGeometry_->regions()) {
    ...
    TString simpleZR_histname =
        TString::Format("pad_simple_zr%s", histname_suffix.Data());
    ...
    for (auto &station : region->stations()) {
      ...
      TString dcEta_histname =
          TString::Format("pad_dcEta%s", histname_suffix2.Data());
    ...
    for (auto &region : GEMGeometry_->regions()) {
      ...
      for (auto &station : region->stations()) {
        ...
        for (int la = 1; la <= 2; la++) {
          ...
          theCSCPad_phipad[region_num][station_num][layer_num] = ibooker.book2D(
              ("pad_dg_phipad" + name_prefix).c_str(),
              ("Digi occupancy: " + label_prefix + "; phi [rad]; Pad number")
                  .c_str(),
              280, -TMath::Pi(), TMath::Pi(), nPads / 2, 0, nPads);
              ...
              ibooker.book1D(("pad_dg" + name_prefix).c_str(),
                             ("Digi occupancy per pad number: " + label_prefix +
                              ";Pad number; entries")
                                 .c_str(),
                             nPads, 0.5, nPads + 0.5);
          theCSCPad_bx[region_num][station_num][layer_num] = ibooker.book1D(
          ...
          theCSCPad_zr[region_num][station_num][layer_num] =
              BookHistZR(ibooker, "pad_dg", "Pad Digi", region_num, station_num,
                         layer_num);
          theCSCPad_xy[region_num][station_num][layer_num] =
              BookHistXY(ibooker, "pad_dg", "Pad Digi", region_num, station_num,
                         layer_num);
          TString xy_name =
              TString::Format("pad_dg_xy%s_odd", name_prefix.c_str());
          TString xy_title = TString::Format(
              "Digi XY occupancy %s at odd chambers", label_prefix.c_str());
          theCSCPad_xy_ch[xy_name.Hash()] =
...
void GEMPadDigiValidation::analyze(const edm::Event &e,
  ...
  try {
    ...
    edm::LogError("GEMPadDigiValidation")
  ...
  for (GEMPadDigiCollection::DigiRangeIterator cItr = gem_digis->begin();
    ...
    for (digiItr = (*cItr).second.first; digiItr != (*cItr).second.second;
      ...
      edm::LogInfo("GEMPadDIGIValidation")
          << "Global x " << g_x << "Global y " << g_y << "\n";
      edm::LogInfo("GEMPadDIGIValidation")
          << "Global pad " << pad << "Global phi " << g_phi << std::endl;
      ...
      TString dcEta_histname =
          TString::Format("pad_dcEta%s", histname_suffix.Data());
