  ...
  for( int region_num = 0 ; region_num <2 ; region_num++ ) {
    for( int station_num = 0 ; station_num < 3 ; station_num++) {
      for( int layer_num = 0 ; layer_num < 2 ; layer_num++) {
        std::string name_prefix  = std::string("_r")+region[region_num]+"_st"+station[station_num]+"_l"+layer[layer_num];
        std::string label_prefix = "region"+region[region_num]+" station "+station[station_num]+" layer "+layer[layer_num];
        theStrip_xy[region_num][station_num][layer_num] = dbe->book2D( ("string_dg_xy"+name_prefix).c_str(), ("Digi occupancy: "+label_prefix+";globalX [cm]; globalY[cm]").c_str(), 260, -260,260,260,-260,260);
        theStrip_phistrip[region_num][station_num][layer_num] = dbe_->book2D( ("strip_dg_phistrip"+name_prefix).c_str(), ("Digi occupancy: "+label_prefix+"; strip number; phi [rad]").c_str(), 192,0,384,280,-TMath::Pi(),TMath::Pi());
        theStrip[region_num][station_num][layer_num] = dbe_->book1D( ("strip_dg"+name_prefix).c_str(), ("Digi occupancy per stip number: "+label_prefix+";strip number; entries").c_str(), 384,0.5,384.5);
