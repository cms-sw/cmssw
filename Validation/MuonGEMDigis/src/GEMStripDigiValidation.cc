...
GEMStripDigiValidation::GEMStripDigiValidation(const edm::ParameterSet &cfg)
  ...
  InputTagToken_ = consumes<GEMDigiCollection>(
      cfg.getParameter<edm::InputTag>("stripLabel"));
...
void GEMStripDigiValidation::bookHistograms(DQMStore::IBooker &ibooker,
  ...
  if (GEMGeometry_ == nullptr)
    return;
  LogDebug("GEMStripDigiValidation")
  ...
  for (auto &region : GEMGeometry_->regions()) {
    ...
    TString simpleZR_histname =
        TString::Format("strip_simple_zr%s", histname_suffix.Data());
    ...
    for (auto &station : region->stations()) {
      ...
      TString dcEta_histname =
          TString::Format("strip_dcEta%s", histname_suffix2.Data());
              ...
              ibooker.book2D(("strip_dg_phistrip" + name_prefix).c_str(),
                             ("Digi occupancy: " + label_prefix +
                              "; phi [rad];strip number")
                                 .c_str(),
                             280, -TMath::Pi(), TMath::Pi(), nstrips / 2, 0,
                             nstrips);
          theStrip[region_num][station_num][layer_num] = ibooker.book1D(
              ("strip_dg" + name_prefix).c_str(),
              ("Digi occupancy per stip number: " + label_prefix +
               ";strip number; entries")
                  .c_str(),
              nstrips, 0.5, nstrips + 0.5);
          theStrip_bx[region_num][station_num][layer_num] = ibooker.book1D(
              ("strip_dg_bx" + name_prefix).c_str(),
              ("Bunch crossing: " + label_prefix + "; bunch crossing ; entries")
                  .c_str(),
              11, -5.5, 5.5);
          theStrip_zr[region_num][station_num][layer_num] =
              BookHistZR(ibooker, "strip_dg", "Strip Digi", region_num,
                         station_num, layer_num);
          theStrip_xy[region_num][station_num][layer_num] =
              BookHistXY(ibooker, "strip_dg", "Strip Digi", region_num,
                         station_num, layer_num);
          TString xy_name =
              TString::Format("strip_dg_xy%s_odd", name_prefix.c_str());
          TString xy_title = TString::Format(
              "Digi XY occupancy %s at odd chambers", label_prefix.c_str());
          theStrip_xy_ch[xy_name.Hash()] =
              ibooker.book2D(xy_name, xy_title, 360, -360, 360, 360, -360, 360);
          xy_name = TString::Format("strip_dg_xy%s_even", name_prefix.c_str());
          xy_title = TString::Format("Digi XY occupancy %s at even chambers",
                                     label_prefix.c_str());
          theStrip_xy_ch[xy_name.Hash()] =
              ibooker.book2D(xy_name, xy_title, 360, -360, 360, 360, -360, 360);
        }
...
void GEMStripDigiValidation::analyze(const edm::Event &e,
  ...
  try {
    ...
    edm::LogError("GEMStripDigiValidation")
  ...
  for (GEMDigiCollection::DigiRangeIterator cItr = gem_digis->begin();
       cItr != gem_digis->end(); cItr++) {
    ...
    for (digiItr = (*cItr).second.first; digiItr != (*cItr).second.second;
      ...
      TString dcEta_histname =
          TString::Format("strip_dcEta%s", histname_suffix.Data());
      ...
      if (detailPlot_) {
        if (theStrip_xy[region_num][station_num][layer_num] != nullptr) {
          ...
          theStrip_phistrip[region_num][station_num][layer_num]->Fill(g_phi,
                                                                      strip);
          ...
          std::cout << "Error is occued when histograms is called."
                    << std::endl;
        }
      }
    }
