...
void GEMStripDigiValidation::bookHistograms(DQMStore::IBooker& booker,
  ...
  for (const auto& region : gem->regions()) {
    ...
    me_occ_zr_[region_id] = bookZROccupancy(booker, region_id, "strip",
                                            "Strip Digi");
    ...
    me_strip_occ_eta_[region_id] = bookHist1D(
        booker, region_id, "matched_strip_occ_eta", "Matched Strip Digi Eta Occupancy",
        50, eta_range_[0], eta_range_[1], "|#eta|");
    ...
    for (const auto& station : region->stations()) {
      ...
      me_strip_occ_phi_[key2] = bookHist1D(
          booker, key2, "matched_strip_occ_phi", "Matched Strip Digi Phi Occupancy",
          51, -M_PI, M_PI, "#phi");
      ...
      for (const auto & chamber : station->superChambers()[0]->chambers()) {
        ...
        if (detail_plot_) {
          ...
          me_detail_occ_xy_[key3] = bookXYOccupancy(booker, key3, "strip",
                                                    "Strip Digi");

          me_detail_occ_strip_[key3] = bookHist1D(
              booker, key3, "strip_occ_strip",
...
void GEMStripDigiValidation::analyze(const edm::Event& event,
  ...
  if (not digi_collection.isValid()) {
    edm::LogError(kLogCategory_)
        << "Cannot get strips by Token stripToken." << std::endl;
    return;
  }
  ...
  for (auto range_iter = digi_collection->begin();
    ...
    for (auto digi = range.first; digi != range.second; ++digi) {
      ...
      GlobalPoint strip_global_pos = surface.toGlobal(
          roll->centreOfStrip(digi->strip()));

      Float_t digi_g_r = strip_global_pos.perp();
      Float_t digi_g_abs_z   = std::abs(strip_global_pos.z());
      
      me_bx_->Fill(bx);
      me_occ_zr_[region_id]->Fill(digi_g_abs_z, digi_g_r),
  ...
  for (const auto& simhit : *simhit_container.product()) {
    ...
    if (gem->idToDet(simhit.detUnitId()) == nullptr) {
      edm::LogError(kLogCategory_)
          << "SimHit did not match with GEMGeometry." << std::endl;
      continue;
    }
