...
MuonGEMDigisHarvestor::MuonGEMDigisHarvestor(const edm::ParameterSet &ps) {
  ...
  compareable_dbe_hist_prefix_ =
      ps.getParameter<std::string>("compareDBEHistPrefix");
...
TProfile *MuonGEMDigisHarvestor::ComputeEff(TH1F *num, TH1F *denum) {
  ...
  for (int i = 1; i <= denum->GetNbinsX(); i++) {
    ...
    if (nNum > nDenum) {
      ...
      nDenum = nNum;
      nNum = temp;
      LogDebug("MuonGEMDigisHarvestor")
...
void MuonGEMDigisHarvestor::ProcessBooking(DQMStore::IBooker &ibooker,
  ...
  if (ig.get(dbe_label.Data()) != nullptr && sh_hist != nullptr &&
    ...
    TString title = TString::Format(
        "Eff. for a SimTrack to have an associated GEM digi in %s;%s;Eff.",
        suffix.Data(), x_axis_title.Data());
    TString title2 =
        TString::Format("Eff. for a SimTrack to have an associated GEM digi in "
  ...
  for (unsigned int i = 0; i < s_suffix.size(); i++) {
    ...
    if (ig.get(eta_label.Data()) != nullptr &&
      ...
      for (unsigned int j = 0; j < l_suffix.size(); j++) {
        ...
        ProcessBooking(ibooker, ig, "dg_eta", suffix, gem_trk_eta[i],
                       sh_eta[i][j]);
        ProcessBooking(ibooker, ig, "pad_eta", suffix, gem_trk_eta[i],
                       sh_eta[i][j]);
        ProcessBooking(ibooker, ig, "copad_eta", suffix, gem_trk_eta[i],
                       sh_eta[i][j]);
        for (unsigned int k = 0; k < c_suffix.size(); k++) {
          ...
          ProcessBooking(ibooker, ig, "dg_phi", suffix, gem_trk_phi[i][k],
                         sh_phi[i][j][k]);
          ProcessBooking(ibooker, ig, "pad_phi", suffix, gem_trk_phi[i][k],
                         sh_phi[i][j][k]);
          ProcessBooking(ibooker, ig, "copad_phi", suffix, gem_trk_phi[i][k],
                         sh_phi[i][j][k]);
        }
      }
    } else
