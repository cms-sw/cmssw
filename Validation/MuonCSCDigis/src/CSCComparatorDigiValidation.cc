...
CSCComparatorDigiValidation::CSCComparatorDigiValidation(
    ...
    : CSCBaseValidation(inputTag),
      theStripDigi_Token_(
          iC.consumes<CSCStripDigiCollection>(stripDigiInputTag)),
      theTimeBinPlots(), theNDigisPerLayerPlots(), theStripDigiPlots(),
      the3StripPlots() {
...
void CSCComparatorDigiValidation::bookHistograms(DQMStore::IBooker &iBooker) {
  theNDigisPerEventPlot =
      iBooker.book1D("CSCComparatorDigisPerEvent",
...
void CSCComparatorDigiValidation::analyze(const edm::Event &e,
  ...
  if (!comparators.isValid()) {
    edm::LogError("CSCDigiDump")
        << "Cannot get comparators by label " << theInputTag.encode();
  }
  ...
  for (CSCComparatorDigiCollection::DigiRangeIterator j = comparators->begin();
    ...
    for (; digiItr != last; ++digiItr) {
      ...
      for (std::vector<CSCStripDigi>::const_iterator stripItr =
               stripRange.first;
        ...
        if (stripItr->getStrip() == strip) {
          ...
          // check neighbors
          if (stripItr != stripRange.first &&
              stripItr != stripRange.second - 1) {
