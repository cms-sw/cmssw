...
# need modification in the future. In particular, when running with CMSSW versions that are not 38X, replace
# "CMSSW_3_8_x" in the https address with the correct version.
...
cat temp.tmp | sed -e 's%|/%mytempstring%g' | sed -e 's/|//g' | sed -e s/" 9000 "//g | sed -e s/" 0 "//g | sed -e s/True//g | sed -e 's%/%__%g' | sed -e 's%mytempstring%wget --ca-directory $X509_CERT_DIR/ --certificate=$X509_USER_PROXY --private-key=$X509_USER_PROXY https://cmsweb.cern.ch/dqm/offline/data/browse/ROOT/RelVal/CMSSW_3_8_x/DQM_V0001_R000000001__%g' | sed -e 's/GEN-SIM-RECO/GEN-SIM-RECO.root/g' > getDQMFiles_${label}.csh
