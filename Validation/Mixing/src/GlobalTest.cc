...
// $Id: TestSuite.cc,v 1.6 2006/06/27 09:13:42 uberthon Exp $
//
//
...
GlobalTest::GlobalTest(const edm::ParameterSet& iConfig): filename_(iConfig.getParameter<std::string>("fileName")), minbunch_(iConfig.getParameter<int>("minBunch")),maxbunch_(iConfig.getParameter<int>("maxBunch"))
{
  ...
  //book histos
    ...
    for (int i=minbunch_;i<=maxbunch_;++i) {
      ...
      nrPileupsH_[ii]    = new TH1I(label,label,100,0,100);
      sprintf(label,"%s_%d",labels[1],i);
      nrVerticesH_[ii]   = new TH1I(label,label,100,0,5000);
      sprintf(label,"%s_%d",labels[2],i);
      nrTracksH_[ii]     = new TH1I(label,label,100,0,10000);
      sprintf(label,"%s_%d",labels[3],i);
      trackPartIdH_[ii]  =  new TH1I(label,label,100,0,100);
      sprintf(label,"%s_%d",labels[4],i);
      caloEnergyEBH_ [ii]  = new TH1F(label,label,100,0.,1000.);
      sprintf(label,"%s_%d",labels[5],i);
      caloEnergyEEH_ [ii]  = new TH1F(label,label,100,0.,1000.);
    }
}
