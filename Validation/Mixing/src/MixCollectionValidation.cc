...
MixCollectionValidation::MixCollectionValidation(
  ...
  ParameterSet mixObjextsSet_ =
      iConfig.getParameter<ParameterSet>("mixObjects");
}
...
void MixCollectionValidation::bookHistograms(
  ...
  for (std::vector<std::string>::iterator it = names.begin(); it != names.end();
    ...
    if (object == "HepMCProduct") {
      ...
      std::string name = "NumberOf" + object;
      nrSimTrackH_ = iBooker.bookProfile(name, title, nbin_, minbunch_,
                                         maxbunch_ + 1, 40, 0., 40.);
      ...
      std::string title = "Log10 Number of " + object;
      std::string name = "NumberOf" + object;
      nrSimVertexH_ = iBooker.bookProfile(name, title, nbin_, minbunch_,
                                          maxbunch_ + 1, 40, 0., 40.);
      ...
      std::vector<std::string> subdets =
          pset.getParameter<std::vector<std::string>>("subdets");
      for (unsigned int ii = 0; ii < subdets.size(); ii++) {

        std::string title = "Log10 Number of " + subdets[ii];
        std::string name = "NumberOf" + subdets[ii];
        SimHitNrmap_[subdets[ii]] = iBooker.bookProfile(
            name, title, nbin_, minbunch_, maxbunch_ + 1, 40, 0., 40.);

        title = "Time of " + subdets[ii];
        name = "TimeOf" + subdets[ii];
        SimHitTimemap_[subdets[ii]] = iBooker.bookProfile(
            name, title, nbin_, minbunch_, maxbunch_ + 1, 40, -125., 375.);
      }

      PSimHitTags_ = tags;
      for (auto const &it : PSimHitTags_)
        crossingFrame_PSimHit_Tokens_.push_back(
            consumes<CrossingFrame<PSimHit>>(
      ...
      for (unsigned int ii = 0; ii < subdets.size(); ii++) {
        ...
        std::string title = "Log10 Number of " + subdets[ii];
        std::string name = "NumberOf" + subdets[ii];
        CaloHitNrmap_[subdets[ii]] = iBooker.bookProfile(
            name, title, nbin_, minbunch_, maxbunch_ + 1, 40, 0., 40.);

        title = "Time of " + subdets[ii];
        name = "TimeOf" + subdets[ii];
        CaloHitTimemap_[subdets[ii]] = iBooker.bookProfile(
            name, title, nbin_, minbunch_, maxbunch_ + 1, 40, -125., 375.);
      }
      ...
      for (auto const &it : PCaloHitTags_)
        crossingFrame_PCaloHit_Tokens_.push_back(
            consumes<CrossingFrame<PCaloHit>>(
                edm::InputTag("mix", it.label() + it.instance())));
    }
  }
}
...
void MixCollectionValidation::analyze(const edm::Event &iEvent,
  ...
  if (!HepMCProductTags_.empty()) {
    ...
    gotHepMCProduct =
        iEvent.getByToken(crossingFrame_Hep_Token_, crossingFrame);
  ...
  if (!SimTrackTags_.empty()) {
    ...
    if (gotSimTrack) {
      std::unique_ptr<MixCollection<SimTrack>> simTracks(
          new MixCollection<SimTrack>(crossingFrame.product()));
  ...
  if (!SimVertexTags_.empty()) {
    ...
    gotSimVertex =
        iEvent.getByToken(crossingFrame_SimVtx_Token_, crossingFrame);

    if (gotSimVertex) {
      std::unique_ptr<MixCollection<SimVertex>> simVerteces(
          new MixCollection<SimVertex>(crossingFrame.product()));
  ...
  if (!PSimHitTags_.empty()) {
    ...
    for (int i = 0; i < (int)PSimHitTags_.size(); i++) {
      bool gotPSimHit;
      gotPSimHit =
          iEvent.getByToken(crossingFrame_PSimHit_Tokens_[i], crossingFrame);

      if (gotPSimHit) {
        std::unique_ptr<MixCollection<PSimHit>> simHits(
            new MixCollection<PSimHit>(crossingFrame.product()));

        MixCollection<PSimHit>::MixItr hitItr;

        fillMultiplicity(hitItr, simHits,
                         SimHitNrmap_[PSimHitTags_[i].instance()]);

        fillSimHitTime(hitItr, simHits,
                       SimHitTimemap_[PSimHitTags_[i].instance()]);
      }
    }
  }
  ...
  if (!PCaloHitTags_.empty()) {
    ...
    for (int i = 0; i < (int)PCaloHitTags_.size(); i++) {
      bool gotPCaloHit;
      std::string PCaloHitLabel =
          PCaloHitTags_[i].label() + PCaloHitTags_[i].instance();
      gotPCaloHit =
          iEvent.getByToken(crossingFrame_PCaloHit_Tokens_[i], crossingFrame);

      if (gotPCaloHit) {
        std::unique_ptr<MixCollection<PCaloHit>> caloHits(
            new MixCollection<PCaloHit>(crossingFrame.product()));

        MixCollection<PCaloHit>::MixItr hitItr;

        fillMultiplicity(hitItr, caloHits,
                         CaloHitNrmap_[PCaloHitTags_[i].instance()]);

        fillCaloHitTime(hitItr, caloHits,
                        CaloHitTimemap_[PCaloHitTags_[i].instance()]);
      }
    }
  }
}
...
void MixCollectionValidation::fillMultiplicity(T1 &theItr_, T2 &theColl_,
  ...
  for (theItr_ = theColl_->begin(); theItr_ != theColl_->end(); ++theItr_) {
    ...
    if (index >= 0 && index < nbin_) {
      ...
      edm::LogWarning("MixCollectionValidation")
          << "fillMultiplicity: bunch number " << bunch << " out of range";
    }
  }

  for (int i = 0; i < nbin_; i++) {
    theProfile_->Fill(float(i + minbunch_ + 0.5),
                      std::log10(std::max(float(0.1), float(theMult[i]))));
  }
}

template <class T1, class T2>
void MixCollectionValidation::fillGenParticleMulti(
    T1 &theItr_, T2 &theColl_, MonitorElement *theProfile_) {
...
template <class T1, class T2>
void MixCollectionValidation::fillSimHitTime(T1 &theItr_, T2 &theColl_,
                                             MonitorElement *theProfile_) {
  ...
  for (theItr_ = theColl_->begin(); theItr_ != theColl_->end(); ++theItr_) {
    ...
    if (index >= 0 && index < nbin_) {
      ...
      edm::LogWarning("MixCollectionValidation")
          << "fillSimHitTime: bunch number " << bunch << " out of range";
    }
  }
}

template <class T1, class T2>
void MixCollectionValidation::fillCaloHitTime(T1 &theItr_, T2 &theColl_,
                                              MonitorElement *theProfile_) {
  ...
  for (theItr_ = theColl_->begin(); theItr_ != theColl_->end(); ++theItr_) {
    ...
    if (index >= 0 && index < nbin_) {
      ...
      edm::LogWarning("MixCollectionValidation")
          << "fillCaloHitTime: bunch number " << bunch << " out of range";
    }
  }
}
