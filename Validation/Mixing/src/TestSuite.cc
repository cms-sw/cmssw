...
TestSuite::TestSuite(const edm::ParameterSet &iConfig)
    : filename_(iConfig.getParameter<std::string>("fileName")),
      ...
      maxbunch_(iConfig.getParameter<int>("maxBunch")), dbe_(nullptr),
...
void TestSuite::analyze(const edm::Event &iEvent,
  ...
  // track histo
  char histotracks[30], sighistotracks[30], histotracksindsig[30],
      histotracksind[30];
  ...
  MonitorElement *trhist =
      dbe_->book1D(histotracks, "Bunchcrossings", maxbunch_ - minbunch_ + 1,
                   minbunch_, maxbunch_ + 1);
  MonitorElement *trhistsig =
      dbe_->book1D(sighistotracks, "Bunchcrossings", maxbunch_ - minbunch_ + 1,
                   minbunch_, maxbunch_ + 1);
  MonitorElement *trindhist =
      dbe_->book1D(histotracksind, "Track to Vertex indices", 100, 0, 500);
  MonitorElement *trindhistsig = dbe_->book1D(
      histotracksindsig, "Signal Track to Vertex indices", 100, 0, 500);
  std::unique_ptr<MixCollection<SimTrack>> col1(
  ...
  // vertex histo
  char histovertices[30], sighistovertices[30], histovertexindices[30],
      histovertexindicessig[30];
  ...
  MonitorElement *vtxhist =
      dbe_->book1D(histovertices, "Bunchcrossings", maxbunch_ - minbunch_ + 1,
                   minbunch_, maxbunch_ + 1);
  MonitorElement *vtxhistsig =
      dbe_->book1D(sighistovertices, "Bunchcrossings",
                   maxbunch_ - minbunch_ + 1, minbunch_, maxbunch_ + 1);
  MonitorElement *vtxindhist =
      dbe_->book1D(histovertexindices, "Vertex to Track Indices", 100, 0, 300);
  MonitorElement *vtxindhistsig = dbe_->book1D(
      histovertexindicessig, "Signal Vertex to Track Indices", 100, 0, 300);
  std::unique_ptr<MixCollection<SimVertex>> col2(
      new MixCollection<SimVertex>(cf_vertex.product()));
  ...
  MonitorElement *tofhist =
      dbe_->book1D(tof, "TrackerHit_ToF", 100, float(bsp * minbunch_),
                   float(bsp * maxbunch_) + 50.);
  sprintf(tof, "SignalTrackerHit_Tof_bcr_%d", bunchcr_);
  MonitorElement *tofhist_sig =
      dbe_->book1D(tof, "TrackerHit_ToF", 100, float(bsp * minbunch_),
                   float(bsp * maxbunch_) + 50.);
  std::unique_ptr<MixCollection<PSimHit>> colsh(
      new MixCollection<PSimHit>(cf_simhit.product()));
  ...
  MonitorElement *tofecalhist =
      dbe_->book1D(tof, "EcalEBHit_ToF", 100, float(bsp * minbunch_),
                   float(bsp * maxbunch_) + 50.);
  ...
  std::unique_ptr<MixCollection<PCaloHit>> colecal(
      new MixCollection<PCaloHit>(cf_calohitEcal.product()));
  ...
  MonitorElement *tofhcalhist =
      dbe_->book1D(tof, "HcalHit_ToF", 100, float(bsp * minbunch_),
                   float(bsp * maxbunch_) + 50.);
  ...
  std::unique_ptr<MixCollection<PCaloHit>> colhcal(
      new MixCollection<PCaloHit>(cf_calohitHcal.product()));
