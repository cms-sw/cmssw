...
ContainmentCorrectionAnalyzer::ContainmentCorrectionAnalyzer(
  ...
  BarrelSuperClusterCollection_ = consumes<reco::SuperClusterCollection>(
      ps.getParameter<InputTag>("BarrelSuperClusterCollection"));
  EndcapSuperClusterCollection_ = consumes<reco::SuperClusterCollection>(
      ps.getParameter<InputTag>("EndcapSuperClusterCollection"));
  reducedBarrelRecHitCollection_ = consumes<EcalRecHitCollection>(
      ps.getParameter<InputTag>("reducedBarrelRecHitCollection"));
  reducedEndcapRecHitCollection_ = consumes<EcalRecHitCollection>(
      ps.getParameter<InputTag>("reducedEndcapRecHitCollection"));
  SimTrackCollection_ = consumes<SimTrackContainer>(
      ps.getParameter<InputTag>("simTrackCollection"));
  SimVertexCollection_ = consumes<SimVertexContainer>(
      ps.getParameter<InputTag>("simVertexCollection"));
}
...
void ContainmentCorrectionAnalyzer::beginJob() {
  ...
  h_EB_eRecoEtrueReference = fs->make<TH1F>(
      "EB_eRecoEtrueReference", "EB_eRecoEtrueReference", 440, 0., 1.1);
  h_EB_e9EtrueReference = fs->make<TH1F>("EB_e9EtrueReference",
                                         "EB_e9EtrueReference", 440, 0., 1.1);
  h_EB_e25EtrueReference = fs->make<TH1F>("EB_e25EtrueReference",
                                          "EB_e25EtrueReference", 440, 0., 1.1);
  h_EE_eRecoEtrueReference = fs->make<TH1F>(
      "EE_eRecoEtrueReference", "EE_eRecoEtrueReference", 440, 0., 1.1);
  h_EE_e9EtrueReference = fs->make<TH1F>("EE_e9EtrueReference",
                                         "EE_e9EtrueReference", 440, 0., 1.1);
  h_EE_e25EtrueReference = fs->make<TH1F>("EE_e25EtrueReference",
...
void ContainmentCorrectionAnalyzer::analyze(const Event &evt,
                                            const EventSetup &es) {

  LogInfo("ContainmentCorrectionAnalyzer")
      << "Analyzing event " << evt.id() << "\n";
  ...
  if (SimTk.isValid())
    ...
    LogError("ContainmentCorrectionAnalyzer")
  ...
  std::vector<EcalSimPhotonMCTruth> photons =
  ...
  for (unsigned int ipho = 0; ipho < photons.size(); ipho++) {
    ...
    double etaTrue =
        ecalEta(photons[ipho].fourMomentum().eta(), vtx.z(), vtxPerp);
    ...
    if (std::fabs(etaTrue) < 1.479) {
      ...
      for (reco::SuperClusterCollection::const_iterator aClus =
        ...
        double deltaR = std::sqrt(std::pow(etaCurrent - etaTrue, 2) +
                                  std::pow(phiCurrent - phiTrue, 2));
      ...
      if (closestParticleDistance < 0.3) {
        ...
        superClusterEt[nRECOphotons - 1] =
            nearSC->rawEnergy() / std::cosh(nearSC->position().eta());
        iMC[nRECOphotons - 1] = nMCphotons - 1;

        const reco::CaloClusterPtr &theSeed = nearSC->seed();
        seedXtal[nRECOphotons - 1] =
            EcalClusterTools::getMaximum(*theSeed, ebRecHits).first;
        e1[nRECOphotons - 1] = EcalClusterTools::eMax(*theSeed, ebRecHits);
        e9[nRECOphotons - 1] =
            EcalClusterTools::e3x3(*theSeed, ebRecHits, topology);
        e25[nRECOphotons - 1] =
            EcalClusterTools::e5x5(*theSeed, ebRecHits, topology);
      }
    }
    ...
    if (std::fabs(etaTrue) >= 1.6) {
      ...
      if (closestParticleDistance < 0.3) {
        ...
        superClusterEta[nRECOphotons - 1] = nearSC->position().eta();
        superClusterPhi[nRECOphotons - 1] = nearSC->position().phi();
        superClusterEt[nRECOphotons - 1] = (nearSC->rawEnergy() + psEnergy) /
                                           std::cosh(nearSC->position().eta());
        iMC[nRECOphotons - 1] = nMCphotons - 1;

        const reco::CaloClusterPtr &theSeed = nearSC->seed();
        seedXtal[nRECOphotons - 1] =
            EcalClusterTools::getMaximum(*theSeed, eeRecHits).first;
        e1[nRECOphotons - 1] =
            EcalClusterTools::eMax(*theSeed, eeRecHits) + psEnergy;
        e9[nRECOphotons - 1] =
            EcalClusterTools::e3x3(*theSeed, eeRecHits, topology) + psEnergy;
        e25[nRECOphotons - 1] =
            EcalClusterTools::e5x5(*theSeed, eeRecHits, topology) + psEnergy;
      }
    }
  }
  ...
  for (int i = 0; i < nRECOphotons; i++) {
    ...
    if (fabs(superClusterEta[i]) < 1.479) {
      if (isConverted[iMC[i]] != 1) {
        ...
        if (ietaAbs > 5 && ietaAbs < 21 && ((iphi % 20) > 5) &&
            ((iphi % 20) < 16)) {
          h_EB_eRecoEtrueReference->Fill(superClusterEnergy[i] /
                                         mcEnergy[iMC[i]]);
      ...
      if (isConverted[iMC[i]] != 1) {
        if (fabs(superClusterEta[i]) > 1.7 && fabs(superClusterEta[i] < 2.3) &&
            ((superClusterPhi[i] > -CLHEP::pi / 2. + 0.1 &&
             ...
             (superClusterPhi[i] > CLHEP::pi / 2. + 0.1) ||
             (superClusterPhi[i] < -CLHEP::pi / 2. - 0.1))) {
          h_EE_eRecoEtrueReference->Fill(superClusterEnergy[i] /
                                         mcEnergy[iMC[i]]);
...
float ContainmentCorrectionAnalyzer::ecalEta(float EtaParticle, float Zvertex,
                                             float plane_Radius) {
...
std::vector<EcalSimPhotonMCTruth> ContainmentCorrectionAnalyzer::findMcTruth(
    std::vector<SimTrack> &theSimTracks,
  ...
  if (npv > 4) { // ievtype = PYTHIA; // UNUSED
    ...
    if (abs(partType1) == 11 &&
        abs(partType2) == 11) { /* ievtype = DOUBLE; ==UNUSED== */
      ievflav = ELECTRON_FLAV;
    } else if (partType1 == 111 &&
               partType2 == 111) { /* ievtype = DOUBLE; ==UNUSED== */
      ievflav = PIZERO_FLAV;
    } else if (partType1 == 22 &&
               partType2 == 22) { /* ievtype = DOUBLE; ==UNUSED== */
      ievflav = PHOTON_FLAV;
    }
  }
  ...
  if (ievflav == PHOTON_FLAV) {
    ...
    int iConv = 0;
    iPho = 0;
    for (std::vector<SimTrack *>::iterator iPhoTk = photonTracks.begin();
         iPhoTk != photonTracks.end(); ++iPhoTk) {
      ...
      if (!trkFromConversion.empty()) {
        ...
        result.push_back(
            EcalSimPhotonMCTruth(isAconversion, (*iPhoTk)->momentum(),
                                 vtxPosition.pt(), vtxPosition.z(), vtxPosition,
                                 primVtx.position(), trkFromConversion));
      } else {
        isAconversion = 0;
...
void ContainmentCorrectionAnalyzer::fillMcTruth(
