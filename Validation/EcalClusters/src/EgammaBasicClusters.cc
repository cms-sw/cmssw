...
void EgammaBasicClusters::beginJob(edm::EventSetup const&) 
{
  ...
  if ( verboseDBE_ )
	...
	dbe_->setCurrentFolder("EcalClusters/BasicClusters");
  ...
  hist_IslandEE_BC_Size_			 = dbe_->book1D("hist_IslandEE_BC_Size_","# Basic Clusters from Island in Endcap",hist_bins_Size_,hist_min_Size_,hist_max_Size_);

	hist_HybridEB_BC_NumRecHits_ = dbe_->book1D("hist_HybridEB_BC_NumRecHits_","# of RecHits in Basic Clusters from Hybrid in Barrel",hist_bins_NumRecHits_,hist_min_NumRecHits_,hist_max_NumRecHits_);
  hist_IslandEB_BC_NumRecHits_ = dbe_->book1D("hist_IslandEB_BC_NumRecHits_","# of RecHits in Basic Clusters from Island in Barrel",hist_bins_NumRecHits_,hist_min_NumRecHits_,hist_max_NumRecHits_);
  hist_IslandEE_BC_NumRecHits_ = dbe_->book1D("hist_IslandEE_BC_NumRecHits_","# of RecHits in Basic Clusters from Island in Endcap",hist_bins_NumRecHits_,hist_min_NumRecHits_,hist_max_NumRecHits_);

  hist_HybridEB_BC_ET_ 				 = dbe_->book1D("hist_HybridEB_BC_ET_","ET of Basic Clusters with Hybrid in Barrel",hist_bins_ET_,hist_min_ET_,hist_max_ET_);
  hist_IslandEB_BC_ET_				 = dbe_->book1D("hist_IslandEB_BC_ET_","ET of Basic Clusters with Island in Barrel",hist_bins_ET_,hist_min_ET_,hist_max_ET_);
  hist_IslandEE_BC_ET_ 			   = dbe_->book1D("hist_IslandEE_BC_ET_","ET of Basic Clusters with Island in Endcap",hist_bins_ET_,hist_min_ET_,hist_max_ET_);

  hist_HybridEB_BC_Eta_			   = dbe_->book1D("hist_HybridEB_BC_Eta_","Eta of Basic Clusters with Hybrid in Barrel",hist_bins_Eta_,hist_min_Eta_,hist_max_Eta_);
  hist_IslandEB_BC_Eta_				 = dbe_->book1D("hist_IslandEB_BC_Eta_","Eta of Basic Clusters with Island in Barrel",hist_bins_Eta_,hist_min_Eta_,hist_max_Eta_);
  hist_IslandEE_BC_Eta_ 			 = dbe_->book1D("hist_IslandEE_BC_Eta_","Eta of Basic Clusters with Island in Endcap",hist_bins_Eta_,hist_min_Eta_,hist_max_Eta_);

  hist_HybridEB_BC_Phi_			   = dbe_->book1D("hist_HybridEB_BC_Phi_","Phi of Basic Clusters with Hybrid in Barrel",hist_bins_Phi_,hist_min_Phi_,hist_max_Phi_);
  hist_IslandEB_BC_Phi_				 = dbe_->book1D("hist_IslandEB_BC_Phi_","Phi of Basic Clusters with Island in Barrel",hist_bins_Phi_,hist_min_Phi_,hist_max_Phi_);
  hist_IslandEE_BC_Phi_ 			 = dbe_->book1D("hist_IslandEE_BC_Phi_","Phi of Basic Clusters with Island in Endcap",hist_bins_Phi_,hist_min_Phi_,hist_max_Phi_);
}
...
void EgammaBasicClusters::analyze( const edm::Event& evt, const edm::EventSetup& es )
{
  ...
  try
	...
	{
		edm::LogError("EgammaBasicClusters") << "Error! can't get collection with label " << hybridBarrelBasicClusterCollection_.label();
  }
  ...
  for(reco::BasicClusterCollection::const_iterator aClus = hybridBarrelBasicClusters->begin(); aClus != hybridBarrelBasicClusters->end(); aClus++)
	{
  ...
  try
	...
	{
		edm::LogError("EgammaBasicClusters") << "Error! can't get collection with label " << islandBarrelBasicClusterCollection_.label();
  }
  ...
  try
	...
	{
		edm::LogError("EgammaBasicClusters") << "Error! can't get collection with label " << islandEndcapBasicClusterCollection_.label();
  }
