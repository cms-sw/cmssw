...
EgammaBasicClusters::EgammaBasicClusters(const edm::ParameterSet &ps)
    : barrelBasicClusterCollection_(consumes<reco::BasicClusterCollection>(
          ps.getParameter<edm::InputTag>("barrelBasicClusterCollection"))),
      endcapBasicClusterCollection_(consumes<reco::BasicClusterCollection>(
          ps.getParameter<edm::InputTag>("endcapBasicClusterCollection"))),
      hsSize_(ps, "Size"), hsNumRecHits_(ps, "NumRecHits"), hsET_(ps, "ET"),
      hsEta_(ps, "Eta"), hsPhi_(ps, "Phi"), hsR_(ps, "R"),
      hist_EB_BC_Size_(nullptr), hist_EE_BC_Size_(nullptr),
      hist_EB_BC_NumRecHits_(nullptr), hist_EE_BC_NumRecHits_(nullptr),
      hist_EB_BC_ET_(nullptr), hist_EE_BC_ET_(nullptr),
      hist_EB_BC_Eta_(nullptr), hist_EE_BC_Eta_(nullptr),
      hist_EB_BC_Phi_(nullptr), hist_EE_BC_Phi_(nullptr),
      hist_EB_BC_ET_vs_Eta_(nullptr), hist_EB_BC_ET_vs_Phi_(nullptr),
      hist_EE_BC_ET_vs_Eta_(nullptr), hist_EE_BC_ET_vs_Phi_(nullptr),
...
void EgammaBasicClusters::bookHistograms(DQMStore::IBooker &_ibooker,
  ...
  hist_EB_BC_NumRecHits_ = _ibooker.book1D(
      "hist_EB_BC_NumRecHits_", "# of RecHits in Basic Clusters in Barrel",
      hsNumRecHits_.bins, hsNumRecHits_.min, hsNumRecHits_.max);
  hist_EE_BC_NumRecHits_ = _ibooker.book1D(
      "hist_EE_BC_NumRecHits_", "# of RecHits in Basic Clusters in Endcap",
      hsNumRecHits_.bins, hsNumRecHits_.min, hsNumRecHits_.max);
  ...
  hist_EB_BC_ET_vs_Eta_ = _ibooker.book2D(
      "hist_EB_BC_ET_vs_Eta_", "Basic Cluster ET versus Eta in Barrel",
      hsET_.bins, hsET_.min, hsET_.max, hsEta_.bins, hsEta_.min, hsEta_.max);

  hist_EB_BC_ET_vs_Phi_ = _ibooker.book2D(
      "hist_EB_BC_ET_vs_Phi_", "Basic Cluster ET versus Phi in Barrel",
      hsET_.bins, hsET_.min, hsET_.max, hsPhi_.bins, hsPhi_.min, hsPhi_.max);

  hist_EE_BC_ET_vs_Eta_ = _ibooker.book2D(
      "hist_EE_BC_ET_vs_Eta_", "Basic Cluster ET versus Eta in Endcap",
      hsET_.bins, hsET_.min, hsET_.max, hsEta_.bins, hsEta_.min, hsEta_.max);

  hist_EE_BC_ET_vs_Phi_ = _ibooker.book2D(
      "hist_EE_BC_ET_vs_Phi_", "Basic Cluster ET versus Phi in Endcap",
      hsET_.bins, hsET_.min, hsET_.max, hsPhi_.bins, hsPhi_.min, hsPhi_.max);

  hist_EE_BC_ET_vs_R_ = _ibooker.book2D(
      "hist_EE_BC_ET_vs_R_", "Basic Cluster ET versus Radius in Endcap",
...
void EgammaBasicClusters::analyze(const edm::Event &evt,
  ...
  if (!pBarrelBasicClusters.isValid()) {
    ...
    edm::LogError("EgammaBasicClusters")
        << "Error! can't get collection with label " << l.module;
  }

  const reco::BasicClusterCollection *barrelBasicClusters =
      pBarrelBasicClusters.product();
  hist_EB_BC_Size_->Fill(barrelBasicClusters->size());

  for (reco::BasicClusterCollection::const_iterator aClus =
           barrelBasicClusters->begin();
    ...
    hist_EB_BC_ET_vs_Eta_->Fill(
        aClus->energy() / std::cosh(aClus->position().eta()), aClus->eta());
    hist_EB_BC_ET_vs_Phi_->Fill(
  ...
  const reco::BasicClusterCollection *endcapBasicClusters =
      pEndcapBasicClusters.product();
  hist_EE_BC_Size_->Fill(endcapBasicClusters->size());

  for (reco::BasicClusterCollection::const_iterator aClus =
           endcapBasicClusters->begin();
    ...
    hist_EE_BC_ET_vs_Eta_->Fill(
        aClus->energy() / std::cosh(aClus->position().eta()), aClus->eta());
    hist_EE_BC_ET_vs_Phi_->Fill(
        aClus->energy() / std::cosh(aClus->position().eta()), aClus->phi());
    hist_EE_BC_ET_vs_R_->Fill(
