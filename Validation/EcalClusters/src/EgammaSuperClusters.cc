...
EgammaSuperClusters::EgammaSuperClusters(const edm::ParameterSet &ps)
    : enableEndcaps_{ps.getParameter<bool>("enableEndcaps")},
      ...
      endcapRawSuperClusterCollectionToken_{enableEndcaps_
                                                ? consumes<reco::SuperClusterCollection>(
                                                      ps.getParameter<edm::InputTag>("endcapRawSuperClusterCollection"))
                                                : edm::EDGetTokenT<reco::SuperClusterCollection>{}},
      endcapPreSuperClusterCollectionToken_{enableEndcaps_
                                                ? consumes<reco::SuperClusterCollection>(
                                                      ps.getParameter<edm::InputTag>("endcapPreSuperClusterCollection"))
                                                : edm::EDGetTokenT<reco::SuperClusterCollection>{}},
      endcapCorSuperClusterCollectionToken_{enableEndcaps_
                                                ? consumes<reco::SuperClusterCollection>(
                                                      ps.getParameter<edm::InputTag>("endcapCorSuperClusterCollection"))
                                                : edm::EDGetTokenT<reco::SuperClusterCollection>{}},
      ...
      endcapRecHitCollectionToken_{
          enableEndcaps_ ? consumes<EcalRecHitCollection>(ps.getParameter<edm::InputTag>("endcapRecHitCollection"))
      ...
      hspreshowerE_{enableEndcaps_ ? HistSpec(ps, "preshowerE") : HistSpec()},
      hsR_(ps, "R"),
...
void EgammaSuperClusters::fillDescriptions(edm::ConfigurationDescriptions &confDesc) {
  edm::ParameterSetDescription desc;

  desc.add<edm::InputTag>("MCTruthCollection", edm::InputTag("generatorSmeared"));
  desc.add<edm::InputTag>("barrelRecHitCollection", edm::InputTag("reducedEcalRecHitsEB"));
  desc.add<edm::InputTag>("barrelRawSuperClusterCollection", edm::InputTag("hybridSuperClusters"));
  desc.add<edm::InputTag>("barrelCorSuperClusterCollection", edm::InputTag("correctedHybridSuperClusters"));
  desc.ifValue(
      edm::ParameterDescription<bool>("enableEndcaps", true, true),
      true >> (edm::ParameterDescription<edm::InputTag>(
                   "endcapRecHitCollection", edm::InputTag("reducedEcalRecHitsEE"), true) and
               edm::ParameterDescription<edm::InputTag>(
                   ...
                   true) and
               edm::ParameterDescription<edm::InputTag>("endcapCorSuperClusterCollection",
                                                        edm::InputTag("correctedMulti5x5SuperClustersWithPreshower"),
                                                        true) and
               edm::ParameterDescription<edm::InputTag>(
                   "endcapPreSuperClusterCollection", edm::InputTag("multi5x5SuperClustersWithPreshower"), true) and
               edm::ParameterDescription<int>("hist_bins_preshowerE", 100, true) and
               edm::ParameterDescription<double>("hist_min_preshowerE", 0., true) and
               edm::ParameterDescription<double>("hist_max_preshowerE", 100., true)) or
          false >> edm::EmptyGroupDescription());
  desc.add<double>("hist_max_Size", 5.);
  desc.add<double>("hist_min_Size", 0.);
  desc.add<int>("hist_bins_Size", 5);
  desc.add<int>("hist_bins_deltaR", 50);
  desc.add<double>("hist_min_deltaR", 0.);
  desc.add<double>("hist_max_deltaR", 0.5);
  desc.add<int>("hist_bins_etaWidth", 100);
  desc.add<double>("hist_min_etaWidth", 0.);
  desc.add<double>("hist_max_etaWidth", 0.1);
  desc.add<int>("hist_bins_Eta", 100);
  desc.add<double>("hist_min_Eta", -3.);
  desc.add<double>("hist_max_Eta", 3.);
  desc.add<int>("hist_bins_EoverTruth", 100);
  desc.add<double>("hist_min_EoverTruth", 0.5);
  desc.add<double>("hist_max_EoverTruth", 1.5);
  desc.add<int>("hist_bins_S25toE", 50);
  desc.add<double>("hist_min_S25toE", 0.);
  desc.add<double>("hist_max_S25toE", 1.);
  desc.add<int>("hist_bins_phiWidth", 100);
  desc.add<double>("hist_min_phiWidth", 0.);
  desc.add<double>("hist_max_phiWidth", 0.1);
  desc.add<int>("hist_bins_Phi", 181);
  desc.add<double>("hist_min_Phi", -3.14159);
  desc.add<double>("hist_max_Phi", 3.14159);
  desc.add<int>("hist_bins_S1toS9", 50);
  desc.add<double>("hist_min_S1toS9", 0.);
  desc.add<double>("hist_max_S1toS9", 1.);
  desc.add<int>("hist_bins_ET", 100);
  desc.add<double>("hist_min_ET", 0.);
  desc.add<double>("hist_max_ET", 50.);
  desc.add<int>("hist_bins_NumBC", 10);
  desc.add<double>("hist_min_NumBC", 0.);
  desc.add<double>("hist_max_NumBC", 10.);
  desc.add<int>("hist_bins_R", 55);
  desc.add<double>("hist_min_R", 0.);
  desc.add<double>("hist_max_R", 175.);
  confDesc.add("egammaSuperClusterAnalyzer", desc);
}
...
void EgammaSuperClusters::bookHistograms(DQMStore::IBooker &_ibooker, edm::Run const &, edm::EventSetup const &) {
  ...
  if (enableEndcaps_) {
    ...
    hist_EE_CorSC_ET_ = _ibooker.book1D(
        "hist_EE_CorSC_ET_", "ET of Corrected SuperClusters in Endcap", hsET_.bins, hsET_.min, hsET_.max);
  ...
  if (enableEndcaps_) {
    hist_EE_RawSC_Eta_ = _ibooker.book1D(
        "hist_EE_RawSC_Eta_", "Eta of Raw SuperClusters in Endcap", hsEta_.bins, hsEta_.min, hsEta_.max);
  ...
  if (enableEndcaps_) {
    hist_EE_RawSC_Phi_ = _ibooker.book1D(
        "hist_EE_RawSC_Phi_", "Phi of Raw SuperClusters in Endcap", hsPhi_.bins, hsPhi_.min, hsPhi_.max);
...
void EgammaSuperClusters::analyze(const edm::Event &evt, const edm::EventSetup &es) {
  ...
  if (!pBarrelRecHitCollection.isValid()) {
    edm::LogError("EgammaSuperClusters") << "Error! can't get EB RecHits collection";
    skipBarrel = true;
  }
  if (enableEndcaps_) {
    ...
    if (!pEndcapRecHitCollection.isValid()) {
      edm::LogError("EgammaSuperClusters") << "Error! can't get EE RecHits collection";
      skipEndcap = true;
    }
  }
  ...
  EcalClusterLazyTools lazyTool =
      enableEndcaps_
          ...
          : EcalClusterLazyTools(evt, ecalClusterToolsESGetTokens_.get(es), barrelRecHitCollectionToken_);
