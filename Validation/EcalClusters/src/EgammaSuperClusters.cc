...
void EgammaSuperClusters::beginJob(edm::EventSetup const&) 
{
  ...
  if ( verboseDBE_ )
	...
	hist_HybridEB_SC_Size_			 = dbe_->book1D("hist_HybridEB_SC_Size_","# Super Clusters from Hybrid in Barrel",hist_bins_Size_,hist_min_Size_,hist_max_Size_);
  hist_IslandEB_SC_Size_			 = dbe_->book1D("hist_IslandEB_SC_Size_","# Super Clusters from Island in Barrel",hist_bins_Size_,hist_min_Size_,hist_max_Size_);
  hist_IslandEE_SC_Size_			 = dbe_->book1D("hist_IslandEE_SC_Size_","# Super Clusters from Island in Endcap",hist_bins_Size_,hist_min_Size_,hist_max_Size_);

	hist_HybridEB_SC_NumBC_ 		 = dbe_->book1D("hist_HybridEB_SC_NumBC_","# of Basic Clusters in Super Clusters from Hybrid in Barrel",hist_bins_NumBC_,hist_min_NumBC_,hist_max_NumBC_);
  hist_IslandEB_SC_NumBC_ 		 = dbe_->book1D("hist_IslandEB_SC_NumBC_","# of Basic Clusters in Super Clusters from Island in Barrel",hist_bins_NumBC_,hist_min_NumBC_,hist_max_NumBC_);
  hist_IslandEE_SC_NumBC_ 		 = dbe_->book1D("hist_IslandEE_SC_NumBC_","# of Basic Clusters in Super Clusters from Island in Endcap",hist_bins_NumBC_,hist_min_NumBC_,hist_max_NumBC_);

  hist_HybridEB_SC_ET_ 				 = dbe_->book1D("hist_HybridEB_SC_ET_","ET of Super Clusters with Hybrid in Barrel",hist_bins_ET_,hist_min_ET_,hist_max_ET_);
  hist_IslandEB_SC_ET_				 = dbe_->book1D("hist_IslandEB_SC_ET_","ET of Super Clusters with Island in Barrel",hist_bins_ET_,hist_min_ET_,hist_max_ET_);
  hist_IslandEE_SC_ET_ 			   = dbe_->book1D("hist_IslandEE_SC_ET_","ET of Super Clusters with Island in Endcap",hist_bins_ET_,hist_min_ET_,hist_max_ET_);

  hist_HybridEB_SC_Eta_			   = dbe_->book1D("hist_HybridEB_SC_Eta_","Eta of Super Clusters with Hybrid in Barrel",hist_bins_Eta_,hist_min_Eta_,hist_max_Eta_);
  hist_IslandEB_SC_Eta_				 = dbe_->book1D("hist_IslandEB_SC_Eta_","Eta of Super Clusters with Island in Barrel",hist_bins_Eta_,hist_min_Eta_,hist_max_Eta_);
  hist_IslandEE_SC_Eta_ 			 = dbe_->book1D("hist_IslandEE_SC_Eta_","Eta of Super Clusters with Island in Endcap",hist_bins_Eta_,hist_min_Eta_,hist_max_Eta_);

  hist_HybridEB_SC_Phi_			   = dbe_->book1D("hist_HybridEB_SC_Phi_","Phi of Super Clusters with Hybrid in Barrel",hist_bins_Phi_,hist_min_Phi_,hist_max_Phi_);
  hist_IslandEB_SC_Phi_				 = dbe_->book1D("hist_IslandEB_SC_Phi_","Phi of Super Clusters with Island in Barrel",hist_bins_Phi_,hist_min_Phi_,hist_max_Phi_);
  hist_IslandEE_SC_Phi_ 			 = dbe_->book1D("hist_IslandEE_SC_Phi_","Phi of Super Clusters with Island in Endcap",hist_bins_Phi_,hist_min_Phi_,hist_max_Phi_);

  hist_HybridEB_SC_S1toS9_		 = dbe_->book1D("hist_HybridEB_SC_S1toS9_","S1/S9 of Super Clusters with Hybrid in Barrel",hist_bins_S1toS9_,hist_min_S1toS9_,hist_max_S1toS9_);
  hist_IslandEB_SC_S1toS9_		 = dbe_->book1D("hist_IslandEB_SC_S1toS9_","S1/S9 of Super Clusters with Island in Barrel",hist_bins_S1toS9_,hist_min_S1toS9_,hist_max_S1toS9_);
  hist_IslandEE_SC_S1toS9_ 		 = dbe_->book1D("hist_IslandEE_SC_S1toS9_","S1/S9 of Super Clusters with Island in Endcap",hist_bins_S1toS9_,hist_min_S1toS9_,hist_max_S1toS9_);
...
void EgammaSuperClusters::analyze( const edm::Event& evt, const edm::EventSetup& es )
{
  ...
  try
	...
	{
		edm::LogError("EgammaSuperClusters") << "Error! can't get collection with label " << hybridBarrelSuperClusterCollection_.label();
  }
  ...
  for(reco::SuperClusterCollection::const_iterator aClus = hybridBarrelSuperClusters->begin(); aClus != hybridBarrelSuperClusters->end(); aClus++)
    ...
    hist_HybridEB_SC_ET_ 				->Fill(aClus->energy()*aClus->position().theta());
  ...
  try
	...
	{
		edm::LogError("EgammaSuperClusters") << "Error! can't get collection with label " << islandBarrelSuperClusterCollection_.label();
  }
  ...
  for(reco::SuperClusterCollection::const_iterator aClus = islandBarrelSuperClusters->begin(); aClus != islandBarrelSuperClusters->end(); aClus++)
    ...
    hist_IslandEB_SC_ET_ 				->Fill(aClus->energy()*aClus->position().theta());
  ...
  try
	...
	{
		edm::LogError("EgammaSuperClusters") << "Error! can't get collection with label " << islandEndcapSuperClusterCollection_.label();
  }
  ...
  for(reco::SuperClusterCollection::const_iterator aClus = islandEndcapSuperClusters->begin(); aClus != islandEndcapSuperClusters->end(); aClus++)
    ...
    hist_IslandEE_SC_ET_ 				->Fill(aClus->energy()*aClus->position().theta());
