...
void GEMTrackMatch::FillWithTrigger(
    MonitorElement* hist[4][3][3], bool array[3][2], Float_t eta, Float_t phi, bool odd[3], bool even[3]) {
  for (unsigned int i = 0; i < nstation; i++) {
    int station = i + 1;
    if (odd[i] && eta > getEtaRange(station, 1).first && eta < getEtaRange(station, 1).second) {
      if (array[i][0]) {
        hist[0][i][0]->Fill(phi);
        if (hist[0][i][1] != nullptr)
          hist[0][i][1]->Fill(phi);
      }
      if (array[i][1]) {
        hist[1][i][0]->Fill(phi);
        if (hist[1][i][1] != nullptr)
          hist[1][i][1]->Fill(phi);
      }
      if (array[i][0] || array[i][1]) {
        hist[2][i][0]->Fill(phi);
        if (hist[2][i][1] != nullptr)
          hist[2][i][1]->Fill(phi);
      }
      if (array[i][0] && array[i][1]) {
        hist[3][i][0]->Fill(phi);
        if (hist[3][i][1] != nullptr)
          hist[3][i][1]->Fill(phi);
      }
    }
    if (even[i] && eta > getEtaRange(station, 2).first && eta < getEtaRange(station, 2).second) {
      ...
      if (array[i][0] || array[i][1]) {
        hist[2][i][0]->Fill(phi);
        if (hist[2][i][2] != nullptr)
          hist[2][i][2]->Fill(phi);
      }
      if (array[i][0] && array[i][1]) {
        hist[3][i][0]->Fill(phi);
        if (hist[3][i][2] != nullptr)
          hist[3][i][2]->Fill(phi);
      }
    }
  }
...
bool GEMTrackMatch::isSimTrackGood(const SimTrack& t) {
  ...
  if (t.momentum().pt() < minPt_)
    return false;
  const float eta(std::abs(t.momentum().eta()));
  if (eta > maxEta_ || eta < minEta_)
...
void GEMTrackMatch::buildLUT(const int maxChamberId) {
  ...
  for (auto it : gem_geom_->chambers()) {
    if (it->id().region() > 0) {
      ...
      edm::LogInfo("GEMTrackMatch") << "added id = " << it->id() << " = " << it->id().rawId() << " to pos ids"
                                    << std::endl;
      ...
      edm::LogInfo("GEMTrackMatch") << "added id = " << it->id() << " = " << it->id().rawId() << " to neg ids"
                                    << std::endl;
      const BoundPlane& bSurface(it->surface());
      GlobalPoint gCentre(bSurface.toGlobal(lCentre));
      int cphi(gCentre.phi().degrees());
...
void GEMTrackMatch::setGeometry(const GEMGeometry& geom) {
  ...
  useRoll_ = 1;
  const auto top_chamber = static_cast<const GEMEtaPartition*>(
      geom.idToDetUnit(GEMDetId(chId.region(), chId.ring(), chId.station(), chId.layer(), chId.chamber(), useRoll_)));
  const int nEtaPartitions(gem_geom_->chambers().front()->nEtaPartitions());
  const auto bottom_chamber = static_cast<const GEMEtaPartition*>(geom.idToDetUnit(
...
std::pair<double, double> GEMTrackMatch::getEtaRangeForPhi(int station) {
  ...
  if (station == 0)
    range = std::make_pair(etaRangeForPhi[0], etaRangeForPhi[1]);
  else if (station == 1)
    range = std::make_pair(etaRangeForPhi[2], etaRangeForPhi[3]);
  else if (station == 2)
    range = std::make_pair(etaRangeForPhi[4], etaRangeForPhi[5]);
