...
const GEMGeometry* GEMBaseValidation::initGeometry(edm::EventSetup const& iSetup) {
  ...
  LogDebug("MuonBaseValidation") << "GEMGeometry_->stations().size() "
                                 << GEMGeometry_->regions().front()->stations().size() << "\n";
...
string GEMBaseValidation::getSuffixName(int region, int station, int layer) {
  if (region == -1)
    ...
    std::cout << "Name)Alert! Region must be -1 or 1 : " << region << " " << station << " " << layer << std::endl;
...
string GEMBaseValidation::getSuffixName(int region, int station) {
  if (region == -1)
    ...
    std::cout << "Name)Alert! Region must be -1 or 1 : " << region << " " << station << std::endl;
...
string GEMBaseValidation::getSuffixName(int region) {
  if (region == -1)
    ...
    std::cout << "Name)Alert! Region must be -1 or 1 : " << region << std::endl;
...
string GEMBaseValidation::getSuffixTitle(int region, int station, int layer) {
  if (region == -1)
    ...
    std::cout << "Title)Alert! Region must be -1 or 1 : " << region << " " << station << " " << layer << std::endl;
  return string("Region ") + regionLabel[region] + " Station " + getStationLabel(station) + " Layer " +
         layerLabel[layer - 1];
}
string GEMBaseValidation::getSuffixTitle(int region, int station) {
  if (region == -1)
    ...
    std::cout << "Title)Alert! Region must be -1 or 1 : " << region << " " << station << std::endl;
...
string GEMBaseValidation::getSuffixTitle(int region) {
  if (region == -1)
    ...
    std::cout << "Title)Alert! Region must be -1 or 1 : " << region << std::endl;
...
MonitorElement* GEMBaseValidation::getSimpleZR(DQMStore::IBooker& ibooker, TString title, TString histname) {
  ...
  for (double i = station1_xmin - 1; i < station2_xmax + 1; i = i + 0.25) {
    if (i > station1_xmax + 1 && i < station2_xmin - 1)
      continue;
...
MonitorElement* GEMBaseValidation::BookHistZR(DQMStore::IBooker& ibooker,
  ...
  if (layer_num == 0 || layer_num == 1) {
    ...
    hist_title = label + string(" occupancy : region") + getSuffixTitle(region_num, station_num + 1, layer_num + 1) +
                 " ; globalZ[cm] ; globalR[cm]";
  } else {
    hist_name = name + string("_zr") + getSuffixName(region_num, station_num + 1);
    hist_title = label + string(" occupancy : region") + getSuffixTitle(region_num, station_num + 1) +
                 " ; globalZ[cm] ; globalR[cm]";
  }
...
MonitorElement* GEMBaseValidation::BookHistXY(DQMStore::IBooker& ibooker,
  ...
  if (layer_num == 0 || layer_num == 1) {
    ...
    hist_title = label + string(" occupancy : ") + getSuffixTitle(region_num, station_num + 1, layer_num + 1) +
