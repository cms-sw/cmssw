...
void GEMSimHitValidation::bookHistograms(DQMStore::IBooker& booker,
  ...
  for (const auto& station : gem->regions()[0]->stations()) {
    ...
    me_tof_mu_[station_id] = booker.book1D(
        tof_name, tof_title,
        40, tof_min, tof_max);
  }
      ...
      for (const auto& station : region->stations()) {
        ...
        for (const auto& chamber : super_chamber->chambers()) {
          ...
          me_detail_tof_[key3] = bookHist1D(
              ...
              40, tof_min, tof_max, tof_xtitle, tof_ytitle);
  ...
  TString eloss_ytitle = "Entries / 100 eV";

  for (const auto& station : gem->regions()[0]->stations()) {
    Int_t station_id = station->station();

    auto eloss_name = TString::Format("eloss_muon_st%d", station_id);
    auto eloss_title = TString::Format("SimHit Energy Loss (Muon only) : Station %d", station_id);

    me_eloss_mu_[station_id] = booker.book1D(
        eloss_name, eloss_title + ";" + eloss_xtitle + ";"  + eloss_ytitle,
        60, 0.0, 6000.0);
  }  // station loop
      ...
      for (const auto& station : region->stations()) {
        ...
        for (const auto& chamber : station->superChambers()[0]->chambers()) {
          ...
          me_detail_eloss_[key3] = bookHist1D(
              ...
              60, 0.0, 6000.0, eloss_xtitle, eloss_ytitle);
  ...
  for (const auto& region : gem->regions()) {
    ...
    me_occ_zr_[region_id] = bookZROccupancy(booker, region_id, "simhit",
                                            "SimHit");
    ...
    for (const auto& station : region->stations()) {
      ...
      ME2IdsKey key2{region_id, station_id};
       ...
       me_occ_det_[key2] = bookDetectorOccupancy(booker, key2, station,
                                                 "simhit", "SimHit");
...
void GEMSimHitValidation::analyze(const edm::Event& event,
  ...
  if (not simhit_container.isValid()) {
    edm::LogError(kLogCategory_)
        << "Cannot get GEMHits by Token simhitLabel" << std::endl;
    return;
  }
  ...
  for (const auto& simhit : *simhit_container.product()) {
    ...
    if (gem->idToDet(gemid) == nullptr) {
      edm::LogError(kLogCategory_)
          << "SimHit did not matched with GEM Geometry." << std::endl;
      continue;
    }
    ...
    GlobalPoint&& simhit_global_pos = gem->idToDet(gemid)->surface().toGlobal(
