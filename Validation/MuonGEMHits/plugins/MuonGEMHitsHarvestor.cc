...
MuonGEMHitsHarvestor::dqmEndJob(DQMStore::IBooker & ibooker, DQMStore::IGetter &ig )
{
  ...
  //Simple efficiency.
  if ( ig.get( "sim_dcEta_trk_re-1_st1") != nullptr) {
    for( Int_t region  = -1; region <=1 ; region= region+2) 
      for( Int_t station = 1 ; station <=2 ; station++) {
        TH2F* simHit_trk     = (TH2F*)ig.get(TString::Format("sim_dcEta_trk_re%d_st%d",  region, station).Data())->getTH2F()->Clone();
        TH2F* stripHit_trk   = (TH2F*)ig.get(TString::Format("strip_dcEta_trk_re%d_st%d",region, station).Data())->getTH2F()->Clone();
        TH2F* padHit_trk     = (TH2F*)ig.get(TString::Format("pad_dcEta_trk_re%d_st%d",  region, station).Data())->getTH2F()->Clone();
        TH2F* copadHit_trk   = (TH2F*)ig.get(TString::Format("copad_dcEta_trk_re%d_st%d",region, station).Data())->getTH2F()->Clone();
