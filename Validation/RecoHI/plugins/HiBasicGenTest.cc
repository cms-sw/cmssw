...
void HiBasicGenTest::bookHistograms(DQMStore::IBooker& ibooker, edm::Run const&, edm::EventSetup const&) {
  ///Setting the DQM top directories
  ibooker.setCurrentFolder("Generator/Particles");

  ///Booking the ME's
  for (int ibin = 0; ibin < 3; ibin++) {
    dnchdeta[ibin] = ibooker.book1D(Form("dnchdeta%d", ibin), ";#eta;dN^{ch}/d#eta", 100, -6.0, 6.0);

    dnchdpt[ibin] = ibooker.book1D(Form("dnchdpt%d", ibin), ";p_{T};dN^{ch}/dp_{T}", 200, 0.0, 100.0);
...
void HiBasicGenTest::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  if (hi) {
    ...
    if (ip < 5.045)
      cbin = 0;
    else if (ip < 7.145 && ip > 5.045)
      cbin = 1;
    else if (ip < 15.202 && ip > 14.283)
      cbin = 2;
    if (cbin < 0)
      return;
