...
TGraph* grQCD = new TGraph ( 11, &jetPt[0], &QCDJets[0] );
TGraph* grUDS = new TGraph ( 11, &jetPt[0], &UDSJets[0] );
TGraph* grGluon = new TGraph ( 11, &jetPt[0], &GluonJets[0] );
TGraph* grC = new TGraph ( 11, &jetPt[0], &CJets[0] );
TGraph* grB = new TGraph ( 11, &jetPt[0], &BJets[0] );
...
TH2F *hz = new TH2F("Systematics","", 
		   100, 29., 620., 100, 0.85, 1.02 );
hz->SetTitle( "CMS Preliminary" );
hz->SetXTitle("p_{T} [GeV/c]" );
hz->SetYTitle("Jet Energy scale");
hz->SetStats(0);
hz->Draw();
gPad->SetGridx();
gPad->SetGridy();
