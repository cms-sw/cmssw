...
void ResolutionDirection(bool barrel, const char* input, const char* output, const char* title="") {

  gROOT->Reset();
  TFile *filePF = new TFile(input);
  if(filePF->IsZombie() ) return;
  
  //filePF->cd("DQMData/PFTask/Benchmarks/iterativeCone5PFJets/Gen");
  TH2F* etaPF2 = barrel ? 
    (TH2F*) filePF->Get("DQMData/PFTask/Benchmarks/iterativeCone5PFJets/Gen/BDEtavsPt") : 
    (TH2F*) filePF->Get("DQMData/PFTask/Benchmarks/iterativeCone5PFJets/Gen/EDEtavsPt");
  TH2F* phiPF2 = barrel ? 
    (TH2F*) filePF->Get("DQMData/PFTask/Benchmarks/iterativeCone5PFJets/Gen/BDPhivsPt") : 
    (TH2F*) filePF->Get("DQMData/PFTask/Benchmarks/iterativeCone5PFJets/Gen/EDPhivsPt");
  ...
  //FormatHisto(h,sback);
  if ( barrel ) 
    h->SetTitle( "CMS Preliminary : #eta resolution (barrel)" );
  else
    h->SetTitle( "CMS Preliminary : #eta resolution (end-caps)" );
  ...
  if ( barrel ) 
    ...
    text.DrawLatex(150,0.095,"1.5 < |#eta| < 2.5");
  ...
  if ( barrel ) 
    h2->SetTitle( "CMS Preliminary : #phi Resolution (barrel)" );
  else
    h2->SetTitle( "CMS Preliminary : #phi Resolution (end-caps)" );
