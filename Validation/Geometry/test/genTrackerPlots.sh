...
for geom in {'Extended2015','Extended2017Plan1'}; do
  {   
   python3 -c "from FWCore.PythonFramework.CmsRun import CmsRun" 2>/dev/null && python3 ${VGEO_DIR}/test/runP_Tracker.py geom=$geom label=Tracker >$TEST_DIR/runP_Tracker_cfg.log 2>&1
  }|| 
  { 
  python -c "from FWCore.PythonFramework.CmsRun import CmsRun" 2>/dev/null && python2 ${VGEO_DIR}/test/runP_Tracker.py geom=$geom label=Tracker >$TEST_DIR/runP_Tracker_cfg.log 2>&1
  } 
