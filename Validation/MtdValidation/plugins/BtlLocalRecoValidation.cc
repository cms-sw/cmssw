  ...
  for (const auto& DetSetClu : *btlRecCluHandle) {
	...
	float z_res = global_point.z() - cluGlobalPosSIM.z();
	  ...
	  if (global_point.z()>0){ 
	    ...
	    if(optionalPlots_){meCluLocalYPullZGlobMinus_->Fill(ylocal_res / std::sqrt(comp->localPositionError().yy()));}
	  }
	  if(optionalPlots_){
...
void BtlLocalRecoValidation::bookHistograms(DQMStore::IBooker& ibook,
  ...
  if(optionalPlots_){
    ...
    meCluSingCrystalLocalYResZGlobPlus_ =
      ibook.book1D("BtlCluSingCrystalLocalYResZGlobPlus", "BTL cluster local Y resolution (single Crystal clusters, Z glob > 0);Y_{RECO}-Y_{SIM} [cm]", 100, -0.2, 0.2);
    meCluSingCrystalLocalYResZGlobMinus_ =
      ibook.book1D("BtlCluSingCrystalLocalYResZGlobMinus", "BTL cluster local Y resolution (single Crystal clusters, Z glob < 0);Y_{RECO}-Y_{SIM} [cm]", 100, -0.2, 0.2);
    ...
    meCluMultiCrystalLocalYResZGlobPlus_ =
      ibook.book1D("BtlCluMultiCrystalLocalYResZGlobPlus", "BTL cluster local Y resolution (Multi-Crystal clusters, Z glob > 0);Y_{RECO}-Y_{SIM} [cm]", 100, -0.2, 0.2);
    meCluMultiCrystalLocalYResZGlobMinus_ =
      ibook.book1D("BtlCluMultiCrystalLocalYResZGlobMinus", "BTL cluster local Y resolution (Multi-Crystal clusters, Z glob < 0);Y_{RECO}-Y_{SIM} [cm]", 100, -0.2, 0.2);
    ...
    meCluCentralLocalYResZGlobPlus_ =
      ibook.book1D("BtlCluCentralLocalYResZGlobPlus", "BTL cluster local Y resolution (central region, Z glob > 0);Y_{RECO}-Y_{SIM} [cm]", 100, -0.2, 0.2);
    meCluCentralLocalYResZGlobMinus_ =
      ibook.book1D("BtlCluCentralLocalYResZGlobMinus", "BTL cluster local Y resolution (central region, Z glob < 0);Y_{RECO}-Y_{SIM} [cm]", 100, -0.2, 0.2);
    ...
    meCluForwardPlusLocalYRes_ =
      ibook.book1D("BtlCluForwardPlusLocalYRes", "BTL cluster local Y resolution (forward region, Z glob > 0);Y_{RECO}-Y_{SIM} [cm]", 100, -0.2, 0.2);
    meCluForwardMinusLocalYRes_ =
      ibook.book1D("BtlCluForwardMinusLocalYRes", "BTL cluster local Y resolution (forward region, Z glob < 0);Y_{RECO}-Y_{SIM} [cm]", 100, -0.2, 0.2);
