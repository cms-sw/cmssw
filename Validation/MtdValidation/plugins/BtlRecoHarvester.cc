...
void BtlRecoHarvester::dqmEndJob(DQMStore::IBooker& ibook, DQMStore::IGetter& igetter) {
  ...
  if (!meTrackEffEtaTot || !meTrackEffPhiTot || !meTrackEffPtTot || !meTrackEffEtaMtd || !meTrackEffPhiMtd || !meTrackEffPtMtd ) {
  ...
  // --- Book  histograms
  ibook.cd(folder_);
  meBtlEtaEff_ = ibook.book1D("BtlEtaEff"," Track Efficiency VS Eta;#eta;Efficiency", meTrackEffEtaTot->getNbinsX(), meTrackEffEtaTot->getTH1()->GetXaxis()->GetXmin(), meTrackEffEtaTot->getTH1()->GetXaxis()->GetXmax());
  meBtlPhiEff_ = ibook.book1D("BtlPhiEff","Track Efficiency VS Phi;#phi;Efficiency [rad]", meTrackEffPhiTot->getNbinsX(), meTrackEffPhiTot->getTH1()->GetXaxis()->GetXmin(), meTrackEffPhiTot->getTH1()->GetXaxis()->GetXmax());
  meBtlPtEff_ = ibook.book1D("BtlPtEff","Track Efficiency VS Pt;Pt;Efficiency [GeV]", meTrackEffPtTot->getNbinsX(), meTrackEffPtTot->getTH1()->GetXaxis()->GetXmin(), meTrackEffPtTot->getTH1()->GetXaxis()->GetXmax());
  ...
  for (int ibin = 1; ibin <= meTrackEffEtaTot->getNbinsX(); ibin++) {
   ...
   double bin_err = sqrt((meTrackEffEtaMtd->getBinContent(ibin)*(meTrackEffEtaTot->getBinContent(ibin)-meTrackEffEtaMtd->getBinContent(ibin)))/pow(meTrackEffEtaTot->getBinContent(ibin),3));
  ...
  for (int ibin = 1; ibin <= meTrackEffPhiTot->getNbinsX(); ibin++) {
   ...
   double bin_err = sqrt((meTrackEffPhiMtd->getBinContent(ibin)*(meTrackEffPhiTot->getBinContent(ibin)-meTrackEffPhiMtd->getBinContent(ibin)))/pow(meTrackEffPhiTot->getBinContent(ibin),3));
  ...
  for (int ibin = 1; ibin <= meTrackEffPtTot->getNbinsX(); ibin++) {
   ...
   double bin_err = sqrt((meTrackEffPtMtd->getBinContent(ibin)*(meTrackEffPtTot->getBinContent(ibin)-meTrackEffPtMtd->getBinContent(ibin)))/pow(meTrackEffPtTot->getBinContent(ibin),3));
