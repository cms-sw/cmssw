...
void makeTimeResPlots(const TString DQMfilename="DQM_V0001_UNKNOWN_R000000001.root"){
  ...
  g_TimeResQ_BTL = new TH1F("g_TimeResQ_BTL", "BTL time resolution vs Q",
			    nBinsQ_BTL, 0., nBinsQ_BTL*binWidthQ_BTL);
  
  for (Int_t ih=0; ih<nBinsQEta_BTL; ++ih) {

    TString hname  = Form("g_TimeResQEta_BTL_%d", ih); 
    TString htitle = Form("BTL time resolution vs Q (%4.2f < |#eta_{hit}| < %4.2f)", 
			  binsQEta_BTL[ih], binsQEta_BTL[ih+1]); 
    g_TimeResQEta_BTL[ih] = new TH1F(hname, htitle, nBinsQ_BTL, 0., nBinsQ_BTL*binWidthQ_BTL);

  }


  g_TimeResEta_BTL = new TH1F("g_TimeResEta_BTL", "BTL time resolution vs |#eta_{hit}|",
			      nBinsEta_BTL, 0., nBinsEta_BTL*binWidthEta_BTL);

  for (Int_t ih=0; ih<nBinsEtaQ_BTL; ++ih) {

    TString hname  = Form("g_TimeResEtaQ_BTL_%d", ih); 
    TString htitle = Form("BTL time resolution vs |#eta_{hit}| (%4.2f < Q < %4.2f)", 
			  binsEtaQ_BTL[ih], binsEtaQ_BTL[ih+1]); 
  ...
  for (Int_t iside=0; iside<2; ++iside) {
    ...
    g_TimeResQ_ETL[iside] = new TH1F(hname, "ETL time resolution vs amplitude",
				     nBinsQ_ETL, 0., nBinsQ_ETL*binWidthQ_ETL);
  
    hname = Form("g_TimeResEta_ETL_%d", iside); 
    g_TimeResEta_ETL[iside] = new TH1F(hname, "ETL time resolution vs |#eta_{hit}|",
				       nBinsEta_ETL, etaMin_ETL, etaMin_ETL+nBinsEta_ETL*binWidthEta_ETL);
  ...
  for (Int_t iq=0; iq<nBinsQ_BTL; ++iq) {
    ...
    if ( h_TimeResQ_BTL[iq]->GetEntries() < minBinContent ) continue;
    ...
    if ( fit_res->Status() != 0 ){
      std::cout << " *** ERROR: fit failed for the histogram" 
		<< h_TimeResQ_BTL[iq]->GetName() << std::endl;
      continue;
    }
  ...
  for (Int_t iq=0; iq<nBinsQ_BTL; ++iq) {
    ...
    for (Int_t ieta=0; ieta<nBinsQEta_BTL; ++ieta) {
      ...
      TString hname = Form("DQMData/Run 1/MTD/Run summary/BTL/LocalReco/TimeResQvsEta_%d_%d",
			   iq, ieta);
      h_TimeResQEta_BTL[iq][ieta] = (TH1F*) input_file->Get(hname);

      if ( h_TimeResQEta_BTL[iq][ieta]->GetEntries() < minBinContent ) continue;

      Float_t low_limit = h_TimeResQEta_BTL[iq][ieta]->GetMean() - 
	                  2.*h_TimeResQEta_BTL[iq][ieta]->GetRMS();
      Float_t up_limit  = h_TimeResQEta_BTL[iq][ieta]->GetMean() + 
	                  3*h_TimeResQEta_BTL[iq][ieta]->GetRMS();

      TFitResultPtr fit_res = h_TimeResQEta_BTL[iq][ieta] ->Fit("gaus","SERQ0","",low_limit,up_limit);

      if ( fit_res->Status() != 0 ){
	std::cout << " *** ERROR: fit failed for the histogram" 
		  << h_TimeResQEta_BTL[iq][ieta]->GetName() << std::endl;
	continue;
      }
  ...
  for (Int_t ieta=0; ieta<nBinsEta_BTL; ++ieta){
    ...
    if ( h_TimeResEta_BTL[ieta]->GetEntries() < minBinContent ) continue;
    ...
    if ( fit_res->Status() != 0 ){
      std::cout << " *** ERROR: fit failed for the histogram" 
		<< h_TimeResEta_BTL[ieta]->GetName() << std::endl;
      continue;
    }
  ...
  for (Int_t ieta=0; ieta<nBinsEta_BTL; ++ieta) {
    ...
    for (Int_t iq=0; iq<nBinsEtaQ_BTL; ++iq) {
      ...
      TString hname = Form("DQMData/Run 1/MTD/Run summary/BTL/LocalReco/TimeResEtavsQ_%d_%d",
			   ieta, iq);
      h_TimeResEtaQ_BTL[ieta][iq] = (TH1F*) input_file->Get(hname);

      if ( h_TimeResEtaQ_BTL[ieta][iq]->GetEntries() < minBinContent ) continue;

      Float_t low_limit = h_TimeResEtaQ_BTL[ieta][iq]->GetMean() - 
	                  2.*h_TimeResEtaQ_BTL[ieta][iq]->GetRMS();
      Float_t up_limit  = h_TimeResEtaQ_BTL[ieta][iq]->GetMean() + 
	                  3*h_TimeResEtaQ_BTL[ieta][iq]->GetRMS();

      TFitResultPtr fit_res = h_TimeResEtaQ_BTL[ieta][iq] ->Fit("gaus","SERQ0","",low_limit,up_limit);

      if ( fit_res->Status() != 0 ){
	std::cout << " *** ERROR: fit failed for the histogram" 
		  << h_TimeResEtaQ_BTL[ieta][iq]->GetName() << std::endl;
	continue;
      }
  ...
  for (Int_t iside=0; iside<2; ++iside) {
    ...
    for (Int_t iq=0; iq<nBinsQ_ETL; ++iq) {
      ...
      if ( h_TimeResQ_ETL[iside][iq]->GetEntries() < minBinContent ) continue;
      ...
      if ( fit_res->Status() != 0 ){
	std::cout << " *** ERROR: fit failed for the histogram" 
		  << h_TimeResQ_ETL[iside][iq]->GetName() << std::endl;
	continue;
      }
    ...
    for (Int_t ieta=0; ieta<nBinsEta_ETL; ++ieta) {
      ...
      if ( h_TimeResEta_ETL[iside][ieta]->GetEntries() < minBinContent ) continue;
      ...
      if ( fit_res->Status() != 0 ){
	std::cout << " *** ERROR: fit failed for the histogram" 
		  << h_TimeResEta_ETL[iside][ieta]->GetName() << std::endl;
	continue;
      }
  ...
  for (Int_t ih=0; ih<nBinsQEta_BTL; ++ih)
    legend_1->AddEntry(g_TimeResQEta_BTL[ih],
		       Form("%4.2f #leq |#eta_{hit}| < %4.2f",  binsQEta_BTL[ih], binsQEta_BTL[ih+1]),
		       "LP");
  legend_1->Draw();
  ...
  for (Int_t ih=1; ih<nBinsEtaQ_BTL; ++ih){
    ...
    legend_3->AddEntry(g_TimeResEtaQ_BTL[ih],
		       Form("%4.0f #leq Q_{hit} < %4.0f pC",  binsEtaQ_BTL[ih], binsEtaQ_BTL[ih+1]),
		       "LP");
  }
