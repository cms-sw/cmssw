...
{
  std::set<unsigned int> result;
  for (const auto& p: superchamber_to_recHits_) result.insert(p.first);
  return result;
}


const GEMRecHitContainer&
...
GlobalPoint
GEMRecHitMatcher::recHitPosition(const GEMRecHit& rechit) const
{
...
GlobalPoint
GEMRecHitMatcher::recHitMeanPosition(const GEMRecHitContainer& rechit) const
{
