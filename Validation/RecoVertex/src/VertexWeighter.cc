...
VertexWeighter::VertexWeighter(const double& sigma_init,
    ...
    : m_sigma_init(sigma_init),
      m_mean_init(mean_init),
      m_sigma_final(sigma_final),
      m_usemain(usemainvtx),
      m_dummy(false) {}
...
const double VertexWeighter::weight(const std::vector<float>& zpositions, const float& zmain) const {
  ...
  if (!m_dummy) {
    for (auto zpos = zpositions.begin(); zpos != zpositions.end(); ++zpos) {
      final_weight *= (m_sigma_init / m_sigma_final) * exp(-pow((*zpos - m_mean_init), 2) / 2. *
                                                           (1. / pow(m_sigma_final, 2) - 1. / pow(m_sigma_init, 2)));
    }

    if (m_usemain) {
      final_weight *= (m_sigma_init / m_sigma_final) * exp(-pow((zmain - m_mean_init), 2) / 2. *
                                                           (1. / pow(m_sigma_final, 2) - 1. / pow(m_sigma_init, 2)));
    }
  }
