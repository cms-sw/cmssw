...
void VertexHistogramMaker::fill(const unsigned int orbit, const int bx, const float bxlumi, const reco::VertexCollection& vertices, const double weight) {
  ...
  for(const auto & vertice : vertices) {
    if(!vertice.isFake()) ntruevtx++;

    if(!(m_trueOnly && vertice.isFake())) {

      double aveweight = m_bsConstrained ? vertice.ndof()/(2.*vertice.tracksSize()) : (vertice.ndof()+3)/(2.*vertice.tracksSize());
      ...
      if(m_runHisto) {
	if(m_hvtxxrun && *m_hvtxxrun )  (*m_hvtxxrun)->Fill(vertice.x(),weight);
	if(m_hvtxyrun && *m_hvtxyrun )  (*m_hvtxyrun)->Fill(vertice.y(),weight);
	if(m_hvtxzrun && *m_hvtxzrun )  (*m_hvtxzrun)->Fill(vertice.z(),weight);
	if(m_runHistoProfile) {
	  if(m_hvtxxvsorbrun && *m_hvtxxvsorbrun )  (*m_hvtxxvsorbrun)->Fill(orbit,vertice.x(),weight);
	  if(m_hvtxyvsorbrun && *m_hvtxyvsorbrun )  (*m_hvtxyvsorbrun)->Fill(orbit,vertice.y(),weight);
	  if(m_hvtxzvsorbrun && *m_hvtxzvsorbrun )  (*m_hvtxzvsorbrun)->Fill(orbit,vertice.z(),weight);
	}
	if(m_runHistoBXProfile) {
	  if(m_hvtxxvsbxrun && *m_hvtxxvsbxrun )  (*m_hvtxxvsbxrun)->Fill(bx%3564,vertice.x(),weight);
	  if(m_hvtxyvsbxrun && *m_hvtxyvsbxrun )  (*m_hvtxyvsbxrun)->Fill(bx%3564,vertice.y(),weight);
	  if(m_hvtxzvsbxrun && *m_hvtxzvsbxrun )  (*m_hvtxzvsbxrun)->Fill(bx%3564,vertice.z(),weight);
	  if(m_runHisto2D) {
	    if(m_hvtxxvsbx2drun && *m_hvtxxvsbx2drun )  (*m_hvtxxvsbx2drun)->Fill(bx%3564,vertice.x(),weight);
	    if(m_hvtxyvsbx2drun && *m_hvtxyvsbx2drun )  (*m_hvtxyvsbx2drun)->Fill(bx%3564,vertice.y(),weight);
	    if(m_hvtxzvsbx2drun && *m_hvtxzvsbx2drun )  (*m_hvtxzvsbx2drun)->Fill(bx%3564,vertice.z(),weight);
	  }
	}
      }
