...
void LheWeightValidation::bookTemplates(std::vector<std::unique_ptr<TH1F>>& scaleVar, std::vector<std::unique_ptr<TH1F>>& pdfVar, std::vector<MonitorElement*>& tmps,
  ...
  tmps.push_back(dqm->book1dHisto(name+"ScaleUp_ratio","Ratio of "+title+" scale upper envelop / Nominal",nbin,low,high,xtitle,ytitle));
  tmps.at(3)->setEfficiencyFlag();
  tmps.push_back(dqm->book1dHisto(name+"ScaleDn_ratio","Ratio of "+title+" scale lower envelop / Nominal",nbin,low,high,xtitle,ytitle));
  ...
  tmps.push_back(dqm->book1dHisto(name+"PdfUp_ratio","Ratio of "+title+" PDF upper RMS / Nominal",nbin,low,high,xtitle,ytitle));
  tmps.at(7)->setEfficiencyFlag();
  tmps.push_back(dqm->book1dHisto(name+"PdfDn_ratio","Ratio of "+title+" PDF lower RMS / Nominal",nbin,low,high,xtitle,ytitle));
  tmps.at(8)->setEfficiencyFlag();

  for (int idx = 0; idx < nScaleVar; idx++) {
    scaleVar.push_back(std::make_unique<TH1F>(std::string(name+"Scale"+std::to_string(idx)).c_str(),std::string(";"+std::string(xtitle)+";"+std::string(ytitle)).c_str(),nbin,low,high));
    scaleVar.at(idx)->Sumw2();
  }

  for (int idx = 0; idx < nPdfVar; idx++) {
    pdfVar.push_back(std::make_unique<TH1F>(std::string(name+"Pdf"+std::to_string(idx)).c_str(),std::string(";"+std::string(xtitle)+";"+std::string(ytitle)).c_str(),nbin,low,high));
    pdfVar.at(idx)->Sumw2();
  }
