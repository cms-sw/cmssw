...
void TauValidation::beginJob(){
  ...
  dbe->setCurrentFolder("Generator/Tau");
    ...
    DecayLength = dbe->book1D("DecayLength","#tau Decay Length", 100 ,-20,20);  DecayLength->setAxisTitle("L_{#tau} (cm)");
    LifeTime =  dbe->book1D("LifeTime","#tau LifeTime ", 500 ,0,10000E-15);     LifeTime->setAxisTitle("#tau_{#tau} (s)");
    ...
    TauSpinEffectsZ_X50to75= dbe->book1D("TauSpinEffectsZX50to75","X of #tau^{-} (50GeV-75GeV)", 10 ,0,1.0);           TauSpinEffectsZ_X->setAxisTitle("X");
    TauSpinEffectsZ_X75to88= dbe->book1D("TauSpinEffectsZX75to88","X of #tau^{-} (75GeV-88GeV)", 10 ,0,1.0);           TauSpinEffectsZ_X->setAxisTitle("X");
    TauSpinEffectsZ_X88to100= dbe->book1D("TauSpinEffectsZX88to100","X of #tau^{-} (88GeV-100GeV)", 10 ,0,1.0);           TauSpinEffectsZ_X->setAxisTitle("X");
    TauSpinEffectsZ_X100to120= dbe->book1D("TauSpinEffectsZX100to120","X of #tau^{-} (100GeV-120GeV)", 10 ,0,1.0);           TauSpinEffectsZ_X->setAxisTitle("X");
    TauSpinEffectsZ_X120UP= dbe->book1D("TauSpinEffectsZX120UP","X of #tau^{-} (>120GeV)", 10 ,0,1.0);           TauSpinEffectsZ_X->setAxisTitle("X");
    ...
    for(unsigned int j=0; j<NJAKID+1;j++){
      ...
      TString tmp="JAKID";
      tmp+=j;
      JAKInvMass.at(j).push_back(dbe->book1D("M"+tmp,"M_{"+tmp+"} (GeV)", 80 ,0,2.0));
      if(j==TauDecay::JAK_A1_3PI ||
	 j==TauDecay::JAK_KPIK ||
	 j==TauDecay::JAK_KPIPI ){
	JAKInvMass.at(j).push_back(dbe->book1D("M13"+tmp,"M_{13,"+tmp+"} (GeV)", 80 ,0,2.0));
	JAKInvMass.at(j).push_back(dbe->book1D("M23"+tmp,"M_{23,"+tmp+"} (GeV)", 80 ,0,2.0));
	JAKInvMass.at(j).push_back(dbe->book1D("M12"+tmp,"M_{12,"+tmp+"} (GeV)", 80 ,0,2.0));
      }
    }
  return;
}
