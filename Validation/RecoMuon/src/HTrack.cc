...
void HTrack::computeResolution(FreeTrajectoryState &fts,
  ...
  hReso->Fill(simTrack.momentum().mag(),
	      ...
	      resolution(fts.momentum().eta(),simTrack.momentum().eta()),
	      resolution(fts.momentum().phi(),simTrack.momentum().phi()),
	      resolution(fts.charge(),-simTrack.type()/ abs(simTrack.type()))); // FIXME
}
...
void HTrack::computeTDRResolution(FreeTrajectoryState &fts,
  ...
  hReso->Fill(simTrack.momentum().mag(),
	      ...
	      //resolution(1./fts.momentum().perp(),1./simTrack.momentum().perp()),
	      resolution(1./fts.momentum().eta(),1./simTrack.momentum().eta()),
	      resolution(1./fts.momentum().phi(),1./simTrack.momentum().phi()),
	      resolution(1./fts.charge(),1./simCharge)); // FIXME
}
...
void HTrack::computePull(FreeTrajectoryState &fts,
  ...
  // x,y,z, px,py,pz
  AlgebraicSymMatrix errors = fts.cartesianError().matrix();
