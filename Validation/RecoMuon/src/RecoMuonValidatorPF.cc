...
//PF-RECO comparisons
  ...
  MEP hPFMomPicked_vs_Pt, hPFDirectionPicked_vs_Pt, hPFMomAssCorrectness_vs_Pt;
...
//
  void bookHistograms(DQMStore* dqm, const string& dirName, const HistoDimensions& hDim)
  {
    ...
    //PF-RECO comparisons
    hErrPt_PF_  = dqm->book1D("ErrPt_PF" , "#Delta(p_{T})|_{PF}/p_{T}", 10.*hDim.nBinErr, hDim.minErrPt , hDim.maxErrPt );
    hErrQPt_PF_  = dqm->book1D("ErrQPt_PF" , "#Delta(q/p_{T})|_{PF}/(q/p_{T})", 10.*hDim.nBinErr, hDim.minErrQPt, hDim.maxErrQPt);

    hPFMomPicked  = dqm->book1D("hPFMomPicked", "Momentum picked by PF vs recoPt",5,0,5);
    hPFDirectionPicked  = dqm->book1D("hPFDirectionPicked", "Direction picked by PF vs recoPt",5,0,5);
    hPFMomAssCorrectness  = dqm->book1D("hPFMomAssCorrectness", "Corrected momentum assignement PF/RECO",5,0,5);

    hdPt_vs_Pt_  = dqm->book2D("dPt_vs_Pt", "#Delta(p_{T}) vs p_{T}",5000,0,5,500,0,500);
    hdPt_vs_Eta_  = dqm->book2D("dPt_vs_Eta", "#Delta(p_{T}) vs #eta",5000,0,5,hDim.nBinEta, hDim.minEta, hDim.maxEta);
    
    hPFMomPicked_vs_Pt  = dqm->book2D("hPFMomPicked_vs_Pt", "Momentum picked by PF vs recoPt",5,0,5,500,0,500);
    hPFDirectionPicked_vs_Pt  = dqm->book2D("hPFDirectionPicked_vs_Pt", "Direction picked by PF vs recoPt",5,0,5,500,0,500);
    hPFMomAssCorrectness_vs_Pt  = dqm->book2D("hPFMomAssCorrectness_vs_Pt", "Corrected momentum assignement PF/RECO",5,0,5,500,0,500);
           ...
           if (muonRef->isGlobalMuon()) {
                  ...
                  //has momentum of global track
                     if (fabs(recoPFPt - muonRef->globalTrack()->pt()) < MatchPFRECOMomMagn) {
                         hPFMomPicked->Fill(1);
                         hPFMomPicked_vs_Pt->Fill(1,recoPt);
                        //corretness
                        if (fabs(recoPFPt-simPt) < fabs(recoPt - simPt)) {
                           hPFMomAssCorrectness->Fill(1);
                           hPFMomAssCorrectness_vs_Pt->Fill(1,recoPt);
                        } else  {
                           //else: PF was wrong
                           hPFMomAssCorrectness->Fill(2);
                           hPFMomAssCorrectness_vs_Pt->Fill(2,recoPt);
                        }
                     } else {
                     //just the direction
                      hPFDirectionPicked->Fill(1);
                      hPFMomPicked->Fill(4);
                      hPFMomPicked_vs_Pt->Fill(4,recoPt);
                     }
                  }
                ...
                if (muonRef->isTrackerMuon()) {
                  ...
                  //has momentum of inner track
                     if (fabs(recoPFPt - muonRef->innerTrack()->pt()) < MatchPFRECOMomMagn) {
                         hPFMomPicked->Fill(2);
                         hPFMomPicked_vs_Pt->Fill(2,recoPt);
                        //corretness
                        if (fabs(recoPFPt-simPt) < fabs(recoPt - simPt)) {
                           hPFMomAssCorrectness->Fill(1);
                           hPFMomAssCorrectness_vs_Pt->Fill(1,recoPt);
                 ...
                 //has direction of outer track
                  ...
                  //has momentum of inner track
                     if (fabs(recoPFPt - muonRef->outerTrack()->pt()) < MatchPFRECOMomMagn) {
                         hPFMomPicked->Fill(3);
                         hPFMomPicked_vs_Pt->Fill(3,recoPt);
                        //corretness
                        if (fabs(recoPFPt-simPt) < fabs(recoPt - simPt)) {
                           hPFMomAssCorrectness->Fill(1);
                           hPFMomAssCorrectness_vs_Pt->Fill(1,recoPt);
