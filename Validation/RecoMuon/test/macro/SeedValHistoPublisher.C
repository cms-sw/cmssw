  ...
  */
  ...
  while ((rKey = (TKey*)iter_r())) {
    ...
    */
    ...
    const char* plots2[] = {
        "seedPt_", "seedPtErrOverPt_", "seedPz_", "seedPzErrOverPz_", "NumberOfRecHitsPerSeed_", "seedPErrOverP_"};
    const char* plotsl2[] = {"seed P_{T}",
                             "seed P_{T} Err/P_{T}",
                             "seed P_{Z}",
                             "seed P_{Z} Err/P_{Z}",
                             "Nr RecHits per seed",
    ...
    TString gscommand = "gs -dBATCH -dNOPAUSE -q -sDEVICE=pdfwrite -sOutputFile=" + mergefile + " " + newDir +
                        "/muonSeed1.pdf " + newDir + "/muonSeed2.pdf ";
