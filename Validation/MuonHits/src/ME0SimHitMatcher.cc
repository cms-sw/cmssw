...
ME0SimHitMatcher::ME0SimHitMatcher(const edm::ParameterSet& ps,
...
void ME0SimHitMatcher::init(const edm::Event& iEvent,
  ...
  if (me0_geom_.isValid()) {
    ...
    hasGeometry_ = false;
    edm::LogWarning("ME0SimHitMatcher")
  ...
  if (hasGeometry_) {
    ...
    if (verbose_) {
      edm::LogInfo("ME0SimHitMatcher")
          << "nTrackIds " << track_ids_.size() << " nSelectedME0SimHits "
          << hits_.size() << endl;
      edm::LogInfo("ME0SimHitMatcher")
          << "detids ME0 " << detIds().size() << endl;
      ...
      for (const auto& id : me0_ch_ids) {
        ...
        edm::LogInfo("ME0SimHitMatcher")
            << "me0chid " << ME0DetId(id) << ": nHits " << me0_simhits.size()
            << " phi " << me0_simhits_gp.phi() << " nCh "
...
const edm::PSimHitContainer& ME0SimHitMatcher::hitsInSuperChamber(
...
std::set<unsigned int> ME0SimHitMatcher::superChamberIdsCoincidences(
    int min_n_layers) const {
...
float ME0SimHitMatcher::simHitsMeanStrip(
...
std::set<int> ME0SimHitMatcher::hitStripsInDetId(unsigned int detid,
  ...
  ME0DetId id(detid);
  int max_nstrips =
      dynamic_cast<const ME0Geometry*>(geometry_)->etaPartition(id)->nstrips();
  for (const auto& h : simhits) {
    const LocalPoint& lp = h.entryPoint();
    int central_strip =
        1 + static_cast<int>(dynamic_cast<const ME0Geometry*>(geometry_)
                                 ->etaPartition(id)
                                 ->topology()
                                 .channel(lp));
