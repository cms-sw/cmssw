...
MuonSimHitMatcher::MuonSimHitMatcher(const edm::ParameterSet& ps,
  ...
  simVertexInput_ = iC.consumes<edm::SimVertexContainer>(
      simVertex.getParameter<edm::InputTag>("inputTag"));
  simTrackInput_ = iC.consumes<edm::SimTrackContainer>(
      simTrack.getParameter<edm::InputTag>("inputTag"));
}

/// initialize the event
void MuonSimHitMatcher::init(const edm::Event& iEvent,
...
std::vector<unsigned int> MuonSimHitMatcher::getIdsOfSimTrackShower(
    unsigned int initial_trk_id, const edm::SimTrackContainer& simTracks,
...
GlobalPoint MuonSimHitMatcher::simHitsMeanPosition(
  ...
  for (const auto& h : sim_hits) {
    ...
    const GlobalPoint& gp =
        geometry_->idToDet(h.detUnitId())->surface().toGlobal(lp);
    sumx += gp.x();
    sumy += gp.y();
    sumz += gp.z();
    ++n;
...
GlobalVector MuonSimHitMatcher::simHitsMeanMomentum(
    const edm::PSimHitContainer& sim_hits) const {
  ...
  for (const auto& h : sim_hits) {
    ...
    const GlobalVector& gv =
        geometry_->idToDet(h.detUnitId())->surface().toGlobal(lv);
    sumx += gv.x();
    sumy += gv.y();
    sumz += gv.z();
    ++n;
