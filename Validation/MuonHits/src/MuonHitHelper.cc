...
bool MuonHitHelper::isDT(unsigned int detId) {
  return (DetId(detId)).det() == DetId::Muon &&
         (DetId(detId)).subdetId() == MuonSubdetId::DT;
}

bool MuonHitHelper::isGEM(unsigned int detId) {
  return (DetId(detId)).det() == DetId::Muon &&
         (DetId(detId)).subdetId() == MuonSubdetId::GEM;
}

bool MuonHitHelper::isCSC(unsigned int detId) {
  return (DetId(detId)).det() == DetId::Muon &&
         (DetId(detId)).subdetId() == MuonSubdetId::CSC;
}

bool MuonHitHelper::isRPC(unsigned int detId) {
  return (DetId(detId)).det() == DetId::Muon &&
         (DetId(detId)).subdetId() == MuonSubdetId::RPC;
}

bool MuonHitHelper::isME0(unsigned int detId) {
  return (DetId(detId)).det() == DetId::Muon &&
         (DetId(detId)).subdetId() == MuonSubdetId::ME0;
}
