...
void BDHadronTrackMonitoringHarvester::dqmEndJob(DQMStore::IBooker &ibook,
  ...
  nTrk_absolute_bjet =
      ibook.book1D("nTrk_absolute_bjet",
                   "absolute average number of tracks in b jets", 6, -0.5, 5.5);
  for (unsigned int i = 0;
       i < BDHadronTrackMonitoringAnalyzer::TrkHistCat.size(); i++) {
    nTrk_absolute_bjet->setBinLabel(
        i + 1, BDHadronTrackMonitoringAnalyzer::TrkHistCat[i], 1);
  }
  ...
  nTrk_relative_bjet =
      ibook.book1D("nTrk_relative_bjet",
                   "relative average number of tracks in b jets", 6, -0.5, 5.5);
  for (unsigned int i = 0;
       i < BDHadronTrackMonitoringAnalyzer::TrkHistCat.size(); i++) {
    nTrk_relative_bjet->setBinLabel(
        i + 1, BDHadronTrackMonitoringAnalyzer::TrkHistCat[i], 1);
  }
  ...
  nTrk_std_bjet = ibook.book1D(
      "nTrk_std_bjet", "RMS of number of tracks in b jets", 6, -0.5, 5.5);
  for (unsigned int i = 0;
       i < BDHadronTrackMonitoringAnalyzer::TrkHistCat.size(); i++) {
    nTrk_std_bjet->setBinLabel(
        i + 1, BDHadronTrackMonitoringAnalyzer::TrkHistCat[i], 1);
  }
  ...
  // c jets
  nTrk_absolute_cjet =
      ibook.book1D("nTrk_absolute_cjet",
                   "absolute average number of tracks in c jets", 6, -0.5, 5.5);
  for (unsigned int i = 0;
       i < BDHadronTrackMonitoringAnalyzer::TrkHistCat.size(); i++) {
    nTrk_absolute_cjet->setBinLabel(
        i + 1, BDHadronTrackMonitoringAnalyzer::TrkHistCat[i], 1);
  }
  ...
  nTrk_relative_cjet =
      ibook.book1D("nTrk_relative_cjet",
                   "relative average number of tracks in c jets", 6, -0.5, 5.5);
  for (unsigned int i = 0;
       i < BDHadronTrackMonitoringAnalyzer::TrkHistCat.size(); i++) {
    nTrk_relative_cjet->setBinLabel(
        i + 1, BDHadronTrackMonitoringAnalyzer::TrkHistCat[i], 1);
  }
  nTrk_relative_cjet->setAxisRange(0, 1, 2);
  nTrk_relative_cjet->setAxisTitle("average fraction of tracks", 2);

  nTrk_std_cjet = ibook.book1D(
      "nTrk_std_cjet", "RMS of number of tracks in c jets", 6, -0.5, 5.5);
  for (unsigned int i = 0;
       i < BDHadronTrackMonitoringAnalyzer::TrkHistCat.size(); i++) {
    nTrk_std_cjet->setBinLabel(
        i + 1, BDHadronTrackMonitoringAnalyzer::TrkHistCat[i], 1);
  }
  ...
  // udsg jets
  nTrk_absolute_dusgjet = ibook.book1D(
      "nTrk_absolute_dusgjet", "absolute average number of tracks in dusg jets",
      6, -0.5, 5.5);
  for (unsigned int i = 0;
       i < BDHadronTrackMonitoringAnalyzer::TrkHistCat.size(); i++) {
    nTrk_absolute_dusgjet->setBinLabel(
        i + 1, BDHadronTrackMonitoringAnalyzer::TrkHistCat[i], 1);
  }
  nTrk_absolute_dusgjet->setAxisRange(0, 5, 2);
  nTrk_absolute_dusgjet->setAxisTitle("average number of tracks", 2);

  nTrk_relative_dusgjet = ibook.book1D(
      "nTrk_relative_dusgjet", "relative average number of tracks in dusg jets",
      6, -0.5, 5.5);
  for (unsigned int i = 0;
       i < BDHadronTrackMonitoringAnalyzer::TrkHistCat.size(); i++) {
    nTrk_relative_dusgjet->setBinLabel(
        i + 1, BDHadronTrackMonitoringAnalyzer::TrkHistCat[i], 1);
  }
  nTrk_relative_dusgjet->setAxisRange(0, 1, 2);
  nTrk_relative_dusgjet->setAxisTitle("average fraction of tracks", 2);

  nTrk_std_dusgjet = ibook.book1D(
      "nTrk_std_dusgjet", "RMS of number of tracks in dusg jets", 6, -0.5, 5.5);
  for (unsigned int i = 0;
       i < BDHadronTrackMonitoringAnalyzer::TrkHistCat.size(); i++) {
    nTrk_std_dusgjet->setBinLabel(
        i + 1, BDHadronTrackMonitoringAnalyzer::TrkHistCat[i], 1);
  }
  ...
  for (unsigned int i = 0;
       i < BDHadronTrackMonitoringAnalyzer::TrkHistCat.size(); i++) {
    nTrk_bjet[i] = iget.get("BDHadronTracks/JetContent/nTrk_bjet_" +
                            BDHadronTrackMonitoringAnalyzer::TrkHistCat[i]);
  }
  MonitorElement *nTrkAll_bjet =
      iget.get("BDHadronTracks/JetContent/nTrkAll_bjet");

  // c jets
  MonitorElement *nTrk_cjet[6];
  for (unsigned int i = 0;
       i < BDHadronTrackMonitoringAnalyzer::TrkHistCat.size(); i++) {
    nTrk_cjet[i] = iget.get("BDHadronTracks/JetContent/nTrk_cjet_" +
                            BDHadronTrackMonitoringAnalyzer::TrkHistCat[i]);
  }
  MonitorElement *nTrkAll_cjet =
      iget.get("BDHadronTracks/JetContent/nTrkAll_cjet");

  // dusg jets
  MonitorElement *nTrk_dusgjet[6];
  for (unsigned int i = 0;
       i < BDHadronTrackMonitoringAnalyzer::TrkHistCat.size(); i++) {
    nTrk_dusgjet[i] = iget.get("BDHadronTracks/JetContent/nTrk_dusgjet_" +
                               BDHadronTrackMonitoringAnalyzer::TrkHistCat[i]);
  }
  MonitorElement *nTrkAll_dusgjet =
      iget.get("BDHadronTracks/JetContent/nTrkAll_dusgjet");
  ...
  for (unsigned int i = 0;
       i < BDHadronTrackMonitoringAnalyzer::TrkHistCat.size(); i++) {
    mean_bjets[i] =
        nTrk_bjet[i]->getMean(1); // mean number of tracks per category
  ...
  for (unsigned int i = 0;
       i < BDHadronTrackMonitoringAnalyzer::TrkHistCat.size(); i++) {
    mean_cjets[i] =
        nTrk_cjet[i]->getMean(1); // mean number of tracks per category
  ...
  for (unsigned int i = 0;
       i < BDHadronTrackMonitoringAnalyzer::TrkHistCat.size(); i++) {
    mean_dusgjets[i] =
        nTrk_dusgjet[i]->getMean(1); // mean number of tracks per category
    std_dusgjets[i] = nTrk_dusgjet[i]->getRMS(1);
    nTrk_absolute_dusgjet->setBinContent(i + 1, mean_dusgjets[i]);
    nTrk_relative_dusgjet->setBinContent(i + 1,
                                         mean_dusgjets[i] / meanAll_dusgjets);
  ...
  for (unsigned int i = 0;
       i < BDHadronTrackMonitoringAnalyzer::TrkHistCat.size(); i++) {
    iget.removeElement("nTrk_dusgjet_" +
                       BDHadronTrackMonitoringAnalyzer::TrkHistCat[i]);
  }
