...
BTagPerformanceAnalyzerMC::BTagPerformanceAnalyzerMC(
    ...
    : jetSelector(pSet.getParameter<double>("etaMin"),
                  ...
                  pSet.getParameter<double>("ptRecJetMax"), 0.0, 99999.0,
  ...
  if (etaRanges.size() <= 1)
    etaRanges = {pSet.getParameter<double>("etaMin"),
                 pSet.getParameter<double>("etaMax")};
  if (ptRanges.size() <= 1)
    ptRanges = {pSet.getParameter<double>("ptRecJetMin"),
                pSet.getParameter<double>("ptRecJetMax")};

  genToken = mayConsume<GenEventInfoProduct>(edm::InputTag("generator"));
  genJetsMatchedToken = mayConsume<edm::Association<reco::GenJetCollection>>(
      pSet.getParameter<InputTag>("genJetsMatched"));
  jetToken = consumes<JetFlavourInfoMatchingCollection>(
      pSet.getParameter<InputTag>("jetMCSrc"));
  caloJetToken = mayConsume<JetFlavourMatchingCollection>(
      pSet.getParameter<InputTag>("caloJetMCSrc"));
  slInfoToken = consumes<SoftLeptonTagInfoCollection>(
      pSet.getParameter<InputTag>("softLeptonInfo"));
  jecMCToken =
      consumes<JetCorrector>(pSet.getParameter<edm::InputTag>("JECsourceMC"));
  jecDataToken = mayConsume<JetCorrector>(
      pSet.getParameter<edm::InputTag>("JECsourceData"));
  ...
  for (vector<edm::ParameterSet>::const_iterator iModule = moduleConfig.begin();
    ...
    if (dataFormatType == "JetTag") {
      ...
      tagCorrelationInputTags.push_back(
          std::pair<edm::InputTag, edm::InputTag>(label1, label2));
      binTagCorrelationPlotters.push_back(
          vector<std::unique_ptr<TagCorrelationPlotter>>());
      tagCorrelationToken.push_back(
          std::pair<edm::EDGetTokenT<reco::JetTagCollection>,
                    edm::EDGetTokenT<reco::JetTagCollection>>(
              consumes<JetTagCollection>(label1),
              consumes<JetTagCollection>(label2)));
    } else {
      vector<edm::InputTag> vIP;
      tiDataFormatType.push_back(dataFormatType);
      binTagInfoPlotters.push_back(
          vector<std::unique_ptr<BaseTagInfoPlotter>>());
      std::vector<edm::EDGetTokenT<edm::View<reco::BaseTagInfo>>> tokens;
      if (dataFormatType == "GenericMVA") {
        const std::vector<InputTag> listInfo =
            iModule->getParameter<vector<InputTag>>("listTagInfos");
...
void BTagPerformanceAnalyzerMC::bookHistograms(DQMStore::IBooker &ibook,
  ...
  for (vector<edm::ParameterSet>::const_iterator iModule = moduleConfig.begin();
    ...
    if (dataFormatType == "JetTag") {
      ...
      double discrCut = -999.;
      if (iModule->exists("differentialPlots") &&
          iModule->getParameter<bool>("differentialPlots") == true) {
      ...
      for (int iEta = iEtaStart; iEta < iEtaEnd; iEta++) {
        ...
        for (int iPt = iPtStart; iPt < iPtEnd; iPt++) {
          ...
          binJetTagPlotters.at(iTag).push_back(std::make_unique<JetTagPlotter>(
              folderName, etaPtBin,
              iModule->getParameter<edm::ParameterSet>("parameters"), mcPlots_,
              false, ibook, false, doDifferentialPlots, discrCut));
        }
      }
    } else if (dataFormatType == "TagCorrelation") {
      iTagCorr++;
      ...
      for (int iEta = iEtaStart; iEta != iEtaEnd; ++iEta) {
        ...
        for (int iPt = iPtStart; iPt != iPtEnd; ++iPt) {
          ...
          binTagCorrelationPlotters.at(iTagCorr).push_back(
              std::make_unique<TagCorrelationPlotter>(
                  label1.label(), label2.label(), etaPtBin,
      ...
      for (int iEta = iEtaStart; iEta < iEtaEnd; iEta++) {
        ...
        for (int iPt = iPtStart; iPt < iPtEnd; iPt++) {
          ...
          binTagInfoPlotters.at(iInfoTag).push_back(theFactory.buildPlotter(
              dataFormatType, moduleLabel.label(), etaPtBin,
              iModule->getParameter<edm::ParameterSet>("parameters"),
              folderName, mcPlots_, false, ibook));
        }
      }
    }
  }
...
void BTagPerformanceAnalyzerMC::analyze(const edm::Event &iEvent,
  ...
  for (unsigned int iJetLabel = 0; iJetLabel != jetTagInputTags.size();
       ++iJetLabel) {
    ...
    LogDebug("Info") << "Found " << tagColl.size()
                     << " B candidates in collection "
                     << jetTagInputTags[iJetLabel];
    ...
    for (JetTagCollection::const_iterator tagI = tagColl.begin();
      ...
      if (!getJetWithFlavour(iEvent, tagI->first, flavours, jetWithFlavour,
                             corrector, genJetsMatched))
        continue;
      if (!jetSelector(jetWithFlavour.first,
                       std::abs(jetWithFlavour.second.getPartonFlavour()),
                       infoHandle, jec))
        continue;
  ...
  for (unsigned int iJetLabel = 0; iJetLabel != tagCorrelationInputTags.size();
       ++iJetLabel) {
    const std::pair<edm::EDGetTokenT<reco::JetTagCollection>,
                    edm::EDGetTokenT<reco::JetTagCollection>> &inputTokens =
    ...
    for (JetTagCollection::const_iterator tagI = tagColl1.begin();
         tagI != tagColl1.end(); ++tagI) {
      ...
      for (int iPlotter = 0; iPlotter != plotterSize; ++iPlotter) {
        bool inBin = false;
        inBin =
            binTagCorrelationPlotters[iJetLabel][iPlotter]->etaPtBin().inBin(
                jetWithFlavour.first, jec);
  ...
  for (unsigned int iJetLabel = 0; iJetLabel != tiDataFormatType.size();
    ...
    for (unsigned int iTagInfos = 0; iTagInfos < nTagInfos; ++iTagInfos) {
      ...
      JetWithFlavour jetWithFlavour;
      // also applies JEC to jet
      if (!getJetWithFlavour(iEvent, jetRef, flavours, jetWithFlavour,
                             corrector, genJetsMatched))
        continue;
      if (!jetSelector(jetWithFlavour.first,
                       std::abs(jetWithFlavour.second.getPartonFlavour()),
...
bool BTagPerformanceAnalyzerMC::getJetWithGenJet(
    edm::RefToBase<Jet> jetRef,
    edm::Handle<edm::Association<reco::GenJetCollection>> genJetsMatched) {
  if (!doPUid)
    return true;
...
bool BTagPerformanceAnalyzerMC::getJetWithFlavour(
    const edm::Event &iEvent, edm::RefToBase<Jet> jetRef,
    const FlavourMap &flavours, JetWithFlavour &jetWithFlavour,
  ...
  if (!eventInitialized) {
    ...
    if (recProdId != refProdId) {
      ...
      const edm::RefToBaseProd<Jet> recJetsProd(
          edm::makeRefToBaseProdFrom(jetRef, iEvent));
  ...
  LogTrace("Info") << "Found jet with flavour "
                   << jetWithFlavour.second.getPartonFlavour() << endl;
  LogTrace("Info") << jetWithFlavour.first.p() << " , "
                   << jetWithFlavour.first.pt() << " - " << endl;
