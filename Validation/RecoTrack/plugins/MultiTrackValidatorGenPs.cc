...
void MultiTrackValidatorGenPs::analyze(const edm::Event& event, const edm::EventSetup& setup){
  ...
  for (unsigned int www=0;www<label.size();www++){
    ...
    for(View<Track>::size_type i=0; i<trackCollection->size(); ++i){
      ...
      histoProducerAlgo_->fill_generic_recoTrack_histos(w,*track, ttopo, bs.position(), nullptr, nullptr, isGenMatched,isSigGenMatched, isChargeMatched, numAssocRecoTracks, puinfo.getPU_NumInteractions(), nSimHits, sharedFraction, dR, mvaDummy);
