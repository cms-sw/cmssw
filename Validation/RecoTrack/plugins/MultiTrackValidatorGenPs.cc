...
MultiTrackValidatorGenPs::MultiTrackValidatorGenPs(
  ...
  gpSelector = GenParticleCustomSelector(
      pset.getParameter<double>("ptMinGP"),
      pset.getParameter<double>("minRapidityGP"),
      pset.getParameter<double>("maxRapidityGP"),
      pset.getParameter<double>("tipGP"), pset.getParameter<double>("lipGP"),
...
void MultiTrackValidatorGenPs::analyze(const edm::Event& event,
  ...
  for (unsigned int ww = 0; ww < associators.size(); ww++) {
    associatorByChi2 =
        dynamic_cast<const TrackAssociatorByChi2*>(associator[ww]);
    ...
    for (unsigned int www = 0; www < label.size(); www++) {
      ...
      if (UseAssociators) {
        ...
        LogTrace("TrackValidator") << "Calling associateRecoToGen method"
                                   << "\n";
        recGenColl = associatorByChi2->associateRecoToGen(
            trackCollection, TPCollectionHfake, &event);
        LogTrace("TrackValidator") << "Calling associateGenToReco method"
                                   << "\n";
        genRecColl = associatorByChi2->associateGenToReco(
            trackCollection, TPCollectionHeff, &event);
      } else {
        edm::LogVerbatim("TrackValidator")
            << "Analyzing " << label[www].process() << ":" << label[www].label()
      ...
      for (GenParticleCollection::size_type i = 0; i < tPCeff.size();
        ...
        // If the GenParticle is collison like, get the momentum and vertex at
        // production state
        if (parametersDefiner == "LhcParametersDefinerForTP") {
          ...
          dxyGen = (-vertex.x() * sin(momentum.phi()) +
        ...
        if (parametersDefiner == "CosmicParametersDefinerForTP") {
          ...
          dxyGen = (-vertexTP.x() * sin(momentumTP.phi()) +
        ...
        if (genRecColl.find(tpr) != genRecColl.end()) {
          rt = (std::vector<std::pair<RefToBase<Track>, double> >)
              genRecColl[tpr];
          if (rt.size() != 0) {
        ...
        histoProducerAlgo_->fill_recoAssociated_simTrack_histos(
            w, *tp, momentumTP, vertexTP, dxyGen, dzGen, nSimHits,
            matchedTrackPointer, puinfo.getPU_NumInteractions());

        sts++;
        if (matchedTrackPointer) asts++;

      }  // End  for (GenParticleCollection::size_type i=0; i<tPCeff.size();
         // i++){
      ...
      // associated to GenTracks from Signal only
      int at(0);  // This counter counts the number of recoTracks that are
                  // associated to GenTracks
      ...
      for (View<Track>::size_type i = 0; i < trackCollection->size(); ++i) {
        ...
        if (recGenColl.find(track) != recGenColl.end()) {
          ...
          if (tp.size() != 0) {
            /*
            std::vector<PSimHit>
            simhits=tp[0].first->trackPSimHit(DetId::Tracker);
        ...
        histoProducerAlgo_->fill_generic_recoTrack_histos(
            w, *track, bs.position(), isGenMatched, isSigGenMatched,
        ...
        */
        ...
        // beamline
        TrackingParticle::Vector momentumTP =
            parametersDefinerTP->momentum(event, setup, *(tpr.get()));
        TrackingParticle::Point vertexTP =
            parametersDefinerTP->vertex(event, setup, *(tpr.get()));
