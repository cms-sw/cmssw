  ...
  for (unsigned int ww = 0; ww < associators.size(); ww++) {
    for (unsigned int www = 0; www < label.size(); www++) {
      edm::LogVerbatim("TrackValidator")
          ...
          << ":" << label[www].instance() << " with " << associators[ww].c_str()
          << "\n";
      //
      ...
      LogTrace("TrackValidator") << "Calling associateRecoToSim method"
                                 << "\n";
      reco::RecoToSimCollectionSeed recSimColl =
          associator[ww]->associateRecoToSim(seedCollection, TPCollectionHfake,
                                             &event, &setup);
      LogTrace("TrackValidator") << "Calling associateSimToReco method"
                                 << "\n";
      reco::SimToRecoCollectionSeed simRecColl =
          associator[ww]->associateSimToReco(seedCollection, TPCollectionHeff,
                                             &event, &setup);
      ...
      for (TrackingParticleCollection::size_type i = 0; i < tPCeff.size();
        ...
        TrackingParticle::Vector momentum =
            parametersDefinerTP->momentum(event, setup, tp);
        TrackingParticle::Point vertex =
            parametersDefinerTP->vertex(event, setup, tp);
        double dxySim = (-vertex.x() * sin(momentum.phi()) +
                         vertex.y() * cos(momentum.phi()));
        double dzSim = vertex.z() -
        ...
        if (matchedSeedPointer) {
          ...
          TransientTrackingRecHit::RecHitPointer recHit = theTTRHBuilder->build(
              &*(matchedSeedPointer->recHits().second - 1));
          TrajectoryStateOnSurface state =
              trajectoryStateTransform::transientState(
                  matchedSeedPointer->startingState(), recHit->surface(),
                  theMF.product());
          TrajectoryStateClosestToBeamLine tsAtClosestApproachSeed =
              tscblBuilder(*state.freeState(),
                           bs);  // as in TrackProducerAlgorithm
          if (!(tsAtClosestApproachSeed.isValid())) {
            edm::LogVerbatim("SeedValidator")
                << "TrajectoryStateClosestToBeamLine not valid";
            continue;
          }
          const reco::TrackBase::Point vSeed1(
              tsAtClosestApproachSeed.trackStateAtPCA().position().x(),
              tsAtClosestApproachSeed.trackStateAtPCA().position().y(),
              tsAtClosestApproachSeed.trackStateAtPCA().position().z());
          const reco::TrackBase::Vector pSeed(
              tsAtClosestApproachSeed.trackStateAtPCA().momentum().x(),
              tsAtClosestApproachSeed.trackStateAtPCA().momentum().y(),
              tsAtClosestApproachSeed.trackStateAtPCA().momentum().z());
          // GlobalPoint
          // vSeed(vSeed1.x()-bs.x0(),vSeed1.y()-bs.y0(),vSeed1.z()-bs.z0());
          PerigeeTrajectoryError seedPerigeeErrors =
              PerigeeConversions::ftsToPerigeeError(
                  tsAtClosestApproachSeed.trackStateAtPCA());
          matchedTrackPointer = new reco::Track(
              0., 0., vSeed1, pSeed, 1, seedPerigeeErrors.covarianceMatrix());
          matchedTrackPointer->appendHits(matchedSeedPointer->recHits().first,
                                          matchedSeedPointer->recHits().second);
        }

        double dR = 0;  // fixme: plots vs dR not implemented for now
        histoProducerAlgo_->fill_recoAssociated_simTrack_histos(
            w, *tp, tp->momentum(), tp->vertex(), dxySim, dzSim, nSimHits,
      ...
      //
      edm::LogVerbatim("TrackValidator")
          << "\n# of TrajectorySeeds with " << label[www].process() << ":"
      ...
      TSCBLBuilderNoMaterial tscblBuilder;
      for (TrajectorySeedCollection::size_type i = 0;
           i < seedCollection->size(); ++i) {
        ...
        TransientTrackingRecHit::RecHitPointer recHit =
            theTTRHBuilder->build(&*(seed->recHits().second - 1));
        TrajectoryStateOnSurface state =
            trajectoryStateTransform::transientState(
                seed->startingState(), recHit->surface(), theMF.product());
        TrajectoryStateClosestToBeamLine tsAtClosestApproachSeed = tscblBuilder(
            *state.freeState(), bs);  // as in TrackProducerAlgorithm
        if (!(tsAtClosestApproachSeed.isValid())) {
          edm::LogVerbatim("SeedValidator")
              << "TrajectoryStateClosestToBeamLine not valid";
          continue;
        ...
        // fixme
        reco::Track* trackFromSeed = new reco::Track(
            0., 0., vSeed1, pSeed, 1, seedPerigeeErrors.covarianceMatrix());
        trackFromSeed->appendHits(seed->recHits().first,
                                  seed->recHits().second);
        ...
        if (recSimColl.find(seed) != recSimColl.end()) {
          ...
          if (tp.size() != 0) {
            ...
            isSimMatched = true;
            if (tp[0].first->charge() !=
                seed->startingState().parameters().charge())
              isChargeMatched = false;
            if (simRecColl.find(tp[0].first) != simRecColl.end())
              numAssocSeeds = simRecColl[tp[0].first].size();
        ...
        histoProducerAlgo_->fill_generic_recoTrack_histos(
            w, *trackFromSeed, bs.position(), isSimMatched, isSigSimMatched,
        ...
        try {
          ...
          histoProducerAlgo_->fill_simAssociated_recoTrack_histos(
              w, *trackFromSeed);

          TrackingParticleRef tpr = tp.begin()->first;

          // compute tracking particle parameters at point of closest approach
          // to the beamline
          ...
          // 	  LogTrace("SeedValidatorTEST") << "assocChi2=" <<
          // tp.begin()->second << "\n"
          // 					 << "" <<  "\n"
          // 					 << "ptREC=" << ptSeed << "\n"
          // 					 << "etaREC=" << etaSeed << "\n"
          ...
          histoProducerAlgo_->fill_ResoAndPull_recoTrack_histos(
              w, momentumTP, vertexTP, tpr->charge(), *trackFromSeed,
              bs.position());

        } catch (cms::Exception e) {
          LogTrace("SeedValidator") << "exception found: " << e.what() << "\n";
        }
      }  // End of for(TrajectorySeedCollection::size_type i=0;
         // i<seedCollection->size(); ++i)
