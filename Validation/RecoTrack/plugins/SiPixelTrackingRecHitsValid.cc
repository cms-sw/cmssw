...
SiPixelTrackingRecHitsValid::SiPixelTrackingRecHitsValid(
    ...
    : conf_(ps), dbe_(0), tfile_(0), t_(0) {
  // Read config file
  MTCCtrack_ = ps.getParameter<bool>("MTCCtrack");
  runStandalone = ps.getParameter<bool>("runStandalone");
  outputFile_ = ps.getUntrackedParameter<std::string>(
      "outputFile", "pixeltrackingrechitshisto.root");
  ...
  recoTrackCollectionToken_ = consumes<reco::TrackCollection>(
      edm::InputTag(ps.getUntrackedParameter<std::string>("src")));
  ...
  debugNtuple_ = ps.getUntrackedParameter<std::string>(
      "debugNtuple", "SiPixelTrackingRecHitsValid_Ntuple.root");
}

void SiPixelTrackingRecHitsValid::bookHistograms(DQMStore::IBooker& ibooker,
                                                 const edm::Run& run,
  ...
  ibooker.setCurrentFolder(
      "Tracking/TrackingRecHits/Pixel/"
      "Histograms_per_ring-layer_or_disk-plaquette");

  // Pixel barrel has 3 layers and 8 rings; book a histogram for each module
  // given by the (layer, ring) pair
  for (int i = 0; i < 3; i++)  // loop over layers
  {
    ...
    meResXvsAlphaBarrelFlippedLaddersLayer[i] = ibooker.bookProfile(
        chisto, chisto, 20, barrel_alphal, barrel_alphah, 100, 0.0, resxh, "");
    sprintf(chisto, "meResYvsAlphaBarrelFlippedLaddersLayer_%d", i + 1);
    meResYvsAlphaBarrelFlippedLaddersLayer[i] = ibooker.bookProfile(
        chisto, chisto, 20, barrel_alphal, barrel_alphah, 100, 0.0, resyh, "");
    sprintf(chisto, "meResXvsBetaBarrelFlippedLaddersLayer_%d", i + 1);
    meResXvsBetaBarrelFlippedLaddersLayer[i] = ibooker.bookProfile(
        chisto, chisto, 20, barrel_betal, barrel_betah, 100, 0.0, resxh, "");
    sprintf(chisto, "meResYvsBetaBarrelFlippedLaddersLayer_%d", i + 1);
    meResYvsBetaBarrelFlippedLaddersLayer[i] = ibooker.bookProfile(
        chisto, chisto, 20, barrel_betal, barrel_betah, 100, 0.0, resyh, "");

    sprintf(chisto, "meResXvsAlphaBarrelNonFlippedLaddersLayer_%d", i + 1);
    meResXvsAlphaBarrelNonFlippedLaddersLayer[i] = ibooker.bookProfile(
        chisto, chisto, 20, barrel_alphal, barrel_alphah, 100, 0.0, resxh, "");
    sprintf(chisto, "meResYvsAlphaBarrelNonFlippedLaddersLayer_%d", i + 1);
    meResYvsAlphaBarrelNonFlippedLaddersLayer[i] = ibooker.bookProfile(
        chisto, chisto, 20, barrel_alphal, barrel_alphah, 100, 0.0, resyh, "");
    sprintf(chisto, "meResXvsBetaBarrelNonFlippedLaddersLayer_%d", i + 1);
    meResXvsBetaBarrelNonFlippedLaddersLayer[i] = ibooker.bookProfile(
        chisto, chisto, 20, barrel_betal, barrel_betah, 100, 0.0, resxh, "");
    sprintf(chisto, "meResYvsBetaBarrelNonFlippedLaddersLayer_%d", i + 1);
    meResYvsBetaBarrelNonFlippedLaddersLayer[i] = ibooker.bookProfile(
        chisto, chisto, 20, barrel_betal, barrel_betah, 100, 0.0, resyh, "");

    for (int j = 0; j < 8; j++)  // loop over rings
    {
      sprintf(chisto, "mePosxBarrelLayerModule_%d_%d", i + 1, j + 1);
      mePosxBarrelLayerModule[i][j] =
          ibooker.book1D(chisto, chisto, 100, xl, xh);
      sprintf(chisto, "mePosyBarrelLayerModule_%d_%d", i + 1, j + 1);
      mePosyBarrelLayerModule[i][j] =
          ibooker.book1D(chisto, chisto, 100, yl, yh);
      sprintf(chisto, "meErrxBarrelLayerModule_%d_%d", i + 1, j + 1);
      meErrxBarrelLayerModule[i][j] =
          ibooker.book1D(chisto, chisto, 100, errxl, errxh);
      sprintf(chisto, "meErryBarrelLayerModule_%d_%d", i + 1, j + 1);
      meErryBarrelLayerModule[i][j] =
          ibooker.book1D(chisto, chisto, 100, erryl, erryh);
      sprintf(chisto, "meResxBarrelLayerModule_%d_%d", i + 1, j + 1);
      meResxBarrelLayerModule[i][j] =
          ibooker.book1D(chisto, chisto, 100, resxl, resxh);
      sprintf(chisto, "meResyBarrelLayerModule_%d_%d", i + 1, j + 1);
      meResyBarrelLayerModule[i][j] =
          ibooker.book1D(chisto, chisto, 100, resyl, resyh);
      sprintf(chisto, "mePullxBarrelLayerModule_%d_%d", i + 1, j + 1);
      mePullxBarrelLayerModule[i][j] =
          ibooker.book1D(chisto, chisto, 100, pullxl, pullxh);
      sprintf(chisto, "mePullyBarrelLayerModule_%d_%d", i + 1, j + 1);
      mePullyBarrelLayerModule[i][j] =
          ibooker.book1D(chisto, chisto, 100, pullyl, pullyh);
      sprintf(chisto, "meNpixBarrelLayerModule_%d_%d", i + 1, j + 1);
      meNpixBarrelLayerModule[i][j] =
          ibooker.book1D(chisto, chisto, 100, npixl, npixh);
      sprintf(chisto, "meNxpixBarrelLayerModule_%d_%d", i + 1, j + 1);
      meNxpixBarrelLayerModule[i][j] =
          ibooker.book1D(chisto, chisto, 100, nxpixl, nxpixh);
      sprintf(chisto, "meNypixBarrelLayerModule_%d_%d", i + 1, j + 1);
      meNypixBarrelLayerModule[i][j] =
          ibooker.book1D(chisto, chisto, 100, nypixl, nypixh);
      sprintf(chisto, "meChargeBarrelLayerModule_%d_%d", i + 1, j + 1);
      meChargeBarrelLayerModule[i][j] =
          ibooker.book1D(chisto, chisto, 100, barrel_chargel, barrel_chargeh);

      sprintf(chisto, "meResXvsAlphaBarrelLayerModule_%d_%d", i + 1, j + 1);
      meResXvsAlphaBarrelLayerModule[i][j] =
          ibooker.bookProfile(chisto, chisto, 20, barrel_alphal, barrel_alphah,
                              100, 0.0, resxh, "");
      sprintf(chisto, "meResYvsAlphaBarrelLayerModule_%d_%d", i + 1, j + 1);
      meResYvsAlphaBarrelLayerModule[i][j] =
          ibooker.bookProfile(chisto, chisto, 20, barrel_alphal, barrel_alphah,
                              100, 0.0, resyh, "");
      sprintf(chisto, "meResXvsBetaBarrelLayerModule_%d_%d", i + 1, j + 1);
      meResXvsBetaBarrelLayerModule[i][j] = ibooker.bookProfile(
          chisto, chisto, 20, barrel_betal, barrel_betah, 100, 0.0, resxh, "");
      sprintf(chisto, "meResYvsBetaBarrelLayerModule_%d_%d", i + 1, j + 1);
      meResYvsBetaBarrelLayerModule[i][j] = ibooker.bookProfile(
          chisto, chisto, 20, barrel_betal, barrel_betah, 100, 0.0, resyh, "");

      sprintf(chisto, "mePullXvsAlphaBarrelLayerModule_%d_%d", i + 1, j + 1);
      mePullXvsAlphaBarrelLayerModule[i][j] =
          ibooker.bookProfile(chisto, chisto, 20, pull_barrel_alphal,
                              pull_barrel_alphah, 100, pullxl, pullxh, "");
      sprintf(chisto, "mePullYvsAlphaBarrelLayerModule_%d_%d", i + 1, j + 1);
      mePullYvsAlphaBarrelLayerModule[i][j] =
          ibooker.bookProfile(chisto, chisto, 20, pull_barrel_alphal,
                              pull_barrel_alphah, 100, pullyl, pullyh, "");
      sprintf(chisto, "mePullXvsBetaBarrelLayerModule_%d_%d", i + 1, j + 1);
      mePullXvsBetaBarrelLayerModule[i][j] =
          ibooker.bookProfile(chisto, chisto, 20, pull_barrel_betal,
                              pull_barrel_betah, 100, pullxl, pullxh, "");
      sprintf(chisto, "mePullYvsBetaBarrelLayerModule_%d_%d", i + 1, j + 1);
      mePullYvsBetaBarrelLayerModule[i][j] =
          ibooker.bookProfile(chisto, chisto, 20, pull_barrel_betal,
                              pull_barrel_betah, 100, pullyl, pullyh, "");
      sprintf(chisto, "mePullXvsPhiBarrelLayerModule_%d_%d", i + 1, j + 1);
      mePullXvsPhiBarrelLayerModule[i][j] =
          ibooker.bookProfile(chisto, chisto, 20, pull_barrel_phil,
                              pull_barrel_phih, 100, pullxl, pullxh, "");
      sprintf(chisto, "mePullYvsPhiBarrelLayerModule_%d_%d", i + 1, j + 1);
      mePullYvsPhiBarrelLayerModule[i][j] =
          ibooker.bookProfile(chisto, chisto, 20, pull_barrel_phil,
                              pull_barrel_phih, 100, pullyl, pullyh, "");
      sprintf(chisto, "mePullXvsEtaBarrelLayerModule_%d_%d", i + 1, j + 1);
      mePullXvsEtaBarrelLayerModule[i][j] =
          ibooker.bookProfile(chisto, chisto, 20, pull_barrel_etal,
                              pull_barrel_etah, 100, pullxl, pullxh, "");
      sprintf(chisto, "mePullYvsEtaBarrelLayerModule_%d_%d", i + 1, j + 1);
      mePullYvsEtaBarrelLayerModule[i][j] =
          ibooker.bookProfile(chisto, chisto, 20, pull_barrel_etal,
                              pull_barrel_etah, 100, pullyl, pullyh, "");
    }  //  for (int j=0; j<8; j++) // loop over rings
  ...
  for (int i = 0; i < 2; i++)    // loop over disks
    for (int j = 0; j < 4; j++)  // loop over plaquettes
    {
      ...
      meResXvsBetaZpPanel1DiskPlaq[i][j] =
          ibooker.bookProfile(chisto, chisto, 20, forward_pos_betal,
                              forward_pos_betah, 100, 0.0, resxh, "");
      sprintf(chisto, "meResYvsBetaZpPanel1DiskPlaq_%d_%d", i + 1, j + 1);
      meResYvsBetaZpPanel1DiskPlaq[i][j] =
          ibooker.bookProfile(chisto, chisto, 20, forward_pos_betal,
                              forward_pos_betah, 100, 0.0, resyh, "");
      ...
      mePullXvsBetaZpPanel1DiskPlaq[i][j] =
          ibooker.bookProfile(chisto, chisto, 20, pull_forward_pos_betal,
                              pull_forward_pos_betah, 100, pullxl, pullxh, "");
      sprintf(chisto, "mePullYvsBetaZpPanel1DiskPlaq_%d_%d", i + 1, j + 1);
      mePullYvsBetaZpPanel1DiskPlaq[i][j] =
          ibooker.bookProfile(chisto, chisto, 20, pull_forward_pos_betal,
                              pull_forward_pos_betah, 100, pullyl, pullyh, "");
      ...
      mePullXvsAlphaZmPanel2DiskPlaq[i][j] =
          ibooker.bookProfile(chisto, chisto, 20, pull_forward_p2_alphal,
                              pull_forward_p2_alphah, 100, pullxl, pullxh, "");
      sprintf(chisto, "mePullYvsAlphaZmPanel2DiskPlaq_%d_%d", i + 1, j + 1);
      mePullYvsAlphaZmPanel2DiskPlaq[i][j] =
          ibooker.bookProfile(chisto, chisto, 20, pull_forward_p2_alphal,
                              pull_forward_p2_alphah, 100, pullyl, pullyh, "");
  ...
  meResXvsAlphaBarrel = ibooker.bookProfile(chisto, chisto, 20, barrel_alphal,
                                            barrel_alphah, 100, 0.0, resxh, "");
  sprintf(chisto, "meResYvsAlphaBarrel");
  meResYvsAlphaBarrel = ibooker.bookProfile(chisto, chisto, 20, barrel_alphal,
                                            barrel_alphah, 100, 0.0, resyh, "");
  sprintf(chisto, "meResXvsBetaBarrel");
  meResXvsBetaBarrel = ibooker.bookProfile(chisto, chisto, 20, barrel_betal,
                                           barrel_betah, 100, 0.0, resxh, "");
  sprintf(chisto, "meResYvsBetaBarrel");
  meResYvsBetaBarrel = ibooker.bookProfile(chisto, chisto, 20, barrel_betal,
                                           barrel_betah, 100, 0.0, resyh, "");
  ...
  meResXvsAlphaBarrelFlippedLadders = ibooker.bookProfile(
      chisto, chisto, 20, barrel_alphal, barrel_alphah, 100, 0.0, resxh, "");
  sprintf(chisto, "meResYvsAlphaBarrelFlippedLadders");
  meResYvsAlphaBarrelFlippedLadders = ibooker.bookProfile(
      chisto, chisto, 20, barrel_alphal, barrel_alphah, 100, 0.0, resyh, "");
  sprintf(chisto, "meResXvsBetaBarrelFlippedLadders");
  meResXvsBetaBarrelFlippedLadders = ibooker.bookProfile(
      chisto, chisto, 20, barrel_betal, barrel_betah, 100, 0.0, resxh, "");
  sprintf(chisto, "meResYvsBetaBarrelFlippedLadders");
  meResYvsBetaBarrelFlippedLadders = ibooker.bookProfile(
      chisto, chisto, 20, barrel_betal, barrel_betah, 100, 0.0, resyh, "");
  ...
  meResXvsAlphaBarrelNonFlippedLadders = ibooker.bookProfile(
      chisto, chisto, 20, barrel_alphal, barrel_alphah, 100, 0.0, resxh, "");
  sprintf(chisto, "meResYvsAlphaBarrelNonFlippedLadders");
  meResYvsAlphaBarrelNonFlippedLadders = ibooker.bookProfile(
      chisto, chisto, 20, barrel_alphal, barrel_alphah, 100, 0.0, resyh, "");
  sprintf(chisto, "meResXvsBetaBarrelNonFlippedLadders");
  meResXvsBetaBarrelNonFlippedLadders = ibooker.bookProfile(
      chisto, chisto, 20, barrel_betal, barrel_betah, 100, 0.0, resxh, "");
  sprintf(chisto, "meResYvsBetaBarrelNonFlippedLadders");
  meResYvsBetaBarrelNonFlippedLadders = ibooker.bookProfile(
      chisto, chisto, 20, barrel_betal, barrel_betah, 100, 0.0, resyh, "");
  ...
  // control histograms
  meTracksPerEvent =
      ibooker.book1D("meTracksPerEvent", "meTracksPerEvent", 200, 0.0, 200.0);
  mePixRecHitsPerTrack = ibooker.book1D("mePixRecHitsPerTrack",
...
void SiPixelTrackingRecHitsValid::analyze(const edm::Event& e,
                                          const edm::EventSetup& es) {
  ...
  if (!MTCCtrack_) {
    ...
    for (TrackerGeometry::DetContainer::const_iterator it = pDD->dets().begin();
      ...
      if (!((subid == 1) || (subid == 2))) continue;  // end subid if
      ...
      for (; pixeliter != pixelrechitRangeIteratorEnd; ++pixeliter) {
        ...
        } else if ((int)detId.subdetId() ==
                   (int)PixelSubdetector::PixelEndcap) {
    ...
    if (tracks->size() > 0) {
      ...
      for (tciter = tracks->begin(); tciter != tracks->end(); tciter++) {
        ...
        for (trackingRecHit_iterator it = tciter->recHitsBegin();
             it != tciter->recHitsEnd(); it++) {
          const TrackingRecHit& thit = **it;
          // Is it a matched hit?
          const SiPixelRecHit* matchedhit =
              dynamic_cast<const SiPixelRecHit*>(&thit);
          ...
          if (matchedhit) {
            ...
            if (!matched.empty()) {
              ...
              for (std::vector<PSimHit>::const_iterator m = matched.begin();
                ...
                float simhitx =
                    0.5 * ((*m).entryPoint().x() + (*m).exitPoint().x());
                float simhity =
                    0.5 * ((*m).entryPoint().y() + (*m).exitPoint().y());
              ...
              // This recHit does not have any simHit with the same particleType
              // as the particles generated
              // Ignore it as most probably come from delta rays.
              if (checkType_ && !found_hit_from_generated_particle) continue;

              if (n_assoc_muon > 1) {
                edm::LogWarning("SiPixelTrackingRecHitsValid")
                    << " ----- This is not good: n_assoc_muon = "
                    << n_assoc_muon;
                edm::LogWarning("SiPixelTrackingRecHitsValid")
                    << "evt = " << evt;
              }
              ...
              simhitx = 0.5 * ((*closestit).entryPoint().x() +
                               (*closestit).exitPoint().x());
              simhity = 0.5 * ((*closestit).entryPoint().y() +
                               (*closestit).exitPoint().y());
              ...
              if ((int)detId.subdetId() == (int)PixelSubdetector::PixelBarrel) {
                ...
                float tmp1 = theGeomDet->surface()
                                 .toGlobal(Local3DPoint(0., 0., 0.))
                                 .perp();
                float tmp2 = theGeomDet->surface()
                                 .toGlobal(Local3DPoint(0., 0., 1.))
                                 .perp();
                ...
                if (tmp2 < tmp1) {  // flipped
                  ...
                  meResXvsAlphaBarrelFlippedLadders->Fill(alpha,
                                                          fabs(rechitresx));
                  meResYvsAlphaBarrelFlippedLadders->Fill(alpha,
                                                          fabs(rechitresy));
                  meResXvsBetaBarrelFlippedLadders->Fill(beta,
                                                         fabs(rechitresx));
                  meResYvsBetaBarrelFlippedLadders->Fill(beta,
                                                         fabs(rechitresy));
                  ...
                  meWPullXvsAlphaBarrelFlippedLadders->Fill(alpha,
                                                            fabs(rechitpullx));
                  meWPullYvsAlphaBarrelFlippedLadders->Fill(alpha,
                                                            fabs(rechitpully));
                  meWPullXvsBetaBarrelFlippedLadders->Fill(beta,
                                                           fabs(rechitpullx));
                  meWPullYvsBetaBarrelFlippedLadders->Fill(beta,
                                                           fabs(rechitpully));
                } else {  // not flipped
                  ...
                  meResXvsAlphaBarrelNonFlippedLadders->Fill(alpha,
                                                             fabs(rechitresx));
                  meResYvsAlphaBarrelNonFlippedLadders->Fill(alpha,
                                                             fabs(rechitresy));
                  meResXvsBetaBarrelNonFlippedLadders->Fill(beta,
                                                            fabs(rechitresx));
                  meResYvsBetaBarrelNonFlippedLadders->Fill(beta,
                                                            fabs(rechitresy));
                  mePullXvsAlphaBarrelNonFlippedLadders->Fill(alpha,
                                                              rechitpullx);
                  mePullYvsAlphaBarrelNonFlippedLadders->Fill(alpha,
                                                              rechitpully);
                  ...
                  meWPullXvsAlphaBarrelNonFlippedLadders->Fill(
                      alpha, fabs(rechitpullx));
                  meWPullYvsAlphaBarrelNonFlippedLadders->Fill(
                      alpha, fabs(rechitpully));
                  meWPullXvsBetaBarrelNonFlippedLadders->Fill(
                      beta, fabs(rechitpullx));
                  meWPullYvsBetaBarrelNonFlippedLadders->Fill(
                      beta, fabs(rechitpully));
                }
                ...
                meResXvsAlphaBarrelLayerModule[layer - 1][mod - 1]->Fill(
                    alpha, fabs(rechitresx));
                meResYvsAlphaBarrelLayerModule[layer - 1][mod - 1]->Fill(
                    alpha, fabs(rechitresy));
                meResXvsBetaBarrelLayerModule[layer - 1][mod - 1]->Fill(
                    beta, fabs(rechitresx));
                meResYvsBetaBarrelLayerModule[layer - 1][mod - 1]->Fill(
                    beta, fabs(rechitresy));
                mePullXvsAlphaBarrelLayerModule[layer - 1][mod - 1]->Fill(
                    alpha, rechitpullx);
                mePullYvsAlphaBarrelLayerModule[layer - 1][mod - 1]->Fill(
                    alpha, rechitpully);
                mePullXvsBetaBarrelLayerModule[layer - 1][mod - 1]->Fill(
                    beta, rechitpullx);
                mePullYvsBetaBarrelLayerModule[layer - 1][mod - 1]->Fill(
                    beta, rechitpully);
                mePullXvsPhiBarrelLayerModule[layer - 1][mod - 1]->Fill(
                    phi, rechitpullx);
                mePullYvsPhiBarrelLayerModule[layer - 1][mod - 1]->Fill(
                    phi, rechitpully);
                mePullXvsEtaBarrelLayerModule[layer - 1][mod - 1]->Fill(
                    eta, rechitpullx);
                mePullYvsEtaBarrelLayerModule[layer - 1][mod - 1]->Fill(
                    eta, rechitpully);
                ...
                if (tmp2 < tmp1) {  // flipped
                  meResXvsAlphaBarrelFlippedLaddersLayer[layer - 1]->Fill(
                      alpha, fabs(rechitresx));
                  meResYvsAlphaBarrelFlippedLaddersLayer[layer - 1]->Fill(
                      alpha, fabs(rechitresy));
                  meResXvsBetaBarrelFlippedLaddersLayer[layer - 1]->Fill(
                      beta, fabs(rechitresx));
                  meResYvsBetaBarrelFlippedLaddersLayer[layer - 1]->Fill(
                      beta, fabs(rechitresy));
                } else {  // not flipped
                  meResXvsAlphaBarrelNonFlippedLaddersLayer[layer - 1]->Fill(
                      alpha, fabs(rechitresx));
                  meResYvsAlphaBarrelNonFlippedLaddersLayer[layer - 1]->Fill(
                      alpha, fabs(rechitresy));
                  meResXvsBetaBarrelNonFlippedLaddersLayer[layer - 1]->Fill(
                      beta, fabs(rechitresx));
                  meResYvsBetaBarrelNonFlippedLaddersLayer[layer - 1]->Fill(
                      beta, fabs(rechitresy));
                }
                ...
                if (side == 1) {
                  if (panel == 1) {
                    ...
                    meErrxZmPanel1DiskPlaq[disk - 1][plaq - 1]->Fill(
                        rechiterrx);
                    meErryZmPanel1DiskPlaq[disk - 1][plaq - 1]->Fill(
                        rechiterry);
                    meResxZmPanel1DiskPlaq[disk - 1][plaq - 1]->Fill(
                        rechitresx);
                    meResyZmPanel1DiskPlaq[disk - 1][plaq - 1]->Fill(
                        rechitresy);
                    mePullxZmPanel1DiskPlaq[disk - 1][plaq - 1]->Fill(
                        rechitpullx);
                    mePullyZmPanel1DiskPlaq[disk - 1][plaq - 1]->Fill(
                        rechitpully);
                    ...
                    meResXvsAlphaZmPanel1DiskPlaq[disk - 1][plaq - 1]->Fill(
                        alpha, fabs(rechitresx));
                    meResYvsAlphaZmPanel1DiskPlaq[disk - 1][plaq - 1]->Fill(
                        alpha, fabs(rechitresy));
                    meResXvsBetaZmPanel1DiskPlaq[disk - 1][plaq - 1]->Fill(
                        beta, fabs(rechitresx));
                    meResYvsBetaZmPanel1DiskPlaq[disk - 1][plaq - 1]->Fill(
                        beta, fabs(rechitresy));
                    mePullXvsAlphaZmPanel1DiskPlaq[disk - 1][plaq - 1]->Fill(
                        alpha, rechitpullx);
                    mePullYvsAlphaZmPanel1DiskPlaq[disk - 1][plaq - 1]->Fill(
                        alpha, rechitpully);
                    mePullXvsBetaZmPanel1DiskPlaq[disk - 1][plaq - 1]->Fill(
                        beta, rechitpullx);
                    mePullYvsBetaZmPanel1DiskPlaq[disk - 1][plaq - 1]->Fill(
                        beta, rechitpully);
                    mePullXvsPhiZmPanel1DiskPlaq[disk - 1][plaq - 1]->Fill(
                        phi, rechitpullx);
                    mePullYvsPhiZmPanel1DiskPlaq[disk - 1][plaq - 1]->Fill(
                        phi, rechitpully);
                    mePullXvsEtaZmPanel1DiskPlaq[disk - 1][plaq - 1]->Fill(
                        eta, rechitpullx);
                    mePullYvsEtaZmPanel1DiskPlaq[disk - 1][plaq - 1]->Fill(
                        eta, rechitpully);
                    ...
                    meErrxZmPanel2DiskPlaq[disk - 1][plaq - 1]->Fill(
                        rechiterrx);
                    meErryZmPanel2DiskPlaq[disk - 1][plaq - 1]->Fill(
                        rechiterry);
                    meResxZmPanel2DiskPlaq[disk - 1][plaq - 1]->Fill(
                        rechitresx);
                    meResyZmPanel2DiskPlaq[disk - 1][plaq - 1]->Fill(
                        rechitresy);
                    mePullxZmPanel2DiskPlaq[disk - 1][plaq - 1]->Fill(
                        rechitpullx);
                    mePullyZmPanel2DiskPlaq[disk - 1][plaq - 1]->Fill(
                        rechitpully);
                    ...
                    meResXvsAlphaZmPanel2DiskPlaq[disk - 1][plaq - 1]->Fill(
                        alpha, fabs(rechitresx));
                    meResYvsAlphaZmPanel2DiskPlaq[disk - 1][plaq - 1]->Fill(
                        alpha, fabs(rechitresy));
                    meResXvsBetaZmPanel2DiskPlaq[disk - 1][plaq - 1]->Fill(
                        beta, fabs(rechitresx));
                    meResYvsBetaZmPanel2DiskPlaq[disk - 1][plaq - 1]->Fill(
                        beta, fabs(rechitresy));
                    mePullXvsAlphaZmPanel2DiskPlaq[disk - 1][plaq - 1]->Fill(
                        alpha, rechitpullx);
                    mePullYvsAlphaZmPanel2DiskPlaq[disk - 1][plaq - 1]->Fill(
                        alpha, rechitpully);
                    mePullXvsBetaZmPanel2DiskPlaq[disk - 1][plaq - 1]->Fill(
                        beta, rechitpullx);
                    mePullYvsBetaZmPanel2DiskPlaq[disk - 1][plaq - 1]->Fill(
                        beta, rechitpully);
                    mePullXvsPhiZmPanel2DiskPlaq[disk - 1][plaq - 1]->Fill(
                        phi, rechitpullx);
                    mePullYvsPhiZmPanel2DiskPlaq[disk - 1][plaq - 1]->Fill(
                        phi, rechitpully);
                    mePullXvsEtaZmPanel2DiskPlaq[disk - 1][plaq - 1]->Fill(
                        eta, rechitpullx);
                    mePullYvsEtaZmPanel2DiskPlaq[disk - 1][plaq - 1]->Fill(
                        eta, rechitpully);
                  ...
                  if (panel == 1) {
                    ...
                    meErrxZpPanel1DiskPlaq[disk - 1][plaq - 1]->Fill(
                        rechiterrx);
                    meErryZpPanel1DiskPlaq[disk - 1][plaq - 1]->Fill(
                        rechiterry);
                    meResxZpPanel1DiskPlaq[disk - 1][plaq - 1]->Fill(
                        rechitresx);
                    meResyZpPanel1DiskPlaq[disk - 1][plaq - 1]->Fill(
                        rechitresy);
                    mePullxZpPanel1DiskPlaq[disk - 1][plaq - 1]->Fill(
                        rechitpullx);
                    mePullyZpPanel1DiskPlaq[disk - 1][plaq - 1]->Fill(
                        rechitpully);
                    ...
                    meResXvsAlphaZpPanel1DiskPlaq[disk - 1][plaq - 1]->Fill(
                        alpha, fabs(rechitresx));
                    meResYvsAlphaZpPanel1DiskPlaq[disk - 1][plaq - 1]->Fill(
                        alpha, fabs(rechitresy));
                    meResXvsBetaZpPanel1DiskPlaq[disk - 1][plaq - 1]->Fill(
                        beta, fabs(rechitresx));
                    meResYvsBetaZpPanel1DiskPlaq[disk - 1][plaq - 1]->Fill(
                        beta, fabs(rechitresy));
                    mePullXvsAlphaZpPanel1DiskPlaq[disk - 1][plaq - 1]->Fill(
                        alpha, rechitpullx);
                    mePullYvsAlphaZpPanel1DiskPlaq[disk - 1][plaq - 1]->Fill(
                        alpha, rechitpully);
                    mePullXvsBetaZpPanel1DiskPlaq[disk - 1][plaq - 1]->Fill(
                        beta, rechitpullx);
                    mePullYvsBetaZpPanel1DiskPlaq[disk - 1][plaq - 1]->Fill(
                        beta, rechitpully);
                    mePullXvsPhiZpPanel1DiskPlaq[disk - 1][plaq - 1]->Fill(
                        phi, rechitpullx);
                    mePullYvsPhiZpPanel1DiskPlaq[disk - 1][plaq - 1]->Fill(
                        phi, rechitpully);
                    mePullXvsEtaZpPanel1DiskPlaq[disk - 1][plaq - 1]->Fill(
                        eta, rechitpullx);
                    mePullYvsEtaZpPanel1DiskPlaq[disk - 1][plaq - 1]->Fill(
                        eta, rechitpully);
                    ...
                    meErrxZpPanel2DiskPlaq[disk - 1][plaq - 1]->Fill(
                        rechiterrx);
                    meErryZpPanel2DiskPlaq[disk - 1][plaq - 1]->Fill(
                        rechiterry);
                    meResxZpPanel2DiskPlaq[disk - 1][plaq - 1]->Fill(
                        rechitresx);
                    meResyZpPanel2DiskPlaq[disk - 1][plaq - 1]->Fill(
                        rechitresy);
                    mePullxZpPanel2DiskPlaq[disk - 1][plaq - 1]->Fill(
                        rechitpullx);
                    mePullyZpPanel2DiskPlaq[disk - 1][plaq - 1]->Fill(
                        rechitpully);
                    ...
                    meResXvsAlphaZpPanel2DiskPlaq[disk - 1][plaq - 1]->Fill(
                        alpha, fabs(rechitresx));
                    meResYvsAlphaZpPanel2DiskPlaq[disk - 1][plaq - 1]->Fill(
                        alpha, fabs(rechitresy));
                    meResXvsBetaZpPanel2DiskPlaq[disk - 1][plaq - 1]->Fill(
                        beta, fabs(rechitresx));
                    meResYvsBetaZpPanel2DiskPlaq[disk - 1][plaq - 1]->Fill(
                        beta, fabs(rechitresy));
                    mePullXvsAlphaZpPanel2DiskPlaq[disk - 1][plaq - 1]->Fill(
                        alpha, rechitpullx);
                    mePullYvsAlphaZpPanel2DiskPlaq[disk - 1][plaq - 1]->Fill(
                        alpha, rechitpully);
                    mePullXvsBetaZpPanel2DiskPlaq[disk - 1][plaq - 1]->Fill(
                        beta, rechitpullx);
                    mePullYvsBetaZpPanel2DiskPlaq[disk - 1][plaq - 1]->Fill(
                        beta, rechitpully);
                    mePullXvsPhiZpPanel2DiskPlaq[disk - 1][plaq - 1]->Fill(
                        phi, rechitpullx);
                    mePullYvsPhiZpPanel2DiskPlaq[disk - 1][plaq - 1]->Fill(
                        phi, rechitpully);
                    mePullXvsEtaZpPanel2DiskPlaq[disk - 1][plaq - 1]->Fill(
                        eta, rechitpullx);
                    mePullYvsEtaZpPanel2DiskPlaq[disk - 1][plaq - 1]->Fill(
                        eta, rechitpully);
            ...
            // else
            // cout << "---------------- RecHit with no associated SimHit !!!
            // -------------------------- " << endl;
