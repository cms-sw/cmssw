...
void MultiTrackValidator::beginRun(Run const&, EventSetup const& setup) {
  ...
  for (unsigned int ww=0;ww<associators.size();ww++){
	...
	h_assocSharedHit.push_back(dbe_->book1D("assocSharedHit","number of shared hits",20,0,20));
      ...
      h_hits_eta.push_back( dbe_->bookProfile("hits_eta","mean #hits vs eta",nint,min,max, 25,0,25) );
      ...
      //      h_ptresmean_vs_eta.push_back( dbe_->book1D("ptres_vs_eta_Mean","mean of p_{t} resolution vs #eta",nint,min,max));
...
void MultiTrackValidator::analyze(const edm::Event& event, const edm::EventSetup& setup){
  ...
  for (unsigned int ww=0;ww<associators.size();ww++){
	...
	int tmp = std::min((int)(tp->trackPSimHit().end()-tp->trackPSimHit().begin()),int(maxHit-1));
	totSIM_hit[w][tmp]++;
