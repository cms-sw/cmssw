...
MultiTrackValidator::MultiTrackValidator(const edm::ParameterSet& pset)
  ...
  // theExtractor = IsoDepositExtractorFactory::get()->create( extractorName,
  // extractorPSet, consumesCollector());

  ParameterSet psetForHistoProducerAlgo =
      pset.getParameter<ParameterSet>("histoProducerAlgoBlock");
  string histoProducerAlgoName =
      psetForHistoProducerAlgo.getParameter<string>("ComponentName");
  histoProducerAlgo_ = MTVHistoProducerAlgoFactory::get()->create(
      histoProducerAlgoName, psetForHistoProducerAlgo, consumesCollector());
  ...
  m_dEdx1Tag = mayConsume<edm::ValueMap<reco::DeDxData> >(
      pset.getParameter<edm::InputTag>("dEdx1Tag"));
  m_dEdx2Tag = mayConsume<edm::ValueMap<reco::DeDxData> >(
      pset.getParameter<edm::InputTag>("dEdx2Tag"));

  tpSelector = TrackingParticleSelector(
      pset.getParameter<double>("ptMinTP"),
      pset.getParameter<double>("minRapidityTP"),
      pset.getParameter<double>("maxRapidityTP"),
      pset.getParameter<double>("tipTP"), pset.getParameter<double>("lipTP"),
  ...
  cosmictpSelector = CosmicTrackingParticleSelector(
      pset.getParameter<double>("ptMinTP"),
      pset.getParameter<double>("minRapidityTP"),
      pset.getParameter<double>("maxRapidityTP"),
      pset.getParameter<double>("tipTP"), pset.getParameter<double>("lipTP"),
  ...
  ParameterSet psetVsEta = psetForHistoProducerAlgo.getParameter<ParameterSet>(
      "TpSelectorForEfficiencyVsEta");
  dRtpSelector = TrackingParticleSelector(
      psetVsEta.getParameter<double>("ptMin"),
  ...
  _simHitTpMapTag =
      mayConsume<SimHitTPAssociationProducer::SimHitTPAssociationList>(
          pset.getParameter<edm::InputTag>("simHitTpMapTag"));

  labelTokenForDrCalculation = consumes<edm::View<reco::Track> >(
      pset.getParameter<edm::InputTag>("trackCollectionForDrCalculation"));
...
void MultiTrackValidator::bookHistograms(DQMStore::IBooker& ibook,
  ...
  for (unsigned int ww = 0; ww < associators.size(); ww++) {
    for (unsigned int www = 0; www < label.size(); www++) {
      ...
      if (algo.process() != "") dirName += algo.process() + "_";
      if (algo.label() != "") dirName += algo.label() + "_";
      if (algo.instance() != "") dirName += algo.instance() + "_";
      ...
      histoProducerAlgo_
          ->initialize();  // TO BE FIXED. I'D LIKE TO AVOID THIS CALL
      ...
      if (runStandalone)
        histoProducerAlgo_->bookRecoHistosForStandaloneRunning(ibook);
...
void MultiTrackValidator::analyze(const edm::Event& event,
  ...
  setup.get<TrackAssociatorRecord>().get(parametersDefiner,
                                         parametersDefinerTPHandle);
  ...
  if (parametersDefiner == "CosmicParametersDefinerForTP") {
    edm::Handle<SimHitTPAssociationProducer::SimHitTPAssociationList>
        simHitsTPAssoc;
    // warning: make sure the TP collection used in the map is the same used in
    // the MTV!
  ...
  for (unsigned int puinfo_ite = 0; puinfo_ite < (*puinfoH).size();
       ++puinfo_ite) {
  ...
  */
  ...
  if (TPCollectionHeff->size()) {
    ...
    for (auto const& tp2 : *TPCollectionHeff) {
      okL[j] = false;
      if (tpSelector(tp2)) {  // calculare dR wrt inclusive collection (also
                              // with PU, low pT, displaced)
        okL[j] = true;
        auto&& p = tp2.momentum();
    ...
    for (auto const& tp : *TPCollectionHeff) {
      ...
      if (dRtpSelector(tp)) {  // only for those needed for efficiency!
        ...
        for (auto j = 0U; j < (*TPCollectionHeff).size(); ++j) {
          ...
          if (okL[j]) {  // calculare dR wrt inclusive collection (also with PU,
                         // low pT, displaced)
  ...
  for (unsigned int ww = 0; ww < associators.size(); ww++) {
    for (unsigned int www = 0; www < label.size(); www++) {
      ...
      if (!event.getByToken(labelToken[www], trackCollection) &&
      ...
      if (UseAssociators) {
        ...
        LogTrace("TrackValidator") << "Calling associateRecoToSim method"
                                   << "\n";
        recSimCollL = std::move(associator[ww]->associateRecoToSim(
            trackCollection, TPCollectionHfake, &event, &setup));
        recSimCollP = &recSimCollL;
        LogTrace("TrackValidator") << "Calling associateSimToReco method"
                                   << "\n";
        simRecCollL = std::move(associator[ww]->associateSimToReco(
            trackCollection, TPCollectionHeff, &event, &setup));
        simRecCollP = &simRecCollL;
      } else {
        edm::LogVerbatim("TrackValidator")
            << "Analyzing " << label[www].process() << ":" << label[www].label()
            << ":" << label[www].instance() << " with " << assMapInput.process()
            << ":" << assMapInput.label() << ":" << assMapInput.instance()
            << "\n";
      ...
      int ats(0);  // This counter counts the number of simTracks that are
                   // "associated" to recoTracks
      int st(0);  // This counter counts the number of simulated tracks passing
                  // the MTV selection (i.e. tpSelector(tp) )
      unsigned sts(0);  // This counter counts the number of simTracks surviving
                        // the bunchcrossing cut
      unsigned asts(0);  // This counter counts the number of simTracks that are
                         // "associated" to recoTracks surviving the
                         // bunchcrossing cut
      for (TrackingParticleCollection::size_type i = 0; i < tPCeff.size();
           i++) {  // loop over TPs collection for tracking efficiency
        TrackingParticleRef tpr(TPCollectionHeff, i);
        TrackingParticle* tp =
            const_cast<TrackingParticle*>(tpr.get());  // why????
        ...
        //---------- THIS PART HAS TO BE CLEANED UP. THE PARAMETER DEFINER WAS
        //NOT MEANT TO BE USED IN THIS WAY ----------
        // If the TrackingParticle is collison like, get the momentum and vertex
        // at production state
        if (parametersDefiner == "LhcParametersDefinerForTP" ||
            parametersDefiner == "hltLhcParametersDefinerForTP") {
          if (!tpSelector(*tp)) continue;
          momentumTP = tp->momentum();
          vertexTP = tp->vertex();
          // Calcualte the impact parameters w.r.t. PCA
          TrackingParticle::Vector momentum =
              parametersDefinerTP->momentum(event, setup, tpr);
          TrackingParticle::Point vertex =
              parametersDefinerTP->vertex(event, setup, tpr);
          dxySim = (-vertex.x() * sin(momentum.phi()) +
                    vertex.y() * cos(momentum.phi()));
          dzSim = vertex.z() -
                  (vertex.x() * momentum.x() + vertex.y() * momentum.y()) /
        ...
        if (parametersDefiner == "CosmicParametersDefinerForTP") {
          ...
          dxySim = (-vertexTP.x() * sin(momentumTP.phi()) +
                    vertexTP.y() * cos(momentumTP.phi()));
          dzSim =
              vertexTP.z() -
              (vertexTP.x() * momentumTP.x() + vertexTP.y() * momentumTP.y()) /
                  sqrt(momentumTP.perp2()) * momentumTP.z() /
                  sqrt(momentumTP.perp2());
        }
        //---------- THE PART ABOVE HAS TO BE CLEANED UP. THE PARAMETER DEFINER
        //WAS NOT MEANT TO BE USED IN THIS WAY ----------

        st++;  // This counter counts the number of simulated tracks passing the
               // MTV selection (i.e. tpSelector(tp) )
        ...
        histoProducerAlgo_->fill_generic_simTrack_histos(
            w, momentumTP, vertexTP, tp->eventId().bunchCrossing());
        ...
        if (simRecColl.find(tpr) != simRecColl.end()) {
          ...
          if (rt.size() != 0) {
            ats++;  // This counter counts the number of simTracks that have a
                    // recoTrack associated
          ...
          edm::LogVerbatim("TrackValidator")
              << "TrackingParticle #" << st
              << " with pt,eta,phi: " << sqrt(momentumTP.perp2()) << " , "
        ...
        histoProducerAlgo_->fill_recoAssociated_simTrack_histos(
            w, *tp, momentumTP, vertexTP, dxySim, dzSim, nSimHits,
            matchedTrackPointer, puinfo.getPU_NumInteractions(), dR);
        sts++;
        if (matchedTrackPointer) asts++;

      }  // End  for (TrackingParticleCollection::size_type i=0;
         // i<tPCeff.size(); i++){
      ...
      int sat(0);  // This counter counts the number of recoTracks that are
                   // associated to SimTracks from Signal only
      int at(0);  // This counter counts the number of recoTracks that are
                  // associated to SimTracks
      int rT(0);  // This counter counts the number of recoTracks in general

      // dE/dx
      // at some point this could be generalized, with a vector of tags and a
      // corresponding vector of Handles
      ...
      for (View<Track>::size_type i = 0; i < trackCollection->size(); ++i) {
        ...
        float eta = etaFromXYZ(p.x(), p.y(), p.z());
        float phi = atan2f(p.y(), p.x());
        for (View<Track>::size_type j = 0;
             j < trackCollectionForDrCalculation->size(); ++j) {
          auto dR_tmp = reco::deltaR2(eta, phi, etaL[j], phiL[j]);
      ...
      for (View<Track>::size_type i = 0; i < trackCollection->size(); ++i) {
        ...
        auto const& tp = (recSimColl.find(track) != recSimColl.end())
                             ? recSimColl[track]
                             : dummyTP;
        ...
        if (!tp.empty()) {
          ...
          for (unsigned int tp_ite = 0; tp_ite < tp.size(); ++tp_ite) {
            ...
            if ((trackpart.eventId().event() == 0) &&
                (trackpart.eventId().bunchCrossing() == 0)) {
          ...
          edm::LogVerbatim("TrackValidator")
              << "reco::Track #" << rT << " with pt=" << track->pt()
        ...
        double dR = dR_trk[i];
        histoProducerAlgo_->fill_generic_recoTrack_histos(
            w, *track, bs.position(), isSimMatched, isSigSimMatched,
            isChargeMatched, numAssocRecoTracks, puinfo.getPU_NumInteractions(),
            nSimHits, sharedFraction, dR);

        // dE/dx
        //	reco::TrackRef track2  = reco::TrackRef( trackCollection, i );
        if (v_dEdx.size() > 0)
          histoProducerAlgo_->fill_dedx_recoTrack_histos(w, track, v_dEdx);
        // if (v_dEdx.size() > 0)
        ...
        if (associators[ww]=="TrackAssociatorByChi2"){
          //association chi2
          double assocChi2 = -tp.begin()->second;//in association map is stored
        -chi2
        ...
        */
        ...
        // Get tracking particle parameters at point of closest approach to the
        // beamline
        TrackingParticle::Vector momentumTP =
            parametersDefinerTP->momentum(event, setup, tpr);
        TrackingParticle::Point vertexTP =
        ...
        histoProducerAlgo_->fill_ResoAndPull_recoTrack_histos(
            w, momentumTP, vertexTP, chargeTP, *track, bs.position());
