...
void TrackValidator::analyze(const edm::Event& event, const edm::EventSetup& setup){
  ...
  for (unsigned int w=0;w<label.size();w++){
    ...
    //       edm::Handle<edm::PCaloHitContainer>  TVCollectionH ;
    //       event.getByLabel("g4SimHits","MuonDTHits",TVCollectionH);
    //       const edm::PCaloHitContainer tVC = *(TVCollectionH.product());
    ...
    for (TrackAssociatorByChi2::RecoToSimPairAssociation::iterator vit=q.begin();vit!=q.end();++vit){
      ...
      SimTrack * assTrack = &(vit->second.begin()->second);
      ...
      double ptres=track->pt()-assTrack->momentum().perp(); 
      double etares=track->eta()-assTrack->momentum().pseudoRapidity();
      double thetares=(track->theta()-assTrack->momentum().theta())/track->thetaError();
      double phi0res=(track->phi0()-assTrack->momentum().phi())/track->phi0Error();
      double d0res=(track->d0()-simVC[assTrack->vertIndex()].position().perp())/track->d0Error();
      double dzres=(track->dz()-simVC[assTrack->vertIndex()].position().z())/track->dzError();
      const HepLorentzVector vertexPosition = simVC[assTrack->vertIndex()].position(); 
      GlobalVector magField=theMF->inTesla(GlobalPoint(vertexPosition.x(),vertexPosition.y(),vertexPosition.z()));
      double simTrCurv = -track->charge()*2.99792458e-3 * magField.z()/assTrack->momentum().perp();
	...
	if (abs(assTrack->momentum().pseudoRapidity())>etaintervals[w][i]&&
	    abs(assTrack->momentum().pseudoRapidity())<etaintervals[w][i+1]) {
	  (*h)->Fill(track->pt()-assTrack->momentum().perp());
	}
	i++;
      }
	...
	if (abs(assTrack->momentum().pseudoRapidity())>etaintervals[w][i]&&
	    abs(assTrack->momentum().pseudoRapidity())<etaintervals[w][i+1]) {
	  totSIM[w][i]++;
	  if (abs(track->pt()-assTrack->momentum().perp())<(assTrack->momentum().perp()*0.1)) totREC[w][i]++;
	}
	i++;
      }
    }
