  ...
  if (hitsHE.isValid()) {
    ...
    for (edm::PCaloHitContainer::const_iterator it = hitsHE->begin(); it != hitsHE->end(); ++it) {
      ...
      if (bh) {
        ...
        double eta1 = (eta >= 0) ? (eta + 0.01) : (eta - 0.01);
        hsi2Oc_->Fill(eta1, (phi - 0.01), energy);
        hsimE1_->Fill(energy);
        hsimTm_->Fill(time, energy);
        hsimOc_->Fill(eta1, (phi - 0.01), energy);
    ...
    for (HGCalDigiCollection::const_iterator it = hecoll->begin(); it != hecoll->end(); ++it) {
      ...
      if (bh) {
        ...
        if (energy > threshold_) {
          ...
          int phi = cell.iphi();
          double eta1 = (eta >= 0) ? (eta + 0.01) : (eta - 0.01);
          hdi2Oc_->Fill(eta1, (phi - 0.01));
          if (bh) {
            hdigEn_->Fill(energy);
            hdigOc_->Fill(eta1, (phi - 0.01));
            hdigLn_->Fill(depth);
