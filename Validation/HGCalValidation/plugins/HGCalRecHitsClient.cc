...
void HGCalRecHitsClient::dqmEndJob(DQMStore::IBooker &ib, DQMStore::IGetter &ig) {
  runClient_(ib, ig);
}
...
int HGCalRecHitsClient::recHitsEndjob(const std::vector<MonitorElement*>& hgcalMEs) {
  ...
  for(auto hgcalME : hgcalMEs){
    if (strcmp(hgcalME->getName().c_str(), name.str().c_str()) == 0){
      MeanHitOccupancy_Plus_.push_back(hgcalME);
  ...
  for(auto hgcalME : hgcalMEs){
    if (strcmp(hgcalME->getName().c_str(), name.str().c_str()) == 0){
      MeanHitOccupancy_Minus_.push_back(hgcalME);
