...
void HGCalWaferStudy::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  std::vector<edm::InputTag> digSources = {edm::InputTag("hgcalDigis","EE"),
...
void HGCalWaferStudy::analyze(const edm::Event& iEvent, 
  ...
  for (unsigned int k=0; k<tok_hits_.size(); ++k) {
	...
	if (ifNose_) {
	  ...
	  layer    = detId.layer();
	  zside    = detId.zside();
	  wvtype   = hgcons_[k]->waferVirtual(layer,detId.waferU(),
					      detId.waferV());
	  xy       = hgcons_[k]->locateCell(layer,detId.waferU(),
					    detId.waferV(),detId.cellU(),
					    detId.cellV(),false,true);
	...
	if (verbosity_>1) 
	  edm::LogVerbatim("HGCalValidation") << "Hit [" << kount 
					      << "] Layer " << layer << ":"
					      << ll << " x|y " << xp << ":"
					      << yp << " Type " << wvtype
					      << " Hist pointers " 
	...
	if (layer >= layerMnSim_[k] && layer <= layerMxSim_[k]) {
	  if (wvtype) h_XYsi2_[k][ll]->Fill(xp,yp);
	  else        h_XYsi1_[k][ll]->Fill(xp,yp);
	}
	++kount;
      }
  ...
  for (unsigned int k=0; k<tok_digi_.size(); ++k) {
	...
	edm::LogVerbatim("HGCalValidation") << nameDetectors_[k] << " with " 
					    << theHGCDigiContainer->size() 
					    << " element(s) Geometry Pointer "
					    << hgeoms_[k] << " # of Hists "
					    << h_XYdi1_[k].size() << ":"
					    << h_XYdi2_[k].size();
      int kount(0);
	...
	double yp = gcoord.y();
	if (verbosity_>1) 
	  edm::LogVerbatim("HGCalValidation") << "Digi [" << kount
					      << "] Layer " << layer << ":"
					      << ll << " x|y " << xp << ":"
					      << yp << " Type " << wvtype
					      << " Hist pointers " 
	...
	if (layer >= layerMnDig_[k] && layer <= layerMxDig_[k]) {
	  if (wvtype) h_XYdi2_[k][ll]->Fill(xp,yp);
	  else        h_XYdi1_[k][ll]->Fill(xp,yp);
	}
	++kount;
      }
    } else {
...
void HGCalWaferStudy::beginRun(const edm::Run&, 
			       const edm::EventSetup& iSetup) {
  for (unsigned int k=0; k<nameDetectors_.size(); ++k) {
    ...
    if (verbosity_>0) edm::LogVerbatim("HGCalValidation") 
			<< nameDetectors_[k] << " defined with "
			<< layerFront_[k] << ":" << layerSim_[k]   << ":"
			<< layerDig_[k] << " Layers which gives limits "
			<< layerMnSim_[k] << ":" << layerMxSim_[k] 
			<< " for Sim " << layerMnDig_[k] << ":"
			<< layerMxDig_[k] << " for Digi "
			<< "\nLimits for plots " << nBinHit_ << ":"
			<< xyMinHit_ << ":" << xyMaxHit_ << " (Sim) "
			<< nBinDig_ << ":" << xyMinDig_  << ":" << xyMaxDig_
			<< " (Digi) with Pointers " << hgcons_.back() << ":"
			<< hgeoms_.back();
  }
  ...
  for (unsigned int ih=0; ih <= nameDetectors_.size(); ++ih) {
    for (int i=layerMnSim_[ih]; i<=layerMxSim_[ih]; ++i) {
      ...
      title << "y vs x (Layer " << i << ") real wafers for "
	    << nameDetectors_[ih] << " at SimHit level";
     if (verbosity_>0) edm::LogVerbatim("HGCalValidation") << i << " book " 
							   << name.str() << ":"
							   << title.str();
      h_XYsi1_[ih].emplace_back(fs->make<TH2D>(name.str().c_str(), 
					       title.str().c_str(),
					       nBinHit_,xyMinHit_,xyMaxHit_,
					       nBinHit_,xyMinHit_,xyMaxHit_));
      name.str(""); title.str("");
      name << "XY_" << nameDetectors_[ih] << "L" << i << "SimV";
      title << "y vs x (Layer " << i << ") virtual wafers for " 
	    << nameDetectors_[ih] << " at SimHit level";
     if (verbosity_>0) edm::LogVerbatim("HGCalValidation") << i << " book " 
							   << name.str() << ":"
							   << title.str();
      h_XYsi2_[ih].emplace_back(fs->make<TH2D>(name.str().c_str(), 
					       title.str().c_str(),
					       nBinHit_,xyMinHit_,xyMaxHit_,
					       nBinHit_,xyMinHit_,xyMaxHit_));
    }
    ...
    for (int i=layerMnDig_[ih]; i<=layerMxDig_[ih]; ++i) {
      ...
      title << "y vs x (Layer " << i << ") real wafers for "
	    << nameDetectors_[ih] << " at Digi level";
     if (verbosity_>0) edm::LogVerbatim("HGCalValidation") << i << " book " 
							   << name.str() << ":"
							   << title.str();
      h_XYdi1_[ih].emplace_back(fs->make<TH2D>(name.str().c_str(), 
					       title.str().c_str(),
					       nBinDig_,xyMinDig_,xyMaxDig_,
					       nBinDig_,xyMinDig_,xyMaxDig_));
      name.str(""); title.str("");
     ...
     if (verbosity_>0) edm::LogVerbatim("HGCalValidation") << i << " book " 
							   << name.str() << ":"
							   << title.str();
      h_XYdi2_[ih].emplace_back(fs->make<TH2D>(name.str().c_str(), 
					       title.str().c_str(),
					       nBinDig_,xyMinDig_,xyMaxDig_,
					       nBinDig_,xyMinDig_,xyMaxDig_));
    }
