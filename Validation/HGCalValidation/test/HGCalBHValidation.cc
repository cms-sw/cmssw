...
void HGCalBHValidation::analyze(const edm::Event& e, const edm::EventSetup& ) {
  ...
  if (hitsHcal.isValid()) {
	...
	if (z==0) eta = -eta;
	hbhe = ((subdet == static_cast<int>(HcalEndcap)) ||
	...
	if (bh) {
      ...
      if (hbhe) hsi2Oc_->Fill((eta+0.1),(phi-0.1),energy);
      if (bh) {
	hsimE1_->Fill(energy);
  ...
  if ((geomType_ == 0) && ifHCAL_) {
	...
	bool hbhe = ((cell.subdetId() == static_cast<int>(HcalEndcap)) ||
		     (cell.subdetId() == static_cast<int>(HcalBarrel)));
	if (hbhe) {
...
template<class T>
void HGCalBHValidation::analyzeDigi(const T& cell,double const& energy,
				    bool const& bh, int const& depth, 
				    unsigned int &kount) {
