...
void ElectronMcSignalPostValidator::bookHistograms( DQMStore::IBooker & iBooker, edm::Run const &, edm::EventSetup const & )
 {
  setBookIndex(-1) ; 
  setBookPrefix("h_ele") ;
  ...
  h1_ele_xOverX0VsEta = ElectronDqmAnalyzerBase::bookH1withSumw2(iBooker, "xOverx0VsEta","mean X/X_0 vs eta",etaAxis->GetNbins(),etaAxis->GetXmin(),etaAxis->GetXmax());
  std::cout << "McSignalPost bookHistograms h1_ele_xOverX0VsEta : " << h1_ele_xOverX0VsEta->getTH1F()->GetXaxis()->GetNbins()+1 << " Nbins" << std::endl;
  ...
  ElectronDqmAnalyzerBase::profileX(iBooker,"PoPtrueVsEta","mean ele momentum / gen momentum vs eta","#eta","<P/P_{gen}>");
  ElectronDqmAnalyzerBase::profileX(iBooker,"PoPtrueVsPhi","mean ele momentum / gen momentum vs phi","#phi (rad)","<P/P_{gen}>");
  ElectronDqmAnalyzerBase::profileX(iBooker,"EoEtruePfVsEg","mean pflow sc energy / true energy vs e/g sc energy","E/E_{gen} (e/g)","<E/E_{gen}> (pflow)") ;
  ElectronDqmAnalyzerBase::profileY(iBooker,"EoEtruePfVsEg","mean e/g sc energy / true energy vs pflow sc energy","E/E_{gen} (pflow)","<E/E_{gen}> (eg)") ;
  ElectronDqmAnalyzerBase::profileX(iBooker,"EtaMnEtaTrueVsEta","mean ele eta - gen eta vs eta","#eta","<#eta_{rec} - #eta_{gen}>");
  ElectronDqmAnalyzerBase::profileX(iBooker,"EtaMnEtaTrueVsPhi","mean ele eta - gen eta vs phi","#phi (rad)","<#eta_{rec} - #eta_{gen}>");
  ElectronDqmAnalyzerBase::profileX(iBooker,"PhiMnPhiTrueVsEta","mean ele phi - gen phi vs eta","#eta","<#phi_{rec} - #phi_{gen}> (rad)");
  ElectronDqmAnalyzerBase::profileX(iBooker,"PhiMnPhiTrueVsPhi","mean ele phi - gen phi vs phi","#phi (rad)","");
  ...
  ElectronDqmAnalyzerBase::profileX(iBooker,"ambiguousTracksVsEta","mean ele # ambiguous tracks  vs eta","#eta","<N_{ambiguous}>");
...
void ElectronMcSignalPostValidator::finalize()
 {
    // --- AC ---
  std::cout << "McSignalPost appel finalize" << std::endl; // to be removed
//  setBookPrefix("h_ele") ;
...
//  std::cout<<"PFX  entries : "<<h2->ElectronDqmAnalyzerBase::profileX(iBooker,)->GetEntries()<<std::endl ;
//  std::cout<<"PFX effective entries : "<<h2->ElectronDqmAnalyzerBase::profileX(iBooker,)->GetEffectiveEntries()<<std::endl ;

  std::cout << "McSignalPost fin appel finalize" << std::endl; // to be removed
