    ...
    if (okGsfFound)
     {
      ...
      if( sclRef.isNonnull() ) {
        ...
        int c = bestGsfElectron.classification();
        if ( c < 100)  h1_scl_EoEmatchingObject_barrel->Fill(sclRef->energy()/moIter->energy());
        if ( c >= 100)  h1_scl_EoEmatchingObject_endcaps->Fill(sclRef->energy()/moIter->energy());
