...
GlobalDigisHistogrammer::GlobalDigisHistogrammer(const edm::ParameterSet &iPSet)
  ...
  // fix for consumes
  GlobalDigisSrc_Token_ = consumes<PGlobalDigi>(
      iPSet.getParameter<edm::InputTag>("GlobalDigisSrc"));
...
void GlobalDigisHistogrammer::bookHistograms(DQMStore::IBooker &ibooker,
  ...
  for (int amend = 0; amend < 19; ++amend) {
    mehSiStripn[amend] =
        ibooker.book1D("hSiStripn_" + SiStripString[amend],
    ...
    mehSiStripADC[amend] =
        ibooker.book1D("hSiStripADC_" + SiStripString[amend],
    ...
    mehSiStripStrip[amend] =
        ibooker.book1D("hSiStripStripADC_" + SiStripString[amend],
  ...
  for (int amend = 0; amend < 4; ++amend) {
    ...
    mehHcalSHE[amend] = ibooker.book1D("hHcalSHE_" + HCalString[amend],
                                       HCalString[amend] + "Cal SHE", 100, 0.0,
                                       SHEUpper[amend]);

    mehHcalSHE[amend]->setAxisTitle("Simulated Hit Energy", 1);
    mehHcalSHE[amend]->setAxisTitle("Count", 2);
    mehHcalAEESHE[amend] = ibooker.book1D(
        "hHcalAEESHE_" + HCalString[amend], HCalString[amend] + "Cal AEE/SHE",
        SHEvAEEnBins[amend], SHEvAEELower[amend], SHEvAEEUpper[amend]);

    mehHcalAEESHE[amend]->setAxisTitle("ADC / SHE", 1);
    mehHcalAEESHE[amend]->setAxisTitle("Count", 2);

    //************  Not sure how to do Profile ME **************
    mehHcalSHEvAEE[amend] = ibooker.bookProfile(
  ...
  for (int amend = 0; amend < 2; ++amend) {
    ...
    //************  Not sure how to do Profile ME **************
    mehEcalSHEvAEESHE[amend] = ibooker.bookProfile(
        "hEcalSHEvAEESHE_" + ECalString[amend],
        ECalString[amend] + "Cal SHE vs. AEE/SHE", 100, (float)0., (float)10.,
        50, (float)0., (float)5., "");
    ...
    mehEcalMultvAEE[amend] = ibooker.bookProfile(
        ...
        ECalString[amend] + "Cal Multi vs. AEE", 100, (float)0., (float)10.,
        400, (float)0., (float)4000., "");
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    mehEScalADC[i] =
        ibooker.book1D("hEcalADC" + ADCNumber[i] + "_ES",
                       "ESCAL  ADC" + ADCNumber[i], 150, 950., 1500.);
  ...
  for (int amend = 0; amend < 7; ++amend) {
    if (amend < 3) {
      mehSiPixeln[amend] =
          ibooker.book1D("hSiPixeln_" + SiPixelString[amend],
  ...
  for (int j = 0; j < 4; ++j) {
    ...
    mehDtMuonTime[j] =
        ibooker.book1D("hDtMuonTime_" + MuonString[j], MuonString[j] + "  Time",
                       300, 400., 1000.);
  ...
  mehCSCStripADC =
      ibooker.book1D("hCSCStripADC", "CSC Strip ADC", 110, 0., 1100.);
  ...
  if (getAllProvenances) {
    ...
    for (unsigned int i = 0; i < EECalDigis.size(); ++i) {
      ...
      if (EECalDigis[i].SHE != 0.) {
        ...
        mehEcalSHEvAEESHE[0]->Fill(EECalDigis[i].AEE / EECalDigis[i].SHE,
                                   EECalDigis[i].SHE, 1);
      }
    }
    ...
    for (unsigned int i = 0; i < EBCalDigis.size(); ++i) {
      ...
      if (EBCalDigis[i].SHE != 0.) {
        ...
        mehEcalSHEvAEESHE[1]->Fill(EBCalDigis[i].AEE / EBCalDigis[i].SHE,
                                   EBCalDigis[i].SHE, 1);
      }
    }
