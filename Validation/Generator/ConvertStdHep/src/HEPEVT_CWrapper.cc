//////////////////////////////////////////////////////////////////////////
// This is a clone of HEPEVT_Wrapper, written by Matt  Dobbs
// Generic Wrapper for the fortran HEPEVT common block
// 
// The static data member initializations must be separate from .h file.
// 
//////////////////////////////////////////////////////////////////////////
...
namespace HepMC {
    ...
    unsigned int HEPEVT_CWrapper::s_sizeof_int = 4;

    unsigned int HEPEVT_CWrapper::s_sizeof_real = sizeof(double);

    unsigned int HEPEVT_CWrapper::s_max_number_entries = 4000;

    ///////////////////
    // Print Methods //
    ///////////////////

    void HEPEVT_CWrapper::print_hepevt( std::ostream& ostr ) 
    {
    ...
    void HEPEVT_CWrapper::print_legend( std::ostream& ostr )
    {
	char outline[81];
    ...
    void HEPEVT_CWrapper::print_hepevt_particle( int i, std::ostream& ostr ) 
    {
    ...
    bool HEPEVT_CWrapper::check_hepevt_consistency( std::ostream& os )
    {
	...
	for ( int i = 1; i <= HEPEVT_CWrapper::number_entries(); ++i ) {
	    // 1. check its mothers
	    int moth1 = HEPEVT_CWrapper::first_parent( i );
	    int moth2 = HEPEVT_CWrapper::last_parent( i );
	    if ( moth2<moth1 ) {
		if ( isConsistent ) {
		    os << header << std::endl;
		    isConsistent = false;
		...
		HEPEVT_CWrapper::print_hepevt_particle( i, os );
	    }
	    for ( int m = moth1; m<=moth2 && m!=0; ++m ) {
		if ( m>HEPEVT_CWrapper::number_entries() || m < 0 ) {
		    if ( isConsistent ) {
			os << header << std::endl;
			isConsistent = false;
			print_legend(os);
		...
		int mChild1 = HEPEVT_CWrapper::first_child(m);
		int mChild2 = HEPEVT_CWrapper::last_child(m);
		...
		if ( i<mChild1 || i>mChild2 ) {
		    ...
		    HEPEVT_CWrapper::print_hepevt_particle( i, os );
		    HEPEVT_CWrapper::print_hepevt_particle( m, os );
		}
	    }
	    // 2. check its daughters
	    int dau1 = HEPEVT_CWrapper::first_child( i );
	    int dau2 = HEPEVT_CWrapper::last_child( i );
	    if ( dau2<dau1 ) {
		if ( isConsistent ) {
		    os << header << std::endl;
		    isConsistent = false;
		...
		if ( d>HEPEVT_CWrapper::number_entries() || d < 0 ) {
		    if ( isConsistent ) {
			os << header << std::endl;
			isConsistent = false;
			print_legend(os);
		...
		int d_moth1 = HEPEVT_CWrapper::first_parent(d);
		int d_moth2 = HEPEVT_CWrapper::last_parent(d);
		// we don't consider null pointers as inconsistent
		...
		if ( i<d_moth1 || i>d_moth2 ) {
		    ...
		    HEPEVT_CWrapper::print_hepevt_particle( i, os );
		    HEPEVT_CWrapper::print_hepevt_particle( d, os );
		}
	    }
	}
	if (!isConsistent) {
	    ...
	    os << "To obtain a file summarizing the inconsistency, you should:"
	       ...
	       << "\n\t HEPEVT_CWrapper::check_hepevt_consistency(myFile); "
	       << "\n\t HEPEVT_CWrapper::print_hepevt(myFile); "
    ...
    void HEPEVT_CWrapper::zero_everything()
    {
	set_event_number( 0 );
