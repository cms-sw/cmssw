...
class JobManager:
    ...
    def Crab_run(self, run, process):
        ...
        for file in CrabList:
            ...
            os.makedirs(Configuration.variables['HomeDirectory']+'DropBox/scratch/'+FileInfo[0]+'/'+FileInfo[1]+'/'+FileInfo[1]+FileInfo[2].split('.')[0]+'/')
            shutil.copyfile(Configuration.variables['HomeDirectory']+'crab/'+file, Configuration.variables['HomeDirectory']+'DropBox/scratch/'+FileInfo[0]+'/'+FileInfo[1]+'/'+FileInfo[1]+FileInfo[2].split('.')[0]+'/'+file)
            shutil.copyfile(Configuration.variables['HomeDirectory']+'crab/crab__'+file, Configuration.variables['HomeDirectory']+'DropBox/scratch/'+FileInfo[0]+'/'+FileInfo[1]+'/'+FileInfo[1]+FileInfo[2].split('.')[0]+'/crab.cfg')
            os.chdir(Configuration.variables['HomeDirectory']+'DropBox/scratch/'+FileInfo[0]+'/'+FileInfo[1]+'/'+FileInfo[1]+FileInfo[2].split('.')[0]+'/')
            #if site.upper() == "FNAL":
            os.system('source /afs/cern.ch/cms/ccs/wm/scripts/Crab/CRAB_2_4_0_Tutorial/crab.csh')
            ...
            for list in os.listdir(Configuration.variables['HomeDirectory']+'DropBox/scratch/'+FileInfo[0]+'/'+FileInfo[1]+'/'+FileInfo[1]+FileInfo[2].split('.')[0]+'/'):
                if os.listdir(os.path.isdir(Configuration.variables['HomeDirectory']+'DropBox/scratch/'+FileInfo[0]+'/'+FileInfo[1]+'/'+FileInfo[1]+FileInfo[2].split('.')[0]+'/'+list)):
                    self.__crab_List.append(Configuration.variables['HomeDirectory']+'DropBox/scratch/'+FileInfo[0]+'/'+FileInfo[1]+'/'+FileInfo[1]+FileInfo[2].split('.')[0]+'/'+list)
    ...
    def NewPublisher(self, run, process, compare, all, job, grid):
        os.chdir(Configuration.variables["HomeDirectory"]+'/bin/')
        if job.upper() != "LOCAL" and job.upper() != 'CRAB':
            condor_status = 0
            ...
            while len(cooked_crab) < len(self.__crab_List):
                ...
                for job in self.__crab_List:
                    ...
                    if crab_status != 0:
                        ...
                        elif total_jobs == jobs_done:
                            print "Crab job: "+job+" done!"
                            out_file = self.CrabCrusher(jobs+'/res/')
                            cooked_crab.append(job)
                            if out_file == None:
