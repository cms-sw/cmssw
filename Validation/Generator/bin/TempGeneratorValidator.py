...
class JobManager:
    ...
    def batch_run(self, run, process, compare, batch):
     ...
     # Function that combines all crab output root files
    ...
    # Implementing a CRAB runner. You need to supply crab.cfg and .py file....
    def Crab_run(self, run, process):
        self.__crab_List = []
        os.chdir(Configuration.variables['HomeDirectory'])
        CrabList = os.listdir(Configuration.variables['HomeDirectory']+'Crab/')
        print run, process
        for file in CrabList:
            if '.cfg' in file:
                continue
            if process.upper() not in file.upper():
                continue
            if run.upper() != "ALL" and run.upper() not in file.upper():
                continue
            #File needs to be written process__generator__generator-release.py
            #and needs to be accompanied by a crab.cfg file named crab__process__generator__generator-release.cfg
            FileInfo = file.split('__')
            scratch = Configuration.variables['HomeDirectory']+'DropBox/scratch/'+FileInfo[0]+'/'+FileInfo[1]+'/'+FileInfo[1]+'__'+FileInfo[2].split('.')[0]+'/'
            if os.path.isdir(scratch) == False:
                os.makedirs(scratch)
            else:
                os.system('rm -r '+scratch+'*')
            shutil.copyfile(Configuration.variables['HomeDirectory']+'Crab/'+file, scratch+file)
            shutil.copyfile(Configuration.variables['HomeDirectory']+'Crab/crab__'+file.split('.')[0]+'.cfg', scratch+'crab.cfg')
            shutil.copyfile(Configuration.variables['HomeDirectory']+'Crab/crab.sh', scratch+'crab.csh')
            CMSstatus, CMSoutput = commands.getstatusoutput('$CMSSW_BASE')
            CMS_dir = CMSoutput.split(':')[1].strip(' ')+'/src/'
            #print CMS_dir
            os.chdir(CMS_dir)
            #os.system('eval `scramv1 r -csh`')
            os.chdir(scratch)
            #if site.upper() == "FNAL":
            #subprocess.call("/afs/cern.ch/cms/ccs/wm/scripts/Crab/CRAB_2_4_0_Tutorial/crab.csh",shell=True)
            #shell_stat, shell_out = commands.getstatusoutput('source crab.csh')
            #print shell_out, "Shell output"
            create_status, create_output = commands.getstatusoutput('crab -create')
            print "CRAB job "+FileInfo[0] + " created"
            submit_status, submit_output = commands.getstatusoutput('crab -submit')
            print "CRAB job "+FileInfo[0] + " submitted"
            for list in os.listdir(scratch):
                if os.path.isdir(scratch+list):
                    self.__crab_List.append(scratch+list)
    ...
    def NewPublisher(self, run, process, compare, all, job, location, grid):
        ...
        if grid.upper() == 'TRUE':
            ...
            total_jobs = 0
            jobs_done = 0
            while len(cooked_crab) < len(self.__crab_List):
                if sub_jobs > 3:
                    print "More than 3 jobs in queue, sleeping for 3 minutes"
                    print "Jobs left: " , int(total_jobs) - jobs_done - sub_jobs, "Out of: ", total_jobs
                    time.sleep(180)
                else:
                ...
                for job in self.__crab_List:
                    if job in cooked_crab:
                        continue
                    crab_status, crab_output = commands.getstatusoutput('crab -status -c '+ job)
                    if crab_status != 0:
                        ...
                        for line in crab_output.split('\n'):
                            ...
                            if 'CLEARED' in line.upper() or 'DONE' in line.upper() and '>>>' not in line:
                        ...
                        if int(total_jobs) == 0:
                            ...
                            cooked_crab.append(job)
                        elif int(total_jobs) == jobs_done:
                            print "Crab job: "+job+" done!"
                            out_stat, out_out = commands.getstatusoutput('crab -getoutput -c '+job)
                            out_file = self.CrabCrusher(job+'/res/')
                            cooked_crab.append(job)
                            if out_file == None:
                                ...
                                if os.path.isdir(Configuration.variables['ReleaseDirectory']+gen+'/'+rel+'/'+proc+'/') == False:
                                    os.makedirs(Configuration.variables['ReleaseDirectory']+gen+'/'+rel+'-preGen/'+proc+'/')
                                shutil.copyfile(out_file, Configuration.variables['ReleaseDirectory']+gen+'/'+rel+'-preGen/'+proc+'/') 
