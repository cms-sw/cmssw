...
class JobManager:
    ...
    def NewPublisher(self, run, process, compare, all, job, location, grid, jobid):
        os.chdir(Configuration.variables["HomeDirectory"]+'/bin/')
        if job.upper() == "BATCH":
        ...
        if grid.upper() == 'TRUE':
            ...
            while len(cooked_crab) < 1:
                ...
                if jobid in cooked_crab:
                    continue
                crab_status, crab_output = commands.getstatusoutput('crab -status -c '+jobid)
                if crab_status != 0:
                        ...
                        if int(total_jobs) == 0:
                            ...
                            out_stat, out_out = commands.getstatusoutput('crab -getoutput -c '+jobid)
