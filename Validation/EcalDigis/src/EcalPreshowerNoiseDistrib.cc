...
void EcalPreshowerNoiseDistrib::analyze(const edm::Event& e, const edm::EventSetup& c) {
  ...
  for (unsigned int digis = 0; digis < EcalDigiES->size(); ++digis) {
    ...
    if (meESDigiCorr_[0]) {
      meESDigiCorr_[0]->Fill(esdf[0].adc(), esdf[1].adc());
    }
    if (meESDigiCorr_[1]) {
      meESDigiCorr_[1]->Fill(esdf[0].adc(), esdf[2].adc());
    }
    if (meESDigiCorr_[2]) {
      meESDigiCorr_[2]->Fill(esdf[1].adc(), esdf[2].adc());
    }
