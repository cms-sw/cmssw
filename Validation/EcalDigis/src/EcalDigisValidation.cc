...
void EcalDigisValidation::analyze(edm::Event const & e, edm::EventSetup const & c){
  ...
  for (auto & theSimVertexe : theSimVertexes){
    LogDebug("EventInfo") <<" Vertex index = " << nvtx << " event Id = " << theSimVertexe.eventId().rawId() << "\n" << " vertex dump: " << theSimVertexe ;
    ++nvtx;
  }
  
  int ntrk = 0;
  for (auto & theSimTrack : theSimTracks){
    LogDebug("EventInfo") <<" Track index = " << ntrk << " track Id = " << theSimTrack.trackId() << " event Id = " << theSimTrack.eventId().rawId() << "\n" << " track dump: " << theSimTrack ; 
    ++ntrk;
  }
