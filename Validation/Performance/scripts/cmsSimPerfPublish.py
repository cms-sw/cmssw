...
#    _debug = 1
    ...
    listdir = dc.opendir(".")
    dc.annotate("/",listdir)
    cmsScimarkDir = filter(cmsscidirreg.search,listdir) 
                ...
                for CurDir in DirName:
                    ...
                    for CurrentProfile in Profile:
                        for step in Step :
                            ...
                            ProfileTemplateLowCaps="%s_%s/*_%s_%s*/%s" % (CurrentCandle,CurDir,step.lower(),CurrentProfile,OutputHtml[CurrentProfile])
                            ...
                            if (CurrentProfile in ProfileReportLink):#It could also not be there
                                ...
                                if (CurrentProfile == Profile[7]):
                                    for i in range(0,3,1):
                                        ...
                                        ProfileTemplateLowCaps="%s_%s/*_%s_%s*/%s" % (CurrentCandle,CurDir,step.lower(),CurrentProfile,IgProfMemAnalyseOut[i])
                                        ProfileReportLink= getcmd("ls %s 2>/dev/null" % ProfileTemplate)
                                        if ( CurrentCandle not in ProfileReportLink ): # no match with caps try low caps
                                            ProfileReportLink=getcmd("ls %s 2>/dev/null"% ProfileTemplateLowCaps) 
                                        if (CurrentProfile in ProfileReportLink ) :#It could also not be there
                                    ...
                                    for i in range(0,3,1):
                                        ...
                                        ProfileTemplateLowCaps="%s_%s/*_%s_%s*/%s" % (CurrentCandle,CurDir,step.lower(),CurrentProfile,memcheck_valgrindOut[i])
                                        ProfileReportLink=getcmd("ls %s 2> /dev/null" % ProfileTemplate)
                                        if ( CurrentCandle not in ProfileReportLink ):#no match with caps try low caps
                                            ProfileReportLink=getcmd("ls %s 2> /dev/null" % ProfileTemplateLowCaps)
                                        if (CurrentProfile in ProfileReportLink) : #It could also not be there
