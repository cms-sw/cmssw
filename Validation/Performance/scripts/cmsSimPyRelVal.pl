...
@AfterPileUpSteps=(
	...
	foreach (@Profile)
	{
		...
		$Command="$cmsDriver $KeywordToCfi{$candle} -n $NumberOfEvents --step=DIGI $InputFileOption $OutputFileOption --pileup LowLumiPileUp --customise=Configuration/PyReleaseValidation/MixingModule.py $cmsDriverOptions";
	    }
