...
Profile codes (multiple codes can be used):
 ...
 8-ValgrindFCE
 9-ValgrindMemCheck
E.g: cmsSimulationCandles.pl 10 AllCandles 1 OR cmsSimulationCandles.pl 50 \"HZZLLLL\" 012\n";
	exit;
}
...
foreach (@Candle)
{
	...
	foreach (@Profile)
	{
		...
		$Command="$cmsDriver $candle -n $NumberOfEvents --step=DIGI $OutputFileOption --PU --customise=Configuration/PyReleaseValidation/MixingModule.py ";
	    }
	    print SIMCANDLES "$Command @@@ $Profiler{$_} @@@ $FileName{$candle}_DIGI_PILEUP_"."$_"."\n";
	}
    }
    #Add the extra "step" RECO with PILE UP only for QCD_80_120:
    if ($candle eq "QCD -e 80_120")
    {
	print SIMCANDLES "#$FileName{$candle}\n";
	print SIMCANDLES "#RECO ON DIGI PILE-UP STEP\n";
	foreach (@Profile)
	{
	    if ($_ eq "EdmSize")
	    {
		$Command="$FileName{$candle}"."_RECO_PILEUP.root ";
	    }
	    else
	    {
		$InputFileOption="--filein=file:$FileName{$candle}"."_DIGI_PILEUP.root";
		$OutputFileOption="--fileout=$FileName{$candle}"."_RECO_PILEUP.root";
		$Command="$cmsDriver $candle -n $NumberOfEvents --step=RECO $InputFileOption $OutputFileOption --customise=Configuration/PyReleaseValidation/Simulation.py ";
	    }
	    print SIMCANDLES "$Command @@@ $Profiler{$_} @@@ $FileName{$candle}_RECO_PILEUP_"."$_"."\n";
	}
    }
}
exit;
