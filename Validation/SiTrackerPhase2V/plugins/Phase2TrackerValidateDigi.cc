...
int Phase2TrackerValidateDigi::fillSimHitInfo(const edm::Event& iEvent, const SimTrack simTrk, const edm::ESHandle<TrackerGeometry> gHandle) {
  ...
  for (const auto& itoken : simHitTokens_) {
    ...
    for(edm::PSimHitContainer::const_iterator isim = simHits.begin(); isim != simHits.end(); ++isim){
      ...
      if (SimulatedXYPositionMap) SimulatedXYPositionMap->Fill(pdPos.x()*10., pdPos.y()*10.);   
      if (SimulatedRZPositionMap) SimulatedRZPositionMap->Fill(pdPos.z()*10., std::hypot(pdPos.x(),pdPos.y())*10.);   
      ...
      if (local_mes.SimHitDx) local_mes.SimHitDx->Fill(std::fabs((*isim).entryPoint().x()-(*isim).exitPoint().x()));
      if (local_mes.SimHitDy) local_mes.SimHitDy->Fill(std::fabs((*isim).entryPoint().y()-(*isim).exitPoint().y()));
      if (local_mes.SimHitDz) local_mes.SimHitDz->Fill(std::fabs((*isim).entryPoint().z()-(*isim).exitPoint().z()));
      
      if (SimulatedTOFEtaMap) SimulatedTOFEtaMap->Fill(pdPos.eta(),(*isim).timeOfFlight());
      if (SimulatedTOFPhiMap) SimulatedTOFPhiMap->Fill(pdPos.phi(),(*isim).timeOfFlight());
      if (SimulatedTOFRMap) SimulatedTOFRMap->Fill(std::hypot(pdPos.x(),pdPos.y()),(*isim).timeOfFlight());
      ...
      if (fabs(eta) <  etaCut_) {
	if (local_mes.SimTrackPt) local_mes.SimTrackPt->Fill(pt);
	if (digiFlag && local_mes.MatchedTrackPt) local_mes.MatchedTrackPt->Fill(pt);
        else if (local_mes.MissedDigiTrackPt) local_mes.MissedDigiTrackPt->Fill(pt);
      }
      if (pt > ptCut_) {
 	if (local_mes.SimTrackEta) local_mes.SimTrackEta->Fill(eta);
        if (digiFlag && local_mes.MatchedTrackEta) local_mes.MatchedTrackEta->Fill(eta);
        else if (local_mes.MissedDigiTrackEta) local_mes.MissedDigiTrackEta->Fill(eta);
      }
      if ( fabs(eta) < etaCut_ && pt > ptCut_) {
	if (local_mes.SimTrackPhi) local_mes.SimTrackPhi->Fill(phi);
	if (digiFlag && local_mes.MatchedTrackPhi) local_mes.MatchedTrackPhi->Fill(phi);
        else if (local_mes.MissedDigiTrackPhi) local_mes.MissedDigiTrackPhi->Fill(phi);
      }
      if (digiFlag) {
        pos->second.nDigis++;
	if (MatchedRZPositionMap) MatchedRZPositionMap->Fill(pdPos.z()*10., std::hypot(pdPos.x(),pdPos.y())*10.);   
	if (MatchedXYPositionMap) MatchedXYPositionMap->Fill(pdPos.x()*10., pdPos.y()*10.);	
	if (nColumns <= 2 && local_mes.MatchedSimHitElossS) local_mes.MatchedSimHitElossS->Fill((*isim).energyLoss()/GeVperElectron);
	else if (local_mes.MatchedSimHitElossP) local_mes.MatchedSimHitElossP->Fill((*isim).energyLoss()/GeVperElectron);
      } else {
	if (local_mes.MissedDigiLocalXposVsYPos) local_mes.MissedDigiLocalXposVsYPos->Fill((*isim).localPosition().x(), (*isim).localPosition().y());
        if (local_mes.MissedDigiTimeWindow) local_mes.MissedDigiTimeWindow->Fill(std::fabs((*isim).timeOfFlight() - pdPos.mag()/cval));
	if (nColumns <= 2 && local_mes.MissedDigiSimHitElossS) local_mes.MissedDigiSimHitElossS->Fill((*isim).energyLoss()/GeVperElectron);
	else if (local_mes.MissedDigiSimHitElossP) local_mes.MissedDigiSimHitElossP->Fill((*isim).energyLoss()/GeVperElectron);
...
void Phase2TrackerValidateDigi::bookHistograms(DQMStore::IBooker & ibooker,
  ...
  if (Parameters.getParameter<bool>("switch")) SimulatedTrackPt = ibooker.book1D(HistoName.str(),HistoName.str(),
  ...
  if (Parameters.getParameter<bool>("switch")) SimulatedTrackPtP = ibooker.book1D(HistoName.str(),HistoName.str(),
				   Parameters.getParameter<int32_t>("Nbins"),
				   Parameters.getParameter<double>("xmin"),
				   Parameters.getParameter<double>("xmax"));
  ...
  if (Parameters.getParameter<bool>("switch")) SimulatedTrackEta = ibooker.book1D(HistoName.str(),HistoName.str(),
				   Parameters.getParameter<int32_t>("Nbins"),
				   Parameters.getParameter<double>("xmin"),
				   Parameters.getParameter<double>("xmax"));
  ...
  if (Parameters.getParameter<bool>("switch"))SimulatedTrackPhi = ibooker.book1D(HistoName.str(),HistoName.str(),
				   Parameters.getParameter<int32_t>("Nbins"),
				   Parameters.getParameter<double>("xmin"),
				   Parameters.getParameter<double>("xmax"));
  ...
  if (Parameters.getParameter<bool>("switch")) SimulatedTrackPhiP = ibooker.book1D(HistoName.str(),HistoName.str(),
				    Parameters.getParameter<int32_t>("Nbins"),
				    Parameters.getParameter<double>("xmin"),
				    Parameters.getParameter<double>("xmax"));
  ...
  if (Parameters.getParameter<bool>("switch")) SimulatedTrackPhiS = ibooker.book1D(HistoName.str(),HistoName.str(),
				    Parameters.getParameter<int32_t>("Nbins"),
				    Parameters.getParameter<double>("xmin"),
				    Parameters.getParameter<double>("xmax"));
  ...
  if (Parameters.getParameter<bool>("switch")) SimulatedXYPositionMap = ibooker.book2D(HistoName.str(), HistoName.str(),
  ...
  if (Parameters.getParameter<bool>("switch")) SimulatedRZPositionMap = ibooker.book2D(HistoName.str(), HistoName.str(),
				 Parameters.getParameter<int32_t>("Nxbins"),
				 Parameters.getParameter<double>("xmin"),
				 Parameters.getParameter<double>("xmax"),
				 Parameters.getParameter<int32_t>("Nybins"),
  ...
  if (Parameters.getParameter<bool>("switch"))  SimulatedTOFEtaMap = ibooker.book2D(HistoName.str(), HistoName.str(),
					  Parameters.getParameter<int32_t>("Nxbins"),
					  Parameters.getParameter<double>("xmin"),
					  Parameters.getParameter<double>("xmax"),
					  Parameters.getParameter<int32_t>("Nybins"),
  ...
  if (Parameters.getParameter<bool>("switch")) SimulatedTOFPhiMap = ibooker.book2D(HistoName.str(), HistoName.str(),
				      Parameters.getParameter<int32_t>("Nxbins"),
				      Parameters.getParameter<double>("xmin"),
				      Parameters.getParameter<double>("xmax"),
				      Parameters.getParameter<int32_t>("Nybins"),
  ...
  if (Parameters.getParameter<bool>("switch")) SimulatedTOFRMap = ibooker.book2D(HistoName.str(), HistoName.str(),
                                      Parameters.getParameter<int32_t>("Nxbins"),
                                      Parameters.getParameter<double>("xmin"),
                                      Parameters.getParameter<double>("xmax"),
                                      Parameters.getParameter<int32_t>("Nybins"),
  ...
  if (Parameters.getParameter<bool>("switch")) SimulatedTOFZMap = ibooker.book2D(HistoName.str(), HistoName.str(),
                                      Parameters.getParameter<int32_t>("Nxbins"),
                                      Parameters.getParameter<double>("xmin"),
                                      Parameters.getParameter<double>("xmax"),
                                      Parameters.getParameter<int32_t>("Nybins"),
  ...
  if (Parameters.getParameter<bool>("switch")) MatchedXYPositionMap = ibooker.book2D(HistoName.str(), HistoName.str(),
				 Parameters.getParameter<int32_t>("Nxbins"),
				 Parameters.getParameter<double>("xmin"),
				 Parameters.getParameter<double>("xmax"),
				 Parameters.getParameter<int32_t>("Nybins"),
  ...
  if (Parameters.getParameter<bool>("switch")) MatchedRZPositionMap = ibooker.book2D(HistoName.str(), HistoName.str(),
				 Parameters.getParameter<int32_t>("Nxbins"),
				 Parameters.getParameter<double>("xmin"),
				 Parameters.getParameter<double>("xmax"),
				 Parameters.getParameter<int32_t>("Nybins"),
...
void Phase2TrackerValidateDigi::bookLayerHistos(DQMStore::IBooker & ibooker, unsigned int det_id, const TrackerTopology* tTopo, bool flag){ 
  ...
  if (pos == layerMEs.end()) {
    ...
    if (Parameters.getParameter<bool>("switch")) local_mes.SimTrackPt = ibooker.book1D(HistoName.str(),HistoName.str(),
						Parameters.getParameter<int32_t>("Nbins"),
						Parameters.getParameter<double>("xmin"),
						Parameters.getParameter<double>("xmax"));
    ...
    if (Parameters.getParameter<bool>("switch")) local_mes.SimTrackEta = ibooker.book1D(HistoName.str(),HistoName.str(),
						Parameters.getParameter<int32_t>("Nbins"),
						Parameters.getParameter<double>("xmin"),
						Parameters.getParameter<double>("xmax"));
    ...
    if (Parameters.getParameter<bool>("switch")) local_mes.SimTrackPhi = ibooker.book1D(HistoName.str(),HistoName.str(),
						Parameters.getParameter<int32_t>("Nbins"),
						Parameters.getParameter<double>("xmin"),
						Parameters.getParameter<double>("xmax"));
    ...
    if (Parameters.getParameter<bool>("switch")) local_mes.MatchedSimHitElossS = ibooker.book1D(HistoName.str(),HistoName.str(),
						Parameters.getParameter<int32_t>("Nbins"),
						Parameters.getParameter<double>("xmin"),
						Parameters.getParameter<double>("xmax"));
    ...
    if (Parameters.getParameter<bool>("switch")) local_mes.SimHitDx = ibooker.book1D(HistoName.str(),HistoName.str(),
					Parameters.getParameter<int32_t>("Nbins"),
					Parameters.getParameter<double>("xmin"),
					Parameters.getParameter<double>("xmax"));
    ...
    if (Parameters.getParameter<bool>("switch")) local_mes.SimHitDy = ibooker.book1D(HistoName.str(),HistoName.str(),
					 Parameters.getParameter<int32_t>("Nbins"),
					 Parameters.getParameter<double>("xmin"),
					 Parameters.getParameter<double>("xmax"));
    ...
    if (Parameters.getParameter<bool>("switch")) local_mes.SimHitDz = ibooker.book1D(HistoName.str(),HistoName.str(),
					 Parameters.getParameter<int32_t>("Nbins"),
					 Parameters.getParameter<double>("xmin"),
					 Parameters.getParameter<double>("xmax"));
