  ...
  for(const auto& DSVItr: *clusterHandle){
	...
	for(auto simhitIt : *psimhitCont) { 
      ...
      if(mType == TrackerGeometry::ModuleType::Ph2PSP){
        local_mes.deltaXPixel->Fill(deltaX);
        local_mes.deltaYPixel->Fill(deltaY);
      } else if(mType == TrackerGeometry::ModuleType::Ph2PSS || mType == TrackerGeometry::ModuleType::Ph2SS) {
        local_mes.deltaXStrip->Fill(deltaX);
        local_mes.deltaYStrip->Fill(deltaY);
      }
         // Primary particles only
      if(phase2tkutil::isPrimary(simTrackIt->second, closestSimHit)) {
        if(mType == TrackerGeometry::ModuleType::Ph2PSP){
          local_mes.deltaXPixelP->Fill(deltaX);
          local_mes.deltaYPixelP->Fill(deltaY);
        }else if(mType == TrackerGeometry::ModuleType::Ph2PSS || mType == TrackerGeometry::ModuleType::Ph2SS){
          local_mes.deltaXStripP->Fill(deltaX);
          local_mes.deltaYStripP->Fill(deltaY);
        }
      }
...
void Phase2OTValidateCluster::bookLayerHistos(DQMStore::IBooker& ibooker,
  ...
  if(layerMEs_.find(folderName) == layerMEs_.end()){
	...
	phase2tkutil::book1DFromPSet(config_.getParameter<edm::ParameterSet>("Delta_X_Pixel"), HistoName.str(), ibooker);
      ...
      local_mes.allDigisPixel = 
      phase2tkutil::book1DFromPSet(config_.getParameter<edm::ParameterSet>("All_Digis_Pixel"), HistoName.str(), ibooker);

      HistoName.str("");
      HistoName << "Other_Digis_Pixel";
      local_mes.otherDigisPixel = 
	phase2tkutil::book1DFromPSet(config_.getParameter<edm::ParameterSet>("Other_Digis_Pixel"), HistoName.str(), ibooker);

      HistoName.str("");
      HistoName << "Delta_X_Pixel_Primary";
      local_mes.deltaXPixelP = 
	...
	phase2tkutil::book1DFromPSet(config_.getParameter<edm::ParameterSet>("Delta_Y_Pixel_Primary"), HistoName.str(), ibooker);
      ...
      local_mes.primaryDigisPixel = 
	phase2tkutil::book1DFromPSet(config_.getParameter<edm::ParameterSet>("Primary_Digis_Pixel"), HistoName.str(), ibooker);


    }
    HistoName.str("");
    HistoName << "Delta_X_Strip";
    local_mes.deltaXStrip = 
    ...
    local_mes.allDigisStrip = 
      phase2tkutil::book1DFromPSet(config_.getParameter<edm::ParameterSet>("All_Digis_Strip"), HistoName.str(), ibooker);

    HistoName.str("");
    HistoName << "Other_Digis_Strip";
    local_mes.otherDigisStrip = 
      phase2tkutil::book1DFromPSet(config_.getParameter<edm::ParameterSet>("Other_Digis_Strip"), HistoName.str(), ibooker);

    // All histos related to simhits for primary tracks only

    ibooker.setCurrentFolder(subdir +'/'+folderName + "/PrimarySimHits"); // Puting primary digis in a subfolder
    HistoName.str("");
    HistoName << "Delta_X_Strip_Primary";
    local_mes.deltaXStripP = 
    ...
    local_mes.primaryDigisStrip = 
      phase2tkutil::book1DFromPSet(config_.getParameter<edm::ParameterSet>("Primary_Digis_Strip"), HistoName.str(), ibooker);
...
Phase2OTValidateCluster::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  {
    edm::ParameterSetDescription psd0;
    psd0.add<std::string>("name", "All_Digis_Pixel");
    psd0.add<std::string>("title", "All_Digis"+mptag+";");
    psd0.add<double>("xmin", 0.0);
    psd0.add<bool>("switch", true);
    psd0.add<double>("xmax", 1000.0);
    psd0.add<int>("NxBins", 100);
    desc.add<edm::ParameterSetDescription>("All_Digis_Pixel", psd0);
  }  
  {
    edm::ParameterSetDescription psd0;
    psd0.add<std::string>("name", "Other_Digis_Pixel");
    psd0.add<std::string>("title", "Other_Digis"+mptag+";");
    psd0.add<double>("xmin", 0.0);
    psd0.add<bool>("switch", true);
    psd0.add<double>("xmax", 1000.0);
    psd0.add<int>("NxBins", 100);
    desc.add<edm::ParameterSetDescription>("Other_Digis_Pixel", psd0);
  }  
  {
    edm::ParameterSetDescription psd0;
  ...
  {
    edm::ParameterSetDescription psd0;
    psd0.add<std::string>("name", "Primary_Digis_Pixel");
    psd0.add<std::string>("title", "Primary_Digis"+mptag+";");
    psd0.add<double>("xmin", 0.0);
    psd0.add<bool>("switch", true);
    psd0.add<double>("xmax", 1000.0);
    psd0.add<int>("NxBins", 100);
    desc.add<edm::ParameterSetDescription>("Primary_Digis_Pixel", psd0);
  }
  //strip sensors
  {
    edm::ParameterSetDescription psd0;
  ...
  }
  {
    edm::ParameterSetDescription psd0;
    psd0.add<std::string>("name", "All_Digis_Pixel");
    psd0.add<std::string>("title", "All_Digis"+striptag+";");
    psd0.add<double>("xmin", 0.0);
    psd0.add<bool>("switch", true);
    psd0.add<double>("xmax", 1000.0);
    psd0.add<int>("NxBins", 100);
    desc.add<edm::ParameterSetDescription>("All_Digis_Strip", psd0);
  }
  {
    edm::ParameterSetDescription psd0;
    psd0.add<std::string>("name", "Other_Digis_Pixel");
    psd0.add<std::string>("title", "Other_Digis"+striptag+";");
    psd0.add<double>("xmin", 0.0);
    psd0.add<bool>("switch", true);
    psd0.add<double>("xmax", 1000.0);
    psd0.add<int>("NxBins", 100);
    desc.add<edm::ParameterSetDescription>("Other_Digis_Strip", psd0);
  }
  {
    edm::ParameterSetDescription psd0;
  ...
  }
  {
    edm::ParameterSetDescription psd0;
    psd0.add<std::string>("name", "Primary_Digis_Pixel");
    psd0.add<std::string>("title", "Primary_Digis"+striptag+";");
    psd0.add<double>("xmin", 0.0);
    psd0.add<bool>("switch", true);
    psd0.add<double>("xmax", 1000.0);
    psd0.add<int>("NxBins", 100);
    desc.add<edm::ParameterSetDescription>("Primary_Digis_Strip", psd0);
  }
