...
SimG4HcalValidation::SimG4HcalValidation(const edm::ParameterSet &p)
  ...
  edm::ParameterSet m_Anal =
      p.getParameter<edm::ParameterSet>("SimG4HcalValidation");
  ...
  labelLayer = m_Anal.getUntrackedParameter<std::string>("LabelLayerInfo",
                                                         "HcalInfoLayer");
  labelNxN =
      m_Anal.getUntrackedParameter<std::string>("LabelNxNInfo", "HcalInfoNxN");
  labelJets = m_Anal.getUntrackedParameter<std::string>("LabelJetsInfo",
                                                        "HcalInfoJets");
...
SimG4HcalValidation::~SimG4HcalValidation() {
  ...
  edm::LogInfo("ValidHcal")
      ...
      << ", Numbering Scheme " << org << " and FromDDD " << numberingFromDDD;
  if (jetf) {
...
void SimG4HcalValidation::update(const G4Step *aStep) {
  ...
  if (aStep != nullptr) {
    ...
    if (layer >= 0 && layer < 20) {
      LogDebug("ValidHcal")
          << "SimG4HcalValidation:: G4Step: " << name << " Layer "
...
void SimG4HcalValidation::fill(const EndOfEvent *evt) {
  ...
  if (HCHCid >= 0 && theHCHC != nullptr) {
    for (j = 0; j < theHCHC->entries(); j++) {
      ...
      org->unpackHcalIndex(unitID, subdet, zside, layer, etaIndex, phiIndex,
                           lay);
      ...
      if (subdet == static_cast<int>(HcalForward)) {
        ...
        if (layer == 1)
          ...
          edm::LogInfo("ValidHcal")
              ...
              << (2 * zside - 1) * etaIndex << " " << phiIndex << " "
              << layer + 1 << " R " << pos.rho() << " Phi " << phi / deg
              << " Edep " << e << " Time " << time;
      ...
      LogDebug("ValidHcal")
          << "SimG4HcalValidation::debugFill Hcal " << det << " layer "
          << std::setw(2) << layer << " lay " << std::setw(2) << lay << " time "
          << std::setw(6) << time << " theta " << std::setw(8) << theta
          << " eta " << std::setw(8) << eta << " phi " << std::setw(8) << phi
          << " e " << std::setw(8) << e << " ID 0x" << std::hex << unitID
  ...
  if (!hcalOnly) { //--------------------------  ECAL hits --------------------
    ...
    for (int idty = 1; idty < ndets; idty++) {
      ...
      CaloG4HitCollection *theECHC =
          (CaloG4HitCollection *)allHC->GetHC(ECHCid);
      LogDebug("ValidHcal")
          << "SimG4HcalValidation:: Hit Collection for " << names[idty]
      ...
      if (ECHCid >= 0 && theECHC != nullptr) {
        for (j = 0; j < theECHC->entries(); j++) {
          ...
          HcalNumberingFromDDD::HcalID id =
              numberingFromDDD->unitID(eta, phi, 1, 1);
          ...
          LogDebug("ValidHcal")
              << "SimG4HcalValidation::debugFill Ecal " << det << " layer "
              << std::setw(2) << layer << " lay " << std::setw(2) << lay
              << " time " << std::setw(6) << time << " theta " << std::setw(8)
              << theta << " eta " << std::setw(8) << eta << " phi "
              << std::setw(8) << phi << " e " << std::setw(8) << e << " ID 0x"
              << std::hex << unitID << " ID dec " << std::dec << (int)unitID;
        }
      }

      LogDebug("ValidHcal")
          << "SimG4HcalValidation:: " << det << " hits : " << nec;
    }
  } // end of if(!hcalOnly)
}
...
void SimG4HcalValidation::nxNAnalysis(PHcalValidInfoNxN &product) {
  ...
  for (hit_itr = hits->begin(); hit_itr < hits->end(); hit_itr++) {
    ...
    if (fabs(eta0 - eta) <= dEta[max - 1] &&
        fabs(phi0 - phi) <= dPhi[max - 1]) {
      etot += e;
...
void SimG4HcalValidation::jetAnalysis(PHcalValidInfoJets &product) {
  ...
  if (!(*result).empty()) {
    ...
    // Di-jet mass
    if (etavec.size() > 1) {
      if (etavec[0] > -2.5 && etavec[0] < 2.5 && etavec[1] > -2.5 &&
          etavec[1] < 2.5) {
...
void SimG4HcalValidation::fetchHits(PHcalValidInfoLayer &product) {
  ...
  for (i = 0, itr = hitcache.begin(); itr != hitcache.end(); i++, itr++) {
    ...
    HcalTestNumbering::unpackHcalIndex(unitID, subdet, zside, group, ieta, iphi,
                                       lay);
    subdet = itr->det();
    lay = itr->layer();
    group = (subdet & 15) << 20;
  ...
  LogDebug("ValidHcal") << "SimG4HcalValidation::fetchHits called with " << nHit
                        << " hits"
                        << " and writes out " << nHits << '(' << hit
                        << ") hits";
}
//---------------------------------------------------
