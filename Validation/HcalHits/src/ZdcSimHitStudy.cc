...
void ZdcSimHitStudy::analyzeHits(std::vector<PCaloHit> &hits) {
  ...
  for (int i = 0; i < nHit; i++) {
    ...
    edm::LogVerbatim("ZdcSimHitStudy") << "Hit[" << i << "] ID " << std::hex << id_ << std::dec << " DetID " << id << " Det " << det << " side " << side << " Section " << section << " channel " << channel << " E " << energy << " time \n" << time;
  ...
  edm::LogVerbatim("HcalSimHitStudy") << "HcalSimHitStudy::analyzeHits: Had " << nZdcHad << " EM " << nZdcEM << " Bad " << nBad << " All " << nHit;
}
