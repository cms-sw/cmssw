...
public:
  /// Constructor
  StandAloneMuonFilter(const edm::ParameterSet &par,
  ...
  void refit(const TrajectoryStateOnSurface &initialState, const DetLayer *,
  ...
  inline bool isCompatibilitySatisfied() const {
    return totalCompatibleChambers >= 2 &&
           ((dtCompatibleChambers + cscCompatibleChambers +
...
private:
  ...
  bool update(const DetLayer *layer, const TrajectoryMeasurement *meas,
              Trajectory &trajectory);

  std::vector<TrajectoryMeasurement>
  findBestMeasurements(const DetLayer *layer,
  ...
  /// The best measurement finder: search for the best measurement among the TMs
  /// available
