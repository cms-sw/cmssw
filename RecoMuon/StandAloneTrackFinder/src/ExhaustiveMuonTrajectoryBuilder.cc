...
ExhaustiveMuonTrajectoryBuilder::ExhaustiveMuonTrajectoryBuilder(
    const edm::ParameterSet &pset, const MuonServiceProxy *proxy,
...
ExhaustiveMuonTrajectoryBuilder::trajectories(const TrajectorySeed &seed) {
  ...
  for (TrajectorySeed::const_iterator recHitItr = range.first;
    ...
    const GeomDet *geomDet =
        theService->trackingGeometry()->idToDet((*recHitItr).geographicalId());
...
void ExhaustiveMuonTrajectoryBuilder::clean(
    TrajectoryContainer &trajectories) const {
  ...
  for (unsigned i = 0; i < ntraj; ++i) {
    ...
    if (nhits > best_nhits) {
      best_nhits = nhits;
      best = i;
    } else if (nhits == best_nhits && trajectories[i]->chiSquared() <
                                          trajectories[best]->chiSquared()) {
      best = i;
    }
  }
  TrajectoryContainer result;
