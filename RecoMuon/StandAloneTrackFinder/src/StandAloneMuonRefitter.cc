...
StandAloneMuonRefitter::singleRefit(const Trajectory &trajectory) {

  theService->eventSetup().get<TrajectoryFitter::Record>().get(theFitterName,
                                                               theFitter);
...
StandAloneMuonRefitter::refit(const Trajectory &trajectory) {
  ...
  for (unsigned int j = 0; j < theNumberOfIterations; ++j) {
    ...
    StandAloneMuonRefitter::RefitResult singleRefitResult =
        singleRefit(lastFitted);
    ...
    if (!singleRefitResult.first) {
      allIter = false;
      LogDebug("Muon|RecoMuon|StandAloneMuonRefitter")
          << "  refit n. " << nSuccess + 1 << ": failed" << endl;
      break;
    }
    ...
    if (lostFract > theMaxFractionOfLostHits) {
      enoughRH = false;
      LogDebug("Muon|RecoMuon|StandAloneMuonRefitter")
          << "  refit n. " << nSuccess + 1 << ": too many RH lost" << endl;
      LogDebug("Muon|RecoMuon|StandAloneMuonRefitter")
  ...
  LogDebug("Muon|RecoMuon|StandAloneMuonRefitter")
      << nSuccess << " successful refits!" << endl;

  // if isForceAllIterations==true  =>   3 successful refits: (true, refitted
  // trajectory)
  //                                    <3 successful refits: (false, original
  //                                    trajectory)
  // if isForceAllIterations==false =>  >0 successful refits: (true, last
  // refitted trajectory)
  //                                     0 successful refits: (false, original
  //                                     trajectory)
  if (!enoughRH)
    return RefitResult(false, trajectory);
  else if (isForceAllIterations)
    return allIter ? RefitResult(allIter, lastFitted)
                   : RefitResult(allIter, trajectory);
  else
    return nSuccess == 0 ? RefitResult(false, trajectory)
                         : RefitResult(true, lastFitted);
}
