...
StandAloneMuonFilter::StandAloneMuonFilter(const ParameterSet &par,
  ...
  if (fitDirectionName == "insideOut")
    ...
    throw cms::Exception("StandAloneMuonFilter constructor")
        << "Wrong fit direction chosen in "
           "StandAloneMuonFilter::StandAloneMuonFilter ParameterSet"
        << "\n"
  ...
  // it isn't used by the updator which does the real fit. In fact, in
  // principle, a looser request onto the measure set can be requested (w.r.t.
  // the request on the accept/reject measure in the fit)
  ...
  theMeasurementExtractor = new MuonDetLayerMeasurements(
      ...
      par.getParameter<InputTag>("ME0RecSegmentLabel"), iC, enableDTMeasurement,
      enableCSCMeasurement, enableRPCMeasurement, enableGEMMeasurement,
      enableME0Measurement);

  theRPCLoneliness = (!(enableDTMeasurement && enableCSCMeasurement))
...
void StandAloneMuonFilter::reset() {
  totalChambers = dtChambers = cscChambers = rpcChambers = gemChambers =
      me0Chambers = 0;
  totalCompatibleChambers = dtCompatibleChambers = cscCompatibleChambers =
      rpcCompatibleChambers = gemCompatibleChambers = me0CompatibleChambers = 0;

  theLastCompatibleTSOS = theLastUpdatedTSOS = theLastButOneUpdatedTSOS =
...
void StandAloneMuonFilter::incrementCompatibleChamberCounters(
...
StandAloneMuonFilter::compatibleLayers(const DetLayer *initialLayer,
  ...
  if (theNavigationType == "Standard") {
    ...
    detLayers = theService->muonNavigationSchool()->compatibleLayers(
        *initialLayer, fts, propDir);
    // I have to fit by hand the first layer until the seedTSOS is defined on
    // the first rechit layer In fact the first layer is not returned by
    // initialLayer->compatibleLayers.
    ...
    edm::LogError("Muon|RecoMuon|StandAloneMuonFilter")
...
void StandAloneMuonFilter::refit(const TrajectoryStateOnSurface &initialTSOS,
  ...
  TrajectoryStateOnSurface lastTSOS = theLastCompatibleTSOS =
      theLastUpdatedTSOS = theLastButOneUpdatedTSOS = initialTSOS;

  double eta0 = initialTSOS.freeTrajectoryState()->momentum().eta();
  vector<const DetLayer *> detLayers = compatibleLayers(
      initialLayer, *initialTSOS.freeTrajectoryState(), propagationDirection());
  ...
  for (layer = detLayers.begin(); layer != detLayers.end(); ++layer) {
    ...
    std::vector<TrajectoryMeasurement> bestMeasurements =
        findBestMeasurements(*layer, lastTSOS);
    ...
    double lastdEta =
        fabs(lastTSOS.freeTrajectoryState()->momentum().eta() - eta0);
    if (bestMeasurements.empty() && lastdEta > 0.1) {
      LogTrace(metname) << "No measurement and big eta variation wrt seed"
                        << endl
    ...
    if (!bestMeasurements.empty()) {
      ...
      bool added = false;
      for (std::vector<TrajectoryMeasurement>::const_iterator tmItr =
               bestMeasurements.begin();
    ...
    // SL in case no valid mesurement is found, still I want to use the
    // predicted state for the following measurement serches. I take the first
    // in the container. FIXME!!! I want to carefully check this!!!!!
    else {
      LogTrace(metname) << "No best measurement found" << endl;
      //       if (!theMeasurementCache.empty()){
      // 	LogTrace(metname)<<"but the #of measurement is
      // "<<theMeasurementCache.size()<<endl;
...
std::vector<TrajectoryMeasurement> StandAloneMuonFilter::findBestMeasurements(
  ...
  if (theOverlappingChambersFlag && layer->hasGroups()) {
    ...
    std::vector<TrajectoryMeasurementGroup> measurementGroups =
        theMeasurementExtractor->groupedMeasurements(layer, tsos, *propagator(),
                                                     *estimator());

    if (theFitDirection == outsideIn) {
      LogTrace(metname) << "Reversing the order of groupedMeasurements as the "
                           "direction of the fit is outside-in";
    ...
    for (std::vector<TrajectoryMeasurementGroup>::const_iterator tmGroupItr =
      ...
      LogTrace(metname) << "Number of Trajectory Measurement: "
                        << measurements.size();
    ...
    measurements = theMeasurementExtractor->measurements(
        layer, tsos, *propagator(), *estimator());
    LogTrace(metname) << "Number of Trajectory Measurement: "
                      << measurements.size();
    const TrajectoryMeasurement *bestMeasurement =
        bestMeasurementFinder()->findBestMeasurement(measurements,
...
bool StandAloneMuonFilter::update(const DetLayer *layer,
  ...
  pair<bool, TrajectoryStateOnSurface> result =
      updator()->update(meas, trajectory, propagator());
...
void StandAloneMuonFilter::createDefaultTrajectory(const Trajectory &oldTraj,
  ...
  for (Trajectory::DataContainer::const_iterator itm = oldMeas.begin();
       itm != oldMeas.end(); itm++) {
    if (!(*itm).recHit()->isValid())
      defTraj.push(*itm, (*itm).estimate());
    else {
      MuonTransientTrackingRecHit::MuonRecHitPointer invRhPtr =
          MuonTransientTrackingRecHit::specificBuild((*itm).recHit()->det(),
                                                     (*itm).recHit()->hit());
      invRhPtr->invalidateHit();
      TrajectoryMeasurement invRhMeas((*itm).forwardPredictedState(),
                                      (*itm).updatedState(), invRhPtr,
                                      (*itm).estimate(), (*itm).layer());
