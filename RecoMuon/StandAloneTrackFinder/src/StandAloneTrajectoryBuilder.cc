...
StandAloneMuonTrajectoryBuilder::StandAloneMuonTrajectoryBuilder(
  ...
  if (doBackwardFilter) {
    ...
    ParameterSet bwFilterPSet =
        par.getParameter<ParameterSet>("BWFilterParameters");
    //  theBWFilter = new StandAloneMuonBackwardFilter(bwFilterPSet,theService);
    //  // FIXME
  ...
  if (doRefit) {
    ...
    ParameterSet refitterPSet =
        par.getParameter<ParameterSet>("RefitterParameters");
    theRefitter = new StandAloneMuonRefitter(refitterPSet, theService);
  }

  // The seed transformer (used to refit the seed and get the seed transient
  // state)
  //  ParameterSet seedTransformerPSet =
...
StandAloneMuonTrajectoryBuilder::trajectories(const TrajectorySeed &seed) {
  ...
  PropagationDirection fwDirection =
      (theSeedPosition == recoMuon::in) ? alongMomentum : oppositeToMomentum;
  ...
  LogTrace(metname) << "Trajectory State on Surface before the extrapolation"
                    << endl;
  ...
  //  this is the last UPDATED TSOS
  TrajectoryStateOnSurface tsosAfterRefit =
      filter()->lastCompatibleTSOS(); // this is the last COMPATIBLE TSOS
  ...
  if( filter()->isCompatibilitySatisfied() ) {
    if( filter()->layers().size() )   //  OR   if( filter()->goodState() ) ???
  Maybe when only RPC hits are used... LogTrace(metname) << debug.dumpLayer(
  filter()->lastDetLayer() ); else { LogTrace(metname) << "Compatibility
  satisfied, but all RecHits are invalid! A trajectory with only invalid hits
  will be loaded!" << endl; trajectoryContainer.push_back(new
  Trajectory(trajectoryFW)); return trajectoryContainer;
    }
  }
  else {
    LogTrace(metname) << "Compatibility NOT satisfied after Forward filter! No
  trajectory will be loaded!" << endl; LogTrace(metname) << "Total chambers: "
  << filter()->getTotalCompatibleChambers() << "; DT: "
                      << filter()->getDTCompatibleChambers() << "; CSC: " <<
  filter()->getCSCCompatibleChambers() << endl; return trajectoryContainer;
  }
  // -- end 1st attempt
  */
  ...
  if (filter()->goodState()) {
    ...
    LogTrace(metname)
        << "Compatibility satisfied after Forward filter, but too few valid "
           "RecHits ("
        << foundValidRh << ")." << endl
    ...
    LogTrace(metname) << "Compatibility NOT satisfied after Forward filter! No "
                      ...
                      << endl;
    LogTrace(metname) << "Total compatible chambers: "
                      << filter()->getTotalCompatibleChambers()
                      ...
                      << ";  ME0: " << filter()->getME0CompatibleChambers()
                      << endl;
    return trajectoryContainer;
  }
  // -- end 2nd attempt
  ...
  if (!doBackwardFilter) {
    LogTrace(metname)
        << "Only forward refit requested. No backward refit will be performed!"
        << endl;

    // ***** Refit of fwd step *****
    //    if (doRefit && !trajectoryFW.empty() && filter()->goodState()){    //
    //    CHECK!!! Can trajectoryFW really be empty at this point??? And
    //    goodState...?
    if (doRefit) {
      ...
      if (refitResult.first) {
        ...
        LogTrace(metname) << "StandAloneMuonTrajectoryBuilder refit output "
                          << endl;
        LogTrace(metname) << debug.dumpTSOS(
            refitResult.second.lastMeasurement().updatedState());
      } else
        trajectoryContainer.push_back(new Trajectory(trajectoryFW));
    } else
      trajectoryContainer.push_back(new Trajectory(trajectoryFW));
  ...
  if (theBWSeedType == "noSeed") {
    ...
    PTrajectoryStateOnDet seedTSOS = trajectoryStateTransform::persistentState(
        tsosAfterRefit,
        trajectoryFW.lastMeasurement().recHit()->geographicalId().rawId());

    edm::OwnVector<TrackingRecHit> recHitsContainer;
    PropagationDirection seedDirection =
        (theSeedPosition == recoMuon::in) ? oppositeToMomentum : alongMomentum;
  ...
  PropagationDirection bwDirection =
      (theSeedPosition == recoMuon::in) ? oppositeToMomentum : alongMomentum;
  ...
  LogTrace(metname) << "StandAloneMuonTrajectoryBuilder BW filter output "
                    << endl;
  ...
  // -- The trajectory is "good" if there are at least 2 chambers used in total
  // and at
  //    least 1 is "tracking" (DT or CSC)
  // -- The trajectory satisfies the "compatibility" requirements if there are
  // at least
  ...
  if (bwfilter()->isCompatibilitySatisfied()) {
    ...
    if (doRefit && !trajectoryBW.empty() && bwfilter()->goodState()){
      ...
      if (refitResult.first){
        trajectoryContainer.push_back(new Trajectory(refitResult.second));
        LogTrace(metname) << "StandAloneMuonTrajectoryBuilder Refit output " <<
  endl; LogTrace(metname) <<
  debug.dumpTSOS(refitResult.second.lastMeasurement().updatedState());
      }
      else
  ...
  //trajectory w/o the latest measurement and a looser chi2 cut
    ...
    LogTrace(metname) << "Compatibility NOT satisfied after Backward filter!" <<
  endl; LogTrace(metname) << "The result of Forward filter will be loaded!" <<
  endl;
  ...
  */
  ...
  if (bwfilter()->goodState()) {
    ...
    LogTrace(metname) << "Compatibility satisfied after Backward filter, but "
                         "too few valid RecHits ("
                      << trajectoryBW.foundHits() << ")." << endl
                      << "The (good) result of FW filter will be loaded!"
                      << endl;
    trajectoryContainer.push_back(new Trajectory(trajectoryFW));
    return trajectoryContainer;
  } else {
    LogTrace(metname)
        ...
        << "The Forward trajectory will be invalidated and then loaded!"
        << endl;
    Trajectory defaultTraj(seed, fwDirection);
    filter()->createDefaultTrajectory(trajectoryFW, defaultTraj);
    trajectoryContainer.push_back(new Trajectory(defaultTraj));
...
StandAloneMuonTrajectoryBuilder::propagateTheSeedTSOS(
  ...
  LogTrace(metname) << "Seed's Pt: "
                    << initialState.freeState()->momentum().perp() << endl;
  ...
  PropagationDirection detLayerOrder =
      (theSeedPosition == recoMuon::in) ? oppositeToMomentum : alongMomentum;
  ...
  if (theNavigationType == "Standard")
    detLayers = theService->muonNavigationSchool()->compatibleLayers(
        *initialLayer, *initialState.freeState(), detLayerOrder);
  else if (theNavigationType == "Direct") {
    DirectMuonNavigation navigation(&*theService->detLayerGeometry());
    detLayers =
        navigation.compatibleLayers(*initialState.freeState(), detLayerOrder);
  } else
    edm::LogError(metname) << "No Properly Navigation Selected!!" << endl;

  LogTrace(metname) << "There are " << detLayers.size() << " compatible layers"
                    << endl;
  ...
  if (!detLayers.empty()) {
    ...
    for (vector<const DetLayer *>::const_iterator layer = detLayers.begin();
      ...
      LogTrace(metname) << debug.dumpMuonId(
    ...
    const TrajectoryStateOnSurface propagatedState =
        theService->propagator(theSeedPropagatorName)
            ->propagate(initialState, finalLayer->surface());

    if (propagatedState.isValid()) {
      result = DetLayerWithState(finalLayer, propagatedState);

      LogTrace(metname) << "Trajectory State on Surface after the extrapolation"
                        << endl;
      LogTrace(metname) << debug.dumpTSOS(propagatedState);
      LogTrace(metname) << debug.dumpLayer(finalLayer);
    } else
      LogTrace(metname) << "Extrapolation failed. Keep the original seed state"
                        << endl;
  } else
    LogTrace(metname) << "No compatible layers. Keep the original seed state"
                      << endl;
