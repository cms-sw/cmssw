...
MuonDetLayerMeasurements::MuonDetLayerMeasurements(
    edm::InputTag dtlabel, edm::InputTag csclabel, edm::InputTag rpclabel,
    edm::InputTag gemlabel, edm::InputTag me0label, edm::ConsumesCollector &iC,
    bool enableDT, bool enableCSC, bool enableRPC, bool enableGEM,
    bool enableME0)
    : enableDTMeasurement(enableDT), enableCSCMeasurement(enableCSC),
      enableRPCMeasurement(enableRPC), enableGEMMeasurement(enableGEM),
      enableME0Measurement(enableME0), theDTRecHits(), theCSCRecHits(),
      theRPCRecHits(), theGEMRecHits(), theDTEventCacheID(0),
      theCSCEventCacheID(0), theRPCEventCacheID(0), theGEMEventCacheID(0),
      theME0EventCacheID(0), theEvent(nullptr) {
...
MuonRecHitContainer
MuonDetLayerMeasurements::recHits(const GeomDet *geomDet,
  ...
  if (geoId.subdetId() == MuonSubdetId::DT) {
    if (enableDTMeasurement) {
      ...
      for (DTRecSegment4DCollection::const_iterator rechit = range.first;
        ...
        result.push_back(
            MuonTransientTrackingRecHit::specificBuild(geomDet, &*rechit));
    }
  }
    ...
    if (enableGEMMeasurement) {
      ...
      LogDebug("Muon|RecoMuon|MuonDetLayerMeasurements")
          << "Number of GEM rechits available =  " << theGEMRecHits->size()
    ...
    if (enableME0Measurement) {
      ...
      LogDebug("Muon|RecoMuon|MuonDetLayerMeasurements")
          << "Number of ME0 rechits available =  " << theME0RecHits->size()
          << ", from chamber: " << chamberId << std::endl;
    ...
    // wrong type
    throw cms::Exception("MuonDetLayerMeasurements")
        << "The DetLayer with det " << geoId.det() << " subdet "
...
MeasurementContainer MuonDetLayerMeasurements::measurements(
...
MeasurementContainer MuonDetLayerMeasurements::measurements(
  ...
  for (std::vector<DetWithState>::const_iterator detWithStateItr = dss.begin();
    ...
    MeasurementContainer detMeasurements = measurements(
        layer, detWithStateItr->first, detWithStateItr->second, est, iEvent);
...
MeasurementContainer MuonDetLayerMeasurements::measurements(
    const DetLayer *layer, const GeomDet *det,
    const TrajectoryStateOnSurface &stateOnDet, const MeasurementEstimator &est,
    const edm::Event &iEvent) {
  MeasurementContainer result;
  ...
  for (MuonRecHitContainer::const_iterator rechit = muonRecHits.begin();
    ...
    MeasurementEstimator::HitReturnType estimate =
        est.estimate(stateOnDet, **rechit);
    LogDebug("RecoMuon") << "Dimension: " << (*rechit)->dimension()
                         << " Chi2: " << estimate.second << std::endl;
    if (estimate.first) {
      result.push_back(
          TrajectoryMeasurement(stateOnDet, *rechit, estimate.second, layer));
    }
  }
...
MeasurementContainer MuonDetLayerMeasurements::fastMeasurements(
  ...
  for (MuonRecHitContainer::const_iterator irh = rhs.begin(); irh != rhs.end();
       irh++) {
    MeasurementEstimator::HitReturnType estimate =
        est.estimate(theStateOnDet, (**irh));
    if (estimate.first) {
      result.push_back(
          TrajectoryMeasurement(theStateOnDet, (*irh), estimate.second, layer));
    }
  }
...
MeasurementContainer MuonDetLayerMeasurements::fastMeasurements(
  ...
  checkEvent();
  return fastMeasurements(layer, theStateOnDet, startingState, prop, est,
                          *theEvent);
}
...
MuonDetLayerMeasurements::groupedMeasurements(
  ...
  for (std::vector<DetGroup>::const_iterator grp = groups.begin();
       grp != groups.end(); ++grp) {
    ...
    for (DetGroup::const_iterator detAndStateItr = grp->begin();
      ...
      std::vector<TrajectoryMeasurement> detMeasurements =
          measurements(layer, detAndStateItr->det(),
                       detAndStateItr->trajectoryState(), est, iEvent);
      groupMeasurements.insert(groupMeasurements.end(), detMeasurements.begin(),
                               detMeasurements.end());
    }

    if (!groupMeasurements.empty())
      std::sort(groupMeasurements.begin(), groupMeasurements.end(),
                TrajMeasLessEstim());
...
MuonRecHitContainer
MuonDetLayerMeasurements::recHits(const DetLayer *layer,
                                  const edm::Event &iEvent) {
