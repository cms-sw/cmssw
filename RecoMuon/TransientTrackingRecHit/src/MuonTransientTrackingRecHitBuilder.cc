...
MuonTransientTrackingRecHitBuilder::build(
  ...
  if (p->geographicalId().det() == DetId::Muon) {
    return MuonTransientTrackingRecHit::specificBuild(
        trackingGeometry->idToDet(p->geographicalId()), p);
  }
...
MuonTransientTrackingRecHitBuilder::build(const TrackingRecHit *p) const {
  if (theTrackingGeometry.isValid())
    ...
    throw cms::Exception("Muon|RecoMuon|MuonTransientTrackingRecHitBuilder")
        << "ERROR! You are trying to build a MuonTransientTrackingRecHit with "
           "a non valid GlobalTrackingGeometry";
}

MuonTransientTrackingRecHitBuilder::ConstRecHitContainer
MuonTransientTrackingRecHitBuilder::build(
    const trackingRecHit_iterator &start,
    const trackingRecHit_iterator &stop) const {
