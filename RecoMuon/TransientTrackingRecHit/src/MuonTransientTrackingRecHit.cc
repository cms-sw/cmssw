...
MuonTransientTrackingRecHit::MuonTransientTrackingRecHit(
    const MuonTransientTrackingRecHit &other)
...
GlobalError MuonTransientTrackingRecHit::globalDirectionError() const {
  return ErrorFrameTransformer().transform(localDirectionError(),
                                           (det()->surface()));
}
...
AlgebraicSymMatrix MuonTransientTrackingRecHit::parametersError() const {
  ...
  if (APE != nullptr) {
    ...
    if (err.num_row() == 1) {
      ...
      directions[0] = 0.;
      directions[1] = 0.;
      LocalErrorExtended lape = ErrorFrameTransformer().transform46(
          APE->globalError(), positions, directions);
      err[0][0] += lape.cxx();
      ...
      if (err.num_row() != lapeMatrix.num_row())
        throw cms::Exception("MuonTransientTrackingRecHit::parametersError")
            << "Discrepancy between alignment error matrix and error matrix: "
               "APE "
            << lapeMatrix.num_row() << ", error matrix " << err.num_row()
            << std::endl;
...
MuonTransientTrackingRecHit::transientHits() const {
  ...
  if (ownRecHits.empty()) {
    theSubTransientRecHits.push_back(
        TransientTrackingRecHit::RecHitPointer(clone()));
  ...
  for (std::vector<const TrackingRecHit *>::const_iterator rechit =
           ownRecHits.begin();
    ...
    if (gemDetMap_iter != gemDetMap.end())
      theSubTransientRecHits.push_back(TransientTrackingRecHit::RecHitPointer(
          new MuonTransientTrackingRecHit(gemDetMap_iter->second, *rechit)));
    else if ((*rechit)->geographicalId() ==
             det()->geographicalId()) // Phi in DT is on Chamber
      theSubTransientRecHits.push_back(TransientTrackingRecHit::RecHitPointer(
          new MuonTransientTrackingRecHit(det(), *rechit)));
  }
  return theSubTransientRecHits;
}
...
void MuonTransientTrackingRecHit::invalidateHit() {
  ...
  if (isDT()) {
    if (dimension() > 1) { // MB4s have 2D, but formatted in 4D segments
      ...
      // load 1D hits (2D --> 1D)
      for (std::vector<TrackingRecHit *>::iterator it = seg2D.begin();
           it != seg2D.end(); ++it) {
        std::vector<TrackingRecHit *> hits1D = (*it)->recHits();
        (*it)->setType(bad);
        for (std::vector<TrackingRecHit *>::iterator it2 = hits1D.begin();
             it2 != hits1D.end(); ++it2)
          (*it2)->setType(bad);
      }
    }
    ...
    if (dimension() == 4) {
      std::vector<TrackingRecHit *> hits =
          recHits(); // load 2D hits (4D --> 1D)
      for (std::vector<TrackingRecHit *>::iterator it = hits.begin();
           it != hits.end(); ++it)
        (*it)->setType(bad);
    }
}
