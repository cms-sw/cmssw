...
TransientTrackingRecHit::ConstRecHitContainer MuonTransientTrackingRecHit::transientHits() const {
  ...
  if (isDT() && dimension() == 2 && ownRecHits.front()->dimension() == 1 &&
    ...
    int sl = 1;
    for (auto geoDet = geomDets.begin(); geoDet != geomDets.end(); ++geoDet) {
  ...
  if (isDT()) {
    if (dimension() > 1) {                             // MB4s have 2D, but formatted in 4D segments
      ...
      for (auto it = seg2D.begin(); it != seg2D.end(); ++it) {
        ...
        (*it)->setType(bad);
        for (auto it2 = hits1D.begin(); it2 != hits1D.end(); ++it2)
          (*it2)->setType(bad);
      }
    }
  } else if (isCSC())
