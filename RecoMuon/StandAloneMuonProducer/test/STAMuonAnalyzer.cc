...
void STAMuonAnalyzer::beginJob() {
  ...
  hPTDiffvsEta = new TH2F("PTDiffvsEta", "p_{T}^{rec} - p_{T}^{gen} VS #eta",
                          100, -2.5, 2.5, 250, -120, 120);
  hPTDiffvsPhi = new TH2F("PTDiffvsPhi", "p_{T}^{rec} - p_{T}^{gen} VS #phi",
                          100, -6, 6, 250, -120, 120);
...
void STAMuonAnalyzer::endJob() {
  if (theDataType == "SimData") {
    cout << endl
         << endl
         << "Number of Sim tracks: " << numberOfSimTracks << endl;
  }
...
void STAMuonAnalyzer::analyze(const Event &event,
  ...
  for (staTrack = staTracks->begin(); staTrack != staTracks->end();
    ...
    cout << " p: " << track.impactPointTSCP().momentum().mag()
         << " pT: " << recPt << endl;
    ...
    cout << " p: " << innerTSOS.globalMomentum().mag()
         << " pT: " << innerTSOS.globalMomentum().perp() << endl;
    ...
    if (recPt && theDataType == "SimData") {
      ...
      hPTDiffvsEta->Fill(track.impactPointTSCP().position().eta(),
                         recPt - simPt);
      hPTDiffvsPhi->Fill(track.impactPointTSCP().position().phi(),
                         recPt - simPt);
