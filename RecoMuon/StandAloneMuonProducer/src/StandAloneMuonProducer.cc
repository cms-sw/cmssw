...
StandAloneMuonProducer::StandAloneMuonProducer(
  ...
  LogTrace("Muon|RecoMuon|StandAloneMuonProducer")
      << "constructor called" << endl;

  // Parameter set for the Builder
  ParameterSet trajectoryBuilderParameters =
      parameterSet.getParameter<ParameterSet>("STATrajBuilderParameters");
  ...
  ParameterSet serviceParameters =
      parameterSet.getParameter<ParameterSet>("ServiceParameters");

  // TrackLoader parameters
  ParameterSet trackLoaderParameters =
      parameterSet.getParameter<ParameterSet>("TrackLoaderParameters");
  ...
  string typeOfBuilder =
      parameterSet.getParameter<string>("MuonTrajectoryBuilder");
  if (typeOfBuilder == "StandAloneMuonTrajectoryBuilder")
    trajectoryBuilder = new StandAloneMuonTrajectoryBuilder(
        trajectoryBuilderParameters, theService, iC);
  else if (typeOfBuilder == "DirectMuonTrajectoryBuilder")
    trajectoryBuilder = new DirectMuonTrajectoryBuilder(
        trajectoryBuilderParameters, theService);
  else if (typeOfBuilder == "Exhaustive")
    trajectoryBuilder = new ExhaustiveMuonTrajectoryBuilder(
        trajectoryBuilderParameters, theService, iC);
  else {
  ...
  produces<reco::TrackCollection>("UpdatedAtVtx")
      .setBranchAlias(theAlias + "UpdatedAtVtxTracks");
  produces<TrackingRecHitCollection>().setBranchAlias(theAlias + "RecHits");
  produces<reco::TrackExtraCollection>().setBranchAlias(theAlias +
                                                        "TrackExtras");
  produces<reco::TrackToTrackMap>().setBranchAlias(theAlias +
                                                   "TrackToTrackMap");

  produces<std::vector<Trajectory>>().setBranchAlias(theAlias + "Trajectories");
  produces<TrajTrackAssociationCollection>().setBranchAlias(theAlias +
                                                            "TrajToTrackMap");
...
/// reconstruct muons
void StandAloneMuonProducer::produce(Event &event,
  ...
  LogTrace(metname) << "Taking the seeds: " << theSeedCollectionLabel.label()
                    << endl;
