...
MuonNavigationPrinter::MuonNavigationPrinter(
    const MuonDetLayerGeometry *muonLayout, MuonNavigationSchool const &sh,
    bool enableCSC, bool enableRPC, bool enableGEM, bool enableME0)
    : school(&sh) {
  ...
  PRINT("MuonNavigationPrinter")
      << "There are " << barrel.size() << " Barrel DetLayers";
  for (auto i : barrel)
    printLayer(i);
  PRINT("MuonNavigationPrinter")
      << "================================" << std::endl;
  ...
  PRINT("MuonNavigationPrinter")
      << "There are " << backward.size() << " Backward DetLayers";
  for (auto i : backward)
    printLayer(i);
  PRINT("MuonNavigationPrinter")
      << "==============================" << std::endl;
  ...
  PRINT("MuonNavigationPrinter")
      << "There are " << forward.size() << " Forward DetLayers" << std::endl;
  for (auto i : forward)
    printLayer(i);
}
...
MuonNavigationPrinter::MuonNavigationPrinter(
  ...
  PRINT("MuonNavigationPrinter")
      << "There are " << tkbarrel.size() << " Tk Barrel DetLayers" << std::endl;
  //  for ( tkiter = tkbarrel.begin(); tkiter != tkbarrel.end(); tkiter++ )
  //  printLayer(*tkiter);
  vector<const DetLayer *> barrel = muonLayout->allBarrelLayers();
  PRINT("MuonNavigationPrinter")
      << "There are " << barrel.size() << " Mu Barrel DetLayers";
  for (auto i : barrel)
    printLayer(i);
  PRINT("MuonNavigationPrinter")
      << "================================" << std::endl;
  PRINT("MuonNavigationPrinter") << "BACKWARD:" << std::endl;
  const vector<const ForwardDetLayer *> &tkbackward =
      tracker->negForwardLayers();
  PRINT("MuonNavigationPrinter") << "There are " << tkbackward.size()
                                 << " Tk Backward DetLayers" << std::endl;
  ///  for ( tkfiter = tkbackward.begin(); tkfiter != tkbackward.end();
  ///  tkfiter++ ) printLayer(*tkfiter);
  vector<const DetLayer *> backward = muonLayout->allBackwardLayers();
  PRINT("MuonNavigationPrinter") << "There are " << backward.size()
  ...
  PRINT("MuonNavigationPrinter")
      << "==============================" << std::endl;
  PRINT("MuonNavigationPrinter") << "FORWARD:" << std::endl;
  const vector<const ForwardDetLayer *> &tkforward =
      tracker->posForwardLayers();
  PRINT("MuonNavigationPrinter") << "There are " << tkforward.size()
                                 << " Tk Forward DetLayers" << std::endl;
  //  for ( tkfiter = tkforward.begin(); tkfiter != tkforward.end(); tkfiter++ )
  //  printLayer(*tkfiter);

  vector<const DetLayer *> forward = muonLayout->allForwardLayers();
  PRINT("MuonNavigationPrinter")
      << "There are " << forward.size() << " Mu Forward DetLayers";
  for (auto i : forward)
    printLayer(i);
}
...
void MuonNavigationPrinter::printLayer(const DetLayer *layer) const {
  ...
  vector<const DetLayer *> compatibleLayers =
      school->compatibleLayers(*layer, insideOut);
  if (const BarrelDetLayer *bdl = dynamic_cast<const BarrelDetLayer *>(layer)) {
    PRINT("MuonNavigationPrinter")
        << layer->location() << " " << layer->subDetector()
        << " layer at R: " << setiosflags(ios::showpoint | ios::fixed)
        << setw(8) << setprecision(2) << bdl->specificSurface().radius()
        << "  length: " << setw(6) << setprecision(2)
        << layer->surface().bounds().length() << std::endl;

  } else if (const ForwardDetLayer *fdl =
                 dynamic_cast<const ForwardDetLayer *>(layer)) {
    PRINT("MuonNavigationPrinter")
        << endl
        << layer->location() << " " << layer->subDetector()
        << "layer at z: " << setiosflags(ios::showpoint | ios::fixed) << setw(8)
        << setprecision(2) << layer->surface().position().z()
        << "  inner r: " << setw(6) << setprecision(2)
        << fdl->specificSurface().innerRadius() << "  outer r: " << setw(6)
        << setprecision(2) << fdl->specificSurface().outerRadius() << std::endl;
  }
  PRINT("MuonNavigationPrinter")
      << " has " << nextLayers.size()
...
void MuonNavigationPrinter::printLayers(
    const vector<const DetLayer *> &nextLayers) const {
  ...
  for (vector<const DetLayer *>::const_iterator inext = nextLayers.begin();
    ...
    if ((*inext)->location() == GeomDetEnumerators::barrel) {
      const BarrelDetLayer *l =
          dynamic_cast<const BarrelDetLayer *>(&(**inext));
      PRINT("MuonNavigationPrinter")
          << (*inext)->location() << " " << (*inext)->subDetector()
          << " layer at R: " << setiosflags(ios::showpoint | ios::fixed)
          << setw(8) << setprecision(2) << l->specificSurface().radius()
          << "   " << std::endl;
    } else {
      const ForwardDetLayer *l =
          dynamic_cast<const ForwardDetLayer *>(&(**inext));
      PRINT("MuonNavigationPrinter")
          << (*inext)->location() << " " << (*inext)->subDetector()
          << " layer at z: " << setiosflags(ios::showpoint | ios::fixed)
          << setw(8) << setprecision(2) << l->surface().position().z() << "   "
          << std::endl;
    }
    PRINT("MuonNavigationPrinter")
        << setiosflags(ios::showpoint | ios::fixed) << setprecision(1)
        << setw(6) << (*inext)->surface().bounds().length() << ", " << setw(6)
...
/// These should not be useful anymore as SubDetector and Location enums now
/// have << operators
