...
MuonBarrelNavigableLayer::nextLayers(const FreeTrajectoryState &fts,
  ...
  if ((isInsideOut(fts) && dir == alongMomentum) ||
      (!isInsideOut(fts) && dir == oppositeToMomentum)) {
...
void MuonBarrelNavigableLayer::pushResult(
    std::vector<const DetLayer *> &result, const MapB &map,
...
void MuonBarrelNavigableLayer::pushResult(
    std::vector<const DetLayer *> &result, const MapE &map,
    const FreeTrajectoryState &fts) const {
...
void MuonBarrelNavigableLayer::pushCompatibleResult(
    std::vector<const DetLayer *> &result, const MapB &map,
    const FreeTrajectoryState &fts) const {
...
void MuonBarrelNavigableLayer::setDetLayer(const DetLayer *dl) {
  edm::LogError("MuonBarrelNavigableLayer")
