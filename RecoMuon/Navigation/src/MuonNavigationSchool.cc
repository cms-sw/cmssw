...
MuonNavigationSchool::MuonNavigationSchool(
    const MuonDetLayerGeometry *muonLayout, bool enableRPC, bool enableCSC,
    bool enableGEM, bool enableME0)
  ...
  // else endcap = muonLayout->allCSCLayers(); //CSC only for all the remaining
  // cases Trying allEndcaplayers in all other cases, as in the GEM PR
  else
...
void MuonNavigationSchool::linkBarrelLayers() {
  ...
  for (MapBI bl = theBarrelLayers.begin(); bl != theBarrelLayers.end(); bl++) {
    ...
    for (MapEI el = theBackwardLayers.begin(); el != theBackwardLayers.end();
         el++) {
    ...
    for (MapEI el = theForwardLayers.begin(); el != theForwardLayers.end();
         el++) {
      if ((*el).second.isCompatible(range)) {
    ...
    theBarrelNLC.push_back(new MuonBarrelNavigableLayer(
        (*bl).first, outerBarrel, outerBackward, outerForward, allOuterBarrel,
        allOuterBackward, allOuterForward));
  }
}
/// linking forward/backward layers outwards
void MuonNavigationSchool::linkEndcapLayers(
    const MapE &layers, vector<MuonForwardNavigableLayer *> &result) {
  ...
  for (MapEI el = layers.begin(); el != layers.end(); el++) {
    ...
    result.push_back(new MuonForwardNavigableLayer((*el).first, outerLayers,
                                                   allOuterLayers));
  }
}
...
void MuonNavigationSchool::createInverseLinks() {
  ...
  for (MapBI bli = theBarrelLayers.begin(); bli != theBarrelLayers.end();
       bli++) {
    // barrel
    MuonBarrelNavigableLayer *mbnl = dynamic_cast<MuonBarrelNavigableLayer *>(
        theAllNavigableLayer[((*bli).first)->seqNum()]);
  ...
  for (MapEI eli = theBackwardLayers.begin(); eli != theBackwardLayers.end();
       eli++) {
    MapE reacheableE = dynamic_cast<MuonForwardNavigableLayer *>(
                           theAllNavigableLayer[((*eli).first)->seqNum()])
    ...
    MapE compatibleE = dynamic_cast<MuonForwardNavigableLayer *>(
                           theAllNavigableLayer[((*eli).first)->seqNum()])
  ...
  for (MapEI eli = theForwardLayers.begin(); eli != theForwardLayers.end();
       eli++) {
  ...
  for (MapEI eli = theBackwardLayers.begin(); eli != theBackwardLayers.end();
       eli++) {
    MuonForwardNavigableLayer *mfnl = dynamic_cast<MuonForwardNavigableLayer *>(
        theAllNavigableLayer[((*eli).first)->seqNum()]);
