...
DirectMuonNavigation::DirectMuonNavigation(
    const edm::ESHandle<MuonDetLayerGeometry> &muonLayout)
    : theMuonDetLayerGeometry(muonLayout), epsilon_(100.), theEndcapFlag(true),
      theBarrelFlag(true) {}

DirectMuonNavigation::DirectMuonNavigation(
    const edm::ESHandle<MuonDetLayerGeometry> &muonLayout,
    const edm::ParameterSet &par)
    : theMuonDetLayerGeometry(muonLayout), epsilon_(100.),
...
void DirectMuonNavigation::inOutBarrel(const FreeTrajectoryState &fts,
                                       vector<const DetLayer *> &output) const {

  bool cont = false;
  const vector<const DetLayer *> &barrel =
      theMuonDetLayerGeometry->allBarrelLayers();

  for (vector<const DetLayer *>::const_iterator iter_B = barrel.begin();
       iter_B != barrel.end(); iter_B++) {

    if (cont)
      output.push_back((*iter_B));
    else if (checkCompatible(fts,
                             dynamic_cast<const BarrelDetLayer *>(*iter_B))) {
      output.push_back((*iter_B));
      cont = true;
    }
  }
...
void DirectMuonNavigation::outInBarrel(const FreeTrajectoryState &fts,
  ...
  for (vector<const DetLayer *>::const_iterator iter_B = rbegin; iter_B != rend;
       iter_B--) {
    if (cont)
      output.push_back((*iter_B));
    else if (checkCompatible(fts,
...
void DirectMuonNavigation::inOutForward(
    const FreeTrajectoryState &fts, vector<const DetLayer *> &output) const {

  const vector<const DetLayer *> &forward =
      theMuonDetLayerGeometry->allForwardLayers();
  bool cont = false;
  for (vector<const DetLayer *>::const_iterator iter_E = forward.begin();
       iter_E != forward.end(); iter_E++) {
    if (cont)
      output.push_back((*iter_E));
    else if (checkCompatible(fts,
                             dynamic_cast<const ForwardDetLayer *>(*iter_E))) {
      output.push_back((*iter_E));
      cont = true;
    }
  }
...
void DirectMuonNavigation::outInForward(
    const FreeTrajectoryState &fts, vector<const DetLayer *> &output) const {
...
void DirectMuonNavigation::inOutBackward(
    const FreeTrajectoryState &fts, vector<const DetLayer *> &output) const {
  bool cont = false;
  const vector<const DetLayer *> &backward =
      theMuonDetLayerGeometry->allBackwardLayers();

  for (vector<const DetLayer *>::const_iterator iter_E = backward.begin();
       iter_E != backward.end(); iter_E++) {
    if (cont)
      output.push_back((*iter_E));
    else if (checkCompatible(fts,
                             dynamic_cast<const ForwardDetLayer *>(*iter_E))) {
...
void DirectMuonNavigation::outInBackward(
    const FreeTrajectoryState &fts, vector<const DetLayer *> &output) const {
