...
void L3MuonCleaner::produce(edm::Event& event, const edm::EventSetup&){
  ...
  for ( reco::TrackCollection::const_iterator trk=tracks->begin(); trk!=tracks->end(); ++trk ){
    ...
    if (trk->getHitPattern().numberOfValidTrackerHits(reco::HitPattern::TRACK_HITS) < m_minTrkHits) continue;
    if (trk->getHitPattern().numberOfValidMuonHits(reco::HitPattern::TRACK_HITS) < m_minMuonHits) continue;
