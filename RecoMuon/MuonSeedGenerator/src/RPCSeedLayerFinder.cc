...
void RPCSeedLayerFinder::unsetInput() { isInputset = false; }
...
void RPCSeedLayerFinder::fill() {
  ...
  if (isConfigured == false || isInputset == false || isOutputset == false) {
    cout << "RPCSeedLayerFinder needs to be configured and set IO before running RPCSeedLayerFinder::fillLayers()"
         << endl;
    return;
  }
...
void RPCSeedLayerFinder::SpecialLayers(int last, unsigned int NumberofLayers, int type) {
  ...
  if (type == 1) {
    ...
    if (last < (BarrelLayerNumber + EachEndcapLayerNumber - 1) ||
        (last == (BarrelLayerNumber + EachEndcapLayerNumber - 1) && !LayersinRPC.empty())) {
      // For -Z case
      for (unsigned int i = (last + 1);
           i <= (BarrelLayerNumber + EachEndcapLayerNumber - NumberofLayers + LayersinRPC.size());
           i++) {
        if (recHitsinLayers[i] != 0) {
          LayersinRPC.push_back(i);
          last = i;
      ...
      // For +Z case
      for (unsigned int i = (last + 1);
           i <= (BarrelLayerNumber + EachEndcapLayerNumber * 2 - NumberofLayers + LayersinRPC.size());
           i++) {
        if (recHitsinLayers[i] != 0) {
          LayersinRPC.push_back(i);
          last = i;
