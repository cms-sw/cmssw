...
void RPCSeedPattern::ThreePointsAlgorithm() {
  ...
  if (NumberofStraight == NumberofPart) {
    ...
    double meanR = 0.;
    for (auto iter = theRecHits.begin(); iter != theRecHits.end(); iter++)
...
void RPCSeedPattern::SegmentAlgorithm() {
  ...
  unsigned int n = 0;
  for (auto iter = theRecHits.begin(); iter != (theRecHits.end() - 2); iter++) {
...
void RPCSeedPattern::SegmentAlgorithmSpecial(const edm::EventSetup& eSetup) {
  ...
  for (auto iter = theRecHits.begin(); iter != (theRecHits.end() - 1); iter++) {
  ...
  for (auto BIter = BValue.begin(); BIter != BValue.end(); BIter++)
    meanB2 += (*BIter);
  meanB2 /= BValue.size();
