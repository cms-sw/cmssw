...
MuonOverlapSeedFromRecHits::MuonOverlapSeedFromRecHits() : MuonSeedFromRecHits() {}
...
std::vector<TrajectorySeed> MuonOverlapSeedFromRecHits::seeds() const {
  ...
  for (const auto& barrelHit : barrelHits) {
    for (const auto& endcapHit : endcapHits) {
      TrajectorySeed seed;
      bool good = makeSeed(barrelHit, endcapHit, bestSegment, seed);
      if (good)
...
MuonTransientTrackingRecHit::ConstMuonRecHitPointer MuonOverlapSeedFromRecHits::bestHit(
