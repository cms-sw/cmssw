...
MuonCSCDetLayerGeometryBuilder::buildLayers(const CSCGeometry &geo) {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    for (int station = 2; station <= CSCDetId::maxStationId(); station++) {
      vector<int> rings;
      for (int ring = CSCDetId::minRingId(); ring <= CSCDetId::maxRingId();
           ring++) {
...
MuRingForwardDoubleLayer *MuonCSCDetLayerGeometryBuilder::buildLayer(
    int endcap, int station, vector<int> &rings, const CSCGeometry &geo) {
  ...
  for (vector<int>::iterator ring = rings.begin(); ring != rings.end();
       ring++) {
...
bool MuonCSCDetLayerGeometryBuilder::isFront(int station, int ring,
                                             int chamber) {
  bool result = false;
...
MuDetRing *
MuonCSCDetLayerGeometryBuilder::makeDetRing(vector<const GeomDet *> &geomDets) {
  const std::string metname =
      "Muon|RecoMuon|RecoMuonDetLayers|MuonCSCDetLayerGeometryBuilder";
