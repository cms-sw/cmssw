...
MuRingForwardLayer::MuRingForwardLayer(
    const vector<const ForwardDetRing *> &rings)
    : RingedForwardLayer(false), theRings(rings),
      theComponents(theRings.begin(), theRings.end()), theBinFinder(nullptr),
      isOverlapping(false) {
  ...
  setSurface(new BoundDisk(
      pos, rot,
      new SimpleDiskBounds(theRmin, theRmax, theZmin - zPos, theZmax - zPos)));
...
vector<GeometricSearchDet::DetWithState> MuRingForwardLayer::compatibleDets(
  ...
  if (tsos.hasError()) {
    LogTrace(metname) << " sR: " << sqrt(tsos.localError().positionError().yy())
                      << " sX: "
                      << sqrt(tsos.localError().positionError().xx());
  }
  ...
  for (unsigned int idet = closest + 1; idet < theRings.size(); idet++) {
    bool inside = false;
    if (tsos.hasError()) {
      inside = theRings[idet]->specificSurface().bounds().inside(
...
vector<DetGroup> MuRingForwardLayer::groupedCompatibleDets(
    const TrajectoryStateOnSurface &startingState, const Propagator &prop,
    const MeasurementEstimator &est) const {
  // FIXME should return only 1 group
  cout << "dummy implementation of MuRingForwardLayer::groupedCompatibleDets()"
       << endl;
