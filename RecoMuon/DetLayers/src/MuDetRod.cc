...
MuDetRod::MuDetRod(const vector<const GeomDet *> &vdets) : DetRodOneR(vdets) {
  init();
}

void MuDetRod::init() {
  theBinFinder =
      BinFinderType(basicComponents().begin(), basicComponents().end());
}
  ...
  if (!result.empty()) {
    ...
    for (unsigned int idet = closest + 1; idet < dets.size(); idet++) {
      ...
      if (fabs(nextPos.y()) < detHalfLen + maxDistance.y()) {
        LogTrace(metname) << "     negativeZ: det:" << idet << " pos "
                          << nextPos.y() << " maxDistance " << maxDistance.y();
        nnextdet++;
    ...
    for (int idet = closest - 1; idet >= 0; idet--) {
      LocalPoint nextPos(dets[idet]->toLocal(startPos));
      if (fabs(nextPos.y()) < detHalfLen + maxDistance.y()) {
        LogTrace(metname) << "     positiveZ: det:" << idet << " pos "
                          << nextPos.y() << " maxDistance " << maxDistance.y();
        nnextdet++;
        if (!add(idet, result, tsos, prop, est))
          break;
