...
MuRodBarrelLayer::MuRodBarrelLayer(vector<const DetRod *> &rods)
    : RodBarrelLayer(false), theRods(rods), theBinFinder(nullptr),
      isOverlapping(false) {
  // Sort rods in phi
  precomputed_value_sort(theRods.begin(), theRods.end(),
                         geomsort::ExtractPhi<DetRod, float>());
  ...
  LogTrace(metname) << "Constructing MuRodBarrelLayer: "
                    << basicComponents().size() << " Dets " << theRods.size()
                    << " Rods "
...
MuRodBarrelLayer::~MuRodBarrelLayer() {
  delete theBinFinder;
  for (vector<const DetRod *>::iterator i = theRods.begin(); i < theRods.end();
       i++) {
    delete *i;
  }
}
...
MuRodBarrelLayer::compatibleDets(const TrajectoryStateOnSurface &startingState,
  ...
  LogTrace(metname) << "     MuRodBarrelLayer::compatibleDets, closestRod: "
                    << closest
  ...
  if (!result.empty()) {
    ...
    if (tsos.globalPosition().phi() - closestRod->surface().position().phi() >
        0.) {
      dist = -1.;
    } else {
      dist = +1.;
  ...
  LogTrace(metname) << "     MuRodBarrelLayer::fastCompatibleDets: found: "
                    << result.size() << " on closest: " << nclosest
                    << " # checked rods: " << 1 + int(checknext);

  return result;
}

vector<DetGroup> MuRodBarrelLayer::groupedCompatibleDets(
    const TrajectoryStateOnSurface &startingState, const Propagator &prop,
    const MeasurementEstimator &est) const {
  // FIXME should return only 1 group
  cout << "dummy implementation of MuRodBarrelLayer::groupedCompatibleDets()"
       << endl;
