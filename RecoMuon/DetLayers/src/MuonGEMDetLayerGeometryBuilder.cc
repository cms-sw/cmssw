...
MuonGEMDetLayerGeometryBuilder::buildEndcapLayers(const GEMGeometry &geo) {
  ...
  for (int endcap = -1; endcap <= 1; endcap += 2) {
    ...
    for (int station = GEMDetId::minStationId;
         station <= GEMDetId::maxStationId; ++station) {
      for (int layer = GEMDetId::minLayerId + 1; layer <= GEMDetId::maxLayerId;
           ++layer) {

        vector<int> rolls, rings, chambers;
        for (int ring = GEMDetId::minRingId; ring <= GEMDetId::maxRingId;
             ++ring)
          rings.push_back(ring);
        for (int chamber = GEMDetId::minChamberId + 1;
             chamber <= GEMDetId::maxChamberId; chamber++)
          chambers.push_back(chamber);
        for (int roll = GEMDetId::minRollId + 1; roll <= GEMDetId::maxRollId;
             ++roll)
          rolls.push_back(roll);

        MuRingForwardDoubleLayer *ringLayer =
            buildLayer(endcap, rings, station, layer, chambers, rolls, geo);
...
MuRingForwardDoubleLayer *MuonGEMDetLayerGeometryBuilder::buildLayer(
    int endcap, vector<int> &rings, int station, int layer,
    vector<int> &chambers, vector<int> &rolls, const GEMGeometry &geo) {
  ...
  for (std::vector<int>::iterator ring = rings.begin(); ring != rings.end() - 2;
       ring++) {
    ...
    for (vector<int>::iterator roll = rolls.begin(); roll != rolls.end();
      ...
      for (std::vector<int>::iterator chamber = chambers.begin();
        ...
        if (geomDet != nullptr) {
          ...
          LogTrace(metname) << "get GEM Endcap roll " << gemId
  ...
  //    geomsort::ExtractZ<GeometricSearchDet,float>());
...
MuDetRing *
MuonGEMDetLayerGeometryBuilder::makeDetRing(vector<const GeomDet *> &geomDets) {
  const std::string metname =
      "Muon|RecoMuon|RecoMuonDetLayers|MuonGEMDetLayerGeometryBuilder";
