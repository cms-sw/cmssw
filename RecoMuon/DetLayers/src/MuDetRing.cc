...
MuDetRing::compatible(const TrajectoryStateOnSurface &ts,
...
MuDetRing::compatibleDets(const TrajectoryStateOnSurface &startingState,
  ...
  LogTrace(metname) << "     MuDetRing::compatibleDets, closest det: "
                    << closest
  ...
  float dphi = 0;
  if (!result
           .empty()) { // If closest is not compatible the next cannot be either
    float nSigmas = 3.;
  ...
  for (int idet = closest + 1; idet < closest + int(dets.size()) / 4 + 1;
       idet++) {
  ...
  for (int idet = closest - 1; idet > closest - int(dets.size()) / 4 - 1;
    ...
    int idetp = theBinFinder.binIndex(idet);
    {
      LogTrace(metname) << "     previous det:" << idetp << " " << idet << " "
