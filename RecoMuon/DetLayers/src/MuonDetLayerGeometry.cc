...
MuonDetLayerGeometry::~MuonDetLayerGeometry() {
  for (vector<const DetLayer *>::const_iterator it = allDetLayers.begin();
       it != allDetLayers.end(); ++it) {
    delete *it;
  }
}

void MuonDetLayerGeometry::addCSCLayers(
    const pair<vector<DetLayer *>, vector<DetLayer *>> &csclayers) {
...
void MuonDetLayerGeometry::addGEMLayers(
    const pair<vector<DetLayer *>, vector<DetLayer *>> &gemlayers) {
...
void MuonDetLayerGeometry::addME0Layers(
    const pair<vector<DetLayer *>, vector<DetLayer *>> &me0layers) {
...
void MuonDetLayerGeometry::addRPCLayers(
    const vector<DetLayer *> &barrelLayers,
  ...
  if (detLayer->subDetector() == GeomDetEnumerators::CSC) {
    ...
    if (id.station() == 1) {
      if (id.ring() == 1 || id.ring() == 4)
        ...
        throw cms::Exception("InvalidCSCRing")
            << " Invalid CSC Ring: " << id.ring() << endl;
    } else
      return CSCDetId(id.endcap(), id.station(), 0, 0, 0);

  } else if (detLayer->subDetector() == GeomDetEnumerators::DT) {
    DTChamberId id(
        detLayer->basicComponents().front()->geographicalId().rawId());
    ...
    throw cms::Exception(
        "InvalidModuleIdentification"); // << detLayer->module();
}
...
const DetLayer *MuonDetLayerGeometry::idToLayer(const DetId &detId) const {
  ...
  if (detId.subdetId() == MuonSubdetId::CSC) {
    ...
    LogDebug("Muon|RecoMuon|MuonDetLayerGeometry")
        << " Found an ME0DetId:  " << me0Id.rawId() << ",id: " << id.rawId()
        << std::endl;

  } else
    throw cms::Exception("InvalidSubdetId") << detId.subdetId();

  std::map<DetId, const DetLayer *>::const_iterator layer =
      detLayersMap.find(id);
...
void MuonDetLayerGeometry::sortLayers() {
  ...
  // precomputed_value_sort(dtLayers.begin(),
  // dtLayers.end(),ExtractR<DetLayer,float>());
  // precomputed_value_sort(cscLayers_fw.begin(),
  // cscLayers_fw.end(),ExtractAbsZ<DetLayer,float>());
  // precomputed_value_sort(cscLayers_bk.begin(),
  // cscLayers_bk.end(),ExtractAbsZ<DetLayer,float>());
  // precomputed_value_sort(rpcLayers_fw.begin(),
  // rpcLayers_fw.end(),ExtractAbsZ<DetLayer,float>());
  // precomputed_value_sort(rpcLayers_bk.begin(),
  // rpcLayers_bk.end(),ExtractAbsZ<DetLayer,float>());
  // precomputed_value_sort(rpcLayers_barrel.begin(), rpcLayers_barrel.end(),
  // ExtractR<DetLayer,float>());

  // Sort these inside-out
  precomputed_value_sort(allBarrel.begin(), allBarrel.end(),
                         ExtractBarrelDetLayerR());
  precomputed_value_sort(allBackward.begin(), allBackward.end(),
                         ExtractAbsZ<DetLayer, float>());
  precomputed_value_sort(allForward.begin(), allForward.end(),
                         ExtractAbsZ<DetLayer, float>());
  ...
  std::copy(cscLayers_bk.begin(), cscLayers_bk.end(),
            back_inserter(cscLayers_all));
  std::reverse(cscLayers_all.begin(), cscLayers_all.end());
  std::copy(cscLayers_fw.begin(), cscLayers_fw.end(),
            back_inserter(cscLayers_all));

  // gemLayers_all: from -Z to +Z
  gemLayers_all.reserve(gemLayers_bk.size() + gemLayers_fw.size());
  std::copy(gemLayers_bk.begin(), gemLayers_bk.end(),
            back_inserter(gemLayers_all));
  std::reverse(gemLayers_all.begin(), gemLayers_all.end());
  std::copy(gemLayers_fw.begin(), gemLayers_fw.end(),
            back_inserter(gemLayers_all));

  // me0Layers_all: from -Z to +Z
  me0Layers_all.reserve(me0Layers_bk.size() + me0Layers_fw.size());
  std::copy(me0Layers_bk.begin(), me0Layers_bk.end(),
            back_inserter(me0Layers_all));
  std::reverse(me0Layers_all.begin(), me0Layers_all.end());
  std::copy(me0Layers_fw.begin(), me0Layers_fw.end(),
            back_inserter(me0Layers_all));

  // rpcLayers_endcap: from -Z to +Z
  rpcLayers_endcap.reserve(rpcLayers_bk.size() + rpcLayers_fw.size());
  std::copy(rpcLayers_bk.begin(), rpcLayers_bk.end(),
            back_inserter(rpcLayers_endcap));
  std::reverse(rpcLayers_endcap.begin(), rpcLayers_endcap.end());
  std::copy(rpcLayers_fw.begin(), rpcLayers_fw.end(),
            back_inserter(rpcLayers_endcap));

  // rpcLayers_all: order is bw, barrel, fw
  rpcLayers_all.reserve(rpcLayers_bk.size() + rpcLayers_barrel.size() +
                        rpcLayers_fw.size());
  std::copy(rpcLayers_bk.begin(), rpcLayers_bk.end(),
            back_inserter(rpcLayers_all));
  std::reverse(rpcLayers_all.begin(), rpcLayers_all.end());
  std::copy(rpcLayers_barrel.begin(), rpcLayers_barrel.end(),
            back_inserter(rpcLayers_all));
  std::copy(rpcLayers_fw.begin(), rpcLayers_fw.end(),
            back_inserter(rpcLayers_all));
  ...
  allEndcapCscGem.reserve(cscLayers_bk.size() + cscLayers_fw.size() +
                          gemLayers_bk.size() + gemLayers_fw.size());
  std::copy(cscLayers_bk.begin(), cscLayers_bk.end(),
            back_inserter(allEndcapCscGem));
  std::copy(gemLayers_bk.begin(), gemLayers_bk.end(),
            back_inserter(allEndcapCscGem));
  ...
  std::copy(cscLayers_fw.begin(), cscLayers_fw.end(),
            back_inserter(allCscGemForward));
  std::copy(gemLayers_fw.begin(), gemLayers_fw.end(),
            back_inserter(allCscGemForward));
  ...
  std::copy(cscLayers_bk.begin(), cscLayers_bk.end(),
            back_inserter(allCscGemBackward));
  std::copy(gemLayers_bk.begin(), gemLayers_bk.end(),
            back_inserter(allCscGemBackward));
  ...
  std::copy(cscLayers_fw.begin(), cscLayers_fw.end(),
            back_inserter(allCscME0Forward));
  std::copy(me0Layers_fw.begin(), me0Layers_fw.end(),
            back_inserter(allCscME0Forward));
  ...
  std::copy(cscLayers_bk.begin(), cscLayers_bk.end(),
            back_inserter(allCscME0Backward));
  std::copy(me0Layers_bk.begin(), me0Layers_bk.end(),
            back_inserter(allCscME0Backward));

  // allEndcapCSCME0: order is  all bw, all fw
  allEndcapCscME0.reserve(cscLayers_bk.size() + cscLayers_fw.size() +
                          me0Layers_bk.size() + me0Layers_fw.size());
  std::copy(cscLayers_bk.begin(), cscLayers_bk.end(),
            back_inserter(allEndcapCscME0));
  std::copy(me0Layers_bk.begin(), me0Layers_bk.end(),
            back_inserter(allEndcapCscME0));
  ...
  allDetLayers.reserve(allBackward.size() + allBarrel.size() +
                       allForward.size());
  std::copy(allBackward.begin(), allBackward.end(),
            back_inserter(allDetLayers));
