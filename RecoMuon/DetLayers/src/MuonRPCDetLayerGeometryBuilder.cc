...
struct ExtractInnerRadius {
  ...
  Scalar operator()(const T *p) const {
    return fabs(p->specificSurface().innerRadius());
  }
  Scalar operator()(const T &p) const {
    return fabs(p.specificSurface().innerRadius());
  }
...
MuonRPCDetLayerGeometryBuilder::buildEndcapLayers(const RPCGeometry &geo) {
  ...
  for (int endcap = -1; endcap <= 1; endcap += 2) {
    ...
    // ME 1/1
    for (int layer = RPCDetId::minLayerId; layer <= RPCDetId::maxLayerId;
         ++layer) {
      vector<int> rolls;
      ...
      MuRingForwardDoubleLayer *ringLayer =
          buildLayer(endcap, rings, firstStation, layer, rolls, geo);
      if (ringLayer)
    ...
    for (int station = 2; station <= RPCDetId::maxStationId; ++station) {
      for (int layer = RPCDetId::minLayerId; layer <= RPCDetId::maxLayerId;
        ...
        vector<int> rolls;
        std::vector<int> rings;
        for (int ring = RPCDetId::minRingForwardId;
             ring <= RPCDetId::maxRingForwardId; ++ring) {
          rings.push_back(ring);
        }
        for (int roll = RPCDetId::minRollId + 1; roll <= RPCDetId::maxRollId;
             ++roll) {
        ...
        MuRingForwardDoubleLayer *ringLayer =
            buildLayer(endcap, rings, station, layer, rolls, geo);
        if (ringLayer)
          result[iendcap].push_back(ringLayer);
      }
    }
...
MuRingForwardDoubleLayer *MuonRPCDetLayerGeometryBuilder::buildLayer(
    int endcap, const std::vector<int> &rings, int station, int layer,
    vector<int> &rolls, const RPCGeometry &geo) {
  ...
  for (std::vector<int>::const_iterator ring = rings.begin();
       ring < rings.end(); ++ring) {
    for (vector<int>::iterator roll = rolls.begin(); roll != rolls.end();
         ++roll) {
      vector<const GeomDet *> frontDets, backDets;
      for (int sector = RPCDetId::minSectorForwardId;
           sector <= RPCDetId::maxSectorForwardId; ++sector) {
        for (int subsector = RPCDetId::minSubSectorForwardId;
             subsector <= RPCDetId::maxSectorForwardId; ++subsector) {
          RPCDetId rpcId(endcap, *ring, station, sector, layer, subsector,
                         (*roll));
          ...
          if (geomDet) {
            ...
            LogTrace(metname) << "get RPC Endcap roll " << rpcId
  ...
  if (!backRings.empty() || !frontRings.empty()) {
    typedef rpcdetlayergeomsort::ExtractInnerRadius<ForwardDetRing, float>
        SortRingByInnerR;
    precomputed_value_sort(frontRings.begin(), frontRings.end(),
                           SortRingByInnerR());
    precomputed_value_sort(backRings.begin(), backRings.end(),
                           SortRingByInnerR());
...
MuonRPCDetLayerGeometryBuilder::buildBarrelLayers(const RPCGeometry &geo) {
  ...
  for (int station = RPCDetId::minStationId; station <= RPCDetId::maxStationId;
       station++) {
    ...
    for (int layer = RPCDetId::minLayerId; layer <= RPCDetId::maxLayerId;
         ++layer) {
      for (int sector = RPCDetId::minSectorId; sector <= RPCDetId::maxSectorId;
           sector++) {
        for (int subsector = RPCDetId::minSubSectorId;
             subsector <= RPCDetId::maxSubSectorId; subsector++) {
          for (int wheel = RPCDetId::minRingBarrelId;
               wheel <= RPCDetId::maxRingBarrelId; wheel++) {
            for (int roll = RPCDetId::minRollId + 1;
                 roll <= RPCDetId::maxRollId; roll++) {
              const GeomDet *geomDet = geo.idToDet(RPCDetId(
                  region, wheel, station, sector, layer, subsector, roll));
              if (geomDet) {
                geomDets.push_back(geomDet);
                LogTrace(metname) << "get RPC Barrel roll "
                                  << RPCDetId(region, wheel, station, sector,
...
void MuonRPCDetLayerGeometryBuilder::makeBarrelLayers(
    vector<const GeomDet *> &geomDets, vector<MuRodBarrelLayer *> &result) {
  const std::string metname =
      "Muon|RPC|RecoMuon|RecoMuonDetLayers|MuonRPCDetLayerGeometryBuilder";
  ...
  for (unsigned int i = 0; i < rClust.size(); i++) {
    ...
    while (separ < last && float((*separ)->position().perp()) - r0 < rSepar) {
      // LogTrace(metname) << "         roll at dphi:  " <<
      // float((*separ)->position().phi())-phi0;
      separ++;
    }
...
void MuonRPCDetLayerGeometryBuilder::makeBarrelRods(
    vector<const GeomDet *> &geomDets, vector<const DetRod *> &result) {
  const std::string metname =
      "Muon|RPC|RecoMuon|RecoMuonDetLayers|MuonRPCDetLayerGeometryBuilder";
  ...
  ClusterizingHistogram hisPhi(int((phiMax - phiMin) / resolution) + 1, phiMin,
                               phiMax);
