...
public:
  ...
  /// Builds the forward (+Z, return.first) and backward (-Z, return.second)
  /// layers. Both vectors are sorted inside-out
  static std::pair<std::vector<DetLayer *>, std::vector<DetLayer *>>
  buildEndcapLayers(const ME0Geometry &geo);

private:
  // static MuRingForwardDoubleLayer* buildLayer(int endcap,int
  // layer,std::vector<int>& chambers,std::vector<int>& rolls,const ME0Geometry&
  // geo); static MuRingForwardLayer* buildLayer(int endcap,int
  // layer,std::vector<int>& chambers,std::vector<int>& rolls,const ME0Geometry&
  // geo);
  static MuRingForwardLayer *buildLayer(int endcap, int layer,
