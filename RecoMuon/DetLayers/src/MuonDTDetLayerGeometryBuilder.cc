...
MuonDTDetLayerGeometryBuilder::buildLayers(const DTGeometry &geo) {
  ...
  for (int station = DTChamberId::minStationId;
    ...
    for (int sector = DTChamberId::minSectorId;
      ...
      for (int wheel = DTChamberId::minWheelId;
           wheel <= DTChamberId::maxWheelId; wheel++) {
        const GeomDet *geomDet =
            geo.idToDet(DTChamberId(wheel, station, sector));
        if (geomDet) {
          geomDets.push_back(geomDet);
          LogTrace(metname)
              << "get DT chamber " << DTChamberId(wheel, station, sector)
      ...
      if (!geomDets.empty()) {
        precomputed_value_sort(geomDets.begin(), geomDets.end(),
                               geomsort::DetZ());
        muDetRods.push_back(new MuDetRod(geomDets));
        LogTrace(metname) << "  New MuDetRod with " << geomDets.size()
                          << " chambers at R="
                          << muDetRods.back()->position().perp()
                          << ", phi=" << muDetRods.back()->position().phi();
      }
    }
    precomputed_value_sort(muDetRods.begin(), muDetRods.end(),
                           geomsort::ExtractPhi<GeometricSearchDet, float>());
  ...
  for (vector<MuRodBarrelLayer *>::const_iterator it = result.begin();
       it != result.end(); it++)
