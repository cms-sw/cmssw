...
// Builds the forward (first) and backward (second) layers - NOTE: Currently
// just one layer, all 'front' Builds chambers (for segments) only, me0RecHits
// not used in track finding
...
MuonME0DetLayerGeometryBuilder::buildEndcapLayers(const ME0Geometry &geo) {
  ...
  for (int endcap = -1; endcap <= 1; endcap += 2) {
    ...
    rolls.push_back(0);
    for (int chamber = ME0DetId::minChamberId + 1;
         chamber <= ME0DetId::maxChamberId; chamber++)
      chambers.push_back(chamber);

    LogTrace(metname) << "Encap =  " << endcap
                      << "Chambers =  " << chambers.size()
                      << "Rolls =  " << rolls.size();
    MuRingForwardLayer *ringLayer =
        buildLayer(endcap, layer, chambers, rolls, geo);
...
MuRingForwardLayer *MuonME0DetLayerGeometryBuilder::buildLayer(
    int endcap, int layer, vector<int> &chambers, vector<int> &rolls,
  ...
  for (vector<int>::iterator roll = rolls.begin(); roll != rolls.end();
       roll++) {
    LogTrace(metname) << "On a roll ";

    vector<const GeomDet *> frontDets, backDets;

    for (std::vector<int>::iterator chamber = chambers.begin();
...
bool MuonME0DetLayerGeometryBuilder::isFront(const ME0DetId &me0Id) {
  ...
  // ME0s do not currently have an arrangement of which are front and which are
  // back, going to always return true
...
MuDetRing *
MuonME0DetLayerGeometryBuilder::makeDetRing(vector<const GeomDet *> &geomDets) {
  const std::string metname =
      "Muon|RecoMuon|RecoMuonDetLayers|MuonME0DetLayerGeometryBuilder";
