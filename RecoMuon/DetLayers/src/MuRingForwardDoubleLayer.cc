...
MuRingForwardDoubleLayer::MuRingForwardDoubleLayer(
    const vector<const ForwardDetRing *> &frontRings,
    const vector<const ForwardDetRing *> &backRings)
    : RingedForwardLayer(true), theFrontLayer(frontRings),
      theBackLayer(backRings), theRings(frontRings), // add back later
      theComponents(), theBasicComponents() {
  ...
  theComponents =
      std::vector<const GeometricSearchDet *>(theRings.begin(), theRings.end());
  ...
  for (vector<const ForwardDetRing *>::const_iterator it = theRings.begin();
    ...
    theBasicComponents.insert(theBasicComponents.end(), tmp2.begin(),
                              tmp2.end());
  }
  ...
  LogTrace(metname) << "Constructing MuRingForwardDoubleLayer: "
                    << basicComponents().size() << " Dets " << theRings.size()
                    << " Rings "
...
BoundDisk *MuRingForwardDoubleLayer::computeSurface() {
  ...
  return new BoundDisk(
      pos, rot, new SimpleDiskBounds(rmin, rmax, zmin - zPos, zmax - zPos));
}

bool MuRingForwardDoubleLayer::isInsideOut(
    const TrajectoryStateOnSurface &tsos) const {
  return tsos.globalPosition().basicVector().dot(
             tsos.globalMomentum().basicVector()) > 0;
}
...
std::pair<bool, TrajectoryStateOnSurface> MuRingForwardDoubleLayer::compatible(
  ...
  const MuRingForwardLayer &closerLayer =
      (insideOut) ? theFrontLayer : theBackLayer;
  LogTrace("Muon|RecoMuon|RecoMuonDetLayers|MuRingForwardDoubleLayer")
      << "MuRingForwardDoubleLayer::compatible is assuming inside-out "
         "direction: "
      << insideOut;
  ...
  TrajectoryStateOnSurface myState =
      prop.propagate(startingState, closerLayer.specificSurface());
  if (!myState.isValid())
  ...
  float zPos = (zmax() + zmin()) / 2.;
  SimpleDiskBounds tmp(rmin() - deltaR, rmax() + deltaR, zmin() - zPos,
                       zmax() - zPos);
...
vector<DetGroup> MuRingForwardDoubleLayer::groupedCompatibleDets(
    const TrajectoryStateOnSurface &startingState, const Propagator &prop,
    const MeasurementEstimator &est) const {
  ...
  // this should be fixed either in
  // RecoMuon/MeasurementDet/MuonDetLayerMeasurements or
  // RecoMuon/DetLayers/MuRingForwardDoubleLayer
  // and removed the reverse operation in
  // StandAloneMuonFilter::findBestMeasurements
...
bool MuRingForwardDoubleLayer::isCrack(const GlobalPoint &gp) const {
  ...
  if (backRings.size() > 1) {
    ...
    LogTrace(metname) << "In a crack:" << crackInner << " " << r << " "
                      << crackOuter;
...
void MuRingForwardDoubleLayer::selfTest() const {
  const std::vector<const GeomDet *> &frontDets =
      theFrontLayer.basicComponents();
