...
MuonDetLayerGeometryESProducer::produce(const MuonRecoGeometryRecord &record) {
  ...
  if (dt.isValid()) {
    muonDetLayerGeometry->addDTLayers(
        MuonDTDetLayerGeometryBuilder::buildLayers(*dt));
  } else {
  ...
  if (csc.isValid()) {
    muonDetLayerGeometry->addCSCLayers(
        MuonCSCDetLayerGeometryBuilder::buildLayers(*csc));
  } else {
  ...
  if (gem.isValid()) {
    muonDetLayerGeometry->addGEMLayers(
        MuonGEMDetLayerGeometryBuilder::buildEndcapLayers(*gem));
  } else {
  ...
  if (me0.isValid()) {
    muonDetLayerGeometry->addME0Layers(
        MuonME0DetLayerGeometryBuilder::buildEndcapLayers(*me0));
  } else {
  ...
  if (rpc.isValid()) {
    muonDetLayerGeometry->addRPCLayers(
        MuonRPCDetLayerGeometryBuilder::buildBarrelLayers(*rpc),
