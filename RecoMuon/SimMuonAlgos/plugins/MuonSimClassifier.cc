...
MuonSimClassifier::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
	...
	edm::LogVerbatim("MuonSimClassifier") <<"\n reco::Muon # "<<i;
		...
		if (genGMom.isNonnull()) {
		  ...
		  edm::LogVerbatim("MuonSimClassifier") << "\t\t mother prod. vertex rho = " << momRho[i] << ", z = " << momZ[i] 
		...
		edm::LogWarning("MuonSimClassifier") 
		  ...
		  <<"\n\t   has NO mother!";
			...
			<< "\t\t SIM mother is in GEN (status " << momStatus[i] << "), grand-mom id = " << gmomPdgId[i];
		      ...
		      edm::LogVerbatim("MuonSimClassifier") << "\t\t SIM mother is in SIM only, grand-mom id = " << gmomPdgId[i];
                    }
                } else {
                  edm::LogVerbatim("MuonSimClassifier") 
		      << "\t Particle pdgId = "<<hitsPdgId[i] << ", (Event,Bx) = "<< "(" <<tpEv[i]<<","<<tpBx[i]<<")"
	      ...
	      if (abs(tp->pdgId()) == 11) {
                ...
                ext[i]     = isGhost ? -1 : 1;
                edm::LogVerbatim("MuonSimClassifier") <<"\t This is not a muon. Sorry. classif[i] = " << classif[i];
	      }
	      ...
	      continue;
		    ...
		    flav[i] = 13;
                    ext[i]  = 10;
                    edm::LogVerbatim("MuonSimClassifier") <<"\t This seems PRIMARY MUON ! classif[i] = " << classif[i];
                    ...
                    edm::LogVerbatim("MuonSimClassifier") <<"\t This seems HEAVY FLAVOUR ! classif[i] = " << classif[i];
                } else {
                    classif[i] = isGhost ? -2 : 2;
                    flav[i]    = momFlav[i];
                    edm::LogVerbatim("MuonSimClassifier") <<"\t This seems LIGHT FLAVOUR ! classif[i] = " << classif[i];
                }
            } else {
                classif[i] = isGhost ? -2 : 2;
                flav[i]    = momFlav[i];
            ...
            edm::LogVerbatim("MuonSimClassifier") <<"\t Extended classification code = " << ext[i];
	}  // if (tp.isNonnull())
