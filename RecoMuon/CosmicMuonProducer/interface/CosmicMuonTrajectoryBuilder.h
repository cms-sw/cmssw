...
public:
  ...
  PropagationDirection checkDirectionByT0(const DTRecSegment4D *,
                                          const DTRecSegment4D *) const;

private:
  MuonTransientTrackingRecHit::MuonRecHitContainer
  unusedHits(const DetLayer *, const TrajectoryMeasurement &) const;

  void buildSecondHalf(Trajectory &);

  void build(const TrajectoryStateOnSurface &, const NavigationDirection &,
             Trajectory &);

  TrajectoryStateOnSurface
  intermediateState(const TrajectoryStateOnSurface &) const;
  ...
  findBestMeasurements(const DetLayer *, const TrajectoryStateOnSurface &,
                       const Propagator *, const MeasurementEstimator *);

  void incrementChamberCounters(const DetLayer *layer, int &dtChambers,
                                int &cscChambers, int &rpcChambers,
                                int &totalChambers);
