...
CosmicMuonProducer::CosmicMuonProducer(const ParameterSet &iConfig) {
  ParameterSet tbpar =
      iConfig.getParameter<ParameterSet>("TrajectoryBuilderParameters");

  theSeedCollectionToken = consumes<edm::View<TrajectorySeed>>(
      iConfig.getParameter<std::string>("MuonSeedCollectionLabel"));
  ...
  ParameterSet trackLoaderParameters =
      iConfig.getParameter<ParameterSet>("TrackLoaderParameters");
  ...
  theTrackFinder = new MuonTrackFinder(
      new CosmicMuonTrajectoryBuilder(tbpar, theService, iC),
