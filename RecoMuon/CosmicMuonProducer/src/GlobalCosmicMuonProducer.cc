...
GlobalCosmicMuonProducer::GlobalCosmicMuonProducer(
  ...
  edm::ParameterSet tbpar =
      iConfig.getParameter<edm::ParameterSet>("TrajectoryBuilderParameters");
  theTrackCollectionToken = consumes<reco::TrackCollection>(
      iConfig.getParameter<edm::InputTag>("MuonCollectionLabel"));
  ...
  edm::ParameterSet trackLoaderParameters =
      iConfig.getParameter<edm::ParameterSet>("TrackLoaderParameters");

  // the services
  theService = new MuonServiceProxy(serviceParameters);
  edm::ConsumesCollector iC = consumesCollector();
  theTrackFinder = new MuonTrackFinder(
      new GlobalCosmicMuonTrajectoryBuilder(tbpar, theService, iC),
...
void GlobalCosmicMuonProducer::produce(edm::Event &iEvent,
  ...
  for (unsigned int position = 0; position != cosMuons->size(); ++position) {
    ...
    MuonTrajectoryBuilder::TrackCand cosCand =
        MuonTrajectoryBuilder::TrackCand((Trajectory *)nullptr, cosTrackRef);
