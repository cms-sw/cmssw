...
CosmicMuonTrajectoryBuilder::CosmicMuonTrajectoryBuilder(
    const edm::ParameterSet &par, const MuonServiceProxy *service,
  ...
  InputTag CSCRecSegmentLabel =
      par.getParameter<InputTag>("CSCRecSegmentLabel");

  //  if(enableRPCMeasurement)
  InputTag RPCRecSegmentLabel =
      par.getParameter<InputTag>("RPCRecSegmentLabel");

  //  if(enableGEMMeasurement)
  //  InputTag GEMRecSegmentLabel =
  //  par.getParameter<InputTag>("GEMRecSegmentLabel");

  theLayerMeasurements = new MuonDetLayerMeasurements(
      DTRecSegmentLabel, CSCRecSegmentLabel, RPCRecSegmentLabel,
      edm::InputTag(), edm::InputTag(), iC, enableDTMeasurement,
      enableCSCMeasurement, enableRPCMeasurement, false, false);

  ParameterSet muonUpdatorPSet =
      par.getParameter<ParameterSet>("MuonTrajectoryUpdatorParameters");

  theNavigation =
      nullptr; // new DirectMuonNavigation(theService->detLayerGeometry());
  theUpdator = new MuonTrajectoryUpdator(muonUpdatorPSet, insideOut);

  theBestMeasurementFinder = new MuonBestMeasurementFinder();

  ParameterSet muonBackwardUpdatorPSet =
      par.getParameter<ParameterSet>("BackwardMuonTrajectoryUpdatorParameters");
  ...
  ParameterSet smootherPSet =
      par.getParameter<ParameterSet>("MuonSmootherParameters");

  theNavigationPSet =
      par.getParameter<ParameterSet>("MuonNavigationParameters");
...
CosmicMuonTrajectoryBuilder::~CosmicMuonTrajectoryBuilder() {
  ...
  LogTrace(category_) << "CosmicMuonTrajectoryBuilder Traversing: "
                      << theNSuccess << "/" << theNTraversing;
}
...
void CosmicMuonTrajectoryBuilder::setEvent(const edm::Event &event) {
  ...
  // DetLayer Geometry
  unsigned long long newCacheId_DG =
      theService->eventSetup().get<MuonRecoGeometryRecord>().cacheIdentifier();
  if (newCacheId_DG != theCacheId_DG) {
    LogTrace(category_) << "Muon Reco Geometry changed!";
    theCacheId_DG = newCacheId_DG;
    if (theNavigation)
      delete theNavigation;
    theNavigation = new DirectMuonNavigation(theService->detLayerGeometry(),
                                             theNavigationPSet);
  }
}
...
CosmicMuonTrajectoryBuilder::trajectories(const TrajectorySeed &seed) {
  ...
  const BoundPlane &bp =
      theService->trackingGeometry()->idToDet(did)->surface();
  TrajectoryStateOnSurface lastTsos = trajectoryStateTransform::transientState(
      ptsd1, &bp, &*theService->magneticField());
  ...
  bool beamhaloFlag = ((did.subdetId() == MuonSubdetId::CSC) &&
                       fabs(lastTsos.globalMomentum().eta()) > 4.0);
  ...
  if (did.subdetId() == MuonSubdetId::DT) {
    // DT
    navLayers =
        navigation()->compatibleLayers(*(lastTsos.freeState()), alongMomentum);
  } else if (beamhaloFlag || (theTraversingMuonFlag && theStrict1LegFlag)) {
    // CSC
    navLayers = navigation()->compatibleEndcapLayers(*(lastTsos.freeState()),
                                                     alongMomentum);
  } else {
    navLayers =
        navigation()->compatibleLayers(*(lastTsos.freeState()), alongMomentum);
  ...
  LogTrace(category_) << "found " << navLayers.size()
  ...
  for (vector<const DetLayer *>::const_iterator layer = navLayers.begin();
    ...
    LogTrace(category_)
        << debug.dumpMuonId(
               (*layer)->basicComponents().front()->geographicalId())
        << debug.dumpLayer(*layer);
  }

  detsWithStates = navLayers.front()->compatibleDets(lastTsos, *propagator(),
                                                     *(updator()->estimator()));
  LogTrace(category_) << "Number of compatible dets: " << detsWithStates.size()
                      << endl;
  ...
  if (!detsWithStates.empty()) {
    ...
    if (detsWithStates.front().second.isValid()) {
      ...
      LogTrace(category_) << debug.dumpMuonId(
                                 detsWithStates.front().first->geographicalId())
                          << debug.dumpLayer(navLayers.front());
      lastTsos = detsWithStates.front().second;
      LogTrace(category_) << "Seed after extrapolation: mom "
  ...
  for (vector<const DetLayer *>::const_iterator rnxtlayer = navLayers.begin();
    ...
    LogTrace(category_)
        << debug.dumpMuonId(
               (*rnxtlayer)->basicComponents().front()->geographicalId())
        << debug.dumpLayer(*rnxtlayer);
    LogTrace(category_) << "from lastTsos " << lastTsos.globalMomentum()
                        << " at " << lastTsos.globalPosition();

    measL = findBestMeasurements(*rnxtlayer, lastTsos, propagator(),
                                 (updator()->estimator()));

    if (measL.empty() &&
        (fabs(theService->magneticField()->inTesla(GlobalPoint(0, 0, 0)).z()) <
         0.01) &&
      ...
      measL = findBestMeasurements(
          *rnxtlayer, lastTsos,
          &*theService->propagator("StraightLinePropagator"),
          (updator()->estimator()));
    }
    if (measL.empty())
      continue;

    for (vector<TrajectoryMeasurement>::const_iterator theMeas = measL.begin();
         theMeas != measL.end(); ++theMeas) {
      pair<bool, TrajectoryStateOnSurface> result =
          updator()->update((&*theMeas), *theTraj, propagator());

      if (result.first) {
        LogTrace(category_) << "update ok ";
        incrementChamberCounters((*rnxtlayer), DTChamberUsedBack,
                                 CSCChamberUsedBack, RPCChamberUsedBack,
                                 TotalChamberUsedBack);
        secondLast = lastTsos;
        if ((!theTraj->empty()) && result.second.isValid()) {
          lastTsos = result.second;
          LogTrace(category_)
              << "get new lastTsos here " << lastTsos.globalMomentum() << " at "
  ...
  if (!theTraj->isValid() || TotalChamberUsedBack < 2 ||
      (DTChamberUsedBack + CSCChamberUsedBack) == 0 || !lastTsos.isValid()) {
    delete theTraj;
    return trajL;
  }
  delete theTraj;
  ...
  if (lastPos.basicVector().dot(momDir.basicVector()) > 0) {
    //      LogTrace("CosmicMuonTrajectoryBuilder")<<"Fit direction changed to
    //      insideOut";
  ...
  if (fabs(lastTsos.globalMomentum().eta()) < 1.0) {
    // DT
    navLayers = navigation()->compatibleLayers(*(lastTsos.freeState()),
                                               oppositeToMomentum);
  } else if (beamhaloFlag || (theTraversingMuonFlag && theStrict1LegFlag)) {
    // CSC
  ...
  LogTrace(category_) << "Begin backward refitting, with " << navLayers.size()
                      << " layers" << endl;
  ...
  for (vector<const DetLayer *>::const_iterator rnxtlayer = navLayers.begin();
    ...
    measL = findBestMeasurements(*rnxtlayer, lastTsos, propagator(),
                                 (backwardUpdator()->estimator()));

    if (measL.empty()) {
      MuonTransientTrackingRecHit::MuonRecHitContainer tmpHits =
          theLayerMeasurements->recHits(*rnxtlayer);
    ...
    for (vector<TrajectoryMeasurement>::const_iterator theMeas = measL.begin();
      ...
      if (rnxtlayer != navLayers.begin()) {
        ...
        if (((*lastlayer)->location() == GeomDetEnumerators::endcap) &&
            ...
            (lastTsos.globalPosition().z() *
                 (theMeas)->predictedState().globalPosition().z() <
             0)) {
          theBKUpdator->setFitDirection(insideOut);
        }
      }
      ...
      //          LogTrace("CosmicMuonTrajectoryBuilder")<<"Fit direction
      //          insideOut";
      //       else LogTrace("CosmicMuonTrajectoryBuilder")<<"Fit direction
      //       outsideIn";
      ...
      if (bkresult.first) {
        ...
        if (theTraversingMuonFlag) {
          ...
          allUnusedHits.insert(allUnusedHits.end(), tmpUnusedHits.begin(),
                               tmpUnusedHits.end());
        }
  ...
  if ((!myTraj.isValid()) || (myTraj.empty()) || ((selfDuplicate(myTraj))) ||
      TotalChamberUsedBack < 2 ||
  ...
  GlobalVector dirFromPos =
      myTraj.measurements().back().recHit()->globalPosition() -
  ...
  LogTrace(category_) << "last hit "
                      << myTraj.measurements().back().recHit()->globalPosition()
                      << endl;
  LogTrace(category_)
      << "first hit "
      << myTraj.measurements().front().recHit()->globalPosition() << endl;
  ...
  LogTrace(category_)
      << "first tsos "
      << myTraj.measurements().front().updatedState().globalPosition()
      << " mom "
      << myTraj.measurements().front().updatedState().globalMomentum() << endl;

  PropagationDirection propDir =
      (dirFromPos.basicVector().dot(myTraj.firstMeasurement()
                                        .updatedState()
                                        .globalMomentum()
                                        .basicVector()) > 0)
          ? alongMomentum
...
void CosmicMuonTrajectoryBuilder::build(const TrajectoryStateOnSurface &ts,
  ...
  if (fabs(fts->momentum().basicVector().eta()) < 1.0) {
    ...
    if (fts->position().basicVector().dot(fts->momentum().basicVector()) > 0) {
      ...
      navLayers =
          navigation()->compatibleEndcapLayers((*fts), oppositeToMomentum);
    }
  } else {
  ...
  TrajectoryStateOnSurface lastTsos =
      (traj.lastMeasurement().updatedState().globalPosition().y() <
       traj.firstMeasurement().updatedState().globalPosition().y())
          ...
          : propagatorOpposite()->propagate((*fts),
                                            navLayers.front()->surface());
  ...
  for (vector<const DetLayer *>::const_iterator rnxtlayer = navLayers.begin();
    ...
    for (vector<TrajectoryMeasurement>::const_iterator theMeas = measL.begin();
      ...
      if (bkresult.first) {
        LogTrace(category_)
            << "update ok : " << (theMeas)->recHit()->globalPosition();
...
void CosmicMuonTrajectoryBuilder::buildSecondHalf(Trajectory &traj) {
  ...
  LogTrace(category_) << "last tsos on traj: pos: " << tsos.globalPosition()
...
TrajectoryStateOnSurface CosmicMuonTrajectoryBuilder::intermediateState(
  ...
  TrajectoryStateOnSurface predTsos =
      propagator()->propagate(tsos, *SteppingPlane);
  if (predTsos.isValid())
    LogTrace(category_) << "intermediateState: a intermediate state: pos: "
                        << predTsos.globalPosition()
...
void CosmicMuonTrajectoryBuilder::selectHits(
    MuonTransientTrackingRecHit::MuonRecHitContainer &hits) const {
    ...
    for (MuonRecHitContainer::const_iterator ihit2 = ihit + 1;
      ...
      if ((*ihit)->geographicalId() == (*ihit2)->geographicalId()) {
        if ((*ihit)->dimension() > (*ihit2)->dimension()) {
          ...
          if ((*ihit)->transientHits().size() >
              (*ihit2)->transientHits().size()) {
            keep[j] = false;
          } else if ((*ihit)->transientHits().size() <
                     (*ihit2)->transientHits().size()) {
            keep[i] = false;
          } else if ((*ihit)->degreesOfFreedom() != 0 &&
                     (*ihit2)->degreesOfFreedom() != 0) {
            if (((*ihit)->chi2() / (*ihit)->degreesOfFreedom()) >
                ((*ihit2)->chi2() / (*ihit)->degreesOfFreedom()))
              keep[i] = false;
            else
              keep[j] = false;
          }
...
bool CosmicMuonTrajectoryBuilder::selfDuplicate(const Trajectory &traj) const {
  ...
  bool result = false;
  for (ConstRecHitContainer::const_iterator ir = hits.begin(); ir != hits.end();
       ir++) {
    if (!(*ir)->isValid())
      continue;
    for (ConstRecHitContainer::const_iterator ir2 = ir + 1; ir2 != hits.end();
         ir2++) {
      if (!(*ir2)->isValid())
        continue;
      if ((*ir) == (*ir2))
...
void CosmicMuonTrajectoryBuilder::reverseTrajectory(Trajectory &traj) const {
  ...
  PropagationDirection newDir =
      (traj.firstMeasurement().recHit()->globalPosition().y() <
...
void CosmicMuonTrajectoryBuilder::flipTrajectory(Trajectory &traj) const {
  ...
  vector<Trajectory> refittedback =
      theSmoother->fit(traj.seed(), hits, lastTSOS);
  if (refittedback.empty()) {
    LogTrace(category_) << "flipTrajectory fail. " << endl;
    return;
  }
  LogTrace(category_) << "flipTrajectory: first "
                      << refittedback.front().firstMeasurement().updatedState()
                      << "\nflipTrajectory: last "
                      << refittedback.front().lastMeasurement().updatedState();
...
void CosmicMuonTrajectoryBuilder::reverseTrajectoryPropagationDirection(
  ...
  if (traj.direction() == anyDirection)
    return;
  PropagationDirection newDir =
      (traj.direction() == alongMomentum) ? oppositeToMomentum : alongMomentum;
  Trajectory newTraj(traj.seed(), newDir);
...
void CosmicMuonTrajectoryBuilder::estimateDirection(Trajectory &traj) const {
  ...
  if (fabs(firstTSOS.globalMomentum().eta()) >
      fabs(lastTSOS.globalMomentum().eta())) {

    vector<Trajectory> refitted =
        theSmoother->trajectories(traj.seed(), hits, firstTSOS);
    if (!refitted.empty())
      traj = refitted.front();
    ...
    utilities()->reverseDirection(lastTSOS, &*theService->magneticField());
    vector<Trajectory> refittedback =
        theSmoother->trajectories(traj.seed(), hits, lastTSOS);
    if (!refittedback.empty())
      traj = refittedback.front();
  }
...
void CosmicMuonTrajectoryBuilder::getDirectionByTime(Trajectory &traj) const {
  ...
  for (TransientTrackingRecHit::ConstRecHitContainer::const_iterator ir =
           hits.begin();
       ir != hits.end(); ir++) {
    if (!(*ir)->isValid()) {
    ...
    LogTrace(category_) << "pos" << pos << "radius " << pos.perp() << "  dim "
                        << (*ir)->dimension() << "  det "
                        << (*ir)->det()->geographicalId().det() << "  sub det "
                        << (*ir)->det()->subDetector() << endl;

    if ((*ir)->det()->geographicalId().det() == 2 &&
        (*ir)->det()->subDetector() == 6) {
      //      const CSCRecHit2D* iCSC = dynamic_cast<const CSCRecHit2D*>(&**ir);
      //      if (iCSC) LogTrace(category_)<<"csc from cast tpeak
      //      "<<iCSC->tpeak();
      CSCRecHit2DCollection::range thisrange =
          cschits_->get(CSCDetId((*ir)->geographicalId()));
      for (CSCRecHit2DCollection::const_iterator rechit = thisrange.first;
           rechit != thisrange.second; ++rechit) {
        if ((*rechit).isValid())
          LogTrace(category_)
    ...
    if ((*ir)->det()->geographicalId().det() == 2 &&
        (*ir)->det()->subDetector() == 7) {
      //      const DTRecHit1D* iDT = dynamic_cast<const DTRecHit1D*>(&**ir);
      //      if (iDT) LogTrace(category_)<<"dt digitime "<<iDT->digiTime();
      DTRecHitCollection::range thisrange =
          dthits_->get(DTLayerId((*ir)->geographicalId()));
      for (DTRecHitCollection::const_iterator rechit = thisrange.first;
           rechit != thisrange.second; ++rechit) {
        if ((*rechit).isValid())
          LogTrace(category_)
...
CosmicMuonTrajectoryBuilder::findBestMeasurements(
    const DetLayer *layer, const TrajectoryStateOnSurface &tsos,
    const Propagator *propagator, const MeasurementEstimator *estimator) {
  ...
  if (layer->hasGroups()) {
    std::vector<TrajectoryMeasurementGroup> measurementGroups =
        theLayerMeasurements->groupedMeasurements(layer, tsos, *propagator,
                                                  *estimator);

    for (std::vector<TrajectoryMeasurementGroup>::const_iterator tmGroupItr =
             measurementGroups.begin();
      ...
      const TrajectoryMeasurement *bestMeasurement =
          theBestMeasurementFinder->findBestMeasurement(measurements,
                                                        propagator);
    ...
    measurements = theLayerMeasurements->measurements(layer, tsos, *propagator,
                                                      *estimator);
    const TrajectoryMeasurement *bestMeasurement =
...
void CosmicMuonTrajectoryBuilder::incrementChamberCounters(
    const DetLayer *layer, int &dtChambers, int &cscChambers, int &rpcChambers,
    int &totalChambers) {
...
double CosmicMuonTrajectoryBuilder::t0(const DTRecSegment4D *dtseg) const {
  ...
  if (phiHits > 5) {
    ...
    if (dtseg->phiSegment()->ist0Valid()) {
      LogTrace(category_) << " Phi t0: " << dtseg->phiSegment()->t0()
                          << " hits: " << phiHits;
    } else {
      LogTrace(category_) << " Phi t0 is invalid: " << dtseg->phiSegment()->t0()
                          << " hits: " << phiHits;
    }
  }
...
//
//
PropagationDirection CosmicMuonTrajectoryBuilder::checkDirectionByT0(
    const DTRecSegment4D *dtseg1, const DTRecSegment4D *dtseg2) const {
