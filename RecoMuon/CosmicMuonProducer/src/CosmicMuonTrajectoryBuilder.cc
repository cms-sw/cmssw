...
MuonTrajectoryBuilder::TrajectoryContainer CosmicMuonTrajectoryBuilder::trajectories(const TrajectorySeed& seed) {
  ...
  for (auto layer = navLayers.begin(); layer != navLayers.end(); layer++) {
  ...
  for (auto rnxtlayer = navLayers.begin(); rnxtlayer != navLayers.end(); ++rnxtlayer) {
    ...
    for (auto theMeas = measL.begin(); theMeas != measL.end(); ++theMeas) {
  ...
  for (auto rnxtlayer = navLayers.begin(); rnxtlayer != navLayers.end(); ++rnxtlayer) {
    ...
    if (measL.empty()) {
      ...
      for (auto ihit = tmpHits.begin(); ihit != tmpHits.end(); ++ihit) {
...
void CosmicMuonTrajectoryBuilder::selectHits(MuonTransientTrackingRecHit::MuonRecHitContainer& hits) const {
  ...
  for (auto ihit = hits.begin(); ihit != hits.end(); ++ihit) {
    ...
    j = i + 1;
    for (auto ihit2 = ihit + 1; ihit2 != hits.end(); ++ihit2) {
      if (!keep[j]) {
        j++;
        continue;
      }
...
bool CosmicMuonTrajectoryBuilder::selfDuplicate(const Trajectory& traj) const {
  ...
  for (auto ir = hits.begin(); ir != hits.end(); ir++) {
    if (!(*ir)->isValid())
      continue;
    for (auto ir2 = ir + 1; ir2 != hits.end(); ir2++) {
      if (!(*ir2)->isValid())
        continue;
      if ((*ir) == (*ir2))
        result = true;
...
void CosmicMuonTrajectoryBuilder::reverseTrajectoryPropagationDirection(Trajectory& traj) const {
  ...
  for (auto itm = meas.begin(); itm != meas.end(); ++itm) {
    newTraj.push(*itm);
  }
