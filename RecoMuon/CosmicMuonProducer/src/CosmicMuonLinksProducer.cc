...
CosmicMuonLinksProducer::CosmicMuonLinksProducer(const ParameterSet &iConfig) {
  ...
  std::vector<edm::ParameterSet> theMapPSets =
      iConfig.getParameter<std::vector<edm::ParameterSet>>("Maps");
  for (std::vector<edm::ParameterSet>::const_iterator iMPS =
           theMapPSets.begin();
    ...
    edm::EDGetTokenT<reco::TrackCollection> subTrackTag =
        consumes<reco::TrackCollection>(sTag);
    edm::EDGetTokenT<reco::TrackCollection> parentTrackTag =
        consumes<reco::TrackCollection>(pTag);
...
void CosmicMuonLinksProducer::produce(Event &iEvent, const EventSetup &iSetup) {
  ...
  unsigned int counter =
      0; /// DAMN I cannot read the label of the TOKEN so I need to do this
         /// stupid thing to create the labels of the products!
  for (std::vector<std::pair<edm::EDGetTokenT<reco::TrackCollection>,
                             edm::EDGetTokenT<reco::TrackCollection>>>::
           const_iterator iLink = theTrackLinks.begin();
    ...
    LogDebug(category_) << "making map between " << labels_first.module
                        << " and " << labels_second.module;
#endif
    std::string mapname = theTrackLinkNames[counter].first + "To" +
                          theTrackLinkNames[counter].second;
    ...
    LogTrace(category_) << "Mapped: " << theTrackLinkNames[counter].first << " "
                        << subTracks->size() << " and "
                        << theTrackLinkNames[counter].second << " "
                        << parentTracks->size() << ", results: " << ttmap.size()
                        << endl;
...
reco::TrackToTrackMap CosmicMuonLinksProducer::mapTracks(
    const Handle<reco::TrackCollection> &subTracks,
    const Handle<reco::TrackCollection> &parentTracks) const {
  reco::TrackToTrackMap map(subTracks, parentTracks);
  for (unsigned int position1 = 0; position1 != subTracks->size();
       ++position1) {
    TrackRef track1(subTracks, position1);
    for (unsigned int position2 = 0; position2 != parentTracks->size();
         ++position2) {
      TrackRef track2(parentTracks, position2);
      int shared = sharedHits(*track1, *track2);
      LogTrace(category_) << "sharedHits " << shared << " track1 "
                          << track1->found() << " track2 " << track2->found()
                          << endl;
...
int CosmicMuonLinksProducer::sharedHits(const reco::Track &track1,
  ...
  for (trackingRecHit_iterator hit1 = track1.recHitsBegin();
       hit1 != track1.recHitsEnd(); ++hit1) {
    if (!(*hit1)->isValid())
      continue;
    DetId id1 = (*hit1)->geographicalId();
    if (id1.det() != DetId::Muon)
      continue; // ONLY MUON
    LogTrace(category_) << "first ID " << id1.rawId() << " "
                        << (*hit1)->localPosition() << endl;
    GlobalPoint pos1 =
        theService->trackingGeometry()->idToDet(id1)->surface().toGlobal(
            (*hit1)->localPosition());

    for (trackingRecHit_iterator hit2 = track2.recHitsBegin();
         hit2 != track2.recHitsEnd(); ++hit2) {
      ...
      //          LogTrace(category_)<<"second ID "<<id2.rawId()<<
      //          (*hit2)->localPosition()<<endl;

      if (id2.rawId() != id1.rawId())
        continue;

      GlobalPoint pos2 =
          theService->trackingGeometry()->idToDet(id2)->surface().toGlobal(
              (*hit2)->localPosition());
