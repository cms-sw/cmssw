...
void CosmicMuonUtilities::reverseDirection(TrajectoryStateOnSurface &tsos,
                                           const MagneticField *mgfield) const {

  GlobalTrajectoryParameters gtp(tsos.globalPosition(), -tsos.globalMomentum(),
                                 -tsos.charge(), mgfield);
...
//
//
string CosmicMuonUtilities::print(
    const MuonTransientTrackingRecHit::ConstMuonRecHitContainer &hits) const {

  stringstream output;

  for (ConstMuonRecHitContainer::const_iterator ir = hits.begin();
       ir != hits.end(); ir++) {
    if (!(*ir)->isValid()) {
    ...
    output << "pos" << pos << "radius " << pos.perp() << "  dim "
           << (*ir)->dimension() << "  det "
...
//
string CosmicMuonUtilities::print(
    const MuonTransientTrackingRecHit::MuonRecHitContainer &hits) const {

  stringstream output;

  for (MuonRecHitContainer::const_iterator ir = hits.begin(); ir != hits.end();
       ir++) {
    if (!(*ir)->isValid()) {
      output << "invalid RecHit" << endl;
      continue;
...
bool CosmicMuonUtilities::isTraversing(const Trajectory &t) const {
  ...
  return ((middle.perp() < frontPos.perp()) &&
          (middle.perp() < backPos.perp()));
}
...
CosmicMuonUtilities::stepPropagate(const TrajectoryStateOnSurface &tsos,
  ...
  LogTrace(metname) << "stepPropagate: propagate from: " << start << " to "
                    << dest;
  ...
  for (int istep = 0; istep < steps - 1; istep++) {
    ...
    TrajectoryStateOnSurface predTsos =
        prop.propagate(currTsos, *SteppingPlane);
    if (predTsos.isValid()) {
      currTsos = predTsos;
      LogTrace(metname) << "stepPropagate: middle state "
                        << currTsos.globalPosition() << endl;
    }
  }
