...
GlobalCosmicMuonTrajectoryBuilder::GlobalCosmicMuonTrajectoryBuilder(
  ...
  ParameterSet smootherPSet =
      par.getParameter<ParameterSet>("SmootherParameters");
  theSmoother = new CosmicMuonSmoother(smootherPSet, theService);

  ParameterSet trackMatcherPSet =
      par.getParameter<ParameterSet>("GlobalMuonTrackMatcher");
  theTrackMatcher = new GlobalMuonTrackMatcher(trackMatcherPSet, theService);

  theTkTrackToken = iC.consumes<reco::TrackCollection>(
      par.getParameter<InputTag>("TkTrackCollectionLabel"));

  theTrackerRecHitBuilderName =
      par.getParameter<string>("TrackerRecHitBuilder");
...
void GlobalCosmicMuonTrajectoryBuilder::setEvent(const edm::Event &event) {
  ...
  theService->eventSetup().get<TransientRecHitRecord>().get(
      theTrackerRecHitBuilderName, theTrackerRecHitBuilder);
  theService->eventSetup().get<TransientRecHitRecord>().get(
      theMuonRecHitBuilderName, theMuonRecHitBuilder);
}
...
GlobalCosmicMuonTrajectoryBuilder::trajectories(const TrackCand &muCand) {
  ...
  LogTrace(category_) << "Found " << theTrackerTracks->size()
                      << " tracker Tracks";
  ...
  LogTrace(category_) << "TrackMatcher found " << matched.size()
  ...
  TrajectoryStateOnSurface muonState1 =
      trajectoryStateTransform::innerStateOnSurface(
          *muTrack, *theService->trackingGeometry(),
          &*theService->magneticField());
  TrajectoryStateOnSurface tkState1 =
      trajectoryStateTransform::innerStateOnSurface(
          *tkTrack, *theService->trackingGeometry(),
          &*theService->magneticField());

  TrajectoryStateOnSurface muonState2 =
      trajectoryStateTransform::outerStateOnSurface(
          *muTrack, *theService->trackingGeometry(),
          &*theService->magneticField());
  TrajectoryStateOnSurface tkState2 =
      trajectoryStateTransform::outerStateOnSurface(
          *tkTrack, *theService->trackingGeometry(),
          &*theService->magneticField());

  TrajectoryStateOnSurface firstState1 =
      (muonState1.globalPosition().y() > tkState1.globalPosition().y())
          ? muonState1
          : tkState1;
  TrajectoryStateOnSurface firstState2 =
      (muonState2.globalPosition().y() > tkState2.globalPosition().y())
          ? muonState2
          : tkState2;

  TrajectoryStateOnSurface firstState =
      (firstState1.globalPosition().y() > firstState2.globalPosition().y())
          ? firstState1
          : firstState2;
  ...
  if (!hits.empty()) {
    ...
    if ((front.perp() < 130 && fabs(front.z()) < 300) ||
        (back.perp() < 130 && fabs(back.z()) < 300)) {
      if (hits.front()->globalPosition().perp() >
      ...
      tkState2 = trajectoryStateTransform::outerStateOnSurface(
          *tkTrack, *theService->trackingGeometry(),
          &*theService->magneticField());
      firstState =
          (tkState1.globalPosition().perp() < tkState2.globalPosition().perp())
              ? tkState1
              : tkState2;
    }
  }
  if (!firstState.isValid())
    return result;
  LogTrace(category_) << "firstTSOS pos: " << firstState.globalPosition()
                      << "mom: " << firstState.globalMomentum();

  // begin refitting

  TrajectorySeed seed;
  vector<Trajectory> refitted =
      theSmoother->trajectories(seed, hits, firstState);
  ...
  LogTrace(category_) << "Orignally there are " << tkTrack->found()
                      << " tk rhs and " << muTrack->found() << " mu rhs.";
...
void GlobalCosmicMuonTrajectoryBuilder::sortHits(ConstRecHitContainer &hits,
  ...
  for (ConstRecHitContainer::iterator ihit = muonHits.begin();
    ...
    GlobalPoint middle((ipos.x() + nextpos.x()) / 2,
                       (ipos.y() + nextpos.y()) / 2,
                       (ipos.z() + nextpos.z()) / 2);
    LogTrace(category_) << "ipos " << ipos << "nextpos" << nextpos << " middle "
                        << middle << endl;
  ...
  if (insertInMiddle) { // if tk hits should be sandwich
    ...
    if ((frontTkPos - jointpointpos).mag() >
        (backTkPos - jointpointpos)
            .mag()) { // check if tk hits order same direction
      reverse(tkHits.begin(), tkHits.end());
    }
    ...
    if (frontTkPos.y() < frontMuPos.y()) { // insert at the end
      LogTrace(category_) << "insert at the end " << frontTkPos << frontMuPos
                          << endl;

      hits = muonHits;
      hits.insert(hits.end(), tkHits.begin(), tkHits.end());
    } else { // insert at the beginning
      LogTrace(category_) << "insert at the beginning " << frontTkPos
                          << frontMuPos << endl;
      hits = tkHits;
...
GlobalCosmicMuonTrajectoryBuilder::getTransientRecHits(
  ...
  auto hitCloner = static_cast<TkTransientTrackingRecHitBuilder const *>(
                       theTrackerRecHitBuilder.product())
                       ->cloner();
  TrajectoryStateOnSurface currTsos =
      trajectoryStateTransform::innerStateOnSurface(
          track, *theService->trackingGeometry(),
          &*theService->magneticField());
  for (trackingRecHit_iterator hit = track.recHitsBegin();
       hit != track.recHitsEnd(); ++hit) {
    if ((*hit)->isValid()) {
...
std::vector<GlobalCosmicMuonTrajectoryBuilder::TrackCand>
GlobalCosmicMuonTrajectoryBuilder::match(
  ...
  TrajectoryStateOnSurface innerTsos =
      trajectoryStateTransform::innerStateOnSurface(
          *(mu.second), *theService->trackingGeometry(),
          &*theService->magneticField());
  ...
  for (reco::TrackCollection::size_type i = 0; i < theTrackerTracks->size();
       ++i) {
    ...
    LogTrace(category_) << "chisq is "
                        << theTrackMatcher->match(mu, tkCand, 0, 0);
    LogTrace(category_) << "d is " << theTrackMatcher->match(mu, tkCand, 1, 0);
    LogTrace(category_) << "r_pos is "
                        << theTrackMatcher->match(mu, tkCand, 2, 0);
  }
  ...
  if ((innerPos.basicVector().dot(innerTsos.globalMomentum().basicVector()) *
           outerPos.basicVector().dot(
               outerTsos.globalMomentum().basicVector()) >
       0)) {

    GlobalPoint geoInnerPos =
        (innerPos.mag() < outerPos.mag()) ? innerPos : outerPos;
    ...
    for (vector<TrackCand>::const_iterator itkCand = tkTrackCands.begin();
         itkCand != tkTrackCands.end(); ++itkCand) {

      reco::TrackRef tkTrack = itkCand->second;

      GlobalPoint tkInnerPos(tkTrack->innerPosition().x(),
                             tkTrack->innerPosition().y(),
                             tkTrack->innerPosition().z());
      GlobalPoint tkOuterPos(tkTrack->outerPosition().x(),
                             tkTrack->outerPosition().y(),
                             tkTrack->outerPosition().z());
      LogTrace(category_) << "tkTrack " << tkInnerPos << " " << tkOuterPos
                          << endl;

      float closetDistance11 = (geoInnerPos - tkInnerPos).mag();
      float closetDistance12 = (geoInnerPos - tkOuterPos).mag();
      float closetDistance1 = (closetDistance11 < closetDistance12)
                                  ? closetDistance11
                                  : closetDistance12;
      ...
      if (true || !isTraversing(*tkTrack)) {
        bool keep = true;
        for (vector<TrackCand>::const_iterator itkCand2 = tkTrackCands.begin();
             itkCand2 != tkTrackCands.end(); ++itkCand2) {
          if (itkCand2 == itkCand)
            continue;
          reco::TrackRef tkTrack2 = itkCand2->second;

          GlobalPoint tkInnerPos2(tkTrack2->innerPosition().x(),
                                  tkTrack2->innerPosition().y(),
                                  tkTrack2->innerPosition().z());
          GlobalPoint tkOuterPos2(tkTrack2->outerPosition().x(),
                                  tkTrack2->outerPosition().y(),
                                  tkTrack2->outerPosition().z());
          LogTrace(category_)
              << "tkTrack2 " << tkInnerPos2 << " " << tkOuterPos2 << endl;

          float farthestDistance21 = (geoInnerPos - tkInnerPos2).mag();
          float farthestDistance22 = (geoInnerPos - tkOuterPos2).mag();
          float farthestDistance2 = (farthestDistance21 > farthestDistance22)
                                        ? farthestDistance21
                                        : farthestDistance22;
          LogTrace(category_)
              << "farthestDistance2 " << farthestDistance2 << endl;
  ...
  LogTrace(category_) << "TrackMatcher found " << matched_trackerTracks.size()
                      << "tracker tracks matched";
  ...
  if (matched_trackerTracks.size() < 2) {
    ...
    for (vector<TrackCand>::const_iterator iter = matched_trackerTracks.begin();
         iter != matched_trackerTracks.end(); iter++) {
...
bool GlobalCosmicMuonTrajectoryBuilder::isTraversing(
  ...
  for (trackingRecHit_iterator hit = track.recHitsEnd() - 1;
       hit != track.recHitsBegin() - 1; --hit) {
    if ((*hit)->isValid()) {
      lastValid = hit;
      break;
    }
  ...
  GlobalPoint posFirst = theService->trackingGeometry()
                             ->idToDet((*firstValid)->geographicalId())
                             ->position();

  GlobalPoint posLast = theService->trackingGeometry()
                            ->idToDet((*lastValid)->geographicalId())
                            ->position();

  GlobalPoint middle((posFirst.x() + posLast.x()) / 2,
                     (posFirst.y() + posLast.y()) / 2,
                     (posFirst.z() + posLast.z()) / 2);
