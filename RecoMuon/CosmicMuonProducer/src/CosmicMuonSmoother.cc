...
// constructor
//
CosmicMuonSmoother::CosmicMuonSmoother(const ParameterSet &par,
...
// fit and smooth trajectory
//
std::vector<Trajectory> CosmicMuonSmoother::trajectories(
    const TrajectorySeed &seed, const ConstRecHitContainer &hits,
...
vector<Trajectory> CosmicMuonSmoother::fit(const Trajectory &t) const {
  ...
  LogTrace(category_) << "hit front" << hits.front()->globalPosition()
                      << " hit back" << hits.back()->globalPosition();

  sortHitsAlongMom(hits, firstTsos);

  LogTrace(category_) << "after sorting hit front"
                      << hits.front()->globalPosition() << " hit back"
...
//
vector<Trajectory>
CosmicMuonSmoother::fit(const TrajectorySeed &seed,
  ...
  if (hits.front()->isValid()) {
    ...
    // FIXME  FIXME  CLONE !!!
    //  TrackingRecHit::RecHitPointer preciseHit =
    //  hits.front()->clone(predTsos);
    auto preciseHit = hits.front();

    LogTrace(category_) << "first hit is at det "
                        << hits.front()->det()->surface().position();
  ...
  for (ConstRecHitContainer::const_iterator ihit = hits.begin() + 1;
    ...
    if (currTsos.isValid() && currTsos.globalMomentum().mag2() > 1e-18f) {
      ...
      // logically, there's no way out: can't expect a valid result out of an
      // invalid state
      LogTrace(category_) << "Input state is not valid. This loop over hits is "
                             "doomed: breaking out";
      break;
    }

    predTsos =
        propagatorAlong()->propagate(currTsos, (**ihit).det()->surface());
    LogTrace(category_) << "predicted state propagate directly "
                        << predTsos.isValid();

    if (!predTsos.isValid()) {
      LogTrace(category_) << "step-propagating from "
                          << currTsos.globalPosition()
                          << " to position: " << (*ihit)->globalPosition();
      predTsos =
          theUtilities->stepPropagate(currTsos, (*ihit), *propagatorAlong());
    }
    if (!predTsos.isValid() &&
        (fabs(theService->magneticField()->inTesla(GlobalPoint(0, 0, 0)).z()) <
         0.01) &&
        (theService->propagator("StraightLinePropagator").isValid())) {
      LogTrace(category_) << "straight-line propagating from "
                          << currTsos.globalPosition()
                          << " to position: " << (*ihit)->globalPosition();
      predTsos = theService->propagator("StraightLinePropagator")
                     ->propagate(currTsos, (**ihit).det()->surface());
    }
    if (predTsos.isValid()) {
    ...
    if (!predTsos.isValid()) {
      ...
      // FIXME  FIXME  CLONE !!!
      // update  (FIXME!)
      // TransientTrackingRecHit::RecHitPointer preciseHit =
      // (**ihit).clone(predTsos);
  ...
  for (std::vector<TrajectoryMeasurement>::const_iterator itm = mytms.begin();
       itm != mytms.end(); ++itm) {
    LogTrace(category_) << "updated pos "
                        << itm->updatedState().globalPosition() << "mom "
...
//
vector<Trajectory>
CosmicMuonSmoother::smooth(const vector<Trajectory> &tc) const {

  vector<Trajectory> result;

  for (vector<Trajectory>::const_iterator it = tc.begin(); it != tc.end();
       ++it) {
...
vector<Trajectory> CosmicMuonSmoother::smooth(const Trajectory &t) const {
  ...
  if (avtm.back().recHit()->isValid()) {
    ...
    myTraj.push(TrajectoryMeasurement(avtm.back().forwardPredictedState(),
                                      predTsos, avtm.back().updatedState(),
                                      avtm.back().recHit(),
                                      avtm.back().estimate() //,
                                      /*avtm.back().layer()*/),
                avtm.back().estimate());
  ...
  for (vector<TrajectoryMeasurement>::reverse_iterator itm = avtm.rbegin() + 1;
    ...
    predTsos = propagatorOpposite()->propagate(
        currTsos, (*itm).recHit()->det()->surface());

    if (!predTsos.isValid()) {
      LogTrace(category_) << "step-propagating from "
                          << currTsos.globalPosition() << " to position: "
                          << (*itm).recHit()->globalPosition();
      predTsos = theUtilities->stepPropagate(currTsos, (*itm).recHit(),
                                             *propagatorOpposite());
    }
    if (predTsos.isValid()) {
      LogTrace(category_) << "predicted pos " << predTsos.globalPosition()
    ...
    if (!predTsos.isValid()) {
      LogTrace(category_)
          << "Error: predTsos is still invalid backward smooth.";
      return vector<Trajectory>();
      //  continue;
      ...
      TrajectoryStateOnSurface smooTsos =
      ...
      myTraj.push(
          TrajectoryMeasurement(
              (*itm).forwardPredictedState(), predTsos, smooTsos,
              (*itm).recHit(),
  ...
  predTsos = propagatorOpposite()->propagate(
      currTsos, avtm.front().recHit()->det()->surface());

  if (!predTsos.isValid()) {
    LogTrace(category_)
        << "Error: last predict TSOS failed, use original one. ";
    //    return vector<Trajectory>();
    ...
    if (avtm.front().recHit()->isValid()) {
      ...
      if (currTsos.isValid())
        myTraj.push(
            TrajectoryMeasurement(
                avtm.front().forwardPredictedState(), predTsos, currTsos,
                avtm.front().recHit(),
                theEstimator->estimate(predTsos, (*avtm.front().recHit()))
                    .second //,
                /*avtm.front().layer()*/),
            avtm.front().estimate());
    }
...
CosmicMuonSmoother::initialState(const Trajectory &t) const {
  if (t.empty())
    return TrajectoryStateOnSurface();

  if (!t.firstMeasurement().updatedState().isValid() ||
...
void CosmicMuonSmoother::sortHitsAlongMom(
    ConstRecHitContainer &hits, const TrajectoryStateOnSurface &tsos) const {
