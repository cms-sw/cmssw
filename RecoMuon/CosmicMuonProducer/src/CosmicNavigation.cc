...
CosmicNavigation::CosmicNavigation(const MuonDetLayerGeometry * muonLayout) : theMuonDetLayerGeometry(muonLayout) {
   epsilon_ = 10.;
...
//  edm::LogInfo("CosmicNavigation")<<"barrel "<<theBarrelLayers.size();
//  edm::LogInfo("CosmicNavigation")<<"backward "<<theBackwardLayers.size();
//  edm::LogInfo("CosmicNavigation")<<"forward "<<theForwardLayers.size();

}
/* Operations */ 
vector<const DetLayer*> 
CosmicNavigation::compatibleLayers( const FreeTrajectoryState& fts, 
...
void CosmicNavigation::addBarrelLayer(BarrelDetLayer* mbp) {
...
//
void CosmicNavigation::addEndcapLayer(ForwardDetLayer* mep) {
...
bool CosmicNavigation::checkCompatible(const FreeTrajectoryState& fts,const BarrelDetLayer* dl) const {
  ...
  const BoundSurface* sur = &dl->surface();
...
bool CosmicNavigation::checkCompatible(const FreeTrajectoryState& fts,const ForwardDetLayer* dl) const {
  ...
  const BoundSurface* sur = &dl->surface();
  const BoundDisk* bd = dynamic_cast<const BoundDisk*>(sur);
...
bool CosmicNavigation::outward(const FreeTrajectoryState& fts) const {
