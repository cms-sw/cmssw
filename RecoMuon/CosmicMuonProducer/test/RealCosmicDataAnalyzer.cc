...
void RealCosmicDataAnalyzer::analyze(const edm::Event &iEvent,
  ...
  for (reco::TrackCollection::const_iterator muon = muons->begin();
       muon != muons->end(); ++muon) {
...
void RealCosmicDataAnalyzer::beginJob() {
  ...
  h_pt_rec =
      fs->make<TH1F>("h_pt_rec", "distribution of P_{T}", 100, 0.0, 100.0);
  h_inOutDis = fs->make<TH1F>(
      "h_inOutDis", "distribution of inner outer distance", 200, 0.0, 1500.0);
