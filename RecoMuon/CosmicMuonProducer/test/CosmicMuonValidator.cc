...
private:
  ...
  PSimHitContainer matchedHit(const GlobalPoint &,
                              const PSimHitContainer &) const;

  TrajectoryStateOnSurface updatedState(const TrajectoryStateOnSurface &,
                                        const PSimHit &) const;
...
CosmicMuonValidator::CosmicMuonValidator(const edm::ParameterSet &iConfig) {
  ...
  edm::ParameterSet serviceParameters =
      iConfig.getParameter<ParameterSet>("ServiceParameters");
  // the services
...
void CosmicMuonValidator::analyze(const edm::Event &iEvent,
  ...
  for (SimTrackContainer::const_iterator simTrack = simTracks->begin();
    ...
    if (abs((*simTrack).type()) == 13) {
      cout << "MC Muon: mom (" << simTrack->momentum().x() << ","
           << simTrack->momentum().y() << "," << simTrack->momentum().z() << ")"
           << endl;
  ...
  cout << "resolution " << (qreco / ptreco - simC / ptsim) * ptsim / simC
       << endl;
  ...
  allSimHits.insert(allSimHits.end(), (cscSimHits)->begin(),
                    (cscSimHits)->end());
  allSimHits.insert(allSimHits.end(), (rpcSimHits)->begin(),
                    (rpcSimHits)->end());
  ...
  if (!msimh.empty()) {
    ...
    GlobalVector simmom =
        theService->trackingGeometry()->idToDet(idSim)->surface().toGlobal(
            msimh.front().momentumAtEntry());

    TrajectoryStateOnSurface innerTSOS =
        trajectoryStateTransform::innerStateOnSurface(
            muon, *theService->trackingGeometry(),
    ...
    h_phi_rec_sim->Fill(
        ((Geom::Phi<float>(simmom.phi())) - Geom::Phi<float>(im.phi())) * 180 /
        acos(-1.));

    h_Pres_inv_sim->Fill((1 / im.perp() - 1 / ((double)simmom.perp())) /
                         (1 / ((double)simmom.perp())));

    h_pt_sim->Fill(((double)simmom.perp()));

    h_theta_rec_sim->Fill((((double)simmom.theta()) - im.theta()) * 180 /
                          acos(-1.));
  }
}
...
void CosmicMuonValidator::beginJob() {
  ...
  hnhit =
      fs->make<TH1F>("hnhit", "Number of Hits in Track by Cos", 60, 0.0, 60.0);

  h_innerPosXY = fs->make<TH2F>("h_innerPosXY", "inner x-y", 100, -700.0, 700.0,
                                100, -700.0, 700.0);
  h_innerPosEP = fs->make<TH2F>("h_innerPosEP", "inner #eta-#phi", 100, -2.4,
                                2.4, 100, -3.3, 3.3);

  h_outerPosXY = fs->make<TH2F>("h_outerPosXY", "outer x-y", 100, -700.0, 700.0,
                                100, -700.0, 700.0);
  h_outerPosEP = fs->make<TH2F>("h_outerPosEP", "outer #eta-#phi", 100, -2.4,
                                2.4, 100, -3.3, 3.3);

  h_pt_rec_sim =
      fs->make<TH1F>("h_pt_res_sim", "diff of P_{T} at SimHit", 50, -2.0, 2.0);
  h_phi_rec_sim =
      fs->make<TH1F>("h_phi_res_sim", "diff of #phi at SimHit", 50, -2.0, 2.0);
  h_theta_rec_sim = fs->make<TH1F>("h_theta_res_sim",
                                   "diff of #theta at SimHit", 50, -2.0, 2.0);
  h_Pres_inv_sim = fs->make<TH1F>(
      "h_Pres_inv_sim", "resolution of P_{T} at SimHit", 70, -1.0, 1.0);

  h_pt_sim = fs->make<TH1F>("h_pt_sim", "distribution of P_{T} at SimHit", 100,
                            0.0, 100.0);
  h_pt_rec = fs->make<TH1F>("h_pt_rec", "distribution of P_{T} at SimHit", 100,
                            0.0, 100.0);
...
void CosmicMuonValidator::endJob() {
  ...
  std::cout << successR << " events are successfully reconstructed. "
            << std::endl;
  std::cout << nNoSignal << " events do not have good enough signals. "
            << std::endl;
  std::cout << "Reconstruction efficiency is approximately " << eff << "%. "
            << std::endl;
  std::cout << "++++++++++++++++++++++++++++++++++++++++" << std::endl;
  ...
  h_res->SetXTitle("(q^{reco}/P^{reco}_{T}-q^{simTrack}/P^{simTrack}_{T})/"
                   "(q^{simTrack}/P^{simTrack}_{T})");
  ...
  h_Pres_inv_sim->SetXTitle("(q^{reco}/P^{reco}_{T}-q^{simHit}/P^{simHit}_{T})/"
                            "(q^{simHit}/P^{simHit}_{T})");
  ...
  if (theDrawOption == 0) {
    ...
    TCanvas *c3 =
        new TCanvas("innerEtaPhi", "Inner #eta #phi", 10, 10, 800, 600);
    c3->SetFillColor(0);
    c3->SetGrid(1);
    ...
    TCanvas *c3a =
        new TCanvas("outerEtaPhi", "Outer #eta #phi", 10, 10, 800, 600);
    c3a->SetFillColor(0);
    c3a->SetGrid(1);
    ...
    TCanvas *cRes1 = new TCanvas(
        "TrackPtRes", "Resolution of P_{T} wrt SimTrack", 10, 10, 800, 600);
    ...
    TCanvas *cRes2 = new TCanvas(
        "TrackTheta", "Resolution of Theta wrt SimTrack", 10, 10, 800, 600);
    ...
    TCanvas *cRes3 = new TCanvas("TrackPhi", "Resolution of phi wrt SimTrack",
                                 10, 10, 800, 600);
    ...
    TCanvas *cRes4 = new TCanvas(
        "inTsosPtDiff", "Resolution of P_{T} at SimHit", 10, 10, 800, 600);
    ...
    TCanvas *cRes5 = new TCanvas("inTsosPhi", "Resolution of Phi at SimHit", 10,
                                 10, 800, 600);
    ...
    TCanvas *cRes6 = new TCanvas(
        "inTsosTheta", "Resolution of #theta at SimHit", 10, 10, 800, 600);
    ...
    TCanvas *cRes7 = new TCanvas("inTsosPtRes", "Resolution of P_{T} at SimHit",
                                 10, 10, 800, 600);
    ...
    TCanvas *c7 =
        new TCanvas("nHits", "Number of RecHits in Track", 10, 10, 800, 600);
    c7->SetFillColor(0);
    c7->SetGrid(1);
    ...
    TCanvas *cRes8 = new TCanvas("PtDis", "Distribution of P_{T} at SimHit", 10,
                                 10, 800, 600);
    ...
    TCanvas *csegs =
        new TCanvas("csegs", "Total & 4D Segments", 10, 10, 800, 600);
    ...
    ttl.DrawLatex(.4, .95,
                  "resolution wrt simHit at innermost (pt, theta, phi) ");
    page_print.str("");
    page_print << page + 1;
    ttl.DrawLatex(.9, .02, page_print.str().c_str());
    pad[page]->Draw();
...
CosmicMuonValidator::bestTrack(const reco::TrackCollection &muons) const {
  ...
  for (reco::TrackCollection::const_iterator muon = muons.begin() + 1;
       muon != muons.end(); ++muon) {

    if (((*muon).found() > bestOne.found()) ||
        (((*muon).found() == bestOne.found()) &&
         ((*muon).chi2() < bestOne.chi2())))
      bestOne = (*muon);
  }
  return bestOne;
}
...
PSimHitContainer
CosmicMuonValidator::matchedHit(const GlobalPoint &tp,
                                const PSimHitContainer &simHs) const {
  ...
  for (PSimHitContainer::const_iterator ish = simHs.begin(); ish != simHs.end();
       ish++) {
...
CosmicMuonValidator::updatedState(const TrajectoryStateOnSurface &tsos,
                                  const PSimHit &hit) const {

  DetId idsim(hit.detUnitId());

  TrajectoryStateOnSurface result = propagator()->propagate(
      tsos, theService->trackingGeometry()->idToDet(idsim)->surface());
