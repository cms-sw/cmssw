...
void DynamicTruncation::updateWithDThits(TrajectoryStateOnSurface &tsos, DTRecSegment4D const &bestDTSeg) {
  ...
  sort(tmprecHits);
  for (const auto &tmprecHit : tmprecHits) {
    DTLayerId layid(tmprecHit->det()->geographicalId());
    TrajectoryStateOnSurface temp = propagator->propagate(tsos, theG->idToDet(layid)->surface());
    if (temp.isValid()) {
      TrajectoryStateOnSurface tempTsos = updatorHandle->update(temp, *tmprecHit);
      if (tempTsos.isValid())
        tsos = tempTsos;
    }
  }
...
void DynamicTruncation::updateWithCSChits(TrajectoryStateOnSurface &tsos, CSCSegment const &bestCSCSeg) {
  ...
  sort(tmprecHits);
  for (const auto &tmprecHit : tmprecHits) {
    const CSCLayer *cscLayer = cscGeom->layer(tmprecHit->det()->geographicalId());
...
TransientTrackingRecHit::ConstRecHitContainer DynamicTruncation::filter(const Trajectory &traj) {
  ...
  for (const auto &muonMeasurement : muonMeasurements) {
    if (!muonMeasurement.recHit()->isValid())
      continue;
    const TransientTrackingRecHit *hit = &(*muonMeasurement.recHit());
    if (hit->geographicalId().det() == DetId::Tracker) {
      result.push_back(muonMeasurement.recHit());
      if (!muonMeasurement.forwardPredictedState().isValid())
        continue;
      if (muonMeasurement.forwardPredictedState().globalPosition().mag() >
          lastTKm.forwardPredictedState().globalPosition().mag())
        lastTKm = muonMeasurement;
    }
  }
...
void DynamicTruncation::filteringAlgo() {
  ...
  for (auto &compatibleId : compatibleIds) {
    int stLayer = stationfromDet(compatibleId.second.front());
    DTRecSegment4D bestDTSeg;
    CSCSegment bestCSCSeg;
    double bestDTEstimator = MAX_THR;
    double bestCSCEstimator = MAX_THR;
    vector<DTRecSegment4D> dtSegs = dtSegMap[compatibleId.first];
    vector<CSCSegment> cscSegs = cscSegMap[compatibleId.first];
...
void DynamicTruncation::compatibleDets(TrajectoryStateOnSurface &tsos, map<int, vector<DetId> > &detMap) {
  ...
  for (auto &navLayer : navLayers) {
    // Skip RPC layers
    if (navLayer->subDetector() != GeomDetEnumerators::DT && navLayer->subDetector() != GeomDetEnumerators::CSC)
      continue;
    ilayerCorrected++;
    vector<DetLayer::DetWithState> comps =
        navLayer->compatibleDets(currentState, *propagatorCompatibleDet, *theEstimator);
    //cout << comps.size() << " compatible Dets with " << navLayers[ilayer]->subDetector() << " Layer " << ilayer << " "
    //<< dumper.dumpLayer(navLayers[ilayer]);
    if (!comps.empty()) {
      for (auto &comp : comps) {
        DetId id(comp.first->geographicalId().rawId());
...
void DynamicTruncation::testDTstation(TrajectoryStateOnSurface &startingState,
  ...
  for (const auto &segment : segments) {
    DTChamberId chamber(segment.chamberId());
    ...
    StateSegmentMatcher estim(tsosdt, segment, apeLoc);
...
void DynamicTruncation::testCSCstation(TrajectoryStateOnSurface &startingState,
  ...
  if (segments.empty())
    return;
  for (const auto &segment : segments) {
    CSCDetId chamber(segment.cscDetId());
    if (!propagator->propagate(startingState, theG->idToDet(chamber)->surface()).isValid())
      continue;
    ...
    StateSegmentMatcher estim(tsoscsc, segment, apeLoc);
    double estimator = estim.value();
