#include "RecoMuon/GlobalTrackingTools/interface/MuonErrorMatrixAnalyzer.h"
...
MuonErrorMatrixAnalyzer::MuonErrorMatrixAnalyzer(const edm::ParameterSet& iConfig)
{
  ...
  theErrorMatrixStore_Error_pset = iConfig.getParameter<edm::ParameterSet>("errorMatrix_Error_pset");
  //  rootFileName  = iConfig.getParameter<std::string>("rootFileName_Error");
  
  theErrorMatrixStore_Spread_pset = iConfig.getParameter<edm::ParameterSet>("errorMatrix_Spread_pset");
  //  rootFileName_Spread = iConfig.getParameter<std::string>("rootFileName_Spread");
...
double MuonErrorMatrixAnalyzer::term(const AlgebraicSymMatrix55 & curv, int i, int j){
  //return sigma^2 or correlation factor
  ...
  if (i==j){
    ...
    if (si<=0 || sj<=0){
      edm::LogError(theCategory)<<"invalid term in the error matrix.\n si: "
...
{
  if(theErrorMatrixStore){
    analyze_from_errormatrix(iEvent,iSetup);
  }

  if(theErrorMatrixStore_Spread){
  ...
  MuonErrorMatrixAnalyzer::analyze_from_errormatrix(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for (uint it=0;it!=tracks->size();++it){
     ...
     /*
          //build the plane accoding to defined convention
	  ...
	  //go to the plane itself
	  //maybe want to change this to tscp because of option in TrackProducerFromSimTrack
	  
	  TrajectoryStateOnSurface tsos(PCAstate,*plane);
	  
	  //get the local error in this plane
	  //Go to this frame and get the error from this frame because this is where the smearing is done in TrackProducerFromSimTrack
	  LocalTrajectoryError localMatrix = tsos.localError();
	  
	  //get the curvilinear error matrix
	  CurvilinearTrajectoryError curvMatrix = tsos.curvilinearError();
	  

     
	  //what is the 5x5 sym matrix you want ?
	  AlgebraicSymMatrix55 errorMatrix = curvMatrix.matrix();
     */
	 ...
	 */
	 ...
	 double value=term(errorMatrix,i,j);
...
MuonErrorMatrixAnalyzer::analyze_from_pull(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for (reco::RecoToSimCollection::const_iterator RtSit=recSimColl.begin();RtSit!=recSimColl.end();++RtSit){
	...
	for (uint i=0;i!=5;++i){
	  for(uint j=i;j<5;++j){
	    uint iH=theErrorMatrixStore_Spread->index(i,j);
	    TProfile3D * ij = theErrorMatrixStore_Spread->get(i,j);
	    ...
	    TH1ptr & theH = (theHist_array[iH])[index(ij,iPt,iEta,iPhi)];
...
{
  if (theErrorMatrixStore_Error_pset.empty()){
    theErrorMatrixStore =0;}
  else{
    //create the error matrix provider, saying that you want to construct the things from here
    theErrorMatrixStore = new ErrorMatrix(theErrorMatrixStore_Error_pset);
  }
  
  if (theErrorMatrixStore_Spread_pset.empty()){
    theErrorMatrixStore_Spread =0;}
  else{
    //create the error matrix provider for the alternative method
    theErrorMatrixStore_Spread = new ErrorMatrix(theErrorMatrixStore_Spread_pset);  
  }
  ...
  for (uint i=0;i!=5;++i){
    for (uint j=i;j<5;++j){
      ...
      theHist_array[iH]= new TH1ptr[maxIndex(ij)];
	...
	for(uint iEta = 0;iEta<etaBin;iEta++){
	  for(uint iPhi = 0;iPhi<phiBin;iPhi++){
	    ...
	    if (i==j){
	      ...
	      theH->SetXTitle("#Delta_{gen-reco}("+ErrorMatrix::vars[i]+")");
	      ...
	      theH->SetXTitle("#Delta_{gen-reco}("+ErrorMatrix::vars[i]+")");
	      theH->SetYTitle("#Delta_{gen-reco}("+ErrorMatrix::vars[j]+")");
...
MuonErrorMatrixAnalyzer::endJob() {
  ...
  if (theErrorMatrixStore){
    ...
    theErrorMatrixStore->close();
  }

  //write the file with all the plots in it.
  if(thePlotFileName!= ""){thePlotFile->Write(); }
  
  //extract the rms and correlation factor from the alt_method object
  for (uint i=0;i!=5;++i){
    for (uint j=i;j<5;++j){
      uint iH=theErrorMatrixStore_Spread->index(i,j);
      TProfile3D * ij = theErrorMatrixStore_Spread->get(i,j);
      TProfile3D * ii = theErrorMatrixStore_Spread->get(i,i);
      TProfile3D * jj = theErrorMatrixStore_Spread->get(j,j);
      if (!ij){edm::LogError(theCategory)<<i<<" "<<j<<" not valid indexes. TProfile3D not found."; continue;}
	...
	for(uint iEta = 0;iEta<etaBin;iEta++){
	  for(uint iPhi = 0;iPhi<phiBin;iPhi++){
	    ...
	    TH1ptr & theH = (theHist_array[iH])[index(ij,iPt,iEta,iPhi)];
	    LogDebug(theCategory)<<theH;
	    LogDebug(theCategory)<<" has name: "<<theH->GetName();
	    double value=0;
	    const bool RMS=false;
	    if (RMS){
	      if (i==j){
		//diagonal term
		double sigma = ((TH1*)theH)->GetRMS();
		LogDebug(theCategory)<<"sigma is: "<<sigma;
		value = sigma*sigma;
	      }
	      else{
		double rho = ((TH2*) theH)->GetCorrelationFactor();
		LogDebug(theCategory)<<"rho is: "<<rho;
		value =rho;
	      }
	      ij->Fill(pt,eta,phi,value);
	    }//if RMS
		...
		ii->Fill(pt,eta,phi,r.x*r.x);
		jj->Fill(pt,eta,phi,r.y*r.y);
  ...
  if (theErrorMatrixStore_Spread){
    //close the at_method object
    theErrorMatrixStore_Spread->close();}
