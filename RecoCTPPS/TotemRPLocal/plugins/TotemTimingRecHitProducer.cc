...
TotemTimingRecHitProducer::TotemTimingRecHitProducer(
    ...
    : digiToken_(consumes<edm::DetSetVector<TotemTimingDigi>>(
...
void TotemTimingRecHitProducer::produce(edm::Event &iEvent,
  ...
  std::unique_ptr<edm::DetSetVector<TotemTimingRecHit>> pOut(
      new edm::DetSetVector<TotemTimingRecHit>);
...
void TotemTimingRecHitProducer::fillDescriptions(
    edm::ConfigurationDescriptions &descr) {
  edm::ParameterSetDescription desc;

  desc.add<edm::InputTag>("digiTag",
                          edm::InputTag("totemTimingRawToDigi", "TotemTiming"))
      ->setComment("input digis collection to retrieve");
  desc.add<unsigned int>("baselinePoints", 8)
      ->setComment("number of points to be used for the baseline");
  desc.add<unsigned int>("risingEdgePointsBeforeTh", 4)
      ->setComment("number of points to be used for the rising edge, before "
                   "the threshold");
  desc.add<unsigned int>("risingEdgePoints", 8)
      ->setComment("number of points to be used for the rising edge");
  desc.add<double>("threholdFactor", 6.)
      ->setComment("the threshold will be threholdFactor*RMS of noise");
  desc.add<double>("cfdFraction", 0.5)->setComment("fraction of the CFD");
  desc.add<double>("hysteresis", 25e-3)
