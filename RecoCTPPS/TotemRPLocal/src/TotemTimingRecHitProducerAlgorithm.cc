...
TotemTimingRecHitProducerAlgorithm::TotemTimingRecHitProducerAlgorithm( const edm::ParameterSet& iConfig ) :
  ...
  risingEdgePointsBeforeTh_     ( iConfig.getParameter<unsigned int>( "risingEdgePointsBeforeTh" ) ),
...
TotemTimingRecHitProducerAlgorithm::build( const CTPPSGeometry* geom, const edm::DetSetVector<TotemTimingDigi>& input, edm::DetSetVector<TotemTimingRecHit>& output )
{
  for ( const auto& vec : input ) {
    ...
    for ( const auto& digi : vec ) {
      ...
      float firstCellTimeInstant = triggerCellTimeInstant - ( SAMPIC_MAX_NUMBER_OF_SAMPLES - OFFSET_SAMPIC ) * SAMPIC_SAMPLING_PERIOD_NS;
      ...
      if ( *max_it < 0.86 ) tmp_1_ = ConstantFractionDiscriminator( time, data );
      ...
      rec_hits.push_back( TotemTimingRecHit( x_pos, x_width, y_pos, y_width, z_pos, z_width, // spatial information
                                             t,
                                             tmp_1_, tmp_2_, amplitude));

    }
  }
}

// Double_t TotemTimingRecHitProducerAlgorithm::ConstantFractionDiscriminator(const std::vector<float>& time, const std::vector<float>& data ) {
//   int i_th=-1;
//   int i_peakStart=0;
...
//
//   std::vector<float>::const_iterator max = std::max_element(data.begin(), data.end());
//   float threshold = *max * cfdFraction_;
//
...
float TotemTimingRecHitProducerAlgorithm::SmartTimeOfArrival(const std::vector<float>& time, const std::vector<float>& data )
{
...
//   // remove baseline
  ...
  // for ( unsigned int i=0; i<data.size(); ++i )
  //   dataCorrected.at(i) = data.at(i) - (baselineRegression.m * time.at(i) + baselineRegression.q);

  // float threshold = 0.4;
  float threshold = threholdFactor_ * baselineRegression.rms + baselineRegression.q;
  int indexOfThresholdCrossing = FastDiscriminator(data, threshold);
  std::cout<< "indexOfThresholdCrossing " << indexOfThresholdCrossing << std::endl;
  // if ( indexOfThresholdCrossing >= 0 && indexOfThresholdCrossing < (int) time.size() -1 ) return time.at(indexOfThresholdCrossing);
  // else return -1;
  ...
  // Find intersection
  float t = ( risingEdgeRegression.m * baselineRegression.q - baselineRegression.m * risingEdgeRegression.q ) / ( risingEdgeRegression.m - baselineRegression.m );
