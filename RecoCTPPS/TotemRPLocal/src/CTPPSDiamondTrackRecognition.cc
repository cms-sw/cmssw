...
CTPPSDiamondTrackRecognition::addHit( const CTPPSDiamondRecHit recHit )
{
  ...
  f_name.append( std::to_string( hit_function_vec_.size() + 1 ) );
  ...
  hit_function_vec_.push_back( hit_f );
}
...
{
  std::vector<double> hit_profile( ( stopAtX_-startFromX_ )/resolution_, 0. );

  for ( unsigned int i=0; i<hit_profile.size(); ++i ) {
    for ( std::vector<TF1>::const_iterator fun_it=hit_function_vec_.begin(); fun_it!=hit_function_vec_.end(); ++fun_it ) { 
  ...
  for ( unsigned int i=0; i<hit_profile.size(); ++i ) {
    ...
    if ( !below ) {
      ...
      if ( hit_profile[i] < threshold_ ) { // going back below the threshold
        ...
        const double threshold = maximum - thresholdFromMaximum_;
