    ...
    const TotemRPGeometry &tot_geom, TotemRPLocalTrack &fitted_track)
{
  ...
  for(auto & applicable_hit : applicable_hits)
  {
    ...
    double sigma_str = applicable_hit.hit->getSigma();
    double sigma_str_2 = sigma_str*sigma_str;
    TVector2 fited_det_xy_point = fitted_track.getTrackPoint(det_z);
    double U_readout = applicable_hit.hit->getPosition() - alg_obj->rec_u_0_;
    ...
    TotemRPLocalTrack::FittedRecHit hit_point(*(applicable_hit.hit), TVector3(fited_det_xy_point.X(),
