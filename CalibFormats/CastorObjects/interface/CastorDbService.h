...
public:
  ...
  const CastorCalibrations &
  getCastorCalibrations(const HcalGenericDetId &fId) const {
    return mCalibSet.getCalibrations(fId);
  }
  const CastorCalibrationWidths &
  getCastorCalibrationWidths(const HcalGenericDetId &fId) const {
    return mCalibWidthSet.getCalibrationWidths(fId);
  }

  const CastorPedestal *getPedestal(const HcalGenericDetId &fId) const;
  const CastorPedestalWidth *
  getPedestalWidth(const HcalGenericDetId &fId) const;
  ...
  const CastorChannelStatus *
  getCastorChannelStatus(const HcalGenericDetId &fId) const;
...
private:
  bool makeCastorCalibration(const HcalGenericDetId &fId,
