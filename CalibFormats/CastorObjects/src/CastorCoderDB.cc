...
template <class Digi>
void CastorCoderDb::fC2adc_(const CaloSamples &clf, Digi &df,
  ...
  for (int i = 0; i < clf.size(); i++) {
    ...
    df.setSample(
        i, HcalQIESample(mCoder->adc(*mShape, clf[i], capId), capId, 0, 0));
  }
}

void CastorCoderDb::adc2fC(const CastorDataFrame &df, CaloSamples &lf) const {
  adc2fC_(df, lf);
}

void CastorCoderDb::fC2adc(const CaloSamples &clf, CastorDataFrame &df,
                           int fCapIdOffset) const {
