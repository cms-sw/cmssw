...
CastorDbService::CastorDbService()
    : mPedestals(nullptr), mPedestalWidths(nullptr), mGains(nullptr),
      mGainWidths(nullptr), mQIEData(nullptr), mChannelQuality(nullptr),
...
void CastorDbService::buildCalibrations() {
  ...
  for (std::vector<DetId>::const_iterator id = ids.begin(); id != ids.end();
       ++id) {
    // make
    ...
    //    std::cout << "Castor calibrations built... detid no. " <<
    //    HcalGenericDetId(*id) << std::endl;
  }
  mCalibSet.sort();
}
...
bool CastorDbService::makeCastorCalibrationWidth(
    const HcalGenericDetId &fId, CastorCalibrationWidths *fObject,
    bool pedestalInADC) const {
  if (fObject) {
    ...
    if (pedestalInADC) {
      ...
      if (pedestalwidth && gainwidth && shape && coder) {
        ...
        *fObject =
            CastorCalibrationWidths(gainwidth->getValues(), pedTrueWidth);
        return true;
      }
    } else {
      if (pedestalwidth && gainwidth) {
        ...
        *fObject =
            CastorCalibrationWidths(gainwidth->getValues(), pedestalWidth);
        return true;
      }
    }
  }
...
const CastorPedestal *
CastorDbService::getPedestal(const HcalGenericDetId &fId) const {
  if (mPedestals) {
...
CastorDbService::getPedestalWidth(const HcalGenericDetId &fId) const {
  if (mPedestalWidths) {
...
const CastorGainWidth *
CastorDbService::getGainWidth(const HcalGenericDetId &fId) const {
  if (mGainWidths) {
...
const CastorQIECoder *
CastorDbService::getCastorCoder(const HcalGenericDetId &fId) const {
  if (mQIEData) {
...
CastorDbService::getCastorChannelStatus(const HcalGenericDetId &fId) const {
