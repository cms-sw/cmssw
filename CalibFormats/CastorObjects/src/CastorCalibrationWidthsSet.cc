...
CastorCalibrationWidthsSet::getCalibrationWidths(const DetId fId) const {
  Item target(fId);
  ...
  if (cell == mItems.end() || cell->id != fId)
    throw cms::Exception("Conditions not found")
        ...
        << HcalGenericDetId(fId);
  return cell->calib;
}

void CastorCalibrationWidthsSet::setCalibrationWidths(
    DetId fId, const CastorCalibrationWidths &ca) {
  sorted_ = false;
  std::vector<Item>::iterator cell = std::find(
      mItems.begin(), mItems.end(), Item(fId)); // slow, but guaranteed
  if (cell == mItems.end()) {
