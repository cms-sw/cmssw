...
CastorCalibrationsSet::getCalibrations(const DetId fId) const {
  Item target(fId);
  ...
  if (cell == mItems.end() || cell->id != fId)
    throw cms::Exception("Conditions not found")
        << "Unavailable CastorCalibrations for cell " << HcalGenericDetId(fId);
  return cell->calib;
}

void CastorCalibrationsSet::setCalibrations(DetId fId,
