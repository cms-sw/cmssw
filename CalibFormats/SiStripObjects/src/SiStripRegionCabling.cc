...
SiStripRegionCabling::SiStripRegionCabling(const uint32_t etadivisions,
    ...
    :
      ...
      phidivisions_(static_cast<int>(phidivisions)), etamax_(etamax),
      regioncabling_()

{}

const SiStripRegionCabling::PositionIndex
SiStripRegionCabling::positionIndex(const Position position) const {
  int eta = static_cast<int>((position.first + etamax_) * etadivisions_ /
                             (2. * etamax_));
  int phi =
      static_cast<int>((position.second + M_PI) * phidivisions_ / (2. * M_PI));
...
SiStripRegionCabling::increment(const PositionIndex index, int deta,
                                int dphi) const {
...
const SiStripRegionCabling::SubDet
SiStripRegionCabling::subdetFromDetId(const uint32_t detid) {
...
void SiStripRegionCabling::print(std::stringstream &ss) const {
  uint32_t valid = 0;
  uint32_t total = 0;
  ss << "[SiStripRegionCabling::" << __func__
     << "] Printing REGION cabling:" << std::endl;
  ss << "Printing cabling for " << regioncabling_.size() << " regions"
     << std::endl;
  ...
  for (; id != jd; ++id) {
    ...
    for (; ir != jr; ++ir) {
      ss << "Printing cabling for " << ir->size() << " wedges for region "
         << static_cast<int32_t>(ir - id->begin()) << std::endl;
      WedgeCabling::const_iterator iw = ir->begin();
      WedgeCabling::const_iterator jw = ir->end();
      for (; iw != jw; ++iw) {
        ss << "Printing cabling for " << iw->size() << " elements for wedge "
           << static_cast<int32_t>(iw - ir->begin()) << std::endl;
        ElementCabling::const_iterator ie = iw->begin();
        ElementCabling::const_iterator je = iw->end();
        for (; ie != je; ++ie) {
          ss << "Printing cabling for " << ie->second.size()
             << " connections for element (DetId) " << ie->first << std::endl;
          std::vector<FedChannelConnection>::const_iterator ic =
              ie->second.begin();
          std::vector<FedChannelConnection>::const_iterator jc =
              ie->second.end();
          for (; ic != jc; ++ic) {
            if (ic->isConnected()) {
              valid++;
