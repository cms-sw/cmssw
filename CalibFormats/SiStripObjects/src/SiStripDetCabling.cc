...
SiStripDetCabling::SiStripDetCabling(const SiStripFedCabling& fedcabling,const TrackerTopology* const topology) : fullcabling_(), connected_(), detected_(), undetected_(), fedCabling_(&fedcabling), tTopo(topology)
{
  ...
  for (unsigned short fed : feds) { // iterate over active feds, get all their FedChannelConnection-s
    SiStripFedCabling::ConnsConstIterRange conns = fedcabling.fedConnections( fed );
    for (const auto & conn : conns) { // loop over FedChannelConnection objects
      addDevices(conn, fullcabling_); // leave separate method, in case you will need to add devices also after constructing
      bool have_fed_id = conn.fedId();
	...
	// there can be at most 6 APVs on one DetId: 0,1,2,3,4,5
        int which_apv_pair = conn.apvPairNumber(); // APVPair (0,1) for 512 strips and (0,1,2) for 768 strips
	
	// patch needed to take into account invalid detids or apvPairs
	if( conn.detId()==0 ||  
	    conn.detId() == sistrip::invalid32_ ||  
	    conn.apvPairNumber() == sistrip::invalid_  ||
	    conn.nApvPairs() == sistrip::invalid_ ) {
	  continue;
	}

	if(conn.i2cAddr(0)) vector_of_connected_apvs.push_back(2*which_apv_pair + 0); // first apv of the pair
	if(conn.i2cAddr(1)) vector_of_connected_apvs.push_back(2*which_apv_pair + 1); // second apv of the pair
      }
      if(vector_of_connected_apvs.size() != 0){ // add only is smth. there, obviously
        std::map<uint32_t, std::vector<int> > map_of_connected_apvs;
        map_of_connected_apvs.insert(std::make_pair(conn.detId(),vector_of_connected_apvs));
  ...
  for(const auto & detected_fed_connection : detected_fed_connections){
    addDevices(detected_fed_connection, fullcabling_);
    bool have_fed_id = detected_fed_connection.fedId();
    std::vector<int> vector_of_detected_apvs;
    if(! have_fed_id){
      int which_apv_pair = detected_fed_connection.apvPairNumber(); // APVPair (0,1) for 512 strips and (0,1,2) for 768 strips
      if(detected_fed_connection.i2cAddr(0)) vector_of_detected_apvs.push_back(2*which_apv_pair + 0); // first apv of the pair
      if(detected_fed_connection.i2cAddr(1)) vector_of_detected_apvs.push_back(2*which_apv_pair + 1); // second apv of the pair
    }
    if(vector_of_detected_apvs.size() != 0){ // add only is smth. there, obviously
      std::map<uint32_t,std::vector<int> > map_of_detected_apvs;
      map_of_detected_apvs.insert(std::make_pair(detected_fed_connection.detId(),vector_of_detected_apvs));
  ...
  for(const auto & undetected_fed_connection : undetected_fed_connections){
    addDevices(undetected_fed_connection, fullcabling_);
    bool have_fed_id = undetected_fed_connection.fedId();
    std::vector<int> vector_of_undetected_apvs;
    if(! have_fed_id){
      int which_apv_pair = undetected_fed_connection.apvPairNumber(); // APVPair (0,1) for 512 strips and (0,1,2) for 768 strips
      if(undetected_fed_connection.i2cAddr(0)) vector_of_undetected_apvs.push_back(2*which_apv_pair + 0); // first apv of the pair
      if(undetected_fed_connection.i2cAddr(1)) vector_of_undetected_apvs.push_back(2*which_apv_pair + 1); // second apv of the pair
    }
    if(vector_of_undetected_apvs.size() != 0){ // add only is smth. there, obviously
      ...
      map_of_undetected_apvs.insert(std::make_pair(undetected_fed_connection.detId(),vector_of_undetected_apvs));
...
void SiStripDetCabling::addActiveDetectorsRawIds(std::vector<uint32_t> & vector_to_fill_with_detids ) const{
  for(const auto & conn_it : connected_){
...
void SiStripDetCabling::addAllDetectorsRawIds(std::vector<uint32_t> & vector_to_fill_with_detids ) const{
  ...
  for(const auto & conn_it : detected_){
    vector_to_fill_with_detids.push_back(conn_it.first);
  }
  for(const auto & conn_it : undetected_){
    vector_to_fill_with_detids.push_back(conn_it.first);
  }
...
const FedChannelConnection& SiStripDetCabling::getConnection( uint32_t det_id, unsigned short apv_pair ) const{
  ...
  for(auto fcconn : fcconns){
    if ( (fcconn != 0) && ((fcconn->apvPairNumber()) == apv_pair) ) { // check if apvPairNumber() of present FedChannelConnection is the same as requested one
      return (*fcconn); // if yes, return the FedChannelConnection object
    }
  }
...
const unsigned int SiStripDetCabling::getDcuId( uint32_t det_id ) const{
  ...
  if(fcconns.size()!=0) {
    ...
    for(auto fcconn : fcconns)
      if (fcconn && fcconn->detId() != sistrip::invalid32_ && fcconn->detId() != 0 )
        return fcconn->dcuId(); // get dcuId of first element - when you build check this consistency
  }
...
const uint16_t SiStripDetCabling::nApvPairs(uint32_t det_id) const{
  ...
  if(fcconns.size()!=0) {
    // patch needed to take into account the possibility that the first component of fcconns is invalid
    for(auto fcconn : fcconns) {
      if ( (fcconn != 0) && (fcconn->nApvPairs() != sistrip::invalid_) ) {
        return fcconn->nApvPairs(); // nr of apvpairs for associated module
      }
    }
  }
  // else {
...
void SiStripDetCabling::addFromSpecificConnection( std::map<uint32_t, std::vector<int> > & map_to_add_to,
  ...
  for(const auto & conn_it : specific_connection){
    ...
    if( it == map_to_add_to.end() ){ // detid does not exist in map, add new entry
      ...
      for(int & inew : new_apv_vector){
        bool there_already = false;
        for(int & iold : existing_apv_vector){
	  if (iold == inew){
	    there_already = true;
