...
SiStripDetCabling::SiStripDetCabling(const TrackerTopology *const topology)
    : fedCabling_(nullptr), tTopo(topology) {}
...
SiStripDetCabling::SiStripDetCabling(const SiStripFedCabling &fedcabling,
    ...
    : fullcabling_(), connected_(), detected_(), undetected_(),
      fedCabling_(&fedcabling), tTopo(topology) {
  ...
  for (auto ifed = feds.begin(); ifed != feds.end();
       ifed++) { // iterate over active feds, get all their
                 // FedChannelConnection-s
    SiStripFedCabling::ConnsConstIterRange conns =
        fedcabling.fedConnections(*ifed);
    for (auto iconn = conns.begin(); iconn != conns.end();
         iconn++) { // loop over FedChannelConnection objects
      addDevices(*iconn,
      ...
      if (have_fed_id) { // these apvpairs are seen from the readout
        ...
        int which_apv_pair =
            iconn->apvPairNumber(); // APVPair (0,1) for 512 strips and (0,1,2)
                                    // for 768 strips
        ...
        if (iconn->i2cAddr(0))
          vector_of_connected_apvs.push_back(2 * which_apv_pair +
                                             0); // first apv of the pair
        if (iconn->i2cAddr(1))
          vector_of_connected_apvs.push_back(2 * which_apv_pair +
                                             1); // second apv of the pair
      }
      if (!vector_of_connected_apvs
               .empty()) { // add only is smth. there, obviously
        std::map<uint32_t, std::vector<int>> map_of_connected_apvs;
        map_of_connected_apvs.insert(
            std::make_pair(iconn->detId(), vector_of_connected_apvs));
  ...
  SiStripFedCabling::ConnsConstIterRange detected_fed_connections =
      fedcabling.detectedDevices();
  for (std::vector<FedChannelConnection>::const_iterator idtct =
           detected_fed_connections.begin();
       idtct != detected_fed_connections.end(); ++idtct) {
    addDevices(*idtct, fullcabling_);
    bool have_fed_id = idtct->fedId();
    std::vector<int> vector_of_detected_apvs;
    if (!have_fed_id) {
      int which_apv_pair =
          idtct->apvPairNumber(); // APVPair (0,1) for 512 strips and (0,1,2)
                                  // for 768 strips
      if (idtct->i2cAddr(0))
        vector_of_detected_apvs.push_back(2 * which_apv_pair +
                                          0); // first apv of the pair
      if (idtct->i2cAddr(1))
        vector_of_detected_apvs.push_back(2 * which_apv_pair +
                                          1); // second apv of the pair
    }
    if (!vector_of_detected_apvs
             .empty()) { // add only is smth. there, obviously
      std::map<uint32_t, std::vector<int>> map_of_detected_apvs;
      map_of_detected_apvs.insert(
          std::make_pair(idtct->detId(), vector_of_detected_apvs));
      addFromSpecificConnection(detected_, map_of_detected_apvs,
                                &(connectionCount[1]));
    }
  }
  // --- UNDETECTED = have neither fedid nor i2caddr
  SiStripFedCabling::ConnsConstIterRange undetected_fed_connections =
      fedcabling.undetectedDevices();
  for (std::vector<FedChannelConnection>::const_iterator iudtct =
           undetected_fed_connections.begin();
       iudtct != undetected_fed_connections.end(); ++iudtct) {
    addDevices(*iudtct, fullcabling_);
    bool have_fed_id = iudtct->fedId();
    std::vector<int> vector_of_undetected_apvs;
    if (!have_fed_id) {
      int which_apv_pair =
          iudtct->apvPairNumber(); // APVPair (0,1) for 512 strips and (0,1,2)
                                   // for 768 strips
      if (iudtct->i2cAddr(0))
        vector_of_undetected_apvs.push_back(2 * which_apv_pair +
                                            0); // first apv of the pair
      if (iudtct->i2cAddr(1))
        vector_of_undetected_apvs.push_back(2 * which_apv_pair +
                                            1); // second apv of the pair
    }
    if (!vector_of_undetected_apvs
             .empty()) { // add only is smth. there, obviously
      std::map<uint32_t, std::vector<int>> map_of_undetected_apvs;
      map_of_undetected_apvs.insert(
          std::make_pair(iudtct->detId(), vector_of_undetected_apvs));
...
void SiStripDetCabling::addDevices(
  ...
  if (conn.detId() &&
      conn.detId() != sistrip::invalid32_ && // check for valid detid
      conn.apvPairNumber() !=
          sistrip::invalid_) { // check for valid apv pair number
    if (conn.fedId() == 0 || conn.fedId() == sistrip::invalid_) {
      edm::LogInfo("") << " SiStripDetCabling::addDevices for connection "
                          "associated to detid "
                       << conn.detId() << " apvPairNumber "
                       << conn.apvPairNumber() << "the fedId is "
                       << conn.fedId();
      return;
    }
...
void SiStripDetCabling::addDevices(
    const FedChannelConnection
        &conn) { // by default add to fullcabling_ connections - special case of
                 // above class
...
void SiStripDetCabling::addActiveDetectorsRawIds(
    std::vector<uint32_t> &vector_to_fill_with_detids) const {
  for (std::map<uint32_t, std::vector<int>>::const_iterator conn_it =
           connected_.begin();
       conn_it != connected_.end(); ++conn_it) {
...
void SiStripDetCabling::addAllDetectorsRawIds(
  ...
  for (std::map<uint32_t, std::vector<int>>::const_iterator conn_it =
           detected_.begin();
       conn_it != detected_.end(); ++conn_it) {
    vector_to_fill_with_detids.push_back(conn_it->first);
  }
  for (std::map<uint32_t, std::vector<int>>::const_iterator conn_it =
           undetected_.begin();
       conn_it != undetected_.end(); ++conn_it) {
    vector_to_fill_with_detids.push_back(conn_it->first);
  }
  // no elements added to vector_to_fill_with_detids is empty connected_,
...
//----
const std::vector<const FedChannelConnection *> &
SiStripDetCabling::getConnections(uint32_t det_id)
    const { // return all connections corresponding to one det_id
  ...
  if (!(detcabl_it == fullcabling_.end())) { // found detid in fullcabling_
    return (detcabl_it->second);
           ...
           // not exist?
    const static std::vector<const FedChannelConnection *>
        default_empty_fedchannelconnection;
...
//----
const FedChannelConnection &
SiStripDetCabling::getConnection(uint32_t det_id,
                                 unsigned short apv_pair) const {
  const std::vector<const FedChannelConnection *> &fcconns =
      getConnections(det_id);
  for (std::vector<const FedChannelConnection *>::const_iterator iconn =
           fcconns.begin();
       iconn != fcconns.end(); ++iconn) {
    if (((*iconn) != nullptr) &&
        (((*iconn)->apvPairNumber()) ==
         apv_pair)) {   // check if apvPairNumber() of present
...
const unsigned int SiStripDetCabling::getDcuId(uint32_t det_id) const {
  ...
  if (!fcconns.empty()) {
    ...
    for (size_t i = 0; i < fcconns.size(); ++i)
      if (fcconns.at(i) && fcconns.at(i)->detId() != sistrip::invalid32_ &&
          fcconns.at(i)->detId() != 0)
...
const uint16_t SiStripDetCabling::nApvPairs(uint32_t det_id) const {
  ...
  if (!fcconns.empty()) {
    ...
    // component of fcconns is invalid
    for (size_t i = 0; i < fcconns.size(); ++i) {
      if ((fcconns.at(i) != nullptr) &&
          (fcconns.at(i)->nApvPairs() != sistrip::invalid_)) {
        return fcconns.at(i)
            ->nApvPairs(); // nr of apvpairs for associated module
      }
    }
  }
  // else {
...
void SiStripDetCabling::addConnected(
    std::map<uint32_t, std::vector<int>> &map_to_add_to) const {
...
//----
void SiStripDetCabling::addFromSpecificConnection(
    std::map<uint32_t, std::vector<int>> &map_to_add_to,
    const std::map<uint32_t, std::vector<int>> &specific_connection,
    std::map<int16_t, uint32_t> *connectionsToFill) const {
  for (std::map<uint32_t, std::vector<int>>::const_iterator conn_it =
           specific_connection.begin();
       conn_it != specific_connection.end(); ++conn_it) {
    uint32_t new_detid = conn_it->first;
    std::vector<int> new_apv_vector = conn_it->second;
    std::map<uint32_t, std::vector<int>>::iterator it =
        map_to_add_to.find(new_detid);
    if (it ==
        map_to_add_to.end()) { // detid does not exist in map, add new entry
      std::sort(
          new_apv_vector.begin(),
      ...
      for (std::vector<int>::iterator inew = new_apv_vector.begin();
           inew != new_apv_vector.end(); ++inew) {
        bool there_already = false;
        for (std::vector<int>::iterator iold = existing_apv_vector.begin();
             iold != existing_apv_vector.end(); ++iold) {
          if (*iold == *inew) {
            there_already = true;
        ...
        if (!there_already) {
          ...
          std::sort(existing_apv_vector.begin(),
                    existing_apv_vector
                        .end()); // not very efficient sort, time consuming?
        } else {
...
uint32_t SiStripDetCabling::detNumber(const std::string &subDet,
  ...
  if (subDet == "TID-")
    ...
    LogDebug("SiStripDetCabling")
        << "Error: Wrong subDet. Please use one of TIB, TID, TOB, TEC."
        << std::endl;
    return 0;
  }
...
void SiStripDetCabling::getAllDetectorsContiguousIds(
    std::map<uint32_t, unsigned int> &allToContiguous) const {
  ...
  for (std::vector<uint32_t>::const_iterator idet = all.begin();
       idet != all.end(); ++idet) {
    ++contiguousIndex;
...
void SiStripDetCabling::getActiveDetectorsContiguousIds(
    std::map<uint32_t, unsigned int> &connectedToContiguous) const {
  ...
  for (std::vector<uint32_t>::const_iterator idet = connected.begin();
       idet != connected.end();
       ++idet) { // select only the indices for active detectors
    std::map<uint32_t, unsigned int>::iterator deco =
        allToContiguous.find(*idet);
...
bool SiStripDetCabling::IsInMap(
    const uint32_t &det_id,
    const std::map<uint32_t, std::vector<int>> &map) const {
...
void SiStripDetCabling::print(std::stringstream &ss) const {
  ...
  ss << "[SiStripDetCabling::" << __func__ << "]"
     << " Printing DET cabling for " << fullcabling_.size() << " modules "
     << std::endl;
  for (; ii != jj; ++ii) {
...
void SiStripDetCabling::printSummary(std::stringstream &ss,
  ...
  for (int connectionType = 0; connectionType < 3; ++connectionType) {
    ...
    std::map<int16_t, uint32_t>::const_iterator iter =
...
void SiStripDetCabling::printDebug(
    std::stringstream &ss, const TrackerTopology * /*trackerTopo*/) const {
  print(ss);
}
