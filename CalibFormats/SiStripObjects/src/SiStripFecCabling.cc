...
void SiStripFecCabling::buildFecCabling(const SiStripFedCabling &fed_cabling) {
  ...
  // Consistency checks
  for (std::vector<SiStripFecCrate>::const_iterator icrate =
           this->crates().begin();
       icrate != this->crates().end(); ++icrate) {
    for (std::vector<SiStripFec>::const_iterator ifec = icrate->fecs().begin();
         ifec != icrate->fecs().end(); ++ifec) {
      for (std::vector<SiStripRing>::const_iterator iring =
               ifec->rings().begin();
           iring != ifec->rings().end(); ++iring) {
        for (std::vector<SiStripCcu>::const_iterator iccu =
                 iring->ccus().begin();
             iccu != iring->ccus().end(); ++iccu) {
          for (std::vector<SiStripModule>::const_iterator imod =
                   iccu->modules().begin();
               imod != iccu->modules().end(); ++imod) {
...
// -----------------------------------------------------------------------------
//
void SiStripFecCabling::connections(
    std::vector<FedChannelConnection> &conns) const {
  LogTrace(mlCabling_) << "[SiStripFecCabling::" << __func__ << "]"
          ...
          for (std::vector<SiStripModule>::const_iterator imod =
            ...
            for (uint16_t ipair = 0; ipair < imod->nApvPairs(); ipair++) {
              conns.push_back(FedChannelConnection(
                  icrate->fecCrate(), ifec->fecSlot(), iring->fecRing(),
                  iccu->ccuAddr(), imod->ccuChan(),
                  ...
                  imod->fedCh(ipair).fedId_, imod->fedCh(ipair).fedCh_,
                  imod->length(), imod->dcu(), imod->pll(), imod->mux(),
                  imod->lld()));
...
auto moduleFrom(T &crates, const FedChannelConnection &conn)
  ...
  if (icrate != crates.end()) {
    ...
    if (ifec != icrate->fecs().end()) {
      ...
      while (iring != ifec->rings().end() &&
             iring->fecRing() != conn.fecRing()) {
        iring++;
      }
      if (iring != ifec->rings().end()) {
        auto iccu = iring->ccus().begin();
        while (iccu != iring->ccus().end() &&
               iccu->ccuAddr() != conn.ccuAddr()) {
          iccu++;
        }
        if (iccu != iring->ccus().end()) {
          auto imod = iccu->modules().begin();
          while (imod != iccu->modules().end() &&
                 imod->ccuChan() != conn.ccuChan()) {
            imod++;
          }
...
const SiStripModule &
SiStripFecCabling::module(const FedChannelConnection &conn) const {
          ...
          for (std::vector<SiStripModule>::const_iterator imod =
            ...
            for (uint16_t ipair = 0; ipair < imod->nApvPairs(); ipair++) {
              ...
              if (fed_id) {
                ...
                std::vector<uint16_t>::iterator icrate =
                ...
                std::vector<uint16_t>::iterator islot =
...
void SiStripFecCabling::print(std::stringstream &ss) const {
  ...
  for (std::vector<SiStripFecCrate>::const_iterator icrate = crates().begin();
       icrate != crates().end(); ++icrate) {
    ss << "Printing cabling for " << icrate->fecs().size() << " FECs for crate "
       << icrate->fecCrate() << std::endl;
    for (std::vector<SiStripFec>::const_iterator ifec = icrate->fecs().begin();
         ifec != icrate->fecs().end(); ++ifec) {
      ss << "Printing cabling for " << ifec->rings().size() << " rings for FEC "
         << ifec->fecSlot() << std::endl;
      for (std::vector<SiStripRing>::const_iterator iring =
               ifec->rings().begin();
           iring != ifec->rings().end(); ++iring) {
        ss << "Printing cabling for " << iring->ccus().size()
           << " CCUs for ring " << iring->fecRing() << std::endl;
        for (std::vector<SiStripCcu>::const_iterator iccu =
                 iring->ccus().begin();
             iccu != iring->ccus().end(); ++iccu) {
          ss << "Printing cabling for " << iccu->modules().size()
          ...
          for (std::vector<SiStripModule>::const_iterator imod =
            ...
            for (; ii != jj; ++ii) {
              if (ii->second.fedId_ != sistrip::invalid_ &&
                  ii->second.fedCh_ != sistrip::invalid_) {
                valid++;
              }
              total++;
            }
