...
public:
  ...
  void
  addDevices(const FedChannelConnection &,
             std::map<uint32_t, std::vector<const FedChannelConnection *>> &);
  void
  addDevices(const FedChannelConnection &); // special case of above addDevices
  // getters
  inline const std::map<uint32_t, std::vector<const FedChannelConnection *>> &
  getDetCabling() const {
    return fullcabling_;
  }
  // for DQM use: all detectors that have at least one connected APV
  void addActiveDetectorsRawIds(std::vector<uint32_t> &)
      const; // add to vector Ids of connected modules (active == connected)
  void addAllDetectorsRawIds(std::vector<uint32_t> &vector_to_fill_with_detids)
      const; // add to vector Ids of all modules
  void getAllDetectorsContiguousIds(std::map<uint32_t, unsigned int> &)
      const; // map of all connected, detected, undetected to contiguous Ids -
             // map is reset first!
  void getActiveDetectorsContiguousIds(std::map<uint32_t, unsigned int> &)
      const; // map of all connected to contiguous Ids - map is reset first!
  // for RECO use
  void addConnected(std::map<uint32_t, std::vector<int>> &)
      const; // map of detector to list of APVs for APVs seen from FECs and FEDs
  void addDetected(std::map<uint32_t, std::vector<int>> &)
      const; // map of detector to list of APVs for APVs seen from FECs but not
             // from FEDs
  void addUnDetected(std::map<uint32_t, std::vector<int>> &)
      const; // map of detector to list of APVs for APVs seen neither from FECS
             // or FEDs
  void addNotConnectedAPVs(std::map<uint32_t, std::vector<int>> &)
      const; // map of detector to list of APVs that are not connected -
  ...
  const FedChannelConnection &getConnection(uint32_t det_id,
                                            unsigned short apv_pair) const;
  const unsigned int getDcuId(uint32_t det_id) const;
  const uint16_t nApvPairs(uint32_t det_id)
      const; // maximal nr. of apvpairs a detector can have (2 or 3)
  ...
  uint32_t connectedNumber(const std::string &subDet,
                           const uint16_t layer) const {
    return detNumber(subDet, layer, 0);
  }
  uint32_t detectedNumber(const std::string &subDet,
                          const uint16_t layer) const {
    return detNumber(subDet, layer, 1);
  }
  uint32_t undetectedNumber(const std::string &subDet,
                            const uint16_t layer) const {
...
private:
  void addFromSpecificConnection(
  ...
  bool IsInMap(const uint32_t &det_id,
               const std::map<uint32_t, std::vector<int>> &) const;
  int16_t layerSearch(const uint32_t detId) const;
  uint32_t detNumber(const std::string &subDet, const uint16_t layer,
  ...
  std::map<uint32_t, std::vector<int>>
      detected_; // seen from FECs but not from FEDs
  std::map<uint32_t, std::vector<int>>
      undetected_; // seen from neither FECs or FEDs, DetIds inferred from
