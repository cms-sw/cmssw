...
public:
  ...
  inline static const ElementIndex
  elementIndex(const uint32_t region, const SubDet, const uint32_t layer);

  inline const ElementIndex elementIndex(const PositionIndex, const SubDet,
                                         const uint32_t layer) const;

  inline const ElementIndex elementIndex(const Position, const SubDet,
                                         const uint32_t layer) const;
...
inline void
SiStripRegionCabling::setRegionCabling(const Cabling &regioncabling) {
  regioncabling_ = regioncabling;
}

inline const SiStripRegionCabling::Cabling &
SiStripRegionCabling::getRegionCabling() const {
...
SiStripRegionCabling::regionDimensions() const {
  return std::pair<double, double>((2. * etamax_) / etadivisions_,
                                   2. * M_PI / phidivisions_);
}

inline const SiStripRegionCabling::Position
SiStripRegionCabling::position(const uint32_t region) const {
  PositionIndex index = positionIndex(region);
  return position(index);
}

inline const SiStripRegionCabling::Position
SiStripRegionCabling::position(const PositionIndex index) const {
  return Position(regionDimensions().first * (index.first + .5) - etamax_,
                  regionDimensions().second * (index.second + .5) - M_PI);
}

inline const SiStripRegionCabling::PositionIndex
SiStripRegionCabling::positionIndex(const uint32_t region) const {
  return PositionIndex(region / phidivisions_, region % phidivisions_);
}

inline const uint32_t
SiStripRegionCabling::region(const PositionIndex index) const {
...
inline const uint32_t
SiStripRegionCabling::elementIndex(const PositionIndex index,
                                   const SubDet subdet,
                                   const uint32_t layer) const {
...
inline const uint32_t
SiStripRegionCabling::elementIndex(const Position position, const SubDet subdet,
                                   const uint32_t layer) const {
...
inline const SiStripRegionCabling::SubDet
SiStripRegionCabling::subdet(const uint32_t index) {
  return static_cast<SiStripRegionCabling::SubDet>((index / ALLLAYERS) %
                                                   ALLSUBDETS);
}
