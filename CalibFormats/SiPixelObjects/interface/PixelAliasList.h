  ...
  public:
    void writefile() {
      ...
      for (unsigned int i = 0; i < pixelConfigAliases_.size(); i++) {
        ...
        out << theAlias.name() << "     " << theAlias.key() << "   ";
      ...
      for (unsigned int i = 0; i < pixelVersionAliases_.size(); i++) {
        ...
        out << theAlias.path() << "  " << theAlias.version() << " " << theAlias.alias() << " ;" << std::endl;
      }
    }
    ...
    void insertVersionAlias(PixelVersionAlias& anAlias) {
      for (unsigned int i = 0; i < pixelVersionAliases_.size(); i++) {
        if (pixelVersionAliases_[i].alias() == anAlias.alias() && pixelVersionAliases_[i].path() == anAlias.path()) {
          std::cout << "[PixelAliasList::insertVersionAlias()]\t\tReplacing existing version alias:" << anAlias.path()
                    << " " << anAlias.alias() << std::endl;
    ...
    unsigned int getVersion(std::string path, std::string alias) {
      for (unsigned int i = 0; i < pixelVersionAliases_.size(); i++) {
        if (pixelVersionAliases_[i].alias() == alias && pixelVersionAliases_[i].path() == path) {
    ...
    // Added by Dario, May 20th, 2008 =====================================================================
    pathVersionAliasMmap getVersionData() { return versionMmap; }
