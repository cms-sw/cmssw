...
*/
  ...
  public:
    ...
    unsigned int getAOHBias(unsigned int AOHNumber) const {
      return getdeviceValuesForAddress(AOHBiasAddressFromAOHNumber(AOHNumber));
    }
    ...
    void setAOHGain(unsigned int AOHNumber, unsigned int value) {
      setAOHGain(AOHGainStringFromAOHNumber(AOHNumber), value);
    }
  ...
  private:
    ...
    bool containsSetting(const std::string &settingName) const {
    ...
    std::map<std::string, unsigned int>
        nameToAddress_;  // translation from name to address, filled in by fillNameToAddress();
