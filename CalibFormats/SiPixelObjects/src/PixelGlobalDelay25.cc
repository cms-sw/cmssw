...
unsigned int PixelGlobalDelay25::getDelay(unsigned int offset) const{
  ...
  if (ret > 127) {
    std::cout<<"PixelGlobalDelay25:: the required total delay "<<ret<<" is out of range."<<endl;
    std::cout<<"PixelGlobalDelay25:: original setting: "<<offset<<", global delay: "<<delay_<<endl;
    std::cout<<"PixelGlobalDelay25:: we will keep the default delay setting..."<<endl;
