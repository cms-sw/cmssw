...
HcalTPGSimpleTranscoder::HcalTPGSimpleTranscoder(int hcalMeV, int egammaMeV, int jetMeV) : 
  hcalLSBMeV_(hcalMeV),
...
HcalTPGSimpleTranscoder::~HcalTPGSimpleTranscoder() {
}


void HcalTPGSimpleTranscoder::htrCompress(const IntegerCaloSamples& ics, const std::vector<bool>& fineGrain, HcalTriggerPrimitiveDigi& digi) {
...
void HcalTPGSimpleTranscoder::rctEGammaUncompress(const HcalTriggerPrimitiveDigi& digi, IntegerCaloSamples& ics) {
...
void HcalTPGSimpleTranscoder::rctJetUncompress(const HcalTriggerPrimitiveDigi& digi, IntegerCaloSamples& ics) {
  ics=IntegerCaloSamples(digi.id(),digi.size());
  for (int i=0; i<digi.size(); i++) {
    unsigned int code=digi[i].compressedEt();
...
void HcalTPGSimpleTranscoder::buildTable() {
...
void HcalTPGSimpleTranscoder::printTable() {
