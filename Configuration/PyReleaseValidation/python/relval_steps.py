...
class InputInfo(object):
    ...
    def das(self):
        ...
        if len(self.run) is not 0:
            command = ";".join(["dasgoclient --limit=0 --query 'file {0}={1} run={2}'".format(query_by, query_source, query_run) for query_run in self.run])
            command = "({0})".format(command)
        else:
            command = "dasgoclient --limit=0 --query 'file {0}={1} site=T2_CH_CERN'".format(query_by, query_source)
...
steps['ZElJan2013Run2012D']={'INPUT':InputInfo(dataSet='/DoubleElectron/Run2012D-ZElectron-22Jan2013-v1/RAW-RECO',label='zEl2012D',location='STD',ib_block='109d2d00-973b-11e2-8bba-003048f02d38')}
