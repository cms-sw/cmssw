...
TCPReceiver::TCPReceiver(unsigned short int port, std::string IP,
...
int TCPReceiver::ReceiveLumiSection(HCAL_HLX::LUMI_SECTION &localSection) {
  ...
  if (acquireMode == 0) { // real data
    if (Connected == false) {
          ...
          if (FD_ISSET(tcpSocket, &fdsRead)) {
            ...
            if ((bytesRcvd = recv(tcpSocket, Buffer, Buffer_Size, 0)) <= 0) {
              ...
              if ((totalBytesRcvd + bytesRcvd) <= bytesToReceive) {
                ...
                std::cout << "***** MEM OVER FLOW: Did someone forget to "
                             "update LumiStructures.hh? *****"
                          << std::endl;
                errorCode = 502;
              }
      ...
      if (errorCode == 0) {
        memcpy(reinterpret_cast<char *>(&localSection), BigBuffer,
               sizeof(localSection));
        errorCode = 1; // success
      }
      delete[] Buffer;
...
#ifdef DEBUG
  std::cout << "Begin and End  " << __PRETTY_FUNCTION__ << " " << Connected
            << std::endl;
#endif
  return Connected;
}
...
#endif
        if (connect(tcpSocket, (struct sockaddr *)&servAddr, sizeof(servAddr)) <
            0) {
