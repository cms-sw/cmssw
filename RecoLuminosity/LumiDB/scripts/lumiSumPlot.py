...
if __name__=='__main__':
    ...
    parser.add_argument('-c',dest='connect',action='store',help='connect string to lumiDB')
    parser.add_argument('-P',dest='authpath',action='store',help='path to authentication file')
    parser.add_argument('-norm',dest='norm',action='store',default='pp7TeV')
    parser.add_argument('-i',dest='inputfile',action='store',help='ls range selection file')
    parser.add_argument('-r',dest='runnumber',action='store',type=int,help='run number')
#    parser.add_argument('-f',dest='fillnum',action='store',help='specific fill',default=None)
    parser.add_argument('-b',dest='beamstatus',action='store',help='selection criteria beam status,e.g.stable')
    parser.add_argument('-amodetag',dest='amodetag',action='store',help='accelerator mode')
    parser.add_argument('-beamenergy',dest='beamenergy',action='store',type=float,default=3500,help='beamenergy (in GeV) selection criteria,e.g. 3.5e3')
    parser.add_argument('-beamfluctuation',dest='beamfluctuation',action='store',type=float,default=0.2,help='allowed fraction of beamenergy to fluctuate, e.g. 0.1')
    ...
    parser.add_argument('-beginTime',dest='begintime',action='store',default='03/01/10 00:00:00',help='min run start time,mm/dd/yy hh:mm:ss')
    parser.add_argument('-endTime',dest='endtime',action='store',help='max run start time,,mm/dd/yy hh:mm:ss')
    ...
    ####plot options
    parser.add_argument('-inplot',dest='inplot',action='store',help='existing base plot(s) in text format')
    parser.add_argument('-outplot',dest='outplot',action='store',help='output plot. By default, a text dump of the plot is produced.')
    parser.add_argument('-yscale',dest='yscale',action='store',default='linear',help='y_scale')
    parser.add_argument('--with-annotation',dest='withannotation',action='store_true',help='annotate boundary run numbers')
    parser.add_argument('--interactive',dest='interactive',action='store_true',help='graphical mode to draw plot in a TK pannel.')
    parser.add_argument('--without-correction',dest='withoutCorrection',action='store_true',help='without fine correction')
    parser.add_argument('--without-textoutput',dest='withoutTextoutput',action='store_true',help='not to write out text output file')
    ...
    lumibyls=lumiCalcAPI.lumiForRange(schema,irunlsdict,amodetag=options.amodetag,egev=options.beamenergy,beamstatus=pbeammode,norm=normfactor,finecorrections=finecorrections)
