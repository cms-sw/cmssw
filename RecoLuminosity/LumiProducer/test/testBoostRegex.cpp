...
int main() {
  ...
  //const std::regex e("TK_HV_ON&N/A&N/A%PIX_HV_ON&N/A&N/A%LHC_RAMPING&false&false%PHYSICS_DECLARED&(true|false|N/A)&(true|false|N/A)%$");
  const std::regex e("%PHYSICS_DECLARED&(true|false|N/A)&(true|false|N/A)%");
  const std::regex ename("^CMS.LVL0:RUNSECTION_DELIMITER_DCSLHCFLAGS_([0-9]+)");
  std::match_results<std::string::const_iterator> what;
  std::regex_search(input, what, e, std::regex_constants::match_default);
  if (what[0].matched) {
    std::cout << "matched" << std::endl;
    std::cout << std::string(what[1].first, what[1].second) << std::endl;
    std::cout << std::string(what[2].first, what[2].second) << std::endl;
  }
  std::regex_match(nameinput, what, ename, std::regex_constants::match_default);
  if (what[0].matched) {
  ...
  while (std::getline(ins, line)) {
    ...
    if (result.size() < 3)
      continue;
    std::cout << "fill num " << result[0] << " , fill scheme " << result[1] << ", ncolliding bunches " << result[2]
              << std::endl;
