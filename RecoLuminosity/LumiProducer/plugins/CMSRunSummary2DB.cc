  ...
  private:
   ...
   void CMSRunSummary2DB::retrieveData( unsigned int runnumber){
	 ...
	 throw er;
      ...
      delete runinfosession;
      std::cout<<"run "<<runnumber<<", sequence : "<<result.sequence<<", l1key : "<<result.l1key<<", hltkey : "<<result.hltkey<<", fillnumber : "<<result.fillnumber<<", amodetag : "<<result.amodetag<<std::endl; 
      
      //std::cout<<"connecting to dest "<<m_dest<<std::endl; 
      if(!m_novalidate&& !isCollisionRun(result) ){
	 std::cout<<"is not collision run"<<std::endl;
	 throw lumi::invalidDataException("is not collision run","isCollisionRun","CMSRunSummary2DB");
      }
