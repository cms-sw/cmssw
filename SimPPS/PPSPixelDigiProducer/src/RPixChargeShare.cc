  ...
  for (std::vector<RPixSignalPoint>::const_iterator i = charge_map.begin(); i != charge_map.end(); ++i) {
    ...
    if (((*i).X() + 16.6 / 2) < 0 || ((*i).X() + 16.6 / 2) > 16.6) {
      edm::LogInfo("RPixChargeShare")
          << "**** Attention ((*i).X()+simX_width_/2.)<0||((*i).X()+simX_width_/2.)>simX_width  ";
      edm::LogInfo("RPixChargeShare") << "(*i).X() = " << (*i).X();
      continue;
    }
    if (((*i).Y() + 24.4 / 2.) < 0 || ((*i).Y() + 24.4 / 2.) > 24.4) {
      edm::LogInfo("RPixChargeShare")
          << "**** Attention ((*i).Y()+simY_width_/2.)<0||((*i).Y()+simY_width_/2.)>simY_width  ";
      edm::LogInfo("RPixChargeShare") << "(*i).Y() = " << (*i).Y();
      continue;
    }

    CTPPSPixelSimTopology::PixelInfo relevant_pixels =
        theRPixDetTopology_.getPixelsInvolved((*i).X(), (*i).Y(), (*i).Sigma(), hit_pos_x, hit_pos_y);
