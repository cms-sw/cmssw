...
//
/**\class PPSSimTrackProducer PPSSimTrackProducer.cc
 SimPPS/PPSSimTrackProducer/plugins/PPSSimTrackProducer.cc
...
PPSSimTrackProducer::PPSSimTrackProducer(const edm::ParameterSet &iConfig) {
  ...
  if (m_transportMethod == "Totem") {
    ...
    throw cms::Exception("Configuration")
        << "LHCTransport (ProtonTransport) requires a Method (Hector or Totem) "
           "\n"
           "which is not present in the configuration file. You should add one "
           "of the method\n"
           "above in the configuration file or remove the module that requires "
           "it.";
  }
  ...
  if (!rng.isAvailable()) {
    throw cms::Exception("Configuration")
        << "LHCTransport (ProtonTransport) requires the "
...
void PPSSimTrackProducer::produce(edm::Event &iEvent,
  ...
  unique_ptr<LHCTransportLinkContainer> NewCorrespondenceMap(
      new edm::LHCTransportLinkContainer());
  edm::LHCTransportLinkContainer thisLink(
      theTransporter->getCorrespondenceMap());
  ...
  // There is no need to delete the pointer to the event, since it is deleted in
  // HepMCProduct, in fact, it MUST NOT be delete here, as a protection is
  // missing in above package
}
// The methods below are pure virtual, so it needs to be implemented even if not
// used
//
...
// module  ------------
void PPSSimTrackProducer::fillDescriptions(
