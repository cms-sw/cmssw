...
void RPDigiProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (simhit_map_iterator it = simHitMap_.begin(); it != simHitMap_.end(); ++it) {
    ...
    (theAlgoMap.find(it->first)->second)->run(simHitMap_[it->first], input_links, digi_collector.data,
        output_digi_links);
...
void
RPDigiProducer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  //RPSiDetDigitizer
  //all distances in [mm]
  ...
  desc.add<double>("RPInterStripCoupling", 1.0); //fraction of charge going to the strip, the missing part is taken by its neighbours
