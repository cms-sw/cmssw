      ...
      for (auto hit : hits) {
        ...
        if ((id.det() == DetId::HGCalEE) || (id.det() == DetId::HGCalHSi)) {
          ...
          if (((id.det() == DetId::HGCalEE) && (nameSense_ == "HGCalEESensitive")) ||
            ...
            std::string_view pid =
	      ((hgc.cassetteShiftSilicon(hid.zside(), hid.layer(), hid.waferU(), hid.waferV())) ? "HGCSim" : "HGCalSim");
          ...
          if ((id.det() == DetId::HGCalHSc) && (nameSense_ == "HGCalHEScintillatorSensitive")) {
            std::string_view pid = ((hgc.cassetteShiftScintillator(hid.zside(), hid.layer(), hid.iphi())) ? "HGCSim" : "HGCalSim");
            GlobalPoint pos = geom->getPosition(id);
            bool valid1 = geom->topology().valid(id);
