  ...
  if (getHits) {
    ...
    if (!hits.empty()) {
	  ...
	  if (((id.det() == DetId::HGCalEE) && (nameSense_ == "HGCalEESensitive")) ||
	    ...
	    std::string_view pid = ((hgc.cassetteShiftSilicon(hid.layer(), hid.waferU(), hid.waferV())) ? "HGCSim" : "HGCalSim");
	    const auto& info = hgc.waferInfo(hid.layer(), hid.waferU(), hid.waferV());
	    if (!valid)
	      st1 << " Wafer Type:Part:Orient:Cassette " << info.type << ":" << info.part << ":" << info.orient << ":" << info.cassette;
	    bool toCheck(false);
	    if (!wafers_.empty()) {
		...
		pid = "HGCalError";
	      edm::LogVerbatim(pid) << "Hit[" << all << ":" << allSi << ":" << good << "]" << hid << " Wafer Type:Part:Orient:Cassette " << info.type << ":" << info.part << ":" << info.orient << ":" << info.cassette << " at (" << pos.x() << ":"<< xx << ":" << dx << ", " << pos.y() << ":" << xy.second << ":" << dy << ", " << pos.z() << ") Valid " << valid1 << ":" << valid2 << " Distance " << diff;
	    }
	  }
	  ...
	  if ((id.det() == DetId::HGCalHSc) && (nameSense_ == "HGCalHEScintillatorSensitive")) {
	    ...
	    edm::LogVerbatim(pid) << "Hit[" << all << ":" << allSc << "] " << hid << " Cassette " << cassette << " at (" << pos.x() << ", " << pos.y() << ", " << pos.z() << ") Valid " << valid1 << ":" << valid2;
	  }
	} else {
