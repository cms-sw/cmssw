...
HGCalMouseBiteTester::HGCalMouseBiteTester(const edm::ParameterSet& iC)
  ...
  edm::LogVerbatim("HGCalGeom") << "Test Guard_Ring for wafer in layer" << layer_ << " U " << waferU_
                                << " V " << waferV_ << " with " << nTrials_ << " trials";
...
void HGCalMouseBiteTester::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  HGCMouseBite mouseBite_(hgcons_, angle_, mouseBiteCut_, 1);
  ...
  edm::LogVerbatim("HGCalGeom") << "Execution time for " << nTrials_ << " events = " << std::chrono::duration<double, std::milli>(diff_t).count() << " ms";
}
