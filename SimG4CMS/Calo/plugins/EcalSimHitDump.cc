...
EcalSimHitDump::EcalSimHitDump(const edm::ParameterSet& ps) :
  ...
  toksCalo_{
  edm::vector_transform(hitLab_,
			[this](const std::string& name) {
			  return consumes<edm::PCaloHitContainer>(
								  edm::InputTag{g4Label_, name});
			    })},
  types_(ps.getParameter<std::vector<int>>("CollectionTypes")),
  maxEvent_(ps.getParameter<int>("MaxEvent")),
  kount_(0) {

    edm::LogVerbatim("HitStudy") << "Module Label: " << g4Label_ << "   with " << hitLab_.size() << " collections and maxEvent = " << maxEvent_;
