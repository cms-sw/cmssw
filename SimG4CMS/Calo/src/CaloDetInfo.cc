...
CaloDetInfo::CaloDetInfo(unsigned int id, std::string name, G4ThreeVector pos, std::vector<double> par) : id_(id), name_(name), pos_(pos), par_(par) {}
...
bool CaloDetInfo::operator<(const CaloDetInfo& info) const {
  return (id_ < info.id()) ? false : true;
}

std::ostream& operator<<(std::ostream& os, const CaloDetInfo& info) {
  os << info.name() << " Id 0x" << std::hex << info.id() << std::dec << " Position " << std::setprecision(4) << info.pos() << " with " << info.par().size() << " parameters:";
#ifdef EDM_ML_DEBUG
