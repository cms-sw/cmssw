...
bool HGCGuardRingPartial::exclude(G4ThreeVector& point, int zside, int frontBack, int layer, int waferU, int waferV) {
  ...
  if ((modeUV_ == HGCalGeometryMode::Hexagon8Cassette) || (modeUV_ == HGCalGeometryMode::Hexagon8CalibCell)) {
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCSim") << "HGCGuardRingPatial:: Layer " << layer << " wafer " << waferU << ":" << waferV << " index " << index << " partial " << partial << " type "<< type;
#endif
    if (partial == HGCalTypes::WaferFull) {
      ...
      double dy = point.y();
#ifdef EDM_ML_DEBUG
      edm::LogVerbatim("HGCSim") << "HGCGuardRingPatial:: orient " << orient << " placement " << placement << " dx " << dx << " dy " << dy;
#endif
      if (type > 0) {
