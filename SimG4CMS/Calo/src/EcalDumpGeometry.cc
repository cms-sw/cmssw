...
EcalDumpGeometry::EcalDumpGeometry(const std::vector<std::string_view>& names, int type) : type_(type) {
  ...
  edm::LogVerbatim("EcalGeom") << "EcalDumpGeometry:: dump geometry information for detector of type " << type_ << " with " << names_.size() << " elements:";
...
#endif
	uint32_t id = ((type_ == 0) ? ebNumbering_.getUnitID(theBaseNumber) : ((type_ == 1) ? eeNumbering_.getUnitID(theBaseNumber) : esNumbering_.getUnitID(theBaseNumber)));
