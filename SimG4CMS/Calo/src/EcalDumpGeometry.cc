...
EcalDumpGeometry::EcalDumpGeometry(const std::vector<std::string_view>& names, const std::string& name1, const std::string& name2, int type) : name1_(name1), name2_(name2), type_(type) {
  std::stringstream ss;
  for (const auto& lvname : names)
    ss << " " << lvname;
  edm::LogVerbatim("EcalGeom") << " Type: " << type << " Depth Names " << name1_ << ":" << name2_ << " with " << names.size() << " LVs: " << ss.str();
...
void EcalDumpGeometry::dumpTouch(G4VPhysicalVolume* pv, unsigned int leafDepth) {
  ...
  for (unsigned int k = 0; k < names_.size(); ++k) {
	...
	uint32_t depth(0);
	if ((name1_ != "") && (namex == name1_)) depth = 1;
	if ((name2_ != "") && (namex == name2_)) depth = 2;
	double r = globalpoint.rho();
#ifdef EDM_ML_DEBUG
        edm::LogVerbatim("EcalGeom") << " Field: " << ss.str() << " ID " << std::hex << id << std::dec << ":" << depth << ":" << r;
#endif
