...
uint32_t HGCalNumberingScheme::getUnitID(int layer, int module, int cell, int iz, const G4ThreeVector& pos, double& wt) {
  ...
  if (hgcons_.waferHexagon8()) {
	...
	if (type != waferType)
#ifdef EDM_ML_DEBUG
	  edm::LogVerbatim("HGCSim") << "HGCalNumberingScheme:: " << name_ << " Layer|u|v|Index|module|cell " << layer << ":" << waferU << ":" << waferV << ":" << HGCalWaferIndex::waferIndex(layer, waferU, waferV, false) << ":" << module << ":" << cell << " has a type mismatch " << waferType << ":" << type;
#endif
