  ...
  if (isEM || other) {
    ...
    if (edep > 0) {
      if ((showerLibrary_.get() || gflash_.get()) && isKilled && pin > edMin_ && (!other)) {
...
#endif
              if (r2 < weight) {
                double time = (equalizeTimeShift_) ? 0 : fibre_->tShift(localPoint, depth, 0);
        ...
        double time = (equalizeTimeShift_) ? 0 : fibre_->tShift(localPoint, 1, 0);  // remaining part
        bool ok = true;
        if (applyFidCut_) {  // @@ For showerlibrary no z-cut for Short (no z)
...
#endif
          if (zz >= gpar_[0]) {
            time = (equalizeTimeShift_) ? 0 : fibre_->tShift(localPoint, 2, 0);
            hit.depth = 2;
            hit.time = tSlice + time;
            hits.push_back(hit);
#ifdef EDM_ML_DEBUG
