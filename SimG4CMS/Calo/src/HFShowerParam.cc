...
HFShowerParam::HFShowerParam(std::string & name, const DDCompactView & cpv,
  ...
  if (dodet) {
    ...
    edm::LogError("HFShower") << "HFShowerParam: cannot get filtered "
			      ...
			      << name;
    throw cms::Exception("Unknown", "HFShowerParam") << "cannot match " 
						     << attribute << " to " 
						     << name <<"\n";
  }
      ...
      if ((showerLibrary || gflash) && kill && pin > edMin && (!other)) {
	    ...
	    if (applyFidCut) {// @@ For showerlibrary no z-cut for Short (no z) 
	      ...
	      edm::LogInfo("HFShower") << "HFShowerParam::getHits:#PMT= "
              ...
              if (npmt <= 0) {
#ifdef DebugLog
		edm::LogInfo("HFShower") << "-#PMT=0 cut-HFShowerParam::getHits: npmt = " << npmt;
#endif
                ok = false;
...
#ifdef DebugLog
                  edm::LogInfo("HFShower") << "-SHORT cut-HFShowerParam::getHits:zMin=" << gpar[0];
#endif
                  ok = false;
                }
              }
