...
HFShowerParam::HFShowerParam(const std::string& name, const DDCompactView& cpv, edm::ParameterSet const& p)
    : showerLibrary(nullptr), fibre(nullptr), gflash(nullptr), fillHisto(false) {
  ...
  edm::LogVerbatim("HFShower") << "HFShowerParam::Use of shower library is set to " << useShowerLibrary
                               << " Use of Gflash is set to " << useGflash << " P.E. per GeV " << pePerGeV
                               << ", ref. index of fibre " << ref_index << ", Track EM Flag " << trackEM << ", edMin "
                               ...
                               << ", Mean lambda " << lambdaMean << ", aperture (cutoff) " << aperture
  ...
  if (tfile.isAvailable()) {
    ...
    em_long_1_tuned = showerDir.make<TH1F>(
  ...
  if (useShowerLibrary)
    showerLibrary = new HFShowerLibrary(name, cpv, p);
  if (useGflash)
    gflash = new HFGflash(p);
  fibre = new HFFibre(name, cpv, p);
  attLMeanInv = fibre->attLength(lambdaMean);
  edm::LogVerbatim("HFShower") << "att. length used for (lambda=" << lambdaMean << ") = " << 1 / (attLMeanInv * cm)
                               << " cm";
}
...
void HFShowerParam::initRun(const HcalDDDSimConstants* hcons) {
  if (showerLibrary)
    showerLibrary->initRun(nullptr, hcons);
  if (fibre)
    fibre->initRun(hcons);
}
...
std::vector<HFShowerParam::Hit> HFShowerParam::getHits(const G4Step* aStep, double weight, bool& isKilled) {
  ...
  edm::LogVerbatim("HFShower") << "HFShowerParam: getHits " << track->GetDefinition()->GetParticleName()
                               ...
                               << (localPoint.z() + 0.5 * gpar[1]) << ") Local " << localPoint;
#endif
  ...
  if (isEM || other) {
    ...
    if ((!trackEM) && ((zz < (gpar[1] - gpar[2])) || parametrizeLast) && (!other)) {
      edep = pin;
      isKilled = true;
    } else if ((track->GetDefinition()->GetPDGCharge() != 0) && (pBeta > (1 / ref_index)) && (dirz > aperture)) {
        ...
        if (showerLibrary) {
          ...
          for (unsigned int i = 0; i < hitSL.size(); ++i) {
...
#ifdef EDM_ML_DEBUG
                  edm::LogVerbatim("HFShower") << "-SHORT cut-HFShowerParam::getHits:zMin=" << gpar[0];
#endif
                  ok = false;
                }
              }
#ifdef EDM_ML_DEBUG
              edm::LogVerbatim("HFShower")
                  << "HFShowerParam: npmt " << npmt << " zv " << std::abs(pe_effect.z()) << ":" << gpar[4] << ":" << zv
                  << ":" << gpar[0] << " ok " << ok << " depth " << depth;
#endif
            } else {
              if (G4UniformRand() > 0.5)
                depth = 2;
              if (depth == 2 && zv < gpar[0])
                ok = false;
            }
            //attenuation
            ...
            edm::LogVerbatim("HFShower") << "HFShowerParam:Distance to PMT (" << npmt << ") " << dist
                                         << ", exclusion flag " << (r1 > exp(-attLMeanInv * zv));
#endif
            if (r1 > exp(-attLMeanInv * dist))
              ok = false;
            if (ok) {
