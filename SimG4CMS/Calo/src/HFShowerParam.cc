...
HFShowerParam::HFShowerParam(std::string & name, const DDCompactView & cpv,
  ...
  : showerLibrary(0), fibre(0), gflash(0), fillHisto(false)
{
...
std::vector<HFShowerParam::Hit> HFShowerParam::getHits(G4Step * aStep)
{
      ...
      particleCode == gammaPDG || other)
  {
    ...
    else if (track->GetDefinition()->GetPDGCharge() != 0 && pBeta > (1/ref_index))
    {
      ...
      if ((showerLibrary || gflash) && kill && pin > edMin && (!other))
      {
        if (showerLibrary)
        {
          std::vector<HFShowerLibrary::Hit>
                                      hitSL = showerLibrary->getHits(aStep,kill, onlyLong);
...
#endif
#ifdef mkdebug
            applyFidCut=true; // @@ temporary
#endif
		if (applyFidCut) // @@ For showerlibrary no z-cut for Short (no z) M.K. ?!
            {
        ...
        else // GFlash clusters with known z
        {
          ...
          {
            bool ok = true;
            G4ThreeVector pe_effect(hitSL[i].position.x(),
                                    hitSL[i].position.y(),
                                    hitSL[i].position.z());
            ...
            if (applyFidCut)
            {
...
#ifdef mkdebug
              std::cout<<"HFShowerParam::getHits:#PMT="<<npmt<<",z="<<zv<<std::endl;
#endif
              if (npmt <= 0)
              {
#ifdef mkdebug
                std::cout<<"-#PMT=0 cut-HFShowerParam::getHits: npmt="<<npmt<<std::endl;
#endif
                ok = false;
              }
              else if (npmt > 24)
              { // a short fibre
                ...
                {
#ifdef mkdebug
                  std::cout<<"-SHORT cut-HFShowerParam::getHits:zMin="<<gpar[0]<<std::endl;
#endif
                  ok = false;
                }
              }
