...
#ifdef EDM_ML_DEBUG
  if (isXtal(lv))
    edm::LogVerbatim("EcalSimX") << "ECalSD::Volume " << lv->GetName() << " DetId " << std::hex << setDetUnitId(aStep) << std::dec << " Global " << (hitPoint->GetPosition()).rho() << ":" << (hitPoint->GetPosition()).z() << " Local Z " << currentLocalPoint.z() << " Depth " << crystalDepth;
...
bool ECalSD::isXtal(const G4LogicalVolume * lv) {
  return (xtalLMap.find(lv) != xtalLMap.end());
}
