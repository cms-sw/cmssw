...
void makeHitStudyPlots(std::string file1 = "analRun3Old.root",
  ...
  std::cout << "Use " << nfile << " files from " << file1 << " and " << file2 <<" and look for " << todoMin << ":" << todoMax << std::endl;
  for (int i0 = todoMin; i0 <= todoMax; ++i0) {
	...
	if (nfile == 2) {
	  ...
	  if (sumNum == 0)
	    sumDen = 0;
	  std::cout << tag1 << " vs " << tag2 << " " << hist0[0]->GetXaxis()->GetTitle() << " Mean deviation " << sumNum << " +- " << sumDen << " maximum " << maxDev << std::endl;
	}
      }
    }
  }
