...
void makeGVPlots(std::string fname = "analG4.root",
                 bool ifG4 = true,
                 int todomin = 0,
                 int todomax = 3,
                 std::string tag = "",
    ...
    for (int i1 = 0; i1 < 4; ++i1) {
      for (int i2 = todomin; i2 <= todomax; ++i2) {
        if (types[i2] == 1) {
          ...
          if (ifG4)
            sprintf(title, "%s (Geant4 Simulation)", text.c_str());
          else
            sprintf(title, "%s (GeantV Simulation)", text.c_str());
        } else {
          continue;
        }
        TH1D *hist1(nullptr);
        ...
        if ((hist1 != nullptr) || (hist2 != nullptr)) {
          if (ifG4)
            sprintf(cname, "%sG4%s", name1, tag.c_str());
          else
            sprintf(cname, "%sGV%s", name1, tag.c_str());
...
void makeGV2Plots(std::string fnmG4 = "analG4.root",
  ...
  std::string title2[2] = {"#eta = 1.0; #phi = 1.0", "|#eta| < 3.0; 0.0 < #phi < 2#pi"};
  std::string title3[4] = {
      "B = 0.0 Tesla", "B = 3.8 Tesla", "Multithreaded (B = 0.0 Tesla)", "Multithreaded (B = 3.8 Tesla)"};
  ...
  if (file1 && file2) {
    ...
    for (int i1 = 0; i1 < 4; ++i1) {
      for (int i2 = todomin; i2 <= todomax; ++i2) {
        sprintf(name, "%s%d", names[i2].c_str(), dets[i1]);
        sprintf(cname, "%s%s%s", namex[i2].c_str(), detName[i1].c_str(), tag.c_str());
        if (modet >= 10)
          sprintf(title, "");
        else if (mode == 1)
        ...
        if ((hist[0] != nullptr) && (hist[1] != nullptr)) {
          ...
          for (int i = 0; i < 2; ++i) {
            ...
            if (i == 0) {
              if (normalize)
                hist[i]->DrawNormalized("hist");
              else
                hist[i]->Draw();
            } else {
              if (normalize)
                hist[i]->DrawNormalized("sames hist");
              else
                hist[i]->Draw("sames");
            }
            pad->Update();
          ...
          for (int i = 0; i < 2; ++i) {
            ...
            if (st != NULL) {
              st->SetLineColor(icol[i]);
              st->SetTextColor(icol[i]);
              st->SetY1NDC(ymax - 0.15);
              st->SetY2NDC(ymax);
              st->SetX1NDC(0.65);
              st->SetX2NDC(0.90);
              ymax -= 0.15;
          ...
          if (normalize) {
            ...
            for (int i = 0; i < nbin; ++i) {
              if (hist[0]->GetBinContent(i + 1) > 0 && hist[1]->GetBinContent(i + 1) > 0) {
                ypt[npt] = (fac * hist[0]->GetBinContent(i + 1) / hist[1]->GetBinContent(i + 1));
            ...
            TGraphAsymmErrors *graph = new TGraphAsymmErrors(npt, xpt, ypt, dxp, dxp, dyp, dyp);
            ...
            if (save != 0) {
              if (save > 0)
                sprintf(name, "c_%s.pdf", canvas->GetName());
              else
...
void makeGVSPlots(std::string fnmG4 = "analG4.root",
                  ...
                  bool save = false,
                  std::string dirnm = "caloSimHitAnalysis") {
  std::string names[8] = {"hitp", "trackp", "stepp", "edepp", "timep", "volp", "edept", "timet"};
  std::string xtitle[8] = {"Hits",
                           "Tracks",
                           "Step Length (cm)",
                           "Energy Deposit (MeV)",
                           "Time (ns)",
                           ...
                           "Energy Deposit (MeV)",
                           "Time (ns)"};
  std::string ytitle[8] = {"Events", "Events", "Hits", "Hits", "Hits", "Events", "Hits", "Hits"};
  std::string detName[6] = {"Barrel Pixel", "Forward Pixel", "TIB", "TID", "TOB", "TEC"};
    ...
    for (int i2 = todomin; i2 <= todomax; ++i2) {
      ...
      for (int i1 = 0; i1 < nhism; ++i1) {
        if (nhis[i2] <= 1 && i1 == 0) {
          ...
          sprintf(title, "%s in %s (Geant4 vs GeantV)", particles[i1 - 1].c_str(), text.c_str());
        } else {
          sprintf(name, "%s%d", names[i2].c_str(), i1);
          sprintf(cname, "%s%d%s", names[i2].c_str(), i1, tag.c_str());
          sprintf(title, "%s in %s (Geant4 vs GeantV)", text.c_str(), detName[i1].c_str());
        }
        TH1D *hist[2];
        hist[0] = (TH1D *)dir1->FindObjectAny(name);
        hist[1] = (TH1D *)dir2->FindObjectAny(name);
