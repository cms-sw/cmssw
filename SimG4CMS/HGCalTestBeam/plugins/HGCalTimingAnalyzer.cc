...
#ifdef EDM_ML_DEBUG
  std::cout << "HGCalTimingAnalyzer:: Detector " << detectorEE_ << " with tags "
            << tmp1 << std::endl;
#endif
...
void HGCalTimingAnalyzer::fillDescriptions(
...
void HGCalTimingAnalyzer::beginRun(const edm::Run&,
  ...
  for (const auto& hit : hits) {
    ...
    if (type == 0) {
      ...
      std::pair<int, int> recoLayerCell =
          hgcons_->simToReco(cell, layer, sector, true);
      id = HGCalDetId((ForwardSubdetector)(subdet), zside, recoLayerCell.second,
                      subsector, sector, recoLayerCell.first)
               .rawId();
#ifdef EDM_ML_DEBUG
      ...
      std::cout << "SimHit:Hit[" << i << "] Beam Subdet " << subdet << " Layer "
                << layer << " x|y " << x << ":" << y << " Energy " << energy
                << " Time " << time << std::endl;
#endif
    }
...
void HGCalTimingAnalyzer::analyzeSimTracks(
...
#ifdef EDM_ML_DEBUG
    std::cout << "Vertex " << vertIndex << " position " << simVtxItr->position()
              << std::endl;
#endif
