...
// user include files
#include "SimG4CMS/HcalTestBeam/interface/HcalTB06Analysis2015.h"
...
HcalTB06Analysis2015::HcalTB06Analysis2015(const edm::ParameterSet &p)
    ...
    m_HcalTag(edm::InputTag("g4SimHits","HcalHits"))
{
  ...
  edm::LogInfo("HcalTB06Analysis2015") 
    << "Beam parameters: E(GeV)= " << m_ener
    << " pdgID= " << m_PDG[0]
    << "  eta= " << m_eta
    << "  phi= " << m_phi
    << "\n        EcalFactor= " << m_factEcal
    << "  EcalWidth= " << m_widthEcal/CLHEP::GeV << " GeV" 
    << "\n        HcalFactor= " << m_factHcal;
  m_histo = new HcalTB06Histo(ptb);
} 
   
HcalTB06Analysis2015::~HcalTB06Analysis2015() 
{
  delete m_histo;
}

void HcalTB06Analysis2015::beginJob() 
{
  edm::LogInfo("HcalTB06Analysis2015") <<" =====> Begin of Run";
}

void HcalTB06Analysis2015::endJob()
{
  edm::LogInfo("HcalTB06Analysis2015") 
    << " =====> End of Run; Total number of events: " << count;
}

void HcalTB06Analysis2015::analyze(const edm::Event & evt, const edm::EventSetup&)
{
  ++count;
  ...
  LogDebug("HcalTBSim") << "HcalTB06Analysis2015:: Etot(MeV)= " << etots 
			<< "   E(Ecal)= " << eecals 
			<< "   E(Hcal)= " << ehcals
			<< "  Nhits(ECAL)= " << ne 
			<< "  Nhits(HCAL)= " << nh;
