...
MuonSimHitDump::MuonSimHitDump(const edm::ParameterSet& ps) :
  ...
  edm::vector_transform(hitLab_,
			[this](const std::string& name) {
			  return consumes<edm::PSimHitContainer>(
...
void MuonSimHitDump::analyze(const edm::Event& e, const edm::EventSetup&) {
  ...
  if ((kount_ <= maxEvent_) || (maxEvent_ <= 0)) {
    for (unsigned int k = 0; k < toksMuon_.size(); ++k) {
      ...
      if (hitsMuon.isValid())
	edm::LogVerbatim("HitStudy") << "MuonSimHitDump: Input " << hitsMuon->size() << " hits of type " << types_[k] << " (" << dets[k] << ")";
      unsigned int i(0);
	...
	if (types_[k] == 0)
	  edm::LogVerbatim("HitStudy") << "[" << i << "] " << DTWireId(id) << " Trk " << track << " p " << p << " dE " << edep << " T " << tof << " Enetry " << entry << " Exit " << exit;
	else if (types_[k] == 1)
	  edm::LogVerbatim("HitStudy") << "[" << i << "] " << CSCDetId(id) << " Trk " << track << " p " << p << " dE " << edep << " T " << tof << " Enetry " << entry << " Exit " << exit;
	else if (types_[k] == 2)
	  edm::LogVerbatim("HitStudy") << "[" << i << "] " << RPCDetId(id) << " Trk " << track << " p " << p << " dE " << edep << " T " << tof << " Enetry " << entry << " Exit " << exit;
	else if (types_[k] == 3)
	  edm::LogVerbatim("HitStudy") << "[" << i << "] " << GEMDetId(id) << " Trk " << track << " p " << p << " dE " << edep << " T " << tof << " Enetry " << entry << " Exit " << exit;
	else 
	  edm::LogVerbatim("HitStudy") << "[" << i << "] " << ME0DetId(id) << " Trk " << track << " p " << p << " dE " << edep << " T " << tof << " Enetry " << entry << " Exit " << exit;
	++i;
      }
    }
  }
