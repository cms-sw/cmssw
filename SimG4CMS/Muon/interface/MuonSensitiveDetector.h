...
private:
  ...
  /**
   * Transform from local coordinates of a volume to local coordinates of a
   * parent volume one or more levels up the volume hierarchy: e.g. levelsUp = 1
   * for immediate parent. This is done by moving from local_1 -> global ->
   * local_2.
   */
  Local3DPoint InitialStepPositionVsParent(const G4Step *currentStep,
                                           G4int levelsUp);
  Local3DPoint FinalStepPositionVsParent(const G4Step *currentStep,
                                         G4int levelsUp);
