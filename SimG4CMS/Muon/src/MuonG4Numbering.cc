...
MuonG4Numbering::MuonG4Numbering(const MuonDDDConstants &muonConstants) {
  ...
  if (theBasePart != 1) {
    std::cout << "MuonDDDNumbering finds unusual base constant:" << theBasePart
              << std::endl;
  }
  if (theSuperPart < 100) {
...
MuonBaseNumber
MuonG4Numbering::PhysicalVolumeToBaseNumber(const G4Step *aStep) {
  ...
  for (int ii = 0; ii < touch->GetHistoryDepth(); ii++) {
    ...
    std::cout << "MuonG4Numbering: " << vol->GetName() << " " << copyno
              << std::endl
              << "Split " << copyNoRelevant(copyno) << ":" << theLevelPart
              << ":" << theSuperPart << " ";
#endif
    if (copyNoRelevant(copyno)) {
      num.addBase(getCopyNoLevel(copyno), getCopyNoSuperNo(copyno),
