...
MuonSensitiveDetector::MuonSensitiveDetector(
    const std::string &name, const DDCompactView &cpv,
    const SensitiveDetectorCatalog &clg, edm::ParameterSet const &p,
    const SimTrackManager *manager)
    : SensitiveTkDetector(name, cpv, clg, p), thePV(nullptr), theHit(nullptr),
      theDetUnitId(0), newDetUnitId(0), theTrackID(0), theManager(manager) {
  edm::ParameterSet m_MuonSD = p.getParameter<edm::ParameterSet>("MuonSD");
  ePersistentCutGeV =
      m_MuonSD.getParameter<double>("EnergyThresholdForPersistency") /
      CLHEP::GeV; // Default 1. GeV
...
uint32_t MuonSensitiveDetector::setDetUnitId(const G4Step *aStep) {
  ...
  LogDebug("MuonSimDebug") << "MuonSensitiveDetector::setDetUnitId :: "
                           << MuonBaseNumbr;
  LogDebug("MuonSimDebug")
...
void MuonSensitiveDetector::createHit(const G4Step *aStep) {
  ...
  if (detector->isBarrel()) {
    // 1 levels up
    theEntryPoint = cmsUnits(theRotation->transformPoint(
        InitialStepPositionVsParent(aStep, 1), aStep));
    theExitPoint = cmsUnits(theRotation->transformPoint(
        FinalStepPositionVsParent(aStep, 1), aStep));
  } else if (detector->isEndcap()) {
    // save local z at current level
    theEntryPoint = theRotation->transformPoint(
        InitialStepPosition(aStep, LocalCoordinates), aStep);
    theExitPoint = theRotation->transformPoint(
        FinalStepPosition(aStep, LocalCoordinates), aStep);
    float zentry = theEntryPoint.z();
    float zexit = theExitPoint.z();
    // 4 levels up
    Local3DPoint tempEntry = theRotation->transformPoint(
        InitialStepPositionVsParent(aStep, 4), aStep);
    Local3DPoint tempExit =
        theRotation->transformPoint(FinalStepPositionVsParent(aStep, 4), aStep);
    // reset local z from z wrt deep-parent volume to z wrt low-level volume
    theEntryPoint =
        cmsUnits(Local3DPoint(tempEntry.x(), tempEntry.y(), zentry));
  ...
  theHit = new UpdatablePSimHit(
      theEntryPoint, theExitPoint, thePabs, theTof, theEnergyLoss,
      theParticleType, theDetUnitId, theTrackID, theThetaAtEntry, thePhiAtEntry,
...
#ifdef DebugLog
  edm::LogVerbatim("MuonSimDebug")
      << "=== NEW Muon hit for " << GetName() << " Edep(GeV)= " << theEnergyLoss
  ...
  edm::LogVerbatim("MuonSimDebug")
      << " theta= " << theThetaAtEntry << " phi= " << thePhiAtEntry
      << " Pabs(GeV/c)  " << thePabs << " Eloss(GeV)= " << theEnergyLoss
      << " Tof(ns)=  " << theTof << " trackID= " << theTrackID
      << " detID= " << theDetUnitId << "\n Local:  entry " << theEntryPoint
      << " exit " << theExitPoint << " delta " << (theExitPoint - theEntryPoint)
      << "\n Global: entry " << preStepPoint->GetPosition() << " exit "
...
void MuonSensitiveDetector::updateHit(const G4Step *aStep) {
  ...
  if (detector->isBarrel()) {
    ...
    Local3DPoint tempExitPoint =
        theRotation->transformPoint(FinalStepPositionVsParent(aStep, 4), aStep);
    theExitPoint =
        cmsUnits(Local3DPoint(tempExitPoint.x(), tempExitPoint.y(), zexit));
  } else {
    theExitPoint = cmsUnits(theRotation->transformPoint(
        FinalStepPosition(aStep, LocalCoordinates), aStep));
  }
  ...
  if ("" != sss)
    edm::LogVerbatim("MuonSimDebug") << sss;
  edm::LogVerbatim("MuonSimDebug")
      << " delEloss(GeV)= " << theEnergyLoss << " Tof(ns)=  " << theTof
...
Local3DPoint
MuonSensitiveDetector::InitialStepPositionVsParent(const G4Step *currentStep,
                                                   G4int levelsUp) {
  ...
  G4ThreeVector localCoordinates = theTouchable->GetHistory()
                                       ->GetTransform(depth - levelsUp)
                                       .TransformPoint(globalCoordinates);

  return ConvertToLocal3DPoint(localCoordinates);
}

Local3DPoint
MuonSensitiveDetector::FinalStepPositionVsParent(const G4Step *currentStep,
                                                 G4int levelsUp) {
