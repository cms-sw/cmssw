...
void TotemTestGem::update(const EndOfEvent* evt) {
  ...
  for (const auto& name : names) {
    int HCid = G4SDManager::GetSDMpointer()->GetCollectionID(name);
    TotemG4HitCollection* theHC = (TotemG4HitCollection*)allHC->GetHC(HCid);
    LogDebug("ForwardSim") << "TotemTestGem :: Hit Collection for " << name << " of ID " << HCid << " is obtained at "
                           << theHC;

    if (HCid >= 0 && theHC != nullptr) {
      int nentries = theHC->entries();
      LogDebug("ForwardSim") << "TotemTestGem :: " << name << " with " << nentries << " entries";
