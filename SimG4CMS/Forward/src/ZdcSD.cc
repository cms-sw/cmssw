...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("ZdcSD") << "ZdcSD::" << GetName() << " ID= " << aStep->GetTrack()->GetTrackID() << " prID= " << aStep->GetTrack()->GetParentID() << " Eprestep= " << aStep->GetPreStepPoint()->GetKineticEnergy() << " step= " << aStep->GetStepLength() << " Edep= " << aStep->GetTotalEnergyDeposit();
#endif
    if (useShowerHits) {
      // check unitID
...
#ifdef EDM_ML_DEBUG
      edm::LogVerbatim("ZdcSD") << "pPR = (" << particleDirection.x() << "," << particleDirection.y() << "," << particleDirection.z() << ")";
...
#ifdef EDM_ML_DEBUG
      edm::LogVerbatim("ZdcSD") << "pLAB = (" << photonMomentum.x() << "," << photonMomentum.y() << "," << photonMomentum.z() << ")";
#endif
...
#ifdef EDM_ML_DEBUG
      edm::LogVerbatim("ZdcSD") << "r = (" << photonPosition.x() << "," << photonPosition.y() << "," << photonPosition.z() << ")" << std::endl;
#endif
    }
...
double ZdcSD::photonEnergyDist(int charge, double beta, double E){
  const std::vector<double> ENERGY_TAB{1.75715,1.81902,1.88311,1.94944,
      2.0183,2.08939,2.16302,2.23919,
      2.31789,2.39954,2.48416,2.57175,
      2.66232,2.75643,2.85349,2.95411,
      3.05756,3.16528,3.2774,3.39218,
      3.5123,3.6359,3.76394,3.89642,
      4.03332,4.17596,4.32302,4.47596,
      4.63319,4.79629
      };
  
  const std::vector<double> RINDEX_TAB{  1.45517,1.45572,1.45631,1.45693,
      1.45758,1.45826,1.45899,1.45976,
      1.46057,1.46144,1.46238,1.46337,
      1.46444,1.46558,1.4668,1.46812,
      1.46953,1.47105,1.4727,1.47447,
      1.4764,1.47847,1.48071,1.48315,
      1.48579,1.48868,1.49182,1.49526,
      1.499,1.5031
      };
...
double ZdcSD::calculateN2InvIntegral(double Emin){
  ...
  const std::vector<double> EMIN_TAB{1.75715,1.81902,1.88311,1.94944,
      2.0183,2.08939,2.16302,2.23919,
      2.31789,2.39954,2.48416,2.57175,
      2.66232,2.75643,2.85349,2.95411,
      3.05756,3.16528,3.2774,3.39218,
  ...
  const std::vector<double> INTEGRAL_TAB{1.39756,1.36835,1.33812,1.30686,
      1.27443,1.24099,1.20638,1.17061,
      1.1337,1.09545,1.05586,1.01493,
      0.972664,0.928815,0.883664,0.836938,
      0.788988,0.739157,0.687404,0.634547,
      0.579368,0.522743,0.464256,0.40393,
      0.341808,0.27732,0.211101,0.142536,
      0.0723891
      };
...
double ZdcSD::pmtEfficiency(double lambda){
  ...
  const std::vector<double> LAMBDA_TAB{263.27,265.98,268.69,271.39,
      273.20,275.90,282.22,282.22,
      293.04,308.38,325.52,346.26,
      367.91,392.27,417.53,440.98,
      463.53,484.28,502.32,516.75,
      528.48,539.30,551.93,564.56,
      574.48,584.41,595.23,606.96,
      616.88,625.00,632.22,637.63,
      642.14,647.55,652.96,656.57,
      661.08,666.49,669.20,673.71,
      677.32,680.93,686.34,692.65
      };
  
  // Hardcoded quantum efficiency values
  const std::vector<double> EFF_TAB{2.215,2.860,3.659,4.724,
      5.989,7.734,9.806,9.806,
      12.322,15.068,17.929,20.570,
      22.963,24.050,23.847,22.798,
      20.445,18.003,15.007,12.282,
      9.869,7.858,6.373,5.121,
      4.077,3.276,2.562,2.077,
      1.669,1.305,1.030,0.805,
      0.629,0.492,0.388,0.303,
      0.239,0.187,0.144,0.113,
      0.088,0.069,0.054,0.043
      };
...
double ZdcSD::evaluateFunction(const std::vector<double> X, const std::vector<double> Y, double x){
  for (unsigned int i = 0; i < X.size()-1; i++) {
    if (x > X[i] && x < X[i+1]) {
#ifdef EDM_ML_DEBUG
      edm::LogVerbatim("ZdcSD") << X[i] << "\t" << Y[i] << "\t" << X[i+1] << "\t" << Y[i+1] << "\t" << x << "\t" << linearInterpolation(X[i],Y[i],X[i+1],Y[i+1],x);
#endif
...
int ZdcSD::setTrackID (G4Step* aStep) {
  ...
  if (primaryID == 0) {
    ...
    edm::LogVerbatim("ZdcSD") << "ZdcSD: Problem with primaryID **** set by force to TkID **** " << theTrack->GetTrackID() << " E " << etrack;
#endif
