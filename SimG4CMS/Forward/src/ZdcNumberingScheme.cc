...
unsigned int ZdcNumberingScheme::getUnitID(const G4Step* aStep) {
  ...
  if (level > 0) {
    ...
    edm::LogVerbatim("ForwardSim")<< "ZdcNumberingScheme:" 
             ...
             << level << std::endl;
    for (int ich = 0; ich < level; ich++)
      edm::LogVerbatim("ForwardSim")<< "  " << ich  << ": copyno " << copyno[ich] 
               << " name="  << name[ich]
...
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("ForwardSim")<< "ZDC packing: section " << section << " layer  " << layer << " fiber " << fiber  << " channel " <<  channel << " zside "  << z << "idx: " <<idx <<  std::endl;
...
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("ForwardSim") << "ZDC unpacking: idx:"<< idx << " -> section " << section << " layer " << layer << " fiber " << fiber << " channel " << channel << " zside " << z <<  std::endl;
#endif 
}
...
void ZdcNumberingScheme::detectorLevel(const G4Step* aStep, int& level, std::vector<int>& copyno, std::vector<G4String>& name) {
