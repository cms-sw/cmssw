...
void ZdcShowerLibrary::initRun(G4ParticleTable *theParticleTable){
  ...
  edm::LogVerbatim("ZdcShower")<< "ZdcShowerLibrary: Particle codes for e- = " << emPDG << ", e+ = " << epPDG << ", gamma = " << gammaPDG << ", pi0 = " << pi0PDG << ", eta = " << etaPDG << ", geantino = " << geantinoPDG << "\n        nu_e = " << nuePDG << ", nu_mu = " << numuPDG << ", nu_tau = " << nutauPDG << ", anti_nu_e = " << anuePDG << ", anti_nu_mu = " << anumuPDG << ", anti_nu_tau = " << anutauPDG;
}
...
int ZdcShowerLibrary::getEnergyFromLibrary(const G4ThreeVector& hitPoint, const G4ThreeVector& momDir, double energy,
  ...
  edm::LogVerbatim("ZdcShower") << "\n ZdcShowerLibrary::getEnergyFromLibrary input/output variables:" << " phi: " << 59.2956*momDir.phi() << " theta: "<< 59.2956*momDir.theta() << " xin : " << hitPoint.x() << " yin : " << hitPoint.y() << " zin : " << hitPoint.z() << " track en: " << energy << "(GeV)" << " section: "<< section << " side: " << side << " channel: "<< channel << " partID: " << parCode;
  ...
  if(eav <0. || edis <0.){
    edm::LogVerbatim("ZdcShower") << " Negative everage energy from parametrization \n" << " xin: " << xin << "(cm)" << " yin: " << yin << "(cm)" << " track en: " << energy << "(GeV)" << " eaverage: "<< eav/CLHEP::GeV << " (GeV)" << " esigma: " << esig/CLHEP::GeV << "  (GeV)" << " edist: " << edis << " (GeV)" <<" dE hit: " << nphotons/CLHEP::GeV << " (GeV)";
    return 0;
  }
  ...
  edm::LogVerbatim("ZdcShower") << " track en: " << energy << "(GeV)" << " eaverage: " << eav/CLHEP::GeV << " (GeV)" << " esigma: " << esig/CLHEP::GeV << "  (GeV)" << " edist: " << edis << " (GeV)" << " dE hit: " << nphotons/CLHEP::GeV <<  " (GeV)";
