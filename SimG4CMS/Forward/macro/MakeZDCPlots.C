...
void makeHitStudyPlots(std::string file1 = "Forward.root",
  ...
  std::cout << "Use " << nfile << " files from " << file1 << "," << file2 << " and " << file3 << " and look for " << todomin << ":" << todomax << std::endl;
  for (int todo = todomin; todo <= todomax; ++todo) {
    ...
    for (int i1 = 0; i1 < nfile; ++i1) {
      ...
      if (debug)
	std::cout << "Reads histogram " << hist0[i1]->GetName() << " for " << hist0[i1]->GetTitle() << " from " << file[i1] << std::endl;
    }
	...
	for (int j = 0; j < nbinR; ++j) {
	  ...
	  if (tden > 0 && tnum > 0) {
	    ...
	    xhigh1 =  hist0[0]->GetXaxis()->GetBinLowEdge((j+1) * rebin[todo]) + hist0[0]->GetXaxis()->GetBinWidth((j+1) * rebin[todo]);
    ...
    if (debug)
      std::cout << namec << " Canvas made for " << hist1[0]->GetName() << " with " << hist1.size() << " plots" << std::endl;
	...
	hist1[i1]->Draw("sames");
      ...
      if (debug)
	std::cout << "Drawing histograms for " << hist1[i1]->GetName() << ":" << i1 << " in canvas " << pad->GetName() << std::endl;
      pad->Update();
