...
// UserVerbosity
// FP420NumberingScheme::cout("FP420NumberingScheme","silent","FP420NumberingScheme");
...
unsigned int FP420NumberingScheme::getUnitID(const G4Step *aStep) const {
  ...
  // std::cout << "FP420NumberingScheme number of levels= " << level <<
  // std::endl;
  ...
  if (level > 0) {
    ...
    for (int ich = 0; ich < level; ich++) {
      ...
      */
      ...
      //
      //  std::cout << "FP420NumberingScheme  " << "ich=" << ich  << "copyno" <<
    ...
    // intindex = myPacker.packEcalIndex (det, zside, station, plane);// see
    // examples intindex = myPacker.packCastorIndex (det, zside, station,
    // plane);// see examples
    intindex = packFP420Index(det, zside, station, plane);
    /*
    //
    std::cout << "FP420NumberingScheme det=" << det << " zside=" << zside << "
    station=" <<station  << " plane=" << plane << std::endl;

    for (int ich = 0; ich < level; ich++) {
    std::cout <<" name = " << name[ich] <<" copy = " << copyno[ich] <<
    std::endl; std::cout << " packed index = intindex" << intindex << std::endl;
    }
    //
    */
...
unsigned FP420NumberingScheme::packFP420Index(int det, int zside, int station,
                                              int plane) {
  //
  unsigned int idx = ((det - 1) & 3)
                     << 19; // bit 19-20 (det-1):0- 3 = 4-->2**2 = 4  -> 4-1
                            // ->((det-1)&3)  2 bit: 0-1
  // unsigned int idx = ((det-1)&1)<<20;//bit 20-20 (det-1):0- 1 = 2-->2**1 = 2
  // -> 2-1  ->((det-1)&1)  1 bit: 0
  idx += (zside & 7) << 7; // bits  7- 9   zside:0- 7 = 8-->2**3 = 8  -> 8-1  ->
                           // (zside&7)  3 bits:0-2
  //  idx += (zside&3)<<7;             //bits  7- 8   zside:0- 3 = 4-->2**2 = 4
  //  -> 4-1  ->  (zside&3)  2 bits:0-1
  idx += (station & 7) << 4; // bits  4- 6 station:0- 7 = 8-->2**3 = 8  -> 8-1
                             // ->(station&7)  3 bits:0-2
  idx += (plane & 15); // bits  0- 3   plane:0-15 =16-->2**4 =16  ->16-1  ->
                       // (plane&15)  4 bits:0-3

  //

  //  std::cout << "FP420 packing: det " << det  << " zside  " << zside << "
  //  station " << station  << " plane " <<  plane << " idx " << idx <<
  //  std::endl; int newdet, newzside, newstation,newplane;
...
void FP420NumberingScheme::unpackFP420Index(const unsigned int &idx, int &det,
  ...
  //  std::cout  << " FP420unpacking: idx=" << idx << " zside  " << zside << "
  //  station " << station  << " plane " <<  plane << std::endl;
