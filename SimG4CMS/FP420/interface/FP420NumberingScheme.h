...
public:
  ...
  static unsigned int packFP420Index(int det, int zside, int station,
                                     int superplane);

  static void unpackFP420Index(const unsigned int &idx, int &det, int &zside,
                               int &station, int &superplane);
  ...
  static unsigned packMYIndex(int rn0, int pn0, int sn0, int det, int zside,
    ...
    unsigned int intindex = dScale * (det - 1) + sScale * (sector - 1) +
                            zScale * (zmodule - 1) + zside;
    //
    return intindex;
  }

  static void unpackMYIndex(const int &idx, int rn0, int pn0, int sn0, int &det,
                            int &zside, int &sector, int &zmodule) {
    int zScale = (rn0 - 1), sScale = (rn0 - 1) * (pn0 - 1),
        dScale = (rn0 - 1) * (pn0 - 1) * (sn0 - 1);

    det = (idx - 1) / dScale + 1;
    sector = (idx - 1 - dScale * (det - 1)) / sScale + 1;
    zmodule =
        (idx - 1 - dScale * (det - 1) - sScale * (sector - 1)) / zScale + 1;
    zside = idx - dScale * (det - 1) - sScale * (sector - 1) -
            zScale * (zmodule - 1);
  }
