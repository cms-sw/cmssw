...
FP420SD::FP420SD(const std::string &name, const DDCompactView &cpv,
    ...
    : SensitiveTkDetector(name, cpv, clg, p), numberingScheme(nullptr),
      hcID(-1), theHC(nullptr), theManager(manager), currentHit(nullptr),
      theTrack(nullptr), currentPV(nullptr), unitID(0), previousUnitID(0),
      preStepPoint(nullptr), postStepPoint(nullptr), eventno(0) {
...
#ifdef debug
  LogDebug("FP420Sim") << "FP420SD : Initialize called for " << name
                       << std::endl;
#endif
...
bool FP420SD::ProcessHits(G4Step *aStep, G4TouchableHistory *) {
  ...
  if (aStep == nullptr) {
    ...
    // AZ
#ifdef debug
    LogDebug("FP420Sim") << "FP420SD :  number of hits = " << theHC->entries()
                         << std::endl;
#endif
...
void FP420SD::GetStepInfo(G4Step *aStep) {
  ...
  hitPointLocal = preStepPoint->GetTouchable()
                      ->GetHistory()
                      ->GetTopTransform()
                      .TransformPoint(hitPoint);
  hitPointLocalExit = preStepPoint->GetTouchable()
                          ->GetHistory()
                          ->GetTopTransform()
                          .TransformPoint(hitPointExit);
  ...
  // Tof          = 1400. +
  // aStep->GetPostStepPoint()->GetGlobalTime()/nanosecond;
...
G4bool FP420SD::HitExists() {
  if (primaryID < 1) {
    edm::LogWarning("FP420Sim")
        << "***** FP420SD error: primaryID = " << primaryID
  ...
  //    LogDebug("FP420Sim") << "FP420SD: HCollection=  " << theHC->entries()
  //    <<std::endl;
...
void FP420SD::CreateNewHit() {
  ...
  LogDebug("FP420Sim") << "FP420SD CreateNewHit for"
                       ...
                       << " PVid = " << currentPV->GetCopyNo() << " Unit "
                       << unitID << std::endl;
  LogDebug("FP420Sim") << " primary " << primaryID << " time slice " << tSliceID
...
void FP420SD::EndOfEvent(G4HCofThisEvent *) {
  ...
  //    LogDebug("FP420Sim") << "FP420SD: warning!!! Number of hits exceed 100
  //    and =" << theHC->entries() << "\n";
  //  }
  ...
  for (int j = 0; j < theHC->entries(); j++) {
    ...
    if ((fabs(aHit->getTof()) > 780. && fabs(aHit->getTof()) < 840. &&
         nhitsHPS240 < 200.) ||
        (fabs(aHit->getTof()) > 1380. && fabs(aHit->getTof()) < 1450. &&
         nhitsFP420 < 200.)) {

#ifdef ddebug
      //    LogDebug("FP420SD") << " FP420Hit " << j << " " << *aHit <<
      //    std::endl;
      LogDebug("FP420Sim") << "hit number" << j
                           << "unit ID = " << aHit->getUnitID() << "\n";
      ...
      slave->processHits(PSimHit(
          locEntryPoint, locExitPoint, // entryPoint(), exitPoint() Local3DPoint
...
void FP420SD::update(const BeginOfRun *) {
  ...
  gammaPDG =
      theParticleTable->FindParticle(particleName = "gamma")->GetPDGEncoding();
}
