...
FP420Test::FP420Test(const edm::ParameterSet &p) {
  ...
  double ZSiStation =
      (pn0 - 1) * (2 * (zBlade + gapBlade) + ZKapton) + 2 * 6. + 0.0; // =  78.5
  ...
  if (verbosity > 0) {
    std::cout << "FP420Test constructor :: Initialized Fp420AnalysisHistManager"
              << std::endl;
  }
}
...
void Fp420AnalysisHistManager::HistInit(const char *name, const char *title,
...
TH1F *Fp420AnalysisHistManager::GetHisto(Int_t Number) {
  ...
  if (Number <= fHistArray->GetLast() &&
      fHistArray->At(Number) != (TObject *)nullptr) {
    ...
    std::cout << "!!!!!!!!!!!!!!!!!!GetHisto!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
              << std::endl;
    std::cout << " WARNING ERROR - HIST ID INCORRECT (TOO HIGH) - " << Number
              << std::endl;
    std::cout << "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
              << std::endl;
...
void FP420Test::update(const EndOfTrack *trk) {
  ...
  if (itrk == 1) {
    G4double tracklength =
        (*trk)()->GetTrackLength(); // Accumulated track length
    ...
    G4ThreeVector vert_pos =
        (*trk)()->GetVertexPosition(); // vertex ,where this track was created
...
void FP420Test::update(const G4Step *aStep) {
  ...
  TrackInformation *trkInfo =
      dynamic_cast<TrackInformation *>(theTrack->GetUserInformation());
  if (trkInfo == nullptr) {
  ...
  // const G4ThreeVector&   vert_pos       = theTrack->GetVertexPosition(); //
  // vertex ,where this track was created const G4ThreeVector&   vert_mom =
  ...
  // std::cout << " trackstatus= " << trackstatus << " entot= " << entot  <<
  // std::endl;
  ...
  G4ThreeVector prelocalpoint =
      theTrack->GetTouchable()->GetHistory()->GetTopTransform().TransformPoint(
          preposition);
  ...
  ////  std::cout << " posvolume = "      <<
  /// aStep->GetPostStepPoint()->GetPhysicalVolume()->GetName() << std::endl;
...
*/
  ...
  // 24.01.2006:
  // tr     :   id    parentID   trackstatus   tracklength   curstepnumber entot
  // vert_pos st     :   stepl EnerDeposit prestep:   preposition   prevolume =
  // SBSTm SIDETL SIDETR       name= SISTATION  copy= 1,2,3    name= SIPLANE
  // copy= 1..5..10

  // gen_track:
  //  id=1    parentID=1   trackstatus=0,2   tracklength(accumulated)
  //  curstepnumber   entot  vert_pos
  if (id == 1) {
    ...
    if (trackstatus != 2) {
      // for SD:   Si Det.:   SISTATION:SIPLANE:(SIDETL+BOUNDDET        +SIDETR
      // + CERAMDET)
      if (prename == "SIDETL" || prename == "SIDETR") {
        ...
        //	     double xPrimAtZhit = vert_pos.x() +
        //(zz-vert_pos.z())*tan(theta)*cos(phi); 	     double yPrimAtZhit
        //= vert_pos.y() + (zz-vert_pos.z())*tan(theta)*sin(phi);
        // =========
        ...
        if ((prename == "SIDETL" && posname != "SIDETL") ||
            (prename == "SIDETR" && posname != "SIDETR")) {
    ...
    if (trackstatus == 2) {
      ...
      // tot. energy of primary track at the end of trajectory(before
      // disappeare)
...
void FP420Test::update(const EndOfEvent *evt) {
  ...
  if (nvertex != 1)
    std::cout << "FP420Test: My warning: NumberOfPrimaryVertex != 1  -->  = "
              << nvertex << std::endl;
  ...
  for (int i = 0; i < nvertex; i++) {
    ...
    if (npart != 1)
      std::cout << "FP420Test: My warning: NumberOfPrimaryPart != 1  -->  = "
                << npart << std::endl;
    if (npart == 0)
      std::cout << "FP420Test End Of Event ERR: no NumberOfParticle"
                << std::endl;
  ...
  if (thePrim != nullptr) {
    ...
    // works OK:
    //      double  costheta
    //      =mom.z()/sqrt(mom.x()*mom.x()+mom.y()*mom.y()+mom.z()*mom.z());
    //      double theta = acos(min(max(costheta,double(-1.)),double(1.)));
    ...
    FP420G4HitCollection *theCAFI =
        (FP420G4HitCollection *)allHC->GetHC(CAFIid);
    //  CaloG4HitCollection* theCAFI = (CaloG4HitCollection*)
    //  allHC->GetHC(CAFIid);
    if (verbosity > 0) {
      // std::cout << "FP420Test: theCAFI = " << theCAFI << std::endl;
      std::cout << "FP420Test: theCAFI->entries = " << theCAFI->entries()
                << std::endl;
    }
    ...
    //    if(  lastpo.z()<z4  ||  abs(lastpo.x())> 5. || lastpo.y()< 10.2 ||
    //    lastpo.y()>30.2   ) {
    // for zzzmarta.xml
    //   if(  lastpo.z()<z4  ||  abs(lastpo.x())> 10. || lastpo.y()< 3.2 ||
    //   lastpo.y()>43.2   ) {
    if (lastpo.z() < z4) {
      //  if(  lastpo.z()<z4 && lastpo.perp()< 100. ) {
    ...
    if (varia == 2) {
      ...
      for (int j = 0; j < theCAFI->entries(); j++) {
        ...
        */
        ...
        //    CaloNumberingPacker::unpackCastorIndex(unitID, det, zside, sector,
        //    zmodule);
        FP420NumberingScheme::unpackFP420Index(unitID, det, zside, sector,
                                               zmodule);
        int justlayer =
            FP420NumberingScheme::unpackLayerIndex(rn00, zside); // 1,2
        ...
        // 1)+zScale*(zmodule - 1)+zside; //intindex=1-30:X,Y,X,Y,X,Y...
        // int zScale = 10;   unsigned int intindex = sScale*(sector -
        // 1)+zScale*(zside - 1)+zmodule;
        // //intindex=1-30:XXXXXXXXXX,YYYYYYYYYY,...
        ...
        if (verbosity > 2) {
          std::cout << "1111111111111111111111111111111111111111111111111111111"
                       "111111111111111111 "
                    << std::endl;
          std::cout << "FP420Test: det, zside, sector, zmodule = " << det
                    << zside << sector << zmodule << std::endl;
          std::cout << "FP420Test: justlayer = " << justlayer << std::endl;
          std::cout << "FP420Test: hitExitLocalPoint = " << hitExitLocalPoint
                    << std::endl;
          std::cout << "FP420Test: hitEntryLocalPoint = " << hitEntryLocalPoint
                    << std::endl;
          std::cout << "FP420Test:  middle= " << middle << std::endl;
          std::cout << "FP420Test:  hitPoint.z()-419000.= "
                    << hitPoint.z() - 419000. << std::endl;

          std::cout << "FP420Test:zHits-419000. = " << themapz[unitID] - 419000.
                    << std::endl;
        }
        //=======================================
        // Y
      ...
      //	  int rn00=rn0;//always
      if (verbosity > 2) {
        std::cout << "222222222222222222222222222222222222222222222222222222222"
                     "22222222222222222 "
                  << std::endl;
      }
      int det = 1;
      ...
      for (int sector = 1; sector < sn0; sector++) {
        for (int zmodule = 1; zmodule < pn0; zmodule++) {
          for (int zsideinorder = 1; zsideinorder < allplacesforsensors;
               zsideinorder++) {
            int zside = FP420NumberingScheme::realzside(
                rn00, zsideinorder); // 1,3,5,2,4,6
            if (verbosity > 2) {
              std::cout << "FP420Test:  sector= " << sector
                        << " zmodule= " << zmodule
            ...
            if (zside != 0) {
              int justlayer =
                  FP420NumberingScheme::unpackLayerIndex(rn00, zside); // 1,2
              if (justlayer < 1 || justlayer > 2) {
                std::cout << "FP420Test:WRONG  justlayer= " << justlayer
                          << std::endl;
              }
              int copyinlayer =
                  FP420NumberingScheme::unpackCopyIndex(rn00, zside); // 1,2,3
              if (copyinlayer < 1 || copyinlayer > 3) {
                std::cout << "FP420Test:WRONG  copyinlayer= " << copyinlayer
                          << std::endl;
              }
              int orientation = FP420NumberingScheme::unpackOrientation(
                  rn00, zside); // Front: = 1; Back: = 2
              if (orientation < 1 || orientation > 2) {
                std::cout << "FP420Test:WRONG  orientation= " << orientation
                          << std::endl;
              }

              // iu is a continues numbering of planes(!)  over two arm FP420
              // set up
              int detfixed =
                  1; // use this treatment for each set up arm, hence no sense
                     // to do it defferently for +FP420 and -FP420;
              //                                                                    and  ...[ii] massives have prepared in such a way
              unsigned int ii =
                  FP420NumberingScheme::packMYIndex(
                      rn00, pn0, sn0, detfixed, justlayer, sector, zmodule) -
                  1;
              // ii = 0-19   --> 20 items
              if (verbosity > 2) {
                std::cout << "FP420Test:  justlayer = " << justlayer
              ...
              double kplane =
                  -(pn0 - 1) / 2 - 0.5 +
                  (zmodule - 1); //-3.5 +0...5 = -3.5,-2.5,-1.5,+2.5,+1.5
              double zcurrent = zinibeg + z420 + (ZSiStep - ZSiPlane) / 2 +
                                kplane * ZSiStep + zdiststat;
              // double zcurrent = zinibeg +(ZSiStep-ZSiPlane)/2  +
              // kplane*ZSiStep + (sector-1)*zUnit;
              if (verbosity > 2) {
                std::cout << "FP420Test:  Leftzcurrent-419000. = "
                          << zcurrent - 419000. << std::endl;
              ...
              //
              if (verbosity > 2) {
                std::cout << "FP420Test:  zcurrent-419000. = "
                          << zcurrent - 419000. << std::endl;
              }
              //================================== end of for loops in
              // continuius number iu:
            } // if(zside!=0
          }   // for superlayer
        }     // for zmodule
      }       // for sector
      ...
      if (totallosenergy == 0.0) {
        std::cout << "FP420Test:     number of hits = " << theCAFI->entries()
                  << std::endl;
        for (int j = 0; j < theCAFI->entries(); j++) {
          FP420G4Hit *aHit = (*theCAFI)[j];
          double losenergy = aHit->getEnergyLoss();
          std::cout << " j hits = " << j << "losenergy = " << losenergy
                    << std::endl;
        }
      }
      //======================================================================================================CHECK
      ...
      for (int sector = 1; sector < 4; sector++) {
        ...
        for (int zmodule = 1; zmodule < 11; zmodule++) {
          for (int zside = 1; zside < 3; zside++) {
            ...
            //      int nhit = 0;
            //	int sScale = 20;
            int index = FP420NumberingScheme::packFP420Index(det, zside, sector,
                                                             zmodule);
      ...
      //======================================================================================================
      // HIT  ALL UserNtuples->fillg43(totalEnergy,1.);
  ...
  // ==========================================================================
  if (verbosity > 0) {
    std::cout << "FP420Test:  END OF Event " << (*evt)()->GetEventID()
              << std::endl;
  }
}
