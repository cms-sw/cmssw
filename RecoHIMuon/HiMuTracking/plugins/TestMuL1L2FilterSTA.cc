...
// $Id: TestMuL1L2FilterSTA.cc,v 1.1 2008/09/14 12:25:19 kodolova Exp $
//
//
...
{
   std::cout<<" Start TestMuL1L2STA constructor "<<std::endl;
   pset_ = ps1;
}
...
//
   ...
   for(edm::SimVertexContainer::const_iterator it=simVertices->begin(); it != simVertices->end(); it++ )
   {
        ...
        if(inum == 0) { 
          cout<<" Vertex position "<<vertex.position().rho()<<" "<<vertex.position().z()<<endl;
         //      theHICConst->setVertex(vertex.position().z()); theFmpConst->setVertex(vertex.position().z()); 
          break; }
        inum++;  
        //cout<<" Vertex position "<<vertex.position().rho()<<" "<<vertex.position().z()<<endl;
   }
