...
void ProvenanceDumper::work_() {
  ...
  auto meta = reader->GetModel().CreateEntry();
  edm::ProductRegistry* pReg = &reg_;
  meta->BindRawPtr(edm::poolNames::productDescriptionBranchName(), pReg);

  {
    auto rntuple = inputFile_->Get<ROOT::RNTuple>("ParameterSets");
    assert(rntuple);
  ...
  meta->BindRawPtr(edm::poolNames::processHistoryBranchName(), pPhv);

  reader->LoadEntry(0, *meta);
  //meta->GetEntry(0);
  assert(nullptr != pReg);
