...
int main(int argc, char* argv[]) {
  ...
  try {
    ...
    for (unsigned int j = 0; j < in.size(); ++j) {
      ...
      std::unique_ptr<TFile> tfile{edm::rntuple_temp::openFileHdl(filesIn[j])};
      if (tfile == nullptr)
        return 1;
      ...
      if (uuid) {
        auto* paramsTree = tfile->Get<ROOT::RNTuple>(edm::poolNames::metaDataTreeName().c_str());
        assert(paramsTree);
        auto reader = ROOT::RNTupleReader::Open(*paramsTree);
        assert(reader);
        if (json) {
          auout << ",\"uuid\":\"" << edm::rntuple_temp::getUuid(reader.get()) << '"';
        } else {
          auout << ", " << edm::rntuple_temp::getUuid(reader.get()) << " uuid";
        }
      }

      // Ok. How many events?
      int nruns = edm::rntuple_temp::numEntries(tfile.get(), edm::poolNames::runTreeName());
      int nlumis = edm::rntuple_temp::numEntries(tfile.get(), edm::poolNames::luminosityBlockTreeName());
      int nevents = edm::rntuple_temp::numEntries(tfile.get(), edm::poolNames::eventTreeName());
      if (json) {
        if (j > 0)
          std::cout << ',' << std::endl;
      ...
      if (events) {
        edm::rntuple_temp::printEventLists(tfile.get());
      }

      if (eventsInLumis) {
        edm::rntuple_temp::printEventsInLumis(tfile.get());
      }
