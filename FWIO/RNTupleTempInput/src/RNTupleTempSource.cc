...
namespace edm::rntuple_temp {
  namespace {
    void checkHistoryConsistency(Principal const& primary, Principal const& secondary) {
      ...
      if (ph1 != ph2 && !isAncestor(ph2, ph1)) {
        throw Exception(errors::MismatchedInputFiles, "RNTupleTempSource::checkConsistency")
  ...
  RNTupleTempSource::~RNTupleTempSource() {}

  void RNTupleTempSource::endJob() {
    if (secondaryFileSequence_)
  ...
  void RNTupleTempSource::closeFile_() { primaryFileSequence_->closeFile(); }

  std::shared_ptr<RunAuxiliary> RNTupleTempSource::readRunAuxiliary_() {
    return primaryFileSequence_->readRunAuxiliary_();
  }

  std::shared_ptr<LuminosityBlockAuxiliary> RNTupleTempSource::readLuminosityBlockAuxiliary_() {
    return primaryFileSequence_->readLuminosityBlockAuxiliary_();
  }

  void RNTupleTempSource::fillProcessBlockHelper_() { primaryFileSequence_->fillProcessBlockHelper_(); }

  bool RNTupleTempSource::nextProcessBlock_(ProcessBlockPrincipal& processBlockPrincipal) {
    return primaryFileSequence_->nextProcessBlock_(processBlockPrincipal);
  }

  void RNTupleTempSource::readProcessBlock_(ProcessBlockPrincipal& processBlockPrincipal) {
    primaryFileSequence_->readProcessBlock_(processBlockPrincipal);
  }

  void RNTupleTempSource::readRun_(RunPrincipal& runPrincipal) {
    ...
    if (secondaryFileSequence_ && shouldWeProcessRun && !branchIDsToReplace_[InRun].empty()) {
      ...
      if (found) {
        ...
        throw Exception(errors::MismatchedInputFiles, "RNTupleTempSource::readRun_")
            << " Run " << runPrincipal.run() << " is not found in the secondary input files\n";
      }
    }
  }

  void RNTupleTempSource::readLuminosityBlock_(LuminosityBlockPrincipal& lumiPrincipal) {
    ...
    if (secondaryFileSequence_ && shouldWeProcessLumi && !branchIDsToReplace_[InLumi].empty()) {
      ...
      if (found) {
        ...
        throw Exception(errors::MismatchedInputFiles, "RNTupleTempSource::readLuminosityBlock_")
  ...
  void RNTupleTempSource::readEvent_(EventPrincipal& eventPrincipal) {
    ...
    if (secondaryFileSequence_ && !branchIDsToReplace_[InEvent].empty()) {
      ...
      if (found) {
        ...
        throw Exception(errors::MismatchedInputFiles, "RNTupleTempSource::readEvent_")
  ...
  bool RNTupleTempSource::readIt(EventID const& id, EventPrincipal& eventPrincipal, StreamContext& streamContext) {
  ...
  InputSource::ItemTypeInfo RNTupleTempSource::getNextItemType() {
  ...
  void RNTupleTempSource::rewind_() { primaryFileSequence_->rewind_(); }

  // Advance "offset" events.  Offset can be positive or negative (or zero).
  void RNTupleTempSource::skip(int offset) { primaryFileSequence_->skipEvents(offset); }

  bool RNTupleTempSource::goToEvent_(EventID const& eventID) { return primaryFileSequence_->goToEvent(eventID); }

  void RNTupleTempSource::fillDescriptions(ConfigurationDescriptions& descriptions) {
  ...
  bool RNTupleTempSource::randomAccess_() const { return true; }

  ProcessingController::ForwardState RNTupleTempSource::forwardState_() const {
    return primaryFileSequence_->forwardState();
  }

  ProcessingController::ReverseState RNTupleTempSource::reverseState_() const {
