...
namespace edm::rntuple_temp {
  ...
  void rootrntuple::BranchInfo::setBranch(TBranch* branch, TClass const* wrapperBaseTClass) {
  ...
  std::unique_ptr<WrapperBase> rootrntuple::BranchInfo::newWrapper() const {
  ...
  void RootRNTuple::addBranch(ProductDescription const& prod, std::string const& oldBranchName) {
    ...
    rootrntuple::BranchInfo info = rootrntuple::BranchInfo(prod);
  ...
  rootrntuple::BranchMap const& RootRNTuple::branches() const { return branches_; }

  std::shared_ptr<TTreeCache> RootRNTuple::createCacheWithSize(unsigned int cacheSize) const {
  ...
  inline TTreeCache* RootRNTuple::checkTriggerCache(TBranch* branch, EntryNumber entryNumber) const {
  ...
  TTreeCache* RootRNTuple::checkTriggerCacheImpl(TBranch* branch, EntryNumber entryNumber) const {
  ...
  inline TTreeCache* RootRNTuple::selectCache(TBranch* branch, EntryNumber entryNumber) const {
  ...
  TTreeCache* RootRNTuple::getAuxCache(TBranch* auxBranch) const {
  ...
  inline void RootRNTuple::getEntryUsingCache(TBranch* branch, EntryNumber entryNumber, TTreeCache* cache) const {
  ...
  void RootRNTuple::startTraining() {
    if (cacheSize_ == 0) {
      return;
    }
    assert(treeCache_);
  ...
  void RootRNTuple::trainCache(char const* branchNames) {
    if (cacheSize_ == 0) {
      return;
    }
    tree_->LoadTree(0);
