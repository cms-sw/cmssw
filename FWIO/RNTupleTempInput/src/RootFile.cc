...
namespace edm::rntuple_temp {
  ...
  public:
    explicit RootFileEventFinder(RootRNTuple& eventTree) : eventTree_(eventTree) {}
    ~RootFileEventFinder() override {}

    EventNumber_t getEventNumberOfEntry(rootrntuple::EntryNumber entry) const override {
      rootrntuple::EntryNumber saveEntry = eventTree_.entryNumber();
  ...
  RootFile::RootFile(FileOptions&& fileOptions,
      ...
      : file_(fileOptions.fileName),
        ...
        filePtr_(fileOptions.filePtr),
        eventSkipperByID_(processingOptions.eventSkipperByID),
        fileFormatVersion_(),
        fid_(),
    ...
    if (productChoices.labelRawDataLikeMC) {
    ...
    validateFile(inputType, processingOptions.usingGoToEvent, orderedProcessHistoryIDs);
  ...
  void RootFile::validateFile(InputType inputType,
                              bool usingGoToEvent,
                              std::vector<ProcessHistoryID>& orderedProcessHistoryIDs) {
    // Validate the file.
    if (!fid_.isValid()) {
    ...
    indexIntoFile_.fixIndexes(orderedProcessHistoryIDs);
  ...
  void RootFile::reportOpened(std::string const& inputType) {
    ...
    std::string moduleName = "RNTupleTempSource";
  ...
  void RootFile::readProcessBlock_(ProcessBlockPrincipal& processBlockPrincipal) {
    assert(inputType_ == InputType::Primary);
    RootRNTuple* rootTree = processBlockTrees_[currentProcessBlockTree_].get();
  ...
  bool RootFile::readRun_(RunPrincipal& runPrincipal) {
    ...
    if (shouldProcessRun) {
      if (inputType_ == InputType::Primary) {
        ...
        RootRNTuple::EntryNumber const& entryNumber = runTree_.entryNumber();
  ...
  void RootFile::dropProcessesAndReorder(StoredProcessBlockHelper& storedProcessBlockHelper,
    ...
    // unneeded RootRNTuples to be deleted.
    if (isModified) {
      std::vector<edm::propagate_const<std::unique_ptr<RootRNTuple>>> newProcessBlockTrees;
  ...
  void RootFile::makeProcessBlockRootRNTuples(std::shared_ptr<InputFile> filePtr,
                                              TTreeOptions const& options,
                                              InputType inputType,
                                              StoredProcessBlockHelper const& storedProcessBlockHelper) {
    // When this functions returns there will be exactly a 1-to-1 correspondence between the
    // processes listed in storedProcessBlockHelper and the RootRNTuple objects created. processBlockTrees_
    // has pointers to the RootRNTuple's and will be filled in the same order. The RootRNTuple constructor
