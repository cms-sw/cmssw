#ifndef FWIO_RNTupleTempInput_RootFile_h
#define FWIO_RNTupleTempInput_RootFile_h
...
namespace edm {
  ...
  namespace rntuple_temp {
    ...
    public:
      virtual std::unique_ptr<ProvenanceReaderBase> makeReader(
          RootRNTuple& eventTree, DaqProvenanceHelper const* daqProvenanceHelper) const = 0;
    ...
    public:
      ...
      RootRNTuple const& eventTree() const { return eventTree_; }
      ...
      bool setEntryAtItem(RunNumber_t run, LuminosityBlockNumber_t lumi, EventNumber_t event) {
        return (event != 0) ? setEntryAtEvent(run, lumi, event)
                            : (lumi ? setEntryAtLumi(run, lumi) : setEntryAtRun(run));
      }
    ...
    private:
      void makeProcessBlockRootRNTuples(std::shared_ptr<InputFile> filePtr,
      ...
      RootRNTuple lumiTree_;
      RootRNTuple runTree_;
      std::vector<edm::propagate_const<std::unique_ptr<RootRNTuple>>> processBlockTrees_;
      unsigned int currentProcessBlockTree_ = 0;
      std::vector<edm::propagate_const<RootRNTuple*>> treePointers_;
