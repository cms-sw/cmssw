...
namespace edm {
  ...
  bool RootOutputRNTuple::checkSplitLevelsAndBasketSizes(TTree* inputTree) const {
  ...
  bool RootOutputRNTuple::checkIfFastClonable(TTree* inputTree) const {
  ...
  void RootOutputRNTuple::setSubBranchBasketSizes(TTree* inputTree) const {
    if (inputTree == nullptr)
      return;
  ...
  bool RootOutputRNTuple::checkEntriesInReadBranches(Long64_t expectedNumberOfEntries) const {
  ...
  void RootOutputRNTuple::fastCloneTTree(TTree* in, std::string const& option) {
    if (in->GetEntries() != 0) {
  ...
  void RootOutputRNTuple::maybeFastCloneTree(bool canFastClone,
