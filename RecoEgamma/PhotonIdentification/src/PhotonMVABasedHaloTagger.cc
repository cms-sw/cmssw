...
double PhotonMVABasedHaloTagger::calculateMVA(const reco::Photon* pho, const edm::Event& iEvent, const edm::EventSetup& es){

  
  bool isEB = pho->isEB();

  if(isEB) return 1.0; /// this MVA is useful and trained only for the EE photons. For EB, there are a lot of other useful handles which can reject beam halo efficiently
  ...
  noZS::EcalClusterLazyTools lazyToolnoZS(iEvent, ecalClusterToolsESGetTokens_.get(es), EBecalCollection_, EEecalCollection_);


  ///calculate the energy weighted X, Y and Z position of the photon cluster
  if(isEB) calphoClusCoordinECAL(geo, pho, &thresholds, barrelHitHandle);
  else calphoClusCoordinECAL(geo, pho, &thresholds, endcapHitHandle);
  ...
  double angle_EE_HE_samedPhi = calAngleBetweenEEAndSubDet(hcalClusNhits_samedPhi_, hcalClusX_samedPhi_, hcalClusY_samedPhi_, hcalClusZ_samedPhi_); //essentially caculates the angle and energy variables in the two hypothesis between EE and HE

  double angle_EE_HE_samedR = calAngleBetweenEEAndSubDet(hcalClusNhits_samedR_, hcalClusX_samedR_, hcalClusY_samedR_, hcalClusZ_samedR_);

  double angle_EE_ES_samedPhi = calAngleBetweenEEAndSubDet(preshowerNhits_samedPhi_, preshowerX_samedPhi_, preshowerY_samedPhi_, preshowerZ_samedPhi_);

  double angle_EE_ES_samedR = calAngleBetweenEEAndSubDet(preshowerNhits_samedR_, preshowerX_samedR_, preshowerY_samedR_, preshowerZ_samedR_);
  ...
  vars[13] = (pho->superCluster()->preshowerEnergyPlane1() + pho->superCluster()->preshowerEnergyPlane2())/pho->superCluster()->rawEnergy();
  vars[14] = rho_;
...
void PhotonMVABasedHaloTagger::calphoClusCoordinECAL(const CaloGeometry* geo,
  ...
  ecalClusX_ = 0; ecalClusY_ = 0; ecalClusZ_ = 0;
  ecalClusNhits_ = 0;
  ecalClusE_ = 0;
  ...
  for(EcalRecHitCollection::const_iterator ecalrechit = ecalRecHits->begin(); ecalrechit != ecalRecHits->end(); ecalrechit++){
...
void PhotonMVABasedHaloTagger::calmatchedHBHECoordForBothHypothesis(
						   ...
						   const reco::Photon* pho,
						   edm::Handle<HBHERecHitCollection> hcalhitsCollHBHE
						   ){
  
  
  hcalClusX_samedPhi_ = 0; hcalClusY_samedPhi_ = 0; hcalClusZ_samedPhi_ = 0;
  
  hcalClusNhits_samedPhi_ = 0;
  hcalClusE_samedPhi_ = 0;
  
  hcalClusX_samedR_ = 0; hcalClusY_samedR_ = 0; hcalClusZ_samedR_ = 0;
  ...
  for (HBHERecHitCollection::const_iterator hbherechit = HBHERecHits->begin(); hbherechit != HBHERecHits->end(); hbherechit++) {
    ...
    if(phoSCEta * rhEta < 0) continue; ///Should be on the same side of Z
    ...
    if(rho>=31 && rho<=172 && dRho <= 26 && fabs(dPhi)<0.15){ ///only valid for the EE; this is 26 cm; hit within 3x3 of HCAL centered at the EECAL xtal                                                                                                      
...
void PhotonMVABasedHaloTagger::calmatchedESCoordForBothHypothesis(const CaloGeometry* geo,
  ...
  preshowerX_samedPhi_ = 0; preshowerY_samedPhi_ = 0; preshowerZ_samedPhi_ = 0;
  preshowerNhits_samedPhi_ = 0;
  preshowerE_samedPhi_ = 0;

  preshowerX_samedR_ = 0; preshowerY_samedR_ = 0; preshowerZ_samedR_ = 0;
  ...
  for( EcalRecHitCollection::const_iterator esrechit = ESRecHits->begin(); esrechit != ESRecHits->end(); esrechit++ ){
    ...
    */
    ...
    if(dRho < tmpDiffdRho && dRho < 2.2){ ////i.e. hit is required to be within the ----> seems better match with the data compared to 2.47 
...
double PhotonMVABasedHaloTagger::calAngleBetweenEEAndSubDet(int nhits, double subdetClusX, double subdetClusY, double subdetClusZ){
  ...
  if(ecalClusNhits_>0 && nhits>0 ){
    ...
    double dR = sqrt( pow(subdetClusX-ecalClusX_,2) + pow(subdetClusY-ecalClusY_,2) + pow(subdetClusZ-ecalClusZ_,2) );
