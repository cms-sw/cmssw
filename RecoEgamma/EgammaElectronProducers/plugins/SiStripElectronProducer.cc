...
SiStripElectronProducer::SiStripElectronProducer(const edm::ParameterSet& iConfig) {
  ...
  rphi_sistrips2dtag_ = consumes<SiStripRecHit2DCollection>(edm::InputTag(siHitProducer_, siRphiHitCollection_));
  stereo_sistrips2dtag_ = consumes<SiStripRecHit2DCollection>(edm::InputTag(siHitProducer_, siStereoHitCollection_));
  ...
  algo_p = new SiStripElectronAlgo(iConfig.getParameter<int32_t>("maxHitsOnDetId"),
...
void SiStripElectronProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  str << " Found " << siStripElectCands << " SiStripElectron Candidates before track fit "
      << "\n"
      << std::endl;
