...
GsfElectronCoreProducer::GsfElectronCoreProducer(const edm::ParameterSet &config)
  ...
  edCoresTag_ =
      consumes<reco::GsfElectronCoreCollection>(config.getParameter<edm::InputTag>("ecalDrivenGsfElectronCoresTag"));
  pfCoresTag_ =
      consumes<reco::GsfElectronCoreCollection>(config.getParameter<edm::InputTag>("pflowGsfElectronCoresTag"));
...
void GsfElectronCoreProducer::produce(edm::Event &event, const edm::EventSetup &setup) {
  ...
  for (edCoreIter = edCoresCollection->begin(); edCoreIter != edCoresCollection->end(); edCoreIter++) {
  ...
  for (eleCore = electrons.begin(); eleCore != electrons.end(); eleCore++) {
    ...
    found = false;
    for (pfCoreIter = pfCoresCollection->begin(); pfCoreIter != pfCoresCollection->end(); pfCoreIter++) {
...
void GsfElectronCoreProducer::produceTrackerDrivenCore(const GsfTrackRef &gsfTrackRef,
                                                       std::list<GsfElectronCore *> &electrons) {
