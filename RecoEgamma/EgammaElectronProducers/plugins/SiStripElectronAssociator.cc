...
SiStripElectronAssociator::SiStripElectronAssociator(const edm::ParameterSet& iConfig) {
  ...
  siStripElectronCollection_ =
      consumes<reco::SiStripElectronCollection>(iConfig.getParameter<edm::InputTag>("siStripElectronCollection"));
...
void SiStripElectronAssociator::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (reco::SiStripElectronCollection::const_iterator strippyIter = siStripElectrons->begin();
       strippyIter != siStripElectrons->end();
       ++strippyIter) {
  ...
  for (unsigned int i = 0; i < tracks.product()->size(); i++) {
    ...
    LogDebug("SiStripElectronAssociator") << " New Track Candidate " << i << " DetId " << id << " pos " << pos << "\n";
    ...
    for (reco::SiStripElectronCollection::const_iterator strippyIter = siStripElectrons->begin();
      ...
      if (!alreadySeen[&(*strippyIter)]) {
        ...
        for (std::vector<SiStripRecHit2D>::const_iterator hitIter = strippyIter->rphiRecHits().begin();
             hitIter != strippyIter->rphiRecHits().end();
             ++hitIter) {
          LogDebug("SiStripElectronAssociator")
              << " SiStripCand "
              << " DetId " << hitIter->geographicalId().rawId() << " localPos " << hitIter->localPosition()
              << " deltasPos " << (hitIter->localPosition() - pos).mag();

          if (hitIter->geographicalId().rawId() == id && (hitIter->localPosition() - pos).mag() < positionTol) {
            hitInCommon = true;
        ...
        if (!hitInCommon) {
          ...
          for (std::vector<SiStripRecHit2D>::const_iterator hitIter = strippyIter->stereoRecHits().begin();
        ...
        if (hitInCommon) {
          ...
          reco::Electron electron(
              (trackPtr->charge() > 0 ? 1 : -1),
              math::XYZTLorentzVector(trackPtr->px(), trackPtr->py(), trackPtr->pz(), trackPtr->p()),
              math::XYZPoint(trackPtr->vx(), trackPtr->vy(), trackPtr->vz()));
    ...
    if (!foundElectron) {
      throw cms::Exception("Configuration") << " Inconsistent track collection!";
    }
  ...
  LogDebug("SiStripElectronAssociator") << " Number of SiStripElectrons returned with a good fit " << countSiElFit
                                        << "\n"
                                        << std::endl;
