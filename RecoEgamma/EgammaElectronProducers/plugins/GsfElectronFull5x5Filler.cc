...
GsfElectronFull5x5Filler::GsfElectronFull5x5Filler(const edm::ParameterSet& cfg) {
  ...
  if (hcalCfg.hOverEConeSize > 0) {
    ...
    hcalCfg.hcalTowers = consumes<CaloTowerCollection>(cfg.getParameter<edm::InputTag>("hcalTowers"));
  ...
  if (hcalCfgPflow.hOverEConeSize > 0) {
    ...
    hcalCfgPflow.hcalTowers = consumes<CaloTowerCollection>(cfg.getParameter<edm::InputTag>("hcalTowers"));
...
void GsfElectronFull5x5Filler::beginLuminosityBlock(edm::LuminosityBlock const& lb, edm::EventSetup const& es) {
...
void GsfElectronFull5x5Filler::calculateShowerShape_full5x5(const reco::SuperClusterRef& theClus,
  ...
  if (pflow) {
    ...
    showerShape.hcalDepth1OverEcalBc =
        _hcalHelperPflow->hcalESumDepth1BehindClusters(showerShape.hcalTowersBehindClusters) / showerShape.e5x5;
    showerShape.hcalDepth2OverEcalBc =
        _hcalHelperPflow->hcalESumDepth2BehindClusters(showerShape.hcalTowersBehindClusters) / showerShape.e5x5;
  } else {
    ...
    showerShape.hcalDepth1OverEcalBc =
        _hcalHelper->hcalESumDepth1BehindClusters(showerShape.hcalTowersBehindClusters) / showerShape.e5x5;
    showerShape.hcalDepth2OverEcalBc =
        _hcalHelper->hcalESumDepth2BehindClusters(showerShape.hcalTowersBehindClusters) / showerShape.e5x5;
  }
}
