...
CkfTrajectoryBuilderWithSeedAssoc::trajectories(const TrajectorySeed& seed) const
{  
...
CkfTrajectoryBuilderWithSeedAssoc::findCompatibleMeasurements( const Trajectory& traj) const
{
  ...
  for (vector<TM>::const_iterator i=result.begin();
    ...
    if (afterInvalid && i->recHit().isValid()) {
      edm::LogError("CkfPattern") << "CkfTrajectoryBuilderWithSeedAssoc error: valid hit after invalid!" ;
    }
  }
#endif
