...
void GsfElectronMCFakeAnalyzer::endJob() {
  histfile_->cd();
...
void GsfElectronMCFakeAnalyzer::analyze(const edm::Event &iEvent, const edm::EventSetup &iSetup) {
  ...
  for (auto moIter = genJets->begin(); moIter != genJets->end(); ++moIter) {
