...
DQMAnalyzer::beginJob(){
  ...
  std::cout << "!!!!!!!!!!!!!!!   - iii -   : histo titles " << std::endl;
...
DQMAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for ( reco::SuperClusterCollection::const_iterator moIter=recoClusters->begin();
   moIter!=recoClusters->end(); moIter++ ) {
	...
	if (Selection_ == 2 || Selection_ == 3 || Selection_ == 4 )
	  {  // Isolation Cut
	...
	if (Selection_ == 4){
	  for (reco::GsfElectronCollection::const_iterator gsfIter2=gsfIter+1;
	    ...
	    // conditions Probe 
	    if(PROBEetcut_ && (gsfIter->superCluster()->energy()/cosh(gsfIter->superCluster()->eta())<minEt_)) continue;
	...
	if (Selection_ == 0 || Selection_ == 1 || Selection_ == 2 || Selection_ == 3){
	  for (reco::GsfElectronCollection::const_iterator gsfIter2=gsfIter+1;
	       gsfIter2!=gsfElectrons->end(); gsfIter2++){
	    math::XYZTLorentzVector p12 = (*gsfIter).p4()+(*gsfIter2).p4();
            float mee2 = p12.Dot(p12);	
	...
	if (matchingCondition_ == "Cone" && Selection_ != 4) {
