...
double ConversionLikelihoodCalculator::calculateLikelihood(reco::ConversionRef conversion, const edm::EventSetup& iSetup)
{
   ...
   mva_.update<PhotonConversionMVAComputerRcd>(iSetup, "label");
   if (!mva_) {
//       std::cerr << "Could not update the PhotonConversionMVAComputerRcd!" << std::endl;
       return -1.;
   } else {
//       std::cout << "Successfully updated the PhotonConversionMVAComputerRcd!" << std::endl;
   }

   Variable::ValueList vars;
   vars.add("log_e_over_p_",log_e_over_p_);
   vars.add("log_abs_cot_theta__",log_abs_cot_theta_);
   vars.add("log_abs_delta_phi__",log_abs_delta_phi_);
   vars.add("log_chi2_max_pt_",log_chi2_max_pt_);
   vars.add("log_chi2_min_pt_",log_chi2_min_pt_);

   float lh = (mva_)->eval(vars);
...
double ConversionLikelihoodCalculator::calculateLikelihood(reco::Conversion& conversion, const edm::EventSetup& iSetup)
{
    using namespace PhysicsTools;
