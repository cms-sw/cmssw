...
double CutBasedElectronID::cicSelection(const reco::GsfElectron* electron,
  ...
  double ip = 0;
  int mishits = electron->gsfTrack()->getHitPattern().numberOfHits(reco::HitPattern::MISSING_INNER_HITS);
...
double CutBasedElectronID::robustSelection(const reco::GsfElectron* electron ,
  ...
  if (type_ == "robust") {
    ...
    if (cut[23] >0 && not (electron->gsfTrack()->getHitPattern().hasValidHitInFirstPixelBarrel(reco::HitPattern::TRACK_HITS)))
      return result;
