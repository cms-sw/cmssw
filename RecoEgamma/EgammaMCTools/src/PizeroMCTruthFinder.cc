...
std::vector<PizeroMCTruth> PizeroMCTruthFinder::find(const std::vector<SimTrack>& theSimTracks, const std::vector<SimVertex>& theSimVertices ) {
  ...
  for (const auto & theSimTrack : theSimTracks){
    ...
    std::cout << " Particle type " <<  theSimTrack.type() << " Sim Track ID " << theSimTrack.trackId() << " momentum " << theSimTrack.momentum() <<  " vertex position " << vertex.position() << std::endl;  

    if ( theSimTrack.vertIndex() == iPV ) {
      npv++;
      if ( std::abs(theSimTrack.type() ) == 111) {

	std::cout << " Found a primary pizero with ID  " << theSimTrack.trackId() << " momentum " << theSimTrack.momentum() <<  std::endl;
  ...
  for (auto & pizeroTrack : pizeroTracks){
    std::cout << " Looping on the primary pizero pt  " << sqrt(pizeroTrack.momentum().perp2()) << " pizero track ID " << pizeroTrack.trackId() << std::endl;
	...
	std::cout << "Matched Photon ID " << mcPhoton.trackId() << "  vtx " << phoParentInd << " with pizero " << pizeroTrack.trackId() << std::endl;
    ...
    CLHEP::HepLorentzVector tmpMom( pizeroTrack.momentum().px(), pizeroTrack.momentum().py(),
                             pizeroTrack.momentum().pz(), pizeroTrack.momentum().e() ) ;
