...
void EgammaHLTMulti5x5ClusterProducer::produce(edm::Event& evt, const edm::EventSetup& es) {
  ...
  if (doEndcaps_) {
    clusterizeECALPart(
        evt, es, endcapHitToken_, endcapClusterCollection_, endcapRegions, reco::CaloID::DET_ECAL_ENDCAP);
  }
  if (doBarrel_) {
    clusterizeECALPart(
        evt, es, barrelHitToken_, barrelClusterCollection_, barrelRegions, reco::CaloID::DET_ECAL_BARREL);
  }
}

const EcalRecHitCollection* EgammaHLTMulti5x5ClusterProducer::getCollection(
    edm::Event& evt, const edm::EDGetTokenT<EcalRecHitCollection>& hitToken) const {
...
void EgammaHLTMulti5x5ClusterProducer::clusterizeECALPart(edm::Event& evt,
  ...
  clusters =
      Multi5x5_p->makeClusters(hitCollection_p, geometry_p, topology_p.get(), geometryES_p, detector, true, regions);
