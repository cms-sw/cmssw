...
EcalListOfFEDSProducer::EcalListOfFEDSProducer(const edm::ParameterSet& pset) {
  ...
  if (EGamma_) {
    ...
    EMl1TagNonIsolated_ =
        consumes<L1EmParticleCollection>(pset.getUntrackedParameter<edm::InputTag>("EM_l1TagNonIsolated"));
...
EcalListOfFEDSProducer::~EcalListOfFEDSProducer() { delete TheMapping; }
...
void EcalListOfFEDSProducer::produce(edm::Event& e, const edm::EventSetup& iSetup) {
  ...
  if (debug_)
    std::cout << " ECAL unpacking module has already run " << nDone << " times. " << std::endl;
  for (unsigned int id = 0; id < nDone; id++) {
    // ignore the FEDs coming from unpacking in pi0 paths
    if (Pi0ListToIgnore_.label() == FEDs_Done[id].provenance()->moduleLabel()) {
      continue;
    }
  ...
  if (debug_)
    std::cout << " For this event, " << Done.size() << " ECAL FEDs have already been unpacked." << std::endl;
...
std::vector<int> EcalListOfFEDSProducer::ListOfFEDS(
