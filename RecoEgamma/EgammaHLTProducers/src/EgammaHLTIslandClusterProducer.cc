...
EgammaHLTIslandClusterProducer::EgammaHLTIslandClusterProducer(const edm::ParameterSet& ps)
    : doBarrel_(ps.getParameter<bool>("doBarrel")),
      ...
      island_p(new IslandClusterAlgo(
          ps.getParameter<double>("IslandBarrelSeedThr"),
          ...
          verb_ == "DEBUG"
              ...
              : (verb_ == "WARNING" ? IslandClusterAlgo::pWARNING
                                    : (verb_ == "INFO" ? IslandClusterAlgo::pINFO : IslandClusterAlgo::pERROR)))) {
...
EgammaHLTIslandClusterProducer::~EgammaHLTIslandClusterProducer() { delete island_p; }
  ...
  if (doIsolated_) {
    for (l1extra::L1EmParticleCollection::const_iterator emItr = emIsolColl->begin(); emItr != emIsolColl->end();
      ...
      if (emItr->et() > l1LowerThr_ && emItr->et() < l1UpperThr_) {
        ...
        int isforw = 0;
        int isbarl = 0;
        if ((float)(etaHigh) > 1.479 || (float)(etaLow) < -1.479)
          isforw = 1;
...
const EcalRecHitCollection* EgammaHLTIslandClusterProducer::getCollection(
    edm::Event& evt, const edm::EDGetTokenT<EcalRecHitCollection>& hitToken_) const {
...
void EgammaHLTIslandClusterProducer::clusterizeECALPart(edm::Event& evt,
  ...
  clusters =
      island_p->makeClusters(hitCollection_p, geometry_p, topology_p.get(), geometryES_p, ecalPart, true, regions);
