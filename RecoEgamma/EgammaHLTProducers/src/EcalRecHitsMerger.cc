...
void EcalRecHitsMerger::produce(edm::StreamID, edm::Event& e, const edm::EventSetup& iSetup) const {
  if (debug_)
    std::cout << " EcalRecHitMerger : Run " << e.id().run() << " Event " << e.id().event() << std::endl;
  ...
  for (unsigned int i = 0; i < nColl; i++) {
    ...
    if (module_label != EgammaSourceEB_.label() && module_label != MuonsSourceEB_.label() &&
        module_label != JetsSourceEB_.label() && module_label != TausSourceEB_.label() &&
        module_label != RestSourceEB_.label() && module_label != Pi0SourceEB_.label())
      continue;
  ...
  if (debug_)
    std::cout << " Number of EB Rechits to merge  = " << nEB << std::endl;
  if (debug_)
    std::cout << " Number of EE Rechits to merge  = " << nEE << std::endl;
  ...
  for (unsigned int i = 0; i < nColl; i++) {
    ...
    if (instance == InputRecHitEB_) {
      for (EcalRecHitCollection::const_iterator it = EcalRecHits_done[i]->begin(); it != EcalRecHits_done[i]->end();
           it++) {
