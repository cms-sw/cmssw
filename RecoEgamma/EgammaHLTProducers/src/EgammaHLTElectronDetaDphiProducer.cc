...
void EgammaHLTElectronDetaDphiProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  if (!useSCRefs_) {
    ...
    for (reco::RecoEcalCandidateCollection::const_iterator iRecoEcalCand = recoEcalCandHandle->begin();
      ...
      if (eleRef.isNonnull())
        dEtaDPhi = calDEtaDPhiSCTrk(eleRef, bsPosition, theMagField.product());
...
std::pair<float, float> EgammaHLTElectronDetaDphiProducer::calDEtaDPhiSCTrk(reco::ElectronRef& eleref,
  ...
  math::XYZPoint SCcorrPosition(
      theClus->x() - bsPosition.x(), theClus->y() - bsPosition.y(), theClus->z() - eleref->track()->vz());
  ...
  if (variablesAtVtx_) {
    ...
    if (deltaphi1 > 6.283185308)
      deltaphi1 -= 6.283185308;
    if (deltaphi1 > 3.141592654)
      deltaphi1 = 6.283185308 - deltaphi1;

    float deltaphi2 = fabs(phi2 - theClus->position().phi());
    if (deltaphi2 > 6.283185308)
      deltaphi2 -= 6.283185308;
    if (deltaphi2 > 3.141592654)
      deltaphi2 = 6.283185308 - deltaphi2;

    deltaphi = deltaphi1;
    if (deltaphi2 < deltaphi1) {
      deltaphi = deltaphi2;
    }
  } else {
...
reco::ElectronRef EgammaHLTElectronDetaDphiProducer::getEleRef(
    const reco::RecoEcalCandidateRef& recoEcalCandRef, const edm::Handle<reco::ElectronCollection>& electronHandle) {
