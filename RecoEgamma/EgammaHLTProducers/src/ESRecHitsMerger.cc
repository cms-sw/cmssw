...
void ESRecHitsMerger::produce(edm::StreamID, edm::Event& e, const edm::EventSetup& iSetup) const {
  if (debug_)
    std::cout << " ESRecHitMerger : Run " << e.id().run() << " Event " << e.id().event() << std::endl;
  ...
  for (unsigned int i = 0; i < nColl; i++) {
    ...
    if (module_label != EgammaSourceES_.label() && module_label != MuonsSourceES_.label() &&
        module_label != JetsSourceES_.label() && module_label != TausSourceES_.label() &&
        module_label != RestSourceES_.label() && module_label != Pi0SourceES_.label() &&
        module_label != EtaSourceES_.label())
      continue;
  ...
  if (debug_)
    std::cout << " Number of ES Rechits to merge  = " << nES << std::endl;
