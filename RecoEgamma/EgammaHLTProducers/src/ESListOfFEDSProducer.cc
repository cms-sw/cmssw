...
ESListOfFEDSProducer::ESListOfFEDSProducer(const edm::ParameterSet& pset) {
  ...
  if (EGamma_) {
    EMl1TagIsolated_ =
        consumes<l1extra::L1EmParticleCollection>(pset.getUntrackedParameter<edm::InputTag>("EM_l1TagIsolated"));
    EMl1TagNonIsolated_ =
        consumes<l1extra::L1EmParticleCollection>(pset.getUntrackedParameter<edm::InputTag>("EM_l1TagNonIsolated"));
  ...
  if (Jets_) {
    ...
    CentralSource_ =
        consumes<l1extra::L1JetParticleCollection>(pset.getUntrackedParameter<edm::InputTag>("CentralSource"));
    ForwardSource_ =
        consumes<l1extra::L1JetParticleCollection>(pset.getUntrackedParameter<edm::InputTag>("ForwardSource"));
...
ESListOfFEDSProducer::~ESListOfFEDSProducer() { delete TheMapping; }
...
void ESListOfFEDSProducer::produce(edm::Event& e, const edm::EventSetup& iSetup) {
  ...
  if (debug_ && nf < 1)
    std::cout << " Warning : no ECAL FED to unpack for Run " << e.id().run() << "  Event " << e.id().event()
              << std::endl;
  ...
  if (debug_) {
    std::cout << int(es_feds.size()) << " feds requested, and ESunpacking have already been done " << esnDone
              << " times." << std::endl;
  }
...
void ESListOfFEDSProducer::Egamma(edm::Event& e,
  ...
  if (debug_)
    std::cout << std::endl << std::endl << " enter in EcalListOfFEDSProducer::Egamma" << std::endl;
  ...
  if (EMdoIsolated_) {
    for (l1extra::L1EmParticleCollection::const_iterator emItr = emIsolColl->begin(); emItr != emIsolColl->end();
      ...
      if (pt < Ptmin_iso_)
        continue;
      if (debug_)
        std::cout << " Here is an L1 isoEM candidate of pt " << pt << std::endl;
      ...
      for (int i = 0; i < (int)feds.size(); i++) {
        if (std::find(FEDs.begin(), FEDs.end(), feds[i]) == FEDs.end() &&
            std::find(done.begin(), done.end(), feds[i]) == done.end())
          FEDs.push_back(feds[i]);
      }
  ...
  if (EMdoNonIsolated_) {
    for (l1extra::L1EmParticleCollection::const_iterator emItr = emNonIsolColl->begin(); emItr != emNonIsolColl->end();
      ...
      float pt = emItr->pt();
      if (debug_)
        std::cout << " Here is an L1 nonisoEM candidate of pt " << pt << std::endl;
      if (pt < Ptmin_noniso_)
        continue;
      // Access the GCT hardware object corresponding to the L1Extra EM object.
      int etaIndex = emItr->gctEmCand()->etaIndex();
...
void ESListOfFEDSProducer::Muon(edm::Event& e,
                                ...
                                std::vector<int>& FEDs) {
  if (debug_)
    std::cout << std::endl << std::endl << " enter in EcalListOfFEDSProducer::Muon" << std::endl;
  ...
  for (L1MuonParticleCollection::const_iterator it = muColl->begin(); it != muColl->end(); it++) {
    ...
    if (debug_)
      std::cout << " here is a L1 muon Seed  with (eta,phi) = " << eta << " " << phi << " and pt " << pt << std::endl;
    if (pt < Ptmin_muon_)
      continue;
...
void ESListOfFEDSProducer::Jets(edm::Event& e,
  ...
  if (debug_)
    std::cout << std::endl << std::endl << " enter in EcalListOfFEDSProducer::Jets" << std::endl;
  double epsilon = 0.01;
  ...
  if (JETSdoCentral_) {
    ...
    for (L1JetParticleCollection::const_iterator it = jetColl->begin(); it != jetColl->end(); it++) {
      ...
      if (debug_)
        std::cout << " here is a L1 CentralJet Seed  with (eta,phi) = " << eta << " " << phi << " and pt " << pt
                  << std::endl;
      if (pt < Ptmin_jets_)
        continue;

      std::vector<int> feds =
          ListOfFEDS(eta, eta, phi - epsilon, phi + epsilon, JETSregionEtaMargin_, JETSregionPhiMargin_);
  ...
  if (JETSdoForward_) {
    ...
    for (L1JetParticleCollection::const_iterator it = jetColl->begin(); it != jetColl->end(); it++) {
      ...
      if (debug_)
        std::cout << " here is a L1 ForwardJet Seed  with (eta,phi) = " << eta << " " << phi << " and pt " << pt
                  << std::endl;
      if (pt < Ptmin_jets_)
        continue;
  ...
  if (JETSdoTau_) {
    ...
    for (L1JetParticleCollection::const_iterator it = jetColl->begin(); it != jetColl->end(); it++) {
      ...
      if (debug_)
        std::cout << " here is a L1 TauJet Seed  with (eta,phi) = " << eta << " " << phi << " and pt " << pt
                  << std::endl;
      if (pt < Ptmin_jets_)
        continue;
...
std::vector<int> ESListOfFEDSProducer::ListOfFEDS(
    double etaLow, double etaHigh, double phiLow, double phiHigh, double etamargin, double phimargin) {
  std::vector<int> FEDs;

  if (phimargin > Geom::pi())
    phimargin = Geom::pi();

  if (debug_)
    std::cout << " etaLow etaHigh phiLow phiHigh " << etaLow << " " << etaHigh << " " << phiLow << " " << phiHigh
              << std::endl;
  ...
  if (dphi > Geom::pi()) {
    ...
    if (debug_)
      std::cout << "fed_low1 fed_low2 " << fed_low1 << " " << fed_low2 << std::endl;
    if (fed_low1 == fed_low2)
      all = true;
    int fed_hi1 = TheMapping->GetFED(etaHigh, phiMinus * 180. / Geom::pi());
    int fed_hi2 = TheMapping->GetFED(etaHigh, phiPlus * 180. / Geom::pi());
    if (debug_)
      std::cout << "fed_hi1 fed_hi2 " << fed_hi1 << " " << fed_hi2 << std::endl;
    if (fed_hi1 == fed_hi2)
      all = true;
  }
  ...
  if (all) {
    if (debug_)
      std::cout << " unpack everything in phi ! " << std::endl;
    phiMinus = -20 * Geom::pi() / 180.;  // -20 deg
    phiPlus = -40 * Geom::pi() / 180.;   // -20 deg
  }

  if (debug_)
    std::cout << " with margins : " << etaLow << " " << etaHigh << " " << phiMinus << " " << phiPlus << std::endl;
