...
std::pair<float, float> EgammaHLTHcalIsolation::getSum(
    const float candEta,
    const float candPhi,
    const HBHERecHitCollection* hbhe,
    const CaloGeometry* geometry,
    const HcalSeverityLevelComputer* hcalSevLvlAlgo,
    const HcalChannelQuality* dbHcalChStatus) const  //note last two pointers can be NULL
{
  float sumE = 0.;
  float sumEt = 0.;
...
bool EgammaHLTHcalIsolation::acceptHit_(
    const HcalDetId id, const GlobalPoint& pos, const float hitEnergy, const float candEta, const float candPhi) const {
  if (passMinE_(hitEnergy, id) &&
      passDepth_(id)) {  //doing the energy and depth cuts first will avoid potentially slow eta calc
    ...
    if (dR2 >= innerConeSq && dR2 < outerConeSq) {  //pass inner and outer cone cuts
      ...
      if (passMinEt_(hitEt, id))
        return true;  //and we've passed the last requirement
    }                 //end dR check
...
bool EgammaHLTHcalIsolation::passMinE_(float energy, const HcalDetId id) const {
  if (id.subdet() == HcalBarrel && energy >= eMinHB_)
    return true;
  else if (id.subdet() == HcalEndcap && energy >= eMinHE_)
    return true;
  else
    return false;
}
...
bool EgammaHLTHcalIsolation::passMinEt_(float et, const HcalDetId id) const {
  if (id.subdet() == HcalBarrel && et >= etMinHB_)
    return true;
  else if (id.subdet() == HcalEndcap && et >= etMinHE_)
    return true;
  else
    return false;
}
...
bool EgammaHLTHcalIsolation::passDepth_(const HcalDetId id) const {
  if (depth_ == -1)
    return true;  //I wish we had chosen 0 as all depths but EgammaTowerIsolation chose -1 and 0 as invalid
  else if (getEffectiveDepth(id) == depth_)
    return true;
  else
    return false;
}
...
bool EgammaHLTHcalIsolation::passCleaning_(const CaloRecHit* hit,
  ...
  if (hcalSevLvlComp == nullptr || hcalChanStatus == nullptr)
    return true;  //return true if we dont have valid pointers
  ...
  if (severityLevel == 0)
    ...
    return useRecoveredHcalHits_;
  else if (severityLevel <= hcalAcceptSeverityLevel_)
    return true;
  else
    return false;
}
...
int EgammaHLTHcalIsolation::getEffectiveDepth(const HcalDetId id) {
  ...
  int depth = id.depth();
  if (iEtaAbs <= 17 || (iEtaAbs <= 29 && depth == 1) || (iEtaAbs >= 27 && iEtaAbs <= 29 && depth == 2)) {
    return 1;
  } else
    return 2;
}
