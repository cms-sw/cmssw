...
float EgammaHLTEcalIsolation::isolPtSum(const reco::RecoCandidate* recocandidate,
  ...
  for (std::vector<const reco::SuperCluster*>::const_iterator scItr = sclusters.begin(); scItr != sclusters.end();
       ++scItr) {
  ...
  for (std::vector<const reco::BasicCluster*>::const_iterator cItr = bclusters.begin(); cItr != bclusters.end();
       ++cItr) {
    cluster = *cItr;
