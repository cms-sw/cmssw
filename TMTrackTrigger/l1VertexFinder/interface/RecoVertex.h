...
public:
  ...
  void              insert(const vertexFinder::L1fittedTrack* fitTrack)     { tracks_.push_back(fitTrack);} // if(fitTrack->getMatchedTP()!= nullptr and fitTrack->getMatchedTP()->physicsCollision()) trueTracks_.insert(fitTrack->getMatchedTP());}
  // FIXME: Re-add if(fitTrack->getMatchedTP() ...) statement 
...
private:
  ...
  std::vector<const vertexFinder::L1fittedTrack*>   tracks_;
