...
// PART I: get z-projections with z-weights
   ...
   {//loop on selected jets
     ...
     {//loop on pixel modules
        ...
        if ((std::abs(deltaPhi(selectedJet->momentum().Phi(),modulepos.phi()))< m_maxDeltaPhi*2)&&(std::abs(zmodule)<(m_maxZ+barrel_lenght))){//if it is a compatible module
        for(const auto & j : detset) 
	  ...
	  const SiPixelCluster & aCluster =  j;
		  ...
		  (m_barrel && std::abs(modulepos.z())<barrel_lenght && std::abs(deltaPhi(selectedJet->momentum().Phi(),v_bs.phi())) <= m_maxDeltaPhi ) || //barrel
		  (m_endCap && std::abs(modulepos.z())>barrel_lenght && std::abs(deltaPhi(selectedJet->momentum().Phi(),v_bs.phi())) <= m_maxDeltaPhi_EC )  //EC
            )
            {
		...
		//calculate zWeight
			...
			float weight_rho = ((m_weight_rho_up - rho)/m_weight_rho_up);
		    	...
		    	float weight_dPhi = exp(-  std::abs(deltaPhi(selectedJet->momentum().Phi(),v_bs.phi()))/m_weight_dPhi );
	        ...
	        {// EC
	        	//calculate weight_dPhi
		    	float weight_dPhi = exp(-  std::abs(deltaPhi(selectedJet->momentum().Phi(),v_bs.phi())) /m_weight_dPhi_EC );
	        	//calculate the final weight
  ...
  for(float & zWeight : zWeights) {zWeightsSquared.push_back(zWeight*zWeight);}
