...
class PixelVertexHeterogeneousProducer
...
public:
  ...
  void beginStreamGPUCuda(edm::StreamID streamId, cuda::stream_t<> &cudaStream) override { m_gpuAlgo.allocate(); }
...
void PixelVertexHeterogeneousProducer::acquireGPUCuda(const edm::HeterogeneousEvent &e,
...
void PixelVertexHeterogeneousProducer::produceGPUCuda(edm::HeterogeneousEvent &e,
                                                      const edm::EventSetup &es,
                                                      cuda::stream_t<> &cudaStream) {
  ...
  if (verbose_)
    std::cout << "PixelVertexHeterogeneousProducer"
              << ": Found " << tracks.size() << " tracks in TrackCollection"
              << "\n";
  ...
  if (verbose_)
    std::cout << "found " << gpuProduct.nVertices << " vertices on GPU using " << gpuProduct.nTracks << " tracks"
              << std::endl;
  if (verbose_)
    std::cout << "original tuple size " << (*tuples_).indToEdm.size() << std::endl;
  ...
  for (int j = int(gpuProduct.nVertices) - 1; j >= 0; --j) {
    ...
    for (auto k = 0U; k < gpuProduct.nTracks; ++k) {
      if (gpuProduct.ivtx[k] == int(i))
        itrk.push_back(gpuProduct.itrk[k]);
    }
    auto nt = itrk.size();
    if (nt == 0) {
  ...
  if (verbose_) {
    ...
    for (unsigned int i = 0; i < vertexes->size(); ++i) {
      edm::LogInfo("PixelVertexHeterogeneousProducer")
    ...
    for (unsigned int i = 0; i < vertexes->size(); ++i) {
      std::cout << "Vertex number " << i << " has " << (*vertexes)[i].tracksSize() << " tracks with a position of "
                << (*vertexes)[i].z() << " +- " << std::sqrt((*vertexes)[i].covariance(2, 2)) << " chi2 "
                << (*vertexes)[i].normalizedChi2() << std::endl;
    }
  }
...
void PixelVertexHeterogeneousProducer::produceCPU(edm::HeterogeneousEvent &iEvent, const edm::EventSetup &iSetup) {
