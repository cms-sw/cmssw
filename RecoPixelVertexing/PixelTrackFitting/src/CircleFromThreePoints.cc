...
void CircleFromThreePoints::init(const Vector2D& b, const Vector2D& c, const Vector2D& offset, double precision) {
  double b2 = b.mag2();
  double c2 = c.mag2();
  ...
  if (fabs(b.x()) > fabs(b.y())) {  // solve for y first
    ...
    if (fabs(div) < precision)
      theCurvature = 0;  // if the 3 points lie on a line
    else {
      oY = (k * b2 - c2) / div;
