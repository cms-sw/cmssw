...
LowPtPixelFitterByHelixProjections::LowPtPixelFitterByHelixProjections(
   const edm::ParameterSet& cfg) : theConfig(cfg), theTracker(0), theField(0),
theTTRecHitBuilder(0) { }

reco::Track* LowPtPixelFitterByHelixProjections::run(
...
//    GlobalPoint p = det->surface().toGlobal( (**ih).localPosition());
  ...
  int charge = LowPtPixelFitterByHelixProjections::charge(points);
  ...
  float valPhi = LowPtPixelFitterByHelixProjections::phi(center.x(), center.y(), charge);
  float errPhi = 0.002;

  float valZip = zip(valTip, curvature, points[0],points[1]);
  float errZip = sqrt(errZip2(valPt, points.back().eta()));

  float valCotTheta = LowPtPixelFitterByHelixProjections::cotTheta
    (points[0],points[1], 1/curvature,valPhi,valTip,valZip);
...
int LowPtPixelFitterByHelixProjections::charge(const vector<GlobalPoint> & points) const
{
...
float LowPtPixelFitterByHelixProjections::cotTheta(
...
float LowPtPixelFitterByHelixProjections::phi(float xC, float yC, int charge) const{
  float phiC = 0.;
...
float LowPtPixelFitterByHelixProjections::zip(float d0, float curv, 
...
double LowPtPixelFitterByHelixProjections::errZip2( float apt, float eta) const 
{
  double ziperr=0;
...
double LowPtPixelFitterByHelixProjections::errTip2(float apt, float eta) const
{
  float pt = (apt <= 10.) ? apt : 10.;
  double p1=0, p2=0;
  float feta = fabs(eta);
