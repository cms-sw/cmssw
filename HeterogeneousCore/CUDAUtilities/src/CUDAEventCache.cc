...
namespace cudautils {
  ...
  std::shared_ptr<cuda::event_t> CUDAEventCache::getCUDAEvent() {
    return cache_[cuda::device::current::get().id()].makeOrGet([](){
        ...
        return std::make_unique<cuda::event_t>(current_device.create_event(// default; we should try to avoid explicit synchronization, so maybe the value doesn't matter much?
