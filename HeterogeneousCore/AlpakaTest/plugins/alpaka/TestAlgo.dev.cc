...
namespace {
  ...
  public:
    ...
    ALPAKA_FN_ACC void operator()(TAcc const& acc, portabletest::TestSoA::TrivialView view, int32_t size) const {
...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
  ...
  void TestAlgo::fill(Queue& queue, portabletest::TestDeviceCollection& collection) const {
    ...
    alpaka::exec<Acc1D>(queue, workDiv, TestAlgoKernel{}, *collection, collection->soaMetadata().size());
  }
