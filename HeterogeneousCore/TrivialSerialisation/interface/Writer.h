...
namespace ngt {
  ...
  public:
    ...
    void initialize(ngt::AnyBuffer const& args) override {
      if constexpr (not HasValidInitialize<T>) {
        ...
        static_assert(not HasTrivialCopyProperties<T>);
      } else if constexpr (not HasTrivialCopyProperties<T>) {
        MemoryCopyTraits<T>::initialize(object());
      } else {
        MemoryCopyTraits<T>::initialize(object(), args.cast_to<TrivialCopyProperties<T>>());
      }
    }
