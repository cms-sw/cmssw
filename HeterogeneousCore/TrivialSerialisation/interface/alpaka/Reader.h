...
namespace ALPAKA_ACCELERATOR_NAMESPACE::ngt {
  ...
  // Reader for device products.  Extends the host Reader with a constructor that
  // accepts a bare T const& (for use with device::Event::get(), which strips the
  // Wrapper<DeviceProduct<T>> layers).
  template <typename T>
