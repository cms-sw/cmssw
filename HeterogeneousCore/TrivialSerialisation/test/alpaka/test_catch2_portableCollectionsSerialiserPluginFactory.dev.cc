...
/*
 * This test demonstrates how the TrivialSerialiser can be used to copy portable types
 * (PortableCollections, PortableObjects, and multi-block collections) of which only the
 * runtime type information is known. The portable data is wrapped in
 * edm::Wrapper<edm::DeviceProduct<T>> and accessed via a WrapperBase pointer.
 *
 * For each type, the test performs the following steps:
 * - Create a portable type and initialize it with some data, wrapped in DeviceProduct.
...
TEST_CASE("Test MemoryCopyTraits", "[MemoryCopyTraits]") {
  ...
  SECTION("PortableCollection<Device, ALPAKA_ACCELERATOR_NAMESPACE::portabletest::TestSoALayout<128, false>>") {
    ...
    using DeviceProductType = edm::DeviceProduct<PortableCollectionType>;
    ...
    for (auto const& device : devices) {
      ...
      checkPortableCollection(cloneptr->bareProduct().getSynchronized());
    }
  }

  SECTION("DeviceProduct<PortableDeviceObject>") {
    using DeviceObjectType = alpaka_portable_test::TestDeviceObject;
    using DeviceProductType = edm::DeviceProduct<DeviceObjectType>;
    ...
    for (auto const& device : devices) {
      ...
      alpaka::memcpy(queue, hostClone.buffer(), cloneptr->bareProduct().getSynchronized().buffer());
      alpaka::wait(queue);
  ...
  SECTION("DeviceProduct<PortableDeviceCollection2>") {
    ...
    using DeviceProductType = edm::DeviceProduct<DeviceCollection2Type>;
    ...
    for (auto const& device : devices) {
      ...
      alpaka::memcpy(queue, verifyHost.buffer(), cloneptr->bareProduct().getSynchronized().buffer());
      alpaka::wait(queue);
  ...
  SECTION("DeviceProduct<PortableDeviceCollection3>") {
    ...
    using DeviceProductType = edm::DeviceProduct<DeviceCollection3Type>;
