  ...
  private:
    ...
    edm::EDPutToken produces(edm::TypeID hostProductType,
      ...
      if constexpr (not detail::useProductDirectly) {
        static_assert(std::is_same_v<std::invoke_result_t<TCopyAsync, Queue&, edm::WrapperBase const&>, std::any>,
        ...
        assert(hostProductType == deviceProductType &&
               "On a backend where host data products are used directly on the device (e.g. the synchronous CPU "
               "backend), the host and device products must be the same");
      }
    ...
    edm::EDPutToken deviceProduces(edm::TypeID deviceProductType,
      ...
      if constexpr (not detail::useProductDirectly) {
        static_assert(
            std::is_same_v<std::invoke_result_t<TCopyAsync, Queue&, EDMetadata&, edm::WrapperBase const&>, std::any>,
            "copyAsync must take (Queue&, EDMetadata&, edm::WrapperBase const&) and return std::any");
        static_assert(std::is_same_v<std::invoke_result_t<TTransform, std::any&>, std::unique_ptr<edm::WrapperBase>>,
                      "transform must take (std::any&) and return std::unique_ptr<edm::WrapperBase>");
