...
void TritonMemResource<IO>::set() {
  tritonThrowIfError(data_->data_->SetSharedMemory(name_, data_->totalByteSize_, 0),
...
void TritonInputHeapResource::copyInput(const void* values, size_t offset) {
  tritonThrowIfError(data_->data_->AppendRaw(reinterpret_cast<const uint8_t*>(values), data_->byteSizePerBatch_),
...
const uint8_t* TritonOutputHeapResource::copyOutput() {
  ...
  tritonThrowIfError(data_->result_->RawData(data_->name_, &values, &contentByteSize),
...
TritonCpuShmResource<IO>::TritonCpuShmResource(TritonData<IO>* data, const std::string& name, size_t size)
  ...
  tritonThrowIfError(this->data_->client()->RegisterSystemSharedMemory(this->name_, this->name_, this->size_),
...
void TritonCpuShmResource<IO>::close() {
  ...
  tritonThrowIfError(this->data_->client()->UnregisterSystemSharedMemory(this->name_),
...
TritonGpuShmResource<IO>::TritonGpuShmResource(TritonData<IO>* data, const std::string& name, size_t size)
  ...
  tritonThrowIfError(this->data_->client()->RegisterCudaSharedMemory(this->name_, *handle_, deviceId_, this->size_),
...
void TritonGpuShmResource<IO>::close() {
  if (this->closed_)
    return;
  tritonThrowIfError(this->data_->client()->UnregisterCudaSharedMemory(this->name_),
