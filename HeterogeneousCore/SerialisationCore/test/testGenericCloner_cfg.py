...
process.cloneInt = cms.EDProducer("ngt::GenericCloner",
...
process.taskInt = cms.Task(
    process.produceInt,
    process.cloneInt
)

process.pathInt = cms.Path(
    process.validateInt,
    process.taskInt
)
...
process.cloneString = cms.EDProducer("ngt::GenericCloner",
...
process.taskString = cms.Task(
    process.produceString,
    process.cloneString
)

process.pathString = cms.Path(
...
process.cloneIdsByLabel = cms.EDProducer("ngt::GenericCloner",
    eventProducts = cms.vstring("eventIds"),
    verbose = cms.untracked.bool(True)
)

process.cloneIdsByBranch = cms.EDProducer("ngt::GenericCloner",
...
process.taskIds = cms.Task(
    process.eventIds,
    process.cloneIdsByLabel,
    process.cloneIdsByBranch
)

process.pathIds = cms.Path(
    process.validateIdsByLabel +
    process.validateIdsByBranch,
    process.taskIds
)
...
process.cloneTransient = cms.EDProducer("ngt::GenericCloner",
    eventProducts = cms.vstring("produceTransient"),
    verbose = cms.untracked.bool(True)
)

process.pathTransient = cms.Path(
    process.produceTransient +
    process.cloneTransient
)
