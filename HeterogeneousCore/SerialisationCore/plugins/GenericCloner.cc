 ...
 * configuration, using either the plugin-based NGT trivial serialisation, or
 * the products' ROOT dictionaries.
 *
 * The products can be specified either as module labels (e.g. "<module label>")
 * or as branch names (e.g. "<product type>_<module label>_<instance name>_<process name>").
 *
 * If a module label is used, no underscore (_) must be present; this module
 * will clone all the products produced by that module, including those produced
 * by the Transformer functionality (such as the implicitly copied-to-host
 ...
 *
 * will cause "module" to run, along with automatic copy of its device products
 * to the host, and will attempt to clone all device and host products.
...
namespace ngt {
  ...
  GenericCloner::GenericCloner(edm::ParameterSet const& config)
    ...
    callWhenNewProductsRegistered([this](edm::ProductDescription const& product) {
      ...
      switch (product.branchType()) {
        case edm::InEvent:
          // Do not clone edm::PathStatus and edm::EndPathStatus objects.
          ...
          for (auto& pattern : eventPatterns_) {
            if (pattern.match(product)) {
              ...
              if (verbose_) {
                edm::LogInfo("GenericCloner") << "will clone Event product " << product.branchName() << " of type '"
                                              << product.unwrappedType() << "'";
              }
              Entry entry;
              entry.objectType_ = product.unwrappedType();
              entry.wrappedType_ = product.wrappedType();
              // Move this to EDConsumerBase::consumes() ?
  ...
  void GenericCloner::produce(edm::StreamID /*unused*/, edm::Event& event, edm::EventSetup const& /*unused*/) const {
    for (auto& product : eventProducts_) {
      // Get the product from the Event, as a const WrapperBase pointer.
      edm::Handle<edm::WrapperBase> handle(product.objectType_.typeInfo());
      event.getByToken(product.getToken_, handle);
      const edm::WrapperBase* wrapper = handle.product();

      // Get for a Serialiser for this object from the plugin factory, if it is available for this type.
      std::unique_ptr<ngt::SerialiserBase> serialiser{
          ngt::SerialiserFactory::get()->tryToCreate(product.objectType_.typeInfo().name())};

      if (serialiser) {
        edm::LogInfo("GenericCloner") << "Found a Serialiser for type '"
                                      << edm::typeDemangle(product.objectType_.typeInfo().name()) << "'.";

        // If the type has a Serialiser, initialise the corresponding Reader and Writer.
        ...
        edm::LogInfo("GenericCloner") << "No Serialiser for type '"
                                      << edm::typeDemangle(product.objectType_.typeInfo().name())
        ...
        // Move the wrapped clone into the Event.
...
DEFINE_FWK_MODULE(ngt::GenericCloner);
