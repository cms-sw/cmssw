...
enum class RangeType {
  Default, ExtentLimited, ExtentLimitedWithShift
};
...
struct testWordDivisionDefaultRange {
  ...
  ALPAKA_FN_ACC void operator() (TAcc const & acc, size_t size, size_t shift, size_t *globalCounter) const {
    size_t & counter = ( loopScope == LoopScope::Grid ? *globalCounter : alpaka::declareSharedVar<size_t, __COUNTER__>(acc));
    ...
    if constexpr (rangeType == RangeType::Default)
      ...
      for ([[maybe_unused]]auto  idx: elements_with_stride(acc, size, shift))
        alpaka::atomicAdd(acc, &counter, 1ul, alpaka::hierarchy::Blocks{});
    alpaka::syncBlockThreads(acc);
...
int main() {
  ...
  for (auto const& device : devices) {
    ...
    for (size_t blocks = 1; blocks < GridSize * 3; blocks++)
      for (auto sizeFuzz: std::initializer_list<ssize_t>{ - BlockSize / 2, -1, 0, 1, BlockSize/2 })
        for (auto shift: std::initializer_list<ssize_t>{0, 1, BlockSize / 2, BlockSize - 1, BlockSize, BlockSize + 1, BlockSize + BlockSize / 2, 2*BlockSize -1, 2*BlockSize, 2*BlockSize + 1}) {
         // Grid level iteration: we need to initialize/check at the grid level
          ...
          alpaka::enqueue(queue, alpaka::createTaskKernel<Acc1D>(workdiv, 
             testWordDivisionDefaultRange<RangeType::Default, LoopScope::Grid>{}, blocks* BlockSize + sizeFuzz, shift, counter_d.data()));
          alpaka::memcpy(queue, counter_h, counter_d);
          alpaka::wait(queue);
          auto expected = expectedCount<RangeType::Default, LoopScope::Grid>(workdiv, blocks* BlockSize + sizeFuzz, shift);
          assert(*counter_h.data() == expected);

          // ExtentLimited range
          alpaka::memset(queue, counter_d, 0);
          alpaka::enqueue(queue, alpaka::createTaskKernel<Acc1D>(workdiv, 
             testWordDivisionDefaultRange<RangeType::ExtentLimited, LoopScope::Grid>{}, blocks* BlockSize + sizeFuzz, shift, counter_d.data()));
          alpaka::memcpy(queue, counter_h, counter_d);
          alpaka::wait(queue);
          expected = expectedCount<RangeType::ExtentLimited, LoopScope::Grid>(workdiv, blocks* BlockSize + sizeFuzz, shift);
          assert(*counter_h.data() == expected);

          // ExtentLimitedWithShift range
          alpaka::memset(queue, counter_d, 0);
          alpaka::enqueue(queue, alpaka::createTaskKernel<Acc1D>(workdiv, 
             testWordDivisionDefaultRange<RangeType::ExtentLimitedWithShift, LoopScope::Grid>{}, blocks* BlockSize + sizeFuzz, shift, counter_d.data()));
          alpaka::memcpy(queue, counter_h, counter_d);
          alpaka::wait(queue);
          expected =  expectedCount<RangeType::ExtentLimitedWithShift, LoopScope::Grid>(workdiv, blocks* BlockSize + sizeFuzz, shift);
          assert(*counter_h.data() == expected);
