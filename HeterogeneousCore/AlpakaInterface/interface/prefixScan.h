  ...
  namespace alpakatools {
    ...
    ALPAKA_FN_ACC ALPAKA_FN_INLINE void blockPrefixScan(
      ...
      if constexpr (!requires_single_thread_per_block_v<TAcc>) {
        ...
        ALPAKA_ASSERT_OFFLOAD(size <= alpaka::warp::getSize(acc) * warpSize);
    ...
    struct multiBlockPrefixScan {
      ...
      ALPAKA_FN_ACC void operator()(const TAcc& acc,
        ...
        // Get shared variable. TODO: The workspace is in fact not needed at all for single thread multi element accelerators TODO
        T* ws = nullptr;
        if constexpr (!requires_single_thread_per_block_v<TAcc>) {
