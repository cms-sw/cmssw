...
namespace cms::alpakatools {
  ...
  inline constexpr Idx min(Idx const x, Idx const y) { return (y > x) ? x : y; }
    ...
    public:
      ...
      class const_iterator {
        ...
        ALPAKA_FN_ACC inline const_iterator(Idx elements, Idx stride, Idx extent, Idx first)
            : elements_{elements},
              stride_{stride},
              extent_{extent},
              first_{min(first, extent)},
              index_{first_},
              range_{min(first + elements, extent)} {}
      ...
      public:
        ...
        ALPAKA_FN_ACC inline const_iterator& operator++() {
          ...
          first_ += stride_;
          index_ = first_;
          range_ = min(first_ + elements_, extent_);
          if (index_ < extent_)
            return *this;
    ...
    public:
      ...
      private:
        ...
        ALPAKA_FN_ACC inline constexpr bool nth_strided_loop() {
          ...
          index_[I] = first_[I];
          range_[I] = min(first_[I] + loop_->elements_[I], loop_->extent_[I]);
          return overflow;
        }
    ...
    public:
      ...
      class const_iterator {
        ...
        ALPAKA_FN_ACC inline const_iterator(Idx stride, Idx extent, Idx first)
            : stride_{stride}, extent_{extent}, first_{min(first, extent)} {}
