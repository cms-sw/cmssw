...
void ProductMetadataBuilder::receiveMetadata(MPI_Message message, int size) {
  assert(size_ == 0 && "metadata receive buffer must be empty");
...
ProductMetadata ProductMetadataBuilder::getNext() {
  if (readOffset_ >= size_) throw std::out_of_range("No more metadata entries");
...
void ProductMetadataBuilder::ensureCapacity(size_t needed) {
  if (size_ + needed <= capacity_) return;
