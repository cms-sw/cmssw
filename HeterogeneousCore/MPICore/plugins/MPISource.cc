...
private:
  ...
  std::once_flag modulesCheckOnce_;
      ...
      mode_(parseMode(config.getUntrackedParameter<std::string>("mode")))  //
{
  ...
  // Receive the number of streams that each follower process will handle and the number of
  // data channels available per controller stream
  controller_.receiveStreamCount(nControllerStreams_);
  LogDebug("MPI") << "Source will handle " << nControllerStreams_ << " streams";
  controller_.receiveDataChannelCount(nDataChannels_);
  ...
  while (!worker.stop.load()) {
    ...
    switch (status.MPI_TAG) {
      ...
      case EDM_MPI_BeginRun: {
        ...
        // Protect shared history_ access now that MPIController is a global module and receive the ProcessHistory
        {
...
bool MPISource::setRunAndEventInfo(edm::EventID& event,
  ...
  std::call_once(modulesCheckOnce_, [this] {
    MPI_Status status;
    MPI_Message message;
    controller_.probeAny(message, status);

    if (status.MPI_TAG != EDM_MPI_SendModulesInfo) {
      throw cms::Exception("InvalidValue") << "The MPISource has received an unexpected message with tag "
                                           << status.MPI_TAG << " while expecting EDM_MPI_SendModulesInfo";
    }
