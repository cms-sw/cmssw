...
public:
  explicit PrintCPPNames(edm::ParameterSet const&) {
    callWhenNewProductsRegistered(
        [](edm::ProductDescription const& product) {
          ...
          if (product.className() == kPathStatus ||
              product.className() == kEndPathStatus)
            return;

          std::cout
              << "PrintCPPNames: considering product "
              << product.friendlyClassName() << '_'
              << product.moduleLabel() << '_'
              << product.productInstanceName() << '_'
              << product.processName()
              << " of type " << product.unwrappedType().name()
              << " branch type " << product.branchType()
              << '\n';
        });
  }
