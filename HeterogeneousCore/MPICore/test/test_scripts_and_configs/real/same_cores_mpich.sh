...
BASE_DIR="../../test_results/mpich/one_buffer_serial/same_cores"
mkdir -p "$BASE_DIR"
...
for combo in "${thread_stream_combos[@]}"; do
    ...
    for i in $(seq 1 $runs); do
        ...
        mpirun -np 1 env MPIR_CVAR_CH4_DEVICE=ch4:ucx UCX_TLS=xpmem,self,shm numactl --physcpubind=0-"${end_core}" cmsRun "$script_remote" \
