...
namespace edm {
  ...
  public:
    // default constructor
    Handle() : HandleBase(), type_(nullptr) {}

    // throws exception if `type` is not a known C++ class type
    Handle(std::string const& type) : HandleBase(), type_(&TypeWithDict::byName(type).typeInfo()) {}

    Handle(WrapperBase const* wrapper, Provenance const* prov, std::string const& type)
        : HandleBase(wrapper, prov), type_(&TypeWithDict::byName(type).typeInfo()) {}

    // throws exception if `type` is invalid
    Handle(std::type_info const& type) : HandleBase(), type_(&type) {}

    Handle(WrapperBase const* wrapper, Provenance const* prov, std::type_info const& type)
        : HandleBase(wrapper, prov), type_(&type) {}

    // used when the attempt to get the data failed
    Handle(std::shared_ptr<HandleExceptionFactory>&& whyFailed) : HandleBase(std::move(whyFailed)), type_(nullptr) {}
    ...
    WrapperBase const* product() const { return static_cast<WrapperBase const*>(productStorage()); }

    WrapperBase const* operator->() const { return product(); }

    WrapperBase const& operator*() const { return *product(); }

    std::type_info const& typeInfo() const { return *type_; }
  ...
  inline void swap(Handle<WrapperBase>& a, Handle<WrapperBase>& b) { a.swap(b); }

  // specialise the conversion from a BasicHandle into a Handle<WrapperBase>

  template <>
  inline void convert_handle(BasicHandle&& bh, Handle<WrapperBase>& result) {
    if (bh.failedToGet()) {
  ...
  public:
    ...
    OrphanHandle() : OrphanHandleBase(), type_(nullptr) {}
    ...
    OrphanHandle(WrapperBase const* wrapper, std::string const& type, ProductID const& id)
        : OrphanHandleBase(wrapper, id), type_(&TypeWithDict::byName(type).typeInfo()) {}
    ...
    OrphanHandle(WrapperBase const* wrapper, std::type_info const& type, ProductID const& id)
        : OrphanHandleBase(wrapper, id), type_(&type) {}
  ...
  inline void swap(OrphanHandle<WrapperBase>& a, OrphanHandle<WrapperBase>& b) { a.swap(b); }

  // specialise the Event methods for getting a WrapperBase

  template <>
  inline bool Event::getByLabel(InputTag const& tag, Handle<WrapperBase>& result) const {
    result.clear();
  ...
  template <>
  inline bool Event::getByLabel(std::string const& label,
                                std::string const& productInstanceName,
                                Handle<WrapperBase>& result) const {
    result.clear();
    BasicHandle bh = provRecorder_.getByLabel_(
        TypeID(result.typeInfo()), label, productInstanceName, emptyString_, moduleCallingContext_);
    convert_handle(std::move(bh), result);  // throws on conversion error
    if (result.failedToGet()) {
      return false;
    }
  ...
  template <>
  inline bool Event::getByToken(EDGetToken token, Handle<WrapperBase>& result) const {
    result.clear();
  ...
  template <>
  inline bool Event::getByToken(EDGetTokenT<WrapperBase> token, Handle<WrapperBase>& result) const {
    result.clear();
    BasicHandle bh = provRecorder_.getByToken_(TypeID(result.typeInfo()), PRODUCT_TYPE, token, moduleCallingContext_);
    convert_handle(std::move(bh), result);  // throws on conversion error
    if (result.failedToGet()) {
  ...
  inline OrphanHandle<WrapperBase> Event::put(EDPutToken token, std::unique_ptr<WrapperBase> product) {
    ...
    if (UNLIKELY(product.get() == nullptr)) {
      ...
      principal_get_adapter_detail::throwOnPutOfNullProduct(
          "Event", TypeID(typeInfo), provRecorder_.productInstanceLabel(token));
    }
    ...
    if (UNLIKELY(provRecorder_.getTypeIDForPutTokenIndex(token.index()) != TypeID(typeInfo))) {
      principal_get_adapter_detail::throwOnPutOfWrongType(typeInfo,
