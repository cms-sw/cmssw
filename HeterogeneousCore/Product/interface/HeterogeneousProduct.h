...
#define DEFINE_DEVICE_PRODUCT(ENUM)                                                  \
  ...
  template <typename T>                                                              \
  struct ProductToEnum<ENUM##Product<T>> {                                           \
    static constexpr const HeterogeneousDevice value = HeterogeneousDevice::k##ENUM; \
  }
...
#undef DEFINE_DEVICE_PRODUCT
  ...
  struct IfInPack<device, Type, Types...> {
    using type = std::conditional_t<device == Type::tag, Type, typename IfInPack<device, Types...>::type>;
  };
  ...
  template <typename FunctionTuple,
            typename ProductTuple,
            typename BitSetArray,
            typename FunctionTupleElement,
            size_t sizeMinusIndex>
  struct CallFunctionIf {
    static bool call(const FunctionTuple& functionTuple, ProductTuple& productTuple, const BitSetArray& bitsetArray) {
      ...
      if (bitsetArray[index].any()) {
        ...
        if (!func) {
          throw cms::Exception("Assert") << "Attempted to call transfer-to-CPU function for device " << index
                                         << " but the std::function object is not valid!";
        }
        func(std::get<index>(productTuple).product(), std::get<0>(productTuple).product());
        return true;
      }
      return CallFunctionIf<FunctionTuple,
                            ProductTuple,
                            BitSetArray,
                            TupleElement_t<index + 1, FunctionTuple>,
                            sizeMinusIndex - 1>::call(functionTuple, productTuple, bitsetArray);
    }
  };
  ...
  struct GetOrTransferProduct {
    template <typename FunctionTuple, typename ProductTuple, typename BitSetArray>
    static const auto& getProduct(const FunctionTuple& functionTuple,
                                  ProductTuple& productTuple,
                                  const BitSetArray& location) {
      constexpr const auto index = static_cast<unsigned int>(device);
      if (!location[index].any()) {
        throw cms::Exception("LogicError")
            << "Called getProduct() for device " << index << " but the data is not there! Location bitfield is "
            << bitsetArrayToString(location);
      }
  ...
  struct GetOrTransferProduct<HeterogeneousDevice::kCPU> {
    ...
    static const auto& getProduct(const FunctionTuple& functionTuple,
      ...
      if (!location[index].any()) {
        auto found =
            CallFunctionIf<FunctionTuple,
                           ProductTuple,
                           BitSetArray,
                           std::tuple_element_t<1, FunctionTuple>,
                           std::tuple_size<BitSetArray>::value - 1>::call(functionTuple, productTuple, location);
        if (!found) {
          throw cms::Exception("LogicError")
              << "Attempted to transfer data to CPU, but the data is not available anywhere! Location bitfield is "
              << bitsetArrayToString(location);
        }
      }
...
class HeterogeneousProductImpl : public HeterogeneousProductBase {
  ...
  using TransferToCPUTuple =
      std::tuple<heterogeneous::Empty,  // no need to transfer from CPU to CPU
                 heterogeneous::CallBackType_t<
                     CPUProduct,
                     std::tuple_element_t<static_cast<unsigned int>(HeterogeneousDevice::kGPUMock), ProductTuple>>,
                 heterogeneous::CallBackType_t<
                     CPUProduct,
                     std::tuple_element_t<static_cast<unsigned int>(HeterogeneousDevice::kGPUCuda), ProductTuple>>>;
  // Some sanity checks
  static_assert(std::tuple_size<ProductTuple>::value == std::tuple_size<TransferToCPUTuple>::value, "Size mismatch");
  static_assert(std::tuple_size<ProductTuple>::value == static_cast<unsigned int>(HeterogeneousDevice::kSize),
                "Size mismatch");

public:
  template <HeterogeneousDevice Device, typename Type>
  struct CanGet {
    using FromType = typename heterogeneous::DataTypeOrEmpty<
        std::tuple_element_t<static_cast<unsigned int>(Device), ProductTuple>>::type;
    static const bool value = std::is_same<Type, FromType>::value;
  };

  template <HeterogeneousDevice Device, typename Type>
  struct CanPut {
    using ToType = typename heterogeneous::DataTypeOrEmpty<
        std::tuple_element_t<static_cast<unsigned int>(Device), ProductTuple>>::type;
  ...
  HeterogeneousProductImpl(heterogeneous::HeterogeneousDeviceTag<Device>,
                           D&& data,
                           HeterogeneousDeviceId location,
                           F transferToCPU) {
    constexpr const auto index = static_cast<unsigned int>(Device);
  ...
  const auto& getProduct() const {
    constexpr const auto index = static_cast<unsigned int>(device);
    static_assert(!std::is_same<std::tuple_element_t<index, ProductTuple>, heterogeneous::Empty>::value,
...
public:
  ...
  const T& get() const {
    ...
    if (typeid(T) != typeid(ref)) {
      throw cms::Exception("LogicError") << "Trying to get HeterogeneousProductImpl " << typeid(T).name()
                                         << " but the product contains " << typeid(ref).name();
    }
