...
void setupMPIAndVectors(MPIData &mpiData, UserChoises &user); //initialize communicator environment for MPI and Resize Vectors with Generating Random numbers.
...
void calculateTimeDuration(Timing &timing, int i, int &root) {
  ...
  if(!root)
  {
    timing.timeInputPreparationRoot[i] = (timing.inputPreparationRoot[1] - timing.inputPreparationRoot[0]);  //getting the time in microseconds
  ...
  else
  {
    ...
    timing.timeOperationOnDeviceByRootHost[i] = (timing.operationOnDeviceByHost[1]- timing.operationOnDeviceByHost[0]); //time taking for Device operation with respect of Host.
...
void calculateAverageDeviation(Timing &timing, int averg, int &root) {
  ...
  for (int i = 0; i < averg; ++i) 
  {
    if(!root)
    {
      ...
      timing.timeInputPreparationRoot[i] = timing.timeInputPreparationRoot[i] * timing.timeInputPreparationRoot[i];  // Square it.
      timing.timeInputPreparationRoot[averg + 1] += timing.timeInputPreparationRoot[i];  //add them togather. averg+1 is location of the Deviation
    ...
    else
    {
      ...
      timing.timeInputPreparationHost[i] = timing.timeInputPreparationHost[i] * timing.timeInputPreparationHost[i];  // Square it.
      timing.timeInputPreparationHost[averg + 1] += timing.timeInputPreparationHost[i];  //add them togather. averg+1 is location of the Deviation
...
Timing blockSendPart1(MPIData& mpiData, Timing &timing, Pointers &pointer, UserChoises &user) {
  ...
  {
    cudaCheck(cudaMalloc((void **)&pointer.dVect1, user.sizeVectorBytes));  //allocate memory space for vector in the global memory of the Device.
    ...
    for (int a = 0; a <= average; ++a) 
    {
      ...
      if (mpiData.rank)  //Only for Workers
      {
        ...
        addVectorsGpu<<<blocks, threads>>>(pointer.dVect1, pointer.dVect2, pointer.dVect3, sizeVector, task);  //call device function to add two vectors and save into vect3Gpu.
    ...
    if (!mpiData.rank)  //Only for root
    {
      ...
      test = checkingResultsPrintout(mpiData.mVectChecking, mpiData.mVectWorker3);  //Checking the results, if error then Print out to the user.
      if(!test)
        exit(-1);
    }
    ...
    if(mpiData.rank)  //Only for Workers
    {
      ...
      for (int a = 0; a <= average; ++a) 
      {
        ...
        if (mpiData.rank)  //Only for Workers
        {
          ...
          cudaCheck(cudaMemcpy(pointer.dVect1, pointer.vect1, user.sizeVectorBytes, cudaMemcpyHostToDevice));  //copy random vector from host to device.
          ...
          cudaCheck(cudaMemcpy(pointer.vect3,pointer.dVect3,user.sizeVectorBytes,cudaMemcpyDeviceToHost));  //copy summing result vector from Device to Host.// Try_Regist(3) delete this
  ...
  bool saveToFile(const std::string &name, const Timing &timing) {
    ...
    file << timing.timeOperationOnDeviceByRootHost[average] << " " << timing.timeOperationOnDeviceByRootHost[average + 1] << std::endl;
    ...
    file << timing.timeOutputPreparationRoot[average] << " " << timing.timeOutputPreparationRoot[average + 1] << std::endl;
...
void printHelp(void)
{
  ...
  std::cout << "\n\t1) mpirun -np <number of Process/ors> -s <size of Vector> -t <number of task> -a <average size> -p <part to run>\n";
  std::cout << "\n\t2) cmsenv_mpirun -np <number of Process/ors> -s <size of Vector> -t <number of task> -a <average size> -p <part to run>\n";
  ...
  std::cout << "\n\tcmsenv_mpirun -H <machine Name as Root>,<machine Name as Host> -np 2 -mca pml ucx -- mpiCudaGeneric -p1 -s200 -t1 -a1";
  std::cout << "\n\n\tFor the Parts, we have in this program 4 Parts:";
  std::cout << "\n\t1)The Root, who does not have a GPU, using MPI Blocking send and receive to Host, The Host is who have a GPU, then Host:";
