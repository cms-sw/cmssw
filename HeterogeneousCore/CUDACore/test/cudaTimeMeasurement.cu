...
void addVectorsCpu(std::vector<float> &vect1, std::vector<float> &vect2, std::vector<float> &vect3); //add two vectors and save the result into a third vector.
...
Timing cudaTimePart1(Timing &timing, Vectors &vect, Pointers &dvect, int size, int startSave); //PArt 1 of Cuda measurement time.
...
void calculateTimeDuration(Timing& timing, int i)
{
  ...
  cudaEventElapsedTime(&timing.operationOnDeviceByDevice, timing.start, timing.stop); //get the time elapse in Device operation with device perspective.
...
void calculateAverageDeviation(Timing& timing)
{
  ...
  for(int i = 0; i < averg; ++i)
  {
    ...
    timing.timeCopyToDevice[averg+1] += timing.timeCopyToDevice[i]; //add them togather. averg+1 is location of the Deviation
  ...
  timing.timeCopyToDevice[averg+1] =  (std::chrono::duration<double, std::micro>) sqrt(timing.timeCopyToDevice[averg+1].count());
  timing.timeOperationOnDeviceByHost[averg+1] =  timing.timeOperationOnDeviceByHost[averg+1]/averg;
  timing.timeOperationOnDeviceByHost[averg+1] = (std::chrono::duration<double, std::micro>) sqrt(timing.timeOperationOnDeviceByHost[averg+1].count());
  timing.timeCopyToHost[averg+1] =  timing.timeCopyToHost[averg+1]/averg;
  timing.timeCopyToHost[averg+1] = (std::chrono::duration<double, std::micro>) sqrt(timing.timeCopyToHost[averg+1].count());
...
void printoutAll(std::vector<Timing>&timing,  bool standerDeviationPrint)
{
  ...
  std::cout << " ";
	...
	{
    if(standerDeviationPrint)
		{std::cout.width(((averageTime.size()+standerDeviation.size())-partsNumberall.size()+1)/2);}//9
    else
    {std::cout.width(((averageTime.size())-partsNumberall.size())/2);}//2
...
bool saveToFile(const std::string& name, const Timing& timing)
{
    ...
    file << timing.timeOperationOnDeviceByHost[averg].count() << " " << timing.timeOperationOnDeviceByHost[averg+1].count() << std::endl;
    file << timing.operationOnDeviceByDeviceAcc[averg] << " " << timing.operationOnDeviceByDeviceAcc[averg+1] << std::endl;
...
Timing cudaTimePart0(Timing &timing, Vectors &vect, Pointers &dvect, int size)
{
  ...
  for( int i = 0; i < averg ; i++)
  {
    ...
    addVectorsGpu<<<blocks,threads>>> (vect.vect1.data(), vect.vect2.data(), vect.vect3Gpu.data(), sizeOfVector, task); //call device function to add two vectors and save into vect3Gpu.
    ...
    test = checkingResultsPrintout(vect.vect3Cpu, vect.vect3Gpu);//Checking the results, if error then Print out to the user.
    if(test)
    {
...
Timing cudaTimePart1(Timing &timing, Vectors &vect, Pointers &dvect, int size, int startSave)
{
    ...
    timing.partChosen = 1;
    CUDA_CHECK(cudaMalloc((void**)&dvect.dVect1, size));//allocate memory space for vector in the global memory of the Device.
  ...
  for( int i = 0; i < averg ; i++)
  {
    ...
    CUDA_CHECK(cudaMemcpy(dvect.dVect1, vect.vect1.data(), size, cudaMemcpyHostToDevice));//copy random vector from host to device.
    ...
    addVectorsGpu<<<blocks,threads>>> (dvect.dVect1, dvect.dVect2, dvect.dVect3, sizeOfVector, task); //call device function to add two vectors and save into vect3Gpu.
    CUDA_CHECK(cudaGetLastError());
    ...
    CUDA_CHECK(cudaMemcpy(vect.vect3Gpu.data(), dvect.dVect3, size, cudaMemcpyDeviceToHost));//copy summing result vector from Device to Host.// Try_Regist(3) delete this
...
timing.operationOnDeviceByHost[0] = std::chrono::steady_clock::now();
CUDA_CHECK(cudaEventRecord(timing.start));
CUDA_CHECK(cudaEventSynchronize(timing.start)); //If the cudaEventBlockingSync flag has not been set, then the CPU thread will busy-wait until the event has been completed by the GPU.
...
Timing cudaTimePart3(Timing &timing, Vectors &vect, Pointers &dvect, int size)
{
  ...
  for( int i = 0; i < averg ; i++)
  {
    ...
    timing.operationOnDeviceByHost[0] = std::chrono::steady_clock::now();
    CUDA_CHECK(cudaEventRecord(timing.start));
    CUDA_CHECK(cudaEventSynchronize(timing.start)); //Waits for an event to complete.If the cudaEventBlockingSync flag has not been set, then the CPU thread will busy-wait until the event has been completed by the GPU.
...
Timing cudaTimePart4(Timing &timing, Vectors &vect, Pointers &dvect, int size)
{
  ...
  for( int i = 0; i < averg ; i++)
  {
    ...
    addVectorsGpu<<<blocks,threads>>> (dvect.dVect1Extra, dvect.dVect2Extra, dvect.dVect3Extra, sizeOfVector, task); //call device function to add two vectors and save into vect3Gpu.
    CUDA_CHECK(cudaGetLastError());
...
Timing cudaTimePart5(Timing &timing, Vectors &vect, Pointers &dvect, int size)
{
  ...
  for( int i = 0; i < averg ; i++)
  {
    ...
    CUDA_CHECK(cudaMemcpy(dvect.dVect1, vect.vect1.data(), size, cudaMemcpyHostToDevice));//copy pinned vector in the host to buffer in the device.
    CUDA_CHECK(cudaMemcpy(dvect.dVect2, vect.vect2.data(), size, cudaMemcpyHostToDevice));
    ...
    CUDA_CHECK(cudaMemcpy(vect.vect3Gpu.data(), dvect.dVect3, size, cudaMemcpyDeviceToHost));//copy  buffer in the device to pinned vector in the host.
    CUDA_CHECK(cudaHostUnregister(vect.vect1.data()));
    CUDA_CHECK(cudaHostUnregister(vect.vect2.data()));
    CUDA_CHECK(cudaHostUnregister(vect.vect3Gpu.data()));
