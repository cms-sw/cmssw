...
private:
  using OutputType =
      HeterogeneousProductImpl<heterogeneous::CPUProduct<unsigned int>,
...
TestHeterogeneousEDProducerGPU::TestHeterogeneousEDProducerGPU(edm::ParameterSet const& iConfig)
    : HeterogeneousEDProducer(iConfig), label_(iConfig.getParameter<std::string>("@module_label")) {
...
void TestHeterogeneousEDProducerGPU::produceGPUCuda(edm::HeterogeneousEvent& iEvent,
  ...
  iEvent.put<OutputType>(
      std::make_unique<TestHeterogeneousEDProducerGPUTask::ResultTypeRaw>(gpuOutput_.first.get(),
