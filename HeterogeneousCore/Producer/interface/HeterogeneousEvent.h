...
#define CASE(ENUM)                                     \
  case ENUM:                                           \
    this->template get<ENUM, Product>(handle, tmp, 0); \
    break
        switch (inputLocation_.deviceType()) {
          ...
          default:
            throw cms::Exception("LogicError") << "edm::HeterogeneousEvent::getByToken(): no case statement for device "
                                               << static_cast<unsigned int>(location().deviceType())
...
#undef CASE
    ...
    void put(std::unique_ptr<Type> product) {
      ...
      event_->put(std::make_unique<HeterogeneousProduct>(
          Product(heterogeneous::HeterogeneousDeviceTag<HeterogeneousDevice::kCPU>(), std::move(*product))));
    }
...
#define CASE(ENUM)                                                                             \
  case ENUM:                                                                                   \
    this->template make<ENUM, Product>(prod, std::move(product), std::move(transferToCPU), 0); \
    break
...
#undef CASE
  ...
  private:
    ...
    typename std::enable_if_t<Product::template CanGet<Device, Type>::value, void> get(
        edm::Handle<Type>& dst, const edm::Handle<HeterogeneousProduct>& src, int) const {
    ...
    void get(edm::Handle<Type>& dst, const edm::Handle<HeterogeneousProduct>& src, long) const {
      throw cms::Exception("Assert") << "Invalid call to get, Device " << static_cast<int>(Device) << " Product "
                                     << typeid(Product).name() << " Type " << typeid(Type).name()
                                     << " CanGet::FromType "
                                     << typeid(typename Product::template CanGet<Device, Type>::FromType).name()
    ...
    typename std::enable_if_t<Product::template CanPut<Device, Type>::value, void> make(
        std::unique_ptr<HeterogeneousProduct>& ret, std::unique_ptr<Type> product, F transferToCPU, int) {
      ret = std::make_unique<HeterogeneousProduct>(Product(
          heterogeneous::HeterogeneousDeviceTag<Device>(), std::move(*product), location(), std::move(transferToCPU)));
    }
    template <HeterogeneousDevice Device, typename Product, typename Type, typename F>
    void make(std::unique_ptr<HeterogeneousProduct>& ret, std::unique_ptr<Type> product, F transferToCPU, long) {
      throw cms::Exception("Assert") << "Invalid call to make, Device " << static_cast<int>(Device) << " Product "
                                     << typeid(Product).name() << " Type " << typeid(Type).name() << " CanPut::ToType "
                                     << typeid(typename Product::template CanPut<Device, Type>::ToType).name()
