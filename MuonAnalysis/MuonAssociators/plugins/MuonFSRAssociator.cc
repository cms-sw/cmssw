...
void MuonFSRAssociator::produce(edm::StreamID streamID, edm::Event& iEvent, const edm::EventSetup& iSetup) const {
  ...
  for (auto muon = muons->begin(); muon != muons->end(); ++muon){
    for(auto iter_pho = photons->begin(); iter_pho != photons->end(); iter_pho++){
      if (iter_pho->hasUserCand("associatedMuon") and iter_pho->userCand("associatedMuon")==reco::CandidatePtr(muons,muon-muons->begin())) muonMapping[muon-muons->begin()] = (iter_pho - photons->begin());
    }
  }
