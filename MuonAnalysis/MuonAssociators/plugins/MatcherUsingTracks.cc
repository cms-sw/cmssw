...
pat::MatcherUsingTracks::MatcherUsingTracks(const edm::ParameterSet &iConfig)
    : srcToken_(consumes<edm::View<reco::Candidate> >(iConfig.getParameter<edm::InputTag>("src"))),
      ...
      writeExtraPATOutput_(
          iConfig.existsAs<bool>("writeExtraPATOutput") ? iConfig.getParameter<bool>("writeExtraPATOutput") : false) {
...
void pat::MatcherUsingTracks::produce(edm::Event &iEvent, const edm::EventSetup &iSetup) {
  using namespace edm;
  using namespace std;
  ...
  if (writeExtraPATOutput_) {
    ...
    for (isrc = 0; isrc < nsrc; ++isrc) {
      if (idxUD[isrc] != -1)
        ptrUD[isrc] = edm::Ptr<pat::UserData>(doneUDVect, idxUD[isrc]);
    }
