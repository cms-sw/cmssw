...
void MuScleFitMuonSelector::selectSimulatedMuons(const edm::Event& event,
  ...
  if (!simTracksFound) {
    simPair.emplace_back(lorentzVector(0., 0., 0., 0.), lorentzVector(0., 0., 0., 0.));
  }
}
...
GenMuonPair MuScleFitMuonSelector::findGenMuFromRes(const reco::GenParticleCollection* genParticles) {
  ...
  for (const auto& genParticle : *genParticles) {
    if (debug_ > 0)
      std::cout << "genParticle has pdgId = " << std::abs(genParticle.pdgId())
                << " and status = " << genParticle.status() << std::endl;
