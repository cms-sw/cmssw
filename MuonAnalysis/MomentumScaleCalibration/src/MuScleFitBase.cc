...
void MuScleFitBase::readProbabilityDistributionsFromFile()
{
  ...
  for (int iY=0; iY<24; iY++) {
    ...
    for (int iy=0; iy<=MuScleFitUtils::nbins; iy++) {
      ...
      if (debug_>2) std::cout << "GLZValue[" << iY << "][500][" << iy << "] = " 
                        << MuScleFitUtils::GLZValue[iY][500][iy] 
                        << " GLZNorm[" << iY << "][" << iy << "] = " 
                        << MuScleFitUtils::GLZNorm[iY][iy] << std::endl;
    }
  }
  ...
  for (int ires=0; ires<6; ires++) {
    ...
    if( nBinsX != MuScleFitUtils::nbins+1 || nBinsY != MuScleFitUtils::nbins+1 ) {
      std::cout << "Error: for histogram \"" << GLZ[ires]->GetName() << "\" bins are not " << MuScleFitUtils::nbins << std::endl; 
      std::cout<< "nBinsX = " << nBinsX << ", nBinsY = " << nBinsY << std::endl;
      exit(1);
    }
    for (int iy=0; iy<=MuScleFitUtils::nbins; iy++) {
      ...
      if (debug_>2) std::cout << "GLValue[" << ires << "][500][" << iy << "] = " 
                        << MuScleFitUtils::GLValue[ires][500][iy] 
                        << " GLNorm[" << ires << "][" << iy << "] = " 
                        << MuScleFitUtils::GLNorm[ires][iy] << std::endl;
    }
  }
...
//   edm::LogInfo("MuScleFit") << "[MuScleFit::readProbabilityDistributions] End Reading MuScleFitLikelihoodPdfRcd" << std::endl;
//   vector<PhysicsTools::Calibration::HistogramD2D>::const_iterator histo = likelihoodPdf->histograms.begin();
//   vector<std::string>::const_iterator name = likelihoodPdf->names.begin();
...
//     int nBinsY = *yBins;
//     if( nBinsX != MuScleFitUtils::nbins+1 || nBinsY != MuScleFitUtils::nbins+1 ) {
//       std::cout << "Error: for histogram \"" << *name << "\" bins are not " << MuScleFitUtils::nbins << std::endl; 
//       std::cout<< "nBinsX = " << nBinsX << ", nBinsY = " << nBinsY << std::endl;
//       exit(1);
//     }
...
//       // std::cout << "For iY = " << iY << " the histogram is \"" << *name << "\"" << std::endl;
...
//       // std::cout << "For ires = " << ires << " the histogram is \"" << *name << "\"" << std::endl;
