                                      ...
                                      const double * massWindowHalfWidth )
{
  ...
  for( unsigned int i=0; i<6; ++i ) {
    ...
    resonanceWindow_.emplace_back( mass, lowerLimit, upperLimit,
  ...
  BOOST_FOREACH(const indexVec & index, indexes) {
    ...
    backgroundWindow_.emplace_back( resMassForRegion[i], leftWindowBorders[i], rightWindowBorders[i], index,
