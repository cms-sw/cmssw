void Plot_mass() {
  ...
  TFile * A = new TFile ("./1_MuScleFit_Z_100K_CTEQ.root");
  // 1_testYJM_SM_f4r4.root");
  A->cd();
  //TH1F * Mu = dynamic_cast<TH1F*> (A->Get("hRecBestRes_Mass_fine")); 
  TH1F * MuZ = dynamic_cast<TH1F*> (A->Get("hRecBestRes_Mass")); 
  ...
  TFile * B = new TFile ("./4_MuScleFit_Z_100K_CTEQ.root");
  // 4_testYJM_SM_f4r4.root");
  B->cd();
  //TH1F * Mc = dynamic_cast<TH1F*> (B->Get("hRecBestRes_Mass_fine")); 
  TH1F * McZ = dynamic_cast<TH1F*> (B->Get("hRecBestRes_Mass")); 
  ...
  for (int ires=0; ires<6; ires++) {
    ...
    //    if (ires==0) {
      ...
      McZp->SetNormFactor(McZ->Integral());
...
//     }
  ...
  for (int ires=0; ires<6; ires++) {
    ...
    //    if (ires==0) {
      ...
      MuZp->SetNormFactor(MuZ->Integral());
