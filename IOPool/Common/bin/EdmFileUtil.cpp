...
int main(int argc, char* argv[]) {
  ...
  try {
    ...
    for (unsigned int j = 0; j < in.size(); ++j) {
      ...
      if (printClusters) {
        edm::BranchType branchType = edm::NumBranchTypes;
        // Ignore ProcessBlock (for now) because getting the tree name would require knowing the process
        for (int i = edm::InEvent; i != edm::InProcess; ++i) {
          if (selectedTree == edm::BranchTypeToProductTreeName(static_cast<edm::BranchType>(i))) {
            branchType = static_cast<edm::BranchType>(i);
          }
        }
        if (branchType == edm::NumBranchTypes) {
          std::cout << "Printing cluster infomration for tree " << selectedTree << " is not supported\n";
          std::cout << "Exiting\n";
          return 1;
        }
        ...
        edm::clusterPrint(printTree, branchType);
      }
