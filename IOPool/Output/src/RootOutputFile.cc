...
namespace edm {
  ...
  RootOutputFile::RootOutputFile(PoolOutputModule* om, std::string const& fileName, std::string const& logicalFileName) :
      ...
      eventTree_(get_underlying(filePtr_), InEvent, om_->splitLevel(), om_->treeMaxVirtualSize()),
      lumiTree_(get_underlying(filePtr_), InLumi, om_->splitLevel(), om_->treeMaxVirtualSize()),
      runTree_(get_underlying(filePtr_), InRun, om_->splitLevel(), om_->treeMaxVirtualSize()),
      treePointers_(),
    ...
    eventTree_.addAuxiliary<StoredProductProvenanceVector>(BranchTypeToProductProvenanceBranchName(InEvent),
                                                     get_underlying(pEventEntryInfoVector_), om_->auxItems()[InEvent].basketSize_);
