...
edmProvDump merged.root | grep -q "accelerators: test-one" || die "Did not find test-one from merged.root provenance" $?
edmProvDump merged.root | grep -q "accelerators: test-two" || die "Did not find test-two from merged.root provenance" $?
