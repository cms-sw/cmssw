...
DQMStreamerReader::DQMStreamerReader(ParameterSet const& pset,
    ...
    : StreamerInputSource(pset, desc),
      ...
      initialNumberOfEventsToSkip_(
          pset.getUntrackedParameter<unsigned int>("skipEvents", 0)) {
  reset_();
}
...
void DQMStreamerReader::reset_() {
  ...
  std::cout << "************ reset: using fileName " << streamerName_
            << std::endl;
...
std::string DQMStreamerReader::getDataFile(int lumi) {
  ...
  newfilename = dqmInputDir_ + "/run" +
                to_padded_string(runNumber_, run_min_length) + "/" + datafile;
  return newfilename;
}
...
bool DQMStreamerReader::isEndOfRun() {
  ...
  endOfRunFileName_ =
      dqmInputDir_ + "/run" + to_padded_string(runNumber_, run_min_length) +
      "/run" + to_padded_string(runNumber_, run_min_length) + "_EOR.jsn";
...
bool DQMStreamerReader::checkNewData(int lumi) {
  ...
  if (totalEventPerLs_ > 0) {
    std::cout << " CheckNewData: JSON file " << make_path(lumi) << " has "
              << totalEventPerLs_ << " events" << std::endl;
    return true;
  } else {
    std::cout << " WARNING: CheckNewData: JSON file " << make_path(lumi)
              << " has no events" << std::endl;
    return false;
  }
  return false;
}
...
std::string DQMStreamerReader::to_padded_string(int n, std::size_t min_length) {
  ...
  if (ret.length() >= min_length) return ret;
...
std::string DQMStreamerReader::make_path(lumisection_t lumisection) {
  ...
  jsonFileName_ =
      dqmInputDir_ + "/run" + to_padded_string(runNumber_, run_min_length) +
      "/run" + to_padded_string(runNumber_, run_min_length) + "_ls" +
      to_padded_string(lumisection, lumisection_min_length) + ".jsn";
  return jsonFileName_;
}

bool DQMStreamerReader::checkNextEvent() {
  if (processedEventPerLs_ > 0) {
    std::cout << "***** checkNextEvent: at least one processed event: check if "
                 "new LS exist" << std::endl;
    if (checkNextLS()) {
      std::cout << "***** checkNextEvent: at least one event has been "
                   "processed from current LS and a new LS has been found "
                << std::endl;
    }
  } else {
    std::cout << "****** checkNextEvent: no event processed in LS "
              << currentLumiSection_ << std::endl;
  }
...
bool DQMStreamerReader::checkNextLS() {
  std::cout << "************ checkNextLS: check for LS "
            << currentLumiSection_ + 1 << std::endl;
  ...
  if (checkNewData(nextLS)) {
    ...
    std::cout << "No new LS found: processing current LS "
              << currentLumiSection_ << std::endl;
    return false;
  }
}
...
EventMsgView const* DQMStreamerReader::getNextEvent() {
  ...
  std::cout << "************  getNextEvent "
            << " event processed " << processedEventPerLs_ << std::endl;
