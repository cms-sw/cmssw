...
namespace edm {
  ...
  StreamerFileReader::reset_() {
    if (streamerNames_.size() > 1) {
      streamReader_ = std::unique_ptr<StreamerInputFile>(new StreamerInputFile(streamerNames_, get_underlying(eventSkipperByID_)));
    } else if (streamerNames_.size() == 1) {
      streamReader_ = std::unique_ptr<StreamerInputFile>(new StreamerInputFile(streamerNames_.at(0), get_underlying(eventSkipperByID_)));
    } else {
