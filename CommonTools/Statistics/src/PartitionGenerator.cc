...
vector<PartitionGenerator::Partition> PartitionGenerator::partitions(int collectionSize, int minCollectionSize) const {
...
vector<std::vector<PartitionGenerator::Partition> > PartitionGenerator::sortedPartitions(int collectionSize,
  ...
  std::vector<Partition> partitions = this->partitions(collectionSize, minCollectionSize);
  ...
  for (const auto& partition : partitions) {
    sortedPartitions[partition.size() - 1].push_back(partition);
  }
