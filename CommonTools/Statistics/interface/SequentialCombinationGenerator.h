...
public:
  ...
  SequentialCombinationGenerator(Partition& part)
      : the_part(part), the_k(part.size()), the_n(accumulate(part.begin(), part.end(), 0)) {
...
private:
  Vecint next_combi(Vecint& cold, int n, const Partition& p) {
    ...
    Vecint ss(n1);
    set_difference(cold.begin(), cold.end(), cnew2.begin(), cnew2.end(), ss.begin());
    int ip = p[0];
    for (int i = 1; i < k; i++) {
      if (p[i] != p[i - 1]) {
