...
namespace {
  void checkCandidate(reco::LeafCandidate const &cand, const std::string &expression, double x) {
    ...
    try {
      ...
      reco::ExpressionEvaluator eval(
          "CommonTools/CandUtils", "reco::ValueOnObject<reco::LeafCandidate>", sexpr);
  ...
  struct MyAnalyzer {
    ...
    explicit MyAnalyzer(std::string const &cut) {
      ...
      try {
        reco::ExpressionEvaluator eval(
            "CommonTools/CandUtils", "reco::MaskCollection<reco::LeafCandidate>", sexpr);
  ...
  struct MyAnalyzer2 {
    ...
    explicit MyAnalyzer2(std::string const &cut) {
      ...
      try {
        reco::ExpressionEvaluator eval(
            "CommonTools/CandUtils", "reco::SelectInCollection<reco::LeafCandidate>", sexpr);
        m_selector = eval.expr<Selector>();
        CPPUNIT_ASSERT(m_selector);
      } catch (cms::Exception const &e) {
        std::cerr << e.what() << std::endl;
