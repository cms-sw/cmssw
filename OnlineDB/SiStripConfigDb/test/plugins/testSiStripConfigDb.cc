...
testSiStripConfigDb::~testSiStripConfigDb() {
  LogTrace(mlCabling_) << "[testSiStripConfigDb::" << __func__ << "]"
                       << " Destructing object...";
}
...
void testSiStripConfigDb::beginJob() {
  ...
  // Check pointer
  if (db_) {
    edm::LogVerbatim(mlCabling_) << "[testSiStripConfigDb::" << __func__ << "]"
                                 << " Pointer to SiStripConfigDb: 0x" << std::setw(8) << std::setfill('0') << std::hex
                                 << db_ << std::dec;
  } else {
    edm::LogError(mlCabling_) << "[testSiStripConfigDb::" << __func__ << "]"
  ...
  if (db_->deviceFactory() || db_->databaseCache()) {
    ...
    ss << "[testSiStripConfigDb::" << __func__ << "]" << std::endl
       << " Pointer to DeviceFactory: 0x" << std::setw(8) << std::setfill('0') << std::hex << db_->deviceFactory()
       << std::dec << std::endl
       << " Pointer to DatabaseCache: 0x" << std::setw(8) << std::setfill('0') << std::hex << db_->databaseCache()
       << std::dec << std::endl;
  ...
  if (db_->dbParams().partitionsSize() < 1) {
    ...
    SiStripDbParams tmp2;
    tmp2 = db_->dbParams();
    std::stringstream ss;
    ss << "[testSiStripConfigDb::" << __func__ << "]"
       << " Test of copy constructors: " << std::endl
       << " ===> 'Original' DbParams: " << std::endl
       << db_->dbParams() << " ===> 'Copy constructed' DbParams: " << std::endl
       << tmp1 << " ===> 'Assignment constructed' DbParams: " << std::endl
       << tmp2;
    ...
    auto iii = db_->dbParams().partitions().begin();
    auto jjj = db_->dbParams().partitions().end();
    for (; ii != jj; ++ii) {
      for (; iii != jjj; ++iii) {
        ss << "\"" << ii->second.partitionName() << "\" is equal to \"" << iii->second.partitionName() << "\"? "
           << std::boolalpha << (ii->second == iii->second) << std::noboolalpha << std::endl;
        ss << "\"" << ii->second.partitionName() << "\" is equal NOT to \"" << iii->second.partitionName() << "\"? "
           << std::boolalpha << (ii->second != iii->second) << std::noboolalpha << std::endl;
      }
    }
    LogTrace(mlCabling_) << ss.str();
  ...
  // Print run numbers for partitions and run types
  {
    std::stringstream ss;
    ss << "[testSiStripConfigDb::" << __func__ << "]"
       << " Retrieving run numbers by partition and run type:" << std::endl;
    ...
    if (partitions.empty()) {
      ...
      for (; ii != jj; ++ii) {
        ...
        if (types.empty()) {
          ...
          auto iii = types.begin();
          auto jjj = types.end();
          for (; iii != jjj; ++iii) {
            if (iii->second.empty()) {
              ss << " Found no runs for partition \"" << ii->first << "\" and run type \"" << iii->first << "\"!"
                 << std::endl;
            } else {
              ss << " Found " << iii->second.size() << " runs for partition \"" << ii->first << "\" and run type \""
                 << SiStripEnumsAndStrings::runType(iii->first) << "\": ";
              uint16_t cntr = 0;
              auto iiii = iii->second.begin();
              auto jjjj = iii->second.end();
              for (; iiii != jjjj; ++iiii) {
                if (cntr < 10) {
  ...
  if (upload_) {
    ...
    if (conns_) {
      ...
      auto iter = db_->dbParams().partitions().begin();
      auto jter = db_->dbParams().partitions().end();
      for (; iter != jter; ++iter) {
    ...
    if (anals_) {
      ...
      for (; ii != jj; ++ii) {
        ...
        SiStripConfigDb::AnalysisDescriptionsRange range =
            db_->getAnalysisDescriptions(type, ii->second.partitionName());
  ...
  if (download_) {
    ...
    if (conns_) {
      ...
      for (; iter != jter; ++iter) {
        ...
        std::stringstream ss;
        ss << "[testSiStripConfigDb::" << __func__ << "]"
           << " Downloaded " << conns.size() << " FED connections!";
        if (!conns.empty()) {
          edm::LogVerbatim("testSiStripConfigDb") << ss.str();
        } else {
          edm::LogWarning("testSiStripConfigDb") << ss.str();
        }
      }
    ...
    if (devices_) {
      ...
      for (; iter != jter; ++iter) {
        ...
        std::stringstream ss;
        ss << "[testSiStripConfigDb::" << __func__ << "]"
           << " Downloaded " << devs.size() << " device descriptions!";
        ...
        SiStripConfigDb::DeviceDescriptionsRange lld =
            db_->getDeviceDescriptions(LASERDRIVER, iter->second.partitionName());
    ...
    if (feds_) {
      ...
      for (; iter != jter; ++iter) {
        ...
        std::stringstream ss;
        ss << "[testSiStripConfigDb::" << __func__ << "]"
           << " Downloaded " << feds.size() << " FED descriptions!";
    ...
    if (dcus_) {
      ...
      for (; iter != jter; ++iter) {
        ...
        std::stringstream ss;
        ss << "[testSiStripConfigDb::" << __func__ << "]"
           << " Downloaded " << range.size() << " DCU-DetId map!";
        if (!range.empty()) {
          edm::LogVerbatim("testSiStripConfigDb") << ss.str();
        } else {
          edm::LogWarning("testSiStripConfigDb") << ss.str();
    ...
    if (anals_) {
      ...
      for (; iter != jter; ++iter) {
        ...
        if (iter->second.runType() == sistrip::PHYSICS) {
          ...
          std::stringstream ss;
          ss << "[testSiStripConfigDb::" << __func__ << "]"
             << " Unexpected run type \"" << SiStripEnumsAndStrings::runType(iter->second.runType())
        ...
        SiStripConfigDb::AnalysisDescriptionsRange anals =
            db_->getAnalysisDescriptions(type, iter->second.partitionName());

        std::stringstream ss;
        ss << "[testSiStripConfigDb::" << __func__ << "]"
           << " Downloaded " << anals.size() << " analysis descriptions!";
        if (!anals.empty()) {
          edm::LogVerbatim("testSiStripConfigDb") << ss.str();
        } else {
          edm::LogWarning("testSiStripConfigDb") << ss.str();
