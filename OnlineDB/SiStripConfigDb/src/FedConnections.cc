...
SiStripConfigDb::FedConnectionsRange SiStripConfigDb::getFedConnections(std::string partition) {
        ...
        if (partition.empty() || partition == iter->second.partitionName()) {
          ...
          if (range == connections_.emptyRange()) {
            ...
            ss << "[SiStripConfigDb::" << __func__ << "]"
               << " Downloaded " << conns.size() << " FED connections to local cache for partition \""
               << iter->second.partitionName() << "\"" << std::endl;
            ss << "[SiStripConfigDb::" << __func__ << "]"
               << " Cache holds FED connections for " << connections_.size() << " partitions.";
  ...
  if (!dbParams_.usingDb()) {
    ss << " in " << dbParams_.inputModuleXmlFiles().size() << " 'module.xml' file(s)";
  } else {
    if (!dbParams_.usingDbCache()) {
...
void SiStripConfigDb::addFedConnections(std::string partition, FedConnectionsV& conns) {
  ...
  if (range == connections_.emptyRange()) {
    ...
    std::stringstream ss;
    ss << "[SiStripConfigDb::" << __func__ << "]"
       << " Added " << conns.size() << " FED connections to local cache for partition \"" << partition << "\""
       << std::endl;
    ss << "[SiStripConfigDb::" << __func__ << "]"
       << " Cache holds FED connections for " << connections_.size() << " partitions.";
    LogTrace(mlConfigDb_) << ss.str();
      ...
      for (; iter != jter; ++iter) {
        if (partition.empty() || partition == iter->second.partitionName()) {
          ...
          if (range != connections_.emptyRange()) {
            ...
            std::stringstream ss;
            ss << "[SiStripConfigDb::" << __func__ << "]"
               << " Uploaded " << conns.size() << " FED connections to database for partition \""
               << iter->second.partitionName() << "\".";
            LogTrace(mlConfigDb_) << ss.str();
...
void SiStripConfigDb::clearFedConnections(std::string partition) {
  ...
  if (partition.empty()) {
    temporary_cache = FedConnections();
  } else {
    auto iter = dbParams_.partitions().begin();
    auto jter = dbParams_.partitions().end();
    for (; iter != jter; ++iter) {
  ...
  if (conns != connections_.emptyRange()) {
    ...
    ss << "[SiStripConfigDb::" << __func__ << "]";
    if (partition.empty()) {
      ss << " Found no FED connections in local cache!";
    } else {
      ss << " Found no FED connections in local cache for partition \"" << partition << "\"!";
    }
    edm::LogWarning(mlConfigDb_) << ss.str();
  }
