  ...
  throw(runtime_error)
{
  ...
  try {
    ...
    m_writeStmt->setSQL("INSERT INTO "+getTable()+" (rec_id, tr_id, tcc_id, tt_id, status ) "
			"VALUES (:1, :2, :3, :4, :5 )");
  ...
  throw(std::runtime_error)
{
  ...
  try {
    m_readStmt->setSQL("SELECT * FROM " + getTable() + "WHERE rec_id = :rec_id order by tr_id, tcc_id, tt_id ");
