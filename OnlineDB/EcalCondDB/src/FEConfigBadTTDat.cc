...
void FEConfigBadTTDat::prepareWrite() noexcept(false) {
...
void FEConfigBadTTDat::writeDB(const FEConfigBadTTDat* item, FEConfigBadTTInfo* iov) noexcept(false) {
  this->checkConnection();
...
void FEConfigBadTTDat::fetchData(std::vector<FEConfigBadTTDat>* p, FEConfigBadTTInfo* iov) noexcept(false) {
  this->checkConnection();
...
void FEConfigBadTTDat::writeArrayDB(const std::vector<FEConfigBadTTDat>& data, FEConfigBadTTInfo* iov) noexcept(false) {
  this->checkConnection();

  int iovID = iov->fetchID();
  if (!iovID) {
    throw(std::runtime_error("FEConfigDelays::writeArrayDB:  FEConfigBadTTInfo not in DB"));
  }
