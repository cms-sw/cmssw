...
void FEConfigFgrGroupDat::prepareWrite() noexcept(false) {
...
void FEConfigFgrGroupDat::writeDB(const EcalLogicID* ecid,
  ...
  if (!iconfID) {
    throw(std::runtime_error("FEConfigFgrGroupDat::writeDB:  ICONF not in DB"));
  }
...
void FEConfigFgrGroupDat::fetchData(map<EcalLogicID, FEConfigFgrGroupDat>* fillMap,
                                    FEConfigFgrInfo* iconf) noexcept(false) {
  this->checkConnection();
  fillMap->clear();
...
void FEConfigFgrGroupDat::writeArrayDB(const std::map<EcalLogicID, FEConfigFgrGroupDat>* data,
  ...
  int iconfID = iconf->fetchID();
  if (!iconfID) {
    throw(std::runtime_error("FEConfigFgrGroupDat::writeArrayDB:  ICONF not in DB"));
  }
