...
void RunFEConfigDat::prepareWrite() noexcept(false) {
...
void RunFEConfigDat::writeDB(const EcalLogicID* ecid, const RunFEConfigDat* item, RunIOV* iov) noexcept(false) {
  this->checkConnection();
  this->checkPrepare();
  ...
  if (!logicID) {
    throw(std::runtime_error("RunFEConfigDat::writeDB:  Bad EcalLogicID"));
  }
...
void RunFEConfigDat::fetchData(map<EcalLogicID, RunFEConfigDat>* fillMap, RunIOV* iov) noexcept(false) {
  this->checkConnection();
  fillMap->clear();
