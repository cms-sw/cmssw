...
void MonPNLed2Dat::prepareWrite() noexcept(false) {
  this->checkConnection();

  try {
    m_writeStmt = m_conn->createStatement();
    m_writeStmt->setSQL(
        "INSERT INTO mon_pn_led2_dat (iov_id, logic_id, "
...
void MonPNLed2Dat::writeDB(const EcalLogicID* ecid, const MonPNLed2Dat* item, MonRunIOV* iov) noexcept(false) {
  this->checkConnection();
  this->checkPrepare();
  ...
  if (!logicID) {
    throw(std::runtime_error("MonPNLed2Dat::writeDB:  Bad EcalLogicID"));
  }
...
void MonPNLed2Dat::fetchData(std::map<EcalLogicID, MonPNLed2Dat>* fillMap, MonRunIOV* iov) noexcept(false) {
  this->checkConnection();
  fillMap->clear();
...
void MonPNLed2Dat::writeArrayDB(const std::map<EcalLogicID, MonPNLed2Dat>* data, MonRunIOV* iov) noexcept(false) {
  this->checkConnection();
  this->checkPrepare();

  int iovID = iov->fetchID();
  if (!iovID) {
    throw(std::runtime_error("MonPNLed2Dat::writeArrayDB:  IOV not in DB"));
  }
  ...
  for (CI p = data->begin(); p != data->end(); ++p) {
    ...
    if (!logicID) {
      throw(std::runtime_error("MonPNLed2Dat::writeArrayDB:  Bad EcalLogicID"));
    }
    ids[count] = logicID;
