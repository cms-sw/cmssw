  ...
  noexcept(false)
{
  ...
  try {
    ...
    m_writeStmt->setSQL("INSERT INTO mon_pn_led2_dat (iov_id, logic_id, "
			"adc_mean_g1, adc_rms_g1, adc_mean_g16, adc_rms_g16, ped_mean_g1, ped_rms_g1, ped_mean_g16, ped_rms_g16, task_status) "
			"VALUES (:iov_id, :logic_id, "
			":3, :4, :5, :6, :7, :8, :9, :10, :11)");
  } catch (SQLException &e) {
    throw(std::runtime_error(std::string("MonPNLed2Dat::prepareWrite():  ")+e.getMessage()));
  }
}
  ...
  noexcept(false)
{
  ...
  if (!iovID) { throw(std::runtime_error("MonPNLed2Dat::writeDB:  IOV not in DB")); }

  int logicID = ecid->getLogicID();
  if (!logicID) { throw(std::runtime_error("MonPNLed2Dat::writeDB:  Bad EcalLogicID")); }
  ...
  try {
    ...
    throw(std::runtime_error(std::string("MonPNLed2Dat::writeDB():  ")+e.getMessage()));
  }
}
  ...
  noexcept(false)
{
  ...
  int iovID = iov->fetchID();
  if (!iovID) { 
    //  throw(std::runtime_error("MonPNLed2Dat::writeDB:  IOV not in DB")); 
    return;
  }
  ...
  try {
    ...
    m_readStmt->setSQL("SELECT cv.name, cv.logic_id, cv.id1, cv.id2, cv.id3, cv.maps_to, "
		 ...
		 "FROM channelview cv JOIN mon_pn_led2_dat d "
    ...
    throw(std::runtime_error(std::string("MonPNLed2Dat::fetchData():  ")+e.getMessage()));
  }
}
  ...
  noexcept(false)
{
  ...
  int iovID = iov->fetchID();
  if (!iovID) { throw(std::runtime_error("MonPNLed2Dat::writeArrayDB:  IOV not in DB")); }
  ...
  for (CI p = data->begin(); p != data->end(); ++p) {
	...
	if (!logicID) { throw(std::runtime_error("MonPNLed2Dat::writeArrayDB:  Bad EcalLogicID")); }
	ids[count]=logicID;
  ...
  try {
    ...
    throw(std::runtime_error(std::string("MonPNLed2Dat::writeArrayDB():  ")+e.getMessage()));
  }
}
