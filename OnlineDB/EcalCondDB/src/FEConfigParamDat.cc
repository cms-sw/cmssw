...
void FEConfigParamDat::prepareWrite() noexcept(false) {
  this->checkConnection();

  try {
    m_writeStmt = m_conn->createStatement();
    m_writeStmt->setSQL(
        "INSERT INTO fe_config_param_dat (lin_conf_id, logic_id, "
...
void FEConfigParamDat::writeDB(const EcalLogicID* ecid,
  ...
  if (!logicID) {
    throw(std::runtime_error("FEConfigParamDat::writeDB:  Bad EcalLogicID"));
  }
...
void FEConfigParamDat::fetchData(map<EcalLogicID, FEConfigParamDat>* fillMap, FEConfigLinInfo* iconf) noexcept(false) {
  this->checkConnection();
  fillMap->clear();
...
void FEConfigParamDat::writeArrayDB(const std::map<EcalLogicID, FEConfigParamDat>* data,
  ...
  int iconfID = iconf->fetchID();
  if (!iconfID) {
    throw(std::runtime_error("FEConfigParamDat::writeArrayDB:  ICONF not in DB"));
  }
  ...
  for (CI p = data->begin(); p != data->end(); ++p) {
    ...
    if (!logicID) {
      throw(std::runtime_error("FEConfigParamDat::writeArrayDB:  Bad EcalLogicID"));
    }
    ids[count] = logicID;
