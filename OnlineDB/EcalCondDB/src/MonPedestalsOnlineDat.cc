...
void MonPedestalsOnlineDat::prepareWrite() noexcept(false) {
...
void MonPedestalsOnlineDat::writeDB(const EcalLogicID* ecid,
  ...
  if (!logicID) {
    throw(std::runtime_error("MonPedestalsOnlineDat::writeDB:  Bad EcalLogicID"));
  }
...
void MonPedestalsOnlineDat::writeArrayDB(const std::map<EcalLogicID, MonPedestalsOnlineDat>* data,
  ...
  int iovID = iov->fetchID();
  if (!iovID) {
    throw(std::runtime_error("MonPedestalsOnlineDat::writeArrayDB:  IOV not in DB"));
  }
