  ...
  noexcept(false)
{
  ...
  try {
    ...
    throw(std::runtime_error(std::string("MODDCCOperationDat::prepareWrite():  ")+e.getMessage()));
  }
}
  ...
  noexcept(false)
{
  ...
  if (!iovID) { throw(std::runtime_error("MODDCCOperationDat::writeDB:  IOV not in DB")); }

  int logicID = ecid->getLogicID();
  if (!logicID) { throw(std::runtime_error("MODDCCOperationDat::writeDB:  Bad EcalLogicID")); }
  ...
  try {
    ...
    throw(std::runtime_error(std::string("MODDCCOperationDat::writeDB():  ")+e.getMessage()));
  }
}
  ...
  noexcept(false)
{
  ...
  try {
    ...
    throw(std::runtime_error(std::string("MODDCCOperationDat::fetchData():  ")+e.getMessage()));
  }
}
  ...
  noexcept(false)
{
  ...
  int iovID = iov->fetchID();
  if (!iovID) { throw(std::runtime_error("MODDCCOperationDat::writeArrayDB:  IOV not in DB")); }
