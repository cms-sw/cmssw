  ...
  noexcept(false)
{
  ...
  try {
    ...
    throw(std::runtime_error(std::string("MonPNIRedDat::prepareWrite():  ")+e.getMessage()));
  }
}
  ...
  noexcept(false)
{
  ...
  if (!iovID) { throw(std::runtime_error("MonPNIRedDat::writeDB:  IOV not in DB")); }

  int logicID = ecid->getLogicID();
  if (!logicID) { throw(std::runtime_error("MonPNIRedDat::writeDB:  Bad EcalLogicID")); }
  ...
  try {
    ...
    throw(std::runtime_error(std::string("MonPNIRedDat::writeDB():  ")+e.getMessage()));
  }
}
  ...
  noexcept(false)
{
  ...
  try {
    ...
    throw(std::runtime_error(std::string("MonPNIRedDat::fetchData():  ")+e.getMessage()));
  }
}
  ...
  noexcept(false)
{
  ...
  int iovID = iov->fetchID();
  if (!iovID) { throw(std::runtime_error("MonPNIRedDat::writeArrayDB:  IOV not in DB")); }
  ...
  try {
    ...
    throw(std::runtime_error(std::string("MonPNIRedDat::writeArrayDB():  ")+e.getMessage()));
  }
}
