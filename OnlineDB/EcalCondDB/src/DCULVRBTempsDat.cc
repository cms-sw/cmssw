...
void DCULVRBTempsDat::prepareWrite() noexcept(false) {
...
void DCULVRBTempsDat::writeDB(const EcalLogicID* ecid, const DCULVRBTempsDat* item, DCUIOV* iov) noexcept(false) {
  this->checkConnection();
  this->checkPrepare();
  ...
  if (!logicID) {
    throw(std::runtime_error("DCULVRBTempsDat::writeDB:  Bad EcalLogicID"));
  }
...
void DCULVRBTempsDat::fetchData(std::map<EcalLogicID, DCULVRBTempsDat>* fillMap, DCUIOV* iov) noexcept(false) {
  this->checkConnection();
  fillMap->clear();
...
void DCULVRBTempsDat::writeArrayDB(const std::map<EcalLogicID, DCULVRBTempsDat>* data, DCUIOV* iov) noexcept(false) {
  this->checkConnection();
  this->checkPrepare();

  int iovID = iov->fetchID();
  if (!iovID) {
    throw(std::runtime_error("DCULVRBTempsDat::writeArrayDB:  IOV not in DB"));
  }
  ...
  for (CI p = data->begin(); p != data->end(); ++p) {
    ...
    if (!logicID) {
      throw(std::runtime_error("DCULVRBTempsDat::writeArrayDB:  Bad EcalLogicID"));
    }
    ids[count] = logicID;
