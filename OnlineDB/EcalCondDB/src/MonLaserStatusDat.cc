...
void MonLaserStatusDat::prepareWrite() noexcept(false) {
  this->checkConnection();

  try {
    m_writeStmt = m_conn->createStatement();
    m_writeStmt->setSQL(
        "INSERT INTO mon_laser_status_dat (iov_id, logic_id, "
...
void MonLaserStatusDat::writeDB(const EcalLogicID* ecid,
  ...
  if (!logicID) {
    throw(std::runtime_error("MonLaserStatusDat::writeDB:  Bad EcalLogicID"));
  }
...
void MonLaserStatusDat::fetchData(std::map<EcalLogicID, MonLaserStatusDat>* fillMap, MonRunIOV* iov) noexcept(false) {
  this->checkConnection();
  fillMap->clear();
