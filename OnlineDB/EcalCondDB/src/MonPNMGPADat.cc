...
void MonPNMGPADat::prepareWrite() noexcept(false) {
  this->checkConnection();

  try {
    m_writeStmt = m_conn->createStatement();
    m_writeStmt->setSQL(
        "INSERT INTO mon_pn_mgpa_dat (iov_id, logic_id, "
...
void MonPNMGPADat::writeDB(const EcalLogicID* ecid, const MonPNMGPADat* item, MonRunIOV* iov) noexcept(false) {
  this->checkConnection();
  this->checkPrepare();
  ...
  if (!logicID) {
    throw(std::runtime_error("MonPNMGPADat::writeDB:  Bad EcalLogicID"));
  }
...
void MonPNMGPADat::fetchData(std::map<EcalLogicID, MonPNMGPADat>* fillMap, MonRunIOV* iov) noexcept(false) {
  this->checkConnection();
  fillMap->clear();
...
void MonPNMGPADat::writeArrayDB(const std::map<EcalLogicID, MonPNMGPADat>* data, MonRunIOV* iov) noexcept(false) {
  this->checkConnection();
  this->checkPrepare();

  int iovID = iov->fetchID();
  if (!iovID) {
    throw(std::runtime_error("MonPNMGPADat::writeArrayDB:  IOV not in DB"));
  }
  ...
  for (CI p = data->begin(); p != data->end(); ++p) {
    ...
    if (!logicID) {
      throw(std::runtime_error("MonPNMGPADat::writeArrayDB:  Bad EcalLogicID"));
    }
    ids[count] = logicID;
