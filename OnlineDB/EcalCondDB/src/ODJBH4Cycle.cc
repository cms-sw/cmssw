...
void ODJBH4Cycle::prepareWrite() noexcept(false) {
  this->checkConnection();

  try {
    m_writeStmt = m_conn->createStatement();
    m_writeStmt->setSQL(
        "INSERT INTO ECAL_JBH4_Cycle (cycle_id, jbh4_configuration_id ) "
        "VALUES (:1, :2 )");
...
void ODJBH4Cycle::clear() { m_jbh4_config_id = 0; }

int ODJBH4Cycle::fetchID() noexcept(false) {
  ...
  try {
    ...
    stmt->setSQL(
        "SELECT cycle_id, jbh4_configuration_id FROM ecal_jbh4_cycle "
        "WHERE cycle_id = :1 ");
    stmt->setInt(1, m_ID);
...
void ODJBH4Cycle::setByID(int id) noexcept(false) {
  this->checkConnection();
...
void ODJBH4Cycle::fetchData(ODJBH4Cycle *result) noexcept(false) {
  this->checkConnection();
  result->clear();
  ...
  try {
    m_readStmt->setSQL(
        "SELECT  jbh4_configuration_id FROM ecal_jbh4_cycle "
        "WHERE cycle_id = :1 ");
...
void ODJBH4Cycle::insertConfig() noexcept(false) {
  try {
    prepareWrite();
    writeDB();
    m_conn->commit();
