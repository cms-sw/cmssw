...
void RunCrystalErrorsDat::prepareWrite() noexcept(false) {
...
void RunCrystalErrorsDat::writeDB(const EcalLogicID* ecid,
  ...
  if (!logicID) {
    throw(std::runtime_error("RunCrystalErrorsDat::writeDB:  Bad EcalLogicID"));
  }
...
void RunCrystalErrorsDat::fetchData(map<EcalLogicID, RunCrystalErrorsDat>* fillMap, RunIOV* iov) noexcept(false) {
  this->checkConnection();
  fillMap->clear();
