...
void FEConfigBadXTDat::prepareWrite() noexcept(false) {
...
void FEConfigBadXTDat::writeDB(const FEConfigBadXTDat* item, FEConfigBadXTInfo* iov) noexcept(false) {
  this->checkConnection();
...
void FEConfigBadXTDat::fetchData(std::vector<FEConfigBadXTDat>* p, FEConfigBadXTInfo* iov) noexcept(false) {
  this->checkConnection();
...
void FEConfigBadXTDat::writeArrayDB(const std::vector<FEConfigBadXTDat>& data, FEConfigBadXTInfo* iov) noexcept(false) {
  this->checkConnection();

  int iovID = iov->fetchID();
  if (!iovID) {
    throw(std::runtime_error("FEConfigDelays::writeArrayDB:  FEConfigBadXTInfo not in DB"));
  }
