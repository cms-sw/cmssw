...
void MonTestPulseDat::prepareWrite() noexcept(false) {
...
void MonTestPulseDat::writeDB(const EcalLogicID* ecid, const MonTestPulseDat* item, MonRunIOV* iov) noexcept(false) {
  this->checkConnection();
  this->checkPrepare();
  ...
  if (!logicID) {
    throw(std::runtime_error("MonTestPulseDat::writeDB:  Bad EcalLogicID"));
  }
...
void MonTestPulseDat::fetchData(std::map<EcalLogicID, MonTestPulseDat>* fillMap, MonRunIOV* iov) noexcept(false) {
  this->checkConnection();
  fillMap->clear();
...
void MonTestPulseDat::writeArrayDB(const std::map<EcalLogicID, MonTestPulseDat>* data, MonRunIOV* iov) noexcept(false) {
  this->checkConnection();
  this->checkPrepare();

  int iovID = iov->fetchID();
  if (!iovID) {
    throw(std::runtime_error("MonTestPulseDat::writeArrayDB:  IOV not in DB"));
  }
  ...
  for (CI p = data->begin(); p != data->end(); ++p) {
    ...
    if (!logicID) {
      throw(std::runtime_error("MonTestPulseDat::writeArrayDB:  Bad EcalLogicID"));
    }
    ids[count] = logicID;
