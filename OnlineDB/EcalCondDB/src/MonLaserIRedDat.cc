...
void MonLaserIRedDat::prepareWrite() noexcept(false) {
  this->checkConnection();

  try {
    m_writeStmt = m_conn->createStatement();
    m_writeStmt->setSQL(
        "INSERT INTO mon_laser_ired_dat (iov_id, logic_id, "
...
void MonLaserIRedDat::writeDB(const EcalLogicID* ecid, const MonLaserIRedDat* item, MonRunIOV* iov) noexcept(false) {
  this->checkConnection();
  this->checkPrepare();
  ...
  if (!logicID) {
    throw(std::runtime_error("MonLaserIRedDat::writeDB:  Bad EcalLogicID"));
  }
...
void MonLaserIRedDat::fetchData(std::map<EcalLogicID, MonLaserIRedDat>* fillMap, MonRunIOV* iov) noexcept(false) {
  this->checkConnection();
...
void MonLaserIRedDat::writeArrayDB(const std::map<EcalLogicID, MonLaserIRedDat>* data, MonRunIOV* iov) noexcept(false) {
  this->checkConnection();
  this->checkPrepare();

  int iovID = iov->fetchID();
  if (!iovID) {
    throw(std::runtime_error("MonLaserIRedDat::writeArrayDB:  IOV not in DB"));
  }
  ...
  for (CI p = data->begin(); p != data->end(); ++p) {
    ...
    if (!logicID) {
      throw(std::runtime_error("MonLaserIRedDat::writeArrayDB:  Bad EcalLogicID"));
    }
    ids[count] = logicID;
