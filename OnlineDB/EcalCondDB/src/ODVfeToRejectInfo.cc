...
int ODVfeToRejectInfo::fetchNextId()  throw(std::runtime_error) {
  ...
  try {
    ...
    throw(runtime_error("ODVfeToRejectInfo::fetchNextId():  "+e.getMessage()));
  }
  ...
  throw(runtime_error)
{
  ...
  try {
    ...
    throw(runtime_error("ODVfeToRejectInfo::prepareWrite():  "+e.getMessage()));
  }

}

void ODVfeToRejectInfo::setParameters(std::map<string,string> my_keys_map){
  ...
  throw(runtime_error)
{
  ...
  try {
    ...
    throw(runtime_error("ODVfeToRejectInfo::writeDB():  "+e.getMessage()));
  }
  // Now get the ID
  if (!this->fetchID()) {
    throw(runtime_error("ODVfeToRejectInfo::writeDB:  Failed to write"));
  } else {
  ...
  throw(runtime_error)
{
  ...
  if(result->getId()==0 && (result->getConfigTag()=="") ){
    throw(runtime_error("ODVfeToRejectInfo::fetchData(): no Id defined for this ODVfeToRejectInfo "));
  }
  ...
  try {
	...
	m_readStmt->setString(2, result->getConfigTag());
      ...
      throw(runtime_error("ODVfeToRejectInfo::fetchData(): no Id defined for this record "));
    }
    ...
    throw(runtime_error("ODVfeToRejectInfo::fetchData():  "+e.getMessage()));
  }
}
...
int ODVfeToRejectInfo::fetchID()    throw(std::runtime_error)
{
  ...
  try {
    ...
    throw(runtime_error("ODVfeToRejectInfo::fetchID:  "+e.getMessage()));
  }
