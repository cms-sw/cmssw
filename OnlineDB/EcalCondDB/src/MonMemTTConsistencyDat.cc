  ...
  noexcept(false)
{
  ...
  try {
    ...
    m_writeStmt->setSQL("INSERT INTO mon_mem_tt_consistency_dat (iov_id, logic_id, "
			"processed_events, problematic_events, problems_id, problems_size, problems_LV1, problems_bunch_X, task_status) "
			"VALUES (:iov_id, :logic_id, "
			":3, :4, :5, :6, :7, :8, :9)");
  } catch (SQLException &e) {
    throw(std::runtime_error(std::string("MonMemTTConsistencyDat::prepareWrite():  ")+e.getMessage()));
  }
}
  ...
  noexcept(false)
{
  ...
  try {
    ...
    throw(std::runtime_error(std::string("MonMemTTConsistencyDat::writeDB():  ")+e.getMessage()));
  }
}
  ...
  noexcept(false)
{
  ...
  try {
    ...
    m_readStmt->setSQL("SELECT cv.name, cv.logic_id, cv.id1, cv.id2, cv.id3, cv.maps_to, "
		 ...
		 "FROM channelview cv JOIN mon_mem_tt_consistency_dat d "
    ...
    throw(std::runtime_error(std::string("MonMemTTConsistencyDat::fetchData():  ")+e.getMessage()));
  }
}
  ...
  noexcept(false)
{
  ...
  const MonMemTTConsistencyDat* dataitem;
  int count=0;
  typedef map< EcalLogicID, MonMemTTConsistencyDat >::const_iterator CI;
  ...
  try {
    ...
    throw(std::runtime_error(std::string("MonMemTTConsistencyDat::writeArrayDB():  ")+e.getMessage()));
  }
}
