...
void MonMemTTConsistencyDat::prepareWrite() noexcept(false) {
...
void MonMemTTConsistencyDat::writeDB(const EcalLogicID* ecid,
  ...
  if (!logicID) {
    throw(std::runtime_error("MonMemTTConsistencyDat::writeDB:  Bad EcalLogicID"));
  }
...
void MonMemTTConsistencyDat::fetchData(std::map<EcalLogicID, MonMemTTConsistencyDat>* fillMap,
                                       MonRunIOV* iov) noexcept(false) {
  this->checkConnection();
  fillMap->clear();
...
void MonMemTTConsistencyDat::writeArrayDB(const std::map<EcalLogicID, MonMemTTConsistencyDat>* data,
  ...
  int iovID = iov->fetchID();
  if (!iovID) {
    throw(std::runtime_error("MonMemTTConsistencyDat::writeArrayDB:  IOV not in DB"));
  }
  ...
  for (CI p = data->begin(); p != data->end(); ++p) {
    ...
    if (!logicID) {
      throw(std::runtime_error("MonMemTTConsistencyDat::writeArrayDB:  Bad EcalLogicID"));
    }
    ids[count] = logicID;
