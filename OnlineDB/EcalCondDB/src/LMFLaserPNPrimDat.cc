...
int LMFLaserPNPrimDat::_color = LMFLaserPNPrimDat::iBlue; //GHM
...
void LMFLaserPNPrimDat::prepareWrite()
  throw(runtime_error)
{
  this->checkConnection();

  // GHM
  std::string command_ = "INSERT INTO XXXXXX (lmf_iov_id, logic_id, flag, mean, rms, peak,  pna_over_pnB_mean, pna_over_pnB_rms, pna_over_pnB_peak ) VALUES (:1, :2, :3, :4, :5, :6, :7, :8, :9 )";
  command_.replace( command_.find("XXXXXX",0), 6, getTable() );
  //   cout << "In prepareWrite: command_" << command_ << endl;

  try {
    m_writeStmt = m_conn->createStatement();
    m_writeStmt->setSQL( command_.c_str() );
  } catch (SQLException &e) {
    throw(runtime_error("LMFLaserPNPrimDat::prepareWrite():  "+e.getMessage()));
  }
}



void LMFLaserPNPrimDat::writeDB(const EcalLogicID* ecid, const LMFLaserPNPrimDat* item, LMFRunIOV* iov)
  throw(runtime_error)
{
  this->checkConnection();
  this->checkPrepare();
  ...
  if (!iovID) { throw(runtime_error("LMFLaserPNPrimDat::writeDB:  IOV not in DB")); }

  int logicID = ecid->getLogicID();
  if (!logicID) { throw(runtime_error("LMFLaserPNPrimDat::writeDB:  Bad EcalLogicID")); }
  ...
  try {
    ...
    throw(runtime_error("LMFLaserPNPrimDat::writeDB():  "+e.getMessage()));
  }
}


void LMFLaserPNPrimDat::writeArrayDB(const std::map< EcalLogicID, LMFLaserPNPrimDat >* data, LMFRunIOV* iov)
  throw(runtime_error)
{
  this->checkConnection();
  this->checkPrepare();
  ...
  int iovID = iov->fetchID();
  if (!iovID) { throw(runtime_error("LMFLaserPNPrimDat::writeArrayDB:  IOV not in DB")); }
  ...
  const LMFLaserPNPrimDat* dataitem;
  int count=0;
  typedef map< EcalLogicID, LMFLaserPNPrimDat >::const_iterator CI;
  for (CI p = data->begin(); p != data->end(); ++p) {
        channel = &(p->first);
	int logicID = channel->getLogicID();
	if (!logicID) { throw(runtime_error("LMFLaserPNPrimDat::writeArrayDB:  Bad EcalLogicID")); }
	ids[count]=logicID;
  ...
  try {
    ...
    throw(runtime_error("LMFLaserPNPrimDat::writeArrayDB():  "+e.getMessage()));
  }
}

void LMFLaserPNPrimDat::fetchData(std::map< EcalLogicID, LMFLaserPNPrimDat >* fillMap, LMFRunIOV* iov)
  throw(runtime_error)
{
  this->checkConnection();
  fillMap->clear();
  ...
  int iovID = iov->fetchID();
  if (!iovID) { 
    //  throw(runtime_error("LMFLaserPNPrimDat::writeDB:  IOV not in DB")); 
    return;
  }

  // GHM
  std::string command_ = "SELECT cv.name, cv.logic_id, cv.id1, cv.id2, cv.id3, cv.maps_to, d.flag, d.mean, d.rms, d.peak, d.pna_over_pnb_mean, d.pna_over_pnb_rms, d.pna_over_pnB_peak FROM channelview cv JOIN XXXXXX d ON cv.logic_id = d.logic_id AND cv.name = cv.maps_to WHERE d.lmf_iov_id = :iov_id";
  command_.replace( command_.find("XXXXXX",0), 6, getTable() );
  ...
  try {
    ...
    m_readStmt->setSQL( command_.c_str() );

    m_readStmt->setInt(1, iovID);

    ResultSet* rset = m_readStmt->executeQuery();

    std::pair< EcalLogicID, LMFLaserPNPrimDat > p;
    LMFLaserPNPrimDat dat;
    while(rset->next()) {
    ...
    throw(runtime_error("LMFLaserPNPrimDat::fetchData():  "+e.getMessage()));
  }
}

void
LMFLaserPNPrimDat::setColor( int color ) { _color = color; }
