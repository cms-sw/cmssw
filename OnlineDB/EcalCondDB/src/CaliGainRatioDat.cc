...
void CaliGainRatioDat::prepareWrite() noexcept(false) {
  this->checkConnection();

  try {
    m_writeStmt = m_conn->createStatement();
    m_writeStmt->setSQL(
        "INSERT INTO cali_gain_ratio_dat (iov_id, logic_id, "
...
void CaliGainRatioDat::writeDB(const EcalLogicID* ecid, const CaliGainRatioDat* item, CaliIOV* iov) noexcept(false) {
  this->checkConnection();
  this->checkPrepare();
  ...
  if (!logicID) {
    throw(std::runtime_error("CaliGainRatioDat::writeDB:  Bad EcalLogicID"));
  }
...
void CaliGainRatioDat::fetchData(std::map<EcalLogicID, CaliGainRatioDat>* fillMap, CaliIOV* iov) noexcept(false) {
  this->checkConnection();
  fillMap->clear();
...
void CaliGainRatioDat::writeArrayDB(const std::map<EcalLogicID, CaliGainRatioDat>* data, CaliIOV* iov) noexcept(false) {
  this->checkConnection();
  this->checkPrepare();

  int iovID = iov->fetchID();
  if (!iovID) {
    throw(std::runtime_error("CaliGainRatioDat::writeArrayDB:  IOV not in DB"));
  }
