...
void FEConfigFgrEEStripDat::prepareWrite() noexcept(false) {
...
void FEConfigFgrEEStripDat::writeDB(const EcalLogicID* ecid,
  ...
  if (!logicID) {
    throw(std::runtime_error("FEConfigFgrEEStripDat::writeDB:  Bad EcalLogicID"));
  }
...
void FEConfigFgrEEStripDat::fetchData(map<EcalLogicID, FEConfigFgrEEStripDat>* fillMap,
                                      FEConfigFgrInfo* iconf) noexcept(false) {
  this->checkConnection();
  fillMap->clear();
...
void FEConfigFgrEEStripDat::writeArrayDB(const std::map<EcalLogicID, FEConfigFgrEEStripDat>* data,
  ...
  int iconfID = iconf->fetchID();
  if (!iconfID) {
    throw(std::runtime_error("FEConfigFgrEEStripDat::writeArrayDB:  ICONF not in DB"));
  }
  ...
  for (CI p = data->begin(); p != data->end(); ++p) {
    ...
    if (!logicID) {
      throw(std::runtime_error("FEConfigFgrEEStripDat::writeArrayDB:  Bad EcalLogicID"));
    }
    ids[count] = logicID;
