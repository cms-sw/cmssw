...
void MonLed2Dat::prepareWrite() noexcept(false) {
  this->checkConnection();

  try {
    m_writeStmt = m_conn->createStatement();
    m_writeStmt->setSQL(
        "INSERT INTO mon_led2_dat (iov_id, logic_id, "
...
void MonLed2Dat::writeDB(const EcalLogicID* ecid, const MonLed2Dat* item, MonRunIOV* iov) noexcept(false) {
  this->checkConnection();
  this->checkPrepare();
  ...
  if (!logicID) {
    throw(std::runtime_error("MonLed2Dat::writeDB:  Bad EcalLogicID"));
  }
...
void MonLed2Dat::fetchData(std::map<EcalLogicID, MonLed2Dat>* fillMap, MonRunIOV* iov) noexcept(false) {
  this->checkConnection();
...
void MonLed2Dat::writeArrayDB(const std::map<EcalLogicID, MonLed2Dat>* data, MonRunIOV* iov) noexcept(false) {
  this->checkConnection();
  this->checkPrepare();

  int iovID = iov->fetchID();
  if (!iovID) {
    throw(std::runtime_error("MonLed2Dat::writeArrayDB:  IOV not in DB"));
  }
  ...
  for (CI p = data->begin(); p != data->end(); ++p) {
    ...
    if (!logicID) {
      throw(std::runtime_error("MonLed2Dat::writeArrayDB:  Bad EcalLogicID"));
    }
    ids[count] = logicID;
