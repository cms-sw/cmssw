  ...
  noexcept(false)
{
  ...
  try {
    ...
    m_writeStmt->setSQL("INSERT INTO mon_pn_blue_dat (iov_id, logic_id, "
			"adc_mean_g1, adc_rms_g1, adc_mean_g16, adc_rms_g16, ped_mean_g1, ped_rms_g1, ped_mean_g16, ped_rms_g16, task_status) "
			"VALUES (:iov_id, :logic_id, "
			":3, :4, :5, :6, :7, :8, :9, :10, :11)");
  } catch (SQLException &e) {
    throw(std::runtime_error(std::string("MonPNBlueDat::prepareWrite():  ")+e.getMessage()));
  }
}



void MonPNBlueDat::writeDB(const EcalLogicID* ecid, const MonPNBlueDat* item, MonRunIOV* iov)
  noexcept(false)
{
  this->checkConnection();
  this->checkPrepare();
  ...
  if (!iovID) { throw(std::runtime_error("MonPNBlueDat::writeDB:  IOV not in DB")); }

  int logicID = ecid->getLogicID();
  if (!logicID) { throw(std::runtime_error("MonPNBlueDat::writeDB:  Bad EcalLogicID")); }
  ...
  try {
    ...
    throw(std::runtime_error(std::string("MonPNBlueDat::writeDB():  ")+e.getMessage()));
  }
}



void MonPNBlueDat::fetchData(std::map< EcalLogicID, MonPNBlueDat >* fillMap, MonRunIOV* iov)
  noexcept(false)
{
  this->checkConnection();
  fillMap->clear();
  ...
  int iovID = iov->fetchID();
  if (!iovID) { 
    //  throw(std::runtime_error("MonPNBlueDat::writeDB:  IOV not in DB")); 
    return;
  }
  ...
  try {
    ...
    m_readStmt->setSQL("SELECT cv.name, cv.logic_id, cv.id1, cv.id2, cv.id3, cv.maps_to, "
		 ...
		 "FROM channelview cv JOIN mon_pn_blue_dat d "
    ...
    throw(std::runtime_error(std::string("MonPNBlueDat::fetchData():  ")+e.getMessage()));
  }
}

void MonPNBlueDat::writeArrayDB(const std::map< EcalLogicID, MonPNBlueDat >* data, MonRunIOV* iov)
  noexcept(false)
{
  this->checkConnection();
  this->checkPrepare();
  ...
  int iovID = iov->fetchID();
  if (!iovID) { throw(std::runtime_error("MonPNBlueDat::writeArrayDB:  IOV not in DB")); }
  ...
  const MonPNBlueDat* dataitem;
  int count=0;
  typedef map< EcalLogicID, MonPNBlueDat >::const_iterator CI;
  for (CI p = data->begin(); p != data->end(); ++p) {
        channel = &(p->first);
	int logicID = channel->getLogicID();
	if (!logicID) { throw(std::runtime_error("MonPNBlueDat::writeArrayDB:  Bad EcalLogicID")); }
	ids[count]=logicID;
  ...
  try {
    ...
    throw(std::runtime_error(std::string("MonPNBlueDat::writeArrayDB():  ")+e.getMessage()));
  }
}
