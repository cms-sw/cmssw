...
void MonPNBlueDat::prepareWrite() noexcept(false) {
...
void MonPNBlueDat::writeDB(const EcalLogicID* ecid, const MonPNBlueDat* item, MonRunIOV* iov) noexcept(false) {
  this->checkConnection();
  this->checkPrepare();
  ...
  if (!logicID) {
    throw(std::runtime_error("MonPNBlueDat::writeDB:  Bad EcalLogicID"));
  }
...
void MonPNBlueDat::fetchData(std::map<EcalLogicID, MonPNBlueDat>* fillMap, MonRunIOV* iov) noexcept(false) {
  this->checkConnection();
  fillMap->clear();
...
void MonPNBlueDat::writeArrayDB(const std::map<EcalLogicID, MonPNBlueDat>* data, MonRunIOV* iov) noexcept(false) {
  this->checkConnection();
  this->checkPrepare();

  int iovID = iov->fetchID();
  if (!iovID) {
    throw(std::runtime_error("MonPNBlueDat::writeArrayDB:  IOV not in DB"));
  }
  ...
  for (CI p = data->begin(); p != data->end(); ++p) {
    ...
    if (!logicID) {
      throw(std::runtime_error("MonPNBlueDat::writeArrayDB:  Bad EcalLogicID"));
    }
    ids[count] = logicID;
