  ...
  noexcept(false)
{
  ...
  try {
    ...
    throw(std::runtime_error(std::string("FEConfigTimingDat::prepareWrite():  ")+e.getMessage()));
  }
}
  ...
  noexcept(false)
{
  ...
  try {
    ...
    throw(std::runtime_error(std::string("FEConfigTimingDat::writeDB():  ")+e.getMessage()));
  }
}
  ...
  try {
    ...
    m_readStmt->setSQL("SELECT cv.name, cv.logic_id, cv.id1, cv.id2, cv.id3, cv.maps_to, "
		 "d.time_par1, d.time_par2  "
		       "FROM channelview cv JOIN " +getTable() + " d ON cv.logic_id = d.logic_id AND cv.name = cv.maps_to "
    ...
    throw(std::runtime_error(std::string("FEConfigTimingDat::fetchData:  ")+e.getMessage()));
  }
}
  ...
  noexcept(false)
{
  ...
  try {
    ...
    throw(std::runtime_error(std::string("FEConfigTimingDat::writeArrayDB():  ")+e.getMessage()));
  }
}
