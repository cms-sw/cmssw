...
std::string LMFSeqDat::writeDBSql(Statement *stmt)
{
  ...
  if (!isValid()) {
    dump();
    throw(runtime_error("LMFSeqDat::writeDB: not valid"));
  }
  ...
  int runIOVID = m_runIOV.getID();
  if (runIOVID == 0) {
    throw(runtime_error("LMFSeqDat::writeDB: RunIOV not set"));
  }
  ...
  throw(runtime_error)
{
  ...
  if (m_runIOV.getID() == 0) { 
    throw(runtime_error("LMFSeqDat:  Given RunIOV does not exist in DB")); 
  }
