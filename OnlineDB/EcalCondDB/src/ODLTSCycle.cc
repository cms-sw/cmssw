  ...
  noexcept(false)
{
  ...
  try {
    ...
    throw(std::runtime_error(std::string("ODLTSCycle::prepareWrite():  ")+e.getMessage()));
  }
}
...
void ODLTSCycle::writeDB()  noexcept(false)
{
  ...
  try {
    ...
    throw(std::runtime_error(std::string("ODLTSCycle::writeDB:  ")+e.getMessage()));
  }
  ...
  noexcept(false)
{
  ...
  try {
    ...
    stmt->setSQL("SELECT cycle_id, lts_configuration_id FROM ecal_lts_cycle "
		 "WHERE cycle_id = :1 ");
    stmt->setInt(1, m_ID);
    ...
    throw(std::runtime_error(std::string("ODLTSCycle::fetchID:  ")+e.getMessage()));
  }
...
void ODLTSCycle::setByID(int id) 
  noexcept(false)
{
   this->checkConnection();
  ...
  noexcept(false)
{
  ...
  try {
    ...
    m_readStmt->setSQL("SELECT  lts_configuration_id FROM ecal_lts_cycle "
		 "WHERE cycle_id = :1 ");
    ...
    throw(std::runtime_error(std::string("ODLTSCycle::fetchData():  ")+e.getMessage()));
  }
}
