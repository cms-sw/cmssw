...
void MonPedestalOffsetsDat::prepareWrite() noexcept(false) {
  this->checkConnection();

  try {
    m_writeStmt = m_conn->createStatement();
    m_writeStmt->setSQL(
        "INSERT INTO mon_pedestal_offsets_dat (iov_id, logic_id, "
...
void MonPedestalOffsetsDat::writeDB(const EcalLogicID* ecid,
  ...
  if (!logicID) {
    throw(std::runtime_error("MonPedestalOffsetsDat::writeDB:  Bad EcalLogicID"));
  }
...
void MonPedestalOffsetsDat::fetchData(std::map<EcalLogicID, MonPedestalOffsetsDat>* fillMap,
  ...
  try {
    m_readStmt->setSQL(
        ...
        "ON cv.logic_id = d.logic_id AND cv.name = " +
        mappa + " WHERE d.iov_id = :iov_id");
...
void MonPedestalOffsetsDat::writeArrayDB(const std::map<EcalLogicID, MonPedestalOffsetsDat>* data,
  ...
  int iovID = iov->fetchID();
  if (!iovID) {
    throw(std::runtime_error("MonPedestalOffsetsDat::writeArrayDB:  IOV not in DB"));
  }
