  ...
  throw(std::runtime_error)
{
  ...
  try {
    ...
    m_writeStmt->setSQL("INSERT INTO "+getTable()+" ( tim_conf_id, tag, version) " 
			" VALUES ( :1, :2, :3 ) " );
...
void FEConfigTimingInfo::setParameters(std::map<string,string> my_keys_map){
  ...
  throw(std::runtime_error)
{
  ...
  try {
    ...
    m_readStmt->setSQL("SELECT * FROM " + getTable() +   
                       " where   tim_conf_id = ( select max( tim_conf_id) from "+ getTable() +" ) " );
    ResultSet* rset = m_readStmt->executeQuery();
...
int FEConfigTimingInfo::fetchID()    throw(std::runtime_error)
{
  ...
  try {
    ...
    stmt->setSQL("SELECT tim_conf_id FROM "+ getTable()+
