  ...
  noexcept(false)
{
  ...
  try {
    ...
    throw(std::runtime_error(std::string("MODDCCDetailsDat::prepareWrite():  ")+e.getMessage()));
  }
}
  ...
  noexcept(false)
{
  ...
  try {
    ...
    throw(std::runtime_error(std::string("MODDCCDetailsDat::writeDB():  ")+e.getMessage()));
  }
}
  ...
  noexcept(false)
{
  ...
  try {
    ...
    m_readStmt->setSQL("SELECT cv.name, cv.logic_id, cv.id1, cv.id2, cv.id3, cv.maps_to, "
		 " d.qpll_error, d.optical_link, d.data_timeout, d.dcc_header, d.event_number, d.bx_number, d.even_parity, d.odd_parity, d.block_size, d.almost_full_fifo, d.full_fifo, d.forced_full_supp " 
    ...
    throw(std::runtime_error(std::string("MODDCCDetailsDat::fetchData():  ")+e.getMessage()));
  }
}
