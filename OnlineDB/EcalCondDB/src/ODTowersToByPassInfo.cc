...
void ODTowersToByPassInfo::prepareWrite() noexcept(false) {
  this->checkConnection();
...
void ODTowersToByPassInfo::setParameters(const std::map<string, string>& my_keys_map) {
...
void ODTowersToByPassInfo::writeDB() noexcept(false) {
  this->checkConnection();
  this->checkPrepare();
  ...
  if (!this->fetchID()) {
    ...
    if (this->getVersion() != old_version)
      std::cout << "ODTowersToByPassInfo>>WARNING version is " << getVersion() << endl;
  }
}

void ODTowersToByPassInfo::fetchData(ODTowersToByPassInfo* result) noexcept(false) {
  this->checkConnection();
  result->clear();
