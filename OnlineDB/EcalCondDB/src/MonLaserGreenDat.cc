...
void MonLaserGreenDat::prepareWrite() noexcept(false) {
  this->checkConnection();

  try {
    m_writeStmt = m_conn->createStatement();
    m_writeStmt->setSQL(
        "INSERT INTO mon_laser_green_dat (iov_id, logic_id, "
...
void MonLaserGreenDat::writeDB(const EcalLogicID* ecid, const MonLaserGreenDat* item, MonRunIOV* iov) noexcept(false) {
  this->checkConnection();
  this->checkPrepare();
  ...
  if (!logicID) {
    throw(std::runtime_error("MonLaserGreenDat::writeDB:  Bad EcalLogicID"));
  }
...
void MonLaserGreenDat::fetchData(std::map<EcalLogicID, MonLaserGreenDat>* fillMap, MonRunIOV* iov) noexcept(false) {
  this->checkConnection();
...
void MonLaserGreenDat::writeArrayDB(const std::map<EcalLogicID, MonLaserGreenDat>* data,
  ...
  int iovID = iov->fetchID();
  if (!iovID) {
    throw(std::runtime_error("MonLaserGreenDat::writeArrayDB:  IOV not in DB"));
  }
  ...
  for (CI p = data->begin(); p != data->end(); ++p) {
    ...
    if (!logicID) {
      throw(std::runtime_error("MonLaserGreenDat::writeArrayDB:  Bad EcalLogicID"));
    }
    ids[count] = logicID;
