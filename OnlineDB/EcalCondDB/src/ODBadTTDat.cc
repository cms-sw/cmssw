...
void ODBadTTDat::prepareWrite() noexcept(false) {
  this->checkConnection();

  try {
    m_writeStmt = m_conn->createStatement();
    m_writeStmt->setSQL("INSERT INTO " + getTable() +
                        " (rec_id, tr_id, fed_id, tt_id, status ) "
                        "VALUES (:1, :2, :3, :4, :5 )");
...
void ODBadTTDat::writeDB(const ODBadTTDat* item, ODBadTTInfo* iov) noexcept(false) {
  this->checkConnection();
...
void ODBadTTDat::fetchData(std::vector<ODBadTTDat>* p, ODBadTTInfo* iov) noexcept(false) {
  this->checkConnection();
...
void ODBadTTDat::writeArrayDB(const std::vector<ODBadTTDat>& data, ODBadTTInfo* iov) noexcept(false) {
  this->checkConnection();

  int iovID = iov->fetchID();
  if (!iovID) {
    throw(std::runtime_error("ODDelays::writeArrayDB:  ODBadTTInfo not in DB"));
  }
