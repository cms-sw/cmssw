...
void MODCCSTRDat::prepareWrite() noexcept(false) {
  this->checkConnection();

  try {
    m_writeStmt = m_conn->createStatement();
    m_writeStmt->setSQL(
        "INSERT INTO OD_CCS_TR_dat (iov_id, logic_id, "
        "ccs_word) "
...
void MODCCSTRDat::writeDB(const EcalLogicID* ecid, const MODCCSTRDat* item, MODRunIOV* iov) noexcept(false) {
  this->checkConnection();
  this->checkPrepare();
  ...
  if (!logicID) {
    throw(std::runtime_error("MODCCSTRDat::writeDB:  Bad EcalLogicID"));
  }
...
void MODCCSTRDat::fetchData(std::map<EcalLogicID, MODCCSTRDat>* fillMap, MODRunIOV* iov) noexcept(false) {
  this->checkConnection();
  fillMap->clear();
...
void MODCCSTRDat::writeArrayDB(const std::map<EcalLogicID, MODCCSTRDat>* data, MODRunIOV* iov) noexcept(false) {
  this->checkConnection();
  this->checkPrepare();

  int iovID = iov->fetchID();
  if (!iovID) {
    throw(std::runtime_error("MODCCSTRDat::writeArrayDB:  IOV not in DB"));
  }
  ...
  for (CI p = data->begin(); p != data->end(); ++p) {
    ...
    if (!logicID) {
      throw(std::runtime_error("MODCCSTRDat::writeArrayDB:  Bad EcalLogicID"));
    }
    ids[count] = logicID;
