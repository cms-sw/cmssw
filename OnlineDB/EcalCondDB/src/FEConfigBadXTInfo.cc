...
void FEConfigBadXTInfo::prepareWrite() noexcept(false) {
  this->checkConnection();
...
void FEConfigBadXTInfo::writeDB() noexcept(false) {
  this->checkConnection();
  this->checkPrepare();
  ...
  if (!this->fetchID()) {
    ...
    if (this->getVersion() != old_version)
      std::cout << "FEConfigBadXTInfo>>WARNING version is " << getVersion() << endl;
  }
}

void FEConfigBadXTInfo::fetchData(FEConfigBadXTInfo* result) noexcept(false) {
  this->checkConnection();
  result->clear();
