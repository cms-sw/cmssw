...
void DCUCapsuleTempRawDat::prepareWrite() noexcept(false) {
  this->checkConnection();

  try {
    m_writeStmt = m_conn->createStatement();
    m_writeStmt->setSQL(
        "INSERT INTO dcu_capsule_temp_raw_dat (iov_id, logic_id, "
...
void DCUCapsuleTempRawDat::writeDB(const EcalLogicID* ecid,
  ...
  if (!logicID) {
    throw(std::runtime_error("DCUCapsuleTempRawDat::writeDB:  Bad EcalLogicID"));
  }
...
void DCUCapsuleTempRawDat::fetchData(std::map<EcalLogicID, DCUCapsuleTempRawDat>* fillMap,
                                     DCUIOV* iov) noexcept(false) {
  this->checkConnection();
  fillMap->clear();
...
void DCUCapsuleTempRawDat::writeArrayDB(const std::map<EcalLogicID, DCUCapsuleTempRawDat>* data,
  ...
  int iovID = iov->fetchID();
  if (!iovID) {
    throw(std::runtime_error("DCUCapsuleTempRawDat::writeArrayDB:  IOV not in DB"));
  }
  ...
  for (CI p = data->begin(); p != data->end(); ++p) {
    ...
    if (!logicID) {
      throw(std::runtime_error("DCUCapsuleTempRawDat::writeArrayDB:  Bad EcalLogicID"));
    }
    ids[count] = logicID;
