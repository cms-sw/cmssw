  ...
  throw(runtime_error)
{
  ...
  try {
    ...
    throw(runtime_error("RunMemTTErrorsDat::prepareWrite():  "+e.getMessage()));
  }
}



void RunMemTTErrorsDat::writeDB(const EcalLogicID* ecid, const RunMemTTErrorsDat* item, RunIOV* iov)
  throw(runtime_error)
{
  this->checkConnection();
  this->checkPrepare();
  ...
  if (!iovID) { throw(runtime_error("RunMemTTErrorsDat::writeDB:  IOV not in DB")); }

  int logicID = ecid->getLogicID();
  if (!logicID) { throw(runtime_error("RunMemTTErrorsDat::writeDB:  Bad EcalLogicID")); }
  ...
  try {
    ...
    throw(runtime_error("RunMemTTErrorsDat::writeDB():  "+e.getMessage()));
  }
}



void RunMemTTErrorsDat::fetchData(map< EcalLogicID, RunMemTTErrorsDat >* fillMap, RunIOV* iov)
  throw(runtime_error)
{
  this->checkConnection();
  fillMap->clear();
  ...
  int iovID = iov->fetchID();
  if (!iovID) { 
    //  throw(runtime_error("RunMemTTErrorsDat::writeDB:  IOV not in DB")); 
    return;
  }
  ...
  try {
    ...
    throw(runtime_error("RunMemTTErrorsDat::fetchData():  "+e.getMessage()));
  }
}
