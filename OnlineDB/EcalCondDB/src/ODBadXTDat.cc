...
void ODBadXTDat::prepareWrite() noexcept(false) {
  this->checkConnection();

  try {
    m_writeStmt = m_conn->createStatement();
    m_writeStmt->setSQL("INSERT INTO " + getTable() +
                        " (rec_id, sm_id, fed_id, tt_id, xt_id, status ) "
                        "VALUES (:1, :2, :3, :4, :5 ,:6 )");
...
void ODBadXTDat::writeDB(const ODBadXTDat* item, ODBadXTInfo* iov) noexcept(false) {
  this->checkConnection();
...
void ODBadXTDat::fetchData(std::vector<ODBadXTDat>* p, ODBadXTInfo* iov) noexcept(false) {
  this->checkConnection();
...
void ODBadXTDat::writeArrayDB(const std::vector<ODBadXTDat>& data, ODBadXTInfo* iov) noexcept(false) {
  this->checkConnection();

  int iovID = iov->fetchID();
  if (!iovID) {
    throw(std::runtime_error("ODDelays::writeArrayDB:  ODBadXTInfo not in DB"));
  }
