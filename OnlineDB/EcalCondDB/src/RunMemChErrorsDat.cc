...
void RunMemChErrorsDat::prepareWrite() noexcept(false) {
...
void RunMemChErrorsDat::writeDB(const EcalLogicID* ecid, const RunMemChErrorsDat* item, RunIOV* iov) noexcept(false) {
  this->checkConnection();
  this->checkPrepare();
  ...
  if (!logicID) {
    throw(std::runtime_error("RunMemChErrorsDat::writeDB:  Bad EcalLogicID"));
  }
...
void RunMemChErrorsDat::fetchData(map<EcalLogicID, RunMemChErrorsDat>* fillMap, RunIOV* iov) noexcept(false) {
  this->checkConnection();
  fillMap->clear();
