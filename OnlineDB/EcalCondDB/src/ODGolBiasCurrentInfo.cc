...
void ODGolBiasCurrentInfo::prepareWrite() noexcept(false) {
  this->checkConnection();
...
void ODGolBiasCurrentInfo::setParameters(const std::map<string, string>& my_keys_map) {
...
void ODGolBiasCurrentInfo::writeDB() noexcept(false) {
  this->checkConnection();
  this->checkPrepare();
  ...
  if (!this->fetchID()) {
    ...
    if (this->getVersion() != old_version)
      std::cout << "ODGolBiasCurrentInfo>>WARNING version is " << getVersion() << endl;
  }
}

void ODGolBiasCurrentInfo::fetchData(ODGolBiasCurrentInfo* result) noexcept(false) {
  this->checkConnection();
  result->clear();
  ...
  try {
    if (result->getId() != 0) {
      ...
      if (result->getVersion() != 0) {
        ...
        m_readStmt->setSQL("SELECT * FROM " + getTable() + " WHERE tag = :1 and version= (select max(version) from " +
                           getTable() + " where tag=:2) ");
        m_readStmt->setString(1, result->getConfigTag());
