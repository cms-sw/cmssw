...
void ODDCCCycle::prepareWrite() noexcept(false) {
  this->checkConnection();

  try {
    m_writeStmt = m_conn->createStatement();
    m_writeStmt->setSQL(
        "INSERT INTO ECAL_DCC_Cycle (cycle_id, dcc_configuration_id ) "
        "VALUES (:1, :2 )");
...
void ODDCCCycle::writeDB() noexcept(false) {
  this->checkConnection();
  this->checkPrepare();
...
void ODDCCCycle::clear() { m_dcc_config_id = 0; }

int ODDCCCycle::fetchID() noexcept(false) {
  ...
  try {
    ...
    stmt->setSQL(
        "SELECT cycle_id, dcc_configuration_id FROM ecal_dcc_cycle "
        "WHERE cycle_id = :1 ");
    stmt->setInt(1, m_ID);
...
void ODDCCCycle::setByID(int id) noexcept(false) {
  this->checkConnection();
...
void ODDCCCycle::fetchData(ODDCCCycle *result) noexcept(false) {
  this->checkConnection();
  result->clear();
  ...
  try {
    m_readStmt->setSQL(
        "SELECT  dcc_configuration_id FROM ecal_dcc_cycle "
        "WHERE cycle_id = :1 ");
...
void ODDCCCycle::insertConfig() noexcept(false) {
  try {
    prepareWrite();
    writeDB();
    m_conn->commit();
