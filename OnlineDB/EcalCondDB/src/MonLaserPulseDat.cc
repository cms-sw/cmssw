  ...
  noexcept(false)
{
  ...
  try {
    ...
    throw(std::runtime_error(std::string("MonLaserPulseDat::prepareWrite():  ")+e.getMessage()));
  }
}
  ...
  noexcept(false)
{
  ...
  if (!iovID) { throw(std::runtime_error("MonLaserPulseDat::writeDB:  IOV not in DB")); }

  int logicID = ecid->getLogicID();
  if (!logicID) { throw(std::runtime_error("MonLaserPulseDat::writeDB:  Bad EcalLogicID")); }
  ...
  try {
    ...
    throw(std::runtime_error(std::string("MonLaserPulseDat::writeDB():  ")+e.getMessage()));
  }
}
  ...
  noexcept(false)
{
  ...
  try {
    ...
    throw(std::runtime_error(std::string("MonLaserPulseDat::fetchData():  ")+e.getMessage()));
  }
}
  ...
  noexcept(false)
{
  ...
  int iovID = iov->fetchID();
  if (!iovID) { throw(std::runtime_error("MonLaserPulseDat::writeArrayDB:  IOV not in DB")); }
  ...
  for (CI p = data->begin(); p != data->end(); ++p) {
	...
	if (!logicID) { throw(std::runtime_error("MonLaserPulseDat::writeArrayDB:  Bad EcalLogicID")); }
	ids[count]=logicID;
  ...
  try {
    ...
    throw(std::runtime_error(std::string("MonLaserPulseDat::writeArrayDB():  ")+e.getMessage()));
  }
}
