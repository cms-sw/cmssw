...
void LMFLaserBlueRawDat::prepareWrite()
  throw(runtime_error)
{
  ...
  try {
    ...
    m_writeStmt->setSQL("INSERT INTO lmf_laser_blue_raw_dat (iov_id, logic_id, "
			"apd_peak, apd_err) "
			"VALUES (:iov_id, :logic_id, "
			":3, :4)");
  } catch (SQLException &e) {
    throw(runtime_error("LMFLaserBlueRawDat::prepareWrite():  "+e.getMessage()));
  }
}



void LMFLaserBlueRawDat::writeDB(const EcalLogicID* ecid, const LMFLaserBlueRawDat* item, LMFRunIOV* iov)
  throw(runtime_error)
{
  this->checkConnection();
  this->checkPrepare();
  ...
  if (!iovID) { throw(runtime_error("LMFLaserBlueRawDat::writeDB:  IOV not in DB")); }

  int logicID = ecid->getLogicID();
  if (!logicID) { throw(runtime_error("LMFLaserBlueRawDat::writeDB:  Bad EcalLogicID")); }
  ...
  try {
    ...
    throw(runtime_error("LMFLaserBlueRawDat::writeDB():  "+e.getMessage()));
  }
}
  ...
  throw(runtime_error)
{
  ...
  int iovID = iov->fetchID();
  if (!iovID) { 
    //  throw(runtime_error("LMFLaserBlueRawDat::writeDB:  IOV not in DB")); 
    return;
  }
