...
void FEConfigLUTDat::prepareWrite() noexcept(false) {
...
void FEConfigLUTDat::writeDB(const EcalLogicID* ecid,
  ...
  if (!logicID) {
    throw(std::runtime_error("FEConfigLUTDat::writeDB:  Bad EcalLogicID"));
  }
...
void FEConfigLUTDat::fetchData(map<EcalLogicID, FEConfigLUTDat>* fillMap, FEConfigLUTInfo* iconf) noexcept(false) {
  this->checkConnection();
  fillMap->clear();
...
void FEConfigLUTDat::writeArrayDB(const std::map<EcalLogicID, FEConfigLUTDat>* data,
  ...
  int iconfID = iconf->fetchID();
  if (!iconfID) {
    throw(std::runtime_error("FEConfigLUTDat::writeArrayDB:  ICONF not in DB"));
  }
  ...
  for (CI p = data->begin(); p != data->end(); ++p) {
    ...
    if (!logicID) {
      throw(std::runtime_error("FEConfigLUTDat::writeArrayDB:  Bad EcalLogicID"));
    }
    ids[count] = logicID;
