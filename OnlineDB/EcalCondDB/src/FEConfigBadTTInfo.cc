...
int FEConfigBadTTInfo::fetchNextId() noexcept(false) {
  int result = 0;
  try {
...
void FEConfigBadTTInfo::prepareWrite() noexcept(false) {
  this->checkConnection();
...
void FEConfigBadTTInfo::setParameters(const std::map<string, string>& my_keys_map) {
...
void FEConfigBadTTInfo::writeDB() noexcept(false) {
  this->checkConnection();
  this->checkPrepare();
  ...
  // Now get the ID
  if (!this->fetchID()) {
    throw(std::runtime_error("FEConfigBadTTInfo::writeDB:  Failed to write"));
  } else {
    ...
    if (this->getVersion() != old_version)
      std::cout << "FEConfigBadTTInfo>>WARNING version is " << getVersion() << endl;
  }
}

void FEConfigBadTTInfo::fetchData(FEConfigBadTTInfo* result) noexcept(false) {
  this->checkConnection();
  result->clear();
  if (result->getId() == 0 && (result->getConfigTag().empty())) {
    throw(std::runtime_error("FEConfigBadTTInfo::fetchData(): no Id defined for this FEConfigBadTTInfo "));
  }
  ...
  try {
    if (result->getId() != 0) {
      ...
      throw(std::runtime_error("FEConfigBadTTInfo::fetchData(): no Id defined for this record "));
    }
...
int FEConfigBadTTInfo::fetchID() noexcept(false) {
