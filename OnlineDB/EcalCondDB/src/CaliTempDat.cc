...
void CaliTempDat::prepareWrite() noexcept(false) {
  this->checkConnection();

  try {
    m_writeStmt = m_conn->createStatement();
    m_writeStmt->setSQL(
        "INSERT INTO cali_temp_dat (iov_id, logic_id, "
...
void CaliTempDat::writeDB(const EcalLogicID* ecid, const CaliTempDat* item, CaliIOV* iov) noexcept(false) {
  this->checkConnection();
  this->checkPrepare();
  ...
  if (!logicID) {
    throw(std::runtime_error("CaliTempDat::writeDB:  Bad EcalLogicID"));
  }
...
void CaliTempDat::fetchData(std::map<EcalLogicID, CaliTempDat>* fillMap, CaliIOV* iov) noexcept(false) {
  this->checkConnection();
  fillMap->clear();
...
void CaliTempDat::writeArrayDB(const std::map<EcalLogicID, CaliTempDat>* data, CaliIOV* iov) noexcept(false) {
  this->checkConnection();
  this->checkPrepare();

  int iovID = iov->fetchID();
  if (!iovID) {
    throw(std::runtime_error("CaliTempDat::writeArrayDB:  IOV not in DB"));
  }
  ...
  for (CI p = data->begin(); p != data->end(); ++p) {
    ...
    if (!logicID) {
      throw(std::runtime_error("CaliTempDat::writeArrayDB:  Bad EcalLogicID"));
    }
    ids[count] = logicID;
