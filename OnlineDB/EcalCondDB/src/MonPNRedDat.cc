...
void MonPNRedDat::prepareWrite() noexcept(false) {
  this->checkConnection();

  try {
    m_writeStmt = m_conn->createStatement();
    m_writeStmt->setSQL(
        "INSERT INTO mon_pn_red_dat (iov_id, logic_id, "
...
void MonPNRedDat::writeDB(const EcalLogicID* ecid, const MonPNRedDat* item, MonRunIOV* iov) noexcept(false) {
  this->checkConnection();
  this->checkPrepare();
  ...
  if (!logicID) {
    throw(std::runtime_error("MonPNRedDat::writeDB:  Bad EcalLogicID"));
  }
...
void MonPNRedDat::fetchData(std::map<EcalLogicID, MonPNRedDat>* fillMap, MonRunIOV* iov) noexcept(false) {
  this->checkConnection();
  fillMap->clear();
...
void MonPNRedDat::writeArrayDB(const std::map<EcalLogicID, MonPNRedDat>* data, MonRunIOV* iov) noexcept(false) {
  this->checkConnection();
  this->checkPrepare();

  int iovID = iov->fetchID();
  if (!iovID) {
    throw(std::runtime_error("MonPNRedDat::writeArrayDB:  IOV not in DB"));
  }
