...
void ODLaserCycle::prepareWrite() noexcept(false) {
...
void ODLaserCycle::writeDB() noexcept(false) {
  this->checkConnection();
  this->checkPrepare();
...
void ODLaserCycle::clear() { m_laser_config_id = 0; }

int ODLaserCycle::fetchID() noexcept(false) {
...
void ODLaserCycle::setByID(int id) noexcept(false) {
  this->checkConnection();
...
void ODLaserCycle::fetchData(ODLaserCycle *result) noexcept(false) {
  this->checkConnection();
  result->clear();
...
void ODLaserCycle::insertConfig() noexcept(false) {
  try {
    prepareWrite();
    writeDB();
    m_conn->commit();
