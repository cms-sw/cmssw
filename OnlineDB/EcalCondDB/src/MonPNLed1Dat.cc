...
void MonPNLed1Dat::prepareWrite() noexcept(false) {
  this->checkConnection();

  try {
    m_writeStmt = m_conn->createStatement();
    m_writeStmt->setSQL(
        "INSERT INTO mon_pn_led1_dat (iov_id, logic_id, "
...
void MonPNLed1Dat::writeDB(const EcalLogicID* ecid, const MonPNLed1Dat* item, MonRunIOV* iov) noexcept(false) {
  this->checkConnection();
  this->checkPrepare();
  ...
  if (!logicID) {
    throw(std::runtime_error("MonPNLed1Dat::writeDB:  Bad EcalLogicID"));
  }
...
void MonPNLed1Dat::fetchData(std::map<EcalLogicID, MonPNLed1Dat>* fillMap, MonRunIOV* iov) noexcept(false) {
  this->checkConnection();
  fillMap->clear();
  ...
  try {
    m_readStmt->setSQL(
        ...
        "FROM channelview cv JOIN mon_pn_led1_dat d "
...
void MonPNLed1Dat::writeArrayDB(const std::map<EcalLogicID, MonPNLed1Dat>* data, MonRunIOV* iov) noexcept(false) {
  this->checkConnection();
  this->checkPrepare();

  int iovID = iov->fetchID();
  if (!iovID) {
    throw(std::runtime_error("MonPNLed1Dat::writeArrayDB:  IOV not in DB"));
  }
  ...
  const MonPNLed1Dat* dataitem;
  int count = 0;
  typedef map<EcalLogicID, MonPNLed1Dat>::const_iterator CI;
  for (CI p = data->begin(); p != data->end(); ++p) {
    channel = &(p->first);
    int logicID = channel->getLogicID();
    if (!logicID) {
      throw(std::runtime_error("MonPNLed1Dat::writeArrayDB:  Bad EcalLogicID"));
    }
    ids[count] = logicID;
