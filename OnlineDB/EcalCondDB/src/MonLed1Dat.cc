...
void MonLed1Dat::prepareWrite() noexcept(false) {
  this->checkConnection();

  try {
    m_writeStmt = m_conn->createStatement();
    m_writeStmt->setSQL(
        "INSERT INTO mon_led1_dat (iov_id, logic_id, "
...
void MonLed1Dat::writeDB(const EcalLogicID* ecid, const MonLed1Dat* item, MonRunIOV* iov) noexcept(false) {
  this->checkConnection();
  this->checkPrepare();
  ...
  if (!logicID) {
    throw(std::runtime_error("MonLed1Dat::writeDB:  Bad EcalLogicID"));
  }
...
void MonLed1Dat::fetchData(std::map<EcalLogicID, MonLed1Dat>* fillMap, MonRunIOV* iov) noexcept(false) {
  this->checkConnection();
...
void MonLed1Dat::writeArrayDB(const std::map<EcalLogicID, MonLed1Dat>* data, MonRunIOV* iov) noexcept(false) {
  this->checkConnection();
  this->checkPrepare();

  int iovID = iov->fetchID();
  if (!iovID) {
    throw(std::runtime_error("MonLed1Dat::writeArrayDB:  IOV not in DB"));
  }
  ...
  for (CI p = data->begin(); p != data->end(); ++p) {
    ...
    if (!logicID) {
      throw(std::runtime_error("MonLed1Dat::writeArrayDB:  Bad EcalLogicID"));
    }
    ids[count] = logicID;
