...
void MonPNGreenDat::prepareWrite() noexcept(false) {
  this->checkConnection();

  try {
    m_writeStmt = m_conn->createStatement();
    m_writeStmt->setSQL(
        "INSERT INTO mon_pn_green_dat (iov_id, logic_id, "
        "adc_mean_g1, adc_rms_g1, adc_mean_g16, adc_rms_g16, ped_mean_g1, ped_rms_g1, ped_mean_g16, ped_rms_g16, "
        "task_status) "
...
void MonPNGreenDat::writeDB(const EcalLogicID* ecid, const MonPNGreenDat* item, MonRunIOV* iov) noexcept(false) {
  this->checkConnection();
  this->checkPrepare();
  ...
  if (!logicID) {
    throw(std::runtime_error("MonPNGreenDat::writeDB:  Bad EcalLogicID"));
  }
...
void MonPNGreenDat::fetchData(std::map<EcalLogicID, MonPNGreenDat>* fillMap, MonRunIOV* iov) noexcept(false) {
  this->checkConnection();
  fillMap->clear();
  ...
  try {
    m_readStmt->setSQL(
        ...
        "d.adc_mean_g1, d.adc_rms_g1, d.adc_mean_g16, d.adc_rms_g16, d.ped_mean_g1,d.ped_rms_g1, d.ped_mean_g16, "
        "d.ped_rms_g16, d.task_status "
...
void MonPNGreenDat::writeArrayDB(const std::map<EcalLogicID, MonPNGreenDat>* data, MonRunIOV* iov) noexcept(false) {
  this->checkConnection();
  this->checkPrepare();

  int iovID = iov->fetchID();
  if (!iovID) {
    throw(std::runtime_error("MonPNGreenDat::writeArrayDB:  IOV not in DB"));
  }
  ...
  for (CI p = data->begin(); p != data->end(); ++p) {
    ...
    if (!logicID) {
      throw(std::runtime_error("MonPNGreenDat::writeArrayDB:  Bad EcalLogicID"));
    }
    ids[count] = logicID;
