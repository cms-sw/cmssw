...
void RunPNErrorsDat::prepareWrite() noexcept(false) {
...
void RunPNErrorsDat::writeDB(const EcalLogicID* ecid, const RunPNErrorsDat* item, RunIOV* iov) noexcept(false) {
  this->checkConnection();
  this->checkPrepare();
  ...
  if (!logicID) {
    throw(std::runtime_error("RunPNErrorsDat::writeDB:  Bad EcalLogicID"));
  }
...
void RunPNErrorsDat::fetchData(map<EcalLogicID, RunPNErrorsDat>* fillMap, RunIOV* iov) noexcept(false) {
  this->checkConnection();
  fillMap->clear();
