...
void ODBadXTInfo::prepareWrite() noexcept(false) {
  this->checkConnection();
...
void ODBadXTInfo::writeDB() noexcept(false) {
  this->checkConnection();
  this->checkPrepare();
  ...
  if (!this->fetchID()) {
    ...
    if (this->getVersion() != old_version)
      std::cout << "ODBadXTInfo>>WARNING version is " << getVersion() << endl;
  }
}

void ODBadXTInfo::fetchData(ODBadXTInfo* result) noexcept(false) {
  this->checkConnection();
  result->clear();
