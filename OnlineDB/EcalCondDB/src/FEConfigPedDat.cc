...
void FEConfigPedDat::prepareWrite() noexcept(false) {
...
void FEConfigPedDat::writeDB(const EcalLogicID* ecid,
                             ...
                             FEConfigPedInfo* iconf) noexcept(false) {
  this->checkConnection();
  ...
  if (!logicID) {
    throw(std::runtime_error("FEConfigPedDat::writeDB:  Bad EcalLogicID"));
  }
...
void FEConfigPedDat::fetchData(map<EcalLogicID, FEConfigPedDat>* fillMap, FEConfigPedInfo* iconf) noexcept(false) {
  this->checkConnection();
  fillMap->clear();
...
void FEConfigPedDat::writeArrayDB(const std::map<EcalLogicID, FEConfigPedDat>* data,
  ...
  int iconfID = iconf->fetchID();
  if (!iconfID) {
    throw(std::runtime_error("FEConfigPedDat::writeArrayDB:  ICONF not in DB"));
  }
  ...
  for (CI p = data->begin(); p != data->end(); ++p) {
    ...
    if (!logicID) {
      throw(std::runtime_error("FEConfigPedDat::writeArrayDB:  Bad EcalLogicID"));
    }
    ids[count] = logicID;
