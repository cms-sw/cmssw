...
int LMFLaserPrimDat::_color = LMFLaserPrimDat::iBlue; //GHM

LMFLaserPrimDat::LMFLaserPrimDat()
{
...
LMFLaserPrimDat::LMFLaserPrimDat( const LMFLaserPrimDat& o ) :  // GHM
  m_Mean(o.m_Mean), m_RMS(o.m_RMS), 
  m_Peak(o.m_Peak), m_Flag(o.m_Flag),
  m_apdOverPNAMean(o.m_apdOverPNAMean),
  m_apdOverPNARMS(o.m_apdOverPNARMS),
  m_apdOverPNAPeak(o.m_apdOverPNAPeak),
  m_apdOverPNBMean(o.m_apdOverPNBMean),
  m_apdOverPNBRMS(o.m_apdOverPNBRMS),
  m_apdOverPNBPeak(o.m_apdOverPNBPeak),
  m_apdOverPNMean(o.m_apdOverPNMean),
  m_apdOverPNRMS(o.m_apdOverPNRMS),
  m_apdOverPNPeak(o.m_apdOverPNPeak),
  m_Alpha(o.m_Alpha),
  m_Beta(o.m_Beta),
  m_ShapeCor(o.m_ShapeCor)
{
  assert( _color>=iBlue && _color<=iIRed );
  ...
  throw(runtime_error)
{
  ...
  try {
    ...
    throw(runtime_error("LMFLaserPrimDat::prepareWrite():  "+e.getMessage()));
  }
}



void LMFLaserPrimDat::writeDB(const EcalLogicID* ecid, const LMFLaserPrimDat* item, LMFRunIOV* iov)
  throw(runtime_error)
{
  this->checkConnection();
  this->checkPrepare();
  ...
  if (!iovID) { throw(runtime_error("LMFLaserPrimDat::writeDB:  IOV not in DB")); }

  int logicID = ecid->getLogicID();
  if (!logicID) { throw(runtime_error("LMFLaserPrimDat::writeDB:  Bad EcalLogicID")); }
  ...
  try {
    ...
    throw(runtime_error("LMFLaserPrimDat::writeDB():  "+e.getMessage()));
  }
}


void LMFLaserPrimDat::writeArrayDB(const std::map< EcalLogicID, LMFLaserPrimDat >* data, LMFRunIOV* iov)
  throw(runtime_error)
{
  this->checkConnection();
  this->checkPrepare();
  ...
  int iovID = iov->fetchID();
  if (!iovID) { throw(runtime_error("LMFLaserPrimDat::writeArrayDB:  IOV not in DB")); }
  ...
  const LMFLaserPrimDat* dataitem;
  int count=0;
  typedef map< EcalLogicID, LMFLaserPrimDat >::const_iterator CI;
  for (CI p = data->begin(); p != data->end(); ++p) {
        channel = &(p->first);
	int logicID = channel->getLogicID();
	if (!logicID) { throw(runtime_error("LMFLaserPrimDat::writeArrayDB:  Bad EcalLogicID")); }
	ids[count]=logicID;
  ...
  try {
    ...
    throw(runtime_error("LMFLaserPrimDat::writeArrayDB():  "+e.getMessage()));
  }
}

void LMFLaserPrimDat::fetchData(std::map< EcalLogicID, LMFLaserPrimDat >* fillMap, LMFRunIOV* iov)
  throw(runtime_error)
{
  this->checkConnection();
  fillMap->clear();
  ...
  int iovID = iov->fetchID();
  if (!iovID) { 
    //  throw(runtime_error("LMFLaserPrimDat::writeDB:  IOV not in DB")); 
    return;
  }

  // GHM
  std::string command_ = "SELECT cv.name, cv.logic_id, cv.id1, cv.id2, cv.id3, cv.maps_to, d.flag, d.mean, d.rms, d.peak, d.apd_over_pnA_mean, d.apd_over_pnA_rms, d.apd_over_pnA_peak, d.apd_over_pnB_mean, d.apd_over_pnB_rms,d.apd_over_pnB_peak, d.apd_over_pn_mean, d.apd_over_pn_rms, d.apd_over_pn_peak, d.alpha, d.beta, d.shape_cor FROM channelview cv JOIN XXXXXX d ON cv.logic_id = d.logic_id AND cv.name = cv.maps_to WHERE d.lmf_iov_id = :iov_id";
  command_.replace( command_.find("XXXXXX",0), 6, getTable() );

  //  cout << "in fetchData: command_=" << command_ << endl;
  ...
  try {
    ...
    std::pair< EcalLogicID, LMFLaserPrimDat > p;
    LMFLaserPrimDat dat;
    while(rset->next()) {
    ...
    throw(runtime_error("LMFLaserPrimDat::fetchData():  "+e.getMessage()));
  }
}

void
LMFLaserPrimDat::setColor( int color ) { _color = color; }
