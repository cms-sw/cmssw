  ...
  noexcept(false)
{
  ...
  try {
    ...
    stmt->setSQL("SELECT tag_id FROM cali_tag WHERE "
		 "gen_tag     = :1 AND "
		 "location_id = :2 AND "
		 "method      = :3 AND "
		 "version     = :4 AND "
    ...
    throw(std::runtime_error(std::string("CaliTag::fetchID:  ")+e.getMessage()));
  }
  ...
  noexcept(false)
{
  ...
  try {
    ...
    stmt->setSQL("SELECT gen_tag, location_id, method, version, data_type "
   ...
   throw(std::runtime_error(std::string("CaliTag::setByID:  ")+e.getMessage()));
  }
}
  ...
  noexcept(false)
{
  ...
  try {
    ...
    stmt->setSQL("INSERT INTO cali_tag (tag_id, gen_tag, location_id, method, version, data_type) "
   ...
   throw(std::runtime_error(std::string("CaliTag::writeDB:  ")+e.getMessage()));
  }
  ...
  noexcept(false)
{
  ...
  try {
    ...
    throw(std::runtime_error(std::string("CaliTag::fetchAllTags:  ")+e.getMessage()));
  }
}
