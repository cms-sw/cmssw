...
void MODCCSFEDat::prepareWrite() noexcept(false) {
...
void MODCCSFEDat::writeDB(const EcalLogicID* ecid, const MODCCSFEDat* item, MODRunIOV* iov) noexcept(false) {
  this->checkConnection();
  this->checkPrepare();
  ...
  if (!logicID) {
    throw(std::runtime_error("MODCCSFEDat::writeDB:  Bad EcalLogicID"));
  }
...
void MODCCSFEDat::fetchData(std::map<EcalLogicID, MODCCSFEDat>* fillMap, MODRunIOV* iov) noexcept(false) {
  this->checkConnection();
  fillMap->clear();
...
void MODCCSFEDat::writeArrayDB(const std::map<EcalLogicID, MODCCSFEDat>* data, MODRunIOV* iov) noexcept(false) {
  this->checkConnection();
  this->checkPrepare();

  int iovID = iov->fetchID();
  if (!iovID) {
    throw(std::runtime_error("MODCCSFEDat::writeArrayDB:  IOV not in DB"));
  }
