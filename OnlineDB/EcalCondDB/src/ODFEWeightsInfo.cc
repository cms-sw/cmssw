...
void ODFEWeightsInfo::prepareWrite() noexcept(false) {
  this->checkConnection();
...
void ODFEWeightsInfo::writeDB() noexcept(false) {
  this->checkConnection();
  this->checkPrepare();
  ...
  if (!this->fetchID()) {
    ...
    if (this->getVersion() != old_version)
      std::cout << "ODFEWeightsInfo>>WARNING version is " << getVersion() << endl;
  }
}

void ODFEWeightsInfo::fetchData(ODFEWeightsInfo* result) noexcept(false) {
  this->checkConnection();
  result->clear();
