...
void ODTTCciCycle::prepareWrite()
  noexcept(false)
{
  ...
  try {
    ...
    throw(std::runtime_error(std::string("ODTTCciCycle::prepareWrite():  ")+e.getMessage()));
  }
}
...
void ODTTCciCycle::writeDB()  noexcept(false)
{
  ...
  try {
    ...
    throw(std::runtime_error(std::string("ODTTCciCycle::writeDB:  ")+e.getMessage()));
  }
...
void ODTTCciCycle::clear(){
  m_ttcci_config_id=0;
}


int ODTTCciCycle::fetchID()
  noexcept(false)
{
  ...
  try {
    ...
    throw(std::runtime_error(std::string("ODTTCciCycle::fetchID:  ")+e.getMessage()));
  }
...
void ODTTCciCycle::setByID(int id) 
  noexcept(false)
{
   this->checkConnection();
...
void ODTTCciCycle::fetchData(ODTTCciCycle * result)
  noexcept(false)
{
  this->checkConnection();
  result->clear();
  ...
  try {
    ...
    throw(std::runtime_error(std::string("ODTTCciCycle::fetchData():  ")+e.getMessage()));
  }
}


void ODTTCciCycle::insertConfig()
  noexcept(false)
{
  try {
