...
void FEConfigSlidingInfo::clear() { m_iov_id = 0; }
...
void FEConfigSlidingInfo::prepareWrite() noexcept(false) {
  this->checkConnection();
  ...
  try {
    ...
    m_writeStmt->setSQL("INSERT INTO " + getTable() +
                        " ( sli_conf_id, tag, version, iov_id) "
                        " VALUES ( :1, :2, :3 , :4) ");
...
void FEConfigSlidingInfo::setParameters(const std::map<string, string>& my_keys_map) {
...
void FEConfigSlidingInfo::writeDB() noexcept(false) {
  this->checkConnection();
  this->checkPrepare();
...
void FEConfigSlidingInfo::fetchData(FEConfigSlidingInfo* result) noexcept(false) {
  this->checkConnection();
  result->clear();
  ...
  try {
    ...
    m_readStmt->setSQL("SELECT * FROM " + getTable() + " where ( sli_conf_id= :1 or (tag=:2 AND version=:3 ) )");
...
void FEConfigSlidingInfo::fetchLastData(FEConfigSlidingInfo* result) noexcept(false) {
  this->checkConnection();
  result->clear();
  try {
    DateHandler dh(m_env, m_conn);

    m_readStmt->setSQL("SELECT * FROM " + getTable() + " where   sli_conf_id = ( select max( sli_conf_id) from " +
                       getTable() + " ) ");
    ResultSet* rset = m_readStmt->executeQuery();
...
int FEConfigSlidingInfo::fetchID() noexcept(false) {
  ...
  try {
    ...
    stmt->setSQL("SELECT sli_conf_id FROM " + getTable() + " WHERE  tag=:1 and version=:2 ");
...
void FEConfigSlidingInfo::setByID(int id) noexcept(false) {
  this->checkConnection();
