...
void FEConfigSlidingDat::prepareWrite() noexcept(false) {
  this->checkConnection();

  try {
    m_writeStmt = m_conn->createStatement();
    m_writeStmt->setSQL(
        "INSERT INTO fe_config_sliding_dat (sli_conf_id, logic_id, "
        "sliding ) "
...
void FEConfigSlidingDat::writeDB(const EcalLogicID* ecid,
                                 ...
                                 FEConfigSlidingInfo* iconf) noexcept(false) {
  this->checkConnection();
  this->checkPrepare();
  ...
  if (!logicID) {
    throw(std::runtime_error("FEConfigSlidingDat::writeDB:  Bad EcalLogicID"));
  }
...
void FEConfigSlidingDat::fetchData(map<EcalLogicID, FEConfigSlidingDat>* fillMap,
                                   FEConfigSlidingInfo* iconf) noexcept(false) {
  this->checkConnection();
  fillMap->clear();
...
void FEConfigSlidingDat::writeArrayDB(const std::map<EcalLogicID, FEConfigSlidingDat>* data,
  ...
  int iconfID = iconf->fetchID();
  if (!iconfID) {
    throw(std::runtime_error("FEConfigSlidingDat::writeArrayDB:  ICONF not in DB"));
  }
  ...
  for (CI p = data->begin(); p != data->end(); ++p) {
    ...
    if (!logicID) {
      throw(std::runtime_error("FEConfigSlidingDat::writeArrayDB:  Bad EcalLogicID"));
    }
    ids[count] = logicID;
