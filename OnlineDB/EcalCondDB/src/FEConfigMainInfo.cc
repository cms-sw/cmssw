...
int FEConfigMainInfo::fetchNextId()  noexcept(false) {
  ...
  try {
    ...
    throw(std::runtime_error(std::string("FEConfigMainInfo::fetchNextId():  ")+e.getMessage()));
  }
  ...
  noexcept(false)
{
 ...
 std::cout << " tag/version " << getConfigTag() <<"/"<<getVersion() << std::endl;
  ...
  try {
    ...
    throw(std::runtime_error(std::string("FEConfigMainInfo::fetchID:  ")+e.getMessage()));
  }
  setByID(m_ID);
  return m_ID;
}
  ...
  noexcept(false)
{
  ...
  try {
    ...
    throw(std::runtime_error(std::string("FEConfigMainInfo::prepareWrite():  ")+e.getMessage()));
  }
  ...
  noexcept(false)
{
  ...
  try {
    ...
    throw(std::runtime_error(std::string("FEConfigMainInfo::writeDB:  ")+e.getMessage()));
  }
  // Now get the ID
  if (!this->fetchID()) {
  ...
  noexcept(false)
{
  ...
  try {
    ...
    throw(std::runtime_error(std::string("ODRunConfigInfo::fetchIDLast:  ")+e.getMessage()));
  }
  ...
  noexcept(false)
{
   ...
   try {
     ...
     throw(std::runtime_error(std::string("FEConfigMainInfo::setByID:  ")+e.getMessage()));
   }
}
...
{ std::cout << " ### 1 getId from FEConfigMainInfo = " << result->getId() << std::endl;
 std::cout << " tag/version " << result->getConfigTag() <<"/"<<result->getVersion() << std::endl;
  ...
  try {
    ...
    throw(std::runtime_error(std::string("FEConfigMainInfo::fetchData():  ")+e.getMessage()));
  }
}
