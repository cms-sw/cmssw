...
void DCUIDarkDat::prepareWrite() noexcept(false) {
  this->checkConnection();

  try {
    m_writeStmt = m_conn->createStatement();
    m_writeStmt->setSQL(
        "INSERT INTO dcu_idark_dat (iov_id, logic_id, "
        "apd_idark) "
...
void DCUIDarkDat::writeDB(const EcalLogicID* ecid, const DCUIDarkDat* item, DCUIOV* iov) noexcept(false) {
  this->checkConnection();
  this->checkPrepare();
  ...
  if (!logicID) {
    throw(std::runtime_error("DCUIDarkDat::writeDB:  Bad EcalLogicID"));
  }
...
void DCUIDarkDat::fetchData(std::map<EcalLogicID, DCUIDarkDat>* fillMap, DCUIOV* iov) noexcept(false) {
  this->checkConnection();
  fillMap->clear();
...
void DCUIDarkDat::writeArrayDB(const std::map<EcalLogicID, DCUIDarkDat>* data, DCUIOV* iov) noexcept(false) {
  this->checkConnection();
  this->checkPrepare();

  int iovID = iov->fetchID();
  if (!iovID) {
    throw(std::runtime_error("DCUIDarkDat::writeArrayDB:  IOV not in DB"));
  }
