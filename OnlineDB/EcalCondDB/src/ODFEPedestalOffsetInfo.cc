...
void ODFEPedestalOffsetInfo::prepareWrite() noexcept(false) {
  this->checkConnection();
...
void ODFEPedestalOffsetInfo::writeDB() noexcept(false) {
  this->checkConnection();
  this->checkPrepare();
  ...
  if (!this->fetchID()) {
    ...
    if (this->getVersion() != old_version)
      std::cout << "ODFEPedestalOffsetInfo>>WARNING version is " << getVersion() << endl;
  }
}

void ODFEPedestalOffsetInfo::fetchData(ODFEPedestalOffsetInfo* result) noexcept(false) {
  this->checkConnection();
  result->clear();
...
void ODFEPedestalOffsetInfo::fetchLastData(ODFEPedestalOffsetInfo* result) noexcept(false) {
  this->checkConnection();
  result->clear();
  try {
    m_readStmt->setSQL("SELECT * FROM " + getTable() + " where  rec_id = ( select max(rec_id) from " + getTable() +
                       " ) ");
    ResultSet* rset = m_readStmt->executeQuery();
