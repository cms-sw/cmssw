...
void ODTCCEEConfig::setParameters(const std::map<string, string>& my_keys_map) {
  ...
  for (std::map<std::string, std::string>::const_iterator ci = my_keys_map.begin(); ci != my_keys_map.end(); ci++) {
    ...
    if (ci->first == "SLB_LATENCY")
      setSLBLatency(atoi(ci->second.c_str()));
    if (ci->first == "SLBLATENCY")
      setSLBLatency(atoi(ci->second.c_str()));

    if (ci->first == "TRIGGER_POS")
      setTriggerPos(atoi(ci->second.c_str()));
  }
}

void ODTCCEEConfig::prepareWrite() noexcept(false) {
  this->checkConnection();
  ...
  try {
    ...
    m_writeStmt->setSQL(
        "INSERT INTO ECAL_TCC_EE_CONFIGURATION (tcc_ee_configuration_id, tcc_ee_tag, "
    ...
    m_writeStmt = m_conn->createStatement();
    m_writeStmt->setSQL(
        "SELECT tcc_configuration, lut_configuration, slb_configuration FROM ECAL_TCC_EE_CONFIGURATION WHERE"
...
void ODTCCEEConfig::writeDB() noexcept(false) {
...
void ODTCCEEConfig::fetchData(ODTCCEEConfig* result) noexcept(false) {
  this->checkConnection();
  result->clear();
...
int ODTCCEEConfig::fetchID() noexcept(false) {
  ...
  try {
    ...
    stmt->setSQL(
        "SELECT tcc_ee_configuration_id FROM ecal_tcc_ee_configuration "
