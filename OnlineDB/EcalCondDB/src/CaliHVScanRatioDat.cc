...
void CaliHVScanRatioDat::prepareWrite() noexcept(false) {
  this->checkConnection();

  try {
    m_writeStmt = m_conn->createStatement();
    m_writeStmt->setSQL(
        "INSERT INTO cali_hv_scan_ratio_dat (iov_id, logic_id, "
...
void CaliHVScanRatioDat::writeDB(const EcalLogicID* ecid,
  ...
  if (!logicID) {
    throw(std::runtime_error("CaliHVScanRatioDat::writeDB:  Bad EcalLogicID"));
  }
...
void CaliHVScanRatioDat::fetchData(std::map<EcalLogicID, CaliHVScanRatioDat>* fillMap, CaliIOV* iov) noexcept(false) {
  this->checkConnection();
  fillMap->clear();
