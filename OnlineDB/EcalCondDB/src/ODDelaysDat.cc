...
void ODDelaysDat::prepareWrite() noexcept(false) {
...
void ODDelaysDat::writeDB(const ODDelaysDat* item, ODFEDelaysInfo* iov) noexcept(false) {
  this->checkConnection();
...
void ODDelaysDat::fetchData(std::vector<ODDelaysDat>* p, ODFEDelaysInfo* iov) noexcept(false) {
  iov->setConnection(m_env, m_conn);
  int iovID = iov->fetchID();
  fetchData(p, iovID);
}

void ODDelaysDat::fetchData(std::vector<ODDelaysDat>* p, int iovID) noexcept(false) {
  this->checkConnection();
...
void ODDelaysDat::writeArrayDB(const std::vector<ODDelaysDat>& data, ODFEDelaysInfo* iov) noexcept(false) {
  this->checkConnection();
