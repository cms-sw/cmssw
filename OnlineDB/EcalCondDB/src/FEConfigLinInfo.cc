...
void FEConfigLinInfo::clear() { m_iov_id = 0; }
...
void FEConfigLinInfo::prepareWrite() noexcept(false) {
  this->checkConnection();
...
void FEConfigLinInfo::setParameters(const std::map<string, string>& my_keys_map) {
...
void FEConfigLinInfo::writeDB() noexcept(false) {
  this->checkConnection();
  this->checkPrepare();
...
void FEConfigLinInfo::fetchData(FEConfigLinInfo* result) noexcept(false) {
  this->checkConnection();
  result->clear();
  ...
  try {
    ...
    m_readStmt->setSQL("SELECT * FROM " + getTable() + " where ( lin_conf_id= :1 or (tag=:2 AND version=:3 ) )");
...
void FEConfigLinInfo::fetchLastData(FEConfigLinInfo* result) noexcept(false) {
  this->checkConnection();
  result->clear();
  try {
    DateHandler dh(m_env, m_conn);

    m_readStmt->setSQL("SELECT * FROM " + getTable() + " where   lin_conf_id = ( select max( lin_conf_id) from " +
                       getTable() + " ) ");
    ResultSet* rset = m_readStmt->executeQuery();
...
int FEConfigLinInfo::fetchID() noexcept(false) {
  ...
  try {
    ...
    stmt->setSQL("SELECT lin_conf_id FROM " + getTable() + " WHERE  tag=:1 and version=:2 ");
...
void FEConfigLinInfo::setByID(int id) noexcept(false) {
  this->checkConnection();
