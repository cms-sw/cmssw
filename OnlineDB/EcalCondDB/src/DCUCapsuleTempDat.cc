...
void DCUCapsuleTempDat::prepareWrite() noexcept(false) {
...
void DCUCapsuleTempDat::writeDB(const EcalLogicID* ecid, const DCUCapsuleTempDat* item, DCUIOV* iov) noexcept(false) {
  this->checkConnection();
  this->checkPrepare();
  ...
  if (!logicID) {
    throw(std::runtime_error("DCUCapsuleTempDat::writeDB:  Bad EcalLogicID"));
  }
...
void DCUCapsuleTempDat::fetchData(std::map<EcalLogicID, DCUCapsuleTempDat>* fillMap, DCUIOV* iov) noexcept(false) {
  this->checkConnection();
  fillMap->clear();
...
void DCUCapsuleTempDat::writeArrayDB(const std::map<EcalLogicID, DCUCapsuleTempDat>* data,
  ...
  int iovID = iov->fetchID();
  if (!iovID) {
    throw(std::runtime_error("DCUCapsuleTempDat::writeArrayDB:  IOV not in DB"));
  }
  ...
  for (CI p = data->begin(); p != data->end(); ++p) {
    ...
    if (!logicID) {
      throw(std::runtime_error("DCUCapsuleTempDat::writeArrayDB:  Bad EcalLogicID"));
    }
    ids[count] = logicID;
