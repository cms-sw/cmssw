...
void MonShapeQualityDat::prepareWrite() noexcept(false) {
...
void MonShapeQualityDat::writeDB(const EcalLogicID* ecid,
  ...
  if (!logicID) {
    throw(std::runtime_error("MonShapeQualityDat::writeDB:  Bad EcalLogicID"));
  }
...
void MonShapeQualityDat::fetchData(std::map<EcalLogicID, MonShapeQualityDat>* fillMap, MonRunIOV* iov) noexcept(false) {
  this->checkConnection();
  fillMap->clear();
...
void MonShapeQualityDat::writeArrayDB(const std::map<EcalLogicID, MonShapeQualityDat>* data,
  ...
  int iovID = iov->fetchID();
  if (!iovID) {
    throw(std::runtime_error("MonShapeQualityDat::writeArrayDB:  IOV not in DB"));
  }
