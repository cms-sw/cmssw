...
void MonTTConsistencyDat::prepareWrite() noexcept(false) {
  this->checkConnection();

  try {
    m_writeStmt = m_conn->createStatement();
    m_writeStmt->setSQL(
        "INSERT INTO mon_tt_consistency_dat (iov_id, logic_id, "
        "processed_events, problematic_events, problems_id, problems_size, problems_LV1, problems_bunch_X, "
        "task_status) "
...
void MonTTConsistencyDat::writeDB(const EcalLogicID* ecid,
  ...
  if (!logicID) {
    throw(std::runtime_error("MonTTConsistencyDat::writeDB:  Bad EcalLogicID"));
  }
...
void MonTTConsistencyDat::fetchData(std::map<EcalLogicID, MonTTConsistencyDat>* fillMap,
                                    MonRunIOV* iov) noexcept(false) {
  this->checkConnection();
  fillMap->clear();
  ...
  try {
    m_readStmt->setSQL(
        ...
        "d.processed_events, d.problematic_events, d.problems_id, d.problems_size, d.problems_LV1, d.problems_bunch_X, "
        "d.task_status "
...
void MonTTConsistencyDat::writeArrayDB(const std::map<EcalLogicID, MonTTConsistencyDat>* data,
  ...
  int iovID = iov->fetchID();
  if (!iovID) {
    throw(std::runtime_error("MonTTConsistencyDat::writeArrayDB:  IOV not in DB"));
  }
