...
void MonH4TablePositionDat::prepareWrite() noexcept(false) {
  this->checkConnection();

  try {
    m_writeStmt = m_conn->createStatement();
    m_writeStmt->setSQL(
        "INSERT INTO mon_h4_table_position_dat (iov_id, logic_id, "
        "table_x, table_y) "
...
void MonH4TablePositionDat::writeDB(const EcalLogicID* ecid,
  ...
  if (!logicID) {
    throw(std::runtime_error("MonH4TablePositionDat::writeDB:  Bad EcalLogicID"));
  }
...
void MonH4TablePositionDat::writeArrayDB(const std::map<EcalLogicID, MonH4TablePositionDat>* data,
  ...
  int iovID = iov->fetchID();
  if (!iovID) {
    throw(std::runtime_error("MonH4TablePositionDat::writeArrayDB:  IOV not in DB"));
  }
