...
void CaliCrystalIntercalDat::prepareWrite() noexcept(false) {
  this->checkConnection();

  try {
    m_writeStmt = m_conn->createStatement();
    m_writeStmt->setSQL(
        "INSERT INTO cali_crystal_intercal_dat (iov_id, logic_id, "
...
void CaliCrystalIntercalDat::writeDB(const EcalLogicID* ecid,
  ...
  if (!logicID) {
    throw(std::runtime_error("CaliCrystalIntercalDat::writeDB:  Bad EcalLogicID"));
  }
...
void CaliCrystalIntercalDat::fetchData(std::map<EcalLogicID, CaliCrystalIntercalDat>* fillMap,
                                       CaliIOV* iov) noexcept(false) {
  this->checkConnection();
  fillMap->clear();
...
void CaliCrystalIntercalDat::writeArrayDB(const std::map<EcalLogicID, CaliCrystalIntercalDat>* data,
  ...
  int iovID = iov->fetchID();
  if (!iovID) {
    throw(std::runtime_error("CaliCrystalIntercalDat::writeArrayDB:  IOV not in DB"));
  }
  ...
  for (CI p = data->begin(); p != data->end(); ++p) {
    ...
    if (!logicID) {
      throw(std::runtime_error("CaliCrystalIntercalDat::writeArrayDB:  Bad EcalLogicID"));
    }
    ids[count] = logicID;
