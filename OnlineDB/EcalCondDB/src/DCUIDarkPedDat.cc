...
void DCUIDarkPedDat::prepareWrite() noexcept(false) {
  this->checkConnection();

  try {
    m_writeStmt = m_conn->createStatement();
    m_writeStmt->setSQL(
        "INSERT INTO dcu_idark_ped_dat (iov_id, logic_id, "
        "ped) "
...
void DCUIDarkPedDat::writeDB(const EcalLogicID* ecid, const DCUIDarkPedDat* item, DCUIOV* iov) noexcept(false) {
  this->checkConnection();
  this->checkPrepare();
  ...
  if (!logicID) {
    throw(std::runtime_error("DCUIDarkPedDat::writeDB:  Bad EcalLogicID"));
  }
...
void DCUIDarkPedDat::fetchData(std::map<EcalLogicID, DCUIDarkPedDat>* fillMap, DCUIOV* iov) noexcept(false) {
  this->checkConnection();
  fillMap->clear();
...
void DCUIDarkPedDat::writeArrayDB(const std::map<EcalLogicID, DCUIDarkPedDat>* data, DCUIOV* iov) noexcept(false) {
  this->checkConnection();
  this->checkPrepare();

  int iovID = iov->fetchID();
  if (!iovID) {
    throw(std::runtime_error("DCUIDarkPedDat::writeArrayDB:  IOV not in DB"));
  }
  ...
  for (CI p = data->begin(); p != data->end(); ++p) {
    ...
    if (!logicID) {
      throw(std::runtime_error("DCUIDarkPedDat::writeArrayDB:  Bad EcalLogicID"));
    }
    ids[count] = logicID;
