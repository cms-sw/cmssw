...
void FEConfigWeightInfo::clear() { m_ngr = 0; }
...
void FEConfigWeightInfo::prepareWrite() noexcept(false) {
  this->checkConnection();
...
void FEConfigWeightInfo::writeDB() noexcept(false) {
  this->checkConnection();
  this->checkPrepare();
...
void FEConfigWeightInfo::fetchData(FEConfigWeightInfo* result) noexcept(false) {
  this->checkConnection();
  result->clear();
  ...
  try {
    ...
    m_readStmt->setSQL("SELECT * FROM " + getTable() + " where ( wei_conf_id= :1 or (tag=:2 AND version=:3 ) )");
...
void FEConfigWeightInfo::fetchLastData(FEConfigWeightInfo* result) noexcept(false) {
  this->checkConnection();
  result->clear();
  try {
    DateHandler dh(m_env, m_conn);

    m_readStmt->setSQL("SELECT * FROM " + getTable() + " where   wei_conf_id = ( select max( wei_conf_id) from " +
                       getTable() + " ) ");
    ResultSet* rset = m_readStmt->executeQuery();
...
int FEConfigWeightInfo::fetchID() noexcept(false) {
  ...
  try {
    ...
    stmt->setSQL("SELECT wei_conf_id FROM " + getTable() + " WHERE  tag=:1 and version=:2 ");
...
void FEConfigWeightInfo::setByID(int id) noexcept(false) {
  this->checkConnection();
