...
void RunTPGConfigDat::prepareWrite() noexcept(false) {
...
void RunTPGConfigDat::writeDB(const EcalLogicID* ecid, const RunTPGConfigDat* item, RunIOV* iov) noexcept(false) {
  this->checkConnection();
  this->checkPrepare();
  ...
  if (!logicID) {
    throw(std::runtime_error("RunTPGConfigDat::writeDB:  Bad EcalLogicID"));
  }
...
void RunTPGConfigDat::fetchData(map<EcalLogicID, RunTPGConfigDat>* fillMap, RunIOV* iov) noexcept(false) {
  this->checkConnection();
  fillMap->clear();
