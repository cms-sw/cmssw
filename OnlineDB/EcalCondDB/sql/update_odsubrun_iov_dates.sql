 ...
 * The run_iov is essentially another tag for a subrun-type IoV table
 */
CREATE OR REPLACE PROCEDURE update_odsubrun_iov_dates
( my_table IN VARCHAR2,
  start_col IN VARCHAR2,
  end_col IN VARCHAR2,
  new_start IN DATE,
  new_end IN OUT DATE,
  new_run_iov_id IN NUMBER ) IS
  ...
  BEGIN
    ...
    sql_str := 'SELECT min(' || start_col || ') FROM ' || my_table || 
               ' WHERE ' || start_col || ' > :s  AND run_iov_id = :r';
    EXECUTE IMMEDIATE sql_str INTO future_start USING new_start,  new_run_iov_id;
    ...
    sql_str := 'SELECT max(' || start_col || ') FROM ' || my_table || 
               ' WHERE ' || start_col || ' <= :s AND  run_iov_id = :r';
    EXECUTE IMMEDIATE sql_str INTO last_start USING new_start, new_run_iov_id;
    ...
    sql_str := 'SELECT ' || end_col || ' FROM ' || my_table || ' WHERE ' || start_col || ' = :s AND run_iov_id = :r';
    EXECUTE IMMEDIATE sql_str INTO last_end USING last_start, new_run_iov_id;
    ...
    ELSIF new_start < last_end THEN
       ...
       sql_str := 'UPDATE ' || my_table || ' SET ' || end_col || ' = :new_start' || 
                  ' WHERE ' || start_col || ' = :last_start AND ' || end_col || ' = :last_end AND run_iov_id = :r';
       EXECUTE IMMEDIATE sql_str USING new_start, last_start, last_end, new_run_iov_id;
    END IF;
  END;
/
show errors;
