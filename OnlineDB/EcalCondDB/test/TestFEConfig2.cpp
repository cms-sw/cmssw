...
public:
  ...
  void testReadOddWeights(int iconf_req) {
	...
	if(i<10)std::cout <<"here is the value for SM:"<< sm_num<<" tower:"<<tow_num<<" strip:"<<strip_num<<" group id:" << rd_weiv[i] << endl;
	i=i+1;
      }
    ...
    for (CIfem p = dataset_mode.begin(); p != dataset_mode.end(); p++) {
      ...
      // ...
      std::cout <<"here is the value for the weight mode: EnableEBOddFilter:"<< rd_modev[0] << " EnableEEOddFilter:"<<  rd_modev[1]<< std::endl;
      k=k+1;
    }
...
int main (int argc, char* argv[])
{
  ...
  try {
    ...
    if(rw_id==1 || rw_id==2 )app.testWriteOddWeights();
    if(rw_id==0 || rw_id==2) app.testReadOddWeights(cfg_id);
  } catch (exception &e) {
