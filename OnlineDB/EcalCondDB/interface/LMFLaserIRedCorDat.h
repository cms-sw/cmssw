#ifndef LMFLASERIREDCORDAT_H
#define LMFLASERIREDCORDAT_H
...
class LMFLaserIRedCorDat : public IDataItem {
 public:
  ...
  // User data methods
  inline std::string getTable() { return "LMF_LASER_IRED_COR_DAT"; }
 ...
 private:
  ...
  void writeDB(const EcalLogicID* ecid, const LMFLaserIRedCorDat* item, LMFRunIOV* iov)
    throw(std::runtime_error);

  void fetchData(std::map< EcalLogicID, LMFLaserIRedCorDat >* fillVec, LMFRunIOV* iov)
     throw(std::runtime_error);
