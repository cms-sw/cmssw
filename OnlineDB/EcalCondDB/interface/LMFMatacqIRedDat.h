#ifndef LMFMATACQIREDDAT_H
#define LMFMATACQIREDDAT_H
...
class LMFMatacqIRedDat : public IDataItem {
 public:
  ...
  // User data methods
  inline std::string getTable() { return "LMF_MATACQ_IRED_DAT"; }
 ...
 private:
  ...
  void writeDB(const EcalLogicID* ecid, const LMFMatacqIRedDat* item, LMFRunIOV* iov)
    throw(std::runtime_error);

  void fetchData(std::map< EcalLogicID, LMFMatacqIRedDat >* fillVec, LMFRunIOV* iov)
     throw(std::runtime_error);
