#ifndef LMFLASERPRIMDAT_H
#define LMFLASERPRIMDAT_H
...
class LMFLaserPrimDat : public IDataItem {
...
// GHM
  ...
  LMFLaserPrimDat();
  LMFLaserPrimDat( const LMFLaserPrimDat& );  // GHM
  ~LMFLaserPrimDat();
  ...
  inline std::string getTable()  // GHM
  {
    switch( _color )
      {
      case iBlue:  return "LMF_LASER_BLUE_PRIM_DAT"; 
      case iGreen: return "LMF_LASER_GREEN_PRIM_DAT"; 
      case iRed:   return "LMF_LASER_RED_PRIM_DAT"; 
      case iIRed:  return "LMF_LASER_IRED_PRIM_DAT"; 
      }
  // default
    return "LMF_LASER_BLUE_PRIM_DAT"; 
  }
 ...
 private:
  ...
  void writeDB(const EcalLogicID* ecid, const LMFLaserPrimDat* item, LMFRunIOV* iov)
    throw(std::runtime_error);

  void writeArrayDB(const std::map< EcalLogicID, LMFLaserPrimDat >* data, LMFRunIOV* iov)
  throw(std::runtime_error);

  void fetchData(std::map< EcalLogicID, LMFLaserPrimDat >* fillVec, LMFRunIOV* iov)
     throw(std::runtime_error);
