...
void prepareWrite() noexcept(false) override
{
  ...
  try {
    ...
    throw(std::runtime_error(std::string("ITimingDat::prepareWrite():  ")+e.getMessage()));
  }
}
  ...
  void writeDB(const EcalLogicID* ecid, const DATT* item, IOVT* iov) noexcept(false)
{
  ...
  try {
    ...
    throw(std::runtime_error(std::string("ITimingDat::writeDB():  ")+e.getMessage()));
  }
}
    ...
    void writeArrayDB(const std::map< EcalLogicID, DATT >* data, IOVT* iov) noexcept(false)
{
  ...
  try {
    ...
    throw(std::runtime_error(std::string("ITimingDat::writeArrayDB():  ")+e.getMessage()));
  }
}
  ...
  void fetchData(std::map< EcalLogicID, DATT >* fillMap, IOVT* iov) noexcept(false)
{
  ...
  try {
    ...
    throw(std::runtime_error(std::string("ITimingDat::fetchData():  ")+e.getMessage()));
  }
}
