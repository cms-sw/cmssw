#ifndef LMFMATACQREDDAT_H
#define LMFMATACQREDDAT_H
...
class LMFMatacqRedDat : public IDataItem {
 public:
  ...
  // User data methods
  inline std::string getTable() { return "LMF_MATACQ_RED_DAT"; }
 ...
 private:
  ...
  void writeDB(const EcalLogicID* ecid, const LMFMatacqRedDat* item, LMFRunIOV* iov)
    throw(std::runtime_error);

  void fetchData(std::map< EcalLogicID, LMFMatacqRedDat >* fillVec, LMFRunIOV* iov)
     throw(std::runtime_error);
