...
//cout<<"eta "<<eta<<endl;
  if(subd==1) {
   ...
   //    cout<<" 1 Phi saved "<< phi<<endl;
    ...
    if( var < 0.95 || var > 1.1) cout<<" ieta iphi "<<eta<<" "<<phi<<" "<<var<<endl;
     }
    if(dep == 1 && eta == -2) {h1etacoefmin2->Fill(var); if( var > 0.95 && var < 1.1) h1etacoefmin2l->Fill(var);
    if( var < 0.95 || var > 1.1) cout<<" ieta iphi "<<eta<<" "<<phi<<endl;}
    ...
    if( var < 0.95 || var > 1.1) cout<<" ieta iphi "<<eta<<" "<<phi<<endl; }       
    if(dep == 1 && eta == -11) {h1etacoefmin11->Fill(var);if( var > 0.95 && var < 1.1) h1etacoefmin11l->Fill(var); 
    if( var < 0.95 || var > 1.1) cout<<" ieta iphi "<<eta<<" "<<phi<<endl;}
...
TFile efile("coefficients_219_val_barrel_minus_8.9mln.root","recreate");
