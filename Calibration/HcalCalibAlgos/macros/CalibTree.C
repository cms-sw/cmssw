...
public:
  struct myEntry {
    myEntry(int k = 0, double f0 = 0, double f1 = 0, double f2 = 0) : kount(k), fact0(f0), fact1(f1), fact2(f2) {}
    int kount;
    double fact0, fact1, fact2;
  };

  struct energyCalor {
    energyCalor(double e1 = 0, double e2 = 0, double e3 = 0) : Etot(e1), Etot2(e2), ehcal(e3) {}
...
void Run(const char *inFileName,
  ...
  std::cout << "Create a chain for " << name << " from " << inFileName << std::endl;
  if (!fillChain(chain, inFileName)) {
    ...
    if (maxIter > maxIterMax)
      maxIter = maxIterMax;
    std::cout << "Tree " << name << " " << chain << " in directory " << dirName << " from file " << inFileName
    ...
    std::cout << "Output file: " << outFileName << " opened in recreate mode" << std::endl;
    fout->cd();
...
Double_t CalibTree::Loop(int loop,
  ...
  for (Long64_t jentry = 0; jentry < nentries; jentry++) {
    ...
    if (!selRun)
      continue;
    if ((t_nVtx < nvxlo_) || (t_nVtx > nvxhi_))
      continue;
    if (cSelect_ != nullptr) {
      if (exclude_) {
    ...
    if (goodTrack()) {
      ...
      if (en.ehcal > 0.001) {
        ...
        if (debug)
          std::cout << " Weights " << evWt << ":" << pufac << " Energy " << en.Etot2 << ":" << en.Etot << ":" << pmom
                    << ":" << t_eMipDR << ":" << t_eHcal << ":" << en.ehcal << " ratio " << ratio << std::endl;
        if (loop == 0) {
        ...
        if ((rmin >= 0 && ratio > rmin) && (rmax >= 0 && ratio < rmax) && l1c) {
          for (unsigned int idet = 0; idet < (*t_DetIds).size(); idet++) {
            ...
            if (okcell && selectPhi((*t_DetIds)[idet])) {
              ...
              double hitEn = 0.0;
              if (debug) {
                std::cout << "idet " << idet << " detid/hitenergy : " << std::hex << (*t_DetIds)[idet] << ":" << detid
  ...
  if (debug)
    std::cout << "# of Good Tracks " << ntkgood << " out of " << nentries << std::endl;
  if (loop == 0) {
    h_pbyE->Write("h_pbyE");
  ...
  for (std::map<unsigned int, TH1D *>::const_iterator itr = histos_.begin(); itr != histos_.end(); ++itr) {
    ...
    if (debug) {
      std::cout << "DETID :" << subdet << "  IETA :" << ieta << " HIST ENTRIES :" << (itr->second)->GetEntries()
                << std::endl;
    }
  }
  ...
  for (std::map<unsigned int, TH1D *>::const_iterator itr = histos_.begin(); itr != histos_.end(); ++itr, ++kount) {
    ...
    if (debug) {
      int subdet, depth, zside, ieta, iphi;
      unpackDetId(itr->first, subdet, zside, ieta, iphi, depth);
      std::cout << "DetId[" << kount << "] " << subdet << ":" << zside * ieta << ":" << depth << " Factor " << factor
  ...
  if (debug)
    std::cout << "SumW with " << SumW.size() << " entries\n";
  ...
  for (; SumWItr != SumW.end(); SumWItr++) {
    ...
    if (debug) {
      std::cout << "Detid|kount|SumWi|SumFac|myId : " << subdet << ":" << zside * ieta << ":" << depth << " | "
                << (SumWItr->second).kount << " | " << (SumWItr->second).fact0 << "|" << (SumWItr->second).fact1 << "|"
    ...
    if (debug) {
      std::cout << "Factor " << factor << " " << dfac1 << " " << dfac << std::endl;
    }
    if (inverse)
      factor = 2. - factor;
    if (useMean_) {
  ...
  if (higheta_ > 0)
    highEtaFactors(ietaMax, debug);

  std::cout << kountus << " detids out of " << kount << " have tracks > " << nMin << std::endl;
  ...
  for (; CprevItr != Cprev.end(); CprevItr++, indx++) {
    ...
    int subdet, depth, zside, ieta, iphi;
    unpackDetId(detid, subdet, zside, ieta, iphi, depth);
    std::cout << "DetId[" << indx << "] " << std::hex << detid << std::dec << "(" << ieta * zside << "," << depth
              << ") (nTrks:" << nTrks[detid] << ") : " << CprevItr->second.first << " +- " << CprevItr->second.second
              << std::endl;
  }
  double mean = (kountus > 0) ? (sumfactor / kountus) : 0;
  std::cout << "Mean deviation " << mean << " from 1 for " << kountus << " DetIds" << std::endl;
...
void CalibTree::writeCorrFactor(const char *corrFileName, int ietaMax) {
  ...
  if (!myfile.is_open()) {
    ...
    myfile << "#" << std::setprecision(4) << std::setw(10) << "detId" << std::setw(10) << "ieta" << std::setw(10)
...
std::pair<double, double> CalibTree::fitMean(TH1D *hist, int mode) {
  ...
  if (hist != 0) {
    ...
    if (mode == 1) {
      ...
      if (LowEdge < hist->GetBinLowEdge(1))
...
void CalibTree::makeplots(
  ...
  for (int ieta = -ietaMax; ieta <= ietaMax; ++ieta) {
    ...
    if (ieta == 0)
      sprintf(title, "Ratio at start");
    else
      sprintf(title, "Ratio at start for i#eta=%d", ieta);
    TH1D *h1 = new TH1D(name, title, 50, rmin, rmax);
    h1->Sumw2();
    sprintf(name, "end%d", ieta);
    if (ieta == 0)
      sprintf(title, "Ratio at the end");
    else
      sprintf(title, "Ratio at the end for i#eta=%d", ieta);
  ...
  for (int ieta = -ietaMax; ieta <= ietaMax; ++ieta) {
    ...
    if (debug) {
      std::cout << ieta << " " << h1->GetName() << " " << mean1 << " +- " << err1 << " and " << fit1.first << " +- "
                << fit1.second << std::endl;
    }
    if (ieta != 0) {
      hbef1->SetBinContent(bin, mean1);
      hbef1->SetBinError(bin, err1);
      hbef2->SetBinContent(bin, fit1.first);
      hbef2->SetBinError(bin, fit1.second);
    }
    h1->Write();
    ...
    if (debug) {
      std::cout << ieta << " " << h2->GetName() << " " << mean2 << " +- " << err2 << " and " << fit2.first << " +- "
                << fit2.second << std::endl;
    }
    if (ieta != 0) {
      haft1->SetBinContent(bin, mean2);
      haft1->SetBinError(bin, err2);
      haft2->SetBinContent(bin, fit2.first);
      haft2->SetBinError(bin, fit2.second);
    }
    h2->Write();
  }
  fitPol0(hbef1, debug);
...
void CalibTree::fitPol0(TH1D *hist, bool debug) {
  ...
  if (debug) {
    std::cout << "Fit to Pol0 to " << hist->GetTitle() << ": " << Fit->Value(0) << " +- " << Fit->FitResult::Error(0)
              << std::endl;
  }
  hist->Write();
}
...
void CalibTree::highEtaFactors(int ietaMax, bool debug) {
  ...
  if (debug) {
    std::cout << "Correction factor for (" << ietaMax << ",1) = " << cfacp.first << ":" << cfacp.second << " and ("
              << -ietaMax << ",1) = " << cfacn.first << ":" << cfacn.second << std::endl;
  }
  for (itr = Cprev.begin(); itr != Cprev.end(); ++itr) {
    unsigned int detid = itr->first;
    int subdet, depth, zside, ieta, iphi;
    ...
    if (ieta > ietaMax) {
      ...
      if (debug) {
        std::cout << "Set correction factor for (" << zside * ieta << "," << depth << ") = " << Cprev[detid].first
