...
TCanvas* example_plot(int iPeriod, int iPos, int ieta, int phi, int idep, int inv, TFile *f, std::string sample1, bool writeExtraText, bool ifHPD, std::string outfile) {
  int W = 800;
  int H = 600;
  int H_ref = 600; 
  int W_ref = 800;
...

  {
    ...
    int rbo = 0;
    std::cout << "problem:" << "ieta" << ":" << ieta << std::endl;
    std::cout << "problem:" << "idep" << ":" << idep << std::endl;
    std::cout << "problem:" << "inv" << ":" << inv << std::endl;
    ...
    // if (!h1) continue;
    if (h1->GetBinContent(1) > h1->GetBinContent(2)) h1->SetBinContent(1, 0) ;
    ...
    log1 << sample1.c_str() << "\tieta " << ieta << "\tiphi "<< phi << "\t\tDEP "<< (idep + 1) << "\tNvtx " << inv << "\t" << peak << "\t" << lError << "\t" << chiSquare << std::endl;
    log1.close();
  }
...
void doFit(std::string infile, std::string text, int iPeriod = 0, int etaMin = 2, int etamax = 3, int dmin = 1, int dmax = 1, int vxmin = 2, int vxmax = 2, int phimin = 1, int phimax = 72, int phiInc = 1, bool ifHPD = true, bool writeExtraText = false) {
