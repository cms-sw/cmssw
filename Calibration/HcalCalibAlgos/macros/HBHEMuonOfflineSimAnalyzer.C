...
HBHEMuonOfflineSimAnalyzer::HBHEMuonOfflineSimAnalyzer(const char* infile,
	...
	if (select) {
	  ...
	  for (int dep=0; dep<nDepth; ++dep) {
		...
		std::cout << "Hello in 4" << std::endl;
	      ...
	      if (debug_) 
		std::cout << "Energy in depth 6 " << hcal_edepth6->size() << ":" << hcal_edepthHot6->size() << std::endl;
	      ...
	      if (debug_)
		std::cout << "Energy in depth 7 " << hcal_edepth7->size() << ":" << hcal_edepthHot7->size() << std::endl;
	    ...
	    if (debug_) {
	      std::cout << "ieta " << eta << "depth " << dep << "indxEta[eta][dep]:" << indxEta[eta][dep][PHI] << std::endl;
...
void HBHEMuonOfflineSimAnalyzer::BookHistograms(const char* fname) {
  ...
  for (int i=0; i<3; ++i) {
	...
	for (int PHI=0; PHI<nPhi; ++PHI) {
	  ...
	  std::cout<< "eta:" << eta << " depth:" << depth << " PHI:" << PHI <<":"<< PHI0 <<" ih:" << ih << std::endl; 
...
void HBHEMuonOfflineSimAnalyzer::etaPhiEcal(unsigned int detId, int& type, 
