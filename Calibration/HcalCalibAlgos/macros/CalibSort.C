...
void combine(const std::string fname1, const std::string fname2,
	     const std::string fname, const std::string outfile="",
	     int debug=0) {
  ...
  if ((!infile1.is_open()) || (!infile2.is_open())) {
    ...
    while (1) {
      ...
      ++kount;
      if (debug > 1) {
	std::cout << kount << " Enrty No. " << entry1 << ":" << entry2 
		  << " eta " <<  ieta1 << ":" << ieta2 << " wt " << wt1 
		  << ":" << wt2 << std::endl;
      }
      if (entry1 == entry2) {
	int ieta = fabs(ieta1);
	double wt = (ieta >= 16) ? wt1 : wt2;
	file << std::setw(8) << entry1 << " " << std::setw(12) 
	...
	std::cout << kount << " Entry " << entry1 << ":" << entry2 << " eta "
		  <<  ieta1 << ":" << ieta2 << " wt " << wt1 << ":" << wt2
		  << " mismatch" << std::endl;
      }
    }
    infile1.close();
...
void plotCombine(const char* infile, int flag=-1, bool drawStatBox=true,
		 bool save=true) {
  ...
  if (flag < 0) {
    ...
    sprintf(title, "Good tracks of %s with %s isolation", mom[flag1].c_str(),
	    iso[flag2].c_str());
  }
  ...
  for (int k=0; k<4; ++k) {
    ...
    if (flag >= 0) sprintf (nameh, "%s%d%d", name[k].c_str(), flag1, flag2);
    else           sprintf (nameh, "%s", name[k].c_str());
...
public :
  ...
  CalibPlotCombine(const char* fname, const char* corrFileName,
...
CalibPlotCombine::CalibPlotCombine(const char* fname, 
				   const char* corrFileName,
				   const std::string dirnm, 
				   int flag) : cFactor_(nullptr),
					       flag_(flag), ifDepth_(3) {

  selectP_     = (((flag_/10)%10) > 0);
  tightSelect_ = ((flag_%10) > 0);
  flag_        = 0;
  if (selectP_) flag_ += 10;
  if (tightSelect_) ++flag_;
...
void CalibPlotCombine::Init(TChain *tree) {
  ...
  sprintf (title, "Correction Factor (%s) for tracks with %s and %s isolation",
	   rng[0].c_str(), mom[flag1].c_str(), iso[flag2].c_str());
  hist0_ = new TH1D(name, title, 100, 0.0, 10.0);
  sprintf (name, "W1%d%d", flag1, flag2);
  sprintf (title, "Correction Factor (%s) for tracks with %s and %s isolation",
	   rng[1].c_str(), mom[flag1].c_str(), iso[flag2].c_str());
  hist1_ = new TH1D(name, title, 100, 0.0, 10.0);
  sprintf (name, "W2%d%d", flag1, flag2);
  sprintf (title, "Correction Factor (%s) for tracks with %s and %s isolation",
	   rng[2].c_str(), mom[flag1].c_str(), iso[flag2].c_str());
  hist2_ = new TH1D(name, title, 100, 0.0, 10.0);
  sprintf (name, "P%d%d", flag1, flag2);
  sprintf (title, "Correction Factor vs i#eta for tracks with %s and %s isolation",
	   mom[flag1].c_str(), iso[flag2].c_str());
...
void CalibPlotCombine::Loop() {
  ...
  for (Long64_t jentry=0; jentry<nentries; jentry++) {
    ...
    if (ientry < 0) break;
    nb = fChain->GetEntry(jentry);   nbytes += nb;
    if (jentry%1000000 == 0) std::cout << "Entry " << jentry << " Run " << t_Run
  ...
  std::cout << "Minimum and maximum correction factors " << wmine << ":" 
	    << wmaxb << " (Barrel) " << wmine << ":" << wmaxe << " (Endcap)"
	    << std::endl;
}

bool CalibPlotCombine::goodTrack(double eHcal, double cut) {
  return ((t_qltyFlag) && t_selectTk && (t_hmaxNearP < cut) && 
	  (t_eMipDR < 1.0) && (eHcal > 0.001));
}

void CalibPlotCombine::savePlot(const std::string& theName, bool append) {
  TFile* theFile = (append ? (new TFile(theName.c_str(), "UPDATE")) :
		    (new TFile(theName.c_str(), "RECREATE")));
  theFile->cd();
  TH1D* hist;
  hist = (TH1D*)(hist0_->Clone()); hist->Write();
  hist = (TH1D*)(hist1_->Clone()); hist->Write();
  hist = (TH1D*)(hist2_->Clone()); hist->Write();
  TProfile* prof = (TProfile*)(prof_->Clone()); prof->Write();
