...
Int_t sample=0;
 ...
 TFile* tf = new TFile("./ValidFile_XX.root","OPEN");
  ...
  //TCut tr_cut = "eTrack>80 && eTrack<120";  label = new TText(0.97,0.1, "100 GeV"); sample=100;
  //TCut tr_cut = "eTrack>250 && eTrack<350";  label = new TText(0.97,0.1, "300 GeV"); sample=300;
...
label -> Draw();
  ...
  ftree -> Draw("numHits>>nh1", selection&&"abs(iEta)<14");
nh1 -> SetBins(26, 0,26);
  ftree -> Draw("numHits>>nh2", selection&&"abs(iEta)>17", "same");
  nh1 -> SetTitle("Number of Hits");
...
hh3 -> SetLineColor(kRed+1);
  hh2 -> SetTitle("eTrack and eHcalClast");
  hh2 -> Draw("");
...
label -> Draw();
  ...
  ftree -> Draw("iPhi>>hh1", selection&&"abs(iEta)<20");
  hh1 -> SetLineColor(kRed+1);
  ftree -> Draw("iPhi>>hh2", selection&&"abs(iEta)>20", "same");
...
label -> Draw();
 ...
 r2 -> Draw();  
  ...
  hh2 -> SetTitle("Response after calibration");
  hh2 -> Draw("");
...
label -> Draw();
  ...
  ftree -> Draw("eCentHitAfter:iPhi>>hh1", selection&&"abs(iEta)>20", "prof");
  hh1 -> SetTitle("eCentralHit vs iPhi for |iEta|>20");
  hh1 -> SetYTitle("energy");
  hh1 -> SetXTitle("iPhi");
  hh1 -> SetMinimum(20);
  hh1 -> SetMaximum(40);
