...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   FILE *Out1 = fopen("var_minbias_8.9mln.txt", "w+");
   FILE *Out2 = fopen("var_noise_8.9mln.txt", "w+");
...
// Calculation of dispersion ===============================
	 ...
	 minmean_MB[abs(ieta)][iphi][depth][mysubd]=minmean_MB[abs(ieta)][iphi][depth][mysubd] + (float)disp_MB;
	 minmean_NS[abs(ieta)][iphi][depth][mysubd]=minmean_NS[abs(ieta)][iphi][depth][mysubd] + (float)disp_NS;
	 ...
	 minnevetaphi[abs(ieta)][iphi][depth][mysubd]++;
         ...
         minerr_MB[abs(ieta)][iphi][depth][mysubd] = minerr_MB[abs(ieta)][iphi][depth][mysubd] + disp_MB*disp_MB;
         minerr_NS[abs(ieta)][iphi][depth][mysubd] = minerr_NS[abs(ieta)][iphi][depth][mysubd] + disp_NS*disp_NS;
...
// Calculation of dispersion ===============================
	 ...
	 plmean_MB[abs(ieta)][iphi][depth][mysubd]=plmean_MB[abs(ieta)][iphi][depth][mysubd] + (float)disp_MB;
	 plmean_NS[abs(ieta)][iphi][depth][mysubd]=plmean_NS[abs(ieta)][iphi][depth][mysubd] + (float)disp_NS;
	 ...
	 plnevetaphi[abs(ieta)][iphi][depth][mysubd]++;
         ...
         plerr_MB[abs(ieta)][iphi][depth][mysubd] = plerr_MB[abs(ieta)][iphi][depth][mysubd] + disp_MB*disp_MB;
         plerr_NS[abs(ieta)][iphi][depth][mysubd] = plerr_NS[abs(ieta)][iphi][depth][mysubd] + disp_NS*disp_NS;
...
//       fprintf(Out4,"%d %d %d %d %.8f\n",mysubdetmin0[ietak][iphik][idep][isub],iietak,iphik,idep,minmeang_mean_Diff_av);
  ...
  TFile f("signal_noise_test_8.9mln.root","recreate");
  h1noise->Write();
  hlnoise->Write();
  hlnoise1->Write();
  hlnoise2->Write();
