...
void HcalIsoTrackStudy::beginRun(edm::Run const& iRun, 
...
std::array<int,3> HcalIsoTrackStudy::fillTree(std::vector< math::XYZTLorentzVector>& vecL1,
  ...
  for (trkDetItr = trkCaloDirections.begin(),nTracks=0; 
	...
	edm::LogVerbatim("HcalIsoTrack") << t_DetIdEC->size() << ":"
					 << t_DetIdHC->size() << " hits from "
					 << "ECAL and HCAL";
	for (unsigned int ll=0; ll<t_DetIdEC->size(); ++ll)
	  edm::LogVerbatim("HcalIsoTrack") << "EC[" << ll << "] ID " << std::hex
					   << t_DetIdEC->at(ll) << std::dec
					   << " D " << t_HitDistEC->at(ll)
					   << " E " << t_HitEnergyEC->at(ll); 
	for (unsigned int ll=0; ll<t_DetIdHC->size(); ++ll)
	  edm::LogVerbatim("HcalIsoTrack") << "HC[" << ll << "] ID " << std::hex
					   << t_DetIdHC->at(ll) << std::dec
					   << " D " << t_HitDistHC->at(ll)
					   << " E " << t_HitEnergyHC->at(ll); 
#endif
	bool accept(false);
	if (t_p>pTrackMin_) {
	  if (t_p<pTrackLow_) {
