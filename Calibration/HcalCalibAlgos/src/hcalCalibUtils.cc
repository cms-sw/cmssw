...
void filterCells3x3(vector<TCell>& selectCells, Int_t iEtaMaxE, UInt_t iPhiMaxE) {
  ...
  for (auto& selectCell : selectCells) {
    ...
    if (abs(dEta) <= 1 || (iEtaMaxE * HcalDetId(selectCell.id()).ieta() == -1))
      passDEta = true;
...
void sumSmallDepths(vector<TCell>& selectCells) {
  ...
  for (auto& manipulatedCell : manipulatedCells) {
    UInt_t dummyId = HcalDetId(HcalDetId(manipulatedCell.id()).subdet(),
                               HcalDetId(manipulatedCell.id()).ieta(),
                               HcalDetId(manipulatedCell.id()).iphi(),
                               1);
