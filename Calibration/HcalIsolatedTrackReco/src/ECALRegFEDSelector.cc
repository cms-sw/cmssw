...
ECALRegFEDSelector::ECALRegFEDSelector(const edm::ParameterSet &iConfig) {
  tok_seed_ = consumes<trigger::TriggerFilterObjectWithRefs>(
      iConfig.getParameter<edm::InputTag>("regSeedLabel"));
...
void ECALRegFEDSelector::produce(edm::Event &iEvent,
  ...
  std::vector<edm::Ref<reco::IsolatedPixelTrackCandidateCollection>>
      isoPixTrackRefs;
  ...
  for (uint32_t p = 0; p < isoPixTrackRefs.size(); p++) {
    ...
    RectangularEtaPhiRegion ecEtaPhi(etaObj_ - delta_, etaObj_ + delta_,
                                     phiObj_ - delta_, phiObj_ + delta_);
    ...
    for (int j = 0; j < FEDNumbering::MAXFEDID; j++) {
      ...
      if (j >= FEDNumbering::MINPreShowerFEDID &&
          j <= FEDNumbering::MAXPreShowerFEDID) {
        fedSaved[j] = true;
        rightFED = true;
      }
      if (!rightFED)
      ...
      if (size > 0) {
        ...
        if (fedDataProd.size() != 0) {
          //		  std::cout << " More than one FEDRawDataCollection with
          // data in FED "; 		  std::cout << j << " Skipping the
          // 2nd\n";
          continue;
        }
