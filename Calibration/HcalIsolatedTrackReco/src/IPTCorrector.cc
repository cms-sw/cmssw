...
IPTCorrector::IPTCorrector(const edm::ParameterSet &config)
    : tok_cor_(consumes<reco::TrackCollection>(
          config.getParameter<edm::InputTag>("corTracksLabel"))),
      tok_uncor_(consumes<trigger::TriggerFilterObjectWithRefs>(
          config.getParameter<edm::InputTag>("filterLabel"))),
      assocCone_(config.getParameter<double>("associationCone")) {
  // register the product
  produces<reco::IsolatedPixelTrackCandidateCollection>();
}

void IPTCorrector::fillDescriptions(
    edm::ConfigurationDescriptions &descriptions) {
  edm::ParameterSetDescription desc;
  desc.add<edm::InputTag>("corTracksLabel",
                          edm::InputTag("hltIter0PFlowCtfWithMaterialTracks"));
...
void IPTCorrector::produce(edm::StreamID, edm::Event &theEvent,
  ...
  for (int p = 0; p < nCand; p++) {
    ...
    for (reco::TrackCollection::const_iterator cit = corTracks->begin();
         cit != corTracks->end(); cit++) {
    ...
    if (ntrk > 0) {
      reco::IsolatedPixelTrackCandidate newCandidate(
          reco::TrackRef(corTracks, citSel - corTracks->begin()),
          isoPixTrackRefs[p]->l1tau(), isoPixTrackRefs[p]->maxPtPxl(),
