...
IsolatedPixelTrackCandidateL1TProducer::IsolatedPixelTrackCandidateL1TProducer(
    ...
    : tok_hlt_(consumes<trigger::TriggerFilterObjectWithRefs>(
      ...
      tok_l1_(consumes<l1t::TauBxCollection>(
...
void IsolatedPixelTrackCandidateL1TProducer::fillDescriptions(
  ...
  desc.add<edm::InputTag>("L1GTSeedLabel",
                          edm::InputTag("hltL1sV0SingleJet60"));
...
void IsolatedPixelTrackCandidateL1TProducer::beginRun(
  ...
  edm::LogVerbatim("IsoTrack")
      << "rEB " << rEB_ << " zEE " << zEE_ << " B " << bfVal_ << std::endl;
}

void IsolatedPixelTrackCandidateL1TProducer::produce(
  ...
  edm::LogVerbatim("IsoTrack")
      << "Sizes " << l1tauobjref.size() << ":" << l1jetobjref.size() << " Trig "
      << ptTriggered << ":" << etaTriggered << ":" << phiTriggered << std::endl;
  ...
  for (unsigned iS = 0; iS < pixelTrackRefs.size(); iS++) {
    ...
    double R =
        reco::deltaR(etaTriggered, phiTriggered, pixelTrackRefs[iS]->eta(),
                     pixelTrackRefs[iS]->phi());
    edm::LogVerbatim("IsoTrack")
        << "Distance to L1 " << R << ":" << tauUnbiasCone_ << " Result "
    ...
    for (l1t::TauBxCollection::const_iterator tj = l1eTauJets->begin();
...
IsolatedPixelTrackCandidateL1TProducer::GetEtaPhiAtEcal(double etaIP,
                                                        double phiIP, double pT,
                                                        int charge,
                                                        double vtxZ) {
