...
public:
  explicit HITRegionalPixelSeedGenerator(const edm::ParameterSet &conf_,
    ...
    edm::LogInfo("HITRegionalPixelSeedGenerator")
    ...
    if (usetracks_)
      token_trks = iC.consumes<reco::TrackCollection>(
          regionPSet.getParameter<edm::InputTag>("trackSrc"));
    if (usetracks_ || useIsoTracks_ || fixedReg_ || usejets_)
      token_vertex = iC.consumes<reco::VertexCollection>(
          regionPSet.getParameter<edm::InputTag>("vertexSrc"));
    if (useIsoTracks_)
      token_isoTrack = iC.consumes<trigger::TriggerFilterObjectWithRefs>(
          regionPSet.getParameter<edm::InputTag>("isoTrackSrc"));
    if (usejets_)
      token_l1jet = iC.consumes<l1extra::L1JetParticleCollection>(
          regionPSet.getParameter<edm::InputTag>("l1tjetSrc"));
  }
  ...
  regions(const edm::Event &e, const edm::EventSetup &es) const override {
    ...
    if (usetracks_) {
      ...
      for (; itr != tracks->end(); itr++) {
        ...
        result.push_back(std::make_unique<RectangularEtaPhiTrackingRegion>(
            globalVector, GlobalPoint(0, 0, originz), ptmin, originradius,
            deltaZVertex, deltaTrackEta, deltaTrackPhi));
      }
    }
    ...
    if (usejets_) {
      ...
      for (l1extra::L1JetParticleCollection::const_iterator iJet =
               jets->begin();
           iJet != jets->end(); iJet++) {
        ...
        result.push_back(std::make_unique<RectangularEtaPhiTrackingRegion>(
            jetVector, vertex, ptmin, originradius, deltaZVertex, deltaL1JetEta,
            deltaL1JetPhi));
      }
    }
    if (fixedReg_) {
      GlobalVector fixedVector(
          cos(phiCenter_) * sin(2 * atan(exp(-etaCenter_))),
      ...
      result.push_back(std::make_unique<RectangularEtaPhiTrackingRegion>(
          fixedVector, vertex, ptmin, originradius, deltaZVertex, deltaL1JetEta,
          deltaL1JetPhi));
    }
