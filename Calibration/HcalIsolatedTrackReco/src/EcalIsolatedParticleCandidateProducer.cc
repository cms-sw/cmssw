...
/**\class EcalIsolatedParticleCandidateProducer
 ...
 Calibration/EcalIsolatedParticleCandidateProducer/src/EcalIsolatedParticleCandidateProducer.cc
...
EcalIsolatedParticleCandidateProducer::EcalIsolatedParticleCandidateProducer(
  ...
  tok_l1tau_ = consumes<l1extra::L1JetParticleCollection>(
      conf.getParameter<edm::InputTag>("L1eTauJetsSource"));
  tok_hlt_ = consumes<trigger::TriggerFilterObjectWithRefs>(
      conf.getParameter<edm::InputTag>("L1GTSeedLabel"));
  tok_EB_ = consumes<EcalRecHitCollection>(
...
void EcalIsolatedParticleCandidateProducer::produce(
  ...
  auto iptcCollection =
      std::make_unique<reco::IsolatedPixelTrackCandidateCollection>();

  //  std::cout<<"loop over l1taus"<<std::endl;

  for (l1extra::L1JetParticleCollection::const_iterator tit = l1Taus->begin();
       tit != l1Taus->end(); tit++) {
    ...
    for (EcalRecHitCollection::const_iterator eItr = ecalEB->begin();
         eItr != ecalEB->end(); eItr++) {
      double phiD, R;
      ...
      if (R < OutConeSize_ && R > InConeSize_ &&
          eItr->energy() > hitCountEthr_) {
        nhitOut++;
      }
    ...
    for (EcalRecHitCollection::const_iterator eItr = ecalEE->begin();
         eItr != ecalEE->end(); eItr++) {
      double phiD, R;
      const GlobalPoint &pos = geo->getPosition(eItr->detid());
      double phihit = pos.phi();
      double etahit = pos.eta();
    ...
    reco::IsolatedPixelTrackCandidate newca(
        l1extra::L1JetParticleRef(l1Taus, tit - l1Taus->begin()), InEnergy,
        OutEnergy, nhitIn, nhitOut);
