...
void SiStripRegFEDSelector::produce(edm::Event &iEvent,
  ...
  for (uint32_t p = 0; p < isoPixTrackRefs.size(); p++) {
    ...
    for (uint32_t i = 0; i < ccab.size(); i++) {
      ...
      if (R - sqrt(pow(regDim.first / 2, 2) + pow(regDim.second / 2, 2)) >
          delta_)
        continue;
      ...
      for (uint32_t idet = 0; idet < SiStripRegionCabling::ALLSUBDETS; idet++) {
        ...
        const SiStripRegionCabling::WedgeCabling regSubdetLayers =
            regSubdets[idet];
        for (uint32_t ilayer = 0; ilayer < SiStripRegionCabling::ALLLAYERS;
             ilayer++) {
          // get map of vectors of feds withing the layer of subdet of region
          const SiStripRegionCabling::ElementCabling fedVectorMap =
              regSubdetLayers[ilayer];
          SiStripRegionCabling::ElementCabling::const_iterator it =
              fedVectorMap.begin();
