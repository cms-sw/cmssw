...
IsolatedPixelTrackCandidateProducer::IsolatedPixelTrackCandidateProducer(
    ...
    : tok_hlt_(consumes<trigger::TriggerFilterObjectWithRefs>(
          config.getParameter<edm::InputTag>("L1GTSeedLabel"))),
      tok_l1_(consumes<l1extra::L1JetParticleCollection>(
          config.getParameter<edm::InputTag>("L1eTauJetsSource"))),
      tok_vert_(consumes<reco::VertexCollection>(
          config.getParameter<edm::InputTag>("VertexLabel"))),
      toks_pix_(edm::vector_transform(
          config.getParameter<std::vector<edm::InputTag>>("PixelTracksSources"),
          [this](edm::InputTag const &tag) {
            return consumes<reco::TrackCollection>(tag);
          })),
      ...
      pixelIsolationConeSizeAtEC_(
          config.getParameter<double>("PixelIsolationConeSizeAtEC")),
      ...
      maxPForIsolationValue_(
          config.getParameter<double>("maxPTrackForIsolation")),
      ebEtaBoundary_(config.getParameter<double>("EBEtaBoundary")), rEB_(-1),
      zEE_(-1), bfVal_(0) {
...
void IsolatedPixelTrackCandidateProducer::fillDescriptions(
    edm::ConfigurationDescriptions &descriptions) {
  edm::ParameterSetDescription desc;
  std::vector<edm::InputTag> tracksrc = {edm::InputTag("hltPixelTracks")};
  desc.add<edm::InputTag>("L1eTauJetsSource",
                          edm::InputTag("hltCaloStage2Digis", "Tau"));
  ...
  desc.add<edm::InputTag>("VertexLabel",
                          edm::InputTag("hltTrimmedPixelVertices"));
...
void IsolatedPixelTrackCandidateProducer::beginRun(
    const edm::Run &run, const edm::EventSetup &theEventSetup) {

  edm::ESHandle<CaloGeometry> pG;
  theEventSetup.get<CaloGeometryRecord>().get(pG);

  const double rad(dynamic_cast<const EcalBarrelGeometry *>(
                       pG->getSubdetectorGeometry(DetId::Ecal, EcalBarrel))
                       ->avgRadiusXYFrontFaceCenter());
  const double zz(dynamic_cast<const EcalEndcapGeometry *>(
  ...
  const VolumeBasedMagneticField *vbfCPtr =
      dynamic_cast<const VolumeBasedMagneticField *>(&(*vbfField));
  GlobalVector BField = vbfCPtr->inTesla(GlobalPoint(0, 0, 0));
  bfVal_ = BField.mag();
}

void IsolatedPixelTrackCandidateProducer::produce(
    edm::Event &theEvent, const edm::EventSetup &theEventSetup) {
...
#ifdef DebugLog
  edm::LogInfo("HcalIsoTrack")
      << "IsolatedPixelTrakCandidate: with" << toks_pix_.size()
  ...
  for (unsigned int iPix = 0; iPix < toks_pix_.size(); iPix++) {
    ...
    for (reco::TrackCollection::const_iterator pit = iPixCol->begin();
         pit != iPixCol->end(); pit++) {
  ...
  for (unsigned iS = 0; iS < pixelTrackRefs.size(); iS++) {
    ...
    double minDZ = 1000;
    bool found(false);
    for (reco::VertexCollection::const_iterator vit = pVert->begin();
         vit != pVert->end(); vit++) {
    ...
    for (l1extra::L1JetParticleCollection::const_iterator tj =
             l1eTauJets->begin();
    ...
    if (found)
      seedCooAtEC = GetEtaPhiAtEcal(
          pixelTrackRefs[iS]->eta(), pixelTrackRefs[iS]->phi(),
          pixelTrackRefs[iS]->pt(), pixelTrackRefs[iS]->charge(), vitSel->z());
    // in case vertex is not found:
    else
      seedCooAtEC = GetEtaPhiAtEcal(
          pixelTrackRefs[iS]->eta(), pixelTrackRefs[iS]->phi(),
          pixelTrackRefs[iS]->pt(), pixelTrackRefs[iS]->charge(), 0);
    seedAtEC seed(iS, (tmatch || vtxMatch), seedCooAtEC.first,
                  seedCooAtEC.second);
  ...
  for (unsigned int i = 0; i < VecSeedsatEC.size(); i++) {
    ...
    for (unsigned int j = 0; j < VecSeedsatEC.size(); j++) {
      ...
      // define preliminary cone around seed track impact point from which
      // tracks will be extrapolated:
      ...
      // cut ot dXY:
      if (found && std::abs(pixelTrackRefs[iSurr]->dxy(vitSel2->position())) >
                       vtxCutIsol_)
        continue;
      // calculate distance at ECAL surface and update isolation:
      if (getDistInCM(VecSeedsatEC[i].eta, VecSeedsatEC[i].phi,
                      VecSeedsatEC[j].eta,
                      VecSeedsatEC[j].phi) < pixelIsolationConeSizeAtEC_) {
    ...
    if (maxP < maxPForIsolationValue_) {
      reco::IsolatedPixelTrackCandidate newCandidate(
          pixelTrackRefs[iSeed],
          l1extra::L1JetParticleRef(l1eTauJets, selj - l1eTauJets->begin()),
          maxP, sumP);
...
double IsolatedPixelTrackCandidateProducer::getDistInCM(double eta1,
  ...
  double angle = acos((sin(theta1) * sin(theta2) *
                           (sin(phi1) * sin(phi2) + cos(phi1) * cos(phi2)) +
...
std::pair<double, double> IsolatedPixelTrackCandidateProducer::GetEtaPhiAtEcal(
  ...
  double ecDist = zEE_; // distance to ECAL andcap from IP (cm), 317 - ecal (not
                        // preshower), preshower -300
  ...
  if (std::abs(etaIP) < ebEtaBoundary_) {
    if ((0.5 * ecRad / Rcurv) > 1) {
      ...
      if (etaIP > 0)
        ...
        zNew = -z * (Rcurv * alpha1) / ecRad + vtxZ; // new z-coordinate of
                                                     // track
