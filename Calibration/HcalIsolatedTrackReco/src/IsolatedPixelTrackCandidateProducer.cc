...
void IsolatedPixelTrackCandidateProducer::beginRun(const edm::Run &run, const edm::EventSetup &theEventSetup) {
  ...
  const double rad (((EcalBarrelGeometry*)(pG->getSubdetectorGeometry(DetId::Ecal,EcalBarrel)))->avgRadiusXYFrontFaceCenter()) ;
  const double zz  (((EcalEndcapGeometry*)( pG->getSubdetectorGeometry(DetId::Ecal,EcalEndcap)))->avgAbsZFrontFaceCenter()) ;
