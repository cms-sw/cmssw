...
/**\class IsolatedEcalPixelTrackCandidateProducer
 ...
 Calibration/HcalIsolatedTrackReco/src/IsolatedEcalPixelTrackCandidateProducer.cc
...
// $Id: IsolatedEcalPixelTrackCandidateProducer.cc,v 1.0 2014/02/11 22:25:52
// wmtan Exp $
//
//
...
IsolatedEcalPixelTrackCandidateProducer::
    IsolatedEcalPixelTrackCandidateProducer(const edm::ParameterSet &conf)
    : tok_ee(consumes<EcalRecHitCollection>(
          conf.getParameter<edm::InputTag>("EERecHitSource"))),
      tok_eb(consumes<EcalRecHitCollection>(
          conf.getParameter<edm::InputTag>("EBRecHitSource"))),
      tok_trigcand(consumes<trigger::TriggerFilterObjectWithRefs>(
          conf.getParameter<edm::InputTag>("filterLabel"))),
...
void IsolatedEcalPixelTrackCandidateProducer::fillDescriptions(
    edm::ConfigurationDescriptions &descriptions) {
  edm::ParameterSetDescription desc;
  desc.add<edm::InputTag>("filterLabel",
                          edm::InputTag("hltIsolPixelTrackL2Filter"));
  desc.add<edm::InputTag>("EBRecHitSource",
                          edm::InputTag("hltEcalRecHit", "EcalRecHitsEB"));
  desc.add<edm::InputTag>("EERecHitSource",
                          edm::InputTag("hltEcalRecHit", "EcalRecHitsEE"));
...
void IsolatedEcalPixelTrackCandidateProducer::produce(
...
#ifdef EDM_ML_DEBUG
  edm::LogInfo("HcalIsoTrack")
      << "ecal Collections isValid: " << ecalEB.isValid() << "/"
      << ecalEE.isValid();
#endif
...
#ifdef EDM_ML_DEBUG
  edm::LogInfo("HcalIsoTrack")
      << "coneSize_ " << coneSizeEta0_ << "/" << coneSizeEta1_
      << " hitCountEthrEB_ " << hitCountEthrEB_ << " hitEthrEB_ " << hitEthrEB_
      << " fachitCountEE_ " << fachitCountEE_ << " hitEthrEE " << hitEthrEE0_
      << ":" << hitEthrEE1_ << ":" << hitEthrEE2_ << ":" << hitEthrEE3_;
#endif
  for (int p = 0; p < nCand; p++) {
    int nhitIn(0), nhitOut(0);
    double inEnergy(0), outEnergy(0);
    std::pair<double, double> etaPhi(isoPixTrackRefs[p]->track()->eta(),
                                     isoPixTrackRefs[p]->track()->phi());
    ...
    double coneSize_ =
        (etaAbs > 1.5)
            ? coneSizeEta1_
            : (coneSizeEta0_ * (1.5 - etaAbs) + coneSizeEta1_ * etaAbs) / 1.5;
#ifdef EDM_ML_DEBUG
    edm::LogInfo("HcalIsoTrack")
        << "Track: eta/phi " << etaPhi.first << "/" << etaPhi.second
    ...
    if (etaAbs < 1.7) {
      ...
      for (auto eItr : *(ecalEB.product())) {
        ...
        double R =
            reco::deltaR(pos.eta(), pos.phi(), etaPhi.first, etaPhi.second);
        if (R < coneSize_) {
          nhitIn++;
    ...
    if (etaAbs > 1.25) {
      ...
      for (auto eItr : *(ecalEE.product())) {
        ...
        if (R < coneSize_) {
          ...
          double hitEthr =
              (((eta * hitEthrEE3_ + hitEthrEE2_) * eta + hitEthrEE1_) * eta +
               hitEthrEE0_);
...
#ifdef EDM_ML_DEBUG
    edm::LogInfo("HcalIsoTrack")
        << "nhitIn:" << nhitIn << " inEnergy:" << inEnergy
        << " nhitOut:" << nhitOut << " outEnergy:" << outEnergy;
#endif
...
#ifdef EDM_ML_DEBUG
  edm::LogInfo("HcalIsoTrack")
      << "ncand:" << nCand << " outcollction size:" << iptcCollection->size();
#endif
