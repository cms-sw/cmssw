...
public:
  ...
  static std::unique_ptr<AlCaIsoTracks::Counters> initializeGlobalCache(edm::ParameterSet const& iConfig) {
    AlCaIsoTracks::Counters *count = new AlCaIsoTracks::Counters();
    ...
    return std::unique_ptr<AlCaIsoTracks::Counters>(count);
  }
...
void AlCaIsoTracksFilter::globalEndJob(const AlCaIsoTracks::Counters* count) {
  ...
  for (unsigned int k=0; k<count->names_.size(); ++k)
    edm::LogInfo("HcalIsoTrack") << "Trigger[" << k << "]: " << count->names_[k]
