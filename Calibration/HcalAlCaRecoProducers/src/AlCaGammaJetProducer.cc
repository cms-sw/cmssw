...
void AlCaGammaJetProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  auto miniBeamSpotCollection = std::make_unique<reco::BeamSpot>(beam.position(),beam.sigmaZ(),
  ...
  if (accept) {
    ...
    for(const auto & gsfItr : gsfele) {
      miniGSFeleCollection->push_back(gsfItr);
    }

    for(const auto & convItr : conv) {
