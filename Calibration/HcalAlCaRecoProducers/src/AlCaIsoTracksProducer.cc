...
void AlCaIsoTracksProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for (reco::TrackCollection::const_iterator track=tC.begin(); track!=tC.end(); track++) {
    ...
    if (track->getHitPattern().numberOfValidHits(reco::HitPattern::TRACK_HITS) < nHitsMinCore_) continue;
	...
	if (track == track1) continue;
        if (track->getHitPattern().numberOfValidHits(reco::HitPattern::TRACK_HITS) < nHitsMinIso_) continue;
