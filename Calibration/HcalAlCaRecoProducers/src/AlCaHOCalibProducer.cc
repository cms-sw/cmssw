...
void AlCaHOCalibProducer::endJob() {
  if (m_occupancy) { 
    for (auto & ij : ho_occupency) {
      ij->Scale(1./std::max(1,Noccu));
    }
  }
...
void AlCaHOCalibProducer::fillHOStore(const reco::TrackRef& ncosm,
  ...
  for (float & ij : tmpHOCalib.caloen) {ij = 0.0;}
  int inearbymuon = 0;
  ...
  if ((m_cosmic) || (tmpHOCalib.caloen[0] <=10.0)) {
    ...
    if (ipath) { //If muon crossed HO laeyrs
      ...
      for (float & ij : tmpHOCalib.hosig) {ij=-100.0;}
      for (float & ij : tmpHOCalib.hocorsig) {ij=-100.0;}
      for (float & ij : tmpHOCalib.hbhesig) {ij=-100.0;}
