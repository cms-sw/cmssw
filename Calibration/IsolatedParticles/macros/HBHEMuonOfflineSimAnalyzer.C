...
void HBHEMuonOfflineSimAnalyzer::Loop() {
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    for (unsigned int ml = 0;ml< pt_of_muon->size();ml++) {
      ...
      if (debug_) std::cout<<"phiHcal"<<phiHcal<<" phiYHcal"<<phiYHcal<<std::endl; 
	...
	if (select) {
	  ...
	  for (int dep=0; dep<nDepth; ++dep) {
	    ...
	    if (dep == 0) {
	      ...
	      if (debug_) std::cout << "Energy in depth 6 " << hcal_edepth6->size() << ":" << hcal_edepthHot6->size() << std::endl;
	      ...
	      if (debug_) std::cout << "Energy in depth 7 " << hcal_edepth7->size() << ":" << hcal_edepthHot7->size() << std::endl;
