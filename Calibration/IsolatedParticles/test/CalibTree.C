...
void Run(//const char *inFileName="output",
	 const char *inFileName="QCD_5_1000",
	 //	 const char *inFileName="root://eoscms//eos/cms/store/caf/user/gwalia/QCD_5_1000",
	 const char *outFileName="CalibHisto",
	 const char *dirname="isopf80", const char *treeName="CalibTree");
...
CalibTree::CalibTree(TTree *tree) : fChain(0) {
  ...
  if (tree == 0) {
    ...
    TDirectory * dir = (TDirectory*)f->Get("output.root:/isopf80");
...
Double_t CalibTree::Loop(int loop) {
  ...
  sprintf(name, "Cfacs_detid_%d", loop);
  fout->WriteTObject(g_fac, name);

  g_fac2 = new TGraph(kount, dets, wfacs); 
  sprintf(name, "Wfacs_detid_%d", loop);
