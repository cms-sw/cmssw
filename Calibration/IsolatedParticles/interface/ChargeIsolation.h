...
namespace spr{
  ...
  double chargeIsolationEcal(const DetId& coreDet, reco::TrackCollection::const_iterator trkItr, const edm::Handle<reco::TrackCollection>& trkCollection, const CaloGeometry* geo, const CaloTopology* caloTopology, const MagneticField* bField, int ieta, int iphi, std::string& theTrackQuality, bool debug=false);
  ...
  double chargeIsolationHcal(reco::TrackCollection::const_iterator trkItr, const edm::Handle<reco::TrackCollection>& trkCollection, const DetId ClosestCell, const HcalTopology* topology, const CaloSubdetectorGeometry* gHB, const MagneticField* bField, int ieta, int iphi, std::string& theTrackQuality, bool debug=false);

  bool chargeIsolation(const DetId anyCell, std::vector<DetId>& vdets) ;

  // Returns the maximum energy of a track within a cone of radius *dR*
  // around the impact poiunt to the ECAL surface
  double coneChargeIsolation(const edm::Event& iEvent, const edm::EventSetup& iSetup, reco::TrackCollection::const_iterator trkItr, const edm::Handle<reco::TrackCollection>& trkCollection, TrackDetectorAssociator& associator, TrackAssociatorParameters& parameters_, const std::string& theTrackQuality, int &nNearTRKs, int &nLayers_maxNearP, int &trkQual_maxNearP, double &maxNearP_goodTrk, const GlobalPoint& hpoint1, const GlobalVector& trackMom, double dR);
