...
EcalPhiSymRecHit::EcalPhiSymRecHit():
    id_(0), chStatus_(0), nHits_(0),
    etSum_(1, 0.), et2Sum_(0),
    lcSum_(0), lc2Sum_(0)
{}
...
EcalPhiSymRecHit::EcalPhiSymRecHit(uint32_t id,
                                   ...
                                   unsigned int status):
    id_(id), chStatus_(status), nHits_(0),
    etSum_(nMisCalibV, 0.), et2Sum_(0),
    lcSum_(0), lc2Sum_(0)
{}
...
void EcalPhiSymRecHit::AddHit(std::vector<float>& etValues, float laserCorr)
{
    AddHit(etValues.data());
}
