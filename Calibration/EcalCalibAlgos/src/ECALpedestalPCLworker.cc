...
ECALpedestalPCLworker::ECALpedestalPCLworker(const edm::ParameterSet& iConfig, const DQMEDAnalyzerGlobalCache*) 
...
void ECALpedestalPCLworker::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  using namespace edm;
  ...
  for (EBDigiCollection::const_iterator pDigi = pDigiEB->begin(); pDigi != pDigiEB->end(); ++pDigi) {
    ...
    if (checkSignal_) {
      ...
      if (maxdiff > sThresholdEB_)
        continue;  // assume there is signal in this frame
    }
...
void ECALpedestalPCLworker::bookHistograms(DQMStore::IBooker& ibooker, edm::Run const& run, edm::EventSetup const& es) {
  ibooker.cd();
