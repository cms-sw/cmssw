  ...
  */
  ...
  for (int ieta = 0; ieta < kBarlRings; ieta++) {
    for (int iphi = 0; iphi < kBarlWedges; iphi++) {
      for (int sign = 0; sign < kSides; sign++) {
        ...
        float epsilon_T_SM = etsum_barl_SM_[ieta][iphi_r][sign] / etsumMean_barl_SM_[ieta] - 1.;
  ...
  for (int ieta = 0; ieta < kBarlRings; ++ieta) {
    for (int iphi_r = 0; iphi_r < int(kBarlWedges / nscx); ++iphi_r) {
      for (int sign = 0; sign < kSides; sign++) {
        scfile << ieta << " " << iphi_r << " " << sign << " " << 1 / (1 + epsilon_M_barl_SM_[ieta][iphi_r][sign])
               << std::endl;
      }
    }
  }
      ...
      for (int sign = 0; sign < kSides; sign++) {
        ...
        if (!(iphi % nscx)) {
          ...
          etsum_barl_SM_[ieta][iphi_r][sign] =
              etsum_barl_SM_[ieta][iphi_r][sign] * nscx / (nscx - nBads_barl_SM_[ieta][iphi_r][sign]);
          //         std::cout << "ETSUM M " << ieta << " " << iphi_r << " " <<
          //	     sign << " " << etsum_barl_SM_[ieta][iphi_r][sign] << "  " << nBads_barl_SM_[ieta][iphi_r][sign]<< endl;
          etsumMean_barl_SM_[ieta] += etsum_barl_SM_[ieta][iphi_r][sign] / (2 * int(kBarlWedges / nscx));
        }
        if (e_.goodCell_barl[ieta][iphi][sign]) {
          float etsum = etsum_barl_[ieta][iphi][sign];
          float esum = esum_barl_[ieta][iphi][sign];
