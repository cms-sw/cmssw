...
{
public:
    explicit EcalPhiSymRecHitProducerBase(const edm::ParameterSet& pSet,
                                                ...
                                                PhiSymCache* streamCache) const
{
    ...
    for(auto& recHit : barrelRecHitsHandle)
    {
        ...
        streamCache->recHitCollEB.at(ebHit.denseIndex()).AddHit(etValues,
                                                                laser.getLaserCorrection(recHit.id(), evtTimeStamp));
    }
    ...
    for(auto& recHit : endcapRecHitsHandle)
    {
        ...
        if(energy < eThresholdsEE_[ring] || etValues[0] > etCutsEE_[ring])
            ...
            ++totHitsEE;
        //---update the rechHit sumEt
        streamCache->recHitCollEE.at(eeHit.denseIndex()).AddHit(etValues,
                                                                laser.getLaserCorrection(recHit.id(), evtTimeStamp));
    }

    //---update the lumi info
    EcalPhiSymInfo thisEvent(totHitsEB, totHitsEE, 1, 0,
                             0, 0, 0);
...
void EcalPhiSymRecHitProducerBase::initializeStreamCache(PhiSymCache* cache) const
{
    ...
    for(auto& ebDetId : barrelDetIds_)
    {
        ...
        cache->recHitCollEB.at(id.denseIndex()) =
            EcalPhiSymRecHit(id.rawId(), nSumEtValues_);
    }
...
private:
    ...
    // event
    void accumulate(edm::StreamID stream,
                    edm::Event const& event,
    ...
    edm::EventSetup const& setup) const
{
    ...
    EcalPhiSymInfo thisLumi(0, 0, 0, 1,
                            lhcinfo.fillNumber(), lhcinfo.delivLumi(), lhcinfo.recLumi());

    //---Reset global cache
    initializeGlobalCache(setup,
                          chStatusTokenLumi_,
                          geoTokenLumi_,
                          cache);
...
void EcalPhiSymRecHitProducerLumi::globalEndLuminosityBlockProduce(
...
void EcalPhiSymRecHitProducerLumi::streamBeginLuminosityBlock(
    edm::StreamID stream,
    edm::LuminosityBlock const& lumi,
    edm::EventSetup const& setup) const
{
...
void EcalPhiSymRecHitProducerLumi::streamEndLuminosityBlock(
    edm::StreamID stream,
    edm::LuminosityBlock const& lumi,
    edm::EventSetup const& setup) const
{
...
private:
    ...
    void beginJob() { initializeJob(); };
    // run
    std::shared_ptr<PhiSymCache> globalBeginRun(edm::Run const& run,
                                                edm::EventSetup const& setup) const override;
    void globalEndRun(edm::Run const& run,
                      edm::EventSetup const& setup) const override {};
    void globalEndRunProduce(edm::Run& run,
                             edm::EventSetup const& setup) const override;    
    // lumi
    std::shared_ptr<PhiSymCache> globalBeginLuminosityBlock(edm::LuminosityBlock const& lumi,
                                                            edm::EventSetup const& setup) const override;
    ...
    void streamBeginRun(edm::StreamID stream,
                        edm::Run const& run,
                        edm::EventSetup const& setup) const override;
    void streamEndRun(edm::StreamID stream,
                      edm::Run const& run,
                      edm::EventSetup const& setup) const override;
    // event
    void accumulate(edm::StreamID stream,
...
std::shared_ptr<PhiSymCache> EcalPhiSymRecHitProducerRun::globalBeginRun(
    edm::Run const& run,
    edm::EventSetup const& setup) const
{
    auto cache = std::make_shared<PhiSymCache>();
...
void EcalPhiSymRecHitProducerRun::streamBeginRun(
    edm::StreamID stream,
    edm::Run const& run,
    edm::EventSetup const& setup) const
{
...
void EcalPhiSymRecHitProducerRun::streamEndRun(
    edm::StreamID stream,
    edm::Run const& run,
    edm::EventSetup const& setup) const
{
...
void EcalPhiSymRecHitProducerRun::globalEndRunProduce(
    edm::Run& run,
    edm::EventSetup const& setup) const 
{
