...
private:
  ...
  float
      overallCorrection_; // The Overall correction to the integrated luminosity
  ...
  std::map<std::pair<unsigned int, unsigned int>, LumiInfo *>::iterator
      lumiInfoMapIterator;
  std::map<std::pair<unsigned int, unsigned int>, LumiInfo *>
      lumiInfoMap; // map to obtain iov for lumiOb corrections to the
                   // luminosity.
  std::map<std::pair<unsigned int, unsigned int>, unsigned int>
      lumiInfoCounter; // number of lumiSections in this block
  ...
  unsigned int
      nTrain; // Number of bunch trains used in calc type 1 and 2 res, frac.
  ...
  unsigned int thisLS; // Ending lumisection for the iov that we save with the
                       // lumiInfo object.
...
CorrPCCProducer::CorrPCCProducer(const edm::ParameterSet &iConfig) {
  pccSrc_ = iConfig.getParameter<edm::ParameterSet>("CorrPCCProducerParameters")
                .getParameter<std::string>("inLumiObLabel");
  prodInst_ =
      iConfig.getParameter<edm::ParameterSet>("CorrPCCProducerParameters")
          .getParameter<std::string>("ProdInst");
  approxLumiBlockSize_ =
      iConfig.getParameter<edm::ParameterSet>("CorrPCCProducerParameters")
          .getParameter<int>("approxLumiBlockSize");
  type2_a_ =
      iConfig.getParameter<edm::ParameterSet>("CorrPCCProducerParameters")
          .getParameter<double>("type2_a");
  type2_b_ =
      iConfig.getParameter<edm::ParameterSet>("CorrPCCProducerParameters")
          .getParameter<double>("type2_b");
  countLumi_ = 0;
...
//--------------------------------------------------------------------------------------------------
// This method builds the single bunch response given an exponential function
// (type 2 only)
...
//--------------------------------------------------------------------------------------------------
// This method takes luminosity from the last bunch in a train and makes a
// comparison with the follow non-active bunch crossing to estimate the spill
// over fraction (type 1 afterglow).
  ...
  for (size_t i = 0; i < LumiConstants::numBX - 1; i++) {
    for (size_t j = i + 1; j < i + LumiConstants::numBX - 1; j++) {
      ...
      if (j < LumiConstants::numBX) {
        corrected_tmp_.at(j) =
            corrected_tmp_.at(j) - bin_i * correctionTemplate_.at(j - i);
      } else {
        corrected_tmp_.at(j - LumiConstants::numBX) =
            corrected_tmp_.at(j - LumiConstants::numBX) -
            bin_i * correctionTemplate_.at(j - i);
      }
    }
  }
...
//--------------------------------------------------------------------------------------------------
void CorrPCCProducer::evaluateCorrectionResiduals(
    std::vector<float> corrected_tmp_) {
  ...
  for (size_t ibx = 2; ibx < LumiConstants::numBX - 5; ibx++) {
    ...
    if (lumi > threshold && afterGlow[0] < threshold &&
        afterGlow[1] < threshold) {
  ...
  mean_type1_residual =
      type1.GetMean(); // Calculate the mean value of the type 1 residual
  mean_type2_residual =
      type2.GetMean(); // Calculate the mean value of the type 2 residual
...
//--------------------------------------------------------------------------------------------------
void CorrPCCProducer::calculateCorrections(
    std::vector<float> uncorrected, std::vector<float> &correctionScaleFactors_,
  ...
  for (int trial = 0; trial < nTrials; trial++) {
    ...
    edm::LogInfo("INFO") << "type 1 fraction after iteration " << trial
                         << " is  " << type1Frac;
  }
  ...
  for (size_t ibx = 0; ibx < corrected_tmp_.size(); ibx++) {
    ...
    if (corrected_tmp_.at(ibx) != 0.0 && uncorrected.at(ibx) != 0.0) {
      correctionScaleFactors_.at(ibx) =
          corrected_tmp_.at(ibx) / uncorrected.at(ibx);
    } else {
...
void CorrPCCProducer::endLuminosityBlock(edm::LuminosityBlock const &lumiSeg,
  ...
  if (totalEvents < minimumNumberOfEvents) {
    edm::LogInfo("INFO") << "number of events in this LS is too few "
                         << totalEvents;
    // return;
  } else {
...
void CorrPCCProducer::endRunProduce(edm::Run &runSeg,
  ...
  edm::LogInfo("INFO") << "Number of Lumisections " << lumiSections.size()
                       << " in run " << runSeg.run();
  ...
  for (unsigned iKey = 0; iKey < nBlocks; iKey++) {
    lsKeys.push_back(std::make_pair(
        lumiSections[(unsigned int)(iKey * nLSPerBlock)],
        lumiSections[(unsigned int)((iKey + 1) * nLSPerBlock) - 1]));
  }

  lsKeys[0].first = 1;

  for (unsigned int lumiSection = 0; lumiSection < lumiSections.size();
       lumiSection++) {
  ...
  for (lumiInfoMapIterator = lumiInfoMap.begin();
       (lumiInfoMapIterator != lumiInfoMap.end()); ++lumiInfoMapIterator) {
    ...
    edm::LuminosityBlockID lu(
        runSeg.id().run(),
        edm::LuminosityBlockNumber_t(lumiInfoMapIterator->first.first));
    thisIOV = (cond::Time_t)(lu.value());

    sprintf(histname1, "CorrectedLumiAvg_%d_%d_%d_%d", runSeg.run(), iBlock,
            lumiInfoMapIterator->first.first,
            lumiInfoMapIterator->first.second);
    sprintf(histname2, "ScaleFactorsAvg_%d_%d_%d_%d", runSeg.run(), iBlock,
            lumiInfoMapIterator->first.first,
            lumiInfoMapIterator->first.second);
    sprintf(histname3, "RawLumiAvg_%d_%d_%d_%d", runSeg.run(), iBlock,
            lumiInfoMapIterator->first.first,
            lumiInfoMapIterator->first.second);

    corrlumiAvg_h =
        new TH1F(histname1, "", LumiConstants::numBX, 1, LumiConstants::numBX);
    scaleFactorAvg_h =
        new TH1F(histname2, "", LumiConstants::numBX, 1, LumiConstants::numBX);
    lumiAvg_h =
        new TH1F(histname3, "", LumiConstants::numBX, 1, LumiConstants::numBX);
    ...
    calculateCorrections(totalLumiByBXAvg_, correctionScaleFactors_,
                         overallCorrection_);

    for (unsigned int bx = 0; bx < LumiConstants::numBX; bx++) {
      corrlumiAvg_h->SetBinContent(bx, totalLumiByBXAvg_[bx] *
                                           correctionScaleFactors_[bx]);
      if (events_.at(bx) != 0) {
        corrlumiAvg_h->SetBinError(bx, totalLumiByBXAvg_[bx] *
                                           correctionScaleFactors_[bx] /
                                           TMath::Sqrt(events_.at(bx)));
      } else {
    ...
    if (poolDbService.isAvailable()) {
      poolDbService->writeOne<LumiCorrections>(pccCorrections, thisIOV,
                                               "LumiCorrectionsRcd");
    } else {
    ...
    for (unsigned int ils = lumiInfoMapIterator->first.first;
         ils < lumiInfoMapIterator->first.second + 1; ils++) {
      if (ils > maxLS) {
    ...
    type1FracGraph->SetPoint(iBlock, thisIOV + approxLumiBlockSize_ / 2.0,
                             type1Frac);
    type1resGraph->SetPoint(iBlock, thisIOV + approxLumiBlockSize_ / 2.0,
                            mean_type1_residual);
    type2resGraph->SetPoint(iBlock, thisIOV + approxLumiBlockSize_ / 2.0,
                            mean_type2_residual);
    type1FracGraph->SetPointError(iBlock, approxLumiBlockSize_ / 2.0,
                                  mean_type1_residual_unc);
    type1resGraph->SetPointError(iBlock, approxLumiBlockSize_ / 2.0,
                                 mean_type1_residual_unc);
    type2resGraph->SetPointError(iBlock, approxLumiBlockSize_ / 2.0,
    ...
    edm::LogInfo("INFO")
        ...
        << iBlock << " " << type1Frac << " " << mean_type1_residual << " "
        << mean_type2_residual << " " << mean_type1_residual_unc << " "
        << mean_type2_residual_unc;
...
void CorrPCCProducer::bookHistograms(DQMStore::IBooker &ibooker,
  ...
  Type1FracMon = ibooker.book1D("type1Fraction",
                                "Type1Fraction;Lumisection;Type 1 Fraction",
                                maxLS, 0, maxLS);
  Type1ResMon = ibooker.book1D("type1Residual",
                               "Type1Residual;Lumisection;Type 1 Residual",
                               maxLS, 0, maxLS);
  Type2ResMon = ibooker.book1D("type2Residual",
                               "Type2Residual;Lumisection;Type 2 Residual",
                               maxLS, 0, maxLS);
}
//--------------------------------------------------------------------------------------------------
