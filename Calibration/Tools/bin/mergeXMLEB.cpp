...
//  std::cerr << "[PG] IMAEBphiWidth = " << IMAEBphiWidth << std::endl ;
  ...
  for (int etaIndex = IMAEBetaStart ; 
       etaIndex < IMAEBetaEnd ; 
       etaIndex += IMAEBetaWidth)
    {
...
//      if (effEtaIndexEnd > 85) { effEtaIndexEnd = 85 ; }
      ...
      nomeFile << coeffFolder << "/EBcalibCoeff_" << currentIndex 
               << "-" << nextIndex << ".xml" ;
...
//      std::cerr << "PG nomefile: " << fileName << std::endl ;
  ...
  for (std::map<int, EcalIntercalibConstantMap>::const_iterator itMap = 
       ...
       ++itMap)
    {
      ...
      if (etaStop * itMap->first <= 0) ++etaStop ;
      ...
      for (int iphi = IMAEBphiStart ; iphi < IMAEBphiEnd ; ++iphi)
        {
          if (!EBDetId::validDetId (ieta,iphi))
            {
              std::cerr << "[WARN] elemento " << ieta << " " << iphi 
                        << " scartato" << std::endl ;
              continue ;
            }
          EBDetId det = EBDetId (ieta,iphi,EBDetId::ETAPHIMODE) ;
          std::cerr << "[INFO] writing " << ieta << " " << iphi 
