...
int main () 
{
  ...
  std::string endcapfile ="/afs/cern.ch/user/p/presotto/CMSSW_1_6_0/src/CalibCalorimetry/CaloMiscalibTools/data/ecal_endcap_startup.xml" ; 
  map.prefillMap () ;
  ...
  if (!endcapfile2.empty ()) endcapreader2.parseXMLMiscalibFile (endcapfile2) ;
  ...
  for (int x=1;x<=100;++x)
   for (int y=1;y<100;++y)
   {
     if (!EEDetId::validDetId(x,y,-1)) continue;
