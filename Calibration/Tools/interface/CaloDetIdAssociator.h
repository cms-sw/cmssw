...
class HCaloDetIdAssociator: public HDetIdAssociator{
 ...
 protected:
   ...
   GlobalPoint getPosition(const DetId& id) override{
	...
	(const_cast<CaloGeometry*>(geometry_))->getPosition(id);
      return point;
   };
   ...
   std::vector<GlobalPoint> getDetIdPoints(const DetId& id) override{
	...
	if(!((CaloSubdetectorGeometry*)(geometry_->getSubdetectorGeometry(id)))->getGeometry(id)) {
	    LogDebug("CaloDetIdAssociator") << "Cannot find CaloCell geometry for " << id.rawId() <<"\n";
	 } else {
	  const CaloCellGeometry::CornersVec& cor ( ((CaloSubdetectorGeometry*)(geometry_->getSubdetectorGeometry(id)))->getGeometry(id)->getCorners() );
