...
BPHPhiToKKBuilder::BPHPhiToKKBuilder(const edm::EventSetup& es,
    ...
    : kPosName("KPos"), kNegName("KNeg"), evSetup(&es), posCollection(kPosCollection), negCollection(kNegCollection) {
...
vector<BPHPlusMinusConstCandPtr> BPHPhiToKKBuilder::build() {
  ...
  bPhi.add(kPosName, posCollection, BPHParticleMasses::kaonMass, BPHParticleMasses::kaonMSigma);
  bPhi.add(kNegName, negCollection, BPHParticleMasses::kaonMass, BPHParticleMasses::kaonMSigma);
...
/// get current cuts
double BPHPhiToKKBuilder::getPtMin() const { return ptSel->getPtMin(); }

double BPHPhiToKKBuilder::getEtaMax() const { return etaSel->getEtaMax(); }

double BPHPhiToKKBuilder::getMassMin() const { return massSel->getMassMin(); }

double BPHPhiToKKBuilder::getMassMax() const { return massSel->getMassMax(); }

double BPHPhiToKKBuilder::getProbMin() const { return chi2Sel->getProbMin(); }

double BPHPhiToKKBuilder::getConstrMass() const { return cMass; }

double BPHPhiToKKBuilder::getConstrSigma() const { return cSigma; }
