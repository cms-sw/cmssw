...
BPHKx0ToKPiBuilder::BPHKx0ToKPiBuilder(const edm::EventSetup& es,
    ...
    : kaonName("Kaon"), pionName("Pion"), evSetup(&es), pCollection(posCollection), nCollection(negCollection) {
...
vector<BPHPlusMinusConstCandPtr> BPHKx0ToKPiBuilder::build() {
  ...
  bKx0.add(kaonName, pCollection, BPHParticleMasses::kaonMass, BPHParticleMasses::kaonMSigma);
  bKx0.add(pionName, nCollection, BPHParticleMasses::pionMass, BPHParticleMasses::pionMSigma);
  ...
  vector<BPHPlusMinusConstCandPtr> tmpList = BPHPlusMinusCandidate::build(bKx0, kaonName, pionName);
  ...
  for (ikx = 0; ikx < nkx; ++ikx) {
    ...
    kxb->add(kaonName, kx0->originalReco(kx0->getDaug(pionName)), BPHParticleMasses::kaonMass);
    if (fabs(kx0->composite().mass() - BPHParticleMasses::kx0Mass) <
        fabs(kxb->composite().mass() - BPHParticleMasses::kx0Mass))
      pxt = px0;
    else
      pxt = pxb;
    if (!massSel->accept(*pxt))
      continue;
    if (!chi2Sel->accept(*pxt))
      continue;
...
/// get current cuts
double BPHKx0ToKPiBuilder::getPtMin() const { return ptSel->getPtMin(); }

double BPHKx0ToKPiBuilder::getEtaMax() const { return etaSel->getEtaMax(); }

double BPHKx0ToKPiBuilder::getMassMin() const { return massSel->getMassMin(); }

double BPHKx0ToKPiBuilder::getMassMax() const { return massSel->getMassMax(); }

double BPHKx0ToKPiBuilder::getProbMin() const { return chi2Sel->getProbMin(); }

double BPHKx0ToKPiBuilder::getConstrMass() const { return cMass; }

double BPHKx0ToKPiBuilder::getConstrSigma() const { return cSigma; }
