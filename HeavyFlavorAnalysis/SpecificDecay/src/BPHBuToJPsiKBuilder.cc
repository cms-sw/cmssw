...
BPHBuToJPsiKBuilder::BPHBuToJPsiKBuilder(const edm::EventSetup& es,
    ...
    : jPsiName("JPsi"), kaonName("Kaon"), evSetup(&es), jCollection(&jpsiCollection), kCollection(kaonCollection) {
...
vector<BPHRecoConstCandPtr> BPHBuToJPsiKBuilder::build() {
  ...
  bBu.add(kaonName, kCollection, BPHParticleMasses::kaonMass, BPHParticleMasses::kaonMSigma);
...
/// get current cuts
double BPHBuToJPsiKBuilder::getJPsiMassMin() const { return jpsiSel->getMassMin(); }

double BPHBuToJPsiKBuilder::getJPsiMassMax() const { return jpsiSel->getMassMax(); }

double BPHBuToJPsiKBuilder::getKPtMin() const { return ptSel->getPtMin(); }

double BPHBuToJPsiKBuilder::getKEtaMax() const { return etaSel->getEtaMax(); }

double BPHBuToJPsiKBuilder::getMassMin() const { return massSel->getMassMin(); }

double BPHBuToJPsiKBuilder::getMassMax() const { return massSel->getMassMax(); }

double BPHBuToJPsiKBuilder::getProbMin() const { return chi2Sel->getProbMin(); }

double BPHBuToJPsiKBuilder::getMassFitMin() const { return mFitSel->getMassMin(); }

double BPHBuToJPsiKBuilder::getMassFitMax() const { return mFitSel->getMassMax(); }

bool BPHBuToJPsiKBuilder::getConstr() const { return massConstr; }
