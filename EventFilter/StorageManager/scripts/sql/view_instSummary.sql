...
BEGIN
    ...
    SELECT COUNT(INSTANCE) INTO lastNumInstance FROM (SELECT RUNNUMBER, INSTANCE, SETUPLABEL, DENSE_RANK() OVER (ORDER BY RUNNUMBER DESC NULLS LAST) run
         FROM SM_INSTANCES WHERE SETUPLABEL = currentSETUP) WHERE run = 2;
