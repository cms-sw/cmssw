...
FROM ( SELECT TO_CHAR ( RUNNUMBER ) AS RUN_NUMBER,
		...
		WHEN (CASE NVL(s_injected,0)
	      ...
	      TO_CHAR ( DELETED_CHECK(    N_INSTANCE,      START_WRITE_TIME, NVL(s_injected,0), NVL(s_Checked,0),  NVL(s_Repacked,0), NVL(s_NotRepacked,0), NVL(s_Deleted, 0), STOP_TRANS_TIME ) ) AS DELETED_STATUS,
	      TO_CHAR ( dr ) AS RANK
FROM ( SELECT runnumber, stream, start_write_time, last_update_time, setuplabel, app_version, 
              ...
              s_injected, s_new, s_copied, s_checked, s_deleted, s_repacked, s_notrepacked, HLTKEY, STOP_WRITE_TIME, 
