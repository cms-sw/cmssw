...
END;
/
CREATE OR REPLACE FUNCTION Safe0_CHECK ( s_safe0 in NUMBER, s_closed in NUMBER, lastWrite in DATE)
   RETURN NUMBER AS 
   status NUMBER;
BEGIN
        status := 0;
...
CREATE OR REPLACE FUNCTION Safe99_CHECK ( lastWrite in DATE, lastTrans in DATE, s_safe0 in number, s_safe99 in number)
   RETURN NUMBER AS 
   status NUMBER;
BEGIN
        status := 0;
...
FROM ( SELECT TO_CHAR ( RUNNUMBER ) AS RUN_NUMBER,
		...
		WHEN (CASE SUM(NVL(s_injected, 0))
	      ...
	      TO_CHAR ( SAFE0_CHECK(SUM(NVL(s_NEW,0)), SUM(NVL(s_injected,0)), MAX(STOP_WRITE_TIME) ) ) AS SAFE0_STATUS,
	      TO_CHAR ( SAFE99_CHECK(MAX(STOP_WRITE_TIME), MAX(STOP_TRANS_TIME), SUM(NVL(s_NEW,0)), SUM(NVL(s_CHECKED,0)) ) ) AS SAFE99_STATUS,
