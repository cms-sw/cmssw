...
void HcalDTCHeader::copySlotData(unsigned int slot_id, const HcalHTRData& data, bool valid) {
  if (slot_id == 0 || slot_id > (unsigned int)SLOT_COUNT)
    return;
  ...
  for (unsigned int i = 1; i < slot_id; i++)
    lenSoFar += getSlotDataLength(i);
...
std::ostream& operator<<(std::ostream& s, const HcalDTCHeader& head) {
  for (int i = 0; i < HcalDTCHeader::SLOT_COUNT; i++) {
    ...
    if (head.getSlotEnabled(i))
      s << "E";
    if (head.getSlotPresent(i))
      s << "P";
    if (head.getSlotValid(i))
      s << "V";
    if (head.getSlotCRCError(i))
      s << "C";
