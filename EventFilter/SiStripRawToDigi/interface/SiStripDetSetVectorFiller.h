...
namespace sistrip {
  ...
  template<typename T, bool dsvIsSparse> inline DetSetVectorFiller<T,dsvIsSparse>::DetSetVectorFiller(const size_t registrySize, const size_t dataSize)
  {
    registry_.reserve(registrySize);
    data_.reserve(dataSize);
  }
  
  template<typename T, bool dsvIsSparse> inline DetSetVectorFiller<T,dsvIsSparse>::~DetSetVectorFiller()
  {}
  
  template<typename T, bool dsvIsSparse> inline void DetSetVectorFiller<T,dsvIsSparse>::newChannel(const uint32_t key, const uint16_t firstItem)
  {
    registry_.push_back( ChannelRegistryItem(key,data_.size(),0,firstItem) );
  }
  
  template<typename T, bool dsvIsSparse> inline void DetSetVectorFiller<T,dsvIsSparse>::addItem(const T& item)
  {
    data_.push_back(item);
    registry_.back().length++;
  }
  
  template<typename T, bool dsvIsSparse> std::unique_ptr< edm::DetSetVector<T> > DetSetVectorFiller<T,dsvIsSparse>::createDetSetVector()
  {
