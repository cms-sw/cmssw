...
void SiStripFEDRawDataAnalyzer::analyze( const edm::Event& event, const edm::EventSetup& setup ) {
  ...
  for (auto & trk_fed : trk_feds) {
	...
	else if ( mode == sistrip::READOUT_MODE_PROC_RAW && sistrip::FEDRawChannelUnpacker::procRawModeUnpacker(buffer->channel(ichan)).hasData()) channels_with_data[ifed].push_back(ichan);
	
	else if ( mode == sistrip::READOUT_MODE_SCOPE && sistrip::FEDRawChannelUnpacker::scopeModeUnpacker(buffer->channel(ichan)).hasData()) channels_with_data[ifed].push_back(ichan);
	
	else channels_missing_data[ifed].push_back(ichan);
      }
  ...
  if ( edm::isDebugEnabled() ) {
    ...
    ss << "[SiStripFEDRawDataAnalyzer::" << __func__ << "]"
       ...
       << " FED buffers that fail on construction of a FEDBuffer";
    edm::LogVerbatim(mlRawToDigi_) << ss.str();
    ss << " with ids: ";
  ...
  if ( edm::isDebugEnabled() ) {
    stringstream ss;
    ss << "[SiStripFEDRawDataAnalyzer::" << __func__ << "]"
       << " Found " << channel_construct.size() 
       <<  " FED channels that fail on FEDBuffer::channelGood()";
    edm::LogVerbatim(mlRawToDigi_) << ss.str();
    ss << " with ids: ";
  ...
  if ( edm::isDebugEnabled() ) {
    ...
    for (auto & cabling_missing_channel : cabling_missing_channels) {
      ss << cabling_missing_channel.first << "(" 
	 << cabling_missing_channel.second.size() << ":";
      for ( Channels::iterator jter = cabling_missing_channel.second.begin(); jter != cabling_missing_channel.second.end(); ++jter ) {
	ss << *jter << ",";
      }
      ss << ") ";
    }
