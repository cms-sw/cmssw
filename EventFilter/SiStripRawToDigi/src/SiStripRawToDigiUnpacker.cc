...
OldSiStripRawToDigiUnpacker::OldSiStripRawToDigiUnpacker( int16_t appended_bytes, int16_t fed_buffer_dump_freq, int16_t fed_event_dump_freq, int16_t trigger_fed_id, bool using_fed_key  ) :
  unpacker_(0),
  fedEvent_(0)
{
...
OldSiStripRawToDigiUnpacker::~OldSiStripRawToDigiUnpacker() 
{
  if (unpacker_) delete unpacker_;
  if (fedEvent_) delete fedEvent_;
}

void OldSiStripRawToDigiUnpacker::createDigis( const SiStripFedCabling& cabling, const FEDRawDataCollection& buffers, SiStripEventSummary& summary, RawDigis& scope_mode, RawDigis& virgin_raw, RawDigis& proc_raw, Digis& zero_suppr ) {
  ...
  for ( ; ifed != cabling.feds().end(); ifed++ ) {
	...
	std::stringstream ss;
	ss << "[OldSiStripRawToDigiUnpacker::" << __func__ << "]"
	   << " Found FED id " 
...
void OldSiStripRawToDigiUnpacker::updateEventSummary( const Fed9U::Fed9UEvent* const fed, SiStripEventSummary& summary ) {
