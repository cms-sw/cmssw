...
struct CSCTMBHeader2007 : public CSCVTMBHeaderFormat {
  ...
  virtual uint16_t BXNCount() const {return bxnCount;}
  virtual uint16_t ALCTMatchTime() const {return matchWin;}
  virtual uint16_t CLCTOnly() const {return clctOnly;}
  virtual uint16_t ALCTOnly() const {return alctOnly;}
  virtual uint16_t TMBMatch() const {return tmbMatch;}
  virtual uint16_t Bxn0Diff() const {return 0;}
  virtual uint16_t Bxn1Diff() const {return 0;}
  virtual uint16_t L1ANumber() const {return l1aNumber;}
  virtual uint16_t NTBins() const {return nTBins;}
  virtual uint16_t NCFEBs() const {return nCFEBs;}
  ///returns CLCT digis
  ...
  virtual unsigned short int NHeaderFrames() const {return nHeaderFrames;}
  /// returns the first data word
  virtual unsigned short * data() {return (unsigned short *) b0cline;}
  virtual bool check() const {return e0bline==0x6e0b;}
