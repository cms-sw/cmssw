...
struct CSCTMBHeader2020_GEM : public CSCVTMBHeaderFormat {
  ...
  // uint16_t GEM_sync_dataword() const {return ((bits.copad_match&0xFF) + (bits.gemA_vpf<<8) + (bits.gemB_vpf<<9)      + (bits.gemA_over_flow<<10) + (bits.gemB_over_flow<<11) + (bits.gemA_sync<<12) + (bits.gemB_sync<<13) + (bits.gems_sync<<14));}
