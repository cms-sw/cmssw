...
bool CastorCORData::check() const {
  ...
  if (trigger + daq + header + trailer > m_rawLength)
    return false;

  return true;
}

void CastorCORData::determineSectionLengths(
    int& tpWords, int& daqWords, int& headerWords, int& trailerWords, int& triggerLen) const {
  ...
  if (daq_lengths != nullptr) {
    for (ts = 0; ts < tsamples; ts++) {
      for (int j = 0; j < 12; j++) {
        ...
        daq_samples[(j * 3) * MAXIMUM_SAMPLES_PER_CHANNEL + ts] =
            ((dat & 0x40000000) >> 20) | ((dat & 0x3fe00000) >> 21) | (dv << 9);
        daq_samples[(j * 3 + 1) * MAXIMUM_SAMPLES_PER_CHANNEL + ts] =
            ((dat & 0x00100000) >> 10) | ((dat & 0x000ff800) >> 11) | (dv << 9);
        daq_samples[(j * 3 + 2) * MAXIMUM_SAMPLES_PER_CHANNEL + ts] =
            ((dat & 0x00000400)) | ((dat & 0x000003fe) >> 1) | (dv << 9);
      }
    }
  ...
  for (ts = 0; ts < timesamples; ts++) {
    ...
    for (j = 0; j < 12; j++) {
      ...
      dat = 0x00000001  // msb is dv for the fibre
            ...
            // daq_samples has ER(1)+DV(1)+CAPID(2)+EXP(2)+Mantissa(5)
            // should check daq_lengths for the 3 channels here ??
            | (s1 & 0x1ff) << 21 | (s2 & 0x1ff) << 11 | (s3 & 0x1ff) << 1 | (s1 & 0x400) << 20 | (s2 & 0x400) << 10 |
            (s3 & 0x400);
      if (somevalid)
        dat |= 0x80000000;
      // should we set dv from daq_samples ??
      if (do_capid)
        dat = (dat & 0xcff3fcff) | capid << 28 | capid << 18 | capid << 8;
...
void CastorCORData::packHeaderTrailer(
...
unsigned int CastorCORData::getOrbitNumber() const { return (m_rawConst[3] >> 11); }
unsigned int CastorCORData::getSubmodule() const { return (m_rawConst[3] & 0x7FF); }
...
bool CastorCORData::isCalibrationStream() const { return (m_formatVersion == -1) ? (false) : (m_rawConst[2] & 0x4000); }
bool CastorCORData::isUnsuppressed() const { return (m_formatVersion < 4) ? (false) : (m_rawConst[6] & 0x8000); }
...
bool CastorCORData::isPatternRAMEvent() const { return (m_formatVersion == -1) ? (false) : (m_rawConst[2] & 0x1000); }
...
int CastorCORData::getNPS() const { return (m_formatVersion == -1) ? (0) : ((m_rawConst[5] >> 4) & 0xF); }
unsigned int CastorCORData::getPipelineLength() const { return (m_rawConst[7] & 0xFF); }
unsigned int CastorCORData::getFirmwareRevision() const { return (m_rawConst[6]); }
