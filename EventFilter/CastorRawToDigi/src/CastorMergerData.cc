...
void CastorMergerData::unpack(unsigned char* tp_lengths, unsigned short* tp_samples) const {
  if (tp_lengths != nullptr)
    memset(tp_lengths, 0, 1);
...
void CastorMergerData::pack(unsigned char* tp_lengths, unsigned short* tp_samples) {
...
void CastorMergerData::packHeaderTrailer(
    int L1Anumber, int bcn, int submodule, int orbitn, int pipeline, int ndd, int nps, int firmwareRev) {
...
unsigned int CastorMergerData::getOrbitNumber() const { return (m_rawConst[3] >> 11); }

unsigned int CastorMergerData::getFirmwareRevision() const { return (m_rawConst[6]); }
