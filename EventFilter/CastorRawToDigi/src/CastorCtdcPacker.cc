...
void CastorCtdcPacker::pack(int fedid,
  ...
  for (int spigot = 0; spigot < CastorCTDCHeader::SPIGOT_COUNT; spigot++) {
      ...
      for (int fiberchan = 0; fiberchan < 3; fiberchan++) {
        ...
        if (mysamples > 0) {
          if (samples < 0)
            ...
            edm::LogError("CASTOR") << "Mismatch of samples in a single COR (unsupported) " << mysamples
                                    << " != " << samples;
            continue;
          }
          if (presamples < 0) {
            ...
            edm::LogError("CASTOR") << "Mismatch of presamples in a single COR (unsupported) " << mypresamples
                                    << " != " << presamples;
            continue;
          }
    ...
    if (npresent > 0) {
      spigots[spigot].pack(&(preclen[0]), &(precdata[0]), &(triglen[0]), &(trigdata[0]), true);
