...
ESUnpacker::ESUnpacker(const edm::ParameterSet &ps)
    : pset_(ps), fedId_(0), run_number_(0), orbit_number_(0), bx_(0), lv1_(0),
      trgtype_(0) {
...
void ESUnpacker::interpretRawData(int fedId, const FEDRawData &rawData,
  ...
  while (moreHeaders) {
    ...
    if (debug_) {
      LogDebug("ESUnpacker")
          << "[ESUnpacker]: FED Header candidate. Is header? "
          << ESHeader.check();
      if (ESHeader.check())
        LogDebug("ESUnpacker") << ". BXID: " << bx_ << " SourceID : " << fedId_
                               << " L1ID: " << lv1_;
      else
  ...
  if (fedId != fedId_) {
    if (debug_)
      edm::LogWarning("Invalid Data")
          << "Invalid ES data with source id " << fedId_;
  ...
  while (moreTrailers) {
    ...
    if (debug_) {
      LogDebug("ESUnpacker")
          << "[ESUnpacker]: FED Trailer candidate. Is trailer? "
          << ESTrailer.check();
      if (ESTrailer.check())
        LogDebug("ESUnpacker")
  ...
  int dccCRC2_ = 0;
  int dccCRC3_ = 0;
  for (const Word64 *word = (header + 1); word != (header + dccWords + 1);
       ++word) {
    if (debug_)
  ...
  if (vmajor_ < 4) {
    if (debug_)
      edm::LogWarning("Invalid Data")
          << "Invalid ES data format : " << vmajor_ << " " << vminor_;
    return;
  }
  if (dccHeaderCount != 6) {
    edm::LogWarning("Invalid Data")
  ...
  for (const Word64 *word = (header + dccWords + 1); word != trailer; ++word) {
    ...
    if (head == 12) {
      if ((opto == 0 && ESDCCHeader.getOptoRX0() == 129) ||
          (opto == 1 && ESDCCHeader.getOptoRX1() == 129) ||
...
void ESUnpacker::word2digi(int kid, int kPACE[4], const Word64 &word,
                           ESDigiCollection &digis) {
  ...
  if (debug_)
    LogDebug("ESUnpacker") << kid << " " << strip << " " << pace << " "
                           << adc[0] << " " << adc[1] << " " << adc[2];
  ...
  if (debug_)
    LogDebug("ESUnpacker") << "DetId : " << zside << " " << plane << " " << ix
                           << " " << iy << " " << strip + 1;
  ...
  if (ESDetId::validDetId(strip + 1, ix, iy, plane, zside)) {
    ...
    if (debug_)
      LogDebug("ESUnpacker")
          << "Si : " << detId.zside() << " " << detId.plane() << " "
          << detId.six() << " " << detId.siy() << " " << detId.strip() << " ("
          << kid << "," << pace << ") " << df.sample(0).adc() << " "
          << df.sample(1).adc() << " " << df.sample(2).adc();
  }
}
