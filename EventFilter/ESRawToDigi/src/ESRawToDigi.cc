...
ESRawToDigi::ESRawToDigi(edm::ParameterSet const &ps) {
  ...
  edm::InputTag fedsListLabel = ps.getUntrackedParameter<edm::InputTag>(
      "ESFedsListLabel", edm::InputTag(":esfedslist"));
...
void ESRawToDigi::fillDescriptions(
  ...
  desc.add<edm::FileInPath>(
      "LookupTable",
      edm::FileInPath("EventFilter/ESDigiToRaw/data/ES_lookup_table.dat"));
...
void ESRawToDigi::produce(edm::Event &e, const edm::EventSetup &es) {
  ...
  if (regional_) {
    for (unsigned int i = 0; i < esFeds_to_unpack.size(); ++i) {
      ...
      ESUnpacker_->interpretRawData(esFeds_to_unpack[i], fedRawData,
                                    *productDCC, *productKCHIP, *productDigis);

      if (debug_)
        std::cout << "FED : " << esFeds_to_unpack[i]
                  << " Data size : " << fedRawData.size() << " (Bytes)"
                  << std::endl;
    }
  } else {
    for (int fedId = FEDNumbering::MINPreShowerFEDID;
         fedId <= FEDNumbering::MAXPreShowerFEDID; ++fedId) {

      const FEDRawData &fedRawData = rawdata->FEDData(fedId);
      ESUnpacker_->interpretRawData(fedId, fedRawData, *productDCC,
                                    *productKCHIP, *productDigis);

      if (debug_)
        std::cout << "FED : " << fedId << " Data size : " << fedRawData.size()
                  << " (Bytes)" << std::endl;
    }
  }
