...
EcalRawToDigi::EcalRawToDigi(edm::ParameterSet const &conf)
    :
      ...
      orderedFedUnpackList_(
          conf.getParameter<std::vector<int>>("orderedFedList")),

      // define the ordered DCCId list
      orderedDCCIdList_(
          conf.getParameter<std::vector<int>>("orderedDCCIdList")),
      ...
      // See if we want to keep data even if we have a mismatch between SR
      // decision and block length
...

{
  ...
  DCCDataUnpacker::silentMode_ =
      conf.getUntrackedParameter<bool>("silentMode", false);

  if (numbXtalTSamples_ < 6 || numbXtalTSamples_ > 64 ||
      (numbXtalTSamples_ - 2) % 4) {
    std::ostringstream output;
    output << "\n Unsuported number of xtal time samples : "
           << numbXtalTSamples_
  ...
  if (numbTriggerTSamples_ != 1 && numbTriggerTSamples_ != 4 &&
      numbTriggerTSamples_ != 8) {
    std::ostringstream output;
    output << "\n Unsuported number of trigger time samples : "
  ...
  // Integrity for xtal data - EE specific (to be rivisited towards EB+EE common
  // collection)
  ...
  //  bool readResult =
  //  myMap_->readDCCMapFile(conf.getParameter<std::string>("DCCMapFile",""));

  // use two arrays from cfg to establish DCCId:FedId. If they are empy, than
  // use hard coded correspondence
  bool readResult =
      myMap_->makeMapFromVectors(orderedFedUnpackList_, orderedDCCIdList_);
  ...
  theUnpacker_ = new DCCDataUnpacker(
      myMap_, headerUnpacking_, srpUnpacking_, tccUnpacking_, feUnpacking_,
      memUnpacking_, syncCheck_, feIdCheck_, forceToKeepFRdata_);
}
  ...
  for (int i = 0; i < EEDetId::kSizeForDenseIndexing; ++i) {
    ...
    if (!id.null()) {
      ...
      if (code) {
        ...
        std::cout << " id " << id.rawId() << " -> (" << id.ix() << ", "
                  << id.iy() << ", " << id.zside() << ") "
                  << "(" << ei.dccId() << " : " << fed_id << ", "
                  << ei.towerId() << ", " << ei.stripId() << ", " << ei.xtalId()
                  << ") "
...
void EcalRawToDigi::fillDescriptions(
...
void EcalRawToDigi::produce(edm::Event &e, const edm::EventSetup &es) {
  ...
  /////////////////////// collections for problems pertaining towers are already
  /// EE+EB communal
  ...
  auto productInvalidMemBlockSizes =
      std::make_unique<EcalElectronicsIdCollection>();
  ...
  for (std::vector<int>::const_iterator i = fedUnpackList_.begin();
    ...
    if (REGIONAL_) {
      std::vector<int>::const_iterator fed_it =
          find(FEDS_to_unpack.begin(), FEDS_to_unpack.end(), *i);
  ...
  if (put_) {
    ...
    if (feUnpacking_) {
      ...
      e.put(std::move(productInvalidGainsSwitch),
      ...
      e.put(std::move(productInvalidEEGainsSwitch),
            "EcalIntegrityGainSwitchErrors");
...
EcalRawToDigi::~EcalRawToDigi() {
  ...
  // cout << "EcalDCCUnpackingModule  " << "N events        " <<
  // (nevts_-1)<<endl; cout << "EcalDCCUnpackingModule  " << " --- SETUP time "
  // << endl; cout << "EcalDCCUnpackingModule  " << "Time (sys)      " <<
  // SETUP_TIME_ << endl; cout << "EcalDCCUnpackingModule  " << "Time in sec. "
  // << SETUP_TIME_/ CLOCKS_PER_SEC  << endl; cout << "EcalDCCUnpackingModule  "
  // << " --- Per event  " << endl;

  // RUNNING_TIME_ = RUNNING_TIME_ / (nevts_-1);

  // cout << "EcalDCCUnpackingModule  "<< "Time (sys)      " << RUNNING_TIME_ <<
  // endl; cout << "EcalDCCUnpackingModule  "<< "Time in sec.    " <<
  // RUNNING_TIME_ / CLOCKS_PER_SEC  << endl;
