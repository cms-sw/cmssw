...
MatacqProducer::MatacqProducer(const edm::ParameterSet &params)
    : fileNames_(params.getParameter<std::vector<std::string>>("fileNames")),
      ...
      inputRawCollection_(
          params.getParameter<edm::InputTag>("inputRawCollection")),
      mergeRaw_(params.getParameter<bool>("mergeRaw")),
      ignoreTriggerType_(params.getParameter<bool>("ignoreTriggerType")),
      matacq_(nullptr, 0), inFile_(nullptr), data_(bufferSize),
      openedFileRunNumber_(0), lastOrb_(0), fastRetrievalThresh_(0),
      orbitOffsetFile_(
          params.getUntrackedParameter<std::string>("orbitOffsetFile", "")),
      inFileName_(""), stats_(stats_init),
      logFileName_(params.getUntrackedParameter<std::string>(
          "logFileName", "matacqProducer.log")),
      eventSkipCounter_(0), onErrorDisablingEvtCnt_(params.getParameter<int>(
                                "onErrorDisablingEvtCnt")),
      timeLogFile_(
          params.getUntrackedParameter<std::string>("timeLogFile", "")),
      runNumber_(0) {
  if (verbosity_ >= 4)
  ...
  if (logFile_.bad()) {
    throw cms::Exception("FileOpen")
        << "Failed to open file " << logFileName_ << " for logging.\n";
  }

  inputRawCollectionToken_ = consumes<FEDRawDataCollection>(
      params.getParameter<InputTag>("inputRawCollection"));
...
void MatacqProducer::produce(edm::Event &event,
  ...
  if (logTiming_) {
    ...
    timeLog_ << t.tv_sec << "." << setfill('0') << setw(3)
...
void MatacqProducer::addMatacqData(edm::Event &event) {
  ...
  if (eventSkipCounter_ == 0) {
    if (sourceColl->FEDData(matacqFedId_).size() > 4 && !produceRaw_) {
      ...
      //      cout << "---> " << (ignoreTriggerType_?"yes":"no") << " " <<
      //      getCalibTriggerType(event) << endl;
        ...
        if (getMatacqFile(runNumber, orbitId, &fileChange)) {
          ...
          if (getMatacqEvent(runNumber, orbitId, fileChange)) {
            ...
            if (produceRaw_) {
              ...
              if (dataLen64 > bufferSize * 8 ||
                  matacq_.getDccLen() != dataLen64) {
                LogWarning("Matacq")
                    << " Error in Matacq event fragment length! "
                    << "DCC len: " << matacq_.getDccLen()
                    << "*8 Bytes, Parsed len: " << matacq_.getParsedLen()
                    << "*8 Bytes.  "
            ...
            LogInfo("Matacq")
                ...
                << matacq_.getOrbitId() << " to dcc event with orbit id "
                << orbitId << std::endl;
            if (isLaserEvent) {
          ...
          LogWarning("Matacq")
              << "No matacq file found for event " << event.id();
        }
      }
    }
...
bool MatacqProducer::getMatacqEvent(uint32_t runNumber, int32_t orbitId,
                                    bool fileChange) {
  filepos_t startPos;
  if (!mtell(startPos))
    return false;
  ...
  if (verbosity_ > 2)
    cout << "[Matacq " << now() << "] Last read orbit: " << lastOrb_
  ...
  if (!posEstim_.invalid() &&
    ...
    if (pos >= 0) {
      ...
      if (mread((char *)&data_[0], headerSize, "Reading matacq header", true)) {
        ...
        didCoarseMove = false;
        if (!mread((char *)&data_[0], headerSize, "Reading event header",
                   true)) {
          return false;
        }
      }
  ...
  while (state == searching) {
    ...
    if (verbosity_ > 3) {
      filepos_t pos = -1;
      mtell(pos);
      cout << "[Matacq " << now() << "] Header read at file position " << pos
           << ":  orbit = " << orb << " len = " << len << "x8 Byte"
           << " run = " << run << "\n";
    }
    if ((abs(orb - orbitId) < orbitTolerance_) &&
      ...
      if ((searchBackward && (orb < orbitId)) ||
        ...
        state = failed;
        if (verbosity_ > 2)
          cout << "[Matacq " << now() << "] No matacq data found for run "
               << run << ", orbit ID " << orbitId << "."
               << "\n";
      } else {
...
bool MatacqProducer::getMatacqFile(uint32_t runNumber, uint32_t orbitId,
                                   bool *fileChange) {
    ...
    for (unsigned i = 0; i < fileNames_.size() && !found; ++i) {
      ...
      if (rc) {
        if (verbosity_ > 1) {
          ...
          case GLOB_NOSPACE:
            std::cout << "[Matacq " << now()
                      << "] Running out of memory while calling glob function "
                         "to look for matacq file paths\n";
            break;
          case GLOB_ABORTED:
            std::cout << "[Matacq " << now()
                      << "] Read error while calling glob function to look for "
                         "matacq file paths\n";
            break;
          case GLOB_NOMATCH:
      ...
      for (unsigned iglob = 0; iglob < g.gl_pathc; ++iglob) {
        ...
        if (!mopen(thePath) && nOpenErrors < maxOpenErrors) {
          std::cout << "[Matacq " << now() << "] Failed to open file "
                    << thePath;
          ++nOpenErrors;
          if (nOpenErrors == maxOpenErrors) {
            std::cout << nOpenErrors << "This is the " << maxOpenErrors
                      << "th occurence of this error. Report of this error is "
                         "now disabled.\n";
          } else {
            std::cout << "\n";
          }
...
uint32_t MatacqProducer::getOrbitId(edm::Event &ev) const {
  ...
  for (int id = 601; id <= 654; ++id) {
    ...
    if (data.size() >=
        8 * (orbitIdOffset64 + 1)) { // orbit id is in 4th 64-bit word
      const unsigned char *pOrbit = data.data() + orbitIdOffset64 * 8;
      int thisOrbit =
          pOrbit[0] | (pOrbit[1] << 8) | (pOrbit[2] << 16) | (pOrbit[3] << 24);
      if (orbit != 0 && thisOrbit != 0 &&
          abs(orbit - thisOrbit) > orbitTolerance_) {
...
int MatacqProducer::getCalibTriggerType(edm::Event &ev) const {
  ...
  if (p < .8) {
    // throw cms::Exception("EventCorruption") << "Inconsitency in detailed
    // trigger type indicated in ECAL DCC data headers\n";
    LogWarning("EventCorruption") << "Inconsitency in detailed trigger type "
                                     "indicated in ECAL DCC data headers\n";
    tType = -1;
  }
  return tType;
}
...
void MatacqProducer::PosEstimator::init(MatacqProducer *mp) {
  ...
  if (!mp->mread((char *)data, headerSize)) {
    ...
    if (verbosity_ > 1)
      cout << "[Matacq " << now() << "] First event orbit: " << firstOrbit_
           << " event length: " << eventLength_ << "*8 byte\n";
  }
  ...
  if (verbosity_ > 1)
    cout << "[Matacq " << now() << "] Last event orbit: " << lastOrb
...
int64_t MatacqProducer::PosEstimator::pos(int orb) const {
  ...
  uint64_t r = orbitStepMean_ != 0
                   ? (((uint64_t)(orb - firstOrbit_)) / orbitStepMean_) *
                         eventLength_ * 8
                   : 0;
  if (verbosity_ > 2)
    cout << "[Matacq " << now() << "] Estimated Position for orbit  " << orb
         << ": " << r << endl;
  return r;
}
  ...
  if (logTiming_ && startTime_.tv_sec != 0) {
    ...
    cout << "[Matacq " << now()
         ...
         << ((t.tv_sec - startTime_.tv_sec) * 1. +
             (t.tv_usec - startTime_.tv_usec) * 1.e-6)
         << "s\n";
  }
}
...
void MatacqProducer::loadOrbitOffset() {
  ...
  if (f.bad()) {
    throw cms::Exception("Matacq")
        << "Failed to open orbit ID correction file '" << orbitOffsetFile_
        << "'\n";
  }
  ...
  while (f.eof()) {
    ...
    if (buf.bad()) {
      throw cms::Exception("Matacq")
          << "Syntax error in Orbit offset file '" << orbitOffsetFile_ << "'";
    }
    cout << run << "\t" << orbit << "\n";
  ...
  try {
    ...
    if (verbosity_) {
      ...
      if (whence == SEEK_SET)
        ...
        cout << "Failed to seek position at " << offset
...
bool MatacqProducer::mrewind() {
  ...
  try {
    ...
    if (verbosity_)
      cout << "Exception cautgh while rewinding file " << inFileName_ << ": "
           << e.what() << ". "
...
bool MatacqProducer::mopen(const std::string &name) {
  ...
  try {
    inFile_ = unique_ptr<Storage>(
        StorageFactory::get()->open(name, IOFlags::OpenRead));
    inFileName_ = name;
  } catch (cms::Exception &e) {
