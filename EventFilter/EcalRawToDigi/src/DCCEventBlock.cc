...
DCCEventBlock::DCCEventBlock(DCCDataUnpacker *u, EcalElectronicsMapper *m,
                             bool hU, bool srpU, bool tccU, bool feU, bool memU,
                             bool forceToKeepFRdata)
    : unpacker_(u), mapper_(m), headerUnpacking_(hU), srpUnpacking_(srpU),
      tccUnpacking_(tccU), feUnpacking_(feU), memUnpacking_(memU),
...
unsigned int
DCCEventBlock::next_tower_search(const unsigned int current_tower_id) {
  ...
  // std::cerr << "header of bad tower = " << current_tower_id << " #" <<
  // dwToEnd_ << " 0x" << std::hex << *data_ << std::dec << std::endl; std::cerr
  // << "mask and sign = 0x" << std::hex << mask << " 0x" << sign << std::dec <<
  // std::endl;
  ...
  while (dwToEnd_ > 0) {
    ...
    // std::cerr << current_tower_id << " #" << dwToEnd_ << " 0x" << std::hex <<
    // *data_ << " 0x" << (*data_ & mask) << std::dec << std::endl;
...
void DCCEventBlock::addHeaderToCollection() {
  ...
  theRuntypeDecoder.Decode(triggerType_, DCCdetTriggerType, DCCruntype,
                           &theDCCheader);
...
bool isSynced(const unsigned int dccBx, const unsigned int bx,
              const unsigned int dccL1, const unsigned int l1,
              const BlockType type, const unsigned int fov) {
  ...
  const bool l1Synced = ((type == FE_MEM) && (l1 == ((dccL1 - 1) & 0xFFF))) ||
