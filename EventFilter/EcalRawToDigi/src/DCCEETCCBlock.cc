...
DCCEETCCBlock::DCCEETCCBlock(DCCDataUnpacker *u, EcalElectronicsMapper *m,
...
void DCCEETCCBlock::addTriggerPrimitivesToCollection() {
  ...
  if (!(dccFOV == dcc_FOV_0 || dccFOV == dcc_FOV_1 || dccFOV == dcc_FOV_2)) {
    if (!DCCDataUnpacker::silentMode_) {
      edm::LogWarning("IncorrectEvent")
          ...
          << "At event: " << event_->l1A() << " with bx " << event_->bx()
  ...
  if (dccFOV == dcc_FOV_0) {
    ...
    if (ps_) {
      for (unsigned int i = 1; i <= NUMB_PSEUDOSTRIPS;
           i++, tccP_++, psInputCounter++) {
  ...
  // real data since ever FOV was initialized; only 2 used >= June 09
    ...
    for (unsigned int i = 1; i <= NUMB_TTS_TPG1; i++, tccP_++) { // 16
      ...
      if (i <= nTTs_) {
        ...
        pTP_ = mapper_->getTPPointer(
            tccId_, i - offset); // pointer to tp digi container
        if (pTP_)
          pTP_->setSample(0, *tccP_); // fill it
      ...
      if (nTTs_ != 28 && nTTs_ != 16) {
        if (!DCCDataUnpacker::silentMode_) {
          edm::LogWarning("IncorrectBlock")
              << "Error on event " << event_->l1A() << " with bx "
              << event_->bx() << " in fed <<" << mapper_->getActiveDCC()
