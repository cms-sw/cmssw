...
DCCMemBlock::DCCMemBlock(DCCDataUnpacker *u, EcalElectronicsMapper *m,
                         DCCEventBlock *e)
  ...
  unsigned int numbOfXtalBlocks =
      (unfilteredTowerBlockLength_ - 1) / numbDWInXtalBlock_;
...
int DCCMemBlock::unpack(const uint64_t **data, unsigned int *dwToEnd,
  ...
  if ((*dwToEnd_) < 1) {
    if (!DCCDataUnpacker::silentMode_) {
      edm::LogWarning("IncorrectEvent")
          << "\nUnable to unpack MEM block for event " << event_->l1A()
  ...
  if (unfilteredTowerBlockLength_ != blockLength_) {
    ...
    if (!DCCDataUnpacker::silentMode_) {
      edm::LogWarning("IncorrectEvent")
          << "\nFor event " << event_->l1A() << ", fed "
          << mapper_->getActiveDCC() << " and tower block " << towerId_
          << "\nExpected mem block size is "
          << (unfilteredTowerBlockLength_ * 8) << " bytes while "
          << (blockLength_ * 8) << " was found";
    }
    return STOP_EVENT_UNPACKING;
  }
  ...
  if ((*dwToEnd_) < blockLength_) {
    if (!DCCDataUnpacker::silentMode_) {
      edm::LogWarning("IncorrectEvent")
          << "\nUnable to unpack MEM block for event " << event_->l1A()
          << " in fed " << mapper_->getActiveDCC() << "\n Only "
          << ((*dwToEnd_) * 8) << " bytes are available while "
  ...
  if (expTowerID_ != towerId_) {
    ...
    if (!DCCDataUnpacker::silentMode_) {
      edm::LogWarning("IncorrectBlock")
          << "For event " << event_->l1A() << " and fed "
          << mapper_->getActiveDCC() << " and sm: " << mapper_->getActiveSM()
          << "\nExpected mem tower block is " << expTowerID_ << " while "
          << towerId_ << " was found ";
    }
  ...
  for (unsigned int expStripId = 1; expStripId <= 5; expStripId++) {
    ...
    for (unsigned int expXtalId = 1; expXtalId <= 5; expXtalId++) {
      ...
      if (expStripId != stripId || expXtalId != xtalId) {
        ...
        EcalElectronicsId id(mapper_->getActiveSM(), towerId_, expStripId,
                             expXtalId);
        ...
        if (!DCCDataUnpacker::silentMode_) {
          edm::LogWarning("IncorrectBlock")
              << "For event " << event_->l1A() << ", fed "
              << mapper_->getActiveDCC() << " and tower mem block " << towerId_
      ...
      for (unsigned int i = 0; i < nTSamples_; i++) {
        ...
        // "<<std::dec<<stripId<<" Xtal id "<<xtalId
        ...
        if (gain >= 2) {
          ...
          EcalElectronicsId id(mapper_->getActiveSM(), towerId_, stripId,
                               xtalId);
          ...
          if (!DCCDataUnpacker::silentMode_) {
            edm::LogWarning("IncorrectGain")
                << "For event " << event_->l1A() << ", fed "
                << mapper_->getActiveDCC() << " , mem tower block " << towerId_
                << "\nIn strip " << stripId << " xtal " << xtalId
                << " the gain is " << gain << " in sample " << (i + 1);
          }
...
void DCCMemBlock::fillPnDiodeDigisCollection() {
  ...
  for (int pnId = 1; pnId <= 5; pnId++) {
    ...
    // solution before sending creation of PnDigi's in mapper as done with
    // crystals
    //     mapper_->getActiveSM()  : this is the 'dccid'
    //     number ranging internally in ECAL from 1 to 54, according convention
    //     specified here:
    //     http://indico.cern.ch/getFile.py/access?contribId=0&resId=0&materialId=slides&confId=11621

    //     mapper_->getActiveDCC() : this is the FED_id (601 - 654 for ECAL at
    //     CMS)
    ...
    if (NUMB_SM_EB_MIN_MIN <= activeSM && activeSM <= NUMB_SM_EB_PLU_MAX) {
      subdet = EcalBarrel;
    } else if ((NUMB_SM_EE_MIN_MIN <= activeSM &&
                activeSM <= NUMB_SM_EE_MIN_MAX) ||
               (NUMB_SM_EE_PLU_MIN <= activeSM &&
                activeSM <= NUMB_SM_EE_PLU_MAX)) {
      subdet = EcalEndcap;
    } else {
      if (!DCCDataUnpacker::silentMode_) {
    ...
    for (unsigned int ts = 0; ts < kSamplesPerPn_; ts++) {
      ...
      short pnDiodeData = pn_[(towerId_ - lastTowerBeforeMem_) * 250 +
                              (pnId - 1) * kSamplesPerPn_ + ts];
      if (pnDiodeData == -1) {
        errorOnPn = true;
        break;
      }
