...
bool EcalDCCHeaderRuntypeDecoder::Decode(
    unsigned long TrigType,    // global header,     bits 56-59
  ...
  EcalDCCHeaderInfos->setMgpaGain(
      int((runType / GainModeOffSet) & TwoBitsMask));
  EcalDCCHeaderInfos->setMemGain(
  ...
  //
  // else if (type ==5 && sequence ==
  // 3){EcalDCCHeaderInfos->setRunType(EcalDCCHeaderBlock::CALIB_GLOBAL);}
    ...
    if (!DCCDataUnpacker::silentMode_) {
      edm::LogError("IncorrectHeader")
          << "Unrecognized runtype and sequence: " << type << " " << sequence;
    }
...
void EcalDCCHeaderRuntypeDecoder::DecodeSettingGlobal(
    unsigned long TrigType, unsigned long detTrigType,
  ...
  // if calibration trigger (gap)
    ...
    int detailedTriggerTypeInTTCCommand =
        (detTrigType >> H_TR_TYPE_B) & H_TR_TYPE_MASK;
    ...
    if (detailedTriggerTypeInTTCCommand == EcalDCCHeaderBlock::TTC_LASER) {
    ...
    else if (detailedTriggerTypeInTTCCommand ==
      ...
      if (!DCCDataUnpacker::silentMode_) {
        edm::LogError("IncorrectHeader")
            << "Unrecognized detailedTriggerTypeInTTCCommand: "
...
void EcalDCCHeaderRuntypeDecoder::DecodeSetting(int Setting,
                                                EcalDCCHeaderBlock *theHeader) {
  EcalDCCHeaderBlock::EcalDCCEventSettings theSettings;
  CleanEcalDCCSettingsInfo(&theSettings);
  ...
  // no settings foreseen
    ...
    if (!DCCDataUnpacker::silentMode_) {
      edm::LogError("IncorrectHeader")
          << "Unrecognized run type: " << theHeader->getRunType();
    }
    WasDecodingOk_ = false;
  }
...
void EcalDCCHeaderRuntypeDecoder::CleanEcalDCCSettingsInfo(
    EcalDCCHeaderBlock::EcalDCCEventSettings *dummySettings) {
