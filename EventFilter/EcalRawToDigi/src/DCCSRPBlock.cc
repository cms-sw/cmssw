...
DCCSRPBlock::DCCSRPBlock(DCCDataUnpacker *u, EcalElectronicsMapper *m,
...
int DCCSRPBlock::unpack(const uint64_t **data, unsigned int *dwToEnd,
                        unsigned int numbFlags) {
  ...
  if ((*dwToEnd_) < blockLength_) {
    if (!DCCDataUnpacker::silentMode_) {
      edm::LogWarning("IncorrectEvent")
          ...
          << "\n Only " << ((*dwToEnd_) * 8) << " bytes are available while "
          << (blockLength_ * 8) << " are needed!";
    }
    ...
    if (!isSynced(dccBx, bx_, dccL1, l1_, TCC_SRP, fov)) {
      if (!DCCDataUnpacker::silentMode_) {
        edm::LogWarning("IncorrectEvent")
            ...
            << "  dccBx = " << dccBx << " bx_ = " << bx_ << " dccL1 = " << dccL1
            << " l1_ = " << l1_ << "\n"
            << "  => Stop event unpacking";
      }
      // Note : add to error collection ?
      //// SRP flags are required to check FE , better using synchronized
      /// data...
      //	   return STOP_EVENT_UNPACKING;
      updateEventPointers();
      return SKIP_BLOCK_UNPACKING;
