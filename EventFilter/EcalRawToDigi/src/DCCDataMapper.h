  ...
  public : 
     ...
     */ 
     bool operator()(DCCDataField *d1, DCCDataField * d2){
	  	bool value(false);
		if (d1->wordPosition() < d2->wordPosition()){ value=true;}
		else if(d1->wordPosition() == d2->wordPosition()){ if(d1->bitPosition() > d2->bitPosition()){value=true;} }
     	return value;  
	  } 
...
class DCCDataMapper{
	...
	protected:
		...
		enum TOWERFIELDS{
			...
			TOWERID_WPOSITION          = 0, TOWERID_BPOSITION          =   0,   TOWERID_MASK          = 0x7F, //FEID remask??
		...
		enum XTALFIELDS{
			...
			XTAL_TZS_WPOSITION         = 0, XTAL_TZS_BPOSITION         = 12,   XTAL_TZS_MASK        = 0x1,
