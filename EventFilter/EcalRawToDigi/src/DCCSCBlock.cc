...
DCCSCBlock::DCCSCBlock(DCCDataUnpacker *u, EcalElectronicsMapper *m,
                       DCCEventBlock *e, bool unpack, bool forceToKeepFRdata)
...
int DCCSCBlock::unpackXtalData(unsigned int expStripID,
                               unsigned int expXtalID) {
  ...
  // std::cout<<"\n DEBUG : unpacked xtal data for strip id "<<stripId<<" and
  // xtal id "<<xtalId<<std::endl; std::cout<<"\n DEBUG : expected strip id
  // "<<expStripID<<" expected xtal id "<<expXtalID<<std::endl;

  if (!zs_ && (expStripID != stripId || expXtalID != xtalId)) {

    if (!DCCDataUnpacker::silentMode_) {
      edm::LogWarning("IncorrectBlock")
          << "For event LV1: " << event_->l1A() << ", fed "
          << mapper_->getActiveDCC() << " and tower " << towerId_
          << "\n The expected strip is " << expStripID << " and " << stripId
          << " was found"
    ...
    pDetId_ =
        (EEDetId *)mapper_->getDetIdPointer(towerId_, expStripID, expXtalID);
    if (pDetId_) {
    ...
    if (stripId == 0 || stripId > 5 || xtalId == 0 || xtalId > 5) {
      ...
      if (!DCCDataUnpacker::silentMode_) {
        edm::LogWarning("IncorrectBlock")
            ...
            << " ids ( last strip was: " << lastStripId_
      ...
      // Check for zs valid Ids 2) if channel-in-strip has increased wrt
      // previous xtal
      ...
      if ((stripId == lastStripId_ && xtalId <= lastXtalId_) ||
        ...
        if (!DCCDataUnpacker::silentMode_) {
          edm::LogWarning("IncorrectBlock")
              << "Xtal id was expected to increase but it didn't - last xtal "
                 "id was "
              << lastXtalId_ << " while current xtal is " << xtalId << " (LV1 "
              << event_->l1A() << " fed " << mapper_->getActiveDCC()
              << " tower " << towerId_ << ")";
        }
        ...
        // adding channel following the last valid
        // pDetId_ = (EEDetId*)
        // mapper_->getDetIdPointer(towerId_,stripId,xtalId);
        //(*invalidChIds_)->push_back(*pDetId_);
        fillEcalElectronicsError(invalidZSXtalIds_);
  ...
  // otherwise, assume channel_id is valid and proceed with making and checking
  // the data frame
  if (errorOnXtal)
    return SKIP_BLOCK_UNPACKING;
  ...
  if (pDetId_) { // checking that requested EEDetId exists
    ...
    for (unsigned int i = 0; i < nTSamples_; i++) {
      ...
      if (gain == 0) {
        wrongGain = true;
      } // although gain==0 found, produce the dataFrame in order to have it,
        // for saturation case
      df.setSample(i, data);
    }
    ...
    if (wrongGain) {
      ...
      // check whether gain==0 and adc() stays constant for (at least) 5
      // consecutive samples
      unsigned int plateauEnd =
          std::min(nTSamples_, (unsigned int)(firstGainZeroSampID + 5));
      for (unsigned int s = firstGainZeroSampID; s < plateauEnd; s++) {
        if (df.sample(s).gainId() == 0 &&
            df.sample(s).adc() == firstGainZeroSampADC) {
          ;
        } else {
          isSaturation = false;
          break;
      ...
      if (!DCCDataUnpacker::silentMode_) {
        if (unpacker_->getChannelValue(mapper_->getActiveDCC(), towerId_,
                                       stripId, xtalId) != 10) {
          edm::LogWarning("IncorrectGain")
              << "Gain zero"
              << (isSaturation ? " with features of saturation" : "")
              << " was found in SC Block"
              << " (L1A " << event_->l1A() << " bx " << event_->bx() << " fed "
              << mapper_->getActiveDCC() << " tower " << towerId_ << " strip "
              << stripId << " xtal " << xtalId << ")";
        }
      }
    ...
    if (numGainWrong > 0) {
      if (!DCCDataUnpacker::silentMode_) {
        edm::LogWarning("IncorrectGain")
            << "A wrong gain transition switch was found for SC Block in strip "
            << stripId << " and xtal " << xtalId << " (L1A " << event_->l1A()
            << " bx " << event_->bx() << " fed " << mapper_->getActiveDCC()
            << " tower " << towerId_ << ")";
      }
    ...
    // Add frame to collection only if all data format and gain rules are
    // respected
    ...
    if (!mapper_->isGhost(mapper_->getActiveDCC(), towerId_,
                          stripId)) { // check the VFE is not a 'ghost'

      // this is real EE VFE - print warning
      if (!DCCDataUnpacker::silentMode_) {
        edm::LogWarning("IncorrectBlock")
            << "An EEDetId was requested that does not exist "
            << "(LV1 " << event_->l1A() << " fed " << mapper_->getActiveDCC()
            << " tower " << towerId_ << " strip " << stripId << " xtal "
            << xtalId << ")";
      }
    }
  }
...
void DCCSCBlock::fillEcalElectronicsError(
    std::unique_ptr<EcalElectronicsIdCollection> *errorColection) {
  ...
  if ((NUMB_SM_EE_MIN_MIN <= activeDCC && activeDCC <= NUMB_SM_EE_MIN_MAX) ||
    ...
    EcalElectronicsId *eleTp =
        mapper_->getSCElectronicsPointer(activeDCC, expTowerID_);
