...
DCCFEBlock::DCCFEBlock(DCCDataUnpacker *u, EcalElectronicsMapper *m,
...
int DCCFEBlock::unpack(const uint64_t **data, unsigned int *dwToEnd, bool zs,
  ...
  if ((*dwToEnd_) < 1) {
    if (!DCCDataUnpacker::silentMode_) {
      edm::LogWarning("IncorrectEvent")
          << "\n Unable to unpack Tower block for event " << event_->l1A()
          << " in fed " << activeDCC << "\n The end of event was reached "
          << "\n(or, previously, pointers intended to navigate outside of "
             "FedBlock (based on block sizes), and were stopped by setting "
             "dwToEnd_ to zero)";
  ...
  if (checkFeId_ && expTowerID_ != towerId_ &&
      expTowerID_ <=
          mapper_->getNumChannelsInDcc(
              activeDCC)) { // fe_id must be within range foreseen in the FED
    if (!DCCDataUnpacker::silentMode_) {
      edm::LogWarning("IncorrectBlock")
          << "Expected tower ID is " << expTowerID_ << " while " << towerId_
          << " was found"
  ...
  // check that expected fe_id==fe_expected is off
           ...
           towerId_ > mapper_->getNumChannelsInDcc(
                          activeDCC)) { // fe_id must still be within range
                                        // foreseen in the FED
    if (!DCCDataUnpacker::silentMode_) {
      edm::LogWarning("IncorrectBlock")
          << "For event " << event_->l1A() << " and fed "
          << mapper_->getActiveDCC() << " (there's no check fe_id==dcc_channel)"
    ...
    if (!isSynced(dccBx, bx_, dccL1, l1_, FE_MEM, fov)) {
      if (!DCCDataUnpacker::silentMode_) {
        ...
        edm::LogWarning("IncorrectBlock")
            ...
            << mapper_->getActiveDCC() << " tower " << towerId_ << ")\n"
    ...
    if (unfilteredDataBlockLength_ != blockLength_) {
      if (!DCCDataUnpacker::silentMode_) {
        edm::LogWarning("IncorrectEvent")
            ...
            << "\n Expected block size is " << (unfilteredDataBlockLength_ * 8)
            << " bytes while " << (blockLength_ * 8) << " was found"
            << "\n => Skipping to next fed block...";
      }
    ...
    if (unfilteredDataBlockLength_ != blockLength_) {
      if (!DCCDataUnpacker::silentMode_) {
        edm::LogWarning("IncorrectBlock")
            ...
            << "\n Expected block size is " << (unfilteredDataBlockLength_ * 8)
            << " bytes while " << (blockLength_ * 8) << " was found"
            << "\n => Keeps unpacking as the unpacker was forced to keep FR "
  ...
  } else if (blockLength_ > unfilteredDataBlockLength_ ||
             (blockLength_ - 1) < numbDWInXtalBlock_) {
    if (!DCCDataUnpacker::silentMode_) {
      edm::LogWarning("IncorrectEvent")
  ...
  for (unsigned int numbXtal = 1;
       numbXtal <= numbOfXtalBlocks && statusUnpackXtal != SKIP_BLOCK_UNPACKING;
       numbXtal++) {
