...
DCCEEEventBlock::DCCEEEventBlock(DCCDataUnpacker *u, EcalElectronicsMapper *m,
                                 bool hU, bool srpU, bool tccU, bool feU,
                                 bool memU, bool forceToKeepFRdata)
...
void DCCEEEventBlock::unpack(const uint64_t *buffer, size_t numbBytes,
  ...
  // Check if event size allows at least building the header
    ...
    if (!DCCDataUnpacker::silentMode_) {
      edm::LogError("IncorrectEvent")
          ...
          << "\n Event size is " << eventSize_ << " bytes while the minimum is "
          << HEADERSIZE << " bytes"
          << "\n => Skipping to next fed block...";
    }
  ...
  if (eventSize_ != blockLength_ * 8) {
    if (!DCCDataUnpacker::silentMode_) {
      edm::LogError("IncorrectEvent")
          << "\n Event L1A: " << l1_ << " in fed: " << fedId_ << "\n size is "
          << eventSize_ << " bytes while " << (blockLength_ * 8)
  ...
  // MEM (forces readout of MEM channels if enabled)is only used in the EE DCCs
  // and for FOV >1
  ...
  if (feUnpacking_ || memUnpacking_) {
    ...
    for (unsigned int chNumber = 1;
         chNumber <= numbChannels && STATUS != STOP_EVENT_UNPACKING;
         chNumber++, it++) {
      ...
      // force MEM readout if mem_ is enabled in EEs regardless on ch suppress
      // status flag
        ...
        if (!DCCDataUnpacker::silentMode_) {
          ...
          if (!ttProblem) {
            edm::LogWarning("IncorrectBlock")
                ...
                << " in the DCC channel: " << chNumber << " (LV1 " << l1_
                << " fed " << fedId_ << ")\n"
      ...
      if (feUnpacking_ && chNumber <= 68) {
        ...
        if (sr_) {
          const bool applyZS =
              ...
              ((srpBlock_->srFlag(chNumber) & SRP_SRVAL_MASK) !=
               SRP_FULLREADOUT);

          STATUS = towerBlock_->unpack(&data_, &dwToEnd_, applyZS, chNumber);

          // If there is a decision to fully suppress data this is updated in
          // channel status by the dcc
        ...
        // no SR (possibly 0 suppression flags)
          ...
          // if tzs_ data are not really suppressed, even though zs flags are
          // calculated
          if (tzs_) {
            zs_ = false;
          }
