...
EcalElectronicsMapper::EcalElectronicsMapper(unsigned int numbXtalTSamples,
                                             unsigned int numbTriggerTSamples)
    : pathToMapFile_(""), numbXtalTSamples_(numbXtalTSamples),
      numbTriggerTSamples_(numbTriggerTSamples), mappingBuilder_(nullptr) {
  resetPointers();
  setupGhostMap();
}
...
void EcalElectronicsMapper::resetPointers() {
  ...
  for (int tccId = 0; tccId < EcalTriggerElectronicsId::MAX_TCCID; tccId++) {
    for (int psCounter = 0; psCounter < EcalTrigTowerDetId::kEBTowersPerSM * 5;
         psCounter++) {
      for (int u = 0; u < 2; u++) {
...
void EcalElectronicsMapper::setEcalElectronicsMapping(
...
bool EcalElectronicsMapper::makeMapFromVectors(
    std::vector<int> &orderedFedUnpackList,
  ...
  if (orderedFedUnpackList.size() == orderedDCCIdList.size() &&
    ...
    edm::LogInfo("EcalElectronicsMapper")
...
unsigned int EcalElectronicsMapper::computeEBTCCBlockLength() {
  ...
  // TCC block size: header (8 bytes) + 17 words with 4 trigger primitives
  // (17*8bytes)
...
unsigned int EcalElectronicsMapper::getDCCId(unsigned int aSMId_) const {
  ...
  if (!DCCDataUnpacker::silentMode_) {
    edm::LogError("IncorrectMapping")
        << "DCC requested for SM id: " << aSMId_ << " not found";
  }
  return 0;
}
...
unsigned int EcalElectronicsMapper::getSMId(unsigned int aDCCId_) const {
  ...
  if (!DCCDataUnpacker::silentMode_) {
    edm::LogError("IncorrectMapping")
        << "SM requested DCC id: " << aDCCId_ << " not found";
  }
  return 0;
}
...
void EcalElectronicsMapper::fillMaps() {
  ...
  for (int smId = 1; smId <= 54; smId++) {
    ...
    if (smId > 9 && smId < 46) {
      ...
      for (int feChannel = 1; feChannel <= 68; feChannel++) {
        ...
        unsigned int rawid =
            (mappingBuilder_->getTrigTowerDetId(tccId, feChannel)).rawId();
        EcalTrigTowerDetId *ttDetId = new EcalTrigTowerDetId(rawid);
        ttDetIds_[tccId - 1][feChannel - 1] = ttDetId;
        EcalElectronicsId *ttEleId =
            new EcalElectronicsId(smId, feChannel, 1, 1);
        ...
        for (int ps = 1; ps <= 5; ps++) {
          psInput_[tccId - 1][feChannel - 1][ps - 1] =
              new EcalPseudoStripInputDigi(
                  EcalTriggerElectronicsId(tccId, feChannel, ps, 1));
          psInput_[tccId - 1][feChannel - 1][ps - 1]->setSize(1);
          psInput_[tccId - 1][feChannel - 1][ps - 1]->setSample(
              0, EcalPseudoStripInputSample(0));
        }
        ...
        for (unsigned int stripId = 1; stripId <= 5; stripId++) {
          ...
          for (unsigned int xtalId = 1; xtalId <= 5; xtalId++) {
            ...
            EBDetId *detId =
                new EBDetId((mappingBuilder_->getDetId(eid)).rawId());
            xtalDetIds_[smId - 1][feChannel - 1][stripId - 1][xtalId - 1] =
                detId;
    ...
    // Fill EE arrays (Todo : waiting SC correction)
      ...
      for (it = pTCCIds->begin(); it != pTCCIds->end(); it++) {
        ...
        for (unsigned int towerInTCC = 1;
             towerInTCC <= numChannelsInDcc_[smId - 1]; towerInTCC++) {

          // Builds Ecal Trigger Tower Det Id
          EcalTrigTowerDetId ttDetId =
              mappingBuilder_->getTrigTowerDetId(tccId, towerInTCC);

          ttDetIds_[tccId - 1][towerInTCC - 1] =
              new EcalTrigTowerDetId(ttDetId.rawId());
          EcalTriggerPrimitiveDigi *tp = new EcalTriggerPrimitiveDigi(ttDetId);
          tp->setSize(numbTriggerTSamples_);
          for (unsigned int i = 0; i < numbTriggerTSamples_; i++) {
            tp->setSample(i, EcalTriggerPrimitiveSample(0));
          ...
          for (int ps = 1; ps <= 5; ps++) {
            psInput_[tccId - 1][towerInTCC - 1][ps - 1] =
                new EcalPseudoStripInputDigi(
                    EcalTriggerElectronicsId(tccId, towerInTCC, ps, 1));
            psInput_[tccId - 1][towerInTCC - 1][ps - 1]->setSize(1);
            psInput_[tccId - 1][towerInTCC - 1][ps - 1]->setSample(
                0, EcalPseudoStripInputSample(0));
          }
        }
      }
      ...
      for (unsigned int feChannel = 1; feChannel <= numChannelsInDcc_[smId - 1];
           feChannel++) {
        ...
        std::vector<EcalScDetId> scDetIds =
            mappingBuilder_->getEcalScDetId(smId, feChannel);
        // scDetIds_[smId-1][feChannel-1] = new EcalScDetId(scDetId.rawId());
        scEleIds_[smId - 1][feChannel - 1] =
            new EcalElectronicsId(smId, feChannel, 1, 1);
        ...
        for (size_t i = 0; i < scDetIds.size(); ++i) {
          ...
          srFlags_[smId - 1][feChannel - 1].push_back(
              new EESrFlag(EcalScDetId(scDetIds[i].rawId()), 0));
        }
        ...
        std::vector<DetId> ecalDetIds =
            mappingBuilder_->dccTowerConstituents(smId, feChannel);
  ...
  // developing mapping for pseudostrip input data:
  // (tccId,psNumber)->(tccId,towerId,psId) initializing array for pseudostrip
  // data
  short numStripInTT[EcalTriggerElectronicsId::MAX_TCCID]
  ...
  for (int tccId = 0; tccId < EcalTriggerElectronicsId::MAX_TCCID; tccId++) {
    ...
    std::vector<DetId> tccConstituents =
        mappingBuilder_->tccConstituents(tccId + 1);

    for (theTCCConstituent = tccConstituents.begin();
         theTCCConstituent != tccConstituents.end(); theTCCConstituent++) {

      int towerId =
          (mappingBuilder_->getTriggerElectronicsId(*theTCCConstituent)).ttId();
      int ps = (mappingBuilder_->getTriggerElectronicsId(*theTCCConstituent))
                   .pseudoStripId();
      ...
      // << "    ps: " << ps << "    numStripInTT: " <<
      // numStripInTT[tccId][towerId-1] << std::endl;
  ...
  for (int tccId = 0; tccId < EcalTriggerElectronicsId::MAX_TCCID; tccId++) {
    ...
    psCounter = 0;
    for (int towerId = 0; towerId < EcalTrigTowerDetId::kEBTowersPerSM;
         towerId++) {
  ...
  //       for (int psCounter=0; psCounter<EcalTrigTowerDetId::kEBTowersPerSM*5;
  //       psCounter++) {
  //           std::cout << "tccId: " << (tccId+1) << "    counter: " <<
  //           (psCounter+1)
  ...
  // + 31-42 is a duplication of the last 12 ps of the previous block, and needs
  // be ignored
  // + 43-60 is further good pseudostrip data

  for (int tcc = EcalTriggerElectronicsId::MIN_TCCID_EEM;
       tcc <= EcalTriggerElectronicsId::MAX_TCCID_EEM; tcc++) {
    int tccId = tcc - 1;
  ...
  for (int tcc = EcalTriggerElectronicsId::MIN_TCCID_EEP;
       tcc <= EcalTriggerElectronicsId::MAX_TCCID_EEP; tcc++) {
    int tccId = tcc - 1;
    short tTandPs_tmp[18][2];
  ...
  // for (int tccId=0; tccId<EcalTriggerElectronicsId::MAX_TCCID; tccId++) {
  // for (int psCounter=0; psCounter<EcalTrigTowerDetId::kEBTowersPerSM*5;
  // psCounter++) { std::cout << "AF tccId: " << (tccId+1) << "    counter: " <<
  // (psCounter+1)
  //<< " tt: " << tTandPs_[tccId][psCounter][0]
  //<< " ps: " << tTandPs_[tccId][psCounter][1]
  //<< std::endl;
...
void EcalElectronicsMapper::setupGhostMap() {
  ...
  const struct {
    int FED, CCU, VFE;
      ...
      {601, 10, 5}, {601, 34, 3}, {601, 34, 4}, {601, 34, 5}, {602, 32, 5},
      {603, 12, 5}, {603, 30, 5}, {604, 12, 5}, {604, 30, 5}, {605, 32, 5},
      {606, 10, 5}, {606, 34, 3}, {606, 34, 4}, {606, 34, 5}, {608, 27, 3},
      {608, 27, 4}, {608, 27, 5}, {608, 3, 3},  {608, 3, 4},  {608, 3, 5},
      {608, 30, 5}, {608, 6, 5},  {646, 10, 5}, {646, 34, 3}, {646, 34, 4},
      {646, 34, 5}, {647, 32, 5}, {648, 12, 5}, {648, 30, 5}, {649, 12, 5},
      {649, 30, 5}, {650, 32, 5}, {651, 10, 5}, {651, 34, 3}, {651, 34, 4},
      {651, 34, 5}, {653, 27, 3}, {653, 27, 4}, {653, 27, 5}, {653, 3, 3},
      {653, 3, 4},  {653, 3, 5},  {653, 30, 5}, {653, 6, 5}};

  for (int i = 0; i < n; ++i)
    ghost_[v[i].FED][v[i].CCU][v[i].VFE] = true;
}

bool EcalElectronicsMapper::isGhost(const int FED, const int CCU,
                                    const int VFE) {
