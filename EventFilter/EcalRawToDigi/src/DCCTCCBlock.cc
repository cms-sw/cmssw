...
DCCTCCBlock::DCCTCCBlock(DCCDataUnpacker *u, EcalElectronicsMapper *m,
                         DCCEventBlock *e, bool unpack)
    : DCCDataBlockPrototype(u, m, e, unpack) {}

int DCCTCCBlock::unpack(const uint64_t **data, unsigned int *dwToEnd,
                        short tccChId) {
  ...
  if (unpackInternalData_) {
    ...
    /*
     ...
     "<<mapper_->getActiveDCC()
         <<"\n Number of time samples is "<<nTSamples_<<" while
     "<<expTriggerTSamples<<" is expected"
