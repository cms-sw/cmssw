...
DCCEBEventBlock::DCCEBEventBlock(DCCDataUnpacker *u, EcalElectronicsMapper *m,
...
void DCCEBEventBlock::unpack(const uint64_t *buffer, size_t numbBytes,
  ...
  if (feUnpacking_ || memUnpacking_) {
    ...
    for (unsigned int chNumber = 1;
      ...
      const bool problematic =
          (chStatus == CH_TIMEOUT || chStatus == CH_HEADERERR ||
        ...
        if (!DCCDataUnpacker::silentMode_) {
          ...
          if (!ttProblem) {
            edm::LogWarning("IncorrectBlock")
                << "Bad DCC channel status: " << chStatus << " (LV1 " << l1_
                << " fed " << fedId_ << " tower " << chNumber << ")\n"
      ...
      if (feUnpacking_ && chNumber <= 68) {
        ...
        if (sr_) {
          ...
          // If there is an action to suppress SR channel the associated channel
          // status should be updated so we can remove this piece of code if ( (
          // srpBlock_->srFlag(chNumber) & SRP_SRVAL_MASK) != SRP_NREAD ){
          //
          //  STATUS = towerBlock_->unpack(&data_,&dwToEnd_,applyZS,chNumber);
          //}
        }
