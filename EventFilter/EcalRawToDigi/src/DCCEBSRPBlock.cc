...
DCCEBSRPBlock::DCCEBSRPBlock(DCCDataUnpacker *u, EcalElectronicsMapper *m,
...
void DCCEBSRPBlock::addSRFlagToCollection() {
  ...
  for (unsigned int n = 0; n < expNumbSrFlags_; n++) {
    ...
    if (NUMB_SM_EB_PLU_MIN <= mapper_->getActiveSM() &&
        mapper_->getActiveSM() <= NUMB_SM_EB_PLU_MAX && fov >= 1) {
  ...
  if (nSRFlags_ != expNumbSrFlags_) {
    if (!DCCDataUnpacker::silentMode_) {
      edm::LogWarning("IncorrectBlock")
          << "Unable to unpack SRP block for event " << event_->l1A()
          << " in fed <<" << mapper_->getActiveDCC() << "\nNumber of flags "
          << nSRFlags_ << " is different from expected " << expNumbSrFlags_;
    }
