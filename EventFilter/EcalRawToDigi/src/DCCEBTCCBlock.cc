...
DCCEBTCCBlock::DCCEBTCCBlock(DCCDataUnpacker *u, EcalElectronicsMapper *m,
                             DCCEventBlock *e, bool unpack)
...
bool DCCEBTCCBlock::checkTccIdAndNumbTTs() {
  ...
  if (tccId_ != expTccId_) {
    ...
    if (!DCCDataUnpacker::silentMode_) {
      edm::LogWarning("IncorrectBlock")
          << "Error on event " << event_->l1A() << " with bx " << event_->bx()
  ...
  if (nTTs_ != expNumbTTs_) {
    if (!DCCDataUnpacker::silentMode_) {
      edm::LogWarning("IncorrectBlock")
          << "Unable to unpack TCC block for event " << event_->l1A()
          << " in fed " << mapper_->getActiveDCC() << "\n Number of TTs "
          << nTTs_ << " is different from expected " << expNumbTTs_
          << "\n TCC Block Skipped ...";
      // todo : add this to error colection
    }
    return false;
...
void DCCEBTCCBlock::addTriggerPrimitivesToCollection() {
  ...
  for (unsigned int i = 1; i <= expNumbTTs_; i++) {
    ...
    if (NUMB_SM_EB_PLU_MIN <= mapper_->getActiveSM() &&
        mapper_->getActiveSM() <= NUMB_SM_EB_PLU_MAX) {
