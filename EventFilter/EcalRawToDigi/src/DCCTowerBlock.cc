...
DCCTowerBlock::DCCTowerBlock(DCCDataUnpacker *u, EcalElectronicsMapper *m,
...
int DCCTowerBlock::unpackXtalData(unsigned int expStripID,
  ...
  if (!zs_ && (expStripID != stripId || expXtalID != xtalId)) {
    ...
    pDetId_ =
        (EBDetId *)mapper_->getDetIdPointer(towerId_, expStripID, expXtalID);
  ...
  // check id in case of 0suppressed data
    ...
    if (stripId == 0 || stripId > 5 || xtalId == 0 || xtalId > 5) {
      ...
      if ((stripId == lastStripId_ && xtalId <= lastXtalId_) ||
        ...
        if (!DCCDataUnpacker::silentMode_) {
          edm::LogWarning("IncorrectBlock")
              << "Xtal id was expected to increase but it didn't - last valid "
                 "xtal id was "
  ...
  for (unsigned int i = 0; i < nTSamples_; i++) { // loop on samples
    ...
    if (gain == 0) {
      wrongGain = true;
      // although gain==0 found, produce the dataFrame in order to have it, for
      // saturation case
    }
    df.setSample(i, data);
  } // loop on samples
  ...
  if (wrongGain) {
    ...
    if (!DCCDataUnpacker::silentMode_) {
      if (unpacker_->getChannelValue(mapper_->getActiveDCC(), towerId_, stripId,
                                     xtalId) != 10) {
  ...
  if (gainSwitchError) {
    if (!DCCDataUnpacker::silentMode_) {
      edm::LogWarning("IncorrectGain")
          << "A wrong gain transition switch was found for Tower Block in "
             "strip "
...
void DCCTowerBlock::fillEcalElectronicsError(
  ...
  if (NUMB_SM_EB_MIN_MIN <= activeDCC && activeDCC <= NUMB_SM_EB_PLU_MAX) {
    EcalElectronicsId *eleTp = mapper_->getTTEleIdPointer(
        activeDCC + TCCID_SMID_SHIFT_EB, expTowerID_);
