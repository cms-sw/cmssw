...
public:
  DCCDataUnpacker(EcalElectronicsMapper *, bool hU, bool srpU, bool tccU,
                  bool feU, bool memU, bool syncCheck, bool feIdCheck,
                  bool forceToKeepFRdata);
  ~DCCDataUnpacker();
  /**
     Unpack data from a buffer
  */
  void unpack(const uint64_t *buffer, size_t bufferSize, unsigned int smId,
              unsigned int fedId);
  ...
  */
  ...
  void
  setInvalidEEGainsSwitchCollection(std::unique_ptr<EEDetIdCollection> *x) {
  ...
  void
  setInvalidTTIdsCollection(std::unique_ptr<EcalElectronicsIdCollection> *x) {
    invalidTTIds_ = x;
  }

  void setInvalidZSXtalIdsCollection(
      std::unique_ptr<EcalElectronicsIdCollection> *x) {
    invalidZSXtalIds_ = x;
  }

  void setInvalidBlockLengthsCollection(
      std::unique_ptr<EcalElectronicsIdCollection> *x) {
    invalidBlockLengths_ = x;
  }

  void
  setPnDiodeDigisCollection(std::unique_ptr<EcalPnDiodeDigiCollection> *x) {
    pnDiodeDigis_ = x;
  }

  void setInvalidMemTtIdsCollection(
      std::unique_ptr<EcalElectronicsIdCollection> *x) {
    invalidMemTtIds_ = x;
  }

  void setInvalidMemBlockSizesCollection(
      std::unique_ptr<EcalElectronicsIdCollection> *x) {
    invalidMemBlockSizes_ = x;
  }

  void setInvalidMemChIdsCollection(
      std::unique_ptr<EcalElectronicsIdCollection> *x) {
    invalidMemChIds_ = x;
  }

  void setInvalidMemGainsCollection(
      std::unique_ptr<EcalElectronicsIdCollection> *x) {
    invalidMemGains_ = x;
  }
  ...
  */
  ...
  uint16_t getChannelValue(const int fed, const int ccu, const int strip,
