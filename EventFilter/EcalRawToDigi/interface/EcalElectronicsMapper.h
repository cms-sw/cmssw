...
public:
   ...
   * Constructor
   */
  EcalElectronicsMapper(unsigned int numbOfXtalTSamples,
  ...
  DetId *getDetIdPointer(unsigned int feChannel, unsigned int strip,
                         unsigned int xtal) {
    return xtalDetIds_[smId_ - 1][feChannel - 1][strip - 1][xtal - 1];
  }

  EcalTrigTowerDetId *getTTDetIdPointer(unsigned int tccId,
                                        unsigned int tower) {
  ...
  EcalTriggerPrimitiveDigi *getTPPointer(unsigned int tccId,
                                         unsigned int tower) {
  ...
  EcalElectronicsId *getSCElectronicsPointer(unsigned int smId,
                                             unsigned int feChannel) {
    return scEleIds_[smId - 1][feChannel - 1];
  }

  EcalPseudoStripInputDigi *getPSInputDigiPointer(unsigned int tccId,
  ...
  EcalPseudoStripInputDigi *getPSInputDigiPointer(unsigned int tccId,
    ...
    return getPSInputDigiPointer(tccId, tTandPs_[tccId - 1][psCounter - 1][0],
                                 tTandPs_[tccId - 1][psCounter - 1][1]);
  }
