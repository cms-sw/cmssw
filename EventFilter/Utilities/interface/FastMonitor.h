...
public:
  ...
  FastMonitor(std::string const& defPath, std::string defGroup, bool strictChecking, bool useSource=true, bool useDefinition=true);
  FastMonitor(DataPointDefinition * dpd, bool strictChecking, bool useSource=true, bool useDefinition=true);

  virtual ~FastMonitor();

  void addFastPathDefinition(std::string const& defPathFast, std::string defGroupFast, bool strict);
