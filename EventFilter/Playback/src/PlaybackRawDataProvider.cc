...
PlaybackRawDataProvider::~PlaybackRawDataProvider() {
  if (nullptr != runNumber_)
    delete[] runNumber_;
  if (nullptr != evtNumber_)
    delete[] evtNumber_;
  if (nullptr != eventQueue_) {
    for (unsigned int i = 0; i < queueSize_; i++)
      if (nullptr != eventQueue_[i])
        delete eventQueue_[i];
    delete[] eventQueue_;
  }
...
//______________________________________________________________________________
void PlaybackRawDataProvider::analyze(const Event& iEvent, const EventSetup& iSetup) {
  if (freeToEof_)
    return;
  waitWriteSem();
  if (freeToEof_)
...
//______________________________________________________________________________
void PlaybackRawDataProvider::endJob() { edm::LogInfo("Summary") << count_ << " events read." << endl; }

//______________________________________________________________________________
void PlaybackRawDataProvider::respondToCloseInputFile(edm::FileBlock const& fb) { filesClosed_ = true; }
...
//______________________________________________________________________________
FEDRawDataCollection* PlaybackRawDataProvider::getFEDRawData(unsigned int& runNumber, unsigned int& evtNumber) {
  FEDRawDataCollection* result = nullptr;
  waitReadSem();
  //do not read data if destructor is called
  if (destroying_)
...
void PlaybackRawDataProvider::sem_print() {
  ...
  cout << "sem_print():"
       << " wsem=" << wsem << " rsem=" << rsem << " writeIndex=" << writeIndex_ << " readIndex=" << readIndex_ << endl;
  unlock();
}
...
bool PlaybackRawDataProvider::areFilesClosed() { return filesClosed_; }
////////////////////////////////////////////////////////////////////////////////
