...
void RPCTwinMuxDigiToRaw::produce(edm::Event & event, edm::EventSetup const & setup)
{
    ...
    RPCTwinMuxPacker::getRPCTwinMuxRecords(*es_lb_link_map, *es_tm_link_map_
                                           , bx_min_, bx_max_, event.bunchCrossing()
                                           , *digi_collection
                                           , amc_bx_tmrecord
                                           , ignore_eod_);
    ...
    for (std::pair<int, std::vector<RPCAMCLink> > const & fed_amcs : fed_amcs_) {
        ...
        for (RPCAMCLink const & amc : fed_amcs.second) {
            std::map<RPCAMCLink, std::vector<std::pair<int, rpctwinmux::RPCRecord> > >::const_iterator bx_tmrecord(amc_bx_tmrecord.find(amc));
            ...
            rpctwinmux::BlockAMCContent amc_content(true
                                                    , true, true
                                                    , true, true, true, true, block_amc_size
                                                    , 0, amc.getAMCNumber()
                                                    , 0);
        ...
        for (RPCAMCLink const & amc : fed_amcs.second) {
            ...
            rpctwinmux::TwinMuxHeader tm_header(amc.getAMCNumber(), event.id().event(), event.bunchCrossing()
                                                , block_amc_size
            ...
            if (bx_tmrecord != amc_bx_tmrecord.end()) {
                for (const auto & tmrecord : bx_tmrecord->second) {
