...
RPCAMCRawToDigi::RPCAMCRawToDigi(edm::ParameterSet const & config)
    ...
    , rpc_unpacker_(RPCAMCUnpackerFactory::get()->create(config.getParameter<std::string>("RPCAMCUnpacker")
                                                         , *this
...
void RPCAMCRawToDigi::produce(edm::Event & event, edm::EventSetup const & setup)
{
    ...
    for (int fed : rpc_unpacker_->getFeds()) {
        ...
        if (!processBlocks(fed
                           , word, word_end
                           , crc, *counters, amc_payload)) {
            continue;
        }
    ...
    rpc_unpacker_->produce(event, setup
                           , amc_payload);
...
bool RPCAMCRawToDigi::processCDFTrailers(int fed, unsigned int nwords
                                         , std::uint64_t const * & word, std::uint64_t const * & word_end
                                         , std::uint16_t & crc
                                         , RPCAMCLinkCounters & counters) const
{
    ...
    while (word < word_end) {
        ...
        for (unsigned int amc = 0 ; amc < n_amc ; ++amc) {
            ...
            if (payload_word_end > word_end) {
                ...
                edm::LogWarning("RPCAMCRawToDigi") << "AMC Block can not be complete for FED " << fed
                                                   << " at AMC " << amc_number;
                return false;
            }
            ...
            if (amc13_amc_header.isFirstBlock()) {
                ...
                if (!amc13_amc_header.isValid()) {
                    ...
                    edm::LogWarning("RPCAMCRawToDigi") << "AMC13 AMC Header is reporting an invalid "
                ...
                if (amc_number!= amc_amc_header.getAMCNumber()) {
                    ...
                    edm::LogWarning("RPCAMCRawToDigi") << "AMC Number inconsistent in AMC13 AMC Header vs AMC Header: " << amc_number
                                                       << " vs " << amc_amc_header.getAMCNumber();
                    payload.setValid(false);
                }

                if (amc_amc_header.hasDataLength()
                    && amc13_amc_header.hasTotalSize()
                    ...
                    edm::LogWarning("RPCAMCRawToDigi") << "AMC size inconsistent in AMC13 AMC Header vs AMC Header: " << amc13_amc_header.getSize()
                                                       << " vs " << amc_amc_header.getDataLength();
                    payload.setValid(false);
                }
            }
            ...
            if (amc13_amc_header.isLastBlock()) {
                ...
                if (!amc13_amc_header.isCRCOk()) {
                    ...
                    edm::LogWarning("RPCAMCRawToDigi") << "AMC13 AMC Header is reporting a mismatched  "
                                                       << "Event Counter or Bunch Counter for FED " << fed
                                                       << ", AMC " << amc_number;
                    payload.setValid(false);
                }
