...
void RPCTwinMuxPacker::getRPCTwinMuxRecords(
  ...
  for (auto mlb_bx_lbrecord_it = mlb_bx_lbrecord.begin(); mlb_bx_lbrecord_it != mlb_bx_lbrecord.end();
       ++mlb_bx_lbrecord_it) {
    // multimap, but no splitting for TwinMux inputs
    auto amc_it(amc_map.getMap().find(mlb_bx_lbrecord_it->first));
    ...
    auto tmrecord_it(bx_tmrecord.begin());
    for (auto bx_lbrecord = mlb_bx_lbrecord_it->second.begin(); bx_lbrecord != mlb_bx_lbrecord_it->second.end();
         ++bx_lbrecord) {
