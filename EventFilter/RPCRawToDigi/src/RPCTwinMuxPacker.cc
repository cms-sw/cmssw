...
void RPCTwinMuxPacker::getRPCTwinMuxRecords(RPCInverseLBLinkMap const & lb_map, RPCInverseAMCLinkMap const & amc_map
                                            ...
                                            , bool ignore_eod)
{
    ...
    RPCLBPacker::getRPCLBRecords(lb_map
                                 , min_bx, max_bx, bcn
                                 , digis
                                 , mlb_bx_lbrecord
                                 , ignore_eod);
    ...
    for (std::map<RPCLBLink, std::vector<std::pair<int, RPCLBRecord> > >::const_iterator mlb_bx_lbrecord_it = mlb_bx_lbrecord.begin()
             ; mlb_bx_lbrecord_it != mlb_bx_lbrecord.end() ; ++mlb_bx_lbrecord_it) {
        ...
        for (std::vector<std::pair<int, RPCLBRecord> >::const_iterator bx_lbrecord = mlb_bx_lbrecord_it->second.begin()
                 ; bx_lbrecord != mlb_bx_lbrecord_it->second.end() ; ++bx_lbrecord) {
            ...
            if (tmrecord_it == bx_tmrecord.end() || tmrecord_it->first != bx_lbrecord->first) {
                tmrecord_it = bx_tmrecord.insert(tmrecord_it
                                                 , std::pair<int, rpctwinmux::RPCRecord>(bx_lbrecord->first
