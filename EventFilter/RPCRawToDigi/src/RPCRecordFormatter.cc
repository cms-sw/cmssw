...
RPCRecordFormatter::RPCRecordFormatter(int fedId, const RPCReadOutMapping* r) : currentFED(fedId), readoutMapping(r) {}

RPCRecordFormatter::~RPCRecordFormatter() {}

std::vector<EventRecords> RPCRecordFormatter::recordPack(uint32_t rawDetId, const RPCDigi& digi, int trigger_BX) const {
  ...
  for (RawDataFrames::const_iterator ir = rawDataFrames.begin(); ir != rawDataFrames.end(); ir++) {
    ...
    if (eleIndex.dccId == currentFED) {
      LogTrace("pack:") << " dccId= " << eleIndex.dccId << " dccInputChannelNum= " << eleIndex.dccInputChannelNum
                        << " tbLinkInputNum= " << eleIndex.tbLinkInputNum << " lbNumInLink=" << eleIndex.lbNumInLink;
...
int RPCRecordFormatter::recordUnpack(const EventRecords& event,
  ...
  if (event.recordCD().eod()) {
    if (counter)
      counter->addReadoutError(currentFED, ReadoutError(eleIndex, ReadoutError::EOD));
  }

  if (readoutMapping == nullptr)
    return error.type();
  const LinkBoardSpec* linkBoard = readoutMapping->location(eleIndex);
  if (!linkBoard) {
    if (debug)
      LogDebug("") << " ** PROBLEM ** Invalid Linkboard location, skip CD event, "
                   << "dccId: " << eleIndex.dccId << "dccInputChannelNum: " << eleIndex.dccInputChannelNum
                   << " tbLinkInputNum: " << eleIndex.tbLinkInputNum << " lbNumInLink: " << eleIndex.lbNumInLink;
    error = ReadoutError(eleIndex, ReadoutError::InvalidLB);
    if (counter)
      counter->addReadoutError(currentFED, error);
    return error.type();
  }
  ...
  if (synchro)
    synchro->push_back(make_pair(eleIndex, event.dataToTriggerDelay()));
