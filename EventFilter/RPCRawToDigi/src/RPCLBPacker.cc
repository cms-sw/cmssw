                                  ...
                                  , bool ignore_eod)
{
    if (max_bx - min_bx >= 128 || min_bx < -3564) { // limit 7 bits RPCLBRecord BCN; avoid overflow
        throw cms::Exception("RPCLBPacker")
            << "Out-of-range input for min_bx, max_bx (" << min_bx << ": " << max_bx << ")";
    }
    ...
    for (RPCDigiCollection::DigiRangeIterator digi_range = digis.begin()
             ; digi_range != digi_range_end ; ++digi_range) {
        ...
        std::pair<RPCInverseLBLinkMap::map_type::const_iterator
                  , RPCInverseLBLinkMap::map_type::const_iterator> lookup_range(lb_map.getMap().equal_range(digi_range_value.first.rawId()));

        for (RPCDigiCollection::const_iterator digi = digi_range_value.second.first
                 ; digi != digi_range_value.second.second ; ++digi) {
            if (digi->bx() < min_bx || digi->bx() > max_bx) {
                continue;
            }

            for (RPCInverseLBLinkMap::map_type::const_iterator link_it = lookup_range.first
                     ; link_it != lookup_range.second ; ++link_it) {
    ...
    for (std::map<RPCLBLink, std::vector<RPCLBRecord> >::iterator mlb_lbrecords_it = mlb_lbrecords.begin()
             ; mlb_lbrecords_it != mlb_lbrecords.end() ; ++mlb_lbrecords_it) {
        ...
        unsigned int idx(0);
        for (std::vector<RPCLBRecord>::const_iterator input_it = input.begin() + 1
                 ; input_it <= input.end() ; ++input_it) {
            if (input_it != input.end()
                && ((last_lbrecord.getRecord() & ~RPCLBRecord::partition_data_mask_)
        ...
        mlb_bx_lbrecord.insert(std::map<RPCLBLink
                               , std::vector<std::pair<int, RPCLBRecord> > >::value_type(mlb_lbrecords_it->first
                                                                                         , bx_lbrecord));
    }
}
