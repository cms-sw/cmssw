...
void Phase2ClusterProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) 
{
    ...
    unsigned int DTCId = 0; 
    const FEDRawData& fedData = fedRawDataCollection->FEDData(DTCId);  // FED ID 0
    if (fedData.size() > 0) 
    {
...
//       }
      ...
      for (unsigned int ichannel = 0; ichannel < 36; ichannel++){
        ...
        for (unsigned int icluster = 0; icluster < numClusters; icluster++){
          ...
          if (n_available_bits - clusterwordlength > 0)
          {
            int shift = linelength - (icluster+1) * clusterwordlength;
            ...
            n_available_bits = n_available_bits - clusterwordlength;
//             std::cout << "\t\t\t remaining bits " << n_available_bits <<  std::endl;
            ...
            int nMask_newLine = 0;
            int bits_to_read = clusterwordlength - n_available_bits;
            ...
            uint16_t cluster3 = ((cluster_part0 << bits_to_read) | cluster_part2);
            clusters_words[icluster] = cluster3;
            std::cout << "\t\t\t cluster " << icluster << std::bitset<14>(clusters_words[icluster]) <<  std::endl;
...
//           
        ...
        // FIXME: all of this should be moved outside the loop on the clusters
        ...
        std::cout << "output cluster collection tmp size = " << outputClusterCollection->size() <<  std::endl;
      } // end loop on channels for this fed
    } // end fed data size > 0
    std::cout << "output cluster collection FINAL size = " << outputClusterCollection->size() <<  std::endl;
