...
class Cluster
{
    ...
    public:
        ...
        Cluster ( unsigned int& z,  double x,  unsigned int& width,  unsigned int& chipId,  unsigned int& sclusteraddress,  unsigned int& mipbit, unsigned int& cicid , TrackerGeometry::ModuleType& type) : z(z), x(x), width(width), chipId(chipId), sclusteraddress(sclusteraddress), mipbit(mipbit), cicId(cicid), ClusterType(type) {}
        ~Cluster() {}
