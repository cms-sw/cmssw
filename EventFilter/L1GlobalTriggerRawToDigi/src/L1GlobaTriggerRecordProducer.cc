...
L1GlobalTriggerRecordProducer::L1GlobalTriggerRecordProducer(const edm::ParameterSet& parSet) {
  ...
  m_l1GtReadoutRecordTag =
      consumes<L1GlobalTriggerReadoutRecord>(parSet.getParameter<edm::InputTag>("L1GtReadoutRecordTag"));
...
void L1GlobalTriggerRecordProducer::produce(edm::Event& iEvent, const edm::EventSetup& evSetup) {
  ...
  for (auto itBit = algoDecisionWord.begin(); itBit != algoDecisionWord.end(); ++itBit) {
    iBit++;
  ...
  for (auto itBit = techDecisionWord.begin(); itBit != techDecisionWord.end(); ++itBit) {
    iBit++;
  ...
  unsigned int pfIndexTech = static_cast<unsigned int>((gtReadoutRecord->gtFdlWord()).gtPrescaleFactorIndexTech());
  unsigned int pfIndexAlgo = static_cast<unsigned int>((gtReadoutRecord->gtFdlWord()).gtPrescaleFactorIndexAlgo());
