...
namespace l1t {
	namespace stage2 {
		namespace emtf {
			...
			void MEBlockUnpacker::checkFormat(const Block& block) { 				

				auto payload = block.payload();

				//Check the number of 16-bit words
				if(payload.size() != 4) edm::LogError("L1T|EMTF") << "Payload size in 'ME Data Record' is different than expected";

				//Check that each word is 16 bits
				if(GetHexBits(payload[0], 16, 31) != 0) edm::LogError("L1T|EMTF") << "Payload[0] has more than 16 bits in 'ME Data Record'";
				if(GetHexBits(payload[1], 16, 31) != 0) edm::LogError("L1T|EMTF") << "Payload[1] has more than 16 bits in 'ME Data Record'";
				if(GetHexBits(payload[2], 16, 31) != 0) edm::LogError("L1T|EMTF") << "Payload[2] has more than 16 bits in 'ME Data Record'";
				if(GetHexBits(payload[3], 16, 31) != 0) edm::LogError("L1T|EMTF") << "Payload[3] has more than 16 bits in 'ME Data Record'";
				...
				//Check Format
				if(GetHexBits(mea, 15, 15) != 1) edm::LogError("L1T|EMTF") << "Format identifier bits in MEa are incorrect";
				if(GetHexBits(meb, 15, 15) != 1) edm::LogError("L1T|EMTF") << "Format identifier bits in MEb are incorrect";
				if(GetHexBits(mec, 15, 15) != 0) edm::LogError("L1T|EMTF") << "Format identifier bits in MEc are incorrect";
				if(GetHexBits(med, 15, 15) != 0) edm::LogError("L1T|EMTF") << "Format identifier bits in MEd are incorrect";
