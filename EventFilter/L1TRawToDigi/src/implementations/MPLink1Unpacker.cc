...
namespace l1t {
   class MPLink1Unpacker : public BaseUnpacker {
      public:
         MPLink1Unpacker(const edm::ParameterSet&, edm::Event&);
         ~MPLink1Unpacker();
   ...
   class MPLink1UnpackerFactory : public BaseUnpackerFactory {
      public:
         MPLink1UnpackerFactory(const edm::ParameterSet&, edm::one::EDProducerBase&);
...
namespace l1t {
   MPLink1Unpacker::MPLink1Unpacker(const edm::ParameterSet& cfg, edm::Event& ev) :
      ev_(ev),
   ...
   MPLink1Unpacker::~MPLink1Unpacker()
   {
      ev_.put(res1_);
      ev_.put(res2_);
   };
   ...
   bool
   MPLink1Unpacker::unpack(const unsigned char *data, const unsigned block_id, const unsigned size)
   {
   ...
   MPLink1UnpackerFactory::MPLink1UnpackerFactory(const edm::ParameterSet& cfg, edm::one::EDProducerBase& prod) : cfg_(cfg), prod_(prod)
   {
      prod_.produces<JetBxCollection>("MPLink1");
      prod_.produces<EtSumBxCollection>("MPLink1");
   }

   std::vector<UnpackerItem>
   MPLink1UnpackerFactory::create(edm::Event& ev, const unsigned& fw, const int fedid) {
     return {std::make_pair(3, std::shared_ptr<BaseUnpacker>(new MPLink1Unpacker(cfg_, ev)))};
   };
}

DEFINE_L1TUNPACKER(l1t::MPLink1UnpackerFactory);
