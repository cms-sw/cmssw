...
namespace l1t {
   class MPLink3Unpacker : public BaseUnpacker {
      public:
         MPLink3Unpacker(const edm::ParameterSet&, edm::Event&);
         ~MPLink3Unpacker();
   ...
   class MPLink3UnpackerFactory : public BaseUnpackerFactory {
      public:
         MPLink3UnpackerFactory(const edm::ParameterSet&, edm::one::EDProducerBase&);
...
namespace l1t {
   MPLink3Unpacker::MPLink3Unpacker(const edm::ParameterSet& cfg, edm::Event& ev) :
      ev_(ev),
   ...
   MPLink3Unpacker::~MPLink3Unpacker()
   {
      ev_.put(res1_);
      ev_.put(res2_);
   };
   ...
   bool
   MPLink3Unpacker::unpack(const unsigned char *data, const unsigned block_id, const unsigned size)
   {
   ...
   MPLink3UnpackerFactory::MPLink3UnpackerFactory(const edm::ParameterSet& cfg, edm::one::EDProducerBase& prod) : cfg_(cfg), prod_(prod)
   {
      prod_.produces<JetBxCollection>("MPLink3");
      prod_.produces<EtSumBxCollection>("MPLink3");
   }

   std::vector<UnpackerItem>
   MPLink3UnpackerFactory::create(edm::Event& ev, const unsigned& fw, const int fedid) {
     return {std::make_pair(7, std::shared_ptr<BaseUnpacker>(new MPLink3Unpacker(cfg_, ev)))};
   };
}

DEFINE_L1TUNPACKER(l1t::MPLink3UnpackerFactory);
