...
namespace l1t {
   class RCTUnpacker : public BaseUnpacker {
      public:
      ...
      private:
         edm::Event& ev_;
         std::auto_ptr<CaloEmCandBxCollection> res1_;
         std::auto_ptr<CaloRegionBxCollection> res2_;
...
namespace l1t {
   ...
   bool
   RCTUnpacker::unpack(const unsigned char *data, const unsigned block_id, const unsigned size)
   {
  ...
  RCTUnpackerFactory::RCTUnpackerFactory(const edm::ParameterSet& cfg, edm::one::EDProducerBase& prod) : cfg_(cfg), prod_(prod)
  {
      prod_.produces<JetBxCollection>("RCT");
      prod_.produces<EtSumBxCollection>("RCT");
  }
  
   std::vector<UnpackerItem>
   RCTUnpackerFactory::create(edm::Event& ev, const unsigned& fw, const int fedid) {
     ...
     if (fedid==2){
       ...
       auto unpacker = std::shared_ptr<BaseUnpacker>(new RCTUnpacker(cfg_, ev));
...
DEFINE_L1TUNPACKER(l1t::RCTUnpackerFactory);
