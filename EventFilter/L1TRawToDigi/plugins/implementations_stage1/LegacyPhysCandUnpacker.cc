...
namespace l1t {
  namespace stage1 {
    namespace legacy {
      bool IsoEGammaUnpacker::unpack(const Block& block, UnpackerCollections* coll) {
        auto res = static_cast<CaloCollections*>(coll)->getEGammas();
        return process(block, res, [](l1t::EGamma eg) {
          eg.setHwIso(1);
          return eg;
        });
      }

      bool NonIsoEGammaUnpacker::unpack(const Block& block, UnpackerCollections* coll) {
        auto res = static_cast<CaloCollections*>(coll)->getEGammas();
        return process(block, res, [](const l1t::EGamma& eg) { return eg; });
      }

      bool CentralJetUnpacker::unpack(const Block& block, UnpackerCollections* coll) {
      ...
      bool ForwardJetUnpacker::unpack(const Block& block, UnpackerCollections* coll) {
        auto res = static_cast<CaloCollections*>(coll)->getJets();

        if (res->size(0) != 4)
          edm::LogWarning("L1T") << "Need to unpack central jets before forward ones";

        return process(block, res, [](l1t::Jet j) {
      ...
      bool IsoTauUnpacker::unpack(const Block& block, UnpackerCollections* coll) {
