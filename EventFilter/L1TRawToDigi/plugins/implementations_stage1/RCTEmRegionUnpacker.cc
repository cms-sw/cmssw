  ...
  namespace stage1 {
    void unpack_em(const Block& block, UnpackerCollections* coll) {
      ...
      for (int bx = firstBX; bx <= lastBX; bx++) {
        ...
        LogDebug("L1T") << "--------------- mp7 link =" << mp7link << "RCT crate id=" << crate
                        << ", RCT crate even=" << even << std::endl;

        if (!even) {
          for (int i = 0; i < 6; i++)
            converter.Set32bitWordLinkOdd(i, uint[i]);
          converter.Convert();
          ...
          for (int j = 0; j < 4; j++) {
            ...
            LogDebug("L1T") << "UNPACKER, CRATE" << crate << "NON ISO em rank=" << rank << ", region=" << reg
                            << ", card=" << card << std::endl;
          ...
          for (int j = 0; j < 4; j++) {
            ...
            LogDebug("L1T") << "UNPACKER, CRATE" << crate << "ISO em rank=" << rank << ", region=" << reg
                            << ", card=" << card << std::endl;
    ...
    void unpack_region(const Block& block, UnpackerCollections* coll) {
      ...
      for (int bx = firstBX; bx <= lastBX; bx++) {
        ...
        if (!even) {
          ...
          for (int i = 0; i < 6; i++)
            converter.Set32bitWordLinkEven(i, uint[i]);
          converter.Convert();
          ...
          for (int j = 0; j < 7; j++) {
            for (int k = 0; k < 2; k++) {
              ...
              LogDebug("L1T") << "UNPACKER, CRATE=" << crate << ",region=" << k << ", card=" << j << ", rgnEt=" << RCet
                              << ", overflow=" << overflow << ", tauveto=" << tauveto << ", hadveto=" << hadveto
                              << std::endl;
