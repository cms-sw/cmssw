...
namespace l1t
{
  namespace stage2
  {
    void ZSnumWheelSectorTrTag(int& wheelNo, int& tagSegID, int linkNo, int amcNo)
    {
    ...
    bool ZScheckQual(const unsigned int& value, const bool& isNewFw)
    {
      if (isNewFw)
	return (value == 7);
      else 
      ...
      for(auto ibx : bxBlocks)//Bx iteration
	{
	  ...
	  ZSnumWheelSectorTrTag(wheel, trTag, blockId/2, block.amc().getAMCNumber());//this returns wheel & tsTag
	  ...
	  for (int iw = 0; iw < 4; iw++)// 4 phi (32-bit) words
		...
		mbPhiB[iw] = (inputWords[iw] >> 12) & 0x3FF;
	      ...
	      //if (mbQual[iw] == 0)
	      if (ZScheckQual(mbQual[iw], isNewFw))
		continue;
    ...
    bool BMTF_ZSUnpackerInputsOldQual::unpack(const Block& block, UnpackerCollections *coll)
    {
      return ZSunpacking(block, coll, linkAndQual_, false);
    }//unpack old quality

    bool BMTF_ZSUnpackerInputsNewQual::unpack(const Block& block, UnpackerCollections *coll)
    {
      return ZSunpacking(block, coll, linkAndQual_, true);
    }//unpack new quality
  }//ns2
}//ns l1t;

DEFINE_L1T_UNPACKER(l1t::stage2::BMTF_ZSUnpackerInputsOldQual);
DEFINE_L1T_UNPACKER(l1t::stage2::BMTF_ZSUnpackerInputsNewQual);
