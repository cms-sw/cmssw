...
namespace l1t {
   namespace stage2 {
      ...
      EMTFSetup::registerConsumes(const edm::ParameterSet& cfg, edm::ConsumesCollector& cc)
      {
      ...
      void
      EMTFSetup::fillDescription(edm::ParameterSetDescription& desc)
      {
         desc.addOptional<edm::InputTag>("EMTFInputLabelAWB")->setComment("for stage2");
      }

      PackerMap
      EMTFSetup::getPackers(int fed, unsigned int fw)
      {
         PackerMap res;
      ...
      void
      EMTFSetup::registerProducts(edm::stream::EDProducerBase& prod)
      {
      ...
      EMTFSetup::getCollections(edm::Event& e)
      {
         return std::unique_ptr<UnpackerCollections>(new EMTFCollections(e));
      }

      UnpackerMap
      EMTFSetup::getUnpackers(int fed, int board, int amc, unsigned int fw)
      {
