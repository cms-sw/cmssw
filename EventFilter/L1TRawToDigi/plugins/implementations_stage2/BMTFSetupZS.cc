...
namespace l1t {
   namespace stage2 {
      ...
      BMTFSetupZS::registerConsumes(const edm::ParameterSet& cfg, edm::ConsumesCollector& cc)
      {
         return std::unique_ptr<PackerTokens>(new BMTFTokens(cfg, cc));
      }

      void BMTFSetupZS::fillDescription(edm::ParameterSetDescription& desc) {};

      PackerMap
      BMTFSetupZS::getPackers(int fed, unsigned int fw)
      {
         PackerMap res;
	 ...
	 if (fed == 1376) {
      ...
      void
      BMTFSetupZS::registerProducts(edm::stream::EDProducerBase& prod)
      {
      ...
      BMTFSetupZS::getCollections(edm::Event& e)
      {
         return std::unique_ptr<UnpackerCollections>(new BMTFCollections(e));
      }

      UnpackerMap
      BMTFSetupZS::getUnpackers(int fed, int board, int amc, unsigned int fw)
      {

         auto outputMuon = UnpackerFactory::get()->make("stage2::BMTF_ZSUnpackerOutput");
         auto inputMuonsOld = UnpackerFactory::get()->make("stage2::BMTF_ZSUnpackerInputsOldQual");
         auto inputMuonsNew = UnpackerFactory::get()->make("stage2::BMTF_ZSUnpackerInputsNewQual");
...
DEFINE_L1T_PACKING_SETUP(l1t::stage2::BMTFSetupZS);
