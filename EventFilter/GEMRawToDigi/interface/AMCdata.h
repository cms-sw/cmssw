...
namespace gem {
  ...
  union EventHeader {
    ...
    struct {
      ...
      uint64_t pType : 4;       // Payload type: can be one refering to different zero suppression schemes in normal data taking or calibration type. Note in calibration type the entire GCT (GEM Chamber Trailer) is skipped
  ...
  union EventTrailer {
    uint64_t word;
    struct {
      uint64_t BCL : 4;         // 1st bit, BC0 locked - If 0, this is a bad condition indicating a problem in the clock or TTC command stream (critical condition)
