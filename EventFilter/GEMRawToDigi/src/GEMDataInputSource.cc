...
// 
/**\class GEMDataInputSource GEMDataInputSource.cc IORawData/SiPixelInputSources/src/GEMDataInputSource.cc
...
#include "EventFilter/GEMRawToDigi/interface/GEMDataInputSource.h"
...
// constructor
GEMDataInputSource::GEMDataInputSource(const edm::ParameterSet& pset, 
...
bool GEMDataInputSource::setRunAndEventInfo(edm::EventID& id, edm::TimeValue_t& time, edm::EventAuxiliary::ExperimentType&) {
  ...
  do{
    ...
    if(m_currenteventnumber<eventnumber)
      LogError("GEMDataInputSource") << " error, the previous event number (" << eventnumber << ") is LARGER than the next event number (" << m_currenteventnumber << ")" << std::endl;
...
void GEMDataInputSource::produce(edm::Event& event) {
...
uint32_t GEMDataInputSource::synchronizeEvents(){
