...
public:
  ...
  void clustersFromZS(
      uint8_t const* data, int offset, int lenght, uint16_t stripOffset, Det const& det, OUT& out) const {
    constexpr int clusterNoiseCutFactor =
        4;                                  // 10^2/5^2 : 10 because noise is store *10; 5 because os a 5 sigma S/N cut
    ...
    while (is < lenght) {
      ...
      auto saveCluster = [&]() {
        ...
        if (extend)
          out.back().extend(adc, adc + clusSize);
        else {
      ...
      for (int ic = 0; ic < clusFedSize; ++ic) {
        ...
        if (stripNoiseCutFactor * ladc < noise)
          ladc = 0;
        else
