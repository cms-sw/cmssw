...
TopBottomClusterInfoProducer::produce(Event& iEvent, const EventSetup& iSetup)
{
	...
	const SiStripCluster * firstOffsetStripOld = & stripClustersOld->data().front();
        edmNew::DetSetVector<SiStripCluster>::const_iterator itdetOld = stripClustersOld->find(idStripNew);
	...
	edmNew::DetSetVector<SiPixelCluster>::const_iterator itdetOld = pixelClustersOld->find(idPixelNew); 
