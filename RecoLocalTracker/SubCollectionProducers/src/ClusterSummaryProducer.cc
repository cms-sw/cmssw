    ...
    verbose(iConfig.getParameter<bool>("verbose"))
{
  ...
  for(const auto& iS : wantedsubdets){
    ...
    selectors.push_back(ModuleSelection(DetIdSelector(ClusterSummary::getSubDetSelection(subdet)),subdet));
    if(subdet > maxEnum) maxEnum = subdet;
    if(verbose)moduleNames.push_back(ClusterSummary::getSubDetName(subdet));
  }
...
ClusterSummaryProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   ...
   auto getSelectons   =  [&] (const uint32_t& detid ){
   ...
   if(verbose){
     for(const auto& iS : selectors){
       ...
       std::cout << "n" << moduleNames[module]   <<", avg size, avg charge = "
...
ClusterSummaryProducer::beginStream(edm::StreamID)
{
  if(!verbose) return;
    ...
    for (const auto& iS : moduleNames ) {std::cout << iS<< " " ;}
    std::cout << std::endl;
}
