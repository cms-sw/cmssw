...
void TestPixTracks::beginRun(const edm::EventSetup &iSetup) { cout << "BeginRun, Verbosity =  " << PRINT << endl; }
...
void TestPixTracks::analyze(const edm::Event &e, const edm::EventSetup &es) {
  using namespace edm;
  ...
  for (reco::TrackCollection::const_iterator t = recTracks->begin(); t != recTracks->end(); ++t) {
    ...
    if (numberOfClusters > 0) {
      ...
      if (PRINT)
        cout << "Lay1: number of clusters per track = " << numOfClustersPerDet1 << endl;
      hclusPerTrk2->Fill(float(numOfClustersPerDet2));
      if (PRINT)
        cout << "Lay2: number of clusters per track = " << numOfClustersPerDet1 << endl;
      hclusPerTrk3->Fill(float(numOfClustersPerDet3));
      if (PRINT)
        cout << "Lay3: number of clusters per track = " << numOfClustersPerDet1 << endl;
    }
#endif  // HISTOS
