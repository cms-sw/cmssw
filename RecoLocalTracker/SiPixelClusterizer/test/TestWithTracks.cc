...
class TestWithTracks : public edm::EDAnalyzer {
 ...
 private:
  ...
  //TProfile *hpixcharld1,  *hpixcharld2,  *hpixcharld3;
...
void TestWithTracks::beginJob() {
  ...
  // profiles versus ladder
   ...
   //hpixcharld1 = fs->make<TProfile>("hpixcharld1","pix char layer 1",23,-11.5,11.5,0.0,1000.);
   //hpixcharld2 = fs->make<TProfile>("hpixcharld2","pix char layer 2",35,-17.5,17.5,0.0,1000.);
   //hpixcharld3 = fs->make<TProfile>("hpixcharld3","pix char layer 3",47,-23.5,23.5,0.0,1000.);
...
void TestWithTracks::analyze(const edm::Event& e, 
  ...
  for(reco::TrackCollection::const_iterator t=recTracks->begin();
    ...
    for ( trackingRecHit_iterator recHit = t->recHitsBegin();
      ...
      int ladderOn = pbn.ladderName();
      //layerOn  = pbn.layerName();
      int module = pbn.moduleName(); // 1 to 4
