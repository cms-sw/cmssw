      ...
      for (uint32_t module : cms::alpakatools::independent_groups(acc, lastModule)) {
        ...
        if (cms::alpakatools::once_per_block(acc)) {
          if (lastPixel - firstPixel > TrackerTraits::maxPixInModule) {
            ...
            // Drop them explicitly: otherwise they are seen as valid pixels with an out-of-range
            // cluster id by ClusterChargeCut and by SiPixelDigisClustersFromSoAAlpaka, which both
            // use the cluster id to index buffers of maxNumClustersPerModules entries.
