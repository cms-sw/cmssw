...
int16_t SiStripAPVRestorer::inspect( const uint32_t& detId,std::vector<int16_t>& digis) {
   ...
   detId_ = detId;
	...
	//std::cout << "STARTING GLOBAL INSPECT  ===================================================================================" << std::endl; 
    //std::cout << "=====================================================================================================" << std::endl; 
	//int i =0;
    //for(std::vector<int16_t>::iterator itDigis = digis.begin(); itDigis != digis.end(); ++itDigis, ++i) std::cout << "strip " << i << " adc " << *itDigis << std::endl;
...
void SiStripAPVRestorer::restore( std::vector<int16_t>& digis, const std::vector< std::pair<short,float> >& vmedians ) {
	...
	//std::cout << "ENDING GLOBAL RESTORE  ===================================================================================" << std::endl; 
    //std::cout << "=====================================================================================================" << std::endl; 
    //std::cout << "=====================================================================================================" << std::endl; 
...
//Inspect methods implementation ==========================================================================================================
//=========================================================================================================================================
template<typename T>
inline
int16_t SiStripAPVRestorer::BaselineFollowerInspect(std::vector<T>& digis){
  ...
  //std::cout << "STARTING BaselineFollowerInspect. Input digi size: " << digis.size() <<" ===================================================================================" << std::endl;  
  ...
  for( uint16_t APV=0; APV< digis.size()/128; ++APV){
	...
	if(DeltaCM < 0 && std::abs(DeltaCM) > DeltaCMThreshold_){
	    //std::cout << "BaselineFollowerInspect---Passed CM Condition (bad CM) APV: " << APV <<"  --------------------------- " << std::endl;
	    ...
	    //std::cout << "BaselineFollowerInspect---NOT Passed CM Condition (good CM) APV: " << APV <<" --------------------------- " << std::endl;
	    apvFlags_.push_back( "" );
    }	
...
//Restore methods implementation ==========================================================================================================
//=========================================================================================================================================
template<typename T>
inline
void SiStripAPVRestorer::BaselineFollowerRestore( std::vector<T>& digis, uint16_t APVn , uint16_t median){
	//std::cout << "STARTING BaselineFollowerRestore. Digi Size: " << digis.size() << " APV: " << APVn << " ===================================================================================" << std::endl;
	...
	//std::cout << "EXITING BaselineFollowerRestore===========================================================================================" << std::endl;
...
bool inline SiStripAPVRestorer::FlatRegionsFinder(std::vector<int16_t>& adcs, DigiMap& smoothedpoints, float median, uint16_t APVn ){
	...
	//std::cout << "FlatFinder Inspect----smoothedPointMapSize " << smoothedpoints.size() << " APV " << APVn << " ===============================" <<std::endl;
	//for(itSmoothedpoints = smoothedpoints.begin(); itSmoothedpoints != smoothedpoints.end(); ++itSmoothedpoints){
	//	std::cout << "FlatFinder Inspect----strip " << itSmoothedpoints->first << " smoothed adc " << itSmoothedpoints->second << std::endl;
	//}
	
    //std::cout << "FlatFinder Inspect----isFlat: " << (int)((MaxSmoothValue-MinSmoothValue) > distortionThreshold_) << " APV " << APVn << " ===============================" <<std::endl;	
...
void inline SiStripAPVRestorer::BaselineFollower(DigiMap& smoothedpoints, std::vector<int16_t>& baseline, float median){
	//std::cout << "BaselineFollower------START " << std::endl;	 
	baseline.clear();
	DigiMapIter itSmoothedpoints;
	
	//if not enough points
	if(smoothedpoints.size() < minStripsToFit_){
	     //std::cout << "BaselineFollower------not enough points : " << smoothedpoints.size() << " minStripsToFit: " << minStripsToFit_ << " --------------------------------- " << std::endl;
		baseline.clear();
		baseline.insert(baseline.begin(),128, median);
	} else {
	    //std::cout << "BaselineFollower------enough points : " << smoothedpoints.size() << " minStripsToFit: " << minStripsToFit_ << " --------------------------------- " << std::endl;  
	    baseline.insert(baseline.begin(),128, 0);  
		...
		//std::cout << "BaselineFollower------firstStripFlat: " << firstStripFlat<< " firstStipFlatADC " << firstStipFlatADC << " lastStripFlat " << lastStripFlat << " lastStipFlatADC "  << lastStipFlatADC << " --------------------------------- " << std::endl; 
		...
		for(itSmoothedpoints = itSmoothedpointsBegin; itSmoothedpoints != itSmoothedpointsEnd; ++itSmoothedpoints){  //IMPORTANT: the itSmoothedpointsEnd should be aat least smaller than smoothedpoints.end() -1
	...
	//std::cout << "Exiting Baseline follower--baseline size " << baseline.size() << " ------------------------------------------" << std::endl;
}


//CM subtracion methods implementation ==========================================================================================================
//=========================================================================================================================================
