	...
	while(deltaj > 0) {
	   for(j=jmin; j<=jmax; j+=deltaj) {
	      if(chi2ybin[j] < -100.) {
		     ...
		     rat=ssa/ss2;
		     if(rat <= 0.) {LOGERROR("SiPixelTemplateReco") << "illegal chi2ymin normalization (1) = " << rat << ENDL; rat = 1.;}
	...
	while(deltaj > 0) {
	   for(j=jmin; j<=jmax; j+=deltaj) {
	      if(chi2xbin[j] < -100.) {
		     ...
		     rat=ssa/ss2;
		     if(rat <= 0.) {LOGERROR("SiPixelTemplateReco") << "illegal chi2xmin normalization (1) = " << rat << ENDL; rat = 1.;}
