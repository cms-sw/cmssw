...
public:
  ...
  inline static float sigmaPitch(LocalPoint const &pos, LocalError const &err,
  ...
  template <typename MonoIterator, typename StereoIterator,
  ...
  void match(const SiStripRecHit2D *monoRH, RecHitIterator begin,
             RecHitIterator end, CollectorMatched &collector,
             const GluedGeomDet *gluedDet, LocalVector trackdirection) const;

  void match(const SiStripRecHit2D *monoRH, SimpleHitIterator begin,
             SimpleHitIterator end, CollectorMatched &collector,
             const GluedGeomDet *gluedDet, LocalVector trackdirection) const;

  // project strip coordinates on Glueddet

  StripPosition project(const GeomDetUnit *det, const GluedGeomDet *glueddet,
  ...
  void match(const SiStripRecHit2D *monoRH, SimpleHitIterator begin,
             SimpleHitIterator end, Collector &collector,
             const GluedGeomDet *gluedDet, LocalVector trackdirection) const;
...
template <typename MonoIterator, typename StereoIterator,
          typename CollectorHelper>
void SiStripRecHitMatcher::doubleMatch(
    MonoIterator monoRHiter, MonoIterator monoRHend, StereoIterator seconditer,
    StereoIterator seconditerend, const GluedGeomDet *gluedDet,
    LocalVector trdir, CollectorHelper &collectorHelper) const {
  ...
  // FIXME we shall find a faster approximation for trdir: not useful to compute
  // it each time for each strip
  ...
  for (; seconditer != seconditerend; ++seconditer) {
    ...
    float sigmap22 = sigmaPitch(secondHit.localPosition(),
                                secondHit.localPositionError(), partnertopol);
    // assert (sigmap22>=0);

    auto STEREOpointX =
        partnertopol.measurementPosition(secondHit.localPositionFast()).x();
    ...
    // in case of no track hypothesis assume a track from the origin through the
    // center of the strip
    if (notk) {
      const LocalPoint &lcenterofstrip = secondHit.localPositionFast();
      GlobalPoint gcenterofstrip =
          partnerStripDetTrans.toGlobal(lcenterofstrip);
  ...
  for (; monoRHiter != monoRHend; ++monoRHiter) {
    ...
    // (mono det)
    // StripPosition
    // stripmono=StripPosition(topol.localPosition(RPHIpointini),topol.localPosition(RPHIpointend));
    ...
    double m00 = -(
        projend[1] -
        projini
            [1]); //-(projectedstripmono.second.y()-projectedstripmono.first.y());
    double m01 =
        (projend[0] -
         projini
             [0]); // (projectedstripmono.second.x()-projectedstripmono.first.x());
    double c0 = m01 * projini[1] +
                m00 * projini[0]; // m01*projectedstripmono.first.y()   +
                                  // m00*projectedstripmono.first.x();
    ...
    for (int i = 0; i != cacheSize; ++i) {
      ...
      Vec3F result = invdet2simd * (si.sigmap22 * cslsimd +
                                    sigmap12 * ccssimd * csssimd * float(l2));
      ...
      if ((gluedDet->surface())
        ...
        collectorHelper.collector()(SiStripMatchedRecHit2D(
            LocalPoint(position), error, *gluedDet, &monoRH, si.secondHit));
      }
