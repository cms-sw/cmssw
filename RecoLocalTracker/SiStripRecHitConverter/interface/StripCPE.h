...
public:
  ...
  localParameters(const SiStripCluster &cl, const GeomDetUnit &) const override;

  StripCPE(edm::ParameterSet &conf, const MagneticField &,
           const TrackerGeometry &, const SiStripLorentzAngle &,
           const SiStripBackPlaneCorrection &, const SiStripConfObject &,
  ...
  AlgoParam getAlgoParam(const GeomDetUnit &det,
                         const LocalTrajectoryParameters &ltp) const {

    StripCPE::Param const &p = param(det);
    SiStripDetId::SubDetector loc =
        SiStripDetId(det.geographicalId()).subDetector();

    LocalVector track = ltp.directionNotNormalized();
    track *= -p.thickness;

    const float fullProjection =
        p.coveredStrips(track + p.drift, ltp.position());
