...
SiStripRecHitConverterAlgorithm::SiStripRecHitConverterAlgorithm(
    ...
    : useQuality(conf.getParameter<bool>("useSiStripQuality")),
      maskBad128StripBlocks(conf.existsAs<bool>("MaskBadAPVFibers") &&
                            conf.getParameter<bool>("MaskBadAPVFibers")),
      tracker_cache_id(0), cpe_cache_id(0), quality_cache_id(0),
...
void SiStripRecHitConverterAlgorithm::run(
    edm::Handle<edmNew::DetSetVector<SiStripCluster>> input, products &output) {
  run(input, output, LocalVector(0., 0., 0.));
}

void SiStripRecHitConverterAlgorithm::run(
    edm::Handle<edmNew::DetSetVector<SiStripCluster>> inputhandle,
    products &output, LocalVector trackdirection) {
  ...
  for (auto const &DS : *inputhandle) {
    ...
    for (auto const &cluster : DS) {
      ...
      StripClusterParameterEstimator::LocalValues parameters =
          parameterestimator->localParameters(cluster, du);
      collector.push_back(SiStripRecHit2D(parameters.first, parameters.second,
                                          du,
...
struct CollectorHelper {
  ...
  static inline SiStripRecHit2D const &
  stereoHit(edmNew::DetSet<SiStripRecHit2D>::const_iterator iter) {
    return *iter;
  }

  static inline SiStripRecHit2D const &
  monoHit(edmNew::DetSet<SiStripRecHit2D>::const_iterator iter) {
    return *iter;
  }
  ...
  struct Add {
    ...
    CollectorHelper &h;
    void operator()(SiStripMatchedRecHit2D const &rh) {
  ...
  void operator()(SiStripMatchedRecHit2D const &rh) {
    m_collectorMatched.push_back(rh);
  }

  CollectorHelper(Collector &i_collector, CollectorMatched &i_collectorMatched,
                  SiStripRecHit2DCollection::FastFiller &i_fillerRphiUnm,
                  std::vector<SiStripRecHit2D::ClusterRef::key_type>
                      &i_matchedSteroClusters)
      : nmatch(0), m_collector(i_collector),
  ...
  void closure(edmNew::DetSet<SiStripRecHit2D>::const_iterator it) {
    if (!m_collectorMatched.empty()) {
      ...
      for (edm::OwnVector<SiStripMatchedRecHit2D>::const_iterator
               itm = m_collectorMatched.begin(),
               edm = m_collectorMatched.end();
           itm != edm; ++itm) {
        m_collector.push_back(*itm);
        // mark the stereo hit cluster as used, so that the hit won't go in the
        // unmatched stereo ones
...
void SiStripRecHitConverterAlgorithm::match(products &output,
  ...
  edm::OwnVector<SiStripMatchedRecHit2D>
      collectorMatched; // gp/FIXME: avoid this
  ...
  // two work vectors for bookeeping clusters used by the stereo part of the
  // matched hits
  std::vector<SiStripRecHit2D::ClusterRef::key_type> matchedSteroClusters;

  for (SiStripRecHit2DCollection::const_iterator itRPhiDet =
           output.rphi->begin();
       itRPhiDet != edRPhiDet; ++itRPhiDet) {
    ...
    if (partnerId == 0) {
      ...
      if (!rphiHits.empty()) {
        SiStripRecHit2DCollection::FastFiller filler(*output.rphiUnmatched,
                                                     rphiHits.detId());
    ...
    SiStripRecHit2DCollection::const_iterator itStereoDet =
        output.stereo->find(partnerId);
    ...
    const GluedGeomDet *gluedDet =
        (const GluedGeomDet *)tracker->idToDet(DetId(specDetId.glued()));
    typedef SiStripMatchedRecHit2DCollection::FastFiller Collector;
    Collector collector(*output.matched, specDetId.glued());

    // Prepare also the list for unmatched rphi hits
    SiStripRecHit2DCollection::FastFiller fillerRphiUnm(*output.rphiUnmatched,
                                                        rphiHits.detId());

    // a list of clusters used by the matched part of the stereo hits in this
    // detector
    matchedSteroClusters.clear(); // at the beginning, empty

#ifdef DOUBLE_MATCH
    CollectorHelper chelper(collector, collectorMatched, fillerRphiUnm,
                            matchedSteroClusters);
    matcher->doubleMatch(rphiHits.begin(), rphiHits.end(), stereoHits.begin(),
                         stereoHits.end(), gluedDet, trackdirection, chelper);
    ...
    for (edmNew::DetSet<SiStripRecHit2D>::const_iterator
             it = stereoHits.begin(),
             ed = stereoHits.end();
         it != ed; ++it) {
    ...
    for (edmNew::DetSet<SiStripRecHit2D>::const_iterator it = rphiHits.begin(),
                                                         ed = rphiHits.end();
         it != ed; ++it) {
      matcher->match(&(*it), stereoSimpleHits.begin(), stereoSimpleHits.end(),
                     collectorMatched, gluedDet, trackdirection);
      if (collectorMatched.size() > 0) {
        nmatch += collectorMatched.size();
        for (edm::OwnVector<SiStripMatchedRecHit2D>::const_iterator
                 itm = collectorMatched.begin(),
                 edm = collectorMatched.end();
             itm != edm; ++itm) {
          collector.push_back(*itm);
          // mark the stereo hit cluster as used, so that the hit won't go in
          // the unmatched stereo ones
    ...
    SiStripRecHit2DCollection::FastFiller fillerStereoUnm(
        *output.stereoUnmatched, stereoHits.detId());
    std::sort(matchedSteroClusters.begin(), matchedSteroClusters.end());
    for (edmNew::DetSet<SiStripRecHit2D>::const_iterator
             it = stereoHits.begin(),
             ed = stereoHits.end();
         it != ed; ++it) {
      if (!std::binary_search(matchedSteroClusters.begin(),
                              matchedSteroClusters.end(),
                              it->cluster().key())) {
  ...
  for (SiStripRecHit2DCollection::const_iterator itStereoDet =
           output.stereo->begin();
       itStereoDet != edStereoDet; ++itStereoDet) {
    edmNew::DetSet<SiStripRecHit2D> stereoHits = *itStereoDet;
    StripSubdetector specDetId(stereoHits.detId());
    uint32_t partnerId = specDetId.partnerDetId();
    if (partnerId == 0)
      continue;
    SiStripRecHit2DCollection::const_iterator itRPhiDet =
        output.rphi->find(partnerId);
    if (itRPhiDet == edRPhiDet) {
      if (!stereoHits.empty()) {
        SiStripRecHit2DCollection::FastFiller filler(*output.stereoUnmatched,
                                                     stereoHits.detId());
...
void SiStripRecHitConverterAlgorithm::fillBad128StripBlocks(
    const uint32_t detid, bool bad128StripBlocks[6]) const {
...
inline bool
SiStripRecHitConverterAlgorithm::isMasked(const SiStripCluster &cluster,
                                          bool bad128StripBlocks[6]) const {
  if (maskBad128StripBlocks) {
    if (bad128StripBlocks[cluster.firstStrip() >> 7]) {
      if (bad128StripBlocks
              [(cluster.firstStrip() + cluster.amplitudes().size()) >> 7] ||
          bad128StripBlocks
              [static_cast<int32_t>(cluster.barycenter() - 0.499999) >> 7]) {
        return true;
      }
    } else {
      if (bad128StripBlocks
...
inline bool
SiStripRecHitConverterAlgorithm::useModule(const uint32_t id) const {
  const StripGeomDetUnit *stripdet =
      (const StripGeomDetUnit *)tracker->idToDetUnit(id);
  if (stripdet == nullptr)
