...
InverseCrosstalkMatrix::unfold(const SiStripCluster &clus, const float x) {
...
InverseCrosstalkMatrix::InverseCrosstalkMatrix(const unsigned N, const float x)
    : N(x > 0 ? N : 0), sq(sqrt(-x * 4 + 1)), lambdaP(1 + (1 + sq) / (-x * 2)),
      lambdaM(1 + (1 - sq) / (-x * 2)),
      denominator(sq * (pow(lambdaP, N + 1) - pow(lambdaM, N + 1))) {}

float InverseCrosstalkMatrix::operator()(const unsigned i,
                                         const unsigned j) const {
  return N == 0 || edm::isNotFinite(denominator)
             ? i == j
             : i >= j ? element(i, j) : element(j, i);
}

inline float InverseCrosstalkMatrix::element(const unsigned i,
                                             const unsigned j) const {
  return (pow(lambdaM, N + 1 - i) - pow(lambdaP, N + 1 - i)) *
         (pow(lambdaM, j) - pow(lambdaP, j)) / denominator;
}
