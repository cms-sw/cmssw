...
SiStripRecHitMatcher::SiStripRecHitMatcher(const edm::ParameterSet &conf)
    : scale_(conf.getParameter<double>("NSigmaInside")),
      preFilter_(conf.existsAs<bool>("PreFilter")
                     ? conf.getParameter<bool>("PreFilter")
                     : false) {}
...
namespace {
// FIXME for c++0X
inline void pb1(std::vector<SiStripMatchedRecHit2D *> &v,
                SiStripMatchedRecHit2D *h) {
  v.push_back(h);
}
inline void pb2(SiStripRecHitMatcher::CollectorMatched &v,
                const SiStripMatchedRecHit2D &h) {
  v.push_back(h);
}
...
void SiStripRecHitMatcher::match(
    const SiStripRecHit2D *monoRH, SimpleHitIterator begin,
    SimpleHitIterator end, edm::OwnVector<SiStripMatchedRecHit2D> &collector,
    const GluedGeomDet *gluedDet, LocalVector trackdirection) const {

  std::vector<SiStripMatchedRecHit2D *> result;
  result.reserve(end - begin);
  match(monoRH, begin, end, result, gluedDet, trackdirection);
  for (std::vector<SiStripMatchedRecHit2D *>::iterator p = result.begin();
       p != result.end(); p++)
...
void SiStripRecHitMatcher::match(
    const SiStripRecHit2D *monoRH, SimpleHitIterator begin,
    SimpleHitIterator end, std::vector<SiStripMatchedRecHit2D *> &collector,
    const GluedGeomDet *gluedDet, LocalVector trackdirection) const {
  Collector result(
      boost::bind(&pb1, boost::ref(collector),
...
void SiStripRecHitMatcher::match(const SiStripRecHit2D *monoRH,
  ...
  return match(monoRH, stereoHits.begin(), stereoHits.end(), collector,
...
void SiStripRecHitMatcher::match(const SiStripRecHit2D *monoRH,
  ...
  // position of the initial and final point of the strip (RPHI cluster) in
  // local strip coordinates
  double RPHIpointX =
      topol.measurementPosition(monoRH->localPositionFast()).x();
  MeasurementPoint RPHIpointini(RPHIpointX, -0.5);
  MeasurementPoint RPHIpointend(RPHIpointX, 0.5);

  // position of the initial and final point of the strip in local coordinates
  // (mono det)
  StripPosition stripmono = StripPosition(topol.localPosition(RPHIpointini),
                                          topol.localPosition(RPHIpointend));

  if (trackdirection.mag2() <
      FLT_MIN) { // in case of no track hypothesis assume a track from the
                 // origin through the center of the strip
  ...
  StripPosition projectedstripmono =
      project(stripdet, gluedDet, stripmono, trackdirection);
  const StripTopology &partnertopol =
      (const StripTopology &)partnerstripdet->topology();
  ...
  double c0 =
      m01 * projectedstripmono.first.y() + m00 * projectedstripmono.first.x();

  // error calculation (the part that depends on mono RH only)
  //  LocalVector
  //  RPHIpositiononGluedendvector=projectedstripmono.second-projectedstripmono.first;
  /*
  ...
  */
  ...
  for (seconditer = begin; seconditer != end;
       ++seconditer) { // iterate on stereo rechits

    // position of the initial and final point of the strip (STEREO cluster)
    double STEREOpointX =
        partnertopol.measurementPosition((*seconditer)->localPositionFast())
            .x();
    ...
    // position of the initial and final point of the strip in local coordinates
    // (stereo det)
    StripPosition stripstereo(partnertopol.localPosition(STEREOpointini),
                              partnertopol.localPosition(STEREOpointend));

    // project stereo hit on glued det
    StripPosition projectedstripstereo =
        project(partnerstripdet, gluedDet, stripstereo, trackdirection);

    double m10 =
        -(projectedstripstereo.second.y() - projectedstripstereo.first.y());
    double m11 =
        (projectedstripstereo.second.x() - projectedstripstereo.first.x());

    // perform the matching
    //(x2-x1)(y-y1)=(y2-y1)(x-x1)
    AlgebraicMatrix22 m;
    AlgebraicVector2 c; // FIXME understand why moving this initializer out of
                        // the loop changes the output!
    m(0, 0) = m00;
    m(0, 1) = m01;
    m(1, 0) = m10;
    m(1, 1) = m11;
    c(0) = c0;
    c(1) = m11 * projectedstripstereo.first.y() +
           m10 * projectedstripstereo.first.x();
    m.Invert();
    ...
    {
      double m00 =
    -(projectedstripmono.second.y()-projectedstripmono.first.y()); double m01 =
    (projectedstripmono.second.x()-projectedstripmono.first.x()); double m10 =
    -(projectedstripstereo.second.y()-projectedstripstereo.first.y()); double
    m11 =  (projectedstripstereo.second.x()-projectedstripstereo.first.x());
      double c0  =  m01*projectedstripmono.first.y()   +
    m00*projectedstripmono.first.x(); double c1  =
    m11*projectedstripstereo.first.y() + m10*projectedstripstereo.first.x();
    ...
    */
    ...
    /*
    LocalVector
    stereopositiononGluedendvector=projectedstripstereo.second-projectedstripstereo.first;
    ...
    */
    ...
    double sigmap22 =
        sigmaPitch((*seconditer)->localPosition(),
                   (*seconditer)->localPositionError(), partnertopol);
    ...
    if ((gluedDet->surface())
            .bounds()
            .inside(position, error,
                    scale_)) { // if it is inside the gluedet bonds
      ...
      collector(SiStripMatchedRecHit2D(position, error, *gluedDet, monoRH,
                                       secondHit));
    }
  }
}
...
SiStripRecHitMatcher::project(const GeomDetUnit *det,
  ...
  LocalPoint projpositiononGluedini =
      positiononGluedini + scale * trackdirection;
  LocalPoint projpositiononGluedend =
      positiononGluedend + scale * trackdirection;
...
SiStripRecHitMatcher::match(const SiStripRecHit2D *monoRH,
  ...
  if (trackdirection.mag2() <
      float(FLT_MIN)) { // in case of no track hypothesis assume a track from
                        // the origin through the center of the strip
    const LocalPoint &lcenterofstrip = monoRH->localPositionFast();
    GlobalPoint gcenterofstrip = (stripdet->surface()).toGlobal(lcenterofstrip);
    GlobalVector gtrackdirection = gcenterofstrip - GlobalPoint(0, 0, 0);
    trackdirection = (gluedDet->surface()).toLocal(gtrackdirection);
  ...
  */
  ...
  // position of the initial and final point of the strip (STEREO cluster)
  auto STEREOpointX =
      partnertopol.measurementPosition(stereoRH->localPositionFast()).x();
  ...
  AlgebraicVector2 c(c0, m11 * projectedstripstereo.first.y() +
                             m10 * projectedstripstereo.first.x());
  m(0, 0) = m00;
  m(0, 1) = m01;
  m(1, 0) = m10;
  ...
  if ((!force) &&
      (!((gluedDet->surface()).bounds().inside(position, 10.f * scale_))))
  ...
  double sigmap22 = sigmaPitch(stereoRH->localPosition(),
                               stereoRH->localPositionError(), partnertopol);
  ...
  if (force || (gluedDet->surface()).bounds().inside(position, error, scale_))
    return std::make_unique<SiStripMatchedRecHit2D>(
