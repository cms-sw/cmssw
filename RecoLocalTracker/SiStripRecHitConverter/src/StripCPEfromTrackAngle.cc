										     ...
										     const LocalTrajectoryParameters& ltp) const 
{
  ...
  //cout << "StripClusterParameterEstimator::LocalValues StripCPEfromTrackAngle::localParameters(...) " << endl;
  ...
  LocalPoint lp = p.topology->localPosition( strip,        ltp.vector() );
  LocalError le = p.topology->localError   ( strip, uerr2, ltp.vector() );

  StripClusterParameterEstimator::LocalValues lv = std::make_pair( lp, le );
