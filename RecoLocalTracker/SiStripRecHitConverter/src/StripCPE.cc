...
StripCPE::StripCPE(edm::ParameterSet &conf, const MagneticField &mag,
                   const TrackerGeometry &geom,
                   const SiStripLorentzAngle &LorentzAngle,
                   const SiStripBackPlaneCorrection &BackPlaneCorrection,
                   const SiStripConfObject &confObj,
                   const SiStripLatency &latency)
    : peakMode_(latency.singleReadOutMode() == 1), geom_(geom), magfield_(mag),
  ...
  for (map_t::const_iterator it = modules.begin(); it != modules.end(); it++) {
    const std::string modeS(peakMode_ ? "Peak" : "Deco"),
...
StripCPE::localParameters(const SiStripCluster &cluster,
  ...
  const float strip =
      barycenter - 0.5f * (1.f - p.backplanecorrection) * fullProjection;
...
LocalVector StripCPE::driftDirection(const StripGeomDetUnit *det) const {
  LocalVector lbfield =
      (det->surface()).toLocal(magfield_.inTesla(det->surface().position()));

  float tanLorentzAnglePerTesla =
      LorentzAngleMap_.getLorentzAngle(det->geographicalId().rawId());
...
void StripCPE::fillParams() {
  ...
  for (unsigned int i = 1; i < 7; ++i) {
    LogDebug("LookingForFirstStrip")
        << "Subdetector " << i << " GeomDetEnumerator "
        ...
        << (geom_.offsetDU(GeomDetEnumerators::tkDetEnum[i]) != dus.size()
                ? dus[geom_.offsetDU(GeomDetEnumerators::tkDetEnum[i])]
                      ->type()
                      .isTrackerStrip()
                : false);
  ...
  for (auto i = m_off; i != dus.size(); ++i) {
    ...
    p.maxLength = std::sqrt(std::pow(bounds.length(), 2.f) +
                            std::pow(bounds.width(), 2.f));
    ...
    p.backplanecorrection = BackPlaneCorrectionMap_.getBackPlaneCorrection(
        stripdet->geographicalId().rawId());

    const TkRadialStripTopology *rtop =
        dynamic_cast<const TkRadialStripTopology *>(
            &stripdet->specificType().specificTopology());
    p.pitch_rel_err2 =
        (rtop) ? pow(0.5f * rtop->angularWidth() * rtop->stripLength() /
                         rtop->localPitch(LocalPoint(0, 0, 0)),
                     2.f) /
                     12.f
               : 0.f;
  }
