...
//        fname1 << "EndCap_Side_" << side;
    ...
    for (int cls = 0; cls < 5; ++cls) {
      ...
      local_histos.globalPosXY[1][cls] = td.make< TH2F >(histoName.str().c_str(), histoName.str().c_str(), 500, -120., 120., 500, -120., 120.); 

      histoName.str(""); histoName << "Global_Position_XY_Strip" << tag.c_str() <<  id << clsstr.c_str();
      local_histos.globalPosXY[2][cls] = td.make< TH2F >(histoName.str().c_str(), histoName.str().c_str(), 500, -120., 120., 500, -120., 120.); 
