...
public:
  ...
  public:
    ...
    void error(const GeomDetUnit &gd) const {
      edm::LogError("FakeCPE")
          << "hit not found in det " << gd.geographicalId().rawId();
    }
    template <typename Cluster>
    void add(const Cluster &cluster, const GeomDetUnit &gd,
             LocalValues const &lv) {
      m_map[encode(cluster, gd)] = lv;
    }

    template <typename Cluster>
    LocalValues const &get(const Cluster &cluster,
                           const GeomDetUnit &gd) const {
    ...
    static uint64_t encode(const SiPixelCluster &cluster,
    ...
    static uint64_t encode(const SiStripCluster &cluster,
                           const GeomDetUnit &det) {
      uint64_t u1 = det.geographicalId().rawId();
