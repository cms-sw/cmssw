...
MTDTransientTrackingRecHitBuilder::MTDTransientTrackingRecHitBuilder(edm::ESHandle<GlobalTrackingGeometry> trackingGeometry):
  theTrackingGeometry(trackingGeometry)
{}


MTDTransientTrackingRecHitBuilder::RecHitPointer
MTDTransientTrackingRecHitBuilder::build (const TrackingRecHit* p, 
					   edm::ESHandle<GlobalTrackingGeometry> trackingGeometry) const {
  
  if ( p->geographicalId().det() == DetId::Forward &&
       p->geographicalId().subdetId() == FastTime) {
    return p->cloneSH();
  }
...
MTDTransientTrackingRecHitBuilder::RecHitPointer
MTDTransientTrackingRecHitBuilder::build(const TrackingRecHit * p) const {
...
MTDTransientTrackingRecHitBuilder::build(const trackingRecHit_iterator& start, const trackingRecHit_iterator& stop) const {
