...
/// Constructor
MTDNavigationSchool::MTDNavigationSchool(const MTDDetLayerGeometry* mtdLayout, bool enableBTL, bool enableETL)
...
void MTDNavigationSchool::linkEndcapLayers(const MapE& layers, vector<ETLNavigableLayer*>& result) {
  for (MapEI el = layers.begin(); el != layers.end(); el++) {
    ...
    result.push_back(new ETLNavigableLayer((*el).first, outerLayers, allOuterLayers));
  }
}
