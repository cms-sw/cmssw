...
MTDNavigationPrinter::MTDNavigationPrinter(const MTDDetLayerGeometry* mtdLayout,
  ...
  if (enableETL)
    backward = mtdLayout->allBackwardLayers();
  else
    backward = mtdLayout->allBackwardLayers();
  ...
  if (enableETL)
    forward = mtdLayout->allForwardLayers();
  else
    forward = mtdLayout->allForwardLayers();
...
void MTDNavigationPrinter::printLayer(const DetLayer* layer) const {
  ...
  if (const BarrelDetLayer* bdl = dynamic_cast<const BarrelDetLayer*>(layer)) {
    PRINT("MTDNavigationPrinter") << layer->location() << " " << layer->subDetector()
    ...
    PRINT("MTDNavigationPrinter") << endl
                                  ...
                                  << setprecision(2) << fdl->specificSurface().innerRadius() << "  outer r: " << setw(6)
                                  << setprecision(2) << fdl->specificSurface().outerRadius() << std::endl;
  }
  PRINT("MTDNavigationPrinter") << " has " << nextLayers.size()
  ...
  PRINT("MTDNavigationPrinter") << " has " << compatibleLayers.size()
...
void MTDNavigationPrinter::printLayers(const vector<const DetLayer*>& nextLayers) const {
  for (vector<const DetLayer*>::const_iterator inext = nextLayers.begin(); inext != nextLayers.end(); inext++) {
    ...
    if ((*inext)->location() == GeomDetEnumerators::barrel) {
      ...
      PRINT("MTDNavigationPrinter") << (*inext)->location() << " " << (*inext)->subDetector()
                                    << " layer at R: " << setiosflags(ios::showpoint | ios::fixed) << setw(8)
    ...
    PRINT("MTDNavigationPrinter") << setiosflags(ios::showpoint | ios::fixed) << setprecision(1) << setw(6)
                                  << (*inext)->surface().bounds().length() << ", " << setw(6)
                                  << (*inext)->surface().bounds().width() << ", " << setw(4)
                                  << (*inext)->surface().bounds().thickness() << " : " << (*inext)->surface().position()
                                  << std::endl;
  }
}
