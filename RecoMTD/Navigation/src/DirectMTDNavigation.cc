...
DirectMTDNavigation::DirectMTDNavigation(const edm::ESHandle<MTDDetLayerGeometry>& mtdLayout)
    : theMTDDetLayerGeometry(mtdLayout), epsilon_(100.), theEndcapFlag(true), theBarrelFlag(true) {}

DirectMTDNavigation::DirectMTDNavigation(const edm::ESHandle<MTDDetLayerGeometry>& mtdLayout,
                                         const edm::ParameterSet& par)
    : theMTDDetLayerGeometry(mtdLayout),
      epsilon_(100.),
...
void DirectMTDNavigation::inOutBarrel(const FreeTrajectoryState& fts, vector<const DetLayer*>& output) const {
  ...
  for (auto iter_B = barrel.begin(); iter_B != barrel.end(); iter_B++) {
    if (cont)
...
void DirectMTDNavigation::outInBarrel(const FreeTrajectoryState& fts, vector<const DetLayer*>& output) const {
  ...
  bool cont = false;
  auto rbegin = barrel.end();
  rbegin--;
  auto rend = barrel.begin();
  rend--;

  for (auto iter_B = rbegin; iter_B != rend; iter_B--) {
    if (cont)
      output.push_back((*iter_B));
    else if (checkCompatible(fts, dynamic_cast<const BarrelDetLayer*>(*iter_B))) {
      output.push_back((*iter_B));
...
void DirectMTDNavigation::inOutForward(const FreeTrajectoryState& fts, vector<const DetLayer*>& output) const {
  ...
  bool cont = false;
  for (auto iter_E = forward.begin(); iter_E != forward.end(); iter_E++) {
    if (cont)
...
void DirectMTDNavigation::outInForward(const FreeTrajectoryState& fts, vector<const DetLayer*>& output) const {
  ...
  bool cont = false;
  const vector<const DetLayer*>& forward = theMTDDetLayerGeometry->allForwardLayers();
  auto rbegin = forward.end();
  rbegin--;
  auto rend = forward.begin();
  rend--;
  for (auto iter_E = rbegin; iter_E != rend; iter_E--) {
    if (cont)
      output.push_back((*iter_E));
    else if (checkCompatible(fts, dynamic_cast<const ForwardDetLayer*>(*iter_E))) {
      output.push_back((*iter_E));
...
void DirectMTDNavigation::inOutBackward(const FreeTrajectoryState& fts, vector<const DetLayer*>& output) const {
  ...
  for (auto iter_E = backward.begin(); iter_E != backward.end(); iter_E++) {
    if (cont)
      output.push_back((*iter_E));
    else if (checkCompatible(fts, dynamic_cast<const ForwardDetLayer*>(*iter_E))) {
      output.push_back((*iter_E));
...
void DirectMTDNavigation::outInBackward(const FreeTrajectoryState& fts, vector<const DetLayer*>& output) const {
  ...
  auto rbegin = backward.end();
  rbegin--;
  auto rend = backward.begin();
  rend--;
  for (auto iter_E = rbegin; iter_E != rend; iter_E--) {
    if (cont)
      output.push_back((*iter_E));
