...
EcalPhaseIIDigiProducer::EcalPhaseIIDigiProducer( const edm::ParameterSet& params, edm::EDProducer& mixMod ) :
   ...
   m_EKdigiCollection ( params.getParameter<std::string>("EKdigiCollection") ) ,
   ...
   m_EBhitsProducerTag( params.getParameter<std::string>("EBhitsCollection") ) ,
   m_EEhitsProducerTag( params.getParameter<std::string>("EEhitsCollection") ) ,
   m_EKhitsProducerTag( params.getParameter<std::string>("EKhitsCollection") ) ,
   m_EShitsProducerTag( params.getParameter<std::string>("EShitsCollection") ) ,
   ...
   m_EKs25notCont     ( params.getParameter<double>     ("EKs25notContainment") ) ,
   ...
   m_EEResponse ( new EEHitResponse( m_ParameterMap,
				     &m_EEShape       ) ) ,
   m_EKResponse ( new EKHitResponse( m_ParameterMap,
				     &m_EKShape       ) ) ,
   ...
   theEKDets(NULL)
{
  if(m_apdSeparateDigi) mixMod.produces<EBDigiCollection>(m_apdDigiTag);
   ...
   mixMod.produces<EKDigiCollection>(m_EKdigiCollection);
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
   ...
   if( cosmicsPhase ) 
   {
      ...
      m_EKResponse->setPhaseShift( 1. + cosmicsShift ) ;
   }

   EcalCorrMatrix ebMatrix[ 3 ] ;
   EcalCorrMatrix eeMatrix[ 3 ] ;
   //   EcalCorrMatrix ekMatrix[ 3 ] ;
   ...
   // need shashlik implementation Shervin
   ...
   m_EndcapDigitizer = new EEDigitizer( m_EEResponse     ,
					m_ElectronicsSim , 
					addNoise            ) ;
   m_ShashlikDigitizer = new EKDigitizer( m_EKResponse     ,
					m_ElectronicsSim , 
					addNoise            ) ;
...
EcalPhaseIIDigiProducer::~EcalPhaseIIDigiProducer() 
{
   ...
   delete m_EKCorrNoise[0]       ; 
   delete m_EKCorrNoise[1]       ; 
   delete m_EKCorrNoise[2]       ; 
...
EcalPhaseIIDigiProducer::initializeEvent(edm::Event const& event, edm::EventSetup const& eventSetup) {
   ...
   m_ShashlikDigitizer->initializeHits();
//    if(m_doFastES) {
...
// Shervin
void
EcalPhaseIIDigiProducer::accumulateCaloHits(HitsHandle const& ebHandle, HitsHandle const& eeHandle, HitsHandle const& ekHandle, HitsHandle const& esHandle, int bunchCrossing) {
  if(ebHandle.isValid()) {
...
EcalPhaseIIDigiProducer::accumulate(edm::Event const& e, edm::EventSetup const& eventSetup) {
  // Step A: Get Inputs
  edm::InputTag ebTag(m_hitsProducerTag, m_EBhitsProducerTag);
  edm::Handle<std::vector<PCaloHit> > ebHandle;
  e.getByLabel(ebTag, ebHandle);

  edm::InputTag eeTag(m_hitsProducerTag,  m_EEhitsProducerTag);
  edm::Handle<std::vector<PCaloHit> > eeHandle;
  e.getByLabel(eeTag, eeHandle);

  edm::InputTag ekTag(m_hitsProducerTag,  m_EKhitsProducerTag);
  edm::Handle<std::vector<PCaloHit> > ekHandle;
  e.getByLabel(ekTag, ekHandle);

  edm::InputTag esTag(m_hitsProducerTag,  m_EShitsProducerTag);
  edm::Handle<std::vector<PCaloHit> > esHandle;
  e.getByLabel(esTag, esHandle);

  
  accumulateCaloHits(ebHandle, eeHandle, ekHandle, esHandle, 0);
}
...
EcalPhaseIIDigiProducer::accumulate(PileUpEventPrincipal const& e, edm::EventSetup const& eventSetup) {
  ...
  accumulateCaloHits(ebHandle, eeHandle, ekHandle, esHandle, e.bunchCrossing());
}
...
EcalPhaseIIDigiProducer::finalizeEvent(edm::Event& event, edm::EventSetup const& eventSetup) {
   ...
   std::auto_ptr<EKDigiCollection> shashlikResult ( new EKDigiCollection() ) ;
...
EcalPhaseIIDigiProducer::checkCalibrations(const edm::Event& event, const edm::EventSetup& eventSetup ) 
{
   ...
   m_EKResponse->setEventTime(eventTimeValue);
   m_EKResponse->setLaserConstants(laser.product(), m_useLCcorrection);
   ...
   const double EKscale (
      (agc->getEKValue())*theGains[1]*(m_Coder->MAXADC)*m_EKs25notCont ) ;
   ...
   LogDebug("EcalDigi") << " GeV/ADC = " << agc->getEKValue() 
			<< "\n" << " notCont = " << m_EKs25notCont 
			<< "\n" << " saturation for EK = " << EKscale 
			<< ", " << m_EKs25notCont ;

   m_Coder->setFullScaleEnergy( EBscale , 
				EEscale   ) ; //fixme? Shervin
...
EcalPhaseIIDigiProducer::checkGeometry( const edm::EventSetup & eventSetup ) 
{
   ...
   //   eventSetup.get<ShashlikNumberingRecord>().get(topo);
   //   const ShashlikTopology topology = *topo;
   //   std::cout << "shashlik topology = " << topology.totalSuperModules() << "\t" << topology.totalModules();
}
