...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
  ...
  */
  ...
  HGCalDigiSoaFiller::HGCalDigiSoaFiller(const edm::ParameterSet& config)
      : digisCEET_(
            consumes<edm::SortedCollection<HGCalDataFrame> >(config.getParameter<edm::InputTag>("eeChannelDigis"))),
        digisCEHSiT_(
            consumes<edm::SortedCollection<HGCalDataFrame> >(config.getParameter<edm::InputTag>("cehsiChannelDigis"))),
        digisCEHSiPMT_(consumes<edm::SortedCollection<HGCalDataFrame> >(
            config.getParameter<edm::InputTag>("cehsipmChannelDigis"))),
  ...
  void HGCalDigiSoaFiller::produce(device::Event& iEvent, device::EventSetup const& iSetup) {
    ...
    for (auto d : digisCEE) {
      ...
      HGCalElectronicsId eleid(
          ::hgcal::mappingtools::getElectronicsIdForSiCell<ModuleInfo, CellInfo>(modules, cells, detid));
      if (eleid.raw() == 0) {
        nfail++;
        continue;
      }
      ...
      auto modcellidx =
          ::hgcal::mappingtools::getModuleCellIndicesForSiCell<ModuleInfo, CellInfo>(modules, cells, detid);
      uint32_t i = finaldigi_size;
      try {
        i = modidx.getIndexForModuleData(
            eleid.localFEDId(), eleid.captureBlock(), eleid.econdIdx(), eleid.econdeRx(), eleid.halfrocChannel());
        if (i >= finaldigi_size) {
