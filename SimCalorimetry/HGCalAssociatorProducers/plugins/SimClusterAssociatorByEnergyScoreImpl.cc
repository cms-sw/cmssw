...
hgcal::association SimClusterAssociatorByEnergyScoreImpl::makeConnections(
  ...
  for (unsigned int i = 0; i < nSimClusters; ++i) {
    ...
    for (unsigned int j = 0; j < layers_ * 2; ++j) {
      ...
      // 	//lcsInSimCluster[i][j].layerClusterIdToEnergyAndScore.reserve(nLayerClusters);
      // 	lcsInSimCluster[i][j].layerClusterIdToEnergyAndScore[lcId].first  = 0.f;
      // 	lcsInSimCluster[i][j].layerClusterIdToEnergyAndScore[lcId].second = 0.f;
      // }
    }
  }
  ...
  for (unsigned int lcId = 0; lcId < nLayerClusters; ++lcId) {
    if (mask_[lcId] != 0.) {
      LogDebug("SimClusterAssociatorByEnergyScoreImpl")
          << "Skipping cluster not belonging to mask " << lcId << std::endl;
      continue;
    }
  ...
  for (unsigned int lcId = 0; lcId < nLayerClusters; ++lcId) {
    ...
    for (unsigned int i = 0; i < numberOfHitsInLC; ++i) {
      ...
      bool hitWithNoSC = (detIdToSimClusterId_Map.find(rh_detid) == detIdToSimClusterId_Map.end());
...
hgcal::RecoToSimCollectionWithSimClusters SimClusterAssociatorByEnergyScoreImpl::associateRecoToSim(
    const edm::Handle<reco::CaloClusterCollection>& cCCH, const edm::Handle<SimClusterCollection>& sCCH) const {
...
hgcal::SimToRecoCollectionWithSimClusters SimClusterAssociatorByEnergyScoreImpl::associateSimToReco(
    const edm::Handle<reco::CaloClusterCollection>& cCCH, const edm::Handle<SimClusterCollection>& sCCH) const {
