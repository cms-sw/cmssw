...
void HGCHEbackSignalScalerAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for(size_t i=0; i<sizeof(hnames)/sizeof(TString); i++){
    ...
    for(int ilay=minLayer; ilay<=maxLayer; ilay++) {
      ...
      layerHistos_[ilay][hnames[i]+"_ieta"] = fs->make<TH1F>(hnameLay+"ieta",";i-#eta;"+htitles[i],nIeta,minIeta,maxIeta);
      layerHistos_[ilay][hnames[i]+"_iphi"] = fs->make<TH1F>(hnameLay+"iphi",";i-#phi;"+htitles[i],nIphi,minIphi,maxIphi);
    }
  }
