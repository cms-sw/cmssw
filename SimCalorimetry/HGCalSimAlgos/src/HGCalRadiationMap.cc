...
double HGCalRadiationMap::getDoseValue(const int subdet, const int layer, const double& radius, bool logVal) {
  ...
  double cellDoseLog10 = doseMap_[key].a_ 
    + doseMap_[key].b_ * r  + doseMap_[key].c_ * r2
    + doseMap_[key].d_ * r3 + doseMap_[key].e_ * r4;
...
double HGCalRadiationMap::getFluenceValue(const int subdet, const int layer, const double& radius, bool logVal) {
  ...
  double cellFluenceLog10 = doseMap_[key].f_ 
    + doseMap_[key].g_ * r  + doseMap_[key].h_ * r2 
    + doseMap_[key].i_ * r3 + doseMap_[key].j_ * r4;
...
std::map<std::pair<int, int>, HGCalRadiationMap::DoseParameters> HGCalRadiationMap::readDosePars(
  ...
  while (getline(infile, line)) {
    ...
    linestream >> subdet >> layer 
               >> dosePars.a_ >> dosePars.b_ >> dosePars.c_ >> dosePars.d_ >> dosePars.e_ >> dosePars.doff_
               >> dosePars.f_ >> dosePars.g_ >> dosePars.h_ >> dosePars.i_ >> dosePars.j_ >> dosePars.foff_;
