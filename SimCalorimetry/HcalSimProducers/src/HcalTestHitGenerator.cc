...
void HcalTestHitGenerator::getNoiseHits(std::vector<PCaloHit> & noiseHits)
{
  ...
  for(int i = 1; i <= 16; ++i)
  {
     ...
     PCaloHit hit(detId.rawId(), e/theBarrelSampling[i-1], 0., 0., 0);
     noiseHits.push_back(hit);
  }

  // ring 16 is special
  HcalDetId detId(HcalEndcap, 16, 1, 3);
  PCaloHit hit(detId.rawId(), e/theEndcapSampling[0], 0., 0., 0);
  noiseHits.push_back(hit);

  for(int i = 17; i <= 29; ++i)
  {
     HcalDetId detId(HcalEndcap, i, 1, 1);
     PCaloHit hit(detId.rawId(), e/theEndcapSampling[i-16], 0., 0., 0);
     noiseHits.push_back(hit);
  }
