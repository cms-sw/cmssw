...
: theCoderFactory(HcalCoderFactory::DB),
  theAlgo(&theCoderFactory,
	  ps.getParameter<bool>("peakFilter"),
...
void HcalTrigPrimDigiProducer::produce(edm::Event& e, const edm::EventSetup& eventSetup) {
  ...
  theCoderFactory.setTPGCoder(inputCoder.product());
  theCoderFactory.setCompressionLUTcoder(outTranscoder->getHcalCompressor().get());
