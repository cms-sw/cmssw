...
HcalHitAnalyzer::HcalHitAnalyzer(edm::ParameterSet const &conf)
    : simParameterMap_(conf), hbheFilter_(), hoFilter_(), hfFilter_(),
      zdcFilter_(), hbheAnalyzer_("HBHE", 1., &simParameterMap_, &hbheFilter_),
      ...
      hbheRecHitCollectionTag_(
          conf.getParameter<edm::InputTag>("hbheRecHitCollectionTag")),
      hoRecHitCollectionTag_(
          conf.getParameter<edm::InputTag>("hoRecHitCollectionTag")),
      hfRecHitCollectionTag_(
          conf.getParameter<edm::InputTag>("hfRecHitCollectionTag")) {}
...
void HcalHitAnalyzer::analyze(edm::Event const &e, edm::EventSetup const &c) {
  ...
  std::unique_ptr<MixCollection<PCaloHit>> hits(
      new MixCollection<PCaloHit>(cf.product()));
