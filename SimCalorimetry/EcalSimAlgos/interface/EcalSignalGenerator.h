...
public:
  ...
  EcalSignalGenerator(const edm::InputTag &inputTag,
                      ...
                      const double EBs25notCont, const double EEs25notCont,
                      const double peToABarrel, const double peToAEndcap)
      : EcalBaseSignalGenerator(), theEvent(nullptr),
        theEventPrincipal(nullptr), theInputTag(inputTag), tok_(t) {
  ...
  void initializeEvent(const edm::Event *event,
    ...
    m_maxEneEB =
        (agc->getEBValue()) * theDefaultGains[1] * MAXADC * m_EBs25notCont;
    m_maxEneEE =
        (agc->getEEValue()) * theDefaultGains[1] * MAXADC * m_EEs25notCont;
  ...
  virtual void fill(edm::ModuleCallingContext const *mcc) {
    ...
    if (theEvent) {
      if (theEvent->getByToken(tok_, pDigis)) {
        ...
        throw cms::Exception("EcalSignalGenerator")
    ...
    // else { std::cout << " NO digis for this input: " << theInputTag <<
    // std::endl;}
  }
...
private:
  ...
  CaloSamples
  samplesInPE(const DIGI &digi); // have to define this separately for ES
