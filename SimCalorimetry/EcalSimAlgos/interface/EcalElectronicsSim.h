...
public:
  ...
  EcalElectronicsSim(const EcalSimParameterMap *parameterMap, EcalCoder *coder,
                     bool applyConstantTerm, double rmsConstantTerm);

  ~EcalElectronicsSim();

  /// from EcalSamples to EcalDataFrame
  void analogToDigital(CLHEP::HepRandomEngine *, EcalSamples &clf,
                       EcalDataFrame &df) const;

  void newEvent() {}

  void setNoiseSignalGenerator(
      const CaloVNoiseSignalGenerator *noiseSignalGenerator) {
