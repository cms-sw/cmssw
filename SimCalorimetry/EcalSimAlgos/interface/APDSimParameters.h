...
public:
  APDSimParameters(bool addToBarrel, bool separateDigi, double simToPELow,
                   double simToPEHigh, double timeOffset, double timeOffWidth,
                   bool doPEStats, const std::string &digiTag,
                   const std::vector<double> &nonlParms)
      :

        m_addToBarrel(addToBarrel), m_separateDigi(separateDigi),
        m_simToPELow(simToPELow), m_simToPEHigh(simToPEHigh),
        m_timeOffset(timeOffset), m_timeOffWidth(fabs(timeOffWidth)),
        m_doPEStats(doPEStats), m_digiTag(digiTag), m_nonlParms(nonlParms) {}
