...
int main() {
  ...
  bool useDBShape = false; // for the purpose of testing the EcalShape class,
                           // doesn't need to (should not) fetch a shape from
                           // the DB but just to assume the testbeam one
  ...
  for (unsigned int i(0); i != 3; ++i) {
    ...
    const DetId id(0 == i || 2 == i ? (DetId)EBDetId(1, 1)
                                    : (DetId)EEDetId(1, 50, 1));

    const EcalShapeBase *theShape(
        0 == i ? (EcalShapeBase *)&theEBShape
               : (1 == i ? (EcalShapeBase *)&theEEShape
    ...
    const int csize = 500;
    TCanvas *showShape =
        new TCanvas("showShape", "showShape", 2 * csize, csize);
    /*

    //  const std::vector<double>& nt = theShape.getTimeTable();
    //  const std::vector<double>& ntd = theShape.getDerivTable();

      TH1F* shape1 = new TH1F("shape1","Tabulated Ecal MGPA
    shape",histsiz,0.,(float)(histsiz)); TH1F* deriv1 = new
    TH1F("deriv1","Tabulated Ecal MGPA derivative",histsiz,0.,(float)(histsiz));



      std::cout << "interpolated ECAL pulse shape and its derivative \n" <<
    std::endl; for ( unsigned int i = 0; i < histsiz; ++i )
      {
         ...
         std::cout << " time (ns) = " << std::fixed << std::setw(6) <<
    std::setprecision(2) << time + T0 + 0.1
                   << " shape = " << std::setw(11) << std::setprecision(8) <<
    myShape
                   << " derivative = " << std::setw(11) << std::setprecision(8)
    << myDeriv << std::endl;
      }
    ...
    */
    ...
    std::cout << "\n Maximum time from tabulated values = " << std::fixed
              << std::setw(6) << std::setprecision(2) << ToM << std::endl;

    std::cout << "\n Tzero from tabulated values        = " << std::fixed
              << std::setw(6) << std::setprecision(2) << T0 << std::endl;

    std::cout << "\n Rising time from tabulated values  = " << std::fixed
              << std::setw(6) << std::setprecision(2) << rT << std::endl;
    ...
    std::cout << "\n computed ECAL " << name
              ...
              << std::endl;
    const double tzero =
        rT - (parameterMap.simParameters(id).binOfMaximum() - 1.) * 25.;
    double x = tzero;
    ...
    for (unsigned int i(0); i != histsiz; ++i) {
      ...
      std::cout << " time (ns) = " << std::fixed << std::setw(6)
                << std::setprecision(2) << x - tzero
                << " shape = " << std::setw(11) << std::setprecision(5) << y
                << " derivative = " << std::setw(11) << std::setprecision(5)
                << dy << std::endl;
      x = x + 1. / (double)tconv;
    }
