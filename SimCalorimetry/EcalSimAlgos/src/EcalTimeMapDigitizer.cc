...
EcalTimeMapDigitizer::EcalTimeMapDigitizer(EcalSubdetector myDet)
    : m_subDet(myDet), m_geometry(nullptr) {
  ...
  // service\n" 	 "in the configuration file or remove the modules that
  // require it.";
  //    }
  ...
  for (unsigned int i(0); i != size; ++i) {
    //       m_vSam.emplace_back(CaloGenericDetId( detId.det(),
    //       detId.subdetId(), i ) ,
    // 			  m_maxBunch-m_minBunch+1, abs(m_minBunch) );
    m_vSam.emplace_back(
        TimeSamples(CaloGenericDetId(detId.det(), detId.subdetId(), i)));
  }

  edm::LogInfo("TimeDigiInfo")
      << "[EcalTimeDigitizer]::Subdetector " << m_subDet
      << "::Reserved size for time digis " << m_vSam.size();

#ifdef ecal_time_debug
  std::cout << "[EcalTimeDigitizer]::Subdetector " << m_subDet
            << "::Reserved size for time digis " << m_vSam.size() << std::endl;
#endif
}
...
void EcalTimeMapDigitizer::add(const std::vector<PCaloHit> &hits,
  ...
  if (bunchCrossing >= m_minBunch && bunchCrossing <= m_maxBunch) {
    ...
    for (std::vector<PCaloHit>::const_iterator it = hits.begin(),
      ...
      if ((*it).energy() <
          MIN_ENERGY_THRESHOLD) // apply a minimal cut on the hit energy
        continue;
...
#ifdef ecal_time_debug
      std::cout << (*it).id() << "\t" << (*it).depth() << "\t" << jitter << "\t"
                << (*it).energy() << "\t"
                << result.average_time[bunchCrossing - m_minBunch] << "\t"
                << result.nhits[bunchCrossing - m_minBunch] << "\t"
...
void EcalTimeMapDigitizer::blankOutUsedSamples() // blank out previously used
                                                 // elements
{
...
#ifdef ecal_time_debug
    for (unsigned int j(0); j != vSamAll(m_index[i])->time_average_capacity;
         ++j)
      std::cout << j << "\t" << vSamAll(m_index[i])->average_time[j] << "\t"
                << vSamAll(m_index[i])->nhits[j] << "\t"
...
#ifdef ecal_time_debug
  std::cout
      << "[EcalTimeMapDigitizer]::Finalizing hits and fill output collection"
      << std::endl;
#endif
...
#ifdef ecal_time_debug
    std::cout << "digi " << output.back().id().rawId() << "\t"
              << output.back().size();
    if (output.back().sampleOfInterest() > 0)
      std::cout << "\tBX0 time "
                << output.back().sample(output.back().sampleOfInterest())
                << std::endl;
    else
...
#ifdef ecal_time_debug
  std::cout << "[EcalTimeMapDigitizer]::Output collection size "
...
double EcalTimeMapDigitizer::timeOfFlight(const DetId &detId, int layer) const {
  ...
  GlobalPoint layerPos =
      (cellGeometry)
          ->getPosition(double(layer) +
                        0.5); // depth in mm in the middle of the layer position
...
const EcalTimeMapDigitizer::TimeSamples *EcalTimeMapDigitizer::
operator[](unsigned int i) const {
  return &m_vSam[i];
}

EcalTimeMapDigitizer::TimeSamples *EcalTimeMapDigitizer::
