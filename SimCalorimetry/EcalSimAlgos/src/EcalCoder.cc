...
EcalCoder::EcalCoder(bool addNoise, bool PreMix1,
...
void EcalCoder::encode(const EcalSamples &ecalSamples, EcalDataFrame &df,
  ...
  for (unsigned int igain(0); igain <= NGAINS; ++igain) {
    ...
    maxADC[igain] =
        ADCGAINSWITCH; // saturation at 4080 for middle and high gains x6 & x12
    if (igain == NGAINS)
  ...
  const Noisifier *noisy[3] = {
      (nullptr == m_eeCorrNoise[0] || EcalBarrel == detId.subdetId()
           ? m_ebCorrNoise[0]
           : m_eeCorrNoise[0]),
  ...
  //   std::cout << " intercal, LSBs, gains " << icalconst << " " << LSB[0] << "
  //   " << LSB[1] << " " << gains[0] << " " << gains[1] << " " << Emax <<
  //   std::endl;
  ...
  for (unsigned int i(0); i != csize; ++i) {
    ...
    do {
      ...
      //	 if( igain != gainId ) std::cout <<"$$$$ Gain switch from " <<
      // gainId
      //					 <<" to "<< igain << std::endl ;
      ...
      {
        noisy[igain - 1]->noisify(noiseframe[igain - 1], engine,
                                  &noisy[0]->vecgau());
      ...
      if (!m_PreMix1) {
        ...
        signal = asignal;
      } else { // Any changes made here must be reverse-engineered in
               // EcalSignalGenerator!
        ...
        if (igain == 1) {
          ...
          signal = asignal;
                   ...
                   3) { // bet that no pileup hit has an energy over Emax/2
          const double asignal(ecalSamples[i] / (LSB[2] * icalconst));
          signal = asignal;
        } else { // not sure we ever get here at gain=0, but hit wil be
                 // saturated anyway
          const double asignal(
              ecalSamples[i] /
              (LSB[3] * icalconst)); // just calculate something
          signal = asignal;
        }
        // old version
      ...
      //	 std::cout << " " << ecalSamples[i] << " " <<
      // noiseframe[igain-1][i] << std::endl;

      const int isignal(signal);
      const int tmpadc(signal - (double)isignal < 0.5 ? isignal : isignal + 1);
      // LogDebug("EcalCoder") << "DetId " << detId.rawId() << " gain " << igain
      // << " caloSample "
      //                       <<  ecalSamples[i] << " pededstal " <<
      //                       pedestals[igain]
      //                       << " noise " << widths[igain] << " conversion
      //                       factor " << LSB[igain]
      //                       << " result (ped,tmpadc)= " << ped << " " <<
      //                       tmpadc;
    ...
    // LogDebug("EcalCoder") << " Writing out frame " << i << " ADC = " << adc
    // << " gainId = " << gainId << " storeGainId = " << storeGainId ;
...
void EcalCoder::findPedestal(const DetId &detId, int gainId, double &ped,
                             double &width) const {
  /*
    ...
    edm::LogError("EcalCoder") << "Could not find pedestal for " <<
    detId.rawId() << " among the " << m_peds->m_pedestals.size(); } else {
  ...
  */
  ...
  LogDebug("EcalCoder") << "Pedestals for " << detId.rawId() << " gain range "
                        << gainId << " : \n"
                        << "Mean = " << ped << " rms = " << width;
}
...
void EcalCoder::findGains(const DetId &detId, double Gains[]) const {
  /*
    EcalGainRatios::EcalGainRatioMap::const_iterator
    grit=m_gainRatios->getMap().find(detId.rawId()); EcalMGPAGainRatio mgpa; if(
    grit!=m_gainRatios->getMap().end() ){ mgpa = grit->second; Gains[0] = 0.;
    Gains[3] = 1.;
    Gains[2] = mgpa.gain6Over1() ;
    Gains[1] = Gains[2]*(mgpa.gain12Over6()) ;
    LogDebug("EcalCoder") << "Gains for " << detId.rawId() << "\n" << " 1 = " <<
    Gains[1] << "\n" << " 2 = " << Gains[2] << "\n" << " 3 = " << Gains[3]; }
    else { edm::LogError("EcalCoder") << "Could not find gain ratios for " <<
    detId.rawId() << " among the " << m_gainRatios->getMap().size();
    }
  */
  ...
  if ((detId.subdetId() != EcalBarrel) && (detId.subdetId() != EcalEndcap)) {
    edm::LogError("EcalCoder")
        << "Could not find gain ratios for " << detId.rawId() << " among the "
...
void EcalCoder::findIntercalibConstant(const DetId &detId,
  ...
  if (icalit != icalMap.end()) {
    ...
    edm::LogError("EcalCoder")
        << "No intercalib const found for xtal " << detId.rawId()
