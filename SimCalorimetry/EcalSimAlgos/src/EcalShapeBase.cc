...
EcalShapeBase::EcalShapeBase(bool useDBShape)
    : m_useDBShape(useDBShape), m_firstIndexOverThreshold(0),
      m_firstTimeOverThreshold(0.0), m_indexOfMax(0), m_timeOfMax(0.0),
      m_thresh(0.0) {}
...
void EcalShapeBase::buildMe(const edm::EventSetup *evtSetup) {
  ...
  fillShape(time_interval, m_thresh, shapeArray,
            evtSetup); // pure virtual function, implementation may vary for
                       // EB/EE/APD ...
  m_arraySize = shapeArray.size(); // original data

  m_denseArraySize =
      10 * m_arraySize; // dense array with interpolation between data
  m_kNBinsPerNSec =
      (unsigned int)(10 / time_interval); // used to be an unsigned int = 10 in
                                          // < CMSSW10X, should work for time
                                          // intervals ~0.1, 0.2, 0.5, 1
  ...
  for (unsigned int denseIndex(0); denseIndex != m_denseArraySize;
       ++denseIndex) {
    ...
    if (0 == ibin || shapeArray.size() ==
                         1 + ibin) // cannot do quadratic interpolation at ends
    {
