...
template <>
CaloSamples
EcalSignalGenerator<EBDigitizerTraits>::samplesInPE(const DIGI &digi) {
  // calibration, for future reference:  (same block for all Ecal types)
  // EcalDetId cell = digi.id();
  //         const EcalCalibrations&
  //         calibrations=conditions->getEcalCalibrations(cell);
  ...
  //    std::cout << " intercal, LSBs, egains " << icalconst << " " << LSB[0] <<
  //    " " << LSB[1] << " " << gainRatios[0] << " " << gainRatios[1] << " " <<
  //    Emax << std::endl;
  ...
  for (int isample = 0; isample < digi.size(); ++isample) {
    ...
    if (gainId == 1) {
      result[isample] =
          float(digi[isample].adc()) / 1000. / peToA; // special coding
    } else if (gainId > 1) {
      result[isample] =
...
template <>
CaloSamples
EcalSignalGenerator<EEDigitizerTraits>::samplesInPE(const DIGI &digi) {
  // calibration, for future reference:  (same block for all Ecal types)
  // EcalDetId cell = digi.id();
  //         const EcalCalibrations&
  //         calibrations=conditions->getEcalCalibrations(cell);
...
template <>
CaloSamples
EcalSignalGenerator<ESDigitizerTraits>::samplesInPE(const DIGI &digi) {
  // calibration, for future reference:  (same block for all Ecal types)
  // EcalDetId cell = digi.id();
  //         const EcalCalibrations&
  //         calibrations=conditions->getEcalCalibrations(cell);
