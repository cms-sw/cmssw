...
EBHitResponse::EBHitResponse(const CaloVSimParameterMap *parameterMap,
                             const CaloVShape *shape, bool apdOnly,
    ...
    :
      ...
      m_apdOnly(apdOnly), m_apdPars(apdPars), m_apdShape(apdShape),
...
void EBHitResponse::putAPDSignal(const DetId &detId, double npe, double time) {
  ...
  const double tzero(apdShape()->timeToRise() - jitter -
...
double EBHitResponse::apdSignalAmplitude(const PCaloHit &hit,
  ...
  double npe(hit.energy() * (2 == iddepth ? apdParameters()->simToPELow()
...
void EBHitResponse::finalizeHits() {
  ...
  if (apdParameters()->addToBarrel() || m_apdOnly) {
    for (unsigned int i(0); i != bSize; ++i) {
      if (0 < m_apdNpeVec[i]) {
        putAPDSignal(EBDetId::detIdFromDenseIndex(i), m_apdNpeVec[i],
                     m_apdTimeVec[i]);
...
void EBHitResponse::run(MixCollection<PCaloHit> &hits,
  ...
  for (MixCollection<PCaloHit>::MixItr hitItr(hits.begin());
    ...
    if (minBunch() <= bunch && maxBunch() >= bunch &&
...
const EcalHitResponse::EcalSamples *EBHitResponse::
