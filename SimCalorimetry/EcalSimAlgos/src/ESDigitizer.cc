...
ESDigitizer::ESDigitizer(EcalHitResponse *hitResponse,
                         ESElectronicsSimFast *electronicsSim, bool addNoise)
    : EcalTDigitizer<ESDigitizerTraits>(hitResponse, electronicsSim, addNoise),
      m_detIds(nullptr), m_ranGeneral(nullptr), m_ESGain(0), m_histoBin(0),
      m_histoInf(0), m_histoWid(0), m_meanNoisy(0), m_trip() {
  m_trip.reserve(2500);
}
...
void ESDigitizer::setGain(const int gain) {
  if (0 != m_ESGain) {
    ...
    if (addNoise()) {
      ...
      if (status != 0)
        std::cerr << "ESDigitizer::could not compute gaussian tail probability "
                     "for the threshold chosen"
                  << std::endl;
      ...
      if (!histofile.good()) {
        ...
        m_ranGeneral = new CLHEP::RandGeneral(nullptr, &t_refHistos.front(),
                                              t_refHistos.size(), 0);
        histofile.close();
      }
    }
...
void ESDigitizer::run(ESDigiCollection &output,
  ...
  assert(nullptr != m_detIds && !m_detIds->empty() &&
         (!addNoise() || nullptr != m_ranGeneral)); // sanity check
  ...
  for (std::vector<DetId>::const_iterator idItr(abThreshCh.begin());
    ...
    {
      ESHitResponse::ESSamples analogSignal(*idItr,
                                            3); // space for the noise hit
      ...
      const_cast<ESElectronicsSimFast *>(elecSim())->analogToDigital(
          engine, analogSignal, digi, true);
