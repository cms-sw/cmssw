...
EcalHitResponse::EcalHitResponse(const CaloVSimParameterMap *parameterMap,
    ...
    : m_parameterMap(parameterMap), m_shape(shape), m_hitCorrection(nullptr),
      m_PECorrection(nullptr), m_hitFilter(nullptr), m_geometry(nullptr),
      m_lasercals(nullptr), m_minBunch(-32), m_maxBunch(10), m_phaseShift(1),
      m_iTime(0), m_useLCcorrection(false) {}
...
void EcalHitResponse::setHitCorrection(
    const CaloVHitCorrection *hitCorrection) {
...
void EcalHitResponse::blankOutUsedSamples() // blank out previously used
                                            // elements
{
...
void EcalHitResponse::putAnalogSignal(const PCaloHit &hit,
                                      CLHEP::HepRandomEngine *engine) {
  const DetId detId(hit.id());
...
double EcalHitResponse::analogSignalAmplitude(const DetId &detId, double energy,
                                              CLHEP::HepRandomEngine *engine) {
  const CaloSimParameters &parameters(*params(detId));
...
double EcalHitResponse::timeOfFlight(const DetId &detId) const {
  ...
  return cellGeometry->getPosition().mag() * cm /
         c_light; // Units of c_light: mm/ns
}
