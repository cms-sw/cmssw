...
ESElectronicsSimFast::ESElectronicsSimFast(bool addNoise, bool PreMix1)
    : m_addNoise(addNoise), m_PreMix1(PreMix1), m_MIPToGeV(0), m_peds(nullptr),
      m_mips(nullptr) {
...
void ESElectronicsSimFast::analogToDigital(CLHEP::HepRandomEngine *engine,
                                           ESSamples &cs, ESDataFrame &df,
                                           bool isNoise) const {
  assert(nullptr != m_peds && nullptr != m_mips &&
  ...
  for (unsigned int i(0); i != cs.size(); ++i) {
    const double noi(isNoise || (!m_addNoise)
                         ? 0
