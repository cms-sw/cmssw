...
void EcalFenixMaxof2::process(std::vector<std::vector<int>> &bypasslinout,
  ...
  for (int istrip = 0; istrip < nstrip; ++istrip){
    for (unsigned int i = 0; i < output.size(); i++){
      if ((bypasslinout[istrip][i] >> bitOddEven) & 1)
        strip_oddmask[istrip][i] = 0;
      else strip_oddmask[istrip][i] = 1;
    }
  }
  ...
  for (unsigned int i = 0; i < output.size(); i++) {
    if (nstrip - 1 == 0) {
      ...
      for (int i2strip = 0; i2strip < nstrip - 1; ++i2strip) {
        sumby2_[i2strip][i] =  strip_oddmask[i2strip][i]*((bypasslinout[i2strip][i]) & mask) + strip_oddmask[i2strip+1][i]*((bypasslinout[i2strip + 1][i]) & mask);
