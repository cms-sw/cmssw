...
HcalSiPMShape::HcalSiPMShape(unsigned int signalShape) : CaloVShape(), nBins_(HcalPulseShapes::nBinsSiPM_*2), nt_(nBins_, 0.) {
...
void HcalSiPMShape::computeShape(unsigned int signalShape) {
  ...
  else if(signalShape==HcalShapes::HE2017) analyticPulseShape = &HcalPulseShapes::analyticPulseShapeSiPMHO;
  else return;
