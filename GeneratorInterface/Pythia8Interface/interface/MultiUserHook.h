...
// Initialisation after beams have been set by Pythia::init().
  bool initAfterBeams() override {
    ...
    for (Pythia8::UserHooks *hook : hooks_) {
      hook->initPtr(infoPtr, settingsPtr,
          particleDataPtr, rndmPtr,
          beamAPtr, beamBPtr,
          beamPomAPtr, beamPomBPtr,
          coupSMPtr, partonSystemsPtr,
          sigmaTotPtr);
    ...
    int nCanEnhance = 0;
    for (Pythia8::UserHooks *hook : hooks_) {
      if (hook->canVetoPT()) ++nCanVetoPT;
      if (hook->canVetoStep()) ++nCanVetoStep;
      if (hook->canVetoMPIStep()) ++nCanVetoMPIStep;
      if (hook->canSetResonanceScale()) ++nCanSetResonanceScale;
      if (hook->canEnhanceEmission() || hook->canEnhanceTrial()) ++nCanEnhance;
    }
  ...
  double multiplySigmaBy(const Pythia8::SigmaProcess* sigmaProcessPtr,
    const Pythia8::PhaseSpace* phaseSpacePtr, bool inEvent) override {
      double factor = 1.;
      for (Pythia8::UserHooks *hook : hooks_) {
        if (hook->canModifySigma()) factor *= hook->multiplySigmaBy(sigmaProcessPtr,phaseSpacePtr,inEvent);
      }
      return factor;
  }
  ...
  double biasSelectionBy(const Pythia8::SigmaProcess* sigmaProcessPtr,
    const Pythia8::PhaseSpace* phaseSpacePtr, bool inEvent) override {
      double factor = 1.;
      for (Pythia8::UserHooks *hook : hooks_) {
        if (hook->canBiasSelection()) factor *= hook->biasSelectionBy(sigmaProcessPtr,phaseSpacePtr,inEvent);
      }
      return factor;
  };
  ...
  double biasedSelectionWeight() override {
    double factor = 1.;
    for (Pythia8::UserHooks *hook : hooks_) {
      if (hook->canBiasSelection()) factor *= hook->biasedSelectionWeight();
    }
    return factor;
  };
  ...
  bool doVetoProcessLevel(Pythia8::Event& event) override {
    bool test = false;
    for (Pythia8::UserHooks *hook : hooks_) {
      if (hook->canVetoProcessLevel()) test |= hook->doVetoProcessLevel(event);
    }
    return test;
  }
  ...
  bool doVetoResonanceDecays(Pythia8::Event& event) override {
    bool test = false;
    for (Pythia8::UserHooks *hook : hooks_) {
      if (hook->canVetoResonanceDecays()) test |= hook->doVetoResonanceDecays(event);
    }
    return test;
  }
  ...
  double scaleVetoPT() override {
    for (Pythia8::UserHooks *hook : hooks_) {
      if (hook->canVetoPT()) return hook->scaleVetoPT();
    }
    return 0.;
  };
  ...
  bool doVetoPT( int iPos, const Pythia8::Event& event) override {
    for (Pythia8::UserHooks *hook : hooks_) {
      if (hook->canVetoPT()) return hook->doVetoPT(iPos,event);
    }
    return false;
  }
  ...
  int numberVetoStep() override {
    for (Pythia8::UserHooks *hook : hooks_) {
      if (hook->canVetoStep()) return hook->numberVetoStep();
    }
    return 1;
  };
  ...
  bool doVetoStep( int iPos, int nISR, int nFSR, const Pythia8::Event& event) override {
    for (Pythia8::UserHooks *hook : hooks_) {
      if (hook->canVetoStep()) return hook->doVetoStep(iPos,nISR,nFSR,event);
    }
    return false;
  }
  ...
  int numberVetoMPIStep() override {
    for (Pythia8::UserHooks *hook : hooks_) {
      if (hook->canVetoMPIStep()) return hook->numberVetoMPIStep();
    }
    return 1;
  };
  ...
  bool doVetoMPIStep( int nMPI, const Pythia8::Event& event) override {
    for (Pythia8::UserHooks *hook : hooks_) {
      if (hook->canVetoMPIStep()) return hook->doVetoMPIStep(nMPI,event);
    }
    return false;
  }
  ...
  bool doVetoPartonLevelEarly( const Pythia8::Event& event) override {
    bool test = false;
    for (Pythia8::UserHooks *hook : hooks_) {
      if (hook->canVetoPartonLevelEarly()) test |= hook->doVetoPartonLevelEarly(event);
    }
    return test;
  }
  ...
  bool doVetoPartonLevel( const Pythia8::Event& event) override {
    bool test = false;
    for (Pythia8::UserHooks *hook : hooks_) {
      if (hook->canVetoPartonLevel()) test |= hook->doVetoPartonLevel(event);
    }
    return test;
  }
  ...
  double scaleResonance( int iRes, const Pythia8::Event& event) override {
    for (Pythia8::UserHooks *hook : hooks_) {
      if (hook->canSetResonanceScale()) return hook->scaleResonance(iRes,event);
    }
    return 0.;
  };
  ...
  bool doVetoISREmission( int sizeOld, const Pythia8::Event& event, int iSys) override {
    bool test = false;
    for (Pythia8::UserHooks *hook : hooks_) {
      if (hook->canVetoISREmission()) test |= hook->doVetoISREmission(sizeOld,event,iSys);
    }
    return test;
  }
  ...
  bool doVetoFSREmission( int sizeOld, const Pythia8::Event& event, int iSys, bool inResonance = false ) override {
    bool test = false;
    for (Pythia8::UserHooks *hook : hooks_) {
      if (hook->canVetoFSREmission()) test |= hook->doVetoFSREmission(sizeOld,event,iSys,inResonance);
    }
    return test;
  }
  ...
  bool doVetoMPIEmission( int sizeOld, const Pythia8::Event &event) override {
    bool test = false;
    for (Pythia8::UserHooks *hook : hooks_) {
      if (hook->canVetoMPIEmission()) test |= hook->doVetoMPIEmission(sizeOld,event);
    }
    return test;
  }
  ...
  bool doReconnectResonanceSystems( int oldSizeEvt, Pythia8::Event &event) override {
    bool test = true;
    for (Pythia8::UserHooks *hook : hooks_) {
      if (hook->canReconnectResonanceSystems()) test &= hook->doReconnectResonanceSystems(oldSizeEvt,event);
    }
    return test;
  }
  ...
  double enhanceFactor( std::string str) override {
    for (Pythia8::UserHooks *hook : hooks_) {
      if (hook->canEnhanceEmission() || hook->canEnhanceTrial()) return hook->enhanceFactor(str);
    }
    return 1.;
  };
  ...
  double vetoProbability( std::string str ) override {
    for (Pythia8::UserHooks *hook : hooks_) {
      if (hook->canEnhanceEmission() || hook->canEnhanceTrial()) return hook->vetoProbability(str);
    }
    return 0.;
  };
