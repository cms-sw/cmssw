...
double Suep_shower::f(double p) { return p * p * exp(-A * p * p / (1 + sqrt(1 + p * p))); }
...
double Suep_shower::test_fun(double p) { return log(f(p) / f(p_m)) + 1.0; }
...
vector<vector<double> > Suep_shower::generate_shower() {
  ...
  for (int n = 0; n < len; n++) {
    ...
    event[n][0] =
        sqrt(event[n][1] * event[n][1] + event[n][2] * event[n][2] + event[n][3] * event[n][3] + (this->m) * (this->m));
  }
