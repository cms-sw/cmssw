...
Pythia8Producer::Pythia8Producer( const ParameterSet & pset) :
  EDProducer(),
  ...
  eventNumber_(0)
{
  ...
  cout << "Pythia8Producer: initializing Pythia. " << endl;
...
//  ToHepMC->set_crash_on_problem();
  ...
  cout << "Pythia8Producer: starting event generation ... " << endl;

}


Pythia8Producer::~Pythia8Producer(){
  cout << "Pythia8Producer: event generation done. " << endl;
  pythia->statistics();
  delete pythia;
  delete ToHepMC;
  clear();
...
void Pythia8Producer::clear() {
 
}


void Pythia8Producer::endRun(Run & r, const EventSetup & es) {
...
void Pythia8Producer::produce(Event & e, const EventSetup & es) {
    ...
    // generate one event with Pythia8:
    if (!pythia->next()) cout << "failed to generate event" << endl;
...
//    hepmcevt->set_event_scale(pypars.pari[16]);
    ...
    ++eventNumber_;
    hepmcevt->set_event_number(eventNumber_);
...
//bool 
//Pythia8Producer::call_pygive(const std::string& iParm ) {
...
bool 
Pythia8Producer::call_txgive(const std::string& iParm ) {
...
bool 
Pythia8Producer::call_txgive_init() {
