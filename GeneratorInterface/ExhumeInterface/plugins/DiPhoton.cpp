...
////////////////////////////////////////////////////////////////////////////////
Exhume::DiPhoton::DiPhoton(const edm::ParameterSet& pset) : TwoSpace(pset) {
  std::cout << std::endl << "   =DiPhoton production selected=" << std::endl << std::endl;
  Partons[0].id = 22;
  Partons[1].id = 22;
  Partons[0].Colour = 0;
...
double Exhume::DiPhoton::MatrixElement() {
  ...
  double _Sigma = -0.5 * (0.5 * (1 + CosTheta * CosTheta)) *
                      (log((1.0 - CosTheta) / (1.0 + CosTheta)) * log((1.0 - CosTheta) / (1.0 + CosTheta)) + PI2) -
...
void Exhume::DiPhoton::LIPS2Amp() { return; }

void Exhume::DiPhoton::Amp2LIPS() { return; }
