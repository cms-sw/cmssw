...
HepMC::GenEvent* TauolappInterface::decay( HepMC::GenEvent* evt ){
   ...
   if(dolhe && lhe!=NULL){
     ...
     for(unsigned int i=0;i<spinup.size();i++){
       particles.emplace_back(HepMC::FourVector(lhe->getHEPEUP()->PUP.at(i)[0],lhe->getHEPEUP()->PUP.at(i)[1],lhe->getHEPEUP()->PUP.at(i)[2],lhe->getHEPEUP()->PUP.at(i)[3]),lhe->getHEPEUP()->IDUP.at(i));
