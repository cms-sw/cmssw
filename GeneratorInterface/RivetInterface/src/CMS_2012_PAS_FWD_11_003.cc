...
namespace Rivet {
  ...
  CMS_2012_PAS_FWD_11_003()
    ...
    double inclEflow_counter_09;
    double inclEflow_counter_276;
    double inclEflow_counter_7;
	...
	_tmp_jet_09  = bookHistogram1D("eflow_jet_09",binEdges(1,1,1)); // Leading jet energy flow in pt
	_tmp_njet_09 = bookHistogram1D("number_jet_09",binEdges(1,1,1)); // Number of events in pt
	
      }

      if(fuzzyEquals(sqrtS()/GeV, 2760, 1E-3)){

	_tmp_jet_276  = bookHistogram1D("eflow_jet_276",binEdges(2,1,1));
	_tmp_njet_276 = bookHistogram1D("number_jet_276",binEdges(2,1,1));
      ...
      if(fuzzyEquals(sqrtS()/GeV, 7000, 1E-3)){ 
        ...
        _tmp_jet_7  = bookHistogram1D("eflow_jet_7",binEdges(3,1,1));
        _tmp_njet_7 = bookHistogram1D("number_jet_7",binEdges(3,1,1));
	...
	vetoEvent;
	  ...
	  if(-5.2 > p.momentum().pseudorapidity() && p.momentum().pseudorapidity() > -6.6 ) inclEflow_counter_09 +=weight * p.momentum().E()/GeV;

       }

	if(fuzzyEquals(sqrtS()/GeV, 2760, 1E-3)) {

	  if(-5.2 > p.momentum().pseudorapidity() && p.momentum().pseudorapidity() > -6.6 ) inclEflow_counter_276 +=weight * p.momentum().E()/GeV;

	}

     	if(fuzzyEquals(sqrtS()/GeV, 7000, 1E-3)) {

	  if(-5.2 > p.momentum().pseudorapidity() && p.momentum().pseudorapidity() > -6.6 ) inclEflow_counter_7 +=weight * p.momentum().E()/GeV;
    ...
    void finalize() {  
      ...
      if(fuzzyEquals(sqrtS()/GeV, 900, 1E-3)){
        ...
        norm_incl_eflow_09 = inclEflow_counter_09/norm_incl; // no normalization to the binwidth
	_tmp_jet_09->scale(1.0/norm_incl_eflow_09);

	//the energy flow ratio
        hf.divide(histoDir() + "/d01-x01-y01", *_tmp_jet_09, *_tmp_njet_09); 
	...
	norm_incl_eflow_276 = inclEflow_counter_276/norm_incl;
	_tmp_jet_276->scale(1.0/norm_incl_eflow_276);

	//the energy flow ratio
        hf.divide(histoDir() + "/d02-x01-y01", *_tmp_jet_276, *_tmp_njet_276); 
	...
	norm_incl_eflow_7 = inclEflow_counter_7/norm_incl;
	_tmp_jet_7->scale(1.0/norm_incl_eflow_7);

	//the energy flow ratio
        hf.divide(histoDir() + "/d03-x01-y01", *_tmp_jet_7, *_tmp_njet_7); 
  ...
  AnalysisBuilder<CMS_2012_PAS_FWD_11_003> plugin_CMS_2012_PAS_FWD_11_003;
