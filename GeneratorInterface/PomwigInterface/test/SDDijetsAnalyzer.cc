...
struct jetptcomp {
  bool operator()(std::pair<reco::GenJetCollection::const_iterator, double> jet1,
                  std::pair<reco::GenJetCollection::const_iterator, double> jet2) {
...
void SDDijetsAnalyzer::endJob() { hEnergyvsEta->Scale(1 / (float)nevents); }
...
void SDDijetsAnalyzer::analyze(const edm::Event& ev, const edm::EventSetup&) {
  ...
  if (jet1 && jet2) {
    if (debug)
      std::cout << ">>> Leading Jet pt,eta: " << jet1->pt() << " , " << jet1->eta() << std::endl;
    hJet1Pt->Fill(jet1->pt());
    hJet1Eta->Fill(jet1->eta());
    hJet1Phi->Fill(jet1->phi());

    if (debug)
      std::cout << ">>> Second leading Jet pt,eta: " << jet2->pt() << " , " << jet2->eta() << std::endl;
