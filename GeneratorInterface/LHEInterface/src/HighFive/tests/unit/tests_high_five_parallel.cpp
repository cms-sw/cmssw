...
void selectionArraySimpleTestParallel() {
  ...
  File file(
      filename.str(), File::ReadWrite | File::Create | File::Truncate, MPIOFileDriver(MPI_COMM_WORLD, MPI_INFO_NULL));
...
BOOST_AUTO_TEST_CASE_TEMPLATE(selectionArraySimple, T, numerical_test_types) { selectionArraySimpleTestParallel<T>(); }
