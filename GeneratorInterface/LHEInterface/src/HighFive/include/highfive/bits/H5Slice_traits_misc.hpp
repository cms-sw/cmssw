...
namespace HighFive {
  ...
  namespace details {
    ...
    inline const DataSet& get_dataset(const Selection* ptr) { return ptr->getDataset(); }
    ...
    inline hid_t get_memspace_id(const Selection* ptr) { return ptr->getMemSpace().getId(); }
  ...
  inline Selection SliceTraits<Derivate>::select(const std::vector<size_t>& offset,
    ...
    if (H5Sselect_hyperslab(space.getId(),
                            H5S_SELECT_SET,
                            offset_local.data(),
                            stride.empty() ? nullptr : stride_local.data(),
                            count_local.data(),
                            nullptr) < 0) {
  ...
  inline Selection SliceTraits<Derivate>::select(const std::vector<size_t>& columns) const {
    ...
    for (std::vector<size_t>::const_iterator i = columns.begin(); i != columns.end(); ++i) {
      offsets[offsets.size() - 1] = *i;
      if (H5Sselect_hyperslab(space.getId(), H5S_SELECT_OR, offsets.data(), nullptr, counts.data(), nullptr) < 0) {
        HDF5ErrMapper::ToException<DataSpaceException>("Unable to select hyperslap");
      }
    }
  ...
  inline void SliceTraits<Derivate>::write(const T& buffer) {
    ...
    if (!details::checkDimensions(mem_space, dim_buffer)) {
      std::ostringstream ss;
      ss << "Impossible to write buffer of dimensions " << dim_buffer << " into dataset of dimensions "
