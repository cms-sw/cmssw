...
namespace HighFive {
  ...
  inline DataSpace::DataSpace(const std::vector<size_t>& dims) : DataSpace(dims.begin(), dims.end()) {}

  template <class IT>
  inline DataSpace::DataSpace(const IT begin, const IT end) {
    std::vector<hsize_t> real_dims(begin, end);

    if ((_hid = H5Screate_simple(int(real_dims.size()), real_dims.data(), nullptr)) < 0) {
  ...
  inline DataSpace::DataSpace(const std::vector<size_t>& dims, const std::vector<size_t>& maxdims) {
    ...
    std::replace(real_maxdims.begin(), real_maxdims.end(), static_cast<hsize_t>(DataSpace::UNLIMITED), H5S_UNLIMITED);
  ...
  inline DataSpace::DataSpace(const size_t dim1) {
    ...
    if ((_hid = H5Screate_simple(1, &dims, nullptr)) < 0) {
  ...
  inline std::vector<size_t> DataSpace::getDimensions() const {
    ...
    if (!dims.empty()) {
      if (H5Sget_simple_extent_dims(_hid, dims.data(), nullptr) < 0) {
  ...
  inline std::vector<size_t> DataSpace::getMaxDimensions() const {
    ...
    if (H5Sget_simple_extent_dims(_hid, nullptr, maxdims.data()) < 0) {
      HDF5ErrMapper::ToException<DataSetException>("Unable to get dataspace dimensions");
    }

    std::vector<size_t> res(maxdims.begin(), maxdims.end());
    std::replace(maxdims.begin(), maxdims.end(), static_cast<size_t>(H5S_UNLIMITED), DataSpace::UNLIMITED);
    return res;
  }
  ...
  inline DataSpace DataSpace::From(const ScalarValue& scalar) {
    (void)scalar;
#if H5_USE_CXX11
    static_assert((std::is_arithmetic<ScalarValue>::value || std::is_enum<ScalarValue>::value ||
  ...
  namespace details {
    ...
    inline bool checkDimensions(const DataSpace& mem_space, size_t input_dims) {
      ...
      for (std::vector<size_t>::const_reverse_iterator i = dims.rbegin(); i != --dims.rend() && *i == 1; ++i)
        --dataset_dims;
      ...
      for (std::vector<size_t>::const_iterator i = dims.begin(); i != --dims.end() && *i == 1; ++i)
        --dataset_dims;
