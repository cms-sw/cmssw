...
namespace lhef {
  ...
  public:
   ...
   H5Handler::H5Handler(const std::string &fileNameIn) {
      if(fileNameIn.empty())
        throw cms::Exception("FileOpenError") << "Empty H5 file name \"" << std::endl;
      //HighFive::File *tmpfs = new HighFive::File( fileNameIn, HighFive::File::ReadOnly );
      HighFive::File *tmpfs = new HighFive::File( fileNameIn );
      h5file.reset( tmpfs );
      ...
      bool indexStatus = h5file->exist("/index");
      hid_t dspace;
      _formatType = 1;

      if( indexStatus ) {
         _index    = h5file->getGroup("index");
     ...
     // _eventsInBlock = _eventsTotal / 5 ; // configurable parameter??
      _blocksRead = 0;
...
// Check if the file contains the npLO, npNLO information
   ...
   void H5Handler::init(const int firstEventIn, const int maxEventsIn) {
      ...
      if( firstEventIn > 0 && _blocksRead > 0 ) _eventsRead = firstEventIn - 1;
   ...
   void H5Handler::readBlock() {
      ...
      if( (unsigned long int) (iStart + nEvents) > _eventsTotal ) return;
   ...
   std::vector<lheh5::Particle> H5Handler::getEvent() {
      ...
      // if( _eventsRead > 20000 - 1 ) return std::vector<lheh5::Particle>();
      if( _eventsRead > _eventsTotal - 1 ) return std::vector<lheh5::Particle>();
      int checkEvents = (_blocksRead ) * _eventsInBlock-1;
      _vE = _events1.mkEvent( _eventsRead );
      _eventsRead++;
      if( checkEvents >=0 && _eventsRead > (unsigned long int) checkEvents ) readBlock();
      return _vE;
   }
...
// fragile, must be called before getEvent
   ...
   std::pair<lheh5::EventHeader, std::vector<lheh5::Particle> > H5Handler::getEventProperties() {
      ...
      _h.nparticles = -1;
      if( _eventsRead > _eventsTotal - 1) return std::make_pair(_h, _vE);
      int checkEvents = (_blocksRead ) * _eventsInBlock-1;
...
// Set maxEvents and firstEvent
        curSource->handler->init(firstEvent,maxEvents);
...
//      for (auto part: curSource->handler->_events1.mkEvent(i)) {
      for (auto part: evp.second ) {
         ...
         tmp.PUP[ip][0] = part.px; tmp.PUP[ip][1]=part.py; tmp.PUP[ip][2]=part.pz;
         tmp.PUP[ip][3] = part.e ; tmp.PUP[ip][4]=part.m;
         ip++;
      }
      tmp.IDPRUP   = hd.pid; 
