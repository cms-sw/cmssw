...
my @SRM_SERVERS = ("srm-public.cern.ch", "srm-alice.cern.ch", "srm-atlas.cern.ch", "srm-cms.cern.ch",
		   "srm-lhcb.cern.ch", "srm-dteam.cern.ch");

my ($opt_h, $longhelp, $opt_a, $header, $dsname, $verbose, $debug, $replace,
    $not2web, $authors, $category, $descriptiononly, $uploadonly, $artid, $srm_server);

GetOptions(
    "h",    \$opt_h,       # short help
    "help", \$longhelp,    # long help
    "a=s",
    \$opt_a,    # set authorization method in MCDB (login, cert, pkcs12, globus)
    ...
    "replace", \$replace
    , # replace description in the article -artid from the header of first file in the argument list
    "not2web", \$not2web
    , # do not post article to WEB (default is post after description is complete)
    "authors=s",       \$authors,          # set additional authors logins
    "category=s",      \$category,         # set categories for this article
    "descriptiononly", \$descriptiononly
    , # do not upload any files only create article with the header from first file in the argument list
    "uploadonly", \$uploadonly
    , # do upload files to article -artid only, do not parse header and create article
    "artid=s",
    \$artid,    # set manually the ArticleID to upload files or set description,
    "srmserver=s", \$srm_server,  # specify SRM accecc point to use
);


# Print help message
longhelp() if defined $longhelp;
...
# ArticleID or DataSetName should be provided for the uploadonly option
...
#  Check the specified files to be present and readable 
    foreach my $file (@ARGV) {
        if ( !-e $file ) {
            print "FILE $file does not exist, please, check it. \n";
            exit(1);
        }
        if ( !-r $file ) {
            print "FILE $file is not readable, please, check it. \n";
            exit(1);
        }
...
#Authorization in MCDB and CASTOR; The default and prefered way is grid-proxy-init other options are valid for MCDB description only not for the CASTOR (GRID)
    if ( $auth eq "login" ) {
        ...
        $ENV{HTTPS_PKCS12_PASSWORD} = 'PKCS12_PASSWORD'
          ;    # you can put here <STDIN> to enter it during the run
    }
    elsif ( $auth eq "cert" ) {
	  ...
	  if($@ || $auth_err){
       ...
       $ENV{HTTPS_CA_DIR} =
          ( defined $ENV{X509_CERT_DIR} )
          ? $ENV{X509_CERT_DIR}
          : "/etc/grid-security/certificates";
        $GSIPROXY =
          ( defined $ENV{X509_USER_PROXY} )
          ? $ENV{X509_USER_PROXY}
          : "/tmp/x509up_u$<";
...
# Try to find the previous unfinished uploading sessions from the same directory
    ...
    if ( scalar(@old_sessions) > 0 ) {
        print "Something is wrong with the previous uploading jobs, there are ",
          $#old_sessions + 1,
...
# HTTP(POST) Request to send LHEF HEADER to MCDB and get ArticleID and CASTOR directory to upload the file
    my $response = $ua->request($req);
    if ( $response->is_success ) {
        print "Header has been uploaded to MCDB, response content: \n ", $response->as_string(), "\n END of the server response content \n"
          if defined $verbose;    #$response->content
    }
    else {
...
# Check the critical points in response from MCDB
    ...
    if ( not defined $descriptiononly ) {
	    ...
	    $hepml = extract_hepml_header( $files[0] )
            ...
            if ( $response->is_success ) {
                ...
                print "ERROR during the registration of $file in MCDB: ",
                  $response->as_string(), "\n";
                exit(1);
            }
...
$0 -uploadonly [-artid N] [-dsname DataSetName] 	#do only uploading of files to article -artid or -dsname
 ...
 -srmserver full.host.name  #point what SRM server should be used to upload  files; default is $SRM_SERVER_DEFAULT
...
of file1 but all other files will be uploaded and attached to the same article.
Possible options:
  -srmserver full.host.name  #point what SRM server should be used to upload  files; default is $SRM_SERVER_DEFAULT
