...
class LH5Source : public edm::ProducerSourceBase {
public:
...
void LH5Source::endJob() { reader_.reset(); }
...
void LH5Source::fillRunInfoProduct(lhef::LHERunInfo const& iInfo, LHERunInfoProduct& oProduct) {
...
void LH5Source::putRunInfoProduct(edm::RunPrincipal& iRunPrincipal) {
  if (runInfoProductLast_) {
    ...
    iRunPrincipal.put(lheProvenanceHelper_.runProductProductDescription_, std::move(rdp));
  }
}

bool LH5Source::setRunAndEventInfo(edm::EventID&, edm::TimeValue_t&, edm::EventAuxiliary::ExperimentType&) {
  nextEvent();
  if (!partonLevel_) {
...
DEFINE_FWK_INPUT_SOURCE(LH5Source);
