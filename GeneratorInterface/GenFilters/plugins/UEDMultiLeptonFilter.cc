...
bool UEDMultiLeptonFilter::filter(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  using namespace edm;
    ...
    if (D_part || S_part) {
      ...
      if (vertex != nullptr) {
        for (HepMC::GenVertex::particles_out_const_iterator part = vertex->particles_out_const_begin();
             part != vertex->particles_out_const_end();
             part++) {
          ...
          if (isLepton(part)) {
            ...
            if (lepton_vertex != nullptr) {
              for (HepMC::GenVertex::particles_out_const_iterator lepton_part =
                       lepton_vertex->particles_out_const_begin();
                   lepton_part != lepton_vertex->particles_out_const_end();
                   lepton_part++) {
                //debug std::cout << "      Part Id: "<< (*lepton_part)->pdg_id() << " status: "<< (*lepton_part)->status()<< std::endl;
                if ((*part)->pdg_id() ==
                    (*lepton_part)
                        ->pdg_id()) {  //debug std::cout << "         Part pt: " << (*part)->momentum().perp() << " mu 1: " << (*lepton_part)->momentum().perp() << std::endl;
                }
                ...
                if ((*part)->pdg_id() == (*lepton_part)->pdg_id() && (*lepton_part)->status() == 1) {
                  if (isLeptonPlus(lepton_part))
                    leptonIDplus.push_back((*lepton_part)->pdg_id());
                  if (isLeptonMinus(lepton_part))
                    leptonIDminus.push_back((*lepton_part)->pdg_id());
                }
              }
            }
          }
  ...
  if ((leptonIDplus.size() == 1 && leptonIDminus.size() == 1) ||
      (leptonIDplus.size() == 1 && leptonIDminus.size() == 1)) {
    //Opposite sign only
    nDileptons++;
    int e = 0;
    int mu = 0;
  ...
  if (leptonIDplus.size() > 1 || leptonIDminus.size() > 1) {
    ...
    if ((leptonIDplus.size() == 2 && leptonIDminus.empty()) || (leptonIDplus.empty() && leptonIDminus.size() == 2)) {
      ...
      if (mu == 2 && e == 0) {
        nSSmumu++;
        if (SSDiMuVetoedFilter == 1)
          return AcceptEvent();
      }
      if (mu == 1 && e == 1) {
        nSSemu++;
        if (SSDiEMuVetoedFilter == 1)
          return AcceptEvent();
      }
      if (mu == 0 && e == 2) {
        nSSee++;
        if (SSDiEVetoedFilter == 1)
          return AcceptEvent();
      }
  ...
  if (leptonIDplus.size() + leptonIDminus.size() == 4) {
    ...
    if (mu == 4 && e == 0) {
      nFour4mu++;
      if (Vetoed4muFilter == 1)
        return AcceptEvent();
    }
    if (mu == 2 && e == 2) {
      nFour2mu2e++;
      if (Vetoed2mu2eFilter == 1)
        return AcceptEvent();
    }
    if (mu == 0 && e == 4) {
      nFour4e++;
  ...
  if (leptonIDplus.size() + leptonIDminus.size() == 3) {
    ...
    if (mu == 3 && e == 0) {
      nTri3mu++;
      if (Vetoed3muFilter == 1)
        return AcceptEvent();
    }
    if (mu == 2 && e == 1) {
      nTri2mu1e++;
      if (Vetoed2mu1eFilter == 1)
        return AcceptEvent();
    }
    if (mu == 1 && e == 2) {
      nTri1mu2e++;
      if (Vetoed1mu2eFilter == 1)
        return AcceptEvent();
    }
    if (mu == 0 && e == 3) {
      nTri3e++;
...
void UEDMultiLeptonFilter::beginJob() {}

// ------------ method called once each job just after ending the event loop  ------------
void UEDMultiLeptonFilter::endJob() {}
...
void UEDMultiLeptonFilter::nLeptons(const std::vector<int>& leptons_id, int& e, int& mu) {
  int nentries = (int)leptons_id.size();
  for (int i = 0; i < nentries; i++) {
    if (abs(leptons_id.at(i)) == 11)
      e++;
    if (abs(leptons_id.at(i)) == 13)
      mu++;
  }
