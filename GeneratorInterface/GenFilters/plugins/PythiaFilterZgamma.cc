#include "GeneratorInterface/GenFilters/plugins/PythiaFilterZgamma.h"
...
bool PythiaFilterZgamma::filter(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  if (selProc == 1) {
    if (el.size() > 1) {
      ...
      if (ptEl1 > ptElMin && ptEl2 > ptElMin && std::abs(etaEl1) < etaElMax && std::abs(etaEl2) < etaElMax) {
        if (gam.size() == 1) {
    ...
    if (mu.size() > 1) {
      ...
      if (ptMu1 > ptMuMin && ptMu2 > ptMuMin && std::abs(etaMu1) < etaMuMax && std::abs(etaMu2) < etaMuMax) {
        if (gam.size() == 1) {
          double ptGam = gam[0]->momentum().perp();
          double etaGam = gam[0]->momentum().eta();
          if (ptGam > ptPhotonMin && std::abs(etaGam) < etaPhotonMax)
