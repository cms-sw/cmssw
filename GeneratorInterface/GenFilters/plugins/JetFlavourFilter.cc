...
JetFlavourFilter::JetFlavourFilter(const edm::ParameterSet& iConfig) {
  ...
  if ((jetType >= 1) && (jetType <= 3))
    jetType = 1;

  noAccepted = 0;
}

JetFlavourFilter::~JetFlavourFilter() { std::cout << "Total number of accepted events = " << noAccepted << std::endl; }

HepMC::GenParticle* JetFlavourFilter::findParticle(const GenPartVect& genPartVect, const int requested_id) {
  for (GenPartVectIt p = genPartVect.begin(); p != genPartVect.end(); p++) {
    if (requested_id == (*p)->pdg_id())
      return *p;
  }
  return nullptr;
}
...
void JetFlavourFilter::printHisto(const HepMC::GenEvent::particle_iterator start,
  ...
  for (p = start; p != end; p++) {
    ...
    for (GenVertex::particles_in_const_iterator iter = outVertex->particles_in_const_begin();
         iter != outVertex->particles_in_const_end();
         iter++)
...
bool JetFlavourFilter::filter(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (p = generated_event->particles_begin(); p != generated_event->particles_end(); p++) {
    if (((*p)->pdg_id() < 1) && ((*p)->pdg_id() > -10)) {  // We have an anti-quark
      ...
      for (GenPartVectIt z = parents.begin(); z != parents.end(); z++) {
        ...
        if (findParticle(child, -(*p)->pdg_id()))
          foundQ.push_back(-(*p)->pdg_id());
      }
  ...
  if ((flavour >= 0) && (flavour <= 3))
    flavour = 1;
  // Do we have more than one heavy flavour ?
  if ((ff[3] && ff[4]) || (ff[3] && ff[5]) || (ff[4] && ff[5]))
    flavour = 0;
