...
#include "GeneratorInterface/GenFilters/plugins/JGJFilter.h"
...
bool JGJFilter::filter(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  if (o4.isValid()) {
    ...
    if (njets > 1 && fabs(jet_eta[0] - jet_eta[1]) > 3.5)
      accepted = true;

    if (njets > 2) {
      if ((jet_pt[2] / jet_pt[1]) > 0.8) {
        if (fabs(jet_eta[0] - jet_eta[2]) > 3.5)
          accepted = true;
        if (fabs(jet_eta[1] - jet_eta[2]) > 3.5)
          accepted = true;
      }
    }
  }  //valid
...
void JGJFilter::beginJob() {}

void JGJFilter::endJob() {}
