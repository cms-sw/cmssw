#include "GeneratorInterface/GenFilters/plugins/PythiaFilterEMJet.h"
...
bool PythiaFilterEMJet::filter(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (HepMC::GenEvent::particle_const_iterator p = myGenEvent->particles_begin(); p != myGenEvent->particles_end();
       ++p) {
    if ((abs((*p)->pdg_id()) == 11 || (*p)->pdg_id() == 22) && (*p)->status() == 1 && (*p)->momentum().perp() > pTMin &&
        (*p)->momentum().perp() < pTMax && fabs((*p)->momentum().eta()) < etaMax &&
  ...
  double phiEMClus = 0;
  double ptEMClus = 0;
  for (std::list<const HepMC::GenParticle*>::const_iterator is = EM_seeds.begin(); is != EM_seeds.end(); ++is) {
    ...
    for (HepMC::GenEvent::particle_const_iterator p = myGenEvent->particles_begin(); p != myGenEvent->particles_end();
      ...
      // 	   abs((*p)->pdg_id()) == 223  )             	       // omega*/
      {
        ...
        if (pt == ptEM)
          continue;  //discard the same particle of the seed
        double eta = (*p)->momentum().eta();
        double phi = (*p)->momentum().phi();

        if (inEB) {
          if (std::abs(eta - etaEM) > deltaEB || std::abs(deltaPhi(phi, phiEM)) > deltaEB)
            continue;
  ...
  if (accepted) {
    theNumberOfSelected++;
    cout << "========>  Event preselected " << theNumberOfSelected << " Proccess ID " << myGenEvent->signal_process_id()
         << endl;
    return true;
  } else
    return false;
