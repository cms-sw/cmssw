...
bool Zto2lFilter::filter(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  using namespace edm;
  ...
  if (Lepton.size() == 2) {
    if ((Lepton[0] + Lepton[1]).M() > minInvariantMass_)
      accept = true;
  }
  //delete evt;
  return accept;
