#include "GeneratorInterface/GenFilters/plugins/BdecayFilter.h"
...
BdecayFilter::~BdecayFilter() { std::cout << "Total number of accepted events = " << noAccepted << std::endl; }
...
HepMC::GenParticle* BdecayFilter::findParticle(HepMC::GenVertex* vertex, const int requested_id) {
...
HepMC::GenEvent::particle_const_iterator BdecayFilter::getNextBs(const HepMC::GenEvent::particle_const_iterator start,
  ...
  for (p = start; p != end; p++) {
    ...
    if (event_particle_id == motherParticle)
      return p;
  }
  return p;
}
  ...
  while (bs != generated_event->particles_end()) {
    ...
    */
    ...
    if ((numChildren == 2) && ((jpsi = findParticle(outVertex, firstDaughter.type)) != nullptr) &&
      ...
      cout << jpsi->momentum().rho() << " " << jpsi->momentum().eta() << " " << phi->momentum().rho() << " "
...
bool BdecayFilter::cuts(const GenParticle* jpsi, const CutStruct& cut) {
  ...
  for (int i = 0; i < numChildren; ++i) {
    ...
    for (int j = 0; j < numChildren; ++j) {
      if (abs(psiChild[i]->pdg_id()) == abs(cut.decayProduct[j]))
        goodPart = true;
    }
    cout << psiChild[i]->momentum().perp() << endl;
    if (!goodPart || (!etaInRange(psiChild[i]->momentum().eta(), cut.etaMin, cut.etaMax)) ||
        (psiChild[i]->momentum().perp() < cut.ptMin))
      return false;
  }
  cout << "cuts true" << endl;
  return true;
...
bool BdecayFilter::etaInRange(float eta, float etamin, float etamax) { return ((etamin < eta) && (eta < etamax)); }
