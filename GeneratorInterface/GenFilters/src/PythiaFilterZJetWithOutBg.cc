...
//      std::cout<<" Muons sign accept "<<mu[0]->momentum().perp()<<" "<<mu[1]->momentum().perp()<<std::endl;
    ...
    if(mu[0]->momentum().perp()<ptMuMin || mu[1]->momentum().perp()<ptMuMin) return false;
    
//    std::cout<<" Muons pt accept "<<std::fabs(mu[0]->momentum().eta())<<" "<<std::fabs(mu[1]->momentum().eta())<<std::endl;
    
    if(std::fabs(mu[0]->momentum().eta()) > etaMuMax) return false;
    if(std::fabs(mu[1]->momentum().eta()) > etaMuMax) return false;
    ...
    double pmup2 = mu[0]->momentum().x()*mu[0]->momentum().x()+
                   mu[0]->momentum().y()*mu[0]->momentum().y()+
		   mu[0]->momentum().z()*mu[0]->momentum().z();
    double pmum2 = mu[1]->momentum().x()*mu[1]->momentum().x()+
                   mu[1]->momentum().y()*mu[1]->momentum().y()+
