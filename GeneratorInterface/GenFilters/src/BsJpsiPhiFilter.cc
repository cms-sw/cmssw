...
BsJpsiPhiFilter::~BsJpsiPhiFilter() { std::cout << "Total number of accepted events = " << noAccepted << std::endl; }
...
HepMC::GenParticle* BsJpsiPhiFilter::findParticle(HepMC::GenVertex* vertex, const int requested_id) {
...
HepMC::GenEvent::particle_const_iterator BsJpsiPhiFilter::getNextBs(
    const HepMC::GenEvent::particle_const_iterator start, const HepMC::GenEvent::particle_const_iterator end) {
...
bool BsJpsiPhiFilter::etaInRange(float eta, float etamin, float etamax) { return ((etamin < eta) && (eta < etamax)); }
