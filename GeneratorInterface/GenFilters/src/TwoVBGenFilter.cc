...
bool
TwoVBGenFilter::filter(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  int nj = 0;
  int ne = 0;
  int nnu = 0;
  ...
  for ( HepMC::GenEvent::particle_const_iterator p = myGenEvent->particles_begin();
    ...
    if(abs((*p)->pdg_id()) !=23 && abs((*p)->pdg_id()) !=24) continue; // If it is not Z or W, go to the next particle.
	...
	if(isQuark(theDaughter)) ++nj; 
	else if(isNeutrino(theDaughter)) ++nnu; 
	else if(isElectron(theDaughter)) ++ne; 
	else if(isMuon(theDaughter)) ++nmu;
	else if(isTau(theDaughter)) ++ntau;

      }                            
    }
  }

  if      (ne==2 && nj == 2 && eejj_) return true;
  else if (ne==1 && nj == 2 && nnu==1 && enujj_) return true;
  else if (nmu==2 && nj == 2 && mumujj_) return true;
  else if (nmu==1 && nj == 2 && nnu==1 && munujj_) return true;
  else if (ntau==2 && nj == 2 && tautaujj_) return true;
  else if (ntau==1 && nj == 2 && nnu==1 && taunujj_) return true;
  else if (nnu==2  && nj == 2 && nunujj_) return true;
  else return false;
}

// ------------ method called once each job just before starting event loop  ------------
void 
TwoVBGenFilter::beginJob()
{
}

// ------------ method called once each job just after ending the event loop  ------------
void 
TwoVBGenFilter::endJob() {
}
