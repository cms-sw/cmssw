...
bool MCDecayingPionKaonFilter::filter(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (HepMC::GenEvent::particle_const_iterator p = myGenEvent->particles_begin(); p != myGenEvent->particles_end();
    ...
    for (unsigned int i = 0; i < particleID.size(); i++) {
      ...
      if ((*p)->momentum().eta() < etaMin[i])
        continue;
      if ((*p)->momentum().eta() > etaMax[i])
        continue;
      ...
      if ((*p)->status() == 2) {
        for (HepMC::GenVertex::particle_iterator vpdec = (*p)->end_vertex()->particles_begin(HepMC::children);
             vpdec != (*p)->end_vertex()->particles_end(HepMC::children);
             ++vpdec) {
          if (abs((*vpdec)->pdg_id()) != 13)
            continue;
