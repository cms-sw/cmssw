...

{
  BHFilter::BHFilter(const edm::ParameterSet& conf) : conf_(conf) {
    ...
    if (trig_ == 0) {
      cout << endl << "trigger is both + and - BSC " << endl;
    }
    if (trig_ == 1) {
      cout << endl << "trigger is + side BSC " << endl;
    }
    if (trig_ == -1) {
      cout << endl << "trigger is - side BSC " << endl;
    }
    ...
    if (trig2_ == 0) {
      cout << endl << "trigger is both PADs and DISKs " << endl;
    }
    if (trig2_ == -1) {
      cout << endl << "trigger is only PADs " << endl;
    }
    if (trig2_ == 1) {
      cout << endl << "trigger is only DISKs " << endl;
    }
  ...
  bool BHFilter::filter(edm::Event& iEvent, const edm::EventSetup& iSetup) {
    ...
    for (HepMC::GenEvent::particle_const_iterator i = MCEvt->particles_begin(); i != MCEvt->particles_end(); ++i) {
      ...
      //	HepMC::FourVector
      XYZTLorentzVector momentum = XYZTLorentzVector(
          ((*i)->momentum()).x(), ((*i)->momentum()).y(), ((*i)->momentum()).z(), ((*i)->momentum()).t());
      ...
      if ((ys < sqrt(450 * 450 - xs * xs) && ys > sqrt(208 * 208 - xs * xs)) ||
          (ys < sqrt(450 * 450 - xs * xs) && xs > 208)) {
        circ_plus = true;
        //  cout << " trig2+" << endl;
      }
