...
void BasicGenTester::beginJob() {
  ...
  fNChgPartFinalState =
      fs->make<TH1D>("NChgPartFinalState", "Number of final state charged particles", fNPart, 0., (double)fNPart);
  fNNeuPartFinalState =
      fs->make<TH1D>("NNeuPartFinalState", "Number of final state neutral particles", fNPart, 0., (double)fNPart);
  fNPartFinalState =
      fs->make<TH1D>("NPartFinalState", "Total number of final state particles", fNPart, 0., (double)fNPart);
  fPtChgPartFinalState =
      fs->make<TH1D>("PtChgPartFinalState", "Pt of final state charged particles", 500, fPtMin, fPtMax);
  fPtNeuPartFinalState =
      fs->make<TH1D>("PtNeuPartFinalState", "Pt of final state neutral particles", 500, fPtMin, fPtMax);
  fEtaChgPartFinalState =
      fs->make<TH1D>("EtaChgPartFinalState", "Eta of final state charged particles", 100, -5.0, 5.0);
  fEtaNeuPartFinalState =
      fs->make<TH1D>("EtaNeuPartFinalState", "Eta of final state neutral particles", 100, -5.0, 5.0);
  return;
}
...
void BasicGenTester::endRun(const edm::Run& r, const edm::EventSetup&) { return; }

void BasicGenTester::endJob() { return; }
