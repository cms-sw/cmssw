...
edm::CosMuoGenEDFilter::CosMuoGenEDFilter( const ParameterSet & pset ) :
...
edm::CosMuoGenEDFilter::~CosMuoGenEDFilter(){
  //CosMuoGen->terminate();
  delete CosMuoGen;
  //  delete fEvt;
  clear();
...
bool edm::CosMuoGenEDFilter::endRun( Run &run, const EventSetup& es )
{
...
void edm::CosMuoGenEDFilter::clear(){}

//bool edm::CosMuoGenEDFilter::produce(Event &e, const edm::EventSetup &es)
bool edm::CosMuoGenEDFilter::filter(Event &e, const edm::EventSetup &es)
{  
  // generate event
  if (!MultiMuon) {
    CosMuoGen->nextEvent();
    ...
    if (!success) {
      std::cout << "CosMuoGenEDFilter.cc: CosMuoGen->nextMultiEvent() failed!" << std::endl;
      return false; //skip event, either bad or no more events in file
    }
  }
