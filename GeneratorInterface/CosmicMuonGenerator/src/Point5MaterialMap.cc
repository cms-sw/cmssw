...
inline int inPlug(double vx, double vy, double vz, double PlugVx = PlugOnShaftVx, double PlugVz = PlugOnShaftVz) {
  if (vy > SurfaceOfEarth && vy < SurfaceOfEarth + PlugWidth) {
    if (vx > PlugVx - PlugXlength / 2. && vx < PlugVx + PlugXlength / 2. && vz > PlugVz - PlugZlength / 2. &&
        vz < PlugVz + PlugZlength / 2.)
      return Plug;
    if (vz >= PlugVz - PlugZlength / 2. - PlugNoseZlength && vz < PlugVz - PlugZlength / 2. &&
        vx > PlugVx - PlugNoseXlength / 2. && vx < PlugVx + PlugNoseXlength / 2.)
      return Plug;
  }
  return Unknown;
}
...
inline int inAirAfterPlug(double vx, double vy, double vz) {
  ...
  if (vy >= SurfaceOfEarth)
    return Air;

  // CMS cavern (UXC 55)
  if (std::fabs(vz) < 26548. && sqrt((vx * 1.1576) * (vx * 1.1576) + vy * vy) < 15460. && vy > -8762)
    return Air;

  // access shaft (PX 56)
  if (vy > 0. && vy < (SurfaceOfEarth - 2250.) && sqrt(vx * vx + (vz - Z_PX56) * (vz - Z_PX56)) < 10250.)
    return Air;

  //surface hall ground floor
  if (vy >= SurfaceOfEarth - 2250. && vy < SurfaceOfEarth) {
    if (sqrt(vx * vx + (vz - Z_PX56) * (vz - Z_PX56)) < 10250. && vz - Z_PX56 > -7000. && vz - Z_PX56 < 7000.)
      return Air;
  ...
  if (vz > -22050. && vz < 62050. && sqrt((vx - 29550.) * (vx - 29550.) + (vy - 3233.) * (vy - 3233.)) < 9050. &&
      vy > -3650.)
    return Air;

  //UXC55 cavern endcap beam openings
  if (((vz >= -29278. && vz < -26548.) || (vz >= 26548 && vz <= 29278.)) && sqrt(vy * vy + vx * vx) < 800.)
    return Air;
  ...
  if (vx > 10000. && vx < 16500. &&  //TX54 galerie 30.9 degree opening to UXC55
      ...
      vz > 14460. - 1865. * (16500. - vx) / (3200.) && vz <= 14460.)
    return Air;
  ...
  if (vz > -85000. && vz <= -29278. &&  //UJ57 junction to UXC55 cavern
      sqrt(vy * vy + (vx - 350.) * (vx - 350.)) < 1900. && vy > -1000.)
    return Air;
  ...
  //UJ56 cavern
  if (vz > -58875. && vz < -33927. && sqrt(vy * vy + (vx - 4450.) * (vx - 4450.)) < 6750. &&
      vy > -1000. &&  //and beam shielding
  ...
  if (vx > 9000. && vx < 18220. &&
      sqrt((vy - 50.) * (vy - 50.) + (vz + 24227. - Z_PX56) * (vz + 24227. - Z_PX56)) < 3550. && vy > -1000.)
    return Air;
...
inline int inWallAfterAir(double vx, double vy, double vz) {
  ...
  if (vy < SurfaceOfEarth && vy >= (SurfaceOfEarth - 2250.)) {
    if (std::fabs(vz - Z_PX56) < 30000. && std::fabs(vx) < 10950)
      return Wall;
    // foundation of crane
  ...
  if (vy > -5450. && vy < (SurfaceOfEarth - 1000.) &&  //t~=t(PX56)/R(PX56)*R(PM56)
      sqrt((vx - 18220.) * (vx - 18220.) + (vz + 24227. - Z_PX56) * (vz + 24227. - Z_PX56)) <
          3550. + 2150. / 10250. * 3550.)
    return Wall;
  else if (vy > SurfaceOfEarth - 1000. && vy < SurfaceOfEarth &&  //t~=t(PX56)/R(PX56)*R(PM56)
           sqrt((vx - 18220.) * (vx - 18220.) + (vz + 24227. - Z_PX56) * (vz + 24227. - Z_PX56)) <
               3550. + 2150. / 10250. * 3550. + 1800.)
    return Wall;

  //Service cavern USC 55
  if (vz > -(22050. + 1150.) && vz < (62050. + 1150.) &&
      sqrt((vx - 29550.) * (vx - 29550.) + (vy - 3233.) * (vy - 3233.)) < 9050. + 950. && vy > -3650. - 2000.)
    return Wall;  //8762=estimate, to be checked
  ...
  if (vz > -85000. && vz < -28510.) {  //UJ57 junction to UXC55 cavern
    if (sqrt(vy * vy + (vx - 350.) * (vx - 350.)) < 2250.)
      return Wall;
  }
  ...
  //UJ56 cavern
  if (vz > -(58875. + 500.) && vz < -(33927. - 500.) && sqrt(vy * vy + (vx - 4450.) * (vx - 4450.)) < (6750. + 500.) &&
      vy > -3650.)
    return Wall;

  //connection between PM56 shaft and UJ56 cavern
  if (vx > 9000. && vx < 18220. &&
      sqrt((vy - 50.) * (vy - 50.) + (vz + 24227. - Z_PX56) * (vz + 24227. - Z_PX56)) < 3550. + 500. && vy > -3650.)
    return Wall;
...
inline int inMat(double vx,
  ...
  //check for Plug
  if (inPlug(vx, vy, vz, PlugVx, PlugVz))
    return Plug;

  //So, it is not plug, Check for air
  if (inAirAfterPlug(vx, vy, vz))
    return Air;

  //So, it is not plug and air, Check for wall
  if (inWallAfterAir(vx, vy, vz))
    return Wall;
  ...
  std::cout << "Point5MaterialMap.h: Warning! No Material recognised for point: vx=" << vx << " vy=" << vy
            << " vz=" << vz << std::endl;
