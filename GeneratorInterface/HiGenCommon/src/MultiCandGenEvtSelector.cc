...
MultiCandGenEvtSelector::MultiCandGenEvtSelector(const edm::ParameterSet& iConfig) : BaseHiGenEvtSelector(iConfig) {
...
bool MultiCandGenEvtSelector::filter(HepMC::GenEvent* evt) {
  ...
  for (HepMC::GenEvent::particle_const_iterator it = begin; it != end; ++it) {
    if (selectParticle(*it, st_, pdg_, ptMin_, etaMax_))
      found++;
    if (found == nTrig_)
      return true;
  }
