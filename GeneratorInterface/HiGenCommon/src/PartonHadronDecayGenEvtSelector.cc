...
PartonHadronDecayGenEvtSelector::PartonHadronDecayGenEvtSelector(const edm::ParameterSet& pset)
  ...
  if (id != st || id != etamax || id != ptmin) {
    throw edm::Exception(edm::errors::LogicError)
        << "Parton selection parameters: " << id << st << etamax << ptmin << endl;
  }
}
...
bool PartonHadronDecayGenEvtSelector::filter(HepMC::GenEvent* evt) {
  ...
  while (!foundParton && it != end) {
    for (unsigned i = 0; i < partonId_.size(); ++i) {
      if (selectParticle(*it, partonStatus_[i], partonId_[i], partonPtMin_[i], partonEtaMax_[i]))
        foundParton = true;
    }
    ++it;
  }
  ...
  if (foundParton) {
    while ((!foundHadron || !foundDecay) && it2 != end) {
      ...
      if (selectParticle(
              *it2, decayStatus_, decayId_, decayEtaMax_, decayEtaMin_, decayPMin_, decayPtMax_, decayPtMin_))
        foundtrig++;
