...
Particle::Particle(
    ParticlePDG *prop, const TLorentzVector &pos, const TLorentzVector &mom, double lit, int lin, int type)
    : fPosition(pos),
      fMomentum(mom),
      fPythiaStatusCode(-1),
      fIndex(-1),
...
int Particle::Encoding() const { return fParticleProperties->GetPDG(); }

double Particle::TableMass() const { return fParticleProperties->GetMass(); }
...
double Particle::Phi() const { return TMath::Pi() + TMath::ATan2(-fMomentum.Py(), -fMomentum.Px()); }

double Particle::Theta() const {
  return !fMomentum.Pz() ? TMath::Pi() / 2 : TMath::ACos(fMomentum.Pz() / fMomentum.P());
}

double Particle::Pt() const { return TMath::Sqrt(fMomentum.Px() * fMomentum.Px() + fMomentum.Py() * fMomentum.Py()); }

double S(const TLorentzVector &v1, const TLorentzVector &v2) {
  return TMath::Power(v1.T() + v2.T(), 2) - TMath::Power(v1.X() + v2.X(), 2) - TMath::Power(v1.Y() + v2.Y(), 2) -
         TMath::Power(v1.Z() + v2.Z(), 2);
}
...
void ParticleAllocator::FreeListNode(List_t &list, LPIT_t it) { fFreeNodes.splice(fFreeNodes.end(), list, it); }

void ParticleAllocator::FreeList(List_t &list) { fFreeNodes.splice(fFreeNodes.end(), list); }
