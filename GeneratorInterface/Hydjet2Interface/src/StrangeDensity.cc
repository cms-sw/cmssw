...
double NAStrangeDensity::ParticleNumberDensity(ParticlePDG* pDef) {
  ...
  double particleChemPotential = fBaryonPotential * particleBaryon + fStrangePotential * particleStrangeness;
  ...
  double postfactor;
  prefactor =
      (particleDegFactor * particleMass * particleMass * fTemperature / hbarc / hbarc / hbarc) / (2. * N_PI * N_PI);
  postfactor = 0.;
