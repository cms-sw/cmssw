...
UEActivityHistograms::UEActivityHistograms( const char* fileName, string *triggerNames )
{
  ...
  TDirectory** subdir = new TDirectory*[12];
  ...
  h_dN_vs_dphi        = new TH2D*[12];
  h_dpT_vs_dphi       = new TH2D*[12];
  h_averagePt_vs_nChg = new TH2D*[12];
  h_pTChg             = new TH2D*[36]; /// one for each region
  h_dN_vs_dpTjet      = new TH2D*[36];
  h_dpT_vs_dpTjet     = new TH2D*[36];
  ...
  for ( unsigned int iregion(0); iregion<3; ++iregion )
    {
      ...
      ///
      nChg  += activity.GetNumParticles(iregion) /(TMath::Pi()/3.);
      pTsum += activity.GetParticlePtSum(iregion)/(TMath::Pi()/3.);
      ...
      ///
      h_dN_vs_dpTjet [iregion*12+iHLTbit]->Fill( pTjet, activity.GetNumParticles(iregion) /(TMath::Pi()/3.) );
      h_dpT_vs_dpTjet[iregion*12+iHLTbit]->Fill( pTjet, activity.GetParticlePtSum(iregion)/(TMath::Pi()/3.) );
      ...
      for ( int i(1); i<=_nbinsPtChg; ++i )
	{
	  ...
	  h_pTChg[iregion*12+iHLTbit]->Fill( x, y );
	}
