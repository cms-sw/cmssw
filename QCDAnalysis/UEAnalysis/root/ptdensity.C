...
void ptdensity()
{
	//plotPtDensityByDphiParticles( 500 );
	//plotPtDensityByDphiParticles( 900 );
	//plotPtDensityByDphiParticles( 1500 );
	plotPtDensityByDphiTracks( 500 );
	plotPtDensityByDphiTracks( 900 );
	plotPtDensityByDphiTracks( 1500 );

	//plotPtDensityParticles( 500 );
	//plotPtDensityParticles( 900 );
	//plotPtDensityParticles( 1500 );
	plotPtDensityTracksByHLT( 500 );
	plotPtDensityTracksByHLT( 900 );
	plotPtDensityTracksByHLT( 1500 );

	plotPtDensityTrackParticleRatio( 500 );
	plotPtDensityTrackParticleRatio( 900 );
	plotPtDensityTrackParticleRatio( 1500 );

	//plotPtDensityParticlesRatio( 500, 900 );
	//plotPtDensityParticlesRatio( 500, 1500 );
	//plotPtDensityParticlesRatio( 900, 1500 );
	plotPtDensityTracksRatio( 500, 900 );
	plotPtDensityTracksRatio( 500, 1500 );
	plotPtDensityTracksRatio( 900, 1500 );
}

//----------------------------------------------------------------------------------

void plotPtDensityByDphiParticles(int PTTHRESHOLD)
{
	...
	double binwidth( 1. );
	char buffer[200];
	sprintf( buffer, "ptdensityByDphiParticles%i", PTTHRESHOLD );
	plotCanvas( buffer );
	gPad->SetLogy();

	TH1D* trans = new TH1D("trans","trans", 6, -180., 180.);
	trans->GetXaxis()->SetTitle("#Delta#phi(particles, chg. gen jet) (#circ)");
	trans->GetYaxis()->SetTitle("< d#Sigmap_{T}/d#Delta#phid#eta > (GeV/c/deg)");
	trans->Fill( -90., 150);
	trans->Fill(  90., 150);
	...
	_file1->cd("All"); TH2D* gen2d = dPt_vs_dphiRECO->Clone(); TProfile* genx = gen2d->ProfileX()->Clone();
...
void plotPtDensityByDphiTracks(int PTTHRESHOLD)
{
	// plot charged particle ptsum-density vs azimuth with respect to leading jet

	double binwidth( 1. );
	char buffer[200];
	sprintf( buffer, "ptdensityByDphiTracks%i", PTTHRESHOLD );
	plotCanvas( buffer );
	gPad->SetLogy();
	...
	_file1->cd("HLTMinBiasPixel"); TH2D* minbiaspixel = dPt_vs_dphiRECO->Clone();
	_file1->cd("HLTMinBiasHcal" ); TH2D* minbiashcal  = dPt_vs_dphiRECO->Clone();
	_file1->cd("HLTMinBiasEcal" ); TH2D* minbiasecal  = dPt_vs_dphiRECO->Clone();
	_file1->cd("HLTMinBias"     ); TH2D* minbias      = dPt_vs_dphiRECO->Clone();
	_file1->cd("HLTZeroBias"    ); TH2D* zerobias     = dPt_vs_dphiRECO->Clone();
	_file1->cd("HLT1jet30"      ); TH2D* hlt1jet30    = dPt_vs_dphiRECO->Clone();
	_file1->cd("HLT1jet50"      ); TH2D* hlt1jet50    = dPt_vs_dphiRECO->Clone();
...
void plotPtDensityParticles(int PTTHRESHOLD)
{
	...
	double binwidth( 2. );
	char buffer[200];
	sprintf( buffer, "ptdensityParticles%i", PTTHRESHOLD );
	plotCanvas( buffer );
	
	sprintf( buffer, "MinBias900GeV.UE%iMeV.root", PTTHRESHOLD );
	TFile *_file1 = TFile::Open( buffer );

	_file1->cd("All"); TH2D* gen2d = dPt_vs_ptJTransMC->Clone();
	TProfile* genx = gen2d->ProfileX()->Clone();
	genx->SetMarkerStyle( 4 );
	genx->SetMarkerSize( 1.3 );

	TH1D* gen = new TH1D( "gen", ";p_{T}(chg. gen jet) (GeV/c);<d#Sigmap_{T}/d#phid#eta> / 2 GeV/c (GeV/c/rad)", 10, 0., 20. );
	gen->SetMinimum( 0. );
	gen->SetMaximum( 0.8 );
...
TH1D* getTH1DPtDensityParticles(int PTTHRESHOLD)
{
	...
	_file1->cd("All"); TH2D* gen2d = dPt_vs_ptJTransMC->Clone(); TProfile* genx = gen2d->ProfileX()->Clone();
...
void plotPtDensityParticlesRatio( int NOM, int DENOM )
{	
	...
	sprintf( buffer, ";p_{T}(chg. gen jet) (GeV/c);d#Sigmap_{T}(%i MeV/c) / d#Sigmap_{T}(%i GeV/c)", NOM, DENOM );
	TH1D* h_ratio = new TH1D( "h_ratio", buffer, 10, 0., 20. );

	sprintf( buffer, "MinBias900GeV.UE%iMeV.root", NOM ); TFile *_file1 = TFile::Open( buffer );
	_file1->cd("All"); TH2D* nom2d = dPt_vs_ptJTransMC->Clone(); TProfile* nomx = nom2d->ProfileX()->Clone();
	...
	_file1->cd("All"); TH2D* denom2d = dPt_vs_ptJTransMC->Clone(); TProfile* denomx = denom2d->ProfileX()->Clone();
	...
	sprintf( buffer, "#frac{d#Sigmap_{T}(p_{T}(particles) > %i MeV/c)}{d#Sigmap_{T}(p_{T}(particles) > %i MeV/c)}", NOM, DENOM );
...
TH1D* getTH1DPtDensityParticlesRatio( int NOM, int DENOM )
{	
...
void plotPtDensityTracksByHLT(int PTTHRESHOLD)
{
	double binwidth( 2. );
	char buffer[200];
	sprintf( buffer, "ptdensityTracks%i", PTTHRESHOLD );
	plotCanvas( buffer );
	
	sprintf( buffer, "MinBias900GeV.UE%iMeV.root", PTTHRESHOLD );
	TFile *_file1 = TFile::Open( buffer );

	_file1->cd("HLTMinBiasPixel"); TH2D* minbiaspixel = dPt_vs_ptJTransRECO->Clone();
	_file1->cd("HLTMinBiasHcal" ); TH2D* minbiashcal  = dPt_vs_ptJTransRECO->Clone();
	_file1->cd("HLTMinBiasEcal" ); TH2D* minbiasecal  = dPt_vs_ptJTransRECO->Clone();
	_file1->cd("HLTMinBias"     ); TH2D* minbias      = dPt_vs_ptJTransRECO->Clone();
	_file1->cd("HLTZeroBias"    ); TH2D* zerobias     = dPt_vs_ptJTransRECO->Clone();
	_file1->cd("HLT1jet30"      ); TH2D* hlt1jet30    = dPt_vs_ptJTransRECO->Clone();
	_file1->cd("HLT1jet50"      ); TH2D* hlt1jet50    = dPt_vs_ptJTransRECO->Clone();

	TH1D* dummy = new TH1D( "dummy", ";p_{T}(tracks jet) (GeV/c);<d#Sigmap_{T}/d#phid#eta> / 2 GeV/c (GeV/c/rad)", 100, 0., 20. );
	dummy->SetMinimum( 0. );
	dummy->SetMaximum( 0.9 );
...
//	hlt1jet50x->SetLineColor( 632 );
	...
	TH1D* gen    = getTH1DPtDensityParticles( PTTHRESHOLD );
...
void plotPtDensityTracksRatio( int NOM, int DENOM )
{	
	char buffer[200];
	sprintf( buffer, "ratioTracks%iBy%i", NOM, DENOM );
	plotCanvas( buffer );

	sprintf( buffer, ";p_{T}(tracks jet) (GeV/c);d#Sigmap_{T}(%i MeV/c) / d#Sigmap_{T}(%i GeV/c)", NOM, DENOM );
	TH1D* h_ratio = new TH1D( "h_ratio", buffer, 100, 0., 200. );

	sprintf( buffer, "MinBias900GeV.UE%iMeV.root", NOM ); TFile *_file1 = TFile::Open( buffer );
	_file1->cd("HLTZeroBias"); TH2D* nom2d = dPt_vs_ptJTransRECO->Clone(); TProfile* nom = nom2d->ProfileX()->Clone();

	sprintf( buffer, "MinBias900GeV.UE%iMeV.root", DENOM ); TFile *_file1 = TFile::Open( buffer );
	_file1->cd("HLTZeroBias"); TH2D* denom2d = dPt_vs_ptJTransRECO->Clone(); TProfile* denom = denom2d->ProfileX()->Clone();
	...
	TH1D* gen = getTH1DPtDensityParticlesRatio( NOM, DENOM );
	plotHisto( gen    , "p, e3"    );
	...
	sprintf( buffer, "#frac{d#Sigmap_{T}(p_{T}(tracks) > %i MeV/c)}{d#Sigmap_{T}(p_{T}(tracks) > %i MeV/c)}", NOM, DENOM );
...
void plotPtDensityTrackParticleRatio( int PTTHRESHOLD )
{	
	...
	TH1D* h_ratio = new TH1D( "h_ratio", ";p_{T}(charged jet) (GeV/c);#Sigmap_{T} correction", 10, 0., 20. );
