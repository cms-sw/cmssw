...
namespace mtd_digitizer {
  ...
  private :
    ...
    // implementations
    Traits::SensorPhysics deviceSim_;       // processes a given simhit into an entry in a MTDSimHitDataAccumulator
    Traits::ElectronicsSim electronicsSim_; // processes a MTDSimHitDataAccumulator into a BTLDigiCollection/ETLDigiCollection
  ...
  void MTDDigitizer<Traits>::finalizeEvent(edm::Event& e, edm::EventSetup const& c, 
    ...
    auto digiCollection = std::make_unique<Traits::DigiCollection>();
