...
CTPPSFastProtonSimulation::transportProtonTrack( const HepMC::GenParticle* in_trk, edm::DetSetVector<TotemRPRecHit>& out_hits )
{
  ...
  for ( const auto& rp : pots_ ) {
    ...
    for ( TotemRPGeometry::mapType::const_iterator it=geometry_->beginDet(); it!=geometry_->endDet(); ++it ) { if ( it->first==rp.detid ) found_detector = true; }
    if ( !found_detector ) continue; //FIXME time-consuming...
    ...
    edm::DetSet<TotemRPRecHit>& hits = out_hits.find_or_insert( rp.detid );

    // get geometry
    const CLHEP::Hep3Vector gl_o = geometry_->LocalToGlobal( rp.detid, CLHEP::Hep3Vector() );
