...
class LHCOpticsApproximator
{
  public:
    ...
    void Train( TTree *inp_tree,
                ...
                int max_degree_x=10, int max_degree_tx=10,
                int max_degree_y=10, int max_degree_ty=10,
                bool common_terms=false, double *prec=NULL );
    ...
    //void PrintCoordinateOpticalFunctions( TMultiDimFet &parametrization, const std::string& coord_name, const std::vector<std::string>& input_vars ) const;

    /*void GetLineariasedTransportMatrixX( double mad_init_x, double mad_init_thx,
                                         double mad_init_y, double mad_init_thy,
                                         double mad_init_xi,
                                         TMatrixD &tr_matrix,
                                         double d_mad_x=10e-6, double d_mad_thx=10e-6 );

    void GetLineariasedTransportMatrixY( double mad_init_x, double mad_init_thx,
                                         double mad_init_y, double mad_init_thy,
                                         double mad_init_xi,
                                         TMatrixD &tr_matrix,
                                         double d_mad_y=10e-6, double d_mad_thy=10e-6 );*/

    double GetDx( double mad_init_x, double mad_init_thx,
                  double mad_init_y, double mad_init_thy,
                  double mad_init_xi,
                  double d_mad_xi=0.001 );

    double GetDxds( double mad_init_x, double mad_init_thx,
                    double mad_init_y, double mad_init_thy,
                    double mad_init_xi,
                    double d_mad_xi=0.001 );

    std::vector<LHCApertureApproximator> GetApertures() const { return apertures_; }
    beam_type GetBeamType() const { return beam; }
  ...
  private:
    ...
    double s_end_;
    beam_type beam;
    double nominal_beam_energy_; // in GeV
    double nominal_beam_momentum_; // in GeV
    /// are the polynomials already trained?
    bool trained_;
    /// pointers to polynomials
    std::vector<TMultiDimFet*> out_polynomials;
    std::vector<std::string> coord_names;
    /// apertures on the path
