 ...
 //  produces<edm::DigiCollectionFP420>();
  ...
  //   produces<edm::DetSetVector<DigiCollectionFP420> >();
    ...
    if(verbosity>0) {
      ...
      std::cout << "DigitizerFP420: dn0=" << dn0 << " sn0=" << sn0 << " pn0=" << pn0 << std::endl;
...
//    std::cout << "DigitizerFP420 Step A done" << std::endl;
  ...
  // ==================================
	...
	for (isim=allTrackerHits->begin(); isim!= allTrackerHits->end();isim++) {
	  ...
	  int det, zside, sector, zmodule, sScale = 2*(pn0-1), dScale = 2*(pn0-1)*(sn0-1);
	  FP420NumberingScheme::unpackFP420Index(unitID, det, zside, sector, zmodule);
	  int zScale=2;  unsigned int intindex = dScale*(det - 1)+sScale*(sector - 1)+zScale*(zmodule - 1)+zside;
    if(verbosity>0) {
	  ...
	  for (int det=1; det<dn0; det++) {
		...
		for (int zside=1; zside<3; zside++) {
		  ...
		  for (;simHitIter != simHitIterEnd; ++simHitIter) {
		    ...
		    std::cout << "DigitizerFP420:check: HDigiFP420:: zside = " << zside << "  det=" << det << std::endl;
