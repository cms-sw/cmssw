///////////////////////////////////////////////////////////////////////////////
// File: HPS240DigiMain.cc
// Date: 12.2006
// Description: HPS240DigiMain for HPS240
// Modifications: 
///////////////////////////////////////////////////////////////////////////////
...
HPS240DigiMain::HPS240DigiMain(const edm::ParameterSet& conf):conf_(conf){
  std::cout << "Creating a HPS240DigiMain" << std::endl;
  ndigis=0;
  verbosity        = conf_.getUntrackedParameter<int>("VerbosityLevel");
  theElectronPerADC= conf_.getParameter<double>("ElectronHPS240PerAdc");
  theThreshold     = conf_.getParameter<double>("AdcHPS240Threshold");
  noNoise          = conf_.getParameter<bool>("NoHPS240Noise");
  addNoisyPixels   = conf_.getParameter<bool>("AddNoisyPixelsHPS240");
  thez420          = conf_.getParameter<double>("zHPS240");
  thezD2           = conf_.getParameter<double>("zHPS240D2");
  thezD3           = conf_.getParameter<double>("zHPS240D3");
  theApplyTofCut   = conf_.getParameter<bool>("ApplyTofCutHPS240");
  tofCut           = conf_.getParameter<double>("LowtofCutHPS240");
  theApplyChargeIneff= conf_.getParameter<bool>("ApplyChargeIneffHPS240");
  ...
  if(verbosity>0) {
    ...
    std::cout << "HPS240DigiMain theElectronPerADC=" << theElectronPerADC << " theThreshold=" << theThreshold << " noNoise=" << noNoise << std::endl;
  }
  ...
  if(verbosity>0) std::cout << "HPS240DigiMain moduleThickness=" << moduleThickness << std::endl;
  ...
  theZSuppressHPS240 = new ZeroSuppressHPS240(conf_,noiseRMS/theElectronPerADC); 
  thePileUpFP420 = new PileUpFP420();
  theDConverterHPS240 = new DigiConverterHPS240(theElectronPerADC,verbosity);
  
  if(verbosity>0) std::cout << "HPS240DigiMain end of constructor" << std::endl;

}


HPS240DigiMain::~HPS240DigiMain(){
  if(verbosity>0) {
    std::cout << "Destroying a HPS240DigiMain" << std::endl;
  }
  delete theGNoiseFP420;
  delete theZSuppressHPS240;
  delete theHitDigitizerFP420;
  delete thePileUpFP420;
  delete theDConverterHPS240;
...
vector <HDigiFP420> HPS240DigiMain::run(const std::vector<PSimHit> &input,
...
*/  
  ...
  for (;simHitIter != simHitIterEnd; ++simHitIter) {
    ...
    if(verbosity>1) {
      ...
      std::cout << " *******HPS240DigiMain:                           intindex= " << iu << std::endl;
  ...
  if(verbosity>0) {
      std::cout << " *******HPS240DigiMain:                           theApplyTofCut= " << theApplyTofCut << std::endl;
      std::cout << " tof= " << tof << "  EnergyLoss= " << losenergy  << "  pabs= " <<  ihit.pabs() << std::endl;
  ...
  // tofCut for HPS240 to be equal 790 ns   ;         for 420m to be = 1390 ns
    if ( ( !(theApplyTofCut)  ||  ( theApplyTofCut &&   abs(tof) > tofCut && abs(tof) < (tofCut+40.)) ) && losenergy > elossCut) {
      ...
      //    std::cout << " *******HPS240DigiMain: ERROR???  losenergy =  " <<  losenergy  << std::endl;
    }
    // main part end (AZ) 
  } //for
  // main loop end (AZ) 
  ...
  //                                                                                                                !!!!!
  push_digis(theZSuppressHPS240->zeroSuppress(theDConverterHPS240->convert(afterNoise),verbosity),
	     theLink,afterNoise);
...
void HPS240DigiMain::push_digis(const DigitalMapType& dm,
			       const HitToDigisMapType& htd,
			       const PileUpFP420::signal_map_type& afterNoise
			       )        {
  //  
  if(verbosity>0) {
    std::cout << " ****HPS240DigiMain: push_digis start: do for loop dm.size()=" <<  dm.size() << std::endl;
  }
  ...
  for ( DigitalMapType::const_iterator i=dm.begin(); i!=dm.end(); i++) {
    ...
    // very useful check:
    if(verbosity>0) {
      std::cout << " ****HPS240DigiMain:push_digis:  ndigis = " << ndigis << std::endl;
