///////////////////////////////////////////////////////////////////////////////
// File: ZeroSuppressHPS240.cc
// Date: 12.2006
// Description: ZeroSuppressHPS240 for HPS240
// Modifications: 
///////////////////////////////////////////////////////////////////////////////
#include "SimRomanPot/SimFP420/interface/ZeroSuppressHPS240.h"
#include "FWCore/MessageLogger/interface/MessageLogger.h"

ZeroSuppressHPS240::ZeroSuppressHPS240(const edm::ParameterSet& conf, float noise) : conf_(conf), theNumFEDalgos(4)
{
  noiseInAdc=noise;
  initParams(conf_);
  ...
  //initParams();
  if(verbosity>0) {
    std::cout << "ZeroSuppressHPS240: constructor: noiseInAdc=  " << noiseInAdc << std::endl;
  }
}

/*
 * The zero suppression algorithm, implemented in the trkFEDclusterizer method.
 * The class publically inherits from the ZSuppressHPS240 class, which requires 
 * the use of a method named zeroSuppress.
 */

void ZeroSuppressHPS240::initParams(edm::ParameterSet const& conf_)
{
  verbosity = conf_.getUntrackedParameter<int>("VerbosityLevel");
  algoConf      = conf_.getParameter<int>("FedHPS240Algorithm"); //: =1  (,2,3,4)
  lowthreshConf      = conf_.getParameter<double>("FedHPS240LowThreshold"); //  =3.
  highthreshConf      = conf_.getParameter<double>("FedHPS240HighThreshold"); //  =4.
  ...
  if(verbosity>0) {
    std::cout << "ZeroSuppressHPS240: initParams: !!!  theFEDalgorithm=  " << theFEDalgorithm << std::endl;
  ...
  if (theFEDalgorithm < 1 || theFEDalgorithm > theNumFEDalgos) {
    edm::LogError("HPS240DigiInfo")<<"ZeroSuppressHPS240 FATAL ERROR: Unknown zero suppress algorithm "<<theFEDalgorithm;
    exit(1);
  }
  
  //Check thresholds
  if (theFEDlowThresh > theFEDhighThresh) {
    edm::LogError("HPS240DigiInfo")<<"ZeroSuppressHPS240 FATAL ERROR: Low threshold exceeds high threshold: "<<theFEDlowThresh<<" > "<<theFEDhighThresh;
    exit(2);
  }
}

ZSuppressHPS240::DigitalMapType ZeroSuppressHPS240::zeroSuppress(const DigitalMapType& notZeroSuppressedMap,int vrb)
{
...
ZSuppressHPS240::DigitalMapType ZeroSuppressHPS240::trkFEDclusterizer(const DigitalMapType &in, int vrb) 
{
  const std::string s2("ZeroSuppressHPS240::trkFEDclusterizer1");
