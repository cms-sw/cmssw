...
GEMNoiseDetailed::GEMNoiseDetailed(const edm::ParameterSet& config)
  : GEMNoise(config)
{
  std::cout << ">>> Using noise model: GEMNoiseDetailed" << std::endl;
...
GEMNoiseDetailed::~GEMNoiseDetailed()
{
...
void
GEMNoiseDetailed::setRandomEngine(CLHEP::HepRandomEngine& eng) 
{
...
void 
GEMNoiseDetailed::setUp(std::vector<GEMStripNoise::StripNoiseItem> vNoise)
{
  noiseRateMap_.clear();
  ...
  for(const auto &det: getGeometry()->dets())
  {
	...
	<< "GEMNoiseDetailed::setUp() - number of strips per partition in configuration ("
...
GEMNoiseDetailed::simulateNoise(const GEMEtaPartition* roll) 
{
