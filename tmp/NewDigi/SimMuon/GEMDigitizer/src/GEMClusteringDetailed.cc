...
GEMClusteringDetailed::GEMClusteringDetailed(const edm::ParameterSet& config)
  : GEMClustering(config)
  , poisson_(0)

{
  std::cout << ">>> Using clustering model: GEMClusteringDetailed" << std::endl;
...
GEMClusteringDetailed::~GEMClusteringDetailed()
{
...
void 
GEMClusteringDetailed::setRandomEngine(CLHEP::HepRandomEngine& eng)
{
...
void 
GEMClusteringDetailed::setUp(std::vector<GEMStripClustering::StripClusteringItem> vCluster)
{
  ...
  for(const auto &det: getGeometry()->dets())
  {
	...
	<< "GEMClusteringDetailed::setUp() - number of strips per partition in configuration ("
...
GEMClusteringDetailed::simulateClustering(const GEMEtaPartition* roll, const PSimHit* simHit, const int bx)
{
