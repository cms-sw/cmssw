...
GEMTimingSimple::GEMTimingSimple(const edm::ParameterSet& config)
  : GEMTiming(config)
  , gauss1_(0)
  , gauss2_(0)
 {
   std::cout << ">>> Using timing model: GEMTimingSimple" << std::endl;
...
GEMTimingSimple::~GEMTimingSimple()
{
...
void 
GEMTimingSimple::setRandomEngine(CLHEP::HepRandomEngine& eng)
{
...
void 
GEMTimingSimple::setUp(std::vector<GEMStripTiming::StripTimingItem> timingVector)
{
  ...
  for(const auto &det: getGeometry()->dets())
  {
	...
	<< "GEMTimingSimple::setUp() - number of strips per partition in configuration ("
...
const int 
GEMTimingSimple::getSimHitBx(const PSimHit* simhit)
{
  ...
  if (roll->id().region() == 0)
  {
    throw cms::Exception("Geometry")
      << "GEMTimingSimple::getSimHitBx() - this GEM id is from barrel, which cannot happen:  " << roll->id() << "\n";
  }
