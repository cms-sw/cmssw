...
/**\class edm::CallNTimesNoWait CallNTimesNoWait.h "CallNTimesNoWait.h"
 ...
 Usage:
    This class allows one to safely do a 'non-side-effect' operation N times in
 a job.
 ...
 \endcode
 The important thing to remember, is there is no guarantee that the operation
 being run
 finishes before a thread which doesn't get to run the operation reaches the
 code following
 the call. Therefore it is useful to suppress messages but should not be used to
 do something
 like filling a container with values since the filling is not guaranteed to
 complete before
...
class CallNTimesNoWait {
 public:
  CallNTimesNoWait(unsigned short iNTimes)
      : m_ntimes(static_cast<int>(iNTimes) - 1), m_done(false) {}
