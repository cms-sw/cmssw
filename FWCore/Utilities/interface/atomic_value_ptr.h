...
class atomic_value_ptr {
 public:
  ...
  atomic_value_ptr(atomic_value_ptr&& orig) : myP(orig.myP) {
    orig.myP.store(nullptr);
  }
  ...
  atomic_value_ptr(std::unique_ptr<T> orig) : myP(orig.release()) {
    orig = nullptr;
  }
 ...
 public:
  operator bool_type() const {
    return myP != nullptr
               ? &atomic_value_ptr<T>::this_type_does_not_support_comparisons
               : nullptr;
  }
