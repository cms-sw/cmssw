...
/**\class TypeIDBase TypeIDBase.h FWCore/Utilities/interface/TypeIDBase.h
 ...
 Usage:
    This class is not meant to be used polymorphically (which is why there is no
 virtual destructor).
 Instead it is used to hold a common methods needed by all type comparing
 classes.
...
class TypeIDBase {
 ...
 private:
  ...
  //  We also are using a pointer rather than a reference so that operator= will
  //  work
