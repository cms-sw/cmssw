...
struct is_derived_or_same {
  static bool const value = boost::is_base_and_derived<BASE, DERIVED>::value ||
...
class dso_export Exception : public std::exception {
 public:
  ...
  // The signature for what() must be identical to that of
  // std::exception::what().
  ...
  friend typename detail::Desired<
      E, detail::is_derived_or_same<Exception, E>::value>::type&
  operator<<(E& e, T const& stuff);

  template <typename E, typename T>
  friend typename detail::Desired<
      E, detail::is_derived_or_same<Exception, E>::value>::type const&
  ...
  //     template <typename E>
  //     friend
  //     typename detail::Desired<E,
  //     (boost::is_base_and_derived<Exception,E>::value ||
...
template <typename E, typename T>
inline typename detail::Desired<
    E, detail::is_derived_or_same<Exception, E>::value>::type&
operator<<(E& e, T const& stuff) {
  e.ost_ << stuff;
  return e;
...
// inline
// typename detail::Desired<E,
// detail::is_derived_or_same<Exception,E>::value>::type &
...
// inline
// typename detail::Desired<E,
// detail::is_derived_or_same<Exception,E>::value>::type const&
