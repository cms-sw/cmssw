...
/**Given a leading memory size, iSizeSoFar, and an alignment boundary
 requirement of iBoundary, returns how much additional memory padding is needed.
 This function assumes that when iSizeSoFar==0 that we are already properly
 aligned.*/
constexpr unsigned int padding_needed(size_t iSizeSoFar,
                                      unsigned int iBoundary) {
  return (iBoundary - iSizeSoFar % iBoundary) % iBoundary;
}

/**
 Given a starting value of I=0, and the index to the argument you want to get,
 J,
 will recursively step through the arguments, Args, until it hits the one you
 want, Jth, and then will return it as return type Ret.
 */

template <unsigned int I, unsigned int J, typename Ret, typename... Args>
struct arg_puller;

/**Given a variable number of arguments, returns the 'J'th one. The first caller
 * must use I=0.*/
template <unsigned int I, unsigned int J, typename Ret, typename F,
          typename... Args>
...
/** Class used by SoATupleHelper to determine the proper alignment of the
 requested type.
 The default is to just use 'alignof(T)'. Users can change the alignment by
 instead using
...
/** Specialization of ALignmentHelper for Aligned<T, ALIGNMENT>. This allows
 users
...
/**Implements most of the internal functions used by SoATuple. The argument I is
 used to recursively step
 through each arugment Args when doing the work. SoATupleHelper<I,Args> actually
 operates on the I-1 argument.
 There is a specialization of SoATulpeHelper with I=0 which is used to stop the
 template recursion.
 */
template <unsigned int I, typename... Args>
struct SoATupleHelper {
  typedef AlignmentHelper<
      typename std::tuple_element<I - 1, std::tuple<Args...>>::type>
      AlignmentInfo;
  ...
  static const std::size_t max_alignment =
      AlignmentInfo::kAlignment > NextHelper::max_alignment
          ? AlignmentInfo::kAlignment
          : NextHelper::max_alignment;

  // ---------- static member functions --------------------
  static size_t moveToNew(char* iNewMemory, size_t iSize, size_t iReserve,
                          void** oToSet);
  static size_t copyToNew(char* iNewMemory, size_t iSize, size_t iReserve,
                          void* const* iFrom, void** oToSet);
  static size_t spaceNeededFor(unsigned int iNElements);
  static void push_back(void** iToSet, size_t iSize,
  ...
  const SoATupleHelper& operator=(const SoATupleHelper&) =
      delete;  // stop default
};
...
struct SoATupleHelper<0, Args...> {
  ...
  static size_t moveToNew(char* /*iNewMemory*/, size_t /*iSize*/,
                          size_t /*iReserve*/, void** /*oToSet*/) {
    return 0;
  }

  static size_t copyToNew(char* /*iNewMemory*/, size_t /*iSize*/,
                          size_t /*iReserve*/, void* const* /*iFrom*/,
                          void** /*oToSet*/) {
    return 0;
  }
};
...
template <unsigned int I, typename... Args>
size_t SoATupleHelper<I, Args...>::moveToNew(char* iNewMemory, size_t iSize,
                                             size_t iReserve, void** oToSet) {
  ...
  Type* newStart = reinterpret_cast<Type*>(iNewMemory + usedSoFar +
                                           padding_needed(usedSoFar, boundary));
  ...
  *oldStart = newStart;
  unsigned int additionalSize =
      padding_needed(usedSoFar, boundary) + iReserve * sizeof(Type);
  return usedSoFar + additionalSize;
}

template <unsigned int I, typename... Args>
size_t SoATupleHelper<I, Args...>::copyToNew(char* iNewMemory, size_t iSize,
                                             size_t iReserve,
                                             void* const* iFrom,
                                             void** oToSet) {
  size_t usedSoFar =
      NextHelper::copyToNew(iNewMemory, iSize, iReserve, iFrom, oToSet);
...
size_t SoATupleHelper<I, Args...>::spaceNeededFor(unsigned int iNElements) {
  ...
  const unsigned int boundary = AlignmentInfo::kAlignment;
  unsigned int additionalSize =
      padding_needed(usedSoFar, boundary) + iNElements * sizeof(Type);
  return usedSoFar + additionalSize;
}

template <unsigned int I, typename... Args>
void SoATupleHelper<I, Args...>::push_back(void** iToSet, size_t iSize,
                                           std::tuple<Args...> const& iValues) {
...
template <unsigned int I, typename... Args>
template <typename... FArgs>
void SoATupleHelper<I, Args...>::emplace_back(void** iToSet, size_t iSize,
                                              FArgs... iValues) {
  new (static_cast<Type*>(*(iToSet + I - 1)) + iSize)
      Type(arg_puller<0, I - 1, Type const&, FArgs...>::pull(
          std::forward<FArgs>(iValues)...));
