...
class InputTag {
 public:
  InputTag();
  InputTag(std::string const& label, std::string const& instance,
           std::string const& processName = "");
  InputTag(char const* label, char const* instance,
  ...
  ProductResolverIndex indexFor(TypeID const& typeID, BranchType branchType,
                                void const* productRegistry) const;

  void tryToCacheIndex(ProductResolverIndex index, TypeID const& typeID,
