...
namespace edm {
  ...
  public:
    ...
    SoATuple(const SoATuple<Args...>& iOther) : m_size(0), m_reserved(0) {
      ...
      soahelper::SoATupleHelper<sizeof...(Args), Args...>::copyToNew(static_cast<char*>(m_values[0]), iOther.m_size,
                                                                     m_reserved, iOther.m_values, m_values);
    ...
    /** Adds one entry to the end of the list. The arguments are used to instantiate each data element in the order
     * defined in the template arguments.*/
    template <typename... FArgs>
    void emplace_back(FArgs&&... values) {
      if (size() + 1 > capacity()) {
        reserve(size() * 2 + 1);
      }
      soahelper::SoATupleHelper<sizeof...(Args), Args...>::emplace_back(m_values, m_size,
                                                                        std::forward<FArgs>(values)...);
      ++m_size;
    }
