...
class VecArray {
 ...
 public:
  ...
  void push_back(const T& value) {
    if (size_ >= N)
      throw std::length_error(
          "push_back on already-full VecArray (N=" + std::to_string(N) + ")");
  ...
  void resize(unsigned int size) {
    if (size > N)
      throw std::length_error("Requesting size " + std::to_string(size) +
                              " while maximum allowed is " + std::to_string(N));

    while (size < size_) pop_back();
    size_ = size;
  }
