...
static __inline__ unsigned long long rdtsc(void) {
  // We will be reading CNTVCT_EL0 (the virtual counter), which is prepared for
  // us by OS.
  // The system counter sits outside multiprocessor in SOC and runs on a
  // different frequency.
  ...
  // agent
  // that indicates that CNTVCT_EL0 must be read, an ISB is used to ensure that
  // the
