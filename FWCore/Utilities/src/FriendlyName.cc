...
// NOTE:  This should probably be rewritten so that we break the class name into a tree where the template arguments are
// the node.  On the way down the tree
// we look for '<' or ',' and on the way up (caused by finding a '>') we can apply the transformation to the output
// string based on the class name for the
// templated class.  Up front we'd register a class name to a transformation function (which would probably take a
// std::vector<std::string> which holds
...
namespace edm {
  namespace friendlyname {
    ...
    static std::regex const reAIKR(", *edm::helper::AssociationIdenticalKeyReference");  // this is a default so can
                                                                                         // replaced with empty
    ...
    // NOTE: the '?' means make the smallest match. This may lead to problems where the template arguments themselves
    // have commas
