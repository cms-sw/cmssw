...
Map typeMap;
typedef tbb::concurrent_unordered_map<std::string, FunctionWithDict>
    FunctionMap;
FunctionMap functionMap;
}
static void throwTypeException(std::string const& function,
...
TypeWithDict TypeWithDict::byName(std::string const& name, long property) {
  ...
  if (name.back() == ']') {
    ...
    ret.arrayDimensions_ =
        value_ptr<std::vector<size_t> >(new std::vector<size_t>);
...
TypeWithDict::TypeWithDict(std::type_info const& ti, long property /*= 0L*/)
  ...
  // Must be done before dataType_ is checked, because dataType_ will be filled
  // for char*
  ...
  if (ti == typeid(void)) {
    // For some reason, "void" has a data type if accessed by name, but not by
    // type_info.
...
bool TypeWithDict::isEnum() const {
...
bool TypeWithDict::isReference() const {
  return (property_ & (long)kIsReference);
}

bool TypeWithDict::isTemplateInstance() const {
...
std::string TypeWithDict::unscopedName() const {
...
std::string TypeWithDict::friendlyClassName() const {
...
void const* TypeWithDict::pointerToBaseType(
    void const* ptr, TypeWithDict const& derivedType) const {
  if (!isClass()) {
    return ptr;
  }
...
void const* TypeWithDict::pointerToContainedType(
    void const* ptr, TypeWithDict const& derivedType) const {
  if (!isClass()) {
    return ptr;
  }
...
FunctionWithDict TypeWithDict::functionMemberByName(
    std::string const& member) const {
  if (!isClass()) {
...
FunctionWithDict TypeWithDict::functionMemberByName(
    std::string const& functionName, std::string const& proto,
    bool isConst) const {
  if (!isClass()) {
    return FunctionWithDict();
  }
  ...
  TMethod* meth = class_->GetMethodWithPrototype(
      functionName.c_str(), proto.c_str(), /*objectIsConst=*/isConst,
...
void TypeWithDict::deallocate(void* address) const {
...
bool operator==(TypeWithDict const& a, std::type_info const& b) {
  if (*a.ti_ == typeid(TypeWithDict::dummyType) || a.isPointer() ||
      a.isArray()) {
    // No accurate type_info
...
IterWithDict<TBaseClass> TypeBases::end() const {
  return IterWithDict<TBaseClass>();
}
...
IterWithDict<TDataMember> TypeDataMembers::end() const {
  return IterWithDict<TDataMember>();
}
...
IterWithDict<TMethod> TypeFunctionMembers::end() const {
  return IterWithDict<TMethod>();
}
