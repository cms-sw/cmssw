...
TypeWithDict FunctionWithDict::finalReturnType() const {
  ...
  TypeWithDict theType =
      TypeWithDict::byName(function_->GetReturnTypeNormalizedName());
...
bool FunctionWithDict::isConst() const {
...
bool FunctionWithDict::isDestructor() const {
  return function_->ExtraProperty() & kIsDestructor;
}

bool FunctionWithDict::isOperator() const {
  return function_->ExtraProperty() & kIsOperator;
}

bool FunctionWithDict::isPublic() const {
...
size_t FunctionWithDict::functionParameterSize(
...
void FunctionWithDict::invoke(
    ObjectWithDict const& obj, ObjectWithDict* ret /*=nullptr*/,
...
IterWithDict<TMethodArg> FunctionWithDict::end() const {
  return IterWithDict<TMethodArg>();
}
