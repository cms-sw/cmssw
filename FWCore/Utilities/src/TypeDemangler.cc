...
/********************************************************************
        TypeDemangler is used to demangle the mangled name provided by type_info
into the type name.

        The conventions used (e.g. const qualifiers before identifiers, no
spaces after commas)
        are chosen to match the type names that can be used by the plug-in
manager to load data dictionaries.
        It also strips comparators from (multi) maps or sets, and always strips
allocators.
        ...
        2) If an enum value is used as a non-type template parameter, the
demangled name cannot
        be used successfully to load the dictionary.  This is because the
...
void removeParameter(std::string& demangledName, std::string const& toRemove) {
  ...
  while ((index = demangledName.find(toRemove)) != std::string::npos) {
    ...
    while ((inx = demangledName.find_first_of(delimiters, inx)) !=
           std::string::npos) {
...
std::string typeDemangle(char const* mangledName) {
  ...
  // The demangled C style string is allocated with malloc, so it must be
  // deleted with free().
