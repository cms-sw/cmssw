...
DecomposedReleaseVersion::DecomposedReleaseVersion(std::string releaseVersion)
    : irregular_(true),
      major_(0),
      minor_(0) /*, point_(0), patch_(0), pre_(0)*/ {
  try {
    std::vector<std::string> parts;
    parts.reserve(releaseVersion.size());
    boost::algorithm::split(parts, releaseVersion, IsNotDigit(),
...
bool DecomposedReleaseVersion::operator<(
    DecomposedReleaseVersion const& other) const {
