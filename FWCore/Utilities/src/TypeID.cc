...
std::string const& TypeID::className() const {
  typedef tbb::concurrent_unordered_map<edm::TypeID, std::string, TypeIDHasher>
      Map;
  ...
  if (s_typeToName.end() == itFound) {
    itFound =
        s_typeToName.insert(Map::value_type(*this, typeToClassName(typeInfo())))
            .first;
  }
...
std::string TypeID::friendlyClassName() const {
