...
InputTag::InputTag(char const* label, char const* instance,
...
std::string InputTag::encode() const {
  // NOTE: since the encoding gets used to form the configuration hash I did not
  // want
...
void InputTag::tryToCacheIndex(ProductResolverIndex index, TypeID const& typeID,
  ...
  unsigned int invalidValue =
      static_cast<unsigned int>(ProductResolverIndexInvalid);
