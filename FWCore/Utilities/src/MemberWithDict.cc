...
TypeWithDict MemberWithDict::declaringType() const {
  return TypeWithDict(dataMember_->GetClass());
}

bool MemberWithDict::isArray() const {
  return dataMember_->Property() & kIsArray;
}

bool MemberWithDict::isConst() const {
  return dataMember_->Property() & kIsConstant;
}

bool MemberWithDict::isPublic() const {
  return dataMember_->Property() & kIsPublic;
}

bool MemberWithDict::isStatic() const {
  return dataMember_->Property() & kIsStatic;
}

bool MemberWithDict::isTransient() const {
...
ObjectWithDict MemberWithDict::get() const {
  return ObjectWithDict(typeOf(),
                        reinterpret_cast<void*>(dataMember_->GetOffset()));
}

ObjectWithDict MemberWithDict::get(ObjectWithDict const& obj) const {
  return ObjectWithDict(
      typeOf(),
      reinterpret_cast<char*>(obj.address()) + dataMember_->GetOffset());
}
