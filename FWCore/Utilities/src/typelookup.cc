...
struct select_size_t_constant {
  // Explicit cast is needed to avoid compiler warnings about possible
  // truncation.
  // The value of the right size,   which is selected by ?:, is anyway not
  // truncated or promoted.
  static constexpr size_t value =
      (size_t)((sizeof(size_t) == sizeof(u)) ? u : ull);
};

constexpr size_t hash_multiplier =
    select_size_t_constant<2654435769U, 11400714819323198485ULL>::value;
...
using TypeNameToValueMap =
    tbb::concurrent_unordered_map<const char*, const std::type_info*,
...
edm::typelookup::NameRegistrar::NameRegistrar(const char* iTypeName,
  ...
  typeNameToValueMap().insert(
      std::pair<const char*, const std::type_info*>(iTypeName, &iInfo));
}

std::pair<const char*, const std::type_info*> edm::typelookup::findType(
    const char* iTypeName) {
  auto itFind = typeNameToValueMap().find(iTypeName);

  if (itFind == typeNameToValueMap().end()) {
    return std::make_pair(static_cast<const char*>(0),
                          static_cast<std::type_info*>(0));
  }
