...
namespace edm {
  ...
  S& print(S& os, JobReport::InputFile const& f) {
    ...
    os << "\n<InputSourceClass>" << TiXmlText(f.inputSourceClassName) << "</InputSourceClass>";
  ...
  S& print(S& os, JobReport::OutputFile const& f) {
    formatFile(f, os);
    os << "\n<OutputModuleClass>" << TiXmlText(f.outputModuleClassName) << "</OutputModuleClass>";
  ...
  void JobReport::JobReportImpl::associateLumiSection(JobReport::Token token, unsigned int runNumber,
                                                      unsigned int lumiSect, unsigned long nEvents) {
    auto& theMap = outputFiles_.at(token).runReports;
    std::map<RunNumber, RunReport>::iterator iter(theMap.lower_bound(runNumber));
    if (iter == theMap.end() || runNumber < iter->first) {                                             // not found
  ...
  JobReport::Token JobReport::inputFileOpened(std::string const& physicalFileName, std::string const& logicalFileName,
  ...
  JobReport::Token JobReport::outputFileOpened(std::string const& physicalFileName, std::string const& logicalFileName,
  ...
  void JobReport::reportFastCopyingStatus(JobReport::Token fileToken, std::string const& inputFileName,
                                          bool fastCopying) {
    JobReport::OutputFile& f = impl_->getOutputFileForToken(fileToken);
    f.fastCopyingInputs.insert(std::make_pair(inputFileName, fastCopying));
  }

  void JobReport::reportLumiSection(JobReport::Token token, unsigned int run, unsigned int lumiSectId,
  ...
  void JobReport::reportPerformanceForModule(std::string const& metricClass, std::string const& moduleName,
                                             std::map<std::string, std::string> const& metrics) {
    if (impl_->ost_) {
      std::ostream& msg = *(impl_->ost_);
      msg << "<PerformanceReport>\n"
