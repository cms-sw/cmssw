...
//                generic os<< lines in
//                JobReport::JobReportImpl::writeOutputFile
...
//
// 24 June 2008   ewv  Correct format for CDATA and for second instance of
// reportError
...
S& print(S& os, JobReport::OutputFile const& f) {
  formatFile(f, os);
  os << "\n<OutputModuleClass>" << TiXmlText(f.outputModuleClassName)
     << "</OutputModuleClass>";
...
JobReport::InputFile& JobReport::JobReportImpl::getInputFileForToken(
    InputType inputType, JobReport::Token t) {
  InputFile* inputFile = nullptr;
  if (inputType == InputType::SecondarySource) {
    if (t >= inputFilesSecSource_.size()) {
      throw edm::Exception(edm::errors::LogicError)
          << "Access reported for secondary source input file with token " << t
...
JobReport::OutputFile& JobReport::JobReportImpl::getOutputFileForToken(
    JobReport::Token t) {
 ...
 */
void JobReport::JobReportImpl::insertInputForOutputs(InputType inputType,
                                                     JobReport::Token t) {
...
void JobReport::JobReportImpl::associateRun(JobReport::Token token,
  ...
  if (iter == theMap.end() || runNumber < iter->first) {  // not found
    theMap.emplace_hint(iter, runNumber,
                        JobReport::RunReport{runNumber, {}});  // insert it
  }
}
...
void JobReport::JobReportImpl::associateInputRun(unsigned int runNumber) {
  for (auto& inputFile : inputFiles_) {
    if (!inputFile.fileHasBeenClosed) {
      ...
      std::map<RunNumber, RunReport>::iterator iter(
          theMap.lower_bound(runNumber));
      if (iter == theMap.end() || runNumber < iter->first) {  // not found
        theMap.emplace_hint(iter, runNumber,
                            JobReport::RunReport{runNumber, {}});  // insert it
      }
...
void JobReport::JobReportImpl::associateLumiSection(JobReport::Token token,
  ...
  if (iter == theMap.end() || runNumber < iter->first) {  // not found
    theMap.emplace_hint(
        iter, runNumber,
        JobReport::RunReport{runNumber, {{{lumiSect, nEvents}}}});  // insert it
  } else {
    iter->second.lumiSectionsToNEvents[lumiSect] += nEvents;
  }
}

void JobReport::JobReportImpl::associateInputLumiSection(
    unsigned int runNumber, unsigned int lumiSect) {
  for (auto& inputFile : inputFiles_) {
    if (!inputFile.fileHasBeenClosed) {
      std::map<RunNumber, RunReport>& theMap = inputFile.runReports;
      std::map<RunNumber, RunReport>::iterator iter(
      ...
      if (iter == theMap.end() || runNumber < iter->first) {  // not found
        theMap.emplace_hint(
            ...
            JobReport::RunReport{
                runNumber,
                {{lumiSect,
                  std::numeric_limits<unsigned long>::max()}}});  // insert it
      } else {
        iter->second.lumiSectionsToNEvents[lumiSect] =
...
JobReport::JobReport() : impl_(new JobReportImpl(0)) {}
...
JobReport::Token JobReport::inputFileOpened(
    std::string const& physicalFileName, std::string const& logicalFileName,
    std::string const& catalog, std::string const& inputType,
    std::string const& inputSourceClassName, std::string const& moduleLabel,
    std::string const& guid, std::vector<std::string> const& branchNames) {
...
void JobReport::eventReadFromFile(InputType inputType,
...
void JobReport::inputFileClosed(InputType inputType,
                                JobReport::Token fileToken) {
  JobReport::InputFile& f = impl_->getInputFileForToken(inputType, fileToken);
...
JobReport::Token JobReport::outputFileOpened(
    std::string const& physicalFileName, std::string const& logicalFileName,
    std::string const& catalog, std::string const& outputModuleClassName,
    std::string const& moduleLabel, std::string const& guid,
    std::string const& dataType, std::string const& branchHash,
  ...
  //
  for (std::vector<Token>::size_type i = 0, iEnd = impl_->inputFiles_.size();
       i < iEnd; ++i) {
    if (!impl_->inputFiles_[i].fileHasBeenClosed) {
      r.contributingInputs.push_back(i);
    }
  }
  for (tbb::concurrent_vector<Token>::size_type
           i = 0,
           iEnd = impl_->inputFilesSecSource_.size();
       i < iEnd; ++i) {
...
void JobReport::reportLumiSection(JobReport::Token token, unsigned int run,
...
void JobReport::reportRunNumber(JobReport::Token token, unsigned int run) {
...
void JobReport::reportAnalysisFile(
  ...
  if (impl_->ost_) {
    std::ostream& msg = *(impl_->ost_);
    {
      ...
      for (const_iterator pos = fileData.begin(), posEnd = fileData.end();
           pos != posEnd; ++pos) {
...
void JobReport::reportError(std::string const& shortDesc,
                            std::string const& longDesc, int const& exitCode) {
  if (impl_->ost_) {
    {
      std::lock_guard<std::mutex> lock(write_mutex);
      std::ostream& msg = *(impl_->ost_);
      msg << "<FrameworkError ExitStatus=\"" << exitCode << "\" Type=\""
          << shortDesc << "\" >\n";
      msg << "<![CDATA[\n" << longDesc << "\n]]>\n";
      msg << "</FrameworkError>\n";
      msg << std::flush;
...
void JobReport::reportMemoryInfo(std::vector<std::string> const& memoryData) {
  if (impl_->ost_) {
    ...
    for (const_iterator pos = memoryData.begin(), posEnd = memoryData.end();
         pos != posEnd; ++pos) {
      msg << *pos << "\n";
    }
    msg << "</MemoryService>\n";
...
void JobReport::reportMessageInfo(
    std::map<std::string, double> const& messageData) {
  if (impl_->ost_) {
    std::ostream& msg = *(impl_->ost_);
    msg << "<MessageSummary>\n";
    typedef std::map<std::string, double>::const_iterator const_iterator;
    for (const_iterator pos = messageData.begin(), posEnd = messageData.end();
         pos != posEnd; ++pos) {
      msg << "  <" << pos->first << "  Value=\"" << pos->second << "\" />"
          << "\n";
    }
...
void JobReport::reportReadBranch(InputType inputType,
                                 std::string const& branchName) {
  if (inputType == InputType::Primary) {
    // Fast cloned branches have already been reported.
    std::set<std::string> const& clonedBranches =
        impl_->inputFiles_.at(impl_->lastOpenedPrimaryInputFile_)
            .fastClonedBranches;
...
void JobReport::reportFastClonedBranches(
    std::set<std::string> const& fastClonedBranches, long long nEvents) {
  std::set<std::string>& clonedBranches =
      impl_->inputFiles_.at(impl_->lastOpenedPrimaryInputFile_)
          .fastClonedBranches;
  for (std::set<std::string>::const_iterator it = fastClonedBranches.begin(),
                                             itEnd = fastClonedBranches.end();
       it != itEnd; ++it) {
...
void JobReport::reportPerformanceSummary(
    std::string const& metricClass,
  ...
  if (impl_->ost_) {
    ...
    typedef std::map<std::string, std::string>::const_iterator const_iterator;
    for (const_iterator iter = metrics.begin(), iterEnd = metrics.end();
         iter != iterEnd; ++iter) {
...
void JobReport::reportPerformanceForModule(
    std::string const& metricClass, std::string const& moduleName,
    std::map<std::string, std::string> const& metrics) {
  if (impl_->ost_) {
    std::ostream& msg = *(impl_->ost_);
    msg << "<PerformanceReport>\n"
