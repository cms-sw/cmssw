...
class AtomicUnsignedInt {
 public:
  ...
  AtomicUnsignedInt(AtomicUnsignedInt const& r)
      : value_(r.value_.load(std::memory_order_acquire)) {}
...
[[cms::thread_safe]] static std::vector<tbb::concurrent_unordered_map<
    ErrorSummaryMapKey, AtomicUnsignedInt, ErrorSummaryMapKey::key_hash>>
    errorSummaryMaps;

MessageSender::MessageSender(ELseverityLevel const& sev, ELstring const& id,
                             bool verbatim, bool suppressed)
    : errorobj_p(suppressed ? nullptr : new ErrorObj(sev, id, verbatim),
                 ErrorObjDeleter()) {
  ...
  try {
    ...
    if (errorSummaryIsBeingKept.load(std::memory_order_acquire) &&
      ...
      if (i != errorSummaryMap.end()) {
        i->second.value().fetch_add(
            1, std::memory_order_acq_rel);  // same as ++errorSummaryMap[key]
      } else {
...
std::vector<ErrorSummaryEntry> LoggedErrorsOnlySummary(
    unsigned int iStreamID) {  //  ChangeLog 2
  std::vector<ErrorSummaryEntry> v;
  assert(iStreamID < errorSummaryMaps.size());
  auto const& errorSummaryMap = errorSummaryMaps[iStreamID];
  auto end = errorSummaryMap.end();
