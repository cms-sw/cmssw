...
class StandAloneScribe : public edm::service::AbstractMLscribe {
 public:
  ...
  // ---------- member functions ---------------------------
  virtual void runCommand(edm::MessageLoggerQ::OpCode opcode,
                          void *operand) override;
...
void StandAloneScribe::runCommand(edm::MessageLoggerQ::OpCode opcode,
                                  void *operand) {
...
std::shared_ptr<edm::service::AbstractMLscribe> MessageLoggerQ::mlscribe_ptr =
...
void MessageLoggerQ::setMLscribe_ptr(
...
void MessageLoggerQ::simpleCommand(OpCode opcode,
                                   void *operand)  // changeLog 8, 10
{
  mlscribe_ptr->runCommand(opcode, operand);
}  // simpleCommand

void MessageLoggerQ::handshakedCommand(
    OpCode opcode, void *operand,
  ...
  try {
    ...
    std::cerr << "exception from MessageLoggerQ::" << commandMnemonic
              << " - exception what() is \n"
              << ex.what();
...
bool MessageLoggerQ::handshaked(
    MessageLoggerQ::OpCode const &op)  // changeLog 9
{
