...
Changes Log 1: 2009/01/14 10:29:00, Natalia Garcia Nebot
        Modified and added some methods to report CPU and memory information
from /proc/cpuinfo
...
class JobReport {
  ...
  */
  ...
  struct JobReportImpl {
     ...
     *
     */
    void associateInputLumiSection(unsigned int runNumber,
    ...
    JobReportImpl(std::ostream* iOst)
        : printedReadBranches_(false), ost_(iOst) {}
    ...
    tbb::concurrent_unordered_map<std::string, AtomicLongLong>
        readBranchesSecSource_;
  ...
  Token outputFileOpened(std::string const& physicalFileName,
                         ...
                         std::string const& guid, std::string const& dataType,
  ...
  // CMS-THREADING Current implementation requires an instance of an
  // OuputModule run on only one thread at a time.
  void eventWrittenToFile(Token fileToken, RunNumber_t run,
                          EventNumber_t event);
  ...
  /// the output file
  ///
  void reportLumiSection(JobReport::Token token, unsigned int run,
                         unsigned int lumiSectId, unsigned long nEvents = 0);
   ...
   */
  void reportFastCopyingStatus(Token t, std::string const& inputFileName,
                               bool fastCopying);
