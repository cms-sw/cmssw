...
namespace edm {
  ...
  public:
    explicit LogInfo(std::string const& id)
        : ap(ELinfo, id, false, (MessageDrop::infoAlwaysSuppressed || !MessageDrop::instance()->infoEnabled))  // Change
                                                                                                               // log 21
    {}
  ...
  public:
    explicit LogVerbatim(std::string const& id)
        : ap(ELinfo, id, true, (MessageDrop::infoAlwaysSuppressed || !MessageDrop::instance()->infoEnabled))  // Change
                                                                                                              // log 21
    {}
...
// The preprocessor symbol controlling suppression of LogDebug is EDM_ML_DEBUG.  Thus by default (BEHAVIOR CHANGE)
// LogDebug is If LogDebug is suppressed, all code past the LogDebug(...) is squelched. See doc/suppression.txt.
