...
//                  The problem was that Suppress_LogDebug_ was missing the
//                  operator<<
...
class LogWarning {
 public:
  explicit LogWarning(std::string const& id)
      : ap(ELwarning, id, false,
           (MessageDrop::warningAlwaysSuppressed ||
            !MessageDrop::instance()->warningEnabled))  // Change log 21
  {}
...
class LogError {
 public:
  explicit LogError(std::string const& id)
      : ap(ELerror, id, false,
           !MessageDrop::instance()->errorEnabled)  // Change log 24
  {}
...
class LogInfo {
 public:
  explicit LogInfo(std::string const& id)
      : ap(ELinfo, id, false,
           (MessageDrop::infoAlwaysSuppressed ||
            !MessageDrop::instance()->infoEnabled))  // Change log 21
  {}
...
class LogDebug_ {
 public:
  LogDebug_() : ap() {}
  explicit LogDebug_(std::string const& id, std::string const& file,
                     int line);  // Change log 17
  ~LogDebug_();
...
class LogWarningThatSuppressesLikeLogInfo {
 public:
  explicit LogWarningThatSuppressesLikeLogInfo(std::string const& id)
      : ap(ELwarning, id, false,
           (MessageDrop::infoAlwaysSuppressed ||
            !MessageDrop::instance()->warningEnabled))  // Change log 22
  {}
  ...
  LogWarningThatSuppressesLikeLogInfo& operator<<(
 ...
 private:
  MessageSender ap;
  LogWarningThatSuppressesLikeLogInfo(
      LogWarningThatSuppressesLikeLogInfo const&);  // Change log 9
  LogWarningThatSuppressesLikeLogInfo& operator=(
      LogWarningThatSuppressesLikeLogInfo const&);
...
// The preprocessor symbol controlling suppression of LogDebug is EDM_ML_DEBUG.
// Thus by default (BEHAVIOR CHANGE) LogDebug is
...
#define IfLogDebug(cond, cat) \
  if (cond) LogDebug(cat)
#define IfLogTrace(cond, cat) \
  if (cond) LogTrace(cat)
#else
