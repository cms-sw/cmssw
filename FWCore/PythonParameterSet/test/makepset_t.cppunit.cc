...
void testmakepset::secsourceTest() {
  try {
    ...
    std::cerr
        << "testmakepset::secsourceTest() caught an unidentified exception\n";
    throw;
  }
}
...
void testmakepset::secsourceAux() {
  ...
  edm::ParameterSet const& secondarySourceParams =
      mixingModuleParams.getParameterSet("input");
  CPPUNIT_ASSERT(secondarySourceParams.getParameter<std::string>(
                     "@module_type") == "EmbeddedRootSource");
  CPPUNIT_ASSERT(secondarySourceParams.getParameter<std::string>(
                     "@module_label") == "input");
  CPPUNIT_ASSERT(
      secondarySourceParams.getUntrackedParameter<std::vector<std::string> >(
          "fileNames")[0] == "file:pileup.root");
}
...
void testmakepset::usingBlockTest() {
  try {
    ...
    std::cerr
        << "testmakepset::usingBlockTest() caught an unidentified exception\n";
    throw;
  }
}
...
void testmakepset::fileinpathTest() {
  try {
    ...
    std::cerr
        << "testmakepset::fileinpathTest() caught an unidentified exception\n";
    throw;
  }
}
...
void testmakepset::fileinpathAux() {
  char const* kTest =
      ...
      "    topo = "
      "cms.FileInPath('Geometry/TrackerSimData/data/trackersens.xml'),\n"
      "    fip = cms.FileInPath('FWCore/PythonParameterSet/test/fip.txt'),\n"
      "    ufip = "
      "cms.untracked.FileInPath('FWCore/PythonParameterSet/test/ufip.txt'),\n"
  ...
  bool localArea = (releaseBase != nullptr && strlen(releaseBase) != 0 &&
                    strcmp(releaseBase, localBase));
  if (localArea) {
  ...
  CPPUNIT_ASSERT(fip.relativePath() ==
                 "FWCore/PythonParameterSet/test/fip.txt");
  CPPUNIT_ASSERT(ufip.relativePath() ==
...
void testmakepset::typesTest() {
  ...
  char const* kTest =
      ...
      "    #big = cms.double(inf),\n"
      "    vinput = cms.VInputTag(cms.InputTag('l1','i1'), cms.InputTag('l2'), "
      "cms.InputTag('l3','i3','p3'), cms.InputTag('l4','','p4'), "
      "cms.InputTag('source'), \n"
      ...
      "    veinput2 = "
      "cms.VESInputTag(cms.ESInputTag(data='blah'),cms.ESInputTag('ESProd'))\n"
      ")\n"
  ...
  // std::cout <<"blank string using single quotes returns
  // \""<<test.getParameter<std::string>("sb1")<<"\""<<std::endl;
  // std::cout <<"blank string using double quotes returns
  // \""<<test.getParameter<std::string>("sb2")<<"\""<<std::endl;
  CPPUNIT_ASSERT("" == test.getParameter<std::string>("sb1"));
  CPPUNIT_ASSERT(
      "" == test.getUntrackedParameter<std::string>("emptyString", "default"));
  CPPUNIT_ASSERT("" == test.getParameter<std::string>("sb2"));
  CPPUNIT_ASSERT(4 == test.getParameter<std::string>("sb3").size());
  std::vector<std::string> vs =
      test.getParameter<std::vector<std::string> >("vs");
  int vssize = vs.size();
  ...
  // std::cout <<"\""<<test.getParameter<std::vector<std::string>
  // >("vs")[0]<<"\" \""<<test.getParameter<std::vector<std::string>
  // >("vs")[1]<<"\" \""
  ...
  static std::vector<unsigned int> const vui(
      vuia, vuia + sizeof(vuia) / sizeof(unsigned int));
  ...
  edm::InputTag inputProduct3 =
      test.getUntrackedParameter<edm::InputTag>("input3");
  ...
  std::vector<edm::InputTag> vtags =
      test.getParameter<std::vector<edm::InputTag> >("vinput");
  ...
  std::vector<edm::ESInputTag> veinput1 =
      test.getParameter<std::vector<edm::ESInputTag> >("veinput1");
  std::vector<edm::ESInputTag> veinput2 =
      test.getParameter<std::vector<edm::ESInputTag> >("veinput2");
  ...
  std::vector<edm::EventID> vEventID =
      test.getParameter<std::vector<edm::EventID> >("vEventID");
  ...
  edm::LuminosityBlockID lumi =
      test.getParameter<edm::LuminosityBlockID>("lumi");
  CPPUNIT_ASSERT(55 == lumi.run());
  CPPUNIT_ASSERT(65 == lumi.luminosityBlock());
  std::vector<edm::LuminosityBlockID> vlumis =
      test.getParameter<std::vector<edm::LuminosityBlockID> >("vlumis");
