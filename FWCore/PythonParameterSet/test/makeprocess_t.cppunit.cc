...
class testmakeprocess : public CppUnit::TestFixture {
 ...
 private:
  ...
  ParameterSetPtr pSet(char const* c) {
    ...
    CPPUNIT_ASSERT(result->getParameter<std::string>("@process_name") ==
                   "test");
    return result;
  }
  //  void emptyPsetTest();
...
edm::ParameterSet modulePSet(std::string const& iLabel,
                             std::string const& iType,
                             std::string const& iCppType) {
...
void testmakeprocess::moduleTest() {
  ...
  edm::ParameterSet const kMainInput(
      modulePSet("@main_input", "InputSource", "Source"));

  edm::ParameterSet const kNoLabelModule(
      modulePSet("", "NoLabelModule", "ESProducer"));
  edm::ParameterSet const kLabelModule(
      modulePSet("labeled", "LabelModule", "ESProducer"));
  edm::ParameterSet const kNoLabelRetriever(
      modulePSet("", "NoLabelRetriever", "ESSource"));
  edm::ParameterSet const kLabelRetriever(
      modulePSet("label", "LabelRetriever", "ESSource"));
  ...
  CPPUNIT_ASSERT(kLabelRetriever ==
                 test->getParameterSet("LabelRetriever@label"));
}
...
void testmakeprocess::taskTest() {
  ...
  std::vector<edm::ParameterSet> const& vpsetServices =
      test->getUntrackedParameterSetVector("services");
  ...
  for (auto const& pset : vpsetServices) {
    ...
    // pset.getParameter<std::string>("@service_type") << std::endl;
  }
...
void testmakeprocess::taskTestWithSchedule() {
  char const* kTest =
      ...
      "process.ess27, process.ess28, process.esp27, process.esp28,"
      "process.serv27, process.serv28)\n"
      "process.schedule = cms.Schedule(process.p1, process.p2, process.e1, "
      "process.e2,"
...
void testmakeprocess::windowsLineEndingTest() {
  ...
  oss << ret << nl
      ...
      << "    s2 = cms.string(" << dquote << backsl << backsl << 'r' << dquote
<< ')' << ret << nl
      << "  )" << ret << nl;
