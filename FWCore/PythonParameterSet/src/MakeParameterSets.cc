...
static void makePSetsFromFile(std::string const& fileName,
  ...
  handle<>(PyRun_String(initCommand.c_str(), Py_file_input, mainNamespace.ptr(),
                        mainNamespace.ptr()));
  std::string command("makeCppPSet(locals(), topPSet)");
  handle<>(PyRun_String(command.c_str(), Py_eval_input, mainNamespace.ptr(),
                        mainNamespace.ptr()));
}
...
static void makePSetsFromString(std::string const& module,
  ...
  handle<>(PyRun_String(command.c_str(), Py_file_input, mainNamespace.ptr(),
                        mainNamespace.ptr()));
}
...
std::shared_ptr<ParameterSet> readConfig(std::string const& config, int argc,
                                         char* argv[]) {
...
std::shared_ptr<ParameterSet> readPSetsFrom(std::string const& module) {
  ...
  boost::python::object mainModule = object(
