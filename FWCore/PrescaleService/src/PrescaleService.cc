...
// constructor
PrescaleService::PrescaleService(ParameterSet const& iPS,
                                 ActivityRegistry& iReg)
    : forceDefault_(iPS.getParameter<bool>("forceDefault")),
      lvl1Labels_(iPS.getParameter<std::vector<std::string> >("lvl1Labels")),
      lvl1Default_(findDefaultIndex(
          iPS.getParameter<std::string>("lvl1DefaultLabel"), lvl1Labels_)),
  ...
  if (lvl1Default_ >= lvl1Labels_.size()) {
    throw cms::Exception("InvalidLvl1Index")
        << "lvl1Default_ '" << lvl1Default_
        << "' exceeds number of prescale columns " << lvl1Labels_.size() << "!";
  }
...
void PrescaleService::postBeginJob() {
  ...
  for (unsigned int i = 0; i < allModules.size(); ++i) {
    ...
    const std::string moduleType =
        pset.getParameter<std::string>("@module_type");
  ...
  for (unsigned int iP = 0; iP < allPaths.size(); ++iP) {
    ...
    std::vector<std::string> modules =
        prcPS.getParameter<std::vector<std::string> >(pathName);
  ...
  // Check paths stored Prescale Table: each path is actually in the process
  // config
...
// const method
unsigned int PrescaleService::getPrescale(
...
// const method
unsigned int PrescaleService::getPrescale(
    unsigned int lvl1Index, std::string const& prescaledPath) const {
...
// static method
unsigned int PrescaleService::findDefaultIndex(
    std::string const& label, std::vector<std::string> const& labels) {
...
// static method
void PrescaleService::fillDescriptions(
