...
TriggerResultsByName::TriggerResultsByName(TriggerResults const* triggerResults,
  ...
  if (triggerResults_->parameterSetID() != triggerNames_->parameterSetID()) {
    throw edm::Exception(edm::errors::Unknown)
        << "TriggerResultsByName::TriggerResultsByName, Trigger names vector "
           "and TriggerResults\n"
           "have different ParameterSetID's.  This should be impossible when "
           "the object is obtained\n"
           "from the function named triggerResultsByName in the Event class, "
           "which is the way\n"
           "TriggerResultsByName should always be created. If this is the "
           "case, please send\n"
           "information to reproduce this problem to the edm developers.  "
           "Otherwise, you are\n"
  ...
  if (triggerResults_->size() != triggerNames_->size()) {
    throw edm::Exception(edm::errors::Unknown)
        << "TriggerResultsByName::TriggerResultsByName, Trigger names vector\n"
           "and TriggerResults have different sizes.  This should be "
           "impossible,\n"
           "please send information to reproduce this problem to the edm "
           "developers.\n";
  }
}
...
HLTPathStatus const& TriggerResultsByName::at(
...
HLTPathStatus const& TriggerResultsByName::operator[](
    std::string const& pathName) const {
  if (triggerResults_ == 0) throwTriggerResultsMissing();
  unsigned i = getAndCheckIndex(pathName);
  return triggerResults_->at(i);
...
unsigned TriggerResultsByName::getAndCheckIndex(
    std::string const& pathName) const {
  if (triggerNames_ == 0) throwTriggerNamesMissing();
  ...
  if (i == triggerNames_->size()) {
    throw edm::Exception(edm::errors::LogicError)
        ...
        << "Requested trigger name \"" << pathName
