...
typedef tbb::concurrent_unordered_map<edm::ParameterSetID, edm::TriggerNames,
                                      key_hash>
    TriggerNamesMap;
...
edm::ParameterSet const* EventBase::parameterSetForID_(
    edm::ParameterSetID const& iPSID) {
  return edm::pset::Registry::instance()->getMapped(iPSID);
}

TriggerNames const* EventBase::triggerNames_(
  ...
  if (0 != (pset = psetRegistry->getMapped(triggerResults.parameterSetID()))) {
    if (pset->existsAs<std::vector<std::string> >("@trigger_paths", true)) {
      ...
      std::pair<TriggerNamesMap::iterator, bool> ret = triggerNamesMap.insert(
          std::pair<edm::ParameterSetID, edm::TriggerNames>(
  ...
  if (triggerResults.getTriggerNames().size() > 0U) {
    ...
    fakePset.addParameter<std::vector<std::string> >(
        "@trigger_paths", triggerResults.getTriggerNames());
    fakePset.registerIt();
    ...
    std::pair<TriggerNamesMap::iterator, bool> ret = triggerNamesMap.insert(
        std::pair<edm::ParameterSetID, edm::TriggerNames>(fakePset.id(),
                                                          triggerNames));
    return &(ret.first->second);
  }
  return 0;
