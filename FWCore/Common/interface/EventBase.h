...
class EventBase {
 public:
  ...
  virtual TriggerNames const& triggerNames(
      edm::TriggerResults const& triggerResults) const = 0;
  ...
  virtual edm::ParameterSet const* parameterSet(
      edm::ParameterSetID const& psID) const = 0;

 protected:
  static TriggerNames const* triggerNames_(
      edm::TriggerResults const& triggerResults);

  static edm::ParameterSet const* parameterSetForID_(
      edm::ParameterSetID const& psID);
...
bool EventBase::getByLabel(InputTag const& tag, Handle<T>& result) const {
  result.clear();
  BasicHandle bh =
      this->getByLabelImpl(typeid(edm::Wrapper<T>), typeid(T), tag);
