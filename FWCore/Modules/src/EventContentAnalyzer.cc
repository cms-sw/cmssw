...
template <typename T>
void doPrint(std::string const& iName, ObjectWithDict const& iObject,
...
template <>
void doPrint<char>(std::string const& iName, ObjectWithDict const& iObject,
                   std::string const& iIndent) {
  LogAbsolute("EventContent") << iIndent << iName << kNameValueSep
                              << static_cast<int>(*reinterpret_cast<char*>(
                                     iObject.address()));  // << "\n";
}

template <>
void doPrint<unsigned char>(std::string const& iName,
...
template <>
void doPrint<bool>(std::string const& iName, ObjectWithDict const& iObject,
                   std::string const& iIndent) {
  LogAbsolute("EventContent")
      << iIndent << iName << kNameValueSep
...
typedef void (*FunctionType)(std::string const&, ObjectWithDict const&,
                             std::string const&);
...
bool printAsBuiltin(std::string const& iName, ObjectWithDict const& iObject,
                    std::string const& iIndent) {
  typedef void (*FunctionType)(std::string const&, ObjectWithDict const&,
                               std::string const&);
  typedef std::map<std::string, FunctionType> TypeToPrintMap;
...
bool printAsContainer(std::string const& iName, ObjectWithDict const& iObject,
                      std::string const& iIndent,
                      std::string const& iIndentDelta);

void printObject(std::string const& iName, ObjectWithDict const& iObject,
                 std::string const& iIndent, std::string const& iIndentDelta) {
  ...
  if (iObject.typeOf().isPointer()) {
    LogAbsolute("EventContent")
        << iIndent << iName << kNameValueSep
        << formatClassName(iObject.typeOf().name()) << std::hex
        << iObject.address() << std::dec;  // << "\n";
    TypeWithDict pointedType =
        iObject.typeOf()
            .toType();  // for Pointers, I get the real type this way
    if (TypeWithDict::byName("void") == pointedType ||
        pointedType.isPointer() || iObject.address() == 0) {
      return;
    }
    return;
    /*
    //have the code that follows print the contents of the data to which the
    pointer points
    objectToPrint = ObjectWithDict(pointedType, iObject.address());
    //try to convert it to its actual type (assuming the original type was a
    base class)
    objectToPrint =
    ObjectWithDict(objectToPrint.castObject(objectToPrint.dynamicType()));
  ...
  for (auto const& dataMember : dataMembers) {
    ...
    // LogAbsolute("EventContent") << "     debug " << member.name() << " " <<
    // member.typeName() << "\n";
    try {
      printObject(member.name(), member.get(objectToPrint), indent,
                  iIndentDelta);
    } catch (std::exception& iEx) {
      LogAbsolute("EventContent")
          << indent << member.name() << " <exception caught(" << iEx.what()
          << ")>\n";
    }
  }
}
...
bool printAsContainer(std::string const& iName, ObjectWithDict const& iObject,
  ...
  try {
    ...
    // std::cout << "size of type '" << sizeObj.name() << "' " <<
    // sizeObj.typeName() << std::endl;
    ...
    try {
      ...
      // std::cerr << "could not get 'at' member because " << x.what() <<
      // std::endl;
      return false;
    }
    LogAbsolute("EventContent") << iIndent << iName << kNameValueSep
    ...
    // std::cout << "return type " << atReturnType.name() << " size of " <<
    // atReturnType.SizeOf()
    // << " pointer? " << atReturnType.isPointer() << " ref? " <<
    // atReturnType.isReference() << std::endl;

    // Return by reference must be treated differently since reflex will not
    // properly create
    // memory for a ref (which should just be a pointer to the object and not
    // the object itself)
    // So we will create memory on the stack which can be used to hold a
    // reference
    bool const isRef = atReturnType.isReference();
    void* refMemoryBuffer = 0;
    size_t index = 0;
    // The argument to the 'at' function is the index. Since the argument list
    // holds pointers to the arguments
    // we only need to create it once and then when the value of index changes
    // the pointer already
    // gets the new value
    ...
    for (; index != size; ++index) {
      ...
      if (isRef) {
        ...
        // Although to hold the return value from a reference reflex requires
        // you to pass it a
        // void** when it tries to call methods on the reference it expects to
        // be given a void*
      ...
      try {
        ...
        LogAbsolute("EventContent")
            << indexIndent << iName << " <exception caught(" << iEx.what()
            << ")>\n";
      }
      if (!isRef) {
...
void printObject(Event const& iEvent, std::string const& iClassName,
                 std::string const& iModuleLabel,
                 std::string const& iInstanceLabel,
                 std::string const& iProcessName, std::string const& iIndent,
                 std::string const& iIndentDelta) {
  try {
  ...
  GenericHandle handle(iClassName);
  iEvent.getByLabel(InputTag(iModuleLabel, iInstanceLabel, iProcessName),
                    handle);
...
EventContentAnalyzer::EventContentAnalyzer(ParameterSet const& iConfig)
    : indentation_(
          iConfig.getUntrackedParameter("indentation", std::string("++"))),
      verboseIndentation_(iConfig.getUntrackedParameter("verboseIndentation",
                                                        std::string("  "))),
      moduleLabels_(iConfig.getUntrackedParameter("verboseForModuleLabels",
                                                  std::vector<std::string>())),
      verbose_(iConfig.getUntrackedParameter("verbose", false) ||
               moduleLabels_.size() > 0),
      getModuleLabels_(iConfig.getUntrackedParameter(
          "getDataForModuleLabels", std::vector<std::string>())),
      getData_(iConfig.getUntrackedParameter("getData", false) ||
  ...
  if (getData_) {
    callWhenNewProductsRegistered(
        [this](edm::BranchDescription const& iBranch) {
          if (getModuleLabels_.empty()) {
            const std::string kPathStatus("edm::PathStatus");
            const std::string kEndPathStatus("edm::EndPathStatus");
            if (iBranch.className() != kPathStatus &&
                iBranch.className() != kEndPathStatus) {
              this->consumes(
                  edm::TypeToGet{iBranch.unwrappedTypeID(), PRODUCT_TYPE},
                  edm::InputTag{iBranch.moduleLabel(),
                                iBranch.productInstanceName(),
                                iBranch.processName()});
            }
          } else {
            for (auto const& mod : this->getModuleLabels_) {
              if (iBranch.moduleLabel() == mod) {
                this->consumes(
                    edm::TypeToGet{iBranch.unwrappedTypeID(), PRODUCT_TYPE},
                    edm::InputTag{mod, iBranch.productInstanceName(),
                                  iBranch.processName()});
                break;
              }
            }
...
void EventContentAnalyzer::analyze(Event const& iEvent, EventSetup const&) {
  ...
  if (listContent_) {
    LogAbsolute("EventContent")
        ...
        << " with friendlyClassName, moduleLabel, productInstanceName and "
           "processName:"
        << std::endl;
  }
  ...
  for (auto const& provenance : provenances) {
    ...
    if (listContent_ || doVerbose) {
      LogAbsolute("EventContent")
          << indentation_ << friendlyName << " \"" << modLabel << "\" \""
          << instanceName << "\" \"" << processName << "\""
          << " (productId = " << provenance->productID() << ")" << std::endl;
    }

    std::string key = friendlyName + std::string(" + \"") + modLabel +
    ...
    if (doVerbose) {
      ...
      printObject(iEvent, className, modLabel, instanceName, processName,
                  startIndent, verboseIndentation_);
      continue;
    }
    if (getData_) {
      ...
      if (getModuleLabels_.empty() ||
        ...
        GenericHandle handle(className);
        iEvent.getByLabel(InputTag(modLabel, instanceName, processName),
                          handle);
      }
    }
  }
...
void EventContentAnalyzer::endJob() {
  ...
  LogAbsolute("EventContent")
      << "\nSummary for key being the concatenation of friendlyClassName, "
         "moduleLabel, productInstanceName and processName"
      << std::endl;
  for (nameMap::const_iterator it = cumulates_.begin(),
                               itEnd = cumulates_.end();
       it != itEnd; ++it) {
    LogAbsolute("EventContent")
        << std::setw(6) << it->second << " occurrences of key " << it->first
        << std::endl;
  }
  ...
  // std::cout << "\nInteger " << k << " expressed as a string is |" << ktext <<
  // "|" << std::endl;
}

void EventContentAnalyzer::fillDescriptions(
  ...
  np->setComment(
      "This string is printed at the beginning of every line printed during "
      "event processing.");
  ...
  defaultString = "  ";
  np = desc.addOptionalUntracked<std::string>("verboseIndentation",
                                              defaultString);
  np->setComment(
      "This string is used to further indent lines when printing the contents "
      "of products in verbose mode.");

  std::vector<std::string> defaultVString;

  np = desc.addOptionalUntracked<std::vector<std::string> >(
      "verboseForModuleLabels", defaultVString);
  np->setComment(
      "If this vector is not empty, then only products with module labels on "
      "this list are printed.");
  ...
  np->setComment(
      "If this vector is not empty, then only products with module labels on "
      "this list are retrieved by getByLabel.");
