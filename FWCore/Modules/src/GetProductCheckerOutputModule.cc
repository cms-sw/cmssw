...
// GetProductCheckerOutputModule::GetProductCheckerOutputModule(GetProductCheckerOutputModule
// const& rhs) {
...
//
// GetProductCheckerOutputModule const&
// GetProductCheckerOutputModule::operator=(GetProductCheckerOutputModule const&
// rhs) {
...
//
template <typename T>
static void check(T const& p, std::string const& id,
  ...
  for (auto const& product : iProducts) {
    ...
    if (nullptr != bh.provenance() &&
        bh.provenance()->branchDescription().branchID() !=
            branchDescription->branchID()) {
      throw cms::Exception("BranchIDMissMatch")
          << "While processing " << id << " getByToken request for "
          << branchDescription->moduleLabel() << " '"
          << branchDescription->productInstanceName() << "' "
          << branchDescription->processName()
          << "\n should have returned BranchID "
          << branchDescription->branchID() << " but returned BranchID "
          << bh.provenance()->branchDescription().branchID() << "\n";
    }
  }
}
...
void GetProductCheckerOutputModule::writeLuminosityBlock(
    LuminosityBlockForOutput const& l) {
  std::ostringstream str;
  str << l.id();
...
void GetProductCheckerOutputModule::fillDescriptions(
