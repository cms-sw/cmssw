...
class IterateNTimesLooper : public EDLooper {
 ...
 private:
  ...
  IterateNTimesLooper const& operator=(
      IterateNTimesLooper const&);  // stop default
...
//
// IterateNTimesLooper const& IterateNTimesLooper::operator=(IterateNTimesLooper
// const& rhs) {
...
EDLooper::Status IterateNTimesLooper::duringLoop(Event const&,
                                                 EventSetup const&) {
  return shouldStop_ ? kStop : kContinue;
}

EDLooper::Status IterateNTimesLooper::endOfLoop(EventSetup const&,
