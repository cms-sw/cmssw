      ...
      if (atLeastOneEntry_) {
        if (!avoidCategories_.empty()) {
          for (unsigned int iE = 0; iE != errorsAndWarnings->size(); ++iE) {
            ...
            if (std::find(avoidCategories_.begin(), avoidCategories_.end(), ((*errorsAndWarnings)[iE]).category) !=
