...
//
/**\class LogErrorFilter LogErrorFilter.cc
 FWCore/LogErrorFilter/src/LogErrorFilter.cc
...
LogErrorFilter::LogErrorFilter(edm::ParameterSet const& iConfig)
    : atLeastOneError_(iConfig.getParameter<bool>("atLeastOneError")),
      ...
      avoidCategories_(
          iConfig.getParameter<std::vector<std::string>>("avoidCategories")) {
  if (!atLeastOneError_ && !atLeastOneWarning_) {
    throw edm::Exception(edm::errors::Configuration)
        << "Useless configuration of the error/warning filter. Need to select "
           "on an error or a warning or both.\n";
  }
  harvesterToken_ = consumes<std::vector<edm::ErrorSummaryEntry>>(
      iConfig.getParameter<edm::InputTag>("harvesterTag"));
  maxErrorKindsPerLumi_ = 999999;
  maxWarningKindsPerLumi_ = 999999;
  if (useThresholdsPerKind_) {
    maxErrorKindsPerLumi_ =
        iConfig.getParameter<unsigned int>("maxErrorKindsPerLumi");
    maxWarningKindsPerLumi_ =
        iConfig.getParameter<unsigned int>("maxWarningKindsPerLumi");
  }
}
  ...
  if (errorsAndWarnings.failedToGet()) {
    ...
    if (useThresholdsPerKind_) {
      ...
      for (unsigned int iE = 0; iE != errorsAndWarnings->size(); ++iE) {
        ...
        if (std::find(avoidCategories_.begin(), avoidCategories_.end(),
                      iSummary.category) != avoidCategories_.end())
          continue;
      ...
      return ((atLeastOneEntry_ &&
               (errorsBelowThreshold > 0 || warningsBelowThreshold > 0)) ||
      ...
      if (atLeastOneEntry_) {
        ...
        if (atLeastOneError_ || atLeastOneWarning_) {
          ...
          for (unsigned int iE = 0; iE != errorsAndWarnings->size(); ++iE) {
            ...
            edm::ELseverityLevel const& severity =
                ((*errorsAndWarnings)[iE]).severity;
            // count errors
...
void LogErrorFilter::fillDescriptions(
