...
// ------------ method called to produce the data  ------------
   ...
   EventSetupRecordDataGetter::doGet(EventSetup const& iSetup) {  
      if(0 == recordToDataKeys_.size()) {
         ...
         if(toGet.empty()) {
            ...
            for(auto & recordKey : recordKeys) {               
               eventsetup::EventSetupRecord const* record = iSetup.find(recordKey);
               assert(record != 0);
               dataKeys.clear();
      ...
      for(auto & recordToDataKey : recordToDataKeys_) {
         EventSetupRecord const* pRecord = iSetup.find(recordToDataKey.first);
         if(0 == pRecord) {
           edm::LogWarning("RecordNotInIOV") <<"The EventSetup Record '"<<recordToDataKey.first.name()<<"' is not available for this IOV.";
         }
         if(0 != pRecord && pRecord->cacheIdentifier() != recordToCacheIdentifier_[recordToDataKey.first]) {
            ...
            for(const auto & key : keys) {
               if(! pRecord->doGet(key)) {
                 edm::LogWarning("DataGetter") << "No data of type \""<<key.type().name() <<"\" with name \""<< key.name().value()<<"\" in record "<<recordToDataKey.first.type().name() <<" found "<< std::endl;
               } else {
                  if(verbose_) {
                    edm::LogSystem("DataGetter") << "got data of type \""<<key.type().name() <<"\" with name \""<< key.name().value()<<"\" in record "<<recordToDataKey.first.type().name() << std::endl;
                  }
               }
            }
         }
