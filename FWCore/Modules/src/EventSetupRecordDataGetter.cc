...
//
/**\class EventSetupRecordDataGetter EventSetupRecordDataGetter.cc
 src/EventSetupRecordDataGetter/src/EventSetupRecordDataGetter.cc

 Description: Can be configured to 'get' any Data in any EventSetup Record.
...
class EventSetupRecordDataGetter : public edm::stream::EDAnalyzer<> {
 ...
 private:
  ...
  typedef std::map<eventsetup::EventSetupRecordKey,
                   std::vector<eventsetup::DataKey> >
      RecordToDataKeys;
  RecordToDataKeys recordToDataKeys_;
  std::map<eventsetup::EventSetupRecordKey, unsigned long long>
...
void EventSetupRecordDataGetter::fillDescriptions(
    ConfigurationDescriptions& descriptions) {
  descriptions.setComment(
      "Retrieves specified data from the EventSetup sytem whenever that data "
      "changes.");
  ...
  toGet.add<std::vector<std::string> >("data")->setComment(
      ...
      "second part is the label used when getting the data (blank is "
      "acceptable). "
  ...
  desc.addVPSet("toGet", toGet, emptyVect)
      ->setComment(
          ...
          "A 'string' named 'record' that holds the name of an EventSetup "
          "record holding the data you want to obtain.\n"
          "a 'vstring' named 'data' that holds identifiers for the data you "
          "wish to retrieve. "
          "The identifier is in two parts separated by a backslash '/'. "
          "The first part is the C++ class name of the data and the "
          "second part is the label used when getting the data (blank is "
          "acceptable). "
          "If there is no label, the backslash may be omitted.\n"
          "If the VPSet is empty it means all data in the EventSetup should be "
          "retrieved.");
...
void EventSetupRecordDataGetter::beginLuminosityBlock(
    LuminosityBlock const&, EventSetup const& iSetup) {
  doGet(iSetup);
}

void EventSetupRecordDataGetter::analyze(edm::Event const& /*iEvent*/,
...
void EventSetupRecordDataGetter::doGet(EventSetup const& iSetup) {
  if (0 == recordToDataKeys_.size()) {
    ...
    for (Parameters::const_iterator itToGet = toGet.begin(),
                                    itToGetEnd = toGet.end();
         itToGet != itToGetEnd; ++itToGet) {
      std::string recordName = itToGet->getParameter<std::string>("record");

      eventsetup::EventSetupRecordKey recordKey(
          eventsetup::EventSetupRecordKey::TypeTag::findType(recordName));
      if (recordKey.type() == eventsetup::EventSetupRecordKey::TypeTag()) {
        // record not found
        edm::LogWarning("DataGetter")
            << "Record \"" << recordName << "\" does not exist " << std::endl;
      ...
      for (Strings::iterator itDatum = dataNames.begin(),
                             itDatumEnd = dataNames.end();
        ...
        eventsetup::TypeTag datumType =
            eventsetup::TypeTag::findType(datumName);
        if (datumType == eventsetup::TypeTag()) {
          // not found
          edm::LogWarning("DataGetter") << "data item of type \"" << datumName
    ...
    if (toGet.empty()) {
      ...
      for (std::vector<eventsetup::EventSetupRecordKey>::iterator
               itRKey = recordKeys.begin(),
  ...
  // For each requested Record get the requested data only if the Record is in a
  // new IOV

  for (RecordToDataKeys::iterator itRecord = recordToDataKeys_.begin(),
                                  itRecordEnd = recordToDataKeys_.end();
    ...
    if (0 != pRecord &&
        pRecord->cacheIdentifier() !=
            recordToCacheIdentifier_[itRecord->first]) {
      ...
      for (Keys::const_iterator itKey = keys.begin(), itKeyEnd = keys.end();
        ...
        if (!pRecord->doGet(*itKey)) {
          edm::LogWarning("DataGetter")
              ...
              << "\" with name \"" << itKey->name().value() << "\" in record "
              << itRecord->first.type().name() << " found " << std::endl;
        } else {
          if (verbose_) {
            edm::LogSystem("DataGetter")
                << "got data of type \"" << itKey->type().name()
                << "\" with name \"" << itKey->name().value() << "\" in record "
                << itRecord->first.type().name() << std::endl;
          }
        }
      }
    }
