...
// void
// EventSetupCacheIdentifierChecker::endRun(edm::Run const&, edm::EventSetup
// const&)
//{
//}
...
// ------------
void EventSetupCacheIdentifierChecker::beginLuminosityBlock(
...
// ------------
// void
// EventSetupCacheIdentifierChecker::endLuminosityBlock(edm::LuminosityBlock
  ...
  for (auto it = m_recordKeysToExpectedCacheIdentifiers.begin(),
    ...
    if (0 != pRecord && pRecord->cacheIdentifier() != it->second[m_index]) {
      throw cms::Exception("IncorrectCacheID")
          ...
          << " was supposed to have cacheIdentifier: " << it->second[m_index]
  ...
  for (auto const& name : recordNames) {
    ...
    if (recordKey.type() == eventsetup::EventSetupRecordKey::TypeTag()) {
      ...
      edm::LogWarning("DataGetter")
          << "Record \"" << name << "\" does not exist " << std::endl;

      continue;
    }

    m_recordKeysToExpectedCacheIdentifiers.insert(std::make_pair(
        recordKey,
        m_pset.getUntrackedParameter<std::vector<unsigned int> >(name)));
  }
}
...
void EventSetupCacheIdentifierChecker::fillDescriptions(
  ...
  desc.addWildcardUntracked<std::vector<unsigned int> >("*")->setComment(
      "The label is the name of an EventSetup Record while the vector contains "
      "the expected cacheIdentifier values for each beginRun, "
      "beginLuminosityBlock and event transition");
