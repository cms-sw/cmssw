...
//
// Implementation:
//     A service which prints which data from the EventSetup have been retrieved
...
PrintEventSetupDataRetrieval::PrintEventSetupDataRetrieval(
    ...
    : m_printProviders(iPS.getUntrackedParameter<bool>("printProviders")),
      m_checkDuringBeginRun(
          iPS.getUntrackedParameter<bool>("checkDuringBeginRun")),
      m_checkDuringBeginLumi(
          iPS.getUntrackedParameter<bool>("checkDuringBeginLumi")),
      m_checkDuringEvent(iPS.getUntrackedParameter<bool>("checkDuringEvent")) {}

// PrintEventSetupDataRetrieval::PrintEventSetupDataRetrieval(const
...
void PrintEventSetupDataRetrieval::fillDescriptions(
  ...
  desc.addUntracked<bool>("checkDuringBeginRun", false)
      ->setComment(
          "If 'true' check for retrieved data during each begin run is "
          "processed");
  desc.addUntracked<bool>("checkDuringBeginLumi", false)
      ->setComment(
          "If 'true' check for retrieved data during each begin lumi is "
          "processed");
  ...
  descriptions.setComment(
      "This analyzer reports when EventSetup data is retrieved by a module in "
      "the job.");
}
...
//
// const PrintEventSetupDataRetrieval&
// PrintEventSetupDataRetrieval::operator=(const PrintEventSetupDataRetrieval&
// rhs)
// {
...
// member functions
//
void PrintEventSetupDataRetrieval::analyze(Event const&,
...
void PrintEventSetupDataRetrieval::endLuminosityBlock(LuminosityBlock const&,
                                                      EventSetup const&) {}
...
void PrintEventSetupDataRetrieval::check(EventSetup const& iES) {
  ...
  for (std::vector<eventsetup::EventSetupRecordKey>::const_iterator
           it = m_recordKeys.begin(),
      ...
      itRetrievedData =
          m_retrievedDataMap
              .insert(std::make_pair(
                  *it, std::pair<unsigned long long,
                                 std::map<eventsetup::DataKey, bool> >()))
              .first;
      itRetrievedData->second.first = r->cacheIdentifier();
      std::vector<eventsetup::DataKey> keys;
      r->fillRegisteredDataKeys(keys);
      for (std::vector<eventsetup::DataKey>::const_iterator
               itData = keys.begin(),
               itDataEnd = keys.end();
    ...
    for (std::map<eventsetup::DataKey, bool>::iterator
      ...
      if (wasGotten != itDatum->second) {
        ...
        *msg << "Retrieved> record:" << it->name()
             << " data:" << itDatum->first.type().name() << " '"
             << itDatum->first.name().value() << "'";
        if (m_printProviders) {
          const edm::eventsetup::ComponentDescription* d =
              r->providerDescription(itDatum->first);
          assert(nullptr != d);
