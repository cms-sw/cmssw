...
EmptyESSource::EmptyESSource(ParameterSet const& pset)
  ...
  std::vector<unsigned int> temp(
      pset.getParameter<std::vector<unsigned int> >("firstValid"));
  for (std::vector<unsigned int>::iterator itValue = temp.begin(),
                                           itValueEnd = temp.end();
       itValue != itValueEnd; ++itValue) {
    if (iovIsTime_) {
...
void EmptyESSource::delaySettingRecords() {
  eventsetup::EventSetupRecordKey recordKey =
      eventsetup::EventSetupRecordKey::TypeTag::findType(recordName_);
  if (recordKey == eventsetup::EventSetupRecordKey()) {
    throw edm::Exception(errors::Configuration)
        << " The Record type named \"" << recordName_
        << "\" could not be found. Please check the spelling. \n"
        << "If the spelling is fine, then no module in the job requires this "
           "Record and therefore EmptyESSource can not function.\n"
           "In such a case please either remove the EmptyESSource with label'"
        << descriptionForFinder().label_
        << "' from your job or add a module which needs the Record to your "
           "job.";
  }
...
void EmptyESSource::setIntervalFor(eventsetup::EventSetupRecordKey const&,
  ...
  std::pair<std::set<IOVSyncValue>::iterator, std::set<IOVSyncValue>::iterator>
      itFound = setOfIOV_.equal_range(iTime);
  ...
  if (itFound.second != setOfIOV_.end()) {
    if (iovIsTime_) {
      endOfInterval =
          IOVSyncValue(Timestamp(itFound.second->time().value() - 1));
    } else {
      endOfInterval =
