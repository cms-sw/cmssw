...
class ModuloStreamIDFilter : public global::EDFilter<> {
 public:
  ...
  virtual bool filter(StreamID, Event& e,
                      EventSetup const& c) const override final;

 private:
  const unsigned int n_;       // accept one in n
  const unsigned int offset_;  // with offset, ie. sequence of events does not
                               // have to start at first event
};
...
bool ModuloStreamIDFilter::filter(StreamID iStreamID, Event&,
                                  EventSetup const&) const {
  return (iStreamID.value() % n_ == offset_);
}

void ModuloStreamIDFilter::fillDescriptions(
    ConfigurationDescriptions& descriptions) {
  ParameterSetDescription desc;
  desc.add<unsigned int>("modulo")->setComment(
      "Accept event if (streamID % modulo) == offset.");
  desc.add<unsigned int>("offset")->setComment(
