...
//
/**\class PrintEventSetupContent PrintEventSetupContent.cc
 GetRecordName/PrintEventSetupContent/src/PrintEventSetupContent.cc
...
namespace edm {
class PrintEventSetupContent
    : public one::EDAnalyzer<one::WatchRuns, one::WatchLuminosityBlocks> {
 public:
...
PrintEventSetupContent::PrintEventSetupContent(ParameterSet const& config)
  ...
  // now do what ever initialization is
  // neededEventSetupRecordDataGetter::EventSetupRecordDataGetter(ParameterSet
  // const& iConfig):
  //  getter = new
  //  EventSetupRecordDataGetter::EventSetupRecordDataGetter(iConfig);
}
...
void PrintEventSetupContent::analyze(Event const&, EventSetup const& iSetup) {
  print(iSetup);
}

void PrintEventSetupContent::beginRun(Run const&, EventSetup const& iSetup) {
  print(iSetup);
}
...
void PrintEventSetupContent::print(EventSetup const& iSetup) {
  ...
  for (Records::iterator itrecords = records.begin(),
                         itrecordsend = records.end();
    ...
    if (0 != rec && cacheIdentifiers_[*itrecords] != rec->cacheIdentifier()) {
      ...
      if (compact_) {
        for (Data::iterator itdata = data.begin(), itdataend = data.end();
             itdata != itdataend; ++itdata) {
          if (not msg)
          ...
          *msg << "ESContent> "
               ...
               << itdata->name().value() << "'";
          if (printProviders_) {
            edm::eventsetup::ComponentDescription const* cd =
                rec->providerDescription(*itdata);
...
void PrintEventSetupContent::fillDescriptions(
    ConfigurationDescriptions& descriptions) {
  descriptions.setComment(
      "Print what data is available in each available EventSetup Record in the "
      "job.\n"
      "As part of the data is the C++ class type, label and which module makes "
      "that data.");
  ParameterSetDescription desc;
  desc.addUntracked<bool>("compact", false)
      ->setComment(
          "If 'true' produces a more compact view, similar to the one used by "
