...
//
/**\class AbortOnEventIDAnalyzer AbortOnEventIDAnalyzer.cc
 FWCore/Modules/src/AbortOnEventIDAnalyzer.cc

 Description: Does a system abort when it seens the specified EventID. Useful
...
AbortOnEventIDAnalyzer::AbortOnEventIDAnalyzer(edm::ParameterSet const& iConfig)
    : ids_(iConfig.getUntrackedParameter<std::vector<edm::EventID> >(
          "eventsToAbort")),
      throwException_(
          iConfig.getUntrackedParameter<bool>("throwExceptionInsteadOfAbort")) {
...
void AbortOnEventIDAnalyzer::analyze(edm::Event const& iEvent,
  ...
  std::vector<edm::EventID>::iterator itFind =
      std::find_if(ids_.begin(), ids_.end(), CompareWithoutLumi(iEvent.id()));
  if (itFind != ids_.end()) {
    if (throwException_) {
      throw cms::Exception("AbortEvent")
          << "Found event " << iEvent.id() << "\n";
    } else {
      abort();
    }
  }
...
void AbortOnEventIDAnalyzer::fillDescriptions(
