...
//
/**\class RunLumiEventChecker RunLumiEventChecker.cc
 FWCore/Modules/src/RunLumiEventChecker.cc
...
class RunLumiEventChecker : public edm::EDAnalyzer {
 ...
 private:
  ...
  virtual void beginLuminosityBlock(edm::LuminosityBlock const& lumi,
                                    edm::EventSetup const& es) override;
  virtual void endLuminosityBlock(edm::LuminosityBlock const& lumi,
                                  edm::EventSetup const& es) override;
...
void RunLumiEventChecker::check(edm::EventID const& iEventID, bool iIsEvent) {
  if (index_ >= ids_.size()) {
  ...
  if (iEventID != ids_[index_]) {
    throw cms::Exception("WrongEvent") << "Was expecting event " << ids_[index_]
...
void RunLumiEventChecker::analyze(edm::Event const& iEvent,
                                  edm::EventSetup const&) {
  check(iEvent.id(), true);
}

void RunLumiEventChecker::beginRun(edm::Run const& run,
                                   edm::EventSetup const&) {
  check(edm::EventID(run.id().run(), 0, 0), false);
}
void RunLumiEventChecker::endRun(edm::Run const& run, edm::EventSetup const&) {
  check(edm::EventID(run.id().run(), 0, 0), false);
}

void RunLumiEventChecker::beginLuminosityBlock(edm::LuminosityBlock const& lumi,
                                               edm::EventSetup const&) {
  check(edm::EventID(lumi.id().run(), lumi.id().luminosityBlock(), 0), false);
}

void RunLumiEventChecker::endLuminosityBlock(edm::LuminosityBlock const& lumi,
                                             edm::EventSetup const&) {
  check(edm::EventID(lumi.id().run(), lumi.id().luminosityBlock(), 0), false);
}
...
void RunLumiEventChecker::fillDescriptions(
