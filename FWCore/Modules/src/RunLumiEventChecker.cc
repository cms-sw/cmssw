...
RunLumiEventChecker::RunLumiEventChecker(edm::ParameterSet const& iConfig)
    : ids_(iConfig.getUntrackedParameter<std::vector<edm::EventID> >("eventSequence")), index_(0) {
...
void RunLumiEventChecker::analyze(edm::Event const& iEvent, edm::EventSetup const&) { check(iEvent.id(), true); }

void RunLumiEventChecker::beginRun(edm::Run const& run, edm::EventSetup const&) {
...
void RunLumiEventChecker::beginLuminosityBlock(edm::LuminosityBlock const& lumi, edm::EventSetup const&) {
  check(edm::EventID(lumi.id().run(), lumi.id().luminosityBlock(), 0), false);
}

void RunLumiEventChecker::endLuminosityBlock(edm::LuminosityBlock const& lumi, edm::EventSetup const&) {
  check(edm::EventID(lumi.id().run(), lumi.id().luminosityBlock(), 0), false);
}
