...
/**\class EventIDChecker EventIDChecker.cc FWCore/Modules/src/EventIDChecker.cc
 ...
 Description: Checks that the events passed to it come in the order specified in
 its configuration
...
EventIDChecker::EventIDChecker(edm::ParameterSet const& iConfig)
    : ids_(iConfig.getUntrackedParameter<std::vector<edm::EventID> >(
          "eventSequence")),
      index_(0),
      multiProcessSequentialEvents_(iConfig.getUntrackedParameter<unsigned int>(
...
void EventIDChecker::analyze(edm::Event const& iEvent, edm::EventSetup const&) {
  if (mustSearch_) {
    if (0 == numberOfEventsLeftBeforeSearch_) {
      ...
      // the event must be after the last event in our list since multicore
      // doesn't go backwards
      std::vector<edm::EventID>::iterator itFind = std::find_if(
          ids_.begin() + index_, ids_.end(), CompareWithoutLumi(iEvent.id()));
      if (itFind == ids_.end()) {
        throw cms::Exception("MissedEvent")
            << "The event " << iEvent.id() << "is not in the list.\n";
      }
  ...
  if (index_ >= ids_.size()) {
    throw cms::Exception("TooManyEvents")
        << "Was passes " << ids_.size()
        << " EventIDs but have processed more events than that\n";
  }
  if (iEvent.id().run() != ids_[index_].run() ||
      iEvent.id().event() != ids_[index_].event()) {
    throw cms::Exception("WrongEvent")
        << "Was expecting event " << ids_[index_] << " but was given "
        << iEvent.id() << "\n";
  }
  ++index_;
}
...
void EventIDChecker::fillDescriptions(
