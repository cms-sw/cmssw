...
// ProvenanceCheckerOutputModule::ProvenanceCheckerOutputModule(ProvenanceCheckerOutputModule
// const& rhs)
// {
...
//
// ProvenanceCheckerOutputModule const&
...
namespace {
void markAncestors(EventForOutput const& e, ProductProvenance const& iInfo,
  ...
  for (BranchID const id : iInfo.parentage().parents()) {
    ...
    if (oMap.find(id) == oMap.end()) {
      // use side effect of calling operator[] which is if the item isn't there
      // it will add it as 'false'
      oMap[id];
...
void ProvenanceCheckerOutputModule::write(EventForOutput const& e) {
  ...
  for (auto const product : keptProducts()[InEvent]) {
    ...
    markAncestors(e, *(bh.provenance()->productProvenance()),
                  seenParentInPrincipal, missingFromMapper);
  ...
  if (missingFromMapper.size()) {
    LogError("ProvenanceChecker")
        << "Missing the following BranchIDs from ProductProvenanceRetriever\n";
    for (std::set<BranchID>::iterator it = missingFromMapper.begin(),
                                      itEnd = missingFromMapper.end();
         it != itEnd; ++it) {
      LogProblem("ProvenanceChecker")
          << *it << " " << *(idToBranchDescriptions[*it]);
    }
  }

  if (missingProductProvenance.size()) {
    LogError("ProvenanceChecker") << "The ProductResolvers for the following "
                                     "BranchIDs have no ProductProvenance\n";
    for (std::set<BranchID>::iterator it = missingProductProvenance.begin(),
                                      itEnd = missingProductProvenance.end();
         it != itEnd; ++it) {
      LogProblem("ProvenanceChecker")
          << *it << " " << *(idToBranchDescriptions[*it]);
    }
  ...
  if (missingFromReg.size()) {
    ...
    for (auto const& item : missingFromReg) {
      LogProblem("ProvenanceChecker")
          << item << " " << *(idToBranchDescriptions[item]);
    }
  }

  if (missingFromMapper.size() || missingProductProvenance.size() ||
      missingFromReg.size()) {
    throw cms::Exception("ProvenanceError")
        << (missingFromMapper.size()
                ? "Having missing ancestors from ProductProvenanceRetriever.\n"
                : "")
        << (missingProductProvenance.size() ? " Have missing "
                                              "ProductProvenance's from "
                                              "ProductResolver in Event.\n"
                                            : "")
...
//
void ProvenanceCheckerOutputModule::fillDescriptions(
