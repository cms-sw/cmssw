...
class Prescaler : public global::EDFilter<> {
 ...
 private:
  ...
  int offset_;  // with offset, ie. sequence of events does not have to start at
                // first event
};
...
bool Prescaler::filter(StreamID, Event&, EventSetup const&) const {
  // have to capture the value here since it could change by the time we do the
  // comparision
  int count = ++count_;
...
void Prescaler::fillDescriptions(ConfigurationDescriptions& descriptions) {
  ...
  desc.add<int>("prescaleFactor")
      ->setComment("Accept one event every N events");
  desc.add<int>("prescaleOffset")
      ->setComment(
          "The first event to accept should be the Mth one. Choose "
          "'prescaleFactor'=1 to accept the first event from the source.");
