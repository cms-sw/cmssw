...
//
/**\class SerialTaskQueue SerialTaskQueue.h
...
create a SerialTaskQueue
 for the resource. When every you need to perform an operation on the resource,
you push a 'task' that
 does that operation onto the queue. The queue then makes sure to run one and
...
class SerialTaskQueue {
 public:
  ...
  /// asynchronously pushes functor iAction into queue and finds next task to
  /// execute
  /**
   * This function is useful if you are accessing the SerialTaskQueue for the
   * execute()
   * method of a TBB task and want to efficiently schedule the next task from
   * the queue.
   * In that case you can take the return value and return it directly from your
   * execute() method.
   * The function will return immediately and not wait for iAction to run.
   * \param[in] iAction Must be a functor that takes no arguments and return no
   * values.
   * \return Returns either the next task that the user must schedule with TBB
   * or a nullptr.
   */
  template <typename T>
...
inline tbb::task* SerialTaskQueue::TaskBase::finishedTask() {
