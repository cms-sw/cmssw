...
class SerialTaskQueueChain {
 public:
   ...
   * \param[in] iAction Must be a functor that takes no arguments and return no
   * values.
   */
  template <typename T>
   ...
   * to find another TBB task to execute while waiting for the iAction to
   * finish.
...
void SerialTaskQueueChain::push(const T& iAction) {
  ...
  if (m_queues.size() == 1) {
    ...
    m_queues[0]->push([this, iAction]() { this->passDownChain(1, iAction); });
  }
}
...
void SerialTaskQueueChain::pushAndWait(const T& iAction) {
  ...
  std::unique_ptr<tbb::task, decltype(destry)> waitTask(
...
template <typename T>
void SerialTaskQueueChain::passDownChain(unsigned int iQueueIndex,
                                         const T& iAction) {
  ...
  if (iQueueIndex + 1 == m_queues.size()) {
    m_queues[iQueueIndex]->push(
        [this, iAction] { this->actionToRun(iAction); });
  } else {
...
void SerialTaskQueueChain::actionToRun(const T& iAction) {
  ...
  std::unique_ptr<SerialTaskQueueChain, decltype(sentryAction)> sentry(
      this, sentryAction);
  iAction();
}
}
