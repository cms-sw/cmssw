...
void OtherThingAlgorithm::run(edm::Handle<ThingCollection> const& parentHandle,
                              OtherThingCollection& result, bool useRefs,
  ...
  for (size_t i = 0; i < numToMake; ++i) {
    ...
    if (!useRefs) {
      ...
      assert(element.oneNullOneNot.size() ==
             2);  // we'll check this in our tests
      ...
      assert(element.ptrOneNullOneNot.size() ==
             2);  // we'll check this in our tests
      edm::RefProd<ThingCollection> refProd =
          edm::RefProd<ThingCollection>(parentHandle);
      ...
      edm::RefVector<ThingCollection>::iterator ri = element.refVec.begin();
      element.refVec.erase(ri);
      element.oneNullOneNot.push_back(
          edm::Ref<ThingCollection>(parentHandle.id()));
      element.oneNullOneNot.push_back(
          edm::Ref<ThingCollection>(parentHandle, 0));
      assert(element.oneNullOneNot.size() ==
             2);  // we'll check this in our tests
    ...
    //      element.refVec.clear(); // no need to clear; 'element' is created
    //      anew on every loop
  }
}
}
