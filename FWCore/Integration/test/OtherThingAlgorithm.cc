  ...
  void OtherThingAlgorithm::run(edm::Handle<ThingCollection> const& parentHandle,
    ...
    for (size_t i = 0; i < numToMake; ++i) {
      ...
      if (!useRefs) {
        ...
        element.ptr = edm::Ptr<Thing>(&parent->at(i), nullptr, i);
        assert (element.ptr == edm::refToPtr(element.ref));
        element.ptrVec.push_back(element.ptr);
        element.ptrVec.push_back(edm::Ptr<Thing>(&parent->at(19-i), nullptr, 19-i));
        element.ptrOneNullOneNot.push_back(edm::Ptr<Thing>(nullptr, nullptr, 0));
        element.ptrOneNullOneNot.push_back(edm::Ptr<Thing>(&parent->at(0), nullptr, 0));
