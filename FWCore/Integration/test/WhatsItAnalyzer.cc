...
//
/**\class WhatsItAnalyzer WhatsItAnalyzer.cc
 test/WhatsItAnalyzer/src/WhatsItAnalyzer.cc
...
WhatsItAnalyzer::WhatsItAnalyzer(const edm::ParameterSet& iConfig)
    : expectedValues_(iConfig.getUntrackedParameter<std::vector<int> >(
...
void WhatsItAnalyzer::analyze(const edm::Event& /*iEvent*/,
  ...
  if (!expectedValues_.empty()) {
    if (expectedValues_.at(index_) != pSetup->a) {
      throw cms::Exception("TestFail")
          << "expected value " << expectedValues_[index_] << " but was got "
          << pSetup->a;
    }
    ++index_;
  }
