...
TestHistoryKeeping::TestHistoryKeeping(edm::ParameterSet const& pset)
    : expectedProcesses_(
          pset.getParameter<std::vector<std::string> >("expected_processes")),
      numberOfExpectedHLTProcessesInEachRun_(pset.getParameter<int>(
          "number_of_expected_HLT_processes_for_each_run")) {
  // Nothing to do.
}
...
void TestHistoryKeeping::analyze(edm::Event const& ev, edm::EventSetup const&) {
  for (std::vector<std::string>::const_iterator i = expectedProcesses_.begin(),
