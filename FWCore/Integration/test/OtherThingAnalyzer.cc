...
void OtherThingAnalyzer::doit(edm::Event const& dv, std::string const& label) {
  ...
  edm::LogInfo("OtherThingAnalyzer")
      << " --------------- next event ------------ \n";
  int i = 0;
  for (OtherThingCollection::const_iterator it = otherThings->begin(),
                                            itEnd = otherThings->end();
       it != itEnd; ++it, ++i) {
    ...
    if (otherThing.oneNullOneNot.size() != 2) {
      throw cms::Exception("Inconsistent Data", "OtherThingAnalyzer::analyze")
          << " oneNullOneNot has wrong length: "
          << otherThing.oneNullOneNot.size() << " should be 2\n";
    }
    ...
    if (otherThing.ptrOneNullOneNot.size() != 2) {
      throw cms::Exception("Inconsistent Data", "OtherThingAnalyzer::analyze")
          << " ptrOneNullOneNot has wrong length: "
          << otherThing.ptrOneNullOneNot.size() << " should be 2\n";
    }
    ...
    if (thingWasDropped_) return;
    {
      ...
      edm::View<Thing>::size_type const viewSize2 =
          otherThing.refToBaseProd->size();
      ...
      edm::Ref<ThingCollection> refFromCast =
          otherThing.refToBase.castTo<edm::Ref<ThingCollection> >();
      edm::Ptr<Thing> ptrFromCast =
          otherThing.refToBase.castTo<edm::Ptr<Thing> >();
      ...
      for (edm::RefVector<ThingCollection>::iterator
               it = otherThing.refVec.begin(),
               itEnd = otherThing.refVec.end();
           it != itEnd; ++it) {
    ...
    }
    {
      ...
      for (edm::PtrVector<Thing>::const_iterator
               it = otherThing.ptrVec.begin(),
               itEnd = otherThing.ptrVec.end();
           it != itEnd; ++it) {
...
void OtherThingAnalyzer::fillDescriptions(
    edm::ConfigurationDescriptions& descriptions) {
  edm::ParameterSetDescription desc;
  desc.addUntracked<bool>("thingWasDropped", false)
      ->setComment(
          "true if the ref to the ThingCollection in the OtherThingCollection "
          "can not be resolved since the ThingCollection was not stored.");
  desc.addUntracked<edm::InputTag>("other",
