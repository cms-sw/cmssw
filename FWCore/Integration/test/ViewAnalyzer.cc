...
ViewAnalyzer::ViewAnalyzer(ParameterSet const&) {
  ...
  consumes<edm::View<edmtest::DSVSimpleProduct::value_type>>(
      edm::InputTag{"dsvsimple"});
...
struct tester {
  static void call(ViewAnalyzer const* va, Event const& e,
                   char const* moduleLabel) {
    va->template testProduct<P, V>(e, moduleLabel);
  }
};

void ViewAnalyzer::analyze(StreamID, Event const& e,
...
template <typename P, typename V>
void ViewAnalyzer::testProduct(Event const& e,
                               std::string const& moduleLabel) const {
...
void ViewAnalyzer::testDSVProduct(Event const& e,
                                  std::string const& moduleLabel) const {
...
void ViewAnalyzer::testAVProduct(Event const& e,
                                 std::string const& moduleLabel) const {
  ...
  while (i_prod != e_prod && i_view != e_view) {
    ...
    edm::Ptr<sequence_t::key_type> ptr(prod.first.id(), prod.first.key(),
                                       &e.productGetter());
...
void ViewAnalyzer::testProductWithBaseClass(
    Event const& e, std::string const& moduleLabel) const {
  ...
  while (i_prod != e_prod && i_view != e_view) {
    ...
    edm::Ref<edm::OwnVector<SimpleDerived>> ref =
        refToBase.castTo<edm::Ref<edm::OwnVector<SimpleDerived>>>();
...
void ViewAnalyzer::testRefVector(Event const& e,
                                 std::string const& moduleLabel) const {
  ...
  while (i_product != e_product && i_view != e_view) {
    ...
    edm::Ref<std::vector<int>> ref2 =
        refToBase.castTo<edm::Ref<std::vector<int>>>();
...
void ViewAnalyzer::testRefToBaseVector(Event const& e,
                                       std::string const& moduleLabel) const {
...
void ViewAnalyzer::testPtrVector(Event const& e,
                                 std::string const& moduleLabel) const {
...
void ViewAnalyzer::testStdVectorPtr(Event const& e,
                                    std::string const& moduleLabel) const {
