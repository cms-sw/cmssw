...
#wantSummary = cms.untracked.bool(True),Rethrow = FWCore.Framework.test.cmsExceptionsFatalOption_cff.Rethrow) process.source = cms.Source("PoolSource", fileNames = cms.untracked.vstring('file:testSeriesOfProcessesTEST.root'), )

#Override the fakeRaw found in the test file process.fakeRaw = cms.EDProducer("IntProducer", ivalue = cms.int32(20))
...
#file input. process.a = cms.EDAnalyzer("TestFindProduct", inputTags = cms.untracked.VInputTag(cms.InputTag("fakeRaw"), cms.InputTag("fakeHLTDebug")),
...
#HLT configuration(the first one in this series). expectedSum = cms.untracked.int32(15300)) process.test1 = cms.Path(process.fakeRaw * process.a) process.out = cms.OutputModule("PoolOutputModule", fileName = cms.untracked.string('testSeriesOfProcessesPROD2TEST.root'), outputCommands = cms.untracked.vstring("keep *")) process.o = cms.EndPath(process.out)