...
OtherThingProducer::OtherThingProducer(edm::ParameterSet const& pset)
    : alg_(), refsAreTransient_(false) {
  produces<OtherThingCollection>("testUserTag");
  useRefs_ = pset.getUntrackedParameter<bool>("useRefs");
  if (useRefs_) {
    thingToken_ =
        consumes<ThingCollection>(pset.getParameter<edm::InputTag>("thingTag"));
  }
...
void OtherThingProducer::produce(edm::Event& e, edm::EventSetup const&) {
  ...
  // Step D: Invoke the algorithm, passing in inputs (NONE) and getting back
  // outputs.
...
void OtherThingProducer::fillDescriptions(
    edm::ConfigurationDescriptions& descriptions) {
  edm::ParameterSetDescription desc;
  desc.add<edm::InputTag>("thingTag", edm::InputTag("Thing"))
      ->setComment("Where to get the ThingCollection");
  desc.addUntracked<bool>("useRefs", true)
      ->setComment(
          "Actually get the ThingCollection and build edm::Refs to the "
          "contained items.");
  desc.addUntracked<bool>("transient", false)
      ->setComment(
          "If true, then the Refs constructed by the ThingCollection can not "
          "be persisted");
