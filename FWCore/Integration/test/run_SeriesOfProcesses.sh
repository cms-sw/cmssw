...
pushd ${LOCAL_TMP_DIR}
        ...
        cmsRun -
        p ${LOCAL_TEST_DIR} / ${test} HLT_cfg.py ||
    die "cmsRun ${test}HLT_cfg.py" $
    ?
    ...
    cmsRun - p ${LOCAL_TEST_DIR} / ${test} TEST2_cfg.py 2 > ${test} TEST2.txt grep
                                                            "Duplicate Process" ${test} TEST2.txt ||
