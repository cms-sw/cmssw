...
namespace {
  ...
  */
  class WaitingServer {
  public:
    WaitingServer(unsigned int iNumberOfStreams, unsigned int iMinNumberOfStreamsBeforeDoingWork,
    ...
    void requestValuesAsync(edm::StreamID, std::vector<int> const* iIn, std::atomic<std::vector<int>*>* iOut,
  ...
  void WaitingServer::requestValuesAsync(edm::StreamID iID, std::vector<int> const* iIn,
                                         std::atomic<std::vector<int>*>* iOut, edm::WaitingTask* iWaitingTask) {
...
namespace edmtest {
  ...
  void WaitingThreadIntProducer::preallocStreams(unsigned int iNStreams) {
    m_server = std::make_unique<WaitingServer>(
        iNStreams, m_numberOfStreamsToAccumulate <= iNStreams ? m_numberOfStreamsToAccumulate : iNStreams,
