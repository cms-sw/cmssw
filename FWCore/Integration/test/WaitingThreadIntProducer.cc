...
/*
 ...
 required number of streams have passed it data (or until a time limit is
 reached)
...
class WaitingServer {
 public:
  ...
  void requestValuesAsync(edm::StreamID, std::vector<int> const* iIn,
...
bool WaitingServer::readyForWork() const {
  ...
  if (m_drainQueue or
      (m_minNumStreamsBeforeDoingWork <= m_waitingStreams.size())) {
    return true;
  }
  return false;
}
  ...
  while (not m_shouldStop) {
    std::vector<unsigned int> streamsToUse;
    {
      ...
      if (not readyForWork()) {
        ...
        m_cond.wait_for(lk, std::chrono::seconds(m_secondsToWait),
                        [this]() -> bool { return readyForWork(); });
      }
...
WaitingThreadIntProducer::WaitingThreadIntProducer(
    ...
    : m_numberOfStreamsToAccumulate(
          iConfig.getUntrackedParameter<unsigned int>("streamsToAccumulate")),
      m_secondsToWaitForWork(iConfig.getUntrackedParameter<unsigned int>(
          "secondsToWaitForWork", 10U)) {
  for (auto const& tag :
       iConfig.getParameter<std::vector<edm::InputTag>>("tags")) {
...
void WaitingThreadIntProducer::preallocStreams(unsigned int iNStreams) {
  m_server = std::make_unique<WaitingServer>(
      iNStreams,
      m_numberOfStreamsToAccumulate <= iNStreams ? m_numberOfStreamsToAccumulate
                                                 : iNStreams,
...
void WaitingThreadIntProducer::produce(edm::StreamID iID, edm::Event& e,
