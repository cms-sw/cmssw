...
namespace {
void getAncestors(edm::Event const& e, edm::BranchID const& branchID,
...
void getAncestorsFromRetriever(edm::ProductProvenanceRetriever const* retriever,
                               ...
                               std::set<edm::BranchID>& ancestors) {
  edm::ProductProvenance const* productProvenance =
      retriever->branchIDToProvenance(branchID);
...
TestParentage::TestParentage(edm::ParameterSet const& pset)
    : inputTag_(pset.getParameter<edm::InputTag>("inputTag")),
      expectedAncestors_(
          pset.getParameter<std::vector<std::string> >("expectedAncestors")),
      callGetProvenance_(
          pset.getUntrackedParameter<bool>("callGetProvenance", true)) {
...
void TestParentage::analyze(edm::Event const& e, edm::EventSetup const&) {
  ...
  if (prov->originalBranchID() !=
      prov->branchDescription().originalBranchID()) {
    std::cerr << "TestParentage::analyze: test of Provenance::originalBranchID "
                 "function failed"
              << std::endl;
    abort();
  }

  std::set<std::string> expectedAncestors(expectedAncestors_.begin(),
                                          expectedAncestors_.end());
  ...
  if (callGetProvenance_) {
    ...
    if (ancestorLabels != expectedAncestors) {
      std::cerr
          << "TestParentage::analyze: ancestors do not match expected ancestors"
          << std::endl;
      abort();
    }
  }
  ...
  getAncestorsFromRetriever(retriever, prov->originalBranchID(),
                            ancestorsFromRetriever);
  ...
  if (ancestorLabels2 != expectedAncestors) {
    std::cerr << "TestParentage::analyze: ancestors do not match expected "
                 "ancestors (parentage from retriever)"
              << std::endl;
    abort();
  }
}
