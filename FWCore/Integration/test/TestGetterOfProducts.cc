...
TestGetterOfProducts::TestGetterOfProducts(edm::ParameterSet const& par)
    : processName_(par.getParameter<std::string>("processName")),
      expectedInputTagLabels_(par.getParameter<std::vector<std::string> >(
          "expectedInputTagLabels")),
      expectedLabelsAfterGet_(par.getParameter<std::vector<std::string> >(
          "expectedLabelsAfterGet")),
...
bool TestGetterOfProducts::filter(edm::Event& event, edm::EventSetup const&) {
  ...
  if (expectedInputTagLabels_.size() != tokens.size()) {
    std::cerr << "Expected number of InputTag's differs from actual number.\n"
              ...
              << "Expected value = " << expectedInputTagLabels_.size()
              << " actual value = " << tokens.size() << std::endl;
    abort();
  }

  std::vector<std::string>::const_iterator iter =
      expectedInputTagLabels_.begin();
  for (auto const& token : tokens) {
    ...
    if (*iter != labels.module) {
      throw cms::Exception("Failed Test")
          ...
          << "In function TestGetterOfProducts::filter\n"
          << "Expected value = " << *iter << " actual value = " << labels.module
          << std::endl;
    }
    ++iter;
  }

  // A handle is added to this vector for each product that is found in the
  // event.
  ...
  if (expectedLabelsAfterGet_.size() != handles.size()) {
    throw cms::Exception("Failed Test")
        ...
        << "In function TestGetterOfProducts::filter\n"
        << "Expected value = " << expectedLabelsAfterGet_.size()
        << " actual value = " << handles.size() << std::endl;
  }
  ...
  for (auto const& handle : handles) {
    if (handle.provenance()->moduleLabel() != *iter) {
      throw cms::Exception("Failed Test")
          << "Expected module label after get differs from actual module "
             "label.\n"
          << "In function TestGetterOfProducts::filter\n"
          << "Expected value = " << *iter
          << " actual value = " << handle.provenance()->moduleLabel()
          << std::endl;
      break;
    }
    ++iter;
...
TestGetterOfProductsA::TestGetterOfProductsA(edm::ParameterSet const& par)
    : processName_(par.getParameter<std::string>("processName")),
      ...
      expectedLabelsAfterGet_(par.getParameter<std::vector<std::string> >(
          "expectedLabelsAfterGet")),
      expectedNumberOfThingsWithLabelA_(
          par.getParameter<unsigned>("expectedNumberOfThingsWithLabelA")),
  ...
  getterOfIntProducts_ =
      edm::GetterOfProducts<IntProduct>(edm::ProcessMatch(processName_), this);
  getterOfProducts_ = edm::GetterOfProducts<Thing>(
      edm::ProcessMatch(processName_), this, branchType_);
  getterUsingLabel_ = edm::GetterOfProducts<Thing>(edm::ModuleLabelMatch("A"),
                                                   this, branchType_);
...
void TestGetterOfProductsA::analyze(edm::Event const& event,
                                    edm::EventSetup const&) {
  ...
  if (expectedNumberOfThingsWithLabelA_ != handles.size()) {
    throw cms::Exception("Failed Test")
        << "Expected number of products with module label A gotten differs "
           "from actual number.\n"
        << "In function TestGetterOfProducts::filter\n"
        << "Expected value = " << expectedNumberOfThingsWithLabelA_
        << " actual value = " << handles.size() << std::endl;
  }
...
void TestGetterOfProductsA::endLuminosityBlock(edm::LuminosityBlock const& lumi,
                                               edm::EventSetup const&) {
...
void TestGetterOfProductsA::endRun(edm::Run const& run,
                                   edm::EventSetup const&) {
