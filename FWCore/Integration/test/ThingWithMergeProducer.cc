...
ThingWithMergeProducer::ThingWithMergeProducer(edm::ParameterSet const& pset)
    : changeIsEqualValue_(
          pset.getUntrackedParameter<bool>("changeIsEqualValue", false)),
      labelsToGet_(pset.getUntrackedParameter<std::vector<std::string>>(
  ...
  produces<ThingWithIsEqual, edm::Transition::BeginLuminosityBlock>(
      "beginLumi");
...
void ThingWithMergeProducer::produce(edm::Event& e, edm::EventSetup const&) {
  ...
  for (Iter iter = labelsToGet_.begin(), ie = labelsToGet_.end(); iter != ie;
       ++iter) {
    edm::Handle<Thing> h;
...
void ThingWithMergeProducer::beginLuminosityBlockProduce(
    edm::LuminosityBlock& lb, edm::EventSetup const&) {
  auto result = std::make_unique<Thing>();
  result->a = 101;
...
void ThingWithMergeProducer::endLuminosityBlockProduce(edm::LuminosityBlock& lb,
...
void ThingWithMergeProducer::beginRunProduce(edm::Run& r,
                                             edm::EventSetup const&) {
  auto result = std::make_unique<Thing>();
  result->a = 10001;
...
void ThingWithMergeProducer::endRunProduce(edm::Run& r,
                                           edm::EventSetup const&) {
  auto result = std::make_unique<Thing>();
  result->a = 100001;
...
void ThingWithMergeProducer::beginLuminosityBlock(
    edm::LuminosityBlock const& lb, edm::EventSetup const&) {
  for (Iter iter = labelsToGet_.begin(), ie = labelsToGet_.end(); iter != ie;
       ++iter) {
    edm::Handle<Thing> h;
...
void ThingWithMergeProducer::endLuminosityBlock(edm::LuminosityBlock const& lb,
                                                edm::EventSetup const&) {
  for (Iter iter = labelsToGet_.begin(), ie = labelsToGet_.end(); iter != ie;
       ++iter) {
    edm::Handle<Thing> h;
...
// Functions that gets called by framework every run
void ThingWithMergeProducer::beginRun(edm::Run const& r,
                                      edm::EventSetup const&) {
  for (Iter iter = labelsToGet_.begin(), ie = labelsToGet_.end(); iter != ie;
       ++iter) {
    edm::Handle<Thing> h;
...
void ThingWithMergeProducer::fillDescriptions(
    edm::ConfigurationDescriptions& descriptions) {
  edm::ParameterSetDescription desc;
  desc.addUntracked<bool>("changeIsEqualValue", false);
  desc.addUntracked<std::vector<std::string>>("labelsToGet",
