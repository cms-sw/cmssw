...
ThrowingSource::ThrowingSource(ParameterSet const& pset,
    ...
    : ProducerSourceBase(pset, desc, false),
      whenToThrow_(
          pset.getUntrackedParameter<int>("whenToThrow", kDoNotThrow)) {
...
bool ThrowingSource::setRunAndEventInfo(EventID&, TimeValue_t&,
...
void ThrowingSource::endLuminosityBlock(LuminosityBlock& lb) {
  if (whenToThrow_ == kEndLumi)
    throw cms::Exception("TestThrow") << "ThrowingSource::endLuminosityBlock";
}
...
void ThrowingSource::endRun(Run& run) {
  if (whenToThrow_ == kEndRun)
    throw cms::Exception("TestThrow") << "ThrowingSource::endRun";
}
...
std::shared_ptr<RunAuxiliary> ThrowingSource::readRunAuxiliary_() {
  ...
  resetNewRun();
  return std::make_shared<RunAuxiliary>(eventID().run(), ts,
...
ThrowingSource::readLuminosityBlockAuxiliary_() {
  ...
  return std::make_shared<LuminosityBlockAuxiliary>(
      eventID().run(), eventID().luminosityBlock(), ts,
      Timestamp::invalidTimestamp());
}
...
void ThrowingSource::readEvent_(EventPrincipal& eventPrincipal) {
  ...
  EventAuxiliary aux(eventID(), processGUID(), Timestamp(presentTime()), false,
                     EventAuxiliary::Undefined);
