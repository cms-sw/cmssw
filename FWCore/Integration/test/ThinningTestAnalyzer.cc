...
ThinningTestAnalyzer::ThinningTestAnalyzer(edm::ParameterSet const& pset) {
  parentToken_ =
      consumes<ThingCollection>(pset.getParameter<edm::InputTag>("parentTag"));
  thinnedToken_ = mayConsume<ThingCollection>(
      pset.getParameter<edm::InputTag>("thinnedTag"));
  associationToken_ = mayConsume<edm::ThinnedAssociation>(
      pset.getParameter<edm::InputTag>("associationTag"));
  trackToken_ = consumes<TrackOfThingsCollection>(
  ...
  if (!parentWasDropped_) {
    expectedParentContent_ =
        pset.getParameter<std::vector<int> >("expectedParentContent");
  }
  ...
  if (!thinnedWasDropped_) {
    expectedThinnedContent_ =
        pset.getParameter<std::vector<int> >("expectedThinnedContent");
  }
  associationShouldBeDropped_ =
      pset.getParameter<bool>("associationShouldBeDropped");
  if (!associationShouldBeDropped_) {
    expectedIndexesIntoParent_ = pset.getParameter<std::vector<unsigned int> >(
        "expectedIndexesIntoParent");
  }
  expectedValues_ = pset.getParameter<std::vector<int> >("expectedValues");
}

void ThinningTestAnalyzer::fillDescriptions(
...
void ThinningTestAnalyzer::analyze(edm::Event const& event,
  ...
  if (parentWasDropped_) {
    ...
    if (parentCollection->size() != expectedParentContent_.size()) {
      throw cms::Exception("TestFailure")
          << "parent collection has unexpected size";
    }
    for (auto const& thing : *parentCollection) {
      ...
      int expected =
          static_cast<int>(expectedParentContent_.at(i) +
                           event.eventAuxiliary().event() * 100 + 100);
      if (thing.a != expected) {
        throw cms::Exception("TestFailure")
            << "parent collection has unexpected content";
      }
      ++i;
    }
  }
  ...
  if (thinnedWasDropped_) {
    ...
    if (thinnedCollection->size() != expectedThinnedContent_.size()) {
      throw cms::Exception("TestFailure")
          << "thinned collection has unexpected size";
    }
    for (auto const& thing : *thinnedCollection) {
      if (thing.a !=
          static_cast<int>(expectedThinnedContent_.at(expectedIndex) +
                           event.eventAuxiliary().event() * 100 + 100)) {
        throw cms::Exception("TestFailure")
            << "thinned collection has unexpected content";
      }
      ++expectedIndex;
    }
  }
  ...
  if (!associationShouldBeDropped_) {
    ...
    if (associationCollection->indexesIntoParent().size() !=
        expectedIndexesIntoParent_.size()) {
      throw cms::Exception("TestFailure")
    ...
    for (auto const& association : associationCollection->indexesIntoParent()) {
      if (association != expectedIndexesIntoParent_.at(expectedIndex)) {
        throw cms::Exception("TestFailure")
            << "association collection has unexpected content";
      }
      ++expectedIndex;
    }
  }
  ...
  if (!thinnedWasDropped_ && !associationShouldBeDropped_) {
    if (associationCollection->thinnedCollectionID() !=
  ...
  int eventOffset =
      static_cast<int>(event.eventAuxiliary().event() * 100 + 100);
  ...
  for (auto const& track : *trackCollection) {
    if (*expectedValue == -1) {
      if (track.ref1.isAvailable()) {
        throw cms::Exception("TestFailure")
            << "ref1 is available when it should not be";
      }
    } else {
      if (!track.ref1.isAvailable()) {
        throw cms::Exception("TestFailure")
      ...
      if (track.ref1->a != *expectedValue + eventOffset) {
        throw cms::Exception("TestFailure")
            << "Unexpected values from ref1 (2nd try)";
      }
    }
    ...
    if (*expectedValue == -1) {
      if (track.refToBase1.isAvailable()) {
        throw cms::Exception("TestFailure")
            << "refToBase1 is available when it should not be";
      }
    } else {
      if (!track.refToBase1.isAvailable()) {
        throw cms::Exception("TestFailure")
            << "refToBase1 is not available when it should be";
      }

      // Check twice to test some possible caching problems.
      if (track.refToBase1->a != *expectedValue + eventOffset) {
        throw cms::Exception("TestFailure")
            << "unexpected values from refToBase1";
      }
      if (track.refToBase1->a != *expectedValue + eventOffset) {
        throw cms::Exception("TestFailure")
            << "unexpected values from refToBase1";
    ...
    if (*expectedValue == -1) {
      if (track.ptr1.isAvailable()) {
        throw cms::Exception("TestFailure")
            << "ptr1 is available when it should not be";
      }
    } else {
      if (!track.ptr1.isAvailable()) {
        throw cms::Exception("TestFailure")
            << "ptr1 is not available when it should be";
      }
    ...
    if (*expectedValue == -1) {
      if (track.ptr2.isAvailable()) {
        throw cms::Exception("TestFailure")
            << "ptr2 is available when it should not be";
      }
    } else {
      if (!track.ptr2.isAvailable()) {
        throw cms::Exception("TestFailure")
            << "ptr2 is not available when it should be";
      }
    ...
    for (auto iExpectedValue : expectedValues_) {
      if (iExpectedValue != -1) {
        if (track.refVector1[k]->a != iExpectedValue + eventOffset) {
          throw cms::Exception("TestFailure")
              << "unexpected values from refVector1";
        }
...
void ThinningTestAnalyzer::incrementExpectedValue(
