...
class ThinningThingSelector {
 public:
  ThinningThingSelector(edm::ParameterSet const& pset,
                        edm::ConsumesCollector&& cc);

  static void fillDescription(edm::ParameterSetDescription& desc);

  void preChoose(edm::Handle<edmtest::ThingCollection> tc,
                 edm::Event const& event, edm::EventSetup const& es);
...
ThinningThingSelector::ThinningThingSelector(edm::ParameterSet const& pset,
                                             edm::ConsumesCollector&& cc) {
  trackToken_ = cc.consumes<TrackOfThingsCollection>(
      pset.getParameter<edm::InputTag>("trackTag"));
  offsetToThinnedKey_ = pset.getParameter<unsigned int>("offsetToThinnedKey");
  expectedCollectionSize_ =
      pset.getParameter<unsigned int>("expectedCollectionSize");
}

void ThinningThingSelector::fillDescription(
...
void ThinningThingSelector::preChoose(edm::Handle<edmtest::ThingCollection> tc,
  ...
  // Just checking to see if the collection got passed in. Not really using it
  // for anything.
  ...
  // Just checking to see the EventSetup works from here. Not really using it
  // for anything.
...
bool ThinningThingSelector::choose(unsigned int iIndex,
                                   edmtest::Thing const& iItem) {
  // Just checking to see the element in the container got passed in OK. Not
  // really using it.
  // Just using %10 because it coincidentally works with the arbitrary numbers I
  // picked, no meaning really.
...
typedef edm::ThinningProducer<edmtest::ThingCollection,
                              edmtest::ThinningThingSelector>
