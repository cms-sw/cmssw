...
TestFindProduct::TestFindProduct(edm::ParameterSet const& pset)
    : inputTags_(
          pset.getUntrackedParameter<std::vector<edm::InputTag> >("inputTags")),
      expectedSum_(pset.getUntrackedParameter<int>("expectedSum", 0)),
      sum_(0),
      inputTagsNotFound_(),
      getByTokenFirst_(
          pset.getUntrackedParameter<bool>("getByTokenFirst", false)),
      inputTagsView_() {
  std::vector<edm::InputTag> emptyTagVector;
  inputTagsNotFound_ = pset.getUntrackedParameter<std::vector<edm::InputTag> >(
      "inputTagsNotFound", emptyTagVector);
  inputTagsView_ = pset.getUntrackedParameter<std::vector<edm::InputTag> >(
...
void TestFindProduct::analyze(edm::Event const& e, edm::EventSetup const&) {
  ...
  std::vector<edm::EDGetTokenT<IntProduct> >::const_iterator iToken =
      tokens_.begin();
  for (std::vector<edm::InputTag>::const_iterator iter = inputTags_.begin(),
    ...
    if (h->value != hToken->value) {
      std::cerr << "TestFindProduct::analyze getByLabel and getByToken return "
  ...
  for (std::vector<edm::InputTag>::const_iterator
           iter = inputTagsNotFound_.begin(),
           iEnd = inputTagsNotFound_.end();
       iter != iEnd; ++iter, ++iToken) {
    e.getByLabel(*iter, h);
    if (h.isValid()) {
      std::cerr << "TestFindProduct::analyze: getByLabel found a product that "
                   "should not be found "
    ...
    e.getByToken(*iToken, hToken);
    if (hToken.isValid()) {
      std::cerr << "TestFindProduct::analyze: getByToken found a product that "
                   "should not be found "
                << hToken.provenance()->moduleLabel() << std::endl;
      abort();
    }
  }
  std::vector<edm::EDGetTokenT<edm::View<int> > >::const_iterator iTokenView =
      tokensView_.begin();
  for (std::vector<edm::InputTag>::const_iterator iter = inputTagsView_.begin(),
                                                  iEnd = inputTagsView_.end();
       iter != iEnd; ++iter, ++iTokenView) {
    if (getByTokenFirst_) {
...
void TestFindProduct::endJob() {
  ...
  if (expectedSum_ != 0 && sum_ != expectedSum_) {
    throw cms::Exception("TestFail") << "TestFindProduct::endJob - Sum of test "
                                        "object values does not equal expected "
                                        "value";
  }
}
}  // namespace edmtest
