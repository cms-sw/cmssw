...
ESTestAnalyzerA::ESTestAnalyzerA(edm::ParameterSet const& pset)
    : runsToGetDataFor_(
          pset.getParameter<std::vector<int> >("runsToGetDataFor")) {}

void ESTestAnalyzerA::analyze(edm::Event const& ev, edm::EventSetup const& es) {
  if (std::find(runsToGetDataFor_.begin(), runsToGetDataFor_.end(), ev.run()) !=
      runsToGetDataFor_.end()) {
    ESTestRecordA const& rec = es.get<ESTestRecordA>();
    edm::ESHandle<ESTestDataA> dataA;
    rec.get(dataA);
    edm::LogAbsolute("ESTestAnalyzerA")
        << "ESTestAnalyzerA: process = " << moduleDescription().processName()
...
void ESTestAnalyzerB::analyze(edm::Event const& ev, edm::EventSetup const& es) {
  if (std::find(runsToGetDataFor_.begin(), runsToGetDataFor_.end(), ev.run()) !=
    ...
    edm::LogAbsolute("ESTestAnalyzerB")
        << "ESTestAnalyzerB: process = " << moduleDescription().processName()
        << ": Data value = " << dataB->value();
  }
}
...
void ESTestAnalyzerK::analyze(edm::Event const& ev, edm::EventSetup const& es) {
  if (std::find(runsToGetDataFor_.begin(), runsToGetDataFor_.end(), ev.run()) !=
    ...
    edm::LogAbsolute("ESTestAnalyzerK")
        << "ESTestAnalyzerK: process = " << moduleDescription().processName()
        << ": Data value = " << dataK->value();
  }
}
...
void ESTestAnalyzerAZ::analyze(edm::Event const& ev,
  ...
  if (std::find(runsToGetDataFor_.begin(), runsToGetDataFor_.end(), ev.run()) !=
    ...
    edm::LogAbsolute("ESTestAnalyzerAZ")
        << "ESTestAnalyzerAZ: process = " << moduleDescription().processName()
