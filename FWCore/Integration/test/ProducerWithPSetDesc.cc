...
namespace edmtest {
  ProducerWithPSetDesc::ProducerWithPSetDesc(edm::ParameterSet const& ps)
  {
    ...
    edm::ParameterSet psetXor = ps.getParameter<edm::ParameterSet>("xorPset");
    assert(psetXor.getParameter<std::string>("type1") == std::string("11"));      
    assert(!psetXor.exists("type2"));
    if (testingAutoGeneratedCfi) {
      assert(psetXor.getParameter<std::string>("type") == std::string("11"));      
    }
    else {
      assert(psetXor.getParameter<unsigned>("type") == 11U);      
    }

    edm::ParameterSet psetOr = ps.getParameter<edm::ParameterSet>("orPset");
    ...
    edm::ParameterSet psetAnd = ps.getParameter<edm::ParameterSet>("andPset");
    ...
    edm::ParameterSet psetIfExists = ps.getParameter<edm::ParameterSet>("ifExistsPset");
    ...
    edm::ParameterSet psetAllowed = ps.getParameter<edm::ParameterSet>("allowedLabelsPset");
    if (testingAutoGeneratedCfi) {
      assert(psetAllowed.getParameter<std::vector<std::string> >("allowedLabels") == std::vector<std::string>());
      assert(psetAllowed.getUntrackedParameter<std::vector<std::string> >("allowedLabelsUntracked") == 
             std::vector<std::string>());
      assert(!psetAllowed.exists("optAllowedLabels"));
      assert(!psetAllowed.exists("optAllowedLabelsUntracked"));
    }
  ...
  fillDescriptions(edm::ConfigurationDescriptions & descriptions) {
    ...
    wildcardPset.addWildcardUntracked<double>(std::string("*"));
    iDesc.add("wildcardPset", wildcardPset);

    // ---------------------------------------------

    edm::ParameterSetDescription switchPset;
    switchPset.ifValue( edm::ParameterDescription<int>("iswitch", 1, true),
                        0 >> edm::ParameterDescription<int>("ivalue", 100, true) or
                        1 >> (edm::ParameterDescription<double>("ivalue1", 101.0, true) and 
    ...
    xorPset.addNode( edm::ParameterDescription<std::string>("type", "11", true) xor 
                     edm::ParameterDescription<unsigned int>("type", 11U, true) );

    xorPset.addNode( edm::ParameterDescription<std::string>("type1", "11", true) xor 
                     edm::ParameterDescription<unsigned int>("type1", 11U, true) );

    xorPset.addOptionalNode( edm::ParameterDescription<std::string>("type2", "11", true) xor 
			     edm::ParameterDescription<unsigned int>("type2", 11U, true), false );
    ...
    orPset.addOptionalNode( edm::ParameterDescription<std::string>("z1", "11", true) or 
			    edm::ParameterDescription<unsigned int>("z2", 11U, true), false );
    ...
    allowedLabelsPset.labelsFrom<int>("allowedLabels");
    allowedLabelsPset.labelsFromUntracked<unsigned>(std::string("allowedLabelsUntracked"));
    allowedLabelsPset.labelsFromOptional<int>("optAllowedLabels", false);
    allowedLabelsPset.labelsFromOptionalUntracked<unsigned>(std::string("optAllowedLabelsUntracked"), false);
