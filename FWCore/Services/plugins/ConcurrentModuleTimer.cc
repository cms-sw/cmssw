...
class ConcurrentModuleTimer {
 public:
  ConcurrentModuleTimer(edm::ParameterSet const& iConfig,
 ...
 private:
  ...
  std::unique_ptr<std::atomic<std::chrono::high_resolution_clock::rep>[]>
      m_timeSums;
...
ConcurrentModuleTimer::ConcurrentModuleTimer(edm::ParameterSet const& iConfig,
    ...
    : m_modulesToExclude(
          iConfig.getUntrackedParameter<std::vector<std::string>>(
              "modulesToExclude")),
      m_time(),
      m_nModules(0),
      m_spinLock{false},
  ...
  if (not m_modulesToExclude.empty()) {
    ...
    iReg.watchPreModuleEvent(
        [this](StreamContext const&, ModuleCallingContext const& iContext) {
          if (trackModule(iContext)) {
            start();
          }
        });
    iReg.watchPostModuleEvent(
        [this](StreamContext const&, ModuleCallingContext const& iContext) {
          if (trackModule(iContext)) {
            stop();
          }
    ...
    iReg.watchPreModuleEventDelayedGet(
        [this](StreamContext const&, ModuleCallingContext const& iContext) {
          if (trackModule(iContext)) {
    ...
    iReg.watchPostModuleEventDelayedGet(
        [this](StreamContext const&, ModuleCallingContext const& iContext) {
          if (trackModule(iContext)) {
            if (iContext.state() == ModuleCallingContext::State::kRunning) {
              start();
    ...
    iReg.watchPreModuleEvent(
        [this](StreamContext const&, ModuleCallingContext const&) { start(); });
    iReg.watchPostModuleEvent(
        [this](StreamContext const&, ModuleCallingContext const&) { stop(); });
...
ConcurrentModuleTimer::~ConcurrentModuleTimer() {
  ...
  for (unsigned int i = 0; i < m_nTimeSums; ++i) {
    std::cout << i << " " << m_timeSums[i] / double(m_timeSums[0]) << " "
              << m_timeSums[i] << std::endl;
  }
}

// ConcurrentModuleTimer::ConcurrentModuleTimer(const ConcurrentModuleTimer&
// rhs)
// {
...
//
// const ConcurrentModuleTimer& ConcurrentModuleTimer::operator=(const
// ConcurrentModuleTimer& rhs)
// {
...
void ConcurrentModuleTimer::start() {
  ...
  while (not m_spinLock.compare_exchange_strong(expected, true,
...
void ConcurrentModuleTimer::stop() {
  ...
  bool expected = false;
  unsigned int nModules;
  while (not m_spinLock.compare_exchange_weak(expected, true,
                                              std::memory_order_acq_rel)) {
    expected = false;
  }
  {
...
//
bool ConcurrentModuleTimer::trackModule(
    ModuleCallingContext const& iContext) const {
...
//
void ConcurrentModuleTimer::fillDescriptions(
    edm::ConfigurationDescriptions& descriptions) {
  edm::ParameterSetDescription desc;
  desc.addUntracked<std::vector<std::string>>("modulesToExclude",
                                              std::vector<std::string>{})
      ->setComment("Module labels to exclude from the timing measurements");
  desc.addUntracked<bool>("excludeSource", false)
      ->setComment("Exclude the time the source is running");
