...
class ResourceEnforcer {
 public:
  ResourceEnforcer(edm::ParameterSet const& iConfig,
...
ResourceEnforcer::ResourceEnforcer(edm::ParameterSet const& iConfig,
    ...
    : m_maxVSize(iConfig.getUntrackedParameter<double>("maxVSize", 0) *
                 1000.),  // convert to MB
...
void ResourceEnforcer::postEventProcessing(StreamContext const&) {
  ...
  if (count > m_nEventsToSkip) {
    bool expected = false;
    if (m_doingCheck.compare_exchange_strong(expected, true,
...
void ResourceEnforcer::check() {
  ...
  if (0 != m_maxVSize && m_maxVSize < pi.vsize) {
    throw edm::Exception(errors::ExceededResourceVSize)
        << "Exceeded maximum allowed VSize of " << m_maxVSize / 1000.
        << " GB (VSize is " << pi.vsize / 1000. << ")";
  }

  if (0 != m_maxRSS && m_maxRSS < pi.rss) {
    throw edm::Exception(errors::ExceededResourceRSS)
        << "Exceeded maximum allowed RSS of " << m_maxRSS / 1000.
        << " GB (VSize is " << pi.rss / 1000. << ")";
  }

  if (0 != m_maxTime && m_maxTime < m_timer.realTime()) {
    throw edm::Exception(errors::ExceededResourceTime)
        << "Exceeded maximum allowed time of " << m_maxTime / 60. / 60.
        << " hours";
  }
}
...
void ResourceEnforcer::fillDescriptions(
  ...
  desc.addUntracked<double>("maxTime", 0.)
      ->setComment(
          "Maximum allowd wallclock time for the job in hours. Ignored if set "
          "to 0.");
