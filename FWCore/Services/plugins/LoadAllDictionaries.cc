...
class LoadAllDictionaries {
 ...
 private:
  ...
  const LoadAllDictionaries& operator=(
      const LoadAllDictionaries&);  // stop default
...
edm::service::LoadAllDictionaries::LoadAllDictionaries(
  ...
  if (doLoad) {
    ...
    CatToInfos::const_iterator itFound =
        db->categoryToInfos().find("Capability");
    ...
    for (edmplugin::PluginManager::Infos::const_iterator
             itInfo = itFound->second.begin(),
      ...
      // NOTE: since we have the library already, we could be more efficient if
      // we just load it ourselves
    }
  }
}

void edm::service::LoadAllDictionaries::fillDescriptions(
    edm::ConfigurationDescriptions& descriptions) {
  edm::ParameterSetDescription desc;
  desc.addUntracked<bool>("doLoad", true)
      ->setComment("Only if 'true' do we load all dictionaries");
  descriptions.add("LoadAllDictionaries", desc);
  descriptions.setComment(
      "This service allows you to force all known dictionaries to be loaded at "
