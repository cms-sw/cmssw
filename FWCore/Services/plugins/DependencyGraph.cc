...
private:
  ...
  static constexpr const char *module_type_desc[]{"Unknown",    "Source",     "ESSource", "ESProducer",
...
DependencyGraph::EDMModuleType DependencyGraph::edmModuleTypeEnum(edm::ModuleDescription const &module) {
  ...
  for (EDMModuleType v :
       {EDMModuleType::Source, EDMModuleType::ESSource, EDMModuleType::ESProducer, EDMModuleType::EDAnalyzer,
        EDMModuleType::EDProducer, EDMModuleType::EDFilter, EDMModuleType::OutputModule}) {
...
void DependencyGraph::preBeginJob(PathsAndConsumesOfModulesBase const &pathsAndConsumes,
  ...
  for (edm::ModuleDescription const *module : pathsAndConsumes.allModules()) {
    m_graph.m_graph[module->id()] = {module->moduleLabel(), module->moduleName(), module->id(),
  ...
  // marke the modules in the paths as scheduled, and add a soft dependency to reflect the order of modules along each
  // path
