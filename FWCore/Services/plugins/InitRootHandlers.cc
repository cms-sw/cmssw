...
namespace {
  ...
  void RootErrorHandlerImpl(int level, char const* location, char const* message) {
    ...
    if ((el_message.find("no dictionary for class") != std::string::npos) ||
        ...
        (el_message.find("Announced number of args different from the real number of argument passed") !=
         std::string::npos) ||  // Always printed if gDebug>0 - regardless of whether warning message is real.
...
// made when we're convinced accessing the thread-local current module is safe.
    ...
    if (tids.count(self) > 0) {
      ...
      if (edm::CurrentModuleOnThread::getCurrentModuleOnThread() != nullptr) {
        strlcat(buff, edm::CurrentModuleOnThread::getCurrentModuleOnThread()->moduleDescription()->moduleName().c_str(),
                moduleBufferSize);
        strlcat(buff, ":", moduleBufferSize);
        strlcat(buff,
                edm::CurrentModuleOnThread::getCurrentModuleOnThread()->moduleDescription()->moduleLabel().c_str(),
...
// calls dlsym.
    ...
    void InitRootHandlers::fillDescriptions(ConfigurationDescriptions& descriptions) {
      ...
      desc.addUntracked<bool>("EnableIMT", false)->setComment("If True, calls ROOT::EnableImplicitMT().");
