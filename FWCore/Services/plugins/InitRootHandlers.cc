...
void RootErrorHandlerImpl(int level, char const* location,
                          char const* message) {
  bool die = false;
  ...
  if (level >= kFatal) {
    ...
    el_severity =
        s_ignoreWarnings ? SeverityLevel::kInfo : SeverityLevel::kWarning;
  }
  ...
  if ((el_message.find("no dictionary for class") != std::string::npos) ||
      ...
      (el_message.find("Announced number of args different from the real "
                       "number of argument passed") !=
       std::string::npos) ||  // Always printed if gDebug>0 - regardless of
                              // whether warning message is real.
      ...
      (el_message.find("nbins is <=0 - set to nbins = 1") !=
       std::string::npos) ||
      (el_message.find("nbinsy is <=0 - set to nbinsy = 1") !=
       std::string::npos) ||
      (level < kError and
       (el_location.find("CINTTypedefBuilder::Setup") != std::string::npos) and
       (el_message.find("possible entries are in use!") !=
        std::string::npos))) {
    el_severity = SeverityLevel::kInfo;
  }
  ...
  if (die &&
      (el_location != std::string("@SUB=TUnixSystem::DispatchSignals"))) {
...
void RootErrorHandler(int level, bool, char const* location,
                      char const* message) {
...
static int full_read(int fd, char* inbuf, size_t len, int timeout_s = -1) {
  ...
  if (timeout_s < 0) {
    flags =
        O_NONBLOCK;  // Prevents us from trying to set / restore flags later.
  ...
  while (count) {
    if (timeout_s >= 0) {
      ...
      int ms_remaining = std::chrono::duration_cast<std::chrono::milliseconds>(
...
void sig_pause_for_stacktrace(int sig, siginfo_t*, void*) {
  ...
  if (InitRootHandlers::doneModules_.is_lock_free() &&
      InitRootHandlers::nextModule_.is_lock_free()) {
...
// resume the signal handlers to store the current module; we are not guaranteed
// they
// will have time to store their modules, so there is a race condition; this
// could be
// avoided by storing the module information before sleeping, a change that may
// be
// made when we're convinced accessing the thread-local current module is safe.
  ...
  // Checking tids.count(self) ensures that we only try to access the current
  // module in
  // CMSSW/TBB threads.  Those threads access the thread-local current module at
  // the same
  // time the thread is registered, so any lazy allocation will have been done
  // at that
  // point.  Not necessary on Linux with the current cmsRun linkage, as the
  // thread-local
...
#ifdef PAUSE_SIGNAL
  // wait a short interval for the paused threads to resume and fill in their
  // module
...
/*
 * We've run into issues where GDB fails to print the thread which calls
 * clone().
 * To avoid this problem, we have an alternate approach below where the signal
 * handler
 * only reads/writes to a dedicated thread via pipes.  The helper thread does
 * the clone()
 * invocation; we don't care if that thread is missing from the traceback in
 * this case.
 */
...
void InitRootHandlers::stacktraceHelperThread() {
  ...
  while (true) {
    ...
    if (result < 0) {
      ...
      // We never set them back to the CMSSW handler because we assume the
      // parent
...
void cmssw_stacktrace_fork() {
  ...
  // On Linux, we currently use jemalloc.  This registers pthread_atfork
  // handlers; these
  // handlers are *not* async-signal safe.  Hence, a deadlock is possible if we
  // invoke
  // fork() from our signal handlers.  Accordingly, we use clone (not POSIX, but
  // AS-safe)
  // as that is closer to the 'raw metal' syscall and avoids pthread_atfork
  // handlers.
  int pid =
#ifdef __linux__
      clone(edm::service::cmssw_stacktrace, child_stack_ptr,
            CLONE_VM | CLONE_FS | SIGCHLD, nullptr);
#else
      fork();
  if (child_stack_ptr) {
...
char* const InitRootHandlers::pstackArgv_[] = {
    pstackName, dashC, InitRootHandlers::pidString_, nullptr};
...
std::vector<std::array<char, moduleBufferSize>>
    InitRootHandlers::moduleListBuffers_;
std::atomic<std::size_t> InitRootHandlers::nextModule_(0),
    InitRootHandlers::doneModules_(0);
...
InitRootHandlers::InitRootHandlers(ParameterSet const& pset,
    ...
    : RootHandlers(),
      unloadSigHandler_(
          pset.getUntrackedParameter<bool>("UnloadRootSigHandler")),
      resetErrHandler_(pset.getUntrackedParameter<bool>("ResetRootErrHandler")),
      loadAllDictionaries_(
          pset.getUntrackedParameter<bool>("LoadAllDictionaries")),
      autoLibraryLoader_(
          loadAllDictionaries_ or
          pset.getUntrackedParameter<bool>("AutoLibraryLoader")) {
  ...
  if (unloadSigHandler_) {
    ...
    sigBusHandler_ = std::shared_ptr<const void>(
        nullptr, [](void*) { installCustomHandler(SIGBUS, sig_abort); });
    installCustomHandler(SIGSEGV, sig_dostack_then_abort);
    sigSegvHandler_ = std::shared_ptr<const void>(
        nullptr, [](void*) { installCustomHandler(SIGSEGV, sig_abort); });
    installCustomHandler(SIGILL, sig_dostack_then_abort);
    sigIllHandler_ = std::shared_ptr<const void>(
        nullptr, [](void*) { installCustomHandler(SIGILL, sig_abort); });
    installCustomHandler(SIGTERM, sig_dostack_then_abort);
    sigTermHandler_ = std::shared_ptr<const void>(
        nullptr, [](void*) { installCustomHandler(SIGTERM, sig_abort); });
  }
  ...
  // Load the library containing dictionaries for std:: classes, if not already
  // loaded.
...
void InitRootHandlers::fillDescriptions(
  ...
  desc.addUntracked<bool>("ResetRootErrHandler", true)
      ->setComment(
          "If True, ROOT messages (e.g. errors, warnings) are handled by this "
          "service, rather than by ROOT.");
  ...
  desc.addUntracked<bool>("AbortOnSignal", true)
      ->setComment(
          "If True, do an abort when a signal occurs that causes a crash. If "
          "False, ROOT will do an exit which attempts to do a clean shutdown.");
...
void InitRootHandlers::enableWarnings_() { s_ignoreWarnings = false; }

void InitRootHandlers::ignoreWarnings_() { s_ignoreWarnings = true; }
...
void InitRootHandlers::cachePidInfo() {
  ...
  if (snprintf(pidString_, pidStringLength_ - 1,
               "gdb -quiet -p %d 2>&1 <<EOF |\n"
               "set width 0\n"
               "set height 0\n"
               "set pagination no\n"
               "thread apply all bt\n"
  ...
  if (-1 == pipe2(childToParent_, O_CLOEXEC)) {
    std::ostringstream sstr;
    sstr << "Failed to create child-to-parent pipes (errno=" << errno
         << "): " << strerror(errno);
    edm::Exception except(edm::errors::OtherCMS, sstr.str());
    throw except;
  }
