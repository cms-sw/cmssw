...
Timing::Timing(ParameterSet const& iPS, ActivityRegistry& iRegistry)
  ...
  iRegistry.preallocateSignal_.connect(
      [this](service::SystemBounds const& iBounds) {
...
void Timing::fillDescriptions(ConfigurationDescriptions& descriptions) {
  ...
  desc.addUntracked<bool>("summaryOnly", false)
      ->setComment("If 'true' do not report timing for each event");
  desc.addUntracked<bool>("useJobReport", true)
      ->setComment("If 'true' write summary information to JobReport");
  desc.addUntracked<double>("excessiveTimeThreshold", 0.)
      ->setComment(
          "Amount of time in seconds before reporting a module or source has "
          "taken excessive time. A value of 0.0 turns off this reporting.");
...
void Timing::postEndJob() {
  ...
  double min_event_time =
      *(std::min_element(min_events_time_.begin(), min_events_time_.end()));
  double max_event_time =
      *(std::max_element(max_events_time_.begin(), max_events_time_.end()));
  ...
  LogImportant("TimeReport")
      << "TimeReport> Time report complete in " << total_job_time << " seconds"
      << "\n"
      << " Time Summary: \n"
  ...
  if (report_summary_) {
    ...
    reportData.insert(
        std::make_pair("AvgEventTime", d2str(average_event_time)));
    reportData.insert(
        std::make_pair("EventThroughput", d2str(event_throughput)));
...
void Timing::postEvent(StreamContext const& iStream) {
  ...
  if (not summary_only_) {
    ...
    LogPrint("TimeEvent") << "TimeEvent> " << eventID.event() << " "
                          << eventID.run() << " " << curr_event_time;
  }
...
void Timing::postModuleEvent(StreamContext const& iStream,
                             ModuleCallingContext const& iModule) {
  auto const& eventID = iStream.eventID();
  auto const& desc = *(iModule.moduleDescription());
  double t = postCommon();
  if (not summary_only_) {
    LogPrint("TimeModule") << "TimeModule> " << eventID.event() << " "
                           << eventID.run() << " " << desc.moduleLabel() << " "
                           << desc.moduleName() << " " << t;
  }
}

void Timing::preSourceEvent(StreamID sid) { pushStack(); }

void Timing::postSourceEvent(StreamID sid) { postCommon(); }

void Timing::preSourceLumi() { pushStack(); }

void Timing::postSourceLumi() { postCommon(); }

void Timing::preSourceRun() { pushStack(); }

void Timing::postSourceRun() { postCommon(); }

void Timing::preOpenFile(std::string const& lfn, bool b) { pushStack(); }
...
void Timing::preModuleGlobal(GlobalContext const&,
                             ModuleCallingContext const&) {
  pushStack();
}

void Timing::postModuleGlobal(GlobalContext const&,
                              ModuleCallingContext const& mcc) {
  postCommon();
}

void Timing::preModuleStream(StreamContext const&,
                             ModuleCallingContext const&) {
  pushStack();
}

void Timing::postModuleStream(StreamContext const&,
                              ModuleCallingContext const& mcc) {
  postCommon();
}
...
double Timing::postCommon() const {
  ...
  if (t > threshold_) {
    LogError("ExcessiveTime")
        ...
        << " seconds of time which exceeds the error threshold configured in "
           "the Timing Service of "
        << threshold_ << " seconds.";
  }
  return t;
}
...
typedef edm::serviceregistry::AllArgsMaker<edm::TimingServiceBase, Timing>
    TimingMaker;
