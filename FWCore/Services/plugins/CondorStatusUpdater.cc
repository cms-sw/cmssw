...
class CondorStatusService {
 public:
  explicit CondorStatusService(ParameterSet const &pset,
 ...
 private:
  ...
  bool updateChirpQuoted(const std::string &key_suffix,
  ...
  void preSourceConstruction(ModuleDescription const &md, int maxEvents,
                             int maxLumis, int maxSecondsUntilRampdown);
  void eventPost(StreamContext const &iContext);
  void lumiPost(GlobalContext const &);
  void runPost(GlobalContext const &);
  void beginPre(PathsAndConsumesOfModulesBase const &,
  ...
  static constexpr float m_defaultEmaInterval =
      15 * 60;  // Time in seconds to average EMA over for event rate.
...
CondorStatusService::CondorStatusService(ParameterSet const &pset,
  ...
  if (pset.exists("updateIntervalSeconds")) {
    m_updateInterval =
        pset.getUntrackedParameter<unsigned int>("updateIntervalSeconds");
  }
...
void CondorStatusService::beginPost() {
  ParameterSet const &processParameterSet =
      edm::getParameterSet(m_processParameterSetID);
  const edm::ParameterSet &pset =
      processParameterSet.getParameterSet("@main_input");
  // PSet info from edm::ScheduleItems
  int maxEvents =
      processParameterSet.getUntrackedParameterSet("maxEvents", ParameterSet())
          .getUntrackedParameter<int>("input", -1);
  int maxLumis =
      processParameterSet
          .getUntrackedParameterSet("maxLuminosityBlocks", ParameterSet())
          .getUntrackedParameter<int>("input", -1);

  // lumisToProcess from EventSkipperByID (PoolSource and similar)
  std::vector<edm::LuminosityBlockRange> toProcess =
  ...
  unsigned int eventsPerLumi = pset.getUntrackedParameter<unsigned int>(
      "numberEventsInLuminosityBlock", 0);
  if ((lumiCount == 0) && (maxEvents > 0) && (eventsPerLumi > 0)) {
    lumiCount = static_cast<unsigned int>(std::ceil(
        static_cast<float>(maxEvents) / static_cast<float>(eventsPerLumi)));
  }
    ...
    for (const auto &counter : storage.second) {
      if (counter.first == static_cast<int>(StorageAccount::Operation::read)) {
        ...
        readTimeTotal += counter.second.timeTotal;
                 ...
                 static_cast<int>(StorageAccount::Operation::readv)) {
        readVOps += counter.second.successes;
        readSegs += counter.second.vector_count;
        readBytes += counter.second.amount;
        readTimeTotal += counter.second.timeTotal;
      } else if ((counter.first ==
                  static_cast<int>(StorageAccount::Operation::write)) ||
                 (counter.first ==
...
bool CondorStatusService::updateChirpQuoted(const std::string &key_suffix,
  ...
  // Remove double-quotes or the \ character (as it has special escaping
  // semantics in ClassAds).
  // Make sure we have ASCII characters.
  // Otherwise, remainder is allowed (including tabs, newlines, single-quotes).
  value_copy.erase(remove_if(value_copy.begin(), value_copy.end(),
                             [](const char &c) {
...
bool CondorStatusService::updateChirpImpl(const std::string &key_suffix,
                                          const std::string &value) {
  std::stringstream ss;
  ss << "ChirpCMSSW" << m_tag << key_suffix;
  std::string key = ss.str();
  if (m_debug) {
    std::cout << "condor_chirp set_job_attr_delayed " << key << " " << value
              << std::endl;
  }
  int pid = 0;
  ...
  argv.push_back(NULL);
  int status = posix_spawnp(&pid, "condor_chirp", &file_actions, NULL,
                            const_cast<char *const *>(&argv[0]), environ);
  close(devnull_fd);
...
void CondorStatusService::fillDescriptions(
    ConfigurationDescriptions &descriptions) {
  ParameterSetDescription desc;
  desc.setComment("Service to update HTCondor with the current CMSSW status.");
  desc.addOptionalUntracked<unsigned int>("updateIntervalSeconds",
                                          m_defaultUpdateInterval)
  ...
  desc.addOptionalUntracked<std::string>("tag")->setComment(
      "Identifier tag for this process (a value of 'Foo' results in ClassAd "
      "attributes of the form 'ChirpCMSSWFoo*')");
  descriptions.add("CondorStatusService", desc);
}

typedef edm::serviceregistry::AllArgsMaker<edm::service::CondorStatusService>
