...
class SimpleMemoryCheck {
 ...
 private:
  ...
  void update();
  void updateMax();
  void andPrint(const std::string& type, const std::string& mdlabel,
                const std::string& mdname) const;
  void updateAndPrint(const std::string& type, const std::string& mdlabel,
                      const std::string& mdname);
  void openFiles();

  char const* smapsLineBuffer() const {
    return get_underlying_safe(smapsLineBuffer_);
  }
  ...
  struct SignificantEvent {
    ...
    SignificantEvent()
        : count(0),
          ...
          pss(0),
          event() {}
    void set(double deltaV, double deltaR, edm::EventID const& e,
             SimpleMemoryCheck* t) {
      count = t->count_;
  ...
  friend std::ostream& operator<<(
      std::ostream& os, SimpleMemoryCheck::SignificantEvent const& se);
  ...
  void eventStatOutput(std::string title, SignificantEvent const& e,
  ...
  friend std::ostream& operator<<(
      std::ostream& os, SimpleMemoryCheck::SignificantModule const& se);
...
smapsInfo SimpleMemoryCheck::fetchSmaps() {
  ...
  while ((read = getline(&smapsLineBuffer(), &smapsLineBufferLen_,
                         smapsFile_)) != -1) {
    if (read > 14) {
      // Private
...
double SimpleMemoryCheck::averageGrowthRate(double current, double past,
                                            int count) {
...
SimpleMemoryCheck::SimpleMemoryCheck(ParameterSet const& iPS,
    ...
    : a_(),
      ...
      jobReportOutputOnly_(
          iPS.getUntrackedParameter<bool>("jobReportOutputOnly")),
      monitorPssAndPrivate_(
          iPS.getUntrackedParameter<bool>("monitorPssAndPrivate")),
      count_(),
      smapsFile_(nullptr),
  ...
  if (!oncePerEventMode_) {  // default, prints on increases
    ...
    iReg.watchPostSourceConstruction(
        this, &SimpleMemoryCheck::postSourceConstruction);
    iReg.watchPostSourceEvent(this, &SimpleMemoryCheck::postSourceEvent);
    iReg.watchPostModuleConstruction(
        this, &SimpleMemoryCheck::postModuleConstruction);
...
void SimpleMemoryCheck::fillDescriptions(
  ...
  if (monitorPssAndPrivate_) {
    ...
    if ((smapsFile_ = fopen(smapsNameOst.str().c_str(), "r")) == 0) {
      throw Exception(errors::Configuration)
          << "Failed to open smaps file " << smapsNameOst.str() << std::endl;
    }
  }
#endif
}
...
void SimpleMemoryCheck::preSourceConstruction(ModuleDescription const& md) {
  bool expected = false;
  if (measurementUnderway_.compare_exchange_strong(expected, true,
...
#define SIMPLE_MEMORY_CHECK_ORIGINAL_XML_OUTPUT
  ...
  if (eventDeltaRssT3_.deltaRss > 0)
    eventStatOutput("ThirdLargestIncreaseRssEvent", eventDeltaRssT3_,
                    reportData);
  if (eventDeltaRssT2_.deltaRss > 0)
    eventStatOutput("SecondLargestIncreaseRssEvent", eventDeltaRssT2_,
                    reportData);
  ...
  reportData.insert(std::make_pair(
  ...
  reportData.insert(std::make_pair(
      ...
      d2str(averageGrowthRate(current_->rss, growthRateRss_, count_))));
  ...
  if (moduleSummaryRequested_) {  // changelog 2
    for (SignificantModulesMap::iterator im = modules_.begin();
      ...
      if (m.totalEarlyVsize > 0) {
        reportData.insert(std::make_pair(label + "TotalEarlyVsize",
                                         d2str(m.totalEarlyVsize)));
        reportData.insert(std::make_pair(label + "MaxEarlyDeltaVsize",
                                         d2str(m.maxEarlyVsize)));
      }
    }
  }
  ...
  if (FILE* fmeminfo = fopen("/proc/meminfo", "r")) {
    ...
    while (fgets(buf, sizeof(buf), fmeminfo)) {
      ...
      if (token != nullptr) {
        ...
        reportMemoryProperties.insert(std::make_pair(
            category.substr(0, strlen(token) - 1), i2str(value)));
      }
    }
...
void SimpleMemoryCheck::preModule(StreamContext const& iStreamContext,
  ...
  if (measurementUnderway_.compare_exchange_strong(expected, true,
    ...
    if (moduleMeasurementUnderway_.compare_exchange_strong(expected, true)) {
      ...
      moduleStreamID_.store(iStreamContext.streamID().value(),
                            std::memory_order_release);
      moduleID_.store(iModuleContext.moduleDescription()->id(),
                      std::memory_order_release);
    }
  }
}
...
void SimpleMemoryCheck::postModule(StreamContext const& iStreamContext,
  ...
  if (!oncePerEventMode_) {
    bool expected = false;
    if (measurementUnderway_.compare_exchange_strong(
  ...
  if (moduleSummaryRequested_) {
    ...
    if (moduleMeasurementUnderway_.load(std::memory_order_acquire) and
        (iStreamContext.streamID().value() ==
         moduleStreamID_.load(std::memory_order_acquire)) and
        (iModuleContext.moduleDescription()->id() ==
         moduleID_.load(std::memory_order_acquire))) {
    ...
    if (count_ >= num_to_skip_) {
      ...
      if (!showMallocInfo_) {  // default
        LogWarning("MemoryCheck")
            << "MemoryCheck: " << type << " " << mdname << ":" << mdlabel
            << " VSIZE " << current_->vsize << " " << deltaVSIZE << " RSS "
            << current_->rss << " " << deltaRSS;
      } else {
#ifdef __linux__
        struct mallinfo minfo = mallinfo();
#endif
...
#ifdef SIMPLE_MEMORY_CHECK_ORIGINAL_XML_OUTPUT
void SimpleMemoryCheck::eventStatOutput(
    std::string title, SignificantEvent const& e,
    std::map<std::string, std::string>& m) const {
  {
  ...
  }
  {
    ...
    os << title << "-b-RUN";
    m.insert(
        std::make_pair(os.str(), d2str(static_cast<double>(e.event.run()))));
  }
  {
    std::ostringstream os;
    os << title << "-c-EVENT";
    m.insert(
        std::make_pair(os.str(), d2str(static_cast<double>(e.event.event()))));
  }
  {
    std::ostringstream os;
    os << title << "-d-VSIZE";
...
#ifdef SIMPLE_MEMORY_CHECK_DIFFERENT_XML_OUTPUT
std::string SimpleMemoryCheck::eventStatOutput(
    std::string title, SignificantEvent const& e) const {
  std::ostringstream os;
  os << "  <" << title << ">\n";
  os << "    " << e.count << ": " << e.event;
...
std::string SimpleMemoryCheck::mallOutput(std::string title,
...
#endif
// changelog 2
void SimpleMemoryCheck::updateModuleMemoryStats(
    SignificantModule& m, double dv, edm::EventID const& currentEventID) {
  if (count_ < num_to_skip_) {
...
std::ostream& operator<<(std::ostream& os,
                         SimpleMemoryCheck::SignificantEvent const& se) {
  os << "[" << se.count << "] " << se.event << "  vsize = " << se.vsize
     << " deltaVsize = " << se.deltaVsize << " rss = " << se.rss
     << " delta = " << se.deltaRss;
...
std::ostream& operator<<(std::ostream& os,
  ...
  if (sm.postEarlyCount > 0) {
    os << "\nPost Early Events:  TotalDeltaVsize: " << sm.totalDeltaVsize
       ...
       << "; max: " << sm.maxDeltaVsize << " during " << sm.eventMaxDeltaV
       << ")";
  }
