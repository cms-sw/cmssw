...
class CPU : public CPUServiceBase {
 ...
 private:
  ...
  bool cpuInfoImpl(std::string &models, double &avgSpeed,
...
CPU::CPU(const ParameterSet &iPS, ActivityRegistry &iRegistry)
    : reportCPUProperties_(
          iPS.getUntrackedParameter<bool>("reportCPUProperties")) {
...
void CPU::postEndJob() {
    ...
    if (entry.first == "processor") {
      if (reportCPUProperties_) {
        if (currentCore.empty()) {  // first core
          ...
          reportSvc->reportPerformanceForModule(
              "SystemCPU", "CPU-" + currentCore, currentCoreProperties);
  ...
  if (!currentCore.empty() && reportCPUProperties_) {
    reportSvc->reportPerformanceForModule("SystemCPU", "CPU-" + currentCore,
                                          currentCoreProperties);
  }
