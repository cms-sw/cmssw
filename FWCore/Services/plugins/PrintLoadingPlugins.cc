...
class PrintLoadingPlugins {
 ...
 private:
  ...
  const PrintLoadingPlugins& operator=(
      const PrintLoadingPlugins&);  // stop default
...
PrintLoadingPlugins::PrintLoadingPlugins() {
  ...
  pm->goingToLoad_.connect(
      std::bind(std::mem_fn(&PrintLoadingPlugins::goingToLoad), this, _1));
}
...
void PrintLoadingPlugins::fillDescriptions(
...
//
// const PrintLoadingPlugins& PrintLoadingPlugins::operator=(const
// PrintLoadingPlugins& rhs)
// {
...
void PrintLoadingPlugins::askedToLoad(const std::string& iCategory,
  ...
  PluginManager::CategoryToInfos::const_iterator itFound =
...
typedef edm::serviceregistry::NoArgsMaker<PrintLoadingPlugins>
