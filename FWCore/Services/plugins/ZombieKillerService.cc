...
ZombieKillerService::ZombieKillerService(edm::ParameterSet const& iPSet,
    ...
    : m_checkThreshold(iPSet.getUntrackedParameter<unsigned int>(
          "numberOfAllowedFailedChecksInARow")),
      m_secsBetweenChecks(
          iPSet.getUntrackedParameter<unsigned int>("secondsBetweenChecks")),
      m_jobDone(false),
      m_stillAlive(true),
  ...
  iRegistry.watchPreModuleBeginStream([this](
      StreamContext const&, ModuleCallingContext const&) { notAZombieYet(); });
  iRegistry.watchPostModuleBeginStream([this](
      StreamContext const&, ModuleCallingContext const&) { notAZombieYet(); });

  iRegistry.watchPreModuleEndStream([this](
      StreamContext const&, ModuleCallingContext const&) { notAZombieYet(); });
  iRegistry.watchPostModuleEndStream([this](
      StreamContext const&, ModuleCallingContext const&) { notAZombieYet(); });

  iRegistry.watchPreModuleEndJob(
      [this](ModuleDescription const&) { notAZombieYet(); });
  iRegistry.watchPostModuleEndJob(
      [this](ModuleDescription const&) { notAZombieYet(); });
  iRegistry.watchPreModuleEvent([this](
      StreamContext const&, ModuleCallingContext const&) { notAZombieYet(); });
  iRegistry.watchPostModuleEvent([this](
      StreamContext const&, ModuleCallingContext const&) { notAZombieYet(); });
  ...
  iRegistry.watchPreModuleGlobalBeginRun([this](
      GlobalContext const&, ModuleCallingContext const&) { notAZombieYet(); });
...
//
// const ZombieKillerService& ZombieKillerService::operator=(const
// ZombieKillerService& rhs)
// {
...
void ZombieKillerService::startThread() {
  m_watchingThread = std::thread([this]() {
    ...
    while (not m_jobDoneCondition.wait_for(
        lock, std::chrono::seconds(m_secsBetweenChecks),
...
void ZombieKillerService::fillDescriptions(
    ConfigurationDescriptions& descriptions) {
  ParameterSetDescription desc;
  desc.addUntracked<unsigned int>("secondsBetweenChecks", 60)
      ->setComment(
          "Number of seconds to wait between checking if progress has been "
          "made.");
