...
template <typename T>
std::enable_if_t<std::is_integral<T>::value> concatenate(std::ostream& os,
                                                         T const t) {
  os << ' ' << t;
}

template <typename H, typename... T>
std::enable_if_t<std::is_integral<H>::value> concatenate(std::ostream& os,
                                                         H const h,
                                                         T const... t) {
  os << ' ' << h;
  concatenate(os, t...);
...
class StallMonitor {
 ...
 private:
  ...
  bool const
      validFile_;  // Separate data member from file to improve efficiency.
...
StallMonitor::StallMonitor(ParameterSet const& iPS, ActivityRegistry& iRegistry)
    : file_{iPS.getUntrackedParameter<std::string>("fileName",
                                                   filename_default)},
      validFile_{file_},
      stallThreshold_{static_cast<long int>(
          iPS.getUntrackedParameter<double>("stallThreshold") * 1000)} {
  iRegistry.watchPreModuleConstruction(this,
                                       &StallMonitor::preModuleConstruction);
  iRegistry.watchPostBeginJob(this, &StallMonitor::postBeginJob);
  iRegistry.watchPostModuleEventPrefetching(
      this, &StallMonitor::postModuleEventPrefetching);
  ...
  if (validFile_) {
    ...
    iRegistry.watchPreEventReadFromSource(
        this, &StallMonitor::preEventReadFromSource);
    iRegistry.watchPostEventReadFromSource(
        this, &StallMonitor::postEventReadFromSource);
    ...
    oss << "# Step                       Symbol Entries\n"
        ...
        << "   <Stream ID> <Run#> <LumiBlock#> <Event#> <Time since beginJob "
           "(ms)>\n"
        << "# postModuleEventPrefetching    "
        << step::postModuleEventPrefetching
...
void StallMonitor::fillDescriptions(ConfigurationDescriptions& descriptions) {
  ...
  desc.addUntracked<std::string>("fileName", filename_default)
      ->setComment(
          "Name of file to which detailed timing information should be "
          "written.\n"
          "An empty filename argument (the default) indicates that no extra\n"
          "information will be written to a dedicated file, but only the "
          "summary\n"
  ...
  descriptions.setComment(
      "This service keeps track of various times in event-processing to "
      "determine which modules are stalling.");
}
...
void StallMonitor::preEvent(StreamContext const& sc) {
  ...
  auto msg = assembleMessage<step::preEvent>(
      stream_id(sc), eid.run(), eid.luminosityBlock(), eid.event(), t);
  file_.write(std::move(msg));
}
...
void StallMonitor::preModuleEvent(StreamContext const& sc,
  ...
  auto const mid = module_id(mcc);
  if (validFile_) {
    auto msg = assembleMessage<step::preModuleEvent>(
        sid, mid,
        duration_cast<milliseconds>(preModEvent - beginTime_).count());
    file_.write(std::move(msg));
  }

  auto const preFetch_to_preModEvent = duration_cast<milliseconds>(
      preModEvent - stallStart_[std::make_pair(sid, mid)]);
...
void StallMonitor::preEventReadFromSource(StreamContext const& sc,
                                          ModuleCallingContext const& mcc) {
  auto const t = duration_cast<milliseconds>(now() - beginTime_).count();
  auto msg = assembleMessage<step::preEventReadFromSource>(stream_id(sc),
                                                           module_id(mcc), t);
  file_.write(std::move(msg));
}
...
void StallMonitor::postEventReadFromSource(StreamContext const& sc,
                                           ModuleCallingContext const& mcc) {
  auto const t = duration_cast<milliseconds>(now() - beginTime_).count();
  auto msg = assembleMessage<step::postEventReadFromSource>(stream_id(sc),
                                                            module_id(mcc), t);
  file_.write(std::move(msg));
}
...
void StallMonitor::postModuleEvent(StreamContext const& sc,
                                   ModuleCallingContext const& mcc) {
  auto const postModEvent =
      duration_cast<milliseconds>(now() - beginTime_).count();
  auto msg = assembleMessage<step::postModuleEvent>(
      stream_id(sc), module_id(mcc), postModEvent);
  file_.write(std::move(msg));
}
...
void StallMonitor::postEvent(StreamContext const& sc) {
  auto const t = duration_cast<milliseconds>(now() - beginTime_).count();
  auto const& eid = sc.eventID();
  auto msg = assembleMessage<step::postEvent>(
      stream_id(sc), eid.run(), eid.luminosityBlock(), eid.event(), t);
  file_.write(std::move(msg));
}
...
void StallMonitor::postEndJob() {
  ...
  out << tag << space << std::setfill('-') << col1(std::string{}) << space
      << col2(std::string{}) << space << col3(std::string{}) << space
      << col4(std::string{}) << '\n';
  ...
  for (auto const& stats : moduleStats_) {
    if (stats.label().empty() ||  // See comment in filling of moduleLabels_;
      ...
      continue;
    out << std::left << tag << space << col1(stats.label()) << space
        << std::right << col2(stats.numberOfStalls()) << space
