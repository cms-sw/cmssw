...
SiteLocalConfigServiceTester::SiteLocalConfigServiceTester(
    ...
    : m_cacheHint(iPSet.getUntrackedParameter<std::string>("sourceCacheHint")),
      ...
      m_ttreeCacheSize(
          iPSet.getUntrackedParameter<unsigned int>("sourceTTreeCacheSize")),
      m_nativeProtocols(iPSet.getUntrackedParameter<std::vector<std::string> >(
          "sourceNativeProtocols")),
      m_valuesSet(iPSet.getUntrackedParameter<bool>("sourceValuesSet", true)) {}

// SiteLocalConfigServiceTester::SiteLocalConfigServiceTester(const
...
//
// const SiteLocalConfigServiceTester&
// SiteLocalConfigServiceTester::operator=(const SiteLocalConfigServiceTester&
// rhs)
// {
...
template <typename T>
static void throwWrongValue(const char* iName, const T& iExpected,
                            const T& iRetrieved) {
  throw cms::Exception("TestFailure")
      << "The value " << iName << " should have been " << iExpected
...
void checkNotSet(const char* iName, const T* iRetrieved) {
  if (0 != iRetrieved) {
    throw cms::Exception("TestFailure")
        << "The value " << iName << " should not have been set but was set to "
        << *iRetrieved;
  }
}

void checkNotSet(const char* iName,
                 const std::vector<std::string>* iRetrieved) {
  if (0 != iRetrieved) {
    throw cms::Exception("TestFailure")
...
//
void SiteLocalConfigServiceTester::analyze(const edm::Event&,
  ...
  if (m_valuesSet) {
    ...
    testValue("sourceTTreeCacheSize", m_ttreeCacheSize,
              pConfig->sourceTTreeCacheSize());
    const std::vector<std::string>* protocols =
        pConfig->sourceNativeProtocols();
    if (0 == protocols) {
    ...
    for (std::vector<std::string>::const_iterator
             it = protocols->begin(),
             itEnd = protocols->end(), itExpect = m_nativeProtocols.begin();
