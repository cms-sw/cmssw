...
template <typename T>
static void overrideFromPSet(char const *iName, edm::ParameterSet const &iPSet,
...
SiteLocalConfigService::SiteLocalConfigService(ParameterSet const &pset)
  ...
  // apply overrides
  overrideFromPSet("overrideSourceCacheTempDir", pset, m_cacheTempDir,
                   m_cacheTempDirPtr);
  overrideFromPSet("overrideSourceCacheMinFree", pset, m_cacheMinFree,
                   m_cacheMinFreePtr);
  overrideFromPSet("overrideSourceCacheHintDir", pset, m_cacheHint,
                   m_cacheHintPtr);
  ...
  overrideFromPSet("overrideSourceTTreeCacheSize", pset, m_ttreeCacheSize,
                   m_ttreeCacheSizePtr);
  overrideFromPSet("overrideSourceTimeout", pset, m_timeout, m_timeoutPtr);
  overrideFromPSet("overridePrefetching", pset, m_enablePrefetching,
                   m_enablePrefetchingPtr);
  const std::string *tmpStringPtr = nullptr;
  overrideFromPSet("overrideStatisticsDestination", pset,
                   m_statisticsDestination, tmpStringPtr);
  ...
  overrideFromPSet("overrideStatisticsInfo", pset, tmpStatisticsInfo,
                   tmpStatisticsInfoPtr);
...
std::string const SiteLocalConfigService::frontierConnect(
    std::string const &servlet) const {
  if (!m_connected) {
    throw cms::Exception("Incomplete configuration")
        << "Valid site-local-config not found at " << m_url;
  }
  ...
  while ((serverurl = m_frontierConnect.find("(serverurl=", nextparen)) !=
         std::string::npos) {
  ...
  complexstr.append(m_frontierConnect, nextparen,
                    m_frontierConnect.length() - nextparen);

  return complexstr;
}

std::string const SiteLocalConfigService::lookupCalibConnect(
  ...
  if (input.substr(0, proto.length()) == proto) {
    ...
    if ((servlet != "") &&
        (servlet.find_first_of(":/)[]") == std::string::npos)) {
...
std::string const SiteLocalConfigService::rfioType(void) const {
  return m_rfioType;
}
...
double const *SiteLocalConfigService::sourceCacheMinFree() const {
  return m_cacheMinFreePtr;
}

std::string const *SiteLocalConfigService::sourceCacheHint() const {
...
std::string const *SiteLocalConfigService::sourceReadHint() const {
...
unsigned int const *SiteLocalConfigService::sourceTimeout() const {
  return m_timeoutPtr;
}
...
std::vector<std::string> const *SiteLocalConfigService::sourceNativeProtocols()
    const {
...
std::string const &SiteLocalConfigService::siteName() const {
  return m_siteName;
}
      ...
      for (XMLSize_t i = 0; i < numSites; ++i) {
        ...
        // Parsing of the calib-data section
        {
          DOMNodeList *calibDataList =
              site->getElementsByTagName(uStr("calib-data").ptr());
        ...
        // Parsing of the source config section
        {
          ...
          if (sourceConfigList->getLength() > 0) {
            ...
            DOMNodeList *cacheTempDirList = sourceConfig->getElementsByTagName(
                uStr("cache-temp-dir").ptr());

            if (cacheTempDirList->getLength() > 0) {
              DOMElement *cacheTempDir =
                  static_cast<DOMElement *>(cacheTempDirList->item(0));
              m_cacheTempDir =
                  toString(cacheTempDir->getAttribute(uStr("name").ptr()));
              m_cacheTempDirPtr = &m_cacheTempDir;
            }

            DOMNodeList *cacheMinFreeList = sourceConfig->getElementsByTagName(
                uStr("cache-min-free").ptr());

            if (cacheMinFreeList->getLength() > 0) {
              DOMElement *cacheMinFree =
                  static_cast<DOMElement *>(cacheMinFreeList->item(0));
              m_cacheMinFree =
                  toDouble(cacheMinFree->getAttribute(uStr("value").ptr()));
            ...
            if (readHintList->getLength() > 0) {
              ...
              m_readHint =
                  toString(readHint->getAttribute(uStr("value").ptr()));
              m_readHintPtr = &m_readHint;
            }

            DOMNodeList *ttreeCacheSizeList =
                sourceConfig->getElementsByTagName(
                    uStr("ttree-cache-size").ptr());

            if (ttreeCacheSizeList->getLength() > 0) {
              DOMElement *ttreeCacheSize =
                  static_cast<DOMElement *>(ttreeCacheSizeList->item(0));
              m_ttreeCacheSize =
                  toUInt(ttreeCacheSize->getAttribute(uStr("value").ptr()));
              m_ttreeCacheSizePtr = &m_ttreeCacheSize;
            }

            DOMNodeList *timeoutList = sourceConfig->getElementsByTagName(
                uStr("timeout-in-seconds").ptr());

            if (timeoutList->getLength() > 0) {
              DOMElement *timeout =
                  static_cast<DOMElement *>(timeoutList->item(0));
              m_timeout = toUInt(timeout->getAttribute(uStr("value").ptr()));
              m_timeoutPtr = &m_timeout;
            }

            DOMNodeList *statsDestList = sourceConfig->getElementsByTagName(
                uStr("statistics-destination").ptr());

            if (statsDestList->getLength() > 0) {
              DOMElement *statsDest =
                  static_cast<DOMElement *>(statsDestList->item(0));
              m_statisticsDestination =
                  toString(statsDest->getAttribute(uStr("endpoint").ptr()));
              if (!m_statisticsDestination.size()) {
                m_statisticsDestination =
                    toString(statsDest->getAttribute(uStr("name").ptr()));
              }
              std::string tmpStatisticsInfo =
                  toString(statsDest->getAttribute(uStr("info").ptr()));
              boost::split(m_statisticsInfo, tmpStatisticsInfo,
                           boost::is_any_of("\t ,"));
            ...
            if (prefetchingList->getLength() > 0) {
              DOMElement *prefetching =
                  static_cast<DOMElement *>(prefetchingList->item(0));
              m_enablePrefetching =
                  toBool(prefetching->getAttribute(uStr("value").ptr()));
              m_enablePrefetchingPtr = &m_enablePrefetching;
            }

            DOMNodeList *nativeProtocolsList =
                sourceConfig->getElementsByTagName(
                    uStr("native-protocols").ptr());
...
void SiteLocalConfigService::fillDescriptions(
  ...
  desc.addOptionalUntracked<std::string>("overrideStatisticsDestination")
      ->setComment(
          "Provide an alternate network destination for I/O statistics (must "
          "be in the form of host:port).");
  desc.addOptionalUntracked<std::vector<std::string> >("overrideStatisticsInfo")
      ->setComment(
          "Provide an alternate listing of statistics to send (comma separated "
          "list; current options are 'dn' or 'nodn').  If left blank, all "
          "information is snet (including DNs).");
