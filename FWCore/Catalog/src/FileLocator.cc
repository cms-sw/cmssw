...
std::string replaceWithRegexp(std::smatch const& matches,
...
FileLocator::FileLocator(std::string const& catUrl, bool fallback)
  ...
  try {
    ...
    throw cms::Exception("TrivialFileCatalog",
                         std::string("Fatal Error on edm::FileLocator:") +
                             toString(e.getMessage()));
  }
  ++s_numberOfInstances;
...
std::string FileLocator::convert(std::string const& input,
...
void FileLocator::parseRule(DOMNode* ruleNode, ProtocolRules& rules) {
  ...
  std::string const protocol =
      toString(ruleElement->getAttribute(uStr("protocol").ptr()));
...
void FileLocator::init(std::string const& catUrl, bool fallback) {
  ...
  if (m_url.empty()) {
    ...
    m_url = (fallback ? localconfservice->fallbackDataCatalog()
                      : localconfservice->dataCatalog());
  }
  ...
  if (m_url.find("file:") == std::string::npos) {
    throw cms::Exception("TrivialFileCatalog",
                         "TrivialFileCatalog::connect: Malformed url for file "
  ...
  if (tokens.size() == 2) {
    ...
    boost::algorithm::split(optionTokens, options,
    ...
    for (size_t oi = 0, oe = optionTokens.size(); oi != oe; ++oi) {
      ...
      if (argTokens.size() != 2) {
        throw cms::Exception("TrivialFileCatalog",
                             "TrivialFileCatalog::connect: Malformed url for "
                             "file catalog configuration");
      }

      if (argTokens[0] == "protocol") {
        boost::algorithm::split(m_protocols, argTokens[1],
                                boost::is_any_of(comma));
  ...
  if (m_protocols.empty()) {
    throw cms::Exception("TrivialFileCatalog",
                         "TrivialFileCatalog::connect: protocol was not "
                         "supplied in the contact string");
  }
...
std::string FileLocator::applyRules(ProtocolRules const& protocolRules,
  ...
  ProtocolRules::const_iterator const rulesIterator =
      protocolRules.find(protocol);
  ...
  for (Rules::const_iterator i = rules.begin(); i != rules.end(); ++i) {
    if (!std::regex_match(destination, destinationMatches,
                          i->destinationMatch)) {
      continue;
    }
