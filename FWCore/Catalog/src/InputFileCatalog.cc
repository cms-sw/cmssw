  ...
  InputFileCatalog::InputFileCatalog(std::vector<std::string> logicalFileNames,
    ...
    if (!overrideFileLocator_ && !override.empty()) {
      ...
      overrideFileLocator_ = std::make_unique<FileLocator>(
          override_struct, filename_storage);  // propagate_const<T> has no reset() function
    } else {
      ...
      for (const auto& catalog : tmp_dataCatalogs) {
        CatalogAttributes catalogAttributes = catalog;
        try {
          if (catalogAttributes.empty()) {
            catalogAttributes = tmp_dataCatalogs[0];
          }
