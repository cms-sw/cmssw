...
class ProducingModuleAdaptorBase {
 public:
  ...
  void registerProductsAndCallbacks(ProducingModuleAdaptorBase const*,
                                    ProductRegistry* reg);
 ...
 protected:
  ...
  void commit(Event& iEvent, L* iList, I* iID) {
    iEvent.commit_(m_streamModules[0]->indiciesForPutProducts(InEvent), iList,
                   iID);
  }
 ...
 private:
  ProducingModuleAdaptorBase(const ProducingModuleAdaptorBase&) =
      delete;  // stop default

  const ProducingModuleAdaptorBase& operator=(
      const ProducingModuleAdaptorBase&) = delete;  // stop default
  ...
  virtual void streamEndRunSummary(T*, edm::Run const&,
