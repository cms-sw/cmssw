...
//
/**\class edm::stream::ProducingModuleAdaptor ProducingModuleAdaptor.h
...
class ProducingModuleAdaptor : public B {
 ...
 private:
  ...
  void doEndJob() override final { MyGlobal::endJob(m_global.get()); }
  void setupRun(M* iProd, RunIndex iIndex) override final {
    MyGlobalRun::set(iProd, m_runs[iIndex].get());
  }
  void streamEndRunSummary(M* iProd, edm::Run const& iRun,
  ...
  void setupLuminosityBlock(M* iProd,
                            LuminosityBlockIndex iIndex) override final {
    MyGlobalLuminosityBlock::set(iProd, m_lumis[iIndex].get());
  }
  void streamEndLuminosityBlockSummary(
      M* iProd, edm::LuminosityBlock const& iLumi,
      edm::EventSetup const& iES) override final {
    auto s = m_lumiSummaries[iLumi.index()].get();
    std::lock_guard<decltype(m_lumiSummaryLock)> guard(m_lumiSummaryLock);
    MyGlobalLuminosityBlockSummary::streamEndLuminosityBlockSummary(
        iProd, iLumi, iES, s);
  }

  void doBeginRun(RunPrincipal const& rp, EventSetup const& c,
                  ModuleCallingContext const* mcc) override final {
    if (T::HasAbility::kRunCache or T::HasAbility::kRunSummaryCache or
        T::HasAbility::kBeginRunProducer) {
  ...
  void doBeginLuminosityBlock(LuminosityBlockPrincipal const& lbp,
                              EventSetup const& c,
                              ModuleCallingContext const* mcc) override final {
    if (T::HasAbility::kLuminosityBlockCache or
      ...
      typename T::RunContext rc(m_runs[ri].get(), m_global.get());
      MyGlobalLuminosityBlock::beginLuminosityBlock(cnstLb, c, &rc,
                                                    m_lumis[li]);
      typename T::LuminosityBlockContext lc(m_lumis[li].get(), m_runs[ri].get(),
                                            m_global.get());
      MyGlobalLuminosityBlockSummary::beginLuminosityBlock(cnstLb, c, &lc,
                                                           m_lumiSummaries[li]);
  ...
  void doEndLuminosityBlock(LuminosityBlockPrincipal const& lbp,
    ...
    if (T::HasAbility::kLuminosityBlockCache or
      ...
      if (T::HasAbility::kEndLuminosityBlockProducer) {
        MyEndLuminosityBlockProduce::produce(lb, c, &lc,
                                             m_lumiSummaries[li].get());
        this->commit(lb);
      }
      MyGlobalLuminosityBlockSummary::globalEndLuminosityBlock(
          lb, c, &lc, m_lumiSummaries[li].get());
      MyGlobalLuminosityBlock::endLuminosityBlock(lb, c, &lc);
    }
  }

  ProducingModuleAdaptor(const ProducingModuleAdaptor&) =
      delete;  // stop default

  const ProducingModuleAdaptor& operator=(const ProducingModuleAdaptor&) =
      delete;  // stop default
  ...
  typename impl::choose_shared_vec<typename T::LuminosityBlockCache const>::type
      m_lumis;
  typename impl::choose_shared_vec<typename T::RunSummaryCache>::type
      m_runSummaries;
  typename impl::choose_mutex<typename T::RunSummaryCache>::type
      m_runSummaryLock;
  typename impl::choose_shared_vec<
      typename T::LuminosityBlockSummaryCache>::type m_lumiSummaries;
  typename impl::choose_mutex<typename T::LuminosityBlockSummaryCache>::type
      m_lumiSummaryLock;
