...
/**\file implementors.h "FWCore/Framework/interface/stream/implementors.h"
 ...
 Description: Base classes used to implement the interfaces for the
 edm::stream::* module  abilities
...
class LuminosityBlockCacheHolder {
 public:
  ...
  LuminosityBlockCacheHolder<C>& operator=(
      LuminosityBlockCacheHolder<C> const&) = delete;
...
class RunSummaryCacheHolder {
 ...
 private:
  virtual void endRunSummary(edm::Run const&, edm::EventSetup const&,
                             C*) const = 0;
};
...
class LuminosityBlockSummaryCacheHolder {
 public:
  ...
  LuminosityBlockSummaryCacheHolder(
      LuminosityBlockSummaryCacheHolder<C> const&) = delete;
  LuminosityBlockSummaryCacheHolder<C>& operator=(
      LuminosityBlockSummaryCacheHolder<C> const&) = delete;
  virtual ~LuminosityBlockSummaryCacheHolder() noexcept(false) {}

 private:
  virtual void endLuminosityBlockSummary(edm::LuminosityBlock const&,
                                         edm::EventSetup const&, C*) const = 0;
};
...
class BeginRunProducer {
 public:
  ...
  /// static void globalBeginRunProduce(edm::Run&, edm::EventSetup const&,
  /// RunContext const* );
};
...
class EndRunProducer {
 ...
 private:
  /// requires the following be defined in the inheriting class
  /// static void globalEndRunProduce(edm::Run&, edm::EventSetup const&,
  /// RunContext const* )
};
...
class BeginLuminosityBlockProducer {
 public:
  ...
  BeginLuminosityBlockProducer& operator=(BeginLuminosityBlockProducer const&) =
      delete;

 private:
  /// requires the following be defined in the inheriting class
  /// static void globalBeginLuminosityBlockProduce(edm::LuminosityBlock&,
  /// edm::EventSetup const&, LuminosityBlockContext const*)
};
...
class EndLuminosityBlockProducer {
 public:
  ...
  EndLuminosityBlockProducer& operator=(EndLuminosityBlockProducer const&) =
      delete;
