...
Description: This is the primary interface for accessing per run EDProducts and
inserting new derived products.
...
class Run : public RunBase {
 ...
 private:
  ...
  SharedResourcesAcquirer* sharedResourcesAcquirer_;  // We do not use
                                                      // propagate_const because
                                                      // the acquirer is itself
                                                      // mutable.
...
void Run::put(std::unique_ptr<PROD> product,
  ...
  if (product.get() == 0) {  // null pointer is illegal
    ...
    principal_get_adapter_detail::throwOnPutOfNullProduct("Run", typeID,
                                                          productInstanceName);
  }
...
bool Run::getByLabel(std::string const& label,
  ...
  if (!provRecorder_.checkIfComplete<PROD>()) {
    principal_get_adapter_detail::throwOnPrematureRead(
        "Run", TypeID(typeid(PROD)), label, productInstanceName);
  }
  result.clear();
...
bool Run::getByLabel(InputTag const& tag, Handle<PROD>& result) const {
  if (!provRecorder_.checkIfComplete<PROD>()) {
    principal_get_adapter_detail::throwOnPrematureRead(
        "Run", TypeID(typeid(PROD)), tag.label(), tag.instance());
  }
  result.clear();
...
void Run::getManyByType(std::vector<Handle<PROD>>& results) const {
  if (!provRecorder_.checkIfComplete<PROD>()) {
    principal_get_adapter_detail::throwOnPrematureRead("Run",
                                                       TypeID(typeid(PROD)));
  }
