...
class Callback : public CallbackBase {
 public:
  ...
  void storeReturnedValues(TReturn iReturn) {
    // std::cout <<" storeReturnedValues "<< iReturn <<" " <<iReturn->value_
    // <<std::endl;
  ...
  void setData(ProductsT& iProducts, const RemainingContainerT*, const DataT*) {
    DataT* temp = reinterpret_cast<DataT*>(
        proxyData_[produce::find_index<TReturn, DataT>::value]);
    if (0 != temp) {
      copyFromTo(iProducts, *temp);
    }
    setData(
        iProducts,
        static_cast<const typename RemainingContainerT::head_type*>(nullptr),
  ...
  void setData(ProductsT& iProducts, const produce::Null*, const DataT*) {
    DataT* temp = reinterpret_cast<DataT*>(
        proxyData_[produce::find_index<TReturn, DataT>::value]);
    // std::cout <<" setData["<< produce::find_index<TReturn,DataT>::value<<"]
    // "<< temp <<std::endl;
    if (0 != temp) {
      copyFromTo(iProducts, *temp);
    }
