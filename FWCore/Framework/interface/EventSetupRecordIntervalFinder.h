...
//
/**\class EventSetupRecordIntervalFinder EventSetupRecordIntervalFinder.h
...
class EventSetupRecordIntervalFinder {
  ...
  */
  const ValidityInterval& findIntervalFor(
      const eventsetup::EventSetupRecordKey&, const IOVSyncValue&);

  void setDescriptionForFinder(
      const eventsetup::ComponentDescription& iDescription) {
    description_ = iDescription;
  }

 protected:
  virtual void setIntervalFor(const eventsetup::EventSetupRecordKey&,
                              const IOVSyncValue&, ValidityInterval&) = 0;
 ...
 private:
  EventSetupRecordIntervalFinder(
      const EventSetupRecordIntervalFinder&);  // stop default

  const EventSetupRecordIntervalFinder& operator=(
      const EventSetupRecordIntervalFinder&);  // stop default

  /** override this method if you need to delay setting what records you will be
   * using until after all modules are loaded*/
