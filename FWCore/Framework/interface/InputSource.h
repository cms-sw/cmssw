...
class InputSource : private ProductRegistryHelper {
 public:
  ...
  void readLuminosityBlock(LuminosityBlockPrincipal& lumiPrincipal,
  ...
  std::shared_ptr<ProductRegistry const> productRegistry() const {
    return get_underlying_safe(productRegistry_);
  }
  ...
  /// Returns nullptr if no resource shared between the Source and a
  /// DelayedReader
  ...
  class EventSourceSentry {
   public:
    ...
    EventSourceSentry(EventSourceSentry const&) =
        delete;  // Disallow copying and moving
    EventSourceSentry& operator=(EventSourceSentry const&) =
        delete;  // Disallow copying and moving
  ...
  class FileOpenSentry {
   public:
    ...
    FileOpenSentry(FileOpenSentry const&) =
        delete;  // Disallow copying and moving
    FileOpenSentry& operator=(FileOpenSentry const&) =
        delete;  // Disallow copying and moving
  ...
  class FileCloseSentry {
   public:
    ...
    FileCloseSentry(FileCloseSentry const&) =
        delete;  // Disallow copying and moving
    FileCloseSentry& operator=(FileCloseSentry const&) =
        delete;  // Disallow copying and moving
 ...
 protected:
  ...
  /// Called by inheriting classes when running multicore when the receiver has
  /// told them to
  /// skip some events.
 ...
 private:
  ...
  virtual std::shared_ptr<LuminosityBlockAuxiliary>
  ...
  virtual void endJob();
  virtual std::pair<SharedResourcesAcquirer*, std::recursive_mutex*>
 ...
 private:
  ...
  edm::propagate_const<std::unique_ptr<ProcessHistoryRegistry>>
      processHistoryRegistry_;
  edm::propagate_const<std::shared_ptr<BranchIDListHelper>> branchIDListHelper_;
  edm::propagate_const<std::shared_ptr<ThinnedAssociationsHelper>>
