...
class Event : public EventBase {
 public:
  ...
  /// Returns a RefProd to a product before that product has been placed into
  /// the Event.
  /// The RefProd (and any Ref's made from it) will no work properly until after
  /// the
  /// Event has been committed (which happens after leaving the
  /// EDProducer::produce method)
  template <typename PROD>
  ...
  template <typename ELEMENT>
  bool getByToken(EDGetTokenT<View<ELEMENT>> token,
                  Handle<View<ELEMENT>>& result) const;
  ...
  virtual edm::ParameterSet const* parameterSet(
      edm::ParameterSetID const& psID) const override;

  size_t size() const;

  virtual edm::TriggerNames const& triggerNames(
      edm::TriggerResults const& triggerResults) const override;
  virtual TriggerResultsByName triggerResultsByName(
      edm::TriggerResults const& triggerResults) const override;
 ...
 private:
  ...
  // override used by EventBase class
  virtual BasicHandle getImpl(std::type_info const& iProductType,
                              ProductID const& pid) const override;
  ...
  void commit_aux(ProductPtrVec& products, bool record_parents,
                  std::vector<BranchID>* previousParentage = 0,
                  ParentageID* previousParentageId = 0);
...
struct RecordInParentless {
  ...
  void do_it(ptrvec_t& /*ignored*/, ptrvec_t& used,
...
struct RecordInParentfull {
  ...
  void do_it(ptrvec_t& used, ptrvec_t& /*ignored*/,
             std::unique_ptr<Wrapper<PROD>> wp,
             BranchDescription const* desc) const {
    used.emplace_back(std::move(wp), desc);
  }
...
bool Event::get(ProductID const& oid, Handle<View<ELEMENT>>& result) const {
  ...
  if (bh.failedToGet()) {
    Handle<View<ELEMENT>> temp(
        makeHandleExceptionFactory([oid]() -> std::shared_ptr<cms::Exception> {
          std::shared_ptr<cms::Exception> whyFailed =
              std::make_shared<edm::Exception>(edm::errors::ProductNotFound);
...
OrphanHandle<PROD> Event::put(std::unique_ptr<PROD> product,
  ...
  if (product.get() == 0) {  // null pointer is illegal
    ...
    principal_get_adapter_detail::throwOnPutOfNullProduct("Event", typeID,
                                                          productInstanceName);
  }
  ...
  std::conditional_t<detail::has_donotrecordparents<PROD>::value,
                     RecordInParentless<PROD>, RecordInParentfull<PROD>>
      parentage_recorder;
  parentage_recorder.do_it(putProducts(), putProductsWithoutParents(),
                           std::move(wp), &desc);
...
RefProd<PROD> Event::getRefBeforePut(std::string const& productInstanceName) {
  ...
  // should keep track of what Ref's have been requested and make sure they are
  // 'put'
...
void Event::getManyByType(std::vector<Handle<PROD>>& results) const {
  ...
  for (typename std::vector<Handle<PROD>>::const_iterator it = results.begin(),
                                                          itEnd = results.end();
       it != itEnd; ++it) {
...
bool Event::getByLabel(InputTag const& tag,
  ...
  BasicHandle bh = provRecorder_.getMatchingSequenceByLabel_(
      TypeID(typeid(ELEMENT)), tag, moduleCallingContext_);
  if (bh.failedToGet()) {
...
bool Event::getByLabel(std::string const& moduleLabel,
  ...
  result.clear();
  BasicHandle bh = provRecorder_.getMatchingSequenceByLabel_(
      TypeID(typeid(ELEMENT)), moduleLabel, productInstanceName, emptyString_,
      moduleCallingContext_);
  if (bh.failedToGet()) {
    Handle<View<ELEMENT>> h(std::move(bh.whyFailedFactory()));
    h.swap(result);
...
bool Event::getByToken(EDGetToken token, Handle<View<ELEMENT>>& result) const {
  result.clear();
  BasicHandle bh = provRecorder_.getByToken_(
      TypeID(typeid(ELEMENT)), ELEMENT_TYPE, token, moduleCallingContext_);
  if (bh.failedToGet()) {
    Handle<View<ELEMENT>> h(std::move(bh.whyFailedFactory()));
    h.swap(result);
    return false;
