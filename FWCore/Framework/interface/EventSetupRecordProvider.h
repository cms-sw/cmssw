...
//
/**\class EventSetupRecordProvider EventSetupRecordProvider.h
 FWCore/Framework/interface/EventSetupRecordProvider.h

 Description: Coordinates all EventSetupDataProviders with the same 'interval of
 validity'
...
class EventSetupRecordProvider {
 public:
  ...
  void resetProxyProvider(ParameterSetIDHolder const&,
                          std::shared_ptr<DataProxyProvider> const&);
  ...
  /// sets interval to this time and returns true if have a valid interval for
  /// time
  bool setValidityIntervalFor(IOVSyncValue const&);

  /// If the provided Record depends on other Records, here are the dependent
  /// Providers
  void setDependentProviders(
      std::vector<std::shared_ptr<EventSetupRecordProvider>> const&);

  /**In the case of a conflict, sets what Provider to call.  This must be called
     after
     all providers have been added.  An empty map is acceptable. */
  void usePreferred(DataToPreferredProviderMap const&);

  /// This will clear the cache's of all the Proxies so that next time they are
  /// called they will run
  void resetProxies();

  std::shared_ptr<EventSetupRecordIntervalFinder const> finder() const {
    return get_underlying_safe(finder_);
  }
  ...
  void getReferencedESProducers(
      std::map<EventSetupRecordKey, std::vector<ComponentDescription const*>>&
          referencedESProducers);

  void fillReferencedDataKeys(
 ...
 protected:
  void addProxiesToRecordHelper(
      edm::propagate_const<std::shared_ptr<DataProxyProvider>>& dpp,
  ...
  std::shared_ptr<EventSetupRecordIntervalFinder> swapFinder(
      std::shared_ptr<EventSetupRecordIntervalFinder> iNew) {
 ...
 private:
  ...
  EventSetupRecordProvider const& operator=(
      EventSetupRecordProvider const&);  // stop default
  ...
  std::vector<edm::propagate_const<std::shared_ptr<DataProxyProvider>>>
      providers_;
