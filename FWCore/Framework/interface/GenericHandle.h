...
// Class  :     GenericHandle
//
/**\class GenericHandle GenericHandle.h
 ...
 Usage:
    The GenericHandle allows one to get data back from the edm::Event as an
 ObjectWithDict instead
...
class Handle<GenericObject> {
 public:
  ...
  Handle(std::string const& iName)
    ...
    if (!bool(type_)) {
      Exception::throwThis(errors::NotFound,
                           ...
                           iName.c_str(),
                           "'.\n Please check spelling or that a module uses "
                           "this type in the job.");
    }
  }
  ...
  Handle(TypeWithDict const& iType) : type_(iType), prod_(), prov_(nullptr) {
    if (!bool(iType)) {
      Exception::throwThis(errors::NotFound,
                           "Handle<GenericObject> given an invalid type");
    }
  }
  ...
  bool isValid() const { return prod_ && 0 != prov_; }
  ...
  void setWhyFailedFactory(
      std::shared_ptr<HandleExceptionFactory> const& iWhyFailed) {
...
/// Specialize the Event's getByLabel method to work with a
/// Handle<GenericObject>
template <>
bool edm::Event::getByLabel<GenericObject>(
    std::string const& label, std::string const& productInstanceName,
