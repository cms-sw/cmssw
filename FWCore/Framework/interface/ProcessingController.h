...
/**\class ProcessingController ProcessingController.h
 ...
 Description: Interface used by an EDLooper to specify what event we should
 process next
...
class ProcessingController {
 public:
  ...
  /** Tells the framework we should backup and run the previous event seen in
   the sequence.
  ...
  /** Tells the framework that the next event to processes should be iID.
   If event iID can not be found in the source, the job will drop out of the
   event loop.
   */
 ...
 private:
  ...
  const ProcessingController& operator=(
      const ProcessingController&);  // stop default
