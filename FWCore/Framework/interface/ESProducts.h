...
/**\class ESProducts ESProducts.h FWCore/Framework/interface/ESProducts.h
 ...
 Usage:
    This is used as a return type from the produce method of an ESProducer.
 Users are not anticipated to ever need
 to directly use this class.  Instead, to create an instance of the class at
 method return time, they should call
 the helper function es::products or 'output' the values to es::produced via the
 '<<' operator.
...
struct OneHolder<std::unique_ptr<T>> {
  ...
  OneHolder() {}
  OneHolder(OneHolder<Type> const& iOther)
      : value_(const_cast<OneHolder<Type>&>(iOther).value_) {}
  OneHolder(Type iPtr) : value_(iPtr) {}

  OneHolder<Type> const& operator=(OneHolder<Type> iRHS) {
    value_ = iRHS.value_;
    return *this;
  }
  template <typename S>
...
template <typename T>
OneHolder<T> operator<<(const Produce&, T iValue) {
...
MultiHolder<MultiHolder<T, U>, V> operator<<(const MultiHolder<T, U>& iHolder,
                                             const V& iValue) {
...
template <typename T1, typename T2 = eventsetup::produce::Null,
          typename T3 = eventsetup::produce::Null>
