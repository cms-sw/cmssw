...
/**\class IOVSyncValue IOVSyncValue.h FWCore/Framework/interface/IOVSyncValue.h
 ...
 Description: Provides the information needed to synchronize the EventSetup IOV
 with an Event
...
class IOVSyncValue {
 public:
  ...
  /** returns true if comparison operations are possible. Comparisons only fail
     if
    ...
    if (haveID_ && iRHS.haveID_) {
      if (luminosityBlockNumber() == 0 || iRHS.luminosityBlockNumber() == 0 ||
        ...
        if (iRHS.eventID_.run() == eventID_.run()) {
          ...
          returnValue =
              op(luminosityBlockNumber(), iRHS.luminosityBlockNumber());
        } else {
          Op<RunNumber_t> op;
