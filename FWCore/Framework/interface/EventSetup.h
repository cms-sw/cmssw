...
class EventSetup {
 ...
 public:
  ...
  template <typename T>
  const T& get() const {
    // NOTE: this will catch the case where T does not inherit from
    // EventSetupRecord
    //  HOWEVER the error message under gcc 3.x is awful
    BOOST_STATIC_ASSERT(
        (boost::is_base_and_derived<edm::eventsetup::EventSetupRecord,
                                    T>::value));
    const T* value = nullptr;
    eventSetupGetImplementation(*this, value);
    // NOTE: by construction, eventSetupGetImplementation should thrown an
    // exception rather than return a null value
    assert(0 != value);
    return *value;
  }
  /** returns the Record of type T.  If no such record available
  ...
  /** can directly access data if data_default_record_trait<> is defined for
   * this data type **/
  template <typename T>
  ...
  const eventsetup::EventSetupRecord* find(
      const eventsetup::EventSetupRecordKey&) const;

  /// clears the oToFill vector and then fills it with the keys for all
  /// available records
  void fillAvailableRecordKeys(
 ...
 protected:
  ...
  void setKnownRecordsSupplier(
      eventsetup::EventSetupKnownRecordsSupplier const* iSupplier) {
 ...
 private:
  ...
  std::map<eventsetup::EventSetupRecordKey, eventsetup::EventSetupRecord const*>
      recordMap_;
