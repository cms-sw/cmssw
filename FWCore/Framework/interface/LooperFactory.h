...
// Class  :     LooperFactory
//
/**\class LooperFactory LooperFactory.h
...
namespace looper {
template <class T>
void addProviderTo(EventSetupProvider& iProvider, std::shared_ptr<T> iComponent,
...
void addProviderTo(EventSetupProvider& /* iProvider */,
                   std::shared_ptr<T> /*iComponent*/, const void*) {
  // do nothing
}

template <class T>
void addFinderTo(EventSetupProvider& iProvider, std::shared_ptr<T> iComponent,
...
struct LooperMakerTraits {
  ...
  static void addTo(EventSetupProvider& iProvider,
                    std::shared_ptr<T> iComponent, ParameterSet const&, bool) {
    // a looper does not always have to be a provider or a finder
    looper::addProviderTo(iProvider, iComponent,
  ...
  static std::shared_ptr<base_type> getComponentAndRegisterProcess(
...
struct LooperMaker
    : public ComponentMaker<edm::eventsetup::LooperMakerTraits, TType> {};
typedef ComponentFactory<LooperMakerTraits> LooperFactory;

typedef edmplugin::PluginFactory<
    edm::eventsetup::ComponentMakerBase<LooperMakerTraits>*()>
    LooperPluginFactory;
}
}

#define DEFINE_FWK_LOOPER(type)                           \
  DEFINE_EDM_PLUGIN(edm::eventsetup::LooperPluginFactory, \
                    edm::eventsetup::LooperMaker<type>, #type)
