...
class EventProcessor {
 public:
  ...
  // The input string 'config' contains the entire contents of a  configuration
  // file.
  // Also allows the attachement of pre-existing services specified  by 'token',
  // and
  // the specification of services by name only (defaultServices and
  // forcedServices).
  // 'defaultServices' are overridden by 'config'.
  // 'forcedServices' the 'config'.
  explicit EventProcessor(
      std::string const& config, ServiceToken const& token = ServiceToken(),
  ...
  // Same as previous constructor, but without a 'token'.  Token will be
  // defaulted.
  ...
  EventProcessor& operator=(EventProcessor const&) =
      delete;  // Disallow copying and moving
 ...
 private:
  ...
  void init(std::shared_ptr<ProcessDesc>& processDesc,
            ServiceToken const& token, serviceregistry::ServiceLegacy);

  bool readNextEventForStream(unsigned int iStreamIndex,
                              std::atomic<bool>* finishedProcessingEvents);

  void handleNextEventForStreamAsync(
      WaitingTask* iTask, unsigned int iStreamIndex,
      std::atomic<bool>* finishedProcessingEvents);
...
inline EventProcessor::StatusCode EventProcessor::run() {
