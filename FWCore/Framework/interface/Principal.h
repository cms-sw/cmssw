...
struct FilledProductPtr {
  bool operator()(
      propagate_const<std::shared_ptr<ProductResolverBase>> const& iObj) {
    return bool(iObj);
  }
};
...
class Principal : public EDProductGetter {
 public:
  typedef std::vector<propagate_const<std::shared_ptr<ProductResolverBase>>>
      ProductResolverCollection;
  typedef boost::filter_iterator<FilledProductPtr,
  ...
  Principal(std::shared_ptr<ProductRegistry const> reg,
            ...
            HistoryAppender* historyAppender, bool isForPrimaryProcess = true);
  ...
  void fillPrincipal(ProcessHistoryID const& hist,
                     ProcessHistoryRegistry const& phr, DelayedReader* reader);
  ...
  BasicHandle getByLabel(KindOfType kindOfType, TypeID const& typeID,
  ...
  BasicHandle getByToken(KindOfType kindOfType, TypeID const& typeID,
                         ProductResolverIndex index, bool skipCurrentProcess,
                         bool& ambiguous, SharedResourcesAcquirer* sra,
                         ModuleCallingContext const* mcc) const;

  void prefetchAsync(WaitingTask* waitTask, ProductResolverIndex index,
                     bool skipCurrentProcess,
                     ModuleCallingContext const* mcc) const;

  void getManyByType(TypeID const& typeID, BasicHandleVec& results,
                     EDConsumerBase const* consumes,
                     SharedResourcesAcquirer* sra,
                     ModuleCallingContext const* mcc) const;
  ...
  ProductResolverBase* getModifiableProductResolver(BranchID const& oid) {
    return const_cast<ProductResolverBase*>(
        const_cast<const Principal*>(this)->getProductResolver(oid));
  }
  ...
  const_iterator begin() const {
    return boost::make_filter_iterator<FilledProductPtr>(
        productResolvers_.begin(), productResolvers_.end());
  }
  const_iterator end() const {
    return boost::make_filter_iterator<FilledProductPtr>(
        productResolvers_.end(), productResolvers_.end());
  }
  ...
  ProductData const* findProductByTag(TypeID const& typeID, InputTag const& tag,
                                      ModuleCallingContext const* mcc) const;
  ...
  ConstProductResolverPtr getProductResolverByIndex(
      ProductResolverIndex const& oid) const;
 ...
 protected:
  ...
  ProductResolverBase const* getExistingProduct(
      ProductResolverBase const& phb) const;

  void putOrMerge(BranchDescription const& bd,
 ...
 private:
  ...
  void findProducts(std::vector<ProductResolverBase const*> const& holders,
                    TypeID const& typeID, BasicHandleVec& results,
                    SharedResourcesAcquirer* sra,
                    ModuleCallingContext const* mcc) const;
  ...
  ProductData const* findProductByLabel(
  ...
  void putOrMerge(std::unique_ptr<WrapperBase> prod,
  ...
  ProductResolverCollection
      productResolvers_;  // products and provenances are persistent
...
inline std::shared_ptr<Wrapper<PROD> const> getProductByTag(
    Principal const& ep, InputTag const& tag, ModuleCallingContext const* mcc) {
  ...
  if (!(result->wrapper()->dynamicTypeInfo() == typeid(PROD))) {
    handleimpl::throwConvertTypeError(typeid(PROD),
                                      result->wrapper()->dynamicTypeInfo());
  }
  return std::static_pointer_cast<Wrapper<PROD> const>(
      result->sharedConstWrapper());
}
}
#endif
