...
//
/**\class PrincipalGetAdapter PrincipalGetAdapter.h
...
edm::RefProd<AppleCollection> refApples =
event.getRefBeforePut<AppleCollection>();
...
class PrincipalGetAdapter {
 public:
  ...
  template <typename PROD>
  void getManyByType(std::vector<Handle<PROD> >& results,
  ...
  BranchDescription const& getBranchDescription(
      TypeID const& type, std::string const& productInstanceName) const;
  ...
  // The following 'get' functions serve to isolate the PrincipalGetAdapter
  // class
  // from the Principal class.

  BasicHandle getByLabel_(TypeID const& tid, InputTag const& tag,
                          ModuleCallingContext const* mcc) const;

  BasicHandle getByLabel_(TypeID const& tid, std::string const& label,
                          std::string const& instance,
                          std::string const& process,
                          ModuleCallingContext const* mcc) const;

  BasicHandle getByToken_(TypeID const& id, KindOfType kindOfType,
                          EDGetToken token,
                          ModuleCallingContext const* mcc) const;

  BasicHandle getMatchingSequenceByLabel_(
      TypeID const& typeID, InputTag const& tag,
      ModuleCallingContext const* mcc) const;

  BasicHandle getMatchingSequenceByLabel_(
      TypeID const& typeID, std::string const& label,
      std::string const& instance, std::string const& process,
      ModuleCallingContext const* mcc) const;

  void getManyByType_(TypeID const& tid, BasicHandleVec& results,
                      ModuleCallingContext const* mcc) const;
 ...
 private:
  ...
  BasicHandle makeFailToGetException(KindOfType, TypeID const&,
                                     EDGetToken) const;

  void throwAmbiguousException(TypeID const& productType,
 ...
 private:
  ...
  // Each PrincipalGetAdapter must have a description of the module executing
  // the
  // "transaction" which the PrincipalGetAdapter represents.
  ModuleDescription const& md_;

  EDConsumerBase const* consumer_;
  SharedResourcesAcquirer* resourcesAcquirer_;  // We do not use propagate_const
                                                // because the acquirer is
                                                // itself mutable.
};
...
struct has_donotrecordparents {
  static bool const value = boost::is_base_of<DoNotRecordParents, T>::value;
};
}
...
// Implementation of  PrincipalGetAdapter  member templates. See
// PrincipalGetAdapter.cc for the
...
template <typename PROD>
inline void PrincipalGetAdapter::getManyByType(
