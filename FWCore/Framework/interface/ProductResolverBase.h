...
ProductResolver: Class to handle access to a WrapperBase and its related
information.
...
class ProductResolverBase {
 public:
  class Resolution {
   public:
    ...
    static std::uintptr_t constexpr kAmbiguityMask =
        std::numeric_limits<std::uintptr_t>::max() ^ kAmbiguityValue;
    explicit Resolution(ProductData const* iData) : m_data(iData) {}

    bool isAmbiguous() const {
      return reinterpret_cast<std::uintptr_t>(m_data) == kAmbiguityValue;
    }

    ProductData const* data() const {
      return reinterpret_cast<ProductData const*>(
          kAmbiguityMask & reinterpret_cast<std::uintptr_t>(m_data));
    }

    static Resolution makeAmbiguous() {
      return Resolution(reinterpret_cast<ProductData const*>(kAmbiguityValue));
    }
  ...
  ProductResolverBase(ProductResolverBase const&) =
      delete;  // Disallow copying and moving
  ProductResolverBase& operator=(ProductResolverBase const&) =
      delete;  // Disallow copying and moving

  Resolution resolveProduct(Principal const& principal, bool skipCurrentProcess,
                            SharedResourcesAcquirer* sra,
                            ModuleCallingContext const* mcc) const {
    return resolveProduct_(principal, skipCurrentProcess, sra, mcc);
  }

  /** oDataFetchedIsValid is allowed to be nullptr in which case no value will
   * be assigned
   */
  void prefetchAsync(WaitingTask* waitTask, Principal const& principal,
                     bool skipCurrentProcess, SharedResourcesAcquirer* sra,
                     ModuleCallingContext const* mcc) const {
  ...
  // Retrieves pointer to a class containing both the event independent and the
  // per even provenance.
  Provenance const* provenance() const;

  // Retrieves pointer to a class containing the event independent provenance.
  StableProvenance const* stableProvenance() const {
    return &provenance()->stable();
  }

  // Initializes the event independent portion of the provenance, plus the
  // process history ID, the product ID, and the provRetriever.
  void setProvenance(ProductProvenanceRetriever const* provRetriever,
                     ProcessHistory const& ph, ProductID const& pid) {
 ...
 private:
  ...
  virtual void prefetchAsync_(WaitingTask* waitTask, Principal const& principal,
                              bool skipCurrentProcess,
                              SharedResourcesAcquirer* sra,
                              ModuleCallingContext const* mcc) const = 0;
  ...
  virtual void resetBranchDescription_(
      std::shared_ptr<BranchDescription const> bd) = 0;
