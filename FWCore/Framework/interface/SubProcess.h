...
class SubProcess : public EDConsumerBase {
 public:
  ...
  void doBeginLuminosityBlock(LuminosityBlockPrincipal const& principal,
                              IOVSyncValue const& ts);
  void doBeginLuminosityBlockAsync(WaitingTaskHolder iHolder,
                                   LuminosityBlockPrincipal const& principal,
                                   IOVSyncValue const& ts);

  void doEndLuminosityBlock(LuminosityBlockPrincipal const& principal,
                            IOVSyncValue const& ts,
                            bool cleaningUpAfterException);
  ...
  void doStreamBeginRun(unsigned int iID, RunPrincipal const& principal,
                        IOVSyncValue const& ts);
  void doStreamBeginRunAsync(WaitingTaskHolder iHolder, unsigned int iID,
                             RunPrincipal const& principal,
                             IOVSyncValue const& ts);

  void doStreamEndRun(unsigned int iID, RunPrincipal const& principal,
                      IOVSyncValue const& ts, bool cleaningUpAfterException);
  ...
  void doStreamBeginLuminosityBlock(unsigned int iID,
                                    LuminosityBlockPrincipal const& principal,
                                    IOVSyncValue const& ts);
  void doStreamBeginLuminosityBlockAsync(
      WaitingTaskHolder iHolder, unsigned int iID,
      LuminosityBlockPrincipal const& principal, IOVSyncValue const& ts);

  void doStreamEndLuminosityBlock(unsigned int iID,
                                  LuminosityBlockPrincipal const& principal,
                                  IOVSyncValue const& ts,
                                  bool cleaningUpAfterException);
  void doStreamEndLuminosityBlockAsync(
      WaitingTaskHolder iHolder, unsigned int iID,
      LuminosityBlockPrincipal const& principal, IOVSyncValue const& ts,
      bool cleaningUpAfterException);

  // Write the luminosity block
  void writeLumi(ProcessHistoryID const& parentPhID, int runNumber,
                 int lumiNumber);

  void deleteLumiFromCache(ProcessHistoryID const& parentPhID, int runNumber,
                           int lumiNumber);
  ...
  void openOutputFiles(FileBlock& fb) {
    ...
    for_all(subProcesses_,
            [&fb](auto& subProcess) { subProcess.openOutputFiles(fb); });
  }
  ...
  void respondToCloseInputFile(FileBlock const& fb) {
    ...
    for_all(subProcesses_, [&fb](auto& subProcess) {
  ...
  /// Return a vector allowing const access to all the ModuleDescriptions for
  /// this SubProcess
  ...
  /// Return the number of events which have been passed by one or more trigger
  /// paths.
  ...
  void enableEndPaths(bool active) {
    ...
    for_all(subProcesses_,
            [active](auto& subProcess) { subProcess.enableEndPaths(active); });
  }
 ...
 private:
  ...
  void endRun(RunPrincipal const& r, IOVSyncValue const& ts,
              bool cleaningUpAfterException);
