...
//
/**\class ComponentFactory ComponentFactory.h
 FWCore/Framework/interface/ComponentFactory.h

 Description: Factory for building the Factories for the various 'plug-in'
 components needed for the EventSetup
...
class ComponentFactory {
 public:
  ...
  std::shared_ptr<base_type> addTo(EventSetupsController& esController,
    ...
    std::string modtype =
        iConfiguration.template getParameter<std::string>("@module_type");
    // cerr << "Factory: module_type = " << modtype << endl;
    typename MakerMap::iterator it = makers_.find(modtype);

    if (it == makers_.end()) {
      std::shared_ptr<Maker> wm(
          edmplugin::PluginFactory<ComponentMakerBase<T>*()>::get()->create(
              modtype));

      if (wm.get() == 0) {
        Exception::throwThis(
            errors::Configuration, "UnknownModule", T::name().c_str(),
            " of type ", modtype.c_str(),
      ...
      std::pair<typename MakerMap::iterator, bool> ret = makers_.insert(
          std::pair<std::string, std::shared_ptr<Maker> >(modtype, wm));

      if (ret.second == false) {
        Exception::throwThis(errors::Configuration,
                             "Maker Factory map insert failed");
      }
    ...
    try {
      ...
      std::string edmtype =
          iConfiguration.template getParameter<std::string>("@module_edm_type");
      std::string label =
          iConfiguration.template getParameter<std::string>("@module_label");
      std::ostringstream ost;
      ost << "Constructing " << edmtype << ": class=" << modtype << " label='"
          << label << "'";
...
#define COMPONENTFACTORY_GET(_type_)                                     \
  EDM_REGISTER_PLUGINFACTORY(                                            \
      edmplugin::PluginFactory<                                          \
          edm::eventsetup::ComponentMakerBase<_type_>*()>,               \
      _type_::name());                                                   \
  static edm::eventsetup::ComponentFactory<_type_> const s_dummyfactory; \
  namespace edm {                                                        \
  namespace eventsetup {                                                 \
  template <>                                                            \
  edm::eventsetup::ComponentFactory<_type_> const*                       \
  edm::eventsetup::ComponentFactory<_type_>::get() {                     \
