...
class EventPrincipal : public Principal {
 public:
  ...
  EventPrincipal(std::shared_ptr<ProductRegistry const> reg,
                 ...
                 HistoryAppender* historyAppender, unsigned int streamIndex = 0,
  ...
  void setLuminosityBlockPrincipal(
  ...
  Provenance getProvenance(ProductID const& pid,
  ...
  virtual WrapperBase const* getThinnedProduct(
      ProductID const& pid, unsigned int& key) const override;
 ...
 private:
  ...
  edm::propagate_const<std::shared_ptr<LuminosityBlockPrincipal>>
      luminosityBlockPrincipal_;

  // Pointer to the 'retriever' that will get provenance information from the
  // persistent store.
  edm::propagate_const<std::shared_ptr<ProductProvenanceRetriever>>
      provRetrieverPtr_;
