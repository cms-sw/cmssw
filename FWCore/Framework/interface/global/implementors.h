...
class StreamCacheHolder : public virtual T {
 ...
 private:
  ...
  virtual void doStreamBeginLuminosityBlock_(
      StreamID id, LuminosityBlock const& lbp,
      EventSetup const& c) override final {
...
class RunCacheHolder : public virtual T {
 ...
 private:
  ...
  // When threaded we will have a container for N items whre N is # of
  // simultaneous runs
...
class LuminosityBlockCacheHolder : public virtual T {
 public:
  ...
  LuminosityBlockCacheHolder<T, C>& operator=(
      LuminosityBlockCacheHolder<T, C> const&) = delete;
 ...
 private:
  ...
  virtual std::shared_ptr<C> globalBeginLuminosityBlock(
...
class RunSummaryCacheHolder : public virtual T {
 public:
  ...
  RunSummaryCacheHolder<T, C>& operator=(RunSummaryCacheHolder<T, C> const&) =
      delete;
  ~RunSummaryCacheHolder() noexcept(false){};

 private:
  friend class EndRunSummaryProducer<T, C>;
  void doBeginRunSummary_(edm::Run const& rp,
                          EventSetup const& c) override final {
  ...
  virtual std::shared_ptr<C> globalBeginRunSummary(
      edm::Run const&, edm::EventSetup const&) const = 0;
  virtual void streamEndRunSummary(StreamID, edm::Run const&,
                                   edm::EventSetup const&, C*) const = 0;

  virtual void globalEndRunSummary(edm::Run const&, edm::EventSetup const&,
                                   C*) const = 0;

  // When threaded we will have a container for N items where N is # of
  // simultaneous runs
  std::shared_ptr<C> cache_;
  std::mutex mutex_;
};
...
class LuminosityBlockSummaryCacheHolder : public virtual T {
 ...
 private:
  ...
  virtual void doStreamEndLuminosityBlockSummary_(
      StreamID id, LuminosityBlock const& lb,
      EventSetup const& c) override final {
    std::lock_guard<std::mutex> guard(mutex_);
...
class BeginRunProducer : public virtual T {
 ...
 private:
  ...
  virtual void globalBeginRunProduce(edm::Run&,
                                     edm::EventSetup const&) const = 0;
};
...
class EndRunSummaryProducer : public RunSummaryCacheHolder<T, C> {
 ...
 private:
  ...
  virtual void globalEndRunProduce(edm::Run&, edm::EventSetup const&,
                                   C const*) const = 0;
};
...
class BeginLuminosityBlockProducer : public virtual T {
 ...
 private:
  void doBeginLuminosityBlockProduce_(LuminosityBlock& lb,
                                      EventSetup const& c) override final;
  virtual void globalBeginLuminosityBlockProduce(
      edm::LuminosityBlock&, edm::EventSetup const&) const = 0;
};
...
class EndLuminosityBlockProducer : public virtual T {
 ...
 private:
  void doEndLuminosityBlockProduce_(LuminosityBlock& lb,
                                    EventSetup const& c) override final;
  virtual void globalEndLuminosityBlockProduce(
      edm::LuminosityBlock&, edm::EventSetup const&) const = 0;
};
...
class EndLuminosityBlockSummaryProducer
 ...
 public:
  ...
  EndLuminosityBlockSummaryProducer(EndLuminosityBlockSummaryProducer const&) =
      delete;
  EndLuminosityBlockSummaryProducer& operator=(
      EndLuminosityBlockSummaryProducer const&) = delete;
 ...
 private:
  void doEndLuminosityBlockProduce_(LuminosityBlock& lb,
    ...
    globalEndLuminosityBlockProduce(
        lb, c, LuminosityBlockSummaryCacheHolder<T, S>::cache_.get());
  }
