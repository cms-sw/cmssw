...
class EventSelector {
 public:
  ...
  static evtSel::OverlapResult testSelectionOverlap(
      Strings const& pathspec1, Strings const& pathspec2,
      Strings const& fullPathList);
 ...
 private:
  ...
  bool acceptOneBit(Bits const& b, HLTGlobalStatus const& tr,
  ...
  static std::vector<Strings::const_iterator> matching_triggers(
      Strings const& trigs, std::string const& s);

  static bool identical(std::vector<bool> const& a, std::vector<bool> const& b);
  static bool identical(EventSelector const& a, EventSelector const& b,
                        unsigned int N);
  static std::vector<bool> expandDecisionList(Bits const& b, bool PassOrFail,
                                              unsigned int n);
