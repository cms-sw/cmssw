...
/**\class eventSetupGetImplementation eventSetupGetImplementation.h
 ...
 Description: decleration of the template function that implements the
 EventSetup::get method
...
template <class T>
inline void eventSetupGetImplementation(EventSetup const& iEventSetup,
                                        T const*& iValue) {
  T const* temp =
      heterocontainer::find<EventSetupRecordKey, T const>(iEventSetup);
  if (0 == temp) {
    throw NoRecordException<T>(
        iovSyncValueFrom(iEventSetup),
...
template <class T>
inline void eventSetupTryToGetImplementation(EventSetup const& iEventSetup,
                                             T const*& iValue) {
