...
/*
 ...
 Unit tests for parts of this class are in
 FWCore/Integration/run_SelectEvents.sh
...
class TriggerNamesService {
 public:
  ...
  std::string const& getEndPathModule(std::string const& name,
                                      size_type const j) const {
    return (end_modulenames_.at(find(end_pos_, name))).at(j);
  }
  std::string const& getEndPathModule(size_type const i,
                                      size_type const j) const {
