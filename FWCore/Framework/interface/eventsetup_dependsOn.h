...
/**\class eventsetup_dependsOn eventsetup_dependsOn.h FWCore/Framework/interface/eventsetup_dependsOn.h
 ...
 Usage:
    The function dependsOn takes a variable number of pointers to member methods as arguments.  These methods are then
called for the ESProducer whenever the dependent Record (determined by looking at the methods lone argument) has changed
since the last time the ESProducer's produce method was called. \code MyProducer : public ESProducer { ... };
...
/*
 ...
 The dependsOn function does not have enough information to convert the list of member function pointers directly into
 the appropriate Decorator class since it is missing the Record type of the Decorator (all it knows are the Record types
 the Decorator depends on).  Therefore we must defer the creation of the Decorator until that Record type is known
 (which happens within the body of the ESProducer::setWhatProduced method).  To allow the deferred construction, the
 dependsOn method returns a compile time linked list created via the TwoHolder class (if there is only one node you get
 a OneHolder).  The dependsOn method always makes sure that the second type of the TwoHolder is the member function
 pointer which is needed for the later construction stage.

 Within the body of ESProducer::setWhatProduced, the proper Decorator is created by calling 'createDecoratorFrom' which
 is given a pointer to the Producer, a dummy pointer to the proper Record and the linked list of member function
 pointers.  The 'createDecoratorFrom' uses the makeCaller and createDependsOnCaller functions to recursively create the
 proper DependsOnCaller functor which is then used by the ESPreFunctorDecorator to do the work.  We use  HolderToCaller
 template class merely to define the return type of the 'createDecoratorFrom' and 'makeCaller' functions.
...
namespace edm {
  namespace eventsetup {
    ...
    // DecoratorFromArg is used to declare the return type of 'createDecoratorFrom' based on the arguments to the
    // function.
