...
called
  for the ESProducer whenever the dependent Record (determined by looking at the methods lone argument) has changed
since the
...
/*
 ...
 'createDecoratorFrom' uses the makeCaller and createDependsOnCaller functions to recursively create the proper
 DependsOnCaller
 functor which is then used by the ESPreFunctorDecorator to do the work.  We use  HolderToCaller template class merely
 to define
