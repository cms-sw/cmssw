...
/**\class ESOutlet ESOutlet.h FWCore/Framework/interface/ESOutlet.h
 ...
 Description: An outlet which gets its data from the EventSetup and passes it to
an edm::ExtensionCord

 Usage:
    If you have a framework module (e.g. an EDProducer) which internally holds
objects and these objects need access
to data in the EventSetup then you can use the edm::ESOutlet and an
edm::ExtensionCord to pass the EventSetup data
...
class ESOutlet : private OutletBase<T> {
 ...
 public:
  ...
  ESOutlet(const edm::EventSetup& iES, const std::string& iLabel,
