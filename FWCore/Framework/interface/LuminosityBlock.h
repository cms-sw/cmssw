...
//
/**\class LuminosityBlock LuminosityBlock.h
...
class LuminosityBlock : public LuminosityBlockBase {
 public:
  LuminosityBlock(LuminosityBlockPrincipal const& lbp,
                  ModuleDescription const& md, ModuleCallingContext const*);
  ~LuminosityBlock();
  ...
  /**\return Reusable index which can be used to separate data for different
   * simultaneous LuminosityBlocks.
   */
 ...
 private:
  ...
  typedef std::vector<
      std::pair<edm::propagate_const<std::unique_ptr<WrapperBase>>,
...
void LuminosityBlock::put(std::unique_ptr<PROD> product,
  ...
  if (product.get() == nullptr) {  // null pointer is illegal
    ...
    principal_get_adapter_detail::throwOnPutOfNullProduct(
        "LuminosityBlock", typeID, productInstanceName);
  }

  // The following will call post_insert if T has such a function,
  // and do nothing if T has no such function.
  std::conditional_t<detail::has_postinsert<PROD>::value, DoPostInsert<PROD>,
                     DoNotPostInsert<PROD>>
      maybe_inserter;
...
bool LuminosityBlock::getByLabel(std::string const& label,
  ...
  if (!provRecorder_.checkIfComplete<PROD>()) {
    principal_get_adapter_detail::throwOnPrematureRead(
        "Lumi", TypeID(typeid(PROD)), label, productInstanceName);
  }
  result.clear();
  BasicHandle bh = provRecorder_.getByLabel_(TypeID(typeid(PROD)), label,
                                             productInstanceName, emptyString_,
...
bool LuminosityBlock::getByLabel(InputTag const& tag,
  ...
  if (!provRecorder_.checkIfComplete<PROD>()) {
    principal_get_adapter_detail::throwOnPrematureRead(
        "Lumi", TypeID(typeid(PROD)), tag.label(), tag.instance());
  }
  result.clear();
...
bool LuminosityBlock::getByToken(EDGetToken token, Handle<PROD>& result) const {
  if (!provRecorder_.checkIfComplete<PROD>()) {
    principal_get_adapter_detail::throwOnPrematureRead(
        "Lumi", TypeID(typeid(PROD)), token);
  }
  result.clear();
  BasicHandle bh = provRecorder_.getByToken_(TypeID(typeid(PROD)), PRODUCT_TYPE,
...
void LuminosityBlock::getManyByType(std::vector<Handle<PROD>>& results) const {
  if (!provRecorder_.checkIfComplete<PROD>()) {
    principal_get_adapter_detail::throwOnPrematureRead("Lumi",
                                                       TypeID(typeid(PROD)));
  }
