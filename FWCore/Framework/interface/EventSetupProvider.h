...
//
/**\class EventSetupProvider EventSetupProvider.h
...
class EventSetupProvider {
 public:
  ...
  /// Used when testing that all code properly updates on IOV changes of all
  /// Records
 ...
 protected:
  ...
  void insert(std::unique_ptr<T> iRecordProvider) {
    ...
    insert(
        eventsetup::heterocontainer::makeKey<typename T::RecordType,
 ...
 private:
  ...
  EventSetupProvider const& operator=(
      EventSetupProvider const&);  // stop default

  void insert(EventSetupRecordKey const&,
  ...
  std::unique_ptr<
      ...
      finders_;
  std::unique_ptr<std::vector<std::shared_ptr<DataProxyProvider> > >
      dataProviders_;
  ...
  std::unique_ptr<
      ...
      recordToFinders_;
  std::unique_ptr<
      std::map<ParameterSetIDHolder, std::set<EventSetupRecordKey> > >
      psetIDToRecordKey_;
  std::unique_ptr<
      std::map<EventSetupRecordKey, std::map<DataKey, ComponentDescription> > >
      recordToPreferred_;
