...
There are some variants for special cases
  ...
  in the Run or LuminosityBlock.
    ...
    getterOfProducts_ =
edm::GetterOfProducts<Thing>(edm::ProcessMatch(processName_), this, edm::InRun);
...
class GetterOfProducts {
 public:
  ...
  GetterOfProducts(U const& match, M* module,
  ...
  void fillHandles(edm::Event const& event,
                   std::vector<edm::Handle<T>>& handles) const {
    handles.clear();
  ...
  void fillHandles(edm::LuminosityBlock const& lumi,
                   std::vector<edm::Handle<T>>& handles) const {
    handles.clear();
  ...
  void fillHandles(edm::Run const& run,
                   std::vector<edm::Handle<T>>& handles) const {
    handles.clear();
 ...
 private:
  ...
  // to std::function's and we want the copies in those to share the same
  // vector.
