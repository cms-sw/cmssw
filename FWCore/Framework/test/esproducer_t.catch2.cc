...
TEST_CASE("ESProducer", "[Framework][EventSetup]") {
  auto m_scheduler = std::make_unique<edm::ThreadsController>(1);

  SECTION("registerTest") {
    ...
    REQUIRE(!testProd.isUsingRecord(depRecordKey));
  ...
  SECTION("getFromTest") {
    ...
    for (int iTime = 1; iTime != 6; ++iTime) {
      ...
      REQUIRE(iTime == pDummy->value_);
    }
  }

  SECTION("getFromLambdaTest") {
    SynchronousEventSetupsController controller;
    edm::ParameterSet pset = createDummyPset();
    EventSetupProvider& provider = *controller.makeProvider(pset, &activityRegistry);
  ...
  SECTION("getfromOptionalTestNull") {
    SynchronousEventSetupsController controller;
    edm::ParameterSet pset = createDummyPset();
    EventSetupProvider& provider = *controller.makeProvider(pset, &activityRegistry);
  ...
  SECTION("decoratorTest") {
    ...
    for (int iTime = 1; iTime != 6; ++iTime) {
      ...
      REQUIRE(iTime - 1 == TestDecorator::s_pre);
      REQUIRE(iTime - 1 == TestDecorator::s_post);
      consumer.prefetch(provider.eventSetupImpl());
      const edm::EventSetup eventSetup(provider.eventSetupImpl(),
                                       static_cast<unsigned int>(edm::Transition::Event),
                                       consumer.esGetTokenIndices(edm::Transition::Event),
      ...
      edm::ESHandle<DummyData> pDummy = eventSetup.getHandle(consumer.m_token);
      REQUIRE(0 != pDummy.product());
      REQUIRE(iTime == TestDecorator::s_pre);
      REQUIRE(iTime == TestDecorator::s_post);
      REQUIRE(iTime == pDummy->value_);
    }
  }
  ...
  SECTION("dependsOnTest") {
    ...
    for (int iTime = 1; iTime != 6; ++iTime) {
      ...
      edm::ESHandle<DummyData> pDummy = eventSetup.getHandle(consumer.m_token);
      REQUIRE(0 != pDummy.product());
      REQUIRE(3 * iTime == pDummy->value_);
    }
  }
  SECTION("consumesTest") {
    ConsumesProducer prod;

    auto const& consumes = prod.esModuleConsumesMinimalInfos();
    REQUIRE(consumes.size() == 1);

    auto const& consumesInfo = consumes[0];
    REQUIRE(consumesInfo.recordForDataKey_ == EventSetupRecordKey::makeKey<DummyRecord>());
    REQUIRE(consumesInfo.dataKey_.type() == edm::eventsetup::DataKey::makeTypeTag<DummyData>());
    REQUIRE(consumesInfo.dataKey_.name() == "");
    REQUIRE(consumesInfo.componentLabel_.empty());
    REQUIRE(consumesInfo.produceMethodID_ == 0);
  }

  SECTION("mayConsumesTest") {
    SetMayConsumeProducer prod;

    auto const& consumes = prod.esModuleConsumesMinimalInfos();
    REQUIRE(consumes.size() == 3);
    {
      auto const& consumesInfo = consumes[0];
      REQUIRE(consumesInfo.recordForDataKey_ == EventSetupRecordKey::makeKey<DummyRecord>());
      REQUIRE(consumesInfo.dataKey_.type() == edm::eventsetup::DataKey::makeTypeTag<DummyData>());
    ...
    }
    {
      ...
      REQUIRE(consumesInfo.recordForDataKey_ == EventSetupRecordKey::makeKey<DummyRecord>());
      REQUIRE(consumesInfo.dataKey_.type() == edm::eventsetup::DataKey::makeTypeTag<DummyData>());
      REQUIRE(consumesInfo.dataKey_.name() == "@mayConsume");
      REQUIRE(consumesInfo.componentLabel_ == "@mayConsume");
      REQUIRE(consumesInfo.produceMethodID_ == 0);
    }

    {
      auto const& consumesInfo = consumes[2];
      REQUIRE(consumesInfo.recordForDataKey_ == EventSetupRecordKey::makeKey<DepRecord>());
      REQUIRE(consumesInfo.dataKey_.type() == edm::eventsetup::DataKey::makeTypeTag<DummyData>());
      REQUIRE(consumesInfo.dataKey_.name() == "");
      REQUIRE(consumesInfo.componentLabel_.empty());
      REQUIRE(consumesInfo.produceMethodID_ == 0);
  ...
  SECTION("producesTest") {
    LabelledProducer prod;

    auto const& produces = prod.producesInfo();
    REQUIRE(produces.size() == 3);
    {
      auto const& producesInfo = produces[0];

      REQUIRE(producesInfo.record() == EventSetupRecordKey::makeKey<DummyRecord>());
      REQUIRE(producesInfo.dataKey().type() == edm::eventsetup::DataKey::makeTypeTag<DummyData>());
      REQUIRE(producesInfo.dataKey().name() == "fi");
      REQUIRE(producesInfo.produceMethodID() == 0);
    }
    {
      auto const& producesInfo = produces[1];
      REQUIRE(producesInfo.record() == EventSetupRecordKey::makeKey<DummyRecord>());
      REQUIRE(producesInfo.dataKey().type() == edm::eventsetup::DataKey::makeTypeTag<DummyData>());
      REQUIRE(producesInfo.dataKey().name() == "fum");
      REQUIRE(producesInfo.produceMethodID() == 0);
    }
    {
      ...
      REQUIRE(producesInfo.record() == EventSetupRecordKey::makeKey<DummyRecord>());
      REQUIRE(producesInfo.dataKey().type() == edm::eventsetup::DataKey::makeTypeTag<DummyData>());
      REQUIRE(producesInfo.dataKey().name() == "foo");
      REQUIRE(producesInfo.produceMethodID() == 1);
    }
  }
  ...
  SECTION("productResolverProviderTest") {
    ...
    REQUIRE(productResolverProvider.isUsingRecord(dummyRecordKey));
    REQUIRE(productResolverProvider.isUsingRecord(dummy2RecordKey));
    REQUIRE(productResolverProvider.isUsingRecord(depRecordKey));
    REQUIRE(productResolverProvider.isUsingRecord(depOn2RecordKey));
    ...
    REQUIRE(keyedResolvers0.recordKey() == dummyRecordKey);
    REQUIRE(keyedResolvers0.size() == 1);
    {
      auto it = keyedResolvers0.begin();
      auto itEnd = keyedResolvers0.end();
      REQUIRE(it.dataKey() == productResolverProvider.dataKeyDummy_0_);
      REQUIRE(it.productResolver() == productResolverProvider.resolverDummy_0_0_.get());
      REQUIRE(it != itEnd);
      ++it;
      REQUIRE(!(it != itEnd));
    }
    ...
    REQUIRE(keyedResolvers1.recordKey() == dummyRecordKey);
    REQUIRE(keyedResolvers1.size() == 1);
    {
      auto it = keyedResolvers1.begin();
      auto itEnd = keyedResolvers1.end();
      REQUIRE(it.dataKey() == productResolverProvider.dataKeyDummy_0_);
      REQUIRE(it.productResolver() == productResolverProvider.resolverDummy_0_1_.get());
      REQUIRE(it != itEnd);
      ++it;
      REQUIRE(!(it != itEnd));
    }
    ...
    REQUIRE(keyedResolvers2.recordKey() == dummyRecordKey);
    REQUIRE(keyedResolvers2.size() == 1);
    {
      auto it = keyedResolvers2.begin();
      auto itEnd = keyedResolvers2.end();
      REQUIRE(it.dataKey() == productResolverProvider.dataKeyDummy_0_);
      REQUIRE(it.productResolver() == productResolverProvider.resolverDummy_0_2_.get());
      REQUIRE(it != itEnd);
      ++it;
      REQUIRE(!(it != itEnd));
    }
    ...
    REQUIRE(keyedResolvers3.recordKey() == dummy2RecordKey);
    REQUIRE(keyedResolvers3.size() == 3);
    {
      auto it = keyedResolvers3.begin();
      auto itEnd = keyedResolvers3.end();
      REQUIRE(it.dataKey() == productResolverProvider.dataKeyDummy2_0_);
      REQUIRE(it.productResolver() == productResolverProvider.resolverDummy2_0_0_.get());
      REQUIRE(it != itEnd);
      ++it;
      REQUIRE(it.dataKey() == productResolverProvider.dataKeyDummy2_1_);
      REQUIRE(it.productResolver() == productResolverProvider.resolverDummy2_1_0_.get());
      REQUIRE(it != itEnd);
      ++it;
      REQUIRE(it.dataKey() == productResolverProvider.dataKeyDummy2_2_);
      REQUIRE(it.productResolver() == productResolverProvider.resolverDummy2_2_0_.get());
      REQUIRE(it != itEnd);
      ++it;
      REQUIRE(!(it != itEnd));
    }
    ESProductResolverProvider::KeyedResolvers& keyedResolvers4 =
        productResolverProvider.keyedResolvers(depRecordKey, 0);
    REQUIRE(keyedResolvers4.recordKey() == depRecordKey);
    REQUIRE(keyedResolvers4.size() == 2);
    {
      auto it = keyedResolvers4.begin();
      auto itEnd = keyedResolvers4.end();
      REQUIRE(it.dataKey() == productResolverProvider.dataKeyDep_0_);
      REQUIRE(it.productResolver() == productResolverProvider.resolverDep_0_0_.get());
      REQUIRE(it != itEnd);
      ++it;
      REQUIRE(it.dataKey() == productResolverProvider.dataKeyDep_1_);
      REQUIRE(it.productResolver() == productResolverProvider.resolverDep_1_0_.get());
      REQUIRE(it != itEnd);
      ++it;
      REQUIRE(!(it != itEnd));
    }
    ESProductResolverProvider::KeyedResolvers& keyedResolvers5 =
        productResolverProvider.keyedResolvers(depRecordKey, 1);
    REQUIRE(keyedResolvers5.recordKey() == depRecordKey);
    REQUIRE(keyedResolvers5.size() == 2);
    {
      auto it = keyedResolvers5.begin();
      auto itEnd = keyedResolvers5.end();
      REQUIRE(it.dataKey() == productResolverProvider.dataKeyDep_0_);
      REQUIRE(it.productResolver() == productResolverProvider.resolverDep_0_1_.get());
      REQUIRE(it != itEnd);
      ++it;
      REQUIRE(it.dataKey() == productResolverProvider.dataKeyDep_1_);
      REQUIRE(it.productResolver() == productResolverProvider.resolverDep_1_1_.get());
      REQUIRE(it != itEnd);
      ++it;
      REQUIRE(!(it != itEnd));
    }
    ...
    REQUIRE(keyedResolvers6.recordKey() == depOn2RecordKey);
    REQUIRE(keyedResolvers6.size() == 0);
    {
    ...
    REQUIRE(keyedResolvers7.recordKey() == depOn2RecordKey);
    REQUIRE(keyedResolvers7.size() == 0);
    {
    ...
    REQUIRE(keyedResolvers8.recordKey() == depOn2RecordKey);
    REQUIRE(keyedResolvers8.size() == 0);
    {
    ...
    REQUIRE(keyedResolvers9.recordKey() == depOn2RecordKey);
    REQUIRE(keyedResolvers9.size() == 0);
    {
    ...
    REQUIRE(keyedResolvers4.contains(productResolverProvider.dataKeyDep_0_));
    REQUIRE(keyedResolvers4.contains(productResolverProvider.dataKeyDep_1_));
    REQUIRE(!keyedResolvers4.contains(productResolverProvider.dataKeyDummy_0_));

    ESProductResolverProvider::KeyedResolvers keyedResolvers10(nullptr, 0);
    REQUIRE(keyedResolvers10.unInitialized());
    REQUIRE(!keyedResolvers0.unInitialized());
  }
}
