 ...
 private:
  ...
  class GlobalProd : public edm::stream::EDProducer<edm::GlobalCache<int>> {
   public:
    ...
    static std::unique_ptr<int> initializeGlobalCache(
        edm::ParameterSet const&) {
      return std::make_unique<int>(1);
    }
    GlobalProd(edm::ParameterSet const&, const int* iGlobal) {
      CPPUNIT_ASSERT(*iGlobal == 1);
    }
  ...
  class RunProd : public edm::stream::EDProducer<edm::RunCache<int>> {
   public:
    ...
    static std::shared_ptr<int> globalBeginRun(edm::Run const&,
    ...
    static void globalEndRun(edm::Run const&, edm::EventSetup const&,
                             RunProd::RunContext const*) {
      ++m_count;
    }
  };

  class LumiProd
      : public edm::stream::EDProducer<edm::LuminosityBlockCache<int>> {
   public:
    static unsigned int m_count;
    LumiProd(edm::ParameterSet const&) {}
    void produce(edm::Event&, edm::EventSetup const&) override { ++m_count; }

    static std::shared_ptr<int> globalBeginLuminosityBlock(
  ...
  class RunSummaryProd
      : public edm::stream::EDProducer<edm::RunSummaryCache<int>> {
   public:
    static unsigned int m_count;
    ...
    static void globalEndRunSummary(edm::Run const&, edm::EventSetup const&,
                                    RunContext const*, int*) {
      ++m_count;
    }
  };

  class LumiSummaryProd
      : public edm::stream::EDProducer<edm::LuminosityBlockSummaryCache<int>> {
   public:
    static unsigned int m_count;
    LumiSummaryProd(edm::ParameterSet const&) {}
    void produce(edm::Event&, edm::EventSetup const&) override { ++m_count; }

    static std::shared_ptr<int> globalBeginLuminosityBlockSummary(
        edm::LuminosityBlock const&, edm::EventSetup const&,
        LuminosityBlockContext const*) {
      ++m_count;
      return std::shared_ptr<int>{};
  ...
  class BeginLumiProd
      : public edm::stream::EDProducer<edm::BeginLuminosityBlockProducer> {
   public:
    static unsigned int m_count;
    ...
    static void globalBeginLuminosityBlockProduce(
  ...
  class EndLumiProd
      : public edm::stream::EDProducer<edm::EndLuminosityBlockProducer> {
   public:
    static unsigned int m_count;
  ...
  class EndRunSummaryProd
      : public edm::stream::EDProducer<edm::EndRunProducer,
...
testStreamModule::testStreamModule()
  ...
  // Only an EventProcessor or SubProcess is allowed to create a StreamID but I
  // need one
  ...
  m_ep.reset(new edm::EventPrincipal(m_prodReg, m_idHelper,
                                     m_associationsHelper, m_procConfig,
                                     nullptr, *pID));
  ...
  m_transToFunc[Trans::kStreamBeginLuminosityBlock] = [this](
      edm::Worker* iBase) {
    typedef edm::OccurrenceTraits<edm::LuminosityBlockPrincipal,
                                  edm::BranchActionStreamBegin>
        Traits;
...
template <typename T>
void testTransition(edm::Worker* iWorker, testStreamModule::Trans iTrans,
  ...
  if (count != T::m_count) {
    std::cout
        << "For trans "
        << static_cast<std::underlying_type<testStreamModule::Trans>::type>(
               iTrans)
...
template <typename T, typename U>
void testStreamModule::testTransitions(std::shared_ptr<U> iMod,
...
void testStreamModule::runTest() {
  runTest<RunProd>(
      {Trans::kGlobalBeginRun, Trans::kEvent, Trans::kGlobalEndRun});
}

void testStreamModule::runSummaryTest() {
  runTest<RunSummaryProd>({Trans::kGlobalBeginRun, Trans::kEvent,
                           Trans::kStreamEndRun, Trans::kGlobalEndRun});
}

void testStreamModule::lumiTest() {
  runTest<LumiProd>({Trans::kGlobalBeginLuminosityBlock, Trans::kEvent,
                     Trans::kGlobalEndLuminosityBlock});
}

void testStreamModule::lumiSummaryTest() {
  runTest<LumiSummaryProd>({Trans::kGlobalBeginLuminosityBlock, Trans::kEvent,
...
void testStreamModule::endRunSummaryProdTest() {
  runTest<EndRunSummaryProd>({Trans::kGlobalEndRun, Trans::kEvent,
                              Trans::kGlobalBeginRun, Trans::kStreamEndRun,
                              Trans::kGlobalEndRun});
}

void testStreamModule::endLumiSummaryProdTest() {
  runTest<EndLumiSummaryProd>({Trans::kGlobalEndLuminosityBlock, Trans::kEvent,
