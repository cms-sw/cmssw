...
public:
  static void commit(edm::Event& iEvent, std::vector<edm::ProductResolverIndex> const& iIndices) {
    iEvent.commit_(iIndices);
  }
};

TEST_CASE("EventGetRefBeforePut", "[Framework]") {
  edm::HistoryAppender historyAppender_;

  SECTION("failGetProductNotRegisteredTest") {
    ...
    try {
      ...
      REQUIRE("Failed to throw required exception" == 0);
    } catch (edm::Exception& x) {
      // nothing to do
    } catch (...) {
      REQUIRE("Threw wrong kind of exception" == 0);
    }
  ...
  SECTION("getRefTest") {
    ...
    try {
      ...
      testEventGetRefBeforePut::commit(event, std::vector<edm::ProductResolverIndex>());
    } catch (cms::Exception& x) {
      std::cerr << x.explainSelf() << std::endl;
      REQUIRE("Threw exception unexpectedly" == 0);
    } catch (std::exception& x) {
      std::cerr << x.what() << std::endl;
      REQUIRE("threw std::exception" == 0);
    } catch (...) {
      std::cerr << "Unknown exception type\n";
      REQUIRE("Threw exception unexpectedly" == 0);
    }
    REQUIRE(refToProd->value == 10);
  }
}
