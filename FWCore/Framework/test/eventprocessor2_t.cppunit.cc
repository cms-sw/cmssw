...
void testeventprocessor2::eventprocessor2Test() {
  try {
    ...
    CPPUNIT_ASSERT(
        "cms Exception caught in testeventprocessor2::eventprocessor2Test" ==
        0);
  } catch (std::runtime_error& e) {
    std::cerr << "Standard library exception caught: " << e.what() << std::endl;
    CPPUNIT_ASSERT(
        "std Exception caught in testeventprocessor2::eventprocessor2Test" ==
        0);
  } catch (...) {
    std::cerr << "Unknown exception caught" << std::endl;
    CPPUNIT_ASSERT(
        "unkown Exception caught in testeventprocessor2::eventprocessor2Test" ==
        0);
  }
}
