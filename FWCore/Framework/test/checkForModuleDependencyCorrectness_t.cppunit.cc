...
private:
  ...
  std::vector<edm::ModuleDescription const*> const& doModulesOnPath(unsigned int pathIndex) const final {return m_modulesOnPath[pathIndex];}
  std::vector<edm::ModuleDescription const*> const& doModulesOnEndPath(unsigned int endPathIndex) const final {return m_modulesOnEndPath[endPathIndex];}
  ...
  std::vector<edm::ConsumesInfo> doConsumesInfo(unsigned int moduleID) const final {return m_moduleConsumesInfo[moduleID];}
  unsigned int doLargestModuleID() const final { if(m_modules.empty()) {return 0;} return m_modules.size()-1;}
...
PathsAndConsumesOfModulesForTest::PathsAndConsumesOfModulesForTest(ModuleDependsOnMap const& iModDeps, PathToModules const& iPaths){
  ...
  m_cache.emplace_back(edm::ParameterSetID{}, "source", "source", &pc, indexForModule("source", modsToIndex, indexToMods));
  ...
  if(not iPaths.empty()) {
    ...
    m_cache.emplace_back(edm::ParameterSetID{}, "TriggerResultInserter", "TriggerResults", &pc, indexForTriggerResults);
  }
