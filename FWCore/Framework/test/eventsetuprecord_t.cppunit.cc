...
static eventsetup::EventSetupRecordProviderFactoryTemplate<
    eventsetuprecord_t::DummyRecord> const s_factory;
...
void testEventsetupRecord::factoryTest() {
  ...
  CPPUNIT_ASSERT(0 !=
                 dynamic_cast<EventSetupRecordProviderTemplate<DummyRecord>*>(
                     &(*dummyProvider)));
}

HCTYPETAG_HELPER_METHODS(Dummy)

class FailingDummyProxy
    : public eventsetup::DataProxyTemplate<DummyRecord, Dummy> {
 protected:
  const value_type* make(const record_type&, const DataKey&) { return 0; }
  void invalidateCache() {}
};

class WorkingDummyProxy
    : public eventsetup::DataProxyTemplate<DummyRecord, Dummy> {
 public:
  WorkingDummyProxy(const Dummy* iDummy)
      : data_(iDummy),
 ...
 protected:
  ...
  void invalidateTransientCache() {
    ...
    eventsetup::DataProxyTemplate<DummyRecord,
                                  Dummy>::invalidateTransientCache();
  }
...
class WorkingDummyProvider : public edm::eventsetup::DataProxyProvider {
 public:
  WorkingDummyProvider(const edm::eventsetup::DataKey& iKey,
                       std::shared_ptr<WorkingDummyProxy> iProxy)
...
void testEventsetupRecord::proxyTest() {
  ...
  const DataKey dummyDataKey(
      DataKey::makeTypeTag<FailingDummyProxy::value_type>(), "");
  ...
  const DataKey dummyFredDataKey(
      DataKey::makeTypeTag<FailingDummyProxy::value_type>(), "fred");
...
void testEventsetupRecord::getTest() {
  ...
  const DataKey workingDataKey(
      DataKey::makeTypeTag<WorkingDummyProxy::value_type>(), "working");

  dummyRecord.add(workingDataKey, &workingProxy);
...
void testEventsetupRecord::introspectionTest() {
  ...
  const DataKey workingDataKey3(
      DataKey::makeTypeTag<WorkingDummyProxy::value_type>(), "working3");

  dummyRecord.add(workingDataKey3, &workingProxy3);
  ...
  const DataKey workingDataKey4(
      DataKey::makeTypeTag<WorkingDummyProxy::value_type>(), "working4");

  dummyRecord.add(workingDataKey4, &workingProxy4);
...
void testEventsetupRecord::doGetExepTest() {
  ...
  // typedef edm::eventsetup::MakeDataException<DummyRecord,Dummy>
  // ExceptionType;
  dummyRecord.doGet(dummyDataKey);
  // CPPUNIT_ASSERT_THROW(dummyRecord.doGet(dummyDataKey), ExceptionType);
}
...
void testEventsetupRecord::proxyResetTest() {
  ...
  EventSetupRecordProviderTemplate<DummyRecord>* prov =
      dynamic_cast<EventSetupRecordProviderTemplate<DummyRecord>*>(
          &(*dummyProvider));
  ...
  Dummy myDummy;
  std::shared_ptr<WorkingDummyProxy> workingProxy =
      std::make_shared<WorkingDummyProxy>(&myDummy);

  const DataKey workingDataKey(
      DataKey::makeTypeTag<WorkingDummyProxy::value_type>(), "");

  std::shared_ptr<WorkingDummyProvider> wdProv =
      std::make_shared<WorkingDummyProvider>(workingDataKey, workingProxy);
...
void testEventsetupRecord::transientTest() {
  ...
  const EventSetupRecordProviderTemplate<DummyRecord>* constProv = prov;
  const EventSetupRecord& dummyRecord = constProv->record();
  EventSetupRecord& nonConstDummyRecord =
      const_cast<EventSetupRecord&>(dummyRecord);
  ...
  wdProv->resetProxiesIfTransient(
      dummyRecord.key());  //   workingProxy->resetIfTransient();
  ...
  // do another transient access which should not do a reset since we have a
  // non-transient access outstanding
  dummyRecord.get(hDummy);
  dummyRecord.get(hTDummy);
  ...
  // Ask for a transient then a non transient to be sure we don't have an
  // ordering problem
  {
    prov->resetProxies();
    Dummy myDummy3;
  ...
  // system should wait until the second event of a run before invalidating the
  // transients
  // need to do 'resetProxies' in order to force the Record to reset since we do
  // not have a Finder
