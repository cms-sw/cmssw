...
TEST_CASE("GenericHandle", "[Framework]") {
  edm::HistoryAppender historyAppender_;

  SECTION("failWrongType") {
    try {
      //intentionally misspelled type
      edm::GenericHandle h("edmtest::DmmyProduct");
      REQUIRE("Failed to throw" == nullptr);
    } catch (cms::Exception& x) {
      // nothing to do
    } catch (...) {
      REQUIRE("Threw wrong kind of exception" == nullptr);
    }
  }
  ...
  SECTION("failgetbyLabelTest") {
    ...
    try {
      ...
      *h;
      ...
      REQUIRE("Threw std::exception!" == nullptr);
    } catch (...) {
      REQUIRE("Threw wrong kind of exception" == nullptr);
    }
    if (!didThrow) {
      REQUIRE("Failed to throw required exception" == nullptr);
    }
  }

  SECTION("getbyLabelTest") {
    ...
    try {
      ...
      REQUIRE("Threw cms::Exception unexpectedly" == nullptr);
    } catch (std::exception& x) {
      std::cerr << x.what() << std::endl;
      REQUIRE("threw std::exception" == nullptr);
    } catch (...) {
      std::cerr << "Unknown exception type\n";
      REQUIRE("Threw exception unexpectedly" == nullptr);
    }
    REQUIRE(h.isValid());
    REQUIRE(h.provenance()->moduleLabel() == label);
  }
}
