...
class TypeToGetConsumer : public edm::EDConsumerBase {
 public:
  TypeToGetConsumer(
      std::vector<std::pair<edm::TypeToGet, edm::InputTag>> const& iTags) {
    m_tokens.reserve(iTags.size());
...
void TestEDConsumerBase::testRegularType() {
  ...
  helper.insert(typeIDVSimpleDerived, "labelC", "instanceC",
                "processC");  // 27, 28, 29, 30

  helper.setFrozen();

  edm::TypeID typeID_vint(typeid(std::vector<int>));
  const auto vint_c = helper.index(edm::PRODUCT_TYPE, typeID_vint, "labelC",
                                   "instanceC", "processC");
  const auto vint_c_no_proc =
      helper.index(edm::PRODUCT_TYPE, typeID_vint, "labelC", "instanceC", 0);
  const auto vint_blank = helper.index(edm::PRODUCT_TYPE, typeID_vint, "label",
                                       "instance", "process");
      ...
      helper.index(edm::PRODUCT_TYPE, typeID_vint, "label", "instance", 0);
  {
    ...
    CPPUNIT_ASSERT(
        vint_c ==
        intConsumer
            .indexFrom(intConsumer.m_tokens[1], edm::InEvent, typeID_vint)
            .productResolverIndex());
    CPPUNIT_ASSERT(
        vint_blank ==
        intConsumer
            .indexFrom(intConsumer.m_tokens[0], edm::InEvent, typeID_vint)
            .productResolverIndex());
  ...
  }
  {
    ...
    CPPUNIT_ASSERT(
        vint_c ==
        intConsumerRev
            .indexFrom(intConsumerRev.m_tokens[0], edm::InEvent, typeID_vint)
            .productResolverIndex());
    CPPUNIT_ASSERT(
        vint_blank ==
        intConsumerRev
            .indexFrom(intConsumerRev.m_tokens[1], edm::InEvent, typeID_vint)
            .productResolverIndex());
  ...
  }
  {
    ...
    CPPUNIT_ASSERT(edm::ProductResolverIndexAndSkipBit(vint_c, false) ==
                   intConsumerRev.indexFrom(intConsumerRev.m_tokens[0],
                                            edm::InEvent, typeID_vint));
    CPPUNIT_ASSERT(edm::ProductResolverIndexAndSkipBit(vint_blank, false) ==
                   intConsumerRev.indexFrom(intConsumerRev.m_tokens[1],
                                            edm::InEvent, typeID_vint));
  ...
  }
  {
    ...
    std::vector<edm::InputTag> vTags = {
        {"label", "instance"}, {"labelC", "instanceC", "@skipCurrentProcess"}};
    IntsConsumer intConsumer{vTags};
    intConsumer.updateLookup(edm::InEvent, helper, false);
    ...
    CPPUNIT_ASSERT(edm::ProductResolverIndexAndSkipBit(vint_c_no_proc, true) ==
                   intConsumer.indexFrom(intConsumer.m_tokens[1], edm::InEvent,
                                         typeID_vint));
    CPPUNIT_ASSERT(
        edm::ProductResolverIndexAndSkipBit(vint_blank_no_proc, false) ==
        intConsumer.indexFrom(intConsumer.m_tokens[0], edm::InEvent,
                              typeID_vint));
...
void TestEDConsumerBase::testViewType() {
  ...
  const auto v_int = helper.index(edm::ELEMENT_TYPE, typeID_int, "label",
                                  "instance", "process");
  const auto v_simple = helper.index(edm::ELEMENT_TYPE, typeID_Simple, "labelC",
                                     "instanceC", "processC");

  const auto v_int_no_proc =
      helper.index(edm::ELEMENT_TYPE, typeID_int, "label", "instance");
...

  {
    std::vector<std::pair<edm::TypeToGet, edm::InputTag>> vT = {
        {edm::TypeToGet::make<edm::View<int>>(),
         {"label", "instance", "process"}},
...
void TestEDConsumerBase::testMany() {
  ...
  const auto productIndex = helper.index(edm::PRODUCT_TYPE, typeID_EventID,
                                         "labelB", "instanceB", "processB");
