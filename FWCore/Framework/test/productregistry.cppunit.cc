...
struct Responder {
  ...
  void respond(edm::BranchDescription const& iDesc) {
    ...
    edm::BranchDescription prod(
        iDesc.branchType(), name_, iDesc.processName(), iDesc.fullClassName(),
...
void testProductRegistry::setUp() {
  ...
  intBranch_.reset(new edm::BranchDescription(
      edm::InEvent, "labeli", "PROD", "int", "int", "int", "", pset.id(),
      edm::TypeWithDict(typeid(int))));

  floatBranch_.reset(new edm::BranchDescription(
      edm::InEvent, "labelf", "PROD", "float", "float", "float", "", pset.id(),
...
void testProductRegistry::testWatch() {
  ...
  // BranchDescription prod2(InEvent, "label", "PROD", "float", "float",
  // "float");
