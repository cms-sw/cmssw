...
class MockEventProcessor {
 public:
  ...
  MockEventProcessor(std::string const& mockData, std::ostream& output,
                     bool iDoNotMerge);
  ...
  void endRun(ProcessHistoryID const& phid, RunNumber_t run,
              bool cleaningUpAfterException);

  void beginLumi(ProcessHistoryID const& phid, RunNumber_t run,
                 LuminosityBlockNumber_t lumi);
  void endLumi(ProcessHistoryID const& phid, RunNumber_t run,
               LuminosityBlockNumber_t lumi, bool cleaningUpAfterException);
  ...
  void writeLumi(ProcessHistoryID const& phid, RunNumber_t run,
                 LuminosityBlockNumber_t lumi);
