...
//
/**\class TestModuleChangeLooper TestModuleChangeLooper.h
 FWCore/TestModuleChangeLooper/interface/TestModuleChangeLooper.h
 ...
 public:
  ...
  Status duringLoop(edm::Event const& iEvent, edm::EventSetup const&) {
    ...
    if (handle->value != m_expectedValue) {
      throw cms::Exception("WrongValue") << "expected value " << m_expectedValue
                                         << " but got " << handle->value;
    }
    return kContinue;
  }
  Status endOfLoop(edm::EventSetup const&, unsigned int iCount) {
    ...
    edm::ParameterSet const* pset =
        scheduleInfo()->parametersForModule(m_tag.label());
    assert(0 != pset);
