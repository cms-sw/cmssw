...
void FailingProducer::produce(edm::StreamID, edm::Event&,
...
// every call to FailingProducer::produce throws a cms exception
//
class FailingInRunProducer
    : public edm::global::EDProducer<edm::BeginRunProducer> {
 public:
...
void FailingInRunProducer::produce(edm::StreamID, edm::Event&,
                                   edm::EventSetup const&) const {}
void FailingInRunProducer::globalBeginRunProduce(edm::Run&,
                                                 edm::EventSetup const&) const {
  // We throw an edm exception with a configurable action.
  throw edm::Exception(edm::errors::NotFound)
      << "Intentional 'NotFound' exception for testing purposes\n";
...
void NonProducer::produce(edm::StreamID, edm::Event&,
                          edm::EventSetup const&) const {}
...
//
// NOTE: this really should be a global::EDProducer<> but for testing we use
// stream
...
void BusyWaitIntProducer::produce(edm::StreamID, edm::Event& e,
                                  edm::EventSetup const&) const {
  double sum = 0.;
...
void ConsumingIntProducer::produce(edm::Event& e, edm::EventSetup const&) {
  e.put(std::make_unique<IntProduct>(value_));
}
...
class EventNumberIntProducer : public edm::global::EDProducer<> {
 public:
  ...
  EventNumberIntProducer() { produces<UInt64Product>(); }
  virtual void produce(edm::StreamID, edm::Event& e,
                       edm::EventSetup const& c) const override;
...
void EventNumberIntProducer::produce(edm::StreamID, edm::Event& e,
                                     edm::EventSetup const&) const {
  // EventSetup is not used.
...
void TransientIntProducer::produce(edm::StreamID, edm::Event& e,
                                   edm::EventSetup const&) const {
  // EventSetup is not used.
...
class IntProducerFromTransient : public edm::global::EDProducer<> {
 public:
  ...
  explicit IntProducerFromTransient() { produces<IntProduct>(); }
...
void IntProducerFromTransient::produce(edm::StreamID, edm::Event& e,
                                       edm::EventSetup const&) const {
  // EventSetup is not used.
...
void Int16_tProducer::produce(edm::StreamID, edm::Event& e,
                              edm::EventSetup const&) const {
  // EventSetup is not used.
 ...
 public:
  explicit AddIntsProducer(edm::ParameterSet const& p)
      : labels_(p.getParameter<std::vector<std::string> >("labels")),
        onlyGetOnEvent_(
            p.getUntrackedParameter<unsigned int>("onlyGetOnEvent", 0u)) {
    produces<IntProduct>();
...
void AddIntsProducer::produce(edm::StreamID, edm::Event& e,
                              edm::EventSetup const&) const {
  // EventSetup is not used.
  int value = 0;
