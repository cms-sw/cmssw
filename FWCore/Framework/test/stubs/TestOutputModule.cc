...
void packIntoString(std::vector<unsigned char> const& source,
  ...
  unsigned int sizeOfPackage =
      1 + ((source.size() - 1) / packInOneByte);  // Two bits per HLT
  ...
  for (unsigned int i = 0; i != source.size(); ++i) {
    ...
    package[whichByte] =
        package[whichByte] | (source[i] << (indxWithinByte * 2));
  }
...
TestOutputModule::TestOutputModule(edm::ParameterSet const& ps)
    : edm::OutputModule(ps),
      ...
      hltbits_(0),
      expectTriggerResults_(
          ps.getUntrackedParameter<bool>("expectTriggerResults", true)),
      resultsToken_(
...
void TestOutputModule::writeLuminosityBlock(
    edm::LuminosityBlockForOutput const& lb) {
