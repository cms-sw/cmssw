...
class StreamIntProducer
    : public edm::global::EDProducer<edm::StreamCache<UnsafeCache>> {
 public:
  ...
  void produce(edm::StreamID iID, edm::Event&,
               edm::EventSetup const&) const override {
    ++m_count;
    auto sCache = streamCache(iID);
    ++(sCache->work);
  ...
  ~StreamIntProducer() {
    if (m_count != trans_) {
      throw cms::Exception("transitions")
          << "StreamIntProducer transitions " << m_count
...
class RunIntProducer
 ...
 public:
  ...
  std::shared_ptr<Cache> globalBeginRun(edm::Run const& iRun,
                                        edm::EventSetup const&) const override {
    ++m_count;
  ...
  void produce(edm::StreamID iID, edm::Event& iEvent,
               edm::EventSetup const&) const override {
  ...
  void globalEndRun(edm::Run const& iRun,
    ...
    if (rCache->value != cvalue_) {
      throw cms::Exception("cache value")
          << "RunIntProducer cache value " << rCache->value
  ...
  ~RunIntProducer() {
    if (m_count != trans_) {
      throw cms::Exception("transitions")
          << "RunIntProducer transitions " << m_count
          << " but it was supposed to be " << trans_;
    }
  }
};
 ...
 public:
  ...
  void globalEndLuminosityBlock(edm::LuminosityBlock const& iLB,
    ...
    if (lCache->value != cvalue_) {
      throw cms::Exception("cache value")
          << "LumiIntProducer cache value " << lCache->value
          << " but it was supposed to be " << cvalue_;
    }
  }
  ...
  ~LumiIntProducer() {
    if (m_count != trans_) {
      throw cms::Exception("transitions")
          << "LumiIntProducer transitions " << m_count
          << " but it was supposed to be " << trans_;
    }
  }
};
...
class RunSummaryIntProducer
 ...
 public:
  ...
  std::shared_ptr<UnsafeCache> globalBeginRunSummary(
      edm::Run const& iRun, edm::EventSetup const&) const override {
    ++m_count;
  ...
  void globalEndRunSummary(edm::Run const&, edm::EventSetup const&,
    ...
    if (gCache->value != cvalue_) {
      throw cms::Exception("cache value")
          << "RunSummaryIntProducer cache value " << gCache->value
          << " but it was supposed to be " << cvalue_;
    }
    --(gCache->run);
  }
  ...
  ~RunSummaryIntProducer() {
    if (m_count != trans_) {
      throw cms::Exception("transitions")
          << "RunSummaryIntProducer transitions " << m_count
          << " but it was supposed to be " << trans_;
    }
  }
};
...
class LumiSummaryIntProducer
 ...
 public:
  ...
  void globalEndLuminosityBlockSummary(edm::LuminosityBlock const&,
    ...
    if (gCache->value != cvalue_) {
      throw cms::Exception("cache value")
          << "LumiSummaryIntProducer cache value " << gCache->value
          << " but it was supposed to be " << cvalue_;
    }
    --(gCache->lumi);
  }
  ...
  ~LumiSummaryIntProducer() {
    if (m_count != trans_) {
      throw cms::Exception("transitions")
          << "LumiSummaryIntProducer transitions " << m_count
          << " but it was supposed to be " << trans_;
    }
  }
};
...
class TestBeginRunProducer
    : public edm::global::EDProducer<edm::RunCache<Dummy>,
...
class TestEndRunProducer : public edm::global::EDProducer<edm::RunCache<Dummy>,
                                                          edm::EndRunProducer> {
 public:
...
class TestBeginLumiBlockProducer
    : public edm::global::EDProducer<edm::LuminosityBlockCache<void>,
 ...
 public:
  ...
  void produce(edm::StreamID iID, edm::Event&,
    ...
    if (!gblp) {
      throw cms::Exception("out of sequence")
          << "produce before globalBeginLuminosityBlockProduce in Stream "
          << iID.value();
    }
  }
