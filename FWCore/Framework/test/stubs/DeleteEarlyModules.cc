...
class DeleteEarlyProducer : public edm::EDProducer {
 public:
  ...
  virtual void beginJob() {
    // Needed because DeleteEarly objects may be allocated and deleted in
    // initialization
...
class DeleteEarlyCheckDeleteAnalyzer : public edm::EDAnalyzer {
 public:
  DeleteEarlyCheckDeleteAnalyzer(edm::ParameterSet const& pset)
      : m_expectedValues(pset.getUntrackedParameter<std::vector<unsigned int>>(
            "expectedValues")),
        m_index(0) {}
  ...
  virtual void analyze(edm::Event const&, edm::EventSetup const&) {
    if (DeleteEarly::nDeletes() != m_expectedValues.at(m_index)) {
      throw cms::Exception("DeleteEarlyError")
          << "On index " << m_index << " we expected "
          << m_expectedValues[m_index] << " deletes but we see "
