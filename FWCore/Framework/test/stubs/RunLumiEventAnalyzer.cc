...
RunLumiEventAnalyzer::RunLumiEventAnalyzer(edm::ParameterSet const& pset)
    : expectedRunLumisEvents0_(),
      ...
      dumpTriggerResults_(
          pset.getUntrackedParameter<bool>("dumpTriggerResults", false)),
      expectedEndingIndex0_(
          pset.getUntrackedParameter<int>("expectedEndingIndex", -1)),
      expectedEndingIndex1_(
          pset.getUntrackedParameter<int>("expectedEndingIndex1", -1)),
      expectedEndingIndex_(expectedEndingIndex0_) {
  if (pset.existsAs<std::vector<unsigned int> >("expectedRunLumiEvents",
                                                false)) {
    ...
    expectedRunLumisEvents0_ =
        pset.getUntrackedParameter<std::vector<unsigned long long> >(
            "expectedRunLumiEvents", std::vector<unsigned long long>());
  }

  if (pset.existsAs<std::vector<unsigned int> >("expectedRunLumiEvents1",
                                                false)) {
    std::vector<unsigned int> temp =
        pset.getUntrackedParameter<std::vector<unsigned int> >(
            "expectedRunLumiEvents1");
...
void RunLumiEventAnalyzer::analyze(edm::Event const& event,
                                   edm::EventSetup const&) {
  if (verbose_) {
    edm::LogAbsolute("RunLumiEvent")
        << "RUN_LUMI_EVENT " << event.run() << ", " << event.luminosityBlock()
        << ", " << event.id().event();
  }
  ...
  if (dumpTriggerResults_) {
    ...
    if (triggerResults.isValid()) {
      edm::LogAbsolute("RunLumiEvent")
          << "TestFailuresAnalyzer dumping TriggerResults";
...
void RunLumiEventAnalyzer::beginRun(edm::Run const& run,
                                    edm::EventSetup const&) {
  if (verbose_) {
    edm::LogAbsolute("RunLumiEvent")
        << "RUN_LUMI_EVENT " << run.run() << ", " << 0 << ", " << 0;
  }
...
void RunLumiEventAnalyzer::beginLuminosityBlock(
    edm::LuminosityBlock const& lumi, edm::EventSetup const&) {
  if (verbose_) {
    edm::LogAbsolute("RunLumiEvent") << "RUN_LUMI_EVENT " << lumi.run() << ", "
                                     << lumi.luminosityBlock() << ", " << 0;
  }
    ...
    if (!(expectedRunLumisEvents_->at(index_) == lumi.luminosityBlock())) {
      throw cms::Exception("UnexpectedLumi")
          << "RunLumiEventAnalyzer::endLuminosityBlock unexpected lumi"
             "  expected "
          << expectedRunLumisEvents_->at(index_) << "  found    "
...
void RunLumiEventAnalyzer::endJob() {
  if (expectedEndingIndex_ != -1 && index_ != expectedEndingIndex_) {
    throw cms::Exception("UnexpectedEvent",
                         "RunLumiEventAnalyzer::endJob. Unexpected number of "
