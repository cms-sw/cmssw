...
//
/**\class TestTBBTasksAnalyzer TestTBBTasksAnalyzer.cc
...
class WaitTask : public tbb::task {
 public:
  WaitTask(unsigned int iSleepUSecs, std::atomic<unsigned int>* iCount,
           std::atomic<unsigned int>* iMaxCount)
      : m_usecondsToSleep(iSleepUSecs),
        m_count(iCount),
...
unsigned int startTasks(unsigned int iNTasks, unsigned int iSleepTime) {
  ...
  for (unsigned int i = 0; i < iNTasks; ++i) {
    tbb::task* t =
        new (sync->allocate_child()) WaitTask(iSleepTime, &count, &maxCount);
    sync->spawn(*t);
  }
  sync->wait_for_all();
  sync->destroy(*sync);
...
TestTBBTasksAnalyzer::TestTBBTasksAnalyzer(const edm::ParameterSet& iConfig)
    : m_nTasksToRun(
          iConfig.getUntrackedParameter<unsigned int>("numTasksToRun")),
      m_expectedNumberOfSimultaneousTasks(
          iConfig.getUntrackedParameter<unsigned int>("nExpectedThreads")),
      m_maxCountedTasks(0),
      m_usecondsToSleep(iConfig.getUntrackedParameter<unsigned int>(
          "usecondsToSleep", 100000)) {
...
void TestTBBTasksAnalyzer::analyze(const edm::Event&,
...
void TestTBBTasksAnalyzer::endJob() {
  if (((m_expectedNumberOfSimultaneousTasks - 1) > m_maxCountedTasks) ||
    ...
    throw cms::Exception("WrongNumberOfTasks")
        << "expected " << m_expectedNumberOfSimultaneousTasks
        << " but instead saw " << m_maxCountedTasks << "\n";
  }
}
