...
class IntVecRefVectorProducer : public edm::global::EDProducer<> {
 ...
 public:
  explicit IntVecRefVectorProducer(edm::ParameterSet const& p)
      : target_{consumes<std::vector<int>>(
            p.getParameter<edm::InputTag>("target"))},
        select_(p.getParameter<int>("select")) {
    produces<product_type>();
  }
  virtual void produce(edm::StreamID, edm::Event& e,
...
void IntVecRefVectorProducer::produce(edm::StreamID, edm::Event& e,
...
class IntVecRefToBaseVectorProducer : public edm::global::EDProducer<> {
 ...
 public:
  explicit IntVecRefToBaseVectorProducer(edm::ParameterSet const& p)
      : target_{
            consumes<edm::View<int>>(p.getParameter<edm::InputTag>("target"))} {
    produces<product_type>();
  }
  virtual void produce(edm::StreamID, edm::Event& e,
                       edm::EventSetup const& c) const override;
...
void IntVecRefToBaseVectorProducer::produce(edm::StreamID, edm::Event& e,
                                            edm::EventSetup const&) const {
  // EventSetup is not used.
  // Get our input:
...
void IntVecPtrVectorProducer::produce(edm::StreamID, edm::Event& e,
                                      edm::EventSetup const&) const {
  // EventSetup is not used.
  // Get our input:
  edm::Handle<edm::View<int>> input;
...
void IntVecStdVectorPtrProducer::produce(edm::StreamID, edm::Event& e,
                                         edm::EventSetup const&) const {
  // EventSetup is not used.
  // Get our input:
  edm::Handle<edm::View<int>> input;
