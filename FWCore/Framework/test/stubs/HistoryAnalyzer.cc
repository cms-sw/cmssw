...
HistoryAnalyzer::HistoryAnalyzer(edm::ParameterSet const& params)
    : expectedSize_(params.getParameter<int>("expectedSize")),
      ...
      expectedDroppedEndPaths_(
          params.getParameter<vstring>("expectedDroppedEndPaths")),
      expectedDroppedModules_(
          params.getParameter<vstring>("expectedDroppedModules")),
      expectedDropFromProcPSet_(
          params.getParameter<vstring>("expectedDropFromProcPSet")),
...
void HistoryAnalyzer::analyze(edm::Event const& event, edm::EventSetup const&) {
  ...
  if (eventCount_ == 0) {
    ...
    event.getProcessParameterSet(event.processHistory().rbegin()->processName(),
                                 proc_pset);
    ...
    if (!expectedPaths_.empty()) {
      ...
      for (vstring::const_iterator i = paths.begin(), iEnd = paths.end();
           i != iEnd; ++i) {
        vstring modulesOnPath =
            proc_pset.getParameter<std::vector<std::string> >(*i);
    ...
    if (!expectedEndPaths_.empty()) {
      ...
      for (vstring::const_iterator i = end_paths.begin(),
                                   iEnd = end_paths.end();
           i != iEnd; ++i) {
        vstring modulesOnEndPath = proc_pset.getParameter<vstring>(*i);
        assert(!modulesOnEndPath.empty());
        vstring expectedModulesOnEndPath =
            expectedModulesOnEndPaths_.getParameter<vstring>(*i);
    ...
    if (!expectedModules_.empty()) {
      ...
      for (vstring::const_iterator i = all_modules.begin(),
                                   iEnd = all_modules.end();
           i != iEnd; ++i) {
        // Make sure the ParameterSet for the module is also present
        edm::ParameterSet const& pset = proc_pset.getParameterSet(*i);
        // This is probably overkill, but also check it can be retrieved by ID
        // from the registry
    ...
    for (vstring::const_iterator i = expectedDroppedModules_.begin(),
      ...
      for (edm::pset::Registry::const_iterator j = reg->begin(),
                                               jEnd = reg->end();
           j != jEnd; ++j) {
