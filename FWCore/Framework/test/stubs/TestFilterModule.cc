...
class TestContextAnalyzer : public edm::global::EDAnalyzer<> {
 public:
  ...
  virtual void analyze(edm::StreamID, edm::Event const& e,
...
class SewerModule : public edm::one::OutputModule<> {
 ...
 private:
  ...
  virtual void writeLuminosityBlock(
...
void TestResultAnalyzer::analyze(edm::Event const& e, edm::EventSetup const&) {
  ...
  if (numbits != prod[0]->size()) {
    std::cerr << "TestResultAnalyzer named: " << name_ << " should have "
              << numbits << ", got " << prod[0]->size()
              << " in TriggerResults\n";
    abort();
  }
}
...
TestContextAnalyzer::TestContextAnalyzer(edm::ParameterSet const& ps)
    : expected_pathname_(ps.getUntrackedParameter<std::string>("pathname", "")),
      expected_modulelabel_(
          ps.getUntrackedParameter<std::string>("modlabel", "")) {}

void TestContextAnalyzer::analyze(edm::StreamID, edm::Event const& e,
                                  edm::EventSetup const&) const {
  assert(e.moduleCallingContext()->moduleDescription()->moduleLabel() ==
         moduleDescription().moduleLabel());

  if (!expected_pathname_.empty()) {
    assert(expected_pathname_ ==
...
void SewerModule::endJob() {
  ...
  if (total_ != num_pass_) {
    std::cerr << "number passed should be " << num_pass_ << ", but got "
              << total_ << "\n";
    abort();
  }
}
...
void SewerModule::fillDescriptions(ConfigurationDescriptions& descriptions) {
  ...
  edm::OutputModule::fillDescription(
      desc, std::vector<std::string>(1U, std::string("drop *")));
