...
//
/**\class TestFailuresAnalyzer TestFailuresAnalyzer.cc
 stubs/TestFailuresAnalyzer/src/TestFailuresAnalyzer.cc
...
TestFailuresAnalyzer::TestFailuresAnalyzer(const edm::ParameterSet& iConfig)
    : whichFailure_(iConfig.getParameter<int>("whichFailure")),
      eventToThrow_(iConfig.getUntrackedParameter<unsigned long long>(
          "eventToThrow", 2U)) {
...
void TestFailuresAnalyzer::analyze(const edm::Event& e /* iEvent */,
                                   const edm::EventSetup& /* iSetup */) {
  if (whichFailure_ == kEvent) {
    throw cms::Exception("Test") << " event";
  }
  if (whichFailure_ == kEventCorruption &&
      eventToThrow_ == e.eventAuxiliary().event()) {
