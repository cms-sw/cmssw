...
class GlobalIntFilter : public edm::stream::EDFilter<edm::GlobalCache<Cache>> {
 public:
  ...
  static std::unique_ptr<Cache> initializeGlobalCache(
...
class RunIntFilter : public edm::stream::EDFilter<edm::RunCache<Cache>> {
 public:
  ...
  static std::shared_ptr<Cache> globalBeginRun(edm::Run const& iRun,
  ...
  static void globalEndRun(edm::Run const& iRun, edm::EventSetup const&,
...
class LumiIntFilter
    : public edm::stream::EDFilter<edm::LuminosityBlockCache<Cache>> {
 public:
  static std::atomic<unsigned int> m_count;
  unsigned int trans_;
  static std::atomic<unsigned int> cvalue_;
  ...
  static std::shared_ptr<Cache> globalBeginLuminosityBlock(
      edm::LuminosityBlock const& iLB, edm::EventSetup const&,
      RunContext const*) {
    ++m_count;
    gbl = true;
    gel = false;
  ...
  static void globalEndLuminosityBlock(
    ...
    if (iLBContext->luminosityBlock()->value != cvalue_) {
      throw cms::Exception("cache value")
          ...
          << iLBContext->luminosityBlock()->value
          << " but it was supposed to be " << cvalue_;
    }
  }

  static void endLuminosityBlock(edm::Run const&, edm::EventSetup const&,
...
class RunSummaryIntFilter
    : public edm::stream::EDFilter<edm::RunCache<Cache>,
                                   edm::RunSummaryCache<Cache>> {
 public:
  static std::atomic<unsigned int> m_count;
  unsigned int trans_;
  static std::atomic<unsigned int> cvalue_;
  ...
  static std::shared_ptr<Cache> globalBeginRunSummary(edm::Run const&,
                                                      edm::EventSetup const&,
                                                      GlobalCache const*) {
    ++m_count;
    gbrs = true;
  ...
  void endRunSummary(edm::Run const&, edm::EventSetup const&,
  ...
  static void globalEndRunSummary(edm::Run const&, edm::EventSetup const&,
                                  RunContext const*, Cache* gCache) {
    ++m_count;
    gbrs = false;
    gers = true;
    if (!ers) {
...
class LumiSummaryIntFilter
    : public edm::stream::EDFilter<edm::LuminosityBlockCache<Cache>,
                                   edm::LuminosityBlockSummaryCache<Cache>> {
 public:
  static std::atomic<unsigned int> m_count;
  unsigned int trans_;
  static std::atomic<unsigned int> cvalue_;
  ...
  static std::shared_ptr<Cache> globalBeginLuminosityBlockSummary(
      edm::LuminosityBlock const&, edm::EventSetup const&,
      LuminosityBlockContext const*) {
    ++m_count;
    gbls = true;
    ...
    if (!gbl) {
      throw cms::Exception("begin out of sequence")
          << "globalBeginLuminosityBlockSummary seen before "
  ...
  static void globalEndLuminosityBlockSummary(edm::LuminosityBlock const&,
    ...
    if (!els) {
      throw cms::Exception("end out of sequence")
          ...
          << "globalEndLuminosityBlockSummary seen before "
  ...
  static void globalEndLuminosityBlock(
    ...
    gbl = false;
    if (!gels) {
      throw cms::Exception("end out of sequence") << "globalEndLuminosityBlockS"
                                                     "ummary seen before "
...
class TestBeginRunFilter : public edm::stream::EDFilter<edm::RunCache<Cache>,
 ...
 public:
  ...
  static void globalBeginRunProduce(edm::Run& iRun, edm::EventSetup const&,
                                    RunContext const*) {
    ++m_count;
    if (!gbr) {
      throw cms::Exception("begin out of sequence")
...
class TestEndRunFilter
    : public edm::stream::EDFilter<edm::RunCache<Cache>, edm::EndRunProducer> {
 public:
  static std::atomic<unsigned int> m_count;
  unsigned int trans_;
  static std::atomic<unsigned int> cvalue_;
  ...
  static void globalEndRunProduce(edm::Run& iRun, edm::EventSetup const&,
                                  RunContext const*) {
    ++m_count;
    if (ger) {
      throw cms::Exception("end out of sequence")
