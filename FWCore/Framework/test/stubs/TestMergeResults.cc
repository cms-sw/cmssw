...
class TestMergeResults : public edm::EDAnalyzer {
 ...
 private:
  ...
  void checkExpectedRunProducts(unsigned int index,
                                std::vector<int> const& expectedValues,
                                edm::InputTag const& tag,
                                char const* functionName, edm::Run const& run);

  void abortWithMessage(char const* whichFunction, char const* type,
                        edm::InputTag const& tag, int expectedValue,
...
TestMergeResults::TestMergeResults(edm::ParameterSet const& ps)
    : default_(),
      defaultvstring_(),
      expectedBeginRunProd_(ps.getUntrackedParameter<std::vector<int> >(
          "expectedBeginRunProd", default_)),
      expectedEndRunProd_(ps.getUntrackedParameter<std::vector<int> >(
          "expectedEndRunProd", default_)),
      expectedBeginLumiProd_(ps.getUntrackedParameter<std::vector<int> >(
          "expectedBeginLumiProd", default_)),
      expectedEndLumiProd_(ps.getUntrackedParameter<std::vector<int> >(
          "expectedEndLumiProd", default_)),

      expectedBeginRunNew_(ps.getUntrackedParameter<std::vector<int> >(
          "expectedBeginRunNew", default_)),
      expectedEndRunNew_(ps.getUntrackedParameter<std::vector<int> >(
          "expectedEndRunNew", default_)),
      expectedBeginLumiNew_(ps.getUntrackedParameter<std::vector<int> >(
          "expectedBeginLumiNew", default_)),
      expectedEndLumiNew_(ps.getUntrackedParameter<std::vector<int> >(
          "expectedEndLumiNew", default_)),

      expectedParents_(ps.getUntrackedParameter<std::vector<std::string> >(
          "expectedParents", defaultvstring_)),
      expectedProcessHistoryInRuns_(
          ps.getUntrackedParameter<std::vector<std::string> >(
              "expectedProcessHistoryInRuns", defaultvstring_)),

      expectedDroppedEvent_(ps.getUntrackedParameter<std::vector<int> >(
          "expectedDroppedEvent", default_)),
      expectedDroppedEvent1_(ps.getUntrackedParameter<std::vector<int> >(
          "expectedDroppedEvent1", default_)),

      nRespondToOpenInputFile_(0),
      nRespondToCloseInputFile_(0),
      expectedRespondToOpenInputFile_(
          ps.getUntrackedParameter<int>("expectedRespondToOpenInputFile", -1)),
      expectedRespondToCloseInputFile_(
          ps.getUntrackedParameter<int>("expectedRespondToCloseInputFile", -1)),

      expectedInputFileNames_(
          ps.getUntrackedParameter<std::vector<std::string> >(
              "expectedInputFileNames", defaultvstring_)),
  ...
  edm::Wrapper<edmtest::ThingWithMerge> w_thingWithMerge(
      std::move(ap_thingwithmerge));
  ...
  edm::Wrapper<edmtest::ThingWithIsEqual> w_thingWithIsEqual(
      std::move(ap_thingwithisequal));
  assert(!w_thingWithIsEqual.isMergeable());
  assert(w_thingWithIsEqual.hasIsProductEqual());

  if (expectedDroppedEvent_.size() > 0) {
    consumes<edmtest::ThingWithIsEqual>(
        edm::InputTag{"makeThingToBeDropped", "event", "PROD"});
    consumes<edmtest::ThingWithMerge>(
        edm::InputTag{"makeThingToBeDropped", "event", "PROD"});

    consumes<edmtest::ThingWithIsEqual, edm::InRun>(
        edm::InputTag{"makeThingToBeDropped", "beginRun", "PROD"});
  }
  ...
  if (expectedDroppedEvent1_.size() > droppedIndex1_) {
    consumes<edmtest::ThingWithIsEqual>(
        edm::InputTag{"makeThingToBeDropped1", "event", "PROD"});
  }
  consumes<edmtest::Thing>(
      edm::InputTag{"thingWithMergeProducer", "event", "PROD"});

  if (testAlias_) {
    consumes<edmtest::Thing>(
        edm::InputTag{"aliasForThingToBeDropped2", "instance2"});
    consumes<edmtest::Thing>(
        edm::InputTag{"aliasForThingToBeDropped2", "instance2", "PROD"});
  }
  ...
  if (testAlias_) {
    consumes<edmtest::Thing, edm::InRun>(
        edm::InputTag{"aliasForThingToBeDropped2", "endRun2"});
  ...
  if (testAlias_) {
    consumes<edmtest::Thing, edm::InLumi>(
        edm::InputTag{"aliasForThingToBeDropped2", "endLumi2"});
...
void TestMergeResults::analyze(edm::Event const& e, edm::EventSetup const&) {
  ...
  if (parentIndex_ < expectedParents_.size()) {
    ...
    edm::BranchID actualParentBranchID =
        h_thing.provenance()->productProvenance()->parentage().parents()[0];
    ...
    edm::InputTag tagparent(prov.moduleLabel(), prov.productInstanceName(),
                            prov.processName());
  ...
  if (testAlias_) {
    ...
    edm::BranchID const& originalBranchID =
        h_thing.provenance()->branchDescription().originalBranchID();
...
void TestMergeResults::endRun(edm::Run const& run, edm::EventSetup const&) {
  ...
  for (edm::ProcessHistory::const_iterator iter = ph.begin(), iEnd = ph.end();
    ...
    if (processHistoryIndex_ < expectedProcessHistoryInRuns_.size()) {
      assert(expectedProcessHistoryInRuns_[processHistoryIndex_] ==
             iter->processName());
...
void TestMergeResults::beginLuminosityBlock(edm::LuminosityBlock const& lumi,
                                            edm::EventSetup const&) {
  index2_ += 3;
  index6_ += 3;
  index3_ += 3;
...
void TestMergeResults::endLuminosityBlock(edm::LuminosityBlock const& lumi,
                                          edm::EventSetup const&) {
  ...
  edm::InputTag tag("thingWithMergeProducer", "endLumi", "PROD");
  checkExpectedLumiProducts(index3_, expectedEndLumiProd_, tag, "endLumi",
                            lumi);

  edm::InputTag tagnew("thingWithMergeProducer", "endLumi");
  checkExpectedLumiProducts(index7_, expectedEndLumiNew_, tagnew, "endLumi",
                            lumi);
...
void TestMergeResults::respondToOpenInputFile(edm::FileBlock const& fb) {
  ...
  if (!expectedInputFileNames_.empty()) {
    if (expectedInputFileNames_.size() <=
            static_cast<unsigned>(nRespondToOpenInputFile_) ||
        expectedInputFileNames_[nRespondToOpenInputFile_] != fb.fileName()) {
      std::cerr << "Error while testing merging of run/lumi products in "
                   "TestMergeResults.cc\n"
                << "Unexpected input filename, expected name = "
                << expectedInputFileNames_[nRespondToOpenInputFile_]
...
void TestMergeResults::endJob() {
  ...
  if (expectedRespondToOpenInputFile_ > -1 &&
      nRespondToOpenInputFile_ != expectedRespondToOpenInputFile_) {
    std::cerr
        << "Error while testing merging of run/lumi products in "
           "TestMergeResults.cc\n"
        << "Unexpected number of calls to the function respondToOpenInputFile"
        << std::endl;
    abort();
  }

  if (expectedRespondToCloseInputFile_ > -1 &&
      nRespondToCloseInputFile_ != expectedRespondToCloseInputFile_) {
    std::cerr
        << "Error while testing merging of run/lumi products in "
           "TestMergeResults.cc\n"
        << "Unexpected number of calls to the function respondToCloseInputFile"
        << std::endl;
    abort();
  }
}

void TestMergeResults::checkExpectedRunProducts(
    unsigned int index, std::vector<int> const& expectedValues,
    edm::InputTag const& tag, char const* functionName, edm::Run const& run) {
  if ((index + 2) < expectedValues.size()) {
    ...
    if (expected != 0) {
      ...
      if (h_thingWithMerge->a != expected) {
        abortWithMessage(functionName, "ThingWithMerge", tag, expected,
                         h_thingWithMerge->a);
      }
    }
    ...
    if (expected != 0) {
      ...
      if (h_thingWithIsEqual->a != expected) {
        abortWithMessage(functionName, "ThingWithIsEqual", tag, expected,
                         h_thingWithIsEqual->a);
      }
    }
  }
}

void TestMergeResults::checkExpectedLumiProducts(
    unsigned int index, std::vector<int> const& expectedValues,
    edm::InputTag const& tag, char const* functionName,
    edm::LuminosityBlock const& lumi) {
  if ((index + 2) < expectedValues.size()) {
    int expected = expectedValues[index];
    if (expected != 0) {
...
void TestMergeResults::abortWithMessage(char const* whichFunction,
  ...
  std::cerr << "Error while testing merging of run/lumi products in "
               "TestMergeResults.cc\n"
            << "In function " << whichFunction
            << " looking for product of type " << type << "\n"
            << tag << "\n"
