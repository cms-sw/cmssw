...
//
/**\class TestESDummyDataAnalyzer TestESDummyDataAnalyzer.cc
 FWCore/Framework/test/stubs/TestESDummyDataAnalyzer.cc
...
void TestESDummyDataAnalyzer::analyze(const edm::Event&,
  ...
  //   std::cout<<"after "<<m_expectedValue<<" pData
  //   "<<pData->value_<<std::endl;

  if (m_expectedValue != pData->value_) {
    throw cms::Exception("WrongValue")
        << "got value " << pData->value_ << " but expected " << m_expectedValue;
  }
}
...
void TestESDummyDataAnalyzer::endJob() {
  if (-1 != m_totalNEvents && m_totalNEvents != m_totalCounter) {
    throw cms::Exception("WrongNumberOfEvents")
        << "expected " << m_totalNEvents << " but instead saw "
        << m_totalCounter << "\n";
  }
}
