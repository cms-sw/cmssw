  ...
  ~SharedResourcesProducer() noexcept(false) {
    if (m_count != trans_) {
      throw cms::Exception("transitions")
          << "SharedResourcesProducer transitions " << m_count
...
class WatchRunsProducer : public edm::one::EDProducer<edm::one::WatchRuns> {
 public:
  ...
  ~WatchRunsProducer() noexcept(false) {
    if (m_count != trans_) {
      throw cms::Exception("transitions")
          << "WatchRunsProducer transitions " << m_count
          << " but it was supposed to be " << trans_;
    }
  }
};

class WatchLumiBlocksProducer
    : public edm::one::EDProducer<edm::one::WatchLuminosityBlocks> {
 public:
  ...
  ~WatchLumiBlocksProducer() noexcept(false) {
    if (m_count != trans_) {
      throw cms::Exception("transitions")
          << "WatchLumiBlockProducer transitions " << m_count
          << " but it was supposed to be " << trans_;
    }
  }
};

class TestBeginRunProducer
    : public edm::one::EDProducer<edm::one::WatchRuns, edm::BeginRunProducer> {
 public:
  ...
  ~TestBeginRunProducer() noexcept(false) {
    if (m_count != trans_) {
      throw cms::Exception("transitions")
          << "TestBeginRunProducer transitions " << m_count
          << " but it was supposed to be " << trans_;
    }
  }
};
...
class TestBeginLumiBlockProducer
 ...
 public:
  ...
  void beginLuminosityBlockProduce(edm::LuminosityBlock&,
                                   edm::EventSetup const&) override {
    ++m_count;
    if (p) {
      throw cms::Exception("out of sequence")
  ...
  ~TestBeginLumiBlockProducer() noexcept(false) {
    if (m_count != trans_) {
      throw cms::Exception("transitions")
          << "TestBeginLumiBlockProducer transitions " << m_count
          << " but it was supposed to be " << trans_;
    }
  }
};

class TestEndRunProducer
 ...
 public:
  ...
  ~TestEndRunProducer() noexcept(false) {
    if (m_count != trans_) {
      throw cms::Exception("transitions")
          << "TestEndRunProducer transitions " << m_count
          << " but it was supposed to be " << trans_;
    }
  }
};
...
class TestEndLumiBlockProducer
 ...
 public:
  ...
  ~TestEndLumiBlockProducer() noexcept(false) {
    if (m_count != trans_) {
      throw cms::Exception("transitions")
          << "TestEndLumiBlockProducer transitions " << m_count
          << " but it was supposed to be " << trans_;
    }
  }
};
