...
class StreamIntAnalyzer
    : public edm::global::EDAnalyzer<edm::StreamCache<UnsafeCache>> {
 public:
  ...
  void streamBeginRun(edm::StreamID iID, edm::Run const&,
    ...
    if ((streamCache(iID))->value != iID.value()) {
      throw cms::Exception("cache value")
          << "StreamIntAnalyzer cache value " << (streamCache(iID))->value
          << " but it was supposed to be " << iID;
    }
  }
  ...
  void analyze(edm::StreamID iID, const edm::Event&,
  ...
  ~StreamIntAnalyzer() {
    if (m_count != trans_) {
      throw cms::Exception("transitions")
          << "StreamIntAnalyzer transitions " << m_count
...
class RunIntAnalyzer : public edm::global::EDAnalyzer<edm::RunCache<Cache>> {
 public:
  ...
  void analyze(edm::StreamID iID, const edm::Event& iEvent,
               const edm::EventSetup&) const override {
    ++m_count;
  ...
  void globalEndRun(edm::Run const& iRun,
    ...
    if ((runCache(iRun.index()))->value != cvalue_) {
      throw cms::Exception("cache value")
          << "RunIntAnalyzer cache value " << (runCache(iRun.index()))->value
  ...
  ~RunIntAnalyzer() {
    if (m_count != trans_) {
      throw cms::Exception("transitions")
          << "RunIntAnalyzer transitions " << m_count
          << " but it was supposed to be " << trans_;
    }
  }
};
...
class LumiIntAnalyzer
    : public edm::global::EDAnalyzer<edm::LuminosityBlockCache<Cache>> {
 public:
  ...
  void analyze(edm::StreamID, const edm::Event& iEvent,
               const edm::EventSetup&) const override {
    ++m_count;
  ...
  void globalEndLuminosityBlock(edm::LuminosityBlock const& iLB,
    ...
    if ((luminosityBlockCache(iLB.index()))->value != cvalue_) {
      throw cms::Exception("cache value")
          ...
          << (luminosityBlockCache(iLB.index()))->value
          << " but it was supposed to be " << cvalue_;
    }
  }
  ...
  ~LumiIntAnalyzer() {
    if (m_count != trans_) {
      throw cms::Exception("transitions")
          << "LumiIntAnalyzer transitions " << m_count
          << " but it was supposed to be " << trans_;
    }
  }
};
 ...
 public:
  ...
  void globalEndRunSummary(edm::Run const&, edm::EventSetup const&,
    ...
    if (gCache->value != cvalue_) {
      throw cms::Exception("cache value")
          << "RunSummaryIntAnalyzer cache value " << gCache->value
          << " but it was supposed to be " << cvalue_;
    }
  }
  ...
  ~RunSummaryIntAnalyzer() {
    if (m_count != trans_) {
      throw cms::Exception("transitions")
          << "RunSummaryIntAnalyzer transitions " << m_count
          << " but it was supposed to be " << trans_;
    }
  }
};
...
class LumiSummaryIntAnalyzer
 ...
 public:
  ...
  void globalEndLuminosityBlockSummary(edm::LuminosityBlock const&,
    ...
    if (gCache->value != cvalue_) {
      throw cms::Exception("cache value")
          << "LumiSummaryIntAnalyzer cache value " << gCache->value
          << " but it was supposed to be " << cvalue_;
    }
  }
  ...
  ~LumiSummaryIntAnalyzer() {
    if (m_count != trans_) {
      throw cms::Exception("transitions")
          << "LumiSummaryIntAnalyzer transitions " << m_count
          << " but it was supposed to be " << trans_;
    }
  }
};
