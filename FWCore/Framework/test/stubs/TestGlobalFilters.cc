...
class StreamIntFilter
    : public edm::global::EDFilter<edm::StreamCache<UnsafeCache>> {
 public:
  ...
  void streamBeginRun(edm::StreamID iID, edm::Run const&,
    ...
    if (sCache->run != 0 || sCache->lumi != 0 || sCache->work != 0 ||
        sCache->strm != 1) {
  ...
  void streamBeginLuminosityBlock(edm::StreamID iID,
    ...
    if (sCache->lumi != 0 || sCache->work != 0) {
      throw cms::Exception("out of sequence")
          << "streamBeginLuminosityBlock out of sequence in Stream "
          << iID.value();
    }
    ++(sCache->lumi);
  }

  bool filter(edm::StreamID iID, edm::Event&,
              edm::EventSetup const&) const override {
    ++m_count;
    auto sCache = streamCache(iID);
    if (sCache->value != iID.value()) {
  ...
  void streamEndLuminosityBlock(edm::StreamID iID, edm::LuminosityBlock const&,
                                edm::EventSetup const&) const override {
    ++m_count;
    auto sCache = streamCache(iID);
    if (sCache->value != iID.value()) {
    ...
    if (sCache->lumi != 0 || sCache->run == 0) {
      throw cms::Exception("out of sequence")
          << "streamEndLuminosityBlock out of sequence in Stream "
          << iID.value();
    }
  }

  void streamEndRun(edm::StreamID iID, edm::Run const&,
                    edm::EventSetup const&) const override {
    ++m_count;
    auto sCache = streamCache(iID);
    if (sCache->value != iID.value()) {
  ...
  ~StreamIntFilter() {
    if (m_count != trans_) {
      throw cms::Exception("transitions")
          << "StreamIntFilter transitions " << m_count
          << " but it was supposed to be " << trans_;
    }
  }
};

class RunIntFilter : public edm::global::EDFilter<edm::StreamCache<UnsafeCache>,
                                                  edm::RunCache<Cache>> {
 public:
  ...
  std::shared_ptr<Cache> globalBeginRun(edm::Run const&,
                                        edm::EventSetup const&) const override {
    ++m_count;
  ...
  void streamBeginRun(edm::StreamID iID, edm::Run const& iRun,
                      edm::EventSetup const&) const override {
  ...
  bool filter(edm::StreamID, edm::Event& iEvent,
              edm::EventSetup const&) const override {
    ++m_count;
  ...
  void streamEndRun(edm::StreamID iID, edm::Run const& iRun,
                    edm::EventSetup const&) const override {
    auto rCache = runCache(iRun.index());
    if (rCache->run == 0) {
      throw cms::Exception("out of sequence")
  ...
  void globalEndRun(edm::Run const& iRun,
                    edm::EventSetup const&) const override {
    ++m_count;
    auto rCache = runCache(iRun.index());
    if (rCache->value != cvalue_) {
      throw cms::Exception("cache value")
          << "RunIntFilter cache value " << rCache->value
  ...
  ~RunIntFilter() {
    if (m_count != trans_) {
      throw cms::Exception("transitions")
          << "RunIntFilter transitions " << m_count
          << " but it was supposed to be " << trans_;
    }
  }
};
...
class LumiIntFilter
 ...
 public:
  ...
  std::shared_ptr<Cache> globalBeginLuminosityBlock(
  ...
  void streamBeginLuminosityBlock(edm::StreamID iID,
    ...
    if (lCache->lumi == 0) {
      throw cms::Exception("out of sequence")
          << "streamBeginLuminosityBlock seen before "
             "globalBeginLuminosityBlock in LuminosityBlock"
          << iLB.luminosityBlock();
    }
  }
  ...
  void streamEndLuminosityBlock(edm::StreamID iID,
    ...
    if (lCache->lumi == 0) {
      throw cms::Exception("out of sequence")
          << "streamEndLuminosityBlock seen before globalEndLuminosityBlock in "
             "LuminosityBlock"
          << iLB.luminosityBlock();
    }
  }

  void globalEndLuminosityBlock(edm::LuminosityBlock const& iLB,
                                edm::EventSetup const&) const override {
    ++m_count;
    auto lCache = luminosityBlockCache(iLB.index());
    --(lCache->lumi);
    if (lCache->lumi != 0) {
      throw cms::Exception("end out of sequence")
          << "globalEndLuminosityBlock seen before globalBeginLuminosityBlock "
             "in LuminosityBlock"
          << iLB.luminosityBlock();
    }
    if (lCache->value != cvalue_) {
      throw cms::Exception("cache value")
          << "LumiIntFilter cache value " << lCache->value
          << " but it was supposed to be " << cvalue_;
    }
  }
  ...
  ~LumiIntFilter() {
    if (m_count != trans_) {
      throw cms::Exception("transitions")
          << "LumiIntFilter transitions " << m_count
          << " but it was supposed to be " << trans_;
    }
  }
};
...
class RunSummaryIntFilter
    : public edm::global::EDFilter<edm::StreamCache<Cache>,
                                   edm::RunSummaryCache<UnsafeCache>> {
 public:
  ...
  std::shared_ptr<UnsafeCache> globalBeginRunSummary(
  ...
  void streamEndRunSummary(edm::StreamID iID, edm::Run const&,
                           edm::EventSetup const&,
                           UnsafeCache* gCache) const override {
    ++m_count;
    if (gCache->run == 0) {
      throw cms::Exception("out of sequence")
          << "streamEndRunSummary after globalEndRunSummary in Stream "
          << iID.value();
    }
    auto sCache = streamCache(iID);
  ...
  void globalEndRunSummary(edm::Run const&, edm::EventSetup const&,
    ...
    if (gCache->value != cvalue_) {
      throw cms::Exception("cache value")
          << "RunSummaryIntFilter cache value " << gCache->value
          << " but it was supposed to be " << cvalue_;
    }
    --(gCache->run);
  }
  ...
  ~RunSummaryIntFilter() {
    if (m_count != trans_) {
      throw cms::Exception("transitions")
          << "RunSummaryIntFilter transitions " << m_count
          << " but it was supposed to be " << trans_;
    }
  }
};
...
class LumiSummaryIntFilter
 ...
 public:
  ...
  std::shared_ptr<UnsafeCache> globalBeginLuminosityBlockSummary(
      edm::LuminosityBlock const&, edm::EventSetup const&) const override {
    ++m_count;
    auto gCache = std::make_shared<UnsafeCache>();
    ++(gCache->lumi);
  ...
  void streamEndLuminosityBlockSummary(edm::StreamID iID,
    ...
    if (gCache->lumi == 0) {
      throw cms::Exception("out of sequence")
          << "streamEndLuminosityBlockSummary after "
             "globalEndLuminosityBlockSummary in Stream "
          << iID.value();
    }
    auto sCache = streamCache(iID);
    gCache->value += sCache->value;
  ...
  void globalEndLuminosityBlockSummary(edm::LuminosityBlock const&,
    ...
    if (gCache->value != cvalue_) {
      throw cms::Exception("cache value")
          << "LumiSummaryIntFilter cache value " << gCache->value
          << " but it was supposed to be " << cvalue_;
    }
    --(gCache->lumi);
  }
  ...
  ~LumiSummaryIntFilter() {
    if (m_count != trans_) {
      throw cms::Exception("transitions")
          << "LumiSummaryIntFilter transitions " << m_count
          << " but it was supposed to be " << trans_;
    }
  }
};

class TestBeginRunFilter : public edm::global::EDFilter<edm::RunCache<Dummy>,
 ...
 public:
  ...
  std::shared_ptr<Dummy> globalBeginRun(edm::Run const& iRun,
                                        edm::EventSetup const&) const override {
    brp = false;
  ...
  ~TestBeginRunFilter() {
    if (m_count != trans_) {
      throw cms::Exception("transitions")
          << "TestBeginRunFilter transitions " << m_count
          << " but it was supposed to be " << trans_;
    }
  }
};

class TestEndRunFilter
    : public edm::global::EDFilter<edm::RunCache<Dummy>, edm::EndRunProducer> {
 public:
  ...
  bool filter(edm::StreamID, edm::Event&,
              edm::EventSetup const&) const override {
    p = true;
    return true;
  }
  ...
  ~TestEndRunFilter() {
    if (m_count != trans_) {
      throw cms::Exception("transitions")
          << "TestEndRunFilter transitions " << m_count
          << " but it was supposed to be " << trans_;
    }
  }
};
...
class TestBeginLumiBlockFilter
    : public edm::global::EDFilter<edm::LuminosityBlockCache<Dummy>,
 ...
 public:
  ...
  std::shared_ptr<Dummy> globalBeginLuminosityBlock(
      edm::LuminosityBlock const& iLB, edm::EventSetup const&) const override {
    gblp = false;
    return std::shared_ptr<Dummy>();
  }
  ...
  bool filter(edm::StreamID iID, edm::Event&,
    ...
    if (!gblp) {
      throw cms::Exception("out of sequence")
          << "filter before globalBeginLuminosityBlockProduce in Stream "
          << iID.value();
    }
    return true;
  }
  ...
  ~TestBeginLumiBlockFilter() {
    if (m_count != trans_) {
      throw cms::Exception("transitions")
          << "TestBeginLumiBlockFilter transitions " << m_count
          << " but it was supposed to be " << trans_;
    }
  }
};
...
class TestEndLumiBlockFilter
 ...
 public:
  ...
  ~TestEndLumiBlockFilter() {
    if (m_count != trans_) {
      throw cms::Exception("transitions")
          << "TestEndLumiBlockFilter transitions " << m_count
          << " but it was supposed to be " << trans_;
    }
  }
};
