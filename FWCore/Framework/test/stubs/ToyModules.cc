...
void SCSimpleProducer::produce(edm::Event& e,
...
void OVSimpleProducer::produce(edm::Event& e,
                               edm::EventSetup const& /* unused */) {
...
void VSimpleProducer::produce(edm::Event& e,
                              edm::EventSetup const& /* unused */) {
  // Fill up a collection
...
void AVSimpleProducer::produce(edm::Event& e,
                               edm::EventSetup const& /* unused */) {
  edm::Handle<std::vector<edmtest::Simple>> vs;
  e.getByLabel(src_, vs);
  // Fill up a collection
  auto p = std::make_unique<AVSimpleProduct>(
      edm::RefProd<std::vector<edmtest::Simple>>(vs));
...
void DSTVProducer::fill_a_data(DSTVSimpleDerivedProduct::data_type& d,
                               unsigned int i) {
  d.key = size_ - i;
  d.value = 1.5 * i;
}

void DSTVProducer::fill_a_data(DSTVSimpleProduct::data_type& d,
                               unsigned int i) {
  d.data = size_ - i;
}
...
void DSTVProducer::make_a_product(edm::Event& e) {
  ...
  for (id_type id = 1; id < static_cast<id_type>(size_); ++id) {
    ...
    item.resize(n);
    for (unsigned int i = 0; i < n; ++i) fill_a_data(item[i], i);
  }
