...
TestTriggerNames::TestTriggerNames(edm::ParameterSet const& ps)
    : iEvent_(0U),
      expected_trigger_paths_(
          ps.getUntrackedParameter<Strings>("trigPaths", Strings())),
      expected_trigger_previous_(
          ps.getUntrackedParameter<Strings>("trigPathsPrevious", Strings())),
      expected_end_paths_(
          ps.getUntrackedParameter<Strings>("endPaths", Strings())),
      streamerSource_(ps.getUntrackedParameter<bool>("streamerSource", false)),
      dumpPSetRegistry_(
          ps.getUntrackedParameter<bool>("dumpPSetRegistry", false)),
      expectedTriggerResultsHLT_(
          ps.getUntrackedParameter<std::vector<unsigned int> >(
              "expectedTriggerResultsHLT", std::vector<unsigned int>())),
      expectedTriggerResultsPROD_(
          ps.getUntrackedParameter<std::vector<unsigned int> >(
              "expectedTriggerResultsPROD", std::vector<unsigned int>())) {
...
void TestTriggerNames::analyze(edm::Event const& e, edm::EventSetup const&) {
  ...
  if (expected_trigger_paths_.size() > 0) {
    ...
    if (triggernames.size() != expected_trigger_paths_.size()) {
      throw cms::Exception("Test Failure")
          << "TestTriggerNames: "
          << "Expected and actual trigger path list not the same size"
          << std::endl;
    }
  ...
  if (expected_trigger_previous_.size() > 0) {
    ...
    edm::Service<edm::service::TriggerNamesService> tns;
    auto index = 0U;
    while ((index < prod.size()) and
           (moduleDescription().processName() ==
            prod[index].provenance()->processName())) {
      ++index;
    }
    if (tns->getTrigPaths(*prod[index], triggernames)) {
      if (triggernames.size() != expected_trigger_previous_.size()) {
        ...
        throw cms::Exception("Test Failure")
            << "TestTriggerNames: "
            << "Expected and actual previous trigger path lists not the same "
               "size"
            << "\n expected: " << et << "\n actual: " << tn << std::endl;
      }
      for (Strings::size_type i = 0; i < expected_trigger_previous_.size();
           ++i) {
      ...
      throw cms::Exception("Test Failure")
          << "TestTriggerNames: "
          << "Failed finding trigger names from a previous process"
          << std::endl;
    }
    ...
    if (!streamerSource_) {
      ...
      if (trigpaths.size() != expected_trigger_previous_.size()) {
        throw cms::Exception("Test Failure")
            ...
            << "Expected and actual previous trigger path not the same size"
            << std::endl;
      }
    }
    ...
    for (Strings::size_type i = 0; i < expected_trigger_previous_.size(); ++i) {
      ...
      if (triggerNamesFromEvent.triggerName(i) !=
          expected_trigger_previous_[i]) {
        throw cms::Exception("Test Failure")
            << "TestTriggerNames: While exercising TriggerNames class\n"
      ...
      if (i !=
          triggerNamesFromEvent.triggerIndex(expected_trigger_previous_[i])) {
        throw cms::Exception("Test Failure")
            << "TestTriggerNames: While exercising TriggerNames class\n"
    ...
    // This causes it to find the results in the map lookup in the TEST
    // configuration
    // and exercises that execution path in the code.
    // If you follow the execution in the debugger in EventBase::triggerNames_
    // in EventBase.cc
  ...
  if (expectedTriggerResultsHLT_.size() > 0) {
    if (e.getByLabel(tag, hTriggerResults)) {
      edm::TriggerResultsByName resultsByNameHLT =
          e.triggerResultsByName(*hTriggerResults);

      if (hTriggerResults->parameterSetID() !=
              resultsByNameHLT.parameterSetID() ||
          ...
          hTriggerResults->at(0).state() !=
              resultsByNameHLT.at("p01").state() ||
          hTriggerResults->at(0).index() !=
              resultsByNameHLT.at("p01").index() ||
      ...
      if (names.triggerNames() != resultsByNameHLT.triggerNames() ||
        ...
        throw cms::Exception("Test Failure")
            << "TestTriggerNames: While testing TriggerResultsByName class\n"
            << "TriggerNames values do not match TriggerResultsByName values"
            << std::endl;
      }
    }
  }
  ...
  if (expectedTriggerResultsHLT_.size() > iEvent_) {
    ...
    if (resultsByNameHLT.accept("p02") !=
        (expectedTriggerResultsHLT_[iEvent_] == 1)) {
      throw cms::Exception("Test Failure")
          << "TestTriggerNames: While testing TriggerResultsByName class\n"
          << "Expected and actual HLT trigger results don't match" << std::endl;
    }
    edm::LogAbsolute("TEST") << "Event " << iEvent_ << "  "
                             << resultsByNameHLT.accept("p02") << std::endl;
  }
  ...
  if (expectedTriggerResultsPROD_.size() > iEvent_) {
    ...
    edm::TriggerResultsByName resultsByNamePROD =
        e.triggerResultsByName(*hTriggerResultsPROD);
    if (!resultsByNamePROD.isValid()) {
      throw cms::Exception("Test Failure")
          << "TestTriggerNames: While testing TriggerResultsByName class\n"
          << "Invalid object for PROD" << std::endl;
    }
    if (resultsByNamePROD.accept("p1") !=
        (expectedTriggerResultsPROD_[iEvent_] == 1)) {
      throw cms::Exception("Test Failure")
          << "TestTriggerNames: While testing TriggerResultsByName class\n"
          << "Expected and actual PROD trigger results don't match"
          << std::endl;
    }
    edm::LogAbsolute("TEST") << "Event " << iEvent_ << "  "
                             << resultsByNamePROD.accept("p1") << std::endl;
  }
  ++iEvent_;
}
