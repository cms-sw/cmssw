...
namespace {
  template <typename T>
  void testTransition(std::shared_ptr<T> iMod, edm::Worker* iWorker, testLimitedModule::Trans iTrans,
                      testLimitedModule::Expectations const& iExpect, std::function<void(edm::Worker*)> iFunc) {
