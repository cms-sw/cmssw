...
void TestEventSetupsController::esProducerGetAndPutTest() {
  ...
  std::multimap<edm::ParameterSetID, edm::eventsetup::ESProducerInfo> const&
      esproducers = esController.esproducers();
  ...
  esController.lookForMatches(pset4.id(), 0, 0, firstProcessWithThisPSet,
                              precedingHasMatchingPSet);
  CPPUNIT_ASSERT(firstProcessWithThisPSet == true);
  CPPUNIT_ASSERT(precedingHasMatchingPSet == false);

  esController.lookForMatches(pset4.id(), 4, 0, firstProcessWithThisPSet,
                              precedingHasMatchingPSet);
  CPPUNIT_ASSERT(firstProcessWithThisPSet == false);
  CPPUNIT_ASSERT(precedingHasMatchingPSet == true);

  esController.lookForMatches(pset4.id(), 4, 1, firstProcessWithThisPSet,
                              precedingHasMatchingPSet);
  CPPUNIT_ASSERT(firstProcessWithThisPSet == false);
  CPPUNIT_ASSERT(precedingHasMatchingPSet == false);

  CPPUNIT_ASSERT_THROW(
      esController.lookForMatches(pset4.id(), 6, 0, firstProcessWithThisPSet,
                                  precedingHasMatchingPSet),
      cms::Exception);

  CPPUNIT_ASSERT(esController.isFirstMatch(pset4.id(), 5, 0));
  CPPUNIT_ASSERT(!esController.isFirstMatch(pset4.id(), 5, 4));
  CPPUNIT_ASSERT_THROW(esController.isFirstMatch(pset4.id(), 6, 4),
                       cms::Exception);
  CPPUNIT_ASSERT_THROW(esController.isFirstMatch(pset4.id(), 5, 1),
                       cms::Exception);

  CPPUNIT_ASSERT(!esController.isLastMatch(pset4.id(), 5, 0));
  CPPUNIT_ASSERT(esController.isLastMatch(pset4.id(), 5, 4));
  CPPUNIT_ASSERT_THROW(esController.isLastMatch(pset4.id(), 6, 4),
                       cms::Exception);
  CPPUNIT_ASSERT_THROW(esController.isLastMatch(pset4.id(), 5, 1),
                       cms::Exception);
  ...
  CPPUNIT_ASSERT_THROW(esController.isMatchingESProducer(pset4.id(), 6, 4),
                       cms::Exception);
  ...
  CPPUNIT_ASSERT_THROW(esController.getESProducerPSet(pset4.id(), 6),
                       cms::Exception);
...
void TestEventSetupsController::esSourceGetAndPutTest() {
  ...
  std::multimap<edm::ParameterSetID, edm::eventsetup::ESSourceInfo> const&
      essources = esController.essources();
  bool isPresent1 = false;
  bool isPresent2 = false;
  bool isPresent3 = false;
  bool isPresent4 = false;
  ...
  CPPUNIT_ASSERT_THROW(esController.isMatchingESSource(pset4.id(), 6, 4),
                       cms::Exception);
