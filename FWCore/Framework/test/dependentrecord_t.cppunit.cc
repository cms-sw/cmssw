...
class DummyProxyProvider : public edm::eventsetup::DataProxyProvider {
 ...
 protected:
  void registerProxies(const edm::eventsetup::EventSetupRecordKey&,
                       KeyedProxies& /*iHolder*/) {}
};
...
void testdependentrecord::dependentConstructorTest() {
  std::unique_ptr<EventSetupRecordProvider> depProvider =
      EventSetupRecordProviderFactoryManager::instance().makeRecordProvider(
          DepRecord::keyForClass());

  CPPUNIT_ASSERT(1 == depProvider->dependentRecords().size());
  CPPUNIT_ASSERT(*(depProvider->dependentRecords().begin()) ==
                 DummyRecord::keyForClass());
...
void testdependentrecord::dependentFinder1Test() {
  std::shared_ptr<EventSetupRecordProvider> dummyProvider(
      EventSetupRecordProviderFactoryManager::instance()
          .makeRecordProvider(DummyRecord::keyForClass())
          .release());
  ...
  CPPUNIT_ASSERT(definedInterval ==
                 finder.findIntervalFor(
                     depRecordKey, edm::IOVSyncValue(edm::EventID(1, 1, 2))));

  dummyFinder->setInterval(edm::ValidityInterval::invalidInterval());
  CPPUNIT_ASSERT(edm::ValidityInterval::invalidInterval() ==
                 finder.findIntervalFor(
                     depRecordKey, edm::IOVSyncValue(edm::EventID(1, 1, 4))));
  ...
  CPPUNIT_ASSERT(unknownedEndInterval ==
                 finder.findIntervalFor(
                     depRecordKey, edm::IOVSyncValue(edm::EventID(1, 1, 5))));
}

void testdependentrecord::dependentFinder2Test() {
  std::shared_ptr<EventSetupRecordProvider> dummyProvider1(
      EventSetupRecordProviderFactoryManager::instance()
          .makeRecordProvider(DummyRecord::keyForClass())
          .release());
  ...
  const edm::EventID eID_1(1, 1, 1);
  const edm::IOVSyncValue sync_1(eID_1);
  const edm::ValidityInterval definedInterval1(
      sync_1, edm::IOVSyncValue(edm::EventID(1, 1, 5)));
  dummyProvider1->setValidityInterval(definedInterval1);

  std::shared_ptr<EventSetupRecordProvider> dummyProvider2(
      EventSetupRecordProviderFactoryManager::instance()
          .makeRecordProvider(DummyRecord::keyForClass())
          .release());
  ...
  const edm::ValidityInterval definedInterval2(
      sync_2, edm::IOVSyncValue(edm::EventID(1, 1, 6)));
  dummyProvider2->setValidityInterval(definedInterval2);

  const edm::ValidityInterval overlapInterval(
      std::max(definedInterval1.first(), definedInterval2.first()),
...
void testdependentrecord::timeAndRunTest() {
  // test case where we have two providers, one synching on time the other on
  // run/lumi/event
  std::shared_ptr<EventSetupRecordProvider> dummyProvider1(
      EventSetupRecordProviderFactoryManager::instance()
          .makeRecordProvider(DummyRecord::keyForClass())
  ...
  const edm::ValidityInterval definedInterval2(
      sync_2, edm::IOVSyncValue(edm::Timestamp(6)));
  dummyProvider2->setValidityInterval(definedInterval2);

  const edm::ValidityInterval overlapInterval(
      definedInterval2.first(), edm::IOVSyncValue::invalidIOVSyncValue());
  ...
  CPPUNIT_ASSERT(overlapInterval ==
                 finder.findIntervalFor(depRecordKey,
                                        edm::IOVSyncValue(edm::EventID(1, 1, 1),
                                                          edm::Timestamp(2))));

  // Change only run/lumi/event based provider
  const edm::ValidityInterval definedInterval3(
      edm::IOVSyncValue(edm::EventID(1, 1, 6)),
      edm::IOVSyncValue(edm::EventID(1, 1, 10)));
  dummyProvider1->setValidityInterval(definedInterval3);

  const edm::ValidityInterval overlapInterval2(
      definedInterval3.first(), edm::IOVSyncValue::invalidIOVSyncValue());
  ...
  const edm::ValidityInterval definedInterval4(
      edm::IOVSyncValue(edm::Timestamp(7)),
      edm::IOVSyncValue(edm::Timestamp(10)));
  dummyProvider2->setValidityInterval(definedInterval4);

  const edm::ValidityInterval overlapInterval3(
      definedInterval4.first(), edm::IOVSyncValue::invalidIOVSyncValue());
  ...
  {
    const edm::ValidityInterval runLumiInterval(
        edm::IOVSyncValue(edm::EventID(1, 2, 11)),
        edm::IOVSyncValue(edm::EventID(1, 3, 20)));
    dummyProvider1->setValidityInterval(runLumiInterval);

    const edm::ValidityInterval timeInterval(
        edm::IOVSyncValue(edm::Timestamp(1ULL << 32)),
        edm::IOVSyncValue(edm::Timestamp(5ULL << 32)));
    dummyProvider2->setValidityInterval(timeInterval);

    const edm::ValidityInterval overlapInterval(
        runLumiInterval.first(), edm::IOVSyncValue::invalidIOVSyncValue());

    CPPUNIT_ASSERT(
        overlapInterval ==
        finder.findIntervalFor(depRecordKey,
                               edm::IOVSyncValue(edm::EventID(1, 2, 12),
                                                 edm::Timestamp(3ULL << 32))));
  }

  // Change both but make time 'closer'
  {
    const edm::ValidityInterval runLumiInterval(
        edm::IOVSyncValue(edm::EventID(1, 3, 21)),
        edm::IOVSyncValue(edm::EventID(1, 10, 40)));
    dummyProvider1->setValidityInterval(runLumiInterval);

    const edm::ValidityInterval timeInterval(
        edm::IOVSyncValue(edm::Timestamp(7ULL << 32)),
        edm::IOVSyncValue(edm::Timestamp(10ULL << 32)));
    dummyProvider2->setValidityInterval(timeInterval);

    const edm::ValidityInterval overlapInterval(
        timeInterval.first(), edm::IOVSyncValue::invalidIOVSyncValue());

    CPPUNIT_ASSERT(
        overlapInterval ==
        finder.findIntervalFor(depRecordKey,
                               edm::IOVSyncValue(edm::EventID(1, 4, 30),
                                                 edm::Timestamp(8ULL << 32))));
  }

  // Change both but make run/lumi/event 'closer'
  {
    const edm::ValidityInterval runLumiInterval(
        edm::IOVSyncValue(edm::EventID(1, 11, 41)),
        edm::IOVSyncValue(edm::EventID(1, 20, 60)));
    dummyProvider1->setValidityInterval(runLumiInterval);

    const edm::ValidityInterval timeInterval(
        edm::IOVSyncValue(edm::Timestamp(11ULL << 32)),
    ...
    CPPUNIT_ASSERT(
        overlapInterval ==
        finder.findIntervalFor(depRecordKey,
                               edm::IOVSyncValue(edm::EventID(1, 12, 50),
                                                 edm::Timestamp(70ULL << 32))));
  }

  // Change both and make it ambiguous because of different run #
  {
    const edm::ValidityInterval runLumiInterval(
        edm::IOVSyncValue(edm::EventID(2, 1, 0)),
        edm::IOVSyncValue(edm::EventID(6, 0, 0)));
    dummyProvider1->setValidityInterval(runLumiInterval);

    const edm::ValidityInterval timeInterval(
        edm::IOVSyncValue(edm::Timestamp(200ULL << 32)),
    ...
    CPPUNIT_ASSERT(
        overlapInterval ==
        finder.findIntervalFor(
            depRecordKey, edm::IOVSyncValue(edm::EventID(4, 12, 50),
                                            edm::Timestamp(400ULL << 32))));
  }
  ...
  CPPUNIT_ASSERT(overlapInterval3 ==
                 finder.findIntervalFor(
                     depRecordKey, edm::IOVSyncValue(edm::EventID(1, 1, 11),
                                                     edm::Timestamp(8))));

  const edm::ValidityInterval definedInterval5(
      edm::IOVSyncValue(edm::EventID(1, 1, 12)),
  ...
  const edm::ValidityInterval overlapInterval4(
      definedInterval5.first(), edm::IOVSyncValue::invalidIOVSyncValue());

  CPPUNIT_ASSERT(overlapInterval4 ==
                 finder.findIntervalFor(
                     depRecordKey, edm::IOVSyncValue(edm::EventID(1, 1, 13),
                                                     edm::Timestamp(11))));

  {
    ...
    const edm::IOVSyncValue sync_1(eID_1);
    const edm::ValidityInterval definedInterval1(
        sync_1, edm::IOVSyncValue(edm::EventID(1, 1, 6)));
    dummyProvider1->setValidityInterval(definedInterval1);
    ...
    const edm::ValidityInterval overlapInterval(
        definedInterval1.first(), edm::IOVSyncValue::invalidIOVSyncValue());
  ...
  }
  {
    ...
    const edm::IOVSyncValue sync_1(eID_1);
    const edm::ValidityInterval definedInterval1(
        sync_1, edm::IOVSyncValue(edm::EventID(1, 1, 10)));
    dummyProvider1->setValidityInterval(definedInterval1);
...

  {
    ...
    std::shared_ptr<edm::eventsetup::DataProxyProvider> dummyProv =
        std::make_shared<DummyProxyProvider>();
    provider.add(dummyProv);

    std::shared_ptr<DummyFinder> dummyFinder = std::make_shared<DummyFinder>();
    dummyFinder->setInterval(
        edm::ValidityInterval(edm::IOVSyncValue(edm::EventID(1, 1, 1)),
                              edm::IOVSyncValue(edm::EventID(1, 1, 5))));
    provider.add(
        std::shared_ptr<edm::EventSetupRecordIntervalFinder>(dummyFinder));

    std::shared_ptr<edm::eventsetup::DataProxyProvider> depProv =
        std::make_shared<DepOn2RecordProxyProvider>();
    provider.add(depProv);

    std::shared_ptr<Dummy2RecordFinder> dummy2Finder =
        std::make_shared<Dummy2RecordFinder>();
    dummy2Finder->setInterval(
        edm::ValidityInterval(edm::IOVSyncValue(edm::Timestamp(1)),
                              edm::IOVSyncValue(edm::Timestamp(5))));
    provider.add(
        std::shared_ptr<edm::EventSetupRecordIntervalFinder>(dummy2Finder));
    {
      const edm::EventSetup& eventSetup1 = provider.eventSetupForInstance(
          edm::IOVSyncValue(edm::EventID(1, 1, 1), edm::Timestamp(1)));
      long long id1 = eventSetup1.get<DepOn2Record>().cacheIdentifier();

      const edm::EventSetup& eventSetup2 = provider.eventSetupForInstance(
          edm::IOVSyncValue(edm::EventID(1, 1, 1), edm::Timestamp(2)));
      long long id2 = eventSetup2.get<DepOn2Record>().cacheIdentifier();
      CPPUNIT_ASSERT(id1 == id2);

      const edm::EventSetup& eventSetup3 = provider.eventSetupForInstance(
          edm::IOVSyncValue(edm::EventID(1, 1, 2), edm::Timestamp(2)));
      long long id3 = eventSetup3.get<DepOn2Record>().cacheIdentifier();
      CPPUNIT_ASSERT(id1 == id3);

      dummy2Finder->setInterval(
          edm::ValidityInterval(edm::IOVSyncValue(edm::Timestamp(6)),
                                edm::IOVSyncValue(edm::Timestamp(10))));

      const edm::EventSetup& eventSetup4 = provider.eventSetupForInstance(
          edm::IOVSyncValue(edm::EventID(1, 1, 4), edm::Timestamp(7)));
      long long id4 = eventSetup4.get<DepOn2Record>().cacheIdentifier();
      CPPUNIT_ASSERT(id1 != id4);

      dummyFinder->setInterval(
          edm::ValidityInterval(edm::IOVSyncValue(edm::EventID(1, 1, 6)),
                                edm::IOVSyncValue(edm::EventID(1, 1, 10))));

      const edm::EventSetup& eventSetup5 = provider.eventSetupForInstance(
          edm::IOVSyncValue(edm::EventID(1, 1, 7), edm::Timestamp(8)));
...
void testdependentrecord::getTest() {
  ...
  std::shared_ptr<edm::eventsetup::DataProxyProvider> depProv =
      std::make_shared<DepRecordProxyProvider>();
  provider.add(depProv);
  {
    const edm::EventSetup& eventSetup = provider.eventSetupForInstance(
        edm::IOVSyncValue(edm::EventID(1, 1, 1)));
  ...
  }
  {
    const edm::EventSetup& eventSetup = provider.eventSetupForInstance(
        edm::IOVSyncValue(edm::EventID(1, 1, 4)));
    CPPUNIT_ASSERT_THROW(eventSetup.get<DepRecord>(),
                         edm::eventsetup::NoRecordException<DepRecord>);
  }
}
...
void testdependentrecord::oneOfTwoRecordTest() {
  ...
  provider.add(depProv);
  {
    ...
    depRecord.getRecord<DummyRecord>();
    CPPUNIT_ASSERT_THROW(depRecord.getRecord<Dummy2Record>(),
                         edm::eventsetup::NoRecordException<Dummy2Record>);
...
void testdependentrecord::alternateFinderTest() {
  ...
  std::shared_ptr<DepRecordFinder> depFinder =
      std::make_shared<DepRecordFinder>();
  const edm::EventID eID_2(1, 1, 2);
  const edm::IOVSyncValue sync_2(eID_2);
  ...
  CPPUNIT_ASSERT(depInterval ==
                 finder.findIntervalFor(
                     depRecordKey, edm::IOVSyncValue(edm::EventID(1, 1, 1))));

  const edm::ValidityInterval dep2Interval(sync_3, edm::IOVSyncValue(eID_4));
  depFinder->setInterval(dep2Interval);
  /*const edm::ValidityInterval tempIOV = */ finder.findIntervalFor(
      depRecordKey, sync_3);
  // std::cout <<  tempIOV.first().eventID()<<" to "<<tempIOV.last().eventID()
  // <<std::endl;
  ...
  const edm::ValidityInterval unknownedEndInterval(
      sync_6, edm::IOVSyncValue::invalidIOVSyncValue());
  dummyFinder->setInterval(unknownedEndInterval);
  ...
  CPPUNIT_ASSERT(unknownedEndInterval ==
                 finder.findIntervalFor(depRecordKey, sync_6));
...
void testdependentrecord::invalidRecordTest() {
  ...
  const edm::ValidityInterval openEnded1(
      definedInterval2.first(), edm::IOVSyncValue::invalidIOVSyncValue());

  CPPUNIT_ASSERT(openEnded1 ==
                 finder.findIntervalFor(
                     depRecordKey, edm::IOVSyncValue(edm::EventID(1, 1, 4))));
  ...
  dummyProvider2->setValidityInterval(invalid);
  const edm::ValidityInterval openEnded2(
      definedInterval1.first(), edm::IOVSyncValue::invalidIOVSyncValue());

  CPPUNIT_ASSERT(openEnded2 ==
                 finder.findIntervalFor(
                     depRecordKey, edm::IOVSyncValue(edm::EventID(1, 1, 7))));
}
...
void testdependentrecord::extendIOVTest() {
  ...
  dummyFinder->setInterval(edm::ValidityInterval{
      startSyncValue, edm::IOVSyncValue{edm::EventID{1, 1, 5}}});
  provider.add(
      std::shared_ptr<edm::EventSetupRecordIntervalFinder>{dummyFinder});
  ...
  std::shared_ptr<Dummy2RecordFinder> dummy2Finder =
      std::make_shared<Dummy2RecordFinder>();
  dummy2Finder->setInterval(edm::ValidityInterval{
      startSyncValue, edm::IOVSyncValue{edm::EventID{1, 1, 6}}});
  provider.add(
      std::shared_ptr<edm::EventSetupRecordIntervalFinder>(dummy2Finder));
  {
    const edm::EventSetup& eventSetup1 = provider.eventSetupForInstance(
    ...
    {
      const edm::EventSetup& eventSetup = provider.eventSetupForInstance(
          edm::IOVSyncValue(edm::EventID(1, 1, 5), edm::Timestamp(2)));
    ...
    dummyFinder->setInterval(edm::ValidityInterval{
        startSyncValue, edm::IOVSyncValue{edm::EventID{1, 1, 7}}});
    {
      const edm::EventSetup& eventSetup = provider.eventSetupForInstance(
          edm::IOVSyncValue(edm::EventID(1, 1, 6), edm::Timestamp(7)));
      unsigned long long id = eventSetup.get<DepOn2Record>().cacheIdentifier();
      CPPUNIT_ASSERT(id1 == id);
      CPPUNIT_ASSERT(id1 == eventSetup.get<DummyRecord>().cacheIdentifier());
      CPPUNIT_ASSERT(id1 == eventSetup.get<Dummy2Record>().cacheIdentifier());
    ...
    dummy2Finder->setInterval(edm::ValidityInterval{
        startSyncValue, edm::IOVSyncValue{edm::EventID{1, 1, 8}}});

    dummyFinder->setInterval(edm::ValidityInterval{
        startSyncValue, edm::IOVSyncValue{edm::EventID{1, 1, 8}}});
    {
      const edm::EventSetup& eventSetup = provider.eventSetupForInstance(
          edm::IOVSyncValue(edm::EventID(1, 1, 8), edm::Timestamp(7)));
      unsigned long long id = eventSetup.get<DepOn2Record>().cacheIdentifier();
      CPPUNIT_ASSERT(id1 == id);
      CPPUNIT_ASSERT(id1 == eventSetup.get<DummyRecord>().cacheIdentifier());
      CPPUNIT_ASSERT(id1 == eventSetup.get<Dummy2Record>().cacheIdentifier());
    ...
    dummy2Finder->setInterval(edm::ValidityInterval{
        startSyncValue, edm::IOVSyncValue{edm::EventID{1, 1, 9}}});

    dummyFinder->setInterval(
        edm::ValidityInterval{edm::IOVSyncValue{edm::EventID{1, 1, 9}},
                              edm::IOVSyncValue{edm::EventID{1, 1, 9}}});
    {
      const edm::EventSetup& eventSetup = provider.eventSetupForInstance(
          edm::IOVSyncValue(edm::EventID(1, 1, 9), edm::Timestamp(7)));
      unsigned long long id = eventSetup.get<DepOn2Record>().cacheIdentifier();
      CPPUNIT_ASSERT(id1 + 1 == id);
      CPPUNIT_ASSERT(id1 + 1 ==
                     eventSetup.get<DummyRecord>().cacheIdentifier());
      CPPUNIT_ASSERT(id1 == eventSetup.get<Dummy2Record>().cacheIdentifier());
    }
    // extend the otherone and create a new IOV for the other
    dummy2Finder->setInterval(
        edm::ValidityInterval{edm::IOVSyncValue{edm::EventID{1, 1, 10}},
                              edm::IOVSyncValue{edm::EventID{1, 1, 10}}});
    ...
    {
      const edm::EventSetup& eventSetup = provider.eventSetupForInstance(
          edm::IOVSyncValue(edm::EventID(1, 1, 10), edm::Timestamp(7)));
      unsigned long long id = eventSetup.get<DepOn2Record>().cacheIdentifier();
      CPPUNIT_ASSERT(id1 + 2 == id);
      CPPUNIT_ASSERT(id1 + 1 ==
                     eventSetup.get<DummyRecord>().cacheIdentifier());
      CPPUNIT_ASSERT(id1 + 1 ==
                     eventSetup.get<Dummy2Record>().cacheIdentifier());
    }
  }
}
