...
//
// Package:     Framework
// Class  :     intersectingiovrecordintervalfinder_t_catch2
//
// Implementation:
...
TEST_CASE("IntersectingIOVRecordIntervalFinder", "[Framework][EventSetup]") {
  SECTION("constructorTest") {
    IntersectingIOVRecordIntervalFinder finder(DummyRecord::keyForClass());
    REQUIRE(finder.findingForRecords().size() == 1);
    std::set<EventSetupRecordKey> s = finder.findingForRecords();
    REQUIRE(s.find(DummyRecord::keyForClass()) != s.end());
  }
  ...
  SECTION("intersectionTest") {
...

    {
      ...
      REQUIRE(edm::ValidityInterval(sync_3, sync_4) == intersectingFinder.findIntervalFor(dummyRecordKey, sync_3));
    }

    {
      ...
      REQUIRE(edm::ValidityInterval::invalidInterval() == dummyFinder2->findIntervalFor(dummyRecordKey, sync_3));

      REQUIRE(edm::ValidityInterval(sync_1, edm::IOVSyncValue::invalidIOVSyncValue()) ==
...

    {
      ...
      REQUIRE(edm::ValidityInterval(sync_3, edm::IOVSyncValue::invalidIOVSyncValue()) ==
