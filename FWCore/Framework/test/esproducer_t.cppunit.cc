...
class testEsproducer : public CppUnit::TestFixture {
 ...
 private:
  ...
  class LabelledProducer : public ESProducer {
   public:
    enum { kFi, kFum };
    typedef edm::ESProducts<edm::es::L<DummyData, kFi>,
                            edm::es::L<DummyData, kFum> >
        ReturnProducts;
    LabelledProducer() : ptr_(new DummyData), fi_(new DummyData) {
      ptr_->value_ = 0;
      fi_->value_ = 0;
      setWhatProduced(this, "foo");
      setWhatProduced(this, &LabelledProducer::produceMore,
                      edm::es::label("fi", kFi)("fum", kFum));
    }
...
void testEsproducer::getFromTest() {
  ...
  for (int iTime = 1; iTime != 6; ++iTime) {
    ...
    pFinder->setInterval(edm::ValidityInterval(edm::IOVSyncValue(time),
                                               edm::IOVSyncValue(time)));
    const edm::EventSetup& eventSetup =
        provider.eventSetupForInstance(edm::IOVSyncValue(time));
...
void testEsproducer::getfromUniqueTest() {
  ...
  std::shared_ptr<DataProxyProvider> pProxyProv =
      std::make_shared<UniqueProducer>();
  provider.add(pProxyProv);
...
void testEsproducer::labelTest() {
  try {
    ...
    std::shared_ptr<DataProxyProvider> pProxyProv =
        std::make_shared<LabelledProducer>();
    provider.add(pProxyProv);
...
void testEsproducer::decoratorTest() {
  ...
  std::shared_ptr<DataProxyProvider> pProxyProv =
      std::make_shared<DecoratorProducer>();
  provider.add(pProxyProv);
...
void testEsproducer::forceCacheClearTest() {
  ...
  pFinder->setInterval(
      edm::ValidityInterval(edm::IOVSyncValue(time), edm::IOVSyncValue(time)));
  const edm::EventSetup& eventSetup =
      provider.eventSetupForInstance(edm::IOVSyncValue(time));
  {
    edm::ESHandle<DummyData> pDummy;
