...
TEST_CASE("test ESRecordsToProxyIndices", "[ESRecordsToProxyIndices]") {
  ...
  SECTION(" test full") {
    ...
    std::vector<std::pair<EventSetupRecordKey,
    std::pair<std::vector<DataKey>, std::vector<ComponentDescription const*> > > > orderedOfKeys = {
      {records[0], {{dataKeys[0], dataKeys[1],dataKeys[2]}, {p+1,p+2,p+3}}},
      {records[1],{{},{}}},
