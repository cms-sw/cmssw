...
class testeventprocessor : public CppUnit::TestFixture {
 ...
 public:
  void setUp() {
    ...
    doInit();
    m_handler = std::make_unique<edm::AssertHandler>();  // propagate_const<T>
                                                         // has no reset()
                                                         // function
    sleep_secs_ = 0;
  }
...
void testeventprocessor::activityRegistryTest() {
  ...
  std::shared_ptr<edm::ParameterSet> parameterSet =
      PythonProcessDesc(configuration).parameterSet();
  ...
  edm::EventProcessor proc(processDesc, token,
...
void testeventprocessor::moduleFailureTest() {
  try {
    ...
    std::cout << "Unexpected exception " << iException.explainSelf()
              << std::endl;
    throw;
  }
}
...
void testeventprocessor::serviceConfigSaveTest() {
  ...
  edm::EventProcessor proc(configuration, true);
  edm::ProcessConfiguration const& processConfiguration =
      proc.processConfiguration();
  edm::ParameterSet const& topPset(
      edm::getParameterSet(processConfiguration.parameterSetID()));
  CPPUNIT_ASSERT(
      topPset.existsAs<edm::ParameterSet>("DummyStoreConfigService", true));
}
