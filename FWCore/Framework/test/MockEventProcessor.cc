...
MockEventProcessor::MockEventProcessor(std::string const& mockData,
                                       std::ostream& output, bool iDoNotMerge)
    : mockData_(mockData),
      output_(output),
      input_(mockData_),
      run_(0),
...
edm::LuminosityBlockNumber_t MockEventProcessor::nextLuminosityBlockID() {
  return lumi_;
}
...
void MockEventProcessor::endRun(ProcessHistoryID const& phid, RunNumber_t run,
                                bool /*cleaningUpAfterException*/) {
  output_ << "\tendRun " << run << "\n";
}

void MockEventProcessor::beginLumi(ProcessHistoryID const&, RunNumber_t run,
                                   LuminosityBlockNumber_t lumi) {
  output_ << "\tbeginLumi " << run << "/" << lumi << "\n";
  throwIfNeeded();
}

void MockEventProcessor::endLumi(ProcessHistoryID const&, RunNumber_t run,
                                 LuminosityBlockNumber_t lumi,
                                 bool /*cleaningUpAfterException*/) {
...
void MockEventProcessor::writeRun(ProcessHistoryID const& phid,
                                  RunNumber_t run) {
  output_ << "\twriteRun " << run << "\n";
}

void MockEventProcessor::deleteRunFromCache(ProcessHistoryID const& phid,
                                            RunNumber_t run) {
  output_ << "\tdeleteRunFromCache " << run << "\n";
}

void MockEventProcessor::writeLumi(ProcessHistoryID const&, RunNumber_t run,
                                   LuminosityBlockNumber_t lumi) {
  output_ << "\twriteLumi " << run << "/" << lumi << "\n";
}

void MockEventProcessor::deleteLumiFromCache(ProcessHistoryID const&,
                                             RunNumber_t run,
                                             LuminosityBlockNumber_t lumi) {
...
bool MockEventProcessor::setDeferredException(std::exception_ptr) {
  return true;
}
}
