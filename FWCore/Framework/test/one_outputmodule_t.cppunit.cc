...
class testOneOutputModule : public CppUnit::TestFixture {
 ...
 private:
  std::map<Trans,
           std::function<void(edm::Worker*, edm::OutputModuleCommunicator*)>>
      m_transToFunc;
   ...
   public:
    ...
    RunOutputModule(edm::ParameterSet const& iPSet)
        : edm::one::OutputModuleBase(iPSet),
          edm::one::OutputModule<edm::one::WatchRuns>(iPSet) {}
    unsigned int m_count = 0;
    void write(edm::EventForOutput const&) override { ++m_count; }
    void writeRun(edm::RunForOutput const&) override { ++m_count; }
    void writeLuminosityBlock(edm::LuminosityBlockForOutput const&) override {
  ...
  class LumiOutputModule
      : public edm::one::OutputModule<edm::one::WatchLuminosityBlocks> {
   public:
    using edm::one::OutputModuleBase::doPreallocate;
    LumiOutputModule(edm::ParameterSet const& iPSet)
        : edm::one::OutputModuleBase(iPSet),
          edm::one::OutputModule<edm::one::WatchLuminosityBlocks>(iPSet) {}
    unsigned int m_count = 0;
    void write(edm::EventForOutput const&) override { ++m_count; }
    void writeRun(edm::RunForOutput const&) override { ++m_count; }
    void writeLuminosityBlock(edm::LuminosityBlockForOutput const&) override {
  ...
  class FileOutputModule : public edm::one::OutputModule<edm::WatchInputFiles> {
   public:
    ...
    FileOutputModule(edm::ParameterSet const& iPSet)
        : edm::one::OutputModuleBase(iPSet),
          edm::one::OutputModule<edm::WatchInputFiles>(iPSet) {}
    unsigned int m_count = 0;
    void write(edm::EventForOutput const&) override { ++m_count; }
    void writeRun(edm::RunForOutput const&) override { ++m_count; }
    void writeLuminosityBlock(edm::LuminosityBlockForOutput const&) override {
  ...
  class ResourceOutputModule
      : public edm::one::OutputModule<edm::one::SharedResources> {
   public:
    using edm::one::OutputModuleBase::doPreallocate;
    ResourceOutputModule(edm::ParameterSet const& iPSet)
        : edm::one::OutputModuleBase(iPSet),
          edm::one::OutputModule<edm::one::SharedResources>(iPSet) {
      usesResource();
    }
    unsigned int m_count = 0;
...
testOneOutputModule::testOneOutputModule()
  ...
  // Worker
  m_transToFunc[Trans::kGlobalOpenInputFile] = [this](
      edm::Worker* iBase, edm::OutputModuleCommunicator*) {
    edm::FileBlock fb;
    iBase->respondToOpenInputFile(fb);
  };

  m_transToFunc[Trans::kGlobalBeginRun] = [this](
      edm::Worker* iBase, edm::OutputModuleCommunicator*) {
    typedef edm::OccurrenceTraits<edm::RunPrincipal,
                                  edm::BranchActionGlobalBegin>
        Traits;
    edm::ParentContext parentContext;
    iBase->doWork<Traits>(*m_rp, *m_es, edm::StreamID::invalidStreamID(),
                          parentContext, nullptr);
  };

  m_transToFunc[Trans::kGlobalBeginLuminosityBlock] = [this](
      edm::Worker* iBase, edm::OutputModuleCommunicator*) {
    typedef edm::OccurrenceTraits<edm::LuminosityBlockPrincipal,
                                  edm::BranchActionGlobalBegin>
        Traits;
    edm::ParentContext parentContext;
    iBase->doWork<Traits>(*m_lbp, *m_es, edm::StreamID::invalidStreamID(),
                          parentContext, nullptr);
  };
  ...
  m_transToFunc[Trans::kGlobalEndLuminosityBlock] = [this](
      edm::Worker* iBase, edm::OutputModuleCommunicator* iComm) {
    typedef edm::OccurrenceTraits<edm::LuminosityBlockPrincipal,
  ...
  m_transToFunc[Trans::kGlobalEndRun] = [this](
      edm::Worker* iBase, edm::OutputModuleCommunicator* iComm) {
  ...
  m_transToFunc[Trans::kGlobalCloseInputFile] = [this](
      edm::Worker* iBase, edm::OutputModuleCommunicator*) {
    edm::FileBlock fb;
...
template <typename T>
void testTransition(
    std::shared_ptr<T> iMod, edm::Worker* iWorker,
    edm::OutputModuleCommunicator* iComm, testOneOutputModule::Trans iTrans,
  ...
  if (count != iMod->m_count) {
    std::cout
        << "For trans "
        << static_cast<std::underlying_type<testOneOutputModule::Trans>::type>(
               iTrans)
...
template <typename T>
void testOneOutputModule::testTransitions(std::shared_ptr<T> iMod,
...
void testOneOutputModule::basicTest() {
  ...
  testTransitions(
      testProd,
      {Trans::kEvent, Trans::kGlobalEndLuminosityBlock, Trans::kGlobalEndRun});
}
...
void testOneOutputModule::runTest() {
  ...
  testTransitions(
      testProd,
      {Trans::kGlobalBeginRun, Trans::kEvent, Trans::kGlobalEndLuminosityBlock,
...
void testOneOutputModule::fileTest() {
  ...
  CPPUNIT_ASSERT(0 == testProd->m_count);
  testTransitions(testProd,
                  {Trans::kGlobalOpenInputFile, Trans::kEvent,
                   Trans::kGlobalEndLuminosityBlock, Trans::kGlobalEndRun,
