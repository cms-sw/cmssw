...
class testGlobalModule : public CppUnit::TestFixture {
 ...
 private:
  ...
  std::shared_ptr<edm::ActivityRegistry> m_actReg;  // We do not use
                                                    // propagate_const because
  ...
  class StreamProd : public edm::global::EDProducer<edm::StreamCache<int>> {
   public:
    ...
    virtual void streamBeginRun(edm::StreamID, edm::Run const&,
                                edm::EventSetup const&) const override {
      ++m_count;
    }
    virtual void streamBeginLuminosityBlock(
        edm::StreamID, edm::LuminosityBlock const&,
        edm::EventSetup const&) const override {
      ++m_count;
    }
    virtual void streamEndLuminosityBlock(
        edm::StreamID, edm::LuminosityBlock const&,
        edm::EventSetup const&) const override {
      ++m_count;
    }
    virtual void streamEndRun(edm::StreamID, edm::Run const&,
                              edm::EventSetup const&) const override {
      ++m_count;
    }
  ...
  class RunProd : public edm::global::EDProducer<edm::RunCache<int>> {
   public:
    ...
    std::shared_ptr<int> globalBeginRun(edm::Run const&,
                                        edm::EventSetup const&) const override {
      ++m_count;
  ...
  class LumiProd
      : public edm::global::EDProducer<edm::LuminosityBlockCache<int>> {
   public:
    mutable unsigned int m_count = 0;
    void produce(edm::StreamID, edm::Event&,
                 edm::EventSetup const&) const override {
    ...
    std::shared_ptr<int> globalBeginLuminosityBlock(
  ...
  class RunSummaryProd
      : public edm::global::EDProducer<edm::RunSummaryCache<int>> {
   public:
    mutable unsigned int m_count = 0;
    void produce(edm::StreamID, edm::Event&,
                 edm::EventSetup const&) const override {
    ...
    std::shared_ptr<int> globalBeginRunSummary(
        edm::Run const&, edm::EventSetup const&) const override {
      ++m_count;
      return std::shared_ptr<int>{};
    }

    void streamEndRunSummary(edm::StreamID, edm::Run const&,
                             edm::EventSetup const&, int*) const override {
      ++m_count;
    }

    void globalEndRunSummary(edm::Run const&, edm::EventSetup const&,
                             int*) const override {
      ++m_count;
    }
  };

  class LumiSummaryProd
      : public edm::global::EDProducer<edm::LuminosityBlockSummaryCache<int>> {
   public:
    mutable unsigned int m_count = 0;
    void produce(edm::StreamID, edm::Event&,
                 edm::EventSetup const&) const override {
    ...
    std::shared_ptr<int> globalBeginLuminosityBlockSummary(
        edm::LuminosityBlock const&, edm::EventSetup const&) const override {
      ++m_count;
      return std::shared_ptr<int>{};
    }
  ...
  class BeginRunProd : public edm::global::EDProducer<edm::BeginRunProducer> {
   public:
    ...
    void globalBeginRunProduce(edm::Run&,
                               edm::EventSetup const&) const override {
      ++m_count;
    }
  };
  ...
  class BeginLumiProd
      : public edm::global::EDProducer<edm::BeginLuminosityBlockProducer> {
   public:
    mutable unsigned int m_count = 0;
    void produce(edm::StreamID, edm::Event&,
                 edm::EventSetup const&) const override {
    ...
    void globalBeginLuminosityBlockProduce(
        edm::LuminosityBlock&, edm::EventSetup const&) const override {
      ++m_count;
    }
  };
  ...
  class EndLumiProd
      : public edm::global::EDProducer<edm::EndLuminosityBlockProducer> {
   public:
    mutable unsigned int m_count = 0;
    void produce(edm::StreamID, edm::Event&,
                 edm::EventSetup const&) const override {
    ...
    void globalEndLuminosityBlockProduce(
        edm::LuminosityBlock&, edm::EventSetup const&) const override {
      ++m_count;
    }
  };

  class EndRunSummaryProd
      : public edm::global::EDProducer<edm::EndRunProducer,
                                       edm::RunSummaryCache<int>> {
   public:
    mutable unsigned int m_count = 0;
    void produce(edm::StreamID, edm::Event&,
                 edm::EventSetup const&) const override {
    ...
    void globalEndRunProduce(edm::Run&, edm::EventSetup const&,
...
testGlobalModule::testGlobalModule()
  ...
  m_rp.reset(new edm::RunPrincipal(runAux, m_prodReg, m_procConfig,
                                   &historyAppender_, 0));
  auto lumiAux =
      std::make_shared<edm::LuminosityBlockAuxiliary>(m_rp->run(), 1, now, now);
  m_lbp.reset(new edm::LuminosityBlockPrincipal(
      lumiAux, m_prodReg, m_procConfig, &historyAppender_, 0));
  m_lbp->setRunPrincipal(m_rp);
  edm::EventAuxiliary eventAux(eventID, uuid, now, true);

  m_ep.reset(new edm::EventPrincipal(
      m_prodReg, m_idHelper, m_associationsHelper, m_procConfig, nullptr));
  ...
  // For each transition, bind a lambda which will call the proper method of the
  // Worker
  ...
  m_transToFunc[Trans::kGlobalBeginRun] = [this](edm::Worker* iBase) {
    typedef edm::OccurrenceTraits<edm::RunPrincipal,
                                  edm::BranchActionGlobalBegin>
        Traits;
    edm::ParentContext nullParentContext;
    iBase->doWork<Traits>(*m_rp, *m_es, s_streamID0, nullParentContext,
                          nullptr);
  };
  ...
  m_transToFunc[Trans::kGlobalBeginLuminosityBlock] =
      [this](edm::Worker* iBase) {
        typedef edm::OccurrenceTraits<edm::LuminosityBlockPrincipal,
                                      edm::BranchActionGlobalBegin>
            Traits;
        edm::ParentContext nullParentContext;
        iBase->doWork<Traits>(*m_lbp, *m_es, s_streamID0, nullParentContext,
                              nullptr);
      };
  m_transToFunc[Trans::kStreamBeginLuminosityBlock] =
      [this](edm::Worker* iBase) {
        typedef edm::OccurrenceTraits<edm::LuminosityBlockPrincipal,
                                      edm::BranchActionStreamBegin>
            Traits;
  ...
  m_transToFunc[Trans::kEvent] = [this](edm::Worker* iBase) {
    typedef edm::OccurrenceTraits<edm::EventPrincipal,
                                  edm::BranchActionStreamBegin>
        Traits;
    edm::StreamContext streamContext(s_streamID0, nullptr);
    edm::ParentContext nullParentContext(&streamContext);
    iBase->setActivityRegistry(m_actReg);
    iBase->doWork<Traits>(*m_ep, *m_es, s_streamID0, nullParentContext,
                          nullptr);
  };
  ...
  m_transToFunc[Trans::kStreamEndRun] = [this](edm::Worker* iBase) {
    typedef edm::OccurrenceTraits<edm::RunPrincipal, edm::BranchActionStreamEnd>
        Traits;
    edm::ParentContext nullParentContext;
    iBase->doWork<Traits>(*m_rp, *m_es, s_streamID0, nullParentContext,
                          nullptr);
  ...
  m_transToFunc[Trans::kGlobalEndRun] = [this](edm::Worker* iBase) {
    typedef edm::OccurrenceTraits<edm::RunPrincipal, edm::BranchActionGlobalEnd>
        Traits;
    edm::ParentContext nullParentContext;
    iBase->doWork<Traits>(*m_rp, *m_es, s_streamID0, nullParentContext,
                          nullptr);
...
namespace {
template <typename T>
void testTransition(std::shared_ptr<T> iMod, edm::Worker* iWorker,
  ...
  if (count != iMod->m_count) {
    std::cout
        << "For trans "
        << static_cast<std::underlying_type<testGlobalModule::Trans>::type>(
               iTrans)
...
template <typename T>
void testGlobalModule::testTransitions(std::shared_ptr<T> iMod,
...
void testGlobalModule::streamTest() {
  ...
  testTransitions(testProd,
                  {Trans::kBeginStream, Trans::kStreamBeginRun,
                   Trans::kStreamBeginLuminosityBlock, Trans::kEvent,
                   Trans::kStreamEndLuminosityBlock, Trans::kStreamEndRun,
                   Trans::kEndStream});
}
...
void testGlobalModule::runTest() {
  ...
  CPPUNIT_ASSERT(0 == testProd->m_count);
  testTransitions(
      testProd, {Trans::kGlobalBeginRun, Trans::kEvent, Trans::kGlobalEndRun});
}
...
void testGlobalModule::runSummaryTest() {
  ...
  CPPUNIT_ASSERT(0 == testProd->m_count);
  testTransitions(testProd,
                  {Trans::kGlobalBeginRun, Trans::kEvent, Trans::kStreamEndRun,
                   Trans::kGlobalEndRun});
}
...
void testGlobalModule::lumiTest() {
  ...
  CPPUNIT_ASSERT(0 == testProd->m_count);
  testTransitions(testProd,
                  {Trans::kGlobalBeginLuminosityBlock, Trans::kEvent,
...
void testGlobalModule::endRunSummaryProdTest() {
  ...
  CPPUNIT_ASSERT(0 == testProd->m_count);
  testTransitions(testProd,
                  {Trans::kGlobalEndRun, Trans::kEvent, Trans::kGlobalBeginRun,
                   Trans::kStreamEndRun, Trans::kGlobalEndRun});
}
...
void testGlobalModule::endLumiSummaryProdTest() {
  ...
  testTransitions(
      testProd,
      {Trans::kGlobalEndLuminosityBlock, Trans::kEvent,
       Trans::kGlobalBeginLuminosityBlock, Trans::kStreamEndLuminosityBlock,
       Trans::kGlobalEndLuminosityBlock});
}
