...
class testGlobalOutputModule : public CppUnit::TestFixture {
 ...
 private:
  ...
  class BasicOutputModule : public edm::global::OutputModule<> {
   public:
    ...
    BasicOutputModule(edm::ParameterSet const& iPSet)
        : edm::global::OutputModuleBase(iPSet),
          edm::global::OutputModule<>(iPSet) {}
  ...
  class FileOutputModule
      : public edm::global::OutputModule<edm::WatchInputFiles> {
   public:
    using edm::global::OutputModuleBase::doPreallocate;
    FileOutputModule(edm::ParameterSet const& iPSet)
        : edm::global::OutputModuleBase(iPSet),
          edm::global::OutputModule<edm::WatchInputFiles>(iPSet) {}
    unsigned int m_count = 0;
    void write(edm::EventForOutput const&) override { ++m_count; }
    void writeRun(edm::RunForOutput const&) override { ++m_count; }
    void writeLuminosityBlock(edm::LuminosityBlockForOutput const&) override {
...
void testTransition(
    ...
    edm::OutputModuleCommunicator* iComm, testGlobalOutputModule::Trans iTrans,
  ...
  if (count != iMod->m_count) {
    std::cout << "For trans "
              << static_cast<
                     std::underlying_type<testGlobalOutputModule::Trans>::type>(
                     iTrans)
              << " expected " << count << " and got " << iMod->m_count
              << std::endl;
  }
...
template <typename T>
void testGlobalOutputModule::testTransitions(std::shared_ptr<T> iMod,
                                             Expectations const& iExpect) {
  iMod->doPreallocate(m_preallocConfig);
  edm::WorkerT<edm::global::OutputModuleBase> w{iMod, m_desc,
                                                m_params.actions_};
  edm::OutputModuleCommunicatorT<edm::global::OutputModuleBase> comm(
      iMod.get());
