...
class test_throwIfImproperDependencies : public CppUnit::TestFixture {
 ...
 private:
  static unsigned int indexForModule(
  ...
  bool testCase(ModuleDependsOnMap const& iModDeps,
                PathToModules const& iPaths) const {
    ...
    // Setup the module to index map by using all module names used in both
    // containers
    // Start with keys from the module dependency to allow control of the
    // numbering scheme
    for (auto const& md : iModDeps) {
      indexForModule(md.first, modsToIndex, indexToMods);
    }

    std::vector<std::vector<unsigned int>> pathIndexToModuleIndexOrder(
        iPaths.size());

    // Need to be able to quickly look up which paths a module appears on
    std::unordered_map<unsigned int, std::vector<unsigned int>>
        moduleIndexToPathIndex;
    ...
    for (auto const& path : iPaths) {
      ...
      for (auto const& mod : path.second) {
        ...
        if (lastModuleIndex != kInvalidIndex) {
          edgeToPathMap[std::make_pair(index, lastModuleIndex)].push_back(
              pathIndex);
        }
      ...
      if (lastModuleIndex != kInvalidIndex) {
        edgeToPathMap[std::make_pair(kFinishedProcessingIndex, lastModuleIndex)]
            .push_back(pathIndex);
      }
    }
    ...
    for (auto const& md : iModDeps) {
      ...
      for (auto const& dependsOn : md.second) {
        unsigned int toIndex =
            indexForModule(dependsOn, modsToIndex, indexToMods);

        // see if all paths containing this module also contain the dependent
        // module earlier in the path
        // if it does, then treat this only as a path dependency and not a data
        // dependency as this
        ...
        if (itPathsFound != moduleIndexToPathIndex.end() and
            moduleIndexToPathIndex.find(toIndex) !=
                moduleIndexToPathIndex.end()) {
        ...
        if (keepDataDependency) {
          edgeToPathMap[std::make_pair(fromIndex, toIndex)].push_back(
              kDataDependencyIndex);
        }
      }
    }

    throwIfImproperDependencies(edgeToPathMap, pathIndexToModuleIndexOrder,
                                pathNames, indexToMods);
...
void test_throwIfImproperDependencies::twoPathsNoCycleTest() {
...

  {
    ...
    PathToModules paths = {{"reco", {"I", "A", "H", "G", "F", "E"}},
                           {"val", {"E", "D", "C", "B", "A"}}};
...

  {
    ...
    ModuleDependsOnMap md = {{"A_TR", {"zEP1", "zEP2"}},
                             {"D", {"B"}},
                             {"E", {"D"}},
                             {"zSEP3", {"A_TR"}}};
...
void test_throwIfImproperDependencies::twoPathsWithCycleTest() {
...

  {
    ...
    PathToModules paths = {{"p1", {"A", "B", "C", "EP1"}},
                           {"p2", {"E", "EP2"}},
                           {"ep", {"BP", "A", "D"}}};
    CPPUNIT_ASSERT_THROW(testCase(md, paths), cms::Exception);
  }

  {
    ...
    // by the order of modules on path p2
    // but NOT by path3
    ModuleDependsOnMap md = {{"A_TR", {"zEP1", "zEP2", "zEP3"}},
                             {"D", {"B"}},
                             {"E", {"D"}},
                             {"zSEP4", {"A_TR"}}};
    PathToModules paths = {{"p1", {"E", "F", "zEP1"}},
