...
ListenMod::ListenMod(ParameterSet const&) {
  callWhenNewProductsRegistered(
      [this](BranchDescription const& branchDescription) {
        this->listen(branchDescription);
      });
}
...
void ListenMod::listen(BranchDescription const& iDesc) {
  ...
  if (iDesc.friendlyClassName() == intType.friendlyClassName()) {
    ...
    // std::cout << iDesc.moduleLabel() << "-" << iDesc.productInstanceName() <<
    // std::endl;
  }
}
...
void ListenFloatMod::listen(BranchDescription const& iDesc) {
  ...
  if (iDesc.friendlyClassName() == intType.friendlyClassName()) {
    ...
    // std::cout
    // <<iDesc.moduleLabel()<<"-"<<iDesc.productInstanceName()<<std::endl;
  }
}
}
...
void testEDProducerProductRegistryCallback::testCircularRef() {
  ...
  auto pc = std::make_shared<ProcessConfiguration>(
      "PROD", dummyProcessPset.id(), edm::getReleaseVersion(),
      edm::getPassID());
