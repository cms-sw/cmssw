...
// class DummyRecord : public
// edm::eventsetup::EventSetupRecordImplementation<DummyRecord> {};
...
using namespace edm;
namespace {
bool non_null(const void* iPtr) { return iPtr != 0; }
}
...
class testEventsetup : public CppUnit::TestFixture {
  ...
  CPPUNIT_TEST_EXCEPTION(getExcTest,
                         edm::eventsetup::NoRecordException<DummyRecord>);
  ...
  CPPUNIT_TEST_EXCEPTION(recordValidityTest,
                         edm::eventsetup::NoRecordException<DummyRecord>);
  CPPUNIT_TEST_EXCEPTION(recordValidityExcTest,
                         edm::eventsetup::NoRecordException<DummyRecord>);
...
void testEventsetup::getTest() {
  ...
  EventSetup const& eventSetup =
      provider.eventSetupForInstance(IOVSyncValue::invalidIOVSyncValue());
  CPPUNIT_ASSERT(non_null(&eventSetup));
  // eventSetup.get<DummyRecord>();
  // CPPUNIT_ASSERT_THROW(eventSetup.get<DummyRecord>(),
  // edm::eventsetup::NoRecordException<DummyRecord>);
...
void testEventsetup::recordProviderTest() {
  ...
  typedef eventsetup::EventSetupRecordProviderTemplate<DummyRecord>
      DummyRecordProvider;
...
void testEventsetup::recordValidityTest() {
  ...
  {
    Timestamp time_1(1);
    /*EventSetup const& eventSetup = */ provider.eventSetupForInstance(
        IOVSyncValue(time_1));
    // CPPUNIT_ASSERT_THROW(eventSetup.get<DummyRecord>(),
    // edm::eventsetup::NoRecordException<DummyRecord>);
    // eventSetup.get<DummyRecord>();
  ...
  finder->setInterval(
      ValidityInterval(IOVSyncValue(time_2), IOVSyncValue(Timestamp(3))));
  {
    EventSetup const& eventSetup =
        provider.eventSetupForInstance(IOVSyncValue(time_2));
    eventSetup.get<DummyRecord>();
  }
  {
    EventSetup const& eventSetup =
        provider.eventSetupForInstance(IOVSyncValue(Timestamp(3)));
    eventSetup.get<DummyRecord>();
  }
  {
    EventSetup const& eventSetup =
        provider.eventSetupForInstance(IOVSyncValue(Timestamp(4)));
    //   CPPUNIT_ASSERT_THROW(eventSetup.get<DummyRecord>(),
    //   edm::eventsetup::NoRecordException<DummyRecord>);
    eventSetup.get<DummyRecord>();
  }
...
void testEventsetup::recordValidityExcTest() {
  ...
  {
    EventSetup const& eventSetup =
        provider.eventSetupForInstance(IOVSyncValue(Timestamp(1)));
    // CPPUNIT_ASSERT_THROW(eventSetup.get<DummyRecord>(),
    // edm::eventsetup::NoRecordException<DummyRecord>);
    eventSetup.get<DummyRecord>();
  }
...
static eventsetup::EventSetupRecordProviderFactoryTemplate<DummyRecord>
    s_factory;

void testEventsetup::proxyProviderTest() {
  eventsetup::EventSetupProvider provider;
  std::shared_ptr<eventsetup::DataProxyProvider> dummyProv =
...
void testEventsetup::producerConflictTest() {
  ...
  // checking for conflicts is now delayed until first time EventSetup is
  // requested
  /*EventSetup const& eventSetup = */ provider.eventSetupForInstance(
...
void testEventsetup::twoSourceTest() {
  ...
  {
    std::shared_ptr<edm::DummyEventSetupRecordRetriever> dummyProv =
        std::make_shared<edm::DummyEventSetupRecordRetriever>();
    ...
    edm::eventsetup::ComponentDescription description2(
        "DummyEventSetupRecordRetriever", "", true);
  ...
  try {
    {
      ...
      std::shared_ptr<eventsetup::DataProxyProvider> dummyProv =
          std::make_shared<DummyProxyProvider>(kBad);
      dummyProv->setDescription(description);
      provider.add(dummyProv);
    }
    {
      ...
      description.pid_ = ps.id();
      std::shared_ptr<eventsetup::DataProxyProvider> dummyProv =
          std::make_shared<DummyProxyProvider>(kGood);
      dummyProv->setDescription(description);
      provider.add(dummyProv);
    }
    EventSetup const& eventSetup =
...
void testEventsetup::preferTest() {
  try {
    ...
    // specific name
    {
      ...
      EventSetupProvider::PreferredProviderInfo preferInfo;
      EventSetupProvider::RecordToDataMap recordToData;
      recordToData.insert(std::make_pair(
          std::string("DummyRecord"),
...
void testEventsetup::iovExtentionTest() {
  ...
  finder->setInterval(
      ValidityInterval(IOVSyncValue{time_2}, IOVSyncValue{Timestamp{4}}));
  {
    EventSetup const& eventSetup =
        provider.eventSetupForInstance(IOVSyncValue{Timestamp{4}});
    CPPUNIT_ASSERT(2 == eventSetup.get<DummyRecord>().cacheIdentifier());
  ...
  finder->setInterval(
      ValidityInterval(IOVSyncValue{Timestamp{5}}, IOVSyncValue{Timestamp{6}}));
  {
    EventSetup const& eventSetup =
        provider.eventSetupForInstance(IOVSyncValue{Timestamp{5}});
