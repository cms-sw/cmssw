...
void testintersectingiovrecordintervalfinder::intersectionTest() {
  ...
  std::vector<edm::propagate_const<
      std::shared_ptr<edm::EventSetupRecordIntervalFinder>>>
      finders;
  std::shared_ptr<DummyFinder> dummyFinder = std::make_shared<DummyFinder>();
  {
    ...
    CPPUNIT_ASSERT(
        definedInterval ==
        intFinder.findIntervalFor(dummyRecordKey,
...

  {
    ...
    const edm::ValidityInterval unknownedEndInterval(
        sync_5, edm::IOVSyncValue::invalidIOVSyncValue());
    dummyFinder->setInterval(unknownedEndInterval);

    CPPUNIT_ASSERT(
        unknownedEndInterval ==
        intFinder.findIntervalFor(dummyRecordKey,
...

  {
    ...
    CPPUNIT_ASSERT(edm::ValidityInterval(
                       sync_1, edm::IOVSyncValue::invalidIOVSyncValue()) ==
                   intFinder.findIntervalFor(dummyRecordKey, sync_3));
  }

  {
    ...
    std::shared_ptr<DummyFinder> dummyFinder2 = std::make_shared<DummyFinder>();
    dummyFinder2->setInterval(edm::ValidityInterval(
        sync_3, edm::IOVSyncValue::invalidIOVSyncValue()));
    finders.push_back(dummyFinder2);
    IntersectingIOVRecordIntervalFinder intFinder(dummyRecordKey);
    intFinder.swapFinders(finders);
