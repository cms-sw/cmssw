...
TEST_CASE("EDConsumerBase", "[Framework]") {
  constexpr const auto skipCurrentProcessLabel = edm::ProductResolverIndexHelper::skipCurrentProcessLabel();

  SECTION("testRegularType") {
    ...
    REQUIRE(edm::ProductResolverIndexInvalid == vint_c_skipCurrent);
    const auto vint_c_no_proc = helper.index(edm::PRODUCT_TYPE, typeID_vint, "labelC", "instanceC", 0);
    REQUIRE(vint_c == vint_c_no_proc);
    ...
    const auto vint_blank_no_proc = helper.index(edm::PRODUCT_TYPE, typeID_vint, "label", "instance", 0);
    {
      ...
      REQUIRE(intConsumer.m_tokens[0].index() == 0);
      REQUIRE(intConsumer.m_tokens[1].index() == 1);
    ...
    }
    {
      ...
      REQUIRE(intConsumerRev.m_tokens[0].index() == 0);
      REQUIRE(intConsumerRev.m_tokens[1].index() == 1);
    ...
    }
    {
      ...
      REQUIRE(edm::ProductResolverIndexAndSkipBit(vint_c, false) ==
              intConsumerRev.indexFrom(intConsumerRev.m_tokens[0], edm::InEvent, typeID_vint));
      REQUIRE(edm::ProductResolverIndexAndSkipBit(vint_blank, false) ==
    ...
    }
    {
      ...
      REQUIRE(edm::ProductResolverIndexAndSkipBit(vint_c_skipCurrent, true) ==
              intConsumer.indexFrom(intConsumer.m_tokens[1], edm::InEvent, typeID_vint));
      REQUIRE(edm::ProductResolverIndexAndSkipBit(vint_blank_no_proc, false) ==
              intConsumer.indexFrom(intConsumer.m_tokens[0], edm::InEvent, typeID_vint));

      std::vector<edm::ProductResolverIndexAndSkipBit> indices;
      intConsumer.itemsToGet(edm::InEvent, indices);

      REQUIRE(1 == indices.size());
      REQUIRE(indices.end() ==
              std::find(indices.begin(), indices.end(), edm::ProductResolverIndexAndSkipBit(vint_c_skipCurrent, true)));
      REQUIRE(indices.end() != std::find(indices.begin(),
                                         indices.end(),
    ...
    }
    {
      ...
      REQUIRE(intConsumer.m_tokens[0].index() == 0);
      REQUIRE(edm::ProductResolverIndexInvalid ==
              intConsumer.indexFrom(intConsumer.m_tokens[0], edm::InEvent, typeID_vint).productResolverIndex());

      std::vector<edm::ProductResolverIndexAndSkipBit> indices;
      intConsumer.itemsToGet(edm::InEvent, indices);
      //nothing to get since not here
      REQUIRE(0 == indices.size());
    }
  ...
  SECTION("testViewType") {
    ...
    REQUIRE(v_simple_no_proc == v_simple);
...

    {
      ...
      REQUIRE(edm::ProductResolverIndexAndSkipBit(v_int, false) ==
              consumer.indexFrom(consumer.m_tokens[0], edm::InEvent, typeID_int));
      REQUIRE(edm::ProductResolverIndexAndSkipBit(v_simple, false) ==
...

    {
      ...
      REQUIRE(edm::ProductResolverIndexAndSkipBit(v_int_no_proc, false) ==
              consumer.indexFrom(consumer.m_tokens[0], edm::InEvent, typeID_int));
      REQUIRE(edm::ProductResolverIndexAndSkipBit(v_simple_skip, true) ==
              consumer.indexFrom(consumer.m_tokens[1], edm::InEvent, typeID_Simple));
      {
        std::vector<edm::ProductResolverIndexAndSkipBit> indices;
        consumer.itemsToGet(edm::InEvent, indices);
        ...
        REQUIRE(1 == indices.size());
        REQUIRE(indices.end() !=
                std::find(indices.begin(), indices.end(), edm::ProductResolverIndexAndSkipBit(v_int_no_proc, false)));
        REQUIRE(indices.end() ==
                std::find(indices.begin(), indices.end(), edm::ProductResolverIndexAndSkipBit(v_simple_skip, true)));
...

    {
      ...
      REQUIRE(consumer.m_tokens[0].index() == 0);
      REQUIRE(edm::ProductResolverIndexInvalid ==
  ...
  SECTION("testMay") {
    ...
    const auto vint_c = helper.index(edm::PRODUCT_TYPE, typeID_vint, "labelC", "instanceC", "processC");
    const auto vint_c_no_proc = helper.index(edm::PRODUCT_TYPE, typeID_vint, "labelC", "instanceC", 0);
    REQUIRE(vint_c_no_proc == vint_c);
    const auto vint_c_skip =
        helper.index(edm::PRODUCT_TYPE, typeID_vint, "labelC", "instanceC", skipCurrentProcessLabel);
    const auto vint_blank = helper.index(edm::PRODUCT_TYPE, typeID_vint, "label", "instance", "process");
    const auto vint_blank_no_proc = helper.index(edm::PRODUCT_TYPE, typeID_vint, "label", "instance", 0);
    {
      ...
      REQUIRE(consumer.m_tokens[0].index() == 0);
      REQUIRE(consumer.m_tokens[1].index() == 1);
      REQUIRE(consumer.m_mayTokens.size() == 0);
...

    {
      ...
      REQUIRE(consumer.m_mayTokens.size() == 2);
      REQUIRE(consumer.m_mayTokens[0].index() == 0);
      REQUIRE(consumer.m_mayTokens[1].index() == 1);
      REQUIRE(consumer.m_tokens.size() == 0);

      REQUIRE(vint_c == consumer.indexFrom(consumer.m_mayTokens[1], edm::InEvent, typeID_vint).productResolverIndex());
      REQUIRE(vint_blank ==
      ...
      std::vector<edm::ProductResolverIndexAndSkipBit> indicesMay;
      consumer.itemsMayGet(edm::InEvent, indicesMay);
      REQUIRE(2 == indicesMay.size());
      REQUIRE(indicesMay.end() !=
              std::find(indicesMay.begin(), indicesMay.end(), edm::ProductResolverIndexAndSkipBit(vint_c, false)));
      REQUIRE(indicesMay.end() !=
              std::find(indicesMay.begin(), indicesMay.end(), edm::ProductResolverIndexAndSkipBit(vint_blank, false)));
    }

    {
      ...
      REQUIRE(consumer.m_mayTokens.size() == 1);
      REQUIRE(consumer.m_tokens.size() == 1);
      REQUIRE(consumer.m_tokens[0].index() == 0);
      REQUIRE(consumer.m_mayTokens[0].index() == 1);

      REQUIRE(vint_c == consumer.indexFrom(consumer.m_mayTokens[0], edm::InEvent, typeID_vint).productResolverIndex());
      REQUIRE(vint_blank == consumer.indexFrom(consumer.m_tokens[0], edm::InEvent, typeID_vint).productResolverIndex());
    ...
    }
    {
      ...
      REQUIRE(edm::ProductResolverIndexAndSkipBit(vint_c_skip, true) ==
