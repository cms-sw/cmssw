...
std::shared_ptr<EventSetupProvider> EventSetupsController::makeProvider(
    ParameterSet& iPSet) {
...
void EventSetupsController::eventSetupForInstance(
  ...
  if (mustFinishConfiguration_) {
    std::for_each(providers_.begin(), providers_.end(),
                  [](std::shared_ptr<EventSetupProvider> const& esp) {
...
EventSetupsController::getESProducerAndRegisterProcess(
...
void EventSetupsController::putESProducer(
    ParameterSet const& pset,
    std::shared_ptr<DataProxyProvider> const& component,
    unsigned subProcessIndex) {
  auto newElement =
      esproducers_.insert(std::pair<ParameterSetID, ESProducerInfo>(
          pset.id(), ESProducerInfo(&pset, component)));
  // Register processes with an exact match
...
void EventSetupsController::putESSource(
    ...
    unsigned subProcessIndex) {
  auto newElement = essources_.insert(std::pair<ParameterSetID, ESSourceInfo>(
      pset.id(), ESSourceInfo(&pset, component)));
  // Register processes with an exact match
  newElement->second.subProcessIndexes().push_back(subProcessIndex);
}
...
void EventSetupsController::lookForMatches(
    ParameterSetID const& psetID, unsigned subProcessIndex,
  ...
  for (auto it = elements.first; it != elements.second; ++it) {
    std::vector<unsigned> const& subProcessIndexes =
        it->second.subProcessIndexes();

    auto iFound = std::find(subProcessIndexes.begin(), subProcessIndexes.end(),
                            subProcessIndex);
...
bool EventSetupsController::isMatchingESSource(
    ParameterSetID const& psetID, unsigned subProcessIndex,
    unsigned precedingProcessIndex) const {
...
bool EventSetupsController::isMatchingESProducer(
    ParameterSetID const& psetID, unsigned subProcessIndex,
    unsigned precedingProcessIndex) const {
  auto elements = esproducers_.equal_range(psetID);
  for (auto it = elements.first; it != elements.second; ++it) {
...
ParameterSet const* EventSetupsController::getESProducerPSet(
    ParameterSetID const& psetID, unsigned subProcessIndex) const {
  auto elements = esproducers_.equal_range(psetID);
  for (auto it = elements.first; it != elements.second; ++it) {
    std::vector<unsigned> const& subProcessIndexes =
        it->second.subProcessIndexes();
...
void EventSetupsController::checkESProducerSharing() {
  ...
  for (; esProvider != esProviderEnd; ++esProvider) {
    ...
    for (auto precedingESProvider = providers_.begin();
         precedingESProvider != esProvider; ++precedingESProvider) {
      (*esProvider)
