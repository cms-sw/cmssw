...
inline bool partial_match(const boost::regex& regularExpression,
...
ProductSelectorRules::Rule::Rule(std::string const& s,
  ...
  if (spec == "*")  // special case for wildcard
  {
    ...
    if (!good) {
      throw edm::Exception(edm::errors::Configuration)
          ...
          << "be a branch specification of the form "
             "'type_label_instance_process'\n"
          << "There must be 4 fields separated by underscores\n"
          << "The fields can only contain alphanumeric characters and the "
             "wildcards * or ?\n"
          << "Alternately, a single * is also allowed for the branch "
             "specification\n"
          << "This is the invalid output configuration rule:\n"
          << "    " << s << "\n"
          << "Exception thrown from ProductSelectorRules::Rule\n";
...
void ProductSelectorRules::Rule::applyToAll(
    std::vector<BranchSelectState>& branchstates) const {
...
void ProductSelectorRules::applyToAll(
    std::vector<BranchSelectState>& branchstates) const {
...
void ProductSelectorRules::Rule::applyToOne(
    edm::BranchDescription const* branch, bool& result) const {
  if (this->appliesTo(branch)) result = selectflag_;
}

bool ProductSelectorRules::Rule::appliesTo(
    edm::BranchDescription const* branch) const {
...
ProductSelectorRules::defaultSelectionStrings() {
  static const std::vector<std::string> s_defaultStrings(1U,
                                                         std::string("keep *"));
  return s_defaultStrings;
}

void ProductSelectorRules::fillDescription(
    ParameterSetDescription& desc, char const* parameterName,
...
ProductSelectorRules::ProductSelectorRules(
    ParameterSet const& pset, std::string const& parameterName,
