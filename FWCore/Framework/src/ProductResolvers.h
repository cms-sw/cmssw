...
class DataManagingProductResolver : public ProductResolverBase {
 public:
  ...
  DataManagingProductResolver(std::shared_ptr<BranchDescription const> bd,
  ...
  virtual void connectTo(ProductResolverBase const&,
                         Principal const*) override final;
 ...
 private:
  ...
  virtual void putOrMergeProduct_(
      std::unique_ptr<WrapperBase> prod) const override final;
  ...
  virtual bool productWasFetchedAndIsValid_(
      bool iSkipCurrentProcess) const override final;

  virtual BranchDescription const& branchDescription_() const override final {
    return *getProductData().branchDescription();
  }
  virtual void resetBranchDescription_(
      std::shared_ptr<BranchDescription const> bd) override final {
  ...
  virtual void setProvenance_(ProductProvenanceRetriever const* provRetriever,
                              ...
                              ProductID const& pid) override final;
...
class InputProductResolver : public DataManagingProductResolver {
 ...
 private:
  ...
  virtual Resolution resolveProduct_(
      Principal const& principal, bool skipCurrentProcess,
  ...
  UnscheduledAuxiliary const* aux_;  // provides access to the delayedGet
                                     // signals
};

class ProducedProductResolver : public DataManagingProductResolver {
 public:
  ProducedProductResolver(std::shared_ptr<BranchDescription const> bd,
                          ProductStatus iDefaultStatus)
...
class UnscheduledProductResolver : public ProducedProductResolver {
 public:
  explicit UnscheduledProductResolver(
...
class AliasProductResolver : public ProductResolverBase {
 public:
  ...
  virtual void connectTo(ProductResolverBase const& iOther,
                         Principal const* iParentPrincipal) override final {
 ...
 private:
  ...
  virtual void prefetchAsync_(WaitingTask* waitTask, Principal const& principal,
                              ...
                              SharedResourcesAcquirer* sra,
                              ModuleCallingContext const* mcc) const override {
    realProduct_.prefetchAsync(waitTask, principal, skipCurrentProcess, sra,
                               mcc);
  }
  virtual bool unscheduledWasNotRun_() const override {
  ...
  virtual bool productWasDeleted_() const override {
    return realProduct_.productWasDeleted();
  }
  virtual bool productWasFetchedAndIsValid_(
      bool iSkipCurrentProcess) const override final {
  ...
  virtual void resetBranchDescription_(
      std::shared_ptr<BranchDescription const> bd) override {
    bd_ = bd;
  }
  virtual Provenance const* provenance_() const override final {
    return realProduct_.provenance();
  }

  virtual std::string const& resolvedModuleLabel_() const override {
    return realProduct_.moduleLabel();
  }
  virtual void setProvenance_(ProductProvenanceRetriever const* provRetriever,
                              ProcessHistory const& ph,
...
class ParentProcessProductResolver : public ProductResolverBase {
 public:
  typedef ProducedProductResolver::ProductStatus ProductStatus;
  explicit ParentProcessProductResolver(
      std::shared_ptr<BranchDescription const> bd)
      : ProductResolverBase(),
        realProduct_(nullptr),
        bd_(bd),
        provRetriever_(nullptr),
        parentPrincipal_(nullptr) {}
 ...
 private:
  virtual Resolution resolveProduct_(
    ...
    return realProduct_->resolveProduct(*parentPrincipal_, skipCurrentProcess,
                                        sra, mcc);
  }
  virtual void prefetchAsync_(WaitingTask* waitTask, Principal const& principal,
                              bool skipCurrentProcess,
                              ...
                              ModuleCallingContext const* mcc) const override {
    skipCurrentProcess = false;
    realProduct_->prefetchAsync(waitTask, *parentPrincipal_, skipCurrentProcess,
                                sra, mcc);
  }
  virtual bool unscheduledWasNotRun_() const override {
  ...
  virtual bool productWasFetchedAndIsValid_(
      bool iSkipCurrentProcess) const override {
...
class NoProcessProductResolver : public ProductResolverBase {
 public:
  ...
  NoProcessProductResolver(
      std::vector<ProductResolverIndex> const& matchingHolders,
  ...
  void prefetchFailed(unsigned int iProcessingIndex, Principal const& principal,
 ...
 private:
  ...
  Resolution tryResolver(unsigned int index, Principal const& principal,
                         bool skipCurrentProcess, SharedResourcesAcquirer* sra,
                         ModuleCallingContext const* mcc) const;

  void setCache(bool skipCurrentProcess, ProductResolverIndex index,
