...
namespace edm {
  ...
  EDFilter::doEvent(EventPrincipal const& ep, EventSetupImpl const& c,
    ...
    rc = this->filter(e, EventSetup{c,static_cast<unsigned int>(Transition::Event)});
  ...
  EDFilter::doBeginRun(RunPrincipal const& rp, EventSetupImpl const& c,
    ...
    r.setConsumer(this);
    Run const& cnstR=r;
    this->beginRun(cnstR, EventSetup{c,static_cast<unsigned int>(Transition::BeginRun)});
    commit_(r);
    return;
  }
  ...
  EDFilter::doEndRun(RunPrincipal const& rp, EventSetupImpl const& c,
    ...
    r.setConsumer(this);
    Run const& cnstR=r;
    this->endRun(cnstR, EventSetup{c,static_cast<unsigned int>(Transition::EndRun)});
    commit_(r);
    return;
  }
  ...
  EDFilter::doBeginLuminosityBlock(LuminosityBlockPrincipal const& lbp, EventSetupImpl const& c,
    ...
    this->beginLuminosityBlock(cnstLb, EventSetup{c,static_cast<unsigned int>(Transition::BeginLuminosityBlock)});
    commit_(lb);
  }
  ...
  EDFilter::doEndLuminosityBlock(LuminosityBlockPrincipal const& lbp, EventSetupImpl const& c,
    ...
    lb.setConsumer(this);
    LuminosityBlock const& cnstLb = lb;
    this->endLuminosityBlock(cnstLb, EventSetup{c,static_cast<unsigned int>(Transition::EndLuminosityBlock)});
    commit_(lb);
    return ;
  }
