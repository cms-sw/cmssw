...
namespace edm {
  ...
  ServiceToken ScheduleItems::initServices(std::vector<ParameterSet>& pServiceSets, ParameterSet& parameterSet,
  ...
  std::shared_ptr<CommonParams> ScheduleItems::initMisc(ParameterSet& parameterSet) {
    ...
    auto common = std::make_shared<CommonParams>(
        ...
        parameterSet.getUntrackedParameterSet("maxLuminosityBlocks", ParameterSet())
            .getUntrackedParameter<int>("input", -1),
        parameterSet.getUntrackedParameterSet("maxSecondsUntilRampdown", ParameterSet())
            .getUntrackedParameter<int>("input", -1));
    return common;
  }

  std::unique_ptr<Schedule> ScheduleItems::initSchedule(ParameterSet& parameterSet, bool hasSubprocesses,
