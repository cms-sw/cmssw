...
SystemTimeKeeper::SystemTimeKeeper(
  ...
  std::sort(
      ...
      [](const ModuleDescription* iLHS, const ModuleDescription* iRHS) -> bool {
  ...
  std::copy(iNamesService.getEndPaths().begin(),
            iNamesService.getEndPaths().end(), std::back_inserter(m_pathNames));
...
// member functions
//
SystemTimeKeeper::PathTiming& SystemTimeKeeper::pathTiming(
    StreamContext const& iStream, PathContext const& iPath) {
  unsigned int offset = 0;
  if (iPath.isEndPath()) {
    offset = m_endPathOffset;
  }
  assert(iPath.pathID() + offset <
         m_streamPathTiming[iStream.streamID().value()].size());
  return m_streamPathTiming[iStream.streamID().value()]
                           [iPath.pathID() + offset];
}
...
void SystemTimeKeeper::startModuleEvent(StreamContext const& iStream,
  ...
  if (checkBounds(iModule.moduleDescription()->id())) {
    auto& mod =
        m_streamModuleTiming[iStream.streamID().value()]
                            [iModule.moduleDescription()->id() - m_minModuleID];
    mod.m_timer.start();
    ++(mod.m_timesRun);
  }
}
void SystemTimeKeeper::stopModuleEvent(StreamContext const& iStream,
  ...
  if (checkBounds(iModule.moduleDescription()->id())) {
    ...
    if (iModule.type() == ParentContext::Type::kPlaceInPath) {
      ...
      auto& modTiming = pathTiming(iStream, *(place->pathContext()))
                            .m_moduleTiming[place->placeInPath()];
...
static void fillPathSummary(
    unsigned int iStartIndex, unsigned int iEndIndex,
  ...
  for (auto const& stream : iPathTimings) {
    ...
    for (unsigned int index = iStartIndex; index < iEndIndex; ++index, ++it) {
      ...
      for (unsigned int modIndex = 0;
           modIndex < pathTiming.m_moduleTiming.size(); ++modIndex) {
...
void SystemTimeKeeper::fillTriggerTimingReport(TriggerTimingReport& rep) const {
  ...
  // Per path summary
  {
    fillPathSummary(0, m_endPathOffset, m_pathNames, m_modulesOnPaths,
                    m_streamPathTiming, rep.trigPathSummaries);
    fillPathSummary(m_endPathOffset, m_streamPathTiming[0].size(), m_pathNames,
                    m_modulesOnPaths, m_streamPathTiming, rep.endPathSummaries);
  }
}
