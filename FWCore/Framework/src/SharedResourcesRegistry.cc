...
const std::string SharedResourcesRegistry::kLegacyModuleResourceName{
    "__legacy__"};
...
SharedResourcesRegistry::createAcquirerForSourceDelayedReader() {
  if (not resourceForDelayedReader_) {
    resourceForDelayedReader_ =
        std::make_shared<std::recursive_mutex>();  // propagate_const<T> has no
                                                   // reset() function
  ...
  return std::make_pair(SharedResourcesAcquirer(std::move(queues)),
                        get_underlying(resourceForDelayedReader_));
}
...
SharedResourcesAcquirer SharedResourcesRegistry::createAcquirer(
  ...
  // Consistent sorting avoids deadlocks and this particular order optimizes
  // performance
  std::map<std::pair<unsigned int, std::string>,
           std::shared_ptr<SerialTaskQueue>>
      sortedResources;

  // Is this acquirer for a module that depends on the legacy shared resource?
  if (std::find(resourceNames.begin(), resourceNames.end(),
                kLegacyModuleResourceName) != resourceNames.end()) {
    for (auto const& resource : resourceMap_) {
      ...
      if (resource.second.second > 1) {
        sortedResources.insert(std::make_pair(
            std::make_pair(resource.second.second, resource.first),
