...
namespace impl {
std::unique_ptr<edm::OutputModuleCommunicator> createCommunicatorIfNeeded(
    void*);
std::unique_ptr<edm::OutputModuleCommunicator> createCommunicatorIfNeeded(
    ::edm::OutputModule*);
std::unique_ptr<edm::OutputModuleCommunicator> createCommunicatorIfNeeded(
...
class OutputModuleCommunicatorT : public edm::OutputModuleCommunicator {
 public:
  ...
  virtual void writeRun(edm::RunPrincipal const& rp,
                        ProcessContext const*) override;

  virtual void writeLumi(edm::LuminosityBlockPrincipal const& lbp,
                         ProcessContext const*) override;

  ///\return true if OutputModule has reached its limit on maximum number of
  ...
  virtual edm::SelectedProductsForBranchType const& keptProducts()
      const override;

  virtual void selectProducts(edm::ProductRegistry const& preg,
  ...
  static std::unique_ptr<edm::OutputModuleCommunicator> createIfNeeded(
      T* iMod) {
