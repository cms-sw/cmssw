...
//
// const DataProxyProvider& DataProxyProvider::operator=(const
// DataProxyProvider& rhs)
// {
...
void DataProxyProvider::invalidateProxies(
    const EventSetupRecordKey& iRecordKey) {
  ...
  for (KeyedProxies::iterator keyedProxy(proxyList.begin());
       keyedProxy != finished; ++keyedProxy) {
    (*((*keyedProxy).second)).invalidate();
  }
}

void DataProxyProvider::resetProxies(const EventSetupRecordKey& iRecordKey) {
  invalidateProxies(iRecordKey);
}

void DataProxyProvider::resetProxiesIfTransient(
    const EventSetupRecordKey& iRecordKey) {
  KeyedProxies& proxyList((*(recordProxies_.find(iRecordKey))).second);
  KeyedProxies::iterator finished(proxyList.end());
  for (KeyedProxies::iterator keyedProxy(proxyList.begin());
...
void DataProxyProvider::setAppendToDataLabel(
    const edm::ParameterSet& iToAppend) {
...
const DataProxyProvider::KeyedProxies& DataProxyProvider::keyedProxies(
    const EventSetupRecordKey& iRecordKey) const {
  ...
  if (itFind->second.empty()) {
    ...
    for (KeyedProxies::iterator itProxy = proxies.begin(),
                                itProxyEnd = proxies.end();
         itProxy != itProxyEnd; ++itProxy) {
      itProxy->second->setProviderDescription(&description());
      if (mustChangeLabels) {
        // Using swap is fine since
        // 1) the data structure is not a map and so we have not sorted on the
        // keys
        // 2) this is the first time filling this so no outside agency has yet
        // seen
        //   the label and therefore can not be dependent upon its value
        std::string temp(std::string(itProxy->first.name().value()) +
                         appendToDataLabel_);
...
void DataProxyProvider::prevalidate(ConfigurationDescriptions& iDesc) {
  if (iDesc.defaultDescription()) {
    if (iDesc.defaultDescription()->isLabelUnused(kAppendToDataLabel)) {
      iDesc.defaultDescription()->add<std::string>(kAppendToDataLabel,
                                                   std::string(""));
    }
  }
  for (auto& v : iDesc) {
