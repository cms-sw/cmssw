...
namespace edm {
  namespace stream {
    ...
    template <typename T>
    void ProducingModuleAdaptorBase<T>::updateLookup(BranchType iType, ProductResolverIndexHelper const& iHelper,
    ...
    template <typename T>
    void ProducingModuleAdaptorBase<T>::doStreamBeginRun(StreamID id, RunPrincipal const& rp, EventSetup const& c,
    ...
    template <typename T>
    void ProducingModuleAdaptorBase<T>::doStreamEndRun(StreamID id, RunPrincipal const& rp, EventSetup const& c,
                                                       ModuleCallingContext const* mcc) {
      auto mod = m_streamModules[id];
      Run r(rp, moduleDescription_, mcc);
      r.setConsumer(mod);
    ...
    template <typename T>
    void ProducingModuleAdaptorBase<T>::doStreamBeginLuminosityBlock(StreamID id, LuminosityBlockPrincipal const& lbp,
                                                                     EventSetup const& c,
                                                                     ModuleCallingContext const* mcc) {
      auto mod = m_streamModules[id];
    ...
    template <typename T>
    void ProducingModuleAdaptorBase<T>::doStreamEndLuminosityBlock(StreamID id, LuminosityBlockPrincipal const& lbp,
                                                                   EventSetup const& c,
                                                                   ModuleCallingContext const* mcc) {
      auto mod = m_streamModules[id];
      LuminosityBlock lb(lbp, moduleDescription_, mcc);
