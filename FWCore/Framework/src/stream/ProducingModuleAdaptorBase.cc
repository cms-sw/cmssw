...
template <typename T>
void ProducingModuleAdaptorBase<T>::doPreallocate(
...
template <typename T>
void ProducingModuleAdaptorBase<T>::registerProductsAndCallbacks(
    ProducingModuleAdaptorBase const*, ProductRegistry* reg) {
  auto firstMod = m_streamModules[0];
  if (firstMod->registrationCallback() and m_streamModules.size() > 1) {
    // we have a callback so we will collect all callbacks and create a new
    // callback which calls them all.
    ...
    firstMod->callWhenNewProductsRegistered(
        [callbacks](BranchDescription const& iBD) {
...
template <typename T>
void ProducingModuleAdaptorBase<T>::itemsToGet(
    BranchType iType,
...
template <typename T>
void ProducingModuleAdaptorBase<T>::itemsMayGet(
    BranchType iType,
    std::vector<ProductResolverIndexAndSkipBit>& iIndices) const {
  assert(not m_streamModules.empty());
...
void ProducingModuleAdaptorBase<T>::modulesWhoseProductsAreConsumed(
    std::vector<ModuleDescription const*>& modules, ProductRegistry const& preg,
    std::map<std::string, ModuleDescription const*> const& labelsToDesc,
    std::string const& processName) const {
  assert(not m_streamModules.empty());
  return m_streamModules[0]->modulesWhoseProductsAreConsumed(
      modules, preg, labelsToDesc, processName);
}

template <typename T>
void ProducingModuleAdaptorBase<T>::convertCurrentProcessAlias(
...
template <typename T>
std::vector<edm::ConsumesInfo> ProducingModuleAdaptorBase<T>::consumesInfo()
    const {
  assert(not m_streamModules.empty());
...
template <typename T>
void ProducingModuleAdaptorBase<T>::updateLookup(
    BranchType iType, ProductResolverIndexHelper const& iHelper,
...
template <typename T>
void ProducingModuleAdaptorBase<T>::resolvePutIndicies(
    BranchType iBranchType,
    std::unordered_multimap<std::string, edm::ProductResolverIndex> const&
        iIndicies,
...
ProducingModuleAdaptorBase<T>::indiciesForPutProducts(
...
template <typename T>
void ProducingModuleAdaptorBase<T>::doStreamBeginRun(
    StreamID id, RunPrincipal const& rp, EventSetup const& c,
...
template <typename T>
void ProducingModuleAdaptorBase<T>::doStreamEndRun(
    StreamID id, RunPrincipal const& rp, EventSetup const& c,
    ModuleCallingContext const* mcc) {
  auto mod = m_streamModules[id];
  Run r(rp, moduleDescription_, mcc);
...
template <typename T>
void ProducingModuleAdaptorBase<T>::doStreamBeginLuminosityBlock(
    StreamID id, LuminosityBlockPrincipal const& lbp, EventSetup const& c,
    ModuleCallingContext const* mcc) {
  auto mod = m_streamModules[id];
...
template <typename T>
void ProducingModuleAdaptorBase<T>::doStreamEndLuminosityBlock(
    StreamID id, LuminosityBlockPrincipal const& lbp, EventSetup const& c,
    ModuleCallingContext const* mcc) {
  auto mod = m_streamModules[id];
  LuminosityBlock lb(lbp, moduleDescription_, mcc);
...
template <typename T>
void ProducingModuleAdaptorBase<T>::doRespondToCloseInputFile(
    FileBlock const&) {}

template <typename T>
void ProducingModuleAdaptorBase<T>::doRegisterThinnedAssociations(
    ProductRegistry const& registry, ThinnedAssociationsHelper& helper) {
  assert(not m_streamModules.empty());
