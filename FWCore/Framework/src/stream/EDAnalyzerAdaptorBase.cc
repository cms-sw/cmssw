...
void EDAnalyzerAdaptorBase::updateLookup(BranchType iType, ProductResolverIndexHelper const& iHelper,
...
void EDAnalyzerAdaptorBase::modulesWhoseProductsAreConsumed(
    ...
    std::map<std::string, ModuleDescription const*> const& labelsToDesc, std::string const& processName) const {
  assert(not m_streamModules.empty());
...
bool EDAnalyzerAdaptorBase::doEvent(EventPrincipal const& ep, EventSetup const& c, ActivityRegistry* act,
...
void EDAnalyzerAdaptorBase::doStreamBeginRun(StreamID id, RunPrincipal const& rp, EventSetup const& c,
                                             ModuleCallingContext const* mcc) {
...
void EDAnalyzerAdaptorBase::doStreamEndRun(StreamID id, RunPrincipal const& rp, EventSetup const& c,
                                           ModuleCallingContext const* mcc) {
  auto mod = m_streamModules[id];
  Run r(rp, moduleDescription_, mcc);
  r.setConsumer(mod);
...
void EDAnalyzerAdaptorBase::doStreamBeginLuminosityBlock(StreamID id, LuminosityBlockPrincipal const& lbp,
...
void EDAnalyzerAdaptorBase::doStreamEndLuminosityBlock(StreamID id, LuminosityBlockPrincipal const& lbp,
                                                       EventSetup const& c, ModuleCallingContext const* mcc) {
  auto mod = m_streamModules[id];
  LuminosityBlock lb(lbp, moduleDescription_, mcc);
  lb.setConsumer(mod);
