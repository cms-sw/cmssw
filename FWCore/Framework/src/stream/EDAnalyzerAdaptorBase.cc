...
// EDAnalyzerAdaptorBase::EDAnalyzerAdaptorBase(const EDAnalyzerAdaptorBase&
// rhs)
// {
...
//
// const EDAnalyzerAdaptorBase& EDAnalyzerAdaptorBase::operator=(const
// EDAnalyzerAdaptorBase& rhs)
// {
...
// member functions
//
void EDAnalyzerAdaptorBase::doPreallocate(
...
void EDAnalyzerAdaptorBase::registerProductsAndCallbacks(
    EDAnalyzerAdaptorBase const*, ProductRegistry* reg) {
  for (auto mod : m_streamModules) {
    mod->registerProductsAndCallbacks(mod, reg);
  }
}

void EDAnalyzerAdaptorBase::itemsToGet(
    BranchType iType,
    std::vector<ProductResolverIndexAndSkipBit>& iIndices) const {
  assert(not m_streamModules.empty());
  m_streamModules[0]->itemsToGet(iType, iIndices);
}
void EDAnalyzerAdaptorBase::itemsMayGet(
    BranchType iType,
    std::vector<ProductResolverIndexAndSkipBit>& iIndices) const {
  assert(not m_streamModules.empty());
...
void EDAnalyzerAdaptorBase::updateLookup(
...
void EDAnalyzerAdaptorBase::convertCurrentProcessAlias(
...
bool EDAnalyzerAdaptorBase::doEvent(EventPrincipal const& ep,
                                    EventSetup const& c, ActivityRegistry* act,
...
void EDAnalyzerAdaptorBase::doStreamEndRun(StreamID id, RunPrincipal const& rp,
                                           EventSetup const& c,
                                           ModuleCallingContext const* mcc) {
  auto mod = m_streamModules[id];
  Run r(rp, moduleDescription_, mcc);
...
void EDAnalyzerAdaptorBase::doStreamBeginLuminosityBlock(
...
void EDAnalyzerAdaptorBase::doStreamEndLuminosityBlock(
    StreamID id, LuminosityBlockPrincipal const& lbp, EventSetup const& c,
    ModuleCallingContext const* mcc) {
  auto mod = m_streamModules[id];
  LuminosityBlock lb(lbp, moduleDescription_, mcc);
