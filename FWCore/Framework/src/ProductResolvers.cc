...
namespace edm {
  ...
  void DataManagingProductResolver::mergeProduct(std::unique_ptr<WrapperBase> iFrom) const {
    ...
    if (original->isMergeable()) {
      ...
      if (!original->isProductEqual(iFrom.get())) {
        ...
        edm::LogError("RunLumiMerging") << "ProductResolver::mergeTheProduct\n"
  ...
  ProductResolverBase::Resolution InputProductResolver::resolveProduct_(Principal const& principal, bool,
  ...
  void InputProductResolver::prefetchAsync_(WaitingTask* waitTask, Principal const& principal, bool skipCurrentProcess,
  ...
  ProductResolverBase::Resolution PuttableProductResolver::resolveProduct_(Principal const&, bool skipCurrentProcess,
                                                                           SharedResourcesAcquirer*,
  ...
  void PuttableProductResolver::prefetchAsync_(WaitingTask* waitTask, Principal const& principal,
                                               bool skipCurrentProcess, SharedResourcesAcquirer* sra,
                                               ModuleCallingContext const* mcc) const {
    if (not skipCurrentProcess) {
      if (branchDescription().availableOnlyAtEndTransition() and not principal.atEndTransition()) {
        return;
      }
      m_waitingTasks.add(waitTask);
  ...
  void UnscheduledProductResolver::prefetchAsync_(WaitingTask* waitTask, Principal const& principal,
                                                  bool skipCurrentProcess, SharedResourcesAcquirer* sra,
                                                  ModuleCallingContext const* mcc) const {
    if (skipCurrentProcess) {
      return;
    ...
    if (prefetchRequested_.compare_exchange_strong(expected, true)) {
      ...
      worker_->doWorkAsync<OccurrenceTraits<EventPrincipal, BranchActionStreamBegin> >(
          t, event, *(aux_->eventSetup()), event.streamID(), parentContext, mcc->getStreamContext());
    }
  }
  ...
  void DataManagingProductResolver::putOrMergeProduct_(std::unique_ptr<WrapperBase> prod) const {
    if (not prod) {
      return;
    }
    if (status() == defaultStatus()) {
  ...
  void AliasProductResolver::setProvenance_(ProductProvenanceRetriever const* provRetriever, ProcessHistory const& ph,
                                            ProductID const& pid) {
  ...
  void AliasProductResolver::putOrMergeProduct_(std::unique_ptr<WrapperBase> edp) const {
    throw Exception(errors::LogicError) << "AliasProductResolver::putOrMergeProduct_(std::unique_ptr<WrapperBase> edp) "
  ...
  void ParentProcessProductResolver::putOrMergeProduct_(std::unique_ptr<WrapperBase> edp) const {
    throw Exception(errors::LogicError) << "ParentProcessProductResolver::putOrMergeProduct_(std::unique_ptr<"
  ...
  ProductResolverBase::Resolution NoProcessProductResolver::tryResolver(unsigned int index, Principal const& principal,
                                                                        bool skipCurrentProcess,
                                                                        SharedResourcesAcquirer* sra,
                                                                        ModuleCallingContext const* mcc) const {
  ...
  ProductResolverBase::Resolution NoProcessProductResolver::resolveProduct_(Principal const& principal,
    ...
    void NoProcessProductResolver::prefetchAsync_(WaitingTask * waitTask, Principal const& principal,
                                                  bool skipCurrentProcess, SharedResourcesAcquirer* sra,
                                                  ModuleCallingContext const* mcc) const {
      if (not skipCurrentProcess) {
        waitingTasks_.add(waitTask);
    ...
    void NoProcessProductResolver::setCache(bool iSkipCurrentProcess, ProductResolverIndex iIndex,
    ...
    namespace {
      ...
      public:
        TryNextResolverWaitingTask(NoProcessProductResolver const* iResolver, unsigned int iResolverIndex,
                                   Principal const* iPrincipal, SharedResourcesAcquirer* iSRA,
                                   ModuleCallingContext const* iMCC, bool iSkipCurrentProcess, ServiceToken iToken)
            : resolver_(iResolver),
    ...
    void NoProcessProductResolver::prefetchFailed(unsigned int iProcessingIndex, Principal const& principal,
    ...
    bool NoProcessProductResolver::dataValidFromResolver(unsigned int iProcessingIndex, Principal const& principal,
    ...
    void NoProcessProductResolver::tryPrefetchResolverAsync(unsigned int iProcessingIndex, Principal const& principal,
                                                            bool skipCurrentProcess, SharedResourcesAcquirer* sra,
      ...
      while (index < lookupProcessOrder.size()) {
        ...
        if (matchingHolders_[k] != ProductResolverIndexInvalid) {
          ...
          productResolver->prefetchAsync(task, principal, skipCurrentProcess, sra, mcc);
    ...
    void NoProcessProductResolver::setProvenance_(ProductProvenanceRetriever const*, ProcessHistory const&,
                                                  ProductID const&) {}
    ...
    void NoProcessProductResolver::putOrMergeProduct_(std::unique_ptr<WrapperBase> edp) const {
      throw Exception(errors::LogicError) << "NoProcessProductResolver::putOrMergeProduct_(std::unique_ptr<WrapperBase>"
    ...
    ProductResolverBase::Resolution SingleChoiceNoProcessProductResolver::resolveProduct_(
        Principal const& principal, bool skipCurrentProcess, SharedResourcesAcquirer* sra,
        ModuleCallingContext const* mcc) const {
    ...
    void SingleChoiceNoProcessProductResolver::prefetchAsync_(WaitingTask * waitTask, Principal const& principal,
                                                              bool skipCurrentProcess, SharedResourcesAcquirer* sra,
                                                              ModuleCallingContext const* mcc) const {
    ...
    void SingleChoiceNoProcessProductResolver::setProvenance_(ProductProvenanceRetriever const*, ProcessHistory const&,
                                                              ProductID const&) {}
    ...
    bool SingleChoiceNoProcessProductResolver::unscheduledWasNotRun_() const {
      throw Exception(errors::LogicError) << "SingleChoiceNoProcessProductResolver::unscheduledWasNotRun_() not "
    ...
    void SingleChoiceNoProcessProductResolver::putOrMergeProduct_(std::unique_ptr<WrapperBase> edp) const {
      throw Exception(errors::LogicError) << "SingleChoiceNoProcessProductResolver::putOrMergeProduct_(std::unique_ptr<"
                                             "WrapperBase> edp) not implemented and should never be called.\n"
                                          << "Contact a Framework developer\n";
    }
