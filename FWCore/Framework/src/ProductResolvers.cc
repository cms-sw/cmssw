...
void DataManagingProductResolver::throwProductDeletedException() const {
  ...
  exception << "ProductResolverBase::resolveProduct_: The product matching all "
               "criteria was already deleted\n"
            ...
            << "The module which is asking for this data must be configured to "
               "state that it will read this data.";
  throw exception;
}

// This is a templated function in order to avoid calling another virtual
// function
template <bool callResolver, typename FUNC>
ProductResolverBase::Resolution DataManagingProductResolver::resolveProductImpl(
    FUNC resolver) const {
  ...
  if (callResolver && presentStatus == ProductStatus::ResolveNotRun) {
    ...
    std::unique_ptr<ProductStatus, decltype(failedStatusSetter)>
        failedStatusGuard(&presentStatus, failedStatusSetter);
...
void DataManagingProductResolver::mergeProduct(
    std::unique_ptr<WrapperBase> iFrom) const {
  ...
  if (original->isMergeable()) {
    ...
    if (!original->isProductEqual(iFrom.get())) {
      ...
      edm::LogError("RunLumiMerging")
          ...
          << "Two run/lumi products for the same run/lumi which should be "
             "equal are not\n"
    ...
    edm::LogWarning("RunLumiMerging")
        << "ProductResolver::mergeTheProduct\n"
        << "Run/lumi product has neither a mergeProduct nor isProductEqual "
           "function\n"
...
ProductResolverBase::Resolution InputProductResolver::resolveProduct_(
    Principal const& principal, bool, SharedResourcesAcquirer*,
  ...
  return resolveProductImpl<true>([this, &principal, mcc]() {
    ...
    auto sentry(make_sentry(
        mcc, [this, branchType](ModuleCallingContext const* iContext) {
          if (branchType == InEvent and aux_) {
            aux_->postModuleDelayedGetSignal_.emit(
                *(iContext->getStreamContext()), *iContext);
          }
        }));
    ...
    if (auto reader = principal.reader()) {
      ...
      if (not productResolved()) {
        ...
        putProduct(reader->getProduct(BranchKey(branchDescription()),
                                      &principal, mcc));
      }
    }
  });
...
void InputProductResolver::retrieveAndMerge_(Principal const& principal) const {
  if (auto reader = principal.reader()) {
    std::unique_lock<std::recursive_mutex> guard;
    if (auto sr = reader->sharedResources().second) {
      guard = std::unique_lock<std::recursive_mutex>(*sr);
...
void InputProductResolver::prefetchAsync_(
    WaitingTask* waitTask, Principal const& principal, bool skipCurrentProcess,
...
void InputProductResolver::setupUnscheduled(
    UnscheduledConfigurator const& iConfigure) {
  aux_ = iConfigure.auxiliary();
}

bool InputProductResolver::isFromCurrentProcess() const { return false; }

ProductResolverBase::Resolution PuttableProductResolver::resolveProduct_(
    Principal const&, bool skipCurrentProcess, SharedResourcesAcquirer*,
...
void PuttableProductResolver::prefetchAsync_(
    WaitingTask* waitTask, Principal const& principal, bool skipCurrentProcess,
    SharedResourcesAcquirer* sra, ModuleCallingContext const* mcc) const {
  if (not skipCurrentProcess) {
    ...
    bool expected = false;
    if (worker_ and
        prefetchRequested_.compare_exchange_strong(expected, true)) {
      ...
      auto waiting = make_waiting_task(
...
void PuttableProductResolver::putProduct_(
...
void PuttableProductResolver::setupUnscheduled(
    UnscheduledConfigurator const& iConfigure) {
  worker_ = iConfigure.findWorker(branchDescription().moduleLabel());
}

void UnscheduledProductResolver::setupUnscheduled(
    UnscheduledConfigurator const& iConfigure) {
  aux_ = iConfigure.auxiliary();
  worker_ = iConfigure.findWorker(branchDescription().moduleLabel());
...
ProductResolverBase::Resolution UnscheduledProductResolver::resolveProduct_(
    ...
    return resolveProductImpl<true>([&principal, this, sra, mcc]() {
      try {
        ...
        auto workCall = [this, &event, &parentContext, mcc]() {
          auto sentry(
              make_sentry(mcc, [this](ModuleCallingContext const* iContext) {
                aux_->postModuleDelayedGetSignal_.emit(
                    *(iContext->getStreamContext()), *iContext);
              }));
          ...
          worker_->doWork<
              ...
              event, *(aux_->eventSetup()), event.streamID(), parentContext,
...
void UnscheduledProductResolver::prefetchAsync_(
    WaitingTask* waitTask, Principal const& principal, bool skipCurrentProcess,
    SharedResourcesAcquirer* sra, ModuleCallingContext const* mcc) const {
  if (skipCurrentProcess) {
    return;
  ...
  bool expected = false;
  if (prefetchRequested_.compare_exchange_strong(expected, true)) {
    // Have to create a new task which will make sure the state for
    // UnscheduledProductResolver
    // is properly set after the module has run
    auto t = make_waiting_task(
        tbb::task::allocate_root(), [this, &principal, skipCurrentProcess, sra,
                                     mcc](std::exception_ptr const* iPtr) {
          // The exception is being rethrown because resolveProductImpl sets the
          // ProductResolver to a failed
          // state for the case where an exception occurs during the call to the
          // function.
          try {
    ...
    worker_->doWorkAsync<
        OccurrenceTraits<EventPrincipal, BranchActionStreamBegin> >(
        t, event, *(aux_->eventSetup()), event.streamID(), parentContext,
        mcc->getStreamContext());
  }
}
...
void ProducedProductResolver::putProduct_(
    std::unique_ptr<WrapperBase> edp) const {
  if (status() != defaultStatus()) {
    throw Exception(errors::InsertFailure)
        << "Attempt to insert more than one product on branch "
...
void ProducedProductResolver::resetFailedFromThisProcess() {
  if (ProductStatus::ResolveFailed == status()) {
    resetProductData_(false);
  }
}

void DataManagingProductResolver::connectTo(ProductResolverBase const& iOther,
                                            Principal const*) {
  assert(false);
}

void DataManagingProductResolver::putOrMergeProduct_(
    std::unique_ptr<WrapperBase> prod) const {
  if (not prod) {
    return;
  }
  if (status() == defaultStatus()) {
...
void DataManagingProductResolver::checkType(WrapperBase const& prod) const {
  ...
  if (typeID != branchDescription().unwrappedTypeID()) {
    ...
    throw Exception(errors::EventCorruption)
        << "Product on branch " << branchDescription().branchName()
        << " is of wrong type.\n"
...
// This routine returns true if it is known that currently there is no real
// product.
...
// This routine returns true if the product was deleted early in order to save
// memory
bool DataManagingProductResolver::productWasDeleted_() const {
  return status() == ProductStatus::ProductDeleted;
}

bool DataManagingProductResolver::productWasFetchedAndIsValid_(
...
void DataManagingProductResolver::setProvenance_(
    ProductProvenanceRetriever const* provRetriever, ProcessHistory const& ph,
    ProductID const& pid) {
...
ProductProvenance const* DataManagingProductResolver::productProvenancePtr_()
    const {
...
void AliasProductResolver::setProvenance_(
    ProductProvenanceRetriever const* provRetriever, ProcessHistory const& ph,
    ProductID const& pid) {
...
void AliasProductResolver::putProduct_(std::unique_ptr<WrapperBase>) const {
  throw Exception(errors::LogicError)
      << "AliasProductResolver::putProduct_() not implemented and should never "
         "be called.\n"
      << "Contact a Framework developer\n";
}

void AliasProductResolver::putOrMergeProduct_(
    std::unique_ptr<WrapperBase> edp) const {
  throw Exception(errors::LogicError)
      << "AliasProductResolver::putOrMergeProduct_(std::unique_ptr<WrapperBase>"
         " edp) not implemented and should never be called.\n"
      << "Contact a Framework developer\n";
}

void ParentProcessProductResolver::setProvenance_(
    ProductProvenanceRetriever const* provRetriever, ProcessHistory const& ph,
    ProductID const& pid) {
  provRetriever_ = provRetriever;
}

void ParentProcessProductResolver::setProcessHistory_(
    ProcessHistory const& ph) {}

ProductProvenance const* ParentProcessProductResolver::productProvenancePtr_()
    const {
  return provRetriever_
             ? provRetriever_->branchIDToProvenance(bd_->originalBranchID())
             : nullptr;
}
...
void ParentProcessProductResolver::putProduct_(
    std::unique_ptr<WrapperBase>) const {
  throw Exception(errors::LogicError)
      << "ParentProcessProductResolver::putProduct_() not implemented and "
         "should never be called.\n"
      << "Contact a Framework developer\n";
}

void ParentProcessProductResolver::putOrMergeProduct_(
    std::unique_ptr<WrapperBase> edp) const {
  throw Exception(errors::LogicError)
      << "ParentProcessProductResolver::putOrMergeProduct_(std::unique_ptr<"
         "WrapperBase> edp) not implemented and should never be called.\n"
      << "Contact a Framework developer\n";
}
...
void ParentProcessProductResolver::throwNullRealProduct() const {
  ...
  throw Exception(errors::LogicError)
      << "ParentProcessProductResolver::throwNullRealProduct RealProduct "
         "pointer not set in this context.\n"
      << "Contact a Framework developer\n";
}

NoProcessProductResolver::NoProcessProductResolver(
...
ProductResolverBase::Resolution NoProcessProductResolver::tryResolver(
    unsigned int index, Principal const& principal, bool skipCurrentProcess,
    SharedResourcesAcquirer* sra, ModuleCallingContext const* mcc) const {
  ProductResolverBase const* productResolver =
      principal.getProductResolverByIndex(matchingHolders_[index]);
  return productResolver->resolveProduct(principal, skipCurrentProcess, sra,
                                         mcc);
}

ProductResolverBase::Resolution NoProcessProductResolver::resolveProduct_(
    Principal const& principal, bool skipCurrentProcess,
    SharedResourcesAcquirer* sra, ModuleCallingContext const* mcc) const {
  ...
  {
    if ((not principal.atEndTransition()) or recheckedAtEnd_) {
      unsigned int checkCacheIndex = skipCurrentProcess
                                         ? lastSkipCurrentCheckIndex_.load()
                                         : lastCheckIndex_.load();
      if (checkCacheIndex != choiceSize + kUnsetOffset) {
        ...
        return tryResolver(checkCacheIndex, principal, skipCurrentProcess, sra,
                           mcc);
      }
    }
  }

  std::atomic<unsigned int>& updateCacheIndex =
      skipCurrentProcess ? lastSkipCurrentCheckIndex_ : lastCheckIndex_;
  ...
  TrueGuard guard(principal.atEndTransition() ? &recheckedAtEnd_ : nullptr,
                  setTrue);

  std::vector<unsigned int> const& lookupProcessOrder =
      principal.lookupProcessOrder();
...
void NoProcessProductResolver::prefetchAsync_(
    WaitingTask* waitTask, Principal const& principal, bool skipCurrentProcess,
    SharedResourcesAcquirer* sra, ModuleCallingContext const* mcc) const {
  if (not skipCurrentProcess) {
    waitingTasks_.add(waitTask);
    ...
    if (principal.atEndTransition()) {
      bool expected = false;
      needToRecheckAtEnd =
          recheckedAtEnd_.compare_exchange_strong(expected, true);
...
class TryNextResolverWaitingTask : public edm::WaitingTask {
 public:
  TryNextResolverWaitingTask(NoProcessProductResolver const* iResolver,
                             ...
                             bool iSkipCurrentProcess, ServiceToken iToken)
      : resolver_(iResolver),
  ...
  tbb::task* execute() override {
    ...
    if (exceptPtr) {
      resolver_->prefetchFailed(index_, *principal_, skipCurrentProcess_,
                                *exceptPtr);
    } else {
      if (not resolver_->dataValidFromResolver(index_, *principal_,
                                               skipCurrentProcess_)) {
        resolver_->tryPrefetchResolverAsync(index_ + 1, *principal_,
                                            skipCurrentProcess_, sra_, mcc_,
                                            serviceToken_);
      }
    }
    return nullptr;
...
void NoProcessProductResolver::prefetchFailed(
    unsigned int iProcessingIndex, Principal const& principal,
    bool iSkipCurrentProcess, std::exception_ptr iExceptPtr) const {
  std::vector<unsigned int> const& lookupProcessOrder =
      principal.lookupProcessOrder();
...
bool NoProcessProductResolver::dataValidFromResolver(
    unsigned int iProcessingIndex, Principal const& principal,
    bool iSkipCurrentProcess) const {
  std::vector<unsigned int> const& lookupProcessOrder =
      principal.lookupProcessOrder();
  auto k = lookupProcessOrder[iProcessingIndex];
  ProductResolverBase const* productResolver =
      principal.getProductResolverByIndex(matchingHolders_[k]);
...
void NoProcessProductResolver::tryPrefetchResolverAsync(
    unsigned int iProcessingIndex, Principal const& principal,
    bool skipCurrentProcess, SharedResourcesAcquirer* sra,
    ModuleCallingContext const* mcc, ServiceToken token) const {
  std::vector<unsigned int> const& lookupProcessOrder =
      principal.lookupProcessOrder();
  ...
  while (index < lookupProcessOrder.size()) {
    ...
    if (matchingHolders_[k] != ProductResolverIndexInvalid) {
      ...
      auto task = new (tbb::task::allocate_root()) TryNextResolverWaitingTask(
          this, index, &principal, sra, mcc, skipCurrentProcess, token);
      ...
      productResolver->prefetchAsync(task, principal, skipCurrentProcess, sra,
                                     mcc);
...
ProductProvenance const* NoProcessProductResolver::productProvenancePtr_()
    const {
  return nullptr;
}

inline unsigned int NoProcessProductResolver::unsetIndexValue() const {
  return ambiguous_.size() + kUnsetOffset;
}
...
bool NoProcessProductResolver::unscheduledWasNotRun_() const {
  throw Exception(errors::LogicError)
      << "NoProcessProductResolver::unscheduledWasNotRun_() not implemented "
...
bool NoProcessProductResolver::productUnavailable_() const {
  throw Exception(errors::LogicError)
      << "NoProcessProductResolver::productUnavailable_() not implemented and "
         "should never be called.\n"
      << "Contact a Framework developer\n";
}
...
bool NoProcessProductResolver::productResolved_() const {
  throw Exception(errors::LogicError)
      << "NoProcessProductResolver::productResolved_() not implemented and "
         "should never be called.\n"
      << "Contact a Framework developer\n";
}
...
bool NoProcessProductResolver::productWasDeleted_() const {
  throw Exception(errors::LogicError)
      << "NoProcessProductResolver::productWasDeleted_() not implemented and "
         "should never be called.\n"
      << "Contact a Framework developer\n";
}

bool NoProcessProductResolver::productWasFetchedAndIsValid_(
    bool /*iSkipCurrentProcess*/) const {
  throw Exception(errors::LogicError)
      << "NoProcessProductResolver::productWasFetchedAndIsValid_() not "
...
void NoProcessProductResolver::putProduct_(std::unique_ptr<WrapperBase>) const {
  throw Exception(errors::LogicError)
      << "NoProcessProductResolver::putProduct_() not implemented and should "
         "never be called.\n"
      << "Contact a Framework developer\n";
}

void NoProcessProductResolver::putOrMergeProduct_(
    std::unique_ptr<WrapperBase> edp) const {
  throw Exception(errors::LogicError)
      << "NoProcessProductResolver::putOrMergeProduct_(std::unique_ptr<"
         "WrapperBase> edp) not implemented and should never be called.\n"
      << "Contact a Framework developer\n";
}
...
BranchDescription const& NoProcessProductResolver::branchDescription_() const {
  throw Exception(errors::LogicError)
      << "NoProcessProductResolver::branchDescription_() not implemented and "
         "should never be called.\n"
      << "Contact a Framework developer\n";
}

void NoProcessProductResolver::resetBranchDescription_(
    std::shared_ptr<BranchDescription const>) {
  throw Exception(errors::LogicError)
      << "NoProcessProductResolver::resetBranchDescription_() not implemented "
         "and should never be called.\n"
      << "Contact a Framework developer\n";
}
...
Provenance const* NoProcessProductResolver::provenance_() const {
  throw Exception(errors::LogicError)
      << "NoProcessProductResolver::provenance_() not implemented and should "
         "never be called.\n"
      << "Contact a Framework developer\n";
}

void NoProcessProductResolver::connectTo(ProductResolverBase const&,
                                         Principal const*) {
  throw Exception(errors::LogicError)
      << "NoProcessProductResolver::connectTo() not implemented and should "
         "never be called.\n"
      << "Contact a Framework developer\n";
}
...
SingleChoiceNoProcessProductResolver::resolveProduct_(
    Principal const& principal, bool skipCurrentProcess,
    SharedResourcesAcquirer* sra, ModuleCallingContext const* mcc) const {
...
void SingleChoiceNoProcessProductResolver::prefetchAsync_(
    WaitingTask* waitTask, Principal const& principal, bool skipCurrentProcess,
    SharedResourcesAcquirer* sra, ModuleCallingContext const* mcc) const {
  principal.getProductResolverByIndex(realResolverIndex_)
      ->prefetchAsync(waitTask, principal, skipCurrentProcess, sra, mcc);
}

void SingleChoiceNoProcessProductResolver::setProvenance_(
    ProductProvenanceRetriever const*, ProcessHistory const&,
    ProductID const&) {}

void SingleChoiceNoProcessProductResolver::setProcessHistory_(
    ProcessHistory const&) {}

ProductProvenance const*
SingleChoiceNoProcessProductResolver::productProvenancePtr_() const {
  return nullptr;
}
...
bool SingleChoiceNoProcessProductResolver::unscheduledWasNotRun_() const {
  throw Exception(errors::LogicError)
      << "SingleChoiceNoProcessProductResolver::unscheduledWasNotRun_() not "
         "implemented and should never be called.\n"
      << "Contact a Framework developer\n";
}
...
bool SingleChoiceNoProcessProductResolver::productUnavailable_() const {
  throw Exception(errors::LogicError)
      << "SingleChoiceNoProcessProductResolver::productUnavailable_() not "
         "implemented and should never be called.\n"
      << "Contact a Framework developer\n";
}
...
bool SingleChoiceNoProcessProductResolver::productResolved_() const {
  throw Exception(errors::LogicError)
      << "SingleChoiceNoProcessProductResolver::productResolved_() not "
         "implemented and should never be called.\n"
      << "Contact a Framework developer\n";
}
...
bool SingleChoiceNoProcessProductResolver::productWasDeleted_() const {
  throw Exception(errors::LogicError)
      << "SingleChoiceNoProcessProductResolver::productWasDeleted_() not "
         "implemented and should never be called.\n"
      << "Contact a Framework developer\n";
}

bool SingleChoiceNoProcessProductResolver::productWasFetchedAndIsValid_(
    bool /*iSkipCurrentProcess*/) const {
  throw Exception(errors::LogicError)
      << "SingleChoiceNoProcessProductResolver::productWasFetchedAndIsValid_() "
         "not implemented and should never be called.\n"
      << "Contact a Framework developer\n";
}

void SingleChoiceNoProcessProductResolver::putProduct_(
    std::unique_ptr<WrapperBase>) const {
  throw Exception(errors::LogicError)
      << "SingleChoiceNoProcessProductResolver::putProduct_() not implemented "
         "and should never be called.\n"
      << "Contact a Framework developer\n";
}

void SingleChoiceNoProcessProductResolver::putOrMergeProduct_(
    std::unique_ptr<WrapperBase> edp) const {
  throw Exception(errors::LogicError)
      << "SingleChoiceNoProcessProductResolver::putOrMergeProduct_(std::unique_"
         "ptr<WrapperBase> edp) not implemented and should never be called.\n"
      << "Contact a Framework developer\n";
}

BranchDescription const&
SingleChoiceNoProcessProductResolver::branchDescription_() const {
  throw Exception(errors::LogicError)
      << "SingleChoiceNoProcessProductResolver::branchDescription_() not "
         "implemented and should never be called.\n"
      << "Contact a Framework developer\n";
}

void SingleChoiceNoProcessProductResolver::resetBranchDescription_(
    std::shared_ptr<BranchDescription const>) {
  throw Exception(errors::LogicError)
      << "SingleChoiceNoProcessProductResolver::resetBranchDescription_() not "
         "implemented and should never be called.\n"
      << "Contact a Framework developer\n";
}
...
Provenance const* SingleChoiceNoProcessProductResolver::provenance_() const {
  throw Exception(errors::LogicError)
      << "SingleChoiceNoProcessProductResolver::provenance_() not implemented "
         "and should never be called.\n"
      << "Contact a Framework developer\n";
}

void SingleChoiceNoProcessProductResolver::connectTo(ProductResolverBase const&,
                                                     Principal const*) {
  throw Exception(errors::LogicError)
      << "SingleChoiceNoProcessProductResolver::connectTo() not implemented "
         "and should never be called.\n"
      << "Contact a Framework developer\n";
}
}
