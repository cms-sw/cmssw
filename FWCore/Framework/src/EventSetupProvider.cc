...
class KnownRecordsSupplierImpl : public EventSetupKnownRecordsSupplier {
 public:
  using Map =
      std::map<EventSetupRecordKey, std::shared_ptr<EventSetupRecordProvider> >;
...
EventSetupProvider::EventSetupProvider(unsigned subProcessIndex,
    ...
    : eventSetup_(),
      ...
      preferredProviderInfo_((0 != iInfo) ? (new PreferredProviderInfo(*iInfo))
                                          : 0),
      finders_(
          new std::vector<std::shared_ptr<EventSetupRecordIntervalFinder> >()),
      dataProviders_(new std::vector<std::shared_ptr<DataProxyProvider> >()),
      referencedDataKeys_(
          new std::map<EventSetupRecordKey,
                       std::map<DataKey, ComponentDescription const*> >),
      recordToFinders_(
          new std::map<
              EventSetupRecordKey,
              std::vector<std::shared_ptr<EventSetupRecordIntervalFinder> > >),
      psetIDToRecordKey_(
          new std::map<ParameterSetIDHolder, std::set<EventSetupRecordKey> >),
      recordToPreferred_(
          new std::map<EventSetupRecordKey,
                       std::map<DataKey, ComponentDescription> >),
...
//
// const EventSetupProvider& EventSetupProvider::operator=(const
// EventSetupProvider& rhs)
// {
...
// member functions
//
void EventSetupProvider::insert(
...
void EventSetupProvider::add(
    std::shared_ptr<EventSetupRecordIntervalFinder> iFinder) {
  assert(iFinder.get() != 0);
  finders_->push_back(iFinder);
}

typedef std::map<EventSetupRecordKey,
                 std::shared_ptr<EventSetupRecordProvider> >
    Providers;
typedef std::map<EventSetupRecordKey,
                 EventSetupRecordProvider::DataToPreferredProviderMap>
    RecordToPreferred;
/// find everything made by a DataProxyProvider and add it to the 'preferred'
/// list
static void preferEverything(const ComponentDescription& iComponent,
    ...
    if (components.find(iComponent) != components.end()) {
      ...
      for (std::set<EventSetupRecordKey>::iterator itRecord = records.begin(),
        ...
        if (!keyedProxies.empty()) {
          ...
          EventSetupRecordProvider::DataToPreferredProviderMap&
              dataToProviderMap = iReturnValue[*itRecord];

          for (DataProxyProvider::KeyedProxies::const_iterator
                   itProxy = keyedProxies.begin(),
                   itProxyEnd = keyedProxies.end();
               itProxy != itProxyEnd; ++itProxy) {
            EventSetupRecordProvider::DataToPreferredProviderMap::iterator
                itFind = dataToProviderMap.find(itProxy->first);
            if (itFind != dataToProviderMap.end()) {
              throw cms::Exception("ESPreferConflict")
                  << "Two providers have been set to be preferred for\n"
                  << itProxy->first.type().name() << " \""
                  << itProxy->first.name().value() << "\""
                  << "\n the providers are "
                  << "\n 1) type=" << itFind->second.type_ << " label=\""
                  << itFind->second.label_ << "\""
                  << "\n 2) type=" << iComponent.type_ << " label=\""
                  << iComponent.label_ << "\""
                  << "\nPlease modify configuration so only one is preferred";
            }
            dataToProviderMap.insert(
                std::make_pair(itProxy->first, iComponent));
          }
        }
      }
  ...
  if (!foundProxyProvider) {
    throw cms::Exception("ESPreferNoProvider")
        << "Could not make type=\"" << iComponent.type_ << "\" label=\""
        << iComponent.label_ << "\" a preferred Provider."
        << "\n  Please check spelling of name, or that it was loaded into the "
           "job.";
  }
}
static RecordToPreferred determinePreferred(
    const EventSetupProvider::PreferredProviderInfo* iInfo,
    const Providers& iProviders) {
  using namespace edm::eventsetup;
  RecordToPreferred returnValue;
  if (0 != iInfo) {
    for (EventSetupProvider::PreferredProviderInfo::const_iterator
             itInfo = iInfo->begin(),
             itInfoEnd = iInfo->end();
      ...
      if (itInfo->second.empty()) {
        ...
        for (EventSetupProvider::RecordToDataMap::const_iterator
          ...
          Providers::const_iterator itRecordProvider =
              iProviders.find(recordKey);
          ...
          if (itProxyProv == components.end()) {
            throw cms::Exception("ESPreferWrongRecord")
                << "The type=" << itInfo->first.type_ << " label=\""
                << itInfo->first.label_
          ...
          if (std::find_if(
                  keyedProxies.begin(), keyedProxies.end(),
                  boost::bind(
                      std::equal_to<DataKey>(), datumKey,
                      boost::bind(
                          &DataProxyProvider::KeyedProxies::value_type::first,
                          _1))) == keyedProxies.end()) {
          ...
          if (dataToProviderMap.end() != dataToProviderMap.find(datumKey)) {
            ...
            throw cms::Exception("ESPreferConflict")
                ...
                << "\n 1) type=" << itFind->second.type_ << " label=\""
                << itFind->second.label_ << "\""
                << "\n 2) type=" << itProxyProv->type_ << " label=\""
                << itProxyProv->label_ << "\""
                << "\nPlease modify configuration so only one is preferred";
          }
...
void EventSetupProvider::finishConfiguration() {
  // we delayed adding finders to the system till here so that everything would
  // be loaded first
  ...
  // Now handle providers since sources can also be finders and the sources can
  // delay registering
  ...
  // used for the case where no preferred Providers have been specified for the
  // Record
  static const EventSetupRecordProvider::DataToPreferredProviderMap kEmptyMap;

  *recordToPreferred_ =
      determinePreferred(preferredProviderInfo_.get(), providers_);
  // For each Provider, find all the Providers it depends on.  If a dependent
  // Provider
  ...
  for (Providers::iterator itProvider = providers_.begin(),
       ...
       itProvider != itProviderEnd; ++itProvider) {
    const EventSetupRecordProvider::DataToPreferredProviderMap* preferredInfo =
        &kEmptyMap;
    ...
    std::set<EventSetupRecordKey> records =
        itProvider->second->dependentRecords();
    if (records.size() != 0) {
      ...
      if (!foundAllProviders) {
        edm::LogInfo("EventSetupDependency")
            ...
            << ") which is not present in the job."
               "\n This may lead to an exception begin thrown during event "
               "processing.\n If no exception occurs during the job than it is "
               "usually safe to ignore this message.";
...
static void findDependents(
  ...
  for (Itr it = itBegin; it != itEnd; ++it) {
    ...
    if (deps.end() != deps.find(iKey)) {
      ...
      // now see who is dependent on this record since they will be indirectly
      // dependent on iKey
...
void EventSetupProvider::resetRecordPlusDependentRecords(
    const EventSetupRecordKey& iKey) {
  ...
  for_all(dependents, boost::bind(&EventSetupRecordProvider::resetProxies, _1));
}
...
void EventSetupProvider::checkESProducerSharing(
  ...
  edm::LogVerbatim("EventSetupSharing")
      << "EventSetupProvider::checkESProducerSharing: Checking processes with "
         "SubProcess Indexes "
  ...
  for (auto const& iRecord : referencedESProducers) {
    for (auto const& iComponent : iRecord.second) {
      ...
      esController.lookForMatches(
          psetID, subProcessIndex_, precedingESProvider.subProcessIndex_,
      ...
      if (iCandidateNotRejectedYet->second == true) {
        ...
        if (iAllComponentsMatch == allComponentsMatch.end()) {
          ...
          // now.
          bool match = doRecordsMatch(precedingESProvider, iRecord.first,
                                      allComponentsMatch, esController);
  ...
  for (auto const& candidate : candidateNotRejectedYet) {
    ...
    if (canBeShared) {
      ParameterSet const& pset =
          *esController.getESProducerPSet(psetID, subProcessIndex_);
      ...
      std::set<EventSetupRecordKey> const& keysForPSetID2 =
...
bool EventSetupProvider::doRecordsMatch(
    EventSetupProvider& precedingESProvider,
  ...
  for (auto const& finder : (*recordToFinders_)[eventSetupRecordKey]) {
    ...
    bool itMatches = esController.isMatchingESSource(
        psetID, subProcessIndex_, precedingESProvider.subProcessIndex_);
    if (!itMatches) {
      return false;
    }
  ...
  for (auto const& dataItem : dataItems) {
    ...
    if (!dataItem.second->isSource_ && !dataItem.second->isLooper_) {
      bool itMatches = esController.isMatchingESProducer(
          dataItem.second->pid_, subProcessIndex_,
          precedingESProvider.subProcessIndex_);
      if (!itMatches) {
        return false;
      }
  ...
  if (itFound != providers_.end()) {
    std::set<EventSetupRecordKey> dependentRecords =
        itFound->second->dependentRecords();
    for (auto const& dependentRecord : dependentRecords) {
      ...
      bool match = doRecordsMatch(precedingESProvider, dependentRecord,
                                  allComponentsMatch, esController);
...
void EventSetupProvider::fillReferencedDataKeys(
    EventSetupRecordKey const& eventSetupRecordKey) {
  if (referencedDataKeys_->find(eventSetupRecordKey) !=
      referencedDataKeys_->end())
    return;
  ...
  if (recordProvider->second) {
    recordProvider->second->fillReferencedDataKeys(
        (*referencedDataKeys_)[eventSetupRecordKey]);
  }
}
...
//
EventSetup const& EventSetupProvider::eventSetupForInstance(
...
std::set<ComponentDescription> EventSetupProvider::proxyProviderDescriptions()
    const {
  using boost::bind;
  typedef std::set<ComponentDescription> Set;
  Set descriptions;
  ...
  if (dataProviders_.get()) {
    for (std::vector<std::shared_ptr<DataProxyProvider> >::const_iterator
             it = dataProviders_->begin(),
             itEnd = dataProviders_->end();
         it != itEnd; ++it) {
...
//
void EventSetupProvider::logInfoWhenSharing(
    ParameterSet const& iConfiguration) {
  std::string edmtype =
      iConfiguration.getParameter<std::string>("@module_edm_type");
  std::string modtype =
      iConfiguration.getParameter<std::string>("@module_type");
