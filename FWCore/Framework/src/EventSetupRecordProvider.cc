...
namespace edm {
  namespace eventsetup {
    ...
    void EventSetupRecordProvider::setDependentProviders(
      ...
      for_all(iProviders,
              std::bind(std::mem_fun(&DependentRecordIntervalFinder::addProviderWeAreDependentOn), &(*newFinder), _1));
      // if a finder was already set, add it as a depedency.  This is done to ensure that the IOVs properly change even
      // if the
    ...
    void EventSetupRecordProvider::usePreferred(const DataToPreferredProviderMap& iMap) {
      ...
      if (1 < multipleFinders_->size()) {
        std::shared_ptr<IntersectingIOVRecordIntervalFinder> intFinder =
            std::make_shared<IntersectingIOVRecordIntervalFinder>(key_);
