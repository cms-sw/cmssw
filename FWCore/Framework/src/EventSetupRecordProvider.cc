...
EventSetupRecordProvider::EventSetupRecordProvider(
    ...
    : key_(iKey),
      ...
      finder_(),
      providers_(),
      multipleFinders_(new std::vector<edm::propagate_const<
...
//
// const EventSetupRecordProvider& EventSetupRecordProvider::operator=(const
// EventSetupRecordProvider& rhs)
// {
...
// member functions
//
void EventSetupRecordProvider::add(
    std::shared_ptr<DataProxyProvider> iProvider) {
...
void EventSetupRecordProvider::addFinder(
  ...
  if (nullptr != multipleFinders_.get()) {
    ...
    // dependent records set there finders after the multipleFinders_ has been
    // released
    ...
    if (nullptr != oldFinder.get()) {
      cms::Exception("EventSetupMultipleSources")
          ...
          << "'\n"
          << "after all the other sources have been dealt with.  This is a "
             "logic error, please send email to the framework group.";
    }
  }
}
void EventSetupRecordProvider::setValidityInterval(
...
void EventSetupRecordProvider::setDependentProviders(
  ...
  std::shared_ptr<DependentRecordIntervalFinder> newFinder =
      std::shared_ptr<DependentRecordIntervalFinder>(
  ...
  for_all(iProviders,
          std::bind(
              std::mem_fun(
                  &DependentRecordIntervalFinder::addProviderWeAreDependentOn),
              &(*newFinder), _1));
  // if a finder was already set, add it as a depedency.  This is done to ensure
  // that the IOVs properly change even if the
  // old finder does not update each time a dependent record does change
  if (old.get() != nullptr) {
    newFinder->setAlternateFinder(old);
  }
}
void EventSetupRecordProvider::usePreferred(
    const DataToPreferredProviderMap& iMap) {
  using std::placeholders::_1;
  for_all(providers_,
          std::bind(&EventSetupRecordProvider::addProxiesToRecordHelper, this,
                    _1, iMap));
  if (1 < multipleFinders_->size()) {
    std::shared_ptr<IntersectingIOVRecordIntervalFinder> intFinder =
        std::shared_ptr<IntersectingIOVRecordIntervalFinder>(
            new IntersectingIOVRecordIntervalFinder(key_));
...
void EventSetupRecordProvider::addProxiesToRecord(
    std::shared_ptr<DataProxyProvider> iProvider,
  ...
  for (ProxyList::const_iterator keyedProxy(keyedProxies.begin());
       keyedProxy != finishedProxyList; ++keyedProxy) {
    PreferredMap::const_iterator itFound = iMap.find(keyedProxy->first);
    if (iMap.end() != itFound) {
      if (itFound->second.type_ !=
              keyedProxy->second->providerDescription()->type_ ||
          itFound->second.label_ !=
              keyedProxy->second->providerDescription()->label_) {
...
void EventSetupRecordProvider::addRecordTo(
    EventSetupProvider& iEventSetupProvider) {
  EventSetupRecord& rec = record();
...
void EventSetupRecordProvider::resetTransients() {
  ...
  if (checkResetTransients()) {
    for_all(providers_,
            std::bind(&DataProxyProvider::resetProxiesIfTransient, _1, key_));
  }
}

void EventSetupRecordProvider::addRecordToIfValid(
    EventSetupProvider& iEventSetupProvider, const IOVSyncValue& iTime) {
  if (setValidityIntervalFor(iTime)) {
    addRecordTo(iEventSetupProvider);
  }
}

bool EventSetupRecordProvider::setValidityIntervalFor(
    const IOVSyncValue& iTime) {
  // we want to wait until after the first event of a new run before
  // we reset any transients just in case some modules get their data at
  // beginRun or beginLumi
...
void EventSetupRecordProvider::resetRecordToProxyPointers(
...
std::set<EventSetupRecordKey> EventSetupRecordProvider::dependentRecords()
    const {
...
std::shared_ptr<DataProxyProvider> EventSetupRecordProvider::proxyProvider(
    ComponentDescription const& iDesc) {
  using std::placeholders::_1;
  auto itFound =
      std::find_if(providers_.begin(), providers_.end(),
                   std::bind(std::equal_to<ComponentDescription>(), iDesc,
                             std::bind(&DataProxyProvider::description, _1)));
