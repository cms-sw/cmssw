...
class GlobalScheduleSignalSentry {
 public:
  GlobalScheduleSignalSentry(ActivityRegistry* a,
 ...
 private:
  ...
  ActivityRegistry* a_;  // We do not use propagate_const because the registry
                         // itself is mutable.
...
class GlobalSchedule {
 public:
  ...
  GlobalSchedule(
      ...
      std::vector<edm::propagate_const<std::shared_ptr<PathStatusInserter>>>&
          pathStatusInserters,
      std::vector<edm::propagate_const<std::shared_ptr<EndPathStatusInserter>>>&
          endPathStatusInserters,
      std::shared_ptr<ModuleRegistry> modReg,
      std::vector<std::string> const& modulesToUse, ParameterSet& proc_pset,
      ProductRegistry& pregistry, PreallocationConfiguration const& prealloc,
 ...
 private:
  ...
  class SendTerminationSignalIfException {
   public:
    SendTerminationSignalIfException(edm::ActivityRegistry* iReg,
                                     edm::GlobalContext const* iContext)
        : reg_(iReg), context_(iContext) {}
    ~SendTerminationSignalIfException() {
      if (reg_) {
        reg_->preGlobalEarlyTerminationSignal_(
            *context_, TerminationOrigin::ExceptionFromThisContext);
      }
    }
    void completedSuccessfully() { reg_ = nullptr; }

   private:
    edm::ActivityRegistry* reg_;  // We do not use propagate_const because the
                                  // registry itself is mutable.
  ...
  std::shared_ptr<ActivityRegistry> actReg_;  // We do not use propagate_const
                                              // because the registry itself is
                                              // mutable.
...
template <typename T>
void GlobalSchedule::processOneGlobal(typename T::MyPrincipal& ep,
                                      EventSetup const& es,
  ...
  SendTerminationSignalIfException terminationSentry(actReg_.get(),
                                                     &globalContext);

  // If we are in an end transition, we need to reset failed items since they
  // might
  ...
  workerManager_.processOneOccurrence<T>(ep, es, StreamID::invalidStreamID(),
                                         &globalContext, &globalContext,
                                         cleaningUpAfterException);

  try {
    convertException::wrap([&]() { runNow<T>(ep, es, &globalContext); });
  } catch (cms::Exception& ex) {
    if (ex.context().empty()) {
      addContextAndPrintException(
          "Calling function GlobalSchedule::processOneGlobal", ex,
          cleaningUpAfterException);
    } else {
  ...
  // If we got here no other exception has happened so we can propogate any
...
void GlobalSchedule::processOneGlobalAsync(WaitingTaskHolder iHolder,
  ...
  auto globalContext = std::make_shared<GlobalContext>(
      T::makeGlobalContext(ep, processContext_));
       ...
       token](std::exception_ptr const* iPtr) mutable {
        ...
        if (iPtr) {
          ...
          try {
            ...
            addContextAndPrintException(ost.str().c_str(), ex,
                                        cleaningUpAfterException);
            excpt = std::current_exception();
          }
          if (actReg_) {
            actReg_->preGlobalEarlyTerminationSignal_(
                *globalContext, TerminationOrigin::ExceptionFromThisContext);
          }
        }
        if (actReg_) {
          try {
  ...
  for (auto& worker : boost::adaptors::reverse((allWorkers()))) {
    worker->doWorkAsync<T>(doneTask, ep, es, StreamID::invalidStreamID(),
                           parentContext, globalContext.get());
  }
}

template <typename T>
void GlobalSchedule::runNow(typename T::MyPrincipal const& p,
                            EventSetup const& es,
                            GlobalContext const* context) {
  ...
  for (auto& worker : allWorkers()) {
    try {
      ...
      worker->doWork<T>(p, es, StreamID::invalidStreamID(), parentContext,
                        context);
    } catch (cms::Exception& ex) {
      if (ex.context().empty()) {
        std::ostringstream ost;
