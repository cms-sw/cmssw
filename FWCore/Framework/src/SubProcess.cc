...
namespace edm {
  ...
  SubProcess::SubProcess(ParameterSet& parameterSet, ParameterSet const& topLevelParameterSet,
                         ...
                         eventsetup::EventSetupsController& esController, ActivityRegistry& parentActReg,
                         ServiceToken const& token, serviceregistry::ServiceLegacy iLegacy,
                         PreallocationConfiguration const& preallocConfig, ProcessContext const* parentProcessContext)
      : EDConsumerBase(),
        serviceToken_(),
  ...
  void SubProcess::processAsync(WaitingTaskHolder iHolder, EventPrincipal const& principal) {
    ...
    WaitingTaskHolder finalizeEventTask(
        make_waiting_task(tbb::task::allocate_root(), [this, &ep, iHolder](std::exception_ptr const* iPtr) mutable {
  ...
  void SubProcess::doBeginRunAsync(WaitingTaskHolder iHolder, RunPrincipal const& principal, IOVSyncValue const& ts) {
    ...
    beginGlobalTransitionAsync<Traits>(std::move(iHolder), *schedule_, rp, ts, esp_->eventSetupForInstance(ts),
                                       subProcesses_);
  }

  void SubProcess::doEndRunAsync(WaitingTaskHolder iHolder, RunPrincipal const& principal, IOVSyncValue const& ts,
    ...
    endGlobalTransitionAsync<Traits>(std::move(iHolder), *schedule_, rp, ts, esp_->eventSetupForInstance(ts),
  ...
  void SubProcess::doBeginLuminosityBlockAsync(WaitingTaskHolder iHolder, LuminosityBlockPrincipal const& principal,
                                               IOVSyncValue const& ts) {
    ServiceRegistry::Operate operate(serviceToken_);

    auto aux = std::make_shared<LuminosityBlockAuxiliary>(principal.aux());
    aux->setProcessHistoryID(principal.processHistoryID());
    auto lbpp = std::make_shared<LuminosityBlockPrincipal>(aux, preg_, *processConfiguration_,
    ...
    beginGlobalTransitionAsync<Traits>(std::move(iHolder), *schedule_, lbp, ts, esp_->eventSetupForInstance(ts),
                                       subProcesses_);
  }

  void SubProcess::doEndLuminosityBlockAsync(WaitingTaskHolder iHolder, LuminosityBlockPrincipal const& principal,
    ...
    endGlobalTransitionAsync<Traits>(std::move(iHolder), *schedule_, lbp, ts, esp_->eventSetupForInstance(ts),
                                     subProcesses_, cleaningUpAfterException);
  }

  void SubProcess::writeLumi(ProcessHistoryID const& parentPhID, int runNumber, int lumiNumber) {
    ServiceRegistry::Operate operate(serviceToken_);
    std::map<ProcessHistoryID, ProcessHistoryID>::const_iterator it = parentToChildPhID_.find(parentPhID);
    assert(it != parentToChildPhID_.end());
    auto const& childPhID = it->second;
  ...
  void SubProcess::deleteLumiFromCache(ProcessHistoryID const& parentPhID, int runNumber, int lumiNumber) {
    std::map<ProcessHistoryID, ProcessHistoryID>::const_iterator it = parentToChildPhID_.find(parentPhID);
    assert(it != parentToChildPhID_.end());
    auto const& childPhID = it->second;
  ...
  void SubProcess::doStreamBeginRunAsync(WaitingTaskHolder iHolder, unsigned int id, RunPrincipal const& principal,
                                         IOVSyncValue const& ts) {
    ServiceRegistry::Operate operate(serviceToken_);
    ...
    beginStreamTransitionAsync<Traits>(std::move(iHolder), *schedule_, id, rp, ts, esp_->eventSetupForInstance(ts),
                                       subProcesses_);
  }

  void SubProcess::doStreamEndRunAsync(WaitingTaskHolder iHolder, unsigned int id, RunPrincipal const& principal,
                                       IOVSyncValue const& ts, bool cleaningUpAfterException) {
    ServiceRegistry::Operate operate(serviceToken_);
    RunPrincipal& rp = *principalCache_.runPrincipalPtr();
    typedef OccurrenceTraits<RunPrincipal, BranchActionStreamEnd> Traits;

    endStreamTransitionAsync<Traits>(std::move(iHolder), *schedule_, id, rp, ts, esp_->eventSetupForInstance(ts),
                                     subProcesses_, cleaningUpAfterException);
  }

  void SubProcess::doStreamBeginLuminosityBlockAsync(WaitingTaskHolder iHolder, unsigned int id,
    ...
    beginStreamTransitionAsync<Traits>(std::move(iHolder), *schedule_, id, lbp, ts, esp_->eventSetupForInstance(ts),
                                       subProcesses_);
  }

  void SubProcess::doStreamEndLuminosityBlockAsync(WaitingTaskHolder iHolder, unsigned int id,
    ...
    endStreamTransitionAsync<Traits>(std::move(iHolder), *schedule_, id, lbp, ts, esp_->eventSetupForInstance(ts),
                                     subProcesses_, cleaningUpAfterException);
  }
