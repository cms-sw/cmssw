...
SubProcess::SubProcess(
    ParameterSet& parameterSet, ParameterSet const& topLevelParameterSet,
    ...
    ActivityRegistry& parentActReg, ServiceToken const& token,
    ...
    : EDConsumerBase(),
      ...
      historyRunOffset_(historyLumiOffset_ +
                        preallocConfig.numberOfLuminosityBlocks()),
      processHistoryRegistries_(historyRunOffset_ +
                                preallocConfig.numberOfRuns()),
  ...
  wantAllEvents_ = detail::configureEventSelector(
      selectevents, tns->getProcessName(), getAllTriggerNames(), selectors_,
      consumesCollector());
  ...
  selector_config_id_ = detail::registerProperSelectionInfo(
      selectevents, "", outputModulePathPositions,
      parentProductRegistry->anyProductProduced());

  std::map<BranchID, bool> keepAssociation;
  selectProducts(*parentProductRegistry, parentThinnedAssociationsHelper,
                 keepAssociation);
  ...
  processParameterSet_ = std::unique_ptr<ParameterSet>(
      parameterSet.popParameterSet(std::string("process")).release());

  // if this process has a maxEvents or maxLuminosityBlocks parameter set,
  // remove them.
  ...
  // if the top level process has a maxEvents or maxLuminosityBlocks parameter
  // set, add them to this process.
  ...
  // If there are subprocesses, pop the subprocess parameter sets out of the
  // process parameter set
  auto subProcessVParameterSet =
      popSubProcessVParameterSet(*processParameterSet_);
  bool hasSubProcesses = subProcessVParameterSet.size() != 0ull;

  ScheduleItems items(*parentProductRegistry, *this);
  actReg_ = items.actReg_;

  ParameterSet const& optionsPset(
      processParameterSet_->getUntrackedParameterSet("options",
                                                     ParameterSet()));
  IllegalParameters::setThrowAnException(
      optionsPset.getUntrackedParameter<bool>("throwIfIllegalParameter", true));
  ...
  auto serviceSets =
      processParameterSet_->popVParameterSet(std::string("services"));

  ServiceToken newToken = items.initServices(serviceSets, *processParameterSet_,
                                             token, iLegacy, false);
  ...
  thinnedAssociationsHelper_->updateFromParentProcess(
      parentThinnedAssociationsHelper, keepAssociation,
      droppedBranchIDToKeptBranchID_);

  // intialize the Schedule
  schedule_ = items.initSchedule(*processParameterSet_, hasSubProcesses,
                                 preallocConfig, &processContext_);
  ...
  subProcessParentageHelper_->update(parentSubProcessParentageHelper,
                                     *parentProductRegistry);

  // CMS-THREADING this only works since Run/Lumis are synchronous so when
  // principalCache asks for
  // the reducedProcessHistoryID from a full ProcessHistoryID that registry will
  // not be in use by
  // another thread. We really need to change how this is done in the
  // PrincipalCache.
  principalCache_.setProcessHistoryRegistry(
      processHistoryRegistries_[historyRunOffset_]);
  ...
  for (unsigned int index = 0; index < preallocConfig.numberOfStreams();
       ++index) {
    auto ep = std::make_shared<EventPrincipal>(
        preg_, branchIDListHelper(), thinnedAssociationsHelper(),
  ...
  for (auto& subProcessPSet : subProcessVParameterSet) {
    subProcesses_.emplace_back(
        subProcessPSet, topLevelParameterSet, preg_, branchIDListHelper(),
        *thinnedAssociationsHelper_, *subProcessParentageHelper_, esController,
        *items.actReg_, newToken, iLegacy, preallocConfig, &processContext_);
  }
}
...
void SubProcess::beginJob() {
  ...
  // object(s) in the parent process from the event. This next call is needed
  // for
  // getByToken to work properly. Normally, this is done by the worker, but
  // since
...
void SubProcess::endJob() {
  ...
  ExceptionCollector c(
      "Multiple exceptions were thrown while executing endJob. An exception "
      "message follows for each.");
  schedule_->endJob(c);
...
void SubProcess::selectProducts(
...
void SubProcess::keepThisBranch(
...
void SubProcess::fixBranchIDListsForEDAliases(
  ...
  for (BranchIDList& branchIDList :
       branchIDListHelper_->mutableBranchIDLists()) {
...
void SubProcess::processAsync(WaitingTaskHolder iHolder,
  ...
  EventPrincipal& ep =
      principalCache_.eventPrincipal(principal.streamID().value());
  auto& processHistoryRegistry =
      processHistoryRegistries_[principal.streamID().value()];
  ...
  ep.fillEventPrincipal(
      ...
      *(principal.productProvenanceRetrieverPtr()),  // NOTE: this transfers the
  ...
  schedule_->processOneEventAsync(std::move(afterProcessTask),
                                  ep.streamID().value(), ep,
                                  esp_->eventSetup());
}

void SubProcess::doBeginRun(RunPrincipal const& principal,
                            IOVSyncValue const& ts) {
  ServiceRegistry::Operate operate(serviceToken_);
  beginRun(principal, ts);
}

void SubProcess::beginRun(RunPrincipal const& principal,
                          IOVSyncValue const& ts) {
  auto aux = std::make_shared<RunAuxiliary>(principal.aux());
  aux->setProcessHistoryID(principal.processHistoryID());
  auto rpp = std::make_shared<RunPrincipal>(
      aux, preg_, *processConfiguration_,
      &(historyAppenders_[historyRunOffset_ + principal.index()]),
      principal.index(), false);
  auto& processHistoryRegistry =
      processHistoryRegistries_[historyRunOffset_ + principal.index()];
  processHistoryRegistry.registerProcessHistory(principal.processHistory());
  ...
  parentToChildPhID_.insert(
      std::make_pair(parentInputReducedPHID, inputReducedPHID));
  ...
  for_all(subProcesses_,
          [&rp, &ts](auto& subProcess) { subProcess.doBeginRun(rp, ts); });
}
...
void SubProcess::doBeginRunAsync(WaitingTaskHolder iHolder,
  ...
  propagateProducts(InRun, principal, rp);
  typedef OccurrenceTraits<RunPrincipal, BranchActionGlobalBegin> Traits;
  beginGlobalTransitionAsync<Traits>(std::move(iHolder), *schedule_, rp, ts,
                                     esp_->eventSetupForInstance(ts),
                                     subProcesses_);
}

void SubProcess::doEndRun(RunPrincipal const& principal, IOVSyncValue const& ts,
                          bool cleaningUpAfterException) {
  ServiceRegistry::Operate operate(serviceToken_);
  endRun(principal, ts, cleaningUpAfterException);
}

void SubProcess::endRun(RunPrincipal const& principal, IOVSyncValue const& ts,
                        bool cleaningUpAfterException) {
  RunPrincipal& rp = *principalCache_.runPrincipalPtr();
  rp.setComplete();
  propagateProducts(InRun, principal, rp);
  ...
  for_all(subProcesses_,
          [&rp, &ts, cleaningUpAfterException](auto& subProcess) {
...
void SubProcess::doEndRunAsync(WaitingTaskHolder iHolder,
  ...
  typedef OccurrenceTraits<RunPrincipal, BranchActionGlobalEnd> Traits;
  rp.setAtEndTransition(true);
  endGlobalTransitionAsync<Traits>(std::move(iHolder), *schedule_, rp, ts,
                                   esp_->eventSetupForInstance(ts),
                                   subProcesses_, cleaningUpAfterException);
}

void SubProcess::writeRun(ProcessHistoryID const& parentPhID, int runNumber) {
  ServiceRegistry::Operate operate(serviceToken_);
  std::map<ProcessHistoryID, ProcessHistoryID>::const_iterator it =
      parentToChildPhID_.find(parentPhID);
  assert(it != parentToChildPhID_.end());
  auto const& childPhID = it->second;
  schedule_->writeRun(principalCache_.runPrincipal(childPhID, runNumber),
                      &processContext_);
  for_all(subProcesses_, [&childPhID, runNumber](auto& subProcess) {
    subProcess.writeRun(childPhID, runNumber);
  });
}

void SubProcess::deleteRunFromCache(ProcessHistoryID const& parentPhID,
                                    int runNumber) {
  std::map<ProcessHistoryID, ProcessHistoryID>::const_iterator it =
      parentToChildPhID_.find(parentPhID);
  assert(it != parentToChildPhID_.end());
  ...
  for_all(subProcesses_, [&childPhID, runNumber](auto& subProcess) {
    subProcess.deleteRunFromCache(childPhID, runNumber);
  });
}

void SubProcess::doBeginLuminosityBlock(
    LuminosityBlockPrincipal const& principal, IOVSyncValue const& ts) {
  ServiceRegistry::Operate operate(serviceToken_);
  beginLuminosityBlock(principal, ts);
}

void SubProcess::beginLuminosityBlock(LuminosityBlockPrincipal const& principal,
                                      IOVSyncValue const& ts) {
  auto aux = std::make_shared<LuminosityBlockAuxiliary>(principal.aux());
  aux->setProcessHistoryID(principal.processHistoryID());
  auto lbpp = std::make_shared<LuminosityBlockPrincipal>(
      aux, preg_, *processConfiguration_,
      &(historyAppenders_[historyLumiOffset_ + principal.index()]),
      principal.index(), false);
  auto& processHistoryRegistry =
      processHistoryRegistries_[historyLumiOffset_ + principal.index()];
  processHistoryRegistry.registerProcessHistory(principal.processHistory());
  lbpp->fillLuminosityBlockPrincipal(processHistoryRegistry,
                                     principal.reader());
  ...
  typedef OccurrenceTraits<LuminosityBlockPrincipal, BranchActionGlobalBegin>
      Traits;
  schedule_->processOneGlobal<Traits>(lbp, esp_->eventSetupForInstance(ts));
  for_all(subProcesses_, [&lbp, &ts](auto& subProcess) {
    subProcess.doBeginLuminosityBlock(lbp, ts);
  });
}

void SubProcess::doBeginLuminosityBlockAsync(
    WaitingTaskHolder iHolder, LuminosityBlockPrincipal const& principal,
    IOVSyncValue const& ts) {
  ServiceRegistry::Operate operate(serviceToken_);
  ...
  typedef OccurrenceTraits<LuminosityBlockPrincipal, BranchActionGlobalBegin>
      Traits;
  beginGlobalTransitionAsync<Traits>(std::move(iHolder), *schedule_, lbp, ts,
                                     esp_->eventSetupForInstance(ts),
                                     subProcesses_);
}

void SubProcess::doEndLuminosityBlock(LuminosityBlockPrincipal const& principal,
                                      IOVSyncValue const& ts,
                                      bool cleaningUpAfterException) {
  ServiceRegistry::Operate operate(serviceToken_);
...
void SubProcess::endLuminosityBlock(LuminosityBlockPrincipal const& principal,
                                    IOVSyncValue const& ts,
                                    bool cleaningUpAfterException) {
  LuminosityBlockPrincipal& lbp = *principalCache_.lumiPrincipalPtr();
  lbp.setComplete();
  propagateProducts(InLumi, principal, lbp);
  typedef OccurrenceTraits<LuminosityBlockPrincipal, BranchActionGlobalEnd>
      Traits;
  lbp.setAtEndTransition(true);
  schedule_->processOneGlobal<Traits>(lbp, esp_->eventSetupForInstance(ts),
                                      cleaningUpAfterException);
  for_all(subProcesses_,
          [&lbp, &ts, cleaningUpAfterException](auto& subProcess) {
            subProcess.doEndLuminosityBlock(lbp, ts, cleaningUpAfterException);
          });
}

void SubProcess::doEndLuminosityBlockAsync(
    WaitingTaskHolder iHolder, LuminosityBlockPrincipal const& principal,
    IOVSyncValue const& ts, bool cleaningUpAfterException) {
  LuminosityBlockPrincipal& lbp = *principalCache_.lumiPrincipalPtr();
  lbp.setComplete();
  propagateProducts(InLumi, principal, lbp);
  typedef OccurrenceTraits<LuminosityBlockPrincipal, BranchActionGlobalEnd>
      Traits;
  lbp.setAtEndTransition(true);
  endGlobalTransitionAsync<Traits>(std::move(iHolder), *schedule_, lbp, ts,
                                   esp_->eventSetupForInstance(ts),
                                   subProcesses_, cleaningUpAfterException);
}

void SubProcess::writeLumi(ProcessHistoryID const& parentPhID, int runNumber,
                           int lumiNumber) {
  ServiceRegistry::Operate operate(serviceToken_);
  std::map<ProcessHistoryID, ProcessHistoryID>::const_iterator it =
      parentToChildPhID_.find(parentPhID);
  ...
  auto const& childPhID = it->second;
  schedule_->writeLumi(
      principalCache_.lumiPrincipal(childPhID, runNumber, lumiNumber),
      &processContext_);
  for_all(subProcesses_, [&childPhID, runNumber, lumiNumber](auto& subProcess) {
    subProcess.writeLumi(childPhID, runNumber, lumiNumber);
  });
}

void SubProcess::deleteLumiFromCache(ProcessHistoryID const& parentPhID,
                                     int runNumber, int lumiNumber) {
  std::map<ProcessHistoryID, ProcessHistoryID>::const_iterator it =
      parentToChildPhID_.find(parentPhID);
  assert(it != parentToChildPhID_.end());
  auto const& childPhID = it->second;
  ...
  for_all(subProcesses_, [&childPhID, runNumber, lumiNumber](auto& subProcess) {
    subProcess.deleteLumiFromCache(childPhID, runNumber, lumiNumber);
  });
}
...
void SubProcess::doBeginStream(unsigned int iID) {
  ...
  for_all(subProcesses_,
          [iID](auto& subProcess) { subProcess.doBeginStream(iID); });
}
...
void SubProcess::doEndStream(unsigned int iID) {
  ...
  for_all(subProcesses_,
          [iID](auto& subProcess) { subProcess.doEndStream(iID); });
}

void SubProcess::doStreamBeginRun(unsigned int id,
                                  RunPrincipal const& principal,
                                  IOVSyncValue const& ts) {
  ServiceRegistry::Operate operate(serviceToken_);
  {
    ...
    schedule_->processOneStream<Traits>(id, rp,
                                        esp_->eventSetupForInstance(ts));
    for_all(subProcesses_, [id, &rp, &ts](auto& subProcess) {
...
void SubProcess::doStreamBeginRunAsync(WaitingTaskHolder iHolder,
  ...
  beginStreamTransitionAsync<Traits>(std::move(iHolder), *schedule_, id, rp, ts,
                                     esp_->eventSetupForInstance(ts),
                                     subProcesses_);
}

void SubProcess::doStreamEndRun(unsigned int id, RunPrincipal const& principal,
                                IOVSyncValue const& ts,
                                bool cleaningUpAfterException) {
  ServiceRegistry::Operate operate(serviceToken_);
  {
    ...
    schedule_->processOneStream<Traits>(id, rp, esp_->eventSetupForInstance(ts),
                                        cleaningUpAfterException);
    for_all(subProcesses_,
            [id, &rp, &ts, cleaningUpAfterException](auto& subProcess) {
              subProcess.doStreamEndRun(id, rp, ts, cleaningUpAfterException);
            });
  }
}

void SubProcess::doStreamEndRunAsync(WaitingTaskHolder iHolder, unsigned int id,
                                     RunPrincipal const& principal,
                                     IOVSyncValue const& ts,
                                     bool cleaningUpAfterException) {
  ServiceRegistry::Operate operate(serviceToken_);
  ...
  endStreamTransitionAsync<Traits>(std::move(iHolder), *schedule_, id, rp, ts,
                                   esp_->eventSetupForInstance(ts),
                                   subProcesses_, cleaningUpAfterException);
}

void SubProcess::doStreamBeginLuminosityBlock(
    unsigned int id, LuminosityBlockPrincipal const& principal,
    IOVSyncValue const& ts) {
  ServiceRegistry::Operate operate(serviceToken_);
  {
    LuminosityBlockPrincipal& lbp = *principalCache_.lumiPrincipalPtr();
    typedef OccurrenceTraits<LuminosityBlockPrincipal, BranchActionStreamBegin>
        Traits;
    schedule_->processOneStream<Traits>(id, lbp,
                                        esp_->eventSetupForInstance(ts));
    for_all(subProcesses_, [id, &lbp, &ts](auto& subProcess) {
      subProcess.doStreamBeginLuminosityBlock(id, lbp, ts);
    });
  }
}
void SubProcess::doStreamBeginLuminosityBlockAsync(
    WaitingTaskHolder iHolder, unsigned int id,
    LuminosityBlockPrincipal const& principal, IOVSyncValue const& ts) {
  ServiceRegistry::Operate operate(serviceToken_);
  ...
  beginStreamTransitionAsync<Traits>(std::move(iHolder), *schedule_, id, lbp,
                                     ts, esp_->eventSetupForInstance(ts),
                                     subProcesses_);
}

void SubProcess::doStreamEndLuminosityBlock(
    unsigned int id, LuminosityBlockPrincipal const& principal,
    IOVSyncValue const& ts, bool cleaningUpAfterException) {
  ServiceRegistry::Operate operate(serviceToken_);
  {
    LuminosityBlockPrincipal& lbp = *principalCache_.lumiPrincipalPtr();
    typedef OccurrenceTraits<LuminosityBlockPrincipal, BranchActionStreamEnd>
        Traits;
    schedule_->processOneStream<Traits>(
        id, lbp, esp_->eventSetupForInstance(ts), cleaningUpAfterException);
    for_all(subProcesses_,
            [id, &lbp, &ts, cleaningUpAfterException](auto& subProcess) {
              subProcess.doStreamEndLuminosityBlock(id, lbp, ts,
                                                    cleaningUpAfterException);
            });
  }
}
...
void SubProcess::doStreamEndLuminosityBlockAsync(
  ...
  typedef OccurrenceTraits<LuminosityBlockPrincipal, BranchActionStreamEnd>
      Traits;
  endStreamTransitionAsync<Traits>(std::move(iHolder), *schedule_, id, lbp, ts,
                                   esp_->eventSetupForInstance(ts),
                                   subProcesses_, cleaningUpAfterException);
}

void SubProcess::propagateProducts(BranchType type,
  ...
  for (auto const& item : keptVector) {
    ...
    ProductResolverBase const* parentProductResolver =
        parentPrincipal.getProductResolver(desc.branchID());
    if (parentProductResolver != nullptr) {
      ProductResolverBase* productResolver =
          principal.getModifiableProductResolver(desc.branchID());
      if (productResolver != nullptr) {
        // Propagate the per event(run)(lumi) data for this product to the
        // subprocess.
...
void SubProcess::updateBranchIDListHelper(BranchIDLists const& branchIDLists) {
  ...
  for_all(subProcesses_, [this](auto& subProcess) {
...
void SubProcess::respondToOpenInputFile(FileBlock const& fb) {
  ...
  for_all(subProcesses_,
          [&fb](auto& subProcess) { subProcess.respondToOpenInputFile(fb); });
}

// free function
std::vector<ParameterSet> popSubProcessVParameterSet(
