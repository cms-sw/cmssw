...
void SourceMakerTraits::replaceExisting(
    EventSetupProvider&, std::shared_ptr<EventSetupRecordIntervalFinder>) {
...
SourceMakerTraits::getComponentAndRegisterProcess(
    EventSetupsController& esController, ParameterSet const& iConfiguration) {
  return esController.getESSourceAndRegisterProcess(
      iConfiguration, esController.indexOfNextProcess());
}

void SourceMakerTraits::putComponent(
    EventSetupsController& esController, ParameterSet const& iConfiguration,
