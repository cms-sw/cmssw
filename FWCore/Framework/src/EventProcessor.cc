    ...
    eventSetupDataToExcludeFromPrefetching_()
{
  ...
  EventProcessor::init(std::shared_ptr<ProcessDesc>& processDesc,
    ...
    input_ = makeInput(*parameterSet,
                       *common,
                       get_underlying(items.preg_),
                       get_underlying(items.branchIDListHelper_),
                       get_underlying(items.thinnedAssociationsHelper_),
                       get_underlying(items.actReg_),
                       get_underlying(items.processConfiguration_),
                       preallocations_);
    ...
    for(unsigned int index = 0; index<preallocations_.numberOfStreams(); ++index ) {
      ...
      auto ep = std::make_shared<EventPrincipal>(preg(), get_underlying(branchIDListHelper_),
           get_underlying(thinnedAssociationsHelper_), *processConfiguration_, historyAppender_.get(), index);
  ...
  EventProcessor::~EventProcessor() {
    ...
    get_underlying(espController_).reset();
    get_underlying(subProcesses_).reset();
    get_underlying(esp_).reset();
    get_underlying(schedule_).reset();
    get_underlying(input_).reset();
    get_underlying(looper_).reset();
  ...
  EventProcessor::endJob() {
    ...
    if(looper_) {
      c.call(std::bind(&EDLooperBase::endOfJob, get_underlying(looper_)));
    }
