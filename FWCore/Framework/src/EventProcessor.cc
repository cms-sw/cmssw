...
class SendSourceTerminationSignalIfException {
 public:
  ...
  ~SendSourceTerminationSignalIfException() {
    if (reg_) {
      reg_->preSourceEarlyTerminationSignal_(
...
// ---------------------------------------------------------------
std::unique_ptr<InputSource> makeInput(
    ParameterSet& params, CommonParams const& common,
  ...
  if (main_input == 0) {
    throw Exception(errors::Configuration)
        ...
        << "It is missing (or there are sufficient syntax errors such that it "
           "is not recognized as the source)\n";
  }
  ...
  ModuleDescription md(
      main_input->id(), main_input->getParameter<std::string>("@module_type"),
      "source", processConfiguration.get(), ModuleDescription::getUniqueID());

  InputSourceDescription isdesc(md, preg, branchIDListHelper,
                                thinnedAssociationsHelper, areg,
  ...
  try {
    ...
    std::shared_ptr<int> sentry(nullptr, [areg, &md](void*) {
      areg->postSourceConstructionSignal_(md);
    });
    convertException::wrap([&]() {
      input = std::unique_ptr<InputSource>(
          InputSourceFactory::get()
              ->makeInputSource(*main_input, isdesc)
              .release());
      input->preEventReadFromSourceSignal_.connect(
          std::cref(areg->preEventReadFromSourceSignal_));
      input->postEventReadFromSourceSignal_.connect(
          std::cref(areg->postEventReadFromSourceSignal_));
    });
  } catch (cms::Exception& iException) {
    std::ostringstream ost;
...
// ---------------------------------------------------------------
std::shared_ptr<EDLooperBase> fillLooper(
    eventsetup::EventSetupsController& esController,
    eventsetup::EventSetupProvider& cp, ParameterSet& params) {
  ...
  for (std::vector<std::string>::iterator itName = loopers.begin(),
                                          itNameEnd = loopers.end();
       itName != itNameEnd; ++itName) {
    ParameterSet* providerPSet = params.getPSetForUpdate(*itName);
    providerPSet->registerIt();
    vLooper = eventsetup::LooperFactory::get()->addTo(esController, cp,
                                                      *providerPSet);
  }
  return vLooper;
}
...
EventProcessor::EventProcessor(std::string const& config,
    ...
    : actReg_(),
      ...
      sourceResourcesAcquirer_(SharedResourcesRegistry::instance()
                                   ->createAcquirerForSourceDelayedReader()
                                   .first),
      sourceMutex_(SharedResourcesRegistry::instance()
                       ->createAcquirerForSourceDelayedReader()
                       .second),
      principalCache_(),
  ...
  std::shared_ptr<ParameterSet> parameterSet =
      PythonProcessDesc(config).parameterSet();
...
void EventProcessor::init(std::shared_ptr<ProcessDesc>& processDesc,
  ...
  ParameterSet const& optionsPset(
      parameterSet->getUntrackedParameterSet("options", ParameterSet()));
  auto const& fileMode =
      optionsPset.getUntrackedParameter<std::string>("fileMode", "FULLMERGE");
  ...
  forceESCacheClearOnNewRun_ = optionsPset.getUntrackedParameter<bool>(
      "forceEventSetupCacheClearOnNewRun", false);
  // threading
  unsigned int nThreads = 1;
  if (optionsPset.existsAs<unsigned int>("numberOfThreads", false)) {
    nThreads =
        optionsPset.getUntrackedParameter<unsigned int>("numberOfThreads");
    if (nThreads == 0) {
      nThreads = 1;
    }
  }
  /* TODO: when we support having each stream run in a different thread use this
     default
  ...
  */
  ...
  if (optionsPset.existsAs<unsigned int>("numberOfStreams", false)) {
    nStreams =
        optionsPset.getUntrackedParameter<unsigned int>("numberOfStreams");
    if (nStreams == 0) {
      nStreams = nThreads;
    }
  }
  ...
  */
  ...
  /*
    if(nRunsSet = optionsPset.existsAs<unsigned
    int>("numberOfConcurrentRuns",false)) {
    nConcurrentRuns = optionsPset.getUntrackedParameter<unsigned
    int>("numberOfConcurrentRuns");
    }
  */
  unsigned int nConcurrentLumis = 1;
  /*
    if(optionsPset.existsAs<unsigned
    int>("numberOfConcurrentLuminosityBlocks",false)) {
    nConcurrentLumis = optionsPset.getUntrackedParameter<unsigned
    int>("numberOfConcurrentLuminosityBlocks");
    } else {
  ...
  */
  ...
  printDependencies_ =
      optionsPset.getUntrackedParameter("printDependencies", false);
  ...
  ServiceToken token =
      items.initServices(serviceSets, *parameterSet, iToken, iLegacy, true);
  ...
  preallocations_ = PreallocationConfiguration{
      nThreads, nStreams, nConcurrentLumis, nConcurrentRuns};

  // initialize the input source
  input_ =
      makeInput(*parameterSet, *common, items.preg(),
                items.branchIDListHelper(), items.thinnedAssociationsHelper(),
                items.actReg_, items.processConfiguration(), preallocations_);

  // intialize the Schedule
  schedule_ = items.initSchedule(*parameterSet, hasSubProcesses,
  ...
  for (unsigned int index = 0; index < preallocations_.numberOfStreams();
       ++index) {
    // Reusable event principal
    auto ep = std::make_shared<EventPrincipal>(
        preg(), branchIDListHelper(), thinnedAssociationsHelper(),
  ...
  for (auto& subProcessPSet : subProcessVParameterSet) {
    subProcesses_.emplace_back(
        subProcessPSet, *parameterSet, preg(), branchIDListHelper(),
        *thinnedAssociationsHelper_, SubProcessParentageHelper(),
        *espController_, *actReg_, token,
        serviceregistry::kConfigurationOverrides, preallocations_,
        &processContext_);
  }
}
...
void EventProcessor::beginJob() {
  ...
  // NOTE: this may throw
  checkForModuleDependencyCorrectness(pathsAndConsumesOfModules_,
                                      printDependencies_);
  ...
  // NOTE: in future we should have a beginOfJob for looper that takes no
  // arguments
  ...
  for (unsigned int i = 0; i < preallocations_.numberOfStreams(); ++i) {
    ...
    for_all(subProcesses_,
            [i](auto& subProcess) { subProcess.doBeginStream(i); });
  }
}
...
std::vector<ModuleDescription const*> EventProcessor::getAllModuleDescriptions()
    const {
...
int EventProcessor::totalEventsPassed() const {
  return schedule_->totalEventsPassed();
}

int EventProcessor::totalEventsFailed() const {
  return schedule_->totalEventsFailed();
}

void EventProcessor::enableEndPaths(bool active) {
  schedule_->enableEndPaths(active);
}

bool EventProcessor::endPathsEnabled() const {
  return schedule_->endPathsEnabled();
}

void EventProcessor::getTriggerReport(TriggerReport& rep) const {
...
InputSource::ItemType EventProcessor::nextTransitionType() {
  ...
  if (checkForAsyncStopRequest(returnCode)) {
    actReg_->preSourceEarlyTerminationSignal_(
        TerminationOrigin::ExternalSignal);
...
edm::LuminosityBlockNumber_t EventProcessor::nextLuminosityBlockID() {
...
void EventProcessor::openOutputFiles() {
  if (fb_.get() != nullptr) {
    ...
    for_all(subProcesses_,
            [this](auto& subProcess) { subProcess.openOutputFiles(*fb_); });
  }
...
void EventProcessor::doErrorStuff() {
  ...
  LogError("StateMachine")
      ...
      << "This likely indicates a bug in an input module or corrupted input or "
         "both\n";
}
...
void EventProcessor::beginRun(ProcessHistoryID const& phid, RunNumber_t run) {
  ...
  }
  {
    ...
    beginStreamsTransitionAsync<Traits>(streamLoopWaitTask.get(), *schedule_,
                                        preallocations_.numberOfStreams(),
                                        runPrincipal, ts, es, subProcesses_);
...
void EventProcessor::endRun(ProcessHistoryID const& phid, RunNumber_t run,
  ...
  IOVSyncValue ts(
      EventID(runPrincipal.run(), LuminosityBlockID::maxLuminosityBlockNumber(),
  ...
  EventSetup const& es = esp_->eventSetup();
  {
    ...
    endStreamsTransitionAsync<Traits>(
        streamLoopWaitTask.get(), *schedule_, preallocations_.numberOfStreams(),
        runPrincipal, ts, es, subProcesses_, cleaningUpAfterException);
...
void EventProcessor::beginLumi(ProcessHistoryID const& phid, RunNumber_t run,
                               LuminosityBlockNumber_t lumi) {
  LuminosityBlockPrincipal& lumiPrincipal =
      principalCache_.lumiPrincipal(phid, run, lumi);
  {
    SendSourceTerminationSignalIfException sentry(actReg_.get());
  ...
  // NOTE: Using 0 as the event number for the begin of a lumi block is a bad
  // idea
  // lumi blocks know their start and end times why not also start and end
  // events?
  IOVSyncValue ts(
      EventID(lumiPrincipal.run(), lumiPrincipal.luminosityBlock(), 0),
      lumiPrincipal.beginTime());
  {
    SendSourceTerminationSignalIfException sentry(actReg_.get());
    espController_->eventSetupForInstance(ts);
    sentry.completedSuccessfully();
  ...
  }
  {
    ...
    beginStreamsTransitionAsync<Traits>(streamLoopWaitTask.get(), *schedule_,
                                        preallocations_.numberOfStreams(),
                                        lumiPrincipal, ts, es, subProcesses_);
    streamLoopWaitTask->wait_for_all();
    if (streamLoopWaitTask->exceptionPtr() != nullptr) {
      std::rethrow_exception(*(streamLoopWaitTask->exceptionPtr()));
    }
  ...
  if (looper_) {
    // looper_->doStreamBeginLuminosityBlock(schedule_->streamID(),lumiPrincipal,
    // es);
  }
}

void EventProcessor::endLumi(ProcessHistoryID const& phid, RunNumber_t run,
                             LuminosityBlockNumber_t lumi,
                             bool cleaningUpAfterException) {
  LuminosityBlockPrincipal& lumiPrincipal =
      principalCache_.lumiPrincipal(phid, run, lumi);
  ...
  // lumi blocks know their start and end times why not also start and end
  // events?
  IOVSyncValue ts(EventID(lumiPrincipal.run(), lumiPrincipal.luminosityBlock(),
                          EventID::maxEventNumber()),
  ...
  EventSetup const& es = esp_->eventSetup();
  {
    ...
    endStreamsTransitionAsync<Traits>(
        streamLoopWaitTask.get(), *schedule_, preallocations_.numberOfStreams(),
        lumiPrincipal, ts, es, subProcesses_, cleaningUpAfterException);
    streamLoopWaitTask->wait_for_all();
    if (streamLoopWaitTask->exceptionPtr() != nullptr) {
      std::rethrow_exception(*(streamLoopWaitTask->exceptionPtr()));
    }
  ...
  if (looper_) {
    // looper_->doStreamEndLuminosityBlock(schedule_->streamID(),lumiPrincipal,
    // es);
  }
  {
    ...
    for_all(subProcesses_,
            [&lumiPrincipal, &ts, cleaningUpAfterException](auto& subProcess) {
              subProcess.doEndLuminosityBlock(lumiPrincipal, ts,
                                              cleaningUpAfterException);
            });
  }
  FDEBUG(1) << "\tendLumi " << run << "/" << lumi << "\n";
...
std::pair<ProcessHistoryID, RunNumber_t> EventProcessor::readAndMergeRun() {
  ...
  assert(input_->reducedProcessHistoryID() ==
         runPrincipal->reducedProcessHistoryID());
...
int EventProcessor::readLuminosityBlock() {
  ...
  auto lbp = std::make_shared<LuminosityBlockPrincipal>(
      input_->luminosityBlockAuxiliary(), preg(), *processConfiguration_,
      historyAppender_.get(), 0);
  {
    SendSourceTerminationSignalIfException sentry(actReg_.get());
...
void EventProcessor::writeRun(ProcessHistoryID const& phid, RunNumber_t run) {
  schedule_->writeRun(principalCache_.runPrincipal(phid, run),
                      &processContext_);
  for_all(subProcesses_,
          [run, phid](auto& subProcess) { subProcess.writeRun(phid, run); });
  FDEBUG(1) << "\twriteRun " << run << "\n";
}

void EventProcessor::deleteRunFromCache(ProcessHistoryID const& phid,
                                        RunNumber_t run) {
  principalCache_.deleteRun(phid, run);
  for_all(subProcesses_, [run, phid](auto& subProcess) {
    subProcess.deleteRunFromCache(phid, run);
  });
  FDEBUG(1) << "\tdeleteRunFromCache " << run << "\n";
}

void EventProcessor::writeLumi(ProcessHistoryID const& phid, RunNumber_t run,
                               LuminosityBlockNumber_t lumi) {
  schedule_->writeLumi(principalCache_.lumiPrincipal(phid, run, lumi),
                       &processContext_);
  for_all(subProcesses_, [&phid, run, lumi](auto& subProcess) {
    subProcess.writeLumi(phid, run, lumi);
  });
  FDEBUG(1) << "\twriteLumi " << run << "/" << lumi << "\n";
}

void EventProcessor::deleteLumiFromCache(ProcessHistoryID const& phid,
                                         RunNumber_t run,
                                         LuminosityBlockNumber_t lumi) {
...
bool EventProcessor::readNextEventForStream(
    unsigned int iStreamIndex, std::atomic<bool>* finishedProcessingEvents) {
  if (shouldWeStop()) {
    return false;
  }
  ...
  try {
    ...
    if (not firstEventInBlock_) {
      ...
      if ((asyncStopRequestedWhileProcessingEvents_ = checkForAsyncStopRequest(
...
void EventProcessor::handleNextEventForStreamAsync(
  ...
  auto recursionTask = make_waiting_task(
      ...
      [this, iTask, iStreamIndex,
       finishedProcessingEvents](std::exception_ptr const* iPtr) {
        if (iPtr) {
          bool expected = false;
        ...
        handleNextEventForStreamAsync(iTask, iStreamIndex,
                                      finishedProcessingEvents);
      });

  sourceResourcesAcquirer_.serialQueueChain().push(
      [this, finishedProcessingEvents, recursionTask, iTask, iStreamIndex]() {
        ServiceRegistry::Operate operate(serviceToken_);
...
InputSource::ItemType EventProcessor::readAndProcessEvents() {
  nextItemTypeFromProcessingEvents_ = InputSource::IsEvent;  // needed for
                                                             // looper
  asyncStopRequestedWhileProcessingEvents_ = false;
  ...
  for (; iStreamIndex < kNumStreams - 1; ++iStreamIndex) {
    eventLoopWaitTask->increment_ref_count();
    tbb::task::enqueue(*make_waiting_task(
        tbb::task::allocate_root(),
        [this, iStreamIndex, finishedProcessingEventsPtr,
         eventLoopWaitTaskPtr](std::exception_ptr const*) {
          handleNextEventForStreamAsync(eventLoopWaitTaskPtr, iStreamIndex,
                                        finishedProcessingEventsPtr);
        }));
  }
  eventLoopWaitTask->increment_ref_count();
  eventLoopWaitTask->spawn_and_wait_for_all(*make_waiting_task(
      tbb::task::allocate_root(),
      [this, iStreamIndex, finishedProcessingEventsPtr,
       eventLoopWaitTaskPtr](std::exception_ptr const*) {
        handleNextEventForStreamAsync(eventLoopWaitTaskPtr, iStreamIndex,
...
void EventProcessor::processEventAsync(WaitingTaskHolder iHolder,
  ...
  assert(principalCache_.lumiPrincipalPtr()->luminosityBlock() ==
         pep->luminosityBlock());
  ...
  schedule_->processOneEventAsync(std::move(afterProcessTask), iStreamIndex,
                                  *pep, esp_->eventSetup());
}
...
void EventProcessor::processEventWithLooper(EventPrincipal& iPrincipal) {
  ...
  do {
    ...
    status = looper_->doDuringLoop(iPrincipal, esp_->eventSetup(), pc,
                                   &streamContext);
    ...
    if (randomAccess) {
      if (pc.requestedTransition() == ProcessingController::kToPreviousEvent) {
        input_->skipEvents(-2);
      } else if (pc.requestedTransition() ==
                 ProcessingController::kToSpecifiedEvent) {
