...
namespace edm {
  ...
  // ---------------------------------------------------------------
  std::unique_ptr<InputSource> makeInput(ParameterSet& params, CommonParams const& common,
  ...
  // ---------------------------------------------------------------
  EventProcessor::EventProcessor(std::string const& config, ServiceToken const& iToken,
  ...
  EventProcessor::EventProcessor(std::string const& config, std::vector<std::string> const& defaultServices,
                                 std::vector<std::string> const& forcedServices)
      : actReg_(),
        preg_(),
        branchIDListHelper_(),
  ...
  EventProcessor::EventProcessor(std::shared_ptr<ProcessDesc> processDesc, ServiceToken const& token,
  ...
  void EventProcessor::init(std::shared_ptr<ProcessDesc>& processDesc, ServiceToken const& iToken,
                            serviceregistry::ServiceLegacy iLegacy) {
...
#include "TransitionProcessors.icc"
  ...
  void EventProcessor::endRun(ProcessHistoryID const& phid, RunNumber_t run, bool globalBeginSucceeded,
    ...
    if (globalBeginSucceeded) {
      ...
      endStreamsTransitionAsync<Traits>(streamLoopWaitTask.get(), *schedule_, preallocations_.numberOfStreams(),
  ...
  void EventProcessor::beginLumi(ProcessHistoryID const& phid, RunNumber_t run, LuminosityBlockNumber_t lumi,
    ...
    EventSetup const& es = esp_->eventSetup();
    {
      ...
      auto globalWaitTask = make_empty_waiting_task();
      globalWaitTask->increment_ref_count();
      beginGlobalTransitionAsync<Traits>(WaitingTaskHolder(globalWaitTask.get()), *schedule_, lumiPrincipal, ts, es,
                                         subProcesses_);
      globalWaitTask->wait_for_all();
      if (globalWaitTask->exceptionPtr() != nullptr) {
        std::rethrow_exception(*(globalWaitTask->exceptionPtr()));
  ...
  void EventProcessor::endLumi(ProcessHistoryID const& phid, RunNumber_t run, LuminosityBlockNumber_t lumi,
    ...
    }
    {
      ...
      endGlobalTransitionAsync<Traits>(WaitingTaskHolder(globalWaitTask.get()), *schedule_, lumiPrincipal, ts, es,
                                       subProcesses_, cleaningUpAfterException);
      globalWaitTask->wait_for_all();
      if (globalWaitTask->exceptionPtr() != nullptr) {
        std::rethrow_exception(*(globalWaitTask->exceptionPtr()));
  ...
  int EventProcessor::readLuminosityBlock() {
    ...
    auto lbp = std::make_shared<LuminosityBlockPrincipal>(input_->luminosityBlockAuxiliary(), preg(),
                                                          *processConfiguration_, historyAppender_.get(), 0);
    {
      SendSourceTerminationSignalIfException sentry(actReg_.get());
  ...
  void EventProcessor::deleteLumiFromCache(ProcessHistoryID const& phid, RunNumber_t run,
  ...
  bool EventProcessor::readNextEventForStream(unsigned int iStreamIndex, std::atomic<bool>* finishedProcessingEvents) {
    if (shouldWeStop()) {
      return false;
    }
    ...
    try {
      ...
      if (not firstEventInBlock_) {
        ...
        if ((asyncStopRequestedWhileProcessingEvents_ =
                 checkForAsyncStopRequest(asyncStopStatusCodeFromProcessingEvents_))) {
  ...
  void EventProcessor::handleNextEventForStreamAsync(WaitingTask* iTask, unsigned int iStreamIndex,
                                                     std::atomic<bool>* finishedProcessingEvents) {
    auto recursionTask =
        make_waiting_task(tbb::task::allocate_root(),
                          [this, iTask, iStreamIndex, finishedProcessingEvents](std::exception_ptr const* iPtr) {
                            if (iPtr) {
                              bool expected = false;
                              if (deferredExceptionPtrIsSet_.compare_exchange_strong(expected, true)) {
  ...
  InputSource::ItemType EventProcessor::readAndProcessEvents() {
    ...
    for (; iStreamIndex < kNumStreams - 1; ++iStreamIndex) {
      ...
      tbb::task::enqueue(*make_waiting_task(
          ...
          [this, iStreamIndex, finishedProcessingEventsPtr, eventLoopWaitTaskPtr](std::exception_ptr const*) {
