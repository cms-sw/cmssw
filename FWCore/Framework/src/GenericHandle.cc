...
void convert_handle(BasicHandle&& orig, Handle<GenericObject>& result) {
  ...
  ObjectWithDict wrap(originalWrap->wrappedTypeInfo(),
                      const_cast<WrapperBase*>(originalWrap));
  assert(bool(wrap));

  ObjectWithDict product(wrap.get("obj"));
  if (!product) {
    throw Exception(errors::LogicError)
        << "GenericObject could not find 'obj' member";
  }
  if (product.typeOf() != result.type()) {
    throw Exception(errors::LogicError)
        << "GenericObject asked for " << result.type().name()
...
bool edm::Event::getByLabel<GenericObject>(
  ...
  BasicHandle bh = provRecorder_.getByLabel_(
      TypeID(result.type().typeInfo()), label, productInstanceName,
...
template <>
bool edm::Event::getByLabel<GenericObject>(
    edm::InputTag const& tag, Handle<GenericObject>& result) const {
  if (tag.process().empty()) {
    return this->getByLabel(tag.label(), tag.instance(), result);
  } else {
    BasicHandle bh = provRecorder_.getByLabel_(
        TypeID(result.type().typeInfo()), tag.label(), tag.instance(),
