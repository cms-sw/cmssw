...
namespace edm {
  ...
  inline void subProcessDoStreamBeginTransitionAsync(WaitingTaskHolder iHolder, SubProcess& iSubProcess, unsigned int i,
  ...
  inline void subProcessDoStreamEndTransitionAsync(WaitingTaskHolder iHolder, SubProcess& iSubProcess, unsigned int i,
                                                   LuminosityBlockPrincipal& iPrincipal, IOVSyncValue const& iTS,
  ...
  void beginStreamTransitionAsync(WaitingTaskHolder iWait, Schedule& iSchedule, unsigned int iStreamIndex,
                                  P& iPrincipal, IOVSyncValue const& iTS, EventSetup const& iES, SC& iSubProcesses) {
    ...
    auto subs = make_waiting_task(tbb::task::allocate_root(), [&iSubProcesses, iWait, iStreamIndex, &iPrincipal, iTS,
      ...
      if (iPtr) {
        ...
        for_all(iSubProcesses, [&h, iStreamIndex, &iPrincipal, iTS](auto& subProcess) {
          subProcessDoStreamBeginTransitionAsync(h, subProcess, iStreamIndex, iPrincipal, iTS);
        });
      } else {
        for_all(iSubProcesses, [&iWait, iStreamIndex, &iPrincipal, iTS](auto& subProcess) {
  ...
  template <typename Traits, typename P, typename SC>
  void beginStreamsTransitionAsync(WaitingTask* iWait, Schedule& iSchedule, unsigned int iNStreams, P& iPrincipal,
                                   IOVSyncValue const& iTS, EventSetup const& iES, SC& iSubProcesses) {
  ...
  template <typename Traits, typename P, typename SC>
  void endStreamTransitionAsync(WaitingTaskHolder iWait, Schedule& iSchedule, unsigned int iStreamIndex, P& iPrincipal,
                                IOVSyncValue const& iTS, EventSetup const& iES, SC& iSubProcesses,
                                bool cleaningUpAfterException) {
    ServiceToken token = ServiceRegistry::instance().presentToken();
  ...
  template <typename Traits, typename P, typename SC>
  void endStreamsTransitionAsync(WaitingTask* iWait, Schedule& iSchedule, unsigned int iNStreams, P& iPrincipal,
                                 IOVSyncValue const& iTS, EventSetup const& iES, SC& iSubProcesses,
                                 bool cleaningUpAfterException) {
    WaitingTaskHolder holdUntilAllStreamsCalled(iWait);
    for (unsigned int i = 0; i < iNStreams; ++i) {
      endStreamTransitionAsync<Traits>(WaitingTaskHolder(iWait), iSchedule, i, iPrincipal, iTS, iES, iSubProcesses,
                                       cleaningUpAfterException);
    }
  }
};
