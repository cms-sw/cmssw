...
struct cycle_detector : public boost::dfs_visitor<> {
  ...
  cycle_detector(
      EdgeToPathMap const& iEdgeToPathMap,
      std::vector<std::vector<unsigned int>> const&
          iPathIndexToModuleIndexOrder,
  ...
  void back_edge(Edge const& iEdge, Graph const& iGraph) {
    ...
    std::unique_ptr<std::vector<Edge>, decltype(pop_stack)> guard(&m_stack,
                                                                  pop_stack);
  ...
  void forward_or_cross_edge(Edge iEdge, Graph const& iGraph) {
    ...
    // If this is a crossing edge whose out vertex is part of a fundamental
    // cycle
    // then this path is also part of a cycle
    if (m_verticiesInFundamentalCycles.end() ==
        m_verticiesInFundamentalCycles.find(out)) {
      return;
    }
    ...
    for (auto const& cycle : m_fundamentalCycles) {
      ...
      // the set of 'in' verticies presently in the stack is used to find where
      // an 'out'
      ...
      if (itLastMatch == cycle.end()) {
        ...
        if ((itStartMatch == cycle.begin()) and
            (cycle.end() == (itLastMatch + 1))) {
          // This is just the entire cycle starting where we've already started
          // before. Given the cycle was OK before, it would also be OK this
          // time
          return;
        }
 ...
 private:
  ...
  void throwOnError(
    ...
    for (auto const& edge : iEdges) {
      ...
      if (pathDependencyOnly) {
        oStream << " after " << moduleName(out) << " [path "
                << pathName(iFound->second[0]) << "]";
      } else {
        oStream << " consumes " << moduleName(out);
      }
    }
    oStream << "\n Running in the threaded framework would lead to "
               "indeterminate results."
               "\n Please change order of modules in mentioned Path(s) to "
               "avoid inconsistent module ordering.";

    throw edm::Exception(edm::errors::ScheduleExecutionFailure,
                         "Unrunnable schedule\n")
        << oStream.str() << "\n";
  }
  ...
  std::vector<Edge> findMinimumCycle(std::vector<Edge> const& iCycleEdges,
    ...
    // The graph library scans the verticies so we have edges in the list which
    // are
    // not part of the cycle but are associated to a vertex contributes to the
    // cycle.
    // To find these unneeded edges we work backwards on the edge list looking
    // for cases
    // where the 'in' on the previous edge is not the 'out' for the next edge.
    // When this
    // happens we know that there are additional edges for that same 'in' which
    // can be
    // removed.
  ...
  void checkCycleForProblem(std::vector<Edge> const& iCycleEdges,
    ...
    // Since we are dealing with a circle, we initialize the 'last' info with
    ...
    // If a data dependency appears to make us jump off a path but that module
    // actually
    // appears on the path that was left, we need to see if we later come back
    // to that
    // path somewhere before that module. If not than it is a false cycle
    std::unordered_multimap<unsigned int, unsigned int>
        pathToModulesWhichMustAppearLater;
    bool moduleAppearedEarlierInPath = false;

    for (auto dependency :
         m_edgeToPathMap.find(SimpleEdge(lastIn, lastOut))->second) {
      if (dependency != edm::graph::kDataDependencyIndex) {
    ...
    for (auto const& edge : iCycleEdges) {
      ...
      for (auto dependency : iFound->second) {
        if (dependency == edm::graph::kDataDependencyIndex) {
          ...
          auto pathToCheckRange =
              pathToModulesWhichMustAppearLater.equal_range(dependency);
          for (auto it = pathToCheckRange.first;
               it != pathToCheckRange.second;) {
            ...
            for (auto index : pathIndicies) {
              if (index == out) {
                // we must have skipped over the module so the earlier worry
                // about the
      ...
      if ((pathsOnEdge != lastPathsSeen)) {
        if ((not edgeHasDataDependency) and (not lastEdgeHasDataDepencency) and
            (not lastPathsSeen.empty())) {
    ...
    if (moduleAppearedEarlierInPath and
        not pathToModulesWhichMustAppearLater.empty()) {
      return;
    }
...
void edm::graph::throwIfImproperDependencies(
    EdgeToPathMap const& iEdgeToPathMap,
  ...
  cycle_detector detector(iEdgeToPathMap, iPathIndexToModuleIndexOrder,
                          iPathNames, iModuleIndexToNames);
