...
class StreamScheduleSignalSentry {
 public:
  StreamScheduleSignalSentry(ActivityRegistry* a,
...
class StreamSchedule {
 ...
 private:
  ...
  class SendTerminationSignalIfException {
   public:
    ...
    ~SendTerminationSignalIfException() {
      if (reg_) {
        reg_->preStreamEarlyTerminationSignal_(
  ...
  template <typename T>
  bool runTriggerPaths(typename T::MyPrincipal const&, EventSetup const&,
                       typename T::Context const*);

  template <typename T>
  void runEndPaths(typename T::MyPrincipal const&, EventSetup const&,
                   typename T::Context const*);
  ...
  // Each EarlyDeleteHelper hold a begin and end range into this vector. The
  // values
...
void StreamSchedule::processOneStream(typename T::MyPrincipal& ep,
  ...
  workerManager_.processOneOccurrence<T>(ep, es, streamID_, &streamContext_,
                                         &streamContext_,
  ...
  try {
    ...
    if (ex.context().empty()) {
      addContextAndPrintException(
          "Calling function StreamSchedule::processOneStream", ex,
          cleaningUpAfterException);
    } else {
...
void StreamSchedule::processOneStreamAsync(WaitingTaskHolder iHolder,
  ...
  auto doneTask = make_waiting_task(
      ...
      [this, iHolder, id, cleaningUpAfterException,
...
template <typename T>
bool StreamSchedule::runTriggerPaths(typename T::MyPrincipal const& ep,
                                     EventSetup const& es,
                                     typename T::Context const* context) {
  for (auto& p : trig_paths_) {
...
template <typename T>
void StreamSchedule::runEndPaths(typename T::MyPrincipal const& ep,
                                 EventSetup const& es,
                                 typename T::Context const* context) {
