...
static const char* const kComment =
    "List contains the branch names for the EDProducts which might be "
    "requested by the module.\n"
    "The format for identifying the EDProduct is the same as the one used for "
    "OutputModules, "
...
void edmodule_mightGet_config(ConfigurationDescriptions& iDesc) {
  // NOTE: by not giving a default, we are intentionally not having 'mightGet'
  // added
  // to any cfi files. This was done intentionally to avoid problems with HLT.
  // If requested,
  ...
  if (iDesc.defaultDescription()) {
    if (iDesc.defaultDescription()->isLabelUnused(kMightGet)) {
      iDesc.defaultDescription()
          ->addOptionalUntracked<std::vector<std::string> >(kMightGet)
          ->setComment(kComment);
    }
  }
  for (auto& v : iDesc) {
    if (v.second.isLabelUnused(kMightGet)) {
      v.second.addOptionalUntracked<std::vector<std::string> >(kMightGet)
          ->setComment(kComment);
    }
  }
}
