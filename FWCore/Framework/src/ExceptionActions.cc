...
char const* actionName(ActionCodes code) {
  ...
  return static_cast<unsigned int>(code) < tab.table_.size() ? tab.table_[code]
                                                             : "UnknownAction";
}
}
...
ExceptionToActionTable::ExceptionToActionTable(ParameterSet const& pset)
    : map_() {
  addDefaults();
...
void ExceptionToActionTable::add(std::string const& category,
                                 exception_actions::ActionCodes code) {
  map_[category] = code;
}

exception_actions::ActionCodes ExceptionToActionTable::find(
