...
// DependentRecordIntervalFinder::DependentRecordIntervalFinder(const
// DependentRecordIntervalFinder& rhs)
// {
...
//
// const DependentRecordIntervalFinder&
// DependentRecordIntervalFinder::operator=(const DependentRecordIntervalFinder&
// rhs)
// {
...
void DependentRecordIntervalFinder::setAlternateFinder(
    std::shared_ptr<EventSetupRecordIntervalFinder> iOther) {
  alternate_ = iOther;
}

void DependentRecordIntervalFinder::setIntervalFor(
    const EventSetupRecordKey& iKey, const IOVSyncValue& iTime,
    ValidityInterval& oInterval) {
  // NOTE: oInterval is the last value that was used so if nothing changed do
  // not modify oInterval
  ...
  if (intervalsWereComparible) {
    if (!haveAValidDependentRecord) {
      // If no Finder has no valid time, then this record is also invalid for
      // this time
      newInterval = ValidityInterval::invalidInterval();
    }
    if (!allRecordsValid) {
      // since some of the dependent providers do not have a valid IOV for this
      // syncvalue
      // we do not know what the true end IOV is.  Therefore we must make it
      // open ended
  ...
  // I'm using an heuristic to pick a reasonable starting point for the IOV. The
  // idea is to
  // assume that lumi sections are 23 seconds long and therefore if we take a
  // difference between
  // iTime and the beginning of a changed IOV we can pick the changed IOV with
  // the start time
  // closest to iTime. This doesn't have to be perfect, we just want to reduce
  // the dependency
  ...
  if (hadChangedIOV) {
    if (closestID.run() != 0) {
      if (closestTimeStamp.value() == 0) {
        // no time
        oInterval = ValidityInterval(IOVSyncValue(closestID),
        ...
        if (closestID.run() == iTime.eventID().run()) {
          ...
          if ((iTime.eventID().luminosityBlock() -
               closestID.luminosityBlock()) *
                  kLumiTimeLength <
            ...
            oInterval = ValidityInterval(IOVSyncValue(closestTimeStamp),
                                         IOVSyncValue::invalidIOVSyncValue());
          }
        } else {
