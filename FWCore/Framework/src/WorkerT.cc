...
template <typename T>
inline WorkerT<T>::WorkerT(std::shared_ptr<T> ed, ModuleDescription const& md,
...
template <typename T>
inline bool WorkerT<T>::implDo(EventPrincipal const& ep, EventSetup const& c,
                               ModuleCallingContext const* mcc) {
  std::shared_ptr<Worker> sentry(this,
                                 [&ep](Worker* obj) { obj->postDoEvent(ep); });
  return module_->doEvent(ep, c, activityRegistry(), mcc);
}

template <typename T>
inline bool WorkerT<T>::implDoPrePrefetchSelection(
    StreamID id, EventPrincipal const& ep, ModuleCallingContext const* mcc) {
  return true;
}

template <>
inline bool WorkerT<OutputModule>::implDoPrePrefetchSelection(
    StreamID id, EventPrincipal const& ep, ModuleCallingContext const* mcc) {
  return module_->prePrefetchSelection(id, ep, mcc);
}

template <>
inline bool WorkerT<edm::one::OutputModuleBase>::implDoPrePrefetchSelection(
    StreamID id, EventPrincipal const& ep, ModuleCallingContext const* mcc) {
  return module_->prePrefetchSelection(id, ep, mcc);
}

template <>
inline bool WorkerT<edm::global::OutputModuleBase>::implDoPrePrefetchSelection(
    StreamID id, EventPrincipal const& ep, ModuleCallingContext const* mcc) {
  return module_->prePrefetchSelection(id, ep, mcc);
}

template <typename T>
inline bool WorkerT<T>::implDoBegin(RunPrincipal const& rp, EventSetup const& c,
                                    ModuleCallingContext const* mcc) {
...
template <typename T>
template <typename D>
void WorkerT<T>::callWorkerStreamBegin(D, StreamID id, RunPrincipal const& rp,
                                       EventSetup const& c,
                                       ModuleCallingContext const* mcc) {
...
template <typename T>
template <typename D>
void WorkerT<T>::callWorkerStreamEnd(D, StreamID id, RunPrincipal const& rp,
                                     EventSetup const& c,
                                     ModuleCallingContext const* mcc) {
...
template <typename T>
inline bool WorkerT<T>::implDoStreamBegin(StreamID id, RunPrincipal const& rp,
                                          EventSetup const& c,
                                          ModuleCallingContext const* mcc) {
...
template <typename T>
inline bool WorkerT<T>::implDoStreamEnd(StreamID id, RunPrincipal const& rp,
                                        EventSetup const& c,
                                        ModuleCallingContext const* mcc) {
  std::conditional_t<workerimpl::has_stream_functions<T>::value,
...
template <typename T>
inline bool WorkerT<T>::implDoEnd(RunPrincipal const& rp, EventSetup const& c,
                                  ModuleCallingContext const* mcc) {
...
template <typename T>
template <typename D>
void WorkerT<T>::callWorkerStreamBegin(D, StreamID id,
...
template <typename T>
template <typename D>
void WorkerT<T>::callWorkerStreamEnd(D, StreamID id,
                                     LuminosityBlockPrincipal const& rp,
                                     EventSetup const& c,
                                     ModuleCallingContext const* mcc) {
...
inline bool WorkerT<T>::implDoStreamBegin(StreamID id,
                                          ...
                                          ModuleCallingContext const* mcc) {
  std::conditional_t<
      workerimpl::has_stream_functions<T>::value,
...
inline void WorkerT<T>::implBeginStream(StreamID id) {
  std::conditional_t<workerimpl::has_stream_functions<T>::value,
                     workerimpl::DoBeginStream<T>, workerimpl::DoNothing>
      might_call;
  might_call(this, id);
}
...
inline void WorkerT<T>::implEndStream(StreamID id) {
  std::conditional_t<workerimpl::has_stream_functions<T>::value,
                     workerimpl::DoEndStream<T>, workerimpl::DoNothing>
      might_call;
  might_call(this, id);
}
...
template <typename T>
inline void WorkerT<T>::implRegisterThinnedAssociations(
    ProductRegistry const& registry, ThinnedAssociationsHelper& helper) {
  module_->doRegisterThinnedAssociations(registry, helper);
}

template <typename T>
inline SerialTaskQueueChain* WorkerT<T>::serializeRunModule() {
  return nullptr;
}
template <>
SerialTaskQueueChain* WorkerT<EDAnalyzer>::serializeRunModule() {
  return &(module_->sharedResourcesAcquirer().serialQueueChain());
}
template <>
SerialTaskQueueChain* WorkerT<EDFilter>::serializeRunModule() {
  return &(module_->sharedResourcesAcquirer().serialQueueChain());
}
template <>
SerialTaskQueueChain* WorkerT<EDProducer>::serializeRunModule() {
  return &(module_->sharedResourcesAcquirer().serialQueueChain());
}
template <>
SerialTaskQueueChain* WorkerT<OutputModule>::serializeRunModule() {
  return &(module_->sharedResourcesAcquirer().serialQueueChain());
}
template <>
SerialTaskQueueChain* WorkerT<one::EDAnalyzerBase>::serializeRunModule() {
  return &(module_->sharedResourcesAcquirer().serialQueueChain());
}
template <>
SerialTaskQueueChain* WorkerT<one::EDFilterBase>::serializeRunModule() {
  return &(module_->sharedResourcesAcquirer().serialQueueChain());
}
template <>
SerialTaskQueueChain* WorkerT<one::EDProducerBase>::serializeRunModule() {
  return &(module_->sharedResourcesAcquirer().serialQueueChain());
}
template <>
SerialTaskQueueChain* WorkerT<one::OutputModuleBase>::serializeRunModule() {
  return &(module_->sharedResourcesAcquirer().serialQueueChain());
}
...
namespace {
void resolvePutIndiciesImpl(
    void*, BranchType iBranchType,
...
void resolvePutIndiciesImpl(
    ProducerBase* iProd, BranchType iBranchType,
    std::unordered_multimap<std::string, edm::ProductResolverIndex> const&
        iIndicies,
    std::string const& iModuleLabel) {
...
void resolvePutIndiciesImpl(
    edm::stream::EDProducerAdaptorBase* iProd, BranchType iBranchType,
    std::unordered_multimap<std::string, edm::ProductResolverIndex> const&
        iIndicies,
    std::string const& iModuleLabel) {
  iProd->resolvePutIndicies(iBranchType, iIndicies, iModuleLabel);
...
void resolvePutIndiciesImpl(
    edm::stream::EDFilterAdaptorBase* iProd, BranchType iBranchType,
    std::unordered_multimap<std::string, edm::ProductResolverIndex> const&
        iIndicies,
    std::string const& iModuleLabel) {
  iProd->resolvePutIndicies(iBranchType, iIndicies, iModuleLabel);
...
std::vector<ProductResolverIndex> const& itemsShouldPutInEventImpl(
    void const*) {
...
std::vector<ProductResolverIndex> const& itemsShouldPutInEventImpl(
    edm::stream::EDProducerAdaptorBase const* iProd) {
  return iProd->indiciesForPutProducts(edm::InEvent);
}

std::vector<ProductResolverIndex> const& itemsShouldPutInEventImpl(
    edm::stream::EDFilterAdaptorBase const* iProd) {
  return iProd->indiciesForPutProducts(edm::InEvent);
}
}
...
void WorkerT<T>::resolvePutIndicies(
    ...
    std::unordered_multimap<std::string, edm::ProductResolverIndex> const&
        iIndicies) {
  resolvePutIndiciesImpl(&module(), iBranchType, iIndicies,
                         description().moduleLabel());
}

template <typename T>
std::vector<ProductResolverIndex> const& WorkerT<T>::itemsShouldPutInEvent()
    const {
