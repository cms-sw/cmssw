...
namespace edm {
  namespace workerimpl {
    ...
    struct DoStreamBeginTrans {
      inline void operator()(WorkerT<T>* iWorker, StreamID id, P const& rp, EventSetup const& c,
  ...
  template <typename T>
  inline bool WorkerT<T>::implDo(EventPrincipal const& ep, EventSetup const& c, ModuleCallingContext const* mcc) {
  ...
  template <typename T>
  inline bool WorkerT<T>::implDoPrePrefetchSelection(StreamID id, EventPrincipal const& ep,
                                                     ModuleCallingContext const* mcc) {
    return true;
  }
  template <typename T>
  ...
  template <>
  inline bool WorkerT<OutputModule>::implDoPrePrefetchSelection(StreamID id, EventPrincipal const& ep,
                                                                ModuleCallingContext const* mcc) {
  ...
  template <>
  inline bool WorkerT<edm::one::OutputModuleBase>::implDoPrePrefetchSelection(StreamID id, EventPrincipal const& ep,
                                                                              ModuleCallingContext const* mcc) {
    return module_->prePrefetchSelection(id, ep, mcc);
  }
  template <>
  ...
  template <>
  inline bool WorkerT<edm::global::OutputModuleBase>::implDoPrePrefetchSelection(StreamID id, EventPrincipal const& ep,
                                                                                 ModuleCallingContext const* mcc) {
    return module_->prePrefetchSelection(id, ep, mcc);
  }
  template <>
  ...
  template <>
  inline bool WorkerT<edm::limited::OutputModuleBase>::implDoPrePrefetchSelection(StreamID id, EventPrincipal const& ep,
                                                                                  ModuleCallingContext const* mcc) {
    return module_->prePrefetchSelection(id, ep, mcc);
  }
  template <>
  ...
  template <typename T>
  inline bool WorkerT<T>::implDoBegin(RunPrincipal const& rp, EventSetup const& c, ModuleCallingContext const* mcc) {
  ...
  template <typename T>
  template <typename D>
  void WorkerT<T>::callWorkerStreamBegin(D, StreamID id, RunPrincipal const& rp, EventSetup const& c,
                                         ModuleCallingContext const* mcc) {
  ...
  template <typename T>
  template <typename D>
  void WorkerT<T>::callWorkerStreamEnd(D, StreamID id, RunPrincipal const& rp, EventSetup const& c,
                                       ModuleCallingContext const* mcc) {
    module_->doStreamEndRun(id, rp, c, mcc);
  }

  template <typename T>
  inline bool WorkerT<T>::implDoStreamBegin(StreamID id, RunPrincipal const& rp, EventSetup const& c,
                                            ModuleCallingContext const* mcc) {
  ...
  template <typename T>
  inline bool WorkerT<T>::implDoStreamEnd(StreamID id, RunPrincipal const& rp, EventSetup const& c,
                                          ModuleCallingContext const* mcc) {
  ...
  template <typename T>
  inline bool WorkerT<T>::implDoEnd(RunPrincipal const& rp, EventSetup const& c, ModuleCallingContext const* mcc) {
    module_->doEndRun(rp, c, mcc);
    return true;
  }

  template <typename T>
  inline bool WorkerT<T>::implDoBegin(LuminosityBlockPrincipal const& lbp, EventSetup const& c,
                                      ModuleCallingContext const* mcc) {
  ...
  template <typename T>
  template <typename D>
  void WorkerT<T>::callWorkerStreamBegin(D, StreamID id, LuminosityBlockPrincipal const& rp, EventSetup const& c,
                                         ModuleCallingContext const* mcc) {
  ...
  template <typename T>
  template <typename D>
  void WorkerT<T>::callWorkerStreamEnd(D, StreamID id, LuminosityBlockPrincipal const& rp, EventSetup const& c,
                                       ModuleCallingContext const* mcc) {
    module_->doStreamEndLuminosityBlock(id, rp, c, mcc);
  }

  template <typename T>
  inline bool WorkerT<T>::implDoStreamBegin(StreamID id, LuminosityBlockPrincipal const& lbp, EventSetup const& c,
                                            ModuleCallingContext const* mcc) {
    std::conditional_t<workerimpl::has_stream_functions<T>::value,
  ...
  template <typename T>
  inline bool WorkerT<T>::implDoStreamEnd(StreamID id, LuminosityBlockPrincipal const& lbp, EventSetup const& c,
                                          ModuleCallingContext const* mcc) {
    std::conditional_t<workerimpl::has_stream_functions<T>::value,
  ...
  template <typename T>
  inline bool WorkerT<T>::implDoEnd(LuminosityBlockPrincipal const& lbp, EventSetup const& c,
                                    ModuleCallingContext const* mcc) {
  ...
  namespace {
    ...
    void resolvePutIndiciesImpl(void*, BranchType iBranchType, ModuleToResolverIndicies const& iIndicies,
                                std::string const& iModuleLabel) {
      // Do nothing
    }

    void resolvePutIndiciesImpl(ProducerBase* iProd, BranchType iBranchType, ModuleToResolverIndicies const& iIndicies,
                                std::string const& iModuleLabel) {
      iProd->resolvePutIndicies(iBranchType, iIndicies, iModuleLabel);
    }

    void resolvePutIndiciesImpl(edm::stream::EDProducerAdaptorBase* iProd, BranchType iBranchType,
                                ModuleToResolverIndicies const& iIndicies, std::string const& iModuleLabel) {
      iProd->resolvePutIndicies(iBranchType, iIndicies, iModuleLabel);
    }
    void resolvePutIndiciesImpl(edm::stream::EDFilterAdaptorBase* iProd, BranchType iBranchType,
                                ModuleToResolverIndicies const& iIndicies, std::string const& iModuleLabel) {
      iProd->resolvePutIndicies(iBranchType, iIndicies, iModuleLabel);
    }
