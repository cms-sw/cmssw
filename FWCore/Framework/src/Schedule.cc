...
namespace edm {
  ...
  namespace {
    ...
    std::shared_ptr<TriggerResultInserter> makeInserter(ParameterSet& proc_pset,
      ...
      try {
        maker::ModuleHolderT<TriggerResultInserter> holder(
            std::make_shared<TriggerResultInserter>(*trig_pset, iPrealloc.numberOfStreams()),
    ...
    void makePathStatusInserters(std::vector<edm::propagate_const<std::shared_ptr<T>>>& pathStatusInserters,
                                 std::vector<std::string> const& pathNames, PreallocationConfiguration const& iPrealloc,
                                 ProductRegistry& preg, std::shared_ptr<ActivityRegistry> areg,
                                 std::shared_ptr<ProcessConfiguration> processConfiguration,
    ...
    void checkAndInsertAlias(std::string const& friendlyClassName, std::string const& moduleLabel,
                             std::string const& productInstanceName, std::string const& processName,
                             std::string const& alias, std::string const& instanceAlias, ProductRegistry const& preg,
                             std::multimap<BranchKey, BranchKey>& aliasMap, std::map<BranchKey, BranchKey>& aliasKeys) {
    ...
    void reduceParameterSet(ParameterSet& proc_pset, vstring const& end_path_name_list, vstring& modulesInConfig,
  ...
  Schedule::Schedule(ParameterSet& proc_pset, service::TriggerNamesService const& tns, ProductRegistry& preg,
                     BranchIDListHelper& branchIDListHelper, ThinnedAssociationsHelper& thinnedAssociationsHelper,
                     SubProcessParentageHelper const* subProcessParentageHelper, ExceptionToActionTable const& actions,
                     std::shared_ptr<ActivityRegistry> areg, std::shared_ptr<ProcessConfiguration> processConfiguration,
                     bool hasSubprocesses, PreallocationConfiguration const& prealloc,
    ...
    if (wantSummary_) {
      ...
      // areg->preModuleEventSignal_.connect([timeKeeperPtr](StreamContext const& iContext, ModuleCallingContext const&
      // iMod) {
  ...
  void Schedule::writeRun(RunPrincipal const& rp, ProcessContext const* processContext) {
    ...
    for_all(all_output_communicators_,
            std::bind(&OutputModuleCommunicator::writeRun, _1, std::cref(rp), processContext));
  }

  void Schedule::writeLumi(LuminosityBlockPrincipal const& lbp, ProcessContext const* processContext) {
    using std::placeholders::_1;
    for_all(all_output_communicators_,
            std::bind(&OutputModuleCommunicator::writeLumi, _1, std::cref(lbp), processContext));
  }
  ...
  void Schedule::processOneEventAsync(WaitingTaskHolder iTask, unsigned int iStreamID, EventPrincipal& ep,
                                      EventSetup const& es) {
    assert(iStreamID < streamSchedules_.size());
