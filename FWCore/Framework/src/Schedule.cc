...
bool binary_search_string(std::vector<std::string> const& v,
                          std::string const& s) {
...
std::shared_ptr<TriggerResultInserter> makeInserter(
    ParameterSet& proc_pset, PreallocationConfiguration const& iPrealloc,
    ProductRegistry& preg, ExceptionToActionTable const& actions,
    std::shared_ptr<ActivityRegistry> areg,
    std::shared_ptr<ProcessConfiguration> processConfiguration) {
  ParameterSet* trig_pset = proc_pset.getPSetForUpdate("@trigger_paths");
  trig_pset->registerIt();

  WorkerParams work_args(trig_pset, preg, &iPrealloc, processConfiguration,
                         actions);
  ModuleDescription md(trig_pset->id(), "TriggerResultInserter",
                       "TriggerResults", processConfiguration.get(),
  ...
  try {
    ...
    if (!postCalled) {
      try {
        ...
        // If post throws an exception ignore it because we are already handling
        // another exception
      }
    }
    throw;
...
void makePathStatusInserters(
    ...
    PreallocationConfiguration const& iPrealloc, ProductRegistry& preg,
    std::shared_ptr<ActivityRegistry> areg,
    std::shared_ptr<ProcessConfiguration> processConfiguration,
  ...
  for (auto const& pathName : pathNames) {
    ModuleDescription md(pset.id(), moduleTypeName, pathName,
    ...
    try {
      maker::ModuleHolderT<T> holder(
          std::make_shared<T>(iPrealloc.numberOfStreams()),
          static_cast<Maker const*>(nullptr));
      holder.setModuleDescription(md);
      holder.registerProductsAndCallbacks(&preg);
      ...
      if (!postCalled) {
        try {
          ...
          // If post throws an exception ignore it because we are already
...
void checkAndInsertAlias(
    std::string const& friendlyClassName, std::string const& moduleLabel,
    std::string const& productInstanceName, std::string const& processName,
    std::string const& alias, std::string const& instanceAlias,
    ProductRegistry const& preg, std::multimap<BranchKey, BranchKey>& aliasMap,
    std::map<BranchKey, BranchKey>& aliasKeys) {
  std::string const star("*");

  BranchKey key(friendlyClassName, moduleLabel, productInstanceName,
                processName);
  if (preg.productList().find(key) == preg.productList().end()) {
    // No product was found matching the alias.
    // We throw an exception only if a module with the specified module label
    // was created in this process.
    for (auto const& product : preg.productList()) {
      if (moduleLabel == product.first.moduleLabel() &&
          processName == product.first.processName()) {
        throw Exception(errors::Configuration, "EDAlias does not match data\n")
            << "There are no products of type '" << friendlyClassName << "'\n"
            << "with module label '" << moduleLabel << "' and instance name '"
            << productInstanceName << "'.\n";
      }
    }
  }

  std::string const& theInstanceAlias(
      instanceAlias == star ? productInstanceName : instanceAlias);
  ...
  if (preg.productList().find(aliasKey) != preg.productList().end()) {
    throw Exception(errors::Configuration, "EDAlias conflicts with data\n")
        ...
        << "with module label '" << alias << "' and instance name '"
        << theInstanceAlias << "'\n"
        << "already exists.\n";
  }
  auto iter = aliasKeys.find(aliasKey);
  if (iter != aliasKeys.end()) {
    // The alias matches a previous one.  If the same alias is used for
    // different product, throw.
    if (iter->second != key) {
    ...
    if (prodIter != preg.productList().end()) {
      if (!prodIter->second.produced()) {
        throw Exception(errors::Configuration, "EDAlias\n")
            ...
            << "' and product instance name '" << productInstanceName << "',\n"
            << "An EDAlias can only be used for products produced in the "
               "current process. This one is not.\n";
      }
...
void processEDAliases(ParameterSet const& proc_pset,
                      std::string const& processName, ProductRegistry& preg) {
  std::vector<std::string> aliases =
      proc_pset.getParameter<std::vector<std::string>>("@all_aliases");
  if (aliases.empty()) {
    return;
  }
  std::string const star("*");
  ...
  std::map<BranchKey, BranchKey>
      aliasKeys;  // Used to search for duplicates or clashes.
  ...
  for (std::string const& alias : aliases) {
    ...
    std::vector<std::string> vPSetNames =
        aliasPSet.getParameterNamesForType<VParameterSet>();
    for (std::string const& moduleLabel : vPSetNames) {
      ...
      for (ParameterSet& pset : vPSet) {
        ...
        std::string productInstanceName =
            pset.getParameter<std::string>("fromProductInstance");
        std::string instanceAlias =
            pset.getParameter<std::string>("toProductInstance");
        if (productInstanceName == star) {
          ...
          for (ProductRegistry::ProductList::const_iterator it =
                   preg.productList().lower_bound(lowerBound);
               it != preg.productList().end() &&
               it->first.friendlyClassName() == friendlyClassName &&
            ...
            checkAndInsertAlias(
                friendlyClassName, moduleLabel, it->first.productInstanceName(),
                processName, alias, instanceAlias, preg, aliasMap, aliasKeys);
          }
          if (!match) {
            // No product was found matching the alias.
            // We throw an exception only if a module with the specified module
            // label was created in this process.
            for (auto const& product : preg.productList()) {
              if (moduleLabel == product.first.moduleLabel() &&
                  processName == product.first.processName()) {
          ...
          checkAndInsertAlias(friendlyClassName, moduleLabel,
                              productInstanceName, processName, alias,
  ...
  for (auto const& aliasEntry : aliasMap) {
    ProductRegistry::ProductList::const_iterator it =
        preg.productList().find(aliasEntry.first);
    assert(it != preg.productList().end());
    preg.addLabelAlias(it->second, aliasEntry.second.moduleLabel(),
                       aliasEntry.second.productInstanceName());
  }
}

typedef std::vector<std::string> vstring;

void reduceParameterSet(
    ParameterSet& proc_pset, vstring const& end_path_name_list,
    vstring& modulesInConfig, std::set<std::string> const& usedModuleLabels,
  ...
  {
    std::set<std::string> noEndPaths(scheduledPaths.begin(),
                                     scheduledPaths.end());
  ...
  for (auto const& modLabel : usedModuleLabels) {
    ...
    // PathStatusInserter and EndPathStatusInserter will not have a
    // ParameterSet.
    if (proc_pset.existsAs<ParameterSet>(modLabel)) {
      edmType = proc_pset.getParameterSet(modLabel).getParameter<std::string>(
          moduleEdmType);
  ...
  for_all(labelsToBeDropped,
          std::bind(&ParameterSet::eraseOrSetUntrackedParameterSet,
                    std::ref(proc_pset), _1));

  // drop the labels from @all_modules
  vstring::iterator endAfterRemove = std::remove_if(
      modulesInConfig.begin(), modulesInConfig.end(),
  ...
  vstring labels;
  for (vstring::const_iterator iEndPath = end_path_name_list.begin(),
                               endEndPath = end_path_name_list.end();
    ...
    for (vstring::iterator iLabel = labels.begin(), iEnd = labels.end();
      ...
      if (binary_search_string(labelsToBeDropped, *iLabel)) {
        if (binary_search_string(outputModuleLabels, *iLabel)) {
          outputModulePathPositions[*iLabel].emplace_back(*iEndPath,
                                                          iSave - iBegin);
        }
      } else {
        if (iSave != iLabel) {
  ...
  endAfterRemove = std::remove_if(
      scheduledPaths.begin(), scheduledPaths.end(),
  ...
  endAfterRemove = std::remove_if(
      scheduledEndPaths.begin(), scheduledEndPaths.end(),
      std::bind(binary_search_string, std::ref(endPathsToBeDropped), _1));
...
Schedule::Schedule(ParameterSet& proc_pset,
    ...
    :  // Only create a resultsInserter if there is a trigger path
      resultsInserter_{tns.getTrigPaths().empty()
                           ? std::shared_ptr<TriggerResultInserter>{}
                           : makeInserter(proc_pset, prealloc, preg, actions,
  ...
  makePathStatusInserters(pathStatusInserters_, *pathNames_, prealloc, preg,
                          areg, processConfiguration,
                          std::string("PathStatusInserter"));

  makePathStatusInserters(endPathStatusInserters_, *endPathNames_, prealloc,
                          preg, areg, processConfiguration,
  ...
  for (unsigned int i = 0; i < prealloc.numberOfStreams(); ++i) {
    streamSchedules_.emplace_back(std::make_shared<StreamSchedule>(
        resultsInserter(), pathStatusInserters_, endPathStatusInserters_,
        moduleRegistry(), proc_pset, tns, prealloc, preg, branchIDListHelper,
        actions, areg, processConfiguration, !hasSubprocesses, StreamID{i},
        processContext));
  }
  ...
  // The unscheduled modules are at the end of the list, but we want them at the
  // front
  ...
  if (n > 0) {
    ...
    std::copy(modulesToUse.begin(), itBeginUnscheduled,
  ...
  globalSchedule_ = std::make_unique<GlobalSchedule>(
      resultsInserter(), pathStatusInserters_, endPathStatusInserters_,
      moduleRegistry(), modulesToUse, proc_pset, preg, prealloc, actions, areg,
      processConfiguration, processContext);
  ...
  std::vector<std::string> modulesInConfig(
      proc_pset.getParameter<std::vector<std::string>>("@all_modules"));
  std::map<std::string, std::vector<std::pair<std::string, int>>>
      outputModulePathPositions;
  reduceParameterSet(proc_pset, tns.getEndPaths(), modulesInConfig,
                     usedModuleLabels, outputModulePathPositions);
  ...
  moduleRegistry_->forAllModuleHolders([this](maker::ModuleHolder* iHolder) {
    ...
    if (comm) {
      all_output_communicators_.emplace_back(
          std::shared_ptr<OutputModuleCommunicator>{comm.release()});
    }
  });
  // Now that the output workers are filled in, set any output limits or
  // information.
...

  {
    ...
    preg.setFrozen(productTypesConsumed, elementTypesConsumed,
                   processConfiguration->processName());
  }

  for (auto& c : all_output_communicators_) {
    c->setEventSelectionInfo(outputModulePathPositions,
                             preg.anyProductProduced());
  }
  ...
  if (wantSummary_) {
    ...
    summaryTimeKeeper_ = std::make_unique<SystemTimeKeeper>(
        prealloc.numberOfStreams(), modDesc, tns, processContext);
    auto timeKeeperPtr = summaryTimeKeeper_.get();

    areg->watchPreModuleEvent(timeKeeperPtr,
                              &SystemTimeKeeper::startModuleEvent);
    areg->watchPostModuleEvent(timeKeeperPtr,
                               &SystemTimeKeeper::stopModuleEvent);
    areg->watchPreModuleEventDelayedGet(timeKeeperPtr,
                                        &SystemTimeKeeper::pauseModuleEvent);
    areg->watchPostModuleEventDelayedGet(timeKeeperPtr,
                                         &SystemTimeKeeper::restartModuleEvent);
    ...
    areg->watchPostBeginJob(timeKeeperPtr,
                            &SystemTimeKeeper::startProcessingLoop);
    areg->watchPreEndJob(timeKeeperPtr, &SystemTimeKeeper::stopProcessingLoop);
    // areg->preModuleEventSignal_.connect([timeKeeperPtr](StreamContext const&
    // iContext, ModuleCallingContext const& iMod) {
...
void Schedule::limitOutput(
    ParameterSet const& proc_pset, BranchIDLists const& branchIDLists,
    SubProcessParentageHelper const* subProcessParentageHelper) {
  std::string const output("output");

  ParameterSet const& maxEventsPSet =
      proc_pset.getUntrackedParameterSet("maxEvents", ParameterSet());
  int maxEventSpecs = 0;
  int maxEventsOut = -1;
  ParameterSet const* vMaxEventsOut = 0;
  std::vector<std::string> intNamesE =
      maxEventsPSet.getParameterNamesForType<int>(false);
  ...
  if (maxEventSpecs > 1) {
    throw Exception(errors::Configuration) << "\nAt most, one form of 'output' "
                                              "may appear in the 'maxEvents' "
                                              "parameter set";
  }
  ...
  for (auto& c : all_output_communicators_) {
    ...
    if (vMaxEventsOut != 0 && !vMaxEventsOut->empty()) {
      ...
      try {
        desc.maxEvents_ =
            vMaxEventsOut->getUntrackedParameter<int>(moduleLabel);
...
void Schedule::endJob(ExceptionCollector& collector) {
  ...
  if (wantSummary_ == false) return;
  {
    ...
    for (auto const& p : tr.trigPathSummaries) {
      LogVerbatim("FwkSummary")
          << "TrigReport " << std::right << std::setw(5) << 1 << std::right
          << std::setw(5) << p.bitPosition << " " << std::right << std::setw(10)
          << p.timesRun << " " << std::right << std::setw(10) << p.timesPassed
          << " " << std::right << std::setw(10) << p.timesFailed << " "
          << std::right << std::setw(10) << p.timesExcept << " " << p.name
          << "";
    }
    ...
    std::vector<std::string>::const_iterator  epn =
    empty_trig_path_names_.begin();
    for (; epi != epe; ++epi, ++epn) {
    ...
    for (auto const& p : tr.endPathSummaries) {
      LogVerbatim("FwkSummary")
          << "TrigReport " << std::right << std::setw(5) << 0 << std::right
          << std::setw(5) << p.bitPosition << " " << std::right << std::setw(10)
          << p.timesRun << " " << std::right << std::setw(10) << p.timesPassed
          << " " << std::right << std::setw(10) << p.timesFailed << " "
          << std::right << std::setw(10) << p.timesExcept << " " << p.name
    ...
    for (auto const& p : tr.trigPathSummaries) {
      ...
      for (auto const& mod : p.moduleInPathSummaries) {
        LogVerbatim("FwkSummary")
            << "TrigReport " << std::right << std::setw(5) << 1 << std::right
            << std::setw(5) << bitpos << " " << std::right << std::setw(10)
            << mod.timesVisited << " " << std::right << std::setw(10)
            << mod.timesPassed << " " << std::right << std::setw(10)
            << mod.timesFailed << " " << std::right << std::setw(10)
  ...
  LogVerbatim("FwkSummary") << "TimeReport "
                            << "---------- Event  Summary ---[sec]----";
  LogVerbatim("FwkSummary") << "TimeReport" << std::setprecision(6)
                            << std::fixed << "       event loop CPU/event = "
                            << tr.eventSummary.cpuTime / totalEvents;
  LogVerbatim("FwkSummary") << "TimeReport" << std::setprecision(6)
                            << std::fixed << "      event loop Real/event = "
                            << tr.eventSummary.realTime / totalEvents;
  LogVerbatim("FwkSummary") << "TimeReport" << std::setprecision(6)
                            << std::fixed << "     sum Streams Real/event = "
                            << tr.eventSummary.sumStreamRealTime / totalEvents;
  LogVerbatim("FwkSummary")
  ...
  for (auto const& p : tr.trigPathSummaries) {
    ...
    LogVerbatim("FwkSummary")
        ...
        << std::setw(kColumn1Size) << p.realTime / totalEvents << " "
  ...
  for (auto const& p : tr.trigPathSummaries) {
    ...
    for (auto const& mod : p.moduleInPathSummaries) {
      LogVerbatim("FwkSummary")
          << "TimeReport " << std::setprecision(6) << std::fixed << std::right
          << std::setw(kColumn1Size) << mod.realTime / totalEvents << " "
          << std::right << std::setw(kColumn2Size)
          << mod.realTime / std::max(1, mod.timesVisited) << "  "
          << mod.moduleLabel << "";
    }
  }
  ...
  for (auto const& worker : tr.workerSummaries) {
    LogVerbatim("FwkSummary")
        << "TimeReport " << std::setprecision(6) << std::fixed << std::right
        << std::setw(kColumn1Size) << worker.realTime / totalEvents << " "
        << std::right << std::setw(kColumn2Size)
        << worker.realTime / std::max(1, worker.timesRun) << " " << std::right
        << std::setw(kColumn3Size)
        << worker.realTime / std::max(1, worker.timesVisited) << "  "
        << worker.moduleLabel << "";
  }
  LogVerbatim("FwkSummary")
      << "TimeReport " << std::right << std::setw(kColumn1Size) << "per event"
...
void Schedule::closeOutputFiles() {
  ...
  for_all(all_output_communicators_,
          std::bind(&OutputModuleCommunicator::closeFile, _1));
}

void Schedule::openOutputFiles(FileBlock& fb) {
  using std::placeholders::_1;
  for_all(all_output_communicators_,
          std::bind(&OutputModuleCommunicator::openFile, _1, std::cref(fb)));
}
...
void Schedule::writeLumi(LuminosityBlockPrincipal const& lbp,
  ...
  using std::placeholders::_1;
  for_all(all_output_communicators_,
          std::bind(&OutputModuleCommunicator::writeLumi, _1, std::cref(lbp),
                    processContext));
}
...
bool Schedule::shouldWeCloseOutput() const {
  ...
  return (std::find_if(all_output_communicators_.begin(),
                       ...
                       std::bind(&OutputModuleCommunicator::shouldWeCloseFile,
                                 _1)) != all_output_communicators_.end());
}

void Schedule::respondToOpenInputFile(FileBlock const& fb) {
  using std::placeholders::_1;
  for_all(allWorkers(),
          std::bind(&Worker::respondToOpenInputFile, _1, std::cref(fb)));
}

void Schedule::respondToCloseInputFile(FileBlock const& fb) {
  using std::placeholders::_1;
  for_all(allWorkers(),
          std::bind(&Worker::respondToCloseInputFile, _1, std::cref(fb)));
}
...
void Schedule::processOneEventAsync(WaitingTaskHolder iTask,
                                    unsigned int iStreamID, EventPrincipal& ep,
                                    EventSetup const& es) {
  assert(iStreamID < streamSchedules_.size());
...
std::vector<ModuleDescription const*> Schedule::getAllModuleDescriptions()
    const {
...
Schedule::AllWorkers const& Schedule::allWorkers() const {
...
void Schedule::triggerPaths(std::vector<std::string>& oLabelsToFill) const {
  oLabelsToFill = *pathNames_;
}

void Schedule::endPaths(std::vector<std::string>& oLabelsToFill) const {
  oLabelsToFill = *endPathNames_;
}

void Schedule::modulesInPath(std::string const& iPathLabel,
                             std::vector<std::string>& oLabelsToFill) const {
  streamSchedules_[0]->modulesInPath(iPathLabel, oLabelsToFill);
}

void Schedule::moduleDescriptionsInPath(
...
void Schedule::moduleDescriptionsInEndPath(
    std::string const& iEndPathLabel,
    std::vector<ModuleDescription const*>& descriptions,
    unsigned int hint) const {
  streamSchedules_[0]->moduleDescriptionsInEndPath(iEndPathLabel, descriptions,
                                                   hint);
}
...
void Schedule::fillModuleAndConsumesInfo(
    ...
    std::vector<std::vector<ModuleDescription const*>>&
  ...
  for (auto const& worker : allWorkers()) {
    ...
    moduleIDToIndex.push_back(
        std::pair<unsigned int, unsigned int>(p->id(), i));
  ...
  for (auto const& worker : allWorkers()) {
    std::vector<ModuleDescription const*>& modules =
        modulesWhoseProductsAreConsumedBy.at(i);
