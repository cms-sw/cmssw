...
bool EDProducer::doEvent(EventPrincipal const& ep, EventSetup const& c,
...
void EDProducer::doBeginRun(RunPrincipal const& rp, EventSetup const& c,
                            ModuleCallingContext const* mcc) {
...
void EDProducer::doEndRun(RunPrincipal const& rp, EventSetup const& c,
                          ModuleCallingContext const* mcc) {
  Run r(rp, moduleDescription_, mcc);
  r.setConsumer(this);
  Run const& cnstR = r;
...
void EDProducer::doRespondToOpenInputFile(FileBlock const& fb) {
  respondToOpenInputFile(fb);
}

void EDProducer::doRespondToCloseInputFile(FileBlock const& fb) {
...
void EDProducer::prevalidate(ConfigurationDescriptions& iConfig) {
