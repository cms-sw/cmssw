...
void principal_get_adapter_detail::throwOnPutOfNullProduct(
    char const* principalType, TypeID const& productType,
...
void principal_get_adapter_detail::throwOnPrematureRead(
    char const* principalType, TypeID const& productType,
    std::string const& moduleLabel, std::string const& productInstanceName) {
  // throw Exception(errors::LogicError)
  LogWarning("LogicError") << "::getByLabel: An attempt was made to read a "
                           << principalType << " product before end"
...
void principal_get_adapter_detail::throwOnPrematureRead(
    char const* principalType, TypeID const& productType) {
  // throw Exception(errors::LogicError)
  LogWarning("LogicError") << "::getManyByType: An attempt was made to read a "
                           << principalType << " product before end"
                           << principalType << "() was called.\n"
                           << "The product is of type '" << productType
                           << "'.\n";
...
void principal_get_adapter_detail::throwOnPrematureRead(
    char const* principalType, TypeID const& productType, EDGetToken token) {
  throw Exception(errors::LogicError)
      << "::getByToken: An attempt was made to read a " << principalType
...
void PrincipalGetAdapter::labelsForToken(EDGetToken const& iToken,
                                         ProductLabels& oLabels) const {
  consumer_->labelsForToken(iToken, oLabels);
}

BasicHandle PrincipalGetAdapter::makeFailToGetException(
    KindOfType kindOfType, TypeID const& productType, EDGetToken token) const {
  ...
  return BasicHandle(makeHandleExceptionFactory(
      [labels, kindOfType, productType]() -> std::shared_ptr<cms::Exception> {
        std::shared_ptr<cms::Exception> exception(
            std::make_shared<Exception>(errors::ProductNotFound));
        if (kindOfType == PRODUCT_TYPE) {
          *exception << "Principal::getByToken: Found zero products matching "
                        "all criteria\nLooking for type: "
                     << productType << "\n"
                     << "Looking for module label: " << labels.module << "\n"
                     << "Looking for productInstanceName: "
                     << labels.productInstance << "\n"
                     << (0 == labels.process[0] ? "" : "Looking for process: ")
                     << labels.process << "\n";
        } else {
          *exception << "Principal::getByToken: Found zero products matching "
                        "all criteria\nLooking for a container with elements "
                        "of type: "
                     << productType << "\n"
                     << "Looking for module label: " << labels.module << "\n"
                     << "Looking for productInstanceName: "
...
void PrincipalGetAdapter::throwAmbiguousException(TypeID const& productType,
  ...
  exception << "Principal::getByToken: More than 1 product matches all "
...
BasicHandle PrincipalGetAdapter::getByLabel_(
  ...
  return principal_.getByLabel(PRODUCT_TYPE, typeID, tag, consumer_,
                               resourcesAcquirer_, mcc);
}

BasicHandle PrincipalGetAdapter::getByLabel_(
    TypeID const& typeID, std::string const& label, std::string const& instance,
  ...
  return principal_.getByLabel(PRODUCT_TYPE, typeID, label, instance, process,
                               consumer_, resourcesAcquirer_, mcc);
}

BasicHandle PrincipalGetAdapter::getByToken_(
    TypeID const& id, KindOfType kindOfType, EDGetToken token,
    ModuleCallingContext const* mcc) const {
  ProductResolverIndexAndSkipBit indexAndBit =
      consumer_->indexFrom(token, branchType(), id);
  ...
  BasicHandle h =
      principal_.getByToken(kindOfType, id, index, skipCurrentProcess,
                            ambiguous, resourcesAcquirer_, mcc);
  if (ambiguous) {
...
BasicHandle PrincipalGetAdapter::getMatchingSequenceByLabel_(
    TypeID const& typeID, std::string const& label, std::string const& instance,
    std::string const& process, ModuleCallingContext const* mcc) const {
  auto h = principal_.getByLabel(ELEMENT_TYPE, typeID, label, instance, process,
                                 consumer_, resourcesAcquirer_, mcc);
  return h;
}

void PrincipalGetAdapter::getManyByType_(
    TypeID const& tid, BasicHandleVec& results,
    ModuleCallingContext const* mcc) const {
...
BranchDescription const& PrincipalGetAdapter::getBranchDescription(
  ...
  ProductResolverIndexHelper const& productResolverIndexHelper =
      principal_.productLookup();
  ProductResolverIndex index = productResolverIndexHelper.index(
      PRODUCT_TYPE, type, md_.moduleLabel().c_str(),
      productInstanceName.c_str(), md_.processName().c_str());
  if (index == ProductResolverIndexInvalid) {
    std::ostringstream str;
    for (auto branchDescription :
         principal_.productRegistry().allBranchDescriptions()) {
    ...
    throw edm::Exception(edm::errors::InsertFailure)
        ...
        << "To correct the problem:\n"
           "   1) make sure the proper 'produce' call is being made in the "
           "module's constructor,\n"
           "   2) if 'produce' exists and uses a product instance name make "
           "sure that same name is used during the 'put' call.";
  }
...
bool PrincipalGetAdapter::isComplete() const { return principal_.isComplete(); }
}
