...
namespace edm {
  ...
  PrincipalGetAdapter::PrincipalGetAdapter(Principal const& pcpl, ModuleDescription const& md)
      :  // putProducts_(),
        principal_(pcpl),
        md_(md),
        consumer_(nullptr),
  ...
  void principal_get_adapter_detail::throwOnPutOfNullProduct(char const* principalType, TypeID const& productType,
  ...
  BasicHandle PrincipalGetAdapter::makeFailToGetException(KindOfType kindOfType, TypeID const& productType,
  ...
  BasicHandle PrincipalGetAdapter::getByLabel_(TypeID const& typeID, InputTag const& tag,
                                               ModuleCallingContext const* mcc) const {
    return principal_.getByLabel(PRODUCT_TYPE, typeID, tag, consumer_, resourcesAcquirer_, mcc);
  }

  BasicHandle PrincipalGetAdapter::getByLabel_(TypeID const& typeID, std::string const& label,
  ...
  BasicHandle PrincipalGetAdapter::getByToken_(TypeID const& id, KindOfType kindOfType, EDGetToken token,
                                               ModuleCallingContext const* mcc) const {
  ...
  BasicHandle PrincipalGetAdapter::getMatchingSequenceByLabel_(TypeID const& typeID, InputTag const& tag,
                                                               ModuleCallingContext const* mcc) const {
    return principal_.getByLabel(ELEMENT_TYPE, typeID, tag, consumer_, resourcesAcquirer_, mcc);
  }

  BasicHandle PrincipalGetAdapter::getMatchingSequenceByLabel_(TypeID const& typeID, std::string const& label,
                                                               std::string const& instance, std::string const& process,
                                                               ModuleCallingContext const* mcc) const {
  ...
  void PrincipalGetAdapter::getManyByType_(TypeID const& tid, BasicHandleVec& results,
                                           ModuleCallingContext const* mcc) const {
