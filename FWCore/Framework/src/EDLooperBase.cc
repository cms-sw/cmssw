...
EDLooperBase::Status EDLooperBase::doDuringLoop(
    edm::EventPrincipal& eventPrincipal, const edm::EventSetup& es,
  ...
  streamContext->setRunIndex(
      eventPrincipal.luminosityBlockPrincipal().runPrincipal().index());
  streamContext->setLuminosityBlockIndex(
      eventPrincipal.luminosityBlockPrincipal().index());
...
EDLooperBase::Status EDLooperBase::doEndOfLoop(const edm::EventSetup& es) {
...
void EDLooperBase::prepareForNextLoop(eventsetup::EventSetupProvider* esp) {
  ...
  std::set<edm::eventsetup::EventSetupRecordKey> const& keys =
      modifyingRecords();
  for_all(keys,
          std::bind(
              &eventsetup::EventSetupProvider::resetRecordPlusDependentRecords,
...
void EDLooperBase::doBeginRun(RunPrincipal& iRP, EventSetup const& iES,
  ...
  GlobalContext globalContext(
      GlobalContext::Transition::kBeginRun, LuminosityBlockID(iRP.run(), 0),
      iRP.index(), LuminosityBlockIndex::invalidLuminosityBlockIndex(),
      iRP.beginTime(), processContext);
...
void EDLooperBase::doEndRun(RunPrincipal& iRP, EventSetup const& iES,
                            ProcessContext* processContext) {
  GlobalContext globalContext(
      GlobalContext::Transition::kEndRun, LuminosityBlockID(iRP.run(), 0),
      iRP.index(), LuminosityBlockIndex::invalidLuminosityBlockIndex(),
      iRP.endTime(), processContext);
  ParentContext parentContext(&globalContext);
  ModuleContextSentry moduleContextSentry(&moduleCallingContext_,
                                          parentContext);
  Run run(iRP, moduleDescription_, &moduleCallingContext_);
...
void EDLooperBase::doBeginLuminosityBlock(LuminosityBlockPrincipal& iLB,
  ...
  GlobalContext globalContext(GlobalContext::Transition::kBeginLuminosityBlock,
                              iLB.id(), iLB.runPrincipal().index(), iLB.index(),
                              iLB.beginTime(), processContext);
  ParentContext parentContext(&globalContext);
  ModuleContextSentry moduleContextSentry(&moduleCallingContext_,
                                          parentContext);
  LuminosityBlock luminosityBlock(iLB, moduleDescription_,
...
std::set<eventsetup::EventSetupRecordKey> EDLooperBase::modifyingRecords()
    const {
  return std::set<eventsetup::EventSetupRecordKey>();
}

void EDLooperBase::copyInfo(const ScheduleInfo& iInfo) {
