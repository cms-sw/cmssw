...
static std::string appendCurrentProcessIfAlias(
    std::string const& processFromInputTag, std::string const& currentProcess) {
...
static void throwProductNotFoundException(char const* where,
...
static std::shared_ptr<cms::Exception> makeNotFoundException(
    char const* where, KindOfType kindOfType, TypeID const& productType,
    std::string const& label, std::string const& instance,
    std::string const& process) {
  std::shared_ptr<cms::Exception> exception =
      std::make_shared<Exception>(errors::ProductNotFound);
  if (kindOfType == PRODUCT_TYPE) {
    ...
    *exception << "Principal::" << where
               << ": Found zero products matching all criteria\nLooking for a "
                  "container with elements of type: "
               << productType << "\n"
               << "Looking for module label: " << label << "\n"
               << "Looking for productInstanceName: " << instance << "\n"
               << (process.empty() ? "" : "Looking for process: ") << process
...
static void throwAmbiguousException(const char* where,
  ...
  exception << "Principal::" << where
            ...
            << (process.empty() ? "" : "Looking for process: ") << process
            << "\n"
            << "This can only occur with get function calls using a "
               "Handle<View> argument.\n"
            << "Try a get not using a View or change the instance name of one "
               "of the products";
  throw exception;
}
...
void failedToRegisterConsumesMany(edm::TypeID const& iType) {
  ...
  exception << "::getManyByType called for " << iType
            << " without a corresponding consumesMany being called for this "
               "module. \n";
  throw exception;
}

void failedToRegisterConsumes(KindOfType kindOfType, TypeID const& productType,
                              std::string const& moduleLabel,
                              std::string const& productInstanceName,
                              std::string const& processName) {
  cms::Exception exception("GetByLabelWithoutRegistration");
  exception << "::getByLabel without corresponding call to consumes or "
               "mayConsumes for this module.\n"
            << (kindOfType == PRODUCT_TYPE ? "  type: " : " type: edm::View<")
            << productType
...
Principal::Principal(
    ...
    ProcessConfiguration const& pc, BranchType bt,
    HistoryAppender* historyAppender, bool isForPrimaryProcess)
    : EDProductGetter(),
      processHistoryPtr_(),
      processHistoryID_(),
  ...
  // The constructor of an alias product holder takes as an argument the product
  // holder for which it is an alias.
  ...
  // empty string
  std::vector<std::string> const& lookupProcessNames =
      productLookup_->lookupProcessNames();
  std::vector<ProductResolverIndex> matchingHolders(
      lookupProcessNames.size(), ProductResolverIndexInvalid);
  ...
  std::vector<ProductResolverIndexHelper::Range> const& ranges =
      productLookup_->ranges();
  std::vector<ProductResolverIndexHelper::IndexAndNames> const& indexAndNames =
      productLookup_->indexAndNames();
  std::vector<char> const& processNamesCharArray =
  ...
  if (!sortedTypeIDs.empty()) {
    ...
    for (unsigned int k = 0, kEnd = sortedTypeIDs.size(); k < kEnd; ++k) {
      ...
      for (unsigned int i = range.begin(); i < range.end(); ++i) {
        ProductResolverIndexHelper::IndexAndNames const& product =
            indexAndNames.at(i);
        if (product.startInProcessNames() == 0) {
          if (productResolverIndex != ProductResolverIndexInvalid) {
            if ((numberOfMatches == 1) and
              ...
              productResolvers_.at(productResolverIndex) =
                  std::make_shared<SingleChoiceNoProcessProductResolver>(
                      lastMatchIndex);
            } else {
              std::shared_ptr<ProductResolverBase> newHolder =
                  std::make_shared<NoProcessProductResolver>(matchingHolders,
                                                             ambiguous);
              productResolvers_.at(productResolverIndex) = newHolder;
            }
            matchingHolders.assign(lookupProcessNames.size(),
          ...
          std::string process(
              &processNamesCharArray.at(product.startInProcessNames()));
          auto iter = std::find(lookupProcessNames.begin(),
                                lookupProcessNames.end(), process);
          ...
          if (iMatchingIndex == ProductResolverIndexAmbiguous) {
            ...
            matchingHolders.at(iter - lookupProcessNames.begin()) =
                iMatchingIndex;
          }
        }
      }
    ...
    std::shared_ptr<ProductResolverBase> newHolder =
        std::make_shared<NoProcessProductResolver>(matchingHolders, ambiguous);
    productResolvers_.at(productResolverIndex) = newHolder;
  }
}
...
size_t Principal::size() const {
  ...
  for (auto const& prod : *this) {
    if (prod->singleProduct() &&  // Not a NoProcessProductResolver
        !prod->productUnavailable() && !prod->unscheduledWasNotRun() &&
...
bool Principal::adjustToNewProductRegistry(ProductRegistry const& reg) {
  ...
  for (auto const& prod : prodsList) {
    ...
    if (!bd.produced() && (bd.branchType() == branchType_)) {
      ...
      if (phb == nullptr ||
          phb->branchDescription().branchName() != cbd->branchName()) {
        return false;
      }
...
void Principal::addScheduledProduct(
...
void Principal::addUnscheduledProduct(
    std::shared_ptr<BranchDescription const> bd) {
  addProductOrThrow(
      std::make_unique<UnscheduledProductResolver>(std::move(bd)));
}
...
void Principal::addAliasedProduct(std::shared_ptr<BranchDescription const> bd) {
  ...
  addProductOrThrow(std::make_unique<AliasProductResolver>(
      std::move(bd),
      dynamic_cast<ProducedProductResolver&>(*productResolvers_[index])));
}

void Principal::addParentProcessProduct(
    std::shared_ptr<BranchDescription const> bd) {
  addProductOrThrow(
      std::make_unique<ParentProcessProductResolver>(std::move(bd)));
}
...
void Principal::setAtEndTransition(bool iAtEnd) { atEndTransition_ = iAtEnd; }
...
void Principal::fillPrincipal(
  ...
  if (historyAppender_ && productRegistry().anyProductProduced()) {
    ...
    if (hist.isValid()) {
      ...
      inputProcessHistory = std::shared_ptr<ProcessHistory const>(
          processHistoryRegistry.getMapped(hist), noDel);
      ...
      inputProcessHistory = std::shared_ptr<ProcessHistory const>(
          &s_emptyProcessHistory, [](void const*) {});
    }
  ...
  if (orderProcessHistoryID_ != processHistoryID_) {
    ...
    // We start with the current process even if it is not in the
    // ProcessHistory.
    ...
    for (auto iEnd = processHistoryPtr_->rend(); iter != iEnd; ++iter) {
      auto nameIter = std::find(lookupProcessNames.begin(),
                                lookupProcessNames.end(), iter->processName());
      if (nameIter == lookupProcessNames.end()) {
        continue;
      }
      lookupProcessOrder_.at(k) = nameIter - lookupProcessNames.begin();
...
ProductResolverBase* Principal::getExistingProduct(BranchID const& branchID) {
  return const_cast<ProductResolverBase*>(
      const_cast<const Principal*>(this)->getExistingProduct(branchID));
}

ProductResolverBase const* Principal::getExistingProduct(
...
ProductResolverBase const* Principal::getExistingProduct(
    ProductResolverBase const& productResolver) const {
  auto phb = getExistingProduct(productResolver.branchDescription().branchID());
  if (nullptr != phb &&
      BranchKey(productResolver.branchDescription()) !=
          BranchKey(phb->branchDescription())) {
    ...
    if (newProduct.branchName() != existing.branchName() &&
        newProduct.branchID() == existing.branchID()) {
      throw cms::Exception("HashCollision")
          << "Principal::getExistingProduct\n"
          << " Branch " << newProduct.branchName()
          << " has same branch ID as branch " << existing.branchName() << "\n"
          << "Workaround: change process name or product instance name of "
...
void Principal::addProduct_(
    std::unique_ptr<ProductResolverBase> productResolver) {
...
void Principal::addProductOrThrow(
    std::unique_ptr<ProductResolverBase> productResolver) {
  ...
  if (phb != nullptr) {
    ...
    throw Exception(errors::InsertFailure, "AlreadyPresent")
        ...
        << "product already exists for (" << bd.friendlyClassName() << ","
        << bd.moduleLabel() << "," << bd.productInstanceName() << ","
        << bd.processName() << ")\n";
  }
  addProduct_(std::move(productResolver));
}

Principal::ConstProductResolverPtr Principal::getProductResolver(
    BranchID const& bid) const {
...
Principal::ConstProductResolverPtr Principal::getProductResolverByIndex(
    ProductResolverIndex const& index) const {
  ConstProductResolverPtr const phb = productResolvers_[index].get();
  return phb;
}

BasicHandle Principal::getByLabel(KindOfType kindOfType, TypeID const& typeID,
                                  InputTag const& inputTag,
                                  EDConsumerBase const* consumer,
                                  SharedResourcesAcquirer* sra,
                                  ModuleCallingContext const* mcc) const {
  ProductData const* result =
      findProductByLabel(kindOfType, typeID, inputTag, consumer, sra, mcc);
  if (result == 0) {
    return BasicHandle(
        makeHandleExceptionFactory([=]() -> std::shared_ptr<cms::Exception> {
          return makeNotFoundException(
              "getByLabel", kindOfType, typeID, inputTag.label(),
              inputTag.instance(),
              appendCurrentProcessIfAlias(
                  inputTag.process(), processConfiguration_->processName()));
        }));
  }
...
BasicHandle Principal::getByLabel(KindOfType kindOfType, TypeID const& typeID,
                                  ...
                                  SharedResourcesAcquirer* sra,
                                  ModuleCallingContext const* mcc) const {
  ProductData const* result = findProductByLabel(
      kindOfType, typeID, label, instance, process, consumer, sra, mcc);
  if (result == 0) {
    return BasicHandle(
        makeHandleExceptionFactory([=]() -> std::shared_ptr<cms::Exception> {
          return makeNotFoundException("getByLabel", kindOfType, typeID, label,
                                       instance, process);
        }));
  }
  return BasicHandle(result->wrapper(), &(result->provenance()));
...
BasicHandle Principal::getByToken(KindOfType, TypeID const&,
                                  ProductResolverIndex index,
                                  bool skipCurrentProcess, bool& ambiguous,
                                  SharedResourcesAcquirer* sra,
                                  ModuleCallingContext const* mcc) const {
  assert(index != ProductResolverIndexInvalid);
  ...
  auto resolution =
      productResolver->resolveProduct(*this, skipCurrentProcess, sra, mcc);
...
void Principal::prefetchAsync(WaitingTask* task, ProductResolverIndex index,
...
void Principal::getManyByType(TypeID const& typeID, BasicHandleVec& results,
                              EDConsumerBase const* consumer,
                              SharedResourcesAcquirer* sra,
                              ModuleCallingContext const* mcc) const {
  assert(results.empty());

  if (unlikely(consumer and
               (not consumer->registeredToConsumeMany(typeID, branchType())))) {
  ...
  // Note that the function isFullyResolved returns true for the
  // ProductResolvers
  ...
  for (unsigned int i = 0; i < matches.numberOfMatches(); ++i) {
    ...
    if (!matches.isFullyResolved(i)) {
      ...
      ProductResolverBase const* productResolver =
          productResolvers_.at(index).get();
...
void Principal::findProducts(
    std::vector<ProductResolverBase const*> const& holders, TypeID const&,
    BasicHandleVec& results, SharedResourcesAcquirer* sra,
    ModuleCallingContext const* mcc) const {
    ...
    for (auto productResolver : holders) {
      ...
      if (process == bd.processName()) {
        ...
        ProductData const* productData =
            productResolver->resolveProduct(*this, false, sra, mcc).data();
        if (productData) {
          // Skip product if not available.
          results.emplace_back(productData->wrapper(),
...
ProductData const* Principal::findProductByLabel(
    KindOfType kindOfType, TypeID const& typeID, InputTag const& inputTag,
    EDConsumerBase const* consumer, SharedResourcesAcquirer* sra,
    ModuleCallingContext const* mcc) const {
  bool skipCurrentProcess = inputTag.willSkipCurrentProcess();

  ProductResolverIndex index =
      inputTag.indexFor(typeID, branchType(), &productRegistry());
  ...
  if (index == ProductResolverIndexInvalid) {
    ...
    index = productLookup().index(kindOfType, typeID, inputTag.label().c_str(),
                                  inputTag.instance().c_str(), processName);

    if (index == ProductResolverIndexAmbiguous) {
      throwAmbiguousException(
          "findProductByLabel", typeID, inputTag.label(), inputTag.instance(),
          appendCurrentProcessIfAlias(inputTag.process(),
                                      processConfiguration_->processName()));
  ...
  if (unlikely(consumer and (not consumer->registeredToConsume(
                                index, skipCurrentProcess, branchType())))) {
    failedToRegisterConsumes(
        kindOfType, typeID, inputTag.label(), inputTag.instance(),
        appendCurrentProcessIfAlias(inputTag.process(),
                                    processConfiguration_->processName()));
  }
...
ProductData const* Principal::findProductByLabel(
    KindOfType kindOfType, TypeID const& typeID, std::string const& label,
    std::string const& instance, std::string const& process,
    EDConsumerBase const* consumer, SharedResourcesAcquirer* sra,
    ModuleCallingContext const* mcc) const {
  ProductResolverIndex index = productLookup().index(
      kindOfType, typeID, label.c_str(), instance.c_str(), process.c_str());

  if (index == ProductResolverIndexAmbiguous) {
    throwAmbiguousException("findProductByLabel", typeID, label, instance,
                            process);
  } else if (index == ProductResolverIndexInvalid) {
    return 0;
  }
...
ProductData const* Principal::findProductByTag(
    TypeID const& typeID, InputTag const& tag,
    ModuleCallingContext const* mcc) const {
  ProductData const* productData =
      findProductByLabel(PRODUCT_TYPE, typeID, tag, nullptr, nullptr, mcc);
  return productData;
}

Provenance Principal::getProvenance(BranchID const& bid,
                                    ModuleCallingContext const* mcc) const {
  ConstProductResolverPtr const phb = getProductResolver(bid);
  if (phb == nullptr) {
    throwProductNotFoundException("getProvenance", errors::ProductNotFound,
                                  bid);
  }

  if (phb->unscheduledWasNotRun()) {
    if (not phb->resolveProduct(*this, false, nullptr, mcc).data()) {
      throwProductNotFoundException("getProvenance(onDemand)",
                                    errors::ProductNotFound, bid);
    }
  }
...
void Principal::getAllProvenance(
    std::vector<Provenance const*>& provenances) const {
  provenances.clear();
  for (auto const& productResolver : *this) {
    if (productResolver->singleProduct() &&
        productResolver->provenanceAvailable() &&
        !productResolver->branchDescription().isAlias()) {
...
void Principal::getAllStableProvenance(
...
void Principal::recombine(Principal& other, std::vector<BranchID> const& bids) {
  for (auto& prod : bids) {
    ...
    get_underlying_safe(productResolvers_[index])
        .swap(get_underlying_safe(other.productResolvers_[indexO]));
  }
...
WrapperBase const* Principal::getThinnedProduct(ProductID const&,
                                                unsigned int&) const {
  assert(false);
  return nullptr;
}
...
void Principal::putOrMerge(std::unique_ptr<WrapperBase> prod,
                           ProductResolverBase const* phb) const {
  phb->putOrMergeProduct(std::move(prod));
}

void Principal::putOrMerge(BranchDescription const& bd,
    ...
    for (auto const& prod : preg_->productList()) {
      ...
      if (bd.branchType() == branchType_) {
        ...
        if (!productResolvers_[index]) {
          // no product holder.  Must add one. The new entry must be an input
          // product holder.
          assert(!bd.produced());
          auto cbd = std::make_shared<BranchDescription const>(bd);
          addInputProduct(cbd);
...
void Principal::readAllFromSourceAndMergeImmediately() {
  if (not reader()) {
    return;
  }
...
void Principal::resetFailedFromThisProcess() {
  for (auto& prod : *this) {
