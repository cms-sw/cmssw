...
std::unique_ptr<EventSetupProvider> makeEventSetupProvider(
  ...
  for (std::vector<std::string>::iterator itName = prefers.begin(),
                                          itNameEnd = prefers.end();
    ...
    for (std::vector<std::string>::iterator itRecordName = recordNames.begin(),
      ...
      try {
        ...
        for (std::vector<std::string>::iterator itDatum = dataInfo.begin(),
          ...
          recordToData.insert(
              std::make_pair(std::string(*itRecordName),
                             std::make_pair(datumName, labelName)));
        }
    ...
    preferInfo[ComponentDescription(
        preferPSet.getParameter<std::string>("@module_type"),
        preferPSet.getParameter<std::string>("@module_label"), false)] =
        recordToData;
  }
...
void fillEventSetupProvider(EventSetupsController& esController,
  ...
  for (std::vector<std::string>::iterator itName = providers.begin(),
                                          itNameEnd = providers.end();
       itName != itNameEnd; ++itName) {
  ...
  for (std::vector<std::string>::iterator itName = sources.begin(),
                                          itNameEnd = sources.end();
       itName != itNameEnd; ++itName) {
    ParameterSet* providerPSet = params.getPSetForUpdate(*itName);
    validateEventSetupParameters(*providerPSet);
    providerPSet->registerIt();
...
void validateEventSetupParameters(ParameterSet& pset) {
  ...
  try {
    ...
    ost << "Validating configuration of ESProducer or ESSource of type "
        << modtype << " with label: '" << moduleLabel << "'";
