...
void ModuleMakerTraits::replaceExisting(
...
ModuleMakerTraits::getComponentAndRegisterProcess(
    EventSetupsController& esController, ParameterSet const& iConfiguration) {
  return esController.getESProducerAndRegisterProcess(
      iConfiguration, esController.indexOfNextProcess());
}

void ModuleMakerTraits::putComponent(
    EventSetupsController& esController, ParameterSet const& iConfiguration,
    std::shared_ptr<base_type> const& component) {
  esController.putESProducer(iConfiguration, component,
