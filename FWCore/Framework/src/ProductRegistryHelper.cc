...
void ProductRegistryHelper::addToRegistry(
    TypeLabelList::const_iterator const& iBegin,
  ...
  for (TypeLabelList::const_iterator p = iBegin; p != iEnd; ++p) {
    ...
    BranchDescription pdesc(
        convertToBranchType(p->transition_), iDesc.moduleLabel(),
        iDesc.processName(), p->typeID_.userClassName(),
        p->typeID_.friendlyClassName(), p->productInstanceName_,
        iDesc.moduleName(), iDesc.parameterSetID(), type, true,
        isEndTransition(p->transition_));

    if (pdesc.transient()) {
      if (!checkDictionary(missingDictionaries, pdesc.wrappedName(),
                           pdesc.wrappedType())) {
  ...
  if (!missingDictionaries.empty()) {
    std::string context(
        "Calling ProductRegistryHelper::addToRegistry, checking dictionaries "
        "for produced types");
