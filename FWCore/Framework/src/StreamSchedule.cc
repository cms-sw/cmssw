...
void transform_into(InputIterator begin, InputIterator end, ForwardIterator out,
                    Func func) {
...
void initializeBranchToReadingWorker(
  ...
  if (not vBranchesToDeleteEarly.empty()) {
    ...
    if (temp.size() != vBranchesToDeleteEarly.size()) {
      ...
      std::set_difference(
          temp.begin(), temp.end(), vBranchesToDeleteEarly.begin(),
          vBranchesToDeleteEarly.end(), std::back_inserter(missingProducts));
      LogInfo l("MissingProductsForCanDeleteEarly");
      l << "The following products in the 'canDeleteEarly' list are not "
           "available in this job and will be ignored.";
    ...
    for (auto const& branch : vBranchesToDeleteEarly) {
      branchToReadingWorker.insert(
          std::make_pair(branch, static_cast<Worker*>(nullptr)));
    }
  }
}
}
...
StreamSchedule::StreamSchedule(
    ...
    std::shared_ptr<ModuleRegistry> modReg, ParameterSet& proc_pset,
    service::TriggerNamesService const& tns,
    PreallocationConfiguration const& prealloc, ProductRegistry& preg,
    BranchIDListHelper& branchIDListHelper,
    ExceptionToActionTable const& actions,
    std::shared_ptr<ActivityRegistry> areg,
    std::shared_ptr<ProcessConfiguration> processConfiguration,
    bool allowEarlyDelete, StreamID streamID,
  ...
  ParameterSet const& opts =
      proc_pset.getUntrackedParameterSet("options", ParameterSet());
  bool hasPath = false;
  ...
  for (auto const& trig_name : pathNames) {
    fillTrigPath(proc_pset, preg, &prealloc, processConfiguration, trig_bitpos,
                 trig_name, results(), endPathNames);
    ++trig_bitpos;
    hasPath = true;
  }
  ...
  for (auto const& end_path_name : endPathNames) {
    fillEndPath(proc_pset, preg, &prealloc, processConfiguration, bitpos,
                end_path_name, endPathNames);
    ++bitpos;
  }
  ...
  if (!unusedLabels.empty()) {
    ...
    for (auto const& label : unusedLabels) {
      ...
      workerManager_.addToUnscheduledWorkers(
          *modulePSet, preg, &prealloc, processConfiguration, label,
    ...
    if (!shouldBeUsedLabels.empty()) {
      ...
      for (std::vector<std::string>::iterator
               itLabel = shouldBeUsedLabels.begin() + 1,
...
void StreamSchedule::initializeEarlyDelete(ModuleRegistry& modReg,
  ...
  // In the future we should use the SubProcess's 'keep list' to decide what can
  // be kept
  if (not allowEarlyDelete) return;

  // see if 'canDeleteEarly' was set and if so setup the list with those
  // products actually
  ...
  modReg.forAllModuleHolders(
      [this, &branchToReadingWorker,
       &nUniqueBranchesToDelete](maker::ModuleHolder* iHolder) {
  ...
  for (auto w : allWorkers()) {
    ...
    auto pset = pset::Registry::instance()->getMapped(
        w->description().parameterSetID());
    if (0 != pset) {
      auto branches = pset->getUntrackedParameter<std::vector<std::string>>(
          "mightGet", kEmpty);
  ...
  }
  {
    ...
    if (not unusedBranches.empty()) {
      ...
      l << "The following products in the 'canDeleteEarly' list are not used "
           "in this job and will be ignored.\n"
           " If possible, remove the producer from the job or add the product "
           "to the producer's own 'mightGet' list.";
  ...
  if (0 != branchToReadingWorker.size()) {
    ...
    for (auto& branchAndWorker : branchToReadingWorker) {
      if (lastBranchName != branchAndWorker.first) {
        // have to put back the period we removed earlier in order to get the
        // proper name
      ...
      if (alreadySeenWorkers.end() == found) {
        // NOTE: we will set aside enough space in
        // earlyDeleteHelperToBranchIndicies_ to accommodate
        // all the branches that might be read by this worker. However,
        // initially we will only tell the
        // EarlyDeleteHelper about the first one. As additional branches are
        // added via 'appendIndex' the
        // EarlyDeleteHelper will automatically advance its internal end
        // pointer.
        size_t index = nextOpenIndex;
        size_t nIndices = reserveSizeForWorker[branchAndWorker.second];
        earlyDeleteHelperToBranchIndicies_[index] =
            earlyDeleteBranchToCount_.size() - 1;
        earlyDeleteHelpers_.emplace_back(beginAddress + index,
                                         beginAddress + index + 1,
                                         &earlyDeleteBranchToCount_);
        branchAndWorker.second->setEarlyDeleteHelper(
            &(earlyDeleteHelpers_.back()));
        alreadySeenWorkers.insert(std::make_pair(
            branchAndWorker.second, &(earlyDeleteHelpers_.back())));
    ...
    // Now we can compactify the earlyDeleteHelperToBranchIndicies_ since we may
    // have over estimated the
    // space needed for each module
    auto itLast = earlyDeleteHelpers_.begin();
    for (auto it = earlyDeleteHelpers_.begin() + 1;
         it != earlyDeleteHelpers_.end(); ++it) {
      if (itLast->end() != it->begin()) {
        // figure the offset for next Worker since it hasn't been moved yet so
        // it has the original address
...
void StreamSchedule::fillWorkers(
    ParameterSet& proc_pset, ProductRegistry& preg,
    PreallocationConfiguration const* prealloc,
    std::shared_ptr<ProcessConfiguration const> processConfiguration,
    std::string const& pathName, bool ignoreFilters, PathWorkers& out,
  ...
  for (auto const& name : modnames) {
    ...
    if (modpset == 0) {
      ...
      throw Exception(errors::Configuration)
          << "The unknown module label \"" << moduleLabel << "\" appears in "
          << pathType << " \"" << pathName
          << "\"\n please check spelling or remove that label from the path.";
    }
    assert(isTracked);

    Worker* worker = workerManager_.getWorker(
        *modpset, preg, prealloc, processConfiguration, moduleLabel);
    if (ignoreFilters && filterAction != WorkerInPath::Ignore &&
        worker->moduleType() == Worker::kFilter) {
      // We have a filter on an end path, and the filter is not explicitly
      // ignored.
      // See if the filter is allowed.
      std::vector<std::string> allowed_filters =
          proc_pset.getUntrackedParameter<vstring>("@filters_on_endpaths");
      if (!search_all(allowed_filters, worker->description().moduleName())) {
        ...
        LogWarning("FilterOnEndPath")
            << "The EDFilter '" << worker->description().moduleName()
            << "' with module label '" << moduleLabel
            << "' appears on EndPath '" << pathName << "'.\n"
            << "The return value of the filter will be ignored.\n"
            << "To suppress this warning, either remove the filter from the "
               "endpath,\n"
            << "or explicitly ignore it in the configuration by using "
               "cms.ignore().\n";
      }
    }
...
void StreamSchedule::fillTrigPath(
    ParameterSet& proc_pset, ProductRegistry& preg,
    PreallocationConfiguration const* prealloc,
    std::shared_ptr<ProcessConfiguration const> processConfiguration,
    ...
    std::vector<std::string> const& endPathNames) {
  PathWorkers tmpworkers;
  fillWorkers(proc_pset, preg, prealloc, processConfiguration, name, false,
              tmpworkers, endPathNames);

  // an empty path will cause an extra bit that is not used
  if (!tmpworkers.empty()) {
    trig_paths_.emplace_back(bitpos, name, tmpworkers, trptr, actionTable(),
                             actReg_, &streamContext_, &skippingEvent_,
...
void StreamSchedule::fillEndPath(
    ParameterSet& proc_pset, ProductRegistry& preg,
    PreallocationConfiguration const* prealloc,
    std::shared_ptr<ProcessConfiguration const> processConfiguration,
    ...
    std::vector<std::string> const& endPathNames) {
  PathWorkers tmpworkers;
  fillWorkers(proc_pset, preg, prealloc, processConfiguration, name, true,
              tmpworkers, endPathNames);

  if (!tmpworkers.empty()) {
    // EndPaths are not supposed to stop if SkipEvent type exception happens
    end_paths_.emplace_back(bitpos, name, tmpworkers, TrigResPtr(),
                            actionTable(), actReg_, &streamContext_, nullptr,
...
std::vector<ModuleDescription const*> StreamSchedule::getAllModuleDescriptions()
    const {
...
void StreamSchedule::processOneEventAsync(
    WaitingTaskHolder iTask, EventPrincipal& ep, EventSetup const& es,
    std::vector<edm::propagate_const<std::shared_ptr<PathStatusInserter>>>&
        pathStatusInserters) {
  this->resetAll();
  ...
  for (int empty_trig_path : empty_trig_paths_) {
    ...
    pathStatusInserters[empty_trig_path]->setPathStatus(streamID_,
                                                        hltPathStatus);
    std::exception_ptr iException =
        pathStatusInserterWorkers_[empty_trig_path]
            ->runModuleDirectly<
                ...
                ep, es, streamID_, ParentContext(&streamContext_),
                &streamContext_);
    if (iException) {
  ...
  auto pathsDone = make_waiting_task(
      tbb::task::allocate_root(), [iTask, &ep, &es, this, serviceToken](
  ...
  for (auto it = trig_paths_.rbegin(), itEnd = trig_paths_.rend(); it != itEnd;
       ++it) {
    it->processOneOccurrenceAsync(pathsDone, ep, es, streamID_,
                                  &streamContext_);
  }
}

void StreamSchedule::finishedPaths(std::exception_ptr iExcept,
                                   WaitingTaskHolder iWait, EventPrincipal& ep,
                                   EventSetup const& es) {
  if (iExcept) {
    try {
  ...
  if ((not iExcept) and (nullptr != results_inserter_.get())) {
    try {
      ...
      results_inserter_->doWork<Traits>(ep, es, streamID_, parentContext,
                                        &streamContext_);
  ...
  if (end_paths_.empty() or iExcept or (not endpathsAreActive_)) {
    ...
    auto endPathsDone = make_waiting_task(
    ...
    for (auto it = end_paths_.rbegin(), itEnd = end_paths_.rend(); it != itEnd;
         ++it) {
      it->processOneOccurrenceAsync(endPathsDone, ep, es, streamID_,
                                    &streamContext_);
    }
  }
}

std::exception_ptr StreamSchedule::finishProcessOneEvent(
  ...
  if (iExcept) {
    ...
    try {
      ...
      if (ex.context().empty()) {
        addContextAndPrintException(
            "Calling function StreamSchedule::processOneEvent", ex,
    ...
    actReg_->preStreamEarlyTerminationSignal_(
        streamContext_, TerminationOrigin::ExceptionFromThisContext);
  }
...
void StreamSchedule::availablePaths(
    std::vector<std::string>& oLabelsToFill) const {
  oLabelsToFill.reserve(trig_paths_.size());
  std::transform(trig_paths_.begin(), trig_paths_.end(),
                 std::back_inserter(oLabelsToFill),
                 std::bind(&Path::name, std::placeholders::_1));
}

void StreamSchedule::modulesInPath(
    std::string const& iPathLabel,
    std::vector<std::string>& oLabelsToFill) const {
  TrigPaths::const_iterator itFound =
      std::find_if(trig_paths_.begin(), trig_paths_.end(),
                   std::bind(std::equal_to<std::string>(), iPathLabel,
                             std::bind(&Path::name, std::placeholders::_1)));
  if (itFound != trig_paths_.end()) {
    oLabelsToFill.reserve(itFound->size());
    for (size_t i = 0; i < itFound->size(); ++i) {
      oLabelsToFill.push_back(
          itFound->getWorker(i)->description().moduleLabel());
    }
  }
}

void StreamSchedule::moduleDescriptionsInPath(
    std::string const& iPathLabel,
...
void StreamSchedule::moduleDescriptionsInEndPath(
  ...
  if (!found) {
    // if the hint did not work, do it the slow way
    itFound =
        std::find_if(end_paths_.begin(), end_paths_.end(),
                     std::bind(std::equal_to<std::string>(), iEndPathLabel,
                               std::bind(&Path::name, std::placeholders::_1)));
...
void StreamSchedule::enableEndPaths(bool active) {
  endpathsAreActive_ = active;
}

bool StreamSchedule::endPathsEnabled() const { return endpathsAreActive_; }

static void fillModuleInPathSummary(Path const& path, size_t which,
...
void StreamSchedule::addToAllWorkers(Worker* w) {
...
void StreamSchedule::makePathStatusInserters(
  ...
  for (auto& pathStatusInserter : pathStatusInserters) {
    ...
    if (indexEmpty < empty_trig_paths_.size() &&
        bitpos == empty_trig_paths_.at(indexEmpty)) {
      ++indexEmpty;
    } else {
      trig_paths_.at(indexOfPath)
          .setPathStatusInserter(inserterPtr.get(), workerPtr.get());
      ++indexOfPath;
    }
    ++bitpos;
  }
  ...
  for (auto& endPathStatusInserter : endPathStatusInserters) {
    ...
    // instantiated and put into end_paths if there are no modules
    // on the configured path.
    if (indexEmpty < empty_end_paths_.size() &&
        bitpos == empty_end_paths_.at(indexEmpty)) {
      ++indexEmpty;
    } else {
      end_paths_.at(indexOfPath)
          .setPathStatusInserter(nullptr, workerPtr.get());
      ++indexOfPath;
    }
    ++bitpos;
  }
