...
Worker* WorkerRegistry::getWorker(WorkerParams const& p,
  ...
  if (workerIt == m_workerMap.end()) {
    MakeModuleParams mmp(p.pset_, *p.reg_, p.preallocate_,
                         p.processConfiguration_);
    auto modulePtr = modRegistry_->getModule(
        mmp, moduleLabel, actReg_->preModuleConstructionSignal_,
    ...
    m_workerMap[moduleLabel] = std::shared_ptr<Worker>(
        workerPtr.release());  // propagate_const<T> has no reset() function
