...
OccurrenceForOutput::OccurrenceForOutput(
    Principal const& p, ModuleDescription const& md,
    ModuleCallingContext const* moduleCallingContext)
    : provRecorder_(p, md), moduleCallingContext_(moduleCallingContext) {}
...
void OccurrenceForOutput::getAllProvenance(
    std::vector<Provenance const*>& provenances) const {
  provRecorder_.principal().getAllProvenance(provenances);
}

void OccurrenceForOutput::getAllStableProvenance(
...
size_t OccurrenceForOutput::size() const {
  return provRecorder_.principal().size();
}

bool OccurrenceForOutput::getByToken(EDGetToken token, TypeID const& typeID,
                                     BasicHandle& result) const {
  result.clear();
  result = provRecorder_.getByToken_(typeID, PRODUCT_TYPE, token,
                                     moduleCallingContext_);
  if (result.failedToGet()) {
    return false;
  }
  if (!provRecorder_.isComplete() && result.wrapper()->isMergeable()) {
    principal_get_adapter_detail::throwOnPrematureRead("RunOrLumi", typeID,
                                                       token);
  }
  return true;
}
