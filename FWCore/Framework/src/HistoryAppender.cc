...
std::shared_ptr<ProcessHistory const> HistoryAppender::appendToProcessHistory(
  ...
  assert((iInputProcessHistory) == nullptr or
         (inputPHID == iInputProcessHistory->id()));
  ...
  ProcessHistory const* inputProcessHistory =
      iInputProcessHistory ? iInputProcessHistory : &s_emptyHistory;
...
void HistoryAppender::checkProcessHistory(
    ProcessHistory const& ph, ProcessConfiguration const& pc) const {
  ...
  for (auto const& process : ph) {
    if (processName == process.processName()) {
      throw edm::Exception(errors::Configuration, "Duplicate Process.")
          ...
          << " was already in the ProcessHistory.\n"
          << "Please modify the configuration file to use a distinct process "
             "name.\n";
    }
  }
}
