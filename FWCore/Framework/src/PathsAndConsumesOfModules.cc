...
void PathsAndConsumesOfModules::initialize(
    Schedule const* schedule, std::shared_ptr<ProductRegistry const> preg) {
  schedule_ = schedule;
  preg_ = preg;
  ...
  for (auto const& endpath : endPaths_) {
    schedule->moduleDescriptionsInEndPath(endpath, modulesOnEndPaths_.at(i),
                                          hint);
    if (!modulesOnEndPaths_.at(i).empty()) ++hint;
    ++i;
  }

  schedule->fillModuleAndConsumesInfo(allModuleDescriptions_, moduleIDToIndex_,
                                      modulesWhoseProductsAreConsumedBy_,
                                      *preg);
}

ModuleDescription const* PathsAndConsumesOfModules::doModuleDescription(
  ...
  std::vector<std::pair<unsigned int, unsigned int>>::const_iterator iter =
      std::lower_bound(moduleIDToIndex_.begin(), moduleIDToIndex_.end(),
                       target);
...
std::vector<ModuleDescription const*> const&
PathsAndConsumesOfModules::doModulesWhoseProductsAreConsumedBy(
    unsigned int moduleID) const {
  return modulesWhoseProductsAreConsumedBy_.at(moduleIndex(moduleID));
}

std::vector<ConsumesInfo> PathsAndConsumesOfModules::doConsumesInfo(
    unsigned int moduleID) const {
  Worker const* worker = schedule_->allWorkers().at(moduleIndex(moduleID));
  return worker->consumesInfo();
}

unsigned int PathsAndConsumesOfModules::moduleIndex(
    unsigned int moduleID) const {
  unsigned int dummy = 0;
  auto target = std::make_pair(moduleID, dummy);
  std::vector<std::pair<unsigned int, unsigned int>>::const_iterator iter =
...
void checkForModuleDependencyCorrectness(
    edm::PathsAndConsumesOfModulesBase const& iPnC, bool iPrintDependencies) {
  ...
  for (auto const& description : iPnC.allModules()) {
    moduleIndexToNames.insert(
        std::make_pair(description->id(), description->moduleLabel()));
    ...
    if (description->moduleName() == kPathStatusInserter ||
      ...
      pathStatusInserterModuleLabelToModuleID[description->moduleLabel()] =
          description->id();
    }
  }
  ...
  /*
  {
    //We need to explicitly check that modules on Paths do not try to read data
  from
    // Modules which are only on EndPaths. The circular dependency finder has
  been
    // known to miss these.
    ...
    for( unsigned int pathIndex = 0; pathIndex != endPaths.size(); ++pathIndex)
  {
    ...
    for(auto moduleIndex : modulesOnPaths) {
      auto const& dependentModules =
  iPnC.modulesWhoseProductsAreConsumedBy(moduleIndex);
  ...
  "Unrunnable schedule\n")
            <<"The module "<<moduleIndexToNames[moduleIndex]<<" is on a Path and
  depends on data from module "
            <<moduleIndexToNames[depDescription->id()]<<" which is on an
  EndPath.";
        }
      }
    }
  ...
  moduleIndexToNames.insert(
      std::make_pair(kFinishedProcessingIndex, kFinishedProcessing));

  pathNames.insert(pathNames.end(), iPnC.endPaths().begin(),
                   iPnC.endPaths().end());
  std::vector<std::vector<unsigned int>> pathIndexToModuleIndexOrder(
      pathNames.size());
  {
    for (unsigned int pathIndex = 0; pathIndex != pathNames.size();
         ++pathIndex) {
      ...
      if (pathIndex < kFirstEndPathIndex) {
        ...
        moduleDescriptions =
            &(iPnC.modulesOnEndPath(pathIndex - kFirstEndPathIndex));
      }
      ...
      for (auto const& description : *moduleDescriptions) {
        ...
        if (found.second) {
          ...
          if (lastModuleIndex != kInvalidIndex) {
            edgeToPathMap[std::make_pair(moduleIndex, lastModuleIndex)]
                .push_back(pathIndex);
          }
      ...
      auto labelToID =
          pathStatusInserterModuleLabelToModuleID.find(pathNames[pathIndex]);
      if (labelToID == pathStatusInserterModuleLabelToModuleID.end()) {
        // should never happen
        throw Exception(errors::LogicError)
            << "PathsAndConsumesOfModules::moduleDescription:"
               "checkForModuleDependencyCorrectness Could not find "
               "PathStatusInserter\n";
      }
      unsigned int pathStatusInserterModuleID = labelToID->second;
      if (pathIndex < kFirstEndPathIndex) {
        if ((lastModuleIndex != kInvalidIndex)) {
          edgeToPathMap[std::make_pair(pathStatusInserterModuleID,
                                       lastModuleIndex)]
              .push_back(pathIndex);
          moduleIndexToNames.insert(
              std::make_pair(pathStatusInserterModuleID, kPathEnded));
          if (kTriggerResultsIndex != kInvalidIndex) {
            edgeToPathMap[std::make_pair(kTriggerResultsIndex,
                                         pathStatusInserterModuleID)]
                .push_back(kDataDependencyIndex);
          }
          // Need to make dependency for finished process
          edgeToPathMap[std::make_pair(kFinishedProcessingIndex,
                                       pathStatusInserterModuleID)]
              .push_back(kDataDependencyIndex);
        ...
        if ((not moduleDescriptions->empty())) {
          if (kTriggerResultsIndex != kInvalidIndex) {
            ...
            edgeToPathMap[std::make_pair(
                              moduleDescriptions->front()->id(),
                              kPathToTriggerResultsDependencyLastIndex)]
                .push_back(pathIndex);
            moduleIndexToNames.insert(std::make_pair(
                kPathToTriggerResultsDependencyLastIndex, kEndPathStart));
            edgeToPathMap[std::make_pair(
                              kPathToTriggerResultsDependencyLastIndex,
                              kTriggerResultsIndex)]
                .push_back(kDataDependencyIndex);
            pathOrder.insert(pathOrder.begin(),
                             kPathToTriggerResultsDependencyLastIndex);
          }
          // Need to make dependency for finished process
          ++kPathToTriggerResultsDependencyLastIndex;
      ...
      for (auto const& depDescription : dependentModules) {
        if (iPrintDependencies) {
          edm::LogAbsolute("ModuleDependency")
              << "ModuleDependency '" << description->moduleLabel()
        ...
        if (itPathsFound != moduleIndexToPathIndex.end() and
            itDepsPathsFound != moduleIndexToPathIndex.end()) {
        ...
        if (keepDataDependency) {
          edgeToPathMap[std::make_pair(moduleIndex, depID)].push_back(
              kDataDependencyIndex);
        }
      }
    }
  }
  graph::throwIfImproperDependencies(edgeToPathMap, pathIndexToModuleIndexOrder,
                                     pathNames, moduleIndexToNames);
}
}
