...
ProducerBase::ProducerBase()
    : ProductRegistryHelper(), callWhenNewProductsRegistered_() {}
ProducerBase::~ProducerBase() noexcept(false) {}

std::function<void(BranchDescription const&)>
...
void ProducerBase::registerProducts(ProducerBase* producer,
  ...
  // If we have a callback, first tell the callback about all the entries
  // already in the
  // product registry, then add any items this producer wants to add to the
  // registry
  // and only after that do we register the callback. This is done so the
  // callback does not
  // get called for items registered by this producer (avoids circular reference
  // problems)
  ...
  ProductRegistryHelper::addToRegistry(plist.begin(), plist.end(), md, *(iReg),
                                       isListener);
  if (registrationCallback()) {
