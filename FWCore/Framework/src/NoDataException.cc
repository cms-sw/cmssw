...
std::string NoDataExceptionBase::providerButNoDataMessage(
    const EventSetupRecordKey& iKey) {
  return std::string(
             " A provider for this data exists, but it's unable to deliver the "
             "data for this \"") +
         iKey.name() +
         "\" record.\n Perhaps no valid data exists for this IOV? Please check "
         "the data's interval of validity.\n";
}

std::string NoDataExceptionBase::noProxyMessage() {
  return std::string(
      "Please add an ESSource or ESProducer to your job which can deliver this "
      "data.\n");
}
...
void NoDataExceptionBase::constructMessage(const char* iClassName,
