...
namespace edm {
  ...
  template <typename T>
  void OutputModuleCommunicatorT<T>::writeRun(edm::RunPrincipal const& rp, ProcessContext const* processContext) {
  ...
  template <typename T>
  void OutputModuleCommunicatorT<T>::writeLumi(edm::LuminosityBlockPrincipal const& lbp,
