...
InputSource::InputSource(ParameterSet const& pset,
  ...
  // is not used when the ParameterSet has been validated and the parameters are
  // not optional
  // in the description.  As soon as all primary input sources and all modules
  // with a secondary
  ...
  if (processingMode == runMode) {
    ...
    throw Exception(errors::Configuration)
        ...
        << processingMode << "'\n"
        << "Legal values are '" << defaultMode << "', '" << runLumiMode
        << "', or '" << runMode << "'.\n";
  }
}
...
// This next function is to guarantee that "runs only" mode does not return
// events or lumis,
// and that "runs and lumis only" mode does not return events.
// For input sources that are not random access (e.g. you need to read through
// the events
// to get to the lumis and runs), this is all that is involved to implement
// these modes.
// For input sources where events or lumis can be skipped, getNextItemType()
// should
// implement the skipping internally, so that the performance gain is realized.
// If this is done for a source, the 'if' blocks in this function will never be
// entered
// for that source.
InputSource::ItemType InputSource::nextItemType_() {
  ItemType itemType = callWithTryCatchAndPrint<ItemType>(
      [this]() { return getNextItemType(); },
...
void InputSource::registerProducts() {
  if (!typeLabelList().empty()) {
    addToRegistry(typeLabelList().begin(), typeLabelList().end(),
                  moduleDescription(), productRegistryUpdate());
  }
}
...
std::unique_ptr<FileBlock> InputSource::readFile() {
  ...
  assert(!limitReached());
  return callWithTryCatchAndPrint<std::unique_ptr<FileBlock> >(
      [this]() { return readFile_(); }, "Calling InputSource::readFile_");
}
...
void InputSource::readRun(RunPrincipal& runPrincipal, HistoryAppender&) {
  ...
  callWithTryCatchAndPrint<void>(
      [this, &runPrincipal]() { readRun_(runPrincipal); },
...
void InputSource::readRun_(RunPrincipal& runPrincipal) {
  ...
  // the
  // random number generator if random numbers are generated during processing
  // of runs
  // (e.g. beginRun(), endRun())
  runPrincipal.fillRunPrincipal(processHistoryRegistry());
}

void InputSource::readLuminosityBlock_(
...
void InputSource::readEvent(EventPrincipal& ep, StreamContext& streamContext) {
  ...
  {
    // block scope, in order to issue the PostSourceEvent signal before calling
    // postRead and issueReports
...
bool InputSource::readEvent(EventPrincipal& ep, EventID const& eventID,
  ...
  if (not limitReached()) {
    // the Pre/PostSourceEvent signals should be generated only if the event is
    // actually found.
    // this should be taken care of by an EventSourceSentry in the implementaion
    // of readIt()

    // result = callWithTryCatchAndPrint<bool>( [this,&eventID,&ep](){ return
    // readIt(eventID, ep); }, "Calling InputSource::readIt" );
...
bool InputSource::goToEvent(EventID const& eventID) {
  return callWithTryCatchAndPrint<bool>(
      [this, &eventID]() { return goToEvent_(eventID); },
...
void InputSource::rewind() {
  ...
  setNewLumi();
  resetEventCached();
  callWithTryCatchAndPrint<void>([this]() { rewind_(); },
...
void InputSource::setLumi(LuminosityBlockNumber_t) {
  throw Exception(errors::LogicError)
      << "InputSource::setLumi()\n"
      << "Luminosity Block ID cannot be modified for this type of Input "
         "Source\n"
      << "Contact a Framework Developer\n";
}
...
void InputSource::doEndRun(RunPrincipal& rp, bool cleaningUpAfterException,
  ...
  Run run(rp, moduleDescription(), nullptr);
  callWithTryCatchAndPrint<void>([this, &run]() { endRun(run); },
                                 "Calling InputSource::endRun",
                                 cleaningUpAfterException);
  run.commit_(std::vector<edm::ProductResolverIndex>());
}
...
ProcessHistoryID const& InputSource::reducedProcessHistoryID() const {
  assert(runAuxiliary());
  return processHistoryRegistry_->reducedProcessHistoryID(
...
InputSource::LumiSourceSentry::LumiSourceSentry(InputSource const& source)
    : sentry_(source.actReg()->preSourceLumiSignal_,
              source.actReg()->postSourceLumiSignal_) {}

InputSource::RunSourceSentry::RunSourceSentry(InputSource const& source)
    : sentry_(source.actReg()->preSourceRunSignal_,
              source.actReg()->postSourceRunSignal_) {}
