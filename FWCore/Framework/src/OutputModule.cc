...
void OutputModule::keepThisBranch(
  ...
  if (!checkDictionary(missingDictionaries, desc.className(),
                       desc.unwrappedType())) {
    std::string context(
        "Calling OutputModule::keepThisBranch, checking dictionaries for kept "
        "types");
...
Trig OutputModule::getTriggerResults(EDGetTokenT<TriggerResults> const& token,
                                     EventForOutput const& e) const {
  // This cast is safe since we only call const functions of the
  // EventForOutputafter this point
  Trig result;
  e.getByToken<TriggerResults>(token, result);
  return result;
}

bool OutputModule::prePrefetchSelection(StreamID id, EventPrincipal const& ep,
...
bool OutputModule::doEvent(EventPrincipal const& ep, EventSetup const&,
...
bool OutputModule::doBeginRun(RunPrincipal const& rp, EventSetup const&,
                              ModuleCallingContext const* mcc) {
...
bool OutputModule::doEndRun(RunPrincipal const& rp, EventSetup const&,
                            ModuleCallingContext const* mcc) {
...
ThinnedAssociationsHelper const* OutputModule::thinnedAssociationsHelper()
    const {
  return thinnedAssociationsHelper_.get();
}

ModuleDescription const& OutputModule::description() const {
