...
class Worker {
 public:
  ...
  bool doWork(typename T::MyPrincipal const&, EventSetup const& c,
              StreamID stream, ParentContext const& parentContext,
              typename T::Context const* context);
  template <typename T>
  void doWorkAsync(WaitingTask* task, typename T::MyPrincipal const&,
                   EventSetup const& c, StreamID stream,
  ...
  void registerThinnedAssociations(ProductRegistry const& registry,
  ...
  int timesPass() const {
    return timesPassed();
  }  // for backward compatibility only - to be removed soon
 ...
 protected:
  ...
  virtual bool implDoPrePrefetchSelection(StreamID id, EventPrincipal const& ep,
                                          ModuleCallingContext const* mcc) = 0;
  virtual bool implDoBegin(RunPrincipal const& rp, EventSetup const& c,
                           ModuleCallingContext const* mcc) = 0;
  virtual bool implDoStreamBegin(StreamID id, RunPrincipal const& rp,
                                 EventSetup const& c,
                                 ModuleCallingContext const* mcc) = 0;
  virtual bool implDoStreamEnd(StreamID id, RunPrincipal const& rp,
                               EventSetup const& c,
                               ModuleCallingContext const* mcc) = 0;
  ...
  virtual bool implDoStreamBegin(StreamID id,
                                 ...
                                 EventSetup const& c,
                                 ModuleCallingContext const* mcc) = 0;
  virtual bool implDoStreamEnd(StreamID id, LuminosityBlockPrincipal const& lbp,
                               EventSetup const& c,
                               ModuleCallingContext const* mcc) = 0;
 ...
 private:
  ...
  virtual void itemsToGet(
      BranchType, std::vector<ProductResolverIndexAndSkipBit>&) const = 0;
  virtual void itemsMayGet(
      BranchType, std::vector<ProductResolverIndexAndSkipBit>&) const = 0;

  virtual std::vector<ProductResolverIndexAndSkipBit> const& itemsToGetFrom(
      BranchType) const = 0;

  virtual std::vector<ProductResolverIndex> const& itemsShouldPutInEvent()
      const = 0;

  virtual void preActionBeforeRunEventAsync(
      WaitingTask* iTask, ModuleCallingContext const& moduleCallingContext,
      Principal const& iPrincipal) const = 0;

  virtual void implRespondToOpenInputFile(FileBlock const& fb) = 0;
  virtual void implRespondToCloseInputFile(FileBlock const& fb) = 0;

  virtual void implRegisterThinnedAssociations(ProductRegistry const&,
                                               ThinnedAssociationsHelper&) = 0;
  ...
  bool shouldRethrowException(std::exception_ptr iPtr,
                              ParentContext const& parentContext, bool isEvent,
  ...
  std::exception_ptr setException(std::exception_ptr iException) {
    ...
    cached_exception_ =
        iException;  // propagate_const<T> has no reset() function
    state_ = Exception;
    return cached_exception_;
  }

  void prefetchAsync(WaitingTask*, ParentContext const& parentContext,
                     Principal const&);

  void emitPostModuleEventPrefetchingSignal() {
    actReg_->postModuleEventPrefetchingSignal_.emit(
        *moduleCallingContext_.getStreamContext(), moduleCallingContext_);
  }

  template <typename T>
  std::exception_ptr runModuleAfterAsyncPrefetch(
      std::exception_ptr const* iEPtr, typename T::MyPrincipal const& ep,
      EventSetup const& es, StreamID streamID,
      ParentContext const& parentContext, typename T::Context const* context);

  template <typename T>
  class RunModuleTask : public WaitingTask {
   public:
    RunModuleTask(Worker* worker, typename T::MyPrincipal const& ep,
                  EventSetup const& es, StreamID streamID,
                  ParentContext const& parentContext,
                  typename T::Context const* context)
        : m_worker(worker),
    ...
    tbb::task* execute() override {
      // Need to make the services available early so other services can see
      // them
      ...
      if (not excptr) {
        if (auto queue = m_worker->serializeRunModule()) {
          ...
          queue->push([worker, &principal, &es, streamID, parentContext,
                       sContext, serviceToken]() {
            // Need to make the services available
            ServiceRegistry::Operate guard(serviceToken);

            std::exception_ptr* ptr = nullptr;
            worker->runModuleAfterAsyncPrefetch<T>(ptr, principal, es, streamID,
      ...
      m_worker->runModuleAfterAsyncPrefetch<T>(
          excptr, m_principal, m_es, m_streamID, m_parentContext, m_context);
      return nullptr;
    }
  ...
  ExceptionToActionTable const*
      actions_;  // memory assumed to be managed elsewhere
  CMS_THREAD_GUARD(state_)
...
class CallImpl<OccurrenceTraits<EventPrincipal, BranchActionStreamBegin>> {
 public:
  ...
  static bool call(Worker* iWorker, StreamID, EventPrincipal const& ep,
                   EventSetup const& es, ActivityRegistry* /* actReg */,
                   ModuleCallingContext const* mcc,
...
class CallImpl<OccurrenceTraits<RunPrincipal, BranchActionGlobalBegin>> {
 public:
  ...
  static bool call(Worker* iWorker, StreamID, RunPrincipal const& ep,
                   EventSetup const& es, ActivityRegistry* actReg,
                   ModuleCallingContext const* mcc,
                   Arg::Context const* context) {
...
class CallImpl<OccurrenceTraits<RunPrincipal, BranchActionStreamBegin>> {
 public:
  ...
  static bool call(Worker* iWorker, StreamID id, RunPrincipal const& ep,
                   EventSetup const& es, ActivityRegistry* actReg,
                   ModuleCallingContext const* mcc,
                   Arg::Context const* context) {
    ModuleSignalSentry<Arg> cpp(actReg, context, mcc);
...
class CallImpl<
 ...
 public:
  ...
  static bool call(Worker* iWorker, StreamID,
                   ...
                   ActivityRegistry* actReg, ModuleCallingContext const* mcc,
                   Arg::Context const* context) {
    ModuleSignalSentry<Arg> cpp(actReg, context, mcc);
    return iWorker->implDoBegin(ep, es, mcc);
  }
...
template <typename T>
void Worker::doWorkAsync(WaitingTask* task, typename T::MyPrincipal const& ep,
                         EventSetup const& es, StreamID streamID,
                         ParentContext const& parentContext,
                         typename T::Context const* context) {
  ...
  if (workStarted_.compare_exchange_strong(expected, true)) {
    ...
    // if have TriggerResults based selection we want to reject the event before
    // doing prefetching
    try {
      if (not workerhelper::CallImpl<T>::prePrefetchSelection(
              this, streamID, ep, &moduleCallingContext_)) {
    ...
    auto runTask = new (tbb::task::allocate_root())
        RunModuleTask<T>(this, ep, es, streamID, parentContext, context);
    prefetchAsync(runTask, parentContext, ep);
  }
}

template <typename T>
std::exception_ptr Worker::runModuleAfterAsyncPrefetch(
    std::exception_ptr const* iEPtr, typename T::MyPrincipal const& ep,
    EventSetup const& es, StreamID streamID, ParentContext const& parentContext,
    typename T::Context const* context) {
...
void Worker::doWorkNoPrefetchingAsync(WaitingTask* task,
  ...
  if (workStarted_.compare_exchange_strong(expected, true)) {
    ...
    auto toDo = [this, &principal, &es, streamID, parentContext, context,
                 serviceToken]() {
      std::exception_ptr exceptionPtr;
      try {
        // Need to make the services available
...
template <typename T>
bool Worker::doWork(typename T::MyPrincipal const& ep, EventSetup const& es,
                    StreamID streamID, ParentContext const& parentContext,
                    typename T::Context const* context) {
  if (T::isEvent_) {
    timesVisited_.fetch_add(1, std::memory_order_relaxed);
  }
  ...
  auto resetContext = [](ModuleCallingContext* iContext) {
    iContext->setContext(ModuleCallingContext::State::kInvalid, ParentContext(),
                         nullptr);
  };
  std::unique_ptr<ModuleCallingContext, decltype(resetContext)> prefetchSentry(
      &moduleCallingContext_, resetContext);

  if (T::isEvent_) {
    try {
      // if have TriggerResults based selection we want to reject the event
      // before doing prefetching
      if (not workerhelper::CallImpl<T>::prePrefetchSelection(
              this, streamID, ep, &moduleCallingContext_)) {
    ...
    auto waitTask = edm::make_empty_waiting_task();
    {
      ...
      std::unique_ptr<ActivityRegistry, decltype(sentryFunc)> signalSentry(
    ...
    if (waitTask->exceptionPtr() != nullptr) {
      TransitionIDValue<typename T::MyPrincipal> idValue(ep);
      if (shouldRethrowException(*waitTask->exceptionPtr(), parentContext,
                                 T::isEvent_, idValue)) {
...
template <typename T>
bool Worker::runModule(typename T::MyPrincipal const& ep, EventSetup const& es,
                       StreamID streamID, ParentContext const& parentContext,
                       typename T::Context const* context) {
  ...
  try {
    convertException::wrap([&]() {
      rc =
          workerhelper::CallImpl<T>::call(this, streamID, ep, es, actReg_.get(),
                                          &moduleCallingContext_, context);
    ...
    TransitionIDValue<typename T::MyPrincipal> idValue(ep);
    if (shouldRethrowException(std::current_exception(), parentContext,
                               T::isEvent_, idValue)) {
...
template <typename T>
std::exception_ptr Worker::runModuleDirectly(
    typename T::MyPrincipal const& ep, EventSetup const& es, StreamID streamID,
    ParentContext const& parentContext, typename T::Context const* context) {
  timesVisited_.fetch_add(1, std::memory_order_relaxed);
  std::exception_ptr const* prefetchingException =
      nullptr;  // null because there was no prefetching to do
  return runModuleAfterAsyncPrefetch<T>(prefetchingException, ep, es, streamID,
