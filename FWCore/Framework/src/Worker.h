...
namespace edm {
  ...
  public:
    ...
    template <typename T>
    bool doWork(typename T::MyPrincipal const&, EventSetup const& c, StreamID stream,
    ...
    void prePrefetchSelectionAsync(WaitingTask* task, StreamID stream, void const*) { assert(false); }

    template <typename T>
    void doWorkAsync(WaitingTask* task, typename T::MyPrincipal const&, EventSetup const& c, StreamID stream,
                     ParentContext const& parentContext, typename T::Context const* context);

    template <typename T>
    void doWorkNoPrefetchingAsync(WaitingTask* task, typename T::MyPrincipal const&, EventSetup const& c,
                                  StreamID stream, ParentContext const& parentContext,
                                  typename T::Context const* context);

    template <typename T>
    std::exception_ptr runModuleDirectly(typename T::MyPrincipal const& ep, EventSetup const& es, StreamID streamID,
                                         ParentContext const& parentContext, typename T::Context const* context);
  ...
  private:
    template <typename T>
    bool runModule(typename T::MyPrincipal const&, EventSetup const& c, StreamID stream,
                   ParentContext const& parentContext, typename T::Context const* context);
    ...
    virtual void preActionBeforeRunEventAsync(WaitingTask* iTask, ModuleCallingContext const& moduleCallingContext,
    ...
    bool shouldRethrowException(std::exception_ptr iPtr, ParentContext const& parentContext, bool isEvent,
    ...
    template <typename T>
    std::exception_ptr runModuleAfterAsyncPrefetch(std::exception_ptr const* iEPtr, typename T::MyPrincipal const& ep,
    ...
    public:
      RunModuleTask(Worker* worker, typename T::MyPrincipal const& ep, EventSetup const& es, StreamID streamID,
                    ParentContext const& parentContext, typename T::Context const* context)
          : m_worker(worker),
            m_principal(ep),
            m_es(es),
        ...
        if (not excptr) {
          if (auto queue = m_worker->serializeRunModule()) {
            ...
            queue.push([worker, &principal, &es, streamID, parentContext, sContext, serviceToken]() {
              ...
              worker->runModuleAfterAsyncPrefetch<T>(ptr, principal, es, streamID, parentContext, sContext);
            });
            return nullptr;
          }
        }
  ...
  namespace workerhelper {
    ...
    public:
      ...
      static bool call(Worker* iWorker, StreamID, EventPrincipal const& ep, EventSetup const& es,
    ...
    public:
      ...
      static bool call(Worker* iWorker, StreamID, RunPrincipal const& ep, EventSetup const& es,
                       ActivityRegistry* actReg, ModuleCallingContext const* mcc, Arg::Context const* context) {
    ...
    public:
      ...
      static bool call(Worker* iWorker, StreamID id, RunPrincipal const& ep, EventSetup const& es,
                       ActivityRegistry* actReg, ModuleCallingContext const* mcc, Arg::Context const* context) {
        ModuleSignalSentry<Arg> cpp(actReg, context, mcc);
    ...
    public:
      ...
      static bool call(Worker* iWorker, StreamID, LuminosityBlockPrincipal const& ep, EventSetup const& es,
                       ActivityRegistry* actReg, ModuleCallingContext const* mcc, Arg::Context const* context) {
        ModuleSignalSentry<Arg> cpp(actReg, context, mcc);
        return iWorker->implDoBegin(ep, es, mcc);
      }
    ...
    public:
      ...
      static bool call(Worker* iWorker, StreamID id, LuminosityBlockPrincipal const& ep, EventSetup const& es,
                       ActivityRegistry* actReg, ModuleCallingContext const* mcc, Arg::Context const* context) {
        ModuleSignalSentry<Arg> cpp(actReg, context, mcc);
        return iWorker->implDoStreamBegin(id, ep, es, mcc);
      }
  ...
  template <typename T>
  void Worker::doWorkAsync(WaitingTask* task, typename T::MyPrincipal const& ep, EventSetup const& es,
                           StreamID streamID, ParentContext const& parentContext, typename T::Context const* context) {
    ...
    if (workStarted_.compare_exchange_strong(expected, true)) {
      ...
      if (workerhelper::CallImpl<T>::needToRunSelection(this)) {
        ...
        auto runTask =
            new (tbb::task::allocate_root()) RunModuleTask<T>(this, ep, es, streamID, parentContext, context);
  ...
  template <typename T>
  void Worker::doWorkNoPrefetchingAsync(WaitingTask* task, typename T::MyPrincipal const& principal,
  ...
  template <typename T>
  bool Worker::doWork(typename T::MyPrincipal const& ep, EventSetup const& es, StreamID streamID,
                      ParentContext const& parentContext, typename T::Context const* context) {
    if (T::isEvent_) {
      timesVisited_.fetch_add(1, std::memory_order_relaxed);
    }
  ...
  template <typename T>
  bool Worker::runModule(typename T::MyPrincipal const& ep, EventSetup const& es, StreamID streamID,
                         ParentContext const& parentContext, typename T::Context const* context) {
  ...
  template <typename T>
  std::exception_ptr Worker::runModuleDirectly(typename T::MyPrincipal const& ep, EventSetup const& es,
                                               StreamID streamID, ParentContext const& parentContext,
                                               typename T::Context const* context) {
    timesVisited_.fetch_add(1, std::memory_order_relaxed);
