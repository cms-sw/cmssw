...
OutputModuleBase::OutputModuleBase(ParameterSet const& pset)
  ...
  wantAllEvents_ = detail::configureEventSelector(
      selectEvents_, process_name_, getAllTriggerNames(), selectors_[0],
      consumesCollector());
}
...
void OutputModuleBase::selectProducts(
    ProductRegistry const& preg,
    ThinnedAssociationsHelper const& thinnedAssociationsHelper) {
  if (productSelector_.initialized()) return;
  productSelector_.initialize(productSelectorRules_,
                              preg.allBranchDescriptions());
  ...
  for (auto association : associationDescriptions) {
    if (keepAssociation_[association->branchID()]) {
      keepThisBranch(*association, trueBranchIDToKeptBranchDesc,
                     keptProductsInEvent);
    } else {
  ...
  // Now fill in a mapping needed in the case that a branch was dropped while
  // its EDAlias was kept.
  ProductSelector::fillDroppedToKept(preg, trueBranchIDToKeptBranchDesc,
                                     droppedBranchIDToKeptBranchID_);

  thinnedAssociationsHelper_->updateFromParentProcess(
      thinnedAssociationsHelper, keepAssociation_,
      droppedBranchIDToKeptBranchID_);
}

void OutputModuleBase::keepThisBranch(
  ...
  switch (desc.branchType()) {
    case InEvent: {
      ...
      token = consumes(TypeToGet{desc.unwrappedTypeID(), PRODUCT_TYPE},
                       InputTag{desc.moduleLabel(), desc.productInstanceName(),
                                desc.processName()});
      break;
    }
    case InLumi: {
      token = consumes<InLumi>(
          TypeToGet{desc.unwrappedTypeID(), PRODUCT_TYPE},
          InputTag(desc.moduleLabel(), desc.productInstanceName(),
                   desc.processName()));
      break;
    }
    case InRun: {
      token = consumes<InRun>(
          TypeToGet{desc.unwrappedTypeID(), PRODUCT_TYPE},
          InputTag(desc.moduleLabel(), desc.productInstanceName(),
                   desc.processName()));
      break;
...
void OutputModuleBase::doPreallocate(PreallocationConfiguration const& iPC) {
  ...
  for (auto& s : selectors_) {
    if (seenFirst) {
      detail::configureEventSelector(selectEvents_, process_name_,
                                     getAllTriggerNames(), s,
                                     consumesCollector());
    } else {
      seenFirst = true;
    }
...
bool OutputModuleBase::doEvent(EventPrincipal const& ep, EventSetup const&,
  ...
  while (keepTrying) {
    ...
    keepTrying =
        !remainingEvents_.compare_exchange_strong(remainingEvents, newValue);
    if (keepTrying) {
...
bool OutputModuleBase::doBeginRun(RunPrincipal const& rp, EventSetup const&,
                                  ModuleCallingContext const* mcc) {
...
bool OutputModuleBase::doEndRun(RunPrincipal const& rp, EventSetup const&,
                                ModuleCallingContext const* mcc) {
  RunForOutput r(rp, moduleDescription_, mcc);
  r.setConsumer(this);
  doEndRun_(r);
...
bool OutputModuleBase::doBeginLuminosityBlock(
    LuminosityBlockPrincipal const& lbp, EventSetup const&,
    ModuleCallingContext const* mcc) {
...
void OutputModuleBase::doWriteLuminosityBlock(
    LuminosityBlockPrincipal const& lbp, ModuleCallingContext const* mcc) {
  LuminosityBlockForOutput lb(lbp, moduleDescription_, mcc);
  lb.setConsumer(this);
    ...
    *branchIDLists_ = *origBranchIDLists_;
    ...
    for (BranchIDList& branchIDList : *branchIDLists_) {
      for (BranchID::value_type& branchID : branchIDList) {
        ...
        // so the alias branch will have the product ID of the original branch.
        std::map<BranchID::value_type, BranchID::value_type>::const_iterator
            iter = droppedBranchIDToKeptBranchID_.find(branchID);
...
ThinnedAssociationsHelper const* OutputModuleBase::thinnedAssociationsHelper()
    const {
...
void OutputModuleBase::fillDescriptions(
...
void OutputModuleBase::setEventSelectionInfo(
  ...
  selector_config_id_ = detail::registerProperSelectionInfo(
      getParameterSet(selector_config_id_), description().moduleLabel(),
