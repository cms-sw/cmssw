...
namespace edm {
  namespace global {
    ...
    bool OutputModuleBase::prePrefetchSelection(StreamID id, EventPrincipal const& ep,
    ...
    bool OutputModuleBase::doEvent(EventPrincipal const& ep, EventSetup const&, ActivityRegistry* act,
                                   ModuleCallingContext const* mcc) {
      {
        EventForOutput e(ep, moduleDescription_, mcc);
        e.setConsumer(this);
    ...
    bool OutputModuleBase::doBeginRun(RunPrincipal const& rp, EventSetup const&, ModuleCallingContext const* mcc) {
    ...
    bool OutputModuleBase::doEndRun(RunPrincipal const& rp, EventSetup const&, ModuleCallingContext const* mcc) {
      RunForOutput r(rp, moduleDescription_, mcc);
      r.setConsumer(this);
      doEndRun_(r);
      return true;
    ...
    void OutputModuleBase::doWriteRun(RunPrincipal const& rp, ModuleCallingContext const* mcc) {
      RunForOutput r(rp, moduleDescription_, mcc);
      r.setConsumer(this);
      writeRun(r);
    }

    bool OutputModuleBase::doBeginLuminosityBlock(LuminosityBlockPrincipal const& lbp, EventSetup const&,
                                                  ModuleCallingContext const* mcc) {
    ...
    bool OutputModuleBase::doEndLuminosityBlock(LuminosityBlockPrincipal const& lbp, EventSetup const&,
                                                ModuleCallingContext const* mcc) {
      LuminosityBlockForOutput lb(lbp, moduleDescription_, mcc);
      lb.setConsumer(this);
        ...
        *branchIDLists_ = *origBranchIDLists_;
        ...
        for (BranchIDList& branchIDList : *branchIDLists_) {
          for (BranchID::value_type& branchID : branchIDList) {
            // Replace BranchID of each dropped branch with that of the kept alias, so the alias branch will have the
            // product ID of the original branch.
