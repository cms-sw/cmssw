...
// 1a M Fischler 2/13/08 Clear the all_must_fail_ array at the start of
// initPathNames.
//			 This is needed in the case of paths with wildcards,
//			 on explicit processes other than a current process
//			 (in which case initPathNames() is called whenever the
//trigger
...
EventSelector::EventSelector(ParameterSet const& config,
    ...
    : pathspecs_(config.empty() ? Strings()
                                : initPathSpecs(config.getParameter<Strings>(
                                      "SelectEvents"))),
      results_from_current_process_(true),
      accept_all_(initAcceptAll()),
      absolute_acceptors_(),
    ...
    if (!psetID_.isValid() || psetID_ != tr.parameterSetID()) {
      ...
      if (tns->getTrigPaths(tr, pathNames, fromPSetRegistry)) {
        ...
        if (fromPSetRegistry) {
          ...
          // This can only happen for very old data, when the path names were
          // stored
          // in TriggerResults itself, rather than in the parameter set
          // registry.
          psetID_.reset();
        }
      } else {
...
bool EventSelector::acceptOneBit(Bits const& b, HLTGlobalStatus const& tr,
  ...
  for (auto const& bit : b) {
    hlt::HLTState bstate = lookForException
                               ? hlt::Exception
                               : bit.accept_state_ ? hlt::Pass : hlt::Fail;
    ...
    try {
      ...
      for (unsigned int iPath = 0; iPath < fullPathCount; iPath++) {
        ...
        for (int iState = static_cast<int>(hlt::Pass);
             iState <= static_cast<int>(hlt::Exception); iState++) {
          sampleResults[iPath] =
 ...
 */
evtSel::OverlapResult EventSelector::testSelectionOverlap(
  ...
  try {
    ...
    std::vector<bool> aPassAbs =
        expandDecisionList(a.absolute_acceptors_, true, N);
    std::vector<bool> aPassCon =
        expandDecisionList(a.conditional_acceptors_, true, N);
    std::vector<bool> aFailAbs =
        expandDecisionList(a.absolute_acceptors_, false, N);
    std::vector<bool> aFailCon =
        expandDecisionList(a.conditional_acceptors_, false, N);
    std::vector<bool> aExc =
        expandDecisionList(a.exception_acceptors_, true, N);
    ...
    overlap = overlapping(aPass, bPass) || overlapping(aFail, bFail) ||
    ...
    for (unsigned int iPath = 0; iPath < fullPathCount; iPath++) {
      ...
      for (int iState = static_cast<int>(hlt::Pass);
        ...
        sampleResults[iPath] =
            HLTPathStatus(static_cast<hlt::HLTState>(iState), 0);
        bool accept1 =
            selector1.wantAll() || selector1.acceptEvent(sampleResults);
        bool accept2 =
            selector2.wantAll() || selector2.acceptEvent(sampleResults);
        if (accept1 != accept2) {
          exactMatch = false;
        }
        if (accept1 && accept2) {
...
std::shared_ptr<TriggerResults> EventSelector::maskTriggerResults(
  ...
  for (unsigned int m = 0; m < this->all_must_fail_noex_.size(); ++m) {
    std::vector<bool> f =
        expandDecisionList(this->all_must_fail_noex_[m], false, N);
    bool all_fail = true;
    for (unsigned int ipath = 0; ipath < N; ++ipath) {
      if ((f[ipath]) && (inputResults[ipath].state() != hlt::Fail)) {
        all_fail = false;
  ...
  std::vector<bool> aPassAbs =
      expandDecisionList(this->absolute_acceptors_, true, N);
  std::vector<bool> aPassCon =
      expandDecisionList(this->conditional_acceptors_, true, N);
  std::vector<bool> aFailAbs =
      expandDecisionList(this->absolute_acceptors_, false, N);
  std::vector<bool> aFailCon =
      expandDecisionList(this->conditional_acceptors_, false, N);
  std::vector<bool> aExc =
      expandDecisionList(this->exception_acceptors_, true, N);
  for (unsigned int ipath = 0; ipath < N; ++ipath) {
  ...
  // TriggerResults
  auto maskedResults =
      std::make_shared<TriggerResults>(mask, inputResults.parameterSetID());
 ...
 */
std::shared_ptr<TriggerResults> EventSelector::maskTriggerResults(
    Strings const& pathspecs, TriggerResults const& inputResults,
    Strings const& fullPathList) {
  // fetch and validate the total number of paths
  unsigned int fullPathCount = fullPathList.size();
  if (fullPathCount != inputResults.size()) {
  ...
  HLTGlobalStatus hltGS(fullPathCount);
  auto maskedResults =
      std::make_shared<TriggerResults>(hltGS, inputResults.parameterSetID());
  for (unsigned int iPath = 0; iPath < fullPathCount; iPath++) {
...
bool EventSelector::identical(EventSelector const& a, EventSelector const& b,
                              unsigned int N) {
  // create the expanded masks for the various decision lists in a and b
