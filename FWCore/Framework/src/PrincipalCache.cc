...
void PrincipalCache::setNumberOfConcurrentPrincipals(
    PreallocationConfiguration const& iConfig) {
  eventPrincipals_.resize(iConfig.numberOfStreams());
}

RunPrincipal& PrincipalCache::runPrincipal(ProcessHistoryID const& phid,
                                           RunNumber_t run) const {
  if (phid != reducedInputProcessHistoryID_ || run != run_ ||
      runPrincipal_.get() == 0) {
    throwRunMissing();
  }
...
std::shared_ptr<RunPrincipal> const& PrincipalCache::runPrincipalPtr(
    ProcessHistoryID const& phid, RunNumber_t run) const {
  if (phid != reducedInputProcessHistoryID_ || run != run_ ||
      runPrincipal_.get() == 0) {
    throwRunMissing();
  }
...
LuminosityBlockPrincipal& PrincipalCache::lumiPrincipal(
    ProcessHistoryID const& phid, RunNumber_t run,
    LuminosityBlockNumber_t lumi) const {
  if (phid != reducedInputProcessHistoryID_ || run != run_ || lumi != lumi_ ||
...
PrincipalCache::lumiPrincipalPtr(ProcessHistoryID const& phid, RunNumber_t run,
                                 LuminosityBlockNumber_t lumi) const {
  if (phid != reducedInputProcessHistoryID_ || run != run_ || lumi != lumi_ ||
      lumiPrincipal_.get() == 0) {
    throwLumiMissing();
...
LuminosityBlockPrincipal& PrincipalCache::lumiPrincipal() const {
...
void PrincipalCache::merge(std::shared_ptr<RunAuxiliary> aux,
  ...
  if (inputProcessHistoryID_ != aux->processHistoryID()) {
    if (reducedInputProcessHistoryID_ !=
        processHistoryRegistry_->reducedProcessHistoryID(
            aux->processHistoryID())) {
      throw edm::Exception(edm::errors::LogicError)
          << "PrincipalCache::merge\n"
          << "Illegal attempt to merge run into cache\n"
          << "Reduced ProcessHistoryID inconsistent with the one already in "
             "cache\n"
          << "Contact a Framework Developer\n";
    }
...
void PrincipalCache::merge(std::shared_ptr<LuminosityBlockAuxiliary> aux,
                           std::shared_ptr<ProductRegistry const> reg) {
  if (lumiPrincipal_.get() == 0) {
    throw edm::Exception(edm::errors::LogicError)
        << "PrincipalCache::merge\n"
  ...
  if (aux->run() != run_ || aux->luminosityBlock() != lumi_) {
    throw edm::Exception(edm::errors::LogicError)
        << "PrincipalCache::merge\n"
        << "Illegal attempt to merge lumi into cache\n"
        << "Run and lumi numbers are inconsistent with the ones already in the "
           "cache\n"
        << "Contact a Framework Developer\n";
  }
...
void PrincipalCache::deleteRun(ProcessHistoryID const& phid, RunNumber_t run) {
  ...
  if (reducedInputProcessHistoryID_ != phid || run != run_) {
    throw edm::Exception(edm::errors::LogicError)
        << "PrincipalCache::deleteRun\n"
        << "Illegal attempt to delete run from cache\n"
        << "Run number or reduced ProcessHistoryID inconsistent with those in "
           "cache\n"
        << "Contact a Framework Developer\n";
  }
  runPrincipal_.reset();
...
void PrincipalCache::deleteLumi(ProcessHistoryID const& phid, RunNumber_t run,
  ...
  if (reducedInputProcessHistoryID_ != phid || run != run_ || lumi != lumi_) {
    throw edm::Exception(edm::errors::LogicError)
        << "PrincipalCache::deleteLumi\n"
        << "Illegal attempt to delete luminosity block from cache\n"
        << "Run number, lumi numbers, or reduced ProcessHistoryID inconsistent "
           "with those in cache\n"
        << "Contact a Framework Developer\n";
  }
...
void PrincipalCache::adjustEventsToNewProductRegistry(
    std::shared_ptr<ProductRegistry const> reg) {
...
void PrincipalCache::throwLumiMissing() const {
  throw edm::Exception(edm::errors::LogicError)
      ...
      << "Requested a luminosity block that is not in the cache (should never "
         "happen)\n"
      << "Contact a Framework Developer\n";
}
}
