...
namespace edm {
  ...
  LuminosityBlockPrincipal& PrincipalCache::lumiPrincipal(ProcessHistoryID const& phid, RunNumber_t run,
                                                          LuminosityBlockNumber_t lumi) const {
    if (phid != reducedInputProcessHistoryID_ || run != run_ || lumi != lumi_ || lumiPrincipal_.get() == nullptr) {
      throwLumiMissing();
    }
    return *lumiPrincipal_.get();
  }

  std::shared_ptr<LuminosityBlockPrincipal> const& PrincipalCache::lumiPrincipalPtr(
  ...
  std::shared_ptr<LuminosityBlockPrincipal> const& PrincipalCache::lumiPrincipalPtr() const {
    if (lumiPrincipal_.get() == nullptr) {
      throwLumiMissing();
    }
    return lumiPrincipal_;
  ...
  void PrincipalCache::insert(std::shared_ptr<LuminosityBlockPrincipal> lbp) {
    ...
    if (inputProcessHistoryID_ != lbp->aux().processHistoryID()) {
      if (reducedInputProcessHistoryID_ !=
          processHistoryRegistry_->reducedProcessHistoryID(lbp->aux().processHistoryID())) {
        throw edm::Exception(edm::errors::LogicError) << "PrincipalCache::insert\n"
                                                      << "Illegal attempt to insert lumi into cache\n"
  ...
  void PrincipalCache::deleteLumi(ProcessHistoryID const& phid, RunNumber_t run, LuminosityBlockNumber_t lumi) {
    if (lumiPrincipal_.get() == nullptr) {
      throw edm::Exception(edm::errors::LogicError) << "PrincipalCache::deleteLumi\n"
