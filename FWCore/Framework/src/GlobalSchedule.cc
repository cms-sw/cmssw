...
GlobalSchedule::GlobalSchedule(
    ...
    std::vector<std::string> const& iModulesToUse, ParameterSet& proc_pset,
  ...
  for (auto const& moduleLabel : iModulesToUse) {
    ...
    if (modpset !=
        nullptr) {  // It will be null for PathStatusInserters, it should
                    // be impossible to be null for anything else
      assert(isTracked);

      // side effect keeps this module around
      addToAllWorkers(workerManager_.getWorker(
          *modpset, pregistry, &prealloc, processConfiguration, moduleLabel));
    }
  }
  if (inserter) {
    results_inserter_ =
        WorkerPtr(new edm::WorkerT<TriggerResultInserter::ModuleType>(
            inserter, inserter->moduleDescription(),
  ...
  for (auto& pathStatusInserter : pathStatusInserters) {
    std::shared_ptr<PathStatusInserter> inserterPtr =
        get_underlying(pathStatusInserter);
    WorkerPtr workerPtr(new edm::WorkerT<PathStatusInserter::ModuleType>(
  ...
  for (auto& endPathStatusInserter : endPathStatusInserters) {
    std::shared_ptr<EndPathStatusInserter> inserterPtr =
...
std::vector<ModuleDescription const*> GlobalSchedule::getAllModuleDescriptions()
    const {
...
void GlobalSchedule::addToAllWorkers(Worker* w) {
