...
namespace edm {
  GlobalSchedule::GlobalSchedule(
      ...
      std::shared_ptr<ModuleRegistry> modReg, std::vector<std::string> const& iModulesToUse, ParameterSet& proc_pset,
      ProductRegistry& pregistry, PreallocationConfiguration const& prealloc, ExceptionToActionTable const& actions,
      std::shared_ptr<ActivityRegistry> areg, std::shared_ptr<ProcessConfiguration> processConfiguration,
      ProcessContext const* processContext)
      : workerManager_(modReg, areg, actions), actReg_(areg), processContext_(processContext) {
    ...
    if (inserter) {
      results_inserter_ = WorkerPtr(new edm::WorkerT<TriggerResultInserter::ModuleType>(
  ...
  void GlobalSchedule::endJob(ExceptionCollector& collector) { workerManager_.endJob(collector); }

  void GlobalSchedule::beginJob(ProductRegistry const& iRegistry) { workerManager_.beginJob(iRegistry); }
  ...
  void GlobalSchedule::addToAllWorkers(Worker* w) { workerManager_.addToAllWorkers(w); }
}
