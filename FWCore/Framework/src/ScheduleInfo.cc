...
//
void ScheduleInfo::availableModuleLabels(
    std::vector<std::string>& oLabelsToFill) const {
  using std::placeholders::_1;
  std::vector<ModuleDescription const*> desc =
      schedule_->getAllModuleDescriptions();
...
const ParameterSet* ScheduleInfo::parametersForModule(
    const std::string& iLabel) const {
  using std::placeholders::_1;
  std::vector<ModuleDescription const*> desc =
      schedule_->getAllModuleDescriptions();
  ...
  std::vector<ModuleDescription const*>::iterator itFound =
      std::find_if(desc.begin(), desc.end(),
                   std::bind(std::equal_to<std::string>(), iLabel,
                             std::bind(&ModuleDescription::moduleLabel, _1)));
  if (itFound == desc.end()) {
    return nullptr;
  }
...
void ScheduleInfo::availablePaths(
    std::vector<std::string>& oLabelsToFill) const {
  schedule_->availablePaths(oLabelsToFill);
}

void ScheduleInfo::modulesInPath(
    const std::string& iPathLabel,
    std::vector<std::string>& oLabelsToFill) const {
