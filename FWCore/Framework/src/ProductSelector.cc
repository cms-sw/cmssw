...
void ProductSelector::initialize(ProductSelectorRules const& rules,
                                 VCBDP const& branchDescriptions) {
...
void ProductSelector::print(std::ostream& os) const {
  os << "ProductSelector at: " << static_cast<void const*>(this) << " has "
     << productsToSelect_.size() << " products to select:\n";
  copy_all(productsToSelect_, std::ostream_iterator<std::string>(os, "\n"));
}

void ProductSelector::checkForDuplicateKeptBranch(
    BranchDescription const& desc, std::map<BranchID, BranchDescription const*>&
  ...
  if (desc.produced()) {
    ...
    if (iter != trueBranchIDToKeptBranchDesc.end()) {
      throw edm::Exception(errors::Configuration, "Duplicate Output Selection")
          << "Two (or more) equivalent branches have been selected for "
             "output.\n"
          << "#1: " << BranchKey(desc) << "\n"
...
// Fills in a mapping needed in the case that a branch was dropped while its
// EDAlias was kept.
void ProductSelector::fillDroppedToKept(
    ProductRegistry const& preg,
    std::map<BranchID, BranchDescription const*> const&
        trueBranchIDToKeptBranchDesc,
  ...
  for (auto const& it : preg.productList()) {
    ...
    std::map<BranchID, BranchDescription const*>::const_iterator iter =
        trueBranchIDToKeptBranchDesc.find(branchID);
    if (iter != trueBranchIDToKeptBranchDesc.end()) {
      // This branch, produced in this process, or an alias of it, was
      // persisted.
      BranchID const& keptBranchID = iter->second->branchID();
      if (keptBranchID != branchID) {
        // An EDAlias branch was persisted.
        droppedBranchIDToKeptBranchID_.insert(
