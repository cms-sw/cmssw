...
Event::Event(EventPrincipal const& ep, ModuleDescription const& md,
    ...
    : provRecorder_(ep, md),
      ...
      luminosityBlock_(ep.luminosityBlockPrincipalPtrValid()
                           ? new LuminosityBlock(ep.luminosityBlockPrincipal(),
...
void Event::setSharedResourcesAcquirer(
...
bool Event::getProcessParameterSet(std::string const& processName,
  ...
  bool process_found =
      processHistory().getConfigurationForProcess(processName, config);
  if (process_found) {
...
edm::ParameterSet const* Event::parameterSet(
    edm::ParameterSetID const& psID) const {
...
void Event::commit_aux(Event::ProductPtrVec& products, bool record_parents,
  ...
  // Check that previousParentageId is still good by seeing if previousParentage
  // matches gotBranchIDs_
  bool sameAsPrevious = ((0 != previousParentage) &&
                         (previousParentage->size() == gotBranchIDs_.size()));
  if (record_parents && !gotBranchIDs_.empty()) {
    ...
    for (BranchIDSet::const_iterator it = gotBranchIDs_.begin(),
                                     itEnd = gotBranchIDs_.end();
         it != itEnd; ++it) {
    ...
    if (!sameAsPrevious && 0 != previousParentage) {
      previousParentage->assign(gotBranchIDVector.begin(),
                                gotBranchIDVector.end());
    }
  }

  // If we don't have a valid previousParentage then we want to use a temp value
  // in order to
  // avoid constantly recalculating the ParentageID which is a time consuming
  // operation
  ParentageID temp;
  if (!previousParentage) {
...
TriggerNames const& Event::triggerNames(
