...
namespace {
struct StackGuard {
  StackGuard(std::string const& iTypeName,
             std::map<std::string, unsigned int>& ioStack, bool iFromListener)
      : numType_(++ioStack[iTypeName]),
        itr_(ioStack.find(iTypeName)),
        fromListener_(iFromListener) {
    if (iFromListener) {
      ++(itr_->second);
    }
  }
...
void SignallingProductRegistry::addCalled(BranchDescription const& iProd,
  ...
  if (guard.numType_ > 2) {
    throw cms::Exception("CircularReference")
        ...
        << " from module " << iProd.moduleLabel() << " with product instance \""
        << iProd.productInstanceName() << "\"\n"
        << "However, this was in reaction to a registration of a production "
           "for the same type \n"
        << "from another module who was also listening to product "
           "registrations.\n"
