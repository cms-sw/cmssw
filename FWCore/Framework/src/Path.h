...
namespace edm {
  ...
  public:
    ...
    Path(int bitpos, std::string const& path_name, WorkersInPath const& workers, TrigResPtr trptr,
         ExceptionToActionTable const& actions, std::shared_ptr<ActivityRegistry> reg,
         StreamContext const* streamContext, std::atomic<bool>* stopProcessEvent, PathContext::PathType pathType);

    Path(Path const&);

    template <typename T>
    void runAllModulesAsync(WaitingTask*, typename T::MyPrincipal const&, EventSetup const&, StreamID const&,
                            typename T::Context const*);

    void processOneOccurrenceAsync(WaitingTask*, EventPrincipal const&, EventSetup const&, StreamID const&,
                                   StreamContext const*);
  ...
  private:
    ...
    bool handleWorkerFailure(cms::Exception& e, int nwrwue, bool isEvent, bool begin, BranchType branchType,
                             ModuleDescription const&, std::string const& id);
    static void exceptionContext(cms::Exception& ex, bool isEvent, bool begin, BranchType branchType,
                                 ModuleDescription const&, std::string const& id, PathContext const&);
    void recordStatus(int nwrwue, bool isEvent);
    void updateCounters(bool succeed, bool isEvent);

    void finished(int iModuleIndex, bool iSucceeded, std::exception_ptr, StreamContext const*,
                  EventPrincipal const& iEP, EventSetup const& iES, StreamID const& streamID);
    ...
    void workerFinished(std::exception_ptr const* iException, unsigned int iModuleIndex, EventPrincipal const& iEP,
                        EventSetup const& iES, StreamID const& iID, StreamContext const* iContext);
    void runNextWorkerAsync(unsigned int iNextModuleIndex, EventPrincipal const&, EventSetup const&, StreamID const&,
                            StreamContext const*);
  };
  ...
  template <typename T>
  void Path::runAllModulesAsync(WaitingTask* task, typename T::MyPrincipal const& p, EventSetup const& es,
