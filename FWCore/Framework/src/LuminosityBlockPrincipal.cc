...
LuminosityBlockPrincipal::LuminosityBlockPrincipal(
    ...
    std::shared_ptr<ProductRegistry const> reg, ProcessConfiguration const& pc,
...
void LuminosityBlockPrincipal::fillLuminosityBlockPrincipal(
...
void LuminosityBlockPrincipal::put(BranchDescription const& bd,
                                   std::unique_ptr<WrapperBase> edp) const {
  putOrMerge(bd, std::move(edp));
}

unsigned int LuminosityBlockPrincipal::transitionIndex_() const {
  return index().value();
}
}
