...
unsigned int EDConsumerBase::recordConsumes(BranchType iBranch,
  ...
  m_tokenInfo.emplace_back(
      TokenLookupInfo{iType.type(), ProductResolverIndexInvalid,
                      skipCurrentProcess, iBranch},
...
void EDConsumerBase::updateLookup(BranchType iBranchType,
  ...
  assert(!containsCurrentProcessAlias_);
  {
    ...
    for (auto itInfo = m_tokenInfo.begin<kLookupInfo>(),
      ...
      if (itInfo->m_branchType == iBranchType) {
        ...
        itInfo->m_index = ProductResolverIndexAndSkipBit(
            iHelper.index(*itKind, itInfo->m_type, moduleLabel,
  ...
  for (size_t i = 0, iEnd = m_tokenInfo.size(); i != iEnd; ++i) {
    ...
    if (info.m_branchType == iBranchType &&
        ...
        m_tokenLabels[m_tokenInfo.get<kLabels>(i).m_startOfModuleLabel] ==
            '\0') {
      // find all matching types
      const auto kind = m_tokenInfo.get<kKind>(i);
      auto matches = iHelper.relatedIndexes(kind, info.m_type);

      // NOTE: This could be changed to contain the true labels for what is
      // being
      ...
      for (unsigned int j = 0; j != matches.numberOfMatches(); ++j) {
        // only keep the ones that are for a specific data item and not a
        // collection
        if (matches.isFullyResolved(j)) {
          auto index = matches.index(j);
          m_tokenInfo.emplace_back(
              TokenLookupInfo{info.m_type, index,
                              info.m_index.skipCurrentProcess(),
                              info.m_branchType},
              alwaysGet, labels, kind);
        }
      }
    }
  }
...
//
ProductResolverIndexAndSkipBit EDConsumerBase::indexFrom(
    EDGetToken iToken, BranchType iBranch, TypeID const& iType) const {
  ...
  return ProductResolverIndexAndSkipBit(edm::ProductResolverIndexInvalid,
                                        false);
}

void EDConsumerBase::itemsToGet(
    BranchType iBranch,
    std::vector<ProductResolverIndexAndSkipBit>& oIndices) const {
...
void EDConsumerBase::itemsMayGet(
    BranchType iBranch,
    std::vector<ProductResolverIndexAndSkipBit>& oIndices) const {
  // how many are we adding?
  unsigned int count = 0;
...
bool EDConsumerBase::registeredToConsume(ProductResolverIndex iIndex,
  ...
  nonConstThis->m_tokenInfo.emplace_back(
      TokenLookupInfo{TypeID{}, iIndex, skipCurrentProcess, iBranch}, true,
      LabelPlacement{0, 0, 0}, PRODUCT_TYPE);
...
bool EDConsumerBase::registeredToConsumeMany(TypeID const& iType,
  ...
  for (auto it = m_tokenInfo.begin<kLookupInfo>(),
    ...
    if (it->m_index.productResolverIndex() == ProductResolverIndexInvalid and
        it->m_type == iType and it->m_branchType == iBranch) {
      return true;
    }
  }
  // TEMPORARY: Remember so we do not have to do this again
  ...
  EDConsumerBase* nonConstThis = const_cast<EDConsumerBase*>(this);
  nonConstThis->m_tokenInfo.emplace_back(
      TokenLookupInfo{iType, ProductResolverIndexInvalid, false, iBranch}, true,
      LabelPlacement{0, 0, 0}, PRODUCT_TYPE);
  return false;
}

void EDConsumerBase::throwTypeMismatch(edm::TypeID const& iType,
                                       EDGetToken iToken) const {
  throw cms::Exception("TypeMismatch")
      << "A get using a EDGetToken used the C++ type '" << iType.className()
      << "' but the consumes call was for type '"
      << m_tokenInfo.get<kLookupInfo>(iToken.index()).m_type.className()
      << "'.\n Please modify either the consumes or get call so the types "
         "match.";
}
void EDConsumerBase::throwBranchMismatch(BranchType iBranch,
                                         EDGetToken iToken) const {
  throw cms::Exception("BranchTypeMismatch")
      << "A get using a EDGetToken was done in " << BranchTypeToString(iBranch)
      << " but the consumes call was for "
      << BranchTypeToString(
             m_tokenInfo.get<kLookupInfo>(iToken.index()).m_branchType)
...
void EDConsumerBase::throwBadToken(edm::TypeID const& iType,
  ...
  if (iToken.isUninitialized()) {
    throw cms::Exception("BadToken")
        << "A get using a EDGetToken with the C++ type '" << iType.className()
        << "' was made using an uninitialized token.\n Please check that the "
           "variable is being initialized from a 'consumes' call.";
  }
  throw cms::Exception("BadToken")
      << "A get using a EDGetToken with the C++ type '" << iType.className()
      << "' was made using a token with a value " << iToken.index()
      << " which is beyond the range used by this module.\n Please check that "
         "the variable is being initialized from a 'consumes' call from this "
         "module.\n You can not share EDGetToken values between modules.";
}

void EDConsumerBase::throwConsumesCallAfterFrozen(
    TypeToGet const& typeToGet, InputTag const& inputTag) const {
...
void insertFoundModuleLabel(
  ...
  std::vector<std::pair<std::string, std::string> > const& aliasToOriginal =
      preg.aliasToOriginal();
  std::pair<std::string, std::string> target(consumedModuleLabel,
                                             std::string());
  auto iter =
      std::lower_bound(aliasToOriginal.begin(), aliasToOriginal.end(), target);
...
void EDConsumerBase::modulesWhoseProductsAreConsumed(
  ...
  for (auto itInfo = m_tokenInfo.begin<kLookupInfo>(),
    ...
    if (itInfo->m_branchType == InEvent and
      ...
      const char* consumedProcessName =
          consumedModuleLabel + itLabels->m_deltaToProcessName;

      if (*consumedModuleLabel != '\0') {  // not a consumesMany
        if (*consumedProcessName !=
            '\0') {  // process name is specified in consumes call
          if (processName == consumedProcessName &&
              iHelper.index(
                  *itKind, itInfo->m_type, consumedModuleLabel,
                  consumedModuleLabel + itLabels->m_deltaToProductInstance,
                  consumedModuleLabel + itLabels->m_deltaToProcessName) !=
                  ProductResolverIndexInvalid) {
            insertFoundModuleLabel(consumedModuleLabel, modules, alreadyFound,
                                   labelsToDesc, preg);
          }
        ...
        // consumesMany case
      } else if (itInfo->m_index.productResolverIndex() ==
                 ProductResolverIndexInvalid) {
        auto matches = iHelper.relatedIndexes(*itKind, itInfo->m_type);
        for (unsigned int j = 0; j < matches.numberOfMatches(); ++j) {
          if (processName == matches.processName(j)) {
            insertFoundModuleLabel(matches.moduleLabel(j), modules,
                                   alreadyFound, labelsToDesc, preg);
          }
        }
      }
    }
...
void EDConsumerBase::convertCurrentProcessAlias(
  ...
  if (containsCurrentProcessAlias_) {
    ...
    for (auto iter = m_tokenInfo.begin<kLabels>(),
              itEnd = m_tokenInfo.end<kLabels>();
         iter != itEnd; ++iter) {
      newProcessName = &m_tokenLabels[iter->m_startOfModuleLabel +
                                      iter->m_deltaToProcessName];
    ...
    for (auto iter = m_tokenInfo.begin<kLabels>(),
      ...
      newTokenLabels.insert(
          newTokenLabels.end(), m_tokenLabels.begin() + startOfModuleLabel,
          m_tokenLabels.begin() + (startOfModuleLabel + deltaToProcessName));

      newTokenLabels.insert(newTokenLabels.end(), newProcessName.begin(),
                            newProcessName.end());
...
std::vector<ConsumesInfo> EDConsumerBase::consumesInfo() const {
  ...
  for (auto itInfo = m_tokenInfo.begin<kLookupInfo>(),
    ...
    result.emplace_back(itInfo->m_type, consumedModuleLabel, consumedInstance,
                        consumedProcessName, itInfo->m_branchType, *itKind,
                        *itAlways, itInfo->m_index.skipCurrentProcess());
  }
  return result;
}
