...
unsigned int EDConsumerBase::recordConsumes(BranchType iBranch, TypeToGet const& iType, edm::InputTag const& iTag,
                                            bool iAlwaysGets) {
  if (frozen_) {
...
void EDConsumerBase::updateLookup(BranchType iBranchType, ProductResolverIndexHelper const& iHelper,
...
//
ProductResolverIndexAndSkipBit EDConsumerBase::indexFrom(EDGetToken iToken, BranchType iBranch,
...
bool EDConsumerBase::registeredToConsume(ProductResolverIndex iIndex, bool skipCurrentProcess,
...
void EDConsumerBase::throwBadToken(edm::TypeID const& iType, EDGetToken iToken) const {
  ...
  throw cms::Exception("BadToken") << "A get using a EDGetToken with the C++ type '" << iType.className()
                                   ...
                                   << " which is beyond the range used by this module.\n Please check that the "
                                      "variable is being initialized from a 'consumes' call from this module.\n You "
                                      "can not share EDGetToken values between modules.";
}
...
namespace {
  void insertFoundModuleLabel(const char* consumedModuleLabel, std::vector<ModuleDescription const*>& modules,
