...
EventPrincipal::EventPrincipal(
    ...
    unsigned int streamIndex, bool isForPrimaryProcess)
...
void EventPrincipal::clearEventPrincipal() {
  ...
  aux_ = EventAuxiliary();
  luminosityBlockPrincipal_ =
      nullptr;  // propagate_const<T> has no reset() function
  provRetrieverPtr_->reset();
  branchListIndexToProcessIndex_.clear();
}

void EventPrincipal::fillEventPrincipal(
    EventAuxiliary const& aux,
    ProcessHistoryRegistry const& processHistoryRegistry,
    EventSelectionIDVector&& eventSelectionIDs,
    BranchListIndexes&& branchListIndexes,
    ProductProvenanceRetriever const& provRetriever, DelayedReader* reader,
  ...
  if (branchIDListHelper_->hasProducedProducts()) {
    ...
    branchListIndexes_.push_back(
        branchIDListHelper_->producedBranchListIndex());
  }
...
void EventPrincipal::fillEventPrincipal(
  ...
  if (aux.event() == invalidEventNumber) {
    throw Exception(errors::LogicError)
        << "EventPrincipal::fillEventPrincipal, Invalid event number provided "
           "in EventAuxiliary, It is illegal for the event number to be 0\n";
  }
  ...
  if (branchListIndexes_.empty() and
      branchIDListHelper_->hasProducedProducts()) {
    // Add index into BranchIDListRegistry for products produced this process
    //  if it hasn't already been filled in by the other fillEventPrincipal or
  ...
  for (auto& prod : *this) {
    if (prod->singleProduct()) {
      // If an alias is in the same process as the original then isAlias will be
      // true.
      //  Under that condition, we want the ProductID to be the same as the
      //  original.
      //  If not, then we've internally changed the original BranchID to the
      //  alias BranchID
      //  in the ProductID lookup so we need the alias BranchID.
      auto const& bd = prod->branchDescription();
      prod->setProvenance(
          productProvenanceRetrieverPtr(), processHistory(),
          branchIDToProductID(bd.isAlias() ? bd.originalBranchID()
                                           : bd.branchID()));
    }
  }
}

void EventPrincipal::setLuminosityBlockPrincipal(
    std::shared_ptr<LuminosityBlockPrincipal> const& lbp) {
  luminosityBlockPrincipal_ = lbp;
}

void EventPrincipal::setRunAndLumiNumber(RunNumber_t run,
...
RunPrincipal const& EventPrincipal::runPrincipal() const {
...
void EventPrincipal::put(BranchDescription const& bd,
  ...
  // assert commented out for DaqSource.  When DaqSource no longer uses put(),
  // the assert can be restored.
  // assert(produced());
  if (edp.get() == nullptr) {
...
BranchID EventPrincipal::pidToBid(ProductID const& pid) const {
  ...
  return productIDToBranchID(pid, branchIDListHelper_->branchIDLists(),
                             branchListIndexes_);
}
...
ProductID EventPrincipal::branchIDToProductID(BranchID const& bid) const {
  ...
  for (Iter it = range.first; it != range.second; ++it) {
    ...
    std::map<BranchListIndex, ProcessIndex>::const_iterator i =
        branchListIndexToProcessIndex_.find(blix);
  ...
  // cannot throw, because some products may legitimately not have product ID's
  // (e.g. pile-up).
  return ProductID();
}

unsigned int EventPrincipal::transitionIndex_() const {
...
static void throwProductDeletedException(
  ...
  exception << "get by product ID: The product with given id: " << pid
            ...
            << "\nwas already deleted. This is a configuration error. Please "
               "change the configuration of the module which caused this "
               "exception to state it reads this data.";
  throw exception;
}
...
BasicHandle EventPrincipal::getByProductID(ProductID const& pid) const {
  ...
  if (phb == nullptr) {
    return BasicHandle(
        makeHandleExceptionFactory([pid]() -> std::shared_ptr<cms::Exception> {
          std::shared_ptr<cms::Exception> whyFailed(std::make_shared<Exception>(
              errors::ProductNotFound, "InvalidID"));
...
WrapperBase const* EventPrincipal::getIt(ProductID const& pid) const {
...
WrapperBase const* EventPrincipal::getThinnedProduct(ProductID const& pid,
  ...
  // Loop over thinned containers which were made by selecting elements from the
  // parent container
  for (auto
           associatedBranches = thinnedAssociationsHelper_->parentBegin(parent),
    ...
    if (associatedBranches->parent() !=
        pidToBid(thinnedAssociation->parentCollectionID())) {
      continue;
    }
    ...
    ProductID const& thinnedCollectionPID =
        thinnedAssociation->thinnedCollectionID();
    BasicHandle bhThinned = getByProductID(thinnedCollectionPID);
    if (!bhThinned.isValid()) {
      // Thinned container is not found, try looking recursively in thinned
      // containers
      // which were made by selecting elements from this thinned container.
      WrapperBase const* wrapperBase =
          getThinnedProduct(thinnedCollectionPID, thinnedIndex);
...
void EventPrincipal::getThinnedProducts(
    ProductID const& pid, std::vector<WrapperBase const*>& foundContainers,
  ...
  for (auto
    ...
    unsigned int doNotLookForThisIndex =
...
Provenance EventPrincipal::getProvenance(
    ProductID const& pid, ModuleCallingContext const* mcc) const {
  BranchID bid = pidToBid(pid);
  return getProvenance(bid, mcc);
}

EventSelectionIDVector const& EventPrincipal::eventSelectionIDs() const {
  return eventSelectionIDs_;
}

BranchListIndexes const& EventPrincipal::branchListIndexes() const {
  return branchListIndexes_;
}

edm::ThinnedAssociation const* EventPrincipal::getThinnedAssociation(
    edm::BranchID const& branchID) const {
  ConstProductResolverPtr const phb = getProductResolver(branchID);

  if (phb == nullptr) {
    throw Exception(errors::LogicError)
        << "EventPrincipal::getThinnedAssociation, ThinnedAssociation "
           "ProductResolver cannot be found\n"
        << "This should never happen. Contact a Framework developer";
  }
  ProductData const* productData =
      (phb->resolveProduct(*this, false, nullptr, nullptr)).data();
  ...
  if (!(typeid(edm::ThinnedAssociation) == product->dynamicTypeInfo())) {
    throw Exception(errors::LogicError) << "EventPrincipal::getThinnedProduct, "
                                           "product has wrong type, not a "
                                           "ThinnedAssociation.\n";
  }
  Wrapper<ThinnedAssociation> const* wrapper =
      static_cast<Wrapper<ThinnedAssociation> const*>(product);
