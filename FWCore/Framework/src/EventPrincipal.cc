...
namespace edm {
  ...
  void EventPrincipal::fillEventPrincipal(EventAuxiliary const& aux,
                                          ProcessHistoryRegistry const& processHistoryRegistry, DelayedReader* reader) {
  ...
  void EventPrincipal::setLuminosityBlockPrincipal(std::shared_ptr<LuminosityBlockPrincipal> const& lbp) {
  ...
  void EventPrincipal::put(BranchDescription const& bd, std::unique_ptr<WrapperBase> edp,
  ...
  void EventPrincipal::putOnRead(BranchDescription const& bd, std::unique_ptr<WrapperBase> edp,
                                 ProductProvenance const* productProvenance) const {
    assert(!bd.produced());
  ...
  void EventPrincipal::getThinnedProducts(ProductID const& pid, std::vector<WrapperBase const*>& foundContainers,
