...
EDM_REGISTER_PLUGINFACTORY(edm::InputSourcePluginFactory,
...
std::unique_ptr<InputSource> InputSourceFactory::makeInputSource(
    ParameterSet const& conf, InputSourceDescription const& desc) const

{
  std::string modtype = conf.getParameter<std::string>("@module_type");
  FDEBUG(1) << "InputSourceFactory: module_type = " << modtype << std::endl;
  std::unique_ptr<InputSource> wm = std::unique_ptr<InputSource>(
      InputSourcePluginFactory::get()->create(modtype, conf, desc));
  ...
  FDEBUG(1) << "InputSourceFactory: created input source " << modtype
            << std::endl;
