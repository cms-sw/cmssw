...
//
// const EventSetupRecord& EventSetupRecord::operator=(const EventSetupRecord&
// rhs)
// {
...
// member functions
//
void EventSetupRecord::set(const ValidityInterval& iInterval) {
  validity_ = iInterval;
}

void EventSetupRecord::getESProducers(
  ...
  for (auto const& iData : proxies_) {
    ComponentDescription const* componentDescription =
        iData.second->providerDescription();
...
void EventSetupRecord::fillReferencedDataKeys(
...
bool EventSetupRecord::add(const DataKey& iKey, const DataProxy* iProxy) {
  //
  ...
  if (0 != proxy) {
    ...
    // POLICY: If a Producer and a Source both claim to deliver the same data,
    // the
    //  Producer 'trumps' the Source. If two modules of the same type claim to
    //  deliver the
    //  same data, this is an error unless the configuration specifically states
    //  which one
    ...
    if (proxy->providerDescription()->isSource_ ==
        iProxy->providerDescription()->isSource_) {
      // should lookup to see if there is a specified 'chosen' one and only if
      // not, throw the exception
      throw cms::Exception("EventSetupConflict")
          << "two EventSetup "
          << (proxy->providerDescription()->isSource_ ? "Sources" : "Producers")
          << " want to deliver type=\"" << iKey.type().name() << "\" label=\""
          << iKey.name().value() << "\"\n"
          << " from record " << key().type().name()
          << ". The two providers are \n"
          << "1) type=\"" << proxy->providerDescription()->type_
          << "\" label=\"" << proxy->providerDescription()->label_ << "\"\n"
          << "2) type=\"" << iProxy->providerDescription()->type_
          << "\" label=\"" << iProxy->providerDescription()->label_ << "\"\n"
          << "Please either\n   remove one of these "
          << (proxy->providerDescription()->isSource_ ? "Sources" : "Producers")
          << "\n   or find a way of configuring one of them so it does not "
             "deliver this data"
          << "\n   or use an es_prefer statement in the configuration to "
             "choose one.";
...
void EventSetupRecord::clearProxies() { proxies_.clear(); }
...
const void* EventSetupRecord::getFromProxy(DataKey const& iKey,
  ...
  if (0 != proxy) {
    try {
      ...
      addTraceInfoToCmsException(e, iKey.name().value(),
                                 proxy->providerDescription(), iKey);
      // NOTE: the above function can't do the 'throw' since it causes the C++
      // class type
...
const DataProxy* EventSetupRecord::find(const DataKey& iKey) const {
...
bool EventSetupRecord::doGet(const DataKey& aKey, bool aGetTransiently) const {
  ...
  if (0 != proxy) {
    try {
      ...
      addTraceInfoToCmsException(e, aKey.name().value(),
                                 proxy->providerDescription(), aKey);
      // NOTE: the above function can't do the 'throw' since it causes the C++
      // class type
      // of the throw to be changed, a 'rethrow' does not have that problem
      throw;
...
bool EventSetupRecord::wasGotten(const DataKey& aKey) const {
  const DataProxy* proxy = find(aKey);
  if (0 != proxy) {
...
void EventSetupRecord::fillRegisteredDataKeys(
    std::vector<DataKey>& oToFill) const {
  oToFill.clear();
  oToFill.reserve(proxies_.size());

  for (std::map<DataKey, const DataProxy*>::const_iterator
           it = proxies_.begin(),
           itEnd = proxies_.end();
       it != itEnd; ++it) {
...
void EventSetupRecord::validate(const ComponentDescription* iDesc,
                                const ESInputTag& iTag) const {
  if (iDesc && iTag.module().size()) {
    ...
    if (!matched) {
      throw cms::Exception("EventSetupWrongModule")
          << "EventSetup data was retrieved using an ESInputTag with the "
             "values\n"
          << "  moduleLabel = '" << iTag.module() << "'\n"
          << "  dataLabel = '" << iTag.data() << "'\n"
          << "but the data matching the C++ class type and dataLabel comes "
             "from module type="
          << iDesc->type_ << " label='" << iDesc->label_
          << "'.\n Please either change the ESInputTag's 'module' label to be "
          << (iDesc->label_.empty() ? iDesc->type_ : iDesc->label_)
          << "\n or add the EventSetup module " << iTag.module()
          << " to the configuration.";
    }
  }
}

void EventSetupRecord::addTraceInfoToCmsException(
    cms::Exception& iException, const char* iName,
    const ComponentDescription* iDescription, const DataKey& iKey) const {
  std::ostringstream ost;
  ost << "Using EventSetup component " << iDescription->type_ << "/'"
      << iDescription->label_ << "' to make data " << iKey.type().name() << "/'"
      << iName << "' in record " << this->key().type().name();
