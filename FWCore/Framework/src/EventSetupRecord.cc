...
namespace edm {
  namespace eventsetup {
    ...
    // member functions
    //
    void EventSetupRecord::set(const ValidityInterval& iInterval) { validity_ = iInterval; }

    void EventSetupRecord::getESProducers(std::vector<ComponentDescription const*>& esproducers) {
      esproducers.clear();
    ...
    void EventSetupRecord::fillReferencedDataKeys(std::map<DataKey, ComponentDescription const*>& referencedDataKeys) {
    ...
    const void* EventSetupRecord::getFromProxy(DataKey const& iKey, const ComponentDescription*& iDesc,
    ...
    bool EventSetupRecord::doGet(const DataKey& aKey, bool aGetTransiently) const {
      ...
      if (nullptr != proxy) {
        try {
          convertException::wrap([&]() { proxy->doGet(*this, aKey, aGetTransiently); });
        } catch (cms::Exception& e) {
    ...
    void EventSetupRecord::addTraceInfoToCmsException(cms::Exception& iException, const char* iName,
