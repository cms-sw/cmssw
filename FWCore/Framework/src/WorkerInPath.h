...
class WorkerInPath {
 public:
  ...
  template <typename T>
  void runWorkerAsync(WaitingTask* iTask, typename T::MyPrincipal const&,
                      EventSetup const&, StreamID streamID,
  ...
  void setPathContext(PathContext const* v) {
    placeInPathContext_.setPathContext(v);
  }
...
void WorkerInPath::runWorkerAsync(WaitingTask* iTask,
  ...
  if (T::isEvent_) {
    ...
    worker_->doWorkNoPrefetchingAsync<T>(iTask, ep, es, streamID, parentContext,
                                         context);
  }
}

template <typename T>
bool WorkerInPath::runWorker(typename T::MyPrincipal const& ep,
                             EventSetup const& es, StreamID streamID,
                             typename T::Context const* context) {
  if (T::isEvent_) {
    ++timesVisited_;
