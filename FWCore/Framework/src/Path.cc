...
Path::Path(int bitpos, std::string const& path_name,
           WorkersInPath const& workers, TrigResPtr trptr,
...
bool Path::handleWorkerFailure(cms::Exception& e, int nwrwue, bool isEvent,
    ...
    default: {
      ...
      if (action == exception_actions::Rethrow) {
        ...
        if (e.category() == pNF) {
          ...
          ost << "If you wish to continue processing events after a " << pNF
              << " exception,\n"
              << "add \"SkipEvent = cms.untracked.vstring('ProductNotFound')\" "
                 "to the \"options\" PSet in the configuration.\n";
...
void Path::exceptionContext(cms::Exception& ex, bool isEvent, bool begin,
...
void Path::setEarlyDeleteHelpers(
  ...
  // we use a temp so we can overset the size but then when moving to
  // earlyDeleteHelpers we only
...
void Path::processOneOccurrenceAsync(WaitingTask* iTask,
  ...
  if (workers_.empty()) {
    finished(-1, true, std::exception_ptr(), iStreamContext, iEP, iES,
             iStreamID);
    return;
  }
...
void Path::workerFinished(std::exception_ptr const* iException,
  ...
  if (iException) {
    ...
    try {
      std::ostringstream ost;
      ost << iEP.id();
      shouldContinue = handleWorkerFailure(
          *pEx, iModuleIndex, /*isEvent*/ true, /*isBegin*/ true, InEvent,
  ...
  finished(iModuleIndex, shouldContinue, finalException, iContext, iEP, iES,
           iID);
}

void Path::finished(int iModuleIndex, bool iSucceeded,
  ...
  try {
    ...
    std::exception_ptr jException =
        pathStatusInserterWorker_->runModuleDirectly<
      ...
      make_waiting_task(tbb::task::allocate_root(),
                        [this, iNextModuleIndex, &iEP, &iES, iID, iContext,
                         token](std::exception_ptr const* iException) {
                          ...
                          this->workerFinished(iException, iNextModuleIndex,
                                               iEP, iES, iID, iContext);
                        });
