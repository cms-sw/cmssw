...
__class__Producer::__class__Producer(const edm::ParameterSet& iConfig)
  ...
  string alias;
  produces<std::vector<__class__> >(alias = outputName_)
      .setBranchAddress(alias);
}
...
__class__Producer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  // for ( vector<Muon>::iterator imuon = muons->begin(); imuon != muons->end();
  // imuon++ )
  //    for ( vector<Muon>::iterator jmuon = imuon + 1; jmuon != muons->end();
  //    jmuon++ )
  //
  for (...;...;...) {
    __class__ hyp;
