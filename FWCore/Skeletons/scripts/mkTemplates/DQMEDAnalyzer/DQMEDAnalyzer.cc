...
@example_stream #include "DQMServices/Core/interface/DQMEDAnalyzer.h" @example_global #include
...
@example_streamclass __class__ : public DQMEDAnalyzer {
  ...
  private:
    @example_stream void bookHistograms(
        DQMStore::IBooker&, @example_stream edm::Run const&, @example_stream edm::EventSetup const&) override;
    ...
    @example_global void dqmAnalyze(
        edm::Event const&, @example_global edm::EventSetup const&, @example_global Histograms___class__ const&)
        const override;
  ...
  //
  __class__::__class__(const edm::ParameterSet& iConfig) : folder_(iConfig.getParameter<std::string>("folder")) {
  ...
  @example_streamvoid __class__::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
    ...
    @example_stream @example_stream eventCount_++;
    @example_stream @example_stream example_->Fill(5);
  ...
  @example_stream @example_streamvoid __class__::bookHistograms(
      DQMStore::IBooker & ibook, @example_stream edm::Run const& run, @example_stream edm::EventSetup const& iSetup) {
    @example_stream ibook.setCurrentFolder(folder_);
    @example_stream @example_stream example_ = ibook.book1D("EXAMPLE", "Example 1D", 20, 0., 10.);
    ...
    @example_stream exampleTProfile2D_ =
        ibook.bookProfile2D("EXAMPLE_TPROFILE2D", "Example TProfile 2D", 20, 0, 20, 15, 0, 15, 0, 100);
    @example_stream
  }

  @example_globalvoid __class__::dqmAnalyze(
      edm::Event const& iEvent, edm::EventSetup const& iSetup, @example_global Histograms___class__ const& histos)
      const {
    @example_global
  }
  @example_global
      ...
      @example_globalvoid
      __class__::bookHistograms(DQMStore::ConcurrentBooker & ibook,
