...
// ------------ method called once per job just before starting to loop over
// events  ------------
...
// ------------ called for each event in the loop.  The present event loop can
// be stopped by return kStop ------------
__class__::Status __class__::duringLoop(const edm::Event&,
                                        const edm::EventSetup&) {
  return kContinue;
}

// ------------ called at the end of each event loop. A new loop will occur if
// you return kContinue ------------
