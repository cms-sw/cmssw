...
EDM_REGISTER_PLUGINFACTORY(edm::PresencePluginFactory,
...
std::unique_ptr<Presence> PresenceFactory::makePresence(
    std::string const& presence_type) const {
  std::unique_ptr<Presence> sp(
      PresencePluginFactory::get()->create(presence_type));
  ...
  FDEBUG(1) << "PresenceFactory: created presence " << presence_type
            << std::endl;
