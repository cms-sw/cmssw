...
//
// const PluginFactoryBase& PluginFactoryBase::operator=(const
// PluginFactoryBase& rhs)
// {
...
void* PluginFactoryBase::findPMaker(const std::string& iName) const {
  ...
  if (itFound == m_plugins.end()) {
    std::string lib =
        PluginManager::get()->load(this->category(), iName).path().string();
    itFound = m_plugins.find(iName);
    if (itFound == m_plugins.end()) {
      throw cms::Exception("PluginCacheError")
          << "The plugin '" << iName << "' should have been in loadable\n '"
          << lib << "'\n but was not there.  This means the plugin cache is "
                    "incorrect.  Please run 'EdmPluginRefresh "
          << lib << "'";
    }
  } else {
    // The item in the container can still be under construction so wait until
    // the m_ptr has been set since that is done last
...
void* PluginFactoryBase::tryToFindPMaker(const std::string& iName) const {
  ...
  Plugins::const_iterator itFound = m_plugins.find(iName);
  if (itFound == m_plugins.end()) {
    const SharedLibrary* slib =
        PluginManager::get()->tryToLoad(this->category(), iName);
    if (0 != slib) {
...
void PluginFactoryBase::fillAvailable(std::vector<PluginInfo>& iReturn) const {
  PluginInfo info;
  for (Plugins::const_iterator it = m_plugins.begin(); it != m_plugins.end();
       ++it) {
    info.name_ = it->first;
...
void PluginFactoryBase::checkProperLoadable(
    const std::string& iName, const std::string& iLoadedFrom) const {
  ...
  if (iLoadedFrom != PluginManager::staticallyLinkedLoadingFileName() &&
    ...
    if (iLoadedFrom !=
      ...
      throw cms::Exception("WrongPluginLoaded")
          ...
          << "'\n because some other plugin was loaded from the latter "
             "loadables.\n"
...
void PluginFactoryBase::registerPMaker(void* iPMaker,
  ...
  assert(0 != iPMaker);
  m_plugins[iName].push_back(
      PluginMakerInfo(iPMaker, PluginManager::loadingFile()));
  newPlugin(iName);
}
