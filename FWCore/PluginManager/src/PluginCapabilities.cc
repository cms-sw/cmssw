...
//
// const PluginCapabilities& PluginCapabilities::operator=(const
// PluginCapabilities& rhs)
// {
...
bool PluginCapabilities::tryToFind(const SharedLibrary& iLoadable) {
  ...
  reinterpret_cast<void (*)(const char**&, int&)>(
      reinterpret_cast<unsigned long>(sym))(names, size);
...
bool PluginCapabilities::tryToLoad(const std::string& iName) {
  if (classToLoadable_.end() == classToLoadable_.find(iName)) {
    const SharedLibrary* lib =
        PluginManager::get()->tryToLoad(category(), iName);
    if (0 == lib) {
      return false;
    }
    // read the items from the 'capabilities' symbol
...
std::vector<PluginInfo> PluginCapabilities::available() const {
  ...
  for (std::map<std::string, boost::filesystem::path>::const_iterator it =
           classToLoadable_.begin();
       it != classToLoadable_.end(); ++it) {
    info.name_ = it->first;
