...
//
static void checkForError(const std::istream& iIn, unsigned long iRecordNumber,
  ...
  if (iIn.bad()) {
    throw cms::Exception("PluginCacheParseFailed")
        << "Reading failed on line " << iRecordNumber << " just after '"
        << iContext << "'";
  }
}
...
void CacheParser::read(std::istream& iIn,
  ...
  for (CacheParser::CategoryToInfos::iterator it = iOut.begin(),
                                              itEnd = iOut.end();
       it != itEnd; ++it) {
...
void CacheParser::write(const CategoryToInfos& iInfos, std::ostream& oOut) {
  ...
  for (CategoryToInfos::const_iterator it = iInfos.begin(); it != iInfos.end();
       ++it) {
...
void CacheParser::write(LoadableToPlugins& iIn, std::ostream& oOut) {
  for (LoadableToPlugins::iterator it = iIn.begin(); it != iIn.end(); ++it) {
    ...
    for (std::vector<std::pair<std::string, std::string> >::iterator it2 =
             it->second.begin();
         it2 != it->second.end(); ++it2) {
      oOut << loadable << " " << replaceSpaces(it2->first) << " "
           << replaceSpaces(it2->second) << "\n";
    }
  }
}
