...
// spawn a new one.
// Due to the different behavior of the linker on different platforms, we
// process
...
ostream& operator<<(std::ostream& o, vector<std::string> const& iValue) {
  std::string sep("");
  std::string commaSep(",");
  for (std::vector<std::string>::const_iterator it = iValue.begin(),
                                                itEnd = iValue.end();
       it != itEnd; ++it) {
    o << sep << *it;
    sep = commaSep;
...
struct Listener {
  ...
  void newFactory(edmplugin::PluginFactoryBase const* iBase) {
    ...
    iBase->newPluginAdded_.connect(
        std::bind(std::mem_fn(&Listener::newPlugin), this, _1, _2));
  }
  void newPlugin(std::string const& iCategory,
                 edmplugin::PluginInfo const& iInfo) {
...
int main(int argc, char** argv) try {
  ...
  std::vector<std::string> defaultDirList =
      edmplugin::standard::config().searchPath();
  ...
  desc.add_options()(kHelpCommandOpt, "produce help message")(
      ...
      //(kAllCommandOpt, "when no paths given, try to update caches for all
      //known directories [default is to only scan the first directory]")
      ;
  ...
  /*if(argc == 1) {
    std::cerr << "Requires at least one argument.  Please pass either one
  directory or a list of files (all in the same directory)." << std::endl;
    return 1;
  } */
  ...
  try {
    std::vector<std::string> requestedPaths(
        vm[kPathsOpt].as<std::vector<std::string> >());

    // first find the directory and create a list of files to look at in that
    // directory
    ...
    if (boost::filesystem::is_directory(directory)) {
      if (requestedPaths.size() > 1) {
        std::cerr << "if a directory is given then only one argument is allowed"
                  << std::endl;
        return 1;
      }

      // if asked to look at whole directory, then we can also remove missing
      // files
      ...
      for (std::vector<std::string>::iterator it = requestedPaths.begin(),
                                              itEnd = requestedPaths.end();
           it != itEnd; ++it) {
        boost::filesystem::path f(*it);
        if (!exists(f)) {
          std::cerr << "the file '" << f.string() << "' does not exist"
                    << std::endl;
          return 1;
        }
        if (is_directory(f)) {
          std::cerr << "either one directory or a list of files are allowed as "
                       "arguments"
                    << std::endl;
          return 1;
        }
        if (directory != f.branch_path()) {
          std::cerr << "all files must have be in the same directory ("
                    << directory.string() << ")\n"
                                             " the file "
    ...
    Listener listener;
    edmplugin::PluginFactoryManager* pfm =
        edmplugin::PluginFactoryManager::get();
    pfm->newFactory_.connect(
        std::bind(std::mem_fn(&Listener::newFactory), &listener, _1));
    edm::for_all(*pfm,
                 std::bind(std::mem_fn(&Listener::newFactory), &listener, _1));
        ...
        while (ci && fi != fe) {
          ...
          try {
            try {
              ...
              // PluginCapabilities is special, the plugins do not call it.
              // Instead, for each shared library load
              ...
              if (iException.category() == "PluginLibraryLoadError") {
                std::cerr << "Caught exception " << iException.what()
                          << " will ignore " << files[fi] << " and continue."
                          << std::endl;
              } else {
                throw;
              }
    ...
    if (removeMissingFiles) {
      for (
          CacheParser::LoadableToPlugins::iterator itFile = old.begin();
          itFile != old.end();
          /*don't advance the iterator here because it may have become invalid */) {
        path loadableFile(directory);
        loadableFile /= (itFile->first);
        if (not exists(loadableFile)) {
          std::cout << "removing file '" << temporaryFilename.c_str() << "'"
                    << std::endl;
          CacheParser::LoadableToPlugins::iterator itToItemBeingRemoved =
              itFile;
