...
//
/**\class PluginFactoryBase PluginFactoryBase.h
 FWCore/PluginManager/interface/PluginFactoryBase.h

 Description: Base class for all plugin factories

 Usage:
    This interface provides access to the most generic information about a
 plugin factory:
...
class PluginFactoryBase {
 public:
  ...
  mutable edm::signalslot::Signal<void(const std::string&, const PluginInfo&)>
      newPluginAdded_;
  ...
  */
  ...
  // since each inheriting class has its own Container type to hold their
  // PMakers
  // this function allows them to share the same code when doing the lookup
  // this routine will throw an exception if iName is unknown therefore the
  ...
  void fillInfo(const PMakers& makers, PluginInfo& iInfo,
 ...
 private:
  ...
  void checkProperLoadable(const std::string& iName,
                           const std::string& iLoadedFrom) const;
  // ---------- member data --------------------------------
  Plugins m_plugins;
};
}
