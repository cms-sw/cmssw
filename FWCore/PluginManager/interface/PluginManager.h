...
class PluginManager {
 ...
 public:
  ...
  /**The container is ordered by category, then plugin name and then by
    precidence order of the plugin files.
    Therefore the first match on category and plugin name will be the proper
    file to load
    */
  const CategoryToInfos& categoryToInfos() const { return categoryToInfos_; }

  // If can not find iPlugin in category iCategory return null pointer, any
  // other failure will cause a throw
  ...
  /// if the value returned from loadingFile matches this string then the file
 ...
 private:
  ...
  const boost::filesystem::path& loadableFor_(
      const std::string& iCategory, const std::string& iPlugin,
      bool& ioThrowIfFailElseSucceedStatus);
  // ---------- member data --------------------------------
  SearchPath searchPath_;
  tbb::concurrent_unordered_map<boost::filesystem::path,
