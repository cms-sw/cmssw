...
/**\class CacheParser CacheParser.h FWCore/PluginManager/interface/CacheParser.h
 ...
 Description: Parses the cache information about which plugins are in which
libraries
...
If a space exists in either of these three fields, it will be replaced with a %,
E.g.
...
class CacheParser {
 public:
  ...
  // ---------- member functions ---------------------------
  /**The std::vector<PluginInfo>'s in CategoryToInfos are guaranteed to be
    ordered by
    PluginInfo.name_ where identical names are ordered by the order they are
    passed to read.
    In this way multiple calls to read for different directories will preserve
    the ordering
    */
  static void read(std::istream&, const boost::filesystem::path& iDirectory,
                   CategoryToInfos& oOut);
 ...
 private:
  ...
  static bool readline(std::istream& iIn,
                       ...
                       unsigned long iRecordNumber, PluginInfo& oInfo,
