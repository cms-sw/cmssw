...
// Class  :     PluginFactory
//
/**\class PluginFactory PluginFactory.h
...
class PluginFactory<R*(Args...)> : public PluginFactoryBase {
 ...
 public:
  ...
  R* create(const std::string& iName, Args... args) const {
    return reinterpret_cast<PMakerBase*>(PluginFactoryBase::findPMaker(iName))
...
#define EDM_REGISTER_PLUGINFACTORY(_factory_, _category_)                  \
  namespace edmplugin {                                                    \
  template <>                                                              \
  edmplugin::PluginFactory<_factory_::TemplateArgType>*                    \
  edmplugin::PluginFactory<_factory_::TemplateArgType>::get() {            \
    [[cms::thread_safe]] static edmplugin::PluginFactory<                  \
        _factory_::TemplateArgType>                                        \
        s_instance;                                                        \
    return &s_instance;                                                    \
  }                                                                        \
  template <>                                                              \
  const std::string&                                                       \
  edmplugin::PluginFactory<_factory_::TemplateArgType>::category() const { \
