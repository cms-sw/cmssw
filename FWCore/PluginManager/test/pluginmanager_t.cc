...
DEFINE_EDM_PLUGIN(testedmplugin::DummyFactory, testedmplugin::DummyThree,
                  "DummyThree");
...
void TestPluginManager::test() {
  ...
  edmplugin::PluginManager::get()->goingToLoad_.connect([&nTimesGoingToLoad](
      const boost::filesystem::path&) { ++nTimesGoingToLoad; });
  ...
  CPPUNIT_ASSERT_THROW(DummyFactory::get()->create("DoesNotExist"),
                       cms::Exception);
