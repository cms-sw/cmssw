...
namespace edm {
  namespace root {
    ...
    std::unique_ptr<WrapperBase> FWLiteDelayedReader::getProduct_(BranchKey const& k, EDProductGetter const* /*ep*/) {
      return getTheProduct(k);
    }

    std::unique_ptr<WrapperBase> FWLiteDelayedReader::getTheProduct(BranchKey const& k) const {
...
Bool_t TFWLiteSelectorBasic::Process(Long64_t iEntry) {
  ...
  if (everythingOK_) {
    ...
    try {
      ...
      m_->ep_->fillEventPrincipal(*eaux, *m_->phreg_, std::move(eventSelectionIDs), std::move(branchListIndexes),
...
void TFWLiteSelectorBasic::setupNewFile(TFile& iFile) {
  ...
  m_->reg_->setFrozen();
  m_->ep_ = std::make_shared<edm::EventPrincipal>(m_->reg(), m_->branchIDListHelper(), m_->thinnedAssociationsHelper(),
                                                  m_->pc_, nullptr);
  everythingOK_ = true;
}
