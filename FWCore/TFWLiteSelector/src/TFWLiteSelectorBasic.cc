...
class FWLiteDelayedReader : public DelayedReader {
 ...
 private:
  ...
  virtual std::unique_ptr<WrapperBase> getProduct_(
      BranchKey const& k, EDProductGetter const* ep) override;
  virtual std::unique_ptr<EventEntryDescription> getProvenance_(
      BranchKey const&) const {
    return std::unique_ptr<EventEntryDescription>();
  }
  virtual void mergeReaders_(DelayedReader*) override {}
  virtual void reset_() override {}

  virtual signalslot::Signal<void(StreamContext const&,
                                  ModuleCallingContext const&)> const*
  preEventReadFromSourceSignal() const override {
    return nullptr;
  }
  virtual signalslot::Signal<void(StreamContext const&,
                                  ModuleCallingContext const&)> const*
  postEventReadFromSourceSignal() const override {
    return nullptr;
  };
...
std::unique_ptr<WrapperBase> FWLiteDelayedReader::getProduct_(
    BranchKey const& k, EDProductGetter const* /*ep*/) {
  return getTheProduct(k);
}

std::unique_ptr<WrapperBase> FWLiteDelayedReader::getTheProduct(
    BranchKey const& k) const {
  ProductRegistry::ProductList::const_iterator itFind =
      reg_->productList().find(k);
  if (itFind == reg_->productList().end()) {
    throw Exception(errors::ProductNotFound)
        << "could not find entry for product " << k;
  }
...
struct TFWLiteSelectorMembers {
  ...
  std::shared_ptr<ProductRegistry const> reg() const {
    return get_underlying_safe(reg_);
  }
...
//
// TFWLiteSelectorBasic const&
// TFWLiteSelectorBasic::operator=(TFWLiteSelectorBasic const& rhs)
// {
...
namespace {
struct Operate {
  Operate(edm::EDProductGetter const* iGetter)
      : old_(setRefCoreStreamer(iGetter)) {}
...
Bool_t TFWLiteSelectorBasic::Process(Long64_t iEntry) {
  ...
  if (everythingOK_) {
    std::unique_ptr<edm::EventAuxiliary> eaux =
        std::make_unique<edm::EventAuxiliary>();
    edm::EventAuxiliary& aux = *eaux;
    edm::EventAuxiliary* pAux = eaux.get();
    TBranch* branch = m_->tree_->GetBranch(
        edm::BranchTypeToAuxiliaryBranchName(edm::InEvent).c_str());
    ...
    TBranch* eventSelectionsBranch = m_->tree_->GetBranch(
        edm::poolNames::eventSelectionsBranchName().c_str());
    ...
    TBranch* branchListIndexBranch = m_->tree_->GetBranch(
    ...
    try {
      ...
      auto runAux = std::make_shared<edm::RunAuxiliary>(aux.run(), aux.time(),
                                                        aux.time());
      auto rp = std::make_shared<edm::RunPrincipal>(runAux, m_->reg(), m_->pc_,
                                                    nullptr, 0);
      auto lumiAux = std::make_shared<edm::LuminosityBlockAuxiliary>(
          rp->run(), 1, aux.time(), aux.time());
      auto lbp = std::make_shared<edm::LuminosityBlockPrincipal>(
          lumiAux, m_->reg(), m_->pc_, nullptr, 0);
      m_->ep_->fillEventPrincipal(*eaux, *m_->phreg_,
                                  std::move(eventSelectionIDs),
                                  std::move(branchListIndexes),
                                  *(m_->provRetriever_), m_->reader_.get());
      ...
      std::cout << "While processing entry " << iEntry
...
void TFWLiteSelectorBasic::setupNewFile(TFile& iFile) {
  ...
  TTree* metaDataTree = dynamic_cast<TTree*>(
      iFile.Get(edm::poolNames::metaDataTreeName().c_str()));
  if (!metaDataTree) {
    std::cout << "could not find TTree " << edm::poolNames::metaDataTreeName()
              << std::endl;
    everythingOK_ = false;
    return;
  }
  ...
  if (metaDataTree->FindBranch(
          edm::poolNames::fileFormatVersionBranchName().c_str()) != nullptr) {
    metaDataTree->SetBranchAddress(
        edm::poolNames::fileFormatVersionBranchName().c_str(), &fftPtr);
  }

  edm::ProductRegistry* pReg = &(*m_->reg_);
  metaDataTree->SetBranchAddress(
      edm::poolNames::productDescriptionBranchName().c_str(), &(pReg));

  typedef std::map<edm::ParameterSetID, edm::ParameterSetBlob> PsetMap;
  PsetMap psetMap;
  PsetMap* psetMapPtr = &psetMap;
  if (metaDataTree->FindBranch(
          edm::poolNames::parameterSetMapBranchName().c_str()) != nullptr) {
    metaDataTree->SetBranchAddress(
        edm::poolNames::parameterSetMapBranchName().c_str(), &psetMapPtr);
  } else {
    TTree* psetTree = dynamic_cast<TTree*>(
        iFile.Get(edm::poolNames::parameterSetsTreeName().c_str()));
    if (nullptr == psetTree) {
      throw edm::Exception(edm::errors::FileReadError)
          << "Could not find tree " << edm::poolNames::parameterSetsTreeName()
          << " in the input file.\n";
    }
    typedef std::pair<edm::ParameterSetID, edm::ParameterSetBlob> IdToBlobs;
    IdToBlobs idToBlob;
    IdToBlobs* pIdToBlob = &idToBlob;
    psetTree->SetBranchAddress(
        edm::poolNames::idToParameterSetBlobsBranchName().c_str(), &pIdToBlob);
  ...
  if (metaDataTree->FindBranch(
          edm::poolNames::processHistoryBranchName().c_str()) != nullptr) {
    metaDataTree->SetBranchAddress(
        edm::poolNames::processHistoryBranchName().c_str(), &pHistVectorPtr);
  }

  edm::ProcessConfigurationVector procConfigVector;
  edm::ProcessConfigurationVector* procConfigVectorPtr = &procConfigVector;
  if (metaDataTree->FindBranch(
          edm::poolNames::processConfigurationBranchName().c_str()) !=
      nullptr) {
    metaDataTree->SetBranchAddress(
        edm::poolNames::processConfigurationBranchName().c_str(),
        &procConfigVectorPtr);
  ...
  edm::BranchIDLists const* branchIDListsPtr =
      &branchIDListsHelper->branchIDLists();
  if (metaDataTree->FindBranch(
          edm::poolNames::branchIDListBranchName().c_str()) != nullptr) {
    metaDataTree->SetBranchAddress(
        edm::poolNames::branchIDListBranchName().c_str(), &branchIDListsPtr);
  }
  ...
  std::unique_ptr<edm::ProductRegistry> newReg =
      std::make_unique<edm::ProductRegistry>();
  ...
  std::vector<edm::EventEntryDescription> temp(prodList2.size(),
  ...
  for (auto& item : prodList2) {
    ...
    if (prod.branchType() == edm::InEvent) {
      ...
      // std::cout << "id " << it->first << " branch " << it->second <<
      // std::endl;
      // m_->pointerToBranchBuffer_.push_back(&(*itB));
      // void* tmp = &(m_->pointerToBranchBuffer_.back());
      // edm::EventEntryDescription* tmp = &(*itB);
      // CDJ need to fix provenance and be backwards compatible, for now just
      // don't read the branch
  ...
  m_->ep_ = std::make_shared<edm::EventPrincipal>(
      m_->reg(), m_->branchIDListHelper(), m_->thinnedAssociationsHelper(),
      m_->pc_, nullptr);
  everythingOK_ = true;
}
