...
//
/**\class TFWLiteSelectorBasic TFWLiteSelectorBasic.h
FWCore/FWLite/interface/TFWLiteSelectorBasic.h

 Description: A ROOT TSelector which accesses data using an edm::Event

 Usage:
    By inheriting from this class one can make a TSelector for ROOT which works
with PROOF and which
...
class TFWLiteSelectorBasic : public TSelector {
 public:
  ...
  // ---------- member functions ---------------------------
  /**Called each time the 'client' begins processing (remote 'slaves' do not see
    this message)
    \param in an assignable pointer to a list of objects you want passed to
    'preProcessing'. This
     list is used to communicate with remote slaves. NOTE: you are responsible
    for deleting this TList
  ...
  /**Called each time the 'slave' is about to start processing
    \param in a pointer to the list of objects created in 'begin()'.  The
    pointer can be 0
    \param out a list of objects that are the result of processing (e.g.
    histograms).
    You should call 'Add()' for each object you want sent to the 'terminate'
    method.
    */
 ...
 private:
  ...
  const TFWLiteSelectorBasic& operator=(
      const TFWLiteSelectorBasic&);  // stop default
