  ...
  namespace soa {
    ...
    public:
      ...
      Table(T const& iContainer, CArgs... iArgs) : m_size(iContainer.size()) {
        using CtrChoice = std::conditional_t<sizeof...(CArgs) == 0, CtrFillerFromAOS, CtrFillerFromContainers>;
      ...
      ~Table() { dtr<0>(m_values, std::true_type{}); }
    ...
    private:
      ...
      private:
        ...
        static void presize(std::array<void*, sizeof...(Args)>& oValues, size_t iSize, std::true_type) {
          ...
          presize<I + 1>(oValues, iSize,
                         std::conditional_t<I + 1 == sizeof...(Args), std::false_type, std::true_type>{});
        }
        template <int I>
        ...
        static void fillElementUsingFiller(F& iFiller, E const& iItem, size_t iIndex,
                                           std::array<void*, sizeof...(Args)>& oValues, std::true_type) {
          using Layout = std::tuple<Args...>;
          using ColumnType = typename std::tuple_element<I, Layout>::type;
          using Type = typename ColumnType::type;
          Type* pElement = static_cast<Type*>(oValues[I]) + iIndex;
          *pElement = iFiller.value(iItem, static_cast<ColumnType*>(nullptr));
          fillElementUsingFiller<I + 1>(
              iFiller, iItem, iIndex, oValues,
              std::conditional_t<I + 1 == sizeof...(Args), std::false_type, std::true_type>{});
        }
        template <int I, typename E, typename F>
        static void fillElementUsingFiller(F&, E const&, size_t, std::array<void*, sizeof...(Args)>& oValues,
                                           std::false_type) {}
      };

      template <int I>
      static void copyFromToWithResize(size_t iNElements, std::array<void*, sizeof...(Args)> const& iFrom,
                                       std::array<void*, sizeof...(Args)>& oTo, std::true_type) {
        using Layout = std::tuple<Args...>;
        using Type = typename std::tuple_element<I, Layout>::type::type;
      ...
      template <int I>
      static void copyFromToWithResize(size_t, std::array<void*, sizeof...(Args)> const&,
                                       std::array<void*, sizeof...(Args)>&, std::false_type) {}
    ...
    namespace impl {
      ...
      struct RemoveColumnCheck<LHS, E, std::tuple<T, U...>> {
        using type = typename std::conditional<
            std::is_same<E, T>::value, typename AddColumns<LHS, std::tuple<U...>>::type,
