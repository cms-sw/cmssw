...
class MassSearchReplaceAnyInputTagVisitor(object):
    ...
    def __init__(self, paramSearch, paramReplace, verbose=False,
                 moduleLabelOnly=False, skipLabelTest=False):
            ...
            for name in pset.parameterNames_():
                ...
                if type == 'cms.PSet':
                    ...
                    for (i, ps) in enumerate(value):
                        self.doIt(ps, "%s.%s[%d]" % (base, name, i))
                elif type == 'cms.VInputTag':
                    for (i, n) in enumerate(value):
                        # VInputTag can be declared as a list of strings, so
                        # ensure that n is formatted correctly
                        n = self.standardizeInputTagFmt(n)
                        if (n == self._paramSearch):
                            if self._verbose:
                                print(
                                    "Replace %s.%s[%d] %s ==> %s " %
                                    (base, name, i, self._paramSearch, self._paramReplace))
                            value[i] = self._paramReplace
                        elif self._moduleLabelOnly and n.moduleLabel == self._paramSearch.moduleLabel:
                            nrep = n
                            nrep.moduleLabel = self._paramReplace.moduleLabel
                            if self._verbose:
                                print(
                                    "Replace %s.%s[%d] %s ==> %s " %
                                    (base, name, i, n, nrep))
                    ...
                    if value == self._paramSearch:
                        if self._verbose:
                            print(
                                "Replace %s.%s %s ==> %s " %
                                (base, name, self._paramSearch, self._paramReplace))
...
def massSearchReplaceAnyInputTag(
        sequence, oldInputTag, newInputTag, verbose=False, moduleLabelOnly=False, skipLabelTest=False):
    ...
    sequence.visit(
        MassSearchReplaceAnyInputTagVisitor(
            ...
            newInputTag,
            verbose=verbose,
            moduleLabelOnly=moduleLabelOnly,
            skipLabelTest=skipLabelTest))


def massReplaceInputTag(process, old="rawDataCollector", new="rawDataRepacker",
                        verbose=False, moduleLabelOnly=False, skipLabelTest=False):
    ...
    if process.schedule_() is not None:
        for task in process.schedule_()._tasks:
            massSearchReplaceAnyInputTag(
                task, old, new, verbose, moduleLabelOnly, skipLabelTest)
    return(process)
...
def massSearchReplaceParam(
        sequence, paramName, paramOldValue, paramValue, verbose=False):
    sequence.visit(
...
def massReplaceParameter(
        process, name="label", old="rawDataCollector", new="rawDataRepacker", verbose=False):
    for s in process.paths_().keys():
    ...
    class TestModuleCommand(unittest.TestCase):
        ...
        def testMassSearchReplaceAnyInputTag(self):
            ...
            p.c = cms.EDProducer("ac", src=cms.InputTag("b"),
                                 nested=cms.PSet(
                src=cms.InputTag("b"), src2=cms.InputTag("c")),
                nestedv=cms.VPSet(
                cms.PSet(
                    src=cms.InputTag("b")), cms.PSet(
                    src=cms.InputTag("d"))),
                vec=cms.VInputTag(cms.InputTag("a"), cms.InputTag(
                    "b"), cms.InputTag("c"), cms.InputTag("d"))
            )
            p.s = cms.Sequence(p.a * p.b * p.c)
            massSearchReplaceAnyInputTag(
                p.s, cms.InputTag("b"), cms.InputTag("new"))
...
""")
            ...
            p.c = cms.EDProducer("ac", src=cms.InputTag("b"),
                                 nested=cms.PSet(
                src=cms.InputTag("a"), src2=cms.InputTag("c")),
                nestedv=cms.VPSet(
                cms.PSet(
                    src=cms.InputTag("a")), cms.PSet(
                    src=cms.InputTag("d"))),
                vec=cms.VInputTag(cms.InputTag("a"), cms.InputTag(
                    "b"), cms.InputTag("c"), cms.InputTag("d"))
            )
        ...
        def testMassReplaceParam(self):
            ...
            massReplaceParameter(
                process1, "src", cms.InputTag("a"), "b", False)
            self.assertEqual(process1.dumpPython(),
                             """import FWCore.ParameterSet.Config as cms
