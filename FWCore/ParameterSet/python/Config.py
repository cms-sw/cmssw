...
class Process(object):
    ...
    def __init__(self, name, *Mods):
        ...
        self.__dict__['_Process__paths'] = DictTypes.SortedKeysDict()
        # of definition
        self.__dict__['_Process__endpaths'] = DictTypes.SortedKeysDict()
    ...
    filters = property(
        filters_,
        doc="dictionary containing the filters for the process")
    ...
    producers = property(
        producers_,
        doc="dictionary containing the producers for the process")
    ...
    source = property(
        source_,
        setSource_,
        doc='the main source or None if not set')
    ...
    looper = property(
        looper_,
        setLooper_,
        doc='the main looper or None if not set')
    ...
    subProcesses = property(
        subProcesses_,
        doc='the SubProcesses that have been added to the Process')
    ...
    analyzers = property(
        analyzers_,
        doc="dictionary containing the analyzers for the process")
    ...
    outputModules = property(
        outputModules_,
        doc="dictionary containing the output_modules for the process")
    ...
    paths = property(
        paths_,
        doc="dictionary containing the paths for the process")
    ...
    endpaths = property(
        endpaths_,
        doc="dictionary containing the endpaths for the process")
    ...
    sequences = property(
        sequences_,
        doc="dictionary containing the sequences for the process")
    ...
    tasks = property(
        tasks_,
        doc="dictionary containing the tasks for the process")
    ...
    schedule = property(
        schedule_,
        setSchedule_,
        doc='the schedule or None if not set')
    ...
    services = property(
        services_,
        doc="dictionary containing the services for the process")
    ...
    es_producers = property(
        es_producers_,
        doc="dictionary containing the es_producers for the process")
    ...
    es_sources = property(
        es_sources_,
        doc="dictionary containing the es_sources for the process")
    ...
    es_prefers = property(
        es_prefers_,
        doc="dictionary containing the es_prefers for the process")
    ...
    aliases = property(
        aliases_,
        doc="dictionary containing the aliases for the process")
    ...
    psets = property(
        psets_,
        doc="dictionary containing the PSets for the process")
    ...
    def __setObjectLabel(self, object, newLabel):
        ...
        if newLabel is None:
            ...
            return
        if (hasattr(self, object.label_()) and id(
                getattr(self, object.label_())) == id(object)):
    ...
    def __setattr__(self, name, value):
        # check if the name is well-formed (only _ and alphanumerics are
        # allowed)
        ...
        if not isinstance(value, _Labelable) and not isinstance(
                value, Source) and not isinstance(value, Looper) and not isinstance(value, Schedule):
            if name == value.type_():
        ...
        if hasattr(self, name) and not (getattr(self, name) == newValue):
            ...
            if newValue._isTaskComponent():
                if not self.__InExtendCall:
                    ...
                    if not isinstance(newValue, Task):
                        # should check to see if used in task before
                        # complaining
                        newFile = 'top level config'
                        if hasattr(value, '_filename'):
                            newFile = value._filename
                        ...
                        if hasattr(oldValue, '_filename'):
                            oldFile = oldValue._filename
                        msg1 = "Trying to override definition of " + \
                            name + " while it is used by the task "
                        msg2 = "\n new object defined in: " + newFile
                        msg2 += "\n existing object defined in: " + oldFile
            ...
            if isinstance(
                    newValue, _Sequenceable) or newValue._isTaskComponent():
                if not self.__InExtendCall:
                    self._replaceInSequences(name, newValue)
                else:
                    # should check to see if used in sequence before
                    # complaining
                    newFile = 'top level config'
                    if hasattr(value, '_filename'):
                        newFile = value._filename
    ...
    def _delHelper(self, name):
        ...
        dicts = [
            item for item in self.__dict__.values() if (
                isinstance(
                    item,
                    dict) or isinstance(
                    item,
    ...
    def _placePath(self, name, mod):
        ...
        try:
            ...
            raise Exception(
                "%sThe module %s in path %s is unknown to the process %s." %
                (context, msg, name, self._Process__name))
    ...
    def _placeEndPath(self, name, mod):
        ...
        try:
            ...
            context = format_outerframe(4)
            raise Exception(
                "%sThe module %s in endpath %s is unknown to the process %s." %
                (context, msg, name, self._Process__name))
    ...
    def extend(self, other, items=()):
        ...
        for name in dir(other):
            ...
            if name == "source" or name == "looper":
                # In these cases 'item' could be None if the specific object
                # was not defined
                if item is not None:
    ...
    def _dumpConfigNamedList(self, items, typeName, options):
        ...
        for name, item in items:
            returnValue += options.indentation() + typeName + ' ' + name + \
    ...
    def _dumpConfigUnnamedList(self, items, typeName, options):
        returnValue = ''
        for name, item in items:
            returnValue += options.indentation() + typeName + ' = ' + \
                item.dumpConfig(options)
        return returnValue
    ...
    def dumpConfig(self, options=PrintOptions()):
        ...
        for name, item in six.iteritems(self.psets):
            config += options.indentation() + item.configTypeName() + ' ' + \
                name + ' = ' + item.configValue(options)
        for name, item in six.iteritems(self.vpsets):
            config += options.indentation() + 'VPSet ' + name + ' = ' + \
                item.configValue(options)
        if self.schedule:
            pathNames = [p.label_() for p in self.schedule]
            config += options.indentation() + \
                'schedule = {' + ','.join(pathNames) + '}\n'
...
#                                  options)
    ...
    def _dumpConfigESPrefers(self, options):
        ...
        for item in six.itervalues(self.es_prefers_()):
            result += options.indentation() + 'es_prefer ' + item.targetLabel_() + \
                ' = ' + item.dumpConfig(options)
        return result
        ...
        for label, item in six.iteritems(processDictionaryOfItems):
            ...
            for containedItem in containedItems:
                ...
                if containedItem.hasLabel_():
                    testItem = processDictionaryOfItems.get(
                        containedItem.label_())
                    if testItem is None or containedItem != testItem:
                        if isinstance(item, Task):
                            ...
                            raise RuntimeError("Sequence has a label, but using its label to get an attribute"
                                               " from the process yields a different object or None\n" +
                                               "label = " + containedItem.label_())
            dependencies[label] = [dep.label_()
                                   for dep in containedItems if dep.hasLabel_()]
    ...
    def dumpPython(self, options=PrintOptions()):
        ...
        result += self._dumpPythonList(
            self._itemsInDependencyOrder(
                self.tasks), options)
        result += self._dumpPythonList(
            self._itemsInDependencyOrder(
                self.sequences), options)
    ...
    def _insertPaths(self, processPSet, nodeVisitor):
        ...
        pathCompositeVisitor = CompositeVisitor(
            pathValidator, nodeVisitor, lister)
        endpathCompositeVisitor = CompositeVisitor(
            endpathValidator, nodeVisitor, lister)
        ...
        processPSet.addVString(
            False,
            "@filters_on_endpaths",
            endpathValidator.filtersOnEndpaths)
...
#                delattr(self, name)
    ...
    def fillProcessDesc(self, processPSet):
        ...
        class TopLevelPSetAcessorAdaptor(object):
            ...
            def newPSet(self):
                return TopLevelPSetAcessorAdaptor(
                    self.__ppset.newPSet(), self.__process)

            def addPSet(self, tracked, name, ppset):
                return self.__ppset.addPSet(
                    tracked, name, self.__extractPSet(ppset))

            def addVPSet(self, tracked, name, vpset):
                return self.__ppset.addVPSet(
                    tracked, name, [self.__extractPSet(x) for x in vpset])
        ...
        self._insertSubProcessesInto(
            adaptor,
            "@all_subprocesses",
            self.subProcesses_(),
            False)
        self._insertManyInto(
            adaptor,
            "@all_esprefers",
            self.es_prefers_(),
            True)
        ...
        all_modules_onTasksOrScheduled = {
            key: value for key,
            value in six.iteritems(all_modules) if value in nodeVisitor.modules}
        self._insertManyInto(
            adaptor,
            "@all_modules",
            all_modules_onTasksOrScheduled,
        ...
        for esProducerName, esProducer in six.iteritems(self.es_producers_()):
            if esProducer in nodeVisitor.esProducers or not (
                    esProducer in processNodeVisitor.esProducers):
        ...
        for esSourceName, esSource in six.iteritems(self.es_sources_()):
            if esSource in nodeVisitor.esSources or not (
                    esSource in processNodeVisitor.esSources):
        ...
        for serviceName, serviceObject in six.iteritems(self.services_()):
            if serviceObject in nodeVisitor.services or not (
                    serviceObject in processNodeVisitor.services):
                serviceObject.insertInto(
                    ServiceInjectorAdaptor(
                        adaptor, services))
...
class ProcessFragment(object):
    ...
    def __dir__(self):
        return [x for x in dir(self.__process) if isinstance(
            getattr(self.__process, x), _ConfigureComponent)]
...
class FilteredStream(dict):
    ...
    def __new__(cls, *args, **kw):
        ...
        keys = sorted(kw.keys())
        if keys != ['content', 'dataTier', 'name',
                    'paths', 'responsible', 'selectEvents']:
            raise ValueError(
        ...
        if not isinstance(kw['content'], vstring) and not isinstance(
                kw['content'], str):
...
class SubProcess(_Unlabelable):
    ...
    def dumpPython(self, options=PrintOptions()):
        ...
        out += "process.addSubProcess(cms.SubProcess(process = childProcess, SelectEvents = " + self.__SelectEvents.dumpPython(
            options) + ", outputCommands = " + self.__outputCommands.dumpPython(options) + "))"
        return out
...
class _ParameterModifier(object):
    ...
    def _raiseUnknownKey(key):
        raise KeyError("Unknown parameter name " + key +
...
class Modifier(object):
    ...
    def _toReplaceWith(toObj, fromObj):
        if isinstance(fromObj, _ModuleSequenceType):
            ...
            # clear old items just incase fromObj is not a complete superset of
            # toObj
    ...
    class TestModuleCommand(unittest.TestCase):
        ...
        def testProcessInsertion(self):
            ...
            self.assertRaises(TypeError, setattr, *
                              (p, 'b', "this should fail"))
            self.assertRaises(TypeError, setattr, *
                              (p, 'bad', Service("MessageLogger")))
            self.assertRaises(ValueError, setattr, *
                              (p, 'bad', Source("PoolSource")))
...
""")
        ...
        def testTask(self):
            ...
            self.assertTrue(l == set(
                ['mesproducer', 'mproducer', 'mproducer2', 'mfilter', 'd', 'messource']))
            ...
            l.sort()
            expectedList = sorted(
                [edproducer, essource, esproducer, service, edfilter, edproducer, edproducer4])
            ...
            process.sequence3 = Sequence(
                (process.mproducer8 + process.mfilter))
            sequence4 = Sequence()
            process.path1 = Path(
                process.mproducer +
                process.mproducer8 +
            ...
            self.assertTrue(l == set(['mproducer',
                                      'd',
                                      'mesproducer',
                                      None,
                                      'mproducer9',
            ...
            process.path22 = process.path21.copyAndExclude(
                [process.d, process.mesproducer, process.mfilter])
            self.assertTrue(process.path22.dumpPython(PrintOptions(
            ...
            process.path23 = process.path22.copyAndExclude(
                [process.messource, process.mproducer10])
            self.assertTrue(process.path23.dumpPython(PrintOptions(
            ...
            process.path25 = process.path24.copyAndExclude(
                [process.a, process.b, process.c])
            self.assertTrue(process.path25.dumpPython(None)
                            == 'cms.Path(process.d)\n')
            # print process.path3
            ...
            self.assertEqual(
                process.path200.dumpPython(None),
...
#            self.assertEqual(str(path),'((a*b)+c)')
            ...
            t = Path(p.a)
            self.assertTrue(t.dumpPython(PrintOptions())
                            == 'cms.Path(process.a)\n')
        ...
        def testSchedule(self):
            ...
            # adding a path not attached to the Process should cause an
            # exception
            p = Process("test")
            p.a = EDAnalyzer("MyAnalyzer")
            path1 = Path(p.a)
...
"""
            equalD = equalD.replace(
                "parentProcess", "parentProcess" + str(hash(process.subProcesses_()[0])))
            ...
            self.assertEqual(
                (True,
                 ['a']),
                p.values["subProcesses"][1][0].values["process"][1].values['@all_modules'])
            self.assertEqual(
                (True,
                 ['p']),
                p.values["subProcesses"][1][0].values["process"][1].values['@paths'])
            self.assertEqual(
                {
                    '@service_type': (
                        True,
                        'Foo')},
                p.values["subProcesses"][1][0].values["process"][1].values["services"][1][0].values)
        ...
        def testRefToPSet(self):
            ...
            self.assertEqual(
                (True, 1), p.values["ref2"][1].values["b"][1].values["a"])
...
""")
        ...
        def testDelete(self):
            ...
            self.assertTrue(p.s.dumpPython(None) ==
                            'cms.Sequence(process.d)\n')
            self.assertTrue(p.path1.dumpPython(
                None) == 'cms.Path(process.a+process.s, cms.Task(process.h))\n')
            self.assertTrue(p.endpath1.dumpPython(
                None) == 'cms.EndPath(process.b)\n')
            del p.s
            self.assertTrue(p.path1.dumpPython(
                None) == 'cms.Path(process.a+(process.d), cms.Task(process.h))\n')
            self.assertTrue(p.schedule_().dumpPython(None) ==
        ...
        def testModifier(self):
            ...
            p = Process("test", m1)
            p.a = EDAnalyzer(
                "MyAnalyzer", flintstones=PSet(
                    fred=int32(1), wilma=int32(1)))
            ...
            p.a = EDAnalyzer(
                ...
                flintstones=PSet(
                    fred=PSet(
                        wilma=int32(1))))
            self.assertRaises(
                KeyError, lambda: m1.toModify(
                    p.a, flintstones=dict(
                        imnothere=dict(
                            wilma=2))))
            ...
            # test IndexError setting a value in a list to an item key not in
            # the list
            m1 = Modifier()
            p = Process("test", m1)
            p.a = EDAnalyzer("MyAnalyzer", fred=vuint32(1, 2, 3))
            raised = False
            try:
                m1.toModify(p.a, fred={5: 7})
            except IndexError as e:
                raised = True
            self.assertEqual(raised, True)
            # test TypeError setting a value in a list using a key that is not
            # an int
            m1 = Modifier()
            p = Process("test", m1)
            p.a = EDAnalyzer(
            ...
            raised = False
            try:
                m1.toModify(p.a, flintstones=dict(bogus=int32(37)))
            ...
            self.assertRaises(
                TypeError, lambda: m1.toReplaceWith(
                    p.a, EDProducer("YourProducer")))
            p.b = EDAnalyzer("BAn")
            p.c = EDProducer("c")
            p.d = EDProducer("d")
            p.tc = Task(p.c)
            ...
            self.assertTrue(
                p.s.dumpPython("") == "cms.Sequence(process.a+process.b, process.td)\n")
            p.e = EDProducer("e")
