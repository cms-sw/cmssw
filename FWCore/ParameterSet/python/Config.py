...
class Process(object):
    ...
    def dumpConfig(self, options=PrintOptions()):
        ...
        config+=self._dumpConfigNamedList(self.subProcesses_(),
                                  'subProcess',
                                  options)
        config+=self._dumpConfigNamedList(self.producers_(six.iteritems()),
                                  'module',
                                  options)
        config+=self._dumpConfigNamedList(self.filters_(six.iteritems()),
                                  'module',
                                  options)
        config+=self._dumpConfigNamedList(self.analyzers_(six.iteritems()),
                                  'module',
                                  options)
        config+=self._dumpConfigNamedList(self.outputModules_(six.iteritems()),
                                  'module',
                                  options)
        config+=self._dumpConfigNamedList(self.sequences_(six.iteritems()),
                                  'sequence',
                                  options)
        config+=self._dumpConfigNamedList(self.paths_(six.iteritems()),
                                  'path',
                                  options)
        config+=self._dumpConfigNamedList(self.endpaths_(six.iteritems()),
                                  'endpath',
                                  options)
        config+=self._dumpConfigUnnamedList(self.services_(six.iteritems()),
                                  'service',
                                  options)
        config+=self._dumpConfigNamedList(self.aliases_(six.iteritems()),
                                  'alias',
                                  options)
        config+=self._dumpConfigOptionallyNamedList(
            self.es_producers_(six.iteritems()),
            'es_module',
            options)
        config+=self._dumpConfigOptionallyNamedList(
            self.es_sources_(six.iteritems()),
            'es_source',
            options)
...
#                                  options)
    ...
    def _dumpConfigESPrefers(self, options):
        result = ''
        for item in self.es_prefers_(six.itervalues()):
...
#                delattr(self, name)
    ...
    def fillProcessDesc(self, processPSet):
        ...
        esProducersToEnable = {}
        for esProducerName, esProducer in self.es_producers_(six.iteritems()):
        ...
        esSourcesToEnable = {}
        for esSourceName, esSource in self.es_sources_(six.iteritems()):
        ...
        services = []
        for serviceName, serviceObject in self.services_(six.iteritems()):
