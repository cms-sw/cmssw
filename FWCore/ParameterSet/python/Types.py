...
class _Untracked(object):
    ...
    def __getattr__(self, name):
        ...
        if name == "__bases__":
            raise AttributeError  # isclass uses __bases__ to recognize class objects
...
class bool(_SimpleParameterTypeBase):
    @staticmethod
    def _isValid(value):
        return (isinstance(value, type(False))
                or isinstance(value, type(True)))
...
class EventID(_ParameterTypeBase):
    def __init__(self, run, *args):
        ...
        if isinstance(run, str):
            ...
            self.__luminosityBlock = self._valueFromString(
    ...
    def pythonValue(self, options=PrintOptions()):
        return str(self.__run) + ', ' + \
            str(self.__luminosityBlock) + ', ' + str(self.__event)

    def cppID(self, parameterSet):
        return parameterSet.newEventID(
            self.run(), self.luminosityBlock(), self.event())

    def insertInto(self, parameterSet, myname):
        parameterSet.addEventID(
            self.isTracked(),
            myname,
            self.cppID(parameterSet))
...
class LuminosityBlockID(_ParameterTypeBase):
    ...
    def cppID(self, parameterSet):
        return parameterSet.newLuminosityBlockID(
            self.run(), self.luminosityBlock())

    def insertInto(self, parameterSet, myname):
        parameterSet.addLuminosityBlockID(
            self.isTracked(), myname, self.cppID(parameterSet))
...
class LuminosityBlockRange(_ParameterTypeBase):
    def __init__(self, start, startSub=None, end=None, endSub=None):
        ...
        if self.__end < self.__start:
            raise RuntimeError('LuminosityBlockRange ' +
                               str(self.__start) +
                               ':' +
                               str(self.__startSub) +
                               '-' +
        ...
        if self.__end == self.__start and (
                self.__endSub != 0 and self.__endSub < self.__startSub):
            raise RuntimeError('LuminosityBlockRange ' +
                               str(self.__start) +
                               ':' +
                               str(self.__startSub) +
    ...
    def cppID(self, parameterSet):
        return parameterSet.newLuminosityBlockRange(
            self.start(), self.startSub(), self.end(), self.endSub())

    def insertInto(self, parameterSet, myname):
        parameterSet.addLuminosityBlockRange(
            self.isTracked(), myname, self.cppID(parameterSet))
...
class EventRange(_ParameterTypeBase):
    def __init__(self, start, *args):
        ...
        if self.__end < self.__start or (
                self.__end == self.__start and self.__endLumi < self.__startLumi):
            raise RuntimeError('EventRange ' +
                               str(self.__start) +
                               ':' +
                               str(self.__startLumi) +
                               ':' +
                               str(self.__startSub) +
                               '-' +
                               str(self.__end) +
        ...
        if self.__end == self.__start and self.__endLumi == self.__startLumi and (
                self.__endSub != 0 and self.__endSub < self.__startSub):
            raise RuntimeError('EventRange ' +
                               str(self.__start) +
                               ':' +
    ...
    def pythonValue(self, options=PrintOptions()):
        return str(self.__start) + ', ' + str(self.__startLumi) + ', ' + str(self.__startSub) + ', ' \
            + str(self.__end) + ', ' + str(self.__endLumi) + \
            ', ' + str(self.__endSub)

    def cppID(self, parameterSet):
        return parameterSet.newEventRange(self.start(), self.startLumi(
        ), self.startSub(), self.end(), self.endLumi(), self.endSub())

    def insertInto(self, parameterSet, myname):
        parameterSet.addEventRange(
            self.isTracked(), myname, self.cppID(parameterSet))
...
class InputTag(_ParameterTypeBase):
    ...
    def setProductInstanceLabel(self, label):
        if self.__productInstance != label:
            ...
            self._isModified = True
    productInstanceLabel = property(
        getProductInstanceLabel,
        setProductInstanceLabel,
    ...
    def insertInto(self, parameterSet, myname):
        parameterSet.addInputTag(
            self.isTracked(),
            myname,
            self.cppTag(parameterSet))
...
class ESInputTag(_ParameterTypeBase):
    ...
    def _setValues(self, moduleLabel='', dataLabel=''):
        ...
        if -1 != moduleLabel.find(":"):
            ...
            if len(toks) > 2:
                raise RuntimeError(
                    "an ESInputTag was passed the value'" +
                    moduleLabel +
...
class SecSource(_ParameterTypeBase, _TypedParameterizable,
                _ConfigureComponent, _Labelable):
...
class PSet(_ParameterTypeBase, _Parameterizable,
    ...
    def configValue(self, options=PrintOptions()):
        ...
        for name in self.parameterNames_():
            ...
            options.indent()
            config += options.indentation() + param.configTypeName() + ' ' + name + \
    ...
    def dumpPython(self, options=PrintOptions()):
        return self.pythonTypeName() + "(\n" + _Parameterizable.dumpPython(self,
                                                                           options) + options.indentation() + ")"

    def clone(self, **params):
        myparams = self.parameters_()
        _modifyParametersFromDict(
            myparams, params, self._Parameterizable__raiseBadSetAttr)
...
class vint32(_ValidatingParameterListBase):
    ...
    def _valueFromString(value):
        return vint32(
            *_ValidatingParameterListBase._itemsFromStrings(value, int32._valueFromString))
...
class vuint32(_ValidatingParameterListBase):
    ...
    def _valueFromString(value):
        return vuint32(
            *_ValidatingParameterListBase._itemsFromStrings(value, uint32._valueFromString))
...
class vint64(_ValidatingParameterListBase):
    ...
    def _valueFromString(value):
        return vint64(
            *_ValidatingParameterListBase._itemsFromStrings(value, int64._valueFromString))
...
class vuint64(_ValidatingParameterListBase):
    ...
    def _valueFromString(value):
        return vuint64(
            *_ValidatingParameterListBase._itemsFromStrings(value, vuint64._valueFromString))
...
class vdouble(_ValidatingParameterListBase):
    ...
    def _valueFromString(value):
        return vdouble(
            *_ValidatingParameterListBase._itemsFromStrings(value, double._valueFromString))
...
class vbool(_ValidatingParameterListBase):
    ...
    def _valueFromString(value):
        return vbool(
            *_ValidatingParameterListBase._itemsFromStrings(value, bool._valueFromString))
...
class vstring(_ValidatingParameterListBase):
    ...
    def _valueFromString(value):
        return vstring(
            *_ValidatingParameterListBase._itemsFromStrings(value, string._valueFromString))
...
class VLuminosityBlockID(_ValidatingParameterListBase):
    ...
    @staticmethod
    def _valueFromString(value):
        return VLuminosityBlockID(
            *_ValidatingParameterListBase._itemsFromStrings(value, LuminosityBlockID._valueFromString))
...
class VInputTag(_ValidatingParameterListBase):
    ...
    def _valueFromString(value):
        return VInputTag(
            *_ValidatingParameterListBase._itemsFromStrings(value, InputTag._valueFromString))
...
class VESInputTag(_ValidatingParameterListBase):
    ...
    def _valueFromString(value):
        return VESInputTag(
            *_ValidatingParameterListBase._itemsFromStrings(value, ESInputTag._valueFromString))
...
class VEventID(_ValidatingParameterListBase):
    ...
    def _valueFromString(value):
        return VEventID(
            *_ValidatingParameterListBase._itemsFromStrings(value, EventID._valueFromString))
...
class VLuminosityBlockRange(_ValidatingParameterListBase):
    ...
    @staticmethod
    def _valueFromString(value):
        return VLuminosityBlockRange(
            *_ValidatingParameterListBase._itemsFromStrings(value, VLuminosityBlockRange._valueFromString))
...
class VEventRange(_ValidatingParameterListBase):
    ...
    def _valueFromString(value):
        return VEventRange(
            *_ValidatingParameterListBase._itemsFromStrings(value, VEventRange._valueFromString))
...
if __name__ == "__main__":
    ...
    class testTypes(unittest.TestCase):
        ...
        def testPSet(self):
            ...
            p3 = untracked.PSet(
                i=int32(1), ui=untracked.int32(2), a=PSet(
                    b=untracked.int32(1)), b=untracked.PSet(
                    b=int32(1)))
            p4 = p3.clone()
...
""")
        ...
        def testPSetConversion(self):
            p = PSet(a=untracked.int32(7),
                     ...
                     s=untracked.VLuminosityBlockID(
                LuminosityBlockID(
                    1, 1), LuminosityBlockID(
                    2, 3)),
                t=untracked.VEventRange(
                EventRange(
                    1, 1, 1, 8, 8, 8), EventRange(
                    9, 9, 9, 18, 18, 18)),
                u=untracked.VLuminosityBlockRange(
                LuminosityBlockRange(
                    1, 1, 8, 8), LuminosityBlockRange(
                    9, 9, 18, 18)),
            ...
            for i in p.parameterNames_():
                self.assertEqual(str(getattr(p, i)), str(
                    getattr(convert.pset.p, i)))
    ...
    class testInequalities(unittest.TestCase):
        ...
        def testincompatibletypes(self):
            import sys
            if sys.version_info < (
                    3, 0):  # python 2, comparing incompatible types compares the class name
