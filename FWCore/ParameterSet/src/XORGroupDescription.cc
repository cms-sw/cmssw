...
XORGroupDescription::XORGroupDescription(
...
void XORGroupDescription::writeCfi_(std::ostream& os, bool& startWithComma,
                                    int indentation,
                                    bool& wroteSomething) const {
  node_left_->writeCfi(os, startWithComma, indentation, wroteSomething);
}

void XORGroupDescription::print_(std::ostream& os, bool optional,
...
void XORGroupDescription::printNestedContent_(std::ostream& os, bool optional,
  ...
  printSpaces(os, indentation);
  if (optional) {
    os << "This optional XOR group requires exactly one or none of the "
          "following to be in the PSet\n";
  } else {
    os << "This XOR group requires exactly one of the following to be in the "
          "PSet\n";
  }
  if (!dfh.brief()) os << "\n";
...
int XORGroupDescription::howManyXORSubNodesExist_(
...
void XORGroupDescription::throwMoreThanOneParameter() const {
  ...
  throw edm::Exception(errors::LogicError)
      ...
      << "parameters described by an \"xor\" operator in a "
         "ParameterSetDescription.\n"
      << "This rule also applies in a more general sense to the other types\n"
      << "of nodes that can appear within a ParameterSetDescription.  Only "
         "one\n"
...
void XORGroupDescription::throwAfterValidation() const {
  ...
  throw edm::Exception(errors::LogicError)
      ...
      << "of nodes that can appear within a ParameterSetDescription.  Only "
         "one\n"
      << "can pass validation as \"existing\".  This error has occurred after "
         "an\n"
