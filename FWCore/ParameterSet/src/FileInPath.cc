...
bool locateFile(bf::path p, std::string const& relative) {
  ...
  if (bf::is_directory(p)) {
    throw edm::Exception(edm::errors::FileInPathError)
        << "Path " << p.string() << " is a directory, not a file\n";
  }
...
FileInPath::FileInPath()
    : relativePath_(), canonicalFilename_(), location_(Unknown) {
  getEnvironment();
}

FileInPath::FileInPath(const std::string& r)
    : relativePath_(r), canonicalFilename_(), location_(Unknown) {
  getEnvironment();
  initialize_();
}

FileInPath::FileInPath(char const* r)
    : relativePath_(r ? r : ""), canonicalFilename_(), location_(Unknown) {
  if (r == nullptr) {
    throw edm::Exception(edm::errors::FileInPathError)
...
void FileInPath::write(std::ostream& os) const {
  if (location_ == Unknown) {
    ...
    if (localTop_.empty()) {
      throw edm::Exception(edm::errors::FileInPathError)
          << "Environment Variable " << LOCALTOP << " is not set.\n";
    }
    std::string::size_type pos = canonicalFilename_.find(localTop_);
    if (pos != 0) {
      throw edm::Exception(edm::errors::FileInPathError)
          << "Path " << canonicalFilename_
          << " is not in the local release area " << localTop_ << "\n";
    }
    os << version << ' ' << relativePath_ << ' ' << location_ << ' '
       << canonicalFilename_.substr(localTop_.size());
  } else if (location_ == Release) {
    // Guarantee a site independent value by stripping $RELEASETOP.
    if (releaseTop_.empty()) {
      throw edm::Exception(edm::errors::FileInPathError)
          << "Environment Variable " << RELEASETOP << " is not set.\n";
    }
    std::string::size_type pos = canonicalFilename_.find(releaseTop_);
    if (pos != 0) {
      throw edm::Exception(edm::errors::FileInPathError)
          << "Path " << canonicalFilename_
          << " is not in the base release area " << releaseTop_ << "\n";
    }
    os << version << ' ' << relativePath_ << ' ' << location_ << ' '
       << canonicalFilename_.substr(releaseTop_.size());
  } else if (location_ == Data) {
    // Guarantee a site independent value by stripping $DATATOP.
    if (dataTop_.empty()) {
      throw edm::Exception(edm::errors::FileInPathError)
          << "Environment Variable " << DATATOP << " is not set.\n";
    }
    std::string::size_type pos = canonicalFilename_.find(dataTop_);
    if (pos != 0) {
      throw edm::Exception(edm::errors::FileInPathError)
          << "Path " << canonicalFilename_ << " is not in the data area "
          << dataTop_ << "\n";
    }
    os << version << ' ' << relativePath_ << ' ' << location_ << ' '
       << canonicalFilename_.substr(dataTop_.size());
  }
}
...
void FileInPath::read(std::istream& is) {
  ...
  if (location_ == Local) {
    ...
    if (oldFormat) {
      ...
      if (pos == 0) {
        // Replace the placehoder with the path to the base release (site
        // dependent).
...
void FileInPath::getEnvironment() {
  static std::string const searchPath =
      removeSymLinksTokens(PathVariableName.c_str());
  if (searchPath.empty()) {
    throw edm::Exception(edm::errors::FileInPathError)
        << PathVariableName << " must be defined\n";
  }
  ...
  if (releaseTop_ == localTop_) {
    // RELEASETOP is the same as LOCALTOP.  This means that the environment is
    // set
    // for the base release itself.  So LOCALTOP actually contains the
    // location of the base release.
    localTop_.clear();
...
void FileInPath::initialize_() {
  ...
  for (auto const& element : pathElements) {
    ...
    if (locateFile(pathPrefix, relativePath_)) {
      ...
      // From the current path element, find the branch path (basically the path
      // minus the
      // last directory, e.g. /src or /share):
      for (bf::path br = pathPrefix.branch_path();
           !br.normalize().string().empty(); br = br.branch_path()) {
        if (!localTop_.empty()) {
          // Create a path object for our local path LOCALTOP:
          bf::path local_(localTop_);
          // If the branch path matches the local path, the file was found
          // locally:
          if (br == local_) {
            location_ = Local;
            return;
        ...
        if (!releaseTop_.empty()) {
          ...
          // If the branch path matches the release path, the file was found in
          // the release:
          if (br == release_) {
            location_ = Release;
            return;
        ...
        if (!dataTop_.empty()) {
          ...
          // If the branch path matches the data path, the file was found in the
          // data area:
          if (br == data_) {
            location_ = Data;
            return;
  ...
  throw edm::Exception(edm::errors::FileInPathError)
      << "edm::FileInPath unable to find file " << relativePath_
      << " anywhere in the search path."
      << "\nThe search path is defined by: " << PathVariableName << "\n${"
      << PathVariableName << "} is: " << getenv(PathVariableName.c_str())
