...
ParameterSet::ParameterSet(std::string const& code)
    : tbl_(), psetTable_(), vpsetTable_(), id_() {
  if (!fromString(code)) {
...
std::unique_ptr<ParameterSet> ParameterSet::popParameterSet(
...
std::vector<ParameterSet> ParameterSet::popVParameterSet(
    std::string const& name) {
  assert(!isRegistered());
...
void ParameterSet::calculateID() {
  ...
  for (vpsettable::iterator i = vpsetTable_.begin(), e = vpsetTable_.end();
       i != e; ++i) {
    i->second.registerPsetsAndUpdateIDs();
  }
  //  The old, string base code is found below. Uncomment this and the assert to
  //  check whether
...
Entry const* ParameterSet::getEntryPointerOrThrow_(
  ...
  if (it->second.isTracked() == false) {
    if (name[0] == '@') {
      ...
      throw Exception(errors::Configuration, "StatusMismatch:")
          ...
          << "Please remove 'untracked' from the configuration file for "
             "parameter '"
          << name << "'.";
    }
  }
...
ParameterSetEntry const& ParameterSet::retrieveParameterSet(
    std::string const& name) const {
  psettable::const_iterator it = psetTable_.find(name);
  if (it == psetTable_.end()) {
    throw Exception(errors::Configuration, "MissingParameter:")
        << "ParameterSet '" << name << "' not found.";
  }
  if (it->second.isTracked() == false) {
    if (name[0] == '@') {
      throw Exception(errors::Configuration, "StatusMismatch:")
...
ParameterSetEntry const* ParameterSet::retrieveUntrackedParameterSet(
    std::string const& name) const {
  psettable::const_iterator it = psetTable_.find(name);
...
VParameterSetEntry const& ParameterSet::retrieveVParameterSet(
    std::string const& name) const {
  vpsettable::const_iterator it = vpsetTable_.find(name);
  if (it == vpsetTable_.end()) {
    throw Exception(errors::Configuration, "MissingParameter:")
        << "VParameterSet '" << name << "' not found.";
  }
  if (it->second.isTracked() == false) {
    throw Exception(errors::Configuration, "StatusMismatch:")
        << "VParameterSet '" << name
...
VParameterSetEntry const* ParameterSet::retrieveUntrackedVParameterSet(
    std::string const& name) const {
  vpsettable::const_iterator it = vpsetTable_.find(name);
...
ParameterSetEntry const* ParameterSet::retrieveUnknownParameterSet(
    std::string const& name) const {
  psettable::const_iterator it = psetTable_.find(name);
  if (it == psetTable_.end()) {
    return 0;
...
VParameterSetEntry const* ParameterSet::retrieveUnknownVParameterSet(
    std::string const& name) const {
  vpsettable::const_iterator it = vpsetTable_.find(name);
  if (it == vpsetTable_.end()) {
    return 0;
...
void ParameterSet::insert(bool okay_to_replace, char const* name,
                          Entry const& value) {
  insert(okay_to_replace, std::string(name), value);
}

void ParameterSet::insert(bool okay_to_replace, std::string const& name,
                          Entry const& value) {
  ...
  if (it == tbl_.end()) {
    if (!tbl_.insert(std::make_pair(name, value)).second)
      throw Exception(errors::Configuration, "InsertFailure")
          << "cannot insert " << name << " into a ParameterSet\n";
...
void ParameterSet::augment(ParameterSet const& from) {
  ...
  for (table::const_iterator b = from.tbl_.begin(), e = from.tbl_.end(); b != e;
       ++b) {
    this->insert(false, b->first, b->second);
  }
  for (psettable::const_iterator b = from.psetTable_.begin(),
                                 e = from.psetTable_.end();
       b != e; ++b) {
    this->insertParameterSet(false, b->first, b->second);
  }
  for (vpsettable::const_iterator b = from.vpsetTable_.begin(),
                                  e = from.vpsetTable_.end();
       b != e; ++b) {
...
void ParameterSet::copyFrom(ParameterSet const& from, std::string const& name) {
  ...
  if (from.existsAs<ParameterSet>(name)) {
    this->insertParameterSet(false, name,
                             *(from.retrieveUnknownParameterSet(name)));
  } else if (from.existsAs<std::vector<ParameterSet> >(name)) {
    this->insertVParameterSet(false, name,
                              *(from.retrieveUnknownVParameterSet(name)));
...
VParameterSetEntry* ParameterSet::getPSetVectorForUpdate(
    std::string const& name) {
  assert(!isRegistered());
  vpsettable::iterator it = vpsetTable_.find(name);
  if (it == vpsetTable_.end()) return 0;
...
void ParameterSet::allToString(std::string& rep) const {
...
void ParameterSet::toStringImp(std::string& rep, bool useAll) const {
  ...
  for (psettable::const_iterator b = psetTable_.begin(), e = psetTable_.end();
       b != e; ++b) {
    if (useAll || b->second.isTracked()) {
      size += 2;
      size += b->first.size();
  ...
  for (vpsettable::const_iterator b = vpsetTable_.begin(),
                                  e = vpsetTable_.end();
       b != e; ++b) {
    if (useAll || b->second.isTracked()) {
      size += 2;
...
bool ParameterSet::fromString(std::string const& from) {
  ...
  if (!split(std::back_inserter(temp), from, '<', ';', '>')) return false;
  ...
  for (std::vector<std::string>::const_iterator b = temp.begin(),
    ...
    if (tbl_.find(name) != tbl_.end()) return false;
...
std::vector<FileInPath>::size_type ParameterSet::getAllFileInPaths(
    std::vector<FileInPath>& output) const {
...
std::vector<std::string> ParameterSet::getParameterNames() const {
  ...
  std::transform(
      psetTable_.begin(), psetTable_.end(), back_inserter(returnValue),
      std::bind(&std::pair<std::string const, ParameterSetEntry>::first, _1));
  std::transform(
      vpsetTable_.begin(), vpsetTable_.end(), back_inserter(returnValue),
...
ParameterSet ParameterSet::trackedPart() const {
  ParameterSet result;
  for (table::const_iterator tblItr = tbl_.begin(); tblItr != tbl_.end();
       ++tblItr) {
  ...
  for (psettable::const_iterator psetItr = psetTable_.begin();
    ...
    if (psetItr->second.isTracked()) {
      result.addParameter<ParameterSet>(psetItr->first,
                                        psetItr->second.pset().trackedPart());
    }
  }
...
/*
  ...
  ParameterSet::getAllParameterSetNames(std::vector<std::string>& output) const
  {
    ...
    std::transform(psetTable_.begin(), psetTable_.end(), back_inserter(output),
                   std::bind(&std::pair<std::string const,
  ParameterSetEntry>::first, _1));
    return output.size();
  }
*/
...
size_t ParameterSet::getParameterSetVectorNames(
    std::vector<std::string>& output, bool trackiness) const {
  for (vpsettable::const_iterator vpsetItr = vpsetTable_.begin();
       vpsetItr != vpsetTable_.end(); ++vpsetItr) {
...
size_t ParameterSet::getNamesByCode_(char code, bool trackiness,
...
template <>
std::vector<std::string> ParameterSet::getParameterNamesForType<FileInPath>(
    bool trackiness) const {
...
bool isTransientEqual(ParameterSet const& a, ParameterSet const& b) {
  ...
  for (Ti i = a.tbl().begin(), e = a.tbl().end(), j = b.tbl().begin(),
          f = b.tbl().end();
       i != e && j != f; ++i, ++j) {
    if (*i != *j) {
      return false;
...
std::string ParameterSet::dump(unsigned int indent) const {
  ...
  for (psettable::const_iterator i = psetTable_.begin(), e = psetTable_.end();
       i != e; ++i) {
    // indent a bit
  ...
  for (vpsettable::const_iterator i = vpsetTable_.begin(),
                                  e = vpsetTable_.end();
       i != e; ++i) {
    // indent a bit
    std::string n = i->first;
...
ParameterSet const& getProcessParameterSetContainingModule(
...
template <>
std::vector<int> ParameterSet::getParameter<std::vector<int> >(
    std::string const& name) const {
...
template <>
std::vector<long long> ParameterSet::getParameter<std::vector<long long> >(
    std::string const& name) const {
...
template <>
unsigned int ParameterSet::getParameter<unsigned int>(
    std::string const& name) const {
...
ParameterSet::getParameter<std::vector<unsigned int> >(
    std::string const& name) const {
...
template <>
unsigned long long ParameterSet::getParameter<unsigned long long>(
    std::string const& name) const {
...
ParameterSet::getParameter<std::vector<unsigned long long> >(
    std::string const& name) const {
...
template <>
std::vector<double> ParameterSet::getParameter<std::vector<double> >(
    std::string const& name) const {
...
template <>
std::string ParameterSet::getParameter<std::string>(
    std::string const& name) const {
  return retrieve(name).getString();
}

template <>
std::vector<std::string> ParameterSet::getParameter<std::vector<std::string> >(
    std::string const& name) const {
...
template <>
FileInPath ParameterSet::getParameter<FileInPath>(
    std::string const& name) const {
...
template <>
std::vector<InputTag> ParameterSet::getParameter<std::vector<InputTag> >(
    std::string const& name) const {
...
template <>
ESInputTag ParameterSet::getParameter<ESInputTag>(
    std::string const& name) const {
...
template <>
std::vector<ESInputTag> ParameterSet::getParameter<std::vector<ESInputTag> >(
    std::string const& name) const {
...
template <>
std::vector<EventID> ParameterSet::getParameter<std::vector<EventID> >(
    std::string const& name) const {
...
template <>
LuminosityBlockID ParameterSet::getParameter<LuminosityBlockID>(
    std::string const& name) const {
...
ParameterSet::getParameter<std::vector<LuminosityBlockID> >(
    std::string const& name) const {
...
template <>
EventRange ParameterSet::getParameter<EventRange>(
    std::string const& name) const {
...
template <>
std::vector<EventRange> ParameterSet::getParameter<std::vector<EventRange> >(
    std::string const& name) const {
...
template <>
LuminosityBlockRange ParameterSet::getParameter<LuminosityBlockRange>(
    std::string const& name) const {
...
ParameterSet::getParameter<std::vector<LuminosityBlockRange> >(
    std::string const& name) const {
...
template <>
ParameterSet ParameterSet::getParameter<ParameterSet>(
    std::string const& name) const {
  return getParameterSet(name);
}

template <>
VParameterSet ParameterSet::getParameter<VParameterSet>(
    std::string const& name) const {
...
template <>
bool ParameterSet::getUntrackedParameter<bool>(std::string const& name,
...
template <>
int ParameterSet::getUntrackedParameter<int>(std::string const& name,
...
template <>
std::vector<int> ParameterSet::getUntrackedParameter<std::vector<int> >(
    std::string const& name, std::vector<int> const& defaultValue) const {
  Entry const* entryPtr = retrieveUntracked(name);
...
template <>
unsigned int ParameterSet::getUntrackedParameter<unsigned int>(
    std::string const& name, unsigned int const& defaultValue) const {
  Entry const* entryPtr = retrieveUntracked(name);
...
template <>
std::vector<unsigned int>
ParameterSet::getUntrackedParameter<std::vector<unsigned int> >(
    std::string const& name,
    std::vector<unsigned int> const& defaultValue) const {
  Entry const* entryPtr = retrieveUntracked(name);
...
template <>
std::vector<unsigned long long>
ParameterSet::getUntrackedParameter<std::vector<unsigned long long> >(
    std::string const& name,
    std::vector<unsigned long long> const& defaultValue) const {
  Entry const* entryPtr = retrieveUntracked(name);
...
template <>
long long ParameterSet::getUntrackedParameter<long long>(
    std::string const& name, long long const& defaultValue) const {
  Entry const* entryPtr = retrieveUntracked(name);
...
ParameterSet::getUntrackedParameter<std::vector<long long> >(
...
template <>
double ParameterSet::getUntrackedParameter<double>(
    std::string const& name, double const& defaultValue) const {
  Entry const* entryPtr = retrieveUntracked(name);
...
std::vector<double> ParameterSet::getUntrackedParameter<std::vector<double> >(
  ...
  Entry const* entryPtr = retrieveUntracked(name);
  return entryPtr == 0 ? defaultValue : entryPtr->getVDouble();
}
...
template <>
std::string ParameterSet::getUntrackedParameter<std::string>(
    std::string const& name, std::string const& defaultValue) const {
  Entry const* entryPtr = retrieveUntracked(name);
...
ParameterSet::getUntrackedParameter<std::vector<std::string> >(
    std::string const& name,
    std::vector<std::string> const& defaultValue) const {
  Entry const* entryPtr = retrieveUntracked(name);
...
template <>
FileInPath ParameterSet::getUntrackedParameter<FileInPath>(
    std::string const& name, FileInPath const& defaultValue) const {
  Entry const* entryPtr = retrieveUntracked(name);
...
template <>
InputTag ParameterSet::getUntrackedParameter<InputTag>(
    std::string const& name, InputTag const& defaultValue) const {
  Entry const* entryPtr = retrieveUntracked(name);
...
template <>
std::vector<InputTag>
ParameterSet::getUntrackedParameter<std::vector<InputTag> >(
...
template <>
ESInputTag ParameterSet::getUntrackedParameter<ESInputTag>(
    std::string const& name, ESInputTag const& defaultValue) const {
  Entry const* entryPtr = retrieveUntracked(name);
...
ParameterSet::getUntrackedParameter<std::vector<ESInputTag> >(
    std::string const& name,
...
template <>
EventID ParameterSet::getUntrackedParameter<EventID>(
    std::string const& name, EventID const& defaultValue) const {
  Entry const* entryPtr = retrieveUntracked(name);
...
template <>
std::vector<LuminosityBlockID>
ParameterSet::getUntrackedParameter<std::vector<LuminosityBlockID> >(
    std::string const& name,
...
template <>
EventRange ParameterSet::getUntrackedParameter<EventRange>(
    std::string const& name, EventRange const& defaultValue) const {
  Entry const* entryPtr = retrieveUntracked(name);
...
ParameterSet::getUntrackedParameter<std::vector<EventRange> >(
    std::string const& name,
...
template <>
std::vector<LuminosityBlockRange>
ParameterSet::getUntrackedParameter<std::vector<LuminosityBlockRange> >(
    std::string const& name,
...
template <>
bool ParameterSet::getUntrackedParameter<bool>(char const* name,
                                               bool const& defaultValue) const {
  Entry const* entryPtr = retrieveUntracked(name);
  return entryPtr == 0 ? defaultValue : entryPtr->getBool();
}
...
template <>
int ParameterSet::getUntrackedParameter<int>(char const* name,
                                             int const& defaultValue) const {
  Entry const* entryPtr = retrieveUntracked(name);
  return entryPtr == 0 ? defaultValue : entryPtr->getInt32();
}
...
template <>
std::vector<int> ParameterSet::getUntrackedParameter<std::vector<int> >(
    char const* name, std::vector<int> const& defaultValue) const {
  Entry const* entryPtr = retrieveUntracked(name);
  return entryPtr == 0 ? defaultValue : entryPtr->getVInt32();
}
...
template <>
unsigned int ParameterSet::getUntrackedParameter<unsigned int>(
    char const* name, unsigned int const& defaultValue) const {
  Entry const* entryPtr = retrieveUntracked(name);
  return entryPtr == 0 ? defaultValue : entryPtr->getUInt32();
}
...
template <>
long long ParameterSet::getUntrackedParameter<long long>(
    char const* name, long long const& defaultValue) const {
  Entry const* entryPtr = retrieveUntracked(name);
  return entryPtr == 0 ? defaultValue : entryPtr->getInt64();
}
...
template <>
double ParameterSet::getUntrackedParameter<double>(
    char const* name, double const& defaultValue) const {
  Entry const* entryPtr = retrieveUntracked(name);
  return entryPtr == 0 ? defaultValue : entryPtr->getDouble();
}
...
template <>
std::string ParameterSet::getUntrackedParameter<std::string>(
    char const* name, std::string const& defaultValue) const {
  Entry const* entryPtr = retrieveUntracked(name);
  return entryPtr == 0 ? defaultValue : entryPtr->getString();
}
...
template <>
FileInPath ParameterSet::getUntrackedParameter<FileInPath>(
    char const* name, FileInPath const& defaultValue) const {
  Entry const* entryPtr = retrieveUntracked(name);
  return entryPtr == 0 ? defaultValue : entryPtr->getFileInPath();
}
...
template <>
InputTag ParameterSet::getUntrackedParameter<InputTag>(
    char const* name, InputTag const& defaultValue) const {
  Entry const* entryPtr = retrieveUntracked(name);
  return entryPtr == 0 ? defaultValue : entryPtr->getInputTag();
}
...
template <>
ESInputTag ParameterSet::getUntrackedParameter<ESInputTag>(
    char const* name, ESInputTag const& defaultValue) const {
  Entry const* entryPtr = retrieveUntracked(name);
  return entryPtr == 0 ? defaultValue : entryPtr->getESInputTag();
}
...
template <>
EventID ParameterSet::getUntrackedParameter<EventID>(
    char const* name, EventID const& defaultValue) const {
  Entry const* entryPtr = retrieveUntracked(name);
  return entryPtr == 0 ? defaultValue : entryPtr->getEventID();
}
...
template <>
EventRange ParameterSet::getUntrackedParameter<EventRange>(
    char const* name, EventRange const& defaultValue) const {
  Entry const* entryPtr = retrieveUntracked(name);
  return entryPtr == 0 ? defaultValue : entryPtr->getEventRange();
}
...
template <>
ParameterSet ParameterSet::getUntrackedParameter<ParameterSet>(
    char const* name, ParameterSet const& defaultValue) const {
  return getUntrackedParameterSet(name, defaultValue);
}

template <>
VParameterSet ParameterSet::getUntrackedParameter<VParameterSet>(
    char const* name, VParameterSet const& defaultValue) const {
  return getUntrackedParameterSetVector(name, defaultValue);
}

template <>
ParameterSet ParameterSet::getUntrackedParameter<ParameterSet>(
    std::string const& name, ParameterSet const& defaultValue) const {
  return getUntrackedParameterSet(name, defaultValue);
}

template <>
VParameterSet ParameterSet::getUntrackedParameter<VParameterSet>(
    std::string const& name, VParameterSet const& defaultValue) const {
  return getUntrackedParameterSetVector(name, defaultValue);
}
...
template <>
void ParameterSet::addParameter<ParameterSet>(std::string const& name,
...
template <>
void ParameterSet::addParameter<VParameterSet>(std::string const& name,
...
template <>
void ParameterSet::addParameter<ParameterSet>(char const* name,
                                              ParameterSet const& value) {
  invalidateRegistration(name);
  insertParameterSet(true, name, ParameterSetEntry(value, true));
}
...
template <>
void ParameterSet::addParameter<VParameterSet>(char const* name,
                                               VParameterSet const& value) {
  invalidateRegistration(name);
  insertVParameterSet(true, name, VParameterSetEntry(value, true));
}
...
template <>
void ParameterSet::addUntrackedParameter<ParameterSet>(
    std::string const& name, ParameterSet const& value) {
  insertParameterSet(true, name, ParameterSetEntry(value, false));
}

template <>
void ParameterSet::addUntrackedParameter<VParameterSet>(
    std::string const& name, VParameterSet const& value) {
  insertVParameterSet(true, name, VParameterSetEntry(value, false));
}

template <>
void ParameterSet::addUntrackedParameter<ParameterSet>(
    char const* name, ParameterSet const& value) {
  insertParameterSet(true, name, ParameterSetEntry(value, false));
}

template <>
void ParameterSet::addUntrackedParameter<VParameterSet>(
    char const* name, VParameterSet const& value) {
  insertVParameterSet(true, name, VParameterSetEntry(value, false));
}
...
template <>
std::vector<std::string> ParameterSet::getParameterNamesForType<ParameterSet>(
    bool trackiness) const {
...
template <>
std::vector<std::string> ParameterSet::getParameterNamesForType<VParameterSet>(
    bool trackiness) const {
  std::vector<std::string> output;
...
ParameterSet const& ParameterSet::getParameterSet(
    std::string const& name) const {
...
ParameterSet ParameterSet::getUntrackedParameterSet(
    std::string const& name, ParameterSet const& defaultValue) const {
...
ParameterSet const& ParameterSet::getUntrackedParameterSet(
    std::string const& name) const {
...
VParameterSet const& ParameterSet::getParameterSetVector(
    std::string const& name) const {
...
VParameterSet ParameterSet::getUntrackedParameterSetVector(
    std::string const& name, VParameterSet const& defaultValue) const {
...
VParameterSet const& ParameterSet::getUntrackedParameterSetVector(
    std::string const& name) const {
