...
void ParameterSwitchBase::throwDuplicateCaseValues(
    std::string const& switchLabel) const {
  throw Exception(errors::LogicError)
      << "When adding a ParameterSwitch to a ParameterSetDescription the "
         "values\n"
...
void ParameterSwitchBase::insertAndCheckLabels(
    std::string const& switchLabel, std::set<std::string>& usedLabels,
    std::set<std::string>& labels) const {
  std::pair<std::set<std::string>::iterator, bool> status =
      labels.insert(switchLabel);
  if (status.second == false) {
    throw Exception(errors::LogicError)
        << "The label used for the switch parameter in a "
           "ParameterSetDescription\n"
        << "must be different from the labels used in the associated cases.  "
           "The following\n"
...
void ParameterSwitchBase::insertAndCheckTypes(
    ParameterTypes switchType,
  ...
  if (caseWildcardTypes.find(switchType) != caseWildcardTypes.end()) {
    throw Exception(errors::LogicError)
        << "The type used for the switch parameter in a "
           "ParameterSetDescription\n"
        << "must be different from the types used for wildcards in the "
           "associated cases.  The following\n"
        << "duplicate type was found: \""
        << parameterTypeEnumToString(switchType) << "\"\n";
  }
...
void ParameterSwitchBase::throwNoCaseForDefault(
    std::string const& switchLabel) const {
  throw Exception(errors::LogicError)
      << "The default value used for the switch parameter in a "
         "ParameterSetDescription\n"
      << "must match the value used to select one of the associated cases.  "
         "This is not\n"
      << "true for the switch named \"" << switchLabel << "\"\n";
}

void ParameterSwitchBase::throwNoCaseForSwitchValue(
    std::string const& message) const {
  throw Exception(errors::Configuration) << message;
}

void ParameterSwitchBase::printBase(std::ostream& os, bool optional,
                                    bool writeToCfi, DocFormatHelper& dfh,
...
void ParameterSwitchBase::printNestedContentBase(
    std::ostream& os, DocFormatHelper& dfh, DocFormatHelper& new_dfh,
    std::string const& switchLabel) const {
...
void ParameterSwitchBase::printCase(
    std::pair<bool, value_ptr<ParameterDescriptionNode> > const& p,
...
void ParameterSwitchBase::printCase(
    std::pair<int, value_ptr<ParameterDescriptionNode> > const& p,
    std::ostream& os, bool /*optional*/, DocFormatHelper& dfh,
    std::string const& switchLabel) {
  if (dfh.pass() == 0) {
    p.second->print(os, false, true, dfh);
...
void ParameterSwitchBase::printCase(
    std::pair<std::string, value_ptr<ParameterDescriptionNode> > const& p,
    std::ostream& os, bool /*optional*/, DocFormatHelper& dfh,
    std::string const& switchLabel) {
  if (dfh.pass() == 0) {
    p.second->print(os, false, true, dfh);
...
int ParameterSwitchBase::howManyXORSubNodesExist_(
