...
ORGroupDescription::ORGroupDescription(
...
void ORGroupDescription::writeCfi_(std::ostream& os, bool& startWithComma,
                                   int indentation,
                                   bool& wroteSomething) const {
  node_left_->writeCfi(os, startWithComma, indentation, wroteSomething);
}

void ORGroupDescription::print_(std::ostream& os, bool optional,
...
void ORGroupDescription::printNestedContent_(std::ostream& os, bool optional,
  ...
  if (optional) {
    ...
    os << "This optional OR group requires at least one or none of the "
          "following to be in the PSet\n";
  } else {
    os << "This OR group requires at least one of the following to be in the "
          "PSet\n";
  }
  if (!dfh.brief()) os << "\n";
...
bool ORGroupDescription::partiallyExists_(ParameterSet const& pset) const {
  return exists(pset);
}

int ORGroupDescription::howManyXORSubNodesExist_(
    ParameterSet const& pset) const {
  return exists(pset) ? 1 : 0;
}

void ORGroupDescription::throwIfDuplicateLabels(
...
void ORGroupDescription::throwIfDuplicateTypes(
  ...
  if (!types1.empty()) {
    ...
    if (!duplicateTypes.empty()) {
      ...
      throw edm::Exception(errors::LogicError)
          ...
          << "\"or\" expression must be different from types used for other "
             "parameters\n"
