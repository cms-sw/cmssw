...
TypeTrans::TypeTrans() : table_(255) {
  ...
  for (CodeMap::const_iterator itCode = table_.begin(),
                               itCodeEnd = table_.end();
       itCode != itCodeEnd; ++itCode) {
...
Entry::Entry(std::string const& name, std::vector<int> const& val,
             bool is_tracked)
    : name_(name), rep(), type('i'), tracked(is_tracked ? '+' : '-') {
...
Entry::Entry(std::string const& name, std::vector<unsigned> const& val,
             bool is_tracked)
    : name_(name), rep(), type('u'), tracked(is_tracked ? '+' : '-') {
...
Entry::Entry(std::string const& name, std::vector<long long> const& val,
             bool is_tracked)
    : name_(name), rep(), type('l'), tracked(is_tracked ? '+' : '-') {
...
Entry::Entry(std::string const& name,
             std::vector<unsigned long long> const& val, bool is_tracked)
    : name_(name), rep(), type('x'), tracked(is_tracked ? '+' : '-') {
...
Entry::Entry(std::string const& name, std::vector<double> const& val,
             bool is_tracked)
    : name_(name), rep(), type('d'), tracked(is_tracked ? '+' : '-') {
...
Entry::Entry(std::string const& name, std::vector<std::string> const& val,
             bool is_tracked)
    : name_(name), rep(), type('s'), tracked(is_tracked ? '+' : '-') {
...
Entry::Entry(std::string const& name, std::vector<InputTag> const& val,
             bool is_tracked)
...
Entry::Entry(std::string const& name, std::vector<ESInputTag> const& val,
             bool is_tracked)
...
Entry::Entry(std::string const& name, std::vector<EventID> const& val,
             bool is_tracked)
    : name_(name), rep(), type('e'), tracked(is_tracked ? '+' : '-') {
...
Entry::Entry(std::string const& name, LuminosityBlockID const& val,
             bool is_tracked)
...
Entry::Entry(std::string const& name, std::vector<LuminosityBlockID> const& val,
             bool is_tracked)
    : name_(name), rep(), type('m'), tracked(is_tracked ? '+' : '-') {
...
Entry::Entry(std::string const& name, LuminosityBlockRange const& val,
             bool is_tracked)
...
Entry::Entry(std::string const& name,
             std::vector<LuminosityBlockRange> const& val, bool is_tracked)
    : name_(name), rep(), type('a'), tracked(is_tracked ? '+' : '-') {
...
Entry::Entry(std::string const& name, std::vector<EventRange> const& val,
             bool is_tracked)
    : name_(name), rep(), type('r'), tracked(is_tracked ? '+' : '-') {
...
Entry::Entry(std::string const& name, std::vector<ParameterSet> const& val,
             bool is_tracked)
    : name_(name), rep(), type('p'), tracked(is_tracked ? '+' : '-') {
...
Entry::Entry(std::string const& name, std::string const& code)
  ...
  if (!fromString(code.begin(), code.end())) throwEncodeError("coded string");
  validate();
}
...
bool Entry::fromString(std::string::const_iterator const b,
                       std::string::const_iterator const e) {
...
std::ostream& operator<<(std::ostream& os, Entry const& entry) {
  ...
  switch (entry.typeCode()) {
    ...
    case 'v': {
      ...
      // vector<string> rather than using the individual encodings of each
      // InputTag
      os << "{";
      std::string start = "";
      std::string const between(",");
    ...
    case kTVESInputTag: {
      ...
      for (std::vector<ESInputTag>::const_iterator it = tags.begin(),
                                                   itEnd = tags.end();
           it != itEnd; ++it) {
        os << start << it->encode();
        start = between;
...
void Entry::throwEntryError(char const* expectedType,
  ...
  throw Exception(errors::Configuration, "EntryError")
      << "can not convert representation of " << name_ << ": " << badRep
