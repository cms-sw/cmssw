...
----------------------------------------------------------------------*/
//------------------------------------------------------------
// Adapts parameter sets by value (fileFormatVersion_.value() < 12) to parameter
// sets by reference
// Adapts untracked @trigger_paths (fileFormatVersion_.value() < 13) to tracked
// @trigger_paths.
...
void insertIntoReplace(ParameterSetConverter::StringMap& replace,
                       std::string const& fromPrefix, std::string const& from,
                       std::string const& fromPostfix,
                       std::string const& toPrefix, std::string const& to,
                       std::string const& toPostfix) {
  replace.insert(std::make_pair(fromPrefix + from + fromPostfix,
                                toPrefix + to + toPostfix));
}
}

MainParameterSet::MainParameterSet(ParameterSetID const& oldID,
  ...
  for (StringVector::const_iterator i = paths_.begin(), e = paths_.end();
       i != e; ++i) {
...
TriggerPath::TriggerPath(ParameterSet const& pset)
    : parameterSet_(pset),
      ...
      triggerPaths_() {
  for (StringVector::const_iterator i = tPaths_.begin(), e = tPaths_.end();
       i != e; ++i) {
    triggerPaths_.insert(*i);
  }
}
...
ParameterSetConverter::ParameterSetConverter(
    ParameterSetMap const& psetMap, ParameterSetIdConverter& idConverter,
  ...
  for (ParameterSetMap::const_iterator i = psetMap.begin(),
                                       iEnd = psetMap.end();
       i != iEnd; ++i) {
  ...
  for (std::vector<MainParameterSet>::iterator j = mainParameterSets_.begin(),
                                               jEnd = mainParameterSets_.end();
       j != jEnd; ++j) {
    for (std::vector<TriggerPath>::iterator i = triggerPaths_.begin(),
                                            iEnd = triggerPaths_.end();
         i != iEnd; ++i) {
  ...
  for (std::vector<MainParameterSet>::iterator i = mainParameterSets_.begin(),
                                               iEnd = mainParameterSets_.end();
       i != iEnd; ++i) {
...
void ParameterSetConverter::noConvertParameterSets() {
  for (StringWithIDList::iterator i = parameterSets_.begin(),
                                  iEnd = parameterSets_.end();
       i != iEnd; ++i) {
...
void ParameterSetConverter::convertParameterSets() {
  ...
  for (StringMap::const_iterator j = replace_.begin(), jEnd = replace_.end();
       j != jEnd; ++j) {
    for (StringWithIDList::iterator i = parameterSets_.begin(),
                                    iEnd = parameterSets_.end();
         i != iEnd; ++i) {
      for (std::string::size_type it = i->first.find(j->first);
           it != std::string::npos; it = i->first.find(j->first)) {
  ...
  for (StringWithIDList::iterator i = parameterSets_.begin(),
                                  iEnd = parameterSets_.end();
       i != iEnd; ++i) {
    if (i->first.find("+P") == std::string::npos &&
        i->first.find("+p") == std::string::npos) {
      if (i->first.find("@all_sources") != std::string::npos) {
        mainParameterSets_.push_back(MainParameterSet(i->second, i->first));
      } else {
        ParameterSet pset(i->first);
        ...
        ParameterSetID newID(pset.id());
        newID.toString(to);
        insertIntoReplace(replace_, loldparam, from, rparam, lparam, to,
                          rparam);
        insertIntoReplace(replace_, comma, from, comma, comma, to, comma);
        insertIntoReplace(replace_, comma, from, rvparam, comma, to, rvparam);
        insertIntoReplace(replace_, loldvparam, from, comma, lvparam, to,
                          comma);
        insertIntoReplace(replace_, loldvparam, from, rvparam, lvparam, to,
                          rvparam);
  ...
  if (!doItAgain && !parameterSets_.empty()) {
    for (StringWithIDList::iterator i = parameterSets_.begin(),
      ...
      for (std::list<std::string>::iterator i = pieces.begin(),
                                            e = pieces.end();
           i != e; ++i) {
        std::string removeName = i->substr(i->find('+'));
        if (removeName.size() >= 4) {
          if (removeName[1] == 'P') {
            std::string psetString(removeName.begin() + 3,
                                   removeName.end() - 1);
            parameterSets_.push_back(
                std::make_pair(psetString, ParameterSetID()));
            doItAgain = true;
          } else if (removeName[1] == 'p') {
            std::string pvec =
                std::string(removeName.begin() + 3, removeName.end() - 1);
            StringList temp;
            split(std::back_inserter(temp), pvec, '{', ',', '}');
            for (StringList::const_iterator j = temp.begin(), f = temp.end();
                 j != f; ++j) {
