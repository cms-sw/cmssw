...
ParameterSetDescription::ParameterSetDescription()
    : anythingAllowed_(false), unknown_(false) {}
...
ParameterDescriptionNode* ParameterSetDescription::addNode(
...
ParameterDescriptionNode* ParameterSetDescription::addOptionalNode(
...
ParameterDescriptionNode* ParameterSetDescription::addNode(
  ...
  node->checkAndGetLabelsAndTypes(nodeLabels, nodeParameterTypes,
                                  nodeWildcardTypes);
...
void ParameterSetDescription::validate(ParameterSet& pset) const {
  ...
  for_all(entries_, std::bind(&ParameterSetDescription::validateNode, _1,
                              std::ref(pset), std::ref(validatedLabels)));
...
void ParameterSetDescription::writeCfi(std::ostream& os, bool startWithComma,
                                       int indentation) const {
  using std::placeholders::_1;
  bool wroteSomething = false;

  for_all(entries_, std::bind(&ParameterSetDescription::writeNode, _1,
                              std::ref(os), std::ref(startWithComma),
                              indentation, std::ref(wroteSomething)));
...
void ParameterSetDescription::validateNode(
    SetDescriptionEntry const& entry, ParameterSet& pset,
    std::set<std::string>& validatedLabels) {
...
void ParameterSetDescription::print(std::ostream& os,
  ...
  if (isUnknown()) {
    dfh.indent(os);
    os << "Description is unknown.  The configured PSet will not be "
          "validated\n";
    dfh.indent(os);
    os << "because the plugin has not defined this parameter set "
          "description.\n";
    if (!dfh.brief()) os << "\n";
  }
  ...
  if (anythingAllowed()) {
    dfh.indent(os);
    os << "Description allows anything. If the configured PSet contains "
          "illegal parameters,\n";
    dfh.indent(os);
    os << "then validation will ignore them instead of throwing an "
          "exception.\n";
    if (!dfh.brief()) os << "\n";
  }
  ...
  dfh.setPass(0);
  dfh.setCounter(0);
  for_all(entries_, std::bind(&ParameterSetDescription::printNode, _1,
                              std::ref(os), std::ref(dfh)));
...
void ParameterSetDescription::throwIllegalParameters(
  ...
  std::insert_iterator<std::set<std::string> > insertIter(diffNames,
                                                          diffNames.begin());
  std::set_difference(parNames.begin(), parNames.end(), validatedLabels.begin(),
                      validatedLabels.end(), insertIter);
  ...
  if (diffNames.size() == 1U) {
    throw edm::Exception(errors::Configuration)
        << "Illegal parameter found in configuration.  The parameter is "
           "named:\n"
        << ss.str()
    ...
    throw edm::Exception(errors::Configuration)
        << "Illegal parameters found in configuration.  The parameters are "
           "named:\n"
        << ss.str()
...
void ParameterSetDescription::writeNode(SetDescriptionEntry const& entry,
                                        std::ostream& os, bool& startWithComma,
                                        int indentation, bool& wroteSomething) {
  if (entry.writeToCfi()) {
...
void ParameterSetDescription::throwIfLabelsAlreadyUsed(
    std::set<std::string> const& nodeLabels) {
  ...
  std::set_intersection(nodeLabels.begin(), nodeLabels.end(),
                        usedLabels_.begin(), usedLabels_.end(), insertIter);
...
void ParameterSetDescription::throwIfWildcardCollision(
  ...
  if (!nodeWildcardTypes.empty()) {
    ...
    std::insert_iterator<std::set<ParameterTypes> > insertIter1(
        duplicateTypes1, duplicateTypes1.begin());
    std::set_intersection(
        typesUsedForParameters_.begin(), typesUsedForParameters_.end(),
        nodeWildcardTypes.begin(), nodeWildcardTypes.end(), insertIter1);
    ...
    if (!duplicateTypes1.empty()) {
      ...
      throw edm::Exception(errors::LogicError)
          << "Within a ParameterSetDescription, the type used for a wildcard "
             "must\n"
          << "not be the same as the type used for other parameters. This "
             "rule\n"
  ...
  if (!typesUsedForWildcards_.empty()) {
    ...
    std::insert_iterator<std::set<ParameterTypes> > insertIter2(
        duplicateTypes2, duplicateTypes2.begin());
    std::set_intersection(
        typesUsedForWildcards_.begin(), typesUsedForWildcards_.end(),
        nodeParameterTypes.begin(), nodeParameterTypes.end(), insertIter2);
  ...
  typesUsedForParameters_.insert(nodeParameterTypes.begin(),
                                 nodeParameterTypes.end());
  typesUsedForWildcards_.insert(nodeWildcardTypes.begin(),
                                nodeWildcardTypes.end());
}

ParameterDescriptionNode* ParameterSetDescription::ifExists(
    ParameterDescriptionNode const& node1,
    ParameterDescriptionNode const& node2, bool optional, bool writeToCfi) {
  std::unique_ptr<ParameterDescriptionNode> pdIfExists =
      std::make_unique<IfExistsDescription>(node1, node2);
