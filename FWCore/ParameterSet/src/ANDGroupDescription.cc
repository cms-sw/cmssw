...
ANDGroupDescription::ANDGroupDescription(
    ParameterDescriptionNode const& node_left,
    ParameterDescriptionNode const& node_right)
    : node_left_(node_left.clone()), node_right_(node_right.clone()) {}

ANDGroupDescription::ANDGroupDescription(
    std::unique_ptr<ParameterDescriptionNode> node_left,
    ParameterDescriptionNode const& node_right)
...
void ANDGroupDescription::checkAndGetLabelsAndTypes_(
    std::set<std::string>& usedLabels, std::set<ParameterTypes>& parameterTypes,
    std::set<ParameterTypes>& wildcardTypes) const {
  std::set<std::string> labelsLeft;
  std::set<ParameterTypes> parameterTypesLeft;
  std::set<ParameterTypes> wildcardTypesLeft;
  node_left_->checkAndGetLabelsAndTypes(labelsLeft, parameterTypesLeft,
                                        wildcardTypesLeft);

  std::set<std::string> labelsRight;
  std::set<ParameterTypes> parameterTypesRight;
  std::set<ParameterTypes> wildcardTypesRight;
  node_right_->checkAndGetLabelsAndTypes(labelsRight, parameterTypesRight,
                                         wildcardTypesRight);
...
void ANDGroupDescription::writeCfi_(std::ostream& os, bool& startWithComma,
                                    int indentation,
...
void ANDGroupDescription::print_(std::ostream& os, bool optional,
                                 bool writeToCfi, DocFormatHelper& dfh) const {
  ...
  if (dfh.pass() == 1) {
    ...
    // not brief
      ...
      if (!comment().empty()) {
        DocFormatHelper::wrapAndPrintText(os, comment(), dfh.startColumn2(),
                                          dfh.commentWidth());
      }
      os << "\n";
    }
...
void ANDGroupDescription::printNestedContent_(std::ostream& os, bool optional,
  ...
  printSpaces(os, indentation);
  if (optional) {
    os << "This optional AND group requires all or none of the following to be "
          "in the PSet\n";
  } else {
...
bool ANDGroupDescription::partiallyExists_(ParameterSet const& pset) const {
  return node_left_->partiallyExists(pset) ||
         node_right_->partiallyExists(pset);
}

int ANDGroupDescription::howManyXORSubNodesExist_(
    ParameterSet const& pset) const {
  return exists(pset) ? 1 : 0;
}

void ANDGroupDescription::throwIfDuplicateLabels(
    std::set<std::string> const& labelsLeft,
    std::set<std::string> const& labelsRight) const {
  std::set<std::string> duplicateLabels;
  std::insert_iterator<std::set<std::string> > insertIter(
      duplicateLabels, duplicateLabels.begin());
  std::set_intersection(labelsLeft.begin(), labelsLeft.end(),
                        labelsRight.begin(), labelsRight.end(), insertIter);
...
void ANDGroupDescription::throwIfDuplicateTypes(
    std::set<ParameterTypes> const& types1,
    std::set<ParameterTypes> const& types2) const {
  if (!types1.empty()) {
    std::set<ParameterTypes> duplicateTypes;
    std::insert_iterator<std::set<ParameterTypes> > insertIter(
        duplicateTypes, duplicateTypes.begin());
    std::set_intersection(types1.begin(), types1.end(), types2.begin(),
                          types2.end(), insertIter);
    if (!duplicateTypes.empty()) {
      std::stringstream ss;
      for (std::set<ParameterTypes>::const_iterator
      ...
      throw edm::Exception(errors::LogicError)
          << "Types used for wildcards in different nodes of a "
             "ParameterSetDescription\n"
          << "\"and\" expression must be different from types used for other "
             "parameters.\n"
