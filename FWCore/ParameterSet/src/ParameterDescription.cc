...
namespace edm {
  ...
  ParameterDescription<ParameterSetDescription>::ParameterDescription(std::string const& iLabel,
                                                                      ...
                                                                      bool isTracked, Comment const& iComment)
  ...
  void ParameterDescription<ParameterSetDescription>::printDefault_(std::ostream& os, bool writeToCfi,
  ...
  void ParameterDescription<ParameterSetDescription>::printNestedContent_(std::ostream& os, bool /*optional*/,
                                                                          DocFormatHelper& dfh) const {
  ...
  void ParameterDescription<std::vector<ParameterSet> >::printDefault_(std::ostream& os, bool writeToCfi,
                                                                       DocFormatHelper& dfh) const {
    os << "see Section " << dfh.section() << "." << dfh.counter();
    if (!writeToCfi) {
      os << " (do not write to cfi)";
  ...
  void ParameterDescription<std::vector<ParameterSet> >::printNestedContent_(std::ostream& os, bool /*optional*/,
                                                                             DocFormatHelper& dfh) const {
    int indentation = dfh.indentation();
    if (dfh.parent() != DocFormatHelper::TOP) {
      indentation -= DocFormatHelper::offsetSectionContent();
  ...
  void ParameterDescription<std::vector<ParameterSet> >::writeOneElementToCfi(ParameterSet const& pset,
                                                                              std::ostream& os, int indentation,
  ...
  void ParameterDescription<std::vector<ParameterSet> >::writeCfi_(std::ostream& os, int indentation) const {
    ...
    for_all(vPset_, std::bind(&writeOneElementToCfi, std::placeholders::_1, std::ref(os), indentation,
  ...
  namespace writeParameterValue {
    ...
    template <>
    void writeSingleValue<LuminosityBlockRange>(std::ostream& os, LuminosityBlockRange const& value,
                                                ValueFormat format) {
      if (format == CFI) {
    ...
    template <typename T>
    void writeValueInVectorWithSpace(T const& value, std::ostream& os, int indentation, bool& startWithComma,
    ...
    void writeVector(std::ostream& os, int indentation, std::vector<T> const& value_, ValueFormat format) {
      ...
      if (value_.empty() && format == DOC) {
        ...
        bool startWithComma = false;
        int i = 0;
        for_all(value_, std::bind(&writeValueInVectorWithSpace<T>, std::placeholders::_1, std::ref(os), indentation + 2,
                                  std::ref(startWithComma), format, std::ref(i)));
        if (format == CFI) {
          os << "\n" << std::setw(indentation) << "";
        }
        if (format == CFI and value_.size() > 255U) {
    ...
    void writeValue(std::ostream& os, int indentation, std::vector<unsigned long long> const& value_,
                    ValueFormat format) {
    ...
    void writeValue(std::ostream& os, int indentation, std::vector<LuminosityBlockRange> const& value_,
                    ValueFormat format) {
