...
ParameterDescription<ParameterSetDescription>::ParameterDescription(
    std::string const& iLabel, ParameterSetDescription const& value,
    bool isTracked)
    : ParameterDescriptionBase(iLabel, k_PSet, isTracked, true),
      psetDesc_(new ParameterSetDescription(value)) {}

ParameterDescription<ParameterSetDescription>::ParameterDescription(
    char const* iLabel, ParameterSetDescription const& value, bool isTracked)
    : ParameterDescriptionBase(iLabel, k_PSet, isTracked, true),
      psetDesc_(new ParameterSetDescription(value)) {}

ParameterDescription<ParameterSetDescription>::~ParameterDescription() {}

void ParameterDescription<ParameterSetDescription>::validate_(
    ParameterSet& pset, std::set<std::string>& validatedLabels,
    bool optional) const {
...
void ParameterDescription<ParameterSetDescription>::printDefault_(
    std::ostream& os, bool writeToCfi, DocFormatHelper& dfh) const {
  os << "see Section " << dfh.section() << "." << dfh.counter();
  if (!writeToCfi) os << " (do not write to cfi)";
  os << "\n";
}

bool ParameterDescription<ParameterSetDescription>::hasNestedContent_() const {
  return true;
}

void ParameterDescription<ParameterSetDescription>::printNestedContent_(
    std::ostream& os, bool /*optional*/, DocFormatHelper& dfh) const {
  ...
  if (dfh.parent() == DocFormatHelper::TOP) {
    new_dfh.setIndentation(indentation +
                           DocFormatHelper::offsetSectionContent());
  }
  psetDesc_->print(os, new_dfh);
}

bool ParameterDescription<ParameterSetDescription>::exists_(
    ParameterSet const& pset) const {
  return pset.existsAs<ParameterSet>(label(), isTracked());
}

ParameterSetDescription const*
ParameterDescription<ParameterSetDescription>::parameterSetDescription() const {
  return psetDesc_.operator->();
}

ParameterSetDescription*
ParameterDescription<ParameterSetDescription>::parameterSetDescription() {
  return psetDesc_.operator->();
}

void ParameterDescription<ParameterSetDescription>::writeCfi_(
    std::ostream& os, int indentation) const {
  bool startWithComma = false;
  indentation += 2;
  psetDesc_->writeCfi(os, startWithComma, indentation);
}

void ParameterDescription<ParameterSetDescription>::writeDoc_(
    std::ostream&, int /*indentation*/) const {}

// These next two should not be needed for this specialization
bool ParameterDescription<ParameterSetDescription>::exists_(
    ParameterSet const&, bool /*isTracked*/) const {
  throw Exception(errors::LogicError);
  return true;
}

void ParameterDescription<ParameterSetDescription>::insertDefault_(
    ParameterSet&) const {
  throw Exception(errors::LogicError);
  return;
}
...
ParameterDescription<std::vector<ParameterSet> >::ParameterDescription(
    std::string const& iLabel, ParameterSetDescription const& psetDesc,
...
ParameterDescription<std::vector<ParameterSet> >::ParameterDescription(
    char const* iLabel, ParameterSetDescription const& psetDesc, bool isTracked,
...
ParameterSetDescription const* ParameterDescription<
    std::vector<ParameterSet> >::parameterSetDescription() const {
  return psetDesc_.operator->();
}

ParameterSetDescription*
ParameterDescription<std::vector<ParameterSet> >::parameterSetDescription() {
  return psetDesc_.operator->();
}

void ParameterDescription<std::vector<ParameterSet> >::validate_(
    ParameterSet& pset, std::set<std::string>& validatedLabels,
    bool optional) const {
...
void ParameterDescription<std::vector<ParameterSet> >::printDefault_(
    std::ostream& os, bool writeToCfi, DocFormatHelper& dfh) const {
  os << "see Section " << dfh.section() << "." << dfh.counter();
  if (!writeToCfi) os << " (do not write to cfi)";
  os << "\n";
...
bool ParameterDescription<std::vector<ParameterSet> >::hasNestedContent_()
    const {
  return true;
}

void ParameterDescription<std::vector<ParameterSet> >::printNestedContent_(
    std::ostream& os, bool /*optional*/, DocFormatHelper& dfh) const {
  int indentation = dfh.indentation();
  if (dfh.parent() != DocFormatHelper::TOP) {
    indentation -= DocFormatHelper::offsetSectionContent();
  ...
  if (!partOfDefaultOfVPSet_) {
    printSpaces(os, indentation);
    os << "Section " << dfh.section() << "." << dfh.counter() << " " << label()
       << " VPSet description:\n";

    printSpaces(os, indentation + DocFormatHelper::offsetSectionContent());
    os << "All elements will be validated using the PSet description in "
          "Section "
       << dfh.section() << "." << dfh.counter() << ".1.\n";
  } else {
    printSpaces(os, indentation);
    os << "Section " << dfh.section() << "." << dfh.counter() << " "
       << " VPSet description for VPSet that is part of the default of a "
          "containing VPSet:\n";
  }
...
bool ParameterDescription<std::vector<ParameterSet> >::exists_(
    ParameterSet const& pset) const {
...
void ParameterDescription<std::vector<ParameterSet> >::writeOneElementToCfi(
    ParameterSet const& pset, std::ostream& os, int indentation,
...
void ParameterDescription<std::vector<ParameterSet> >::writeCfi_(
    std::ostream& os, int indentation) const {
  bool nextOneStartsWithAComma = false;
  for_all(vPset_,
          std::bind(&writeOneElementToCfi, std::placeholders::_1, std::ref(os),
                    indentation, std::ref(nextOneStartsWithAComma)));
  os << "\n";
  printSpaces(os, indentation);
}

void ParameterDescription<std::vector<ParameterSet> >::writeDoc_(
    std::ostream&, int /*indentation*/) const {}
...
void ParameterDescription<std::vector<ParameterSet> >::insertDefault_(
    ParameterSet&) const {
  throw Exception(errors::LogicError);
  return;
}
...
template <>
void writeSingleValue<EventID>(std::ostream& os, EventID const& value,
                               ValueFormat format) {
  if (format == CFI) {
...
template <>
void writeSingleValue<LuminosityBlockID>(std::ostream& os,
...
template <>
void writeSingleValue<EventRange>(std::ostream& os, EventRange const& value,
                                  ValueFormat format) {
  if (value.startLumi() == 0U) {
    if (format == CFI)
      os << "'" << value.startRun() << ":" << value.startEvent() << "-"
    ...
    if (format == CFI)
      os << "'" << value.startRun() << ":" << value.startLumi() << ":"
         << value.startEvent() << "-" << value.endRun() << ":"
...
void writeSingleValue<LuminosityBlockRange>(std::ostream& os,
  ...
  if (format == CFI)
    ...
    os << value.startRun() << ":" << value.startLumi() << "-" << value.endRun()
       << ":" << value.endLumi();
}

template <>
void writeSingleValue<InputTag>(std::ostream& os, InputTag const& value,
                                ValueFormat format) {
  if (format == CFI) {
    os << "'" << value.label() << "'";
...
template <>
void writeSingleValue<FileInPath>(std::ostream& os, FileInPath const& value,
                                  ValueFormat) {
...
template <>
void writeValueInVector<EventID>(std::ostream& os, EventID const& value,
                                 ValueFormat format) {
  if (value.luminosityBlock() == 0U) {
    if (format == CFI)
      os << "'" << value.run() << ":" << value.event() << "'";
...
template <>
void writeValueInVector<LuminosityBlockID>(std::ostream& os,
                                           LuminosityBlockID const& value,
                                           ValueFormat format) {
  if (format == CFI)
    os << "'" << value.run() << ":" << value.luminosityBlock() << "'";
...
template <>
void writeValueInVector<InputTag>(std::ostream& os, InputTag const& value,
                                  ValueFormat) {
  os << "'" << value.label();
  if (!value.instance().empty() || !value.process().empty()) {
    os << ":" << value.instance();
...
template <typename T>
void writeValueInVectorWithSpace(T const& value, std::ostream& os,
                                 int indentation, bool& startWithComma,
                                 ValueFormat format, int& i) {
...
template <typename T>
void writeVector(std::ostream& os, int indentation,
                 std::vector<T> const& value_, ValueFormat format) {
  std::ios_base::fmtflags ff = os.flags(std::ios_base::dec);
  ...
  if (value_.size() == 0U && format == DOC) {
    ...
    int i = 0;
    for_all(value_,
            std::bind(&writeValueInVectorWithSpace<T>, std::placeholders::_1,
                      std::ref(os), indentation + 2, std::ref(startWithComma),
                      format, std::ref(i)));
    if (format == CFI) os << "\n" << std::setw(indentation) << "";
    if (format == CFI and value_.size() > 255U) os << ") ";
  }
...
void writeValue(std::ostream& os, int indentation,
                std::vector<int> const& value_, ValueFormat format) {
  writeVector<int>(os, indentation, value_, format);
}

void writeValue(std::ostream& os, int, unsigned const& value_,
                ValueFormat format) {
  writeValue<unsigned>(os, value_, format);
}

void writeValue(std::ostream& os, int indentation,
                std::vector<unsigned> const& value_, ValueFormat format) {
  writeVector<unsigned>(os, indentation, value_, format);
}

void writeValue(std::ostream& os, int, long long const& value_,
                ValueFormat format) {
  writeValue<long long>(os, value_, format);
}

void writeValue(std::ostream& os, int indentation,
                std::vector<long long> const& value_, ValueFormat format) {
  writeVector<long long>(os, indentation, value_, format);
}

void writeValue(std::ostream& os, int, unsigned long long const& value_,
                ValueFormat format) {
...
void writeValue(std::ostream& os, int, double const& value_,
                ValueFormat format) {
  writeValue<double>(os, value_, format);
}

void writeValue(std::ostream& os, int indentation,
                std::vector<double> const& value_, ValueFormat format) {
...
void writeValue(std::ostream& os, int, std::string const& value_,
                ValueFormat format) {
  writeValue<std::string>(os, value_, format);
}

void writeValue(std::ostream& os, int indentation,
                std::vector<std::string> const& value_, ValueFormat format) {
  writeVector<std::string>(os, indentation, value_, format);
}

void writeValue(std::ostream& os, int, EventID const& value_,
                ValueFormat format) {
  writeValue<EventID>(os, value_, format);
}

void writeValue(std::ostream& os, int indentation,
                std::vector<EventID> const& value_, ValueFormat format) {
  writeVector<EventID>(os, indentation, value_, format);
}

void writeValue(std::ostream& os, int, LuminosityBlockID const& value_,
                ValueFormat format) {
...
void writeValue(std::ostream& os, int, LuminosityBlockRange const& value_,
                ValueFormat format) {
...
void writeValue(std::ostream& os, int, EventRange const& value_,
                ValueFormat format) {
  writeValue<EventRange>(os, value_, format);
}

void writeValue(std::ostream& os, int indentation,
                std::vector<EventRange> const& value_, ValueFormat format) {
  writeVector<EventRange>(os, indentation, value_, format);
}

void writeValue(std::ostream& os, int, InputTag const& value_,
                ValueFormat format) {
  writeValue<InputTag>(os, value_, format);
}

void writeValue(std::ostream& os, int indentation,
                std::vector<InputTag> const& value_, ValueFormat format) {
  writeVector<InputTag>(os, indentation, value_, format);
}

void writeValue(std::ostream& os, int, FileInPath const& value_,
                ValueFormat format) {
