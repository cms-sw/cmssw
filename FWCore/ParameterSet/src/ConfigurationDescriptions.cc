...
namespace edm {
  ...
  void ConfigurationDescriptions::writeCfis(std::set<std::string>& usedCfiFileNames) const {
    ...
    CfiOptions ops = wroteClassFile ? CfiOptions{cfi::LabelValue{paths}} : CfiOptions{cfi::Full{}};
  ...
  cfi::Paths ConfigurationDescriptions::writeClassFile(ParameterSetDescription const& iDesc,
    ...
    int indentation = 4;
    CfiOptions ops = willUseWithCfis ? CfiOptions{cfi::Default{}} : CfiOptions{cfi::Full{}};
    ...
    outFile.close();
    return std::holds_alternative<cfi::Default>(ops) ? std::get<cfi::Default>(ops).releasePaths() : cfi::Paths{};
  }
