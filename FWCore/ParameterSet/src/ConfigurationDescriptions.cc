...
namespace {
void matchLabel(
    std::pair<std::string, edm::ParameterSetDescription> const& thePair,
...
void ConfigurationDescriptions::add(
    char const* label, ParameterSetDescription const& psetDescription) {
  std::string labelString(label);
  add(labelString, psetDescription);
}

void ConfigurationDescriptions::add(
    std::string const& label, ParameterSetDescription const& psetDescription) {
  if (0 == strcmp(baseType_.c_str(), kSource)) {
    if (0 != strcmp(label.c_str(), k_source)) {
      throw edm::Exception(edm::errors::LogicError,
                           "ConfigurationDescriptions::add, when adding a "
                           "ParameterSetDescription for a source the label "
                           "must be \"source\"\n");
    }
    if (descriptions_.size() != 0U || defaultDescDefined_ == true) {
      throw edm::Exception(edm::errors::LogicError,
                           "ConfigurationDescriptions::add, for a source only "
                           "1 ParameterSetDescription may be added\n");
    }
  } else if (0 == strcmp(baseType_.c_str(), kService)) {
    if (descriptions_.size() != 0U || defaultDescDefined_ == true) {
      throw edm::Exception(edm::errors::LogicError,
                           "ConfigurationDescriptions::add, for a service only "
                           "1 ParameterSetDescription may be added\n");
    }
  }
...
void ConfigurationDescriptions::addDefault(
    ParameterSetDescription const& psetDescription) {
  if (0 == strcmp(baseType_.c_str(), kSource) ||
      0 == strcmp(baseType_.c_str(), kService)) {
    if (descriptions_.size() != 0U || defaultDescDefined_ == true) {
      throw edm::Exception(edm::errors::LogicError,
                           "ConfigurationDescriptions::addDefault, for a "
                           "source or service only 1 ParameterSetDescription "
                           "may be added\n");
    }
  }
...
void ConfigurationDescriptions::validate(ParameterSet& pset,
  ...
  for_all(descriptions_, std::bind(&matchLabel, std::placeholders::_1,
                                   std::cref(moduleLabel), std::ref(psetDesc)));
...
void ConfigurationDescriptions::writeCfis(std::string const& baseType,
  ...
  for_all(descriptions_, std::bind(&ConfigurationDescriptions::writeCfiForLabel,
                                   std::placeholders::_1, std::cref(baseType),
                                   std::cref(pluginName)));
}
...
void ConfigurationDescriptions::writeCfiForLabel(
  ...
  if (0 == strcmp(baseType.c_str(), kService) &&
      labelAndDesc.first != pluginName) {
    throw edm::Exception(edm::errors::LogicError,
                         "ConfigurationDescriptions::writeCfiForLabel\nFor a "
                         "service the label and the plugin name must be the "
                         "same.\n")
        << "This error probably is caused by an incorrect label being "
           "passed\nto the ConfigurationDescriptions::add function earlier.\n"
        << "plugin name = \"" << pluginName << "\"  label name = \""
        << labelAndDesc.first << "\"\n";
  }
...
void ConfigurationDescriptions::print(std::ostream& os,
                                      std::string const& moduleLabel,
                                      bool brief, bool printOnlyLabels,
                                      size_t lineWidth, int indentation,
                                      int iPlugin) const {
  if (!brief) {
    if (!comment().empty()) {
  ...
  if (descriptions_.empty() && defaultDescDefined_ &&
    ...
    os << std::setfill(' ') << std::setw(indentation) << "";
    os << "Its PSets will not be validated, and no cfi files will be "
          "generated.\n";
    os << std::setfill(oldFill);
    if (!brief) os << "\n";
    return;
    ...
    if (defaultDescDefined_) {
      if (descriptions_.empty()) {
        ss << "This plugin has only one PSet description. "
           ...
           << "Because this configuration has no label, no cfi files will be "
              "generated.";
      } else {
        ss << "This plugin has " << (descriptions_.size() + 1U)
           << " PSet descriptions. "
           << "The description used to validate a configuration is selected by "
           << "matching the module labels. If none match, then the last "
              "description, "
           << "which has no label, is selected. "
           << "A cfi file will be generated for each configuration with a "
              "module label.";
      }
    } else {
      if (descriptions_.size() == 1U) {
        ss << "This plugin has " << descriptions_.size()
           << " PSet description. "
           << "This description is always used to validate configurations. "
        ...
        ss << "This plugin has " << descriptions_.size()
           << " PSet descriptions. "
           << "The description used to validate a configuration is selected by "
           << "matching the module labels. If none match the first description "
              "below is used. "
           << "The module labels below are also used when generating the cfi "
              "files.";
      }
    }
  ...
  for_all(
      descriptions_,
      boost::bind(&ConfigurationDescriptions::printForLabel, this, _1,
                  std::ref(os), std::cref(moduleLabel), brief, printOnlyLabels,
                  lineWidth, indentation, std::ref(counter)));

  if (defaultDescDefined_) {
    printForLabel(os, std::string("@default"), defaultDesc_, moduleLabel, brief,
                  printOnlyLabels, lineWidth, indentation, counter);
  }
}
...
void ConfigurationDescriptions::printForLabel(
  ...
  printForLabel(os, labelAndDesc.first, labelAndDesc.second, moduleLabel, brief,
                printOnlyLabels, lineWidth, indentation, counter);
}

void ConfigurationDescriptions::printForLabel(
    std::ostream& os, std::string const& label,
    ParameterSetDescription const& description, std::string const& moduleLabel,
    bool brief, bool printOnlyLabels, size_t lineWidth, int indentation,
    DescriptionCounter& counter) const {
  ++counter.iModule;
