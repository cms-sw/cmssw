...
VParameterSetEntry::VParameterSetEntry()
    : tracked_(false), theVPSet_(), theIDs_() {}

VParameterSetEntry::VParameterSetEntry(std::vector<ParameterSet> const& vpset,
                                       bool isTracked)
    : tracked_(isTracked), theVPSet_(new std::vector<ParameterSet>), theIDs_() {
  for (std::vector<ParameterSet>::const_iterator i = vpset.begin(),
                                                 e = vpset.end();
       i != e; ++i) {
...
void VParameterSetEntry::toString(std::string& result) const {
  ...
  for (std::vector<ParameterSetID>::const_iterator i = theIDs_->begin(),
                                                   e = theIDs_->end();
       i != e; ++i) {
    result += start;
    i->toString(result);
...
void VParameterSetEntry::registerPsetsAndUpdateIDs() {
  fillVPSet();
  theIDs_ =
      value_ptr<std::vector<ParameterSetID>>(new std::vector<ParameterSetID>);
  theIDs_->resize(theVPSet_->size());
  for (std::vector<ParameterSet>::iterator i = theVPSet_->begin(),
                                           e = theVPSet_->end();
       i != e; ++i) {
    if (!i->isRegistered()) {
      i->registerIt();
...
std::string VParameterSetEntry::dump(unsigned int indent) const {
  ...
  os << "VPSet " << (isTracked() ? "tracked" : "untracked") << " = ({"
     << std::endl;
  std::string start;
  std::string const between(",\n");
  for (std::vector<ParameterSet>::const_iterator i = vps.begin(), e = vps.end();
       i != e; ++i) {
