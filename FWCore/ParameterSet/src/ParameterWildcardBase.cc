...
ParameterWildcardBase::ParameterWildcardBase(
    ParameterTypes iType, bool isTracked, WildcardValidationCriteria criteria)
...
void ParameterWildcardBase::throwIfInvalidPattern(
    std::string const& pattern) const {
  if (pattern != std::string("*")) {
    throw Exception(errors::Configuration)
        << "Currently, the only supported wildcard in "
           "ParameterSetDescriptions\n"
        << "is the single character \"*\".  The configuration contains a "
           "wildcard\n"
        << "with pattern \"" << pattern << "\" and type \""
        << parameterTypeEnumToString(type()) << "\"\n"
        << "At some future date, globbing or regular expression support may be "
           "added\n"
        << "if there are any requests for it from users.\n";
  }
}

void ParameterWildcardBase::validateMatchingNames(
    std::vector<std::string> const& matchingNames,
  ...
  if (criteria_ == RequireAtLeastOne && matchingNames.size() < 1U &&
    ...
    if ((matchingNames.size() < 1U && !optional) || matchingNames.size() > 1U) {
      throw Exception(errors::Configuration)
          << "Parameter wildcard of type \""
          << parameterTypeEnumToString(type()) << "\" requires\n"
...
void ParameterWildcardBase::print_(std::ostream& os, bool optional,
                                   bool /*writeToCfi*/,
...
int ParameterWildcardBase::howManyXORSubNodesExist_(
