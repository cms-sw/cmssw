...
AllowedLabelsDescriptionBase::AllowedLabelsDescriptionBase(
    std::string const& label, ParameterTypes iType, bool isTracked)
...
void AllowedLabelsDescriptionBase::checkAndGetLabelsAndTypes_(
  ...
  parameterHoldingLabels_.checkAndGetLabelsAndTypes(usedLabels, parameterTypes,
                                                    wildcardTypes);
}

void AllowedLabelsDescriptionBase::validate_(
  ...
  if (parameterHoldingLabels_.exists(pset)) {
    ...
    if (isTracked()) {
      allowedLabels = pset.getParameter<std::vector<std::string> >(
          parameterHoldingLabels_.label());
    } else {
      allowedLabels = pset.getUntrackedParameter<std::vector<std::string> >(
          parameterHoldingLabels_.label());
    }
...
void AllowedLabelsDescriptionBase::writeCfi_(std::ostream& os,
  ...
  parameterHoldingLabels_.writeCfi(os, startWithComma, indentation,
                                   wroteSomething);
}

void AllowedLabelsDescriptionBase::print_(std::ostream& os, bool optional,
                                          bool writeToCfi,
...
void AllowedLabelsDescriptionBase::printNestedContent_(
    std::ostream& os, bool optional, DocFormatHelper& dfh) const {
  printNestedContentBase_(os, optional, dfh);
  if (!dfh.brief()) os << "\n";
}

void AllowedLabelsDescriptionBase::printNestedContentBase_(
    std::ostream& os, bool optional, DocFormatHelper& dfh) const {
...
bool AllowedLabelsDescriptionBase::partiallyExists_(
    ParameterSet const& pset) const {
  return exists(pset);
}

int AllowedLabelsDescriptionBase::howManyXORSubNodesExist_(
    ParameterSet const& pset) const {
