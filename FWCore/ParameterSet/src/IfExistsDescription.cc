...
IfExistsDescription::IfExistsDescription(
...
void IfExistsDescription::checkAndGetLabelsAndTypes_(
...
void IfExistsDescription::writeCfi_(std::ostream& os, bool& startWithComma,
                                    int indentation,
...
void IfExistsDescription::print_(std::ostream& os, bool optional,
...
void IfExistsDescription::printNestedContent_(std::ostream& os, bool optional,
  ...
  printSpaces(os, indentation);
  if (optional) {
    os << "If the first parameter exists, then the second is allowed to "
          "exist\n";
  } else {
    os << "If the first parameter exists, then the second is required to "
          "exist\n";
  }
  if (!dfh.brief()) os << "\n";
...
bool IfExistsDescription::partiallyExists_(ParameterSet const& pset) const {
  return exists(pset);
}

int IfExistsDescription::howManyXORSubNodesExist_(
    ParameterSet const& pset) const {
  return exists(pset) ? 1 : 0;
}

void IfExistsDescription::throwIfDuplicateLabels(
...
void IfExistsDescription::throwIfDuplicateTypes(
  ...
  if (!types1.empty()) {
    ...
    if (!duplicateTypes.empty()) {
      ...
      throw edm::Exception(errors::LogicError)
          ...
          << "\"ifExists\" expression must be different from types used for "
             "other parameters\n"
