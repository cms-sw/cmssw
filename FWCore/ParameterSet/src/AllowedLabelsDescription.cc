...
AllowedLabelsDescription<ParameterSetDescription>::AllowedLabelsDescription(
    char const* label, bool isTracked)
    : AllowedLabelsDescriptionBase(label, k_PSet, isTracked), psetDesc_() {}

AllowedLabelsDescription<ParameterSetDescription>::AllowedLabelsDescription(
    std::string const& label, ParameterSetDescription const& value,
    bool isTracked)
...
AllowedLabelsDescription<ParameterSetDescription>::clone() const {
...
ParameterDescriptionNode*
AllowedLabelsDescription<std::vector<ParameterSet> >::clone() const {
  return new AllowedLabelsDescription(*this);
}
...
void AllowedLabelsDescription<std::vector<ParameterSet> >::
    validateAllowedLabel_(std::string const& allowedLabel, ParameterSet& pset,
                          std::set<std::string>& validatedLabels) const {
  if (pset.existsAs<std::vector<ParameterSet> >(allowedLabel, isTracked())) {
    ...
    if (psetDesc_) {
      VParameterSetEntry* vpsetEntry =
          pset.getPSetVectorForUpdate(allowedLabel);
      assert(vpsetEntry);
