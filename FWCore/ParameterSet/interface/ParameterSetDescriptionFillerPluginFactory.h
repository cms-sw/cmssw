...
/**\class ParameterSetDescriptionFillerPluginFactory
 ...
 Usage:
    The Factory allows loading of plugins and querying them for their
...
namespace edm {
typedef edmplugin::PluginFactory<ParameterSetDescriptionFillerBase*(void)>
    ParameterSetDescriptionFillerPluginFactory;
}

#define DEFINE_FWK_PSET_DESC_FILLER(type)                               \
  static const edm::ParameterSetDescriptionFillerPluginFactory::PMaker< \
      edm::ParameterSetDescriptionFiller<type> >                        \
      EDM_PLUGIN_SYM(s_filler, __LINE__)(#type)
// NOTE: Can't do the below since this appears on the same line as another
// factory and w'ed have two variables with the same name
// DEFINE_EDM_PLUGIN
// (edm::ParameterSetDescriptionFillerPluginFactory,type,#type)
