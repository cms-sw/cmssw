...
class ParameterDescriptionNode {
 public:
  ...
  void validate(ParameterSet& pset, std::set<std::string>& validatedLabels,
                bool optional) const {
  ...
  void writeCfi(std::ostream& os, bool& startWithComma, int indentation,
                bool& wroteSomething) const {
    writeCfi_(os, startWithComma, indentation, wroteSomething);
  }

  // Print out the description in human readable format
  void print(std::ostream& os, bool optional, bool writeToCfi,
             DocFormatHelper& dfh) const;

  bool hasNestedContent() const { return hasNestedContent_(); }

  void printNestedContent(std::ostream& os, bool optional,
                          DocFormatHelper& dfh) const;
  ...
  avoid that problem.
      ...
      we check for collisions.  The function that builds the
  ...
  would tend to confuse a user trying to configure a module or a module
  developer
  ...
  */
  void checkAndGetLabelsAndTypes(
 ...
 protected:
  ...
  virtual void writeCfi_(std::ostream& os, bool& startWithComma,
                         int indentation, bool& wroteSomething) const = 0;

  virtual void print_(std::ostream&, bool /*optional*/, bool /*writeToCfi*/,
                      DocFormatHelper&) const {}

  virtual bool hasNestedContent_() const { return false; }

  virtual void printNestedContent_(std::ostream&, bool /*optional*/,
                                   DocFormatHelper&) const {}
...
std::unique_ptr<ParameterDescriptionCases<bool> > operator>>(
    bool caseValue, ParameterDescriptionNode const& node);

std::unique_ptr<ParameterDescriptionCases<int> > operator>>(
    int caseValue, ParameterDescriptionNode const& node);

std::unique_ptr<ParameterDescriptionCases<std::string> > operator>>(
    std::string const& caseValue, ParameterDescriptionNode const& node);

std::unique_ptr<ParameterDescriptionCases<std::string> > operator>>(
    char const* caseValue, ParameterDescriptionNode const& node);

std::unique_ptr<ParameterDescriptionCases<bool> > operator>>(
    bool caseValue, std::unique_ptr<ParameterDescriptionNode> node);

std::unique_ptr<ParameterDescriptionCases<int> > operator>>(
    int caseValue, std::unique_ptr<ParameterDescriptionNode> node);

std::unique_ptr<ParameterDescriptionCases<std::string> > operator>>(
    std::string const& caseValue,
...
std::unique_ptr<ParameterDescriptionNode> operator&&(
