...
//
/**\class ParameterSetDescription ParameterSetDescription.h
 ...
 Implementation Details:
    ...
    details.  This class holds a container full of nodes.  One node can
 represent
...
class ParameterSetDescription {
 public:
  class SetDescriptionEntry {
   public:
    ...
    ParameterDescriptionNode* setNode(
  ...
  // This is set only for parameterizables which have not set their
  // descriptions.
  ...
  template <typename U>
  ParameterDescriptionBase* addVPSetOptional(
      U const& iLabel, ParameterSetDescription const& validator,
      std::vector<ParameterSet> const& defaults) {
  ...
  template <typename U>
  ParameterDescriptionBase* addVPSetOptionalUntracked(
      U const& iLabel, ParameterSetDescription const& validator,
      std::vector<ParameterSet> const& defaults) {
  ...
  ParameterDescriptionNode* addNode(
      std::unique_ptr<ParameterDescriptionNode> node);
  ParameterDescriptionNode* addOptionalNode(
      ParameterDescriptionNode const& node, bool writeToCfi);
  ParameterDescriptionNode* addOptionalNode(
      std::unique_ptr<ParameterDescriptionNode> node, bool writeToCfi);
  ...
  template <typename T>
  ParameterDescriptionNode* ifValue(
      ParameterDescription<T> const& switchParameter,
      std::unique_ptr<ParameterDescriptionCases<T>> cases) {
    return ifValue<T>(switchParameter, std::move(cases), false, true);
  }

  template <typename T>
  ParameterDescriptionNode* ifValueOptional(
      ParameterDescription<T> const& switchParameter,
      std::unique_ptr<ParameterDescriptionCases<T>> cases, bool writeToCfi) {
  ...
  ParameterDescriptionNode* ifExistsOptional(
      ParameterDescriptionNode const& node1,
      ParameterDescriptionNode const& node2, bool writeToCfi) {
  ...
  template <typename T, typename U>
  ParameterDescriptionNode* labelsFromOptional(U const& iLabel,
                                               bool writeToCfi) {
    return labelsFrom<T, U>(iLabel, true, true, writeToCfi);
  }

  template <typename T, typename U>
  ParameterDescriptionNode* labelsFromOptionalUntracked(U const& iLabel,
                                                        bool writeToCfi) {
 ...
 private:
  template <typename T, typename U>
  ParameterDescriptionBase* add(U const& iLabel, T const& value, bool isTracked,
                                bool isOptional, bool writeToCfi);

  template <typename T, typename U>
  ParameterDescriptionBase* add(U const& iLabel, bool isTracked,
                                bool isOptional, bool writeToCfi);
  ...
  ParameterDescriptionBase* addVPSet(U const& iLabel,
                                     ParameterSetDescription const& validator,
                                     std::vector<ParameterSet> const& defaults,
                                     bool isTracked, bool isOptional,
                                     bool writeToCfi);
  ...
  ParameterDescriptionNode* addNode(
      std::unique_ptr<ParameterDescriptionNode> node, bool optional,
      bool writeToCfi);

  template <typename T>
  ParameterDescriptionNode* ifValue(
      ParameterDescription<T> const& switchParameter,
      std::unique_ptr<ParameterDescriptionCases<T>> cases, bool optional,
      bool writeToCfi);
  ...
  template <typename T, typename U>
  ParameterDescriptionNode* labelsFrom(U const& iLabel, bool isTracked,
                                       bool optional, bool writeToCfi);
  ...
  static void validateNode(SetDescriptionEntry const& entry, ParameterSet& pset,
                           std::set<std::string>& validatedNames);

  static void throwIllegalParameters(
      std::vector<std::string> const& parameterNames,
      std::set<std::string> const& validatedNames);

  static void writeNode(SetDescriptionEntry const& entry, std::ostream& os,
                        bool& startWithComma, int indentation,
                        bool& wroteSomething);

  static void printNode(SetDescriptionEntry const& entry, std::ostream& os,
                        DocFormatHelper& dfh);

  void throwIfLabelsAlreadyUsed(std::set<std::string> const& nodeLabels);
  void throwIfWildcardCollision(
      std::set<ParameterTypes> const& nodeParameterTypes,
...
ParameterDescriptionBase* ParameterSetDescription::add(U const& iLabel,
  ...
  ParameterDescriptionNode* pnode =
      addNode(std::move(node), isOptional, writeToCfi);
...
ParameterDescriptionBase* ParameterSetDescription::add(U const& iLabel,
                                                       ...
                                                       bool writeToCfi) {
  std::unique_ptr<ParameterDescriptionNode> node =
      std::make_unique<ParameterDescription<T>>(iLabel, isTracked);
  ParameterDescriptionNode* pnode =
      addNode(std::move(node), isOptional, writeToCfi);
  return static_cast<ParameterDescriptionBase*>(pnode);
}
...
ParameterDescriptionBase* ParameterSetDescription::addVPSet(
    U const& iLabel, ParameterSetDescription const& validator,
    std::vector<ParameterSet> const& defaults, bool isTracked, bool isOptional,
    bool writeToCfi) {
  std::unique_ptr<ParameterDescriptionNode> node =
      std::make_unique<ParameterDescription<std::vector<ParameterSet>>>(
          iLabel, validator, isTracked, defaults);
  ParameterDescriptionNode* pnode =
      addNode(std::move(node), isOptional, writeToCfi);
  return static_cast<ParameterDescriptionBase*>(pnode);
}
...
template <typename U>
ParameterDescriptionBase* ParameterSetDescription::addVPSet(
    U const& iLabel, ParameterSetDescription const& validator, bool isTracked,
    bool isOptional, bool writeToCfi) {
  std::unique_ptr<ParameterDescriptionNode> node =
      std::make_unique<ParameterDescription<std::vector<ParameterSet>>>(
          iLabel, validator, isTracked);
  ParameterDescriptionNode* pnode =
      addNode(std::move(node), isOptional, writeToCfi);
  return static_cast<ParameterDescriptionBase*>(pnode);
}
...
ParameterWildcardBase* ParameterSetDescription::addWildcard(U const& pattern,
                                                            bool isTracked) {
  std::unique_ptr<ParameterDescriptionNode> node =
      std::make_unique<ParameterWildcard<T>>(pattern, RequireZeroOrMore,
                                             isTracked);
...
template <typename T>
ParameterDescriptionNode* ParameterSetDescription::ifValue(
    ParameterDescription<T> const& switchParameter,
    std::unique_ptr<ParameterDescriptionCases<T>> cases, bool optional,
    bool writeToCfi) {
...
ParameterDescriptionNode* ParameterSetDescription::labelsFrom(U const& iLabel,
  ...
  std::unique_ptr<ParameterDescriptionNode> pd =
      std::make_unique<AllowedLabelsDescription<T>>(iLabel, isTracked);
