...
class ParameterSwitch : public ParameterSwitchBase {
 public:
  ...
  typedef typename std::map<T, edm::value_ptr<ParameterDescriptionNode> >::
      const_iterator CaseMapConstIter;
 ...
 private:
  virtual void checkAndGetLabelsAndTypes_(
    ...
    for_all(cases_,
            std::bind(&ParameterSwitch::checkCaseLabels, std::placeholders::_1,
                      std::ref(caseLabels), std::ref(caseParameterTypes),
                      std::ref(caseWildcardTypes)));

    insertAndCheckLabels(switch_.label(), usedLabels, caseLabels);

    insertAndCheckTypes(switch_.type(), caseParameterTypes, caseWildcardTypes,
                        parameterTypes, wildcardTypes);
  ...
  virtual void validate_(ParameterSet& pset,
    ...
    if (switch_.exists(pset)) {
      ...
      if (selectedCase != cases_.end()) {
        ...
        std::stringstream ss;
        ss << "The switch parameter with label \"" << switch_.label()
  ...
  virtual void writeCfi_(std::ostream& os, bool& startWithComma,
    ...
    typename CaseMap::const_iterator selectedCase =
        cases_.find(switch_.getDefaultValue());
    if (selectedCase != cases_.end()) {
      selectedCase->second->writeCfi(os, startWithComma, indentation,
                                     wroteSomething);
    }
  }
  ...
  virtual void print_(std::ostream& os, bool optional, bool writeToCfi,
    ...
    printBase(os, optional, writeToCfi, dfh, switch_.label(),
  ...
  virtual void printNestedContent_(std::ostream& os, bool optional,
    ...
    for_all(cases_, std::bind(&ParameterSwitchBase::printCaseT<T>,
                              std::placeholders::_1, std::ref(os), optional,
                              std::ref(new_dfh), std::cref(switch_.label())));
  ...
  static void checkCaseLabels(
      std::pair<T, edm::value_ptr<ParameterDescriptionNode> > const& thePair,
      std::set<std::string>& labels, std::set<ParameterTypes>& parameterTypes,
      std::set<ParameterTypes>& wildcardTypes) {
    thePair.second->checkAndGetLabelsAndTypes(labels, parameterTypes,
                                              wildcardTypes);
  }
