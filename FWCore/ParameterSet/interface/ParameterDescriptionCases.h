...
//                                  0 >> ParameterDescription<int>("label1", 11)
//                                  or
//                                  1 >> ParameterDescription<float>("label2",
//                                  11.0f) or
//                                  2 >>
//                                  ParameterDescription<std::string>("label3",
//                                  "aValue");
...
class ParameterDescriptionCases {
 public:
  ...
  void insert(T caseValue, std::unique_ptr<ParameterDescriptionNode> node) {
    std::pair<T, edm::value_ptr<ParameterDescriptionNode> > casePair(
        caseValue, edm::value_ptr<ParameterDescriptionNode>());
 ...
 private:
  friend std::unique_ptr<ParameterDescriptionCases<bool> > operator>>(
      bool caseValue, std::unique_ptr<ParameterDescriptionNode> node);

  friend std::unique_ptr<ParameterDescriptionCases<int> > operator>>(
  ...
  // The constructor is intentionally private so that only the operator>>
  // functions
  // can create these.
  ParameterDescriptionCases(T const& caseValue,
                            std::unique_ptr<ParameterDescriptionNode> node)
