...
void testps::uintTest() {
  ...
  testbody<std::vector<unsigned int> >(
      std::vector<unsigned int>(1, std::numeric_limits<unsigned int>::max()));
}
...
void testps::stringTest() {
  ...
  std::vector<std::string> vs2 =
      p1.getParameter<std::vector<std::string> >("vs");
...
void testps::eventIDTest() {
  ...
  testbody<edm::EventID>(edm::EventID(edm::EventID::maxRunNumber(),
...
void testps::vEventRangeTest() {
  ...
  testbody<std::vector<edm::EventRange> >(std::vector<edm::EventRange>(
      1, edm::EventRange(1, 1, 1, edm::EventID::maxRunNumber(),
                         edm::EventID::maxLuminosityBlockNumber(),
                         edm::EventID::maxEventNumber())));

  testbody<std::vector<edm::EventRange> >(std::vector<edm::EventRange>(
      1, edm::EventRange(1, 0, 1, edm::EventID::maxRunNumber(), 0,
                         edm::EventID::maxEventNumber())));
...
void test_for_name() {
  ...
  // The following causes failure, because of an apparent GCC bug in comparing
  // bools!
  // T value;
...
void testps::testEmbeddedPSet() {
  ...
  CPPUNIT_ASSERT(trackedPart.getParameterSet("psEmbedded")
                     .getParameterSet("psDeeper")
                     .getParameter<int>("deepest") == 6);
  CPPUNIT_ASSERT(ps.getUntrackedParameter<unsigned long long>("u64") == 64);
  CPPUNIT_ASSERT(!trackedPart.exists("u64"));
  std::vector<edm::ParameterSet> const& vpset1 =
      trackedPart.getParameterSetVector("psVPset");
