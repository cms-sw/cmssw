...
void testDesc(edm::ParameterDescriptionNode const& node,
              ...
              edm::ParameterSet& pset, bool exists, bool validates) {
...
void testWildcards() {
...

  {
    edm::ParameterSetDescription set;
    edm::ParameterWildcard<edm::ParameterSetDescription> w(
        std::string("*"), edm::RequireZeroOrMore, true);
    set.addNode(w);
    edm::ParameterSet pset;
    testDesc(w, set, pset, true, true);
    pset.addParameter<edm::ParameterSet>("nested1", nestedPset);
...

  {
    edm::ParameterSetDescription set;
    edm::ParameterWildcard<edm::ParameterSetDescription> w(
        "*", edm::RequireZeroOrMore, true, nestedDesc);
    set.addNode(w);
    edm::ParameterSet pset;
    testDesc(w, set, pset, true, true);
    pset.addParameter<edm::ParameterSet>("nested1", nestedPset);
...

  {
    edm::ParameterSetDescription set;
    edm::ParameterWildcard<edm::ParameterSetDescription> w(
        "*", edm::RequireAtLeastOne, true, nestedDesc);
    set.addNode(w);
    edm::ParameterSet pset;
    testDesc(w, set, pset, false, false);
    pset.addParameter<edm::ParameterSet>("nested1", nestedPset);
  ...
  descElement.addUntracked<unsigned>("n11", 1);
  {
    edm::ParameterSetDescription set;
    edm::ParameterWildcard<std::vector<edm::ParameterSet>> w(
        "*", edm::RequireZeroOrMore, true, descElement);
    set.addNode(w);
    edm::ParameterSet pset;
    testDesc(w, set, pset, true, true);
    pset.addParameter<std::vector<edm::ParameterSet>>("nested1", nestedVPset);
...

  {
    edm::ParameterSetDescription set;
    edm::ParameterWildcard<std::vector<edm::ParameterSet>> w(
        "*", edm::RequireAtLeastOne, true, descElement);
    set.addNode(w);
    edm::ParameterSet pset;
    testDesc(w, set, pset, false, false);
    pset.addParameter<std::vector<edm::ParameterSet>>("nested1", nestedVPset);
...
void testSwitch() {
  ...
  try {
    psetDesc.ifValue(
        edm::ParameterDescription<int>("oiswitch", 1, true),
        0 >> edm::ParameterDescription<int>("oivalue", 100, true) or
            ...
            1 >>
                edm::ParameterDescription<std::string>("oivalue", "102", true));
    assert(0);
  } catch (edm::Exception const&) { /* There should be an exception */
  }

  // Types used in case parameters cannot duplicate type already used in a
  // wildcard
  ...
  // Types used in the switch parameter cannot duplicate type already used in a
  // wildcard
  ...
  try {
    psetDesc2.ifValue(
        edm::ParameterDescription<int>("aswitch", 1, true),
  ...
  try {
    psetDesc3.ifValue(
        edm::ParameterDescription<int>("xswitch", 1, true),
  ...
  try {
    psetDesc4.ifValue(
        edm::ParameterDescription<int>("xswitch", 1, true),
        0 >> edm::ParameterWildcard<unsigned>("*", edm::RequireAtLeastOne,
                                              true) or
            1 >> (edm::ParameterDescription<double>("xvalue1", 101.0, true) and
                  edm::ParameterDescription<double>("xvalue2", 101.0, true)));
    assert(0);
  ...
  // No problem is wildcard type and parameter type are the same for different
  // cases.
  edm::ParameterSetDescription psetDesc5;
  psetDesc5.ifValue(
      edm::ParameterDescription<int>("uswitch", 1, true),
      0 >> edm::ParameterWildcard<unsigned>("*", edm::RequireAtLeastOne,
                                            true) or
          1 >> (edm::ParameterDescription<unsigned>("uvalue1", 101, true) and
                edm::ParameterDescription<unsigned>("uvalue2", 101, true)));

  // The switch parameter label cannot be the same as a label that already
  // exists
  ...
  try {
    psetDesc6.ifValue(
        edm::ParameterDescription<int>("xswitch", 1, true),
        0 >> edm::ParameterWildcard<double>("*", edm::RequireAtLeastOne,
                                            true) or
            1 >> (edm::ParameterDescription<double>("xvalue1", 101.0, true) and
                  edm::ParameterDescription<double>("xvalue2", 101.0, true)));
    assert(0);
  ...
  edm::ParameterSwitch<int> pswitch(
      edm::ParameterDescription<int>("xswitch", 1, true),
...
void testAllowedLabels() {
  edm::ParameterSetDescription psetDesc1;
  std::unique_ptr<edm::ParameterDescriptionNode> node1(
      std::make_unique<edm::AllowedLabelsDescription<int>>("allowedLabels",
                                                           true));
  ...
  edm::ParameterSetDescription psetDesc5;
  psetDesc5.labelsFrom<edm::ParameterSetDescription>(
      "allowedLabelsA", edm::ParameterSetDescription());
  ...
  psetDesc6.labelsFrom<edm::ParameterSetDescription>("allowedLabelsA",
                                                     psetDesc7);
  ...
  edm::ParameterSetDescription psetDesc9;
  edm::ParameterSetDescription psetDesc10;
  psetDesc8.labelsFrom<edm::ParameterSetDescription>(
      std::string("allowedLabelsA"));
  psetDesc9.labelsFrom<edm::ParameterSetDescription>(
      std::string("allowedLabelsA"), edm::ParameterSetDescription());
  psetDesc10.labelsFrom<edm::ParameterSetDescription>(
      std::string("allowedLabelsA"), psetDesc7);
  ...
  // necessary to test the vector<ParameterSet> case
  {
    ...
    psetDesc15.labelsFrom<std::vector<edm::ParameterSet>>(
        "allowedLabelsC", edm::ParameterSetDescription());
    // Now it should fail because the description says the contained
    // vector<ParameterSet>
    ...
    // Now include "y" in the description and it should once again pass
    // validation
    edm::ParameterSetDescription psetDesc6;
    edm::ParameterSetDescription psetDesc7;
    psetDesc7.add<int>("y", 1);

    psetDesc6.labelsFrom<std::vector<edm::ParameterSet>>("allowedLabelsC",
                                                         psetDesc7);
    ...
    psetDesc18.labelsFrom<std::vector<edm::ParameterSet>>(
        std::string("allowedLabelsC"));
    psetDesc19.labelsFrom<std::vector<edm::ParameterSet>>(
        std::string("allowedLabelsC"), edm::ParameterSetDescription());
    psetDesc20.labelsFrom<std::vector<edm::ParameterSet>>(
        std::string("allowedLabelsC"), psetDesc7);
...
void testWrongTrackiness() {
  ...
  edm::ParameterSetDescription psetDesc2;
  psetDesc2.addUntracked<edm::ParameterSetDescription>(
      "test2", edm::ParameterSetDescription());
  try {
  ...
  edm::ParameterSet pset3;
  pset3.addParameter<std::vector<edm::ParameterSet>>(
      "test3", std::vector<edm::ParameterSet>());
...
void testWrongType() {
  ...
  edm::ParameterSetDescription psetDesc2;
  psetDesc2.add<edm::ParameterSetDescription>("test2",
...
int main(int, char**) try {
  std::cout << "Running TestFWCoreParameterSetDescription from "
...

  {
    ...
    // at a real file.
    edm::ParameterSetDescription psetDesc;
    edm::ParameterDescriptionBase* par =
        psetDesc.add<edm::FileInPath>("fileInPath", edm::FileInPath());
    assert(par->type() == edm::k_FileInPath);
    assert(edm::parameterTypeEnumToString(par->type()) ==
  ...
  int a = 1;
  edm::ParameterDescriptionBase* par =
      psetDesc.add<int>(std::string("ivalue"), a);
  ...
  pset.addUntrackedParameter<std::vector<edm::ParameterSet>>("nestLevel0",
                                                             vPset);
