...
//	 A command SHUT_UP to deactivate, and in the LOG_A_MESSGE case, response
//to
//	 that command.  This allows supression of the generator info in case of
//a
...
void MessageLoggerScribe::runCommand(  // changeLog 32
    MessageLoggerQ::OpCode opcode, void* operand) {
  switch (opcode) {  // interpret the work item
    ...
    case MessageLoggerQ::CONFIGURE: {  // changelog 17
      if (singleThread) {
        job_pset_p = std::shared_ptr<PSet>(static_cast<PSet*>(
            operand));  // propagate_const<T> has no reset() function
        configure_errorlog();
        break;
      } else {
        ...
        job_pset_p = std::shared_ptr<PSet>(static_cast<PSet*>(
            h_p->p));  // propagate_const<T> has no reset() function
...
void MessageLoggerScribe::configure_errorlog() {
  ...
  String preconfiguration_message = getAparameter<String>(
...
void MessageLoggerScribe::configure_dest(
    std::shared_ptr<ELdestination> dest_ctrl, String const& filename) {
  ...
  for (vString::const_iterator id_it = categories.begin();
    ...
    int interval = getAparameter<int>(category_pset, "reportEvery",
                                      category_default_interval);
    ...
    int timespan = getAparameter<int>(category_pset, "timespan",
                                      category_default_timespan);
...
void MessageLoggerScribe::configure_ordinary_destinations()  // Changelog 16
{
  ...
  for (vString::const_iterator it = destinations.begin();
    ...
    if (filename_default == empty_String) {
      filename_default =
          messageLoggerDefaults->output(psetname);  // change log 31
      if (filename_default == empty_String) {
...
void MessageLoggerScribe::configure_statistics() {
  ...
  for (vString::const_iterator it = statistics.begin(); it != statistics.end();
       ++it) {
    String statname = *it;
...
void MessageLoggerScribe::parseCategories(std::string const& s,
...
void MessageLoggerScribe::triggerFJRmessageSummary(
    std::map<std::string, double>& sm)  // ChangeLog 29
{
