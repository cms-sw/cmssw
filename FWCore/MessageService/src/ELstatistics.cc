...
ELstring ELstatistics::formSummary(ELmap_stats& stats) {
  ...
  for (ELmap_stats::const_iterator i = stats.begin(); i != stats.end(); ++i) {
    ...
    s << right << std::setw(5) << ++n << ' ' << left << std::setw(20)
      ...
      << (*i).first.subroutine.substr(0, 16) << right << std::setw(7)
      << (*i).second.n << left << std::setw(1)
      << ((*i).second.ignoredFlag ? '*' : ' ') << right << std::setw(8)
      << (*i).second.aggregateN << '\n';
  ...
  for (std::set<std::string>::iterator g = groupedCategories.begin();
       g != gcEnd; ++g) {
    int groupTotal = 0;
    ...
    if (groupTotal > 0) {
      ...
      //
      s << right << std::setw(5) << ++n << ' ' << left << std::setw(20)
        << (*g).substr(0, 20) << ' ' << left << std::setw(2)
        << severityLevel.getSymbol() << ' ' << left << std::setw(16)
        << "  <Any Module>  " << ' ' << left << std::setw(16)
        << "<Any Function>" << right << std::setw(7) << groupTotal << left
        << std::setw(1) << (groupIgnored ? '*' : ' ') << right << std::setw(8)
        << groupAggregateN << '\n';
  ...
  for (ELmap_stats::const_iterator i = stats.begin(); i != stats.end(); ++i) {
    ...
    s << right << std::setw(5) << ++n << ' ' << left << std::setw(20)
      << (*i).first.id.c_str() << ' ' << left << std::setw(16)
      << (*i).second.context1.c_str() << ' ' << left << std::setw(16)
