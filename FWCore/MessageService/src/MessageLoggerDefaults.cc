...
int MessageLoggerDefaults::limit(std::string const& dest,
  ...
  if (lim == NO_VALUE_SET) {
    if (dd != destination.end()) {
      Destination& def_destin = dd->second;
      std::map<std::string, Category>::iterator c =
          def_destin.category.find(cat);
  ...
  if (lim == NO_VALUE_SET) {
    if (d != destination.end()) {
      Destination& destin = d->second;
      std::map<std::string, Category>::iterator cd =
          destin.category.find("default");
  ...
  if (lim == NO_VALUE_SET) {
    if (dd != destination.end()) {
      Destination& def_destin = dd->second;
      std::map<std::string, Category>::iterator cdd =
          def_destin.category.find("default");
