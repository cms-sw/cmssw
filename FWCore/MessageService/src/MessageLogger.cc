...
namespace {
  char const* const s_globalTransitionNames[] = {"@beginJob", "@beginRun", "@beginLumi", "@endLumi",
...
namespace edm {
  ...
  namespace service {
    ...
    void MessageLogger::establishModule(unsigned int transitionIndex, ModuleCallingContext const& mod,
                                        const char* whichPhase)  // ChangeLog 13, 17
    {
      MessageDrop* messageDrop = MessageDrop::instance();
      ...
      if (previous) {
        ...
        if (ParentContext::Type::kGlobal == top->type()) {
          ...
          if (tran == GlobalContext::Transition::kBeginLuminosityBlock or
            ...
            establishModule(lumiInfoBegin_ + globalContext->luminosityBlockIndex(), *previous,
          ...
          establishModule(stream->streamID().value(), *previous,
    ...
    void MessageLogger::postModuleConstruction(const ModuleDescription& iDescription) {  // it is now guaranteed that
                                                                                         // this will be called even if
                                                                                         // the module throws
