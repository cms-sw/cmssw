...
//
// 10 mf   6/18/07	Insert into the PostEndJob a possible
...
//
// 18 mf	  11/2/10	Eliminated curr_module, since it was only being
// used
...
char const* const s_globalTransitionNames[] = {
    "@beginJob", "@beginRun", "@beginLumi", "@endLumi",
    "@endRun",   "@endJob",   "@writeRun",  "@writeLumi"};
...
//
edm::service::MessageLogger::MessageLogger(ParameterSet const& iPS,
      ...
      nonModule_errorEnabled(true)  // change log 20
{
  ...
  iRegistry.watchPreSourceConstruction(this,
                                       &MessageLogger::preSourceConstruction);
  iRegistry.watchPostSourceConstruction(this,
                                        &MessageLogger::postSourceConstruction);
  // change log 3
  ...
  iRegistry.watchPreModuleBeginStream(this,
                                      &MessageLogger::preModuleBeginStream);
  iRegistry.watchPostModuleBeginStream(this,
                                       &MessageLogger::postModuleBeginStream);
  ...
  iRegistry.watchPreModuleStreamBeginRun(
      this, &MessageLogger::preModuleStreamBeginRun);
  iRegistry.watchPostModuleStreamBeginRun(
      this, &MessageLogger::postModuleStreamBeginRun);
  iRegistry.watchPreModuleStreamEndRun(this,
                                       &MessageLogger::preModuleStreamEndRun);
  iRegistry.watchPostModuleStreamEndRun(this,
                                        &MessageLogger::postModuleStreamEndRun);
  iRegistry.watchPreModuleStreamBeginLumi(
      this, &MessageLogger::preModuleStreamBeginLumi);
  iRegistry.watchPostModuleStreamBeginLumi(
      this, &MessageLogger::postModuleStreamBeginLumi);
  iRegistry.watchPreModuleStreamEndLumi(this,
                                        &MessageLogger::preModuleStreamEndLumi);
  iRegistry.watchPostModuleStreamEndLumi(
      this, &MessageLogger::postModuleStreamEndLumi);

  iRegistry.watchPreModuleGlobalBeginRun(
      this, &MessageLogger::preModuleGlobalBeginRun);
  iRegistry.watchPostModuleGlobalBeginRun(
      this, &MessageLogger::postModuleGlobalBeginRun);
  iRegistry.watchPreModuleGlobalEndRun(this,
                                       &MessageLogger::preModuleGlobalEndRun);
  iRegistry.watchPostModuleGlobalEndRun(this,
                                        &MessageLogger::postModuleGlobalEndRun);
  iRegistry.watchPreModuleGlobalBeginLumi(
      this, &MessageLogger::preModuleGlobalBeginLumi);
  iRegistry.watchPostModuleGlobalBeginLumi(
      this, &MessageLogger::postModuleGlobalBeginLumi);
  iRegistry.watchPreModuleGlobalEndLumi(this,
                                        &MessageLogger::preModuleGlobalEndLumi);
  iRegistry.watchPostModuleGlobalEndLumi(
      this, &MessageLogger::postModuleGlobalEndLumi);
...
void MessageLogger::unEstablishModule(ModuleDescription const& /*unused*/,
                                      const char* state) {
...
void MessageLogger::unEstablishModule(ModuleCallingContext const& mod,
                                      const char* state) {
  ...
  if (previous) {
    ...
    if (ParentContext::Type::kGlobal == top->type()) {
      ...
      if (tran == GlobalContext::Transition::kBeginLuminosityBlock or
        ...
        establishModule(runInfoBegin_ + globalContext->runIndex(), *previous,
                        s_globalTransitionNames[static_cast<int>(tran)]);
      }
    } else {
      auto stream = previous->getStreamContext();
      establishModule(
          stream->streamID().value(), *previous,
          s_streamTransitionNames[static_cast<int>(stream->transition())]);
    }
  } else {
    MessageDrop* messageDrop = MessageDrop::instance();
    messageDrop->streamID = std::numeric_limits<unsigned int>::max();
...
void MessageLogger::postModuleConstruction(
    const ModuleDescription& iDescription) {  // it is now guaranteed that this
...
void MessageLogger::postSourceConstruction(
    const ModuleDescription& iDescription) {
...
void MessageLogger::preModuleStreamBeginRun(StreamContext const& stream,
  ...
  establishModule(stream.streamID().value(), mod,
                  s_streamTransitionNames[static_cast<int>(
                      StreamContext::Transition::kBeginRun)]);
}
...
void MessageLogger::preModuleStreamBeginLumi(StreamContext const& stream,
  ...
  establishModule(stream.streamID().value(), mod,
                  s_streamTransitionNames[static_cast<int>(
                      StreamContext::Transition::kBeginLuminosityBlock)]);
}
...
void MessageLogger::preModuleStreamEndLumi(StreamContext const& stream,
  ...
  establishModule(stream.streamID().value(), mod,
                  s_streamTransitionNames[static_cast<int>(
                      StreamContext::Transition::kEndLuminosityBlock)]);
}
...
void MessageLogger::preModuleStreamEndRun(StreamContext const& stream,
  ...
  establishModule(stream.streamID().value(), mod,
                  s_streamTransitionNames[static_cast<int>(
                      StreamContext::Transition::kEndRun)]);  // ChangeLog 13
}
...
void MessageLogger::preModuleGlobalBeginRun(GlobalContext const& context,
  ...
  establishModule(runInfoBegin_ + context.runIndex().value(), mod,
                  s_globalTransitionNames[static_cast<int>(
                      GlobalContext::Transition::kBeginRun)]);
}
...
void MessageLogger::preModuleGlobalBeginLumi(GlobalContext const& context,
  ...
  establishModule(lumiInfoBegin_ + context.luminosityBlockIndex().value(), mod,
                  s_globalTransitionNames[static_cast<int>(
                      GlobalContext::Transition::kBeginLuminosityBlock)]);
}
...
void MessageLogger::preModuleGlobalEndLumi(GlobalContext const& context,
  ...
  establishModule(lumiInfoBegin_ + context.luminosityBlockIndex().value(), mod,
                  s_globalTransitionNames[static_cast<int>(
                      GlobalContext::Transition::kEndLuminosityBlock)]);
}
...
void MessageLogger::preModuleGlobalEndRun(GlobalContext const& context,
  ...
  establishModule(runInfoBegin_ + context.runIndex().value(), mod,
                  s_globalTransitionNames[static_cast<int>(
                      GlobalContext::Transition::kEndRun)]);  // ChangeLog 13
}
...
void MessageLogger::preEvent(StreamContext const& iContext) {
  ...
  edm::MessageDrop::instance()->setSinglet(
      "PreEventProcessing");  // changelog 17
...
void MessageLogger::preStreamBeginRun(
    StreamContext const& iContext)  // change log 14
{
  std::ostringstream ost;
  ost << "Run: " << iContext.eventID().run()
      << " Stream: " << iContext.streamID().value();
  ;
  transitionInfoCache_[iContext.streamID().value()] = ost.str();
  edm::MessageDrop::instance()->runEvent = ost.str();
  edm::MessageDrop::instance()->setSinglet(
      "PreStreamBeginRun");  // changelog 17
}
void MessageLogger::postStreamBeginRun(StreamContext const&) {
  edm::MessageDrop::instance()->runEvent = "PostStreamBeginRun";
  edm::MessageDrop::instance()->setSinglet(
      "PostStreamBeginRun");  // changelog 17
...
void MessageLogger::postStreamEndLumi(StreamContext const&) {
  ...
  edm::MessageDrop::instance()->setSinglet(
      "PostStreamEndLumi");  // changelog 17
}

void MessageLogger::preGlobalBeginRun(
    GlobalContext const& iContext)  // change log 14
{
  std::ostringstream ost;
...
void MessageLogger::postPathEvent(StreamContext const&, PathContext const&,
                                  HLTPathStatus const&) {
...
void MessageLogger::preGlobalBeginLumi(GlobalContext const& iContext) {
  ...
  ost << "Run: " << id.run() << " Lumi: " << id.luminosityBlock();
  transitionInfoCache_[lumiInfoBegin_ + iContext.luminosityBlockIndex()] =
      ost.str();
  edm::MessageDrop::instance()->runEvent = ost.str();
  edm::MessageDrop::instance()->setSinglet(
      "PreGlobalBeginLumi");  // changelog 17
}

void MessageLogger::postGlobalBeginLumi(GlobalContext const&) {
  edm::MessageDrop::instance()->runEvent = "PostGlobalBeginLumi";
  edm::MessageDrop::instance()->setSinglet(
      "PostGlobalBeginLumi");  // changelog 17
}
...
void MessageLogger::postGlobalEndLumi(GlobalContext const&) {
  ...
  edm::MessageDrop::instance()->setSinglet(
      "PostGlobalEndLumi");  // changelog 17
}
