...
//
//  3 6/11/07  mf	In emitToken():  In preamble mode, do not break and
//  indent
...
static ELstring formatTime(const time_t t) {  // Change log 7
  ...
  static char const dummy[] =
      "dd-Mon-yyyy hh:mm:ss TZN     ";  // Change log 7 for length only
  char ts[sizeof(dummy)];               // Change log 7

  struct tm timebuf;  // Change log 7

  strftime(ts, sizeof(dummy), "%d-%b-%Y %H:%M:%S %Z",
           localtime_r(&t, &timebuf));  // Change log 7
                                        // mf 4-9-04
