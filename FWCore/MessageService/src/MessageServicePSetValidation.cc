...
std::string edm::service::MessageServicePSetValidation::operator()(
    ParameterSet const& pset) {
  messageLoggerPSet(pset);
  return flaws.str();
}  // operator() to validate the PSet passed in

void edm::service::MessageServicePSetValidation::messageLoggerPSet(
    ParameterSet const& pset) {
  ...
  std::string dumps = check<std::string>(pset, "MessageLogger",
  ...
  noneExcept<std::string>(pset, "MessageLogger", "string", "threshold",
                          "generate_preconfiguration_message");
  ...
  if (!flaws.str().empty()) {
    flaws << "\nThe above are from MessageLogger configuration validation.\n"
          << "In most cases, these involve lines that the logger configuration "
             "code\n"
          << "would not process, but which the cfg creator obviously meant to "
             "have "
          << "effect.\n";
  }
...
void edm::service::MessageServicePSetValidation::psetLists(
    ParameterSet const& pset) {
  ...
  noDuplicates(fwkJobReports, destinations, "MessageLogger", "fwkJobReports",
               "destinations");
  noDuplicates(fwkJobReports, statistics, "MessageLogger", "fwkJobReports",
               "statistics");
  ...
  noDuplicates(categories, destinations, "MessageLogger", "categories",
               "destinations");
  noDuplicates(categories, statistics, "MessageLogger", "categories",
               "statistics");
  noDuplicates(categories, fwkJobReports, "MessageLogger", "categories",
               "fwkJobReports");
  ...
  noDuplicates(messageIDs, destinations, "MessageLogger", "messageIDs",
               "destinations");
  noDuplicates(messageIDs, statistics, "MessageLogger", "messageIDs",
               "statistics");
  noDuplicates(messageIDs, fwkJobReports, "MessageLogger", "messageIDs",
               "fwkJobReports");
  noDuplicates(messageIDs, fwkJobReports, "MessageLogger", "messageIDs",
               "categories");
...
void edm::service::MessageServicePSetValidation::suppressionLists(
    ParameterSet const& pset) {
...
void edm::service::MessageServicePSetValidation::vStringsCheck(
...
bool edm::service::MessageServicePSetValidation::allowedVstring(
    std::string const& s) {
...
bool edm::service::MessageServicePSetValidation::validateThreshold(
    std::string const& thresh, std::string const& psetName) {
...
bool edm::service::MessageServicePSetValidation::checkThreshold(
    std::string const& thresh) {
...
void edm::service::MessageServicePSetValidation::noDuplicates(
...
void edm::service::MessageServicePSetValidation::noDuplicates(
    vString const& v1, vString const& v2, std::string const& psetName,
...
void edm::service::MessageServicePSetValidation::noCoutCerrClash(
    vString const& v, std::string const& psetName,
    std::string const& parameterLabel) {
  vString::const_iterator end = v.end();
...
void edm::service::MessageServicePSetValidation::noKeywords(
    vString const& v, std::string const& psetName,
    std::string const& parameterLabel) {
  vString::const_iterator end = v.end();
  for (vString::const_iterator i = v.begin(); i != end; ++i) {
...
bool edm::service::MessageServicePSetValidation::keywordCheck(
...
void edm::service::MessageServicePSetValidation::noNonPSetUsage(
    ParameterSet const& pset, vString const& v, std::string const& psetName,
    std::string const& parameterLabel) {
  ...
  disallowedParam<std::vector<std::string> >(pset, v, psetName, parameterLabel,
                                             "vstring");
}  // noNonPSetUsage

void edm::service::MessageServicePSetValidation::noBadParams(
    vString const& v, vString const& params, std::string const& psetName,
    std::string const& parameterLabel, std::string const& type) {
...
void edm::service::MessageServicePSetValidation::noOtherPsets(
    ParameterSet const& pset) {
  vString psnames;
...
bool edm::service::MessageServicePSetValidation::lookForMatch(
    vString const& v, std::string const& s) {
  vString::const_iterator begin = v.begin();
  vString::const_iterator end = v.end();
  return (std::find(begin, end, s) != end);
}

void edm::service::MessageServicePSetValidation::destinationPSets(
    ParameterSet const& pset) {
  ParameterSet empty_PSet;
...
void edm::service::MessageServicePSetValidation::destinationPSet(
    ParameterSet const& pset, std::string const& psetName) {
  // Category PSets
...
void edm::service::MessageServicePSetValidation::defaultPSet(
    ParameterSet const& main_pset) {
  ParameterSet empty_PSet;
  ParameterSet const& pset =
      main_pset.getUntrackedParameterSet("default", empty_PSet);
...
void edm::service::MessageServicePSetValidation::statisticsPSets(
    ParameterSet const& pset) {
  ParameterSet empty_PSet;
...
void edm::service::MessageServicePSetValidation::statisticsPSet(
    ParameterSet const& pset, std::string const& psetName) {
  // Category PSets
...
void edm::service::MessageServicePSetValidation::fwkJobReportPSets(
    ParameterSet const& pset) {
  ParameterSet empty_PSet;
...
void edm::service::MessageServicePSetValidation::fwkJobReportPSet(
    ParameterSet const& pset, std::string const& psetName) {
  // Category PSets
...
void edm::service::MessageServicePSetValidation::noNoncategoryPsets(
    ParameterSet const& pset, std::string const& psetName) {
  vString psnames;
  pset.getParameterSetNames(psnames, false);
  vString::const_iterator end = psnames.end();
...
void edm::service::MessageServicePSetValidation::categoryPSets(
    ParameterSet const& pset, std::string const& psetName) {
...
void edm::service::MessageServicePSetValidation::categoryPSet(
    ParameterSet const& pset, std::string const& OuterPsetName,
  ...
  ParameterSet empty_PSet;
  ParameterSet const& c =
      pset.getUntrackedParameterSet(categoryName, empty_PSet);
...
void edm::service::MessageServicePSetValidation::catInts(
    ParameterSet const& pset, std::string const& psetName,
    std::string const& categoryName) {
  ...
  for (vString::const_iterator i = x.begin(); i != end; ++i) {
    ...
    flaws << categoryName << " category PSet nested in " << psetName
          << " PSet: \n"
...
void edm::service::MessageServicePSetValidation::catNoPSets(
    ParameterSet const& pset, std::string const& psetName,
    std::string const& categoryName) {
  vString psnames;
  pset.getParameterSetNames(psnames, false);
...
void edm::service::MessageServicePSetValidation::catBoolRestriction(
    ParameterSet const& pset, std::string const& psetName,
    std::string const& categoryName, std::string const& type) {
