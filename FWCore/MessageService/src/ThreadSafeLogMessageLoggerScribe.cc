  ...
  switch (opcode) {  // interpret the work item
    ...
    case MessageLoggerQ::LOG_A_MESSAGE: {
      ...
      try {
        ...
        std::cerr << "ThreadSafeLogMessageLoggerScribe caught an unknown "
                     "exception and "
...
void ThreadSafeLogMessageLoggerScribe::configure_errorlog() {
  ...
  m_waitingThreshold =
      getAparameter<unsigned int>(*job_pset_p, "waiting_threshold", 100);
...
void ThreadSafeLogMessageLoggerScribe::configure_dest(
...
void ThreadSafeLogMessageLoggerScribe::
    configure_ordinary_destinations()  // Changelog 16
{
  vString empty_vString;
  String empty_String;
  PSet empty_PSet;
...
void ThreadSafeLogMessageLoggerScribe::parseCategories(
    std::string const& s, std::vector<std::string>& cats) {
  const std::string::size_type npos = std::string::npos;
