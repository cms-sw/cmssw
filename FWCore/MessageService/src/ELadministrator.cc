...
void ELadministrator::log(edm::ErrorObj& msg) {
  ...
  ++severityCounts_[lev];
  if (lev > highSeverity_.getLevel()) highSeverity_ = msg.xid().severity;
  ...
  for (auto& sink : sinks_)
    if (sink->log(msg)) msg.setReactedTo(true);
...
std::shared_ptr<ELdestination> ELadministrator::attach(
    std::shared_ptr<ELdestination> sink) {
...
int ELadministrator::severityCount(const ELseverityLevel& from,
                                   const ELseverityLevel& to) const {
  int k = from.getLevel();
  int sum = severityCounts_[k];

  while (++k != to.getLevel()) sum += severityCounts_[k];
...
void ELadministrator::resetSeverityCount(const ELseverityLevel& from,
                                         const ELseverityLevel& to) {
  for (int k = from.getLevel(); k <= to.getLevel(); ++k) severityCounts_[k] = 0;
