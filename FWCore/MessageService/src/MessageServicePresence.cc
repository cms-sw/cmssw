...
MessageServicePresence::MessageServicePresence()
  ...
  , m_scribeThread
         ( ( (void) MessageLoggerQ::instance() // ensure Q's static data init'd
            , std::bind(&runMessageLoggerScribe, get_underlying(m_queue))
	  ...
	  // first executing the before-the-comma statement. 
{
  MessageLoggerQ::setMLscribe_ptr(
    std::shared_ptr<edm::service::AbstractMLscribe>(std::make_shared<MainThreadMLscribe>(get_underlying(m_queue))));
    								// change log 3
