...
void runMessageLoggerScribe(std::shared_ptr<ThreadQueue> queue) {
  ...
  // 1) When this terminates, the main thread may not yet have done a join() and
  // we
  //    don't want to handle the sigs at that point in this thread
  // 2) If we re-enable sigs, we will get the entire stack of accumulated ones
  // (if any)
}
}  // namespace
...
// Note that m_scribeThread, which is a boost::thread, has a single-argument
// ctor -
...
// first executing the before-the-comma statement.
{
  MessageLoggerQ::setMLscribe_ptr(
      std::shared_ptr<edm::service::AbstractMLscribe>(
          std::make_shared<MainThreadMLscribe>(queue())));
  // change log 3
