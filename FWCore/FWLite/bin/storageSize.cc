...
int main(int argc, char* argv[]) try {
  ...
  descString +=
      "] class_name"
      "\n The program dumps information about how much storage space is needed "
      "to store the class"
      "\nAllowed options";
  ...
  if (nullptr == objInstance) {
    std::cerr << "unable to create a default instance of the class "
              << className;
    return 1;
  }
  ...
  std::cout << "\nNOTE: add 4 bytes for each 'has written' value because of a "
               "bug in ROOT's printout of the accounting"
            << "\n  Each class (inheriting or as member data) has metadata an "
