...
//
/**\class BranchMapReader BranchMapReader.h
 FWCore/FWLite/interface/BranchMapReader.h
...
class BMRStrategy {
 public:
  ...
  virtual const edm::BranchDescription& productToBranch(
      const edm::ProductID& pid) = 0;
  virtual const edm::BranchDescription& branchIDToBranch(
      const edm::BranchID& bid) const = 0;
  virtual const std::vector<edm::BranchDescription>&
  getBranchDescriptions() = 0;
  virtual const edm::BranchListIndexes& branchListIndexes() const = 0;
  virtual const edm::ThinnedAssociationsHelper& thinnedAssociationsHelper()
      const = 0;
...
class BranchMapReader {
 public:
  ...
  edm::BranchID productToBranchID(const edm::ProductID& pid) {
    return strategy_->productToBranchID(pid);
  }
  const edm::BranchDescription& productToBranch(const edm::ProductID& pid);
  const edm::BranchDescription& branchIDToBranch(
      const edm::BranchID& bid) const {
  ...
  // ---------- member data --------------------------------
 private:
  std::unique_ptr<internal::BMRStrategy> newStrategy(TFile* file,
                                                     int fileVersion);
  std::unique_ptr<internal::BMRStrategy>
      strategy_;  // Contains caches, so we do not propagate_const
  int fileVersion_;
};
}
