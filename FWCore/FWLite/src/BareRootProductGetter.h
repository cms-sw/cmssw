...
//
/**\class BareRootProductGetter BareRootProductGetter.h
 FWCore/FWLite/interface/BareRootProductGetter.h
...
class BareRootProductGetter : public edm::EDProductGetter {
 public:
  ...
  // nullptr is returned.
  virtual edm::WrapperBase const* getThinnedProduct(
      edm::ProductID const&, unsigned int& key) const override;
  ...
  virtual void getThinnedProducts(
      edm::ProductID const&,
 ...
 private:
  ...
  struct Buffer {
    Buffer(edm::WrapperBase const* iProd, TBranch* iBranch, void* iAddress,
        ...
        : product_(iProd),
          ...
          class_(iClass) {}
    Buffer()
        : product_(), branch_(), address_(), eventEntry_(-1), class_(nullptr) {}

    std::shared_ptr<edm::WrapperBase const> product_;
    edm::propagate_const<TBranch*> branch_;
    void* address_;  // the address to pass to Root since as of 5.13 they cache
                     // that info
  ...
  BareRootProductGetter const& operator=(
      BareRootProductGetter const&);  // stop default

  Buffer* createNewBuffer(edm::BranchID const&) const;
  edm::ThinnedAssociation const* getThinnedAssociation(
      edm::BranchID const& branchID, Long_t eventEntry) const;
