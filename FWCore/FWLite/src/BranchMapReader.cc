...
class Strategy : public BMRStrategy {
 public:
  ...
  virtual edm::BranchDescription const& productToBranch(
      edm::ProductID const& pid) override;
  virtual edm::BranchDescription const& branchIDToBranch(
      edm::BranchID const& bid) const override;
  virtual std::vector<edm::BranchDescription> const& getBranchDescriptions()
      override;
  virtual edm::ThinnedAssociationsHelper const& thinnedAssociationsHelper()
      const override {
...
bool Strategy::updateFile(TFile* file) {
  ...
  eventTree_ = dynamic_cast<TTree*>(
      currentFile_->Get(edm::poolNames::eventTreeName().c_str()));
  luminosityBlockTree_ = dynamic_cast<TTree*>(
      currentFile_->Get(edm::poolNames::luminosityBlockTreeName().c_str()));
  runTree_ = dynamic_cast<TTree*>(
      currentFile_->Get(edm::poolNames::runTreeName().c_str()));
...
TBranch* Strategy::getBranchRegistry(edm::ProductRegistry** ppReg) {
  ...
  TTree* metaDataTree = dynamic_cast<TTree*>(
      currentFile_->Get(edm::poolNames::metaDataTreeName().c_str()));
  if (nullptr != metaDataTree) {
    bReg = metaDataTree->GetBranch(
        edm::poolNames::productDescriptionBranchName().c_str());
...
edm::BranchID Strategy::productToBranchID(edm::ProductID const&) {
  throw edm::Exception(edm::errors::UnimplementedFeature)
      << "Unsupported EDM file version";
}

edm::BranchDescription const& Strategy::productToBranch(
...
edm::BranchDescription const& Strategy::branchIDToBranch(
    edm::BranchID const& bid) const {
  bidToDesc::const_iterator bdi = branchDescriptionMap_.find(bid);
  if (branchDescriptionMap_.end() == bdi) {
    return kDefaultBranchDescription;
  }
...
BranchMapReaderStrategyV1::BranchMapReaderStrategyV1(TFile* file,
                                                     int fileVersion)
...
// v7 has differences in product status that are not implemented in
// BranchMapReader yet
...
BranchMapReaderStrategyV7::BranchMapReaderStrategyV7(TFile* file,
                                                     int fileVersion)
...
BranchMapReaderStrategyV8::BranchMapReaderStrategyV8(TFile* file,
                                                     int fileVersion)
    : Strategy(file, fileVersion),
...
bool BranchMapReaderStrategyV8::updateFile(TFile* file) {
  ...
  TTree* metaDataTree = dynamic_cast<TTree*>(
      currentFile_->Get(edm::poolNames::eventMetaDataTreeName().c_str()));
  if (nullptr != metaDataTree) {
    entryInfoBranch_ = metaDataTree->GetBranch(
        BranchTypeToBranchEntryInfoBranchName(edm::InEvent).c_str());
    //         std::cout << "entryInfoBranch for " <<
    //         BranchTypeToBranchEntryInfoBranchName(edm::InEvent) << " " <<
    //         entryInfoBranch_ << std::endl;
  } else {
    return false;
  }
  pEventEntryInfoVector_ = &eventEntryInfoVector_;
...
BranchMapReaderStrategyV11::BranchMapReaderStrategyV11(TFile* file,
                                                       int fileVersion)
    : Strategy(file, fileVersion),
...
bool BranchMapReaderStrategyV11::updateFile(TFile* file) {
  ...
  if (metaDataTree->FindBranch(
          edm::poolNames::branchIDListBranchName().c_str()) != 0) {
    TBranch* b = metaDataTree->GetBranch(
        edm::poolNames::branchIDListBranchName().c_str());
  ...
  eventHistoryTree_ = dynamic_cast<TTree*>(
      currentFile_->Get(edm::poolNames::eventHistoryTreeName().c_str()));
  ...
  if (nullptr != br) {
    ...
    for (auto& item : prodList) {
      ...
      if (edm::InEvent == prod.branchType()) {
        ...
        branchDescriptionMap_.insert(
            bidToDesc::value_type(prod.branchID(), prod));
        //             std::cout << "v11 updatefile " << prod.branchID() <<
        //             std::endl;
      }
    }
  }
...
bool BranchMapReaderStrategyV11::updateMap() {
  if (!mapperFilled_) {
    TBranch* eventHistoryBranch = eventHistoryTree_->GetBranch(
        edm::poolNames::eventHistoryBranchName().c_str());
...
edm::BranchID BranchMapReaderStrategyV11::productToBranchID(
    edm::ProductID const& pid) {
  updateMap();
  return edm::productIDToBranchID(pid, *branchIDLists_,
...
BranchMapReaderStrategyV17::BranchMapReaderStrategyV17(TFile* file,
                                                       int fileVersion)
    : Strategy(file, fileVersion),
      eventsTree_(nullptr),
      pBranchListIndexes_(&branchListIndexes_) {
  updateFile(file);
}
...
bool BranchMapReaderStrategyV17::updateFile(TFile* file) {
  ...
  thinnedAssociationsHelper_ =
      std::make_unique<edm::ThinnedAssociationsHelper>();
  edm::ThinnedAssociationsHelper* thinnedAssociationsHelperPtr =
      thinnedAssociationsHelper_.get();
  if (metaDataTree->FindBranch(
          edm::poolNames::thinnedAssociationsHelperBranchName().c_str()) !=
      nullptr) {
    TBranch* b = metaDataTree->GetBranch(
        edm::poolNames::thinnedAssociationsHelperBranchName().c_str());
  ...
  eventsTree_ = dynamic_cast<TTree*>(
      currentFile_->Get(edm::poolNames::eventTreeName().c_str()));
...
bool BranchMapReaderStrategyV17::updateMap() {
  if (!mapperFilled_) {
    TBranch* branchListIndexesBranch = eventsTree_->GetBranch(
        edm::poolNames::branchListIndexesBranchName().c_str());
...
edm::BranchID BranchMapReaderStrategyV17::productToBranchID(
    edm::ProductID const& pid) {
  updateMap();
...
int BranchMapReader::getFileVersion(TFile* file) {
  TTree* metaDataTree = dynamic_cast<TTree*>(
      file->Get(edm::poolNames::metaDataTreeName().c_str()));
  if (nullptr == metaDataTree) {
    return 0;
  }
  ...
  TBranch* bVer = metaDataTree->GetBranch(
      edm::poolNames::fileFormatVersionBranchName().c_str());
  bVer->SetAddress(&pV);
  bVer->GetEntry(0);
...
bool BranchMapReader::updateFile(TFile* file) {
  ...
  if (!isNew) {
    // could still have a new TFile which just happens to share the same memory
    // address as the previous file
    // will assume that if the Event tree's address and UUID are the same as
    // before then we do not have
    // to treat this like a new file
    TTree* eventTreeTemp = dynamic_cast<TTree*>(
        currentFile->Get(edm::poolNames::eventTreeName().c_str()));
    isNew = eventTreeTemp != strategy_->eventTree_ ||
...
edm::BranchDescription const& BranchMapReader::productToBranch(
    edm::ProductID const& pid) {
...
std::unique_ptr<internal::BMRStrategy> BranchMapReader::newStrategy(
    TFile* file, int fileVersion) {
  std::unique_ptr<internal::BMRStrategy> s;

  if (fileVersion >= 17) {
    s = std::make_unique<internal::BranchMapReaderStrategyV17>(file,
                                                               fileVersion);
  } else if (fileVersion >= 11) {
    s = std::make_unique<internal::BranchMapReaderStrategyV11>(file,
                                                               fileVersion);
  } else if (fileVersion >= 8) {
    s = std::make_unique<internal::BranchMapReaderStrategyV8>(file,
                                                              fileVersion);
  } else if (fileVersion >= 7) {
    s = std::make_unique<internal::BranchMapReaderStrategyV7>(file,
                                                              fileVersion);
  } else {
    s = std::make_unique<internal::BranchMapReaderStrategyV1>(file,
                                                              fileVersion);
  }
  return s;
}
