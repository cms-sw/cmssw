...
//
// BareRootProductGetter const&
// BareRootProductGetter::operator=(BareRootProductGetter const& rhs) {
...
//
edm::WrapperBase const* BareRootProductGetter::getIt(
    edm::ProductID const& pid) const {
  ...
  if (nullptr == eventTree) {
    throw cms::Exception("NoEventsTree")
        ...
        << "If the above is not the file you expect then please open your data "
  ...
  if (eventEntry < 0) {
    throw cms::Exception("GetEntryNotCalled")
        << "please call GetEntry for the 'Events' TTree for each event in "
           "order to make edm::Ref's work."
        << "\n Also be sure to call 'SetAddress' for all Branches after "
           "calling the GetEntry.";
  }
...
edm::WrapperBase const* BareRootProductGetter::getIt(
  ...
  if (nullptr == buffer->branch_) {
    throw cms::Exception("NullBranch")
        ...
        << "\n Please contact the developers since this message should not "
           "happen.";
  }
  if (buffer->eventEntry_ != eventEntry) {
    // NOTE: Need to reset address because user could have set the address
    // themselves
    // std::cout << "new event" << std::endl;

    // ROOT WORKAROUND: Create new objects so any internal data cache will get
    // cleared
    void* address = buffer->class_->New();

    static TClass const* edproductTClass =
        TClass::GetClass(typeid(edm::WrapperBase));
...
edm::WrapperBase const* BareRootProductGetter::getThinnedProduct(
  ...
  for (auto associatedBranches = thinnedAssociationsHelper.parentBegin(parent),
    ...
    if (associatedBranches->parent() !=
        branchMap_.productToBranchID(
            thinnedAssociation->parentCollectionID())) {
      continue;
    }
    ...
    edm::ProductID const& thinnedCollectionPID =
...
void BareRootProductGetter::getThinnedProducts(
  ...
  for (auto associatedBranches = thinnedAssociationsHelper.parentBegin(parent),
    ...
    edm::ProductID thinnedCollectionPID =
        thinnedAssociation->thinnedCollectionID();
    edm::WrapperBase const* thinnedCollection = getIt(thinnedCollectionPID);
...
BareRootProductGetter::Buffer* BareRootProductGetter::createNewBuffer(
  ...
  TBranch* branch =
      branchMap_.getEventTree()->GetBranch(bdesc.branchName().c_str());
  if (nullptr == branch) {
    // we do not thrown on missing branches since 'getIt' should not throw under
    // that condition
    return nullptr;
  }
  // find the class type
...
edm::ThinnedAssociation const* BareRootProductGetter::getThinnedAssociation(
  ...
  if (wrapperBase == nullptr) {
    throw edm::Exception(edm::errors::LogicError)
        << "BareRootProductGetter::getThinnedAssociation, product "
           "ThinnedAssociation not found.\n";
  }
  if (!(typeid(edm::ThinnedAssociation) == wrapperBase->dynamicTypeInfo())) {
    throw edm::Exception(edm::errors::LogicError)
        << "BareRootProductGetter::getThinnedAssociation, product has wrong "
