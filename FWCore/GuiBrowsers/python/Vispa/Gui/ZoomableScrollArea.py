...
class ZoomableScrollArea(Zoomable, QScrollArea):
    ...
    def __init__(self, parent=None):
        ...
        # Call after QScrollArea constructor, required by setZoom()
        Zoomable.__init__(self)

        self.connect(
            self.verticalScrollBar(),
            SIGNAL("valueChanged(int)"),
            self.scrollBarValueChanged)
        self.connect(
    ...
    def wheelEvent(self, event):
        ...
        if event.modifiers() == Qt.ControlModifier:
            ...
            self.ensureVisible(
                newLeft - event.x() + self.viewport().width() / 2.0,
                newTop - event.y() + self.viewport().height() / 2.0,
    ...
    def autosizeScrollWidget(self):
        ...
        width = max(self.viewport().width(),
                    childrenRect.bottomRight().x()) - min(0,
                                                          childrenRect.topLeft().x())
        height = max(self.viewport().height(),
                     childrenRect.bottomRight().y()) - min(0,
                                                           childrenRect.topLeft().y())
