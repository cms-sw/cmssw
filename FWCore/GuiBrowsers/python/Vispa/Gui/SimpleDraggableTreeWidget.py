...
class SimpleDraggableTreeWidget(QTreeWidget):
    ...
    def __init__(self, headerLabel, dragEnabled=False,
                 mimeType=None, parent=None):
        """ Constructor.
        """
        QTreeWidget.__init__(self, parent)
        self.setMimeType(mimeType)
        self.setAutoFillBackground(True)
        # print "color roles", QPalette.Base, QPalette.Window,
        # self.backgroundRole()
    ...
    def startDrag(self, supportedActions):
        ...
        if len(indexes) > 0:
            ...
            if self.model().data(
                    indexes[0], Qt.DecorationRole).type() == QVariant.Icon:
