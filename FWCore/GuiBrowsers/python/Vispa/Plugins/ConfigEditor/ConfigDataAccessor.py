...
class ConfigDataAccessor(BasicDataAccessor, RelativeDataAccessor):
    ...
    def _readRecursive(self, mother, pth):
        ...
        entry = None
        if isinstance(pth, (cms.Path, cms.EndPath, cms.Sequence, cms.SequencePlaceholder, cms.Source,
                            mod._Module, cms.Service, cms.ESSource, cms.ESProducer, cms.ESPrefer, cms.PSet, cms.VPSet)):
            entry = pth
    ...
    def readConnections(self, objects, toNeighbors=False):
        ...
        for connection in compareObjectList:
            if self._cancelOperationsFlag:
                break
            try:
                if (not connection in checkedObjects) and (
                        connection not in self._connections):
...
# import input-config and make list of all imported configs
        ...
        self._file = imp.load_module(
            os.path.splitext(
                os.path.basename(
                    self._filename))[0].replace(
                ".", "_"), theFile, self._filename, ("py", "r", 1))
        theFile.close()
...
# make dictionary that connects every cms-object with the file in which it
# is defined
...
# collect all path/sequences/modules of the input-config in a list
        if self.process():
            ...
            if not self._isReplaceConfig and hasattr(
            ...
            for entry in dir(self._file):
                ...
                if entry[0] != "_" and entry != "cms" and hasattr(o, "label_"):
                    ...
                    text = os.path.splitext(
                        os.path.basename(file_dict[o.label_()]))[0]
                    if text == os.path.splitext(os.path.basename(self._filename))[
                            0] and not o in self._allObjects:
    ...
    def setProcess(self, process):
        ...
        if self.process().schedule is not None:
        ...
        folder_list += [
            ("modules",
             self._sort_list(
                 self.process().producers.values() +
                 self.process().filters.values() +
                 self.process().analyzers.values()))]
        folder_list += [("services",
                         self._sort_list(self.process().services.values()))]
        folder_list += [("psets",
                         self._sort_list(self.process().psets.values()))]
        folder_list += [("vpsets",
                         self._sort_list(self.process().vpsets.values()))]
        folder_list += [("essources",
                         self._sort_list(self.process().es_sources.values()))]
        folder_list += [("esproducers",
                         self._sort_list(self.process().es_producers.values()))]
        folder_list += [("esprefers",
                         self._sort_list(self.process().es_prefers.values()))]
        folders = {}
        ...
        if True:
            ...
            names = [l for t, l, p, pr in self.applyCommands(
                self.outputEventContent(), self.outputCommands())]
    ...
    def _readHeaderInfo(self):
        ...
        while theFile and not (
                foundHeaderPart1 and foundHeaderPart2) and lines > 0:
            line = theFile.readline()
            lines -= 1
            if "Generated by ConfigEditor" in line:
                foundHeaderPart1 = True
            splitline = line.split("'")
            if foundHeaderPart1 and len(
                    splitline) == 5 and splitline[0] == "sys.path.append(os.path.abspath(os.path.expandvars(os.path.join(" and splitline[4] == "))))\n":
                search_paths += [
                    os.path.abspath(
                        os.path.expandvars(
                            os.path.join(
            ...
            splitline = line.split()
            if foundHeaderPart1 and len(
                    splitline) == 4 and splitline[0] == "from" and splitline[2] == "import":
                for search_path in search_paths:
                    if os.path.exists(os.path.join(
                            search_path, splitline[1] + ".py")):
    ...
    def label(self, object):
        ...
        text = ""
        if hasattr(object, "label_") and (not hasattr(
                object, "hasLabel_") or object.hasLabel_()):
    ...
    def isContainer(self, object):
        # cms.SequencePlaceholder assumed to be a module
        return isinstance(object, (ConfigFolder, list,
                                   cms.Path, cms.EndPath, cms.Sequence))

    def nonSequenceChildren(self, object):
        objects = []
        if self.isContainer(object):
            for o in self.allChildren(object):
                if not self.isContainer(o) and len(
                        self.children(o)) == 0 and not o in objects:
                    objects += [o]
        else:
            for o in self.motherRelations(
                    object) + [object] + self.daughterRelations(object):
                if not o in objects:
                    objects += [o]
        return objects
...
#            text=object._filename
        if text == "" or text.find(
                "FWCore/ParameterSet") >= 0 or text.find("/build/") >= 0:
    ...
    def lineNumber(self, object):
        ...
        text = ""
        if hasattr(object, "_filename"):
            if object._filename.find(
                    "FWCore/ParameterSet") < 0 and object._filename.find("ConfigEditor") < 0:
    ...
    def pypackage(self, object):
        match_compiled = re.match(
            r'(?:^|.*?/)CMSSW[0-9_]*/python/((?:\w*/)*\w*)\.py$',
            self.fullFilename(object))
        if match_compiled:
            return match_compiled.group(1).replace('/', '.')

        match_norm = re.match(
            r'(?:^|.*?/)(\w*)/(\w*)/(?:test|python)/((?:\w*/)*)(\w*)\.py$',
            self.fullFilename(object))
        if match_norm:
            return '%s.%s.%s%s' % (match_norm.group(1), match_norm.group(
                2), match_norm.group(3).replace('/', '.'), match_norm.group(4))
        return ''

    def pypath(self, object):
        match_compiled = re.match(
            r'(?:^|.*?/)CMSSW[0-9_]*/python/((?:\w*/){2})((?:\w*/)*)(\w*\.py)$',
            self.fullFilename(object))
        if match_compiled:
            return '%spython/%s%s' % (match_compiled.group(1),
                                      match_compiled.group(2), match_compiled.group(3))
        match_norm = re.match(
            r'(?:^|.*?/)(\w*/\w*/(?:test|python)/(?:\w*/)*\w*\.py)$',
            self.fullFilename(object))
        if match_norm:
    ...
    def package(self, object):
        ...
        shortdirname = os.path.dirname(
            self.fullFilename(object)).split('python/')
        text = ""
    ...
    def parameters(self, object):
        ...
        if hasattr(object, "parameters_"):
            ...
            if hasattr(object._seq, "dumpSequencePython"):
                ...
                this_parameters = [
                    ('sequence', 'WARNING: object was removed from a sequence.')]
        ...
        if isinstance(value, cms.VInputTag):
            for i in range(len(value)):
                if isinstance(value[i], str):
                    ...
                    self._addInputTag(
                        value[i], this_key + "[" + str(i) + "]", this_inputtags)
        elif isinstance(value, list):
            for i in value:
                self._addInputTag(i, this_key, this_inputtags)
        if hasattr(value, "parameters_"):
            this_inputtags += self._readInputTagsRecursive(
                value.parameters_().items(), this_key)
        ...
        if not object in self._foundInDict.keys():
            ...
            for entry in self._allObjects:
                for daughter in self.children(entry):
                    if self.label(object) == self.label(daughter) and len(
                            self.children(entry)) > 0 and not self.label(entry) in foundin:
        ...
        if not object in self._usedByDict.keys():
            ...
            for entry in self._allObjects:
                for uses in self.uses(entry):
                    if self.label(object) == uses and not self.label(
                            entry) in usedby:
    ...
    def properties(self, object):
        ...
        if self.package(object) != "":
            properties += [("String", "package",
                            self.package(object), None, True)]
        if self.fullFilename(object) != "":
            properties += [("String", "full filename",
                            self.fullFilename(object), None, True)]
    ...
    def setProperty(self, object, name, value, categoryName):
        ...
        if hasattr(object, "_seq") and name == "sequence":
            ...
            try:
                ...
                error = "Cannot set parameter " + name + \
                    " (see logfile for details):\n" + str(e)
                logging.warning(
                    __name__ +
                    ": setProperty: Cannot set parameter " +
                    name +
                    ": " +
                    exception_traceback())
                return error
            ...
            for key, value in self.inputTags(object):
                ...
                if not module in allLabels:
                    ...
                    if "*_" + module + "_" + product + "_" + process in content_objects.keys():
                        content_objects["*_" + module + "_" + product +
                                        "_" + process] += "," + self.label(object)
                    else:
                        content_objects["*_" +
                                        module +
                                        "_" +
    ...
    def outputCommands(self):
        ...
        if len(outputModules) > 0 and hasattr(
                outputModules[0], "outputCommands"):
