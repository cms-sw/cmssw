...
def auto_inspect():
    ...
    stack = inspect.stack()
    while len(stack) >= 1 and len(stack[0]) >= 2 and (
            'FWCore/ParameterSet' in stack[0][1] or 'FWCore/GuiBrowsers' in stack[0][1]):
        stack = stack[1:]
...
def new_history(self, removeDuplicates=False):
    return self.__dict__['_Process__history'] + \
        self.dumpModificationsWithObjects(removeDuplicates)
...
def new_recurseDumpModifications_(self, name, o):
    ...
    if isinstance(o, cms.Process):
        for mod in o.__dict__['_Process__modifications']:
            if hasattr(o, mod['name']) and hasattr(
                    getattr(o, mod['name']), 'dumpPython'):
...
def new_dumpModifications(self, comments=True, process=True,
                          module=False, sequence=True, value=True, sort=True, group=True):
    """
    ...
    * sort: whether to sort all the names before printing (otherwise they're in
    ...
    if not checkpoint is None:
        modifications = [x for x in modifications if any(
            [x['name'].startswith(check) for check in checkpoint])]
    if module:
        value = False
        comments = False
        ...
        for i, m in enumerate(modifications):
            ...
            if not group or i == len(
                    modifications) - 1 or not modifications[i + 1]['name'] == m['name']:
...
def new_Parameterizable_setattr(self, name, value):
    if (not self.isFrozen()) and (
            not name.startswith('_')) and (name in self.__dict__):
        stack = auto_inspect()
        self._modifications.append(
            {
                'file': stack[0][1],
...
try:
    ...
    def new__SequenceOperator_name(self):
        return str(self._left._name_()) + str(self._pySymbol) + \
...
if __name__ == '__main__':
    ...
    class TestModificationTracking(unittest.TestCase):
        ...
        def testSeq(self):
            ...
            self.assertEqual(
                process.seq1._modifications[-1]['action'], 'replace')

            process.seq2.remove(process.f5)
            self.assertEqual(
                process.seq2._modifications[-1]['action'], 'remove')
            ...
            self.assertEqual(
                process.path._modifications[-1]['action'], 'append')
            process.path.remove(process.f6)
            self.assertEqual(
                process.path._modifications[-1]['action'], 'remove')
            process.path.replace(process.f2, ~process.f2)
            self.assertEqual(
                process.path._modifications[-1]['action'], 'replace')
        ...
        def testdumpHistory(self):
            ...
            process.source = Source(
                "PoolSource", fileNames=cms.untracked.string("file:file.root"))

            changeSource(process, "file:filename.root")
            self.assertEqual(
                changeSource._parameters['source'].value,
                "file:filename.root")
