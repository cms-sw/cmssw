...
class ConfigToolBase(object):
    ...
    # if the user gives a valid default values and if there is not a list of
    # allowed values)
    def addParameter(self, dict, parname, parvalue, description,
                     Type=None, allowedValues=None, acceptNoneValue=False):
    ...
    def setParameter(self, name, value, typeNone=False):
        ...
        self.typeError(name)
        # check about input value (it works if allowedValues for the specific
        # parameter is set)
        if self._defaultParameters[name].allowedValues is not None:
            self.isAllowed(name, value)
    ...
    def dumpPython(self):
        ...
        for key in self._parameters.keys():
            if repr(self._parameters[key].value) != repr(
                    self._defaultParameters[key].value):
                dumpPython += ", " + str(key) + " = "
    ...
    def errorMessage(self, value, type):
        return "The type for parameter " + '"' + \
            str(value) + '"' + " is not " + '"' + str(type) + '"'
    # method isAllowed is called by setParameter to check input values for a
    # specific parameter
        ...
        if value == []:
            ...
            for key in value.keys():
                if (key not in self._parameters[name].allowedValues):
                    raise ValueError("The input key value " +
                                     '"' +
                                     str(key) +
                                     '"' +
                                     " for parameter " +
                                     ...
                                     str(self._parameters[name].allowedValues)[1:-
                                                                               1])
            ...
            for val in value:
                if (val not in self._parameters[name].allowedValues):
                    raise ValueError("The input value " +
                                     '"' +
                                     str(val) +
                                     '"' +
                                     " for parameter " +
            ...
            if (value not in self._parameters[name].allowedValues and value is None) and (
                    not self._parameters[name].acceptNoneValue):
                self.parAccepted = False
        elif not isinstance(self._parameters[name].allowedValues, list):
            if (value != self._parameters[name].allowedValues and value is None) and (
                    not self._parameters[name].acceptNoneValue):
                self.parAccepted = False
    ...
    def typeError(self, name):
        if self._parameters[name].acceptNoneValue is False:
            ...
            if not (isinstance(
                    self._parameters[name].value, self._parameters[name].type) or self._parameters[name].value is None):
                raise TypeError(
                    self.errorMessage(
                        self._parameters[name].value,
                        self._parameters[name].type))
