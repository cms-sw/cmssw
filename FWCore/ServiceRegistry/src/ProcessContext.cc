...
ProcessContext::ProcessContext()
    : processConfiguration_(nullptr), parentProcessContext_(nullptr) {}

ProcessContext const& ProcessContext::parentProcessContext() const {
  if (!isSubProcess()) {
    throw Exception(errors::LogicError)
        << "ProcessContext::parentProcessContext This function should only be "
           "called for SubProcesses.\n"
        << "If necessary, you can check this by calling isSubProcess first.\n";
  }
  return *parentProcessContext_;
}

void ProcessContext::setProcessConfiguration(
    ProcessConfiguration const* processConfiguration) {
  processConfiguration_ = processConfiguration;
}

void ProcessContext::setParentProcessContext(
...
std::ostream& operator<<(std::ostream& os, ProcessContext const& pc) {
  ...
  if (pc.processConfiguration()) {
    os << pc.processConfiguration()->processName() << " "
       << pc.processConfiguration()->parameterSetID() << "\n";
  } else {
    os << "invalid\n";
    return os;
  }
