...
ModuleCallingContext::ModuleCallingContext(
    ModuleDescription const* moduleDescription, State state,
...
void ModuleCallingContext::setContext(
    State state, ParentContext const& parent,
...
StreamContext const* ModuleCallingContext::getStreamContext() const {
  ...
  if (mcc->type() == ParentContext::Type::kPlaceInPath) {
    ...
    throw Exception(errors::LogicError)
        << "ModuleCallingContext::getStreamContext() called in context not "
           "linked to a StreamContext\n";
  }
...
GlobalContext const* ModuleCallingContext::getGlobalContext() const {
  ...
  if (mcc->type() != ParentContext::Type::kGlobal) {
    throw Exception(errors::LogicError)
        << "ModuleCallingContext::getGlobalContext() called in context not "
           "linked to a GlobalContext\n";
  }
  return mcc->globalContext();
}

ModuleCallingContext const* ModuleCallingContext::getTopModuleCallingContext()
    const {
...
std::ostream& operator<<(std::ostream& os, ModuleCallingContext const& mcc) {
  ...
  if (mcc.previousModuleOnThread()) {
    if (mcc.type() == ParentContext::Type::kModule &&
        mcc.moduleCallingContext() == mcc.previousModuleOnThread()) {
