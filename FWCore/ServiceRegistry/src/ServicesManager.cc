...
ServicesManager::MakerHolder::MakerHolder(
    std::shared_ptr<ServiceMakerBase> iMaker, ParameterSet& iPSet,
...
ServicesManager::ServicesManager(ServiceToken iToken, ServiceLegacy iLegacy,
                                 std::vector<ParameterSet>& iConfiguration,
                                 bool associate)
    : associatedManager_(associate ? iToken.manager_
  ...
  for (Type2Maker::iterator itType = type2Maker_->begin(),
                            itTypeEnd = type2Maker_->end();
       itType != itTypeEnd; ++itType) {
    configTypes.insert(itType->first);
  }

  TypeSet tokenTypes;
  if (0 != iToken.manager_.get()) {
    for (Type2Service::iterator
             itType = iToken.manager_->type2Service_.begin(),
    ...
    switch (iLegacy) {
      ...
      case kTokenOverrides:
        ...
        // remove from type2Maker the overlapping services so we never try to
        // make them
        for (IntersectionType::iterator itType = intersection.begin(),
                                        itTypeEnd = intersection.end();
             itType != itTypeEnd; ++itType) {
          Type2Maker::iterator itFound = type2Maker_->find(*itType);
          // HLT needs it such that even if a service isn't created we store its
          // PSet if needed
      ...
      case kConfigurationOverrides:
        // get all the services from the Configuration, except process wide
        // services
        type2Service_ = iToken.manager_->type2Service_;
        ...
        for (IntersectionType::iterator itType = intersection.begin(),
          ...
          if (itFound->second.maker_->processWideService()) {
            // This is a process wide service, so the token overrides the
            // configuration.
            // HLT needs it such that even if a service isn't created we store
            // its PSet if needed
            if (itFound->second.maker_->saveConfiguration()) {
              itFound->second.pset_->addUntrackedParameter("@save_config",
                                                           true);
            }
            ...
            LogInfo("Configuration")
                ...
                << "This particular service may not be reconfigured in a "
                   "subprocess.\n"
                << "The reconfiguration will be ignored.\n";
            type2Maker_->erase(itFound);
          } else {
            // This is not a process wide service, so the configuration
            // overrides the token.
...
ServicesManager::~ServicesManager() {
  ...
  // do not get destroyed until the ServicesManager object that created them is
  // destroyed
  // which occurs after the body of this destructor is executed (the correct
  // order).
  // Services directly passed in by a put and not created in the constructor
  // may or not be detroyed in the desired order because this class does not
  // control
  // their creation (as I'm writing this comment everything in a standard cmsRun
  // executable is destroyed in the desired order).
  for (std::vector<TypeIDBase>::const_reverse_iterator
           idIter = actualCreationOrder_.rbegin(),
...
void ServicesManager::fillListOfMakers(
    std::vector<ParameterSet>& iConfiguration) {
  for (std::vector<ParameterSet>::iterator itParam = iConfiguration.begin(),
                                           itParamEnd = iConfiguration.end();
       itParam != itParamEnd; ++itParam) {
    std::shared_ptr<ServiceMakerBase> base(ServicePluginFactory::get()->create(
    ...
    Type2Maker::iterator itFound =
        type2Maker_->find(TypeIDBase(base->serviceType()));
    ...
    type2Maker_->insert(
        Type2Maker::value_type(TypeIDBase(base->serviceType()),
                               MakerHolder(base, *itParam, registry_)));
...
void ServicesManager::createServiceFor(MakerHolder const& iMaker) {
  std::string serviceType =
      iMaker.pset_->getParameter<std::string>("@service_type");
...
void ServicesManager::createServices() {
  ...
  // when this goes out of scope, it will revert back to the previous Service
  // set
  ServiceRegistry::Operate operate(token);

  // Now, make each Service.  If a service depends on a service that has yet to
  // be
  // created, that other service will automatically be made

  for (std::vector<TypeIDBase>::const_iterator
           idIter = requestedCreationOrder_.begin(),
