...
ParentContext::ParentContext() : type_(Type::kInvalid) {
...
ModuleCallingContext const* ParentContext::moduleCallingContext() const {
  if (type_ != Type::kModule) {
    throw Exception(errors::LogicError) << "ParentContext::"
                                           "moduleCallingContext called for "
                                           "incorrect type of context";
  }
...
PlaceInPathContext const* ParentContext::placeInPathContext() const {
  if (type_ != Type::kPlaceInPath) {
    throw Exception(errors::LogicError) << "ParentContext::placeInPathContext "
                                           "called for incorrect type of "
                                           "context";
  }
...
InternalContext const* ParentContext::internalContext() const {
  if (type_ != Type::kInternal) {
    throw Exception(errors::LogicError) << "ParentContext::internalContext "
                                           "called for incorrect type of "
                                           "context";
  }
...
std::ostream& operator<<(std::ostream& os, ParentContext const& pc) {
  if (pc.type() == ParentContext::Type::kGlobal && pc.globalContext()) {
    os << *pc.globalContext();
  } else if (pc.type() == ParentContext::Type::kInternal &&
             pc.internalContext()) {
    os << *pc.internalContext();
  } else if (pc.type() == ParentContext::Type::kModule &&
             pc.moduleCallingContext()) {
    os << *pc.moduleCallingContext();
  } else if (pc.type() == ParentContext::Type::kPlaceInPath &&
             pc.placeInPathContext()) {
