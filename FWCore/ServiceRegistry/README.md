...
Signal actions are functors registered to be called when a signal is emitted. When a signal is emitted, all signal actions are called.

**Generally it is best for the signal actions to not throw exceptions.** The framework will catch and handle such exceptions (by gracefully terminating the process), but the detailed behavior is unspecified and should not be relied upon. If any of the actions throws an exception, the `emit()` call will throw the first of those exceptions.
...
**The `Post` signal actions should not throw exceptions.** For example, in many cases those would lead to `std::terminate()` being called, and in some cases the exception would be ignored.
...
The naming of `ActivityRegistry` member objects, that are used for emitting the signals, follow a pattern where the first character is in lower case, and `Signal_` is appended to the end of the signal type name. For example, the object for `PostServicesConstruction` is `postServicesConstructionSignal_`.

The order of the signals in this list is an approximate order in which they would be emitted in a typical single-thread job (to be kept consistent with the `ActivityRegistry.h`).
