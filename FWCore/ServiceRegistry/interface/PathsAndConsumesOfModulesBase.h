...
namespace edm {
  ...
  public:
    ...
    // ConsumesInfo objects for all transitions are in the same vector,
    ...
    std::vector<EventSetupConsumesInfo> eventSetupConsumesInfo(unsigned int moduleID) const {
      return doEventSetupConsumesInfo(moduleID);
    }
    ...
    std::vector<eventsetup::ESProductResolverProvider const*> const& allESProductResolverProviders() const {
      return doAllESProductResolverProviders();
    }
    ...
    // in the vector returned by allESProductResolverProviders (same size and order).
    ...
    // includes producers for all transitions together in the same vector and
    ...
    // Returns a container of information for one ESProducer. The outer vector is
    // indexed by the transitionID, which is a counter of setWhatProduced calls in
    ...
    std::vector<std::vector<EventSetupConsumesInfo>> eventSetupConsumesInfo(ESProducer const& esProducer) const {
      return doEventSetupConsumesInfo(esProducer);
    }
  ...
  private:
    ...
    virtual std::vector<EventSetupConsumesInfo> doEventSetupConsumesInfo(unsigned int moduleID) const = 0;
    virtual unsigned int doLargestModuleID() const = 0;
    virtual std::vector<eventsetup::ESProductResolverProvider const*> const& doAllESProductResolverProviders() const = 0;
    ...
    virtual std::vector<std::vector<EventSetupConsumesInfo>> doEventSetupConsumesInfo(
        ESProducer const& esProducer) const = 0;
  };
}  // namespace edm
#endif
