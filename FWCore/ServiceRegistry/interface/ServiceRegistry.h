...
//
/**\class ServiceRegistry ServiceRegistry.h
...
class ServiceRegistry {
 ...
 public:  // Made public (temporarily) at the request of Emilio Meschi.
  ...
  static ServiceToken createSet(std::vector<ParameterSet>&, ServiceToken,
  ...
  static ServiceToken createContaining(std::unique_ptr<T> iService) {
    ...
    auto wrapper = std::make_shared<serviceregistry::ServiceWrapper<T> >(
        std::move(iService));
    manager->put(wrapper);
    return manager;
  }
  ...
  static ServiceToken createContaining(std::unique_ptr<T> iService,
    ...
    std::vector<edm::ParameterSet> config;
    auto manager = std::make_shared<serviceregistry::ServicesManager>(
        iToken, iLegacy, config);
    auto wrapper = std::make_shared<serviceregistry::ServiceWrapper<T> >(
        std::move(iService));
    manager->put(wrapper);
    return manager;
  ...
  template <typename T>
  static ServiceToken createContaining(
      std::shared_ptr<serviceregistry::ServiceWrapper<T> > iWrapper) {
    std::vector<edm::ParameterSet> config;
    auto manager = std::make_shared<serviceregistry::ServicesManager>(config);
  ...
  static ServiceToken createContaining(
      std::shared_ptr<serviceregistry::ServiceWrapper<T> > iWrapper,
      ServiceToken iToken, serviceregistry::ServiceLegacy iLegacy) {
    std::vector<edm::ParameterSet> config;
    auto manager = std::make_shared<serviceregistry::ServicesManager>(
        iToken, iLegacy, config);
    manager->put(iWrapper);
