...
//
/**\class ServicesManager ServicesManager.h
...
class ServicesManager {
 public:
  struct MakerHolder {
    MakerHolder(std::shared_ptr<ServiceMakerBase> iMaker, ParameterSet& iPSet,
                ActivityRegistry&);
    bool add(ServicesManager&) const;

    edm::propagate_const<std::shared_ptr<ServiceMakerBase>> maker_;
    ParameterSet* pset_;
    ActivityRegistry* registry_;  // We do not use propagate_const because the
                                  // registry itself is mutable
    mutable bool wasAdded_;
  };
  typedef std::map<TypeIDBase, std::shared_ptr<ServiceWrapperBase>>
      Type2Service;
  ...
  */
  ...
  template <typename T>
  T& get() const {
    Type2Service::const_iterator itFound =
        type2Service_.find(TypeIDBase(typeid(T)));
    ...
    if (itFound == type2Service_.end()) {
      ...
      if (0 == type2Maker_.get() ||
          type2Maker_->end() ==
              (itFoundMaker = type2Maker_->find(TypeIDBase(typeid(T))))) {
        Exception::throwThis(errors::NotFound,
                             "Service Request unable to find requested service "
                             "with compiler type name '",
                             typeid(T).name(), "'.\n");
      } else {
        const_cast<ServicesManager&>(*this).createServiceFor(
            itFoundMaker->second);
    ...
    std::shared_ptr<ServiceWrapper<T>> ptr(
        std::dynamic_pointer_cast<ServiceWrapper<T>>(itFound->second));
    assert(0 != ptr.get());
    return ptr->get();
  }
