...
class RandomEngineSentry {
 public:
  explicit RandomEngineSentry(T* t, CLHEP::HepRandomEngine* engine)
      : t_(t), engine_(engine) {
    if (t) {
  ...
  explicit RandomEngineSentry(T* t, StreamID const& streamID)
      : t_(t), engine_(nullptr) {
    if (t) {
      Service<RandomNumberGenerator> rng;
      if (!rng.isAvailable()) {
        throw cms::Exception("Configuration")
            << "Attempt to get a random engine when the "
               "RandomNumberGeneratorService is not configured.\n"
  ...
  explicit RandomEngineSentry(T* t, LuminosityBlockIndex const& lumi)
      : t_(t), engine_(nullptr) {
    if (t) {
      Service<RandomNumberGenerator> rng;
      if (!rng.isAvailable()) {
  ...
  CLHEP::HepRandomEngine const* randomEngine() const {
    return get_underlying_safe(engine_);
  }
