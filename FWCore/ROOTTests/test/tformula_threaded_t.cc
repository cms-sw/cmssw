...
int main(int argc, char** argv) {
  ...
  TThread::Initialize();
  // When threading, also have to keep ROOT from logging all TObjects into a
  // list
  ...
  for (int i = 0; i < kNThreads; ++i) {
    threads.emplace_back([i, &canStart]() {
      ...
      TFormula f("testFormula",
                 "1./(1.+(4.61587e+06*(((1./(0.5*TMath::Max(1.e-6,x+1.)))-1.)/"
                 "1.16042e+07)))");
