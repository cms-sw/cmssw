...
EventSkipperByID::EventSkipperByID(ParameterSet const& pset)
    :  // The default value provided as the second argument to the
       // getUntrackedParameter function call
      // is not used when the ParameterSet has been validated and the parameters
      // are not optional
      // in the description.  As soon as all primary input sources and all
      // modules with a secondary
      // input sources have defined descriptions, the defaults in the
      // getUntrackedParameterSet function
      // calls can and should be deleted from the code.
      firstRun_(pset.getUntrackedParameter<unsigned int>("firstRun", 1U)),
      firstLumi_(
          pset.getUntrackedParameter<unsigned int>("firstLuminosityBlock", 0U)),
      ...
      whichEventsToSkip_(pset.getUntrackedParameter<std::vector<EventRange> >(
          "eventsToSkip", std::vector<EventRange>())),
      whichEventsToProcess_(
          pset.getUntrackedParameter<std::vector<EventRange> >(
              "eventsToProcess", std::vector<EventRange>())),
      skippingLumis_(
          !(whichLumisToSkip_.empty() && whichLumisToProcess_.empty())),
      skippingEvents_(
          !(whichEventsToSkip_.empty() && whichEventsToProcess_.empty())),
      somethingToSkip_(
          skippingLumis_ || skippingEvents_ ||
...
std::unique_ptr<EventSkipperByID> EventSkipperByID::create(
...
// Determines whether a run, lumi, or event will be skipped based on the run,
// lumi, and event number.
// This function is called by a predicate, so it must not modify the state of
// the EventSkipperByID_ object.
// The mutable lumi_ and event_ data members are just temporary caches, so they
// may be modified.
bool EventSkipperByID::skipIt(RunNumber_t run, LuminosityBlockNumber_t lumi,
  ...
  if (run == firstRun_) {
    ...
    if (lumi < firstLumi_) {
      // This lumi or event entry is for a lumi prior to the first lumi to be
      // processed.  Skip it.
      return true;
    }
    if (firstLumi_ == 0 || lumi == firstLumi_) {
      // If we get here, this entry is in the first lumi to be processed in the
      // first run.
      // Note that if firstLumi_ == 0, we are processing all lumis in the run.
      if (event != 0U && event < firstEvent_) {
        // This is an event entry prior to the first event to be processed. Skip
        // it.
        return true;
      }
    }
  ...
  if (skippingLumis()) {
    // If we get here, the entry was not skipped due to firstRun,
    // firstLuminosityBlock, and/or firstEvent.
    LuminosityBlockID lumiID = LuminosityBlockID(run, lumi);
    LuminosityBlockRange lumiRange = LuminosityBlockRange(lumiID, lumiID);
    bool (*lt)(LuminosityBlockRange const&, LuminosityBlockRange const&) =
        &lessThan;
    if (binary_search_all(whichLumisToSkip_, lumiRange, lt)) {
      // The entry is in a lumi specified in whichLumisToSkip.  Skip it.
      return true;
    }
    if (!whichLumisToProcess_.empty() &&
        !binary_search_all(whichLumisToProcess_, lumiRange, lt)) {
      // The entry is not in a lumi specified in non-empty whichLumisToProcess.
      // Skip it.
      return true;
    }
  }
  ...
  if (skippingEvents()) {
    ...
    if (!whichEventsToProcess_.empty() &&
        !binary_search_all(whichEventsToProcess_, eventRange, lt) &&
        !binary_search_all(whichEventsToProcess_, eventRangeNoLumi, lt)) {
      // The entry is not an event specified in non-empty whichEventsToProcess.
      // Skip it.
      return true;
    }
  }
...
void EventSkipperByID::fillDescription(ParameterSetDescription& desc) {
  ...
  desc.addUntracked<unsigned int>("firstLuminosityBlock", 0U)
      ->setComment(
          "Skip any lumi in run 'firstRun' with lumi number < "
  ...
  desc.addNode(
      ...
      ->setComment(
          ...
          "uint64, default:1\n"
          "If 'firstLuminosityBlock' == 0, skip any event in run 'firstRun' "
          "with event number < 'firstEvent'.\n"
          "If 'firstLuminosityBlock' != 0, skip any event in lumi "
          "'firstRun:firstLuminosityBlock' with event number < 'firstEvent'.");

  std::vector<LuminosityBlockRange> defaultLumis;
  desc.addUntracked<std::vector<LuminosityBlockRange> >("lumisToSkip",
                                                        defaultLumis)
      ->setComment(
          "Skip any lumi inside the specified run:lumi range. In python do "
          "'help(cms.LuminosityBlockRange)' for documentation.");
  desc.addUntracked<std::vector<LuminosityBlockRange> >("lumisToProcess",
                                                        defaultLumis)
      ->setComment(
          "If not empty, skip any lumi outside the specified run:lumi range. "
          "In python do 'help(cms.LuminosityBlockRange)' for documentation.");

  std::vector<EventRange> defaultEvents;
  desc.addUntracked<std::vector<EventRange> >("eventsToSkip", defaultEvents)
      ->setComment(
          "Skip any event inside the specified run:event or run:lumi:event "
          "range. In python do 'help(cms.EventRange)' for documentation.");
  desc.addUntracked<std::vector<EventRange> >("eventsToProcess", defaultEvents)
      ->setComment(
          "If not empty, skip any event outside the specified run:event or "
          "run:lumi:event range. In python do 'help(cms.EventRange)' for "
          "documentation.");
}
}
