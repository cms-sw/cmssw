...
EDM_REGISTER_PLUGINFACTORY(edm::VectorInputSourcePluginFactory,
                           "CMS EDM Framework VectorInputSource");
...
VectorInputSourceFactory::makeVectorInputSource(
  ...
  FDEBUG(1) << "VectorInputSourceFactory: module_type = " << modtype
            << std::endl;
  std::unique_ptr<VectorInputSource> wm(
      VectorInputSourcePluginFactory::get()->create(modtype, conf, desc));
  ...
  FDEBUG(1) << "VectorInputSourceFactory: created input source " << modtype
            << std::endl;
