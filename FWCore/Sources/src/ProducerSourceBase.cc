...
ProducerSourceBase::ProducerSourceBase(ParameterSet const& pset,
    ...
    : InputSource(pset, desc),
      numberEventsInRun_(pset.getUntrackedParameter<unsigned int>(
          "numberEventsInRun", remainingEvents())),
      numberEventsInLumi_(pset.getUntrackedParameter<unsigned int>(
          "numberEventsInLuminosityBlock", remainingEvents())),
      presentTime_(pset.getUntrackedParameter<unsigned long long>(
          "firstTime", 1ULL)),  // time in ns
      origTime_(presentTime_),
      timeBetweenEvents_(pset.getUntrackedParameter<unsigned long long>(
          "timeBetweenEvents", kNanoSecPerSec / kAveEventPerSec)),
      eventCreationDelay_(
          pset.getUntrackedParameter<unsigned int>("eventCreationDelay", 0)),
      numberEventsInThisRun_(0),
      numberEventsInThisLumi_(0),
      zerothEvent_(
          pset.existsAs<unsigned int>("firstEvent", false)
              ? pset.getUntrackedParameter<unsigned int>("firstEvent", 1) - 1
              : pset.getUntrackedParameter<unsigned long long>("firstEvent",
                                                               1) -
                    1),
      eventID_(
          pset.getUntrackedParameter<unsigned int>("firstRun", 1),
  ...
  // We need to map this string to the EventAuxiliary::ExperimentType
  // enumeration
  // std::string eType =
  // pset.getUntrackedParameter<std::string>("experimentType",
  // std::string("Any"))),
}
...
std::shared_ptr<RunAuxiliary> ProducerSourceBase::readRunAuxiliary_() {
  ...
  resetNewRun();
  return std::make_shared<RunAuxiliary>(eventID_.run(), ts,
...
void ProducerSourceBase::beginJob() {
  // initialize cannot be called from the constructor, because it is a virtual
  // function
  // that needs to be invoked from a derived class if the derived class
  // overrides it.
  initialize(eventID_, presentTime_, timeBetweenEvents_);
}

void ProducerSourceBase::beginRun(Run&) {}

void ProducerSourceBase::endRun(Run&) {}

void ProducerSourceBase::beginLuminosityBlock(LuminosityBlock&) {}

void ProducerSourceBase::endLuminosityBlock(LuminosityBlock&) {}
...
void ProducerSourceBase::retreatToPrevious(EventID& eventID,
                                           TimeValue_t& time) {
  if (numberEventsInRun_ < 1 || numberEventsInThisRun_ > 0) {
    ...
    eventID =
        eventID.previousRunLastEvent(origEventID_.luminosityBlock() +
                                     numberEventsInRun_ / numberEventsInLumi_);
    eventID =
        EventID(numberEventsInRun_, eventID.luminosityBlock(), eventID.run());
    numberEventsInThisLumi_ = numberEventsInLumi_;
...
void ProducerSourceBase::fillDescription(ParameterSetDescription& desc) {
  ...
  desc.addUntracked<unsigned long long>("firstTime", 1)
      ->setComment("Time before first event (ns) (for timestamp).");
  desc.addUntracked<unsigned long long>("timeBetweenEvents",
                                        kNanoSecPerSec / kAveEventPerSec)
  ...
  desc.addNode(
          edm::ParameterDescription<unsigned int>("firstEvent", 1U, false) xor
          edm::ParameterDescription<unsigned long long>("firstEvent", 1ULL,
                                                        false))
      ->setComment(
          "'firstEvent' is an XOR group because it can have type uint32 or "
          "uint64, default:1\n"
  ...
  desc.addUntracked<unsigned int>("firstRun", 1)
      ->setComment("Run number of first run to generate.");
