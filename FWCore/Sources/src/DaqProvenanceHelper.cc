...
DaqProvenanceHelper::DaqProvenanceHelper(TypeID const& rawDataType)
    : constBranchDescription_(
          BranchDescription(InEvent, "rawDataCollector"
                            //, "source"
                            ,
                            "LHC"
                            // , "HLT"
                            ...
                            "FedRawDataInputSource", ParameterSetID(),
                            TypeWithDict(rawDataType.typeInfo()), false)),
  ...
  // We don't currently use the untracked parameters, However, we make them
  ...
  processParameterSet_.addParameter<ParameterSet>("@trigger_paths",
                                                  triggerPaths);
...
ProcessHistoryID DaqProvenanceHelper::daqInit(
  ...
  ProcessHistory ph;
  ph.emplace_back(constBranchDescription_.processName(),
                  processParameterSet_.id(), getReleaseVersion(), getPassID());
...
bool DaqProvenanceHelper::matchProcesses(ProcessConfiguration const& newPC,
                                         ProcessHistory const& ph) const {
  for (auto const& pc : ph) {
    if (pc.processName() == oldProcessName_) {
      return (pc.releaseVersion() == newPC.releaseVersion() &&
              pc.passID() == newPC.passID());
    }
  }
  return false;
}

void DaqProvenanceHelper::fixMetaData(std::vector<ProcessConfiguration>& pcv,
                                      std::vector<ProcessHistory>& phv) {
  phv.push_back(ProcessHistory());  // For new processHistory, containing only
                                    // processConfiguration_
  std::vector<ProcessConfiguration> newPCs;
  for (auto const& pc : pcv) {
    if (pc.processName() == oldProcessName_) {
      newPCs.emplace_back(constBranchDescription_.processName(),
                          processParameterSet_.id(), pc.releaseVersion(),
                          pc.passID());
    }
  }
  assert(!newPCs.empty());
...
void DaqProvenanceHelper::fixMetaData(
...
ProcessHistoryID const& DaqProvenanceHelper::mapProcessHistoryID(
...
ParentageID const& DaqProvenanceHelper::mapParentageID(
...
BranchID const& DaqProvenanceHelper::mapBranchID(
    BranchID const& branchID) const {
  return (branchID == oldBranchID_ ? newBranchID_ : branchID);
}

void DaqProvenanceHelper::setOldParentageIDToNew(ParentageID const& iOld,
