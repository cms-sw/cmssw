...
def main():
    inputName = 'muTau.root'
    outputName = 'muTauSys.root'
    ...
    for dirKey in inFile.GetListOfKeys():
        ...
        for histKey in subDir.GetListOfKeys():
            ...
            if 'QCD' == histKey.GetName() and ('1jet' in dirKey.GetName() or 'vbf' in dirKey.GetName()):
                ...
                # QCDShape_mutau_1jet_medium_8TeV
                outFile.cd()
                subdir = outFile.mkdir(dirKey.GetName())
                subdir.cd()
                objUp.Write(histKey.GetName() + '_CMS_htt_QCDShape_' + dirKey.GetName().replace('muTau', 'mutau') + '_8TeVUp')
                objDown.Write(histKey.GetName() + '_CMS_htt_QCDShape_' + dirKey.GetName().replace('muTau', 'mutau') + '_8TeVDown')
                # CMS_htt_QCDShape_

            ### --- Uncorrelated tau ES uncertainty --- ###
            if 'CMS_scale_t_mutau_8TeV' in histKey.GetName():
                inFile.cd()
                obj = subDir.Get(histKey.GetName())
                ...
                if 'jet_medium' in dirKey.GetName():
                    ...
                    cat = '_high'
                objUncorr.Write(histKey.GetName().replace('CMS_scale_t_mutau_8TeV', 'CMS_scale_t_mutau{cat}_8TeV'.format(cat=cat)))

    outFile.Write()
    outFile.Close()

    os.system('hadd -f htt_mt.inputs-sm-8TeV.root muTau.root muTauSys.root')
