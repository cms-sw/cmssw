...
def main():
    for mode in ['', '-fb']:
        inputName = 'htt_mt.inputs-mssm-8TeV-0{mode}.root'.format(mode=mode)
        outputName = 'htt_mt.inputs-mssm-8TeV-0{mode}_sys.root'.format(mode=mode)
        ...
        for dirKey in inFile.GetListOfKeys():
            ...
            for histKey in subDir.GetListOfKeys():
                ...
                if histKey.GetName().startswith('QCD'):
                    ...
                    subdir.cd()
                    outName = 'QCD_CMS_htt_QCDShape_' + dirKey.GetName().replace('muTau', 'mutau')
                    fb = ''
        ...
        outFile.Write()
        outFile.Close()
        os.system('cp htt_mt.inputs-mssm-8TeV-0{mode}.root htt_mt.inputs-mssm-8TeV-0{mode}_tmp.root'.format(mode=mode))
        os.system('hadd -f htt_mt.inputs-mssm-8TeV-0{mode}.root htt_mt.inputs-mssm-8TeV-0{mode}_tmp.root htt_mt.inputs-mssm-8TeV-0{mode}_sys.root'.format(mode=mode))
