#include "../interface/PileupJetIdAlgoSubstructure.h"
#include "../interface/Nsubjettiness.h"
...
// ------------------------------------------------------------------------------------------
PileupJetIdAlgoSubstructure::PileupJetIdAlgoSubstructure(const edm::ParameterSet & ps) 
{
	...
	for(int i0 = 0; i0 < 3; i0++) { 
	  ...
	  if(i0 == PileupJetIdentifierSubstructure::kMedium) lCutType = "Medium";
	  if(i0 == PileupJetIdentifierSubstructure::kLoose)  lCutType = "Loose";
	  int nCut = 1;
	  if(cutBased_) nCut++;
...
// ------------------------------------------------------------------------------------------
PileupJetIdAlgoSubstructure::PileupJetIdAlgoSubstructure(int version,
...
// ------------------------------------------------------------------------------------------
void PileupJetIdAlgoSubstructure::setup()
{
	initVariables();
...
// ------------------------------------------------------------------------------------------
PileupJetIdAlgoSubstructure::~PileupJetIdAlgoSubstructure() 
{
	if( reader_ ) {
		delete reader_;
	}
...
// ------------------------------------------------------------------------------------------
void  PileupJetIdAlgoSubstructure::assign(const std::vector<float> & vec, float & a, float & b, float & c, float & d )
{
	size_t sz = vec.size();
	a = ( sz > 0 ? vec[0] : 0. );
	b = ( sz > 1 ? vec[1] : 0. );
...
// ------------------------------------------------------------------------------------------
void  PileupJetIdAlgoSubstructure::setPtEtaPhi(const reco::Candidate & p, float & pt, float & eta, float &phi )
{
	pt  = p.pt();
	eta = p.eta();
	phi = p.phi();
...
// ------------------------------------------------------------------------------------------
void PileupJetIdAlgoSubstructure::bookReader()
{
...
// ------------------------------------------------------------------------------------------
void PileupJetIdAlgoSubstructure::set(const PileupJetIdentifierSubstructure & id)
{
	internalId_ = id;
}

// ------------------------------------------------------------------------------------------
void PileupJetIdAlgoSubstructure::runMva()
{
	if( cutBased_ ) {
...
// ------------------------------------------------------------------------------------------
std::pair<int,int> PileupJetIdAlgoSubstructure::getJetIdKey(float jetPt, float jetEta)
{
  int ptId = 0;                                                                                                                                    
...
// ------------------------------------------------------------------------------------------
int PileupJetIdAlgoSubstructure::computeCutIDflag(float betaStarClassic,float dR2Mean,float nvtx, float jetPt, float jetEta)
{
  std::pair<int,int> jetIdKey = getJetIdKey(jetPt,jetEta);
  float betaStarModified = betaStarClassic/log(nvtx-0.64);
  int idFlag(0);
  if(betaStarModified < betaStarCut_[PileupJetIdentifierSubstructure::kTight ][jetIdKey.first][jetIdKey.second] && 
     dR2Mean          < rmsCut_     [PileupJetIdentifierSubstructure::kTight ][jetIdKey.first][jetIdKey.second] 
     ) idFlag += 1 <<  PileupJetIdentifierSubstructure::kTight;

  if(betaStarModified < betaStarCut_[PileupJetIdentifierSubstructure::kMedium ][jetIdKey.first][jetIdKey.second] && 
     dR2Mean          < rmsCut_     [PileupJetIdentifierSubstructure::kMedium ][jetIdKey.first][jetIdKey.second] 
     ) idFlag += 1 <<  PileupJetIdentifierSubstructure::kMedium;
  
  if(betaStarModified < betaStarCut_[PileupJetIdentifierSubstructure::kLoose  ][jetIdKey.first][jetIdKey.second] && 
     dR2Mean          < rmsCut_     [PileupJetIdentifierSubstructure::kLoose  ][jetIdKey.first][jetIdKey.second] 
     ) idFlag += 1 <<  PileupJetIdentifierSubstructure::kLoose;
  return idFlag;
}
// ------------------------------------------------------------------------------------------
int PileupJetIdAlgoSubstructure::computeIDflag(float mva, float jetPt, float jetEta)
{
  std::pair<int,int> jetIdKey = getJetIdKey(jetPt,jetEta);
...
// ------------------------------------------------------------------------------------------
int PileupJetIdAlgoSubstructure::computeIDflag(float mva,int ptId,int etaId)
{
  int idFlag(0);
  if(mva > mvacut_[PileupJetIdentifierSubstructure::kTight ][ptId][etaId]) idFlag += 1 << PileupJetIdentifierSubstructure::kTight;
  if(mva > mvacut_[PileupJetIdentifierSubstructure::kMedium][ptId][etaId]) idFlag += 1 << PileupJetIdentifierSubstructure::kMedium;
  if(mva > mvacut_[PileupJetIdentifierSubstructure::kLoose ][ptId][etaId]) idFlag += 1 << PileupJetIdentifierSubstructure::kLoose;
  return idFlag;
}


// ------------------------------------------------------------------------------------------
PileupJetIdentifierSubstructure PileupJetIdAlgoSubstructure::computeMva()
{
	runMva();
	return PileupJetIdentifierSubstructure(internalId_);
}

// ------------------------------------------------------------------------------------------
PileupJetIdentifierSubstructure PileupJetIdAlgoSubstructure::computeIdVariables(const reco::Jet * jet, float jec, const reco::Vertex * vtx,
							...
							bool calculateMva) 
{
	...
	for(constituents_iterator it=constituents.begin(); it!=constituents.end(); ++it) {
		...
		//Fast Jet
		FJparticles.push_back( fastjet::PseudoJet( icand->px(),icand->py(),icand->pz(),icand->energy() ) );
		// all particles
...
// ------------------------------------------------------------------------------------------
std::string PileupJetIdAlgoSubstructure::dumpVariables() const
{
...
// ------------------------------------------------------------------------------------------
void PileupJetIdAlgoSubstructure::resetVariables()
{
...
// ------------------------------------------------------------------------------------------
void PileupJetIdAlgoSubstructure::initVariables()
{
	internalId_.idFlag_    = 0;
	...
	//Jet Substructure
	INIT_VARIABLE(trimmass   ,"",0.);
	INIT_VARIABLE(trimarea   ,"",0.);
	INIT_VARIABLE(filtermass ,"",0.);
	INIT_VARIABLE(filterarea ,"",0.);
	INIT_VARIABLE(prunedmass ,"",0.);
	INIT_VARIABLE(prunedarea ,"",0.);
	INIT_VARIABLE(nsubjets,"",0.);
	INIT_VARIABLE(massdrop,"",0.);
	INIT_VARIABLE(massdropcorr,"",0.);
	INIT_VARIABLE(tau1,"",0.);
	INIT_VARIABLE(tau2,"",0.);
	INIT_VARIABLE(tau3,"",0.);
	INIT_VARIABLE(tau4,"",0.);
	INIT_VARIABLE(tau2tau1,"",0.);
}
