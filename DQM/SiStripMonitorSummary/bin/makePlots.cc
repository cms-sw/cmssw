...
void makePlots(std::string inputFileName, std::string outputFileName) {
  ...
  plotType.push_back("BadModules");
  plotType.push_back("BadFibers");
  plotType.push_back("BadAPVs");
  plotType.push_back("BadStrips");
  plotType.push_back("BadStripsFromAPVs");
  plotType.push_back("AllBadStrips");
  ...
  for (std::vector<std::string>::iterator itPlot = plotType.begin(); itPlot != plotType.end(); itPlot++) {
    ...
    if (hTracker) {
      ...
      std::cout << histoName << " not found" << std::endl;
    }
    ...
    for (std::vector<std::string>::iterator itSub = subDetName.begin(); itSub != subDetName.end(); itSub++) {
      ...
      if (legend == nullptr) {
        std::cerr << "Unknown itSub type " << *itSub << std::endl;
        assert(false);
      }
...
double findNormFactor(const std::string currentPlotType, const std::string currentPart, const bool stackOption) {
  ...
  if (stackOption) {
    ...
    } else if (currentPlotType == "AllBadStrips" || currentPlotType == "BadStripsFromAPVs" ||
    ...
    } else if (currentPlotType == "BadStrips" || currentPlotType == "BadStripsFromAPVs" ||
