...
SiStripThresholdDQM::SiStripThresholdDQM(const edm::EventSetup &eSetup,
  ...
  if (HistoMaps_On_) {
    ...
    if (WhichThreshold == "Low")
      Tk_HM_L = std::make_unique<TkHistoMap>(tkDetMap, "SiStrip/Histo_Map",
                                             "LowThresh_TkMap", 0.);
    if (WhichThreshold == "High")
      Tk_HM_H = std::make_unique<TkHistoMap>(tkDetMap, "SiStrip/Histo_Map",
                                             "HighThresh_TkMap", 0.);
  }
}
...
// -----
void SiStripThresholdDQM::fillModMEs(
...
void SiStripThresholdDQM::fillMEsForDet(const ModMEs &_selModME_,
                                        uint32_t selDetId_,
  ...
  SiStripThreshold::Range ThresholdRange =
      thresholdHandle_->getRange(selDetId_);
...
//=======================================================================================
// -----
void SiStripThresholdDQM::fillSummaryMEs(
    const std::vector<uint32_t> &selectedDetIds, const edm::EventSetup &es) {
...
void SiStripThresholdDQM::fillMEsForLayer(
  ...
  if (hPSet_.getParameter<bool>("FillSummaryAtLayerLevel")) {
    ...
    std::vector<uint32_t> sameLayerDetIds_ =
        GetSameLayerDetId(activeDetIds, selDetId_, tTopo);
