...
SiStripQualityDQM::SiStripQualityDQM(const edm::EventSetup &eSetup,
  ...
  if (HistoMaps_On_) {
    ...
    Tk_HM_ = std::make_unique<TkHistoMap>(
        tkDetMapHandle.product(), "SiStrip/Histo_Map", "Quality_TkMap", 0.);
  }
}
// -----
...
//===================================================
// -----
void SiStripQualityDQM::fillMEsForDet(const ModMEs &_selModME_,
                                      uint32_t selDetId_,
  ...
  for (int istrip = 0; istrip < nStrip; ++istrip) {
    selModME_.ProfileDistr->Fill(
        istrip + 1, qualityHandle_->IsStripBad(qualityRange, istrip) ? 0. : 1.);
...
void SiStripQualityDQM::fillMEsForLayer(
  ...
  if (hPSet_.getParameter<bool>("FillSummaryAtLayerLevel")) {
    ...
    //%%%%%%%%%%%%%%%%%%%%%%%
    if (std::binary_search(alreadyFilledLayers.begin(),
                           alreadyFilledLayers.end(), sameLayerDetIds_[0]))
      return;
    ...
    for (unsigned int i = 0; i < sameLayerDetIds_.size(); i++) {
      ...
      SiStripQuality::Range qualityRange =
          qualityHandle_->getRange(sameLayerDetIds_[i]);
      int nStrip =
          reader->getNumberOfApvsAndStripLength(sameLayerDetIds_[i]).first *
          128;
...
void SiStripQualityDQM::fillGrandSummaryMEs(const edm::EventSetup &eSetup) {
  ...
  std::string hSummary_BadObjects_xTitle =
      hPSet_.getParameter<std::string>("Summary_BadObjects_histo_xTitle");

  std::string hSummary_BadModules_name =
      hPSet_.getParameter<std::string>("Summary_BadModules_histo_name");
  std::string hSummary_BadModules_yTitle =
      hPSet_.getParameter<std::string>("Summary_BadModules_histo_yTitle");

  std::string hSummary_BadFibers_name =
      hPSet_.getParameter<std::string>("Summary_BadFibers_histo_name");
  std::string hSummary_BadFibers_yTitle =
      hPSet_.getParameter<std::string>("Summary_BadFibers_histo_yTitle");

  std::string hSummary_BadApvs_name =
      hPSet_.getParameter<std::string>("Summary_BadApvs_histo_name");
  std::string hSummary_BadApvs_yTitle =
      hPSet_.getParameter<std::string>("Summary_BadApvs_histo_yTitle");

  std::string hSummary_BadStrips_name =
      hPSet_.getParameter<std::string>("Summary_BadStrips_histo_name");
  std::string hSummary_BadStrips_yTitle =
      hPSet_.getParameter<std::string>("Summary_BadStrips_histo_yTitle");
  ...
  ME[0] = dqmStore_->book1D(hSummary_BadModules_name, hSummary_BadModules_name,
                            NchX, LowX, HighX);
  ME[0]->setAxisTitle(hSummary_BadObjects_xTitle, 1);
  ME[0]->setAxisTitle(hSummary_BadModules_yTitle, 2);

  ME[1] = dqmStore_->book1D(hSummary_BadFibers_name, hSummary_BadFibers_name,
                            NchX, LowX, HighX);
  ME[1]->setAxisTitle(hSummary_BadObjects_xTitle, 1);
  ME[1]->setAxisTitle(hSummary_BadFibers_yTitle, 2);

  ME[2] = dqmStore_->book1D(hSummary_BadApvs_name, hSummary_BadApvs_name, NchX,
                            LowX, HighX);
  ME[2]->setAxisTitle(hSummary_BadObjects_xTitle, 1);
  ME[2]->setAxisTitle(hSummary_BadApvs_yTitle, 2);

  ME[3] = dqmStore_->book1D(hSummary_BadStrips_name, hSummary_BadStrips_name,
                            NchX, LowX, HighX);
...
//=====================
void SiStripQualityDQM::SetBadComponents(int i, int component,
