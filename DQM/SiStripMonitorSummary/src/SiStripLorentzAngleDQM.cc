  ...
  if (SummaryOnStringLevel_On_) { // FILLING FOR STRING LEVEL
    ...
    for (unsigned int i = 0; i < sameLayerDetIds_.size(); i++) {
      selME_.SummaryOfProfileDistr->Fill(
          i + 1, lorentzangleHandle_->getLorentzAngle(sameLayerDetIds_[i]));

      // Fill the Histo_TkMap+TkMap with the LA:
      if (HistoMaps_On_)
        Tk_HM_->fill(sameLayerDetIds_[i],
                     lorentzangleHandle_->getLorentzAngle(sameLayerDetIds_[i]));
