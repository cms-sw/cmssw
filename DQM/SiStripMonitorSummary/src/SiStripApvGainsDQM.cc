...
SiStripApvGainsDQM::SiStripApvGainsDQM(const edm::EventSetup &eSetup,
  ...
  if (HistoMaps_On_) {
    ...
    Tk_HM_ = std::make_unique<TkHistoMap>(
        tkDetMapHandle.product(), "SiStrip/Histo_Map", "MeanApvGain_TkMap", 0.);
  }
}
// -----
...
// -----
void SiStripApvGainsDQM::fillMEsForDet(const ModMEs &_selModME_,
                                       uint32_t selDetId_,
  ...
  for (int iapv = 0; iapv < nApv; ++iapv) {
    if (CondObj_fillId_ == "onlyProfile" ||
      ...
      selModME_.ProfileDistr->Fill(iapv + 1,
  ...
  if (hPSet_.getParameter<bool>("FillSummaryProfileAtLayerLevel")) {
    ...
    for (int iapv = 0; iapv < nApv; ++iapv) {
      ...
      selME_.SummaryOfProfileDistr->Fill(
          iapv + 1, gainHandle_->getApvGain(iapv, gainRange));
