...
SiStripBaseCondObjDQM::SiStripBaseCondObjDQM(const edm::EventSetup &eSetup,
    ...
    : eSetup_(eSetup), hPSet_(hPSet), fPSet_(fPSet), cacheID_memory(0),
      dqmStore_(edm::Service<DQMStore>().operator->()), runNumber_(iRun) {

  reader = new SiStripDetInfoFileReader(
      edm::FileInPath(
          std::string("CalibTracker/SiStripCommon/data/SiStripDetInfo.dat"))
          .fullPath());
  ...
  SummaryOnStringLevel_On_ =
      fPSet_.getParameter<bool>("SummaryOnStringLevel_On");
...
void SiStripBaseCondObjDQM::analysis(const edm::EventSetup &eSetup_) {
  ...
  if (fPSet_.getParameter<bool>("ActiveDetIds_On") ||
      hPSet_.getParameter<bool>("ActiveDetIds_On"))
...
void SiStripBaseCondObjDQM::analysisOnDemand(const edm::EventSetup &eSetup_,
  ...
  if (requestedSubDetector == "TIB") {
    SiStripSubStructure::getTIBDetectors(activeDetIds, requestedDetIds_, tTopo,
                                         requestedLayer, 0, 0, 0);
  } else if (requestedSubDetector == "TID") {
    SiStripSubStructure::getTIDDetectors(activeDetIds, requestedDetIds_, tTopo,
                                         requestedSide, requestedLayer, 0, 0);
  } else if (requestedSubDetector == "TOB") {
    SiStripSubStructure::getTOBDetectors(activeDetIds, requestedDetIds_, tTopo,
                                         requestedLayer, 0, 0);
  } else if (requestedSubDetector == "TEC") {
    SiStripSubStructure::getTECDetectors(activeDetIds, requestedDetIds_, tTopo,
                                         requestedSide, requestedLayer, 0, 0, 0,
                                         0);
  }
...
void SiStripBaseCondObjDQM::selectModules(std::vector<uint32_t> &detIds_,
  ...
  edm::LogInfo("SiStripBaseCondObjDQM")
      << "[SiStripBaseCondObjDQM::selectModules] input detIds_: "
  ...
  if (fPSet_.getParameter<bool>("restrictModules")) {
    ...
    std::vector<std::string> included_subdets =
        fPSet_.getParameter<std::vector<std::string>>(
            "ModulesToBeIncluded_DetIdSelector");
    for (std::vector<std::string>::const_iterator wsdps =
             included_subdets.begin();
         wsdps != included_subdets.end(); ++wsdps) {
      included_subdetsels.push_back(DetIdSelector(*wsdps));
    }

    std::vector<uint32_t> modulesToBeIncluded;
    for (std::vector<uint32_t>::const_iterator detid = detIds_.begin();
         detid != detIds_.end(); ++detid) {
      for (std::vector<DetIdSelector>::const_iterator detidsel =
               included_subdetsels.begin();
           detidsel != included_subdetsels.end(); ++detidsel) {
    ...
    for (std::vector<std::string>::const_iterator wsdps =
             excluded_subdets.begin();
         wsdps != excluded_subdets.end(); ++wsdps) {
    ...
    for (std::vector<uint32_t>::const_iterator detid = detIds_.begin();
         detid != detIds_.end(); ++detid) {
      for (std::vector<DetIdSelector>::const_iterator detidsel =
               excluded_subdetsels.begin();
           detidsel != excluded_subdetsels.end(); ++detidsel) {
        if (detidsel->isSelected(*detid)) {
    ...
    ModulesToBeExcluded_ =
        fPSet_.getParameter<std::vector<unsigned int>>("ModulesToBeExcluded");
    ModulesToBeIncluded_ =
        fPSet_.getParameter<std::vector<unsigned int>>("ModulesToBeIncluded");
    SubDetectorsToBeExcluded_ = fPSet_.getParameter<std::vector<std::string>>(
        "SubDetectorsToBeExcluded");
    ...
    modulesToBeIncluded.insert(modulesToBeIncluded.end(),
                               ModulesToBeIncluded_.begin(),
                               ModulesToBeIncluded_.end());
    edm::LogInfo("SiStripBaseCondObjDQM")
        << "[SiStripBaseCondObjDQM::selectModules] modulesToBeIncluded: "
        << modulesToBeIncluded.size() << std::endl;
    modulesToBeExcluded.insert(modulesToBeExcluded.end(),
                               ModulesToBeExcluded_.begin(),
                               ModulesToBeExcluded_.end());
    edm::LogInfo("SiStripBaseCondObjDQM")
        << "[SiStripBaseCondObjDQM::selectModules] modulesToBeExcluded: "
        << modulesToBeExcluded.size() << std::endl;
    ...
    if (!modulesToBeExcluded.empty()) {
      for (std::vector<uint32_t>::const_iterator mod =
               modulesToBeExcluded.begin();
           mod != modulesToBeExcluded.end(); mod++) {

        std::vector<uint32_t>::iterator detid =
            std::lower_bound(detIds_.begin(), detIds_.end(), *mod);
    ...
    if (*(SubDetectorsToBeExcluded_.begin()) != "none") {
      ...
      for (std::vector<std::string>::const_iterator modIter_ =
               SubDetectorsToBeExcluded_.begin();
        ...
        if (*modIter_ == "TIB") {
          ...
          SiStripSubStructure::getTECDetectors(detIds_, tmp, tTopo, 0, 0, 0, 0,
                                               0, 0);
        } else {
          edm::LogWarning("SiStripBaseCondObjDQM")
  ...
  edm::LogInfo("SiStripBaseCondObjDQM")
      << "[SiStripBaseCondObjDQM::selectModules] output detIds_: "
      << detIds_.size() << std::endl;

  // -----
  // *** fill only one Module per layer ***

  if (fPSet_.getParameter<std::string>("ModulesToBeFilled") ==
      "onlyOneModulePerLayer") {
    ...
    for (unsigned int i = 1; i < 10; i++) {
      tmp.clear();
      SiStripSubStructure::getTECDetectors(detIds_, tmp, tTopo, 1, i, 0, 0, 0,
                                           0);
      if (!tmp.empty()) {
        layerDetIds.push_back(*(tmp.begin()));
      }
      SiStripSubStructure::getTECDetectors(detIds_, tmp, tTopo, 2, i, 0, 0, 0,
                                           0);
      if (!tmp.empty()) {
        layerDetIds.push_back(*(tmp.begin()));
      }
...
void SiStripBaseCondObjDQM::getModMEs(ModMEs &CondObj_ME,
  ...
  std::map<uint32_t, ModMEs>::const_iterator ModMEsMap_iter =
  ...
  if (ModMEsMap_iter != ModMEsMap_.end()) {
    ...
    if ((CondObj_fillId_ == "ProfileAndCumul" ||
         CondObj_fillId_ == "onlyProfile") &&
        CondObj_ME.ProfileDistr) {
      CondObj_ME.ProfileDistr->Reset();
    }

    if ((CondObj_fillId_ == "ProfileAndCumul" ||
         CondObj_fillId_ == "onlyCumul") &&
        CondObj_ME.CumulDistr) {
...
void SiStripBaseCondObjDQM::getSummaryMEs(ModMEs &CondObj_ME,
  ...
  if (CondObj_name_ == "lorentzangle" && SummaryOnStringLevel_On_) {
    SummaryMEsMap_iter =
        SummaryMEsMap_.find(getStringNameAndId(detId_, tTopo).second);
  } else if (CondObj_name_ == "bpcorrection" && SummaryOnStringLevel_On_) {
    SummaryMEsMap_iter =
        SummaryMEsMap_.find(getStringNameAndId(detId_, tTopo).second);
  } else {
    SummaryMEsMap_iter =
        SummaryMEsMap_.find(getLayerNameAndId(detId_, tTopo).second);
  }
  ...
  if ((CondObj_fillId_ == "ProfileAndCumul" ||
      ...
      (CondObj_name_ == "pedestal" || CondObj_name_ == "noise" ||
       CondObj_name_ == "lowthreshold" || CondObj_name_ == "highthreshold" ||
       CondObj_name_ == "apvgain" || CondObj_name_ == "bpcorrection" ||
  ...
  if ((CondObj_fillId_ == "ProfileAndCumul" ||
       CondObj_fillId_ == "onlyCumul") &&
      (CondObj_name_ == "lorentzangle" || CondObj_name_ == "bpcorrection" ||
       CondObj_name_ == "noise")) {
  ...
  // apvgain
  if (CondObj_name_ == "noise" || CondObj_name_ == "lowthreshold" ||
      CondObj_name_ == "highthreshold" || CondObj_name_ == "apvgain" ||
      CondObj_name_ == "pedestal" || CondObj_name_ == "quality") {
  ...
  if (CondObj_name_ == "lorentzangle" && SummaryOnStringLevel_On_) {
    ...
    SummaryMEsMap_.insert(
        std::make_pair(getStringNameAndId(detId_, tTopo).second, CondObj_ME));
  } else if (CondObj_name_ == "bpcorrection" && SummaryOnStringLevel_On_) {
    // FIXME getStringNameandId takes time. not need to call it every timne. put
    // the call at the beginning of the method and caache the string
    SummaryMEsMap_.insert(
    ...
    SummaryMEsMap_.insert(
        std::make_pair(getLayerNameAndId(detId_, tTopo).second, CondObj_ME));
  }
}
// ----
...
//====================================================
// -----
void SiStripBaseCondObjDQM::bookProfileMEs(
    SiStripBaseCondObjDQM::ModMEs &CondObj_ME, const uint32_t &detId_,
    const TrackerTopology *tTopo) {
  ...
  hProfile_description =
      hPSet_.getParameter<std::string>("Profile_description");
  ...
  CondObj_ME.ProfileDistr = dqmStore_->book1D(
      hProfile_Name, hProfile, hProfile_NchX, hProfile_LowX, hProfile_HighX);
...
//=============================================
// -----
void SiStripBaseCondObjDQM::bookCumulMEs(
    SiStripBaseCondObjDQM::ModMEs &CondObj_ME, const uint32_t &detId_,
    const TrackerTopology *tTopo) {
  ...
  CondObj_ME.CumulDistr = dqmStore_->book1D(
      hCumul_name, hCumul_title, hCumul_NchX, hCumul_LowX, hCumul_HighX);
...
void SiStripBaseCondObjDQM::bookSummaryProfileMEs(
    SiStripBaseCondObjDQM::ModMEs &CondObj_ME, const uint32_t &detId_,
    const TrackerTopology *tTopo) {
  ...
  hSummaryOfProfile_xTitle =
      hPSet_.getParameter<std::string>("SummaryOfProfile_xTitle");
  hSummaryOfProfile_yTitle =
      hPSet_.getParameter<std::string>("SummaryOfProfile_yTitle");
  ...
  hSummaryOfProfile_HighY =
      hPSet_.getParameter<double>("SummaryOfProfile_HighY");
  ...
  if (CondObj_name_ == "pedestal" || CondObj_name_ == "noise" ||
      CondObj_name_ == "lowthreshold" ||
      CondObj_name_ == "highthreshold") { // plot in strip number

    if ((layerId_ > 610 &&
         layerId_ < 620) || // TID & TEC have 768 strips at maximum
  ...
  } else if (((CondObj_name_ == "lorentzangle" ||
    ...
    case StripSubdetector::TIB:
      SiStripSubStructure::getTIBDetectors(activeDetIds, sameLayerDetIds_,
                                           tTopo, tTopo->tibLayer(detId_), 0, 0,
                                           tTopo->tibString(detId_));
      break;
    ...
    case StripSubdetector::TOB:
      SiStripSubStructure::getTOBDetectors(
          activeDetIds, sameLayerDetIds_, tTopo, tTopo->tobLayer(detId_), 0, 0);
      break;
    ...
    case StripSubdetector::TIB:
      if (tTopo->tibIsInternalString(detId_)) {
        SiStripSubStructure::getTIBDetectors(activeDetIds, sameLayerDetIds_,
                                             tTopo, tTopo->tibLayer(detId_), 0,
                                             1, tTopo->tibString(detId_));
      } else if (tTopo->tibIsExternalString(detId_)) {
        SiStripSubStructure::getTIBDetectors(activeDetIds, sameLayerDetIds_,
                                             tTopo, tTopo->tibLayer(detId_), 0,
                                             2, tTopo->tibString(detId_));
      }
      break;
    case StripSubdetector::TID:
      SiStripSubStructure::getTIDDetectors(activeDetIds, sameLayerDetIds_,
    ...
    if ((layerId_ > 610 &&
         layerId_ < 620) || // TID & TEC have 6 apvs at maximum
        (layerId_ > 620 && layerId_ < 630) ||
        (layerId_ > 410 && layerId_ < 414) ||
        (layerId_ > 420 && layerId_ < 424)) {
      nApv = 6;
  ...
  if ((CondObj_name_ == "lorentzangle" || CondObj_name_ == "bpcorrection") &&
    ...
    hSummaryOfProfile_name = hidmanager.createHistoLayer(
        ...
        getStringNameAndId(detId_, tTopo).first, "");
  } else {
    hSummaryOfProfile_name =
        hidmanager.createHistoLayer(hSummaryOfProfile_description, "layer",
  ...
  CondObj_ME.SummaryOfProfileDistr = dqmStore_->bookProfile(
      hSummaryOfProfile_name, hSummaryOfProfile_title, hSummaryOfProfile_NchX,
      hSummaryOfProfile_LowX, hSummaryOfProfile_HighX, hSummaryOfProfile_NchY,
      0., 0.);
  ...
  CondObj_ME.SummaryOfProfileDistr->setAxisRange(hSummaryOfProfile_LowY,
                                                 hSummaryOfProfile_HighY, 2);
  ...
  if (CondObj_name_ == "lorentzangle" || CondObj_name_ == "bpcorrection") {
    ...
    reverse(sameLayerDetIds_.begin(),
            sameLayerDetIds_.begin() + sameLayerDetIds_.size() / 2);
...
//=============================================================
// -----
void SiStripBaseCondObjDQM::bookSummaryCumulMEs(
    SiStripBaseCondObjDQM::ModMEs &CondObj_ME, const uint32_t &detId_,
    const TrackerTopology *tTopo) {
  ...
  hSummaryOfCumul_xTitle =
      hPSet_.getParameter<std::string>("SummaryOfCumul_xTitle");
  hSummaryOfCumul_yTitle =
      hPSet_.getParameter<std::string>("SummaryOfCumul_yTitle");
  ...
  if ((CondObj_name_ == "lorentzangle" || CondObj_name_ == "bpcorrection") &&
    ...
    hSummaryOfCumul_name =
        hidmanager.createHistoLayer(hSummaryOfCumul_description, "layer",
                                    getLayerNameAndId(detId_, tTopo).first, "");
  }
  ...
  CondObj_ME.SummaryOfCumulDistr = dqmStore_->book1D(
      hSummaryOfCumul_name, hSummaryOfCumul_title, hSummaryOfCumul_NchX,
      hSummaryOfCumul_LowX, hSummaryOfCumul_HighX);
...
void SiStripBaseCondObjDQM::bookSummaryMEs(
    SiStripBaseCondObjDQM::ModMEs &CondObj_ME, const uint32_t &detId_,
    const TrackerTopology *tTopo) {
  ...
  hSummary_name =
      hidmanager.createHistoLayer(hSummary_description, "layer",
                                  getLayerNameAndId(detId_, tTopo).first, "");
  ...
  CondObj_ME.SummaryDistr = dqmStore_->bookProfile(
      hSummary_name, hSummary_title, hSummary_NchX, hSummary_LowX,
      hSummary_HighX, hSummary_NchY, 0., 0.);
  //						   hSummary_LowY,
  //						   hSummary_HighY);
...
SiStripBaseCondObjDQM::getLayerNameAndId(const uint32_t &detId_,
                                         const TrackerTopology *tTopo) {
...
std::pair<std::string, uint32_t>
SiStripBaseCondObjDQM::getStringNameAndId(const uint32_t &detId_,
                                          const TrackerTopology *tTopo) {
  ...
  if (subdetectorId_ == 3) { // TIB
    if (tTopo->tibLayer(detId_) == 1 &&
        tTopo->tibIsInternalString(detId_)) { // 1st layer int
    ...
    } else if (tTopo->tibLayer(detId_) == 1 &&
               tTopo->tibIsExternalString(detId_)) { // 1st layer ext
    ...
    } else if (tTopo->tibLayer(detId_) == 2 &&
               tTopo->tibIsInternalString(detId_)) { // 2nd layer int
      for (unsigned int i = 1; i < 35; i++) {
        if (tTopo->tibString(detId_) == i) {
          ...
          layerStringId_ = 302100 + i;
               ...
               tTopo->tibIsExternalString(detId_)) { // 2nd layer ext
    ...
    } else if (tTopo->tibLayer(detId_) == 3 &&
               tTopo->tibIsInternalString(detId_)) { // 3rd layer int
      for (unsigned int i = 1; i < 45; i++) {
        if (tTopo->tibString(detId_) == i) {
          ...
          layerStringId_ = 303100 + i;
               ...
               tTopo->tibIsExternalString(detId_)) { // 3rd layer ext
    ...
    } else if (tTopo->tibLayer(detId_) == 4 &&
               tTopo->tibIsInternalString(detId_)) { // 4th layer int
      for (unsigned int i = 1; i < 53; i++) {
        if (tTopo->tibString(detId_) == i) {
          ...
          layerStringId_ = 304100 + i;
               ...
               tTopo->tibIsExternalString(detId_)) { // 4th layer ext
...
std::vector<uint32_t> SiStripBaseCondObjDQM::GetSameLayerDetId(
    const std::vector<uint32_t> &activeDetIds, uint32_t selDetId,
    const TrackerTopology *tTopo) {
  ...
  case StripSubdetector::TIB:
    SiStripSubStructure::getTIBDetectors(activeDetIds, sameLayerDetIds, tTopo,
                                         tTopo->tibLayer(selDetId), 0, 0, 0);
    break;
  case StripSubdetector::TID:
    SiStripSubStructure::getTIDDetectors(activeDetIds, sameLayerDetIds, tTopo,
                                         tTopo->tidSide(selDetId),
                                         tTopo->tidWheel(selDetId), 0, 0);
    break;
  case StripSubdetector::TOB:
    SiStripSubStructure::getTOBDetectors(activeDetIds, sameLayerDetIds, tTopo,
                                         tTopo->tobLayer(selDetId), 0, 0);
    break;
  case StripSubdetector::TEC:
    SiStripSubStructure::getTECDetectors(activeDetIds, sameLayerDetIds, tTopo,
                                         tTopo->tecSide(selDetId),
                                         tTopo->tecWheel(selDetId), 0, 0, 0, 0);
    break;
  }
...
void SiStripBaseCondObjDQM::saveTkMap(const std::string &TkMapname,
  ...
  if (!tkMapScaler.empty()) {
    ...
    if (maxValue < imax) {
      edm::LogInfo("SiStripBaseCondObjDQM")
          << "Resetting TkMap maxValue from " << maxValue << " to " << imax;
      maxValue = imax;
    }
    // reset minValue;
    if (minValue > imin) {
      edm::LogInfo("SiStripBaseCondObjDQM")
          << "Resetting TkMap minValue from " << minValue << " to " << imin;
      minValue = imin;
    }
  }
