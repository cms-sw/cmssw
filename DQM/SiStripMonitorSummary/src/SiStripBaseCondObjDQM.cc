...
void SiStripBaseCondObjDQM::selectModules(std::vector<uint32_t> &detIds_, const TrackerTopology *tTopo) {
  ...
  if (fPSet_.getParameter<bool>("restrictModules")) {
    ...
    for (const auto &included_subdet : included_subdets) {
      included_subdetsels.push_back(DetIdSelector(included_subdet));
    }

    std::vector<uint32_t> modulesToBeIncluded;
    for (unsigned int detId : detIds_) {
      for (const auto &included_subdetsel : included_subdetsels) {
        if (included_subdetsel.isSelected(detId)) {
    ...
    for (const auto &excluded_subdet : excluded_subdets) {
      excluded_subdetsels.push_back(DetIdSelector(excluded_subdet));
    }

    std::vector<uint32_t> modulesToBeExcluded;
    for (unsigned int detId : detIds_) {
      for (const auto &excluded_subdetsel : excluded_subdetsels) {
        if (excluded_subdetsel.isSelected(detId)) {
    ...
    if (!modulesToBeExcluded.empty()) {
      for (unsigned int mod : modulesToBeExcluded) {
    ...
    if (*(SubDetectorsToBeExcluded_.begin()) != "none") {
      ...
      for (const auto &modIter_ : SubDetectorsToBeExcluded_) {
        tmp.clear();
    ...
    for (unsigned int sameLayerDetId : sameLayerDetIds_) {
      ...
      if (SummaryOnStringLevel_On_) {
        ...
        if (subdetectorId_ == 3) {  // re-abelling for TIB
          ...
          if (tTopo->tobIsZPlusSide(sameLayerDetId)) {
            sprintf(sameLayerDetIds_Name, "%i", tTopo->tobModule(sameLayerDetId));
          } else if (tTopo->tobIsZMinusSide(sameLayerDetId)) {
            sprintf(sameLayerDetIds_Name, "%i", -tTopo->tobModule(sameLayerDetId));
          }
          CondObj_ME.SummaryOfProfileDistr->setBinLabel(iBin, sameLayerDetIds_Name);
        }
