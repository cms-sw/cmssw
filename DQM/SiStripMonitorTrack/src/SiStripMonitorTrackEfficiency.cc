...
SiStripMonitorTrackEfficiency::SiStripMonitorTrackEfficiency(const edm::ParameterSet& conf):
...
//------------------------------------------------------------------------
void SiStripMonitorTrackEfficiency::beginRun(const edm::Run& run,const edm::EventSetup& es)
{
  //get geom 
  es.get<TrackerDigiGeometryRecord>().get( tkgeom );
  edm::LogInfo("SiStripMonitorTrackEfficiency") << "[SiStripMonitorTrackEfficiency::beginRun] There are "<<tkgeom->detUnits().size() <<" detectors instantiated in the geometry" << std::endl;  
...
//------------------------------------------------------------------------
void SiStripMonitorTrackEfficiency::endJob(void)
{
...
void SiStripMonitorTrackEfficiency::analyze(const edm::Event& e, const edm::EventSetup& es)
{
  tracksCollection_in_EventTree=true;
  trackAssociatorCollection_in_EventTree=true;
  
  //initialization of global quantities
  edm::LogInfo("SiStripMonitorTrackEfficiency") << "[SiStripMonitorTrackEfficiency::analyse]  " << "Run " << e.id().run() << " Event " << e.id().event() << std::endl;
  runNb   = e.id().run();
  eventNb = e.id().event();
  ...
  if (trackCollection.isValid()){
    ...
    LogDebug("SiStripMonitorTrackEfficiency")<<" Track Collection is not valid !!" <<std::endl;
  ...
  if( TItkAssociatorCollection.isValid()){
    ...
    LogDebug("SiStripMonitorTrackEfficiency")<<"Association not found "<<std::endl;
  ...
  if (dsv_SiStripCluster.isValid() && OffHisto_On_){
    ...
    LogDebug("SiStripMonitorTrackEfficiency")<< "ClusterCollection is not valid!!" << std::endl;
  }
...
//------------------------------------------------------------------------  
void SiStripMonitorTrackEfficiency::book() 
{
  ...
  for (std::vector<uint32_t>::const_iterator detid_iter=vdetId_.begin();detid_iter!=vdetId_.end();detid_iter++){  //loop on all the active detid
    ...
    if (detid < 1){
      edm::LogError("SiStripMonitorTrackEfficiency")<< "[" <<__PRETTY_FUNCTION__ << "] invalid detid " << detid<< std::endl;
      continue;
    }
    ...
    // 	LogTrace("SiStripMonitorTrackEfficiency") << " Detid " << *detid 
...
//--------------------------------------------------------------------------------
void SiStripMonitorTrackEfficiency::bookModMEs(TString name, uint32_t id)//Histograms at MODULE level
{
...
void SiStripMonitorTrackEfficiency::bookTrendMEs(TString name,int32_t layer,uint32_t id,std::string flag)//Histograms and Trends at LAYER LEVEL
{
  char rest[1024];
...
void SiStripMonitorTrackEfficiency::bookSubDetMEs(TString name,TString flag)//Histograms at SubDet level
{
...
MonitorElement* SiStripMonitorTrackEfficiency::bookME1D(const char* ParameterSetLabel, const char* HistoName)
{
...
//--------------------------------------------------------------------------------
MonitorElement* SiStripMonitorTrackEfficiency::bookME2D(const char* ParameterSetLabel, const char* HistoName)
{
  Parameters =  conf_.getParameter<edm::ParameterSet>(ParameterSetLabel);
...
//--------------------------------------------------------------------------------
MonitorElement* SiStripMonitorTrackEfficiency::bookME3D(const char* ParameterSetLabel, const char* HistoName)
{
  Parameters =  conf_.getParameter<edm::ParameterSet>(ParameterSetLabel);
...
//--------------------------------------------------------------------------------
MonitorElement* SiStripMonitorTrackEfficiency::bookMEProfile(const char* ParameterSetLabel, const char* HistoName)
{
    Parameters =  conf_.getParameter<edm::ParameterSet>(ParameterSetLabel);
...
//--------------------------------------------------------------------------------
MonitorElement* SiStripMonitorTrackEfficiency::bookMETrend(const char* ParameterSetLabel, const char* HistoName)
{
  Parameters =  conf_.getParameter<edm::ParameterSet>(ParameterSetLabel);
...
//------------------------------------------------------------------------------------------
void SiStripMonitorTrackEfficiency::trackStudy(const edm::EventSetup& es)
{

  LogDebug("SiStripMonitorTrackEfficiency") << "inside trackstudy" << std::endl;
  ...
  for(TrajTrackAssociationCollection::const_iterator it =  TItkAssociatorCollection->begin();it !=  TItkAssociatorCollection->end(); ++it){
    ...
    LogTrace("SiStripMonitorTrackEfficiency")
      << "Track number "<< i+1 
    ...
    for(traj_mes_iterator=measurements.begin();traj_mes_iterator!=measurements.end();traj_mes_iterator++){//loop on measurements
      ...
      TrajectoryStateOnSurface tsos = tsoscomb( traj_mes_iterator->forwardPredictedState(), traj_mes_iterator->backwardPredictedState() );
	...
	if(hit!=0){
	 ...
	 << "\nLocal x-z plane angle: "<<atan2(statedirection.x(),statedirection.z());	      
      LogTrace("SiStripMonitorTrackEfficiency") <<ss.str() << std::endl;
    }
    
  }
}

  void SiStripMonitorTrackEfficiency::RecHitInfo(const SiStripRecHit2D* tkrecHit, LocalVector LV,reco::TrackRef track_ref, const edm::EventSetup& es){
    
    if(!tkrecHit->isValid()){
      LogTrace("SiStripMonitorTrackEfficiency") <<"\t\t Invalid Hit " << std::endl;
      return;  
    }
    ...
    if (find(ModulesToBeExcluded_.begin(),ModulesToBeExcluded_.end(),detid)!=ModulesToBeExcluded_.end()){
      LogTrace("SiStripMonitorTrackEfficiency") << "Modules Excluded" << std::endl;
      return;
    }
    ...
    if ( tkrecHit != NULL ){
      LogTrace("SiStripMonitorTrackEfficiency") << "GOOD hit" << std::endl;
	...
	countOn++;
      ...
      LogTrace("SiStripMonitorTrackEfficiency") << "NULL hit" << std::endl;
    }	  
  }

//------------------------------------------------------------------------

void SiStripMonitorTrackEfficiency::AllClusters( const edm::EventSetup& es)
{
  ...
  for ( edmNew::DetSetVector<SiStripCluster>::const_iterator DSViter=dsv_SiStripCluster->begin(); DSViter!=dsv_SiStripCluster->end();DSViter++){
    ...
    //Loop on Clusters
    LogDebug("SiStripMonitorTrackEfficiency") << "on detid "<< detid << " N Cluster= " << DSViter->size();
	...
	LogDebug("SiStripMonitorTrackEfficiency") << "ClusIter " << &*ClusIter << "\t " 
...
//------------------------------------------------------------------------
bool SiStripMonitorTrackEfficiency::clusterInfos(SiStripClusterInfo* cluster, const uint32_t& detid,std::string flag, const LocalVector LV)
{
...
//--------------------------------------------------------------------------------
void SiStripMonitorTrackEfficiency::fillTrend(MonitorElement* me ,float value)
{
  if(!me) return;
...
//--------------------------------------------------------------------------------
void SiStripMonitorTrackEfficiency::fillModMEs(SiStripClusterInfo* cluster,TString name,float cos)
{
  std::map<TString, ModMEs>::iterator iModME  = ModMEsMap.find(name);
  if(iModME!=ModMEsMap.end()){
...
void SiStripMonitorTrackEfficiency::fillTrendMEs(SiStripClusterInfo* cluster,std::string name,float cos, std::string flag)
{ 
  std::map<TString, ModMEs>::iterator iModME  = ModMEsMap.find(name);
  if(iModME!=ModMEsMap.end()){
    if(flag=="OnTrack"){
