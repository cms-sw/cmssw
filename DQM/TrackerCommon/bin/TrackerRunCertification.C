...
const TString sSubSys_[nSubSys_] = {
    // sub-system directory names in DQM files
    "SiStrip",
    "Pixel",
    "Tracking",
};
...
int main(int argc, char* argv[]) {
  ...
  sArguments["-g"] = "";  // group
  sArguments["-p"] =
      "/afs/cern.ch/cms/CAF/CMSCOMM/COMM_DQM/data/OfflineData/Run2010/StreamExpress";  // path to DQM files
  ...
  for (int iArgument = 1; iArgument < argc; ++iArgument) {
    if (sArguments.find(argv[iArgument]) != sArguments.end()) {
      if (sArguments.find(argv[iArgument + 1]) == sArguments.end() &&
          sOptions.find(argv[iArgument + 1]) == sOptions.end()) {
...
Bool_t createInputFileList() {
  ...
  while (fileListRead.good()) {
    ...
    if (!RunNumber(pathFile).IsDigit())
      continue;
    ++nFiles;
    const Int_t iRun(RunNumber(pathFile).Atoi());
    if (minRange_ > iRun || iRun > maxRange_)
      continue;
  ...
  if (nFiles == 0 || maxRun_ < minRun_) {
    ...
    cerr << "  no files found in " << sArguments["-p"] << " between the run numbers " << minRange_ << " and "
         << maxRange_ << endl;
    return kFALSE;
  }
  return kTRUE;
...
Bool_t createRRFile() {
  ...
  TString commandBase(TString(gSystem->Getenv("CMSSW_BASE"))
                          .Append("/src/DQM/TrackerCommon/bin/getRunRegistry.py")
                          .Append(" -s ")
                          .Append(sArguments["-R"])
                          .Append("/xmlrpc")
                          .Append(" ")
  ...
  if (sOptions["-v"])
    cerr << endl << endl << "    " << commandRuns.Data() << endl;
  ...
  if (sOptions["-v"])
    cerr << "    " << commandTracker.Data() << endl << endl << "  ...";
  ...
  while (lines <= 1 && fileRunsRR.getline(xmlLine, maxLength))
    ++lines;
  if (lines <= 1) {
  ...
  while (lines <= 1 && fileLumisRR.getline(xmlLine, maxLength))
    ++lines;
  if (lines <= 1) {
  ...
  while (lines <= 1 && fileTrackerRR.getline(xmlLine, maxLength))
    ++lines;
  if (lines <= 1) {
...
Bool_t readRR(const TString& pathFile) {
          ...
          while (nodeDataset) {
            ...
            if (nodeDatasetChild && TString(xmlRR->GetNodeContent(nodeDatasetChild)) == sArguments["-d"]) {
              ...
              while (nodeDatasetChild && xmlRR->GetNodeName(nodeDatasetChild) != TString("STATE"))
                nodeDatasetChild = xmlRR->GetNext(nodeDatasetChild);
              if (sOptions["-a"] ||
                  (nodeDatasetChild && TString(xmlRR->GetNodeContent(nodeDatasetChild)) == "SIGNOFF")) {
                foundSignoff = kTRUE;
                nodeDatasetChild = xmlRR->GetChild(nodeDataset);
  ...
  if (iFlagsRR_[sSubSys_[SiStrip]] == EXCL)
    ++nRunsExclSiStrip_;
  if (iFlagsRR_[sSubSys_[Pixel]] == EXCL)
    ++nRunsExclPixel_;
  if (iFlagsRR_[sSubSys_[SiStrip]] == MISSING)
    ++nRunsMissSiStrip_;
  if (iFlagsRR_[sSubSys_[Pixel]] == MISSING)
    ++nRunsMissPixel_;
  if ((iFlagsRR_[sSubSys_[SiStrip]] == EXCL || iFlagsRR_[sSubSys_[SiStrip]] == MISSING) &&
      (iFlagsRR_[sSubSys_[Pixel]] == EXCL || iFlagsRR_[sSubSys_[Pixel]] == MISSING))
    ++nRunsNoTracking_;
      ...
      if (nodeRunChild) {
        ...
        if (nodeDatasetChild && xmlRR->GetNodeContent(nodeDatasetChild) == sArguments["-d"]) {
          ...
          while (nodeLumiRange) {
            ...
            for (map<TString, Bool_t>::const_iterator iMap = bLumiSiStripOn_.begin(); iMap != bLumiSiStripOn_.end();
                 ++iMap) {
              if (!iMap->second)
                siStripOn = kFALSE;
              break;
            }
            for (map<TString, Bool_t>::const_iterator iMap = bLumiPixelOn_.begin(); iMap != bLumiPixelOn_.end();
                 ++iMap) {
              if (!iMap->second)
                pixelOn = kFALSE;
              break;
      ...
      if (nodeRunChild && xmlRR->GetNodeContent(nodeRunChild) == sArguments["-g"]) {
        ...
        if (nodeRunChild) {
          ...
          while (nodeDataset) {
            ...
            //             if ( nodeDatasetChild && TString( xmlRR->GetNodeContent( nodeDatasetChild ) ) == sArguments[ "-d" ] ) {
            if (nodeDatasetChild &&
                TString(xmlRR->GetNodeContent(nodeDatasetChild)) ==
                    TString("/Global/Online/ALL")) {  // currently cretaed under this dataset name in RR TRACKER
              foundDataset = kTRUE;
              nodeDatasetChild = xmlRR->GetChild(nodeDataset);
              while (nodeDatasetChild && TString(xmlRR->GetNodeName(nodeDatasetChild)) != "CMPS")
                nodeDatasetChild = xmlRR->GetNext(nodeDatasetChild);
...
Bool_t readDQM(const TString& pathFile) {
  ...
  for (UInt_t iDir = 0; iDir < nameCertDir.size(); ++iDir) {
    const TString nameCurDir(nameCertDir.at(iDir).Contains(pathRunFragment_)
                                 ? nameCertDir.at(iDir).Insert(nameCertDir.at(iDir).Index("Run ") + 4, sRunNumber_)
                                 : nameCertDir.at(iDir));
...
void certifyRun() {
  ...
  if (bAvailable_[sSubSys_[SiStrip]]) {
    ...
    Bool_t flagDAQ(fCertificates_["SiStripDAQSummary"] == (Double_t)EXCL ||
                   fCertificates_["SiStripDAQSummary"] > minGood_);
    Bool_t flagDCS(fCertificates_["SiStripDCSSummary"] == (Double_t)EXCL ||
                   fCertificates_["SiStripDCSSummary"] == (Double_t)GOOD);
    ...
    if (!flagDet)
      comments.push_back("too low overall fraction of good modules");
    if (!flagDAQ)
      comments.push_back("DAQSummary BAD");
    if (!flagDCS)
      comments.push_back("DCSSummary BAD");
    //     if ( ! bSiStripOn_ ) comments.push_back( "HV off" );
    if (!flagCert)
      comments.push_back(TString("Tracker shifter: " + sRRTrackerCommentsSiStrip_[sRunNumber_]));
    if (iFlags[sSubSys_[SiStrip]] == BAD) {
      ++nRunsBadSiStrip_;
      if (flagCert)
        comments.push_back(TString("Tracker shifter differs (GOOD): " + sRRTrackerCommentsSiStrip_[sRunNumber_]));
  ...
  if (bAvailable_[sSubSys_[Pixel]]) {
    ...
    Bool_t flagDAQ(
        (fCertificates_["DAQPixelBarrelFraction"] == (Double_t)EXCL || fCertificates_["DAQPixelBarrelFraction"] > 0.) &&
        (fCertificates_["DAQPixelEndcapFraction"] == (Double_t)EXCL ||
         fCertificates_["DAQPixelEndcapFraction"] > 0.));  // unidentified bug in Pixel DAQ fraction determination
    ...
    if (!flagReportSummary)
      comments.push_back("ReportSummary BAD");
    if (!flagDAQ)
      comments.push_back("DAQSummary BAD");
    if (!flagDCS)
      comments.push_back("DCSSummary BAD");
    ...
    if (!flagCert)
      comments.push_back(TString("Tracker shifter: " + sRRTrackerCommentsPixel_[sRunNumber_]));
    if (iFlags[sSubSys_[Pixel]] == BAD) {
      ++nRunsBadPixel_;
      if (flagCert)
        comments.push_back(TString("Tracker shifter differs (GOOD): " + sRRTrackerCommentsPixel_[sRunNumber_]));
  ...
  if (bAvailable_[sSubSys_[Tracking]]) {
    ...
    if (iFlagsRR_[sSubSys_[SiStrip]] == EXCL && iFlagsRR_[sSubSys_[Pixel]] == EXCL) {
      ...
      if (!flagChi2)
        comments.push_back("Chi2/DoF too low");
      if (!flagRate)
        comments.push_back("Track rate too low");
      if (!flagRecHits)
        comments.push_back("Too few RecHits");
      //       if ( ! bSiStripOn_ ) comments.push_back( "HV SiStrip off" );
      if (!flagCert)
        comments.push_back(TString("Tracker shifter: " + sRRTrackerCommentsTracking_[sRunNumber_]));
    }
    ...
    if (iFlags[sSubSys_[Tracking]] == BAD) {
      ++nRunsBadTracking_;
      if (flagCert)
        comments.push_back(TString("Tracker shifter differs (GOOD): " + sRRTrackerCommentsTracking_[sRunNumber_]));
...
void writeOutput() {
  ...
  if (!sOptions["-a"])
    fileLog << "# of runs not in SIGNOFF in RR           : " << nRunsNotSignoff_ << endl;
  fileLog << "# of runs not found in RR Tracker        : " << nRunsNotRRTracker_ << endl
          << "# of runs group not found in RR Tracker  : " << nRunsNotGroupTracker_
          << endl
  ...
  for (UInt_t iRun = 0; iRun < sRunNumbers_.size(); ++iRun) {
    if (sRRSiStrip_[sRunNumbers_.at(iRun)] != sSiStrip_[sRunNumbers_.at(iRun)]) {
      fileLog << "  " << sRunNumbers_.at(iRun) << ": " << sRRSiStrip_[sRunNumbers_.at(iRun)] << " --> "
              << sSiStrip_[sRunNumbers_.at(iRun)] << endl;
  ...
  for (UInt_t iRun = 0; iRun < sRunNumbers_.size(); ++iRun) {
    if (sRRPixel_[sRunNumbers_.at(iRun)] != sPixel_[sRunNumbers_.at(iRun)]) {
      fileLog << "  " << sRunNumbers_.at(iRun) << ": " << sRRPixel_[sRunNumbers_.at(iRun)] << " --> "
              << sPixel_[sRunNumbers_.at(iRun)] << endl;
  ...
  for (UInt_t iRun = 0; iRun < sRunNumbers_.size(); ++iRun) {
    if (sRRTracking_[sRunNumbers_.at(iRun)] != sTracking_[sRunNumbers_.at(iRun)]) {
      fileLog << "  " << sRunNumbers_.at(iRun) << ": " << sRRTracking_[sRunNumbers_.at(iRun)] << " --> "
              << sTracking_[sRunNumbers_.at(iRun)] << endl;
...
void displayHelp() {
  cerr << "  TrackerRunCertification" << endl
       ...
       << "  Purpose:" << endl
       << endl
       << "  The procedure of certifying data of a given run range is automated in order to speed up the procedure and "
          "to reduce the Tracker Offline Shift Leader's workload."
       << endl
       << endl
       << "  Input:" << endl
       << endl
       ...
       << "  It contains a list of all flags changed with respect to the RunRegistry, including the reason(s) in case "
          "the flag is changed to BAD."
       << endl
       << endl
       << "  The verbose ('-v') stdout can provide a complete list of all in-/output flags of all analyzed runs and at "
          "its end a summary only with the output flags."
       << endl
