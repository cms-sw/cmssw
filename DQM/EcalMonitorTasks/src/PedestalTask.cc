...
namespace ecaldqm {
  PedestalTask::PedestalTask() : DQWorkerTask(), gainToME_(), pnGainToME_() { std::fill_n(enable_, nDCC, false); }
  ...
  bool PedestalTask::filterRunType(short const* _runType) {
    ...
    for (int iFED(0); iFED < nDCC; iFED++) {
      if (_runType[iFED] == EcalDCCHeaderBlock::PEDESTAL_STD || _runType[iFED] == EcalDCCHeaderBlock::PEDESTAL_GAP) {
        enable = true;
        enable_[iFED] = true;
      } else
        enable_[iFED] = false;
