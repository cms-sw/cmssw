...
std::string subdet[4]={"HB","HE","HO","HF"};
  ...
  for(int sd=0;sd<4;sd++){
     ...
     for(int phi=1;phi<=72;phi++) for(int depth=fdepth;depth<=tdepth;depth++) for(int eta=feta;eta<=teta;eta++){
	   ...
	   problem[i] =get_channel_status(subdet[sd],eta,phi,depth,i+1);
	      ...
	      HcalDetId hcalid(SD, eta, phi, depth, false);
	      if(sd==0){  if(eta>0){ 
                  HBP[i]++; 
  ...
  for(int sd=0;sd<4;sd++){
      ...
      for(int phi=1;phi<=72;phi++) for(int depth=fdepth;depth<=tdepth;depth++) for(int eta=feta;eta<=teta;eta++){
         ...
         if(missing>0){
	       HcalDetId *detid=0;
               if(sd==0) detid=new HcalDetId(HcalBarrel,eta,phi,depth,false);
               if(sd==1) detid=new HcalDetId(HcalEndcap,eta,phi,depth,false);
               if(sd==2) detid=new HcalDetId(HcalOuter,eta,phi,depth,false);
               if(sd==3) detid=new HcalDetId(HcalForward,eta,phi,depth,false);
