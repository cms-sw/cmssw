...
public:
  ...
  uint32_t getHash(HcalElectronicsId const &eid) const override {
    return hashfunctions::hash_eid[_htype - hashfunctions::nHashType_did - 1](
        eid);
  }
  uint32_t getHash(HcalTrigTowerDetId const &tid) const override {
    return hashfunctions::hash_tid[_htype - hashfunctions::nHashType_eid - 1](
        tid);
  }
  ...
  std::string getName(HcalElectronicsId const &eid) const override {
    return hashfunctions::name_eid[_htype - hashfunctions::nHashType_did - 1](
        eid);
  }
  std::string getName(HcalTrigTowerDetId const &tid) const override {
    return hashfunctions::name_tid[_htype - hashfunctions::nHashType_eid - 1](
        tid);
  }
  ...
  virtual bool isEHash() const {
    return (_htype > hashfunctions::nHashType_did &&
            _htype < hashfunctions::nHashType_eid)
               ? true
               : false;
  }
  virtual bool isTHash() const {
    return (_htype > hashfunctions::nHashType_eid &&
            _htype < hashfunctions::nHashType_tid)
               ? true
               : false;
  }
