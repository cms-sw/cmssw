 ...
 */
template <class Digi>
CaloSamples loadADC2fCDB(const edm::ESHandle<HcalDbService> &conditions,
                         const HcalDetId did, const Digi &digi) {
...
double aveTSDB(const edm::ESHandle<HcalDbService> &conditions,
               const CaloSamples &calo_samples, const HcalDetId did,
               const Digi &digi, unsigned int i_start, unsigned int i_end) {
  double sumQ = 0;
  double sumQT = 0;
 ...
 */
template <typename DIGI> int maxTS(DIGI const &digi, double ped = 0) {
  int maxTS = -1;
  double maxQ = -100;
 ...
 *	Log Functions
 */
template <typename STDTYPE> void dqmdebug(STDTYPE const &x, int debug = 0) {
  if (debug == 0)
    return;
