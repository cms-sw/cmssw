...
public:
  ...
  std::vector<std::string> getLabels() override {
    ...
    for (std::vector<flag::Flag>::const_iterator it = _flags.begin();
         it != _flags.end(); ++it)
...
public:
  ...
  LumiSectionCoarse(int n, int binning)
      : ValueQuantity(fLS), _n(n), _binning(binning) {}
...
public:
  EventType() {}
  EventType(std::vector<uint32_t> const &vtypes) : ValueQuantity(fN) {
    this->setup(vtypes);
  }
  ~EventType() override {}
...
public:
  std::vector<std::string> getLabels() override {
    ...
    for (auto const &v : _types) {
      labels[v.second] =
          utilities::ogtype2string((constants::OrbitGapType)v.first);
    }
    return labels;
  }
