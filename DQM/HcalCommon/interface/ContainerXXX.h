...
template <typename STDTYPE> class ContainerXXX {
public:
  ContainerXXX() {}
...
template <typename STDTYPE> uint32_t ContainerXXX<STDTYPE>::size() {
  return (uint32_t)(_cmap.size());
}

template <typename STDTYPE> void ContainerXXX<STDTYPE>::dump(Container1D *c) {
  for (auto &p : _cmap) {
    STDTYPE &x = p.second;
...
template <typename STDTYPE> void ContainerXXX<STDTYPE>::print() {
...
template <typename STDTYPE> void ContainerXXX<STDTYPE>::reset() {
  for (auto &p : _cmap) {
    p.second = 0;
  }
}
