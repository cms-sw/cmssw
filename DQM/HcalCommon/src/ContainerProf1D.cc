  ...
  if (_hashmap.isDHash()) {
    ...
    for (std::vector<HcalGenericDetId>::const_iterator it = dids.begin();
      ...
      _mes.insert(std::make_pair(
          hash, ib.bookProfile(_hashmap.getName(did), _hashmap.getName(did),
                               _qx->nbins(), _qx->min(), _qx->max(), _qy->min(),
                               _qy->max())));
      customize(_mes[hash]);
    }
    ...
    for (std::vector<HcalElectronicsId>::const_iterator it = eids.begin();
      ...
      _mes.insert(std::make_pair(
          hash, ib.bookProfile(_hashmap.getName(eid), _hashmap.getName(eid),
                               _qx->nbins(), _qx->min(), _qx->max(), _qy->min(),
                               _qy->max())));
      customize(_mes[hash]);
    }
    ...
    for (std::vector<HcalTrigTowerDetId>::const_iterator it = tids.begin();
      ...
      _mes.insert(std::make_pair(
          hash, ib.bookProfile(_hashmap.getName(tid), _hashmap.getName(tid),
                               _qx->nbins(), _qx->min(), _qx->max(), _qy->min(),
                               _qy->max())));
      customize(_mes[hash]);
    }
