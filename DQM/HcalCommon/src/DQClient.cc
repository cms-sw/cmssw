...
void DQClient::beginRun(edm::Run const &r, edm::EventSetup const &es) {
  ...
  if (_ptype != fOffline) { // hidefed2crate
    ...
    for (std::vector<int>::const_iterator it = _vFEDs.begin();
      ...
      //
      if (*it == 0) {
        _vhashFEDs.push_back(HcalElectronicsId(0, SLOT_uTCA_MIN,
      ...
      if (*it > FED_VME_MAX) {
        ...
        _vhashFEDs.push_back(HcalElectronicsId(cspair.first, cspair.second,
                                               FIBER_uTCA_MIN1, FIBERCH_MIN,
                                               false)
                                 .rawId());
      } else
...
void DQClient::beginLuminosityBlock(DQMStore::IBooker &, DQMStore::IGetter &,
...
void DQClient::endLuminosityBlock(DQMStore::IBooker &, DQMStore::IGetter &,
                                  edm::LuminosityBlock const &lb,
