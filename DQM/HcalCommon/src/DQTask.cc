...
DQTask::DQTask(edm::ParameterSet const &ps)
    : DQModule(ps), _cEvsTotal(_name, "EventsTotal"),
      _cEvsPerLS(_name, "EventsPerLS"), _cRunKeyVal(_name, "RunKeyValue"),
      _cRunKeyName(_name, "RunKeyName"),
      _cProcessingTypeName(_name, "ProcessingType"), _procLSs(0) {
  //	tags and Tokens
  _tagRaw = ps.getUntrackedParameter<edm::InputTag>(
      "tagRaw", edm::InputTag("rawDataCollector"));
...
void DQTask::bookHistograms(DQMStore::IBooker &ib, edm::Run const &r,
  ...
  if (auto runInfoRec = es.tryToGet<RunInfoRcd>()) {
    ...
    for (std::vector<int>::const_iterator it = vfeds.begin(); it != vfeds.end();
      ...
      if (*it >= constants::FED_VME_MIN && *it <= FED_VME_MAX)
        _vcdaqEids.push_back(HcalElectronicsId(constants::FIBERCH_MIN,
                                               constants::FIBER_VME_MIN,
                                               SPIGOT_MIN, (*it) - FED_VME_MIN)
                                 .rawId());
        ...
        _vcdaqEids.push_back(HcalElectronicsId(cspair.first, cspair.second,
                                               FIBER_uTCA_MIN1, FIBERCH_MIN,
                                               false)
                                 .rawId());
      }
    }
...
void DQTask::beginLuminosityBlock(edm::LuminosityBlock const &lb,
                                  edm::EventSetup const &es) {
...
void DQTask::endLuminosityBlock(edm::LuminosityBlock const &lb,
                                edm::EventSetup const &es) {
  _procLSs++;
}
