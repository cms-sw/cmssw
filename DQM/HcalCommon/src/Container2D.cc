...
/* virtual */ double Container2D::getBinContent(HcalDetId const &id) {
  return _mes[_hashmap.getHash(id)]->getBinContent(_qx->getBin(id),
                                                   _qy->getBin(id));
}
...
/* virtual */ double Container2D::getBinContent(HcalDetId const &id, int x) {
  if (_qx->isCoordinate())
    ...
    return _mes[_hashmap.getHash(id)]->getBinContent(_qx->getBin(x),
                                                     _qy->getBin(id));
}
...
/* virtual */ void Container2D::setBinContent(HcalDetId const &id, int x) {
  _mes[_hashmap.getHash(id)]->setBinContent(_qx->getBin(id), _qy->getBin(id),
                                            x);
}
...
/* virtual */ void Container2D::setBinContent(HcalDetId const &id, int x,
                                              int y) {
  if (_qx->isCoordinate())
    _mes[_hashmap.getHash(id)]->setBinContent(_qx->getBin(id), _qy->getBin(x),
                                              y);
  else
    _mes[_hashmap.getHash(id)]->setBinContent(_qx->getBin(x), _qy->getBin(id),
                                              y);
}
  ...
  if (_hashmap.isDHash()) {
    ...
    for (std::vector<HcalGenericDetId>::const_iterator it = dids.begin();
      ...
      _mes.insert(std::make_pair(
          hash, ib.book2D(_hashmap.getName(did), _hashmap.getName(did),
    ...
    for (std::vector<HcalElectronicsId>::const_iterator it = eids.begin();
      ...
      _mes.insert(std::make_pair(
          hash, ib.book2D(_hashmap.getName(eid), _hashmap.getName(eid),
                          _qx->nbins(), _qx->min(), _qx->max(), _qy->nbins(),
                          _qy->min(), _qy->max())));
      customize(_mes[hash]);
    }
    ...
    for (std::vector<HcalTrigTowerDetId>::const_iterator it = tids.begin();
      ...
      _logger.debug(_hashmap.getName(tid));
      _mes.insert(std::make_pair(
          hash, ib.book2D(_hashmap.getName(tid), _hashmap.getName(tid),
                          _qx->nbins(), _qx->min(), _qx->max(), _qy->nbins(),
                          _qy->min(), _qy->max())));
