...
int getValue_Crate(HcalElectronicsId const &eid) {
  ...
  return ((it_crate == std::end(crateList))
              ? -1
              : std::distance(crateList.begin(), it_crate));
}

int getValue_CrateuTCA(HcalElectronicsId const &eid) {
  unsigned int crate = eid.crateId();
  auto it_crate =
      std::find(std::begin(crateListuTCA), std::end(crateListuTCA), crate);
  return ((it_crate == std::end(crateListuTCA))
              ? -1
              : std::distance(crateListuTCA.begin(), it_crate));
}

int getValue_CrateVME(HcalElectronicsId const &eid) {
  unsigned int crate = eid.crateId();
  auto it_crate =
      std::find(std::begin(crateListVME), std::end(crateListVME), crate);
  return ((it_crate == std::end(crateListVME))
              ? -1
              : std::distance(crateListVME.begin(), it_crate));
}
...
HcalElectronicsId getEid_Crate(int v) {
  return v < CRATE_VME_NUM
             ...
             : HcalElectronicsId(v - CRATE_VME_NUM + CRATE_uTCA_MIN,
                                 SLOT_uTCA_MIN, FIBER_uTCA_MIN1, FIBERCH_MIN,
                                 false);
}
...
HcalElectronicsId getEid_CrateuTCA(int v) {
  return HcalElectronicsId(v + CRATE_uTCA_MIN, SLOT_uTCA_MIN, FIBER_uTCA_MIN1,
                           FIBERCH_MIN, false);
}

HcalElectronicsId getEid_SlotuTCA(int v) {
  return HcalElectronicsId(CRATE_uTCA_MIN, v + SLOT_uTCA_MIN, FIBER_uTCA_MIN1,
                           FIBERCH_MIN, false);
}
...
HcalElectronicsId getEid_Spigot(int v) {
  return HcalElectronicsId(FIBERCH_MIN, FIBER_VME_MIN, v, CRATE_VME_MIN);
}
...
HcalElectronicsId getEid_FiberCh(int v) {
  return HcalElectronicsId(CRATE_uTCA_MIN, SLOT_uTCA_MIN, FIBER_uTCA_MIN1, v,
                           false);
}
...
HcalElectronicsId getEid_FEDuTCASlot(int v) {
  ...
  return HcalElectronicsId(fedeid.crateId(), sloteid.slot(), FIBER_uTCA_MIN1,
                           FIBERCH_MIN, false);
}
...
HcalElectronicsId getEid_FEDVMESpigot(int v) {
  ...
  return HcalElectronicsId(FIBERCH_MIN, FIBER_VME_MIN, spid.spigot(),
                           fedeid.dccid());
}
...
HcalElectronicsId getEid_FiberuTCAFiberCh(int v) {
  ...
  return HcalElectronicsId(CRATE_uTCA_MIN, SLOT_uTCA_MIN, fibereid.fiberIndex(),
                           fcheid.fiberChanId(), false);
}

HcalElectronicsId getEid_FiberVMEFiberCh(int v) {
  HcalElectronicsId fibereid = getEid_FiberVME(v / FIBERCH_NUM);
  HcalElectronicsId fcheid = getEid_FiberCh(v % FIBERCH_NUM);
  return HcalElectronicsId(fcheid.fiberChanId(), fibereid.fiberIndex(),
                           SPIGOT_MIN, CRATE_VME_MIN);
}

HcalElectronicsId getEid_SLB(int v) {
  return HcalElectronicsId(SLBCH_MIN, v + SLB_MIN, SPIGOT_MIN, CRATE_VME_MIN,
                           CRATE_VME_MIN, SLOT_VME_MIN1, 0);
}

HcalElectronicsId getEid_SLBCh(int v) {
  return HcalElectronicsId(v + SLBCH_MIN, SLB_MIN, SPIGOT_MIN, CRATE_VME_MIN,
                           CRATE_VME_MIN, SLOT_VME_MIN1, 0);
}
...
HcalElectronicsId getEid_SLBSLBCh(int v) {
  ...
  return HcalElectronicsId(slbcheid.slbChannelIndex(), slbeid.slbSiteNumber(),
                           SPIGOT_MIN, CRATE_VME_MIN, CRATE_VME_MIN,
                           SLOT_VME_MIN1, 0);
}

HcalElectronicsId getEid_FiberuTCATP(int v) {
  return HcalElectronicsId(CRATE_uTCA_MIN, SLOT_uTCA_MIN, v + TPFIBER_MIN,
                           TPFIBERCH_MIN, true);
}

HcalElectronicsId getEid_FiberChuTCATP(int v) {
  return HcalElectronicsId(CRATE_uTCA_MIN, SLOT_uTCA_MIN, TPFIBER_MIN,
                           v + TPFIBERCH_MIN, true);
}
...
HcalElectronicsId getEid_FiberuTCATPFiberChuTCATP(int v) {
  ...
  return HcalElectronicsId(CRATE_uTCA_MIN, SLOT_uTCA_MIN, fibeid.fiberIndex(),
                           fibcheid.fiberChanId(), true);
}
...
std::vector<std::string> getLabels_FEDuTCASlot() {
  ...
  for (int i = 0; i < FED_uTCA_NUM; i++)
    for (int j = 0; j < SLOT_uTCA_NUM; j++) {
      ...
      sprintf(name, "%d-%d", utilities::crate2fed(eid.crateId(), eid.slot()),
              eid.slot());
      labels.push_back(std::string(name));
    }
  return labels;
  ...
  for (int i = 0; i < TPFIBER_NUM; i++)
    for (int j = 0; j < TPFIBERCH_NUM; j++) {
      HcalElectronicsId eid =
          getEid_FiberuTCATPFiberChuTCATP(i * TPFIBERCH_NUM + j);
      sprintf(name, "%d-%d", eid.fiberIndex(), eid.fiberChanId());
      labels.push_back(std::string(name));
    }
...
void CrateQuantity::setup(std::vector<int> crates,
                          std::map<int, uint32_t> crateHashes) {
