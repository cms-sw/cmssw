...
std::pair<uint16_t, uint16_t> fed2crate(int fed) {
  ...
  std::pair<uint16_t, uint16_t> crate_slot =
      std::make_pair<uint16_t, uint16_t>(0, 0);
  auto it_fed2crate = constants::fed2crate_map.find(fed);
  if (it_fed2crate != constants::fed2crate_map.end()) {
    crate_slot = std::make_pair<uint16_t const, uint16_t const>(
        (uint16_t const)it_fed2crate->second, (uint16_t const)slot);
  }
  return crate_slot;
}
...
uint16_t crate2fed(int crate, int slot) {
  ...
  if (it_crate2fed != constants::crate2fed_map.end()) {
    ...
    if (fed <= FED_VME_MAX && fed > 0) {
      if (slot > 10 && (std::find(constants::crateListVME.begin(),
                                  constants::crateListVME.end(),
                                  crate) != constants::crateListVME.end())) {
        ++fed;
      }
    } else {
      if (slot > 6 && (std::find(constants::crateListuTCA.begin(),
                                 constants::crateListuTCA.end(), crate) !=
                       constants::crateListuTCA
                           .end())) { // needed to handle dual fed readout
        ++fed;
      }
    }
  }
...
std::vector<int> getCrateList(HcalElectronicsMap const *emap) {
  ...
  for (std::vector<HcalElectronicsId>::const_iterator it = vids.begin();
       it != vids.end(); ++it) {
...
std::vector<int> getFEDList(HcalElectronicsMap const *emap) {
  ...
  for (std::vector<HcalElectronicsId>::const_iterator it = vids.begin();
    ...
    uint32_t n = 0;
    for (std::vector<int>::const_iterator jt = vfeds.begin(); jt != vfeds.end();
         ++jt)
      if (fed == *jt)
        break;
      else
