...
ContainerProf2D::ContainerProf2D(std::string const &folder,
                                 hashfunctions::HashType hashtype, Quantity *qx,
                                 Quantity *qy, Quantity *qz)
  ...
  if (_hashmap.isDHash()) {
    ...
    for (std::vector<HcalGenericDetId>::const_iterator it = dids.begin();
      ...
      _mes.insert(std::make_pair(
          hash,
          ib.bookProfile2D(_hashmap.getName(did), _hashmap.getName(did),
                           _qx->nbins(), _qx->min(), _qx->max(), _qy->nbins(),
                           _qy->min(), _qy->max(), _qz->min(), _qz->max())));
      customize(_mes[hash]);
    }
  } else if (_hashmap.isEHash()) {
    //      for Electronics hashes
    std::vector<HcalElectronicsId> eids = emap->allElectronicsIdPrecision();
    for (std::vector<HcalElectronicsId>::const_iterator it = eids.begin();
         it != eids.end(); ++it) {
      ...
      _mes.insert(std::make_pair(
          hash,
          ib.bookProfile2D(_hashmap.getName(eid), _hashmap.getName(eid),
                           _qx->nbins(), _qx->min(), _qx->max(), _qy->nbins(),
                           _qy->min(), _qy->max(), _qz->min(), _qz->max())));
      customize(_mes[hash]);
    }
    ...
    for (std::vector<HcalTrigTowerDetId>::const_iterator it = tids.begin();
      ...
      _mes.insert(std::make_pair(
          hash,
          ib.bookProfile2D(_hashmap.getName(tid), _hashmap.getName(tid),
                           _qx->nbins(), _qx->min(), _qx->max(), _qy->nbins(),
                           _qy->min(), _qy->max(), _qz->min(), _qz->max())));
      customize(_mes[hash]);
    }
...
/* virtual */ void ContainerProf2D::fill(HcalElectronicsId const &eid, double x,
                                         double y, double z) {
  _mes[_hashmap.getHash(eid)]->Fill(_qx->getValue(x), _qy->getValue(y),
                                    _qz->getValue(z));
}

/* virtual */ void ContainerProf2D::fill(HcalTrigTowerDetId const &tid,
                                         double x, double y, double z) {
  _mes[_hashmap.getHash(tid)]->Fill(_qx->getValue(x), _qy->getValue(y),
                                    _qz->getValue(z));
}
