...
/* virtuial */ void
Container1D::initialize(std::string const &folder, std::string const &qname,
                        hashfunctions::HashType hashtype, Quantity *qx,
                        Quantity *qy /* = ... */, int debug /* =0 */) {
...
/* virtual */ void Container1D::load(DQMStore::IGetter &ig,
  ...
  if (_hashmap.isDHash()) {
    ...
    for (std::vector<HcalGenericDetId>::const_iterator it = dids.begin();
      ...
      _mes.insert(
          std::make_pair(hash, ig.get(path + "/" + _hashmap.getName(did))));
    }
  }
    ...
    for (std::vector<HcalElectronicsId>::const_iterator it = eids.begin();
      ...
      _mes.insert(
          std::make_pair(hash, ig.get(path + "/" + _hashmap.getName(eid))));
    }
  } else if (_hashmap.isTHash()) {
    ...
    for (std::vector<HcalTrigTowerDetId>::const_iterator it = tids.begin();
      ...
      _mes.insert(
          std::make_pair(hash, ig.get(path + "/" + _hashmap.getName(tid))));
    }
  }
}
  ...
  if (_hashmap.isDHash()) {
    ...
    for (std::vector<HcalGenericDetId>::const_iterator it = dids.begin();
      ...
      _mes.insert(std::make_pair(
          hash, ib.book1D(_hashmap.getName(did), _hashmap.getName(did),
    ...
    for (std::vector<HcalElectronicsId>::const_iterator it = eids.begin();
      ...
      _logger.debug(_hashmap.getName(eid));
      _mes.insert(std::make_pair(
          hash, ib.book1D(_hashmap.getName(eid), _hashmap.getName(eid),
                          _qx->nbins(), _qx->min(), _qx->max())));
    ...
    for (std::vector<HcalTrigTowerDetId>::const_iterator it = tids.begin();
      ...
      _logger.debug(_hashmap.getName(tid));
      _mes.insert(std::make_pair(
          hash, ib.book1D(_hashmap.getName(tid), _hashmap.getName(tid),
                          _qx->nbins(), _qx->min(), _qx->max())));
      //  customize
      customize(_mes[hash]);
    }
