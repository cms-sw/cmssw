...
uint32_t hash_SubdetPM(HcalDetId const &did) {
  return utilities::hash(
      HcalDetId(did.subdet(), did.ieta() > 0 ? 1 : -1, 1, 1));
}

uint32_t hash_SubdetPMiphi(HcalDetId const &did) {
  return utilities::hash(
      HcalDetId(did.subdet(), did.ieta() > 0 ? 1 : -1, did.iphi(), 1));
}

uint32_t hash_iphi(HcalDetId const &did) {
  return utilities::hash(HcalDetId(HcalBarrel, 1, did.iphi(), 1));
}

uint32_t hash_ieta(HcalDetId const &did) {
  return utilities::hash(HcalDetId(HcalBarrel, did.ieta(), 1, 1));
}
...
uint32_t hash_HFPMiphi(HcalDetId const &did) {
  return utilities::hash(
      HcalDetId(HcalForward, did.ieta() > 0 ? 1 : -1, did.iphi(), 1));
}
...
uint32_t hash_SubdetPM(std::string const &name) {
  for (int i = 0; i < constants::SUBDET_NUM * 2; i++)
    if (constants::SUBDETPM_NAME[i] == name)
      return HcalDetId((HcalSubdetector)(i / 2 + 1), i % 2 == 0 ? -1 : 1, 1, 1)
          .rawId();
...
std::string name_Subdetiphi(HcalDetId const &did) {
  char name[10];
  sprintf(name, "%siphi%d", constants::SUBDET_NAME[did.subdet() - 1].c_str(),
          did.iphi());
  return std::string(name);
}
...
std::string name_Subdetieta(HcalDetId const &did) {
  char name[20];
  sprintf(name, "%sieta%d", constants::SUBDET_NAME[did.subdet() - 1].c_str(),
          did.ieta());
  return std::string(name);
}
...
std::string name_Subdetdepth(HcalDetId const &did) {
  char name[20];
  sprintf(name, "%sdepth%d", constants::SUBDET_NAME[did.subdet() - 1].c_str(),
          did.depth());
  return std::string(name);
}
...
std::string name_SubdetPMiphi(HcalDetId const &did) {
  char name[20];
  sprintf(name, "%s%siphi%d", constants::SUBDET_NAME[did.subdet() - 1].c_str(),
...
uint32_t hash_SubdetPMiphi(std::string const &name) {
  ...
  for (int i = 0; i < constants::SUBDET_NUM * 2; i++)
    if (constants::SUBDETPM_NAME[i] == sub)
      return HcalDetId((HcalSubdetector)(i / 2 + 1), i % 2 == 0 ? -1 : 1, iphi,
                       1)
          .rawId();
...
std::string name_HFPMiphi(HcalDetId const &did) {
  char name[12];
  snprintf(name, sizeof(name), "HF%ciphi%d", did.ieta() > 0 ? 'P' : 'M',
           did.iphi());
  return std::string(name);
}
...
std::string name_DChannel(HcalDetId const &did) {
  char name[40];
  sprintf(name, "%s-%d-%d-%d", constants::SUBDET_NAME[did.subdet() - 1].c_str(),
...
uint32_t hash_FED(HcalElectronicsId const &eid) {
  if (eid.isVMEid())
    return utilities::hash(
        HcalElectronicsId(FIBERCH_MIN, FIBER_VME_MIN, SPIGOT_MIN, eid.dccid()));
  else {
    ...
    return utilities::hash(HcalElectronicsId(
        eid.crateId(), slotToUse, FIBER_uTCA_MIN1, FIBERCH_MIN, false));
  }
}
...
uint32_t hash_FEDSpigot(HcalElectronicsId const &eid) {
  ...
  return eid.isVMEid()
             ? utilities::hash(HcalElectronicsId(FIBERCH_MIN, FIBER_VME_MIN,
                                                 eid.spigot(), eid.dccid()))
             : utilities::hash(HcalElectronicsId(eid.crateId(), eid.slot(),
...
uint32_t hash_Crate(HcalElectronicsId const &eid) {
  ...
  return eid.isVMEid()
             ? utilities::hash(HcalElectronicsId(
                   FIBERCH_MIN, FIBER_VME_MIN, SPIGOT_MIN,
                   (eid.dccid() % 2 == 0 ? eid.dccid() : eid.dccid() - 1)))
             : utilities::hash(HcalElectronicsId(eid.crateId(), SLOT_uTCA_MIN,
                                                 FIBER_uTCA_MIN1, FIBERCH_MIN,
                                                 false));
}
...
uint32_t hash_CrateSpigot(HcalElectronicsId const &eid) {
  ...
  return eid.isVMEid()
             ? utilities::hash(HcalElectronicsId(
                   FIBERCH_MIN, FIBER_VME_MIN, eid.spigot(),
                   (eid.dccid() % 2 == 0 ? eid.dccid() : eid.dccid() - 1)))
             : utilities::hash(HcalElectronicsId(eid.crateId(), eid.slot(),
                                                 FIBER_uTCA_MIN1, FIBERCH_MIN,
                                                 false));
...
uint32_t hash_Electronics(HcalElectronicsId const &eid) {
  return eid.isVMEid()
             ...
             : utilities::hash(HcalElectronicsId(CRATE_uTCA_MIN, SLOT_uTCA_MIN,
                                                 FIBER_uTCA_MIN1, FIBERCH_MIN,
                                                 false));
...
uint32_t hash_EChannel(HcalElectronicsId const &eid) {
  return eid.isVMEid()
             ? utilities::hash(HcalElectronicsId(eid.fiberChanId(),
                                                 eid.fiberIndex(), eid.spigot(),
                                                 eid.dccid()))
             : utilities::hash(HcalElectronicsId(eid.crateId(), eid.slot(),
                                                 eid.fiberIndex(),
                                                 eid.fiberChanId(), false));
}
...
uint32_t hash_FED(std::string const &name) {
  ...
  if (fed >= constants::FED_uTCA_MIN) {
    ...
    return HcalElectronicsId(cspair.first, cspair.second, FIBER_uTCA_MIN1,
                             FIBERCH_MIN, false)
        .rawId();
  } else
    return HcalElectronicsId(FIBERCH_MIN, FIBER_VME_MIN, SPIGOT_MIN,
                             fed - FED_VME_MIN)
        .rawId();
...
uint32_t hash_FEDSpigot(std::string const &name) {
  ...
  if (fed >= FED_uTCA_MIN) {
    ...
    return HcalElectronicsId(FIBERCH_MIN, FIBER_VME_MIN, s, fed - FED_VME_MIN)
        .rawId();
...
std::string name_CrateSpigot(HcalElectronicsId const &eid) {
  char name[20];
  sprintf(name, "Crate%dS%d", eid.crateId(),
          eid.isVMEid() ? eid.spigot() : eid.slot());
  return std::string(name);
}
...
uint32_t hash_Electronics(std::string const &name) {
  if (name == "VME")
    ...
    return HcalElectronicsId(CRATE_uTCA_MIN, SLOT_uTCA_MIN, FIBER_uTCA_MIN1,
                             FIBERCH_MIN, false)
        .rawId();
...
std::string name_EChannel(HcalElectronicsId const &eid) {
  char name[20];
  if (eid.isVMEid())
    sprintf(name, "%d-%d-%d-%d", eid.dccid(), eid.spigot(), eid.fiberIndex(),
            eid.fiberChanId());
  else
    sprintf(name, "%d-%d-%d-%d", eid.crateId(), eid.slot(), eid.fiberIndex(),
            eid.fiberChanId());
  return std::string(name);
}
...
uint32_t hash_TTSubdetPM(HcalTrigTowerDetId const &tid) {
  return utilities::hash(HcalTrigTowerDetId(tid.ietaAbs() < 29
                                                ? (tid.ieta() < 0 ? -1 : 1)
                                                : (tid.ieta() < 0 ? -29 : 29),
                                            1));
}
...
uint32_t hash_TChannel(HcalTrigTowerDetId const &tid) {
  return utilities::hash(
      HcalTrigTowerDetId(tid.ieta(), tid.iphi(), tid.depth()));
}
...
std::string name_TTSubdetPM(HcalTrigTowerDetId const &tid) {
  return constants::TPSUBDETPM_NAME[tid.ietaAbs() < 29
                                        ? (tid.ieta() < 0 ? 0 : 1)
                                        : (tid.ieta() < 0 ? 2 : 3)];
}
