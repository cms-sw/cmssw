...
private:
  ...
  const edm::EDGetTokenT<ZVertexHost>
      tokenSoAVertexReferenceSoA_;  //these two are both on Host but originally they have been
  const edm::EDGetTokenT<ZVertexHost> tokenSoAVertexTargetSoA_;  //produced on Host or on Device
...
void SiPixelCompareVertices::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  if (case_ == "AlpakavsCUDA")
    analyzeSeparate(tokenSoAVertexReferenceSoA_, tokenSoAVertexTargetCUDA_, iEvent);
}
...
void SiPixelCompareVertices::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  desc.add<edm::InputTag>("pixelVertexReferenceSoA", edm::InputTag("pixelVerticesSoA@cpu"));
  desc.add<edm::InputTag>("pixelVertexReferenceCUDA", edm::InputTag("pixelVerticesSoA@cpu"));
  desc.add<edm::InputTag>("pixelVertexTargetSoA", edm::InputTag("pixelVerticesSoA@cpu"));
