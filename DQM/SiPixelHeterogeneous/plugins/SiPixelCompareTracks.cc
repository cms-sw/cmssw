...
public:
  ...
  void analyzeAlpaka(const edm::EDGetTokenT<PixelTrackSoA> tokenRef,
                     const edm::EDGetTokenT<PixelTrackSoA> tokenTar,
                     const edm::Event& iEvent);
  void analyzeAlpakavsCUDA(const edm::EDGetTokenT<PixelTrackSoA> tokenRef,
                           const edm::EDGetTokenT<PixelTrackSoACUDA> tokenTar,
                           const edm::Event& iEvent);
...
private:
  ...
  const edm::EDGetTokenT<PixelTrackSoA>
      tokenSoATrackReference_;  //these two are both on Host but originally they have been
  const edm::EDGetTokenT<PixelTrackSoA> tokenSoATrackTarget_;  //produced on Host or on Device
...
template <typename T>
void SiPixelCompareTracks<T>::analyzeAlpaka(const edm::EDGetTokenT<PixelTrackSoA> tokenRef,
                                            const edm::EDGetTokenT<PixelTrackSoA> tokenTar,
                                            const edm::Event& iEvent) {
...
template <typename T>
void SiPixelCompareTracks<T>::analyzeAlpakavsCUDA(const edm::EDGetTokenT<PixelTrackSoA> tokenRef,
                                                  const edm::EDGetTokenT<PixelTrackSoACUDA> tokenTar,
                                                  const edm::Event& iEvent) {
...
void SiPixelCompareTracks<T>::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  if (case_ == "Alpaka")
    analyzeAlpaka(tokenSoATrackReference_, tokenSoATrackTarget_, iEvent);
  if (case_ == "AlpakavsCUDA")
    analyzeAlpakavsCUDA(tokenSoATrackReference_, tokenSoATrackTargetCUDA_, iEvent);
}
...
void SiPixelCompareTracks<T>::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  desc.add<edm::InputTag>("pixelTrackReferenceSoA", edm::InputTag("pixelTracksSoA@cpu"));
  desc.add<edm::InputTag>("pixelTrackTargetSoA", edm::InputTag("pixelTracksSoA@cpu"));
