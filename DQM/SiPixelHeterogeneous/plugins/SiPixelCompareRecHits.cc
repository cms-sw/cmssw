...
private:
  ...
  const edm::EDGetTokenT<HitsSoA> tokenSoAHitsReference_;  //these two are both on Host but originally they have been
  const edm::EDGetTokenT<HitsSoA> tokenSoAHitsTarget_;     //produced on Host or on Device
...
void SiPixelCompareRecHits<T>::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  if (case_ == "AlpakavsCUDA")
    analyzeSeparate(tokenSoAHitsReference_, tokenSoAHitsTargetCUDA_, iEvent);
}
...
void SiPixelCompareRecHits<T>::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  desc.add<edm::InputTag>("pixelHitsReferenceSoA", edm::InputTag("siPixelRecHitsPreSplittingSoA@cpu"));
  desc.add<edm::InputTag>("pixelHitsTargetSoA", edm::InputTag("siPixelRecHitsPreSplittingSoA@cpu"));
