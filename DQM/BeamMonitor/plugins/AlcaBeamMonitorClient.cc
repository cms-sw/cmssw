...
AlcaBeamMonitorClient::AlcaBeamMonitorClient(const ParameterSet& ps)
  ...
  histoByCategoryNames_.insert(
      pair<string, string>("lumi", "Lumibased PrimaryVertex"));
  histoByCategoryNames_.insert(
      pair<string, string>("lumi", "Lumibased DataBase"));
  histoByCategoryNames_.insert(
      pair<string, string>("lumi", "Lumibased Scalers"));
  histoByCategoryNames_.insert(
      pair<string, string>("lumi", "Lumibased PrimaryVertex-DataBase fit"));
  histoByCategoryNames_.insert(
      pair<string, string>("lumi", "Lumibased PrimaryVertex-Scalers fit"));
  histoByCategoryNames_.insert(
      pair<string, string>("validation", "Lumibased Scalers-DataBase fit"));
  histoByCategoryNames_.insert(
      pair<string, string>("validation", "Lumibased PrimaryVertex-DataBase"));
  histoByCategoryNames_.insert(
      pair<string, string>("validation", "Lumibased PrimaryVertex-Scalers"));

  for (vector<string>::iterator itV = varNamesV_.begin();
...
//----------------------------------------------------------------------------------------------------------------------
void AlcaBeamMonitorClient::beginRun(const edm::Run& r,
                                     const EventSetup& context) {
  for (HistosContainer::iterator itM = histosMap_.begin();
       itM != histosMap_.end(); itM++) {
    for (map<string, map<string, MonitorElement*> >::iterator itMM =
             itM->second.begin();
         itMM != itM->second.end(); itMM++) {
      if (itMM->first != "run") {
        for (map<string, MonitorElement*>::iterator itMMM =
          ...
          if (itMMM->second != nullptr) {
            if (itMM->first == "lumi") {
              ...
              dbe_->removeElement(monitorName_ + "Validation",
                                  itMMM->second->getName());
            } else {
...
void AlcaBeamMonitorClient::endLuminosityBlock(const LuminosityBlock& iLumi,
  ...
  for (int i = 0; i < 3 * numberOfValuesToSave_; i++) {
    valuesMap_[iLumi.id().luminosityBlock()].push_back(
        tmp->getTProfile()->GetBinContent(i + 1));
  }
}
...
void AlcaBeamMonitorClient::endRun(const Run& iRun, const EventSetup& context) {
  ...
  LogInfo("AlcaBeamMonitorClient") << "End of run " << iRun.id().run() << "("
                                   << firstLumi << "-" << lastLumi << ")";
  ...
  unsigned int bin = 0;
  for (HistosContainer::iterator itH = histosMap_.begin();
       itH != histosMap_.end(); itH++) {
    for (map<string, map<string, MonitorElement*> >::iterator itHH =
             itH->second.begin();
         itHH != itH->second.end(); itHH++) {
      for (map<string, MonitorElement*>::iterator itHHH = itHH->second.begin();
        ...
        for (map<LuminosityBlockNumber_t, vector<double> >::iterator itVal =
          ...
          if (itHHH->second != nullptr) {
            ...
            if (itVal->second[positionsMap_[itH->first][itHH->first]
              ...
              itHHH->second->setBinContent(
                  bin, itVal->second[positionsMap_[itH->first][itHH->first]
                                                  [itHHH->first]]);
              itHHH->second->setBinError(
                  bin, itVal->second[positionsMap_[itH->first][itHH->first]
                                                  [itHHH->first] +
                                     1]);
            }
          ...
          if (itHHH->second != nullptr) {
            for (int bin = 1; bin <= itHHH->second->getTH1()->GetNbinsX();
              ...
              if (itHHH->second->getTH1()->GetBinError(bin) != 0 ||
                  itHHH->second->getTH1()->GetBinContent(bin) != 0) {
          ...
          if (itHHH->second != nullptr) {
            if (itHHH->first == "Lumibased BeamSpotFit" ||
              ...
              if ((max == -bigNumber && min == bigNumber) || max - min == 0) {
                itHHH->second->getTH1()->SetMinimum(
                    itHHH->second->getTH1()->GetMinimum() - 0.01);
                itHHH->second->getTH1()->SetMaximum(
                    itHHH->second->getTH1()->GetMaximum() + 0.01);
              } else {
              ...
              if ((maxDelta == -bigNumber && minDelta == bigNumber) ||
                ...
                itHHH->second->getTH1()->SetMinimum(minDelta -
                                                    2 * (maxDelta - minDelta));
                itHHH->second->getTH1()->SetMaximum(maxDelta +
                                                    2 * (maxDelta - minDelta));
              }
            }
            else {
