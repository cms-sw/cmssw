...
BeamSpotDipServer::BeamSpotDipServer(const edm::ParameterSet& ps)
{
  ...
  sourceFile  = ps.getUntrackedParameter<string>("sourceFile" );
  sourceFile1 = ps.getUntrackedParameter<string>("sourceFile1");
...
void BeamSpotDipServer::dqmEndLuminosityBlock(
  const edm::LuminosityBlock& lumiSeg, const edm::EventSetup&)
{
  cerr << "[Dip] --------------------- end of LS "
  ...
  if (!logFile.good()) {
    ...
    if (lastModTime > lastFitTime) {
      ...
      if(getFileSize(sourceFile) > 0) {
        ...
        if (readRcd(logFile)) {
          if (verbose) cout << "[Dip] got new record" << endl;
          trueRcd();
          alive.reset();
          alive.flip(7);
        }
...
/*****************************************************************************/ 
bool BeamSpotDipServer::readRcd(ifstream & file)
{
  int nthLnInRcd = 0;
  bool rcdQlty = false;
