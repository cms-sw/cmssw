	...
	const edm::Run& iRun, const edm::EventSetup& iSetup)
{
 ...
 const int NEtow = 20;
  ...
  sprintf(s,"Castor Energy by trigger");
 h2ETrigger = ibooker.bookProfile(s,s,320,-0.5,319.5, 100,0,1.e10,"");
   h2ETrigger->getTProfile()->GetXaxis()->SetTitle("triggerID");
//   h2ETrigger = ibooker.book1D(s,s,320,-0.5, 319.5);
...
//  iTrig!= hltResults->end(); iTrig++) {;}
 ...
 for (size_t i=0; i<hltResults.size(); i++) 
     if(hltResults.accept(i)) h2ETrigger->Fill(i,etot);
