...
CastorDigiMonitor::CastorDigiMonitor(const edm::ParameterSet& ps)
{
 ...
 if(fVerbosity>0) LogPrint("CastorDigi") 
	...
	const edm::Run& iRun, const edm::EventSetup& iSetup)
{
  ...
  hpTrigRes = ibooker.bookProfile("E(digi)vsTriggerIndex",
  ...
  sprintf(s,"CASTOR TSmax Significance All chan");
    ...
    h2QmeantsvsCh = ibooker.bookProfile2D(st,st+";"+string(sTileIndex)+";TS",
        224,0.,224., 10,0.,10., 0.,1.e10,"");
      h2QmeantsvsCh->getTProfile2D()->SetOption("colz");

    st = "Castor cells avr digiRMS(fC) per event Map TS vs Channel";
      h2QrmsTSvsCh = ibooker.book2D(st,st+";"+string(sTileIndex)+";TS",
	224,0.,224., 10,0.,10.);
...
//     if(err !=0) continue;
   ...
   LogPrint("CastorDigiMonitor")<<"event/idigi=" <<ievt_<<"/"<<i<<
   " cap=cap1_dv_er_err: "<<capid <<"="<< capid1 <<" "<< dv <<" "<<er<<" "<<err;
...
//if(r<0.|| r>1.) cout<<"ievt="<<ievt<<" r="<<r<<" amax= "<<am<<" sum="<<sum<<endl;
...
void CastorDigiMonitor::endRun() {
 if(fVerbosity>0)
  LogPrint("CastorDigiMonitor")<<"DigiMonitor::endRun: trigger max index = "
   <<TrigIndexMax <<"  TriggerIndexies(N):" <<endl;
...
void CastorDigiMonitor::fillTrigRes(edm::Event const& event,
	const edm::TriggerResults& TrigResults, double Etotal) {
...
void CastorDigiMonitor::getDbData(const edm::EventSetup& iSetup) {
 ...
 if(fVerbosity>0) { 
  ...
  if(fVerbosity>0) 
   LogPrint("CastorDigiMonitor")<<"CastorDigiMonitor::getDBData: QualityRcdSize="
    <<channels.size();
 for(std::vector<DetId>::iterator ch=channels.begin(); ch!=channels.end(); ch++) {
    ...
    if(mod>0 && mod<16 && sec>0 && sec<16) ChannelStatus[mod][sec]=value;
 if(fVerbosity>0)
    LogPrint("CastorDigiMonitor")<<chInd<<" module="<<mod<<" sec="<<sec
     <<" rawId="<<rawId<<" value="<<value<<endl;
