...
void Phase2ITMonitorRecHit::fillITHistos(const edm::Event& iEvent, 
  ...
  for (DSViter = rechits->begin(); DSViter != rechits->end(); ++DSViter) {
	...
	globalRZ_endcap_->Fill(gz, gr);
      ...
      //layer wise histo
      if(layerMEs_[key].clusterSizeX) layerMEs_[key].clusterSizeX->Fill(rechitIt->cluster()->sizeX());
      if(layerMEs_[key].clusterSizeY) layerMEs_[key].clusterSizeY->Fill(rechitIt->cluster()->sizeY());
      if(layerMEs_[key].globalPosXY) layerMEs_[key].globalPosXY->Fill(gx, gy); 
      if(layerMEs_[key].globalPosRZ) layerMEs_[key].globalPosRZ->Fill(gz, gr); 
      if(layerMEs_[key].localPosXY) layerMEs_[key].localPosXY->Fill(lp.x(), lp.y()); 
      if(layerMEs_[key].posX) layerMEs_[key].posX->Fill(lp.x());
      if(layerMEs_[key].posY) layerMEs_[key].posX->Fill(lp.y());
      if(layerMEs_[key].posX) layerMEs_[key].poserrX->Fill(rechitIt->localPositionError().xx());
      if(layerMEs_[key].posY) layerMEs_[key].poserrY->Fill(rechitIt->localPositionError().yy());
...
void Phase2ITMonitorRecHit::bookHistograms(DQMStore::IBooker& ibooker,
  ...
  numberRecHits_ = Phase2TkUtil::book1DFromPSet(config_.getParameter<edm::ParameterSet>("GlobalNumberRecHits"),
						HistoName.str(),
						ibooker);
  HistoName.str("");
  HistoName << "Global_Position_XY_IT_barrel";
  globalXY_barrel_ = Phase2TkUtil::book2DFromPSet(config_.getParameter<edm::ParameterSet>("GlobalPositionXY_PXB"),
						  HistoName.str(),
						  ibooker);
  HistoName.str("");
  HistoName << "Global_Position_RZ_IT_barrel";
  globalRZ_barrel_ = Phase2TkUtil::book2DFromPSet(config_.getParameter<edm::ParameterSet>("GlobalPositionRZ_PXB"),
						  HistoName.str(),
						  ibooker);
  HistoName.str("");
  HistoName << "Global_Position_XY_IT_endcap";
  globalXY_endcap_ = Phase2TkUtil::book2DFromPSet(config_.getParameter<edm::ParameterSet>("GlobalPositionXY_PXEC"),
						  HistoName.str(),
						  ibooker);
  HistoName.str("");
  HistoName << "Global_Position_RZ_IT_endcap";
  globalRZ_endcap_ = Phase2TkUtil::book2DFromPSet(config_.getParameter<edm::ParameterSet>("GlobalPositionRZ_PXEC"),
						  HistoName.str(),
						  ibooker);
  ...
  if (theTkDigiGeomWatcher.check(iSetup)) {
	...
	continue;
      ...
      edm::LogInfo("Phase2ITMonitorRecHit") << "Detid:" << detId_raw
...
void Phase2ITMonitorRecHit::bookLayerHistos(DQMStore::IBooker& ibooker,
  ...
  if (layerMEs_.find(key) == layerMEs_.end()) {
    ...
    local_histos.numberRecHits = 
      Phase2TkUtil::book1DFromPSet(config_.getParameter<edm::ParameterSet>("LocalNumberRecHits"),
				   histoName.str(),
				   ibooker);
    histoName.str("");
    histoName << "RecHit_X";
    local_histos.posX = 
      Phase2TkUtil::book1DFromPSet(config_.getParameter<edm::ParameterSet>("RecHitPosX"),
				   histoName.str(),
				   ibooker);
    ...
    histoName << "RecHit_Y";
    local_histos.posY = 
      Phase2TkUtil::book1DFromPSet(config_.getParameter<edm::ParameterSet>("RecHitPosY"),
				   histoName.str(),
				   ibooker);
    ...
    local_histos.poserrX = 
      Phase2TkUtil::book1DFromPSet(config_.getParameter<edm::ParameterSet>("RecHitPosErrorX"),
				   histoName.str(),
				   ibooker);
    ...
    local_histos.poserrY = 
      Phase2TkUtil::book1DFromPSet(config_.getParameter<edm::ParameterSet>("RecHitPosErrorY"),
				   histoName.str(),
				   ibooker);
    ...
    local_histos.clusterSizeX = 
      Phase2TkUtil::book1DFromPSet(config_.getParameter<edm::ParameterSet>("LocalClusterSizeX"),
				   histoName.str(),
				   ibooker);
    histoName.str("");
    histoName << "Cluster_SizeY";
    local_histos.clusterSizeY = 
      Phase2TkUtil::book1DFromPSet(config_.getParameter<edm::ParameterSet>("LocalClusterSizeY"),
				   histoName.str(),
				   ibooker);
    ...
    local_histos.globalPosXY = 
      Phase2TkUtil::book2DFromPSet(config_.getParameter<edm::ParameterSet>("GlobalPositionXY_perlayer"),
				   histoName.str(),
				   ibooker);    
   ...
   histoName.str("");
    ...
    local_histos.globalPosRZ = 
      Phase2TkUtil::book2DFromPSet(config_.getParameter<edm::ParameterSet>("GlobalPositionRZ_perlayer"),
				   histoName.str(),
				   ibooker);    
    histoName.str("");
    histoName << "Local_Position_XY" ;
    local_histos.localPosXY = 
      Phase2TkUtil::book2DFromPSet(config_.getParameter<edm::ParameterSet>("LocalPositionXY"),
				   histoName.str(),
				   ibooker);
