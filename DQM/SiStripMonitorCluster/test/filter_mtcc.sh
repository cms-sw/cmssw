...
###############################################################################################################
#DEFAULT_INPUT_DIRECTORY="/castor/cern.ch/user/d/dkcira/MTCC/2006_08_16_090/";
...
get_list_of_input_files(){
 ...
 if [ "$TYPE_INPUT_DIRECTORY" == "1"  ] ; then
   LIST_OF_DATA_FILES=`nsls $WHICH_INPUT_DIRECTORY | grep "${RUNNR}_rec" | grep '\.root' | head -${MAX_FILES_TO_RUN_OVER}`
 elif [ "$TYPE_INPUT_DIRECTORY" == "2"  ] ; then
   LIST_OF_DATA_FILES=`ls $WHICH_INPUT_DIRECTORY | grep "${RUNNR}_rec" | grep '\.root' | head -${MAX_FILES_TO_RUN_OVER}`
 else
