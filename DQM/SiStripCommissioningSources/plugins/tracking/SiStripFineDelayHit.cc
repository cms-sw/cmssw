...
std::pair<const SiStripCluster*, double> SiStripFineDelayHit::closestCluster(
  ...
  if (homeMadeClusters_) {
    ...
    for (auto DSViter = hits.begin(); DSViter != hits.end(); DSViter++) {
      if (DSViter->id == det_id) {
        ...
        auto rangeStart = DSViter->end();
        auto rangeStop = DSViter->end();
        for (auto digiIt = DSViter->begin(); digiIt != DSViter->end(); ++digiIt) {
