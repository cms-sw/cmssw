    ...
    cppfdigiSource_token_( consumes<l1t::CPPFDigiCollection>(ps.getParameter< InputTag >("rpcdigiSource") ))
{
  ...
  if(verbose_) cout << "L1TStage2CPPF: constructor...." << endl;
...
void L1TStage2CPPF::bookHistograms(DQMStore::IBooker &ibooker, edm::Run const&, edm::EventSetup const&) 
{
  ...
  rpcdqm::utils rpcUtils;
   ...
   for(int d = -numberOfDisks_; d<=numberOfDisks_; d++ ){
     ...
     if(useNormalization_){
       ...
       CPPFInputNormOccupDisk[d+offset] = ibooker.book2D(histoName.str().c_str(), histoName.str().c_str(), 36, 0.5, 36.5, 3*numberOfRings_, 0.5,3*numberOfRings_+ 0.5);
	...
	for( int region = -1; region < 2; region+=2){ 
          ...
          for(int i = 1; i <= 96 ; i++) {
            if (i ==1) meInputDiskRing_1st[os.str()]->setBinLabel(i, "1", 1);
            else if (i==16) meInputDiskRing_1st[os.str()]->setBinLabel(i, "RollA", 1);
            else if (i==32) meInputDiskRing_1st[os.str()]->setBinLabel(i, "32", 1);
            else if (i==33) meInputDiskRing_1st[os.str()]->setBinLabel(i, "1", 1);
            else if (i==48) meInputDiskRing_1st[os.str()]->setBinLabel(i, "RollB", 1);
            else if (i==64) meInputDiskRing_1st[os.str()]->setBinLabel(i, "32", 1);
            else if (i==65) meInputDiskRing_1st[os.str()]->setBinLabel(i, "1", 1);
            else if (i==80) meInputDiskRing_1st[os.str()]->setBinLabel(i, "RollC", 1);
            else if (i==96) meInputDiskRing_1st[os.str()]->setBinLabel(i, "32", 1);
            else  meInputDiskRing_1st[os.str()]->setBinLabel(i, "", 1);
          }
          ...
          for(int i = 1; i <= 96 ; i++) {
            if (i ==1) meInputDiskRing_2nd[os.str()]->setBinLabel(i, "1", 1);
            else if (i==16) meInputDiskRing_2nd[os.str()]->setBinLabel(i, "RollA", 1);
            else if (i==32) meInputDiskRing_2nd[os.str()]->setBinLabel(i, "32", 1);
            else if (i==33) meInputDiskRing_2nd[os.str()]->setBinLabel(i, "1", 1);
            else if (i==48) meInputDiskRing_2nd[os.str()]->setBinLabel(i, "RollB", 1);
            else if (i==64) meInputDiskRing_2nd[os.str()]->setBinLabel(i, "32", 1);
            else if (i==65) meInputDiskRing_2nd[os.str()]->setBinLabel(i, "1", 1);
            else if (i==80) meInputDiskRing_2nd[os.str()]->setBinLabel(i, "RollC", 1);
            else if (i==96) meInputDiskRing_2nd[os.str()]->setBinLabel(i, "32", 1);
            else  meInputDiskRing_2nd[os.str()]->setBinLabel(i, "", 1);
          }
         } //loop on region
       }  //loop ring
   ...
   CPPFInput_Occupancy_DiskRing_Vs_Segment = ibooker.book2D("CPPFInput_Occupancy_DiskRing_Vs_Segment", "CPPF Input Chamber Occupancy", 36, 0.5, 36.5, 12, 0, 12);
   ...
   CPPFInput_Occupancy_DiskRing_Vs_Segment->getTH2F()->GetXaxis()->SetCanExtend(false); // Needed to stop multi-thread summing


   CPPFInput_Occupancy_Ring_Vs_Disk = ibooker.book2D("CPPFInput_Occupancy_Ring_Vs_Disk", "CPPF Input Occupancy Ring Vs Disk", (int )numberOfDisks_*2.0, 0.5, ((float )numberOfDisks_*2.0)+0.5, 2, 1.5, 3.5);
  ...
  ibooker.setCurrentFolder("L1T/L1TCPPF/Outputs");
   ...
   CPPFOutput_Occupancy_DiskRing_Vs_Segment = ibooker.book2D("CPPFOutput_Occupancy_DiskRing_Vs_Segment", "CPPF Chamber Occupancy", 36, 0.5, 36.5, 12, 0, 12);
   ...
   CPPFOutput_Occupancy_DiskRing_Vs_Segment->getTH2F()->GetXaxis()->SetCanExtend(false); // Needed to stop multi-thread summing
   ...
   CPPFOutput_Occupancy_Ring_Vs_Disk = ibooker.book2D("CPPFOutput_Occupancy_Ring_Vs_Disk", "CPPF Output Occupancy Ring Vs Disk", (int )numberOfDisks_*2.0, 0.5, ((float )numberOfDisks_*2.0)+0.5, 2, 1.5, 3.5);
...
{
  nev_++; 
  if(verbose_) cout << "L1TStage2CPPF: analyze...." << endl;
  ...
  const std::map<std::pair<int, int>, int> histIndexCPPF = { {{4,3}, 0}, {{4,2}, 1}, {{3,3}, 2}, {{3,2}, 3}, {{2,2}, 4}, {{1,2}, 5} };
	   ...
	   if( region !=0){
	     ...
	     if(geoServ.segment() > 0 && geoServ.segment() < 19 ){ 
	   	  ...
	   	  if(meInputDiskRing_2nd[os.str()]){
	   		  meInputDiskRing_2nd[os.str()]->Fill(strip + 32*(detId.roll()-1),  geoServ.segment()-18);
	   	  }
