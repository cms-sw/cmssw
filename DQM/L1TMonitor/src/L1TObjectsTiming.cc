      ...
      algoNameLastBxInTrain_(ps.getUntrackedParameter<std::string>("lastBXInTrainAlgo",""))
{
    ...
    const auto algInputTag = ps.getParameter<edm::InputTag>("ugtProducer");
    const auto extInputTag = ps.getParameter<edm::InputTag>("ugtProducer");
    gtUtil_ = new l1t::L1TGlobalUtil(ps, consumesCollector(), *this, algInputTag, extInputTag);

//    const unsigned int bxrange = 5; //this is the out bx range 
    muons_eta_phi.reserve(bxrange);
    jet_eta_phi.reserve(bxrange);
    egamma_eta_phi.reserve(bxrange);
    tau_eta_phi.reserve(bxrange);
    etsum_eta_phi_MET.reserve(bxrange);
    etsum_eta_phi_METHF.reserve(bxrange);
    etsum_eta_phi_MHT.reserve(bxrange);
    etsum_eta_phi_MHTHF.reserve(bxrange);
    muons_eta_phi_isolated.reserve(bxrange);
    jet_eta_phi_isolated.reserve(bxrange);
    egamma_eta_phi_isolated.reserve(bxrange);
    tau_eta_phi_isolated.reserve(bxrange);
    etsum_eta_phi_MET_isolated.reserve(bxrange);
    etsum_eta_phi_METHF_isolated.reserve(bxrange);
    etsum_eta_phi_MHT_isolated.reserve(bxrange);
    etsum_eta_phi_MHTHF_isolated.reserve(bxrange);
    muons_eta_phi_firstbunch.reserve(bxrange);
    jet_eta_phi_firstbunch.reserve(bxrange);
    egamma_eta_phi_firstbunch.reserve(bxrange);
    tau_eta_phi_firstbunch.reserve(bxrange);
    etsum_eta_phi_MET_firstbunch.reserve(bxrange);
    etsum_eta_phi_METHF_firstbunch.reserve(bxrange);
    etsum_eta_phi_MHT_firstbunch.reserve(bxrange);
    etsum_eta_phi_MHTHF_firstbunch.reserve(bxrange);
    muons_eta_phi_lastbunch.reserve(bxrange);
    jet_eta_phi_lastbunch.reserve(bxrange);
    egamma_eta_phi_lastbunch.reserve(bxrange);
    tau_eta_phi_lastbunch.reserve(bxrange);
    etsum_eta_phi_MET_lastbunch.reserve(bxrange);
    etsum_eta_phi_METHF_lastbunch.reserve(bxrange);
    etsum_eta_phi_MHT_lastbunch.reserve(bxrange);
    etsum_eta_phi_MHTHF_lastbunch.reserve(bxrange);
...
void L1TObjectsTiming::bookHistograms(DQMStore::IBooker& ibooker, const edm::Run&, const edm::EventSetup&) {
  ...
  ibooker.setCurrentFolder(monitorDir+"/L1TMuon"+"/timing"); 
  ...
  ibooker.setCurrentFolder(monitorDir+"/L1TJet"+"/timing"); 
  for(unsigned int i=0; i<bxrange; ++i) { 
  ...
  ibooker.setCurrentFolder(monitorDir+"/L1TEGamma"+"/timing"); 
  for(unsigned int i=0; i<bxrange; ++i) {
  ...
  ibooker.setCurrentFolder(monitorDir+"/L1TTau"+"/timing");
  for(unsigned int i=0; i<bxrange; ++i) {
  ...
  ibooker.setCurrentFolder(monitorDir+"/L1TEtSum"+"/timing");
  for(unsigned int i=0; i<bxrange; ++i) {
...
if(algoBitFirstBxInTrain_ > -1 && algoBitLastBxInTrain_ > -1) {
  ...
  ibooker.setCurrentFolder(monitorDir+"/L1TJet"+"/timing"+"/Isolated_bunch");
  for(unsigned int i=0; i<bxrange; ++i) {
  ...
  ibooker.setCurrentFolder(monitorDir+"/L1TEGamma"+"/timing"+"/Isolated_bunch");
  for(unsigned int i=0; i<bxrange; ++i) {
  ...
  ibooker.setCurrentFolder(monitorDir+"/L1TTau"+"/timing"+"/Isolated_bunch");
  for(unsigned int i=0; i<bxrange; ++i) {
  ...
  ibooker.setCurrentFolder(monitorDir+"/L1TEtSum"+"/timing"+"/Isolated_bunch");
  for(unsigned int i=0; i<bxrange; ++i) {
...
if(algoBitFirstBxInTrain_ > -1) {
  ...
  ibooker.setCurrentFolder(monitorDir+"/L1TJet"+"/timing"+"/First_bunch");
  for(unsigned int i=0; i<bxrange-2; ++i) {
  ...
  ibooker.setCurrentFolder(monitorDir+"/L1TEGamma"+"/timing"+"/First_bunch");
  for(unsigned int i=0; i<bxrange-2; ++i) {
  ...
  ibooker.setCurrentFolder(monitorDir+"/L1TTau"+"/timing"+"/First_bunch");
  for(unsigned int i=0; i<bxrange-2; ++i) {
  ...
  ibooker.setCurrentFolder(monitorDir+"/L1TEtSum"+"/timing"+"/First_bunch");
  for(unsigned int i=0; i<bxrange-2; ++i) {
...
if(algoBitLastBxInTrain_ > -1) {
  ...
  ibooker.setCurrentFolder(monitorDir+"/L1TJet"+"/timing"+"/Last_bunch");
  for(unsigned int i=0; i<bxrange-2; ++i) {
  ...
  ibooker.setCurrentFolder(monitorDir+"/L1TEGamma"+"/timing"+"/Last_bunch");
  for(unsigned int i=0; i<bxrange-2; ++i) {
  ...
  ibooker.setCurrentFolder(monitorDir+"/L1TTau"+"/timing"+"/Last_bunch");
  for(unsigned int i=0; i<bxrange-2; ++i) {
  ...
  ibooker.setCurrentFolder(monitorDir+"/L1TEtSum"+"/timing"+"/Last_bunch");
  for(unsigned int i=0; i<bxrange-2; ++i) {
...
void L1TObjectsTiming::analyze(const edm::Event& e, const edm::EventSetup& c) {
  ...
  if (verbose) edm::LogInfo("L1TObjectsTiming") << "L1TObjectsTiming: analyze..." << std::endl;
  ...
  e.getByToken(l1tStage2uGtProducer_, uGtAlgs);
    ...
    for (int itBX = MuonBxCollection->getFirstBX(); itBX <= MuonBxCollection->getLastBX(); ++itBX) {
      for (l1t::MuonBxCollection::const_iterator Muon = MuonBxCollection->begin(itBX); Muon != MuonBxCollection->end(itBX); ++Muon) {
        int index = (int)itBX - std::min(0, 1 - (int)bxrange%2 - (int)std::floor(bxrange/2.)); // the correlation from itBX to respective index of the vector
