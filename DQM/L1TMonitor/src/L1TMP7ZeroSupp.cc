...
void L1TMP7ZeroSupp::bookHistograms(DQMStore::IBooker& ibooker, const edm::Run&, const edm::EventSetup&) {
  ...
  zeroSuppVal_ = ibooker.book1D("zeroSuppVal", "Zero suppression validation summary", 5, -2.5, 2.5);
...
void L1TMP7ZeroSupp::analyze(const edm::Event& e, const edm::EventSetup& c) {
  ...
  if (verbose) edm::LogInfo("L1TMP7ZeroSupp") << "L1TMP7ZeroSupp: analyze..." << std::endl;

  edm::Handle<FEDRawDataCollection> feds;
  e.getByToken(fedDataToken_, feds);

  if (!feds.isValid()) {
    LogError("L1TDQM") << "Cannot analyse: no FEDRawDataCollection found";
    return;
  }
  ...
  for (const auto& fedId: fedIds_) {
    ...
    LogDebug("L1TDQM") << "Found FEDRawDataCollection with ID " << fedId << " and size " << l1tRcd.size();

    if ((int) l1tRcd.size() < slinkHeaderSize_ + slinkTrailerSize_ + amc13HeaderSize_ + amc13TrailerSize_ + amcHeaderSize_ + amcTrailerSize_) {
      if (l1tRcd.size() > 0) {
        LogError("L1TDQM") << "Cannot analyse: invalid L1T raw data (size = " << l1tRcd.size() << ") for ID " << fedId << ".";
      }
      continue;
    } else {
      valid_count++;
    ...
    if (header.check()) {
      LogDebug("L1TDQM") << "Found SLink header:" << " Trigger type " << header.triggerType() << " L1 event ID " << header.lvl1ID() << " BX Number " << header.bxID() << " FED source " << header.sourceID() << " FED version " << header.version();
    } else {
      LogWarning("L1TDQM") << "Did not find a SLink header!";
    }

    FEDTrailer trailer(data + (l1tRcd.size() - slinkTrailerSize_));

    if (trailer.check()) {
      LogDebug("L1TDQM") << "Found SLink trailer:" << " Length " << trailer.lenght() << " CRC " << trailer.crc() << " Status " << trailer.evtStatus() << " Throttling bits " << trailer.ttsBits();
    } else {
      LogWarning("L1TDQM") << "Did not find a SLink trailer!";
    }
    ...
    if (!packet.parse(
             ...
             false,
             false)) {
       LogError("L1TDQM") << "Could not extract AMC13 Packet.";
       return;
    }
...
//          LogDebug("L1TDQM") << "Cannot find an unpacker for"
...
//          LogDebug("L1TDQM") << "Error unpacking data for block ID "
