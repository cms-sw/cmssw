  ...
  if( dbe )
    {
	  ...
	  trackModeVsQ = dbe->book2D("CSCTF_Track_ModeVsQual","CSC Track Mode Vs Quality", 19, -0.5, 18.5, 3, 0.5, 3.5);
      ...
      csctfTrackEta->setBinLabel(2, "-2.4", 1);
      ...
      csctfTrackEta->setBinLabel(63,"2.4", 1);
...
void L1TCSCTF::analyze(const Event& e, const EventSetup& c)
{
	...
	if( trackProducer.label() != "null" )
    {
		...
		for(L1CSCTrackCollection::const_iterator trk=tracks->begin(); trk<tracks->end(); trk++)
		{
			...
			trackModeVsQ->Fill( trk->first.modeExtended(), trk->first.mode() );
			...
			*/
			...
			//if( trigMode > 1 && trigMode !=15) csctfCoincL1ABXN->Fill(L1ABXN);
			//GP_end
		}
	}
