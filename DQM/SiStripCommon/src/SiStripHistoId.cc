...
std::string SiStripHistoId::createHistoId(const std::string& description, const std::string& id_type,uint32_t component_id){
...
std::string SiStripHistoId::createHistoLayer(const std::string& description, const std::string& id_type,const std::string& path,const std::string& flag){
  size_t pos1 = description.find( "__", 0 ); // check if std::string 'description' contains by mistake the 'separator1'
  size_t pos2 = description.find( "__", 0 ); // check if std::string 'description' contains by mistake the 'separator2'
  std::string local_histo_id;
  if ( pos1 == std::string::npos && pos2 == std::string::npos ){ // ok, not found either separator
...
uint32_t SiStripHistoId::getComponentId(std::string histoid){
  ...
  std::istringstream input(returnIdPart(std::move(histoid),3)); input >> local_component_id; // use std::istringstream for casting from std::string to uint32_t
