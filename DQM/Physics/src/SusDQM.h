...
class SusDQM : public DQMEDAnalyzer {
public:
  explicit SusDQM(const edm::ParameterSet&);
  ~SusDQM() override;
...
private:
  ...
  virtual bool goodSusElectron(const Ele*);
  virtual bool goodSusMuon(const Mu*);
...
template <typename Mu, typename Ele, typename Jet, typename Met>
SusDQM<Mu, Ele, Jet, Met>::SusDQM(const edm::ParameterSet& pset) {
  parameters_ = pset;
...
template <typename Mu, typename Ele, typename Jet, typename Met>
SusDQM<Mu, Ele, Jet, Met>::~SusDQM() {}

template <typename Mu, typename Ele, typename Jet, typename Met>
void SusDQM<Mu, Ele, Jet, Met>::bookHistograms(DQMStore::IBooker& iBooker, edm::Run const&, edm::EventSetup const&) {
...
template <typename Mu, typename Ele, typename Jet, typename Met>
bool SusDQM<Mu, Ele, Jet, Met>::goodSusElectron(const Ele* ele) {
...
template <typename Mu, typename Ele, typename Jet, typename Met>
bool SusDQM<Mu, Ele, Jet, Met>::goodSusMuon(const Mu* mu) {
...
template <typename Mu, typename Ele, typename Jet, typename Met>
void SusDQM<Mu, Ele, Jet, Met>::analyze(const edm::Event& evt, const edm::EventSetup& iSetup) {
  ...
  for (typename std::vector<Mu>::const_iterator mu_i = muons->begin(); mu_i != muons->end(); ++mu_i) {
    if (!(goodSusMuon(&(*mu_i)) && mu_i->pt() > RAL_muon_pt_cut_))
      continue;
    ++nMuons;
    ...
    for (typename std::vector<Mu>::const_iterator mu_j = muons->begin(); mu_j != muons->end(); ++mu_j) {
      if (mu_i >= mu_j)
        continue;
      if (!(goodSusMuon(&(*mu_j)) && mu_j->pt() > RAL_muon_pt_cut_))
        continue;
    ...
    for (typename std::vector<Ele>::const_iterator ele_j = elecs->begin(); ele_j != elecs->end(); ++ele_j) {
      if (!(goodSusElectron(&(*ele_j)) && ele_j->pt() > RAL_elec_pt_cut_))
        continue;
  ...
  for (typename std::vector<Ele>::const_iterator ele_i = elecs->begin(); ele_i != elecs->end(); ++ele_i) {
    if (!(goodSusElectron(&(*ele_i)) && ele_i->pt() > RAL_elec_pt_cut_))
      continue;
    nElectrons++;
...
typedef SusDQM<reco::Muon, reco::GsfElectron, reco::CaloJet, reco::CaloMET> RecoSusDQM;
