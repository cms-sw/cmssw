...
vector<float> dqm::Hemisphere::getAxis1(){
    if (status != 1) {
        if (rejectISR == 0) {
            this->Reconstruct();
        } else {
...
vector<float> dqm::Hemisphere::getAxis2(){
    if (status != 1) {
        if (rejectISR == 0) {
            this->Reconstruct();
        } else {
...
vector<int> dqm::Hemisphere::getGrouping(){
    if (status != 1) {
        if (rejectISR == 0) {
            this->Reconstruct();
        } else {
...
int dqm::Hemisphere::Reconstruct(){
    ...
    if (seed_meth == 1) {
        ...
        for (int i = 0; i < vsize; ++i){
            float DeltaR = sqrt((Object_Eta[i] - Object_Eta[I_Max])*(Object_Eta[i] - Object_Eta[I_Max])
                    + (reco::deltaPhi(Object_Phi[i], Object_Phi[I_Max]))*(reco::deltaPhi(Object_Phi[i], Object_Phi[I_Max]))  );
        ...
        for (int i = 0; i < vsize; ++i){
            if( i == I_Max) continue;
            float DeltaR = sqrt((Object_Eta[i] - Object_Eta[I_Max])*(Object_Eta[i] - Object_Eta[I_Max]) + reco::deltaPhi(Object_Phi[i], Object_Phi[I_Max])*reco::deltaPhi(Object_Phi[i], Object_Phi[I_Max]));
