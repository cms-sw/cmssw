...

{
   ...
   elecTag_ = consumes<edm::View<reco::GsfElectron>>(iConfig.getUntrackedParameter<edm::InputTag>("ElecTag", edm::InputTag("gsfElectrons")));
...
void EwkElecDQM::bookHistograms(DQMStore::IBooker & ibooker,
  ...
  Phistar_= ibooker.book1D("PHISTAR",
     "Phi star", 100, 0., 10);
  Phistar_after_= ibooker.book1D("PHISTAR_AFTER",
     "Phi star", 100, 0., 10);
 ...
 char name3[100],title3[100];
  ...
  invpt_before_ = ibooker.book1D("INVPT_BEFORECUTS",
      "Di-electron invariant pt [GeV]", 100, 0., 200.);

  deltaPhi_ = ibooker.book1D("DELTA_PHI_J1_J2",
     "deltaphi b/w j1 and j2", 50, 0.,3.);


  deltaPhi_afterZ_ = ibooker.book1D("DELTA_PHI_J1_J2_aftercuts",
     "deltaphi b/w j1 and j2 after cuts", 50, 0.,3.);


  invmass_after_ = ibooker.book1D("INVMASS_AFTERCUTS",
      "Di-electron invariant mass [GeV]", 100, 0., 200.);

  invpt_after_ = ibooker.book1D("INVPT_AFTERCUTS",
      "Di-electron invariant pt [GeV]", 100, 0., 200.);
  InVaMassJJ_ = ibooker.book1D("INVARIANT MASS_JJ",
    "Invariant mass", 100, 0, 700);

  InVaMassJJ_after_ = ibooker.book1D("INVARIANT MASS_JJ_AFTERCUTS",
    "Invariant mass", 100, 0, 700);

  invmassPU_before_ = ibooker.book2D("INVMASS_PU_BEFORECUTS",
     "Di-electron invariant mass [GeV] vs PU; mass [GeV]; PU count", 100, 0.,200., PUBinCount_, -0.5, PUMax_ + 0.5);
...
void EwkElecDQM::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
 ...
 // Vertices in the event
  ...
  for (unsigned int i = 0; i < jetCollectionSize; i++) {
    ...
    if (jet2_px != -1 && jet2_py != -1 && jet2_pz != -1 && jet2_p != -1 && jet3_px != -1 && jet3_py != -1 && jet3_pz != -1 && jet3_p != -1)
   {
   const math::XYZTLorentzVector jets(jet3_px+ jet2_px,jet3_py+ jet2_py,jet3_pz+ jet2_pz,jet3_p+ jet2_p);
...

{
  ...
  const math::XYZTLorentzVector Reco(electron[0][3]+ electron[1][3], electron[0][4]+electron[1][4],electron[0][5]+electron[1][5], electron[0][2] +electron[1][2] );   
  ...
  const math::XYZTLorentzVector Reco(goodElectron[0][3]+ goodElectron[1][3], goodElectron[0][4]+goodElectron[1][4],goodElectron[0][5]+goodElectron[1][5], goodElectron[0][2] +goodElectron[1][2] );
