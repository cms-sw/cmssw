...
SMPDQM::SMPDQM(const edm::ParameterSet& iConfig)
{
  ...
  elecs_ = consumes<reco::GsfElectronCollection>(
  						 iConfig.getParameter<edm::InputTag>("elecCollection"));
  jets_ = consumes<edm::View<reco::PFJet> >(
...
void
SMPDQM::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  bool verbose = false;//true;
   ...
   if (!iEvent.getByToken(*met_, met)) continue;
  ...
  // Muons
   ...
   if(!iEvent.getByToken(pvs_, pvs)) { return;}

   unsigned int pvMult = 0;
  
   if(verbose)cout<<"check this"<<pvs->size()<<endl;
     ...
     NPV->Fill(pvMult);
     if(verbose)cout<<"filled vertices"<<pvMult<<endl;
   ...
   if (!muons.failedToGet()) {
     ...
     if(verbose)cout<<"going to muons \t "<<muons->size()<<endl;
     for (mu = muons->begin(); mu != muons->end(); ++mu) {
       if(mu->pt() < 3.0)continue;
       if(verbose)cout<<"looking into muons "<<endl;
       TLorentzVector Mu;
       ...
       if(verbose)cout<<"looking into muon kinematics "<<endl;
   ...
   if(verbose)cout<<"done with muons"<<endl;
   ...
   if (!elecs.failedToGet()) {
     ...
     if(verbose)cout<<"looking into electrons\t"<<elecs->size()<<endl;
     for (elec = elecs->begin(); elec != elecs->end(); ++elec) {
       if(elec->pt() < 5.0) continue;
       TLorentzVector El;
       El.SetPtEtaPhiM(elec->pt(),elec->eta(),elec->phi(),0.0);
       selected_lep.push_back(El);
       
       
       if(verbose)cout<<"looking into electrons kinematics "<<endl;
     ...
     if(verbose)cout<<"done with electrons "<<endl;
   // jets

   edm::Handle<edm::View<reco::PFJet> > jets;
   if(!iEvent.getByToken(jets_, jets)) { return;}
   ...
   if(verbose)   std::cout<<"size of uncleaned jets"<<recoPFJets.size()<<std::endl;
   ...
   for (unsigned int i=0;i <recoPFJets.size();i++){
	 ...
	 goodjet=false;
	 if(verbose)  std::cout<<"jet\t"<<"\t matched to lep with deltaR = \t"<<recoPFJets[i].DeltaR(selected_lep[j])<<"\t"<<selected_lep[j].Pt()<<"\t"<<recoPFJets[i].Pt()<<std::endl;
         break;
       }
     }
     if(goodjet){
       TLorentzVector temp; temp.Clear();
