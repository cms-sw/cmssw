...
void MonitorEnsemble::book(std::string directory) {
  ...
  hists_["muonPFRelIso_"] =
      store_->book1D("MuonPFRelIso", "PFIso_{Rel}(#mu)", 50, 0., 1.);
  hists_["muonRelIso_"] =
      store_->book1D("MuonRelIso", "Iso_{Rel}(#mu)", 50, 0., 1.);
  ...
  hists_["elecPFRelIso_"] =
      store_->book1D("ElecPFRelIso", "PFIso_{Rel}(e)", 50, 0., 1.);

  // multiplicity of btagged jets (for track counting high efficiency) with
  // pt(L2L3)>30
  hists_["jetMultBEff_"] =
      store_->book1D("JetMultBEff", "N_{30}(b/eff)", 10, 0., 10.);
  // btag discriminator for track counting high efficiency for jets with
  // pt(L2L3)>30
  hists_["jetBDiscEff_"] =
      store_->book1D("JetBDiscEff", "Disc_{b/eff}(jet)", 100, 0., 10.);
  ...
  hists_["TaggedJetEta_"] =
      store_->book1D("TaggedJetEta", "#eta (Tagged jet)", 50, -5., 5.);
  hists_["TaggedJetPt_"] =
      store_->book1D("TaggedJetPt", "pt_{L2L3}(Tagged jet)", 60, 0., 300.);

  // eta and pt of the jet not passing b-tag (filled only when nJets==2)
  hists_["UnTaggedJetEta_"] =
      store_->book1D("UnTaggedJetEta", "#eta (UnTagged jet)", 50, -5., 5.);
  hists_["UnTaggedJetPt_"] =
      store_->book1D("UnTaggedJetPt", "pt_{L2L3}(UnTagged jet)", 60, 0., 300.);

  // eta and pt of the most forward jet in the event with nJets==2
  hists_["FwdJetEta_"] =
      store_->book1D("FwdJetEta", "#eta (Fwd jet)", 50, -5., 5.);
  hists_["FwdJetPt_"] =
      store_->book1D("FwdJetPt", "pt_{L2L3}(Fwd jet)", 60, 0., 300.);

  // 2D histogram (pt,eta) of the b-tagged jet (filled only when nJets==2)
  hists_["TaggedJetPtEta_"] =
      store_->book2D("TaggedJetPt_Eta", "(pt vs #eta)_{L2L3}(Tagged jet)", 60,
                     0., 300., 50, -5., 5.);

  // 2D histogram (pt,eta) of the not-b tagged jet (filled only when nJets==2)
  hists_["UnTaggedJetPtEta_"] =
      store_->book2D("UnTaggedJetPt_Eta", "(pt vs #eta)_{L2L3}(UnTagged jet)",
                     60, 0., 300., 50, -5., 5.);
  ...
  hists_["muonChHadIso_"] =
      store_->book1D("MuonChHadIso", "Iso_{ChHad}(#mu)", 100, 0., 1.);
  // relative muon isolation from neutral hadrons for the leading muon
  hists_["muonNeuHadIso_"] =
      store_->book1D("MuonNeuHadIso", "Iso_{NeuHad}(#mu)", 100, 0., 1.);
  // relative muon isolation from photons for the leading muon
  hists_["muonPhIso_"] =
      store_->book1D("MuonPhIso", "Iso_{Ph}(#mu)", 100, 0., 1.);

  // relative electron isolation from charged hadrons for the leading electron
  hists_["elecChHadIso_"] =
      store_->book1D("ElecChHadIso", "Iso_{ChHad}(e)", 100, 0., 1.);
  // relative electron isolation from neutral hadrons for the leading electron
  hists_["elecNeuHadIso_"] =
      store_->book1D("ElecNeuHadIso", "Iso_{NeuHad}(e)", 100, 0., 1.);
  // relative electron isolation from photons for the leading electron
  hists_["elecPhIso_"] =
      store_->book1D("ElecPhIso", "Iso_{Ph}(e)", 100, 0., 1.);

  // multiplicity of btagged jets (for track counting high purity) with
  // pt(L2L3)>30
  hists_["jetMultBPur_"] =
      store_->book1D("JetMultBPur", "N_{30}(b/pur)", 10, 0., 10.);
  // btag discriminator for track counting high purity
  hists_["jetBDiscPur_"] =
      store_->book1D("JetBDiscPur", "Disc_{b/pur}(Jet)", 200, -10., 10.);
  // btag discriminator for track counting high purity for 1. leading jet
  hists_["jet1BDiscPur_"] =
      store_->book1D("Jet1BDiscPur", "Disc_{b/pur}(Jet1)", 200, -10., 10.);
  // btag discriminator for track counting high purity for 2. leading jet
  hists_["jet2BDiscPur_"] =
      store_->book1D("Jet2BDiscPur", "Disc_{b/pur}(Jet2)", 200, -10., 10.);

  // multiplicity of btagged jets (for simple secondary vertex) with pt(L2L3)>30
  hists_["jetMultBVtx_"] =
      store_->book1D("JetMultBVtx", "N_{30}(b/vtx)", 10, 0., 10.);
  // btag discriminator for simple secondary vertex
  hists_["jetBDiscVtx_"] =
      store_->book1D("JetBDiscVtx", "Disc_{b/vtx}(Jet)", 35, -1., 6.);

  // multiplicity of btagged jets (for combined secondary vertex) with
  // pt(L2L3)>30
  hists_["jetMultBCombVtx_"] =
      store_->book1D("JetMultBCombVtx", "N_{30}(b/CSV)", 10, 0., 10.);
  // btag discriminator for combined secondary vertex
  hists_["jetBDiscCombVtx_"] =
      store_->book1D("JetBDiscCombVtx", "Disc_{b/CSV}(Jet)", 60, -1., 2.);
  // btag discriminator for combined secondary vertex for 1. leading jet
  hists_["jet1BDiscCombVtx_"] =
      store_->book1D("Jet1BDiscCombVtx", "Disc_{b/CSV}(Jet1)", 60, -1., 2.);
  // btag discriminator for combined secondary vertex for 2. leading jet
  hists_["jet2BDiscCombVtx_"] =
      store_->book1D("Jet2BDiscCombVtx", "Disc_{b/CSV}(Jet2)", 60, -1., 2.);
