...
void NPSDQM::bookHistograms(DQMStore::IBooker& bei, edm::Run const&, edm::EventSetup const&) {
  ...
  for (const auto& label : jetLabels_) {
    CentralJet_HT.push_back(
        bei.book1D("CentralJet_HT_" + label, "Central Jet HT (|#eta| < 2.5, pT > 30) " + label + "; GeV", 50, 0, 2000));
    ForwardJet_HT.push_back(
        bei.book1D("ForwardJet_HT_" + label, "Forward Jet HT (|#eta| > 2.5, pT > 30) " + label + "; GeV", 50, 0, 2000));
    ...
    Jet_btagParticleNet.push_back(
        bei.book1D("Jet_btagParticleNet_" + label, "Jet b-tag Score (ParticleNet) " + label, 50, 0, 1.0));
    Jet_btagRobustParT.push_back(
        bei.book1D("Jet_btagRobustParT_" + label, "Jet b-tag Score (RobustParT) " + label, 50, 0, 1.0));
...
void NPSDQM::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (unsigned int icoll = 0; icoll < jetLabels_.size(); ++icoll) {
    ...
    if (!iEvent.getByToken(jetTokens_[icoll], pfJetCollection))
      continue;
    ...
    for (unsigned int ijet = 0; ijet < pfJetCollection->size(); ++ijet) {
      ...
      if (jet_count < 4 && validMET) {
        ...
        if (jet_count == 0)
          dPhi_Jet1_MET[icoll]->Fill(dphi);
        else if (jet_count == 1)
          dPhi_Jet2_MET[icoll]->Fill(dphi);
        else if (jet_count == 2)
          dPhi_Jet3_MET[icoll]->Fill(dphi);
        else if (jet_count == 3)
          dPhi_Jet4_MET[icoll]->Fill(dphi);
      }

      // Safely fill all four B-Tagging algorithms!
      if (validDeepCSV)
        Jet_btagDeepCSV[icoll]->Fill((*btagDeepCSVColl)[jetRef]);
      if (validDeepJet)
        Jet_btagDeepJet[icoll]->Fill((*btagDeepJetColl)[jetRef]);
      if (validParticleNet)
        Jet_btagParticleNet[icoll]->Fill((*btagParticleNetColl)[jetRef]);
      if (validRobustParT)
        Jet_btagRobustParT[icoll]->Fill((*btagRobustParTColl)[jetRef]);
