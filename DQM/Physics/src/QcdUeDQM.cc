...
//--------------------------------------------------------------------------------------------------
void QcdUeDQM::fill1D(std::vector<MonitorElement *> &mes, double val,
                      double w) {
  // Loop over histograms and fill if trigger has fired.
...
//--------------------------------------------------------------------------------------------------
void QcdUeDQM::fill2D(std::vector<TH2F *> &hs, double valx, double valy,
                      double w) {
  // Loop over histograms and fill if trigger has fired.
...
//--------------------------------------------------------------------------------------------------
void QcdUeDQM::fill2D(std::vector<MonitorElement *> &mes, double valx,
...
//--------------------------------------------------------------------------------------------------
void QcdUeDQM::fillProfile(std::vector<TProfile *> &hs, double valx,
                           double valy, double w) {
  // Loop over histograms and fill if trigger has fired.
...
//--------------------------------------------------------------------------------------------------
void QcdUeDQM::fillProfile(std::vector<MonitorElement *> &mes, double valx,
                           double valy, double w) {
  // Loop over histograms and fill if trigger has fired.
...
//--------------------------------------------------------------------------------------------------
bool QcdUeDQM::trackSelection(const reco::Track &trk, const reco::BeamSpot *bs,
...
bool QcdUeDQM::fillVtxPlots(const reco::BeamSpot *bs,
  ...
  for (const auto & theVertice : theVertices) {
    ...
    if (fabs(theVertice.z() - bs->z0()) < diffvtxbs_ && theVertice.ndof() >= 4 &&
        theVertice.position().rho() <= 2.0) {
      goodVtx = true;
      vtx1 = theVertice;
...
void QcdUeDQM::fillUE_with_ChargedJets(
    const std::vector<const reco::Track *> &track,
    const edm::Handle<reco::TrackJetCollection> &trackJets) {
  double w = 0.119;
  double nTrk500_TransReg = 0;
