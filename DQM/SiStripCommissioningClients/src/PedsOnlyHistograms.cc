...
void PedsOnlyHistograms::histoAnalysis(bool debug) {
  ...
  for (iter = histos().begin(); iter != histos().end(); iter++) {
    ...
    if (iter->second.empty()) {
      edm::LogWarning(mlDqmClient_) << "[PedsOnlyHistograms::" << __func__ << "]"
    ...
    auto* anal = new PedsOnlyAnalysis(iter->first);
  ...
  if (!histos().empty()) {
    edm::LogVerbatim(mlDqmClient_) << "[PedsOnlyHistograms::" << __func__ << "]"
