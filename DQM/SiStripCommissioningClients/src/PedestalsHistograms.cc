...
void PedestalsHistograms::histoAnalysis(bool debug) {
  ...
  for (iter = histos().begin(); iter != histos().end(); iter++) {
    ...
    if (iter->second.empty()) {
      edm::LogWarning(mlDqmClient_) << "[PedestalsHistograms::" << __func__ << "]"
    ...
    auto* anal = new PedestalsAnalysis(iter->first);
  ...
  if (!histos().empty()) {
    edm::LogVerbatim(mlDqmClient_) << "[PedestalsHistograms::" << __func__ << "]"
