...
void RPCEfficiencySecond::dqmEndJob(DQMStore::IBooker& ibooker, DQMStore::IGetter& igetter) {
        ...
        if (region == 0) {                    //Barrel
          ...
          if (histoRPC && histoDT) {
            for (int i = 1; i <= int((*r)->nstrips()); ++i) {
              ...
              LogDebug("rpcefficiencysecond")
                  << "Strip=" << i << " RPC=" << histoRPC->getBinContent(i) << " DT=" << histoDT->getBinContent(i)
                  << " buffef=" << buffef << " sumbuffef=" << sumbuffef
                  << " NumberStripsPointed=" << NumberStripsPointed << " NumberWithOutPrediction"
                  << NumberWithOutPrediction;
            }
          ...
          int layer = 0;
          if (station == 1 && geolayer == 1)
            layer = 1;
          else if (station == 1 && geolayer == 2)
            layer = 2;
          else if (station == 2 && geolayer == 1)
            layer = 3;
          else if (station == 2 && geolayer == 2)
            layer = 4;
          else if (station == 3)
            layer = 5;
          else if (station == 4)
            layer = 6;
          ExsectorEffW[wheel + 2]->Fill(sector, p);
          OcsectorEffW[wheel + 2]->Fill(sector, o);
          ExpLayerW[wheel + 2]->Fill(layer, p);
          ObsLayerW[wheel + 2]->Fill(layer, o);
          ...
          if (histoRPC && histoCSC) {
            ...
            for (int i = 1; i <= int((*r)->nstrips()); ++i) {
              ...
              LogDebug("rpcefficiencysecond")
                  << "Strip=" << i << " RPC=" << histoRPC->getBinContent(i) << " CSC=" << histoCSC->getBinContent(i)
                  << " buffef=" << buffef << " buffer=" << buffer << " sumbuffef=" << sumbuffef
                  << " sumbuffer=" << sumbuffer << " NumberStripsPointed=" << NumberStripsPointed
          ...
          if (Disk > numberOfDisks_ || Disk < -numberOfDisks_) {
            continue;
          }  //remove strange disk numbers!!
          int dIndex = -1;
          if (region == -1) {
          ...
          if (dIndex < 0 || dIndex >= numberOfDisks_ * 2) {
            continue;
          }  //extra check on disk numering
          if (p != 0) {
            ef = double(o) / double(p);
            er = sqrt(ef * (1. - ef) / double(p));
          }
          ...
          if (EffDistroD[dIndex]) {
            EffDistroD[dIndex]->Fill(averageeff);
          }
          indexDisk[dIndex]++;
          if (EffGlobD[dIndex]) {
  ...
  for (int d = 0; d < (numberOfDisks_ * 2); d++) {
    for (k = 1; k <= 36; k++) {
      err = 0;
      eff = 0;
      N = ExGregR2D[d]->getBinContent(k);
      if (N != 0.) {
        eff = OcGregR2D[d]->getBinContent(k) / N;
        err = sqrt(eff * (1 - eff) / N);
      }
      GregR2D[d]->setBinContent(k, eff);
      GregR2D[d]->setBinError(k, err);
      err = 0;
      eff = 0;
      N = ExGregR3D[d]->getBinContent(k);
      if (N != 0.) {
        eff = OcGregR3D[d]->getBinContent(k) / N;
        err = sqrt(eff * (1 - eff) / N);
      }
      GregR3D[d]->setBinContent(k, eff);
      GregR3D[d]->setBinError(k, err);
    }
  }
  for (int w = -2; w <= 2; w++) {
    for (k = 1; k <= 12; k++) {
      err = 0;
      eff = 0;
      N = ExsectorEffW[w + 2]->getBinContent(k);
      if (N != 0.) {
        eff = OcsectorEffW[w + 2]->getBinContent(k) / N;
        err = sqrt(eff * (1 - eff) / N);
      }
      sectorEffW[w + 2]->setBinContent(k, eff);
      sectorEffW[w + 2]->setBinError(k, err);
    }
  }
  //Ranges for Both
  //Barrel
