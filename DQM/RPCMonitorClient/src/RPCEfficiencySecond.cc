...
void RPCEfficiencySecond::endRun(const edm::Run& r, const edm::EventSetup& iSetup){
  ...
  int indexDisk[10];
  for(int j=0;j<(numberOfDisks_*2);j++){
    indexDisk[j]=0;
  }
  ...
  for(TrackingGeometry::DetContainer::const_iterator it=rpcGeo->dets().begin();it<rpcGeo->dets().end();it++){
	...
	if(region==0){//Barrel
	  ...
	  int layer = 0;
	    ...
	    if(region==1){ //Positive Endcap
	      
	      if(ring==2){ ExGregR2D[Disk+numberOfDisks_-1]->Fill(rpcsrv.segment(),p);OcGregR2D[Disk+numberOfDisks_-1]->Fill(rpcsrv.segment(),o);} 
	      else if(ring==3){ ExGregR3D[Disk+numberOfDisks_-1]->Fill(rpcsrv.segment(),p);OcGregR3D[Disk+numberOfDisks_-1]->Fill(rpcsrv.segment(),o);}
		...
		DiskSummary[Disk+numberOfDisks_-1]->setBinContent(xBin, yBin, averageeff);
		
	      }
	      
	      
	      EffDistroD[Disk+numberOfDisks_-1]->Fill(averageeff);
	      indexDisk[Disk+numberOfDisks_-1]++;  
	      EffGlobD[Disk+numberOfDisks_-1]->setBinContent(indexDisk[Disk+numberOfDisks_-1],ef);  
	      EffGlobD[Disk+numberOfDisks_-1]->setBinError(indexDisk[Disk+numberOfDisks_-1],er);  
	      EffGlobD[Disk+numberOfDisks_-1]->setBinLabel(indexDisk[Disk+numberOfDisks_-1],camera,1);  
	      
	      
	      
	    
	    }else if(region==-1){//Negative Encap
	  
	      if(ring==2){ ExGregR2D[Disk+numberOfDisks_]->Fill(rpcsrv.segment(),p);OcGregR2D[Disk+numberOfDisks_]->Fill(rpcsrv.segment(),o);} 
	      else if(ring==3){ ExGregR3D[Disk+numberOfDisks_]->Fill(rpcsrv.segment(),p);OcGregR3D[Disk+numberOfDisks_]->Fill(rpcsrv.segment(),o);}
		...
		DiskSummary[Disk+numberOfDisks_]->setBinContent(xBin, yBin, averageeff);
		
	      }
	      
	      
	      EffDistroD[Disk+numberOfDisks_]->Fill(averageeff);
	      indexDisk[Disk+numberOfDisks_]++;  
	      EffGlobD[Disk+numberOfDisks_]->setBinContent(indexDisk[Disk+numberOfDisks_],ef);  
	      EffGlobD[Disk+numberOfDisks_]->setBinError(indexDisk[Disk+numberOfDisks_],er);  
	      EffGlobD[Disk+numberOfDisks_]->setBinLabel(indexDisk[Disk+numberOfDisks_],camera,1);
	    }
	  }
	}
      }
