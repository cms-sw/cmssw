...
void RPCEfficiencyShiftHisto::dqmEndJob(DQMStore::IBooker &ibooker, DQMStore::IGetter &igetter) {
      ...
      for (int s = 1; s <= myMe->getNbinsX(); s++) {
        for (int r = 1; r <= myMe->getNbinsY(); r++) {
          ...
          if (effBarrel >= 0) {  //5
            ...
            if (EffBarrelRoll)
              EffBarrelRoll->Fill(effBarrel);
            entriesBarrel++;

            if (effBarrel >= effCut_)
              entriesBarrelBeyondEff++;
          }
        }
      }
    }
  ...
  if (entriesBarrel != 0) {
    ...
    if (RollPercentage)
      RollPercentage->setBinContent(1, 2, percBarrel);
  }
    ...
    if (myMe) {
      for (int x = 1; x <= myMe->getNbinsX(); x++) {
        for (int y = 1; y <= myMe->getNbinsY(); y++) {
          ...
          if (d < 0) {
            entriesEndcapMinus++;
            if (EffEndcapMinusRoll)
              EffEndcapMinusRoll->Fill(effEndcap);
            if (effEndcap >= effCut_)
              entriesEndcapMinusBeyondEff++;
          } else {
            entriesEndcapPlus++;
            if (EffEndcapPlusRoll)
              EffEndcapPlusRoll->Fill(effEndcap);
            if (effEndcap >= effCut_)
              entriesEndcapPlusBeyondEff++;
          }
        }
      }
    }
  ...
  if (entriesEndcapMinus != 0) {
    ...
    if (RollPercentage)
      RollPercentage->setBinContent(1, 3, percEndcapMinus);
  }

  if (entriesEndcapPlus != 0) {
    percEndcapPlus = 100 * entriesEndcapPlusBeyondEff / entriesEndcapPlus;
    if (RollPercentage)
      RollPercentage->setBinContent(1, 1, percEndcapPlus);
  }
}
