...
CTPPSDiamondDQMSource::PotPlots::PotPlots( DQMStore::IBooker& ibooker, unsigned int id )
{
  ...
  path.replace( 0, 5, "TimingDiamond" );
  ibooker.setCurrentFolder( std::string( "CTPPS/" ) + path );
//   if ( verbosity_ ) cout<<"Booking RP: "<<path<<"\t\t"<<id<<endl;
  ...
  hitDistribution2d = ibooker.book2D( "hits in planes (2D)", title+";plane number;x (mm)", 9, -0.5, 4, 1900, -1, 18 );
  hitDistribution2dOOT= ibooker.book2D( "hits with OOT (x0_25) in planes (2D)", title+";plane number;x (mm)", 17, -0.25, 4, 600, 0, 18 );
  activePlanes = ibooker.book1D( "active planes", title+";number of active planes", 6, -0.5, 5.5);

  trackDistribution = ibooker.book1D( "tracks", title+";x (mm)", 950, -1, 18 );
  trackDistribution_minus1 = ibooker.book1D( "tracks OOT -1", title+";x (mm)", 950, -1, 18 );
  trackDistributionOOT = ibooker.book2D( "tracks with OOT", title+";plane number;x (mm)", 9, -0.5, 4, 600, 0, 18 );
  ...
  leadingEdgeCumulativePot = ibooker.book1D( "Leading Edge", title+";leading edge", 1001, 0, 100 );
  timeOverThresholdCumulativePot = ibooker.book1D( "Time over Threshold", title+";time over threshold", 2e3, -1000, 1000 );
  leadingTrailingCorrelationPot = ibooker.book2D( "Leading Trailing Correlation", title+";leading;trailing", 3e2, 0, 60, 3e2, 0, 60 );
  ...
  ec_check = ibooker.book2D("optorxEC(8bit) - vfatEC", title+";HPTDC ID;optorxEC-vfatEC", 9,-0.5,4,512,-256,256);
...
CTPPSDiamondDQMSource::PlanePlots::PlanePlots( DQMStore::IBooker& ibooker, unsigned int id )
{
  ...
  path.replace( 0, 5, "TimingDiamond" );
  ibooker.setCurrentFolder( std::string( "CTPPS/" ) + path );
//   if ( verbosity_ ) cout<<"Booking Plane: "<<path<<"\t\t"<<id<<endl;
  ...
  hit_profile = ibooker.book1D( "hit (old) profile", title+";hits/detector/event", 180, 0, 18 );
...
{
  std::string path, title;
  CTPPSDiamondDetId( id ).channelName( path, CTPPSDiamondDetId::nPath );	//TODO
  path.replace( 0, 5, "TimingDiamond" );
  ibooker.setCurrentFolder( std::string( "CTPPS/" ) + path );
//   if ( verbosity_ ) cout<<"Booking Channel: "<<path<<"\t\t"<<id<<endl;
  ...
  channel_ec_check = ibooker.book2D( "optorxEC(8bit) - vfatEC vs optorxEC", title+";optorxEC-vfatEC", 7e3+1, 0, 7e3, 513, -256, 256 );
...
//          potPlots_[detId_pot].ec_check->Fill(DiamondTmp::HPTDCId(dit->getID()),(int)((optorx.getLV1()& 0xFF)-((unsigned int) dit->getEC() & 0xFF)) & 0xFF);
...
//          potPlots_[detId_pot].ec_check->Fill(DiamondTmp::HPTDCId(dit->getID()),(int)((optorx.getLV1()& 0xFF)-dit->getEC()) & 0xFF);
  ...
  for ( edm::DetSetVector<CTPPSDiamondRecHit>::const_iterator it=hits->begin(); it != hits->end(); ++it ) {
	...
	const double rnd = ( 2. * randomGen.Rndm() ) - 1.;
        ...
        /*if (hitIt->getToT() != 0.0) */potPlots_[detId_pot].timeOverThresholdCumulativePot->Fill( hitIt->getToT() );
        /*if (hitIt->getToT() != 0.0) */potPlots_[detId_pot].leadingTrailingCorrelationPot->Fill( hitIt->getT(), hitIt->getT() + hitIt->getToT() );
  ...
  for ( edm::DetSetVector<CTPPSDiamondLocalTrack>::const_iterator it=localTracks->begin(); it != localTracks->end(); ++it ) {
    ...
    for ( edm::DetSet<CTPPSDiamondLocalTrack>::const_iterator trackIt = it->begin(); trackIt != it->end(); ++trackIt ) {
      ...
      if ( potPlots_.find( detId_pot ) != potPlots_.end() ) {
        ...
        if ( trackIt->getOOTIndex() == 0 ) potPlots_[detId_pot].trackDistribution_minus1->Fill( trackIt->getX0()+rnd*trackIt->getX0Sigma() );
...
// 		  potPlots_[detId_pot].activity_per_fedbx_filtered->Fill(fit.getBX());
// 		  potPlots_[detId_pot].activity_per_fedbx_short_filtered->Fill(fit.getBX());
// //                   cout<<"added"<<endl;
// 		}
...
//     unsigned int DetId = DiamondDetId::rawToDecId(it->detId());
    for ( edm::DetSet<CTPPSDiamondDigi>::const_iterator dit = it->begin(); dit != it->end(); ++dit ) {
      ...
      if ( planePlots_.find( detId_plane ) != planePlots_.end() ) {
        ...
        if ( dit->getLeadingEdge() != 0 /*&& dit->getTrailingEdge() != 0.0*/ ) planePlots_[detId_plane].digi_profile_cumulative->Fill( detId.channel() );
//        if ( (dit->getLeadingEdge() != 0 /*&& dit->getTrailingEdge() != 0.0*/ ) and ( dit->getLeadingEdge()>0 && dit->getLeadingEdge()<1024 ) ) planePlots_[detId_plane].hit_profile->Fill( DiamondTmp::XRndPosition( detId ) );
      ...
      if ( channelPlots_.find(detId) != channelPlots_.end() ) {
        for ( auto &optorx : *fedInfo ) {
          if ( ( detId.arm() == 0 && optorx.getFEDId() == 583 ) || ( detId.arm() == 1 && optorx.getFEDId() == 582 ) )
            channelPlots_[detId].channel_ec_check->Fill( ( optorx.getLV1() & 0xFFF ), static_cast<int>( ( optorx.getLV1() & 0xFF )-dit->getEC() ) & 0xFF );
    ...
    for ( edm::DetSet<CTPPSDiamondDigi>::const_iterator dit = it->begin(); dit != it->end(); ++dit ) {
      if ( channelPlots_.find( detId ) != channelPlots_.end() ) {
        if ( dit->getLeadingEdge() != 0 ) {
          ...
          if (dit->getTrailingEdge() != 0) channelPlots_[detId].time_over_threshold_cumulative->Fill( HPTDC_bin_width * ( dit->getLeadingEdge() - dit->getTrailingEdge() ) );
          if (dit->getTrailingEdge() != 0) channelPlots_[detId].leading_trailing_correlation->Fill( dit->getTrailingEdge() * HPTDC_bin_width, dit->getLeadingEdge() * HPTDC_bin_width );
...
// 		channelPlots_[detId].time_over_threshold_corrections->Fill( ... );
// 	      }
// 	    }	    
        }
