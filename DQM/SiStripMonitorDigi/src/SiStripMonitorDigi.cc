...
void SiStripMonitorDigi::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup){
    ...
    for (std::vector< uint32_t >::const_iterator iterDets = layer_dets.begin() ;
	 iterDets != layer_dets.end() ; iterDets++) {
      ...
      for(unsigned int f=0; f<Fedconnections.size();f++){
        if(f>0&& Fedconnections.at(f)!=NULL && Fedconnections.at(f-1)!=NULL){
          int fed1 = Fedconnections[f]->fedId();
          int fed2 = Fedconnections[f-1]->fedId();
          /*if(fed1!=fed2){
            std::cout << "!WARNING!" << std::endl;
            std::cout << "============ DIFFERENT FEDID for Fedconnections from single detID !!!!!! ================"<< std::endl;
            std::cout << "SiStripMonitorDigi::analyze: FED connection @ " << f << " FEDID = " << Fedconnections[f]->fedId() <<  std::endl;
            std::cout << "SiStripMonitorDigi::analyze: FED connection @ " << f-1 << " FEDID = " << Fedconnections[f-1]->fedId() <<  std::endl;
            std::cout << "============================================================"<< std::endl;
          }*/
        }
      }
  ...
  if(good_fcc_index!=-999 && Fedconnections.at(good_fcc_index)!=NULL){
        ...
        std::cout << "SiStripMonitorCluster::analyze WARNING! no good connections for detid = " << detid << std::endl;
      }
