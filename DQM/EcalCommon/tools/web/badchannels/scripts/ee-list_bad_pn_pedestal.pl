...
die "Usage:  list_bad_pn_pedestal.pl site run status\n" unless ($#ARGV >= 2);
...
my $sql = qq[ SELECT riov.run_num run,
                     cv.id1 sm,
                     cv.id2 pn,
                     CAST(pnp.ped_mean_g1 AS NUMBER(6,1)) mean01,
                     CAST(pnp.ped_rms_g1 AS NUMBER(6,1)) rms01,
                     CAST(pnp.ped_mean_g16 AS NUMBER(6,1)) mean16,
                     CAST(pnp.ped_rms_g16 AS NUMBER(6,1)) rms16,
                     pnp.task_status status
                FROM run_iov riov
                ...
                JOIN mon_pn_ped_dat pnp ON pnp.iov_id = miov.iov_id
                LEFT OUTER JOIN channelview cv ON cv.logic_id = pnp.logic_id AND cv.name = cv.maps_to
               WHERE ldef.location = ?
                 AND riov.run_num = ?
                 AND pnp.task_status = ?
            ORDER BY run, sm, pn];
