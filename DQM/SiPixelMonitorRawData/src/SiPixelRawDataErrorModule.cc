...
const uint32_t SiPixelRawDataErrorModule::LINK_mask =
    ~(~uint32_t(0) << LINK_bits);
const uint32_t SiPixelRawDataErrorModule::ROC_mask =
    ~(~uint32_t(0) << ROC_bits);
const uint32_t SiPixelRawDataErrorModule::DCOL_mask =
    ~(~uint32_t(0) << DCOL_bits);
const uint32_t SiPixelRawDataErrorModule::PXID_mask =
    ~(~uint32_t(0) << PXID_bits);
const uint32_t SiPixelRawDataErrorModule::ADC_mask =
    ~(~uint32_t(0) << ADC_bits);
const uint32_t SiPixelRawDataErrorModule::DataBit_mask =
    ~(~uint32_t(0) << DataBit_bits);
...
const int SiPixelRawDataErrorModule::EVTLGT_shift =
    TRLRBGN_shift + TRLRBGN_bits;
const int SiPixelRawDataErrorModule::TRLREND_shift = EVTLGT_shift + EVTLGT_bits;

const unsigned long long SiPixelRawDataErrorModule::TRLREND_mask =
    ~(~0ULL << TRLREND_bits);
const unsigned long long SiPixelRawDataErrorModule::EVTLGT_mask =
    ~(~0ULL << EVTLGT_bits);
const unsigned long long SiPixelRawDataErrorModule::TRLRBGN_mask =
    ~(~0ULL << TRLRBGN_bits);
//
// Constructors
//
...
// Fill histograms
//
int SiPixelRawDataErrorModule::fill(
    const edm::DetSetVector<SiPixelRawDataError> &input,
    std::map<std::string, MonitorElement **> *meMapFEDs, bool modon, bool ladon,
    bool bladeon) {
  bool barrel =
  ...
  edm::DetSetVector<SiPixelRawDataError>::const_iterator isearch =
      input.find(id_); // search  errors of detid
    ...
    for (di = isearch->data.begin(); di != isearch->data.end(); di++) {
      ...
      if (modon) {
        if (errorType == 32 || errorType == 33 || errorType == 34) {
          ...
          chanNmbr = (errorWord >> LINK_shift) &
                     LINK_mask; // default way to get channel number.  Only
      ...
      if (!(errorType == 30) || notReset) {
        ...
        if (errorType < 30)
          ...
          messageType = errorType - 24;
                 ...
                 (TBMMessage == 2 || TBMMessage == 3 || TBMMessage == 4))
...
int SiPixelRawDataErrorModule::fillFED(
    const edm::DetSetVector<SiPixelRawDataError> &input,
    std::map<std::string, MonitorElement **> *meMapFEDs) {
  unsigned int numberOfSeriousErrors = 0;

  edm::DetSetVector<SiPixelRawDataError>::const_iterator isearch =
      input.find(0xffffffff);   // search  errors of detid
      ...
      if (FedId == static_cast<int>(id_)) {
        ...
        if (!(errorType == 30) || notReset) {
          ...
          if ((*meMapFEDs)["meFedChLErr_"][id_])
            (*meMapFEDs)["meFedChLErr_"][id_]->setBinContent(chanNmbr + 1,
                                                             errorType);
