...
// $Id: HcalDetDiagLaserMonitor.cc,v 1.7.4.6 2010/03/25 10:43:21 temple Exp $
//
//
...
public: 
   ...
   float  ref_time_rms;
   int    status;
   int    nChecks,nBadTime,nBadEnergy;
};
...
static int  lastHBHEorbit,lastHOorbit,lastHForbit,nChecksHBHE,nChecksHO,nChecksHF;
   if(ievt_==0){ HBHEseq=HOseq=HFseq=false; lastHBHEorbit=lastHOorbit=lastHForbit=-1;nChecksHBHE=nChecksHO=nChecksHF=0; }
   ...
   if(!LocalRun && Online_){
      if(HBHEseq && (orbit-lastHBHEorbit)>(11223*10)){
         HBHEseq=false;
      ...
      if(HOseq && (orbit-lastHOorbit)>(11223*10)){
         HOseq=false;
      ...
      if(HFseq && (orbit-lastHForbit)>(11223*10)){
         HFseq=false;
   ...
   if(LocalRun==false || LaserEvent==false){
	  ...
	  if(value==hc_HBHEHPD){ HBHEseq=true; HOseq=HFseq=false; lastHBHEorbit=orbit; }
	  if(value==hc_HOHPD){   HOseq=true; HBHEseq=HFseq=false; lastHOorbit=orbit;   }
	  if(value==hc_HFPMT){   HFseq=true; HBHEseq=HOseq=false; lastHForbit=orbit;   }
...
char   Subdet[10],str[500];
       ...
       for(int eta=-16;eta<=16;eta++) for(int phi=1;phi<=72;phi++) for(int depth=1;depth<=2;depth++){
	     ...
	     hb_data[eta+42][phi-1][depth-1].reset1();
	     tree->Fill();
          }
       } 
       sprintf(Subdet,"HE");
       for(int eta=-29;eta<=29;eta++) for(int phi=1;phi<=72;phi++) for(int depth=1;depth<=3;depth++){
	    ...
	    he_data[eta+42][phi-1][depth-1].reset1();
	    tree->Fill();
         }
       } 
       sprintf(Subdet,"HO");
       for(int eta=-15;eta<=15;eta++) for(int phi=1;phi<=72;phi++) for(int depth=4;depth<=4;depth++){
	     ...
	     ho_data[eta+42][phi-1][depth-1].reset1();
	     tree->Fill();
         }
       } 
       sprintf(Subdet,"HF");
       for(int eta=-42;eta<=42;eta++) for(int phi=1;phi<=72;phi++) for(int depth=1;depth<=2;depth++){
	     ...
	     hf_data[eta+42][phi-1][depth-1].get_average_time1(&time,&time_rms);
             hf_data[eta+42][phi-1][depth-1].reset1();
	     tree->Fill();
         }
       }
