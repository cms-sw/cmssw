...
void HcalLaserMonitor::setup( const edm::ParameterSet& iConfig, DQMStore* dbe ) {
  ...
  if (fVerbosity) printf( "====================================================\n" );

  doPerChannel_ = iConfig.getUntrackedParameter<bool>( "LaserPerChannel", false );
  AllowedCalibTypes_ = iConfig.getUntrackedParameter<vector<int> >("LaserMonitor_AllowedCalibTypes",AllowedCalibTypes_);

  if (fVerbosity) printf( "Laser Monitor per channel set to %s\n", doPerChannel_ ? "true" : "false" );

  etaMax_ = iConfig.getUntrackedParameter<double>( "MaxEta",  41.5 );
  etaMin_ = iConfig.getUntrackedParameter<double>( "MinEta", -41.5 );
  etaBins_ = (int)(etaMax_ - etaMin_);
  if (fVerbosity) std::cout << "Laser Monitor eta min/max set to " << etaMin_ << "/" << etaMax_ << std::endl;

  phiMax_ = iConfig.getUntrackedParameter<double>( "MaxPhi", 73 );
  phiMin_ = iConfig.getUntrackedParameter<double>( "MinPhi",  0 );
  phiBins_ = (int)(phiMax_ - phiMin_);
  if (fVerbosity) std::cout << "Laser Monitor phi min/max set to " << phiMin_ << "/" << phiMax_ << std::endl;

  sigS0_ = iConfig.getUntrackedParameter<int>( "FirstSignalBin", 0 );
  sigS1_ = iConfig.getUntrackedParameter<int>( "LastSignalBin",  9 );
  if( sigS0_ < 0 ) {
    if (fVerbosity) printf( "HcalLaserMonitor::setup, illegal range for first sample: %d\n", sigS0_ );
    sigS0_ = 0;
  }
  if( sigS1_ > 9 ) {
    if (fVerbosity) printf( "HcalLaserMonitor::setup, illegal range for last sample: %d\n", sigS1_ );
    sigS1_ = 9;
  }
  if( sigS0_ > sigS1_ ) {
    if (fVerbosity) printf( "HcalLaserMonitor::setup, illegal range for first/last sample: %d/%d\n", sigS0_, sigS1_ );
    sigS0_ = 0; sigS1_ = 9;
  }
  if (fVerbosity) std::cout << "Laser Monitor signal window set to " << sigS0_ << "-" << sigS1_ << std::endl;

  adcThresh_ = iConfig.getUntrackedParameter<double>( "Laser_ADC_Thresh", -10000 );
  if (fVerbosity) std::cout << "Laser Monitor threshold set to " << adcThresh_ << endl;

  if (fVerbosity) printf( "====================================================\n" );
  return;
} // void HcalLaserMonitor::setup(...)

void HcalLaserMonitor::beginRun()
{
				     ...
				     int CalibType) 
{
  ...
  if (fVerbosity>1) std::cout <<"<HcalLaserMonitor::processEvent>  calibType = "<<CalibType<<"  processing event? "<<processevent<<endl;
  if (!processevent)
    return;
...
inline void HcalLaserMonitor::perChanHists( const int id, const HcalDetId detid, const float* pedSubTS, map<HcalDetId, MonitorElement*>& tShape, map<HcalDetId, MonitorElement*>& tTime,
  ...
  MonitorElement* _me;
  if( m_dbe == NULL ) {
    if (fVerbosity) std::cout<< "HcalLaserMonitor::perChanHists - Null MonitorElement!\n"<<std::endl;
    return;
  }
  ...
  if     ( id == HcalBarrel  ) type = "HB";
    ...
    if (fVerbosity) std::cout<< "HcalLaserMonitor::perChanHists - ID not understood!\n"<<std::endl;
    return;
  }
  ...
  if( meIter_ != tShape.end() ) {
    _me = meIter_->second;
    if( _me == NULL ) {
      if (fVerbosity) std::cout<< "HcalLaserAnalysis::perChanHists - This histo is NULL!!??"<<std::endl;
      return;
    }
    else {
      float en = 0, numerator = 0, denominator = 0, maxADC = 0;
