...
void FineDelayHistosUsingDb::computeDelays() {
  ...
  for (unsigned short id : ids) {
    ...
    for (const auto & conn : conns) {
      ...
      if(DetId(conn.detId()).det()!=DetId::Tracker) continue;
      ...
      if(tracker_) {
        ...
        delays_[SiStripFecKey( conn.fecCrate(),
                               conn.fecSlot(),
                               conn.fecRing(),
                               conn.ccuAddr(),
                               conn.ccuChan(), 0 ).key()] = delay;
...
void FineDelayHistosUsingDb::update( SiStripConfigDb::FedDescriptionsRange feds ) {
  ...
  for (auto fed : feds) {
    ...
    if ( find( ids.begin(), ids.end(), fed->getFedId() ) == ids.end() ) { continue; }
    ...
    for (const auto & conn : conns) {
      ...
      // extract the delay from the map
      int delay = int(round( delays_[SiStripFecKey( conn.fecCrate(),
                                                    conn.fecSlot(),
                                                    conn.fecRing(),
                                                    conn.ccuAddr(),
                                                    conn.ccuChan(), 0 ).key()]));
      ...
      ss << "[FineDelayHistosUsingDb::" << __func__ << "]"
         ...
         << " for loop FED id/ch "
         << fed->getFedId() << "/" << conn.fedCh()
         << " from "
         << fed->getCoarseDelay( fedChannel) << "/" << fed->getFineDelay( fedChannel)
         << " to ";
