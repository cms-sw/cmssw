...
void NoiseHistosUsingDb::uploadConfigurations() {
  ...
  if (!db()) {
    edm::LogError(mlDqmClient_) << "[NoiseHistosUsingDb::" << __func__ << "]"
  ...
  update(feds);
  if (doUploadConf()) {
    edm::LogVerbatim(mlDqmClient_) << "[NoiseHistosUsingDb::" << __func__ << "]"
                                   << " Uploading pedestals/noise to DB...";
    db()->uploadFedDescriptions();
    edm::LogVerbatim(mlDqmClient_) << "[NoiseHistosUsingDb::" << __func__ << "]"
                                   << " Completed database upload of " << feds.size() << " FED descriptions!";
  } else {
    edm::LogWarning(mlDqmClient_) << "[NoiseHistosUsingDb::" << __func__ << "]"
...
// -----------------------------------------------------------------------------
/** */
void NoiseHistosUsingDb::create(SiStripConfigDb::AnalysisDescriptionsV& desc, Analysis analysis) {
