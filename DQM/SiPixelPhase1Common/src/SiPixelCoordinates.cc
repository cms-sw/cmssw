...
void SiPixelCoordinates::init(edm::EventSetup const& iSetup) {
  ...
  if (phase_==-1) {
    if (tGeom_ -> isThere(GeomDetEnumerators::PixelBarrel) && 
	tGeom_ -> isThere(GeomDetEnumerators::PixelEndcap) )
      phase_ = 0;
    else if (tGeom_ -> isThere(GeomDetEnumerators::P1PXB) && 
	     tGeom_ -> isThere(GeomDetEnumerators::P1PXEC) )
      phase_ = 1;
    else if (tGeom_ -> isThere(GeomDetEnumerators::P1PXB) && 
	     tGeom_ -> isThere(GeomDetEnumerators::P1PXEC) )
      phase_ = 2;
...
int SiPixelCoordinates::quadrant(const DetId& detid) {
  if (quadrant_.count(detid.rawId())) return quadrant_[detid.rawId()];
  if (!isPixel_(detid)) return quadrant_[detid.rawId()] = -9999;
...
int SiPixelCoordinates::side(const DetId& detid) {
  if (side_.count(detid.rawId())) return side_[detid.rawId()];
  if (!isPixel_(detid)) return side_[detid.rawId()] = -9999;
  if(detid.subdetId() == PixelSubdetector::PixelBarrel)
...
int SiPixelCoordinates::module(const DetId& detid) {
  if (module_.count(detid.rawId())) return module_[detid.rawId()];
  if (!isPixel_(detid)) return module_[detid.rawId()] = -9999;
  if(detid.subdetId() == PixelSubdetector::PixelBarrel)
...
int SiPixelCoordinates::layer(const DetId& detid) {
  if (layer_.count(detid.rawId())) return layer_[detid.rawId()];
  if (!isBPix_(detid)) return layer_[detid.rawId()] = -9999;
  return layer_[detid.rawId()] = tTopo_->pxbLayer(detid);
}

// Taken from pixel naming class for barrel
int SiPixelCoordinates::sector(const DetId& detid) {
  if (sector_.count(detid.rawId())) return sector_[detid.rawId()];
  if (!isBPix_(detid)) return sector_[detid.rawId()] = -9999;
...
int SiPixelCoordinates::ladder(const DetId& detid) {
  if (ladder_.count(detid.rawId())) return ladder_[detid.rawId()];
  if (!isBPix_(detid)) return ladder_[detid.rawId()] = -9999;
...
int SiPixelCoordinates::signed_ladder(const DetId& detid) {
  if (signed_ladder_.count(detid.rawId())) return signed_ladder_[detid.rawId()];
  if (!isBPix_(detid)) return signed_ladder_[detid.rawId()] = -9999;
  int signed_ladder = PixelBarrelName(detid, tTopo_, phase_).ladderName();
  if (quadrant(detid)%2) signed_ladder *= -1;
...
int SiPixelCoordinates::signed_module(const DetId& detid) {
  if (signed_module_.count(detid.rawId())) return signed_module_[detid.rawId()];
  if (!isBPix_(detid)) return signed_module_[detid.rawId()] = -9999;
  int signed_module = PixelBarrelName(detid, tTopo_, phase_).moduleName();
  if (quadrant(detid)<3) signed_module *= -1;
...
int SiPixelCoordinates::half(const DetId& detid) {
  if (half_.count(detid.rawId())) return half_[detid.rawId()];
  if (!isBPix_(detid)) return half_[detid.rawId()] = -9999;
...
int SiPixelCoordinates::outer(const DetId& detid) {
  if (outer_.count(detid.rawId())) return outer_[detid.rawId()];
  if (!isBPix_(detid)) return outer_[detid.rawId()] = -9999;
  int outer = -9999;
  int layer = tTopo_->pxbLayer(detid.rawId());
  bool odd_ladder = tTopo_->pxbLadder(detid.rawId())%2;
  if (phase_ == 0) {
    if (layer==2) outer = !odd_ladder;
    else outer = odd_ladder;
  } else if (phase_ == 1) {
    if (layer==4) outer = odd_ladder;
    else outer = !odd_ladder;
  }
...
int SiPixelCoordinates::flipped(const DetId& detid) {
  if (flipped_.count(detid.rawId())) return flipped_[detid.rawId()];
  if (!isBPix_(detid)) return flipped_[detid.rawId()] = -9999;
  int flipped = -9999;
  if (phase_ < 2) flipped = outer(detid)==0;
...
int SiPixelCoordinates::disk(const DetId& detid) {
  if (disk_.count(detid.rawId())) return disk_[detid.rawId()];
  if (!isFPix_(detid)) return disk_[detid.rawId()] = -9999;
...
int SiPixelCoordinates::signed_disk(const DetId& detid) {
  if (signed_disk_.count(detid.rawId())) return signed_disk_[detid.rawId()];
  if (!isFPix_(detid)) return signed_disk_[detid.rawId()] = -9999;
  int signed_disk = disk(detid);
  if (quadrant(detid)<3) signed_disk *= -1;
  return signed_disk_[detid.rawId()] = signed_disk;
}

// Taken from TrackerTopology
int SiPixelCoordinates::panel(const DetId& detid) {
  if (panel_.count(detid.rawId())) return panel_[detid.rawId()];
  if (!isFPix_(detid)) return panel_[detid.rawId()] = -9999;
...
int SiPixelCoordinates::ring(const DetId& detid) {
  if (ring_.count(detid.rawId())) return ring_[detid.rawId()];
  if (!isFPix_(detid)) return ring_[detid.rawId()] = -9999;
  int ring = -9999;
  if (phase_==0) {
...
int SiPixelCoordinates::blade(const DetId& detid) {
  if (blade_.count(detid.rawId())) return blade_[detid.rawId()];
  if (!isFPix_(detid)) return blade_[detid.rawId()] = -9999;
...
int SiPixelCoordinates::signed_blade(const DetId& detid) {
  if (signed_blade_.count(detid.rawId())) return signed_blade_[detid.rawId()];
  if (!isFPix_(detid)) return signed_blade_[detid.rawId()] = -9999;
  int signed_blade = PixelEndcapName(detid, tTopo_, phase_).bladeName();
  if (quadrant(detid)%2) signed_blade *= -1;
...
unsigned int SiPixelCoordinates::fedid(const DetId& detid) {
  if (fedid_.count(detid.rawId())) return fedid_[detid.rawId()];
  if (!isPixel_(detid)) return fedid_[detid.rawId()] = 9999;
...
int SiPixelCoordinates::channel(const DetId& detid, const std::pair<int, int>& pixel) {
  ...
  uint64_t pseudo_roc_num = uint64_t(1<<16) * detid.rawId() + (1<<8) * (pixel.first/rowsperroc) + pixel.second/colsperroc;
  if (channel_.count(pseudo_roc_num)) return channel_[pseudo_roc_num];
...
int SiPixelCoordinates::channel(const SiPixelRecHit* rechit) {
  if (!isPixel_(rechit->geographicalId())) return -9999;
...
int SiPixelCoordinates::roc(const DetId& detid, const std::pair<int, int>& pixel) {
  ...
  // If a ROC number was ever found, then binary search in a map will be much quicker
  uint64_t pseudo_roc_num = uint64_t(1<<16) * detid.rawId() + (1<<8) * (pixel.first/rowsperroc) + pixel.second/colsperroc;
  if (roc_.count(pseudo_roc_num)) return roc_[pseudo_roc_num];
  ...
  if (phase_==2||channel_.size()) {
    ...
    if (phase_==0&&detid.subdetId() == PixelSubdetector::PixelEndcap) rocsY = pan+mod;
...
float SiPixelCoordinates::module_coord(const SiPixelRecHit* rechit) {
  if (!isBPix_(rechit->geographicalId())) return -9999;
...
float SiPixelCoordinates::ring_coord(const DetId& detid, const std::pair<int, int>& pixel) {
  ...
  if (phase_==0) {
    ...
    if (pan==1) {
      if      (mod==1) coord_shift = (-xcoord_on_module_(detid, pixel)      )/4;
      else if (mod==2) coord_shift = (-xcoord_on_module_(detid, pixel) + 2.0)/4;
      else if (mod==3) coord_shift = (-xcoord_on_module_(detid, pixel)      )/4;
      else if (mod==4) coord_shift = (-xcoord_on_module_(detid, pixel) + 1.5)/4;
    } else {
      if      (mod==1) coord_shift = ( xcoord_on_module_(detid, pixel)      )/4;
      else if (mod==2) coord_shift = ( xcoord_on_module_(detid, pixel) - 2.0)/4;
      else if (mod==3) coord_shift = ( xcoord_on_module_(detid, pixel)      )/4;
...
float SiPixelCoordinates::ring_coord(const SiPixelRecHit* rechit) {
  if (!isFPix_(rechit->geographicalId())) return -9999;
...
float SiPixelCoordinates::signed_disk_coord(const DetId& detid, const std::pair<int, int>& pixel) {
  ...
  if (signed_disk_coord<0) coord_shift = -coord_shift;
...
float SiPixelCoordinates::signed_disk_ring_coord(const DetId& detid, const std::pair<int, int>& pixel) {
  ...
  coord_shift = (ring_coord(detid,pixel) - 1.5  ) / 2.0;
  // Mirror -z side, so plots are symmetric
  if (signed_disk_ring_coord<0) coord_shift = -coord_shift;
...
float SiPixelCoordinates::blade_coord(const DetId& detid, const std::pair<int, int>& pixel) {
  ...
  if (phase_==0) {
    ...
    if (side(detid)==1) coord_shift = -coord_shift;
  } else if (phase_==1) {
    coord_shift = xcoord_on_module_(detid, pixel) - 0.5;
    if ((side(detid)+panel(detid))%2==0) coord_shift = -coord_shift;
  }
...
float SiPixelCoordinates::signed_blade_coord(const DetId& detid, const std::pair<int, int>& pixel) {
  ...
  if (phase_==0) {
    int rocsY = panel(detid)+module(detid);
    coord_shift = ycoord_on_module_(detid, pixel) - rocsY/10.;
    if (side(detid)==2) coord_shift = -coord_shift;
  } else if (phase_==1) {
    coord_shift = xcoord_on_module_(detid, pixel) - 0.5;
    if ((side(detid)+panel(detid))%2==1) coord_shift = -coord_shift;
  }
...
float SiPixelCoordinates::signed_shifted_blade_panel_coord(const DetId& detid, const std::pair<int, int>& pixel) {
  ...
  if (phase_==1&&ring(detid)==1) signed_shifted_blade_panel_coord *= 1.5;
