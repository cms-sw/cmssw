...
void
EEPedestalOnlineTask::endLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
  if(init_ && dqmStore_ && !dqmStore_->dirExists(prefixME_ + "/EEPedestalOnlineTask" + (subfolder_.size() == 0 ? "" : "/" + subfolder_))){
	cleanup();
	setup();
  }
}
