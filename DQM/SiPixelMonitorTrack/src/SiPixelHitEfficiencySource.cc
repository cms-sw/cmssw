...
// pSet.getUntrackedParameter<bool>("updateEfficiencies",false) )
{
  ...
  vtxsrc_ = pSet_.getUntrackedParameter<std::string>("vtxsrc",
                                                     "offlinePrimaryVertices");
  vertexCollectionToken_ = consumes<reco::VertexCollection>(vtxsrc_);
  tracksrc_ = consumes<TrajTrackAssociationCollection>(
      pSet_.getParameter<edm::InputTag>("trajectoryInput"));
  clusterCollectionToken_ = consumes<edmNew::DetSetVector<SiPixelCluster>>(
      std::string("siPixelClusters"));

  measurementTrackerEventToken_ =
      consumes<MeasurementTrackerEvent>(std::string("MeasurementTrackerEvent"));
...
SiPixelHitEfficiencySource::~SiPixelHitEfficiencySource() {
  ...
  for (struct_iter = theSiPixelStructure.begin();
       struct_iter != theSiPixelStructure.end(); struct_iter++) {
    delete struct_iter->second;
    struct_iter->second = nullptr;
  }
}

void SiPixelHitEfficiencySource::fillClusterProbability(int layer, int disk,
                                                        bool plus,
                                                        double probability) {
...
void SiPixelHitEfficiencySource::dqmBeginRun(const edm::Run &r,
  ...
  if (debug_)
    LogVerbatim("PixelDQM") << "TrackerGeometry " << &(*TG) << " size is "
                            << TG->dets().size() << endl;

  // build theSiPixelStructure with the pixel barrel and endcap dets from
  // TrackerGeometry
  for (TrackerGeometry::DetContainer::const_iterator pxb =
           TG->detsPXB().begin();
    ...
    if (dynamic_cast<PixelGeomDetUnit const *>((*pxb)) != nullptr) {
      ...
      theSiPixelStructure.insert(pair<uint32_t, SiPixelHitEfficiencyModule *>(
          (*pxb)->geographicalId().rawId(), module));
    }
  }
  for (TrackerGeometry::DetContainer::const_iterator pxf =
           TG->detsPXF().begin();
    ...
    if (dynamic_cast<PixelGeomDetUnit const *>((*pxf)) != nullptr) {
      ...
      theSiPixelStructure.insert(pair<uint32_t, SiPixelHitEfficiencyModule *>(
          (*pxf)->geographicalId().rawId(), module));
    }
  }
  LogInfo("PixelDQM") << "SiPixelStructure size is "
                      << theSiPixelStructure.size() << endl;
}
...
void SiPixelHitEfficiencySource::bookHistograms(DQMStore::IBooker &iBooker,
  ...
  for (std::map<uint32_t, SiPixelHitEfficiencyModule *>::iterator pxd =
  ...
  meClusterProbabilityL1_Plus_ =
      iBooker.book1D("ClusterProbabilityXY_Layer1_Plus",
  ...
  meClusterProbabilityL1_Minus_ =
      iBooker.book1D("ClusterProbabilityXY_Layer1_Minus",
  ...
  meClusterProbabilityL2_Plus_ =
      iBooker.book1D("ClusterProbabilityXY_Layer2_Plus",
  ...
  meClusterProbabilityL2_Minus_ =
      iBooker.book1D("ClusterProbabilityXY_Layer2_Minus",
  ...
  meClusterProbabilityL3_Plus_ =
      iBooker.book1D("ClusterProbabilityXY_Layer3_Plus",
  ...
  meClusterProbabilityL3_Minus_ =
      iBooker.book1D("ClusterProbabilityXY_Layer3_Minus",
  ...
  meClusterProbabilityD1_Plus_ =
      iBooker.book1D("ClusterProbabilityXY_Disk1_Plus",
  ...
  meClusterProbabilityD1_Minus_ =
      iBooker.book1D("ClusterProbabilityXY_Disk1_Minus",
  ...
  meClusterProbabilityD2_Plus_ =
      iBooker.book1D("ClusterProbabilityXY_Disk2_Plus",
  ...
  meClusterProbabilityD2_Minus_ =
      iBooker.book1D("ClusterProbabilityXY_Disk2_Minus",
...
void SiPixelHitEfficiencySource::analyze(const edm::Event &iEvent,
  ...
  for (reco::VertexCollection::const_iterator it = vertices.begin();
    ...
    if (fabs(it->z()) <= 20. && fabs(it->position().rho()) <= 2. &&
        it->ndof() > 4)
      nvtx_++;
  }
  if (nvtx_ < 1)
  ...
  iEvent.getByToken(measurementTrackerEventToken_,
                    measurementTrackerEventHandle);
  ...
  for (TrajTrackAssociationCollection::const_iterator it = ttac.begin();
       it != ttac.end(); ++it) {
    ...
    int D2hits = 0;
    int D3hits = 0;
    std::vector<TrajectoryMeasurement> tmeasColl =
        traj_iterator->measurements();
    ...
    for (tmeasIt = tmeasColl.begin(); tmeasIt != tmeasColl.end(); tmeasIt++) {
      ...
      if ((testSubDetID == PixelSubdetector::PixelBarrel &&
        ...
        if (testhit->isValid()) {
          if (tmeasIt == tmeasColl.end() - 1) {
            ...
            tmeasIt++;
            TransientTrackingRecHit::ConstRecHitPointer nextRecHit =
                tmeasIt->recHit();
            uint nextSubDetID = (nextRecHit->geographicalId().subdetId());
            int nextlayer =
                PixelBarrelName(nextRecHit->geographicalId()).layerName();
            if (nextSubDetID == PixelSubdetector::PixelBarrel &&
                nextlayer == extrapolateTo_) {
      ...
      if (lastValidL2) {
        std::vector<const BarrelDetLayer *> pxbLayers =
            measurementTrackerHandle->geometricSearchTracker()
                ->pixelBarrelLayers();
        const DetLayer *pxb1 = pxbLayers[extrapolateTo_ - 1];
        const MeasurementEstimator *estimator = est.product();
        const LayerMeasurements *theLayerMeasurements = new LayerMeasurements(
            *measurementTrackerHandle, *measurementTrackerEventHandle);
        const TrajectoryStateOnSurface tsosPXB2 = tmeasIt->updatedState();
        expTrajMeasurements = theLayerMeasurements->measurements(
            *pxb1, tsosPXB2, *thePropagator, *estimator);
      ...
      TrajectoryStateOnSurface chkPredTrajState = trajStateComb(
          tmeasIt->forwardPredictedState(), tmeasIt->backwardPredictedState());
      ...
      if (testSubDetID == PixelSubdetector::PixelBarrel &&
        ...
        for (size_t iexp = 0; iexp < expTrajMeasurements.size(); iexp++) {
          const DetId &exphit_detId =
              expTrajMeasurements[iexp].recHit()->geographicalId();
          ...
          TrajectoryStateOnSurface predTrajState =
              expTrajMeasurements[iexp].updatedState();
          float x = predTrajState.globalPosition().x();
          float y = predTrajState.globalPosition().y();
          float z = predTrajState.globalPosition().z();
          float dxyz = sqrt((chkx - x) * (chkx - x) + (chky - y) * (chky - y) +
                            (chkz - z) * (chkz - z));
        ...
        if (!expTrajMeasurements.empty()) {
          if (glmatch <
              9999.) { // if there is any propagated trajectory for this hit
            const DetId &matchhit_detId =
                expTrajMeasurements[imatch].recHit()->geographicalId();
            ...
            if (dladder > 10)
              dladder = 20 - dladder;
            LocalPoint lp =
                expTrajMeasurements[imatch].updatedState().localPosition();
    ...
    if (isBpixtrack || isFpixtrack) {
      if (trackref->pt() < 0.6 || nStripHits < 8 ||
      ...
      for (std::vector<TrajectoryMeasurement>::const_iterator tmeasIt =
               tmeasColl.begin();
           tmeasIt != tmeasColl.end(); tmeasIt++) {
        ...
        if (hit->geographicalId().det() != DetId::Tracker)
          ...
          const SiPixelRecHit *pixhit =
              dynamic_cast<const SiPixelRecHit *>(hit->hit());

          if (IntSubDetID == PixelSubdetector::PixelBarrel) { // it's a BPIX hit
            layer = PixelBarrelName(hit_detId, pTT, isUpgrade).layerName();
            isHalfModule =
                PixelBarrelName(hit_detId, pTT, isUpgrade).isHalfModule();

            if (hit->isValid()) { // fill the cluster probability in barrel
              bool plus = true;
              if ((PixelBarrelName(hit_detId, pTT, isUpgrade).shell() ==
                   PixelBarrelName::Shell::mO) ||
                  (PixelBarrelName(hit_detId, pTT, isUpgrade).shell() ==
                   PixelBarrelName::Shell::mI))
                plus = false;
              double clusterProbability = pixhit->clusterProbability(0);
              if (clusterProbability != 0)
                fillClusterProbability(layer, 0, plus,
                                       log10(clusterProbability));
            }

          } else if (IntSubDetID ==
                     PixelSubdetector::PixelEndcap) { // it's an FPIX hit
            ...
            if (hit->isValid()) {
              bool plus = true;
              if ((PixelEndcapName(hit_detId, pTT, isUpgrade).halfCylinder() ==
                   PixelEndcapName::HalfCylinder::mO) ||
                  (PixelEndcapName(hit_detId, pTT, isUpgrade).halfCylinder() ==
                   PixelEndcapName::HalfCylinder::mI))
                plus = false;
              double clusterProbability = pixhit->clusterProbability(0);
              if (clusterProbability != 0)
                fillClusterProbability(0, disk, plus,
                                       log10(clusterProbability));
            }
          }
          ...
          if (layer == 1) {
            if (fabs(trackref->dxy(bestVtx->position())) > 0.01 ||
                fabs(trackref->dz(bestVtx->position())) > 0.1)
              continue;
            if (!(L2hits > 0 && L3hits > 0) && !(L2hits > 0 && D1hits > 0) &&
                !(D1hits > 0 && D2hits > 0))
              continue;
          } else if (layer == 2) {
              ...
              !((panel == 1 &&
                 ((module == 1 && fabs(ly) < 0.7) ||
                  ...
                  ((module == 4 && fabs(ly) < 1.9) &&
                   !(disk == -2 && ly > 1.5) && !(disk == 2 && ly < -1.5)))) ||
                (panel == 2 &&
                 ((module == 1 && fabs(ly) < 0.7) ||
                  (module == 2 && fabs(ly) < 1.2 && !(disk > 0 && ly > 1.1) &&
                   !(disk < 0 && ly < -1.1)) ||
                  (module == 3 && fabs(ly) < 1.6 && !(disk > 0 && ly > 1.5) &&
                   !(disk < 0 && ly < -1.5))))))
            passedFiducial = false;
          if (IntSubDetID == PixelSubdetector::PixelEndcap &&
              ((panel == 1 &&
                (module == 1 || (module >= 3 && abs(disk) == 1))) ||
          ...
          if (cpEstimator.isValid()) {
            ...
            if (tracker.isValid()) {
              ...
              iEvent.getByToken(clusterCollectionToken_,
                                clusterCollectionHandle);
              if (clusterCollectionHandle.isValid()) {
                ...
                edmNew::DetSetVector<SiPixelCluster>::const_iterator
                    itClusterSet = clusterCollection.begin();
                float minD[2];
                minD[0] = minD[1] = 10000.;
                for (; itClusterSet != clusterCollection.end();
                     itClusterSet++) {
                  ...
                  const PixelGeomDetUnit *pixdet =
                      (const PixelGeomDetUnit *)tkgeom->idToDetUnit(detId);
                  edmNew::DetSet<SiPixelCluster>::const_iterator itCluster =
                      itClusterSet->begin();
                  for (; itCluster != itClusterSet->end(); ++itCluster) {
                    ...
                    PixelClusterParameterEstimator::ReturnType params =
                        cpe.getParameters(*itCluster, *pixdet);
                    lp = std::get<0>(params);
                    float D = sqrt((lp.x() - lx) * (lp.x() - lx) +
                                   (lp.y() - ly) * (lp.y() - ly));
                    if (D < minD[0]) {
                      minD[1] = minD[0];
                      dx_cl[1] = dx_cl[0];
          ...
          if (pxd != theSiPixelStructure.end() && passedFiducial &&
              (isHitValid || isHitMissing))
            (*pxd).second->fill(pTT, ltp, isHitValid, modOn, ladOn, layOn,
                                phiOn, bladeOn, diskOn, ringOn);
