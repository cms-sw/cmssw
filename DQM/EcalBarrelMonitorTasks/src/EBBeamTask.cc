...
void EBBeamTask::analyze(const Event& e, const EventSetup& c){
  ...
  try{
    ...
    try {
      ...
      LogError("EBBeamTask") << "EcalTBEventHeader not present in event TOO! Returning." << std::endl;
    }
  ...
  try {
    ...
    LogDebug("EBBeamTask") << "EcalUncalibRecHitsEB found in event " << ievt_ << "; hits collection size " << neh;
  } 
  catch ( std::exception& ex ) {
    LogError("EBBeamTask") << "EcalUncalibRecHitsEB not found in event! Returning." << std::endl;
  }
  ...
  try {
    ...
    recTDC = pTDC.product();
    LogDebug("EBBeamTask") << " TDC offset is: " << recTDC->offset() << endl;
  } catch ( std::exception& ex ) {
  ...
  LogDebug("EBBeamTask") << " TDC raw, Hodo raw, uncalRecH and DCCheader found." << std::endl;
  ...
  if (!recHodo)
   ...
   LogDebug("EBBeamTask") << " Hodo reco found." << std::endl;
