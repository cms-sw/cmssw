...
void EBTimingTask::setup(void){
  ...
  if ( dqmStore_ ) {
    ...
    meTimeSummary1D_ = dqmStore_->book1D(histo, histo, 250, 0., 10.);
    meTimeSummary1D_->setAxisTitle("jitter (clocks)", 1);

    sprintf(histo, "EBTMT timing map");
    meTimeSummaryMap_ = dqmStore_->bookProfile2D(histo, histo, 72, 0., 360., 34, -85, 85, 250, 0., 10., "s");
    ...
    meTimeSummaryMapProjEta_ = dqmStore_->bookProfile(histo, histo, 34, -85., 85., 250, 0., 10., "s");
    ...
    meTimeSummaryMapProjPhi_ = dqmStore_->bookProfile(histo, histo, 72, 0., 360., 250, 0., 10., "s");
