...
void EBBeamCaloTask::analyze(const Event& e, const EventSetup& c){
  ...
  if(tb_moving){
    TableMoving_->Fill(1);
    if( PreviousTableStatus_[0] == 0 &&  PreviousTableStatus_[1] == 1){reset_histos_moving=true;}
    ...
    TableMoving_->Fill(0);
    if( PreviousTableStatus_[0] == 1 &&  PreviousTableStatus_[1] == 0){reset_histos_stable=true;}
