...
def main():
    ...
    dirERROR="DQMData/Run " + runNum + "/PixelPhase1/Run summary/SiPixelQualityPCL/BadROC_FEDerror/"
    ...
    for lyr in range(1,5):
        histOccList.append(rootf.FindObjectAny(hdeadB+str(lyr)))
                        ...
                        if hist2.GetBinContent(binx,biny)!=0 and check:
                            ...
                            if len(binTBM)!=0:
                                ...
                                if hist2.GetBinContent(binx,biny)!=0:
                                    binTBM.append(binx)
                c.SaveAs('MergedFEDerror_BPix_Layer{0}_TBM.pdf'.format(lyr))
                os.system('gs -dBATCH -dNOPAUSE -sDEVICE=png16m -dUseCropBox -sOutputFile=MergedFEDerror_BPix_Layer{0}_TBM.png -r144 -q MergedFEDerror_BPix_Layer{0}_TBM.pdf'.format(lyr))
                os.system('rm -f MergedFEDerror_BPix_Layer{0}_TBM.pdf'.format(lyr))
        else :
            print("Some Error in get the histograms for FPIX")
    #FPIX
    print("----> Build maps for FPix")
    for rng in range(1,3):
        histOccList.append(rootf.FindObjectAny(hdeadF+str(rng)))
                    ...
                    for binx in range(1,hist2.GetNbinsX()+1):
                        ...
                        if hist2.GetBinContent(binx,biny)!=0 and check:
                            ...
                            if len(binTBM)!=0:
                                ...
                                if hist2.GetBinContent(binx,biny)!=0:
                                    binTBM.append(binx)
                c.SaveAs('MergedFEDerror_FPix_Ring{0}_TBM.pdf'.format(ring))
                os.system('gs -dBATCH -dNOPAUSE -sDEVICE=png16m -dUseCropBox -sOutputFile=MergedFEDerror_FPix_Ring{0}_TBM.png -r144 -q MergedFEDerror_FPix_Ring{0}_TBM.pdf'.format(ring))
                os.system('rm -f MergedFEDerror_FPix_Ring{0}_TBM.pdf'.format(ring))
