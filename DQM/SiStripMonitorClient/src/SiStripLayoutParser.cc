...
SiStripLayoutParser::~SiStripLayoutParser() {
  edm::LogInfo("SiStripActionExecutor") << " Deleting SiStripLayoutParser "
                                        << "\n";
}
//
...
bool SiStripLayoutParser::getAllLayouts(std::map<std::string, std::vector<std::string> >& layouts) {
  ...
  xercesc::DOMNodeList* layoutList = doc()->getElementsByTagName(qtxml::_toDOMS("layout"));
  ...
  for (unsigned int k = 0; k < layoutList->getLength(); k++) {
    ...
    xercesc::DOMNodeList* meList = layoutElement->getElementsByTagName(qtxml::_toDOMS("monitorable"));
