...
bool
SiStripSummaryCreator::readConfiguration(std::string const& file_path)
{
  summaryMEs_.clear();
...
{
  summaryMEs_.clear();
  for (std::map<std::string, std::string>::const_iterator isum =
         me_names.begin();
       isum != me_names.end();
       isum++) {
    summaryMEs_.insert(
      std::pair<std::string, std::string>(isum->first, isum->second));
  }
}
...
//
void
SiStripSummaryCreator::createSummary(DQMStore& dqm_store)
{
  if (summaryMEs_.empty()) return;
...
//
void
SiStripSummaryCreator::fillSummaryHistos(DQMStore& dqm_store)
{
  std::string currDir = dqm_store.pwd();
  ...
  for (std::map<std::string, std::string>::const_iterator isum =
    ...
    for (std::vector<std::string>::const_iterator it = subdirs.begin();
      ...
      dqm_store.cd(*it);
      ndet++;
      std::vector<MonitorElement*> contents =
        dqm_store.getContents(dqm_store.pwd());
      dqm_store.goUp();
      for (std::vector<MonitorElement *>::const_iterator im = contents.begin();
        ...
        std::map<std::string, MonitorElement*>::iterator iPos =
          MEMap.find(name);
        MonitorElement* me;
        // Get the Summary ME
        if (iPos == MEMap.end()) {
...
//
void
SiStripSummaryCreator::fillGrandSummaryHistos(DQMStore& dqm_store)
{
  std::map<std::string, MonitorElement*> MEMap;
  std::string currDir = dqm_store.pwd();
...
void
SiStripSummaryCreator::fillHistos(int const ival,
                                  int const istep,
      ...
      if (hist2 &&  name.find("NoisyStrips") != std::string::npos) {
        ...
        if (entries > 0.0) {
          ...
          for (int k = 1; k < nbins + 1; k++) {
            float noisy =
              me_src->getBinContent(k, 3) + me_src->getBinContent(k, 5);
            float dead =
              me_src->getBinContent(k, 2) + me_src->getBinContent(k, 4);
