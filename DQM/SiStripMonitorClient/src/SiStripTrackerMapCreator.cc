...
//
void
SiStripTrackerMapCreator::create(const edm::ParameterSet& tkmapPset,
                                 DQMStore& dqm_store,
...
void
SiStripTrackerMapCreator::createForOffline(const edm::ParameterSet& tkmapPset,
                                           DQMStore& dqm_store,
                                           std::string& map_type,
                                           const edm::EventSetup& eSetup)
{
  ...
  unsigned int runNumber_ =
    tkmapPset.getUntrackedParameter<unsigned int>("RunNumber", 1);
  std::stringstream ss;
  ss << runNumber_;
  sRunNumber = ss.str();
  std::cout << sRunNumber << "\n\n\n\n\n\n";
  ...
  if(tkmapPset.exists("numberTopModules"))
    numTopModules_ =
      tkmapPset.getUntrackedParameter<uint32_t>("numberTopModules");
  else
    numTopModules_ = 20;
...
//
void
SiStripTrackerMapCreator::setTkMapFromAlarm(DQMStore& dqm_store,
                                            const edm::EventSetup& eSetup)
{
...
void
SiStripTrackerMapCreator::setTkMapFromHistogram(DQMStore& dqm_store,
                                                ...
                                                edm::EventSetup const& eSetup)
{
  ...
  auto topNmodVec =
    topModules_ ? new std::vector<std::pair<float, uint32_t>>{} : nullptr;
  for (auto const& sd : subdet_folder) {
    ...
    for (auto const& layer : layerVec) {
      if (layer.find("BadModuleList") != std::string::npos) continue;
  ...
  dqm_store.cd();
  if (topNmodVec) printTopModules(*topNmodVec, eSetup);
}
  ...
  std::vector<std::pair<float, uint32_t>>* topNmodVec)
{
  ...
  for ( DetId det_id : tkDetMap_->getDetsForLayer(TkDetMap::getLayerNum(lname)) ) {
    ...
    nDet_++;
    const TkLayerMap::XYbin& xyval =
      tkDetMap_->getXY(det_id, cachedDetId_, cachedLayer_, cachedXYbin_);
    float fval = 0.0;
  ...
  std::ostringstream& comment)
{
  ...
  std::vector<const FedChannelConnection*> conns =
    detCabling_->getConnections(det_id);
  for (auto const conn : conns) {
    if (conn) {
      comment << std::setw(3) << conn->fedId() << "/" << std::setw(2)
              << conn->fedCh() << " ";
    }
    else {
...
SiStripTrackerMapCreator::createInfoFile(
  std::vector<std::string> const& map_names,
  TTree* tkinfo_tree,
  DQMStore& dqm_store,
...
SiStripTrackerMapCreator::printBadModuleList(
  std::map<unsigned int, std::string> const& badmodmap,
  const edm::EventSetup& eSetup)
{
  // Retrieve tracker topology from geometry
  edm::ESHandle<TrackerTopology> tTopoHandle;
  ...
  bool tibDone = false, tidSide1Done = false, tidSide2Done = false,
       tobDone = false, tecSide1Done = false, tecSide2Done = false;
  constexpr unsigned int tibFirst = 369120277 - 1,
                         tidSide1First = 402664197 - 1,
                         tidSide2First = 402672389 - 1,
                         tobFirst = 436228134 - 1,
                         tecSide1First = 470044965 - 1,
                         tecSide2First = 470307109 - 1;
  ...
  edm::LogVerbatim("BadModuleList")
    << "Run: " << sRunNumber << ", Number of bad modules in total:";
  edm::LogVerbatim("BadModuleList")
    << "--------------------------------------------------------------";
  ...
  const edm::EventSetup& eSetup)
{
  ...
  if (topNmodVec.size() > numTopModules_) topNmodVec.resize(numTopModules_);
  ...
  for (auto const& aPair : topNmodVec) {
    ...
    if (flag == 0)
      edm::LogVerbatim("TopModules")
        << subdetector << comment.str() << " value: " << aPair.first;
  }
  edm::LogVerbatim("TopModules")
    << "------------------------------------------------------";
}
...
//
void
SiStripTrackerMapCreator::setTkMapRange(std::string const& map_type)
{
  tkMapMin_ = 0.0;
  if (tkMapMax_ == 0.0) {
