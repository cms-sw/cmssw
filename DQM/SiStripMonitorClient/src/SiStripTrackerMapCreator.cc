...
uint16_t SiStripTrackerMapCreator::getDetectorFlagAndComment(DQMStore* const dqm_store,
  ...
  if (conns.empty())        { comment << std::string(indent_width*3, ' '); }
  else if (conns.size()==1) { comment << std::string(indent_width*2, ' '); }
  else if (conns.size()==2) { comment << std::string(indent_width, ' '); }
...
void SiStripTrackerMapCreator::printBadModuleList(std::map<unsigned int,std::string> const& badmodmap, const edm::EventSetup& eSetup)
{
  //Retrieve tracker topology from geometry
  edm::ESHandle<TrackerTopology> tTopoHandle;
  eSetup.get<TrackerTopologyRcd>().get(tTopoHandle);
