...
void SiStripTrackerMapCreator::createForOffline(const edm::ParameterSet & tkmapPset, 
  ...
  if (map_type == "ResidualsMean"){
    ...
    trackerMap_->save(true, -0.005,0.005, map_type+namesuffix+".svg");  
    trackerMap_->save(true, -0.005,0.005, map_type+namesuffix+".png",4500,2400);
    ResidualsRMS_ = true;
    ...
    trackerMap_->save(true, 0.0, 0.005, map_type+namesuffix+".svg");  
    trackerMap_->save(true, 0.0, 0.005, map_type+namesuffix+".png",4500,2400);
  } else {
    edm::LogInfo("TkMapToBeSaved") << "Ready to save TkMap " << map_type << namesuffix << " with range set to " << tkMapMin_ << " - " << tkMapMax_;
