...
//
void
SiStripQualityChecker::bookStatus(DQMStore& dqm_store)
{
  if (bookedStripStatus_) return;
...
void SiStripQualityChecker::printStatusReport() {
  ...
  for (std::map<std::string, SubDetMEs>::const_iterator it =
         SubDetMEsMap.begin();
       it != SubDetMEsMap.end();
       it++) {
...
//
void
SiStripQualityChecker::getModuleStatus(DQMStore& dqm_store,
  ...
  for (std::vector<MonitorElement*>::const_iterator it = layer_mes.begin();
    ...
    for (std::vector<DQMChannel>::iterator it = bad_channels_me.begin(); it != bad_channels_me.end(); it++){
      ...
      if (iPos != bad_modules.end()){
        ...
        std::string thisMEpath = currentdir.substr(0, currentdir.rfind("/")) +
...
//
void
SiStripQualityChecker::fillStatusHistogram(MonitorElement const* me,
                                           int const xbin,
                                           int const ybin,
                                           float const val)
{
...
//
void
SiStripQualityChecker::fillFaultyModuleStatus(DQMStore& dqm_store,
                                              const edm::EventSetup& eSetup)
{
...
//
void
SiStripQualityChecker::fillStatusAtLumi(DQMStore& dqm_store)
{
  if (!bookedStripStatus_) bookStatus(dqm_store);
  fillDummyStatus();
...
//
void
SiStripQualityChecker::fillDetectorStatusAtLumi(DQMStore& dqm_store)
{
  dqm_store.cd();
  std::string rdir = "ReadoutView";
  if (!SiStripUtility::goToDir(dqm_store, rdir)) return;
  std::string fullpath =
    dqm_store.pwd() + "/PerLumiSection/" + "lumiErrorFraction";
