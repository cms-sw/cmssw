...
public:
  ...
  static void split(std::string const& str,
                    std::vector<std::string>& tokens,
                    std::string const& delimiters = " ");
  static void getMEStatusColor(int status, int& rval, int& gval, int& bval);
  static void
  getDetectorStatusColor(int status, int& rval, int& gval, int& bval);
  ...
  static void getModuleFolderList(DQMStore& dqm_store,
  ...
  static bool goToDir(DQMStore::IBooker& ibooker,
                      DQMStore::IGetter& igetter,
                      std::string const& name);
  static void getSubDetectorTag(uint32_t det_id,
                                std::string& subdet_tag,
                                const TrackerTopology* tTopo);
  static void setBadModuleFlag(std::string& hname, uint16_t& flg);
  static void getBadModuleStatus(uint16_t flag, std::string& message);
  static void getTopFolderPath(DQMStore& dqm_store,
