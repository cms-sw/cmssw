...
class SiStripTrackerMapCreator {
 public:
  ...
  void create(edm::ParameterSet const& tkmapPset,
              DQMStore& dqm_store, std::string const& htype, edm::EventSetup const& eSetup);
  void createForOffline(edm::ParameterSet const& tkmapPset,
                        DQMStore& dqm_store, std::string& htype, edm::EventSetup const& eSetup);
 ...
 private:
  ...
  uint16_t getDetectorFlag(uint32_t const det_id) { return detFlag_.find(det_id) != detFlag_.end() ? detFlag_[det_id] : 0; }
