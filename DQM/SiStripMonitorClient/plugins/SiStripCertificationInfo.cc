...
void SiStripCertificationInfo::bookSiStripCertificationMEs(DQMStore& dqm_store)
{
  ...
  else dqm_store.setCurrentFolder("SiStrip/EventInfo");
  ...
  for (auto const& pr : SubDetMEsMap) {
    ...
    std::string tag;
  dqm_store.cd();
  if (!strip_dir.empty()) dqm_store.setCurrentFolder(strip_dir+"/EventInfo/CertificationContents");
  else dqm_store.setCurrentFolder("SiStrip/EventInfo/CertificationContents");
    tag = "TIB";
