...
EcalLogicID towerID(EcalElectronicsId const &_id) {
  ...
  if (iDCC <= kEEmHigh || iDCC >= kEEpLow) {
    ...
    return EcalLogicID("EB_trigger_tower",
                       1021000000UL + 10000 * ism + _id.towerId(), ism,
                       _id.towerId());
  }
}
...
EcalLogicID memChannelID(EcalPnDiodeDetId const &_id) {
  ...
  if (iDCC <= kEEmHigh || iDCC >= kEEpLow) {
    return EcalLogicID("EE_mem_channel", 100 * (601 + iDCC) + memId, 601 + iDCC,
                       memId);
  } else {
    int ism(iDCC <= kEBmHigh ? 19 + iDCC - kEBmLow : 1 + iDCC - kEBpLow);
    return EcalLogicID("EB_mem_channel", 1191000000UL + 10000 * ism + memId,
                       ism, memId);
  }
}
...
EcalLogicID memTowerID(EcalElectronicsId const &_id) {
  unsigned iDCC(_id.dccId() - 1);
  if (iDCC <= kEEmHigh || iDCC >= kEEpLow) {
    return EcalLogicID("EE_mem_TT", 100 * (601 + iDCC) + _id.towerId(),
                       601 + iDCC, _id.towerId());
  } else {
    int ism(iDCC <= kEBmHigh ? 19 + iDCC - kEBmLow : 1 + iDCC - kEBpLow);
    return EcalLogicID("EB_mem_TT", 1181000000UL + 10000 * ism + _id.towerId(),
                       ism, _id.towerId());
  }
}
...
EcalLogicID lmPNID(EcalPnDiodeDetId const &_id) {
  ...
  if (iDCC <= kEEmHigh || iDCC >= kEEpLow) {
    ...
    int ism(iDCC <= kEBmHigh ? 19 + iDCC - kEBmLow : 1 + iDCC - kEBpLow);
    return EcalLogicID("EB_LM_PN", 1131000000UL + 10000 * ism + pnid, ism,
                       pnid);
  }
}
