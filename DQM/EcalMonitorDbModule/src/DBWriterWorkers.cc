...
DBWriterWorker::DBWriterWorker(std::string const &_name,
  ...
  std::vector<std::string> runTypes(
      params.getUntrackedParameter<std::vector<std::string>>("runTypes"));
  ...
  edm::ParameterSet const &sourceParams(
      params.getUntrackedParameterSet("source"));
  ...
  for (unsigned iP(0); iP < meNames.size(); ++iP) {
    ...
    edm::ParameterSet const &meParam(
        sourceParams.getUntrackedParameterSet(meName));
...
void DBWriterWorker::retrieveSource(DQMStore::IGetter &_igetter) {
  ...
  for (MESetCollection::iterator sItr(this->source_.begin());
       sItr != this->source_.end(); ++sItr) {
    if (!sItr->second->retrieve(_igetter, &failedPath)) {
      edm::LogError("EcalDQM") << name_ << ": MESet " << sItr->first << "@"
...
bool IntegrityWriter::run(EcalCondDBInterface *_db, MonRunIOV &_iov) {
  ...
  */
  ...
  for (MESet::const_iterator dItr(digiME.beginChannel()); dItr != dEnd;
       dItr.toNextChannel()) {
    DetId id(dItr->getId());
  ...
  for (unsigned iMD(0); iMD < memDCC.size(); ++iMD) {
    ...
    for (unsigned iPN(1); iPN <= 10; ++iPN) {
      ...
      if (memchid > 0 || memgain > 0) {
        MonMemChConsistencyDat &data(
            memChannelConsistencies[memChannelID(pnid)]);
...
LaserWriter::LaserWriter(edm::ParameterSet const &_ps)
  ...
  std::vector<int> laserWavelengths(
      _ps.getUntrackedParameter<std::vector<int>>("laserWavelengths"));
  ...
  MESetMulti const &amplitude(
      static_cast<MESetMulti const &>(source_.at("Amplitude")));
...
bool LaserWriter::run(EcalCondDBInterface *_db, MonRunIOV &_iov) {
  ...
  */
  ...
  for (std::map<int, unsigned>::iterator wlItr(wlToME_.begin());
       wlItr != wlToME_.end(); ++wlItr) {
    int wl(wlItr->first);
    ...
    for (MESet::const_iterator aItr(ampME.beginChannel()); aItr != aEnd;
         aItr.toNextChannel()) {
...
PedestalWriter::PedestalWriter(edm::ParameterSet const &_ps)
  ...
  std::vector<int> MGPAGains(
      _ps.getUntrackedParameter<std::vector<int>>("MGPAGains"));
  std::vector<int> MGPAGainsPN(
      _ps.getUntrackedParameter<std::vector<int>>("MGPAGainsPN"));

  MESet::PathReplacements repl;

  MESetMulti const &pedestal(
      static_cast<MESetMulti const &>(source_.at("Pedestal")));
  ...
  MESetMulti const &pnPedestal(
      static_cast<MESetMulti const &>(source_.at("PNPedestal")));
...
bool PedestalWriter::run(EcalCondDBInterface *_db, MonRunIOV &_iov) {
  ...
  */
  ...
  for (std::map<int, unsigned>::iterator gainItr(gainToME_.begin());
       gainItr != gainToME_.end(); ++gainItr) {
    ...
    MESet::const_iterator qItr(qualityME);
    for (MESet::const_iterator pItr(pedestalME.beginChannel()); pItr != pEnd;
         pItr.toNextChannel()) {
  ...
  for (std::map<int, unsigned>::iterator gainItr(pnGainToME_.begin());
       gainItr != pnGainToME_.end(); ++gainItr) {
    int gain(gainItr->first);
    int iM(gainItr->second);
...
TestPulseWriter::TestPulseWriter(edm::ParameterSet const &_ps)
  ...
  MESetMulti const &pnAmplitude(
      static_cast<MESetMulti const &>(source_.at("PNAmplitude")));
  unsigned nGPN(MGPAGainsPN.size());
  for (unsigned iG(0); iG != nGPN; ++iG) {
    int gain(MGPAGainsPN[iG]);
    if (gain != 1 && gain != 16)
...
bool TestPulseWriter::run(EcalCondDBInterface *_db, MonRunIOV &_iov) {
  ...
  */
  ...
  for (std::map<int, unsigned>::iterator gainItr(gainToME_.begin());
    ...
    MESet::const_iterator qItr(qualityME);
    for (MESet::const_iterator aItr(amplitudeME.beginChannel()); aItr != aEnd;
         aItr.toNextChannel()) {
...
bool TimingWriter::run(EcalCondDBInterface *_db, MonRunIOV &_iov) {
  ...
  */
  ...
  MESet::const_iterator qItr(qualityME);
  for (MESet::const_iterator tItr(timingME.beginChannel()); tItr != tEnd;
       tItr.toNextChannel()) {
...
LedWriter::LedWriter(edm::ParameterSet const &_ps)
    : DBWriterWorker("Led", _ps), wlToME_() {
  std::vector<int> ledWavelengths(
      _ps.getUntrackedParameter<std::vector<int>>("ledWavelengths"));
...
bool OccupancyWriter::run(EcalCondDBInterface *_db, MonRunIOV &_iov) {
  ...
  */
  ...
  for (MESet::const_iterator oItr(occupancyME.beginChannel()); oItr != oEnd;
       oItr.toNextChannel()) {
