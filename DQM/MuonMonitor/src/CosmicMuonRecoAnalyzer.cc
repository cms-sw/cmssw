...
void CosmicMuonRecoAnalyzer::bookHistograms(DQMStore::IBooker & ibooker,
  ...
  qOverPStaTrack_p = ibooker.book2D(histname+"_qoverp_p","qoverp_p_{STA}", pBin, pMin, pMax, pResBin, pResMin, pResMax );                                       
  ...
  phiVsetaStaTrack = ibooker.book2D(histname+"_phiVseta",      "#phi vs #eta (STA)", etaBin/2, etaMin, etaMax, phiBin/2, phiMin, phiMax);
...
nValidHitsStaTrack_eta = ibooker.book2D(histname+"_nValidHits_eta", "#valid hits vs eta (STA)", etaBin, etaMin, etaMax, hitsBin, hitsMin, hitsMax);                         
                                                                                                                 
nValidHitsStaTrack_phi = ibooker.book2D(histname+"_nValidHits_phi", "#valid hits vs phi (STA)", phiBin, phiMin, phiMax, hitsBin, hitsMin, hitsMax);  
