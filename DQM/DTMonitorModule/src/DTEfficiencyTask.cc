...
DTEfficiencyTask::DTEfficiencyTask(const ParameterSet& pset) {
  ...
  doDetailedPlots = pset.getUntrackedParameter<bool>("doDetailedPlots", false);
  // label for dtDigis
  ...
  for (; ch_it != ch_end; ++ch_it) {
    ...
    for (; sl_it != sl_end; ++sl_it) {
      ...
      for (; l_it != l_end; ++l_it) {
        ...
        ibooker.setCurrentFolder("DT/04-OccupancyEfficiency/Wheel" + wheel.str() + "/Station" + station.str() + "/Sector" +
...
void DTEfficiencyTask::analyze(const edm::Event& event, const edm::EventSetup& setup) {
  ...
  //##################Detailed plots Layer by Layer###########################3
  if (doDetailedPlots){
  ...
  for (chamberId = all4DSegments->id_begin(); chamberId != all4DSegments->id_end(); ++chamberId) {
    ...
    // Get the 4D segment range for the corresponding ChamberId
    ...
    }  // End of loop over the 4D segments inside a single chamber
  }    // End of loop over all the chambers with at least a 4D segment in the event
 }     // doDetailedPlots
}
