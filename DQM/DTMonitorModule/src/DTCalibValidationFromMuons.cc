...
void DTCalibValidationFromMuons::analyze(const edm::Event& event, const edm::EventSetup& setup) {
  ...
  for (auto &imuon : *muons) {
    for (const auto &ch : imuon.matches()) {
        ...
        if ( chId.det() != DetId::Muon ) continue;
        if ( chId.subdetId() != MuonSubdetId::DT ) continue;
        if ( imuon.pt()<15 ) continue;
        ...
        for (DTRecSegment4DCollection::const_iterator segment = segs.first; segment!=segs.second; ++segment) {
          ...
          if (dr<5) selectedSegment4Ds.push_back(&(*segment));
        }
    }
...
float
DTCalibValidationFromMuons::recHitDistFromWire(const DTRecHit1DPair& hitPair, const DTLayer* layer) {
...
float
DTCalibValidationFromMuons::recHitDistFromWire(const DTRecHit1D& recHit, const DTLayer* layer) {
...
float
DTCalibValidationFromMuons::recHitPosition(const DTRecHit1DPair& hitPair, const DTLayer* layer, const DTChamber* chamber, float segmentPos, int sl) {
...
float
DTCalibValidationFromMuons::recHitPosition(const DTRecHit1D& recHit, const DTLayer* layer, const DTChamber* chamber, float segmentPos, int sl) {
...
void DTCalibValidationFromMuons::compute(const DTGeometry *dtGeom, const DTRecSegment4D& segment) {
  ...
  if(!computeResidual) ++wrongSegment;
  ...
  if(computeResidual) {
	...
	LogTrace("DTCalibValidationFromMuons") << "  Warning: dist segment-wire: " << SegmDistance;
      ...
      // Fill the histos
      if(sl == 1 || sl == 3)
        fillHistos(wireId.superlayerId(), SegmDistance, residualOnDistance, (wirePosInChamber.x() - segPosAtZWire.x()), residualOnPosition, 3);
      else
        fillHistos(wireId.superlayerId(), SegmDistance, residualOnDistance, (wirePosInChamber.y() - segPosAtZWire.y()), residualOnPosition, 3);
    }
  }
}
  ...
  for (; ch_it != ch_end; ++ch_it) {
    ...
    for(; sl_it != sl_end; ++sl_it) {
      ...
      for(int step = firstStep; step <= 3; ++step) {
         ...
         ibooker.setCurrentFolder("DT/DTCalibValidationFromMuons/Wheel" + wheel.str() +
	          		   "/Station" + station.str() +
		        	   "/Sector" + sector.str());
