...
//	cout<<setfill('0'); //set for formatting
...
//Fed9U::Fed9UDebugEvent::EnableDebug(true);
	...
	cout<< "FEDIDSIZE"<<fedIds_.size()<<endl;

	cout<< "EVENTNUMB"<<iEvent.id().event()<<endl;
...
//XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
	...
	if(data_u32 == NULL){ cerr<<"data_u32 is NULL !!"<<endl; continue; }
	...
	for(unsigned int i = 0; i < (size_u32 - 1); i+=2){
		...
		*/
		...
		//dumps a specified number of 32 bit words to the screen prior ot initalization
                ...
                cerr<<dec<<"BUFFER DUMP FOR FED NUMBER "<<*ifed<<endl;
...
/*
	...
	cout<<" INVERSIONTEST "<<inv8.invert8(221)<<endl;

	cout<<dec<<"FEE FOR FED # "<<*ifed<<" is "<<static_cast<uint16_t>(fedEvent_->getSpecialFeEnableReg() )<<endl;
...
//	edm::LogInfo("FE FSOP 8_3") << static_cast<uint16_t>(fedEvent_->getFSOP_8_3() );
	...
	cout<<"LAST"<<static_cast<uint16_t>(fedEvent_->getFSOP_8_3() )<<endl;
...
//takes into account frame synch out packet bits 0 - 31
	...
	if( 0x1 & (feEnable >> fpga) ){
		feCtr++; //increment the front end couter var
...
//		cout<<" VECADDS1 " <<" fed no: "<<*ifed<<" "<<feMajorAddress[*ifed][fpga]<<endl;
//-------------------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------------------
		...
		for(int i = 31; i >= 0; i--){
//block describing the 0 - 31 bits of what we have going on (for humans Fill( n +1 ) where (n+1) = Fiber Number------	
...
//reporting the majority APV address of the FED
        ...
        cout<<dec<<" FED NUMBER : "<<*ifed<<" CTR "<<feCtr<<" # FE enabled : "<<(feCtr - tmpCtr)<<" FEDKTR "<<fedCounter<<endl;
	cout<<dec<<" CTR: "<<feCtr<<endl;
	cout<<dec<<"For EVT "<<iEvent.id().event()<<endl;
//	cout<<dec<<" Mapping "<<*ifed<<" to Bin No "<< fedCtr <<endl;
	tmpCtr = feCtr;
	cout<<endl;
...
cout<<"GPRNUM"<<(double(100)-goodAPVper)<<" for event "<<iEvent.id().event()<<endl;
...
//Debug Stuff.......
	...
	cout<<"MIDLOC"<<(feCtr / 2)<<" "<<" feCtr: "<<feCtr<<" vector.size "<<feMedianAddr.size()<<" evtnumber : "<<iEvent.id().event()<<endl;	
	
	cout<<"ENUM"<<iEvent.id().event()<<endl;
	medianAddr = feMedianAddr[(feCtr / 2)];
	
	cout<<"MEDADDR"<<medianAddr<<endl;	
/*
	for(int i = 0; i < feCtr; i++){
	cout<<"VK# "<<" "<<i<<" "<<feMedianAddr[i]<<endl;
	}
*/
	for(int i = 0; i < feCtr; i++){
	...
	cout<<" PRCT "<< prct <<" for : "<<iEvent.id().event()<<endl;
	}
...
/*
	for(int i = 0; i < fenumbers.size(); i++){
		tempn = fenumbers[i];
		cout<<" NUMBERING " <<tempn<<endl;
	}
	for(int i = 0; i < fenumbers.size(); i++){
		tempfen = fen[i];
		cout<<" FENUMBERING " <<tempfen<<endl;
	}
...
for(int i = 0; i < fednumbers.size(); i++){
	...
	for(int fpga = 0; fpga < 8 ; fpga++){
                        ...
                        if( 0x1 & (tempfen >> fpga) ){
				cout<<" VECADDS2 " <<" fed no: "<<tempn<<" "<<feMajorAddress[tempn][fpga]<<endl;
               			cout<<" WHERRS "<<" fed no: "<<tempn<<" fpga no :"<<fpga<<WHError[tempn][fpga]<<endl;
               			cout<<" LKERRS "<<" fed no: "<<tempn<<" fpga no :"<<fpga<<LKError[tempn][fpga]<<endl;
               			cout<<" SYERRS "<<" fed no: "<<tempn<<" fpga no :"<<fpga<<SYError[tempn][fpga]<<endl;
               			cout<<" RWHERRS "<<" fed no: "<<tempn<<" fpga no :"<<fpga<<RWHError[tempn][fpga]<<endl;
...
//APVs in synch PRCTS + Low and High and MODE Calc-----------------------------------------------------------------
	...
	cout<<"NRML"<<normalize<<" for event # "<<iEvent.id().event()<<endl;
	cout<<"NRML2"<<normalize2<<" for event # "<<iEvent.id().event()<<endl;
	cout<<"BIZZY"<<badApvCounter<<" for event # "<<iEvent.id().event()<<endl;
	cout<<"GIZZY"<<goodApvCounter<<" for event # "<<iEvent.id().event()<<endl;
	...
	cout<<" APVPRCT "<<apvperct<<" EvT "<<iEvent.id().event()<<endl;
	cout<<" NOLOCK "<<nolock<<" EvT "<<iEvent.id().event()<<endl;
	cout<<" BADAPVCOUNTER "<<badApvCounter<<" EvT "<<iEvent.id().event()<<endl;
...
CnBAnalyzer::endJob() {
	...
	int sz = apvPrct.size();
        ...
        cout<<" MIZZY "<<apvPrct[sz - 1]<<endl;
        cout<<" LIZZY "<<apvPrct[0]<<endl;
