...
void FEDHistograms::initialise(const edm::ParameterSet& iConfig, std::ostringstream* pDebugStream) {
...
void FEDHistograms::fillFEDHistograms(FEDErrors& aFedErr,
  ...
  if (lFedLevelErrors.DataPresent)
    fillHistogram(dataPresent_, lFedId);
...
void FEDHistograms::fillFEHistograms(const unsigned int aFedId,
  ...
  */
  if (aFeLevelErrors.Overflow)
    fillHistogram(feOverflowDetailedMap_[aFedId], lFeId);
  else if (aFeLevelErrors.Missing)
    fillHistogram(feMissingDetailedMap_[aFedId], lFeId);
  else if (aFeLevelErrors.BadMajorityAddress)
    fillHistogram(badMajorityAddressDetailedMap_[aFedId], lFeId);
  ...
  if (aFeLevelErrors.TimeDifference != 0) {
    ...
    fillHistogram(
        feTimeDiffvsDBX_,
        aEventProp.deltaBX,
        aFeLevelErrors.TimeDifference < 0 ? aFeLevelErrors.TimeDifference + 192 : aFeLevelErrors.TimeDifference);
...
void FEDHistograms::fillAPVsHistograms(const unsigned int aFedId,
  ...
  */
  ...
  if (aAPVErr.APVStatusBit)
    fillHistogram(badStatusBitsDetailedMap_[aFedId], lChId);
  if (aAPVErr.APVError)
    fillHistogram(apvErrorDetailedMap_[aFedId], lChId);
  if (aAPVErr.APVAddressError)
    fillHistogram(apvAddressErrorDetailedMap_[aFedId], lChId);
}
...
void FEDHistograms::fillMajorityHistograms(const unsigned int aPart,
  ...
  if (aPart == 0)
    fillHistogram(feMajFracTIB_, aValue);
  else if (aPart == 1)
    fillHistogram(feMajFracTOB_, aValue);
  else if (aPart == 2)
    fillHistogram(feMajFracTECB_, aValue);
  else if (aPart == 3)
...
void FEDHistograms::bookTopLevelHistograms(DQMStore::IBooker& ibooker,
  ...
  bookHistogram(
      ibooker, nFEDErrors_, "nFEDErrors", "Number of FEDs with errors (FED or FE Level) per event", "# FEDErrors");

  bookHistogram(
      ibooker, nFEDDAQProblems_, "nFEDDAQProblems", "Number of FEDs with DAQ problems per event", "# FEDDAQProblems");
  ...
  bookHistogram(ibooker, apveAddress_, "ApveAddress", "apve Address", 256, 0, 256, "apveAddress");

  bookHistogram(ibooker, feMajAddress_, "FeMajAddress", "FE Majority Address", 256, 0, 256, "feMajAddress");
  ...
  bookHistogram(ibooker, nUnlocked_, "nUnlocked", "Number of channels Unlocked per event", "# channels unlocked");

  bookHistogram(ibooker, nOutOfSync_, "nOutOfSync", "Number of channels OutOfSync per event", "# channels out-of-sync");
  ...
  bookHistogram(ibooker, medianAPV0_, "MedianAPV0", "Median APV0", "medianAPV0");

  bookHistogram(ibooker, medianAPV1_, "MedianAPV1", "Median APV1", "MedianAPV1");
  ...
  bookHistogram(ibooker, nAPVError_, "nAPVError", "Number of APVs with APVError per event", "#APVs with APVError");
  ...
  bookProfile(ibooker,
              nTotalBadActiveChannelsvsTime_,
  ...
  if (fedErrorsVsIdVsLumi_.globalswitchon) {
    bookProfile2D(ibooker,
                  fedErrorsVsIdVsLumi_,
                  "fedErrorsVsIdVsLumi",
...
bool FEDHistograms::tkHistoMapEnabled(unsigned int aIndex) { return tkMapConfig_.enabled; }

TkHistoMap* FEDHistograms::tkHistoMapPointer(unsigned int aIndex) { return tkmapFED_.get(); }
