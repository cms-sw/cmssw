...
void Phase2TrackerMonitorDigi::fillITPixelDigiHistos(const edm::Handle<edm::DetSetVector<PixelDigi>>  handle, const edm::ESHandle<TrackerGeometry> gHandle) {
  ...
  for (typename edm::DetSetVector<PixelDigi>::const_iterator DSViter = digis->begin(); DSViter != digis->end(); DSViter++) {
	...
	if (XYPositionMap) XYPositionMap->Fill(pdPos.x()*10., pdPos.y()*10.);
	if (RZPositionMap) RZPositionMap->Fill(pdPos.z()*10., std::hypot(pdPos.x(),pdPos.y())*10.);  
      }
      nDigi++;
      edm::LogInfo("Phase2TrackerMonitorDigi")<< "  column " << col << " row " << row  <<
        std::dec  << std::endl;
      if (local_mes.ChargeXYMap) local_mes.ChargeXYMap->Fill(col, row, adc);
      if (local_mes.PositionOfDigisP) local_mes.PositionOfDigisP->Fill(row+1, col+1);
	...
	if (abs(row - row_last) == 1 && col == col_last) {
	  ...
	  charges.push_back(adc);
          ...
          position /= width;  
          if (local_mes.ClusterWidth)local_mes.ClusterWidth->Fill(width);
	  if (local_mes.ClusterPositionP) local_mes.ClusterPositionP->Fill(position, col+1);
	  for (auto v: charges) 
	    if (local_mes.ChargeOfDigisVsWidth) local_mes.ChargeOfDigisVsWidth->Fill(v, width);
   	  charges.clear();
          charges.push_back(adc);
	  width  = 1;
	  position = row+1;
    ...
    if (local_mes.NumberOfClustersPerDet) local_mes.NumberOfClustersPerDet->Fill(nclus);  
    ...
    if (geomDet) {
      ...
      if (XYOccupancyMap) XYOccupancyMap->Fill(gp.x()*10., gp.y()*10, occupancy);
      if (RZOccupancyMap) RZOccupancyMap->Fill(gp.z()*10., std::hypot(gp.x(),gp.y())*10., occupancy);  
  ...
  for ( auto& ilayer : layerMEs) {
    ...
    if (local_mes.TotalNumberOfDigisPerLayer) local_mes.TotalNumberOfDigisPerLayer->Fill(local_mes.nDigiPerLayer);
    if (local_mes.NumberOfHitDetectorsPerLayer) local_mes.NumberOfHitDetectorsPerLayer->Fill(local_mes.nHitDetsPerLayer);
...
void Phase2TrackerMonitorDigi::fillOTDigiHistos(const edm::Handle<edm::DetSetVector<Phase2TrackerDigi>>  handle, const edm::ESHandle<TrackerGeometry> gHandle) {
  ...
  for (typename edm::DetSetVector<Phase2TrackerDigi>::const_iterator DSViter = digis->begin(); DSViter != digis->end(); DSViter++) {
	...
	if (RZPositionMap) RZPositionMap->Fill(pdPos.z()*10., std::hypot(pdPos.x(),pdPos.y())*10.);  
      ...
      edm::LogInfo("Phase2TrackerMonitorDigi")<< "  column " << col << " row " << row  <<
        std::dec  << std::endl;
      if (nColumns > 2  && local_mes.PositionOfDigisP) local_mes.PositionOfDigisP->Fill(row+1, col+1);
      if (nColumns <= 2 && local_mes.PositionOfDigisS) local_mes.PositionOfDigisS->Fill(row+1, col+1);
	...
	if (abs(row - row_last) == 1 && col == col_last) {
	  ...
	  width++;
          ...
          position /= width;
          if (local_mes.ClusterWidth) local_mes.ClusterWidth->Fill(width);
	  if (local_mes.ClusterPositionP && nColumns > 2) local_mes.ClusterPositionP -> Fill(position, col+1);
	  if (local_mes.ClusterPositionS && nColumns <= 2) local_mes.ClusterPositionS -> Fill(position, col+1);
	...
	if (local_mes.DigiOccupancyS) local_mes.DigiOccupancyS->Fill(occupancy);
	if (local_mes.EtaOccupancyProfS)local_mes.EtaOccupancyProfS->Fill(gp.eta(), occupancy);
...
void Phase2TrackerMonitorDigi::bookHistograms(DQMStore::IBooker & ibooker,
  ...
  if (Parameters.getParameter<bool>("switch"))  XYPositionMap = ibooker.book2D("DigiXPosVsYPos","DigiXPosVsYPos",
  ...
  else XYPositionMap = nullptr;

  if (Parameters.getParameter<bool>("switch") && ParametersOcc.getParameter<bool>("switch")) XYOccupancyMap = ibooker.bookProfile2D("OccupancyInXY","OccupancyInXY",
					 Parameters.getParameter<int32_t>("Nxbins"),
					 Parameters.getParameter<double>("xmin"),
					 Parameters.getParameter<double>("xmax"),
					 Parameters.getParameter<int32_t>("Nybins"),
  ...
  else XYOccupancyMap = nullptr;
  
  Parameters =  config_.getParameter<edm::ParameterSet>("RZPositionMapH");  
  if (Parameters.getParameter<bool>("switch")) RZPositionMap = ibooker.book2D("DigiRPosVsZPos","DigiRPosVsZPos",
				 Parameters.getParameter<int32_t>("Nxbins"),
				 Parameters.getParameter<double>("xmin"),
				 Parameters.getParameter<double>("xmax"),
				 Parameters.getParameter<int32_t>("Nybins"),
				 Parameters.getParameter<double>("ymin"),
				 Parameters.getParameter<double>("ymax"));
  else RZPositionMap = nullptr;

  if (Parameters.getParameter<bool>("switch") && ParametersOcc.getParameter<bool>("switch"))  RZOccupancyMap = ibooker.bookProfile2D("OccupancyInRZ","OccupancyInRZ",
					 Parameters.getParameter<int32_t>("Nxbins"),
					 Parameters.getParameter<double>("xmin"),
					 Parameters.getParameter<double>("xmax"),
					 Parameters.getParameter<int32_t>("Nybins"),
...
void Phase2TrackerMonitorDigi::bookLayerHistos(DQMStore::IBooker & ibooker, unsigned int det_id, const TrackerTopology* tTopo){ 
  ...
  if (pos == layerMEs.end()) {
    ...
    if (Parameters.getParameter<bool>("switch")) local_mes.NumberOfDigisPerDet = ibooker.book1D(HistoName.str(), HistoName.str(),
					     Parameters.getParameter<int32_t>("Nbins"),
					     Parameters.getParameter<double>("xmin"),
					     Parameters.getParameter<double>("xmax"));
    else local_mes.NumberOfDigisPerDet = nullptr;
    ...
    if (pixelFlag_ || (layer < 4 || layer > 6)){
      ...
      if (Parameters.getParameter<bool>("switch")) local_mes.DigiOccupancyP = ibooker.book1D(HistoName.str(), HistoName.str(),
											     Parameters.getParameter<int32_t>("Nbins"),
											     Parameters.getParameter<double>("xmin"),
											     Parameters.getParameter<double>("xmax"));
      else local_mes.DigiOccupancyP = nullptr;
    
      
      HistoName.str("");
      HistoName << "DigiOccupancyVsEtaP_" << fname2.str();
      if (Parameters.getParameter<bool>("switch") && 
	  EtaParameters.getParameter<bool>("switch")) local_mes.EtaOccupancyProfP = 
	...
	local_mes.PositionOfDigisP = ibooker.book2D(HistoName.str(), HistoName.str(),
						    ...
						    Parameters.getParameter<double>("ymin"),
						    Parameters.getParameter<double>("ymax"));
      else local_mes.PositionOfDigisP = nullptr;
      
      Parameters =  config_.getParameter<edm::ParameterSet>("ClusterPositionPH");
      HistoName.str("");
      HistoName << "ClusterPositionP_" << fname2.str();
      if (Parameters.getParameter<bool>("switch")) 
	local_mes.ClusterPositionP = ibooker.book2D(HistoName.str(), HistoName.str(),
						    Parameters.getParameter<int32_t>("Nxbins"),
						    Parameters.getParameter<double>("xmin"),
						    Parameters.getParameter<double>("xmax"),
						    Parameters.getParameter<int32_t>("Nybins"),
						    Parameters.getParameter<double>("ymin"),
						    Parameters.getParameter<double>("ymax"));
      else local_mes.ClusterPositionP = nullptr;
    ...
    if (Parameters.getParameter<bool>("switch")) local_mes.ChargeXYMap = ibooker.book2D(HistoName.str(), HistoName.str(),
					       Parameters.getParameter<int32_t>("Nxbins"),
					       Parameters.getParameter<double>("xmin"),
					       Parameters.getParameter<double>("xmax"),
					       Parameters.getParameter<int32_t>("Nybins"),
					       Parameters.getParameter<double>("ymin"),
					       Parameters.getParameter<double>("ymax"));
    else local_mes.ChargeXYMap = nullptr;
    ...
    if (Parameters.getParameter<bool>("switch")) local_mes.TotalNumberOfDigisPerLayer = ibooker.book1D(HistoName.str(), HistoName.str(),
					     Parameters.getParameter<int32_t>("Nbins"),
					     Parameters.getParameter<double>("xmin"),
					     Parameters.getParameter<double>("xmax"));
    else local_mes.TotalNumberOfDigisPerLayer = nullptr;
    ...
    if (Parameters.getParameter<bool>("switch")) local_mes.NumberOfHitDetectorsPerLayer = ibooker.book1D(HistoName.str(), HistoName.str(),
					     Parameters.getParameter<int32_t>("Nbins"),
					     Parameters.getParameter<double>("xmin"),
					     Parameters.getParameter<double>("xmax"));
    else local_mes.NumberOfHitDetectorsPerLayer = nullptr;

    Parameters =  config_.getParameter<edm::ParameterSet>("NumberOfClustersPerDetH");
    HistoName.str("");
    HistoName << "NumberOfClustersPerDet_" << fname2.str();
    if (Parameters.getParameter<bool>("switch")) local_mes.NumberOfClustersPerDet = ibooker.book1D(HistoName.str(), HistoName.str(),
					     Parameters.getParameter<int32_t>("Nbins"),
					     Parameters.getParameter<double>("xmin"),
					     Parameters.getParameter<double>("xmax"));
    else local_mes.NumberOfClustersPerDet = nullptr;

    Parameters =  config_.getParameter<edm::ParameterSet>("ClusterWidthH");
    HistoName.str("");
    HistoName << "ClusterWidth_" << fname2.str();
    if (Parameters.getParameter<bool>("switch")) local_mes.ClusterWidth = ibooker.book1D(HistoName.str(), HistoName.str(),
					    Parameters.getParameter<int32_t>("Nbins"),
					    Parameters.getParameter<double>("xmin"),
					    Parameters.getParameter<double>("xmax"));
    else local_mes.ClusterWidth = nullptr;
    ...
    if (!pixelFlag_) {
      ...
      if (Parameters.getParameter<bool>("switch")) local_mes.DigiOccupancyS = ibooker.book1D(HistoName.str(), HistoName.str(),
						Parameters.getParameter<int32_t>("Nbins"),
						Parameters.getParameter<double>("xmin"),
						Parameters.getParameter<double>("xmax"));
      else local_mes.DigiOccupancyS = nullptr;

      HistoName.str("");
      HistoName << "DigiOccupancyVsEtaS_" << fname2.str();
      if (Parameters.getParameter<bool>("switch") && EtaParameters.getParameter<bool>("switch")) local_mes.EtaOccupancyProfS = ibooker.bookProfile(HistoName.str(), HistoName.str(),
							EtaParameters.getParameter<int32_t>("Nbins"),
							EtaParameters.getParameter<double>("xmin"),
							EtaParameters.getParameter<double>("xmax"),
							Parameters.getParameter<double>("xmin"),
							Parameters.getParameter<double>("xmax"),"");
      else local_mes.EtaOccupancyProfS = nullptr;
      ...
      if (Parameters.getParameter<bool>("switch") && EtaParameters.getParameter<bool>("switch")) local_mes.FractionOfOvTBitsVsEta= ibooker.bookProfile(HistoName.str(), HistoName.str(), 	
							    EtaParameters.getParameter<int32_t>("Nbins"),
							    EtaParameters.getParameter<double>("xmin"),
							    EtaParameters.getParameter<double>("xmax"),
							    Parameters.getParameter<double>("xmin"),
							    Parameters.getParameter<double>("xmax"),"");
      else local_mes.FractionOfOvTBitsVsEta = nullptr;

      Parameters =  config_.getParameter<edm::ParameterSet>("ClusterPositionSH");
      HistoName.str("");
      HistoName << "ClusterPositionS_" << fname2.str();
      if (Parameters.getParameter<bool>("switch")) local_mes.ClusterPositionS = ibooker.book2D(HistoName.str(), HistoName.str(),
											     Parameters.getParameter<int32_t>("Nxbins"),
											     Parameters.getParameter<double>("xmin"),
											     Parameters.getParameter<double>("xmax"),
											     Parameters.getParameter<int32_t>("Nybins"),
	...
	local_mes.PositionOfDigisS = ibooker.book2D(HistoName.str(), HistoName.str(),
						    ...
						    Parameters.getParameter<double>("ymin"),
						    Parameters.getParameter<double>("ymax"));
      else local_mes.PositionOfDigisS = nullptr;
    }
    else {
      ...
      if (Parameters.getParameter<bool>("switch")) local_mes.ChargeOfDigis = ibooker.book1D(HistoName.str(),HistoName.str(),
					       Parameters.getParameter<int32_t>("Nbins"),
					       Parameters.getParameter<double>("xmin"),
					       Parameters.getParameter<double>("xmax"));
      else local_mes.ChargeOfDigis = nullptr;
	...
	local_mes.ChargeOfDigisVsWidth = ibooker.book2D(HistoName.str(),HistoName.str(),
      ...
      else local_mes.ChargeOfDigisVsWidth = nullptr;
