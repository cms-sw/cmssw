...
void RPCEfficiency::bookHistograms(DQMStore::IBooker& ibooker, edm::Run const& run, edm::EventSetup const& iSetup) {
  ...
  //Endcap
  hGlobalResClu1R3C =
      ibooker.book1D("GlobalResidualsClu1R3C", "RPC Residuals Ring 3 Roll C Cluster Size 1", 101, -10., 10.);
  hGlobalResClu1R3B =
      ibooker.book1D("GlobalResidualsClu1R3B", "RPC Residuals Ring 3 Roll B Cluster Size 1", 101, -10., 10.);
  hGlobalResClu1R3A =
      ibooker.book1D("GlobalResidualsClu1R3A", "RPC Residuals Ring 3 Roll A Cluster Size 1", 101, -10., 10.);
  hGlobalResClu1R2C =
      ibooker.book1D("GlobalResidualsClu1R2C", "RPC Residuals Ring 2 Roll C Cluster Size 1", 101, -10., 10.);
  hGlobalResClu1R2B =
      ibooker.book1D("GlobalResidualsClu1R2B", "RPC Residuals Ring 2 Roll B Cluster Size 1", 101, -10., 10.);
  hGlobalResClu1R2A =
      ibooker.book1D("GlobalResidualsClu1R2A", "RPC Residuals Ring 2 Roll A Cluster Size 1", 101, -10., 10.);

  hGlobalResClu2R3C =
      ibooker.book1D("GlobalResidualsClu2R3C", "RPC Residuals Ring 3 Roll C Cluster Size 2", 101, -10., 10.);
  hGlobalResClu2R3B =
      ibooker.book1D("GlobalResidualsClu2R3B", "RPC Residuals Ring 3 Roll B Cluster Size 2", 101, -10., 10.);
  hGlobalResClu2R3A =
      ibooker.book1D("GlobalResidualsClu2R3A", "RPC Residuals Ring 3 Roll A Cluster Size 2", 101, -10., 10.);
  hGlobalResClu2R2C =
      ibooker.book1D("GlobalResidualsClu2R2C", "RPC Residuals Ring 2 Roll C Cluster Size 2", 101, -10., 10.);
  hGlobalResClu2R2B =
      ibooker.book1D("GlobalResidualsClu2R2B", "RPC Residuals Ring 2 Roll B Cluster Size 2", 101, -10., 10.);
  hGlobalResClu2R2A =
      ibooker.book1D("GlobalResidualsClu2R2A", "RPC Residuals Ring 2 Roll A Cluster Size 2", 101, -10., 10.);

  hGlobalResClu3R3C =
      ibooker.book1D("GlobalResidualsClu3R3C", "RPC Residuals Ring 3 Roll C Cluster Size 3", 101, -10., 10.);
  hGlobalResClu3R3B =
      ibooker.book1D("GlobalResidualsClu3R3B", "RPC Residuals Ring 3 Roll B Cluster Size 3", 101, -10., 10.);
  hGlobalResClu3R3A =
      ibooker.book1D("GlobalResidualsClu3R3A", "RPC Residuals Ring 3 Roll A Cluster Size 3", 101, -10., 10.);
  hGlobalResClu3R2C =
      ibooker.book1D("GlobalResidualsClu3R2C", "RPC Residuals Ring 2 Roll C Cluster Size 3", 101, -10., 10.);
  hGlobalResClu3R2B =
      ibooker.book1D("GlobalResidualsClu3R2B", "RPC Residuals Ring 2 Roll B Cluster Size 3", 101, -10., 10.);
  hGlobalResClu3R2A =
      ibooker.book1D("GlobalResidualsClu3R2A", "RPC Residuals Ring 2 Roll A Cluster Size 3", 101, -10., 10.);

  edm::ESHandle<RPCGeometry> rpcGeo;
  iSetup.get<MuonGeometryRecord>().get(rpcGeo);

  for (TrackingGeometry::DetContainer::const_iterator it = rpcGeo->dets().begin(); it < rpcGeo->dets().end();
       it++) {  //Loop on all detector units
    ...
    if (dynamic_cast<const RPCChamber*>(*it) != nullptr) {  // check if chamber exists
      ...
      for (std::vector<const RPCRoll*>::const_iterator r = roles.begin(); r != roles.end(); ++r) {  //Loop on all rolls
        ...
        bookDetUnitSeg(ibooker,
                       rpcId,
                       (*r)->nstrips(),
                       folderPath + "MuonSegEff/",
                       meCollection[rpcId.rawId()]);  // book histograms
        ...
        if (region == 0 && (incldt || incldtMB4)) {
          ...
          if (rollstoreDT.find(ind) != rollstoreDT.end()) {
            myrolls = rollstoreDT[ind];
          }
          ...
          if (rpcId.ring() == 2 || rpcId.ring() == 3) {
            ...
            if (rollstoreCSC.find(ind2) != rollstoreCSC.end()) {
              myrolls2 = rollstoreCSC[ind2];
            }
            ...
            if (rollstoreCSC.find(ind3) != rollstoreCSC.end()) {
              myrolls3 = rollstoreCSC[ind3];
            }
  ...
  if (incldt) {
    ...
    if (all4DSegments.isValid()) {
      if (all4DSegments->size() > 0) {
            ...
            if (segment->dimension() == 4) {
              ...
              for (std::set<RPCDetId>::iterator iteraRoll = rollsForThisDT.begin(); iteraRoll != rollsForThisDT.end();
                ...
                if (extrapolatedDistance <= MaxD) {
                  ...
                  if (fabs(PointExtrapolatedRPCFrame.z()) < 10. && fabs(PointExtrapolatedRPCFrame.x()) < rsize * 0.5 &&
                    ...
                    const float stripPredicted = rollasociated->strip(
                        LocalPoint(PointExtrapolatedRPCFrame.x(), PointExtrapolatedRPCFrame.y(), 0.));

                    LogDebug("rpcefficiency")
                    ...
                    typedef std::pair<RPCRecHitCollection::const_iterator, RPCRecHitCollection::const_iterator>
                        rangeRecHits;
                    ...
                    for (recHit = recHitCollection.first; recHit != recHitCollection.second; recHit++) {
                      ...
                      LogDebug("rpcefficiency")
                          << "DT  \t \t \t \t \t Found Rec Hit at " << res << "cm of the prediction.";
                    ...
                    if (countRecHits == 0) {
                      ...
                      if (fabs(minres) <= (rangestrips + cluSize * 0.5) * stripw) {
                        ...
                        if (rollId.station() == 1 && rollId.layer() == 1) {
                          if (cluSize == 1 * dupli) {
                            hGlobalResClu1La[0]->Fill(minres);
                          } else if (cluSize == 2 * dupli) {
                            hGlobalResClu2La[0]->Fill(minres);
                          } else if (cluSize == 3 * dupli) {
                            hGlobalResClu3La[0]->Fill(minres);
                          }
                        } else if (rollId.station() == 1 && rollId.layer() == 2) {
                          if (cluSize == 1 * dupli) {
                            hGlobalResClu1La[1]->Fill(minres);
                          } else if (cluSize == 2 * dupli) {
                            hGlobalResClu2La[1]->Fill(minres);
                          } else if (cluSize == 3 * dupli) {
                            hGlobalResClu3La[1]->Fill(minres);
                          }
                        } else if (rollId.station() == 2 && rollId.layer() == 1) {
                          if (cluSize == 1 * dupli) {
                            hGlobalResClu1La[2]->Fill(minres);
                          } else if (cluSize == 2 * dupli) {
                            hGlobalResClu2La[2]->Fill(minres);
                          } else if (cluSize == 3 * dupli) {
                            hGlobalResClu3La[2]->Fill(minres);
                          }
                        } else if (rollId.station() == 2 && rollId.layer() == 2) {
                          if (cluSize == 1 * dupli) {
                            hGlobalResClu1La[3]->Fill(minres);
                          }
                          if (cluSize == 2 * dupli) {
                            hGlobalResClu2La[3]->Fill(minres);
                          } else if (cluSize == 3 * dupli) {
                            hGlobalResClu3La[3]->Fill(minres);
                          }
                        } else if (rollId.station() == 3) {
                          if (cluSize == 1 * dupli) {
                            hGlobalResClu1La[4]->Fill(minres);
                          } else if (cluSize == 2 * dupli) {
                            hGlobalResClu2La[4]->Fill(minres);
                          } else if (cluSize == 3 * dupli) {
                            hGlobalResClu3La[4]->Fill(minres);
                          }
                        }
                        meIdRPC.str("");
        ...
        LogDebug("rpcefficiency")
            << "DT This Event doesn't have any DT4DDSegment";  //is ther more than 1 segment in this event?
      }
    }
  }
        ...
        if (DTSegmentCounter[DTId] == 1 && DTId.station() == 4) {
          ...
          if (segment->dimension() == 2) {
            ...
            for (segMB3 = all4DSegments->begin(); segMB3 != all4DSegments->end(); ++segMB3) {
              ...
              if (distsector_tmp(dtid3.sector(), DTId.sector()) <= 1 && dtid3.station() == 3 &&
                  dtid3.wheel() == DTId.wheel() && DTSegmentCounter[dtid3] == 1 && segMB3->dimension() == 4) {
                ...
                if (cosAng > MinCosAng) {
                  ...
                  std::set<RPCDetId> rollsForThisDT =
                      rollstoreDT[DTStationIndex(0, dtWheel, dtSector, dtStation)];  //It should be always 4
                      ...
                      if (fabs(PointExtrapolatedRPCFrame.z()) < 5. &&
                        ...
                        if (countRecHits == 0) {
                          ...
                          if (fabs(minres) <= (rangestrips + cluSize * 0.5) * stripw) {
                            ...
                            if (cluSize == 1 * dupli) {
                              hGlobalResClu1La[5]->Fill(minres);
                            } else if (cluSize == 2 * dupli) {
                              hGlobalResClu2La[5]->Fill(minres);
                            } else if (cluSize == 3 * dupli) {
                              hGlobalResClu3La[5]->Fill(minres);
                            }
                  ...
                  LogDebug("rpcefficiency") << "MB4 \t \t \t \t I found segments in MB4 and MB3 adjacent or same wheel "
                                               "and sector but not compatibles Diferent Directions";
                }
      ...
      if (allCSCSegments->size() > 0) {
        ...
        for (segment = allCSCSegments->begin(); segment != allCSCSegments->end(); ++segment) {
          ...
          if (CSCSegmentsCounter[CSCId] == 1 && CSCId.ring() != 1 && allCSCSegments->size() >= 2) {
            ...
            int rpcRegion = 1;
            if (cscEndCap == 2)
              rpcRegion = -1;  //Relation among the endcaps
            int rpcRing = cscRing;
            if (cscRing == 4)
              rpcRing = 1;
            ...
            LogDebug("rpcefficiency")
                ...
                << acos(dz) * 180 / 3.1415926 << " < 135? ";

            if (segment->dimension() == 4 && (segment->nRecHits() <= 10 && segment->nRecHits() >= 4) &&
                acos(dz) * 180 / 3.1415926 > 45. && acos(dz) * 180 / 3.1415926 < 160.) {
              float Xo = segmentPosition.x();
              float Yo = segmentPosition.y();
              float Zo = segmentPosition.z();
              float dx = segmentDirection.x();
              ...
              std::set<RPCDetId> rollsForThisCSC =
                  rollstoreCSC[CSCStationIndex(rpcRegion, rpcStation, rpcRing, rpcSegment)];
                ...
                for (std::set<RPCDetId>::iterator iteraRoll = rollsForThisCSC.begin();
                  ...
                  if (extrapolatedDistance <= MaxD) {
                    ...
                    if (fabs(PointExtrapolatedRPCFrame.z()) < 10. &&
                      ...
                      LogDebug("rpcefficiency") << "CSC  \t \t \t \t \t Candidate" << rollId << " "
                      ...
                      for (recHit = recHitCollection.first; recHit != recHitCollection.second; recHit++) {
                        ...
                        float res = PointExtrapolatedRPCFrame.x() - recHitPos.x();
                        LogDebug("rpcefficiency")
                            << "CSC  \t \t \t \t \t \t Found Rec Hit at " << res << "cm of the prediction.";
                        if (fabs(res) < fabs(minres)) {
                          minres = res;
                          cluSize = recHit->clusterSize();
                      ...
                      if (countRecHits == 0) {
                        ...
                        if (fabs(minres) <= (rangestrips + cluSize * 0.5) * stripw) {
                          ...
                          if (rollId.ring() == 2 && rollId.roll() == 1) {
                            if (cluSize == 1 * dupli) {
                              hGlobalResClu1R2A->Fill(minres);
                            } else if (cluSize == 2 * dupli) {
                              hGlobalResClu2R2A->Fill(minres);
                            } else if (cluSize == 3 * dupli) {
                              hGlobalResClu3R2A->Fill(minres);
                            }
                          } else if (rollId.ring() == 2 && rollId.roll() == 2) {
                            if (cluSize == 1 * dupli) {
                              hGlobalResClu1R2B->Fill(minres);
                            } else if (cluSize == 2 * dupli) {
                              hGlobalResClu2R2B->Fill(minres);
                            } else if (cluSize == 3 * dupli) {
                              hGlobalResClu3R2B->Fill(minres);
                            }
                          } else if (rollId.ring() == 2 && rollId.roll() == 3) {
                            if (cluSize == 1 * dupli) {
                              hGlobalResClu1R2C->Fill(minres);
                            } else if (cluSize == 2 * dupli) {
                              hGlobalResClu2R2C->Fill(minres);
                            } else if (cluSize == 3 * dupli) {
                              hGlobalResClu3R2C->Fill(minres);
                            }
                          } else if (rollId.ring() == 3 && rollId.roll() == 1) {
                            if (cluSize == 1 * dupli) {
                              hGlobalResClu1R3A->Fill(minres);
                            } else if (cluSize == 2 * dupli) {
                              hGlobalResClu2R3A->Fill(minres);
                            } else if (cluSize == 3 * dupli) {
                              hGlobalResClu3R3A->Fill(minres);
                            }
                          } else if (rollId.ring() == 3 && rollId.roll() == 2) {
                            if (cluSize == 1 * dupli) {
                              hGlobalResClu1R3B->Fill(minres);
                            } else if (cluSize == 2 * dupli) {
                              hGlobalResClu2R3B->Fill(minres);
                            } else if (cluSize == 3 * dupli) {
                              hGlobalResClu3R3B->Fill(minres);
                            }
                          } else if (rollId.ring() == 3 && rollId.roll() == 3) {
                            if (cluSize == 1 * dupli) {
                              hGlobalResClu1R3C->Fill(minres);
                            } else if (cluSize == 2 * dupli) {
                              hGlobalResClu2R3C->Fill(minres);
                            } else if (cluSize == 3 * dupli) {
                              hGlobalResClu3R3C->Fill(minres);
                            }
                          }
                          meIdRPC.str("");
