...
DTChamberEfficiencyTest::~DTChamberEfficiencyTest() {
  edm::LogVerbatim("DTDQM|DTMonitorClient|DTChamberEfficiencyTest")
      << "DTChamberEfficiencyTest: analyzed " << nevents << " events";
}
      ...
      for (int xBin = 1; xBin <= lastBinX; xBin++) {
        ...
        for (int yBin = 1; yBin <= lastBinY; yBin++) {
          if (GoodSegDen_histo_root->GetBinContent(xBin, yBin) != 0) {
            float XvsYefficiency = GoodCloseSegNum_histo_root->GetBinContent(xBin, yBin) /
                                   GoodSegDen_histo_root->GetBinContent(xBin, yBin);
...
string DTChamberEfficiencyTest::getMEName(string histoTag, const DTChamberId& chID) {
  ...
  string folderName = folderRoot + "DT/01-DTChamberEfficiency/Task/Wheel" + wheel.str() + "/Sector" + sector.str() +
                      "/Station" + station.str() + "/";
...
void DTChamberEfficiencyTest::bookHistos(DQMStore::IBooker& ibooker, const DTChamberId& chId) {
  ...
  ibooker.setCurrentFolder("DT/01-DTChamberEfficiency/Wheel" + wheel.str() + "/Sector" + sector.str() + "/Station" +
                           station.str());

  xEfficiencyHistos[HistoName] =
      ibooker.book1D(xEfficiencyHistoName.c_str(), xEfficiencyHistoName.c_str(), 25, -250., 250.);
  yEfficiencyHistos[HistoName] =
      ibooker.book1D(yEfficiencyHistoName.c_str(), yEfficiencyHistoName.c_str(), 25, -250., 250.);
  xVSyEffHistos[HistoName] =
      ibooker.book2D(xVSyEffHistoName.c_str(), xVSyEffHistoName.c_str(), 25, -250., 250., 25, -250., 250.);
}
