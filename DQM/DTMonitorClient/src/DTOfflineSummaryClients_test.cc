...
DTOfflineSummaryClients_test::DTOfflineSummaryClients_test(const ParameterSet& ps) : nevents(0) {

  LogVerbatim("DTDQM|DTMonitorClient|DTOfflineSummaryClients_test") << "[DTOfflineSummaryClients_test]: Constructor";
...
DTOfflineSummaryClients_test::~DTOfflineSummaryClients_test(){
  LogVerbatim ("DTDQM|DTMonitorClient|DTOfflineSummaryClients_test") << "DTOfflineSummaryClients_test: analyzed " << nevents << " events";
  
}

void DTOfflineSummaryClients_test::beginRun(Run const& run, EventSetup const& eSetup) {

  LogVerbatim("DTDQM|DTMonitorClient|DTOfflineSummaryClients_test") <<"[DTOfflineSummaryClients_test]: BeginRun"; 

  // book the summary histos
  dbe->setCurrentFolder("DT/EventInfo_test"); 
  ...
  dbe->setCurrentFolder("DT/EventInfo_test/reportSummaryContents_test");
...
void DTOfflineSummaryClients_test::endJob(void){
  
  LogVerbatim ("DTDQM|DTMonitorClient|DTOfflineSummaryClients_test") <<"[DTOfflineSummaryClients_test]: endJob"; 

}


void DTOfflineSummaryClients_test::endRun(Run const& run, EventSetup const& eSetup) {
  
  LogVerbatim ("DTDQM|DTMonitorClient|DTOfflineSummaryClients_test") <<"[DTOfflineSummaryClients_test]: endRun"; 

}


void DTOfflineSummaryClients_test::analyze(const Event& event, const EventSetup& context){

   nevents++;
   if(nevents%1000 == 0) {
     LogVerbatim("DTDQM|DTMonitorClient|DTOfflineSummaryClients_test") << "[DTOfflineSummaryClients_test] Analyze #Run: " << event.id().run()
...
void DTOfflineSummaryClients_test::endLuminosityBlock(LuminosityBlock const& lumiSeg, EventSetup const& context) {
  
  LogVerbatim("DTDQM|DTMonitorClient|DTOfflineSummaryClients_test")
    << "[DTOfflineSummaryClients_test]: End of LS transition, performing the DQM client operation" << endl;
  ...
  for(int wheel=-2; wheel<=2; wheel++) { // loop over wheels
	...
	for(int station = 1; station != 5; ++station) { // loop over stations
	  ...
	  LogTrace("DTDQM|DTMonitorClient|DTOfflineSummaryClients_test")
	...
	if(eff_result > 0.7) summaryReportMap->Fill(sector,wheel,1.);
      ...
      LogWarning("DTDQM|DTMonitorClient|DTOfflineSummaryClients_test")
	<< " [DTOfflineSummaryClients_test] Segment Summary not found with name: " << str.str() << endl;
    }
  }
