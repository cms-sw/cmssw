...
// -----------------------------------------------------------------------------
//
void SummaryHistogramFactory<ApvTimingAnalysis>::init( const sistrip::SummaryHisto& histo, 
...
uint32_t SummaryHistogramFactory<ApvTimingAnalysis>::extract( const map<uint32_t,ApvTimingAnalysis>& data ) {
  ...
  for ( ; iter != data.end(); iter++ ) {
    if ( histo_ == sistrip::APV_TIMING_PLL_COARSE ) {
      generator_->fillMap( level_, gran_, iter->first, iter->second.pllCoarse() ); 
    } else if ( histo_ == sistrip::APV_TIMING_PLL_FINE ) { 
    ...
    } else if ( histo_ == sistrip::APV_TIMING_BASE ) { 
      generator_->fillMap( level_, gran_, iter->first, iter->second.base() ); 
    } else if ( histo_ == sistrip::APV_TIMING_PEAK ) { 
