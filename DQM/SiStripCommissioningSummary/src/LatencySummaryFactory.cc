...
// -----------------------------------------------------------------------------
//
SummaryHistogramFactory<LatencyAnalysis>::SummaryHistogramFactory() :
...
// -----------------------------------------------------------------------------
//
void SummaryHistogramFactory<LatencyAnalysis>::init(const sistrip::Monitorable& mon, 
...
//------------------------------------------------------------------------------
//
uint32_t SummaryHistogramFactory<LatencyAnalysis>::extract( const std::map<uint32_t,LatencyAnalysis>& data ) {
  ...
  generator_->clearMap();
  std::map<uint32_t,LatencyAnalysis>::const_iterator iter = data.begin();
...
//------------------------------------------------------------------------------
//
void SummaryHistogramFactory<LatencyAnalysis>::fill( TH1& summary_histo ) {
...
// -----------------------------------------------------------------------------
//
template class SummaryHistogramFactory<LatencyAnalysis>;
