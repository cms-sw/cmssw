...
//         Created:
//
// Updated by Suvankar Roy Chowdhury February 2021
//
...
std::shared_ptr<SiPixelDigiCounter> SiPixelDigiSource::globalBeginLuminosityBlock(const edm::LuminosityBlock& lumi,
  ...
  if (modOn && resetCounters && averageDigiOccupancy) {
    lsc->nBPIXDigis = 0;
    lsc->nFPIXDigis = 0;
    for (int i = 0; i != FEDNumbering::MAXSiPixelFEDID; i++)
      lsc->nDigisPerFed[i] = 0;
  }
  ...
  if (modOn && resetCounters) {
    lsc->ROCMapToReset = true;  //the ROC map is reset each 10 lumisections

    for (int i = 0; i < 2; i++)
      lsc->NzeroROCs[i] = 0;
    for (int i = 0; i < 2; i++)
      lsc->NloEffROCs[i] = 0;  //resetting also Zero and low eff. ROC counters
    ...
    lsc->DoZeroRocsBMO1 = true;
    lsc->DoZeroRocsBMO2 = true;
    lsc->DoZeroRocsBMO3 = true;

    lsc->DoZeroRocsBMI1 = true;
    lsc->DoZeroRocsBMI2 = true;
    lsc->DoZeroRocsBMI3 = true;

    lsc->DoZeroRocsBPO1 = true;
    lsc->DoZeroRocsBPO2 = true;
    lsc->DoZeroRocsBPO3 = true;

    lsc->DoZeroRocsBPI1 = true;
    lsc->DoZeroRocsBPI2 = true;
    lsc->DoZeroRocsBPI3 = true;

    lsc->DoZeroRocsFPO1 = true;
    lsc->DoZeroRocsFPO2 = true;

    lsc->DoZeroRocsFMO1 = true;
    lsc->DoZeroRocsFMO2 = true;

    lsc->DoZeroRocsFPI1 = true;
    lsc->DoZeroRocsFPI2 = true;

    lsc->DoZeroRocsFMI1 = true;
    lsc->DoZeroRocsFMI2 = true;
  }
  return lsc;
}
...
void SiPixelDigiSource::globalEndLuminosityBlock(const edm::LuminosityBlock& lb, edm::EventSetup const&) {
  ...
  auto latestCounters = luminosityBlockCache(lb.index());

  float averageBPIXFed = float(latestCounters->nBPIXDigis) / 32.;
  float averageFPIXFed = float(latestCounters->nFPIXDigis) / 8.;
  ...
  if (averageDigiOccupancy) {
    for (int i = 0; i != FEDNumbering::MAXSiPixelFEDID; i++) {
      ...
      if (i < 32) {
        if (averageBPIXFed > 0.)
          averageOcc = latestCounters->nDigisPerFed[i] / averageBPIXFed;
      } else {
        if (averageFPIXFed > 0.)
          averageOcc = latestCounters->nDigisPerFed[i] / averageFPIXFed;
      }
      if (!modOn) {
        averageDigiOccupancy->Fill(
            i,
            latestCounters->nDigisPerFed
                [i]);  //In offline we fill all digis and normalise at the end of the run for thread safe behaviour.
        avgfedDigiOccvsLumi->setBinContent(thisls, i + 1, latestCounters->nDigisPerFed[i]);  //Same plot vs lumi section
      }
      if (modOn) {
        if (thisls % 10 == 0)
          averageDigiOccupancy->Fill(
...
void SiPixelDigiSource::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  auto latestCounters = luminosityBlockCache(iEvent.getLuminosityBlock().index());
  // get input data
  ...
  for (struct_iter = thePixelStructure.begin(); struct_iter != thePixelStructure.end(); struct_iter++) {
    ...
    if (numberOfDigisMod > 0) {
      ...
      if (barrel) {  // Barrel
        ...
        if (latestCounters->ROCMapToReset && shell == PixelBarrelName::mO && twoDimOnlyLayDisk) {
          if (latestCounters->DoZeroRocsBMO1 && layer == 1)
            CountZeroROCsInSubstructure(barrel, latestCounters->DoZeroRocsBMO1, (*struct_iter).second, latestCounters);
          if (latestCounters->DoZeroRocsBMO2 && layer == 2)
            CountZeroROCsInSubstructure(barrel, latestCounters->DoZeroRocsBMO2, (*struct_iter).second, latestCounters);
          if (latestCounters->DoZeroRocsBMO3 && layer == 3)
            CountZeroROCsInSubstructure(barrel, latestCounters->DoZeroRocsBMO3, (*struct_iter).second, latestCounters);
        } else if (latestCounters->ROCMapToReset && shell == PixelBarrelName::mI && twoDimOnlyLayDisk) {
          if (latestCounters->DoZeroRocsBMI1 && layer == 1)
            CountZeroROCsInSubstructure(barrel, latestCounters->DoZeroRocsBMI1, (*struct_iter).second, latestCounters);
          if (latestCounters->DoZeroRocsBMI2 && layer == 2)
            CountZeroROCsInSubstructure(barrel, latestCounters->DoZeroRocsBMI2, (*struct_iter).second, latestCounters);
          if (latestCounters->DoZeroRocsBMI3 && layer == 3)
            CountZeroROCsInSubstructure(barrel, latestCounters->DoZeroRocsBMI3, (*struct_iter).second, latestCounters);
        } else if (latestCounters->ROCMapToReset && shell == PixelBarrelName::pO && twoDimOnlyLayDisk) {
          if (latestCounters->DoZeroRocsBPO1 && layer == 1)
            CountZeroROCsInSubstructure(barrel, latestCounters->DoZeroRocsBPO1, (*struct_iter).second, latestCounters);
          if (latestCounters->DoZeroRocsBPO2 && layer == 2)
            CountZeroROCsInSubstructure(barrel, latestCounters->DoZeroRocsBPO2, (*struct_iter).second, latestCounters);
          if (latestCounters->DoZeroRocsBPO3 && layer == 3)
            CountZeroROCsInSubstructure(barrel, latestCounters->DoZeroRocsBPO3, (*struct_iter).second, latestCounters);
        } else if (latestCounters->ROCMapToReset && shell == PixelBarrelName::pI && twoDimOnlyLayDisk) {
          if (latestCounters->DoZeroRocsBPI1 && layer == 1)
            CountZeroROCsInSubstructure(barrel, latestCounters->DoZeroRocsBPI1, (*struct_iter).second, latestCounters);
          if (latestCounters->DoZeroRocsBPI2 && layer == 2)
            CountZeroROCsInSubstructure(barrel, latestCounters->DoZeroRocsBPI2, (*struct_iter).second, latestCounters);
          if (latestCounters->DoZeroRocsBPI3 && layer == 3)
            CountZeroROCsInSubstructure(barrel, latestCounters->DoZeroRocsBPI3, (*struct_iter).second, latestCounters);
        }

        latestCounters->nBPIXDigis = latestCounters->nBPIXDigis + numberOfDigisMod;
        for (int i = 0; i != nBPiXmodules; ++i) {
          if ((*struct_iter).first == I_detId[i]) {
            latestCounters->nDigisPerFed[I_fedId[i]] = latestCounters->nDigisPerFed[I_fedId[i]] + numberOfDigisMod;
            int index1 = 0;
            int index2 = 0;
            if (I_linkId1[i] > 0)
        ...
        latestCounters->nFPIXDigis = latestCounters->nFPIXDigis + numberOfDigisMod;
        ...
        if (latestCounters->ROCMapToReset && side == PixelEndcapName::mO && twoDimOnlyLayDisk) {
          if (latestCounters->DoZeroRocsFMO1 && disk == 1)
            CountZeroROCsInSubstructure(barrel, latestCounters->DoZeroRocsFMO1, (*struct_iter).second, latestCounters);
          if (latestCounters->DoZeroRocsFMO2 && disk == 2)
            CountZeroROCsInSubstructure(barrel, latestCounters->DoZeroRocsFMO2, (*struct_iter).second, latestCounters);
        } else if (latestCounters->ROCMapToReset && side == PixelEndcapName::mI && twoDimOnlyLayDisk) {
          if (latestCounters->DoZeroRocsFMI1 && disk == 1)
            CountZeroROCsInSubstructure(barrel, latestCounters->DoZeroRocsFMI1, (*struct_iter).second, latestCounters);
          if (latestCounters->DoZeroRocsFMI2 && disk == 2)
            CountZeroROCsInSubstructure(barrel, latestCounters->DoZeroRocsFMI2, (*struct_iter).second, latestCounters);
        } else if (latestCounters->ROCMapToReset && side == PixelEndcapName::pO && twoDimOnlyLayDisk) {
          if (latestCounters->DoZeroRocsFPO1 && disk == 1)
            CountZeroROCsInSubstructure(barrel, latestCounters->DoZeroRocsFPO1, (*struct_iter).second, latestCounters);
          if (latestCounters->DoZeroRocsFPO2 && disk == 2)
            CountZeroROCsInSubstructure(barrel, latestCounters->DoZeroRocsFPO2, (*struct_iter).second, latestCounters);
        } else if (latestCounters->ROCMapToReset && side == PixelEndcapName::pI && twoDimOnlyLayDisk) {
          if (latestCounters->DoZeroRocsFPI1 && disk == 1)
            CountZeroROCsInSubstructure(barrel, latestCounters->DoZeroRocsFPI1, (*struct_iter).second, latestCounters);
          if (latestCounters->DoZeroRocsFPI2 && disk == 2)
            CountZeroROCsInSubstructure(barrel, latestCounters->DoZeroRocsFPI2, (*struct_iter).second, latestCounters);
        }
  ...
  if (lumiSection % 10 == 0 && latestCounters->ROCMapToReset) {
    for (int i = 0; i < 2; ++i)
      latestCounters->NloEffROCs[i] = latestCounters->NloEffROCs[i] - latestCounters->NzeroROCs[i];
    if (noOccROCsBarrel)
      noOccROCsBarrel->setBinContent(lumiSection / 10, latestCounters->NzeroROCs[0]);
    if (loOccROCsBarrel)
      loOccROCsBarrel->setBinContent(lumiSection / 10, latestCounters->NloEffROCs[0]);
    if (noOccROCsEndcap)
      noOccROCsEndcap->setBinContent(lumiSection / 10, latestCounters->NzeroROCs[1]);
    if (loOccROCsEndcap)
      loOccROCsEndcap->setBinContent(lumiSection / 10, latestCounters->NloEffROCs[1]);
    latestCounters->ROCMapToReset =
...
void SiPixelDigiSource::CountZeroROCsInSubstructure(bool barrel,
                                                    ...
                                                    SiPixelDigiCounter* latestCounters) {
  std::pair<int, int> tempPair = mod->getZeroLoEffROCs();

  if (barrel) {
    latestCounters->NzeroROCs[0] += tempPair.first;
    latestCounters->NloEffROCs[0] += tempPair.second;
  } else {
    latestCounters->NzeroROCs[1] += tempPair.first;
    latestCounters->NloEffROCs[1] += tempPair.second;
  }
