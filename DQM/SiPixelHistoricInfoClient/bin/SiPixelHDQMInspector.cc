 ...
 */
void runSiPixelInspector( const string & dbName, const string &tagName, const string & Password, const int Start, const int End, const int nRuns )
{
...
void SiPixelHDQMInspector( const string & dbName, const string &tagName, const string & password, const int start, const int end )
{
  runSiPixelInspector( dbName, tagName, password, start, end, 0 );
}

/// Simple method to create the trends. The actual operations are performed in runSiPixelInspector.
void SiPixelHDQMInspector( const string & dbName, const string & tagName, const string & password, const int nRuns )
{
  runSiPixelInspector( dbName, tagName, password, 0, 0, nRuns );
}
...
int main (int argc, char* argv[])
{
  ...
  if (argc == 5) {
    ...
    SiPixelHDQMInspector( argv[1], argv[2], argv[3], atoi(argv[4]) );
  } else if(argc == 6) {
    cout << "Creating trends for range:  " << argv[4] << " " << argv[5] << " for tag: " << argv[2] << endl;
    SiPixelHDQMInspector( argv[1], argv[2], argv[3], atoi(argv[4]), atoi(argv[5]) );
  }
