...
public:
  ...
  static bool checkME(std::string element, std::string name,
                      std::string &full_path);
  static int getMEList(std::string name, std::string &dir_path,
  ...
  static void
  fillPaveText(TPaveText *pave,
               const std::map<std::string, std::pair<int, double>> &messages);
  static void createStatusLegendMessages(
