...
void SiPixelActionExecutor::readConfiguration() {
  ...
  if (offlineXMLfile_)
    localPath =
        string("DQM/SiPixelMonitorClient/test/sipixel_tier0_config.xml");
  else
    localPath = string(
...
// -- Read Configuration File
//
bool SiPixelActionExecutor::readConfiguration(
    int &tkmap_freq, int &sum_barrel_freq, int &sum_endcap_freq,
    int &sum_grandbarrel_freq, int &sum_grandendcap_freq, int &message_limit_,
    int &source_type_, int &calib_type_) {
...
void SiPixelActionExecutor::createSummary(DQMStore::IBooker &iBooker,
  ...
  if (!configParser_->getMENamesForBarrelSummary(barrel_structure_name,
                                                 barrel_me_names)) {
  ...
  if (!configParser_->getMENamesForEndcapSummary(endcap_structure_name,
                                                 endcap_me_names)) {
    edm::LogInfo("SiPixelActionExecutor")
        << "Failed to read Endcap Summary configuration parameters!! "
        << "\n";
  ...
  if (source_type_ == 0 || source_type_ == 5 ||
      source_type_ == 20) { // do this only if RawData source is present
    string federror_structure_name;
    vector<string> federror_me_names;
    if (!configParser_->getMENamesForFEDErrorSummary(federror_structure_name,
                                                     federror_me_names)) {
    ...
    fillFEDErrorSummary(iBooker, iGetter, federror_structure_name,
                        federror_me_names);
    iBooker.setCurrentFolder("Pixel/");
    iGetter.setCurrentFolder("Pixel/");
  }
...
void SiPixelActionExecutor::bookDeviations(DQMStore::IBooker &iBooker,
  ...
  DEV_adc_Barrel = iBooker.book1D(
      "DEV_adc_Barrel", "Deviation from reference;Module;<adc_ref>-<adc>",
      nBPixModules, 0., nBPixModules);
  DEV_ndigis_Barrel =
      iBooker.book1D("DEV_ndigis_Barrel",
                     "Deviation from reference;Module;<ndigis_ref>-<ndigis>",
                     nBPixModules, 0., nBPixModules);
  DEV_charge_Barrel =
      iBooker.book1D("DEV_charge_Barrel",
                     "Deviation from reference;Module;<charge_ref>-<charge>",
                     nBPixModules, 0., nBPixModules);
  DEV_nclusters_Barrel = iBooker.book1D(
      "DEV_nclusters_Barrel",
      "Deviation from reference;Module;<nclusters_ref>-<nclusters>",
      nBPixModules, 0., nBPixModules);
  DEV_size_Barrel = iBooker.book1D(
      "DEV_size_Barrel", "Deviation from reference;Module;<size_ref>-<size>",
      nBPixModules, 0., nBPixModules);
  iBooker.cd();
  iBooker.setCurrentFolder("Pixel/Endcap");
  DEV_adc_Endcap = iBooker.book1D(
      "DEV_adc_Endcap", "Deviation from reference;Module;<adc_ref>-<adc>", 672,
      0., 672.);
  DEV_ndigis_Endcap = iBooker.book1D(
      "DEV_ndigis_Endcap",
      "Deviation from reference;Module;<ndigis_ref>-<ndigis>", 672, 0., 672.);
  DEV_charge_Endcap = iBooker.book1D(
      "DEV_charge_Endcap",
      "Deviation from reference;Module;<charge_ref>-<charge>", 672, 0., 672.);
  DEV_nclusters_Endcap = iBooker.book1D(
      "DEV_nclusters_Endcap",
      "Deviation from reference;Module;<nclusters_ref>-<nclusters>", 672, 0.,
      672.);
  DEV_size_Endcap = iBooker.book1D(
      "DEV_size_Endcap", "Deviation from reference;Module;<size_ref>-<size>",
      672, 0., 672.);
  iBooker.cd();
}
...
void SiPixelActionExecutor::GetBladeSubdirs(DQMStore::IBooker &iBooker,
  ...
  for (vector<string>::const_iterator it = panels.begin(); it != panels.end();
       it++) {
    iGetter.cd(*it);
    iBooker.cd(*it);
    modules = iGetter.getSubdirs();
    for (vector<string>::const_iterator m_it = modules.begin();
         m_it != modules.end(); m_it++) {
...
void SiPixelActionExecutor::fillSummary(DQMStore::IBooker &iBooker,
                                        DQMStore::IGetter &iGetter,
                                        string dir_name,
                                        vector<string> &me_names, bool isbarrel,
                                        bool isUpgrade) {
  ...
  if (currDir.find(dir_name) != string::npos) {
    ...
    for (vector<string>::const_iterator iv = me_names.begin();
      ...
      if (source_type_ == 5 || source_type_ == 6) {
        if ((*iv) == "errorType" || (*iv) == "NErrors" || (*iv) == "fullType" ||
            (*iv) == "chanNmbr" || (*iv) == "TBMType" || (*iv) == "EvtNbr" ||
            (*iv) == "evtSize" || (*iv) == "linkId" || (*iv) == "ROCId" ||
            (*iv) == "DCOLId" || (*iv) == "PXId" || (*iv) == "ROCNmbr" ||
            (*iv) == "TBMMessage" || (*iv) == "Type36Hitmap")
          prefix = "SUMRAW";
        else if ((*iv) == "ndigis" || (*iv) == "adc")
          prefix = "SUMDIG";
        else if ((*iv) == "nclusters" || (*iv) == "x" || (*iv) == "y" ||
                 (*iv) == "charge" || (*iv) == "size" || (*iv) == "sizeX" ||
                 (*iv) == "sizeY" || (*iv) == "minrow" || (*iv) == "maxrow" ||
                 (*iv) == "mincol" || (*iv) == "maxcol")
          prefix = "SUMCLU";
        if (currDir.find("Track") != string::npos)
          ...
          prefix = "SUMTRK";
                 ...
                 (*iv) == "nRecHits" || (*iv) == "ErrorX" || (*iv) == "ErrorY")
          prefix = "SUMHIT";
                 ...
                 (*iv) == "GainSaturate2d" ||
                 (*iv) == "ScurveChi2ProbSummary" ||
                 (*iv) == "ScurveFitResultSummary" ||
                 (*iv) == "ScurveSigmasSummary" ||
      ...
      if ((*iv).find("residual") != string::npos) { // track residuals
        ...
        if ((*iv) == "Gain1d" || (*iv) == "GainChi2NDF1d" ||
            (*iv) == "GainChi2Prob1d" || (*iv) == "GainNPoints1d" ||
            (*iv) == "GainHighPoint1d" || (*iv) == "GainLowPoint1d" ||
            (*iv) == "GainEndPoint1d" || (*iv) == "GainDynamicRange2d" ||
            (*iv) == "GainSaturate2d" || (*iv) == "Pedestal1d" ||
            (*iv) == "ScurveChi2ProbSummary" ||
            (*iv) == "ScurveFitResultSummary" ||
            (*iv) == "ScurveSigmasSummary" ||
            (*iv) == "ScurveThresholdSummary") {
        ...
        tag =
            prefix + "_" + (*iv) + "_" + currDir.substr(currDir.find(dir_name));
        temp = getSummaryME(iBooker, iGetter, tag, isUpgrade);
        sum_mes.push_back(temp);
        if ((*iv) == "ndigis") {
        ...
        if (prefix == "SUMCLU" && (*iv) == "charge") {
          ...
          if (temp) {
            ...
            temp =
                iBooker.book1D(tag.c_str(), tag.c_str(), 100, 0.,
    ...
    for (vector<string>::const_iterator it = subdirs.begin();
      ...
      if (prefix != "SUMOFF" && (*it).find("Module_") == string::npos)
        continue;
      if (prefix == "SUMOFF" &&
          (*it).find(isbarrel ? "Layer_" : "Disk_") == string::npos)
        continue;
      iBooker.cd(*it);
      iGetter.cd(*it);
      ndet++;
      ...
      for (vector<MonitorElement *>::const_iterator isum = sum_mes.begin();
        ...
        for (vector<string>::const_iterator im = contents.begin();
          ...
          if (sname.find("ALLMODS_chargeCOMB_") != string::npos)
            tname = "charge_";
          if (sname.find("_charge_") != string::npos &&
              sname.find("Track_") == string::npos)
            tname = "charge_";
          if (sname.find("_nclusters_") != string::npos &&
              sname.find("Track_") == string::npos)
            tname = "nclusters_";
          if (sname.find("_size_") != string::npos &&
              sname.find("Track_") == string::npos)
            tname = "size_";
          ...
          if (((*im)).find(tname) == 0) {
            ...
            if (me) {
              if (sname.find("_charge") != string::npos &&
                  sname.find("Track_") == string::npos &&
              ...
              if (sname.find("_nclusters_") != string::npos &&
                  ...
                  me->getName().find("Track_") != string::npos)
                continue;
              if (sname.find("_size") != string::npos &&
                  sname.find("Track_") == string::npos &&
                  me->getName().find("Track_") != string::npos)
                continue;
              // fill summary histos:
              if (sname.find("_RMS_") != string::npos &&
                ...
                for (int cols = 1; cols != me->getNbinsX() + 1; cols++)
                  for (int rows = 1; rows != me->getNbinsY() + 1; rows++) {
                    ...
                    SumOfSquaredEntries += (me->getBinContent(cols, rows)) *
                                           (me->getBinContent(cols, rows));
                    SumOfPixels++;
                  }
                  ...
                  for (int jj = 1; jj < 16; jj++) {
                    if (me1->getBinContent(jj) > 0.) {
                      if (jj == 6) { // errorType=30 (reset)
                        string path2 = path1;
                        path2 = path2.replace(path2.find("errorType"), 9,
                                              "TBMMessage");
                        MonitorElement *me2 = iGetter.get(path2);
                        if (me2)
                          if (me2->getBinContent(6) > 0. ||
              ...
              } else if ((sname.find("_charge_") != string::npos &&
                          sname.find("Track_") == string::npos &&
                          me->getName().find("Track_") == string::npos) ||
                         (sname.find("_charge_") != string::npos &&
                          sname.find("_OnTrack_") != string::npos &&
                          me->getName().find("_OnTrack_") != string::npos) ||
                         (sname.find("_charge_") != string::npos &&
                          sname.find("_OffTrack_") != string::npos &&
                          me->getName().find("_OffTrack_") != string::npos) ||
                         (sname.find("_nclusters_") != string::npos &&
                          sname.find("Track_") == string::npos &&
                          me->getName().find("Track_") == string::npos) ||
                         (sname.find("_nclusters_") != string::npos &&
                          sname.find("_OnTrack_") != string::npos &&
              ...
              string title = " ";
              if (sname.find("_RMS_") != string::npos) {
                title = "RMS of " + sname.substr(7, (sname.find("_", 7) - 7)) +
                        " per module";
              } else if (sname.find("_FracOfPerfectPix_") != string::npos) {
                title = "FracOfPerfectPix " +
                        sname.substr(7, (sname.find("_", 7) - 7)) +
                        " per module";
              } else if (sname.find("_NCalibErrors_") != string::npos) {
                title = "Number of CalibErrors " +
                        sname.substr(7, (sname.find("_", 7) - 7)) +
                        " per module";
              } else if (sname.find("_NNegativeFits_") != string::npos) {
                title = "Number of pixels with neg. fit result " +
                        sname.substr(7, (sname.find("_", 7) - 7)) +
                        " per module";
                ...
                if (prefix == "SUMOFF")
                  title = "Mean " + sname.substr(7, (sname.find("_", 7) - 7)) +
                          (isbarrel ? " per Ladder" : " per Blade");
                else
                  title = "Mean " + sname.substr(7, (sname.find("_", 7) - 7)) +
                          " per Module";
              }
    ...
    if (isbarrel) {
      ...
      if (!configParser_->getMENamesForGrandBarrelSummary(
              grandbarrel_structure_name, grandbarrel_me_names)) {
        cout << "SiPixelActionExecutor::createSummary: Failed to read Grand "
                "Barrel Summary configuration parameters!! ";
        return;
      }
      fillGrandBarrelSummaryHistos(iBooker, iGetter, grandbarrel_me_names,
                                   isUpgrade);
    ...
    } else // Endcap
    {
      ...
      if (!configParser_->getMENamesForGrandEndcapSummary(
              grandendcap_structure_name, grandendcap_me_names)) {
        cout << "SiPixelActionExecutor::createSummary: Failed to read Grand "
                "Endcap Summary configuration parameters!! ";
        return;
      }
      fillGrandEndcapSummaryHistos(iBooker, iGetter, grandendcap_me_names,
                                   isUpgrade);
    }
  }
  ...
  if (currDir.find(dir_name) != string::npos) {
    ...
    for (vector<string>::const_iterator iv = me_names.begin();
      ...
      if ((*iv) == "errorType" || (*iv) == "NErrors" || (*iv) == "fullType" ||
        ...
        sum_mes.push_back(temp);
      } else if ((*iv) == "FedChLErr" || (*iv) == "FedChNErr" ||
                 (*iv) == "FedETypeNErr") {
        string tag = prefix + "_" + (*iv);
        MonitorElement *temp;
        if ((*iv) == "FedChLErr") {
          if (!isBooked)
            temp = iBooker.book2D("FedChLErr", "Type of last error", 40, -0.5,
                                  39.5, 37, 0., 37.);
          else {
        ...
        if ((*iv) == "FedChNErr") {
          if (!isBooked)
            temp = iBooker.book2D("FedChNErr", "Total number of errors", 40,
                                  -0.5, 39.5, 37, 0., 37.);
          else {
            string fullpathname = iBooker.pwd() + "/" + (*iv);
            temp = iGetter.get(fullpathname);
        ...
        if ((*iv) == "FedETypeNErr") {
          if (!isBooked) {
            temp = iBooker.book2D("FedETypeNErr", "Number of each error type",
                                  40, -0.5, 39.5, 21, 0., 21.);
            ...
            if (me) {
              if (me->getMean() > 0.) {
                if (sname.find("_NErrors_") != string::npos) {
                  ...
                  if (notReset)
                    (*isum)->setBinContent(ndet, (*isum)->getBinContent(ndet) +
                                                     me1->getEntries());
                } else
                  (*isum)->setBinContent(ndet, (*isum)->getBinContent(ndet) +
                                                   me->getEntries());
...
//=============================================================================================================
void SiPixelActionExecutor::fillGrandBarrelSummaryHistos(
    DQMStore::IBooker &iBooker, DQMStore::IGetter &iGetter,
    vector<string> &me_names, bool isUpgrade) {
  //  cout<<"Entering
  ...
  if ((dir_name.find("DQMData") == 0) || (dir_name.find("Pixel") == 0) ||
      ...
      (dir_name.find("Endcap") == 0) || (dir_name.find("HalfCylinder") == 0) ||
      (dir_name.find("Disk") == 0) || (dir_name.find("Blade") == 0) ||
    ...
    for (vector<string>::const_iterator im = contents.begin();
      ...
      for (vector<string>::const_iterator iv = me_names.begin();
        ...
        if ((*im).find(var) != string::npos) {
          if ((var == "_charge_" || var == "_nclusters_" || var == "_size_" ||
               var == "_sizeX_" || var == "_sizeY_") &&
          ...
          if (!me)
            continue;
          if (source_type_ == 5 || source_type_ == 6) {
            if ((*iv) == "errorType" || (*iv) == "NErrors" ||
                (*iv) == "fullType" || (*iv) == "chanNmbr" ||
                (*iv) == "TBMType" || (*iv) == "EvtNbr" || (*iv) == "evtSize" ||
                (*iv) == "linkId" || (*iv) == "ROCId" || (*iv) == "DCOLId" ||
                (*iv) == "PXId" || (*iv) == "ROCNmbr" ||
                (*iv) == "TBMMessage" || (*iv) == "Type36Hitmap")
              prefix = "SUMRAW";
            else if ((*iv) == "ndigis" || (*iv) == "adc" ||
              ...
              prefix = "SUMDIG";
                     ...
                     (*iv) == "charge" || (*iv) == "chargeCOMB" ||
                     (*iv) == "size" || (*iv) == "sizeX" || (*iv) == "sizeY" ||
                     (*iv) == "minrow" || (*iv) == "maxrow" ||
                     (*iv) == "mincol" || (*iv) == "maxcol")
              prefix = "SUMCLU";
            if (currDir.find("Track") != string::npos)
              ...
              prefix = "SUMTRK";
                     ...
                     (*iv) == "nRecHits" || (*iv) == "ErrorX" ||
                     (*iv) == "ErrorY")
              prefix = "SUMHIT";
            else if ((*iv) == "Gain1d_mean" || (*iv) == "GainChi2NDF1d_mean" ||
                     (*iv) == "GainChi2Prob1d_mean" ||
                     (*iv) == "Pedestal1d_mean" ||
                     (*iv) == "ScurveChi2ProbSummary_mean" ||
                     (*iv) == "ScurveFitResultSummary_mean" ||
                     (*iv) == "ScurveSigmasSummary_mean" ||
                     (*iv) == "ScurveThresholdSummary_mean" ||
                     (*iv) == "Gain1d_RMS" || (*iv) == "GainChi2NDF1d_RMS" ||
                     (*iv) == "GainChi2Prob1d_RMS" ||
                     (*iv) == "Pedestal1d_RMS" ||
                     (*iv) == "GainNPoints1d_mean" ||
                     (*iv) == "GainNPoints1d_RMS" ||
                     (*iv) == "GainHighPoint1d_mean" ||
                     (*iv) == "GainHighPoint1d_RMS" ||
                     (*iv) == "GainLowPoint1d_mean" ||
                     (*iv) == "GainLowPoint1d_RMS" ||
                     (*iv) == "GainEndPoint1d_mean" ||
                     (*iv) == "GainEndPoint1d_RMS" ||
                     (*iv) == "GainFitResult2d_mean" ||
                     (*iv) == "GainFitResult2d_RMS" ||
                     (*iv) == "GainDynamicRange2d_mean" ||
                     (*iv) == "GainDynamicRange2d_RMS" ||
                     (*iv) == "GainSaturate2d_mean" ||
                     (*iv) == "GainSaturate2d_RMS" ||
                     (*iv) == "ScurveChi2ProbSummary_RMS" ||
                     (*iv) == "ScurveFitResultSummary_RMS" ||
                     (*iv) == "ScurveSigmasSummary_RMS" ||
                     (*iv) == "ScurveThresholdSummary_RMS" ||
                     (*iv) == "pixelAliveSummary_mean" ||
                     (*iv) == "pixelAliveSummary_FracOfPerfectPix" ||
          ...
          if (first_subdir && !isUpgrade) {
            ...
            if ((*iv) == "adcCOMB" || (*iv) == "chargeCOMB")
              ...
              nbin = 768;
                     ...
                     dir_name.find("Shell") != string::npos)
              nbin = 48;
          ...
          for (vector<MonitorElement *>::const_iterator igm = gsum_mes.begin();
            ...
            if ((*igm)->getName().find(var) != string::npos) {
              ...
              string title = "";
              if ((*igm)->getName().find("NErrors_") != string::npos &&
                  prefix == "SUMOFF")
                title = "Total number of errors per Ladder";
              else if ((*igm)->getName().find("NErrors_") != string::npos &&
                       prefix == "SUMRAW")
                title = "Total number of errors per Module";
              else if (prefix == "SUMOFF")
              ...
              // Setting binning
              if (!isUpgrade) {
                if ((*igm)->getName().find("ALLMODS_adcCOMB_") !=
                    string::npos) {
                  nbin_subdir = 128;
                } else if ((*igm)->getName().find("ALLMODS_chargeCOMB_") !=
                           string::npos) {
                  nbin_subdir = 100;
                ...
                if (((*igm)->getName().find("adcCOMB") != string::npos &&
                  ...
                  (*igm)->getTH1F()->Add(me->getTH1F());
                           ...
                           ((*igm)->getName().find("size_") != string::npos &&
                            ...
                            me->getName().find("Track_") == string::npos) ||
                           ((*igm)->getName().find("charge_OffTrack_") !=
                                string::npos &&
                            me->getName().find("charge_OffTrack_") !=
                                string::npos) ||
                           ((*igm)->getName().find("nclusters_OffTrack_") !=
                                string::npos &&
                            me->getName().find("nclusters_OffTrack_") !=
                                string::npos) ||
                           ((*igm)->getName().find("size_OffTrack_") !=
                                string::npos &&
                            me->getName().find("size_OffTrack_") !=
                                string::npos) ||
                           ((*igm)->getName().find("charge_OnTrack_") !=
                                string::npos &&
                            me->getName().find("charge_OnTrack_") !=
                                string::npos) ||
                           ((*igm)->getName().find("nclusters_OnTrack_") !=
                                string::npos &&
                            me->getName().find("nclusters_OnTrack_") !=
                                string::npos) ||
                           ((*igm)->getName().find("size_OnTrack_") !=
                                string::npos &&
                            me->getName().find("size_OnTrack_") !=
                                string::npos) ||
...
//=============================================================================================================
void SiPixelActionExecutor::fillGrandEndcapSummaryHistos(
    DQMStore::IBooker &iBooker, DQMStore::IGetter &iGetter,
    vector<string> &me_names, bool isUpgrade) {
  // printing cout<<"Entering
  ...
  if ((dir_name.find("DQMData") == 0) || (dir_name.find("Pixel") == 0) ||
      (dir_name.find("AdditionalPixelErrors") == 0) ||
      (dir_name.find("Barrel") == 0) || (dir_name.find("Shell") == 0) ||
      (dir_name.find("Layer") == 0) || (dir_name.find("Ladder") == 0))
    return;
  vector<string> subdirs = iGetter.getSubdirs();
  int iDir = 0;
  int nbin = 0;
          ...
          if (first_subdir && !isUpgrade) {
            ...
            if ((*iv) == "adcCOMB" || (*iv) == "chargeCOMB")
              ...
              nbin = 672;
                     ...
                     dir_name.find("HalfCylinder") != string::npos)
              nbin = 24;
            else if (dir_name.find("HalfCylinder") != string::npos)
              nbin = 168;
            else if (prefix == "SUMOFF" &&
                     dir_name.find("Disk") != string::npos)
              nbin = 12;
            ...
            if ((*igm)->getName().find(var) != string::npos) {
              ...
              if (!isUpgrade) {
                if ((*igm)->getName().find("ALLMODS_adcCOMB_") !=
                  ...
                  nbin_i = ((cnt - 1) % 12) * 7;
                  nbin_subdir = 7;
                } else if ((*igm)->getName().find("HalfCylinder") !=
                           string::npos) {
                  if (prefix != "SUMOFF") {
                    nbin_subdir = 84;
                    if ((*im).find("_2") != string::npos)
...
void SiPixelActionExecutor::getGrandSummaryME(DQMStore::IBooker &iBooker,
  ...
  if (me_name.find("ALLMODS_adcCOMB_") != string::npos)
    ...
    temp_me =
        iBooker.book1D(me_name.c_str(), me_name.c_str(), nbin, 1., nbin + 1.);
  if (temp_me)
...
//=============================================================================================================
MonitorElement *SiPixelActionExecutor::getFEDSummaryME(
    DQMStore::IBooker &iBooker, DQMStore::IGetter &iGetter, string me_name) {
                                               ...
                                               bool isbarrel) // Polymorphism
{
  ...
  for (vector<string>::const_iterator it = subdirs.begin(); it != subdirs.end();
    ...
    if (!hiRes) {
      ...
      OccupancyMap = iBooker.book2D(
          (isbarrel ? "barrelOccupancyMap" : "endcapOccupancyMap"),
          "Barrel Digi Occupancy Map (4 pix per bin)", isbarrel ? 208 : 130, 0.,
          isbarrel ? 416. : 260., 80, 0., 160.);
    } else {
      // occupancyprinting cout<<"booking high res barrel occ plot now!"<<endl;
      OccupancyMap = iBooker.book2D(
          (isbarrel ? "barrelOccupancyMap" : "endcapOccupancyMap"),
          "Barrel Digi Occupancy Map (1 pix per bin)", isbarrel ? 416 : 260, 0.,
          isbarrel ? 416. : 260., 160, 0., 160.);
    }
...
void SiPixelActionExecutor::fillOccupancy(DQMStore::IBooker &iBooker,
  ...
  if (dname.find("Layer_") != string::npos ||
      dname.find("Disk_") != string::npos) {
    vector<string> meVec = iGetter.getMEs();
    for (vector<string>::const_iterator it = meVec.begin(); it != meVec.end();
         it++) {
      string full_path = currDir + "/" + (*it);
      if (full_path.find("hitmap_siPixelDigis") !=
          string::npos) { // If we have the hitmap ME
        MonitorElement *me = iGetter.get(full_path);
        if (!me)
          continue;
        ...
        while (path.find_last_of("/") != 5) // Stop before Pixel/
        {
          ...
          // path
          //<< endl;
          OccupancyMap = iGetter.get(
              path + "/" + (isbarrel ? "barrel" : "endcap") + "OccupancyMap");

          if (OccupancyMap) {
            for (int i = 1; i != me->getNbinsX() + 1; i++)
              for (int j = 1; j != me->getNbinsY() + 1; j++) {
                float previous = OccupancyMap->getBinContent(i, j);
                OccupancyMap->setBinContent(i, j,
                                            previous + me->getBinContent(i, j));
              }
            OccupancyMap->getTH2F()->SetEntries(
                OccupancyMap->getTH2F()->Integral());
          }
        }
      }
    }
    ...
    for (vector<string>::const_iterator it = subdirs.begin();
      ...
      iBooker.cd(*it);
      if (*it != "Pixel" &&
          ((isbarrel && (*it).find("Barrel") == string::npos) ||
           (!isbarrel && (*it).find("Endcap") == string::npos)))
        continue;
...
void SiPixelActionExecutor::normaliseAvDigiOcc(DQMStore::IBooker &iBooker,
  ...
  for (int i = 1; i != 41; i++) {
    if (i < 33)
      roccupancyPlot->setBinContent(i, roccupancyPlot->getBinContent(i) /
                                           averageBPIXOcc);
    else
      roccupancyPlot->setBinContent(i, roccupancyPlot->getBinContent(i) /
                                           averageFPIXOcc);
...
void SiPixelActionExecutor::normaliseAvDigiOccVsLumi(DQMStore::IBooker &iBooker,
  ...
  MonitorElement *avgfedDigiOccvsLumi =
      iGetter.get("Pixel/avgfedDigiOccvsLumi");
  ...
  for (int i = 1; i != 41; i++) {
    if (i < 33)
      avgfedDigiOccvsLumi->setBinContent(
          lumisec, i,
          avgfedDigiOccvsLumi->getBinContent(lumisec, i) / averageBPIXOcc);
    else
      avgfedDigiOccvsLumi->setBinContent(
          lumisec, i,
          avgfedDigiOccvsLumi->getBinContent(lumisec, i) / averageFPIXOcc);
  }
  ...
  if (!isUpgrade) {
    if (Tier0Flag_) {
      HitEfficiency_L1 = iBooker.book2D(
          "HitEfficiency_L1", "Hit Efficiency in Barrel_Layer1;Module;Ladder",
          9, -4.5, 4.5, 21, -10.5, 10.5);
      HitEfficiency_L2 = iBooker.book2D(
          "HitEfficiency_L2", "Hit Efficiency in Barrel_Layer2;Module;Ladder",
          9, -4.5, 4.5, 33, -16.5, 16.5);
      HitEfficiency_L3 = iBooker.book2D(
          "HitEfficiency_L3", "Hit Efficiency in Barrel_Layer3;Module;Ladder",
    ...
    if (Tier0Flag_) {
      HitEfficiency_L1 = iBooker.book2D(
          "HitEfficiency_L1", "Hit Efficiency in Barrel_Layer1;z-side;Ladder",
          2, -1., 1., 12, -6., 6.);
      HitEfficiency_L2 = iBooker.book2D(
          "HitEfficiency_L2", "Hit Efficiency in Barrel_Layer2;z-side;Ladder",
          2, -1., 1., 28, -14., 14.);
      HitEfficiency_L3 = iBooker.book2D(
          "HitEfficiency_L3", "Hit Efficiency in Barrel_Layer3;z-side;Ladder",
          2, -1., 1., 44, -22., 22.);
      HitEfficiency_L4 = iBooker.book2D(
          "HitEfficiency_L4", "Hit Efficiency in Barrel_Layer4;z-side;Ladder",
          2, -1., 1., 64, -32., 32.);
    } else {
      HitEfficiency_L1 = iBooker.book2D(
          "HitEfficiency_L1", "Hit Efficiency in Barrel_Layer1;Module;Ladder",
      ...
      HitEfficiency_L3 = iBooker.book2D(
          ...
          8, -4., 4., 44, -22., 22.);
      HitEfficiency_L4 = iBooker.book2D(
          "HitEfficiency_L4", "Hit Efficiency in Barrel_Layer4;Module;Ladder",
          8, -4., 4., 64, -32., 32.);
    }
  } // endifUpgrade
  // Endcap
  ...
  if (!isUpgrade) {
    if (Tier0Flag_) {
      HitEfficiency_Dp1 = iBooker.book2D(
          "HitEfficiency_Dp1", "Hit Efficiency in Endcap_Disk_p1;Blade;Panel",
          26, -13., 13., 2, 0.5, 2.5);
      HitEfficiency_Dp2 = iBooker.book2D(
          "HitEfficiency_Dp2", "Hit Efficiency in Endcap_Disk_p2;Blade;Panel",
          26, -13., 13., 2, 0.5, 2.5);
      HitEfficiency_Dm1 = iBooker.book2D(
          "HitEfficiency_Dm1", "Hit Efficiency in Endcap_Disk_m1;Blade;Panel",
          26, -13., 13., 2, 0.5, 2.5);
      HitEfficiency_Dm2 = iBooker.book2D(
          "HitEfficiency_Dm2", "Hit Efficiency in Endcap_Disk_m2;;Blade;Panel",
          26, -13., 13., 2, 0.5, 2.5);
    } else {
      HitEfficiency_Dp1 =
          iBooker.book2D("HitEfficiency_Dp1",
                         "Hit Efficiency in Endcap_Disk_p1;Blades;Modules", 24,
                         -12., 12., 7, 1., 8.);
      HitEfficiency_Dp2 =
          iBooker.book2D("HitEfficiency_Dp2",
                         "Hit Efficiency in Endcap_Disk_p2;Blades;Modules", 24,
                         -12., 12., 7, 1., 8.);
      HitEfficiency_Dm1 =
          iBooker.book2D("HitEfficiency_Dm1",
                         "Hit Efficiency in Endcap_Disk_m1;Blades;Modules", 24,
                         -12., 12., 7, 1., 8.);
      HitEfficiency_Dm2 =
          iBooker.book2D("HitEfficiency_Dm2",
                         "Hit Efficiency in Endcap_Disk_m2;Blades;Modules", 24,
                         -12., 12., 7, 1., 8.);
    }
  } else if (isUpgrade) {
    if (Tier0Flag_) {
      HitEfficiency_Dp1 = iBooker.book2D(
          "HitEfficiency_Dp1", "Hit Efficiency in Endcap_Disk_p1;Blades;", 28,
          -17., 11., 1, 0., 1.);
      HitEfficiency_Dp2 = iBooker.book2D(
          "HitEfficiency_Dp2", "Hit Efficiency in Endcap_Disk_p2;Blades;", 28,
          -17., 11., 1, 0., 1.);
      HitEfficiency_Dp3 = iBooker.book2D(
          "HitEfficiency_Dp3", "Hit Efficiency in Endcap_Disk_p3;Blades;", 28,
          -17., 11., 1, 0., 1.);
      HitEfficiency_Dm1 = iBooker.book2D(
          "HitEfficiency_Dm1", "Hit Efficiency in Endcap_Disk_m1;Blades;", 28,
          -17., 11., 1, 0., 1.);
      HitEfficiency_Dm2 = iBooker.book2D(
          "HitEfficiency_Dm2", "Hit Efficiency in Endcap_Disk_m2;Blades;", 28,
          -17., 11., 1, 0., 1.);
      HitEfficiency_Dm3 = iBooker.book2D(
          "HitEfficiency_Dm3", "Hit Efficiency in Endcap_Disk_m3;Blades;", 28,
          -17., 11., 1, 0., 1.);
    } else {
      HitEfficiency_Dp1 =
          iBooker.book2D("HitEfficiency_Dp1",
                         "Hit Efficiency in Endcap_Disk_p1;Blades;Modules", 28,
                         -17., 11., 2, 1., 3.);
      HitEfficiency_Dp2 =
          iBooker.book2D("HitEfficiency_Dp2",
                         "Hit Efficiency in Endcap_Disk_p2;Blades;Modules", 28,
                         -17., 11., 2, 1., 3.);
      HitEfficiency_Dp3 =
          iBooker.book2D("HitEfficiency_Dp3",
                         "Hit Efficiency in Endcap_Disk_p3;Blades;Modules", 28,
                         -17., 11., 2, 1., 3.);
      HitEfficiency_Dm1 =
          iBooker.book2D("HitEfficiency_Dm1",
                         "Hit Efficiency in Endcap_Disk_m1;Blades;Modules", 28,
                         -17., 11., 2, 1., 3.);
      HitEfficiency_Dm2 =
          iBooker.book2D("HitEfficiency_Dm2",
                         "Hit Efficiency in Endcap_Disk_m2;Blades;Modules", 28,
                         -17., 11., 2, 1., 3.);
      HitEfficiency_Dm3 =
          iBooker.book2D("HitEfficiency_Dm3",
                         "Hit Efficiency in Endcap_Disk_m3;Blades;Modules", 28,
                         -17., 11., 2, 1., 3.);
    }
  } // endif(isUpgrade)
  iBooker.cd();
  iBooker.cd("Pixel/");
  string bins[] = {"Layer1", "Layer2", "Layer3", "Disk1+",
                   "Disk2+", "Disk1-", "Disk2-"};
  HitEfficiencySummary = iBooker.book1D(
      "HitEfficiencySummary", "Hit efficiency per sub detector", 7, 0, 7);
    ...
    if (isbarrel && dname.find("Ladder_") != string::npos) {
      if (!isUpgrade) {
        ...
        for (vector<string>::const_iterator it = meVec.begin();
          ...
          if (full_path.find("missingMod_") !=
              string::npos) { // If we have missing hits ME
            ...
            if (!missing)
              continue;
            string new_path =
                full_path.replace(full_path.find("missing"), 7, "valid");
            ...
            if (currDir.find("Shell_mO") != string::npos ||
                currDir.find("Shell_pO") != string::npos) {
              biny = -biny;
            }
            const int nMod = 4;
            for (int i = 1; i < nMod + 1; i++) {
              ...
              if (currDir.find("Layer_1") != string::npos) {
                ...
                if (HitEfficiency_L1)
                  HitEfficiency_L1->setBinContent(
                      HitEfficiency_L1->getTH2F()->FindBin(binx, biny),
                      (float)hitEfficiency);
              } else if (currDir.find("Layer_2") != string::npos) {
                HitEfficiency_L2 = iGetter.get("Pixel/Barrel/HitEfficiency_L2");
                if (HitEfficiency_L2)
                  HitEfficiency_L2->setBinContent(
                      HitEfficiency_L2->getTH2F()->FindBin(binx, biny),
                      (float)hitEfficiency);
              } else if (currDir.find("Layer_3") != string::npos) {
                HitEfficiency_L3 = iGetter.get("Pixel/Barrel/HitEfficiency_L3");
                if (HitEfficiency_L3)
                  HitEfficiency_L3->setBinContent(
                      HitEfficiency_L3->getTH2F()->FindBin(binx, biny),
                      (float)hitEfficiency);
              }
            }
          }
        ...
        for (vector<string>::const_iterator it = meVec.begin();
          ...
          if (full_path.find("missing_") !=
            ...
            if (currDir.find("Layer_1") != string::npos) {
              HitEfficiency_L1 = iGetter.get("Pixel/Barrel/HitEfficiency_L1");
              if (HitEfficiency_L1)
                HitEfficiency_L1->setBinContent(binx, biny,
                                                (float)hitEfficiency);
            } else if (currDir.find("Layer_2") != string::npos) {
              HitEfficiency_L2 = iGetter.get("Pixel/Barrel/HitEfficiency_L2");
              if (HitEfficiency_L2)
                HitEfficiency_L2->setBinContent(binx, biny,
                                                (float)hitEfficiency);
            } else if (currDir.find("Layer_3") != string::npos) {
              HitEfficiency_L3 = iGetter.get("Pixel/Barrel/HitEfficiency_L3");
              if (HitEfficiency_L3)
                HitEfficiency_L3->setBinContent(binx, biny,
                                                (float)hitEfficiency);
      ...
      for (vector<string>::const_iterator it = meVec.begin(); it != meVec.end();
        ...
        if (full_path.find("missing_") !=
          ...
          int biny = 0;
          binx = getBlade(dname);
          if (currDir.find("HalfCylinder_mI") != string::npos ||
              currDir.find("HalfCylinder_pI") != string::npos) {
            binx = binx + 14;
          } else {
            binx = 13 - binx;
          }
          ...
          for (int i = 1; i < nPanel + 1; i++) {
            ...
            if (validHits + missingHits > 0.)
              hitEfficiency = validHits / (validHits + missingHits);
            if (currDir.find("Disk_1") != string::npos &&
                currDir.find("HalfCylinder_m") != string::npos) {
              HitEfficiency_Dm1 = iGetter.get("Pixel/Endcap/HitEfficiency_Dm1");
              if (HitEfficiency_Dm1)
                HitEfficiency_Dm1->setBinContent(binx, biny,
                                                 (float)hitEfficiency);
            } else if (currDir.find("Disk_2") != string::npos &&
                       currDir.find("HalfCylinder_m") != string::npos) {
              HitEfficiency_Dm2 = iGetter.get("Pixel/Endcap/HitEfficiency_Dm2");
              if (HitEfficiency_Dm2)
                HitEfficiency_Dm2->setBinContent(binx, biny,
                                                 (float)hitEfficiency);
            } else if (currDir.find("Disk_1") != string::npos &&
                       currDir.find("HalfCylinder_p") != string::npos) {
              ...
              if (HitEfficiency_Dp2)
                HitEfficiency_Dp2->setBinContent(binx, biny,
                                                 (float)hitEfficiency);
            }
          } // EndOfFor
        }
        ...
        if (full_path.find("missing_") !=
          ...
          if (currDir.find("Disk_1") != string::npos &&
            ...
            if (HitEfficiency_Dm2)
              HitEfficiency_Dm2->setBinContent(binx, biny,
                                               (float)hitEfficiency);
          } else if (currDir.find("Disk_3") != string::npos &&
                     currDir.find("HalfCylinder_m") != string::npos) {
            HitEfficiency_Dm3 = iGetter.get("Pixel/Endcap/HitEfficiency_Dm3");
            if (HitEfficiency_Dm3)
              HitEfficiency_Dm3->setBinContent(binx, biny,
                                               (float)hitEfficiency);
          } else if (currDir.find("Disk_1") != string::npos &&
                     currDir.find("HalfCylinder_p") != string::npos) {
            HitEfficiency_Dp1 = iGetter.get("Pixel/Endcap/HitEfficiency_Dp1");
            ...
            if (HitEfficiency_Dp3)
              HitEfficiency_Dp3->setBinContent(binx, biny,
                                               (float)hitEfficiency);
          }
        ...
        if (full_path.find("missing_") !=
          ...
          if (isbarrel) {
            ...
            if (!isUpgrade) {
              ...
              if (currDir.find("HalfCylinder_mO") != string::npos ||
                  currDir.find("HalfCylinder_pO") != string::npos) {
                binx = binx + 12;
              }
...
void SiPixelActionExecutor::fillEfficiencySummary(DQMStore::IBooker &iBooker,
  ...
  string hitEfficiencyPostfix[] = {"L1",  "L2",  "L3", "Dp1",
                                   "Dp2", "Dm1", "Dm2"};
  std::vector<std::vector<float>> ignoreXBins = {
      {-4, 2},    {4, 4, -1, -3, 3, -4, -3, -2, -1, -4, -3, -2, -1, 1, -4},
      {1, -4, 1}, {},
      {},         {},
      {}};
  std::vector<std::vector<float>> ignoreYBins = {
      {-9, -3},   {1, 16, 1, -13, -13, -5, -5, -5, -5, -6, -6, -6, -6, -8, -8},
      {3, 14, 6}, {},
      {},         {},
      {}};
  ...
  for (int i = 0; i < 7; i++) {
    ...
    if (i < 3)
      subdetName = "Barrel/";
    char meName[50];
    sprintf(meName, "Pixel/%sHitEfficiency_%s", subdetName.c_str(),
            hitEfficiencyPostfix[i].c_str());
