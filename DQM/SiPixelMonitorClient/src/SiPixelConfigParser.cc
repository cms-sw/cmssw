...
bool SiPixelConfigParser::getMENamesForTrackerMap(string &tkmap_name,
  ...
  me_names.clear();
  unsigned int tkMapNodes =
      doc()->getElementsByTagName(qtxml::_toDOMS("TkMap"))->getLength();
  if (tkMapNodes != 1) {
    cout << ACYellow << ACBold
         << "[SiPixelConfigParser::getMENamesForTrackerMap()]" << ACRed
         << ACBold << "No TkMap tag found in configuration file" << ACPlain
         << endl;
    return false;
  }
  /// Get Node
  DOMNode *tkMapNode =
      doc()->getElementsByTagName(qtxml::_toDOMS("TkMap"))->item(0);
  // Get QTEST name
  if (!tkMapNode) {
    cout << ACYellow << ACBold
         << "[SiPixelConfigParser::getMENamesForTrackerMap()]" << ACRed
         << ACBold << " No TkMap tag elements found in configuration file"
         << ACPlain << endl;
    return false;
  }
  ...
  if (!tkMapElement) {
    cout << ACYellow << ACBold
         << "[SiPixelConfigParser::getMENamesForTrackerMap()]" << ACRed
         << ACBold << " No TkMap tag dom elements found in configuration file"
         << ACPlain << endl;
    return false;
  }
  tkmap_name =
      qtxml::_toString(tkMapElement->getAttribute(qtxml::_toDOMS("name")));
  ...
  if (meList->getLength() == 0) {
    cout << ACYellow << ACBold
         << "[SiPixelConfigParser::getMENamesForTrackerMap()]" << ACRed
         << ACBold << " No MonElement found in configuration file" << ACPlain
         << endl;
  }
  for (unsigned int k = 0; k < meList->getLength(); k++) {
    ...
    if (!meNode) {
      cout << ACYellow << ACBold
           << "[SiPixelConfigParser::getMENamesForTrackerMap()]" << ACRed
           << ACBold << " No MonElement item found in configuration file"
           << ACPlain << endl;
      return false;
    }
...
bool SiPixelConfigParser::getFrequencyForTrackerMap(int &u_freq) {
  ...
  u_freq = atoi(qtxml::_toString(tkMapElement->getAttribute(
                                     qtxml::_toDOMS("update_frequency")))
                    .c_str());
  return true;
}
//
...
bool SiPixelConfigParser::getMENamesForTree(string &structure_name,
  ...
  unsigned int structureNodes =
      doc()
          ->getElementsByTagName(qtxml::_toDOMS("SubStructureLevel"))
          ->getLength();
  if (structureNodes == 0)
    return false;
  /// Get Node
  DOMNode *structureNode =
      doc()->getElementsByTagName(qtxml::_toDOMS("SubStructureLevel"))->item(0);
  // Get QTEST name
  if (!structureNode)
    return false;
  ...
  structure_name =
      qtxml::_toString(structureElement->getAttribute(qtxml::_toDOMS("name")));
...
bool SiPixelConfigParser::getMENamesForBarrelSummary(string &structure_name,
  ...
  unsigned int structureNodes =
      doc()
          ->getElementsByTagName(qtxml::_toDOMS("SubStructureBarrelLevel"))
          ->getLength();
  if (structureNodes == 0)
    return false;
  /// Get Node
...
bool SiPixelConfigParser::getMENamesForEndcapSummary(string &structure_name,
  ...
  unsigned int structureNodes =
      doc()
          ->getElementsByTagName(qtxml::_toDOMS("SubStructureEndcapLevel"))
          ->getLength();
  if (structureNodes == 0)
    return false;
  /// Get Node
...
bool SiPixelConfigParser::getMENamesForFEDErrorSummary(
    string &structure_name, vector<string> &me_names) {
  // cout<<"Entering
  ...
  unsigned int structureNodes =
      doc()
          ->getElementsByTagName(qtxml::_toDOMS("SubStructureNonDetId"))
          ->getLength();
  if (structureNodes == 0)
    return false;
  /// Get Node
...
bool SiPixelConfigParser::getFrequencyForBarrelSummary(int &u_freq) {
  ...
  u_freq = atoi(qtxml::_toString(structureElement->getAttribute(
                                     qtxml::_toDOMS("update_frequency")))
                    .c_str());
  return true;
}
...
bool SiPixelConfigParser::getMENamesForGrandBarrelSummary(
    string &structure_name, vector<string> &me_names) {
  // cout<<"Entering
  ...
  unsigned int structureNodes =
      doc()
          ->getElementsByTagName(qtxml::_toDOMS("SubStructureGrandBarrelLevel"))
          ->getLength();
  if (structureNodes == 0)
    return false;
  /// Get Node
...
bool SiPixelConfigParser::getMENamesForGrandEndcapSummary(
    string &structure_name, vector<string> &me_names) {
  // cout<<"Entering
  ...
  unsigned int structureNodes =
      doc()
          ->getElementsByTagName(qtxml::_toDOMS("SubStructureGrandEndcapLevel"))
          ->getLength();
  if (structureNodes == 0)
    return false;
  /// Get Node
...
bool SiPixelConfigParser::getMessageLimitForQTests(int &u_freq) {
  ...
  unsigned int structureNodes =
      doc()
          ->getElementsByTagName(qtxml::_toDOMS("QTestMessageLimit"))
          ->getLength();
  if (structureNodes == 0)
    return false;
  /// Get Node
  DOMNode *structureNode =
      doc()->getElementsByTagName(qtxml::_toDOMS("QTestMessageLimit"))->item(0);
  // Get Node name
  if (!structureNode)
    return false;
  DOMElement *structureElement = static_cast<DOMElement *>(structureNode);
  ...
  u_freq = atoi(
      qtxml::_toString(structureElement->getAttribute(qtxml::_toDOMS("value")))
          .c_str());
  return true;
}
...
bool SiPixelConfigParser::getSourceType(int &u_freq) {
  ...
  unsigned int structureNodes =
      doc()->getElementsByTagName(qtxml::_toDOMS("SourceType"))->getLength();
  if (structureNodes == 0)
    return false;
  /// Get Node
  DOMNode *structureNode =
      doc()->getElementsByTagName(qtxml::_toDOMS("SourceType"))->item(0);
  // Get Node name
  if (!structureNode)
    return false;
  DOMElement *structureElement = static_cast<DOMElement *>(structureNode);
  ...
  u_freq = atoi(
      qtxml::_toString(structureElement->getAttribute(qtxml::_toDOMS("code")))
          .c_str());
  return true;
}
...
bool SiPixelConfigParser::getCalibType(int &u_freq) {
  ...
  unsigned int structureNodes =
      doc()->getElementsByTagName(qtxml::_toDOMS("CalibType"))->getLength();
  if (structureNodes == 0)
    return false;
  /// Get Node
  DOMNode *structureNode =
      doc()->getElementsByTagName(qtxml::_toDOMS("CalibType"))->item(0);
  // Get Node name
  if (!structureNode)
    return false;
  DOMElement *structureElement = static_cast<DOMElement *>(structureNode);
