...
SiPixelCertification::SiPixelCertification(const edm::ParameterSet &ps) {
  edm::LogInfo("SiPixelCertification")
...
void SiPixelCertification::dqmEndLuminosityBlock(
  ...
  // value:
  float dcsFrac =
      (iGetter.getElement("Pixel/EventInfo/DCSSummary"))->getFloatValue();
  float daqFrac =
      (iGetter.getElement("Pixel/EventInfo/DAQSummary"))->getFloatValue();
  float dqmFrac =
      (iGetter.getElement("Pixel/EventInfo/reportSummary"))->getFloatValue();
  float pixel_all = std::min(dcsFrac, daqFrac);
  pixel_all = std::min(pixel_all, dqmFrac);
  // std::cout<<"Pixel numbers: "<<dcsFrac<<" , "<<daqFrac<<" , "<<dqmFrac<<" ,
  // "<<pixel_all<<std::endl;
  CertificationPixel =
      iGetter.getElement("Pixel/EventInfo/CertificationSummary");
  if (CertificationPixel)
    CertificationPixel->Fill(pixel_all);

  dcsFrac =
      (iGetter.getElement("Pixel/EventInfo/DCSContents/PixelBarrelFraction"))
          ->getFloatValue();
  daqFrac =
      (iGetter.getElement("Pixel/EventInfo/DAQContents/PixelBarrelFraction"))
          ->getFloatValue();
  dqmFrac = (iGetter.getElement(
  ...
  CertificationBarrel = iGetter.getElement(
      "Pixel/EventInfo/CertificationContents/PixelBarrelFraction");
  if (CertificationBarrel)
    CertificationBarrel->Fill(pixel_barrel);

  dcsFrac =
      (iGetter.getElement("Pixel/EventInfo/DCSContents/PixelEndcapFraction"))
          ->getFloatValue();
  daqFrac =
      (iGetter.getElement("Pixel/EventInfo/DAQContents/PixelEndcapFraction"))
          ->getFloatValue();
  dqmFrac = (iGetter.getElement(
  ...
  CertificationEndcap = iGetter.getElement(
