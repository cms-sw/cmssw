...
ZeeCandidateFilter::ZeeCandidateFilter(const edm::ParameterSet &iConfig) {
  ...
  pfMetCollectionToken_ =
      consumes<pat::METCollection>(iConfig.getUntrackedParameter<edm::InputTag>("pfMetCollectionTag"));
  tcMetCollectionToken_ =
      consumes<pat::METCollection>(iConfig.getUntrackedParameter<edm::InputTag>("tcMetCollectionTag"));

  PrimaryVerticesCollectionToken_ =
      consumes<std::vector<reco::Vertex> >(iConfig.getUntrackedParameter<edm::InputTag>("PrimaryVerticesCollection"));
  ...
  if (electronMatched2HLT_ && useTriggerInfo_) {
    std::cout << "ZeeCandidateFilter: At least one electron is required to match an HLT object with DR < "
              << electronMatched2HLT_DR_ << std::endl;
  } else {
    std::cout << "ZeeCandidateFilter: Electron Candidates NOT required to match HLT object " << std::endl;
  }

  if (useValidFirstPXBHit1_) {
    std::cout << "ZeeCandidateFilter: Electron Candidate #1 required to have a valid hit in 1st PXB layer "
              << std::endl;
  }

  if (useValidFirstPXBHit2_) {
    std::cout << "ZeeCandidateFilter: Electron Candidate #2 required to have a valid hit in 1st PXB layer "
              << std::endl;
  }

  if (calculateValidFirstPXBHit1_) {
    std::cout << "ZeeCandidateFilter: Info about whether there is a valid 1st layer PXB hit for electron candidate #1 "
                 "will be stored: you can access that later by myElec.userInt(\"PassValidFirstPXBHit\")==1"
              << std::endl;
  }

  if (calculateValidFirstPXBHit2_) {
    std::cout << "ZeeCandidateFilter: Info about whether there is a valid 1st layer PXB hit for electron candidate #2 "
                 "will be stored: you can access that later by myElec.userInt(\"PassValidFirstPXBHit\")==1"
              << std::endl;
  }

  if (useExpectedMissingHits1_) {
    std::cout << "ZeeCandidateFilter: Electron Candidate #1 is required to have less than "
              << maxNumberOfExpectedMissingHits1_ << " expected hits missing " << std::endl;
  }

  if (useExpectedMissingHits2_) {
    std::cout << "ZeeCandidateFilter: Electron Candidate #2 is required to have less than "
              << maxNumberOfExpectedMissingHits2_ << " expected hits missing " << std::endl;
  }

  if (calculateExpectedMissingHits1_) {
    std::cout << "ZeeCandidateFilter: Missing Hits from expected inner layers for electron candidate #1 will be "
                 "calculated and stored: you can access them later by myElec.userInt(\"NumberOfExpectedMissingHits\")"
              << std::endl;
  }

  if (calculateExpectedMissingHits2_) {
    std::cout << "ZeeCandidateFilter: Missing Hits from expected inner layers for electron candidate #2 will be "
                 "calculated and stored: you can access them later by myElec.userInt(\"NumberOfExpectedMissingHits\")"
              << std::endl;
  }

  if (useConversionRejection1_) {
    std::cout << "ZeeCandidateFilter: Electron Candidate #1 is required to pass EGAMMA Conversion Rejection criteria"
              << std::endl;
  }

  if (useConversionRejection2_) {
    std::cout << "ZeeCandidateFilter: Electron Candidate #2 is required to pass EGAMMA Conversion Rejection criteria"
              << std::endl;
  }

  if (calculateConversionRejection1_) {
    std::cout << "ZeeCandidateFilter: EGAMMA Conversion Rejection criteria for electron candidate #1 will be "
                 "calculated and stored: you can access them later by demanding for a successful electron "
                 "myElec.userInt(\"PassConversionRejection\")==1"
              << std::endl;
  }

  if (calculateConversionRejection2_) {
    std::cout << "ZeeCandidateFilter: EGAMMA Conversion Rejection criteria for electron candidate #2 will be "
                 "calculated and stored: you can access them later by demanding for a successful electron "
                 "myElec.userInt(\"PassConversionRejection\")==1"
              << std::endl;
  }
  ...
  if (useSpikeRejection_) {
    std::cout << "ZeeCandidateFilter: Spike Cleaning will be done with the Swiss Cross Criterion cutting at "
              << spikeCleaningSwissCrossCut_ << std::endl;
  }
...
Bool_t ZeeCandidateFilter::filter(edm::Event &iEvent, const edm::EventSetup &iSetup) {
  ...
  std::cout << "FILTER-MSG: Begin Processing ... "
            ...
            << "Event = " << iEvent.eventAuxiliary().event() << " " << std::endl;
  ...
  if (HLTResults.isValid()) {
    ...
    std::cout << "SK_HLT_INFO"
              ...
              << "passTrigger = " << passTrigger << std::endl;
    ...
    if (useExtraTrigger_ && passTrigger == 0) {
      for (Int_t itrig = 0; itrig < (Int_t)vHltpathExtra_.size(); ++itrig) {
        ...
        std::cout << "SK_HLT_INFO"
                  ...
                  << "vHltpathExtra_.size() = " << vHltpathExtra_.size() << std::endl;
    ...
    std::cout << "TriggerResults are missing from this event.." << std::endl;
    if (useTriggerInfo_) {
  ...
  if (passTrigger == 0 && useTriggerInfo_) {
    std::cout << "No HLT Path is firing in this event" << std::endl;
  ...
  if (useExtraTrigger_) {
    for (Int_t itrig = 0; itrig < (Int_t)vHltpathFilterExtra_.size(); ++itrig) {
  ...
  if (!patElectron.isValid()) {
    std::cout << "No electrons found in this event with tag " << electronCollectionTag_ << std::endl;
  ...
  if (useValidFirstPXBHit1_ || calculateValidFirstPXBHit1_) {
    Bool_t fail =
        !maxETelec1.gsfTrack()->hitPattern().hasValidHitInPixelLayer(PixelSubdetector::SubDetector::PixelBarrel, 1);

    if (useValidFirstPXBHit1_ && fail) {
      std::cout << "Filter: there is no valid hit for electron #1 in 1st layer PXB" << std::endl;
      delete[] sorted;
      delete[] et;
      return false;
    }
  ...
  if (useValidFirstPXBHit2_ || calculateValidFirstPXBHit2_) {
    Bool_t fail =
        !maxETelec2.gsfTrack()->hitPattern().hasValidHitInPixelLayer(PixelSubdetector::SubDetector::PixelBarrel, 1);
  ...
  if (useExpectedMissingHits1_ || calculateExpectedMissingHits1_) {
    Int_t numberOfInnerHits =
        (Int_t)(maxETelec1.gsfTrack()->hitPattern().numberOfLostHits(reco::HitPattern::MISSING_INNER_HITS));
  ...
  if (useExpectedMissingHits2_ || calculateExpectedMissingHits2_) {
    Int_t numberOfInnerHits =
        (Int_t)(maxETelec2.gsfTrack()->hitPattern().numberOfLostHits(reco::HitPattern::MISSING_INNER_HITS));
  ...
  if (useConversionRejection1_ || calculateConversionRejection1_) {
    ...
    if (iEvent.getByToken(tracksToken_, ctfTracks)) {
      ...
      std::cout << "WARNING! Track Collection with input name: generalTracks was not found. Conversion Rejection for "
                   "electron #1 is not going to be applied!!!"
                << std::endl;
    }
  }
  ...
  if (useConversionRejection2_ || calculateConversionRejection2_) {
    ...
    if (iEvent.getByToken(tracksToken_, ctfTracks)) {
      ...
      std::cout << "WARNING! Track Collection with input name: generalTracks was not found. Conversion Rejection for "
                   "electron #2 is not going to be applied!!!"
                << std::endl;
    }
  }
    ...
    if (finalpathfound) {
      if (nF != filterInd) {
        ...
        for (Int_t iTrig = 0; iTrig < nK; ++iTrig) {
          ...
          Double_t dr_ele_HLT1 =
              reco::deltaR(maxETelec1.superCluster()->eta(), maxETelec1.superCluster()->phi(), TO.eta(), TO.phi());
          Double_t dr_ele_HLT2 =
              reco::deltaR(maxETelec2.superCluster()->eta(), maxETelec2.superCluster()->phi(), TO.eta(), TO.phi());
  ...
  if (HLTResults.isValid()) {
    ...
    HLTFilterType[0] =
        edm::InputTag("hltL1NonIsoHLTNonIsoSinglePhotonEt10HcalIsolFilter", "", process);  //HLT_Photon10_L1R
    HLTFilterType[1] =
        edm::InputTag("hltL1NonIsoHLTNonIsoSinglePhotonEt15HcalIsolFilter", "", process);  //HLT_Photon15_L1R
    HLTFilterType[2] =
        edm::InputTag("hltL1NonIsoHLTNonIsoSinglePhotonEt20HcalIsolFilter", "", process);  //HLT_Photon20_L1R
    HLTFilterType[3] =
        edm::InputTag("hltL1NonIsoSinglePhotonEt15HTITrackIsolFilter", "", process);  //HLT_Photon15_TrackIso_L1R
    HLTFilterType[4] =
        edm::InputTag("hltL1NonIsoSinglePhotonEt15LEIHcalIsolFilter", "", process);  //HLT_Photon15_LooseEcalIso_L1R
    HLTFilterType[5] =
        edm::InputTag("hltL1NonIsoHLTNonIsoSinglePhotonEt15EtFilterESet308E29", "", process);  //HLT_Photon30_L1R_8E29
    HLTFilterType[6] =
        edm::InputTag("hltL1NonIsoHLTNonIsoSinglePhotonEt15HcalIsolFilter", "", process);  //HLT_Photon30_L1R_8E29
  ...
  std::cout << "Run = " << iEvent.run() << " "
            ...
            << "FILTER-MSG: Event Accepted for Z Candidate" << std::endl;
