...
reco::SuperCluster fEtaScCorr(const reco::SuperCluster& sc) {
  ...
  if (ieta < p0)
    ...
    newE = sc.rawEnergy() / (1 + p1 * (ieta - p0) * (ieta - p0));
...
reco::SuperCluster fBremScCorr(const reco::SuperCluster& sc, const edm::ParameterSet& ps) {
  ...
  if (bremFrac < bremFrLowThr)
    bremFrac = bremFrLowThr;
  if (bremFrac < bremFrHighThr)
    bremFrac = bremFrHighThr;
...
reco::SuperCluster fEtEtaCorr(const reco::SuperCluster& sc, const edm::ParameterSet& ps) {
  ...
  fCorr = p0 + p1 * atan(fEtEtaParams[12] * (fEtEtaParams[13] - fabs(eta))) + fEtEtaParams[14] * fabs(eta) +
          p1 * fEtEtaParams[15] * fabs(eta) + p2 * fEtEtaParams[16] * eta * eta;
