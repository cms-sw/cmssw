#ifndef aod2patFilterWenu_H
#define aod2patFilterWenu_H
...
class aod2patFilterWenu : public edm::EDFilter {
   public:
      explicit aod2patFilterWenu(const edm::ParameterSet&);
      ~aod2patFilterWenu();
...
aod2patFilterWenu::aod2patFilterWenu(const edm::ParameterSet& iConfig)
{
...
aod2patFilterWenu::~aod2patFilterWenu()
{
...
bool
aod2patFilterWenu::filter(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  using namespace edm;
  using namespace std;
  using namespace pat;
  ...
  if (!gsfElectrons.isValid()) {
    std::cout <<"aod2patFilterWenu: Could not get electron collection with label: "
  ...
  if (! calomets.isValid()) {
    std::cout << "aod2patFilterWenu: Could not get met collection with label: "
...
void 
aod2patFilterWenu::beginJob(const edm::EventSetup&)
{
}

// ------------ method called once each job just after ending the event loop  -
void 
aod2patFilterWenu::endJob() {
}


//define this as a plug-in
DEFINE_FWK_MODULE(aod2patFilterWenu);
