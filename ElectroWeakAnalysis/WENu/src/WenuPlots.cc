...
WenuPlots::analyze(const edm::Event& iEvent, const edm::EventSetup& es)
{
  ...
  //
  ele_cr_mhitsinner= myElec->gsfTrack()->getHitPattern().numberOfHits(reco::HitPattern::MISSING_INNER_HITS);
  ...
  if (myElec->userInt("hasSecondElectron") == 1 && storeExtraInformation_) {
    ...
    if (storeAllSecondElectronVariables_) {
      ...
      ele2nd_cr_mhitsinner = mySecondElec->gsfTrack()->getHitPattern().numberOfHits(reco::HitPattern::MISSING_INNER_HITS);
  ...
  std::cout << "tracIso: " <<  trackIso << ", " << myElec->trackIso() << ", ecaliso: " << ecalIso
	    ...
	    << ", mishits: "
	    << myElec->gsfTrack()->getHitPattern().numberOfHits(reco::HitPattern::MISSING_INNER_HITS)
	    << std::endl;
