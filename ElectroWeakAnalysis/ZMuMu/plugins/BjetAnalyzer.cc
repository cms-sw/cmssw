...
void BjetAnalysis::analyze(const edm::Event& evt, const edm::EventSetup&) {
  ...
  for (unsigned int i = 0; i != bTags.size(); ++i) {
    cout << " Jet " << i << " has b tag discriminator (trackCountingHighEffBJetTags)= " << bTags[i].second
  ...
  for (unsigned int i = 0; i != bTags2.size(); ++i) {
    cout << " Jet " << i << " has b tag discriminator (jetProbabilityBJetTags) = " << bTags2[i].second
  ...
  for (unsigned int i = 0; i != bTags3.size(); ++i) {
    cout << " Jet " << i << " has b tag discriminator (jetBProbabilityBJetTags) = " << bTags3[i].second
