...
struct ZSelector {  // modify this selector in order to return an integer (0: no eta cut, 1: eta cut only, 2 eta && pt cut, 3: eta, pt and Mass cut, 4: mass cut on the denominator Z MC)
  ZSelector(double ptMin,
            ...
            double massMaxZMC)
      : ptMin_(ptMin),
        etaDau0Min_(etaDau0Min),
        etaDau0Max_(etaDau0Max),
        etaDau1Min_(etaDau1Min),
        etaDau1Max_(etaDau1Max),
        massMin_(massMin),
        massMax_(massMax),
        massMinZMC_(massMinZMC),
        massMaxZMC_(massMaxZMC) {}
  int operator()(const Candidate& c) const {
    //   std::cout << "c.numberOfDaughters(): " << c.numberOfDaughters()<< std::endl;
    if (c.numberOfDaughters() < 2)
      return 0;
    if (c.numberOfDaughters() >= 6)
      return 0;
    const Candidate* d0 = c.daughter(0);
    const Candidate* d1 = c.daughter(1);
    if (c.numberOfDaughters() > 2) {
      if (d0->numberOfDaughters() > 0)
        d0 = mcMuDaughter(d0);
      if (d1->numberOfDaughters() > 0)
        d1 = mcMuDaughter(d1);
    }
    int temp_cut = 0;
    /// allowing asymmetric cut....
    if ((fabs(d0->eta()) > etaDau0Min_ && fabs(d1->eta()) > etaDau1Min_ && fabs(d0->eta()) < etaDau0Max_ &&
         fabs(d1->eta()) < etaDau1Max_) ||
        (fabs(d0->eta()) > etaDau1Min_ && fabs(d1->eta()) > etaDau0Min_ && fabs(d0->eta()) < etaDau1Max_ &&
         fabs(d1->eta()) < etaDau0Max_)) {
      temp_cut = 1;
      if (d0->pt() > ptMin_ && d1->pt() > ptMin_) {
        ...
        if (m > massMin_ && m < massMax_)
          temp_cut = 3;
        if (c.mass() > massMinZMC_ && c.mass() < massMaxZMC_)
          temp_cut = 4;
      }
    }
...
MCAcceptanceAnalyzer::MCAcceptanceAnalyzer(const ParameterSet& cfg)
    : zToMuMu_(cfg.getParameter<InputTag>("zToMuMu")),
      ...
      nZToMuMuMC_(0),
      selZToMuMuMC_(0),
      nZToMuMuMCMatched_(0),
      selZToMuMuMCMatched_(0),
      nZToMuMuMCDen_(0),
      select_(cfg.getParameter<double>("ptMin"),
              cfg.getParameter<double>("etaDau0Min"),
              cfg.getParameter<double>("etaDau0Max"),
              cfg.getParameter<double>("etaDau1Min"),
              cfg.getParameter<double>("etaDau1Max"),
              cfg.getParameter<double>("massMin"),
              cfg.getParameter<double>("massMax"),
              cfg.getParameter<double>("massMinZMC"),
              cfg.getParameter<double>("massMaxZMC")),
      select_OnlyMassCut_(-1,
                          -9999,
                          9999,
                          -9999,
                          9999,
...
void MCAcceptanceAnalyzer::analyze(const Event& evt, const EventSetup&) {
  ...
  for (long i = 0; i < nZToMuMuMatched; ++i) {
    ...
    if (mcRef.isNonnull()) {  // z candidate matched to Z MC
      ...
      if (selectMC == 4)
        ++selZToMuMuMCMatched_;

      if (selectZ != selectMC) {
        cout << ">>> select reco: " << selectZ << ", select mc: " << selectMC << endl;
        if ((selectZ * selectMC) == 0)
          break;
        if (z.numberOfDaughters() > 1) {
          const Candidate *d0 = z.daughter(0), *d1 = z.daughter(1);
          if (mcRef->numberOfDaughters() > 1) {
            const Candidate *mcd0 = mcMuDaughter(mcRef->daughter(0)), *mcd1 = mcMuDaughter(mcRef->daughter(1));
            double m = z.mass(), mcm = (mcd0->p4() + mcd1->p4()).mass();
            cout << ">>> reco pt1, eta1: " << d0->pt() << ", " << d0->eta() << ", 2: " << d1->pt() << ", " << d1->eta()
                 << ", mass = " << m << endl;
            cout << ">>> mc   pt1, eta1: " << mcd0->pt() << ", " << mcd0->eta() << ", 2: " << mcd1->pt() << ", "
                 << mcd1->eta() << ", mass = " << mcm << endl;
          }
        }
      }
      // to avoid double counting
      if ((selectZ == 3) && (selectMC == 3))
        break;
    }
  }
}
...
void MCAcceptanceAnalyzer::endJob() {
  ...
  cout << ">>> " << zToMuMu_ << ": " << selZToMuMu_ << "/" << nZToMuMu_ << " = " << effZToMuMu << " +/- " << errZToMuMu
       << endl;
  cout << ">>> " << zToMuMuMC_ << " - matched: " << selZToMuMuMCMatched_ << "/" << nZToMuMuMCMatched_ << " = "
       << effZToMuMuMCMatched << " +/- " << errZToMuMuMCMatched << endl;
  cout << " if the two numbers above are the same we can neglete resolution effect and quote the acceptance as the "
          "number below.... "
       << endl;
  cout << "********* acceptance m>sampleMCMassCut (usually 20 or 40) ********  " << endl;
  cout << ">>> " << zToMuMuMC_ << ": " << selZToMuMuMC_ << "/" << nZToMuMuMC_ << " = " << effZToMuMuMC << " +/- "
       << errZToMuMuMC << endl;
  cout << "********* acceptance in the given mass range ********  " << endl;
  cout << ">>> " << zToMuMuMC_ << ": " << selZToMuMuMC_ << "/" << nZToMuMuMCDen_ << " = " << effZToMuMuMCDen << " +/- "
       << errZToMuMuMCDen << endl;
}
