...
ZToMuMuSelector::ZToMuMuSelector(const ParameterSet &cfg)
    : muonToken_(consumes<TrackCollection>(cfg.getParameter<edm::InputTag>("MuonTag"))),
      ...
      trackerToken_(mayConsume<TrackCollection>(
          cfg.getUntrackedParameter<edm::InputTag>("TrackerTag", edm::InputTag("ctfWithMaterialTracks")))),
      isoTrackerToken_(mayConsume<edm::ValueMap<bool> >(
          cfg.getUntrackedParameter<edm::InputTag>("TrackerIsolationTag", edm::InputTag("zMuMuTrackerIsolations")))),
...
bool ZToMuMuSelector::filter(Event &ev, const EventSetup &) {
  ...
  for (unsigned int i = 0; i < muonCollection->size(); i++) {
    ...
    for (unsigned int j = i + 1; j < muonCollection->size(); j++) {
      ...
      LogTrace("") << "\t... Z_en, Z_px, Z_py, Z_pz= " << z_en << ", " << z_px << ", " << z_py << ", " << z_pz
                   << " GeV";
    ...
    if (onlyGlobalMuons_ || globalCombinationFound)
      continue;
