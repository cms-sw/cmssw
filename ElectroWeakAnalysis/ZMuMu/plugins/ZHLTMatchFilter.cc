...
namespace modules {
  struct ZHLTMatchFilter {
    ZHLTMatchFilter(const edm::ParameterSet& cfg, edm::ConsumesCollector& iC)
        : cond_(cfg.getParameter<std::string>("condition")), hltPath_(cfg.getParameter<std::string>("hltPath")) {}
    bool operator()(const reco::Candidate& z) const {
      ...
      if ((((cond_ != "exactlyOneMatched" && cond_ != "atLeastOneMatched") && cond_ != "bothMatched") &&
           cond_ != "firstMatched") &&
          cond_ != "globalisMatched")
      ...
      if (mu0 != nullptr && (firstismuon || firstisStandAlone || firstisTrackerMuon)) {
        ...
        const pat::TriggerObjectStandAloneCollection mu0HLTMatches = mu0->triggerObjectMatchesByPath(hltPath_);
      ...
      if (mu1 != nullptr && (secondismuon || secondisStandAlone || secondisTrackerMuon)) {
        // get the vector of trigger objects matched to the muon corresponding to hltPath_
        const pat::TriggerObjectStandAloneCollection mu1HLTMatches = mu1->triggerObjectMatchesByPath(hltPath_);
      ...
      if (!singleTrigFlag0 && !singleTrigFlag1)
        return false;
      if ((singleTrigFlag0 && singleTrigFlag1) && firstismuon && secondismuon)
        bothTriggerFlag = true;
      if (((singleTrigFlag0 && !singleTrigFlag1) && firstismuon && secondismuon) ||
          ((!singleTrigFlag0 && singleTrigFlag1) && firstismuon && secondismuon))
        exactlyOneTriggerFlag = true;
      if ((((singleTrigFlag0 && firstismuon) && (secondisStandAlone || secondisTrackerMuon)) && !secondismuon) ||
          (((singleTrigFlag1 && secondismuon) && (firstisStandAlone || firstisTrackerMuon)) && !firstismuon))
        globalisTriggerFlag = true;

      if ((singleTrigFlag0 && !singleTrigFlag1) && !secondismuon)
        FirstTriggerFlag = true;
      if ((singleTrigFlag0 || singleTrigFlag1) && firstismuon && secondismuon)
        atLeastOneTriggerFlag = true;
      if (cond_ == "exactlyOneMatched")
        return exactlyOneTriggerFlag;
      if (cond_ == "atLeastOneMatched")
        return atLeastOneTriggerFlag;
      if (cond_ == "bothMatched")
        return bothTriggerFlag;
      if (cond_ == "firstMatched")
        return FirstTriggerFlag;
      if (cond_ == "globalisMatched")
...
typedef SingleObjectSelector<edm::View<reco::Candidate>, modules::ZHLTMatchFilter> ZHLTMatchFilter;
