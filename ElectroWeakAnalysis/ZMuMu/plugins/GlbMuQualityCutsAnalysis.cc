...
void GlbMuQualityCutsAnalysis::analyze(const edm::Event& evt, const edm::EventSetup&) {
  ...
  for (CandidateView::const_iterator i = src->begin(); i != src->end(); ++i) {
    ...
    if (mu1->isGlobalMuon())
      nChi2_1 = mu1->normChi2();
    std::cout << "chi2 of GlbMu1: " << nChi2_1 << std::endl;
    if (mu2->isGlobalMuon())
      nChi2_2 = mu2->normChi2();
    ...
    if (mu1->isGlobalMuon() && (nChi2_1 > chi2Cut_)) {
      std::cout << "found a GlbMuon with chi2 " << nChi2_1
                << ", it has chi2 of track: " << mu1->innerTrack()->normalizedChi2()
                << ", and chi2 of Sta: " << mu1->outerTrack()->normalizedChi2() << ", eta: " << mu1->eta()
                << ",pt: " << mu1->pt() << std::endl;
    }
    if (mu2->isGlobalMuon() && (nChi2_2 > chi2Cut_)) {
      std::cout << "found a GlbMuon with chi2 " << nChi2_2
                << ", it has chi2 of track: " << mu2->innerTrack()->normalizedChi2()
                << ", and chi2 of Sta: " << mu2->outerTrack()->normalizedChi2() << ", eta:  " << mu2->eta()
                << ",pt: " << mu2->pt() << std::endl;
    }
  }
}
