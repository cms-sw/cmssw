...
testAnalyzer::testAnalyzer(const edm::ParameterSet& pset)
  ...
  h_numberTrigMuon_ =
      fs->make<TH1D>("NumeratoreTrigMuon", "Number of Triggered Muons vs Eta", nbinsEta_, minEta_, maxEta_);
  h_numberMuon_ptStudy_ = fs->make<TH1D>("DenominatorePtStudy", "Number of Muons vs Pt", nbinsPt_, minPt_, maxPt_);
  h_numberTrigMuon_ptStudy_ =
      fs->make<TH1D>("NumeratoreTrigMuonPtStudy", "Number of Triggered Muons vs Pt", nbinsPt_, minPt_, maxPt_);
...
void testAnalyzer::analyze(const edm::Event& event, const edm::EventSetup& setup) {
  ...
  if (zmumuSize > 0) {
    for (int i = 0; i < zmumuSize; ++i) {
      ...
      const pat::TriggerObjectStandAloneCollection mu0HLTMatches = mu0.triggerObjectMatchesByPath(pathName_);
      const pat::TriggerObjectStandAloneCollection mu1HLTMatches = mu1.triggerObjectMatchesByPath(pathName_);
...
void testAnalyzer::endJob() {
  cout << "DoubleTrigger = " << DoubleTrigger_ << " , SingleTrigger = " << SingleTrigger_
       << " , NoTrigger = " << NoTrigger_ << " ,zmumuIncrement = " << zmumuIncrement_ << endl;
  ...
  for (int i = 0; i < dimVec; ++i)
    cout << "Pt = " << vectorPt[i] << " ==> Eta = " << vectorEta[i] << endl;
}
