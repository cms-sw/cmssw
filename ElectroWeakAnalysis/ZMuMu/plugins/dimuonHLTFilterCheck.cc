  ...
  if (muonTable[0] > 1) {
    for (unsigned int i = 0; i < muons->size(); ++i) {  //loop on candidates
      ...
      if (muCandRef->isGlobalMuon()) {
        ...
        cout << "muCandStaComponent : " << i << "  pt " << muStaComponentRef->pt() << "  eta "
             << muStaComponentRef->eta() << endl;
      }
    }
  }
...
vector<int> dimuonHLTFilterCheck::nDimuonsByType(const Handle<CandidateView> d) {
  ...
  for (unsigned int i = 0; i < d->size(); ++i) {  //loop on candidates
    ...
    if (mu0->isGlobalMuon())
      mu0global = true;
    if (!mu0->isGlobalMuon() && mu0->isStandAloneMuon() && mu0->isTrackerMuon())
      mu0trackerSta = true;
    if (!mu0->isGlobalMuon() && mu0->isStandAloneMuon() && !mu0->isTrackerMuon())
      mu0sta = true;
    if (!mu0->isGlobalMuon() && !mu0->isStandAloneMuon() && mu0->isTrackerMuon())
    ...
    if (mu1->isGlobalMuon())
      mu1global = true;
    if (!mu1->isGlobalMuon() && mu1->isStandAloneMuon() && mu1->isTrackerMuon())
      mu1trackerSta = true;
    if (!mu1->isGlobalMuon() && mu1->isStandAloneMuon() && !mu1->isTrackerMuon())
      mu1sta = true;
    if (!mu1->isGlobalMuon() && !mu1->isStandAloneMuon() && mu1->isTrackerMuon())
    ...
    if (mu0global && mu1global)
      n_[0]++;
    if ((mu0global && mu1trackerSta) || (mu1global && mu0trackerSta))
      n_[1]++;
    if ((mu0global && mu1sta) || (mu1global && mu0sta))
      n_[2]++;
    if ((mu0global && mu1tracker) || (mu1global && mu0tracker))
      n_[3]++;
    if (mu0trackerSta && mu1trackerSta)
      n_[4]++;
    if ((mu0trackerSta && mu1sta) || (mu1trackerSta && mu0sta))
      n_[5]++;
    if ((mu0trackerSta && mu1tracker) || (mu1trackerSta && mu0tracker))
      n_[6]++;
    if (mu0sta && mu1sta)
      n_[7]++;
    if ((mu0sta && mu1tracker) || (mu1sta && mu0tracker))
      n_[8]++;
    if (mu0tracker && mu1tracker)
      n_[9]++;
  }
  return n_;
}
...
vector<int> dimuonHLTFilterCheck::nMuonsByType(const Handle<CandidateView> d) {
  ...
  for (unsigned int i = 0; i < d->size(); ++i) {  //loop on candidates
    ...
    if (muCandRef->isGlobalMuon())
      muglobal = true;
    if (!muCandRef->isGlobalMuon() && muCandRef->isStandAloneMuon() && muCandRef->isTrackerMuon())
      mutrackerSta = true;
    if (!muCandRef->isGlobalMuon() && muCandRef->isStandAloneMuon() && !muCandRef->isTrackerMuon())
      musta = true;
    if (!muCandRef->isGlobalMuon() && !muCandRef->isStandAloneMuon() && muCandRef->isTrackerMuon())
      mutracker = true;
    cout << "muglobal " << muglobal << " mutrackserSta " << mutrackerSta << " must " << musta << " mutracker "
         << mutracker << endl;
    if (muglobal)
      n_[0]++;
    if (mutrackerSta)
