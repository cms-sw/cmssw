...
bool IsMuMatchedToHLTMu(const reco::Candidate* dau, std::vector<reco::Particle> HLTMu, double DR, double DPtRel) {
  ...
  for (unsigned int k = 0; k < dim; k++) {
    if ((deltaR(HLTMu[k], *dau) < DR) && (fabs(HLTMu[k].pt() - dau->pt()) / HLTMu[k].pt() < DPtRel)) {
      nPass++;
    }
  }
  return (nPass > 0);
...
void ZGoldenFilter::newEvent(const edm::Event& ev, const edm::EventSetup&) {
  ...
  if (!ev.getByToken(trigEvToken_, handleTriggerEvent_)) {
    edm::LogError("errorTriggerEventValid")
        << "trigger::TriggerEvent product with InputTag " << trigEv_.encode() << " not in event";
    return;
  }
...
bool ZGoldenFilter::operator()(const reco::Candidate& z) const {
  ...
  if (((singleTrigFlag0 && !singleTrigFlag1) && secondismuon) ||
      ((!singleTrigFlag0 && singleTrigFlag1) && secondismuon))
    exactlyOneTriggerFlag = true;
  if ((((singleTrigFlag0 && firstdismuon) && secondisStandAlone) && !secondismuon) ||
      (((singleTrigFlag1 && secondismuon) && firstdisStandAlone) && !firstdismuon))
