...
int main(int ac, char *av[]) {
  ...
  try{
    ...
    desc.add_options()
      ...
      ("directories,d", po::value< vector<string> >(), "directory")
    ...
    if (vm.count("directories")) {
      cout << " directories are: " 
	   << vm["directories"].as< vector<string> >() << "\n";
      vector<string> v_dir = vm["directories"].as< vector<string> >();
      for(vector<string>::const_iterator itd = v_dir.begin(); 
	  itd != v_dir.end(); ++itd) {
	TFile * root_file = new TFile(file.c_str(),"read");
	//TFile * root_file = new TFile("MuTrigger_133874_133828_768ub.root","read");
	//string d_string = *itd;
	string  dirDen =  *itd + "/hTrigMuonPtDenS";
	string  dirNum =  *itd + "/hTrigMuonPtNumS";
	
	
	TH1D * den = (TH1D*) root_file->Get( dirDen.c_str()  );
	TH1D * num = (TH1D*) root_file->Get( dirNum.c_str() );
	...
	graphCP.Draw("P");
	string plot= "HLTMu_9" + *itd + file + "." +  ext ; 
	c->SaveAs(plot.c_str());
        string outfile= "Effhisto" +  *itd + file;
