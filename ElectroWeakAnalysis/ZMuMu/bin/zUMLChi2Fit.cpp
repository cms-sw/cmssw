...
int main(int ac, char *av[]) {
  ...
  try {
    ...
    fit::RootMinuitCommands<fit::CombinedChiSquaredLikelihood<ChiSquared, Likelihood> > commands("zUMLChi2Fit.txt");

    const unsigned int rebinMuMuNoIso = 2,rebinMuMu = 1, rebinMuMu1HLT = 1, rebinMuMu2HLT = 1, rebinMuTk = 2, rebinMuSa = 1;
	...
	Expr zMuTkEffTerm = _2 * (effTk ^ _2) * effSa * (_1 - effSa) * (effIso ^ _2) * (_1 - ((_1 - effHLT)^_2));
	...
	Expr zPdfMuSaBkg = _1 / (cFMax - cFMin);
	Expr f = yieldZMuMu * zMuSaEffTerm * zPdfMuSa + yieldBkgZMuSa * zPdfMuSaBkg;
	funct::Expression zMuSaEffTermExp = _2 * (effSa ^ _2) * effTk * (_1 - effTk) * (effIso ^ _2) * effHLT;

	funct::Expression totYield = yieldZMuMu * zMuSaEffTermExp + yieldBkgZMuSa;
	Likelihood like(masses, f, totYield);
	...
	fit::RootMinuit<fit::CombinedChiSquaredLikelihood<ChiSquared, Likelihood> > minuit(combo, true);
	...
	commands.run(minuit);
	const unsigned int nPar = 16;//WARNIG: this must be updated manually for now
