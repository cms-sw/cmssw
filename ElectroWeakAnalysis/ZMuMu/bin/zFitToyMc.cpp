...
TH1* getHisto(TFile* file, const char* name, unsigned int rebin) {
  ...
  if (h == nullptr)
    throw edm::Exception(edm::errors::Configuration) << "Can't find object " << name << "\n";
...
int main(int ac, char* av[]) {
  ...
  try {
    ...
    desc.add_options()("help,h", "produce help message")("input-file,i", po::value<vector<string> >(), "input file")(
        ...
        "plot-format,p", po::value<string>(&ext)->default_value("eps"), "output plot format");
    ...
    if (vm.count("input-file")) {
      ...
      for (vector<string>::const_iterator it = v_file.begin(); it != v_file.end(); ++it) {
        ...
        Expr zMuSa =
            rebinMuSaConst * (zMuSaEffTerm * yieldZMuMu * zPdfMuSa);  // + (yieldBkgZMuSa * funct::Exponential(beta) ));
        ...
        myfile << Y << " " << dY << " " << tk_eff << " " << dtk_eff << " " << sa_eff << " " << dsa_eff << " " << iso_eff
               << " " << diso_eff << " " << hlt_eff << " " << dhlt_eff << " "
               << chi2() / (chi2.numberOfBins() - minuit.numberOfFreeParameters());
