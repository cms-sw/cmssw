...
//     max_sign=atof(argv[4]);
  ...
  double fMin , fMax,  lumi;
  int seed;
  // if toy....
  Bool_t toy = kFALSE;
  Bool_t  fitFromData = kFALSE;
  string  infile, outfile;
  // setting seed as well missing!!!!
 po::options_description desc("Allowed options");
    desc.add_options()
      ("help,h", "produce help message")
      ("toy,t", "toy enabled")
      ("seed,s", po::value<int>(&seed)->default_value(34567), "seed value for toy")
      ("luminosity,l", po::value<double>(&lumi)->default_value(100), "luminosity value for toy ")
