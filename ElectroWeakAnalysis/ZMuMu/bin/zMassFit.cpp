...
int main(int ac, char *av[]) { 
  try {
	...
	("convzg", "fit to the convolution between the Z Line Shape and a gaussian")
	...
	("convz2gf", "fit to the convolution between the Z line shape and a linear combination of fixed gaussians")
	("convzf2g", "fit to the convolution between the fixed Z line shape and a linear combination of gaussians")
      ...
      if (vm.count("convbwg"))
	{
      ...
      if (vm.count("convzg"))
	{
	  cout << "Fitting histograms in input files to the convolution between the Z Line Shape and a Gaussian\n";
	  cout << ">>> set pars: " << endl;
	  cout << yield << " ; " << dyield << endl; 
	  cout << mass << " ; " << dmass << endl; 
	  cout << gamma << " ; " << dgamma << endl; 
	  ...
	  for(size_t i = 0; i < v_ZMassHistos.size(); ++i) { 
	    ...
	    zMass->Draw("e");
	    fun.Draw("same");
	    string epsFilename = "ZMassFitCoZLSG_" + v_eps[i];
	    canvas->SaveAs(epsFilename.c_str());
	    canvas->SetLogy();
	    string epsLogFilename = "ZMassFitCoZLSG_Log_" + v_eps[i];
	    canvas->SaveAs(epsLogFilename.c_str());
	  }
	}
	...
	{ 
	  ...
	  for(size_t i = 0; i < v_ZMassHistos.size(); ++i) { 
	    ...
	    typedef Convolution<BreitWigner, Gaussian> ConvBwG;
	    ConvBwG cbg1(bw, gaus1, -range , range, 200);
	    ConvBwG cbg2(bw, gaus2, -range , range, 200);
	    typedef Product<Constant, ConvBwG> ConstConvBwG;
	    ConstConvBwG cocobg1 = c1 * cbg1;
	    ConstConvBwG cocobg2 = c2 * cbg2;
	    typedef Sum<ConstConvBwG, ConstConvBwG> FitFunction;
	...
	{ 
      ...
      if (vm.count("convz2gf")) 
	{ 
	  cout << "Fitting histograms in input files to the convolution between the Z Line Shape and a linear combination of fixed Gaussians\n";
	  cout << ">>> set pars: " << endl;
	  cout << yield1 << " ; " << dyield1 << endl; 
	  cout << yield2 << " ; " << dyield2 << endl; 
	  cout << mass << " ; " << dmass << endl; 
	  ...
	  for(size_t i = 0; i < v_ZMassHistos.size(); ++i) { 
	    ...
	    Gaussian gaus2(mean2, sigma2);
	    double range = 3 * max(sigma1.value(), sigma2.value());
	    typedef Convolution<ZLineShape, Gaussian> ConvZG;
	    ConvZG czg1(zls, gaus1, -range , range, 200);
	    ConvZG czg2(zls, gaus2, -range , range, 200);
	    typedef Product<Constant, ConvZG> ConstConvZG;
	    ConstConvZG coczg1 = c1 * czg1;
	    ConstConvZG coczg2 = c2 * czg2;
	    typedef Sum<ConstConvZG, ConstConvZG> FitFunction;
	    ...
	    zMass->Draw("e");
	    fun.Draw("same");
	    string epsFilename = "ZMassFitCoZLSGGf_" + v_eps[i];
	    canvas->SaveAs(epsFilename.c_str());
	    canvas->SetLogy();
	    string epsLogFilename = "ZMassFitCoZLSGGf_Log_" + v_eps[i];
	    canvas->SaveAs(epsLogFilename.c_str());
	  }
	}
      
      if(vm.count("convzf2g"))
	{ 
	  cout << "Fitting histograms in input files to the convolution between the fixed Z Line Shape and a linear combination of Gaussians\n";
	  cout << ">>> set pars: " << endl;
	  cout << yield1 << " ; " << dyield1 << endl; 
	  cout << yield2 << " ; " << dyield2 << endl; 
	  cout << mass << " ; " << dmass << endl; 
	  ...
	  for(size_t i = 0; i < v_ZMassHistos.size(); ++i) { 
	    ...
	    zMass->Draw("e");
	    fun.Draw("same");
	    string epsFilename = "ZMassFitCoZLSfGG_" + v_eps[i];
	    canvas->SaveAs(epsFilename.c_str());
	    canvas->SetLogy();
	    string epsLogFilename = "ZMassFitCoZLSfGG_Log_" + v_eps[i];
	    canvas->SaveAs(epsLogFilename.c_str());
	  }
	}
      cout << "It works!\n";
