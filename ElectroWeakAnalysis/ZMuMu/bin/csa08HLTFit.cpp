...
int main(int ac, char* av[]) {
  ...
  try {
    ...
    desc.add_options()("help,h", "produce help message")("input-file,i", po::value<vector<string> >(), "input file")(
        ...
        "plot-format,p", po::value<string>(&ext)->default_value("ps"), "output plot format");
    ...
    const unsigned int rebinMuMuNoIso = 2, rebinMuMu = 1, rebinMuMu1HLT = 1, rebinMuMu2HLT = 1, rebinMuTk = 2,
                       rebinMuSa = 8;
    ...
    if (vm.count("input-file")) {
      ...
      for (vector<string>::const_iterator it = v_file.begin(); it != v_file.end(); ++it) {
        ...
        root::plot<Expr>(ZMuMu1HLTPlot.c_str(),
                         *histoZMuMu1HLT,
                         zMuMu1HLT,
                         fMin,
                         fMax,
                         ...
                         100,
                         "Z -> #mu #mu mass",
                         "#mu #mu invariant mass (GeV/c^{2})",
                         "Events");

        string ZMuMu2HLTPlot = "ZMuMu2HLTFit_" + plot_string;
        root::plot<Expr>(ZMuMu2HLTPlot.c_str(),
                         *histoZMuMu2HLT,
                         zMuMu2HLT,
                         fMin,
                         fMax,
        ...
        root::plot<Expr>(ZMuMuNoIsoPlot.c_str(),
                         *histoZMuMuNoIso,
                         zMuMuNoIso,
                         fMin,
                         fMax,
                         ...
                         kDashed,
                         100,
                         "Z -> #mu #mu Not Iso mass",
                         "#mu #mu invariant mass (GeV/c^{2})",
                         "Events");

        string ZMuTkPlot = "ZMuTkFit_X_" + plot_string;
        root::plot<Expr>(ZMuTkPlot.c_str(),
                         *histoZMuTk,
                         zMuTk,
                         fMin,
                         fMax,
                         ...
                         kDashed,
                         100,
                         "Z -> #mu + (unmatched) track mass",
                         "#mu #mu invariant mass (GeV/c^{2})",
                         "Events");
        ZMuTkPlot = "ZMuTkFit_" + plot_string;
        TF1 funZMuTk = root::tf1_t<sig_tag, Expr>("ZMuTkFunction",
                                                  zMuTk,
                                                  fMin,
                                                  fMax,
                                                  effTk,
        ...
        TF1 funZMuTkBkg =
            root::tf1_t<bkg_tag, Expr>("ZMuTkBack", zMuTkBkgScaled, fMin, fMax, yieldBkgZMuTk, lambda, a0, a1, a2);
        ...
        root::plot<Expr>(ZMuSaPlot.c_str(),
                         *histoZMuSa,
                         zMuSa,
                         fMin,
                         fMax,
