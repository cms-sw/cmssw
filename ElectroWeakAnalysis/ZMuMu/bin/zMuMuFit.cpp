...
int main(int ac, char *av[]) {
  ...
  try {
    typedef funct::Convolution<funct::BreitWigner, funct::Gaussian>::type ZPeak;
    typedef 
      funct::Sum<
        funct::Product<funct::Parameter, ZPeak>::type,
        funct::Product<funct::Parameter, funct::GammaPropagator>::type
      >::type ZMuMuSig;
	...
	fix(histoZMuMu);
	histoZMuMu->SetTitle("Z -> #mu #mu mass with isolation cut");
	histoZMuMu->SetXTitle("#mu #mu invariant mass (GeV/c^{2})");
	histoZMuMu->SetYTitle("Events");
	...
	fix(histoZMuSa);
	histoZMuSa->SetTitle("Z -> #mu + (unmatched) standalone mass with isolation cut");
	...
	const char * kYieldPhotonMuMu = "YieldPhotonMuMu";
	const char * kYieldPhotonMuTk = "YieldPhotonMuTk";
	...
	//Parameters for convolutions with gaussians
	...
	funct::Parameter yieldPhotonMuMu(kYieldPhotonMuMu, commands.par(kYieldPhotonMuMu)); 
	funct::Parameter yieldPhotonMuTk(kYieldPhotonMuTk, commands.par(kYieldPhotonMuTk)); 
	...
	ZPeak zPeak = funct::conv(funct::BreitWigner(mass, gamma), funct::Gaussian(meanZMuMu, sigmaZMuMu), 
				  -3*sigmaZMuMu.value(), 3*sigmaZMuMu.value(), 200);
	ZMuMu zMuMu = yieldZMuMu * zPeak + yieldPhotonMuMu * funct::GammaPropagator();
	ZMuTkBkg zMuTkBkg = 
	  yieldBkgZMuTk * (funct::Exponential(lambda) * funct::Polynomial<2>(a0, a1, a2));
	ZMuTk zMuTk = yieldZMuTk * zPeak + yieldPhotonMuTk * funct::GammaPropagator() + zMuTkBkg;
	...
	commands.add(minuit, yieldPhotonMuMu);
	commands.add(minuit, yieldPhotonMuTk);
	...
	commands.run(minuit);
	//const size_t numPars = minuit.numberOfParameters();
	const unsigned int nPar = 16;
