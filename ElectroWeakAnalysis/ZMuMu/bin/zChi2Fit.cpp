...
int main_t(const vector<string>& v_file) {
  ...
  for (vector<string>::const_iterator it = v_file.begin(); it != v_file.end(); ++it) {
    ...
    TF1 funZMuMuNoIso = root::tf1_t<sig_tag, Expr>("ZMuMuNoIsoFunction",
                                                   zMuMuNoIso,
                                                   fMin,
                                                   fMax,
                                                   effTk,
    ...
    TF1 funZMuMuNoIsoBkg = root::tf1_t<bkg_tag, Expr>(
        "ZMuMuNoIsoBack", zMuMuNoIsoBkgScaled, fMin, fMax, yieldBkgZMuMuNotIso, alpha, b0, b1, b2);
    ...
    TF1 funZMuTk = root::tf1_t<sig_tag, Expr>(
        "ZMuTkFunction", zMuTk, fMin, fMax, effTk, effSa, effIso, effHLT, yieldZMuMu, yieldBkgZMuTk, lambda, a0, a1, a2);
    ...
    TF1 funZMuSa = root::tf1_t<sig_tag, Expr>(
        "ZMuSaFunction", zMuSa, fMin, fMax, effTk, effSa, effIso, effHLT, yieldZMuMu, yieldBkgZMuSa, beta, c0, c1, c2);
    ...
    TF1 funZMuSaBkg =
        root::tf1_t<bkg_tag, Expr>("ZMuSaBack", zMuSaBkgScaled, fMin, fMax, yieldBkgZMuSa, beta, c0, c1, c2);
...
int main(int ac, char* av[]) {
  ...
  desc.add_options()("help,h", "produce help message")("input-file,i", po::value<vector<string> >(), "input file")(
      ...
      "rebins values: rebinMuMu2HLT , rebinMuMu1HLT , rebinMuMuNoIso , rebinMuSa, rebinMuTk")(
      "chi2,c", "perform chi-squared fit")("plr,p", "perform Poisson likelihood-ratio fit")(
      "nonIsoTemplateFromMC,I",
      po::value<bool>(&nonIsoTemplateFromMC)->default_value(false),
      "take the template for nonIso sample from MC")(
      "plot-format,f", po::value<string>(&ext)->default_value("eps"), "output plot format")(
      "detectorRegion,r",
      po::value<string>(&region)->default_value("all"),
      "detector region in which muons are detected");
  ...
  if (vm.count("rebins")) {
    ...
    if (v_rebin.size() != 5) {
      cerr << " please provide 5 numbers in the given order:  rebinMuMu2HLT , rebinMuMu1HLT , rebinMuMuNoIso, "
              "rebinMuSa, rebinMuTk \n";
      return 1;
    }
    rebinMuMuNoIso = v_rebin[2], rebinMuMu1HLT = v_rebin[1], rebinMuMu2HLT = v_rebin[0], rebinMuTk = v_rebin[4],
    rebinMuSa = v_rebin[3];
  }
