...
SimpleSystematicsAnalyzer::SimpleSystematicsAnalyzer(const edm::ParameterSet& pset)
    : selectorPath_(pset.getUntrackedParameter<std::string>("SelectorPath", "")),
      ...
      weightTokens_(
          edm::vector_transform(weightTags_, [this](edm::InputTag const& tag) { return consumes<double>(tag); })),
...
void SimpleSystematicsAnalyzer::endJob() {
  ...
  edm::LogVerbatim("SimpleSystematicsAnalysis")
      << "Total number of selected data: " << selectedEvents_ << " [events], corresponding to acceptance: ["
      << originalAcceptance * 100 << " +- "
      << 100 * sqrt(originalAcceptance * (1. - originalAcceptance) / originalEvents_) << "] %";
  ...
  for (unsigned int i = 0; i < weightTags_.size(); ++i) {
    ...
    edm::LogVerbatim("SimpleSystematicsAnalysis")
        << "\tTotal Events after reweighting: " << weightedEvents_[i] << " [events]";
    edm::LogVerbatim("SimpleSystematicsAnalysis")
        << "\tEvents selected after reweighting: " << weightedSelectedEvents_[i] << " [events]";
    edm::LogVerbatim("SimpleSystematicsAnalysis")
    ...
    if (deltaxi > 0)
      ...
      deltaxi = 0.;
    edm::LogVerbatim("SimpleSystematicsAnalysis")
        << "\ti.e. [" << std::setprecision(4) << 100 * xi / originalAcceptance << " +- " << std::setprecision(4)
        << 100 * deltaxi / originalAcceptance << "] % relative variation with respect to the original acceptance";
  }
...
bool SimpleSystematicsAnalyzer::filter(edm::Event& ev, const edm::EventSetup&) {
  ...
  if (pathFound) {
    if (triggerResults->accept(pathIndex))
      selectedEvent = true;
  }
