...
PdfSystematicsAnalyzer::PdfSystematicsAnalyzer(const edm::ParameterSet& pset)
    : selectorPath_(pset.getUntrackedParameter<std::string>("SelectorPath", "")),
      ...
      pdfWeightTokens_(edm::vector_transform(
          pdfWeightTags_, [this](edm::InputTag const& tag) { return consumes<std::vector<double> >(tag); })),
...
void PdfSystematicsAnalyzer::endJob() {
  ...
  edm::LogVerbatim("PDFAnalysis") << "Total number of selected data: " << selectedEvents_
  ...
  for (unsigned int i = 0; i < pdfWeightTags_.size(); ++i) {
    ...
    if (i < pdfWeightTags_.size() - 1)
      nmembers = pdfStart_[i + 1] - pdfStart_[i];
    ...
    edm::LogVerbatim("PDFAnalysis")
        << "\ti.e. [" << std::setprecision(4) << 100 * (events_central - selectedEvents_) / selectedEvents_ << " +- "
    ...
    if (npairs > 0) {
      ...
      edm::LogVerbatim("PDFAnalysis") << "\tRelative uncertainty with respect to central member: +"
  ...
  for (unsigned int i = 0; i < pdfWeightTags_.size(); ++i) {
    ...
    edm::LogVerbatim("PDFAnalysis") << "\tEstimate for central PDF member acceptance: [" << acc_central * 100 << " +- "
                                    << 100 *
                                           sqrt((acc2_central / waverage - acc_central * acc_central) / originalEvents_)
                                    << "] %";
    double xi = acc_central - originalAcceptance;
    double deltaxi = (acc2_central - (originalAcceptance + 2 * xi + xi * xi)) / originalEvents_;
    if (deltaxi > 0)
      deltaxi = sqrt(deltaxi);  //else deltaxi = 0.;
    edm::LogVerbatim("PDFAnalysis") << "\ti.e. [" << std::setprecision(4) << 100 * xi / originalAcceptance << " +- "
    ...
    if (npairs > 0) {
      ...
      for (unsigned int j = 0; j < npairs; ++j) {
        ...
        if (weightedEvents_[pdfStart_[i] + 2 * j + 1] > 0)
          wa = (weightedSelectedEvents_[pdfStart_[i] + 2 * j + 1] / weightedEvents_[pdfStart_[i] + 2 * j + 1]) /
                   acc_central -
               1.;
        double wb = 0.;
        if (weightedEvents_[pdfStart_[i] + 2 * j + 2] > 0)
          wb = (weightedSelectedEvents_[pdfStart_[i] + 2 * j + 2] / weightedEvents_[pdfStart_[i] + 2 * j + 2]) /
                   acc_central -
               1.;
        if (nnpdfFlag) {
          if (wa > 0.) {
