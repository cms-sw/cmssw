...
void HLTEgammaEtFilterPairs::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
bool HLTEgammaEtFilterPairs::hltFilter(edm::Event& iEvent,
  ...
  for (unsigned int i = 0; i < recoecalcands.size(); i = i + 2) {
    ...
    bool second =
        (fabs(r2->eta()) < 1.479 && r2->et() >= etcutEB2_) || (fabs(r2->eta()) >= 1.479 && r2->et() >= etcutEE2_);
