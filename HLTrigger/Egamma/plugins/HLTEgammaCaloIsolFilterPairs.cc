...
HLTEgammaCaloIsolFilterPairs::HLTEgammaCaloIsolFilterPairs(const edm::ParameterSet& iConfig) : HLTFilter(iConfig) {
  ...
  if (AlsoNonIso_1 || AlsoNonIso_2)
    nonIsoToken_ = consumes<reco::RecoEcalCandidateIsolationMap>(nonIsoTag_);
}

HLTEgammaCaloIsolFilterPairs::~HLTEgammaCaloIsolFilterPairs() = default;

void HLTEgammaCaloIsolFilterPairs::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
bool HLTEgammaCaloIsolFilterPairs::hltFilter(edm::Event& iEvent,
  ...
  if (AlsoNonIso_1 || AlsoNonIso_2)
    iEvent.getByToken(nonIsoToken_, depNonIsoMap);
  ...
  for (unsigned int i = 0; i < recoecalcands.size(); i = i + 2) {
    ...
    if (PassCaloIsolation(r1, *depMap, *depNonIsoMap, 1, AlsoNonIso_1) &&
        PassCaloIsolation(r2, *depMap, *depNonIsoMap, 2, AlsoNonIso_2)) {
      n++;
...
bool HLTEgammaCaloIsolFilterPairs::PassCaloIsolation(const edm::Ref<reco::RecoEcalCandidateCollection>& ref,
  ...
  if (vali < isolcut || IsoOE < FracCut || IsoOE2 < IsoloEt2) {
    return true;
  }
  return false;
}
