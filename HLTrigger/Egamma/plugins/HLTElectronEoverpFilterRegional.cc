...
void HLTElectronEoverpFilterRegional::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
bool HLTElectronEoverpFilterRegional::hltFilter(edm::Event& iEvent,
  ...
  for (auto& recoecalcand : recoecalcands) {
    ...
    for (auto iElectron = electronIsolatedHandle->begin(); iElectron != electronIsolatedHandle->end(); iElectron++) {
      ...
      if (&(*recr2) == &(*theClus)) {
        ...
        if (trackMom.R() != 0)
          elecEoverp = electronref->superCluster()->energy() / trackMom.R();
