...
#include "HLTElectronOneOEMinusOneOPFilterRegional.h"
...
HLTElectronOneOEMinusOneOPFilterRegional::HLTElectronOneOEMinusOneOPFilterRegional(const edm::ParameterSet& iConfig)
  ...
  if (!doIsolated_)
    electronNonIsolatedToken_ = consumes<reco::ElectronCollection>(electronNonIsolatedProducer_);
}

void HLTElectronOneOEMinusOneOPFilterRegional::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
bool HLTElectronOneOEMinusOneOPFilterRegional::hltFilter(edm::Event& iEvent,
  ...
  if (saveTags()) {
    ...
    if (not doIsolated_)
      filterproduct.addCollectionTag(electronNonIsolatedProducer_);
  }
  ...
  for (auto& recoecalcand : recoecalcands) {
    ...
    for (auto iElectron = electronIsolatedHandle->begin(); iElectron != electronIsolatedHandle->end(); iElectron++) {
      ...
      reco::ElectronRef electronref(
          reco::ElectronRef(electronIsolatedHandle, iElectron - electronIsolatedHandle->begin()));
      const reco::SuperClusterRef theClus = electronref->superCluster();
      if (&(*recr2) == &(*theClus)) {
        float elecEoverp = 0;
        const math::XYZVector trackMom = electronref->track()->momentum();
        if (trackMom.R() != 0)
          elecEoverp = fabs((1 / electronref->superCluster()->energy()) - (1 / trackMom.R()));
    ...
    if (!doIsolated_) {
      //loop over the electrons to find the matching one
      for (auto iElectron = electronNonIsolatedHandle->begin(); iElectron != electronNonIsolatedHandle->end();
           iElectron++) {
        reco::ElectronRef electronref(
            reco::ElectronRef(electronNonIsolatedHandle, iElectron - electronNonIsolatedHandle->begin()));
        const reco::SuperClusterRef theClus = electronref->superCluster();
