 ...
 *
 * $Id: HLTEgammaHOEFilter.cc,v 1.5 2007/04/02 17:14:14 mpieri Exp $
 *
...
HLTEgammaHOEFilter::filter(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for (unsigned int i=0; i<recoecalcands->size(); i++) {
    ...
    //std::cout<<"MARCO HLTEgammaHOEFilter i "<<i<<" "<<std::endl;
    candref = recoecalcands->getParticleRef(i);
    //std::cout<<"MARCO HLTEgammaHOEFilter candref "<<(long) candref<<" "<<std::endl;    
    reco::RecoEcalCandidateRef recr = candref.castTo<reco::RecoEcalCandidateRef>();
    //std::cout<<"MARCO HLTEgammaHOEFilter recr "<<recr<<" "<<std::endl;
    ...
    if(mapi==(*depMap).end()) {
     ...
     float vali = mapi->val / candref.get()->et();
     //std::cout<<"MARCO HLTEgammaHOEFilter vali "<<vali<<" "<<std::endl;
