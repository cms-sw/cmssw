...
template <typename T> 
HLTEPFMTFilter<T>::HLTEPFMTFilter(const edm::ParameterSet& iConfig) : HLTFilter(iConfig)
{
  // MHT parameters
...
template <typename T> 
HLTEPFMTFilter<T>::~HLTEPFMTFilter(){}

template <typename T> 
void HLTEPFMTFilter<T>::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  descriptions.add(std::string("hlt")+std::string(typeid(HLTEPFMTFilter<T>).name()),desc);
}

template <typename T> 
bool  HLTEPFMTFilter<T>::hltFilter(edm::Event& iEvent, const edm::EventSetup& iSetup, trigger::TriggerFilterObjectWithRefs & filterproduct) const
{
  using namespace std;
  using namespace edm;
  ...
  // Sanity check:
  if(!pfMHT.isValid()) {    
    edm::LogError("HLTEPFMTFilter") << "missing input Met collection!";    
  }
