...
def customizeHLTfor45212(process):
    ...
    for p in ["PFClusterProducer", "LegacyPFClusterProducer", "PFMultiDepthClusterProducer"]:
        for prod in producers_by_type(process, p):
            if hasattr(prod, "pfClusterBuilder"):
                for n in ["positionCalc", "allCellsPositionCalc"]:
                    if hasattr(prod.pfClusterBuilder, n) and hasattr(getattr(prod.pfClusterBuilder, n), "algoName") and (getattr(prod.pfClusterBuilder, n).algoName == "Basic2DGenericPFlowPositionCalc"):
