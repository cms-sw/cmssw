...
bool HLTDoubletSinglet<T1, T2, T3>::hltFilter(edm::Event& iEvent,
  ...
  LogVerbatim("HLTDoubletSinglet") << " XXX " << moduleLabel() << " 0 " << std::endl;
    ...
    if (saveTags()) {
      ...
      for (size_type i1 = 0; i1 != n1; ++i1) {
        ...
        if (tagOld.encode() != tagNew.encode()) {
          ...
          tagOld = tagNew;
          LogVerbatim("HLTDoubletSinglet") << " XXX " << moduleLabel() << " 1b " << tagNew.encode() << std::endl;
        }
      }
      ...
      for (size_type i2 = 0; i2 != n2; ++i2) {
        ...
        if (tagOld.encode() != tagNew.encode()) {
          filterproduct.addCollectionTag(tagNew);
          tagOld = tagNew;
          LogVerbatim("HLTDoubletSinglet") << " XXX " << moduleLabel() << " 2b " << tagNew.encode() << std::endl;
        }
      }
      ...
      for (size_type i3 = 0; i3 != n3; ++i3) {
        ...
        if (tagOld.encode() != tagNew.encode()) {
          filterproduct.addCollectionTag(tagNew);
          tagOld = tagNew;
          LogVerbatim("HLTDoubletSinglet") << " XXX " << moduleLabel() << " 3b " << tagNew.encode() << std::endl;
        }
      }
    }
