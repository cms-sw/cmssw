...
HLTCAWZTagFilter::HLTCAWZTagFilter(const edm::ParameterSet& iConfig)
    : HLTFilter(iConfig),
...
bool HLTCAWZTagFilter::hltFilter(edm::Event& iEvent,
                                 const edm::EventSetup& iSetup,
                                 trigger::TriggerFilterObjectWithRefs& filterobject) const {
  ...
  auto ihardJet = pBasicJets->begin(), ihardJetEnd = pBasicJets->end();
  ...
  for (; ihardJet != ihardJetEnd; ++ihardJet, ++ipfJet) {
    ...
    if (properties.nSubJets != 2 || properties.wMass < minWMass_ || properties.wMass > maxWMass_)
      continue;
    else {
