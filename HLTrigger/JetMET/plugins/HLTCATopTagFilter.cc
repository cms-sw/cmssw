...
HLTCATopTagFilter::HLTCATopTagFilter(const edm::ParameterSet& iConfig)
    : HLTFilter(iConfig),
...
bool HLTCATopTagFilter::hltFilter(edm::Event& iEvent,
  ...
  for (; ihardJet != ihardJetEnd; ++ihardJet, ++ipfJet) {
    ...
    if (properties.nSubJets < 3 || properties.minMass < minMinMass_ || properties.topMass < minTopMass_ ||
        properties.topMass > maxTopMass_)
      continue;
    else {
