...
//
template <typename T>
HLTDiJetAveEtaFilter<T>::HLTDiJetAveEtaFilter(const edm::ParameterSet& iConfig)
    : HLTFilter(iConfig),
  ...
  LogDebug("") << "HLTDiJetAveEtaFilter: Input/minPtAve/minDphi/triggerType : " << inputJetTag_.encode() << " "
               << minPtAve_
               << " "
               //<< minPtJet3_ << " "
...
template <typename T>
void HLTDiJetAveEtaFilter<T>::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
template <typename T>
bool HLTDiJetAveEtaFilter<T>::hltFilter(edm::Event& iEvent,
