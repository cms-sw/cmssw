...
//
template <typename T>
HLTDiJetAveFilter<T>::HLTDiJetAveFilter(const edm::ParameterSet& iConfig)
    : HLTFilter(iConfig),
  ...
  LogDebug("") << "HLTDiJetAveFilter: Input/minPtAve/minPtJet3/minDphi/triggerType : " << inputJetTag_.encode() << " "
               << minPtAve_ << " " << minPtJet3_ << " " << minDphi_ << " " << triggerType_;
}
...
template <typename T>
void HLTDiJetAveFilter<T>::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
template <typename T>
bool HLTDiJetAveFilter<T>::hltFilter(edm::Event& iEvent,
