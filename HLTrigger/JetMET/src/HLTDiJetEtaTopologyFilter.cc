...
//
template <typename T>
HLTDiJetEtaTopologyFilter<T>::HLTDiJetEtaTopologyFilter(const edm::ParameterSet& iConfig)
    : HLTFilter(iConfig),
  ...
  LogDebug("") << "HLTDiJetEtaTopologyFilter: Input/minDphi/triggerType : " << inputJetTag_.encode()
               << " "
               //<< minPtJet3_ << " "
               << minDphi_ << " " << triggerType_;
}
...
template <typename T>
void HLTDiJetEtaTopologyFilter<T>::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  desc.add<bool>("applyAbsToTag", false), desc.add<bool>("applyAbsToProbe", false),
      desc.add<bool>("oppositeEta", false), desc.add<int>("triggerType", trigger::TriggerJet);
  descriptions.add(defaultModuleLabel<HLTDiJetEtaTopologyFilter<T>>(), desc);
}

// ------------ method called to produce the data  ------------
template <typename T>
bool HLTDiJetEtaTopologyFilter<T>::hltFilter(edm::Event& iEvent,
  ...
  if (objects->size() > 1) {  // events with two or more jets
    ...
    for (; iProbe != iEnd; ++iProbe) {
      ...
      for (; iTag != iEnd; ++iTag) {
        if (iTag == iProbe)
          continue;
        if (iTag->pt() < minPtTag_)
          continue;
        if (std::max(iTag->pt(), iProbe->pt()) < atLeastOneJetAbovePT_)
          continue;
        ...
        if (eta2 < tagEtaMin_ || eta2 > tagEtaMax_)
          continue;
