...
void HLTCaloJetIDProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (auto const& j : *calojets) {
    ...
    if (!(j.energy() > 0.))
      continue;  // skip jets with zero or negative energy
    ...
    if (std::abs(j.eta()) >= 2.6) {
      ...
      if (min_N90hits_ > 0)
        jetIDHelper_.calculate(iEvent, iSetup, j);
      if ((j.emEnergyFraction() >= min_EMF_) && (j.emEnergyFraction() <= max_EMF_) && (j.n90() >= min_N90_) &&
