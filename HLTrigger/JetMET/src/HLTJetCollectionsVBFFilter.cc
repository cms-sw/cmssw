...
//
template <typename T>
HLTJetCollectionsVBFFilter<T>::HLTJetCollectionsVBFFilter(const edm::ParameterSet& iConfig)
    : HLTFilter(iConfig),
...
template <typename T>
void HLTJetCollectionsVBFFilter<T>::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
template <typename T>
bool HLTJetCollectionsVBFFilter<T>::hltFilter(edm::Event& iEvent,
  ...
  for (unsigned int collection = 0; collection < theJetCollections.size(); ++collection) {
    ...
    for (; jetOne != refVector.end(); jetOne++) {
      ...
      if (thereAreVBFJets)
        break;
      if (jetOneRef->pt() < hardJetPt_)
        break;
      if (std::abs(jetOneRef->eta()) > maxAbsJetEta_)
        continue;
      ...
      for (; jetTwo != refVector.end(); jetTwo++) {
        ...
        if (jetTwoRef->pt() < softJetPt_)
          break;
        if (std::abs(jetTwoRef->eta()) > maxAbsJetEta_)
          continue;

        if (std::abs(jetTwoRef->eta() - jetOneRef->eta()) < minDeltaEta_)
          continue;
    ...
    if (minNJets_ > 2 && thereAreVBFJets) {
      ...
      for (; jetThree != refVector.end(); jetThree++) {
        ...
        if (thirdJetIndex == firstJetIndex || thirdJetIndex == secondJetIndex)
          continue;
