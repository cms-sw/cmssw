...
HLTForwardBackwardJetsFilter<T>::HLTForwardBackwardJetsFilter(const edm::ParameterSet& iConfig)
  ...
  LogDebug("") << "HLTForwardBackwardJetsFilter: Input/minPt/minEta/maxEta/triggerType : " << inputTag_.encode() << " "
               << minPt_ << " " << minEta_ << " " << maxEta_ << " " << nNeg_ << " " << nPos_ << " " << nTot_ << " "
               << triggerType_;
}
...
bool HLTForwardBackwardJetsFilter<T>::hltFilter(edm::Event& iEvent,
  ...
  // filter decision
  const bool accept((nNegJets >= nNeg_) && (nPosJets >= nPos_) && ((nNegJets + nPosJets) >= nTot_));
