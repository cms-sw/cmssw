...
//
template <typename T>
HLTMonoJetFilter<T>::HLTMonoJetFilter(const edm::ParameterSet& iConfig)
    : HLTFilter(iConfig),
  ...
  LogDebug("") << "HLTMonoJetFilter: Input/maxPtSecondJet/maxDeltaPhi/triggerType : " << inputJetTag_.encode() << " "
               << maxPtSecondJet_ << " " << maxDeltaPhi_ << " " << triggerType_;
}
...
template <typename T>
void HLTMonoJetFilter<T>::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
//
template <typename T>
bool HLTMonoJetFilter<T>::hltFilter(edm::Event& iEvent,
