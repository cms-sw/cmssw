...
//
template <typename T>
HLTJetEtaTopologyFilter<T>::HLTJetEtaTopologyFilter(const edm::ParameterSet& iConfig)
    : HLTFilter(iConfig),
  ...
  LogDebug("") << "HLTJetEtaTopologyFilter: Input/minDphi/triggerType : " << inputJetTag_.encode() << " "
               << triggerType_;
}
...
template <typename T>
void HLTJetEtaTopologyFilter<T>::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  desc.add<bool>("applyAbsToJet", false), desc.add<int>("triggerType", trigger::TriggerJet);
  descriptions.add(defaultModuleLabel<HLTJetEtaTopologyFilter<T>>(), desc);
}

// ------------ method called to produce the data  ------------
template <typename T>
bool HLTJetEtaTopologyFilter<T>::hltFilter(edm::Event& iEvent,
  ...
  if (!objects->empty()) {  // events with two or more jets
    ...
    for (; iJet != iEnd; ++iJet) {
      ...
      if (eta < jetEtaMin_ || eta > jetEtaMax_)
        continue;
