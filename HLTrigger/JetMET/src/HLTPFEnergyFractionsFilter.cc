...
bool HLTPFEnergyFractionsFilter::hltFilter(edm::Event& iEvent,
  ...
  // The filter object
  if (saveTags())
    filterproduct.addCollectionTag(inputPFJetTag_);
  ...
  if (recopfjets->size() >= nJet_) {
    ...
    for (; i != recopfjets->end(); ++i) {
      if (countJet >= nJet_)
        break;
      //
      if (i->chargedEmEnergyFraction() < min_CEEF_)
        accept = false;
      if (i->chargedEmEnergyFraction() > max_CEEF_)
        accept = false;
      //
      if (i->neutralEmEnergyFraction() < min_NEEF_)
        accept = false;
      if (i->neutralEmEnergyFraction() > max_NEEF_)
        accept = false;
      //
      if (i->chargedHadronEnergyFraction() < min_CHEF_)
        accept = false;
      if (i->chargedHadronEnergyFraction() > max_CHEF_)
        accept = false;
      //
      if (i->neutralHadronEnergyFraction() < min_NHEF_)
        accept = false;
      if (i->neutralHadronEnergyFraction() > max_NHEF_)
        accept = false;
      //
      if (accept == false)
        break;
