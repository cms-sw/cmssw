...
void HLTDTActivityFilter::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  desc.add<bool>("orDigi", false)->setComment(" # && of trig & digi info");
  ...
  desc.add<int>("minTPGQual", 2.)->setComment(" # 0-1=L 2-3=H 4=LL 5=HL 6=HH");
...
bool HLTDTActivityFilter::hltFilter(edm::Event& iEvent,
  ...
  if (process_[DCC]) {
    ...
    for (; iph != iphe; ++iph) {
      ...
      if (ch <= maxStation_ && bx >= minBX_[DCC] && bx <= maxBX_[DCC] && qual >= minQual_ && qual < 7) {
  ...
  if (process_[DDU]) {
    ...
    for (detUnitIt = trigsDDU->begin(); detUnitIt != trigsDDU->end(); ++detUnitIt) {
      ...
      if (!activeSecs_[(*detUnitIt).first.sector()])
        continue;
      ...
      for (DTLocalTriggerCollection::const_iterator trigIt = range.first; trigIt != range.second; ++trigIt) {
        ...
        if (ch <= maxStation_ && bx >= minBX_[DDU] && bx <= maxBX_[DDU] && qual >= minQual_ && qual < 7) {
  ...
  if (process_[DIGI]) {
    ...
    for (dtLayerIdIt = dtdigis->begin(); dtLayerIdIt != dtdigis->end(); dtLayerIdIt++) {
      ...
      if (!activeSecs_[(*dtLayerIdIt).first.sector()])
        continue;
