...
//
/**\class RPCPathChambFilter HLTRPCTrigNoSyncFilter.cc HLTRPCTrigNoSyncFilter.cc
...
bool bigmag(const RPC4DHit& Point1, const RPC4DHit& Point2) {
  if ((Point2).gp.mag() > (Point1).gp.mag())
    return true;
  else
    return false;
}
...
bool HLTRPCTrigNoSyncFilter::hltFilter(edm::Event& iEvent,
  ...
  for (recHit = rpcRecHits->begin(); recHit != rpcRecHits->end(); recHit++) {
    ...
    if (BX != 0)
      GlobalRPC4DHitsNoBx0.push_back(ThisHit);
    k++;
  }
...
void HLTRPCTrigNoSyncFilter::beginJob() {}

// ------------ method called once each job just after ending the event loop  ------------
void HLTRPCTrigNoSyncFilter::endJob() {}
