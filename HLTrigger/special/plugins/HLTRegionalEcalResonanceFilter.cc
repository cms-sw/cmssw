...
HLTRegionalEcalResonanceFilter::HLTRegionalEcalResonanceFilter(const edm::ParameterSet &iConfig) {
  ...
  if (doSelBarrel_) {
    ...
    // EB region 1
    region1_Barrel_ =
        barrelSelection.getParameter<double>("region1_Barrel");  //eta dividing between region 1 and region 2
  ...
  if (doSelEndcap_) {
    ...
    // EE region 1
    region1_EndCap_ =
        endcapSelection.getParameter<double>("region1_EndCap");  //eta dividing between region 1 and region 2
    ...
    // EE region 2
    region2_EndCap_ =
        endcapSelection.getParameter<double>("region2_EndCap");  //eta dividing between region 2 and region 3
...
bool HLTRegionalEcalResonanceFilter::filter(edm::Event &iEvent, const edm::EventSetup &iSetup) {
  ...
  if (debug_ >= 1)
    LogDebug("AlCaEcalResonanceProducer")
        << " barrel_input_size:  run " << iEvent.id().run() << " event " << iEvent.id().event()
        << " nhitEB:  " << hitCollection_eb->size() << " nCluster: " << clusterCollection_eb->size();
  ...
  if (debug_ >= 1)
    LogDebug("AlCaEcalResonanceProducer") << " barrel_output_size_run " << iEvent.id().run() << " event "
                                          << iEvent.id().event() << " nEBSaved " << selEBRecHitCollection->size();
  ///==============End of  barrel ==================///
  ...
  if (debug_ >= 1)
    LogDebug("AlCaEcalResonanceProducer")
        << " endcap_input_size:  run " << iEvent.id().run() << " event " << iEvent.id().event()
  ...
  if (debug_ >= 1)
    LogDebug("AlCaEcalResonanceProducer")
        << " endcap_output_size run " << iEvent.id().run() << "_" << iEvent.id().event() << " nEESaved "
        << selEERecHitCollection->size() << " nESSaved: " << selESRecHitCollection->size();
...
void HLTRegionalEcalResonanceFilter::doSelection(
  ...
  for (auto it_bc = clusterCollection->begin(); it_bc != clusterCollection->end(); it_bc++) {
    ...
    v1 *=
        (en1 /
         v1.R());  // rescale vector's magnitude to the energy in order to get momentum vector (assuming massless particles)
    ...
    for (auto it_bc2 = it_bc + 1; it_bc2 != clusterCollection->end(); it_bc2++) {
      ...
      v2 *=
          (en2 /
           v2.R());  // rescale vector's magnitude to the energy in order to get momentum vector (assuming massless particles)
      ...
      if (detector == EcalBarrel) {  // BARREL
        ...
        if (etapair <= region1_Barrel_) {  //EB region 1
          if (ptmin < selePtGammaBarrel_region1_ || pt_pair < selePtPairBarrel_region1_)
            continue;
        }
        //EB region 2
        else {
          if (ptmin < selePtGammaBarrel_region2_ || pt_pair < selePtPairBarrel_region2_)
            continue;
        }
      } else {  // ENDCAP
        //EE region 1
        ...
        //EE region 3
          ...
          if (pt_pair > selePtPairMaxEndCap_region3_)
            continue;  // there is also a possible maximum pt for the very forward region
        }
      }
      ...
      if (detector == EcalBarrel) {
        //EB region 1
        if (etapair <= region1_Barrel_) {  //EB region 1
          if (Nxtal_pho1 < seleNxtalBarrel_region1_ || Nxtal_pho2 < seleNxtalBarrel_region1_)
            continue;
        }
        //EB region 2
        else {
          if (Nxtal_pho1 < seleNxtalBarrel_region2_ || Nxtal_pho2 < seleNxtalBarrel_region2_)
            continue;
        }
      }
      // ENDCAP
        ...
        //EE region 1
        if (etapair <= region1_EndCap_) {
          if (Nxtal_pho1 < seleNxtalEndCap_region1_ || Nxtal_pho2 < seleNxtalEndCap_region1_)
            continue;
        }
        //EE region 2
        else if (etapair <= region2_EndCap_) {
          if (Nxtal_pho1 < seleNxtalEndCap_region2_ || Nxtal_pho2 < seleNxtalEndCap_region2_)
            continue;
        }
        //EE region 3
        else {
          if (Nxtal_pho1 < seleNxtalEndCap_region3_ || Nxtal_pho2 < seleNxtalEndCap_region3_)
            continue;
        }
      }
      ...
      if (detector == EcalBarrel) {
        //EB region 1
        if (etapair <= region1_Barrel_) {  //EB region 1
          if (iso_val > seleIsoBarrel_region1_)
            continue;
        }
        //EB region 2
        else {
          if (iso_val > seleIsoBarrel_region2_)
            continue;
        }
      }
      // ENDCAP
        ...
        //EE region 1
        if (etapair <= region1_EndCap_) {
          if (iso_val > seleIsoEndCap_region1_)
            continue;
        }
        //EE region 2
        else if (etapair <= region2_EndCap_) {
          if (iso_val > seleIsoEndCap_region2_)
            continue;
        }
        //EE region 3
        else {
          if (iso_val > seleIsoEndCap_region3_)
            continue;
        }
      }
      //-------------------------------------
...
void HLTRegionalEcalResonanceFilter::calcShowerShape(const reco::BasicCluster &bc,
                                                     const EcalChannelStatus &channelStatus,
...
void HLTRegionalEcalResonanceFilter::calcPaircluster(const reco::BasicCluster &bc1,
...
bool HLTRegionalEcalResonanceFilter::checkStatusOfEcalRecHit(const EcalChannelStatus &channelStatus,
                                                             const EcalRecHit &rh) {
...
void HLTRegionalEcalResonanceFilter::makeClusterES(
