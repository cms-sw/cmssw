...
#include "HLTSingleVertexPixelTrackFilter.h"
...
void HLTSingleVertexPixelTrackFilter::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
bool HLTSingleVertexPixelTrackFilter::hltFilter(edm::Event& iEvent,
    ...
    if (npixelvertices != 0) {
      ...
      if (trackCollection.isValid()) {
        ...
        for (tracksItr = tracks->begin(); tracksItr != tracks->end(); ++tracksItr) {
          ...
          if (fabs(eta) > max_Eta_)
            continue;
          double pt = tracksItr->pt();
          if (pt < min_Pt_ || pt > max_Pt_)
            continue;
          double vz = tracksItr->vz();
          if (fabs(vz - vzmax) > min_sep_)
            continue;
