...
void HLTVertexFilter::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
bool HLTVertexFilter::hltFilter(edm::Event& event,
  ...
  if (vertices.isValid()) {
    for (auto const& vertex : *vertices) {
      if (vertex.isValid() and not vertex.isFake() and (vertex.chi2() <= m_maxChi2) and (vertex.ndof() >= m_minNDoF) and
          (std::abs(vertex.z()) <= m_maxZ) and (std::abs(vertex.y()) <= m_maxD0) and
          (std::abs(vertex.x()) <= m_maxD0) and
          (std::sqrt(std::pow(vertex.x(), 2) + std::pow(vertex.y(), 2)) <= m_maxD0))
        ++n;
    }
  }
