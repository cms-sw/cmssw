...
bool HLTDisplacedtktktkFilter::hltFilter(edm::Event& iEvent,
  ...
  for (auto const& displacedVertex : displacedVertexColl) {
    ...
    if (displacedVertex.tracksSize() != 3) {
      edm::LogError("HLTDisplacedtktktkFilter") << "HLTDisplacedtktktkFilter: ERROR: the tktktk vertex must have "
                                                   "exactly three tracks by definition. It now has n tracks = "
    ...
    if (iFoundRefs < 3) {
      edm::LogError("HLTDisplacedtktktkFilter") << "HLTDisplacedtktktkFilter: ERROR: the tracks matched with the "
                                                   "tktktk vertex tracks should be at least three by definition."
                                                << std::endl;
      return false;
    }
    ...
    GlobalPoint displacementFromBeamspot(
        -1 * ((vertexBeamSpot.x0() - vpoint.x()) + (vpoint.z() - vertexBeamSpot.z0()) * vertexBeamSpot.dxdz()),
        -1 * ((vertexBeamSpot.y0() - vpoint.y()) + (vpoint.z() - vertexBeamSpot.z0()) * vertexBeamSpot.dydz()),
        0);
    ...
    ref3 = reco::RecoChargedCandidateRef(
        edm::Ref<reco::RecoChargedCandidateCollection>(trackcands, distance(trackcands->begin(), cand3)));
