	...
	if (electrons.isValid()) {  
		...
		for (reco::GsfElectronCollection::const_iterator i = myelectrons.begin(); i != myelectrons.end(); i++) {  
			...
			if(i->gsfTrack().isNonnull()){  
				elNLostHits[iel]   = i->gsfTrack()->getHitPattern().numberOfLostHits(reco::HitPattern::MISSING_INNER_HITS);
			...
			elmishits[iel] = i->gsfTrack()->getHitPattern().numberOfHits(reco::HitPattern::MISSING_INNER_HITS); 
