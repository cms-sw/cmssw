...
void HLTMuon::setup(const edm::ParameterSet& pSet, TTree* HltTree) {
    ...
    for ( std::vector<std::string>::iterator iParam = parameterNames.begin();
        ...
        else if ( (*iParam) == "DoL1Muons" ) _DoL1Muons =  myEmParams.getUntrackedParameter<bool>( *iParam ,true);
        else if ( (*iParam) == "DoL2Muons" ) _DoL2Muons =  myEmParams.getUntrackedParameter<bool>( *iParam ,true);
        else if ( (*iParam) == "DoL3Muons" ) _DoL3Muons =  myEmParams.getUntrackedParameter<bool>( *iParam ,true);
        else if ( (*iParam) == "DoOfflineMuons" ) _DoOfflineMuons =  myEmParams.getUntrackedParameter<bool>( *iParam , true);
        else if ( (*iParam) == "DoQuarkonias" ) _DoQuarkonias =  myEmParams.getUntrackedParameter<bool>( *iParam , true);
    }
...
void HLTMuon::analyze(const edm::Handle<reco::MuonCollection>                 & Muon,
    ...
    }else {nOniaPixelCand = 0; nOniaTrackCand = 0;}
