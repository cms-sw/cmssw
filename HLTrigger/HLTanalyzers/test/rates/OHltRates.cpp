...
void ShowUsage() {
  ...
  cout << "default:  ./OHltRates -1 default startup 2008-June-10 0 " << endl;
}
...
int main(int argc, char *argv[]){
  ...
  collisionRate = (nFilledBunches / maxFilledBunches) / bunchCrossingTime ;  // Hz
  ...
  if( sConditions.CompareTo("startup") == 0) {
    ...
    // Ztautau
    ProcFil.clear();
    ProcFil.push_back("Ztautau_hltree_1pb.root");
    ...
    xsec.push_back(1.63E8); // PYTHIA cross-section for QCD 30-50 (pb)
    skmeff.push_back(1.);  //

    // QCD 50-80
    ProcFil.clear();
    ProcFil.push_back("QCD_50_80_hltree_1pb.root");
    TabChain.push_back(new TChain("HltTree"));
    for (unsigned int ipfile = 0; ipfile < ProcFil.size(); ipfile++){
      TabChain.back()->Add(ProcFil[ipfile]);
    }
    ...
    xsec.push_back(2.16E7); // PYTHIA cross-section for QCD 50-80 (pb)
    skmeff.push_back(1.);  //

    // QCD 80-120
    ProcFil.clear();
    ProcFil.push_back("QCD_80_120_hltree_1pb.root");
    TabChain.push_back(new TChain("HltTree"));
    for (unsigned int ipfile = 0; ipfile < ProcFil.size(); ipfile++){
      TabChain.back()->Add(ProcFil[ipfile]);
    }
    ...
    xsec.push_back(3.08E6); // PYTHIA cross-section for QCD 80-120 (pb)
    skmeff.push_back(1.);  //
...
void BookMenu_Default(OHltMenu*  menu, double &iLumi, double &nBunches) {
  ...
  //menu->AddHlt("HLT2Muon3","L1_DoubleMu3",1,1,"NI (3,3)","new");              // 
  ...
  menu->AddHlt("CandHLT1ElectronStartup","L1_SingleIsoEG12",1,1,"-","new");
  menu->AddHlt("CandHLT1ElectronRelaxedStartup","L1_SingleIsoEG15",1,1,"-","new");
  menu->AddHlt("CandHLT2ElectronStartup","L1_DoubleIsoEG8",1,1,"-","new");
  menu->AddHlt("CandHLT2ElectronRelaxedStartup","L1_DoubleIsoEG10",1,1,"-","new");
  ...
  menu->AddHlt("HLTMinBias","L1_MinBias_HTT10",3E5,1,"-","1e32");  // L1: 300000
  menu->AddHlt("HLTMinBiasPixel","L1_ZeroBias",3E5,1,"- ","1e32");  // L1: 300000
  ...
  menu->AddHlt("HLTZeroBias","L1_ZeroBias",3E5,1,"-","1e32"); // L1: 300000 
}
