...
class OHltMenu {
 public:
  ...
  inline std::map<TString,float>&       GetPrescaleMap() {return prescales;}
  inline float                          GetPrescale(int i) {return prescales[names[i]];}
  inline float                          GetPrescale(TString s) {return prescales[s];}
  ...
  void AddTrigger(TString trigname, float prescale, float eventSize);
  void AddTrigger(TString trigname, TString seedcond, float prescale, float eventSize, int referencePrescale);
  ...
  inline std::map<TString,float>& GetL1PrescaleMap() {return L1prescales;}
  inline float                GetL1Prescale(int i) {return L1prescales[L1names[i]];}
  inline float                GetL1Prescale(TString s) {return L1prescales[s];}
