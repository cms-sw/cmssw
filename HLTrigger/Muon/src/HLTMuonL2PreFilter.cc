...
bool HLTMuonL2PreFilter::hltFilter(edm::Event& iEvent, const edm::EventSetup& iSetup, trigger::TriggerFilterObjectWithRefs & filterproduct) const
{
  ...
  for(RecoChargedCandidateCollection::const_iterator cand=allMuons->begin(); cand!=allMuons->end(); cand++){
	...
	if(mu->getHitPattern().muonStationsWithAnyHits(HitPattern::TRACK_HITS) < minNstations_[i]) {
	  failNstations=true;
	}
	...
	if( cutOnChambers_ &&
	    (mu->getHitPattern().dtStationsWithAnyHits(HitPattern::TRACK_HITS) 
         + mu->getHitPattern().cscStationsWithAnyHits(HitPattern::TRACK_HITS) < minNchambers_[i]) ) {
	  failNchambers=true;
	}
	break;
      }
  ...
  if(edm::isDebugEnabled()){
    ...
    for (RecoChargedCandidateCollection::const_iterator cand = allMuons->begin(); cand != allMuons->end(); cand++) {
      ...
      ss<<setprecision(2)
        ...
        <<'\t'<<fixed<<mu->eta()
        <<'\t'<<fixed<<mu->phi()
        <<'\t'<<mu->getHitPattern().muonStationsWithAnyHits(HitPattern::TRACK_HITS)
