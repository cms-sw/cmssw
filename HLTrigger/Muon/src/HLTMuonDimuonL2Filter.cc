...
HLTMuonDimuonL2Filter::hltFilter(edm::Event& iEvent, const edm::EventSetup& iSetup, trigger::TriggerFilterObjectWithRefs & filterproduct) const
{
   ...
   for (cand1=mucands->begin(); cand1!=mucands->end(); cand1++) {
      ...
      // number of stations
      if (tk1->getHitPattern().muonStationsWithAnyHits(HitPattern::TRACK_HITS) < min_Nstations_) continue;

      // number of chambers
      if(tk1->getHitPattern().dtStationsWithAnyHits(HitPattern::TRACK_HITS)
              + tk1->getHitPattern().cscStationsWithAnyHits(HitPattern::TRACK_HITS) < min_Nchambers_) continue;
      ...
      for (; cand2!=mucands->end(); cand2++) {
	    ...
	    // number of stations
	    if (tk2->getHitPattern().muonStationsWithAnyHits(HitPattern::TRACK_HITS) < min_Nstations_) continue;

	    // number of chambers
	    if(tk2->getHitPattern().dtStationsWithAnyHits(HitPattern::TRACK_HITS) +
	       tk2->getHitPattern().cscStationsWithAnyHits(HitPattern::TRACK_HITS) < min_Nchambers_) continue;
