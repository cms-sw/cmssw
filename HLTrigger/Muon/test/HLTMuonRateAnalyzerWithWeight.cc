...
void HLTMuonRateAnalyzerWithWeight::beginJob(){
  ...
  snprintf(chtitle, 255, "Light Quark events Efficiency (%%) vs L1 Pt threshold (GeV), label=%s", theL1CollectionLabel.encode().c_str());
  ...
  snprintf(chtitle, 255, "Light Quark events Rate (Hz) vs L1 Pt threshold (GeV), label=%s, L=%.2E (cm^{-2} s^{-1})", theL1CollectionLabel.encode().c_str(), theLuminosity*1.e33);
  ...
  snprintf(chtitle, 255, "BC Quark events Efficiency (%%) vs L1 Pt threshold (GeV), label=%s", theL1CollectionLabel.encode().c_str());
  ...
  snprintf(chtitle, 255, "BC Quark events Rate (Hz) vs L1 Pt threshold (GeV), label=%s, L=%.2E (cm^{-2} s^{-1})", theL1CollectionLabel.encode().c_str(), theLuminosity*1.e33);
  ...
  for (unsigned int i=0; i<theHLTCollectionLabels.size(); i++) {
      ...
      snprintf(chtitle, 255, "Light Quark events Efficiency (%%) vs HLT Pt threshold (GeV), label=%s", theHLTCollectionLabels[i].encode().c_str());
      ...
      snprintf(chtitle, 255, "Light Quark events Rate (Hz) vs HLT Pt threshold (GeV), label=%s, L=%.2E (cm^{-2} s^{-1})", theHLTCollectionLabels[i].encode().c_str(), theLuminosity*1.e33);
      ...
      snprintf(chtitle, 255, "BC Quark events Efficiency (%%) vs HLT Pt threshold (GeV), label=%s", theHLTCollectionLabels[i].encode().c_str());
      ...
      snprintf(chtitle, 255, "BC Quark events Rate (Hz) vs HLT Pt threshold (GeV), label=%s, L=%.2E (cm^{-2} s^{-1})", theHLTCollectionLabels[i].encode().c_str(), theLuminosity*1.e33);
...
void HLTMuonRateAnalyzerWithWeight::analyze(const Event & event, const EventSetup& eventSetup){
  ...
  if(bcevent)theNumberOfBCEvents += this_event_weight;
  ...
  for (unsigned int j=0; j<theNbins; j++) {
	...
	if (bcevent) hBCL1eff->Fill(ptcut,this_event_weight);
        else hLightL1eff->Fill(ptcut,this_event_weight);
      }      
	  ...
	  for (HepMC::GenVertex::particles_in_const_iterator ic=parent->particles_in_const_begin() ; ic!=parent->particles_in_const_end() ; ic++ )
	    {
	      ...
	      if (apid>10000)apid=apid-(apid/10000)*10000;
              if (apid>1000) apid/=1000;
              if (apid>100 && apid != 130) apid/=100;
	      LogInfo("HLTMuonRateAnalyzerWithWeight") << " It will be treated as "<<apid;
	      if ( apid==5)AdditionalWeight=1./8.4;        //b mesons
	      else if ( apid==4)AdditionalWeight=1./6.0; //c mesons
	      else if ( apid==15 )AdditionalWeight=1./8.7;                  //taus
	      else if ( apid==3 || apid==130 )AdditionalWeight=1./7.3; //s-mesons
	      else if ( apid==2) AdditionalWeight=1./0.8;       //pions
	    }
	}
      }
