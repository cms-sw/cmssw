...
HLTMuonL1TtoL3TkPreFilter::HLTMuonL1TtoL3TkPreFilter(const ParameterSet& iConfig)
    : HLTFilter(iConfig),
  ...
  LogDebug("HLTMuonL1TtoL3TkPreFilter") << " CandTag/MinN/MaxEta/MinNhits/MaxDr/MaxDz/MinPt/NSigmaPt : "
...
bool HLTMuonL1TtoL3TkPreFilter::hltFilter(Event& iEvent,
  ...
  for (; L1toL3s_it != L1toL3s_end; ++L1toL3s_it) {
    ...
    for (; iTk != maxItk; iTk++) {
      ...
      LogTrace("HLTMuonL1TtoL3TkPreFilter") << " ...Muon in loop, pt= " << pt << ", ptLx= " << ptLx;
      if (ptLx < min_Pt_)
        continue;
  ...
  for (auto& i : vref) {
    TrackRef tk = i->track();
    LogDebug("HLTMuonL1TtoL3TkPreFilter") << " Track passing filter: pt= " << tk->pt() << ", eta: " << tk->eta();
  }

  // filter decision
  const bool accept((int)n >= min_N_);

  LogDebug("HLTMuonL1TtoL3TkPreFilter") << " >>>>> Result of HLTMuonL1TtoL3TkPreFilter is " << accept
                                        << ", number of muons passing thresholds= " << n;

  return accept;
}

bool HLTMuonL1TtoL3TkPreFilter::triggeredAtL1(const l1t::MuonRef& l1mu, std::vector<l1t::MuonRef>& vcands) const {
  bool ok = false;
