...
protected:
  ...
  const DualTrajectoryInput
  referenceStateAndRecHits(const ConstTrajTrackPair &track) const;

  const TrajectoryStateOnSurface
  propagateExternal(const TrajectoryStateOnSurface &external,
...
DualTrajectoryFactory::trajectories(const edm::EventSetup &setup,
  ...
  if (magneticField->inTesla(GlobalPoint(0., 0., 0.)).mag2() < 1.e-6) {
    edm::LogWarning("Alignment")
        ...
        << "B-field in z is "
        << magneticField->inTesla(GlobalPoint(0., 0., 0.)).z()
        << ": You should probably use the DualBzeroTrajectoryFactory\n"
        << "or fix this code to switch automatically as the "
           "ReferenceTrajectoryFactory does.";
  }
  ...
  while (itTracks != tracks.end()) {
    ...
    if (input.refTsos.isValid()) {
      ...
      ReferenceTrajectoryPtr ptr(new DualReferenceTrajectory(
          input.refTsos, input.fwdRecHits, input.bwdRecHits,
  ...
  while (itTracks != tracks.end()) {
    ...
    if (input.refTsos.isValid()) {
      if ((*itExternal).isValid()) {
        TrajectoryStateOnSurface propExternal = propagateExternal(
            *itExternal, input.refTsos.surface(), magneticField.product());
        ...
        config.allowZeroMaterial = allowZeroMaterial_;
        ReferenceTrajectoryPtr ptr(new DualReferenceTrajectory(
            propExternal, input.fwdRecHits, input.bwdRecHits,
            magneticField.product(), beamSpot, config));

        AlgebraicSymMatrix externalParamErrors(
            asHepMatrix<5>(propExternal.localError().matrix()));
...
DualTrajectoryFactory::referenceStateAndRecHits(
  ...
  // get the trajectory measurements in the correct order, i.e. reverse if
  // needed
  Trajectory::DataContainer allTrajMeas =
...
const TrajectoryStateOnSurface DualTrajectoryFactory::propagateExternal(
    const TrajectoryStateOnSurface &external, const Surface &surface,
    const MagneticField *magField) const {
...
DEFINE_EDM_PLUGIN(TrajectoryFactoryPlugin, DualTrajectoryFactory,
