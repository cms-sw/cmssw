  ...
  while (itTracks != tracks.end()) {
    ...
    if (input.first.isValid()) {
      ...
      trajectories.push_back(ReferenceTrajectoryPtr(
          new BzeroReferenceTrajectory(input.first, input.second,
                                       magneticField.product(), beamSpot,
                                       config)));
    }
  ...
  while (itTracks != tracks.end()) {
    ...
    if (input.first.isValid()) {
      if ((*itExternal).isValid() &&
        ...
        config.hitsAreReverse = false;
        ReferenceTrajectoryPtr refTraj(new BzeroReferenceTrajectory(
            *itExternal, input.second, magneticField.product(), beamSpot,
            config));
...
DEFINE_EDM_PLUGIN(TrajectoryFactoryPlugin, BzeroReferenceTrajectoryFactory,
