...
/// A factory that produces instances of class ReferenceTrajectory from a given
/// TrajTrackPairCollection. If |B| = 0 T and configuration parameter
/// UseBzeroIfFieldOff is True, hand-over to the
...
public:
  ...
  const ReferenceTrajectoryCollection
...
ReferenceTrajectoryFactory::ReferenceTrajectoryFactory(
    const ReferenceTrajectoryFactory &other)
    : TrajectoryFactoryBase(other), theMass(other.theMass),
      theUseBzeroIfFieldOff(other.theUseBzeroIfFieldOff),
      theBzeroFactory(nullptr) // copy data members, but no double pointing to
                               // same Bzero factory...
{}
...
ReferenceTrajectoryFactory::trajectories(
    const edm::EventSetup &setup, const ConstTrajTrackPairCollection &tracks,
    const reco::BeamSpot &beamSpot) const {
  edm::ESHandle<MagneticField> magneticField;
  setup.get<IdealMagneticFieldRecord>().get(magneticField);
  if (theUseBzeroIfFieldOff &&
      magneticField->inTesla(GlobalPoint(0., 0., 0.)).mag2() < 1.e-6) {
  ...
  while (itTracks != tracks.end()) {
    ...
    // Check input: If all hits were rejected, the TSOS is initialized as
    // invalid.
    if (input.first.isValid()) {
      ReferenceTrajectoryBase::Config config(materialEffects(),
      ...
      trajectories.push_back(ReferenceTrajectoryPtr(
          new ReferenceTrajectory(input.first, input.second,
                                  magneticField.product(), beamSpot, config)));
    }
...
ReferenceTrajectoryFactory::trajectories(
  ...
  if (theUseBzeroIfFieldOff &&
      magneticField->inTesla(GlobalPoint(0., 0., 0.)).mag2() < 1.e-6) {
    return this->bzeroFactory()->trajectories(setup, tracks, external,
                                              beamSpot);
  }
  ...
  while (itTracks != tracks.end()) {
    ...
    if (input.first.isValid()) {
      if ((*itExternal).isValid() &&
        ...
        config.includeAPEs = includeAPEs_;
        config.allowZeroMaterial = allowZeroMaterial_;
        // set the flag for reversing the RecHits to false, since they are
        // already in the correct order.
        config.hitsAreReverse = false;
        ReferenceTrajectoryPtr refTraj(
            new ReferenceTrajectory(*itExternal, input.second,
                                    magneticField.product(), beamSpot, config));

        AlgebraicSymMatrix externalParamErrors(
            asHepMatrix<5>((*itExternal).localError().matrix()));
        ...
        config.allowZeroMaterial = allowZeroMaterial_;
        config.hitsAreReverse = false;
        trajectories.push_back(ReferenceTrajectoryPtr(new ReferenceTrajectory(
            input.first, input.second, magneticField.product(), beamSpot,
            config)));
      }
    }
...
const TrajectoryFactoryBase *ReferenceTrajectoryFactory::bzeroFactory() const {
  if (!theBzeroFactory) {
    ...
    // the momentum parameter as expected by BzeroReferenceTrajectoryFactory and
    // create it:
    ...
    pset.addParameter("TrajectoryFactoryName",
                      std::string("BzeroReferenceTrajectoryFactory"));
    pset.addParameter("MomentumEstimate",
                      myPset.getParameter<double>("MomentumEstimateFieldOff"));
    theBzeroFactory = new BzeroReferenceTrajectoryFactory(pset);
  }
  return theBzeroFactory;
}

DEFINE_EDM_PLUGIN(TrajectoryFactoryPlugin, ReferenceTrajectoryFactory,
