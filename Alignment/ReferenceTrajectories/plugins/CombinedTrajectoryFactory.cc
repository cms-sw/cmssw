...
/// A factory that can combine the functionality of several 'trajectory
/// factories'. At construction time, it is given an ordered list of what kinds
/// factories it should use. When called, all the factories are called one after
/// each other,
...
/// can't produce a trajectory from two given tracks, the tracks can still be
/// used for 'ordinary' reference trajectories (see also
/// TrajectoryFactories.cff).
...
CombinedTrajectoryFactory::CombinedTrajectoryFactory(
    ...
    : TrajectoryFactoryBase(config),
      theUseAllFactories(config.getParameter<bool>("useAllFactories")) {
  vector<string> factoryNames =
      config.getParameter<vector<string>>("TrajectoryFactoryNames");
  vector<string>::iterator itFactoryName;
  for (itFactoryName = factoryNames.begin();
       itFactoryName != factoryNames.end(); ++itFactoryName) {
    // auto_ptr to avoid missing a delete due to throw...
    std::unique_ptr<TObjArray> namePset(
        TString((*itFactoryName).c_str()).Tokenize(","));
    ...
    theFactories.push_back(TrajectoryFactoryPlugin::get()->create(
        namePset->At(0)->GetName(), factoryCfg));
  }
}
...
CombinedTrajectoryFactory::trajectories(
  ...
  for (itFactory = theFactories.begin(); itFactory != theFactories.end();
       ++itFactory) {
    tmpTrajectories = (*itFactory)->trajectories(setup, tracks, beamSpot);
    trajectories.insert(trajectories.end(), tmpTrajectories.begin(),
                        tmpTrajectories.end());
...
CombinedTrajectoryFactory::trajectories(
  ...
  for (itFactory = theFactories.begin(); itFactory != theFactories.end();
       ++itFactory) {
    tmpTrajectories =
        (*itFactory)->trajectories(setup, tracks, external, beamSpot);
    trajectories.insert(trajectories.end(), tmpTrajectories.begin(),
                        tmpTrajectories.end());
...
DEFINE_EDM_PLUGIN(TrajectoryFactoryPlugin, CombinedTrajectoryFactory,
