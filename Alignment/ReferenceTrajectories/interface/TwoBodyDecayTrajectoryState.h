...
/** Computes the trajectory states and their derivatives w.r.t. the decay
 * parameters.
 */
...
public:
  typedef std::pair<TrajectoryStateOnSurface, TrajectoryStateOnSurface>
      TsosContainer;
  typedef std::pair<AlgebraicMatrix, AlgebraicMatrix> Derivatives;

  /** The constructor takes the two trajectory states that are to be updated
   * (typically the innermost trajectory states of two tracks) and the decay
   * parameters.
   */
  TwoBodyDecayTrajectoryState(const TsosContainer &tsos,
                              const TwoBodyDecay &tbd, double particleMass,
  ...
  inline const TsosContainer &
  trajectoryStates(bool useRefittedState = true) const {
