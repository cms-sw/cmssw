 ...
 *
 * similarly the local x/y cordinates of the reference trajectory with
 *covariance,
 *
 * the parameters of the (ideal) 'trajectory'
 * (e.g. 5 values for a single track or 9 for a two-track-state with vertex
 *constraint),
 *
 ...
 *
 * Take care to check validity of the calculation (isValid()) before using the
 *results.
 *
 *.............................................................................
 *
 * 090730 C. Kleinwort: 'Break Points' introduced for better description of
 *multiple scattering (correlations)
 *
 * For each detector layer (in the hit collection) two ortogonal scattering
 *angles are introduced as new local track parameters ("break points"). To
 *constrain those to the expected mean (=0.) and variance (~1/p^2 X/X0)
 *corresponding measurements are added. Invalid hits may be used to produce
 *break points too (UseInvalidHits = True).
 *
 * Break points have been implemented for: ReferenceTrajectory,
 *BzeroReferenceTrajectory, DualReferenceTrajectory and
 *DualBzeroReferenceTrajectory For 'Dual' trajectories they make no sense as
 *only the correlations between the hits in a track half are accounted for and
 *not those between the halves!
 *
 * Break Points are selected by TrajectoryFactory.MaterialEffects =
 *"BreakPoints"
 *
 * 090909 C. Kleinwort: 'Broken Lines' introduced for description of multiple
 *scattering (V. Blobel, Nuclear Instruments and Methods A, 566 (2006), pp.
 *14-17) Fine Broken Lines are selected by TrajectoryFactory.MaterialEffects =
 *"BrokenLinesFine" (exact derivatives) Coarse Broken Lines are selected by
 *TrajectoryFactory.MaterialEffects = "BrokenLines[Coarse]" (approximate
 *derivatives, closeby hits (ds<1cm) combined)
 *
 * 091127 C. Kleinwort:
 ...
 *
 * 141103 C. Kleinwort: 'General Broken Lines' introduced for description of
 *multiple scattering (C. Kleinwort, Nuclear Instruments and Methods A, 673
 *(2012), pp. 107-110) needs GBL version >= V01-13-00 (from svnsrv.desy.de)
 *        Selected by TrajectoryFactory.MaterialEffects = "LocalGBL" or =
 *"CurvlinGBL" (for trajectory constructed in local or curvilinear system)
 */
...
public:
  typedef ReferenceCountingPointer<ReferenceTrajectoryBase>
  ...
  struct Config {
    Config(MaterialEffects matEff, PropagationDirection direction,
        ...
        : materialEffects(matEff), propDir(direction), mass(m),
  ...
  /** Returns the full covariance matrix of the measurements. ORCA-equivalent:
   * covariance()
   */
   ...
   *  trajectory (i.e. trajectoryPositions) w.r.t. the initial
   ...
   */
  std::vector<std::pair<std::vector<gbl::GblPoint>, Eigen::MatrixXd>> &
  gblInput() {
    return theGblInput;
  }
   ...
   */
  inline const AlgebraicSymMatrix &parameterErrors() const {
  ...
  inline unsigned int numberOfVirtualMeas() const {
    return theNumberOfVirtualMeas;
  }
  inline unsigned int numberOfVirtualPar() const {
    return theNumberOfVirtualPars;
  }
  inline unsigned int numberOfHitMeas() const {
...
protected:
  ...
  unsigned int numberOfUsedRecHits(
      const TransientTrackingRecHit::ConstRecHitContainer &recHits) const;
  bool
  ...
  unsigned int
      theNumberOfVirtualPars; // number of parameters for virtual measurements
  ...
  // CHK for beamspot   transformation trajectory parameter to curvilinear at
  // refTSos
  AlgebraicMatrix theInnerTrajectoryToCurvilinear;
  // CHK for TwoBodyD.  transformation local to trajectory parameter at refTsos
  AlgebraicMatrix theInnerLocalToTrajectory;
  // CHK GBL input:     list of (list of points on trajectory and transformation
  // at inner (first) point)
  std::vector<std::pair<std::vector<gbl::GblPoint>, Eigen::MatrixXd>>
      theGblInput;
  int theNomField;
