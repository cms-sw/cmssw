...
public:
  ...
  BeamSpotTransientTrackingRecHit(const reco::BeamSpot &beamSpot,
                                  const BeamSpotGeomDet *geom, double phi)
      : TValidTrackingRecHit(*geom) {

    localPosition_ = det()->toLocal(
        GlobalPoint(beamSpot.x0(), beamSpot.y0(), beamSpot.z0()));
...
private:
  ...
  operator=(const BeamSpotTransientTrackingRecHit &t) {
    return *(this);
  }
