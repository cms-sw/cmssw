...
/**
 * Author     : Gero Flucke (based on code by Edmund Widl replacing ORCA's
 * TkReferenceTrack) date       : 2006/09/17 last update: $Date: 2012/06/20
 * 12:07:28 $ by         : $Author: flucke $
 *
...
public:
  ...
  /**Constructor with Tsos at first hit (in physical order) and list of hits
     ...
     the magnetic field and beamSpot of the event are needed for propagations
     etc.
   */
  ReferenceTrajectory(
...
protected:
  // ReferenceTrajectory(unsigned int nPar, unsigned int nHits, MaterialEffects
  // materialEffects);
   ...
   */
  MaterialEffectsUpdator *createUpdator(MaterialEffects materialEffects,
                                        double mass) const;
   ...
   */
  virtual void fillMeasurementAndError(
      const TransientTrackingRecHit::ConstRecHitPointer &hitPtr,
      unsigned int iRow, const TrajectoryStateOnSurface &updatedTsos);
   ...
   */
  virtual bool addMaterialEffectsCov(
   ...
   */
  virtual bool
  addMaterialEffectsBp(const std::vector<AlgebraicMatrix> &allJacobians,
                       const std::vector<AlgebraicMatrix> &allProjections,
                       const std::vector<AlgebraicSymMatrix> &allCurvChanges,
                       const std::vector<AlgebraicSymMatrix> &allDeltaParaCovs,
   ...
   */
  virtual bool
  addMaterialEffectsBrl(const std::vector<AlgebraicMatrix> &allJacobians,
                        const std::vector<AlgebraicMatrix> &allProjections,
                        const std::vector<AlgebraicSymMatrix> &allCurvChanges,
                        const std::vector<AlgebraicSymMatrix> &allDeltaParaCovs,
                        const std::vector<AlgebraicMatrix> &allLocalToCurv,
                        const GlobalTrajectoryParameters &gtp);
  /** internal methods to add material effects using broken lines (coarse
   * version)
   */
  virtual bool
  addMaterialEffectsBrl(const std::vector<AlgebraicMatrix> &allProjections,
                        const std::vector<AlgebraicSymMatrix> &allDeltaParaCovs,
                        const std::vector<AlgebraicMatrix> &allLocalToCurv,
   ...
   * local system)
   */
  virtual bool addMaterialEffectsLocalGbl(
      const std::vector<AlgebraicMatrix> &allJacobians,
      const std::vector<AlgebraicMatrix> &allProjections,
   ...
   * curvilinear system)
   */
  virtual bool addMaterialEffectsCurvlinGbl(
      const std::vector<AlgebraicMatrix> &allJacobians,
      const std::vector<AlgebraicMatrix> &allProjections,
      const std::vector<AlgebraicSymMatrix> &allCurvChanges,
      const std::vector<AlgebraicSymMatrix> &allDeltaParaCovs,
  ...
  /// Don't care for propagation direction 'anyDirection' - in that case the
  /// material effects are anyway not updated ...
  inline SurfaceSide surfaceSide(const PropagationDirection dir) const {
    return (dir == alongMomentum) ? SurfaceSideDefinition::beforeSurface
                                  : SurfaceSideDefinition::afterSurface;
  }

  /** first (generic) helper to get the projection matrix
   */
  AlgebraicMatrix getHitProjectionMatrix(
  ...
  AlgebraicMatrix getHitProjectionMatrixT(
      const TransientTrackingRecHit::ConstRecHitPointer &recHit) const;
...
private:
  ...
  template <typename Derived>
  void clhep2eigen(const AlgebraicSymMatrix &in,
