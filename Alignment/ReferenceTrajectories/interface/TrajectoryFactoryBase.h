...
/// Base class for factories producing reference trajectories, i.e. instances of
/// classes deriving from ReferenceTrajectoryBase, from a
...
public:
  ...
  trajectories(const edm::EventSetup &setup,
  ...
  inline MaterialEffects materialEffects(void) const {
    return materialEffects_;
  }
  inline PropagationDirection propagationDirection(void) const {
    return propDir_;
  }
  inline const edm::ParameterSet &configuration() const { return cfg_; }
  inline unsigned int tracksPerTrajectory() const {
    return tracksPerTrajectory_;
  }

protected:
  virtual const TrajectoryInput
  innermostStateAndRecHits(const ConstTrajTrackPair &track) const;
  virtual const Trajectory::DataContainer
  orderedTrajectoryMeasurements(const Trajectory &trajectory) const;
  bool sameSurface(const Surface &s1, const Surface &s2) const;
  bool
  useRecHit(const TransientTrackingRecHit::ConstRecHitPointer &hitPtr) const;
...
private:
  ...
  const edm::ParameterSet
      cfg_; // need to keep for possible re-use after constructor... :-(
