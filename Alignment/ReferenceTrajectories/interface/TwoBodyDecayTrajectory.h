...
public:
  ...
  typedef std::pair<ConstRecHitContainer, ConstRecHitContainer>
   ...
   */
  inline const std::pair<int, int> numberOfRecHits(void) {
...
private:
  bool construct(const TwoBodyDecayTrajectoryState &state,
                 const ConstRecHitCollection &recHits,
                 const MagneticField *field, const reco::BeamSpot &beamSpot);
  ...
  void constructSingleTsosWithErrors(const TrajectoryStateOnSurface &tsos,
                                     int iTsos, const MagneticField *field);
