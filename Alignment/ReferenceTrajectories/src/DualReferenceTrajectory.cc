...
DualReferenceTrajectory::DualReferenceTrajectory(
    ...
    const ReferenceTrajectoryBase::Config &config)
    : ReferenceTrajectoryBase(nPar, nHits, 0, 0), mass_(config.mass),
...
bool DualReferenceTrajectory::construct(
  ...
  const std::vector<TrajectoryStateOnSurface> &fwdTsosVec =
      fwdTraj->trajectoryStates();
  const std::vector<TrajectoryStateOnSurface> &bwdTsosVec =
      bwdTraj->trajectoryStates();
  ...
  theMeasurements.sub(nFwdMeas + 1,
                      bwdTraj->measurements().sub(nMeasPerHit + 1, nBwdMeas));

  theMeasurementsCov.sub(1, fwdTraj->measurementErrors().sub(1, nFwdMeas));
  theMeasurementsCov.sub(nFwdMeas + 1, bwdTraj->measurementErrors().sub(
                                           nMeasPerHit + 1, nBwdMeas));

  theTrajectoryPositions.sub(1, fwdTraj->trajectoryPositions());
  theTrajectoryPositions.sub(nFwdMeas + 1, bwdTraj->trajectoryPositions().sub(
                                               nMeasPerHit + 1, nBwdMeas));

  theTrajectoryPositionCov.sub(1, fwdTraj->trajectoryPositionErrors());
  theTrajectoryPositionCov.sub(
      nFwdMeas + 1,
      bwdTraj->trajectoryPositionErrors().sub(nMeasPerHit + 1, nBwdMeas));
  ...
  // DUAL with break points makes no sense: (MS) correlations between the two
  // parts are lost !
  if (nFwdBP > 0) {
    theMeasurements.sub(nMeas + 1, fwdTraj->measurements().sub(
                                       nFwdMeas + 1, nFwdMeas + nFwdBP));
    theMeasurementsCov.sub(nMeas + 1, fwdTraj->measurementErrors().sub(
                                          nFwdMeas + 1, nFwdMeas + nFwdBP));
    theDerivatives.sub(
        1, nParam + 1,
        fwdTraj->derivatives().sub(1, nFwdMeas, nParam + 1, nParam + nFwdBP));
    theDerivatives.sub(nMeas + 1, nParam + 1,
                       fwdTraj->derivatives().sub(nFwdMeas + 1,
                                                  nFwdMeas + nFwdBP, nParam + 1,
                                                  nParam + nFwdBP));
  }
  if (nBwdBP > 0) {
    theMeasurements.sub(
        nMeas + nFwdBP + 1,
        bwdTraj->measurements().sub(nBwdMeas + 1, nBwdMeas + nBwdBP));
    theMeasurementsCov.sub(
        nMeas + nFwdBP + 1,
        bwdTraj->measurementErrors().sub(nBwdMeas + 1, nBwdMeas + nBwdBP));
    theDerivatives.sub(nFwdMeas + 1, nParam + nFwdBP + 1,
                       bwdTraj->derivatives().sub(nMeasPerHit + 1, nBwdMeas,
                                                  nParam + 1, nParam + nBwdBP));
    theDerivatives.sub(nMeas + nFwdBP + 1, nParam + nFwdBP + 1,
                       bwdTraj->derivatives().sub(nBwdMeas + 1,
                                                  nBwdMeas + nBwdBP, nParam + 1,
                                                  nParam + nBwdBP));
  }
...
ReferenceTrajectory *DualReferenceTrajectory::construct(
    ...
    const reco::BeamSpot &beamSpot, const bool revertDirection) const {
  if (materialEffects_ >= breakPoints)
    throw cms::Exception("BadConfig")
        << "[DualReferenceTrajectory::construct] Wrong MaterialEffects: "
        << materialEffects_;
  ...
  ReferenceTrajectoryBase::Config config(
      materialEffects_,
      (revertDirection ? oppositeDirection(propDir_) : propDir_), mass_);
  config.useBeamSpot = useBeamSpot_;
  config.hitsAreReverse = false;
  return new ReferenceTrajectory(referenceTsos, recHits, magField, beamSpot,
                                 config);
}

AlgebraicVector DualReferenceTrajectory::extractParameters(
