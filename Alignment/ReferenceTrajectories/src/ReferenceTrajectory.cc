...
//  TkReferenceTrack) date       : 2006/09/17 last update: $Date: 2012/12/25
//  16:42:04 $ by         : $Author: innocent $
...
ReferenceTrajectory::ReferenceTrajectory(
    ...
    : ReferenceTrajectoryBase(
          (config.materialEffects >= brokenLinesCoarse)
              ? 1
              : refTsos.localParameters().mixedFormatVector().kSize,
          ...
          (config.materialEffects >= brokenLinesCoarse)
              ...
              : ((config.materialEffects == breakPoints)
                     ? 2 * ((config.useBeamSpot) ? recHits.size() + 1
                                                 : recHits.size()) -
                           2
                     : 0),
          (config.materialEffects >= brokenLinesCoarse)
              ...
              : ((config.materialEffects == breakPoints)
                     ? 2 * ((config.useBeamSpot) ? recHits.size() + 1
                           ...
                           2
                     : 0)),
      mass_(config.mass), materialEffects_(config.materialEffects),
      propDir_(config.propDir), useBeamSpot_(config.useBeamSpot),
...
ReferenceTrajectory::ReferenceTrajectory(
    unsigned int nPar, unsigned int nHits,
    const ReferenceTrajectoryBase::Config &config)
    : ReferenceTrajectoryBase(
          (config.materialEffects >= brokenLinesCoarse) ? 1 : nPar, nHits,
          (config.materialEffects >= brokenLinesCoarse)
              ? 2 * nHits
...
bool ReferenceTrajectory::construct(
    const TrajectoryStateOnSurface &refTsos,
    const TransientTrackingRecHit::ConstRecHitContainer &recHits,
    const MagneticField *magField, const reco::BeamSpot &beamSpot) {
  ...
  // auto_ptr to avoid memory leaks in case of not reaching delete at end of
  // method:
  ...
  AlgebraicMatrix fullJacobian(theParameters.num_row(),
                               theParameters.num_row());
  ...
  // local storage vector of all rechits (including rechit for beam spot in case
  // it is used)
  ...
  if (useBeamSpot_ && propDir_ == alongMomentum) {
    ...
    TrajectoryStateClosestToBeamLine tsctbl(
        TSCBLBuilderNoMaterial()(*(refTsos.freeState()), beamSpot));
    if (!tsctbl.isValid()) {
    ...
    // There is also a constructor taking the magentic field. Use this one
    // instead?
    theRefTsos = TrajectoryStateOnSurface(pcaFts, bsGeom->surface());

    TransientTrackingRecHit::ConstRecHitPointer bsRecHit(
        new BeamSpotTransientTrackingRecHit(
            beamSpot, bsGeom, theRefTsos.freeState()->momentum().phi()));
  ...
  for (itRecHit = allRecHits.begin(); itRecHit != allRecHits.end();
       ++itRecHit) {
    ...
    if (0 == iRow) {
      ...
      // compute the derivatives of the reference-track's parameters w.r.t. the
      // initial ones derivative of the initial reference-track parameters
      // w.r.t. themselves is of course the identity
      fullJacobian =
          AlgebraicMatrix(theParameters.num_row(), theParameters.num_row(), 1);
      allJacobians.push_back(fullJacobian);
      theTsosVec.push_back(theRefTsos);
      const JacobianLocalToCurvilinear startTrafo(
          hitPtr->det()->surface(), theRefTsos.localParameters(), *magField);
      const AlgebraicMatrix localToCurvilinear =
          asHepMatrix<5>(startTrafo.jacobian());
      ...
      if (!this->propagate(previousHitPtr->det()->surface(), previousTsos,
                           hitPtr->det()->surface(), nextTsos, nextJacobian,
                           nextCurvlinJacobian, nextStep, magField)) {
        return false; // stop if problem...// no delete aMaterialEffectsUpdator
                      // needed
      }
      ...
      const JacobianLocalToCurvilinear startTrafo(
          hitPtr->det()->surface(), nextTsos.localParameters(), *magField);
      const AlgebraicMatrix localToCurvilinear =
          asHepMatrix<5>(startTrafo.jacobian());
      allLocalToCurv.push_back(localToCurvilinear);
      if (nextStep == 0.) {
        ...
        // brokenLinesFine will not work, brokenLinesCoarse combines close by
        // layers
    ...
    // take material effects into account. since trajectory-state is constructed
    // with errors equal zero, the updated state contains only the uncertainties
    // due to interactions in the current layer.
    const TrajectoryStateOnSurface tmpTsos(
        theTsosVec.back().localParameters(), zeroErrors,
        theTsosVec.back().surface(), magField, surfaceSide);
    const TrajectoryStateOnSurface updatedTsos =
        aMaterialEffectsUpdator->updateState(tmpTsos, propDir_);

    if (!updatedTsos.isValid())
      return false; // no delete aMaterialEffectsUpdator needed

    if (theTsosVec.back().localParameters().charge()) {
      previousChangeInCurvature[0][0] =
          updatedTsos.localParameters().signedInverseMomentum() /
          theTsosVec.back().localParameters().signedInverseMomentum();
    }

    // get multiple-scattering covariance-matrix
    allDeltaParameterCovs.push_back(
        asHepMatrix<5>(updatedTsos.localError().matrix()));
    ...
    AlgebraicVector mixedLocalParams =
        asHepVector<5>(theTsosVec.back().localParameters().mixedFormatVector());
    this->fillTrajectoryPositions(allProjections.back(), mixedLocalParams,
                                  iRow);
    if (useRecHit(hitPtr))
  ...
  case combined:
    ...
    break;
  case breakPoints:
    msOK = this->addMaterialEffectsBp(allJacobians, allProjections,
                                      allCurvatureChanges,
                                      allDeltaParameterCovs, allLocalToCurv);
    break;
  case brokenLinesCoarse:
    msOK = this->addMaterialEffectsBrl(allProjections, allDeltaParameterCovs,
                                       allLocalToCurv, allSteps,
                                       refTsos.globalParameters());
    break;
  case brokenLinesFine:
    msOK = this->addMaterialEffectsBrl(
        allCurvlinJacobians, allProjections, allCurvatureChanges,
        allDeltaParameterCovs, allLocalToCurv, refTsos.globalParameters());
    break;
  case localGBL:
    msOK = this->addMaterialEffectsLocalGbl(allJacobians, allProjections,
                                            allCurvatureChanges,
  ...
  if (refTsos.hasError()) {
    AlgebraicSymMatrix parameterCov =
        asHepMatrix<5>(refTsos.localError().matrix());
    AlgebraicMatrix parDeriv;
    if (theNumberOfVirtualPars > 0) {
      parDeriv = theDerivatives.sub(1, nMeasPerHit * allJacobians.size(), 1,
                                    theParameters.num_row());
    } else {
...
ReferenceTrajectory::createUpdator(MaterialEffects materialEffects,
                                   double mass) const {
  switch (materialEffects) {
    // MultipleScatteringUpdator doesn't change the trajectory-state
    // during update and can therefore be used if material effects should be
    // ignored:
  case none:
  case multipleScattering:
...
bool ReferenceTrajectory::propagate(
    const Plane &previousSurface, const TrajectoryStateOnSurface &previousTsos,
    const Plane &newSurface, TrajectoryStateOnSurface &newTsos,
    AlgebraicMatrix &newJacobian, AlgebraicMatrix &newCurvlinJacobian,
    double &nextStep, const MagneticField *magField) const {
  ...
  defaultRKPropagator::Product rkprod(magField,
                                      propDir_); // double tolerance = 5.e-5)
  ...
  // calculate derivative of reference-track parameters on the actual layer
  // w.r.t. the ones on the previous layer (both in global coordinates)
  const AnalyticalCurvilinearJacobian aJacobian(
      previousTsos.globalParameters(), tsosWithPath.first.globalPosition(),
      tsosWithPath.first.globalMomentum(), tsosWithPath.second);
  const AlgebraicMatrix curvilinearJacobian =
      asHepMatrix<5, 5>(aJacobian.jacobian());

  // jacobian of the track parameters on the previous layer for local->global
  // transformation
  const JacobianLocalToCurvilinear startTrafo(
      previousSurface, previousTsos.localParameters(), *magField);
  const AlgebraicMatrix localToCurvilinear =
      asHepMatrix<5>(startTrafo.jacobian());

  // jacobian of the track parameters on the actual layer for global->local
  // transformation
  const JacobianCurvilinearToLocal endTrafo(
      newSurface, tsosWithPath.first.localParameters(), *magField);
  const AlgebraicMatrix curvilinearToLocal =
      asHepMatrix<5>(endTrafo.jacobian());

  // compute derivative of reference-track parameters on the actual layer w.r.t.
  // the ones on the previous layer (both in their local representation)
...
void ReferenceTrajectory::fillMeasurementAndError(
    const TransientTrackingRecHit::ConstRecHitPointer &hitPtr,
    unsigned int iRow, const TrajectoryStateOnSurface &updatedTsos) {
  // get the measurements and their errors, use information updated with tsos if
  // improving (GF: Also for measurements or only for errors or do the former
  // not change?) GF 10/2008: I doubt that it makes sense to update the hit with
  // the tsos here:
  //             That is an analytical extrapolation and not the best guess of
  //             the real track state on the module, but the latter should be
  //             better to get the best hit uncertainty estimate!

  // FIXME FIXME  CLONE
  const auto &newHitPtr = hitPtr;
  //  TransientTrackingRecHit::ConstRecHitPointer
  //  newHitPtr(hitPtr->canImproveWithTrack() ?
  //							hitPtr->clone(updatedTsos)
  //: hitPtr);

  const LocalPoint localMeasurement = newHitPtr->localPosition();
  const LocalError localMeasurementCov =
      newHitPtr->localPositionError(); // CPE+APE
...
void ReferenceTrajectory::fillDerivatives(const AlgebraicMatrix &projection,
                                          ...
                                          unsigned int iRow) {
  // derivatives of the local coordinates of the reference track w.r.t. to the
  // inital track-parameters
...
void ReferenceTrajectory::fillTrajectoryPositions(
    const AlgebraicMatrix &projection, const AlgebraicVector &mixedLocalParams,
    unsigned int iRow) {
...
bool ReferenceTrajectory::addMaterialEffectsCov(
  ...
  // the uncertainty due to multiple scattering is 'transferred' to the error
  // matrix of the hits
  ...
  // additional covariance-matrix of the measurements due to material-effects
  // (single measurement)
  AlgebraicSymMatrix deltaMaterialEffectsCov;

  // additional covariance-matrix of the parameters due to material-effects
  AlgebraicSymMatrix paramMaterialEffectsCov(
      allDeltaParameterCovs[0]); // initialization
  // error-propagation to state after energy loss
  // GFback  paramMaterialEffectsCov =
  // paramMaterialEffectsCov.similarity(allCurvatureChanges[0]);
  ...
  for (unsigned int k = 1; k < allJacobians.size(); ++k) {
    ...
    paramMaterialEffectsCov =
        paramMaterialEffectsCov.similarity(allJacobians[k]);

    // get dependences for the measurements
    deltaMaterialEffectsCov =
        paramMaterialEffectsCov.similarity(allProjections[k]);
    materialEffectsCov[nMeasPerHit * k][nMeasPerHit * k] =
        deltaMaterialEffectsCov[0][0];
    materialEffectsCov[nMeasPerHit * k][nMeasPerHit * k + 1] =
        deltaMaterialEffectsCov[0][1];
    materialEffectsCov[nMeasPerHit * k + 1][nMeasPerHit * k] =
        deltaMaterialEffectsCov[1][0];
    materialEffectsCov[nMeasPerHit * k + 1][nMeasPerHit * k + 1] =
        deltaMaterialEffectsCov[1][1];

    // GFback add uncertainties for the following layers due to scattering at
    // this layer
    ...
    for (unsigned int l = k + 1; l < allJacobians.size(); ++l) {
      tempParameterCov =
          allJacobians[l] * allCurvatureChanges[l] * tempParameterCov;
      tempMeasurementCov =
          allProjections[l] * tempParameterCov * allProjections[k].T();

      materialEffectsCov[nMeasPerHit * l][nMeasPerHit * k] =
          tempMeasurementCov[0][0];
      materialEffectsCov[nMeasPerHit * k][nMeasPerHit * l] =
          tempMeasurementCov[0][0];

      materialEffectsCov[nMeasPerHit * l][nMeasPerHit * k + 1] =
          tempMeasurementCov[0][1];
      materialEffectsCov[nMeasPerHit * k + 1][nMeasPerHit * l] =
          tempMeasurementCov[0][1];

      materialEffectsCov[nMeasPerHit * l + 1][nMeasPerHit * k] =
          tempMeasurementCov[1][0];
      materialEffectsCov[nMeasPerHit * k][nMeasPerHit * l + 1] =
          tempMeasurementCov[1][0];

      materialEffectsCov[nMeasPerHit * l + 1][nMeasPerHit * k + 1] =
          tempMeasurementCov[1][1];
      materialEffectsCov[nMeasPerHit * k + 1][nMeasPerHit * l + 1] =
          tempMeasurementCov[1][1];
    }
    // add uncertainties for the following layers due to scattering at this
    // layer GFback paramMaterialEffectsCov += allDeltaParameterCovs[k];
    // error-propagation to state after energy loss
    paramMaterialEffectsCov =
        paramMaterialEffectsCov.similarity(allCurvatureChanges[k]);
  }
...
bool ReferenceTrajectory::addMaterialEffectsBp(
    const std::vector<AlgebraicMatrix> &allJacobians,
    const std::vector<AlgebraicMatrix> &allProjections,
    const std::vector<AlgebraicSymMatrix> &allCurvatureChanges,
    const std::vector<AlgebraicSymMatrix> &allDeltaParameterCovs,
  ...
  for (unsigned int k = 1; k < allJacobians.size(); ++k) {
    ...
    theMeasurementsCov[offsetMeas + nMeasPerHit * kbp]
                      [offsetMeas + nMeasPerHit * kbp] = tempMSCovProj[0][0];
    theMeasurementsCov[offsetMeas + nMeasPerHit * kbp + 1]
                      [offsetMeas + nMeasPerHit * kbp + 1] =
                          tempMSCovProj[1][1];
    theDerivatives[offsetMeas + nMeasPerHit * kbp][offsetPar + 2 * kbp] = 1.0;
    theDerivatives[offsetMeas + nMeasPerHit * kbp + 1]
                  [offsetPar + 2 * kbp + 1] = 1.0;

    tempMSJacProj = (allProjections[k] *
                     (tempJacobian * allLocalToCurv[k - 1].inverse(ierr))) *
                    MSprojection.T();
    if (ierr) {
      edm::LogError("Alignment")
    ...
    theDerivatives[nMeasPerHit * k][offsetPar + 2 * kbp + 1] =
        tempMSJacProj[0][1];
    theDerivatives[nMeasPerHit * k + 1][offsetPar + 2 * kbp] =
        tempMSJacProj[1][0];
    theDerivatives[nMeasPerHit * k + 1][offsetPar + 2 * kbp + 1] =
        tempMSJacProj[1][1];
    ...
    for (unsigned int l = k + 1; l < allJacobians.size(); ++l) {
      ...
      tempMSJacProj = (allProjections[l] *
                       (tempJacobian * allLocalToCurv[k - 1].inverse(ierr))) *
                      MSprojection.T();
      if (ierr) {
        edm::LogError("Alignment")
      ...
      theDerivatives[nMeasPerHit * l][offsetPar + 2 * kbp] =
          tempMSJacProj[0][0];
      theDerivatives[nMeasPerHit * l][offsetPar + 2 * kbp + 1] =
          tempMSJacProj[0][1];
      theDerivatives[nMeasPerHit * l + 1][offsetPar + 2 * kbp] =
          tempMSJacProj[1][0];
      theDerivatives[nMeasPerHit * l + 1][offsetPar + 2 * kbp + 1] =
          tempMSJacProj[1][1];
    }
  }
...
bool ReferenceTrajectory::addMaterialEffectsBrl(
    const std::vector<AlgebraicMatrix> &allCurvlinJacobians,
    const std::vector<AlgebraicMatrix> &allProjections,
    const std::vector<AlgebraicSymMatrix> &allCurvatureChanges,
    const std::vector<AlgebraicSymMatrix> &allDeltaParameterCovs,
    const std::vector<AlgebraicMatrix> &allLocalToCurv,
  ...
  // broken lines: pair of offsets (u1,u2) = (xt,yt) (in curvilinear frame
  // (q/p,lambda,phi,xt,yt)) at each layer
  //              scattering angles (alpha1,alpha2) = (cosLambda*dPhi, dLambda)
  //              (cosLambda cancels in Chi2) DU' = (dU'/dU)*DU +
  //              (dU'/dAlpha)*DAlpha + (dU'/dQbyp)*DQbyp (propagation of U)
  ...
  double cosLambda = sqrt((p.x() * p.x() + p.y() * p.y()) /
  ...
  AlgebraicMatrix JacCurvToOffsetL, JacOffsetToOffsetL, JacAngleToOffsetL,
      JacQbypToOffsetL, JacOffsetToAngleL;
  AlgebraicMatrix JacCurvToOffsetN, JacOffsetToOffsetN, JacAngleToOffsetN,
      JacQbypToOffsetN, JacOffsetToAngleN;

  // transformation from trajectory to curvilinear parameters

  JacCurvToOffsetN =
      CurvToOffset *
      allCurvlinJacobians[1]; // (dU'/dCurv') * (dCurv'/dCurv) @ 2nd point
  JacOffsetToOffsetN =
      JacCurvToOffsetN * OffsetToCurv; // J: (dU'/dU)     = (dU'/dCurv') *
                                       // (dCurv'/dCurv) * (dCurv/dU)
  JacAngleToOffsetN =
      JacCurvToOffsetN * AngleToCurv; // S: (dU'/dAlpha) = (dU'/dCurv') *
                                      // (dCurv'/dCurv) * (dCurv/dAlpha)
  JacQbypToOffsetN =
      JacCurvToOffsetN * QbypToCurv; // d: (dU'/dQbyp)  = (dU'/dCurv') *
                                     // (dCurv'/dCurv) * (dCurv/dQbyp)
  ...
  // (dAlpha/dTraj) = (dAlpha/dQbyp) * (dQbyp/dTraj) + (dAlpha/dU1) *
  // (dU1/dTraj) + (dAlpha/dU2) * (dU2/dTraj)
  AlgebraicMatrix JacTrajToAngle = JacQbypToAngleC * TrajToQbyp +
                                   JacOffsetToAngleC * TrajToOff1 +
                                   JacOffsetToAngleN * TrajToOff2;
  // (dCurv/dTraj) = (dCurv/dQbyp) * (dQbyp/dTraj) + (dCurv/dAlpha) *
  // (dAlpha/dTraj) + (dCurv/dU) * (dU/dTraj)
  theInnerTrajectoryToCurvilinear = QbypToCurv * TrajToQbyp +
                                    AngleToCurv * JacTrajToAngle +
                                    OffsetToCurv * TrajToOff1;
  theInnerLocalToTrajectory =
      theInnerTrajectoryToCurvilinear.inverse(ierr) * allLocalToCurv[0];
  if (ierr) {
    edm::LogError("Alignment")
        << "@SUB=ReferenceTrajectory::addMaterialEffectsBrl"
  ...
  for (unsigned int k = 0; k < allCurvlinJacobians.size(); ++k) {
    ...
    JacOffsetToMeas =
        (allProjections[k] * allLocalToCurv[k].inverse(ierr)) * OffsetToCurv;
    if (ierr) {
      edm::LogError("Alignment")
          << "@SUB=ReferenceTrajectory::addMaterialEffectsBrl"
    ...
    theDerivatives[nMeasPerHit * k][offsetPar + 2 * k + 1] =
        JacOffsetToMeas[0][1];
    theDerivatives[nMeasPerHit * k + 1][offsetPar + 2 * k] =
        JacOffsetToMeas[1][0];
    theDerivatives[nMeasPerHit * k + 1][offsetPar + 2 * k + 1] =
        JacOffsetToMeas[1][1];
  }
  ...
  for (unsigned int k = 1; k < allCurvlinJacobians.size() - 1; ++k) {
    ...
    // amount of multiple scattering in layer k  (angular error perp to
    // direction)
    tempMSCov = allDeltaParameterCovs[k].similarity(allLocalToCurv[k]);
    tempMSCovProj = tempMSCov.similarity(CurvToAngle);
    theMeasurementsCov[offsetMeas + nMeasPerHit * iMsMeas]
                      [offsetMeas + nMeasPerHit * iMsMeas] =
                          tempMSCovProj[1][1];
    theMeasurementsCov[offsetMeas + nMeasPerHit * iMsMeas + 1]
                      [offsetMeas + nMeasPerHit * iMsMeas + 1] =
                          tempMSCovProj[0][0];
    ...
    JacCurvToOffsetL =
        CurvToOffset *
        tempJacL.inverse(ierr); // (dU'/dCurv') * (dCurv'/dCurv) @ last point
    ...
    JacOffsetToOffsetL =
        JacCurvToOffsetL * OffsetToCurv; // J-: (dU'/dU)     = (dU'/dCurv') *
                                         // (dCurv'/dCurv) * (dCurv/dU)
    JacAngleToOffsetL =
        JacCurvToOffsetL * AngleToCurv; // S-: (dU'/dAlpha) = (dU'/dCurv') *
                                        // (dCurv'/dCurv) * (dCurv/dAlpha)
    JacQbypToOffsetL =
        JacCurvToOffsetL * QbypToCurv; // d-: (dU'/dQbyp)  = (dU'/dCurv') *
                                       // (dCurv'/dCurv) * (dCurv/dQbyp)
    ...
    JacCurvToOffsetN =
        CurvToOffset * tempJacN; // (dU'/dCurv') * (dCurv'/dCurv) @ next point
    JacOffsetToOffsetN =
        JacCurvToOffsetN * OffsetToCurv; // J+: (dU'/dU)     = (dU'/dCurv') *
                                         // (dCurv'/dCurv) * (dCurv/dU)
    JacAngleToOffsetN =
    ...
    JacOffsetToAngleC = -(JacOffsetToAngleL * JacOffsetToOffsetL +
                          JacOffsetToAngleN * JacOffsetToOffsetN);
    JacQbypToAngleC = -(JacOffsetToAngleL * JacQbypToOffsetL +
                        JacOffsetToAngleN * JacQbypToOffsetN);

    // bending
    theDerivatives[offsetMeas + nMeasPerHit * iMsMeas][0] =
        JacQbypToAngleC[0][0];
    theDerivatives[offsetMeas + nMeasPerHit * iMsMeas + 1][0] =
        JacQbypToAngleC[1][0];
    // last layer
    theDerivatives[offsetMeas + nMeasPerHit * iMsMeas][offsetPar + 2 * l] =
        JacOffsetToAngleL[0][0];
    theDerivatives[offsetMeas + nMeasPerHit * iMsMeas][offsetPar + 2 * l + 1] =
        JacOffsetToAngleL[0][1];
    theDerivatives[offsetMeas + nMeasPerHit * iMsMeas + 1][offsetPar + 2 * l] =
        JacOffsetToAngleL[1][0];
    theDerivatives[offsetMeas + nMeasPerHit * iMsMeas + 1]
                  [offsetPar + 2 * l + 1] = JacOffsetToAngleL[1][1];
    // current layer
    theDerivatives[offsetMeas + nMeasPerHit * iMsMeas][offsetPar + 2 * k] =
        JacOffsetToAngleC[0][0];
    theDerivatives[offsetMeas + nMeasPerHit * iMsMeas][offsetPar + 2 * k + 1] =
        JacOffsetToAngleC[0][1];
    theDerivatives[offsetMeas + nMeasPerHit * iMsMeas + 1][offsetPar + 2 * k] =
        JacOffsetToAngleC[1][0];
    theDerivatives[offsetMeas + nMeasPerHit * iMsMeas + 1]
                  [offsetPar + 2 * k + 1] = JacOffsetToAngleC[1][1];

    // next layer
    theDerivatives[offsetMeas + nMeasPerHit * iMsMeas][offsetPar + 2 * n] =
        JacOffsetToAngleN[0][0];
    theDerivatives[offsetMeas + nMeasPerHit * iMsMeas][offsetPar + 2 * n + 1] =
        JacOffsetToAngleN[0][1];
    theDerivatives[offsetMeas + nMeasPerHit * iMsMeas + 1][offsetPar + 2 * n] =
        JacOffsetToAngleN[1][0];
    theDerivatives[offsetMeas + nMeasPerHit * iMsMeas + 1]
                  [offsetPar + 2 * n + 1] = JacOffsetToAngleN[1][1];
  }
...
bool ReferenceTrajectory::addMaterialEffectsBrl(
    ...
    const std::vector<AlgebraicSymMatrix> &allDeltaParameterCovs,
    const std::vector<AlgebraicMatrix> &allLocalToCurv,
    const std::vector<double> &allSteps, const GlobalTrajectoryParameters &gtp,
    const double minStep) {
  // CHK: add material effects using broken lines
  // BrokenLinesCoarse: combine close by detectors,
  //                   use approximate Jacobians from Steps (limit Qbyp -> 0),
  //                   bending only in RPhi (B=(0,0,Bz)), no energy loss
  //                   correction
  ...
  theNumberOfVirtualMeas =
      2 * (nPlane - 1); // unsigned underflow for nPlane == 0...
  ...
  for (unsigned int k = 0; k < allSteps.size(); ++k) {
    ...
    if (last[iPlane] == first[iPlane]) { // single plane
      theDerivatives[nMeasPerHit * k][offsetPar + 2 * iPlane] =
          JacOffsetToMeas[0][0];
      theDerivatives[nMeasPerHit * k][offsetPar + 2 * iPlane + 1] =
          JacOffsetToMeas[0][1];
      theDerivatives[nMeasPerHit * k + 1][offsetPar + 2 * iPlane] =
          JacOffsetToMeas[1][0];
      theDerivatives[nMeasPerHit * k + 1][offsetPar + 2 * iPlane + 1] =
          JacOffsetToMeas[1][1];
      ...
      theDerivatives[nMeasPerHit * k][offsetPar + 2 * iPlane] =
          JacOffsetToMeas[0][0] * iFrac;
      theDerivatives[nMeasPerHit * k][offsetPar + 2 * iPlane + 1] =
          JacOffsetToMeas[0][1] * iFrac;
      theDerivatives[nMeasPerHit * k + 1][offsetPar + 2 * iPlane] =
          JacOffsetToMeas[1][0] * iFrac;
      theDerivatives[nMeasPerHit * k + 1][offsetPar + 2 * iPlane + 1] =
          JacOffsetToMeas[1][1] * iFrac;
      theDerivatives[nMeasPerHit * k][offsetPar + 2 * jPlane] =
          JacOffsetToMeas[0][0] * jFrac;
      theDerivatives[nMeasPerHit * k][offsetPar + 2 * jPlane + 1] =
          JacOffsetToMeas[0][1] * jFrac;
      theDerivatives[nMeasPerHit * k + 1][offsetPar + 2 * jPlane] =
          JacOffsetToMeas[1][0] * jFrac;
      theDerivatives[nMeasPerHit * k + 1][offsetPar + 2 * jPlane + 1] =
          JacOffsetToMeas[1][1] * jFrac;
      // 2nd order neglected
      // theDerivatives[nMeasPerHit*k  ][                   0] =
      // -0.5*bFac*sDiff*iFrac*sDiff*jFrac*cosLambda;
    }
  }
  ...
  for (unsigned int i = 1; i < nPlane; ++i) {
    ...
    // broken line measurements for layer k, correlations between both planes
    // neglected
    ...
    theDerivatives[offsetMeas + nMeasPerHit * iMsMeas][0] =
        -0.5 * bFac * (stepK + stepN) * cosLambda;
    // last layer
    theDerivatives[offsetMeas + nMeasPerHit * iMsMeas][offsetPar + 2 * l] =
        deltaK;
    theDerivatives[offsetMeas + nMeasPerHit * iMsMeas + 1]
                  [offsetPar + 2 * l + 1] = deltaK;
    // current layer
    theDerivatives[offsetMeas + nMeasPerHit * iMsMeas][offsetPar + 2 * i] =
        -(deltaK + deltaN);
    theDerivatives[offsetMeas + nMeasPerHit * iMsMeas + 1]
                  [offsetPar + 2 * i + 1] = -(deltaK + deltaN);
    // next layer
    theDerivatives[offsetMeas + nMeasPerHit * iMsMeas][offsetPar + 2 * n] =
        deltaN;
    theDerivatives[offsetMeas + nMeasPerHit * iMsMeas + 1]
                  [offsetPar + 2 * n + 1] = deltaN;
  }
...
bool ReferenceTrajectory::addMaterialEffectsLocalGbl(
    const std::vector<AlgebraicMatrix> &allJacobians,
    const std::vector<AlgebraicMatrix> &allProjections,
    const std::vector<AlgebraicSymMatrix> &allCurvatureChanges,
    const std::vector<AlgebraicSymMatrix> &allDeltaParameterCovs) {
  ...
  const double minPrec = 1.0; // minimum precision to use measurement (reject
                              // measurements in strip direction)
  ...
  for (unsigned int k = 0; k < numHits; ++k) {
    ...
    clhep2eigen(theMeasurements.sub(2 * k + 1, 2 * k + 2) -
                    theTrajectoryPositions.sub(2 * k + 1, 2 * k + 2),
                measurement);
    ...
    if (std::abs(covariance(0, 1)) < std::numeric_limits<double>::epsilon()) {
      ...
      for (unsigned int row = 0; row < 2; ++row) {
        measPrecDiag(row) =
            (0. < covariance(row, row) ? 1.0 / covariance(row, row) : 0.);
      }
      aGblPoint.addMeasurement(proLocalToMeas, measurement, measPrecDiag,
                               minPrec);
    } else {
      // covariance matrix needs diagonalization
      aGblPoint.addMeasurement(proLocalToMeas, measurement,
                               covariance.inverse(), minPrec);
    }

    // GBL multiple scattering (full matrix in local system)
    clhep2eigen(allDeltaParameterCovs[k].similarityT(SlopeToLocal),
                scatPrecision);
    if (!(scatPrecision.colPivHouseholderQr().isInvertible())) {
      if (!allowZeroMaterial_) {
        throw cms::Exception("Alignment")
            ...
            << "\nEncountered singular scatter covariance-matrix without "
               "allowing "
            << "for zero material.";
      }
    } else {
  ...
  // add list of points and transformation local to fit (=local) system at first
  // hit
...
bool ReferenceTrajectory::addMaterialEffectsCurvlinGbl(
    const std::vector<AlgebraicMatrix> &allCurvlinJacobians,
    const std::vector<AlgebraicMatrix> &allProjections,
    const std::vector<AlgebraicSymMatrix> &allCurvatureChanges,
    const std::vector<AlgebraicSymMatrix> &allDeltaParameterCovs,
  ...
  for (unsigned int k = 0; k < numHits; ++k) {
    ...
    // GBL point to point jacobian
    clhep2eigen(allCurvlinJacobians[k] * allCurvatureChanges[k],
                jacPointToPoint);
  ...
  // add list of points and transformation local to fit (=curvilinear) system at
  // first hit
...
AlgebraicMatrix ReferenceTrajectory::getHitProjectionMatrixT(
  ...
  // input for the holder - but dummy is OK here to just get the projection
  // matrix:
