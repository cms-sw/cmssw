...
BzeroReferenceTrajectory::BzeroReferenceTrajectory(
    ...
    : ReferenceTrajectory(tsos.localParameters().mixedFormatVector().kSize,
                          recHits.size(), config),
  ...
  LocalTrajectoryParameters locParamWithFixedMomentum(
      asSVector<5>(theParameters), tsos.localParameters().pzSign(),
      tsos.localParameters().charge());

  const TrajectoryStateOnSurface refTsosWithFixedMomentum(
      locParamWithFixedMomentum, tsos.localError(), tsos.surface(), magField,
  ...
  if (config.hitsAreReverse) {
    ...
    for (TransientTrackingRecHit::ConstRecHitContainer::const_reverse_iterator
             it = recHits.rbegin();
         it != recHits.rend(); ++it)
      fwdRecHits.push_back(*it);

    theValidityFlag = this->construct(refTsosWithFixedMomentum, fwdRecHits,
                                      magField, beamSpot);
  } else {
    theValidityFlag =
        this->construct(refTsosWithFixedMomentum, recHits, magField, beamSpot);
  }

  // Exclude momentum from the parameters and also the derivatives of the
  // measurements w.r.t. the momentum.
  theParameters = theParameters.sub(2, 5);
  theDerivatives = theDerivatives.sub(1, theDerivatives.num_row(), 2,
                                      theDerivatives.num_col());
}
