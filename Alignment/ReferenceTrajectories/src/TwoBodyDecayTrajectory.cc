...
TwoBodyDecayTrajectory::TwoBodyDecayTrajectory(
    const TwoBodyDecayTrajectoryState &tsos,
    const ConstRecHitCollection &recHits, const MagneticField *magField,
    ...
    : ReferenceTrajectoryBase(
          ...
          (config.materialEffects >= breakPoints)
              ? 2 * (recHits.first.size() + recHits.second.size()) - 4
              : 0,
          (config.materialEffects >= breakPoints)
              ? 2 * (recHits.first.size() + recHits.second.size()) - 3
              : 1),
      materialEffects_(config.materialEffects), propDir_(config.propDir),
      useRefittedState_(config.useRefittedState),
      constructTsosWithErrors_(config.constructTsosWithErrors)

{
  if (config.hitsAreReverse) {
    TransientTrackingRecHit::ConstRecHitContainer::const_reverse_iterator
        itRecHits;
    ConstRecHitCollection fwdRecHits;

    fwdRecHits.first.reserve(recHits.first.size());
    for (itRecHits = recHits.first.rbegin(); itRecHits != recHits.first.rend();
         ++itRecHits) {
      fwdRecHits.first.push_back(*itRecHits);
    }

    fwdRecHits.second.reserve(recHits.second.size());
    for (itRecHits = recHits.second.rbegin();
         itRecHits != recHits.second.rend(); ++itRecHits) {
...
TwoBodyDecayTrajectory::TwoBodyDecayTrajectory(void)
    : ReferenceTrajectoryBase(0, 0, 0, 0), materialEffects_(none),
      propDir_(anyDirection), useRefittedState_(false),
...
bool TwoBodyDecayTrajectory::construct(const TwoBodyDecayTrajectoryState &state,
                                       ...
                                       const reco::BeamSpot &beamSpot) {
  const TwoBodyDecayTrajectoryState::TsosContainer &tsos =
      state.trajectoryStates(useRefittedState_);
  ...
  ReferenceTrajectory trajectory1(tsos.first, recHits.first, field, beamSpot,
                                  config);
  ...
  ReferenceTrajectory trajectory2(tsos.second, recHits.second, field, beamSpot,
                                  config);
  ...
  if (materialEffects_ >= localGBL) {
    ...
    theGblInput.push_back(
        std::make_pair(trajectory1.gblInput().front().first,
    ...
    // add second body
    theGblInput.push_back(
        std::make_pair(trajectory2.gblInput().front().first,
    ...
    theGblExtMeasurements(0) =
        state.primaryMass() -
        state.decayParameters()[TwoBodyDecayParameters::mass];
    theGblExtPrecisions.resize(1);
    theGblExtPrecisions(0) =
        1.0 / (state.primaryWidth() * state.primaryWidth());
    // nominal field
    ...
    AlgebraicMatrix fullDeriv1 =
        trajectory1.derivatives().sub(1, nHitMeas1 + nVirtualMeas1, 1, nLocal) *
    ...
    AlgebraicMatrix fullDeriv2 =
        trajectory2.derivatives().sub(1, nHitMeas2 + nVirtualMeas2, 1, nLocal) *
    ...
    theNumberOfVirtualMeas =
        nVirtualMeas1 + nVirtualMeas2 + 1; // add virtual mass measurement

    // hit measurements from trajectory 1
    int rowOffset = 1;
    int colOffset = 1;
    theDerivatives.sub(rowOffset, colOffset,
                       fullDeriv1.sub(1, nHitMeas1, 1, nTbd));
    colOffset += nTbd;
    theDerivatives.sub(rowOffset, colOffset,
                       trajectory1.derivatives().sub(1, nHitMeas1, nLocal + 1,
                                                     nPar1 + nVirtualPar1));
    // hit measurements from trajectory 2
    rowOffset += nHitMeas1;
    colOffset = 1;
    theDerivatives.sub(rowOffset, colOffset,
                       fullDeriv2.sub(1, nHitMeas2, 1, nTbd));
    colOffset += (nPar1 + nVirtualPar1 + nTbd - nLocal);
    theDerivatives.sub(rowOffset, colOffset,
                       trajectory2.derivatives().sub(1, nHitMeas2, nLocal + 1,
                                                     nPar2 + nVirtualPar2));
    ...
    theDerivatives.sub(
        rowOffset, colOffset,
        fullDeriv1.sub(nHitMeas1 + 1, nHitMeas1 + nVirtualMeas1, 1, nTbd));
    colOffset += nTbd;
    theDerivatives.sub(
        rowOffset, colOffset,
        trajectory1.derivatives().sub(nHitMeas1 + 1, nHitMeas1 + nVirtualMeas1,
                                      nLocal + 1, nPar1 + nVirtualPar1));
    // MS measurements from trajectory 2
    rowOffset += nVirtualMeas1;
    colOffset = 1;
    theDerivatives.sub(
        rowOffset, colOffset,
        fullDeriv2.sub(nHitMeas2 + 1, nHitMeas2 + nVirtualMeas2, 1, nTbd));
    colOffset += (nPar1 + nVirtualPar1 + nTbd - nLocal);
    theDerivatives.sub(
        rowOffset, colOffset,
        trajectory2.derivatives().sub(nHitMeas2 + 1, nHitMeas2 + nVirtualMeas2,
                                      nLocal + 1, nPar2 + nVirtualPar2));

    theMeasurements.sub(1, trajectory1.measurements().sub(1, nHitMeas1));
    theMeasurements.sub(nHitMeas1 + 1,
                        trajectory2.measurements().sub(1, nHitMeas2));
    theMeasurements.sub(nHitMeas1 + nHitMeas2 + 1,
                        trajectory1.measurements().sub(
                            nHitMeas1 + 1, nHitMeas1 + nVirtualMeas1));
    theMeasurements.sub(nHitMeas1 + nHitMeas2 + nVirtualMeas1 + 1,
                        trajectory2.measurements().sub(
                            nHitMeas2 + 1, nHitMeas2 + nVirtualMeas2));

    theMeasurementsCov.sub(1,
                           trajectory1.measurementErrors().sub(1, nHitMeas1));
    theMeasurementsCov.sub(nHitMeas1 + 1,
                           trajectory2.measurementErrors().sub(1, nHitMeas2));
    theMeasurementsCov.sub(nHitMeas1 + nHitMeas2 + 1,
                           trajectory1.measurementErrors().sub(
                               nHitMeas1 + 1, nHitMeas1 + nVirtualMeas1));
    theMeasurementsCov.sub(nHitMeas1 + nHitMeas2 + nVirtualMeas1 + 1,
                           trajectory2.measurementErrors().sub(
                               nHitMeas2 + 1, nHitMeas2 + nVirtualMeas2));

    theTrajectoryPositions.sub(1, trajectory1.trajectoryPositions());
    theTrajectoryPositions.sub(nHitMeas1 + 1,
                               trajectory2.trajectoryPositions());

    theTrajectoryPositionCov = state.decayParameters().covariance().similarity(
    ...
    theMeasurements[indMass] =
        state.primaryMass() -
        state.decayParameters()[TwoBodyDecayParameters::mass];
    theMeasurementsCov[indMass][indMass] =
        state.primaryWidth() * state.primaryWidth();
    theDerivatives[indMass][TwoBodyDecayParameters::mass] = 1.0;
  }

  theRecHits.insert(theRecHits.end(), recHits.first.begin(),
                    recHits.first.end());
  theRecHits.insert(theRecHits.end(), recHits.second.begin(),
                    recHits.second.end());

  if (constructTsosWithErrors_) {
    constructTsosVecWithErrors(trajectory1, trajectory2, field);
  } else {
    theTsosVec.insert(theTsosVec.end(), trajectory1.trajectoryStates().begin(),
                      trajectory1.trajectoryStates().end());

    theTsosVec.insert(theTsosVec.end(), trajectory2.trajectoryStates().begin(),
...
void TwoBodyDecayTrajectory::constructTsosVecWithErrors(
    const ReferenceTrajectory &traj1, const ReferenceTrajectory &traj2,
    const MagneticField *field) {
  int iTsos = 0;

  std::vector<TrajectoryStateOnSurface>::const_iterator itTsos;

  for (itTsos = traj1.trajectoryStates().begin();
       itTsos != traj1.trajectoryStates().end(); itTsos++) {
    constructSingleTsosWithErrors(*itTsos, iTsos, field);
    iTsos++;
  }

  for (itTsos = traj2.trajectoryStates().begin();
       itTsos != traj2.trajectoryStates().end(); itTsos++) {
    constructSingleTsosWithErrors(*itTsos, iTsos, field);
    iTsos++;
  }
}

void TwoBodyDecayTrajectory::constructSingleTsosWithErrors(
    const TrajectoryStateOnSurface &tsos, int iTsos,
    const MagneticField *field) {
  ...
  localErrors[3][3] =
      theTrajectoryPositionCov[nMeasPerHit * iTsos][nMeasPerHit * iTsos];
  localErrors[3][4] =
      theTrajectoryPositionCov[nMeasPerHit * iTsos][nMeasPerHit * iTsos + 1];
  localErrors[4][4] = theTrajectoryPositionCov[nMeasPerHit * iTsos + 1]
                                              [nMeasPerHit * iTsos + 1];

  // construct tsos with local errors
  theTsosVec[iTsos] = TrajectoryStateOnSurface(
      tsos.localParameters(), LocalTrajectoryError(localErrors), tsos.surface(),
      field, tsos.surfaceSide());
}
