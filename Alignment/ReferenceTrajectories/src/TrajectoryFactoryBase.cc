...
TrajectoryFactoryBase::TrajectoryFactoryBase(const edm::ParameterSet &config,
    ...
    : cfg_(config), tracksPerTrajectory_(tracksPerTrajectory),
      materialEffects_(
          materialEffects(config.getParameter<std::string>("MaterialEffects"))),
      propDir_(propagationDirection(
          config.getParameter<std::string>("PropagationDirection"))),
  ...
  edm::LogInfo("Alignment")
      ...
      << "TrajectoryFactory '"
      << cfg_.getParameter<std::string>("TrajectoryFactoryName")
      << "' with following settings:"
      << "\nmaterial effects: "
      << cfg_.getParameter<std::string>("MaterialEffects") << "\npropagation: "
      << cfg_.getParameter<std::string>("PropagationDirection")
...
TrajectoryFactoryBase::innermostStateAndRecHits(
  ...
  // get the innermost valid trajectory state - the corresponding hit must be
  // o.k. as well
...
TrajectoryFactoryBase::orderedTrajectoryMeasurements(
  ...
  const bool hitsAreReverse =
      ((dir == propDir_ || propDir_ == anyDirection) ? false : true);

  const Trajectory::DataContainer &original = trajectory.measurements();

  if (hitsAreReverse) {
    // Simply use this line instead of the copying by hand?
    // const Trajectory::DataContainer reordered(original.rbegin(),
    // original.rend());
...
bool TrajectoryFactoryBase::sameSurface(const Surface &s1,
  ...
  // - Should not rely on floating point equality, but make a minimal range,
  // e.g. 1.e-6 ?
  return (s1.eta() == s2.eta()) && (s1.phi() == s2.phi()) &&
         (s1.position().perp() == s2.position().perp());
}

bool TrajectoryFactoryBase::useRecHit(
...
TrajectoryFactoryBase::materialEffects(const std::string &strME) const {
  ...
  throw cms::Exception("BadConfig")
      << "[TrajectoryFactoryBase::materialEffects] Unknown parameter: "
      << strME;
}
...
TrajectoryFactoryBase::propagationDirection(const std::string &strPD) const {
  ...
  throw cms::Exception("BadConfig")
      << "[TrajectoryFactoryBase::propagationDirection] Unknown parameter: "
      << strPD;
}
