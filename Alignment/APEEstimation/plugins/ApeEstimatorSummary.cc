...
//
/**\class ApeEstimatorSummary ApeEstimatorSummary.cc
 Alignment/APEEstimation/src/ApeEstimatorSummary.cc
...
void ApeEstimatorSummary::openInputFile() {
  const std::string inputFileName(
      parameterSet_.getParameter<std::string>("InputFile"));
  ...
  if (inputFile_) {
    edm::LogInfo("CalculateAPE") << "Input file from loop over tracks and "
                                    "corresponding hits sucessfully opened";
  } else {
    edm::LogError("CalculateAPE") << "There is NO input file\n"
                                  << "...APE calculation stopped. Please check "
                                     "path of input file name in config:\n"
                                  << "\t" << inputFileName;
...
void ApeEstimatorSummary::getTrackerSectorStructs() {
  // At present first of the plugins registered in TFileService needs to be the
  // one containing the normalized residual histos per sector per error bin
  ...
  for (; sectorBool; ++iSector) {
    ...
    if (sectorDir) {
      ...
      for (unsigned int iInterval(1); intervalBool; ++iInterval) {
        ...
        intervalDir =
            (TDirectory *)inputFile_->TDirectory::GetDirectory(fullName);
        if (intervalDir) {
          intervalDir->GetObject("h_sigmaX;1",
                                 tkSector.m_binnedHists[iInterval]["sigmaX"]);
          intervalDir->GetObject("h_norResX;1",
                                 tkSector.m_binnedHists[iInterval]["norResX"]);
          intervalDir->GetObject("h_sigmaY;1",
                                 tkSector.m_binnedHists[iInterval]["sigmaY"]);
          intervalDir->GetObject("h_norResY;1",
                                 tkSector.m_binnedHists[iInterval]["norResY"]);
          if (tkSector.m_binnedHists[iInterval]["sigmaY"] &&
              tkSector.m_binnedHists[iInterval]["norResY"]) {
      ...
      if (resultsDir) {
        ...
        for (int entry = 0; entry < rawIdTree->GetEntries(); ++entry) {
          ...
          // command "hadd" adds entries in TTree, so rawId are existing as
          // often as number of files are added
      ...
      sectorBool = false;
      edm::LogInfo("CalculateAPE")
          << "There are " << iSector - 1 << " sectors defined in input file";
    }
  }
...
void ApeEstimatorSummary::bookHists() {
  ...
  for (auto &i_sector : m_tkSector_) {
    i_sector.second.WeightX = new TH1F(
        "h_weightX",
        "relative weight w_{x}/w_{tot,x};#sigma_{x}  [#mum];w_{x}/w_{tot,x}",
        v_binX.size() - 1, &(v_binX[0]));
    i_sector.second.MeanX =
        new TH1F("h_meanX",
                 "residual mean <r_{x}/#sigma_{r,x}>;#sigma_{x}  "
                 "[#mum];<r_{x}/#sigma_{r,x}>",
                 v_binX.size() - 1, &(v_binX[0]));
    i_sector.second.RmsX =
        new TH1F("h_rmsX",
                 "residual rms RMS(r_{x}/#sigma_{r,x});#sigma_{x}  "
                 "[#mum];RMS(r_{x}/#sigma_{r,x})",
                 v_binX.size() - 1, &(v_binX[0]));
    i_sector.second.FitMeanX1 =
        new TH1F("h_fitMeanX1",
                 "fitted residual mean #mu_{x};#sigma_{x}  [#mum];#mu_{x}",
                 v_binX.size() - 1, &(v_binX[0]));
    i_sector.second.ResidualWidthX1 =
        new TH1F("h_residualWidthX1",
                 "residual width #Delta_{x};#sigma_{x}  [#mum];#Delta_{x}",
                 v_binX.size() - 1, &(v_binX[0]));
    i_sector.second.CorrectionX1 =
        new TH1F("h_correctionX1",
                 "correction to APE_{x};#sigma_{x}  "
                 "[#mum];#Delta#sigma_{align,x}  [#mum]",
                 v_binX.size() - 1, &(v_binX[0]));
    i_sector.second.FitMeanX2 =
        new TH1F("h_fitMeanX2",
                 "fitted residual mean #mu_{x};#sigma_{x}  [#mum];#mu_{x}",
                 v_binX.size() - 1, &(v_binX[0]));
    i_sector.second.ResidualWidthX2 =
        new TH1F("h_residualWidthX2",
                 "residual width #Delta_{x};#sigma_{x}  [#mum];#Delta_{x}",
                 v_binX.size() - 1, &(v_binX[0]));
    i_sector.second.CorrectionX2 =
        new TH1F("h_correctionX2",
                 "correction to APE_{x};#sigma_{x}  "
                 "[#mum];#Delta#sigma_{align,x}  [#mum]",
                 v_binX.size() - 1, &(v_binX[0]));

    if (i_sector.second.isPixel) {
      i_sector.second.WeightY = new TH1F(
          "h_weightY",
          "relative weight w_{y}/w_{tot,y};#sigma_{y}  [#mum];w_{y}/w_{tot,y}",
          v_binX.size() - 1, &(v_binX[0]));
      i_sector.second.MeanY =
          new TH1F("h_meanY",
                   "residual mean <r_{y}/#sigma_{r,y}>;#sigma_{y}  "
                   "[#mum];<r_{y}/#sigma_{r,y}>",
                   v_binX.size() - 1, &(v_binX[0]));
      i_sector.second.RmsY =
          new TH1F("h_rmsY",
                   "residual rms RMS(r_{y}/#sigma_{r,y});#sigma_{y}  "
                   "[#mum];RMS(r_{y}/#sigma_{r,y})",
                   v_binX.size() - 1, &(v_binX[0]));
      i_sector.second.FitMeanY1 =
          new TH1F("h_fitMeanY1",
                   "fitted residual mean #mu_{y};#sigma_{y}  [#mum];#mu_{y}",
                   v_binX.size() - 1, &(v_binX[0]));
      i_sector.second.ResidualWidthY1 =
          new TH1F("h_residualWidthY1",
                   "residual width #Delta_{y};#sigma_{y}  [#mum];#Delta_{y}",
                   v_binX.size() - 1, &(v_binX[0]));
      i_sector.second.CorrectionY1 =
          new TH1F("h_correctionY1",
                   "correction to APE_{y};#sigma_{y}  "
                   "[#mum];#Delta#sigma_{align,y}  [#mum]",
                   v_binX.size() - 1, &(v_binX[0]));
      i_sector.second.FitMeanY2 =
          new TH1F("h_fitMeanY2",
                   "fitted residual mean #mu_{y};#sigma_{y}  [#mum];#mu_{y}",
                   v_binX.size() - 1, &(v_binX[0]));
      i_sector.second.ResidualWidthY2 =
          new TH1F("h_residualWidthY2",
                   "residual width #Delta_{y};#sigma_{y}  [#mum];#Delta_{y}",
                   v_binX.size() - 1, &(v_binX[0]));
      i_sector.second.CorrectionY2 =
          new TH1F("h_correctionY2",
                   "correction to APE_{y};#sigma_{y}  "
                   "[#mum];#Delta#sigma_{align,y}  [#mum]",
                   v_binX.size() - 1, &(v_binX[0]));
    }
...
void ApeEstimatorSummary::writeHists() {
  TFile *resultsFile =
      new TFile(parameterSet_.getParameter<std::string>("ResultsFile").c_str(),
                "RECREATE");
...
void ApeEstimatorSummary::calculateApe() {
  ...
  // Read in baseline file for calculation of APE value (if not setting
  // baseline) Has same format as iterationFile
  const std::string baselineFileName(
      parameterSet_.getParameter<std::string>("BaselineFile"));
  ...
  if (!setBaseline) {
    ...
    if (baselineFile) {
      ...
      edm::LogWarning("CalculateAPE")
          << "There is NO baseline file for APE values, so normalized residual "
             "width =1 for ideal conditions is assumed";
    }
  }

  // Set up root file for iterations on APE value (or for setting baseline in
  // setBaseline mode)
  const std::string iterationFileName(
  ...
  TFile *iterationFile =
      new TFile(iterationFileName.c_str(), setBaseline ? "RECREATE" : "UPDATE");

  // Set up TTree for iterative APE values on first pass (first iteration) or
  // read from file (further iterations)
  ...
  // The same for TTree containing the names of the sectors (no additional
  // check, since always handled exactly as iterationTree)
  TTree *sectorNameTree(nullptr);
  iterationFile->GetObject("nameTree;1", sectorNameTree);

  bool firstIter(false);
  if (!iterationTreeX) { // should be always true in setBaseline mode, since
                         // file is recreated
    firstIter = true;
    if (!setBaseline) {
      iterationTreeX =
          new TTree("iterTreeX", "Tree for APE x values of all iterations");
      iterationTreeY =
          new TTree("iterTreeY", "Tree for APE y values of all iterations");
      edm::LogInfo("CalculateAPE") << "First APE iteration (number 0.), create "
                                      "iteration file with TTree";
    ...
    edm::LogWarning("CalculateAPE")
        << "NOT the first APE iteration (number 0.) but the " << iteration
        << ". one, is this wanted or forgot to delete old iteration file with "
           "TTree?";
  }
  ...
  for (auto const &i_sector : m_tkSector_) {
    ...
    if (!setBaseline && baselineTreeX) {
      baselineTreeX->SetBranchAddress(ss_sector.str().c_str(),
                                      &a_baselineSectorX[iSector]);
      baselineTreeX->GetEntry(0);
      if (pixelSector) {
        baselineTreeY->SetBranchAddress(ss_sector.str().c_str(),
                                        &a_baselineSectorY[iSector]);
        baselineTreeY->GetEntry(0);
      }
      sectorNameBaselineTree->SetBranchAddress(ss_sector.str().c_str(),
                                               &a_sectorBaselineName[iSector]);
    ...
    if (firstIter) { // should be always true in setBaseline mode, since file is
                     // recreated
      ss_sectorSuffixed << ss_sector.str() << "/D";
      iterationTreeX->Branch(ss_sector.str().c_str(), &a_apeSectorX[iSector],
                             ss_sectorSuffixed.str().c_str());
      if (pixelSector) {
        iterationTreeY->Branch(ss_sector.str().c_str(), &a_apeSectorY[iSector],
                               ss_sectorSuffixed.str().c_str());
      }
      sectorNameTree->Branch(ss_sector.str().c_str(), &a_sectorName[iSector],
                             32000, 00);
    } else {
      iterationTreeX->SetBranchAddress(ss_sector.str().c_str(),
                                       &a_apeSectorX[iSector]);
      iterationTreeX->GetEntry(iterationTreeX->GetEntries() - 1);
      if (pixelSector) {
        iterationTreeY->SetBranchAddress(ss_sector.str().c_str(),
                                         &a_apeSectorY[iSector]);
        iterationTreeY->GetEntry(iterationTreeY->GetEntries() - 1);
      }
      sectorNameTree->SetBranchAddress(ss_sector.str().c_str(),
                                       &a_sectorName[iSector]);
      sectorNameTree->GetEntry(0);
    }
  }

  // Check whether sector definitions are identical with the ones of previous
  // iterations and with the ones in baseline file
  for (auto &i_sector : m_tkSector_) {
    ...
    if (!firstIter) {
      ...
      if (name != nameLastIter) {
        edm::LogError("CalculateAPE")
            ...
            << "...APE calculation stopped. Please check sector definitions in "
               "config!\n";
        return;
      }
    } else {
  ...
  if (!setBaseline) {
    const std::string apeOutputFileName(
        parameterSet_.getParameter<std::string>("ApeOutputFile"));
    ...
    if (apeOutputFile.is_open()) {
      ...
      edm::LogError("CalculateAPE")
          ...
          << "...APE calculation stopped. Please check path of text file name "
             "in config:\n"
          << "\t" << apeOutputFileName;
      return;
    }
  ...
  const std::string apeWeightName(
      parameterSet_.getParameter<std::string>("apeWeight"));
  ...
  if (apeWeight == wInvalid) {
    edm::LogError("CalculateAPE")
        ...
        << "\"\nimplemented apeWeights are \"unity\", \"entries\", "
           "\"entriesOverSigmaX2\""
        << "\n...APE calculation stopped.";
    return;
  }
  const double minHitsPerInterval(
      parameterSet_.getParameter<double>("minHitsPerInterval"));
  const double sigmaFactorFit(
      parameterSet_.getParameter<double>("sigmaFactorFit"));
  const double correctionScaling(
      parameterSet_.getParameter<double>("correctionScaling"));
  const bool smoothIteration(
      parameterSet_.getParameter<bool>("smoothIteration"));
  const double smoothFraction(
      parameterSet_.getParameter<double>("smoothFraction"));
  ...
  for (auto &i_sector : m_tkSector_) {
    ...
    typedef std::pair<double, Error2AndResidualWidth2PerBin>
        WeightAndResultsPerBin;
    ...
    for (auto const &i_errBins : i_sector.second.m_binnedHists) {
      ...
      double maximumX =
          mHists["norResX"]->GetBinContent(mHists["norResX"]->GetMaximumBin());
      ...
      if (integralX > minHitsPerInterval) {
        if (mHists["norResX"]->Fit(&funcX_1, fitOpt)) {
          edm::LogWarning("CalculateAPE")
              << "Fit1 did not work: "
              << mHists["norResX"]->Fit(&funcX_1, fitOpt);
          continue;
        }
        LogDebug("CalculateAPE")
            << "FitResultX1\t" << mHists["norResX"]->Fit(&funcX_1, fitOpt)
            << "\n";
      }
      ...
      if (integralX > minHitsPerInterval) {
        if (mHists["norResX"]->Fit(&funcX_2, fitOpt)) {
          edm::LogWarning("CalculateAPE")
              << "Fit2 did not work for x : "
              << mHists["norResX"]->Fit(&funcX_2, fitOpt);
          continue;
        }
        LogDebug("CalculateAPE")
            << "FitResultX2\t" << mHists["norResX"]->Fit(&funcX_2, fitOpt)
            << "\n";
      }
      ...
      if (i_sector.second.isPixel) {
        ...
        double maximumY = mHists["norResY"]->GetBinContent(
            mHists["norResY"]->GetMaximumBin());
        ...
        if (integralY > minHitsPerInterval) {
          if (mHists["norResY"]->Fit(&funcY_1, fitOpt)) {
            edm::LogWarning("CalculateAPE")
                << "Fit1 did not work: "
                << mHists["norResY"]->Fit(&funcY_1, fitOpt);
            continue;
          }
          LogDebug("CalculateAPE")
              << "FitResultY1\t" << mHists["norResY"]->Fit(&funcY_1, fitOpt)
              << "\n";
        }
        ...
        if (integralY > minHitsPerInterval) {
          if (mHists["norResY"]->Fit(&funcY_2, fitOpt)) {
            edm::LogWarning("CalculateAPE")
                << "Fit2 did not work for y : "
                << mHists["norResY"]->Fit(&funcY_2, fitOpt);
            continue;
          }
          LogDebug("CalculateAPE")
              << "FitResultY2\t" << mHists["norResY"]->Fit(&funcY_2, fitOpt)
              << "\n";
        }
      ...
      correctionX2_1 = meanSigmaX * meanSigmaX *
                       (residualWidthX_1 * residualWidthX_1 - baselineWidthX2);
      correctionX2_2 = meanSigmaX * meanSigmaX *
                       (residualWidthX_2 * residualWidthX_2 - baselineWidthX2);
      if (i_sector.second.isPixel) {
        correctionY2_1 =
            meanSigmaY * meanSigmaY *
            (residualWidthY_1 * residualWidthY_1 - baselineWidthY2);
        correctionY2_2 =
            meanSigmaY * meanSigmaY *
            (residualWidthY_2 * residualWidthY_2 - baselineWidthY2);
      }

      double correctionX_1 = correctionX2_1 >= 0. ? std::sqrt(correctionX2_1)
                                                  : -std::sqrt(-correctionX2_1);
      double correctionX_2 = correctionX2_2 >= 0. ? std::sqrt(correctionX2_2)
                                                  : -std::sqrt(-correctionX2_2);
      double correctionY_1 = correctionY2_1 >= 0. ? std::sqrt(correctionY2_1)
                                                  : -std::sqrt(-correctionY2_1);
      double correctionY_2 = correctionY2_2 >= 0. ? std::sqrt(correctionY2_2)
                                                  : -std::sqrt(-correctionY2_2);
      // Meanwhile, this got very bad default values, or? (negative corrections
      // allowed)
      ...
      i_sector.second.ResidualWidthX1->SetBinContent(i_errBins.first,
                                                     residualWidthX_1);
      i_sector.second.CorrectionX1->SetBinContent(i_errBins.first,
                                                  correctionX_1 * 10000.);

      i_sector.second.FitMeanX2->SetBinContent(i_errBins.first, fitMeanX_2);
      i_sector.second.ResidualWidthX2->SetBinContent(i_errBins.first,
                                                     residualWidthX_2);
      i_sector.second.CorrectionX2->SetBinContent(i_errBins.first,
                                                  correctionX_2 * 10000.);
      ...
      if (i_sector.second.isPixel) {
        ...
        i_sector.second.ResidualWidthY1->SetBinContent(i_errBins.first,
                                                       residualWidthY_1);
        i_sector.second.CorrectionY1->SetBinContent(i_errBins.first,
                                                    correctionY_1 * 10000.);

        i_sector.second.FitMeanY2->SetBinContent(i_errBins.first, fitMeanY_2);
        i_sector.second.ResidualWidthY2->SetBinContent(i_errBins.first,
                                                       residualWidthY_2);
        i_sector.second.CorrectionY2->SetBinContent(i_errBins.first,
                                                    correctionY_2 * 10000.);
      }
      ...
      const Error2AndResidualWidth2PerBin error2AndResidualWidth2PerBinX(
          meanSigmaX * meanSigmaX, residualWidthX_1 * residualWidthX_1);
      const WeightAndResultsPerBin weightAndResultsPerBinX(
          weightX, error2AndResidualWidth2PerBinX);
      if (!(entriesX < minHitsPerInterval)) {
      ...
      const Error2AndResidualWidth2PerBin error2AndResidualWidth2PerBinY(
          meanSigmaY * meanSigmaY, residualWidthY_1 * residualWidthY_1);
      const WeightAndResultsPerBin weightAndResultsPerBinY(
          weightY, error2AndResidualWidth2PerBinY);
      if (!(entriesY < minHitsPerInterval)) {
        // Fill absolute weights
    ...
    if (!setBaseline) {
      ...
      for (auto const &i_apeBin : v_weightAndResultsPerBinX) {
        if (firstIntervalX) {
          correctionX2 = i_apeBin.first * i_apeBin.second.first *
                         (i_apeBin.second.second - baselineWidthX2);
      ...
      for (auto const &i_apeBin : v_weightAndResultsPerBinX) {
        numeratorX +=
            i_apeBin.first * i_apeBin.second.first * i_apeBin.second.second;
    ...
    if (i_sector.second.isPixel) {
      if (!setBaseline) {
        ...
        for (auto const &i_apeBin : v_weightAndResultsPerBinY) {
          if (firstIntervalY) {
            correctionY2 = i_apeBin.first * i_apeBin.second.first *
                           (i_apeBin.second.second - baselineWidthY2);
    ...
    if (!setBaseline) {
      ...
      // scale APE Correction with value given in cfg (not if smoothed iteration
      // is used)
      edm::LogInfo("CalculateAPE")
          << "Unscaled correction x for sector " << i_sector.first << " is "
          << (correctionX2 > 0. ? +1. : -1.) *
      ...
      if (i_sector.second.isPixel) {
        edm::LogInfo("CalculateAPE")
            << "Unscaled correction y for sector " << i_sector.first << " is "
            << (correctionY2 > 0. ? +1. : -1.) *
      ...
      if (!smoothIteration || firstIter) {
        ...
        const double apeXtmp = smoothFraction * std::sqrt(apeX2new) +
                               (1 - smoothFraction) * std::sqrt(apeX2old);
        const double apeYtmp = smoothFraction * std::sqrt(apeY2new) +
                               (1 - smoothFraction) * std::sqrt(apeY2old);
      ...
      for (auto const &i_rawId : i_sector.second.v_rawId) {
        if (i_sector.second.isPixel) {
          apeOutputFile << i_rawId << " " << std::fixed << std::setprecision(5)
                        << apeX << " " << apeY << " " << apeZ << "\n";
        } else {
          apeOutputFile << i_rawId << " " << std::fixed << std::setprecision(5)
                        << apeX << " " << apeX << " " << apeX << "\n";
    ...
    } else { // In setBaseline mode, just fill estimated mean value of residual
             // width
  ...
  iterationTreeX->Write("iterTreeX",
                        TObject::kOverwrite); // TObject::kOverwrite needed to
                                              // not produce another iterTreeX;2
  iterationTreeY->Fill();
  iterationTreeY->Write("iterTreeY",
                        TObject::kOverwrite); // TObject::kOverwrite needed to
                                              // not produce another iterTreeY;2
  if (firstIter) {
    sectorNameTree->Fill();
...
void ApeEstimatorSummary::analyze(const edm::Event &iEvent,
  ...
  // Load APEs from the GT and write them to root files similar to the ones from
  // calculateAPE()
  ...
  if (firstEvent) {
    ...
    if (!setBaseline) {
      ...
      if (baselineFile) {
        ...
        edm::LogWarning("CalculateAPE")
            << "There is NO baseline file for APE values, so normalized "
               "residual width =1 for ideal conditions is assumed";
      }
    }

    // Set up root file for default APE values
    const std::string defaultFileName(
        parameterSet_.getParameter<std::string>("DefaultFile"));
    TFile *defaultFile = new TFile(defaultFileName.c_str(), "RECREATE");

    // Naming in the root files has to be iterTreeX to be consistent for the
    // plotting tool
    ...
    // The same for TTree containing the names of the sectors (no additional
    // check, since always handled exactly as defaultTree)
    TTree *sectorNameTree(nullptr);
    defaultFile->GetObject("nameTree;1", sectorNameTree);

    bool firstIter(false);
    if (!defaultTreeX) { // should be always true in setBaseline mode, since
                         // file is recreated
      firstIter = true;
      defaultTreeX =
          new TTree("iterTreeX", "Tree for default APE x values from GT");
      defaultTreeY =
          new TTree("iterTreeY", "Tree for default APE y values from GT");
      edm::LogInfo("CalculateAPE")
          << "First APE iteration (number 0.), create default file with TTree";
      sectorNameTree = new TTree("nameTree", "Tree with names of sectors");
    } else {
      edm::LogWarning("CalculateAPE")
          << "NOT the first APE iteration (number 0.), is this wanted or "
             "forgot to delete old iteration file with TTree?";
    }
    ...
    for (auto const &i_sector : m_tkSector_) {
      ...
      if (!setBaseline && sectorNameBaselineTree) {
        sectorNameBaselineTree->SetBranchAddress(
            ss_sector.str().c_str(), &a_sectorBaselineName[iSector]);
        sectorNameBaselineTree->GetEntry(0);
      }

      if (firstIter) { // should be always true in setBaseline mode, since file
                       // is recreated
        ss_sectorSuffixed << ss_sector.str() << "/D";
        defaultTreeX->Branch(ss_sector.str().c_str(),
                             &a_defaultSectorX[iSector],
                             ss_sectorSuffixed.str().c_str());
        if (pixelSector) {
          defaultTreeY->Branch(ss_sector.str().c_str(),
                               &a_defaultSectorY[iSector],
                               ss_sectorSuffixed.str().c_str());
        }
        sectorNameTree->Branch(ss_sector.str().c_str(), &a_sectorName[iSector],
                               32000, 00);
      } else {
        defaultTreeX->SetBranchAddress(ss_sector.str().c_str(),
                                       &a_defaultSectorX[iSector]);
        defaultTreeX->GetEntry(0);
        if (pixelSector) {
          defaultTreeY->SetBranchAddress(ss_sector.str().c_str(),
                                         &a_defaultSectorY[iSector]);
      ...
      if (!firstIter) {
        ...
        if (name != nameLastIter) {
          edm::LogError("CalculateAPE")
              ...
              << "Recent iteration has name \"" << name
              << "\", while previous had \"" << nameLastIter << "\"\n"
              << "...APE calculation stopped. Please check sector definitions "
                 "in config!\n";
          return;
        }
      } else
    ...
    for (auto &i_sector : m_tkSector_) {
      ...
      for (auto const &i_rawId : i_sector.second.v_rawId) {
        std::vector<AlignTransformErrorExtended> alignErrors =
            alignmentErrors->m_alignError;
        for (std::vector<AlignTransformErrorExtended>::const_iterator
                 i_alignError = alignErrors.begin();
             i_alignError != alignErrors.end(); ++i_alignError) {
