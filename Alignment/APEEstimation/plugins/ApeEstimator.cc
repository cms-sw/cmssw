  ...
  for(std::map<unsigned int,TrackerSectorStruct>::const_iterator i_sector = m_tkSector_.begin(); i_sector != m_tkSector_.end(); ++i_sector){
    ...
    for(++i_sector2; i_sector2 != m_tkSector_.end(); ++i_sector2){
      ...
      for(std::vector<unsigned int>::const_iterator i_module = (*i_sector).second.v_rawId.begin(); i_module != (*i_sector).second.v_rawId.end(); ++i_module){
        for(unsigned int i_module2 : (*i_sector2).second.v_rawId){
...
ApeEstimator::bookSectorHistsForAnalyzerMode(){
  ...
  for(auto & i_sector : m_tkSector_){
    ...
    for(unsigned int & v_errHist : v_errHists){
      double xMin(0.01*(v_errHist-1)), xMax(0.01*v_errHist);
      std::stringstream sigmaXHit, sigmaXTrk, sigmaX;
      sigmaXHit << "h_sigmaXHit_" << v_errHist;
      sigmaXTrk << "h_sigmaXTrk_" << v_errHist;
      sigmaX    << "h_sigmaX_"    << v_errHist;
      ...
      sigmaYHit << "h_sigmaYHit_" << v_errHist;
      sigmaYTrk << "h_sigmaYTrk_" << v_errHist;
      sigmaY    << "h_sigmaY_"    << v_errHist;
...
ApeEstimator::hitSelected(TrackStruct::HitParameterStruct& hitParams)const{
  ...
  for(const auto & i_hitSelection : m_hitSelection_){
  ...
  for(const auto & i_hitSelection : m_hitSelectionUInt_){
    const std::string& hitSelection(i_hitSelection.first);
...
ApeEstimator::fillHistsForAnalyzerMode(const TrackStruct& trackStruct){
  ...
  for(std::vector<TrackStruct::HitParameterStruct>::const_iterator i_hit = trackStruct.v_hitParams.begin();
    ...
    for(auto & i_sector : m_tkSector_){
      ...
      for(unsigned int i_hitSector : hit.v_sector){
...
ApeEstimator::fillHistsForApeCalculation(const TrackStruct& trackStruct){
  ...
  for(const auto & v_hitParam : trackStruct.v_hitParams){
    // Put here from earlier method
    if(v_hitParam.hitState == TrackStruct::notAssignedToSectors)continue;
    
    for(auto & i_sector : m_tkSector_){
      
      bool moduleInSector(false);
      for(std::vector<unsigned int>::const_iterator i_hitSector = v_hitParam.v_sector.begin(); i_hitSector != v_hitParam.v_sector.end(); ++i_hitSector){
	if(i_sector.first == *i_hitSector){moduleInSector = true; break;}
      }
      if(!moduleInSector)continue;      
      ...
      if(v_hitParam.goodXMeasurement){
        for(std::map<unsigned int,std::pair<double,double> >::const_iterator i_errBins = m_resErrBins_.begin();
            i_errBins != m_resErrBins_.end(); ++i_errBins){
	  ...
	  if(v_hitParam.errXWoApe < (*i_errBins).second.first || v_hitParam.errXWoApe >= (*i_errBins).second.second){
	    continue;
	  }
	  i_sector.second.m_binnedHists[(*i_errBins).first]["sigmaX"] ->Fill(v_hitParam.errXWoApe);
	  i_sector.second.m_binnedHists[(*i_errBins).first]["norResX"]->Fill(v_hitParam.norResX);
	  break;
        }
	i_sector.second.ResX->Fill(v_hitParam.resX*10000.);
	i_sector.second.NorResX->Fill(v_hitParam.norResX);
      }
      
      if(v_hitParam.goodYMeasurement){
        for(std::map<unsigned int,std::pair<double,double> >::const_iterator i_errBins = m_resErrBins_.begin();
            i_errBins != m_resErrBins_.end(); ++i_errBins){
	  // Separate the bins for residual resolution w/o APE, to be consistent within iterations where APE will change (have same hit always in same bin)
	  // So also fill this value in the histogram sigmaY
	  // But of course use the normalized residual regarding the APE to have its influence in its width
	  if(v_hitParam.errYWoApe < (*i_errBins).second.first || v_hitParam.errYWoApe >= (*i_errBins).second.second){
	    continue;
	  }
	  i_sector.second.m_binnedHists[(*i_errBins).first]["sigmaY"] ->Fill(v_hitParam.errYWoApe);
	  i_sector.second.m_binnedHists[(*i_errBins).first]["norResY"]->Fill(v_hitParam.norResY);
	  break;
        }
	i_sector.second.ResY->Fill(v_hitParam.resY*10000.);
	i_sector.second.NorResY->Fill(v_hitParam.norResY);
      }
    }
  }
}
