...
// Class:      ApeEstimator
//
/**\class ApeEstimator ApeEstimator.cc
 Alignment/APEEstimation/src/ApeEstimator.cc
...
private:
  struct PositionAndError2 {
    PositionAndError2()
        : posX(-999.F), posY(-999.F), errX2(-999.F), errY2(-999.F){};
    PositionAndError2(float x, float y, float eX, float eY)
        : posX(x), posY(y), errX2(eX), errY2(eY){};
    float posX;
    float posY;
    float errX2;
    float errY2;
  };
  typedef std::pair<TrackStruct::HitState, PositionAndError2>
  ...
  void sectorBuilder();
  bool checkIntervalsForSectors(const unsigned int sectorCounter,
                                const std::vector<double> &) const;
  bool checkModuleIds(const unsigned int,
  ...
  TrackStruct::TrackParameterStruct fillTrackVariables(const reco::Track &,
                                                       const Trajectory &,
                                                       const reco::BeamSpot &);
  TrackStruct::HitParameterStruct
  fillHitVariables(const TrajectoryMeasurement &, const edm::EventSetup &);

  StatePositionAndError2 positionAndError2(const LocalPoint &,
                                           const LocalError &,
                                           const TransientTrackingRecHit &);
  PositionAndError2 rectangularPositionAndError2(const LocalPoint &,
                                                 const LocalError &);
  PositionAndError2 radialPositionAndError2(const LocalPoint &,
                                            const LocalError &,
                                            const RadialStripTopology &);
  ...
  bool inUintInterval(const std::vector<unsigned int> &, const unsigned int,
                      const unsigned int = 999) const;

  void fillHistsForAnalyzerMode(const TrackStruct &);
  void fillHitHistsXForAnalyzerMode(const TrackStruct::HitParameterStruct &,
                                    TrackerSectorStruct &);
  void fillHitHistsYForAnalyzerMode(const TrackStruct::HitParameterStruct &,
                                    TrackerSectorStruct &);
...
ApeEstimator::ApeEstimator(const edm::ParameterSet &iConfig)
    : parameterSet_(iConfig),
      tjTagToken_(consumes<TrajTrackAssociationCollection>(
          parameterSet_.getParameter<edm::InputTag>("tjTkAssociationMapTag"))),
      offlinebeamSpot_(
          consumes<reco::BeamSpot>(edm::InputTag("offlineBeamSpot"))),
      trackCut_(false),
      maxTracksPerEvent_(
          parameterSet_.getParameter<unsigned int>("maxTracksPerEvent")),
      minGoodHitsPerTrack_(
          parameterSet_.getParameter<unsigned int>("minGoodHitsPerTrack")),
...
void ApeEstimator::sectorBuilder() {
  ...
  TFile *tkTreeFile(TFile::Open(
      (parameterSet_.getParameter<std::string>("TrackerTreeFile")).c_str()));
  if (tkTreeFile) {
  ...
  unsigned int rawId(999), subdetId(999), layer(999), side(999), half(999),
      rod(999), ring(999), petal(999), blade(999), panel(999), outerInner(999),
  ...
  float posR(999.F), posPhi(999.F), posEta(999.F), posX(999.F), posY(999.F),
      posZ(999.F);
  ...
  std::vector<edm::ParameterSet> v_sectorDef(
      parameterSet_.getParameter<std::vector<edm::ParameterSet>>("Sectors"));
  edm::LogInfo("SectorBuilder")
      << "There are " << v_sectorDef.size() << " Sectors definded";
  for (auto const &parSet : v_sectorDef) {
    ...
    std::vector<unsigned int> v_rawId(
        parSet.getParameter<std::vector<unsigned int>>("rawId")),
    ...
    std::vector<int> v_uDirection(
        parSet.getParameter<std::vector<int>>("uDirection")),
        v_vDirection(parSet.getParameter<std::vector<int>>("vDirection")),
        v_wDirection(parSet.getParameter<std::vector<int>>("wDirection"));
    std::vector<double> v_posR(
        parSet.getParameter<std::vector<double>>("posR")),
    ...
    if (isPixel && isStrip) {
      edm::LogError("SectorBuilder")
          << "Incorrect Sector Definition: there are pixel and strip modules "
             "within one sector"
    ...
    edm::LogInfo("SectorBuilder") << "There are " << tkSector.v_rawId.size()
...
bool ApeEstimator::checkIntervalsForSectors(
    const unsigned int sectorCounter, const std::vector<double> &v_id) const {

  if (v_id.empty())
    return true;
  if (v_id.size() % 2 == 1) {
    edm::LogError("SectorBuilder")
        << "Incorrect Sector Definition: Position Vectors need even number of "
           "arguments (Intervals)"
        << "\n... sector selection is not applied, sector " << sectorCounter
        << " is not built";
    return false;
  ...
  for (auto const &i_id : v_id) {
    ...
    if (entry % 2 == 0 && intervalBegin > i_id) {
      edm::LogError("SectorBuilder")
          ...
          << intervalBegin << " is bigger than " << i_id
...
bool ApeEstimator::checkModuleIds(const unsigned int id,
                                  const std::vector<unsigned int> &v_id) const {
...
bool ApeEstimator::checkModuleBools(
    const bool id, const std::vector<unsigned int> &v_id) const {
...
bool ApeEstimator::checkModuleDirections(const int id,
                                         const std::vector<int> &v_id) const {
...
void ApeEstimator::statistics(const TrackerSectorStruct &allSectors,
                              const int nModules) const {
  bool commonModules(false);
  for (std::map<unsigned int, TrackerSectorStruct>::const_iterator i_sector =
           m_tkSector_.begin();
       i_sector != m_tkSector_.end(); ++i_sector) {
    std::map<unsigned int, TrackerSectorStruct>::const_iterator i_sector2(
        i_sector);
    for (++i_sector2; i_sector2 != m_tkSector_.end(); ++i_sector2) {
      ...
      if (nCommonModules == 0)
        ; // edm::LogInfo("SectorBuilder")<<"Sector "<<(*i_sector).first<<" and
          // Sector "<<(*i_sector2).first<< " have ZERO Modules in common";
      else {
        edm::LogError("SectorBuilder")
            << "Sector " << (*i_sector).first << " and Sector "
            << (*i_sector2).first << " have " << nCommonModules
            << " Modules in common";
        commonModules = true;
      }
    }
  ...
  if (static_cast<int>(allSectors.v_rawId.size()) == nModules)
    ...
    edm::LogWarning("SectorBuilder")
        << "There are " << allSectors.v_rawId.size()
        << " Modules in all Sectors"
        << " out of " << nModules << " Tracker Modules";
  if (!commonModules)
    edm::LogInfo("SectorBuilder") << "There are ZERO modules associated to "
                                     "different sectors, no ambiguities exist";
  else
    edm::LogError("SectorBuilder")
        << "There are modules associated to different sectors, APE value "
           "cannot be assigned reasonably";
}

// -----------------------------------------------------------------------------------------------------------

void ApeEstimator::residualErrorBinning() {
  std::vector<double> v_residualErrorBinning(
      parameterSet_.getParameter<std::vector<double>>("residualErrorBinning"));
  if (v_residualErrorBinning.size() == 1) {
    edm::LogError("ResidualErrorBinning")
        << "Incorrect selection of Residual Error Bins (used for APE "
           "calculation): \t"
  ...
  edm::LogInfo("ResidualErrorBinning")
      << m_resErrBins_.size()
      << " Intervals of residual errors used for separate APE calculation "
         "sucessfully set";
}
...
void ApeEstimator::bookSectorHistsForAnalyzerMode() {
  std::vector<unsigned int> v_errHists(
      parameterSet_.getParameter<std::vector<unsigned int>>("vErrHists"));
  for (std::vector<unsigned int>::iterator i_errHists = v_errHists.begin();
       i_errHists != v_errHists.end(); ++i_errHists) {
    for (std::vector<unsigned int>::iterator i_errHists2 = i_errHists;
         i_errHists2 != v_errHists.end();) {
      ++i_errHists2;
      if (*i_errHists == *i_errHists2) {
        edm::LogError("BookSectorHists")
  ...
  for (auto &i_sector : m_tkSector_) {
    ...
    i_sector.second.Name =
        secDir.make<TH1F>("z_name", i_sector.second.name.c_str(), 1, 0, 1);
    ...
    i_sector.second.setCorrHistParams(&secDir, norResXAbsMax, sigmaXHitMax,
                                      sigmaXMax);
    ...
    // Cluster Parameters
    i_sector.second.m_correlationHistsX["WidthX"] =
        i_sector.second.bookCorrHistsX(
            "WidthX", "cluster width", "w_{cl,x}", "[# channels]",
            static_cast<int>(widthMax), static_cast<int>(widthMax), 0.,
            widthMax, "nph");
    i_sector.second.m_correlationHistsX["BaryStripX"] =
        i_sector.second.bookCorrHistsX(
            "BaryStripX", "barycenter of cluster charge", "b_{cl,x}",
            "[# channels]", 800, 100, -10., 790., "nph");

    if (pixelSector) {
      i_sector.second.m_correlationHistsY["WidthY"] =
          i_sector.second.bookCorrHistsY(
              "WidthY", "cluster width", "w_{cl,y}", "[# channels]",
              static_cast<int>(widthMax), static_cast<int>(widthMax), 0.,
              widthMax, "nph");
      i_sector.second.m_correlationHistsY["BaryStripY"] =
          i_sector.second.bookCorrHistsY(
              "BaryStripY", "barycenter of cluster charge", "b_{cl,y}",
              "[# channels]", 800, 100, -10., 790., "nph");

      i_sector.second.m_correlationHistsX["ChargePixel"] =
          i_sector.second.bookCorrHistsX("ChargePixel", "cluster charge",
                                         "c_{cl}", "[e]", 100, 50, 0.,
                                         chargePixelMax, "nph");
      i_sector.second.m_correlationHistsX["ClusterProbXY"] =
          i_sector.second.bookCorrHistsX(
              "ClusterProbXY", "cluster probability xy", "prob_{cl,xy}", "",
              100, 50, 0., 1., "nph");
      i_sector.second.m_correlationHistsX["ClusterProbQ"] =
          i_sector.second.bookCorrHistsX("ClusterProbQ",
                                         "cluster probability q", "prob_{cl,q}",
                                         "", 100, 50, 0., 1., "nph");
      i_sector.second.m_correlationHistsX["ClusterProbXYQ"] =
          i_sector.second.bookCorrHistsX(
              "ClusterProbXYQ", "cluster probability xyq", "prob_{cl,xyq}", "",
              100, 50, 0., 1., "nph");
      i_sector.second.m_correlationHistsX["LogClusterProb"] =
          i_sector.second.bookCorrHistsX(
              "LogClusterProb", "cluster probability xy", "log(prob_{cl,xy})",
              "", 60, 30, logClusterProbMin, 0., "nph");
      i_sector.second.m_correlationHistsX["IsOnEdge"] =
          i_sector.second.bookCorrHistsX("IsOnEdge", "IsOnEdge", "isOnEdge", "",
                                         2, 2, 0, 2, "nph");
      i_sector.second.m_correlationHistsX["HasBadPixels"] =
          i_sector.second.bookCorrHistsX("HasBadPixels", "HasBadPixels",
                                         "hasBadPixels", "", 2, 2, 0, 2, "nph");
      i_sector.second.m_correlationHistsX["SpansTwoRoc"] =
          i_sector.second.bookCorrHistsX("SpansTwoRoc", "SpansTwoRoc",
                                         "spansTwoRoc", "", 2, 2, 0, 2, "nph");
      i_sector.second.m_correlationHistsX["QBin"] =
          i_sector.second.bookCorrHistsX("QBin", "q bin", "q bin", "", 8, 8, 0,
                                         8, "nph");

      i_sector.second.m_correlationHistsY["ChargePixel"] =
          i_sector.second.bookCorrHistsY("ChargePixel", "cluster charge",
                                         "c_{cl}", "[e]", 100, 50, 0.,
                                         chargePixelMax, "nph");
      i_sector.second.m_correlationHistsY["ClusterProbXY"] =
          i_sector.second.bookCorrHistsY(
              "ClusterProbXY", "cluster probability xy", "prob_{cl,xy}", "",
              100, 50, 0., 1., "nph");
      i_sector.second.m_correlationHistsY["ClusterProbQ"] =
          i_sector.second.bookCorrHistsY("ClusterProbQ",
                                         "cluster probability q", "prob_{cl,q}",
                                         "", 100, 50, 0., 1., "nph");
      i_sector.second.m_correlationHistsY["ClusterProbXYQ"] =
          i_sector.second.bookCorrHistsY(
              "ClusterProbXYQ", "cluster probability xyq", "prob_{cl,xyq}", "",
              100, 50, 0., 1., "nph");
      i_sector.second.m_correlationHistsY["LogClusterProb"] =
          i_sector.second.bookCorrHistsY(
              "LogClusterProb", "cluster probability xy", "log(prob_{cl,xy})",
              "", 60, 30, logClusterProbMin, 0., "nph");
      i_sector.second.m_correlationHistsY["IsOnEdge"] =
          i_sector.second.bookCorrHistsY("IsOnEdge", "IsOnEdge", "isOnEdge", "",
                                         2, 2, 0, 2, "nph");
      i_sector.second.m_correlationHistsY["HasBadPixels"] =
          i_sector.second.bookCorrHistsY("HasBadPixels", "HasBadPixels",
                                         "hasBadPixels", "", 2, 2, 0, 2, "nph");
      i_sector.second.m_correlationHistsY["SpansTwoRoc"] =
          i_sector.second.bookCorrHistsY("SpansTwoRoc", "SpansTwoRoc",
                                         "spansTwoRoc", "", 2, 2, 0, 2, "nph");
      i_sector.second.m_correlationHistsY["QBin"] =
          i_sector.second.bookCorrHistsY("QBin", "q bin", "q bin", "", 8, 8, 0,
                                         8, "nph");
    }

    else {
      i_sector.second.m_correlationHistsX["ChargeStrip"] =
          i_sector.second.bookCorrHistsX("ChargeStrip", "cluster charge",
                                         "c_{cl}", "[APV counts]", 100, 50, 0.,
                                         chargeStripMax, "nph");
      i_sector.second.m_correlationHistsX["MaxStrip"] =
          i_sector.second.bookCorrHistsX("MaxStrip", "strip with max. charge",
                                         "n_{cl,max}", "[# strips]", 800, 800,
                                         -10., 790., "npht");
      i_sector.second.m_correlationHistsX["MaxCharge"] =
          i_sector.second.bookCorrHistsX(
              "MaxCharge", "charge of strip with max. charge", "c_{cl,max}",
              "[APV counts]", 300, 75, -10., 290., "nph");
      i_sector.second.m_correlationHistsX["MaxIndex"] =
          i_sector.second.bookCorrHistsX(
              "MaxIndex", "cluster-index of strip with max. charge",
              "i_{cl,max}", "[# strips]", 10, 10, 0., 10., "nph");
      i_sector.second.m_correlationHistsX["ChargeOnEdges"] =
          i_sector.second.bookCorrHistsX(
              "ChargeOnEdges", "fraction of charge on edge strips",
              "(c_{st,L}+c_{st,R})/c_{cl}", "", 60, 60, -0.1, 1.1, "nph");
      i_sector.second.m_correlationHistsX["ChargeAsymmetry"] =
          i_sector.second.bookCorrHistsX(
              "ChargeAsymmetry", "asymmetry of charge on edge strips",
              "(c_{st,L}-c_{st,R})/c_{cl}", "", 110, 55, -1.1, 1.1, "nph");
      i_sector.second.m_correlationHistsX["ChargeLRplus"] =
          i_sector.second.bookCorrHistsX(
              "ChargeLRplus", "fraction of charge not on maxStrip",
              "(c_{cl,L}+c_{cl,R})/c_{cl}", "", 60, 60, -0.1, 1.1, "nph");
      i_sector.second.m_correlationHistsX["ChargeLRminus"] =
          i_sector.second.bookCorrHistsX(
              "ChargeLRminus", "asymmetry of charge L and R of maxStrip",
              "(c_{cl,L}-c_{cl,R})/c_{cl}", "", 110, 55, -1.1, 1.1, "nph");
      i_sector.second.m_correlationHistsX["SOverN"] =
          i_sector.second.bookCorrHistsX("SOverN", "signal over noise", "s/N",
                                         "", 100, 50, 0, sOverNMax, "nph");
      i_sector.second.m_correlationHistsX["WidthProj"] =
          i_sector.second.bookCorrHistsX("WidthProj", "projected width",
                                         "w_{p}", "[# strips]", 200, 20, 0.,
                                         widthMax, "nph");
      i_sector.second.m_correlationHistsX["WidthDiff"] =
          i_sector.second.bookCorrHistsX(
              "WidthDiff", "width difference", "w_{p} - w_{cl}", "[# strips]",
              200, 20, -widthMax / 2., widthMax / 2., "nph");

      i_sector.second.WidthVsWidthProjected = secDir.make<TH2F>(
          "h2_widthVsWidthProj",
          "w_{cl} vs. w_{p};w_{p}  [# strips];w_{cl}  [# strips]",
          static_cast<int>(widthMax), 0, widthMax, static_cast<int>(widthMax),
          0, widthMax);
      i_sector.second.PWidthVsWidthProjected = secDir.make<TProfile>(
          "p_widthVsWidthProj",
          "w_{cl} vs. w_{p};w_{p}  [# strips];w_{cl}  [# strips]",
      ...
      i_sector.second.WidthDiffVsMaxStrip =
          secDir.make<TH2F>("h2_widthDiffVsMaxStrip",
                            "(w_{p} - w_{cl}) vs. n_{cl,max};n_{cl,max};w_{p} "
                            "- w_{cl}  [# strips]",
                            800, -10., 790., static_cast<int>(widthMax),
                            -widthMax / 2., widthMax / 2.);
      i_sector.second.PWidthDiffVsMaxStrip = secDir.make<TProfile>(
          "p_widthDiffVsMaxStrip",
          "(w_{p} - w_{cl}) vs. n_{cl,max};n_{cl,max};w_{p} - w_{cl}  [# "
          "strips]",
          800, -10., 790.);

      i_sector.second.WidthDiffVsSigmaXHit = secDir.make<TH2F>(
          "h2_widthDiffVsSigmaXHit",
          "(w_{p} - w_{cl}) vs. #sigma_{hit,x};#sigma_{hit,x}  [cm];w_{p} - "
          "w_{cl}  [# strips]",
          100, 0., sigmaXMax, 100, -10., 10.);
      i_sector.second.PWidthDiffVsSigmaXHit = secDir.make<TProfile>(
          "p_widthDiffVsSigmaXHit",
          "(w_{p} - w_{cl}) vs. #sigma_{hit,x};#sigma_{hit,x}  [cm];w_{p} - "
          "w_{cl}  [# strips]",
          100, 0., sigmaXMax);

      i_sector.second.WidthVsPhiSensX = secDir.make<TH2F>(
          "h2_widthVsPhiSensX",
          "w_{cl} vs. #phi_{module,x};#phi_{module,x}  [ ^{o}];w_{cl}  [# "
          "strips]",
          93, -93, 93, static_cast<int>(widthMax), 0, widthMax);
      i_sector.second.PWidthVsPhiSensX =
          secDir.make<TProfile>("p_widthVsPhiSensX",
                                "w_{cl} vs. #phi_{module,x};#phi_{module,x}  [ "
                                "^{o}];w_{cl}  [# strips]",
                                93, -93, 93);
    }

    // Hit Parameters (transform errors and residuals from [cm] in [mum])
    i_sector.second.m_correlationHistsX["SigmaXHit"] =
        i_sector.second.bookCorrHistsX(
            "SigmaXHit", "hit error", "#sigma_{hit,x}", "[#mum]", 105, 20,
            sigmaXMin * 10000., sigmaXMax * 10000., "np");
    i_sector.second.m_correlationHistsX["SigmaXTrk"] =
        i_sector.second.bookCorrHistsX(
            "SigmaXTrk", "track error", "#sigma_{trk,x}", "[#mum]", 105, 20,
            sigmaXMin * 10000., sigmaXMax * 10000., "np");
    i_sector.second.m_correlationHistsX["SigmaX"] =
        i_sector.second.bookCorrHistsX(
            "SigmaX", "residual error", "#sigma_{r,x}", "[#mum]", 105, 20,
            sigmaXMin * 10000., sigmaXMax * 10000., "np");
    i_sector.second.m_correlationHistsX["PhiSens"] =
        i_sector.second.bookCorrHistsX("PhiSens", "track angle on sensor",
                                       "#phi_{module}", "[ ^{o}]", 96, 48, -3,
                                       93, "nphtr");
    i_sector.second.m_correlationHistsX["PhiSensX"] =
        i_sector.second.bookCorrHistsX("PhiSensX", "track angle on sensor",
                                       "#phi_{module,x}", "[ ^{o}]", 186, 93,
                                       -phiSensXMax, phiSensXMax, "nphtr");
    i_sector.second.m_correlationHistsX["PhiSensY"] =
        i_sector.second.bookCorrHistsX("PhiSensY", "track angle on sensor",
                                       "#phi_{module,y}", "[ ^{o}]", 186, 93,
                                       -93, 93, "nphtr");

    i_sector.second.XHit = secDir.make<TH1F>(
        "h_XHit", " hit measurement x_{hit};x_{hit}  [cm];# hits", 100, -20,
        20);
    i_sector.second.XTrk = secDir.make<TH1F>(
        "h_XTrk", "track prediction x_{trk};x_{trk}  [cm];# hits", 100, -20,
        20);
    i_sector.second.SigmaX2 = secDir.make<TH1F>(
        "h_SigmaX2",
        "squared residual error #sigma_{r,x}^{2};#sigma_{r,x}^{2}  "
        "[#mum^{2}];# hits",
        105, sigmaXMin * 10000., sigmaX2Max * 10000. * 10000.); // no mistake !
    i_sector.second.ResX = secDir.make<TH1F>(
        "h_ResX", "residual r_{x};x_{trk}-x_{hit}  [#mum];# hits", 100,
        -resXAbsMax * 10000., resXAbsMax * 10000.);
    i_sector.second.NorResX = secDir.make<TH1F>(
        "h_NorResX",
        "normalized residual "
        "r_{x}/#sigma_{r,x};(x_{trk}-x_{hit})/#sigma_{r,x};# hits",
        100, -norResXAbsMax, norResXAbsMax);
    i_sector.second.ProbX = secDir.make<TH1F>(
        "h_ProbX",
        "residual probability;prob(r_{x}^{2}/#sigma_{r,x}^{2},1);# hits", 60,
        probXMin, probXMax);

    i_sector.second.PhiSensXVsBarycentreX =
        secDir.make<TH2F>("h2_phiSensXVsBarycentreX",
                          "#phi_{module,x} vs. b_{cl,x};b_{cl,x}  [# "
                          "channels];#phi_{module,x}  [ ^{o}]",
                          200, -10., 790., 93, -93, 93);
    ...
    if (pixelSector) {
      i_sector.second.m_correlationHistsY["SigmaYHit"] =
          i_sector.second.bookCorrHistsY(
              "SigmaYHit", "hit error", "#sigma_{hit,y}", "[#mum]", 105, 20,
              sigmaXMin * 10000., sigmaXMax * 10000., "np");
      i_sector.second.m_correlationHistsY["SigmaYTrk"] =
          i_sector.second.bookCorrHistsY(
              "SigmaYTrk", "track error", "#sigma_{trk,y}", "[#mum]", 105, 20,
              sigmaXMin * 10000., sigmaXMax * 10000., "np");
      i_sector.second.m_correlationHistsY["SigmaY"] =
          i_sector.second.bookCorrHistsY(
              "SigmaY", "residual error", "#sigma_{r,y}", "[#mum]", 105, 20,
              sigmaXMin * 10000., sigmaXMax * 10000., "np");
      i_sector.second.m_correlationHistsY["PhiSens"] =
          i_sector.second.bookCorrHistsY("PhiSens", "track angle on sensor",
                                         "#phi_{module}", "[ ^{o}]", 96, 48, -3,
                                         93, "nphtr");
      i_sector.second.m_correlationHistsY["PhiSensX"] =
          i_sector.second.bookCorrHistsY("PhiSensX", "track angle on sensor",
                                         "#phi_{module,x}", "[ ^{o}]", 186, 93,
                                         -phiSensXMax, phiSensXMax, "nphtr");
      i_sector.second.m_correlationHistsY["PhiSensY"] =
          i_sector.second.bookCorrHistsY("PhiSensY", "track angle on sensor",
                                         "#phi_{module,y}", "[ ^{o}]", 186, 93,
                                         -93, 93, "nphtr");

      i_sector.second.YHit = secDir.make<TH1F>(
          "h_YHit", " hit measurement y_{hit};y_{hit}  [cm];# hits", 100, -20,
          20);
      i_sector.second.YTrk = secDir.make<TH1F>(
          "h_YTrk", "track prediction y_{trk};y_{trk}  [cm];# hits", 100, -20,
          20);
      i_sector.second.SigmaY2 = secDir.make<TH1F>(
          "h_SigmaY2",
          "squared residual error #sigma_{r,y}^{2};#sigma_{r,y}^{2}  "
          "[#mum^{2}];# hits",
          105, sigmaXMin * 10000.,
          sigmaX2Max * 10000. * 10000.); // no mistake !
      i_sector.second.ResY = secDir.make<TH1F>(
          "h_ResY", "residual r_{y};y_{trk}-y_{hit}  [#mum];# hits", 100,
          -resXAbsMax * 10000., resXAbsMax * 10000.);
      i_sector.second.NorResY = secDir.make<TH1F>(
          "h_NorResY",
          "normalized residual "
          "r_{y}/#sigma_{r,y};(y_{trk}-y_{hit})/#sigma_{r,y};# hits",
          100, -norResXAbsMax, norResXAbsMax);
      i_sector.second.ProbY = secDir.make<TH1F>(
          "h_ProbY",
          "residual probability;prob(r_{y}^{2}/#sigma_{r,y}^{2},1);# hits", 60,
          probXMin, probXMax);

      i_sector.second.PhiSensYVsBarycentreY =
          secDir.make<TH2F>("h2_phiSensYVsBarycentreY",
                            "#phi_{module,y} vs. b_{cl,y};b_{cl,y}  [# "
                            "channels];#phi_{module,y}  [ ^{o}]",
                            200, -10., 790., 93, -93, 93);
    ...
    i_sector.second.m_correlationHistsX["HitsValid"] =
        i_sector.second.bookCorrHistsX("HitsValid", "# hits", "[valid]", 50, 0,
                                       50, "npt");
    i_sector.second.m_correlationHistsX["HitsInvalid"] =
        i_sector.second.bookCorrHistsX("HitsInvalid", "# hits", "[invalid]", 20,
                                       0, 20, "npt");
    i_sector.second.m_correlationHistsX["Hits2D"] =
        i_sector.second.bookCorrHistsX("Hits2D", "# hits", "[2D]", 20, 0, 20,
                                       "npt");
    i_sector.second.m_correlationHistsX["LayersMissed"] =
        i_sector.second.bookCorrHistsX("LayersMissed", "# layers", "[missed]",
                                       10, 0, 10, "npt");
    i_sector.second.m_correlationHistsX["HitsPixel"] =
        i_sector.second.bookCorrHistsX("HitsPixel", "# hits", "[pixel]", 10, 0,
                                       10, "npt");
    i_sector.second.m_correlationHistsX["HitsStrip"] =
        i_sector.second.bookCorrHistsX("HitsStrip", "# hits", "[strip]", 40, 0,
                                       40, "npt");
    i_sector.second.m_correlationHistsX["HitsGood"] =
        i_sector.second.bookCorrHistsX("HitsGood", "# hits", "[good]", 50, 0,
                                       50, "npt");
    i_sector.second.m_correlationHistsX["NorChi2"] =
        i_sector.second.bookCorrHistsX("NorChi2", "#chi^{2}/f", "", 50, 0,
                                       norChi2Max, "npr");
    i_sector.second.m_correlationHistsX["Theta"] =
        i_sector.second.bookCorrHistsX("Theta", "#theta", "[ ^{o}]", 40, -10,
                                       190, "npt");
    i_sector.second.m_correlationHistsX["Phi"] = i_sector.second.bookCorrHistsX(
        "Phi", "#phi", "[ ^{o}]", 76, -190, 190, "npt");
    i_sector.second.m_correlationHistsX["D0Beamspot"] =
        i_sector.second.bookCorrHistsX("D0Beamspot", "d_{0, BS}", "[cm]", 40,
                                       -d0Max, d0Max, "npt");
    i_sector.second.m_correlationHistsX["Dz"] = i_sector.second.bookCorrHistsX(
        "Dz", "d_{z}", "[cm]", 40, -dzMax, dzMax, "npt");
    i_sector.second.m_correlationHistsX["Pt"] = i_sector.second.bookCorrHistsX(
        "Pt", "p_{t}", "[GeV]", 50, 0, pMax, "npt");
    i_sector.second.m_correlationHistsX["P"] =
        i_sector.second.bookCorrHistsX("P", "|p|", "[GeV]", 50, 0, pMax, "npt");
    i_sector.second.m_correlationHistsX["InvP"] =
        i_sector.second.bookCorrHistsX("InvP", "1/|p|", "[GeV^{-1}]", 25, 0,
                                       invPMax, "t");
    i_sector.second.m_correlationHistsX["MeanAngle"] =
        i_sector.second.bookCorrHistsX("MeanAngle", "<#phi_{module}>",
                                       "[ ^{o}]", 25, -5, 95, "npt");
    // i_sector.second.m_correlationHistsX[""] =
    // i_sector.second.bookCorrHistsX("","","",,,,"nphtr");

    if (pixelSector) {
      i_sector.second.m_correlationHistsY["HitsValid"] =
          i_sector.second.bookCorrHistsY("HitsValid", "# hits", "[valid]", 50,
                                         0, 50, "npt");
      i_sector.second.m_correlationHistsY["HitsInvalid"] =
          i_sector.second.bookCorrHistsY("HitsInvalid", "# hits", "[invalid]",
                                         20, 0, 20, "npt");
      i_sector.second.m_correlationHistsY["Hits2D"] =
          i_sector.second.bookCorrHistsY("Hits2D", "# hits", "[2D]", 20, 0, 20,
                                         "npt");
      i_sector.second.m_correlationHistsY["LayersMissed"] =
          i_sector.second.bookCorrHistsY("LayersMissed", "# layers", "[missed]",
                                         10, 0, 10, "npt");
      i_sector.second.m_correlationHistsY["HitsPixel"] =
          i_sector.second.bookCorrHistsY("HitsPixel", "# hits", "[pixel]", 10,
                                         0, 10, "npt");
      i_sector.second.m_correlationHistsY["HitsStrip"] =
          i_sector.second.bookCorrHistsY("HitsStrip", "# hits", "[strip]", 40,
                                         0, 40, "npt");
      i_sector.second.m_correlationHistsY["HitsGood"] =
          i_sector.second.bookCorrHistsY("HitsGood", "# hits", "[good]", 50, 0,
                                         50, "npt");
      i_sector.second.m_correlationHistsY["NorChi2"] =
          i_sector.second.bookCorrHistsY("NorChi2", "#chi^{2}/f", "", 50, 0,
                                         norChi2Max, "npr");
      i_sector.second.m_correlationHistsY["Theta"] =
          i_sector.second.bookCorrHistsY("Theta", "#theta", "[ ^{o}]", 40, -10,
                                         190, "npt");
      i_sector.second.m_correlationHistsY["Phi"] =
          i_sector.second.bookCorrHistsY("Phi", "#phi", "[ ^{o}]", 76, -190,
                                         190, "npt");
      i_sector.second.m_correlationHistsY["D0Beamspot"] =
          i_sector.second.bookCorrHistsY("D0Beamspot", "d_{0, BS}", "[cm]", 40,
                                         -d0Max, d0Max, "npt");
      i_sector.second.m_correlationHistsY["Dz"] =
          i_sector.second.bookCorrHistsY("Dz", "d_{z}", "[cm]", 40, -dzMax,
                                         dzMax, "npt");
      i_sector.second.m_correlationHistsY["Pt"] =
          i_sector.second.bookCorrHistsY("Pt", "p_{t}", "[GeV]", 50, 0, pMax,
                                         "npt");
      i_sector.second.m_correlationHistsY["P"] = i_sector.second.bookCorrHistsY(
          "P", "|p|", "[GeV]", 50, 0, pMax, "npt");
      i_sector.second.m_correlationHistsY["InvP"] =
          i_sector.second.bookCorrHistsY("InvP", "1/|p|", "[GeV^{-1}]", 25, 0,
                                         invPMax, "t");
      i_sector.second.m_correlationHistsY["MeanAngle"] =
          i_sector.second.bookCorrHistsY("MeanAngle", "<#phi_{module}>",
                                         "[ ^{o}]", 25, -5, 95, "npt");
    }
    ...
    for (auto const &i_errHists : v_errHists) {
      ...
      i_sector.second.m_sigmaX["sigmaXHit"].push_back(secDir.make<TH1F>(
          sigmaXHit.str().c_str(),
          "hit error #sigma_{hit,x};#sigma_{hit,x}  [#mum];# hits", 100,
          xMin * 10000., xMax * 10000.));
      i_sector.second.m_sigmaX["sigmaXTrk"].push_back(secDir.make<TH1F>(
          sigmaXTrk.str().c_str(),
          "track error #sigma_{trk,x};#sigma_{trk,x}  [#mum];# hits", 100,
          xMin * 10000., xMax * 10000.));
      i_sector.second.m_sigmaX["sigmaX"].push_back(secDir.make<TH1F>(
          sigmaX.str().c_str(),
      ...
      if (pixelSector) {
        ...
        i_sector.second.m_sigmaY["sigmaYHit"].push_back(secDir.make<TH1F>(
            sigmaYHit.str().c_str(),
            "hit error #sigma_{hit,y};#sigma_{hit,y}  [#mum];# hits", 100,
            xMin * 10000., xMax * 10000.));
        i_sector.second.m_sigmaY["sigmaYTrk"].push_back(secDir.make<TH1F>(
            sigmaYTrk.str().c_str(),
            "track error #sigma_{trk,y};#sigma_{trk,y}  [#mum];# hits", 100,
            xMin * 10000., xMax * 10000.));
        i_sector.second.m_sigmaY["sigmaY"].push_back(secDir.make<TH1F>(
            sigmaY.str().c_str(),
...
void ApeEstimator::bookSectorHistsForApeCalculation() {
  ...
  for (auto &i_sector : m_tkSector_) {
    ...
    // Distributions in each interval (stay in [cm], to have all calculations in
    // [cm])
    if (m_resErrBins_
            .empty()) { // default if no selection taken into account: calculate
                        // APE with one bin with residual error 0-100um
    ...
    for (auto const &i_errBins : m_resErrBins_) {
      ...
      i_sector.second.m_binnedHists[i_errBins.first]["sigmaX"] =
          intDir.make<TH1F>(
              "h_sigmaX",
              "residual resolution #sigma_{x};#sigma_{x}  [cm];# hits", 100, 0.,
              0.01);
      i_sector.second.m_binnedHists[i_errBins.first]["norResX"] =
          intDir.make<TH1F>(
              "h_norResX",
              "normalized residual "
              "r_{x}/#sigma_{r,x};(x_{trk}-x_{hit})/#sigma_{r,x};# hits",
      ...
      if (i_sector.second.isPixel) {
        i_sector.second.m_binnedHists[i_errBins.first]["sigmaY"] =
            intDir.make<TH1F>(
                "h_sigmaY",
                "residual resolution #sigma_{y};#sigma_{y}  [cm];# hits", 100,
                0., 0.01);
        i_sector.second.m_binnedHists[i_errBins.first]["norResY"] =
            intDir.make<TH1F>(
                "h_norResY",
                "normalized residual "
                "r_{y}/#sigma_{r,y};(y_{trk}-y_{hit})/#sigma_{r,y};# hits",
    ...
    i_sector.second.RawId = resDir.make<TTree>(
        "rawIdTree", "Tree containing rawIds of all modules in sector");
    ...
    // (transform errors and residuals from [cm] in [mum])
    std::vector<double> v_binX(parameterSet_.getParameter<std::vector<double>>(
        "residualErrorBinning"));
    for (auto &i_binX : v_binX) {
      i_binX *= 10000.;
    }
    i_sector.second.EntriesX =
        resDir.make<TH1F>("h_entriesX", "# hits used;#sigma_{x}  [#mum];# hits",
                          v_binX.size() - 1, &(v_binX[0]));
    if (i_sector.second.isPixel) {
      i_sector.second.EntriesY = resDir.make<TH1F>(
          "h_entriesY", "# hits used;#sigma_{y}  [#mum];# hits",
          v_binX.size() - 1, &(v_binX[0]));
    }

    // In fact these are un-needed Analyzer plots, but I want to have them
    // always for every sector visible (transform errors and residuals from [cm]
    // in [mum])
    i_sector.second.ResX = resDir.make<TH1F>(
        "h_ResX", "residual r_{x};x_{trk}-x_{hit}  [#mum];# hits", 100,
        -0.03 * 10000., 0.03 * 10000.);
...
void ApeEstimator::bookTrackHists() {
  ...
  tkDetector_.TrkSize =
      evtDir.make<TH1F>("h_trackSize", "# tracks  [all];# tracks;# events",
                        trackSizeBins, -1, trackSizeMax);
  tkDetector_.TrkSizeGood =
      evtDir.make<TH1F>("h_trackSizeGood", "# tracks  [good];# tracks;# events",
                        trackSizeBins, -1, trackSizeMax);
  TFileDirectory trkDir = fileService->mkdir("TrackVariables");
  tkDetector_.HitsSize =
      trkDir.make<TH1F>("h_hitsSize", "# hits;# hits;# tracks", 51, -1, 50);
  tkDetector_.HitsValid = trkDir.make<TH1F>(
      "h_hitsValid", "# hits  [valid];# hits  [valid];# tracks", 51, -1, 50);
  tkDetector_.HitsInvalid = trkDir.make<TH1F>(
      "h_hitsInvalid", "# hits  [invalid];# hits  [invalid];# tracks", 21, -1,
      20);
  tkDetector_.Hits2D = trkDir.make<TH1F>(
      "h_hits2D", "# hits  [2D];# hits  [2D];# tracks", 21, -1, 20);
  tkDetector_.LayersMissed = trkDir.make<TH1F>(
      "h_layersMissed", "# layers  [missed];# layers  [missed];# tracks", 11,
      -1, 10);
  tkDetector_.HitsPixel = trkDir.make<TH1F>(
      "h_hitsPixel", "# hits  [pixel];# hits  [pixel];# tracks", 11, -1, 10);
  tkDetector_.HitsStrip = trkDir.make<TH1F>(
      "h_hitsStrip", "# hits  [strip];# hits  [strip];# tracks", 41, -1, 40);
  tkDetector_.Charge =
      trkDir.make<TH1F>("h_charge", "charge q;q  [e];# tracks", 5, -2, 3);
  tkDetector_.Chi2 = trkDir.make<TH1F>("h_chi2", " #chi^{2};#chi^{2};# tracks",
                                       100, 0, chi2Max);
  tkDetector_.Ndof = trkDir.make<TH1F>(
      "h_ndof", "# degrees of freedom f;f;# tracks", 101, -1, 100);
  tkDetector_.NorChi2 =
      trkDir.make<TH1F>("h_norChi2", "normalized #chi^{2};#chi^{2}/f;# tracks",
                        200, 0, norChi2Max);
  tkDetector_.Prob = trkDir.make<TH1F>(
      "h_prob", " #chi^{2} probability;prob(#chi^{2},f);# tracks", 50, 0, 1);
  tkDetector_.Eta = trkDir.make<TH1F>(
      "h_eta", "pseudorapidity #eta;#eta;# tracks", 100, -5, 5);
  tkDetector_.EtaErr = trkDir.make<TH1F>(
      "h_etaErr", "Error of #eta;#sigma(#eta);# tracks", 100, 0, 0.001);
  tkDetector_.EtaSig = trkDir.make<TH1F>(
      "h_etaSig", "Significance of #eta;#eta/#sigma(#eta);# tracks", 100,
      -20000, 20000);
  tkDetector_.Theta = trkDir.make<TH1F>(
      "h_theta", "polar angle #theta;#theta  [ ^{o}];# tracks", 100, -10, 190);
  tkDetector_.Phi = trkDir.make<TH1F>(
      "h_phi", "azimuth angle #phi;#phi  [ ^{o}];# tracks", 190, -190, 190);
  tkDetector_.PhiErr = trkDir.make<TH1F>(
      "h_phiErr", "Error of #phi;#sigma(#phi)  [ ^{o}];# tracks", 100, 0, 0.04);
  tkDetector_.PhiSig = trkDir.make<TH1F>(
      "h_phiSig", "Significance of #phi;#phi/#sigma(#phi)  [ ^{o}];# tracks",
      100, -50000, 50000);
  tkDetector_.D0Beamspot = trkDir.make<TH1F>(
      "h_d0Beamspot",
      "Closest approach d_{0} wrt. beamspot;d_{0, BS}  [cm];# tracks", 200,
      -d0max, d0max);
  tkDetector_.D0BeamspotErr = trkDir.make<TH1F>(
      "h_d0BeamspotErr", "Error of d_{0, BS};#sigma(d_{0, BS})  [cm];# tracks",
      200, 0, 0.01);
  tkDetector_.D0BeamspotSig = trkDir.make<TH1F>(
      "h_d0BeamspotSig",
      "Significance of d_{0, BS};d_{0, BS}/#sigma(d_{0, BS});# tracks", 100, -5,
      5);
  tkDetector_.Dz =
      trkDir.make<TH1F>("h_dz", "Closest approach d_{z};d_{z}  [cm];# tracks",
                        200, -dzmax, dzmax);
  tkDetector_.DzErr = trkDir.make<TH1F>(
      "h_dzErr", "Error of d_{z};#sigma(d_{z})  [cm];# tracks", 200, 0, 0.01);
  tkDetector_.DzSig = trkDir.make<TH1F>(
      "h_dzSig", "Significance of d_{z};d_{z}/#sigma(d_{z});# tracks", 100,
      -10000, 10000);
  tkDetector_.Pt = trkDir.make<TH1F>(
      "h_pt", "transverse momentum p_{t};p_{t}  [GeV];# tracks", 100, 0, pMax);
  tkDetector_.PtErr = trkDir.make<TH1F>(
      "h_ptErr", "Error of p_{t};#sigma(p_{t})  [GeV];# tracks", 100, 0, 1.6);
  tkDetector_.PtSig = trkDir.make<TH1F>(
      "h_ptSig", "Significance of p_{t};p_{t}/#sigma(p_{t});# tracks", 100, 0,
      200);
  tkDetector_.P = trkDir.make<TH1F>(
      "h_p", "momentum magnitude |p|;|p|  [GeV];# tracks", 100, 0, pMax);
  tkDetector_.MeanAngle = trkDir.make<TH1F>(
      "h_meanAngle",
      "mean angle on module <#phi_{module}>;<#phi_{module}>  [ ^{o}];# tracks",
      100, -5, 95);
  tkDetector_.HitsGood = trkDir.make<TH1F>(
      "h_hitsGood", "# hits  [good];# hits  [good];# tracks", 51, -1, 50);

  tkDetector_.MeanAngleVsHits = trkDir.make<TH2F>(
      "h2_meanAngleVsHits",
      "<#phi_{module}> vs. # hits;# hits;<#phi_{module}>  [ ^{o}]", 51, -1, 50,
      50, -5, 95);
  tkDetector_.HitsGoodVsHitsValid = trkDir.make<TH2F>(
      "h2_hitsGoodVsHitsValid",
      "# hits  [good] vs. # hits  [valid];# hits  [valid];# hits  [good]", 51,
      -1, 50, 51, -1, 50);
  tkDetector_.HitsPixelVsEta = trkDir.make<TH2F>(
      "h2_hitsPixelVsEta", "# hits  [pixel] vs. #eta;#eta;# hits  [pixel]", 60,
      -3, 3, 11, -1, 10);
  tkDetector_.HitsPixelVsTheta =
      trkDir.make<TH2F>("h2_hitsPixelVsTheta",
                        "# hits  [pixel] vs. #theta;#theta;# hits  [pixel]",
                        100, -10, 190, 11, -1, 10);
  tkDetector_.HitsStripVsEta = trkDir.make<TH2F>(
      "h2_hitsStripVsEta", "# hits  [strip] vs. #eta;#eta;# hits  [strip]", 60,
      -3, 3, 31, -1, 40);
  tkDetector_.HitsStripVsTheta =
      trkDir.make<TH2F>("h2_hitsStripVsTheta",
                        "# hits  [strip] vs. #theta;#theta;# hits  [strip]",
                        100, -10, 190, 31, -1, 40);
  tkDetector_.PtVsEta =
      trkDir.make<TH2F>("h2_ptVsEta", "p_{t} vs. #eta;#eta;p_{t}  [GeV]", 60,
                        -3, 3, 100, 0, pMax);
  tkDetector_.PtVsTheta =
      trkDir.make<TH2F>("h2_ptVsTheta", "p_{t} vs. #theta;#theta;p_{t}  [GeV]",
                        100, -10, 190, 100, 0, pMax);
  ...
  tkDetector_.PHitsGoodVsHitsValid = trkDir.make<TProfile>(
      ...
      "# hits  [good] vs. # hits  [valid];# hits  [valid];# hits  [good]", 51,
      -1, 50);
  tkDetector_.PHitsPixelVsEta = trkDir.make<TProfile>(
      "p_hitsPixelVsEta", "# hits  [pixel] vs. #eta;#eta;# hits  [pixel]", 60,
      -3, 3);
  tkDetector_.PHitsPixelVsTheta = trkDir.make<TProfile>(
      "p_hitsPixelVsTheta", "# hits  [pixel] vs. #theta;#theta;# hits  [pixel]",
      100, -10, 190);
  tkDetector_.PHitsStripVsEta = trkDir.make<TProfile>(
      "p_hitsStripVsEta", "# hits  [strip] vs. #eta;#eta;# hits  [strip]", 60,
      -3, 3);
  tkDetector_.PHitsStripVsTheta = trkDir.make<TProfile>(
      "p_hitsStripVsTheta", "# hits  [strip] vs. #theta;#theta;# hits  [strip]",
      100, -10, 190);
  tkDetector_.PPtVsEta = trkDir.make<TProfile>(
      "p_ptVsEta", "p_{t} vs. #eta;#eta;p_{t}  [GeV]", 60, -3, 3);
  tkDetector_.PPtVsTheta = trkDir.make<TProfile>(
      "p_ptVsTheta", "p_{t} vs. #theta;#theta;p_{t}  [GeV]", 100, -10, 190);
}

// -----------------------------------------------------------------------------------------------------------

TrackStruct::TrackParameterStruct
ApeEstimator::fillTrackVariables(const reco::Track &track,
                                 const Trajectory &traj,
                                 const reco::BeamSpot &beamSpot) {
  const math::XYZPoint beamPoint(beamSpot.x0(), beamSpot.y0(), beamSpot.z0());
  double d0BeamspotErr =
      std::sqrt(track.d0Error() * track.d0Error() +
  ...
  trkParams.hitsValid = track.found(); // invalid is every hit from every single
                                       // module that expects a hit
  trkParams.hitsInvalid = trkParams.hitsSize - trkParams.hitsValid;
  trkParams.layersMissed =
      track.lost(); // lost hit means, that a crossed layer doesn't contain a
                    // hit (can be more than one invalid hit)
  ...
  for (auto const &i_meas : v_meas) {
    ...
    TrajectoryStateOnSurface tsos =
        tsoscomb(meas.forwardPredictedState(), meas.backwardPredictedState());
    const align::LocalVector mom(tsos.localDirection());
    meanPhiSensToNorm +=
        atan(fabs(sqrt(mom.x() * mom.x() + mom.y() * mom.y()) / mom.z()));
  }
  ...
  if (parameterSet_.getParameter<bool>("applyTrackCuts")) {
    trackCut_ = false;
    if (trkParams.hitsStrip < 11 || trkParams.hits2D < 2 ||
        trkParams.hitsPixel < 2 || // trkParams.hitsInvalid>2 ||
        trkParams.hitsStrip > 35 || trkParams.hitsPixel > 7 ||
        trkParams.norChi2 > 5. || trkParams.pt < 25. || trkParams.pt > 150. ||
        std::abs(trkParams.d0Beamspot) > 0.02 || std::abs(trkParams.dz) > 15.)
      trackCut_ = true;
  } else {
    trackCut_ = false;
  }
...
TrackStruct::HitParameterStruct
ApeEstimator::fillHitVariables(const TrajectoryMeasurement &i_meas,
  ...
  float phiSensX =
      ...
      static_cast<float>(
          m_tkTreeVar_[rawId].vDirection); // check for orientation of E- and B-
                                           // Field (thoughts for barrel)
  float phiSensY = std::atan(std::fabs(mom.y() / mom.z())) *
                   static_cast<float>(m_tkTreeVar_[rawId].vDirection);
  hitParams.phiSens = std::atan(
      std::fabs(std::sqrt(mom.x() * mom.x() + mom.y() * mom.y()) / mom.z()));
  ...
  // use APE also for the hit error, while APE is automatically included in tsos
  // error
  //
  //  no need to add  APE to hitError anymore by Ajay 27 Oct 2014
  const LocalError &errHitApe =
      hit.localPositionError(); // now sum of CPE+APE as said by MARCO?
  ...
  if (Pixel) {
    ...
    if (lape.valid()) {
      errorWithoutAPE =
          LocalError(errHitApe.xx() - lape.xx(), errHitApe.xy() - lape.xy(),
                     errHitApe.yy() - lape.yy());
    }
  }
  if (Strip) {
  ...
  const StatePositionAndError2 positionAndError2Hit =
      this->positionAndError2(lPHit, errHitApe, hit);
  const StatePositionAndError2 positionAndError2HitWoApe =
      this->positionAndError2(lPHit, errHitWoApe, hit);
  edm::LogInfo("CalculateAPE")
      << "errHitWoApe  " << errHitWoApe << "errHitApe  " << errHitApe;

  const StatePositionAndError2 positionAndError2Trk =
      this->positionAndError2(lPTrk, errTrk, hit);
  ...
  if (stateHit == TrackStruct::invalid ||
      stateHitWoApe == TrackStruct::invalid ||
      stateTrk == TrackStruct::invalid) {
    hitParams.hitState = TrackStruct::invalid;
    return hitParams;
  } else if (stateHit == TrackStruct::negativeError ||
             stateHitWoApe == TrackStruct::negativeError ||
    ...
    // edm::LogError("Negative error
    // Value")<<"@SUB=ApeEstimator::fillHitVariables"<<ss_error.str();
  ...
  // Take global orientation into account for residuals (sign is not important
  // for errors)

  float resXprime(999.F), resYprime(999.F), norResXprime(999.F),
      norResYprime(999.F);
  if (m_tkTreeVar_[rawId].uDirection == 1) {
    ...
    edm::LogError("FillHitVariables") << "Incorrect value of uDirection, which "
                                         "gives global module orientation";
    hitParams.hitState = TrackStruct::invalid;
    return hitParams;
  }
  if (m_tkTreeVar_[rawId].vDirection == 1) {
    ...
    edm::LogError("FillHitVariables") << "Incorrect value of vDirection, which "
                                         "gives global module orientation";
    hitParams.hitState = TrackStruct::invalid;
    return hitParams;
  }
  ...
  if (m_tkTreeVar_[rawId].subdetId == PixelSubdetector::PixelBarrel ||
    ...
    hitParams.logClusterProbability =
        std::log10(hitParams.clusterProbabilityXY);
    ...
    if (!(dynamic_cast<const SiStripRecHit2D *>(&recHit) ||
      ...
      edm::LogError("FillHitVariables")
          << "RecHit in Strip is 'Matched' or 'Projected', but here all should "
             "be monohits per module";
      hitParams.hitState = TrackStruct::invalid;
      return hitParams;
    }
    ...
    if (m_tkTreeVar_[rawId].subdetId == StripSubdetector::TIB ||
      ...
      if (dynamic_cast<const SiStripRecHit1D *>(&recHit)) {
        ...
        edm::LogWarning("FillHitVariables")
            << "Data has TIB/TOB hits as SiStripRecHit2D and not 1D. Probably "
               "data is processed with CMSSW<34X. Nevertheless everything "
               "should work fine";
    ...
    const SiStripClusterInfo clusterInfo =
        SiStripClusterInfo(stripCluster, iSetup, rawId, std::string(""));

    const std::vector<uint8_t>::const_iterator stripChargeL(
        clusterInfo.stripCharges().begin());
    const std::vector<uint8_t>::const_iterator stripChargeR(
        --(clusterInfo.stripCharges().end()));
    const std::pair<uint16_t, uint16_t> stripChargeLR =
        std::make_pair(*stripChargeL, *stripChargeR);
    ...
    hitParams.maxStripInv =
        m_tkTreeVar_[rawId].nStrips - hitParams.maxStrip + 1;
    ...
    hitParams.chargeAsymmetry =
        static_cast<float>(stripChargeLR.first - stripChargeLR.second) /
        static_cast<float>(stripChargeLR.first + stripChargeLR.second);
    hitParams.chargeLRplus = static_cast<float>(clusterInfo.chargeLR().first +
                                                clusterInfo.chargeLR().second) /
                             static_cast<float>(hitParams.chargeStrip);
    hitParams.chargeLRminus =
        static_cast<float>(clusterInfo.chargeLR().first -
                           clusterInfo.chargeLR().second) /
        static_cast<float>(hitParams.chargeStrip);
    ...
    iSetup.get<SiStripLorentzAngleDepRcd>().get(
        lorentzAngleHandle); // MODIFIED BY LOIC QUERTENMONT

    const StripGeomDetUnit *stripDet = (const StripGeomDetUnit *)(&detUnit);
    const MagneticField *magField(magFieldHandle.product());
    LocalVector bField =
        (stripDet->surface())
            .toLocal(magField->inTesla(stripDet->surface().position()));
    const SiStripLorentzAngle *lorentzAngle(lorentzAngleHandle.product());
    float tanLorentzAnglePerTesla =
        lorentzAngle->getLorentzAngle(stripDet->geographicalId().rawId());
    ...
    float maxLength =
        std::sqrt(std::pow(bounds.length(), 2) + std::pow(bounds.width(), 2));
    float thickness = bounds.thickness();

    const StripTopology &topol =
        dynamic_cast<const StripTopology &>(detUnit.type().topology());
    ...
    float projEdge1 =
        topol.measurementPosition(momentumPos - 0.5 * scaledMomentumDir).x();
    if (projEdge1 < 0.)
      projEdge1 = 0.;
...
ApeEstimator::positionAndError2(const LocalPoint &localPoint,
                                const LocalError &localError,
                                const TransientTrackingRecHit &hit) {
  StatePositionAndError2 vPE2 =
      std::make_pair(TrackStruct::invalid, PositionAndError2());
  ...
  if (localError.xx() < 0. || localError.yy() < 0.) {
    // Do not print error message by default
    // edm::LogError("Negative error
    // Value")<<"@SUB=ApeEstimator::fillHitVariables"
    //                                     <<"One of the squared error methods
  ...
  if (subdetId == PixelSubdetector::PixelBarrel ||
    ...
    vPE2 = std::make_pair(TrackStruct::ok, this->rectangularPositionAndError2(
    ...
    const RadialStripTopology &topol =
        dynamic_cast<const RadialStripTopology &>(detUnit.type().topology());
    ...
    vPE2 = std::make_pair(TrackStruct::ok, this->radialPositionAndError2(
                                               localPoint, localError, topol));
  } else {
    edm::LogError("FillHitVariables")
        << "Incorrect subdetector ID, hit not associated to tracker";
  }
...
ApeEstimator::rectangularPositionAndError2(const LocalPoint &lP,
                                           const LocalError &lE) {
  const float x(lP.x());
  const float y(lP.y());
...
ApeEstimator::PositionAndError2
ApeEstimator::radialPositionAndError2(const LocalPoint &lP,
                                      const LocalError &lE,
                                      const RadialStripTopology &topol) {
  ...
  x = phi * r_0;
  // Radial y (not symmetric around 0; radial distance with minimum at middle
  // strip at lower edge [0, yMax])
  ...
  // Radial y (not symmetric around 0, real radial distance from intersection
  // point)
  const float cosPhi4(std::pow(cosPhi, 4)),
      sinPhi2(std::sin(phi) * std::sin(phi));
...
void ApeEstimator::hitSelection() {
  ...
  for (auto &i_hitSelection : m_hitSelection_) {
    if (!i_hitSelection.second.empty()) {
      int entry(1);
      double intervalBegin(999.);
      for (std::vector<double>::iterator i_hitInterval =
               i_hitSelection.second.begin();
           i_hitInterval != i_hitSelection.second.end(); ++entry) {
        if (entry % 2 == 1) {
          intervalBegin = *i_hitInterval;
          ++i_hitInterval;
        } else {
          if (intervalBegin > *i_hitInterval) {
            edm::LogError("HitSelector")
                << "INVALID Interval selected for  " << i_hitSelection.first
                << ":\t" << intervalBegin << " > " << (*i_hitInterval)
                << "\n ... delete Selection for " << i_hitSelection.first;
            i_hitSelection.second.clear();
            i_hitInterval = i_hitSelection.second
                                .begin(); // emptyMap = true; i_hitSelection =
                                          // m_hitSelection_.begin();
          } else {
  ...
  for (auto &i_hitSelection : m_hitSelectionUInt_) {
    if (!i_hitSelection.second.empty()) {
      ...
      for (std::vector<unsigned int>::iterator i_hitInterval =
               i_hitSelection.second.begin();
           i_hitInterval != i_hitSelection.second.end(); ++entry) {
        if (entry % 2 == 1) {
          intervalBegin = *i_hitInterval;
...
void ApeEstimator::setHitSelectionMap(const std::string &cutVariable) {
  edm::ParameterSet parSet(
      parameterSet_.getParameter<edm::ParameterSet>("HitSelector"));
  std::vector<double> v_cutVariable(
      parSet.getParameter<std::vector<double>>(cutVariable));
...
void ApeEstimator::setHitSelectionMapUInt(const std::string &cutVariable) {
  edm::ParameterSet parSet(
      parameterSet_.getParameter<edm::ParameterSet>("HitSelector"));
  std::vector<unsigned int> v_cutVariable(
      parSet.getParameter<std::vector<unsigned int>>(cutVariable));
  if (v_cutVariable.size() % 2 == 1) {
    edm::LogError("HitSelector")
        << "Invalid Hit Selection for " << cutVariable
        << ": need even number of arguments (intervals)"
...
bool ApeEstimator::hitSelected(
    TrackStruct::HitParameterStruct &hitParams) const {
  if (hitParams.hitState == TrackStruct::notInTracker)
    return false;
  if (hitParams.hitState == TrackStruct::invalid ||
      hitParams.hitState == TrackStruct::negativeError)
    return false;
  ...
  for (auto &i_hitSelection : m_hitSelection_) {
    ...
    if (hitParams.isPixelHit) {
      if (hitSelection == "chargePixel") {
        ...
        if (!this->inDoubleInterval(v_hitSelection,
                                    hitParams.clusterProbabilityXY))
          isGoodHit = false;
      } else if (hitSelection == "clusterProbabilityQ") {
        if (!this->inDoubleInterval(v_hitSelection,
                                    hitParams.clusterProbabilityQ))
          isGoodHit = false;
      } else if (hitSelection == "clusterProbabilityXYQ") {
        if (!this->inDoubleInterval(v_hitSelection,
                                    hitParams.clusterProbabilityXYQ))
          isGoodHit = false;
      ...
      if (hitSelection == "widthProj") {
        ...
        if (!this->inDoubleInterval(v_hitSelection,
                                    hitParams.projWidth -
                                        static_cast<float>(hitParams.widthX)))
          isGoodHit = false;
  ...
  for (auto &i_hitSelection : m_hitSelectionUInt_) {
    ...
    if (hitParams.isPixelHit) {
      ...
      if (hitSelection == "width") {
        ...
        if (!this->inUintInterval(v_hitSelection, hitParams.maxStrip,
                                  hitParams.maxStripInv))
          isGoodHit = false;
...
bool ApeEstimator::inUintInterval(
    const std::vector<unsigned int> &v_hitSelection,
    const unsigned int variable, const unsigned int variable2) const {
  int entry(0);
  unsigned int intervalBegin(999);
  bool isSelected(false);
  for (auto i_hitInterval : v_hitSelection) {
    ...
    if (entry % 2 == 1)
      ...
      if (variable2 == 999 ||
          (variable2 >= intervalBegin && variable2 <= i_hitInterval))
        isSelected = true;
    }
  }
  return isSelected;
...
void ApeEstimator::fillHistsForAnalyzerMode(const TrackStruct &trackStruct) {
  ...
  tkDetector_.HitsGoodVsHitsValid->Fill(trackStruct.trkParams.hitsValid,
                                        goodHitsPerTrack);
  tkDetector_.PHitsGoodVsHitsValid->Fill(trackStruct.trkParams.hitsValid,
                                         goodHitsPerTrack);
  ...
  tkDetector_.EtaSig->Fill(trackStruct.trkParams.eta /
                           trackStruct.trkParams.etaErr);
  ...
  tkDetector_.PhiSig->Fill(trackStruct.trkParams.phi /
                           trackStruct.trkParams.phiErr);
  ...
  tkDetector_.D0BeamspotSig->Fill(trackStruct.trkParams.d0Beamspot /
                                  trackStruct.trkParams.d0BeamspotErr);
  tkDetector_.Dz->Fill(trackStruct.trkParams.dz);
  tkDetector_.DzErr->Fill(trackStruct.trkParams.dzErr);
  tkDetector_.DzSig->Fill(trackStruct.trkParams.dz /
                          trackStruct.trkParams.dzErr);
  ...
  tkDetector_.PtSig->Fill(trackStruct.trkParams.pt /
                          trackStruct.trkParams.ptErr);
  tkDetector_.MeanAngle->Fill(trackStruct.trkParams.meanPhiSensToNorm * 180. /
                              M_PI);

  tkDetector_.MeanAngleVsHits->Fill(trackStruct.trkParams.hitsSize,
                                    trackStruct.trkParams.meanPhiSensToNorm *
                                        180. / M_PI);
  tkDetector_.HitsPixelVsEta->Fill(trackStruct.trkParams.eta,
                                   trackStruct.trkParams.hitsPixel);
  tkDetector_.HitsPixelVsTheta->Fill(trackStruct.trkParams.theta * 180. / M_PI,
                                     trackStruct.trkParams.hitsPixel);
  tkDetector_.HitsStripVsEta->Fill(trackStruct.trkParams.eta,
                                   trackStruct.trkParams.hitsStrip);
  tkDetector_.HitsStripVsTheta->Fill(trackStruct.trkParams.theta * 180. / M_PI,
                                     trackStruct.trkParams.hitsStrip);
  tkDetector_.PtVsEta->Fill(trackStruct.trkParams.eta,
                            trackStruct.trkParams.pt);
  tkDetector_.PtVsTheta->Fill(trackStruct.trkParams.theta * 180. / M_PI,
                              trackStruct.trkParams.pt);

  tkDetector_.PMeanAngleVsHits->Fill(trackStruct.trkParams.hitsSize,
                                     trackStruct.trkParams.meanPhiSensToNorm *
                                         180. / M_PI);
  tkDetector_.PHitsPixelVsEta->Fill(trackStruct.trkParams.eta,
                                    trackStruct.trkParams.hitsPixel);
  tkDetector_.PHitsPixelVsTheta->Fill(trackStruct.trkParams.theta * 180. / M_PI,
                                      trackStruct.trkParams.hitsPixel);
  tkDetector_.PHitsStripVsEta->Fill(trackStruct.trkParams.eta,
                                    trackStruct.trkParams.hitsStrip);
  tkDetector_.PHitsStripVsTheta->Fill(trackStruct.trkParams.theta * 180. / M_PI,
                                      trackStruct.trkParams.hitsStrip);
  tkDetector_.PPtVsEta->Fill(trackStruct.trkParams.eta,
                             trackStruct.trkParams.pt);
  tkDetector_.PPtVsTheta->Fill(trackStruct.trkParams.theta * 180. / M_PI,
                               trackStruct.trkParams.pt);
  ...
  for (auto &i_hit : trackStruct.v_hitParams) {
    ...
    for (auto &i_sector : m_tkSector_) {
      ...
      if (hit.goodXMeasurement) {
        std::map<std::string, TrackerSectorStruct::CorrelationHists>
            &m_corrHists(sector.m_correlationHistsX);

        // Cluster and Hit Parameters
        this->fillHitHistsXForAnalyzerMode(hit, sector);

        // Track Parameters
        m_corrHists["HitsValid"].fillCorrHistsX(
            hit, trackStruct.trkParams.hitsValid);
        m_corrHists["HitsGood"].fillCorrHistsX(hit, goodHitsPerTrack);
        m_corrHists["HitsInvalid"].fillCorrHistsX(
            hit, trackStruct.trkParams.hitsInvalid);
        m_corrHists["Hits2D"].fillCorrHistsX(hit, trackStruct.trkParams.hits2D);
        m_corrHists["LayersMissed"].fillCorrHistsX(
            hit, trackStruct.trkParams.layersMissed);
        m_corrHists["HitsPixel"].fillCorrHistsX(
            hit, trackStruct.trkParams.hitsPixel);
        m_corrHists["HitsStrip"].fillCorrHistsX(
            hit, trackStruct.trkParams.hitsStrip);
        m_corrHists["NorChi2"].fillCorrHistsX(hit,
                                              trackStruct.trkParams.norChi2);
        m_corrHists["Theta"].fillCorrHistsX(hit, trackStruct.trkParams.theta *
                                                     180. / M_PI);
        m_corrHists["Phi"].fillCorrHistsX(hit, trackStruct.trkParams.phi *
                                                   180. / M_PI);
        m_corrHists["D0Beamspot"].fillCorrHistsX(
            hit, trackStruct.trkParams.d0Beamspot);
        ...
        m_corrHists["MeanAngle"].fillCorrHistsX(
            hit, trackStruct.trkParams.meanPhiSensToNorm * 180. / M_PI);
      }

      if (hit.goodYMeasurement) {
        std::map<std::string, TrackerSectorStruct::CorrelationHists>
            &m_corrHists(sector.m_correlationHistsY);

        // Cluster and Hit Parameters
        this->fillHitHistsYForAnalyzerMode(hit, sector);

        // Track Parameters
        m_corrHists["HitsValid"].fillCorrHistsY(
            hit, trackStruct.trkParams.hitsValid);
        ...
        m_corrHists["LayersMissed"].fillCorrHistsY(
            hit, trackStruct.trkParams.layersMissed);
        m_corrHists["HitsPixel"].fillCorrHistsY(
            hit, trackStruct.trkParams.hitsPixel);
        m_corrHists["HitsStrip"].fillCorrHistsY(
            hit, trackStruct.trkParams.hitsStrip);
        m_corrHists["NorChi2"].fillCorrHistsY(hit,
                                              trackStruct.trkParams.norChi2);
        m_corrHists["Theta"].fillCorrHistsY(hit, trackStruct.trkParams.theta *
                                                     180. / M_PI);
        m_corrHists["Phi"].fillCorrHistsY(hit, trackStruct.trkParams.phi *
                                                   180. / M_PI);
        m_corrHists["D0Beamspot"].fillCorrHistsY(
            hit, trackStruct.trkParams.d0Beamspot);
        ...
        m_corrHists["MeanAngle"].fillCorrHistsY(
            hit, trackStruct.trkParams.meanPhiSensToNorm * 180. / M_PI);
      }
...
void ApeEstimator::fillHitHistsXForAnalyzerMode(
    const TrackStruct::HitParameterStruct &hit, TrackerSectorStruct &sector) {
  std::map<std::string, TrackerSectorStruct::CorrelationHists> &m_corrHists(
      sector.m_correlationHistsX);
  ...
  if (hit.isPixelHit) {
    ...
    m_corrHists["ClusterProbXYQ"].fillCorrHistsX(hit,
                                                 hit.clusterProbabilityXYQ);
    m_corrHists["LogClusterProb"].fillCorrHistsX(hit,
    ...
    m_corrHists["WidthDiff"].fillCorrHistsX(
        hit, hit.projWidth - static_cast<float>(hit.widthX));

    sector.WidthVsWidthProjected->Fill(hit.projWidth, hit.widthX);
    sector.PWidthVsWidthProjected->Fill(hit.projWidth, hit.widthX);

    sector.WidthDiffVsMaxStrip->Fill(
        hit.maxStrip, hit.projWidth - static_cast<float>(hit.widthX));
    sector.PWidthDiffVsMaxStrip->Fill(
        hit.maxStrip, hit.projWidth - static_cast<float>(hit.widthX));

    sector.WidthDiffVsSigmaXHit->Fill(
        hit.errXHit, hit.projWidth - static_cast<float>(hit.widthX));
    sector.PWidthDiffVsSigmaXHit->Fill(
        hit.errXHit, hit.projWidth - static_cast<float>(hit.widthX));
  ...
  sector.PhiSensXVsBarycentreX->Fill(hit.baryStripX,
                                     hit.phiSensX * 180. / M_PI);
  sector.PPhiSensXVsBarycentreX->Fill(hit.baryStripX,
                                      hit.phiSensX * 180. / M_PI);
}

void ApeEstimator::fillHitHistsYForAnalyzerMode(
    const TrackStruct::HitParameterStruct &hit, TrackerSectorStruct &sector) {
  std::map<std::string, TrackerSectorStruct::CorrelationHists> &m_corrHists(
      sector.m_correlationHistsY);
  ...
  sector.PhiSensYVsBarycentreY->Fill(hit.baryStripY,
                                     hit.phiSensY * 180. / M_PI);
  sector.PPhiSensYVsBarycentreY->Fill(hit.baryStripY,
                                      hit.phiSensY * 180. / M_PI);
}
  ...
  for (auto const &i_hit : trackStruct.v_hitParams) {
    ...
    for (auto &i_sector : m_tkSector_) {
      ...
      if (i_hit.goodXMeasurement) {
        for (auto const &i_errBins : m_resErrBins_) {
          // Separate the bins for residual resolution w/o APE, to be consistent
          // within iterations where APE will change (have same hit always in
          // same bin) So also fill this value in the histogram sigmaX But of
          // course use the normalized residual regarding the APE to have its
          // influence in its width
          if (i_hit.errXWoApe < i_errBins.second.first ||
              i_hit.errXWoApe >= i_errBins.second.second) {
            continue;
          }
          i_sector.second.m_binnedHists[i_errBins.first]["sigmaX"]->Fill(
              i_hit.errXWoApe);
          i_sector.second.m_binnedHists[i_errBins.first]["norResX"]->Fill(
              i_hit.norResX);
          break;
        }
      ...
      if (i_hit.goodYMeasurement) {
        for (auto const &i_errBins : m_resErrBins_) {
          // Separate the bins for residual resolution w/o APE, to be consistent
          // within iterations where APE will change (have same hit always in
          // same bin) So also fill this value in the histogram sigmaY But of
          // course use the normalized residual regarding the APE to have its
          // influence in its width
          if (i_hit.errYWoApe < i_errBins.second.first ||
              i_hit.errYWoApe >= i_errBins.second.second) {
            continue;
          }
          i_sector.second.m_binnedHists[i_errBins.first]["sigmaY"]->Fill(
              i_hit.errYWoApe);
          i_sector.second.m_binnedHists[i_errBins.first]["norResY"]->Fill(
              i_hit.norResY);
          break;
        }
...
void ApeEstimator::analyze(const edm::Event &iEvent,
  ...
  for (i_trajTrack = m_TrajTracksMap->begin();
       i_trajTrack != m_TrajTracksMap->end(); ++i_trajTrack) {
  ...
  for (iTrack = trajTracks.begin(); iTrack != trajTracks.end(); ++iTrack) {
    ...
    // Loop over Hits
    for (std::vector<TrajectoryMeasurement>::const_iterator i_meas =
             v_meas.begin();
         i_meas != v_meas.end(); ++i_meas) {
      TrackStruct::HitParameterStruct hitParams =
          this->fillHitVariables(*i_meas, iSetup);
