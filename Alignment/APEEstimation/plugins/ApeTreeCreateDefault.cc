...
//
/**\class ApeTreeCreateDefault ApeTreeCreateDefault.cc
 Alignment/APEEstimation/src/ApeTreeCreateDefault.cc
...
ApeTreeCreateDefault::ApeTreeCreateDefault(const edm::ParameterSet &iConfig)
    : resultFile_(iConfig.getParameter<std::string>("resultFile")),
      ...
      sectors_(
          iConfig.getParameter<std::vector<edm::ParameterSet>>("sectors")) {}
...
void ApeTreeCreateDefault::sectorBuilder() {
  ...
  edm::LogInfo("SectorBuilder")
      << "There are " << v_sectorDef.size() << " Sectors defined";
...
bool ApeTreeCreateDefault::checkIntervalsForSectors(
...
bool ApeTreeCreateDefault::checkModuleIds(
    const unsigned int id, const std::vector<unsigned int> &v_id) const {
...
bool ApeTreeCreateDefault::checkModuleBools(
...
bool ApeTreeCreateDefault::checkModuleDirections(
    const int id, const std::vector<int> &v_id) const {
...
bool ApeTreeCreateDefault::checkModulePositions(
    const float id, const std::vector<double> &v_id) const {
...
void ApeTreeCreateDefault::analyze(const edm::Event &iEvent,
  ...
  for (auto const &i_sector : m_tkSector_) {
    ...
    defaultTreeX->Branch(ss_sector.str().c_str(),
                         &(*a_defaultSectorX[iSector - 1]),
                         ss_sectorSuffixed.str().c_str());

    if (pixelSector) {
      defaultTreeY->Branch(ss_sector.str().c_str(),
                           &(*a_defaultSectorY[iSector - 1]),
                           ss_sectorSuffixed.str().c_str());
    }
    sectorNameTree->Branch(ss_sector.str().c_str(),
                           &(*a_sectorName[iSector - 1]), 32000, 00);
  }
...
void ApeTreeCreateDefault::fillDescriptions(
