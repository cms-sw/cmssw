...
void AlignmentMonitorMuonVsCurvature::book()
{
  ...
  {
    if (!m_doDT && wheel > kWheelPlus2) continue;
    if (!m_doCSC && wheel < kWheelMEm11) continue;
    ...
    {
      char sectorname[20];
      sprintf(sectorname,"sector%02d_",sector+1);

      for (int component = 0;  component < kNumComponents;  component++)
      {
	std::stringstream th2f_name, tprofile_name;
	th2f_name << "th2f_" << wheelname.str() << sectorname;
	tprofile_name << "tprofile_" << wheelname.str() << sectorname;

	double yminmax = 15., xminmax = 200.;
	int ynbins = 30; 
	...
	th2f_wheelsector[wheel][sector][component] = book2D("/iterN/", th2f_name.str().c_str(), "", 30, -xminmax, xminmax, ynbins, -yminmax, yminmax);
	tprofile_wheelsector[wheel][sector][component] = bookProfile("/iterN/", tprofile_name.str().c_str(), "", 30,  -xminmax, xminmax);
      }
    }
  }

  if (m_doCSC) for (int endcap = 0;  endcap < kNumEndcap;  endcap++)
  {
	...
	tprofile_odds[endcap][component] = bookProfile("/iterN/", tprofile_odds_name.str().c_str(), "", 25, -0.05, 0.05);
      ...
      {
        char chambername[20];
        sprintf(chambername,"chamber%02d_",chamber+1);

	std::stringstream th2f_name, tprofile_name;
	th2f_name << "th2f_" << endcapname.str() << chambername << componentname.str();
	tprofile_name << "tprofile_" << endcapname.str() << chambername << componentname.str();
...
void AlignmentMonitorMuonVsCurvature::event(const edm::Event &iEvent, const edm::EventSetup &iSetup, const ConstTrajTrackPairCollection& trajtracks)
{
  ...
  for (ConstTrajTrackPairCollection::const_iterator trajtrack = trajtracks.begin();  trajtrack != trajtracks.end();  ++trajtrack)
  {
    ...
    if (track->pt() > m_minTrackPt  && track->p() > m_minTrackP  &&  fabs(track->dxy()) < m_maxDxy)
    {
            ...
            if (cscid.station() == 1)
            {
              ...
              if (csc != NULL  &&  csc->numHits() >= m_minCSCHits)
              {
                ...
                for (std::vector<TrajectoryMeasurement>::const_iterator im = measurements.begin();  im != measurements.end();  ++im) 
                {
                  ...
                  {
                    GlobalPoint pos = tsos.globalPosition();
                    if (pos.perp() < 200.  &&  fabs(pos.z()) < 400. && fabs(pos.z()) > last_tracker_absZ)
                    {
                      last_tracker_tsos = tsos;
