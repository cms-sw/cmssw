...
class TestMTDReader : public edm::one::EDAnalyzer<> {
public:
  explicit TestMTDReader(const edm::ParameterSet&) : aliToken_(esConsumes()), aliErrToken_(esConsumes()), rot(0) {}

  virtual void analyze(const edm::Event&, const edm::EventSetup&);

private:
  // ----------member data ---------------------------
  const edm::ESGetToken<Alignments, MTDAlignmentRcd> aliToken_;
  const edm::ESGetToken<AlignmentErrorsExtended, MTDAlignmentErrorExtendedRcd> aliErrToken_;
  float x, y, z, phi, theta, length, thick, width;
  TRotMatrix* rot;
};

void TestMTDReader::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  edm::LogInfo("MTDAlignment") << "Starting!";
  ...
  edm::LogInfo("MTDAlignment") << "Done!";
}

//define this as a plug-in
DEFINE_FWK_MODULE(TestMTDReader);
