...
///
/// Calibration of back plane corrections for the strip tracker, integrated in
/// the alignment algorithms. Note that not all algorithms support this...
///
...
private:
  ...
  bool checkBackPlaneCorrectionInput(const edm::EventSetup &setup,
  ...
  SiStripBackPlaneCorrection *createFromTree(const char *fileName,
                                             const char *treeName) const;
...
SiStripBackplaneCalibration::SiStripBackplaneCalibration(
    ...
    : IntegratedCalibrationBase(cfg),
      ...
      siStripBackPlaneCorrInput_(nullptr), moduleGroupSelector_(nullptr),
      moduleGroupSelCfg_(
          cfg.getParameter<edm::ParameterSet>("BackplaneModuleGroups")) {
...
SiStripBackplaneCalibration::~SiStripBackplaneCalibration() {
  ...
  //  std::cout << "Destroy SiStripBackplaneCalibration named " << this->name()
  //  << std::endl;
...
//======================================================================
unsigned int SiStripBackplaneCalibration::derivatives(
  ...
  const_cast<SiStripBackplaneCalibration *>(this)
      ->checkBackPlaneCorrectionInput(setup, eventInfo);
  ...
  if (mode == readoutMode_) {
    if (hit.det()) { // otherwise 'constraint hit' or whatever
      ...
      if (index >= 0) { // otherwise not treated
        ...
        // std::cout << "SiStripBackplaneCalibration derivatives " <<
        // readoutModeName_ << std::endl;
        const double dZ =
            hit.det()->surface().bounds().thickness(); // it's a float only...
        const double tanPsi =
            tsos.localParameters().mixedFormatVector()[1]; // float...

        edm::ESHandle<SiStripLorentzAngle> lorentzAngleHandle;
        setup.get<SiStripLorentzAngleRcd>().get(readoutModeName_,
                                                lorentzAngleHandle);
        // Yes, mobility (= LA/By) stored in object called LA...
        const double mobility =
            lorentzAngleHandle->getLorentzAngle(hit.det()->geographicalId());
        // shift due to dead back plane has two parts:
        // 1) Lorentz Angle correction formula gets reduced thickness dz*(1-bp)
        // 2) 'Direct' effect is shift of effective module position in local z
        // by bp*dz/2
        //   (see GF's presentation in alignment meeting 25.10.2012,
        //    https://indico.cern.ch/conferenceDisplay.py?confId=174266#2012-10-25)
        //        const double xDerivative = 0.5 * dZ * (mobility *
        //        bFieldLocal.y() - tanPsi);
        // FIXME: +tanPsi? At least that fits the sign of the dr/dw residual
        //       in KarimakiDerivatives...
        const double xDerivative =
            0.5 * dZ * (mobility * bFieldLocal.y() + tanPsi);
        // std::cout << "derivative is " << xDerivative << " for index " <<
        // index
        // 	  << std::endl;
        //         if (index >= 10) {
        //           std::cout << "mobility * y-field " << mobility *
        //           bFieldLocal.y()
        //                     << ", \n tanPsi " << tanPsi /*<< ", dZ " << dZ */
        //                     << std::endl;
        //           std::cout << "|tanPsi| - |mobility * y-field| "
        //                     << fabs(tanPsi) - fabs(mobility *
        //                     bFieldLocal.y())
        //                     << std::endl;
        //         }
...
//======================================================================
double
SiStripBackplaneCalibration::getParameterError(unsigned int index) const {
...
void SiStripBackplaneCalibration::beginOfJob(AlignableTracker *aliTracker,
  ...
  // specify the sub-detectors for which the back plane correction is
  // determined: all strips
  const std::vector<int> sdets = boost::assign::list_of(SiStripDetId::TIB)(
      SiStripDetId::TID)(SiStripDetId::TOB)(SiStripDetId::TEC);

  moduleGroupSelector_ =
      new TkModuleGroupSelector(aliTracker, moduleGroupSelCfg_, sdets);
...
void SiStripBackplaneCalibration::endOfJob() {
  ...
  const SiStripBackPlaneCorrection *input =
      this->getBackPlaneCorrectionInput(); // never NULL
  const std::string treeName(this->name() + '_' + readoutModeName_ + '_');
  this->writeTree(input, treeInfo,
                  (treeName + "input").c_str()); // empty treeInfo for input...
...
//======================================================================
bool SiStripBackplaneCalibration::checkBackPlaneCorrectionInput(
  ...
  if (!siStripBackPlaneCorrInput_) {
    ...
    siStripBackPlaneCorrInput_ =
        new SiStripBackPlaneCorrection(*backPlaneCorrHandle);
    // FIXME: Should we call 'watchBackPlaneCorrRcd_.check(setup)' as well?
    //        Otherwise could be that next check has to check via following
    //        'else', though no new IOV has started... (to be checked)
  } else {
    if (watchBackPlaneCorrRcd_.check(
            setup)) { // new IOV of input - but how to check peak vs deco?
      setup.get<SiStripBackPlaneCorrectionRcd>().get(readoutModeName_,
                                                     backPlaneCorrHandle);
      if (backPlaneCorrHandle->getBackPlaneCorrections() // but only bad if
                                                         // non-identical values
          != siStripBackPlaneCorrInput_
                 ->getBackPlaneCorrections()) { // (comparing maps)
...
SiStripBackplaneCalibration::getBackPlaneCorrectionInput() {
  ...
  // identical. If this job has run on events, still check that back plane
  // corrections are identical to the ones from mergeFileNames_.
  const std::string treeName(((this->name() + '_') += readoutModeName_) +=
                             "_input");
  for (auto iFile = mergeFileNames_.begin(); iFile != mergeFileNames_.end();
       ++iFile) {
    SiStripBackPlaneCorrection *bpCorr =
        this->createFromTree(iFile->c_str(), treeName.c_str());
    // siStripBackPlaneCorrInput_ could be non-null from previous file of this
    // loop or from checkBackPlaneCorrectionInput(..) when running on data in
    // this job as well
    if (!siStripBackPlaneCorrInput_ ||
        siStripBackPlaneCorrInput_->getBackPlaneCorrections().empty()) {
      ...
      if (bpCorr &&
          !bpCorr->getBackPlaneCorrections()
               .empty() && // single job might not have got events
          bpCorr->getBackPlaneCorrections() !=
              siStripBackPlaneCorrInput_->getBackPlaneCorrections()) {
...
//======================================================================
double
SiStripBackplaneCalibration::getParameterForDetId(unsigned int detId,
...
//======================================================================
void SiStripBackplaneCalibration::writeTree(
  ...
  for (auto iterIdValue =
           backPlaneCorrection->getBackPlaneCorrections().begin();
...
SiStripBackplaneCalibration::createFromTree(const char *fileName,
                                            const char *treeName) const {
