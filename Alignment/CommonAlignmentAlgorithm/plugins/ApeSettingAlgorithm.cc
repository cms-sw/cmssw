 ...
 *#    saveLocalNotGlobal -- Do we write the APEs in the local or global
 *coordinates? #    apeASCIISaveFile -- The name of the save-file. #
 *readApeFromASCII -- Do we read in APEs from a text file? # readLocalNotGlobal
 *-- Do we read APEs in the local or the global frame? #    readFullLocalMatrix
 *-- Do we read the full local matrix or just the diagonal elements? # -- Always
 *write full matrix # Full matrix format: DetID dxx dxy dyy dxz dyz dzz #
 *Diagonal element format: DetID sqrt(dxx) sqrt(dyy) sqrt(dzz) # setComposites
 *-- Do we set the APEs for composite detectors or just ignore them? #
 *apeASCIIReadFile -- Input file name. # Also note: #
 *process.AlignmentProducer.saveApeToDB -- to save as an sqlite file # and
 *associated entries in _cfg.py
 */
...
public:
  ...
  void initialize(const edm::EventSetup &setup, AlignableTracker *tracker,
...
ApeSettingAlgorithm::ApeSettingAlgorithm(const edm::ParameterSet &cfg)
  ...
  saveLocalNotGlobal_ =
      theConfig.getUntrackedParameter<bool>("saveLocalNotGlobal");
...
void ApeSettingAlgorithm::initialize(const edm::EventSetup &setup,
  ...
  if (readApeFromAscii_) {
    std::ifstream apeReadFile(
        theConfig.getParameter<edm::FileInPath>("apeASCIIReadFile")
            .fullPath()
            .c_str()); // requires <fstream>
    if (!apeReadFile.good()) {
    ...
    while (!apeReadFile.eof()) {
      ...
      if (!readLocalNotGlobal_ || readFullLocalMatrix_) {
        apeReadFile >> apeId >> x11 >> x21 >> x22 >> x31 >> x32 >> x33 >>
            ignore >> ignore >> ignore >> ignore >> ignore >> ignore >>
            ignore >> ignore >> ignore >> ignore >> ignore >> ignore >>
            ignore >> ignore >> ignore >> std::ws;
      } else {
      ...
      if (apeId != 0) // read appears valid?
      {
        if (apeList.find(apeId) == apeList.end()) // Not previously done
        {
          ...
          AlignableDetOrUnitPtr alidet(
              theAlignableNavigator->alignableFromDetId(id)); // NULL if none
          if (alidet) {
            if ((alidet->components().empty()) ||
                setComposites_) // the problem with glued dets...
            {
              ...
              if (readLocalNotGlobal_) {
                ...
                if (readFullLocalMatrix_)
                  globErr =
                      GlobalErrorExtended(x11, x21, x31, 0, 0, 0, x22, x32, 0,
                                          0, 0, x33, 0, 0, 0, 0, 0, 0, 0, 0, 0);
                else {
                  globErr = GlobalErrorExtended(
                      x11 * x11, 0, 0, 0, 0, 0, x22 * x22, 0, 0, 0, 0,
                      x33 * x33, 0, 0, 0, 0, 0, 0, 0, 0, 0);
                }
              }
              alidet->setAlignmentPositionError(
                  globErr, false);   // do not propagate down!
...
void ApeSettingAlgorithm::terminate(const edm::EventSetup &iSetup) {
  if (saveApeToAscii_) {
    ...
    std::ofstream apeSaveFile(
        theConfig.getUntrackedParameter<std::string>("apeASCIISaveFile")
            .c_str()); // requires <fstream>
    for (int i = 0; i < theSize; ++i) {
      int id = aliErr->m_alignError[i].rawId();
      AlignableDetOrUnitPtr alidet(
          theAlignableNavigator->alignableFromDetId(DetId(id))); // NULL if none
...
DEFINE_EDM_PLUGIN(AlignmentAlgorithmPluginFactory, ApeSettingAlgorithm,
                  "ApeSettingAlgorithm");
