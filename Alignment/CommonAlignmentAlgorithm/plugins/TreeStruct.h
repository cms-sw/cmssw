...
struct TreeStruct {
  ...
  TreeStruct(float del, float err, int ind)
      : delta(del), error(err), paramIndex(ind) {}
  ...
  /// List of leaves to pass as 3rd argument to TTree::Branch(...) if 2nd
  /// argument is a pointer to TreeStruct - keep in synch with data members
  /// above!
