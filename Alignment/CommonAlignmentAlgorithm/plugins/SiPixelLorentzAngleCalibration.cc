...
public:
  ...
  /// Return non-zero derivatives for x- and y-measurements with their indices
  /// by reference. Return value is their number.
  ...
  void beginOfJob(AlignableTracker *tracker, AlignableMuon *muon,
...
private:
  ...
  /// - or created from files of passed by configuration (i.e. from parallel
  /// processing)
  const SiPixelLorentzAngle *
  getLorentzAnglesInput(const align::RunNumber & = 0);
  ...
  SiPixelLorentzAngle createFromTree(const char *fileName,
                                     const char *treeName) const;
...
SiPixelLorentzAngleCalibration::SiPixelLorentzAngleCalibration(
    ...
    : IntegratedCalibrationBase(cfg),
      ...
      mergeFileNames_(
          cfg.getParameter<std::vector<std::string>>("mergeTreeFiles")),
      lorentzAngleLabel_(cfg.getParameter<std::string>("lorentzAngleLabel")),
      moduleGroupSelCfg_(
          cfg.getParameter<edm::ParameterSet>("LorentzAngleModuleGroups")) {}
...
void SiPixelLorentzAngleCalibration::beginRun(const edm::Run &run,
  ...
  if (cachedLorentzAngleInputs_.find(firstRun) ==
      cachedLorentzAngleInputs_.end()) {
    cachedLorentzAngleInputs_.emplace(firstRun,
                                      SiPixelLorentzAngle(*lorentzAngleHandle));
  } else {
    if (lorentzAngleRcd.validityInterval().first().eventID().run() > firstRun &&
        lorentzAngleHandle
                ->getLorentzAngles() // only bad if non-identical values
            != cachedLorentzAngleInputs_[firstRun]
                   .getLorentzAngles()) { // (comparing maps)
      // Maps are containers sorted by key, but comparison problems may arise
      // from 'floating point comparison' problems (FIXME?)
      throw cms::Exception("BadInput")
          << "Trying to cache SiPixelLorentzAngle payload for a run ("
          << runNumber << ") in an IOV (" << firstRun
...
//======================================================================
unsigned int SiPixelLorentzAngleCalibration::derivatives(
    std::vector<ValuesIndexPair> &outDerivInds,
    const TransientTrackingRecHit &hit, const TrajectoryStateOnSurface &tsos,
    const edm::EventSetup &setup, const EventInfo &eventInfo) const {
  outDerivInds.clear();

  if (hit.det()) { // otherwise 'constraint hit' or whatever

    const int index = moduleGroupSelector_->getParameterIndexFromDetId(
        hit.det()->geographicalId(), eventInfo.eventId().run());
    if (index >= 0) { // otherwise not treated
      edm::ESHandle<MagneticField> magneticField;
      setup.get<IdealMagneticFieldRecord>().get(magneticField);
      const GlobalVector bField(
          magneticField->inTesla(hit.det()->surface().position()));
      const LocalVector bFieldLocal(hit.det()->surface().toLocal(bField));
      const double dZ =
          hit.det()->surface().bounds().thickness(); // it is a float only...
      ...
      const double xDerivative =
          bFieldLocal.y() * dZ * -0.5; // parameter is mobility!
      const double yDerivative =
          bFieldLocal.x() * dZ * 0.5; // parameter is mobility!
      if (xDerivative ||
          yDerivative) { // If field is zero, this is zero: do not return it
...
//======================================================================
double
SiPixelLorentzAngleCalibration::getParameterError(unsigned int index) const {
  return (index >= paramUncertainties_.size() ? 0.
                                              : paramUncertainties_[index]);
}

//======================================================================
void SiPixelLorentzAngleCalibration::beginOfJob(
  ...
  moduleGroupSelector_ = std::make_unique<TkModuleGroupSelector>(
      aliTracker, moduleGroupSelCfg_, sdets);
...
void SiPixelLorentzAngleCalibration::endOfJob() {
  ...
  for (unsigned int iPar = 0; iPar < parameters_.size(); ++iPar) {
    out << iPar << ": " << parameters_[iPar] << " +- "
        << paramUncertainties_[iPar] << "\n";
  }
  edm::LogInfo("Alignment")
      << "@SUB=SiPixelLorentzAngleCalibration::endOfJob" << out.str();

  std::map<unsigned int, TreeStruct>
      treeInfo; // map of TreeStruct for each detId
  ...
  for (unsigned int iIOV = 0; iIOV < moduleGroupSelector_->numIovs(); ++iIOV) {
    ...
    this->writeTree(inputs.back(), treeInfo,
                    (treeName + "input_" + std::to_string(firstRunOfIOV))
                        .c_str()); // empty treeInfo for input...
  ...
  const unsigned int nonZeroParamsOrErrors = // Any determined value?
      count_if(parameters_.begin(), parameters_.end(),
               [](auto c) { return c != 0.; }) +
  ...
  for (unsigned int iIOV = 0; iIOV < moduleGroupSelector_->numIovs(); ++iIOV) {
    auto firstRunOfIOV =
        static_cast<cond::Time_t>(moduleGroupSelector_->firstRunOfIOV(iIOV));
    ...
    for (const auto &iterIdValue : inputs[iIOV]->getLorentzAngles()) {
      ...
      // put result in output, i.e. sum of input and determined parameter, but
      // the nasty putLorentzAngle(..) takes float by reference - not even const
      // reference:
      auto value = iterIdValue.second + static_cast<float>(param);
      output.putLorentzAngle(detId, value);
      const int paramIndex = moduleGroupSelector_->getParameterIndexFromDetId(
          detId, firstRunOfIOV);
      treeInfo[detId] =
          TreeStruct(param, this->getParameterError(paramIndex), paramIndex);
    }

    if (saveToDB_ || nonZeroParamsOrErrors != 0) { // Skip writing mille jobs...
      this->writeTree(&output, treeInfo,
                      (treeName + Form("result_%lld", firstRunOfIOV)).c_str());
    }
...
SiPixelLorentzAngleCalibration::getLorentzAnglesInput(
    const align::RunNumber &run) {
  const auto &resolvedRun = run > 0 ? run : currentIOV_;
  // For parallel processing in Millepede II, create SiPixelLorentzAngle
  // from info stored in files of parallel jobs and check that they are
  // identical. If this job has run on data, still check that LA is identical to
  // the ones from mergeFileNames_.
  const std::string treeName{this->name() + "_input_" +
                             std::to_string(resolvedRun)};
  for (const auto &iFile : mergeFileNames_) {
    auto la = this->createFromTree(iFile.c_str(), treeName.c_str());
    // siPixelLorentzAngleInput_ could be non-null from previous file of this
    // loop or from checkLorentzAngleInput(..) when running on data in this job
    // as well
    if (!siPixelLorentzAngleInput_ ||
        siPixelLorentzAngleInput_->getLorentzAngles().empty()) {
      ...
      if (!la.getLorentzAngles()
               .empty() && // single job might not have got events
          la.getLorentzAngles() !=
              siPixelLorentzAngleInput_->getLorentzAngles()) {
        // Throw exception instead of error?
        edm::LogError("NoInput")
            << "@SUB=SiPixelLorentzAngleCalibration::getLorentzAnglesInput"
            << "Different input values from tree " << treeName << " in file "
            << iFile << ".";
      }
    }
  }
  ...
  if (!siPixelLorentzAngleInput_) { // no files nor ran on events
    // [] operator default-constructs an empty SiPixelLorentzAngle object in
    // place:
    siPixelLorentzAngleInput_ = &(cachedLorentzAngleInputs_[resolvedRun]);
    currentIOV_ = resolvedRun;
    edm::LogError("NoInput")
...
double SiPixelLorentzAngleCalibration::getParameterForDetId(
    unsigned int detId, edm::RunNumber_t run) const {
  const int index =
      moduleGroupSelector_->getParameterIndexFromDetId(detId, run);
  return (index < 0 ? 0. : parameters_[index]);
}

//======================================================================
void SiPixelLorentzAngleCalibration::writeTree(
  ...
  for (auto iterIdValue = lorentzAngle->getLorentzAngles().begin();
    ...
    if (treeStructIter != treeInfo.end()) {
      ...
      const int ind =
          moduleGroupSelector_->getParameterIndexFromDetId(id, run1of1stIov);
...
//======================================================================
SiPixelLorentzAngle
SiPixelLorentzAngleCalibration::createFromTree(const char *fileName,
                                               const char *treeName) const {
