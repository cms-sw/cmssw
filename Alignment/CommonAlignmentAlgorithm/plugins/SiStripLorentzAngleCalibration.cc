...
private:
  ...
  double effectiveThickness(const GeomDet *det, int16_t mode,
  ...
  SiStripLorentzAngle createFromTree(const char *fileName,
                                     const char *treeName) const;
...
void SiStripLorentzAngleCalibration::beginRun(const edm::Run &run,
  ...
  if (cachedLorentzAngleInputs_.find(firstRun) ==
    ...
    cachedLorentzAngleInputs_.emplace(firstRun,
                                      SiStripLorentzAngle(*lorentzAngleHandle));
  } else {
...
//======================================================================
unsigned int SiStripLorentzAngleCalibration::derivatives(
  ...
  if (mode == readoutMode_) {
    if (hit.det()) { // otherwise 'constraint hit' or whatever
      ...
      if (index >= 0) { // otherwise not treated
        ...
        // '-' since we have derivative of the residual r = hit - trk and mu is
        // part of trk model
        ...
        //      track model...
        // GM: sign convention is the same as for pixel LA, i.e. adopt it here,
        // too
...
//======================================================================
double
SiStripLorentzAngleCalibration::getParameterError(unsigned int index) const {
  return (index >= paramUncertainties_.size() ? 0.
                                              : paramUncertainties_[index]);
}

//======================================================================
void SiStripLorentzAngleCalibration::beginOfJob(
  ...
  edm::LogInfo("Alignment")
      ...
      << "Created with name " << this->name() << " for readout mode '"
      << readoutModeName_ << "',\n"
...
//======================================================================
double SiStripLorentzAngleCalibration::effectiveThickness(
    const GeomDet *det, int16_t mode, const edm::EventSetup &setup) const {
  if (!det)
    return 0.;
  ...
  // FIXME: which one? DepRcd->get(handle) or Rcd->get(readoutModeName_,
  // handle)??
  // setup.get<SiStripBackPlaneCorrectionDepRcd>().get(backPlaneHandle); // get
  // correct mode
  setup.get<SiStripBackPlaneCorrectionRcd>().get(readoutModeName_,
                                                 backPlaneHandle);
  const double bpCor =
      backPlaneHandle->getBackPlaneCorrection(id); // it's a float...
  //  std::cout << "bpCor " << bpCor << " in subdet " << id.subdetId() <<
  //  std::endl;
  dZ *= (1. - bpCor);
...
SiStripLorentzAngleCalibration::getLorentzAnglesInput(
  ...
  // identical. If this job has run on events, still check that LA is identical
  // to the ones from mergeFileNames_.
  ...
  for (const auto &iFile : mergeFileNames_) {
    ...
    // siStripLorentzAngleInput_ could be non-null from previous file of this
    // loop or from checkLorentzAngleInput(..) when running on data in this job
    // as well
    if (!siStripLorentzAngleInput_ ||
        siStripLorentzAngleInput_->getLorentzAngles().empty()) {
      ...
      if (!la.getLorentzAngles()
          ...
          la.getLorentzAngles() !=
              siStripLorentzAngleInput_->getLorentzAngles()) {
  ...
  if (!siStripLorentzAngleInput_) { // no files nor ran on events
    // [] operator default-constructs an empty SiStripLorentzAngle object in
    // place:
    siStripLorentzAngleInput_ = &(cachedLorentzAngleInputs_[resolvedRun]);
    currentIOV_ = resolvedRun;
    edm::LogError("NoInput")
...
//======================================================================
double SiStripLorentzAngleCalibration::getParameterForDetId(
...
//======================================================================
void SiStripLorentzAngleCalibration::writeTree(
...
//======================================================================
SiStripLorentzAngle
SiStripLorentzAngleCalibration::createFromTree(const char *fileName,
                                               const char *treeName) const {
