 ...
 *  \author Gero Flucke (selection by strings taken from
 * AlignableParameterBuilder)
 *
...
public:
  ...
  /// vector of selection 'strings' for alignables, parallel to
  /// selectedAlignables()
  ...
  /// Add several selections defined by the PSet which must contain a vstring
  /// like e.g. vstring alignParams = {
  /// "PixelHalfBarrelLadders,111000,pixelSelection",
  ///                         "BarrelDSRods,111ff0",
  ///                         "BarrelSSRods,101ff0"}
  /// The e.g. '111ff0' is decoded into vector<char> and stored.
  /// Returns number of added selections or -1 if problems (then also an error
  /// is logged) If a string contains a third, comma separated part (e.g.
  /// ',pixelSelection'), a further PSet of that name is expected to select
  /// eta/phi/r/x/y/z-ranges.
  unsigned int addSelections(const edm::ParameterSet &pSet);
  /// set geometrical restrictions to be applied on all following selections
  /// (slices defined by vdouble 'etaRanges', 'phiRanges', 'xRanges', 'yRanges',
  /// 'zRanges' and 'rRanges', empty array means no restriction)
  void setGeometryCuts(const edm::ParameterSet &pSet);
  /// add Alignables corresponding to predefined name, taking into account
  /// geometrical restrictions as defined in setSpecials, returns number of
  /// added alignables
  unsigned int addSelection(const std::string &name,
                            const std::vector<char> &paramSel);
  /// as addSelection with one argument, but overwriting geometrical
  /// restrictions
  unsigned int addSelection(const std::string &name,
                            const std::vector<char> &paramSel,
  ...
  /// true if ranges.size() is even and ranges[i] <= value < ranges[i+1] for any
  /// even i ( => false if ranges.empty() == true), if(isPhi==true) takes into
  /// account phi periodicity for the integer specialized method, true is
  /// returned for ranges[i] <= value <= ranges[i+1] and isPhi is ignored
  template <typename T>
  bool insideRanges(T value, const std::vector<T> &ranges,
  ...
  std::vector<std::string> decompose(const std::string &s,
                                     std::string::value_type delimiter) const;
  /// Converting std::string into std::vector<char>
  std::vector<char> convertParamSel(const std::string &selString) const;

protected:
  /// adding alignables which fulfil geometrical restrictions and special
  /// switches
...
private:
  ...
  /// geometrical restrictions in eta, phi, r, x, y, z to be applied for next
  /// addSelection
  ...
  /// DetId restrictions in eta, phi, r, x, y, z to be applied for next
  /// addSelection
  ...
  /// Setting the special switches and returning input string, but after
  /// removing the 'special indicators' from it. Known specials are: "SS"
  /// anywhere in name: in TIB/TOB restrict to single sided Dets/Rods/Layers
  /// "DS" anywhere in name: in TIB/TOB restrict to double sided
  /// Dets/Rods/Layers "Layers14" at end of name: in tracker restrict to
  /// layers/disks 1 to 4, similar for other digits "UnitStereo" and "UnitRphi"
  /// anywhere in name:
  ///      for a DetUnit in strip select only if stereo or rphi module (keep
  ///      'Unit' in name!)
