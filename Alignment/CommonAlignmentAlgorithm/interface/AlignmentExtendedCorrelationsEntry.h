...
/// Data container for a correlations matrix (represented by a vector of
/// floats), with basic access functions. NOTE: This class is designed
/// specifically for the use within AlignmentExtendedCorrelationsStore, and is
...
public:
  ...
  explicit AlignmentExtendedCorrelationsEntry(short unsigned int nRows,
                                              short unsigned int nCols);

  /// Constructor. Initializes all elements of the correlations matrix to the
  /// given value.
  explicit AlignmentExtendedCorrelationsEntry(short unsigned int nRows,
                                              short unsigned int nCols,
                                              const float init);
  ...
  /// Read or write an element of the correlations matrix. NOTE: Indexing starts
  /// from [0,0].
  inline float &operator()(short unsigned int iRow, short unsigned int jCol) {
    return theData[iRow * theNCols + jCol];
  }

  /// Read or write an element of the correlations matrix. NOTE: Indexing starts
  /// from [0,0].
  inline const float operator()(short unsigned int iRow,
                                short unsigned int jCol) const {
    return theData[iRow * theNCols + jCol];
  }
