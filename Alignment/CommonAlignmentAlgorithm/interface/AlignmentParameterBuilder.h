...
public:
  ...
  /// Constructor adding selections by passing the ParameterSet named
  /// 'AlignmentParameterSelector' (expected in pSet) to addSelections(..)
  AlignmentParameterBuilder(AlignableTracker *alignableTracker,
                            AlignableExtras *alignableExtras,
  ...
  /// Add selections of Alignables, using
  /// AlignmenParameterSelector::addSelections. For each Alignable,
  /// AlignmentParameters of type parType will be attached using the selection
  /// of active parameters done in AlignmenParameterSelector, e.g. for RigidBody
  /// a selection string '11100' selects the degrees of freedom in (x,y,z), but
  /// not (alpha,beta,gamma). Returns number of added selections
  unsigned int
  addSelections(const edm::ParameterSet &pset,
                AlignmentParametersFactory::ParametersType parType);

  /// Add arbitrary selection of Alignables, return number of higher level
  /// Alignables
  unsigned int add(const align::Alignables &alignables,
                   AlignmentParametersFactory::ParametersType parType,
                   const std::vector<bool> &sel);
  /// Add a single Alignable, true if it is higher level, false if it is an
  /// AlignableDet
  bool add(Alignable *alignable,
           AlignmentParametersFactory::ParametersType parType,
           const std::vector<bool> &sel);
...
private:
  ...
  /// from ParameterSelector) and then convert the selection to bool (for
  /// AlignmentParameters). True if (after removal of spaces) anything else than
  /// 0 and 1 is found in vector<char>.
  bool decodeParamSel(std::vector<char> &paramSelChar,
                      std::vector<bool> &result) const;
  /// add SelectionUserVariables corresponding to fullSel
  bool addFullParamSel(AlignmentParameters *aliPar,
