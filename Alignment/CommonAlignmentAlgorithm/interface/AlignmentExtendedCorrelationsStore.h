...
/// This class manages the storage and retrieval of correlations between
/// Alignables for the AlignmentParameterStore. This implementation does not
/// stores the entries of the "big covariance matrix" itself, but the
/// statistical correlations, i.e. R_ij=C_ij/sqrt(C_ii*C_jj) rather than C_ij.
///
/// If a correlation exceeds a certain value (especially corrupted correlations
/// with an absolute value bigger than 1) it is downweighted.
...
public:
  ...
  typedef std::map<Alignable *, ExtendedCorrelationsEntry>
      ExtendedCorrelationsTable;
  typedef std::map<Alignable *, ExtendedCorrelationsTable *>
  ...
  void correlations(Alignable *ap1, Alignable *ap2, AlgebraicSymMatrix &cov,
  ...
  void setCorrelations(Alignable *ap1, Alignable *ap2,
                       const AlgebraicSymMatrix &cov, int row,
                       int col) override;

  /// Set correlations without checking whether the maximum
  /// number of updates has already been reached.
  void setCorrelations(Alignable *ap1, Alignable *ap2,
                       AlgebraicMatrix &mat) override;

  /// Get correlations.
  virtual void getCorrelations(Alignable *ap1, Alignable *ap2,
                               AlgebraicMatrix &mat) const;
...
private:
  ...
  void fillCovariance(Alignable *ap1, Alignable *ap2,
                      const ExtendedCorrelationsEntry &entry,
                      AlgebraicSymMatrix &cov, int row, int col) const;

  void fillCovarianceT(Alignable *ap1, Alignable *ap2,
                       const ExtendedCorrelationsEntry &entry,
                       AlgebraicSymMatrix &cov, int row, int col) const;

  void readFromCovariance(Alignable *ap1, Alignable *ap2,
                          ExtendedCorrelationsEntry &entry,
                          const AlgebraicSymMatrix &cov, int row, int col);

  void readFromCovarianceT(Alignable *ap1, Alignable *ap2,
                           ExtendedCorrelationsEntry &entry,
                           const AlgebraicSymMatrix &cov, int row, int col);

  void resizeCorruptCorrelations(ExtendedCorrelationsEntry &entry,
                                 double maxCorr);
