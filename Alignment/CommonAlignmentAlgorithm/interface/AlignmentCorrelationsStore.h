...
/// Alignables for the AlignmentParameterStore. This basic implementation simply
/// stores the off diagonal entries of the "big covariance matrix". ATTENTION:
/// Definition of data structure "Correlations" differs from definition in
/// AlignmentParameterStore, but is used only internally.
...
private:
  ...
  void fillCovariance(Alignable *ap1, Alignable *ap2,
                      const AlgebraicMatrix &entry, AlgebraicSymMatrix &cov,
  ...
  void readFromCovariance(Alignable *ap1, Alignable *ap2,
                          AlgebraicMatrix &entry, const AlgebraicSymMatrix &cov,
                          int row, int col);
