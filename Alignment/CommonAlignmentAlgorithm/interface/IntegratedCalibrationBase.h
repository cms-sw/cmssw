...
public:
  ...
  typedef std::pair<Values, unsigned int>
      ValuesIndexPair; /// Values and their parameter index
  ...
  /// Return all derivatives for x- (Values.first) and y-measurement
  /// (Values.second), default implementation uses other derivatives(..) method,
  ...
  /// should return false if out-of-bounds or errors not treated, true
  /// otherwise.
  ...
  /// Return current value of parameter identified by index.
  /// Should return 0. if index out-of-bounds or if errors not
  /// treated/undetermined.
  virtual double getParameterError(unsigned int index) const = 0;

  /// Call at beginning of job:
  /// default implementation is dummy, to be overwritten in derived class if
  /// useful.
  virtual void beginOfJob(AlignableTracker *tracker, AlignableMuon *muon,
  ...
  /* virtual void endRun(const EndRunInfo &runInfo, const edm::EventSetup
   * &setup) {}; */
  ...
  const std::string &name() const {
    return name_;
  } // non-virtual since refering to private member
