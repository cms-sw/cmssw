...
public:
  ...
  typedef std::map<std::pair<Alignable *, Alignable *>, AlgebraicMatrix>
      Correlations;
  typedef std::vector<unsigned int> DetIds;

  /// constructor
  AlignmentParameterStore(const align::Alignables &alis,
  ...
  /// select parameters
  /// (for backward compatibility, use with vector<AlignableDetOrUnitPtr> as
  /// argument instead)
  CompositeAlignmentParameters
  selectParameters(const std::vector<AlignableDet *> &alignabledets) const;
  /// select parameters
  CompositeAlignmentParameters selectParameters(
      const std::vector<AlignableDetOrUnitPtr> &alignabledets) const;
  /// select parameters
  CompositeAlignmentParameters
  selectParameters(const align::Alignables &) const;
  ...
  /// Obsolete: Use AlignableNavigator::alignableDetFromGeomDet and
  /// alignableFromAlignableDet
  /*   Alignable* alignableFromGeomDet( const GeomDet* geomDet ) const; */

  /// get Alignable corresponding to given AlignableDet (non-const ref. argument
  /// since might be returned)
  Alignable *
  alignableFromAlignableDet(const AlignableDetOrUnitPtr &alignableDet) const;

  /// Obsolete: Use AlignableNavigator::alignableDetFromDetId and
  /// alignableFromAlignableDet
  ...
  /// restore for the given run the previously cached position, rotation and
  /// other parameters
  ...
  void attachAlignmentParameters(const align::Alignables &alivec,
  ...
  void attachCorrelations(const align::Alignables &alivec,
                          const Correlations &cormap, bool overwrite,
                          int &ierr);

  /// Attach correlations to alignables
  void attachCorrelations(const Correlations &cormap, bool overwrite,
                          int &ierr);
  ...
  void setAlignmentPositionError(const align::Alignables &alivec,
                                 double valshift, double valrot);

  /// Obtain type and layer from Alignable
  std::pair<int, int> typeAndLayer(const Alignable *ali,
  ...
  /// (cf. Alignable::firstParamComponents), paramIdsVecOut and factorsVecOut
  /// will be filled (in parallel) with constraints on the selected alignment
  /// parameters of aliMaster to get rid of the additionally introduced degrees
  /// of freedom: The 'vector product' of the parameters identified by
  /// ParameterId in std::vector<ParameterId> and the factors in
  /// std::vector<double> has to vanish (i.e. == 0.), |factor| < epsilon will be
  /// treated as 0. If all == false, skip constraint on aliMaster's degree of
  /// freedom 'i' if 'i'th alignment parameter of aliMaster is not selected,
  /// i.e. constrain only for otherwise doubled d.o.f. If all == true, produce
  /// one constraint for each of the aliMaster's parameters irrespective of
  /// whether they are selecte dor not. paramIdsVecOut and factorsVecOut contain
  /// exactly one std::vector per selected alignment parameter of aliMaster, but
  /// in principle these can be empty... Note that not all combinations of
  /// AlignmentParameters classes ar supported. If not there will be an
  /// exception (and false returned...)
  bool
  hierarchyConstraints(const Alignable *aliMaster,
