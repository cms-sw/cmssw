...
AlignmentParameterBuilder::AlignmentParameterBuilder(
    AlignableTracker *alignableTracker, AlignableExtras *alignableExtras)
    : theAlignables(), theAlignableTracker(alignableTracker),
      theAlignableMuon(nullptr), theAlignableExtras(alignableExtras) {}
...
AlignmentParameterBuilder::AlignmentParameterBuilder(
    ...
    : theAlignables(), theAlignableTracker(alignableTracker),
      theAlignableMuon(alignableMuon), theAlignableExtras(alignableExtras) {}
...
AlignmentParameterBuilder::AlignmentParameterBuilder(
    AlignableTracker *alignableTracker, AlignableMuon *alignableMuon,
    AlignableExtras *alignableExtras, const edm::ParameterSet &pSet)
    : theAlignables(), theAlignableTracker(alignableTracker),
      theAlignableMuon(alignableMuon), theAlignableExtras(alignableExtras) {
  this->addAllSelections(pSet);
}
...
//__________________________________________________________________________________________________
void AlignmentParameterBuilder::addAllSelections(
    const edm::ParameterSet &pSet) {
  AlignmentParameterSelector selector(nullptr);
  std::vector<std::string> selsTypes(
      pSet.getParameter<std::vector<std::string>>("parameterTypes"));
  ...
  for (unsigned int i = 0; i < selsTypes.size(); ++i) {
    ...
    this->addSelections(
        pSet.getParameter<edm::ParameterSet>(selSetType[0]),
...
//__________________________________________________________________________________________________
unsigned int
AlignmentParameterBuilder::addSelections(const edm::ParameterSet &pSet,
  ...
  const std::vector<std::vector<char>> &paramSels =
  ...
  while (iAli != alignables.end() && iParamSel != paramSels.end()) {
    ...
    std::vector<char> parSel(
        *iParamSel); // copy, since decodeParamSel may manipulate
...
bool AlignmentParameterBuilder::add(Alignable *alignable,
  ...
  AlignmentParameters *paras =
      AlignmentParametersFactory::createParameters(alignable, parType, sel);
...
//__________________________________________________________________________________________________
bool AlignmentParameterBuilder::decodeParamSel(
    std::vector<char> &paramSelChar, std::vector<bool> &result) const {
  result.clear();
  // remove all spaces from paramSelChar - useful to group the parameters if
  // they are many
...
//__________________________________________________________________________________________________
bool AlignmentParameterBuilder::addFullParamSel(
    AlignmentParameters *aliParams, const std::vector<char> &fullSel) const {
  if (!aliParams)
    return false;
