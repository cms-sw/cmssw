...
int AlignmentIORootBase::openRoot(const char *filename, int iteration,
                                  bool write) {
  bWrite = write;
  int iter;
  ...
  if (bWrite) { // writing
    ...
    if (iterfile == -1) {
      ...
      edm::LogError("AlignmentIORootBase")
          << "Tree " << treename << " does not exist in file " << filename;
      return -1;
    } else {
      if (iteration == -1) {
        iter = iterfile;
        ...
        if (iteration > iterfile) {
          edm::LogError("AlignmentIORootBase")
              << "Iteration " << iteration << " does not exist for tree "
              << treename;
          return -1;
        }
        iter = iteration;
