...
AlignmentParameterSelector::AlignmentParameterSelector(
    ...
    : theTracker(aliTracker), theMuon(aliMuon), theExtras(aliExtras),
      theSelectedAlignables(), theRangesEta(), theRangesPhi(), theRangesR(),
      theRangesX(), theRangesY(), theRangesZ() {
  this->setSpecials(""); // init theOnlyDS, theOnlySS, theSelLayers,
                         // theMinLayer, theMaxLayer, theRphiOrStereoDetUnit
}
...
unsigned int
AlignmentParameterSelector::addSelections(const edm::ParameterSet &pSet) {
  ...
  for (unsigned int iSel = 0; iSel < selections.size(); ++iSel) {
    ...
    if (decompSel.size() < 2) {
      throw cms::Exception("BadConfig")
          << "@SUB=AlignmentParameterSelector::addSelections"
          << selections[iSel] << " from alignableParamSelector: "
          << " should have at least 2 ','-separated parts";
    } else if (decompSel.size() > 2) {
      const edm::ParameterSet geoSel(
          pSet.getParameter<edm::ParameterSet>(decompSel[2].c_str()));
...
//________________________________________________________________________________
void AlignmentParameterSelector::setGeometryCuts(
...
//________________________________________________________________________________
void AlignmentParameterSelector::setPXBDetIdCuts(
    const edm::ParameterSet &pSet) {
  // Allow non-specified arrays to be interpreted as empty (i.e. no selection),
  // but take care that nothing unknown is configured (to fetch typos!).
  ...
  for (std::vector<std::string>::const_iterator iParam = parameterNames.begin(),
    ...
    if (*iParam == "ladderRanges") {
      pSet.getParameter<std::vector<int>>(*iParam).swap(
          thePXBDetIdRanges.theLadderRanges);
    } else if (*iParam == "layerRanges") {
      pSet.getParameter<std::vector<int>>(*iParam).swap(
          thePXBDetIdRanges.theLayerRanges);
    } else if (*iParam == "moduleRanges") {
      pSet.getParameter<std::vector<int>>(*iParam).swap(
          thePXBDetIdRanges.theModuleRanges);
    } else {
      throw cms::Exception("BadConfig")
...
//________________________________________________________________________________
void AlignmentParameterSelector::setPXFDetIdCuts(
    const edm::ParameterSet &pSet) {
  // Allow non-specified arrays to be interpreted as empty (i.e. no selection),
  // but take care that nothing unknown is configured (to fetch typos!).
  ...
  for (std::vector<std::string>::const_iterator iParam = parameterNames.begin(),
    ...
    if (*iParam == "bladeRanges") {
      pSet.getParameter<std::vector<int>>(*iParam).swap(
          thePXFDetIdRanges.theBladeRanges);
    } else if (*iParam == "diskRanges") {
      pSet.getParameter<std::vector<int>>(*iParam).swap(
          thePXFDetIdRanges.theDiskRanges);
    } else if (*iParam == "moduleRanges") {
      pSet.getParameter<std::vector<int>>(*iParam).swap(
          thePXFDetIdRanges.theModuleRanges);
    } else if (*iParam == "panelRanges") {
      pSet.getParameter<std::vector<int>>(*iParam).swap(
          thePXFDetIdRanges.thePanelRanges);
    } else if (*iParam == "sideRanges") {
      pSet.getParameter<std::vector<int>>(*iParam).swap(
          thePXFDetIdRanges.theSideRanges);
    } else {
      throw cms::Exception("BadConfig")
...
//________________________________________________________________________________
void AlignmentParameterSelector::setTIBDetIdCuts(
    const edm::ParameterSet &pSet) {
  // Allow non-specified arrays to be interpreted as empty (i.e. no selection),
  // but take care that nothing unknown is configured (to fetch typos!).
  ...
  for (std::vector<std::string>::const_iterator iParam = parameterNames.begin(),
    ...
    if (*iParam == "layerRanges") {
      pSet.getParameter<std::vector<int>>(*iParam).swap(
          theTIBDetIdRanges.theLayerRanges);
    } else if (*iParam == "moduleRanges") {
      pSet.getParameter<std::vector<int>>(*iParam).swap(
          theTIBDetIdRanges.theModuleRanges);
    } else if (*iParam == "stringRanges") {
      pSet.getParameter<std::vector<int>>(*iParam).swap(
          theTIBDetIdRanges.theStringRanges);
    } else if (*iParam == "sideRanges") {
      pSet.getParameter<std::vector<int>>(*iParam).swap(
          theTIBDetIdRanges.theSideRanges);
    } else {
      throw cms::Exception("BadConfig")
...
//________________________________________________________________________________
void AlignmentParameterSelector::setTIDDetIdCuts(
    const edm::ParameterSet &pSet) {
  // Allow non-specified arrays to be interpreted as empty (i.e. no selection),
  // but take care that nothing unknown is configured (to fetch typos!).
  ...
  for (std::vector<std::string>::const_iterator iParam = parameterNames.begin(),
    ...
    if (*iParam == "diskRanges") {
      pSet.getParameter<std::vector<int>>(*iParam).swap(
          theTIDDetIdRanges.theDiskRanges);
    } else if (*iParam == "moduleRanges") {
      pSet.getParameter<std::vector<int>>(*iParam).swap(
          theTIDDetIdRanges.theModuleRanges);
    } else if (*iParam == "ringRanges") {
      pSet.getParameter<std::vector<int>>(*iParam).swap(
          theTIDDetIdRanges.theRingRanges);
    } else if (*iParam == "sideRanges") {
      pSet.getParameter<std::vector<int>>(*iParam).swap(
          theTIDDetIdRanges.theSideRanges);
    } else {
      throw cms::Exception("BadConfig")
...
//________________________________________________________________________________
void AlignmentParameterSelector::setTOBDetIdCuts(
    const edm::ParameterSet &pSet) {
  // Allow non-specified arrays to be interpreted as empty (i.e. no selection),
  // but take care that nothing unknown is configured (to fetch typos!).
  ...
  for (std::vector<std::string>::const_iterator iParam = parameterNames.begin(),
    ...
    if (*iParam == "layerRanges") {
      pSet.getParameter<std::vector<int>>(*iParam).swap(
          theTOBDetIdRanges.theLayerRanges);
    } else if (*iParam == "moduleRanges") {
      pSet.getParameter<std::vector<int>>(*iParam).swap(
          theTOBDetIdRanges.theModuleRanges);
    } else if (*iParam == "sideRanges") {
      pSet.getParameter<std::vector<int>>(*iParam).swap(
          theTOBDetIdRanges.theSideRanges);
    } else if (*iParam == "rodRanges") {
      pSet.getParameter<std::vector<int>>(*iParam).swap(
          theTOBDetIdRanges.theRodRanges);
    } else {
      throw cms::Exception("BadConfig")
...
void AlignmentParameterSelector::setTECDetIdCuts(
  ...
  for (std::vector<std::string>::const_iterator iParam = parameterNames.begin(),
    ...
    if (*iParam == "wheelRanges") {
      pSet.getParameter<std::vector<int>>(*iParam).swap(
          theTECDetIdRanges.theWheelRanges);
    } else if (*iParam == "petalRanges") {
      pSet.getParameter<std::vector<int>>(*iParam).swap(
          theTECDetIdRanges.thePetalRanges);
    } else if (*iParam == "moduleRanges") {
      pSet.getParameter<std::vector<int>>(*iParam).swap(
          theTECDetIdRanges.theModuleRanges);
    } else if (*iParam == "ringRanges") {
      pSet.getParameter<std::vector<int>>(*iParam).swap(
          theTECDetIdRanges.theRingRanges);
    } else if (*iParam == "sideRanges") {
      pSet.getParameter<std::vector<int>>(*iParam).swap(
          theTECDetIdRanges.theSideRanges);
    } else {
      throw cms::Exception("BadConfig")
...
//________________________________________________________________________________
unsigned int
AlignmentParameterSelector::addSelection(const std::string &name,
...
//________________________________________________________________________________
unsigned int
AlignmentParameterSelector::addSelection(const std::string &nameInput,
                                         const std::vector<char> &paramSel) {
  const std::string name(
      this->setSpecials(nameInput)); // possibly changing name
  ...
  if (name.find("Tracker") == 0) { // string starts with "Tracker"
    ...
    const std::string substructName(name,
                                    7); // erase "Tracker" at the beginning
    numAli += this->add(theTracker->subStructures(substructName), paramSel);
  }
  ////////////////////////////////////
  // Old hardcoded, but geometry-independent tracker section (NOTE: no check on
  // theTracker != 0)
  ////////////////////////////////////
...
//________________________________________________________________________________
unsigned int
AlignmentParameterSelector::add(const align::Alignables &alignables,
                                const std::vector<char> &paramSel) {
  unsigned int numAli = 0;
...
bool AlignmentParameterSelector::layerDeselected(const Alignable *ali) const {
  if (theOnlySS || theOnlyDS || theSelLayers) {
    ...
    std::pair<int, int> typeLayer = idProvider.typeAndLayerFromDetId(
        ali->id(), alignableTracker()->trackerTopology());
    ...
    if (theOnlySS // only single sided
        && (std::abs(type) == SiStripDetId::TIB ||
            std::abs(type) == SiStripDetId::TOB) &&
        layer <= 2) {
      return true;
    }
...
bool AlignmentParameterSelector::detUnitDeselected(const Alignable *ali) const {
  ...
  if (theRphiOrStereoDetUnit != Both &&
      ali->alignableObjectId() == align::AlignableDetUnit) {
    const SiStripDetId detId(
        ali->geomDetId()); // do not know yet whether right type...
    if (detId.det() == DetId::Tracker &&
      ...
      // We have a DetUnit in strip, so check for a selection of stereo/rphi
      // (DetUnits in 1D layers are rphi):
...
//_________________________________________________________________________
bool AlignmentParameterSelector::outsideGeometricalRanges(
    const Alignable *alignable) const {
  const align::PositionType &position(alignable->globalPosition());

  if (!theRangesEta.empty() &&
      !this->insideRanges<double>((position.eta()), theRangesEta))
    return true;
  if (!theRangesPhi.empty() &&
      !this->insideRanges<double>((position.phi()), theRangesPhi, true))
    return true;
  if (!theRangesR.empty() &&
      !this->insideRanges<double>((position.perp()), theRangesR))
    return true;
  if (!theRangesX.empty() &&
      !this->insideRanges<double>((position.x()), theRangesX))
    return true;
  if (!theRangesY.empty() &&
      !this->insideRanges<double>((position.y()), theRangesY))
    return true;
  if (!theRangesZ.empty() &&
      !this->insideRanges<double>((position.z()), theRangesZ))
    return true;
...
//_________________________________________________________________________
bool AlignmentParameterSelector::outsideDetIdRanges(
    const Alignable *alignable) const {
  ...
  if (alignableTracker()) {
    ...
    if (!theDetIds.empty() &&
        !this->isMemberOfVector((detId.rawId()), theDetIds))
      return true;
    if (!theDetIdRanges.empty() &&
        !this->insideRanges<int>((detId.rawId()), theDetIdRanges))
      return true;
    if (!theExcludedDetIds.empty() &&
        this->isMemberOfVector((detId.rawId()), theExcludedDetIds))
      return true;
    if (!theExcludedDetIdRanges.empty() &&
        this->insideRanges<int>((detId.rawId()), theExcludedDetIdRanges))
      return true;
    ...
    if (detId.det() == DetId::Tracker) {
      ...
      if (subdetId == static_cast<int>(PixelSubdetector::PixelBarrel)) {
        if (!thePXBDetIdRanges.theLadderRanges.empty() &&
            !this->insideRanges<int>(tTopo->pxbLadder(detId),
                                     thePXBDetIdRanges.theLadderRanges))
          return true;
        if (!thePXBDetIdRanges.theLayerRanges.empty() &&
            !this->insideRanges<int>(tTopo->pxbLayer(detId),
                                     thePXBDetIdRanges.theLayerRanges))
          return true;
        if (!thePXBDetIdRanges.theModuleRanges.empty() &&
            !this->insideRanges<int>(tTopo->pxbModule(detId),
                                     thePXBDetIdRanges.theModuleRanges))
          return true;
      }
      ...
      if (subdetId == static_cast<int>(PixelSubdetector::PixelEndcap)) {
        if (!thePXFDetIdRanges.theBladeRanges.empty() &&
            !this->insideRanges<int>(tTopo->pxfBlade(detId),
                                     thePXFDetIdRanges.theBladeRanges))
          return true;
        if (!thePXFDetIdRanges.theDiskRanges.empty() &&
            !this->insideRanges<int>(tTopo->pxfDisk(detId),
                                     thePXFDetIdRanges.theDiskRanges))
          return true;
        if (!thePXFDetIdRanges.theModuleRanges.empty() &&
            !this->insideRanges<int>(tTopo->pxfModule(detId),
                                     thePXFDetIdRanges.theModuleRanges))
          return true;
        if (!thePXFDetIdRanges.thePanelRanges.empty() &&
            !this->insideRanges<int>(tTopo->pxfPanel(detId),
                                     thePXFDetIdRanges.thePanelRanges))
          return true;
        if (!thePXFDetIdRanges.theSideRanges.empty() &&
            !this->insideRanges<int>(tTopo->pxfSide(detId),
                                     thePXFDetIdRanges.theSideRanges))
          return true;
      }
      ...
      if (subdetId == static_cast<int>(SiStripDetId::TIB)) {
        if (!theTIBDetIdRanges.theLayerRanges.empty() &&
            !this->insideRanges<int>(tTopo->tibLayer(detId),
                                     theTIBDetIdRanges.theLayerRanges))
          return true;
        if (!theTIBDetIdRanges.theModuleRanges.empty() &&
            !this->insideRanges<int>(tTopo->tibModule(detId),
                                     theTIBDetIdRanges.theModuleRanges))
          return true;
        if (!theTIBDetIdRanges.theSideRanges.empty() &&
            !this->insideRanges<int>(tTopo->tibSide(detId),
                                     theTIBDetIdRanges.theSideRanges))
          return true;
        if (!theTIBDetIdRanges.theStringRanges.empty() &&
            !this->insideRanges<int>(tTopo->tibString(detId),
                                     theTIBDetIdRanges.theStringRanges))
          return true;
      }
      ...
      if (subdetId == static_cast<int>(SiStripDetId::TID)) {
        if (!theTIDDetIdRanges.theDiskRanges.empty() &&
            !this->insideRanges<int>(tTopo->tidWheel(detId),
                                     theTIDDetIdRanges.theDiskRanges))
          return true;
        if (!theTIDDetIdRanges.theModuleRanges.empty() &&
            !this->insideRanges<int>(tTopo->tidModule(detId),
                                     theTIDDetIdRanges.theModuleRanges))
          return true;
        if (!theTIDDetIdRanges.theRingRanges.empty() &&
            !this->insideRanges<int>(tTopo->tidRing(detId),
                                     theTIDDetIdRanges.theRingRanges))
          return true;
        if (!theTIDDetIdRanges.theSideRanges.empty() &&
            !this->insideRanges<int>(tTopo->tidSide(detId),
                                     theTIDDetIdRanges.theSideRanges))
          return true;
      }
      ...
      if (subdetId == static_cast<int>(SiStripDetId::TOB)) {
        if (!theTOBDetIdRanges.theLayerRanges.empty() &&
            !this->insideRanges<int>(tTopo->tobLayer(detId),
                                     theTOBDetIdRanges.theLayerRanges))
          return true;
        if (!theTOBDetIdRanges.theModuleRanges.empty() &&
            !this->insideRanges<int>(tTopo->tobModule(detId),
                                     theTOBDetIdRanges.theModuleRanges))
          return true;
        if (!theTOBDetIdRanges.theRodRanges.empty() &&
            !this->insideRanges<int>(tTopo->tobRod(detId),
                                     theTOBDetIdRanges.theRodRanges))
          return true;
        if (!theTOBDetIdRanges.theSideRanges.empty() &&
            !this->insideRanges<int>(tTopo->tobSide(detId),
                                     theTOBDetIdRanges.theSideRanges))
          return true;
      }
      ...
      if (subdetId == static_cast<int>(SiStripDetId::TEC)) {
        if (!theTECDetIdRanges.theWheelRanges.empty() &&
            !this->insideRanges<int>(tTopo->tecWheel(detId),
                                     theTECDetIdRanges.theWheelRanges))
          return true;
        if (!theTECDetIdRanges.thePetalRanges.empty() &&
            !this->insideRanges<int>(tTopo->tecPetalNumber(detId),
                                     theTECDetIdRanges.thePetalRanges))
          return true;
        if (!theTECDetIdRanges.theModuleRanges.empty() &&
            !this->insideRanges<int>(tTopo->tecModule(detId),
                                     theTECDetIdRanges.theModuleRanges))
          return true;
        if (!theTECDetIdRanges.theRingRanges.empty() &&
            !this->insideRanges<int>(tTopo->tecRing(detId),
                                     theTECDetIdRanges.theRingRanges))
          return true;
        if (!theTECDetIdRanges.theSideRanges.empty() &&
            !this->insideRanges<int>(tTopo->tecSide(detId),
                                     theTECDetIdRanges.theSideRanges))
          return true;
      }
    }
...
bool AlignmentParameterSelector::insideRanges(T value,
  ...
  for (unsigned int i = 0; i < ranges.size(); i += 2) {
    if (isPhi) { // mapping into (-pi,+pi] and checking for range including sign
                 // flip area
      ...
      if (rangePhi2 < rangePhi1 &&
          (rangePhi1 <= valuePhi || valuePhi < rangePhi2)) { // 'sign flip'
        return true;
      }
...
bool AlignmentParameterSelector::insideRanges<int>(
    int value, const std::vector<int> &ranges, bool /*isPhi*/) const {
  if (ranges.size() % 2 != 0) {
    cms::Exception("BadConfig")
        << "@SUB=AlignmentParameterSelector::insideRanges"
        << " need even number of entries in ranges instead of "
...
bool AlignmentParameterSelector::isMemberOfVector(
    int value, const std::vector<int> &values) const {
...
AlignmentParameterSelector::decompose(const std::string &s,
...
//__________________________________________________________________________________________________
std::vector<char> AlignmentParameterSelector::convertParamSel(
    const std::string &selString) const {
  ...
  // But I prefer to be more general and allow other Alignables. It will throw
  // anyway if RigidBodyAlignmentParameters are build with wrong selection
  // length.
...
std::string AlignmentParameterSelector::setSpecials(const std::string &name) {
  // Use new string only, although direct erasing of found indicator causes
  // problems for 'DSS', but 'DSS' makes absolutely no sense!
  ...
  if (layers != std::string::npos &&
      size - layers - 2 == 6 // 2 digits, 6: length of 'Layers'
  ...
  if (newName != name) {
    LogDebug("Alignment") << "@SUB=AlignmentParameterSelector::setSpecials"
                          << name << " becomes " << newName
                          << ", makes theOnlySS " << theOnlySS << ", theOnlyDS "
                          << theOnlyDS << ", theSelLayers " << theSelLayers
                          << ", theMinLayer " << theMinLayer << ", theMaxLayer "
                          << theMaxLayer << ", theRphiOrStereoDetUnit "
...
//________________________________________________________________________________
unsigned int
AlignmentParameterSelector::addAllDets(const std::vector<char> &paramSel) {
  unsigned int numAli = 0;
  ...
  numAli +=
      this->add(theTracker->pixelHalfBarrelGeomDets(), paramSel); // PixelBarrel
  numAli +=
      this->add(theTracker->pixelEndcapGeomDets(), paramSel); // PixelEndcap
...
//________________________________________________________________________________
unsigned int
AlignmentParameterSelector::addAllRods(const std::vector<char> &paramSel) {
  unsigned int numAli = 0;

  numAli += this->add(theTracker->barrelRods(), paramSel); // TIB+TOB
  numAli +=
      this->add(theTracker->pixelHalfBarrelLadders(), paramSel);  // PixelBarrel
...
//________________________________________________________________________________
unsigned int
AlignmentParameterSelector::addAllLayers(const std::vector<char> &paramSel) {
  unsigned int numAli = 0;

  numAli += this->add(theTracker->barrelLayers(), paramSel); // TIB+TOB
  numAli +=
      this->add(theTracker->pixelHalfBarrelLayers(), paramSel);   // PixelBarrel
...
//________________________________________________________________________________
unsigned int AlignmentParameterSelector::addAllAlignables(
    const std::vector<char> &paramSel) {
  unsigned int numAli = 0;
