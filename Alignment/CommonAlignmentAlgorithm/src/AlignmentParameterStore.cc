...
AlignmentParameterStore::AlignmentParameterStore(
    const align::Alignables &alis, const edm::ParameterSet &config)
    : theAlignables(alis) {
  ...
  const std::string cfgStrTypeOfConstraints(
      config.getParameter<std::string>("TypeOfConstraints"));
  if (cfgStrTypeOfConstraints == "hierarchy") {
    ...
    edm::LogWarning("Alignment")
        ...
        << "  AlignmentProducer.ParameterStore.TypeOfConstraints = "
           "cms.string('hierarchy')\n\n\n";
  } else {
    edm::LogError("BadArgument") << "@SUB=AlignmentParameterStore"
                                 << "Unknown type of hierarchy constraints '"
                                 << cfgStrTypeOfConstraints << "'";
  }
}
...
CompositeAlignmentParameters AlignmentParameterStore::selectParameters(
  ...
  for (std::vector<AlignableDetOrUnitPtr>::const_iterator iad =
           alignabledets.begin();
  ...
  for (it1 = alignables.begin(); it1 != alignables.end(); ++it1) {
    ...
    for (it2 = alignables.begin(); it2 != it1; ++it2) {
      theCorrelationsStore->correlations(*it1, *it2, *selcov, ipos - 1,
                                         jpos - 1);
  ...
  AlignmentParametersData::DataContainer data(
      new AlignmentParametersData(selpar, selcov));
  CompositeAlignmentParameters aap(data, alignables, alidettoalimap, aliposmap,
                                   alilenmap);
...
CompositeAlignmentParameters AlignmentParameterStore::selectParameters(
  ...
  std::map<AlignableDetOrUnitPtr, Alignable *>
      alidettoalimap; // This map won't be filled!!!
  std::map<Alignable *, int> aliposmap;
  std::map<Alignable *, int> alilenmap;
  int nparam = 0;
  ...
  for (ita = alignables.begin(); ita != alignables.end(); ++ita) {
    // Check if Alignable already there, insert into vector if not
    if (find(selectedAlignables.begin(), selectedAlignables.end(), *ita) ==
        selectedAlignables.end()) {
  ...
  int ipos = 1; // NOTE: .sub indices start from 1
  align::Alignables::const_iterator it1;
  for (it1 = selectedAlignables.begin(); it1 != selectedAlignables.end();
       ++it1) {
    AlignmentParameters *ap = (*it1)->alignmentParameters();
    selpar->sub(ipos, ap->selectedParameters());
    selcov->sub(ipos, ap->selectedCovariance());
  ...
  AlignmentParametersData::DataContainer data(
      new AlignmentParametersData(selpar, selcov));
  CompositeAlignmentParameters aap(data, selectedAlignables, alidettoalimap,
                                   aliposmap, alilenmap);
...
//__________________________________________________________________________________________________
void AlignmentParameterStore::updateParameters(
    const CompositeAlignmentParameters &aap, bool updateCorrelations) {
  ...
  for (align::Alignables::const_iterator it = alignables.begin();
    ...
    if (updateCorrelations) {
      int jpos = 1;
      for (align::Alignables::const_iterator it2 = alignables.begin();
           it2 != it; ++it2) {
        theCorrelationsStore->setCorrelations(*it, *it2, covariance, ipos - 1,
                                              jpos - 1);
        jpos += (*it2)->alignmentParameters()->numSelected();
      }
    }
...
align::Alignables AlignmentParameterStore::validAlignables(void) const {
  ...
  LogDebug("Alignment") << "@SUB=AlignmentParameterStore::validAlignables"
                        << "Valid alignables: " << result.size() << "out of "
                        << theAlignables.size();
  return result;
}
...
Alignable *AlignmentParameterStore::alignableFromAlignableDet(
    const AlignableDetOrUnitPtr &_alignableDet) const {
...
void AlignmentParameterStore::applyParameters(Alignable *alignable) {
  ...
  AlignmentParameters *pars =
      (alignable ? alignable->alignmentParameters() : nullptr);
  if (!pars) {
    throw cms::Exception("BadAlignable")
        << "applyParameters: provided alignable does not have alignment "
           "parameters";
  }
  pars->apply();
}
...
void AlignmentParameterStore::acquireRelativeParameters(void) {
  ...
  for (unsigned int i = 0; i < nAlignables; ++i) {
    ...
    RigidBodyAlignmentParameters *ap =
        dynamic_cast<RigidBodyAlignmentParameters *>(
    ...
    align::EulerAngles euloc =
        align::toAngles(ali->surface().toLocal(ali->rotation()));
    par[3] = euloc(1);
    par[4] = euloc(2);
    par[5] = euloc(3);
...
std::pair<int, int>
AlignmentParameterStore::typeAndLayer(const Alignable *ali,
                                      const TrackerTopology *tTopo) const {
  return TrackerAlignableId().typeAndLayerFromDetId(ali->id(), tTopo);
}

//__________________________________________________________________________________________________
void AlignmentParameterStore::applyAlignableAbsolutePositions(
    const align::Alignables &alivec, const AlignablePositions &newpos,
    int &ierr) {
  unsigned int nappl = 0;
  ierr = 0;

  // Iterate over list of alignables
  for (align::Alignables::const_iterator iali = alivec.begin();
    ...
    for (AlignablePositions::const_iterator ipos = newpos.begin();
      ...
      if (id == ipos->id() && typeId == ipos->objId()) {
        if (found) {
          ...
          SurfaceDeformationFactory::Type dtype =
              SurfaceDeformationFactory::kNoDeformations;
          std::vector<double> dold;
          if (1 == ali->surfaceDeformationIdPairs(
                       dold_id_pairs)) { // might not have any...
          ...
          // deformation has the same type and size.
          if (!dold.empty() &&
              dtype != SurfaceDeformationFactory::kNoDeformations &&
              dnew.size() == dold.size()) {
            ...
            edm::LogInfo("Alignment")
                ...
                << ", size " << defDiff.size() << " and first element "
                << defDiff.at(0) << " to alignable with id / type: " << id
                << " / " << typeId;
            ali->addSurfaceDeformation(deform, true);
            delete deform;
          }
          // In case no old surface deformation record exists, only ensure that
          // the new surface deformation record has size>0. Size check is done
          // elsewhere.
          else if (!dnew.empty()) {
            ...
            edm::LogInfo("Alignment")
                ...
                << SurfaceDeformationFactory::surfaceDeformationTypeName(
                       (SurfaceDeformationFactory::Type)deform->type())
                << ", size " << dnew.size() << " and first element "
                << dnew.at(0) << " to alignable with id / type: " << id << " / "
                << typeId;
            ali->addSurfaceDeformation(
                deform,
                true); // Equivalent to setSurfaceDeformation in this case
            delete deform;
          }
...
//__________________________________________________________________________________________________
void AlignmentParameterStore::applyAlignableRelativePositions(
    const align::Alignables &alivec, const AlignableShifts &shifts, int &ierr) {
  ierr = 0;
  unsigned int nappl = 0;
...
//__________________________________________________________________________________________________
void AlignmentParameterStore::attachAlignmentParameters(
...
//__________________________________________________________________________________________________
void AlignmentParameterStore::attachAlignmentParameters(
    const align::Alignables &alivec, const Parameters &parvec, int &ierr) {
  int ipass = 0;
  int ifail = 0;
  ierr = 0;
  ...
  for (align::Alignables::const_iterator iali = alivec.begin();
    ...
    bool found = false;
    for (Parameters::const_iterator ipar = parvec.begin(); ipar != parvec.end();
         ++ipar) {
...
//__________________________________________________________________________________________________
void AlignmentParameterStore::attachCorrelations(
    const align::Alignables &alivec, const Correlations &cormap, bool overwrite,
    int &ierr) {
  ierr = 0;
  int icount = 0;

  // Iterate over correlations
  for (Correlations::const_iterator icor = cormap.begin(); icor != cormap.end();
       ++icor) {
    ...
    if (find(alivec.begin(), alivec.end(), ali1) != alivec.end() &&
      ...
      if (!theCorrelationsStore->correlationsAvailable(ali1, ali2) ||
          (overwrite)) {
...
//__________________________________________________________________________________________________
void AlignmentParameterStore::attachUserVariables(
...
//__________________________________________________________________________________________________
void AlignmentParameterStore::setAlignmentPositionError(
    const align::Alignables &alivec, double valshift, double valrot) {
  unsigned int nAlignables = alivec.size();
  ...
  for (unsigned int i = 0; i < nAlignables; ++i) {
    ...
    // ali->setAlignmentPositionError(AlignmentPositionError
    // ape(valshift,valshift,valshift),true);
...
//__________________________________________________________________________________________________
bool AlignmentParameterStore ::hierarchyConstraints(
    const Alignable *ali, const align::Alignables &aliComps,
    std::vector<std::vector<ParameterId>> &paramIdsVecOut,
    std::vector<std::vector<double>> &factorsVecOut, bool all,
    double epsilon) const {
  // Weak point if all = false:
  // Ignores constraints between non-subsequent levels in case the parameter is
  // not considered in the intermediate level, e.g. global z for dets and layers
  // is aligned, but not for rods!
  ...
  bool firstComp = true;
  for (align::Alignables::const_iterator iComp = aliComps.begin(),
                                         iCompE = aliComps.end();
       iComp != iCompE; ++iComp) {
    ...
    const std::vector<bool> &aliCompSel =
        (*iComp)->alignmentParameters()->selector();
    for (unsigned int iParMast = 0, iParMastUsed = 0; iParMast < aliSel.size();
         ++iParMast) {
      ...
      for (unsigned int iParComp = 0; iParComp < aliCompSel.size();
           ++iParComp) {
        if (aliCompSel[iParComp]) {
          ...
          if (fabs(factor) > epsilon) {
            paramIdsVecOut[iParMastUsed].push_back(
