...
AlignmentExtendedCorrelationsStore::AlignmentExtendedCorrelationsStore(
  ...
  edm::LogInfo("Alignment") << "@SUB=AlignmentExtendedCorrelationsStore::"
...
void AlignmentExtendedCorrelationsStore::correlations(Alignable *ap1,
  ...
  if (ap1 == previousAlignable) {
    ExtendedCorrelationsTable::const_iterator itC2 =
        previousCorrelations->find(ap2);
...
void AlignmentExtendedCorrelationsStore::setCorrelations(
    Alignable *ap1, Alignable *ap2, const AlgebraicSymMatrix &cov, int row,
    int col) {
  static Alignable *previousAlignable = nullptr;
  static ExtendedCorrelationsTable *previousCorrelations;
...
void AlignmentExtendedCorrelationsStore::setCorrelations(Alignable *ap1,
  ...
  if (itC1 != theCorrelations.end()) {
    ...
    if (itC2 != itC1->second->end()) {
      itC2->second = transpose ? ExtendedCorrelationsEntry(mat.T())
                               : ExtendedCorrelationsEntry(mat);
    } else {
      (*itC1->second)[ap2] = transpose ? ExtendedCorrelationsEntry(mat.T())
                                       : ExtendedCorrelationsEntry(mat);
    }
  } else {
    ExtendedCorrelationsTable *newTable = new ExtendedCorrelationsTable;
    (*newTable)[ap2] = transpose ? ExtendedCorrelationsEntry(mat.T())
                                 : ExtendedCorrelationsEntry(mat);
    theCorrelations[ap1] = newTable;
  }
}

void AlignmentExtendedCorrelationsStore::getCorrelations(
    Alignable *ap1, Alignable *ap2, AlgebraicMatrix &mat) const {
  bool transpose = (ap2 > ap1);
  if (transpose)
    std::swap(ap1, ap2);
...
bool AlignmentExtendedCorrelationsStore::correlationsAvailable(
    Alignable *ap1, Alignable *ap2) const {
  bool transpose = (ap2 > ap1);
  if (transpose)
    std::swap(ap1, ap2);
...
void AlignmentExtendedCorrelationsStore::fillCorrelationsTable(
    Alignable *ap1, Alignable *ap2, ExtendedCorrelationsTable *table,
...
void AlignmentExtendedCorrelationsStore::fillCovariance(
    Alignable *ap1, Alignable *ap2, const ExtendedCorrelationsEntry &entry,
  ...
  for (int iRow = 0; iRow < nRow; ++iRow) {
    ...
    if (edm::isNotFinite(factor))
      throw cms::Exception("LogicError")
          << "[AlignmentExtendedCorrelationsStore::fillCovariance] "
...
void AlignmentExtendedCorrelationsStore::fillCovarianceT(
    Alignable *ap1, Alignable *ap2, const ExtendedCorrelationsEntry &entry,
    AlgebraicSymMatrix &cov, int row, int col) const {
  int nRow = entry.numRow();
  int nCol = entry.numCol();
  ...
  for (int iRow = 0; iRow < nRow; ++iRow) {
    ...
    if (edm::isNotFinite(factor))
      throw cms::Exception("LogicError")
          << "[AlignmentExtendedCorrelationsStore::fillCovarianceT] "
...
void AlignmentExtendedCorrelationsStore::readFromCovariance(
    Alignable *ap1, Alignable *ap2, ExtendedCorrelationsEntry &entry,
...
void AlignmentExtendedCorrelationsStore::resizeCorruptCorrelations(
    ExtendedCorrelationsEntry &entry, double maxCorr) {
  if (maxCorr > 1.) {
