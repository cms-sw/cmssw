...
int AlignmentCorrelationsIORoot::write(const align::Correlations &cor,
  ...
  for (align::Correlations::const_iterator it = cor.begin(); it != cor.end();
       ++it) {
    ...
    if ((ali1->alignmentParameters()->isValid() &&
         ali2->alignmentParameters()->isValid()) ||
        !(validCheck)) {
  ...
  edm::LogInfo("AlignmentCorrelationsIORoot")
      << "Writing correlations: all,written: " << cor.size() << "," << icount;
  return 0;
}
...
AlignmentCorrelationsIORoot::read(const align::Alignables &alivec, int &ierr) {
  ...
  std::map<std::pair<unsigned int, int>, Alignable *>::const_iterator
      aliSearch1;
  std::map<std::pair<unsigned int, int>, Alignable *>::const_iterator
      aliSearch2;
  int nfound = 0;
  ...
  edm::LogInfo("AlignmentCorrelationsIORoot")
      << "Read correlations: all,read: " << alivec.size() << "," << nfound;
