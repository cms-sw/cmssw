...
int AlignableDataIO::writeAbsPos(Alignable *ali, bool validCheck) {
  ...
  if (!(validCheck) || ali->alignmentParameters()->isValid()) {
    ...
    if (ali->alignableObjectId() ==
        align::AlignableDetUnit) { // only detunits have them
      std::vector<std::pair<int, SurfaceDeformation *>> result;
      if (1 ==
          ali->surfaceDeformationIdPairs(result)) { // might not have any...
    ...
    // write
    return writeAbsRaw(
        AlignableAbsData(pos, rot, ali->id(), ali->alignableObjectId(), pars));
  }
...
int AlignableDataIO::writeRelPos(Alignable *ali, bool validCheck) {
  if (!(validCheck) || ali->alignmentParameters()->isValid()) {
    ...
    // write
    return writeRelRaw(
        AlignableRelData(pos, rot, ali->id(), ali->alignableObjectId(), pars));
  }
...
int AlignableDataIO::writeOrgPos(Alignable *ali, bool validCheck) {
  if (!(validCheck) || ali->alignmentParameters()->isValid()) {
    ...
    align::RotationType rot =
        ali->globalRotation() * ali->rotation().transposed();
    // FIXME: should add something to store changes of surface deformations...
    std::vector<double> pars;
    // write
    return writeAbsRaw(
...
AlignablePositions AlignableDataIO::readAbsPos(const align::Alignables &alivec,
  ...
  LogDebug("ReadAbsPos") << "all,written: " << alivec.size() << ","
                         << retvec.size();
...
AlignablePositions AlignableDataIO::readOrgPos(const align::Alignables &alivec,
  ...
  LogDebug("ReadOrgPos") << "all,read: " << alivec.size() << ", "
                         << retvec.size();
...
AlignableShifts AlignableDataIO::readRelPos(const align::Alignables &alivec,
  ...
  LogDebug("ReadRelPos") << "all,read: " << alivec.size() << ", "
                         << retvec.size();
