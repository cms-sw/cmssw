...
TkModuleGroupSelector::TkModuleGroupSelector(AlignableTracker *aliTracker,
  ...
  const edm::RunNumber_t defaultReferenceRun =
      (cfg.exists("ReferenceRun")
           ? cfg.getParameter<edm::RunNumber_t>("ReferenceRun")
           : 0);

  // extract run range to be used for all module groups (if not locally
  // overwritten)
  const std::vector<edm::RunNumber_t> defaultRunRange =
      (cfg.exists("RunRange")
           ? cfg.getParameter<std::vector<edm::RunNumber_t>>("RunRange")
           : std::vector<edm::RunNumber_t>());

  // finally create everything from configuration
  this->createModuleGroups(aliTracker,
                           cfg.getParameter<edm::VParameterSet>("Granularity"),
...
void TkModuleGroupSelector::fillDetIdMap(const unsigned int detid,
  ...
  if (mapDetIdGroupId_.find(detid) == mapDetIdGroupId_.end()) {
    mapDetIdGroupId_.insert(
        std::pair<unsigned int, unsigned int>(detid, groupid));
  } else {
    throw cms::Exception("BadConfig")
...
//============================================================================
const bool
TkModuleGroupSelector::testSplitOption(const edm::ParameterSet &pset) const {
  bool split = false;
...
bool TkModuleGroupSelector::createGroup(
    unsigned int &Id, const std::vector<edm::RunNumber_t> &range,
  ...
  if (refrun > 0 && !range.empty()) { // FIXME: last condition not really
                                      // needed?
    Id += range.size() - 1;
  ...
  if (refrun > 0 && range.front() > refrun) { // range.size() > 0 checked before
    throw cms::Exception("BadConfig")
        ...
        << "Invalid combination of reference run number and specified run "
           "dependence"
...
void TkModuleGroupSelector::verifyParameterNames(const edm::ParameterSet &pset,
  ...
  for (std::vector<std::string>::const_iterator iParam = parameterNames.begin();
       iParam != parameterNames.end(); ++iParam) {
    const std::string name = (*iParam);
    if (name != "levels" && name != "RunRange" && name != "split" &&
        name != "ReferenceRun") {
      throw cms::Exception("BadConfig")
          << "@SUB=TkModuleGroupSelector::verifyParameterNames:"
          << " Unknown parameter name '" << name << "' in PSet number "
          << psetnr << ". Maybe a typo?";
    }
  }
}
...
//============================================================================
void TkModuleGroupSelector::createModuleGroups(
    AlignableTracker *aliTracker, const edm::VParameterSet &granularityConfig,
    const std::vector<edm::RunNumber_t> &defaultRunRange,
  ...
  for (edm::VParameterSet::const_iterator pset = granularityConfig.begin();
       pset != granularityConfig.end(); ++pset) {
    ...
    bool modules_selected =
        false; // track whether at all a module has been selected in this group
    const std::vector<edm::RunNumber_t> range =
        ((*pset).exists("RunRange")
             ? pset->getParameter<std::vector<edm::RunNumber_t>>("RunRange")
             : defaultRunRange);
    if (range.empty()) {
      throw cms::Exception("BadConfig")
    ...
    for (const auto &it : alis) {
      ...
      for (const auto &iD : aliDaughts) {
        if (iD->alignableObjectId() == align::AlignableDetUnit ||
          ...
          if (split) {
            modules_selected = this->createGroup(
                Id, range, std::list<Alignable *>(1, iD), refrun);
          } else {
...
//============================================================================
edm::RunNumber_t
TkModuleGroupSelector::firstRunOfIOV(unsigned int iovNum) const {
  return iovNum < this->numIovs() ? globalRunRange_.at(iovNum) : 0;
}

//======================================================================
int TkModuleGroupSelector::getParameterIndexFromDetId(
  ...
  for (std::vector<int>::const_iterator itSubDets = subdetids_.begin();
       itSubDets != subdetids_.end(); ++itSubDets) {
  ...
  if (it != mapDetIdGroupId_.end()) {
    ...
    if (refrun > 0) { // if > 0 a reference run number has been provided
      ...
      if (run > refrun) {
        // iovNum > 0 due to checks in createGroup(...) and
        // createModuleGroups(...) remove IOV in which the reference run can be
        // found
        iovNum -= 1;
      }
    }
