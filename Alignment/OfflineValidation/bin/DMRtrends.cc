...
struct Point {
  ...
  /*! \fn Point
     ...
     */
  Point(float Run, TH1 *histo) : Point(Run, histo->GetMean(), histo->GetMeanError()) {}
  ...
  float GetDeltaMu() const {
    if (muplus == DUMMY && muminus == DUMMY) {
      ...
      return DMRFactor * (muplus - muminus);
    }
  }
  float GetSigmaDeltaMu() const {
    if (sigmaplus == DUMMY && sigmaminus == DUMMY) {
      ...
      return DMRFactor * hypot(sigmaplus, sigmaminus);
    }
  };
};
...
bool checkrunlist(vector<int> runs, vector<int> IOVlist, TString Year) {
  ...
  if (problemfound) {
    ...
    if (!missingruns.empty()) {
      cout << "WARNING: some runs are missing in the run/luminosity txt file" << endl
...
void PlotDMRTrends(vector<string> labels,
                   TString Year,
                   string myValidation,
                   vector<string> geometries,
                   vector<Color_t> colours,
...
int main(int argc, char *argv[]) {
  if (argc == 1) {
    vector<int> IOVlist = {290543, 296702, 296966, 297224, 297281, 297429, 297467, 297484, 297494, 297503,
                           297557, 297599, 297620, 297660, 297670, 298678, 298996, 299062, 299096, 299184,
                           299327, 299368, 299381, 299443, 299480, 299592, 299594, 299649, 300087, 300155,
                           300233, 300237, 300280, 300364, 300389, 300399, 300459, 300497, 300515, 300538,
                           300551, 300574, 300636, 300673, 300780, 300806, 300812, 301046, 301417, 302131,
                           302573, 302635, 303825, 303998, 304170, 304505, 304672, 305040, 305081};  //UL17
  ...
  TString runlist = argv[1], all_labels = argv[2], Year = argv[3], pathtoDMRs = argv[4],
          geometrieandcolours = argv[5],  //name1:title1:color1,name2:title2:color2,name3:title3:color3
      outputdirectory = argv[6], pixelupdatelist = argv[7];
  bool showpixelupdate = argv[8], showlumi = argv[9], FORCE = argv[10];
