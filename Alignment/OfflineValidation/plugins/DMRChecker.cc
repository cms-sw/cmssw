...
class DMRChecker : public edm::EDAnalyzer {
 public:
    ...
    */
	...
	if(hit->isValid() && 
	  ...
	  if( subid == StripSubdetector::TIB ) {
	    ...
	    // absolute residuals
	    this->updateOnlineMomenta(runningMeanOfResTIB_,
				      runningVarOfResTIB_,
				      detId.rawId(),
	    ...
	    this->updateOnlineMomenta(runningNormMeanOfResTIB_,
				      runningNormVarOfResTIB_,
				      detId.rawId(),
				      resErrX
				      );
	    ...
	    // absolute residuals
	    this->updateOnlineMomenta(runningMeanOfResTOB_,
				      runningVarOfResTOB_,
				      detId.rawId(),
				      uOrientation*resX*10000.
				      );
	    ...
	    // normalized residuals 
	    this->updateOnlineMomenta(runningNormMeanOfResTOB_,
				      runningNormVarOfResTOB_,
				      detId.rawId(),
				      resErrX
				      );
	    ...
	    // absolute residuals
	    this->updateOnlineMomenta(runningMeanOfResTID_,
				      runningVarOfResTID_,
				      detId.rawId(),
				      uOrientation*resX*10000.
				      );
	    ...
	    // normalized residuals 
	    this->updateOnlineMomenta(runningNormMeanOfResTID_,
				      runningNormVarOfResTID_,
				      detId.rawId(),
				      resErrX
				      );
	    ...
	    // absolute residuals
	    this->updateOnlineMomenta(runningMeanOfResTEC_,
				      runningVarOfResTEC_,
				      detId.rawId(),
				      uOrientation*resX*10000.
				      );
	    ...
	    // normalized residuals 
	    this->updateOnlineMomenta(runningNormMeanOfResTEC_,
				      runningNormVarOfResTEC_,
				      detId.rawId(),
				      resErrX
				      );
	...
	if(pixhit){
	  if(pixhit->isValid() ) {
		...
		*/
		...
		this->updateOnlineMomenta(runningMeanOfResBPixX_,
					  runningVarOfResBPixX_,
					  detId.rawId(),
					  uOrientation*resX*10000.
					  );

		// normalized residuals X
		this->updateOnlineMomenta(runningNormMeanOfResBPixX_,
					  runningNormVarOfResBPixX_,
					  detId.rawId(),
					  resErrX
					  );
	
		// absolute residuals Y
		this->updateOnlineMomenta(runningMeanOfResBPixY_,
					  runningVarOfResBPixY_,
					  detId.rawId(),
					  vOrientation*resY*10000.
					  );

		// normalized residuals Y
		this->updateOnlineMomenta(runningNormMeanOfResBPixY_,
					  runningNormVarOfResBPixY_,
					  detId.rawId(),
					  resErrY
					  );
		...
		*/
		...
		// absolute residuals X		
		this->updateOnlineMomenta(runningMeanOfResFPixX_,
					  runningVarOfResFPixX_,
					  detId.rawId(),
					  uOrientation*resX*10000.
					  );

		// normalized residuals X
		this->updateOnlineMomenta(runningNormMeanOfResFPixX_,
					  runningNormVarOfResFPixX_,
					  detId.rawId(),
					  resErrX
					  );
	
		// absolute residuals Y
		this->updateOnlineMomenta(runningMeanOfResFPixY_,
					  runningVarOfResFPixY_,
					  detId.rawId(),
					  vOrientation*resY*10000.
					  );

		// normalized residuals Y
		this->updateOnlineMomenta(runningNormMeanOfResFPixY_,
					  runningNormVarOfResFPixY_,
					  detId.rawId(),
					  resErrY
					  );
  ...
  //*************************************************************
  void updateOnlineMomenta( std::map<int,std::pair<float,int> >  &mean, std::map<int,std::pair<float,int> > &var,int theID, float data){
