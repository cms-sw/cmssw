...
void
SplitVertexResolution::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  using namespace edm;
  ...
  if(!runNumbersTimesLog_.count(RunNumber_)){
    ...
    if(debug_){
      ...
      edm::LogInfo("SplitVertexResolution") << RunNumber_ << " has start time: " << times.first << " - "<< times.second << std::endl;
      edm::LogInfo("SplitVertexResolution") <<"human readable time: " << std::asctime(std::gmtime(&start_time)) << std::endl;
    }
...
SplitVertexResolution::beginJob()
{
  ...
  h_OrigVertexErrX = outfile_->make<TH1F>( "h_OrigVertexErrX"  , "x-coordinate vertex error;vertex error (x) [#mum];vertices", 300,0., 300. );
  h_OrigVertexErrY = outfile_->make<TH1F>( "h_OrigVertexErrY"  , "y-coordinate vertex error;vertex error (y) [#mum];vertices", 300,0., 300. );
  h_OrigVertexErrZ = outfile_->make<TH1F>( "h_OrigVertexErrZ"  , "z-coordinate vertex error;vertex error (z) [#mum];vertices", 500,0., 500. );

  h_errX = outfile_->make<TH1F>( "h_errX"  , "x-coordinate vertex resolution error;vertex resoltuion error (x) [#mum];vertices", 300,0., 300. );
  h_errY = outfile_->make<TH1F>( "h_errY"  , "y-coordinate vertex resolution error;vertex resolution error (y) [#mum];vertices", 300,0., 300. );
  h_errZ = outfile_->make<TH1F>( "h_errZ"  , "z-coordinate vertex resolution error;vertex resolution error (z) [#mum];vertices", 500,0., 500. );
...
SplitVertexResolution::endJob() 
{
  ...
  tksByTrigger_ = outfile_->make<TH1D>("tksByTrigger","tracks by HLT path;;% of # traks",nFiringTriggers,-0.5,nFiringTriggers-0.5);
  evtsByTrigger_= outfile_->make<TH1D>("evtsByTrigge","events by HLT path;;% of # events",nFiringTriggers,-0.5,nFiringTriggers-0.5);
  ...
  for(std::map<std::string,std::pair<int,int> >::iterator it=triggerMap_.begin(); it!=triggerMap_.end(); ++it){
    ...
    std::cout<<"HLT path: "<<std::setw(60) << std::left << it->first << " | events firing: " << std::right << std::setw(8)<<(it->second).first << " ("<< std::setw(8) << std::fixed << evtpercent <<"%)"
	     <<" | tracks collected: " <<std::setw(10)<< (it->second).second << " ("<< std::setw(8) << std::fixed << trkpercent <<"%)"<<  '\n';
...
void
SplitVertexResolution::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
//*************************************************************
{
  ...
  if(debug_){
    edm::LogInfo("SplitVertexResolution")<<runInfo.product()->m_start_time_str << " " << runInfo.product()->m_stop_time_str << std::endl;
  }
...
{  
  ...
  for(auto iterator = h.begin(); iterator != h.end(); iterator++) {
      ...
      case PVValHelper::MAD:
	{
      ...
      default:
	edm::LogWarning("SplitVertexResolution")<<"fillTrendPlotByIndex() "<<fitPar_<<" unknown estimator!"<<std::endl;
	break;
      }
  }
}
...
//*************************************************************
{
  ...
  if(TMath::IsNaN(mean) || TMath::IsNaN(sigma)){  
    ...
    edm::LogWarning("SplitVertexResolution")<< "FitPVResiduals::fitResiduals(): histogram" << hist->GetName()  << " mean or sigma are NaN!!"<< std::endl;
  }
