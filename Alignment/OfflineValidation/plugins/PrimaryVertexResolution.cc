...
void PrimaryVertexResolution::beginJob() {
  ...
  h_ntrks = outfile_->make<TH1F>("h_ntrks",
                                 ...
                                 myNTrack_bins_.data());
  h_nhalftrks = outfile_->make<TH1F>("h_halfntrks",
                                     "number of tracks in sub-vertex;sub-vertex multeplicity;vertices",
                                     myNTrack_bins_.size() - 1,
                                     myNTrack_bins_.data());
  ...
  h_errX1 = SubVertices.make<TH1F>(
      "h_errX1", "x-coordinate vertex uncertainty;vertex1 err_{x} [#mum];vertices", 250, 0., 500.);
  h_errY1 = SubVertices.make<TH1F>(
      "h_errY1", "y-coordinate vertex uncertainty;vertex1 err_{y} [#mum];vertices", 250, 0., 500.);
  h_errZ1 = SubVertices.make<TH1F>(
      "h_errZ1", "z-coordinate vertex uncertainty;vertex1 err_{z} [#mum];vertices", 250, 0., 500.);
  ...
  h_errX2 = SubVertices.make<TH1F>(
      "h_errX2", "x-coordinate vertex 2 uncertainty;vertex2 err_{x} [#mum];vertices", 250, 0., 500.);
  h_errY2 = SubVertices.make<TH1F>(
      "h_errY2", "y-coordinate vertex 2 uncertainty;vertex2 err_{y} [#mum];vertices", 250, 0., 500.);
  h_errZ2 = SubVertices.make<TH1F>(
      "h_errZ2", "z-coordinate vertex 2 uncertainty;vertex2 err_{z} [#mum];vertices", 250, 0., 500.);
...
void PrimaryVertexResolution::endJob() {
  // resolutions
...
//*************************************************************
void PrimaryVertexResolution::fillTrendPlotByIndex(TH1F* trendPlot,
                                                   std::vector<TH1F*>& h,
