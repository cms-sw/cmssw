...
def plot(json_files, labels, output_file, f_ymin, f_ymax):
    ...
    for ix,x in enumerate(x_store):
        ...
        # Plot
        plt.errorbar(newXpruned,  new_y_store[ix], yerr=new_y_err_store[ix], color = colors[ix], fmt='o',  capsize=5, label=labels[ix], zorder=1)
        plt.plot(newXpruned, poly1d_fn(newXpruned), '--', color = colors[ix], label="lin. "+labels[ix])
