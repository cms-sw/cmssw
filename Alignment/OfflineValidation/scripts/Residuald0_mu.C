...
//&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
hframe = new TH2F("hframe","#Delta(d_{0})",100,-0.03,0.03,32,0.,0.32);
hframe->SetXTitle("#Delta(d_{0}) [#mum]");
hframe->SetYTitle("N. ev. /0.0006");
hframe->Draw();
...
for(int i=0; i<5; i++){
  sprintf(name,"Resd0[%d]",i);
  Resd0[i] = new TH1F(name,name,100,-0.03,0.03);
  sprintf(histoname,"Resd0[%d]",i);
  MyTree[i]->Project(histoname,"resd0","eff==1 && TrackID==13");

  cout << "Entries " << Resd0[i]->GetEntries() <<endl;
  Resd0[i]->Scale(1/Resd0[i]->GetEntries());
  Resd0[i]->SetTitle("#Delta(d_{0})");    
  Resd0[i]->SetXTitle("#Delta(d_{0}) ");
  Resd0[i]->SetYTitle("arb. units");
  
  Resd0[i]->SetLineColor(i+2);
  Resd0[i]->SetLineStyle(i+1);
  Resd0[i]->SetLineWidth(i+2);
  if (i==0) Resd0[i]->Draw("same");
  else Resd0[i]->Draw("same");
  //  c1->WaitPrimitive();
  c1->Update();
}

Legend("Resd0[0]","Resd0[1]","Resd0[2]","Resd0[3]","Resd0[4]",Resd0[0],Resd0[1],Resd0[2],Resd0[3],Resd0[4]);

c1->SaveAs("Residuald0_mu.eps");
c1->SaveAs("Residuald0_mu.gif");
gROOT->Reset();
gROOT->Clear();
