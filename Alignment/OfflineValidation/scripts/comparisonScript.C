...
void comparisonScript(){
	...
	comparisonPlots c1("fromMarkus/testCompare.root","fromMarkus/output_testCompare.root");
	...
	// ETA CUTS
	TCut EtaMinusCut = "(-ln((arctan(r/z)/2)) <= -1)";
	//TCut EtaMiddleCut = "((-ln((arctan(r/z)/2)) >= -1)&&(-ln((arctan(r/z)/2)) <= 1))";
	TCut EtaMiddleCut = "(r/z > 1)";
	TCut EtaPlusCut = "(-ln((arctan(r/z)/2)) >= 1)";
	...
	c1.plot3x5( levelCut+TIBCut+EtaMiddleCut );
