...
import configTemplates_ZeroAPE_0T
...
class Alignment:
    ...
    def getLoadTemplate(self):
        return replaceByMap( configTemplates_ZeroAPE_0T.dbLoadTemplate, self.getRepMap() )
...
"""
    ...
    def createConfiguration(self, path ):
        ...
        cfgs = {"TkAlCompareToNTuple.%s_cfg.py"%self.alignmentToValidate.name:
                    replaceByMap( configTemplates_ZeroAPE_0T.intoNTuplesTemplate, repMap)}
        if not self.referenceAlignment == "IDEAL":
            ...
            cfgs[ cfgFileName ] = replaceByMap( configTemplates_ZeroAPE_0T.intoNTuplesTemplate, referenceRepMap)
        ...
        for common in self.__compares:
            ...
            if self.__compares[common][1].split()[0] == "true":
                repMap["dbOutputService"] = configTemplates_ZeroAPE_0T.dbOutputTemplate
            else:
                repMap["dbOutputService"] = ""
            cfgName = replaceByMap("TkAlCompareCommon.oO[common]Oo...oO[name]Oo._cfg.py",repMap)
            cfgs[ cfgName ] = replaceByMap(configTemplates_ZeroAPE_0T.compareTemplate, repMap)
...
cd .oO[workdir]Oo.
"""
        scripts = {scriptName: replaceByMap( configTemplates_ZeroAPE_0T.scriptTemplate, repMap ) }  
...
class OfflineValidation(GenericValidation):
    ...
    def createConfiguration(self, path ):
        ...
        cfgs = {cfgName:replaceByMap( configTemplates_ZeroAPE_0T.offlineTemplate, repMap)}
    ...
    def getRepMap(self, alignment = None):
        ...
        repMap.update({
                ...
                "zeroAPE": configTemplates_ZeroAPE_0T.zeroAPETemplate,
...
class MonteCarloValidation(GenericValidation):
    ...
    def createConfiguration(self, path ):
        ...
        repMap["outputFile"] = os.path.expandvars( repMap["outputFile"] )
        repMap["outputFile"] = os.path.abspath( repMap["outputFile"] )
        cfgs = {cfgName:replaceByMap( configTemplates_ZeroAPE_0T.mcValidateTemplate, repMap)}
...
class TrackSplittingValidation(GenericValidation):
    ...
    def createConfiguration(self, path ):
        ...
        repMap["outputFile"] = os.path.expandvars( repMap["outputFile"] )
        repMap["outputFile"] = os.path.abspath( repMap["outputFile"] )
        cfgs = {cfgName:replaceByMap( configTemplates_ZeroAPE_0T.TrackSplittingTemplate, repMap)}
        self.filesToCompare[ GenericValidation.defaultReferenceName ] = repMap["outputFile"]
        GenericValidation.createConfiguration(self, cfgs, path)
...
def createExtendedValidationScript(offlineValidationList, outFilePath):
    ...
    theFile.write( replaceByMap( configTemplates_ZeroAPE_0T.extendedVaidationTemplate ,repMap ) )
    theFile.close()
...
def createMergeScript( path, validations ):
    ...
    if "OfflineValidation" in comparisonLists:
        ...
        repMap["RunExtendedOfflineValidation"] = replaceByMap(configTemplates_ZeroAPE_0T.extendedVaidationExecution, repMap)
    ...
    for validationId in comparisonLists:
        ...
        repMap["CompareAllignments"] += replaceByMap( configTemplates_ZeroAPE_0T.compareAlignmentsExecution, repMap )
      
    filePath = os.path.join(path, "TkAlMerge.sh")
    theFile = open( filePath, "w" )
    theFile.write( replaceByMap( configTemplates_ZeroAPE_0T.mergeTemplate, repMap ) )
    theFile.close()
...
def loadTemplates( config ):
    if config.has_section("alternateTemplates"):
        for templateName in config.options("alternateTemplates"):
            ...
            configTemplates_ZeroAPE_0T.alternateTemplate(templateName, newTemplateName)
