...
{
    return operator() (firstRun, run);
}

float Run2Lumi::operator() () const
{
    return operator() (firstRun, lastRun);
}

template<typename T> void CopyStyle (T * objIn, T * objOut)
{
...
TGraph * Run2Lumi::operator() (TGraph * gIn) const 
{
    ...
    for (int i = 0; i < n-1; ++i) {
        ...
        if (currentRun < firstRun) continue;
        if (currentRun >= lastRun) break;

        int nextRun = gIn->GetPointX(i+1);

        auto lumi_edge  = operator()(firstRun, currentRun),
             lumi_width = operator()(currentRun, nextRun);
        x.push_back(lumi_edge + lumi_width/2);
        ex.push_back(           lumi_width/2);

        auto point = gIn->GetPointY(i),
             error = gIn->GetErrorY(i);
        y.push_back(point);
        ey.push_back(error);
    }
...
TH1 * Run2Lumi::operator() (TH1 * hIn) const
{
    ...
    for (int i = 1; i <= N; ++i) {
        ...
        if (nextRun < firstRun) continue;
        if (nextRun >= lastRun) break;

        edges.push_back(operator()(nextRun));

        auto content = hIn->GetBinContent(i),
    ...
    N = edges.size()-1;
    TString name = hIn->GetName(); name += "_byLumi";
    ...
    lumiType(lumiAxisType)
{
    ...
    for (auto& type: lines) {
        ...
        if (style) v->SetLineStyle(StyleParser(*style));

        auto color = line->get_optional<string>("color");
        if (color) v->SetLineColor(ColorParser(*color));

        auto width = line->get_optional<int>("width");
        if (width) v->SetLineWidth(*width);

        auto title = line->get_optional<string>("title");
        if (title) lgd.AddEntry(v, title->c_str(), "l");
...
void Trend::operator() (TObject * obj,
        TString drawOpt, TString lgdOpt, bool fullRange)
{
    cout << __func__ << endl;
    c.cd();
    ...
    if (classname.Contains("TGraph")) {
        ...
        else cout << " -> hole between end of graph and right edge" << endl;
        g = GetLumi(g);
        g->Draw("same" + drawOpt); 
    }
