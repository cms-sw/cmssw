...
#define INSIDE_VECTOR(vector) \
    std::cout << #vector << "={"; for (unsigned int i = 0 ; i < vector.size()-1 ; i++) std::cout << vector[i] << ","; std::cout << vector.back() << "}";
    ...
    _canvas_index(canvas_idx)
{
    ...
    std::cout << __FILE__ << ":"<< __LINE__ << ":Info: inside constructor of GeometryComparisonPlotter utility"<< std::endl;
#endif
    // _canvas_index
    //_canvas_index = 0;
    ...
    { 
#ifdef TALKATIVE
        std::cout << __FILE__ << ":" << __LINE__ << ":Info: computing the rdphi branch from r and dphi branches (assuming they exist...)" << std::endl;
#endif
...
{
#ifdef DEBUG
    std::cout << __FILE__ << ":" << __LINE__ << ":Info: in destructor of the GeometryComparisonPlotter utility" << std::endl;
#endif
    tree_file->Close();
#ifdef DEBUG
...
#define CHECK_BRANCHES(branchname_vector) \
    ...
    {   \
        ...
        {   \
            std::cout << __FILE__ << ":" << __LINE__ << ":Error: The branch " << branchname_vector[i] << " is not recognised." << std::endl; \
            return; \
        }   \
    }
    CHECK_BRANCHES(x);
...
#ifdef TALKATIVE
    std::cout << __FILE__ << ":" << __LINE__ << ":Info: ";    INSIDE_VECTOR(x);   std::cout << std::endl
         << __FILE__ << ":" << __LINE__ << ":Info: ";    INSIDE_VECTOR(y);   std::cout << std::endl;
#endif
...
#ifdef DEBUG
    std::cout << __FILE__ << ":" << __LINE__ << ":Info: Creation of the TGraph[" << x.size() << "][" << y.size() << "][" << NB_SUBLEVELS*NB_Z_SLICES*NB_MODULE_QUALITY << "] ended." << std::endl;
#endif
...
//#endif
    ...
    {   // opening the file to write the graphs
        ...
        if (output->IsZombie())
        {
            std::cout << __FILE__ << ":" << __LINE__ << ":Error: Opening of " << _output_directory+TString(_output_filename) << " failed" << std::endl;
            exit(-1);
        }
#ifdef TALKATIVE
        std::cout << __FILE__ << ":"<< __LINE__ << ":Info: output file is " << _output_directory+TString(_output_filename) << std::endl;
#endif
    }
    ...
    for (unsigned int ix = 0 ; ix < x.size() ; ix++)
    {
        ...
        for (unsigned int iy = 0 ; iy < y.size() ; iy++)
        {
...
#ifdef DEBUG
            std::cout << __FILE__ << ":" << __LINE__ << ":Info: x[" << ix << "]=" << x[ix]
                                                <<   " and y[" << iy << "]=" << y[iy] 
            ...
            for (unsigned short int jgraph = 0 ; jgraph < NB_SUBLEVELS*NB_Z_SLICES*n_module_types ; jgraph++)
            {
                ...
                {
#ifdef TALKATIVE
                    std::cout <<  __FILE__ << ":" << __LINE__ << ":Info: " << graphs[ix][iy][igraph]->GetName()  << " is empty." << std::endl;
#endif
                    continue;
                }
#ifdef DEBUG
...
#ifdef TALKATIVE
                else    std::cout << __FILE__ << ":" << __LINE__ << ":Info: graphs[ix][iy][isublevel-1]=" << graphs[ix][iy][isublevel-1]->GetName() << " is empty -> not added into " << mgraphs[ix][iy][isublevel]->GetName() << std::endl;
#endif
...
#ifdef TALKATIVE
                else    std::cout << __FILE__ << ":" << __LINE__ << ":Info: graphs[ix][iy][NB_SUBLEVEL+isublevel-1]=" << graphs[ix][iy][NB_Z_SLICES+isublevel-1]->GetName() << " is empty -> not added into " << mgraphs[ix][iy][isublevel]->GetName() << std::endl;
#endif
#if NB_Z_SLICES!=2
			...
			for (unsigned int iy = 0 ; iy < y.size() ; iy++)
            {
	            ...
	            for (unsigned int isublevel = 1 ; isublevel <= NB_SUBLEVELS ; isublevel++)
	            {
	                ...
	                if (_print && !_print_only_global) c_hist[ix][iy][isublevel]->Print(_output_directory 
																	+ TString::Format("Profile_plot_%s_vs_%s_%s_%d", x[ix].Data(), y[iy].Data(),_sublevel_names[isublevel-1].Data(), _canvas_index)
																	+ ExtensionFromPrintOption(_print_option),
																	_print_option);
...
#ifdef TALKATIVE
    std::cout << __FILE__ << ":" << __LINE__ << ":Info: ";    INSIDE_VECTOR(x);   std::cout << std::endl;
    std::cout << __FILE__ << ":" << __LINE__ << ":Info: ";    INSIDE_VECTOR(y);   std::cout << std::endl;
#endif
...
#ifdef DEBUG
    std::cout << __FILE__ << ":" << __LINE__ << ":Info: Creation of the TH1F[" << y.size() << "][" << NB_SUBLEVELS*NB_Z_SLICES*NB_X_SLICES << "] ended." << std::endl;
#endif
...
TString GeometryComparisonPlotter::ExtensionFromPrintOption (TString print_option)
{
    ...
    {
        std::cout << __FILE__ << ":" << __LINE__ << ":Warning: unknown format. Returning .pdf, but possibly wrong..." << std::endl;
                                             return TString(".pdf");
    }
}
