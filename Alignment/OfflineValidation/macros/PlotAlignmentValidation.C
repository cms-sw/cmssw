...
PlotAlignmentValidation::PlotAlignmentValidation(
    const char* inputFile, std::string legendName, int lineColor, int lineStyle, bool bigtext)
 ...
 */
void PlotAlignmentValidation::useFitForDMRplots(bool usefit) { useFit_ = usefit; }
...
//------------------------------------------------------------------------------
void PlotAlignmentValidation::plotOutlierModules(const char* outputFileName,
                                                 std::string plotVariable,
                                                 float plotVariable_cut,
                                                 int unsigned minHits) {
  Int_t counter = 0;
  ...
  c1->Print((outputFile + "]").Data());
  if (counter == 0)
    cout << "no bad modules found" << endl;
...
//------------------------------------------------------------------------------
void PlotAlignmentValidation::setTreeBaseDir(std::string dir) { treeBaseDir = dir; }
...
THStack* PlotAlignmentValidation::addHists(
    const TString& selection, const TString& residType, TLegend** myLegend, bool printModuleIds, bool validforphase0) {
  enum ResidType {
    xPrimeRes,
    yPrimeRes,
    xPrimeNormRes,
          ...
          for (int iDisk = 1; iDisk <= 9; iDisk++)
            for (int iSide = 1; iSide <= 2; iSide++)
              for (int iPetal = 1; iPetal <= 8; iPetal++)
                ...
                {
                  stringstream s;
                  s << "TrackerOfflineValidationStandalone/Strip/TECEndcap_" << iEndcap << "/TECDisk_" << iDisk
                    << "/TECSide_" << iSide << "/TECPetal_" << iPetal << "/" << basename << "TECRing_" << iRing;
...
void PlotAlignmentValidation::setTitleStyle(
    TNamed& hist, const char* titleX, const char* titleY, int subDetId, bool isSurfaceDeformation, TString secondline) {
...
//------------------------------------------------------------------------------
std::string PlotAlignmentValidation::getSelectionForDMRPlot(int minHits, int subDetId, int direction, int layer) {
...
void PlotAlignmentValidation::residual_by_moduleID(unsigned int moduleid) {
  ...
  for (auto it : sourceList) {
    ...
    TString histnamex = legendname + " NEntries: " + TString(to_string(int(hist[0]->GetEntries())));
    ...
    TString histnamey = legendname + " NEntries: " + TString(to_string(int(hist[1]->GetEntries())));
