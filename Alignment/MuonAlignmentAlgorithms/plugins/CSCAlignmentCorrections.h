...
class CSCAlignmentCorrections {
public:
  CSCAlignmentCorrections(std::string fitterName, double oldchi2, double newchi2): m_fitterName(std::move(fitterName)), m_oldchi2(oldchi2), m_newchi2(newchi2) {};
  ...
  void insertResidual(const std::string& i, const std::string& j, double before, double uncert, double residual, double pull) {
    m_i.push_back(i);
    m_j.push_back(j);
