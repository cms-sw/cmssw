...
void MuonResidualsAngleFitter::plot(std::string name, TFileDirectory *dir) {
  ...
  if (residualsModel() == kPureGaussian) {
    ...
    narrowed_fit->SetParameters(scale_factor, value(kAngle) * 10., value(kSigma) * 10.);
    narrowed_fit->Write();
  }
    ...
    narrowed_fit->SetParameters(scale_factor, value(kAngle) * 10., value(kSigma) * 10., value(kGamma) * 10.);
    narrowed_fit->Write();
  }

  TF1 *qoverpt_fit = new TF1(qoverpt_name.str().c_str(), "[0]+x*[1]", -0.05, 0.05);
  qoverpt_fit->SetParameters(value(kAngle) * 10., value(kBfield) * 10.);
  qoverpt_fit->Write();
  ...
  for (std::vector<double*>::const_iterator resiter = residuals_begin();  resiter != residuals_end();  ++resiter) {
    ...
    if (inRange(corrected_residual)) {
      qoverpt_hist->Fill(qoverpt, raw_residual * 10.);
    }
  }
}
