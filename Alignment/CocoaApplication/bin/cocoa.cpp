...
int main(int argc, char **argv) {
  ...
  case 2:
    ...
    break;
  default:
    std::cerr << "WARNING: more than two arguments, from third on will not be "
                 "taken into account "
              << std::endl;
    break;
  }
  ...
  time_start = now;
  if (ALIUtils::debug >= 0)
    std::cout << "TIME:START_READING  : " << now << " "
              << difftime(now, ALIUtils::time_now()) / 1.E6 << "   "
  ...
  now = clock();
  if (ALIUtils::debug >= 0)
    std::cout << "TIME:ENDED_READING  : " << now << " "
              << difftime(now, ALIUtils::time_now()) / 1.E6 << "   "
              << ALIUtils::debug << std::endl;
  ALIUtils::set_time_now(now);
  ...
  now = clock();
  if (ALIUtils::debug >= 0)
    std::cout << "TIME:PROGRAM ENDED  : " << now << " "
              << difftime(now, ALIUtils::time_now()) / 1.E6 << std::endl;
  if (ALIUtils::debug >= 0)
    std::cout << "TIME:TOTAL PROGRAM  : " << now << " "
              << difftime(now, time_start) / 1.E6 << std::endl;
