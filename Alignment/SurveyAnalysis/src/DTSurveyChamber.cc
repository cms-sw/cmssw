...
DTSurveyChamber::DTSurveyChamber(int m_wheel, int m_station, int m_sector,
                                 long m_rawId) {
...
void DTSurveyChamber::addPoint(int code, const TMatrixD &r,
                               const TMatrixD &disp, const TMatrixD &err) {
...
TMatrixD &DTSurveyChamber::makeVector() {
  ...
  result->Zero();
  int real = 0;
  for (std::vector<TMatrixD>::iterator p = pointsDiff.begin();
       p != pointsDiff.end(); ++p) {
...
TMatrixD &DTSurveyChamber::makeErrors() {
  ...
  result->Zero();
  int real = 0;
  for (std::vector<TMatrixD>::iterator p = pointsError.begin();
       p != pointsError.end(); ++p) {
...
TMatrixD &DTSurveyChamber::makeMatrix() {
  ...
  result->Zero();
  int real = 0;
  for (std::vector<TMatrixD>::iterator p = pointsTheoretical.begin();
       p != pointsTheoretical.end(); p++) {
    (*result)(real * 3, 0) = 1.0;
...
std::ostream &operator<<(std::ostream &flujo, const DTSurveyChamber &obj) {
  ...
  flujo << obj.getId() << " " << obj.getDeltaX() << " " << obj.getDeltaXError()
        << " " << obj.getDeltaY() << " " << obj.getDeltaYError() << " "
        << obj.getDeltaZ() << " " << obj.getDeltaZError() << " "
        << obj.getAlpha() << " " << obj.getAlphaError() << " " << obj.getBeta()
        << " " << obj.getBetaError() << " " << obj.getGamma() << " "
