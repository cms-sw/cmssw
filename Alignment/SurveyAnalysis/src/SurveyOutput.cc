...
void SurveyOutput::write(unsigned int iter) {
  ...
  for (unsigned int i = 0; i < N; ++i) {
    ...
    align::EulerAngles angles =
        align::toAngles(ali->rotation()) * 1e3; // to mrad

    nt->Fill(shifts.x(), shifts.y(), shifts.z(), angles(1), angles(2),
             angles(3));
    //     const AlgebraicVector& pars =
    //     ali->alignmentParameters()->parameters();
