...
const DTSurveyChamber *DTSurvey::getChamber(int station, int sector) const {
...
void DTSurvey::ReadChambers(edm::ESHandle<DTGeometry> pDD) {
  ...
  for (int cont_stat = 0; cont_stat < 4; cont_stat++) {
    ...
    for (int cont_sect = 0; cont_sect < 14; cont_sect++) {
      ...
      chambers[cont_stat][cont_sect] =
          new DTSurveyChamber(id, cont_stat + 1, cont_sect + 1, mId.rawId());
    }
  }
  ...
  while (!file.eof()) {
    ...
    GlobalPoint rt(r(0, 0) - disp(0, 0), r(1, 0) - disp(1, 0),
                   r(2, 0) - disp(2, 0));
    DTChamberId mId(id, station + 1, sector + 1);
    const DTChamber *mChamber =
        static_cast<const DTChamber *>(pDD->idToDet(mId));
...
void DTSurvey::FillWheelInfo() {
  ...
  wheeltowheel >> zOffset >> deltax >> deltay >> deltaz >> alpha >> beta >>
      gamma;
  wheeltowheel.close();
...
std::ostream &operator<<(std::ostream &flux, const DTSurvey &obj) {
  ...
  for (int stationCounter = 0; stationCounter < 4; stationCounter++) {
    for (int sectorCounter = 0; sectorCounter < 14; sectorCounter++) {
      if (obj.getChamber(stationCounter, sectorCounter)->getNumberPoints() >
          2) {
        const DTSurveyChamber *m_chamber =
            obj.getChamber(stationCounter, sectorCounter);
        flux << *m_chamber;
      }
    }
  }
