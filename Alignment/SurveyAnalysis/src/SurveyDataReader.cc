...
void SurveyDataReader::readFile(const std::string &textFileName,
  ...
  if (!myfile.is_open())
    throw cms::Exception("FileAccess")
  ...
  while (!myfile.eof() && myfile.good()) {
    ...
    if (fileType == "TID") {
      ...
      m_detId = tTopo->tidDetId(d_inputs[2], d_inputs[3], d_inputs[4],
                                d_inputs[5], d_inputs[6], ster);
    } else if (fileType == "TIB") {

      m_detId = tTopo->tibDetId(d_inputs[2], d_inputs[3], d_inputs[4],
                                d_inputs[5], d_inputs[6], ster);
    }

    if (abs(int(m_detId) - int(d_inputs[1])) >
        2) { // Check DetId calculation ...
      std::cout << "ERROR : DetId - detector position mismatch! Found "
                << nErrors << std::endl;
      nErrors++;
    }
