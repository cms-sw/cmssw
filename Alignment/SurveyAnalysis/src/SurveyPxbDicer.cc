...
std::string SurveyPxbDicer::doDice(const fidpoint_t &fidpointvec,
                                   const idPair_t &id, const bool rotate) {
  ...
  oss << id.first << " " << sign * ranGauss(p0.x(), sigma_x) << " "
      << -sign * ranGauss(p0.y(), sigma_y) << " "
      << sign * ranGauss(p1.x(), sigma_x) << " "
      << -sign * ranGauss(p1.y(), sigma_y) << " " << id.second << " "
      << sign * ranGauss(p2.x(), sigma_x) << " "
      << -sign * ranGauss(p2.y(), sigma_y) << " "
      << sign * ranGauss(p3.x(), sigma_x) << " "
      << -sign * ranGauss(p3.y(), sigma_y) << " " << sigma_x << " " << sigma_y
      << " " << rotate << " # MC-truth:"
      << " a0-a3: " << a0 << " " << a1 << " " << a2 << " " << a3
      << " S: " << scale << " phi: " << phi << " x0: " << fidpointvec[0].x()
      << " " << fidpointvec[0].y() << " x1: " << fidpointvec[1].x() << " "
      << fidpointvec[1].y() << " x2: " << fidpointvec[2].x() << " "
      << fidpointvec[2].y() << " x3: " << fidpointvec[3].x() << " "
      << fidpointvec[3].y() << std::endl;
  return oss.str();
}

void SurveyPxbDicer::doDice(const fidpoint_t &fidpointvec, const idPair_t &id,
                            std::ofstream &outfile, const bool rotate) {
...
SurveyPxbDicer::getParByName(const std::string &name, const std::string &par,
                             const std::vector<edm::ParameterSet> &pars) {
  std::vector<edm::ParameterSet>::const_iterator it;
  it = std::find_if(pars.begin(), pars.end(), [&name](auto const &c) {
    return findParByName()(name, c);
  });
  if (it == pars.end()) {
