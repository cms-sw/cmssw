...
void SurveyPxbImageLocalFit::doFit(const fidpoint_t &fidpointvec) {
...
#ifdef DEBUG
  std::cout << "gamma: " << gamma << " gamma left: " << gammal
  ...
  if (ifail !=
      0) { // TODO: ifail Pruefung auf message logger ausgeben statt cout
  ...
  chi2_ = ROOT::Math::Dot(y, W * y) -
          ROOT::Math::Dot(a, ROOT::Math::Transpose(A) * W * y);
#ifdef DEBUG
  std::cout << "a: " << a << " S= " << sqrt(a[2] * a[2] + a[3] * a[3])
            << " phi= " << atan(a[3] / a[2]) << " chi2= " << chi2_ << std::endl;
  ...
  localDerivsMatrix_(0, 2) =
      +fidpointvec[0].x() + cosg * fidpoints_[0].x() - sing * fidpoints_[0].y();
  localDerivsMatrix_(0, 3) =
      +fidpointvec[0].y() + cosg * fidpoints_[0].y() + sing * fidpoints_[0].x();
  localDerivsMatrix_(1, 2) =
      +fidpointvec[0].y() + cosg * fidpoints_[0].y() + sing * fidpoints_[0].x();
  localDerivsMatrix_(1, 3) =
      -fidpointvec[0].x() - cosg * fidpoints_[0].x() + sing * fidpoints_[0].y();
  localDerivsMatrix_(2, 2) =
      +fidpointvec[1].x() + cosg * fidpoints_[1].x() - sing * fidpoints_[1].y();
  localDerivsMatrix_(2, 3) =
      +fidpointvec[1].y() + cosg * fidpoints_[1].y() + sing * fidpoints_[1].x();
  localDerivsMatrix_(3, 2) =
      +fidpointvec[1].y() + cosg * fidpoints_[1].y() + sing * fidpoints_[1].x();
  localDerivsMatrix_(3, 3) =
      -fidpointvec[1].x() - cosg * fidpoints_[1].x() + sing * fidpoints_[1].y();
  localDerivsMatrix_(4, 2) =
      +fidpointvec[2].x() + cosg * fidpoints_[2].x() - sing * fidpoints_[2].y();
  localDerivsMatrix_(4, 3) =
      +fidpointvec[2].y() + cosg * fidpoints_[2].y() + sing * fidpoints_[2].x();
  localDerivsMatrix_(5, 2) =
      +fidpointvec[2].y() + cosg * fidpoints_[2].y() + sing * fidpoints_[2].x();
  localDerivsMatrix_(5, 3) =
      -fidpointvec[2].x() - cosg * fidpoints_[2].x() + sing * fidpoints_[2].y();
  localDerivsMatrix_(6, 2) =
      +fidpointvec[3].x() + cosg * fidpoints_[3].x() - sing * fidpoints_[3].y();
  localDerivsMatrix_(6, 3) =
      +fidpointvec[3].y() + cosg * fidpoints_[3].y() + sing * fidpoints_[3].x();
  localDerivsMatrix_(7, 2) =
      +fidpointvec[3].y() + cosg * fidpoints_[3].y() + sing * fidpoints_[3].x();
  localDerivsMatrix_(7, 3) =
      -fidpointvec[3].x() - cosg * fidpoints_[3].x() + sing * fidpoints_[3].y();

  // Fill vector with global derivatives and labels (8x3)
  globalDerivsMatrix_(0, 0) = +a(2);
  globalDerivsMatrix_(0, 1) = +a(3);
  globalDerivsMatrix_(0, 2) =
      +cosg * (a(3) * fidpoints_[0].x() - a(2) * fidpoints_[0].y()) -
      sing * (a(2) * fidpoints_[0].x() + a(3) * fidpoints_[0].y());
  globalDerivsMatrix_(1, 0) = -a(3);
  globalDerivsMatrix_(1, 1) = +a(2);
  globalDerivsMatrix_(1, 2) =
      +cosg * (a(2) * fidpoints_[0].x() + a(3) * fidpoints_[0].y()) -
      sing * (a(2) * fidpoints_[0].y() - a(3) * fidpoints_[0].x());
  globalDerivsMatrix_(2, 0) = +a(2);
  globalDerivsMatrix_(2, 1) = +a(3);
  globalDerivsMatrix_(2, 2) =
      +cosg * (a(3) * fidpoints_[1].x() - a(2) * fidpoints_[1].y()) -
      sing * (a(2) * fidpoints_[1].x() + a(3) * fidpoints_[1].y());
  globalDerivsMatrix_(3, 0) = -a(3);
  globalDerivsMatrix_(3, 1) = +a(2);
  globalDerivsMatrix_(3, 2) =
      +cosg * (a(2) * fidpoints_[1].x() + a(3) * fidpoints_[1].y()) -
      sing * (a(2) * fidpoints_[1].y() - a(3) * fidpoints_[1].x());

  globalDerivsMatrix_(4, 0) = +a(2);
  globalDerivsMatrix_(4, 1) = +a(3);
  globalDerivsMatrix_(4, 2) =
      +cosg * (a(3) * fidpoints_[2].x() - a(2) * fidpoints_[2].y()) -
      sing * (a(2) * fidpoints_[2].x() + a(3) * fidpoints_[2].y());
  globalDerivsMatrix_(5, 0) = -a(3);
  globalDerivsMatrix_(5, 1) = +a(2);
  globalDerivsMatrix_(5, 2) =
      +cosg * (a(2) * fidpoints_[2].x() + a(3) * fidpoints_[2].y()) -
      sing * (a(2) * fidpoints_[2].y() - a(3) * fidpoints_[2].x());
  globalDerivsMatrix_(6, 0) = +a(2);
  globalDerivsMatrix_(6, 1) = +a(3);
  globalDerivsMatrix_(6, 2) =
      +cosg * (a(3) * fidpoints_[3].x() - a(2) * fidpoints_[3].y()) -
      sing * (a(2) * fidpoints_[3].x() + a(3) * fidpoints_[3].y());
  globalDerivsMatrix_(7, 0) = -a(3);
  globalDerivsMatrix_(7, 1) = +a(2);
  globalDerivsMatrix_(7, 2) =
      +cosg * (a(2) * fidpoints_[3].x() + a(3) * fidpoints_[3].y()) -
      sing * (a(2) * fidpoints_[3].y() - a(3) * fidpoints_[3].x());

  fitValidFlag_ = true;
}

void SurveyPxbImageLocalFit::doFit(value_t u1, value_t v1, value_t g1,
                                   value_t u2, value_t v2, value_t g2) {
...
SurveyPxbImageLocalFit::getLocalParameters() {
  if (!fitValidFlag_)
    throw std::logic_error(
        "SurveyPxbImageLocalFit::getLocalParameters(): Fit is not valid. Call "
        "doFit(...) before calling this function.");
  return a_;
}
...
SurveyPxbImageLocalFit::value_t SurveyPxbImageLocalFit::getChi2() {
  if (!fitValidFlag_)
    throw std::logic_error(
        "SurveyPxbImageLocalFit::getChi2(): Fit is not valid. Call doFit(...) "
