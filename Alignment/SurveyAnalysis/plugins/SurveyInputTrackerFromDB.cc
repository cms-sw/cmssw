...
void SurveyInputTrackerFromDB::analyze(const edm::Event &,
  ...
  if (theFirstEvent) {
    ...
    //  std::cout << "***************ENTERING INITIALIZATION******************"
    //  << std::endl;
    ...
    AlignmentErrorsExtended *myAlignmentErrorsExtended =
        detector()->alignmentErrors();
    ...
    poolDbService->writeOne<Alignments>(
        myAlignments, poolDbService->beginOfTime(), "TrackerAlignmentRcd");
    poolDbService->writeOne<AlignmentErrorsExtended>(
        myAlignmentErrorsExtended, poolDbService->beginOfTime(),
...
void SurveyInputTrackerFromDB::addSurveyInfo(Alignable *ali) {
  ...
  if (it != uIdMap.end()) {
    ...
    ali->move(gvector);
    // rotation
    Basic3DVector<align::Scalar> rot_aa(parameters[3], parameters[4],
                                        parameters[5]);
