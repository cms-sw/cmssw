...
DTSurveyConvert::DTSurveyConvert(const edm::ParameterSet &iConfig) {
  ...
  nameChambers_m2 =
      iConfig.getUntrackedParameter<std::string>("nameChambers_m2");
  nameChambers_m1 =
      iConfig.getUntrackedParameter<std::string>("nameChambers_m1");
  nameChambers_0 = iConfig.getUntrackedParameter<std::string>("nameChambers_0");
  nameChambers_p1 =
      iConfig.getUntrackedParameter<std::string>("nameChambers_p1");
  nameChambers_p2 =
      iConfig.getUntrackedParameter<std::string>("nameChambers_p2");
...
void DTSurveyConvert::analyze(const edm::Event &,
  ...
  if (WriteToDB == true) {
    ...
    while (!inFile.eof()) {
      ...
      inFile >> dx >> sigma_dx >> dy >> sigma_dy >> dz >> sigma_dz >> alpha >>
          sigma_alpha >> beta >> sigma_beta >> gamma >> sigma_gamma;
      if (inFile.eof())
        break;
