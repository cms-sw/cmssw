...
SurveyInputCSCfromPins::SurveyInputCSCfromPins(const edm::ParameterSet &cfg)
    : m_pinPositions(cfg.getParameter<std::string>("pinPositions")),
      ...
      m_missingErrorTranslation(
          cfg.getParameter<double>("missingErrorTranslation")),
...
void SurveyInputCSCfromPins::orient(align::LocalVector LC1,
                                    align::LocalVector LC2, double a, double b,
                                    double &T, double &dx, double &dy,
                                    double &dz, double &PhX, double &PhZ) {
  double cosPhX, sinPhX, cosPhZ, sinPhZ;

  LocalPoint LP1(LC1.x(), LC1.y() + a, LC1.z() + b);
  LocalPoint LP2(LC2.x(), LC2.y() - a, LC2.z() + b);

  LocalPoint P((LP1.x() - LP2.x()) / (2. * a), (LP1.y() - LP2.y()) / (2. * a),
               (LP1.z() - LP2.z()) / (2. * a));
  LocalPoint Pp((LP1.x() + LP2.x()) / (2.), (LP1.y() + LP2.y()) / (2.),
                (LP1.z() + LP2.z()) / (2.));
...
void SurveyInputCSCfromPins::errors(double a, double b, bool missing1,
                                    bool missing2, double &dx_dx, double &dy_dy,
                                    double &dz_dz, double &phix_phix,
  ...
  for (int i = 0; i < trials; i++) {
    ...
    if (missing1) {
      ...
      LC1 =
          LocalVector(gRandom->Gaus(0., m_errorX), gRandom->Gaus(0., m_errorY),
                      gRandom->Gaus(0., m_errorZ));
    }
...
void SurveyInputCSCfromPins::analyze(const edm::Event &,
  ...
  if (theFirstEvent) {
    ...
    Double_t x1, y1, z1, x2, y2, z2, a, b, tot = 0.0, maxErr = 0.0, h, s1, dx,
                                           dy, dz, PhX, PhZ, T;
    ...
    AlignableMuon *theAlignableMuon =
        new AlignableMuon(&(*dtGeometry), &(*cscGeometry));
    AlignableNavigator *theAlignableNavigator =
        new AlignableNavigator(theAlignableMuon);
    ...
    while (in.good()) {
      ...
      in >> ID1 >> ID2 >> ID3 >> ID4 >> ID5 >> x1 >> y1 >> z1 >> x2 >> y2 >>
          z2 >> a >> b;

      if (fabs(x1 - 1000.) < 1e5 && fabs(y1 - 1000.) < 1e5 &&
          fabs(z1 - 1000.) < 1e5) {
        missing1 = true;
        x1 = x2;
        y1 = y2;
      ...
      if (fabs(x2 - 1000.) < 1e5 && fabs(y2 - 1000.) < 1e5 &&
          fabs(z2 - 1000.) < 1e5) {
        missing2 = true;
        x2 = x1;
        y2 = y1;
      ...
      // We cannot use chamber ID (when ID5=0), because AlignableNavigator gives
      // the error (aliDet and aliDetUnit are undefined for chambers)

      Alignable *theAlignable1 =
          theAlignableNavigator->alignableFromDetId(layerID);
      ...
      GlobalPoint PG1 = chamberAli->surface().toGlobal(
          LocalPoint(LC1.x(), LC1.y() + a, LC1.z() + b));
      chamberAli->surface().toGlobal(
          LocalPoint(LC2.x(), LC2.y() - a, LC2.z() + b));
      ...
      errors(a, b, missing1, missing2, dx_dx, dy_dy, dz_dz, phix_phix,
             phiz_phiz, dy_phix);
      ...
      if (m_verbose) {
        ...
        edm::LogInfo("SurveyInputCSCfromPins")
            << " survey information = " << chamberAli->survey() << "  \n";
...
void SurveyInputCSCfromPins::fillAllRecords(Alignable *ali) {
  if (ali->survey() == nullptr) {
    ...
    AlignableCSCChamber *ali_AlignableCSCChamber =
        dynamic_cast<AlignableCSCChamber *>(ali);
    AlignableCSCStation *ali_AlignableCSCStation =
        dynamic_cast<AlignableCSCStation *>(ali);

    if (ali_AlignableCSCChamber != nullptr) {
      CSCDetId detid(ali->geomDetId());
      if (abs(detid.station()) == 1 &&
          (detid.ring() == 1 || detid.ring() == 4)) {
        align::ErrorMatrix error = ROOT::Math::SMatrixIdentity();
        ...
        double dx_dx, dy_dy, dz_dz, phix_phix, phiz_phiz, dy_phix;
        errors(a, b, true, true, dx_dx, dy_dy, dz_dz, phix_phix, phiz_phiz,
               dy_phix);
        align::ErrorMatrix error = ROOT::Math::SMatrixIdentity();
        error(0, 0) = dx_dx;
        error(1, 1) = dy_dy;
