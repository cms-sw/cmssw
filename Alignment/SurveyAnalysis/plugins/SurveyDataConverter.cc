...
void SurveyDataConverter::applyFineSurveyInfo(TrackerAlignment &tr_align,
  ...
  for (MapType::const_iterator it = map.begin(); it != map.end(); it++) {
    ...
    align::RotationType bRotation(
        align_params[6], align_params[9], align_params[3], align_params[7],
        align_params[10], align_params[4], align_params[8], align_params[11],
        align_params[5]);

    align::RotationType fRotation(
        align_params[15], align_params[18], align_params[12], align_params[16],
        align_params[19], align_params[13], align_params[17], align_params[20],
        align_params[14]);

    // Use "false" for debugging only
    tr_align.moveAlignableTIBTIDs((it)->first, translations, bRotation,
                                  fRotation, true);
  }
}
...
void SurveyDataConverter::applyAPEs(TrackerAlignment &tr_align) {
  ...
  std::vector<double> TIBerrors =
      theParameterSet.getParameter<std::vector<double>>("TIBerrors");
  std::vector<double> TOBerrors =
      theParameterSet.getParameter<std::vector<double>>("TOBerrors");
  std::vector<double> TIDerrors =
      theParameterSet.getParameter<std::vector<double>>("TIDerrors");
  std::vector<double> TECerrors =
      theParameterSet.getParameter<std::vector<double>>("TECerrors");

  if (TIBerrors.size() < 3 || TOBerrors.size() < 4 || TIDerrors.size() < 4 ||
      TECerrors.size() < 4) {
    std::cout << "APE info not valid : please check test/run-converter.cfg"
              << std::endl;
    return;
  }
  ...
  for (iter = theTIBhb.begin(); iter != theTIBhb.end(); ++iter) {
    theModifier.addAlignmentPositionErrorLocal(
        *iter, TIBerrors.at(0), TIBerrors.at(0), TIBerrors.at(0));
  }
  const align::Alignables &theTIBlayers =
      theAlignableTracker->innerBarrelLayers();
  for (iter = theTIBlayers.begin(); iter != theTIBlayers.end(); ++iter) {
    theModifier.addAlignmentPositionErrorLocal(
        *iter, TIBerrors.at(1), TIBerrors.at(1), TIBerrors.at(1));
  }
  const align::Alignables &theTIBgd =
      theAlignableTracker->innerBarrelGeomDets();
  for (iter = theTIBgd.begin(); iter != theTIBgd.end(); ++iter) {
    theModifier.addAlignmentPositionErrorLocal(
        *iter, TIBerrors.at(2), TIBerrors.at(2), TIBerrors.at(2));
  }
  ...
  for (iter = theTOBhb.begin(); iter != theTOBhb.end(); ++iter) {
    theModifier.addAlignmentPositionErrorLocal(
        *iter, TOBerrors.at(0), TOBerrors.at(0), TOBerrors.at(1));
  }
  const align::Alignables &theTOBrods = theAlignableTracker->outerBarrelRods();
  for (iter = theTOBrods.begin(); iter != theTOBrods.end(); ++iter) {
    theModifier.addAlignmentPositionErrorLocal(
        *iter, TOBerrors.at(2), TOBerrors.at(2), TOBerrors.at(2));
  }
  const align::Alignables &theTOBgd =
      theAlignableTracker->outerBarrelGeomDets();
  for (iter = theTOBgd.begin(); iter != theTOBgd.end(); ++iter) {
    theModifier.addAlignmentPositionErrorLocal(
        *iter, TOBerrors.at(3), TOBerrors.at(3), TOBerrors.at(3));
  }
