...
void TestIdealGeometry::analyze(const edm::Event &iEvent,
  ...
  //	iGeomDet != alignments->m_align.end(); iGeomDet++ )
  {
    ...
    if (countDet == 0) { // Enter only once for double-sided dets
      ...
      if (((*iGeomDet)->geographicalId()).subdetId() ==
        ...
        std::vector<unsigned int> theString =
            tTopo->tibStringInfo((*iGeomDet)->geographicalId());
      ...
      } else if (((*iGeomDet)->geographicalId()).subdetId() ==
                 int(StripSubdetector::TID)) {
        ...
        std::vector<unsigned int> theModule =
            tTopo->tidModuleInfo((*iGeomDet)->geographicalId());
      ...
      for (MapTypeOr::const_iterator it = theSurveyMap.begin();
        ...
        if (locPos[0] == int(comparisonVect[0]) &&
          ...
          cout << "X pos : TRACKER_GEOM = " << std::fixed
               << std::setprecision(2) << (*iGeomDet)->position().x()
               << " / IDEAL RICCARDO = " << align_params[0] << endl;
          dy_ = (*iGeomDet)->position().y() - align_params[1];
          cout << "Y pos : TRACKER_GEOM = " << std::fixed
               << std::setprecision(2) << (*iGeomDet)->position().y()
               << " / IDEAL RICCARDO = " << align_params[1] << endl;
          dz_ = (*iGeomDet)->position().z() - align_params[2];
          cout << "Z pos : TRACKER_GEOM = " << std::fixed
               << std::setprecision(2) << (*iGeomDet)->position().z()
               << " / IDEAL RICCARDO = " << align_params[2] << endl;
          dtx_ = (*iGeomDet)->rotation().xx() - align_params[6];
          cout << "Trans vect X : TRACKER_GEOM = " << std::fixed
               << std::setprecision(3) << (*iGeomDet)->rotation().xx()
               << " / IDEAL RICCARDO = " << align_params[6] << endl;
          dty_ = (*iGeomDet)->rotation().xy() - align_params[7];
          cout << "Trans vect Y : TRACKER_GEOM = " << std::fixed
               << std::setprecision(3) << (*iGeomDet)->rotation().xy()
               << " / IDEAL RICCARDO = " << align_params[7] << endl;
          dtz_ = (*iGeomDet)->rotation().xz() - align_params[8];
          cout << "Trans vect Z : TRACKER_GEOM = " << std::fixed
               << std::setprecision(3) << (*iGeomDet)->rotation().xz()
               << " / IDEAL RICCARDO = " << align_params[8] << endl;
          dkx_ = (*iGeomDet)->rotation().yx() - align_params[9];
          cout << "Long vect X : TRACKER_GEOM = " << std::fixed
               << std::setprecision(3) << (*iGeomDet)->rotation().yx()
               << " / IDEAL RICCARDO = " << align_params[9] << endl;
          dky_ = (*iGeomDet)->rotation().yy() - align_params[10];
          cout << "Long vect Y : TRACKER_GEOM = " << std::fixed
               << std::setprecision(3) << (*iGeomDet)->rotation().yy()
               << " / IDEAL RICCARDO = " << align_params[10] << endl;
          dkz_ = (*iGeomDet)->rotation().yz() - align_params[11];
          cout << "Long vect Z : TRACKER_GEOM = " << std::fixed
               << std::setprecision(3) << (*iGeomDet)->rotation().yz()
               << " / IDEAL RICCARDO = " << align_params[11] << endl;
          dnx_ = (*iGeomDet)->rotation().zx() - align_params[3];
          cout << "Norm vect X : TRACKER_GEOM = " << std::fixed
               << std::setprecision(3) << (*iGeomDet)->rotation().zx()
               << " / IDEAL RICCARDO = " << align_params[3] << endl;
          dny_ = (*iGeomDet)->rotation().zy() - align_params[4];
          cout << "Norm vect Y : TRACKER_GEOM = " << std::fixed
               << std::setprecision(3) << (*iGeomDet)->rotation().zy()
               << " / IDEAL RICCARDO = " << align_params[4] << endl;
          dnz_ = (*iGeomDet)->rotation().zz() - align_params[5];
          cout << "Norm vect Z : TRACKER_GEOM = " << std::fixed
               << std::setprecision(3) << (*iGeomDet)->rotation().zz()
