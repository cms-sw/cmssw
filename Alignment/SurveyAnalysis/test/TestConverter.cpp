...
void TestConverter::analyze(const edm::Event &iEvent,
    ...
    */
    ...
    for (MapTypeOr::const_iterator it = theSurveyMap.begin();
      ...
      if (thecomparison == 0) {
        ...
        for (std::vector<AlignTransformErrorExtended>::const_iterator it =
                 alignErrors.begin();
          ...
          if ((*it).rawId() == (*iGeomDet).rawId()) {
            ...
            edm::LogInfo("TrackerAlignment")
                << "DetId = " << Id_ << "\n"
                << "DetId decodified = " << locPos[0] << " " << locPos[2] << " "
                << locPos[3] << " " << locPos[4] << " " << locPos[5] << " "
                << locPos[6] << "\n"
                << "X pos : TRACKER_MOVED = " << std::fixed
                << std::setprecision(2) << (*iGeomDet).translation().x()
                << " / SURVEY RICCARDO = " << align_params[15] << "\n"
                << "Y pos : TRACKER_MOVED = " << std::fixed
                << std::setprecision(2) << (*iGeomDet).translation().y()
                << " / SURVEY RICCARDO = " << align_params[16] << "\n"
                << "Z pos : TRACKER_MOVED = " << std::fixed
