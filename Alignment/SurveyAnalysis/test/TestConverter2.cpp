...
void TestConverter2::analyze(const edm::Event &iEvent,
  ...
  for (auto iGeomDet = alignments->m_align.begin();
    ...
    for (auto iDet = trackerGeometry->dets().begin();
      ...
      // std::cout << (*iDet)->geographicalId().rawId() << " " <<
      // (*iGeomDet).rawId() << std::endl;
        ...
        for (auto it = alignErrors.begin(); it != alignErrors.end(); it++) {
          ...
          if ((*it).rawId() == (*iGeomDet).rawId()) {
            ...
            if (thisId.subdetId() == int(StripSubdetector::TIB)) {
              ...
              std::vector<unsigned int> theString =
                  tTopo->tibStringInfo(thisId);
              fwbw_ = theString[0];
              ...
              std::vector<unsigned int> theModule =
                  tTopo->tidModuleInfo(thisId);
              ...
              std::cout << "WARNING!!! this DetId (" << thisId.rawId()
                        << ") does not belong to SiStrip tracker" << std::endl;
              break;
            }
            ...
            align::RotationType rotation(
                fromAngles.xx(), fromAngles.xy(), fromAngles.xz(),
            ...
            cout << "DetId decodified = " << subdid_ << " " << fwbw_ << " "
                 << frontback_ << " " << layerdisk_ << " " << stringrod_ << " "
                 << petal_ << " " << module_ << endl;
            cout << "X pos : TRACKER_MOVED = " << std::fixed
                 << std::setprecision(2) << (*iGeomDet).translation().x()
                 << " / TRACKER_ORIGINAL = " << (*iDet)->position().x() << endl;
            cout << "Y pos : TRACKER_MOVED = " << std::fixed
                 << std::setprecision(2) << (*iGeomDet).translation().y()
                 << " / TRACKER_ORIGINAL = " << (*iDet)->position().y() << endl;
            cout << "Z pos : TRACKER_MOVED = " << std::fixed
                 << std::setprecision(2) << (*iGeomDet).translation().z()
            ...
            cout << "Trans vect X : TRACKER_MOVED = " << std::fixed
                 << std::setprecision(5) << rotation.xx()
                 << " / TRACKER_ORIGINAL = " << (*iDet)->rotation().xx()
                 << endl;
            cout << "Trans vect Y : TRACKER_MOVED = " << std::fixed
                 << std::setprecision(5) << rotation.xy()
                 << " / TRACKER_ORIGINAL = " << (*iDet)->rotation().xy()
                 << endl;
            cout << "Trans vect Z : TRACKER_MOVED = " << std::fixed
                 << std::setprecision(5) << rotation.xz()
                 << " / TRACKER_ORIGINAL = " << (*iDet)->rotation().xz()
                 << endl;
            cout << "Long vect X : TRACKER_MOVED = " << std::fixed
                 << std::setprecision(5) << rotation.yx()
                 << " / TRACKER_ORIGINAL = " << (*iDet)->rotation().yx()
                 << endl;
            cout << "Long vect Y : TRACKER_MOVED = " << std::fixed
                 << std::setprecision(5) << rotation.yy()
                 << " / TRACKER_ORIGINAL = " << (*iDet)->rotation().yy()
                 << endl;
            cout << "Long vect Z : TRACKER_MOVED = " << std::fixed
                 << std::setprecision(5) << rotation.yz()
                 << " / TRACKER_ORIGINAL = " << (*iDet)->rotation().yz()
                 << endl;
            cout << "Norm vect X : TRACKER_MOVED = " << std::fixed
                 << std::setprecision(5) << rotation.zx()
                 << " / TRACKER_ORIGINAL = " << (*iDet)->rotation().zx()
                 << endl;
            cout << "Norm vect Y : TRACKER_MOVED = " << std::fixed
                 << std::setprecision(5) << rotation.zy()
                 << " / TRACKER_ORIGINAL = " << (*iDet)->rotation().zy()
                 << endl;
            cout << "Norm vect Z : TRACKER_MOVED = " << std::fixed
                 << std::setprecision(5) << rotation.zz()
                 << " / TRACKER_ORIGINAL = " << (*iDet)->rotation().zz()
                 << endl;
          }
        }
      }
