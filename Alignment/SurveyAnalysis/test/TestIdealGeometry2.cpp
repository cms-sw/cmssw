...
TestIdealGeometry2::TestIdealGeometry2(const edm::ParameterSet &iConfig)
  ...
  std::string fileName = theParameterSet.getUntrackedParameter<std::string>(
      "fileName", "testideal.root");
...
void TestIdealGeometry2::analyze(const edm::Event &iEvent,
  ...
  //
  edm::ParameterSet textFiles =
      theParameterSet.getParameter<edm::ParameterSet>("textFileNames");
  std::string textFileNames[NFILES];
  std::string fileType[NFILES];
  textFileNames[0] =
      textFiles.getUntrackedParameter<std::string>("forTIB", "NONE");
  fileType[0] = "TIB";
  textFileNames[1] =
      textFiles.getUntrackedParameter<std::string>("forTID", "NONE");
  fileType[1] = "TID";
  ...
  for (int ii = 0; ii < NFILES; ii++) {
    if (textFileNames[ii] == "NONE")
      throw cms::Exception("BadConfig")
          << fileType[ii] << " input file not found in configuration";
  ...
  // Now loop on detector units, and store difference position and orientation
  // w.r.t. survey

  for (std::vector<AlignTransform>::const_iterator iGeomDet =
    ...
    if (countDet == 0) { // Store only r-phi for double-sided modules
      ...
      for (MapTypeOr::const_iterator it = theSurveyMap.begin();
           it != theSurveyMap.end(); it++) {
        ...
        if (locPos[0] == int(comparisonVect[0]) &&
          ...
          align::RotationType rotation(rot.xx(), rot.xy(), rot.xz(), rot.yx(),
                                       rot.yy(), rot.yz(), rot.zx(), rot.zy(),
                                       rot.zz());

          Id_ = (*iGeomDet).rawId();
          // cout << "DetId = " << Id_ << " " << endl;
          // cout << "DetId decodified = " << comparisonVect[0] << " " <<
          // comparisonVect[1] << " " << comparisonVect[2] << " " <<
          // comparisonVect[3] << " " << comparisonVect[4] << " " <<
          // comparisonVect[5] << endl;
          dx_ = (*iGeomDet).translation().x() - align_params[0];
          // cout << "X pos : TRACKER_ALIGN = " << std::fixed <<
          // std::setprecision(2) << (*iGeomDet).translation().x() << " / IDEAL
          // RICCARDO = " << align_params[0] << endl;
          dy_ = (*iGeomDet).translation().y() - align_params[1];
          // cout << "Y pos : TRACKER_ALIGN = " << std::fixed <<
          // std::setprecision(2) << (*iGeomDet).translation().y() << " / IDEAL
          // RICCARDO = " << align_params[1] << endl;
          dz_ = (*iGeomDet).translation().z() - align_params[2];
          // cout << "Z pos : TRACKER_ALIGN = " << std::fixed <<
          // std::setprecision(2) << (*iGeomDet).translation().z() << " / IDEAL
          // RICCARDO = " << align_params[2] << endl; cout << "SPATIAL DISTANCE
          // = " << std::fixed << std::setprecision(3) <<
          // sqrt(pow(dx_,2)+pow(dy_,2)+pow(dz_,2)) << endl;
          dtx_ = rotation.xx() - align_params[6];
          // cout << "Trans vect X : TRACKER_ALIGN = " << std::fixed <<
          // std::setprecision(3) << rotation.xx() << " / IDEAL RICCARDO = " <<
          // align_params[6] << endl;
          dty_ = rotation.xy() - align_params[7];
          // cout << "Trans vect Y : TRACKER_ALIGN = " << std::fixed <<
          // std::setprecision(3) << rotation.xy() << " / IDEAL RICCARDO = " <<
          // align_params[7] << endl;
          dtz_ = rotation.xz() - align_params[8];
          // cout << "Trans vect Z : TRACKER_ALIGN = " << std::fixed <<
          // std::setprecision(3) << rotation.xz() << " / IDEAL RICCARDO = " <<
          // align_params[8] << endl;
          dkx_ = rotation.yx() - align_params[9];
          // cout << "Long vect X : TRACKER_ALIGN = " << std::fixed <<
          // std::setprecision(3) << rotation.yx() << " / IDEAL RICCARDO = " <<
          // align_params[9] << endl;
          dky_ = rotation.yy() - align_params[10];
          // cout << "Long vect Y : TRACKER_ALIGN = " << std::fixed <<
          // std::setprecision(3) << rotation.yy() << " / IDEAL RICCARDO = " <<
          // align_params[10] << endl;
          dkz_ = rotation.yz() - align_params[11];
          // cout << "Long vect Z : TRACKER_ALIGN = " << std::fixed <<
          // std::setprecision(3) << rotation.yz() << " / IDEAL RICCARDO = " <<
          // align_params[11] << endl;
          dnx_ = rotation.zx() - align_params[3];
          // cout << "Norm vect X : TRACKER_ALIGN = " << std::fixed <<
          // std::setprecision(3) << rotation.zx() << " / IDEAL RICCARDO = " <<
          // align_params[3] << endl;
          dny_ = rotation.zy() - align_params[4];
          // cout << "Norm vect Y : TRACKER_ALIGN = " << std::fixed <<
          // std::setprecision(3) << rotation.zy() << " / IDEAL RICCARDO = " <<
          // align_params[4] << endl;
          dnz_ = rotation.zz() - align_params[5];
          // cout << "Norm vect Z : TRACKER_ALIGN = " << std::fixed <<
          // std::setprecision(3) << rotation.zz() << " / IDEAL RICCARDO = " <<
          // align_params[5] << endl;
          theTree->Fill();
        }
      }
