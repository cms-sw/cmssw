 ...
 *  Inheriting from AlignmentParameters is fake, just to attach it to an
 * Alignable, re-using the AlignmentParameters data member. (Should look for
 * another solution...)
 *
...
public:
  ...
  SurveyParameters(Alignable *, const AlgebraicVector &par,
  ...
  AlignmentParameters *clone(const AlgebraicVector &,
                             const AlgebraicSymMatrix &) const override;
  ...
  /// Derivatives not implemented.
  AlgebraicMatrix
  selectedDerivatives(const TrajectoryStateOnSurface &,
                      const AlignableDetOrUnitPtr &) const override;
};
