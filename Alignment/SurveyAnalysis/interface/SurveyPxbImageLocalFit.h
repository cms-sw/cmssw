...
public:
  ...
  SurveyPxbImageLocalFit()
      : SurveyPxbImage(), a_(nLcPars, 0), fidpoints_(nFidpoints),
        fitValidFlag_(false), derivsValidFlag_(false) {
    initFidPoints();
  };

  //! Constructor from istringstream
  SurveyPxbImageLocalFit(std::istringstream &iss)
      : SurveyPxbImage(iss), a_(nLcPars, 0), fidpoints_(nFidpoints),
        fitValidFlag_(false), derivsValidFlag_(false) {
    initFidPoints();
  };
  ...
  void doFit(const fidpoint_t &fidpointvec, const pede_label_t &label1,
             const pede_label_t &label2);
  void doFit(value_t x1, value_t y1, value_t g1, value_t x2, value_t y2,
             value_t g2);
