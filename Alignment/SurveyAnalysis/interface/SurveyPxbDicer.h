...
public:
  ...
  //! Invoke the dicer
  //! \param fidpointvec vector with fiducial points where values need to be
  //! diced for and transformed to the photo fram \param idPair pair of the id
  //! values
  std::string doDice(const fidpoint_t &fidpointvec, const idPair_t &id,
                     const bool rotate = false);
  void doDice(const fidpoint_t &fidpointvec, const idPair_t &id,
              std::ofstream &outfile, const bool rotate = false);
...
private:
  ...
  //! \param name name of the parameter to be searched for in field 'name' of
  //! the VPSet \param par selects the value, i.e. mean or sigma \param pars
  //! reference to VPSet
  value_t getParByName(const std::string &name, const std::string &par,
  ...
  coord_t transform(const coord_t &x, const value_t &a0, const value_t &a1,
                    const value_t &a2, const value_t &a3);

  //! Function object for searching for a parameter in the VPSet
  struct findParByName {
    bool operator()(const std::string &name,
                    const edm::ParameterSet &parset) const {
