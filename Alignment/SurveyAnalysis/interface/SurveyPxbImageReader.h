...
public:
  ...
  SurveyPxbImageReader(std::ifstream &infile, measurements_t &measurements,
                       SurveyPxbImage::count_t reserve = 800) {
    read(infile, measurements, reserve);
  };

  //! Constructor with filename and destination vector
  SurveyPxbImageReader(std::string filename, measurements_t &measurements,
                       SurveyPxbImage::count_t reserve = 800) {
    std::ifstream infile(filename.c_str());
    if (!infile) {
      std::cerr << "Cannot open file " << filename << " - operation aborted."
                << std::endl;
    }
    read(infile, measurements, reserve);
  };
  ...
  //! \param measurements Vector containing the measurements, previous content
  //! will be deleted \param reserve Initial size of the vector, set with
  //! vector::reserve() \return number of succesfully read entries
