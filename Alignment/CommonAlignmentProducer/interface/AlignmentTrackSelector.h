...
public:
  ...
  /// select tracks
  Tracks select(const Tracks &tracks, const edm::Event &evt,
...
private:
  ...
  Tracks basicCuts(const Tracks &tracks, const edm::Event &evt,
                   const edm::EventSetup &eSetup) const;
  /// checking hit requirements beyond simple number of valid hits
  bool detailedHitsCheck(const reco::Track *track, const edm::Event &evt,
                         const edm::EventSetup &eSetup) const;
  bool isHit2D(const TrackingRecHit &hit) const;
  /// if valid, check for minimum charge (currently only in strip), if invalid
  /// give true
  ...
  // filter tracks that do not have a min # of hits taken by the Skim&Prescale
  // workflow
  ...
  const bool countStereoHitAs2D_; // count hits on stereo components of GluedDet
                                  // for nHitMin2D_?
  ...
  const int minHitsinTIDplus_, minHitsinTIDminus_, minHitsinTECplus_,
      minHitsinTECminus_;
  ...
  const edm::InputTag
      clusterValueMapTag_; // ValueMap containing association cluster - flag
  edm::EDGetTokenT<AliClusterValueMap>
      clusterValueMapToken_; // ValueMap containing association cluster - flag
