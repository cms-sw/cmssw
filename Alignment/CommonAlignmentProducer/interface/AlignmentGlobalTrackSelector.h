...
private:
  /// returns [tracks] if there are less than theMaxCount Jets with theMinJetPt
  /// and an empty set if not
  ...
  /// matches [src] with [comp] returns collection with matching Tracks coming
  /// from [src]
