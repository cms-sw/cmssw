...
GlobalPositionRcdScan::GlobalPositionRcdScan(const edm::ParameterSet &iConfig)
    : eulerAngles_(false), alignAngles_(false), matrix_(false), firstRun_(0),
      lastRun_(0) {
...
void GlobalPositionRcdScan::analyze(const edm::Event &evt,
  ...
  if (watcher_.check(evtSetup)) {
    ...
    std::cout << "=====================================================\n"
              << "GlobalPositionRcd content starting from run " << evt.run()
              << ":" << std::endl;
    ...
    for (std::vector<AlignTransform>::const_iterator i =
      ...
      if (alignAngles_) {
        ...
        std::cout << "     alpha, beta, gamma (" << angles[0] << ", "
                  << angles[1] << ", " << angles[2] << ')' << std::endl;
      }
      if (matrix_) {
...
void GlobalPositionRcdScan::endJob() {
  std::cout << "\n=====================================================\n"
            << "=====================================================\n"
            << "Checked run range " << firstRun_ << " to " << lastRun_ << "."
            << std::endl;
}
