...
GlobalPositionRcdWrite::toMatrix(double alpha, double beta, double gamma) {
  ...
  return AlignTransform::Rotation(CLHEP::HepRep3x3(
      alignRotation.xx(), alignRotation.xy(), alignRotation.xz(),
      alignRotation.yx(), alignRotation.yy(), alignRotation.yz(),
      alignRotation.zx(), alignRotation.zy(), alignRotation.zz()));
}

void GlobalPositionRcdWrite::analyze(const edm::Event &evt,
  ...
  AlignTransform tracker(
      AlignTransform::Translation(m_tracker.getParameter<double>("x"),
      ...
      (m_useEulerAngles != true)
          ...
          : AlignTransform::EulerAngles(
                m_tracker.getParameter<double>("alpha"),
                m_tracker.getParameter<double>("beta"),
                m_tracker.getParameter<double>("gamma")),
      DetId(DetId::Tracker).rawId());

  AlignTransform muon(
      AlignTransform::Translation(m_muon.getParameter<double>("x"),
  ...
  AlignTransform ecal(
      AlignTransform::Translation(m_ecal.getParameter<double>("x"),
  ...
  AlignTransform hcal(
      AlignTransform::Translation(m_hcal.getParameter<double>("x"),
  ...
  AlignTransform calo(
      AlignTransform::Translation(m_calo.getParameter<double>("x"),
  ...
  std::cout << "\nProvided rotation angles are interpreted as "
            << ((m_useEulerAngles != true) ? "rotations around X, Y and Z"
                                           : "Euler angles")
            << ".\n"
            << std::endl;
  ...
  //       poolDbService->createNewIOV<Alignments>(&(*globalPositions),
  //       poolDbService->endOfTime(), "GlobalPositionRcd");
  //    } else {
  //       poolDbService->appendSinceTime<Alignments>(&(*globalPositions),
  //       poolDbService->currentTime(), "GlobalPositionRcd");
  //    }
