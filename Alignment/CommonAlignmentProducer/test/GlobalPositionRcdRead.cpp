...
void GlobalPositionRcdRead::analyze(const edm::Event &evt,
  ...
  std::cout << "Reading from database in GlobalPositionRcdRead::analyze..."
            << std::endl;
  ...
  for (std::vector<AlignTransform>::const_iterator i =
           globalPositionRcd->m_align.begin();
       i != globalPositionRcd->m_align.end(); ++i) {
    std::cout << "entry " << i->rawId() << " translation " << i->translation()
