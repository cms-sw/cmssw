...
// $Id: AlignmentMuonHIPTrajectorySelector.cc,v 1.4 2010/01/06 15:26:10 mussgill
// Exp $
//
//
...
AlignmentMuonHIPTrajectorySelector::AlignmentMuonHIPTrajectorySelector(
    ...
    : m_input(iConfig.getParameter<edm::InputTag>("input")),
      ...
      m_maxTrackerForwardRedChi2(
          iConfig.getParameter<double>("maxTrackerForwardRedChi2")),
      m_minTrackerDOF(iConfig.getParameter<int>("minTrackerDOF")),
      m_maxMuonResidual(iConfig.getParameter<double>("maxMuonResidual")),
      m_hists(iConfig.getParameter<bool>("hists")), m_pt(nullptr),
      m_tracker_forwardredchi2(nullptr), m_tracker_dof(nullptr) {
  ...
  if (m_hists) {
    ...
    m_tracker_forwardredchi2 =
        fs->make<TH1F>("trackerForwardRedChi2",
                       "forward-biased reduced chi2 in tracker", 100, 0., 5.);
    m_tracker_dof =
        fs->make<TH1F>("trackerDOF", "DOF in tracker", 61, -0.5, 60.5);
    m_resid_before = fs->make<TH1F>(
        "residBefore", "muon residuals before cut (cm)", 100, -20, 20);
    m_resid_after = fs->make<TH1F>(
        "residAfter", "muon residuals after cut (cm)", 100, -20, 20);
  }
}
...
void AlignmentMuonHIPTrajectorySelector::produce(
  ...
  for (TrajTrackAssociationCollection::const_iterator iPair =
           originalTrajTrackMap->begin();
       iPair != originalTrajTrackMap->end(); ++iPair) {
    if (m_hists) {
    ...
    if ((*(*iPair).val).pt() > m_minPt) {
      ...
      std::vector<TrajectoryMeasurement> measurements =
          (*(*iPair).key).measurements();

      bool has_bad_residual = false;

      double tracker_forwardchi2 = 0.;
      double tracker_dof = 0.;
      for (std::vector<TrajectoryMeasurement>::const_iterator im =
               measurements.begin();
        ...
        if (hit->isValid() && id.det() == DetId::Tracker) {
          if (hit->dimension() == 1) {
            double residual = meas.forwardPredictedState().localPosition().x() -
                              hit->localPosition().x();
            double error2 =
                meas.forwardPredictedState().localError().positionError().xx() +
            ...
            double residualx =
                meas.forwardPredictedState().localPosition().x() -
                hit->localPosition().x();
            double residualy =
                meas.forwardPredictedState().localPosition().y() -
                hit->localPosition().y();
            double errorxx2 =
                meas.forwardPredictedState().localError().positionError().xx() +
                hit->localPositionError().xx();
            double errorxy2 =
                meas.forwardPredictedState().localError().positionError().xy() +
                hit->localPositionError().xy();
            double erroryy2 =
                meas.forwardPredictedState().localError().positionError().yy() +
                hit->localPositionError().yy();

            tracker_forwardchi2 +=
                (residualx * residualx + residualy * residualy) /
                (errorxx2 + 2. * errorxy2 + erroryy2);
            tracker_dof += 2.;
          }
        } // end if a tracker hit

        if (hit->isValid() && id.det() == DetId::Muon &&
            (id.subdetId() == MuonSubdetId::DT ||
             id.subdetId() == MuonSubdetId::CSC)) {
          TrajectoryStateOnSurface tsosc = tsoscomb.combine(
              meas.forwardPredictedState(), meas.backwardPredictedState());
          double residual =
              tsosc.localPosition().x() - hit->localPosition().x();
      ...
      if (tracker_forwardredchi2 < m_maxTrackerForwardRedChi2 &&
          tracker_dof >= m_minTrackerDOF && !has_bad_residual) {
