...
GlobalTrackerMuonAlignment::GlobalTrackerMuonAlignment(
    ...
    : trackTags_(iConfig.getParameter<edm::InputTag>("tracks")),
      ...
      debug_(iConfig.getUntrackedParameter<bool>("debug")), defineFitter(true) {
#ifdef NO_FALSE_FALSE_MODE
  if (cosmicMuonMode_ == false && isolatedMuonMode_ == false) {
    throw cms::Exception("BadConfig")
        << "Muon collection not set! "
...
void GlobalTrackerMuonAlignment::endJob() {
  ...
  if (iErI != 0) {
    if (alarm)
      std::cout << " endJob Error inverse HessL matrix !!!!!!!!!!!"
                << std::endl;
  }
  // end of Local CLHEP
  ...
  std::cout << " Similated shifts[cm] " << MuGlShift(1) << " " << MuGlShift(2)
            << " " << MuGlShift(3) << " "
            << " angles[rad] " << MuGlAngle(1) << " " << MuGlAngle(2) << " "
            << MuGlAngle(3) << " " << std::endl;
  std::cout << " d    " << -d << std::endl;
  ;
  std::cout << " +-   " << sqrt(InfI(0, 0)) << " " << sqrt(InfI(1, 1)) << " "
            << sqrt(InfI(2, 2)) << std::endl;
  std::cout << " dG   " << d3(1) << " " << d3(2) << " " << d3(3) << " " << d3(4)
            << " " << d3(5) << " " << d3(6) << std::endl;
  ;
  std::cout << " +-   " << sqrt(Errd3(1, 1)) << " " << sqrt(Errd3(2, 2)) << " "
            << sqrt(Errd3(3, 3)) << " " << sqrt(Errd3(4, 4)) << " "
            << sqrt(Errd3(5, 5)) << " " << sqrt(Errd3(6, 6)) << std::endl;
  std::cout << " dL   " << dLI(1) << " " << dLI(2) << " " << dLI(3) << " "
            << dLI(4) << " " << dLI(5) << " " << dLI(6) << std::endl;
  ;
  std::cout << " +-   " << sqrt(ErrdLI(1, 1)) << " " << sqrt(ErrdLI(2, 2))
            << " " << sqrt(ErrdLI(3, 3)) << " " << sqrt(ErrdLI(4, 4)) << " "
            << sqrt(ErrdLI(5, 5)) << " " << sqrt(ErrdLI(6, 6)) << std::endl;
...
void GlobalTrackerMuonAlignment::analyzeTrackTrack(
  ...
  cosmicMuonMode_ =
      true; // true: both Cosmic and IsolatedMuon are treated with 1,2 tracks
  isolatedMuonMode_ =
      !cosmicMuonMode_; // true: only IsolatedMuon are treated with 1 track

  // if(N_event == 1)
  // GlobalPositionRcdRead1::analyze(const edm::Event& iEvent, const
  ...
  if (debug_) {
    std::cout << " ievBunch " << iEvent.bunchCrossing() << " runN "
              << (int)iEvent.run() << std::endl;
    std::cout << " N tracks s/amu gmu selmu " << tracks->size() << " "
  ...
  for (MuonCollection::const_iterator itMuon = smuons->begin();
    ...
    if (debug_) {
      ...
      std::cout << "          Out " << pointTrackOut << " lenght "
                << lenghtTrack << std::endl;
      std::cout << " point MuonIn " << pointMuonIn << std::endl;
      std::cout << "          Out " << pointMuonOut << " lenght " << lenghtMuon
                << std::endl;
      std::cout << " momeTrackIn Out " << momentumTrackIn << " "
    ...
    if (isolatedMuonMode_) { //------------------------------- Isolated Muon
      ...
      if (debug_) {
        ...
        std::cout << "  global " << Ng.x() << " " << Ng.y() << " " << Ng.z()
                  << std::endl;
        std::cout << "      lp " << Nlp.x() << " " << Nlp.y() << " " << Nlp.z()
                  << std::endl;
    ...
    if (cosmicMuonMode_) { //------------------------------- Cosmic Muon -----
      ...
      if ((distanceOutIn <= distanceInOut) & (distanceOutIn <= distanceInIn) &
        ...
        if (debug_)
          std::cout << " extrapolationT.isValid " << extrapolationT.isValid()
                    << std::endl;
        ...
        if (false & debug_) {
          // std::cout<<" ->propDir
          // "<<propagator->propagationDirection()<<std::endl;
          PropagationDirectionChooser Chooser;
          std::cout << " propDirCh "
                    << Chooser.operator()(*outerTrackTSOS.freeState(),
                                          refSurface)
                    << " Ch == along "
                    << (alongMomentum ==
                        Chooser.operator()(*outerTrackTSOS.freeState(),
                                           refSurface))
                    << std::endl;
          std::cout << " --- Nlocal " << Nl.x() << " " << Nl.y() << " "
                    << Nl.z() << " "
                    << " alfa " << int(asin(Nl.x()) * 57.29578) << std::endl;
        }
      } //                                       enf of ---- Out - In -----

      else if ((distanceInOut <= distanceInIn) &
               (distanceInOut <= distanceOutIn) &
        ...
        ConstReferenceCountingPointer<TangentPlane> tpMuGlobal(
            refSurface.tangentPlane(outerMuTSOS.globalPosition()));
        Nl = tpMuGlobal->normalVector();
        ...
        if (false & debug_) {
          ...
          PropagationDirectionChooser Chooser;
          std::cout << " propDirCh "
                    << Chooser.operator()(*innerTrackTSOS.freeState(),
                                          refSurface)
                    << " Ch == oppisite "
                    << (oppositeToMomentum ==
                        Chooser.operator()(*innerTrackTSOS.freeState(),
      ...
      else if ((distanceOutOut <= distanceInOut) &
               (distanceOutOut <= distanceInIn) &
    ...
    if (debug_) {
      std::cout << " Nx Ny Nz alfa_x " << Nl.x() << " " << Nl.y() << " "
                << Nl.z() << " " << alfa_x << std::endl;
      // std::cout<<" Rm "<<Rm<<std::endl<<" Rt "<<Rt<<std::endl<<" resR
      // "<<resR<<std::endl
    ...
    if (ierrLoc && debug_ && info) {
      std::cout << " ==== Error inverting Local covariance matrix ==== "
                << std::endl;
      continue;
    }
    double chi_Loc = ROOT::Math::Similarity(Vml, m);
    if (debug_)
      std::cout << " chi_Loc px/pz/err " << chi_Loc << " "
                << Vml(1) / sqrt(Cml(1, 1)) << std::endl;
    ...
    */
    ...
    if (debug_) { //--------------------------------- debug print ----------
      ...
      std::cout << " diag 0[ " << C0(0, 0) << " " << C0(1, 1) << " " << C0(2, 2)
                << " " << C0(3, 3) << " " << C0(4, 4) << " " << C0(5, 5) << " ]"
                << std::endl;
      std::cout << " diag e[ " << Ce(0, 0) << " " << Ce(1, 1) << " " << Ce(2, 2)
                << " " << Ce(3, 3) << " " << Ce(4, 4) << " " << Ce(5, 5) << " ]"
                << std::endl;
      std::cout << " diag 1[ " << C1(0, 0) << " " << C1(1, 1) << " " << C1(2, 2)
      ...
      std::cout << " resR " << resR.x() << " " << resR.y() << " " << resR.z()
                ...
                << std::endl;
      std::cout << " Rm-t " << (Rm - Rt).x() << " " << (Rm - Rt).y() << " "
                << (Rm - Rt).z() << " Pm-t " << (Pm - Pt).x() << " "
                << (Pm - Pt).y() << " " << (Pm - Pt).z() << std::endl;
      std::cout << " Vm   " << Vm << std::endl;
      std::cout << " +-   " << sqrt(Cm(0, 0)) << " " << sqrt(Cm(1, 1)) << " "
                << sqrt(Cm(2, 2)) << " " << sqrt(Cm(3, 3)) << " "
                << sqrt(Cm(4, 4)) << " " << sqrt(Cm(5, 5)) << std::endl;
      std::cout << " Pmuon Ptrack dP/Ptrack " << itMuon->outerTrack()->p()
                << " " << itMuon->track()->outerP() << " "
                << (itMuon->outerTrack()->p() - itMuon->track()->outerP()) /
                       itMuon->track()->outerP()
                << std::endl;
      std::cout << " cov matrix " << std::endl;
      std::cout << Cm << std::endl;
      std::cout << " diag [ " << Cm(0, 0) << " " << Cm(1, 1) << " " << Cm(2, 2)
      ...
      if (ierr) {
        if (alarm || debug_)
          std::cout << " Error inverse covariance matrix !!!!!!!!!!!"
                    << std::endl;
        continue;
      }
...
void GlobalTrackerMuonAlignment::analyzeTrackTrajectory(
  ...
  cosmicMuonMode_ =
      true; // true: both Cosmic and IsolatedMuon are treated with 1,2 tracks
  // cosmicMuonMode_ = false; // true: both Cosmic and IsolatedMuon are treated
  // with 1,2 tracks
  //-*-*-*-*-*-*-
  isolatedMuonMode_ =
      !cosmicMuonMode_; // true: only IsolatedMuon are treated with 1 track
  ...
  if (defineFitter) {
    if (debug_)
      std::cout << " ............... DEFINE FITTER ..................."
                << std::endl;
    KFUpdator *theUpdator = new KFUpdator();
    // Chi2MeasurementEstimator* theEstimator = new
    // Chi2MeasurementEstimator(30);
    ...
    if (debug_) {
      std::cout << " theTrackingGeometry.isValid() "
                << theTrackingGeometry.isValid() << std::endl;
  ...
  for (MuonCollection::const_iterator itMuon = smuons->begin();
    ...
    if (lenghtMuon < 300.)
      continue;
    if (innerMuTSOS.globalMomentum().mag() < 5. ||
        outerMuTSOS.globalMomentum().mag() < 5.)
      continue;
    if (momentumTrackIn.mag() < 15. || momentumTrackOut.mag() < 15.)
      continue;
    if (trackTT.charge() != muTT.charge())
    ...
    if (isolatedMuonMode_) { //------------------------------- Isolated Muon ---
      ...
      if (!refitTrack_)
        ...
        GlobalTrackerMuonAlignment::trackFitter(itMuon->track(), trackTT,
                                                alongMomentum, trackFittedTSOS);
      ...
      if (refitMuon_)
        GlobalTrackerMuonAlignment::muonFitter(
            itMuon->outerTrack(), muTT, oppositeToMomentum, muonFittedTSOS);
      ...
      if ((distanceOutIn <= distanceInOut) & (distanceOutIn <= distanceInIn) &
        ...
        if (!refitTrack_)
          ...
          GlobalTrackerMuonAlignment::trackFitter(
              itMuon->track(), trackTT, alongMomentum, trackFittedTSOS);
          if (trackFittedTSOS.isValid())
            extrapolationT = alongSmPr.propagate(trackFittedTSOS, refSurface);
        }
        ...
        if (!refitTrack_)
          ...
          GlobalTrackerMuonAlignment::trackFitter(
              itMuon->track(), trackTT, oppositeToMomentum, trackFittedTSOS);
          if (trackFittedTSOS.isValid())
            extrapolationT =
                oppositeSmPr.propagate(trackFittedTSOS, refSurface);
        }
        ...
        if (refitMuon_)
          GlobalTrackerMuonAlignment::muonFitter(itMuon->outerTrack(), muTT,
                                                 alongMomentum, muonFittedTSOS);
    ...
    if (refitTrack_) {
      if (!trackFittedTSOS.isValid()) {
        if (info)
          std::cout << " =================  trackFittedTSOS notValid !!!!!!!! "
                    << std::endl;
        continue;
      }
      if (debug_)
    ...
    if (refitMuon_) {
      if (!muonFittedTSOS.isValid()) {
        if (info)
          std::cout << " =================  muonFittedTSOS notValid !!!!!!!! "
                    << std::endl;
        continue;
      }
      if (debug_)
...
void GlobalTrackerMuonAlignment::gradientGlobalAlg(GlobalVector &Rt,
  ...
  if (debug_) {
    ...
    std::cout << " +/- " << 1. / sqrt(Wi(0)) << " " << 1. / sqrt(Wi(1)) << " "
              << 1. / sqrt(Wi(2)) << " " << std::endl;
...
void GlobalTrackerMuonAlignment::gradientGlobal(
  ...
  if (iEr3I != 0) {
    if (alarm)
      std::cout << " gradientGlobal error inverse  Hess matrix !!!!!!!!!!!"
                << std::endl;
  }

  if (info || debug_) {
    std::cout << " dG   " << d3I(1) << " " << d3I(2) << " " << d3I(3) << " "
              << d3I(4) << " " << d3I(5) << " " << d3I(6) << std::endl;
    ;
    std::cout << " +-   " << sqrt(Errd3I(1, 1)) << " " << sqrt(Errd3I(2, 2))
              << " " << sqrt(Errd3I(3, 3)) << " " << sqrt(Errd3I(4, 4)) << " "
              << sqrt(Errd3I(5, 5)) << " " << sqrt(Errd3I(6, 6)) << std::endl;
  }
  ...
  // d(r,p) / d shift
  std::cout << " -- An d(r,p)/d(" << ii << ") " << Jac(1, ii) << " "
            << Jac(2, ii) << " " << Jac(3, ii) << " " << Jac(4, ii) << " "
            << Jac(5, ii) << " " << Jac(6, ii) << std::endl;
  std::cout << "    Nu d(r,p)/d(" << ii << ") " << (r(1) - r0(1)) / delta << " "
            << (r(2) - r0(2)) / delta << " " << (r(3) - r0(3)) / delta << " "
            << (p(1) - p0(1)) / delta << " " << (p(2) - p0(2)) / delta << " "
            << (p(3) - p0(3)) / delta << std::endl;
  // d(r,p) / d angle
  std::cout << " -- An d(r,p)/a(" << ii << ") " << Jac(1, ii + 3) << " "
            << Jac(2, ii + 3) << " " << Jac(3, ii + 3) << " " << Jac(4, ii + 3)
            << " " << Jac(5, ii + 3) << " " << Jac(6, ii + 3) << std::endl;
...
void GlobalTrackerMuonAlignment::gradientLocal(
  ...
  */
  ...
  if (iErI != 0) {
    if (alarm)
      std::cout << " gradLocal Error inverse  Hess matrix !!!!!!!!!!!"
                << std::endl;
  }
  ...
  if (debug_) {
    // std::cout<<" dV(da3) {"<<-JacCorLoc(1,6)*0.002<<"
    // "<<-JacCorLoc(2,6)*0.002<<" "
    ...
    // std::cout<<"Jac(,a3){"<<Jac(1,6)<<" "<<Jac(2,6)<<" "<<Jac(3,6)<<"
    // "<<Jac(4,6)<<" "
    ...
    std::cout << " JacCLo(i," << i << ") = {" << JacCorLoc(1, i) << " "
              << JacCorLoc(2, i) << " " << JacCorLoc(3, i) << " "
              << JacCorLoc(4, i) << "}" << std::endl;
    std::cout << "   rotLoc " << rotLoc << std::endl;
    std::cout << " position " << R0 << std::endl;
    std::cout << " Pm,l " << Pm.T() << " " << Pml(1) / Pml(3) << " "
              << Pml(2) / Pml(3) << std::endl;
    std::cout << " Pt,l " << Pt.T() << " " << Ptl(1) / Ptl(3) << " "
  ...
  if (GNorm.z() > 0.95)
    ...
    std::cout << " Barrel  N " << GNorm << std::endl;
  std::cout << " dldc Num (i," << ii << ") " << (V1(1) - V0(1)) / delta << " "
            << (V1(2) - V0(2)) / delta << " " << (V1(3) - V0(3)) / delta << " "
            << (V1(4) - V0(4)) / delta << std::endl;
  std::cout << " dldc Ana (i," << ii << ") " << JacToLoc(1, ii) << " "
            << JacToLoc(2, ii) << " " << JacToLoc(3, ii) << " "
...
void GlobalTrackerMuonAlignment::misalignMuon(
  ...
  a(2) = 0.0000;
  a(3) = 0.0000; // zero
  // d(1)=0.0; d(2)=0.0; d(3)=0.0; a(1)=0.0020; a(2)=0.0000; a(3)=0.0000; //
  // a1=.002 d(1)=0.0; d(2)=0.0; d(3)=0.0; a(1)=0.0000; a(2)=0.0020;
  // a(3)=0.0000;
  // // a2=.002 d(1)=0.0; d(2)=0.0; d(3)=0.0; a(1)=0.0000; a(2)=0.0000;
  // a(3)=0.0020; // a3=.002 d(1)=1.0; d(2)=2.0; d(3)=3.0; a(1)=0.0000;
  // a(2)=0.0000; a(3)=0.0000; // 12300 d(1)=0.0; d(2)=0.0; d(3)=0.0;
  // a(1)=0.0000; a(2)=0.0000; a(3)=0.0020; // a3=0.002 d(1)=10.; d(2)=20.;
  // d(3)=30.; a(1)=0.0100; a(2)=0.0200; a(3)=0.0300; // huge d(1)=10.;
  // d(2)=20.; d(3)=30.; a(1)=0.2000; a(2)=0.2500; a(3)=0.3000; // huge angles
  // d(1)=0.3; d(2)=0.4; d(3)=0.5; a(1)=0.0005; a(2)=0.0006; a(3)=0.0007; //
  // 0345 0567 d(1)=0.3; d(2)=0.4; d(3)=0.5; a(1)=0.0001; a(2)=0.0002;
  // a(3)=0.0003; // 0345 0123 d(1)=0.2; d(2)=0.2; d(3)=0.2; a(1)=0.0002;
  // a(2)=0.0002; a(3)=0.0002; // 0111 0111
  ...
  // Pm = GlobalVector(CLHEP::dot(Pm0,ex), CLHEP::dot(Pm0, ey),
  // CLHEP::dot(Pm0,ez));// =T*Pm0
  ...
  if (debug_) { //    -------------  debug tranformation
    ...
    // double sl = CLHEP::dot(T*(Rt0 - Rml), T*Norm) / CLHEP::dot(Ti * Pm1,
    // Norm);
    double sl =
        CLHEP_dot(T * (Rt0 - Rml), T * Norm) / CLHEP_dot(Ti * Pm1, Norm);
    ...
    std::cout << " -- Rml-(Rm0+si*Pm0) " << (Rml - Rm0 - si * Pm0).T()
              << std::endl;
...
void GlobalTrackerMuonAlignment::misalignMuonL(
  ...
  a(2) = 0.0000;
  a(3) = 0.0000; // zero
  // d(1)=0.0000001; d(2)=0.0; d(3)=0.0; a(1)=0.0000; a(2)=0.0000; a(3)=0.0000;
  // // nonzero d(1)=0.0; d(2)=0.0; d(3)=0.0; a(1)=0.0020; a(2)=0.0000;
  // a(3)=0.0000; // a1=.002 d(1)=0.0; d(2)=0.0; d(3)=0.0; a(1)=0.0000;
  // a(2)=0.0020; a(3)=0.0000; // a2=.002 d(1)=0.0; d(2)=0.0; d(3)=0.0;
  // a(1)=0.0000; a(2)=0.0000; a(3)=0.0020; // a3=.002 d(1)=0.0; d(2)=0.0;
  // d(3)=0.0; a(1)=0.0000; a(2)=0.0000; a(3)=0.0200; // a3=.020 d(1)=1.0;
  // d(2)=2.0; d(3)=3.0; a(1)=0.0000; a(2)=0.0000; a(3)=0.0000; // 12300
  // d(1)=0.0; d(2)=0.0; d(3)=0.0; a(1)=0.0000; a(2)=0.0000; a(3)=0.0020; //
  // a3=0.002 d(1)=10.; d(2)=20.; d(3)=30.; a(1)=0.0100; a(2)=0.0200;
  // a(3)=0.0300; // huge d(1)=10.; d(2)=20.; d(3)=30.; a(1)=0.2000;
  // a(2)=0.2500; a(3)=0.3000; // huge angles d(1)=0.3; d(2)=0.4; d(3)=0.5;
  // a(1)=0.0005; a(2)=0.0006; a(3)=0.0007; // 0345 0567 d(1)=0.3; d(2)=0.4;
  // d(3)=0.5; a(1)=0.0001; a(2)=0.0002; a(3)=0.0003; // 0345 0123 d(1)=0.1;
  // d(2)=0.2; d(3)=0.3; a(1)=0.0003; a(2)=0.0004; a(3)=0.0005; // 0123 0345
  ...
  Vm(2) = newRl(1);
  Vm(3) = newRl(2);
  std::cout << " ========= dVm/da" << ii << " " << (Vm - Vm0) * (1. / del)
            << std::endl;
#endif

  if (debug_) {
    // std::cout<<" dRm/da3 "<<((Rm1-Rm10)*(1./del)).T()<<"
    // "<<((Pm1-Pm10)*(1./del)).T()<<std::endl; std::cout<<"             Norm
    // "<<Norm.T()<<std::endl;
    ...
    std::cout << "    pxyz/p loc0 " << (Tl * Pm0 / (Tl * Pm0).norm()).T()
              << std::endl;
    ...
    // std::cout<<"             Tl   "<<Tl<<endl;
    std::cout << " ---- phi g0 g1 l0 l1  " << atan2(Pm0(2), Pm0(1)) << " "
              << atan2(Pm1(2), Pm1(1)) << " "
              << atan2((Tl * Pm0)(2), (Tl * Pm0)(1)) << " "
              << atan2(newPl(2), newPl(1)) << std::endl;
    std::cout << " ---- angl Gl Loc "
              << atan2(Pm1(2), Pm1(1)) - atan2(Pm0(2), Pm0(1)) << " "
  ...
  if (debug_) {
    ...
    std::cout << " dXl dYl " << Vm(2) - tsosMuon.localParameters().vector()(3)
              ...
              << std::endl;
    std::cout << " localM    " << tsosMuon.localParameters().vector()
              << std::endl;
    std::cout << "       Vm  " << Vm << std::endl;
  ...
  if (debug_) { //    ----------  how to transform cartesian from shifted to
                //    correct
...
void GlobalTrackerMuonAlignment::trackFitter(
    reco::TrackRef alongTr, reco::TransientTrack &alongTTr,
    PropagationDirection direction, TrajectoryStateOnSurface &trackFittedTSOS) {
  bool info = false;
  bool smooth = false;
  ...
  if (info)
    std::cout << " ... Own recHit.size() " << recHit.size() << " "
              << trackDetId.rawId() << std::endl;
  ...
  if (info)
    std::cout << " ... recHitTrack.size() " << recHit.size() << " "
              << trackDetId.rawId() << " ======> recHitMu " << std::endl;
  ...
  */
  if (info)
    std::cout << " ...along.... recHitMu.size() " << recHitMu.size()
              << std::endl;
  if (direction != alongMomentum) {
    MuonTransientTrackingRecHitBuilder::ConstRecHitContainer recHitMuAlong =
        recHitMu;
    recHitMu.clear();
  ...
  if (info)
    std::cout << " ...opposite... recHitMu.size() " << recHitMu.size()
              << std::endl;
  ...
  TrajectoryStateOnSurface initialTSOS(firstTSOS.localParameters(),
                                       LocalTrajectoryError(CovLoc),
                                       firstTSOS.surface(), &*magneticField_);

  PTrajectoryStateOnDet PTraj = trajectoryStateTransform::persistentState(
      initialTSOS, trackDetId.rawId());
  ...
  if (!smooth) {
    ...
    if (!trajSVec.empty())
      this->debugTrajectorySOSv("smoothed geom InnermostState",
...
void GlobalTrackerMuonAlignment::muonFitter(
    reco::TrackRef alongTr, reco::TransientTrack &alongTTr,
    PropagationDirection direction, TrajectoryStateOnSurface &trackFittedTSOS) {
  bool info = false;
  bool smooth = false;
  ...
  if (info)
    std::cout << " ... Own recHit.size() DetId==Muon " << recHit.size() << " "
              << trackDetId.rawId() << std::endl;
...
void GlobalTrackerMuonAlignment::debugTrackHit(const std::string title,
                                               TransientTrack &alongTr) {
  std::cout << " ------- " << title << " --------" << std::endl;
  int nHit = 1;
  for (trackingRecHit_iterator i = alongTr.recHitsBegin();
       i != alongTr.recHitsEnd(); i++) {
...
void GlobalTrackerMuonAlignment::debugTrajectorySOS(
    const std::string title, TrajectoryStateOnSurface &trajSOS) {
  std::cout << "    --- " << title << " --- " << std::endl;
  if (!trajSOS.isValid()) {
    std::cout << "      Not valid !!!! " << std::endl;
    return;
...
// ----  debug printout TrajectoryStateOnSurface  ------
void GlobalTrackerMuonAlignment::debugTrajectorySOSv(
    const std::string title, TrajectoryStateOnSurface trajSOS) {
  std::cout << "    --- " << title << " --- " << std::endl;
  if (!trajSOS.isValid()) {
    std::cout << "      Not valid !!!! " << std::endl;
...
void GlobalTrackerMuonAlignment::debugTrajectory(const std::string title,
  ...
  this->debugTrajectorySOSv(" firstMeasurementTSOS ",
                            traj.firstMeasurement().updatedState());
  // this->debugTrajectorySOSv(" firstMeasurementTSOS
  // ",traj.firstMeasurement().predictedState());
  this->debugTrajectorySOSv(" lastMeasurementTSOS ",
                            traj.lastMeasurement().updatedState());
  // this->debugTrajectorySOSv(" geom InnermostState",
  // traj.geometricalInnermostState());
  std::cout << "    . . . . . . . . . . . . . . . . . . . . . . . . . . . . \n"
            << std::endl;
}
...
void GlobalTrackerMuonAlignment::writeGlPosRcd(CLHEP::HepVector &paramVec) {
  ...
  colY = CLHEP::Hep3Vector(c1 * s3 + s1 * s2 * c3, c1 * c3 - s1 * s2 * s3,
                           -s1 * c2);
  colZ = CLHEP::Hep3Vector(s1 * s3 - c1 * s2 * c3, s1 * c3 + c1 * s2 * s3,
                           c1 * c2);
#endif
  ...
  if (debug_)
    std::cout << " Old muon Rcd Euler angles " << angMuGlRcd.phi() << " "
              << angMuGlRcd.theta() << " " << angMuGlRcd.psi() << std::endl;
  AlignTransform muon;
