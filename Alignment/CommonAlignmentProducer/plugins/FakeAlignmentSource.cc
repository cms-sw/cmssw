...
//
/**\class FakeAlignmentSource FakeAlignmentSource.cc
Alignment/FakeAlignmentProducer/plugins/FakeAlignmentSource.cc

Description: Producer of fake alignment data for all geometries (currently:
Tracker, DT and CSC) (including IOV, in contrast to FakeAlignmentProducer)
...
*/
//
// Original Author:  Gero Flucke (based on FakeAlignmentProducer written by
// Frederic Ronga)
...
class FakeAlignmentSource : public edm::ESProducer,
...
FakeAlignmentSource::FakeAlignmentSource(const edm::ParameterSet &iConfig)
    : produceTracker_(iConfig.getParameter<bool>("produceTracker")),
      ...
      produceGlobalPosition_(
          iConfig.getParameter<bool>("produceGlobalPosition")),
      produceTrackerSurfaceDeformation_(
          iConfig.getParameter<bool>("produceTrackerSurfaceDeformation")) {
  ...
  if (produceTrackerSurfaceDeformation_) {
    this->setWhatProduced(
        this, &FakeAlignmentSource::produceTrackerSurfaceDeformation);
  }
...
void FakeAlignmentSource::setIntervalFor(
    const edm::eventsetup::EventSetupRecordKey & /*dummy*/,
    const edm::IOVSyncValue &ioSyncVal, edm::ValidityInterval &outValidity) {
  // Implementation copied from SiStripGainFakeESSource: unlimited IOV
  outValidity =
      edm::ValidityInterval(ioSyncVal.beginOfTime(), ioSyncVal.endOfTime());
}
