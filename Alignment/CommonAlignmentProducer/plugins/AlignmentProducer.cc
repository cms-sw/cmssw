...
void AlignmentProducer::endOfJob() {
  ...
  if (!finish()) {
    edm::LogError("Alignment") << "@SUB=AlignmentProducer::endOfJob"
                               << "Did not process any events in last loop, do "
                                  "not dare to store to DB.";
  }
}
...
//------------------------------------------------------------------------------
edm::EDLooper::Status
AlignmentProducer::endOfLoop(const edm::EventSetup &iSetup,
                             unsigned int iLoop) {

  if (0 == nEvent()) {
    // beginOfJob is usually called by the framework in the first event of the
    // first loop (a hack: beginOfJob needs the EventSetup that is not well
    // defined without an event) and the algorithms rely on the initialisations
    // done in beginOfJob. We cannot call this->beginOfJob(iSetup); here either
    // since that will access the EventSetup to get some geometry information
    // that is not defined either without having seen an event.
...
//------------------------------------------------------------------------------
void AlignmentProducer::beginLuminosityBlock(
    const edm::LuminosityBlock &lumiBlock, const edm::EventSetup &setup) {
  beginLuminosityBlockImpl(lumiBlock, setup);
}

//------------------------------------------------------------------------------
void AlignmentProducer::endLuminosityBlock(
    const edm::LuminosityBlock &lumiBlock, const edm::EventSetup &setup) {
