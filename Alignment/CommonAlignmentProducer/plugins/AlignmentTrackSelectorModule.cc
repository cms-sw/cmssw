...
// including recoTrackExtras and TrackingRecHitsOwned (in future also "owned
// clusters"?). if you remove it the code will compile, but the cloned tracks
// have only the recoTracks branch!
...
struct TrackConfigSelector {
  ...
  TrackConfigSelector(const edm::ParameterSet &cfg, edm::ConsumesCollector &&iC)
      : theBaseSelector(cfg, iC),
        ...
        theTwoBodyDecaySelector(
            cfg.getParameter<edm::ParameterSet>("TwoBodyDecaySelector"), iC) {
  ...
  void select(const edm::Handle<reco::TrackCollection> &c,
              const edm::Event &evt, const edm::EventSetup &eSetup) {
    ...
    if (theBaseSwitch)
      theSelectedTracks =
          theBaseSelector.select(theSelectedTracks, evt, eSetup);
    if (theGlobalSwitch)
      theSelectedTracks =
          theGlobalSelector.select(theSelectedTracks, evt, eSetup);
    if (theTwoBodyDecaySwitch)
      theSelectedTracks =
          theTwoBodyDecaySelector.select(theSelectedTracks, evt, eSetup);
  }
