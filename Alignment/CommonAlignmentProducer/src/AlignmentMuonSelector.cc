...
AlignmentMuonSelector::AlignmentMuonSelector(const edm::ParameterSet &cfg)
  ...
  if (applyMassPairFilter)
    edm::LogInfo("AlignmentMuonSelector")
        << "apply Mass Pair filter minMassPair=" << minMassPair
        << " maxMassPair=" << maxMassPair;
}
...
AlignmentMuonSelector::select(const Muons &muons, const edm::Event &evt) const {
  ...
  edm::LogInfo("AlignmentMuonSelector")
      << "muons all,kept: " << muons.size() << "," << result.size();
...
AlignmentMuonSelector::basicCuts(const Muons &muons) const {
  ...
  for (Muons::const_iterator it = muons.begin(); it != muons.end(); ++it) {
    ...
    edm::LogInfo("AlignmentMuonSelector")
        ...
        << "," << eta << "," << phi << "," << nhitSA << "," << chi2nSA << ","
        << nhitGB << "," << chi2nGB << "," << nhitTO << "," << chi2nTO;

    if (p > pMin && p < pMax && pt > ptMin && pt < ptMax && eta > etaMin &&
        eta < etaMax && phi > phiMin && phi < phiMax && nhitSA >= nHitMinSA &&
        nhitSA <= nHitMaxSA && chi2nSA < chi2nMaxSA && nhitGB >= nHitMinGB &&
        nhitGB <= nHitMaxGB && chi2nGB < chi2nMaxGB && nhitTO >= nHitMinTO &&
        nhitTO <= nHitMaxTO && chi2nTO < chi2nMaxTO) {
...
AlignmentMuonSelector::theNHighestPtMuons(const Muons &muons) const {
  ...
  int n = 0;
  for (Muons::const_iterator it = sortedMuons.begin(); it != sortedMuons.end();
       ++it) {
    if (n < nHighestPt) {
      result.push_back(*it);
      n++;
...
AlignmentMuonSelector::theBestMassPairCombinationMuons(
  ...
  // Criteria:
  // a) maxMassPair !=    minMassPair: the two highest pt muons with mass pair
  // inside the given mass window b) maxMassPair ==    minMassPair: the muon
  // pair with massPair closest to given mass value
  for (Muons::const_iterator it1 = sortedMuons.begin();
    ...
    for (Muons::const_iterator it2 = it1 + 1; it2 != sortedMuons.end(); ++it2) {
      mu1 = TLorentzVector((*it1)->momentum().x(), (*it1)->momentum().y(),
                           (*it1)->momentum().z(), (*it1)->p());
      mu2 = TLorentzVector((*it2)->momentum().x(), (*it2)->momentum().y(),
                           (*it2)->momentum().z(), (*it2)->p());
      pair = mu1 + mu2;
      mass = pair.M();
