...
AlignmentGlobalTrackSelector::AlignmentGlobalTrackSelector(
  ...
  if (theGMFilterSwitch) {
    ...
    LogDebug("Alignment")
        ...
        << theMuonSource << " , " << theMaxTrackDeltaR << " , "
        << theMinIsolatedCount;
  }
  ...
  if (theIsoFilterSwitch) {
    ...
    LogDebug("Alignment")
        ...
        << theJetIsoSource << " , " << theMaxJetPt << " ," << theMinJetDeltaR
        << " ," << theMinGlobalMuonCount;
  }

  if (theJetCountFilterSwitch) {
    edm::InputTag theJetCountSource =
        cfg.getParameter<InputTag>("jetCountSource");
    ...
    LogDebug("Alignment")
        ...
        << theJetCountSource << " , " << theMinJetPt << " ," << theMaxJetCount;
  }
}
...
AlignmentGlobalTrackSelector::select(const Tracks &tracks,
  ...
  LogDebug("Alignment") << ">  Global: tracks all, kept: " << tracks.size()
                        << ", " << result.size();
...
AlignmentGlobalTrackSelector::Tracks
AlignmentGlobalTrackSelector::findMuons(const Tracks &tracks,
...
AlignmentGlobalTrackSelector::Tracks
AlignmentGlobalTrackSelector::checkIsolation(const Tracks &cands,
                                             const edm::Event &iEvent) const {
  Tracks result;
  result.clear();
  ...
  if (jets.isValid()) {
    for (Tracks::const_iterator it = cands.begin(); it != cands.end(); ++it) {
      ...
      for (reco::CaloJetCollection::const_iterator itJet = jets->begin();
        ...
        isolated &= !((*itJet).pt() > theMaxJetPt &&
                      deltaR(*(*it), (*itJet)) < theMinJetDeltaR);

      if (isolated)
        result.push_back(*it);
    }
    //    LogDebug("Alignment") << "D  Found "<<result.size()<<" isolated of "<<
    //    cands.size()<<" Tracks!";
...
AlignmentGlobalTrackSelector::Tracks
AlignmentGlobalTrackSelector::checkJetCount(const Tracks &tracks,
                                            const edm::Event &iEvent) const {
  Tracks result;
  result.clear();
...
/// from [src]
AlignmentGlobalTrackSelector::Tracks
AlignmentGlobalTrackSelector::matchTracks(const Tracks &src,
                                          const Tracks &comp) const {
  Tracks result;
  for (Tracks::const_iterator itComp = comp.begin(); itComp != comp.end();
       ++itComp) {
    int match = -1;
    double min = theMaxTrackDeltaR;
    for (unsigned int i = 0; i < src.size(); i++) {
      // LogDebug("Alignment") << "> Trackmatch dist:
      // "<<deltaR(src.at(i),*itComp);
