...
AlignmentTrackSelector::AlignmentTrackSelector(const edm::ParameterSet &cfg,
    ...
    : applyBasicCuts_(cfg.getParameter<bool>("applyBasicCuts")),
      ...
      // Ugly to use the same getParameter n times, but this allows const cut
      // variables...
      ...
      minHitsinPIX_(cfg.getParameter<edm::ParameterSet>("minHitsPerSubDet")
                        .getParameter<int>("inPIXEL")),
      minHitsinTIDplus_(cfg.getParameter<edm::ParameterSet>("minHitsPerSubDet")
                            .getParameter<int>("inTIDplus")),
      minHitsinTIDminus_(cfg.getParameter<edm::ParameterSet>("minHitsPerSubDet")
                             .getParameter<int>("inTIDminus")),
      minHitsinTECplus_(cfg.getParameter<edm::ParameterSet>("minHitsPerSubDet")
                            .getParameter<int>("inTECplus")),
      minHitsinTECminus_(cfg.getParameter<edm::ParameterSet>("minHitsPerSubDet")
                             .getParameter<int>("inTECminus")),
      minHitsinFPIXplus_(cfg.getParameter<edm::ParameterSet>("minHitsPerSubDet")
                             .getParameter<int>("inFPIXplus")),
      minHitsinFPIXminus_(
          cfg.getParameter<edm::ParameterSet>("minHitsPerSubDet")
              .getParameter<int>("inFPIXminus")),
      minHitsinENDCAP_(cfg.getParameter<edm::ParameterSet>("minHitsPerSubDet")
                           .getParameter<int>("inENDCAP")),
      minHitsinENDCAPplus_(
          cfg.getParameter<edm::ParameterSet>("minHitsPerSubDet")
              .getParameter<int>("inENDCAPplus")),
      minHitsinENDCAPminus_(
          cfg.getParameter<edm::ParameterSet>("minHitsPerSubDet")
              .getParameter<int>("inENDCAPminus")),
      maxHitDiffEndcaps_(cfg.getParameter<double>("maxHitDiffEndcaps")),
      nLostHitMax_(cfg.getParameter<double>("nLostHitMax")),
      RorZofFirstHitMin_(
          cfg.getParameter<std::vector<double>>("RorZofFirstHitMin")),
      RorZofFirstHitMax_(
          cfg.getParameter<std::vector<double>>("RorZofFirstHitMax")),
      RorZofLastHitMin_(
          cfg.getParameter<std::vector<double>>("RorZofLastHitMin")),
      RorZofLastHitMax_(
          cfg.getParameter<std::vector<double>>("RorZofLastHitMax")),
      ...
      applyPrescaledHitsFilter_(!clusterValueMapTag_.encode().empty() &&
                                minPrescaledHits_ > 0) {
  if (applyIsolation_) {
    rphirecHitsToken_ = iC.consumes<SiStripRecHit2DCollection>(
        cfg.getParameter<edm::InputTag>("rphirecHits"));
    matchedrecHitsToken_ = iC.consumes<SiStripMatchedRecHit2DCollection>(
        cfg.getParameter<edm::InputTag>("matchedrecHits"));
  }

  if (applyPrescaledHitsFilter_) {
    clusterValueMapToken_ =
        iC.consumes<AliClusterValueMap>(clusterValueMapTag_);
  }
  ...
  if (!trkQualityStrings.empty()) {
    ...
    for (unsigned int i = 0; i < trkQualityStrings.size(); ++i) {
      ...
      trkQualities_.push_back(
          reco::TrackBase::qualityByName(trkQualityStrings[i]));
    }
  } else
    applyTrkQualityCheck_ = false;

  std::vector<std::string> trkIterStrings(
      cfg.getParameter<std::vector<std::string>>("iterativeTrackingSteps"));
  ...
  if (applyBasicCuts_) {
    ...
    edm::LogInfo("AlignmentTrackSelector")
        ...
        << minHitsinENDCAP_ << "/" << minHitsinENDCAPplus_ << "/"
        << minHitsinENDCAPminus_;
  ...
  // Checking whether cuts on positions of first and last track hits are defined
  // properly
  ...
  if (RorZofFirstHitMin_.at(0) > RorZofLastHitMax_.at(0) &&
      RorZofFirstHitMin_.at(1) > RorZofLastHitMax_.at(1)) {
    throw cms::Exception("BadConfig")
        << "@SUB=AlignmentTrackSelector::AlignmentTrackSelector"
        << "Position of the first hit is set to larger distance than the last "
           "hit:."
        << " First hit(min): [" << RorZofFirstHitMin_.at(0) << ", "
        << RorZofFirstHitMin_.at(1) << "]; Last hit(max): ["
...
AlignmentTrackSelector::select(const Tracks &tracks, const edm::Event &evt,
...
AlignmentTrackSelector::Tracks
AlignmentTrackSelector::basicCuts(const Tracks &tracks, const edm::Event &evt,
                                  const edm::EventSetup &eSetup) const {
  Tracks result;
  ...
  for (Tracks::const_iterator it = tracks.begin(); it != tracks.end(); ++it) {
    ...
    if (pt > ptMin_ && pt < ptMax_ && p > pMin_ && p < pMax_ && eta > etaMin_ &&
        eta < etaMax_ && phi > phiMin_ && phi < phiMax_ && nhit >= nHitMin_ &&
        nhit <= nHitMax_ && nlosthit <= nLostHitMax_ && chi2n < chi2nMax_ &&
        isChargeOk && d0 >= d0Min_ && d0 <= d0Max_ && dz >= dzMin_ &&
        dz <= dzMax_) {
...
bool AlignmentTrackSelector::detailedHitsCheck(
    const reco::Track *trackp, const edm::Event &evt,
    const edm::EventSetup &eSetup) const {
  ...
  if (minHitsinTIB_ || minHitsinTOB_ || minHitsinTID_ || minHitsinTEC_ ||
      ...
      (maxHitDiffEndcaps_ < 999) || minHitsinTIDplus_ || minHitsinTIDminus_ ||
      minHitsinFPIXplus_ || minHitsinFPIXminus_ || minHitsinTECplus_ ||
      minHitsinTECminus_ || minHitsinFPIX_ || minHitsinBPIX_ || minHitsinPIX_ ||
      nHitMin2D_ || chargeCheck_ || applyIsolation_ ||
      (seedOnlyFromAbove_ == 1 || seedOnlyFromAbove_ == 2) ||
      !RorZofFirstHitMin_.empty() || !RorZofFirstHitMax_.empty() ||
      !RorZofLastHitMin_.empty() || !RorZofLastHitMax_.empty()) {
    ...
    for (auto const &hit : trackp->recHits()) {
      ...
      if (seedOnlyFromAbove_ == 1 && thishit == 1 &&
          (subdetId == int(SiStripDetId::TOB) ||
           subdetId == int(SiStripDetId::TEC))) {
        return false;
      }
      if (seedOnlyFromAbove_ == 2 && thishit == 1 &&
          subdetId == int(SiStripDetId::TIB)) {
        return false;
      }
    ...
    // Checking whether the track satisfies requirement of the first and last
    // hit positions
    ...
    bool passedFirstHitPosition =
        passedFirstHitPositionR || passedFirstHitPositionZ;
    bool passedLastHitPosition =
        passedLastHitPositionR || passedLastHitPositionZ;

    return (nhitinTIB >= minHitsinTIB_ && nhitinTOB >= minHitsinTOB_ &&
            nhitinTID >= minHitsinTID_ && nhitinTEC >= minHitsinTEC_ &&
            nhitinENDCAP >= minHitsinENDCAP_ &&
            nhitinENDCAPplus >= minHitsinENDCAPplus_ &&
            nhitinENDCAPminus >= minHitsinENDCAPminus_ &&
            std::abs(nhitinENDCAPplus - nhitinENDCAPminus) <=
                maxHitDiffEndcaps_ &&
            nhitinTIDplus >= minHitsinTIDplus_ &&
            nhitinTIDminus >= minHitsinTIDminus_ &&
            nhitinFPIXplus >= minHitsinFPIXplus_ &&
            nhitinFPIXminus >= minHitsinFPIXminus_ &&
            nhitinTECplus >= minHitsinTECplus_ &&
            nhitinTECminus >= minHitsinTECminus_ &&
            nhitinBPIX >= minHitsinBPIX_ && nhitinFPIX >= minHitsinFPIX_ &&
            nhitinPIXEL >= minHitsinPIX_ && nHit2D >= nHitMin2D_ &&
            passedFirstHitPosition && passedLastHitPosition);
...
bool AlignmentTrackSelector::isHit2D(const TrackingRecHit &hit) const {
  ...
  if (!hit.isValid() || (hit.dimension() < 2 && !countStereoHitAs2D_ &&
                         !dynamic_cast<const SiStripRecHit1D *>(&hit))) {
    return false; // real RecHit1D - but SiStripRecHit1D depends on
                  // countStereoHitAs2D_
  } else {
    const DetId detId(hit.geographicalId());
    if (detId.det() == DetId::Tracker) {
      if (detId.subdetId() == kBPIX || detId.subdetId() == kFPIX) {
        ...
        if (stripId.stereo())
          ...
          const ProjectedSiStripRecHit2D *pH =
              static_cast<const ProjectedSiStripRecHit2D *>(&hit);
          return (countStereoHitAs2D_ &&
                  this->isHit2D(pH->originalHit())); // depends on original...
        } else {
...
bool AlignmentTrackSelector::isOkCharge(const TrackingRecHit *hit) const {
  ...
  else if (type == typeid(SiStripMatchedRecHit2D)) { // or matched (should not
                                                     // occur anymore due to hit
                                                     // splitting since 20X)
...
bool AlignmentTrackSelector::isOkChargeStripHit(
    const SiStripRecHit2D &siStripRecHit2D) const {
  double charge = 0.;
...
bool AlignmentTrackSelector::isOkChargeStripHit(
    const SiStripRecHit1D &siStripRecHit1D) const {
  double charge = 0.;
...
bool AlignmentTrackSelector::isIsolated(const TrackingRecHit *hit,
  ...
  const SiStripRecHit2DCollection::DataContainer &stripcollSt =
      rphirecHits->data();
  const SiStripMatchedRecHit2DCollection::DataContainer &stripcollStm =
      matchedrecHits->data();
  ...
  for (istripSt = stripcollSt.begin(); istripSt != stripcollSt.end();
       ++istripSt) {
  ...
  // FIXME: see above
  for (istripStm = stripcollStm.begin(); istripStm != stripcollStm.end();
       ++istripStm) {
...
AlignmentTrackSelector::Tracks
AlignmentTrackSelector::checkPrescaledHits(const Tracks &tracks,
                                           const edm::Event &evt) const {
  Tracks result;
  ...
  for (auto const &trackp : tracks) {
    ...
    for (auto const &hit : trackp->recHits()) {
      ...
      if (!isPixelHit) {
        ...
        if (type == typeid(SiStripRecHit2D)) {
          const SiStripRecHit2D *striphit =
              dynamic_cast<const SiStripRecHit2D *>(hit);
          if (striphit != nullptr) {
          ...
          const SiStripRecHit1D *striphit =
              dynamic_cast<const SiStripRecHit1D *>(hit);
          if (striphit != nullptr) {
          ...
          edm::LogError("AlignmentTrackSelector")
              << "ERROR in <AlignmentTrackSelector::checkPrescaledHits>: "
                 "Dynamic cast of Strip RecHit failed!"
              << " Skipping this hit.";
          continue;
        }
        ...
        const SiPixelRecHit *pixelhit =
            dynamic_cast<const SiPixelRecHit *>(hit);
        if (pixelhit != nullptr) {
          ...
          edm::LogError("AlignmentTrackSelector")
              << "ERROR in <AlignmentTrackSelector::checkPrescaledHits>: "
                 "Dynamic cast of Pixel RecHit failed!  ";
        }
