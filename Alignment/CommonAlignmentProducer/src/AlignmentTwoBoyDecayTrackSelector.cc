...
AlignmentTwoBodyDecayTrackSelector::AlignmentTwoBodyDecayTrackSelector(
  ...
  if (theMassrangeSwitch) {
    ...
    theCandNumber = cfg.getParameter<unsigned int>(
        "numberOfCandidates"); // Number of candidates to keep
    secThrBool = cfg.getParameter<bool>("applySecThreshold");
    thesecThr = cfg.getParameter<double>("secondThreshold");
    LogDebug("Alignment") << ">  Massrange min,max         :   " << theMinMass
                          << "," << theMaxMass
                          << "\n>  Mass of daughter Particle :   "
                          << theDaughterMass;
  ...
  if (theChargeSwitch) {
    ...
    LogDebug("Alignment") << ">  Desired Charge, unsigned: " << theCharge
                          << " , " << theUnsignedSwitch;
  } else {
    theCharge = 0;
  ...
  if (theMissingETSwitch) {
    edm::InputTag theMissingETSource =
        cfg.getParameter<InputTag>("missingETSource");
    theMissingETToken =
        iC.consumes<reco::CaloMETCollection>(theMissingETSource);
    LogDebug("Alignment") << ">  missing Et Source: " << theMissingETSource;
  }
  theAcoplanarityFilterSwitch =
      cfg.getParameter<bool>("applyAcoplanarityFilter");
...
AlignmentTwoBodyDecayTrackSelector::select(const Tracks &tracks,
  ...
  LogDebug("Alignment") << ">  TwoBodyDecay tracks all,kept: " << tracks.size()
                        << "," << result.size();
  return result;
}
...
AlignmentTwoBodyDecayTrackSelector::checkMass(const Tracks &cands) const {
  ...
  for (unsigned int iCand = 0; iCand < cands.size(); iCand++) {
    ...
    track0.SetXYZT(cands.at(iCand)->px(), cands.at(iCand)->py(),
                   cands.at(iCand)->pz(),
                   sqrt(cands.at(iCand)->p() * cands.at(iCand)->p() +
                        theDaughterMass * theDaughterMass));

    for (unsigned int jCand = iCand + 1; jCand < cands.size(); jCand++) {

      track1.SetXYZT(cands.at(jCand)->px(), cands.at(jCand)->py(),
                     cands.at(jCand)->pz(),
                     sqrt(cands.at(jCand)->p() * cands.at(jCand)->p() +
                          theDaughterMass * theDaughterMass));
      if (secThrBool == true && track1.Pt() < thesecThr &&
          track0.Pt() < thesecThr)
        continue;
      ...
      if (mother.M() > theMinMass && mother.M() < theMaxMass && correctCharge &&
          acoplanarTracks) {
        candCollection.push_back(
            candCollectionItem(mother.Pt(), constTrackPair(trk1, trk2)));
      }
    }
  }
  ...
  for (unsigned int i = 0; i < candCollection.size() && i < theCandNumber;
       i++) {
...
AlignmentTwoBodyDecayTrackSelector::checkMETMass(
  ...
  for (reco::CaloMETCollection::const_iterator itMET = missingET->begin();
       itMET != missingET->end(); ++itMET) {

    met4.SetXYZT((*itMET).px(), (*itMET).py(), (*itMET).pz(), (*itMET).p());

    for (unsigned int iCand = 0; iCand < cands.size(); iCand++) {

      track.SetXYZT(cands.at(iCand)->px(), cands.at(iCand)->py(),
                    cands.at(iCand)->pz(),
                    sqrt(cands.at(iCand)->p() * cands.at(iCand)->p() +
                         theDaughterMass * theDaughterMass));
...
bool AlignmentTwoBodyDecayTrackSelector::checkCharge(
    const reco::Track *trk1, const reco::Track *trk2) const {
...
bool AlignmentTwoBodyDecayTrackSelector::checkAcoplanarity(
    const reco::Track *trk1, const reco::Track *trk2) const {
...
/// checks if the [cands] are acoplanar (returns empty set if not)
bool AlignmentTwoBodyDecayTrackSelector::checkMETAcoplanarity(
    const reco::Track *trk1, const reco::CaloMET *met) const {
...
void AlignmentTwoBodyDecayTrackSelector::printTracks(const Tracks &col) const {
  ...
  for (Tracks::const_iterator it = col.begin(); it < col.end(); ++it, ++count) {
    LogDebug("Alignment") << ">  Track No. " << count << ": p = ("
                          << (*it)->px() << "," << (*it)->py() << ","
                          << (*it)->pz() << ")\n"
                          << ">                        pT = " << (*it)->pt()
                          << " eta = " << (*it)->eta()
