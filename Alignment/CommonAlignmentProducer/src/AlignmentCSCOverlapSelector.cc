...
AlignmentCSCOverlapSelector::AlignmentCSCOverlapSelector(
    ...
    : m_station(iConfig.getParameter<int>("station")),
      m_minHitsPerChamber(
          iConfig.getParameter<unsigned int>("minHitsPerChamber")) {
  if (m_station == 0) {
    edm::LogInfo("AlignmentCSCOverlapSelector")
        << "Acceptable tracks must have " << m_minHitsPerChamber
...
AlignmentCSCOverlapSelector::select(const Tracks &tracks,
