...
AlignmentCSCBeamHaloSelector::AlignmentCSCBeamHaloSelector(
    ...
    : m_minStations(iConfig.getParameter<unsigned int>("minStations")),
      m_minHitsPerStation(
          iConfig.getParameter<unsigned int>("minHitsPerStation")) {
  edm::LogInfo("AlignmentCSCBeamHaloSelector")
      << "Acceptable tracks must have at least " << m_minHitsPerStation
      << " hits in " << m_minStations << " different CSC stations."
      << std::endl;
}
...
AlignmentCSCBeamHaloSelector::select(const Tracks &tracks,
  ...
  for (auto const &track : tracks) {
    ...
    for (auto const &hit : track->recHits()) {
      ...
      if (id.det() == DetId::Muon && id.subdetId() == MuonSubdetId::CSC) {
        ...
        std::map<int, unsigned int>::const_iterator station_iter =
            station_map.find(station);
    ...
    for (std::map<int, unsigned int>::const_iterator station_iter =
             station_map.begin();
         station_iter != station_map.end(); ++station_iter) {
