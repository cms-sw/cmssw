...
AlignmentProducerBase::AlignmentProducerBase(const edm::ParameterSet &config)
    : doTracker_{config.getUntrackedParameter<bool>("doTracker")},
      ...
      tjTkAssociationMapTag_{
          config.getParameter<edm::InputTag>("tjTkAssociationMapTag")},
      beamSpotTag_{config.getParameter<edm::InputTag>("beamSpotTag")},
      tkLasBeamTag_{config.getParameter<edm::InputTag>("tkLasBeamTag")},
      clusterValueMapTag_{
          config.getParameter<edm::InputTag>("hitPrescaleMapTag")},
      uniqueRunRanges_{align::makeUniqueRunRanges(
          config.getParameter<edm::VParameterSet>("RunRangeSelection"),
          cond::timeTypeSpecs[cond::runnumber].beginValue)},
      config_{config}, stNFixAlignables_{config.getParameter<int>(
                           "nFixAlignables")},
      ...
      checkDbAlignmentValidity_{
          config.getUntrackedParameter<bool>("checkDbAlignmentValidity")},
      doMisalignmentScenario_{
          config.getParameter<bool>("doMisalignmentScenario")},
      ...
      enableAlignableUpdates_{
          config.getParameter<bool>("enableAlignableUpdates")} {
...
bool AlignmentProducerBase::processEvent(const edm::Event &event,
  ...
  initBeamSpot(
      event); // must happen every event and before incrementing 'nevent_'
  ...
  if (!alignmentAlgo_->processesEvents()) {
    edm::LogInfo("Alignment") << "@SUB=AlignmentProducerBase::processEvent"
                              << "Skipping event. The current configuration of "
  ...
  if (getTrajTrackAssociationCollection(event, handleTrajTracksCollection)) {
    ...
    for (auto iter = handleTrajTracksCollection->begin();
         iter != handleTrajTracksCollection->end(); ++iter) {
      trajTracks.push_back(
    ...
    const AlignmentAlgorithmBase::EventInfo eventInfo{
        event.id(), trajTracks, *beamSpot_, clusterValueMapPtr};
...
void AlignmentProducerBase::endRunImpl(const edm::Run &run,
  ...
  if (!tkLasBeamTag_.encode().empty()) {
    ...
    alignmentAlgo_->endRun(EndRunInfo(run.id(), &(*lasBeams), &(*tsoses)),
                           setup);
  } else {
...
//------------------------------------------------------------------------------
void AlignmentProducerBase::beginLuminosityBlockImpl(
    const edm::LuminosityBlock &, const edm::EventSetup &setup) {
  // Do not forward edm::LuminosityBlock
  alignmentAlgo_->beginLuminosityBlock(setup);
}

//------------------------------------------------------------------------------
void AlignmentProducerBase::endLuminosityBlockImpl(
    const edm::LuminosityBlock &, const edm::EventSetup &setup) {
  // Do not forward edm::LuminosityBlock
...
void AlignmentProducerBase::createAlignmentAlgorithm() {
  ...
  algoConfig.addUntrackedParameter(
      "RunRangeSelection",
      config_.getParameter<edm::VParameterSet>("RunRangeSelection"));
  algoConfig.addUntrackedParameter<align::RunNumber>(
      "firstIOV", runAtPCL_ ? 1 : uniqueRunRanges_.front().first);
...
void AlignmentProducerBase::createMonitors() {
  const auto &monitorConfig =
      config_.getParameter<edm::ParameterSet>("monitorConfig");
  auto monitors =
  ...
  for (const auto &miter : monitors) {
    std::unique_ptr<AlignmentMonitorBase> newMonitor{
        AlignmentMonitorPluginFactory::get()->create(
            miter, monitorConfig.getUntrackedParameterSet(miter))};
...
void AlignmentProducerBase::createCalibrations() {
  const auto &calibrations =
      config_.getParameter<edm::VParameterSet>("calibrations");
  for (const auto &iCalib : calibrations) {
    calibrations_.push_back(IntegratedCalibrationPluginFactory::get()->create(
...
void AlignmentProducerBase::initAlignmentAlgorithm(const edm::EventSetup &setup,
  ...
  alignmentAlgo_->initialize(setup, alignableTracker_, alignableMuon_,
                             alignableExtras_, alignmentParameterStore_);
  ...
  if (!isTrueUpdate) { // only needed the first time
    ...
    for (const auto &monitor : monitors_) {
      monitor->beginOfJob(alignableTracker_, alignableMuon_,
...
void AlignmentProducerBase::initBeamSpot(const edm::Event &event) {
  ...
  if (nevent_ == 0 && alignableExtras_) {
    ...
    alignableExtras_->initializeBeamSpot(beamSpot_->x0(), beamSpot_->y0(),
                                         beamSpot_->z0(), beamSpot_->dxdz(),
                                         beamSpot_->dydz());
  }
}
...
void AlignmentProducerBase::applyAlignmentsToDB(const edm::EventSetup &setup) {
  ...
  if (applyDbAlignment_) {
    ...
    if (doTracker_) {
      applyDB<TrackerGeometry, TrackerAlignmentRcd,
              TrackerAlignmentErrorExtendedRcd>(
          trackerGeometry_.get(), setup,
          align::DetectorGlobalPosition(*globalPositions_,
                                        DetId(DetId::Tracker)));
...
void AlignmentProducerBase::createAlignables(const TrackerTopology *tTopo,
  ...
  if (doMuon_) {
    if (update) {
      ...
      alignableMuon_ =
          new AlignableMuon(muonDTGeometry_.get(), muonCSCGeometry_.get());
    }
  }

  if (useExtras_) {
    if (update) {
      // FIXME: Requires further code changes to track beam spot condition
      // changes
    } else {
...
void AlignmentProducerBase::applyMisalignment() {
  ...
  if (doMisalignmentScenario_ && (doTracker_ || doMuon_)) {
    ...
    const auto &scenarioConfig =
        config_.getParameterSet("MisalignmentScenario");
...
void AlignmentProducerBase::simpleMisalignment(const align::Alignables &alivec,
  ...
  if (shift > 0. || rot > 0.) {
    ...
    if (selection != "-1") {
      AlignmentParameterSelector aSelector(
          nullptr, nullptr); // no alignable needed here...
      ...
      if (cSel.size() < RigidBodyAlignmentParameters::N_PARAM) {
        throw cms::Exception("BadConfig")
            ...
            << "(Most probably you have to adjust the parameter "
               "'parameterSelectorSimple'.)";
      }
    ...
    for (const auto &ali : alivec) {
      std::vector<bool> mysel(
          commSel.empty() ? ali->alignmentParameters()->selector() : commSel);
...
void AlignmentProducerBase::applyAlignmentsToGeometry() {
  ...
  // Propagate changes to reconstruction geometry (from initialisation or
  // iteration)
  ...
  if (doTracker_) {
    ...
    std::unique_ptr<AlignmentSurfaceDeformations> aliDeforms{
...
void AlignmentProducerBase::storeAlignmentsToDB() {
  ...
  const auto &uniqueRunRanges =
      (runAtPCL_ ? align::makeUniqueRunRanges(runRangeSelectionVPSet, firstRun_)
                 : uniqueRunRanges_);
  ...
  for (const auto &iRunRange : uniqueRunRanges) {
    ...
    if (alignableExtras_) {
      ...
      if (!alis.empty()) {
        auto beamSpotAliPars = dynamic_cast<BeamSpotAlignmentParameters *>(
            alis[0]->alignmentParameters());
        if (!beamSpotAliPars) {
          throw cms::Exception("LogicError")
  ...
  if (alignableExtras_) {
    ...
    for (auto iRunRange = uniqueRunRanges.cbegin();
         iRunRange != uniqueRunRanges.cend(); ++iRunRange, ++itPar) {
      bsOutput << "Run range: " << (*iRunRange).first << " - "
               << (*iRunRange).second << "\n";
...
void AlignmentProducerBase::writeForRunRange(cond::Time_t time) {
  if (doTracker_) { // first tracker
    const AlignTransform *trackerGlobal{
        nullptr};           // will be 'removed' from constants
    ...
    this->writeDB(alignments, "TrackerAlignmentRcd", alignmentErrors,
                  "TrackerAlignmentErrorExtendedRcd", trackerGlobal, time);

    // Save surface deformations to database
    if (saveDeformationsToDB_) {
      auto alignmentSurfaceDeformations =
          alignableTracker_->surfaceDeformations();
      this->writeDB(alignmentSurfaceDeformations,
                    "TrackerSurfaceDeformationRcd", time);
    }
  }

  if (doMuon_) { // now muon
    const AlignTransform *muonGlobal{
        nullptr};           // will be 'removed' from constants
    if (globalPositions_) { // i.e. applied before in applyDB
      muonGlobal =
          &align::DetectorGlobalPosition(*globalPositions_, DetId(DetId::Muon));
    }
    // Get alignments+errors, first DT - ownership taken over by writeDB(..), so
    // no delete
    auto alignments = alignableMuon_->dtAlignments();
    auto alignmentErrors = alignableMuon_->dtAlignmentErrorsExtended();
    this->writeDB(alignments, "DTAlignmentRcd", alignmentErrors,
                  "DTAlignmentErrorExtendedRcd", muonGlobal, time);

    // Get alignments+errors, now CSC - ownership taken over by writeDB(..), so
    // no delete
    alignments = alignableMuon_->cscAlignments();
    alignmentErrors = alignableMuon_->cscAlignmentErrorsExtended();
    this->writeDB(alignments, "CSCAlignmentRcd", alignmentErrors,
...
void AlignmentProducerBase::writeDB(Alignments *alignments,
  ...
  if (globalCoordinates // happens only if (applyDbAlignment_ == true)
    ...
    tempAlignmentErrorsExtended =
        new AlignmentErrorsExtended(); // final alignments and errors

    GeometryAligner aligner;
    aligner.removeGlobalTransform(alignments, alignmentErrors,
                                  *globalCoordinates, tempAlignments,
                                  tempAlignmentErrorsExtended);
  ...
  if (saveToDB_) {
    edm::LogInfo("Alignment")
        << "Writing Alignments for run " << time << " to " << alignRcd << ".";
  ...
  if (saveApeToDB_) {
    edm::LogInfo("Alignment") << "Writing AlignmentErrorsExtended for run "
                              << time << " to " << errRcd << ".";
    poolDb->writeOne<AlignmentErrorsExtended>(tempAlignmentErrorsExtended, time,
                                              errRcd);
  } else { // poolDb->writeOne(..) takes over 'alignmentErrors' ownership,...
    delete tempAlignmentErrorsExtended; // ...otherwise we have to delete, as
                                        // promised!
  }
}

//------------------------------------------------------------------------------
void AlignmentProducerBase::writeDB(
    AlignmentSurfaceDeformations *alignmentSurfaceDeformations,
    const std::string &surfaceDeformationRcd, cond::Time_t time) const {
  // Call service
  edm::Service<cond::service::PoolDBOutputService> poolDb;
  if (!poolDb.isAvailable()) {           // Die if not available
  ...
  if (saveDeformationsToDB_) {
    ...
    poolDb->writeOne<AlignmentSurfaceDeformations>(alignmentSurfaceDeformations,
                                                   time, surfaceDeformationRcd);
  } else { // poolDb->writeOne(..) takes over 'surfaceDeformation' ownership,...
    delete alignmentSurfaceDeformations; // ...otherwise we have to delete, as
                                         // promised!
  }
}
