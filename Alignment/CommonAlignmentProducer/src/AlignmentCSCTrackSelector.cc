...
AlignmentCSCTrackSelector::select(const Tracks &tracks,
  ...
  for (auto const &track : tracks) {
    ...
    if (m_stationA == 0)
      ...
      stationAokay = (m_minHitsPerStation <= hitsOnStationA &&
                      hitsOnStationA <= m_maxHitsPerStation);

    bool stationBokay;
    if (m_stationB == 0)
      stationBokay = (m_minHitsDT <= hitsOnStationB);
    else
      stationBokay = (m_minHitsPerStation <= hitsOnStationB &&
                      hitsOnStationB <= m_maxHitsPerStation);
