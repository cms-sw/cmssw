...
///
///
void LASCoordinateSet::GetCoordinates(
    double& aPhi, double& aPhiError, double& aR, double& aRError, double& aZ, double& aZError) const {
  aPhi = phi;
  aPhiError = phiError;
  aR = r;
  aRError = rError;
...
///
///
void LASCoordinateSet::SetCoordinates(
    double aPhi, double aPhiError, double aR, double aRError, double aZ, double aZError) {
  phi = aPhi;
  phiError = aPhiError;
  r = aR;
  rError = aRError;
