						   ...
						   const std::string &levelName)
{
  ...
  for (auto alignable : alignables) {
    ...
    if ( alignable->size() > 0 && parameterSetNames.size() > 0 )
                                                                 ...
                                                                 const edm::ParameterSet& pSet ) const
{
  ...
  for (auto & pName : pNames) {
    if (pName.find(levelName) != 0) continue; // parameter not starting with levelName
    ...
    while (std::string::npos != pos || std::string::npos != lastPos) {
      ...
      if (atoi(digit.c_str()) == iComponent) {
        ++nFittingPsets;
        LogDebug("getParameterSet_") << indent_ << "found " << pName << " matching "
