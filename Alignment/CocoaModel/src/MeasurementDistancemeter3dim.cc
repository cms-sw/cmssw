...
void MeasurementDistancemeter3dim::calculateSimulatedValue(ALIbool firstTime) {
  ...
  if (OptOList().size() == 2) {
    if ((*vocite)->type() == "distance_target" && ((*(vocite + 1))->type() == "distancemeter3dim")) {
      right_objects = true;
    }
  }
  if (!right_objects) {
    std::cerr << "!!! ERROR in MeasurementDistancemeter3dim: " << name()
...
void MeasurementDistancemeter3dim::setConversionFactor(const std::vector<ALIstring>& wordlist) {
  ...
  if (dimfac == 0) {
    ...
    std::cerr << " !!!EXITING: error in global option distancemeter3dim_meas_value_dimension, it can only take values "
                 "0,1,2, not "
              << dimfac;
    std::exception();
  }
...
void MeasurementDistancemeter3dim::correctValueAndSigma() {
  ...
  if (ALIUtils::debug >= 4)
    std::cout << "MeasurementDistancemeter3dim::correctValueAndSigma: old value" << val << " +- " << sig << std::endl;
  ...
  //-  sig *= theFactor;
  if (ALIUtils::debug >= 4)
    std::cout << "MeasuremenDistancemeter3dim::correctValueAndSigma: new value " << val << " +- " << sig << std::endl;
  setValue(0, val);
  setSigma(0, sig);
}
