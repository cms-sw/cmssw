...
Entry::Entry(const ALIstring& type) : type_(type), fitPos_(-1) {
  ...
  if (ALIUtils::debug >= 5)
    std::cout << this << " theValueDisplacementByFitting set " << valueDisplacementByFitting_ << std::endl;
}
...
void Entry::fill(const std::vector<ALIstring>& wordlist) {
  ...
  if (ALIUtils::debug >= 4)
    std::cout << "@@@ Filling entry: " << name() << std::endl;
  ...
  if (ALIUtils::debug >= 5)
    std::cout << " entryData " << entryData << " " << OptOCurrent()->longName() << " " << name() << std::endl;
...
void Entry::fillFromInputFileSigma(const std::vector<ALIstring>& wordlist) {
  ...
  if (!ALIUtils::IsNumber(wordlist[2])) {
    if (parmgr->getParameterValue(wordlist[2], sig) == 0) {
      ...
      std::cerr << "!!! parameter for sigma not found: " << wordlist[0] << " " << wordlist[1] << " " << wordlist[2]
                << std::endl;
      exit(2);
    }
...
void Entry::fillFromInputFileQuality(const std::vector<ALIstring>& wordlist) {
  ...
  if (ALIUtils::debug >= 4)
    std::cout << OptOCurrent()->name() << " " << name() << " " << sigma_ << "QUALITY:" << quality_ << std::endl;
...
void Entry::fillFromReportOutFileValue(EntryData* entryData) {
  ...
  if (type_ == "centre" || type_ == "length") {
    ...
    if (ALIUtils::debug >= 5)
      std::cout << " fillFromReportOut " << OptOCurrent()->name() << " " << name() << "" << value_ << " disp "
                << entryData->valueDisplacement() * entryMgr->getDimOutLengthVal() << std::endl;
    ...
    if (ALIUtils::debug >= 5)
      std::cout << " fillFromReportOut " << OptOCurrent()->name() << " " << name() << "" << value_ << " disp "
                << entryData->valueDisplacement() * entryMgr->getDimOutAngleVal() << std::endl;
  }
...
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
void Entry::fillFromReportOutFileQuality(const EntryData* entryData) { quality_ = entryData->quality(); }

//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
//@@ Fill the name (in derived classes is not simply calling setName)
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
void Entry::fillName(const ALIstring& name) { setName(name); }
...
void Entry::displace(ALIdouble disp) {
  if (ALIUtils::debug >= 9)
    std::cout << "ExtraEntry::Displace" << disp << std::endl;
...
void Entry::displaceOriginal(ALIdouble disp) {
  if (ALIUtils::debug >= 9)
    std::cout << "ExtraEntry::DisplaceOriginal" << disp << std::endl;
  ALIuint entryNo = OptOCurrent()->extraEntryNo(name());
...
void Entry::displaceOriginalOriginal(ALIdouble disp) {
  if (ALIUtils::debug >= 9)
    std::cout << "ExtraEntry::DisplaceOriginalOriginal" << disp << std::endl;
  ALIuint entryNo = OptOCurrent()->extraEntryNo(name());
...
void Entry::addFittedDisplacementToValue(const ALIdouble val) {
  ...
  if (ALIUtils::debug >= 3)
    std::cout << OptOCurrent()->name() << " " << name() << " Entry::addFittedDisplacementToValue " << val
              << " total= " << valueDisplacementByFitting_ << std::endl;
...
ALIdouble Entry::valueDisplaced() const {
  ALIuint entryNo = OptOCurrent()->extraEntryNo(name());
  if (ALIUtils::debug >= 5)
    std::cout << entryNo << " Entry::valueDisplaced " << name() << " in " << OptOCurrent()->name() << " orig "
              << OptOCurrent()->ExtraEntryValueOriginalList()[entryNo] << " new "
              << OptOCurrent()->ExtraEntryValueList()[entryNo] << std::endl;
  return OptOCurrent()->ExtraEntryValueList()[entryNo] - OptOCurrent()->ExtraEntryValueOriginalList()[entryNo];
}

//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
void Entry::resetValueDisplacementByFitting() { valueDisplacementByFitting_ = 0.; }
...
std::ostream& operator<<(std::ostream& os, const Entry& c) {
  os << "ENTRY: " << c.name() << " of type: " << c.type() << std::endl
     ...
     << " fitpos " << c.fitPos_ << " valueDisplacementByFitting " << c.valueDisplacementByFitting_
     << " lastAdditionToValueDisplacementByFitting " << c.lastAdditionToValueDisplacementByFitting_ << std::endl;
