...
void OptOCOPS::defaultBehaviour(LightRay& lightray, Measurement& meas) {
  if (ALIUtils::debug >= 5)
    std::cout << "***** OptOCOPS::defaultBehaviour" << std::endl;
...
void OptOCOPS::makeMeasurement(LightRay& lightray, Measurement& meas) {
  if (ALIUtils::debug >= 4)
    std::cout << "***** OptOCOPS::makeMeasurement(lightray, meas) " << std::endl;
  ...
  if (posx12 == 0.)
    posx12 = -0.045;  //samir changed sign to correct the dowel 1st pixel
  ...
  if (CCDlength == 0.)
    CCDlength = 2048 * 14 * 1.E-6;  // (in meters, the default unit)
  ...
  */
  ...
  if (ALIUtils::debug >= 2)
    std::cout << "***** OptOCOPS::makeMeasurement  - identify pathological cases U and D intersected by same line"
              << std::endl;
  ALIbool xlaserDir[4];
  for (ii = 0; ii < 4; ii++) {
  ...
  if (xlaserDir[0] ^ xlaserDir[1]) {
    std::cerr << "!!EXITING up and down CCDs intersected by different x-laser line " << xlaserDir[0] << " "
              << xlaserDir[1] << std::endl;
    exit(1);
  }
  if (xlaserDir[2] ^ xlaserDir[3]) {
    std::cerr << "!!EXITING right and left CCDs intersected by different x-laser line " << xlaserDir[0] << " "
              << xlaserDir[1] << std::endl;
    exit(1);
  }

  if (ALIUtils::debug >= 5)
    std::cout << "***** OptOCOPS::makeMeasurement - now output sim values" << std::endl;

  if (ALIUtils::debug >= 1) {
    ALIstring chrg = "";
    std::cout << "REAL value: " << chrg << "U: " << 1000 * meas.value()[0] << chrg << " D: " << 1000 * meas.value()[1]
              << " L: " << 1000 * meas.value()[2] << " R: " << 1000 * meas.value()[3] << " (mm)  " << (this)->name()
              << std::endl;
    ALIdouble detU = 1000 * meas.valueSimulated(0);
    if (std::abs(detU) <= 1.e-9)
      detU = 0.;
    ALIdouble detD = 1000 * meas.valueSimulated(1);
    if (std::abs(detD) <= 1.e-9)
      detD = 0.;
    ALIdouble detL = 1000 * meas.valueSimulated(2);
    if (std::abs(detL) <= 1.e-9)
      detL = 0.;
    ALIdouble detR = 1000 * meas.valueSimulated(3);
    if (std::abs(detR) <= 1.e-9)
      detR = 0.;
    std::cout << "SIMU value: " << chrg
              << "U: "
              // << setprecision(3) << setw(4)
              << detU << chrg << " D: " << detD << chrg << " L: " << detL << chrg << " R: " << detR << " (mm)  "
              << (this)->name() << std::endl;
  }
}
...
void OptOCOPS::fastTraversesLightRay(LightRay& lightray) {
  if (ALIUtils::debug >= 5)
    std::cout << "***** OptOCOPS::fastTraversesLightRay" << std::endl;
  if (ALIUtils::debug >= 2)
    std::cout << "LR: FAST TRAVERSES COPS  " << name() << std::endl;
...
ALIdouble* OptOCOPS::convertPointToLocalCoordinates(const CLHEP::Hep3Vector& point) {
  if (ALIUtils::debug >= 1)
    std::cout << "***** OptOCOPS::convertPointToLocalCoordinates" << std::endl;
...
ALIdouble OptOCOPS::getMeasFromInters(ALILine& line_xhair, ALILine& ccd, CLHEP::Hep3Vector& cops_line) {
  if (ALIUtils::debug >= 5)
    std::cout << "***** OptOCOPS::getMeasFromInters" << std::endl;
