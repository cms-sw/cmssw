...
namespace gs {
template <class T> struct IOIsExternal {
  enum { value = 0 };
};
} // namespace gs
...
#define gs_declare_type_external(T) /**/                                       \
  namespace gs {                                                               \
  template <> struct IOIsExternal<T> {                                         \
    enum { value = 1 };                                                        \
  };                                                                           \
  template <> struct IOIsExternal<T const> {                                   \
    enum { value = 1 };                                                        \
  };                                                                           \
  template <> struct IOIsExternal<T volatile> {                                \
    enum { value = 1 };                                                        \
  };                                                                           \
  template <> struct IOIsExternal<T const volatile> {                          \
    enum { value = 1 };                                                        \
  };                                                                           \
  }
