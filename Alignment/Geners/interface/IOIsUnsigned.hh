...
namespace gs {
template <class T> struct IOIsUnsigned {
  enum { value = 0 };
};
} // namespace gs
...
#define gs_declare_type_as_unsigned(T) /**/                                    \
  namespace gs {                                                               \
  template <> struct IOIsUnsigned<T> {                                         \
    enum { value = 1 };                                                        \
  };                                                                           \
  template <> struct IOIsUnsigned<T const> {                                   \
    enum { value = 1 };                                                        \
  };                                                                           \
  template <> struct IOIsUnsigned<T volatile> {                                \
    enum { value = 1 };                                                        \
  };                                                                           \
  template <> struct IOIsUnsigned<T const volatile> {                          \
    enum { value = 1 };                                                        \
  };                                                                           \
  }
