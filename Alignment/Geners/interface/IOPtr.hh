...
public:
  ...
  inline T *const &getIOReference() const {
    if (!ptr_)
      throw gs::IOInvalidArgument("In gs::IOPtr::getIOReference: unusable "
...
public:
  ...
  inline T *const &getIOReference() const {
    if (!ptr_)
      throw gs::IOInvalidArgument("In gs::IOProxy::getIOReference: unusable "
                                  "const reference to null pointer");
    return ptr_;
  }
...
template <typename T> inline IOPtr<T> make_IOPtr(T &obj) {
  return IOPtr<T>(&obj);
}
...
//
template <typename T> inline IOProxy<T> make_IOProxy(T &obj, const char *name) {
