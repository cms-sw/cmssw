...
namespace gs {
template <typename T> class Reference : public AbsReference {
public:
  ...
  inline Reference(AbsArchive &ar, const std::string &name,
                   const char *category)
      : AbsReference(ar, ClassId::makeId<T>(), "gs::Single", name.c_str(),
                     category) {}

  inline Reference(AbsArchive &ar, const char *name,
                   const std::string &category)
      : AbsReference(ar, ClassId::makeId<T>(), "gs::Single", name,
                     category.c_str()) {}
  ...
  inline Reference(AbsArchive &ar, const SearchSpecifier &namePattern,
      ...
      : AbsReference(ar, ClassId::makeId<T>(), "gs::Single", namePattern,
                     categoryPattern) {}
...
inline T *Reference<T>::getPtr(const unsigned long index) const {
  ...
  if (GenericReader<std::istream, std::vector<ClassId>, T *,
          ...
          process(barePtr, this->positionInputStream(itemId), &state, true))
    assert(barePtr);
  else {
    delete barePtr;
    barePtr = 0;
...
template <typename T>
inline CPP11_shared_ptr<T>
Reference<T>::getShared(const unsigned long index) const {
