...
namespace gs {
template <class T> struct IOTraits {
  ...
  static const bool IsPOD =
      (IOIsPOD<T>::value && !IOIsContainer<T>::value &&
       !CPP11_is_pointer<T>::value && !IOIsExternal<T>::value);
  enum { ISPOD = 2 };

  static const bool IsWritable =
      (IOIsWritable<T>::value && !IOIsExternal<T>::value);
  enum { ISWRITABLE = 4 };

  static const bool IsStdContainer =
      (IOIsContainer<T>::value && !IOIsWritable<T>::value &&
       !IOIsExternal<T>::value);
  enum { ISSTDCONTAINER = 8 };

  // Readable objects are required to be writable
  static const bool IsPlaceReadable =
      (IOIsPlaceReadable<T>::value && IOIsWritable<T>::value);
  enum { ISPLACEREADABLE = 16 };

  // Prefer place readability to heap readability
  static const bool IsHeapReadable =
      (IOIsHeapReadable<T>::value && !IOIsPlaceReadable<T>::value &&
  ...
  static const bool IsSharedPtr =
      (IOIsSharedPtr<T>::value && !IOIsExternal<T>::value);
  ...
  static const int Signature =
      IsClass * ISCLASS + IsPOD * ISPOD + IsWritable * ISWRITABLE +
      IsStdContainer * ISSTDCONTAINER + IsPlaceReadable * ISPLACEREADABLE +
      IsHeapReadable * ISHEAPREADABLE + IsPointer * ISPOINTER +
      IsSharedPtr * ISSHAREDPTR + IsPair * ISPAIR + IsString * ISSTRING +
      IsContiguous * ISCONTIGUOUS + IsTuple * ISTUPLE + IsIOPtr * ISIOPTR +
      IsExternal * ISEXTERNAL + IsPointer * ISNULLPOINTER +
      IsHeapReadable * ISPUREHEAPREADABLE;
};

template <class T> inline IOTraits<T> IOItemTraits(const T &) {
  return IOTraits<T>();
}
} // namespace gs
...
std::ostream &operator<<(std::ostream &os, const gs::IOTraits<T> &) {
  ...
  os << "IsClass = " << Tr::IsClass << ", IsPOD = " << Tr::IsPOD
     ...
     << ", IsSharedPtr = " << Tr::IsSharedPtr << ", IsPair = " << Tr::IsPair
     << ", IsString = " << Tr::IsString
     << ", IsContiguous = " << Tr::IsContiguous << ", IsTuple = " << Tr::IsTuple
     << ", IsIOPtr = " << Tr::IsIOPtr << ", IsExternal = " << Tr::IsExternal;
  return os;
}
