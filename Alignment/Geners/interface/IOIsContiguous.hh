...
namespace gs {
template <class T> struct IOIsContiguous {
  enum { value = 0 };
};
...
// having it here.
template <class T, class Alloc> struct IOIsContiguous<std::vector<T, Alloc>> {
  enum { value = 1 };
};
...
template <> struct IOIsContiguous<std::vector<bool>> {
  enum { value = 0 };
};

template <> struct IOIsContiguous<const std::vector<bool>> {
  enum { value = 0 };
};

template <> struct IOIsContiguous<volatile std::vector<bool>> {
  enum { value = 0 };
};

template <> struct IOIsContiguous<const volatile std::vector<bool>> {
  enum { value = 0 };
};
} // namespace gs
