...
namespace gs {
template <typename T> class IOIsHeapReadableHelper {
private:
...
public:
  enum {
    value = sizeof(IOIsHeapReadableHelper<T>::template test<T>(nullptr)) == 1
  };
};
...
template <typename T> struct IOIsHeapReadable<T, true> {
  enum {
    value = IOIsHeapReadableHelper<typename StrippedType<T>::type>::value
  };
};

template <typename T> class IOIsPlaceReadableHelper {
private:
...
template <typename T> struct IOIsPlaceReadable<T, true> {
  enum {
    value = IOIsPlaceReadableHelper<typename StrippedType<T>::type>::value
  };
};
} // namespace gs
