...
#define gs_specialize_complex_read_write(T) /**/                               \
  ...
  template <>                                                                  \
  inline void read_pod_array<std::complex<T>>(std::istream & in,               \
                                              std::complex<T> *pod,            \
                                              const unsigned long len) {       \
    if (len) {                                                                 \
      assert(pod);                                                             \
...
gs_specialize_complex_read_write(float) gs_specialize_complex_read_write(double)
    gs_specialize_complex_read_write(long double)
        ...
        namespace gs {
  template <typename T> struct IOIsPOD<std::complex<T>> {
    enum { value = 1 };
  };

  template <typename T> struct IOIsPOD<const std::complex<T>> {
    enum { value = 1 };
  };

  template <typename T> struct IOIsPOD<volatile std::complex<T>> {
    enum { value = 1 };
  };

  template <typename T> struct IOIsPOD<const volatile std::complex<T>> {
    enum { value = 1 };
  };

  template <typename T> class IOIsContainer<std::complex<T>> {
  public:
    enum { value = 0 };
  };

  template <typename T> class IOIsContainer<const std::complex<T>> {
  public:
    enum { value = 0 };
  };

  template <typename T> class IOIsContainer<volatile std::complex<T>> {
  public:
    enum { value = 0 };
  };

  template <typename T> class IOIsContainer<const volatile std::complex<T>> {
  public:
    enum { value = 0 };
  };
}
