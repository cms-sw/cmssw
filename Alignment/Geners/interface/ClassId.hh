...
public:
  ...
  template <class T> inline ClassId(const T &) {
  ...
  // member methods).
  template <class T> static ClassId makeId();

  // "Universal" item id which also works for built-in types
  template <class T> static ClassId itemId(const T &);
...
template <class T> struct ClassIdSpecialization {
...
#define gs_specialize_template_id_T(name, version, MAX) /**/                   \
  namespace gs {                                                               \
  gs_specialize_template_help_T(GENERS_EMPTY_TYPE_QUALIFYER_, name, version,   \
                                MAX)                                           \
      gs_specialize_template_help_T(const, name, version, MAX)                 \
          gs_specialize_template_help_T(volatile, name, version, MAX)          \
              gs_specialize_template_help_T(const volatile, name, version,     \
                                            MAX)                               \
  }
...
#define gs_specialize_template_id_TT(name, version, MAX) /**/                  \
  namespace gs {                                                               \
  gs_specialize_template_help_TT(GENERS_EMPTY_TYPE_QUALIFYER_, name, version,  \
                                 MAX)                                          \
      gs_specialize_template_help_TT(const, name, version, MAX)                \
          gs_specialize_template_help_TT(volatile, name, version, MAX)         \
              gs_specialize_template_help_TT(const volatile, name, version,    \
                                             MAX)                              \
  }
...
#define gs_specialize_template_help_TTT(qualifyer, name, version, MAX) /**/    \
  ...
  struct ClassIdSpecialization<qualifyer name<T, U, V>> {                      \
    inline static ClassId classId(const bool isPtr = false) {                  \
      return ClassId(template_class_name<T, U, V>(#name, MAX), version,        \
                     isPtr);                                                   \
    }                                                                          \
  };
...
#define gs_specialize_template_id_TTT(name, version, MAX) /**/                 \
  namespace gs {                                                               \
  gs_specialize_template_help_TTT(GENERS_EMPTY_TYPE_QUALIFYER_, name, version, \
                                  MAX)                                         \
      gs_specialize_template_help_TTT(const, name, version, MAX)               \
          gs_specialize_template_help_TTT(volatile, name, version, MAX)        \
              gs_specialize_template_help_TTT(const volatile, name, version,   \
                                              MAX)                             \
  }
...
#define gs_specialize_template_help_TTTT(qualifyer, name, version, MAX) /**/   \
  ...
  struct ClassIdSpecialization<qualifyer name<T, U, V, X>> {                   \
    inline static ClassId classId(const bool isPtr = false) {                  \
      return ClassId(template_class_name<T, U, V, X>(#name, MAX), version,     \
                     isPtr);                                                   \
    }                                                                          \
  };
...
#define gs_specialize_template_id_TTTT(name, version, MAX) /**/                \
  namespace gs {                                                               \
  gs_specialize_template_help_TTTT(GENERS_EMPTY_TYPE_QUALIFYER_, name,         \
                                   version, MAX)                               \
      gs_specialize_template_help_TTTT(const, name, version, MAX)              \
          gs_specialize_template_help_TTTT(volatile, name, version, MAX)       \
              gs_specialize_template_help_TTTT(const volatile, name, version,  \
                                               MAX)                            \
  }
...
#define gs_specialize_template_hlp_TTTTT(qualifyer, name, version, MAX) /**/   \
  ...
  struct ClassIdSpecialization<qualifyer name<T, U, V, X, Y>> {                \
    inline static ClassId classId(const bool isPtr = false) {                  \
      return ClassId(template_class_name<T, U, V, X, Y>(#name, MAX), version,  \
                     isPtr);                                                   \
    }                                                                          \
  };
...
#define gs_specialize_template_id_TTTTT(name, version, MAX) /**/               \
  namespace gs {                                                               \
  gs_specialize_template_hlp_TTTTT(GENERS_EMPTY_TYPE_QUALIFYER_, name,         \
                                   version, MAX)                               \
      gs_specialize_template_hlp_TTTTT(const, name, version, MAX)              \
          gs_specialize_template_hlp_TTTTT(volatile, name, version, MAX)       \
              gs_specialize_template_hlp_TTTTT(const volatile, name, version,  \
                                               MAX)                            \
  }
...
#define gs_specialize_template_h_TTTTTT(qualifyer, name, version, MAX) /**/    \
  ...
  struct ClassIdSpecialization<qualifyer name<T, U, V, X, Y, Z>> {             \
    inline static ClassId classId(const bool isPtr = false) {                  \
      return ClassId(template_class_name<T, U, V, X, Y, Z>(#name, MAX),        \
                     version, isPtr);                                          \
    }                                                                          \
  };
...
#define gs_specialize_template_id_TTTTTT(name, version, MAX) /**/              \
  namespace gs {                                                               \
  gs_specialize_template_h_TTTTTT(GENERS_EMPTY_TYPE_QUALIFYER_, name, version, \
                                  MAX)                                         \
      gs_specialize_template_h_TTTTTT(const, name, version, MAX)               \
          gs_specialize_template_h_TTTTTT(volatile, name, version, MAX)        \
              gs_specialize_template_h_TTTTTT(const volatile, name, version,   \
                                              MAX)                             \
  }
...
#define gs_specialize_template_id_TN(name, version, MAX) /**/                  \
  namespace gs {                                                               \
  gs_specialize_template_help_TN(GENERS_EMPTY_TYPE_QUALIFYER_, name, version,  \
                                 MAX)                                          \
      gs_specialize_template_help_TN(const, name, version, MAX)                \
          gs_specialize_template_help_TN(volatile, name, version, MAX)         \
              gs_specialize_template_help_TN(const volatile, name, version,    \
                                             MAX)                              \
  }
...
template <class T> struct ClassIdSpecialization<T &> {
  inline static ClassId classId(const bool isPtr = false) {
...
template <class T> struct ClassIdSpecialization<T *const> {
  inline static ClassId classId(const bool /* isPtr */ = false) {
    return ClassIdSpecialization<T>::classId(true);
  }
};

template <class T> struct ClassIdSpecialization<T *volatile> {
  inline static ClassId classId(const bool /* isPtr */ = false) {
    return ClassIdSpecialization<T>::classId(true);
  }
};

template <class T> struct ClassIdSpecialization<T *const volatile> {
  inline static ClassId classId(const bool /* isPtr */ = false) {
    return ClassIdSpecialization<T>::classId(true);
  }
};
...
template <class T> struct ClassIdSpecialization<CPP11_shared_ptr<T>> {
  inline static ClassId classId(const bool /* isPtr */ = false) {
    return ClassIdSpecialization<T>::classId(true);
  }
};

template <class T> struct ClassIdSpecialization<const CPP11_shared_ptr<T>> {
  inline static ClassId classId(const bool /* isPtr */ = false) {
    return ClassIdSpecialization<T>::classId(true);
  }
};

template <class T> struct ClassIdSpecialization<volatile CPP11_shared_ptr<T>> {
  inline static ClassId classId(const bool /* isPtr */ = false) {
    return ClassIdSpecialization<T>::classId(true);
  }
};
...
template <class T> struct ClassIdSpecialization<IOPtr<T>> {
  inline static ClassId classId(const bool isPtr = false) {
    return ClassIdSpecialization<T>::classId(isPtr);
  }
};

template <class T> struct ClassIdSpecialization<const IOPtr<T>> {
  inline static ClassId classId(const bool isPtr = false) {
    return ClassIdSpecialization<T>::classId(isPtr);
  }
};

template <class T> struct ClassIdSpecialization<volatile IOPtr<T>> {
  inline static ClassId classId(const bool isPtr = false) {
    return ClassIdSpecialization<T>::classId(isPtr);
  }
};

template <class T> struct ClassIdSpecialization<const volatile IOPtr<T>> {
  inline static ClassId classId(const bool isPtr = false) {
    return ClassIdSpecialization<T>::classId(isPtr);
  }
};
...
template <class T> struct ClassIdSpecialization<IOProxy<T>> {
  inline static ClassId classId(const bool isPtr = false) {
    return ClassIdSpecialization<T>::classId(isPtr);
  }
};

template <class T> struct ClassIdSpecialization<const IOProxy<T>> {
  inline static ClassId classId(const bool isPtr = false) {
    return ClassIdSpecialization<T>::classId(isPtr);
  }
};

template <class T> struct ClassIdSpecialization<volatile IOProxy<T>> {
  inline static ClassId classId(const bool isPtr = false) {
    return ClassIdSpecialization<T>::classId(isPtr);
  }
};

template <class T> struct ClassIdSpecialization<const volatile IOProxy<T>> {
  inline static ClassId classId(const bool isPtr = false) {
    return ClassIdSpecialization<T>::classId(isPtr);
  }
};
...
template <class T> inline ClassId ClassId::makeId() {
  return ClassIdSpecialization<T>::classId();
}

namespace Private {
template <bool, class T> struct CallClassId {
  static inline ClassId get(const T &) {
    return ClassIdSpecialization<T>::classId();
  }
};

template <class T> struct CallClassId<true, T> {
...
template <class Tp> class TypeHasClassIdHelper {
  template <typename T, T> struct TypeCheck;
  template <typename T> struct FcnType1 { typedef ClassId (T::*fptr)() const; };
...
template <class T, bool b = IOIsClassType<T>::value> struct TypeHasClassId {
  static const bool value = 0;
};

template <typename T> struct TypeHasClassId<T, true> {
  static const bool value = TypeHasClassIdHelper<T>::value;
};
} // namespace Private

template <class T> inline ClassId ClassId::itemId(const T &item) {
  // Make sure that item is not a pointer.
  static_assert((!IOIsAnyPtr<T>::value),
                "can not use pointers with this method");
...
gs_specialize_class_id(float, 0) gs_specialize_class_id(double, 0)
    gs_specialize_class_id(long double, 0) gs_specialize_class_id(int, 0)
        gs_specialize_class_id(unsigned, 0) gs_specialize_class_id(long, 0)
            gs_specialize_class_id(long long,
                                   0) gs_specialize_class_id(unsigned long, 0)
                gs_specialize_class_id(unsigned long long,
                                       0) gs_specialize_class_id(short, 0)
                    gs_specialize_class_id(unsigned short,
                                           0) gs_specialize_class_id(bool, 0)
                ...
                gs_specialize_template_id_T(std::char_traits, 0, 1)
                    gs_specialize_template_id_TT(std::vector, 0, 1)
                        gs_specialize_template_id_TT(std::pair, 0, 2)
                            gs_specialize_template_id_TTT(std::basic_string, 0,
                                                          2)
