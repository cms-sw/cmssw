...
template <typename T> inline void write_pod(std::ostream &of, const T &pod) {
  of.write(reinterpret_cast<const char *>(&pod), sizeof(T));
}

template <typename T> inline void read_pod(std::istream &in, T *pod) {
  assert(pod);
...
template <>
inline void read_pod_array<std::string>(std::istream &in, std::string *pod,
                                        const unsigned long len) {
  if (len) {
    assert(pod);
    for (unsigned long i = 0; i < len; ++i)
...
//
template <typename T> inline bool write_obj(std::ostream &of, const T &obj) {
  return obj.classId().write(of) && obj.write(of);
}

template <typename T> inline CPP11_auto_ptr<T> read_obj(std::istream &in) {
  const ClassId id(in, 1);
  return CPP11_auto_ptr<T>(T::read(id, in));
}

template <typename T> inline void restore_obj(std::istream &in, T *obj) {
  assert(obj);
  const ClassId id(in, 1);
  T::restore(id, in, obj);
}
...
template <typename T>
inline void read_heap_obj_array(std::istream &in, CPP11_shared_ptr<T> *arr,
                                const unsigned long len) {
  if (len) {
    assert(arr);
    const ClassId id(in, 1);
...
template <typename Reader>
inline void
read_base_obj_array(std::istream &in, const Reader &f,
