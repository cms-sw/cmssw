...
template <typename T> class ArrayAdaptor {
public:
...
template <class T> struct IOIsContiguous<ArrayAdaptor<T>> {
  enum { value = 1 };
};

template <class T> struct IOIsContiguous<const ArrayAdaptor<T>> {
  enum { value = 1 };
};

template <class T> struct IOIsContiguous<volatile ArrayAdaptor<T>> {
  enum { value = 1 };
};

template <class T> struct IOIsContiguous<const volatile ArrayAdaptor<T>> {
  enum { value = 1 };
};

template <class T> struct InsertContainerItem<ArrayAdaptor<T>> {
...
template <class T> struct InsertContainerItem<volatile ArrayAdaptor<T>> {
  typedef ArrayAdaptor<T> A;
  static inline void insert(A &obj, const typename A::value_type &item,
                            const std::size_t itemNumber) {
    obj.at(itemNumber) = item;
...
struct GenericWriter<Stream, State, ArrayAdaptor<T>, InPODArray> {
  inline static bool process(const ArrayAdaptor<T> &a, Stream &os, State *,
                             bool) {
...
struct GenericWriter<Stream, State, ArrayAdaptor<T>, InContainerHeader> {
  ...
  inline static bool process(const Container &c, Stream &os, State *,
                             const bool processClassId) {
    bool status =
        processClassId ? ClassId::makeId<Container>().write(os) : true;
