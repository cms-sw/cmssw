...
CatalogEntry::CatalogEntry()
    : ItemDescriptor(), id_(0), len_(0),
      location_(ItemLocation(std::streampos(0), nullptr)) {}

CatalogEntry::CatalogEntry(const ItemDescriptor &r, const unsigned long long id,
                           const unsigned compressionCod,
                           const unsigned long long itemLength,
                           const ItemLocation &location,
                           const unsigned long long offset)
    : ItemDescriptor(r), id_(id), len_(itemLength), offset_(offset),
      compressionCode_(compressionCod), location_(location) {
  if (!id)
...
bool CatalogEntry::isEqual(const ItemDescriptor &other) const {
  ...
  return id_ == r.id_ && len_ == r.len_ && offset_ == r.offset_ &&
         compressionCode_ == r.compressionCode_ && location_ == r.location_;
}
...
CatalogEntry *CatalogEntry::read(const ClassId &id, const ClassId &locId,
                                 std::istream &in) {
  ...
  if (!in.fail()) {
    ...
    if (loc.get())
      rec = new CatalogEntry(ItemDescriptor(itemClass, ioPrototype.c_str(),
