...
public:
  inline NotWritableRecord(const gs::ClassId &classId, const char *ioPrototype,
...
AbsArchive::AbsArchive(const char *name)
    : name_(name ? name : ""), lastItemId_(0), lastItemLength_(0) {}
...
unsigned long long AbsArchive::copyItem(const unsigned long long id,
  ...
  NotWritableRecord record(entry->type(), entry->ioPrototype().c_str(), name,
                           category);
...
static std::string local_error_message(gs::AbsArchive &ar,
  ...
  err << "In operator<<(gs::AbsArchive& ar, const gs::AbsRecord& record): "
      << "failed to " << failedAction << " to the archive \"" << ar.name()
      << "\" for item with type \"" << record.type().name() << "\", name \""
      << record.name() << "\", and category \"" << record.category() << '"';
  return err.str();
}
...
gs::AbsArchive &operator<<(gs::AbsArchive &ar, const gs::AbsRecord &record) {
  ...
  if (os.fail())
    throw gs::IOWriteFailure(
        local_error_message(ar, record, "transfer compressed data"));
  ...
  if (id == 0ULL)
    throw gs::IOWriteFailure(
        local_error_message(ar, record, "add catalog entry"));
