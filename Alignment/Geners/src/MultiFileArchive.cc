...
MultiFileArchive::MultiFileArchive(const char *basename, const char *mode,
    ...
    : BinaryArchiveBase(basename, mode), filebuf_(nullptr), readbuf_(nullptr),
      catabuf_(nullptr), annotation_(ann ? std::string(ann) : std::string("")),
      catalogFileName_(AbsArchive::name() + ".gsbmf"), // binary metafile
      writeFileURI_("/ / / / / / /\\ \\ \\ \\"), readFileURI_(writeFileURI_),
      lastpos_(0), jumppos_(0),
      maxpos_(std::streamoff(1048576LL * typicalFileSizeInMB)),
      writeFileNumber_(0), catalogMergeLevel_(1), annotationsMerged_(false),
      streamFlushed_(true) {
  if (!modeValid())
    return;
...
void MultiFileArchive::updateReadStream(const std::string &uri) {
  ...
  separateReadStream_.open(readFileName_.c_str(),
...
unsigned long long
MultiFileArchive::addToCatalog(const AbsRecord &record,
