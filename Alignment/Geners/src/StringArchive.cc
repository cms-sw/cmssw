...
bool StringArchive::isEqual(const AbsArchive &cata) const {
  ...
  return lastpos_ == r.lastpos_ && name() == r.name() && stream_ == r.stream_ &&
         catalog_ == r.catalog_;
}
...
bool StringArchive::write(std::ostream &of) const {
  ...
  write_pod(of, name());
  return !of.fail() && stream_.classId().write(of) && stream_.write(of) &&
         catalog_.classId().write(of) && catalog_.write(of);
}
