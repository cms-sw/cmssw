...
ItemDescriptor::ItemDescriptor(const ClassId &classId, const char *ioPrototype,
                               const char *name, const char *categ)
    : classId_(classId), ioProto_(ioPrototype ? ioPrototype : ""),
...
bool ItemDescriptor::isSameClassIdandIO(const ItemDescriptor &r) const {
  return !classId_.name().empty() && classId_.name() == r.classId_.name() &&
         ioProto_ == r.ioProto_;
}

bool ItemDescriptor::isEqual(const ItemDescriptor &r) const {
  return !classId_.name().empty() && classId_ == r.classId_ &&
         ioProto_ == r.ioProto_ && nameCat_ == r.nameCat_;
}
} // namespace gs
