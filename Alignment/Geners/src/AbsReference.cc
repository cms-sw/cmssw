...
namespace gs {
std::istream &
AbsReference::positionInputStream(const unsigned long long id) const {
...
bool AbsReference::isIOCompatible(const CatalogEntry &r) const {
  return !classId_.name().empty() && classId_.name() == r.type().name() &&
...
AbsReference::AbsReference(AbsArchive &ar, const ClassId &classId,
    ...
    : archive_(ar), classId_(classId), ioProto_(ioPrototype ? ioPrototype : ""),
      searchId_(itemId), namePattern_(nullptr), categoryPattern_(nullptr),
      itemId_(0), initialized_(false) {
  if (!itemId)
...
AbsReference::AbsReference(AbsArchive &ar, const ClassId &classId,
    ...
    : archive_(ar), classId_(classId), ioProto_(ioPrototype ? ioPrototype : ""),
      searchId_(0), namePattern_(namePattern),
      categoryPattern_(categoryPattern), itemId_(0), initialized_(false) {}
} // namespace gs
