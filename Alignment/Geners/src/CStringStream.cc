...
static void doZlibCompression(const char *data, const unsigned long long len,
                              const bool defl, z_stream_s &strm, char *buffer,
...
static void doBZ2Compression(const char *data, const unsigned long long len,
                             const bool defl, bz_stream &strm, char *buffer,
                             const unsigned long long bufLen,
                             std::ostream &sink) {
  assert(buffer);
  assert(bufLen);
...
CStringStream::CStringStream(const CompressionMode m,
    ...
    : mode_(m), compressionLevel_(compressionLevel),
      ...
      // in a foot
      comprBuf_(bufSize > 1024U ? bufSize : 1024U), sink_(nullptr) {
  this->init(&buf_);
}
...
void CStringStream::readCompressed(std::istream &in,
  ...
  case BZIP2: {
    ...
    doBZ2Compression(&readBuf_[0], len, false, strm, &comprBuf_[0],
                     comprBuf_.size(), *this);
  } break;
...
CStringStream::CompressionMode CStringStream::writeCompressed() {
  ...
  case ZLIB: {
    ...
    doZlibCompression(data, len, true, deflator_->strm(), &comprBuf_[0],
                      comprBuf_.size(), *sink_);
  } break;
  ...
  case BZIP2: {
    ...
    doBZ2Compression(data, len, true, strm, &comprBuf_[0], comprBuf_.size(),
                     *sink_);
  } break;
