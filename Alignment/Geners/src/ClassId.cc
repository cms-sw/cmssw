...
void ClassId::ensureSameId(const ClassId &id) const {
  ...
  if (id_ != id.id_) {
    std::ostringstream os;
    os << "In gs::ClassId::ensureSameId: expected \"" << id_ << "\", got \""
       << id.id_ << "\"";
...
void ClassId::ensureSameName(const ClassId &id) const {
  ...
  if (name_ != id.name_) {
    std::ostringstream os;
    os << "In gs::ClassId::ensureSameName: expected class name \"" << name_
       << "\", got \"" << id.name_ << "\"";
    throw gs::IOInvalidArgument(os.str());
  }
}
...
void ClassId::ensureSameVersion(const ClassId &id) const {
  ...
  if (version_ != id.version_) {
    std::ostringstream os;
    os << "In gs::ClassId::ensureSameVersion: expected version " << version_
       << " for class " << id.name_ << ", got " << id.version_;
    throw gs::IOInvalidArgument(os.str());
  }
}
...
void ClassId::ensureVersionInRange(const unsigned vmin,
  ...
  if (version_ < vmin || version_ > vmax) {
    ...
    os << "In gs::ClassId::ensureVersionInRange: expected version"
       << " number for class " << name_ << " to be in range [" << vmin << ", "
       << vmax << "], got " << version_;
    throw gs::IOInvalidArgument(os.str());
  }
}
...
ClassId::ClassId(std::istream &in, int) {
  read_pod(in, &id_);
  if (in.fail())
    throw IOReadFailure("In gs::ClassId::ClassId(std::istream&, int): "
...
bool ClassId::isTemplate() const {
  ...
  return leftBrak != std::string::npos && rightBrak != std::string::npos &&
         leftBrak < rightBrak;
}
...
void ClassId::templateParameters(
  ...
  const std::size_t leftBrak = id_.find('<');
  const std::size_t rightBrak = id_.rfind('>');
  if (leftBrak != std::string::npos && rightBrak != std::string::npos &&
      leftBrak < rightBrak) {
