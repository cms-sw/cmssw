...
WriteOnlyCatalog::WriteOnlyCatalog(std::ostream &os,
    ...
    : AbsCatalog(), os_(os), count_(0), smallestId_(firstId ? firstId : 1ULL),
      largestId_(0) {}
...
unsigned long long WriteOnlyCatalog::makeEntry(const ItemDescriptor &descriptor,
  ...
  lastEntry_ = CPP11_auto_ptr<const CatalogEntry>(
      new CatalogEntry(descriptor, id, compressionCode, itemLen, loc, off));
...
WriteOnlyCatalog *WriteOnlyCatalog::read(const ClassId &id, std::istream &in) {
  ...
  CPP11_auto_ptr<WriteOnlyCatalog> cat(
...
WriteOnlyCatalog::retrieveEntry(unsigned long long) const {
...
void WriteOnlyCatalog::search(const SearchSpecifier &, const SearchSpecifier &,
