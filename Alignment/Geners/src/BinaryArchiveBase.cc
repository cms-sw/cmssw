...
static bool parse_unsigned(std::ostringstream &err, const char *c,
                           unsigned *result) {
  char *endptr;
  errno = 0;
...
BinaryArchiveBase::BinaryArchiveBase(const char *name, const char *mode)
    : AbsArchive(name), mode_(parseMode(mode)), errorStream_(nullptr),
      cStream_(nullptr), catalog_(nullptr), storedEntryId_(nullptr),
      storedLocationId_(nullptr), catalogIsSet_(false),
  ...
  modeIsValid_ =
      parseArchiveOptions(err, mode, &m, &compressionLevel, &minSizeToCompress,
                          &bufSize, &addCatalogToData_);
  if (modeIsValid_)
    cStream_ =
        new CStringStream(m, compressionLevel, minSizeToCompress, bufSize);
  else {
...
void BinaryArchiveBase::itemSearch(
    const SearchSpecifier &namePattern, const SearchSpecifier &categoryPattern,
...
bool BinaryArchiveBase::parseArchiveOptions(
    std::ostringstream &err, const char *modeIn,
    CStringStream::CompressionMode *m, int *compressionLevel,
    unsigned *minSizeToCompress, unsigned *bufSize, bool *multiplexCatalog) {
  if (!modeIn)
    return true;
...
void BinaryArchiveBase::search(AbsReference &reference) {
  if (catalog_) {
    ...
    catalog_->search(reference.namePattern(), reference.categoryPattern(),
                     &idlist);
