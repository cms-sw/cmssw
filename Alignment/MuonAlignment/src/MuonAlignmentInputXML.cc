...
//
// $Id: MuonAlignmentInputXML.cc,v 1.17 2011/06/07 19:38:24 khotilov Exp $
//
...
void MuonAlignmentInputXML::recursiveGetId(AliNavigator &alignableNavigator, const align::Alignables &alignables) const
{
...
void MuonAlignmentInputXML::fillAliToIdeal(Ali2Ideal &ali2ideal, const align::Alignables alignables, const align::Alignables ideals) const
{
  ...
  {
    ali2ideal[*alignable] = *ideal;
    fillAliToIdeal(ali2ideal, (*alignable)->components(), (*ideal)->components());
    ++alignable;
    ++ideal;
  }
...
AlignableMuon *MuonAlignmentInputXML::newAlignableMuon(const edm::EventSetup& iSetup) const
{
  ...
  AliNavigator alignableNavigator;  // real AlignableNavigators don't have const methods
  ...
  AliNavigator ideal_alignableNavigator;  // real AlignableNavigators don't have const methods
  ...
  {
    throw cms::Exception("XMLException") << "Xerces XML parser threw an exception on initialization.\n";
  }
  ...
  catch (const XMLException &toCatch)
  {
    ...
    throw cms::Exception("XMLException") << "Xerces XML parser threw this exception: " << message << "\n";
  }
  ...
  {
    throw cms::Exception("XMLException") << "Xerces XML parser threw an unknown exception\n";
  }
  ...
  Ali2Ideal ali2ideal;
  fillAliToIdeal(ali2ideal, alignableMuon->DTBarrel(), ideal_alignableMuon->DTBarrel());
  fillAliToIdeal(ali2ideal, alignableMuon->CSCEndcaps(), ideal_alignableMuon->CSCEndcaps());

  std::map<std::string, AliSet > alicollections;
  for (unsigned int i = 0;  i < collections->getLength();  i++)
  {
    ...
    if (collection->getParentNode() == node_MuonAlignment)
    {
      ...
      if (node_name == NULL) throw cms::Exception("XMLException") << "<collection> requires a name attribute\n";
      ...
      for (unsigned int j = 0;  j < children->getLength();  j++)
      {
        ...
        if (node->getNodeType() == DOMNode::ELEMENT_NODE)
        {
          ...
          if (ali == NULL)  throw cms::Exception("XMLException") << "<collection> must contain only alignables\n";
          aliset[ali] = true;
  ...
  for (unsigned int i = 0;  i < operations->getLength();  i++)
  {
    ...
    if (operation->getParentNode() != node_MuonAlignment)  throw cms::Exception("XMLException") << "All operations must be top-level elements\n";
    ...
    for (unsigned int j = 0;  j < children->getLength();  j++)
    {
      ...
      if (node->getNodeType() == DOMNode::ELEMENT_NODE) {
        ...
        else if (XMLString::equals(node->getNodeName(), str_collection))
        {
          ...
          std::map<std::string, AliSet >::const_iterator alicollections_iter = alicollections.find(name);
          if (alicollections_iter == alicollections.end()) throw cms::Exception("XMLException") << "<collection name=\"" << name << "\"> hasn't been defined\n";

          for (AliSet::const_iterator aliiter = alicollections_iter->second.begin(); aliiter != alicollections_iter->second.end(); ++aliiter)
          {
    ...
    {
      DOMNode *node = children->item(j);
      const XMLCh * nname = node->getNodeName();
      if (node->getNodeType() == DOMNode::ELEMENT_NODE)
      {
        if (XMLString::equals(nname, str_setposition))           do_setposition((DOMElement*)(node), aliset, ali2ideal);
        else if (XMLString::equals(nname, str_setape))           do_setape((DOMElement*)(node), aliset, ali2ideal);
        else if (XMLString::equals(nname, str_setsurveyerr))     do_setsurveyerr((DOMElement*)(node), aliset, ali2ideal);
        else if (XMLString::equals(nname, str_moveglobal))       do_moveglobal((DOMElement*)(node), aliset, ali2ideal);
        else if (XMLString::equals(nname, str_movelocal))        do_movelocal((DOMElement*)(node), aliset, ali2ideal);
        else if (XMLString::equals(nname, str_rotatelocal))      do_rotatelocal((DOMElement*)(node), aliset, ali2ideal);
        else if (XMLString::equals(nname, str_rotatebeamline))   do_rotatebeamline((DOMElement*)(node), aliset, ali2ideal);
        else if (XMLString::equals(nname, str_rotateglobalaxis)) do_rotateglobalaxis((DOMElement*)(node), aliset, ali2ideal);
        else throw cms::Exception("XMLException") << "Unrecognized operation: \"" << XMLString::transcode(nname) << "\"\n";
...
Alignable *MuonAlignmentInputXML::getNode(AliNavigator &alignableNavigator, const DOMElement *node) const
{
  const XMLCh * nname = node->getNodeName();
  if      (XMLString::equals(nname, str_DTBarrel))     return getDTnode(align::AlignableDTBarrel, alignableNavigator, node);
  else if (XMLString::equals(nname, str_DTWheel))      return getDTnode(align::AlignableDTWheel, alignableNavigator, node);
  else if (XMLString::equals(nname, str_DTStation))    return getDTnode(align::AlignableDTStation, alignableNavigator, node);
  else if (XMLString::equals(nname, str_DTChamber))    return getDTnode(align::AlignableDTChamber, alignableNavigator, node);
  else if (XMLString::equals(nname, str_DTSuperLayer)) return getDTnode(align::AlignableDTSuperLayer, alignableNavigator, node);
  else if (XMLString::equals(nname, str_DTLayer))      return getDTnode(align::AlignableDetUnit, alignableNavigator, node);
  else if (XMLString::equals(nname, str_CSCEndcap))  return getCSCnode(align::AlignableCSCEndcap, alignableNavigator, node);
  else if (XMLString::equals(nname, str_CSCStation)) return getCSCnode(align::AlignableCSCStation, alignableNavigator, node);
  else if (XMLString::equals(nname, str_CSCRing))    return getCSCnode(align::AlignableCSCRing, alignableNavigator, node);
  else if (XMLString::equals(nname, str_CSCChamber)) return getCSCnode(align::AlignableCSCChamber, alignableNavigator, node);
  else if (XMLString::equals(nname, str_CSCLayer))   return getCSCnode(align::AlignableDetUnit, alignableNavigator, node);
  else return NULL;
}


Alignable *MuonAlignmentInputXML::getDTnode(align::StructureType structureType, AliNavigator &alignableNavigator, const DOMElement *node) const
{
  ...
  if (node_rawId != NULL)
  {
    ...
    {
      throw cms::Exception("XMLException") << "Value of \"rawId\" must be an integer\n";
    }
  }
  else
  {
    ...
    if (structureType != align::AlignableDTBarrel)
    {
      ...
      if (node_wheel == NULL) throw cms::Exception("XMLException") << "DT node is missing required \"wheel\" attribute\n";
      try
      {
      ...
      {
        throw cms::Exception("XMLException") << "Value of \"wheel\" must be an integer\n";
      }

      if (structureType != align::AlignableDTWheel)
      {
        DOMAttr *node_station = node->getAttributeNode(str_station);
        if (node_station == NULL) throw cms::Exception("XMLException") << "DT node is missing required \"station\" attribute\n";
        try
        {
        ...
        {
          throw cms::Exception("XMLException") << "Value of \"station\" must be an integer\n";
        }

        if (structureType != align::AlignableDTStation)
        {
          DOMAttr *node_sector = node->getAttributeNode(str_sector);
          if (node_sector == NULL) throw cms::Exception("XMLException") << "DT node is missing required \"sector\" attribute\n";
          try
          {
          ...
          {
            throw cms::Exception("XMLException") << "Value of \"sector\" must be an integer\n";
          }

          if (structureType != align::AlignableDTChamber)
          {
            DOMAttr *node_superlayer = node->getAttributeNode(str_superlayer);
            if (node_superlayer == NULL) throw cms::Exception("XMLException") << "DT node is missing required \"superlayer\" attribute\n";
            try
            {
            ...
            {
              throw cms::Exception("XMLException") << "Value of \"superlayer\" must be an integer\n";
            }

            if (structureType != align::AlignableDTSuperLayer)
            {
              DOMAttr *node_layer = node->getAttributeNode(str_layer);
              if (node_layer == NULL) throw cms::Exception("XMLException") << "DT node is missing required \"layer\" attribute\n";
              try
              {
              ...
              {
                throw cms::Exception("XMLException") << "Value of \"layer\" must be an integer\n";
              }
  ...
  if (ali == NULL) throw cms::Exception("XMLException") << "rawId \"" << rawId << "\" is not recognized\n";
  ...
  while (ali->alignableObjectId() != structureType)
  {
    ...
    if (ali == NULL)
    {
      if (structureType == align::AlignableDTBarrel) throw cms::Exception("XMLException") << "rawId \"" << rawId << "\" is not a DTBarrel\n";
      else if (structureType == align::AlignableDTWheel) throw cms::Exception("XMLException") << "rawId \"" << rawId << "\" is not a DTWheel\n";
      else if (structureType == align::AlignableDTStation) throw cms::Exception("XMLException") << "rawId \"" << rawId << "\" is not a DTStation\n";
      else if (structureType == align::AlignableDTChamber) throw cms::Exception("XMLException") << "rawId \"" << rawId << "\" is not a DTChamber\n";
      else if (structureType == align::AlignableDTSuperLayer) throw cms::Exception("XMLException") << "rawId \"" << rawId << "\" is not a DTSuperLayer\n";
      else if (structureType == align::AlignableDetUnit) throw cms::Exception("XMLException") << "rawId \"" << rawId << "\" is not a DTLayer\n";
    }
  }
  return ali;
}
...
Alignable *MuonAlignmentInputXML::getCSCnode(align::StructureType structureType, AliNavigator &alignableNavigator, const DOMElement *node) const
{
  unsigned int rawId;
  ...
  else
  {
    ...
    if (node_endcap == NULL) throw cms::Exception("XMLException") << "CSC node is missing required \"endcap\" attribute\n";
    try
    {
    ...
    {
      throw cms::Exception("XMLException") << "Value of \"endcap\" must be an integer\n";
    }
    ...
    {
      DOMAttr *node_station = node->getAttributeNode(str_station);
      if (node_station == NULL) throw cms::Exception("XMLException") << "CSC node is missing required \"station\" attribute\n";
      try
      {
        station = XMLString::parseInt(node_station->getValue());
      }
      ...
      if (structureType != align::AlignableCSCStation)
      {
        ...
        if (node_ring == NULL) throw cms::Exception("XMLException") << "CSC node is missing required \"ring\" attribute\n";
        try
        {
        ...
        {
          throw cms::Exception("XMLException") << "Value of \"ring\" must be an integer\n";
        }

        if (structureType != align::AlignableCSCRing)
        {
          DOMAttr *node_chamber = node->getAttributeNode(str_chamber);
          if (node_chamber == NULL) throw cms::Exception("XMLException") << "CSC node is missing required \"chamber\" attribute\n";
          try
          {
          ...
          {
            throw cms::Exception("XMLException") << "Value of \"chamber\" must be an integer\n";
          }

          if (structureType != align::AlignableCSCChamber)
          {
            DOMAttr *node_layer = node->getAttributeNode(str_layer);
            if (node_layer == NULL) throw cms::Exception("XMLException") << "CSC node is missing required \"layer\" attribute\n";
            try
            {
              layer = XMLString::parseInt(node_layer->getValue());
            }
  ...
  while (ali->alignableObjectId() != structureType)
  {
    ...
    if (ali == NULL)
    {
      if (structureType == align::AlignableCSCEndcap) throw cms::Exception("XMLException") << "rawId \"" << rawId << "\" is not a CSCEndcap\n";
      else if (structureType == align::AlignableCSCStation) throw cms::Exception("XMLException") << "rawId \"" << rawId << "\" is not a CSCStation\n";
      else if (structureType == align::AlignableCSCRing) throw cms::Exception("XMLException") << "rawId \"" << rawId << "\" is not a CSCRing\n";
      else if (structureType == align::AlignableCSCChamber) throw cms::Exception("XMLException") << "rawId \"" << rawId << "\" is not a CSCChamber\n";
      else if (structureType == align::AlignableDetUnit) throw cms::Exception("XMLException") << "rawId \"" << rawId << "\" is not a CSCLayer\n";
    }
  }
  return ali;
}
...
void MuonAlignmentInputXML::do_setposition(const DOMElement *node, AliSet &aliset, Ali2Ideal &ali2ideal) const
{
  DOMAttr *node_relativeto = node->getAttributeNode(str_relativeto);
  if (node_relativeto == NULL) throw cms::Exception("XMLException") << "<setposition> is missing required \"relativeto\" attribute\n";

  int relativeto = 0;
  if (XMLString::equals(node_relativeto->getValue(), str_none))           relativeto = 0;
  else if (XMLString::equals(node_relativeto->getValue(), str_ideal))     relativeto = 1;
  else if (XMLString::equals(node_relativeto->getValue(), str_container)) relativeto = 2;
  else
  {
    char *message = XMLString::transcode(node_relativeto->getValue());
    throw cms::Exception("XMLException") << "relativeto must be \"none\", \"ideal\", or \"container\", not \"" << message << "\"\n";
  }

  for (AliSet::const_iterator aliiter = aliset.begin();  aliiter != aliset.end();  ++aliiter)
  {
    // first reconstruct the old position: how it would look in this coordinate system

    Alignable *ali = aliiter->first;
    Alignable *ideal = ali2ideal[ali];
    ...
    {
      throw cms::Exception("XMLException") << "<setposition> must either have phix, phiy, and phiz or alpha, beta, and gamma, but not both\n";
    }
    ...
    if (relativeto == 0)  // relativeto="none"
    {
      set_one_position(aliiter->first, pos, rot);
    }
    else if (relativeto == 1) //relativeto="ideal"
    {
      Alignable *ali = aliiter->first;
      Alignable *ideal = ali2ideal[ali];
    ...
    else if (relativeto == 2) // relativeto="container"
    {
      Alignable *ali = aliiter->first;
	...
	set_one_position(ali, newpos, newrot);
      ...
      else set_one_position(ali, pos, rot);
    }
...
//    }
  ...
  if (survey != NULL)  matrix6x6 = survey->errors();  // save the constraint information
  ali->setSurvey(new SurveyDet(ali->surface(), matrix6x6));
}


void MuonAlignmentInputXML::do_setape(const DOMElement *node, AliSet &aliset, Ali2Ideal &ali2ideal) const
{
  ...
  if (node_xx == NULL) throw cms::Exception("XMLException") << "<setape> is missing required \"xx\" attribute\n";
  if (node_xy == NULL) throw cms::Exception("XMLException") << "<setape> is missing required \"xy\" attribute\n";
  if (node_xz == NULL) throw cms::Exception("XMLException") << "<setape> is missing required \"xz\" attribute\n";
  if (node_yy == NULL) throw cms::Exception("XMLException") << "<setape> is missing required \"yy\" attribute\n";
  if (node_yz == NULL) throw cms::Exception("XMLException") << "<setape> is missing required \"yz\" attribute\n";
  if (node_zz == NULL) throw cms::Exception("XMLException") << "<setape> is missing required \"zz\" attribute\n";
...
void MuonAlignmentInputXML::do_setsurveyerr(const DOMElement *node, AliSet &aliset, Ali2Ideal &ali2ideal) const
{
  DOMAttr *node_xx = node->getAttributeNode(str_xx);
  DOMAttr *node_xy = node->getAttributeNode(str_xy);
  DOMAttr *node_xz = node->getAttributeNode(str_xz);
  ...
  if (node_xx == NULL) throw cms::Exception("XMLException") << "<setsurveyerr> is missing required \"xx\" attribute\n";
  if (node_xy == NULL) throw cms::Exception("XMLException") << "<setsurveyerr> is missing required \"xy\" attribute\n";
  if (node_xz == NULL) throw cms::Exception("XMLException") << "<setsurveyerr> is missing required \"xz\" attribute\n";
  if (node_xa == NULL) throw cms::Exception("XMLException") << "<setsurveyerr> is missing required \"xa\" attribute\n";
  if (node_xb == NULL) throw cms::Exception("XMLException") << "<setsurveyerr> is missing required \"xb\" attribute\n";
  if (node_xc == NULL) throw cms::Exception("XMLException") << "<setsurveyerr> is missing required \"xc\" attribute\n";
  if (node_yy == NULL) throw cms::Exception("XMLException") << "<setsurveyerr> is missing required \"yy\" attribute\n";
  if (node_yz == NULL) throw cms::Exception("XMLException") << "<setsurveyerr> is missing required \"yz\" attribute\n";
  if (node_ya == NULL) throw cms::Exception("XMLException") << "<setsurveyerr> is missing required \"ya\" attribute\n";
  if (node_yb == NULL) throw cms::Exception("XMLException") << "<setsurveyerr> is missing required \"yb\" attribute\n";
  if (node_yc == NULL) throw cms::Exception("XMLException") << "<setsurveyerr> is missing required \"yc\" attribute\n";
  if (node_zz == NULL) throw cms::Exception("XMLException") << "<setsurveyerr> is missing required \"zz\" attribute\n";
  if (node_za == NULL) throw cms::Exception("XMLException") << "<setsurveyerr> is missing required \"za\" attribute\n";
  if (node_zb == NULL) throw cms::Exception("XMLException") << "<setsurveyerr> is missing required \"zb\" attribute\n";
  if (node_zc == NULL) throw cms::Exception("XMLException") << "<setsurveyerr> is missing required \"zc\" attribute\n";
  if (node_aa == NULL) throw cms::Exception("XMLException") << "<setsurveyerr> is missing required \"aa\" attribute\n";
  if (node_ab == NULL) throw cms::Exception("XMLException") << "<setsurveyerr> is missing required \"ab\" attribute\n";
  if (node_ac == NULL) throw cms::Exception("XMLException") << "<setsurveyerr> is missing required \"ac\" attribute\n";
  if (node_bb == NULL) throw cms::Exception("XMLException") << "<setsurveyerr> is missing required \"bb\" attribute\n";
  if (node_bc == NULL) throw cms::Exception("XMLException") << "<setsurveyerr> is missing required \"bc\" attribute\n";
  if (node_cc == NULL) throw cms::Exception("XMLException") << "<setsurveyerr> is missing required \"cc\" attribute\n";
...
void MuonAlignmentInputXML::do_moveglobal(const DOMElement *node, AliSet &aliset, Ali2Ideal &ali2ideal) const
{
  DOMAttr *node_x = node->getAttributeNode(str_x);
  DOMAttr *node_y = node->getAttributeNode(str_y);
  DOMAttr *node_z = node->getAttributeNode(str_z);

  if (node_x == NULL) throw cms::Exception("XMLException") << "<moveglobal> is missing required \"x\" attribute\n";
  if (node_y == NULL) throw cms::Exception("XMLException") << "<moveglobal> is missing required \"y\" attribute\n";
  if (node_z == NULL) throw cms::Exception("XMLException") << "<moveglobal> is missing required \"z\" attribute\n";
...
void MuonAlignmentInputXML::do_movelocal(const DOMElement *node, AliSet &aliset, Ali2Ideal &ali2ideal) const
{
  DOMAttr *node_x = node->getAttributeNode(str_x);
  DOMAttr *node_y = node->getAttributeNode(str_y);
  DOMAttr *node_z = node->getAttributeNode(str_z);

  if (node_x == NULL) throw cms::Exception("XMLException") << "<movelocal> is missing required \"x\" attribute\n";
  if (node_y == NULL) throw cms::Exception("XMLException") << "<movelocal> is missing required \"y\" attribute\n";
  if (node_z == NULL) throw cms::Exception("XMLException") << "<movelocal> is missing required \"z\" attribute\n";
...
void MuonAlignmentInputXML::do_rotatelocal(const DOMElement *node, AliSet &aliset, Ali2Ideal &ali2ideal) const
{
  ...
  if (node_axisx == NULL) throw cms::Exception("XMLException") << "<rotatelocal> is missing required \"axisx\" attribute\n";
  if (node_axisy == NULL) throw cms::Exception("XMLException") << "<rotatelocal> is missing required \"axisy\" attribute\n";
  if (node_axisz == NULL) throw cms::Exception("XMLException") << "<rotatelocal> is missing required \"axisz\" attribute\n";
  if (node_angle == NULL) throw cms::Exception("XMLException") << "<rotatelocal> is missing required \"angle\" attribute\n";
...
void MuonAlignmentInputXML::do_rotatebeamline(const DOMElement *node, AliSet &aliset, Ali2Ideal &ali2ideal) const
{
  DOMAttr *node_rphi = node->getAttributeNode(str_rphi);
  DOMAttr *node_phi = node->getAttributeNode(str_phi);
  if (node_rphi == NULL  &&  node_phi == NULL) throw cms::Exception("XMLException") << "<rotatebeamline> is missing required \"*phi\" attribute\n";
  if (node_rphi != NULL  &&  node_phi != NULL) throw cms::Exception("XMLException") << "<rotatebeamline> can't have both an \"rphi\" and a \"phi\" attribute\n";
...
void MuonAlignmentInputXML::do_rotateglobalaxis(const DOMElement *node, AliSet &aliset, Ali2Ideal &ali2ideal) const
{
  DOMAttr *node_x = node->getAttributeNode(str_x);
  DOMAttr *node_y = node->getAttributeNode(str_y);
  DOMAttr *node_z = node->getAttributeNode(str_z);
  DOMAttr *node_angle = node->getAttributeNode(str_angle);
  if (node_x == NULL) throw cms::Exception("XMLException") << "<rotateglobalaxis> is missing required \"x\" attribute\n";
  if (node_y == NULL) throw cms::Exception("XMLException") << "<rotateglobalaxis> is missing required \"y\" attribute\n";
  if (node_z == NULL) throw cms::Exception("XMLException") << "<rotateglobalaxis> is missing required \"z\" attribute\n";
  if (node_angle == NULL) throw cms::Exception("XMLException") << "<rotateglobalaxis> is missing required \"angle\" attribute\n";
