...
// Dx plot
  ...
  makeGraph(sizeI, smi, sma, minV, maxV, dxh, grx, "delX_vs_position", "Local #delta X vs position",
...
// Dy plot
  ...
  makeGraph(sizeI, smi, sma, minV, maxV, dxh, grx, "delY_vs_position", "Local #delta Y vs position",
...
// Dz plot
  ...
  makeGraph(sizeI, smi, sma, minV, maxV, dxh, grx, "delZ_vs_position", "Local #delta Z vs position",
...
// Dphi plot
  ...
  makeGraph(sizeI, smi, sma, minV, maxV, dxh, grx, "delphi_vs_position", "#delta #phi vs position",
...
// Dr plot
  ...
  makeGraph(sizeI, smi, sma, minV, maxV, dxh, grx, "delR_vs_position", "#delta R vs position", "GdelR_vs_position",
...
// Drphi plot
  ...
  makeGraph(sizeI, smi, sma, minV, maxV, dxh, grx, "delRphi_vs_position", "R #delta #phi vs position",
...
// Dalpha plot
  ...
  makeGraph(sizeI, smi, sma, minV, maxV, dxh, grx, "delalpha_vs_position", "#delta #alpha vs position",
...
// Dbeta plot
  ...
  makeGraph(sizeI, smi, sma, minV, maxV, dxh, grx, "delbeta_vs_position", "#delta #beta vs position",
...
// Dgamma plot
  ...
  makeGraph(sizeI, smi, sma, minV, maxV, dxh, grx, "delgamma_vs_position", "#delta #gamma vs position",
...
// Drotx plot
  ...
  makeGraph(sizeI, smi, sma, minV, maxV, dxh, grx, "delrotX_vs_position", "#delta rotX vs position",
...
// Droty plot
  ...
  makeGraph(sizeI, smi, sma, minV, maxV, dxh, grx, "delrotY_vs_position", "#delta rotY vs position",
...
// Drotz plot
  ...
  makeGraph(sizeI, smi, sma, minV, maxV, dxh, grx, "delrotZ_vs_position", "#delta rotZ vs position",
...
{
  if (minV >= maxV || smi >= sma || sizeI <= 1 || xp == 0x0 || yp == 0x0) return; // out of bounds, bail
...
void MuonGeometryArrange::analyze(const edm::Event&, const edm::EventSetup& iSetup)
{
  if (firstEvent_)
  {
    ...
    Alignable* inputGeometry2Copy2 = static_cast< Alignable* >(inputAlign2a->getAlignableMuon());
...
void MuonGeometryArrange::compareGeometries(Alignable* refAli, Alignable* curAli, Alignable* curCopy)
{
  ...
  if (refComp.size() != curComp.size()) return;
...
//  CLHEP::Hep3Vector* Rsubtotal, Wsubtotal, DRsubtotal, DWsubtotal;
  ...
  for (int ich = 0; ich < nComp; ich++)
  {
    ...
    if (_weightById)
    {
      if (!align::readModuleList(curComp[ich]->id(), curComp[ich]->id(), _weightByIdVector)) useIt = false;
    }
    if (!useIt) continue;
  ...
  for (int ich = 0; ich < nComp; ich++)
  {
    if (_weightById)
    {
      if (!align::readModuleList(curComp[ich]->id(), curComp[ich]->id(), _weightByIdVector)) continue;
    }
    ...
    for (int i = 0; i < 100; i++)
    {
      AlgebraicVector diff = align::diffAlignables(refComp[ich], curComp[ich], _weightBy, _weightById,
                                                   _weightByIdVector);
      ...
      AlgebraicVector check = align::diffAlignables(refComp[ich], curComp[ich], _weightBy, _weightById,
                                                    _weightByIdVector);
...
//         << ", subdetId: "<< detid.subdetId() << "): " << diff;
  ...
  for (int ich = 0; ich < nComp; ich++)
  {
    ...
    for (int i = 0; i < 100; i++)
    {
      AlgebraicVector diff = align::diffAlignables(refComp[ich], curComp2[ich], _weightBy, _weightById,
                                                   _weightByIdVector);
      CLHEP::Hep3Vector dR(diff[0], diff[1], diff[2]);
      Rtotal += dR;
      CLHEP::Hep3Vector dW(diff[3], diff[4], diff[5]);
      ...
      float tolerance = 1e-7;
      AlgebraicVector check = align::diffAlignables(refComp[ich], curComp2[ich], _weightBy, _weightById,
                                                    _weightByIdVector);
      align::GlobalVector checkR(check[0], check[1], check[2]);
      align::GlobalVector checkW(check[3], check[4], check[5]);
      if ((checkR.mag() > tolerance) || (checkW.mag() > tolerance)) { }
...
bool MuonGeometryArrange::checkChosen(Alignable* ali)
{
  ...
  if (ali->geomDetId().det() != DetId::Muon || ali->geomDetId().subdetId() != MuonSubdetId::CSC) return false;
