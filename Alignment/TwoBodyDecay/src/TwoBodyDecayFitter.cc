...
TwoBodyDecayFitter::TwoBodyDecayFitter(
    const edm::ParameterSet &config, const LinearizationPointFinder *vf,
    const TwoBodyDecayLinearizationPointFinder *lpf,
    const TwoBodyDecayEstimator *est)
    : theVertexFinder(vf->clone()), theLinPointFinder(lpf->clone()),
      theEstimator(est->clone()) {}

TwoBodyDecayFitter::~TwoBodyDecayFitter(void) {}

const TwoBodyDecay
TwoBodyDecayFitter::estimate(const std::vector<reco::TransientTrack> &tracks,
  ...
  linTracks.push_back(
      theLinTrackStateFactory.linearizedTrackState(linVertex, tracks[0]));
  linTracks.push_back(
      theLinTrackStateFactory.linearizedTrackState(linVertex, tracks[1]));

  // get full linearization point (geomatrical & kinematical)
  const TwoBodyDecayParameters linPoint =
      theLinPointFinder->getLinearizationPoint(linTracks, vm.primaryMass(),
                                               vm.secondaryMass());
...
TwoBodyDecayFitter::estimate(const std::vector<reco::TransientTrack> &tracks,
  ...
  GlobalPoint linVertex =
      theVertexFinder->getLinearizationPoint(freeTrajStates);

  // create linearized track states
  std::vector<RefCountedLinearizedTrackState> linTracks;
  linTracks.push_back(theLinTrackStateFactory.linearizedTrackState(
      linVertex, tracks[0], tsos[0]));
  linTracks.push_back(theLinTrackStateFactory.linearizedTrackState(
      linVertex, tracks[1], tsos[1]));
