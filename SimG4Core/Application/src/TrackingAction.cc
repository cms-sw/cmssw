  ...
  if (eventAction_->trackContainer() != nullptr) {
...
#ifdef EDM_ML_DEBUG
      edm::LogVerbatim("SimTrackManager") << "TrackingAction addTkCaloStateInfo " << id << " of momentum " << mom << " at "
                                  << pos;
#endif
    }

    bool withAncestor =
        ((extractor_(aTrack).getIDonCaloSurface() == aTrack->GetTrackID()) || (extractor_(aTrack).getIDfineCalo() == aTrack->GetTrackID()) || (extractor_(aTrack).isAncestor()));
    ...
    if (extractor_(aTrack).isInHistory()) {
...
#ifdef EDM_ML_DEBUG
      edm::LogVerbatim("SimTrackManager") << "TrackingAction addTrack "  
      ...
      math::XYZVectorD pos((aTrack->GetStep()->GetPostStepPoint()->GetPosition()).x(),
                           (aTrack->GetStep()->GetPostStepPoint()->GetPosition()).y(),
                           (aTrack->GetStep()->GetPostStepPoint()->GetPosition()).z());
      edm::LogVerbatim("SimTrackManager") << "TrackingAction addTrack " << currentTrack_->trackID() << " added with " << true
...
#ifdef EDM_ML_DEBUG
      edm::LogVerbatim("SimTrackManager") << "TrackingAction addTrack " << currentTrack_->trackID() << " added with " << false
                                  << " and " << false;
#endif
