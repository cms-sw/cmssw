...
void RunManager::produce(edm::Event& inpevt, const edm::EventSetup & es)
{
    ...
    if (m_currentEvent->GetNumberOfPrimaryVertex()==0)
    {
       ...
       edm::LogError("SimG4CoreApplication") << " RunManager::produce event " << i
...
//       std::cout << " Aborting run " << std::endl;
    ...
    edm::LogInfo("SimG4CoreApplication") << " RunManager: saved : Event  " << i << " of weight " << m_simEvent->weight()
...
//       std::cout << " WARNING: No generator; initialized only RunAction!" 
//		 << std::endl;
    }
