...
ElectronLimiter::ElectronLimiter(const edm::ParameterSet &p, const G4ParticleDefinition* part) 
  : G4VEmProcess("eLimiter", fGeneral), ionisation_(nullptr), particle_(part), nRegions_(0), 
    rangeCheckFlag_(false), fieldCheckFlag_(false), killTrack_(false) {
...
void ElectronLimiter::InitialiseProcess(const G4ParticleDefinition*) {
  ...
  if(lManager->IsMaster()) {
    edm::LogVerbatim("SimG4CoreApplication")
      ...
      << " ioni: " << ionisation_ << " rangeCheckFlag: " << rangeCheckFlag_
      << " fieldCheckFlag: " << fieldCheckFlag_ 
      << " " << nRegions_ << " regions for tracking cuts\n";
  }
}
