...
void RunManagerMTWorker::initializeRun() {
  ...
  if (m_tls->userRunAction) {
    m_tls->userRunAction->BeginOfRunAction(m_tls->currentRun.get());
  }
}
...
void RunManagerMTWorker::produce(const edm::Event& inpevt, const edm::EventSetup& es, RunManagerMT& runManagerMaster) {
