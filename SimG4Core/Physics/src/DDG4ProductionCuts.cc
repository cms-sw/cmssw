...
DDG4ProductionCuts::DDG4ProductionCuts(
    const G4LogicalVolumeToDDLogicalPartMap &map, int verb,
...
/** helper function to compare parts through their name instead of comparing
   them by their pointers. It's guaranteed to produce the same order in
   subsequent application runs, while pointers usually can't guarantee this
*/
...
void DDG4ProductionCuts::initialize() {
  ...
  // sort all root volumes - to get the same sequence at every run of the
  // application. (otherwise, the sequence will depend on the pointer (memory
  // address) of the involved objects, because 'new' does no guarantee that you
  // allways get a higher (or lower) address when allocating an object of the
  // same type ...
  ...
  for (G4LogicalVolumeToDDLogicalPartMap::Vector::iterator tit = vec_.begin();
    ...
    unsigned int num =
        map_.toString(m_KeywordRegion, (*tit).second, regionName);
...
void DDG4ProductionCuts::setProdCuts(const DDLogicalPart lpart,
  ...
  if (temp != 1) {
    throw cms::Exception("SimG4CorePhysics",
                         " DDG4ProductionCuts::setProdCuts: Problem with "
                         "Region tags - no/more than one ProdCutsForGamma.");
  }
  temp = map_.toDouble("ProdCutsForElectrons", lpart, electroncut);
  if (temp != 1) {
    throw cms::Exception(
        "SimG4CorePhysics",
        " DDG4ProductionCuts::setProdCuts: Problem with Region tags - no/more "
        "than one ProdCutsForElectrons.");
  }
