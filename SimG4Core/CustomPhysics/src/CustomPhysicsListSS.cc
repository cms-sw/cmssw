      ...
      if (pmanager) {
        ...
        if (cp->GetCloud() && fHadronicInteraction &&
          ...
          edm::LogVerbatim("SimG4CoreCustomPhysics")
              ...
              << " GeV; SpectatorMass= "
              << cp->GetSpectator()->GetPDGMass() / GeV << " GeV.";

          if (!myHelper.get()) {
            myHelper.reset(
                new G4ProcessHelper(myConfig, fParticleFactory.get()));
          }
          pmanager->AddDiscreteProcess(
              new FullModelHadronicProcess(myHelper.get()));
        }
        if (particle->GetParticleType() == "darkpho") {
          CMSDarkPairProductionProcess *darkGamma =
              new CMSDarkPairProductionProcess(dfactor);
