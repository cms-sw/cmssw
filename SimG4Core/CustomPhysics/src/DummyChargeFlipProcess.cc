...
DummyChargeFlipProcess::DummyChargeFlipProcess(const G4String &processName)
...
void DummyChargeFlipProcess::BuildPhysicsTable(
  ...
  G4HadronicProcess::GetCrossSectionDataStore()->BuildPhysicsTable(
      aParticleType);
}

G4double DummyChargeFlipProcess::GetMicroscopicCrossSection(
    const G4DynamicParticle * /*aParticle*/, const G4Element *element,
    G4double /*aTemp*/) {
...
G4VParticleChange *
DummyChargeFlipProcess::PostStepDoIt(const G4Track &aTrack,
                                     const G4Step & /*aStep*/) {
