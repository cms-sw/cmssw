...
void CustomParticleFactory::loadCustomParticles(const std::string &filePath) {
  ...
  while (getline(configFile, line)) {
    ...
    if (line.find("DECAY") < line.npos) {
      ...
      // assume SLHA format, e.g.: DECAY  1000021  5.50675438E+00   # gluino
      // decays
      edm::LogInfo("SimG4CoreCustomPhysics")
      ...
      aParticle->SetPDGLifeTime(1.0 / (width * CLHEP::GeV) * 6.582122e-22 *
                                CLHEP::MeV * CLHEP::s);
      G4ParticleDefinition *aAntiParticle =
          theParticleTable->FindAntiParticle(pdgId);
      if (aAntiParticle && aAntiParticle->GetPDGEncoding() != pdgId) {
        ...
        aAntiParticle->SetPDGLifeTime(1.0 / (width * CLHEP::GeV) *
...
void CustomParticleFactory::addCustomParticle(int pdgCode, double mass,
  ...
  CustomParticle *particle =
      new CustomParticle(name, massGeV, width, charge, spin, parity,
                         conjugation, isospin, isospinZ, gParity, pType, lepton,
                         baryon, pdgCode, stable, lifetime, decaytable);
  ...
  if (pType == "rhadron" && name != "~g") {
    ...
    CustomParticle *tmpParticle =
        new CustomParticle(cloudname, cloudmass, 0.0, 0, 0, +1, 0, 0, 0, 0,
                           cloudtype, 0, +1, 0, true, -1.0, nullptr);
    ...
    edm::LogInfo("SimG4CoreCustomPhysics")
        ...
        << "                        Masses: " << mass << " Gev, "
        << spectatormass / CLHEP::GeV << " GeV and " << cloudmass / CLHEP::GeV
        << " GeV.";
...
void CustomParticleFactory::getMassTable(std::ifstream *configFile) {
  ...
  while (getline(*configFile, line)) {
    ...
    sstr >> pdgId >> mass >> tmp >>
        name; // Assume SLHA format, e.g.: 1000001 5.68441109E+02 # ~d_L
    ...
    if (aParticle && !CustomPDGParser::s_isRHadron(pdgId) &&
      ...
      if (sign == -1 && pdgId != 25 && pdgId != 35 && pdgId != 36 &&
          pdgId != 37 && pdgId != 1000039) {
        tmp = "anti_" + name;
        if (!theParticleTable->FindParticle(-pdgId)) {
          edm::LogInfo("SimG4CoreCustomPhysics")
              << "CustomParticleFactory: Calling addCustomParticle for "
                 "antiparticle with pdgId: "
    ...
    if (pdgId == 1000039)
      theParticleTable->FindParticle(pdgId)->SetAntiPDGEncoding(
          pdgId); // gravitino
...
G4DecayTable *CustomParticleFactory::getDecayTable(std::ifstream *configFile,
  ...
  std::string parentName =
      theParticleTable->FindParticle(pdgId)->GetParticleName();
  ...
  while (getline(*configFile, line)) {
    ...
    if (line.at(0) == '#' && ToLower(line).find("br") < line.npos &&
        ToLower(line).find("nda") < line.npos)
    ...
    std::stringstream sstr(line);
    sstr >> br >> nDaughters; // assume SLHA format, e.g.:  1.49435135E-01  2
                              // -15  16  # BR(H+ -> tau+ nu_tau)
    LogDebug("SimG4CoreCustomPhysics")
        << "CustomParticleFactory: Branching Ratio: " << br
        << ", Number of Daughters: " << nDaughters;
    if (nDaughters > 4) {
      edm::LogError("SimG4CoreCustomPhysics")
          << "CustomParticleFactory: Number of daughters is too large (max = "
             "4): "
          << nDaughters << " for pdgId: " << pdgId;
      break;
    }
    if (br <= 0.0) {
      edm::LogError("SimG4CoreCustomPhysics")
    ...
    ////Set the G4 decay
    G4PhaseSpaceDecayChannel *aDecayChannel = new G4PhaseSpaceDecayChannel(
    ...
    edm::LogInfo("SimG4CoreCustomPhysics")
        ...
        << " for pdgID= " << pdgId << "  " << parentName << " BR= " << br
        << " Daughters: " << name[0] << "  " << name[1] << "  " << name[2]
        << "  " << name[3];
  }
  return decaytable;
}
...
G4DecayTable *
CustomParticleFactory::getAntiDecayTable(int pdgId,
  ...
  for (int i = 0; i < theDecayTable->entries(); ++i) {
    ...
    for (int j = 0; j < nd; ++j) {
      ...
      if (!part) {
        edm::LogWarning("SimG4CoreCustomPhysics")
            << "CustomParticleFactory: antiparticle with PDG code" << id
            << " not found!";
        continue;
      }
    ...
    G4PhaseSpaceDecayChannel *aDecayChannel =
        new G4PhaseSpaceDecayChannel(parentName, theDecayChannel->GetBR(), nd,
                                     name[0], name[1], name[2], name[3]);
    decaytable->Insert(aDecayChannel);
    edm::LogInfo("SimG4CoreCustomPhysics")
        << "CustomParticleFactory: inserted decay channel "
        << " for pdgID= " << -pdgId << "  " << parentName
        << " BR= " << theDecayChannel->GetBR() << " Daughters: " << name[0]
...
const std::vector<G4ParticleDefinition *> &
