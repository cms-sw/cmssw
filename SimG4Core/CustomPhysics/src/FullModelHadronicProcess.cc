...
G4double FullModelHadronicProcess::GetMicroscopicCrossSection(
    const G4DynamicParticle *aParticle, const G4Element *anElement,
    G4double aTemp) {
  // Get the cross section for this particle/element combination from the
  // ProcessHelper
  G4double InclXsec = theHelper->GetInclusiveCrossSection(aParticle, anElement);
  //  G4cout<<"Returned cross section from helper was: "<<InclXsec/millibarn<<"
  //  millibarn"<<G4endl;
  return InclXsec;
}

G4double FullModelHadronicProcess::GetMeanFreePath(const G4Track &aTrack,
                                                   G4double,
                                                   G4ForceCondition *) {
  ...
  for (G4int i = 0; i < nElements; ++i) {
    G4double xSection = GetMicroscopicCrossSection(
        aParticle, (*aMaterial->GetElementVector())[i], aTemp);
...
G4VParticleChange *FullModelHadronicProcess::PostStepDoIt(const G4Track &aTrack,
                                                          const G4Step &aStep) {
  //  G4cout<<"**** Entering FullModelHadronicProcess::PostStepDoIt
  //  ******"<<G4endl;
  ...
  CustomParticle *CustomIncident =
      static_cast<CustomParticle *>(IncidentRhadron->GetDefinition());
  const G4ThreeVector &aPosition = aTrack.GetPosition();
  //  G4cout<<"G: "<<aStep.GetStepLength()/cm<<G4endl;
  const G4int theIncidentPDG =
      IncidentRhadron->GetDefinition()->GetPDGEncoding();
  ...
  */
  ...
  if (cloudParticle->GetDefinition() == nullptr) {
    G4cout << "FullModelHadronicProcess::PostStepDoIt  Definition of particle "
              "cloud not available!!"
           << G4endl;
  }
  /*
  G4cout<<"Incoming particle was
  "<<IncidentRhadron->GetDefinition()->GetParticleName()
  <<". Corresponding cloud is
  "<<cloudParticle->GetDefinition()->GetParticleName()<<G4endl; G4cout<<"Kinetic
  energy was: "<<IncidentRhadron->GetKineticEnergy()/GeV<<" GeV"<<G4endl;
  */
  ...
  */
  ...
  if (ek + gluinoMomentum.e() - gluinoMomentum.m() <= 0.1 * MeV || ek <= 0.) {
    ...
    G4cout << "Full R-hadron: "
           << (ek + gluinoMomentum.e() - gluinoMomentum.m()) / MeV << " MeV"
           << G4endl;
  ...
  // We're not using them for anything yet, I know, but let's make sure the
  // machinery is there
  for (ReactionProduct::iterator it = rp.begin(); it != rp.end(); ++it) {
    ...
    if (tempCust != nullptr) {
      ...
      if (outgoingCloud == nullptr) {
        G4cout << "FullModelHadronicProcess::PostStepDoIt  Definition of "
                  "outgoing particle cloud not available!"
               << G4endl;
      }
      /*
        G4cout<<"Outgoing Rhadron is:
        "<<outgoingRhadron->GetParticleName()<<G4endl; G4cout<<"Outgoing cloud
        is: "<<outgoingCloud->GetParticleName()<<G4endl;
      */
    }
  ...
  for (std::vector<G4DynamicParticle*>::iterator it =
  theDynamicParticles.begin(); it != theDynamicParticles.end(); it++){ G4cout<<
  ...
  */
  ...
  // Maybe I need to calculate trafo from R-hadron... Bollocks! Labframe does
  // not move. CMS does.
  ...
  G4cout<<"currentParticle: "<<currentParticle.GetMomentum()/GeV<<" GeV vs.
  "<<OrgPart->Get4Momentum()/GeV<<" GeV"<<G4endl; G4cout<<"targetParticle:
  "<<targetParticle.GetMomentum()/GeV<<" GeV"<<G4endl; G4cout<<"Fourmomentum
  from originalIncident: "<<originalIncident->Get4Momentum()<<" vs
  "<<OrgPart->Get4Momentum()<<G4endl;
  */
  ...
  G4FastVector<G4ReactionProduct, MYGHADLISTSIZE>
      vec; // vec will contain the secondary particles
  G4int vecLen = 0;
  vec.Initialize(0);
  ...
  if (incidentHasChanged)
    modifiedOriginal.SetDefinition(
        outgoingCloud); // Is this correct? It solves the "free energy" checking
                        // in ReactionDynamics
  if (targetHasChanged)
  ...
  /*
  G4cout<<"Current particle starts as:
  "<<currentParticle.GetDefinition()->GetParticleName()<<G4endl; G4cout<<"with
  momentum: "<<currentParticle.GetMomentum()/GeV<<" GeV"<<G4endl; G4cout<<"May
  be killed?: "<<currentParticle.GetMayBeKilled()<<G4endl;
  */
  ...
  //  G4cout <<"Cloud loss:
  //  "<<(e_temp-currentParticle.GetKineticEnergy())/GeV<<" GeV"<<G4endl;
  ...
  G4cout<<"and kinetic energy: "<<currentParticle.GetKineticEnergy()/GeV<<"
  GeV"<<G4endl; G4cout<<"May be killed?:
  "<<currentParticle.GetMayBeKilled()<<G4endl; G4cout<<"Modified original is:
  "<<modifiedOriginal.GetDefinition()->GetParticleName()<<G4endl; G4cout<<"with
  momentum: "<<modifiedOriginal.GetMomentum()/GeV<<" GeV"<<G4endl; G4cout<<"and
  kinetic energy: "<<modifiedOriginal.GetKineticEnergy()/GeV<<" GeV"<<G4endl;
  G4cout<<"May be killed?: "<<modifiedOriginal.GetMayBeKilled()<<G4endl;
  G4cout<<"Target particle is:
  "<<targetParticle.GetDefinition()->GetParticleName()<<G4endl; G4cout<<"with
  momentum: "<<targetParticle.GetMomentum()/GeV<<" GeV"<<G4endl; G4cout<<"and
  kinetic energy: "<<targetParticle.GetKineticEnergy()/GeV<<" GeV"<<G4endl;
  ...
  */
  cloud_p4_new.setVectM(currentParticle.GetMomentum(),
                        currentParticle.GetMass());
  cloud_p4_new *= trans;

  G4LorentzVector cloud_p4_old_full =
      Cloud4Momentum; // quark system in CMS BEFORE collision
  cloud_p4_old_full.boost(trafo_full_cms);
  G4LorentzVector cloud_p4_old_cloud =
      Cloud4Momentum; // quark system in cloud CMS BEFORE collision
  cloud_p4_old_cloud.boost(trafo_cloud_cms);
  G4LorentzVector cloud_p4_full =
      cloud_p4_new; // quark system in CMS AFTER collision
  cloud_p4_full.boost(trafo_full_cms);
  G4LorentzVector cloud_p4_cloud =
      cloud_p4_new; // quark system in cloud CMS AFTER collision
  ...
  //  G4cout<<"P4-diff: "<<(p4_new-cloud_p4_new-gluinoMomentum)/GeV<<",
  //  magnitude: "
  ...
  aParticleChange.ProposeLocalEnergyDeposit(
      (p4_new - cloud_p4_new - gluinoMomentum).m());
  ...
  if (incidentHasChanged) {
    ...
    /*
    G4cout<<"Adding a particle
    "<<p0->GetDefinition()->GetParticleName()<<G4endl; G4cout<<"with momentum:
    "<<p0->GetMomentum()/GeV<<" GeV"<<G4endl; G4cout<<"and kinetic energy:
    "<<p0->GetKineticEnergy()/GeV<<" GeV"<<G4endl;
    */
    if (p0->GetKineticEnergy() > e_kin_0) {
      G4cout << "ALAAAAARM!!! (incident changed from "
             << IncidentRhadron->GetDefinition()->GetParticleName() << " to "
             << p0->GetDefinition()->GetParticleName() << ")" << G4endl;
      G4cout << "Energy loss: " << (e_kin_0 - p0->GetKineticEnergy()) / GeV
             << " GeV (should be positive)" << G4endl;
      // Turns out problem is only in 2 -> 3 (Won't fix 2 -> 2 energy
      // deposition)
      if (rp.size() != 3)
    ...
    if (p > DBL_MIN)
      aParticleChange.ProposeMomentumDirection(p_new.x() / p, p_new.y() / p,
                                               p_new.z() / p);
    else
  ...
  if (targetParticle.GetMass() >
      0.0) // targetParticle can be eliminated in TwoBody
  {
    G4DynamicParticle *p1 = new G4DynamicParticle;
    p1->SetDefinition(targetParticle.GetDefinition());
    // G4cout<<"Target secondary:
  ...
  */
  ...
  if (theRhadron != nullptr || IncidentSurvives) {
    ...
    if (IncidentSurvives) {
      ...
      if (CustomPDGParser::s_isRMeson(
              theRhadron->GetDefinition()->GetPDGEncoding()) !=
        ...
        G4cout << "Rm: "
               << CustomPDGParser::s_isRMeson(
                      theRhadron->GetDefinition()->GetPDGEncoding())
               << " vs: " << CustomPDGParser::s_isRMeson(theIncidentPDG)
               << G4endl;
        G4cout << "Sm: "
               << CustomPDGParser::s_isMesonino(
                      theRhadron->GetDefinition()->GetPDGEncoding())
               << " vs: " << CustomPDGParser::s_isMesonino(theIncidentPDG)
               << G4endl;
      }
    }

    // Calculating relevant scattering angles.
    G4LorentzVector p4_old_full =
        FullRhadron4Momentum; // R-hadron in CMS BEFORE collision
    p4_old_full.boost(trafo_full_cms);
    G4LorentzVector p4_old_cloud =
        FullRhadron4Momentum; // R-hadron in cloud CMS BEFORE collision
    ...
    // G4cout<<p4_full.m()<<" /
    // "<<(cloud_p4_new+gluinoMomentum).boost(trafo_full_cms).m()<<G4endl;
    ...
    if (AbsDeltaE > 10 * GeV) {
      ...
      G4cout << "Gamma: "
             << IncidentRhadron->GetTotalEnergy() /
                    IncidentRhadron->GetDefinition()->GetPDGMass()
             << G4endl;
...
void FullModelHadronicProcess::CalculateMomenta(
    ...
    G4ReactionProduct
        &modifiedOriginal, // Fermi motion and evap. effects included
    G4Nucleus &targetNucleus, G4ReactionProduct &currentParticle,
    G4ReactionProduct &targetParticle, G4bool &incidentHasChanged,
    G4bool &targetHasChanged, G4bool quasiElastic) {
  ...
  G4bool leadFlag = MarkLeadingStrangeParticle(currentParticle, targetParticle,
                                               leadingStrangeParticle);

  //
  // Note: the number of secondaries can be reduced in GenerateXandPt and
  // TwoCluster
  //
  ...
  if ((annihilation || (vecLen >= 6) ||
    ...
    finishedGenXPt = theReactionDynamics.GenerateXandPt(
        vec, vecLen, modifiedOriginal, originalIncident, currentParticle,
        targetParticle, targetNucleus, incidentHasChanged, targetHasChanged,
  ...
  if (modifiedOriginal.GetTotalMomentum() / MeV < 1.0) {
    ...
    theReactionDynamics.SuppressChargedPions(
        vec, vecLen, modifiedOriginal, currentParticle, targetParticle,
        targetNucleus, incidentHasChanged, targetHasChanged);

    try {
      finishedTwoClu = theReactionDynamics.TwoCluster(
          vec, vecLen, modifiedOriginal, originalIncident, currentParticle,
          targetParticle, targetNucleus, incidentHasChanged, targetHasChanged,
          leadFlag, leadingStrangeParticle);
...
G4bool FullModelHadronicProcess::MarkLeadingStrangeParticle(
    ...
    const G4ReactionProduct &targetParticle, G4ReactionProduct &leadParticle) {
  // the following was in GenerateXandPt and TwoCluster
  // add a parameter to the GenerateXandPt function telling it about the strange
  // particle
  //
  ...
  } else if ((targetParticle.GetMass() >=
              G4KaonPlus::KaonPlus()->GetPDGMass()) &&
...
void FullModelHadronicProcess::Rotate(
    G4FastVector<G4ReactionProduct, MYGHADLISTSIZE> &vec, G4int &vecLen) {
...
FullModelHadronicProcess::FindRhadron(G4ParticleChange *aParticleChange) {
  ...
  while (i != nsec && !found) {
    //    G4cout<<"Checking
    //    "<<aParticleChange->GetSecondary(i)->GetDynamicParticle()->GetDefinition()->GetParticleName()<<G4endl;
    //    if
    //    (aParticleChange->GetSecondary(i)->GetDynamicParticle()->GetDefinition()->GetParticleType()=="rhadron")
    //    found = true;
    if (dynamic_cast<CustomParticle *>(aParticleChange->GetSecondary(i)
                                           ->GetDynamicParticle()
