...
G4VParticleChange* G4SQInelasticProcess::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep) {
  ...
  if (aTrack.GetTrackStatus() != fAlive) {
    edm::LogInfo("G4SQInelasticProcess::PostStepDoIt") << "No interaction because primary is not alive" << std::endl;
    return theTotalResult;
  }

  edm::LogInfo("G4SQInelasticProcess::PostStepDoIt") << "Start a possible interaction?" << std::endl;
  ...
  try {
    ...
    G4Exception("G4SQInelasticProcess::PostStepDoIt", "had003", FatalException, ed);
  }
  ...
  if (aTrack.GetTrackStatus() != fAlive && aTrack.GetTrackStatus() != fSuspend) {
    if (aTrack.GetTrackStatus() == fStopAndKill || aTrack.GetTrackStatus() == fKillTrackAndSecondaries ||
      ...
      G4ExceptionDescription ed;
      ed << "G4SQInelasticProcess: track in unusable state - " << aTrack.GetTrackStatus() << G4endl;
  ...
  try {
    ...
    G4Exception("G4SQInelasticProcess::PostStepDoIt", "had005", FatalException, ed);
  }
  ...
  do {
    try {
      ...
      G4Exception("G4SQInelasticProcess::PostStepDoIt", "had006", FatalException, ed);
    }

    edm::LogInfo("G4SQInelasticProcess::PostStepDoIt") << "Call for " << theInteraction->GetModelName() << std::endl;
    edm::LogInfo("G4SQInelasticProcess::PostStepDoIt")
        << "Target element " << anElement->GetName() << "  Z=" << targetNucleus.GetZ_asInt()
        << "  A=" << targetNucleus.GetA_asInt() << std::endl;

    edm::LogInfo("G4SQInelasticProcess::PostStepDoIt")
        << "Nr of secondaries: " << result->GetNumberOfSecondaries() << std::endl
        << "Momentum change and E deeposit: " << result->GetMomentumChange() << " " << result->GetLocalEnergyDeposit()
        << std::endl
    ...
    edm::LogInfo("G4SQInelasticProcess::PostStepDoIt")
        << "In tracker volume? " << (r < (100 * cm) && z < (200 * cm) ? " YES!  " : " NO!  ") << "r=" << r / cm
        << " z=" << z / cm << std::endl;
  ...
  edm::LogInfo("G4SQInelasticProcess::PostStepDoIt") << "=== Anti-sexaquark interaction succeeded!" << std::endl;
...
G4HadFinalState* G4SQInelasticProcess::CheckResult(const G4HadProjectile& aPro,
  ...
  edm::LogInfo("G4SQInelasticProcess::PostStepDoIt") << "checkresult - " << theModel << std::endl;
  ...
  if (theModel) {
    ...
    for (G4int i = 0; i < nSec; i++) {
      ...
      edm::LogInfo("G4SQInelasticProcess::PostStepDoIt")
          << "checkresult - secondary pdgId / E : " << result->GetSecondary(i)->GetParticle()->GetPDGcode() << "\t"
          << result->GetSecondary(i)->GetParticle()->GetTotalEnergy() / GeV << std::endl;
    }
    G4double deltaE = nuclearMass + aPro.GetTotalEnergy() - finalE;
    edm::LogInfo("G4SQInelasticProcess::PostStepDoIt") << "checkresult - Total E: " << finalE / GeV << std::endl;
    edm::LogInfo("G4SQInelasticProcess::PostStepDoIt") << "checkresult - Energy balance: " << deltaE / GeV << std::endl;
    ...
    if (std::abs(deltaE) > checkLevels.second && std::abs(deltaE) > checkLevels.first * aPro.GetKineticEnergy()) {
      ...
      G4Exception(
          "G4SQInelasticProcess:CheckResult()", "had012", epReportLevel < 0 ? EventMustBeAborted : JustWarning, desc);
    }
  }
  return result;
}
