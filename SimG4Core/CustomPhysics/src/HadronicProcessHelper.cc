    ...
    for (size_t i = 2; i != tokens.size(); i++) {
      ...
      if (m_particleTable->contains(part)) {
        ...
        G4Exception("HadronicProcessHelper", "UnkownParticle", FatalException,
                    "Initialization: The reaction product list contained an "
                    "unknown particle");
      }
    }
    if (target == "proton") {
...
G4bool
HadronicProcessHelper::applicabilityTester(const G4ParticleDefinition &aPart) {
...
G4double
HadronicProcessHelper::inclusiveCrossSection(const G4DynamicParticle *particle,
                                             const G4Element *element) {

  // We really do need a dedicated class to handle the cross sections. They
  ...
  for (std::vector<G4int>::iterator it = quarks.begin(); it != quarks.end();
       it++) {
  ...
  return totalNucleonCrossSection * pow(element->GetN(), 0.7) *
         1.25; // * 0.523598775598299;
}
...
HadronicProcessHelper::ReactionProduct HadronicProcessHelper::finalState(
  ...
  for (size_t elm = 0; elm < material->GetNumberOfElements(); elm++) {
    ...
    numberOfProtons +=
        numberOfAtomsPerVolume[elm] * (*elementVector)[elm]->GetZ();
    // Summing nucleons (not neutrons)
    numberOfNucleons +=
        numberOfAtomsPerVolume[elm] * (*elementVector)[elm]->GetN();
  }

  // random decision of the target
  if (G4UniformRand() <
      numberOfProtons / numberOfNucleons) { // target is a proton
  ...
  const G4int incidentPDG =
      incidentDynamicParticle->GetDefinition()->GetPDGEncoding();

  // Making a pointer directly to the ReactionProductList we are looking at.
  // Makes life easier :-)
  ...
  for (ReactionProductList::iterator prod_it = reactionProductList->begin();
    ...
    if (m_reactionIsPossible(*prod_it, incidentDynamicParticle, target)) {
      ...
      if (secondaries == 2) {
        ...
        G4cerr << "ReactionProduct has unsupported number of secondaries: "
               << secondaries << G4endl;
      }
    } /*else {
  ...
  //  G4cout<<"The size of the ReactionProductList is:
  //  "<<theReactionProductList.size()<<G4endl;

  if (goodReactionProductList.empty())
    G4Exception(
        "HadronicProcessHelper", "NoProcessPossible", FatalException,
  ...
  // Keep looping over the list until we have a choice, or until we have tried
  // 100 times
  size_t i;
...
G4double HadronicProcessHelper::m_reactionProductMass(
  ...
  for (ReactionProduct::const_iterator r_it = reactionProd.begin();
       r_it != reactionProd.end(); r_it++) {
...
G4bool HadronicProcessHelper::m_reactionIsPossible(
    const ReactionProduct &aReaction, const G4DynamicParticle *aDynamicParticle,
    G4ParticleDefinition *target) {
...
G4double
HadronicProcessHelper::m_phaseSpace(const ReactionProduct &aReaction,
                                    const G4DynamicParticle *aDynamicParticle,
                                    G4ParticleDefinition *target) {
  G4double qValue = m_reactionProductMass(aReaction, aDynamicParticle, target);
  G4double phi =
      sqrt(1 + qValue / (2 * 0.139 * GeV)) * pow(qValue / (1.1 * GeV), 3. / 2.);
  return (phi / (1 + phi));
}
