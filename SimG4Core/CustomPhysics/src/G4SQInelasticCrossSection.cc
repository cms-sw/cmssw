...
G4SQInelasticCrossSection::G4SQInelasticCrossSection(double mass) : G4VCrossSectionDataSet("SQ-neutron") {
...
G4bool G4SQInelasticCrossSection::IsElementApplicable(const G4DynamicParticle* aPart, G4int Z, const G4Material*) {
  return ((0 < Z) && (aPart->GetDefinition() == theSQ || aPart->GetDefinition() == theAntiSQ));
}

G4double G4SQInelasticCrossSection::GetElementCrossSection(const G4DynamicParticle* aPart, G4int Z, const G4Material*) {
