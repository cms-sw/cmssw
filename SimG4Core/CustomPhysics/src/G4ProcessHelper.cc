...
G4ProcessHelper::G4ProcessHelper(const edm::ParameterSet &p,
  ...
  while (getline(process_stream, line)) {
    ...
    edm::LogInfo("SimG4CoreCustomPhysics")
        << "ProcessHelper: Incident " << incident << "; Target " << target;
...
G4double
G4ProcessHelper::GetInclusiveCrossSection(const G4DynamicParticle *aParticle,
  ...
  double boost = (aParticle->GetKineticEnergy() + aParticle->GetMass()) /
                 aParticle->GetMass();
  ...
  if (!reggemodel) {
    ...
    if (CustomPDGParser::s_isRGlueball(thePDGCode)) {
      ...
      //    edm::LogInfo("SimG4CoreCustomPhysics")<<"Number of quarks:
      //    "<<nq.size()<<G4endl;
      for (std::vector<G4int>::iterator it = nq.begin(); it != nq.end(); it++) {
        //	  edm::LogInfo("SimG4CoreCustomPhysics")<<"Quarkvector:
        //"<<*it<<G4endl;
        if (*it == 1 || *it == 2)
    ...
    if (thePDGCode > 0) {
      ...
      if (CustomPDGParser::s_isRMeson(thePDGCode) ||
          CustomPDGParser::s_isRGlueball(thePDGCode))
  ...
  if (resonant) {
    double e_0 =
        ek_0 + aParticle->GetDefinition()->GetPDGMass(); // Now total energy

    e_0 = sqrt(aParticle->GetDefinition()->GetPDGMass() *
    ...
    //      edm::LogInfo("SimG4CoreCustomPhysics")<<ek_0/GeV<<"
    //      "<<aParticle->GetDefinition()->GetPDGMass()/GeV<<"
    //      "<<theProton->GetPDGMass()/GeV<<G4endl;
    double sqrts =
        sqrt(aParticle->GetDefinition()->GetPDGMass() *
                 aParticle->GetDefinition()->GetPDGMass() +
             theProton->GetPDGMass() * theProton->GetPDGMass() +
    ...
    double res_result = amplitude * (gamma * gamma / 4.) /
                        ((sqrts - e_0) * (sqrts - e_0) +
                         (gamma * gamma / 4.)); // Non-relativistic Breit Wigner

    theXsec += res_result;
    //      if(fabs(aParticle->GetKineticEnergy()/GeV-200)<10)
    //      std::cout<<sqrts/GeV<<" "<<theXsec/millibarn<<std::endl;
  }

  //  std::cout<<"Xsec/nucleon: "<<theXsec/millibarn<<"millibarn, Total Xsec:
  //  "<<theXsec * anElement->GetN()/millibarn<<" millibarn"<<std::endl;
...
ReactionProduct G4ProcessHelper::GetFinalState(const G4Track &aTrack,
  ...
  for (size_t elm = 0; elm < aMaterial->GetNumberOfElements(); elm++) {
    ...
    NumberOfProtons +=
        NbOfAtomsPerVolume[elm] * (*theElementVector)[elm]->GetZ();
    // Summing nucleons (not neutrons)
    NumberOfNucleons +=
        NbOfAtomsPerVolume[elm] * (*theElementVector)[elm]->GetN();
  }
  ...
  if (reggemodel && CustomPDGParser::s_isMesonino(theIncidentPDG) &&
  ...
  // Makes life easier :-)
  ReactionProductList *aReactionProductList =
      &((*theReactionMap)[theIncidentPDG]);
  ...
  G4int N23 =
      0; // Number of 2 -> 3 processes. Couldn't think of more informative names
  ...
  for (ReactionProductList::iterator prod_it = aReactionProductList->begin();
       prod_it != aReactionProductList->end(); prod_it++) {
    ...
    if (ReactionIsPossible(*prod_it, aDynamicParticle) &&
      ...
      edm::LogInfo("SimG4CoreCustomPhysics")<<"There was an impossible
      process"<<G4endl;
      }*/
  }
  //  edm::LogInfo("SimG4CoreCustomPhysics")<<"The size of the
  //  ReactionProductList is: "<<theReactionProductList.size()<<G4endl;

  if (theReactionProductList.empty())
    G4Exception(
        "G4ProcessHelper", "NoProcessPossible", FatalException,
        "GetFinalState: No process could be selected from the given list.");

  // For the Regge model no phase space considerations. We pick a process at
  // random
  if (reggemodel) {
  ...
  for (unsigned int i = 0; i != theReactionProductList.size(); i++) {
    ...
    //    edm::LogInfo("SimG4CoreCustomPhysics")<<"Pushing back cumulated
    //    probability: "<<CumulatedProbability<<G4endl;
  }
  ...
  while (!selected && tries < 100) {
    ...
    if (!TwotoThreeFlag[i]) {
      ...
      if (PhaseSpace(theReactionProductList[i], aDynamicParticle) >
          G4UniformRand())
        selected = true;
      // selected = true;
    }
    //    double suppressionfactor=0.5;
    if (selected && particleTable->FindParticle(theReactionProductList[i][0])
                            ->GetPDGCharge() !=
                        aDynamicParticle->GetDefinition()->GetPDGCharge()) {
      /*
      edm::LogInfo("SimG4CoreCustomPhysics")<<"Incoming particle
      "<<aDynamicParticle->GetDefinition()->GetParticleName()
            <<" has charge
      "<<aDynamicParticle->GetDefinition()->GetPDGCharge()<<G4endl;
      edm::LogInfo("SimG4CoreCustomPhysics")<<"Suggested particle
      "<<particleTable->FindParticle(theReactionProductList[i][0])->GetParticleName()
            <<" has charge
      "<<particleTable->FindParticle(theReactionProductList[i][0])->GetPDGCharge()<<G4endl;
      */
  ...
  //  edm::LogInfo("SimG4CoreCustomPhysics")<<"Sec's:
  //  "<<theReactionProductList[i].size()<<G4endl;
  ...
  //  edm::LogInfo("SimG4CoreCustomPhysics")<<"n_22: "<<n_22<<" n_23: "<<n_23<<"
  //  Checkfraction: "<<checkfraction<<G4endl;
  //  edm::LogInfo("SimG4CoreCustomPhysics") <<"Biig number:
  //  "<<n_22+n_23<<G4endl;
  // Return the chosen ReactionProduct
  return theReactionProductList[i];
}

G4double G4ProcessHelper::ReactionProductMass(
    const ReactionProduct &aReaction,
    const G4DynamicParticle *aDynamicParticle) {
  // Incident energy:
  G4double E_incident = aDynamicParticle->GetTotalEnergy();
  // edm::LogInfo("SimG4CoreCustomPhysics")<<"Total energy: "<<E_incident<<"
  // Kinetic: "<<aDynamicParticle->GetKineticEnergy()<<G4endl;
  // sqrt(s)= sqrt(m_1^2 + m_2^2 + 2 E_1 m_2)
  G4double m_1 = aDynamicParticle->GetDefinition()->GetPDGMass();
  G4double m_2 = theTarget->GetPDGMass();
  // edm::LogInfo("SimG4CoreCustomPhysics")<<"M_R: "<<m_1/GeV<<" GeV, M_np:
  // "<<m_2/GeV<<" GeV"<<G4endl;
  G4double sqrts = sqrt(m_1 * m_1 + m_2 * (m_2 + 2 * E_incident));
  // edm::LogInfo("SimG4CoreCustomPhysics")<<"sqrt(s) = "<<sqrts/GeV<<"
  // GeV"<<G4endl;
  // Sum of rest masses after reaction:
  G4double M_after = 0;
  for (ReactionProduct::const_iterator r_it = aReaction.begin();
       r_it != aReaction.end(); r_it++) {
    // edm::LogInfo("SimG4CoreCustomPhysics")<<"Mass contrib:
    // "<<(particleTable->FindParticle(*r_it)->GetPDGMass())/MeV<<"
    // MeV"<<G4endl;
    M_after += particleTable->FindParticle(*r_it)->GetPDGMass();
  }
  // edm::LogInfo("SimG4CoreCustomPhysics")<<"Intending to return this
  // ReactionProductMass: "<<(sqrts - M_after)/MeV<<" MeV"<<G4endl;
  return sqrts - M_after;
}

G4bool
G4ProcessHelper::ReactionIsPossible(const ReactionProduct &aReaction,
                                    const G4DynamicParticle *aDynamicParticle) {
...
G4bool G4ProcessHelper::ReactionGivesBaryon(const ReactionProduct &aReaction) {
  for (ReactionProduct::const_iterator it = aReaction.begin();
       it != aReaction.end(); it++)
...
G4double
G4ProcessHelper::PhaseSpace(const ReactionProduct &aReaction,
                            const G4DynamicParticle *aDynamicParticle) {
