...
RHStopDump::RHStopDump(edm::ParameterSet const &parameters)
    : mStream(parameters.getParameter<std::string>("stoppedFile").c_str()),
      mProducer(parameters.getUntrackedParameter<std::string>("producer",
                                                              "g4SimHits")) {}
...
void RHStopDump::analyze(const edm::Event &fEvent, const edm::EventSetup &) {
  ...
  if (names->size() != xs->size() || xs->size() != ys->size() ||
      ys->size() != zs->size()) {
    edm::LogError("RHStopDump")
        << "mismatch array sizes name/x/y/z:" << names->size() << '/'
        << xs->size() << '/' << ys->size() << '/' << zs->size() << std::endl;
  } else {
    for (size_t i = 0; i < names->size(); ++i) {
      mStream << (*names)[i] << ' ' << (*xs)[i] << ' ' << (*ys)[i] << ' '
              << (*zs)[i] << ' ' << (*ts)[i] << std::endl;
      mStream << (*ids)[i] << ' ' << (*masses)[i] << ' ' << (*charges)[i]
              << std::endl;
    }
  }
}
