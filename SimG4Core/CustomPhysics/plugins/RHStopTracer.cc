...
RHStopTracer::RHStopTracer(edm::ParameterSet const &p) {
  edm::ParameterSet parameters =
      p.getParameter<edm::ParameterSet>("RHStopTracer");
  mStopRegular =
      parameters.getUntrackedParameter<bool>("stopRegularParticles", false);
...
void RHStopTracer::update(const BeginOfEvent *fEvent) {
  LogDebug("SimG4CoreCustomPhysics")
      << "RHStopTracer::update-> begin of the event "
      << (*fEvent)()->GetEventID();
}
...
void RHStopTracer::update(const BeginOfTrack *fTrack) {
  ...
  if (matched || track->GetKineticEnergy() > mTraceEnergy) {
    LogDebug("SimG4CoreCustomPhysics")
        << "RHStopTracer::update-> new track: "
           "ID/Name/pdgId/mass/charge/Parent: "
        ...
        << track->GetPosition().phi() << "   4vec " << track->GetMomentum();
...
void RHStopTracer::update(const EndOfTrack *fTrack) {
  ...
  if (matched || track->GetKineticEnergy() > mTraceEnergy) {
    LogDebug("SimG4CoreCustomPhysics")
        << "RHStopTracer::update-> stop track: "
           "ID/Name/pdgId/mass/charge/Parent: "
        << track->GetTrackID() << '/' << part->GetParticleName() << '/'
        << part->GetPDGEncoding() << '/' << part->GetPDGMass() / GeV << " GeV/"
        << part->GetPDGCharge() << '/' << track->GetParentID()
    ...
    if (track->GetMomentum().mag() < 0.001) {
      ...
      mStopPoints.push_back(StopPoint(
          track->GetDefinition()->GetParticleName(), track->GetPosition().x(),
          track->GetPosition().y(), track->GetPosition().z(),
          track->GetGlobalTime(), track->GetDefinition()->GetPDGEncoding(),
