...
public:
  ...
  ReactionProduct GetFinalState(const G4Track &aTrack,
...
private:
  ...
  G4double PhaseSpace(const ReactionProduct &aReaction,
                      const G4DynamicParticle *aDynamicParticle);

  G4double ReactionProductMass(const ReactionProduct &aReaction,
                               const G4DynamicParticle *aDynamicParticle);

  G4bool ReactionGivesBaryon(const ReactionProduct &aReaction);

  G4bool ReactionIsPossible(const ReactionProduct &aReaction,
                            const G4DynamicParticle *aDynamicParticle);

  void ReadAndParse(const G4String &str, std::vector<G4String> &tokens,
