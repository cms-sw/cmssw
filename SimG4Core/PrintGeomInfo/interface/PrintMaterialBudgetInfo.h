...
typedef std::map<G4VPhysicalVolume *, G4VPhysicalVolume *,
                 std::less<G4VPhysicalVolume *>>
    mpvpv;
typedef std::multimap<G4LogicalVolume *, G4VPhysicalVolume *,
...
private:
  ...
  void printInfo(G4VPhysicalVolume *pv, G4LogicalVolume *lv,
                 unsigned int leafDepth, std::ostream &weightOut = std::cout,
                 std::ostream &texOut = std::cout);
