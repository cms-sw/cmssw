...
void PrintGeomInfoAction::update(const BeginOfJob *job) {
  ...
  if (_dumpSense) {
    ...
    for (unsigned int i = 0; i < names.size(); i++) {
      ...
      G4cout << "PrintGeomInfoAction:: Get Filtered view for " << attribute
             << " = " << sd << G4endl;
...
void PrintGeomInfoAction::dumpSummary(std::ostream &out) {
  ...
  out << " @@@ Geometry built inside world volume: " << theTopPV->GetName()
      << G4endl;
...
void PrintGeomInfoAction::dumpG4LVLeaf(G4LogicalVolume *lv,
                                       ...
                                       unsigned int count, std::ostream &out) {

  for (unsigned int ii = 0; ii < leafDepth; ii++)
    out << "  ";
  out << " LV:(" << leafDepth << ") " << lv->GetName() << " (" << count << ")"
      << G4endl;
  //--- If a volume is placed n types as daughter of this LV, it should only be
  // counted once
  ...
  for (int ii = 0; ii < lv->GetNoDaughters(); ii++) {
    ...
    if (cite != lvCount.end())
      ...
      lvCount.insert(std::pair<G4LogicalVolume *, unsigned int>(
          lv->GetDaughter(ii)->GetLogicalVolume(), 1));
  }
...
void PrintGeomInfoAction::dumpHierarchyLeafPVLV(G4LogicalVolume *lv,
  ...
  for (scite = lvDaughters.begin(); scite != lvDaughters.end(); scite++) {
    std::pair<mmlvpv::iterator, mmlvpv::iterator> mmER =
        lvpvDaughters.equal_range(*scite);
...
void PrintGeomInfoAction::dumpLV(G4LogicalVolume *lv, unsigned int leafDepth,
                                 std::ostream &out) {
  ...
  if (_dumpLV) {
    ...
    if (_dumpAtts) {
      ...
      if (fVA != nullptr) {
        ...
        out << spaces << "    IsDaughtersInvisible "
            << fVA->IsDaughtersInvisible() << G4endl;
        ...
        out << spaces << "    IsForceDrawingStyle "
            << fVA->IsForceDrawingStyle() << G4endl;
        out << spaces << "    ForcedDrawingStyle "
            << fVA->GetForcedDrawingStyle() << G4endl;
      }
      ...
      G4Track dummy;
      if (fUL != nullptr) {
        out << spaces << "    MaxAllowedStep " << fUL->GetMaxAllowedStep(dummy)
            << G4endl;
        out << spaces << "    UserMaxTrackLength "
            << fUL->GetUserMaxTrackLength(dummy) << G4endl;
        out << spaces << "    UserMaxTime " << fUL->GetUserMaxTime(dummy)
            << G4endl;
        out << spaces << "    UserMinEkine " << fUL->GetUserMinEkine(dummy)
            << G4endl;
        out << spaces << "    UserMinRange " << fUL->GetUserMinRange(dummy)
            << G4endl;
      }
      ...
      out << spaces
          << "        Quality for optimisation, average number of voxels to be "
             "spent per content "
...
void PrintGeomInfoAction::dumpPV(G4VPhysicalVolume *pv, unsigned int leafDepth,
                                 std::ostream &out) {
  ...
  if (_dumpPV) {
    ...
    out << leafDepth << spaces << "### VOLUME = " << pv->GetName()
        << " Copy No " << pv->GetCopyNo() << " in " << mother << " at "
        << pv->GetTranslation();
  }
  if (!pv->IsReplicated()) {
    if (_dumpPV) {
    ...
    if (_dumpReplica) {
      ...
      out << spaces << "     axis " << axis << G4endl << spaces
          << "     nReplicas " << nReplicas << G4endl;
      if (pv->GetParameterisation() != nullptr)
        out << spaces << "    It is parameterisation " << G4endl;
      else
        out << spaces << "     width " << width << G4endl << spaces
            << "     offset " << offset << G4endl << spaces << "     consuming"
            << consuming << G4endl;
      if (pv->GetParameterisation() != nullptr)
        out << spaces << "    It is parameterisation " << G4endl;
    }
...
void PrintGeomInfoAction::dumpTouch(G4VPhysicalVolume *pv,
                                    unsigned int leafDepth, std::ostream &out) {
...
void PrintGeomInfoAction::dumpSolid(G4VSolid *sol, unsigned int leafDepth,
                                    std::ostream &out) {
