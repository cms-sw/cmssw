...
PrintMaterialBudgetInfo::PrintMaterialBudgetInfo(const edm::ParameterSet &p) {
  ...
  std::cout << "PrintMaterialBudget output file " << weightFileName
            << std::endl;
  std::cout << "PrintMaterialBudget output file " << elementFileName
            << std::endl;
...
void PrintMaterialBudgetInfo::update(const BeginOfRun *run) {
  ...
  // Physical Volume
  theTopPV = G4TransportationManager::GetTransportationManager()
  ...
  if (elementNames.empty() && elementTotalWeight.empty() &&
      elementWeightFraction.empty()) {
...
void PrintMaterialBudgetInfo::printInfo(G4VPhysicalVolume *pv,
  ...
  //----- dump info
  weightOut << leafDepth << "\t" << volumeName << "\t" << pv->GetCopyNo()
            << "\t" << solidName << "\t" << materialName << "\t"
            << G4BestUnit(density, "Volumic Mass") << "\t"
            << G4BestUnit(weight, "Mass") << "\t" << std::endl;
  //
  texOut << "\t" << leafDepth << "\t & " << stringLaTeXUnderscore(volumeName)
         << "\t & " << pv->GetCopyNo() << "\t & "
  ...
  for (unsigned int iElement = 0;
       iElement < (unsigned int)lv->GetMaterial()->GetNumberOfElements();
       iElement++) {
    // exclude Air in element weight fraction computation
    if (materialName.find("Air")) {
      std::string elementName =
          lv->GetMaterial()->GetElement(iElement)->GetName();
      double elementMassFraction =
          lv->GetMaterial()->GetFractionVector()[iElement];
...
void PrintMaterialBudgetInfo::dumpElementMassFraction(
    std::ostream &elementOut) {
  // calculate mass fraction
  double totalWeight = 0.0;
  double totalFraction = 0.0;
  for (unsigned int iElement = 0;
       iElement < (unsigned int)elementTotalWeight.size(); iElement++) {
  ...
  for (unsigned int iElement = 0;
    ...
    elementWeightFraction[iElement] =
        elementTotalWeight[iElement] / totalWeight;
  ...
  for (unsigned int iElement = 0;
    ...
    if (elementNames[iElement] != "rr") {
      ...
      elementOut << elementNames[iElement] << "\t" << iElement << "\t"
                 ...
                 << elementWeightFraction[iElement] << std::endl;
    }
  }
  elementOut << "\n\t\tTotal Weight without Air "
             << G4BestUnit(totalWeight, "Mass") << "\tTotal Fraction "
             << totalFraction << std::endl;
}

std::string
PrintMaterialBudgetInfo::stringLaTeXUnderscore(std::string stringname) {
...
std::string
PrintMaterialBudgetInfo::stringLaTeXSuperscript(std::string stringname) {
