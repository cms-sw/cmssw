...
PrintGeomSummary::PrintGeomSummary(const edm::ParameterSet &p)
    : theTopPV_(nullptr) {
  std::vector<std::string> defNames;
  nodeNames_ =
      p.getUntrackedParameter<std::vector<std::string>>("NodeNames", defNames);
...
void PrintGeomSummary::addSolid(const DDLogicalPart &part) {
  ...
  std::map<DDSolidShape, std::string>::iterator it =
      solidShape_.find(solid.shape());
...
void PrintGeomSummary::dumpSummary(std::ostream &out, std::string name) {
  ...
  out << G4endl << G4endl << "@@@@@@@@@@@@@@@@@@ Dumping Summary For Node "
      << name << G4endl;
  ...
  kount_.clear();
  for (std::vector<G4VSolid *>::iterator it = sls_.begin(); it != sls_.end();
       ++it) {
...
void PrintGeomSummary::addName(std::string name) {
  ...
  DDSolidShape shape =
      (jt == solidMap_.end()) ? DDSolidShape::dd_not_init : jt->second;
  std::map<DDSolidShape, std::pair<int, int>>::iterator itr =
      kount_.find(shape);
  if (itr == kount_.end()) {
    kount_[shape] =
        (refl) ? std::pair<int, int>(0, 1) : std::pair<int, int>(1, 0);
  } else {
    kount_[shape] = (refl) ? std::pair<int, int>(((itr->second).first),
                                                 ++((itr->second).second))
                           : std::pair<int, int>(++((itr->second).first),
                                                 ((itr->second).second));
  }
}
...
void PrintGeomSummary::printSummary(std::ostream &out) {
  int k(0);
  for (std::map<DDSolidShape, std::pair<int, int>>::iterator itr =
           kount_.begin();
