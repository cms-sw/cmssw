...
void PrintGeomSummary::update(const BeginOfJob * job) {
  ...
  const DDCompactView::GraphType & gra = cpv->graph();
  DDCompactView::GraphType::index_type i=0;
  solidMap_.clear();
  for (DDCompactView::GraphType::const_adj_iterator git=gra.begin();
       git != gra.end(); ++git) {
    ...
    if (!git->empty()) {
      ...
      for (DDCompactView::GraphType::edge_list::const_iterator cit  = git->begin();
	   cit != git->end(); ++cit) {
