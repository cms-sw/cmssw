...
TrackingVerboseAction::TrackingVerboseAction(edm::ParameterSet const &p)
  ...
  fHighEtPhotons =
      p.getUntrackedParameter<bool>("CheckForHighEtPhotons", false);
  ...
  if (fDEBUG) {
    G4cout << "TV: fTVTrackMin " << fTVTrackMin << " fTVTrackMax "
           << fTVTrackMax << " fTVTrackStep " << fTVTrackStep << " fTVEventMin "
           << fTVEventMin << " fTVEventMax " << fTVEventMax << " fTVEventStep "
           << fTVEventStep << " fVerboseLevel " << fVerboseLevel
           << " fG4Verbose " << fG4Verbose << " PDGIds     " << fPdgIds.size()
           << G4endl;
...
void TrackingVerboseAction::update(const BeginOfEvent *evt) {
  ...
  if (fDEBUG)
    G4cout << " TV: fTkVerbThisEventON " << fTkVerbThisEventON
           << " fTrackingVerboseON " << fTrackingVerboseON << " fTVEventMin "
           << fTVEventMin << " fTVEventMax " << fTVEventMax << G4endl;
    ...
    if (aTrack->GetDefinition()->GetParticleName() == "gamma" &&
      ...
      if ((tkPx * tkPx + tkPy * tkPy + tkPz * tkPz) > 1000.0 * 1000.0 &&
        ...
        G4cout << "ToT E = " << aTrack->GetTotalEnergy()
               ...
               << " Tot P = " << tkP
               << " Pt = " << sqrt(tkPx * tkPx + tkPy * tkPy) << " VTX=("
               << tvtx << "," << tvty << "," << tvtz << ")" << G4endl;
        ...
        if (touchable != nullptr && touchable->GetVolume() != nullptr &&
          ...
          G4cout << "G4LCapture Gamma E(GeV) " << aTrack->GetTotalEnergy() / GeV
                 << "  " << material->GetName() << " "
    ...
    if (aTrack->GetDefinition()->GetParticleName() == "gamma") {
      ...
      if (Tgamma > 2.5 * GeV) { //&& ProcName!="Decay" && ProcName!="eBrem")
        ...
        G4cout << ProcName << "**** ALL photons: gamma E(GeV) "
               << aTrack->GetTotalEnergy() / GeV << "  " << materialName << " "
               << volumeName << G4endl;
