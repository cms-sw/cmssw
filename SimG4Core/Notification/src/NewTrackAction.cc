...
void NewTrackAction::secondary(const G4Track *aSecondary, const G4Track &mother,
                               int flag) const {
  secondary(const_cast<G4Track *>(aSecondary), mother, flag);
}

void NewTrackAction::secondary(G4Track *aSecondary, const G4Track &mother,
                               int flag) const {
...
void NewTrackAction::addUserInfoToSecondary(G4Track *aTrack,
  ...
  // ralf.ulrich@kit.edu: it is more efficient to use the constructor to copy
  // all data and modify later only when needed
  ...
  if (flag > 0) {
    ...
    trkInfo->setIDonCaloSurface(
        aTrack->GetTrackID(), motherInfo.getIDCaloVolume(),
        motherInfo.getIDLastVolume(), aTrack->GetDefinition()->GetPDGEncoding(),
        aTrack->GetMomentum().mag());
  } else {
    // transfer calo ID from mother (to be checked in TrackingAction)
    trkInfo->setIDonCaloSurface(
        motherInfo.getIDonCaloSurface(), motherInfo.getIDCaloVolume(),
        motherInfo.getIDLastVolume(), motherInfo.caloSurfaceParticlePID(),
