...
TrackWithHistory::TrackWithHistory(const G4Track *g4trk)
    : trackID_(0), particleID_(0), parentID_(0),
      momentum_(math::XYZVectorD(0., 0., 0.)), totalEnergy_(0),
      vertexPosition_(math::XYZVectorD(0., 0., 0.)), globalTime_(0),
  ...
  if (g4trk != nullptr) {
    ...
    momentum_ =
        math::XYZVectorD(g4trk->GetMomentum().x(), g4trk->GetMomentum().y(),
                         g4trk->GetMomentum().z());
    totalEnergy_ = g4trk->GetTotalEnergy();
    vertexPosition_ =
        math::XYZVectorD(g4trk->GetPosition().x(), g4trk->GetPosition().y(),
                         g4trk->GetPosition().z());
...
#ifdef DEBUG
    LogDebug("TrackInformation") << " TrackWithHistory : created history for "
                                 << trackID_ << " with mother " << parentID_;
#endif
  }
}
...
void TrackWithHistory::checkAtEnd(const G4Track *gt) {
  ...
  math::XYZVectorD vposdir(gt->GetVertexPosition().x(),
                           gt->GetVertexPosition().y(),
                           gt->GetVertexPosition().z());
  math::XYZVectorD vmomdir(gt->GetVertexMomentumDirection().x(),
  ...
  if ((vertexPosition_ - vposdir).Mag2() > eps2) {
    edm::LogWarning("TrackInformation")
