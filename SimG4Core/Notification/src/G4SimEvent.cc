...
G4SimEvent::G4SimEvent()
    : hepMCEvent(nullptr), weight_(0),
      collisionPoint_(math::XYZTLorentzVectorD(0., 0., 0., 0.)), nparam_(0),
      param_(0) {}
...
void G4SimEvent::load(edm::SimTrackContainer &c) const {
  for (unsigned int i = 0; i < g4tracks.size(); i++) {
    ...
    math::XYZTLorentzVectorD p(trk->momentum().x() / GeV,
                               trk->momentum().y() / GeV,
                               trk->momentum().z() / GeV, trk->energy() / GeV);
    int iv = trk->ivert();
    int ig = trk->igenpart();
    int id = trk->id();
...
void G4SimEvent::load(edm::SimVertexContainer &c) const {
  for (unsigned int i = 0; i < g4vertices.size(); i++) {
    ...
    // vv = position
    // t  = global time
    // iv = index of the parent in the SimEvent SimTrack container (-1 if no
    // parent)
