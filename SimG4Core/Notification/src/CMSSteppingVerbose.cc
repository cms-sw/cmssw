...
CMSSteppingVerbose::CMSSteppingVerbose(G4int verb, G4double ekin,
    ...
    : m_PrintEvent(false), m_PrintTrack(false), m_smInitialized(false),
      m_verbose(verb), m_EventNumbers(evtNum), m_PrimaryVertex(primV),
      m_TrackNumbers(trNum), m_EkinThreshold(ekin) {
...
void CMSSteppingVerbose::TrackStarted(const G4Track *track, bool isKilled) {
  ...
  if (!m_smInitialized) {
    G4SteppingManager *stepman = G4EventManager::GetEventManager()
                                     ->GetTrackingManager()
                                     ->GetSteppingManager();
  ...
  G4cout << std::setw(5) << track->GetCurrentStepNumber() << " " << std::setw(8)
         << track->GetPosition().x() / CLHEP::cm << " " << std::setw(8)
         << track->GetPosition().y() / CLHEP::cm << " " << std::setw(8)
         << track->GetPosition().z() / CLHEP::cm << " " << std::setw(9)
         << track->GetKineticEnergy() / CLHEP::GeV << " " << std::setw(8)
...
void CMSSteppingVerbose::StackFilled(const G4Track *track,
  ...
  if (2 >= m_verbose || !m_PrintTrack ||
      track->GetKineticEnergy() < m_EkinThreshold) {
    return;
  }
  G4int prec = G4cout.precision(4);

  G4cout << std::setw(10) << track->GetTrackID() << " " << std::setw(8)
         << track->GetPosition().x() / CLHEP::cm << " " << std::setw(8)
         << track->GetPosition().y() / CLHEP::cm << " " << std::setw(8)
         << track->GetPosition().z() / CLHEP::cm << " " << std::setw(9)
...
void CMSSteppingVerbose::NextStep(const G4Step *step,
  ...
  if (3 <= m_verbose) {
    ...
    G4cout << "   -------------------------------------------------------"
           << "-------------------------------" << G4endl;
    G4cout << "      Position - x (cm)   : " << std::setw(30)
           << preStep->GetPosition().x() / CLHEP::cm << std::setw(30)
           << postStep->GetPosition().x() / CLHEP::cm << G4endl;
    G4cout << "      Position - y (cm)   : " << std::setw(30)
           << preStep->GetPosition().y() / CLHEP::cm << std::setw(30)
           << postStep->GetPosition().y() / CLHEP::cm << G4endl;
    G4cout << "      Position - z (cm)   : " << std::setw(30)
           << preStep->GetPosition().z() / CLHEP::cm << std::setw(30)
           << postStep->GetPosition().z() / CLHEP::cm << G4endl;
    G4cout << "      Global Time (ns)    : " << std::setw(30)
           << preStep->GetGlobalTime() / CLHEP::ns << std::setw(30)
           << postStep->GetGlobalTime() / CLHEP::ns << G4endl;
    G4cout << "      Local Time (ns)     : " << std::setw(30)
           << preStep->GetLocalTime() / CLHEP::ns << std::setw(30)
           << postStep->GetLocalTime() / CLHEP::ns << G4endl;
    G4cout << "      Proper Time (ns)    : " << std::setw(30)
           << preStep->GetProperTime() / CLHEP::ns << std::setw(30)
           << postStep->GetProperTime() / CLHEP::ns << G4endl;
    G4cout << "      Momentum Direct - x : " << std::setw(30)
           << preStep->GetMomentumDirection().x() << std::setw(30)
           << postStep->GetMomentumDirection().x() << G4endl;
    G4cout << "      Momentum Direct - y : " << std::setw(30)
           << preStep->GetMomentumDirection().y() << std::setw(30)
           << postStep->GetMomentumDirection().y() << G4endl;
    G4cout << "      Momentum Direct - z : " << std::setw(30)
           << preStep->GetMomentumDirection().z() << std::setw(30)
           << postStep->GetMomentumDirection().z() << G4endl;
    G4cout << "      Momentum - x (GeV/c): " << std::setw(30)
           << preStep->GetMomentum().x() / CLHEP::GeV << std::setw(30)
           << postStep->GetMomentum().x() / CLHEP::GeV << G4endl;
    G4cout << "      Momentum - y (GeV/c): " << std::setw(30)
           << preStep->GetMomentum().y() / CLHEP::GeV << std::setw(30)
           << postStep->GetMomentum().y() / CLHEP::GeV << G4endl;
    G4cout << "      Momentum - z (GeV/c): " << std::setw(30)
           << preStep->GetMomentum().z() / CLHEP::GeV << std::setw(30)
           << postStep->GetMomentum().z() / CLHEP::GeV << G4endl;
    G4cout << "      Total Energy (GeV)  : " << std::setw(30)
           << preStep->GetTotalEnergy() / CLHEP::GeV << std::setw(30)
           << postStep->GetTotalEnergy() / CLHEP::GeV << G4endl;
    G4cout << "      Kinetic Energy (GeV): " << std::setw(30)
           << preStep->GetKineticEnergy() / CLHEP::GeV << std::setw(30)
           << postStep->GetKineticEnergy() / CLHEP::GeV << G4endl;
    G4cout << "      Velocity (mm/ns)    : " << std::setw(30)
           << preStep->GetVelocity() << std::setw(30) << postStep->GetVelocity()
           << G4endl;
    G4cout << "      Volume Name         : " << std::setw(30)
           << preStep->GetPhysicalVolume()->GetName();
    G4String volName = (postStep->GetPhysicalVolume())
                           ? postStep->GetPhysicalVolume()->GetName()
                           : "OutOfWorld";

    G4cout << std::setw(30) << volName << G4endl;
    G4cout << "      Safety (mm)         : " << std::setw(30)
           << preStep->GetSafety() << std::setw(30) << postStep->GetSafety()
           << G4endl;
    G4cout << "      Polarization - x    : " << std::setw(30)
           << preStep->GetPolarization().x() << std::setw(30)
           << postStep->GetPolarization().x() << G4endl;
    G4cout << "      Polarization - y    : " << std::setw(30)
           << preStep->GetPolarization().y() << std::setw(30)
           << postStep->GetPolarization().y() << G4endl;
    G4cout << "      Polarization - Z    : " << std::setw(30)
           << preStep->GetPolarization().z() << std::setw(30)
           << postStep->GetPolarization().z() << G4endl;
    G4cout << "      Weight              : " << std::setw(30)
           << preStep->GetWeight() << std::setw(30) << postStep->GetWeight()
           << G4endl;
    G4cout << "      Step Status         : ";
  ...
  if (4 <= m_verbose) {
    ...
    if (pv2) {
      ...
      G4cout << "       Rotation:    " << pv2->GetObjectRotationValue()
             << G4endl;
    ...
    if (5 <= m_verbose) {
      ...
      for (G4int k = 1; k < tch1->GetHistoryDepth(); ++k) {
        ...
        if (pv) {
          ...
          G4cout << " Depth # " << k << "  PhysVolume " << pv->GetName()
                 << G4endl;
          G4cout << "       Translation: " << pv->GetObjectTranslation()
                 << G4endl;
          G4cout << "       Rotation:    " << pv->GetObjectRotationValue()
                 << G4endl;
  ...
  for (G4int i = 0; i < nt; ++i) {
    ...
    G4cout << "      (" << std::setw(9)
           << (*tv)[i]->GetPosition().x() / CLHEP::cm << " " << std::setw(9)
           << (*tv)[i]->GetPosition().y() / CLHEP::cm << " " << std::setw(9)
           << (*tv)[i]->GetPosition().z() / CLHEP::cm << ") cm, "
           << std::setw(9) << (*tv)[i]->GetKineticEnergy() / CLHEP::GeV
           << " GeV, " << std::setw(9) << (*tv)[i]->GetGlobalTime() / CLHEP::ns
           << " ns, " << std::setw(18)
