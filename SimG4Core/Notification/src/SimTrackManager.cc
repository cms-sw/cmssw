...
SimTrackManager::SimTrackManager(bool iCollapsePrimaryVertices)
    : m_trksForThisEvent(nullptr), m_nVertices(0),
      m_collapsePrimaryVertices(iCollapsePrimaryVertices), lastTrack(0),
      lastHist(0), theLHCTlink(nullptr) {}
...
void SimTrackManager::saveTrackAndItsBranch(TrackWithHistory *trkWHist) {
  ...
  TrackContainer::const_iterator tk_itr = std::lower_bound(
      (*m_trksForThisEvent).begin(), (*m_trksForThisEvent).end(), parent,
...
void SimTrackManager::storeTracks(G4SimEvent *simEvent) {
  ...
  stable_sort(m_trksForThisEvent->begin(), m_trksForThisEvent->end(),
              trkIDLess());
...
void SimTrackManager::reallyStoreTracks(G4SimEvent *simEvent) {
  ...
  for (unsigned int it = 0; it < m_trksForThisEvent->size(); it++) {
    ...
    std::map<uint32_t, std::pair<math::XYZVectorD, math::XYZTLorentzVectorD>>::
        const_iterator cit = mapTkCaloStateInfo.find(trkH->trackID());
    ...
    simEvent->add(new G4SimTrack(trkH->trackID(), trkH->particleID(),
                                 trkH->momentum(), trkH->totalEnergy(), ivertex,
                                 ig, pm, tcinfo.first, tcinfo.second));
  }
}
...
int SimTrackManager::getOrCreateVertex(TrackWithHistory *trkH, int iParentID,
  ...
  for (std::vector<TrackWithHistory *>::const_iterator it =
           (*m_trksForThisEvent).begin();
  ...
  if (iterator != m_vertexMap.end()) {
    ...
    for (unsigned int k = 0; k < m_vertexMap[parent].size(); k++) {
      if (sqrt((trkH->vertexPosition() - (((m_vertexMap[parent])[k]).second))
                   .Mag2()) < 0.001) {
  ...
  simEvent->add(new G4SimVertex(trkH->vertexPosition(), trkH->globalTime(),
                                parent, ptype));
  m_vertexMap[parent].push_back(
      MapVertexPosition(m_nVertices, trkH->vertexPosition()));
  m_nVertices++;
...
void SimTrackManager::cleanTracksWithHistory() {
...
#ifdef DebugLog
  LogDebug("SimTrackManager")
      << "SimTrackManager::cleanTracksWithHistory has " << idsave.size()
  ...
  stable_sort(m_trksForThisEvent->begin() + lastTrack,
              m_trksForThisEvent->end(), trkIDLess());

  stable_sort(idsave.begin(), idsave.end());

#ifdef DebugLog
  LogDebug("SimTrackManager")
      << " SimTrackManager::cleanTracksWithHistory knows "
      << m_trksForThisEvent->size() << " tracks with history before branching";
  for (unsigned int it = 0; it < (*m_trksForThisEvent).size(); it++) {
    LogDebug("SimTrackManager")
  ...
  for (unsigned int it = 0; it < (*m_trksForThisEvent).size(); it++) {
    LogDebug("SimTrackManager")
        << " Track in position " << it << " G4 track number "
        << (*m_trksForThisEvent)[it]->trackID() << " mother "
        << (*m_trksForThisEvent)[it]->parentID() << " Status "
...
void SimTrackManager::resetGenID() {
  ...
  for (unsigned int it = 0; it < m_trksForThisEvent->size(); it++) {
    ...
    if (genParticleID_ == -1) {
      ...
      for (unsigned int itrlink = 0; itrlink < (*theLHCTlink).size();
           itrlink++) {
