...
//
/**\class SimActivityRegistryEnroller SimActivityRegistryEnroller.h
 SimG4Core/Notification/interface/SimActivityRegistryEnroller.h

 Description: Based on what classes an object inherts, enrolls that object with
 the proper signal
...
template <class T> struct Enrollment {
...
template <class T> struct LastEnrollerHelper {
  static void enroll(SimActivityRegistry &, T *) {}
};

template <class T, class TVector> struct EnrollerHelper {
  ...
  static void enroll(SimActivityRegistry &iReg, T *iT) {
    ...
    // end of TVector
    Enrollment<typename boost::mpl::deref<typename boost::mpl::prior<
        typename boost::mpl::end<TVector>::type>::type>::type>::enroll(iReg,
                                                                       iT);

    // If TVector is not at its end, call EnrollerHelper with a vector
    // that had our last type 'popped off' the end
    typedef typename boost::mpl::eval_if<
        boost::mpl::empty<TVector>, boost::mpl::identity<LastEnrollerHelper<T>>,
        boost::mpl::identity<EnrollerHelper<
            T, typename boost::mpl::pop_back<TVector>::type>>>::type
        NextEnroller;
...
public:
  ...
  typedef boost::mpl::vector<BeginOfJob, DDDWorld, BeginOfRun, BeginOfEvent,
                             BeginOfTrack, BeginOfStep, G4Step, EndOfTrack,
                             EndOfEvent, EndOfRun>
      Signals;
  ...
  template <class T> static void enroll(SimActivityRegistry &iReg, T *iObj) {
...
private:
  ...
  const SimActivityRegistryEnroller &
  operator=(const SimActivityRegistryEnroller &) = delete; // stop default
