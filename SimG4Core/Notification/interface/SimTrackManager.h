...
//
/**\class SimTrackManager SimTrackManager.h
...
public:
  ...
  void addTkCaloStateInfo(
    ...
    std::map<uint32_t, std::pair<math::XYZVectorD, math::XYZTLorentzVectorD>>::
        const_iterator it = mapTkCaloStateInfo.find(t);

    if (it == mapTkCaloStateInfo.end()) {
      mapTkCaloStateInfo.insert(
          std::pair<uint32_t,
                    std::pair<math::XYZVectorD, math::XYZTLorentzVectorD>>(t,
                                                                           p));
    }
  }
