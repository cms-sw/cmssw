...
/**\class Signaler Signaler.h SimG4Core/Notification/interface/Signaler.h
 ...
 Usage:
    ...
    Having Signaler inherit frmo Observer<const T*> allows one Signaler to
 forward its signal to another Signaler.

    All connected Observers are required to have a lifetime greater than the
 Signaler to which they are attached.
*/
//
// Original Author:
...
namespace sim_act {
template <typename T> class Signaler : public Observer<const T *> {
...
public:
  ...
  void operator()(const T *iSignal) const {
    typedef
        typename std::vector<Observer<const T *> *>::const_iterator iterator;
