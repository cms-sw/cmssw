...
public:
  ...
  G4SimTrack(int iid, int ipart, const math::XYZVectorD &ip, double ie)
      : id_(iid), ipart_(ipart), ip_(ip), ie_(ie), ivert_(-1), igenpart_(-1),
        parentID_(-1), parentMomentum_(math::XYZVectorD(0., 0., 0.)),
        tkSurfacePosition_(math::XYZVectorD(0., 0., 0.)),
        tkSurfaceMomentum_(math::XYZTLorentzVectorD(0., 0., 0., 0.)) {}

  G4SimTrack(int iid, int ipart, const math::XYZVectorD &ip, double ie, int iv,
             int ig, const math::XYZVectorD &ipmom)
  ...
  G4SimTrack(int iid, int ipart, const math::XYZVectorD &ip, double ie, int iv,
      ...
      : id_(iid), ipart_(ipart), ip_(ip), ie_(ie), ivert_(iv), igenpart_(ig),
        parentMomentum_(ipmom), tkSurfacePosition_(tkpos),
