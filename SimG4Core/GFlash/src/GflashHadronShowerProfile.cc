...
GflashHadronShowerProfile::GflashHadronShowerProfile(G4Region* envelope)
  : theMediaMap(new GflashMediaMap(envelope))
{
  jCalorimeter = kNULL;
  showerType   = 0;
}
...
GflashCalorimeterNumber GflashHadronShowerProfile::getCalorimeterNumber(const G4FastTrack& fastTrack)
{
...
void GflashHadronShowerProfile::hadronicParameterization(const G4FastTrack& fastTrack)
{
  ...
  const G4int    maxNumberOfSpots = 5000;  
  ...
  G4ThreeVector xhatShower = zhatShower.orthogonal();
  G4ThreeVector yhatShower = zhatShower.cross(xhatShower);
  ...
  // Limit number of spots to 1500
  numberOfSpots = std::min(numberOfSpots,1500);
  ...
  static Genfun::IncompleteGamma gammaDist;
  ...
  while(energy > 0.0 && stepLengthLeft > 0.0) {
    ...
    if (jCalorimeter == kHB || jCalorimeter==kHE ) samplingFluctuation(fluctuatedEnergy,einc); 
    ...
    */
    ...
    for (G4int ispot = 0 ;  ispot < numSpotInStep ; ispot++) {
      ...
      rShower  = std::max( R50 * rxPDF, rxMax);
	...
	deltaZ/static_cast<float>(numSpotInStep)*zhatShower*(ispot+1/2.-numSpotInStep/2.);
      ...
      eSpot.SetEnergy(hadSpotEnergy+emSpotEnergy);
...
void GflashHadronShowerProfile::loadParameters(G4double einc)
{
  ...
  // fill fluctuation vector

  G4double RMX[234]; //190+8*9/2+8
  ...
  GflashCalorimeterNumber jcalor = kESPM; 
