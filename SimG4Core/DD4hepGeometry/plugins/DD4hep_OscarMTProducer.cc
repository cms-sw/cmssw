...
class DD4hep_OscarMTProducer : public edm::stream::EDProducer<
  edm::GlobalCache<DD4hep_OscarMTMasterThread>,
...
public:
  ...
  explicit DD4hep_OscarMTProducer(edm::ParameterSet const & p, const DD4hep_OscarMTMasterThread *);
  ~DD4hep_OscarMTProducer() override;

  static std::unique_ptr<DD4hep_OscarMTMasterThread> initializeGlobalCache(const edm::ParameterSet& iConfig);
  static std::shared_ptr<int> globalBeginRun(const edm::Run& iRun, const edm::EventSetup& iSetup, const DD4hep_OscarMTMasterThread *masterThread);
  static void globalEndRun(const edm::Run& iRun, const edm::EventSetup& iSetup, const RunContext *iContext);
  static void globalEndJob(DD4hep_OscarMTMasterThread *masterThread);
...
private:
  ...
  std::unique_ptr<DD4hep_RunManagerMTWorker> m_runManagerWorker;
};
...
DD4hep_OscarMTProducer::DD4hep_OscarMTProducer(edm::ParameterSet const & p, const DD4hep_OscarMTMasterThread *)
{
  // Random number generation not allowed here
  StaticRandomEngineSetUnset random(nullptr);

  m_runManagerWorker.reset(new DD4hep_RunManagerMTWorker(p, consumesCollector()));
...
DD4hep_OscarMTProducer::~DD4hep_OscarMTProducer() 
{ }

std::unique_ptr<DD4hep_OscarMTMasterThread>
DD4hep_OscarMTProducer::initializeGlobalCache(const edm::ParameterSet& iConfig) {
  // Random number generation not allowed here
  StaticRandomEngineSetUnset random(nullptr);

  return std::unique_ptr<DD4hep_OscarMTMasterThread>(new DD4hep_OscarMTMasterThread(iConfig));
}

std::shared_ptr<int>
DD4hep_OscarMTProducer::globalBeginRun(const edm::Run& iRun, const edm::EventSetup& iSetup, const DD4hep_OscarMTMasterThread *masterThread) {
  // Random number generation not allowed here
  StaticRandomEngineSetUnset random(nullptr);
...
void
DD4hep_OscarMTProducer::globalEndRun(const edm::Run& iRun, const edm::EventSetup& iSetup, const RunContext *iContext) {
  iContext->global()->endRun();
}

void
DD4hep_OscarMTProducer::globalEndJob(DD4hep_OscarMTMasterThread *masterThread) {
  masterThread->stopThread();
}

void 
DD4hep_OscarMTProducer::endRun(const edm::Run&, const edm::EventSetup&)
{
  // Random number generation not allowed here
  StaticRandomEngineSetUnset random(nullptr);
...
void
DD4hep_OscarMTProducer::produce(edm::Event & e, const edm::EventSetup & es)
{
...
DEFINE_FWK_MODULE(DD4hep_OscarMTProducer);
