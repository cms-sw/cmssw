...
DD4hep_RunManagerMT::DD4hep_RunManagerMT(edm::ParameterSet const & p):
...
DD4hep_RunManagerMT::~DD4hep_RunManagerMT() 
{
  stopG4();
}

void
DD4hep_RunManagerMT::initG4(const cms::DDDetector *pDD, const cms::DDSpecParRegistry *specPars, const MagneticField *pMF, 
			    const HepPDT::ParticleDataTable *fPDGTable)
{
  if (m_managerInitialized) return;

  edm::LogVerbatim("SimG4CoreApplication") 
    << "DD4hep_RunManagerMT: start initialisation of geometry";

  // DDDWorld: get the DDCV from the ES and use it to build the World
  dd4hep::sim::Geant4GeometryMaps::VolumeMap lvMap;
  m_world.reset(new DDDWorld(pDD, lvMap));
  m_registry.dddWorldSignal_(m_world.get());

  // setup the magnetic field
  edm::LogVerbatim("SimG4CoreApplication") 
    << "DD4hep_RunManagerMT: start initialisation of magnetic field";
  ...
  // Create physics list
  edm::LogVerbatim("SimG4CoreApplication") 
    << "DD4hep_RunManagerMT: create PhysicsList";

  std::string pType = m_pPhysics.getParameter<std::string> ("type");
  ...
  edm::LogVerbatim("SimG4CoreApplication") 
    << "DD4hep_RunManagerMT: start initialisation of PhysicsList for master";
  ...
  if(m_pPhysics.getParameter<bool>("CutsPerRegion")) {
    m_prodCuts.reset(new DD4hep_DDG4ProductionCuts(specPars, lvMap, verb, m_pPhysics));	
    m_prodCuts->update();
  }
  ...
  if(!m_G4Commands.empty()) {
    G4cout << "DD4hep_RunManagerMT: Requested UI commands: " << G4endl;
...
void
DD4hep_RunManagerMT::initializeUserActions() {
...
void
DD4hep_RunManagerMT::Connect(RunAction* runAction)
{
...
void
DD4hep_RunManagerMT::terminateRun() {
  if(m_userRunAction) {
...
void
DD4hep_RunManagerMT::DumpMagneticField(const G4Field* field) const
{
