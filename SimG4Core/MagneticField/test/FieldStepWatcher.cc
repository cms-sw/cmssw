...
void FieldStepWatcher::update(const BeginOfRun * ) {
  ...
  if (dbe_) {
    ...
    for (unsigned int i=0; i<lvnames.size(); i++) {
      ...
      sprintf (titl, "Step Length in Volume %s", lvnames[i].c_str());
      MonitorElement* m1 = dbe_->book1D(name, titl, 5000, 0., 10000.);
      meStep.push_back(m1);
      sprintf (name, "Call%d", i);
      sprintf (titl, "Number of steps in Volume %s", lvnames[i].c_str());
...
void FieldStepWatcher::update(const G4Step * aStep) {
  ...
  if (aStep) {
    ...
    if (indx >= 0) {
      steps[indx]++;
      if (dbe_) {
	meStep[indx]->Fill(aStep->GetStepLength());
	if (code == 11 || code == -11) {
	  meStepE[indx]->Fill(aStep->GetStepLength());
	} else if (code == 22) {
	  meStepG[indx]->Fill(aStep->GetStepLength());
	} else if (code == 13 || code == -13) {
	  meStepMu[indx]->Fill(aStep->GetStepLength());
	} else if (code == 12 || code == -12 || code == 14 || code == -14 ||
		   code == 16 || code == -16) {
	  meStepNu[indx]->Fill(aStep->GetStepLength());
	} else if (charge == 0) {
	  meStepNH[indx]->Fill(aStep->GetStepLength());
	} else {
	  meStepCH[indx]->Fill(aStep->GetStepLength());
	}
      }
    }
  }
