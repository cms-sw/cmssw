...
class G4TDormandPrince45 : public G4MagIntegratorStepper
{
  public:
    ...
    void RightHandSideInl( const G4double y[], G4double invMom,
                                 G4double dydx[] )
    {
      fEquation_Rhs->T_Equation::TRightHandSide(y, invMom, dydx);
    }
                                                ...
                                                G4double yErr[] )
{
    ...
    auto invMom = inv_momentum_magnitude;
    RightHandSideInl(yTemp, invMom, ak2);              // 2nd stage
    
    for(unsigned int i = 0; i < N; ++i)
    {
        yTemp[i] = fyIn[i] + hstep * (b31 * dydx[i] + b32 * ak2[i]);
    }
    RightHandSideInl(yTemp, invMom, ak3);              // 3rd stage
    ...
    RightHandSideInl(yTemp, invMom, ak4);              // 4th stage
    ...
    RightHandSideInl(yTemp, invMom, ak5);              // 5th stage
    ...
    RightHandSideInl(yTemp, invMom, ak6);              // 6th stage
    ...
    RightHandSideInl(yOut, invMom, ak7);               // 7th and Final stage
...
void G4TDormandPrince45<T_Equation,N>::SetupInterpolation5thOrder()
{
    ...
    G4double inv_momentum_magnitude = 1.0 / std::sqrt( momentum_mag_square );
    auto invMom = inv_momentum_magnitude;
    RightHandSideInl(yTemp, invMom, ak8);          // 8th Stage
    ...
    RightHandSideInl(yTemp, invMom, ak9);          // 9th Stage
}
