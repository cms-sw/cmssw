...
static void createWatchers(const edm::ParameterSet &iP,
  ...
  for (std::vector<ParameterSet>::iterator itWatcher = watchers.begin();
    ...
    std::unique_ptr<SimWatcherMakerBase> maker(SimWatcherFactory::get()->create(
  ...
  if (m_pUseSensitiveDetectors) {
      ...
      m_attach = new AttachSD;
    {
      std::pair<std::vector<SensitiveTkDetector *>,
          ...
          sensDets = m_attach->create((*pDD), catalog_, m_p,
                                      m_trackManager.get(), m_registry);
    ...
    edm::LogInfo("GeometryProducer")
        ...
        << m_sensTkDets.size() << " Tk type Producers, and "
        << m_sensCaloDets.size() << " Calo type producers ";
  }
