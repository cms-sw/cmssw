...
QGSPCMS_BERT_LPM::QGSPCMS_BERT_LPM(G4LogicalVolumeToDDLogicalPartMap& map,
  ...
  edm::LogInfo("PhysicsList") << "You are using the simulation engine: "
			      << "QGSP_BERT_LPM 3.3 with Flags for EM Physics "
			      << emPhys << " and for Hadronic Physics "
			      << hadPhys << "\n";

  if (emPhys) {
    // EM Physics
    RegisterPhysics( new CMSEmStandardPhysicsLPM("standard EM LPM",table_,ver,charge));
