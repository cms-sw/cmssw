...
UrbanMscModel93::UrbanMscModel93(const G4String &nam) : G4VMscModel(nam) {
  ...
  coeffc1 = 0.;
  coeffc2 = 0.;
  scr1ini = fine_structure_const * fine_structure_const * electron_mass_c2 *
            electron_mass_c2 / (0.885 * 0.885 * 4. * pi);
...
G4double UrbanMscModel93::ComputeCrossSectionPerAtom(
  ...
  static const G4double Zdat[15] = {4.,  6.,  13., 20., 26., 29., 32., 38.,
                                    47., 50., 56., 64., 74., 79., 82.};

  static const G4double Tdat[22] = {
      100 * eV,  200 * eV,  400 * eV,  700 * eV,  1 * keV,  2 * keV,
      4 * keV,   7 * keV,   10 * keV,  20 * keV,  40 * keV, 70 * keV,
      100 * keV, 200 * keV, 400 * keV, 700 * keV, 1 * MeV,  2 * MeV,
      4 * MeV,   7 * MeV,   10 * MeV,  20 * MeV};
  ...
  static const G4double sig0[15] = {
      0.2672 * barn, 0.5922 * barn, 2.653 * barn, 6.235 * barn, 11.69 * barn,
      13.24 * barn,  16.12 * barn,  23.00 * barn, 35.13 * barn, 39.95 * barn,
      50.85 * barn,  67.19 * barn,  91.15 * barn, 104.4 * barn, 113.1 * barn};

  static const G4double hecorr[15] = {120.70, 117.50, 105.00, 92.92,  79.23,
                                      74.510, 68.29,  57.39,  41.97,  36.14,
                                      24.53,  10.21,  -7.855, -16.84, -22.30};
  ...
  if (eKineticEnergy <= Tlim) {
    ...
    c2 = bg2lim * sig0[iZ + 1] * (1. + hecorr[iZ + 1] * (beta2 - beta2lim)) /
         bg2;
...
G4double
UrbanMscModel93::ComputeTruePathLengthLimit(const G4Track &track,
  ...
  if (steppingAlgorithm == fUseDistanceToBoundary) {
    ...
    if (firstStep || stepStatus == fGeomBoundary) {
      ...
      //	 << " tlimitmin= " << tlimitmin << " geomlimit= " << geomlimit
      //<<G4endl;
...
G4double UrbanMscModel93::ComputeGeomPathLength(G4double) {
  ...
  if (tPathLength < currentRange * dtrl) {
    ...
    par3 = 1. + par2;
    if (tPathLength < currentRange)
      zmean = (1. - G4Exp(par3 * G4Log(1. - tPathLength / currentRange))) /
              (par1 * par3);
    else
      zmean = 1. / (par1 * par3);
  } else {
  ...
  if (zPathLength > lambda0)
    zPathLength = lambda0;
  // G4cout << "zPathLength= " << zPathLength << " lambda1= " << lambda0 <<
  // G4endl;
  return zPathLength;
}
...
G4double UrbanMscModel93::ComputeTrueStepLength(G4double geomStepLength) {
  ...
  // G4cout << "tPathLength= " << tPathLength << " step= " << geomStepLength <<
  // G4endl;
    ...
    if (tau >= taubig)
      ...
      if (G4UniformRand() < qprob) {
        ...
        if (KineticEnergy > 5 * GeV && cth < 0.9) {
          G4cout << "UrbanMscModel93::SampleCosineTheta: E(GeV)= "
                 << KineticEnergy / GeV << " 1-cosT= " << 1 - cth
                 << " length(mm)= " << trueStepLength << " Zeff= " << Zeff
                 << " tau= " << tau << " prob= " << prob << " var= " << var
                 << G4endl;
          G4cout << "  c= " << c << " qprob= " << qprob << " eb1= " << eb1
                 << " ebx= " << ebx << " c1= " << c1 << " b= " << b
                 << " b1= " << b1 << " bx= " << bx << " d= " << d
        ...
        if (KineticEnergy > 5 * GeV) {
          G4cout << "UrbanMscModel93::SampleCosineTheta: E(GeV)= "
                 << KineticEnergy / GeV << " length(mm)= " << trueStepLength
                 << " Zeff= " << Zeff << " qprob= " << qprob << G4endl;
        }
      }
    }
  }
