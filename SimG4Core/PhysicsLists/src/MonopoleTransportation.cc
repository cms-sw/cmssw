...
MonopoleTransportation::MonopoleTransportation(const Monopole *mpl, G4int verb)
    : G4VProcess(G4String("MonopoleTransportation"), fTransportation),
      fParticleDef(mpl), fieldMgrCMS(nullptr), fLinearNavigator(nullptr),
      fFieldPropagator(nullptr), fParticleIsLooping(false),
      fPreviousSftOrigin(0., 0., 0.), fPreviousSafety(0.0),
      fThreshold_Warning_Energy(100 * MeV),
      fThreshold_Important_Energy(250 * MeV), fThresholdTrials(10),
      fNoLooperTrials(0), fSumEnergyKilled(0.0), fMaxEnergyKilled(0.0),
...
MonopoleTransportation::~MonopoleTransportation() {
  if ((verboseLevel > 0) && (fSumEnergyKilled > 0.0)) {
    ...
    G4endl; G4cout << "   Max energy of loopers killed: " <<  fMaxEnergyKilled
    << G4endl;
    */
  }
}
...
G4double MonopoleTransportation::AlongStepGetPhysicalInteractionLength(
    const G4Track &track,
  ...
  *selection = CandidateForSelection;
  ...
  } else //  A field exerts force
  {
    ...
    G4ChargeState chargeState(
        particleElectricCharge, // The charge can change (dynamic)
    ...
    equationOfMotion->SetChargeMomentumMass(
        chargeState,       //  Was particleMagneticCharge - in Mev/c
        momentumMagnitude, //  Was particleElectricCharge
        restMass);
    // SetChargeMomentumMass now passes both the electric and magnetic charge -
    // in chargeState
...
G4VParticleChange *
MonopoleTransportation::AlongStepDoIt(const G4Track &track,
  ...
  if (fParticleIsLooping) {
    ...
    if ((endEnergy < fThreshold_Important_Energy) ||
      ...
      if ((verboseLevel > 1) || (endEnergy > fThreshold_Warning_Energy)) {
        G4cout << " MonopoleTransportation is killing track that is looping or "
                  "stuck "
               << G4endl << "   This track has "
...
#ifdef G4VERBOSE
      if ((verboseLevel > 2)) {
        G4cout << "   MonopoleTransportation::AlongStepDoIt(): Particle "
                  "looping -  "
...
G4double MonopoleTransportation::PostStepGetPhysicalInteractionLength(
    const G4Track &,
    G4double, // previousStepSize
...
void MonopoleTransportation::StartTracking(G4Track *aTrack) {
  ...
  if (DoesGlobalFieldExist()) {
    ...
    //  including safety values (in case of overlaps and to wipe for first
    //  track).
...
G4double
MonopoleTransportation::AtRestGetPhysicalInteractionLength(const G4Track &,
                                                           G4ForceCondition *) {
  return -1.0;
}
