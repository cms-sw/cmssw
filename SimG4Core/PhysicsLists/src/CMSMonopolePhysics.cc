...
CMSMonopolePhysics::CMSMonopolePhysics(const HepPDT::ParticleDataTable *pdt,
  ...
  if (pdt && mass > 0.0) {
    ...
    for (HepPDT::ParticleDataTable::const_iterator p = pdt->begin();
      ...
      if (strcmp(particleName.c_str(), "Monopole") == 0) {
        ...
        if (verbose > 0)
          G4cout << "CMSMonopolePhysics: Monopole[" << ii << "] "
                 << particleName << " Mass " << particle.mass()
  ...
  if (verbose > 0)
    G4cout << "CMSMonopolePhysics has " << names.size()
...
void CMSMonopolePhysics::ConstructParticle() {
  ...
  for (unsigned int ii = 0; ii < names.size(); ++ii) {
    ...
    if (!monopoles[ii]) {
      ...
      Monopole *mpl = new Monopole(names[ii], pdgEncodings[ii], masses[ii], mc,
                                   elCharges[ii]);
      monopoles[ii] = mpl;
      if (verbose > 0)
        G4cout << "Create Monopole " << names[ii] << " of mass "
               << masses[ii] / CLHEP::GeV << " GeV, magnetic charge " << mc
               << ", electric charge " << elCharges[ii] << " and PDG encoding "
               << pdgEncodings[ii] << " at " << monopoles[ii] << G4endl;
    }
  }
}
    ...
    if (monopoles[ii]) {
      ...
      if (verbose > 1) {
        G4cout << "### CMSMonopolePhysics instantiates for "
               << mpl->GetParticleName() << " at " << mpl << " Mass "
               << mass / CLHEP::GeV << " GeV Mag " << magn
      ...
      if (magn != 0.0) {
        ...
        pmanager->AddProcess(new MonopoleTransportation(mpl, verbose), -1, 0,
                             0);
      }
