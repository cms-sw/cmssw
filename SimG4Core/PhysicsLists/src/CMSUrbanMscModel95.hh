...
#ifndef CMSUrbanMscModel95_h
#define CMSUrbanMscModel95_h 1
...
class CMSUrbanMscModel95 : public G4VMscModel
{

public:

  CMSUrbanMscModel95(const G4String& nam = "UrbanMsc95");

  virtual ~CMSUrbanMscModel95();
...
private:
  ...
  CMSUrbanMscModel95 & operator=(const  CMSUrbanMscModel95 &right);
  CMSUrbanMscModel95(const  CMSUrbanMscModel95&);
...
inline
G4double CMSUrbanMscModel95::GetLambda(G4double e)
{
  G4double x;
  if(theLambdaTable) {
...
inline
void CMSUrbanMscModel95::SetParticle(const G4ParticleDefinition* p)
{
  if (p != particle) {
    particle = p;
    mass = p->GetPDGMass();
...
inline
void CMSUrbanMscModel95::UpdateCache()                                   
{
    lnZ = std::log(Zeff);
