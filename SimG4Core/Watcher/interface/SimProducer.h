...
template <class T> class ProductInfo : public ProductInfoBase {
public:
...
public:
  ...
  void registerProducts(edm::ProducerBase &iProd) {
    std::for_each(m_info.begin(), m_info.end(),
                  std::bind(&simproducer::ProductInfoBase::registerProduct,
                            std::placeholders::_1, &iProd));
  }

protected:
  template <class T> void produces() { produces<T>(""); }

  template <class T> void produces(const std::string &instanceName) {
    m_info.push_back(
