...
{
    DCOUT('G', "DDG4Builder::convertLV(): DDLogicalPart = " << part);
...
{
    DCOUT('G', "DDDetConstr::ConvertMaterial: material=" << material);
    G4Material * result = 0;
    if (material)
    {
	// only if it's a valid DDD-material
	if((result = mats_[material]))
	{ DCOUT('g', "  is already converted"); return result; }
    } 
    else
    {
	// only if it's NOT a valid DDD-material
	DCOUT('G', "  material is not valid (in the DDD sense!)");
	...
	DCOUT('g', "  building an elementary material"
	      << " z=" << material.z()
	      << " a=" << material.a()/g*mole
...
G4LogicalVolume * DDG4Builder::BuildGeometry()
{
	...
	if (git->size()) 
	{
		...
		if ((rm.colX().cross(rm.colY()))*rm.colZ()<0)
		    DCOUT('G', ">>Reflection encountered: " << gra.edgeData(cit->second)->rot_ );
		DCOUT('G', ">>Placement d=" << gra.nodeData(cit->first).ddname() 
