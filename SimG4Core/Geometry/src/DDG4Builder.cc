...
G4LogicalVolume *DDG4Builder::BuildGeometry(SensitiveDetectorCatalog &catalog) {
  ...
  for (; git != gend; ++git) {
	...
	edm::LogVerbatim("SimG4CoreGeometry") << "Position " << gra.nodeData(cit->first).name().name() << ":" << gra.edgeData(cit->second)->copyno() + offset + tag << " in " << g4LV->GetName() << " at " << tempTran << " with rotation matrix (" << x.X() << ", " << x.Y() << ", " << x.Z() << ", " << y.X() << ", " << y.Y() << ", " << y.Z() << ", " << z.X() << ", " << z.Y() << ", " << z.Z() << ")";
