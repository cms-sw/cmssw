...
DDGeometryReturnType DDG4Builder::BuildGeometry() {
  ...
  typedef DDCompactView::GraphType GraphType;
  const GraphType & gra = compactView->graph();
  typedef GraphType::const_adj_iterator adjl_iterator;
  adjl_iterator git = gra.begin();
  adjl_iterator gend = gra.end();    

  GraphType::index_type i=0;
  for (; git != gend; ++git) {
    ...
    if (!git->empty()) {
      ...
      GraphType::edge_list::const_iterator cit  = git->begin();
      GraphType::edge_list::const_iterator cend = git->end();
      for (; cit != cend; ++cit) {
	// fetch specific data
