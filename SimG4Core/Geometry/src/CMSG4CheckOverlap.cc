...
CMSG4CheckOverlap::CMSG4CheckOverlap(const edm::ParameterSet &p, std::string& regFile, CustomUIsession* session, G4VPhysicalVolume* world) {
  ...
  if(mat) {
	...
	<< "CMSG4CheckOverlap: output file <" << sss << "> is opened";
  ...
  if(reg) {
    const std::string qqq = (regFile == "") ? ss : regFile;
  ...
  if(oFlag) {
	...
	<< "CMSG4CheckOverlap: file <" << sss << "> is not opened - no report provided";
      ...
      edm::LogVerbatim("SimG4CoreGeometry") 
	<< "CMSG4CheckOverlap: output file <" << ss << "> is opened";
      session->sendToFile(&fout);
...
void CMSG4CheckOverlap::makeReportForMaterials(std::ofstream& fout) {
  ...
  for(int i=0; i<nmat; ++i) {
    fout << "### Material " << i << "   " << ((*mattab)[i])->GetNumberOfElements()
	 << " elements\n";
    fout << (*mattab)[i] << "\n";
  }
  fout << "=====================================================================" << "\n"; 
  ...
  for(int i=0; i<ncouples; ++i) {
    ...
    if(couple->IsUsed()) { fout << "Yes\n"; }
    else                 { fout << "No \n"; }
    fout << " Material : " << couple->GetMaterial()->GetName() << "\n";
    fout << " Range cuts        : " 
	 << " gamma  " << G4BestUnit(aCut->GetProductionCut(0),"Length")
	 << "    e-  " << G4BestUnit(aCut->GetProductionCut(1),"Length")
	 << "    e+  " << G4BestUnit(aCut->GetProductionCut(2),"Length")
	 << " proton " << G4BestUnit(aCut->GetProductionCut(3),"Length") << "\n";
    fout << " Energy thresholds : ";
    fout << " gamma  " << G4BestUnit((*gcut)[i],"Energy")
         << "    e-  " << G4BestUnit((*ecut)[i],"Energy")
	 << "    e+  " << G4BestUnit((*pcut)[i],"Energy") 
	 << " proton " << G4BestUnit((*icut)[i],"Energy") << "\n";
  }
  fout << "======================================================================" << "\n"; 
}
...
void CMSG4CheckOverlap::makeReportForOverlaps(std::ofstream& fout, const edm::ParameterSet &p) {
  ...
  fout << "CMSG4OverlapCheck is initialised with " << nodeNames.size() << " nodes; "
       ...
       << " mm; verbose: " << verbose << "\n"
       << "               RegionFlag: " << regionFlag << "  PVname: " << PVname 
       << "  LVname: " << LVname << "\n"
       << "               Nlv= " << numLV << "   Npv= " << numPV << "\n";
  fout << "=====================================================================" << "\n"; 
