...
void SensitiveDetectorCatalog::insert(const std::string &cN,
  ...
  LogDebug("SimG4CoreGeometry")
      ...
      << ")\n"
      << "                         has     " << readoutNames().size()
      << " ROUs " << readoutNames().front() << "\n"
      << "                         has     " << classNames().size()
      << " classes " << classNames().front();
#endif
}
...
SensitiveDetectorCatalog::readoutNames(const std::string &className) const {
  return theClassNameMap.at(className);
}

const std::vector<std::string> &
SensitiveDetectorCatalog::logicalNames(const std::string &readoutName) const {
  return theROUNameMap.at(readoutName);
}

std::vector<std::string> SensitiveDetectorCatalog::logicalNamesFromClassName(
...
std::string
SensitiveDetectorCatalog::className(const std::string &readoutName) const {
