...
G4CheckOverlap::G4CheckOverlap(const edm::ParameterSet &p) {
  ...
  G4cout << "G4OverlapCheck is initialised with " << nodeNames.size()
         << " nodes; "
         << " nPoints= " << nPoints << "; tolerance= " << tolerance / mm
         << " mm; verbose: " << verbose << "\n"
  ...
  if (0 < nn) {
    for (unsigned int ii = 0; ii < nn; ++ii) {
      if (nodeNames[ii].empty() || "world" == nodeNames[ii] ||
          "World" == nodeNames[ii]) {
        ...
        G4cout
            << "---------------------------------------------------------------"
            << G4endl;
        G4cout << "### Check overlaps for G4Region Node[" << ii
               << "] : " << nodeNames[ii] << G4endl;
        ...
        for (unsigned int iLV = 0; iLV < numRootLV; ++iLV, ++rootLVItr) {
          ...
          lv = *rootLVItr;
          G4cout << "### Check overlaps for G4LogicalVolume " << lv->GetName()
                 << G4endl;
        ...
        G4cout << "### Check overlaps for PhysVolume Node[" << ii
               << "] : " << nodeNames[ii] << G4endl;
  ...
  if (!PVname.empty()) {
    G4cout << "----------- List of PhysVolumes by name -----------------"
           << G4endl;
    for (unsigned int i = 0; i < numPV; ++i) {
      if (PVname == ((*pvs)[i])->GetName()) {
        G4cout << " ##### PhysVolume " << PVname << " ["
               << ((*pvs)[i])->GetCopyNo()
               << "]  LV: " << ((*pvs)[i])->GetLogicalVolume()->GetName()
               << " Mother LV: " << ((*pvs)[i])->GetMotherLogical()->GetName()
               << " Region: "
               << ((*pvs)[i])->GetLogicalVolume()->GetRegion()->GetName()
               << G4endl;
        G4cout << "       Translation: " << ((*pvs)[i])->GetObjectTranslation()
               << G4endl;
        G4cout << "       Rotation:    "
               << ((*pvs)[i])->GetObjectRotationValue() << G4endl;
        if (gdmlFlag) {
          G4GDMLParser gdml;
          gdml.Write(PVname + ".gdml", (*pvs)[i], true);
        }
  ...
  if (!LVname.empty()) {
    G4cout << "---------- List of Logical Volumes by name ------------------"
           << G4endl;
