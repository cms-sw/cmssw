...
DDG4SolidConverter::DDG4SolidConverter() {
  ...
  convDispatch_[DDSolidShape::ddpolycone_rrz] =
      DDG4SolidConverter::polycone_rrz;
  convDispatch_[DDSolidShape::ddpolycone_rz] = DDG4SolidConverter::polycone_rz;
  convDispatch_[DDSolidShape::ddpolyhedra_rrz] =
      DDG4SolidConverter::polyhedra_rrz;
  convDispatch_[DDSolidShape::ddpolyhedra_rz] =
      DDG4SolidConverter::polyhedra_rz;
  convDispatch_[DDSolidShape::ddextrudedpolygon] =
  ...
  convDispatch_[DDSolidShape::ddintersection] =
  ...
  convDispatch_[DDSolidShape::ddellipticaltube] =
...
G4VSolid *DDG4SolidConverter::convert(const DDSolid &solid) {
  ...
  std::map<DDSolidShape, FNPTR>::iterator it =
      convDispatch_.find(solid.shape());
...
G4VSolid *DDG4SolidConverter::polycone_rz(const DDSolid &solid) {
  ...
  LogDebug("SimG4CoreGeometry")
      << "sp=" << (*par_)[0] / CLHEP::deg << " ep=" << (*par_)[1] / CLHEP::deg;
  /*
  ...
  */
  return new G4Polycone(solid.name().name(), (*par_)[0],
...
G4VSolid *DDG4SolidConverter::polyhedra_rz(const DDSolid &solid) {
  ...
  return new G4Polyhedra(solid.name().name(), (*par_)[1], (*par_)[2],
...
G4VSolid *DDG4SolidConverter::unionsolid(const DDSolid &solid) {
  LogDebug("SimG4CoreGeometry")
      << "DDG4SolidConverter: unionsolid = " << solid.name();
  ...
  if (bs) {
    ...
    CLHEP::HepRep3x3 temprep(tdbl[0], tdbl[1], tdbl[2], tdbl[3], tdbl[4],
                             tdbl[5], tdbl[6], tdbl[7], tdbl[8]);
    CLHEP::Hep3Vector temphvec(bs.translation().X(), bs.translation().Y(),
                               bs.translation().Z());
    us = new G4UnionSolid(solid.name().name(), sa, sb,
...
G4VSolid *DDG4SolidConverter::subtraction(const DDSolid &solid) {
  ...
  if (bs) {
    ...
    CLHEP::Hep3Vector temphvec(bs.translation().X(), bs.translation().Y(),
                               bs.translation().Z());
    us = new G4SubtractionSolid(solid.name().name(), sa, sb,
                                new CLHEP::HepRotation(temprep), temphvec);
  }
  return us;
}
...
G4VSolid *DDG4SolidConverter::intersection(const DDSolid &solid) {
  ...
  if (bs) {
    ...
    CLHEP::Hep3Vector temphvec(bs.translation().X(), bs.translation().Y(),
                               bs.translation().Z());
    us = new G4IntersectionSolid(solid.name().name(), sa, sb,
                                 new CLHEP::HepRotation(temprep), temphvec);
  }
  return us;
}
...
G4VSolid *DDG4SolidConverter::pseudotrap(const DDSolid &solid) {
  ...
  if (intersec) {
    ...
    G4VSolid *tubicCap = new G4SubtractionSolid(
        name, tubs, new G4Box(name, 1.1 * x, sqrt(r * r - x * x), 1.1 * h),
...
G4VSolid *DDG4SolidConverter::trunctubs(const DDSolid &solid) {
  ...
  LogDebug("SimG4CoreGeometry")
      << "MantisConverter: solidshape="
      << DDSolidShapesName::name(solid.shape()) << " " << solid;
  ...
  if (rIn <= 0 || rOut <= 0 || cutAtStart <= 0 || cutAtDelta <= 0) {
    throw cms::Exception(
        ...
        "TruncTubs " + std::string(tt.name().fullname()) +
  ...
  LogDebug("SimG4CoreGeometry")
      << "G4Tubs: " << rIn / CLHEP::cm << ' ' << rOut / CLHEP::cm << ' '
  ...
  double boxX(30. * rOut),
      boxY(20. *
           rOut); // exaggerate dimensions - does not matter, it's subtracted!
...
G4VSolid *DDG4SolidConverter::sphere(const DDSolid &solid) {
  ...
  DDSphere sp(solid);
  return new G4Sphere(solid.name().name(), sp.innerRadius(), sp.outerRadius(),
                      sp.startPhi(), sp.deltaPhi(), sp.startTheta(),
                      sp.deltaTheta());
}
...
G4VSolid *DDG4SolidConverter::ellipticaltube(const DDSolid &solid) {
  ...
  return new G4EllipticalTube(solid.name().name(), sp.xSemiAxis(),
                              sp.ySemiAxis(), sp.zHeight());
}
