...
bool DTParametrizedDriftAlgo::compute(
  ...
  return compute(layer, wireId, digi.time(), angle, globWirePos, leftPoint, rightPoint, error, 1);
}
...
bool DTParametrizedDriftAlgo::compute(const DTLayer* layer,
  ...
  return compute(layer, wireId, recHit1D.digiTime(), angle, globWirePos, newHit1D, 2);
}
...
bool DTParametrizedDriftAlgo::compute(const DTLayer* layer,
                                      ...
                                      DTRecHit1D& newHit1D) const {
  return compute(layer, recHit1D.wireId(), recHit1D.digiTime(), angle, globPos, newHit1D, 3);
}
