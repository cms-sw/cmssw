...
/* Operations */
void DTAnalyzerDetailed::analyze(const Event& event, const EventSetup& eventSetup) {
...
void DTAnalyzerDetailed::analyzeDTSegments(const Event& event, const EventSetup& eventSetup) {
  if (debug)
  ...
  for (auto ch : chs) {
    ...
    for (DTRecSegment4DCollection::const_iterator seg = segsch.first; seg != segsch.second; ++seg) {
      ...
      if (phiSeg) {
        ...
        for (double tMax : tMaxs1) {
          histo(hName("hMeanTimerSeg", slid1))->Fill(tMax);
          histo2d(hName("hMeanTimerSegVsNHits", slid1))->Fill(phiHits.size(), tMax);
          if ((*seg).hasZed()) {
            histo2d(hName("hMeanTimerSegAlongWire", slid1))->Fill((*seg).localPosition().y(), tMax);
            histo2d(hName("hMeanTimerSegVsAngle", slid1))->Fill(M_PI - (*seg).localDirection().theta(), tMax);
          }
        }
      ...
      if (zedSeg) {
        ...
        for (double tMax : tMaxs) {
          ...
          histo2d(hName("hMeanTimerSegVsNHits", slid))->Fill(zedHits.size(), tMax);
          if ((*seg).hasPhi()) {
            histo2d(hName("hMeanTimerSegAlongWire", slid))->Fill((*seg).localPosition().x(), tMax);
            histo2d(hName("hMeanTimerSegVsAngle", slid))->Fill(M_PI - (*seg).localDirection().theta(), tMax);
          }
        }
      }

      histo(hName("hChi2Seg", chid))->Fill((*seg).chi2() / (*seg).degreesOfFreedom());
      if (phiSeg) {
        histo(hName("hNHitsSegPhi", chid))->Fill(phiSeg->recHits().size());
        histo2d(hName("hNHitsSegPhiVsAngle", chid))
            ->Fill(M_PI - phiSeg->localDirection().theta(), phiSeg->recHits().size());
        ...
        histo2d(hName("hNHitsSegPhiVsOtherHits", chid))
            ->Fill((rangeH1.second - rangeH1.first) + (rangeH3.second - rangeH3.first), phiSeg->recHits().size());
        histo2d(hName("hNHitsSegPhiVsNumSegs", chid))->Fill(nSegsCh, phiSeg->recHits().size());
        histo2d(hName("hNHitsSegPhiVsChi2", chid))
            ->Fill(phiSeg->chi2() / phiSeg->degreesOfFreedom(), phiSeg->recHits().size());
        histo(hName("hChi2SegPhi", chid))->Fill(phiSeg->chi2() / phiSeg->degreesOfFreedom());
      }

      if (zedSeg) {
        histo(hName("hNHitsSegZed", chid))->Fill(zedSeg->recHits().size());
        histo2d(hName("hNHitsSegZedVsAngle", chid))
            ->Fill(M_PI - zedSeg->localDirection().theta(), zedSeg->recHits().size());
        DTSuperLayerId slid(chid, 2);
        DTRecHitCollection::range rangeH = dtRecHits->get(DTRangeMapAccessor::layersBySuperLayer(slid));
        histo2d(hName("hNHitsSegZedVsOtherHits", chid))->Fill(rangeH.second - rangeH.first, zedSeg->recHits().size());
        histo2d(hName("hNHitsSegZedVsNumSegs", chid))->Fill(nSegsCh, zedSeg->recHits().size());
        histo2d(hName("hNHitsSegZedVsChi2", chid))
            ->Fill(zedSeg->chi2() / zedSeg->degreesOfFreedom(), zedSeg->recHits().size());
        histo(hName("hChi2SegZed", chid))->Fill(zedSeg->chi2() / zedSeg->degreesOfFreedom());
      }
      if (phiSeg && zedSeg)
      ...
      if (phiSeg) {
        ...
        for (const auto& delta : deltas) {
          ...
          histo2d(hName("hHitResidualSegVsWireDis", ch->id()))->Fill(delta.wireDistance, delta.value);

          histo2d(hName("hHitResidualSegVsAngle", ch->id()))->Fill(delta.angle, delta.value);
          histo2d(hName("hHitResidualSegVsNHits", ch->id()))->Fill(phiSeg->recHits().size(), delta.value);
          histo2d(hName("hHitResidualSegVsChi2", ch->id()))->Fill(phiSeg->chi2(), delta.value);

          if ((*seg).hasPhi())
            histo2d(hName("hHitResidualSegAlongWire", ch->id()))->Fill((*seg).localPosition().x(), delta.value);
        }
      }
      ...
      if (zedSeg) {
        ...
        for (const auto& delta : deltas) {
          ...
          histo2d(hName("hHitResidualSegVsWireDis", sl->id()))->Fill(delta.wireDistance, delta.value);

          histo2d(hName("hHitResidualSegVsAngle", sl->id()))->Fill(delta.angle, delta.value);
          histo2d(hName("hHitResidualSegVsNHits", sl->id()))->Fill(zedSeg->recHits().size(), delta.value);
          histo2d(hName("hHitResidualSegVsChi2", sl->id()))->Fill(zedSeg->chi2(), delta.value);

          if ((*seg).hasPhi())
            histo2d(hName("hHitResidualSegAlongWire", sl->id()))->Fill((*seg).localPosition().x(), delta.value);
        }
      }  // if ZedSeg
...
TH1F* DTAnalyzerDetailed::histo(const string& name) const {
  if (TH1F* h = dynamic_cast<TH1F*>(theFile->Get(name.c_str())))
    ...
    throw cms::Exception("DTAnalyzerDetailed") << " Not a TH1F " << name;
}
...
TH2F* DTAnalyzerDetailed::histo2d(const string& name) const {
  if (TH2F* h = dynamic_cast<TH2F*>(theFile->Get(name.c_str())))
    ...
    throw cms::Exception("DTAnalyzerDetailed") << " Not a TH2F " << name;
}
...
template <class T>
string DTAnalyzerDetailed::hName(const string& s, const T& id) const {
