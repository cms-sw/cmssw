...
process.load("RecoLocalMuon.DTSegment.dt4DSegments_LPPatternReco4D_ParamDrift_cfi")
...
dtlocalreco = cms.Sequence(process.dt1DRecHits*process.dt4DSegments)
#process.jobPath = cms.Path(process.muonDTDigis + dtlocalreco)

#if instead we want also the validation
validation = cms.Sequence(process.rechivalidationNo2D *process.seg2dsuperphivalidation*process.seg4dvalidation)
process.jobPath = cms.Path(process.muonDTDigis + dtlocalreco + validation)
