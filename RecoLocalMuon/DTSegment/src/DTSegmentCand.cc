...
DTSegmentCand::operator DTSLRecSegment2D*() const{
  ...
  for(const auto & theHit : theHits) {
    ...
    GlobalPoint hitGlobalPos =
      theSL->toGlobal( theHit.first->localPosition(theHit.second) );

    LocalPoint hitPosInLayer = 
      theSL->layer( theHit.first->id().layerId() )->toLocal(hitGlobalPos);
...
DTSegmentCand::operator DTChamberRecSegment2D*() const{
  ...
  for(const auto & theHit : theHits) {
    ...
    LocalPoint hitPosInLayer =
      ...
      ->layer( theHit.first->id().layerId() )->toLocal(hitGlobalPos);
