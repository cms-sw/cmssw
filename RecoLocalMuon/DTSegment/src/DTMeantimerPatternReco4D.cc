...
OwnVector<DTRecSegment4D> DTMeantimerPatternReco4D::reconstruct() {
  ...
  if (allDTRecHits) {
    ...
    if (sl) {
      ...
      if (debug)
        cout << "Reconstructing Theta segments" << endl;
    ...
    for (vector<DTSegmentCand*>::const_iterator phi = resultPhi.begin(); phi != resultPhi.end(); ++phi) {
      ...
      if (hasZed) {
        ...
        for (vector<DTSLRecSegment2D>::const_iterator zed = theSegments2DTheta.begin(); zed != theSegments2DTheta.end();
          ...
          if (applyT0corr)
            theUpdator->update(newSeg, true, true);
...
vector<DTSegmentCand*> DTMeantimerPatternReco4D::buildPhiSuperSegmentsCandidates(
  ...
  if (!theHitsFromPhi1.empty())
    ...
    if (debug)
      cout << "DTMeantimerPatternReco4D::buildPhiSuperSegmentsCandidates: "
