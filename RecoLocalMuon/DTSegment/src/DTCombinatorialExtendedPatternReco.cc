...
vector<DTSegmentCand::AssPoint> DTCombinatorialExtendedPatternReco::findCompatibleHits(
  ...
  if (nCompatibleHits < 3 || find(theTriedPattern.begin(), theTriedPattern.end(), tried) == theTriedPattern.end()) {
    ...
    if (debug) {
      auto t = find(theTriedPattern.begin(), theTriedPattern.end(), tried);
      cout << "Already tried";
...
DTSegmentExtendedCand* DTCombinatorialExtendedPatternReco::buildBestSegment(std::vector<DTSegmentCand::AssPoint>& hits,
  ...
  // by #hits and chi2.
  auto bestCandIter = extendedCands.end();
  double minChi2 = 999999.;
  unsigned int maxNumHits = 0;
  for (auto iter = extendedCands.begin(); iter != extendedCands.end(); ++iter) {
...
vector<DTSegmentExtendedCand*> DTCombinatorialExtendedPatternReco::extendCandidates(vector<DTSegmentCand*>& candidates,
  ...
  if (sl->id().superLayer() != 2) {
    for (auto candidate : candidates) {
      auto* extendedCand = new DTSegmentExtendedCand(candidate);
  ...
  for (const auto& theCluster : theClusters) {
    if (debug)
      cout << "CLUS: " << theCluster.superLayerId() << endl;
    if (theCluster.superLayerId().superLayer() == 2 && closeSL(sl->id(), theCluster.superLayerId())) {
      // and then get their pos in the actual SL frame
      const DTSuperLayer* clusSl = theDTGeometry->superLayer(theCluster.superLayerId());
      LocalPoint pos = sl->toLocal(clusSl->toGlobal(theCluster.localPosition()));
      //LocalError err=sl->toLocal(clusSl->toGlobal((*clus).localPositionError()));
      LocalError err = theCluster.localPositionError();
      clustersWithPos.emplace_back(theCluster, pos, err);
    }
  }
  if (debug)
  ...
  for (auto candidate : candidates) {
    ...
    if (extendedCand->superLayer()->id().superLayer() == 2) {
      ...
      for (const auto& clustersWithPo : clustersWithPos) {
        if (extendedCand->isCompatible(clustersWithPo)) {
          if (debug)
            cout << "is compatible " << endl;
          // add compatible cluster
          extendedCand->addClus(clustersWithPo);
        }
      }
      // fit the segment
      if (debug)
