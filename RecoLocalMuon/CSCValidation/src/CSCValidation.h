...
private: 
  ...
  void  doNoiseHits(const edm::Handle<CSCRecHit2DCollection>& recHits, const edm::Handle<CSCSegmentCollection>& cscSegments,
                    edm::ESHandle<CSCGeometry> cscGeom,  const edm::Handle<CSCStripDigiCollection>& strips);
  ...
  void  doTimeMonitoring(const edm::Handle<CSCRecHit2DCollection>& recHits, const edm::Handle<CSCSegmentCollection>& cscSegments,
                         const edm::Handle<CSCALCTDigiCollection>& alcts, const edm::Handle<CSCCLCTDigiCollection>& clcts,
                         const edm::Handle<CSCCorrelatedLCTDigiCollection>& correlatedlcts,
                         const edm::Handle<L1MuGMTReadoutCollection>& pCollection, const edm::ESHandle<CSCGeometry>& cscGeom,
                         const edm::EventSetup& eventSetup, const edm::Event &event);
  bool  doHLT(const edm::Handle<edm::TriggerResults>& hltResults);


  // some useful functions
  bool   filterEvents(const edm::Handle<CSCRecHit2DCollection>& recHits, const edm::Handle<CSCSegmentCollection>& cscSegments,
  ...
  void   findNonAssociatedRecHits(const edm::ESHandle<CSCGeometry>& cscGeom,  const edm::Handle<CSCStripDigiCollection>& strips);
  ...
  double extrapolate1D(double initPosition, double initDirection, double parameterOfTheLine){
    ...
    bool withinSensitiveRegion(const LocalPoint& localPos, const std::array<const float, 4> & layerBounds, int station, int ring, float shiftFromEdge, float shiftFromDeadZone);
