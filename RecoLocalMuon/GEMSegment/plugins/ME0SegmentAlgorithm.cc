...
ME0SegmentAlgorithm::chainHits(const ME0Chamber * chamber, const HitAndPositionContainer& rechits) {
  ...
  // Create one seed per hit
  for (const auto & rechit : rechits){
    if(std::abs(rechit.rh->tof()) > dTimeChainBoxMax) continue;
    seeds.push_back(HitAndPositionPtrContainer(1,&rechit));
  }
...
bool ME0SegmentAlgorithm::isGoodToMerge(const ME0Chamber * chamber, const HitAndPositionPtrContainer& newChain, const HitAndPositionPtrContainer& oldChain) {
  for(auto iRH_new : newChain){
    ...
    for(auto iRH_old : oldChain){
      ...
      if (std::abs(int(iRH_new->layer) - int(iRH_old->layer)) >= (chamber->id().nlayers()-1)) continue;
      ...
      if (std::abs(iRH_new->rh->tof() - iRH_old->rh->tof()) >= dTimeChainBoxMax) continue;
