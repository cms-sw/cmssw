...
std::vector<GEMSegment> GEMSegAlgoPV::run(const GEMEnsemble& ensemble, const EnsembleHitContainer& rechits) {
  ...
  edm::LogVerbatim("GEMSegAlgoPV") << "[GEMSegAlgoPV::run] build segments in chamber " << chId;
  ...
  if(preClustering) {
    ...
    if(preClustering_useChaining){
      ...
      edm::LogVerbatim("GEMSegAlgoPV") << "[GEMSegAlgoPV::run] preClustering :: use Chaining";
      rechits_clusters = this->chainHits( rechits );
    }
    else{
      // it uses X,Y information + configurable parameters
      edm::LogVerbatim("GEMSegAlgoPV") << "[GEMSegAlgoPV::run] Clustering";
      rechits_clusters = this->clusterHits(rechits );
    }
    // loop over the found clusters:
      edm::LogVerbatim("GEMSegAlgoPV") << "[GEMSegAlgoPV::run] Loop over clusters and build segments";
...
bool GEMSegAlgoPV::isGoodToMerge(const EnsembleHitContainer& newChain, const EnsembleHitContainer& oldChain) {
  ...
  */
  ...
  // edm::LogVerbatim("GEMSegAlgoPV") << "[GEMSegAlgoPV::isGoodToMerge] :: oldChain ::";
  ...
  for(size_t jRH_new = 0; jRH_new<phi_new.size(); ++jRH_new){
	...
	// edm::LogVerbatim("GEMSegAlgoPV") << "[GoodToMerge::Compare] :: layer_new = "<<layer_new[jRH_new]<<" layer_old = "<<layer_old[jRH_old]<<" ==> bool = "<<layerRequirementOK;
      }
...
std::vector<GEMSegment> GEMSegAlgoPV::buildSegments(const EnsembleHitContainer& rechits) {
  ...
  edm::LogVerbatim("GEMSegAlgoPV") << "[GEMSegAlgoPV::buildSegments] GEMSegment fit done :: fit is good = "<<goodfit;
