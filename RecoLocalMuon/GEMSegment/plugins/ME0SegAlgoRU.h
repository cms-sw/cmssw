...
private:
	...
	void lookForSegments(const SegmentParameters& params, unsigned int n_seg_min, const HitAndPositionContainer& rechits, const std::vector<unsigned int>& recHits_per_layer, BoolContainer& used, std::vector<ME0Segment>& segments) const;
	//Look for any hits between the two seed hits consistent with a segment
	void tryAddingHitsToSegment( float maxTOF,float maxETA,  float maxPhi, float maxChi2, std::unique_ptr<MuonSegFit>& current_fit, HitAndPositionPtrContainer& proto_segment,
	...
	void pruneBadHits(float maxChi2, HitAndPositionPtrContainer& proto_segment,  std::unique_ptr<MuonSegFit>& fit, unsigned int n_seg_min) const;
	...
	bool areHitsCloseInEta(float maxETA, bool beamConst,  const GlobalPoint& h1, const GlobalPoint& h2) const;
	bool areHitsCloseInGlobalPhi(float maxPHI, unsigned int nLayDisp, const GlobalPoint& h1, const GlobalPoint& h2) const;
	...
	bool hasHitOnLayer(const HitAndPositionPtrContainer& proto_segment, unsigned int layer) const;
	//Produce a new fit
	...
	bool areHitsConsistentInTime(float maxTOF, const HitAndPositionPtrContainer& proto_segment, const HitAndPosition& h)  const;
	//Is this new hit btw the seeds near the segment fit?
	bool isHitNearSegment(float maxETA, float maxPHI, const std::unique_ptr<MuonSegFit>& fit, const HitAndPositionPtrContainer& proto_segment, const HitAndPosition& h)  const;
	...
	void increaseProtoSegment(float maxChi2,  std::unique_ptr<MuonSegFit>& current_fit, HitAndPositionPtrContainer& current_proto_segment, const HitAndPosition& new_hit) const;
