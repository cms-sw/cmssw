...
void CSCSegmentVisualise::analyze(const edm::Event& event, const edm::EventSetup& eventSetup) {
  ...
  for (chIt=chambers.begin(); chIt != chambers.end(); ++chIt) {
    ...
    if ( int(cscRecHits.size()) < minRechitChamber ) continue;
    if ( int(cscRecHits.size()) > maxRechitChamber ) continue;
    if ( chamber->id().ring() == 4) continue;  // not interested in ME1a for now... 
    ...
    for (int i = 0; i < nHits; ++i) {
      ...
      if (i == 0 ) {
        ...
        if (lphit.x() < xmin) xmin = lphit.x();
        if (lphit.y() < ymin) ymin = lphit.y();
        if (lphit.x() > xmax) xmax = lphit.x();
        if (lphit.y() > ymax) ymax = lphit.y();
      }
    }
   ...
   std::cout << "done filling rechit histos " << std::endl;
    ...
    for (CSCRecHit2DCollection::const_iterator rec_it = range.first; rec_it != range.second; rec_it++) {            
      ...
      if (isElec) eRecHits.push_back(&(*rec_it));
    }    
