...
bool CSCEfficiency::filter(edm::Event &event,
  ...
  //---- zero or more than one instance exists in the event an exception is
  // thrown.
  if (printalot)
    printf("\tpass handles\n");
  ...
  if (useTrigger) {
    ...
    // trigger names can be find in
    // HLTrigger/Configuration/python/HLT_2E30_cff.py (or?)
  ...
  fillDigiInfo(alcts, clcts, correlatedlcts, wires, strips, simhits, rechits,
               segments, cscGeom);
  //
  ...
  if (isIPdata) {
    ...
    for (reco::MuonCollection::const_iterator muon = muons->begin();
      ...
      DataFlow->Fill(31.);
      if (printalot) {
        std::cout << "  iMuon = " << muon - muons->begin()
                  << " charge = " << muon->charge() << " p = " << muon->p()
                  << " pt = " << muon->pt() << " eta = " << muon->eta()
                  << " phi = " << muon->phi()
                  << " matches = " << muon->matches().size()
                  << " matched Seg = "
                  << muon->numberOfMatches(
                         reco::Muon::SegmentAndTrackArbitration)
                  << " GLB/TR/STA = " << muon->isGlobalMuon() << "/"
                  << muon->isTrackerMuon() << "/" << muon->isStandAloneMuon()
                  << std::endl;
      }
      ...
      DataFlow->Fill(32.);
      double relISO = (muon->isolationR03().sumPt + muon->isolationR03().emEt +
                       muon->isolationR03().hadEt) /
                      muon->track()->pt();
      if (printalot) {
        std::cout << " relISO = " << relISO
                  << " emVetoEt = " << muon->isolationR03().emVetoEt
                  << " caloComp = " << muon::caloCompatibility(*(muon))
                  << " dxy = "
                  << fabs(muon->track()->dxy(vertexBeamSpot.position()))
                  << std::endl;
      }
      if (
          // relISO>0.1 || muon::caloCompatibility(*(muon))<.90 ||
          fabs(muon->track()->dxy(vertexBeamSpot.position())) > 0.2 ||
          muon->pt() < 6.) {
        continue;
      }
      DataFlow->Fill(33.);
      ...
      // endcap region
      // else if( fabs(zOuter) > 550. && fabs(zOuter) < 650. && rhoOuter <
      // 300.){
      else if (fabs(zOuter) > 550. && fabs(zOuter) < 650. && rhoOuter < 300.) {
        passDepth = false;
      }
      // overlap region
      // else if ( fabs(zOuter) > 680. && fabs(zOuter) < 730. && rhoOuter <
      // 480.){
  ...
  if (printalot)
    std::cout << "Start track loop over " << trackCollection.size() << " tracks"
              << std::endl;
  for (edm::View<reco::Track>::size_type i = 0; i < trackCollection.size();
       ++i) {
    DataFlow->Fill(2.);
    edm::RefToBase<reco::Track> track(trackCollectionH, i);
    // std::cout<<" iTR = "<<i<<" eta = "<<track->eta()<<" phi =
    // "<<track->phi()<<std::cout<<" pt = "<<track->pt()<<std::endl;
    if (isIPdata) {
      if (printalot) {
        std::cout << " nNegativeZ = " << nNegativeZ
      ...
      bool trackOK = false;
      if (printalot) {
        std::cout << " goodMuons_it.size() = " << goodMuons_it.size()
                  << std::endl;
      }
      for (size_t iM = 0; iM < goodMuons_it.size(); ++iM) {
        // std::cout<<" iM = "<<iM<<" eta =
        // "<<goodMuons_it[iM]->track()->eta()<< " phi =
        //"<<goodMuons_it[iM]->track()->phi()<< " pt =
        //"<<goodMuons_it[iM]->track()->pt()<<std::endl;
        ...
        if (printalot) {
          std::cout << " TR mu match to a tr: deltaR = " << deltaR << " dPt = "
                    << track->pt() - goodMuons_it[iM]->track()->pt()
                    << std::endl;
        }
        if (deltaR > 0.01 ||
            fabs(track->pt() - goodMuons_it[iM]->track()->pt()) > 0.1) {
          continue;
        } else {
          trackOK = true;
          if (printalot) {
          ...
          muonOuterZPosition =
              goodMuons_it[iM]->combinedMuon()->outerPosition().z();
          break;
          //++nChosenTracks;
        }
      }
      ...
      if (!i && 2 == trackCollection.size()) {
        ...
        if (track->outerPosition().z() * trackTwo->outerPosition().z() >
            0) { // in one and the same "endcap"
          break;
        }
      }
    }
    ...
    if (printalot) {
      ...
      std::cout << "quality undef/loose/tight/high/confirmed/goodIt/size "
                ...
                << track->quality(reco::Track::qualitySize) << std::endl;
      std::cout << " pt = " << track->pt() << " +-" << track->ptError()
                << " q/pt = " << track->qoverp() << " +- "
                << track->qoverpError() << std::endl;
      // std::cout<<" const Pmin = "<<minTrackMomentum<<" pMax =
      // "<<maxTrackMomentum<<" maxNormChi2 = "<<maxNormChi2<<std::endl;
      ...
      theTrackingGeometry->idToDet((*recHit)->geographicalId()); std::cout<<"hit
      "<<iRH<<" loc pos = " <<(*recHit)->localPosition()<< " glob pos = "
      <<geomDet->toGlobal((*recHit)->localPosition())<<std::endl;
        ++iRH;
      }
      */
    }
    ...
    DataFlow->Fill(7.);
    if (magField &&
        (track->p() < minP || track->p() > maxP)) { // proper energy range
      break;
    }
    DataFlow->Fill(8.);
    ...
    CLHEP::Hep3Vector r3T_inner(track->innerPosition().x(),
                                track->innerPosition().y(),
                                track->innerPosition().z());
    CLHEP::Hep3Vector r3T(track->outerPosition().x(),
                          track->outerPosition().y(),
                          track->outerPosition().z());
    chooseDirection(r3T_inner, r3T); // for non-IP

    CLHEP::Hep3Vector p3T(track->outerMomentum().x(),
    ...
    FreeTrajectoryState ftsStart =
        getFromCLHEP(p3T, r3T, charge, covT, &*(theService->magneticField()));
    if (printalot) {
      std::cout << " p = " << track->p()
                << " norm chi2 = " << track->normalizedChi2() << std::endl;
      std::cout << " dump the very first FTS  = " << debug.dumpFTS(ftsStart)
                << std::endl;
    }
    ...
    for (size_t iSt = 0; iSt < refME.size(); ++iSt) {
      if (printalot) {
        ...
        std::cout << "refME[iSt]: st = " << refME[iSt].station()
      ...
      const CSCChamber *cscChamber_base =
          cscGeom->chamber(refME[iSt].chamberId());
      DetId detId = cscChamber_base->geographicalId();
      if (printalot) {
        std::cout << " base iStation : eta = "
                  << cscGeom->idToDet(detId)->surface().position().eta()
                  << " phi = "
                  << cscGeom->idToDet(detId)->surface().position().phi()
                  << " y = "
                  << cscGeom->idToDet(detId)->surface().position().y()
                  << std::endl;
        std::cout << " dump base iStation detid  = " << debug.dumpMuonId(detId)
                  << std::endl;
        std::cout << " dump FTS start  = " << debug.dumpFTS(ftsStart)
                  << std::endl;
      }
      ...
      if (tSOSDest.isValid()) {
        ...
        if (printalot)
          std::cout << "  dump FTS end   = " << debug.dumpFTS(ftsStart)
                    << std::endl;
        ...
        for (iter = chamberTypes.begin(); iter != chamberTypes.end(); iter++) {
          ...
          if (iter->second && (iterations - 1) == int(iSt)) {
            if (printalot) {
              std::cout << " Chamber type " << iter->first
                        << " is a candidate..." << std::endl;
              std::cout << " station() = " << refME[iSt].station()
                        << " ring() = " << refME[iSt].ring() << " iSt = " << iSt
                        << std::endl;
            }
            std::vector<int> coupleOfChambers;
            //---- which chamber (and its closes neighbor) is penetrated by the
            // track - candidates
            chamberCandidates(refME[iSt].station(), refME[iSt].ring(), phi,
                              coupleOfChambers);
            //---- loop over the two chamber candidates
            for (size_t iCh = 0; iCh < coupleOfChambers.size(); ++iCh) {
              DataFlow->Fill(11.);
              if (printalot)
                std::cout << " Check chamber N = " << coupleOfChambers.at(iCh)
                          << std::endl;
              ;
              if ((!getAbsoluteEfficiency) &&
                  (true == emptyChambers[refME[iSt].endcap() - 1]
                                        [refME[iSt].station() - 1]
                                        [refME[iSt].ring() - 1]
                                        [coupleOfChambers.at(iCh) - FirstCh])) {
                continue;
              }
              CSCDetId theCSCId(refME[iSt].endcap(), refME[iSt].station(),
                                refME[iSt].ring(), coupleOfChambers.at(iCh));
              const CSCChamber *cscChamber =
                  cscGeom->chamber(theCSCId.chamberId());
              const BoundPlane bpCh =
                  cscGeom->idToDet(cscChamber->geographicalId())->surface();
              ...
              //---- only detectors between the inner and outer points of the
              // track are considered for non IP-data
              if (printalot) {
                std::cout << " zIn = " << track->innerPosition().z()
                          << " zOut = " << track->outerPosition().z()
                          << " zSurf = " << bpCh.position().z() << std::endl;
              }
              if (!isIPdata &&
                  (zDistInner * zDistOuter > 0. || fabs(zDistInner) < 15. ||
                   fabs(zDistOuter) < 15.)) { // for non IP-data
                if (printalot) {
                  std::cout
                      << " Not an intermediate (as defined) point... Skip."
                      << std::endl;
                }
                continue;
              }
              ...
              DataFlow->Fill(13.);
              //---- propagate to the chamber (from this ME) if it is a
              // different surface (odd/even chambers)
              ...
              for (int iLayer = 0; iLayer < 6; ++iLayer) {
                ...
                if (printalot) {
                  std::cout << " iLayer = " << iLayer
                            << "   dump FTS init  = " << debug.dumpFTS(ftsInit)
                            << std::endl;
                  std::cout << " dump detid  = "
                            << debug.dumpMuonId(cscChamber->geographicalId())
                            << std::endl;
                  std::cout
                      << "Surface to propagate to:  pos = "
                      << cscChamber->layer(iLayer + 1)->surface().position()
                      << " eta = "
                      << cscChamber->layer(iLayer + 1)
                             ->surface()
                             .position()
                             .eta()
                      << " phi = "
                ...
                tSOSDest = propagate(ftsInit,
                                     cscChamber->layer(iLayer + 1)->surface());
                if (tSOSDest.isValid()) {
                  ftsInit = *tSOSDest.freeState();
                  if (printalot)
                    std::cout << " Propagation between layers successful:  "
                                 "dump FTS end  = "
                              << debug.dumpFTS(ftsInit) << std::endl;
                  getFromFTS(ftsInit, p3_propagated, r3_propagated, charge,
                             cov_propagated);
                } else {
                  if (printalot)
                    std::cout << "Propagation between layers not successful - "
                                 "notValid TSOS"
                              << std::endl;
                ...
                if (extrapolationPassed) {
                  GlobalPoint theExtrapolationPoint(
                      r3_propagated.x(), r3_propagated.y(), r3_propagated.z());
                  LocalPoint theLocalPoint =
                      cscChamber->layer(iLayer + 1)
                          ->toLocal(theExtrapolationPoint);
                  // std::cout<<" Candidate chamber: extrapolated LocalPoint =
                  // "<<theLocalPoint<<std::endl;
                  inDeadZone = (inDeadZone ||
                                !inSensitiveLocalRegion(
                                    theLocalPoint.x(), theLocalPoint.y(),
                                    refME[iSt].station(), refME[iSt].ring()));
                  if (printalot) {
                    std::cout
                        << " Candidate chamber: extrapolated LocalPoint = "
                        << theLocalPoint << "inDeadZone = " << inDeadZone
                        << std::endl;
                  }
              ...
              if (!inDeadZone) { //---- for any layer
                ...
                angle_flag =
                    efficienciesPerChamber(theCSCId, cscChamber, ftsStart);
                ...
                if (printalot)
                  std::cout << " Not in active area for all layers"
                            << std::endl;
              }
            }
            if (tSOSDest.isValid()) {
...
//
bool CSCEfficiency::inSensitiveLocalRegion(double xLocal, double yLocal,
                                           int station, int ring) {
  //---- Good region means sensitive area of a chamber. "Local" stands for the
  // local system
  bool pass = false;
...
bool CSCEfficiency::checkLocal(double yLocal, double yBoundary, int station,
                               int ring) {
  ...
  const float deadZoneHalf =
      0.32 * 7 / 2; // wire spacing * (wires missing + 1)/2
  ...
  if (station > 1 && station < 5) {
    if (2 == ring) {
      ...
      if (yLocal > yBoundary && ((yLocal > deadZoneCenter[0] + cutZone &&
                                  yLocal < deadZoneCenter[1] - cutZone) ||
                                 (yLocal > deadZoneCenter[1] + cutZone &&
                                  yLocal < deadZoneCenter[2] - cutZone) ||
                                 (yLocal > deadZoneCenter[2] + cutZone &&
                                  yLocal < deadZoneCenter[3] - cutZone) ||
                                 (yLocal > deadZoneCenter[3] + cutZone &&
                                  yLocal < deadZoneCenter[4] - cutZone) ||
                                 (yLocal > deadZoneCenter[4] + cutZone &&
                                  yLocal < deadZoneCenter[5] - cutZone))) {
        pass = true;
      }
    } else if (1 == ring) {
      if (2 == station) {
...
void CSCEfficiency::fillDigiInfo(
    edm::Handle<CSCALCTDigiCollection> &alcts,
    edm::Handle<CSCCLCTDigiCollection> &clcts,
    edm::Handle<CSCCorrelatedLCTDigiCollection> &correlatedlcts,
    edm::Handle<CSCWireDigiCollection> &wires,
    ...
    for (int iS = 0; iS < 4; iS++) {
      for (int iR = 0; iR < 4; iR++) {
        for (int iC = 0; iC < NumCh; iC++) {
          ...
          allCLCT[iE][iS][iR][iC] = allALCT[iE][iS][iR][iC] =
              allCorrLCT[iE][iS][iR][iC] = false;
...
void CSCEfficiency::fillLCT_info(
    edm::Handle<CSCALCTDigiCollection> &alcts,
    edm::Handle<CSCCLCTDigiCollection> &clcts,
    edm::Handle<CSCCorrelatedLCTDigiCollection> &correlatedlcts) {
  //---- ALCTDigis
  ...
  for (CSCALCTDigiCollection::DigiRangeIterator j = alcts->begin();
    ...
    for (CSCALCTDigiCollection::const_iterator digiIt = range.first;
      ...
      if ((*digiIt).isValid()) {
        allALCT[id.endcap() - 1][id.station() - 1][id.ring() - 1]
               [id.chamber() - FirstCh] = true;
      }
    } // for digis in layer
  }   // end of for (j=...
  ...
  for (CSCCLCTDigiCollection::DigiRangeIterator j = clcts->begin();
    ...
    for (; digiIt != last; ++digiIt) {
      // Valid digi in the chamber (or in neighbouring chamber)
      if ((*digiIt).isValid()) {
        allCLCT[id.endcap() - 1][id.station() - 1][id.ring() - 1]
               [id.chamber() - FirstCh] = true;
      }
    }
  }
  ...
  for (CSCCorrelatedLCTDigiCollection::DigiRangeIterator j =
    ...
    const CSCDetId &id = (*j).first;
    std::vector<CSCCorrelatedLCTDigi>::const_iterator digiIt =
        (*j).second.first;
    std::vector<CSCCorrelatedLCTDigi>::const_iterator last = (*j).second.second;
    for (; digiIt != last; ++digiIt) {
      // Valid digi in the chamber (or in neighbouring chamber)
      if ((*digiIt).isValid()) {
        allCorrLCT[id.endcap() - 1][id.station() - 1][id.ring() - 1]
                  [id.chamber() - FirstCh] = true;
      }
    }
  }
...
void CSCEfficiency::fillWG_info(edm::Handle<CSCWireDigiCollection> &wires,
  ...
  for (CSCWireDigiCollection::DigiRangeIterator j = wires->begin();
    ...
    for (; digiItr != last; ++digiItr) {
      std::pair<int, float> WG_pos(
          digiItr->getWireGroup(),
          layerGeom->yOfWireGroup(digiItr->getWireGroup()));
      std::pair<std::pair<int, float>, int> LayerSignal(WG_pos,
                                                        digiItr->getTimeBin());

      //---- AllWG contains basic information about WG (WG number and
      // Y-position, time bin)
      allWG[id.endcap() - 1][id.station() - 1][id.ring() - 1]
           [id.chamber() - FirstCh][id.layer() - 1]
               .push_back(LayerSignal);
      if (printalot) {
        // std::cout<<" WG check : "<<std::endl;
        // printf("\t\tendcap/station/ring/chamber/layer:
...
void CSCEfficiency::fillStrips_info(
    edm::Handle<CSCStripDigiCollection> &strips) {
  //---- STRIPS
  for (CSCStripDigiCollection::DigiRangeIterator j = strips->begin();
    ...
    for (; digiItr != last; ++digiItr) {
      ...
      if (largestADCValue > maxADC) { // FIX IT!!!
        ...
        //---- (strip number and peak signal for most significant strip in the
        // layer)
        allStrips[id.endcap() - 1][id.station() - 1][id.ring() - 1]
                 [id.chamber() - 1][id.layer() - 1]
                     .clear();
        allStrips[id.endcap() - 1][id.station() - 1][id.ring() - 1]
                 [id.chamber() - 1][id.layer() - 1]
                     .push_back(LayerSignal);
...
void CSCEfficiency::fillSimhit_info(
    edm::Handle<edm::PSimHitContainer> &simhits) {
  //---- SIMHITS
  ...
  for (dSHsimIter = simhits->begin(); dSHsimIter != simhits->end();
    ...
    std::pair<LocalPoint, int> simHitPos((*dSHsimIter).localPosition(),
                                         (*dSHsimIter).particleType());
    allSimhits[sId.endcap() - 1][sId.station() - 1][sId.ring() - 1]
              [sId.chamber() - FirstCh][sId.layer() - 1]
                  .push_back(simHitPos);
  }
}
//
void CSCEfficiency::fillRechitsSegments_info(
    edm::Handle<CSCRecHit2DCollection> &rechits,
    edm::Handle<CSCSegmentCollection> &segments,
    edm::ESHandle<CSCGeometry> &cscGeom) {
  ...
  for (recIt = rechits->begin(); recIt != rechits->end(); recIt++) {
    ...
    if (printalot) {
      ...
      printf("\t\tendcap/station/ring/chamber/layer: %i/%i/%i/%i/%i\n",
             id.endcap(), id.station(), id.ring(), id.chamber(), id.layer());
      printf("\t\tx,y,z: %f, %f, %f\texx,eey,exy: %f, %f, %f\tglobal x,y,z: "
             "%f, %f, %f \n",
             rhitlocal.x(), rhitlocal.y(), rhitlocal.z(), rerrlocal.xx(),
             rerrlocal.yy(), rerrlocal.xy(), rhitglobal.x(), rhitglobal.y(),
             rhitglobal.z());
    }
    std::pair<LocalPoint, bool> recHitPos((*recIt).localPosition(), false);
    allRechits[id.endcap() - 1][id.station() - 1][id.ring() - 1]
              [id.chamber() - FirstCh][id.layer() - 1]
                  .push_back(recHitPos);
  }
  //---- "Empty" chambers
  ...
  for (CSCSegmentCollection::const_iterator it = segments->begin();
       it != segments->end(); it++) {
    CSCDetId id = (CSCDetId)(*it).cscDetId();
    StHist[id.endcap() - 1][id.station() - 1].segmentChi2_ndf->Fill(
        (*it).chi2() / (*it).degreesOfFreedom());
    StHist[id.endcap() - 1][id.station() - 1].hitsInSegment->Fill(
        (*it).nRecHits());
    if (printalot) {
      printf("\tendcap/station/ring/chamber: %i %i %i %i\n", id.endcap(),
             id.station(), id.ring(), id.chamber());
      std::cout << "\tposition(loc) = " << (*it).localPosition()
                << " error(loc) = " << (*it).localPositionError() << std::endl;
      std::cout << "\t chi2/ndf = " << (*it).chi2() / (*it).degreesOfFreedom()
                << " nhits = " << (*it).nRecHits() << std::endl;
    }
    allSegments[id.endcap() - 1][id.station() - 1][id.ring() - 1][id.chamber() -
                                                                  FirstCh]
        .push_back(make_pair((*it).localPosition(), (*it).localDirection()));
    ...
    for (vector<CSCRecHit2D>::const_iterator iRH = theseRecHits.begin();
      ...
      if (printalot) {
        printf("\t%i RH\tendcap/station/ring/chamber/layer: %i/%i/%i/%i/%i\n",
               layerRH, idRH.endcap(), idRH.station(), idRH.ring(),
               idRH.chamber(), idRH.layer());
      }
      for (size_t jRH = 0;
           jRH <
           allRechits[idRH.endcap() - 1][idRH.station() - 1][idRH.ring() - 1]
                     [idRH.chamber() - FirstCh][idRH.layer() - 1]
                         .size();
           ++jRH) {
        float xDiff =
            iRH->localPosition().x() -
        ...
        if (fabs(xDiff) < 0.0001 && fabs(yDiff) < 0.0001) {
          std::pair<LocalPoint, bool> recHitPos(
              allRechits[idRH.endcap() - 1][idRH.station() - 1][idRH.ring() - 1]
                        [idRH.chamber() - FirstCh][idRH.layer() - 1][jRH]
                            .first,
              true);
          allRechits[idRH.endcap() - 1][idRH.station() - 1][idRH.ring() - 1]
                    [idRH.chamber() - FirstCh][idRH.layer() - 1][jRH] =
                        recHitPos;
          if (printalot) {
            std::cout << " number of the rechit (from zero) in the segment = "
                      << jRH << std::endl;
          }
        }
      }
...
void CSCEfficiency::ringCandidates(int station, float feta,
                                   std::map<std::string, bool> &chamberTypes) {
...
//
void CSCEfficiency::chamberCandidates(int station, int ring, float phi,
                                      std::vector<int> &coupleOfChambers) {
  ...
  if (printalot)
    std::cout << " phi = " << phi << " phi_zero = " << phi_zero
              << " phi_const = " << phi_const
...
bool CSCEfficiency::efficienciesPerChamber(CSCDetId &id,
  ...
  if (secondRing > -1) {
    secondCondition =
        !allSegments[ec][st][secondRing][ch].empty() ? true : false;
  }
  ...
  if (useDigis) {
    ...
    if (firstCondition || secondCondition) {
      ...
      if (fabs(dxdz) < local_DX_DZ_Max) {
        StHist[ec][st].EfficientALCT_momTheta->Fill(
            ftsChamber.momentum().theta());
      ...
      if (fabs(dxdz) < local_DX_DZ_Max) {
        StHist[ec][st].InefficientALCT_momTheta->Fill(
            ftsChamber.momentum().theta());
    ...
    if (firstCondition || secondCondition) {
      ...
      if (dydz < local_DY_DZ_Max && dydz > local_DY_DZ_Min) {
        StHist[ec][st].EfficientCLCT_momPhi->Fill(
            ftsChamber.momentum().phi()); // - phi chamber...
      ...
      if (dydz < local_DY_DZ_Max && dydz > local_DY_DZ_Min) {
        StHist[ec][st].InefficientCLCT_momPhi->Fill(
            ftsChamber.momentum().phi()); // - phi chamber...
...
//
bool CSCEfficiency::stripWire_Efficiencies(CSCDetId &id,
                                           FreeTrajectoryState &ftsChamber) {
  int ec, st, rg, ch, secondRing;
  returnTypes(id, ec, st, rg, ch, secondRing);
  ...
  for (int iLayer = 0; iLayer < 6; iLayer++) {
    ...
    if (printalot) {
      printf(
          ...
          iLayer + 1, id.endcap(), id.station(), id.ring(), id.chamber(),
          iLayer + 1);
      std::cout << " size S = "
                << allStrips[id.endcap() - 1][id.station() - 1][id.ring() - 1]
                            [id.chamber() - FirstCh][iLayer]
                                .size()
                << "size W = "
                << allWG[id.endcap() - 1][id.station() - 1][id.ring() - 1]
                        [id.chamber() - FirstCh][iLayer]
    ...
    if (secondRing > -1) {
      secondCondition =
          !allStrips[ec][st][secondRing][ch][iLayer].empty() ? true : false;
    }
    if (firstCondition || secondCondition) {
    ...
    if (secondRing > -1) {
      secondCondition =
          !allWG[ec][st][secondRing][ch][iLayer].empty() ? true : false;
    }
    if (firstCondition || secondCondition) {
  ...
  if (missingLayers_s != missingLayers_wg) {
    ...
    if (6 == missingLayers_wg) {
      ...
      ChHist[ec][st][rg][ch].NoWires_momTheta->Fill(
          ftsChamber.momentum().theta());
    }
    if (6 == missingLayers_s) {
      ChHist[ec][st][rg][ch].StripWiresCorrelations->Fill(4);
      ChHist[ec][st][rg][ch].NoStrips_momPhi->Fill(
          ftsChamber.momentum().theta());
    }
...
bool CSCEfficiency::recSimHitEfficiency(CSCDetId &id,
  ...
  for (int iLayer = 0; iLayer < 6; iLayer++) {
    ...
    if (secondRing > -1) {
      secondCondition =
          !allSimhits[ec][st][secondRing][ch][iLayer].empty() ? true : false;
      if (secondCondition) {
        thisRing = secondRing;
      }
    }
    if (firstCondition || secondCondition) {
      for (size_t iSH = 0;
           iSH < allSimhits[ec][st][thisRing][ch][iLayer].size(); ++iSH) {
...
//
bool CSCEfficiency::recHitSegment_Efficiencies(
    CSCDetId &id, const CSCChamber *cscChamber,
    FreeTrajectoryState &ftsChamber) {
  int ec, st, rg, ch, secondRing;
  returnTypes(id, ec, st, rg, ch, secondRing);
  bool firstCondition, secondCondition;
  ...
  for (int iLayer = 0; iLayer < 6; ++iLayer) {
    ...
    if (secondRing > -1) {
      secondCondition =
          !allRechits[ec][st][secondRing][ch][iLayer].empty() ? true : false;
      if (secondCondition) {
        thisRing = secondRing;
      }
    }
    if (firstCondition || secondCondition) {
      ...
      for (size_t iR = 0; iR < allRechits[ec][st][thisRing][ch][iLayer].size();
           ++iR) {
  ...
  if (firstCondition || secondCondition) {
    if (printalot)
      std::cout << "segments - start ec = " << ec << " st = " << st
                << " rg = " << rg << " ch = " << ch << std::endl;
    StHist[ec][st].EfficientSegments_XY->Fill(ftsChamber.position().x(),
                                              ftsChamber.position().y());
    if (1 == allSegments[ec][st][rg][ch].size() + secondSize) {
      globalDir =
          cscChamber->toGlobal(allSegments[ec][st][thisRing][ch][0].second);
      globalPos =
          cscChamber->toGlobal(allSegments[ec][st][thisRing][ch][0].first);
      StHist[ec][st].EfficientSegments_eta->Fill(
          fabs(ftsChamber.position().eta()));
      ...
      if (printalot)
        std::cout << " fts.position() = " << ftsChamber.position()
                  << " segPos = " << globalPos << " res = " << residual
                  << std::endl;
    ...
    for (int iLayer = 0; iLayer < 6; ++iLayer) {
      ...
      firstCondition =
          !allRechits[ec][st][rg][ch][iLayer].empty() ? true : false;
      secondCondition = false;
      if (secondRing > -1) {
        secondCondition =
            !allRechits[ec][st][secondRing][ch][iLayer].empty() ? true : false;
      ...
      if (1 == allSegments[ec][st][rg][ch].size() + secondSize) {
        ...
        const LocalPoint lp_extrapol =
            cscChamber->layer(iLayer + 1)->toLocal(gp_extrapol);
        posXY.at(0) = lp_extrapol.x();
        posXY.at(1) = lp_extrapol.y();
        int nearestStrip = cscChamber->layer(iLayer + 1)
                               ->geometry()
                               ->nearestStrip(lp_extrapol);
        stripAngle = cscChamber->layer(iLayer + 1)
                         ->geometry()
                         ->stripAngle(nearestStrip) -
                     M_PI / 2.;
      }
      if (firstCondition || secondCondition) {
        ChHist[ec][st][rg][ch].EfficientRechits_inSegment->Fill(iLayer + 1);
        if (oneSegment) {
          ChHist[ec][st][rg][ch].Y_EfficientRecHits_inSegment[iLayer]->Fill(
              posXY.at(1));
          ChHist[ec][st][rg][ch].Phi_EfficientRecHits_inSegment[iLayer]->Fill(
              stripAngle);
        }
      } else {
        if (oneSegment) {
          ChHist[ec][st][rg][ch].Y_InefficientRecHits_inSegment[iLayer]->Fill(
              posXY.at(1));
          ChHist[ec][st][rg][ch].Phi_InefficientRecHits_inSegment[iLayer]->Fill(
              stripAngle);
        }
      }
    }
  } else {
    StHist[ec][st].InefficientSegments_XY->Fill(ftsChamber.position().x(),
                                                ftsChamber.position().y());
    if (printalot) {
  ...
  ChHist[ec][st][rg][id.chamber() - FirstCh].EfficientRechits_inSegment->Fill(
      9);

  return true;
}
//
void CSCEfficiency::returnTypes(CSCDetId &id, int &ec, int &st, int &rg,
                                int &ch, int &secondRing) {
  ec = id.endcap() - 1;
  st = id.station() - 1;
  rg = id.ring() - 1;
  secondRing = -1;
...
FreeTrajectoryState CSCEfficiency::getFromCLHEP(const CLHEP::Hep3Vector &p3,
  ...
  return cov.kRows == 6 ? FreeTrajectoryState(tPars, tCov)
                        : FreeTrajectoryState(tPars);
}
...
void CSCEfficiency::linearExtrapolation(GlobalPoint initialPosition,
  ...
  double paramLine =
      lineParameter(initialPosition.z(), zSurface, initialDirection.z());
  double xPosition =
      extrapolate1D(initialPosition.x(), initialDirection.x(), paramLine);
  double yPosition =
      extrapolate1D(initialPosition.y(), initialDirection.y(), paramLine);
  posZY.clear();
  posZY.push_back(xPosition);
  posZY.push_back(yPosition);
}
//
double CSCEfficiency::extrapolate1D(double initPosition, double initDirection,
...
//
double CSCEfficiency::lineParameter(double initZPosition, double destZPosition,
...
TrajectoryStateOnSurface CSCEfficiency::propagate(FreeTrajectoryState &ftsStart,
                                                  const BoundPlane &bpDest) {
  TrajectoryStateOnSurface tSOSDest;
  std::string propagatorName;
  /*
  // it would work if cosmic muons had properly assigned direction...
    bool dzPositive = bpDest.position().z() - ftsStart.position().z() > 0 ? true
  : false;
   //---- Be careful with trigger conditions too
    if(!isIPdata){
...
bool CSCEfficiency::applyTrigger(edm::Handle<edm::TriggerResults> &hltR,
  ...
  if (pointToTriggers.size() != myTriggers.size()) {
    pointToTriggers.clear();
    if (printalot) {
      std::cout << " Not all trigger names found - all trigger specifications "
                   "will be ignored. Check your cfg file!"
                << std::endl;
    }
  } else {
    if (!pointToTriggers.empty()) {
      if (printalot) {
        std::cout << "The following triggers will be required in the event: "
                  << std::endl;
        for (size_t imyT = 0; imyT < pointToTriggers.size(); ++imyT) {
          std::cout << "  " << hlNames[pointToTriggers[imyT]];
        }
        std::cout << std::endl;
        std::cout << " in condition (AND/OR) : " << !andOr << "/" << andOr
                  << std::endl;
      }
    }
  }
  ...
  if (hltR.isValid()) {
    if (pointToTriggers.empty()) {
      if (printalot) {
        std::cout << " No triggers specified in the configuration or all "
                     "ignored - no trigger information will be considered"
                  << std::endl;
      }
    }
    for (size_t imyT = 0; imyT < pointToTriggers.size(); ++imyT) {
    ...
    if (printalot) {
      std::cout << " TriggerResults handle returns invalid state?! No trigger "
...
CSCEfficiency::CSCEfficiency(const edm::ParameterSet &pset) {
  ...
  printout_NEvents =
      pset.getUntrackedParameter<unsigned int>("printout_NEvents", 0);
  rootFileName =
      pset.getUntrackedParameter<string>("rootFileName", "cscHists.root");
  ...
  getAbsoluteEfficiency =
      pset.getUntrackedParameter<bool>("getAbsoluteEfficiency", true); //
  useDigis = pset.getUntrackedParameter<bool>("useDigis", true);       //
  distanceFromDeadZone =
      pset.getUntrackedParameter<double>("distanceFromDeadZone", 10.); //
  ...
  minTrackHits =
      pset.getUntrackedParameter<unsigned int>("minTrackHits", 10); //

  applyIPangleCuts =
      pset.getUntrackedParameter<bool>("applyIPangleCuts", false); //
  local_DY_DZ_Max =
      pset.getUntrackedParameter<double>("local_DY_DZ_Max", -0.1); //
  local_DY_DZ_Min =
      pset.getUntrackedParameter<double>("local_DY_DZ_Min", -0.8); //
  local_DX_DZ_Max =
      pset.getUntrackedParameter<double>("local_DX_DZ_Max", 0.2); //
  ...
  rh_token = consumes<CSCRecHit2DCollection>(
      pset.getParameter<edm::InputTag>("rechitTag"));
  se_token = consumes<CSCSegmentCollection>(
      pset.getParameter<edm::InputTag>("segmentTag"));
  tk_token = consumes<edm::View<reco::Track>>(
      pset.getParameter<edm::InputTag>("tracksTag"));
  sh_token = consumes<edm::PSimHitContainer>(
      pset.getParameter<edm::InputTag>("simHitTag"));

  edm::ParameterSet serviceParameters =
      pset.getParameter<edm::ParameterSet>("ServiceParameters");
  ...
  ht_token = consumes<edm::TriggerResults>(
      pset.getParameter<edm::InputTag>("HLTriggerResults"));
  ...
  TriggersFired = new TH1F(SpecName, "Triggers fired;trigger number;entries",
                           140, -0.5, 139.5);
  //
  int Chan = 50;
  float minChan = -0.5;
  ...
  ALCTPerEvent = new TH1F(SpecName, "ALCTs per event;N digis;entries", Chan,
                          minChan, maxChan);
  //
  sprintf(SpecName, "CLCTPerEvent");
  CLCTPerEvent = new TH1F(SpecName, "CLCTs per event;N digis;entries", Chan,
                          minChan, maxChan);
  //
  sprintf(SpecName, "recHitsPerEvent");
  recHitsPerEvent =
      new TH1F(SpecName, "RecHits per event;N digis;entries", 150, -0.5, 149.5);
  //
  sprintf(SpecName, "segmentsPerEvent");
  segmentsPerEvent = new TH1F(SpecName, "segments per event;N digis;entries",
  ...
  for (int ec = 0; ec < 2; ++ec) {
    for (int st = 0; st < 4; ++st) {
      ...
      StHist[ec][st].segmentChi2_ndf = new TH1F(
          SpecName, "Chi2/ndf of a segment;chi2/ndf;entries", 100, 0., 20.);
      //
      sprintf(SpecName, "hitsInSegment_St%d", st + 1);
      StHist[ec][st].hitsInSegment = new TH1F(
          SpecName, "Number of hits in a segment;nHits;entries", 7, -0.5, 6.5);
      //
      Chan = 170;
      minChan = 0.85;
      maxChan = 2.55;
      ...
      StHist[ec][st].AllSegments_eta = new TH1F(
          SpecName, "All segments in eta;eta;entries", Chan, minChan, maxChan);
      //
      sprintf(SpecName, "EfficientSegments_eta_St%d", st + 1);
      StHist[ec][st].EfficientSegments_eta =
          new TH1F(SpecName, "Efficient segments in eta;eta;entries", Chan,
                   minChan, maxChan);
      //
      sprintf(SpecName, "ResidualSegments_St%d", st + 1);
      StHist[ec][st].ResidualSegments = new TH1F(
          SpecName, "Residual (segments);residual,cm;entries", 75, 0., 15.);
      //
      Chan = 200;
      minChan = -800.;
      maxChan = 800.;
      ...
      StHist[ec][st].EfficientSegments_XY =
          new TH2F(SpecName, "Efficient segments in XY;X;Y", Chan, minChan,
                   maxChan, Chan2, minChan2, maxChan2);
      sprintf(SpecName, "InefficientSegments_XY_St%d", st + 1);
      StHist[ec][st].InefficientSegments_XY =
          new TH2F(SpecName, "Inefficient segments in XY;X;Y", Chan, minChan,
                   maxChan, Chan2, minChan2, maxChan2);
      //
      Chan = 80;
      minChan = 0;
      ...
      StHist[ec][st].EfficientALCT_momTheta = new TH1F(
          SpecName, "Efficient ALCT in theta (momentum);theta, rad;entries",
          Chan, minChan, maxChan);
      //
      sprintf(SpecName, "InefficientALCT_momTheta_St%d", st + 1);
      StHist[ec][st].InefficientALCT_momTheta = new TH1F(
          SpecName, "Inefficient ALCT in theta (momentum);theta, rad;entries",
          Chan, minChan, maxChan);
      //
      Chan = 160;
      minChan = -3.2;
      ...
      StHist[ec][st].EfficientCLCT_momPhi = new TH1F(
          SpecName, "Efficient CLCT in phi (momentum);phi, rad;entries", Chan,
          minChan, maxChan);
      //
      sprintf(SpecName, "InefficientCLCT_momPhi_St%d", st + 1);
      StHist[ec][st].InefficientCLCT_momPhi = new TH1F(
          SpecName, "Inefficient CLCT in phi (momentum);phi, rad;entries", Chan,
          minChan, maxChan);
      //
      theFile->cd();
      for (int rg = 0; rg < 3; ++rg) {
        ...
        for (int iChamber = FirstCh; iChamber < FirstCh + NumCh; iChamber++) {
          ...
          theFile->cd();
          sprintf(SpecName, "Chambers__E%d_S%d_R%d_Chamber_%d", ec + 1, st + 1,
                  rg + 1, iChamber);
          theFile->mkdir(SpecName);
          theFile->cd(SpecName);
          //
          ...
          ChHist[ec][st][rg][iChamber - FirstCh].EfficientALCT_dydz = new TH1F(
              SpecName,
              "Efficient ALCT; local dy/dz (ME 3 and 4 flipped);entries", Chan,
              minChan, maxChan);
          //
          ...
          ChHist[ec][st][rg][iChamber - FirstCh].EfficientCLCT_dxdz =
              new TH1F(SpecName, "Efficient CLCT; local dxdz;entries", Chan,
                       minChan, maxChan);
          //
          sprintf(SpecName, "InefficientCLCT_dxdz_Ch%d", iChamber);
          ChHist[ec][st][rg][iChamber - FirstCh].InefficientCLCT_dxdz =
              new TH1F(SpecName, "Inefficient CLCT; local dxdz;entries", Chan,
                       minChan, maxChan);
          //
          ...
          ChHist[ec][st][rg][iChamber - FirstCh].NoWires_momTheta =
              new TH1F(SpecName,
                       "No wires (all strips present) - in theta "
                       "(momentum);theta, rad;entries",
                       Chan, minChan, maxChan);
          //
          Chan = 160;
          minChan = -3.2;
          ...
          ChHist[ec][st][rg][iChamber - FirstCh].NoStrips_momPhi =
              new TH1F(SpecName,
                       "No strips (all wires present) - in phi (momentum);phi, "
                       "rad;entries",
                       Chan, minChan, maxChan);
          //
          for (int iLayer = 0; iLayer < 6; iLayer++) {
            sprintf(SpecName, "Y_InefficientRecHits_inSegment_Ch%d_L%d",
                    iChamber, iLayer);
            ChHist[ec][st][rg][iChamber - FirstCh]
                .Y_InefficientRecHits_inSegment.push_back(
                    new TH1F(SpecName,
                             "Missing RecHit/layer in a segment (local system, "
                             "whole chamber);Y, cm; entries",
                             nYbins, Ymin, Ymax));
            //
            sprintf(SpecName, "Y_EfficientRecHits_inSegment_Ch%d_L%d", iChamber,
                    iLayer);
            ChHist[ec][st][rg][iChamber - FirstCh]
                .Y_EfficientRecHits_inSegment.push_back(new TH1F(
                    SpecName,
                    "Efficient (extrapolated from the segment) RecHit/layer in "
                    "a segment (local system, whole chamber);Y, cm; entries",
                    nYbins, Ymin, Ymax));
            //
            Chan = 200;
            minChan = -0.2;
            maxChan = 0.2;
            sprintf(SpecName, "Phi_InefficientRecHits_inSegment_Ch%d_L%d",
                    iChamber, iLayer);
            ChHist[ec][st][rg][iChamber - FirstCh]
                .Phi_InefficientRecHits_inSegment.push_back(
                    new TH1F(SpecName,
                             "Missing RecHit/layer in a segment (local system, "
                             "whole chamber);Phi, rad; entries",
                             Chan, minChan, maxChan));
            //
            sprintf(SpecName, "Phi_EfficientRecHits_inSegment_Ch%d_L%d",
                    iChamber, iLayer);
            ChHist[ec][st][rg][iChamber - FirstCh]
                .Phi_EfficientRecHits_inSegment.push_back(new TH1F(
                    SpecName,
                    "Efficient (extrapolated from the segment) in a segment "
                    "(local system, whole chamber);Phi, rad; entries",
                    Chan, minChan, maxChan));
          }
          //
          sprintf(SpecName, "Sim_Rechits_Ch%d", iChamber);
          ChHist[ec][st][rg][iChamber - FirstCh].SimRechits =
              new TH1F(SpecName, "Existing RecHit (Sim);layers (1-6);entries",
                       nLayer_bins, Layer_min, Layer_max);
          //
          sprintf(SpecName, "Sim_Simhits_Ch%d", iChamber);
          ChHist[ec][st][rg][iChamber - FirstCh].SimSimhits =
              new TH1F(SpecName, "Existing SimHit (Sim);layers (1-6);entries",
                       nLayer_bins, Layer_min, Layer_max);
          //
          /*
          sprintf(SpecName,"Sim_Rechits_each_Ch%d",iChamber);
          ChHist[ec][st][rg][iChamber-FirstCh].SimRechits_each =
            new TH1F(SpecName,"Existing RecHit (Sim), each;layers
          (1-6);entries",nLayer_bins,Layer_min,Layer_max);
          //
          sprintf(SpecName,"Sim_Simhits_each_Ch%d",iChamber);
          ChHist[ec][st][rg][iChamber-FirstCh].SimSimhits_each =
            new TH1F(SpecName,"Existing SimHit (Sim), each;layers
          (1-6);entries",nLayer_bins,Layer_min,Layer_max);
          */
          theFile->cd();
        }
    ...
    for (int st = 0; st < 4; ++st) {
      ...
      for (int rg = 0; rg < 3; ++rg) {
        ...
        for (int iChamber = FirstCh; iChamber < FirstCh + NumCh; iChamber++) {
          ...
          snprintf(SpecName, sizeof(SpecName),
                   "Chambers__E%d_S%d_R%d_Chamber_%d", ec + 1, st + 1, rg + 1,
                   iChamber);
          theFile->cd(SpecName);
          ...
          for (unsigned int iLayer = 0; iLayer < 6; iLayer++) {
            ChHist[ec][st][rg][iChamber - FirstCh]
                ...
                ->Write();
            ChHist[ec][st][rg][iChamber - FirstCh]
                .Y_EfficientRecHits_inSegment[iLayer]
                ->Write();
            ChHist[ec][st][rg][iChamber - FirstCh]
                .Phi_InefficientRecHits_inSegment[iLayer]
                ->Write();
            ChHist[ec][st][rg][iChamber - FirstCh]
                .Phi_EfficientRecHits_inSegment[iLayer]
                ->Write();
          }
