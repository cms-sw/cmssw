...
private:
  ...
  std::vector<std::pair<int, float>>
      allStrips[2][4][4][NumCh][6]; // endcap/station/ring/chamber/layer

  //----Wires: WG number and Y-position, time bin
  std::vector<std::pair<std::pair<int, float>, int>>
      allWG[2][4][4][NumCh][6]; // endcap/station/ring/chamber/layer

  // SetOfSimHits  (*all_SimHits)[2][4][4][ NumCh];
  //---- Simhits
  std::vector<std::pair<LocalPoint, int>>
      allSimhits[2][4][4][NumCh][6]; // endcap/station/ring/chamber/layer

  // rechits
  // SetOfRecHits  (*all_RecHits)[2][4][4][ NumCh];
  std::vector<std::pair<LocalPoint, bool>>
      allRechits[2][4][4][NumCh][6]; // endcap/station/ring/chamber/layer

  // segments
  std::vector<std::pair<LocalPoint, LocalVector>>
      allSegments[2][4][4][NumCh]; // endcap/station/ring/chamber
  ...
  void
  fillLCT_info(edm::Handle<CSCALCTDigiCollection> &alcts,
               edm::Handle<CSCCLCTDigiCollection> &clcts,
               edm::Handle<CSCCorrelatedLCTDigiCollection> &correlatedlcts);
  void fillWG_info(edm::Handle<CSCWireDigiCollection> &wires,
                   edm::ESHandle<CSCGeometry> &cscGeom);
  ...
  void ringCandidates(int station, float absEta,
                      std::map<std::string, bool> &chamberTypes);
  void chamberCandidates(int station, int ring, float phi,
                         std::vector<int> &coupleOfChambers);

  bool efficienciesPerChamber(CSCDetId &id, const CSCChamber *cscChamber,
                              FreeTrajectoryState &ftsChamber);
  bool stripWire_Efficiencies(CSCDetId &cscDetId,
                              FreeTrajectoryState &ftsChamber);
  bool recHitSegment_Efficiencies(CSCDetId &cscDetId,
                                  const CSCChamber *cscChamber,
                                  FreeTrajectoryState &ftsChamber);
  bool recSimHitEfficiency(CSCDetId &id, FreeTrajectoryState &ftsChamber);
  //
  void returnTypes(CSCDetId &id, int &ec, int &st, int &rg, int &ch,
                   int &secondRing);
  //
  void getFromFTS(const FreeTrajectoryState &fts, CLHEP::Hep3Vector &p3,
                  CLHEP::Hep3Vector &r3, int &charge,
                  AlgebraicSymMatrix66 &cov);

  FreeTrajectoryState getFromCLHEP(const CLHEP::Hep3Vector &p3,
                                   const CLHEP::Hep3Vector &r3, int charge,
                                   const AlgebraicSymMatrix66 &cov,
                                   const MagneticField *field);

  void linearExtrapolation(GlobalPoint initialPosition,
                           GlobalVector initialDirection, float zSurface,
                           std::vector<float> &posZY);
  double extrapolate1D(double initPosition, double initDirection,
                       double parameterOfTheLine);
  double lineParameter(double initZPosition, double destZPosition,
                       double initZDirection);
  bool inSensitiveLocalRegion(double xLocal, double yLocal, int station,
                              int ring);
  bool checkLocal(double yLocal, double yBoundary, int station, int ring);
  void chooseDirection(CLHEP::Hep3Vector &innerPosition,
                       CLHEP::Hep3Vector &outerPosition);
  const Propagator *propagator(std::string propagatorName) const;
  TrajectoryStateOnSurface propagate(FreeTrajectoryState &ftsStart,
                                     const BoundPlane &bp);
