...
int main() {
  ...
  for (int ieta=1; ieta<5; ieta++) {
    ...
    hf.push_back(HFRecHit(HcalDetId(HcalOuter,ieta+31,iphi,1),ieta*29,4));
  }

  CaloRecHitMetaCollectionFast f;

  for (HBHERecHitCollection::iterator i=hbhe1.begin(); i!=hbhe1.end(); i++) 
    f.add(&(*i));

  for (HBHERecHitCollection::iterator i=hbhe2.begin(); i!=hbhe2.end(); i++) 
    f.add(&(*i));

  for (HORecHitCollection::iterator i=ho.begin(); i!=ho.end(); i++) 
    f.add(&(*i));

  for (HFRecHitCollection::iterator i=hf.begin(); i!=hf.end(); i++) 
    f.add(&(*i));


  for (CaloRecHitMetaCollectionFast::const_iterator j=f.begin(); j!=f.end(); j++) {
    std::cout << (*j) << std::endl;
  }
