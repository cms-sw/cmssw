...
public:
  ...
  void inline selectCallback(const GlobalPoint &p,
    ...
    // TODO: heuristics of when it is better to loop over inputCollection
    // instead (small # hits)
    for (int subdet = subdet_;
         subdet <= 7 && (subdet_ == 0 || subdet_ == subdet); subdet++) {
      const CaloSubdetectorGeometry *sdg =
          geom_->getSubdetectorGeometry(detector_, subdet);
      if (sdg != nullptr) {
        // get the list of detids within range (from geometry)
        CaloSubdetectorGeometry::DetIdSet dis = sdg->getCells(p, deltaR_);
        // loop over detids...
        typename edm::SortedCollection<T>::const_iterator j,
            je = inputCollection.end();

        for (CaloSubdetectorGeometry::DetIdSet::iterator i = dis.begin();
             i != dis.end(); i++) {
          if (i->subdetId() != subdet)
            continue; // possible for HCAL where the same geometry object
                      // handles all the detectors
