...
void TestCaloSelectors::analyze(const edm::Event &evt,
  ...
  double maxEt = -1;
  GlobalPoint pMax;
  for (HBHERecHitCollection::const_iterator i = mhbhe.begin(); i != mhbhe.end();
       i++) {
  ...
  std::cout << "Center at " << pMax.eta() << "," << pMax.phi()
            << " (ET=" << maxEt << ")" << std::endl;
