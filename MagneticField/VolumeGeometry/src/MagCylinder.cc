...
MagCylinder::MagCylinder(const PositionType &pos, const RotationType &rot,
                         const std::vector<VolumeSide> &faces,
                         const MagneticFieldProvider<float> *mfp)
    : MagVolume(pos, rot, mfp), theFaces(faces), theZmin(0.), theZmax(0.),
      theInnerR(0.), theOuterR(0.) {
  using SurfaceOrientation::GlobalFace;

  unsigned int def = 0;
  for (std::vector<VolumeSide>::const_iterator i = faces.begin();
       i != faces.end(); ++i) {
    if (i->globalFace() == SurfaceOrientation::zminus) {
      ...
      ++def;
    } else if (i->globalFace() == SurfaceOrientation::outer ||
               i->globalFace() == SurfaceOrientation::inner) {
      const Cylinder *cyl = dynamic_cast<const Cylinder *>(&(i->surface()));
      if (cyl == nullptr) {
        throw MagGeometryError(
...
bool MagCylinder::inside(const LocalPoint &lp, double tolerance) const {
  Scalar r(lp.perp());
  return lp.z() > theZmin - tolerance && lp.z() < theZmax + tolerance &&
         r > theInnerR - tolerance && r < theOuterR + tolerance;
}
