...
MagBLayer::MagBLayer(vector<MagBSector *> &sectors, double rMin)
    : theSectors(sectors), theSingleVolume(nullptr), theRMin(rMin),
      theBinFinder(nullptr) {
  // TOFIX
  //   if (verbose.debugOut) cout << "Building MagBLayer with " <<
  //   theSectors.size()
  // 		  << " sectors, minR " << theRMin << endl;
  // FIXME: PeriodicBinFinderInPhi gets *center* of first bin
  theBinFinder = new PeriodicBinFinderInPhi<float>(
      theSectors.front()->minPhi() + Geom::pi() / 12., 12);
}

/// Constructor for a trivial layer consisting of one single volume.
MagBLayer::MagBLayer(MagVolume *aVolume, double rMin)
    : theSingleVolume(nullptr), theRMin(rMin), theBinFinder(nullptr) {
  // TOFIX
...
const MagVolume *MagBLayer::findVolume(const GlobalPoint &gp,
  ...
  // TOFIX
  if (verbose::debugOut)
    cout << "***In guessed bsector" << (result == nullptr ? " failed " : " OK ")
         << endl;
  ...
  if (result == nullptr) { // If fails, can be in previous bin.
    ...
    result =
        theSectors[theBinFinder->binIndex(bin - 1)]->findVolume(gp, tolerance);
    // TOFIX
    if (verbose::debugOut)
