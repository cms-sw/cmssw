...
void VectorFieldInterpolation::defineCellPoint000(double X1, double X2,
                                                  double X3, double F1,
                                                  double F2, double F3) {
  CellPoint000[0] = X1;
  CellPoint000[1] = X2;
...
void VectorFieldInterpolation::defineCellPoint100(double X1, double X2,
                                                  double X3, double F1,
                                                  double F2, double F3) {
  CellPoint100[0] = X1;
  CellPoint100[1] = X2;
...
void VectorFieldInterpolation::defineCellPoint010(double X1, double X2,
                                                  double X3, double F1,
                                                  double F2, double F3) {
  CellPoint010[0] = X1;
  CellPoint010[1] = X2;
...
void VectorFieldInterpolation::defineCellPoint110(double X1, double X2,
                                                  double X3, double F1,
                                                  double F2, double F3) {
  CellPoint110[0] = X1;
  CellPoint110[1] = X2;
...
void VectorFieldInterpolation::defineCellPoint001(double X1, double X2,
                                                  double X3, double F1,
                                                  double F2, double F3) {
  CellPoint001[0] = X1;
  CellPoint001[1] = X2;
...
void VectorFieldInterpolation::defineCellPoint101(double X1, double X2,
                                                  double X3, double F1,
                                                  double F2, double F3) {
  CellPoint101[0] = X1;
  CellPoint101[1] = X2;
...
void VectorFieldInterpolation::defineCellPoint011(double X1, double X2,
                                                  double X3, double F1,
                                                  double F2, double F3) {
  CellPoint011[0] = X1;
  CellPoint011[1] = X2;
...
void VectorFieldInterpolation::defineCellPoint111(double X1, double X2,
                                                  double X3, double F1,
                                                  double F2, double F3) {
  CellPoint111[0] = X1;
  CellPoint111[1] = X2;
...
void VectorFieldInterpolation::putSCoordGetVField(double X1, double X2,
                                                  double X3, double &F1,
                                                  double &F2, double &F3) {
  SC[0] = X1;
  SC[1] = X2;
  ...
  for (int i = 0; i < 5; ++i) {
    ...
    HelpPoint00[i] =
        CellPoint000[ip] +
        DeltaSC0X000overDeltaX100X000 * (CellPoint100[ip] - CellPoint000[ip]);

    // interpolate between CellPoint010 and CellPoint110
    HelpPoint10[i] =
        CellPoint010[ip] +
        DeltaSC0X010overDeltaX110X010 * (CellPoint110[ip] - CellPoint010[ip]);

    // interpolate between CellPoint001 and CellPoint101
    HelpPoint01[i] =
        CellPoint001[ip] +
        DeltaSC0X001overDeltaX101X001 * (CellPoint101[ip] - CellPoint001[ip]);

    // interpolate between CellPoint011 and CellPoint111
    HelpPoint11[i] =
        CellPoint011[ip] +
        DeltaSC0X011overDeltaX111X011 * (CellPoint111[ip] - CellPoint011[ip]);
  }
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    HelpPoint0[i] = HelpPoint00[ip] + DeltaSC1X00overDeltaX10X00 *
                                          (HelpPoint10[ip] - HelpPoint00[ip]);

    // interpolate between HelpPoint01 and HelpPoint11
    HelpPoint1[i] = HelpPoint01[ip] + DeltaSC1X01overDeltaX11X01 *
                                          (HelpPoint11[ip] - HelpPoint01[ip]);
  }
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    VF[i] = HelpPoint0[ip] +
            DeltaSC2X0overDeltaX1X0 * (HelpPoint1[ip] - HelpPoint0[ip]);
  }
