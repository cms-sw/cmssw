...
TrapezoidalCartesianMFGrid::TrapezoidalCartesianMFGrid(
  ...
  // In version 1103l the reference sector is at phi=0 so that local y is along
  // global X. The increasing grid steps for the same volume are then along Y
  // instead than along X. To use Trapezoid2RectangleMappingX to map the
  // trapezoidal geometry to a rectangular cartesian geometry, we have to
  // exchange (global) X and Y appropriately when constructing it.
  int nx, ny;
  ...
  if (!easya && easyb && easyc) {
    ...
    throw MagGeometryError("TrapezoidalCartesianMFGrid only implemented for "
  ...
  if (fabs(baMinus1) > 0.000001) {
    ...
    // FIXME ASSUMPTION: here we assume that the local reference frame is
    // oriented with X along the direction of where the grid is not uniform.
    // This is the case for all current geometries
  ...
  if (increasingAlongX) {
    ...
    // The reason why gridY and gridX have to be exchanged is because
    // Grid3D::index(i,j,k) assumes a specific order for the fieldValues, and we
    // cannot rearrange this vector. Given that we exchange grids, we will have
    // to exchange the outpouts of mapping_rectangle() and the inputs of
    // mapping_.trapezoid() in the following...
...
void TrapezoidalCartesianMFGrid::dump() const {
  ...
  // CAVEAT: if convertToLocal = true in the ctor, points have been converted to
  // LOCAL coordinates. To match those from .table files they have to be
  // converted back to global
...
MFGrid::LocalVector
TrapezoidalCartesianMFGrid::uncheckedValueInTesla(const LocalPoint &p) const {

  double xrec, yrec;
  mapping_.rectangle(p.x(), p.y(), xrec, yrec);

  // cout << "TrapezoidalCartesianMFGrid::valueInTesla at local point " << p <<
  // endl;
  //   cout << p.x() << " " << p.y()
...
void TrapezoidalCartesianMFGrid::toGridFrame(const LocalPoint &p, double &a,
...
MFGrid::LocalPoint TrapezoidalCartesianMFGrid::fromGridFrame(double a, double b,
                                                             double c) const {
  double xtrap, ytrap;
  if (increasingAlongX) {
