...
RectangularCylindricalMFGrid::RectangularCylindricalMFGrid(
    binary_ifstream &inFile, const GloballyPositioned<float> &vol)
    : MFGrid3D(vol) {
  // The parameters read from the data files are given in global coordinates.
  // In version 85l, local frame has the same orientation of global frame for
  // the reference volume, i.e. the r.f. transformation is only a translation.
  // There is therefore no need to convert the field values to local
  // coordinates. Check this assumption:
...
#ifdef DEBUG_GRID
  cout << "Grid reference point in grid system: " << xref << "," << yref << ","
       << zref << endl;
  ...
  // Grid1D gridY( lrefp.phi(), lrefp.phi() + stepy*(n2-1), n2); // wrong: gives
  // zero
...
void RectangularCylindricalMFGrid::dump() const {
  ...
  cout << "Number of points from Grid1D " << grid_.grida().nodes() << " "
       << grid_.gridb().nodes() << " " << grid_.gridc().nodes() << endl;

  cout << "Reference Point from Grid1D " << grid_.grida().lower() << " "
       << grid_.gridb().lower() << " " << grid_.gridc().lower() << endl;

  cout << "Basic Distance from Grid1D " << grid_.grida().step() << " "
       << grid_.gridb().step() << " " << grid_.gridc().step() << endl;
...
MFGrid::LocalVector
RectangularCylindricalMFGrid::uncheckedValueInTesla(const LocalPoint &p) const {
  const float minimalSignificantR =
      1e-6; // [cm], points below this radius are treated as zero radius
  float R = p.perp();
...
void RectangularCylindricalMFGrid::toGridFrame(const LocalPoint &p, double &a,
