...
TrapezoidalCylindricalMFGrid::TrapezoidalCylindricalMFGrid(
  ...
  for (int i = 0; i < 3; ++i)
    for (int j = 0; j < 3; ++j) {
      cout << "BasicDistance1[" << i << "][" << j
           << "] = " << BasicDistance1[i][j] << "BasicDistance2[" << i << "]["
  ...
  double b = a + BasicDistance1[0][1] * (n2 - 1) * (n1 - 1) +
             BasicDistance1[0][2] * (n3 - 1) * (n1 - 1);
  //  double h = stepy * (n2-1);
  double h = stepz * (n3 - 1);
  double delta =
      -BasicDistance2[0][1] * (n2 - 1) - BasicDistance2[0][2] * (n3 - 1);
...
TrapezoidalCylindricalMFGrid::uncheckedValueInTesla(const LocalPoint &p) const {
  ...
  //   (*TimingReport::current())["MagneticFieldProvider::valueInTesla(TrapezoidalCylindricalMFGrid)"];
...
void TrapezoidalCylindricalMFGrid::toGridFrame(const LocalPoint &p, double &a,
