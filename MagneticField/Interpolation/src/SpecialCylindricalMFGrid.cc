...
SpecialCylindricalMFGrid::SpecialCylindricalMFGrid(
    binary_ifstream &inFile, const GloballyPositioned<float> &vol, int gridType)
    : MFGrid3D(vol) {
  if (gridType == 5) {
    sector1 = false;
  } else if (gridType == 6) {
  ...
  for (int iLine = 0; iLine < nLines; ++iLine) {
    inFile >> Bx >> By >> Bz;
    // This would be fine only if local r.f. has the axes oriented as the global
    // r.f. For this volume we know that the local and global r.f. have
    // different axis orientation, so we do not try to be clever.
    //    fieldValues.push_back(BVector(Bx,By,Bz));

    // Preserve double precision!
    Vector3DBase<double, LocalTag> lB =
        frame().toLocal(Vector3DBase<double, GlobalTag>(Bx, By, Bz));
...
MFGrid::LocalVector
SpecialCylindricalMFGrid::uncheckedValueInTesla(const LocalPoint &p) const {
  //   static TimingReport::Item & timer=
  //   (*TimingReport::current())["MagneticFieldProvider::valueInTesla(SpecialCylindricalMFGrid)"];
  //   TimeMe t(timer,false);

  LinearGridInterpolator3D interpol(grid_);
  double a, b, c;
  toGridFrame(p, a, b, c);
  // the following holds if B values was not converted to local coords -- see
  // ctor
...
MFGrid::LocalPoint SpecialCylindricalMFGrid::fromGridFrame(double a, double b,
                                                           double c) const {
...
void SpecialCylindricalMFGrid::toGridFrame(const LocalPoint &p, double &a,
