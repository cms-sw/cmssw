...
CylinderFromSectorMFGrid::CylinderFromSectorMFGrid(
    const GloballyPositioned<float> &vol, double phiMin, double phiMax,
    MFGrid *sectorGrid)
    : MFGrid(vol), thePhiMin(phiMin), thePhiMax(phiMax),
...
MFGrid::LocalVector
CylinderFromSectorMFGrid::valueInTesla(const LocalPoint &p) const {
  double phi = p.phi();
  if (phi < thePhiMax && phi > thePhiMin)
    ...
    MFGrid::LocalVector tmp =
        theSectorGrid->valueInTesla(LocalPoint(xrot, yrot, p.z()));

    // rotate field back to original sector
    return MFGrid::LocalVector(tmp.x() * c - tmp.y() * s,
                               tmp.x() * s + tmp.y() * c, tmp.z());
  }
}
...
void CylinderFromSectorMFGrid::toGridFrame(const LocalPoint &p, double &a,
                                           double &b, double &c) const {
  throwUp("Not implemented yet");
}

MFGrid::LocalPoint CylinderFromSectorMFGrid::fromGridFrame(double a, double b,
                                                           double c) const {
  throwUp("Not implemented yet");
  return LocalPoint();
}
...
MFGrid::LocalPoint CylinderFromSectorMFGrid::nodePosition(int i, int j,
                                                          int k) const {
  throwUp("Not implemented yet");
  return LocalPoint();
}

MFGrid::LocalVector CylinderFromSectorMFGrid::nodeValue(int i, int j,
                                                        int k) const {
  throwUp("Not implemented yet");
  return LocalVector();
}
