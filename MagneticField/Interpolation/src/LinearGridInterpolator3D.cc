...
LinearGridInterpolator3D::interpolate(Scalar a, Scalar b, Scalar c) {
  ...
  */
  ...
  if (InterpolationDebug::debug) {
    using std::cout;
    using std::endl;
    cout << "LinearGridInterpolator3D called with a,b,c " << a << "," << b
         << "," << c << endl;
    cout << " i,j,k = " << i << "," << j << "," << k << " s,t,u = " << s << ","
         << t << "," << u << endl;
    cout << "Node positions for " << i << "," << j << "," << k << " : "
         << grida.node(i) << "," << gridb.node(j) << "," << gridc.node(k)
         << endl;
    cout << "Node positions for " << i + 1 << "," << j + 1 << "," << k + 1
         << " : " << grida.node(i + 1) << "," << gridb.node(j + 1) << ","
         << gridc.node(k + 1) << endl;
    ...
    cout << "Grid(" << i + 1 << "," << j + 1 << "," << k
         << ") = " << grid(i + 1, j + 1, k) << " ";
    cout << "Grid(" << i + 1 << "," << j + 1 << "," << k + 1
         << ") = " << grid(i + 1, j + 1, k + 1) << endl;
    cout << "number of nodes: " << grida.nodes() << "," << gridb.nodes() << ","
         << gridc.nodes() << endl;

    //     cout << (1-s)*(1-t)*(1-u)*grid(i,  j,  k) << " " <<
    //     (1-s)*(1-t)*u*grid(i,  j,  k+1) << endl
    //       << (1-s)*   t *(1-u)*grid(i,  j+1,k) << " " << (1-s)*   t
    //       *u*grid(i,  j+1,k+1) << endl
    //       << s    *(1-t)*(1-u)*grid(i+1,j,  k) << " " <<  s
    //       *(1-t)*u*grid(i+1,j,  k+1) << endl
    //       << s    *   t *(1-u)*grid(i+1,j+1,k) << " " << s    *   t
    //       *u*grid(i+1,j+1,k+1) << endl;
  }
  ...
  __m128 resultSIMD =
      _mm_mul_ps(_mm_set1_ps((1.f - s) * (1.f - t) * u),
                 _mm_sub_ps(grid(ind + s3).v.vec, grid(ind).v.vec));
  resultSIMD = _mm_add_ps(
      resultSIMD,
      _mm_mul_ps(_mm_set1_ps((1.f - s) * t * u),
                 _mm_sub_ps(grid(ind + s2 + s3).v.vec, grid(ind + s2).v.vec)));
  resultSIMD = _mm_add_ps(
      resultSIMD,
      _mm_mul_ps(_mm_set1_ps(s * (1.f - t) * u),
                 _mm_sub_ps(grid(ind + s1 + s3).v.vec, grid(ind + s1).v.vec)));
  resultSIMD = _mm_add_ps(resultSIMD,
                          _mm_mul_ps(_mm_set1_ps(s * t * u),
                                     _mm_sub_ps(grid(ind + s1 + s2 + s3).v.vec,
                                                grid(ind + s1 + s2).v.vec)));
  resultSIMD =
      _mm_add_ps(resultSIMD,
                 _mm_mul_ps(_mm_set1_ps((1.f - s) * t),
                            _mm_sub_ps(grid(ind + s2).v.vec, grid(ind).v.vec)));
  resultSIMD = _mm_add_ps(
      resultSIMD,
      _mm_mul_ps(_mm_set1_ps(s * t),
                 _mm_sub_ps(grid(ind + s1 + s2).v.vec, grid(ind + s1).v.vec)));
  resultSIMD =
      _mm_add_ps(resultSIMD,
                 _mm_mul_ps(_mm_set1_ps(s),
                            _mm_sub_ps(grid(ind + s1).v.vec, grid(ind).v.vec)));
