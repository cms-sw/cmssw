// Magnetic Field Interpolation Test Program (formerly
// prepareFieldInterpolation.cpp) by droll (03/02/04) Updated N. Amapane 3/07
// -former name
...
int main(int argc, char **argv) {
  if (argc > 4) {
    ...
    cout << "Example:" << endl
         << " prepareFieldTable "
            "/afs/cern.ch/cms/OO/mag_field/version_85l_030919/v-xyz-217.table "
            "grid.217.bin"
         << endl;
    return 1;
  }
  ...
  MFG001.countTrueNumberOfPoints(
      filename1); // check, if file contains some points twice

  MFG001.fillFromFile(filename1); // determine grid structure (standard cases)
  int type = MFG001.gridType();   // grid type
  if (type == 0) {
    cout << "  standard grid sructure detection failed, retrying with special "
            "grid sructure"
         << endl;
  } else {
    if (MFG001.isReady())
  ...
  MFG002.fillFromFileSpecial(
      filename1);           // determine grid structure (special cases)
