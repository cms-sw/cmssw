...
void prepareMagneticFieldGrid::countTrueNumberOfPoints(
  ...
  if (file.good()) {
    while (getline(file, line)) {
      ...
      if (file) {
        ...
        // 	  cout << "Read: " <<  setprecision(12) << Bx << " " << tBx <<
        // endl;
        // 	}
  ...
  for (int iLine = 0; iLine < nLines; ++iLine) {
    ...
    for (int i = 0; i < nLines; ++i) {
      if (i < iLine) {
        ...
        XBVector.getV6(x1, x2, x3, Bx, By, Bz);
        double distPP =
            sqrt((pnt[0] - x1) * (pnt[0] - x1) + (pnt[1] - x2) * (pnt[1] - x2) +
                 (pnt[2] - x3) * (pnt[2] - x3));
        if (distPP < epsilonRadius) {
          isSinglePoint = false;
          cout << "same points(" << iLine << ") with dR = " << distPP << endl;
          cout << "             point 1: " << iLine << " " << pnt[0] << " "
               << pnt[1] << " " << pnt[2] << " Bz: " << tmpBz << endl;
          cout << "             point 2: " << i << " " << x1 << " " << x2 << " "
               << x3 << " Bz: " << Bz << endl;
        }
      }
    }
...
void prepareMagneticFieldGrid::fillFromFile(const std::string &name) {
  ...
  if ((name.substr(ibeg + 1, iend - ibeg - 1)) == "xyz") {
    ...
    cout
        << " Unrecognized input file name: valid formats are *-xyz-* or *-rpz-*"
        << endl;
    abort();
  }
  ...
  if (file.good()) {
      ...
      if (file) {
        if (rotateSector > 1) {
          if (RpzCoordinates) {
            x2 = x2 - phi;
            if (fabs(x2) > 0.78) {
              cout << "ERROR: Input coordinates do not belong to sector "
                   << rotateSector << " : " << fabs(x2) << endl;
              abort();
            }
          } else {
            double x = x1;
        ...
        //	cerr << fixed << x1 << " " <<  x2 << " " <<  x3 << " " <<  Bx <<
        //" " <<  By << " " <<  Bz << " " <<  perm << " " <<  poten << " " <<
        // endl;

        // Check that the table is homogeneous (ie does not mix perm=1 with
        // perm>>1)
        if (perm > 1.)
          ++n_iron;
        else
    ...
    if (n_air && n_iron)
      cout << "WARNING: table " << name << " contains " << n_air
           << " points with perm=1 and " << n_iron << " with perm>1" << endl;
  } else
    return;
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    // calculate difference between the two reference points minus the total
    // difference (=0)
    double totDiff =
        secondRefPoint[i] - (ReferencePoint[i] + offset + stepSize * nSteps[i]);
  ...
  if (KnownStructure) {
    ...
    if (PRINT) {
      ...
      cout << "  # of points:   N1 = " << NumberOfPoints[0]
           << "   N2 = " << NumberOfPoints[1] << "   N3 = " << NumberOfPoints[2]
           << endl;
      cout << "  ref. point :   X1 = " << ReferencePoint[0]
           << "   X2 = " << ReferencePoint[1] << "   X3 = " << ReferencePoint[2]
           << endl;
      cout << "  step parm0 :   A1 = " << BasicDistance0[0]
           << "   A2 = " << BasicDistance0[1] << "   A3 = " << BasicDistance0[2]
           << endl;
      cout << "  easy grid  :   E1 = " << EasyCoordinate[0]
           << "   E2 = " << EasyCoordinate[1] << "   E3 = " << EasyCoordinate[2]
           << endl;
  ...
  if (!KnownStructure) {
    ...
    if (nGoodInd < 3) {
      ...
      cout << "# good indices = " << nGoodInd << " (" << goodIndex[0]
           << goodIndex[1] << goodIndex[2] << ") " << endl;
    ...
    for (int i = 0; i < 3; ++i) {
      ...
      // calculate difference between the two reference points minus the total
      // difference (=0)
      double totDiff = secondRefPoint[i] -
                       (ReferencePoint[i] + offset + stepSize * nSteps[i]);
      if (std::abs(totDiff) < EPSILON * 10.)
        systematicGrid[i] = true;
      else
        systematicGrid[i] = false;
    ...
    if (KnownStructure) {
      ...
      if (PRINT) {
        ...
        if (KnownStructure)
          ...
          if (NumberOfPoints[0] * NumberOfPoints[1] * NumberOfPoints[2] !=
              nLines) {
            cout << endl;
...
void prepareMagneticFieldGrid::fillFromFileSpecial(const std::string &name) {
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    // difference (=0)
    double totDiff =
        secondRefPoint[i] - (ReferencePoint[i] + stepSize * nSteps[i]);
    if (std::abs(totDiff) < EPSILON * 10.)
      systematicGrid[i] = true;
    else
      systematicGrid[i] = false;
  ...
  if (!KnownStructure) {
    ...
    double totStepSize = RParAsFunOfPhi[0] + RParAsFunOfPhi[1] / sinPhi -
                         RParAsFunOfPhi[2] - RParAsFunOfPhi[3] / sinPhi;
    ...
    if (!systematicGrid[0])
      KnownStructure = false;
    totDiff =
        secondRefPoint[1] - (ReferencePoint[1] + BasicDistance0[1] * nSteps[1]);
    if (std::abs(totDiff) < EPSILON * 10.)
    ...
    if (!systematicGrid[1])
      KnownStructure = false;
    totDiff =
        secondRefPoint[2] - (ReferencePoint[2] + BasicDistance0[2] * nSteps[2]);
    if (std::abs(totDiff) < EPSILON * 10.)
    ...
    if (PRINT) {
      ...
      cout << "  r param.s. :   R1 = " << RParAsFunOfPhi[0]
           << " RHO1 = " << RParAsFunOfPhi[1] << "   R2 = " << RParAsFunOfPhi[2]
           << " RHO2 = " << RParAsFunOfPhi[3] << endl;
      cout << "  easy grid  :   E1 = " << EasyCoordinate[0]
           << "   E2 = " << EasyCoordinate[1] << "   E3 = " << EasyCoordinate[2]
           << endl;
      cout << "             :   I1 = " << goodIndex[0]
           << "   I2 = " << goodIndex[1] << "   I3 = " << goodIndex[2] << endl;
      cout << "  structure  :   S1 = " << systematicGrid[0]
           << "   S2 = " << systematicGrid[1]
           << "   S3 = " << systematicGrid[2];
      if (KnownStructure)
...
void prepareMagneticFieldGrid::validateAllPoints() {
    ...
    for (index[0] = 0; index[0] < NumberOfPoints[0]; ++index[0]) {
      for (index[1] = 0; index[1] < NumberOfPoints[1]; ++index[1]) {
        for (index[2] = 0; index[2] < NumberOfPoints[2]; ++index[2]) {
          ...
          putIndicesGetXAndB(index[0], index[1], index[2], x1, x2, x3, Bx, By,
                             Bz);

          // first check: calculate indices from coordinates and compare with
          // original indices
          double pnt[3] = {x1, x2, x3};
          int tmpIdx[3] = {999, 999, 999};
          putCoordGetIndices((x1 + EPSILON), (x2 + EPSILON), (x3 + EPSILON),
          ...
          // second check: calculate coordinate from indices and compare with
          // original values
          double tmpPnt[3] = {0.0, 0.0, 0.0};
          putIndCalcXReturnB(index[0], index[1], index[2], tmpPnt[0], tmpPnt[1],
                             tmpPnt[2], Bx, By, Bz);
          for (int i = 0; i < 3; ++i) {
          ...
          for (int i = 0; i < 3; ++i) {
            if (tmpIdx[i] != index[i]) {
              putCoordGetIndices((x1 - EPSILON), (x2 + EPSILON), (x3 + EPSILON),
                                 tmpIdx[0], tmpIdx[1], tmpIdx[2]);
              if (tmpIdx[i] != index[i]) {
                putCoordGetIndices((x1 + EPSILON), (x2 - EPSILON),
                                   (x3 + EPSILON), tmpIdx[0], tmpIdx[1],
                                   tmpIdx[2]);
                if (tmpIdx[i] != index[i]) {
                  putCoordGetIndices((x1 - EPSILON), (x2 - EPSILON),
                                     (x3 + EPSILON), tmpIdx[0], tmpIdx[1],
  ...
  if (numberOfErrors > 0)
    KnownStructure = false;
  if (PRINT)
    cout << "  grid validation of all points done  -->  # errors = "
         << numberOfErrors << endl;
...
void prepareMagneticFieldGrid::saveGridToFile(const std::string &outName) {
  ...
  if (GridType == 2) {
    ...
    outFile << ReferencePoint[0] << ReferencePoint[1] << ReferencePoint[2];
    outFile << BasicDistance0[0] << BasicDistance0[1] << BasicDistance0[2];
    outFile << BasicDistance1[0][0] << BasicDistance1[1][0]
            << BasicDistance1[2][0];
    outFile << BasicDistance1[0][1] << BasicDistance1[1][1]
            << BasicDistance1[2][1];
    outFile << BasicDistance1[0][2] << BasicDistance1[1][2]
            << BasicDistance1[2][2];
    outFile << BasicDistance2[0][0] << BasicDistance2[1][0]
            << BasicDistance2[2][0];
    outFile << BasicDistance2[0][1] << BasicDistance2[1][1]
            << BasicDistance2[2][1];
    outFile << BasicDistance2[0][2] << BasicDistance2[1][2]
            << BasicDistance2[2][2];
  ...
  if (GridType == 5 || GridType == 6) {
    ...
    outFile << ReferencePoint[0] << ReferencePoint[1] << ReferencePoint[2];
    outFile << BasicDistance0[0] << BasicDistance0[1] << BasicDistance0[2];
    outFile << RParAsFunOfPhi[0] << RParAsFunOfPhi[1] << RParAsFunOfPhi[2]
            << RParAsFunOfPhi[3];
  }
  ...
  for (int iLine = 0; iLine < nLines; ++iLine) {
    ...
    outFile << Bx << By << Bz;
    //    if (iLine<5) cout << setprecision(12) << Bx << " " << GridPoint.bx()
    //    << endl;
  }
...
void prepareMagneticFieldGrid::interpolateAtPoint(double X1, double X2,
                                                  double X3, double &Bx,
                                                  double &By, double &Bz) {
  ...
  if (KnownStructure) {
    ...
    putIndicesGetXAndB(index0[0], index0[1], index0[2], tmpX[0], tmpX[1],
                       tmpX[2], tmpB[0], tmpB[1], tmpB[2]);
    MagInterpol.defineCellPoint000(tmpX[0], tmpX[1], tmpX[2], tmpB[0], tmpB[1],
                                   tmpB[2]);
    putIndicesGetXAndB(index1[0], index0[1], index0[2], tmpX[0], tmpX[1],
                       tmpX[2], tmpB[0], tmpB[1], tmpB[2]);
    MagInterpol.defineCellPoint100(tmpX[0], tmpX[1], tmpX[2], tmpB[0], tmpB[1],
                                   tmpB[2]);
    putIndicesGetXAndB(index0[0], index1[1], index0[2], tmpX[0], tmpX[1],
                       tmpX[2], tmpB[0], tmpB[1], tmpB[2]);
    MagInterpol.defineCellPoint010(tmpX[0], tmpX[1], tmpX[2], tmpB[0], tmpB[1],
                                   tmpB[2]);
    putIndicesGetXAndB(index1[0], index1[1], index0[2], tmpX[0], tmpX[1],
                       tmpX[2], tmpB[0], tmpB[1], tmpB[2]);
    MagInterpol.defineCellPoint110(tmpX[0], tmpX[1], tmpX[2], tmpB[0], tmpB[1],
                                   tmpB[2]);
    putIndicesGetXAndB(index0[0], index0[1], index1[2], tmpX[0], tmpX[1],
                       tmpX[2], tmpB[0], tmpB[1], tmpB[2]);
    MagInterpol.defineCellPoint001(tmpX[0], tmpX[1], tmpX[2], tmpB[0], tmpB[1],
                                   tmpB[2]);
    putIndicesGetXAndB(index1[0], index0[1], index1[2], tmpX[0], tmpX[1],
                       tmpX[2], tmpB[0], tmpB[1], tmpB[2]);
    MagInterpol.defineCellPoint101(tmpX[0], tmpX[1], tmpX[2], tmpB[0], tmpB[1],
                                   tmpB[2]);
    putIndicesGetXAndB(index0[0], index1[1], index1[2], tmpX[0], tmpX[1],
                       tmpX[2], tmpB[0], tmpB[1], tmpB[2]);
    MagInterpol.defineCellPoint011(tmpX[0], tmpX[1], tmpX[2], tmpB[0], tmpB[1],
                                   tmpB[2]);
    putIndicesGetXAndB(index1[0], index1[1], index1[2], tmpX[0], tmpX[1],
                       tmpX[2], tmpB[0], tmpB[1], tmpB[2]);
    MagInterpol.defineCellPoint111(tmpX[0], tmpX[1], tmpX[2], tmpB[0], tmpB[1],
                                   tmpB[2]);
    // interpolate
...
void prepareMagneticFieldGrid::putCoordGetIndices(double X1, double X2,
                                                  double X3, int &Index1,
  ...
  if (GridType == 5 || GridType == 6) {
    ...
    double stepSize = RParAsFunOfPhi[0] + RParAsFunOfPhi[1] / sinPhi -
                      RParAsFunOfPhi[2] - RParAsFunOfPhi[3] / sinPhi;
...
void prepareMagneticFieldGrid::putIndicesGetXAndB(int Index1, int Index2,
                                                  int Index3, double &X1,
                                                  double &X2, double &X3,
                                                  double &Bx, double &By,
                                                  double &Bz) {
...
void prepareMagneticFieldGrid::putIndCalcXReturnB(int Index1, int Index2,
                                                  int Index3, double &X1,
                                                  double &X2, double &X3,
                                                  double &Bx, double &By,
                                                  double &Bz) {
...
int prepareMagneticFieldGrid::lineNumber(int Index1, int Index2, int Index3) {
  return Index1 * NumberOfPoints[1] * NumberOfPoints[2] +
         Index2 * NumberOfPoints[2] + Index3;
}

bool prepareMagneticFieldGrid::IndexedDoubleVector::
operator<(const IndexedDoubleVector &x) const {
...
void prepareMagneticFieldGrid::IndexedDoubleVector::putI3(int index1,
                                                          int index2,
                                                          int index3) {
  I3[0] = index1;
  I3[1] = index2;
...
void prepareMagneticFieldGrid::IndexedDoubleVector::getI3(int &index1,
                                                          int &index2,
                                                          int &index3) {
  index1 = I3[0];
  index2 = I3[1];
...
void prepareMagneticFieldGrid::IndexedDoubleVector::putV6(double X1, double X2,
                                                          double X3, double Bx,
                                                          double By,
                                                          double Bz) {
  V6[0] = X1;
...
void prepareMagneticFieldGrid::SixDPoint::putP6(double X1, double X2, double X3,
                                                double Bx, double By,
                                                double Bz) {
  P6[0] = X1;
  P6[1] = X2;
