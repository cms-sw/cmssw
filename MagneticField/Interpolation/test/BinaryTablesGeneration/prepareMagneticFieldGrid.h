...
private:
  ...
  public:
    ...
    // destructor
    ~IndexedDoubleVector() {}
    // less operator (defined for indices: 1st index = highest priority, 2nd
    // index = 2nd highest priority, ...)
  ...
  public:
    ...
    void putV6(double X1, double X2, double X3, double Bx, double By,
               double Bz);
    void getV6(double &X1, double &X2, double &X3, double &Bx, double &By,
               double &Bz);
  };
  class SixDPoint {
  public:
  ...
  public:
    // Accessors
    void putP6(double X1, double X2, double X3, double Bx, double By,
               double Bz);
    double x1();
    double x2();
    double x3();
  ...
  double RParAsFunOfPhi[4];    // R = f(phi) or const. (0,2: const. par. ; 1,3:
                               // const./sin(phi))
...
public:
  /// check, if number of lines corresponds to number of points in space (double
  /// counting test)
  void countTrueNumberOfPoints(const std::string &name) const;
  /// reads the corresponding ASCII file, detects the logic of the points, and
  /// saves them on a grid
  void fillFromFile(const std::string &name);
  /// sames as fillFromFile, but for special cases which are not covered by the
  /// standard algorithm
  void fillFromFileSpecial(const std::string &name);
  /// returns value of GridType (and eventually prints the type + short
  /// description)
  int gridType();
  /// possibility to check existing MagneticFieldGrid point by point (all
  /// points)
  ...
  /// interpolates the magnetic field at input coordinate point and returns
  /// field values
  void interpolateAtPoint(double X1, double X2, double X3, double &Bx,
                          double &By, double &Bz);

  // calculates indices from coordinates
  void putCoordGetIndices(double X1, double X2, double X3, int &Index1,
                          int &Index2, int &Index3);
  // takes indices and returns coordinates and magnetic field values
  void putIndicesGetXAndB(int Index1, int Index2, int Index3, double &X1,
                          double &X2, double &X3, double &Bx, double &By,
                          double &Bz);
  // takes indices, calculates coordinates, and returns coordinates + magnetic
  // fiels values (for testing purposes only)
  void putIndCalcXReturnB(int Index1, int Index2, int Index3, double &X1,
                          double &X2, double &X3, double &Bx, double &By,
                          double &Bz);
