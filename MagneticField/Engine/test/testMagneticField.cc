 ...
 *
 *  outputTable: generate txt file with values to be used for regression. Points
 * are generated according to InnerRadius, OuterRadius, HalfLength
 *
 *  inputTable: file with input values to be checked against, format depends on
 * inputFileType: xyz = cartesian coordinates in cm rpz_m = r, phi, Z in m xyz_m
 * = cartesian in m TOSCA = input test tables, searches for the corresponding
 * volume/sector determined from the file name and path. TOSCAFileList = file
 * with a list of TOSCA tables TOSCASecorComparison: compare each if the listed
 * TOSCA txt tables with those of the other sectors
 *
...
public:
  ...
  void go(GlobalPoint g) {
    std::cout << "At: " << g << " phi=" << g.phi()
              << " B= " << field->inTesla(g) << std::endl;
  }
  ...
  void analyze(const edm::Event &event, const edm::EventSetup &setup) override {
    ...
    //      go(GlobalPoint(Cylindrical2Cartesian<float>(89.,phi,145.892)),
    //      magfield.product());
    //   }
  }
  ...
  void parseTOSCATablePath(string filename, int &volNo, int &sector,
                           string &type);
  void fillFromTable(string inputFile, vector<GlobalPoint> &p,
                     vector<GlobalVector> &b, string type);
...
void testMagneticField::writeValidationTable(int npoints, string filename) {
  GlobalPointProvider p(InnerRadius, OuterRadius, -Geom::pi(), Geom::pi(),
...
void testMagneticField::validate(string filename, string type) {
  ...
  while (getline(file, line) && count < numberOfPoints) {
    ...
    if (gp.perp() < InnerRadius || gp.perp() > OuterRadius ||
        fabs(gp.z()) > HalfLength)
      continue;
    ...
    if ((newB - oldB).mag() > reso) {
      ...
      if (delta > maxdelta)
        maxdelta = delta;
      cout << " Discrepancy at: # " << count + 1 << " " << gp << " R "
           << gp.perp() << " Phi " << gp.phi() << " delta : " << newB - oldB
           << " " << delta << endl;
...
void testMagneticField::parseTOSCATablePath(string filename, int &volNo,
                                            int &sector, string &type) {
  // Determine volume number, type, and sector from filename, assumed to be
  // like: [path]/s01_1/v-xyz-1156.table
...
void testMagneticField::validateVsTOSCATable(string filename) {
  // The magic here is that we want to check against the result of the master
  // volume only as grid points on the border of volumes can end up in the
  // neighbor volume.
  ...
  if (vol == nullptr) {
    cout << "   ERROR: volume " << volNo << ":" << sector << "not found"
         << endl;
    return;
  }

  cout << "Validate interpolation vs TOSCATable: " << filename << " volume "
       << volNo << ":[" << sector << "], type " << type << endl;
  ...
  // 	cout << vol->toGlobal(interpolator->nodePosition(i,j,k)) << " " <<
  // vol->toGlobal(interpolator->nodeValue(i,j,k)) <<endl;
  //       }
  //     }
  //   }
  ...
  while (getline(file, line) && count < numberOfPoints) {
    ...
    //    if (gp.perp() < InnerRadius || gp.perp() > OuterRadius || fabs(gp.z())
    //    > HalfLength) continue;
    ...
    if (vol->inside(gp, 0.03)) {
      ...
      cout << "ERROR: grid point # " << count + 1 << " " << gp
           << " is not inside volume " << endl;
    }
  ...
  if (count == 0) {
    ...
    cout << endl
         << " testMagneticField::validateVsTOSCATable: tested " << count
         << " points " << fail << " failures; max delta = " << maxdelta << endl
         << endl;
  }
}
...
void testMagneticField::compareSectorTables(string file1) {
  ...
  // cout << "Comparing tables for all sectors for volume " << volNo << " with "
  // << file1 << endl;
  if (!list)
    cout << volNo;
  ...
  for (int sector2 = 1; sector2 <= 12; ++sector2) {
    ...
    string binTable =
        "/data/n/namapane/MagneticField/120812/grid_120812_3_8t_v7_large";
    binTable += ssec;
    binTable += "_";
    ...
    for (unsigned int i = 0; i < p1.size(); ++i) {
      ...
      GlobalPoint p12(cphi * p1[i].x() - sphi * p1[i].y(),
                      sphi * p1[i].x() + cphi * p1[i].y(), p1[i].z());
      float pd = (p12 - p2[i]).mag();
      if (pd > 0.005) {
        cout << "ERROR: " << p12 << " " << p2[i] << " " << (p12 - p2[i]).mag()
             << endl;
      }

      b12.push_back(GlobalVector(cphi * b1[i].x() - sphi * b1[i].y(),
                                 sphi * b1[i].x() + cphi * b1[i].y(),
                                 b1[i].z()));
    ...
    if (list)
      cout << volNo << " " << sector2 << " " << avgdelta << " " << maxdelta
           << " " << size << endl;
    else {
      cout << "   " << maxdelta;
      //      cout << "   " << avgdelta;
    ...
    //     cout << "MAX: " << volNo << " " << sector2 << " " <<  setprecision(3)
    //     << maxdelta << endl; cout << imaxdelta << " " << b2[imaxdelta] << " "
    //     << b12[imaxdelta] << " " << (b2[imaxdelta]-b12[imaxdelta]) << endl;
  }
  cout << endl;
}

void testMagneticField::fillFromTable(string inputFile, vector<GlobalPoint> &p,
                                      vector<GlobalVector> &b, string type) {
...
const MagVolume6Faces *testMagneticField::findVolume(GlobalPoint &gp) {
  const VolumeBasedMagneticField *vbffield =
      dynamic_cast<const VolumeBasedMagneticField *>(field);
  if (vbffield) {
...
const MagVolume6Faces *testMagneticField::findMasterVolume(int volume,
                                                           int sector) {
  const MagGeometry *vbffield =
      (dynamic_cast<const VolumeBasedMagneticField *>(field))->field;
