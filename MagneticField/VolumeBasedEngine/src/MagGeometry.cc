...
MagGeometry::MagGeometry(int geomVersion,
    ...
    : lastVolume(nullptr), theBLayers(tbl), theESectors(tes), theBVolumes(tbv),
      theEVolumes(tev), cacheLastVolume(true), geometryVersion(geomVersion) {
  ...
  theEndcapBinFinder = new PeriodicBinFinderInPhi<float>(
      theESectors.front()->minPhi() + Geom::pi() / nEBins, nEBins);
}
...
GlobalVector MagGeometry::fieldInTesla(const GlobalPoint &gp) const {
  ...
  if (edm::isNotFinite(gp.mag())) {
    LogWarning("InvalidInput")
        << "Input value invalid (not a number): " << gp << endl;

  } else {
    LogWarning("MagneticField")
        << "MagGeometry::fieldInTesla: failed to find volume for " << gp
        << endl;
  }
  return GlobalVector();
}
...
MagVolume const *MagGeometry::findVolume1(const GlobalPoint &gp,
  ...
  if (inBarrel(gp)) { // Barrel
    for (vector<MagVolume6Faces const *>::const_iterator v =
             theBVolumes.begin();
         v != theBVolumes.end(); ++v) {
...
MagVolume const *MagGeometry::findVolume(const GlobalPoint &gp,
  ...
  if (inBarrel(gp)) { // Barrel
    ...
    for (int bin1 = bin; bin1 >= max(0, bin - 3); --bin1) {
      if (verbose::debugOut)
        cout << "Trying layer at R " << theBLayers[bin1]->minR() << " " << R
             << endl;
      result = theBLayers[bin1]->findVolume(gp, tolerance);
      if (verbose::debugOut)
        cout << "***In blayer " << bin1 - bin << " "
    ...
    if (verbose::debugOut)
      cout << "Trying endcap sector at phi " << theESectors[bin]->minPhi()
           << " " << phi << endl;
    result = theESectors[bin]->findVolume(gp, tolerance);
    if (verbose::debugOut)
      cout << "***In guessed esector "
           << (result == nullptr ? " failed " : " OK ") << endl;
  }
...
bool MagGeometry::inBarrel(const GlobalPoint &gp) const {
  ...
  if (geometryVersion >= 120812) {
    return (Z < 350. || (R > 172.4 && Z < 633.29) ||
            (R > 308.735 && Z < 662.01));
  } else if (geometryVersion >= 90812) { // FIXME no longer supported
    return (Z < 350. || (R > 172.4 && Z < 633.89) ||
            (R > 308.755 && Z < 662.01));
