...
bool SiPixelVCal::putSlopeAndOffset(const uint32_t& pixid, float& slopeValue,
                                    float& offsetValue) {
  std::map<unsigned int, VCal>::const_iterator id = m_vcal.find(pixid);
  if (id != m_vcal.end()) {
    edm::LogError("SiPixelVCal")
...
SiPixelVCal::VCal SiPixelVCal::getSlopeAndOffset(const uint32_t& pixid) const {
  ...
  if (id != m_vcal.end())
    ...
    edm::LogError("SiPixelVCal")
        << "SiPixelVCal for pixid " << pixid << " is not stored" << std::endl;
  VCal vcal_default;
  return vcal_default;
}
...
float SiPixelVCal::getSlope(const uint32_t& pixid) const {
  ...
  if (id != m_vcal.end())
    ...
    edm::LogError("SiPixelVCal") << "SiPixelVCal slope for pixid " << pixid
...
float SiPixelVCal::getOffset(const uint32_t& pixid) const {
  ...
  if (id != m_vcal.end())
    ...
    edm::LogError("SiPixelVCal") << "SiPixelVCal offset for pixid " << pixid
                                 << " is not stored" << std::endl;
  return -60.;
}
