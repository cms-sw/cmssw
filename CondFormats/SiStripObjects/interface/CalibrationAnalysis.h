...
class CalibrationAnalysis : public CommissioningAnalysis {
 ...
 private:
  ...
  VVBool  isvalid_;
  VFloat  mean_amplitude_,mean_tail_,mean_riseTime_,mean_decayTime_, mean_turnOn_, mean_peakTime_, mean_undershoot_, mean_baseline_, mean_smearing_,mean_chi2_;
  VFloat  min_amplitude_,min_tail_,min_riseTime_,min_decayTime_, min_turnOn_, min_peakTime_, min_undershoot_, min_baseline_, min_smearing_,min_chi2_;
  VFloat  max_amplitude_,max_tail_,max_riseTime_,max_decayTime_, max_turnOn_, max_peakTime_, max_undershoot_, max_baseline_, max_smearing_,max_chi2_;
  VFloat  spread_amplitude_,spread_tail_,spread_riseTime_,spread_decayTime_, spread_turnOn_, spread_peakTime_, spread_undershoot_, spread_baseline_, spread_smearing_,spread_chi2_;
