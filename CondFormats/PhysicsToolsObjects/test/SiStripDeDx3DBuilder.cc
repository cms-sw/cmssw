...
void SiStripDeDx3DBuilder::analyze(const edm::Event& evt, const edm::EventSetup& iSetup){
  ...
  edm::LogInfo("SiStripDeDx3DBuilder") << "... creating dummy PhysicsToolsObjects::Calibration::HistogramD3D Data for Run " << run << "\n " << std::endl;

  //  PhysicsToolsObjects::Calibration::HistogramD2D* obj = new PhysicsTools::Calibration::HistogramD2D(300, 0., 3., 1000,0.,1000.);
  PhysicsTools::Calibration::HistogramD3D * obj = new PhysicsTools::Calibration::HistogramD3D(5, 0, 5, 100, 0., 3., 100,0.,1000.); 
...
//        edm::LogInfo("SiStripDeDx3DBuilder") << "X = " << ix << " Y = " << iy << " Z = " << iz << std::endl;
  ...
  if( mydbservice.isAvailable() ){
    if( mydbservice->isNewTagRequest("SiStripDeDxProton_3D_Rcd") ){
      mydbservice->createNewIOV<PhysicsTools::Calibration::HistogramD3D >(obj,mydbservice->beginOfTime(),mydbservice->endOfTime(),"SiStripDeDxProton_3D_Rcd");      
    } else {
      mydbservice->appendSinceTime< PhysicsTools::Calibration::HistogramD3D >(obj,mydbservice->currentTime(),"SiStripDeDxProton_3D_Rcd");      
    }
  }else{
