...
void SiStripDeDx2DBuilder::analyze(const edm::Event& evt, const edm::EventSetup& iSetup){
  ...
  edm::LogInfo("SiStripDeDx2DBuilder") << "... creating dummy PhysicsToolsObjects::Calibration::HistogramD2D Data for Run " << run << "\n " << std::endl;
...
//  }
  ...
  if( mydbservice.isAvailable() ){
    if( mydbservice->isNewTagRequest("SiStripDeDxProton_2D_Rcd") ){
      mydbservice->createNewIOV<PhysicsTools::Calibration::VHistogramD2D >(obj,mydbservice->beginOfTime(),mydbservice->endOfTime(),"SiStripDeDxProton_2D_Rcd");      
    } else {
      mydbservice->appendSinceTime< PhysicsTools::Calibration::VHistogramD2D >(obj,mydbservice->currentTime(),"SiStripDeDxProton_2D_Rcd");      
    }
  }else{
