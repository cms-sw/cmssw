...
void SiStripDeDxMipBuilder::analyze(const edm::Event& evt, const edm::EventSetup& iSetup){
  ...
  edm::LogInfo("SiStripDeDxMipBuilder") << "... creating dummy PhysicsToolsObjects::Calibration::HistogramD2D Data for Run " << run << "\n " << std::endl;
...
//  }
  ...
  if( mydbservice.isAvailable() ){
    if( mydbservice->isNewTagRequest("SiStripDeDxMipRcd") ){
      mydbservice->createNewIOV<PhysicsTools::Calibration::HistogramD2D>(obj,mydbservice->beginOfTime(),mydbservice->endOfTime(),"SiStripDeDxMipRcd");      
    } else {
      mydbservice->appendSinceTime<PhysicsTools::Calibration::HistogramD2D>(obj,mydbservice->currentTime(),"SiStripDeDxMipRcd");      
    }
  }else{
