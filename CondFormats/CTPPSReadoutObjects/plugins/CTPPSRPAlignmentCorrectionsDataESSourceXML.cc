...
CTPPSRPAlignmentCorrectionsDataSequence CTPPSRPAlignmentCorrectionsDataESSourceXML::Merge(const vector<CTPPSRPAlignmentCorrectionsDataSequence>& seqs) const
{
...
void CTPPSRPAlignmentCorrectionsDataESSourceXML::PrepareSequence(const string &label, CTPPSRPAlignmentCorrectionsDataSequence &seq, const vector<string> &files) const
{
  if (verbosity)
						   ...
						   const IOVSyncValue& iosv, ValidityInterval& valInt)
{
  ...
  if (strcmp(key.name(), "RPMisalignedAlignmentRecord") == 0)
  {
   ...
   if (p_corr == nullptr)
     throw cms::Exception("CTPPSRPAlignmentCorrectionsDataESSourceXML::setIntervalFor") << "Unknown record " << key.name();
