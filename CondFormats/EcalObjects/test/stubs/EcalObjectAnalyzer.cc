...
**/
  ...
  // old weight object EcalWeightRecAlgoWeights
    std::cout << "Fetch old object EcalWeightRecAlgoWeights from DB" << std::endl;
    edm::ESHandle<EcalWeightRecAlgoWeights> poldWgts;
    context.get<EcalWeightRecAlgoWeightsRcd>().get(poldWgts);
    const EcalWeightRecAlgoWeights* oldwgts = poldWgts.product();

    //typedef std::vector< std::vector<EcalWeight> > EcalWeightMatrix;
    EcalWeightMatrix& mat1 = oldwgts->getWeightsBeforeGainSwitch();
    EcalWeightMatrix& mat2 = oldwgts->getWeightsAfterGainSwitch();

    std::cout << "check the size of its data members: " << endl;
    std::cout << "WeightsBeforeGainSwitch.size(): " << mat1.size()
              << ", WeightsAfterGainSwitch.size(): " << mat2.size()
              << std::endl;
