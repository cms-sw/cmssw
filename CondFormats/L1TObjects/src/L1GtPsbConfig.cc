...
L1GtPsbConfig::L1GtPsbConfig(const int& psbSlot) : m_gtBoardSlot(psbSlot) {
  m_gtPsbCh0SendLvds = false;
  m_gtPsbCh1SendLvds = false;
...
// unequal operator
bool L1GtPsbConfig::operator!=(const L1GtPsbConfig& result) const { return !(result == *this); }
...
// set board slot
void L1GtPsbConfig::setGtBoardSlot(const int& gtBoardSlotValue) { m_gtBoardSlot = gtBoardSlotValue; }
...
void L1GtPsbConfig::print(std::ostream& myCout) const {
  myCout << "PSB Board slot " << m_gtBoardSlot << " ( 0x" << std::hex << m_gtBoardSlot << std::dec
         << " ):" << std::endl;

  myCout << "    CH0_SEND_LVDS_NOT_DS92LV16 = " << (m_gtPsbCh0SendLvds ? "True" : "False") << std::endl;
  myCout << "    CH1_SEND_LVDS_NOT_DS92LV16 = " << (m_gtPsbCh1SendLvds ? "True" : "False") << std::endl;
  myCout << std::endl;

  int iLvds = -1;
  for (std::_Bit_const_iterator::const_reference m_gtPsbEnableRecLvd : m_gtPsbEnableRecLvds) {
    iLvds++;
    myCout << "\n    Enable_Rec_LVDS [" << iLvds << "] = " << (m_gtPsbEnableRecLvd ? "True" : "False");
  }
  myCout << std::endl;

  int iCh = -1;
  for (std::_Bit_const_iterator::const_reference cIt : m_gtPsbEnableRecSerLink) {
    iCh++;
