...
void L1GtTriggerMenu::buildGtConditionMap() {
  ...
  chipNr = -1;
  for (auto itCondOnChip = m_vecJetCountsTemplate.begin(); itCondOnChip != m_vecJetCountsTemplate.end();
       itCondOnChip++) {
    chipNr++;
  ...
  chipNr = -1;
  for (auto itCondOnChip = m_vecCastorTemplate.begin(); itCondOnChip != m_vecCastorTemplate.end(); itCondOnChip++) {
    chipNr++;
  ...
  chipNr = -1;
  for (auto itCondOnChip = m_vecHfBitCountsTemplate.begin(); itCondOnChip != m_vecHfBitCountsTemplate.end();
       itCondOnChip++) {
    chipNr++;
  ...
  chipNr = -1;
  for (auto itCondOnChip = m_vecHfRingEtSumsTemplate.begin(); itCondOnChip != m_vecHfRingEtSumsTemplate.end();
       itCondOnChip++) {
    chipNr++;
  ...
  chipNr = -1;
  for (auto itCondOnChip = m_vecBptxTemplate.begin(); itCondOnChip != m_vecBptxTemplate.end(); itCondOnChip++) {
    chipNr++;
...
void L1GtTriggerMenu::print(std::ostream& myCout, int& printVerbosity) const {
  ...
  switch (printVerbosity) {
    case 0: {
      ...
      for (auto itBit = ttBitToTt.begin(); itBit != ttBitToTt.end(); itBit++) {
        int bitNumber = itBit->first;
        std::string aName = (itBit->second)->algoName();
        std::string aAlias = (itBit->second)->algoAlias();
