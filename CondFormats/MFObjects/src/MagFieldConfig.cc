...
MagFieldConfig::MagFieldConfig(const edm::ParameterSet &pset, bool debug) {
  ...
  if (!fileSpec.empty()) {
    for (VPSet::const_iterator rule = fileSpec.begin(); rule != fileSpec.end();
         ++rule) {
      string s_volumes = rule->getParameter<string>("volumes");
      string s_sectors =
          rule->getParameter<string>("sectors"); // 0 means all volumes
      ...
      if (debug) {
        ...
        copy(volumes.begin(), volumes.end(),
             ostream_iterator<unsigned>(cout, " "));
        cout << endl;
        cout << " Expanded sectors: ";
        copy(sectors.begin(), sectors.end(),
             ostream_iterator<unsigned>(cout, " "));
        cout << endl;
      }

      for (vector<unsigned>::iterator i = volumes.begin(); i != volumes.end();
           ++i) {
        for (vector<unsigned>::iterator j = sectors.begin(); j != sectors.end();
             ++j) {
          ...
          if (gridFiles.find(vpacked) == gridFiles.end()) {
            ...
            throw cms::Exception("ConfigurationError")
                << "VolumeBasedMagneticFieldESProducer: malformed gridFiles "
                   "config parameter"
                << endl;
          }
        }
  ...
  // Slave field label. Either a label of an existing map (legacy support), or
  // the type of parametrization to be constructed with the "paramData"
  // parameters.
