...
HBHEHFLogicalMapEntry::HBHEHFLogicalMapEntry(int in_fi_ch,
                                             ...
                                             int in_cr,
                                             int in_htr,
                                             const std::string& in_s_fpga,
                                             const std::string& in_s_det,
                                             int in_sid,
                                             int in_et,
                                             int in_ph,
                                             int in_dep,
...
char* HBHEHFLogicalMapEntry::printLMapLine() {
  ...
  (mysubdet == 1)
      ? mydet = "HB"
      : ((mysubdet == 2) ? mydet = "HE"
                         : ((mysubdet == 3) ? mydet = "HO" : ((mysubdet == 4) ? mydet = "HF" : mydet = "invalid")));

  (hcaleid.htrTopBottom() == 0) ? myfpga = "bot" : myfpga = "top";

  sprintf(myline,
          "%1d %6d %6d %6d %6d %6d %6s %7s %6d %6d %6d",
          0,
          hcaldid.zside(),
          hcaldid.ietaAbs(),
          hcaldid.iphi(),
          ...
          rbxid.rm(),
          rbxid.pixel());
  sprintf(myline + strlen(myline),
          "%6d %6d %6d %6d %6d %6d %6s",
          rbxid.qieCard(),
          rbxid.adc(),
          rbxid.rmFiber(),
          hcaleid.fiberChanId(),
          hcaleid.readoutVMECrateId(),
          hcaleid.htrSlot(),
          myfpga.c_str());
  sprintf(myline + strlen(myline),
          "%8d %7d %6d %6d %6d %6s",
    ...
    (mysubdet == HcalBarrel)
        ...
        : ((mysubdet == HcalEndcap)
               ...
               : ((mysubdet == HcalOuter) ? mydet = "HO"
                                          : ((mysubdet == HcalForward) ? mydet = "HF" : mydet = "invalid")));
...
ZDCLogicalMapEntry::ZDCLogicalMapEntry(int in_fi_ch,
  ...
  if (in_s_det == "ZDC_EM")
    myzdcsec = HcalZDCDetId::EM;
  else if (in_s_det == "ZDC_HAD")
    myzdcsec = HcalZDCDetId::HAD;
  else if (in_s_det == "ZDC_LUM")
    myzdcsec = HcalZDCDetId::LUM;
  else if (in_s_det == "ZDC_RPD")
    myzdcsec = HcalZDCDetId::RPD;
  else
    myzdcsec = HcalZDCDetId::Unknown;
...
char* HTLogicalMapEntry::printLMapLine() {
  ...
  sprintf(myline,
          ...
          mydet_.c_str(),
          mywedge_,
          hcaltrigeid.readoutVMECrateId());
  sprintf(myline + strlen(myline),
          "%4d %5s %7d %6d %4d %4d %6s %7s %5d ",
          hcaltrigeid.htrSlot(),
          myfpga.c_str(),
          mydcc_sl,
          hcaltrigeid.spigot(),
          mydcc,
          hcaltrigeid.slbSiteNumber(),
          myslbin_.c_str(),
          myslbin2_.c_str(),
          hcaltrigeid.slbChannelIndex());
  sprintf(myline + strlen(myline),
          "%13s %7d %7d %7d %17s %6d\n",
          myslnam_.c_str(),
          myrctcra_,
