...
HcalLinearCompositionFunctor::HcalLinearCompositionFunctor(
    boost::shared_ptr<AbsHcalFunctor> p,
    const double ia, const double ib)
    : other_(p), a_(ia), b_(ib)
{
    if (!other_.get()) throw cms::Exception(
        "In HcalLinearCompositionFunctor constructor: "
        "can not use a null pointer to another functor");
}

double HcalLinearCompositionFunctor::operator()(const double x) const
{
    return a_*(*other_)(x) + b_;
}
