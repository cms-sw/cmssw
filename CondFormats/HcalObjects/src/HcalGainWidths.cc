...
const float* HcalGainWidths::getValues (unsigned long fId) const {
  Item target;
  target.mId = fId;
...
float HcalGainWidths::getValue (unsigned long fId, int fCapId) const {
  const float* values;
...
bool HcalGainWidths::addValue (unsigned long fId, const float fValues [4]) {
  return addValue (fId, fValues [0], fValues [1], fValues [2], fValues [3]);
}

bool HcalGainWidths::addValue (unsigned long fId, float fValue1, float fValue2, float fValue3, float fValue4) {
  Item item;
  item.mId = fId;
...
std::vector<unsigned long> HcalGainWidths::getAllChannels () const {
