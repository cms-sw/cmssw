...
public:
    ...
    {
        if (index < coll_.size()) return coll_.get(index);
        else return default_.get();
    }

    // Convenience function for getting what we need by id.
    // This method can return nullptr.
    inline const Item* get(const HcalDetId& id) const
        {return getByIndex(getIndex(id));}
    ...
    inline const Item& at(const HcalDetId& id) const
    {
        ...
        if (ptr == nullptr) throw cms::Exception(
