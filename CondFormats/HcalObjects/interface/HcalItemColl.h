...
public:
    ...
    inline void push_back(std::unique_ptr<Item> ptr)
    ...
    {
        if (index < data_.size()) return data_[index].get();
        else return nullptr;
    }

    // The following function throws an exception if the index is out of range
    inline const Item& at(const unsigned index) const
    ...
    inline bool operator!=(const HcalItemColl& r) const
        {return !(*this == r);}
