...
public:
    ...
    boost::shared_ptr<AbsOOTPileupCorrection> get(
        const std::string& name, const std::string& category) const;

    bool exists(const std::string& name, const std::string& category) const;

    bool operator==(const OOTPileupCorrectionColl& r) const;

    inline bool operator!=(const OOTPileupCorrectionColl& r) const
        {return !(*this == r);}
