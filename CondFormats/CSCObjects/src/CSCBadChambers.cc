...
bool CSCBadChambers::isInBadChamber( const CSCDetId& id ) const {
  ...
  if ( skipBadChambers() || ( numberOfChambers() == 0 ) ) return false;
  ...
  std::vector<int>::const_iterator badbegin = chambers_.begin();
  std::vector<int>::const_iterator badend   = chambers_.end();
