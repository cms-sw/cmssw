...
CSCTriggerMappingFromFile::CSCTriggerMappingFromFile(std::string filename) : filename_(std::move(filename)) { fill(); }
...
void CSCTriggerMappingFromFile::fill(void) {
  ...
  if (!in) {
    std::cout << "CSCTriggerMappingFromFile: ERROR! Failed to open file containing mapping, " << filename_ << std::endl;
  } else {
    std::cout << "CSCTriggerMappingFromFile: opened file containing mapping, " << filename_ << std::endl;
