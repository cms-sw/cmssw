...
namespace edmtest
{
  ...
  void
   CSCReadChamberTimeCorrectionsValuesAnalyzer::analyze(const edm::Event& e, const edm::EventSetup& context)
  {
    ...
    int count=0;
    for( it=myChamberTimeCorrections->chamberCorrections.begin();it!=myChamberTimeCorrections->chamberCorrections.end(); ++it ){
      count=count+1;
