...
namespace edmtest
{
  ...
  void
   CSCReadBadWiresAnalyzer::analyze(const edm::Event& e, const edm::EventSetup& context)
  {
    ...
    for( itcham=theBadWires->chambers.begin();itcham!=theBadWires->chambers.end(); ++itcham ){    
      ...
      std::cout<<counter<<"  "<<itcham->chamber_index<<"  "<<itcham->pointer<<"  "<<itcham->bad_channels<<std::endl;
	...
	// Test whether this bad channel has indeed been flagged in the badWireWord
	  ...
	  std::cout << "count " << +ifailed << " failed to see bad channel " << chan << " in layer " << id2 << std::endl;
	}
      }
    }
    ...
    */
    ...
    if ( outputToFile ) {
      ...
      for( itcham=theBadWires->chambers.begin();itcham!=theBadWires->chambers.end(); ++itcham ){    
        counter++;
        BadWireFile<<counter<<"  "<<itcham->chamber_index<<"  "<<itcham->pointer<<"  "<<itcham->bad_channels<<std::endl;
      }
      counter = 0;
      for( itchan=theBadWires->channels.begin();itchan!=theBadWires->channels.end(); ++itchan ){    
        counter++;
        BadWireFile<<counter<<"  "<<itchan->layer<<"  "<<itchan->channel<<"  "<<itchan->flag1<<std::endl;
      }
    }
  ...
  void CSCReadBadWiresAnalyzer::fillBadWireWords() {
    ...
    for (auto chamber : theBadWires->chambers) { // loop over bad chambers
