...
//------------------------------------------------------------------------
/*
METCorrectorParameters::Definitions::Definitions(const std::vector<std::string>& fBinVar, const std::vector<std::string>& fParVar, const std::string& fFormula, bool fIsResponse)
{
...
METCorrectorParameters::Definitions::Definitions(const std::string& fLine)
{
  ...
  if (!tokens.empty())
    ...
    std::cout << " METCorrectorParameters::Definitions::Definitions 2 " << std::endl;
    // Added by Sasha (end)
}
*/
//------------------------------------------------------------------------
...
//------------------------------------------------------------------------
/*
METCorrectorParameters::Record::Record(const std::string& fLine,unsigned fNvar) : mMin(0),mMax(0)
{
  mNvar = fNvar;
  ...
  if (!tokens.empty())
    ...
    // Added by Sasha (begin)
    std::cout << " METCorrectorParameters::Record::Record " << std::endl;
    // Added by Sasha (end)
}
*/
//------------------------------------------------------------------------
  ...
  while (std::getline(input,line)) 
    {
      ...
      if (section.empty() && !tmp.empty()){
         std::cout << " METCorrectorParameters section " << section << std::endl;
         std::cout << " METCorrectorParameters tmp " << tmp << std::endl;
      } 
      // Added by Sasha (end)
      if (!section.empty() && tmp.empty()) 
        {
...
/*
const char * 
METCorrectorParametersCollection::labelsArray_[METCorrectorParametersCollection::N_LEVELS] = 
  {
    "L1Offset",
    "L2Relative",
    "L3Absolute",
...
const char *
METCorrectorParametersCollection::l5FlavorArray_[METCorrectorParametersCollection::N_L5_SPECIES] = 
  {
    "L5Flavor_bJ",
    "L5Flavor_cJ",
    "L5Flavor_qJ",
...
const char *
METCorrectorParametersCollection::l7PartonArray_[METCorrectorParametersCollection::N_L7_SPECIES] = 
  {
    "L7Parton_gJ",
    "L7Parton_qJ",
    "L7Parton_cJ",
...
METCorrectorParametersCollection::labels_(labelsArray_, 
					  labelsArray_ + sizeof(labelsArray_)/sizeof(*labelsArray_) );

std::vector<std::string>
METCorrectorParametersCollection::l5Flavors_(l5FlavorArray_, 
					     l5FlavorArray_ + sizeof(l5FlavorArray_)/sizeof(*l5FlavorArray_) );

std::vector<std::string>
METCorrectorParametersCollection::l7Partons_(l7PartonArray_, 
...
METCorrectorParametersCollection::getL5Bin( std::string const & flav ){
...
METCorrectorParametersCollection::key_type
METCorrectorParametersCollection::getL7Bin( std::string const & flav ){
  std::vector<std::string>::const_iterator found = 
...
bool METCorrectorParametersCollection::isL5( key_type k ) {
  return k == L5Flavor ||
    ( k / 100 > 0 && k / 1000 == 0 );
}
// Check if this is an L7 hashed value
bool METCorrectorParametersCollection::isL7( key_type k ) {
  return k == L7Parton ||
    ( k / 1000 > 0 );
}
...
METCorrectorParametersCollection::findKey( std::string const & label ) const {
  ...
  if ( found2 != l7Partons_.end() ) {Event Setup Recodrd CMS
