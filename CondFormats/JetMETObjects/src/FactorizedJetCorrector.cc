  ...
  for(unsigned int i=0;i<mLevels.size();i++)
    { 
      if (mLevels[i]=="L1" || mLevels[i]=="L2" || mLevels[i]=="L3" || mLevels[i]=="L4" || mLevels[i]=="L6")
        ...
        throw cms::Exception("CombinedJetCorrector")<<" asking L5Flavor correction without specifying flavor option";
      else if (mLevels[i]=="L5" && FlavorOption.length()>0)
        mCorrectors.push_back(new SimpleJetCorrector(DataFiles[i],FlavorOption));
      else if (mLevels[i]=="L7" && PartonOption.length()==0)
        throw cms::Exception("CombinedJetCorrector")<<" asking L7Parton correction without specifying parton option";
      else if (mLevels[i]=="L7" && PartonOption.length()>0)
        mCorrectors.push_back(new SimpleJetCorrector(DataFiles[i],PartonOption));
      else
        throw cms::Exception("CombinedJetCorrector")<<" unknown correction level: "<<mLevels[i];
    } 
}
//------------------------------------------------------------------------ 
//--- Consistency checker ------------------------------------------------
...
void FactorizedJetCorrector::checkConsistency(const std::vector<std::string>& fLevels, const std::vector<std::string>& fTags)
{
  ...
  if (fLevels.size() != fTags.size())
    throw cms::Exception("CombinedJetCorrector")<<" number of correction levels: "<<fLevels.size()<<" doesn't match the number of data file tags: "<<fTags.size();
  ...
  for(unsigned int i=0;i<fTags.size();i++)
    if ((int)fTags[i].find(fLevels[i])<0)
      throw cms::Exception("CombinedJetCorrector")<<" inconsistent tag: "<<fTags[i]<<" for the requested correction: "<<fLevels[i];
}
//------------------------------------------------------------------------ 
//--- String parser ------------------------------------------------------
//------------------------------------------------------------------------
  ...
  for(unsigned i=0;i<fNames.size();i++)
    {
      if (fNames[i] == "JetEta")
        {
          if (!mIsJetEtaset)
            throw cms::Exception("CombinedJetCorrector")<<" jet eta is not set";
          result.push_back(mJetEta);
        }
      else if (fNames[i] == "JetPt")
        {
          if (!mIsJetPtset)
            throw cms::Exception("CombinedJetCorrector")<<" jet pt is not set";  
          result.push_back(mJetPt);
        }
      else if (fNames[i] == "JetPhi")
        {
          if (!mIsJetPhiset)
            throw cms::Exception("CombinedJetCorrector")<<" jet phi is not set";  
          result.push_back(mJetPt);
        }
      else if (fNames[i] == "JetE")
        {
          if (!mIsJetEset)
            throw cms::Exception("CombinedJetCorrector")<<" jet energy is not set";
          result.push_back(mJetE);
        }
      else if (fNames[i] == "JetEMF")
        {
          if (!mIsJetEMFset)
            throw cms::Exception("CombinedJetCorrector")<<" jet emf is not set";
          result.push_back(mJetEMF);
        } 
      else if (fNames[i] == "LepPx")
        {
          if (!mIsLepPxset)
            throw cms::Exception("CombinedJetCorrector")<<" lepton px is not set";  
          result.push_back(mLepPx);
        }
      else if (fNames[i] == "LepPy")
        {
          if (!mIsLepPyset)
            throw cms::Exception("CombinedJetCorrector")<<" lepton py is not set";  
          result.push_back(mLepPy);
        }
      else if (fNames[i] == "LepPz")
        {
          if (!mIsLepPzset)
            throw cms::Exception("CombinedJetCorrector")<<" lepton pz is not set";  
          result.push_back(mLepPz);
        }
      else
        throw cms::Exception("CombinedJetCorrector")<<" unknown parameter "<<fNames[i];
    }     
  return result;      
}
//------------------------------------------------------------------------ 
...
float FactorizedJetCorrector::getPtRel()
{
  ...
  if (!(lj2 > 0))
    throw cms::Exception("CombinedJetCorrector")<<" not positive lepton-jet momentum: "<<lj2;
