...
namespace JME {
    ...
    JetResolutionObject::Definition::Definition(const std::string& definition) {
        ...
        if (tokens.size() < 3) {
            throw edm::Exception(edm::errors::ConfigFileReadError, "Definition line needs at least three tokens. Please check file format.");
        }

        size_t n_bins = std::stoul(tokens[0]);

        if (tokens.size() < (n_bins + 2)) {
            throw edm::Exception(edm::errors::ConfigFileReadError, "Invalid file format. Please check.");
        }
    ...
    JetResolutionObject::Record::Record(const std::string& line, const Definition& def) {
        ...
        if (tokens.size() < (def.nBins() * 2 + def.nVariables() * 2 + 1)) {
            throw edm::Exception(edm::errors::ConfigFileReadError, "Invalid record. Please check file format. Record: " + line);
        }
    ...
    JetResolutionObject::JetResolutionObject(const std::string& filename) {
        ...
        if (! f.good()) {
            throw edm::Exception(edm::errors::FileReadError, "Can't read input file '" + filename + "'");
        }
