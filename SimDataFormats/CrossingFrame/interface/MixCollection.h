...
public:
  ...
  const T &getObject(unsigned int ip) const {
    if (ip >= (unsigned int)size())
      throw cms::Exception("BadIndex") << "MixCollection::getObject called "
                                          "with an invalid index!"; // ip >= 0,
                                                                    // since ip
                                                                    // is
                                                                    // unsigned
    int n = ip;
    ...
    -      iframe=ii;
    -      int
    s=crossingFrames_[iframe]->getNrSignals()+crossingFrames_[iframe]->getNrPileups();
    -      if (n<s) break;
    */
    ...
    */
    ...
    throw cms::Exception("InternalError")
  ...
  public:
    ...
    MixItr(const MixCollection *shc, int nrDets)
        : mixCol_(shc), nrDets_(nrDets), first_(true), iSignal_(0), iPileup_(0),
          internalCtr_(0) {
      ;
    }
    ...
    bool operator!=(const MixItr &itr) {
      return internalCtr2_ != itr.internalCtr2_;
    }
  ...
  private:
    ...
    bool trigger_;
    unsigned int internalCtr_; // this is the internal counter pointing into the
                               // vector of piled up objects
    unsigned int internalCtr2_ =
        0; // this is the internal counter for the number of iterated elements,
           // needed for operator!=
...
MixCollection<T>::MixCollection(const CrossingFrame<T> *cf,
                                const std::pair<int, int> bunchRange)
  ...
  if (cf) {
    ...
    edm::LogWarning("MixCollectionInvalidCtr")
        << "Could not construct MixCollection for " << typeid(T).name()
        << ", pointer to CrossingFrame invalid!" << std::endl;
}

template <class T>
MixCollection<T>::MixCollection(
    const std::vector<const CrossingFrame<T> *> &cfs,
    const std::pair<int, int> bunchRange)
    : inRegistry_(false), nrDets_(0) {
...
void MixCollection<T>::init(const std::pair<int, int> bunchRange) {
  ...
  // in case of several crossingFrames, we have verified before that they have
  // the same bunchrange
  ...
  if (bunchRange_ == range(-999, 999))
    ...
    if (bunchRange_.first < defaultrange.first ||
        bunchRange_.second > defaultrange.second)
      throw cms::Exception("BadRunRange")
          << " You are asking for a runrange (" << bunchRange_.first << ","
          << bunchRange_.second << "), outside of the existing runrange ("
          << defaultrange.first << ", " << defaultrange.second << ")\n";
...
template <class T>
bool MixCollection<T>::MixItr::getNewSignal(
...
template <class T>
bool MixCollection<T>::MixItr::getNewPileups(
    typename std::vector<const T *>::const_iterator &first,
    typename std::vector<const T *>::const_iterator &last) {
  ...
  while (iPileup_ < nrDets_) {
    ...
    int s = 0;
    for (typename std::vector<const T *>::const_iterator it = first; it != last;
         it++) {
      s++;
    }
...
const typename MixCollection<T>::MixItr MixCollection<T>::MixItr::next() {
  ...
  if (ok) {
    // debug print start
    //    for (auto dbIt=pMixItr_;dbIt!=pMixItrEnd_;++dbIt)  printf("Found
    //    pointer %p\n",(*dbIt));fflush(stdout);
    // debug print end
    internalCtr_ = 0;
  }
...
std::ostream &operator<<(std::ostream &o, const MixCollection<T> &col) {
  o << "MixCollection with bunchRange: " << (col.bunchrange()).first << ","
    << (col.bunchrange()).second << " size of signal: " << col.sizeSignal()
    << " ,size of pileup: " << col.sizePileup();
