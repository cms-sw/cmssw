...
public:
  ...
  CrossingFrame()
      : firstCrossing_(0), lastCrossing_(0), bunchSpace_(75), subdet_(""),
        maxNbSources_(0) {}
  ...
  // we keep the shared pointer in the object that will be only destroyed at the
  // end of the event (transient object!) because of HepMCProduct, we need 2
  // versions...
  /*   void setPileupPtr(std::shared_ptr<edm::Wrapper<std::vector<T> > const>
   * shPtr) {shPtrPileups_=shPtr;} */
  ...
  void
  setPileupPtr(std::shared_ptr<edm::Wrapper<PCrossingFrame<T>> const> shPtr);
  ...
  unsigned int getNrPileups(int bcr) const {
    return bcr == lastCrossing_
               ? pileups_.size() -
                     pileupOffsetsBcr_[lastCrossing_ - firstCrossing_]
               : pileupOffsetsBcr_[bcr - firstCrossing_ + 1] -
                     pileupOffsetsBcr_[bcr - firstCrossing_];
  }
  ...
  // get object in pileup when position in the vector is known (for DigiSimLink
  // typically)
  ...
  const T &getObject(unsigned int ip) const {
    ...
    if (ip > getNrSignals() + getNrPileups())
      throw cms::Exception("BadIndex")
          << "CrossingFrame::getObject called with an invalid index- index was "
          << ip << "!"; // ip >=0, since ip is unsigned
    if (ip < getNrSignals()) {
      return *(signals_[ip]);
    } else {
  ...
  void setPileupOffsetsSource(
      const std::vector<std::vector<unsigned int>> &pOffsetsS) {
...
private:
  ...
  // for playback option
  edm::EventID idFirstPileup_; // EventId fof the first pileup event used for
                               // this signal event
  unsigned int
      pileupFileNr_; // ordinal number of the pileup file this event was in
  ...
  std::vector<std::shared_ptr<edm::Wrapper<std::vector<T>> const>>
      shPtrPileups_;
  std::vector<std::shared_ptr<edm::Wrapper<T> const>>
      shPtrPileups2_; // fore HepMCProduct
  /*   std::shared_ptr<edm::Wrapper<std::vector<T> > const> shPtrPileups_;  */
  /*   std::shared_ptr<edm::Wrapper<T> const> shPtrPileups2_;   // fore
   * HepMCProduct */
  std::shared_ptr<edm::Wrapper<PCrossingFrame<T>> const> shPtrPileupsPCF_;
  //  std::shared_ptr<edm::Wrapper<PCrossingFrame<edm::HepMCProduct> const>
  //  shPtrPileupsHepMCProductPCF_;
...
template <class T>
CrossingFrame<T>::CrossingFrame(int minb, int maxb, int bunchsp,
                                std::string subdet, unsigned int maxNbSources)
    : firstCrossing_(minb), lastCrossing_(maxb), bunchSpace_(bunchsp),
      subdet_(subdet), maxNbSources_(maxNbSources) {
...
template <typename T>
inline CrossingFrame<T> &CrossingFrame<T>::
operator=(CrossingFrame<T> const &rhs) {
...
template <class T>
void CrossingFrame<T>::getPileups(
...
template <class T> int CrossingFrame<T>::getSourceType(unsigned int ip) const {
  ...
  for (unsigned int i = 0; i < pileupOffsetsSource_.size() - 1; ++i) {
    if (ip >= (pileupOffsetsSource_[i])[bcr] &&
        ip < (pileupOffsetsSource_[i + 1])[bcr])
      return i;
  }
...
std::ostream &operator<<(std::ostream &o, const CrossingFrame<T> &cf) {
  ...
  o << "\nCrossingFrame for subdet " << cf.getEventID()
    << ",  bunchrange = " << range.first << "," << range.second
...
template <class T>
void CrossingFrame<T>::setPileupPtr(
    std::shared_ptr<edm::Wrapper<PCrossingFrame<T>> const> shPtr) {
  shPtrPileupsPCF_ = shPtr;
}

template <class T> void CrossingFrame<T>::addPileups(T const &product) {
...
template <class T> void CrossingFrame<T>::setTof() { ; }
