...
void RandomEngineStates::getRandomEngineStates(
    std::vector<RandomEngineState> &states) const {
  ...
  for (std::vector<std::string>::const_iterator label = moduleLabels_.begin(),
       ...
       label != labelEnd; ++label, ++seedLength, ++stateLength) {
    ...
    if (state != states.end() && *label == state->getLabel()) {
      if (*seedLength != state->getSeed().size() ||
        ...
        throw edm::Exception(edm::errors::Configuration)
            << "When attempting to replay processing with the "
               "RandomNumberGeneratorService,\n"
            << "the engine type for each module must be the same in the replay "
               "configuration\n"
            << "and the original configuration.  If this is not the problem, "
               "then the data\n"
            << "is somehow corrupted or there is a bug because the vector in "
               "the data containing\n"
            << "the seeds or engine state is the incorrect size for the type "
               "of random engine.\n";
      }
...
void RandomEngineStates::setRandomEngineStates(
    std::vector<RandomEngineState> const &states) {
...
bool RandomEngineStates::isProductEqual(
    RandomEngineStates const &randomEngineStates) const {
