...
public:
  ...
  GenHIEvent()
      : b_(-99), npart_(-99), ncoll_(-99), nhard_(-99), phi_(-99),
        eccentricity_(-99), nCharged_(-99), nChargedMR_(-99), meanPt_(-99),
        meanPtMR_(-99), EtMR_(-99), nChargedPtCut_(-99), nChargedPtCutMR_(-99) {
    subevents_.reserve(0);
    ;
  }

  GenHIEvent(double b, int npart, int ncoll, int nhard, double phi,
             double ecc = -99, double nCharged = -99, double nChargedMR = -99,
             double meanPt = -99, double meanPtMR = -99, double EtMR = -99,
             int nChargedPtCut = -99, int nChargedPtCutMR = -99)
      : b_(b), npart_(npart), ncoll_(ncoll), nhard_(nhard), phi_(phi),
        eccentricity_(ecc), nCharged_(nCharged), nChargedMR_(nChargedMR),
        meanPt_(meanPt), meanPtMR_(meanPtMR), EtMR_(EtMR),
        nChargedPtCut_(nChargedPtCut), nChargedPtCutMR_(nChargedPtCutMR) {
    subevents_.reserve(0);
    ;
  }
