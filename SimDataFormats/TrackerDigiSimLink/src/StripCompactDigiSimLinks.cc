...
StripCompactDigiSimLinks::Links StripCompactDigiSimLinks::getLinks(const StripCompactDigiSimLinks::key_type &key) const {
  auto last = trackRecords_.end();
  auto match = std::lower_bound(trackRecords_.begin(), last, key);
...
std::map<uint32_t, std::vector<StripCompactDigiSimLinks::RevLink> > StripCompactDigiSimLinks::makeReverseMap() const {
  ...
  auto hstart = hitRecords_.begin(), ith = hstart;
  for (auto itt = trackRecords_.begin(), endt = trackRecords_.end(); itt != endt; ++itt) {
    auto edh = (itt + 1 != endt ? hstart + (itt + 1)->start : hitRecords_.end());
    for (; ith < edh; ++ith) {
