...
StripCompactDigiSimLinks::Links StripCompactDigiSimLinks::getLinks(
    const StripCompactDigiSimLinks::key_type &key) const {
  std::vector<TrackRecord>::const_iterator last = trackRecords_.end();
  std::vector<TrackRecord>::const_iterator match =
      std::lower_bound(trackRecords_.begin(), last, key);
  if ((match != last) && (*match == key)) {
    // std::vector<TrackRecord>::const_iterator next = match+1;
    unsigned int end =
        (match + 1 == last ? hitRecords_.size() : (match + 1)->start);
...
StripCompactDigiSimLinks::StripCompactDigiSimLinks(
    const StripCompactDigiSimLinks::Filler &filler) {
  ...
  for (auto const &pair : filler.storage()) {
    ...
    hitRecords_.insert(hitRecords_.end(), pair.second.begin(),
                       pair.second.end());
  }
}
...
void StripCompactDigiSimLinks::Filler::insert(
    const StripCompactDigiSimLinks::key_type &key,
...
StripCompactDigiSimLinks::makeReverseMap() const {
  ...
  for (trk_it itt = trackRecords_.begin(), endt = trackRecords_.end();
       itt != endt; ++itt) {
    hit_it edh =
        (itt + 1 != endt ? hstart + (itt + 1)->start : hitRecords_.end());
    for (; ith < edh; ++ith) {
