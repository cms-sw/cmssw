...
public:
  ...
  struct TrackRecord {
    ...
    // unsigned int   length;
    inline bool operator<(const TrackRecord &other) const {
      return key < other.key;
    }
    inline bool operator<(const key_type &otherKey) const {
      return key < otherKey;
    }
    inline bool operator==(const key_type &otherKey) const {
      return key == otherKey;
...
inline void swap(StripCompactDigiSimLinks &a, StripCompactDigiSimLinks &b) {
  a.swap(b);
}
#endif
