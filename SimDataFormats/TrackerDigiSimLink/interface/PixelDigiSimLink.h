...
public:
  ...
  PixelDigiSimLink(unsigned int ch, unsigned int tkId, unsigned int counter,
                   unsigned int tofBin, EncodedEventId e, float a) {
    chan = ch;
    simTkId = tkId;
    CFpos = tofBin == LowTof ? counter & 0x7FFFFFFF
                             : (counter & 0x7FFFFFFF) | 0x80000000;
    fract = a;
    eId = e;
  };
  PixelDigiSimLink(unsigned int ch, unsigned int tkId, EncodedEventId e,
                   float a) {
    chan = ch;
    simTkId = tkId;
    CFpos = 0;
  ...
  unsigned int TofBin() const {
    return (CFpos & 0x80000000) == 0 ? LowTof : HighTof;
  }
  EncodedEventId eventId() const { return eId; }
  float fraction() const { return fract; };

  inline bool operator<(const PixelDigiSimLink &other) const {
...
private:
  ...
  uint32_t CFpos; // position of the PSimHit in the CrossingFrame vector
                  // for the subdetector collection; bit 31 set if from the
                  // HighTof collection
  EncodedEventId eId;
  float fract;
};
