...
public:
  ...
  StripDigiSimLink(unsigned int ch, unsigned int tkId, unsigned int counter,
                   unsigned int tofBin, EncodedEventId e, float a)
      : chan(ch), simTkId(tkId),
  ...
  StripDigiSimLink(unsigned int ch, unsigned int tkId, unsigned int counter,
                   EncodedEventId e, float a)
      : chan(ch), simTkId(tkId), CFpos(counter & 0x7FFFFFFF), eId(e), fract(a) {
    ;
  }

  StripDigiSimLink(unsigned int ch, unsigned int tkId, EncodedEventId e,
                   float a)
  ...
  inline bool operator<(const StripDigiSimLink &other) const {
