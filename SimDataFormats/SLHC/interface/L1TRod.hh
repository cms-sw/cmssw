...
public:
  ...
  void addStub(const L1TStub& aStub){
    ...
    if (!inPhiRange(aStub.phi(),phi1m(aStub.module()),phi2m(aStub.module()))) {
      cout << "Warning: addStub stubphi:" << aStub.phi() << " " <<phi1_[aStub.module()]
	   << " " <<phi2_[aStub.module()] << " " <<aStub.r()*(aStub.phi()-phi1_[aStub.module()])<< " " <<aStub.r() << " " << r1m(aStub.module())<<" "<<aStub.layer() <<endl;
  ...
  void binaryTracklet(double &rinv_n, double &phi0_n, double &z0_n, double &t_n,
    ...
    ic=BASE7-ib.value(); 
    is2=is1*ic/BASE7;
    it1=ix1*DeltaOverr/BASE5; 
    it2=idelta-it1;         
    it3=it2*TwoOverOuterr/BASE6; 
    irinv=(is2*it3/(BASE2/BASE3)).value();     
    ialpha=lookup_alpha(ix1.value(),r);
    ibeta=lookup_beta(irinv,ix1.value(),r);
    iphi0=(ibeta-ialpha).value();      
    it=(ideltaz*is1/(BASE2/BASE3)).value();     
    it4=irinv*irinv/(BASE3*BASE3/BASE); 
    it5=it4*DXrsq/(BASE4*(BASE/BASE8));    
    it6p=ib.value()/(2*BASE7/BASE8)+BASE8;
    it6=it6p+it5;    
    it7=it*roverDX/BASE3;     
    it8=it6*it7/BASE8; 
  ...
  unsigned int matchTracklets(L1TTracklets& tracklets){
	...
	double phic;
	double r1=getrmin(stubs_[j].module(),phic); //this is approximate!
	...
	iphiproj=iphi0-lookup_beta(irinv,ix1.value(),r1);
	ia=ix1*DXOverr;
	ib=ia*ia/BASE9;
	it4=irinv*irinv/(BASE3*BASE3/BASE2);
	it5=it4*DXrsq/(BASE4*(BASE2/BASE8)); 
	it6p=ib/(2*BASE9/BASE8)+BASE8;
	it6=it6p+it5;    
	it7=it*roverDX/BASE3;     
	it8=it6*it7/BASE8;    
	int izproj=iz0+it8.value();
  ...
  void clean(){
    stubs_.clear();
    stubmultiplicity_.clear();
    stubmultiplicityroc_.clear();
  }
