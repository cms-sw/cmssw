...
public:
  ...
  PSimHit(const Local3DPoint &entry, const Local3DPoint &exit, float pabs,
          float tof, float eloss, int particleType, unsigned int detId,
          unsigned int trackId, float theta, float phi,
          unsigned short processType = 0)
      : theEntryPoint(entry), theSegment(exit - entry), thePabs(pabs),
        theEnergyLoss(eloss), theThetaAtEntry(theta), thePhiAtEntry(phi),
        theTof(tof), theParticleType(particleType), theProcessType(processType),
