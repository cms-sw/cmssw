...
public:
  MaterialAccountingTrack(void)
      : m_total(), m_current_volume(nullptr), m_detector(), m_steps(),
        m_detectors() {
  ...
  void enterDetector(const G4VPhysicalVolume *volume,
                     const GlobalPoint &position, double cosTheta);
...
private:
  MaterialAccountingStep
      m_total; // cache position along track (length and material)
