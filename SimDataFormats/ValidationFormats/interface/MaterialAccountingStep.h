...
public:
  MaterialAccountingStep(void)
      : m_length(0.), m_radiationLengths(0.), m_energyLoss(0.), m_in(),
        m_out() {}

  MaterialAccountingStep(double position, double radlen, double loss,
                         const GlobalPoint &in, const GlobalPoint &out)
      : m_length(position), m_radiationLengths(radlen), m_energyLoss(loss),
        m_in(in), m_out(out) {}
...
public:
  ...
  std::pair<MaterialAccountingStep, MaterialAccountingStep>
  ...
  MaterialAccountingStep &operator-=(const MaterialAccountingStep &step) {
    ...
    if ((step.m_in.perp2() <= m_in.perp2()) and
        (step.m_out.perp2() >= m_in.perp2()))
      m_in = step.m_out;

    if ((step.m_out.perp2() >= m_out.perp2()) and
        (step.m_in.perp2() <= m_out.perp2()))
      m_out = step.m_in;
...
inline MaterialAccountingStep operator+(const MaterialAccountingStep &x,
...
inline MaterialAccountingStep operator*(double y,
