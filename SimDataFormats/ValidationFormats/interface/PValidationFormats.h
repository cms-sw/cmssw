...
public:
  PGlobalSimHit()
      : nRawGenPart(0), nG4Vtx(0), nG4Trk(0), nECalHits(0), nPreShHits(0),
        nHCalHits(0), nPxlFwdHits(0), nPxlBrlHits(0), nSiFwdHits(0),
        nSiBrlHits(0), nMuonDtHits(0), nMuonCscHits(0), nMuonRpcFwdHits(0),
        nMuonRpcBrlHits(0) {}
...
public:
  PGlobalDigi()
      : nEBCalDigis(0), nEECalDigis(0), nESCalDigis(0), nHBCalDigis(0),
        nHECalDigis(0), nHOCalDigis(0), nHFCalDigis(0), nTIBL1Digis(0),
        nTIBL2Digis(0), nTIBL3Digis(0), nTIBL4Digis(0), nTOBL1Digis(0),
        nTOBL2Digis(0), nTOBL3Digis(0), nTOBL4Digis(0), nTIDW1Digis(0),
        nTIDW2Digis(0), nTIDW3Digis(0), nTECW1Digis(0), nTECW2Digis(0),
        nTECW3Digis(0), nTECW4Digis(0), nTECW5Digis(0), nTECW6Digis(0),
        nTECW7Digis(0), nTECW8Digis(0), nBRL1Digis(0), nBRL2Digis(0),
        nBRL3Digis(0), nFWD1pDigis(0), nFWD1nDigis(0), nFWD2pDigis(0),
        nFWD2nDigis(0), nMB1Digis(0), nMB2Digis(0), nMB3Digis(0), nMB4Digis(0),
  ...
  // put functions
  void putHBCalDigis(const std::vector<float> &aee,
                     const std::vector<float> &she);
  void putHECalDigis(const std::vector<float> &aee,
                     const std::vector<float> &she);
  void putHOCalDigis(const std::vector<float> &aee,
                     const std::vector<float> &she);
  void putHFCalDigis(const std::vector<float> &aee,
                     const std::vector<float> &she);
  // get functions
  ...
  // put functions
  void putTIBL1Digis(const std::vector<float> &adc,
                     const std::vector<int> &strip);
  void putTIBL2Digis(const std::vector<float> &adc,
                     const std::vector<int> &strip);
  void putTIBL3Digis(const std::vector<float> &adc,
                     const std::vector<int> &strip);
  void putTIBL4Digis(const std::vector<float> &adc,
                     const std::vector<int> &strip);
  void putTOBL1Digis(const std::vector<float> &adc,
                     const std::vector<int> &strip);
  void putTOBL2Digis(const std::vector<float> &adc,
                     const std::vector<int> &strip);
  void putTOBL3Digis(const std::vector<float> &adc,
                     const std::vector<int> &strip);
  void putTOBL4Digis(const std::vector<float> &adc,
                     const std::vector<int> &strip);
  void putTIDW1Digis(const std::vector<float> &adc,
                     const std::vector<int> &strip);
  void putTIDW2Digis(const std::vector<float> &adc,
                     const std::vector<int> &strip);
  void putTIDW3Digis(const std::vector<float> &adc,
                     const std::vector<int> &strip);
  void putTECW1Digis(const std::vector<float> &adc,
                     const std::vector<int> &strip);
  void putTECW2Digis(const std::vector<float> &adc,
                     const std::vector<int> &strip);
  void putTECW3Digis(const std::vector<float> &adc,
                     const std::vector<int> &strip);
  void putTECW4Digis(const std::vector<float> &adc,
                     const std::vector<int> &strip);
  void putTECW5Digis(const std::vector<float> &adc,
                     const std::vector<int> &strip);
  void putTECW6Digis(const std::vector<float> &adc,
                     const std::vector<int> &strip);
  void putTECW7Digis(const std::vector<float> &adc,
                     const std::vector<int> &strip);
  void putTECW8Digis(const std::vector<float> &adc,
                     const std::vector<int> &strip);
  // get functions
  ...
  // put functions
  void putMB1Digis(const std::vector<int> &slayer,
                   const std::vector<float> &time,
                   const std::vector<int> &layer);
  void putMB2Digis(const std::vector<int> &slayer,
                   const std::vector<float> &time,
                   const std::vector<int> &layer);
  void putMB3Digis(const std::vector<int> &slayer,
                   const std::vector<float> &time,
                   const std::vector<int> &layer);
  void putMB4Digis(const std::vector<int> &slayer,
                   const std::vector<float> &time,
                   const std::vector<int> &layer);
  // get functions
...
public:
  PGlobalRecHit()
      : nEBCalRecHits(0), nEECalRecHits(0), nESCalRecHits(0), nHBCalRecHits(0),
        nHECalRecHits(0), nHOCalRecHits(0), nHFCalRecHits(0), nTIBL1RecHits(0),
        nTIBL2RecHits(0), nTIBL3RecHits(0), nTIBL4RecHits(0), nTOBL1RecHits(0),
        nTOBL2RecHits(0), nTOBL3RecHits(0), nTOBL4RecHits(0), nTIDW1RecHits(0),
        nTIDW2RecHits(0), nTIDW3RecHits(0), nTECW1RecHits(0), nTECW2RecHits(0),
        nTECW3RecHits(0), nTECW4RecHits(0), nTECW5RecHits(0), nTECW6RecHits(0),
        nTECW7RecHits(0), nTECW8RecHits(0), nBRL1RecHits(0), nBRL2RecHits(0),
        nBRL3RecHits(0), nFWD1pRecHits(0), nFWD1nRecHits(0), nFWD2pRecHits(0),
        nFWD2nRecHits(0), nDTRecHits(0), nCSCRecHits(0), nRPCRecHits(0) {}
  ...
  // put functions
  void putEBCalRecHits(const std::vector<float> &re,
                       const std::vector<float> &she);
  void putEECalRecHits(const std::vector<float> &re,
                       const std::vector<float> &she);
  void putESCalRecHits(const std::vector<float> &re,
                       const std::vector<float> &she);
  // get functions
  ...
  // put functions
  void putHBCalRecHits(const std::vector<float> &rec,
                       const std::vector<float> &r,
                       const std::vector<float> &she);
  void putHECalRecHits(const std::vector<float> &rec,
                       const std::vector<float> &r,
                       const std::vector<float> &she);
  void putHOCalRecHits(const std::vector<float> &rec,
                       const std::vector<float> &r,
                       const std::vector<float> &she);
  void putHFCalRecHits(const std::vector<float> &rec,
                       const std::vector<float> &r,
                       const std::vector<float> &she);
  // get functions
  ...
  // put functions
  void putDTRecHits(const std::vector<float> &rhd,
  ...
  // put functions
  void putRPCRecHits(const std::vector<float> &rhx,
...
public:
  PEcalValidInfo()
      : ee1(0.0), ee4(0.0), ee9(0.0), ee16(0.0), ee25(0.0), eb1(0.0), eb4(0.0),
        eb9(0.0), eb16(0.0), eb25(0.0), totalEInEE(0.0), totalEInEB(0.0),
        totalEInES(0.0), totalHits(0), nHitsInEE(0), nHitsInEB(0), nHitsInES(0),
        nHitsIn1ES(0), nHitsIn2ES(0) {}
...
public:
  PHcalValidInfoJets()
      : nJetHit(0), nJet(0), ecalJet(0.), hcalJet(0.), hoJet(0.), etotJet(0.),
        detaJet(0.), dphiJet(0.), drJet(0.), dijetM(0.) {}
  ...
  void fillEtaPhiProfileJet(double eta0, double phi0, double eta, double phi,
                            double dist);
...
public:
  PHcalValidInfoLayer()
      : hitN(0), eHO(0.0), eHBHE(0.0), eEBEE(0.0), elongHF(0.0), eshortHF(0.0),
        eEcalHF(0.0), eHcalHF(0.0) {}
...
public:
  PHcalValidInfoNxN()
      : nNxN(0), ecalNxNr(0), hcalNxNr(0.), hoNxNr(0.), etotNxNr(0.),
        ecalNxN(0.), hcalNxN(0.), hoNxN(0.), etotNxN(0.) {}
...
public:
  PMuonSimHit()
      : nRawGenPart(0), nG4Vtx(0), nG4Trk(0), nCSCHits(0), nDTHits(0),
        nRPCHits(0) {}
  virtual ~PMuonSimHit() {}
  ...
  struct CSC {
    CSC()
        : _cscId(0), _detUnitId(0), _trackId(0), _processType(0),
          _particleType(0), _pabs(0), _globposz(0), _globposphi(0),
          _globposeta(0), _locposx(0), _locposy(0), _locposz(0), _locdirx(0),
          _locdiry(0), _locdirz(0), _locdirtheta(0), _locdirphi(0),
          _exitpointx(0), _exitpointy(0), _exitpointz(0), _entrypointx(0),
          _entrypointy(0), _entrypointz(0), _enloss(0), _tof(0) {}
  ...
  struct DT {
    DT()
        : _detUnitId(0), _trackId(0), _processType(0), _particleType(0),
          _pabs(0), _globposz(0), _globposphi(0), _globposeta(0), _locposx(0),
          _locposy(0), _locposz(0), _locdirx(0), _locdiry(0), _locdirz(0),
          _locdirtheta(0), _locdirphi(0), _exitpointx(0), _exitpointy(0),
          _exitpointz(0), _entrypointx(0), _entrypointy(0), _entrypointz(0),
          _enloss(0), _tof(0) {}
...
public:
  ...
  struct Hit {
    Hit()
        : _sysID(0), _detUnitId(0), _trackId(0), _processType(0),
          _particleType(0), _pabs(0), _lpx(0), _lpy(0), _lpz(0), _ldx(0),
          _ldy(0), _ldz(0), _ldtheta(0), _ldphi(0), _exx(0), _exy(0), _exz(0),
  ...
  void putHits(const std::vector<int> &_sysID,
               ...
               const std::vector<float> &_pabs, const std::vector<float> &_lpx,
               const std::vector<float> &_lpy, const std::vector<float> &_lpz,
               const std::vector<float> &_ldx, const std::vector<float> &_ldy,
               const std::vector<float> &_ldz,
               const std::vector<float> &_ldtheta,
               const std::vector<float> &_ldphi, const std::vector<float> &_exx,
               const std::vector<float> &_exy, const std::vector<float> &_exz,
               const std::vector<float> &_enx, const std::vector<float> &_eny,
               const std::vector<float> &_enz, const std::vector<float> &_eloss,
               const std::vector<float> &_tof);
