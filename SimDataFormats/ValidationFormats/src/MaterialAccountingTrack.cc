...
void MaterialAccountingTrack::enterDetector(const G4VPhysicalVolume *volume,
  ...
  if (m_current_volume != nullptr) {
    ...
    if (m_current_volume != volume)
      std::cerr << "MaterialAccountingTrack::leaveDetector(...): ERROR: "
                   "entering volume ("
                << volume << ") while inside volume (" << m_current_volume
                << ")" << std::endl;
    else
      std::cerr << "MaterialAccountingTrack::leaveDetector(...): ERROR: "
                   "entering volume ("
                << volume << ") twice" << std::endl;
    m_detector.clear();
    return;
  }
...
void MaterialAccountingTrack::leaveDetector(const G4VPhysicalVolume *volume,
  ...
  if (m_current_volume != volume) {
    ...
    if (m_current_volume)
      ...
      std::cerr << "MaterialAccountingTrack::leaveDetector(...): ERROR: "
                   "leaving volume ("
                << volume << ") while not inside any volume" << std::endl;
    m_detector.clear();
    return;
  }
