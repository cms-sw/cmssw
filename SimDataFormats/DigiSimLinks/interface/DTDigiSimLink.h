...
public:
  ...
  // uniquely multiple digis on the same wire), the TDC counts, the SimTrack Id
  // and the EncodedEvent Id.
  explicit DTDigiSimLink(int wireNr, int digiNr, int nTDC, unsigned int trackId,
                         EncodedEventId evId);

  // Construct from the wire number and the digi number (this identifies
  // uniquely multiple digis on the same wire), the time (ns), the SimTrack Id
  // and the EncodedEvent Id. time is converted in TDC counts (1 TDC = 25./32.
  // ns)
  explicit DTDigiSimLink(int wireNr, int digiNr, double tdrift,
                         unsigned int trackId, EncodedEventId evId);
...
inline std::ostream &operator<<(std::ostream &o,
                                const DTDigiSimLink &digisimlink) {
  return o << "wire:" << digisimlink.wire() << " digi:" << digisimlink.number()
