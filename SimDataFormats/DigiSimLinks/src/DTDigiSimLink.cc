...
DTDigiSimLink::DTDigiSimLink(int wireNr, int digiNr, int nTDC,
                             unsigned int trackId, EncodedEventId evId)
    : theWire(wireNr), theDigiNumber(digiNr), theCounts(nTDC),
      theSimTrackId(trackId), theEventId(evId) {}

DTDigiSimLink::DTDigiSimLink(int wireNr, int digiNr, double tdrift,
                             unsigned int trackId, EncodedEventId evId)
    : theWire(wireNr), theDigiNumber(digiNr),
      theCounts(static_cast<int>(tdrift / reso)), theSimTrackId(trackId),
      theEventId(evId) {}

DTDigiSimLink::DTDigiSimLink()
    : theWire(0), theDigiNumber(0), theCounts(0), theSimTrackId(0),
      theEventId(0) {}
...
DTDigiSimLink::ChannelType DTDigiSimLink::channel() const {
  ...
  DTDigiSimLink::ChannelType *p_result =
      reinterpret_cast<DTDigiSimLink::ChannelType *>(&result);
  return *p_result;
}
