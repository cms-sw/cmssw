...
HFShowerPhoton::HFShowerPhoton(const Point &p, float t, float lambda)
    : position_(p), lambda_(lambda), time_(t) {}
...
std::ostream &operator<<(std::ostream &os, const HFShowerPhoton &it) {
  os << "X " << std::setw(6) << it.x() << " Y " << std::setw(6) << it.y()
     << " Z " << std::setw(6) << it.z() << " t " << std::setw(6) << it.t()
     << " lambda " << it.lambda();
  return os;
}
