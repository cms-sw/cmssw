...
void PHcalTB04Info::setIDs(const std::vector<int> &ide,
  ...
  LogDebug("SimHCalData") << "PHcalTB04Info:: Called with " << nCrystal
                          << " crystals";
  for (int i = 0; i < nCrystal; i++) {
    idEcal.push_back(ide[i]);
    LogDebug("SimHCalData")
        << "\tIndex for " << i << " =  0x" << std::hex << idEcal[i] << std::dec;
  }
  ...
  LogDebug("SimHCalData") << "PHcalTB04Info:: Called with " << nTower
                          << " HCal towers";
  for (int i = 0; i < nTower; i++) {
    idHcal.push_back(idh[i]);
    LogDebug("SimHCalData")
        << "\tIndex for " << i << " =  0x" << std::hex << idHcal[i] << std::dec;
  }
}
...
void PHcalTB04Info::setPrimary(int primary, int id, double energy, double etav,
  ...
  etaBeam = (float)(etav);
  phiBeam = (float)(phiv);
  LogDebug("SimHCalData") << "PHcalTB04Info::setPrimary: nPrimary " << nPrimary
...
void PHcalTB04Info::setEdep(double simtot, double sime, double simh,
  ...
  LogDebug("SimHCalData") << "PHcalTB04Info::setEdep: simEtot " << simEtot
                          << " simEEc " << simEEc << " simEHc " << simEHc
                          << " digEtot " << digEtot << " digEEc " << digEEc
                          << " digEHc " << digEHc;
}
...
void PHcalTB04Info::setEdepEcal(const std::vector<double> &esim,
  ...
  for (int i = 0; i < nCrystal; i++) {
    ...
    LogDebug("SimHCalData")
        << "PHcalTB04Info::setEdepEcal [" << i << "] Esim = " << esime[i]
        << " Edig = " << edige[i];
  }
}
...
void PHcalTB04Info::setEdepHcal(const std::vector<double> &esim,
  ...
  for (int i = 0; i < nTower; i++) {
    ...
    LogDebug("SimHCalData")
        << "PHcalTB04Info::setEdepHcal [" << i << "] Esim = " << esimh[i]
        << " Edig = " << edigh[i];
  }
}
  ...
  if (siz > 0) {
    ...
    for (int i = 0; i < siz; i++) {
      ...
      LogDebug("SimHCalData")
          ...
          << "] latsimEta = " << latsimEta[i] << " latdigEta = " << latdigEta[i]
          << " latsimPhi = " << latsimPhi[i] << " latdigPhi = " << latdigPhi[i];
    }
  }
}
...
void PHcalTB04Info::setLongProf(const std::vector<double> &es,
  ...
  if (siz > 0) {
    ...
    for (int i = 0; i < siz; i++) {
      ...
      LogDebug("SimHCalData")
          << "PHcalTB04Info::setLongProf [" << i << "] longsim = " << longsim[i]
          << " longdig = " << longdig[i];
    }
  }
}
...
void PHcalTB04Info::saveHit(int det, int lay, int eta, int phi, double e,
  ...
  LogDebug("SimHCalData") << "PHcalTB04Info::saveHit " << hit << " Det "
                          << detHit[nh] << " layer " << layHit[nh] << " Eta "
                          << etaHit[nh] << " Phi " << phiHit[nh] << " E "
                          << eHit[nh] << " t " << tHit[nh];
}
...
void PHcalTB04Info::setVtxPrim(int evNum, int type, double x, double y,
  ...
  v1V = (float)(v);
  v1W = (float)(w);
  LogDebug("SimHCalData") << "PHcalTB04Info::setVtxPrim: EvNum " << v1EvNum
...
void PHcalTB04Info::setVtxSec(int id, int pdg, double px, double py, double pz,
  ...
  LogDebug("SimHCalData") << "PHcalTB04Info::setVtxSec " << v1Nsec << " ID "
                          << v1secTrackID[ns] << " PDG Code " << v1secPartID[ns]
                          << " Px/Py/Pz/Ek " << v1secPx[ns] << "/"
                          << v1secPy[ns] << "/" << v1secPz[ns] << "/"
                          << v1secEk[ns];
}
