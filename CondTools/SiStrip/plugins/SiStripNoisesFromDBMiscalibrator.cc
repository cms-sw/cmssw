...
void
SiStripNoisesFromDBMiscalibrator::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   using namespace edm;
...
SiStripNoisesFromDBMiscalibrator::getNewObject_withDefaults(const std::map<std::pair<uint32_t,int>,float>& theMap,const float theDefault) 
{
  ...
  for(std::map<uint32_t, SiStripDetInfoFileReader::DetInfo >::const_iterator it = DetInfos.begin(); it != DetInfos.end(); it++){    
    ...
    if(isMissing) missingDetIds.push_back(it->first);
 
    if ( ! obj->put(it->first,theSiStripVector) ) {
      edm::LogError("SiStripNoisesFromDBMiscalibrator")<<"[SiStripNoisesFromDBMiscalibrator::analyze] detid already exists"<<std::endl;
    }
  }
...
//********************************************************************************//
std::unique_ptr<SiStripNoises>
SiStripNoisesFromDBMiscalibrator::getNewObject(const std::map<std::pair<uint32_t,int>,float>& theMap) 
{
  std::unique_ptr<SiStripNoises> obj = std::unique_ptr<SiStripNoises>(new SiStripNoises());
...
void
SiStripNoisesFromDBMiscalibrator::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  edm::ParameterSetDescription desc;
  
  desc.setComment("Creates rescaled / smeared SiStrip Noise payload."
