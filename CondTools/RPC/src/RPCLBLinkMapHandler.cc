...
RPCDetId RPCLBLinkMapHandler::getRPCDetId(
  ...
  if (!region) {  // barrel
    ...
    // sector well-defined
    subsector = 1;
    if (subsector_string == "+" && (station == 3 || (station == 4 && (sector != 4 && sector != 9 && sector != 11))))
      subsector = 2;
...
void RPCLBLinkMapHandler::getNewObjects() {
  ...
  query->addToOutputList("CAST(DECODE(SIGN(MAX_STRIP.STRIP - CHAMBERSTRIP.CHAMBERSTRIPNUMBER), 1, 1, -1) AS INTEGER)",
                         "SLOPE");
  query->addToOutputList("MIN_CHANNEL.CHANNELS", "CHANNELS");
  query->addToOutputList(
      "CAST(DECODE(CHAMBERLOCATION.BARRELORENDCAP, 'Barrel', 0, DECODE(SIGN(CHAMBERLOCATION.DISKORWHEEL), 1, 1, -1)) "
      "AS INTEGER)",
      "REGION");
  ...
  subquery_min_channel.setCondition("CABLECHANNELNUM IS NOT NULL", subquery_min_channel_condition_data);
  ...
  subquery_max_strip.setCondition(
  ...
  subquery_max_channel.setCondition("CABLECHANNELNUM IS NOT NULL", subquery_max_channel_condition_data);
  ...
  RPCLBLinkMap::map_type& lb_link_map = lb_link_map_object->getMap();
  RPCLBLink lb_link;
  RPCDetId det_id;
  ...
  while (cursor.next()) {
    ...
    lb_link_map.insert(
        std::pair<RPCLBLink, RPCFebConnector>(lb_link, RPCFebConnector(det_id, first_strip, slope, channels)));
  }
  cursor.close();
...
std::string RPCLBLinkMapHandler::id() const { return id_; }
