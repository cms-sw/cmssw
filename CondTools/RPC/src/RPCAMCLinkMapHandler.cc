    ...
    , txt_file_(config.getUntrackedParameter<std::string>("txtFile", ""))
{
    ...
    for (std::vector<long long>::const_iterator sector_amc_packed = wos_sector_amc_packed.begin()
             ; sector_amc_packed != wos_sector_amc_packed.end() ; ++sector_amc_packed, ++sector_amc) {
...
void RPCAMCLinkMapHandler::getNewObjects()
{
    ...
    RPCAMCLinkMap::map_type & amc_link_map
        = amc_link_map_object->getMap();
    RPCLBLink lb_link;
    ...
    while (input_file) {
        ...
        if (sector > n_sectors_) {
            throw cms::Exception("RPCAMCLinkMapHandler") << "Found sector greater than the number of sectors: "
                                                         << sector << " > " << n_sectors_;
        }

        if (wos >= wos_fed_.size()) {
            throw cms::Exception("RPCAMCLinkMapHandler") << "Found " << (wheel_not_side_ ? "wheel" : "side") << " outside range: "
                                                         << wos << " >= " << wos_fed_.size();
        }
        ...
        amc_link_map.insert(std::pair<RPCAMCLink, RPCLBLink>(RPCAMCLink(wos_fed_.at(wos), amc_number, amc_input)
                                                             , lb_link));
