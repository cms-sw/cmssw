...
void RPCLBLinkNameParser::parse(std::string const & name, RPCLBLink & lb_link)
{
    ...
    switch (name.at(pos)) {
    case 'B': lb_link.setRegion(0); break;
    case 'E': lb_link.setRegion(1); break;
    default:
    ...
    if (pos == std::string::npos || (pos += 2) >= size)
        throw cms::Exception("InvalidLinkBoardName") << "Name too short: " << name;
    switch (name.at(pos)) {
    case 'N': lb_link.setSide(0); break;
    case 'M': lb_link.setSide(1); break;
    case 'P': lb_link.setSide(2); break;
    default:
