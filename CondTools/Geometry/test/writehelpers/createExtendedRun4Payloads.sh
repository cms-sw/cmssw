...
if [ $# -ne 1 ]
then
  echo Error: createExtendedRun4Payloads.sh requires exactly one argument which is the tag
  exit 1
fi
mytag=$1
echo ${mytag}
...
sed -i {s/TagXX/${mytag}/g} splitExtendedRun4Database.sh
...
# GeometryExtendedRun4_cff                       geSingleBigFile.xml
cmsRun geometryExtendedRun4_xmlwriter.py
...
cmsRun geometryExtendedRun4_writer.py
