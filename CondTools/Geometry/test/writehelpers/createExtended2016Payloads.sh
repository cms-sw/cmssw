...
if [ $# -ne 1 ]
then
  echo Error: createExtended2016Payloads.sh requires exactly one argument which is the tag
  exit 1
fi
mytag=$1
echo ${mytag}
...
# GeometryExtended2016_cff                       geSingleBigFile.xml
cmsRun geometryExtended2016_xmlwriter.py
...
cmsRun geometryExtended2016_writer.py
