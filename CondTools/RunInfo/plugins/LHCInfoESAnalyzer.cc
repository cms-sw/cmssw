...
namespace edmtest
{
  ...
  {
  public:
    explicit  LHCInfoESAnalyzer(edm::ParameterSet const& p) { 
      std::cout<<"LHCInfoESAnalyzer"<<std::endl;
    }
    explicit  LHCInfoESAnalyzer(int i) {
      std::cout<<"LHCInfoESAnalyzer "<<i<<std::endl; 
    }
  ...
  void LHCInfoESAnalyzer::analyze(const edm::Event& e, const edm::EventSetup& context){
    ...
    edm::eventsetup::EventSetupRecordKey recordKey(edm::eventsetup::EventSetupRecordKey::TypeTag::findType("LHCInfoRcd"));
    if( recordKey.type() == edm::eventsetup::EventSetupRecordKey::TypeTag()) {
      //record not found
      std::cout <<"Record \"LHCInfoRcd"<<"\" does not exist "<<std::endl;
    }
