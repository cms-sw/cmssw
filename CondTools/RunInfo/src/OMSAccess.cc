...
namespace cond {
  ...
  bool OMSServiceResultIterator::operator==(const OMSServiceResultIterator& rhs){
  ...
  OMSServiceResultIterator OMSServiceResult::begin() const {
    return OMSServiceResultIterator(m_data->begin());
  }

  OMSServiceResultIterator OMSServiceResult::end() const {
    return OMSServiceResultIterator(m_data->end());
  }
  ...
  bool OMSServiceResult::empty() const {
    return size()==0;
  }
  ...
  unsigned long OMSServiceQuery::status(){
    return m_status;
  }
  ...
  std::string OMSServiceQuery::url(){
    return m_url+m_filter+m_varList;
  }
 
  OMSService::OMSService():m_baseUrl(){}

  void OMSService::connect(const std::string& baseUrl){
    m_baseUrl=baseUrl;
  }
