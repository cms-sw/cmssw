...
namespace LHCInfoPerLSImpl {
  ...
  std::vector<std::pair<cond::Time_t, std::shared_ptr<LHCInfoPerLS>>>::const_iterator search(
      const cond::Time_t& val, const std::vector<std::pair<cond::Time_t, std::shared_ptr<LHCInfoPerLS>>>& container) {
    if (container.empty())
  ...
  for (auto r : queryResult) {
    if (r.get<std::string>("beams_stable") == "true") {
      ...
      edm::LogInfo(m_name) << "Buffered first lumisection of stable beam: LS: "
                           << r.get<std::string>("lumisection_number") << " run: " << r.get<std::string>("run_number");
