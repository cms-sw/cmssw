...
namespace cond {
  ...
  namespace impl {
    ...
    inline float s_to_f(const std::string& val){
      return std::stof(val);
    }
    template <> 
    ...
    inline int s_to_i(const std::string& val){
      return std::stoi(val);
    }
    template <> 
    ...
    inline unsigned long s_to_ul(const std::string& val){
  ...
  public:
    ...
    // getter for arrays
    template <typename primitive> std::vector<primitive> getArray( const std::string& attributeName ){
