  ...
  public:
    PopConBTransitionSourceHandler(edm::ParameterSet const& pset)
        : m_run(pset.getParameter<edm::ParameterSet>("BTransition").getParameter<unsigned long long>("runNumber")),
          m_currentThreshold(pset.getParameter<edm::ParameterSet>("BTransition")
                                 .getUntrackedParameter<double>("currentThreshold", 18000.)),
          m_tagForRunInfo(
              pset.getParameter<edm::ParameterSet>("BTransition").getParameter<std::string>("tagForRunInfo")),
          ...
          m_connectionPset(
              pset.getParameter<edm::ParameterSet>("BTransition").getParameter<edm::ParameterSet>("DBParameters")) {
    ...
    bool checkBOn() {
      ...
      LogDebug("PopConBTransitionSourceHandler")
          << "Comparing value of magnet current: " << avg_current << " A for run: " << m_run
      ...
      edm::LogInfo("PopConBTransitionSourceHandler")
          << "["
          << "PopConBTransitionSourceHandler::" << __func__ << "]: "
          << "The magnet was " << (isBOn ? "ON" : "OFF") << " during run " << m_run << std::endl;
      trans.close();
      return isBOn;
    }
    ...
    virtual void getObjectsForBTransition(bool isBOn) {
      ...
      if (currentIov.payloadId != this->tagInfo().lastPayloadToken) {
        ...
        edm::LogInfo("PopConBTransitionSourceHandler")
            << "["
            << "PopConBTransitionSourceHandler::" << __func__ << "]: " << ss.str() << std::endl;
        ...
        edm::LogInfo("PopConBTransitionSourceHandler")
            ...
            << " corresponding to the calibrations for magnetic field " << (isBOn ? "ON" : "OFF")
            << " is still valid for run " << m_run << " in the destination tag " << destTag << ".\nNo transfer needed."
            << std::endl;
      }
      trans.close();
    }
