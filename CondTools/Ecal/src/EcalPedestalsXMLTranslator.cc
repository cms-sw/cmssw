...
int EcalPedestalsXMLTranslator::readXML(const std::string& filename, EcalCondHeader& header, EcalPedestals& record) {
...
std::string EcalPedestalsXMLTranslator::dumpXML(const EcalCondHeader& header, const EcalPedestals& record) {
  ...
  DOMDocument* doc = impl->createDocument(nullptr, cms::xerces::uStr(Pedestals_tag.c_str()).ptr(), doctype);
  ...
  for (int cellid = EBDetId::MIN_HASH; cellid < EBDetId::kSizeForDenseIndexing; ++cellid) {
    ...
    if (record.find(rawid) == record.end())
      continue;
    if (!record[rawid].mean_x12 && !record[rawid].rms_x12)
      continue;
