...
void popcon::EcalIntercalibHandler::getNewObjects()
{
  ...
  EcalIntercalibConstant the_cal = 0.;
  
  int iX=50;
  int iY=5;
  int iZ=-1;
  float the_value_high=0.75585;
  ...
  if(irun>max_since) {
    ...
    } else if(mag_cur<3000. && magnet_high ) {
      something_to_transfer=true;
