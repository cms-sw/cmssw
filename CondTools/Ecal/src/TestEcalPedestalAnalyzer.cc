...
void TestEcalPedestalAnalyzer::TestEcalPedestalAnalyzer::initSource(const edm::Event& evt, const edm::EventSetup& est)
{
	...
	if(m_source=="H2") { 
	  this->m_handler_object =new EcalH2PedestalsHandler("EcalPedestals",m_offline_connection, evt,est, m_firstRun,m_lastRun,  m_sid,  m_user,  m_pass );
	} else if (m_source=="H4") { 
	  this->m_handler_object =new EcalH2PedestalsHandler("EcalPedestals",m_offline_connection, evt,est, m_firstRun,m_lastRun,  m_sid,  m_user,  m_pass );
	} else if (m_source=="P5") { 
	  this->m_handler_object =new EcalPedestalsHandler("EcalPedestals",m_offline_connection, evt,est, m_firstRun,m_lastRun,  m_sid,  m_user,  m_pass, m_gentag, m_location );
	} 
}
