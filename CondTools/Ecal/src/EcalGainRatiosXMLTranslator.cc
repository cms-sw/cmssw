...
int EcalGainRatiosXMLTranslator::readXML(const std::string& filename, EcalCondHeader& header, EcalGainRatios& record) {
...
std::string EcalGainRatiosXMLTranslator::dumpXML(const EcalCondHeader& header, const EcalGainRatios& record) {
  ...
  for (int cellid = EBDetId::MIN_HASH; cellid < EBDetId::kSizeForDenseIndexing; ++cellid) {
    ...
    if (record.find(rawid) == record.end())
      continue;
    if (!record[rawid].gain12Over6() && !record[rawid].gain6Over1())
      continue;
