  ...
  public:
    PopConESTransitionSourceHandler(edm::ParameterSet const& pset)
        : m_run(pset.getParameter<edm::ParameterSet>("ESTransition").getParameter<unsigned long long>("runNumber")),
          ...
          m_tagForRunInfo(
              pset.getParameter<edm::ParameterSet>("ESTransition").getParameter<std::string>("tagForRunInfo")),
          m_ESGain(pset.getParameter<edm::ParameterSet>("ESTransition").getParameter<std::string>("ESGain")),
          m_tagForLowGain(
              pset.getParameter<edm::ParameterSet>("ESTransition").getParameter<std::string>("ESLowGainTag")),
          m_tagForHighGain(
              pset.getParameter<edm::ParameterSet>("ESTransition").getParameter<std::string>("ESHighGainTag")),
          m_connectionString(pset.getParameter<edm::ParameterSet>("ESTransition").getParameter<std::string>("connect")),
          m_connectionPset(
              pset.getParameter<edm::ParameterSet>("ESTransition").getParameter<edm::ParameterSet>("DBParameters")) {
      edm::LogInfo("PopConESTransitionSourceHandler") << "["
    ...
    bool checkLowGain() {
      ...
      LogDebug("PopConESTransitionSourceHandler")
    ...
    virtual void getObjectsForESTransition(bool isLowGain) {
      ...
      cond::persistency::IOVProxy iov = session.readIov(isLowGain ? m_tagForLowGain : m_tagForHighGain);
      cond::Iov_t currentIov = iov.getInterval(m_run);
      LogDebug("PopConESTransitionSourceHandler")
          << "Loaded IOV sequence from tag " << (isLowGain ? m_tagForLowGain : m_tagForHighGain)
      ...
      if (currentIov.payloadId != this->tagInfo().lastInterval.payloadId) {
        ...
        ss << "Adding iov with since " << m_run << " pointing to hash " << currentIov.payloadId
           << " corresponding to the ES Gain " << (isLowGain ? "LOW" : "HIGH");
        edm::LogInfo("PopConESTransitionSourceHandler")
            << "["
            << "PopConESTransitionSourceHandler::" << __func__ << "]: " << ss.str() << std::endl;
        ...
        edm::LogInfo("PopConESTransitionSourceHandler")
            << "["
            << "PopConESTransitionSourceHandler::" << __func__ << "]: "
            << "The payload with hash " << currentIov.payloadId << " corresponding to ES Gain"
            << (isLowGain ? "LOW" : "HIGH") << " is still valid for run " << m_run << " in the destination tag "
