...
// parameters to setup 
  ...
  void DTTimeUtility::analyze( const edm::Event& e,
                          const edm::EventSetup& context ) {
//    using namespace edm::eventsetup;
    long long int condConv = 
          ( ( ( ( condTime >> 32 ) & 0xFFFFFFFF ) * 1000000000 ) +
            ( (   condTime         & 0xFFFFFFFF ) * 1000       ) );
    coral::TimeStamp condStamp( condConv );
    std::cout <<     " condTime " << condTime
              << " -> coralTime " << condConv << " ( " 
              << condStamp.year()   << " , "
              << condStamp.month()  << " , "
              << condStamp.day()    << " h "
    ...
    long long int coralConv = 
            ( ( ( coralTime / 1000000000 )  << 32 ) +
              ( ( coralTime % 1000000000 ) / 1000 ) );
    coral::TimeStamp convStamp( coralTime );
    std::cout <<   " coralTime " << coralTime
              << " -> condTime " << coralConv << " ( " 
              << convStamp.year()   << " , "
              << convStamp.month()  << " , "
              << convStamp.day()    << " h "
    ...
    coralTime = coralStamp.total_nanoseconds();
     condTime = ( ( ( coralTime / 1000000000 )  << 32 ) +
                  ( ( coralTime % 1000000000 ) / 1000 ) );
    std::cout << coralStamp.year()   << " , "
              << coralStamp.month()  << " , "
              << coralStamp.day()    << " h "
              << coralStamp.hour()   << ":"
              << coralStamp.minute() << ":"
