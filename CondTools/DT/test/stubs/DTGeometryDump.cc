  ...
  void DTGeometryDump::analyze(const edm::Event& e, const edm::EventSetup& context) {
    ...
    while (ch_iter != ch_iend) {
      ...
      while (ch_id_iter != ch_id_iend) {
        if (*ch_id_iter++ == ch_id) {
          ch_found = true;
          std::cout << "chamber already found:: " << ch_id.wheel() << " " << ch_id.sector() << " " << ch_id.station()
                    << std::endl;
          break;
        }
      }
      if (!ch_found)
        ch_id_cont.push_back(ch_id);
      else
        continue;
      ...
      while (sl_iter != sl_iend) {
        ...
        while (sl_id_iter != sl_id_iend) {
          if (*sl_id_iter++ == sl_id) {
            sl_found = true;
            std::cout << "superlayer already found: " << sl_id.wheel() << " " << sl_id.sector() << " "
                      << sl_id.station() << " " << sl_id.superlayer() << std::endl;
            break;
          }
        }
        if (!sl_found)
          sl_id_cont.push_back(sl_id);
        else
          continue;
        ...
        while (cl_iter != cl_iend) {
          ...
          while (cl_id_iter != cl_id_iend) {
            if (*cl_id_iter++ == cl_id) {
              cl_found = true;
              std::cout << "layer already found: " << cl_id.wheel() << " " << cl_id.sector() << " " << cl_id.station()
                        << " " << cl_id.superlayer() << " " << cl_id.layer() << std::endl;
              break;
            }
          }
          if (!cl_found)
            cl_id_cont.push_back(cl_id);
          else
            continue;
          ...
          std::cout << "layer: " << cl_id.wheel() << " " << cl_id.sector() << " " << cl_id.station() << " "
                    << cl_id.superlayer() << " " << cl_id.layer() << " has cells from " << fcell << " to " << lcell
                    << std::endl;
        }
      }
    }
