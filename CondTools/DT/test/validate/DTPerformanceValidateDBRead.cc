...
void DTPerformanceValidateDBRead::analyze(const edm::Event& e,
  ...
  std::cout << std::distance( mP->begin(), mP->end() )
  ...
  while ( iter != iend ) {
    ...
    status = mP->get( mpId.wheelId,
                      ...
                      DTTimeUnits::counts );
    if ( status ) logFile << "ERROR while getting sl performance "
                          << mpId.wheelId   << " "
                          << mpId.stationId << " "
                          << mpId.sectorId  << " "
                          << mpId.slId      << " , status = "
                          << status << std::endl;
    if ( ( fabs( mpData.meanT0         - meanT0         ) > 0.0001 ) ||
         ( fabs( mpData.meanTtrig      - meanTtrig      ) > 0.1    ) ||
         ( fabs( mpData.meanMtime      - meanMtime      ) > 0.0001 ) ||
         ( fabs( mpData.meanNoise      - meanNoise      ) > 0.0001 ) ||
         ...
         logFile << "MISMATCH WHEN READING sl performance "
                 << mpId.wheelId   << " "
                 << mpId.stationId << " "
                 << mpId.sectorId  << " "
                 << mpId.slId      << " : "
                 ...
                 << mpData.meanMtime      << " "
                 << mpData.meanNoise      << " "
                 << mpData.meanAfterPulse << " "
                 << mpData.meanResolution << " "
                 << mpData.meanEfficiency << " -> "
                 <<        meanT0         << " "
                 <<        meanTtrig      << " "
                 <<        meanMtime      << " "
                 <<        meanNoise      << " "
  ...
  while ( chkFile >> whe
    ...
    if ( ( fabs( ckmeanT0         - meanT0         ) > 0.0001 ) ||
         ...
         logFile << "MISMATCH IN WRITING AND READING sl performance "
                 << whe << " "
                 << sta << " "
                 << sec << " "
                 << qua << " : "
