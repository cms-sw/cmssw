...
void DTCCBConfigValidateDBRead::analyze( const edm::Event& e,
  ...
  if ( cfrDiff( fchkConf, fullConf ) )
       logFile << "MISMATCH IN WRITING AND READING full configuration"
               << std::endl;
  ...
  while ( chkFile >> whe
    ...
    status = conf->configKey( whe,
                              sta,
                              sec,
                              ccbConf );
    if ( status ) logFile << "ERROR while reading CCB configuration"
                          << whe << " "
                          << sta << " "
                          << sec << " , status = "
                          << status << std::endl;
    if ( cfrDiff( chkConf, ccbConf ) )
         logFile << "MISMATCH WHEN READING CCB configuration "
                 << whe << " "
                 << sta << " "
                 << sec << std::endl;
  }
...
bool DTCCBConfigValidateDBRead::cfrDiff( const std::vector<int>& l_conf,
...
bool DTCCBConfigValidateDBRead::cfrDiff( const
                                         std::vector<DTConfigKey>& l_conf,
                                         const
                                         std::vector<DTConfigKey>& r_conf ) {
  if ( l_conf.size() != r_conf.size() ) return true;
  ...
  while ( ( l_iter != l_iend ) && 
    ...
    if ( l_key.confType != r_key.confType ) return true;
    if ( l_key.confKey  != r_key.confKey  ) return true;
  }
  return false;
}
