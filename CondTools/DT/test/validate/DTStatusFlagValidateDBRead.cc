...
void DTStatusFlagValidateDBRead::analyze( const edm::Event& e,
  ...
  std::cout << std::distance( sf->begin(), sf->end() )
  ...
  while ( iter != iend ) {
    ...
    if ( status ) logFile << "ERROR while getting cell flags "
                          << sfId.wheelId   << " "
                          << sfId.stationId << " "
                          << sfId.sectorId  << " "
                          << sfId.slId      << " "
                          ...
                          << sfId.cellId    << " , status = "
                          << status << std::endl;
    if ( ( sfData.noiseFlag ^ noiseFlag ) ||
         ( sfData.   feMask ^    feMask ) ||
         ( sfData.  tdcMask ^   tdcMask ) ||
         ( sfData. trigMask ^  trigMask ) ||
         ( sfData. deadFlag ^  deadFlag ) ||
         ( sfData. nohvFlag ^  nohvFlag ) )
         logFile << "MISMATCH WHEN READING cell flags "
                 << sfId.wheelId   << " "
                 << sfId.stationId << " "
                 << sfId.sectorId  << " "
                 << sfId.slId      << " "
                 << sfId.layerId   << " "
                 << sfId.cellId    << " : "
                 << sfData.noiseFlag << " " << sfData.  feMask << " "
                 << sfData.  tdcMask << " " << sfData.trigMask << " "
                 << sfData. deadFlag << " " << sfData.nohvFlag << " -> "
                 <<        noiseFlag << " " <<          feMask << " "
                 <<          tdcMask << " " <<        trigMask << " "
  ...
  while ( chkFile >> whe
    ...
    if ( ( cknoiseFlag ^ noiseFlag ) ||
         ...
         (  ckdeadFlag ^  deadFlag ) ||
         (  cknohvFlag ^  nohvFlag ) )
         logFile << "MISMATCH IN WRITING AND READING cell flags "
                 << whe << " "
                 << sta << " "
                 << sec << " "
                 << qua << " "
