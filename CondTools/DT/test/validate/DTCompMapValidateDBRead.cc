...
void DTCompMapValidateDBRead::analyze(const edm::Event& e,
...
//  std::ifstream chkFile( run_fn.str().c_str() );
  ...
  std::cout << ro->mapRobRos() << " "
            << ro->mapCellTdc() << std::endl;
  std::cout << std::distance( ro->begin(), ro->end() )
...
*/
    ...
    if ( ( ckwhe != whe ) ||
         ...
         ( cklay != lay ) ||
         ( ckcel != cel ) )
         logFile << "MISMATCH IN WRITING AND READING chan->cell map "
                 << ckddu << " "
                 << ckros << " "
                 << ckrob << " "
                 << cktdc << " "
    ...
    if ( ( ckddu != ddu ) ||
         ...
         ( cktdc != tdc ) ||
         ( ckcha != cha ) )
         logFile << "MISMATCH IN WRITING AND READING cell->chan map "
                 << ckwhe << " "
                 << cksta << " "
                 << cksec << " "
                 << ckqua << " "
