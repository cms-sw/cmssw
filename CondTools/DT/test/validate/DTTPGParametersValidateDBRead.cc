...
void DTTPGParametersValidateDBRead::analyze( const edm::Event& e,
  ...
  std::cout << std::distance( tpg->begin(), tpg->end() )
  ...
  while ( iter != iend ) {
    ...
    status = tpg->get( tpgId.wheelId,
                       tpgId.stationId,
                       tpgId.sectorId,
                       nClock, tPhase, DTTimeUnits::counts );
    if ( status ) logFile << "ERROR while getting cell TPGParameters "
                          << tpgId.wheelId   << " "
                          << tpgId.stationId << " "
    ...
    if (       ( tpgData.nClock != nClock )            ||
         ...
         logFile << "MISMATCH WHEN READING cell TPGParameters "
                 ...
                 << tpgId.stationId << " "
                 << tpgId.sectorId  << " : "
                 << tpgData.nClock << " " << tpgData.tPhase << " -> "
                 <<         nClock << " " <<         tPhase << std::endl;
    iter++;
  }
