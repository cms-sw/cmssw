...
void DTHVStatusValidateDBRead::analyze(const edm::Event& e,
  ...
  while ( iter != iend ) {
    ...
    if ( status ) logFile << "ERROR while getting HV status"
                          << hvId.wheelId   << " "
                          << hvId.stationId << " "
                          << hvId.sectorId  << " "
                          << hvId.slId      << " "
                          ...
                          << hvId.partId    << " , status = "
                          << status << std::endl;
    if ( ( hvData.fCell != fCell ) ||
         ( hvData.lCell != lCell ) ||
         ( hvData.flagA != flagA ) ||
         ( hvData.flagC != flagC ) ||
         ( hvData.flagS != flagS ) )
         logFile << "MISMATCH WHEN READING HV status"
                 << hvId.wheelId   << " "
                 << hvId.stationId << " "
                 << hvId.sectorId  << " "
                 << hvId.slId      << " : "
  ...
  while ( chkFile >> whe
    ...
    status = hv->get( whe,
                      ...
                      fCell, lCell,
                      flagA, flagC, flagS );
    if ( !flagA && !flagC && !flagS ) ckfCell = cklCell = 0;
    if ( ( ckfCell != fCell ) ||
         ( cklCell != lCell ) ||
         ( ckflagA != flagA ) ||
         ( ckflagC != flagC ) ||
         ( ckflagS != flagS ) )
         logFile << "MISMATCH IN WRITING AND READING HV status "
                 << whe << " "
                 << sta << " "
                 << sec << " "
                 << qua << " : "
