...
void DTRangeT0ValidateDBRead::analyze(const edm::Event& e, const edm::EventSetup& context) {
  ...
  while (iter != iend) {
    ...
    status = tR->get(tRId.wheelId, tRId.stationId, tRId.sectorId, tRId.slId, t0min, t0max);
    if (status)
      logFile << "ERROR while getting range T0" << tRId.wheelId << " " << tRId.stationId << " " << tRId.sectorId << " "
              << tRId.slId << " , status = " << status << std::endl;
    if ((std::abs(tRData.t0min - t0min) > 0.0001) || (std::abs(tRData.t0max - t0max) > 0.0001))
      logFile << "MISMATCH WHEN READING range T0" << tRId.wheelId << " " << tRId.stationId << " " << tRId.sectorId
              << " " << tRId.slId << " : " << tRData.t0min << " " << tRData.t0max << " -> " << t0min << " " << t0max
              << std::endl;
    iter++;
  }

  while (chkFile >> whe >> sta >> sec >> qua >> ckt0min >> ckt0max) {
    status = tR->get(whe, sta, sec, qua, t0min, t0max);
    if ((std::abs(ckt0min - t0min) > 0.0001) || (std::abs(ckt0max - t0max) > 0.0001))
      logFile << "MISMATCH IN WRITING AND READING range T0 " << whe << " " << sta << " " << sec << " " << qua << " : "
              << ckt0min << " " << ckt0max << " -> " << t0min << " " << t0max << std::endl;
  }
}
