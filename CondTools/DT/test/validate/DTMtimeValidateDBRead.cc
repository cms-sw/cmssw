...
void DTMtimeValidateDBRead::analyze(const edm::Event& e,
  ...
  std::cout << std::distance( mT->begin(), mT->end() )
  ...
  while ( iter != iend ) {
    ...
    if ( status ) logFile << "ERROR while getting sl Mtime "
                          ...
                          << mTId.slId      << " , status = "
                          << status << std::endl;
    if ( ( fabs( mTData.mTime - mTime ) > 0.01   ) ||
         ( fabs( mTData.mTrms - mTrms ) > 0.0001 ) )
         logFile << "MISMATCH WHEN READING sl Mtime "
                 << mTId.wheelId   << " "
                 << mTId.stationId << " "
                 << mTId.sectorId  << " "
                 << mTId.slId      << " : "
                 << mTData.mTime << " " << mTData.mTrms << " -> "
                 <<        mTime << " " <<        mTrms << std::endl;
    iter++;
  }
