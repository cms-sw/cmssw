...
void DTROMapValidateDBRead::analyze(const edm::Event& e,
...
//  std::ifstream chkFile( run_fn.str().c_str() );
  ...
  while ( iter != iend ) {
    ...
    status = ro->readOutToGeometry( link.dduId,
                                    link.rosId,
                                    link.robId,
                                    link.tdcId,
                                    link.channelId,
                                    ...
                                    lay,
                                    cel );
    if ( status ) logFile << "ERROR while getting chan->cell map "
                          << link.    dduId << " "
                          << link.    rosId << " "
                          << link.    robId << " "
                          << link.    tdcId << " "
                          << link.channelId << " , status = "
                          << status << std::endl;
    if ( ( link.  wheelId != whe ) ||
         ( link.stationId != sta ) ||
         ( link. sectorId != sec ) ||
         ...
         ( link.  layerId != lay ) ||
         ( link.   cellId != cel ) )
         logFile << "MISMATCH WHEN READING chan->cell  "
                 << link.    dduId << " "
                 << link.    rosId << " "
                 << link.    robId << " "
                 << link.    tdcId << " "
                 ...
                 << lay << " "
                 << cel << std::endl;
    status = ro->geometryToReadOut( link.wheelId,
                                    link.stationId,
                                    link.sectorId,
                                    link.slId,
                                    link.layerId,
                                    ...
                                    tdc,
                                    cha );
    if ( status ) logFile << "ERROR while getting cell->chan map "
                          << link.wheelId   << " "
                          << link.stationId << " "
                          << link.sectorId  << " "
                          << link.slId      << " "
    ...
    if ( ( link.    dduId != ddu ) ||
         ...
         ( link.    tdcId != tdc ) ||
         ( link.channelId != cha ) )
         logFile << "MISMATCH WHEN READING cell->chan  "
                 << link.  wheelId << " "
                 << link.stationId << " "
                 << link. sectorId << " "
                 << link.     slId << " "
