...
void DTLVStatusValidateDBRead::analyze(const edm::Event& e,
  ...
  std::cout << std::distance( lv->begin(), lv->end() )
  ...
  while ( iter != iend ) {
    ...
    status = lv->get( lvId.wheelId,
                      lvId.stationId,
                      lvId.sectorId,
                      flagCFE, flagDFE, flagCMC, flagDMC );
    if ( status ) logFile << "ERROR while getting LV status"
                          << lvId.wheelId   << " "
                          << lvId.stationId << " "
                          << lvId.sectorId  << " , status = "
                          << status << std::endl;
    if ( ( lvData.flagCFE != flagCFE ) ||
         ( lvData.flagDFE != flagDFE ) ||
         ( lvData.flagCMC != flagCMC ) ||
         ( lvData.flagDMC != flagDMC ) )
         logFile << "MISMATCH WHEN READING LV status"
                 << lvId.wheelId   << " "
                 << lvId.stationId << " "
                 << lvId.sectorId  << " : "
                 << lvData.flagCFE << " "
  ...
  while ( chkFile >> whe
    ...
    if ( ( ckflagCFE != flagCFE ) ||
         ...
         ( ckflagCMC != flagCMC ) ||
         ( ckflagDMC != flagDMC ) )
         logFile << "MISMATCH IN WRITING AND READING LV status "
                 << whe << " "
                 << sta << " "
                 << sec << " : "
                 << ckflagCFE << " "
