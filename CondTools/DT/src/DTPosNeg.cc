...
int DTPosNeg::getPN(const DTChamberId& cha) { return getPN(cha.wheel(), cha.sector(), cha.station()); }
...
int DTPosNeg::getCT(const DTChamberId& cha) { return getCT(cha.wheel(), cha.sector(), cha.station()); }
...
int DTPosNeg::idCode(int whe, int sec, int sta) { return ((((whe + 3) * 100) + sec) * 10) + sta; }

int DTPosNeg::pnCode(int p, int t) { return (p * 1000) + t; }
