...
void 
SiPixel2DTemplateDBObjectReader::beginJob()
{
}

void
SiPixel2DTemplateDBObjectReader::analyze(const edm::Event& iEvent, const edm::EventSetup& setup)
{
...
hp*/
		...
		for(int i=0; i<numOfTempl;++i) {
			...
			std::cout << "Calibration " << i+1 << " of " << numOfTempl << ", with 2DTemplate ID " << dbobject.sVector()[index]
			...
			tout << the2DTemplateCalibrationLocation.c_str() << "/data/template2D_IOV5/template_summary2D_zp"
		...
		if(theFull2DTemplateDBOutput) std::cout << dbobject << std::endl;
	}
}


void 
SiPixel2DTemplateDBObjectReader::endJob()
{
}
