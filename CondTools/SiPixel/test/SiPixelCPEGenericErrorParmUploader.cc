...
void
SiPixelCPEGenericErrorParmUploader::analyze(const edm::Event& iEvent, const edm::EventSetup& setup)
{
}

void 
SiPixelCPEGenericErrorParmUploader::endJob()
{
  ...
  if( poolDbService.isAvailable() ) {
    if ( poolDbService->isNewTagRequest("SiPixelCPEGenericErrorParmRcd") )
      poolDbService->
	createNewIOV<SiPixelCPEGenericErrorParm>( obj,
					    poolDbService->beginOfTime(),
					    poolDbService->endOfTime(),
					    "SiPixelCPEGenericErrorParmRcd"  );
    else
      poolDbService->
	appendSinceTime<SiPixelCPEGenericErrorParm>( obj, 
