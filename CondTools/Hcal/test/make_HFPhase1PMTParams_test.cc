...
std::unique_ptr<HFPhase1PMTParams> make_HFPhase1PMTParams_test()
{
    ...
    cuts[HFPhase1PMTData::T_1_MIN] = boost::shared_ptr<AbsHcalFunctor>(
        new HcalPiecewiseLinearFunctor(pts, false, false));
#else
    cuts[HFPhase1PMTData::T_1_MIN] = boost::shared_ptr<AbsHcalFunctor>(
        new HcalLinearCompositionFunctor(cuts[HFPhase1PMTData::T_0_MIN], 1.0, -2.0));
#endif
    ...
    cuts[HFPhase1PMTData::T_1_MAX] = boost::shared_ptr<AbsHcalFunctor>(
        new HcalPiecewiseLinearFunctor(pts, false, false));
#else
    cuts[HFPhase1PMTData::T_1_MAX] = boost::shared_ptr<AbsHcalFunctor>(
        new HcalLinearCompositionFunctor(cuts[HFPhase1PMTData::T_0_MAX], 1.0, 2.0));
#endif
    ...
    pts.push_back(P(5.0*minChargeAsymm, -2.0));
    pts.push_back(P(10.0*minChargeAsymm, -1.0));
    cuts[HFPhase1PMTData::ASYMM_MIN] = boost::shared_ptr<AbsHcalFunctor>(
        new HcalPiecewiseLinearFunctor(pts, false, false));
#else
    cuts[HFPhase1PMTData::ASYMM_MIN] = boost::shared_ptr<AbsHcalFunctor>(
        new HcalLinearCompositionFunctor(cuts[HFPhase1PMTData::ASYMM_MAX], -1.0, 0.0));
#endif

    std::unique_ptr<HFPhase1PMTData> defaultItem(new HFPhase1PMTData(
              cuts, minCharge0, minCharge1, minChargeAsymm));
...
#else
    cuts[HFPhase1PMTData::ASYMM_MIN] = boost::shared_ptr<AbsHcalFunctor>(
        new HcalLinearCompositionFunctor(cuts[HFPhase1PMTData::ASYMM_MAX], -1.0, 1.0));
#endif

    std::unique_ptr<HFPhase1PMTData> firstItem(new HFPhase1PMTData(
              cuts, minCharge0, minCharge1, minChargeAsymm));
