...
modGains::modGains(const edm::ParameterSet& iConfig) : vectorop(false) {
  ...
  if ((std::strcmp(s_operation.c_str(), "add") == 0) || (std::strcmp(s_operation.c_str(), "sub") == 0) ||
    ...
    vectorop = true;
  } else if ((std::strcmp(s_operation.c_str(), "sadd") == 0) || (std::strcmp(s_operation.c_str(), "ssub") == 0) ||
    ...
    throw cms::Exception("Unknown", "modGains") << "Unknown operator. " << s_operation << " Stopping.\n";
  }
}
...
void modGains::analyze(edm::Event const&, edm::EventSetup const& iSetup) {
  ...
  for (unsigned i = 0; i < channels.size(); i++) {
    ...
    if (vectorop) {  // vector operation
      if ((std::strcmp(s_operation.c_str(), "mult") == 0) || (std::strcmp(s_operation.c_str(), "div") == 0))
        val = 1.0;  // mult,div
      if ((std::strcmp(s_operation.c_str(), "add") == 0) || (std::strcmp(s_operation.c_str(), "sub") == 0))
        val = 0.0;  // add,sub
    ...
    if ((std::strcmp(s_operation.c_str(), "add") == 0) || (std::strcmp(s_operation.c_str(), "sadd") == 0))
      p_item = std::make_unique<HcalGain>(id,
                                          gainsIn.getValues(id)->getValue(0) + val,
                                          gainsIn.getValues(id)->getValue(1) + val,
                                          gainsIn.getValues(id)->getValue(2) + val,
                                          gainsIn.getValues(id)->getValue(3) + val);
