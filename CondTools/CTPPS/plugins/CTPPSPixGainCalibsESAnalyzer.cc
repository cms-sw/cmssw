...
CTPPSPixGainCalibsESAnalyzer::analyze(const edm::Event& e, const edm::EventSetup& context){
  ...
  if( recordKey.type() == edm::eventsetup::EventSetupRecordKey::TypeTag()) {
    //record not found
    edm::LogPrint("CTPPSPixGainCalibsReader") <<"Record \"CTPPSPixelGainCalibrationsRcd"<<"\" does not exist ";
  }
  ...
  for  (CTPPSPixelGainCalibrations::CalibMap::const_iterator it = mymap.begin(); it !=mymap.end() ; ++it) {
    ...
    edm::LogPrint("CTPPSPixGainCalibsReader")<<"Address of  detId = "<< (&detId) << " and of it = "<< (&it) << " and of it->first = "<< (&(it->first)) ;
    ...
    edm::LogPrint("CTPPSPixGainCalibsReader")<<"Address of  mycalibs0 = "<< (&mycalibs0) << " and of mycalibs = "<< (&mycalibs) << " and of it->second "<<(&(it->second));
	...
	if(mycalibs.isDead(icol+jrow*ncols))
	  {edm::LogPrint("CTPPSPixGainCalibsReader")<<"Dead Pixel icol ="<< icol << " jrow ="<< jrow ; continue;}
	if(mycalibs.isNoisy(icol+jrow*ncols))
	  {edm::LogPrint("CTPPSPixGainCalibsReader")<<"Noisy Pixel icol ="<< icol << " jrow ="<< jrow ;continue;}
