...
void L1CondDBIOVWriter::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  using namespace edm;
  ...
  if (!m_ignoreTriggerKey) {
    if (!m_tscKey.empty()) {
      edm::LogVerbatim("L1-O2O") << "Object key for L1TriggerKey@L1TriggerKeyRcd: " << m_tscKey;
      ...
      triggerKeyIOVUpdated = m_writer.updateIOV("L1TriggerKeyRcd", keyToken, run, m_logTransactions);
  ...
  if (triggerKeyIOVUpdated || m_forceUpdate) {
    ...
    for (; itr != end; ++itr) {
      ...
      if (objectKey == L1TriggerKey::kNullKey) {
        edm::LogVerbatim("L1-O2O") << "L1CondDBIOVWriter: null object key for " << recordType
        ...
        if (payloadToken.empty()) {
          edm::LogVerbatim("L1-O2O") << "L1CondDBIOVWriter: empty payload token for " + recordType + ", key " +
                                            objectKey;
