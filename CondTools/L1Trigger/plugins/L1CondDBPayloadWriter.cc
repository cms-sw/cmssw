...
void L1CondDBPayloadWriter::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  using namespace edm;
  ...
  if (triggerKeyOK && m_writeL1TriggerKey) {
    edm::LogVerbatim("L1-O2O") << "Object key for L1TriggerKeyRcd@L1TriggerKey: " << key->tscKey();
    token = m_writer.writePayload(iSetup, "L1TriggerKeyRcd@L1TriggerKey");
  }
  ...
  if (!token.empty() || !m_writeL1TriggerKey) {
    ...
    if (m_writeL1TriggerKey) {
      ...
      if (!(keyList->addKey(key->tscKey(), token, m_overwriteKeys))) {
        throw cond::Exception("L1CondDBPayloadWriter: TSC key " + key->tscKey() + " already in L1TriggerKeyList");
      }
    }
...
void L1CondDBPayloadWriter::endJob() {}
