...
void QGLikelihoodSystematicsDBReader::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  edm::LogInfo("UserOutput") << "There are " << data.size()
                             << " entries (categories with parameters for smearing):" << std::endl;
  for (auto idata = data.begin(); idata != data.end(); ++idata) {
    ...
    char buff[1000];
    sprintf(buff,
            "qg=%1d, ptMin=%8.2f, ptMax=%8.2f, etaMin=%3.1f, etaMax=%3.1f, rhoMin=%6.2f, rhoMax=%6.2f, a=%7.3f, "
            "b=%7.3f, lmin=%6.2f, lmax=%6.2f",
            qgBin,
            ptMin,
            ptMax,
            etaMin,
