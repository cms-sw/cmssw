...
void CMSmidpointAlgorithm::findStableConesFromSeeds(JetableObjectHelper& theHelper,
  ...
  CandidateRefVector seedTowers = theHelper.etOrderedCaloTowers(theSeedThreshold);  //This gets towers

  // Loop over all Seeds
  for(candidate_iterator i = seedTowers.begin(); i != seedTowers.end(); ++i) {
...
//
    ...
    while(++nIterations <= theMaxIterations + 1 && keepJet){
      ...
      CandidateRefVector towersInSeedCluster 
	...
	double endE        = endLorentzVector.e();
	  ...
	  for(candidate_iterator i = towersInSeedCluster.begin(); i != towersInSeedCluster.end(); ++i) {
	    const Candidate &t = **i;
  ...
  if(theDebugLevel>=2){
    ...
    for(int i = 0; i < numProtojets ; ++i){
      ...
      cout << endl << "[CMSmidpointAlgorithm] ProtoJet " << i << ": PT=" << stableCones[i].getLorentzVector().perp()
			      ...
			      << ", ntow="<< numTowers << endl;     
      CandidateRefVector protojetTowers = stableCones[i].getTowerList(); 
  ...
  while(mergingNotFinished){
   ...
   sort(stableCones.begin(),stableCones.end(),ProtoJetPtGreater());
	...
	// Calculate overlap of the two cones.
        bool overlap = false;
        CandidateRefVector overlapTowers;  // Make a list to hold the overlap towers
        //cout << "1st cone num towers=" << stableConeIter1->getTowerList().size() << endl;
        //int numTowers1=0;

        //Loop over towers in higher Pt cone
 	for(candidate_iterator towerIter1 = stableConeIter1->getTowerList().begin();
	  ...
	  for(candidate_iterator towerIter2 = stableConeIter2->getTowerList().begin();
	    ...
	    //cout << "2st cone tower " << numTowers2 << endl;
	        ...
	        (abs((*towerIter1)->energy()-(*towerIter2)->energy())<.001)){
	...
	if(overlap){   
	  ...
	  if(overlap.getLorentzVector().perp() >= theOverlapThreshold*stableConeIter2->getLorentzVector().perp()){
	    ...
	    // Merge the two cones.
            ...
            CandidateRefVector stableCone1Towers = stableConeIter1->getTowerList(); 
		...
		++towerIter2){
	      ...
	      for(candidate_iterator overlapTowerIter = overlapTowers.begin();
		  ...
		  ++overlapTowerIter){
	            ...
	            (abs((*overlapTowerIter)->energy()-(*towerIter2)->energy())<.001))
		    isInOverlap = true;
              }
	    ...
	    CandidateRefVector removeFromCone1,removeFromCone2;

	    // Which tower goes where?
            // Loop over the overlap towers
	    for(candidate_iterator towerIter = overlapTowers.begin();
	    ...
	    // Remove towers from proto-jet 1.
            CandidateRefVector towerList1 = stableConeIter1->getTowerList(); 

            // Loop over towers in remove list
	    for(candidate_iterator towerIter = removeFromCone1.begin();
		towerIter != removeFromCone1.end();
		++towerIter)
	    {

                // Loop over towers in protojet
                for(candidate_iterator towerIter1 = towerList1.begin(); towerIter1 != towerList1.end(); ++towerIter1)
		{

                   // Check if they are equal
                   if((abs((*towerIter)->eta()-(*towerIter1)->eta())<.001) && 
	              (abs((*towerIter)->phi()-(*towerIter1)->phi())<.001) &&
	              (abs((*towerIter)->energy()-(*towerIter1)->energy())<.001))
                  {
	    ...
	    // Remove towers from cone 2.
            CandidateRefVector towerList2 = stableConeIter2->getTowerList(); 

            // Loop over towers in remove list
	    for(candidate_iterator towerIter = removeFromCone2.begin();
		towerIter != removeFromCone2.end();
		++towerIter)
	    {
               // Loop over towers in protojet
               for(candidate_iterator towerIter2 = towerList2.begin(); towerIter2 != towerList2.end(); ++towerIter2)
		{
                  // Check if they are equal
                   if((abs((*towerIter)->eta()-(*towerIter2)->eta())<.001) && 
	              (abs((*towerIter)->phi()-(*towerIter2)->phi())<.001) &&
	              (abs((*towerIter)->energy()-(*towerIter2)->energy())<.001))
                  {
                    // Remove the tower
