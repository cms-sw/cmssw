...
namespace {  // anonymous
  ...
  public:
    ...
    ProcTMVA(const char *name, const AtomicId *id, MVATrainer *trainer);
    ~ProcTMVA() override;
  ...
  private:
    ...
    std::string getTreeName() const { return trainer->getName() + '_' + (const char *)getName(); }

    std::string getWeightsFile(const Method &meth, const char *ext) const {
      return "weights/" + getTreeName() + '_' + meth.name + ".weights." + ext;
    }

    enum Iteration { ITER_EXPORT, ITER_DONE } iteration;
  ...
  ProcTMVA::ProcTMVA(const char *name, const AtomicId *id, MVATrainer *trainer)
      : TrainProcessor(name, id, trainer),
        iteration(ITER_EXPORT),
        treeSig(nullptr),
        treeBkg(nullptr),
        needCleanup(false),
        doUserTreeSetup(false),
        setupOptions("SplitMode = Block:!V") {}
  ...
  void ProcTMVA::configure(DOMElement *elem) {
    ...
    for (std::vector<SourceVariable *>::const_iterator iter = inputs.begin(); iter != inputs.end(); iter++) {
      ...
      if (std::find(names.begin(), names.end(), name) != names.end()) {
        for (unsigned i = 1;; i++) {
          ...
          ss << name << "_" << i;
          if (std::find(names.begin(), names.end(), ss.str()) == names.end()) {
            name = ss.str();
            break;
          }
        }
    ...
    for (DOMNode *node = elem->getFirstChild(); node; node = node->getNextSibling()) {
      ...
      if (!isMethod && !isSetup)
        throw cms::Exception("ProcTMVA") << "Expected method or setup tag in config section." << std::endl;
      ...
      if (isMethod) {
        Method method;
        method.type =
            TMVA::Types::Instance().GetMethodType(XMLDocument::readAttribute<std::string>(elem, "type").c_str());

        method.name = XMLDocument::readAttribute<std::string>(elem, "name");

        method.description = (const char *)XMLSimpleStr(node->getTextContent());

        methods.push_back(method);
      } else if (isSetup) {
        if (doUserTreeSetup)
          throw cms::Exception("ProcTMVA") << "Multiple appeareances of setup "
                                              "tag in config section."
                                           << std::endl;

        doUserTreeSetup = true;

        setupCuts = XMLDocument::readAttribute<std::string>(elem, "cuts");
        setupOptions = XMLDocument::readAttribute<std::string>(elem, "options");
      }
    }

    if (methods.empty())
      throw cms::Exception("ProcTMVA") << "Expected TMVA method in config section." << std::endl;
  }

  bool ProcTMVA::load() {
    bool ok = true;
    for (std::vector<Method>::const_iterator iter = methods.begin(); iter != methods.end(); ++iter) {
  ...
  Calibration::VarProcessor *ProcTMVA::getCalibration() const {
    ...
    std::ifstream in(getWeightsFile(methods[0], "xml").c_str(), std::ios::binary | std::ios::in);
    if (!in.good())
      throw cms::Exception("ProcTMVA") << "Weights file " << getWeightsFile(methods[0], "xml")
  ...
  void ProcTMVA::trainBegin() {
    if (iteration == ITER_EXPORT) {
      ...
      file = std::unique_ptr<TFile>(TFile::Open(trainer->trainFileName(this, "root", "input").c_str(), "RECREATE"));
      if (!file.get())
        throw cms::Exception("ProcTMVA") << "Could not open ROOT file for writing." << std::endl;

      file->cd();
      treeSig = new TTree((getTreeName() + "_sig").c_str(), "MVATrainer signal");
      treeBkg = new TTree((getTreeName() + "_bkg").c_str(), "MVATrainer background");
      ...
      for (std::vector<std::string>::const_iterator iter = names.begin(); iter != names.end(); iter++, pos++) {
        treeSig->Branch(iter->c_str(), &*pos, (*iter + "/D").c_str());
        treeBkg->Branch(iter->c_str(), &*pos, (*iter + "/D").c_str());
      }
  ...
  void ProcTMVA::trainData(const std::vector<double> *values, bool target, double weight) {
    if (iteration != ITER_EXPORT)
      return;
  ...
  void ProcTMVA::runTMVATrainer() {
    ...
    if (nSignal < 1 || nBackground < 1)
      throw cms::Exception("ProcTMVA") << "Not going to run TMVA: "
                                          "No signal ("
                                       << nSignal << ") or background (" << nBackground << ") events!" << std::endl;

    std::unique_ptr<TFile> file(TFile::Open(trainer->trainFileName(this, "root", "output").c_str(), "RECREATE"));
    if (!file.get())
      throw cms::Exception("ProcTMVA") << "Could not open TMVA ROOT file for writing." << std::endl;

    std::unique_ptr<TMVA::Factory> factory(new TMVA::Factory(getTreeName().c_str(), file.get(), ""));
    ...
    if (doUserTreeSetup) {
      loader->PrepareTrainingAndTestTree(setupCuts.c_str(), setupOptions);
    } else {
      loader->PrepareTrainingAndTestTree("", 0, 0, 0, 0, "SplitMode=Block:!V");
    }
    ...
    switch (iteration) {
      case ITER_EXPORT:
        /* ROOT context-safe */ {
          ...
          file->Close();
          file.reset();
          file = std::unique_ptr<TFile>(TFile::Open(trainer->trainFileName(this, "root", "input").c_str()));
          if (!file.get())
            throw cms::Exception("ProcTMVA") << "Could not open ROOT file for "
                                                "reading."
                                             << std::endl;
          treeSig = dynamic_cast<TTree *>(file->Get((getTreeName() + "_sig").c_str()));
          treeBkg = dynamic_cast<TTree *>(file->Get((getTreeName() + "_bkg").c_str()));
