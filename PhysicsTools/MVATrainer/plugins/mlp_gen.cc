...
/* Subroutine */ int dgels_(char *trans,
                            int *m,
                            int *n,
                            int *nrhs,
                            double *a,
...
L10:
  ...
  MLP_MatrixVectorBias(NET.vWeights[1], NET.Outn[0], NET.Outn[1], NET.Nneur[1], NET.Nneur[0]);

  for (il = 2; il < NET.Nlayer; il++) {
    MLP_vSigmoideDeriv(NET.Outn[il - 1], deriv1[il - 1], NET.Nneur[il - 1]);
    MLP_MatrixVectorBias(NET.vWeights[il], NET.Outn[il - 1], NET.Outn[il], NET.Nneur[il], NET.Nneur[il - 1]);
  }
  ...
  for (il = 1; il < NET.Nlayer; il++) {
    ...
    for (in = 0; in < NET.Nneur[il]; in++) {
      ...
      if (m == 0)
        goto L20;
      for (j = 1; j <= m; j++)
        a += NET.Weights[il][in][j] * NET.Outn[ilm1][j - 1];
    L20:
      mp1 = m + 1;
      for (j = mp1; j <= NET.Nneur[ilm1]; j += 4) {
        a += NET.Weights[il][in][j + 3] * NET.Outn[ilm1][j + 2] + NET.Weights[il][in][j + 2] * NET.Outn[ilm1][j + 1] +
             NET.Weights[il][in][j + 1] * NET.Outn[ilm1][j] + NET.Weights[il][in][j] * NET.Outn[ilm1][j - 1];
      }
      switch (NET.T_func[il][in]) {
        case 2:
          NET.Outn[il][in] = MLP_Sigmoide(a);
          break;
        case 1:
          NET.Outn[il][in] = a;
          break;
...
L10:
  ...
  MLP_MatrixVectorBias(NET.vWeights[1], NET.Outn[0], NET.Outn[1], nhid, nin);
...
dbl MLP_Test_MM(int ifile, dbl *tmp) {
  ...
  for (ipat = 0; ipat < npat - 1; ipat += 2) {
    MLP_MM2rows(tmp, &(PAT.vRin[ifile][ipat * (nin + 1)]), NET.vWeights[1], 2, nhid, nin + 1, nin + 1, nin + 1);
    ...
    for (jpat = 0; jpat < 2; jpat++) {
      ...
      for (il = 2; il < NET.Nlayer; il++) {
        ...
        for (in = 0; in < NET.Nneur[il]; in++) {
          ...
          if (m == 0)
            goto L20;
          for (j = 1; j <= m; j++, pweights++)
            a += (*pweights) * NET.Outn[ilm1][j - 1];
        L20:
          mp1 = m + 1;
          for (j = mp1; j <= NET.Nneur[ilm1]; j += 4, pweights += 4) {
            a += *(pweights + 3) * NET.Outn[ilm1][j + 2] + *(pweights + 2) * NET.Outn[ilm1][j + 1] +
                 *(pweights + 1) * NET.Outn[ilm1][j] + *(pweights)*NET.Outn[ilm1][j - 1];
          }
          switch (NET.T_func[il][in]) {
            case 2:
              NET.Outn[il][in] = MLP_Sigmoide(a);
        ...
        if (il == NET.Nlayer - 1) {
          for (in = 0; in < NET.Nneur[NET.Nlayer - 1]; in++) {
            ...
            err += (rrans - NET.Outn[NET.Nlayer - 1][in]) * (rrans - NET.Outn[NET.Nlayer - 1][in]) *
  ...
  for (/*'ipat' set above*/; ipat < npat; ipat++) {
    MLP_MatrixVector(NET.vWeights[1], &(PAT.vRin[ifile][ipat * (nin + 1)]), tmp, nhid, nin + 1);
    ...
    for (il = 2; il < NET.Nlayer; il++) {
      ...
      if (il == NET.Nlayer - 1) {
        for (in = 0; in < NET.Nneur[NET.Nlayer - 1]; in++) {
          ...
          err +=
              (rrans - NET.Outn[NET.Nlayer - 1][in]) * (rrans - NET.Outn[NET.Nlayer - 1][in]) * PAT.Pond[ifile][ipat];
        }
      }
    }
  }
  ...
  if (tmp == nullptr) /* not enough memory */
  {
    ...
    for (ipat = 0; ipat < PAT.Npat[ifile]; ipat++) {
      ...
      for (in = 0; in < NET.Nneur[NET.Nlayer - 1]; in++) {
        ...
        err += (rrans - NET.Outn[NET.Nlayer - 1][in]) * (rrans - NET.Outn[NET.Nlayer - 1][in]) * PAT.Pond[ifile][ipati];
      }
    }
    ...
    if (regul >= 1) {
      for (in = 0; in < NET.Nneur[NET.Nlayer - 1]; in++)
        for (jn = 0; jn <= NET.Nneur[NET.Nlayer - 2]; jn++) {
          err += LEARN.Alambda * NET.Weights[NET.Nlayer - 1][in][jn] * NET.Weights[NET.Nlayer - 1][in][jn];
        }
    }
    free(tmp);
    return (err);
  ...
  for (ipat = 0; ipat < PAT.Npat[0]; ipat++) {
    ...
    for (il = NET.Nlayer - 2; il > 0; il--) {
      ...
      for (/*'in' set above*/; in < NET.Nneur[il]; in++) {
        ...
        pdelta = &(NET.Delta[il + 1][1]);
        for (in1 = 1; in1 < NET.Nneur[il + 1]; in1++, pdelta++) {
          a += *pdelta * weights[il + 1][in1][in + 1];
        }
      L2:
    ...
    if (eps == 0) {
      for (il = 1; il < NET.Nlayer; il++) {
        ...
        for (in = 0; in < NET.Nneur[il] - 3; in += 4) {
          ...
          for (in1 = 2; in1 <= inm1; ++in1, ++pout, ++pw1, ++pw2, ++pw3, ++pw4) {
            *pw1 += a1 * *pout;
            *pw2 += a2 * *pout;
            *pw3 += a3 * *pout;
            *pw4 += a4 * *pout;
        ...
        for (/*'in' set above*/; in < NET.Nneur[il]; in++) {
          ...
          pout++;
          pw1 = &(weights[il][in][2]);
          for (in1 = 2; in1 <= inm1; ++in1, ++pout, ++pw1) {
            *pw1 += a1 * *pout;
          }
        }
      }
  ...
  if (LEARN.Meth == 1) {
    ...
    if (LEARN.Meth == 7) {
      if ((iepoch - 1) % LEARN.Nreset == 0) {
        ...
        if (NET.Debug >= 5)
          printf("Before GetGammaDelta \n");
        GetGammaDelta();
        if (NET.Debug >= 5)
          printf("After GetGammaDelta \n");
        ierr = GetBFGSH(Nweights - Nlinear);
        if (NET.Debug >= 5)
          printf("After GetBFGSH \n");
        if (ierr) {
          SteepestDir();
          InitBFGSH(Nweights - Nlinear);
        } else {
...
/* extern "C"Dllexport */ int MLP_Train(int *ipat, dbl *err) {
  ...
  for (in = 0; in < NET.Nneur[NET.Nlayer - 1]; in++) {
    *err += ((dbl)PAT.Rans[0][*ipat][in] - NET.Outn[NET.Nlayer - 1][in]) *
            ((dbl)PAT.Rans[0][*ipat][in] - NET.Outn[NET.Nlayer - 1][in]) * PAT.Pond[0][*ipat];
  }
  ...
  for (il = NET.Nlayer - 2; il > 0; il--) {
    for (in = 0; in < NET.Nneur[il]; in++) {
      ...
      for (in1 = 0; in1 <= NET.Nneur[il - 1]; in1++) {
        LEARN.Odw[il][in][in1] = -eta * LEARN.DeDw[il][in][in1] + eps * LEARN.Odw[il][in][in1];
      }
...
/* extern "C"Dllexport */ int StochStep() {
  ...
  for (il = NET.Nlayer - 1; il > 0; il--) {
    for (in1 = 0; in1 <= NET.Nneur[il - 1]; in1++) {
      /* compute delta weights */
      for (in = 0; in < NET.Nneur[il]; in++) {
        LEARN.Odw[il][in][in1] = eta * (LEARN.DeDw[il][in][in1] + epseta * LEARN.Odw[il][in][in1]);
        NET.Weights[il][in][in1] += LEARN.Odw[il][in][in1];
      }
    }
  }
  ...
  for (il = 1; il < NET.Nlayer; il++)
    for (in = 0; in < NET.Nneur[il]; in++)
      for (jn = 0; jn <= NET.Nneur[il - 1]; jn++)
        dd += LEARN.DeDw[il][in][jn] * LEARN.DeDw[il][in][jn];
  return (dd);
}
...
/* extern "C"Dllexport */ dbl DeDwProd() {
  ...
  for (il = 1; il < NET.Nlayer; il++)
    for (in = 0; in < NET.Nneur[il]; in++)
      for (jn = 0; jn <= NET.Nneur[il - 1]; jn++)
        dd += LEARN.DeDw[il][in][jn] * LEARN.ODeDw[il][in][jn];
  return (dd);
}
...
/* extern "C"Dllexport */ int DeDwSum(type_pat *ans, dbl *out, int ipat) {
  ...
  for (in = 0; in < NET.Nneur[NET.Nlayer - 1]; in++) {
    deriv = NET.Deriv1[NET.Nlayer - 1][in];
    NET.Delta[NET.Nlayer - 1][in] = (out[in] - (dbl)ans[in]) * deriv * b;
  }
...
/* extern "C"Dllexport */ int SetTransFunc(int layer, int neuron, int func) {
  if (layer > NLMAX)
    return (1);
...
/* extern "C"Dllexport */ void CGDir(dbl beta) {
  ...
  for (il = 1; il < NET.Nlayer; il++)
    for (in = 0; in < NET.Nneur[il]; in++)
      for (jn = 0; jn <= NET.Nneur[il - 1]; jn++) {
        dir[il][in][jn] = -LEARN.DeDw[il][in][jn] + beta * dir[il][in][jn];
      }
}
...
/* extern "C"Dllexport */ void GetGammaDelta() {
  ...
  for (il = 1; il < NET.Nlayer; il++)
    for (in = 0; in < NET.Nneur[il]; in++)
      for (jn = 0; jn <= NET.Nneur[il - 1]; jn++) {
        Gamma[i] = LEARN.DeDw[il][in][jn] - LEARN.ODeDw[il][in][jn];
...
/* extern "C"Dllexport */ int GetBFGSH(int Nweights) {
  ...
  for (i = 0; i < Nweights; i++) {
    b = (dble)delta[i];
    for (j = 0; j < Nweights; j++)
      BFGSH[i][j] += (dbl)(factor * b * (dble)delta[j] - (tmp[j] * b + Hgamma[i] * (dble)delta[j])) * a;
  }
  free(Hgamma);
  free(tmp);
  return 0;
...
int LineSearch(dbl *alpmin, int *Ntest, dbl Err0) {
  ...
  *Ntest = 0;
  ...
  for (il = 1; il < NET.Nlayer; il++) {
    ...
    for (in = 0; in < NET.Nneur[il]; in++) {
      w0[il][in] = (dbl *)malloc((NET.Nneur[il - 1] + 1) * sizeof(dbl));
      for (jn = 0; jn <= NET.Nneur[il - 1]; jn++) {
  ...
  *alpmin =
      0.5 * (alpha1 + alpha3 - (err3 - err1) / ((err3 - err2) / (alpha3 - alpha2) - (err2 - err1) / (alpha2 - alpha1)));
  if (*alpmin > 10000)
    *alpmin = 10000;
  ...
  for (il = 1; il < NET.Nlayer; il++)
    for (in = 0; in < NET.Nneur[il]; in++)
      for (jn = 0; jn <= NET.Nneur[il - 1]; jn++)
        LEARN.Odw[il][in][jn] = NET.Weights[il][in][jn] - w0[il][in][jn];
...
void MLP_Line(dbl ***w0, dbl alpha) {
  ...
  for (il = 1; il < NET.Nlayer; il++)
    for (in = 0; in < NET.Nneur[il]; in++)
      for (jn = 0; jn <= NET.Nneur[il - 1]; jn++)
        NET.Weights[il][in][jn] = w0[il][in][jn] + alpha * dir[il][in][jn];
}
...
int LineSearchHyb(dbl *alpmin, int *Ntest) {
  ...
  *Ntest = 0;
  ...
  (*Ntest)++;
  if (NET.Debug >= 4)
    printf("LinesearchHyb err depart= %f\n", err1);
...
void SetLambda(double Wmax) {
  dbl err;
  err = MLP_Test(0, 0);
  LEARN.Alambda = LEARN.Lambda * err / (Wmax * Wmax * (dbl)(NET.Nneur[NET.Nlayer - 2] + 1));
}
...
void MLP_ResLin() {
  ...
  for (ipat = 0; ipat < PAT.Npat[0]; ipat++) {
    ...
    for (in = 0; in < NET.Nneur[il]; in++) {
      khr = M * (in + 1) + ipat;
      HR[khr] = NET.Outn[il][in] * (dbl)sqrt(PAT.Pond[0][ipat]);
    }
  }
  il = NET.Nlayer - 2;
  lambda = sqrt(lambda2);
  ...
  /*                                                                */
  nrhs = 1;
  ierr = dgels_(&Trans, &M, &Nl, &nrhs, HR, &M, dpat, &M, Work, &Lwork, &iret);
  if (iret != 0)
    printf("Warning from dgels: iret = %d\n", (int)iret);
  if (ierr != 0)
    printf("Warning from dgels: ierr = %d\n", (int)ierr);
...
void EtaDecay() { LEARN.eta *= LEARN.Decay; }
...
/* extern "C"Dllexport */
double MLP_Rand(dbl mini, dbl maxi) { return mini + (maxi - mini) * random() / RAND_MAX; }
  ...
  for (ilayer = 1; ilayer < NET.Nlayer; ilayer++)
    for (ineur = 0; ineur < NET.Nneur[ilayer]; ineur++)
      for (i = 0; i <= NET.Nneur[ilayer - 1]; i++)
        NET.Weights[ilayer][ineur][i] = (dbl)MLP_Rand(-0.5, 0.5);
}
...
void PrintWeights() {
  ...
  for (ilayer = 1; ilayer < NET.Nlayer; ilayer++) {
    ...
    for (ineur = 0; ineur < NET.Nneur[ilayer]; ineur++) {
      ...
      for (i = 0; i <= NET.Nneur[ilayer - 1]; i++) {
        printf(" %f", (double)NET.Weights[ilayer][ineur][i]);
      }
      printf("\n");
    }
    printf("\n");
    ...
    if (*s == 'N') {
      if (*(s + 1) == 'N') /* NNEU */
      {
        ...
        *inet = 1;
        ...
        {
          LEARN.Nreset = l;
          printf("Reset to steepest descent every %d epochs\n", l);
        }
      }
    } else if (*s == 'L') {
      if (*(s + 1) == 'P') /* LPAR */
        ...
        *ilearn = 1;
        ...
        switch (LEARN.Meth) {
          case 1:
            ...
            break;
          case 2:
            printf("Steepest descent with fixed step\n");
            break;
          case 3:
            printf("Steepest descent with line search\n");
            break;
          case 4:
            printf("Polak-Ribiere Conjugate Gradients\n");
            break;
          case 5:
    ...
    } else if (*s == 'O') /* OUTx */
    {
      ...
      } else if (*(s + 3) == 'F') /* OUTF */
      {
        ...
        if (*cc2 == 'F' || *cc2 == 'C') {
          ...
          printf(" *** Error while loading file %s at line %s :", filename, s);
      ...
      } else /* answers */
      {
        ...
        for (i = 0; i < ntot; i++) {
          ...
          if (i < nout) {
            ...
            if (PAT.Iponde == 1) {
              PAT.Pond[ifile][ipat] = (type_pat)p;
            } else {
              PAT.Pond[ifile][ipat] = (type_pat)PAT.Ponds[(int)p - 1];
            }
          }
        }
      }
...
int CountLexemes(char *s) {
  ...
  if (strtok_r(tmp, " ", &saveptr)) {
    i = 1;
    while (strtok_r(nullptr, " ", &saveptr))
      i++;
  }
  return i;
}
  ...
  for (il = 0; il < NET.Nlayer; il++) {
    ...
    for (in = 0; in < NET.Nneur[il]; in++) {
      if (il == 0) {
        ...
        dir[0][in] = (dbl *)malloc(101 * sizeof(dbl));
        if (dir[0][in] == nullptr)
          return -111;
      } else {
        dir[il][in] = (dbl *)malloc((NET.Nneur[il - 1] + 1) * sizeof(dbl));
        if (dir[il][in] == nullptr)
          return -111;
...
int MLP_PrFFun(char *filename) {
  ...
  for (in = 0; in < NET.Nneur[0]; in++) {
    if (DIVERS.Norm == 0) {
      ...
      fprintf(W, "      OUT%d = (RIN(%d)-%e)/%e\n", in + 1, in + 1, STAT.mean[in], STAT.sigma[in]);
    }
  }
  for (il = 1; il < NET.Nlayer - 1; il++) {
    fprintf(W, "C\n");
    fprintf(W, "C     layer %d\n", il + 1);
    for (in = 0; in < NET.Nneur[il]; in++) {
      fprintf(W, "      RIN%d = %e\n", in + 1, (double)NET.Weights[il][in][0]);
...
int MLP_PrCFun(char *filename) {
  ...
  if (NET.Nlayer >= 3)
    fprintf(W, "      double out3[%d];\n", NET.Nneur[2]);
  if (NET.Nlayer >= 4)
    fprintf(W, "      double out4[%d];\n", NET.Nneur[3]);
  fprintf(W, "\n");
  ...
  for (in = 0; in < NET.Nneur[0]; in++) {
    if (DIVERS.Norm == 0) {
      ...
      fprintf(W, "      out1[%d] = (rin[%d]-%e)/%e;\n", in, in, STAT.mean[in], STAT.sigma[in]);
    }
  }
  ...
  for (il = 1; il <= NET.Nlayer - 1; il++) {
    ...
    fprintf(W, "/*     layer %d */\n", il + 1);
    for (in = 0; in < NET.Nneur[il]; in++) {
      fprintf(W, "      out%d[%d] = %e\n", il + 1, in, (double)NET.Weights[il][in][0]);
      for (jn = 1; jn <= NET.Nneur[il - 1]; jn++)
...
/* extern "C"Dllexport */ int AllocPatterns(int ifile, int npat, int nin, int nout, int iadd) {
  ...
  if (ExamplesMemory == 0) {
    ...
    if (PAT.Pond == nullptr || PAT.Rin == nullptr || PAT.Rans == nullptr || PAT.vRin == nullptr)
      return -111;
  }
  ...
  if (iadd == 0 || PatMemory[ifile] == 0) {
    ...
    if (PAT.Pond[ifile] == nullptr)
      return -111;
    for (j = 0; j < npat; j++)
      PAT.Pond[ifile][j] = 1;

    PAT.Rin[ifile] = (type_pat **)malloc(npat * sizeof(type_pat *));
    if (PAT.Rin[ifile] == nullptr)
      return -111;
    PAT.Rans[ifile] = (type_pat **)malloc(npat * sizeof(type_pat *));
    if (PAT.Rans[ifile] == nullptr)
      return -111;

    PAT.vRin[ifile] = (type_pat *)malloc(npat * (nin + 1) * sizeof(type_pat));
    if (PAT.vRin[ifile] == nullptr)
      return -111;
...
int MLP_StatInputs(int Nexamples, int Ninputs, type_pat **inputs, dbl *mean, dbl *sigma, dbl *minimum, dbl *maximum) {
  ...
  for (j = 0; j < Ninputs; j++) {
    ...
    for (ipat = 0; ipat < Nexamples; ipat++) {
      ...
      sigma[j] += ((dbl)inputs[ipat][j] - fmean[j]) * ((dbl)inputs[ipat][j] - fmean[j]);
    ...
    sigma[j] = sqrt(sigma[j] / (dbl)Nexamples - (mean[j] - fmean[j]) * (mean[j] - fmean[j]));
  }
  free(fmean);
  return 0;
}
...
int MLP_PrintInputStat() {
  ...
  if (mean && sigma && minimum && maximum) {
    ...
    for (j = 0; j < NET.Nneur[0]; j++) {
      printf("var%d \t %e \t %e \t %e \t %e\n", j + 1, mean[j], sigma[j], minimum[j], maximum[j]);
    }
...
/* extern "C"Dllexport */ int NormalizeInputs() {
  ...
  if (mean && sigma && minimum && maximum && STAT.mean && STAT.sigma) {
    ...
    for (j = 0; j < NET.Nneur[0]; j++) {
      ...
      for (ipat = 0; ipat < PAT.Npat[0]; ipat++) {
        PAT.Rin[0][ipat][j] = (PAT.Rin[0][ipat][j] - (float)mean[j]) / (float)sigma[j];
      }
      for (ipat = 0; ipat < PAT.Npat[1]; ipat++) {
        PAT.Rin[1][ipat][j] = (PAT.Rin[1][ipat][j] - (float)mean[j]) / (float)sigma[j];
      }
    }
    returnCode = 0;  // everything went fine
  }
...
int AllocNetwork(int Nlayer, int *Neurons) {
  ...
  if (NET.T_func == nullptr || NET.Deriv1 == nullptr || NET.Inn == nullptr || NET.Outn == nullptr ||
      NET.Delta == nullptr)
    return -111;
  ...
  for (i = 0; i < Nlayer; i++) {
    ...
    if (NET.T_func[i] == nullptr || NET.Deriv1[i] == nullptr || NET.Inn[i] == nullptr || NET.Outn[i] == nullptr ||
        NET.Delta[i] == nullptr)
      return -111;
  }
  ...
  if (NET.Weights == nullptr || NET.vWeights == nullptr || LEARN.Odw == nullptr || LEARN.ODeDw == nullptr ||
      LEARN.DeDw == nullptr)
    return -111;

  for (i = 1; i < Nlayer; i++) {
    k = Neurons[i - 1] + 1;
    NET.vWeights[i] = (dbl *)malloc(k * Neurons[i] * sizeof(dbl));
    ...
    if (NET.Weights[i] == nullptr || NET.vWeights[i] == nullptr || LEARN.Odw[i] == nullptr ||
        LEARN.ODeDw[i] == nullptr || LEARN.DeDw[i] == nullptr)
      return -111;
    ...
    for (j = 0; j < Neurons[i]; j++) {
      ...
      if (LEARN.Odw[i][j] == nullptr || LEARN.ODeDw[i][j] == nullptr || LEARN.DeDw[i][j] == nullptr)
        return -111;
...
void MLP_MatrixVectorBias(dbl *M, dbl *v, dbl *r, int n, int m) {
  ...
  for (i = 0; i < n - 3; i += 4, pM1 += 3 * mp1, pM2 += 3 * mp1, pM3 += 3 * mp1, pM4 += 3 * mp1, pr += 4) {
    a1 = *pM1;
    a2 = *pM2;
    a3 = *pM3;
    a4 = *pM4;
...
void MLP_MM2rows(dbl *c, type_pat *a, dbl *b, int Ni, int Nj, int Nk, int NaOffs, int NbOffs) {
  ...
  for (j = 0; j <= Nj - 2; j += 2) {
    ...
    for (k = 0, pb0 = b + k + NbOffs * j, pb1 = b + k + NbOffs * (j + 1), pa0 = a + k, pa1 = a + k + NaOffs; k < Nk;
         k++, pa0++, pa1++, pb0++, pb1++) {
      s00 += (*pa0) * (*pb0);
      s01 += (*pa0) * (*pb1);
      s10 += (*pa1) * (*pb0);
  ...
  for (/*j set above*/; j < Nj; j++) {
    ...
    s00 = 0.0;
    s10 = 0.0;
    for (k = 0, pb0 = b + k + NbOffs * j, pa0 = a + k, pa1 = a + k + NaOffs; k < Nk; k++, pa0++, pa1++, pb0++) {
      s00 += (*pa0) * (*pb0);
      s10 += (*pa1) * (*pb0);
    }
    *pc0 = s00;
