...
extern struct stat_ { dbl *mean, *sigma; } stat_ MLP_HIDDEN;
#define STAT stat_
...
extern int MLP_StatInputs(
    int Nexamples, int Ninputs, type_pat **inputs, dbl *mean, dbl *sigma, dbl *minimum, dbl *maximum) MLP_HIDDEN;
...
extern int ReadPatterns(char *filename, int ifile, int *inet, int *ilearn, int *iexamples) MLP_HIDDEN;
...
extern void MLP_MM2rows(dbl *c, type_pat *a, dbl *b, int Ni, int Nj, int Nk, int NaOffs, int NbOffs) MLP_HIDDEN;
extern void MLP_MatrixVector(dbl *M, type_pat *v, dbl *r, int n, int m) MLP_HIDDEN;
extern void MLP_MatrixVectorBias(dbl *M, dbl *v, dbl *r, int n, int m) MLP_HIDDEN;
