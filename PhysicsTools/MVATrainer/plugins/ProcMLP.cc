...
namespace {  // anonymous
  ...
  public:
    ...
    ProcMLP(const char *name, const AtomicId *id, MVATrainer *trainer);
    ~ProcMLP() override;
  ...
  private:
    ...
    enum Iteration { ITER_COUNT, ITER_TRAIN, ITER_WAIT, ITER_DONE } iteration;
  ...
  ProcMLP::ProcMLP(const char *name, const AtomicId *id, MVATrainer *trainer)
  ...
  void ProcMLP::configure(DOMElement *elem) {
    ...
    if (!node)
      throw cms::Exception("ProcMLP") << "Expected MLP config in config section." << std::endl;

    if (std::strcmp(XMLSimpleStr(node->getNodeName()), "config") != 0)
      throw cms::Exception("ProcMLP") << "Expected config tag in config section." << std::endl;
    ...
    if (node)
      throw cms::Exception("ProcMLP") << "Superfluous tags in config section." << std::endl;
  ...
  Calibration::VarProcessor *ProcMLP::getCalibration() const {
    ...
    std::unique_ptr<MLP> mlp(new MLP(getInputs().size() - (boost >= 0 ? 1 : 0), getOutputs().size(), layout));
    mlp->load(trainer->trainFileName(this, "txt"));

    std::string fileName = trainer->trainFileName(this, "txt");
    std::ifstream in(fileName.c_str(), std::ios::binary | std::ios::in);
    if (!in.good())
      throw cms::Exception("ProcMLP") << "Weights file " << fileName << "cannot be opened for reading." << std::endl;
  ...
  void ProcMLP::trainBegin() {
    ...
    switch (iteration) {
      ...
      case ITER_TRAIN:
        try {
          mlp = std::unique_ptr<MLP>(new MLP(getInputs().size() - (boost >= 0 ? 1 : 0), getOutputs().size(), layout));
          mlp->init(count);
          row = 0;
  ...
  void ProcMLP::trainData(const std::vector<double> *values, bool target, double weight) {
    if (boost >= 0) {
  ...
  void ProcMLP::runMLPTrainer() {
    for (unsigned int i = 0; i < steps; i++) {
      ...
      if ((i % 10) == 0)
        std::cout << "Training MLP epoch " << mlp->getEpoch() << ", rel chi^2: " << (error / weightSum) << std::endl;
    }
  }
