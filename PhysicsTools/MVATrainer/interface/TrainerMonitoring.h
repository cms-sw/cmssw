...
namespace PhysicsTools {
  ...
  protected:
    ...
    public:
      RootObject(const std::string &name, T *object) : Object(name), object(object) {}
      ~RootObject() override {}

      void write(TDirectory *dir) override {
        dir->WriteObjectAny(object.get(), TClass::GetClass(typeid(T)), getName().c_str());
      }
  ...
  inline void TrainerMonitoring::Module::reg(const std::string &name, T *object) {
    helper::trainerMonitoringRootClear(object,
                                       boost::mpl::or_<boost::is_base_of<TH1, T>, boost::is_base_of<TTree, T> >());
