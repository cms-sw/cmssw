...
int main(int argc, char **argv) {
  ...
  argc--;
  while (argc > 0 && **args == '-') {
    if (!std::strcmp(*args, "-l") || !std::strcmp(*args, "--load"))
      load = true;
    else if (!std::strcmp(*args, "-s") || !std::strcmp(*args, "--no-save"))
      save = false;
    else if (!std::strcmp(*args, "-m") || !std::strcmp(*args, "--no-monitoring"))
      monitoring = false;
    else if (!std::strcmp(*args, "-w") || !std::strcmp(*args, "--no-weights"))
      weights = false;
    else if (!std::strncmp(*args, "--xslt=", 7)) {
      useXSLT = true;
      styleSheet = *args + 7;
    } else if (!std::strcmp(*args, "-x") || !std::strcmp(*args, "--xslt"))
      useXSLT = true;
    else if (!std::strcmp(*args, "-v") || !std::strcmp(*args, "--cross-validation")) {
      args++;
      argc--;
      if (argc < 1) {
        std::cerr << "Option " << *args << " needs a parameter." << std::endl;
        continue;
      }
      std::istringstream ss(*args);
      ss >> crossValidation;
      if (!(crossValidation > 0.0 && crossValidation < 1.0)) {
        crossValidation = -1.0;
        std::cerr << "Option " << args[-1] << " has an invalid argument." << std::endl;
        continue;
      }
    } else
      std::cerr << "Unsupported option " << *args << "." << std::endl;
    args++;
    argc--;
  }
  ...
  try {
    ...
    if (nTarget == 0 && trees.size() == 2)
      treeTrainer.reset(new TreeTrainer(trees[0], trees[1], weights ? -1.0 : 1.0));
    else if (nTarget == trees.size()) {
      treeTrainer.reset(new TreeTrainer());
      for (std::vector<TTree *>::iterator iter = trees.begin(); iter != trees.end(); ++iter)
        treeTrainer->addTree(*iter, -1, weights ? -1.0 : 1.0);
    } else
    ...
    std::unique_ptr<Calibration::MVAComputer> calib(trainer.getCalibration());
