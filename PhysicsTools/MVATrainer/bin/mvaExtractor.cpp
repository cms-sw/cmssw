...
int main(int argc, char **argv) {
  ...
  try {
    ...
    if (!calib) {
      std::cerr << "MVA calibration could not be read." << std::endl;
      return 1;
    }
    ...
    for (std::vector<Calibration::Variable>::const_iterator iter = calib->inputSet.begin();
         ...
         ++iter) {
      out << "\t\t<var name=\"" << escape(iter->name) << "\" multiple=\"true\" optional=\"true\"/>" << std::endl;
    ...
    std::vector<Calibration::VarProcessor *> procs = calib->getProcessors();
    for (std::vector<Calibration::VarProcessor *>::const_iterator iter = procs.begin(); iter != procs.end(); ++iter) {
      std::string name = (*iter)->getInstanceName();

      PhysicsTools::BitSet in = PhysicsTools::Calibration::convert((*iter)->inputVars);

      int j = 0;
      for (unsigned int i = vars.size(); i < in.size(); i++) {
        std::string var = mkNumber("var", ++j);
        out << "\t\t\t<var name=\"" << var << "\"/>" << std::endl;
      ...
      out << "\t<processor id=\"" << proc << "\" name=\"" << name << "\">" << std::endl;
      out << "\t\t<input>" << std::endl;

      for (unsigned int i = 0; i < in.size(); i++) {
        if (in[i])
          out << "\t\t\t<var source=\"" << escape(vars.at(i).first) << "\" name=\"" << escape(vars.at(i).second)
              << "\"/>" << std::endl;
      }
    ...
    out << "\t<output>" << std::endl;
    out << "\t\t<var source=\"" << escape(vars.at(calib->output).first) << "\" name=\""
        << escape(vars.at(calib->output).second) << "\"/>" << std::endl;
    out << "\t</output>" << std::endl;
