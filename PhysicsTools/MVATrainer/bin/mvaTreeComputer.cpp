...
int main(int argc, char **argv) {
  ...
  try {
    ...
    for (std::vector<TTree *>::const_iterator iter = trees.begin(); iter != trees.end(); ++iter) {
      TTree *tree = *iter;

      if (!outTree) {
        outTree = tree->CloneTree(0);
        outTree->SetDirectory(outFile);
        discrBranch = outTree->Branch("__DISCR__", &discr, "__DISCR__/D");
      }

      TTreeCloner cloner(tree, outTree, "");
      if (!cloner.IsValid()) {
        std::cerr << "Tree cloner is invalid." << std::endl;
        return 1;
      }
