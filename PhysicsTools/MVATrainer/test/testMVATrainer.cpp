...
static double gauss() {
  return std::sqrt(-2.0 * std::log((random() + 0.5) / RAND_MAX)) * std::cos(random() * (2 * M_PI / RAND_MAX));
}
...
static void train(MVAComputer *computer) {
  ...
  for (unsigned int i = 0; i < 20000; i++) {
    ...
    Variable::Value sig[] = {
        Variable::Value(MVATrainer::kTargetId, true), Variable::Value(idX, x), Variable::Value(idY, y)};

    x = -1 + 2 * gauss();
    y = -2 + 2 * gauss();

    Variable::Value bkg[] = {
        Variable::Value(MVATrainer::kTargetId, false), Variable::Value(idX, x), Variable::Value(idY, y)};
...
void test() {
  ...
  for (;;) {
    std::unique_ptr<Calibration::MVAComputer> calib(trainer.getTrainCalibration());

    if (!calib.get())
      break;

    std::unique_ptr<MVAComputer> computer(new MVAComputer(calib.get()));
