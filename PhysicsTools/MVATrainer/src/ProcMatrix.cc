...
namespace {  // anonymous
  ...
  public:
    ...
    ProcMatrix(const char *name, const AtomicId *id, MVATrainer *trainer);
    ~ProcMatrix() override;
  ...
  ProcMatrix::ProcMatrix(const char *name, const AtomicId *id, MVATrainer *trainer)
  ...
  void ProcMatrix::configure(DOMElement *elem) {
    ...
    if (std::strcmp(XMLSimpleStr(node->getNodeName()), "fill") != 0)
      throw cms::Exception("ProcMatrix") << "Expected fill tag in config section." << std::endl;

    elem = static_cast<DOMElement *>(node);

    fillSignal = XMLDocument::readAttribute<bool>(elem, "signal", false);
    fillBackground = XMLDocument::readAttribute<bool>(elem, "background", false);
    doNormalization = XMLDocument::readAttribute<bool>(elem, "normalize", false);
    ...
    if (node)
      throw cms::Exception("ProcMatrix") << "Superfluous tags in config section." << std::endl;

    if (!fillSignal && !fillBackground)
      throw cms::Exception("ProcMatrix") << "Filling neither background nor signal in config." << std::endl;
  }
  ...
  void ProcMatrix::trainData(const std::vector<double> *values, bool target, double weight) {
    if (iteration != ITER_FILL)
      return;
    ...
    for (unsigned int i = 0; i < ls->getSize(); i++, values++) {
      if (values->empty())
        throw cms::Exception("ProcMatrix") << "Variable \"" << (const char *)getInputs().get()[i]->getName()
                                           << "\" is not set in ProcMatrix trainer." << std::endl;
      vars[i] = values->front();
    }
  ...
  void ProcMatrix::trainEnd() {
    switch (iteration) {
      case ITER_FILL:
        ...
        if (lsSignal.get()) {
          unsigned int n = ls->getSize();
          double weight = lsSignal->getCoefficients()(n + 1, n + 1);
          if (weight > 1.0e-9)
        ...
        if (lsBackground.get()) {
          unsigned int n = ls->getSize();
          double weight = lsBackground->getCoefficients()(n + 1, n + 1);
          if (weight > 1.0e-9)
    ...
    if (iteration == ITER_DONE && monitoring) {
      ...
      histo->SetNameTitle("CorrMatrix",
                          (fillSignal && fillBackground)
                              ...
                              : (fillSignal ? "correlation matrix (signal)" : "correlation matrix (background)"));
      ...
      for (std::vector<SourceVariable *>::const_iterator iter = inputs.begin(); iter != inputs.end(); ++iter) {
        ...
        histo->GetXaxis()->SetBinLabel(inputs.size() + 1, "target");
        histo->GetYaxis()->SetBinLabel(inputs.size() + 1, "target");
      }
      ...
      for (unsigned int i = 0; i < ranks.size(); i++) {
        ...
        name = (const char *)var->getSource()->getName() + std::string("_") + (const char *)var->getName();
  ...
  bool ProcMatrix::load() {
    ...
    if (std::strcmp(XMLSimpleStr(elem->getNodeName()), "ProcMatrix") != 0)
      throw cms::Exception("ProcMatrix") << "XML training data file has bad root node." << std::endl;
    ...
    if (!node)
      throw cms::Exception("ProcMatrix") << "Train data file empty." << std::endl;
    ...
    if (node)
      throw cms::Exception("ProcMatrix") << "Train data file contains superfluous tags." << std::endl;
  ...
  double targetCorrelation(const TMatrixDSym &coeffs, const std::vector<bool> &use) {
