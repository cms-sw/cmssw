...
namespace {  // anonymous
  ...
  public:
    ...
    ProcMultiply(const char *name, const AtomicId *id, MVATrainer *trainer);
  ...
  ProcMultiply::ProcMultiply(const char *name, const AtomicId *id, MVATrainer *trainer)
  ...
  void ProcMultiply::configure(DOMElement *elem) {
    ...
    for (DOMNode *node = elem->getFirstChild(); node; node = node->getNextSibling()) {
      ...
      if (std::strcmp(XMLSimpleStr(node->getNodeName()), "product") != 0)
        throw cms::Exception("ProcMultiply") << "Expected product tag in config section." << std::endl;

      std::string data = (const char *)XMLSimpleStr(node->getTextContent());
      ...
      for (size_t pos = 0, next = 0; next != std::string::npos; pos = next + 1) {
        ...
        if (ss.bad() || ss.peek() != std::istringstream::traits_type::eof())
          throw cms::Exception("ProcMultiply") << "Expected list of indices separated"
                                               << "by asterisks" << std::endl;
        if (index >= nInputs)
          throw cms::Exception("ProcMultiply") << "Variable index " << index << " out of range." << std::endl;
