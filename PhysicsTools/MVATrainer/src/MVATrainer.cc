...
namespace PhysicsTools {
  ...
  namespace {  // anonymous
    ...
    public:
      ...
      inline void setCalibration(MVATrainerComputer *calib) { this->calib = calib; }

      std::vector<Variable::Flags> configure(const MVAComputer *computer,
                                             unsigned int n,
                                             const std::vector<Variable::Flags> &flags) override = 0;

      double intercept(const std::vector<double> *values) const override = 0;
    ...
    public:
      ...
      double intercept(const std::vector<double> *values) const override;
    };
    ...
    public:
      ...
      MVATrainerComputer(const std::vector<Interceptor> &interceptors, bool autoSave, UInt_t seed, double split);

      ~MVATrainerComputer() override;

      std::vector<Calibration::VarProcessor *> getProcessors() const override;
      void initFlags(std::vector<Variable::Flags> &flags) const override;

      void configured(BaseInterceptor *interceptor) const;
      void next();
      void done();

      inline void addFlag(Variable::Flags flag) { flags.push_back(flag); }

      inline bool useForTraining() const { return splitResult; }
      inline bool useForTesting() const { return split <= 0.0 || !splitResult; }

      inline bool isConfigured() const { return nConfigured == interceptors.size(); }
    ...
    template <typename T>
    struct auto_cleaner {
      inline ~auto_cleaner() { std::for_each(clean.begin(), clean.end(), deleter<T>); }
  ...
  std::vector<Variable::Flags> InitInterceptor::configure(const MVAComputer *computer,
                                                          unsigned int n,
  ...
  double InitInterceptor::intercept(const std::vector<double> *values) const {
    calib->next();
    return 0.0;
  }
  ...
  std::vector<Variable::Flags> TrainInterceptor::configure(const MVAComputer *computer,
                                                           unsigned int n,
                                                           const std::vector<Variable::Flags> &flags) {
    const SourceVariableSet &inputSet = const_cast<const TrainProcessor *>(proc)->getInputs();
  ...
  void TrainInterceptor::init() {
    edm::LogInfo("MVATrainer") << "TrainProcessor \"" << (const char *)proc->getName()
                               << "\" training iteration starting...";

    proc->doTrainBegin();
  }

  double TrainInterceptor::intercept(const std::vector<double> *values) const {
    if (values[targetIdx].size() != 1) {
      if (values[targetIdx].empty())
        throw cms::Exception("MVATrainer") << "Trainer input lacks target variable." << std::endl;
      else
        throw cms::Exception("MVATrainer") << "Multiple targets supplied in input." << std::endl;
    }
    double target = values[targetIdx].front();

    double weight = 1.0;
    if (values[weightIdx].size() > 1)
      throw cms::Exception("MVATrainer") << "Multiple weights supplied in input." << std::endl;
    else if (values[weightIdx].size() == 1)
      weight = values[weightIdx].front();

    if (tmp.empty())
      proc->doTrainData(values + 2, target > 0.5, weight, calib->useForTraining(), calib->useForTesting());
    else {
      ...
      proc->doTrainData(&tmp.front(), target > 0.5, weight, calib->useForTraining(), calib->useForTesting());
    }
  ...
  void TrainInterceptor::finish(bool save) {
    ...
    edm::LogInfo("MVATrainer") << "... processor \"" << (const char *)proc->getName() << "\" training iteration done.";

    if (proc->isTrained()) {
      edm::LogInfo("MVATrainer") << "* Completed training of \"" << (const char *)proc->getName() << "\".";
  ...
  MVATrainerComputer::MVATrainerComputer(const std::vector<Interceptor> &interceptors,
                                         bool autoSave,
                                         UInt_t seed,
                                         double split)
      : interceptors(interceptors), nConfigured(0), doAutoSave(autoSave), random(seed), split(split) {
    for (std::vector<Interceptor>::const_iterator iter = interceptors.begin(); iter != interceptors.end(); ++iter)
  ...
  std::vector<Calibration::VarProcessor *> MVATrainerComputer::getProcessors() const {
    std::vector<Calibration::VarProcessor *> processors = Calibration::MVAComputer::getProcessors();

    for (std::vector<Interceptor>::const_iterator iter = interceptors.begin(); iter != interceptors.end(); ++iter)

      processors.insert(processors.begin() + iter->first, 1, iter->second);
  ...
  void MVATrainerComputer::next() { splitResult = random.Uniform(1.0) >= split; }
  ...
  static std::string escape(const std::string &in) {
    std::string result("'");
    for (std::string::const_iterator iter = in.begin(); iter != in.end(); ++iter) {
      switch (*iter) {
        case '\'':
          result += "'\\''";
          break;
  ...
  MVATrainer::MVATrainer(const std::string &fileName, bool useXSLT, const char *styleSheet)
      : input(nullptr),
        output(nullptr),
        name("MVATrainer"),
        doAutoSave(true),
    ...
    if (useXSLT) {
      std::string sheet;
      if (!styleSheet)
        sheet = edm::FileInPath("PhysicsTools/MVATrainer/data/MVATrainer.xsl").fullPath();
      else
        sheet = styleSheet;

      std::string preproc = "xsltproc --xinclude " + escape(sheet) + " " + escape(fileName);
    ...
    if (std::strcmp(XMLSimpleStr(node->getNodeName()), "MVATrainer") != 0)
      throw cms::Exception("MVATrainer") << "Invalid XML root node." << std::endl;

    enum State { STATE_GENERAL, STATE_FIRST, STATE_MIDDLE, STATE_LAST } state = STATE_GENERAL;
    ...
    for (node = node->getFirstChild(); node; node = node->getNextSibling()) {
      ...
      switch (state) {
        case STATE_GENERAL: {
          if (name != "general")
            throw cms::Exception("MVATrainer") << "Expected general config as first "
                                                  "tag."
                                               << std::endl;
          ...
          for (DOMNode *subNode = elem->getFirstChild(); subNode; subNode = subNode->getNextSibling()) {
            ...
            if (std::strcmp(XMLSimpleStr(subNode->getNodeName()), "option") != 0)
              throw cms::Exception("MVATrainer") << "Expected option tag." << std::endl;

            elem = static_cast<DOMElement *>(subNode);
            name = XMLDocument::readAttribute<std::string>(elem, "name");
            std::string content = XMLSimpleStr(elem->getTextContent());
            ...
            if (name == "id")
              ...
              throw cms::Exception("MVATrainer") << "Unknown option \"" << name << "\"." << std::endl;
          }
        ...
        case STATE_FIRST: {
          if (name != "input")
            throw cms::Exception("MVATrainer") << "Expected input config as second "
                                                  "tag."
                                               << std::endl;

          AtomicId id = XMLDocument::readAttribute<std::string>(elem, "id");
          input = new Source(id, true);
          input->getOutputs().append(createVariable(input, kTargetId, Variable::FLAG_NONE), SourceVariableSet::kTarget);
          input->getOutputs().append(createVariable(input, kWeightId, Variable::FLAG_OPTIONAL),
        ...
        case STATE_MIDDLE: {
          if (name == "output") {
            AtomicId zero;
            output = new TrainProcessor("output", &zero, this);
            fillInputVars(output->getInputs(), elem);
            state = STATE_LAST;
            continue;
          } else if (name != "processor")
            throw cms::Exception("MVATrainer") << "Unexpected tag after input "
                                                  "config."
                                               << std::endl;

          AtomicId id = XMLDocument::readAttribute<std::string>(elem, "id");
          std::string name = XMLDocument::readAttribute<std::string>(elem, "name");

          makeProcessor(elem, id, name.c_str());
        } break;
        case STATE_LAST:
          throw cms::Exception("MVATrainer") << "Unexpected tag found after output." << std::endl;
          break;
      }
    }

    if (state == STATE_FIRST)
      throw cms::Exception("MVATrainer") << "Expected input variable config." << std::endl;
    else if (state == STATE_MIDDLE)
      throw cms::Exception("MVATrainer") << "Expected output variable config." << std::endl;
  ...
  MVATrainer::~MVATrainer() {
    ...
    for (std::map<AtomicId, Source *>::const_iterator iter = sources.begin(); iter != sources.end(); iter++) {
      TrainProcessor *proc = dynamic_cast<TrainProcessor *>(iter->second);
    ...
    delete output;
    std::for_each(variables.begin(), variables.end(), deleter<SourceVariable>);
  }

  void MVATrainer::loadState() {
    for (std::vector<AtomicId>::const_iterator iter = this->processors.begin(); iter != this->processors.end();
         iter++) {
      std::map<AtomicId, Source *>::const_iterator pos = sources.find(*iter);
      assert(pos != sources.end());
      TrainProcessor *source = dynamic_cast<TrainProcessor *>(pos->second);
      assert(source);

      if (source->load())
        edm::LogInfo("MVATrainer") << source->getId() << " configuration for \"" << (const char *)source->getName()
                                   << "\" loaded from file.";
    }
  }
  ...
  void MVATrainer::makeProcessor(DOMElement *elem, AtomicId id, const char *name) {
    ...
    for (DOMNode *node = elem->getFirstChild(); node; node = node->getNextSibling()) {
      ...
      if (!cur->tag)
        throw cms::Exception("MVATrainer") << "Superfluous tag " << tag << "encountered in processor." << std::endl;
      else if (tag != cur->tag && cur->mandatory)
        throw cms::Exception("MVATrainer")
            << "Expected tag " << cur->tag << ", got " << tag << " instead in processor." << std::endl;
      else if (tag != cur->tag) {
        cur++;
        continue;
      }
    ...
    while (cur->tag && !cur->mandatory)
      cur++;
    if (cur->tag)
      throw cms::Exception("MVATrainer") << "Unexpected end of processor configuration, "
                                         << "expected tag " << cur->tag << "." << std::endl;

    std::unique_ptr<TrainProcessor> proc(TrainProcessor::create(name, &id, this));
    if (!proc.get())
      throw cms::Exception("MVATrainer") << "Variable processor trainer " << name
                                         << " could not be instantiated. Most likely because"
                                            " the trainer plugin for \""
                                         << name
                                         << "\""
                                            " does not exist."
                                         << std::endl;
    ...
    if (sources.find(id) != sources.end())
      throw cms::Exception("MVATrainer") << "Duplicate variable processor id " << (const char *)id << "." << std::endl;

    fillInputVars(proc->getInputs(), xmlInput);
    fillOutputVars(proc->getOutputs(), proc.get(), xmlOutput);

    edm::LogInfo("MVATrainer") << "Configuring " << (const char *)proc->getId() << " \""
  ...
  std::string MVATrainer::trainFileName(const TrainProcessor *proc,
    ...
    return stdStringPrintf(trainFileMask.c_str(), (const char *)proc->getName(), arg_.c_str(), ext.c_str());
  }
  ...
  TrainerMonitoring::Module *MVATrainer::bookMonitor(const std::string &name) {
    ...
    if (!monitoring.get()) {
      std::string fileName = stdStringPrintf(trainFileMask.c_str(), "monitoring", "", "root");
  ...
  SourceVariable *MVATrainer::createVariable(Source *source, AtomicId name, Variable::Flags flags) {
  ...
  void MVATrainer::fillInputVars(SourceVariableSet &vars, XERCES_CPP_NAMESPACE_QUALIFIER DOMElement *xml) {
    std::vector<SourceVariable *> tmp;
    SourceVariable *target = nullptr;
    SourceVariable *weight = nullptr;

    for (DOMNode *node = xml->getFirstChild(); node; node = node->getNextSibling()) {
      if (node->getNodeType() != DOMNode::ELEMENT_NODE)
        continue;

      if (std::strcmp(XMLSimpleStr(node->getNodeName()), "var") != 0)
        throw cms::Exception("MVATrainer") << "Invalid input variable node." << std::endl;

      DOMElement *elem = static_cast<DOMElement *>(node);

      AtomicId source = XMLDocument::readAttribute<std::string>(elem, "source");
      AtomicId name = XMLDocument::readAttribute<std::string>(elem, "name");

      SourceVariable *var = getVariable(source, name);
      if (!var)
        throw cms::Exception("MVATrainer")
            << "Input variable " << (const char *)source << ":" << (const char *)name << " not found." << std::endl;

      if (XMLDocument::readAttribute<bool>(elem, "target", false)) {
        if (target)
          throw cms::Exception("MVATrainer") << "Target variable defined twice" << std::endl;
        target = var;
      }
      if (XMLDocument::readAttribute<bool>(elem, "weight", false)) {
        if (weight)
          throw cms::Exception("MVATrainer") << "Weight variable defined twice" << std::endl;
        weight = var;
      }
    ...
    if (!weight) {
      ...
      assert(weight);
      tmp.insert(tmp.begin() + (target == input->getOutput(kTargetId)), 1, weight);
    }
    if (!target) {
    ...
    unsigned int n = 0;
    for (std::vector<SourceVariable *>::const_iterator iter = variables.begin(); iter != variables.end(); iter++) {
      std::vector<SourceVariable *>::const_iterator pos = std::find(tmp.begin(), tmp.end(), *iter);
      if (pos == tmp.end())
        continue;
      ...
      if (vars.append(*iter, magic, pos - tmp.begin())) {
        ...
        throw cms::Exception("MVATrainer")
            << "Input variable " << (const char *)source << ":" << (const char *)name << " defined twice." << std::endl;
      }
  ...
  void MVATrainer::fillOutputVars(SourceVariableSet &vars,
                                  Source *source,
    ...
    for (DOMNode *node = xml->getFirstChild(); node; node = node->getNextSibling()) {
      ...
      if (std::strcmp(XMLSimpleStr(node->getNodeName()), "var") != 0)
        throw cms::Exception("MVATrainer") << "Invalid output variable node." << std::endl;

      DOMElement *elem = static_cast<DOMElement *>(node);

      AtomicId name = XMLDocument::readAttribute<std::string>(elem, "name");
      if (!name)
        throw cms::Exception("MVATrainer") << "Output variable tag missing name." << std::endl;
      if (isMagic(name))
        throw cms::Exception("MVATrainer") << "Cannot use magic variable names in output." << std::endl;

      Variable::Flags flags = Variable::FLAG_NONE;

      if (XMLDocument::readAttribute<bool>(elem, "optional", true))
        flags = (PhysicsTools::Variable::Flags)(flags | Variable::FLAG_OPTIONAL);

      if (XMLDocument::readAttribute<bool>(elem, "multiple", true))
        flags = (PhysicsTools::Variable::Flags)(flags | Variable::FLAG_MULTIPLE);

      SourceVariable *var = createVariable(source, name, flags);
      if (!var || vars.append(var))
        throw cms::Exception("MVATrainer") << "Output variable " << (const char *)source->getName() << ":"
                                           << (const char *)name << " defined twice." << std::endl;
    }
  }

  void MVATrainer::connectProcessors(Calibration::MVAComputer *calib,
                                     const std::vector<CalibratedProcessor> &procs,
                                     bool withTarget) const {
    std::map<SourceVariable *, unsigned int> vars;
    unsigned int size = 0;

    MVATrainerComputer *trainCalib = dynamic_cast<MVATrainerComputer *>(calib);

    for (unsigned int i = 0; i < input->getOutputs().size(true); i++) {
      if (i < 2 && !withTarget)
        continue;
    ...
    for (std::vector<CalibratedProcessor>::const_iterator iter = procs.begin(); iter != procs.end(); iter++) {
      bool isInterceptor = dynamic_cast<BaseInterceptor *>(iter->calib) != nullptr;
      ...
      if (iter->processor)
        inoutVars = iter->processor->getInputs().get(isInterceptor);
      for (std::vector<SourceVariable *>::const_iterator iter2 = inoutVars.begin(); iter2 != inoutVars.end(); iter2++) {
        std::map<SourceVariable *, unsigned int>::const_iterator pos = vars.find(*iter2);

        assert(pos != vars.end());

        if (pos->second < last)
          throw cms::Exception("MVATrainer") << "Input variables not declared "
                                                "in order of appearance in \""
                                             << (const char *)iter->processor->getName() << "\"." << std::endl;
      ...
      for (std::vector<SourceVariable *>::const_iterator iter = inoutVars.begin(); iter != inoutVars.end(); iter++) {
        vars[*iter] = size++;
      }
    }

    if (output->getInputs().size() != 1)
      throw cms::Exception("MVATrainer") << "Exactly one output variable has to be specified." << std::endl;

    SourceVariable *outVar = output->getInputs().get()[0];
    std::map<SourceVariable *, unsigned int>::const_iterator pos = vars.find(outVar);
    if (pos != vars.end())
      calib->output = pos->second;
  }

  Calibration::MVAComputer *MVATrainer::makeTrainCalibration(const AtomicId *compute, const AtomicId *train) const {
    ...
    for (const AtomicId *iter = compute; *iter; iter++) {
      ...
      Calibration::ProcForeach *looper = dynamic_cast<Calibration::ProcForeach *>(proc);
      if (looper) {
        std::vector<AtomicId>::const_iterator pos2 = std::find(this->processors.begin(), this->processors.end(), *iter);
        ...
        for (int i = 0; i < (int)looper->nProcs; ++i, ++pos2) {
          ...
          std::map<AtomicId, TrainInterceptor *>::iterator pos3 = interceptors.find(*pos2);
          if (pos3 != interceptors.end()) {
            n++;
            baseInterceptors.push_back(std::make_pair(processors.size(), pos3->second));
            processors.push_back(CalibratedProcessor(pos3->second->getProcessor(), pos3->second));
    ...
    for (std::map<AtomicId, TrainInterceptor *>::const_iterator iter = interceptors.begin(); iter != interceptors.end();
         ++iter) {
      TrainProcessor *proc = iter->second->getProcessor();
      baseInterceptors.push_back(std::make_pair(processors.size(), iter->second));
      processors.push_back(CalibratedProcessor(proc, iter->second));
    }

    std::unique_ptr<Calibration::MVAComputer> calib(
        new MVATrainerComputer(baseInterceptors, doAutoSave, randomSeed, crossValidation));
  ...
  void MVATrainer::doneTraining(Calibration::MVAComputer *trainCalibration) const {
    MVATrainerComputer *calib = dynamic_cast<MVATrainerComputer *>(trainCalibration);

    if (!calib)
      throw cms::Exception("MVATrainer") << "Invalid training calibration passed to "
                                            "doneTraining()"
                                         << std::endl;
  ...
  std::vector<AtomicId> MVATrainer::findFinalProcessors() const {
    ...
    while (!toCheck.empty()) {
      ...
      for (std::vector<SourceVariable *>::const_iterator iter = inputs.begin(); iter != inputs.end(); ++iter) {
    ...
    for (std::vector<AtomicId>::const_iterator iter = processors.begin(); iter != processors.end(); ++iter) {
      std::map<AtomicId, Source *>::const_iterator pos = sources.find(*iter);
  ...
  Calibration::MVAComputer *MVATrainer::getCalibration() const {
    ...
    for (std::vector<AtomicId>::const_iterator iter = used.begin(); iter != used.end(); iter++) {
      std::map<AtomicId, Source *>::const_iterator pos = sources.find(*iter);
      assert(pos != sources.end());
      TrainProcessor *source = dynamic_cast<TrainProcessor *>(pos->second);
      assert(source);
      ...
      Calibration::ProcForeach *foreach = dynamic_cast<Calibration::ProcForeach *>(proc);
      if (foreach) {
        std::vector<AtomicId>::const_iterator begin =
            std::find(this->processors.begin(), this->processors.end(), *iter);
        assert(this->processors.end() - begin > (int)(foreach->nProcs + 1));
        ++begin;
        std::vector<AtomicId>::const_iterator end = begin + foreach->nProcs;
        foreach
          ->nProcs = 0;
        for (std::vector<AtomicId>::const_iterator iter2 = iter; iter2 != used.end(); ++iter2)
  ...
  void MVATrainer::findUntrainedComputers(std::vector<AtomicId> &compute, std::vector<AtomicId> &train) const {
    compute.clear();
    train.clear();
    ...
    for (std::vector<AtomicId>::const_iterator iter = processors.begin(); iter != processors.end(); iter++) {
      std::map<AtomicId, Source *>::const_iterator pos = sources.find(*iter);
      assert(pos != sources.end());
      TrainProcessor *proc = dynamic_cast<TrainProcessor *>(pos->second);
      assert(proc);

      bool trainedDeps = true;
      std::vector<SourceVariable *> inputVars = proc->getInputs().get();
      for (std::vector<SourceVariable *>::const_iterator iter2 = inputVars.begin(); iter2 != inputVars.end(); iter2++) {
        if (trainedSources.find((*iter2)->getSource()) == trainedSources.end()) {
          trainedDeps = false;
          break;
        }
      }
    ...
    if (doMonitoring && !output->isTrained() &&
        trainedSources.find(output->getInputs().get()[0]->getSource()) != trainedSources.end())
