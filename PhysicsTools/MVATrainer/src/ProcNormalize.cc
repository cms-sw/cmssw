...
namespace {  // anonymous
  ...
  public:
    ...
    ProcNormalize(const char *name, const AtomicId *id, MVATrainer *trainer);
  ...
  ProcNormalize::ProcNormalize(const char *name, const AtomicId *id, MVATrainer *trainer)
      : TrainProcessor(name, id, trainer), categoryIdx(-1), nCategories(1) {}
  ...
  void ProcNormalize::configure(DOMElement *elem) {
    ...
    for (DOMNode *node = elem->getFirstChild(); node; node = node->getNextSibling()) {
      ...
      if (categoryIdx >= 0)
        throw cms::Exception("ProcNormalize") << "More than one category variable given." << std::endl;
    ...
    for (DOMNode *node = elem->getFirstChild(); node; node = node->getNextSibling()) {
      ...
      if (std::strcmp(nodeName, "pdf") != 0)
        throw cms::Exception("ProcNormalize") << "Expected pdf tag in config section." << std::endl;
      elem = static_cast<DOMElement *>(node);

      PDF pdf;

      pdf.distr.resize(XMLDocument::readAttribute<unsigned int>(elem, "size", 100));

      pdf.smooth = XMLDocument::readAttribute<unsigned int>(elem, "smooth", 40);

      pdf.fillSignal = XMLDocument::readAttribute<bool>(elem, "signal", true);
      pdf.fillBackground = XMLDocument::readAttribute<bool>(elem, "background", true);

      if (!pdf.fillSignal && !pdf.fillBackground)
        throw cms::Exception("ProcNormalize") << "Filling neither background nor signal "
                                                 "in config."
                                              << std::endl;
    ...
    if (pdfs.size() != nInputs * nCategories)
      throw cms::Exception("ProcNormalize")
          << "Got " << pdfs.size() << " pdf configs in total for " << nCategories << " categories and " << nInputs
  ...
  void ProcNormalize::trainData(const std::vector<double> *values, bool target, double weight) {
    int category = 0;
    if (categoryIdx >= 0)
    ...
    int i = 0;
    for (std::vector<PDF>::iterator iter = pdfs.begin() + category; iter < pdfs.end(); iter += nCategories, values++) {
      if (i++ == categoryIdx)
        values++;
      ...
      switch (iter->iteration) {
        ...
        case ITER_RANGE:
          for (std::vector<double>::const_iterator value = values->begin(); value != values->end(); value++) {
            ...
            iter->range.max = std::max(iter->range.max, *value);
          }
          continue;
        case ITER_FILL:
          break;
  ...
  void ProcNormalize::trainEnd() {
    bool done = true;
    for (std::vector<PDF>::iterator iter = pdfs.begin(); iter != pdfs.end(); iter++) {
      switch (iter->iteration) {
        case ITER_EMPTY:
        case ITER_RANGE:
        ...
        case ITER_FILL:
          iter->distr.front() *= 2;
          iter->distr.back() *= 2;
          smoothArray(iter->distr.size(), &iter->distr.front(), iter->smooth);
    ...
    if (done && monitoring) {
      ...
      for (std::vector<PDF>::iterator iter = pdfs.begin(); iter != pdfs.end(); iter++) {
        ...
        unsigned int n = iter->distr.size() - 1;
        double min = iter->range.min - 0.5 * iter->range.width() / n;
        double max = iter->range.max + 0.5 * iter->range.width() / n;
        TH1F *histo = monitoring->book<TH1F>(name + "_pdf", name.c_str(), title.c_str(), n + 1, min, max);
        for (unsigned int i = 0; i < n; i++)
  ...
  bool ProcNormalize::load() {
    ...
    if (std::strcmp(XMLSimpleStr(elem->getNodeName()), "ProcNormalize") != 0)
      throw cms::Exception("ProcNormalize") << "XML training data file has bad root node." << std::endl;

    unsigned int version = XMLDocument::readAttribute<unsigned int>(elem, "version", 1);

    if (version < 1 || version > 2)
      throw cms::Exception("ProcNormalize") << "Unsupported version " << version << "in train file." << std::endl;
    ...
    for (DOMNode *node = elem->getFirstChild(); node; node = node->getNextSibling()) {
      ...
      if (std::strcmp(XMLSimpleStr(node->getNodeName()), "pdf") != 0)
        throw cms::Exception("ProcNormalize") << "Expected pdf tag in train file." << std::endl;
      elem = static_cast<DOMElement *>(node);
      ...
      switch (version) {
        case 1:
          if (cur == pdfs.end())
            throw cms::Exception("ProcNormalize") << "Superfluous PDF in train data." << std::endl;
          pdf = &*cur++;
          break;
        case 2: {
          ...
          std::map<Id, PDF *>::const_iterator pos = pdfMap.find(id);
          if (pos == pdfMap.end())
            continue;
          else
            pdf = pos->second;
      ...
      for (DOMNode *subNode = elem->getFirstChild(); subNode; subNode = subNode->getNextSibling()) {
        ...
        if (std::strcmp(XMLSimpleStr(subNode->getNodeName()), "value") != 0)
          throw cms::Exception("ProcNormalize") << "Expected value tag in train file." << std::endl;

        elem = static_cast<DOMElement *>(node);

        pdf->distr.push_back(XMLDocument::readContent<double>(subNode));
      }
    }

    if (version == 1 && cur != pdfs.end())
      throw cms::Exception("ProcNormalize") << "Missing PDF in train data." << std::endl;

    trained = true;
    for (std::vector<PDF>::const_iterator iter = pdfs.begin(); iter != pdfs.end(); ++iter) {
  ...
  void ProcNormalize::save() {
    ...
    for (std::vector<PDF>::const_iterator iter = pdfs.begin(); iter != pdfs.end(); iter++) {
      ...
      for (std::vector<double>::const_iterator iter2 = iter->distr.begin(); iter2 != iter->distr.end(); iter2++) {
