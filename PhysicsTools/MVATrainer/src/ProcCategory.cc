...
namespace {  // anonymous
  ...
  public:
    ...
    ProcCategory(const char *name, const AtomicId *id, MVATrainer *trainer);
  ...
  ProcCategory::ProcCategory(const char *name, const AtomicId *id, MVATrainer *trainer)
      : TrainProcessor(name, id, trainer) {}

  ProcCategory::~ProcCategory() {}

  void fillRange(unsigned int n,
                 int *matrix,
                 unsigned int off,
                 const unsigned int *strides,
                 const std::pair<unsigned int, unsigned int> *ranges,
                 int value) {
    for (unsigned int i = ranges->first; i < ranges->second; i++) {
      if (n > 1)
        fillRange(n - 1, matrix, off + *strides * i, strides + 1, ranges + 1, value);
      else
        matrix[off + i] = value;
    }
  }
  ...
  void ProcCategory::configure(DOMElement *elem) {
    ...
    for (DOMNode *node = elem->getFirstChild(); node; node = node->getNextSibling()) {
      ...
      if (std::strcmp(XMLSimpleStr(node->getNodeName()), "group") != 0)
        throw cms::Exception("ProcCategory") << "Expected group tag in config section." << std::endl;

      for (DOMNode *subNode = node->getFirstChild(); subNode; subNode = subNode->getNextSibling()) {
        if (subNode->getNodeType() != DOMNode::ELEMENT_NODE)
          continue;

        if (std::strcmp(XMLSimpleStr(subNode->getNodeName()), "box") != 0)
          throw cms::Exception("ProcCategory") << "Expected box tag in config section." << std::endl;

        Box box;
        box.group = group;

        unsigned int i = 0;
        for (DOMNode *rangeNode = subNode->getFirstChild(); rangeNode; rangeNode = rangeNode->getNextSibling()) {
          if (rangeNode->getNodeType() != DOMNode::ELEMENT_NODE)
            continue;

          if (std::strcmp(XMLSimpleStr(rangeNode->getNodeName()), "range") != 0)
            throw cms::Exception("ProcCategory") << "Expected range tag in config section." << std::endl;

          if (i >= n)
            throw cms::Exception("ProcCategory") << "Number of ranges exceeded." << std::endl;
        ...
        if (i < n)
          throw cms::Exception("ProcCategory") << "Not enough ranges." << std::endl;
    ...
    for (unsigned int i = 0; i < n; i++)
      std::copy(sLimits[i].begin(), sLimits[i].end(), std::back_inserter(limits[i]));
    sLimits.clear();
    ...
    for (std::vector<Box>::reverse_iterator iter = boxes.rbegin(); iter != boxes.rend(); iter++) {
      for (unsigned int i = 0; i < n; i++) {
        const Range &range = iter->ranges[i];

        unsigned int minIdx;
        if (range.hasMin) {
          BinLimits::const_iterator pos = std::find(limits[i].begin(), limits[i].end(), range.min);
          if (pos == limits[i].end())
            throw cms::Exception("ProcCategory") << "Fatal: min limit not found" << std::endl;
        ...
        unsigned int maxIdx;
        if (range.hasMax) {
          BinLimits::const_iterator pos = std::find(limits[i].begin(), limits[i].end(), range.max);
          if (pos == limits[i].end())
            throw cms::Exception("ProcCategory") << "Fatal: max limit not found" << std::endl;
      ...
      fillRange(n, &matrix.front(), 0, &strides.front(), &ranges.front(), iter->group);
    }
