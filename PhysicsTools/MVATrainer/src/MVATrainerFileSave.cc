...
namespace PhysicsTools {
  ...
  MVATrainerFileSave::MVATrainerFileSave(const edm::ParameterSet &params)
      : trained(params.getUntrackedParameter<bool>("trained", true)), saved(false) {
  ...
  void MVATrainerFileSave::analyze(const edm::Event &event, const edm::EventSetup &es) {
    if (calib.get() || saved)
      return;
    ...
    edm::LogInfo("MVATrainerFileSave") << "Got the trained calibration data";
