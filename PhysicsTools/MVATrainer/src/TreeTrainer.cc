...
namespace PhysicsTools {
  ...
  TreeTrainer::TreeTrainer(TTree *tree, double weight) { addTree(tree, -1, weight); }
  ...
  TreeTrainer::~TreeTrainer() { reset(); }

  Calibration::MVAComputer *TreeTrainer::train(const std::string &trainFile, double crossValidation, bool useXSLT) {
  ...
  void TreeTrainer::reset() {
    readers.clear();
    std::for_each(weights.begin(), weights.end(), std::ptr_fun(&::operator delete));
    weights.clear();
  }
  ...
  void TreeTrainer::addTree(TTree *tree, int target, double weight) {
    ...
    if (target >= 0) {
      if (tree->GetBranch("__TARGET__"))
        throw cms::Exception("TreeTrainer") << "__TARGET__ branch already present in file." << std::endl;
  ...
  void TreeTrainer::addReader(const TreeReader &reader) { readers.push_back(reader); }
