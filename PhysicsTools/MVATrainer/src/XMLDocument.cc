...
namespace {  // anonymous
  ...
  public:
    ...
    stdio_istream(FILE *file) : file_(file), filebuf_(file, std::ios_base::in) { this->init(&filebuf_); }

    ~stdio_istream() override { close(file_); }

    __filebuf_type *rdbuf() const { return const_cast<__filebuf_type *>(&filebuf_); }
...
XMLSize_t STLInputStream::readBytes(XMLByte *const buf, const XMLSize_t size) {
  ...
  if (in.bad())
    throw cms::Exception("XMLDocument") << "I/O stream bad in STLInputStream::readBytes()" << std::endl;
...
XMLDocument::XMLDocument(const std::string &fileName, bool write)
    : platform(new XercesPlatform()), fileName(fileName), write(write), impl(nullptr), doc(nullptr), rootNode(nullptr) {
  if (write)
    openForWrite(fileName);
  else {
    std::unique_ptr<std::istream> inputStream(new std::ifstream(fileName.c_str()));
    if (!inputStream->good())
      throw cms::Exception("XMLDocument") << "XML input file \"" << fileName
                                          << "\" "
...
XMLDocument::XMLDocument(const std::string &fileName, const std::string &command)
    : platform(new XercesPlatform()), fileName(fileName), write(false), impl(nullptr), doc(nullptr), rootNode(nullptr) {
  FILE *file = popen(command.c_str(), "r");
  if (!file)
    throw cms::Exception("XMLDocument") << "Could not execute XML preprocessing "
                                           " command \""
                                        << command << "\"." << std::endl;

  std::unique_ptr<std::istream> inputStream(new stdio_istream<pclose>(file));
  if (!inputStream->good())
    throw cms::Exception("XMLDocument") << "XML preprocessing command \"" << fileName
                                        << "\" stream could not be opened for reading." << std::endl;
...
XMLDocument::~XMLDocument() {
  ...
  try {
    std::unique_ptr<XMLFormatTarget> target(new LocalFileFormatTarget(fileName.c_str()));
...
void XMLDocument::openForRead(std::unique_ptr<std::istream> &stream) {
  ...
  try {
    ...
    if (parser->getErrorCount())
      throw cms::Exception("XMLDocument") << "XML parser reported errors." << std::endl;
  } catch (const XMLException &e) {
    throw cms::Exception("XMLDocument") << "XML parser reported DOM error no. " << (unsigned long)e.getCode() << ": "
                                        << XMLSimpleStr(e.getMessage()) << "." << std::endl;
  } catch (const SAXException &e) {
    throw cms::Exception("XMLDocument") << "XML parser reported: " << XMLSimpleStr(e.getMessage()) << "." << std::endl;
  }
  ...
  if (!node)
    throw cms::Exception("XMLDocument") << "XML document didn't contain a valid "
                                        << "root node." << std::endl;
...
void XMLDocument::openForWrite(const std::string &fileName) {
  impl = DOMImplementationRegistry::getDOMImplementation(XMLUniStr("LS"));
  assert(impl);
}

DOMDocument *XMLDocument::createDocument(const std::string &root) {
  if (doc)
    throw cms::Exception("XMLDocument") << "Document already exists in createDocument." << std::endl;
...
static bool isBool(std::string value) {
  ...
  if (value == "1" || value == "y" || value == "yes" || value == "true" || value == "ok")
    return true;

  if (value == "0" || value == "n" || value == "no" || value == "false")
    return false;

  throw cms::Exception("XMLDocument") << "Invalid boolean value in XML document" << std::endl;
}

static const char *makeBool(bool value) { return value ? "true" : "false"; }

bool XMLDocument::hasAttribute(XERCES_CPP_NAMESPACE_QUALIFIER DOMElement *elem, const char *name) {
  XMLUniStr uniName(name);
  return elem->hasAttribute(uniName);
}

template <>
bool XMLDocument::readAttribute(XERCES_CPP_NAMESPACE_QUALIFIER DOMElement *elem, const char *name) {
  XMLUniStr uniName(name);
  if (!elem->hasAttribute(uniName))
    throw cms::Exception("MVAComputer") << "Missing attribute " << name << " in tag "
                                        << XMLSimpleStr(elem->getNodeName()) << "." << std::endl;
...
template <>
void XMLDocument::writeAttribute(XERCES_CPP_NAMESPACE_QUALIFIER DOMElement *elem, const char *name, const bool &value) {
