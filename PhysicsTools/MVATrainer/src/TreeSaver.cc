...
namespace {  // anonymous
  ...
  public:
    ...
    TreeSaver(const char *name, const AtomicId *id, MVATrainer *trainer);
    ~TreeSaver() override;
  ...
  private:
    ...
    struct Var {
      ...
      bool hasName(std::string other) const { return name == other; }
    };
  ...
  TreeSaver::TreeSaver(const char *name, const AtomicId *id, MVATrainer *trainer)
      : TrainProcessor(name, id, trainer), iteration(ITER_EXPORT), tree(nullptr), flagsPassed(false), begun(false) {}
  ...
  void TreeSaver::configure(DOMElement *elem) {
    ...
    for (auto const &input : inputs) {
      ...
      if (std::find_if(vars.begin(), vars.end(), [&name](auto const &c) { return c.hasName(name); }) != vars.end()) {
        for (unsigned i = 1;; i++) {
          std::ostringstream ss;
          ss << name << "_" << i;
          if (std::find_if(vars.begin(), vars.end(), [&ss](auto c) { return c.hasName(ss.str()); }) == vars.end()) {
            name = ss.str();
            break;
          }
        }
  ...
  void TreeSaver::init() {
    ...
    for (std::vector<Var>::iterator iter = vars.begin(); iter != vars.end(); iter++, pos++) {
      if (iter->flags & Variable::FLAG_MULTIPLE) {
        iter->ptr = &iter->values;
        tree->Branch(iter->name.c_str(), "std::vector<double>", &pos->ptr);
      } else
        tree->Branch(iter->name.c_str(), &pos->value, (iter->name + "/D").c_str());
    }
  }
  ...
  void TreeSaver::passFlags(const std::vector<Variable::Flags> &flags) {
    ...
    unsigned int idx = 0;
    for (std::vector<Variable::Flags>::const_iterator iter = flags.begin(); iter != flags.end(); ++iter, idx++)
      vars[idx].flags = *iter;
  ...
  void TreeSaver::trainBegin() {
    if (iteration == ITER_EXPORT) {
      ...
      file = std::unique_ptr<TFile>(TFile::Open(trainer->trainFileName(this, "root").c_str(), "RECREATE"));
      if (!file.get())
        throw cms::Exception("TreeSaver") << "Could not open ROOT file for writing." << std::endl;

      file->cd();
      tree = new TTree(getTreeName().c_str(), "MVATrainer signal and background");
  ...
  void TreeSaver::trainData(const std::vector<double> *values, bool target, double weight) {
    if (iteration != ITER_EXPORT)
      return;
