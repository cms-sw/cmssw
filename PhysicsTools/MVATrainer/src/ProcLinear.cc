...
namespace {  // anonymous
  ...
  public:
    ...
    ProcLinear(const char *name, const AtomicId *id, MVATrainer *trainer);
    ~ProcLinear() override;
    ...
    void trainData(const std::vector<double> *values, bool target, double weight) override;
  ...
  private:
    enum Iteration { ITER_FILL, ITER_DONE } iteration;
  ...
  ProcLinear::ProcLinear(const char *name, const AtomicId *id, MVATrainer *trainer)
      : TrainProcessor(name, id, trainer), iteration(ITER_FILL) {}
  ...
  void ProcLinear::configure(DOMElement *elem) {
    ...
    if (std::strcmp(XMLSimpleStr(node->getNodeName()), "coefficients") != 0)
      throw cms::Exception("ProcLinear") << "Expected coefficients tag in config section." << std::endl;
  ...
  void ProcLinear::trainData(const std::vector<double> *values, bool target, double weight) {
    if (iteration != ITER_FILL)
      return;
  ...
  bool ProcLinear::load() {
    ...
    if (std::strcmp(XMLSimpleStr(elem->getNodeName()), "ProcLinear") != 0)
      throw cms::Exception("ProcLinear") << "XML training data file has bad root node." << std::endl;
    ...
    if (!node)
      throw cms::Exception("ProcLinear") << "Train data file empty." << std::endl;
    ...
    if (node)
      throw cms::Exception("ProcLinear") << "Train data file contains superfluous tags." << std::endl;
