...
void HistoMuon::fill( const Muon *muon, uint iMu )
{
  ...
  h_nChambers_->fill( muon->numberOfChambers(), iMu );

// For CMSSW 1_6_x

  h_calCompat_->fill( muon->getCaloCompatibility(), iMu );
  h_type_->fill( muon->getType(), iMu );
  reco::MuonEnergy muEnergy = muon->getCalEnergy();

// For CMSSW 2_0_x

//   h_calCompat_->fill( muon->caloCompatibility(), iMu );
//   h_type_->fill( muon->type(), iMu );
