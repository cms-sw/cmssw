...
void testTorchFromBufferModelEvalSinglePass(torch::jit::script::Module& model, const int * a_cpu,  const int *b_cpu, int * c_cpu, 
  ...
  cudaMallocAsync((void**)&a_gpu, bytes, c10::cuda::getCurrentCUDAStream().stream());
  cudaMallocAsync((void**)&b_gpu, bytes, c10::cuda::getCurrentCUDAStream().stream());
  cudaMallocAsync((void**)&c_gpu, bytes, c10::cuda::getCurrentCUDAStream().stream());
  allocRange.end();
