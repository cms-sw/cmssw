...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
  ...
  GENERATE_SOA_LAYOUT(SoAOutputsTemplate, SOA_COLUMN(float, prob))

  using SoAInputs = SoAInputsTemplate<>;
  using SoAOutputs = SoAOutputsTemplate<>;

  class TestPortableInferenceAOT : public CppUnit::TestFixture {
    CPPUNIT_TEST_SUITE(TestPortableInferenceAOT);
    CPPUNIT_TEST(test);
  ...
  CPPUNIT_TEST_SUITE_REGISTRATION(TestPortableInferenceAOT);

  std::string TestPortableInferenceAOT::shared_lib() {
  ...
  void TestPortableInferenceAOT::test() {
    // alpaka setup
    Platform platform;
...

    {
      ...
      CPPUNIT_ASSERT_MESSAGE("FAILED: Architecture compiled shared library is missing.", !lib_path.empty());
      auto model = cms::torch::alpaka::Model<cms::torch::alpaka::CompilationType::kAheadOfTime>(lib_path);
      model.to(queue);
      ...
      alpaka::wait(queue);
      for (size_t i = 0; i < batch_size; i++) {
        CPPUNIT_ASSERT(outputs_host.const_view().prob()[i] == 0.5f);
      }
    }
  }
