...
int main(int argc, const char* argv[])
{
    ...
    try {
      ...
      model = torch::jit::load("/data/user/dvalsecc/CMSSW_14_0_X_2023-11-29-1100/src/PhysicsTools/PyTorch/test/simple_dnn_largeinput.pt");
      model.to(device);
	...
	assert(c_gpu_tensor.equal(output));
    }
    catch (exception& e)
    {
        cout << e.what() << endl;
