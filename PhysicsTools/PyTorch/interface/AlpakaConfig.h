 ...
 */
   ...
   * 
   * Provides a mechanism to disable multi-threading and ensure correct PyTorch device management.
  ...
  private:
    void set() {
       ...
       * @note Global call.
       */
      [[maybe_unused]] static bool threading_guard = [] {
        at::set_num_threads(1);
        at::set_num_interop_threads(1);
        return true;
      }();
      static_cast<T *>(this)->set_impl();
    }
...
// #elif ALPAKA_ACC_GPU_HIP_ENABLED
// Similar structure can be added for HIP support if needed. torch::cuda ns is hip when using ROCm/HIP
#elif ALPAKA_ACC_CPU_B_SEQ_T_SEQ_ENABLED
  ...
  public:
    ...
    using Base::Base;
    void set_impl() { /**< nothing to be done, only threading is disabled (in base class) */ }
