...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
   ...
   */
  class JitClassificationProducer : public stream::EDProducer<edm::GlobalCache<JitModel>> {
  public:
    JitClassificationProducer(const edm::ParameterSet &params, const JitModel *cache);
  ...
  JitClassificationProducer::JitClassificationProducer(edm::ParameterSet const &params, const JitModel *cache)
  ...
  /**
 * @brief Initializes the global model cache by loading a TorchScript JIT model.
 * @param params Configuration parameters.
 * @return A unique pointer to the model.
 */
  std::unique_ptr<JitModel> JitClassificationProducer::initializeGlobalCache(const edm::ParameterSet &param) {
    auto model_path = param.getParameter<edm::FileInPath>("modelPath").fullPath();
    return std::make_unique<JitModel>(model_path);
  }

  /**
   * @brief Called once at global job end. No-op here.
   * @param cache The global model cache instance.
   */
  void JitClassificationProducer::globalEndJob(const JitModel *cache) {}
  ...
  void JitClassificationProducer::produce(device::Event &event, const device::EventSetup &event_setup) {
    ...
    NvtxScopedRange produce_range("Classifier::produce");
    ...
    std::cout << "(Classification) OK - " << std::chrono::duration_cast<std::chrono::microseconds>(t2 - t1).count()
              << " us" << std::endl;
    produce_range.end();
  }
