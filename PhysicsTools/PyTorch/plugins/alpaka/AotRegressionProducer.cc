...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
   ...
   */
  class AotRegressionProducer : public stream::EDProducer<edm::GlobalCache<AotModel>> {
  public:
    AotRegressionProducer(const edm::ParameterSet &params, const AotModel *cache);
  ...
  AotRegressionProducer::AotRegressionProducer(edm::ParameterSet const &params, const AotModel *cache)
  ...
  /**
   * @brief Initializes the global cache by loading the model.
   * @param params Configuration parameters.
   * @return A unique pointer to the loaded model.
   */
  std::unique_ptr<AotModel> AotRegressionProducer::initializeGlobalCache(const edm::ParameterSet &param) {
    auto model_path = param.getParameter<edm::FileInPath>("modelPath").fullPath();
    return std::make_unique<AotModel>(model_path);
  }

  /**
   * @brief Called at the end of the job to clean up global cache.
   * @param cache Pointer to the global model cache.
   */
  void AotRegressionProducer::globalEndJob(const AotModel *cache) {}
  ...
  void AotRegressionProducer::produce(device::Event &event, const device::EventSetup &event_setup) {
    ...
    NvtxScopedRange produce_range("Regression::produce");
    ...
    std::cout << "(Regression) OK - " << std::chrono::duration_cast<std::chrono::microseconds>(t2 - t1).count() << " us"
              << std::endl;
    produce_range.end();
  }
