...
namespace PhysicsTools {
  ...
  MVAComputer::MVAComputer(const Calibration::MVAComputer *calib) : nVars(0), output(0) { setup(calib); }

  MVAComputer::MVAComputer(Calibration::MVAComputer *calib, bool owned) : nVars(0), output(0) {
    if (owned)
      this->owned.reset(calib);
    setup(calib);
  }

  MVAComputer::MVAComputer(const char *filename) : nVars(0), output(0), owned(readCalibration(filename)) {
    setup(owned.get());
  }

  MVAComputer::MVAComputer(std::istream &is) : nVars(0), output(0), owned(readCalibration(is)) { setup(owned.get()); }
  ...
  void MVAComputer::setup(const Calibration::MVAComputer *calib) {
    ...
    for (std::vector<Calibration::VarProcessor *>::const_iterator iter = processors.begin(); iter != processors.end();
      ...
      VarProcessor::ConfigCtx::iterator::difference_type pos = config.end() - config.begin();
      processor->configure(config);
      unsigned int nOutput = (config.end() - config.begin()) - pos;
      if (!nOutput)
        throw cms::Exception("InvalidProcessor") << name
                                                 << " rejected input variable "
                                                    "configuration"
                                                 << std::endl;
    ...
    if (output >= nVars)
      ...
      throw cms::Exception("InvalidOutput") << "Output variable at index " << output << " invalid." << std::endl;
    ...
    while (iter != varProcessors.end()) {
      ...
      while (status != VarProcessor::kStop) {
        ...
        if (status != VarProcessor::kSkip)
          ctx.eval(
              &*iter->processor, outConf, output, loopStart ? loopStart : loopOutConf, loopCtx, offset, iter->nOutput);

#ifdef DEBUG_EVAL
        for (unsigned int i = 0; i < iter->nOutput; i++, outConf++) {
          std::cout << "\tVar " << (outConf - ctx.conf()) << std::endl;
        ...
        status = loop->processor->loop(output, outConf, nextOutput, loopCtx, offset);
  ...
  Calibration::MVAComputer *MVAComputer::readCalibration(std::istream &is) {
    if (!is.good())
      throw cms::Exception("InvalidFileState") << "Stream passed to MVAComputer::readCalibration "
                                                  "has an invalid state."
                                               << std::endl;

    char header[sizeof STANDALONE_HEADER - 1] = {
        0,
    };
    if (is.readsome(header, sizeof header) != sizeof header ||
        std::memcmp(header, STANDALONE_HEADER, sizeof header) != 0)
      throw cms::Exception("InvalidFileFormat") << "Stream passed to MVAComputer::readCalibration "
                                                   "is not a valid calibration file."
                                                << std::endl;

    TClass *rootClass = TClass::GetClass("PhysicsTools::Calibration::MVAComputer");
    if (!rootClass)
      throw cms::Exception("DictionaryMissing") << "CondFormats dictionary for "
    ...
    TBufferFile buffer(TBuffer::kRead, buf.size(), const_cast<void *>(static_cast<const void *>(buf.c_str())), kFALSE);
    buffer.InitMap();
  ...
  void MVAComputer::writeCalibration(const char *filename, const Calibration::MVAComputer *calib) {
    std::ofstream file(filename);
    writeCalibration(file, calib);
  }

  void MVAComputer::writeCalibration(std::ostream &os, const Calibration::MVAComputer *calib) {
    if (!os.good())
      throw cms::Exception("InvalidFileState") << "Stream passed to MVAComputer::writeCalibration "
                                                  "has an invalid state."
                                               << std::endl;
    ...
    buffer.StreamObject(const_cast<void *>(static_cast<const void *>(calib)), rootClass);
  ...
  void MVAComputer::DerivContext::eval(const VarProcessor *proc,
                                       int *outConf,
                                       double *output,
                                       int *loop,
    ...
    proc->deriv(values(), conf(), output, outConf, loop, ctx, offset, n(), out, deriv_);
  }

  double MVAComputer::DerivContext::output(unsigned int output, std::vector<double> &derivs) const {
    derivs.clear();
    derivs.reserve(n_);

    int pos = conf_[output];
    if (pos >= (int)n_)
      std::copy(&deriv_.front() + n_ * (pos - n_), &deriv_.front() + n_ * (pos - n_ + 1), std::back_inserter(derivs));
    else {
      derivs.resize(n_);
      derivs[pos] = 1.;
    }
