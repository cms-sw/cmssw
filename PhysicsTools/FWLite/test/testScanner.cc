...
int main(int argc, char *argv[]) {
  if (argc != 2) {
    std::cerr << "usage: " << argv[0] << " cmssw_reco_file.root" << std::endl;
    return 2;
  }
  ...
  ntracks =
      new fwlite::ObjectCountSelector<std::vector<reco::Track> >("generalTracks", "", "", "pt > 0.5 && abs(eta)<1", 2);
  sc.addEventSelector(ntracks);
  TH1 *heta2 = sc.draw("eta", 5, -2.5, 2.5);
  heta2->Sumw2();
  heta2->Scale(1.0 / sc.countEvents());
  heta2->SetLineColor(4);
  ntracks->setMin(0);
  ...
  sc.setMaxEvents(20);
  RooDataSet *ds = sc.fillDataSet(
      "pt:eta:@hits=hitPattern.numberOfValidHits", "@highPurity=quality('highPurity'):@highPt=pt>2", "pt > 0.5");
  ds->Print("v");
  delete ds;
