...
class KinVtxFitter {
 public:
  ...
  KinVtxFitter(const std::vector<reco::TransientTrack> tracks,
               const std::vector<double> masses, std::vector<float> sigmas);
  ...
  float kin_chi2() const {
    return kin_chi2_;
  }  // should they be merged in a single value?
  ...
  const math::PtEtaPhiMLorentzVector daughter_p4(size_t i) const {
    ...
    return math::PtEtaPhiMLorentzVector(
        state.globalMomentum().perp(), state.globalMomentum().eta(),
        state.globalMomentum().phi(), state.mass());
  }
