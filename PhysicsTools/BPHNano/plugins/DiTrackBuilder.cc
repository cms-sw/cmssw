...
class DiTrackBuilder : public edm::global::EDProducer<> {
 public:
  ...
  explicit DiTrackBuilder(const edm::ParameterSet &cfg)
      : trk1_selection_{cfg.getParameter<std::string>("trk1Selection")},
        ...
        pfcands_{consumes<pat::CompositeCandidateCollection>(
            cfg.getParameter<edm::InputTag>("tracks"))},
        ttracks_{consumes<TransientTrackCollection>(
 ...
 private:
  ...
  const StringCutObjectSelector<pat::CompositeCandidate>
      trk2_selection_;  // sub-leading cand
  const StringCutObjectSelector<pat::CompositeCandidate>
      pre_vtx_selection_;  // cut on the di-track before the SV fit
  const StringCutObjectSelector<pat::CompositeCandidate>
      post_vtx_selection_;  // cut on the di-track after the SV fit
  const edm::EDGetTokenT<pat::CompositeCandidateCollection>
      pfcands_;  // input PF cands this is sorted in pT in previous step
  const edm::EDGetTokenT<TransientTrackCollection>
      ttracks_;  // input TTracks of PF cands
  const edm::EDGetTokenT<reco::BeamSpot> beamspot_;
  double trk1_mass_;
  double trk2_mass_;
};

void DiTrackBuilder::produce(edm::StreamID, edm::Event &evt,
  ...
  // output
  std::unique_ptr<pat::CompositeCandidateCollection> ditrack_out(
  ...
  for (size_t trk1_idx = 0; trk1_idx < pfcands->size(); ++trk1_idx) {
    ...
    for (size_t trk2_idx = trk1_idx + 1; trk2_idx < pfcands->size();
         ++trk2_idx) {
      ...
      ditrack_cand.addUserFloat("fitted_mass_Kpi",
                                fitter_Kpi.fitted_candidate().mass());
      ditrack_cand.addUserFloat(
          "fitted_mass_Kpi_Err",
          sqrt(
      ...
      ditrack_cand.addUserFloat("fitted_mass_piK",
                                fitter_piK.fitted_candidate().mass());
      ditrack_cand.addUserFloat(
          "fitted_mass_piK_Err",
          sqrt(
      ...
      ditrack_cand.setVertex(reco::Candidate::Point(fitter.fitted_vtx().x(),
                                                    fitter.fitted_vtx().y(),
                                                    fitter.fitted_vtx().z()));
      ...
      ditrack_cand.addUserFloat(
          "fitted_pt", fitter.fitted_candidate().globalMomentum().perp());
      ditrack_cand.addUserFloat(
          "fitted_eta", fitter.fitted_candidate().globalMomentum().eta());
      ditrack_cand.addUserFloat(
          "fitted_phi", fitter.fitted_candidate().globalMomentum().phi());
