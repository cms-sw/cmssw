...
//      cand.addUserInt("fitted_charge",V0_ptr->userInt("fit_trk1_charge") + V0_ptr->userInt("fit_trk2_charge") + pi_ptr->charge());
	...
	if( !pre_vtx_selection_(cand) ) continue;
        ...
        if(!xifitter.success()) continue;
        auto XiTT = xifitter.fitted_candidate_ttrk();
	...
	// vertex vars
        ...
        std::pair<bool,Measurement1D> cur2DIP_Xi = bph::signedTransverseImpactParameter(Xitsos, fitter.fitted_refvtx(), *beamspot);
        std::pair<bool,Measurement1D> cur3DIP_Xi = bph::signedImpactParameter3D(Xitsos, fitter.fitted_refvtx(), *beamspot, (*pvtxs)[0].position().z());
