 ...
 public:
  explicit TriggerObjectTableBParkProducer(const edm::ParameterSet &iConfig)
      : name_(iConfig.getParameter<std::string>("name")),
        src_(consumes<std::vector<pat::TriggerObjectStandAlone>>(
            iConfig.getParameter<edm::InputTag>("src"))),
        l1Muon_(consumes<l1t::MuonBxCollection>(
            iConfig.getParameter<edm::InputTag>("l1Muon"))) {
    std::vector<edm::ParameterSet> selPSets =
    ...
    for (auto &pset : selPSets) {
      ...
      if (!sels_.back().qualityBitsDoc.empty()) {
        qualitystr << sels_.back().qualityBitsDoc << " for "
                   << sels_.back().name << "; ";
      }
    }
    idDoc_ = idstr.str();
  ...
  struct SelectedObject {
    ...
    SelectedObject(const edm::ParameterSet &pset)
      ...
      if (pset.existsAs<std::string>("l1seed")) {
        l1cut = StringCutObjectSelector<pat::TriggerObjectStandAlone>(
            pset.getParameter<std::string>("l1seed"));
        l1DR2 = std::pow(pset.getParameter<double>("l1deltaR"), 2);
      }
      if (pset.existsAs<std::string>("l1seed_2")) {
        l1cut_2 = StringCutObjectSelector<pat::TriggerObjectStandAlone>(
            pset.getParameter<std::string>("l1seed_2"));
        l1DR2_2 = std::pow(pset.getParameter<double>("l1deltaR_2"), 2);
      }
      if (pset.existsAs<std::string>("l2seed")) {
        l2cut = StringCutObjectSelector<pat::TriggerObjectStandAlone>(
            pset.getParameter<std::string>("l2seed"));
...
void TriggerObjectTableBParkProducer::produce(edm::Event &iEvent,
                                              const edm::EventSetup &iSetup) {
  edm::Handle<std::vector<pat::TriggerObjectStandAlone>> src;
  iEvent.getByToken(src_, src);

  std::vector<
      std::pair<const pat::TriggerObjectStandAlone *, const SelectedObject *>>
      selected;
  ...
  for (unsigned int i = 0; i < selected.size(); ++i) {
    ...
    for (unsigned int j = 0; j < i; ++j) {
      ...
      if (sel.id == sel2.id && abs(obj.pt() - obj2.pt()) < 1e-6 &&
          deltaR2(obj, obj2) < 1e-6) {
        selected_bits[&obj2] |=
            selected_bits[&obj];  // Keep filters from all the objects
  ...
  for (l1t::MuonBxCollection::const_iterator it = l1Muon->begin(0);
       it != l1Muon->end(0); it++) {
  ...
  std::vector<float> pt(nobj, 0), eta(nobj, 0), phi(nobj, 0), l1pt(nobj, 0),
      l1pt_2(nobj, 0), l2pt(nobj, 0);
  ...
  tab->addColumn<float>("l1pt_2", l1pt_2, "pt of associated secondary L1 seed",
                        8);
  tab->addColumn<float>(
      "l2pt", l2pt, "pt of associated 'L2' seed (i.e. HLT before tracking/PF)",
      10);
  tab->addColumn<int>("filterBits", bits,
                      "extra bits of associated information: " + bitsDoc_);
