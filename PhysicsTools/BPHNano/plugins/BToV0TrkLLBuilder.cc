...
void BToV0TrkLLBuilder::produce(edm::StreamID, edm::Event &evt, edm::EventSetup const &iSetup) const {
  ...
  for(size_t V0_idx = 0; V0_idx < V0s->size(); ++V0_idx) {
    ...
    math::PtEtaPhiMLorentzVector V0_p4(
      V0_ptr->userFloat("fitted_pt"),
...
//    float pi2_dr = V0_ptr->userFloat("trk2_dr");
    for(size_t pi_idx = 0; pi_idx < pions->size(); ++pi_idx) {
      ...
      if( pi1_idx==pi_idx || pi2_idx==pi_idx ) continue;
...
//      cand.addUserInt("fitted_charge",V0_ptr->userInt("fit_trk1_charge") + V0_ptr->userInt("fit_trk2_charge") + pi_ptr->charge());
      ...
      for(size_t ll_idx = 0; ll_idx < dileptons->size(); ++ll_idx) {
        ...
        cand.setCharge(ll_ptr->charge() + V0_ptr->userInt("fit_trk1_charge") + V0_ptr->userInt("fit_trk2_charge") + pi_ptr->charge());
	...
	KinVtxFitter fitter(
          {leptons_ttracks->at(l1_idx), leptons_ttracks->at(l2_idx),pions_ttracks->at(pi_idx), V0s_ttracks->at(V0_idx)},
	...
	if(!fitter.success()) continue; 
        ...
        TrajectoryStateOnSurface V0tsos = extrapolator.extrapolate(V0s_ttracks->at(V0_idx).impactPointState(), fitter.fitted_vtx());
        ...
        TrajectoryStateOnSurface pitsos = extrapolator.extrapolate(pions_ttracks->at(pi_idx).impactPointState(),fitter.fitted_vtx());
        ...
        cand.addUserFloat("V0trk_dz",V0tsos.globalPosition().z()-pions_ttracks->at(pi_idx).track().vz());//pitsos.globalPosition().z());
	...
	cand.addUserFloat("vtx_x", cand.vx());
        ...
        std::pair<bool,Measurement1D> cur2DIP_V0 = bph::signedTransverseImpactParameter(V0tsos, fitter.fitted_refvtx(), *beamspot);
        std::pair<bool,Measurement1D> cur3DIP_V0 = bph::signedImpactParameter3D(V0tsos, fitter.fitted_refvtx(), *beamspot, (*pvtxs)[0].position().z());
        std::pair<bool,Measurement1D> cur2DIP_pi = bph::signedTransverseImpactParameter(pitsos, fitter.fitted_refvtx(), *beamspot);
        std::pair<bool,Measurement1D> cur3DIP_pi = bph::signedImpactParameter3D(pitsos, fitter.fitted_refvtx(), *beamspot, (*pvtxs)[0].position().z());
...
//	std::cout << "                                      post" << endl;
      ...
      const double jpsi_bin[2] = {2.8, 3.2}; // {2.9, 3.2}; Start bin from 2.8 to be able to measure systematics later
      const double psi2s_bin[2] = {3.55, 3.8};
      if ((dilepton_mass > jpsi_bin[0] && dilepton_mass < jpsi_bin[1]) || (dilepton_mass > psi2s_bin[0] && dilepton_mass < psi2s_bin[1]))
      {
        // JPsi  mass constrait
