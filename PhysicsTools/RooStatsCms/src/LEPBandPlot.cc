...
LEPBandPlot::LEPBandPlot(const char* name,
  ...
  // Line for 0
  m_zero_line = new TLine(m_b_line_graph->GetXaxis()->GetXmin(), 0, m_b_line_graph->GetXaxis()->GetXmax(), 0);
...
LEPBandPlot::LEPBandPlot(const char* name,
  ...
  // bkg hypothesis band 1 sigma
  m_b_band_graph_1sigma = new TGraphAsymmErrors(n_points, x_vals, b_vals, nullptr, nullptr, b_down_bars1, b_up_bars1);
  m_b_band_graph_1sigma->SetFillColor(kGreen);
  m_b_band_graph_1sigma->SetLineColor(kGreen);
  m_b_band_graph_1sigma->SetMarkerColor(kGreen);

  // bkg hypothesis band 2 sigma
  m_b_band_graph_2sigma = new TGraphAsymmErrors(n_points, x_vals, b_vals, nullptr, nullptr, b_down_bars2, b_up_bars2);
  m_b_band_graph_2sigma->SetFillColor(kYellow);
  m_b_band_graph_2sigma->SetFillColor(kYellow);
  m_b_band_graph_2sigma->SetLineColor(kYellow);
  m_b_band_graph_2sigma->SetMarkerColor(kYellow);
...
**/
void LEPBandPlot::setXaxisTitle(const char* title) { m_b_band_graph_2sigma->GetXaxis()->SetTitle(title); }
...
plot. Indeed its axes will be the only one to be drawn.
**/
void LEPBandPlot::setTitle(const char* title) { m_b_band_graph_2sigma->SetTitle(title); }
...
void LEPBandPlot::print(const char* options) { std::cout << "\nLEPBandPlot object " << GetName() << ":\n"; }
