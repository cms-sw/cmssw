 ...
 public: // interface
  ...
  bool TopPag12LjetsCuts( const pat::Muon & muon, pat::strbitset & ret){
    ...
    if ( muon.globalTrack().isNonnull() && muon.globalTrack().isAvailable() ){
      ...
      minTrackerLayers = static_cast<int> (muon.track()->getHitPattern().trackerLayersWithMeasurement(reco::HitPattern::TRACK_HITS));
      minValidMuonHits = static_cast<int> (muon.globalTrack()->getHitPattern().numberOfValidMuonHits(reco::HitPattern::TRACK_HITS));
      _ip = muon.dB();
      minPixelHits = muon.innerTrack()->getHitPattern().numberOfValidPixelHits(reco::HitPattern::TRACK_HITS);
    }
