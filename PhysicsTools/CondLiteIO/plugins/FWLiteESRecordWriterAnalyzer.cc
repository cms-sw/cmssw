...
FWLiteESRecordWriterAnalyzer::update(const edm::EventSetup& iSetup)
{
   ...
   if(m_handlers.empty()) {
      ...
      for(auto & m_recordToDataName : m_recordToDataNames) {
         ...
         if(tt == HCTypeTag()) {
            throw cms::Exception("UnknownESRecordType")<<"The name '"<<m_recordToDataName.first<<"' is not associated with a known EventSetupRecord.\n"
         ...
         if(0==rec) {
            throw cms::Exception("UnknownESRecordType")<<"The name '"<<m_recordToDataName.first<<"' is not associated with a type which is not an EventSetupRecord.\n"
         ...
         if(data.empty()) {
            ...
            for(auto & key : keys) {
               data.push_back(std::make_pair(std::string(key.type().name()),
         ...
         for (auto & itData : data) {
            ...
            if(tt == HCTypeTag()) {
               throw cms::Exception("UnknownESDataType")<<"The name '"<<itData.first<<"' is not associated with a known type held in the "<<m_recordToDataName.first<<" Record.\n"
            ...
            if(!bool(edm::TypeWithDict( tt.value() ))) {
               throw cms::Exception("NoDictionary")<<"The type '"<<itData.first<<"' can not be retrieved from the Record "<<m_recordToDataName.first<<" and stored \n"
