...
void MuonFSRProducer::produce(edm::StreamID streamID, edm::Event& iEvent, const edm::EventSetup& iSetup) const {
  ...
  for (auto muon = muons->begin(); muon != muons->end(); ++muon) {
    ...
    for (auto iter_pf = pfcands->begin(); iter_pf != pfcands->end(); iter_pf++) {
      ...
      // eta requirements
      if (fabs(pc.eta()) > 1.4442 and (fabs(pc.eta()) < 1.566))
        continue;
      if (fabs(pc.eta()) > 2.5)
        continue;
      //cout<<"ETA pass!"<<endl;
      ...
      if (deltaR(muon->eta(), muon->phi(), pc.eta(), pc.phi()) < 0.0001)
        continue;
      if (deltaR(muon->eta(), muon->phi(), pc.eta(), pc.phi()) > 0.5)
        continue;
...
double MuonFSRProducer::computeRelativeIsolation(const pat::PackedCandidate& photon,
  ...
  for (auto pfcand : pfcands) {
    ...
    if (deltaR(photon.eta(), photon.phi(), pfcand.eta(), pfcand.phi()) > isoConeMax)
      continue;
    // footprint removal
    if (deltaR(photon.eta(), photon.phi(), pfcand.eta(), pfcand.phi()) < isoConeMin)
      continue;
    ...
    if (pfcand.vertexRef().isNonnull()) {
      if (abs(pfcand.pdgId()) == 211) {  // charged hadrons
        if (pfcand.fromPV(0) <= 0)
          continue;
        isoval_charged += pfcand.pt();
      } else if (abs(pfcand.pdgId()) == 22) {  // photons
        isoval_gamma += pfcand.pt();
      } else if (abs(pfcand.pdgId()) != 11 and abs(pfcand.pdgId()) != 13 and abs(pfcand.pdgId()) != 15 and
                 abs(pfcand.pdgId()) != 24) {
