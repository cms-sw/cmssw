...
private:
  ...
  const float Scale(const Int_t, const Float_t, const bool) const;
  void saveInputs(const tensorflow::Tensor& tensor,
                  const std::string& block_name,
                  const edm::EventID& eventId,
                  size_t jetIndex) const;

  const std::string graphPath_;
  const edm::EDGetTokenT<pat::JetCollection> jets_token;
  const edm::EDGetTokenT<pat::PackedCandidateCollection> cands_token;
  const bool save_inputs_;
  const unsigned batch_size_;

  // TensorFlow resources (mutable for thread safety)
  mutable std::mutex session_mutex_;
  mutable tensorflow::GraphDef* graphDef_;
...
template <typename FeatureT>
const float DisTauTag::Scale(const Int_t idx, const Float_t value, const bool inner) const {
...
void DisTauTag::produce(edm::StreamID, edm::Event& event, const edm::EventSetup& setup) const {
  ...
  for (size_t batch_idx = 0; batch_idx < n_batches; ++batch_idx) {
    ...
    tensorflow::Tensor input_1(tensorflow::DT_FLOAT,
                               {static_cast<int>(current_batch_size), Setup::nSeq_PfCand, Setup::n_PfCand});
    tensorflow::Tensor input_2(tensorflow::DT_FLOAT,
                               {static_cast<int>(current_batch_size), Setup::nSeq_PfCand, Setup::n_PfCandCategorical});
    fill_zero(input_1);
    fill_zero(input_2);
    ...
    for (size_t batch_jet_idx = 0; batch_jet_idx < current_batch_size; ++batch_jet_idx) {
      ...
      while (tensor_idx < static_cast<size_t>(Setup::nSeq_PfCand) && daughter_idx < nDaughters) {
    ...
    if (save_inputs_) {
      for (size_t batch_jet_idx = 0; batch_jet_idx < current_batch_size; ++batch_jet_idx) {
        ...
        tensorflow::Tensor single_input_1(tensorflow::DT_FLOAT, {1, Setup::nSeq_PfCand, Setup::n_PfCand});
        tensorflow::Tensor single_input_2(tensorflow::DT_FLOAT, {1, Setup::nSeq_PfCand, Setup::n_PfCandCategorical});

        for (size_t i = 0; i < static_cast<size_t>(Setup::nSeq_PfCand); ++i) {
          for (size_t j = 0; j < static_cast<size_t>(Setup::n_PfCand); ++j) {
            single_input_1.tensor<float, 3>()(0, i, j) = input_1.tensor<float, 3>()(batch_jet_idx, i, j);
          }
          for (size_t j = 0; j < static_cast<size_t>(Setup::n_PfCandCategorical); ++j) {
