...
void IsoValueMapProducer<pat::Photon>::doPFIsoPho(edm::Event& iEvent) const {
  ...
  for (const auto& obj : *src) {
    ...
    PFIsoAllQuadratic.push_back(PFIsoChg.back() +
  ...
  std::unique_ptr<edm::ValueMap<float>> PFIsoChgQuadraticV( new edm::ValueMap<float>() );
  ...
  std::unique_ptr<edm::ValueMap<float>> PFIsoAllQuadraticV( new edm::ValueMap<float>() );
...
void IsoValueMapProducer<T>::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  if ((typeid(T) == typeid(pat::Photon))) {
    ...
    desc.add<edm::InputTag>("mapIsoEcal")->setComment("input ecal PF isolation calculated in VID for photons");
    desc.add<edm::InputTag>("mapIsoHcal")->setComment("input hcal PF isolation calculated in VID for photons");
