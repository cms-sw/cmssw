...
for sel in selections80X:
    if sel.name=='Muon':
        ...
        sel.qualityBits = cms.string("filter('*CaloIdLTrackIdLIsoVL*TrackIso*Filter') + 2*filter('hltEle*WPTight*TrackIsoFilter*') + 4*filter('hltEle*WPLoose*TrackIsoFilter') + 8*filter('*OverlapFilter*IsoEle*PFTau*') + 2048*filter('*Ele*CaloIdVTGsfTrkIdTGsf*Filter') + 4096*path('HLT_Ele*PFJet*') + 8192*filter('hltEG175HEFilter')")
        sel.qualityBitsDoc = cms.string("1 = CaloIdL_TrackIdL_IsoVL, 2 = WPTight, 4 = WPLoose, 8 = OverlapFilter PFTau, 2048 = 1e (CaloIdVT_GsfTrkIdT), 4096 = 1e (PFJet), 8192 = 1e (Photon175)")
