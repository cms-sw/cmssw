...
genParticleTable = cms.EDProducer("SimpleCandidateFlatTableProducer",
    ...
    variables = cms.PSet(
         pt  = Var("pt",  float, precision=cms.string("(abs(pdgId)==11|| abs(pdgId)==13 || abs(pdgId)==15||abs(pdgId)== 12 || abs(pdgId)== 14 || abs(pdgId)== 16|| abs(pdgId)== 6|| abs(pdgId)== 24|| pdgId== 23|| pdgId== 25)?24:8")),
         phi = Var("phi", float,precision=cms.string(("(abs(pdgId)==11|| abs(pdgId)==13 || abs(pdgId)==15||abs(pdgId)== 12 || abs(pdgId)== 14 || abs(pdgId)== 16|| abs(pdgId)== 6|| abs(pdgId)== 24|| pdgId== 23|| pdgId== 25)?24:8")),
         eta  = Var("eta",  float,precision=cms.string(("(abs(pdgId)==11|| abs(pdgId)==13 || abs(pdgId)==15||abs(pdgId)== 12 || abs(pdgId)== 14 || abs(pdgId)== 16|| abs(pdgId)== 6|| abs(pdgId)== 24|| pdgId== 23|| pdgId== 25)?24:8")),
         mass = Var("?mass>10 || (pdgId==22 && mass > 1) || abs(pdgId)==24 || pdgId==23?mass:0", float,precision=cms.string("(abs(pdgId)== 6|| abs(pdgId)== 24|| pdgId== 23|| pdgId== 25)?24:8"),doc="Mass stored for all particles with mass > 10 GeV and photons with mass > 1 GeV. For other particles you can lookup from PDGID"),
