...
muonMVATTH= cms.EDProducer("MuonBaseMVAValueMapProducer",
    ...
    variablesOrder = cms.vstring(["LepGood_pt","LepGood_eta","LepGood_jetNDauChargedMVASel","LepGood_miniRelIsoCharged","LepGood_miniRelIsoNeutral","LepGood_jetPtRelv2","LepGood_jetBTagCSV","LepGood_jetPtRatio","LepGood_dxy","LepGood_sip3d","LepGood_dz","LepGood_segmentComp"]),
    variables = cms.PSet(
...
for modifier in run2_miniAOD_80XLegacy, run2_nanoAOD_94X2016:
    modifier.toModify(muonMVATTH,
        weightFile = "RecoMuon/MuonIdentification/data/mu_2016_BDTG.weights.xml",
    )
...
muonTable = cms.EDProducer("SimpleCandidateFlatTableProducer",
    ...
    variables = cms.PSet(CandVars,
        ...
        LepGood_pt = Var("pt",float,doc="kk"),
        LepGood_eta = Var("eta",float,doc="kk"),
        LepGood_jetNDauChargedMVASel = Var("?userCand('jetForLepJetVar').isNonnull()?userFloat('jetNDauChargedMVASel'):0",int,doc="kk"),
        LepGood_miniRelIsoCharged = Var("userFloat('miniIsoChg')/pt",float,doc="kk"),
        LepGood_miniRelIsoNeutral = Var("(userFloat('miniIsoAll')-userFloat('miniIsoChg'))/pt",float,doc="kk"),
        LepGood_jetPtRelv2 = Var("?userCand('jetForLepJetVar').isNonnull()?userFloat('ptRel'):0",float,doc="kk"),
        LepGood_jetBTagCSV = Var("?userCand('jetForLepJetVar').isNonnull()?max(userCand('jetForLepJetVar').bDiscriminator('pfCombinedInclusiveSecondaryVertexV2BJetTags'),0.0):0.0",float,doc="kk"),
        LepGood_jetPtRatio = Var("?userCand('jetForLepJetVar').isNonnull()?min(userFloat('ptRatio'),1.5):1.0/(1.0+(pfIsolationR04().sumChargedHadronPt + max(pfIsolationR04().sumNeutralHadronEt + pfIsolationR04().sumPhotonEt - pfIsolationR04().sumPUPt/2,0.0))/pt)",float,doc="kk"),
        LepGood_dxy = Var("log(abs(dB('PV2D')))",float,doc="kk"),
        LepGood_sip3d = Var("abs(dB('PV3D')/edB('PV3D'))",float,doc="kk"),
        LepGood_dz = Var("log(abs(dB('PVDZ')))",float,doc="kk"),
        LepGood_segmentComp = Var("segmentCompatibility",float,doc="kk"),
                     ),
