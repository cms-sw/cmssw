...
svCandidateTable =  simpleCandidateFlatTableProducer.clone(
    ...
    variables = cms.PSet(P4Vars,
        x   = Var("position().x()", float, doc = "secondary vertex X position, in cm",precision=10, lazyEval=True),
        y   = Var("position().y()", float, doc = "secondary vertex Y position, in cm",precision=10, lazyEval=True),
        z   = Var("position().z()", float, doc = "secondary vertex Z position, in cm",precision=14, lazyEval=True),
        ndof    = Var("vertexNdof()", float, doc = "number of degrees of freedom",precision=8, lazyEval=True),
        chi2    = Var("vertexNormalizedChi2()", float, doc = "reduced chi2, i.e. chi/ndof",precision=8, lazyEval=True),
