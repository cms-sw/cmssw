    ...
    vplots = cms.PSet(
        Electron = cms.PSet(
            ...
            plots = cms.VPSet(
                ...
                Plot1D('dxy', 'dxy', 20, -8, 8, 'dxy (with sign) wrt first PV, in cm'),
                Plot1D('dxyErr', 'dxyErr', 20, 0, 2, 'dxy uncertainty, in cm'),
                Plot1D('dz', 'dz', 20, 0, 30, 'dz (with sign) wrt first PV, in cm'),
                Plot1D('dzErr', 'dzErr', 20, 0, 4, 'dz uncertainty, in cm'),
                ...
                Plot1D('hoe', 'hoe', 20, 0, 5, 'H over E'),
                Plot1D('ip3d', 'ip3d', 20, 0, 10, '3D impact parameter wrt first PV, in cm'),
                Plot1D('jetIdx', 'jetIdx', 15, -1.5, 13.5, 'index of the associated jet (-1 if none)'),
                Plot1D('lostHits', 'lostHits', 2, -0.5, 1.5, 'number of missing inner hits'),
                Plot1D('mass', 'mass', 20, -0.2, 0.2, 'mass'),
                ...
                Plot1D('pdgId', 'pdgId', 20, -20, 20, 'PDG code assigned by the event reconstruction (not by MC truth)'),
                Plot1D('phi', 'phi', 20, -3.14159, 3.14159, 'phi'),
                NoPlot('photonIdx'),
                Plot1D('pt', 'pt', 20, 0, 200, "pt*userFloat('eCorr')"),
                Plot1D('r9', 'r9', 20, 0, 30, 'R9 of the supercluster, calculated with full 5x5 region'),
                Plot1D('sieie', 'sieie', 20, 0, 0.08, 'sigma_IetaIeta of the supercluster, calculated with full 5x5 region'),
        ...
        FatJet = cms.PSet(
            ...
            plots = cms.VPSet(
                ...
                Plot1D('subJetIdx1', 'subJetIdx1', 1, -1.5, -0.5, 'index of first subjet'),
                Plot1D('subJetIdx2', 'subJetIdx2', 1, -1.5, -0.5, 'index of second subjet'),
                Plot1D('subJetIdx3', 'subJetIdx3', 1, -1.5, -0.5, 'index of third subjet'),
        ...
        Jet = cms.PSet(
            ...
            plots = cms.VPSet(
                ...
                Plot1D('btagCMVA', 'btagCMVA', 20, -1, 1, 'CMVA V2 btag discriminator'),
                Plot1D('btagDeepB', 'btagDeepB', 20, -1, 1, 'DeepCSV b tag discriminator'),
                Plot1D('btagDeepBB', 'btagDeepBB', 20, -1, 1, 'DeepCSV bb tag discriminator'),
                Plot1D('btagDeepC', 'btagDeepC', 20, -1, 1, 'DeepCSV charm btag discriminator'),
                ...
                Plot1D('neEmEF', 'neEmEF', 20, 0, 2, 'charged Electromagnetic EnergyFraction'),
                Plot1D('neHEF', 'neHEF', 20, 0, 3, 'neutral Hadron Energy Fraction'),
                Plot1D('partonFlavour', 'partonFlavour', 20, -30, 30, 'flavour from parton matching'),
                Plot1D('phi', 'phi', 20, -3.14159, 3.14159, 'phi'),
                Plot1D('pt', 'pt', 20, 0, 400, 'pt'),
                Plot1D('puId', 'puId', 8, -0.5, 7.5, 'Pilup ID flags'),
                Plot1D('qgl', 'qgl', 20, -1, 1, 'Quark vs Gluon likelihood discriminator'),
                Plot1D('rawFactor', 'rawFactor', 20, -2, 2, '1 - Factor to get back to raw pT'),
            )
        ),
        MET = cms.PSet(
            sels = cms.PSet(),
        ...
        Muon = cms.PSet(
            sels = cms.PSet(
                good = cms.string('pt > 20 && abs(dxy) < 5 * dxyErr')
            ),
            plots = cms.VPSet(
                ...
                Plot1D('charge', 'charge', 3, -1.5, 1.5, 'electric charge'),
                Plot1D('cleanmask', 'cleanmask', 2, -0.5, 1.5, 'simple cleaning mask with priority to leptons'),
                Plot1D('dxy', 'dxy', 20, -4, 4, 'dxy (with sign) wrt first PV, in cm'),
                Plot1D('dxyErr', 'dxyErr', 20, 0, 0.3, 'dxy uncertainty, in cm'),
                Plot1D('dz', 'dz', 20, 0, 30, 'dz (with sign) wrt first PV, in cm'),
                Plot1D('dzErr', 'dzErr', 20, 0, 3, 'dz uncertainty, in cm'),
                Plot1D('eta', 'eta', 20, -3, 3, 'eta'),
                Plot1D('highPtId', 'highPtId', 3, -0.5, 2.5, 'POG highPt muon ID (1 = tracker high pT, 2 = global high pT, which includes tracker high pT)'),
                Plot1D('ip3d', 'ip3d', 20, 0, 20, '3D impact parameter wrt first PV, in cm'),
                NoPlot('jetIdx'),
                Plot1D('mass', 'mass', 20, 0.2, 0.2, 'mass'),
                ...
                Plot1D('sip3d', 'sip3d', 20, 0, 10000, '3D impact parameter significance wrt first PV'),
                Plot1D('softId', 'softId', 2, -0.5, 1.5, 'POG Soft muon ID (using the relaxed cuts in the data Run 2016 B-F periods, and standard cuts elsewhere)'),
                Plot1D('tightCharge', 'tightCharge', 1, 1.5, 2.5, 'Tight charge criterion using pterr/pt of muonBestTrack (0:fail, 2:pass)'),
                Plot1D('tightId', 'tightId', 2, -0.5, 1.5, 'POG Tight muon ID'),
            )
        ),
        OtherPV = cms.PSet(
            sels = cms.PSet(),
        ...
        PV = cms.PSet(
            ...
            plots = cms.VPSet(
                ...
                Plot1D('x', 'x', 20, -0.04, -0.03, 'main primary vertex position x coordinate'),
                Plot1D('y', 'y', 20, 0.07, 0.1, 'main primary vertex position y coordinate'),
        ...
        Photon = cms.PSet(
            ...
            plots = cms.VPSet(
                ...
                Plot1D('jetIdx', 'jetIdx', 12, -1.5, 10.5, 'index of the associated jet (-1 if none)'),
                Plot1D('mass', 'mass', 20, -2e-05, 2e-05, 'mass'),
                ...
                Plot1D('mcMatchIdx', 'mcMatchIdx', 20, -200, 200, 'Index into genParticle list for MC matching to status==1 photons or electrons'),
                ...
                Plot1D('pt', 'pt', 20, 0, 200, "pt*userFloat('eCorr')"),
                Plot1D('r9', 'r9', 20, 0, 20, 'R9 of the supercluster, calculated with full 5x5 region'),
                Plot1D('sieie', 'sieie', 20, 0.004, 0.07, 'sigma_IetaIeta of the supercluster, calculated with full 5x5 region'),
            )
        ),
        Pileup = cms.PSet(
            sels = cms.PSet(),
        ...
        SV = cms.PSet(
            ...
            plots = cms.VPSet(
                ...
                Plot1D('dlen', 'dlen', 20, 0, 200, 'decay length in cm'),
                Plot1D('dlenSig', 'dlenSig', 20, 0, 900, 'decay length significance'),
                Plot1D('eta', 'eta', 20, -3, 3, 'eta'),
                Plot1D('mass', 'mass', 20, 0, 20, 'mass'),
                Plot1D('ndof', 'ndof', 20, -30, 30, 'number of degrees of freedom'),
                Plot1D('phi', 'phi', 20, -3.14159, 3.14159, 'phi'),
                Plot1D('pt', 'pt', 20, 0, 200, 'pt'),
                Plot1D('x', 'x', 20, -50, 50, 'secondary vertex X position, in cm'),
                Plot1D('y', 'y', 20, -20, 20, 'secondary vertex Y position, in cm'),
                Plot1D('z', 'z', 20, -200, 200, 'secondary vertex Z position, in cm'),
            )
        ),
        ...
        TrigObj = cms.PSet(
            ...
            plots = cms.VPSet(
                ...
                Plot1D('l1pt', 'l1pt', 20, 0, 0, 'pt of associated L1 seed'),
                Plot1D('l2pt', 'l2pt', 20, 0, 3000, "pt of associated 'L2' seed (i.e. HLT before tracking/PF)"),
                Plot1D('phi', 'phi', 20, -3.14159, 3.14159, 'phi'),
                Plot1D('pt', 'pt', 20, 0, 200, 'pt'),
            )
        ),
