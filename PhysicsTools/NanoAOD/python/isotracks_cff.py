...
isoTrackTable = simpleCandidateFlatTableProducer.clone(
    ...
    variables = cms.PSet(P3Vars,
        ...
        pfRelIso03_chg = Var("pfIsolationDR03().chargedHadronIso/pt",float,doc="PF relative isolation dR=0.3, charged component",precision=10, lazyEval=True),
        pfRelIso03_all = Var("(pfIsolationDR03().chargedHadronIso + max(pfIsolationDR03().neutralHadronIso + pfIsolationDR03().photonIso - pfIsolationDR03().puChargedHadronIso/2,0.0))/pt",float,doc="PF relative isolation dR=0.3, total (deltaBeta corrections)",precision=10, lazyEval=True),
        isPFcand = Var("packedCandRef().isNonnull()",bool,doc="if isolated track is a PF candidate", lazyEval=True),
        fromPV = Var("fromPV", "int16", doc="isolated track comes from PV", lazyEval=True),
        pdgId = Var("pdgId",int,doc="PDG id of PF cand"),
        isHighPurityTrack = Var("isHighPurityTrack",bool,doc="track is high purity", lazyEval=True),
