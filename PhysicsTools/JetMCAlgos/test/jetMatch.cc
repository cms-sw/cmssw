...
void jetMatch::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  cout << "**********************" << endl;
  for (CandViewMatchMap::const_iterator f = matchedjetsOne->begin(); f != matchedjetsOne->end(); f++) {
  ...
  cout << "**********************" << endl;
  for (CandMatchMapMany::const_iterator f = matchedjetsMany->begin(); f != matchedjetsMany->end(); f++) {
    const Candidate* sourceRef = &*(f->key);

    printf(
        "[GenJetTest] (pt,eta,phi) source = %6.2f %5.2f %5.2f\n", sourceRef->et(), sourceRef->eta(), sourceRef->phi());
