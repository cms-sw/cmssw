...
void matchOneToOne::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  try {
    ...
    iEvent.getByToken(matchedjetsOne2Token_, matchedjetsOne2);
  ...
  cout << "**********************" << endl;
  for (CandViewMatchMap::const_iterator f = matchedjetsOne1->begin(); f != matchedjetsOne1->end(); f++) {
  ...
  for (CandViewMatchMap::const_iterator f = matchedjetsOne2->begin(); f != matchedjetsOne2->end(); f++) {
    const Candidate* sourceRef = &*(f->key);
    const Candidate* matchRef = &*(f->val);
    dR = DeltaR(sourceRef->p4(), matchRef->p4());
