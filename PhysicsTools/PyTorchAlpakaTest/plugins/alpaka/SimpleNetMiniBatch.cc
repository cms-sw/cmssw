...
namespace ALPAKA_ACCELERATOR_NAMESPACE::torchtest {
  ...
  public:
    SimpleNetMiniBatch(const edm::ParameterSet &params)
        : EDProducer<>(params),
          particlesFPX_token_(consumes(params.getParameter<edm::InputTag>("particles"))),
    ...
    void produce(device::Event &event, const device::EventSetup &event_setup) override {
      // if it's the first event and the device is 'cuda' move the model to device in FP16 precision
      if (isFirstEvent_) {
        isFirstEvent_ = false;
#if defined ALPAKA_ACC_GPU_CUDA_ENABLED
        model_.to(event.queue(), ::torch::kHalf);
#endif
      }
      // in/out collections
      const auto &particlesFPX = event.get(particlesFPX_token_);
      auto regression_collectionFPX = portabletest::SimpleNetDeviceCollectionFPX(event.queue(), total_size_);
      auto n_batches = total_size_ / batch_size_;

      // records
      auto input_records = particlesFPX.const_view().records();
      auto output_records = regression_collectionFPX.view().records();
      ...
      event.emplace(simple_net_token_, std::move(regression_collectionFPX));
    }

  private:
    // event query tokens
    const device::EDGetToken<portabletest::ParticleDeviceCollectionFPX> particlesFPX_token_;
    const device::EDPutToken<portabletest::SimpleNetDeviceCollectionFPX> simple_net_token_;
    // model
