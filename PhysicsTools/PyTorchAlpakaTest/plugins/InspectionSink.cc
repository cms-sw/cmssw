...
namespace torchtest {
  ...
  private:
    ...
    const edm::EDGetTokenT<portabletest::SimpleNetHostCollectionFPX> simple_net_batch_token_;
    ...
    void print(const ViewT& simple_net,
               std::string_view simple_net_backend,
      ...
      if (range < size) {  // <-- small fix here (see below)
        fmt::format_to(std::back_inserter(buffer), "| {:>5} | {:>7} |\n", "...", "...");
      }
