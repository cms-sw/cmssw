...
FloatArrays ONNXRuntime::run(const std::vector<std::string>& input_names, FloatArrays input_values,
    const std::vector<std::string>& output_names, int64_t batch_size) const {
  ...
  for (const auto &name : input_node_strings_){
    ...
    auto input_tensor = Value::CreateTensor<float>(memory_info, value->data(), value->size(),
  ...
  // run
  auto output_tensors = session_->Run(RunOptions{nullptr}, input_node_names_.data(), input_tensors.data(), input_tensors.size(),
