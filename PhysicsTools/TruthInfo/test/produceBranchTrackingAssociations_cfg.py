...
# Smoke test for the reco-track<->Branch AssociationMap producer: rebuilds the
# truth-graph chain and runs truthBranchTrackingAssociationProducer, writing the two
# association maps (trackToBranch, branchToTrack) to a pool file.
...
parser.add_argument('-o', "--out", default="branch_tracking_assoc.root")
args = parser.parse_args()
if '/' not in args.inputFile and ':' not in args.inputFile:
    args.inputFile = 'file:' + args.inputFile

process = cms.Process("BRANCHTRKASSOC")
...
process.truthBranchTrackingAssociationProducer = cms.EDProducer(
...
process.out = cms.OutputModule(
    ...
    outputCommands=cms.untracked.vstring(
        "drop *",
        "keep *_truthBranchTrackingAssociationProducer_*_*",
