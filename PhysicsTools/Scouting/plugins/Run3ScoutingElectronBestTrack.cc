...
void Run3ScoutingElectronBestTrackProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (size_t iElectron = 0; iElectron < num_electrons; ++iElectron) {
    ...
    if (besttrk_idx[iElectron] >= 0) {
      besttrk_d0s[iElectron] = electron.trkd0().at(besttrk_idx[iElectron]);
      besttrk_dzs[iElectron] = electron.trkdz().at(besttrk_idx[iElectron]);
      besttrk_pts[iElectron] = electron.trkpt().at(besttrk_idx[iElectron]);
      besttrk_etas[iElectron] = electron.trketa().at(besttrk_idx[iElectron]);
      besttrk_phis[iElectron] = electron.trkphi().at(besttrk_idx[iElectron]);
      besttrk_pModes[iElectron] = electron.trkpMode().at(besttrk_idx[iElectron]);
      besttrk_etaModes[iElectron] = electron.trketaMode().at(besttrk_idx[iElectron]);
      besttrk_phiModes[iElectron] = electron.trkphiMode().at(besttrk_idx[iElectron]);
      besttrk_qoverpModeErrors[iElectron] = electron.trkqoverpModeError().at(besttrk_idx[iElectron]);
      besttrk_chi2overndfs[iElectron] = electron.trkchi2overndf().at(besttrk_idx[iElectron]);
      besttrk_charges[iElectron] = electron.trkcharge().at(besttrk_idx[iElectron]);
    }
  }
