...
//,vetos_(new AbsVetos())
{
  ...
  for (const auto &veto : vetos) {
    vetos_.push_back(IsoDepositVetoFactory::make(veto.c_str(), evdep, iC));
    if (evdep)
...
double CandIsolatorFromDeposits::SingleDeposit::compute(const reco::CandidateBaseRef &cand) {
  ...
  switch (mode_) {
    case Count:
      return weight * dep.countWithin(deltaR_, vetos_, skipDefaultVeto_);
    case Sum:
      return weight * dep.sumWithin(deltaR_, vetos_, skipDefaultVeto_);
    case SumRelative:
      return weight * dep.sumWithin(deltaR_, vetos_, skipDefaultVeto_) / dep.candEnergy();
    case Sum2:
      return weight * dep.sum2Within(deltaR_, vetos_, skipDefaultVeto_);
    case Sum2Relative:
      return weight * dep.sum2Within(deltaR_, vetos_, skipDefaultVeto_) / (dep.candEnergy() * dep.candEnergy());
    case Max:
      return weight * dep.maxWithin(deltaR_, vetos_, skipDefaultVeto_);
    case NearestDR:
      return weight * dep.nearestDR(deltaR_, vetos_, skipDefaultVeto_);
    case MaxRelative:
      return weight * dep.maxWithin(deltaR_, vetos_, skipDefaultVeto_) / dep.candEnergy();
    case MeanDR:
      return weight * dep.algoWithin<reco::IsoDeposit::MeanDRAlgo>(deltaR_, vetos_, skipDefaultVeto_);
    case SumDR:
      return weight * dep.algoWithin<reco::IsoDeposit::SumDRAlgo>(deltaR_, vetos_, skipDefaultVeto_);
  }
...
CandIsolatorFromDeposits::CandIsolatorFromDeposits(const ParameterSet &par) {
  ...
  for (const auto &depPSet : depPSets) {
    sources_.emplace_back(depPSet, consumesCollector());
  }
  if (sources_.empty())
