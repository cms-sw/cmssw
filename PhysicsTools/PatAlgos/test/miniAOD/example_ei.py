...
process.pfNoPU = cms.EDFilter("CandPtrSelector", src = cms.InputTag("packedPFCandidates"), cut = cms.string("fromPV"))
process.selectedMuons = cms.EDFilter("CandPtrSelector", src = cms.InputTag("slimmedMuons"), cut = cms.string('''abs(eta)<2.5 && pt>10. &&
    (pfIsolationR04().sumChargedHadronPt) / pt < 0.10 && 
    (isPFMuon && (isGlobalMuon || isTrackerMuon) )'''))
process.pfNoMuon =  cms.EDProducer("CandPtrProjector", src = cms.InputTag("pfNoPU"), veto = cms.InputTag("selectedMuons"))
...
process.ak5PFJetsCHS = ak5PFJets.clone(src = 'pfNoMuon', doAreaFastjet = True) # no idea while doArea is false by default, but it's True in RECO so we have to set it
...
process.OUT = cms.OutputModule("PoolOutputModule",
    ...
    outputCommands = cms.untracked.vstring(['drop *','keep patJets_patJets_*_*','keep *_*_*_PAT','keep *_selectedMuons_*_*','keep *_pfNoPU_*_*','keep *_pfNoMuon_*_*'])
)
