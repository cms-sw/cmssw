...
namespace pat {
  ...
  template<typename C1, typename C2, typename S, 
	   typename D = pat::PATMatchByDR<typename C1::value_type, 
	   ...
	   helper::LessByMatchDistance< DeltaR<typename C1::value_type, 
					...
					C1, C2 >
  >
  class PATCandMatcher : public edm::EDProducer {
  public:
    PATCandMatcher(const edm::ParameterSet & cfg);
    ~PATCandMatcher();
  private:
...
//     DeltaR<typename C1::value_type, typename C2::value_type> testDR_;
  ...
  PATCandMatcher<C1, C2, S, D, Q>::PATCandMatcher(const edm::ParameterSet & cfg) :
    config_(cfg),
  ...
  template<typename C1, typename C2, typename S, typename D, typename Q>
  PATCandMatcher<C1, C2, S, D, Q>::~PATCandMatcher() { }

  template<typename C1, typename C2, typename S, typename D, typename Q>    
  void PATCandMatcher<C1, C2, S, D, Q>::produce(edm::Event& evt, const edm::EventSetup&) {
    using namespace edm;
    using namespace std;
    typedef std::pair<size_t, size_t> IndexPair;
    typedef std::vector<IndexPair> MatchContainer;
