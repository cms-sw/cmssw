...
pat::GenJetFlavourInfoPreserver::produce(edm::Event & iEvent, const edm::EventSetup & iSetup) {
    ...
    for( edm::View<reco::GenJet>::const_iterator genJetsIt = genJets->begin(); genJetsIt != genJets->end(); ++genJetsIt){
        slimmedGenJetRef = (*slimmedGenJetAssociation)[genJets->refAt(genJetsIt-(genJets->begin()))];

        if(!slimmedGenJetRef) continue;
        (*jetFlavourInfos)[reco::JetBaseRef(slimmedGenJetRef)] = (*genJetFlavourInfos)[genJetsIt-(genJets->begin())].second;
    }
