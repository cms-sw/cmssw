...
namespace pat {
  ...
  class PackedCandidateTrackChi2Producer : public edm::stream::EDProducer<> {
    ...
    public:
      ...
      {
        if(doLostTracks_){
          track2LostTrackToken_ = consumes<edm::Association<pat::PackedCandidateCollection> >( iConfig.getParameter<edm::InputTag>("candidates"));
        }
        else{
          candidate2PFToken_ = consumes<edm::Association<reco::PFCandidateCollection> >( iConfig.getParameter<edm::InputTag>("candidates"));
        }
