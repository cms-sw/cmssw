    ...
    if ( addL1Algos_ ) {
      ...
      for (const auto & l1GtAlgorithm : l1GtAlgorithms) {
        ...
        if ( ! ( l1GtAlgorithm.second.algoBitNumber() < int( L1GlobalTriggerReadoutSetup::NumberPhysTriggers ) ) ) {
          LogError( "l1Algo" ) << "L1 physics algorithm '" << algoName << "' has bit number " << l1GtAlgorithm.second.algoBitNumber() << " >= " << L1GlobalTriggerReadoutSetup::NumberPhysTriggers << "\n"
                               << "Skipping";
          continue;
        }
        ...
        TriggerAlgorithm triggerAlgo( algoName, l1GtAlgorithm.second.algoAlias(), category == L1GtUtils::TechnicalTrigger, (unsigned)bit, (unsigned)prescale, (bool)mask, decisionBeforeMask, decisionAfterMask );
...
//         if ( ( algorithmResult != decisionBeforeMask ) && ( decisionBeforeMask == true || prescale == 1 ) ) {
      ...
      for (const auto & l1GtTechTrigger : l1GtTechTriggers) {
        ...
        if ( ! ( l1GtTechTrigger.second.algoBitNumber() < int( L1GlobalTriggerReadoutSetup::NumberTechnicalTriggers ) ) ) {
          LogError( "l1Algo" ) << "L1 technical trigger '" << algoName << "' has bit number " << l1GtTechTrigger.second.algoBitNumber() << " >= " << L1GlobalTriggerReadoutSetup::NumberTechnicalTriggers << "\n"
                               << "Skipping";
          continue;
        }
        L1GtUtils::TriggerCategory category;
        ...
        TriggerAlgorithm triggerAlgo( algoName, l1GtTechTrigger.second.algoAlias(), category == L1GtUtils::TechnicalTrigger, (unsigned)bit, (unsigned)prescale, (bool)mask, decisionBeforeMask, decisionAfterMask );
