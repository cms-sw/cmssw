...
void pat::PATMETSlimmer::maybeReadShifts(const edm::ParameterSet &basePSet, const std::string &name, pat::MET::METCorrectionType level, bool readFromMiniAOD) {
	...
	if(level==pat::MET::T1) {
	  ...
	  shifts_.emplace_back(pat::MET::UnclusteredEnUp,   level, baseTag, consumesCollector(), readFromMiniAOD, false, true);
	  shifts_.emplace_back(pat::MET::UnclusteredEnDown, level, baseTag, consumesCollector(), readFromMiniAOD, false, true);
	}
	  ...
	  shifts_.emplace_back(pat::MET::JetResDown, level, baseTag, consumesCollector(), readFromMiniAOD, false, true, true);
	}
	else {
