...
bool pat::PATLostTracks::passTrkCuts(const reco::Track& tr)const
{
    ...
    bool passTrkPt = tr.pt() > minPt_;
    bool passTrkHits = tr.numberOfValidHits() >= minHits_ && 
                       tr.hitPattern().numberOfValidPixelHits() >= minPixelHits_;
    bool passTrkQual = passesQuality(tr,qualsToAutoAccept_);
    return (passTrkPt && passTrkHits) || passTrkQual;
}
