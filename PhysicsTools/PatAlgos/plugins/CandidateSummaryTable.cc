...
namespace pathelpers {
  struct Record {
    ...
    std::atomic<size_t> present, empty, min, max, total;
  ...
  struct RecordCache { 
    ...
    mutable std::vector<Record> collections_; // size of vector is never altered later! (atomics are in the class below)
  };  
