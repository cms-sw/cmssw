...
slimmedElectrons = cms.EDProducer("PATElectronSlimmer",
   ...
   dropCorrections = cms.string("obj.pt() < 5"), # you can put a cut to slim selectively, e.g. pt < 10
   dropIsolations = cms.string("obj.pt() < 5"), # you can put a cut to slim selectively, e.g. pt < 10
   dropShapes = cms.string("obj.pt() < 5"), # you can put a cut to slim selectively, e.g. pt < 10
   dropExtrapolations  = cms.string("obj.pt() < 5"), # you can put a cut to slim selectively, e.g. pt < 10
   dropClassifications  = cms.string("obj.pt() < 5"), # you can put a cut to slim selectively, e.g. pt < 10
   linkToPackedPFCandidates = cms.bool(True),
   recoToPFMap = cms.InputTag("reducedEgamma","reducedGsfElectronPfCandMap"),
   packedPFCandidates = cms.InputTag("packedPFCandidates"), 
   saveNonZSClusterShapes = cms.string("obj.pt() > 5"), # save additional user floats: (sigmaIetaIeta,sigmaIphiIphi,sigmaIetaIphi,r9,e1x5_over_e5x5)_NoZS 
