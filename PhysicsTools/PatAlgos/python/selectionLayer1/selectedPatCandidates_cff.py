...
(pp_on_AA | _mAOD).toReplaceWith(selectedPatCandidatesTask,
                                 selectedPatCandidatesTask.copyAndExclude([selectedPatLowPtElectrons, selectedPatDisplacedMuons]))
(pp_on_AA | _mAOD).toModify(selectedPatCandidateSummary.candidates,
                            func = lambda list: [l for l in list if l not in [cms.InputTag("selectedPatLowPtElectrons"), cms.InputTag("selectedPatDisplacedMuons")]] )
