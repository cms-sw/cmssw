...
jetTest::jetTest(const edm::ParameterSet& iConfig)
{
  ...
  matchedjets_    = iConfig.getParameter<InputTag> ("matchMap");
...
void jetTest::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  try {
    ...
    iEvent.getByLabel (matchedjets_, matchedjets);
  ...
  float dR=-1.;
  for( CandMatchMap::const_iterator f  = matchedjets->begin();
                                    f != matchedjets->end();
                                    f++) {
