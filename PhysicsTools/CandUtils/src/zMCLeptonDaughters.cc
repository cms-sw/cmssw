...
std::pair<const Candidate*, const Candidate*> zMCLeptonDaughters(const Candidate & z, int leptonPdgId) {
  ...
  for(size_t i0 = 0; i0 < dau0->numberOfDaughters(); ++i0) {
    ...
    if(std::abs(ddau0->pdgId())==leptonPdgId && ddau0->status()==1) {
      dau0 = ddau0; break;
    }
  }
  for(size_t i1 = 0; i1 < dau1->numberOfDaughters(); ++i1) {
    const Candidate * ddau1 = dau1->daughter(i1);
    if(std::abs(ddau1->pdgId())==leptonPdgId && ddau1->status()==1) {
      dau1 = ddau1; break;
    }
  }
  assert(std::abs(dau0->pdgId())==leptonPdgId && dau0->status()==1);
  assert(std::abs(dau1->pdgId())==leptonPdgId && dau1->status()==1);
  return std::make_pair(dau0, dau1);
}
