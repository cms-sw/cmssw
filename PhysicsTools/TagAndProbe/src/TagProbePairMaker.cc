...
tnp::TagProbePairMaker::TagProbePairMaker(const edm::ParameterSet &iConfig, edm::ConsumesCollector &&iC)
  ...
  if (arbitration == "None") {
    ...
    throw cms::Exception("Configuration") << "TagProbePairMakerOnTheFly: the only currently "
...
tnp::TagProbePairs tnp::TagProbePairMaker::run(const edm::Event &iEvent) const {
  ...
  for (reco::CandidateView::const_iterator it = src->begin(), ed = src->end(); it != ed; ++it) {
    ...
    if (mother.numberOfDaughters() != 2)
      throw cms::Exception("CorruptData") << "Tag&Probe pair with " << mother.numberOfDaughters() << " daughters\n";
    pairs.push_back(tnp::TagProbePair(mother.daughter(0)->masterClone(),
                                      mother.daughter(1)->masterClone(),
                                      src->refAt(it - src->begin()),
                                      mother.mass()));
  }
...
void tnp::TagProbePairMaker::phiCutByEventNumber(TagProbePairs &pairs, int eventNumber) const {
  ...
  for (auto &pair : pairs) {
    ...
    if (eventNumber % 2) {
      ...
      if (!(pair.tag->phi() > 0. && pair.tag->phi() < 3.141592654)) {
        ...
        nclean--;
        pair.tag = reco::CandidateBaseRef();
        --nclean;
      } else {
  ...
  if (nclean == 0) {
    ...
    for (auto &pair : pairs) {
      if (pair.tag.isNonnull())
        cleaned.push_back(pair);
    }
    pairs.swap(cleaned);
  }
}
...
void tnp::TagProbePairMaker::arbitrate(TagProbePairs &pairs) const {
  ...
  for (TagProbePairs::iterator it = pairs.begin(), ed = pairs.end(); it != ed; ++it) {
    ...
    for (TagProbePairs::iterator it2 = it + 1; it2 != ed; ++it2) {  // it+1 <= ed, otherwise we don't arrive here
      ...
      if ((arbitration_ == OnePair) && std::abs(it->mass - it2->mass) < 1e-4 &&
          std::abs(it->probe->phi() - it2->tag->phi()) < 1e-5 && std::abs(it->probe->eta() - it2->tag->eta()) < 1e-5 &&
          std::abs(it->probe->pt() - it2->tag->pt()) < 1e-5 && std::abs(it2->probe->phi() - it->tag->phi()) < 1e-5 &&
          std::abs(it2->probe->eta() - it->tag->eta()) < 1e-5 && std::abs(it2->probe->pt() - it->tag->pt()) < 1e-5) {
        it2->tag = reco::CandidateBaseRef();
        --nclean;
      }
    }
    if (invalidateThis) {
      it->tag = reco::CandidateBaseRef();
      --nclean;
    }
  }
