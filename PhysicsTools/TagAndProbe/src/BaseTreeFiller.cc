...
void tnp::BaseTreeFiller::addBranches_(TTree *tree,
  ...
  for (const auto &stringVar : stringVars) {
    vars_.emplace_back(branchNamePrefix + stringVar, variables.getParameter<std::string>(stringVar));
  }
  ...
  for (const auto &inputTagVar : inputTagVars) {
    vars_.emplace_back(branchNamePrefix + inputTagVar,
  ...
  for (const auto &stringFlag : stringFlags) {
    flags_.emplace_back(branchNamePrefix + stringFlag, flags.getParameter<std::string>(stringFlag));
  }
  //.. the ones that are InputTags
  std::vector<std::string> inputTagFlags = flags.getParameterNamesForType<edm::InputTag>();
  for (const auto &inputTagFlag : inputTagFlags) {
    flags_.emplace_back(branchNamePrefix + inputTagFlag,
