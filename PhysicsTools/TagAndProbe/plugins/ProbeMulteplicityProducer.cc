...
void 
ProbeMulteplicityProducer::produce(edm::Event & iEvent, const edm::EventSetup & iSetup) {
    using namespace edm;
    ...
    for (pair = pairs->begin(); pair != endpairs; ++pair, ++i) {
      ...
      if (pairCut_(*pair) && probeCut_(*probeRef)) {
          for (unsigned int j = 0; j < i; ++j) if (tagKeys[j] == tagKey) copies++;
          for (unsigned int j = 0; j < i; ++j) if (tagKeys[j] == tagKey) values[j] = copies;
      } else {
