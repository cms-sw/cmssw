...
class TagProbeFitter {
  ...
  TagProbeFitter(const std::vector<std::string>& inputFileNames, const std::string& inputDirectoryName, const std::string& inputTreeName, const std::string& outputFileName, int numCPU = 1, bool saveWorkspace_ = false, bool floatShapeParameters = true, const std::vector<std::string>& fixVars_ = std::vector<std::string>() );

  ///destructor closes the files
  ~TagProbeFitter();

  ///adds a new real variable to the set of variables describing the data in the tree
  bool addVariable(const std::string& variableName, const std::string& title, double low, double hi, const std::string& units);
  ...
  std::string calculateEfficiency(std::string dirName, const std::string& efficiencyCategory, const std::string& efficiencyState, std::vector<std::string>& unbinnedVariables, std::map<std::string, std::vector<double> >& binnedReals, std::map<std::string, std::vector<std::string> >& binnedCategories, std::vector<std::string>& binToPDFmap) {
    std::vector<std::string> efficiencyCategories(1,efficiencyCategory);
    std::vector<std::string> efficiencyStates(1,efficiencyState);
    return calculateEfficiency(std::move(dirName), efficiencyCategories, efficiencyStates, unbinnedVariables, binnedReals, binnedCategories, binToPDFmap);
  }

  std::string calculateEfficiency(const std::string& dirName,const std::vector<std::string>& efficiencyCategories, const std::vector<std::string>& efficiencyStates, std::vector<std::string>& unbinnedVariables, std::map<std::string, std::vector<double> >& binnedReals, std::map<std::string, std::vector<std::string> >& binnedCategories, std::vector<std::string>& binToPDFmap);
