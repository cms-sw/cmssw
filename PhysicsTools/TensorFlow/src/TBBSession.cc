...
namespace tensorflow {
  ...
  void TBBSession::SchedClosure(tbb::task_arena& task_arena, tbb::task_group& task_group, std::function<void()> c) {
    task_arena.execute([&task_group, &c]() { task_group.run(c); });
  }
  ...
  Status TBBSession::RunInternal(int64 step_id,
    ...
    if (executors_and_keys->collective_graph_key != BuildGraphOptions::kNoCollectiveGraphKey) {
      ...
      if (!collective_executor_mgr_) {
        ...
        std::unique_ptr<ParamResolverInterface> cprl(
            new CollectiveParamResolverLocal(device_mgr_.get(), drl.get(), "/job:localhost/replica:0/task:0"));
    ...
    std::unique_ptr<tbb::task_group, decltype(done_with_task_group)> guard(&task_group, done_with_task_group);
    ...
    WaitForNotification(task_arena,
                        task_group,
                        &run_state,
  ...
  Status TBBSession::Run(const RunOptions& run_options,
    ...
    TF_RETURN_IF_ERROR(RunInternal(step_id, run_options, &call_frame, executors_and_keys, run_metadata));
  ...
  Status TBBSession::CreateGraphs(const BuildGraphOptions& subgraph_options,
    ...
    *collective_graph_key = client_graph->collective_graph_key;
    ...
    if (subgraph_options.callable_options.feed_size() != static_cast<int>(client_graph->feed_types.size())) {
    ...
    if (subgraph_options.callable_options.fetch_size() != static_cast<int>(client_graph->fetch_types.size())) {
  ...
  void TBBSession::WaitForNotification(tbb::task_arena& task_arena,
                                       tbb::task_group& task_group,
                                       RunState* run_state,
    ...
    const Status status = WaitForNotification(task_arena, task_group, &run_state->executors_done, timeout_in_ms);
    if (!status.ok()) {
      {
        mutex_lock l(run_state->mu_);
