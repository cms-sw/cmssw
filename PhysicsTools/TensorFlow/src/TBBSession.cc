...
https://github.com/tensorflow/tensorflow/blob/v2.0.0/tensorflow/core/common_runtime/direct_session.cc
...
namespace tensorflow {
  ...
  public:
    ...
    Status NewSession(const SessionOptions& options, Session** out_session) override {
      ...
      TBBSession* session = new TBBSession(options, new DeviceMgr(std::move(devices)), this);
      {
        mutex_lock l(sessions_lock_);
  ...
  TBBSession::TBBSession(const SessionOptions& options, const DeviceMgr* device_mgr, TBBSessionFactory* const factory)
    ...
    session_handle_ = strings::StrCat("tbb", strings::FpToString(random::New64()));
    int devices_added = 0;
  ...
  Status TBBSession::Create(const GraphDef& graph) { return Create(GraphDef(graph)); }

  Status TBBSession::Create(GraphDef&& graph) {
  ...
  Status TBBSession::Extend(const GraphDef& graph) { return Extend(GraphDef(graph)); }

  Status TBBSession::Extend(GraphDef&& graph) {
    TF_RETURN_IF_ERROR(CheckNotClosed());
    mutex_lock l(graph_state_lock_);
    return ExtendLocked(std::move(graph));
  }

  Status TBBSession::ExtendLocked(GraphDef graph) {
  ...
  Status TBBSession::CreateDebuggerState(const CallableOptions& callable_options,
                                         int64 global_step,
  ...
  Status TBBSession::RunInternal(int64 step_id,
  ...
  Status TBBSession::CreateExecutors(const CallableOptions& callable_options,
  ...
  void TBBSession::WaitForNotification(tbb::task_arena& taskArena,
                                       tbb::task_group& taskGroup,
                                       RunState* run_state,
  ...
  Status TBBSession::MakeCallable(const CallableOptions& callable_options, CallableHandle* out_handle) {
    TF_RETURN_IF_ERROR(CheckNotClosed());
  ...
  ::tensorflow::Status TBBSession::RunCallable(CallableHandle handle,
                                               const std::vector<Tensor>& feed_tensors,
                                               std::vector<Tensor>* fetch_tensors,
                                               RunMetadata* run_metadata) {
