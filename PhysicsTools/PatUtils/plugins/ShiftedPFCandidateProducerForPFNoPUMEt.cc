...
void ShiftedPFCandidateProducerForPFNoPUMEt::produce(edm::Event& evt, const edm::EventSetup& es)
{
  ...
  for (const auto & originalPFCandidate : *originalPFCandidates) {
    ...
    for (auto & selectedJet : selectedJets) {
      for ( std::vector<reco::PFCandidatePtr>::const_iterator jetConstituent = selectedJet->getPFConstituents().begin();
	    jetConstituent != selectedJet->getPFConstituents().end() && jet_matched==nullptr; ++jetConstituent ) {
	if ( deltaR2(originalPFCandidate.p4(), (*jetConstituent)->p4()) < dR2Match ) jet_matched = selectedJet;
      }
    }
