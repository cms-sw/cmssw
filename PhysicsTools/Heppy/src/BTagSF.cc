...
namespace heppy {
  ...
  Bool_t BTagSF::isbtagged(Float_t pt,
                           Float_t eta,
                           Float_t csv,
                           Int_t jetflavor,
                           Bool_t isdata,
    ...
    if (std::abs(jetflavor) == 5) {  // real b-jet
      if (csv > 0.679) {             // if tagged
        ...
        if (promoteProb_btag > 0 && randm->Uniform() < promoteProb_btag)
          ...
          btagged = kFALSE;  // leave it untagged
      }
      return btagged;
    }
    ...
    if (std::abs(jetflavor) == 4) {
      ...
      eff_l =
          0.192 *
          SFl;  // eff_c in MC for CSVM = (-1.5734604211*x*x*x*x +  1.52798999269*x*x*x +  0.866697059943*x*x +  -1.66657942274*x +  0.780639301724), x = 0.679
    } else {
  ...
  Double_t BTagSF::getSFb(Float_t pt, UInt_t btagsys, Bool_t is2012) {
    ...
    Double_t SFb_error_2011[] = {0.0295675,
                                 0.0295095,
                                 0.0210867,
                                 0.0219349,
                                 0.0227033,
    ...
    Double_t SFb_error_2012[] = {0.0415707,
                                 0.0204209,
                                 0.0223227,
                                 0.0206655,
                                 0.0199325,
