...
namespace heppy {
  ...
  TriggerBitChecker::TriggerBitChecker(const std::vector<std::string> &paths) : paths_(paths.size()) {
    for (size_t i = 0; i < paths.size(); ++i)
      paths_[i] = returnPathStruct(paths[i]);
    rmstar();
  }
  ...
  bool TriggerBitChecker::check(const edm::EventBase &event, const edm::TriggerResults &result) const {
    if (result.parameterSetID() != lastID_) {
      syncIndices(event, result);
      lastID_ = result.parameterSetID();
    }
    for (unsigned int indice : indices_) {
      if (result.accept(indice))
        return true;
    }
    return false;
  }

  bool TriggerBitChecker::check_unprescaled(const edm::EventBase &event,
    ...
    bool outcome = true;
    for (unsigned int indice : indices_) {
      if (result.getPrescaleForIndex(indice) != 1) {
        outcome = false;
        break;
      }
    }
