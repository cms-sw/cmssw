...
static const char *plots[] = {
	"allIP", "allIPErr", "allIPSig",
	"trackIP", "trackIPErr", "trackIPSig",
	"negativeIP", "negativeIPErr", "negativeIPSig",
	"allDeltaR", "nTracks",
	0
};
...
void patBJetTracks_showPlots()
{
	...
	for(const char **plot = plots; *plot; plot++) {
		if (i >= 3) {
			canv = new TCanvas(Form("canv%d", j++), "track counting variables", 800, 400);
			canv->Divide(3, 2);
			i -= 3;
		}

		canv->cd(i + 1);
		if (TString(*plot).Contains("IP"))
			gPad->SetLogy(1);
		TH1 *h = (TH1*)file->Get(Form("%s/%s_all", directory, *plot));
		TString title = h->GetTitle();
		if (TString(*plot).Contains("IP") || TString(*plot).Contains("nTrack"))
			title.Resize(title.Index(" in "));
		else if (TString(*plot).Contains("DeltaR")) {
			title.Resize(title.Index(" and "));
			title += " and jet";
		}
		h->SetTitle(title);
		setHistStyle(h);
