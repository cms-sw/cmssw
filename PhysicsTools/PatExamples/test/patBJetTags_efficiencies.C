// name of analyzer
static const char *directory = "analyzeBJetTags";

static const char *algos[] = { "TC", "SSV", "CSV", 0 };
...
void patBJetTags_efficiencies()
{
	...
	// open file
	TFile* file = new TFile("analyzePatBJetTags.root");
	...
	for(const char **algo = algos; *algo; algo++, j++) {
		...
		for(const char **flavour = flavours; *flavour; flavour++, i++) {
			TH1 *h = (TH1*)file->Get(Form("%s/discr%s_%s", directory, *algo, *flavour));
