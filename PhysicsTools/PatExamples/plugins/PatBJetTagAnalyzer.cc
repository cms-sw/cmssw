...
void PatBJetTagAnalyzer::beginJob() {
  ...
  for (unsigned int i = 0; i < N_JET_TYPES; i++) {
    ...
    plots.discrTC = fs->make<TH1F>(
        Form("discrTC_%s", name), Form("track counting (\"high efficiency\") in %s", flavour), 100, 0, 20);
    plots.discrSSV =
        fs->make<TH1F>(Form("discrSSV_%s", name), Form("simple secondary vertex in %s", flavour), 100, 0, 10);
    plots.discrCSV =
        fs->make<TH1F>(Form("discrCSV_%s", name), Form("combined secondary vertex in %s", flavour), 100, 0, 1);
  }
}
