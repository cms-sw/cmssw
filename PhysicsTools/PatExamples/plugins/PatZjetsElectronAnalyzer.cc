...
void PatZjetsElectronAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (edm::View<pat::Electron>::const_iterator elec = elecs->begin(); elec != elecs->end(); ++elec) {
    ...
    for (int bin = 1; bin <= histContainer_["dr"]->GetNbinsX(); ++bin) {
      ...
      histContainer_["dr"]->Fill(
          histContainer_["dr"]->GetBinCenter(bin),
          elec->trackIsoDeposit()->depositWithin(upperEdge) - elec->trackIsoDeposit()->depositWithin(lowerEdge));
    }
