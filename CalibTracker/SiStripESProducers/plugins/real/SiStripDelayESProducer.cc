...
SiStripDelayESProducer::SiStripDelayESProducer(const edm::ParameterSet& iConfig) {
  ...
  for(const auto& pset: iConfig.getParameter<std::vector<edm::ParameterSet>>("ListOfRecordToMerge")) {
    ...
    edm::LogInfo("SiStripDelayESProducer") << "[SiStripDelayESProducer::ctor] Going to get data from record " << recordName << " with label " << label;
    ...
    if(recordName == "SiStripBaseDelayRcd") {
      ...
      edm::LogError("SiStripDelayESProducer") << "[SiStripDelayESProducer::ctor] Skipping the requested data for unexisting record " << recordName << " with tag " << label << std::endl;
    }
  }
}
