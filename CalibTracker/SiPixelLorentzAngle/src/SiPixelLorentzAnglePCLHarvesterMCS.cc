...
SiPixelLorentzAnglePCLHarvesterMCS::SiPixelLorentzAnglePCLHarvesterMCS(const edm::ParameterSet& iConfig): 
  ...
  if (fitParametersInitValues_.size() != 5) {
    throw cms::Exception("SiPixelLorentzAnglePCLHarvesterMCS") << "Wrong number of initial values for fit parameters specified";    
  }
...
void SiPixelLorentzAnglePCLHarvesterMCS::dqmEndJob(DQMStore::IBooker& iBooker, DQMStore::IGetter& iGetter) {
  ...
  hists_.h_fpixMeanHistoFitStatus_ = iBooker.book1D("fpixMeanHistoFitStatus", "fit status by angles/rings/panels/sides; angle/ring/panel/side; fit status", nBins, -0.5, nBins-0.5);
  hists_.h_fpixMinClusterSizeCotAngle_ = iBooker.book1D("fpixMinClusterSizeCotAngle", "cot angle of minimal cluster size by angles/rings/panels/sides; angle/ring/panel/side; ", nBins, -0.5, nBins-0.5);
  hists_.h_fpixNhitsClusterSizeCotAngle_ = iBooker.book1D("fpixNhitsClusterSizeCotAngle", "number of hits by angles/rings/panels/sides; angle/ring/panel/side; ", nBins, -0.5, nBins-0.5);    
  ...
  for (int r=0; r < hists_.nRings_;++r){
	...
	SiPixelLAHarvestMCS::fitStatus statusAlphaFit = entriesAlpha < minHitsCut_? SiPixelLAHarvestMCS::kFitNotPerformed : fitMCSHistogram(hists_.h_fpixMean_[idx]);
	SiPixelLAHarvestMCS::fitStatus statusBetaFit = entriesBeta < minHitsCut_? SiPixelLAHarvestMCS::kFitNotPerformed : fitMCSHistogram(hists_.h_fpixMean_[idxBeta]);
	...
	if (statusAlphaFit == SiPixelLAHarvestMCS::kFitConverged && statusBetaFit == SiPixelLAHarvestMCS::kFitConverged){
...
SiPixelLAHarvestMCS::fitStatus SiPixelLorentzAnglePCLHarvesterMCS::fitMCSHistogram(dqm::reco::MonitorElement* h_mean){
  ...
  while (nFits < 5){
    ...
    retVal = r == -1? SiPixelLAHarvestMCS::kNoFitResult: r->IsValid()? SiPixelLAHarvestMCS::kFitConverged:SiPixelLAHarvestMCS::kFitFailed;        
  }
  return retVal;
}
...
void SiPixelLorentzAnglePCLHarvesterMCS::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  desc.add<std::vector<double>>("fitParameters", {1., 0.1, -1.6, 0., 1.6})->setComment("initial values for fit parameters");  
  desc.add<std::vector<double>>("fitParametersMuHFit", {0.08, 0.08, 0.08, 0.08})->setComment("initial values for fit parameters (muH fit)");  
