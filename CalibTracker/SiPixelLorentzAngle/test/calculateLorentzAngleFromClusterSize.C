...
// 	fLorentzFit << "module" << "\t" << "layer" << "\t" << "offset" << "\t" << "error" << "\t" << "slope" << "\t" << "error" << "\t" "rel.err" << "\t" "pull" << "\t" << "chi2" << "\t" << "prob" << endl;
	TH2F * h_sizex_alpha = new TH2F("h_sizex_alpha", "h_sizex_alpha",10 , 0, 10, anglebins, 0, 180);
	...
	for(int ientrie = 0 ; ientrie < nentries ; ientrie++){
		...
		if( (chi2_/ndof_) < 10 && !large_pix){
			if(trackhit_.alpha > 0)	h_sizex_alpha->Fill(clust_.size_x, trackhit_.alpha*180. / TMath::Pi());
			else h_sizex_alpha->Fill(clust_.size_x, (trackhit_.alpha + TMath::Pi())*180. / TMath::Pi());
		} 
	}
	
	TH1F * h_mean = new TH1F("h_mean","h_mean", anglebins, 0, 180);
	TH1F * h_slice_ = new TH1F("h_slice","h_slice", 10, 0, 10);
...
// 				findMean(i, (i_module + (i_layer - 1) * 8));
		...
		for( int j = 1; j<= 10; j++){
			h_slice_->SetBinContent(j,h_sizex_alpha->GetBinContent(j,i));
