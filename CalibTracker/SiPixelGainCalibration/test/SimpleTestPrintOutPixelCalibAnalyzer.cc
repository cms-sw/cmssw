...
{
  tPixelCalibDigi = consumes<edm::DetSetVector<SiPixelCalibDigi>>(
      edm::InputTag("siPixelCalibDigis"));
}
...
// member functions
//
void SimpleTestPrintOutPixelCalibAnalyzer::printInfo(
  ...
  for (digiIter = pIn->begin(); digiIter != pIn->end(); ++digiIter) {
    ...
    for (ipix = digiIter->data.begin(); ipix != digiIter->end(); ++ipix) {
      ...
      for (uint32_t ipoint = 0; ipoint < ipix->getnpoints(); ++ipoint)
        std::cout << "\t Det ID " << detid << " row:" << ipix->row()
                  << " col:" << ipix->col() << " point " << ipoint << " has "
                  << ipix->getnentries(ipoint)
                  << " entries, adc: " << ipix->getsum(ipoint)
