...
//
/**\class SiPixelCalibDigiProducer SiPixelCalibDigiProducer.cc
 CalibTracker/SiPixelGainCalibration/src/SiPixelCalibDigiProducer.cc
...
private:
  ...
  virtual void fillPixel(uint32_t detid, short row, short col, short ipoint,
                         short adc);
  ...
  edm::ESHandle<SiPixelCalibConfiguration>
      calib_; // keeps track of the calibration constants
  edm::ESHandle<TrackerGeometry> theGeometry_; // the tracker geometry
  edm::ESHandle<SiPixelFedCablingMap> theCablingMap_;

  // worker variables
  std::map<pixelstruct, SiPixelCalibDigi>
      intermediate_data_; // data container, copied over into the event every
                          // pattern_repeat_ events
  std::map<pixelstruct, SiPixelCalibDigiError> error_data_;
  //      std::vector<SiPixelCalibDigi> intermediate_data_; // data container,
  //      copied over into the event every pattern_repeat_ events
  std::vector<pixelstruct>
      detPixelMap_; // map to keep track of which pixels are filled where in
                    // intermediate_data_
  uint32_t pattern_repeat_; // keeps track of when the pattern should change
  std::map<uint32_t, uint32_t>
      detid_to_fedid_; // keeps track in which fed each detid is present.

  std::vector<std::pair<short, short>>
      currentpattern_; // keeps track of which pattern we are at
