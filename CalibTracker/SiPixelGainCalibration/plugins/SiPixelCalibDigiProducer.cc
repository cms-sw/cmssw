...
/**\class SiPixelCalibDigiProducer SiPixelCalibDigiProducer.cc
 CalibTracker/SiPixelCalibDigiProducer/src/SiPixelCalibDigiProducer.cc
...
SiPixelCalibDigiProducer::SiPixelCalibDigiProducer(
    ...
    : src_(iConfig.getParameter<edm::InputTag>("src")), iEventCounter_(0),
      ignore_non_pattern_(iConfig.getParameter<bool>("ignoreNonPattern")),
      control_pattern_size_(
          iConfig.getParameter<bool>("checkPatternEachEvent")),
      includeErrors_(
          iConfig.getUntrackedParameter<bool>("includeErrors", false)),
      errorType(iConfig.getUntrackedParameter<int>("errorTypeNumber", 1)),
      conf_(iConfig), number_of_pixels_per_pattern_(0),
...
// this function checks where/when the pattern changes so that one can fill the
// data from the temporary container into the event
bool SiPixelCalibDigiProducer::store() {
  //  std::cout << "in store() " << std::endl;
  if (iEventCounter_ % pattern_repeat_ == 0) {
    //    std::cout << "now at event " << iEventCounter_ <<" where we save the
    //    calibration information into the CMSSW digi";
    return true;
...
////////////////////////////////////////////////////////////////////
// function description:
// fill function, uses maps to keep track of which pixel is where in the local
// storage container. Called every event to fill and compress the digi data into
// calibdig format
void SiPixelCalibDigiProducer::fill(edm::Event &iEvent,
  ...
  edm::LogInfo("SiPixelCalibProducer")
      << "in fill(), calibpoint " << icalibpoint << " ndigis "
      << pixelDigis->size() << std::endl;
  // loop over the data and store things
  edm::DetSetVector<PixelDigi>::const_iterator digiIter;
  for (digiIter = pixelDigis->begin(); digiIter != pixelDigis->end();
       ++digiIter) { // ITERATOR OVER DET IDs
...
////////////////////////////////////////////////////
// function description:
// this is the function where we check the cabling map and see if we can assign
// a fed id to the det ID. returns false if no fed <-> detid association was
// found
bool SiPixelCalibDigiProducer::checkFED(uint32_t detid) {
  ...
  for (int fedid = 0; fedid <= 40; ++fedid) {
    //    edm::LogInfo("SiPixelCalibProducer") << " looking at fedid " << fedid
    //    << std::endl;
    ...
    if (converter.hasDetUnit(detid)) {
      ...
      edm::LogInfo("SiPixelCalibDigiProducer")
          << "matched detid " << detid << " to fed " << detid_to_fedid_[detid]
          << std::endl;
      return true;
    }
  }
...
////////////////////////////////////////////////////
// function description:
// this is the function where we look in the maps to find the correct
// calibration digi container, after which the data is filled.
void SiPixelCalibDigiProducer::fillPixel(uint32_t detid, short row, short col,
                                         short ipoint, short adc) {
  //  edm::LogInfo("SiPixelCalibProducer") << " in fillpixel()" << std::endl;

  //  edm::LogInfo("SiPixelCalibProducer") << "in fillPixel " << detid << " " <<
  //  row << " " << col << " " << ipoint << " " << adc << std::endl;
  if (!checkFED(detid)) {
    edm::LogError("SiPixelCalibDigiProducer")
        << " was unable to match detid " << detid << " to a FED!" << std::endl;
    return;
  }
  ...
  std::map<pixelstruct, SiPixelCalibDigi>::const_iterator ipix =
...
//////////////////////////////////////////////////////////////
// function description:
// this function cleans up after everything in the pattern is filled. This
// involves setting the content of the intermediate_data_ containers to zero and
// completely emptying the map
void SiPixelCalibDigiProducer::clear() {
  ...
  if (tempsize > number_of_pixels_per_pattern_) {
    edm::LogError("SiPixelCalibDigiProducer")
        ...
        << ", size is was " << number_of_pixels_per_pattern_ << std::endl;
    number_of_pixels_per_pattern_ = tempsize;
  }

  intermediate_data_.erase(intermediate_data_.begin(),
                           intermediate_data_.end());
...
////////////////////////////////////////////////////
// function description:
// This method gets the pattern from the calib_ (SiPixelCalibConfiguration)
// object and fills a vector of pairs that is easier to check
void SiPixelCalibDigiProducer::setPattern() {
  ...
  edm::LogInfo("SiPixelCalibDigiProducer")
      << " rowpatternnumbers = " << rowpatternnumber << " " << colpatternnumber
  ...
  for (uint32_t icol = 0; icol < calibcols.size(); icol++) {
    if (calibcols[icol] == -1) {
      ...
      // edm::LogInfo("SiPixelCalibProducer") << "col " << calibcols[icol] <<
      // std::endl;
  ...
  for (uint32_t irow = 0; irow < calibrows.size(); irow++) {
    // edm::LogInfo("SiPixelCalibProducer") << "row " << irow <<" "<<
    // nminusrow<<" "  << calibrows[irow] << std::endl;
    if (calibrows[irow] == -1)
      nminusrow++;
...
void SiPixelCalibDigiProducer::produce(edm::Event &iEvent,
  ...
  //  edm::LogInfo("SiPixelCalibDigiProducer") << "now starting fill..." <<
  //  std::endl;
  fill(iEvent,
       iSetup); // fill method where the actual looping over the digis is done.
  //  edm::LogInfo("SiPixelCalibDigiProducer") << "done filling..." <<
  //  std::endl;
  ...
  if (store()) {
    ...
    for (std::map<pixelstruct, SiPixelCalibDigi>::const_iterator idet =
             intermediate_data_.begin();
         idet != intermediate_data_.end(); ++idet) {
      uint32_t detid = idet->first.first;
      if (!control_pattern_size_) {
        if (!checkPixel(idet->first.first, idet->first.second.first,
                        idet->first.second.second))
          continue;
      }
    ...
    if (includeErrors_) {
      for (std::map<pixelstruct, SiPixelCalibDigiError>::const_iterator ierr =
               error_data_.begin();
           ierr != error_data_.end(); ++ierr) {
        uint32_t detid = ierr->first.first;
        SiPixelCalibDigiError temperror = ierr->second;
        edm::DetSet<SiPixelCalibDigiError> &errSet =
            pErr->find_or_insert(detid);
        errSet.data.push_back(temperror);
      }
    }
    edm::LogInfo("INFO") << "now filling event " << iEventCounter_
                         << " as pixel pattern changes every "
...
bool SiPixelCalibDigiProducer::checkPixel(uint32_t detid, short row,
                                          short col) {
  ...
  edm::LogInfo("SiPixelCalibDigiProducer")
      << "Event" << iEventCounter_ << ",now in checkpixel() " << std::endl;
  ...
  // however, the coordinates now need to be converted from dcl, pxid to the
  // row,col coordinates used in the calibration info
  ...
  for (uint32_t i = 0; i < currentpattern_.size(); ++i) {
    //    edm::LogInfo("SiPixelCalibDigiProducer") << "found pair " <<
    //    currentpair_.first << "," << currentpair_.second << " calib " <<
    //    currentpattern_[i].first << ","<< currentpattern_[i].second << " input
    //    " << row << "," << col << std::endl;
  ...
  errorlog << "DETID " << detid << ", row, col (offline)=" << row << "," << col
           << " row, col (ROC) =" << currentpair_.first << ","
           << currentpair_.second << " found no match in list of patterns: ";
  for (uint32_t i = 0; i < currentpattern_.size(); ++i) {
  ...
  if (includeErrors_) { // book the error
    ...
    temppixelworker.second.first = row;
    temppixelworker.second.second = col;
    std::map<pixelstruct, SiPixelCalibDigiError>::const_iterator ierr =
