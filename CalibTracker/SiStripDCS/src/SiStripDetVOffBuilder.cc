...
void SiStripDetVOffBuilder::BuildDetVOffObj()
{
  ...
  if (debug_) {
	...
	std::cout << "detid = " << finalid << " LV off = " << (modulesOff[i].first)->IsModuleLVOff(finalid) << " HV off = " 
		  << (modulesOff[i].first)->IsModuleHVOff(finalid) << std::endl;
      }
    }
  }
}
...
#endif
      if (testSec >= limitSec) {setting = location;}
    }
  }
  return setting;
}
