...
SiStripDetVOffBuilder::SiStripDetVOffBuilder(const edm::ParameterSet &pset,
    ...
    : onlineDbConnectionString(pset.getParameter<std::string>("onlineDB")),
      ...
      debug_(pset.getParameter<bool>("debugModeOn")), tDefault(7, 0),
      ...
      excludedDetIdListFile_(
          pset.getParameter<std::string>("ExcludedDetIdListFile")),
      highVoltageOnThreshold_(
          pset.getParameter<double>("HighVoltageOnThreshold")) {
  ...
  // set up vectors based on pset parameters (tDefault purely for
  // initialization)

  whichQuery = (whichTable == "STATUSCHANGE" ||
                (whichTable == "LASTVALUE" && !fromFile));

  // Define the query interval [Tmin, Tmax]
  // where Tmax comes from the cfg
  //      Tmin comes from the cfg for the first o2o, after that it is extracted
  //      from Offline DB

  tmax = coral::TimeStamp(tmax_par[0], tmax_par[1], tmax_par[2], tmax_par[3],
                          tmax_par[4], tmax_par[5], tmax_par[6]);

  if (whichQuery) {
    // Is there a better way to do this?  TODO - investigate
    tmin = coral::TimeStamp(tmin_par[0], tmin_par[1], tmin_par[2], tmin_par[3],
                            tmin_par[4], tmin_par[5], tmin_par[6]);
  }

  if (whichTable == "LASTVALUE") {
    tsetmin =
        coral::TimeStamp(tset_par[0], tset_par[1], tset_par[2], tset_par[3],
                         tset_par[4], tset_par[5], tset_par[6]);
  }

  if (onlineDbConnectionString.empty()) {
    edm::LogError("SiStripDetVOffBuilder")
        << "[SiStripDetVOffBuilder::SiStripDetVOffBuilder] DB name has not "
           "been set properly ... Returning ...";
    return;
  }
...
SiStripDetVOffBuilder::~SiStripDetVOffBuilder() {
  edm::LogError("SiStripDetVOffBuilder")
      << "[SiStripDetVOffBuilder::" << __func__ << "]: destructing ...";
}
...
void SiStripDetVOffBuilder::BuildDetVOffObj() {
  ...
  coralInterface.reset(new SiStripCoralIface(onlineDbConnectionString,
                                             authenticationPath, debug_));
  edm::LogError("SiStripDetVOffBuilder")
      << "[SiStripDetVOffBuilder::BuildDetVOff]: Query type is " << whichTable
      << endl;
  ...
  if (lastStoredCondObj.second > 0) {
    edm::LogError("SiStripDetVOffBuilder")
        << "[SiStripDetVOffBuilder::BuildDetVOff]: retrieved last time stamp "
           "from DB: "
        << lastStoredCondObj.second << endl;
  }
  // access the information!
  // We've been using the STATUSCHANGE query only in last year or so...
  // LASTVALUE may have untested issues... In either case the idea is that the
  // results of the query are saved into the timesAndValues struct ready to be
  // anaylized (i.e. translated into detIDs, HV/LV statuses)
  ...
  // Initialize the stuct that will be used to keep the detID-translated
  // information
  DetIdListTimeAndStatus dStruct;

  // build PSU - det ID map
  // The following method actually "builds" 4 maps: LVMap, HVMap,
  // HVUnmapped_Map, HVCrosstalking_Map. It also takes the timesAndValues from
  // the query above and using the maps, it processes the information populating
  // the DetIDListTimeAndStatus struct that will hold the information by detid.
  ...
  // Successivamente:
  // - loop su tutti gli elementi del detidV, che stato letto dal pvss (questi
  // elementi sono pair<vettore di detid, time>)
  ...
  // Master loop over all the results of the query stored in the dStruct (that
  // contains vectors with vectors of detids, statuses, isHV flags, etc and in
  // particular a vector of timestamps for which the info is valid... basically
  // it is a loop over the timestamps (i.e. IOVs).
  for (unsigned int i = 0; i < dStruct.detidV.size(); i++) {
    ...
    if (iovtime != saveIovTime) { // time is different, so create new object
      ...
      if (modulesOff.empty()) {
        // create completely new object and set the initial state to Tracker all
        // off
        ...
        // FIXME:
        // Following code is actually broken (well not until the cfg has "" for
        // excludedDetIDListFile parameter! Fix it if felt necessary (remember
        // that it assumes that whatever detids are excluded should NOT be in
        // the regular map breaking our current situation with a fully mapped
        // (LV-wise) tracker...
        // Careful: if a module is in the exclusion list it must be ignored and
        // the initial status is set to ON. These modules are expected to not be
        // in the PSU-DetId map, so they will never get any status change from
        // the query.
        ...
        for (std::map<uint32_t,
                      SiStripDetInfoFileReader::DetInfo>::const_iterator it =
                 detInfos.begin();
             it != detInfos.end(); ++it) {
          std::vector<std::pair<uint32_t, std::string>>::const_iterator exclIt =
              excludedDetIdMap.begin();
          bool excluded = false;
      ...
      modV = (modulesOff.back())
                 .first; // modify previous object (TEST THIS if possible! it's
                         // fundamental in handling changes at the edges of O2O
                         // executions and also in case of PVSS DB buffering!
    }
    ...
    std::pair<int, int> hvlv = extractDetIdVector(
        i, modV, dStruct); // Returns a pair like this HV OFF->1,-1 HV ON->0,-1
                           // LV OFF->-1,1 LV ON->-1,0
    // Basically a LV OFF of -1  means that the information (IOV) in question is
    // from an HV channel turning on or off and viceversa for an HVOFF of -1.
    // This could be confusing when reading the debug output!

    for (unsigned int j = 0; j < detids->size(); j++) {
      if (debug_)
        cout << "at time = " << iovtime << " detid[" << j
             << "] = " << (*detids)[j] << " has hv = " << hvlv.first
  ...
  if (lastStoredCondObj.first != nullptr && lastStoredCondObj.second > 0) {
    if (*(lastStoredCondObj.first) == *(modulesOff[0].first)) {
      if (modulesOff.size() == 1) {
        // if no HV/LV transition was found in this period: update the last IOV
        // to be tmax
        modulesOff[0].second = getCondTime(tmax);
      } else {
        // HV/LV transitions found: remove the first one (which came from
        // previous transfer)
  ...
  if (debug_) {
    ...
    for (unsigned int i = 0; i < modulesOff.size(); i++) {
      ...
      for (unsigned int j = 0; j < finalids.size(); j++) {
        std::cout << "detid = " << finalids[j] << " LV off = "
                  << (modulesOff[i].first)->IsModuleLVOff(finalids[j])
                  << " HV off = "
                  << (modulesOff[i].first)->IsModuleHVOff(finalids[j])
                  << std::endl;
      }
    }
  }
...
int SiStripDetVOffBuilder::findSetting(
    uint32_t id, const coral::TimeStamp &changeDate,
  ...
  // NB.  entries ordered by date!
    ...
    for (unsigned int j = 0; j < locations.size(); j++) {
      ...
      const boost::posix_time::ptime &limitSec =
          settingDate[(unsigned int)locations[j]].time();
#else
...
int SiStripDetVOffBuilder::findSetting(
    std::string dpname, const coral::TimeStamp &changeDate,
...
void SiStripDetVOffBuilder::readLastValueFromFile(
    std::vector<uint32_t> &dpIDs, std::vector<float> &vmonValues,
...
cond::Time_t
SiStripDetVOffBuilder::getCondTime(const coral::TimeStamp &coralTime) {

  // const boost::posix_time::ptime& t = coralTime.time();
  cond::Time_t condTime = cond::time::from_boost(coralTime.time());

  // cout << "[SiStripDetVOffBuilder::getCondTime] Converting CoralTime into
  // CondTime: "
  //      << " coralTime = (coralTimeInNs) " <<  coralTime.total_nanoseconds()
  //      << " condTime " << (condTime>> 32) << " - " << (condTime & 0xFFFFFFFF)
  //      << endl;
...
coral::TimeStamp SiStripDetVOffBuilder::getCoralTime(cond::Time_t iovTime) {
  // This method is defined in the TimeConversions header and it does the
  // following:
  // - takes the seconds part of the iovTime (bit-shifting of 32)
  // - adds the nanoseconds part (first 32 bits mask)
  // - adds the time0 that is the time from begin of times
  ...
  if (debug_) {
    ...
    cout << "[SiStripDetVOffBuilder::getCoralTime] Converting CondTime into "
            "CoralTime: "
...
cond::Time_t SiStripDetVOffBuilder::findMostRecentTimeStamp(
    const std::vector<coral::TimeStamp> &coralDate) {
...
void SiStripDetVOffBuilder::reduce(
    std::vector<std::pair<SiStripDetVOff *, cond::Time_t>>::iterator &it,
  ...
  if (debug_ &&
      (it->first->getLVoffCounts() - initialIt->first->getLVoffCounts() == 0) &&
      (it->first->getHVoffCounts() - initialIt->first->getHVoffCounts() == 0)) {
    cout << "Same number of LV and HV at start and end of sequence: LV off = "
  ...
  if ((it->first->getLVoffCounts() - initialIt->first->getLVoffCounts() > 0) ||
      (it->first->getHVoffCounts() - initialIt->first->getHVoffCounts() > 0)) {
    // Set the time of the current (last) iov as the time of the initial iov of
    // the sequence replace the first iov with the last one
    // Naughty use of const bool last... by default it is false (=0), and for
    // the case of the last timestamp in the query results it is set to true(=1)
    // in the call
  ...
  // if it was going on
  else if ((it->first->getLVoffCounts() - initialIt->first->getLVoffCounts() <=
            0) ||
           (it->first->getHVoffCounts() - initialIt->first->getHVoffCounts() <=
            0)) {
...
void SiStripDetVOffBuilder::discardIOVs(
    std::vector<std::pair<SiStripDetVOff *, cond::Time_t>>::iterator &it,
    std::vector<std::pair<SiStripDetVOff *, cond::Time_t>>::iterator &initialIt,
    std::vector<std::pair<SiStripDetVOff *, cond::Time_t>> &resultVec,
    const bool last, const unsigned int first) {
  if (debug_) {
    cout << "first (1->means the sequence started at the first timestamp in "
            "the query results, 0-> that it did not)= "
         << first << endl;
    cout << "initial->first (initial SiStripDetVOff object of the IOV "
            "sequence)= "
         << initialIt->first
         << ", second (initial timestamp of the IOV sequence) = "
         << initialIt->second << endl;
    cout << "last (0->means that the sequence is not ending with the last item "
            "in the query results, 1-> that it DOES!)= "
         << last << endl;
  }
  if (last == true) {
    resultVec.erase(initialIt + first, it + 1);
    // Minus 2 because it will be incremented at the end of the loop becoming
    // end()-1.
    it = resultVec.end() - 2;
  } else {
...
// This is the method that (called by GetModulesOff, declared in the header
// file) executes the reduction by massaging modulesOff
...
void SiStripDetVOffBuilder::statusChange(cond::Time_t &lastTime,
  ...
  coralInterface->doQuery(whichTable, tmin, tmax, tStruct.changeDate,
                          tStruct.actualValue, tStruct.dpname);
...
void SiStripDetVOffBuilder::lastValue(TimesAndValues &tStruct) {
  ...
  coralInterface->doSettingsQuery(tsetmin, tmax, settingDate, settingValue,
                                  settingDpname, settingDpid);
  ...
  for (unsigned int j = 0; j < tStruct.dpname.size(); j++) {
    int setting = findSetting(tStruct.dpname[j], tStruct.changeDate[j],
                              settingDpname, settingDate);
    if (setting >= 0) {
      if (tStruct.actualValue[j] >
          (highVoltageOnThreshold_ * (settingValue[setting]))) {
  ...
  LogDebug("SiStripDetVOffBuilder")
      << "[SiStripDetVOffBuilder::BuildDetVOff]: Number of channels with no "
         "setting information "
      << missing;
  LogDebug("SiStripDetVOffBuilder") << "[SiStripDetVOffBuilder::BuildDetVOff]: "
                                       "Number of entries in dpname vector "
                                    << tStruct.dpname.size();
}
...
void SiStripDetVOffBuilder::lastValueFromFile(TimesAndValues &tStruct) {
  ...
  for (unsigned int j = 0; j < tStruct.dpid.size(); j++) {
    int setting = findSetting(tStruct.dpid[j], tStruct.changeDate[j],
                              settingDpid, settingDate);
    if (setting >= 0) {
      if (tStruct.actualValue[j] >
          (highVoltageOnThreshold_ * settingValue[setting])) {
        tStruct.actualStatus[j] = 1;
...
string SiStripDetVOffBuilder::timeToStream(const cond::Time_t &condTime,
  ...
  stringstream ss;
  ss << comment << (condTime >> 32) << " - " << (condTime & 0xFFFFFFFF)
     << std::endl;
  return ss.str();
}
...
string SiStripDetVOffBuilder::timeToStream(const coral::TimeStamp &coralTime,
  ...
  ss << "Starting from IOV time in the database : year = " << coralTime.year()
     << ", month = " << coralTime.month() << ", day = " << coralTime.day()
     << ", hour = " << coralTime.hour() << ", minute = " << coralTime.minute()
     << ", second = " << coralTime.second()
     << ", nanosecond = " << coralTime.nanosecond() << std::endl;
  return ss.str();
}
bool SiStripDetVOffBuilder::FileExists(string FileName) {
  // Helper method to check if local files exist (needed to handle HVUnmapped,
  // HVCrosstalking modules)
...
void SiStripDetVOffBuilder::buildPSUdetIdMap(
    TimesAndValues &psuStruct, DetIdListTimeAndStatus &detIdStruct)
// This function builds a PSU to DetID map one way or the other. Then it
// processes the psuStruct that contains the results of the CAEN status query to
// the Online DB, filling the detIdStruct with the detIDs corresponding to the
// PSU (channel in some cases, PSU only in others) reported in the CAEN status
// query to the Online DB. It may make sense to split this method eventually.
{
  SiStripPsuDetIdMap map_;
  if (psuDetIdMapFile_.empty()) {
    std::cout << "PLEASE provide the name of a valid PSUDetIDMapFile in the "
                 "cfg: currently still necessary to have a file, soon will "
                 "access the info straight from the DB!"
              << endl;
    // map_.BuildMap();//This method is not currently used (it would try to
    // build a map based on a query to SiStripConfigDB, and the info there is
    // STALE!)
  } else {
    map_.BuildMap(psuDetIdMapFile_,
                  debug_); // This is the method used to build the map.
  }
  LogTrace("SiStripDetVOffBuilder") << "[SiStripDetVOffBuilder::BuildDetVOff] "
                                       "PSU(Channel)-detID map(s) built";
  // Following method to be replaced by printMaps... to print all 4 maps built!
  map_.printMap(); // This method prints to the info.log file, notice that it is
                   // overwritten by each individual O2O job running in the same
                   // dir.

  // use map info to build input for list of objects
  // no need to check for duplicates, as put method for SiStripDetVOff checks
  // for you!
  ...
  // Create 2 extra maps that we'll use to keep track of unmapped and
  // crosstalking detids when turning ON and OFF HV: -unmapped need to be both
  // turned OFF when any HV goes OFF and to be turned ON when both are ON
  //-crosstaling need to be both turned ON when any HV goes ON and to be turned
  // OFF ONLY when BOTH are OFF.
  std::map<std::string, bool> UnmappedState, CrosstalkingState;
  // Get the HVUnmapped map from the map, so that we can set know which PSU are
  // unmapped:
  std::map<std::string, std::vector<uint32_t>> UnmappedPSUs =
      map_.getHVUnmappedMap();
  // Check here if there is a file already, otherwise initialize to OFF all
  // channels in these PSU!
  if (FileExists("HVUnmappedChannelState.dat")) {
    ...
    while (getline(ifs, line)) {
      if (!line.empty()) {
        ...
        ss >> PSUChannel;
        ss >> HVStatus;
        // Extract the PSU from the PSUChannel (since the HVUnmapped_Map uses
        // PSU as key
        ...
        std::map<std::string, std::vector<uint32_t>>::iterator iter =
            UnmappedPSUs.find(PSU);
        if (iter != UnmappedPSUs.end()) {
          UnmappedState[PSUChannel] = HVStatus;
        } else {
          std::cout << "WARNING!!! There are channels in the local file with "
                       "the channel status for HVUnmapped channels, that ARE "
                       "NOT CONSIDERED AS UNMAPPED in the current map!"
                    << std::endl;
        }
      }
    } // End of the while loop reading and initializing UnmappedState map from
      // file
    // Extra check:
    // Should check if there any HVUnmapped channels in the map that are not
    // listed in the local file!
    bool MissingChannels = false;
    for (std::map<std::string, vector<uint32_t>>::iterator it =
             UnmappedPSUs.begin();
         it != UnmappedPSUs.end(); it++) {
      ...
      if (iter == UnmappedState.end()) {
        std::cout << "ERROR! The local file with the channel status for "
                     "HVUnmapped channels IS MISSING one of the following "
                     "unmapped channel voltage status information:"
                  << std::endl;
    ...
    if (MissingChannels) {
      std::cout << "!!!!\n"
                << "Exiting now... please check the local "
                   "HVUnmappedChannelState.dat and the mapfile you provided ("
                << psuDetIdMapFile_ << ")" << std::endl;
      exit(1);
    }
  } else { // If the file HVUnmappedChannelState.dat does not exist, initialize
           // the map to all OFF.
    //(see below for creating the file at the end of the execution with the
    // latest state of unmapped channels.
    for (std::map<std::string, vector<uint32_t>>::iterator it =
             UnmappedPSUs.begin();
         it != UnmappedPSUs.end(); it++) {
      std::string chan002 = it->first + "channel002";
  ...
  // Get the HVCrosstalking map from the map, so that we can set know which PSU
  // are crosstalking:
  std::map<std::string, std::vector<uint32_t>> CrosstalkingPSUs =
      map_.getHVCrosstalkingMap();
  // Check here if there is a file already, otherwise initialize to OFF all
  // channels in these PSU!
  if (FileExists("HVCrosstalkingChannelState.dat")) {
    ...
    while (getline(ifs, line)) {
      if (!line.empty()) {
        ...
        ss >> PSUChannel;
        ss >> HVStatus;
        // Extract the PSU from the PSUChannel (since the HVCrosstalking_Map
        // uses PSU as key
        std::string PSU = PSUChannel.substr(0, PSUChannel.size() - 10);
        // Look for the PSU in the unmapped map!
        std::map<std::string, std::vector<uint32_t>>::iterator iter =
            CrosstalkingPSUs.find(PSU);
    ...
    } // End of the while loop reading and initializing CrosstalkingState map
      // from file
    // Extra check:
    // Should check if there any HVCrosstalking channels in the map that are not
    // listed in the local file!
    bool MissingChannels = false;
    for (std::map<std::string, vector<uint32_t>>::iterator it =
             CrosstalkingPSUs.begin();
         it != CrosstalkingPSUs.end(); it++) {
      std::string chan002 = it->first + "channel002";
      std::string chan003 = it->first + "channel003";
      std::map<std::string, bool>::iterator iter =
          CrosstalkingState.find(chan002);
      if (iter == CrosstalkingState.end()) {
        std::cout << "ERROR! The local file with the channel status for "
                     "HVCrosstalking channels IS MISSING one of the following "
                     "unmapped channel voltage status information:"
                  << std::endl;
        std::cout << chan002 << std::endl;
        MissingChannels = true;
    ...
    if (MissingChannels) {
      std::cout
          << "!!!!\n"
          << "Exiting now... please check the local "
             "HVCrosstalkingChannelState.dat and the mapfile you provided ("
          << psuDetIdMapFile_ << ")" << std::endl;
      exit(1);
    }
  } else { // If the file HVCrosstalkingChannelState.dat does not exist,
           // initialize the map to all OFF.
    //(see below for creating the file at the end of the execution with the
    // latest state of unmapped channels.
    for (std::map<std::string, vector<uint32_t>>::iterator it =
             CrosstalkingPSUs.begin();
  ...
  if (debug_) {
    ...
    for (std::map<std::string, bool>::iterator it = UnmappedState.begin();
         it != UnmappedState.end(); it++) {
      std::cout << it->first << "\t\t" << it->second << std::endl;
    }
    // print out the CrosstalkingState map:
    std::cout << "Printing the CrosstalkingChannelState initial map:"
              << std::endl;
    std::cout << "PSUChannel\t\tHVON?(true or false)" << std::endl;
    for (std::map<std::string, bool>::iterator it = CrosstalkingState.begin();
         it != CrosstalkingState.end(); it++) {
      std::cout << it->first << "\t\t" << it->second << std::endl;
    }
  }

  // Loop over the psuStruct (DB query results), lopping over the PSUChannels
  // This will probably change int he future when we will change the query
  // itself to report directly the detIDs associated with a channel Probably we
  // will report in the query results the detID, the changeDate and whether the
  // channel is HV mapped, HV unmapped, HV crosstalking using a flag...
  for (unsigned int dp = 0; dp < psuStruct.dpname.size(); dp++) {
    ...
    if (PSUChannel != "UNKNOWN") {
      ...
      std::string Channel = PSUChannel.substr(
          PSUChannel.size() -
          10); // Channel is the channel, i.e. channel000, channel001 etc
      std::string PSU = PSUChannel.substr(0, PSUChannel.size() - 10);

      // Get the detIDs corresponding to the given PSU channel using the
      // getDetID function of SiStripPsuDetIdMap.cc
      // NOTA BENE
      ...
      // We want to differentiate the behavior depending on the status reported
      // for the channel for channels that are unmapped! 1-if the channel is
      // turning OFF (!=1) then we want to report all detIDs for that channel
      // and all detIDs that are listed as channel000 for that PSU. 2-if the
      // channel is turning ON (==1) then we want to turn on all detIDs for that
      // channel but turn on all detIDs listed as channel000 for that PSU ONLY
      // IF BOTH channel002 and channel003 are BOTH ON! Need to handle the case
      // of coupled Power supplies (that only turn off when both are turned
      // off).

      // Fixed SiStripPSUdetidMap.cc to make sure now getDetID gets the correct
      // list of detIDs: -for channels 000/001 all the detIDs connected to the
      // PSU -for channels 002/003 HV1/HV2 modules only (exclusively) UPDATE for
      // HV channels: actually fixed it to report also detIDs listed as
      // channel000 on the same supply of channel002 or channel003
      // and the crosstalking ones (channel999) too..

      // Get the detIDs associated with the DPNAME (i.e. PSUChannel) reported by
      // the query Declare the vector to be passed as reference parameters to
      // the getDetID method std::vector<uint32_t>
      // ids,unmapped_ids,crosstalking_ids;
      std::vector<uint32_t> ids;
      // map_.getDetID(PSUChannel, debug_, ids, unmapped_ids, crosstalking_ids);
      // Actually the method above is a bit of an overkill, we could already use
      // the individual methods: getLvDetID getHvDetID
      ...
      if (Channel == "channel000" || Channel == "channel001") {
        LVCase = true;
        ids = map_.getLvDetID(
            PSU); // Since in the LV case only 1 list of detids is returned
                  // (unmapped and crosstalking are irrelevant for LV) return
                  // the vector directly
      }
      // HV CASE
      else { // if (Channel=="channel002" || Channel=="channel003") {
        LVCase = false;
        map_.getHvDetID(PSUChannel, ids, unmapped_ids,
                        crosstalking_ids); // In the HV case since 3 vectors are
                                           // filled, use reference parameters
      }

      if (debug_) {
        cout << "dpname[" << dp << "] = " << PSUChannel
             << ", for time = " << timeToStream(psuStruct.changeDate[dp])
             << endl;
        if (!ids.empty()) {
          if (Channel == "channel000" || Channel == "channel001") {
        ...
        // The unmapped_ids and crosstalking_ids are only filled for HV
        // channels!
        if (!unmapped_ids.empty()) {
          cout << "Corresponding to HV unmapped (PSU-)matching detids: "
               << endl;
          for (unsigned int i_detid = 0; i_detid < unmapped_ids.size();
               i_detid++) {
            cout << unmapped_ids[i_detid] << std::endl;
          }
        }
        if (!crosstalking_ids.empty()) {
          cout << "Corresponding to HV crosstalking (PSU-)matching detids: "
               << endl;
          for (unsigned int i_detid = 0; i_detid < crosstalking_ids.size();
               i_detid++) {
      ...
      // NOW implement the new logic using the detids, unmapped_detids,
      // crosstalking_detids!

      // First check whether the channel we're looking at is turning OFF or
      // turning ON!
      ...
      if (psuStruct.actualStatus[dp] != 1) {
        ...
        if (LVCase) {
          ...
          //-unmapped matching
          //-crosstalking
          // for the LV case all the detids are automatically reported in the
          // ids vector unmapped and crosstalking are only differentiated
          // (relevant) for HV.
          if (!ids.empty()) {
            ...
            // And put it in the detidV vector of the detIdStruct that will
            // contain all the results
            detIdStruct.detidV.push_back(
                std::make_pair(ids, psuStruct.changeDate[dp]));
        ...
        // HV case:
          ...
          // Debug variables increment:
          if (!ids.empty() || !unmapped_ids.empty() ||
              !crosstalking_ids.empty()) {
          ...
          // First sum the ids (positively matching detids) and the unmapped_ids
          // (since both should be TURNED OFF):
          std::vector<uint32_t> OFFids;
          OFFids.insert(OFFids.end(), ids.begin(),
                        ids.end()); // Add the ids (if any!)
          OFFids.insert(OFFids.end(), unmapped_ids.begin(),
                        unmapped_ids.end()); // Add the unmapped_ids (if any!)
          // Now for the cross-talking ids this is a bit more complicated!
          if (!crosstalking_ids
                   .empty()) { // This already means that the PSUChannel is one
                               // of the crosstalking ones (even if only a few
                               // modules in that PSU are showing crosstalking
                               // behavior both its channels have to be
                               // considered crosstalking of course!
            // Set the channel OFF in the CrosstalkingState map!
            CrosstalkingState[PSUChannel] =
                false; // Turn OFF the channel in the state map!

            // Need to check if both channels (HV1==channel002 or
            // HV2==channel003) are OFF!
            if (!CrosstalkingState[PSUChannel.substr(0, PSUChannel.size() - 1) +
                                   "2"] &&
                !CrosstalkingState[PSUChannel.substr(0, PSUChannel.size() - 1) +
                                   "3"]) { // if HV1 & HV2 both OFF (false)
              OFFids.insert(
                  OFFids.end(), crosstalking_ids.begin(),
                  crosstalking_ids.end()); // Add the crosstalking_ids (if any!)
                                           // since both HV1 and HV2 are OFF!
              if (debug_) {
                std::cout << "Adding the unmapped detids corresponding to "
                             "(HV1/2 cross-talking) PSU "
                          << PSUChannel.substr(0, PSUChannel.size() - 10)
                          << " to the list of detids turning OFF" << std::endl;
              }
            }
          }
          // Handle the crosstalking channel by setting it to OFF in the
          // CrosstalkingState map!
          if (!unmapped_ids.empty()) { // This already means that the PSUChannel
                                       // is one of the unmapped ones (even if
                                       // only a few modules in that PSU are
                                       // unmapped both its channels have to be
                                       // considered crosstalking of course!
            UnmappedState[PSUChannel] =
                false; // Turn OFF the channel in the state map!
          }
          if (!OFFids.empty()) {
            // Create a pair with the relevant detIDs (vector) and its timestamp
            ...
            // have to handle crosstalking_ids here... (only OFF if they
            // corresponding PSU HV1/HV2 is off already... then add all three
            // vectors to the pair below...
            detIdStruct.detidV.push_back(
                std::make_pair(OFFids, psuStruct.changeDate[dp]));
      ...
      // TURNING ON CASE
        ...
        // HV case:
          ...
          // First sum the ids (positively matching detids) and the
          // crosstalking_ids (since all ids on a crosstalking PSU should be
          // TURNED ON when at least one HV channel is ON):
          std::vector<uint32_t> ONids;
          ONids.insert(ONids.end(), ids.begin(),
                       ids.end()); // Add the ids (if any!)
          ONids.insert(
              ONids.end(), crosstalking_ids.begin(),
              crosstalking_ids.end()); // Add the crosstalking_ids (if any!)
          ...
          if (!unmapped_ids.empty()) { // This already means that the PSUChannel
                                       ...
                                       // only a few modules in that PSU are
                                       // unmapped both its channels have to be
                                       // considered unmapped of course!
            // Set the HV1 channel on in the UnmappedState map!
            UnmappedState[PSUChannel] =
                true; // Turn ON the channel in the state map!

            // Need to check if BOTH channels (HV1==channel002 or
            // HV2==channel003) are ON!
            if (UnmappedState[PSUChannel.substr(0, PSUChannel.size() - 1) +
                              "2"] &&
                UnmappedState[PSUChannel.substr(0, PSUChannel.size() - 1) +
                              "3"]) { // if HV1 & HV2 are both ON (true)
              ONids.insert(
                  ONids.end(), unmapped_ids.begin(),
                  unmapped_ids.end()); // Add the unmapped_ids (if any!) since
                                       // both HV1 and HV2 are ON!
              if (debug_) {
                std::cout
          ...
          if (!ONids.empty()) {
            ...
            // vectors to the pair below...
            detIdStruct.detidV.push_back(
                std::make_pair(ONids, psuStruct.changeDate[dp]));
  ...
  // At this point we need to (over)write the 2 files that will keep the
  // HVUnmapped and HVCrosstalking channels status:
  ...
  if (debug_) {
    std::cout << "Number of channels that turned OFF in this O2O interval"
              << std::endl;
    std::cout << "Channel000 = " << ch0bad << " Channel001 = " << ch1bad
              << std::endl;
    std::cout << "Channel002 = " << ch2bad << " Channel003 = " << ch3bad
              << std::endl;
    std::cout << "Number of LV detIDs that turned OFF in this O2O interval = "
              << numLvBad.size() << std::endl;
    std::cout << "Number of HV detIDs that turned OFF in this O2O interval = "
              << numHvBad.size() << std::endl;
  }

  LogTrace("SiStripDetVOffBuilder")
      << "[SiStripDetVOffBuilder::" << __func__
  ...
  for (unsigned int t = 0; t < numLvBad.size(); t++) {
    std::vector<unsigned int>::iterator iter =
        std::find(numHvBad.begin(), numHvBad.end(), numLvBad[t]);
    if (iter != numHvBad.end()) {
      dupCount++;
    }
  }
  if (debug_)
    std::cout << "Number of channels for which LV & HV turned OFF in this O2O "
                 "interval = "
              << dupCount << std::endl;
}
...
pair<int, int>
SiStripDetVOffBuilder::extractDetIdVector(const unsigned int i,
                                          SiStripDetVOff *modV,
  ...
  if (detIdStruct.isHV[i] == 1) {
    ...
    if (psuDetIdMapFile_.empty()) {
      // temporary fix to handle the fact that we don't know which HV channel
      // the detIDs are associated to
      if (i > 0) {
        std::string iChannel =
            detIdStruct.psuName[i].substr((detIdStruct.psuName[i].size() - 3));
        std::string iPsu = detIdStruct.psuName[i].substr(
            0, (detIdStruct.psuName[i].size() - 3));
        if (iChannel == "002" || iChannel == "003") {
          bool lastStatusOfOtherChannel = true;
          for (unsigned int j = 0; j < i; j++) {
            std::string jPsu = detIdStruct.psuName[j].substr(
                0, (detIdStruct.psuName[j].size() - 3));
            std::string jChannel = detIdStruct.psuName[j].substr(
                (detIdStruct.psuName[j].size() - 3));
            if (iPsu == jPsu && iChannel != jChannel &&
                (jChannel == "002" || jChannel == "003")) {
              if (debug_)
                cout << "psu[" << i << "] = " << detIdStruct.psuName[i]
                     << " with status = " << detIdStruct.StatusGood[i]
                     << " and psu[" << j << "] = " << detIdStruct.psuName[j]
                     << " with status " << detIdStruct.StatusGood[j] << endl;
