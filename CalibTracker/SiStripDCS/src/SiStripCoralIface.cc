...
SiStripCoralIface::SiStripCoralIface(std::string connectionString,
                                     ...
                                     const bool debug)
    : m_connectionString(connectionString), m_authPath(authenticationPath),
      m_session(), debug_(debug) {
...
SiStripCoralIface::~SiStripCoralIface() {
  LogTrace("SiStripCoralIface")
      << "[SiStripCoralIface::" << __func__ << "] Destructor called.";
  // m_connection.close();
}
...
void SiStripCoralIface::initialize() {
  ...
  try {
    m_transaction.reset(
        new cond::persistency::TransactionScope(m_session.transaction()));
    m_transaction->start(true);
    LogTrace("SiStripCoralIface")
        << "[SiStripCoralIface::" << __func__ << "] Database connection opened";
  } catch (...) {
...
void SiStripCoralIface::doQuery(std::string queryType,
  ...
  std::unique_ptr<coral::IQuery> query(
      m_session.coralSession()
          .schema(std::string("CMS_TRK_DCS_PVSS_COND"))
          .newQuery());
  std::string condition;

  LogTrace("SiStripCoralIface") << "[SiStripCoralIface::" << __func__
  ...
  if (queryType == "STATUSCHANGE") {
    ...
    condition =
        "FWCAENCHANNEL.DPID = DP_NAME2ID.ID AND "
        "FWCAENCHANNEL.CHANGE_DATE<=:tmax AND FWCAENCHANNEL.ACTUAL_STATUS IS "
        "NOT NULL AND FWCAENCHANNEL.CHANGE_DATE >=:tmin AND (DP_NAME2ID.DPNAME "
        "like '%easyBoard%')";
    ...
    query->addToTableList("DP_NAME2ID");
    condition = "DCSLASTVALUE_VOLTAGE.DPID = DP_NAME2ID.ID AND "
                "DCSLASTVALUE_VOLTAGE.CHANGE_DATE<=:tmax AND "
                "DCSLASTVALUE_VOLTAGE.CHANGE_DATE>=:tmin AND "
                "DCSLASTVALUE_VOLTAGE.ACTUAL_VMON IS NOT NULL AND "
  ...
  if (debug_)
    std::cout << "[SiStripCoralIface::" << __func__
  ...
  if (debug_)
    std::cout << "[SiStripCoralIface::" << __func__
              ...
              << " rows were retrieved from PVSS Cond DB (both Pixel and Strip "
                 "CAEN supplies)"
              << std::endl;
  edm::LogInfo("SiStripCoralIface")
      << "[SiStripCoralIface::" << __func__ << "] " << numberRow
      << " rows retrieved from PVSS Cond DB";
}

// access the channel settings in the status change table
void SiStripCoralIface::doSettingsQuery(
    const coral::TimeStamp &startTime, const coral::TimeStamp &endTime,
    std::vector<coral::TimeStamp> &vec_changedate,
    std::vector<float> &vec_settings, std::vector<std::string> &vec_dpname,
  ...
  query->addToTableList("DP_NAME2ID");
  std::string condition =
      "FWCAENCHANNEL.DPID = DP_NAME2ID.ID AND FWCAENCHANNEL.CHANGE_DATE<=:tmax "
      "AND FWCAENCHANNEL.SETTINGS_V0 IS NOT NULL AND FWCAENCHANNEL.CHANGE_DATE "
      ">=:tmin AND (DP_NAME2ID.DPNAME like '%easyBoard%')";
