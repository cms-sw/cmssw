...
SyncDCSO2O::~SyncDCSO2O() {
  ...
  TH1F *digis =
      new TH1F("digis", "digis", timeInfo_.size(), 0, timeInfo_.size());
  TH1F *digisWithMasking = new TH1F("digisWithMasking", "digisWithMasking",
                                    timeInfo_.size(), 0, timeInfo_.size());
  TH1F *HVoff =
      new TH1F("HVoff", "HVoff", timeInfo_.size(), 0, timeInfo_.size());
  ...
  for (; it != timeInfo_.end(); ++it, ++i) {
    ...
    // N.B. we add 1 hour to the coralTime because it is the conversion from
    // posix_time which is non-adjusted. The shift of +1 gives the CERN time
    // zone.
    TDatime date1(coralTime.year(), coralTime.month(), coralTime.day(),
                  coralTime.hour() + 1, coralTime.minute(), coralTime.second());
    // timeArray[i-1] = date1.Convert();
    time->SetBinContent(i, date1.Convert());
  }

  // TCanvas * canvas = new TCanvas("digiAndHVvsTimeCanvas", "digi and HVoff vs
  // time", 1000, 800); canvas->Draw(); canvas->cd();
...
void SyncDCSO2O::analyze(const edm::Event &iEvent,
  ...
  std::cout << "year = " << coralTime.year()
            << ", month = " << coralTime.month()
            << ", day = " << coralTime.day();
  // N.B. we add 1 hour to the coralTime because it is the conversion from
  // posix_time which is non-adjusted. The shift of +1 gives the CERN time zone.
  std::cout << ", hour = " << coralTime.hour() + 1
  ...
  if (!(digiDetsetVector_[0].isValid()))
    ...
    timeInfo_.push_back(TimeInfo(iEvent.time().value(), totDigis,
                                 totDigisWithMasking,
...
void SyncDCSO2O::getDigis(const edm::Event &iEvent) {
  ...
  for (; itDigiProducersList != digiProducersList_.end();
       ++itDigiProducersList) {
    std::string digiProducer =
        itDigiProducersList->getParameter<std::string>("DigiProducer");
    std::string digiLabel =
        itDigiProducersList->getParameter<std::string>("DigiLabel");
    // std::cout << "Reading digi for " << digiProducer << " with label: " <<
    // digiLabel << std::endl;
