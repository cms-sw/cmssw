...
/**\class SyncDCSO2O SyncDCSO2O.cc
 ...
 Description: Produces a histogram with the digi occupancy vs time and the
 change of payload.
...
private:
  ...
  struct TimeInfo {
    TimeInfo(unsigned long long inputTime, unsigned int inputDigiOccupancy,
             unsigned int inputDigiOccupancyWithMasking,
             unsigned int inputHVoff)
        : time(inputTime), digiOccupancy(inputDigiOccupancy),
