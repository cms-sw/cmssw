...
# Read the tMin from file unless it is the first job
if os.path.isfile("tMax.txt"):
    tMaxFile = open("tMax.txt", "r")
    tMin = tMaxFile.read()
    tMaxFile.close()
else:
...
# Save this tMax to file to be reused as tMin by the next job
tMaxFile = open("tMax.txt", "w")
tMaxFile.write(tMax)
tMaxFile.close()
