...
std::vector<int> vectorDate(const int year, const int month, const int day,
...
public:
  ...
  void setUp() {
    ...
    pset.addParameter(
        "PsuDetIdMapFile",
        std::string("CalibTracker/SiStripDCS/data/PsuDetIdMap.dat"));
  ...
  void testStatusChange() {
    object->coralInterface.reset(new SiStripCoralIface(
        object->onlineDbConnectionString, object->authenticationPath, false));
  ...
  void testReduction() {
    ...
    cout << "number of IOVs before reduction = " << object->modulesOff.size()
         << endl;

    object->reduction(1, 1000);

    cout << "number of IOVs after reduction = " << object->modulesOff.size()
         << endl;
    vector<pair<SiStripDetVOff *, cond::Time_t>>::const_iterator iovContent =
        object->modulesOff.begin();
    for (; iovContent != object->modulesOff.end(); ++iovContent) {
      coral::TimeStamp coralTime(object->getCoralTime(iovContent->second));
      cout << "iov seconds = " << coralTime.second()
           << ", nanoseconds = " << coralTime.nanosecond();
      cout << ", number of modules with HV off = "
           << iovContent->first->getHVoffCounts() << endl;
      cout << ", number of modules with LV off = "
           << iovContent->first->getLVoffCounts() << endl;
    }
  ...
  void fillModule(const unsigned int detId, const unsigned int HVoff,
                  const unsigned int LVoff, const unsigned int seconds,
                  const unsigned int nanoseconds) {
    // Build the cond time from the
    cond::Time_t condTime = object->getCondTime(
        coral::TimeStamp(2009, 12, 1, 1, 0, seconds, nanoseconds));
    // Carefull, there is a memory leakage here. Fine if the program is kept
    // simple.
