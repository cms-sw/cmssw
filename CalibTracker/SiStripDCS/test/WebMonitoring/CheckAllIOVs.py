...
# print "cmscond_list_iov -c "+database+" -t "+sys.argv[2]+" -P /afs/cern.ch/cms/DB/conddb"
iovs = os.popen("cmscond_list_iov -c "+database+" -t "+tagName+" -P /afs/cern.ch/cms/DB/conddb")
iovsList = iovs.read()
...
for i in range(0, len(splittedList), 2):
    ...
    if len(iov) > 1:
        ...
        # To be safe we always repeat the last IOV (one IOV takes few seconds)
        lastIOV = time.strftime("%d/%m/%Y %H:%M:%S", time.localtime(int(start)>>32))
        fullDates="_FROM_"+startDate.replace(" ", "_").replace(":", "_")+"_TO_"+endDate.replace(" ", "_").replace(":", "_")
        fileName="DetVOffPrint"+fullDates+"_cfg.py"
        os.system("cat templateCheckAllIOVs_cfg.py | sed -e \"s/STARTTIME/"+start+"/g\" | sed -e \"s/ENDTIME/"+end+"/g\" | sed -e \"s/DATE/"+fullDates+"/g\" | sed -e \"s#TAGNAME#"+tagName+"#g\" | sed -e \"s#DATABASE#"+database+"#g\" > "+fileName)
...
# Save the last written IOV
lastIOVfile = open("lastIOV.txt", "w")
lastIOVfile.write(lastIOV)
