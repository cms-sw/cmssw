...
/**
   ...
   \brief Extension to SiStripConfigDb to map PSU channels to DetIDs using
   DCU-PSU map and DCU-DetID map \author J.Cole
*/
...
public:
  ...
  void getHvDetID(std::string psuchannel, std::vector<uint32_t> &ids,
                  std::vector<uint32_t> &unmapped_ids,
                  std::vector<uint32_t> &crosstalking_ids);

  // Produces 3 list of detIDs:
  // 1-detids (positively matching the PSUChannel for HV case, positively
  // matching the PSU for the LV case) 2-unmapped_detids (matching the
  // PSUChannel000 for the HV case, empty for LV case) 3-crosstalking_detids
  // (matching the PSUChannel999 for the HV case, empty for the LV case)
  void getDetID(std::string pvss, bool, std::vector<uint32_t> &detids,
                std::vector<uint32_t> &unmapped_detids,
                std::vector<uint32_t> &crosstalking_detids);
  /** Returns the PSU channel name for the specified Det ID, for power groups
   * only. */
  ...
  /** Returns the detector location for the specified Det ID, for power groups
   * only. */
  ...
  /** Returns the detector location for the specified Det ID and specified group
   * type (PG or CG). */
  std::string getDetectorLocation(uint32_t detid, std::string group);
  /** Returns the DCU ID for the specified PSU channel - checks power and
   * control groups. */
  uint32_t getDcuId(std::string pvss);
  /** Returns the DCU ID associated to the specified Det ID.  NB.  This checks
   * power groups only, by definition. */
  uint32_t getDcuId(uint32_t detid);

  // Return the HVUnmapped PSU channels as a map initialized to all channels
  // (002/003) OFF (false):
  std::map<std::string, std::vector<uint32_t>> getHVUnmappedMap() {
    return HVUnmapped_Map;
  }
  // Return the HVCrosstalking PSU channels as a map initialized to all channels
  // (002/003) OFF (false):
  ...
  // PsuDetIdMap getHVCrosstalkingChannelMap() {return
  // HVCrossTalking_ChanStatus}
  /** Return the PG PSU-DETID map as a vector. */
  std::vector<std::pair<uint32_t, std::string>> getPsuDetIdMap() {
    return pgMap;
  }
  /** Return the PG detector locations as a vector - one-to-one correspondance
   * with the contents of the PSU-DetID map vector. */
  ...
  /** Return the CG detector locations as a vector - one-to-one correspondance
   * with the contents of the PSU-DetID map vector. */
  ...
  void
  BuildMap(const std::string &mapFile, const bool debug,
  ...
  /** Returns 1 if the specified PSU channel is a HV channel, 0 if it is a LV
   * channel.  -1 means error. */
...
private:
  ...
  void getDcuPsuMap(DcuPsusRange &pRange, DcuPsusRange &cRange,
                    std::string partition);
  /** Extracts the DCU device descriptions and stores them for further use. Only
   * used for control groups. */
  //  std::vector< std::pair<uint32_t, SiStripConfigDb::DeviceAddress> >
  //  retrieveDcuDeviceAddresses(std::string partition);
  std::vector<std::pair<std::vector<uint16_t>, std::vector<uint32_t>>>
  retrieveDcuDeviceAddresses(std::string partition);
  /** Searches the DCU device descriptions for the specified DCU ID. Needed for
   * control groups. */
  ...
  // for debugging
  void checkMapInputValues(const SiStripConfigDb::DcuDetIdsV &dcuDetIds_,
