...
class SiStripPsuDetIdMap
{
 public:
  ...
  void getHvDetID(const std::string& psuchannel, std::vector<uint32_t> & ids, std::vector<uint32_t> & unmapped_ids, std::vector<uint32_t> & crosstalking_ids );
 ...
 private:
  ...
  std::vector< std::pair< std::vector<uint16_t> , std::vector<uint32_t> > > retrieveDcuDeviceAddresses(const std::string& partition);
