...
public:
  ...
  /** Default constructor. */
  SiStripDetVOffBuilder(const edm::ParameterSet &,
  ...
  /** Store the last payload transferred to DB as starting point for creation of
     new object list. ONLY WORKS FOR STATUSCHANGE OPTION. */
  void setLastSiStripDetVOff(SiStripDetVOff *lastPayload,
                             cond::Time_t lastTimeStamp);

  /// Operates the reduction of the fast sequences of ramping up and down of the
  /// voltages
  void
  reduce(std::vector<std::pair<SiStripDetVOff *, cond::Time_t>>::iterator &it,
         std::vector<std::pair<SiStripDetVOff *, cond::Time_t>>::iterator
             &initialIt,
...
private:
  // typedefs
  typedef std::vector<std::pair<std::vector<uint32_t>, coral::TimeStamp>>
  ...
  std::string timeToStream(const coral::TimeStamp &coralTime,
                           const string &comment = "");
  std::string timeToStream(const cond::Time_t &condTime,
                           const string &comment = "");

  /** Returns the PSU channel setting, based on date.  Works from DP ID. */
  int findSetting(uint32_t id, const coral::TimeStamp &changeDate,
                  const std::vector<uint32_t> &settingID,
                  const std::vector<coral::TimeStamp> &settingDate);

  /** Returns the PSU channel setting, based on date.  Works from PSU channel
   * name. Overloaded. */
  int findSetting(std::string dpname, const coral::TimeStamp &changeDate,
  ...
  /** Utility code to convert a coral timestamp to the correct time format for
   * O2O timestamp. */
  ...
  /** */
  cond::Time_t
  findMostRecentTimeStamp(const std::vector<coral::TimeStamp> &coralDate);
  ...
  struct TimesAndValues {
    ...
    cond::Time_t
        latestTime; // used for timestamp when using lastValue from file
  };
  ...
  void buildPSUdetIdMap(TimesAndValues &tStruct,
                        DetIdListTimeAndStatus &dStruct);

  void setPayloadStats(const uint32_t afterV, const uint32_t numAdded,
