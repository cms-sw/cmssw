...
private:
  ...
  // IOV of plotting.
  cond::Time_t m_IOV;
  // Or use datatime string. Format: "2002-01-20 23:59:59.000". Set IOV to 0 to
  // use this.
  std::string m_Time;
...
SiStripDetVOffTkMapPlotter::SiStripDetVOffTkMapPlotter(
    ...
    : m_connectionPool(),
      ...
      m_plotFormat(
          iConfig.getUntrackedParameter<std::string>("plotFormat", "png")),
      m_outputFile(
...
void SiStripDetVOffTkMapPlotter::analyze(const edm::Event &evt,
  ...
  if (m_IOV != 0) {
    ...
    theIov =
        cond::time::from_boost(boost::posix_time::time_from_string(m_Time));
  } else {
    // Use the current time if no input. Will get the last IOV.
    theIov = cond::time::from_boost(
        boost::posix_time::second_clock::universal_time());
  }

  // open db session
  edm::LogInfo("SiStripDetVOffMapPlotter")
      << "[SiStripDetVOffMapPlotter::" << __func__ << "] "
      << "Query the condition database " << m_condDb << " for tag "
      << m_plotTag;
  ...
  if (iiov == iovProxy.end())
    throw cms::Exception("Input IOV " + std::to_string(m_IOV) + "/" + m_Time +
                         " is invalid!");
