...
public:
  const std::vector<Color_t> PLOT_COLORS{kRed, kBlue, kBlack, kOrange,
                                         kMagenta};
...
private:
  std::string formatIOV(cond::Time_t iov,
  ...
  int m_interval;
  // Or manually specify the start/end time. Format: "2002-01-20 23:59:59.000".
  // Set m_interval to non-positive number to use this.
  std::string m_startTime;
  std::string m_endTime;
  // Specify output plot file name. Will use the timestamps if left empty.
  std::string m_outputPlot;
  // Specify output root file name. Leave empty if do not want to save plots in
  // a root file.
  std::string m_outputRootFile;
  // Specify output CSV file name. Leave empty if do not want to dump HV/LV
...
SiStripDetVOffTrendPlotter::SiStripDetVOffTrendPlotter(
    ...
    : m_connectionPool(),
      ...
      m_outputPlot(
          iConfig.getUntrackedParameter<std::string>("outputPlot", "")),
      m_outputRootFile(
          iConfig.getUntrackedParameter<std::string>("outputRootFile", "")),
      m_outputCSV(iConfig.getUntrackedParameter<std::string>("outputCSV", "")),
      fout(nullptr) {
  m_connectionPool.setParameters(
      iConfig.getParameter<edm::ParameterSet>("DBParameters"));
...
void SiStripDetVOffTrendPlotter::analyze(const edm::Event &evt,
  ...
  cond::persistency::Session condDbSession =
      m_connectionPool.createSession(m_condDb);
  ...
  for (unsigned itag = 0; itag < m_plotTags.size(); ++itag) {
    ...
    auto color =
        itag < PLOT_COLORS.size() ? PLOT_COLORS.at(itag) : kGreen + itag;
    ...
    cond::persistency::IOVProxy iovProxy =
        condDbSession.readIov(tag, true); // load all?
    ...
    while (iiov != iovProxy.end() && (*iiov).since <= (*eiov).since) {
      ...
      auto payload =
          condDbSession.fetchPayload<SiStripDetVOff>((*iiov).payloadId);
  ...
  std::string plot_postfix =
      !m_outputPlot.empty()
          ? m_outputPlot
          : "from_" + formatIOV(startIov) + "_to_" + formatIOV(endIov) + ".png";
...
void SiStripDetVOffTrendPlotter::prepGraph(TGraph *gr, TString name,
                                           TString title, Color_t color) {
  gr->SetName(name);
  gr->SetTitle(title);
...
void SiStripDetVOffTrendPlotter::dumpCSV(bool isHV) {
  ...
  std::string outCSV =
      isHV ? "HVOff_table_" + m_outputCSV : "LVOff_table_" + m_outputCSV;
  std::ofstream csv;
  csv.open(outCSV);
  ...
  for (const auto &v : iovMap) {
    auto iov = v.first;
    auto timestamp =
        boost::posix_time::to_simple_string(cond::time::to_boost(iov));
    csv << iov << "," << timestamp << ",";
    for (const auto &tag : m_plotTags) {
