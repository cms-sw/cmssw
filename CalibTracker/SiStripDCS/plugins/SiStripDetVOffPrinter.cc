...
private:
  ...
  // Specify output text file name. Leave empty if do not want to dump HV/LV
  // counts in a file.
  std::string m_output;
...
void SiStripDetVOffPrinter::analyze(const edm::Event &evt,
  ...
  edm::LogInfo("SiStripDetVOffPrinter")
  ...
  cond::persistency::IOVProxy iovProxy =
      condDbSession.readIov(m_tagName, true); // load all?
  ...
  for (; itTime != vTime.end(); ++itTime) {
    mIOVsDuration[*itPreviousTime] =
        cond::time::to_boost(*itTime) - cond::time::to_boost(*itPreviousTime);
    itPreviousTime = itTime;
  }
  boost::posix_time::time_duration time_period =
      cond::time::to_boost(*(--vTime.end())) -
      cond::time::to_boost(*(vTime.begin()));

  // debug
  /*for( itTime=vTime.begin(); itTime!=itLastTime; ++itTime ) {
        std::cout<<boost::posix_time::to_simple_string(
  cond::time::to_boost(*itTime) ) << "    "
  <<boost::posix_time::to_simple_string(mIOVsDuration[ *itTime ])<<std::endl;
  }*/

  // Print summary per module
  edm::LogInfo("SiStripDetVOffPrinter")
      << "[SiStripDetVOffPrinter::" << __func__ << "] " << detidMap.size()
      << " modules were Off at some point during the time interval";
  ss.str("");

  // Loop over detIds
  std::map<uint32_t, std::vector<cond::Time_t>>::const_iterator itMap =
      detidMap.begin();
  for (; itMap != detidMap.end(); ++itMap) {
    ...
    for (; itTime != vecTime.end(); ++itTime) {
      if (iovMap_HVOff[*itTime].find(itMap->first) !=
          iovMap_HVOff[*itTime].end())
        cumul_time_HVOff += mIOVsDuration[*itTime];
      if (iovMap_LVOff[*itTime].find(itMap->first) !=
          iovMap_LVOff[*itTime].end())
        cumul_time_LVOff += mIOVsDuration[*itTime];
    }
    ss << "detId " << itMap->first << " #IOVs: " << vecTime.size()
       << "  HVOff: " << cumul_time_HVOff << " "
       << cumul_time_HVOff.total_milliseconds() * 100.0 /
              time_period.total_milliseconds()
       << "% "
       << "  LVOff: " << cumul_time_LVOff << " "
       << cumul_time_LVOff.total_milliseconds() * 100.0 /
              time_period.total_milliseconds()
       << "%" << std::endl;
  }
...
void SiStripDetVOffPrinter::fillDescriptions(
    edm::ConfigurationDescriptions &descriptions) {

  edm::ParameterSetDescription desc;
  desc.add<std::string>("conditionDatabase",
