...
SiStripDetVOffHandler::SiStripDetVOffHandler(const edm::ParameterSet &iConfig)
    : m_connectionPool(),
      ...
      maxTimeBeforeNewIOV_(
          iConfig.getUntrackedParameter<int>("maxTimeBeforeNewIOV", 24)) {
...
void SiStripDetVOffHandler::analyze(const edm::Event &evt,
  ...
  cond::persistency::Session localFileSession =
      m_connectionPool.createSession(m_localCondDbFile, true);
  localFileSession.transaction().start(false);
  if (lastPayload && newPayloads.size() == 1 &&
      *newPayloads[0].first == *lastPayload) {
    ...
    if (maxTimeBeforeNewIOV_ < 0)
      ...
      auto deltaT = cond::time::to_boost(newPayloads[0].second) -
                    cond::time::to_boost(lastIov);
    ...
    if (!forceNewIOV) {
      ...
      edm::LogInfo("SiStripDetVOffHandler")
          ...
          << boost::posix_time::to_simple_string(
                 cond::time::to_boost(newPayloads[0].second))
          << ") with the same payload.";
    }
  }

  cond::persistency::IOVEditor iovEditor;
  if (localFileSession.existsDatabase() &&
      localFileSession.existsIov(m_targetTag)) {
    edm::LogWarning("SiStripDetVOffHandler")
        << "[SiStripDetVOffHandler::" << __func__ << "] "
        << "IOV of tag " << m_targetTag << " already exists in sqlite file "
        << m_localCondDbFile;
    iovEditor = localFileSession.editIov(m_targetTag);
  } else {
    iovEditor = localFileSession.createIov<SiStripDetVOff>(m_targetTag,
                                                           cond::timestamp);
    iovEditor.setDescription("New IOV");
  }
  for (const auto &payload : newPayloads) {
    cond::Hash thePayloadHash =
        localFileSession.storePayload<SiStripDetVOff>(*payload.first);
  ...
  edm::LogInfo("SiStripDetVOffHandler")
      << "[SiStripDetVOffHandler::" << __func__ << "] " << newPayloads.size()
