...
const PixelToLNKAssociateFromAscii::CablingRocId *PixelToLNKAssociateFromAscii::
operator()(const PixelToLNKAssociateFromAscii::DetectorRocId &roc) const {

  typedef std::vector<std::pair<DetectorRocId, CablingRocId>>::const_iterator
      IM;
  for (IM im = theConnection.begin(); im != theConnection.end(); im++) {
    if ((*(im->first.module) == *roc.module) &&
        (im->first.rocDetId == roc.rocDetId)) {
      return &(im->second);
    }
  }
  return nullptr;
...
// This is where the reading and interpretation of the ascci cabling input file
// is
void PixelToLNKAssociateFromAscii::init(const string &cfg_name) {
    ...
    while (getline(file, line)) {
      ...
      if (posM != string::npos) {
        if (posR != string::npos) {
          ...
          // cout<<" range find "<<strR<<" "<<strR.size()<<" "<<range.min()<<"
          // "<<range.max()<<endl;
  ...
  for (ICON ic = theConnection.begin(); ic != theConnection.end(); ic++) {
    str << (*ic).first.module->name() << ", rocDetId=" << (*ic).first.rocDetId
        << ", fedId=" << ic->second.fedId << ", linkId=" << ic->second.linkId
        << ", rocLinkId=" << ic->second.rocLinkId << endl;
  }
  edm::LogInfo("PixelToLNKAssociateFromAscii") << str.str();
}

void PixelToLNKAssociateFromAscii::addConnections(int fedId, int linkId,
                                                  std::string module,
                                                  Range rocDetIds) {
  ...
  if (pos != string::npos) {
    ...
    // // sector
    // pos = module.find("_");
    // if (pos ==  string::npos) throw cms::Exception("problem with sector
    // formatting");
    // // int sector = atoi( module.substr(3,pos-3).c_str());
    // module = module.substr(pos+1);
    // // layer
    // pos = module.find("_");
    // if (pos ==  string::npos) throw cms::Exception("problem with layer
    // formatting"); int layer = atoi( module.substr(3,pos-3).c_str()); module =
    // module.substr(pos+1);
    // // ladder
    // pos = module.find("_");
    // if (pos ==  string::npos) throw cms::Exception("problem with ladder
    // formatting"); int ladder = atoi( module.substr(3,pos-3).c_str()); module
    // = module.substr(pos+1);
    // // z-module
    // int zmodule = atoi( module.substr(3,pos-3).c_str());
    // // place modules in connections
    // PixelBarrelName * name0 = new PixelBarrelName(part, layer, zmodule,
    // ladder, phase1_); if(name->name() != module0) cout<<" wrong name
    // "<<fedId<<" "<<linkId<<" "
    // 				     <<module0<<" "<<name->name()<<"
    // "<<name0->name()<<endl; if(name->name() != name0->name()) cout<<" wrong
    // name "<<fedId<<" "<<linkId<<" "
    // 				     <<module0<<" "<<name->name()<<"
    // "<<name0->name()<<endl;
    ...
    for (int rocDetId = rocDetIds.min(); rocDetId <= rocDetIds.max();
         rocDetId++) {
      loopExecuted = true;
      rocLnkId++;
      ...
      edm::LogInfo(" roc ")
          << rocDetId << " " << rocLnkId << " " << name->isHalfModule() << endl;
  ...
  if (pos != string::npos) {
    ...
    // plaquete
    //     pos = module.find("_");
    //     if (pos ==  string::npos) throw cms::Exception("problem with
    //     plaquette formatting"); int plaq = atoi(
    //     module.substr(3,pos-3).c_str());
    ...
    if (pannelType == PixelPannelType::p4L) {
      ...
      for (int plaq = 1; plaq <= 4; plaq++) {
        ...
        PixelEndcapName *name =
            new PixelEndcapName(part, disk, blade, pannel, plaq);
        for (int iroc = rocs.min(); iroc <= rocs.max(); iroc++) {
          rocLnkId++;
          int rocDetId = firstRoc + step * iroc;

          DetectorRocId detectorRocId;
          // detectorRocId.module = new
          // PixelEndcapName(part,disk,blade,pannel,plaq);
          detectorRocId.module = name;
          detectorRocId.rocDetId = rocDetId;
          ...
          theConnection.push_back(make_pair(detectorRocId, cablingRocId));
          //         cout <<"PLAQ:"<<plaq<<" rocDetId: "<<rocDetId<<"
          //         rocLnkId:"<<rocLnkId<<endl;
        }
      }
      ...
      //       {
      PixelEndcapName *name =
          new PixelEndcapName(part, disk, blade, pannel, ring, phase1_);
      bool loopExecuted = false;
      for (int rocDetId = rocDetIds.min(); rocDetId <= rocDetIds.max();
           rocDetId++) {
        loopExecuted = true;
        ...
        edm::LogInfo("PixelToLNKAssociateFromAscii FPix ")
            << " rocDetId: " << rocDetId << " rocLnkId:" << rocLnkId
            << " fedId = " << fedId << " linkId = " << linkId
            << " name = " << name->name();
...
PixelToLNKAssociateFromAscii::readRange(const string &l) const {
  ...
  // this method is very stupid it relies on a space being present after the
  // last number! exchange with string opertaions (above)
  // bool first = true;
  // int num1 = -1;
  // int num2 = -1;
  ...
  //   //{ stringstream s; s<<"read from line: "; s<<num; LogTrace("") <<
  //   s.str(); } if (evp != line) {
  //     line = evp +1;
  //     //cout<<i<<" "<<num<<" "<<evp<<" "<<line<<endl;
  ...
  // if(!phase1_ && (i2!=num2)) cout<<" something wrong with max range "<<i2<<"
  // "<<num2<<endl; cout<<" min max "<<num1<<" "<<num2<<endl; return
  // Range(num1,num2);
}
