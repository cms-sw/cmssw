...
SiPixelFedCablingMapBuilder::produce(const edm::EventSetup &setup) {

  // Access geometry
  edm::LogInfo("read tracker geometry...");
  edm::ESHandle<TrackerGeometry> pDD;
  setup.get<TrackerDigiGeometryRecord>().get(pDD);
  edm::LogInfo("tracker geometry read")
      << "There are: " << pDD->dets().size() << " detectors";

  // Test new TrackerGeometry features
  // cout << "Test of TrackerGeometry::isThere";
  // cout  << " is there PixelBarrel: " <<
  // pDD->isThere(GeomDetEnumerators::PixelBarrel); cout  << " is there
  // PixelEndcap: " << pDD->isThere(GeomDetEnumerators::PixelEndcap); cout  << "
  // is there P1PXB: " << pDD->isThere(GeomDetEnumerators::P1PXB); cout  << " is
  // there P1PXEC: " << pDD->isThere(GeomDetEnumerators::P1PXEC); cout  << endl;
  ...
  if (phase1_) {
    ...
    MAXFEDID =
        FEDNumbering::MAXSiPixeluTCAFEDID; // is actually 1349, might work
  }
  TRange<int> fedIds(MINFEDID, MAXFEDID);
  edm::LogInfo("SiPixelFedCablingMapBuilder")
  ...
  // PixelToFEDAssociate * associator = new
  // PixelToLNKAssociateFromAscii("pixelToLNK.ascii",phase1_);
  PixelToFEDAssociate *associator =
      new PixelToLNKAssociateFromAscii(fileName_, phase1_);
  ...
  for (ITG it = pDD->dets().begin(); it != pDD->dets().end(); it++) {
    const PixelGeomDetUnit *pxUnit =
        dynamic_cast<const PixelGeomDetUnit *>(*it);
    if (pxUnit == nullptr)
      continue;
    npxdets++;
  ...
  //   }
  //   edm::LogInfo("tracker geometry read")<<"There are: "<< npxdets<<" pixel
  //   detetors";
  //   // construct FEDs
  ...
  for (UNITS::iterator iu = units.begin(); iu != units.end(); iu++) {
    ...
    for (int rocDetId = 0; rocDetId < 16; rocDetId++) {
      ...
      const PixelToFEDAssociate::CablingRocId *cablingRocId =
          name2fed(detectorRocId);
      if (cablingRocId) {
        sipixelobjects::PixelROC roc(iu->second, rocDetId,
                                     cablingRocId->rocLinkId);
        result->addItem(cablingRocId->fedId, cablingRocId->linkId, roc);
        edm::LogInfo(" ok ")
            << name->name() << " " << rocDetId << " " << cablingRocId->fedId
            << " " << cablingRocId->linkId;
      } else { // did it fail?
...
std::string
SiPixelFedCablingMapBuilder::myprint(const PixelGeomDetUnit *pxUnit) {
