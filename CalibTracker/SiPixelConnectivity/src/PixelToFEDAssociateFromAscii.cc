...
int PixelToFEDAssociateFromAscii::operator()(const PixelBarrelName &id) const {
  for (BarrelConnections::const_iterator ibc = theBarrel.begin();
       ibc != theBarrel.end(); ibc++) {
    for (vector<Bdu>::const_iterator ibd = (*ibc).second.begin();
         ibd != (*ibc).second.end(); ibd++) {
      if (ibd->b == id.shell() && ibd->l.inside(id.layerName()) &&
          ibd->z.inside(id.moduleName()) && ibd->f.inside(id.ladderName()))
        return (*ibc).first;
    }
  }
...
int PixelToFEDAssociateFromAscii::operator()(const PixelEndcapName &id) const {
  for (EndcapConnections::const_iterator iec = theEndcap.begin();
       iec != theEndcap.end(); iec++) {
    for (vector<Edu>::const_iterator ied = (*iec).second.begin();
         ied != (*iec).second.end(); ied++) {
      if (ied->e == id.halfCylinder() && ied->d.inside(id.diskName()) &&
          ied->b.inside(id.bladeName()))
        return iec->first;
    }
  }
  edm::LogError("** PixelToFEDAssociateFromAscii WARNING, name: ")
...
void PixelToFEDAssociateFromAscii::send(pair<int, vector<Bdu>> &b,
                                        pair<int, vector<Edu>> &e) {
  if (!b.second.empty())
...
PixelToFEDAssociateFromAscii::getBdu(string line) const {
  Bdu result;
...
PixelToFEDAssociateFromAscii::getEdu(string line) const {
  Edu result;
  string::size_type pos;
