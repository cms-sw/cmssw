...
SiPixelFedCablingMapWriter::SiPixelFedCablingMapWriter(
    ...
    : record_(cfg.getParameter<std::string>("record")),
      ...
      fileName_(cfg.getUntrackedParameter<std::string>("fileName",
                                                       "pixelToLNK.ascii")) {
...
void SiPixelFedCablingMapWriter::analyze(const edm::Event &iEvent,
  ...
  if (1 == first) {
    ...
    // std::cout << "-------HERE-----------" << endl;
    // cabling =
    // SiPixelFedCablingMapBuilder(pixelToFedAssociator_).produce(iSetup);
    ...
    // std::cout << "-------Print Map ----------- DOES NOT WORK for phase1 " <<
    // endl;
    edm::LogInfo("PRINTING MAP (Does not work for phase1: ")
        << cabling->print(3);
  }
}
  ...
  try {
    if (mydbservice->isNewTagRequest(record_)) {
      mydbservice->createNewIOV<SiPixelFedCablingMap>(
          result, mydbservice->beginOfTime(), mydbservice->endOfTime(),
          record_);
    } else {
      mydbservice->appendSinceTime<SiPixelFedCablingMap>(
          result, mydbservice->currentTime(), record_);
    }
