...
void SiPixelSCurveCalibrationAnalysis::makeThresholdSummary(void) {
  ...
  for (detIDHistogramMap::iterator thisDetIdHistoGrams = histograms_.begin();
       thisDetIdHistoGrams != histograms_.end(); ++thisDetIdHistoGrams) {
    ...
    const MonitorElement *thresholdhist =
        (*thisDetIdHistoGrams).second[kThresholds];
    ...
    for (int irow = 0; irow < total_rows; ++irow) {
      for (int icol = 0; icol < total_columns; ++icol) {
        float threshold_error = sigmahist->getBinContent(
            icol + 1, irow + 1); // +1 because root bins start at 1
        if (writeZeroes_ || (!writeZeroes_ && threshold_error > 0)) {
          ...
          const sipixelobjects::PixelROC *theRoc =
              theCablingMap_->findItem(path);
          // END of FIX
          int newrow = locpixel.rocRow();
          int newcol = locpixel.rocCol();
          myfile << rocname << theRoc->idInDetUnit() << " " << newcol << " "
                 << newrow << " "
...
void chi2toMinimize(int &npar, double *grad, double &fcnval, double *xval,
                    int iflag) {
  ...
  const std::vector<short> *theVCalValues =
      SiPixelSCurveCalibrationAnalysis::getVcalValues();
  for (uint32_t i = 0; i < theVCalValues->size(); i++) {
    float chi = (SiPixelSCurveCalibrationAnalysis::efficiencies_[i] -
                 theFormula->Eval((*theVCalValues)[i]));
...
void SiPixelSCurveCalibrationAnalysis::doSetup(
  ...
  useDetectorHierarchyFolders_ =
      iConfig.getUntrackedParameter<bool>("useDetectorHierarchyFolders", true);
  saveCurvesThatFlaggedBad_ =
      iConfig.getUntrackedParameter<bool>("saveCurvesThatFlaggedBad", false);
  detIDsToSaveVector_ = iConfig.getUntrackedParameter<std::vector<uint32_t>>(
      "detIDsToSave", anEmptyDefaultVectorOfUInts);
  maxCurvesToSave_ =
      iConfig.getUntrackedParameter<uint32_t>("maxCurvesToSave", 1000);
  write2dHistograms_ =
      iConfig.getUntrackedParameter<bool>("write2dHistograms", true);
  write2dFitResult_ =
      iConfig.getUntrackedParameter<bool>("write2dFitResult", true);
  printoutthresholds_ =
      iConfig.getUntrackedParameter<bool>("writeOutThresholdSummary", true);
  thresholdfilename_ = iConfig.getUntrackedParameter<std::string>(
      "thresholdOutputFileName", "thresholds.txt");
  minimumChi2prob_ =
      iConfig.getUntrackedParameter<double>("minimumChi2prob", 0);
  minimumThreshold_ =
      iConfig.getUntrackedParameter<double>("minimumThreshold", -10);
  maximumThreshold_ =
      iConfig.getUntrackedParameter<double>("maximumThreshold", 300);
  ...
  minimumEffAsymptote_ =
      iConfig.getUntrackedParameter<double>("minimumEffAsymptote", 0);
  maximumEffAsymptote_ =
      iConfig.getUntrackedParameter<double>("maximumEffAsymptote", 1000);
  maximumSigmaBin_ =
      iConfig.getUntrackedParameter<double>("maximumSigmaBin", 10);
  maximumThresholdBin_ =
      iConfig.getUntrackedParameter<double>("maximumThresholdBin", 255);

  writeZeroes_ =
      iConfig.getUntrackedParameter<bool>("alsoWriteZeroThresholds", false);
...
void SiPixelSCurveCalibrationAnalysis::buildACurveHistogram(
    const uint32_t &detid, const uint32_t &row, const uint32_t &col,
    sCurveErrorFlag errorFlag, const std::vector<float> &efficiencies,
  ...
  humanName << translateDetIdToString(detid) << "_" << rootName.str()
            << "_ErrorFlag_" << (int)errorFlag;

  unsigned int numberOfVCalPoints =
      vCalPointsAsFloats_.size() - 1; // minus one is necessary since the lower
                                      // edge of the last bin must be added
  if (efficiencies.size() != numberOfVCalPoints ||
      errors.size() != numberOfVCalPoints) {
    edm::LogError("SiPixelSCurveCalibrationAnalysis")
        << "Error saving single curve histogram!  Number of Vcal values ("
        << numberOfVCalPoints
  ...
  MonitorElement *aBadHisto = bookDQMHistogram1D(
      detid, rootName.str(), humanName.str(), numberOfVCalPoints,
...
void SiPixelSCurveCalibrationAnalysis::calibrationSetup(
  ...
  edm::LogInfo("SiPixelSCurveCalibrationAnalysis")
      << "Calibration Settings: VCalLow: " << vCalValues_[0]
      << "  VCalHigh: " << vCalValues_[vCalValues_.size() - 1]
  ...
  if (saveCurvesThatFlaggedBad_) {
    ...
    edm::LogWarning("SiPixelSCurveCalibrationAnalysis")
        ...
        << maxCurvesToSave_
        << " This can lead to large memory consumption! (Got "
        << numberOfVCalPoints << " VCal Points";
    for (unsigned int i = 0; i < numberOfVCalPoints; i++) {
      vCalPointsAsFloats_.push_back(static_cast<float>((*theVCalValues)[i]));
      edm::LogInfo("SiPixelSCurveCalibrationAnalysis")
          << "Adding calibration Vcal: " << (*theVCalValues)[i];
    }
    // must add lower edge of last bin to the vector
    vCalPointsAsFloats_.push_back(vCalPointsAsFloats_[numberOfVCalPoints - 1] +
                                  1);
  }

  fitFunction_ =
      new TF1("sCurve", "0.5*[2]*(1+TMath::Erf( (x-[0]) / ([1]*sqrt(2)) ) )",
              vCalValues_[0], vCalValues_[vCalValues_.size() - 1]);
}
...
sCurveErrorFlag SiPixelSCurveCalibrationAnalysis::estimateSCurveParameters(
    const std::vector<float> &eff, float &threshold, float &sigma) {
  ...
  for (uint32_t iVcalPt = 0; iVcalPt < eff.size(); iVcalPt++) {
    if (allZeroSoFar && eff[iVcalPt] != 0) {
      ...
      sigma = delta * 0.682;
      if (sigma <
          1) // check to make sure sigma guess is larger than our X resolution.
...
sCurveErrorFlag SiPixelSCurveCalibrationAnalysis::fittedSCurveSanityCheck(
    float threshold, float sigma, float amplitude) {
  // check if nonsensical
  if (threshold > vCalValues_[vCalValues_.size() - 1] ||
      threshold < vCalValues_[0] ||
...
void calculateEffAndError(int nADCResponse, int nTriggers, float &eff,
                          float &error) {
  eff = (float)nADCResponse / (float)nTriggers;
  double effForErrorCalculation = eff;
  if (eff <= 0 || eff >= 1)
    effForErrorCalculation = 0.5 / (double)nTriggers;
  error = TMath::Sqrt(effForErrorCalculation * (1 - effForErrorCalculation) /
                      (double)nTriggers);
}
...
void SiPixelSCurveCalibrationAnalysis::newDetID(uint32_t detid) {
  ...
  if (insertResult.second) // indicates successful insertion
  {
    ...
    if (write2dHistograms_) {
      MonitorElement *D2sigma = bookDQMHistoPlaquetteSummary2D(
          detid, "ScurveSigmas", detIdName + " Sigmas");
      MonitorElement *D2thresh = bookDQMHistoPlaquetteSummary2D(
          detid, "ScurveThresholds", detIdName + " Thresholds");
      MonitorElement *D2chi2 = bookDQMHistoPlaquetteSummary2D(
          detid, "ScurveChi2Prob", detIdName + " Chi2Prob");
    ...
    if (write2dFitResult_) {
      MonitorElement *D2FitResult = bookDQMHistoPlaquetteSummary2D(
          detid, "ScurveFitResult", detIdName + " Fit Result");
      insertResult.first->second.insert(
          std::make_pair(kFitResults, D2FitResult));
    }
    MonitorElement *D1sigma = bookDQMHistogram1D(detid, "ScurveSigmasSummary",
                                                 detIdName + " Sigmas Summary",
                                                 100, 0, maximumSigmaBin_);
    MonitorElement *D1thresh = bookDQMHistogram1D(
        detid, "ScurveThresholdSummary", detIdName + " Thresholds Summary", 255,
        0, maximumThresholdBin_);
    MonitorElement *D1chi2 =
        bookDQMHistogram1D(detid, "ScurveChi2ProbSummary",
                           detIdName + " Chi2Prob Summary", 101, 0, 1.01);
    MonitorElement *D1FitResult =
        bookDQMHistogram1D(detid, "ScurveFitResultSummary",
                           detIdName + " Fit Result Summary", 10, -0.5, 9.5);
    insertResult.first->second.insert(std::make_pair(kSigmaSummary, D1sigma));
    insertResult.first->second.insert(
        std::make_pair(kThresholdSummary, D1thresh));
    insertResult.first->second.insert(std::make_pair(kChi2Summary, D1chi2));
    insertResult.first->second.insert(
        std::make_pair(kFitResultSummary, D1FitResult));
  }
}

bool SiPixelSCurveCalibrationAnalysis::doFits(
    uint32_t detid, std::vector<SiPixelCalibDigi>::const_iterator calibDigi) {
  ...
  for (uint32_t iVcalPt = 0; iVcalPt < nVCalPts; iVcalPt++) {
    float eff;
    float error;
    calculateEffAndError(calibDigi->getnentries(iVcalPt), nTriggers_, eff,
                         error);
    edm::LogInfo("SiPixelSCurveCalibrationAnalysis")
        << "Eff: " << eff << " Error:  " << error
  ...
  errorFlag =
      estimateSCurveParameters(efficiencies_, thresholdGuess, sigmaGuess);
  ...
  if (errorFlag == errOK) // only do fit if curve is fittable
  {
    ...
    gMinuit->DefineParameter(0, "Threshold", (Double_t)thresholdGuess, 1, -50,
                             300);
    // sigma
  ...
  if (thisDetIdHistoGrams != histograms_.end()) {
    edm::LogInfo("SiPixelSCurveCalibrationAnalysisHistogramReport")
        << "Filling histograms for [detid](col/row):  [" << detid << "](" << col
        << "/" << row << ") ErrorFlag: " << errorFlag;
    // always fill fit result
    (*thisDetIdHistoGrams).second[kFitResultSummary]->Fill(errorFlag);
    if (write2dFitResult_)
      (*thisDetIdHistoGrams)
          .second[kFitResults]
          ->setBinContent(col + 1, row + 1,
  ...
  if (saveCurvesThatFlaggedBad_) {
    ...
    if (detIDsToSave_.find(detid) !=
        detIDsToSave_.end()) // see if we want to save this histogram
    ...
    if (errorFlag != errOK || thisDetIDinList) {
      ...
      buildACurveHistogram(detid, row, col, errorFlag, efficiencies_,
                           effErrors_);
    }
  }
