...
public:
  bool VisitEnter(const tinyxml2::XMLElement &elem,
    ...
    if (std::strcmp(elem.Value(), "Vector") == 0) {
      ...
      if (att_type == "numeric") {
        ...
        if (0 ==
            att_name.compare(0, SubdetName.size(), SubdetName)) { // starts with
          const std::string att_nEntries{elem.Attribute("nEntries")};
          const std::size_t nEntries =
              att_nEntries.empty() ? 0 : std::stoul(att_nEntries);
          ...
          if (nEntries != vals.size()) {
            throw cms::Exception(
                ...
                ("Problem parsing element with name '" + att_name + "': " +
                 "'nEntries' attribute claims " + std::to_string(nEntries) +
                 " elements, but parsed " + std::to_string(vals.size())));
          }
          ...
          case PixelSubdetector::PixelEndcap: // side, disk, blade, panel,
                                              // module
          ...
          case StripSubdetector::TIB: // layer, str_fw_bw, str_int_ext, str,
                                      // module, ster
          ...
          case StripSubdetector::TID: // side, wheel, ring, module_fw_bw,
                                      // module, ster
          ...
          case StripSubdetector::TEC: // side, wheel, petal_fw_bw, petal, ring,
                                      // module, ster
  ...
  TrackerTopology getTrackerTopology() const {
    if (!(foundPXB && foundPXF && foundTIB && foundTID && foundTOB &&
          foundTEC)) {
      throw cms::Exception(
          "StandaloneTrackerTopology",
          "Could not find parameters for all tracker subdetectors");
    }
    return TrackerTopology(pxbVals_, pxfVals_, tecVals_, tibVals_, tidVals_,
                           tobVals_);
  }
...
private:
  ...
  bool foundPXB = false, foundPXF = false, foundTIB = false, foundTID = false,
       foundTOB = false, foundTEC = false;
...
TrackerTopology fromTrackerParametersXMLFile(const std::string &xmlFileName) {
  ...
  if (!xmlDoc.Error()) {
    ...
    throw cms::Exception("StandaloneTrackerTopology",
                         std::string{"Failed to parse file "} + xmlFileName +
                             ": " + xmlDoc.ErrorStr());
  }
}
