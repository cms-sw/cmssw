...
void PredictedVsObserved(){
   ...
   string inputFileTree2 = "../../Data_Run_252116_to_252126_PCL/Gains_Tree.root";
   ...
   TrackerMap* tkmap = new TrackerMap("  Gain : Measured(R252116) / Predicted(R251252*StrenghRatio from DelayScan)");
   ...
   for (unsigned int ientry = 0; ientry < t1->GetEntries(); ientry++) {
      ...
      if((tree1_APVId==0 && PreviousId>0) || ientry==t1->GetEntries()-1){  //new module or very last APV
         fprintf(pFile,"%i ",PreviousId); for(unsigned int i=0;i<gains.size();i++){fprintf(pFile,"%f ", gains[i]);}   fprintf(pFile, "\n");
          tkmap->fill(tree1_DetId, maxRatio);
          maxRatio = 1;
         gains.clear();
      }
...
//      gains.push_back(tree1_Gain * modifier);
      gains.push_back(tree2_Gain);

      if(fabs((tree2_Gain / (tree1_Gain * modifier)) - 1) > fabs(maxRatio -1)) maxRatio = tree2_Gain / (tree1_Gain * modifier);

   }printf("\n");
   fclose(pFile);

//   tkmap->setTitle("Gain: Predicted/Current    (white=unmodified)");
   tkmap->save(true, 0.9, 1.1, "tkMapPredictedVsObserved.png");
   tkmap->reset();    
