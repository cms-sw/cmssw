...
SiStripHotStripAlgorithmFromClusterOccupancy::
    ...
    : prob_(1.E-7), ratio_(1.5), MinNumEntries_(0), MinNumEntriesPerStrip_(0),
      Nevents_(0), occupancy_(0), OutFileName_("Occupancy.root"),
      tTopo(theTopo),
...
void SiStripHotStripAlgorithmFromClusterOccupancy::extractBadStrips(
    SiStripQuality *OutSiStripQuality, HistoMap &DM,
  ...
  if (WriteOutputFile_ == true) {
    ...
    striptree->Branch("StripGlobalPositionX", &global_position_x,
                      "StripGlobalPositionX/F");
    striptree->Branch("StripGlobalPositionY", &global_position_y,
                      "StripGlobalPositionY/F");
    striptree->Branch("StripGlobalPositionZ", &global_position_z,
  ...
  for (; it != itEnd; ++it) {
    ...
    for (int apv = 0; apv < numberAPVs; apv++) {
        ...
        if (InSiStripQuality->IsApvBad(detid, apv)) {
          if (edm::isDebugEnabled())
            LogTrace("SiStripHotStrip")
                << "(Module and Apv number) " << detid << " , " << apv
      ...
      LogTrace("SiStripHotStrip") << "Number of clusters in APV " << apv << ": "
    ...
    for (int strip = 0; strip < number_strips; strip++) {
      ...
      GlobalPoint pos_strip_global =
          (TkGeom->idToDet(detectorId))->surface().toGlobal(pos_strip_local);
...
void SiStripHotStripAlgorithmFromClusterOccupancy::iterativeSearch(
    pHisto &histo, std::vector<unsigned int> &vect, int apv) {
  if (!histo._NEntries || histo._NEntries <= MinNumEntries_ ||
      histo._NEntries <= minNevents_)
    return;
  ...
  for (size_t i = ibinStart; i < ibinStop; ++i) {
    if (ishot[(apv * 128) + i - 1] == 0) {
      goodstripentries[nGoodStrips] =
          (unsigned int)histo._th1f->GetBinContent(i);
      nGoodStrips++;
    }
  }
  ...
  if (edm::isDebugEnabled())
    LogTrace("SiStripHotStrip")
        ...
        << " NEmptyBins=" << histo._NEmptyBins
        << " NEntries=" << histo._NEntries << " thEntries "
        << histo._th1f->GetEntries() << " startingSize " << startingSize
...
void SiStripHotStripAlgorithmFromClusterOccupancy::evaluatePoissonian(
...
void SiStripHotStripAlgorithmFromClusterOccupancy::setNumberOfEvents(
    double Nevents) {
  Nevents_ = Nevents;
  ...
  if (edm::isDebugEnabled())
    LogTrace("SiStripHotStrip")
        << " [SiStripHotStripAlgorithmFromClusterOccupancy::setNumberOfEvents] "
           "minNumber of Events per strip used to consider a strip bad"
