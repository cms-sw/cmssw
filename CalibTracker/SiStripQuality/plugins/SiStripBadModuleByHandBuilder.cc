...
SiStripBadModuleByHandBuilder::SiStripBadModuleByHandBuilder(
  ...
  BadModuleList_ =
      iConfig.getUntrackedParameter<std::vector<uint32_t>>("BadModuleList");
...
std::unique_ptr<SiStripBadStrip> SiStripBadModuleByHandBuilder::getNewObject() {
  ...
  for (std::vector<uint32_t>::const_iterator it = BadModuleList_.begin();
       it != BadModuleList_.end(); ++it) {

    std::vector<unsigned int> theSiStripVector;

    NconsecutiveBadStrips =
        reader->getNumberOfApvsAndStripLength(*it).first * 128;
    ...
    SiStripBadStrip::Range range(theSiStripVector.begin(),
                                 theSiStripVector.end());
    if (!obj->put(*it, range))
      edm::LogError("SiStripBadModuleByHandBuilder")
          << "[SiStripBadModuleByHandBuilder::analyze] detid already exists"
          << std::endl;
  }
  return obj;
}
