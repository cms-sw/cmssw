...
SiStripQualityHotStripIdentifier::SiStripQualityHotStripIdentifier(
    ...
    : ConditionDBWriter<SiStripBadStrip>(iConfig), m_cacheID_(0),
      ...
      tracksCollection_in_EventTree(
          iConfig.getUntrackedParameter<bool>("RemoveTrackClusters", false)),
      tTopo(nullptr) {
  reader = new SiStripDetInfoFileReader(fp_.fullPath());

  edm::ParameterSet pset = iConfig.getUntrackedParameter<edm::ParameterSet>(
      "ClusterSelection", edm::ParameterSet());
...
SiStripQualityHotStripIdentifier::getNewObject() {
  ...
  if (AlgoName == "SiStripHotStripAlgorithmFromClusterOccupancy") {
    ...
    edm::LogInfo("SiStripQualityHotStripIdentifier")
        << " [SiStripQualityHotStripIdentifier::getNewObject] call to "
           "SiStripHotStripAlgorithmFromClusterOccupancy"
        << std::endl;

    SiStripHotStripAlgorithmFromClusterOccupancy theIdentifier(conf_, tTopo);
    theIdentifier.setProbabilityThreshold(
        parameters.getUntrackedParameter<double>("ProbabilityThreshold",
                                                 1.E-7));
    theIdentifier.setMinNumEntries(
        parameters.getUntrackedParameter<uint32_t>("MinNumEntries", 100));
    theIdentifier.setMinNumEntriesPerStrip(
    ...
    edm::LogInfo("SiStripQualityHotStripIdentifier")
        << " [SiStripQualityHotStripIdentifier::getNewObject] copy "
           "SiStripObject in SiStripBadStrip"
        << std::endl;
    ...
    for (; rIter != rIterEnd; ++rIter) {
      ...
      if (!obj->put(rIter->detid, range))
        edm::LogError("SiStripQualityHotStripIdentifier")
            << "[SiStripQualityHotStripIdentifier::getNewObject] detid already "
               "exists"
            << std::endl;
    }
    edm::LogInfo("SiStripQualityHotStripIdentifier")
        << " [SiStripQualityHotStripIdentifier::getNewObject] " << ss.str()
        << std::endl;

  } else {
    edm::LogError("SiStripQualityHotStripIdentifier")
        << " [SiStripQualityHotStripIdentifier::getNewObject] call for a "
           "unknow HotStrip identification algoritm"
        << std::endl;
...
void SiStripQualityHotStripIdentifier::algoBeginRun(
    const edm::Run &run, const edm::EventSetup &iSetup) {
...
void SiStripQualityHotStripIdentifier::bookHistos() {
  ...
  std::map<uint32_t, SiStripDetInfoFileReader::DetInfo>::const_iterator it =
      reader->getAllData().begin();
  std::map<uint32_t, SiStripDetInfoFileReader::DetInfo>::const_iterator iEnd =
      reader->getAllData().end();
  for (; it != iEnd; ++it) {
    sprintf(hname, "h_%d", it->first);
    SiStrip::QualityHistosMap::iterator ref =
        ClusterPositionHistoMap.find(it->first);
    if (ref == ClusterPositionHistoMap.end()) {
      ClusterPositionHistoMap[it->first] =
          boost::shared_ptr<TH1F>(new TH1F(hname, hname, it->second.nApvs * 128,
                                           -0.5, it->second.nApvs * 128 - 0.5));
    } else
      edm::LogError("SiStripQualityHotStripIdentifier")
          << " [SiStripQualityHotStripIdentifier::bookHistos] DetId "
          << it->first << " already found in map. Ignoring new data"
          << std::endl;
  }
}
...
void SiStripQualityHotStripIdentifier::fillHisto(uint32_t detid, float value) {
  ...
  if (ref != ClusterPositionHistoMap.end())
    ...
    edm::LogError("SiStripQualityHotStripIdentifier")
        << " [SiStripQualityHotStripIdentifier::fillHisto] Histogram not found "
           "in the list for DetId "
        << detid << " Ignoring data value " << value << std::endl;
}

void SiStripQualityHotStripIdentifier::algoAnalyze(
    const edm::Event &e, const edm::EventSetup &eSetup) {
  ...
  if (tracksCollection_in_EventTree) {
    ...
    if (!trackCollection.isValid()) {
      edm::LogError("SiStripQualityHotStripIdentifier")
          << " [SiStripQualityHotStripIdentifier::algoAnalyze] missing "
             "trackCollection with label "
          << Track_src_ << std::endl;
    }
  }
  ...
  if (tracksCollection_in_EventTree) {
    ...
    int i = 0;
    for (reco::TrackCollection::const_iterator track = tC.begin();
         track != tC.end(); track++) {
      ...
      for (auto const &recHit : track->recHits()) {
        ...
        const SiStripRecHit2D *singleHit =
        ...
        const ProjectedSiStripRecHit2D *projectedHit =
  ...
  edm::DetSetVector<SiStripCluster>::const_iterator DSViter =
      dsv_SiStripCluster->begin();
  for (; DSViter != dsv_SiStripCluster->end(); DSViter++) {
    edm::DetSet<SiStripCluster>::const_iterator ClusIter =
        DSViter->data.begin();
    edm::DetSet<SiStripCluster>::const_iterator ClusIterEnd =
        DSViter->data.end();
    for (; ClusIter != ClusIterEnd; ++ClusIter) {
      if (MinClusterWidth_ <= ClusIter->amplitudes().size() &&
          ClusIter->amplitudes().size() <= MaxClusterWidth_) {
        if (std::find(vPSiStripCluster.begin(), vPSiStripCluster.end(),
                      (void *)&*ClusIter) == vPSiStripCluster.end()) {
          if (edm::isDebugEnabled())
            ss << " adding cluster to histo for detid " << DSViter->id
