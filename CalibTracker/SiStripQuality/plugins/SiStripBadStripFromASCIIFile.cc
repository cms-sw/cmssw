...
std::unique_ptr<SiStripBadStrip> SiStripBadStripFromASCIIFile::getNewObject() {
  ...
  if (!infile) {
    std::cout << "Problem while trying to open File: "
              << (fp_.fullPath()).c_str() << std::endl;
  }
  ...
  while (!infile.eof()) {
    ...
    if (detid == tempdetid) {
      if (channel != tempchannel + count || flag != tempflag) {
        ...
        theBadStripRange = SiStripBadStrip_->encode(
            tempchannel - 1, count,
            tempflag); // In the quality object, strips are counted from 0 to
                       // 767!!! Therefore "tempchannel-1"! In the input
                       // txt-file, they have to be from 1 to 768 instead!!!
        ...
        if (infile.eof()) { // Don't forget to save the last strip before eof!!!
          ...
          if (!SiStripBadStrip_->put(tempdetid, range))
            edm::LogError("SiStripBadStripFromASCIIFile")
                << "[SiStripBadStripFromASCIIFile::GetNewObject] detid already "
                   "exists"
                << std::endl;
    ...
    if (detid != tempdetid) {
      ...
      theBadStripRange = SiStripBadStrip_->encode(
          tempchannel - 1, count,
          tempflag); // In the quality object, strips are counted from 0 to
                     // 767!!! Therefore "tempchannel-1"! In the input txt-file,
                     // they have to be from 1 to 768 instead!!!
      edm::LogInfo("SiStripBadStripFromASCIIFile")
          << "detid " << tempdetid << " \t"
          << " firstBadStrip " << tempchannel << "\t "
          << " NconsecutiveBadStrips " << count << "\t "
