...
make_and_fit_symmchi2(Book& book) {
  for(Book::iterator it = book.begin(".*_all"); it!=book.end(); ++it) {
    ...
    TH1* const prob_w1 = w1==book.end()     ? 0 : subset_probability( base+method(PROB1,false) ,w1->second,all->second);
    TH1* const rmsv_w2 = var_w2==book.end() ? 0 :        rms_profile( base+method(RMSV2,false), (TProfile*const)var_w2->second);
    TH1* const rmsv_w3 = var_w3==book.end() ? 0 :        rms_profile( base+method(RMSV3,false), (TProfile*const)var_w3->second);
