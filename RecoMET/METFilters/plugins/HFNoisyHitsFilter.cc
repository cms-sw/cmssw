...
std::vector<int> HFNoisyHitsFilter::getNoiseBits(std::vector<std::string> noiseList ) const{
  ...
  for(unsigned int i =  0; i< noiseList.size();i++){
    if(noiseList[i]=="HFLongShort") result.push_back(HcalPhase1FlagLabels::HFLongShort);
    else if(noiseList[i]=="HFS8S1Ratio") result.push_back(HcalPhase1FlagLabels::HFS8S1Ratio);
    else if(noiseList[i]=="HFPET") result.push_back(HcalPhase1FlagLabels::HFPET);
    else if(noiseList[i]=="HFSignalAsymmetry") result.push_back(HcalPhase1FlagLabels::HFSignalAsymmetry);
    else if(noiseList[i]=="HFAnomalousHit") result.push_back(HcalPhase1FlagLabels::HFAnomalousHit);
    else if(debug_) edm::LogWarning("HFNoisyHitsFilter") << "Couldn't find the bit index associated to this string: " << noiseList[i];
  }
