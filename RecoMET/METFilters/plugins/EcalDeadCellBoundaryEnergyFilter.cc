...
EcalDeadCellBoundaryEnergyFilter::EcalDeadCellBoundaryEnergyFilter(const edm::ParameterSet& iConfig)
  ...
  if (skimGap_ && debug_)
    edm::LogInfo("EcalDeadCellBoundaryEnergyFilter") << "Skim Gap!";
  if (skimDead_ && debug_)
    edm::LogInfo("EcalDeadCellBoundaryEnergyFilter") << "Skim Dead!";
...
bool EcalDeadCellBoundaryEnergyFilter::filter(edm::StreamID, edm::Event& iEvent, const edm::EventSetup& iSetup) const {
  ...
  if (!limitFilterToEE_) {
    ...
    for (EcalRecHitCollection::const_iterator hit = EBRecHits->begin(); hit != EBRecHits->end(); ++hit) {
      ...
      for (int channelAllowed : limitDeadCellToChannelStatusEB_) {
        for (int& sit : deadNeighbourStati) {
      ...
      for (auto& sameFlagDetId : sameFlagDetIds) {
        if (currDetId == sameFlagDetId)
      ...
      if (!detIdAlreadyChecked && deadNeighbourStati.empty() &&
        ...
        for (auto& detId : gapinfo.detIds) {
          sameFlagDetIds.push_back(detId);
        }
        ...
        if (gapinfo.boundaryEnergy > cutBoundEnergyGapEB) {
          ...
          if (debug_)
            edm::LogInfo("EcalDeadCellBoundaryEnergyFilter")
                << "EB: gap cluster energy: " << gapinfo.boundaryEnergy << " deadCells: " << gapinfo.detIds.size();
        }
      }
      ...
      if (!detIdAlreadyChecked &&
        ...
        for (auto& detId : boundinfo.detIds) {
          sameFlagDetIds.push_back(detId);
        }
        ...
        if (boundinfo.boundaryEnergy > cutBoundEnergyDeadCellsEB) {
          ...
          if (debug_)
            edm::LogInfo("EcalDeadCellBoundaryEnergyFilter")
                << "EB: boundary Energy dead RecHit: " << boundinfo.boundaryEnergy << " ET: " << boundinfo.boundaryET
                << " deadCells: " << boundinfo.detIds.size();
        }
      }
    }
  ...
  if (!limitFilterToEB_) {
    ...
    for (EcalRecHitCollection::const_iterator hit = EERecHits->begin(); hit != EERecHits->end(); ++hit) {
      ...
      for (int channelAllowed : limitDeadCellToChannelStatusEE_) {
        for (int& sit : deadNeighbourStati) {
          if (channelAllowed == sit || (channelAllowed < 0 && std::abs(channelAllowed) <= sit)) {
            passChannelLimitation = true;
            break;
      ...
      if (!detIdAlreadyChecked && deadNeighbourStati.empty() &&
        ...
        if (gapinfo.boundaryEnergy > cutBoundEnergyGapEE) {
          ...
          if (debug_)
            edm::LogInfo("EcalDeadCellBoundaryEnergyFilter")
                << "EE: gap cluster energy: " << gapinfo.boundaryEnergy << " deadCells: " << gapinfo.detIds.size();
        }
      }
      ...
      if (!detIdAlreadyChecked &&
        ...
        if (boundinfo.boundaryEnergy > cutBoundEnergyDeadCellsEE) {
          ...
          if (debug_)
            edm::LogInfo("EcalDeadCellBoundaryEnergyFilter")
                << "EE: boundary Energy dead RecHit: " << boundinfo.boundaryEnergy << " ET: " << boundinfo.boundaryET
                << " deadCells: " << boundinfo.detIds.size();
        }
      }
    }
  ...
  auto pAnomalousECALVariables = std::make_unique<AnomalousECALVariables>(
      v_enNeighboursGap_EB, v_enNeighboursGap_EE, v_boundaryInfoDeadCells_EB, v_boundaryInfoDeadCells_EE);

  bool isGap = pAnomalousECALVariables->isGapEcalCluster(cutBoundEnergyGapEB, cutBoundEnergyGapEE);
  bool isBoundary = pAnomalousECALVariables->isDeadEcalCluster(
      maxBoundaryEnergy_, limitDeadCellToChannelStatusEB_, limitDeadCellToChannelStatusEE_);
