...
private:
  ...
  const bool
      useTTsum_;  //If set to true, the filter will compare the sum of the 5x5 tower to the provided energy threshold
  ...
  int setEvtRecHitstatus(const double& tpValCut,
                         const int& chnStatus,
                         const int& towerTest,
...
EcalDeadCellTriggerPrimitiveFilter::EcalDeadCellTriggerPrimitiveFilter(const edm::ParameterSet& iConfig)
  ...
  callWhenNewProductsRegistered([this](edm::BranchDescription const& iBranch) {
    ...
    if (iBranch.moduleLabel() == ebReducedRecHitCollection_.label() ||
        iBranch.moduleLabel() == eeReducedRecHitCollection_.label()) {
      hasReducedRecHits_++;
...
void EcalDeadCellTriggerPrimitiveFilter::beginStream(edm::StreamID) {
  ...
  if (debug_)
    edm::LogInfo("EcalDeadCellTriggerPrimitiveFilter")
        << "useTPmethod_ : " << useTPmethod_ << "  useHITmethod_ : " << useHITmethod_;
}
...
void EcalDeadCellTriggerPrimitiveFilter::loadEcalDigis(edm::Event& iEvent,
  ...
  if (!pTPDigis.isValid()) {
    edm::LogWarning("EcalDeadCellTriggerPrimitiveFilter")
        << "Can't get the product " << tpDigiCollection_.instance() << " with label " << tpDigiCollection_.label();
    return;
  }
}
...
int EcalDeadCellTriggerPrimitiveFilter::setEvtRecHitstatus(const double& tpValCut,
                                                           ...
                                                           const EERecHitCollection& HitecalEE) {
  if (debug_ && verbose_ >= 2)
    edm::LogInfo("EcalDeadCellTriggerPrimitiveFilter") << "***begin setEvtTPstatusRecHits***";
...
*/
  ...
  for (ebrechit = HitecalEB.begin(); ebrechit != HitecalEB.end(); ebrechit++) {
    ...
    if (toDo) {
      ...
      if (ebrechit->checkFlag(EcalRecHit::kTPSaturated) && usekTPSaturated_)
        return 1;
      ...
      for (std::vector<DetId>::const_iterator dit = vid.begin(); dit != vid.end(); ++dit) {
        ...
        if (towerTest > 0 && bit2Itor->second.back() == towerTest)
          continue;
        if (towerTest < 0 && bit2Itor->second.back() >= abs(towerTest))
          continue;
        towerTestCnt++;
      }
      if (towerTestCnt != 0 && debug_ && verbose_ >= 2)
        edm::LogInfo("EcalDeadCellTriggerPrimitiveFilter")
            << "towerTestCnt : " << towerTestCnt << "  for towerTest : " << towerTest;

      std::vector<DetId>::iterator avoidItor;
      avoidItor = find(avoidDuplicateVec.begin(), avoidDuplicateVec.end(), det);
  ...
  for (eerechit = HitecalEE.begin(); eerechit != HitecalEE.end(); eerechit++) {
    ...
    // This might be suitable for channels with status other than 13,
    // since this function is written as a general one ...
    if (!eerechit->isRecovered())
      toDo = false;
    //     if( !eerechit->checkFlag(EcalRecHit::kTowerRecovered) ) toDo = false;

    if (toDo) {
      //If we considerkTPSaturated and a recHit has a flag set, we can immediately flag the event.
      if (eerechit->checkFlag(EcalRecHit::kTPSaturated) && usekTPSaturated_)
        return 1;
  ...
  for (ttetItor = accuTTetMap.begin(); ttetItor != accuTTetMap.end(); ttetItor++) {
    ...
    if (ttchnItor->second != 25 && debug_ && verbose_ >= 2)
      edm::LogInfo("EcalDeadCellTriggerPrimitiveFilter")
          << "WARNING ... ttchnCnt : " << ttchnItor->second << "  NOT equal  25!";

    if (ttetVal >= tpValCut) {
      isPassCut = 1;
      isPassCut *= ttzsideItor->second;
    }
  }
  ...
  for (scetItor = accuSCetMap.begin(); scetItor != accuSCetMap.end(); scetItor++) {
    ...
    if (scchnItor->second != 25 && debug_ && verbose_ >= 2)
      edm::LogInfo("EcalDeadCellTriggerPrimitiveFilter")
          << "WARNING ... scchnCnt : " << scchnItor->second << "  NOT equal  25!";

    if (scetVal >= tpValCut) {
      isPassCut = 1;
      isPassCut *= sczsideItor->second;
    }
  }

  if (debug_ && verbose_ >= 2)
    edm::LogInfo("EcalDeadCellTriggerPrimitiveFilter") << "***end setEvtTPstatusRecHits***";

  return isPassCut;
}

int EcalDeadCellTriggerPrimitiveFilter::setEvtTPstatus(EcalTrigPrimDigiCollection const& tpDigis,
                                                       const double& tpValCut,
                                                       const int& chnStatus,
                                                       EcalTPGScale& ecalScale) {
  if (debug_ && verbose_ >= 2)
    edm::LogInfo("EcalDeadCellTriggerPrimitiveFilter") << "***begin setEvtTPstatus***";
  ...
  if (debug_ && verbose_ >= 2)
    edm::LogInfo("EcalDeadCellTriggerPrimitiveFilter") << "***end setEvtTPstatus***";
