...
std::vector<HcalNoiseHPD>::iterator HcalNoiseRBXArray::findHPD(int hpdindex) {
  ...
  if (!HcalHPDRBXMap::isValidHPD(hpdindex))
    return endHPD();
  ...
  for (auto it = rbx.hpds_.begin(); it != rbx.hpds_.end(); ++it) {
    if (it->idnumber_ == hpdindex)
      return it;
  }
...
HcalNoiseRBXArray::iterator HcalNoiseRBXArray::findRBX(int rbxindex) {
  if (!HcalHPDRBXMap::isValidRBX(rbxindex))
    return endRBX();
...
std::vector<HcalNoiseHPD>::iterator HcalNoiseRBXArray::findHPD(const HBHEDataFrame& f) { return findHPD(f.id()); }

std::vector<HcalNoiseHPD>::const_iterator HcalNoiseRBXArray::findHPD(const HBHEDataFrame& f) const {
  return findHPD(f.id());
}

HcalNoiseRBXArray::iterator HcalNoiseRBXArray::findRBX(const HBHEDataFrame& f) { return findRBX(f.id()); }

HcalNoiseRBXArray::const_iterator HcalNoiseRBXArray::findRBX(const HBHEDataFrame& f) const { return findRBX(f.id()); }

std::vector<HcalNoiseHPD>::iterator HcalNoiseRBXArray::findHPD(const HBHERecHit& h) { return findHPD(h.id()); }

std::vector<HcalNoiseHPD>::const_iterator HcalNoiseRBXArray::findHPD(const HBHERecHit& h) const {
  return findHPD(h.id());
}

HcalNoiseRBXArray::iterator HcalNoiseRBXArray::findRBX(const HBHERecHit& h) { return findRBX(h.id()); }

HcalNoiseRBXArray::const_iterator HcalNoiseRBXArray::findRBX(const HBHERecHit& h) const { return findRBX(h.id()); }

void HcalNoiseRBXArray::findHPD(const CaloTower& tower,
                                std::vector<std::vector<HcalNoiseHPD>::const_iterator>& vec) const {
  // clear the vector
  vec.clear();

  // check if the tower corresponds to a valid HPD/RBX
  if (!HcalHPDRBXMap::isValid(tower.ieta(), tower.iphi()))
    return;

  // find the HPD indices
  std::vector<int> hpdindices;
  HcalHPDRBXMap::indexHPDfromEtaPhi(tower.ieta(), tower.iphi(), hpdindices);
  for (auto it = hpdindices.begin(); it != hpdindices.end(); ++it)
...
void HcalNoiseRBXArray::findRBX(const CaloTower& tower, std::vector<HcalNoiseRBXArray::iterator>& vec) {
  ...
  for (auto it = rbxindices.begin(); it != rbxindices.end(); ++it)
