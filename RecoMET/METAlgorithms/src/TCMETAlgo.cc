...
//____________________________________________________________________________||
TCMETAlgo::TCMETAlgo() : response_function_(nullptr), showerRF_(nullptr) {}
...
void TCMETAlgo::configure(const edm::ParameterSet& iConfig, edm::ConsumesCollector&& iConsumesCollector) {
  ...
  electronToken_ =
      iConsumesCollector.consumes<reco::GsfElectronCollection>(iConfig.getParameter<edm::InputTag>("electronInputTag"));
  metToken_ = iConsumesCollector.consumes<edm::View<reco::MET>>(iConfig.getParameter<edm::InputTag>("metInputTag"));
  trackToken_ =
      iConsumesCollector.consumes<reco::TrackCollection>(iConfig.getParameter<edm::InputTag>("trackInputTag"));
  beamSpotToken_ = iConsumesCollector.consumes<reco::BeamSpot>(iConfig.getParameter<edm::InputTag>("beamSpotInputTag"));
  vertexToken_ =
      iConsumesCollector.consumes<reco::VertexCollection>(iConfig.getParameter<edm::InputTag>("vertexInputTag"));

  usePFClusters_ = iConfig.getParameter<bool>("usePFClusters");

  if (usePFClusters_) {
    clustersECALToken_ =
        iConsumesCollector.consumes<reco::PFClusterCollection>(iConfig.getParameter<edm::InputTag>("PFClustersECAL"));
    clustersHCALToken_ =
        iConsumesCollector.consumes<reco::PFClusterCollection>(iConfig.getParameter<edm::InputTag>("PFClustersHCAL"));
    clustersHFToken_ =
        iConsumesCollector.consumes<reco::PFClusterCollection>(iConfig.getParameter<edm::InputTag>("PFClustersHF"));
  }

  muonDepValueMapToken_ = iConsumesCollector.consumes<edm::ValueMap<reco::MuonMETCorrectionData>>(
  ...
  std::transform(
      trackAlgoNames.begin(), trackAlgoNames.end(), std::back_inserter(trackAlgos_), [](const std::string& name) {
...
reco::MET TCMETAlgo::CalculateTCMET(edm::Event& event, const edm::EventSetup& setup) {
  if (usePFClusters_)
    initialize_MET_with_PFClusters(event);
  else
    initialize_MET_with_CaloMET(event);
...
void TCMETAlgo::initialize_MET_with_CaloMET(edm::Event& event) {
  ...
  met_x_ = (metHandle->front()).px();
  met_y_ = (metHandle->front()).py();
  sumEt_ = (metHandle->front()).sumEt();
}
...
void TCMETAlgo::correct_MET_for_Muons() {
  for (unsigned int i = 0; i < muonHandle_->size(); ++i) {
    ...
    reco::MuonMETCorrectionData muCorrData = (*tcmetDepValueMapHandle_)[muonRef];

    reco::MuonMETCorrectionData::Type muCorrType = muCorrData.type();
    if (muCorrType == reco::MuonMETCorrectionData::Type::CombinedTrackUsed ||
        muCorrType == reco::MuonMETCorrectionData::Type::GlobalTrackUsed) {
      if (!muonRef->isGlobalMuon()) {
        edm::LogError("TCMETAlgo") << "This is not a global muon, but is flagged as one by the TCMET ValueMap.  "
                                   << "Not correcting for this muon.  Check your collection!!!" << std::endl;
        continue;
      }

      reco::TrackRef track = muonRef->globalTrack();
      correctMETforMuon(track, muonRef);
    } else if (muCorrType == reco::MuonMETCorrectionData::Type::InnerTrackUsed ||
               muCorrType == reco::MuonMETCorrectionData::Type::TrackUsed) {
      ...
      correctMETforMuon(track, muonRef);
    } else if (muCorrType == reco::MuonMETCorrectionData::Type::OuterTrackUsed ||
               muCorrType == reco::MuonMETCorrectionData::Type::StandAloneTrackUsed) {
      ...
      reco::TrackRef track = muonRef->outerTrack();
      correctMETforMuon(track, muonRef);
    } else if (muCorrType == reco::MuonMETCorrectionData::Type::TreatedAsPion) {
      reco::TrackRef track;
      if (muonRef->isGlobalMuon())
        track = muonRef->innerTrack();
      else if (muonRef->isTrackerMuon())
        track = muonRef->innerTrack();
      else if (muonRef->isCaloMuon())
        track = muonRef->innerTrack();
      else {
        edm::LogError("TCMETAlgo")
            << "You are attempting to correct a muon that does not have an associated track as a pion!"
            << "Check your value map!" << std::endl;
        continue;
      }
    ...
    } else if (muCorrType == reco::MuonMETCorrectionData::Type::MuonP4V4QUsed ||
               muCorrType == reco::MuonMETCorrectionData::Type::MuonCandidateValuesUsed) {
      if (!muonRef->isGlobalMuon() && !muonRef->isTrackerMuon() && muonRef->isStandAloneMuon())
        edm::LogVerbatim("TCMETAlgo") << "Are you sure you want to correct using the StandAlone fit??" << std::endl;

      correctMETforMuon(muonRef);
    } else if (muCorrType == reco::MuonMETCorrectionData::Type::NotUsed)
      continue;
    else
...
void TCMETAlgo::correct_MET_for_Tracks() {
  hasValidVertex_ = false;
  if (usePvtxd0_ && vertexHandle_.isValid()) {
    vertexColl_ = vertexHandle_.product();
    hasValidVertex_ = isValidVertex();
  }
  ...
  for (unsigned int i = 0; i < nTracks; ++i) {
    ...
    if ((!isCosmics_) && isElectron(trackRef))
      continue;

    if (!isGoodTrack(trackRef))
      continue;

    if (electronVetoCone_ && closeToElectron(trackRef))
      continue;

    const TVector3 outerTrackPosition = propagateTrackToCalorimeterFace(trackRef);

    if (correctShowerTracks_) {
      if (usedeltaRRejection_ && nearGoodShowerTrack(trackRef, goodShowerTracks))
        continue;
...
void TCMETAlgo::findDuplicateTracks() {
  ...
  for (unsigned int trk_idx = 0; trk_idx < nTracks; trk_idx++) {
    ...
    if (isMuon(trkref))
      continue;
    if (!isCosmics_ && isElectron(trkref))
      continue;

    if (trkref->pt() < dupMinPt_)
      continue;
    ...
    for (unsigned int trk_idx2 = trk_idx + 1; trk_idx2 < nTracks; trk_idx2++) {
      ...
      if (isMuon(trkref2))
        continue;
      if (!isCosmics_ && isElectron(trkref2))
        continue;

      if (trkref->charge() * trkref2->charge() < 0)
        continue;
      if (trkref2->pt() < dupMinPt_)
        continue;
...
int TCMETAlgo::vetoTrack(int i1, int i2) {
  ...
  reco::TrackRef t1(trackHandle_, i1);
  reco::TrackRef t2(trackHandle_, i2);

  if (t1->numberOfValidHits() < t2->numberOfValidHits())
    return i1;
  else if (t1->numberOfValidHits() > t2->numberOfValidHits())
    return i2;
  else if (t1->chi2() / t1->ndof() > t2->chi2() / t2->ndof())
    return i1;
  else if (t1->chi2() / t1->ndof() < t2->chi2() / t2->ndof())
    return i2;
  else if (t1->ptError() > t2->ptError())
    return i1;
  else if (t1->ptError() < t2->ptError())
    return i2;
  else
...
bool TCMETAlgo::isValidVertex() {
  if (vertexColl_->begin()->isFake())
    return false;
  if (vertexColl_->begin()->ndof() < vertexNdof_)
    return false;
  if (fabs(vertexColl_->begin()->z()) > vertexZ_)
    return false;
  if (sqrt(pow(vertexColl_->begin()->x(), 2) + pow(vertexColl_->begin()->y(), 2)) > vertexRho_)
    return false;
...
bool TCMETAlgo::closeToElectron(const reco::TrackRef track) {
  ...
  for (reco::GsfElectronCollection::const_iterator electron_it = electronHandle_->begin();
    ...
    if (!el_track.isNonnull())
      continue;

    if (el_track->pt() < eVetoMinElectronPt_)
      continue;
    ...
    if (deltaPhi < eVetoDeltaPhi_ && deltaCotTh < eVetoDeltaCotTheta_ && deltaR < eVetoDeltaR_)
      return true;
  }
...
bool TCMETAlgo::nearGoodShowerTrack(const reco::TrackRef track, const std::vector<int>& goodShowerTracks) {
  ...
  for (unsigned int itrk = 0; itrk < goodShowerTracks.size(); itrk++) {
    reco::TrackRef trkref(trackHandle_, itrk);
...
//____________________________________________________________________________||
int TCMETAlgo::nLayers(const reco::TrackRef track) { return track->hitPattern().trackerLayersWithMeasurement(); }
...
bool TCMETAlgo::isGoodTrack(const reco::TrackRef track) {
  ...
  if (hasValidVertex_) {
    ...
    if (fabs(dz) < vertexMaxDZ_) {
      ...
      Point bspot = haveBeamSpot ? beamSpotHandle_->position() : Point(0, 0, 0);
  ...
  if (std::find(trackAlgos_.begin(), trackAlgos_.end(), track->algo()) != trackAlgos_.end()) {
    ...
    if (fabs(d0) > d0cut)
      return false;
    if (nLayers(track) < nLayers_)
      return false;

  } else {
    //last 2 tracking iterations (tighten chi2, nhits, pt error, nlayers cuts)
    if (track->normalizedChi2() > maxchi2_tight_)
      return false;
    if (track->numberOfValidHits() < minhits_tight_)
      return false;
    if ((track->ptError() / track->pt()) > maxPtErr_tight_)
      return false;
    if (nLayers(track) < nLayersTight_)
      return false;
  }

  if (track->numberOfValidHits() < minhits_)
    return false;
  if (track->normalizedChi2() > maxchi2_)
    return false;
  if (fabs(track->eta()) > maxeta_)
    return false;
  if (track->pt() > maxpt_)
    return false;
  if ((track->ptError() / track->pt()) > maxPtErr_)
    return false;
  if (fabs(track->eta()) > 2.5 && track->pt() > maxpt_eta25_)
    return false;
  if (fabs(track->eta()) > 2.0 && track->pt() > maxpt_eta20_)
    return false;
  ...
  if (vetoDuplicates_) {
    for (unsigned int iDup = 0; iDup < duplicateTracks_.size(); ++iDup) {
      if ((int)track.key() == duplicateTracks_.at(iDup))
        return false;
    }
  }
...
void TCMETAlgo::correctMETforMuon(const reco::TrackRef track, reco::MuonRef& muonRef) {
  ...
  met_x_ -= (track->px() - delx);
  met_y_ -= (track->py() - dely);
  sumEt_ += (track->pt() - TMath::Sqrt(delx * delx + dely * dely));
}
...
void TCMETAlgo::correctMETforMuon(reco::MuonRef& muonRef) {
  ...
  met_x_ -= (muonRef->px() - delx);
  met_y_ -= (muonRef->py() - dely);
  sumEt_ += (muonRef->pt() - TMath::Sqrt(delx * delx + dely * dely));
}

//____________________________________________________________________________||
void TCMETAlgo::correctMETforTrack(const reco::TrackRef track, TH2D* rf, const TVector3& outerTrackPosition) {
  if (track->pt() < minpt_) {
    met_x_ -= track->pt() * cos(track->phi());
    met_y_ -= track->pt() * sin(track->phi());
  } else {
    if (checkTrackPropagation_ && outerTrackPosition.Theta() < -9.99) {
      ...
      met_x_ += (fracTrackEnergy * track->p() * sin(outerTrackPosition.Theta()) *
                     cos(outerTrackPosition.Phi())    //remove expected amount of energy track deposited in calorimeter
                 - track->pt() * cos(track->phi()));  //add track at vertex

      met_y_ += (fracTrackEnergy * track->p() * sin(outerTrackPosition.Theta()) *
                     sin(outerTrackPosition.Phi())    //remove expected amount of energy track deposited in calorimeter
...
void TCMETAlgo::correctSumEtForTrack(const reco::TrackRef track, TH2D* rf, const TVector3& outerTrackPosition) {
  if (track->pt() < minpt_) {
    ...
    if (checkTrackPropagation_ && outerTrackPosition.Theta() < -9.99) {
      ...
      int bin_index = rf->FindBin(track->eta(), track->pt());
      double fracTrackEnergy = rf->GetBinContent(bin_index);  //get correction factor from response function
      sumEt_ += (1 - fracTrackEnergy) * track->pt();
    }
  }
}
...
TH2D* TCMETAlgo::getResponseFunction_shower() {
  Double_t xAxis1[53] = {-2.5,   -2.322, -2.172, -2.043, -1.93,  -1.83,  -1.74,  -1.653, -1.566, -1.479, -1.392,
                         -1.305, -1.218, -1.131, -1.044, -0.957, -0.879, -0.783, -0.696, -0.609, -0.522, -0.435,
                         -0.348, -0.261, -0.174, -0.087, 0,      0.087,  0.174,  0.261,  0.348,  0.435,  0.522,
                         0.609,  0.696,  0.783,  0.879,  0.957,  1.044,  1.131,  1.218,  1.305,  1.392,  1.479,
...
TH2D* TCMETAlgo::getResponseFunction_mode() {
  ...
  Double_t xAxis2[53] = {-2.5,   -2.322, -2.172, -2.043, -1.93,  -1.83,  -1.74,  -1.653, -1.566, -1.479, -1.392,
                         -1.305, -1.218, -1.131, -1.044, -0.957, -0.879, -0.783, -0.696, -0.609, -0.522, -0.435,
                         -0.348, -0.261, -0.174, -0.087, 0,      0.087,  0.174,  0.261,  0.348,  0.435,  0.522,
                         0.609,  0.696,  0.783,  0.879,  0.957,  1.044,  1.131,  1.218,  1.305,  1.392,  1.479,
                         1.566,  1.653,  1.74,   1.83,   1.93,   2.043,  2.172,  2.322,  2.5};

  Double_t yAxis2[29] = {0,  0.5, 1,  1.5, 2,  3,  4,  5,  6,  7,  8,  9,  10, 12, 14,
                         16, 18,  20, 25,  30, 35, 40, 45, 50, 60, 70, 80, 90, 100};
