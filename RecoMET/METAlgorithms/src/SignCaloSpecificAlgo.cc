...
// $Id: SignCaloSpecificAlgo.cc,v 1.5 2009/08/21 11:39:02 fblekman Exp $
//
//
...
{ 
  ...
  if( towers->size() == 0 )  // if there are no towers, return specific = 0
    {
      edm::LogInfo("SignCaloSpecificAlgo") << "[CaloMET] Number of Candidate CaloTowers is zero : Unable to calculate calo specific info. " << endl;
...
{ 
  if( towers->size() == 0 )  // if there are no towers, return specific = 0
    {
      edm::LogInfo("SignCaloSpecificAlgo") << "[CaloMET] Number of Candidate CaloTowers is zero : Unable to calculate METSignificance"  << endl;
      return 0.0;
    }
  //retreive calo tower information from candidates
  //start with the first element of the candidate list
