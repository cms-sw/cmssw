...
NoPileUpPFMEtDataProducer::NoPileUpPFMEtDataProducer(
    ...
    : moduleLabel_(cfg.getParameter<std::string>("@module_label")),
      looseJetIdAlgo_(nullptr), pfMEtSignInterface_(nullptr) {
  srcJets_ = consumes<reco::PFJetCollection>(
      cfg.getParameter<edm::InputTag>("srcJets"));
  srcJetIds_ = consumes<edm::ValueMap<int>>(
      cfg.getParameter<edm::InputTag>("srcJetIds"));
  minJetPt_ = cfg.getParameter<double>("minJetPt");
  std::string jetIdSelection_string =
      cfg.getParameter<std::string>("jetIdSelection");
  if (jetIdSelection_string == "loose")
    ...
    throw cms::Exception("NoPileUpPFMEtDataProducer")
        << "Invalid Configuration Parameter 'jetIdSelection' = "
        << jetIdSelection_string << " !!\n";
  jetEnOffsetCorrLabel_ = cfg.getParameter<std::string>("jetEnOffsetCorrLabel");

  srcPFCandidates_ = consumes<reco::PFCandidateCollection>(
      cfg.getParameter<edm::InputTag>("srcPFCandidates"));
  srcPFCandidatesView_ = consumes<edm::View<reco::PFCandidate>>(
      cfg.getParameter<edm::InputTag>("srcPFCandidates"));
  srcPFCandToVertexAssociations_ = consumes<PFCandToVertexAssMap>(
      cfg.getParameter<edm::InputTag>("srcPFCandToVertexAssociations"));
  srcJetsForMEtCov_ = mayConsume<reco::PFJetCollection>(
      cfg.getParameter<edm::InputTag>("srcJetsForMEtCov"));
  minJetPtForMEtCov_ = cfg.getParameter<double>("minJetPtForMEtCov");
  srcHardScatterVertex_ = consumes<reco::VertexCollection>(
      cfg.getParameter<edm::InputTag>("srcHardScatterVertex"));
  ...
  pfMEtSignInterface_ = new PFMEtSignInterfaceBase(
      cfg.getParameter<edm::ParameterSet>("resolution"));

  maxWarnings_ =
      (cfg.exists("maxWarnings")) ? cfg.getParameter<int>("maxWarnings") : 1;
  numWarnings_ = 0;
...
namespace {
void setPFCandidateFlag(
    const reco::PFJet &pfJet,
    const edm::View<reco::PFCandidate> &pfCandidateCollection,
    std::vector<int> &flags, int value, int &numWarnings, int maxWarnings,
  ...
  for (std::vector<reco::PFCandidatePtr>::const_iterator pfJetConstituent =
           pfConsts.begin();
       pfJetConstituent != pfConsts.end(); ++pfJetConstituent) {
    std::vector<int> idxs;
    if (!pfCandidateCollection.empty() &&
        pfJetConstituent->id() == viewProductID) {
      idxs.push_back(pfJetConstituent->key());
    } else {
      bool isMatched_fast = false;
      if (pfJetConstituent->key() < pfCandidateCollection.size()) {
        edm::Ptr<reco::PFCandidate> pfCandidatePtr =
            pfCandidateCollection.ptrAt(pfJetConstituent->key());
      ...
      if (!isMatched_fast) {
        ...
        for (size_t iPFCandidate = 0; iPFCandidate < numPFCandidates;
             ++iPFCandidate) {
          edm::Ptr<reco::PFCandidate> pfCandidatePtr =
              pfCandidateCollection.ptrAt(iPFCandidate);
          double dR2 = deltaR2((*pfJetConstituent)->p4(), pfCandidatePtr->p4());
          if (dR2 < dR2Min) {
            idxs.push_back(pfCandidatePtr.key());
          }
        ...
        if (numWarnings < maxWarnings) {
          edm::LogWarning("setPFCandidateFlag")
              << " The productIDs of PFJetConstituent and "
                 "PFCandidateCollection passed as function arguments don't "
                 "match.\n"
              << "NOTE: The return value will be unaffected, but the code will "
                 "run MUCH slower !!";
          ++numWarnings;
        }
      }
    ...
    if (!idxs.empty()) {
      ...
      edm::LogError("setPFCandidateFlag")
          << " Failed to associated PFJetConstituent with index = "
          << pfJetConstituent->key() << " to any PFCandidate !!";
    }
  }
}
} // namespace
...
void NoPileUpPFMEtDataProducer::produce(edm::Event &evt,
  ...
  std::vector<const reco::PFJet *> pfCandidateToJetAssociations(
      pfCandidates->size());
  ...
  if (!jetEnOffsetCorrLabel_.empty()) {
    jetEnOffsetCorrector =
        JetCorrector::getJetCorrector(jetEnOffsetCorrLabel_, es);
  ...
  for (size_t iJet = 0; iJet < numJets; ++iJet) {
    ...
    if (!passesLooseJetId) {
      setPFCandidateFlag(*jet, *pfCandidates, pfCandidateFlags,
                         flag_isWithinFakeJet, numWarnings_, maxWarnings_);
    }
    setPFCandidateFlag(*jet, *pfCandidates, pfCandidateFlags,
                       flag_isWithinSelectedJet, numWarnings_, maxWarnings_);

    reco::PUSubMETCandInfo jetInfo;
    jetInfo.setP4(jet->p4());
    int jetId = (*jetIds)[jet];
    bool jetIdSelection_passed =
        PileupJetIdentifier::passJetId(jetId, jetIdSelection_);
    jetInfo.setType((jetIdSelection_passed) ? reco::PUSubMETCandInfo::kHS
                                            : reco::PUSubMETCandInfo::kPU);
    jetInfo.setPassesLooseJetId(passesLooseJetId);
    double jetEnergy_uncorrected =
        jet->chargedHadronEnergy() + jet->neutralHadronEnergy() +
        jet->photonEnergy() + jet->electronEnergy() + jet->muonEnergy() +
        jet->HFHadronEnergy() + jet->HFEMEnergy();
    double jetPx_uncorrected =
        cos(jet->phi()) * sin(jet->theta()) * jetEnergy_uncorrected;
    double jetPy_uncorrected =
        sin(jet->phi()) * sin(jet->theta()) * jetEnergy_uncorrected;
    double jetPz_uncorrected = cos(jet->theta()) * jetEnergy_uncorrected;
    reco::Candidate::LorentzVector rawJetP4(
        jetPx_uncorrected, jetPy_uncorrected, jetPz_uncorrected,
        jetEnergy_uncorrected);
    reco::PFJet rawJet(*jet);
    rawJet.setP4(rawJetP4);
    double jetNeutralEnFrac =
        (jetEnergy_uncorrected > 0.)
            ? (jet->neutralEmEnergy() + jet->neutralHadronEnergy()) /
                  jetEnergy_uncorrected
            : -1.;
    ...
    jetInfo.setOffsetEnCorr(
        (jetEnOffsetCorrector)
            ? rawJet.energy() *
                  (1. - jetEnOffsetCorrector->correction(rawJet, evt, es))
            : 0.);
  ...
  for (reco::PFJetCollection::const_iterator jet = jets->begin();
    ...
    if (jet->pt() > minJetPtForMEtCov_) {
      setPFCandidateFlag(*jet, *pfCandidates, pfCandidateFlags,
                         flag_isWithinJetForMEtCov, numWarnings_, maxWarnings_,
  ...
  for (size_t iPFCandidate = 0; iPFCandidate < numPFCandidates;
    ...
    // CV: need to call isVertexAssociated_fast instead of isVertexAssociated
    // function
    //    (makes run-time of MVAPFMEtDataProducer::produce decrease from ~1s per
    //    event to ~0.35s per event)
    // int vtxAssociationType = isVertexAssociated(*pfCandidatePtr,
    // *pfCandToVertexAssociations, *hardScatterVertex, dZcut_);
    ...
    int vtxAssociationType = noPuUtils::isVertexAssociated_fast(
        pfCandidateRef, pfCandToVertexAssociations_reversed, *hardScatterVertex,
        dZcut_, numWarnings_, maxWarnings_);
    bool isHardScatterVertex_associated =
    ...
    pfCandInfo.setIsWithinJet(
        (pfCandidateFlags[idx] & flag_isWithinSelectedJet));
    if (pfCandInfo.isWithinJet())
      pfCandInfo.setPassesLooseJetId(
          (pfCandidateFlags[idx] & flag_isWithinFakeJet));
    else
      pfCandInfo.setPassesLooseJetId(true);

    // CV: for PFCandidates that are within PFJets (of Pt between
    // 'minJetPtForMEtCov' and 'minJetPt'),
    ...
    if (jet_matched) {
      metsig::SigInputObj pfCandResolution =
          pfMEtSignInterface_->compResolution(pfCandidatePtr.get());
      metsig::SigInputObj jetResolution =
          pfMEtSignInterface_->compResolution(jet_matched);

      metsig::SigInputObj metSign;
      metSign.set(pfCandResolution.get_type(), pfCandResolution.get_energy(),
                  pfCandResolution.get_phi(),
                  jetResolution.get_sigma_e() *
                      (pfCandidatePtr->energy() / jet_matched->energy()),
                  jetResolution.get_sigma_tan());
      pfCandInfo.setMEtSignObj(metSign);
    } else {
      pfCandInfo.setMEtSignObj(
          pfMEtSignInterface_->compResolution(pfCandidatePtr.get()));
    }
