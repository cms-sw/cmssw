...
PFMETProducerMVA::PFMETProducerMVA(const edm::ParameterSet &cfg)
  ...
  srcCorrJets_ = consumes<reco::PFJetCollection>(
      cfg.getParameter<edm::InputTag>("srcCorrJets"));
  srcUncorrJets_ = consumes<reco::PFJetCollection>(
      cfg.getParameter<edm::InputTag>("srcUncorrJets"));
  srcJetIds_ = consumes<edm::ValueMap<float>>(
      cfg.getParameter<edm::InputTag>("srcMVAPileupJetId"));
  srcPFCandidatesView_ = consumes<reco::CandidateView>(
      cfg.getParameter<edm::InputTag>("srcPFCandidates"));
  srcVertices_ = consumes<reco::VertexCollection>(
      cfg.getParameter<edm::InputTag>("srcVertices"));
  vInputTag srcLeptonsTags = cfg.getParameter<vInputTag>("srcLeptons");
  for (vInputTag::const_iterator it = srcLeptonsTags.begin();
       it != srcLeptonsTags.end(); it++) {
    srcLeptons_.push_back(consumes<reco::CandidateView>(*it));
  }
  mJetCorrector_ = consumes<reco::JetCorrector>(
      cfg.getParameter<edm::InputTag>("corrector"));
  ...
  verbosity_ =
      (cfg.exists("verbosity")) ? cfg.getParameter<int>("verbosity") : 0;
...
void PFMETProducerMVA::produce(edm::Event &evt, const edm::EventSetup &es) {
  ...
  if (minNumLeptons_ > 0) {
    int numLeptons = 0;
    for (std::vector<edm::EDGetTokenT<reco::CandidateView>>::const_iterator
             srcLeptons_i = srcLeptons_.begin();
    ...
    if (!(numLeptons >= minNumLeptons_)) {
      LogDebug("produce") << "<PFMETProducerMVA::produce>:" << std::endl
                          << "Run: " << evt.id().run()
                          << ", LS: " << evt.luminosityBlock()
                          << ", Event: " << evt.id().event() << std::endl
                          << " numLeptons = " << numLeptons
                          << ", minNumLeptons = " << minNumLeptons_
                          << " --> skipping !!" << std::endl;
  ...
  // take vertex with highest sum(trackPt) as the vertex of the "hard scatter"
  // interaction
  // (= first entry in vertex collection)
  const reco::Vertex *hardScatterVertex =
      (!vertices->empty()) ? &(vertices->front()) : nullptr;
  ...
  std::vector<reco::PUSubMETCandInfo> leptonInfo =
      computeLeptonInfo(srcLeptons_, *pfCandidates_view, hardScatterVertex, lId,
                        lHasPhotons, evt);
  ...
  CommonMETData pfMEt_data =
      metAlgo_.run((*pfCandidates_view), globalThreshold_);
  SpecificPFMETData specificPfMET =
      pfMEtSpecificAlgo_.run((*pfCandidates_view));
  const reco::Candidate::LorentzVector p4(pfMEt_data.mex, pfMEt_data.mey, 0.0,
                                          pfMEt_data.met);
  ...
  std::vector<reco::PUSubMETCandInfo> pfCandidateInfo =
      computePFCandidateInfo(*pfCandidates_view, hardScatterVertex);
  std::vector<reco::PUSubMETCandInfo> jetInfo = computeJetInfo(
      *uncorrJets, corrJets, *jetIds, *vertices, hardScatterVertex, *corrector,
      evt, es, leptonInfo, pfCandidateInfo);
  ...
  LogDebug("produce") << "Run: " << evt.id().run()
                      << ", LS: " << evt.luminosityBlock()
                      << ", Event: " << evt.id().event() << std::endl
                      << " PFMET: Pt = " << pfMEtP4_original.pt()
                      << ", phi = " << pfMEtP4_original.phi() << " "
                      << "(Px = " << pfMEtP4_original.px()
                      << ", Py = " << pfMEtP4_original.py() << ")" << std::endl
                      << " MVA MET: Pt = " << pfMEt.pt()
                      << " phi = " << pfMEt.phi() << " (Px = " << pfMEt.px()
                      << ", Py = " << pfMEt.py() << ")" << std::endl
                      << " Cov:" << std::endl
                      << (mvaMEtAlgo_.getMEtCov())(0, 0) << "  "
                      << (mvaMEtAlgo_.getMEtCov())(0, 1) << std::endl
                      << (mvaMEtAlgo_.getMEtCov())(1, 0) << "  "
                      << (mvaMEtAlgo_.getMEtCov())(1, 1) << std::endl
                      << std::endl;
...
std::vector<reco::PUSubMETCandInfo> PFMETProducerMVA::computeLeptonInfo(
    ...
    const reco::Vertex *hardScatterVertex, int &lId, bool &lHasPhotons,
    edm::Event &evt) {
  ...
  for (std::vector<edm::EDGetTokenT<reco::CandidateView>>::const_iterator
    ...
    for (reco::CandidateView::const_iterator lepton1 = leptons->begin();
      ...
      bool pMatch = false;
      for (std::vector<edm::EDGetTokenT<reco::CandidateView>>::const_iterator
               srcLeptons_j = srcLeptons_.begin();
        ...
        for (reco::CandidateView::const_iterator lepton2 = leptons2->begin();
          ...
          if (pMatch && lepton1->pt() == lepton2->pt()) {
            ...
            for (unsigned int i0 = 0; i0 < leptonInfo.size(); i0++) {
              if (std::abs(lepton1->pt() - leptonInfo[i0].p4().pt()) <
                  dPtMatch) {
                pMatch = true;
                break;
              }
      ...
      pLeptonInfo.setChargedEnFrac(
          chargedEnFrac(&(*lepton1), pfCandidates_view, hardScatterVertex));
...
std::vector<reco::PUSubMETCandInfo> PFMETProducerMVA::computeJetInfo(
    const reco::PFJetCollection &uncorrJets,
    const edm::Handle<reco::PFJetCollection> &corrJets,
    const edm::ValueMap<float> &jetIds, const reco::VertexCollection &vertices,
    const reco::Vertex *hardScatterVertex, const reco::JetCorrector &iCorrector,
  ...
  for (reco::PFJetCollection::const_iterator uncorrJet = uncorrJets.begin();
    ...
    for (size_t cjIdx = 0; cjIdx < corrJets->size(); ++cjIdx, ++corrJet) {
      ...
      float chEnF =
          (uncorrJet->chargedEmEnergy() + uncorrJet->chargedHadronEnergy() +
           uncorrJet->chargedMuEnergy()) /
          uncorrJet->energy();
      if (useType1_)
...
std::vector<reco::PUSubMETCandInfo> PFMETProducerMVA::computePFCandidateInfo(
  ...
  for (reco::CandidateView::const_iterator pfCandidate = pfCandidates.begin();
    ...
    if (hardScatterVertex) {
      const reco::PFCandidate *pfc =
          dynamic_cast<const reco::PFCandidate *>(&(*pfCandidate));
      if (pfc != nullptr) { // PF candidate for RECO and PAT levels
        ...
        const pat::PackedCandidate *pfc =
            dynamic_cast<const pat::PackedCandidate *>(&(*pfCandidate));
...
std::vector<reco::Vertex::Point>
PFMETProducerMVA::computeVertexInfo(const reco::VertexCollection &vertices) {
...
bool PFMETProducerMVA::passPFLooseId(const PFJet *iJet) {
  ...
  if (iJet->nConstituents() < 2)
    return false;
  if (iJet->chargedHadronEnergy() / iJet->energy() <= 0 &&
      std::abs(iJet->eta()) < 2.4)
    return false;
  if (iJet->chargedEmEnergy() / iJet->energy() > 0.99 &&
      std::abs(iJet->eta()) < 2.4)
    return false;
...
double PFMETProducerMVA::chargedFracInCone(
    const reco::Candidate *iCand, const reco::CandidateView &pfCandidates,
