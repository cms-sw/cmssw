...
private:
  ...
  computeJetInfo(const reco::PFJetCollection &,
  ...
  std::vector<reco::PUSubMETCandInfo>
  computePFCandidateInfo(const reco::CandidateView &, const reco::Vertex *);
  std::vector<reco::Vertex::Point>
  computeVertexInfo(const reco::VertexCollection &);
  ...
  int minNumLeptons_; // CV: option to skip MVA MET computation in case there
                      // are less than specified number of leptons in the event
