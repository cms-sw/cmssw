...
JVFJetIdProducer::JVFJetIdProducer(const edm::ParameterSet &cfg) {
  ...
  std::string neutralJetOption_string =
      cfg.getParameter<std::string>("neutralJetOption");
  if (neutralJetOption_string == "PU")
    ...
    throw cms::Exception("JVFJetIdProducer")
        << "Invalid Configuration Parameter 'neutralJetOption' = "
        << neutralJetOption_string << " !!\n";
...
double computeJVF(const reco::PFJet &jet,
                  ...
                  double minTrackPt, int verbosity) {
  ...
  for (std::vector<reco::PFCandidatePtr>::const_iterator jetConstituent =
           pfConsts.begin();
       jetConstituent != pfConsts.end(); ++jetConstituent) {

    if ((*jetConstituent)->charge() == 0)
      continue;

    double trackPt = 0.;
    if ((*jetConstituent)->gsfTrackRef().isNonnull() &&
        (*jetConstituent)->gsfTrackRef().isAvailable())
      trackPt = (*jetConstituent)->gsfTrackRef()->pt();
    else if ((*jetConstituent)->trackRef().isNonnull() &&
             (*jetConstituent)->trackRef().isAvailable())
      trackPt = (*jetConstituent)->trackRef()->pt();
    else
      trackPt = (*jetConstituent)->pt();

    if (trackPt > minTrackPt) {
      int jetConstituent_vtxAssociationType = noPuUtils::isVertexAssociated(
          (*jetConstituent), pfCandToVertexAssociations, vertices, dZ);
      // isVertexAssociated_fast(pfCandidateRef,
      // pfCandToVertexAssociations_reversed, *hardScatterVertex, dZcut_,
      // numWarnings_, maxWarnings_);
      bool jetConstituent_isVtxAssociated =
          (jetConstituent_vtxAssociationType == noPuUtils::kChHSAssoc);
  ...
  LogDebug("computeJVF") << "trackSum: associated = "
                         << trackSum_isVtxAssociated
                         << ", unassociated = " << trackSum_isNotVtxAssociated
                         << std::endl
                         << " --> JVF = " << jvf << std::endl;
...
void JVFJetIdProducer::produce(edm::Event &evt, const edm::EventSetup &es) {
  ...
  for (size_t iJet = 0; iJet < numJets; ++iJet) {
    ...
    double jetJVF =
        computeJVF(*jet, *pfCandToVertexAssociations, *hardScatterVertex,
                   dZcut_, minTrackPt_, verbosity_ && jet->pt() > 20.);
  ...
  edm::ValueMap<double>::Filler jetIdDiscriminantFiller(
      *jetIdDiscriminants_ptr);
  jetIdDiscriminantFiller.insert(jets, jetIdDiscriminants.begin(),
                                 jetIdDiscriminants.end());
