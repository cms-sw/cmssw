...
//#include "DataFormats/METReco/interface/PFMEtSignCovMatrix.h" //never used so
// far
...
NoPileUpPFMEtProducer::NoPileUpPFMEtProducer(const edm::ParameterSet &cfg)
  ...
  srcMEt_ = consumes<reco::PFMETCollection>(
      cfg.getParameter<edm::InputTag>("srcMEt"));
  srcMEtCov_ =
      edm::InputTag(); // MM, disabled for the moment until we really need it
  //( cfg.exists("srcMEtCov") ) ?
  //  consumes<edm::Handle<> >(cfg.getParameter<edm::InputTag>("srcMEtCov")) :
  //  edm::InputTag();
  srcJetInfo_ = consumes<reco::PUSubMETCandInfoCollection>(
      cfg.getParameter<edm::InputTag>("srcPUSubMETDataJet"));
  srcJetInfoLeptonMatch_ = consumes<reco::PUSubMETCandInfoCollection>(
      cfg.getParameter<edm::InputTag>("srcPUSubMETDataJetLeptonMatch"));
  srcPFCandInfo_ = consumes<reco::PUSubMETCandInfoCollection>(
      cfg.getParameter<edm::InputTag>("srcPUSubMETDataPFCands"));
  srcPFCandInfoLeptonMatch_ = consumes<reco::PUSubMETCandInfoCollection>(
      cfg.getParameter<edm::InputTag>("srcPUSubMETDataPFCandsLeptonMatch"));
  ...
  srcType0Correction_ = consumes<CorrMETData>(
      cfg.getParameter<edm::InputTag>("srcType0Correction"));
  ...
  saveInputs_ =
      (cfg.exists("saveInputs")) ? cfg.getParameter<bool>("saveInputs") : false;
...
void scaleAndAddPFMEtSignObjects(
    std::vector<metsig::SigInputObj> &metSignObjects_scaled,
    const std::vector<metsig::SigInputObj> &metSignObjects, double sf,
    double sfMin, double sfMax) {
  double sf_value = sf;
  if (sf_value > sfMax)
    sf_value = sfMax;
  if (sf_value < sfMin)
    sf_value = sfMin;
  for (std::vector<metsig::SigInputObj>::const_iterator metSignObject =
           metSignObjects.begin();
       metSignObject != metSignObjects.end(); ++metSignObject) {
    metsig::SigInputObj metSignObject_scaled;
    metSignObject_scaled.set(
        metSignObject->get_type(), sf_value * metSignObject->get_energy(),
        metSignObject->get_phi(), sf_value * metSignObject->get_sigma_e(),
        metSignObject->get_sigma_tan());
    metSignObjects_scaled.push_back(metSignObject_scaled);
  }
}

reco::METCovMatrix computePFMEtSignificance(
...
void printP4(const std::string &label_part1, int idx,
             const std::string &label_part2, const reco::Candidate &candidate) {
  std::cout << label_part1 << " #" << idx << label_part2
            << ": Pt = " << candidate.pt() << ", eta = " << candidate.eta()
            << ", phi = " << candidate.phi()
...
void printCommonMETData(const std::string &label,
  ...
  std::cout << label << ": Px = " << metData.mex << ", Py = " << metData.mey
            << ", sumEt = " << metData.sumet << std::endl;
}

void printMVAMEtJetInfo(const std::string &label, int idx,
  ...
  if (jet.type() == reco::PUSubMETCandInfo::kHS)
    ...
    std::cout << "PU";
  std::cout << "): Pt = " << jet.p4().pt() << ", eta = " << jet.p4().eta()
            << ", phi = " << jet.p4().phi();
  std::cout << " id. flags: anti-noise = " << jet.passesLooseJetId()
            << std::endl;
  std::cout << std::endl;
}

void printMVAMEtPFCandInfo(const std::string &label, int idx,
  ...
  if (pfCand.type() == reco::PUSubMETCandInfo::kChHS)
    ...
    std::cout << "neutral";
  std::cout << "): Pt = " << pfCand.p4().pt() << ", eta = " << pfCand.p4().eta()
            << ", phi = " << pfCand.p4().phi();
  ...
  if (pfCand.isWithinJet())
    std::cout << " Jet id. flags: anti-noise = " << pfCand.passesLooseJetId()
              << std::endl;
  std::cout << std::endl;
}
...
void NoPileUpPFMEtProducer::produce(edm::Event &evt,
  ...
  for (std::vector<edm::EDGetTokenT<edm::View<reco::Candidate>>>::const_iterator
    ...
    for (reco::CandidateView::const_iterator lepton = leptons_i->begin();
      ...
      metSignObjectsLeptons.push_back(
          pfMEtSignInterface_->compResolution(&(*lepton)));
      sumLeptonP4s += lepton->p4();
      ++leptonIdx;
    }
  }
  LogDebug("produce") << " sum(leptons): Pt = " << sumLeptonP4s.pt()
  ...
  reco::PUSubMETCandInfoCollection jets_leptons =
      utils_.cleanJets(*jetsLeptonMatch, leptons, 0.5, true);
  ...
  for (std::vector<CommonMETData>::iterator sumJetsPlusPFCandidates =
           sumJetsPlusPFCandidates_leptons.begin();
  ...
  for (reco::PUSubMETCandInfoCollection::const_iterator jet =
           jets_leptons.begin();
       jet != jets_leptons.end(); ++jet) {
    int leptonIdx_dRmin = findBestMatchingLepton(leptons, jet->p4());
    assert(leptonIdx_dRmin >= 0 &&
           leptonIdx_dRmin < (int)sumJetsPlusPFCandidates_leptons.size());
    ...
    LogDebug("produce") << "jet-to-lepton match:"
                        ...
                        << ", leptonPhi = " << leptons[leptonIdx_dRmin].phi()
                        << std::endl;
  ...
  for (reco::PUSubMETCandInfoCollection::const_iterator pfCandidate =
    ...
    if (!isWithinJet_lepton) {
      ...
      sumJetsPlusPFCandidates_leptons[leptonIdx_dRmin].mex +=
          pfCandidate->p4().px();
      sumJetsPlusPFCandidates_leptons[leptonIdx_dRmin].mey +=
          pfCandidate->p4().py();
      sumJetsPlusPFCandidates_leptons[leptonIdx_dRmin].sumet +=
          pfCandidate->p4().pt();
    } else {
      LogDebug("produce") << " pfCandidate is within jet --> skipping."
                          << std::endl;
    }
  }
  ...
  for (std::vector<CommonMETData>::iterator sumJetsPlusPFCandidates =
    ...
    if (sumJetsPlusPFCandidates->sumet > leptons[leptonIdx].pt()) {
      ...
      sumLeptonIsoCones->mex +=
          (leptonIsoConeEnFrac * sumJetsPlusPFCandidates->mex);
      sumLeptonIsoCones->mey +=
          (leptonIsoConeEnFrac * sumJetsPlusPFCandidates->mey);
      sumLeptonIsoCones->sumet +=
          (leptonIsoConeEnFrac * sumJetsPlusPFCandidates->sumet);
    } else {
  ...
  reco::PUSubMETCandInfoCollection jets_cleaned =
      utils_.cleanJets(*jets, leptons, 0.5, false);
  reco::PUSubMETCandInfoCollection pfCandidates_cleaned =
      utils_.cleanPFCandidates(*pfCandidates, leptons, 0.3, false);
    ...
    if (jet->passesLooseJetId()) {
      if (jet->type() == reco::PUSubMETCandInfo::kHS) {
        ...
        metsig::SigInputObj pfMEtSignObjectOffsetEnCorr(
            jet->metSignObj().get_type(), jet->offsetEnCorr(),
            jet->metSignObj().get_phi(),
            (jet->offsetEnCorr() / jet->p4().E()) *
                jet->metSignObj().get_sigma_e(),
            jet->metSignObj().get_sigma_tan());
        metSignObjectsNoPUjetOffsetEnCorr.push_back(
            pfMEtSignObjectOffsetEnCorr);
      } else {
    ...
    if (pfCandidate->passesLooseJetId()) {
      if (!pfCandidate->isWithinJet()) {
        if (pfCandidate->type() == reco::PUSubMETCandInfo::kChHS) {
          ...
          metSignObjectsNoPUunclChargedCands.push_back(
              pfCandidate->metSignObj());
  ...
  double noPileUpScaleFactor =
      (sumPUunclChargedCands->sumet > 0.)
          ? (sumPUunclChargedCands->sumet /
             (sumNoPUunclChargedCands->sumet + sumPUunclChargedCands->sumet))
          : 1.;
  LogDebug("produce") << "noPileUpScaleFactor = " << noPileUpScaleFactor
                      << std::endl;

  double noPileUpMEtPx =
      -(sumLeptons->mex + sumNoPUjets->mex + sumNoPUunclChargedCands->mex +
        noPileUpScaleFactor *
            (sfNoPUjetOffsetEnCorr_ * sumNoPUjetOffsetEnCorr->mex +
             sfUnclNeutralCands_ * sumUnclNeutralCands->mex +
             sfPUunclChargedCands_ * sumPUunclChargedCands->mex +
             sfPUjets_ * sumPUjets->mex)) +
      noPileUpScaleFactor * sfType0Correction_ * type0Correction_output->mex;
  if (sfLeptonIsoCones_ >= 0.)
    noPileUpMEtPx -=
        (noPileUpScaleFactor * sfLeptonIsoCones_ * sumLeptonIsoCones->mex);
  else
  ...
  double noPileUpMEtPy =
      -(sumLeptons->mey + sumNoPUjets->mey + sumNoPUunclChargedCands->mey +
        noPileUpScaleFactor *
            (sfNoPUjetOffsetEnCorr_ * sumNoPUjetOffsetEnCorr->mey +
             sfUnclNeutralCands_ * sumUnclNeutralCands->mey +
             sfPUunclChargedCands_ * sumPUunclChargedCands->mey +
             sfPUjets_ * sumPUjets->mey)) +
      noPileUpScaleFactor * sfType0Correction_ * type0Correction_output->mey;
  if (sfLeptonIsoCones_ >= 0.)
    noPileUpMEtPy -=
        (noPileUpScaleFactor * sfLeptonIsoCones_ * sumLeptonIsoCones->mey);
  else
    noPileUpMEtPy -= (std::abs(sfLeptonIsoCones_) * sumLeptonIsoCones->mey);
  double noPileUpMEtPt =
      sqrt(noPileUpMEtPx * noPileUpMEtPx + noPileUpMEtPy * noPileUpMEtPy);
  reco::Candidate::LorentzVector noPileUpMEtP4(noPileUpMEtPx, noPileUpMEtPy, 0.,
                                               noPileUpMEtPt);
  ...
  scaleAndAddPFMEtSignObjects(metSignObjects_scaled, metSignObjectsLeptons, 1.0,
                              sfMEtCovMin_, sfMEtCovMax_);
  scaleAndAddPFMEtSignObjects(
      metSignObjects_scaled, metSignObjectsNoPUjetOffsetEnCorr,
      sfNoPUjetOffsetEnCorr_, sfMEtCovMin_, sfMEtCovMax_);
  scaleAndAddPFMEtSignObjects(metSignObjects_scaled, metSignObjectsNoPUjets,
                              sfNoPUjets_, sfMEtCovMin_, sfMEtCovMax_);
  scaleAndAddPFMEtSignObjects(metSignObjects_scaled, metSignObjectsPUjets,
                              noPileUpScaleFactor * sfPUjets_, sfMEtCovMin_,
                              sfMEtCovMax_);
  scaleAndAddPFMEtSignObjects(
      metSignObjects_scaled, metSignObjectsNoPUunclChargedCands,
      sfNoPUunclChargedCands_, sfMEtCovMin_, sfMEtCovMax_);
  scaleAndAddPFMEtSignObjects(
      metSignObjects_scaled, metSignObjectsPUunclChargedCands,
      noPileUpScaleFactor * sfPUunclChargedCands_, sfMEtCovMin_, sfMEtCovMax_);
  scaleAndAddPFMEtSignObjects(
      metSignObjects_scaled, metSignObjectsUnclNeutralCands,
      noPileUpScaleFactor * sfUnclNeutralCands_, sfMEtCovMin_, sfMEtCovMax_);
  reco::METCovMatrix pfMEtCov_recomputed =
      computePFMEtSignificance(metSignObjects_scaled);
  ...
  LogDebug("produce") << "<NoPileUpPFMEtProducer::produce>:" << std::endl
                      ...
                      << " PFMET: Pt = " << pfMEt_original.pt()
                      << ", phi = " << pfMEt_original.phi() << " "
                      << "(Px = " << pfMEt_original.px()
                      << ", Py = " << pfMEt_original.py() << ")" << std::endl
                      << " Cov:" << std::endl
                      << " " << pfMEtCov(0, 0) << "  " << pfMEtCov(0, 1)
                      << "\n " << pfMEtCov(1, 0) << "  " << pfMEtCov(1, 1)
                      << std::endl
                      << " no-PU MET: Pt = " << noPileUpMEt.pt()
                      << ", phi = " << noPileUpMEt.phi() << " "
                      << "(Px = " << noPileUpMEt.px()
                      << ", Py = " << noPileUpMEt.py() << ")" << std::endl
                      << " Cov:" << std::endl
                      << " " << (noPileUpMEt.getSignificanceMatrix())(0, 0)
                      << "  " << (noPileUpMEt.getSignificanceMatrix())(0, 1)
                      << std::endl
                      << (noPileUpMEt.getSignificanceMatrix())(1, 0) << "  "
                      << (noPileUpMEt.getSignificanceMatrix())(1, 1)
                      << std::endl;
