...
public:
  ...
  metsig::SigInputObj compResolution(const T *particle) const {
    ...
    if (dynamic_cast<const reco::GsfElectron *>(particle) != nullptr ||
      ...
      // WARNING: SignAlgoResolutions::PFtype2 needs to be kept in sync with
      // reco::PFCandidate::e !!
      double dpt =
          pfMEtResolution_->eval(metsig::PFtype2, metsig::ET, pt, phi, eta);
      double dphi =
          pfMEtResolution_->eval(metsig::PFtype2, metsig::PHI, pt, phi, eta);
      // std::cout << "electron: pt = " << pt << ", eta = " << eta << ", phi = "
      // << phi
      ...
      // CV: assume resolutions for photons to be identical to electron
      // resolutions
      std::string particleType = "electron";
      // WARNING: SignAlgoResolutions::PFtype2 needs to be kept in sync with
      // reco::PFCandidate::e !!
      ...
      double dphi =
          pfMEtResolution_->eval(metsig::PFtype2, metsig::PHI, pt, phi, eta);
      // std::cout << "photon: pt = " << pt << ", eta = " << eta << ", phi = "
      // << phi
      //          << " --> dpt = " << dpt << ", dphi = " << dphi << std::endl;
      return metsig::SigInputObj(particleType, pt, phi, dpt, dphi);
      ...
      if (muonTrack) {
        ...
        // WARNING: SignAlgoResolutions::PFtype3 needs to be kept in sync with
        // reco::PFCandidate::mu !!
        dpt = pfMEtResolution_->eval(metsig::PFtype3, metsig::ET, pt, phi, eta);
        dphi =
            pfMEtResolution_->eval(metsig::PFtype3, metsig::PHI, pt, phi, eta);
      }
      // std::cout << "muon: pt = " << pt << ", eta = " << eta << ", phi = " <<
      // phi
      //	  << " --> dpt = " << dpt << ", dphi = " << dphi << std::endl;
      return metsig::SigInputObj(particleType, pt, phi, dpt, dphi);
      ...
      if (dynamic_cast<const reco::PFJet *>(particle) != nullptr) {
        ...
        if (jet->isPFJet()) {
          reco::PFJet pfJet(jet->p4(), jet->vertex(), jet->pfSpecific(),
        ...
        assert(0);
      // std::cout << "pfJet: pt = " << pt << ", eta = " << eta << ", phi = " <<
      // phi << std::endl;
      ...
      if (lut_ && pt > 10.) {
        ...
        double y = pt;
        if (x > lut_->GetXaxis()->GetXmin() &&
            x < lut_->GetXaxis()->GetXmax() &&
            y > lut_->GetYaxis()->GetXmin() &&
            y < lut_->GetYaxis()->GetXmax()) {
          int binIndex = lut_->FindBin(x, y);
          double addJERcorrFactor = lut_->GetBinContent(binIndex);
          // std::cout << " addJERcorrFactor = " << addJERcorrFactor <<
          // std::endl;
      ...
      const reco::PFCandidate *pfCandidate =
          dynamic_cast<const reco::PFCandidate *>(particle);
      // std::cout << "pfCandidate: pt = " << pt << ", eta = " << eta << ", phi
      // = " << phi << std::endl;
      return pfMEtResolution_->evalPF(pfCandidate);
    } else
      throw cms::Exception("addPFMEtSignObjects")
          << "Invalid type of particle:"
          << " valid types = { reco::GsfElectron/pat::Electron, "
             "reco::Photon/pat::Photon, reco::Muon/pat::Muon, "
             "reco::PFTau/pat::Tau,"
  ...
  compResolution(const std::list<T *> &particles) const {
