...
PFMEtSignInterfaceBase::PFMEtSignInterfaceBase(const edm::ParameterSet &cfg)
  ...
  if (cfg.exists("addJERcorr")) {
    edm::ParameterSet cfgJERcorr =
        cfg.getParameter<edm::ParameterSet>("addJERcorr");
    edm::FileInPath inputFileName =
        cfgJERcorr.getParameter<edm::FileInPath>("inputFileName");
...
reco::METCovMatrix PFMEtSignInterfaceBase::
operator()(const std::vector<metsig::SigInputObj> &pfMEtSignObjects) const {
  // if ( this->verbosity_ ) {
  //   std::cout << "<PFMEtSignInterfaceBase::operator()>:" << std::endl;
  //   std::cout << " pfMEtSignObjects: entries = " << pfMEtSignObjects.size()
  //   << std::endl; double dpt2Sum = 0.; for (
  //   std::vector<metsig::SigInputObj>::const_iterator pfMEtSignObject =
  //   pfMEtSignObjects.begin();
  // 	  pfMEtSignObject != pfMEtSignObjects.end(); ++pfMEtSignObject ) {
  //     std::cout << pfMEtSignObject->get_type() << ": pt = " <<
  //     pfMEtSignObject->get_energy() << ","
  // 		<< " phi = " << pfMEtSignObject->get_phi() << " --> dpt = " <<
  // pfMEtSignObject->get_sigma_e() << std::endl;
  ...
  if (pfMEtSignObjects.size() >= 2) {
    ...
    //   //     and are sorted such that the one corresponding to the highest
    //   eigenvalue is in the **first** column for ( unsigned iEigenVector = 0;
    //   iEigenVector < 2; ++iEigenVector ) {
    // 	std::cout << "eigenVector #" << iEigenVector << " (eigenValue = " <<
    // eigenValues(iEigenVector) << "):"
    // 		  << " x = " << eigenVectors(0, iEigenVector) << ", y = " <<
    // eigenVectors(1, iEigenVector) << std::endl;
    //   }
    // }
