...
int isVertexAssociated(const reco::PFCandidatePtr &pfCandidate,
  ...
  if (pfCandidate->charge() != 0) {
    ...
    for (PFCandToVertexAssMap::const_iterator pfCandToVertexAssociation =
             pfCandToVertexAssociations.begin();
         pfCandToVertexAssociation != pfCandToVertexAssociations.end();
         ++pfCandToVertexAssociation) {

      const noPuUtils::CandQualityPairVector &pfCandidates_vertex =
          pfCandToVertexAssociation->val;

      for (noPuUtils::CandQualityPairVector::const_iterator pfCandidate_vertex =
               pfCandidates_vertex.begin();
           pfCandidate_vertex != pfCandidates_vertex.end();
           ++pfCandidate_vertex) {

        const reco::PFCandidatePtr pfcVtx =
            edm::refToPtr(pfCandidate_vertex->first); //<reco::PFCandidatePtr>
        // std::cout<<pfCandidate<<"   "<<test<<std::endl;

        if (pfCandidate != pfcVtx)
          continue; // std::cout<<" pouet "<<pfCandidate<<"  "<<test<<std::endl;

        // if(deltaR2(pfCandidate->p4(), pfCandidate_vertex->first->p4()) >
        // dR2Min ) continue;
        double z = pfCandToVertexAssociation->key->position().z();
        int quality = pfCandidate_vertex->second;
        promoteAssocToHSAssoc(quality, z, vertices, dZ, vtxAssociationType,
                              false);
      }
    }
  }
...
noPuUtils::reversedPFCandToVertexAssMap reversePFCandToVertexAssociation(
  ...
  for (PFCandToVertexAssMap::const_iterator pfCandToVertexAssociation =
    ...
    for (noPuUtils::CandQualityPairVector::const_iterator pfCandidate_vertex =
         ...
         pfCandidate_vertex != pfCandidates_vertex.end();
         ++pfCandidate_vertex) {
      revPfCandToVtxAssoc.insert(
          pfCandidate_vertex->first,
          std::make_pair(vertex, pfCandidate_vertex->second));
    }
  }
...
int isVertexAssociated_fast(
    const reco::PFCandidateRef &pfCandidate,
    const noPuUtils::reversedPFCandToVertexAssMap &pfCandToVertexAssociations,
    const reco::VertexCollection &vertices, double dZ, int &numWarnings,
    int maxWarnings) {
  int vtxAssociationType = noPuUtils::kNeutral;
  ...
  if (pfCandidate->charge() != 0) {
    ...
    if (itPfcToVtxAss != pfCandToVertexAssociations.end()) {
      ...
      for (noPuUtils::reversedPFCandToVertexAssMap::const_iterator
               pfcToVtxAssoc = pfCandToVertexAssociations.begin();
    ...
    if (pfCandAssocVtxs != nullptr) {
      for (noPuUtils::VertexQualityPairVector::const_iterator pfcAssVtx =
               pfCandAssocVtxs->begin();
...
void promoteAssocToHSAssoc(int quality, double z,
                           const reco::VertexCollection &vertices, double dZ,
                           int &vtxAssociationType, bool checkdR2) {
