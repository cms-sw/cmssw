...
MvaMEtUtilities::MvaMEtUtilities(const edm::ParameterSet &cfg) {
  ...
  edm::ParameterSet jetConfig =
  iConfig.getParameter<edm::ParameterSet>("JetIdParams"); for(int i0 = 0; i0 <
  3; i0++) { std::string lCutType                            = "Tight"; if(i0 ==
  PileupJetIdentifier::kMedium) lCutType = "Medium"; if(i0 ==
  PileupJetIdentifier::kLoose)  lCutType = "Loose"; std::vector<double> pt010  =
  jetConfig.getParameter<std::vector<double> >(("Pt010_" +lCutType).c_str());
    std::vector<double> pt1020 = jetConfig.getParameter<std::vector<double>
  >(("Pt1020_"+lCutType).c_str()); std::vector<double> pt2030 =
  jetConfig.getParameter<std::vector<double> >(("Pt2030_"+lCutType).c_str());
    std::vector<double> pt3050 = jetConfig.getParameter<std::vector<double>
  >(("Pt3050_"+lCutType).c_str()); for(int i1 = 0; i1 < 4; i1++)
  mvacut_[i0][0][i1] = pt010 [i1]; for(int i1 = 0; i1 < 4; i1++)
  mvacut_[i0][1][i1] = pt1020[i1]; for(int i1 = 0; i1 < 4; i1++)
  mvacut_[i0][2][i1] = pt2030[i1]; for(int i1 = 0; i1 < 4; i1++)
  mvacut_[i0][3][i1] = pt3050[i1];
    }
  */
  // Tight Id => not used
  mvaCut_[0][0][0] = 0.5;
  ...
  ptThreshold_ = (cfg.exists("ptThreshold"))
                     ? cfg.getParameter<int>("ptThreshold")
                     : -1000;
}
...
MvaMEtUtilities::leadJetP4(const std::vector<reco::PUSubMETCandInfo> &jets) {
  return jetP4(jets, 0);
}

reco::Candidate::LorentzVector
MvaMEtUtilities::subleadJetP4(const std::vector<reco::PUSubMETCandInfo> &jets) {
  return jetP4(jets, 1);
}

reco::Candidate::LorentzVector
MvaMEtUtilities::jetP4(const std::vector<reco::PUSubMETCandInfo> &jets,
                       unsigned idx) {
...
unsigned MvaMEtUtilities::numJetsAboveThreshold(
    const std::vector<reco::PUSubMETCandInfo> &jets, double ptThreshold) {
  unsigned retVal = 0;
...
MvaMEtUtilities::cleanJets(const std::vector<reco::PUSubMETCandInfo> &jets,
  ...
  for (std::vector<reco::PUSubMETCandInfo>::const_iterator jet = jets.begin();
    ...
    for (std::vector<reco::PUSubMETCandInfo>::const_iterator lepton =
             leptons.begin();
...
std::vector<reco::PUSubMETCandInfo> MvaMEtUtilities::cleanPFCands(
    ...
    const std::vector<reco::PUSubMETCandInfo> &leptons, double dRmatch,
    bool invert) {

  double dR2match = dRmatch * dRmatch;
  std::vector<reco::PUSubMETCandInfo> retVal;
  for (std::vector<reco::PUSubMETCandInfo>::const_iterator pfCandidate =
           pfCandidates.begin();
...
CommonMETData MvaMEtUtilities::computeCandSum(
    int compKey, double dZmax, int dZflag, bool iCharged, bool mvaPassFlag,
  ...
  for (std::vector<reco::PUSubMETCandInfo>::const_iterator object =
           objects.begin();
...
CommonMETData MvaMEtUtilities::computeRecoil(int metType) {
  ...
  if (metType == MvaMEtUtilities::kHS) {
    ...
    // MET (3) in JME-13-003
    retVal.mex =
        leptonsChSum_.mex - (pfCandChHSSum_.mex + neutralJetHSSum_.mex);
    retVal.mey =
        leptonsChSum_.mey - (pfCandChHSSum_.mey + neutralJetHSSum_.mey);
    retVal.sumet =
        pfCandChHSSum_.sumet + neutralJetHSSum_.sumet - leptonsChSum_.sumet;
  }
  if (metType == MvaMEtUtilities::kPU) {
    // MET = - charged PU - neutral PU in jets
    // MET = -recoil in that particular case, - sign not useful for the MVA and
    // then discarded motivated as PU IS its own recoil
    // MET (4) in JME-13-003
  ...
  if (metType == MvaMEtUtilities::kHSMinusNeutralPU) {
    ...
    // MET (5) in JME-13-003
    retVal.mex = leptonsSum_.mex -
                 (pfCandSum_.mex - pfCandChPUSum_.mex - neutralJetPUSum_.mex);
    retVal.mey = leptonsSum_.mey -
                 (pfCandSum_.mey - pfCandChPUSum_.mey - neutralJetPUSum_.mey);
    retVal.sumet =
        (pfCandSum_.sumet - pfCandChPUSum_.sumet - neutralJetPUSum_.sumet) -
        leptonsSum_.sumet;
  }
...
void MvaMEtUtilities::computeAllSums(
  ...
  pfCandChHSSum_ =
      computeCandSum(kPFCands, dzCut_, 0, false, false, pfCandidates);
  pfCandChPUSum_ =
      computeCandSum(kPFCands, dzCut_, 1, false, false, pfCandidates);
