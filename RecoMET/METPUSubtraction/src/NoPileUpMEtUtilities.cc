...
reco::PUSubMETCandInfoCollection NoPileUpMEtUtilities::cleanJets(
  ...
  for (reco::PUSubMETCandInfoCollection::const_iterator jet = jets.begin();
    ...
    for (std::vector<reco::Candidate::LorentzVector>::const_iterator lepton =
             leptons.begin();
...
CommonMETData NoPileUpMEtUtilities::computeCandidateSum(
    const reco::PUSubMETCandInfoCollection &cands, bool neutralFracOnly,
    double &sumAbsPx, double &sumAbsPy) {
...
reco::PUSubMETCandInfoCollection NoPileUpMEtUtilities::cleanPFCandidates(
  ...
  reco::PUSubMETCandInfoCollection retVal;
  for (reco::PUSubMETCandInfoCollection::const_iterator pfCandidate =
           pfCandidates.begin();
...
reco::PUSubMETCandInfoCollection NoPileUpMEtUtilities::selectCandidates(
    const reco::PUSubMETCandInfoCollection &cands, double minPt, double maxPt,
    int type, bool isCharged, int isWithinJet) {
  reco::PUSubMETCandInfoCollection retVal;
  for (reco::PUSubMETCandInfoCollection::const_iterator cand = cands.begin();
       cand != cands.end(); ++cand) {
    ...
    // for pf candidates
    if (isWithinJet != NoPileUpMEtUtilities::kAll &&
        (cand->isWithinJet() != isWithinJet))
      continue;
...
void NoPileUpMEtUtilities::computeAllSums(
    const reco::PUSubMETCandInfoCollection &jets,
    const reco::PUSubMETCandInfoCollection &pfCandidates) {

  reco::PUSubMETCandInfoCollection pfcsCh = selectCandidates(
      pfCandidates, minPtDef_, maxPtDef_, reco::PUSubMETCandInfo::kUndefined,
      false, NoPileUpMEtUtilities::kAll);

  reco::PUSubMETCandInfoCollection pfcsChHS = selectCandidates(
      pfCandidates, minPtDef_, maxPtDef_, reco::PUSubMETCandInfo::kChHS, true,
      NoPileUpMEtUtilities::kAll);

  reco::PUSubMETCandInfoCollection pfcsChPU = selectCandidates(
      pfCandidates, minPtDef_, maxPtDef_, reco::PUSubMETCandInfo::kChPU, true,
      NoPileUpMEtUtilities::kAll);

  reco::PUSubMETCandInfoCollection pfcsNUnclustered = selectCandidates(
      pfCandidates, minPtDef_, maxPtDef_, reco::PUSubMETCandInfo::kNeutral,
      false, NoPileUpMEtUtilities::kOutsideJet);

  reco::PUSubMETCandInfoCollection jetsHS =
      selectCandidates(jets, 10.0, maxPtDef_, reco::PUSubMETCandInfo::kHS,
                       false, NoPileUpMEtUtilities::kAll);
  reco::PUSubMETCandInfoCollection jetsPU =
      selectCandidates(jets, 10.0, maxPtDef_, reco::PUSubMETCandInfo::kPU,
                       false, NoPileUpMEtUtilities::kAll);

  // not used so far
  //_chPfcSum = computeCandidateSum(pfcsCh, false, &_chPfcSumAbsPx,
  //&_chPfcSumAbsPy);
  chHSPfcSum_ =
      computeCandidateSum(pfcsChHS, false, chHSPfcSumAbsPx_, chHSPfcSumAbsPy_);
  chPUPfcSum_ =
      computeCandidateSum(pfcsChPU, false, chPUPfcSumAbsPx_, chPUPfcSumAbsPy_);
  nUncPfcSum_ = computeCandidateSum(pfcsNUnclustered, false, nUncPfcSumAbsPx_,
                                    nUncPfcSumAbsPy_);

  nHSJetSum_ =
      computeCandidateSum(jetsHS, true, nHSJetSumAbsPx_, nHSJetSumAbsPy_);
  nPUJetSum_ =
      computeCandidateSum(jetsPU, true, nPUJetSumAbsPx_, nPUJetSumAbsPy_);
}

CommonMETData NoPileUpMEtUtilities::computeRecoil(int metType, double &sumAbsPx,
                                                  double &sumAbsPy) {
  CommonMETData retVal;
  ...
  //   CommonMETData chPfcSum = computeCandidateSum(_pfcsCh, false,
  //   &trackSumAbsPx, &trackSumAbsPy); retVal.mex   = -chPfcSum.mex; retVal.mey
  //   = -chPfcSum.mey; retVal.sumet = chPfcSum.sumet; retSumAbsPx = ;
  //   retSumAbsPy = ;
  // }
