...
const GBRForest *
PFMETAlgorithmMVA::loadMVAfromFile(const edm::FileInPath &inputFileName,
  ...
  std::vector<std::string> *lVec =
      (std::vector<std::string> *)inputFile->Get("varlist");

  if (lVec == nullptr) {
    throw cms::Exception("PFMETAlgorithmMVA::loadMVA")
        << " Failed to load mva file : " << inputFileName.fullPath().data()
  ...
  if (mvaName.find(mvaNameU_) != std::string::npos)
    ...
    throw cms::Exception("PFMETAlgorithmMVA::loadMVA")
        << "MVA MET weight file tree names do not match specified inputs"
        << std::endl;
...
PFMETAlgorithmMVA::PFMETAlgorithmMVA(const edm::ParameterSet &cfg)
    : utils_(cfg), mvaReaderU_(nullptr), mvaReaderDPhi_(nullptr),
      mvaReaderCovU1_(nullptr), mvaReaderCovU2_(nullptr), cfg_(cfg) {
  mvaType_ = kBaseline;
...
void PFMETAlgorithmMVA::initialize(const edm::EventSetup &es) {
  edm::ParameterSet cfgInputRecords =
      cfg_.getParameter<edm::ParameterSet>("inputRecords");
  ...
  if (loadMVAfromDB_) {
    ...
    edm::ParameterSet cfgInputFileNames =
        cfg_.getParameter<edm::ParameterSet>("inputFileNames");

    edm::FileInPath inputFileNameU =
        cfgInputFileNames.getParameter<edm::FileInPath>("U");
    mvaReaderU_ = loadMVAfromFile(inputFileNameU, mvaNameU_);
    edm::FileInPath inputFileNameDPhi =
        cfgInputFileNames.getParameter<edm::FileInPath>("DPhi");
    mvaReaderDPhi_ = loadMVAfromFile(inputFileNameDPhi, mvaNameDPhi_);
    edm::FileInPath inputFileNameCovU1 =
        cfgInputFileNames.getParameter<edm::FileInPath>("CovU1");
    mvaReaderCovU1_ = loadMVAfromFile(inputFileNameCovU1, mvaNameCovU1_);
    edm::FileInPath inputFileNameCovU2 =
        cfgInputFileNames.getParameter<edm::FileInPath>("CovU2");
    mvaReaderCovU2_ = loadMVAfromFile(inputFileNameCovU2, mvaNameCovU2_);
  }
}

//-------------------------------------------------------------------------------
void PFMETAlgorithmMVA::setInput(
  ...
  const std::vector<reco::PUSubMETCandInfo> jets_cleaned =
      utils_.getCleanedJets();
  ...
  CommonMETData hsMinusNeutralPUMEt_data =
      utils_.computeRecoil(MvaMEtUtilities::kHSMinusNeutralPU);
  ...
  var_["pileUpCorrected_SumET"] =
      hsMinusNeutralPUMEt_data.sumet / var_["particleFlow_SumET"];
...
PFMETAlgorithmMVA::createFloatVector(std::vector<std::string> variableNames) {
...
void PFMETAlgorithmMVA::evaluateMVA() {
  ...
  mvaOutputCovU1_ = std::pow(GetResponse(mvaReaderCovU1_, varForCovU1_) *
                                 mvaOutputU_ * var_["particleFlow_U"],
                             2);
  mvaOutputCovU2_ = std::pow(GetResponse(mvaReaderCovU2_, varForCovU2_) *
                                 mvaOutputU_ * var_["particleFlow_U"],
                             2);
  ...
  if (hasPhotons_) {
    ...
    double sumLeptonPt = std::max(
        sqrt(sumLeptonPx_ * sumLeptonPx_ + sumLeptonPy_ * sumLeptonPy_), 1.);
    if (var_["track_U"] / sumLeptonPt < 0.1 ||
        var_["noPileUp_U"] / sumLeptonPt < 0.1) {
      mvaOutputU_ = 1.;
      mvaOutputDPhi_ = 0.;
    }
  }
...
void PFMETAlgorithmMVA::computeMET() {
  ...
  // compute MET uncertainties in dirrections parallel and perpendicular to
  // hadronic recoil (neglecting uncertainties on lepton momenta)
  mvaMEtCov_(0, 0) =
      mvaOutputCovU1_ * cosPhi * cosPhi + mvaOutputCovU2_ * sinPhi * sinPhi;
  mvaMEtCov_(0, 1) =
      -mvaOutputCovU1_ * sinPhi * cosPhi + mvaOutputCovU2_ * sinPhi * cosPhi;
  mvaMEtCov_(1, 0) = mvaMEtCov_(0, 1);
  mvaMEtCov_(1, 1) =
      mvaOutputCovU1_ * sinPhi * sinPhi + mvaOutputCovU2_ * cosPhi * cosPhi;
}

//-------------------------------------------------------------------------------
const float
PFMETAlgorithmMVA::GetResponse(const GBRForest *Reader,
                               std::vector<std::string> &variableNames) {
...
void PFMETAlgorithmMVA::print(std::ostream &stream) const {
  ...
  stream << " covU1 = " << mvaOutputCovU1_ << ", covU2 = " << mvaOutputCovU2_
         << std::endl;
  stream << " sum(leptons): Pt = "
         << sqrt(sumLeptonPx_ * sumLeptonPx_ + sumLeptonPy_ * sumLeptonPy_)
         << ","
         << " phi = " << atan2(sumLeptonPy_, sumLeptonPx_) << " "
         << "(Px = " << sumLeptonPx_ << ", Py = " << sumLeptonPy_ << ")";
  stream << " MVA output: U = " << mvaOutputU_ << ", dPhi = " << mvaOutputDPhi_
         << ","
         << " covU1 = " << mvaOutputCovU1_ << ", covU2 = " << mvaOutputCovU2_
         << std::endl;
  stream << std::endl;
}
