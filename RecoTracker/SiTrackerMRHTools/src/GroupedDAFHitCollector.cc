							      ...
							      const MeasurementTrackerEvent *theMTE) const
{
  ...
  if (meas.empty()) //return TransientTrackingRecHit::ConstRecHitContainer();	
	...
	LogDebug("MultiRecHitCollector") << "Layer "  << mol.back().first  << " has " << mol.back().second.size() << " measurements";
        ...
        for(auto & iLay : mol){
          LogTrace("MultiRecHitCollector") << "  Layer "  << iLay.first  << " has " << iLay.second.size() << " measurements:";
          vector<TrajectoryMeasurement>::const_iterator ibeg = iLay.second.begin();
          vector<TrajectoryMeasurement>::const_iterator iend = iLay.second.end();
...
void GroupedDAFHitCollector::buildMultiRecHits(const vector<TrajectoryMeasurementGroup>& measgroup, vector<TrajectoryMeasurement>& result, const MeasurementTrackerEvent*& theMTE) const {
  ...
  for (const auto & igroup : measgroup){
    ...
    for (auto & hit : hits){  
      ...
      if (hit->getType() == TrackingRecHit::missing ) validity = "missing !should not happen!";
      else if (hit->getType() == TrackingRecHit::inactive) validity = "inactive";
      else if (hit->getType() == TrackingRecHit::bad) validity = "bad";   
      LogTrace("MultiRecHitCollector") << "DetId " << hit->geographicalId().rawId()  
				       << " validity: " << validity 
				       << " surface position " << getMeasurementTracker()->geomTracker()->idToDet(hit->geographicalId())->position()  
				       << " hit local position " << hit->localPosition();
    }
    //should be fixed!!
