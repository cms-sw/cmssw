...
std::unique_ptr<MultiRecHitCollector> MultiRecHitCollectorESProducer::produce(const MultiRecHitRecord& iRecord) {
  ...
  if (conf_.getParameter<std::string>("Mode") == "Simple")
    mode = "Simple";
  ...
  iRecord.getRecord<CkfComponentsRecord>().getRecord<TrackingComponentsRecord>().get(propagatorAlongName,
                                                                                     propagatorhandle);
  ...
  if (mode == "Grouped") {
    ...
    iRecord.getRecord<CkfComponentsRecord>().getRecord<TrackingComponentsRecord>().get(propagatorOppositeName,
                                                                                       propagatorOppositehandle);
