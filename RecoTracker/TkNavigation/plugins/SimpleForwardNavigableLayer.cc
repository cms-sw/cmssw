...
SimpleForwardNavigableLayer::SimpleForwardNavigableLayer(const ForwardDetLayer* detLayer,
  ...
  for (auto bl = outerBL.begin(); bl != outerBL.end(); bl++)
    theOuterLayers.push_back(*bl);
  for (auto fl = outerFL.begin(); fl != outerFL.end(); fl++)
...
vector<const DetLayer*> SimpleForwardNavigableLayer::compatibleLayers(NavigationDirection dir) const {
...
void SimpleForwardNavigableLayer::setInwardLinks(const BDLC& innerBL, const FDLC& innerFL, TkLayerLess sorter) {
  ...
  for (auto bl = innerBL.begin(); bl != innerBL.end(); bl++)
    theInnerLayers.push_back(*bl);
  for (auto fl = innerFL.begin(); fl != innerFL.end(); fl++)
