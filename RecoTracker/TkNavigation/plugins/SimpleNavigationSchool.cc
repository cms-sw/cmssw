...
void SimpleNavigationSchool::cleanMemory() {
  ...
  for (auto ib = theBarrelNLC.begin(); ib != theBarrelNLC.end(); ib++) {
    delete (*ib);
  }
  theBarrelNLC.clear();
  for (auto ifl = theForwardNLC.begin(); ifl != theForwardNLC.end(); ifl++) {
    delete (*ifl);
  }
  theForwardNLC.clear();
}
...
vector<SimpleNavigationSchool::FDLC> SimpleNavigationSchool::splitForwardLayers() {
  ...
  for (auto ivec = result.begin(); ivec != result.end(); ivec++) {
