...
BeamHaloNavigationSchool::BeamHaloNavigationSchool(const GeometricSearchTracker* theInputTracker,
  ...
  auto layerIt = allLayers.begin();
  auto layerIt_end = allLayers.end();
  for (; layerIt != layerIt_end; ++layerIt) {
    //convert to SimpleNavigableLayer
    auto* snl = dynamic_cast<SimpleNavigableLayer*>(*layerIt);
    if (!snl) {
...
void BeamHaloNavigationSchool::establishInverseRelations() {
  ...
  for (auto bli = theBarrelLayers.begin(); bli != theBarrelLayers.end(); bli++) {
    ...
    for (auto i = reachedLC.begin(); i != reachedLC.end(); i++) {
      reachedBarrelLayersMap[*i].push_back(*bli);
    }
  }

  for (auto fli = theForwardLayers.begin(); fli != theForwardLayers.end(); fli++) {
...
void BeamHaloNavigationSchool::linkOtherEndLayers(SymmetricLayerFinder& symFinder) {
  ...
  //for each of them
  for (auto fl = reachableFL.begin(); fl != reachableFL.end(); fl++) {
...
void BeamHaloNavigationSchool::addInward(const DetLayer* det, const ForwardDetLayer* newF) {
  ...
  auto* navigableLayer = dynamic_cast<SimpleNavigableLayer*>(theAllNavigableLayer[(det)->seqNum()]);
  ...
  BDLC inwardsBarrel;
  FDLC inwardsForward;
  for (auto dli = inwardsLayers.begin(); dli != inwardsLayers.end(); dli++) {
  ...
  auto new_end = unique(inwardsForward.begin(), inwardsForward.end());
