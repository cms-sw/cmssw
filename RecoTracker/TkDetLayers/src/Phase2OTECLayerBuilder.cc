...
ForwardDetLayer* Phase2OTECLayerBuilder::build(const GeometricDet* aPhase2OTECLayer,
                                                 const TrackerGeometry* theGeomDetGeometry) {
  vector<const GeometricDet*>  theGeometricPanels = aPhase2OTECLayer->components();
  ...
  */
  ...
  // done in the Phase2OTECLayer class: if they are opposite, there
  ...
  if ( aPhase2OTECLayer->type()==GeometricDet::PixelEndCapPhase1 )
    return new PixelForwardLayerPhase1(theBlades);
  return new Phase2OTECLayer(theBlades);
}
