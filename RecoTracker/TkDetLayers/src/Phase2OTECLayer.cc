...
Phase2OTECLayer::Phase2OTECLayer(vector<const PixelBlade*>& blades):
    theComps(blades.begin(),blades.end())
{
  ...
  //--------- DEBUG INFO --------------
  LogDebug("TkDetLayers") << "DEBUG INFO for Phase2OTECLayer" << "\n"
			  ...
			  << "Phase2OTECLayer.surfcace.phi(): "  << std::endl
			  << this->surface().position().phi() << "\n"
			  << "Phase2OTECLayer.surfcace.z(): "
			  << this->surface().position().z() << "\n"
			  << "Phase2OTECLayer.surfcace.innerR(): "
			  << this->specificSurface().innerRadius() << "\n"
			  << "Phase2OTECLayer.surfcace.outerR(): "
...
Phase2OTECLayer::~Phase2OTECLayer(){
...
void
Phase2OTECLayer::groupedCompatibleDetsV( const TrajectoryStateOnSurface& tsos,
  ...
  //    edm::LogInfo("TkDetLayers") << "inner computeCrossings returns invalid in Phase2OTECLayer::groupedCompatibleDets:";
  //    return;
  //  }
  if (!crossings_outer.isValid){
    edm::LogInfo("TkDetLayers") << "outer computeCrossings returns invalid in Phase2OTECLayer::groupedCompatibleDets:";
    return;
  }
...
void
Phase2OTECLayer::searchNeighbors( const TrajectoryStateOnSurface& tsos,
				    const Propagator& prop,
				    const MeasurementEstimator& est,
...
int
Phase2OTECLayer::computeHelicity(const GeometricSearchDet* firstBlade,const GeometricSearchDet* secondBlade) const
{
  if( fabs(firstBlade->position().z()) < fabs(secondBlade->position().z()) ) return 0;
  return 1;
}

Phase2OTECLayer::SubTurbineCrossings
Phase2OTECLayer::computeCrossings( const TrajectoryStateOnSurface& startingState,
				     PropagationDirection propDir, bool innerDisk) const
{
  ...
  if (turbinePoint.perp() < 20) {
    ...
    // middle one, and so on (see Phase2OTECLayerBuilder for further
...
float
Phase2OTECLayer::computeWindowSize( const GeomDet* det,
