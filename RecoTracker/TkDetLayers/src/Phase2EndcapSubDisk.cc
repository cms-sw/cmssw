...
void Phase2EndcapSubDisk::groupedCompatibleDetsV(const TrajectoryStateOnSurface& startingState,
  ...
  if (theRingSize > 1) {
    if (std::abs(theComps[0]->position().z()) < std::abs(theComps[1]->position().z())) {
      ...
      throw DetLayerException("Rings in Endcap SubDisk have same z position, no idea how to order them!");
    }
  }
...
std::array<int, 3> Phase2EndcapSubDisk::findThreeClosest(std::vector<tkDetUtil::RingPar> ringParams,
