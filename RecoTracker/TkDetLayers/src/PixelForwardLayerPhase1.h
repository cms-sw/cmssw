...
private:
  ...
  struct SubTurbineCrossings {
    ...
    SubTurbineCrossings(int ci, int ni, float nd) : isValid(true), closestIndex(ci), nextIndex(ni), nextDistance(nd) {}
  ...
  SubTurbineCrossings computeCrossings(const TrajectoryStateOnSurface& startingState,
