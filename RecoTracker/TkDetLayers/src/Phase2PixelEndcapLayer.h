#ifndef TkDetLayers_Phase2PixelEndcapLayer_h
#define TkDetLayers_Phase2PixelEndcapLayer_h
...
class Phase2PixelEndcapLayer final : public ForwardDetLayer {
 public:
  Phase2PixelEndcapLayer(std::vector<const Phase2OTEndcapRing*>& rings)  __attribute__ ((cold));
  ~Phase2PixelEndcapLayer()  __attribute__ ((cold));

  // Default implementations would not properly manage memory
  Phase2PixelEndcapLayer( const Phase2PixelEndcapLayer& ) = delete;
  Phase2PixelEndcapLayer& operator=( const Phase2PixelEndcapLayer&) = delete;
