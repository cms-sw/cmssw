...
// 	$Id: DcxCloudsToTracks.cc,v 1.18 2004/08/06 05:58:21 bartoldu Exp $
//
// Description:
...
DcxCloudsToTracks::DcxCloudsToTracks(){
 cout << "DcxCloudsToTracks null constructor - does nothing" << endl;}

//points
DcxCloudsToTracks::DcxCloudsToTracks(std::vector<DcxHit*> &listohits)
{ 
  cout << "listohits.size() = " << listohits.size() << " in DcxCloudsToTracks" << endl;
...
//        cout << "axial strip radius = " << rhit << endl;
    }
    for (unsigned int j=0; j<listohits.size(); ++j) {
      for (unsigned int k=0; k<listohits.size(); ++k) {
//        cout << "layers " << listohits[i]->Layer() << " " << listohits[j]->Layer() << " " 
//             << listohits[k]->Layer() << endl;
...
//           cout << "Pivot lwlwlw " << l1ptr->Layer() << " " << l1ptr->WireNo()
//                << " " << l2ptr->Layer() << " " << l2ptr->WireNo()
//                << " " << l3ptr->Layer() << " " << l3ptr->WireNo() << endl;
//           cout << "trial circ " << xc << " " << yc << " " << rc 
//                << " " << d0 << " " << dmax << " " << s3 << endl;
           double d0h=-s3*d0;
...
//           cout << "listohits.size(), axlist.size() " << listohits.size() << " " << axlist.size() << endl;
...
//                 cout << "listohits.size(), outlist.size() " << listohits.size() << " " << outlist.size() << endl;
                 if ((n_stereo>2)&&(n_axial>6)){
                   ...
                   if (real_fit.Prob()>0.001){
                     ntrk++;
                     cout << "ntrk xprob pt nax nst d0 phi0 omega z0 tanl " << ntrk << " " << real_fit.Prob() 
                          << " " << real_fit.Pt() << " " << n_axial << " " << n_stereo
                          << " " << real_fit.D0() << " " << real_fit.Phi0() << " " << real_fit.Omega() 
                          << " " << real_fit.Z0() << " " << real_fit.Tanl() << endl;
//                     real_fit.print();
                     for (unsigned int n=0; n<outlist.size(); ++n){outlist[n]->SetUsedOnHel(ntrk);}
                   }else{
//                     cout << " ntrk xprob pt nax nst = " << ntrk << " " << real_fit.Prob() 
//                          << " " << real_fit.Pt() << " " << n_axial << " " << n_stereo ; 
//                     real_fit.print();
                   }
                 }
...
//        cout << "In check_axial, doca(mmm) len xh yh " << 10000.0*doca << " " << len 
//        << " " << make_a_hel.Xh(len) << " " << make_a_hel.Yh() << endl;
...
void DcxCloudsToTracks::check_stereo( std::vector<DcxHit*> &listohits, std::vector<DcxHit*> &outlist, DcxHel make_a_hel){
  for (unsigned int i=0; i<listohits.size(); ++i) {
    ...
    if ((try_me->stereo())&&(!try_me->GetUsedOnHel())){
        double doca = try_me->residual(make_a_hel);
//        double len = make_a_hel.Doca_Len();
//        cout << "In check_stereo, doca(mmm) len xh yh " << 10000.0*doca << " " << len 
//        << " " << make_a_hel.Xh(len) << " " << make_a_hel.Yh(len) << endl;
       if (fabs(doca)<0.100)outlist.push_back(try_me);
    } 
  }
}
