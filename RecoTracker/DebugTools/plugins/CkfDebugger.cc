						...
						const TransientTrackingRecHitBuilder* aTTRHBuilder)
{
  ...
  for(const auto & mea : meas){
    if (mea.recHit()->isValid()) LogTrace("CkfDebugger") << "valid hit at position:" << mea.recHit()->globalPosition() ;
  }
  ...
  for (const auto & mea : meas) {
    ...
    if (mea.recHit()->isValid()) {
      edm::LogVerbatim("CkfDebugger") << "RecHit at " << mea.recHit()->globalPosition()
				      << " layer " <<   ((mea.recHit()->det()->geographicalId().rawId() >>16) & 0xF)
				      << " subdet " << mea.recHit()->det()->geographicalId().subdetId() 
				      << " Chi2 " << mea.estimate() ;
    }
    else if (mea.recHit()->det() == 0) {
...
bool CkfDebugger::correctTrajectory( const Trajectory& traj,unsigned int& trajId) const
{
  ...
  for (Trajectory::RecHitContainer::const_iterator rh=hits.begin(); rh!=hits.end(); ++rh) {
    ...
    for (auto & assSimHit : assSimHits){
      if (goodSimHit(assSimHit)) nogoodhit=false;
    }
