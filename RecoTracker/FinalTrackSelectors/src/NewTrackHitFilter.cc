...
class NewTrackHitFilter : public edm::EDProducer {
    public:
       NewTrackHitFilter(const edm::ParameterSet &iConfig) ; 
...
NewTrackHitFilter::Rule::Rule(const std::string &str) {
...
int NewTrackHitFilter::Rule::layer(DetId detid) const {
...
NewTrackHitFilter::NewTrackHitFilter(const edm::ParameterSet &iConfig) :
...
void
NewTrackHitFilter::produce(edm::Event &iEvent, const edm::EventSetup &iSetup) 
{
...
TrackCandidate
NewTrackHitFilter::makeCandidate(const reco::Track &tk, std::vector<TrackingRecHit *>::iterator hitsBegin, std::vector<TrackingRecHit *>::iterator hitsEnd) {
...
using reco::modules::NewTrackHitFilter;
DEFINE_FWK_MODULE(NewTrackHitFilter);
