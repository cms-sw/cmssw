...
// one per event, fills it in its extractor kernel and scores it in the same produce(). The column
// order is the positional input of the models (a model reads the first N columns it was trained
// on), so columns are only ever appended. Columns 0-16 are written for every preselected track;
// caFitChi2 onwards only when useHitFeatures is true.
