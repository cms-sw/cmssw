...
VertexBeamspotOrigins::VertexBeamspotOrigins(const edm::ParameterSet& regPSet, edm::ConsumesCollector& iC) {
  ...
  else  throw cms::Exception("Configuration") <<"Unknown operation mode string: "<<operationModeString;
  ...
  if (m_operationMode == OperationMode::VERTICES_FIXED)  m_zErrorVertex     = regPSet.getParameter<double>("zErrorVertex");
  m_nSigmaZBeamSpot = -1.;
...
void VertexBeamspotOrigins::fillDescriptions(edm::ParameterSetDescription& desc, const std::string& defaultBeamSpot,
                                             const std::string& defaultVertex, int defaultMaxVertices) {
