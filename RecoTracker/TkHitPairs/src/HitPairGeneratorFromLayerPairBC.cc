...
HitPairGeneratorFromLayerPairBC::HitPairGeneratorFromLayerPairBC(const LayerWithHits* inner, 
...
/*
  ...
  if (debug) {
    cout << "** HitPairGeneratorFromLayerPairBC ** ";
...
void HitPairGeneratorFromLayerPairBC::
		       ...
		       const edm::EventSetup& iSetup)
{
  ...
  for (HI oh=outerHits.begin(); oh!= outerHits.end(); oh++) {
    ...
    for (HI ih = innerCandid.begin(); ih != innerCandid.end(); ih++) {
      ...
      if (! crossRange.empty() ) {
        result.push_back( OrderedHitPair(ctfseeding::SeedingHit(*ih,iSetup), ctfseeding::SeedingHit(*oh,iSetup) ) );
      }
    } 
    delete checkRZ;
  }
