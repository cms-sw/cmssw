...
SeedForPhotonConversionFromQuadruplets::SeedForPhotonConversionFromQuadruplets(
    const edm::ParameterSet& cfg, edm::ConsumesCollector& iC, const edm::ParameterSet& SeedComparitorPSet)
...
*/
  ...
  double maxIPrho2 = IPrho + CleaningmaxRadialDistance;
  maxIPrho2 *= maxIPrho2;
  ...
  std::cout << " >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
            << "\n";
  std::cout << " >>>>>>>>>>> Conv Cand: "
            << " Vertex X: " << candVtx.X() << " [cm] Y: " << candVtx.Y() << " [cm] pt+: " << candPtPlus
            << " [GeV] pt-: " << candPtMinus << " [GeV]; #its: " << nite << "\n";
#endif
  ...
  if (candPtPlus < minLegPt)
    return nullptr;
  if (candPtMinus < minLegPt)
    return nullptr;
  //
  if (candPtPlus > maxLegPt)
    return nullptr;
  if (candPtMinus > maxLegPt)
    return nullptr;
  //
  // Cut on radial distance between estimated conversion vertex and inner hits
  double cr = std::sqrt(candVtx.Perp2());
  double maxr2 = (maxRadialDistance + cr);
  maxr2 *= maxr2;
  if (h2.Perp2() > maxr2)
    return nullptr;
  if (h3.Perp2() > maxr2)
  ...
  if (ClusterShapeFiltering) {
    ...
    if (theComparitor && !theComparitor->compatible(mhits, mkine, mhelix)) {
      return nullptr;
    }
  }
  ...
  if (similarQuadExist(thisQuad, quadV) && applyArbitration)
    return nullptr;
  ...
  if (vError > 15.)
    vError = 1.;
  std::cout << "QuadDispLine " << vgPhotVertex.x() << " " << vgPhotVertex.y() << " " << vgPhotVertex.z() << " "
            << vError << " " << vHit[0].x() << " " << vHit[0].y() << " " << vHit[0].z() << " "
            << sqrt(getSqrEffectiveErrorOnZ(ptth2, region)) << " " << vHit[1].x() << " " << vHit[1].y() << " "
            << vHit[1].z() << " " << sqrt(getSqrEffectiveErrorOnZ(ptth1, region)) << " " << vHit[2].x() << " "
            << vHit[2].y() << " " << vHit[2].z() << " " << sqrt(getSqrEffectiveErrorOnZ(mtth1, region)) << " "
            << vHit[3].x() << " " << vHit[3].y() << " " << vHit[3].z() << " "
            << sqrt(getSqrEffectiveErrorOnZ(mtth2, region)) << " " << candVtx.X() << " " << candVtx.Y() << " "
            << candVtx.Z() << " " << fittedPrimaryVertex.X() << " " << fittedPrimaryVertex.Y() << " "
  ...
  std::cout << " \t " << detid << " " << ptth2->localPosition() << " " << ptth2->globalPosition() << "\nhelix momentum "
            << kinePlus.momentum() << " pt " << kinePlus.momentum().perp() << " radius "
            << 1 / kinePlus.transverseCurvature() << " q " << kinePlus.charge();
  std::cout << " \n\t mtth1 ";
  ...
  std::cout << " \t " << detid << " " << mtth2->localPosition() << " " << mtth2->globalPosition() << "\nhelix momentum "
            << kineMinus.momentum() << " pt " << kineMinus.momentum().perp() << " radius "
            << 1 / kineMinus.transverseCurvature() << " q " << kineMinus.charge();
  std::cout << "\n <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<"
            << "\n";
#endif
...
bool SeedForPhotonConversionFromQuadruplets::buildSeedBool(TrajectorySeedCollection& seedCollection,
  ...
  if (apply_dzCut) {
    ...
    math::XYZVector EstMomGam(
        updatedState.globalMomentum().x(), updatedState.globalMomentum().y(), updatedState.globalMomentum().z());
    math::XYZVector EstPosGam(
        updatedState.globalPosition().x(), updatedState.globalPosition().y(), updatedState.globalPosition().z());

    double EstMomGamLength =
        std::sqrt(EstMomGam.x() * EstMomGam.x() + EstMomGam.y() * EstMomGam.y() + EstMomGam.z() * EstMomGam.z());
    math::XYZVector EstMomGamNorm(
        EstMomGam.x() / EstMomGamLength, EstMomGam.y() / EstMomGamLength, EstMomGam.z() / EstMomGamLength);

    //Calculate dz of point of closest approach of the two lines (WA approach) -> cut on dz

    const GlobalPoint EstPosGamGlobalPoint(
        updatedState.globalPosition().x(), updatedState.globalPosition().y(), updatedState.globalPosition().z());
    ...
    const GlobalTrajectoryParameters myGlobalTrajectoryParameter(
        EstPosGamGlobalPoint, EstMomGamGlobalVector, qCharge, magField);
...
SeedingHitSet::RecHitPointer SeedForPhotonConversionFromQuadruplets::refitHit(
    SeedingHitSet::ConstRecHitPointer hit, const TrajectoryStateOnSurface& state) const {
...
void SeedForPhotonConversionFromQuadruplets::stupidPrint(const char* s, GlobalPoint* d) {
  (*pss) << "\n" << s << "\t";
  for (size_t i = 0; i < 2; ++i)
    (*pss) << std::setw(20) << d[i] << " r " << d[i].perp() << " phi " << d[i].phi() << " | ";
}

void SeedForPhotonConversionFromQuadruplets::stupidPrint(const char* s, GlobalPoint* d, int n) {
  (*pss) << "\n" << s << "\n";
  for (int i = 0; i < n; ++i)
    (*pss) << std::setw(20) << d[i] << " r " << d[i].perp() << " phi " << d[i].phi() << "\n";
}
...
bool SeedForPhotonConversionFromQuadruplets::similarQuadExist(Quad& thisQuad, std::vector<Quad>& quadV) {
  for (auto const& quad : quadV) {
    ...
    //  return true;
    //  }
    if (sqrt(dx * dx + dy * dy) < 1. && fabs(thisQuad.ptPlus - quad.ptPlus) < 0.5 * quad.ptPlus &&
