...
bool HitPairGeneratorFromLayerPairForPhotonConversion::checkBoundaries(const DetLayer& layer,const ConversionRegion& convRegion,float maxSearchR, float maxSearchZ){
...
bool HitPairGeneratorFromLayerPairForPhotonConversion::getPhiRange(float& Phimin, float& Phimax, const DetLayer& layer, const ConversionRegion &convRegion, const edm::EventSetup& es){
   if(layer.location() == GeomDetEnumerators::barrel){
...
bool HitPairGeneratorFromLayerPairForPhotonConversion::getPhiRange(float& Phimin, float& Phimax, const float& layerR, const ConversionRegion &convRegion, const edm::EventSetup& es){
...
bool HitPairGeneratorFromLayerPairForPhotonConversion::
checkRZCompatibilityWithSeedTrack(const RecHitsSortedInPhi::Hit& hit,const DetLayer& layer,const ConversionRegion& convRegion){
  static const float nSigmaRZ = std::sqrt(12.f);
  Range hitCotTheta;
...
double HitPairGeneratorFromLayerPairForPhotonConversion::
getCot(double dz, double dr){
