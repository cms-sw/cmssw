...
// float unsafe_asinhf(float x) { return std::abs(x)>10.f ? std::copysign(3.f,x)
// : unsafe_logf<3>(x+std::sqrt(1.f+x*x)); }
inline float unsafe_asinhf(float x) {
  return unsafe_logf<3>(x + std::sqrt(1.f + x * x));
}
} // namespace
...
void MSLayersKeeperX0AtEta::init(const edm::EventSetup &iSetup) {
  ...
  for (int idxbin = 0; idxbin < 2 * theHalfNBins; idxbin++) {
    ...
    for (it = layers.begin(); it != layers.end(); it++) {
      ...
      float sumX0D =
          theLayersData[idxbin].sumX0D(zero, it->crossing(line).first);
...
//------------------------------------------------------------------------------
void MSLayersKeeperX0AtEta::setX0(vector<MSLayer> &layers, float eta,
  ...
  for (il = layers.begin(); il != layers.end(); il++) {
    ...
    float rN =
        (il + 1 != layers.end()) ? (il + 1)->crossing(line).first.r() : BIG;
