...
void MSLayersAtAngle::init() {
  ...
  for (auto const& l : theLayers) {
    ...
    if (sq < 0)
      continue;
    if (sq >= int(indeces.size()))
      indeces.resize(sq + 1, -1);
    indeces[sq] = i;
  }
}
...
float MSLayersAtAngle::sumX0D(int il, int ol, const PixelRecoPointRZ& pointI, const PixelRecoPointRZ& pointO) const {
...
float MSLayersAtAngle::sumX0D(const PixelRecoPointRZ& pointI,
  ...
  if (doPrint)
    std::cout << "old " << (*iI).seqNum() << " " << iI - theLayers.begin() << ", " << (*iM).seqNum() << " "
              << iM - theLayers.begin() << ", " << (*iO).seqNum() << " " << iO - theLayers.begin() << "  " << pointI.r()
              << " " << pointI.z() << " " << sum << std::endl;

  return sum;
}

float MSLayersAtAngle::sumX0D(
    float zV, int il, int ol, const PixelRecoPointRZ& pointI, const PixelRecoPointRZ& pointO) const {
  ...
  if (doPrint)
    std::cout << "new " << il << " " << (*iI).seqNum() << " " << iI - theLayers.begin() << ", " << ol << " "
              << (*iO).seqNum() << " " << iO - theLayers.begin() << "  " << zV << " " << sum << std::endl;
...
float MSLayersAtAngle::sum2RmRn(MSLayersAtAngle::LayerItr i1,
  ...
  for (auto it = i1; it < i2; it++) {
    ...
    if (cross.second) {
      ...
      if (x0 > 1.e-5f)
        dr *= 1.f + 0.038f * unsafe_logf<2>(x0);
      sum2 += x0 * dr * dr;
    }
...
void MSLayersAtAngle::print() const {
  for (auto it = theLayers.begin(); it != theLayers.end(); it++)
    cout << *it << endl;
}
