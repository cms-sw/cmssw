...
float MSLayersAtAngle::sumX0D(int il, int ol, const PixelRecoPointRZ &pointI,
                              const PixelRecoPointRZ &pointO) const {
  // if layer not at this angle (WHY???) revert to slow comp
  if (il >= int(indeces.size()) || ol >= int(indeces.size()) ||
      indeces[il] < 0 || indeces[ol] < 0)
...
float MSLayersAtAngle::sumX0D(const PixelRecoPointRZ &pointI,
  ...
  if (doPrint)
    std::cout << "old " << (*iI).seqNum() << " " << iI - theLayers.begin()
              ...
              << pointI.r() << " " << pointI.z() << " " << sum << std::endl;
...
float MSLayersAtAngle::sumX0D(float zV, int il, int ol,
  ...
  if (doPrint)
    std::cout << "new " << il << " " << (*iI).seqNum() << " "
              << iI - theLayers.begin() << ", " << ol << " " << (*iO).seqNum()
              << " " << iO - theLayers.begin() << "  " << zV << " " << sum
              << std::endl;
...
float MSLayersAtAngle::sum2RmRn(MSLayersAtAngle::LayerItr i1,
                                MSLayersAtAngle::LayerItr i2, float rTarget,
...
MSLayersAtAngle::findLayer(const PixelRecoPointRZ &point,
