...
//------------------------------------------------------------------------------
vector<MSLayer>
MultipleScatteringGeometry::detLayers(const edm::EventSetup &iSetup) const {
...
//------------------------------------------------------------------------------
vector<MSLayer>
MultipleScatteringGeometry::detLayers(float eta, float z,
  ...
  for (il = theLayers.begin(); il != theLayers.end(); il++) {
    bool contains = false;
    //  if ((*il)->subDetector() != PixelBarrel && (*il)->subDetector()!=
    //  PixelEndcap) continue;
...
//------------------------------------------------------------------------------
vector<MSLayer>
MultipleScatteringGeometry::otherLayers(float eta,
                                        const edm::EventSetup &iSetup) const {
  vector<MSLayer> result;
  // zero
  ...
  // endflanges
  PixelRecoPointRZ endfPoint =
      (eta > 0) ? PixelRecoPointRZ(endflangesZ / sinh(eta), endflangesZ)
                : PixelRecoPointRZ(-endflangesZ / sinh(eta), -endflangesZ);
  if (0 < endfPoint.r() && endfPoint.r() < supportR) {
    MSLayer endflanges(endcap, endfPoint.z(),
                       MSLayer::Range(0.1, supportR - 0.1));
