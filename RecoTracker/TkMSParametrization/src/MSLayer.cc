...
MSLayer::MSLayer(const DetLayer *layer, const DataX0 &dataX0)
    : theFace(layer->location()), theSeqNum(layer->seqNum()),
      theX0Data(dataX0) {
...
//----------------------------------------------------------------------
MSLayer::MSLayer(Location part, float position, Range range,
                 float halfThickness, const DataX0 &dataX0)
    : theFace(part), thePosition(position), theRange(range),
      theHalfThickness(halfThickness), theSeqNum(-1), theX0Data(dataX0) {}
...
//----------------------------------------------------------------------
pair<PixelRecoPointRZ, bool>
MSLayer::crossing(const PixelRecoLineRZ &line) const {
  const float eps = 1.e-5;
  bool inLayer = true;
  float value =
      (theFace == barrel) ? line.zAtR(thePosition) : line.rAtZ(thePosition);
...
float MSLayer::x0(float cotTheta) const {
  if
    LIKELY(theX0Data.hasX0) {
      ...
      return (theFace == barrel)
                 ? theX0Data.x0 * OverSinTheta
    ...
    LIKELY(theX0Data.hasX0) {
      ...
      case barrel:
        return theX0Data.sumX0D *
               std::sqrt(
                   std::sqrt((1.f + cotTheta * cotTheta) /
                             (1.f + theX0Data.cotTheta * theX0Data.cotTheta)));
      case endcap:
        return (theX0Data.hasFSlope)
                   ? theX0Data.sumX0D +
                         theX0Data.slopeSumX0D *
                             (1.f / cotTheta - 1.f / theX0Data.cotTheta)
                   : theX0Data.sumX0D;
      case invalidLoc:
