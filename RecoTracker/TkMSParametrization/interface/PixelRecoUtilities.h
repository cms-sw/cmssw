 ...
 */
template <typename T> T bendingRadius(T pt, const edm::EventSetup &iSetup) {
 ...
 *  pT in GeV, magnetic field taken at (0,0,0)
 */
template <typename T> T curvature(T InversePt, const edm::EventSetup &iSetup) {
  return InversePt / fieldInInvGev(iSetup);
}

/** inverse pt from curvature **/
template <typename T> T inversePt(T curvature, const edm::EventSetup &iSetup) {
 ...
 */
inline double longitudinalBendingCorrection(double radius, double pt,
