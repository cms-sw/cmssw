...
public:
  ...
  float rAtZ(float z) const {
    return (std::abs(theCotLine) > 1.e-4f)
               ? addTIP(theOrigin.r() + (z - theOrigin.z()) / theCotLine)
               : 99999.f;
  }
...
public:
  ...
  SimpleLineRZ(const Point &aOrigin, const Point &aPoint)
      : theOrigin(aOrigin), theCotLine((aPoint.z() - theOrigin.z()) /
                                       (aPoint.r() - theOrigin.r())) {}
