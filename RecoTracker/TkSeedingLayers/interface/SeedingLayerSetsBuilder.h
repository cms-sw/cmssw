...
public:
  using SeedingLayerId =
      std::tuple<GeomDetEnumerators::SubDetector, TrackerDetSide, int>;
  ...
  unsigned short numberOfLayerSets() const {
    return theNumberOfLayersInSet > 0
               ? theLayerSetIndices.size() / theNumberOfLayersInSet
               : 0;
  }
  std::vector<SeedingLayerId>
  layers() const; // please call at most once per job per client
  ...
  const std::vector<SeedingLayerSetsHits::LayerSetIndex> &
  layerSetIndices() const {
    return theLayerSetIndices;
  }

  std::unique_ptr<SeedingLayerSetsHits> hits(const edm::Event &ev,
                                             const edm::EventSetup &es);
  // new function for FastSim only
  std::unique_ptr<SeedingLayerSetsHits>
  makeSeedingLayerSetsHitsforFastSim(const edm::Event &ev,
                                     const edm::EventSetup &es);
...
private:
  edm::ParameterSet layerConfig(const std::string &nameLayer,
  ...
  struct LayerSpec {
    LayerSpec(unsigned short index, const std::string &layerName,
              const edm::ParameterSet &cfgLayer, edm::ConsumesCollector &iC);
    ~LayerSpec() = default;
  ...
  std::vector<SeedingLayerSetsHits::LayerSetIndex>
      theLayerSetIndices; // indices to theLayers to form the layer sets
