...
SeedingLayerSetsBuilder::nameToEnumId(const std::string &name) {
...
SeedingLayerSetsBuilder::LayerSpec::LayerSpec(unsigned short index,
  ...
  if (subdet == GeomDetEnumerators::PixelBarrel ||
      subdet == GeomDetEnumerators::PixelEndcap) {
    extractor =
        std::make_unique<HitExtractorPIX>(side, idLayer, pixelHitProducer, iC);
  } else if (subdet != GeomDetEnumerators::invalidDet) { // strip
    auto extr = std::make_unique<HitExtractorSTRP>(subdet, side, idLayer,
                                                   clusterChargeCut(cfgLayer));
    if (cfgLayer.exists("matchedRecHits")) {
      extr->useMatchedHits(
          cfgLayer.getParameter<edm::InputTag>("matchedRecHits"), iC);
    }
    if (cfgLayer.exists("rphiRecHits")) {
      extr->useRPhiHits(cfgLayer.getParameter<edm::InputTag>("rphiRecHits"),
                        iC);
    }
    if (cfgLayer.exists("stereoRecHits")) {
      extr->useStereoHits(cfgLayer.getParameter<edm::InputTag>("stereoRecHits"),
                          iC);
    }
    if (cfgLayer.exists("useRingSlector") &&
        cfgLayer.getParameter<bool>("useRingSlector")) {
      extr->useRingSelector(cfgLayer.getParameter<int>("minRing"),
                            cfgLayer.getParameter<int>("maxRing"));
    }
    bool useSimpleRphiHitsCleaner =
        cfgLayer.exists("useSimpleRphiHitsCleaner")
            ? cfgLayer.getParameter<bool>("useSimpleRphiHitsCleaner")
            : true;
    extr->useSimpleRphiHitsCleaner(useSimpleRphiHitsCleaner);

    double minAbsZ = cfgLayer.exists("MinAbsZ")
                         ? cfgLayer.getParameter<double>("MinAbsZ")
                         : 0.;
    if (minAbsZ > 0.) {
      extr->setMinAbsZ(minAbsZ);
    }
    if (skipClusters) {
      bool useProjection = cfgLayer.exists("useProjection")
                               ? cfgLayer.getParameter<bool>("useProjection")
                               : false;
      if (useProjection) {
        LogDebug("SeedingLayerSetsBuilder")
            << layerName << " will project partially masked matched rechit";
      } else {
  ...
  if (extractor && skipClusters) {
    extractor->useSkipClusters(
        cfgLayer.getParameter<edm::InputTag>("skipClusters"), iC);
  }
}

std::string SeedingLayerSetsBuilder::LayerSpec::print(
...
SeedingLayerSetsBuilder::SeedingLayerSetsBuilder(
  ...
  fastSimrecHitsToken_ =
      iC.consumes<FastTrackerRecHitCollection>(fastsimHitTag);
}
...
SeedingLayerSetsBuilder::SeedingLayerSetsBuilder(const edm::ParameterSet &cfg,
                                                 edm::ConsumesCollector &iC) {
  std::vector<std::string> namesPset =
      cfg.getParameter<std::vector<std::string>>("layerList");
  std::vector<std::vector<std::string>> layerNamesInSets =
      this->layerNamesInSets(namesPset);
  ...
  for (IT it = layerNamesInSets.begin(); it != layerNamesInSets.end(); it++) {
    if (it->size() != theNumberOfLayersInSet)
      throw cms::Exception("Configuration")
          << "Assuming all SeedingLayerSets to have same number of layers. "
             "LayerSet "
          << (it - layerNamesInSets.begin()) << " has " << it->size()
          << " while 0th has " << theNumberOfLayersInSet;
    for (const std::string &layerName : *it) {
      auto found =
          std::find(theLayerNames.begin(), theLayerNames.end(), layerName);
      ...
      if (found != theLayerNames.end()) {
        ...
        if (std::numeric_limits<unsigned short>::max() ==
          ...
          throw cms::Exception("Assert")
              ...
              << ", we may have to enlarge the index type from unsigned short "
                 "to unsigned int";
        }

        layerIndex = theLayers.size();
        theLayers.emplace_back(theLayerNames.size(), layerName,
                               layerConfig(layerName, cfg), iC);
...
void SeedingLayerSetsBuilder::fillDescriptions(
    edm::ParameterSetDescription &desc) {
  edm::ParameterSetDescription empty;
  empty.setAllowAnything(); // for now accept any parameter in the PSets,
                            // consider improving later
...
edm::ParameterSet
SeedingLayerSetsBuilder::layerConfig(const std::string &nameLayer,
...
SeedingLayerSetsBuilder::layerNamesInSets(const vector<string> &namesPSet) {
  std::vector<std::vector<std::string>> result;
  for (std::vector<std::string>::const_iterator is = namesPSet.begin();
       is < namesPSet.end(); ++is) {
...
void SeedingLayerSetsBuilder::updateEventSetup(const edm::EventSetup &es) {
  ...
  const std::vector<ForwardDetLayer const *> &fpx_pos =
  ...
  const std::vector<ForwardDetLayer const *> &fpx_neg =
  ...
  for (const auto &layer : theLayers) {
    ...
    if (layer.subdet == GeomDetEnumerators::PixelBarrel) {
      ...
      throw cms::Exception("Configuration")
          << "Did not find DetLayer for layer "
          << theLayerNames[layer.nameIndex];
    }
...
std::vector<SeedingLayerSetsBuilder::SeedingLayerId>
SeedingLayerSetsBuilder::layers() const {
...
SeedingLayerSetsBuilder::hits(const edm::Event &ev, const edm::EventSetup &es) {
  ...
  auto ret = std::make_unique<SeedingLayerSetsHits>(
      theNumberOfLayersInSet, &theLayerSetIndices, &theLayerNames,
      &theLayerDets);

  for (auto &layer : theLayers) {
    ret->addHits(
        layer.nameIndex,
        layer.extractor->hits((const TkTransientTrackingRecHitBuilder
                                   &)(*theTTRHBuilders[layer.nameIndex]),
                              ev, es));
  }
  ret->shrink_to_fit();
  return ret;
...
SeedingLayerSetsBuilder::makeSeedingLayerSetsHitsforFastSim(
  ...
  for (auto &layer : theLayers) {
    ...
    for (auto &rh : *fastSimrechits_) {
      ...
      TrackerDetSide side = TrackerDetSide::Barrel;
      int idLayer = 0;
      if ((rh.det()->geographicalId()).subdetId() ==
          PixelSubdetector::PixelBarrel) {
        subdet = GeomDetEnumerators::PixelBarrel;
        side = TrackerDetSide::Barrel;
        idLayer = tTopo->pxbLayer(rh.det()->geographicalId());
      } else if ((rh.det()->geographicalId()).subdetId() ==
      ...
      if (layer.subdet == subdet && layer.side == side &&
