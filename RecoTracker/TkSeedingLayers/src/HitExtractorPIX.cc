...
HitExtractorPIX::HitExtractorPIX(TrackerDetSide side, int idLayer,
...
HitExtractor::Hits
HitExtractorPIX::hits(const TkTransientTrackingRecHitBuilder &ttrhBuilder,
  ...
  if (theSide == TrackerDetSide::Barrel) {
    range2SeedingHits(*pixelHits, result,
                      ttopo.pxbDetIdLayerComparator(theIdLayer));
  } else {
    range2SeedingHits(*pixelHits, result,
                      ttopo.pxfDetIdDiskComparator(
                          static_cast<unsigned int>(theSide), theIdLayer));
  }

  if (skipClusters) {
    LogDebug("HitExtractorPIX")
        << "getting : " << result.size() << " pixel hits.";
    ...
    for (unsigned int iH = 0; iH != result.size(); ++iH) {
      if (result[iH]->isValid()) { // can be NOT valid???
        ...
        if (pixelClusterMask->mask(concrete.cluster().key())) {
          // too much debug LogDebug("HitExtractorPIX")<<"skipping a pixel hit
          // on: "<< result[iH]->hit()->geographicalId().rawId()<<" key:
          // "<<find(f->begin(),f->end(),concrete->cluster())->key();
          skipped++;
          result[iH].reset();
        }
      }
    ...
    // std::cout << "HitExtractorPIX " <<"skipped :"<<skipped<<" pixel clusters
    // out of " << result.size() << std::endl;
    if (skipped > 0) {
      auto last = std::remove_if(result.begin(), result.end(),
                                 [](HitPointer const &p) { return p.empty(); });
      result.resize(last - result.begin());
    }
  }
  LogDebug("HitExtractorPIX") << "giving :" << result.size() << " rechits out";
  // std::cout << "HitExtractorPIX "<<"giving :"<<result.size()<<" rechits out"
  // << std::endl;
  return result;
}
