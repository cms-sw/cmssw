...
HitExtractor::Hits HitExtractorPIX::hits(const SeedingLayer & sl,const edm::Event& ev, const edm::EventSetup& es) const
{
  ...
  if (theSide==SeedingLayer::Barrel) {
    range2SeedingHits( *pixelHits, result, ttopo.pxbDetIdLayerComparator(theIdLayer), sl, es );
  } else {
    range2SeedingHits( *pixelHits, result,ttopo.pxfDetIdDiskComparator(theSide,theIdLayer), sl, es );
  }
