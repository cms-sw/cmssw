...
public:
  ...
  HitExtractorSTRP(GeomDetEnumerators::SubDetector subdet, TrackerDetSide side,
                   int idLayer, float iminGoodCharge);
  ...
  void cleanedOfClusters(const TkTransientTrackingRecHitBuilder &ttrhBuilder,
                         ...
                         bool matched, unsigned int cleanFrom = 0) const;

  std::pair<bool, ProjectedSiStripRecHit2D *>
  skipThis(const TkTransientTrackingRecHitBuilder &ttrhBuilder,
           TkHitRef matched,
           edm::Handle<edm::ContainerMask<edmNew::DetSetVector<SiStripCluster>>>
               &stripClusterMask) const;

  bool
  skipThis(DetId id, OmniClusterRef const &clus,
           edm::Handle<edm::ContainerMask<edmNew::DetSetVector<SiStripCluster>>>
               &stripClusterMask) const;
...
private:
  ...
  typedef edm::ContainerMask<edmNew::DetSetVector<SiStripCluster>>
