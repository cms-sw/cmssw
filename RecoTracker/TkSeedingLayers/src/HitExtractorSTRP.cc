...
HitExtractorSTRP::HitExtractorSTRP(GeomDetEnumerators::SubDetector subdet,
                                   TrackerDetSide side, int idLayer,
                                   float iminGoodCharge)
    : theLayerSubDet(subdet), theSide(side), theIdLayer(idLayer), minAbsZ(0),
      theMinRing(1), theMaxRing(0), hasMatchedHits(false), hasRPhiHits(false),
      hasStereoHits(false), hasRingSelector(false),
...
bool HitExtractorSTRP::skipThis(
    DetId id, OmniClusterRef const &clus,
    edm::Handle<edm::ContainerMask<edmNew::DetSetVector<SiStripCluster>>>
        &stripClusterMask) const {
  ...
  return siStripClusterTools::chargePerCM(id, *clus.cluster_strip()) <=
         minGoodCharge;
}

std::pair<bool, ProjectedSiStripRecHit2D *> HitExtractorSTRP::skipThis(
    const TkTransientTrackingRecHitBuilder &ttrhBuilder, TkHitRef matched,
    edm::Handle<edm::ContainerMask<edmNew::DetSetVector<SiStripCluster>>>
        &stripClusterMask) const {
  ...
  replaceMe =
      cloner.project(hit, rejectSt, TrajectoryStateOnSurface()).release();
  if (rejectSt)
    LogDebug("HitExtractorSTRP") << "a matched hit is partially masked, and "
                                    "the mono hit got projected onto: "
                                 << replaceMe->geographicalId().rawId()
                                 << " key: " << hit.monoClusterRef().key();
  else
    LogDebug("HitExtractorSTRP") << "a matched hit is partially masked, and "
                                    "the stereo hit got projected onto: "
                                 << replaceMe->geographicalId().rawId()
...
void HitExtractorSTRP::cleanedOfClusters(
    const TkTransientTrackingRecHitBuilder &ttrhBuilder, const edm::Event &ev,
    HitExtractor::Hits &hits, bool matched, unsigned int cleanFrom) const {
  ...
  for (unsigned int iH = cleanFrom; iH < hits.size(); ++iH) {
    ...
    if (matched) {
      ...
      std::tie(replace, replaceMe) =
          skipThis(ttrhBuilder, *hits[iH], stripClusterMask);
      if (replace) {
        if (!replaceMe) {
          LogDebug("HitExtractorSTRP") << "skipping a matched hit on :"
  ...
  auto last = std::remove_if(hits.begin() + cleanFrom, hits.end(),
                             [](HitPointer const &p) { return p.empty(); });
  hits.resize(last - hits.begin());

  // std::cout << "HitExtractorSTRP " <<"skipped :"<<skipped<<" strip rechits
  // because of clusters and projected: "<<projected << std::endl;
  LogDebug("HitExtractorSTRP")
      << "skipped :" << skipped
...
HitExtractor::Hits
HitExtractorSTRP::hits(const TkTransientTrackingRecHitBuilder &ttrhBuilder,
  ...
  if (theLayerSubDet == GeomDetEnumerators::TIB) {
    if (hasMatchedHits) {
      ...
      range2SeedingHits(*matchedHits, result,
                        tTopo->tibDetIdLayerComparator(theIdLayer));
      if (skipClusters)
    ...
    if (hasRPhiHits) {
      ...
      if (hasMatchedHits) {
        if (!hasSimpleRphiHitsCleaner) { // this is a brutal "cleaning". Add
                                         // something smarter in the future
          if (skipClusters)
            cleanFrom = result.size();
          range2SeedingHits(*rphiHits, result,
                            tTopo->tibDetIdLayerComparator(theIdLayer));
          if (skipClusters)
    ...
    if (hasStereoHits) {
      ...
      if (skipClusters)
        cleanFrom = result.size();
      range2SeedingHits(*stereoHits, result,
                        tTopo->tibDetIdLayerComparator(theIdLayer));
      if (skipClusters)
        cleanedOfClusters(ttrhBuilder, ev, result, false, cleanFrom);
    }
  ...
  //
    ...
    if (hasMatchedHits) {
      ...
      if (skipClusters)
        cleanFrom = result.size();
      auto getter = tTopo->tidDetIdWheelComparator(
          static_cast<unsigned int>(theSide), theIdLayer);
      SiStripMatchedRecHit2DCollection::Range range =
          matchedHits->equal_range(getter.first, getter.second);
      for (SiStripMatchedRecHit2DCollection::const_iterator it = range.first;
           it != range.second; ++it) {
        int ring = tTopo->tidRing(it->detId());
        if (!ringRange(ring))
          continue;
        for (SiStripMatchedRecHit2DCollection::DetSet::const_iterator
                 hit = it->begin(),
                 end = it->end();
             hit != end; ++hit) {
    ...
    if (hasRPhiHits) {
      ...
      auto getter = tTopo->tidDetIdWheelComparator(
          static_cast<unsigned int>(theSide), theIdLayer);
      SiStripRecHit2DCollection::Range range =
          rphiHits->equal_range(getter.first, getter.second);
      for (SiStripRecHit2DCollection::const_iterator it = range.first;
           it != range.second; ++it) {
        int ring = tTopo->tidRing(it->detId());
        if (!ringRange(ring))
          continue;
        if ((SiStripDetId(it->detId()).partnerDetId() != 0) &&
            hasSimpleRphiHitsCleaner)
          continue; // this is a brutal "cleaning". Add something smarter in the
                    // future
        for (SiStripRecHit2DCollection::DetSet::const_iterator
                 hit = it->begin(),
                 end = it->end();
             hit != end; ++hit) {
          result.emplace_back(*hit);
    ...
    if (hasStereoHits) {
      ...
      auto getter = tTopo->tidDetIdWheelComparator(
          static_cast<unsigned int>(theSide), theIdLayer);
      SiStripRecHit2DCollection::Range range =
          stereoHits->equal_range(getter.first, getter.second);
      for (SiStripRecHit2DCollection::const_iterator it = range.first;
           it != range.second; ++it) {
        int ring = tTopo->tidRing(it->detId());
        if (!ringRange(ring))
  ...
  //
    ...
    if (hasMatchedHits) {
      ...
      if (minAbsZ > 0.) {
        ...
        range2SeedingHits(*matchedHits, result,
                          tTopo->tobDetIdLayerComparator(theIdLayer));
      }
      if (skipClusters)
        cleanedOfClusters(ttrhBuilder, ev, result, true, cleanFrom);
    }
  ...
  //
    ...
    if (hasMatchedHits) {
      ...
      if (skipClusters)
        cleanFrom = result.size();
      auto getter = tTopo->tecDetIdWheelComparator(
          static_cast<unsigned int>(theSide), theIdLayer);
      SiStripMatchedRecHit2DCollection::Range range =
          matchedHits->equal_range(getter.first, getter.second);
      for (SiStripMatchedRecHit2DCollection::const_iterator it = range.first;
  ...
  // std::cout << "HitExtractorSTRP "<<" giving: "<<result.size() << " for
  // charge cut " << minGoodCharge << std::endl;
  return result;
}
