...
public:
  ...
  virtual Hits hits(const TkTransientTrackingRecHitBuilder &ttrhBuilder,
                    const edm::Event &, const edm::EventSetup &) const = 0;
...
protected:
  virtual void useSkipClusters_(const edm::InputTag &m,
                                edm::ConsumesCollector &iC) = 0;
};

template <typename DSTV, typename A, typename B>
inline void range2SeedingHits(DSTV const &dstv, HitExtractor::Hits &v,
  ...
  v.reserve(ts);
  for (typename DSTV::const_iterator id = range.first; id != range.second;
       id++) {
    for (auto const &h : (*id))
      v.emplace_back(h);
  }
