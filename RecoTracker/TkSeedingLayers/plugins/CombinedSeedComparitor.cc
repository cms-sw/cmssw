...
CombinedSeedComparitor::CombinedSeedComparitor(const edm::ParameterSet &cfg,
  ...
  if (mode == "and")
    ...
    throw cms::Exception("Configuration",
                         "Parameter 'mode' of CombinedSeedComparitor must be "
                         "either 'and' or 'or'\n");

  typedef std::vector<edm::ParameterSet> VPSet;
  VPSet psets = cfg.getParameter<VPSet>("comparitors");
  for (VPSet::const_iterator it = psets.begin(), ed = psets.end(); it != ed;
       ++it) {
...
bool CombinedSeedComparitor::compatible(const SeedingHitSet &hits) const {
  ...
  for (ITC it = comparitors_.begin(), ed = comparitors_.end(); it != ed; ++it) {
    ...
    if (isAnd_ != pass)
      return pass; // break on failures if doing an AND, and on successes if
                   // doing an OR
  }
  return isAnd_; // if we arrive here, we have no successes for OR, and no
                 // failures for AND
}

bool CombinedSeedComparitor::compatible(
...
DEFINE_EDM_PLUGIN(SeedComparitorFactory, CombinedSeedComparitor,
