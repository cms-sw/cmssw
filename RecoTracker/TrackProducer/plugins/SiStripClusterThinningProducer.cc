    ...
    for (const auto& hit : *trackingRecHits) {
      ...
      if (singleHit != nullptr) {
        ...
        if (matched2DHit != nullptr) {
          edm::Ref<edmNew::DetSetVector<SiStripCluster>, SiStripCluster> const& monoRef = matched2DHit->monoClusterRef().cluster_strip();
          edm::Ref<edmNew::DetSetVector<SiStripCluster>, SiStripCluster> const& stereoRef = matched2DHit->stereoClusterRef().cluster_strip();
