...
void LSTEvent::createTriplets() {
  ...
  auto nSegments_buf_d =
      cms::alpakatools::make_device_view(queue_, segmentsDC_->const_view<SegmentsOccupancySoA>().nSegments());
...
void LSTEvent::createPixelTriplets() {
  ...
  alpaka::memcpy(queue_, pixelTypes_buf, cms::alpakatools::make_device_view(queue_, pixelSeeds.pixelType()));
...
void LSTEvent::addMiniDoubletsToEventExplicit() {
  ...
  auto nMDs_view = cms::alpakatools::make_device_view(queue_, mdsOccupancy.nMDs());  // exclude pixel part
  ...
  auto module_subdets_view = cms::alpakatools::make_device_view(queue_, modules.subdets());  // only lower modules
  ...
  auto module_layers_view = cms::alpakatools::make_device_view(queue_, modules.layers());  // only lower modules
...
const TrackCandidatesConst& LSTEvent::getTrackCandidates(bool inCMSSW, bool sync) {
  if (!trackCandidatesHC_) {
    ...
    alpaka::memcpy(queue_,
                   ...
                   cms::alpakatools::make_device_view(queue_, (*trackCandidatesDC_)->hitIndices()));
    alpaka::memcpy(queue_,
    ...
    if (not inCMSSW) {
      alpaka::memcpy(queue_,
                     ...
                     cms::alpakatools::make_device_view(queue_, (*trackCandidatesDC_)->logicalLayers()));
      alpaka::memcpy(queue_,
      ...
      alpaka::memcpy(queue_,
                     ...
                     cms::alpakatools::make_device_view(queue_, (*trackCandidatesDC_)->objectIndices()));
    }
    alpaka::memcpy(queue_,
