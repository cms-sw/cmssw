...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
  ...
  void OTStubProducerVectorHitStyle::produce(device::Event& iEvent, device::EventSetup const& iSetup) {
    ...
    if (nStubs > 0) {
      ...
      auto errView = cms::alpakatools::make_device_view(alpaka::getDev(queue), stubsView.dPhiDrError().data(), nStubs);
      auto errPrecView =
          cms::alpakatools::make_device_view(alpaka::getDev(queue), stubsView.dPhiDrErrorPrec().data(), nStubs);
