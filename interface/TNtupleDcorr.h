// @(#)root/tree:$Name:  $:$Id: TNtupleDcorr.h,v 1.7 2007/01/25 22:53:05 pcanal Exp $
...
#ifndef ROOT_TNtupleDcorr
#define ROOT_TNtupleDcorr
...
class TNtupleDcorr : public TTree {
...
private:
   TNtupleDcorr(const TNtupleDcorr&);             // not implemented
   TNtupleDcorr& operator=(const TNtupleDcorr&);  // not implemented

public:
   TNtupleDcorr();
   TNtupleDcorr(const char *name,const char *title, const char *varlist, Int_t bufsize=32000);
   virtual ~TNtupleDcorr();
...
//   virtual Long64_t  ReadFile(const char *filename, const char *branchDescriptor="");
   ...
   ClassDef(TNtupleDcorr,1)  //A simple tree with branches of floats.
};
