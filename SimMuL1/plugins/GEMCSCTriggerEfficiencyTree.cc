#include "GEMCode/SimMuL1/plugins/GEMCSCTriggerEfficiencyTree.h"
...
// class' constants
//
const std::string GEMCSCTriggerEfficiencyTree::csc_type[CSC_TYPES+1] = 
  { "ME1/1", "ME1/2", "ME1/3", "ME1/a", "ME2/1", "ME2/2", "ME3/1", "ME3/2", "ME4/1", "ME4/2", "ME1/T"};
const std::string GEMCSCTriggerEfficiencyTree::csc_type_[CSC_TYPES+1] = 
  { "ME11", "ME12", "ME13", "ME1A", "ME21", "ME22", "ME31", "ME32", "ME41", "ME42", "ME1T"};
const std::string GEMCSCTriggerEfficiencyTree::csc_type_a[CSC_TYPES+2] =
  { "N/A", "ME1/a", "ME1/b", "ME1/2", "ME1/3", "ME2/1", "ME2/2", "ME3/1", "ME3/2", "ME4/1", "ME4/2", "ME1/T"};
const std::string GEMCSCTriggerEfficiencyTree::csc_type_a_[CSC_TYPES+2] =
  { "NA", "ME1A", "ME1B", "ME12", "ME13", "ME21", "ME22", "ME31", "ME32", "ME41", "ME42", "ME1T"};

const int GEMCSCTriggerEfficiencyTree::NCHAMBERS[CSC_TYPES] = 
  { 36,  36,  36,  36, 18,  36,  18,  36,  18,  36};

const int GEMCSCTriggerEfficiencyTree::MAX_WG[CSC_TYPES] = 
  { 48,  64,  32,  48, 112, 64,  96,  64,  96,  64};//max. number of wiregroups

const int GEMCSCTriggerEfficiencyTree::MAX_HS[CSC_TYPES] = 
  { 128, 160, 128, 96, 160, 160, 160, 160, 160, 160}; // max. # of halfstrips

//const int GEMCSCTriggerEfficiencyTree::ptype[CSCConstants::NUM_CLCT_PATTERNS_PRE_TMB07]= 
//  { -999,  3, -3,  2, -2,  1, -1,  0};  // "signed" pattern (== phiBend)
const int GEMCSCTriggerEfficiencyTree::pbend[CSCConstants::NUM_CLCT_PATTERNS]= 
  { -999,  -5,  4, -4,  3, -3,  2, -2,  1, -1,  0}; // "signed" pattern (== phiBend)


const double GEMCSCTriggerEfficiencyTree::PT_THRESHOLDS[N_PT_THRESHOLDS] = {0,10,20,30,40,50};
const double GEMCSCTriggerEfficiencyTree::PT_THRESHOLDS_FOR_ETA[N_PT_THRESHOLDS] = {10,15,30,40,55,70};



// ================================================================================================
GEMCSCTriggerEfficiencyTree::GEMCSCTriggerEfficiencyTree(const edm::ParameterSet& iConfig):
...
// ================================================================================================
GEMCSCTriggerEfficiencyTree::~GEMCSCTriggerEfficiencyTree()
{
  /*
...
// ================================================================================================
void 
GEMCSCTriggerEfficiencyTree::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  nevt++;
...
/*
  ...
  // size of MC collection
  std::cout << "MC candidates: " << cands.size() << std::endl;
  // check MC truch loop
...
//       h_phi_mctr->Fill(mcphi);
      ...
      std::cout << "match with SimTrack" << std::endl;
      std::cout << "number of candidates: " << simTracks.size() << std::endl; 
	...
	// delta R based monte carlo matching
...
// 	h_DR_mctr_simtr->Fill(dr);
	if (dr < minDeltaRSimTr)
	{
      ...
      std::cout << "number of good muon tracks: " << numberSimTr << std::endl;
...
*/
  ...
  */  double w[7] = {0., 0., 0., 1, 2, 3, 444};
  // for (int i = 7 - 1; i >= 0; i--) 
  //   cout << w[i] << std::endl;
  std::vector<double> myvec(w, w + 7);



  // find primary vertex index and fill trkId2Index association:
  std::cout<<"locate primary vertex "<<std::endl;
  ...
  for (edm::SimTrackContainer::const_iterator istrk = simTracks.begin(); istrk != simTracks.end(); ++istrk)
  {
//     int iTrack = istrk-simTracks.begin(); 
    std::cout << "simtrack: " << std::endl;
...
//     etrk_.st_phi[iTrack] = stphi;
    ...
    if (!goodPhiEta) 
    {
      std::cout<<"  rejected -- muon SimTrack: phi = "<<stphi<<" eta = "<<steta<<" outside region"<<std::endl;
      continue;
    }

    std::cout<<"  --Accepting muon SimTrack: pt = "<<stpt<<"  phi = "<<stphi<<" eta = "<<steta<<std::endl;
    */
    /*    
    MatchCSCMuL1 * match = new MatchCSCMuL1(&*istrk, &(simVertices[istrk->vertIndex()]), cscGeometry);
    match->muOnly = doStrictSimHitToTrackMatch_;
...
/*
  ...
  if (debugALLEVENT) {
    ...
    if (ch_overlap.size()==0)std::cout<<std::endl;
    else {
...
// ================================================================================================
void 
GEMCSCTriggerEfficiencyTree::cleanUp()
{
  if (addGhostLCTs_)
    {
      for (size_t i=0; i<ghostLCTs.size();i++) if (ghostLCTs[i]) delete ghostLCTs[i];
...
// ================================================================================================
void 
GEMCSCTriggerEfficiencyTree::propagateToCSCStations(MatchCSCMuL1 *match)
{
  // do not propagate for high etas
...
// ================================================================================================
/*
 * Match this simtrack to the CSC simhits
 */
void 
GEMCSCTriggerEfficiencyTree::matchSimTrack2SimHits( MatchCSCMuL1 * match, 
						    ...
						    const edm::PSimHitContainer * allCSCSimHits )
{
  ...
  for (unsigned i=0; i<matchingSimHits.size();i++) {
    if (goodChambersOnly_ && theStripConditions->isInBadChamber(CSCDetId(matchingSimHits[i].detUnitId()))){
      std::cout << "  --skipping bad chamber" << std::endl;
      continue; 
    }
    match->addSimHit(matchingSimHits[i]);
  }

  // calculate number of good simhits
  unsigned stNhist = 0;
  for (edm::PSimHitContainer::const_iterator hit = allCSCSimHits->begin();  hit != allCSCSimHits->end();  ++hit){
    if (hit->trackId() != match->strk->trackId()){
      std::cout << "  rejected -- simhit track Id is different from simtrack id: " << hit->trackId() << " " << match->strk->trackId() << std::endl;
      continue;
    }
    
    // get the detId for this hit
    CSCDetId chId(hit->detUnitId());

    if ( chId.station() == 1 && chId.ring() == 4 && !doME1a_){
      std::cout << "  rejected -- simhit is in invalid location" << std::endl;
      continue;
    }
    stNhist++;
  }

  // check if the number of mathed simhits agree
...
// ================================================================================================
void
GEMCSCTriggerEfficiencyTree::matchSimTrack2CLCTs(MatchCSCMuL1 *match, 
				 const edm::PSimHitContainer* allCSCSimHits, 
...
// ================================================================================================
void
GEMCSCTriggerEfficiencyTree::matchSimTrack2LCTs(MatchCSCMuL1 *match, 
...
// ================================================================================================
void
GEMCSCTriggerEfficiencyTree::matchSimTrack2MPLCTs(MatchCSCMuL1 *match, 
...
// ================================================================================================
void
GEMCSCTriggerEfficiencyTree::matchSimtrack2TFTRACKs( MatchCSCMuL1 *match,
...
// ================================================================================================
void
GEMCSCTriggerEfficiencyTree::matchSimtrack2TFCANDs( MatchCSCMuL1 *match,
				    edm::ESHandle< L1MuTriggerScales > &muScales,
				    edm::ESHandle< L1MuTriggerPtScale > &muPtScale,
...
// ================================================================================================
void
GEMCSCTriggerEfficiencyTree::matchSimtrack2GMTCANDs( MatchCSCMuL1 *match, 
             edm::ESHandle< L1MuTriggerScales > &muScales,
             edm::ESHandle< L1MuTriggerPtScale > &muPtScale,
...
// ================================================================================================
std::vector<unsigned> 
GEMCSCTriggerEfficiencyTree::fillSimTrackFamilyIds(unsigned  id,
...
// ================================================================================================
std::vector<PSimHit> 
GEMCSCTriggerEfficiencyTree::hitsFromSimTrack(std::vector<unsigned> ids, SimHitAnalysis::PSimHitMap &hitMap)
{
  std::vector<PSimHit> result;

  std::cout<<"--- hitsFromSimTrack std::vector size "<<ids.size()<<std::endl;

  for (size_t id = 0; id < ids.size(); id++)
  {
    std::vector<PSimHit> resultd = hitsFromSimTrack(ids[id], hitMap);
    result.insert(result.end(), resultd.begin(), resultd.end());
    std::cout<<"  --- n "<<id<<" id "<<ids[id]<<" size "<<resultd.size()<<std::endl;
  }
  return result;
}
...
// ================================================================================================
std::vector<PSimHit> 
GEMCSCTriggerEfficiencyTree::hitsFromSimTrack(unsigned id, SimHitAnalysis::PSimHitMap &hitMap)
{
  int fdebug = 1;
...
// ================================================================================================
std::vector<PSimHit> 
GEMCSCTriggerEfficiencyTree::hitsFromSimTrack(unsigned id, int detId, SimHitAnalysis::PSimHitMap &hitMap)
{
  int fdebug = 1;
  ...
  for(size_t h = 0; h< hits.size(); h++) {
    if(hits[h].trackId() == id) {
      ...
      if (fdebug)  
	std::cout<< "   --- "<<detId<<" -> "<< hits[h] <<" "<<hits[h].exitPoint()<<" "
		 << hits[h].momentumAtEntry()<<" "<<hits[h].energyLoss()<<" "<<hits[h].particleType()<<" "<<hits[h].trackId()<<std::endl;
    }
  }
  return result;
}
...
// ================================================================================================
int 
GEMCSCTriggerEfficiencyTree::particleType(int simTrack) 
{
  int result = 0;
...
// ================================================================================================
bool 
GEMCSCTriggerEfficiencyTree::compareSimHits(PSimHit &sh1, PSimHit &sh2)
{
  int fdebug = 0;
...
// ================================================================================================
unsigned
GEMCSCTriggerEfficiencyTree::matchCSCAnodeHits(const std::vector<CSCAnodeLayerInfo>& allLayerInfo, 
...
// ================================================================================================
unsigned
GEMCSCTriggerEfficiencyTree::matchCSCCathodeHits(const std::vector<CSCCathodeLayerInfo>& allLayerInfo, 
				 std::vector<PSimHit> &matchedHit) 
{
  // It first tries to look for the SimHit in the key layer.  If it is
  // unsuccessful, it loops over all layers and looks for an associated
...
// ================================================================================================
int 
GEMCSCTriggerEfficiencyTree::calculate2DStubsDeltas(MatchCSCMuL1 *match, MatchCSCMuL1::ALCT &alct)
{
...
// ================================================================================================
int 
GEMCSCTriggerEfficiencyTree::calculate2DStubsDeltas(MatchCSCMuL1 *match, MatchCSCMuL1::CLCT &clct)
{
  //** fit muon's hits to a 2D linear stub in a chamber :
...
GEMCSCTriggerEfficiencyTree::propagateSimTrackToZ(const SimTrack *track, const SimVertex *vtx, double z)
{
...
unsigned int 
GEMCSCTriggerEfficiencyTree::findQuality(const CSCALCTDigi& aLCT, const CSCCLCTDigi& cLCT)
{
...
void GEMCSCTriggerEfficiencyTree::dumpWireDigis(CSCDetId &id, const CSCWireDigiCollection* wiredc)
{
  // foolproof 1st layer
...
int 
GEMCSCTriggerEfficiencyTree::getCSCType(CSCDetId &id) 
{
  int type = -999;
...
int
GEMCSCTriggerEfficiencyTree::isME11(int t)
{
...
int
GEMCSCTriggerEfficiencyTree::getCSCSpecsType(CSCDetId &id)
{
...
// ================================================================================================
int
GEMCSCTriggerEfficiencyTree::cscTriggerSubsector(CSCDetId &id)
{
...
void GEMCSCTriggerEfficiencyTree::setupTFModeHisto(TH1D* h)
{
  if (h==0) return;
...
std::pair<float, float> GEMCSCTriggerEfficiencyTree::intersectionEtaPhi(CSCDetId id, int wg, int hs)
{
...
csctf::TrackStub GEMCSCTriggerEfficiencyTree::buildTrackStub(const CSCCorrelatedLCTDigi &d, CSCDetId id)
{
...
bool GEMCSCTriggerEfficiencyTree::isGEMDPhiGood(double dphi, double tfpt, int is_odd)
{
...
void 
GEMCSCTriggerEfficiencyTree::beginJob() {}

void 
GEMCSCTriggerEfficiencyTree::endJob() {}


//define this as a plug-in
DEFINE_FWK_MODULE(GEMCSCTriggerEfficiencyTree);
