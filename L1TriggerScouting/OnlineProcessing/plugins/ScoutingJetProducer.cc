...
void ScoutingJetProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (const unsigned& bx : caloTowerCollection->getFilledBxs()) {
    ...
    for (const auto& ct : cts) {
      ROOT::Math::PtEtaPhiMVector ctLV(calol1::fEt(ct.hwEt()), calol1::fEta(ct.hwEta()), calol1::fPhi(ct.hwPhi()), 0);
    ...
    for (const auto& incJet : incJets) {
      ...
      FastJet fj = FastJet(incJet.Et(), incJet.eta(), incJet.phi(), incJet.m(), nConst, area);
