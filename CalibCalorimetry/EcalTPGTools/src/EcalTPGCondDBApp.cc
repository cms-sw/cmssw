...
EcalTPGCondDBApp::EcalTPGCondDBApp(string host, string sid, string user, string pass, int port)
{
  try {
...
EcalTPGCondDBApp::EcalTPGCondDBApp(string sid, string user, string pass)
{
  try {
...
EcalTPGCondDBApp::~EcalTPGCondDBApp() 
{
  delete econn;
}


int EcalTPGCondDBApp::writePedestals(int runa, int runb ) {
  ...
  if(mon_runs>0) {
    ...
    for (CImped p = dataset_ped.begin(); p != dataset_ped.end(); p++) {
      ...
      ped_m12[xt_num]=int(rd_ped.getPedMeanG12()+0.5) ;
      ped_m6[xt_num]=int(rd_ped.getPedMeanG6()+0.5) ;
      ped_m1[xt_num]=int(rd_ped.getPedMeanG1()+0.5) ;
      
      int safety = 0 ;
      ped_m12[xt_num]-= safety ;
      ped_m6[xt_num]-= safety ;
      ped_m1[xt_num]-= safety ;
...
void EcalTPGCondDBApp::readPedestals(int iconf_req ) {
...
void EcalTPGCondDBApp::writeLUT() {
  // now we do something else 
...
void EcalTPGCondDBApp::writeWeights() {
  // now we do something else 
...
void EcalTPGCondDBApp::printTag( const RunTag* tag) const
{
  cout << endl;
  cout << "=============RunTag:" << endl;
...
void EcalTPGCondDBApp::printIOV( const RunIOV* iov) const
{
  cout << endl;
  cout << "=============RunIOV:" << endl;
