...
// TODO: handling case of weight encoding saturation: weights shall be
// downscaled to prevent saturation
...
EcalDccWeightBuilder::EcalDccWeightBuilder(edm::ParameterSet const &ps)
    : dcc1stSample_(ps.getParameter<int>("dcc1stSample")),
      ...
      dccWeightsWithIntercalib_(
          ps.getParameter<bool>("dccWeightsWithIntercalib")),
      ...
      sqlMode_(ps.getParameter<bool>("sqlMode")), calibMap_(emptyCalibMap_) {
...
void EcalDccWeightBuilder::computeAllWeights(bool withIntercalib,
  ...
  for (vector<DetId>::const_iterator it = detIds.begin(); it != detIds.end();
       ++it) {
    ...
    try {
      ...
      if (phase != prevPhase) {
        if (imode_ == COMPUTE_WEIGHTS) {
          if (it->subdetId() == EcalBarrel) {
            computeWeights(*pShape, binOfMax, phase, dcc1stSample_ - 1,
                           nDccWeights_, iSkip0_, baseWeights);
          }
          prevPhase = phase;
        }
      }
...
void EcalDccWeightBuilder::computeWeights(const EcalShapeBase &shape,
                                          ...
                                          int iFirst, int nWeights, int iSkip,
  ...
  const double tzero =
      -(binOfMax - 1) * 25 + timePhase + shape.timeToRise(); // ns
  ...
  for (int i = 0; i < nWeights; ++i) {
    if (i == iSkip) {
      ...
      result[i] = (result[i] - sum / nActualWeights) /
                  (sum2 - sum * sum / nActualWeights);
    }
  }
}
...
template <class T>
void EcalDccWeightBuilder::sort(const std::vector<T> &a, std::vector<int> &s,
...
void EcalDccWeightBuilder::unbiasWeights(std::vector<double> &weights,
  ...
  while (wsum != 0) {
    ...
    i += wsumSign;
    if (i < 0 ||
        i >= (int)nw) { // recompute the residuals if a second iteration is
...
double EcalDccWeightBuilder::intercalib(const DetId &detId) {
  ...
  if (itCalib != calibMap_.end()) {
    ...
    coef = 1.;
    std::cout << (uint32_t)detId << " not found in EcalIntercalibConstantMap"
              << std::endl;
  }
#if 0
  cout << __FILE__ << ":" << __LINE__ << ": ";
...
void EcalDccWeightBuilder::writeWeightToAsciiFile() {
  string fName = !asciiOutputFileName_.empty() ? asciiOutputFileName_.c_str()
                                               : "dccWeights.txt";
  ofstream file(fName.c_str());
  if (!file.good()) {
    throw cms::Exception("Output")
        << "Failed to open file '" << fName << "'for writing DCC weights\n";
  }
  ...
  if (sqlMode_) {
    ...
    file << "\n"
         ...
         << " loop\n"
            "    insert into dcc_weightsample_dat (rec_id, logic_id, "
            "sample_id, \n"
            "    weight_number)\n"
            "    values(:recid,fedid,"
         << dcc1stSample_
  ...
  if (!file.good()) {
    throw cms::Exception("Output")
        << "Error while writing DCC weights to '" << fName << "' file.";
  }
}
void EcalDccWeightBuilder::writeWeightToRootFile() {
  string fName = !rootOutputFileName_.empty() ? rootOutputFileName_.c_str()
                                              : "dccWeights.root";
  ...
  const int nWeightMax = 20; // normally n_weights = 6. A different might be
                             // used
  ...
  for (map<DetId, std::vector<int32_t>>::const_iterator it =
    ...
    if (buf.n_weights > nWeightMax) {
      throw cms::Exception("EcalDccWeight")
          << "Number of weights (" << buf.n_weights << ") for DetId "
          << buf.detId << " exceeded maximum limit (" << nWeightMax
...
void EcalDccWeightBuilder::writeWeightToDB() {
  cout << "going to write to the online DB " << dbSid_ << " user " << dbUser_
       << endl;
  ;
