...
HcalRecHitRecalib::HcalRecHitRecalib(const edm::ParameterSet &iConfig) {
  ...
  //   HBHEHitsProducer_ = iConfig.getParameter< std::string >
  //   ("HBHERecHitsProducer"); HOHitsProducer_ = iConfig.getParameter<
  //   std::string > ("HERecHitsProducer"); HFHitsProducer_ =
  //   iConfig.getParameter< std::string > ("HERecHitsProducer"); HBHEHits_ =
  //   iConfig.getParameter< std::string > ("HBHEHitCollection"); HFHits_ =
  //   iConfig.getParameter< std::string > ("HFHitCollection"); HOHits_ =
  //   iConfig.getParameter< std::string > ("HOHitCollection");

  RecalibHBHEHits_ =
      iConfig.getParameter<std::string>("RecalibHBHEHitCollection");
  ...
  hcalfileinpath_ =
      iConfig.getUntrackedParameter<std::string>("fileNameHcal", "");
  edm::FileInPath hcalfiletmp("CalibCalorimetry/CaloMiscalibTools/data/" +
                              hcalfileinpath_);
...
void HcalRecHitRecalib::produce(edm::Event &iEvent,
  ...
  if (HBHERecHits) {
    ...
    for (itHBHE = HBHERecHits->begin(); itHBHE != HBHERecHits->end();
      ...
      //	EcalIntercalibConstants::EcalIntercalibConstantMap::const_iterator
      // icalit=ical->getMap().find(itb->id().rawId());
      //	EcalIntercalibConstants::EcalIntercalibConstant icalconst;

      //	if( icalit!=ical->getMap().end() ){
      //	  icalconst = icalit->second;
      // edm::LogDebug("EcalRecHitMiscalib") << "Found intercalib for xtal " <<
      // EBDetId(itb->id()) << " " << icalconst ;

      //	} else {
      //	  edm::LogError("EcalRecHitMiscalib") << "No intercalib const
      // found for xtal " << EBDetId(itb->id()) << "! something wrong with
      // EcalIntercalibConstants in your DB? "
      //              ;
      //          }
      ...
      HBHERecHit aHit(itHBHE->id(), itHBHE->energy() * icalconst,
                      itHBHE->time());
