...
EcalRecHitRecalib::EcalRecHitRecalib(const edm::ParameterSet &iConfig) {
  ...
  RecalibBarrelHits_ =
      iConfig.getParameter<std::string>("RecalibBarrelHitCollection");
  RecalibEndcapHits_ =
      iConfig.getParameter<std::string>("RecalibEndcapHitCollection");
  refactor_ = iConfig.getUntrackedParameter<double>("Refactor", (double)1);
  refactor_mean_ =
      iConfig.getUntrackedParameter<double>("Refactor_mean", (double)1);
...
void EcalRecHitRecalib::produce(edm::Event &iEvent,
  ...
  if (!barrelRecHitsHandle.isValid()) {
    LogDebug("") << "EcalREcHitMiscalib: Error! can't get product!"
                 << std::endl;
  } else {
  ...
  if (EBRecHits) {
    ...
    for (itb = EBRecHits->begin(); itb != EBRecHits->end(); ++itb) {
      ...
      EcalIntercalibConstantMap::const_iterator icalit =
          ical->getMap().find(itb->id().rawId());
      EcalIntercalibConstant icalconst = -1;

      if (icalit != ical->getMap().end()) {
        icalconst = (*icalit);
        // edm::LogDebug("EcalRecHitRecalib") << "Found intercalib for xtal " <<
        // EBDetId(itb->id()) << " " << icalconst ;

      } else {
        edm::LogError("EcalRecHitRecalib")
            << "No intercalib const found for xtal " << EBDetId(itb->id())
      ...
      icalconst =
          ...
          (icalconst - refactor_mean_) *
              refactor_; // apply additional scaling factor (works if gaussian)
  ...
  if (EERecHits) {
    ...
    for (ite = EERecHits->begin(); ite != EERecHits->end(); ++ite) {
      ...
      // find intercalib constant for this xtal
      EcalIntercalibConstantMap::const_iterator icalit =
          ical->getMap().find(ite->id().rawId());
      EcalIntercalibConstant icalconst = -1;

      if (icalit != ical->getMap().end()) {
        icalconst = (*icalit);
        // edm:: LogDebug("EcalRecHitRecalib") << "Found intercalib for xtal "
        // << EEDetId(ite->id()) << " " << icalconst ;
      } else {
        edm::LogError("EcalRecHitRecalib")
            << "No intercalib const found for xtal " << EEDetId(ite->id())
            << "! something wrong with EcalIntercalibConstants in your DB? ";
      }
