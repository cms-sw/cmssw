...
DetId MiscalibReaderFromXMLHcal::parseCellEntry(
...
HcalDetId MiscalibReaderFromXMLHcal::getCellFromAttributes(int idet, int ieta,
                                                           int iphi,
                                                           int idepth) {
  try {
    ...
    std::cout << "Null coordinates = " << idet << "," << ieta << "," << iphi
              << "," << idepth << std::endl;
    return HcalDetId(0);
  }
}
