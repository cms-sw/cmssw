...
int MiscalibReaderFromXML::s_numberOfInstances =
    0; // to check that there is only 1 instance
...
int MiscalibReaderFromXML::getIntAttribute(DOMNamedNodeMap *attribute,
                                           const std::string &attribute_name) {
  bool well_formed_string;
  int retval = MiscalibReaderFromXMLDomUtils::getIntAttribute(
      attribute, attribute_name, well_formed_string);
  if (!well_formed_string)
    std::cout << "MiscalibReaderFromXML::getIntAttribute PROBLEMS ...!!!"
              << std::endl;
...
double MiscalibReaderFromXML::getScalingFactor(
...
double
MiscalibReaderFromXML::getFloatAttribute(DOMNamedNodeMap *attribute,
                                         const std::string &attribute_name) {
  bool well_formed_string;
  double retval = MiscalibReaderFromXMLDomUtils::getFloatAttribute(
      attribute, attribute_name, well_formed_string);
  if (!well_formed_string)
    std::cout << "MiscalibReaderFromXML::getFloatAttribute PROBLEMS ...!!!"
              << std::endl;
...
bool MiscalibReaderFromXML::parseXMLMiscalibFile(std::string configFile) {
  ...
  unsigned int linkTagsNum =
      doc->getElementsByTagName(_toDOMS("Cell"))->getLength();
  // The following should be on LogInfo
  // std::cout << "Read number of Cells = " << linkTagsNum << std::endl;

  if (linkTagsNum == 0)
    std::cout << "Number of Cells in file is 0 - probably bad file format"
              << std::endl;
