      ...
      for (int iPhi = EBDetId::MIN_IPHI; iPhi <= EBDetId::MAX_IPHI; ++iPhi) {
        if (EBDetId::validDetId(iEta, iPhi)) {
          ...
          EcalIntercalibConstantMap::const_iterator icalit =
              map_.find(ebdetid.rawId());
          EcalIntercalibConstant icalconst;
          icalconst = (*icalit);

          icount++;
          if (icount % 230 == 0) {
            std::cout << "here is value for chan eta/phi " << iEta << "/"
                      << iPhi << "=" << icalconst << std::endl;
          }
        }
      }
    }
    for (int iX = EEDetId::IX_MIN; iX <= EEDetId::IX_MAX; ++iX) {
      for (int iY = EEDetId::IY_MIN; iY <= EEDetId::IY_MAX; ++iY) {
        if (EEDetId::validDetId(iX, iY, 1)) {
          EEDetId eedetidpos(iX, iY, 1);
          EcalIntercalibConstantMap::const_iterator icalit =
              map_.find(eedetidpos.rawId());
          EcalIntercalibConstant icalconst;
          icalconst = (*icalit);

          EEDetId eedetidneg(iX, iY, -1);
          EcalIntercalibConstantMap::const_iterator icalit2 =
              map_.find(eedetidneg.rawId());
          ...
          if (icount % 230 == 0) {
            ...
            std::cout << "here is value for chan x/y " << iX << "/" << iY
                      << " pos side is =" << icalconst
