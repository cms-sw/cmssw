...
WatcherStreamFileReader::WatcherStreamFileReader(edm::ParameterSet const &pset)
    : inputDir_(pset.getParameter<std::string>("inputDir")),
      filePatterns_(
          pset.getParameter<std::vector<std::string>>("filePatterns")),
      ...
      tokenFile_(pset.getUntrackedParameter<std::string>("tokenFile",
                                                         "watcherSourceToken")),
      timeOut_(pset.getParameter<int>("timeOutInSec")), end_(false),
  ...
  for (unsigned i = 0; i < dirs.size(); ++i) {
    ...
    if (0 == stat(dir.c_str(), &fileStat)) {
      ...
      if (0 != mkdir(dir.c_str(), 0755)) {
        throw cms::Exception("[WatcherSource]")
            << "Failed to create directory " << dir << " for writing data.";
      }
    }
  }
  ...
  fileListCmdBuf << "/bin/find " << inputDir_
  ...
  for (unsigned i = 0; i < filePatterns_.size(); ++i) {
    if (i > 0)
      fileListCmdBuf << "|";
    //     if(filePatterns_[i].size()>0 && filePatterns_[0] != "/"){//relative
    //     path
      ...
      if (filesInQueue_.empty()) {
        if (!waiting) {
          ...
          if ((timeOut_ >= 0) && (dt > timeOut_)) {
            ...
            // remove(tokenFile_.c_str()); //we do not delete the token,
            // otherwise sorting process on the monitoring farm will not be
            // restarted by the runloop.sh script.
            end_ = true;
            break;
          }
        }
    ...
    while (streamerInputFile_.get() == nullptr && !filesInQueue_.empty()) {
      ...
      if (fd != 0) {
        ...
        cout << "[WatcherSource " << now() << "]"
             ...
             << flush;
        streamerInputFile_ = unique_ptr<edm::StreamerInputFile>(
            new edm::StreamerInputFile(fileName_));
