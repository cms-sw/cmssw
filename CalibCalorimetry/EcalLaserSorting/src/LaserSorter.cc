...
LaserSorter::LaserSorter(const edm::ParameterSet &pset)
    : lumiBlock_(0), lumiBlockPrev_(0), formatVersion_(5),
      ...
      orbit_(-1), orbitZeroTime_(nullTime) {
...
LaserSorter::~LaserSorter() {
  logFile_ << "Summary. Event count: " << stats_.nRead << " processed, "
           << stats_.nWritten << " written, " << stats_.nInvalidDccStrict
           << " with errors in DCC ID values, " << stats_.nInvalidDccWeak
           << " with unusable DCC ID values, " << stats_.nRestoredDcc
...
void LaserSorter::analyze(const edm::Event &event, const edm::EventSetup &es) {
  ...
  // orbit number
  // FIXME: orbit from edm Event is currently wrong. Forcing to use of CMS orbit
  // until it is fixed. See
  // https://hypernews.cern.ch/HyperNews/CMS/get/commissioning/5343/2.html
#if 0
  ...
  //  std::cerr << "Orbit ID CMS, ECAL: " << orbit_ << "\t" <<
  //  getOrbitFromDcc(rawdata) << "\n";
  ...
  logFile_ << event.id().run() << "\t" << lumiBlock_ << "\t"
           << event.id().event() << "\t" << trigType << "\t" << triggeredFedId
           << "\t" << side;
  ...
  if (disableOutput_) {
    ...
    if (out != nullptr) {
      ...
      if (out->excludedOrbit().find(orbit_) == out->excludedOrbit().end()) {
        if (verbosity_ > 1)
          cout << "[LaserSorter " << now() << "] "
               << "Writing out event from FED " << triggeredFedId << " LB "
               << lumiBlock_ << " orbit " << orbit_ << "\n";
        int dtt = (detailedTrigType_ >= 0)
                      ? detailedTrigType_
                      : -1; // shall we use -1 or 0 for undefined value?
        ...
        if (verbosity_)
          cout << "[LaserSorter " << now() << "] "
               ...
               << triggeredFedId << ", LB = " << lumiBlock_ << " with orbit ID "
...
int LaserSorter::getOrbitFromDcc(
    const edm::Handle<FEDRawDataCollection> &rawdata) const {
  const int orbit32 = 6;
...
int LaserSorter::getDetailedTriggerType(
    const edm::Handle<FEDRawDataCollection> &rawdata, double *proba) {
  Majority<int> stat;
  bool ecalData = false;
  for (int id = ecalDccFedIdMin_; id <= ecalDccFedIdMax_; ++id) {
    if (!FEDNumbering::inRange(id))
  ...
  if (p < 0) {
    ...
    edm::LogWarning("EventCorruption")
        << "Inconsitency in detailed trigger type indicated in ECAL DCC data "
           "headers\n";
    tType = -1;
  }
  if (proba)
...
LaserSorter::getStream(int fedId, edm::LuminosityBlockNumber_t lumiBlock) {
  ...
  if (verbosity_ > 1)
    cout << "[LaserSorter " << now() << "] "
         << "Looking for an opened output file for FED " << fedId << " LB "
         << lumiBlock << "\n";

  // first look if stream is already open:
  for (OutStreamList::iterator it = outStreamList_.begin();
       it != outStreamList_.end(); ++it) {
    if (it->fedId() == fedId && (abs((int)it->startingLumiBlock() -
                                     (int)lumiBlock) <= lumiBlockSpan_)) {
      // stream found!
      return &(*it);
    }
  }
...
bool LaserSorter::writeEvent(OutStreamRecord &outRcd, const edm::Event &event,
                             int dtt, const FEDRawDataCollection &data) {
  ...
  if (orbitZeroTime_.tv_sec == 0 && data.FEDData(matacqFedId_).size() != 0) {
    ...
    MatacqRawEvent mre(data.FEDData(matacqFedId_).data(),
                       data.FEDData(matacqFedId_).size());
    mre.getTimeStamp(ts);
    uint32_t orb = mre.getOrbitId();
    if (ts.tv_sec != 0) {
      div_t dt =
          div(orb * 89.1, 1000 * 1000); // an orbit lasts 89.1 microseconds
  ...
  for (unsigned iFed = 0; iFed < fedIds.size() && rc; ++iFed) {
    if (verbosity_ > 3)
      cout << "[LaserSorter " << now() << "] "
           ...
           << ". Data size: " << data.FEDData(fedIds[iFed]).size() << "\n";
...
LaserSorter::createOutStream(int fedId,
  ...
  if (out->is_open()) { // temporary file already exists. Making a backup:
    ...
    if (!renameAsBackup(tmpName, newName)) {
      throw cms::Exception("LaserSorter")
          << "Failed to rename file " << tmpName << "  to " << newName << "\n";
    }
    if (verbosity_)
      cout << "[LaserSorter " << now() << "] "
  ...
  if (out->fail()) { // failed to create file
    delete out;
    throw cms::Exception("LaserSorter")
        << "Failed to create file " << tmpName << " for writing event from FED "
  ...
  if (in.good()) { // file already exists with final name.
    ...
    if (vers == -1) {
        ...
        if (verbosity_ > 2)
          cout << "[LaserSorter " << now() << "] "
               << "Index table offset of "
                  "original file "
               << finalName << ": 0x" << hex << setfill('0') << setw(8)
               << indexTableOffsetValue << dec << setfill(' ') << "\n";
      }
      in.clear();
      in.seekg(0, ios::beg);
      ...
      while (!in.eof() && (toRead > 0) &&
             (nread = in.readsome(
                  buffer, min(toRead, (uint32_t)sizeof(buffer)))) != 0) {
  ...
  OutStreamRecord *outRcd =
      new OutStreamRecord(fedId, lumiBlock, out, tmpName, finalName);
...
bool LaserSorter::writeEventHeader(std::ofstream &out, const edm::Event &evt,
                                   int dtt, unsigned nFeds) {
  uint32_t data[10];
  timeval tt = {0, 0};
  if ((evt.time().value() >> 32)) {
    ...
    div_t dt =
        div(orbit_ * 89.1, 1000 * 1000); // one orbit lasts 89.1 microseconds
  ...
  if (verbosity_ > 1) {
    cout << "[LaserSorter " << now() << "] "
         ...
         << ", LB: " << evt.luminosityBlock() << ", Run: " << evt.run()
         << ", Bx: " << evt.bunchCrossing()
         << ", Event ID: " << evt.id().event() << ", Detailed trigger type: 0x"
         << hex << dtt << dec << " (" << detailedTrigNames[(dtt >> 8) & 0x7]
...
void LaserSorter::streamFileName(int fedId,
  ...
  buf.str("");
  buf << "Run" << runNumber_ << "_LB" << setfill('0') << setw(4) << lumiBlock
      << ".lmf";
...
LaserSorter::OutStreamList::iterator
LaserSorter::closeOutStream(LaserSorter::OutStreamList::iterator streamRecord) {
  ...
  if (0 != rename(tmpFileName.c_str(), finalFileName.c_str())) {
    cout << "[LaserSorter " << now() << "] "
         << " Failed to rename output file from " << tmpFileName << " to "
         << finalFileName << ". " << strerror(errno) << "\n";
  }
...
bool LaserSorter::isDccEventEmpty(const FEDRawData &data, size_t *dccLen,
  ...
  if (nWord32 == 0) {
    // cout << "[LaserSorter " << now() << "] " << "FED block completly
    // empty\n";
    return true;
  }
...
void LaserSorter::getOutputFedList(const edm::Event &event,
  ...
  for (int id = ecalDccFedIdMin_; id <= ecalDccFedIdMax_; ++id) {
    ...
    if (dccLen * sizeof(uint64_t) != dccEvent.size()) {
      edm::LogWarning("LaserSorter")
          << "Length error in data of FED " << id << " in event " << event.id()
...
bool LaserSorter::readIndexTable(std::ifstream &in, std::string &inName,
  ...
  if (nevts > maxEvents_) {
    errMsg << "Number of events indicated in event index of file " << inName
           << " (" << nevts << ") "
...
int LaserSorter::readFormatVersion(std::ifstream &in,
  ...
  if (lmf != magic) {
    edm::LogWarning("LaserSorter")
        << "File " << fileName << "is not an LMF file.\n";
  }
