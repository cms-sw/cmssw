...
LmfSource::LmfSource(const ParameterSet &pset,
    ...
    : ProducerSourceBase(pset, desc, true),
      fileNames_(pset.getParameter<vector<string>>("fileNames")), iFile_(-1),
      fedId_(-1), fileHeader_(fileHeaderSize), dataFormatVers_(5),
      rcRead_(false), preScale_(pset.getParameter<unsigned>("preScale")),
      iEvent_(0), iEventInFile_(0), indexTablePos_(0),
...
bool LmfSource::readFileHeader() {
  ...
  if (verbosity_)
    cout << "[LmfSource] File position of index table: 0x" << setfill('0')
         << hex << setw(8) << indexTablePos_ << setfill(' ') << dec << "\n";
  ...
  if (orderedRead_) {
    ...
    if (verbosity_) {
      cout << "[LmfSource] move to event with orbit Id "
           << indexTable_[iEventInFile_].orbit << " at file position 0x" << hex
           << setfill('0') << setw(8) << indexTable_[iEventInFile_].filePos
           << setfill(' ') << dec << "\n";
    }
    ...
    in_.seekg(pos);
    if (in_.bad()) {
      cout << "[LmfSource] Problem while reading file " << currentFileName_
...
bool LmfSource::readEventWithinFile(bool doSkip) {
  ...
  int activeFedId = activeFedId32[iv] >= 0 ? header_[activeFedId32[iv]]
                                           : ((calibTrig_ & 0x3F) + 600);
  ...
  if (verbosity_) {
    ...
    cout << "[LmfSource] "
         ...
         << " (" << tbuf << " " << t_ms_us.quot << " ms " << t_ms_us.rem
         << " us)\n"
  ...
  for (int iFed = 0; iFed < nFeds_; ++iFed) {
    ...
    if (eventSize8 > maxEventSize_) {
      throw cms::Exception("FileReadError")
          ...
          << " data of file " << currentFileName_ << "is unexpctively large ("
          << (eventSize8 >> 10) << " kByte). "
      ...
      if (dataFormatVers_ <= 4) { // calib trigger in not in event header.
        ...
        if (verbosity_) {
          cout
              ...
              << hex << setfill('0') << setw(3) << calibTrig_ << setfill(' ')
              << dec << "\n";
        }
      }
    }
...
void LmfSource::checkFileNames() {
  for (unsigned i = 0; i < fileNames_.size(); ++i) {
    ...
    const char s[] = "file:";
    if (fileName.compare(0, sizeof(s) - 1, s) == 0) { // file: prefix => to
                                                      // strip
...
void LmfSource::readIndexTable() {
  ...
  stringstream errMsg;
  errMsg << "Error while reading event index table of file " << currentFileName_
         << ". Try to read it with "
