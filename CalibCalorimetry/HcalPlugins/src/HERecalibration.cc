...
void HERecalibration::setDsegm( const std::vector<std::vector<int>>& m_segmentation) 
{
  ...
  for (int ieta = 0; ieta < nEtaBins_HEDarkening; ieta++) {
    //    std::cout << "["<< ieta << "]  ieta =" << ieta + 16 << "  ";
    for(int ilay = 0; ilay < nScintLayers_HEDarkening; ilay++) {
...
void HERecalibration::initialize() {
  ...
  double dval[nEtaBins_HEDarkening][nDepths_HERecalibration];  // conversion of lval into depths-averaged values - denominator (including degradation for iLumi) 
  double nval[nEtaBins_HEDarkening][nDepths_HERecalibration];  // conversion of lval into depths-averaged values - numerator (no degradation)

  for (int j = 0; j < nEtaBins_HEDarkening; j++) {
    for (int k = 0; k < nDepths_HERecalibration; k++) {
      dval[j][k] = 0.0;
      nval[j][k] = 0.0;
    }
  }

  double lval[nEtaBins_HEDarkening][nScintLayers_HEDarkening]    // raw table of mean energy in each layer for each ieta at 0 lumi
    = {
  ...
  for (int ieta = 0; ieta < nEtaBins_HEDarkening; ieta++) {
    ...
    for(int idepth = 0; idepth < nDepths_HERecalibration; idepth++){
