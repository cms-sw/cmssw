...
void HERecalibration::setDsegm( const std::vector<std::vector<int>>& m_segmentation) 
{
  ...
  for (int ieta = 0; ieta < nEtaBins; ieta++) {
    ...
    for(int ilay = 0; ilay < nScintLayers; ilay++) {
...
void HERecalibration::initialize() {
  ...
  double dval[nEtaBins][nDepths];  // conversion of lval into depths-averaged values - denominator (including degradation for iLumi) 
  double nval[nEtaBins][nDepths];  // conversion of lval into depths-averaged values - numerator (no degradation)
  ...
  double lval[nEtaBins][nScintLayers]    // raw table of mean energy in each layer for each ieta at 0 lumi
    = {
  ...
  for (int ieta = 0; ieta < nEtaBins; ieta++) {
    ...
    for(int idepth = 0; idepth < nDepths; idepth++){
