...
std::shared_ptr<HcalDbService> HcalDbProducer::produce(const HcalDbRecord& record) {
  ...
  setupHcalDbService<HcalMCParams, HcalMCParamsRcd>(*host, record, "withTopo", "MCParams", "New HCAL MCParams set");
  ...
  setupHcalDbService<HcalQIETypes, HcalQIETypesRcd>(*host, record, "withTopo", "QIETypes", "New HCAL QIETypes set");

  setupHcalDbService<HcalQIEData, HcalQIEDataRcd>(*host, record, "withTopo", "QIEData", "New HCAL QIEData set");

  setupHcalDbService<HcalTimeCorrs, HcalTimeCorrsRcd>(*host, record, "withTopo", "TimeCorrs", "New HCAL TimeCorrs set");

  setupHcalDbService<HcalPFCorrs, HcalPFCorrsRcd>(*host, record, "withTopo", "PFCorrs", "New HCAL PFCorrs set");

  setupHcalDbService<HcalLUTCorrs, HcalLUTCorrsRcd>(*host, record, "withTopo", "LUTCorrs", "New HCAL LUTCorrs set");
...
std::unique_ptr<HcalPedestalWidths> HcalDbProducer::produceEffectivePedestalWidthsWithTopo(
    const HcalPedestalWidthsRcd& record) {
...
std::unique_ptr<HcalL1TriggerObjects> HcalDbProducer::produceL1triggerObjectsWithTopo(
    const HcalL1TriggerObjectsRcd& record) {
...
std::unique_ptr<HcalTPChannelParameters> HcalDbProducer::produceTPChannelParametersWithTopo(
    const HcalTPChannelParametersRcd& record) {
...
void HcalDbProducer::setupPedestals(const HcalPedestalsRcd& record, HcalDbService& service) {
...
void HcalDbProducer::setupEffectivePedestals(const HcalPedestalsRcd& record, HcalDbService& service) {
  edm::ESHandle<HcalPedestals> item;
...
void HcalDbProducer::setupPedestalWidths(const HcalPedestalWidthsRcd& record, HcalDbService& service) {
...
void HcalDbProducer::setupEffectivePedestalWidths(const HcalPedestalWidthsRcd& record, HcalDbService& service) {
  edm::ESHandle<HcalPedestalWidths> item;
  record.get("withTopoEff", item);
  service.setData(item.product(), true);

  if (std::find(mDumpRequest.begin(), mDumpRequest.end(), std::string("EffectivePedestalWidths")) !=
      mDumpRequest.end()) {
