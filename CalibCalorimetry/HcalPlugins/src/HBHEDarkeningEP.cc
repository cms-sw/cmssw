...
void HBHEDarkeningEP::setIntervalFor(const edm::eventsetup::EventSetupRecordKey& iKey, const edm::IOVSyncValue& iTime, edm::ValidityInterval& oInterval) {
...
HBHEDarkeningEP::produce(const HBHEDarkeningRecord& iRecord) {
	...
	for(const auto& p_year : p_years){
		years.emplace_back(
			p_year.getParameter<std::string>("year"),
