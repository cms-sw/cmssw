  ...
  toTime_(ps.getParameter<int>("toTime"))
{
  ...
  if(eventListFileName_.size()!=0){
    ...
    if(eventList_==0) throw cms::Exception("User") << "Failed to open file " << eventListFileName_ << "\n";
  }
}


EcalLaserCondTools::~EcalLaserCondTools(){
  if(ferr_) fclose(ferr_); 
  if(fout_) fclose(fout_);
}

void
EcalLaserCondTools::analyze(const edm::Event& event, const edm::EventSetup& es){
  static bool done = false;
  
  if(mode_ == "ascii_file_to_db"){
    if(done) return;
    if(verb_>2) cout << "ascii_file_to_db mode" << endl;
    if( !db_.isAvailable() ){
...
void EcalLaserCondTools::fillDb(CorrReader& r){
  ...
  if(verb_>2) cout << "Starting filling DB...\n";
  int t1 = 0;
  int t3 = 0;
  int t2[nLmes];
  ...
  if(eventList_){
    ...
    while(!feof(eventList_)){
      ...
      if(verb_>1) cout << "Event time: t = " << t << ", " << timeToString(t) << "\n";
      ++iline;
      if(n!=1) throw cms::Exception("User") << "Syntax error in event list file " << eventListFileName_ << " at line " << iline
                                            << " " << n << " "<< ".\n";
      if(t < prevT ) throw cms::Exception("User") << "Events in the event list file " << eventListFileName_
                                                  << " are not ordered in increased time as required! See line "
                                                  << iline << "\n";
      if(t == 0) throw cms::Exception("User") << "Found an unexpected t = 0 time value "
        "in the event list file" << eventListFileName_ << " at line " << iline << "!\n";
      //Look for IOV:
      bool iovFound = true;
      if(t <= t3){//IOV already inserted for previous event.
        if(verb_ > 1) cout << "Event in same IOV than previous one.\n";
        continue; 
      }
      ...
      if(iovFound){
        if(t1 < prevT1 && t3 < prevT3) throw cms::Exception("User") << "IOVs in the correction ascii file are not ordered in increased time as required!\n";
        else if(t1 < prevT1 || t3 < prevT3) throw cms::Exception("User") << "Found interleaved IOVs in the correction ascii file!\n"; 
        processIov(r, t1, t2, t3);
      } else{
        cout << "Warning: event beyond last IOV t3. Event time: " << timeToString(t) << ". Last IOV t3: " << timeToString(t3) << "\n";
      }
    }
  } else while(r.readTime(t1, t2, t3)){
    ++iIov;
    if(iIov <= skipIov_) { 
...
void EcalLaserCondTools::processIov(CorrReader& r, int t1, int t2[92], int t3){
  static int iIov = 0;
  ++iIov;
...
//  if(iIov <= skipIov_) { std::cout << "Skipping IOV " << iIov << "\n"; return; }
  ...
  if(verb_>1){
    cout << "t1:" << t1 << "(" << timeToString(t1) << ") \n"
        ...
        for(int i = 0; i < 92; ++i) cout << t2[i]-t1 << "\t";
        cout << "\n";
  }
  if(t1 < fromTime_) { std::cout << "Skipping IOV " << iIov << ", " << ", which is before 'fromTime',"
                                 << timeToString(fromTime_) << "(" << fromTime_ << ").\n"; return; }

//   if(toTime_!=-1 && t2 < toTime_) { std::cout << "Skipping IOV " << iIov << ", " << ", which is beyond 'toTime',"
//                                               << timeToString(toTime_) << "(" << toTime_ << ").\n"; return; }

  if(t1==0) { std::cout << "Skipping IOV with t1 = 0" << "\n"; return; }
  ...
  for(int i = 0; i < ncrystals; ++i){
    ...
    if(verb_>2) cout << "Reading " << toNth(i+1) << " crystal\n";
    if(!r.readPs(detid, corr)){
      throw cms::Exception("LasCor") << "Failed to read "
                                     << toNth(i+1)
                                     << " crystal correction.\n";
    }
    ...
    if(!res.second){//detid already processed
      std::cout << "Duplicate det id, for IOV " << iIov << " t1 = " << t1
                << " detid = " << int(detid) << endl;
    }
    
//     static const float minp = 0.9;
//     static const float maxp = 1.1;
     static const float minp = 0.00001;
     static const float maxp = 1.8;
     //static const float minp = -100;
     //static const float maxp = 100;
    
    if(!isfinite(corr.p1)
       || !isfinite(corr.p2)
       || !isfinite(corr.p3)
       || corr.p1<minp || corr.p1>maxp
       || corr.p2<minp || corr.p2>maxp
       || corr.p3<minp || corr.p3>maxp){
    ...
    if (detid.subdetId() == EcalBarrel) {
            if (corr.p1 < 0.55) corr.p1 = .985;
            if (corr.p2 < 0.55) corr.p2 = .985;
            if (corr.p3 < 0.55) corr.p3 = .985;
    }
  ...
  try{
    ...
    if(db_->isNewTagRequest("EcalLaserAPDPNRatiosRcd")){
      if(verb_) cout << "First IOV, extending starting time." << endl;
    iovStart = db_->beginOfTime();
    }
    timeval t;
    gettimeofday(&t, 0);
    if(verb_>1) cout << "[" << timeToString(t.tv_sec) << "] " << "Write IOV "
                     << iIov << " starting from " << timeToString(iovStart >>32) << "... ";
  ...
  for(;;){
    ...
    if(ifile_ >= fnames_.size()){
      if(verb_>1) cout << "No more correction files.\n";
      return false;
    }
    if(verb_>1) cout << "Opening file " << fnames_[ifile_] << "\n";
...
bool EcalLaserCondTools::FileReader::readTime(int& t1, int t2[92], int& t3){
  trim();
  if((f_ == 0 || feof(f_)) && !nextFile()){
    if(verb_>1) cout << "No more record\n"; 
    return false;
  }
  int i;
  char* buf = 0;
  size_t s = 0;
  while((i = fgetc(f_)) != 'T' && i != 'L' && i >=0) getline(&buf, &s, f_);
  if(buf) free(buf); 
  buf = 0;

  if(i=='L'){ //last record put 3 consecutive times starting from end of prev. IOV
    t1 = t3;
    for(int i = 0; i < 92; ++i) t2[i] = t1 + 1;
    t3 = t1 + 2;
    return true;
  }

  if(i!='T'){
    if(verb_>1) cout << "No more record or bad line type/marker (getc returned " << i <<")\n"; 
    return false;
  }
  ...
  if(n != (2+92) || nnn!=0) throw cms::Exception("LasCorFile") << "File " << fnames_[ifile_]
                                                               << " line " << iline_
                                                               << ": syntax error. Expecting 'T' marker followed by 94 values: "
    "t1 t2 t3(lme 1) t3(lme 2) ... t3(lme 92)\n";
...
bool EcalLaserCondTools::FileReader::readPs(DetId& detid, EcalLaserAPDPNRatios::EcalLaserAPDPNpair& corr){
  if(f_==0){
    if(verb_) cout << "Requested to read p1..p3 parameter line while no file is closed.\n";
    return false;
  }
  ...
  if(i!='P'){
    if(verb_ && i >= 0) cout  << "File " << fnames_[ifile_]
                              << " line " << iline_
...
//    corr.p2=corr.p1;
//    corr.p3=corr.p1;
    throw cms::Exception("I/O") << "Syntax error at line "
                                << iline_
                                << "of file "
                                << fnames_[ifile_]
                                << " read " << n << " values,"
  ...
  detid = rawdetid;
  const int ECALID = 3;
  if(detid.det()!=ECALID) throw cms::Exception("InvalidValue") << "Line "
                                                               << iline_
                                                               << "of file "
                                                               << fnames_[ifile_]
                                                               << " contains an invalid det ID (detector code is not ECAL!)\n";    
  if(detid.subdetId()==EcalBarrel) EBDetId(detid); //in order to throw an exception if detid is not valid
  if(detid.subdetId()==EcalEndcap) EEDetId(detid); //in order to throw an exception if detid is not valid
  ++iline_;
  return true;
}
...
void EcalLaserCondTools::dbToAscii(const edm::EventSetup& es){  
  ...
  if(fout_==0){
    ...
    if(fout_==0) throw cms::Exception("LasCor") << "Failed to create file corr_dump.txt\n";
  }
  ...
  if(verb_) cout << "Processing IOV " << t1 << " - " << t3
  ...
  for(unsigned i = 0; i < t.size(); ++i){
    if(t[i].t1.unixTime() !=t1 || t[i].t3.unixTime() != t3){
      throw cms::Exception("LasCor") << "Inconsitency in t1, t3: "
                                     << "t1(lme 1) =" << t1
                                     << ", t1(lme " << (i+1) << ") = " << t[i].t1.unixTime()
                                     << ", t3(lme 1) =" << t3
                                     << ", t3(lme " << (i+1) << ") = " << t[i].t3.unixTime() << "\n";
    }
