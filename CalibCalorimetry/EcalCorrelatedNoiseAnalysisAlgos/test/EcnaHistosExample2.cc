...
#define HGLO
      ...
      fMyRootFileEB->FileParameters(fKeyAnaType,    fKeyNbOfSamples,
				    ...
				    fKeySuMoNumber, fEcnaParPathsEB->ResultsRootFilePath().Data());
      ...
      if( fMyRootFileEB->LookAtRootFile() == kTRUE )
	{
	  ...
	  MyHistosEB->SetHistoMax(3.5);  MyHistosEB->SetHistoScaleY("LOG");
      ...
      if( xAlreadyRead == 1 )
	{
	  ...
	  if( fMyRootFileEB->LookAtRootFile() == kTRUE )
	    { 
	      ...
	      read_matrix_sample = fMyRootFileEB->ReadMatrix(fEcalParEB->MaxSampADC(),
							     "cor", "samples", SMtower, TowEcha);
	  ...
	  if( fMyRootFileEB->LookAtRootFile() == kTRUE )
	    {
	      ...
	      MyHistosEB->SetHistoMax(3.5);  MyHistosEB->SetHistoScaleY("LIN");
	      MyHistosEB->NewCanvas("SAME n");
