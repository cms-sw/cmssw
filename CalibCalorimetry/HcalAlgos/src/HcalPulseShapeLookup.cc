...
void HcalPulseShapeLookup::dumpToTxt(const std::string& filename, const unsigned precision) const {
  ...
  const unsigned nShapeTypes = shapeTypes_.size();
  const unsigned nChannelsMapped = nShapeTypes - std::count(shapeTypes_.begin(), shapeTypes_.end(), -1);
  ...
  for (unsigned densId = 0; densId < nShapeTypes; ++densId)
