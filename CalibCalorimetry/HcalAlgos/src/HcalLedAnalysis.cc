...
HcalLedAnalysis::~HcalLedAnalysis(){
  ...
  for(_meol=hbHists.LEDTRENDS.begin(); _meol!=hbHists.LEDTRENDS.end(); _meol++){
    for(int i=0; i<4; i++) _meol->second[i].first->Delete();
  }
...
void HcalLedAnalysis::GetLedConst(map<HcalDetId, map<int,LEDBUNCH> > &toolT){
  double param2=0; double param1=0; double param3=0; double param4=0;
  double dparam2=0; double dparam1=0; double dparam3=0; double dparam4=0;
...
// scale the LED pulse to 1 event
    ...
    if(fitflag==0 || fitflag==4){
      param1 = _meol->second[10].first->GetMean();
      dparam1 = _meol->second[10].first->GetRMS()/sqrt((float)evt_curr*(m_endTS-m_startTS+1));
    }
...
// put proper errors
    ...
    if(fitflag==1 || fitflag==3 || fitflag==4){
      ...
      param2=fit->GetParameter(1);
      dparam2=fit->GetParError(1);
    }
    if(fitflag==2 || fitflag==4){
      param3 = _meol->second[12].first->GetMean();
      dparam3 = _meol->second[12].first->GetRMS()/sqrt((float)_meol->second[12].first->GetEntries());
    }
    if(fitflag==3 || fitflag==4){
      param4 = _meol->second[13].first->GetMean();
      dparam4 = _meol->second[13].first->GetRMS()/sqrt((float)_meol->second[13].first->GetEntries());
    }
    for (int i=0; i<10; i++){
...
//      if(m_hiSaveflag>0)_meol->second[i].first->Write();
    ...
    _meol->second[10].second.first[0].push_back(param1);
    _meol->second[10].second.first[1].push_back(dparam1);
    _meol->second[11].second.first[0].push_back(param2);
    _meol->second[11].second.first[1].push_back(dparam2);
    ...
    _meol->second[12].second.first[0].push_back(param3);
    _meol->second[12].second.first[1].push_back(dparam3);
    ...
    _meol->second[13].second.first[0].push_back(param4);
    _meol->second[13].second.first[1].push_back(dparam4);
// Ascii printout
    HcalDetId detid = _meol->first;
    if (m_outputFileText!=""){
      if(fitflag==0) m_outFile<<detid<<"   "<<param1<<" "<<dparam1<<std::endl;
      else if(fitflag==1) m_outFile<<detid<<"   "<<param2<<" "<<dparam2<<std::endl;
      else if(fitflag==2) m_outFile<<detid<<"   "<<param3<<" "<<dparam3<<std::endl;
      else if(fitflag==3) m_outFile<<detid<<"   "<<param4<<" "<<dparam4<<std::endl;
      else if(fitflag==4) m_outFile<<detid<<"   "<<param1<<" "<<dparam1<<"   "<<param2<<" "<<dparam2<<"   "<<param3<<" "<<dparam3<<"   "<<param4<<" "<<dparam4<<std::endl;
    }
  }
}
...
// here we loop over time slices
      for (int i=m_startTS; i<digi.size() && i<=m_endTS; i++) {
        LedTSHists(0,digi.id(),i,digi.sample(i),hbHists.LEDTRENDS);
      }
    }
  }
  catch (int i ) {
//    m_logFile<< "Event with " << i<<" HBHE Digis passed." << std::endl;
  ...
  try{
    ...
    for (HODigiCollection::const_iterator j=ho.begin(); j!=ho.end(); j++){
      ...
      m_coder = cond.getHcalCoder(digi.id());
      for (int i=m_startTS; i<digi.size() && i<=m_endTS; i++) {	   
        LedTSHists(1,digi.id(),i,digi.sample(i),hoHists.LEDTRENDS);
      }
    }        
  } 
  catch (int i ) {
//    m_logFile << "Event with " << i<<" HO Digis passed." << std::endl;
  ...
  try{
    ...
    for (HFDigiCollection::const_iterator j=hf.begin(); j!=hf.end(); j++){
      ...
      m_coder = cond.getHcalCoder(digi.id());
      for (int i=m_startTS; i<digi.size() && i<=m_endTS; i++) {
        LedTSHists(2,digi.id(),i,digi.sample(i),hfHists.LEDTRENDS);
      }
    }
  } 
  catch (int i ) {
...
//-----------------------------------------------------------------------------
void HcalLedAnalysis::LedTSHists(int id, const HcalDetId detid, int TS, const HcalQIESample& qie1, map<HcalDetId, map<int,LEDBUNCH> > &toolT) {
...
// full range (and is the same for all channels and cap-ids)
  static const float adc2fc[128]={-0.5, 0.5, 1.5, 2.5, 3.5, 4.5, 5.5, 6.5, 7.5, 8.5, 9.5, 10.5, 11.5, 12.5, 13.5, 15., 17., 19., 21., 23., 25., 27., 29.5, 32.5, 35.5, 38.5, 42., 46., 50., 54.5, 59.5, 64.5, 59.5, 64.5, 69.5, 74.5, 79.5, 84.5, 89.5, 94.5, 99.5, 104.5, 109.5, 114.5, 119.5, 124.5, 129.5, 137., 147., 157., 167., 177., 187., 197., 209.5, 224.5, 239.5, 254.5, 272., 292., 312., 334.5, 359.5, 384.5, 359.5, 384.5, 409.5, 434.5, 459.5, 484.5, 509.5, 534.5, 559.5, 584.5, 609.5, 634.5, 659.5, 684.5, 709.5, 747., 797., 847., 897., 947., 997., 1047., 1109.5, 1184.5, 1259.5, 1334.5, 1422., 1522., 1622., 1734.5, 1859.5, 1984.5, 1859.5, 1984.5, 2109.5, 2234.5, 2359.5, 2484.5, 2609.5, 2734.5, 2859.5, 2984.5, 3109.5, 3234.5, 3359.5, 3484.5, 3609.5, 3797., 4047., 4297., 4547., 4797., 5047., 5297., 5609.5, 5984.5, 6359.5, 6734.5, 7172., 7672., 8172., 8734.5, 9359.5, 9984.5};
...
// if histos for this channel already exist, just fill them
    ...
    if((evt-1)%m_nevtsample==0 && state[0]){
      ...
      for(int i=0; i<11; i++) _mei[i].first->Reset();
    }
    _mei[TS].first->Fill(adc2fc[qie1.adc()]);
    _mei[10].first->AddBinContent(TS+1,adc2fc[qie1.adc()]);
    if(fitflag>1){
      if(TS==m_startTS)_mei[11].first->Reset();
      _mei[11].first->SetBinContent(TS+1,adc2fc[qie1.adc()]);
      float fcgap;
...
// and then do all the same things - this is a bit silly as well
    ...
    insert[TS].first->Fill(adc2fc[qie1.adc()]);
    insert[10].first->AddBinContent(TS+1,adc2fc[qie1.adc()]);
    if(fitflag>1){
      insert[11].first->SetBinContent(TS+1,adc2fc[qie1.adc()]);
      float fcgap;
      if(qie1.adc()==0)fcgap=0.3*(adc2fc[1]-adc2fc[0]);
      else if(qie1.adc()==127)fcgap=0.3*(adc2fc[127]-adc2fc[126]);
      else fcgap=0.15*(adc2fc[qie1.adc()+1]-adc2fc[qie1.adc()-1]);
