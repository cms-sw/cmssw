...
HBHERecalibration::HBHERecalibration(float intlumi, float cutoff, std::string meanenergies)
    : intlumi_(intlumi),
      ...
      ieta_shift_(0),
      max_depth_(0),
      meanenergies_(HBHEDarkening::readDoseMap(meanenergies)),
      darkening_(nullptr) {}
...
float HBHERecalibration::getCorr(int ieta, int depth) const {
  ...
  //bounds check
  if (ieta < 0 or ieta >= int(corr_.size()))
    return 1.0;
  if (depth < 0 or depth >= int(corr_[ieta].size()))
    return 1.0;

  if (cutoff_ > 1 and corr_[ieta][depth] > cutoff_)
    return cutoff_;
  else
    return corr_[ieta][depth];
}

void HBHERecalibration::initialize() {
  std::vector<std::vector<float>> vtmp(dsegm_.size(), std::vector<float>(max_depth_, 0.0));
  auto dval =
      vtmp;  //conversion of meanenergies into depths-averaged values - denominator (including degradation for intlumi)
  ...
  for (unsigned int ieta = 0; ieta < dsegm_.size(); ++ieta) {
    ...
    for (unsigned int ilay = 0; ilay < std::min(meanenergies_[ieta].size(), dsegm_[ieta].size()); ++ilay) {
      ...
      dval[ieta][depth] +=
          meanenergies_[ieta][ilay] *
          darkening_->degradation(intlumi_, ieta + ieta_shift_, ilay + 1);  //be careful of eta and layer numbering
    }

    //compute factors, w/ safety checks
    for (int depth = 0; depth < max_depth_; ++depth) {
      if (dval[ieta][depth] > 0)
        corr_[ieta][depth] = nval[ieta][depth] / dval[ieta][depth];
      else
        corr_[ieta][depth] = 1.0;

      if (corr_[ieta][depth] < 1.0)
        corr_[ieta][depth] = 1.0;
    }
  }
}
