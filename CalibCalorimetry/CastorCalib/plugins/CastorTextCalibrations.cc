...
{
  // parsing parameters
  std::vector<edm::ParameterSet> data =
      iConfig.getParameter<std::vector<edm::ParameterSet>>("input");
  ...
  for (; request != data.end(); ++request) {
    ...
    if (objectName == "Pedestals") {
      ...
      std::cerr << "CastorTextCalibrations-> Unknown object name '"
                << objectName << "', known names are: "
                << "Pedestals PedestalWidths Gains GainWidths QIEData "
                   "ChannelQuality ElectronicsMap RecoParams SaturationCorrs"
                << std::endl;
    }
  }
...
// member functions
//
void CastorTextCalibrations::setIntervalFor(
  ...
  oInterval = edm::ValidityInterval(edm::IOVSyncValue::beginOfTime(),
                                    edm::IOVSyncValue::endOfTime()); // infinite
}

template <class T> std::unique_ptr<T> produce_impl(const std::string &fFile) {
  std::unique_ptr<T> result(new T());
  std::ifstream inStream(fFile.c_str());
  if (!inStream.good()) {
    std::cerr << "CastorTextCalibrations-> Unable to open file '" << fFile
              << "'" << std::endl;
    throw cms::Exception("FileNotFound")
        << "Unable to open '" << fFile << "'" << std::endl;
  }
  if (!CastorDbASCIIIO::getObject(inStream, *result)) {
    std::cerr << "CastorTextCalibrations-> Can not read object from file '"
              << fFile << "'" << std::endl;
    throw cms::Exception("ReadError")
        << "Can not read object from file '" << fFile << "'" << std::endl;
  }
  return result;
}

std::unique_ptr<CastorPedestals>
CastorTextCalibrations::producePedestals(const CastorPedestalsRcd &) {
  return produce_impl<CastorPedestals>(mInputs["Pedestals"]);
}

std::unique_ptr<CastorPedestalWidths>
CastorTextCalibrations::producePedestalWidths(const CastorPedestalWidthsRcd &) {
  return produce_impl<CastorPedestalWidths>(mInputs["PedestalWidths"]);
}

std::unique_ptr<CastorGains>
CastorTextCalibrations::produceGains(const CastorGainsRcd &) {
  return produce_impl<CastorGains>(mInputs["Gains"]);
}

std::unique_ptr<CastorGainWidths>
CastorTextCalibrations::produceGainWidths(const CastorGainWidthsRcd &) {
  return produce_impl<CastorGainWidths>(mInputs["GainWidths"]);
}

std::unique_ptr<CastorQIEData>
CastorTextCalibrations::produceQIEData(const CastorQIEDataRcd &rcd) {
  return produce_impl<CastorQIEData>(mInputs["QIEData"]);
}

std::unique_ptr<CastorChannelQuality>
...
std::unique_ptr<CastorElectronicsMap>
...
std::unique_ptr<CastorRecoParams>
CastorTextCalibrations::produceRecoParams(const CastorRecoParamsRcd &rcd) {
  return produce_impl<CastorRecoParams>(mInputs["RecoParams"]);
}

std::unique_ptr<CastorSaturationCorrs>
