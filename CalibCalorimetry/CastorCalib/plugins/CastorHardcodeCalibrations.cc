...

{
  ...
  std::vector<std::string> toGet =
      iConfig.getUntrackedParameter<std::vector<std::string>>("toGet");
  for (std::vector<std::string>::iterator objectName = toGet.begin();
    ...
    if ((*objectName == "ChannelQuality") ||
        (*objectName == "channelQuality") || all) {
      setWhatProduced(this, &CastorHardcodeCalibrations::produceChannelQuality);
      findingRecord<CastorChannelQualityRcd>();
    }
    if ((*objectName == "ElectronicsMap") ||
        (*objectName == "electronicsMap") || all) {
...
// member functions
//
void CastorHardcodeCalibrations::setIntervalFor(
  ...
  edm::LogInfo("HCAL") << "CastorHardcodeCalibrations::setIntervalFor-> key: "
                       << record << " time: " << iTime.eventID() << '/'
                       << iTime.time().value();
...
CastorHardcodeCalibrations::producePedestals(const CastorPedestalsRcd &) {
...
CastorHardcodeCalibrations::produceGains(const CastorGainsRcd &) {
...
CastorHardcodeCalibrations::produceGainWidths(const CastorGainWidthsRcd &) {
...
CastorHardcodeCalibrations::produceQIEData(const CastorQIEDataRcd &rcd) {
...
CastorHardcodeCalibrations::produceRecoParams(const CastorRecoParamsRcd &rcd) {
