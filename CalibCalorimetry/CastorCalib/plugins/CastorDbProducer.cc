...
//
/**\class CastorDbProducer CastorDbProducer.h
 CalibFormats/CastorDbProducer/interface/CastorDbProducer.h
...
CastorDbProducer::CastorDbProducer(const edm::ParameterSet &fConfig)
  ...
  mDumpRequest = fConfig.getUntrackedParameter<std::vector<std::string>>(
      "dump", std::vector<std::string>());
  if (!mDumpRequest.empty()) {
    std::string otputFile =
        fConfig.getUntrackedParameter<std::string>("file", "");
    mDumpStream =
        otputFile.empty() ? &std::cout : new std::ofstream(otputFile.c_str());
  }
}
...
std::shared_ptr<CastorDbService>
CastorDbProducer::produce(const CastorDbRecord &record) {
  ...
  host->ifRecordChanges<CastorElectronicsMapRcd>(
      record,
      [this, h = host.get()](auto const &rec) { setupElectronicsMap(rec, h); });
  host->ifRecordChanges<CastorChannelQualityRcd>(
      record,
      [this, h = host.get()](auto const &rec) { setupChannelQuality(rec, h); });
  host->ifRecordChanges<CastorGainWidthsRcd>(
      record, [this, h = host.get(), &needBuildCalibWidths](auto const &rec) {
        setupGainWidths(rec, h);
        needBuildCalibWidths = true;
      });
  host->ifRecordChanges<CastorQIEDataRcd>(
      record, [this, h = host.get(), &needBuildCalibrations,
               &needBuildCalibWidths](auto const &rec) {
        setupQIEData(rec, h);
        needBuildCalibrations = true;
        needBuildCalibWidths = true;
      });
  host->ifRecordChanges<CastorPedestalWidthsRcd>(
      record, [this, h = host.get(), &needBuildCalibWidths](auto const &rec) {
        setupPedestalWidths(rec, h);
        needBuildCalibWidths = true;
      });
  host->ifRecordChanges<CastorGainsRcd>(
      record, [this, h = host.get(), &needBuildCalibrations](auto const &rec) {
        setupGains(rec, h);
        needBuildCalibrations = true;
      });
  host->ifRecordChanges<CastorPedestalsRcd>(
      record, [this, h = host.get(), &needBuildCalibrations](auto const &rec) {
...
void CastorDbProducer::setupPedestals(const CastorPedestalsRcd &fRecord,
  ...
  if (std::find(mDumpRequest.begin(), mDumpRequest.end(),
                std::string("Pedestals")) != mDumpRequest.end()) {
    *mDumpStream << "New HCAL/CASTOR Pedestals set" << std::endl;
    CastorDbASCIIIO::dumpObject(*mDumpStream, *(item.product()));
  }
}

void CastorDbProducer::setupPedestalWidths(
    const CastorPedestalWidthsRcd &fRecord, CastorDbService *service) {
  edm::ESHandle<CastorPedestalWidths> item;
  fRecord.get(item);
  service->setData(item.product());
  if (std::find(mDumpRequest.begin(), mDumpRequest.end(),
                std::string("PedestalWidths")) != mDumpRequest.end()) {
    *mDumpStream << "New HCAL/CASTOR Pedestals set" << std::endl;
    CastorDbASCIIIO::dumpObject(*mDumpStream, *(item.product()));
  }
}
...
void CastorDbProducer::setupGains(const CastorGainsRcd &fRecord,
  ...
  service->setData(item.product());
  if (std::find(mDumpRequest.begin(), mDumpRequest.end(),
                std::string("Gains")) != mDumpRequest.end()) {
...
void CastorDbProducer::setupGainWidths(const CastorGainWidthsRcd &fRecord,
  ...
  service->setData(item.product());
  if (std::find(mDumpRequest.begin(), mDumpRequest.end(),
                std::string("GainWidths")) != mDumpRequest.end()) {
...
void CastorDbProducer::setupQIEData(const CastorQIEDataRcd &fRecord,
  ...
  service->setData(item.product());
  if (std::find(mDumpRequest.begin(), mDumpRequest.end(),
                std::string("QIEData")) != mDumpRequest.end()) {
    *mDumpStream << "New HCAL/CASTOR QIEData set" << std::endl;
    CastorDbASCIIIO::dumpObject(*mDumpStream, *(item.product()));
  }
}

void CastorDbProducer::setupChannelQuality(
    const CastorChannelQualityRcd &fRecord, CastorDbService *service) {
  edm::ESHandle<CastorChannelQuality> item;
  fRecord.get(item);
  service->setData(item.product());
  if (std::find(mDumpRequest.begin(), mDumpRequest.end(),
                std::string("ChannelQuality")) != mDumpRequest.end()) {
    *mDumpStream << "New HCAL/CASTOR ChannelQuality set" << std::endl;
    CastorDbASCIIIO::dumpObject(*mDumpStream, *(item.product()));
  }
}

void CastorDbProducer::setupElectronicsMap(
    const CastorElectronicsMapRcd &fRecord, CastorDbService *service) {
  edm::ESHandle<CastorElectronicsMap> item;
  fRecord.get(item);
  service->setData(item.product());
  if (std::find(mDumpRequest.begin(), mDumpRequest.end(),
                std::string("ElectronicsMap")) != mDumpRequest.end()) {
