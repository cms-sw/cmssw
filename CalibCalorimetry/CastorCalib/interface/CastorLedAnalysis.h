...
private:
  ...
  //###
  typedef std::pair<TH1F *, std::pair<std::map<int, std::vector<double>>,
                                      std::vector<TH1F *>>>
      LEDBUNCH;
  typedef struct {
    TProfile *avePulse[3];
  ...
  TFile *m_file;
  void LedCastorHists(const HcalDetId &detid, const CastorDataFrame &ledDigi,
                      std::map<HcalDetId, std::map<int, LEDBUNCH>> &toolT,
                      const CastorDbService &cond);
  void SetupLEDHists(int id, const HcalDetId detid,
                     std::map<HcalDetId, std::map<int, LEDBUNCH>> &toolT);
