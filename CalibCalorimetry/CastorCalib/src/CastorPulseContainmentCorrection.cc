...
template <>
RecoFCcorFactorAlgo<CastorPulseShapes::Shape>::RecoFCcorFactorAlgo(
    int num_samples, double fixedphase_ns) {
  ...
  for (int shift_ns = 0; shift_ns < binsize_ns; shift_ns++) {
    ...
    double bin1val =
        (double)shape_.integrate(tmin + binsize_ns, tmin + 2 * binsize_ns);
...
RecoFCcorFactorAlgo<CastorPulseShapes::Shape>::calcpair(double truefc) {
...
/// Generate energy correction factors based on a predetermined phase of the hit
/// + time slew
//
CastorPulseContainmentCorrection::CastorPulseContainmentCorrection(
    int num_samples, float fixedphase_ns, float max_fracerror) {
  RecoFCcorFactorAlgo<CastorPulseShapes::Shape> corFalgo(num_samples,
  ...
  //
  genlkupmap<RecoFCcorFactorAlgo<CastorPulseShapes::Shape>>(
      1.0, 5000.0f,  // generation domain
