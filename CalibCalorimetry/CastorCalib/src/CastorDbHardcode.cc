...
CastorPedestal CastorDbHardcode::makePedestal(HcalGenericDetId fId,
  ...
  float value0 =
      fId.genericSubdet() == HcalGenericDetId::HcalGenForward ? 11. : 4.; // fC
  float value[4] = {value0, value0, value0, value0};
  if (fSmear) {
    for (int i = 0; i < 4; i++) {
      value[i] = CLHEP::RandGauss::shoot(
          value0, width.getWidth(i) /
                      100.); // ignore correlations, assume 10K pedestal run
      while (value[i] <= 0)
...
CastorDbHardcode::makeCalibrationQIECoder(HcalGenericDetId fId) {
...
CastorDbHardcode::makeSaturationCorr(HcalGenericDetId fId) {
...
void CastorDbHardcode::makeHardcodeMap(CastorElectronicsMap &emap) {
  ...
  int ihslotho[EMAP_NHSETSHO][EMAP_NHTRSHO] = {
  ...
  /* ihbheetadepth - unique HBHE {eta,depth} assignments per fiber and fiber
   * channel */
  ...
  int ihfetadepth[EMAP_NTOPBOT][EMAP_NFBR][EMAP_NFCH][2] = {
      ...
      {{{41, 1}, {37, 1}, {35, 1}}, /* bot */
       ...
       {{40, 1}, {38, 1}, {36, 1}},
       {{37, 1}, {35, 1}, {39, 1}}}};
  /* ihoetasidephi - unique HO {eta,side,phi} assignments per fiber and fiber
   * channel */
  ...
  int iside, ieta, iphi, idepth, icrate, ihtr, ihtr_fi, ifi_ch, ispigot, idcc,
      /*idcc_sl,*/ ifed;
  std::string det;
  std::string fpga;
  // printf("      side       eta       phi     depth       det     crate htr
  // fpga    htr_fi     fi_ch     spigo       dcc    dcc_sl     fedid\n");
  /* all HBHE crates */
        ...
        for (itb = 0; itb < EMAP_NTOPBOT; itb++) {
          ...
          for (ifb = 0; ifb < EMAP_NFBR; ifb++) {
            ...
            for (ifc = 0; ifc < EMAP_NFCH; ifc++) {
              ...
              HcalDetId hId((det == "HB") ? (HcalBarrel) : (HcalEndcap),
                            ieta * iside, iphi, idepth);
              emap.mapEId2chId(elId, hId);

              //	      printf(" %9d %9d %9d %9d %9s %9d %9d %9s %9d %9d
              //%9d %9d %9d
        ...
        for (itb = 0; itb < EMAP_NTOPBOT; itb++) {
          ...
          for (ifb = 0; ifb < EMAP_NFBR; ifb++) {
            ...
            for (ifc = 0; ifc < EMAP_NFCH; ifc++) {
              ...
              iphi =
                  (ieta > 39)
                      ? (ihfphis[ic] + (is % 2) * 12 + ih * 4 - 3) % 72 + 1
                      : (ihfphis[ic] + (is % 2) * 12 + ih * 4 + (ifb / 4) * 2 -
                         1) % 72 +
                            1;
              ispigot = (is % 2) * 6 + ih * 2 + itb;
              idcc = is < EMAP_NHSETS / 2 ? 1 : 2;
              ...
              emap.mapEId2chId(elId, hId);
              // printf(" %9d %9d %9d %9d %9s %9d %9d %9s %9d %9d %9d %9d %9d
              // %9d\n",iside,ieta,iphi,idepth,&det,icrate,ihtr,&fpga,ihtr_fi,ifi_ch,ispigot,idcc,idcc_sl,ifed);
            }
          }
        }
          ...
          for (ifb = 0; ifb < EMAP_NFBR; ifb++) {
            ...
            for (ifc = 0; ifc < EMAP_NFCH; ifc++) {
              ...
              if (ieta == 0) { // unmapped
                ...
                HcalDetId hId(HcalOuter, ieta * iside, iphi,
                              4); // HO is officially "depth=4"
                emap.mapEId2chId(elId, hId);
              }
              // printf(" %9d %9d %9d %9d %9s %9d %9d %9s %9d %9d %9d %9d %9d
              // %9d\n",iside,ieta,iphi,idepth,&det,icrate,ihtr,&fpga,ihtr_fi,ifi_ch,ispigot,idcc,idcc_sl,ifed);
