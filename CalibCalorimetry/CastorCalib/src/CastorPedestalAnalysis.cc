...
CastorPedestalAnalysis::CastorPedestalAnalysis(const edm::ParameterSet &ps)
    : fRefPedestals(nullptr), fRefPedestalWidths(nullptr),
      fRawPedestals(nullptr), fRawPedestalWidths(nullptr),
      fValPedestals(nullptr), fValPedestalWidths(nullptr) {
  evt = 0;
  sample = 0;
  m_file = nullptr;
  m_AllPedsOK = 0;
  ...
  if (!m_outputFileMean.empty()) {
    cout << "Castor pedestal means will be saved to "
         << m_outputFileMean.c_str() << endl;
  }
  m_outputFileWidth = ps.getUntrackedParameter<string>("outputFileWidths", "");
  if (!m_outputFileWidth.empty()) {
    cout << "Castor pedestal widths will be saved to "
         << m_outputFileWidth.c_str() << endl;
  }
  m_outputFileROOT = ps.getUntrackedParameter<string>("outputFileHist", "");
  if (!m_outputFileROOT.empty()) {
    cout << "Castor pedestal histograms will be saved to "
  ...
  if (m_nevtsample > 0 && m_pedValflag > 0) {
    cout << "WARNING - incompatible cfg options: nevtsample = " << m_nevtsample
  ...
  castorHists.ALLPEDS =
      new TH1F("Castor All Pedestals", "HF All Peds", 10, 0, 9);
  castorHists.PEDRMS =
      new TH1F("Castor All Pedestal Widths", "HF All Pedestal RMS", 100, 0, 3);
  castorHists.PEDMEAN =
      new TH1F("Castor All Pedestal Means", "HF All Pedestal Means", 100, 0, 9);
  castorHists.CHI2 = new TH1F("Castor Chi2/ndf for whole range Gauss fit",
...
CastorPedestalAnalysis::~CastorPedestalAnalysis() {
  ...
  for (_meot = castorHists.PEDTRENDS.begin();
       _meot != castorHists.PEDTRENDS.end(); _meot++) {
    for (int i = 0; i < 16; i++)
      ...
      for (int i = m_startTS; i < digi.size() && i <= m_endTS; i++) {
        for (int flag = 0; flag < 4; flag++) {
          if (i + flag < digi.size() && i + flag <= m_endTS) {
            per2CapsHists(flag, 2, digi.id(), digi.sample(i),
                          digi.sample(i + flag), castorHists.PEDTRENDS, cond);
          }
        }
      }
      if (m_startTS == 0 && m_endTS > 4) {
        AllChanHists(digi.id(), digi.sample(0), digi.sample(1), digi.sample(2),
                     digi.sample(3), digi.sample(4), digi.sample(5),
...
//-----------------------------------------------------------------------------
void CastorPedestalAnalysis::per2CapsHists(
    int flag, int id, const HcalDetId detid, const HcalQIESample &qie1,
    const HcalQIESample &qie2, map<HcalDetId, map<int, PEDBUNCH>> &toolT,
    const CastorDbService &cond) {

  // this function is due to be called for every time slice, it fills either a
  // charge histo for a single capID (flag=0) or a product histo for two capIDs
  // (flag>0)
  ...
  */
  ...
  if (_meot == toolT.end()) {
    ...
    for (int i = 0; i < 4; i++) {
      lo = -0.5;
      // fix from Andy: if you convert to fC and then bin in units of 1, you may
      // 'skip' a bin while filling, since the ADCs are quantized
      if (m_pedsinADC)
        hi = 9.5;
      else
        hi = 11.5;
      sprintf(name, "%s Pedestal, eta=%d phi=%d d=%d cap=%d", type.c_str(),
              detid.ieta(), detid.iphi(), detid.depth(), i);
      insert[i].first = new TH1F(name, name, bins, lo, hi);
      sprintf(name, "%s Product, eta=%d phi=%d d=%d caps=%d*%d", type.c_str(),
              detid.ieta(), detid.iphi(), detid.depth(), i, (i + 1) % 4);
      insert[4 + i].first = new TH1F(name, name, bins2, 0., 100.);
      sprintf(name, "%s Product, eta=%d phi=%d d=%d caps=%d*%d", type.c_str(),
              detid.ieta(), detid.iphi(), detid.depth(), i, (i + 2) % 4);
      insert[8 + i].first = new TH1F(name, name, bins2, 0., 100.);
      sprintf(name, "%s Product, eta=%d phi=%d d=%d caps=%d*%d", type.c_str(),
              detid.ieta(), detid.iphi(), detid.depth(), i, (i + 3) % 4);
      insert[12 + i].first = new TH1F(name, name, bins2, 0., 100.);
    }
    sprintf(name, "%s Signal in TS 4+5, eta=%d phi=%d d=%d", type.c_str(),
            detid.ieta(), detid.iphi(), detid.depth());
    insert[16].first = new TH1F(name, name, 21, -0.5, 20.5);
    sprintf(name, "%s Signal in TS 4+5-2-3, eta=%d phi=%d d=%d", type.c_str(),
            detid.ieta(), detid.iphi(), detid.depth());
    insert[17].first = new TH1F(name, name, 21, -10.5, 10.5);
    sprintf(name, "%s Signal in TS 4+5-(0+1+2+3)/2., eta=%d phi=%d d=%d",
            type.c_str(), detid.ieta(), detid.iphi(), detid.depth());
  ...
  if (flag > 0) {
    ...
    // float
    // charge1=(qie1.adc()-qiecalib[qie1.capid()+4])/qiecalib[qie1.capid()];
    // float
    // charge2=(qie2.adc()-qiecalib[qie2.capid()+4])/qiecalib[qie2.capid()];
...
//-----------------------------------------------------------------------------
void CastorPedestalAnalysis::AllChanHists(
    const HcalDetId detid, const HcalQIESample &qie0, const HcalQIESample &qie1,
    const HcalQIESample &qie2, const HcalQIESample &qie3,
    const HcalQIESample &qie4, const HcalQIESample &qie5,
  ...
  _mei[18].first->Fill(qie4.adc() + qie5.adc() -
                       (qie0.adc() + qie1.adc() + qie2.adc() + qie3.adc()) /
                           2.);
}
...
//-----------------------------------------------------------------------------
void CastorPedestalAnalysis::GetPedConst(
    map<HcalDetId, map<int, PEDBUNCH>> &toolT, TH1F *PedMeans,
    TH1F *PedWidths) {
  // Completely rewritten version oct 2006
  // Compute pedestal constants and fill into CastorPedestals and
  // CastorPedestalWidths objects
  float cap[4];
  float sig[4][4];
  float dcap[4];
  float dsig[4][4];
  ...
  for (_meot = toolT.begin(); _meot != toolT.end(); _meot++) {
    ...
    // fill the objects - at this point only close and medium correlations are
...
//-----------------------------------------------------------------------------
int CastorPedestalAnalysis::done(
    const CastorPedestals *fInputPedestals,
  ...
  if (m_nevtsample > 0) {
    m_file->cd();
    m_file->cd("Castor");
    Trendings(castorHists.PEDTRENDS, castorHists.CHI2,
              castorHists.CAPID_AVERAGE, castorHists.CAPID_CHI2);
  }
  ...
  if (m_nevtsample < 1) {
    ...
    if (m_pedValflag > 0) {
      ...
      int NPedErrors =
          CastorPedVal(nstat, fRefPedestals, fRefPedestalWidths, fRawPedestals,
                       fRawPedestalWidths, fValPedestals, fValPedestalWidths);
  ...
  m_file->Close();
  cout << "Hcal/Castor histograms written to " << m_outputFileROOT.c_str()
       << endl;
  return (int)m_AllPedsOK;
}

//-----------------------------------------------------------------------------
void CastorPedestalAnalysis::Trendings(
    map<HcalDetId, map<int, PEDBUNCH>> &toolT, TH1F *Chi2, TH1F *CapidAverage,
    TH1F *CapidChi2) {
  ...
  for (_meot = toolT.begin(); _meot != toolT.end(); _meot++) {
    for (int i = 0; i < 4; i++) {
      char name[1024];
      HcalDetId detid = _meot->first;
      sprintf(name, "Pedestal trend, eta=%d phi=%d d=%d cap=%d", detid.ieta(),
              detid.iphi(), detid.depth(), i);
      int bins = _meot->second[i].second.first[0].size();
      float lo = 0.5;
      float hi = (float)bins + 0.5;
      _meot->second[i].second.second.push_back(
          new TH1F(name, name, bins, lo, hi));
      sprintf(name, "Width trend, eta=%d phi=%d d=%d cap=%d", detid.ieta(),
              detid.iphi(), detid.depth(), i);
      bins = _meot->second[i].second.first[2].size();
      hi = (float)bins + 0.5;
      _meot->second[i].second.second.push_back(
          new TH1F(name, name, bins, lo, hi));
      sprintf(name, "Correlation trend, eta=%d phi=%d d=%d caps=%d*%d",
              detid.ieta(), detid.iphi(), detid.depth(), i, (i + 1) % 4);
      bins = _meot->second[i].second.first[5].size();
      hi = (float)bins + 0.5;
      _meot->second[i].second.second.push_back(
          new TH1F(name, name, bins, lo, hi));
      /*      sprintf(name,"Correlation trend, eta=%d phi=%d d=%d
         caps=%d*%d",detid.ieta(),detid.iphi(),detid.depth(),i,(i+2)%4); bins =
         _meot->second[i].second.first[7].size(); hi = (float)bins+0.5;
            _meot->second[i].second.second.push_back(new
         TH1F(name,name,bins,lo,hi)); sprintf(name,"Correlation trend, eta=%d
         phi=%d d=%d
         caps=%d*%d",detid.ieta(),detid.iphi(),detid.depth(),i,(i+3)%4); bins =
         _meot->second[i].second.first[9].size(); hi = (float)bins+0.5;
            _meot->second[i].second.second.push_back(new
         TH1F(name,name,bins,lo,hi)); */
      ...
      _meot->second[i].second.second[2]->GetYaxis()->SetTitle(
          "Close correlation");
      ...
      /*     j=0;
           ...
           _meot->second[i].second.second[3]->GetYaxis()->SetTitle("Intermediate
         correlation"); _meot->second[i].second.second[3]->Write(); j=0;
           ...
           _meot->second[i].second.second[4]->GetYaxis()->SetTitle("Distant
         correlation"); _meot->second[i].second.second[4]->Write(); */
      // chi2
      j = 0;
  ...
  CapidAverage =
      new TH1F("Constant fit: Pedestal Values", "Constant fit: Pedestal Values",
  ...
  CapidChi2 = new TH1F("Constant fit: Chi2/ndf", "Constant fit: Chi2/ndf",
...
//-----------------------------------------------------------------------------
int CastorPedestalAnalysis::CastorPedVal(
    int nstat[4], const CastorPedestals *fRefPedestals,
    const CastorPedestalWidths *fRefPedestalWidths,
    CastorPedestals *fRawPedestals, CastorPedestalWidths *fRawPedestalWidths,
  ...
  for (int i = 0; i < (int)RawChanns.size(); i++) {
    ...
    if (isinRef[detid] == false) {
      PedValLog << "CastorPedVal: channel " << detid
                << " not found in reference set" << std::endl;
      std::cerr << "CastorPedVal: channel " << detid
                << " not found in reference set" << std::endl;
    }
  }
  ...
  for (int i = 0; i < (int)RefChanns.size(); i++) {
    ...
    for (int icap = 0; icap < 4; icap++) {
      ...
      for (int icap2 = icap; icap2 < 4; icap2++) {
        RefPedSigs[icap][icap2] =
            fRefPedestalWidths->getValues(detid)->getSigma(icap, icap2);
        if (icap2 != icap)
    ...
    if (isinRaw[detid]) {
      for (int icap = 0; icap < 4; icap++) {
        ...
        for (int icap2 = icap; icap2 < 4; icap2++) {
          RawPedSigs[icap][icap2] =
              fRawPedestalWidths->getValues(detid)->getSigma(icap, icap2);
          if (icap2 != icap)
            RawPedSigs[icap2][icap] = RawPedSigs[icap][icap2];
        }
      }

      // first quick check if raw values make sense: if not, the channel is
      // treated like absent
      for (int icap = 0; icap < 4; icap++) {
        ...
        for (int icap2 = icap; icap2 < 4; icap2++) {
          if (fabs(RawPedSigs[icap][icap2] /
                   sqrt(RawPedSigs[icap][icap] * RawPedSigs[icap2][icap2])) >
              1.)
            isinRaw[detid] = false;
        }
      }
    ...
    if (isinRaw[detid]) {
      for (int icap = 0; icap < 4; icap++) {
        ...
        float erof2 =
            sqrt(erof1 * erof1 + RawPedSigs[icap][icap] / (float)nstat[icap]);
        float erofwidth = width / sqrt(2. * nstat[icap]);
        float diffof1 = RawPedVals[icap] - RefPedVals[icap];
        float diffof2 = RawPedVals[icap] + RawPedVals[icap2] -
                        RefPedVals[icap] - RefPedVals[icap2];
        ...
        if (nTS == 1 && fabs(diffof1) > 0.5 + erof1) {
          erflag += 1;
          PedValLog << "HcalPedVal: drift in channel " << detid << " cap "
                    << icap << ": " << RawPedVals[icap] << " - "
                    << RefPedVals[icap] << " = " << diffof1 << std::endl;
        }
        if (nTS == 2 && fabs(diffof2) > 0.5 + erof2) {
          erflag += 1;
          PedValLog << "HcalPedVal: drift in channel " << detid << " caps "
                    << icap << "+" << icap2 << ": " << RawPedVals[icap] << "+"
                    << RawPedVals[icap2] << " - " << RefPedVals[icap] << "+"
                    << RefPedVals[icap2] << " = " << diffof2 << std::endl;
        }
        if (fabs(diffofw) > 0.15 * width + erofwidth) {
          erflag += 1;
          PedValLog << "HcalPedVal: width changed in channel " << detid
                    << " cap " << icap << ": " << width << " - "
                    << sqrt(RefPedSigs[icap][icap]) << " = " << diffofw
                    << std::endl;
        }
      }
    }
    ...
    // for disconnected/bad channels restore reference values
      ...
      PedValLog << "HcalPedVal: no valid data from channel " << detid
                << std::endl;
      erflag += 100000;
      CastorPedestal item(detid, RefPedVals[0], RefPedVals[1], RefPedVals[2],
                          RefPedVals[3]);
  ...
  // if anything changed, validated set = raw set + reference for missing/bad
  // channels
  else {
    for (int i = 0; i < (int)RawChanns.size(); i++) {
      detid = HcalDetId(RawChanns[i]);
      if (isinRaw[detid]) {
        ...
        fValPedestalWidths->addValues(widthsp);
        CastorPedestal item(detid, RawPedVals[0], RawPedVals[1], RawPedVals[2],
                            RawPedVals[3]);
        fValPedestals->addValues(item);
      }
    }
