...
void dumpHeader(std::ostream &fOutput, unsigned fRun,
                const std::string &fTableName, const std::string &fTypeName) {
  fOutput << "  <HEADER>" << std::endl;
  fOutput << "    <TYPE>" << std::endl;
  fOutput << "      <EXTENSION_TABLE_NAME>" << fTableName
...
void dumpChannelId(std::ostream &fOutput, DetId fChannel) {
  ...
  fOutput << "   <Z>" << (converter.getField(1) > 0 ? "1" : "-1") << "</Z> "
          << std::endl;
  fOutput << "   <DETECTOR_NAME>" << converter.getFlavor()
          << "</DETECTOR_NAME> " << std::endl;
  fOutput << "   <HCAL_CHANNEL_ID>" << converter.getId().rawId()
          << "</HCAL_CHANNEL_ID> " << std::endl;
  fOutput << "</CHANNEL>" << std::endl;
  fOutput << std::endl;
}

void dumpData(std::ostream &fOutput, const float *fValues,
...
void dumpDataset(std::ostream &fOutput, unsigned fVersion = 0,
  ...
  if (!fFileName.empty())
    fOutput << "      <DATA_FILE_NAME>" << fFileName << "</DATA_FILE_NAME>"
            << std::endl;
  if (!fDescription.empty())
    fOutput << "      <COMMENT_DESCRIPTION>" << fDescription
...
void dumpMapping(std::ostream &fOutput, unsigned fRun, const std::string &fKind,
                 ...
                 const std::string &fTag, unsigned fVersion,
  ...
  while (--i >= 0) {
    ...
    dumpRun(fOutput, fRun);
    fOutput << "<KIND_OF_CONDITION><NAME>" << fKind
  ...
  fOutput << "   <INTERVAL_OF_VALIDITY_BEGIN>" << fGMTIOVBegin
          << "</INTERVAL_OF_VALIDITY_BEGIN>" << std::endl;
  fOutput << "   <INTERVAL_OF_VALIDITY_END>" << fGMTIOVEnd
  ...
  fOutput << "   <COMMENT_DESCRIPTION>Automatically created by "
             "CastorDbXml</COMMENT_DESCRIPTION>"
          << std::endl;
...
bool CastorDbXml::dumpObject(std::ostream &fOutput, unsigned fRun,
                             unsigned long fGMTIOVBegin,
                             unsigned long fGMTIOVEnd, const std::string &fTag,
                             unsigned fVersion,
                             const CastorPedestals &fObject) {
  float dummyError = 0.0001;
  std::cout << "CastorDbXml::dumpObject-> set default errors: 0.0001, 0.0001, "
               "0.0001, 0.0001"
            << std::endl;
  ...
  return dumpObject(fOutput, fRun, fGMTIOVBegin, fGMTIOVEnd, fTag, fVersion,
                    fObject, widths);
}
...
bool CastorDbXml::dumpObject(std::ostream &fOutput, unsigned fRun,
                             unsigned long fGMTIOVBegin,
                             unsigned long fGMTIOVEnd, const std::string &fTag,
                             unsigned fVersion, const CastorPedestals &fObject,
  ...
  for (std::vector<DetId>::iterator channel = channels.begin();
    ...
    if (!errors) {
      std::cerr << "CastorDbXml::dumpObject-> Can not get errors for channel "
                << CastorText2DetIdConverter(chId).toString()
  ...
  dumpMapping(fOutput, fRun, KIND, fGMTIOVBegin, fGMTIOVEnd, fTag, fVersion,
              channels);
...
bool CastorDbXml::dumpObject(std::ostream &fOutput, unsigned fRun,
                             unsigned long fGMTIOVBegin,
                             unsigned long fGMTIOVEnd, const std::string &fTag,
                             unsigned fVersion, const CastorGains &fObject) {
  float dummyErrors[4] = {0., 0., 0., 0.};
  std::cout << "CastorDbXml::dumpObject-> set default errors: 4 x 0.0"
            << std::endl;
  ...
  for (std::vector<DetId>::iterator channel = channels.begin();
       channel != channels.end(); ++channel) {
    CastorGainWidth item(*channel, dummyErrors[0], dummyErrors[1],
                         dummyErrors[2], dummyErrors[3]);
    widths.addValues(item);
  }
