...
CastorLedAnalysis::CastorLedAnalysis(const edm::ParameterSet &ps) {
  ...
  if (!m_outputFileText.empty()) {
    cout << "Castor LED results will be saved to " << m_outputFileText.c_str()
         << endl;
  ...
  if (!m_outputFileROOT.empty()) {
    cout << "Castor LED histograms will be saved to "
         << m_outputFileROOT.c_str() << endl;
  }
  ...
  castorHists.LEDRMS =
      new TH1F("Castor All LED RMS", "HF All LED RMS", 100, 0, 3);
  castorHists.LEDMEAN =
      new TH1F("Castor All LED Means", "HF All LED Means", 100, 0, 9);
  castorHists.CHI2 = new TH1F("Castor Chi2 by ndf for Landau fit",
  ...
  m_outputFileXML << output << endl;
  snprintf(output, sizeof output, "      <RUN_NUMBER>%06i</RUN_NUMBER>",
           runNum);
  m_outputFileXML << output << endl;
  snprintf(
      output, sizeof output,
  ...
  m_outputFileXML << output << endl;
  snprintf(output, sizeof output, "        <TAG_NAME>laser_led_%06i<TAG_NAME>",
           runNum);
  m_outputFileXML << output << endl;
  snprintf(output, sizeof output,
...
CastorLedAnalysis::~CastorLedAnalysis() {
  ...
  for (_meol = castorHists.LEDTRENDS.begin();
       _meol != castorHists.LEDTRENDS.end(); _meol++) {
    for (int i = 0; i < 15; i++)
...
void CastorLedAnalysis::GetLedConst(map<HcalDetId, map<int, LEDBUNCH>> &toolT) {
  ...
  if (!m_outputFileText.empty()) {
    if (m_fitflag == 0 || m_fitflag == 2)
      ...
      m_outFile << "Det Eta,Phi,D   Mean    Error      Peak    Error       "
                   "MeanEv  Error       PeakEv  Error"
                << std::endl;
  }
  for (_meol = toolT.begin(); _meol != toolT.end(); _meol++) {
    ...
    if (m_fitflag == 0 || m_fitflag == 4) {
      ...
      dtime1 = _meol->second[10].first->GetRMS() /
               sqrt((float)evt_curr * (m_endTS - m_startTS + 1));
    }
    if (m_fitflag == 1 || m_fitflag == 4) {
      // put proper errors
      for (int j = 0; j < 10; j++)
        _meol->second[10].first->SetBinError(
            j + 1, _meol->second[j].first->GetRMS() / sqrt((float)evt_curr));
    }
    ...
    if (m_fitflag == 2 || m_fitflag == 4) {
      ...
      dtime3 = _meol->second[12].first->GetRMS() /
               sqrt((float)_meol->second[12].first->GetEntries());
    }
    if (m_fitflag == 3 || m_fitflag == 4) {
      time4 = _meol->second[13].first->GetMean();
      dtime4 = _meol->second[13].first->GetRMS() /
               sqrt((float)_meol->second[13].first->GetEntries());
    }
    for (int i = 0; i < 10; i++) {
    ...
    if (!m_outputFileText.empty()) {
      if (m_fitflag == 0) {
        ...
        m_outputFileXML << output << endl;
        snprintf(output, sizeof output, "      <ETA>%2i</ETA>",
                 detid.ietaAbs());
        m_outputFileXML << output << endl;
        snprintf(output, sizeof output, "      <PHI>%2i</PHI>", detid.iphi());
        m_outputFileXML << output << endl;
        snprintf(output, sizeof output, "      <DEPTH>%2i</DEPTH>",
                 detid.depth());
        m_outputFileXML << output << endl;
        ...
        m_outputFileXML << output << endl;
        snprintf(output, sizeof output, "      <MEAN_TIME>%7f</MEAN_TIME>",
                 time1);
        m_outputFileXML << output << endl;
        snprintf(output, sizeof output, "      <OFFSET_TIME> 0</OFFSET_TIME>");
        m_outputFileXML << output << endl;
        snprintf(output, sizeof output, "      <ERROR_STAT>%7f</ERROR_STAT>",
                 dtime1);
        m_outputFileXML << output << endl;
        snprintf(output, sizeof output,
        ...
        m_outFile << detid << "   " << time1 << " " << dtime1 << "   " << time2
                  << " " << dtime2 << "   " << time3 << " " << dtime3 << "   "
                  << time4 << " " << dtime4 << std::endl;
      }
    }
  }
}
...
//-----------------------------------------------------------------------------
void CastorLedAnalysis::LedTrendings(
    map<HcalDetId, map<int, LEDBUNCH>> &toolT) {

  for (_meol = toolT.begin(); _meol != toolT.end(); _meol++) {
    char name[1024];
    HcalDetId detid = _meol->first;
    sprintf(name, "LED timing trend, eta=%d phi=%d depth=%d", detid.ieta(),
            detid.iphi(), detid.depth());
    int bins = _meol->second[10 + m_fitflag].second.first[0].size();
    float lo = 0.5;
    float hi = (float)bins + 0.5;
    _meol->second[10 + m_fitflag].second.second.push_back(
    ...
    for (sample_it = _meol->second[10 + m_fitflag].second.first[0].begin();
         ...
         ++sample_it) {
      _meol->second[10 + m_fitflag].second.second[0]->SetBinContent(++j,
                                                                    *sample_it);
    }
    j = 0;
    for (sample_it = _meol->second[10 + m_fitflag].second.first[1].begin();
         ...
         ++sample_it) {
      _meol->second[10 + m_fitflag].second.second[0]->SetBinError(++j,
                                                                  *sample_it);
    }
    sprintf(name, "Sample (%d events)", m_nevtsample);
    _meol->second[10 + m_fitflag].second.second[0]->GetXaxis()->SetTitle(name);
    _meol->second[10 + m_fitflag].second.second[0]->GetYaxis()->SetTitle(
        "Peak position");
...
void CastorLedAnalysis::processLedEvent(const CastorDigiCollection &castor,
  ...
  try {
    ...
    for (CastorDigiCollection::const_iterator j = castor.begin();
         j != castor.end(); ++j) {
...
//----------------------------------------------------------------------------
void CastorLedAnalysis::SetupLEDHists(
    int id, const HcalDetId detid, map<HcalDetId, map<int, LEDBUNCH>> &toolT) {
  ...
  if (_meol == toolT.end()) {
    ...
    char name[1024];
    for (int i = 0; i < 10; i++) {
      sprintf(name, "%s Pulse height, eta=%d phi=%d depth=%d TS=%d",
              type.c_str(), detid.ieta(), detid.iphi(), detid.depth(), i);
      insert[i].first = new TH1F(name, name, 200, 0., 2000.);
    }
    sprintf(name, "%s LED Mean pulse, eta=%d phi=%d depth=%d", type.c_str(),
            detid.ieta(), detid.iphi(), detid.depth());
    insert[10].first = new TH1F(name, name, 10, -0.5, 9.5);
    sprintf(name, "%s LED Pulse, eta=%d phi=%d depth=%d", type.c_str(),
            detid.ieta(), detid.iphi(), detid.depth());
    insert[11].first = new TH1F(name, name, 10, -0.5, 9.5);
    sprintf(name, "%s Mean TS, eta=%d phi=%d depth=%d", type.c_str(),
            detid.ieta(), detid.iphi(), detid.depth());
    insert[12].first = new TH1F(name, name, 200, 0., 10.);
    sprintf(name, "%s Peak TS, eta=%d phi=%d depth=%d", type.c_str(),
            detid.ieta(), detid.iphi(), detid.depth());
    insert[13].first = new TH1F(name, name, 200, 0., 10.);
    sprintf(name, "%s Peak TS error, eta=%d phi=%d depth=%d", type.c_str(),
            detid.ieta(), detid.iphi(), detid.depth());
    insert[14].first = new TH1F(name, name, 200, 0., 0.05);
    sprintf(name, "%s Fit chi2, eta=%d phi=%d depth=%d", type.c_str(),
            detid.ieta(), detid.iphi(), detid.depth());
    insert[15].first = new TH1F(name, name, 100, 0., 50.);
    sprintf(name, "%s Integrated Signal, eta=%d phi=%d depth=%d", type.c_str(),
            detid.ieta(), detid.iphi(), detid.depth());
...
//-----------------------------------------------------------------------------
void CastorLedAnalysis::LedCastorHists(
    const HcalDetId &detid, const CastorDataFrame &ledDigi,
    map<HcalDetId, map<int, LEDBUNCH>> &toolT, const CastorDbService &cond) {
  ...
  // Most of this is borrowed from CastorSimpleReconstructor, so thanks
  // Jeremy/Phil
  ...
  for (int TS = m_startTS; TS < m_endTS && TS < ledDigi.size(); TS++) {
    ...
    if (adc < 0) {
      adc = 0;
    } // to prevent negative adcs after ped subtraction, which should really
      // only happen if you're using the wrong peds.
    double fC = m_coder->charge(*m_shape, adc, capid);
    // ta = (fC - m_ped->getValue(capid));
    ta = fC;
    // cout << "DetID: " << detid << "  CapID: " << capid << "  ADC: " << adc <<
    // "  Ped: " << m_ped->getValue(capid) << "  fC: " << fC << endl;
    _mei[TS].first->Fill(ta);
    _mei[10].first->AddBinContent(
        TS + 1,
        ta); // This is average pulse, could probably do better (Profile?)
    if (m_fitflag > 1) {
      if (TS == m_startTS)
        _mei[11].first->Reset();
  ...
  if (sum > 100) {
    ...
    _mei[16].first->Fill(
        _mei[11].first->Integral()); // Integrated charge (may be more usfull to
                                     // convert to Energy first?)
    if (m_fitflag == 3 || m_fitflag == 4) {
...
float CastorLedAnalysis::BinsizeCorr(float time) {
  ...
  // This should eventually be replaced with the more thorough treatment from
  // Phil

  float corrtime = 0.;
  static const float tstrue[32] = {
      0.003, 0.03425, 0.06548, 0.09675, 0.128, 0.15925, 0.1905, 0.22175,
  ...
  static const float tsreco[32] = {
      -0.00422, 0.01815, 0.04409, 0.07346, 0.09799, 0.12192, 0.15072, 0.18158,
  ...
  for (int i = 0; i <= 32; i++) {
    ...
    if (restime >= lolim && restime < uplim) {
      corrtime = (tsdown * (uplim - restime) + tsup * (restime - lolim)) /
                 (uplim - lolim);
    }
  }
  corrtime += inttime;
