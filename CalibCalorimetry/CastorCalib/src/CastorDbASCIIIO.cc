...
void dumpId(std::ostream &fOutput, DetId id) {
  ...
  char buffer[1024];
  sprintf(buffer, "  %15s %15s %15s %15s", converter.getField1().c_str(),
          converter.getField2().c_str(), converter.getField3().c_str(),
...
bool dumpCastorObject(std::ostream &fOutput, const T &fObject) {
  ...
  sprintf(buffer, "# %15s %15s %15s %15s %8s %8s %8s %8s %10s\n", "eta", "phi",
          "dep", "det", "cap0", "cap1", "cap2", "cap3", "DetId");
  fOutput << buffer;
  ...
  for (std::vector<DetId>::iterator channel = channels.begin();
    ...
    if (values) {
      ...
      sprintf(buffer, " %8.5f %8.5f %8.5f %8.5f %10X\n", values[0], values[1],
              values[2], values[3], channel->rawId());
      fOutput << buffer;
    }
  }
  return true;
...
bool getCastorSingleFloatObject(std::istream &fInput, T &fObject) {
  ...
  while (fInput.getline(buffer, 1024)) {
    ...
    if (items.size() < 5) {
      edm::LogWarning("Format Error")
          << "Bad line: " << buffer
          << "\n line must contain 5 items: eta, phi, depth, subdet, value"
          << std::endl;
      continue;
    }
    DetId id = getId(items);
...
template <class S, class T>
bool getCastorSingleIntObject(std::istream &fInput, T &fObject,
                              S *fCondObject) {
  char buffer[1024];
  while (fInput.getline(buffer, 1024)) {
    if (buffer[0] == '#')
...
bool dumpObject(std::ostream &fOutput, const CastorGains &fObject) {
  return dumpCastorObject(fOutput, fObject);
}
    ...
    if (items.empty())
      ...
      if (items[0] == "#U") {
        if (items[1] == (std::string) "ADC")
          ...
          edm::LogWarning("Pedestal Unit Error")
        ...
        edm::LogWarning("Pedestal Unit Missing")
            << "The unit for the pedestals is missing in the txt file."
            << std::endl;
        return false;
      }
    }
  ...
  while (fInput.getline(buffer, 1024)) {
    ...
    if (items.size() < 8) {
      edm::LogWarning("Format Error")
          << "Bad line: " << buffer
          << "\n line must contain 8 items: eta, phi, depth, subdet, 4x values"
          << " or 12 items: eta, phi, depth, subdet, 4x values for mean, 4x "
             "values for width"
          << std::endl;
      continue;
    }
    ...
    {
      CastorPedestal fCondObject(id, atof(items[4].c_str()),
                                 atof(items[5].c_str()), atof(items[6].c_str()),
                                 atof(items[7].c_str()), 0., 0., 0., 0.);
      fObject.addValues(fCondObject);
    } else // new format with widths
    {
      CastorPedestal fCondObject(
...
bool dumpObject(std::ostream &fOutput, const CastorPedestals &fObject) {
  ...
  sprintf(buffer,
          ...
          "phi", "dep", "det", "cap0", "cap1", "cap2", "cap3", "widthcap0",
          "widthcap1", "widthcap2", "widthcap3", "DetId");
  fOutput << buffer;
  ...
  for (std::vector<DetId>::iterator channel = channels.begin();
    ...
    if (values) {
      ...
      sprintf(buffer, " %8.5f %8.5f %8.5f %8.5f %8.5f %8.5f %8.5f %8.5f %10X\n",
              ...
              values[6], values[7], channel->rawId());
      fOutput << buffer;
    }
  }
  return true;
  ...
  while (fInput.getline(buffer, 1024)) {
    ...
    if (items.size() < 5) {
      edm::LogWarning("Format Error") << "Bad line: " << buffer
                                      << "\n line must contain 5 items: eta, "
                                         "phi, depth, subdet, GOOD/BAD/HOT/DEAD"
                                      << std::endl;
      continue;
    }
    DetId id = getId(items);
    ...
    if (fObject.exists(id)) {
      edm::LogWarning("Redefining Channel")
          << "line: " << buffer << "\n attempts to redefine data. Ignored"
          << std::endl;
      continue;
    }
    //    else
      ...
      if (items[0] == (std::string) "#U") {
        if (items[1] == (std::string) "ADC")
          ...
          edm::LogWarning("Pedestal Width Unit Error")
              << "Unrecognized unit for pedestal widths. Assuming fC."
              << std::endl;
          fObject.setUnitADC(false);
        }
        break;
        ...
        edm::LogWarning("Pedestal Width Unit Missing")
            << "The unit for the pedestal widths is missing in the txt file."
            << std::endl;
        return false;
      }
    }
  ...
  while (fInput.getline(buffer, 1024)) {
    ...
    if (items.size() < 14) {
      edm::LogWarning("Format Error")
          ...
          << "\n line must contain 14 items: eta, phi, depth, subdet, 10x "
             "correlations"
...
bool dumpObject(std::ostream &fOutput, const CastorPedestalWidths &fObject) {
  ...
  sprintf(buffer,
          "# %15s %15s %15s %15s %8s %8s %8s %8s %8s %8s %8s %8s %8s %8s %8s "
          "%8s %8s %8s %8s %8s %10s\n",
          "eta", "phi", "dep", "det", "cov_0_0", "cov_0_1", "cov_0_2",
          "cov_0_3", "cov_1_0", "cov_1_1", "cov_1_2", "cov_1_3", "cov_2_0",
          "cov_2_1", "cov_2_2", "cov_2_3", "cov_3_0", "cov_3_1", "cov_3_2",
          "cov_3_3", "DetId");
  fOutput << buffer;
  std::vector<DetId> channels = fObject.getAllChannels();
  std::sort(channels.begin(), channels.end(), DetIdLess());
  for (std::vector<DetId>::iterator channel = channels.begin();
    ...
    if (item) {
      ...
      sprintf(buffer,
              ...
              "%8.5f %8.5f %8.5f %8.5f %8.5f %8.5f %10X\n",
              item->getSigma(0, 0), item->getSigma(0, 1), item->getSigma(0, 2),
              item->getSigma(0, 3), item->getSigma(1, 0), item->getSigma(1, 1),
              item->getSigma(1, 2), item->getSigma(1, 3), item->getSigma(2, 0),
              item->getSigma(2, 1), item->getSigma(2, 2), item->getSigma(2, 3),
              item->getSigma(3, 0), item->getSigma(3, 1), item->getSigma(3, 2),
              item->getSigma(3, 3), channel->rawId());
      fOutput << buffer;
    }
  }
  return true;
  ...
  while (fInput.getline(buffer, 1024)) {
    ...
    if (items[0] == "SHAPE") { // basic shape
      if (items.size() < 33) {
        edm::LogWarning("Format Error")
            << "Bad line: " << buffer
            << "\n line must contain 33 items: SHAPE  32 x low QIE edges for "
               "first 32 bins"
            << std::endl;
        continue;
      }
      ...
      if (items.size() < 36) {
        edm::LogWarning("Format Error")
            << "Bad line: " << buffer
            << "\n line must contain 36 items: eta, phi, depth, subdet, 4 "
               "capId x 4 Ranges x offsets, 4 capId x 4 Ranges x slopes"
            << std::endl;
        continue;
      }
      DetId id = getId(items);
...
bool dumpObject(std::ostream &fOutput, const CastorQIEData &fObject) {
  char buffer[1024];
  fOutput
      << "# QIE basic shape: SHAPE 32 x low edge values for first 32 channels"
      << std::endl;
  ...
  sprintf(buffer,
          ...
          "eta", "phi", "dep", "det", "4 x offsets cap0", "4 x offsets cap1",
          "4 x offsets cap2", "4 x offsets cap3", "4 x slopes cap0",
          "4 x slopes cap1", "4 x slopes cap2", "4 x slopes cap3");
  fOutput << buffer;
  std::vector<DetId> channels = fObject.getAllChannels();
  std::sort(channels.begin(), channels.end(), DetIdLess());
  for (std::vector<DetId>::iterator channel = channels.begin();
...
bool getObject(std::istream &fInput, CastorCalibrationQIEData &fObject) {
  ...
  while (fInput.getline(buffer, 1024)) {
    ...
    if (items.size() < 36) {
      edm::LogWarning("Format Error") << "Bad line: " << buffer
                                      << "\n line must contain 36 items: eta, "
                                         "phi, depth, subdet, 32 bin values"
                                      << std::endl;
      continue;
    }
    DetId id = getId(items);
...
bool getObject(std::istream &fInput, CastorElectronicsMap &fObject) {
  ...
  while (fInput.getline(buffer, 1024)) {
      ...
      if (items[8] == "NA" || items[8] == "NT") {
        ...
        if (items.size() == 11)
          ...
          edm::LogError("MapFormat")
              ...
              << "\n HT line must contain at least 11 items: i  cr sl tb dcc "
                 "spigot fiber fiberchan subdet=HT ieta iphi";
          continue;
        }
      } else {
        edm::LogError("MapFormat")
            << "CastorElectronicsMap-> Bad line: " << buffer
            << "\n line must contain 12 items: i  cr sl tb dcc spigot fiber "
               "fiberchan subdet ieta iphi depth";
        continue;
      }
    }
    //    std::cout << "CastorElectronicsMap-> processing line: " << buffer <<
    //    std::endl;
    ...
    if (items[8] == "NA") { // undefined channel
      ...
      CastorText2DetIdConverter converter(items[8], items[9], items[10],
                                          items[11]);
...
bool dumpObject(std::ostream &fOutput, const CastorElectronicsMap &fObject) {
  ...
  sprintf(buf, "# %7s %3s %3s %3s %4s %7s %10s %14s %7s %5s %5s %6s", "i", "cr",
          "sl", "tb", "dcc", "spigot", "fiber/slb", "fibcha/slbcha", "subdet",
          "ieta", "iphi", "depth");
  ...
  for (unsigned i = 0; i < eids.size(); i++) {
    ...
    if (eid.isTriggerChainId()) {
      ...
      if (trigger.rawId()) {
        ...
        // changes by Jared, 6.03.09/(included 25.03.09)
        //	sprintf (buf, " %10X %6d %6d %6c %6d %6d %6d %6d %15s %15s %15s
        //%15s",
        sprintf(buf, " %7X %3d %3d %3c %4d %7d %10d %14d %7s %5s %5s %6s",
                //		 i,
                converter.getId().rawId(),
                // changes by Jared, 6.03.09/(included 25.03.09)
                //		 eid.readoutVMECrateId(), eid.htrSlot(),
                // eid.htrTopBottom()>0?'t':'b', eid.dccid(), eid.spigot(),
                // eid.fiberIndex(), eid.fiberChanId(),
                eid.readoutVMECrateId(), eid.htrSlot(),
                eid.htrTopBottom() > 0 ? 't' : 'b', eid.dccid(), eid.spigot(),
                eid.slbSiteNumber(), eid.slbChannelIndex(),
                converter.getFlavor().c_str(), converter.getField1().c_str(),
                converter.getField2().c_str(), converter.getField3().c_str());
        fOutput << buf << std::endl;
      }
    } else {
      ...
      if (channel.rawId()) {
        ...
        sprintf(buf, " %7X %3d %3d %3c %4d %7d %10d %14d %7s %5s %5s %6s",
                //		 i,
                converter.getId().rawId(), eid.readoutVMECrateId(),
                eid.htrSlot(), eid.htrTopBottom() > 0 ? 't' : 'b', eid.dccid(),
...
bool getObject(std::istream &fInput, CastorRecoParams &fObject) {
  ...
  while (fInput.getline(buffer, 1024)) {
    ...
    if (items.size() < 6) {
      edm::LogWarning("Format Error")
          << "Bad line: " << buffer
          << "\n line must contain 6 items: eta, phi, depth, subdet, "
    ...
    CastorRecoParam fCondObject(id, atoi(items[4].c_str()),
                                atoi(items[5].c_str()));
    fObject.addValues(fCondObject);
  }
  return true;
...
bool dumpObject(std::ostream &fOutput, const CastorRecoParams &fObject) {
  ...
  sprintf(buffer, "# %15s %15s %15s %15s %18s %15s %10s\n", "eta", "phi", "dep",
          "det", "firstSample", "samplesToAdd", "DetId");
  fOutput << buffer;
  std::vector<DetId> channels = fObject.getAllChannels();
  std::sort(channels.begin(), channels.end(), DetIdLess());
  for (std::vector<DetId>::iterator channel = channels.begin();
    ...
    sprintf(buffer, " %15d %15d %16X\n",
            ...
            fObject.getValues(*channel)->samplesToAdd(), channel->rawId());
    fOutput << buffer;
  }
  return true;
}
