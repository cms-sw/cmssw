...
FTLUncalibratedRecHit FTLSimpleUncalibRecHitAlgo::makeRecHit(const FTLDataFrame& dataFrame) const {
  ...
  LogDebug("FTLSimpleUncalibRecHit") << "ADC+: set the charge to: " << amplitude << ' ' << sample.data() << ' '
                                     << adcLSB_ << ' ' << std::endl;
  LogDebug("FTLSimpleUncalibRecHit") << "ADC+: set the time to: " << time << ' ' << sample.toa() << ' ' << toaLSBToNS_
                                     << ' ' << std::endl;
  LogDebug("FTLSimpleUncalibRecHit") << "Final uncalibrated amplitude : " << amplitude << std::endl;

  return FTLUncalibratedRecHit(
      dataFrame.id(), std::make_pair(amplitude, 0.), std::make_pair(time, 0.), timeError_, flag);
}
