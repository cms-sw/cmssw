...
FTLUncalibratedRecHitProducer::FTLUncalibratedRecHitProducer(const edm::ParameterSet& ps)
  ...
  barrel_ = std::unique_ptr<FTLUncalibratedRecHitAlgoBase>{
      FTLUncalibratedRecHitAlgoFactory::get()->create(barrelAlgo, barrel, sumes)};
  ...
  endcap_ = std::unique_ptr<FTLUncalibratedRecHitAlgoBase>{
      FTLUncalibratedRecHitAlgoFactory::get()->create(endcapAlgo, endcap, sumes)};
}

FTLUncalibratedRecHitProducer::~FTLUncalibratedRecHitProducer() {}

void FTLUncalibratedRecHitProducer::produce(edm::Event& evt, const edm::EventSetup& es) {
