...
goodMuons = cms.EDFilter("PATMuonSelector",
    ...
    cut = cms.string(
        ...
        ' && track.getHtPattern.trackerLayersWithMeasurement(\'TRACK_HITS\') > 5 & innerTrack.getHitPattern.numberOfValidPixelHits(\'TRACK_HITS\') > 0'
        ' && abs(dB) < 0.2 && globalTrack.normalizedChi2 < 10'
        ' && globalTrack.getHitPattern.numberOfValidMuonHits(\'TRACK_HITS\') > 0 && numberOfMatchedStations > 1'
    ),
    filter = cms.bool(False)
)
