...
bool DYToMuTauGenFilter::muondecay(const reco::Candidate *d, int depth)
{
	...
	if (d->status()!= 1)
	{
		if(d->numberOfDaughters() == 3)
		{
			if(std::abs(d->daughter(0)->pdgId()) == 14 
			|| std::abs(d->daughter(1)->pdgId()) == 14 
			|| std::abs(d->daughter(2)->pdgId()) == 14)	check = true;
		}
		else if (d->numberOfDaughters() > 3) return false;
		if (d->numberOfDaughters() < 4)
		{
			...
			{
				int new_depth = depth + 1;
				if(muondecay(d->daughter(k), new_depth) == true) check = true;
			}
		}	
	}
	return check;
...
void
DYToMuTauGenFilter::beginStream(edm::StreamID)
{
}

// ------------ method called once each stream after processing all runs, lumis and events  ------------
void
DYToMuTauGenFilter::endStream() {
}

// ------------ method fills 'descriptions' with the allowed parameters for the module  ------------
void
DYToMuTauGenFilter::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
