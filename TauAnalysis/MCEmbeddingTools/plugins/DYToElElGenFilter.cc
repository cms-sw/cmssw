    ...
    for(unsigned int i = 0; i < gen_handle->size(); i++)
    {
      const reco::GenParticle gen_particle = (*gen_handle)[i];
		...
		if (gen_particle.pdgId() == 23 && gen_particle.numberOfDaughters() == 2)
		{
		  ...
		  if (std::abs(gen_particle.daughter(0)->pdgId()) == 11 
          && std::abs(gen_particle.daughter(1)->pdgId()) == 11  
...
void
DYToElElGenFilter::beginStream(edm::StreamID)
{
}

// ------------ method called once each stream after processing all runs, lumis and events  ------------
void
DYToElElGenFilter::endStream() {
}

// ------------ method fills 'descriptions' with the allowed parameters for the module  ------------
void
DYToElElGenFilter::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
