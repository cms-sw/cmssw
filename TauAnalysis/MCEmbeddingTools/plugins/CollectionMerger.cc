...
template <>
void  CollectionMerger<std::vector<reco::ElectronSeed>,  reco::ElectronSeed>::willproduce(std::string instance, std::string alias)
{
    produces<MergeCollection>(instance);
}
...
void  CollectionMerger<T1,T2>::fill_output_obj_seed(edm::Event& iEvent, std::unique_ptr<MergeCollection > & output, std::vector<edm::Handle<MergeCollection> > &inputCollections)
{
  ...
  // First merge the two collections again
  for (auto const & inputCollection : inputCollections){
    for ( typename MergeCollection::const_iterator seed = inputCollection->begin(); seed!= inputCollection->end(); ++seed ) {
      T2 newSeed(*seed);
...
template <typename T1, typename T2>
void CollectionMerger<T1, T2>::fill_output_obj(edm::Event& iEvent, std::unique_ptr<MergeCollection> &output,
                                               std::vector<edm::Handle<MergeCollection> > &inputCollections) {
...
template <>
void CollectionMerger<edm::SortedCollection<CastorRecHit>, CastorRecHit>::fill_output_obj(edm::Event& iEvent,
    std::unique_ptr<MergeCollection> &output, std::vector<edm::Handle<MergeCollection> > &inputCollections) {
  fill_output_obj_calo(output, inputCollections);
}
...
// Here Electron Seeds
template <>
void  CollectionMerger<std::vector<reco::ElectronSeed>,  reco::ElectronSeed>::fill_output_obj(edm::Event& iEvent,std::unique_ptr<MergeCollection > & output, std::vector<edm::Handle<MergeCollection> > &inputCollections)
{
