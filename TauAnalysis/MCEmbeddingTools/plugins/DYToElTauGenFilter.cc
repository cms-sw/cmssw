...
bool DYToElTauGenFilter::leptondecay(const reco::Candidate *d, int depth)
{
	...
	if (d->status()!= 1)
	{
		if(d->numberOfDaughters() == 3)
		{
			...
			|| std::abs(d->daughter(2)->pdgId()) == 12)	check = true;
		}
		...
		if (d->numberOfDaughters() < 4)
		{
			for (unsigned int k = 0; k < d->numberOfDaughters(); k++)
			{
				...
				if(leptondecay(d->daughter(k), new_depth) == true) check = true;
			}
		}
	}
	return check;
...
bool DYToElTauGenFilter::electrondecay(const reco::Candidate *d, int depth)
{
	...
	if (d->status()!= 1)
	{
		if(d->numberOfDaughters() == 3)
		{
			if(std::abs(d->daughter(0)->pdgId()) == 12 
			|| std::abs(d->daughter(1)->pdgId()) == 12 
			|| std::abs(d->daughter(2)->pdgId()) == 12)	check = true;
		}
		else if (d->numberOfDaughters() > 3) return false;
		if (d->numberOfDaughters() < 4)
		{
			...
			{
				int new_depth = depth + 1;
				if(electrondecay(d->daughter(k), new_depth) == true) check = true;
			}
		}	
	}
	return check;
      ...
      const reco::GenParticle gen_particle = (*gen_handle)[i];
		...
		if (gen_particle.pdgId() == 23 && gen_particle.numberOfDaughters() == 2)
		{
		  ...
		  if (std::abs(gen_particle.daughter(0)->pdgId()) == 15 
		      && std::abs(gen_particle.daughter(0)->eta())<2.1  
		      && gen_particle.daughter(0)->pt()>25
		      && gen_particle.daughter(1)->pt()>18)
			{
...
void
DYToElTauGenFilter::beginStream(edm::StreamID)
{
}

// ------------ method called once each stream after processing all runs, lumis and events  ------------
void
DYToElTauGenFilter::endStream() {
}

// ------------ method fills 'descriptions' with the allowed parameters for the module  ------------
void
DYToElTauGenFilter::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
