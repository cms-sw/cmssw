...
namespace cmsutils {
  ...
  template <class T>
  inline void intrusive_ptr_add_ref(_bqueue_item<T> *it) {
    it->addRef();
  }
  template <class T>
  inline void intrusive_ptr_release(_bqueue_item<T> *it) {
    it->delRef();
  }
  ...
  public:
    ...
    // move
    bqueue(bqueue<T> &&cp) noexcept : m_size(cp.m_size), m_head(std::move(cp.m_head)), m_tail(std::move(cp.m_tail)) {
      cp.m_size = 0;
    }
    ...
    bqueue<T> fork() const { return *this; }
