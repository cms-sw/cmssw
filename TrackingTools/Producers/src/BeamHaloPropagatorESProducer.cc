...
BeamHaloPropagatorESProducer::BeamHaloPropagatorESProducer(
  ...
  theEndCapTrackerPropagatorName =
      parameterSet.getParameter<string>("EndCapTrackerPropagator");
  theCrossingTrackerPropagatorName =
      parameterSet.getParameter<string>("CrossingTrackerPropagator");
...
BeamHaloPropagatorESProducer::produce(const TrackingComponentsRecord &iRecord) {
  ...
  return std::make_unique<BeamHaloPropagator>(
      *endcapPropagator, *crossPropagator, &*magField, thePropagationDirection);
}
