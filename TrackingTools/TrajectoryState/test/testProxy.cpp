...
struct A {
  virtual ~A(){} virtual std::shared_ptr<A> clone() const =0; 

  template<typename T, typename... Args>
  static std::shared_ptr<A> build(Args && ...args){ return std::allocate_shared<T>(std::allocator<T>(),std::forward<Args>(args)...);}
...
void astd(int k) {
  ...
  std::cout << b.references() << ' ' << &b.data() << ' ';std::cout << &b.unsharedData() << std::endl;
  c=b;
  std::cout << b.references() << ' ' << &b.data() << ' ';
  std::cout << &b.unsharedData() << std::endl;
  std::cout << b.references() << ' ' << &b.data() << ' ';std::cout << &b.unsharedData() << std::endl;
  std::cout << c.references() << ' ' << &c.data() << ' ';std::cout << &c.unsharedData() << std::endl;
