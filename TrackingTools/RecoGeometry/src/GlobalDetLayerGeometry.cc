...
const DetLayer *GlobalDetLayerGeometry::idToLayer(const DetId &detId) const {
  ...
  else if (mtd_ != nullptr && detId.det() == DetId::Forward &&
           detId.subdetId() == FastTime)
    return mtd_->idToLayer(detId);
  else {
    throw cms::Exception("DetLayers")
        << "Error: called GlobalDetLayerGeometry::idToLayer() for a detId "
           "which is neither Tracker nor Muon "
