  ...
  explicit CompositeLogicalTrajectoryFilter(const edm::ParameterSet& pset, edm::ConsumesCollector& iC) {
    ...
    for (unsigned int i = 0; i != vpset.size(); i++) {
      ...
      filters.emplace_back(l,
                           std::unique_ptr<TrajectoryFilter>(TrajectoryFilterFactory::get()->create(
                               vpset[i].getParameter<std::string>("ComponentName"), vpset[i], iC)));
    }
  }
...
protected:
  ...
  bool TBC(T& traj) const {
    ...
    for (; i < n; i++) {
      if (!filters[i].second->toBeContinued(traj))
        return false;
    }
    return true;
  }
  ...
  bool QF(const T& traj) const {
    ...
    unsigned int n = filters.size();
    if (n == 0) {
      edm::LogError("CompositeLogicalTrajectoryFilter") << n << " filters !.";
      return false;
    }
