...
public:
  explicit Chi2Strip1DEstimator(double maxChi2, double nSigma = 3.) : Chi2MeasurementEstimatorBase(maxChi2, nSigma) {}

  std::pair<bool, double> estimate(const TrajectoryStateOnSurface&, const TrackingRecHit&) const override;

  Chi2Strip1DEstimator* clone() const override { return new Chi2Strip1DEstimator(*this); }
};
