...
public:
  ...
  CandidatePtrTransientTrack(const CandidatePtr &tk, const double time,
                             const double dtime, const MagneticField *field);

  CandidatePtrTransientTrack(
      const CandidatePtr &tk, const MagneticField *field,
      const edm::ESHandle<GlobalTrackingGeometry> &trackingGeometry);
  CandidatePtrTransientTrack(
      const CandidatePtr &tk, const double time, const double dtime,
      const MagneticField *field,
      const edm::ESHandle<GlobalTrackingGeometry> &trackingGeometry);
  ...
  bool impactPointStateAvailable() const override {
    return (m_TSOS.load() == kSet ? true : false);
  }
