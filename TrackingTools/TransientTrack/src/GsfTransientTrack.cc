...
GsfTransientTrack::GsfTransientTrack(const GsfTrack &tk,
    ...
    : GsfTrack(tk), tkr_(), hasTime(false), timeExt_(0.), dtErrorExt_(0.),
      theField(field), initialTSOSAvailable(false), initialTSCPAvailable(false),
...
GsfTransientTrack::GsfTransientTrack(const GsfTrack &tk, const double time,
                                     const double dtime,
                                     const MagneticField *field)
    : GsfTrack(tk), tkr_(), hasTime(true), timeExt_(time), dtErrorExt_(dtime),
      theField(field), initialTSOSAvailable(false), initialTSCPAvailable(false),
      blStateAvailable(false) {
...
GsfTransientTrack::GsfTransientTrack(const GsfTrackRef &tk,
                                     const MagneticField *field)
    : GsfTrack(*tk), tkr_(tk), hasTime(false), timeExt_(0.), dtErrorExt_(0.),
      theField(field), initialTSOSAvailable(false), initialTSCPAvailable(false),
      blStateAvailable(false),
...
GsfTransientTrack::GsfTransientTrack(const GsfTrackRef &tk, const double time,
                                     const double dtime,
                                     const MagneticField *field)
    : GsfTrack(*tk), tkr_(tk), hasTime(true), timeExt_(time),
      dtErrorExt_(dtime), theField(field), initialTSOSAvailable(false),
      initialTSCPAvailable(false), blStateAvailable(false),
      theTIPExtrapolator(AnalyticalPropagator(field, alongMomentum)) {
...
GsfTransientTrack::GsfTransientTrack(
    const GsfTrack &tk, const MagneticField *field,
...
GsfTransientTrack::GsfTransientTrack(
    const GsfTrack &tk, const double time, const double dtime,
...
GsfTransientTrack::GsfTransientTrack(
    const GsfTrackRef &tk, const MagneticField *field,
    const edm::ESHandle<GlobalTrackingGeometry> &tg)
    : GsfTrack(*tk), tkr_(tk), hasTime(false), timeExt_(0.), dtErrorExt_(0.),
      theField(field), initialTSOSAvailable(false), initialTSCPAvailable(false),
      blStateAvailable(false), theTrackingGeometry(tg),
...
GsfTransientTrack::GsfTransientTrack(
    const GsfTrackRef &tk, const double time, const double dtime,
    const MagneticField *field, const edm::ESHandle<GlobalTrackingGeometry> &tg)
    : GsfTrack(*tk), tkr_(tk), hasTime(true), timeExt_(time),
      dtErrorExt_(dtime), theField(field), initialTSOSAvailable(false),
      initialTSCPAvailable(false), blStateAvailable(false),
...
GsfTransientTrack::GsfTransientTrack(const GsfTransientTrack &tt)
    : GsfTrack(tt), tkr_(tt.persistentTrackRef()), hasTime(tt.hasTime),
...
void GsfTransientTrack::setTrackingGeometry(
    const edm::ESHandle<GlobalTrackingGeometry> &tg) {
...
TrajectoryStateOnSurface GsfTransientTrack::outermostMeasurementState() const {
  ...
  return theMTransform.outerStateOnSurface((*this), *theTrackingGeometry,
                                           theField);
}

TrajectoryStateOnSurface GsfTransientTrack::innermostMeasurementState() const {
  MultiTrajectoryStateTransform theMTransform;
  return theMTransform.innerStateOnSurface((*this), *theTrackingGeometry,
                                           theField);
}
...
TrajectoryStateClosestToPoint GsfTransientTrack::trajectoryStateClosestToPoint(
