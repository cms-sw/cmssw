...
TrackTransientTrack::TrackTransientTrack(const Track &tk,
    ...
    : Track(tk), tkr_(), hasTime(false), timeExt_(0.), dtErrorExt_(0.),
      theField(field), m_TSOS(kUnset), m_TSCP(kUnset), m_SCTBL(kUnset) {
...
TrackTransientTrack::TrackTransientTrack(
    const Track &tk, const MagneticField *field,
...
TrackTransientTrack::TrackTransientTrack(
    const TrackRef &tk, const MagneticField *field,
    const edm::ESHandle<GlobalTrackingGeometry> &tg)
    : Track(*tk), tkr_(tk), hasTime(false), timeExt_(0.), dtErrorExt_(0.),
      theField(field), m_TSOS(kUnset), m_TSCP(kUnset), m_SCTBL(kUnset),
      theTrackingGeometry(tg) {
...
TrackTransientTrack::TrackTransientTrack(
    const Track &tk, const double time, const double dtime,
...
TrackTransientTrack::TrackTransientTrack(
    const TrackRef &tk, const double time, const double dtime,
    const MagneticField *field, const edm::ESHandle<GlobalTrackingGeometry> &tg)
    : Track(*tk), tkr_(tk), hasTime(true), timeExt_(time), dtErrorExt_(dtime),
      theField(field), m_TSOS(kUnset), m_TSCP(kUnset), m_SCTBL(kUnset),
      theTrackingGeometry(tg) {
...
TrackTransientTrack::TrackTransientTrack(const TrackTransientTrack &tt)
    : Track(tt), tkr_(tt.persistentTrackRef()), hasTime(tt.hasTime),
      timeExt_(tt.timeExt()), dtErrorExt_(tt.dtErrorExt()),
      theField(tt.field()), initialFTS(tt.initialFreeState()), m_TSOS(kUnset),
      m_TSCP(kUnset) {
...
void TrackTransientTrack::setTrackingGeometry(
    const edm::ESHandle<GlobalTrackingGeometry> &tg) {
...
TrackTransientTrack::outermostMeasurementState() const {
...
TrajectoryStateOnSurface
TrackTransientTrack::innermostMeasurementState() const {
