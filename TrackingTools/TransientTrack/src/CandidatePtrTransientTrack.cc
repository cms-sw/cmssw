...
CandidatePtrTransientTrack::CandidatePtrTransientTrack()
    : Track(), ptr_(), hasTime(false), timeExt_(0), dtErrorExt_(0),
      theField(nullptr), m_TSOS(kUnset), m_TSCP(kUnset), m_SCTBL(kUnset) {}

CandidatePtrTransientTrack::CandidatePtrTransientTrack(
    const CandidatePtr &ptr, const MagneticField *field)
    : Track(*ptr->bestTrack()), ptr_(ptr), hasTime(false), timeExt_(0),
      dtErrorExt_(0), theField(field), m_TSOS(kUnset), m_TSCP(kUnset),
      m_SCTBL(kUnset) {

  initialFTS =
      trajectoryStateTransform::initialFreeState(*ptr->bestTrack(), field);
}

CandidatePtrTransientTrack::CandidatePtrTransientTrack(
    const CandidatePtr &ptr, const double time, const double dtime,
    const MagneticField *field)
    : Track(*ptr->bestTrack()), ptr_(ptr), hasTime(true), timeExt_(time),
      dtErrorExt_(dtime), theField(field), m_TSOS(kUnset), m_TSCP(kUnset),
      m_SCTBL(kUnset) {
...
CandidatePtrTransientTrack::CandidatePtrTransientTrack(
    ...
    : Track(*ptr->bestTrack()), ptr_(ptr), hasTime(false), timeExt_(0),
      dtErrorExt_(0), theField(field), m_TSOS(kUnset), m_TSCP(kUnset),
      m_SCTBL(kUnset), theTrackingGeometry(tg) {
...
CandidatePtrTransientTrack::CandidatePtrTransientTrack(
    const CandidatePtr &ptr, const double time, const double dtime,
    const MagneticField *field, const edm::ESHandle<GlobalTrackingGeometry> &tg)
    : Track(*ptr->bestTrack()), ptr_(ptr), hasTime(true), timeExt_(time),
      dtErrorExt_(dtime), theField(field), m_TSOS(kUnset), m_TSCP(kUnset),
      m_SCTBL(kUnset), theTrackingGeometry(tg) {
...
CandidatePtrTransientTrack::CandidatePtrTransientTrack(
    const CandidatePtrTransientTrack &tt)
    : Track(tt), ptr_(tt.candidate()), hasTime(tt.hasTime),
      timeExt_(tt.timeExt_), dtErrorExt_(tt.dtErrorExt_), theField(tt.field()),
...
void CandidatePtrTransientTrack::setTrackingGeometry(
    const edm::ESHandle<GlobalTrackingGeometry> &tg) {
...
CandidatePtrTransientTrack::impactPointTSCP() const {
  // see ThreadSafe statement above about the order of operator= and store
  if (kSet == m_TSCP.load())
    return initialTSCP;
...
CandidatePtrTransientTrack::outermostMeasurementState() const {

  return trajectoryStateTransform::outerStateOnSurface(
      (*this), *theTrackingGeometry, theField);
}

TrajectoryStateOnSurface
CandidatePtrTransientTrack::innermostMeasurementState() const {

  return trajectoryStateTransform::innerStateOnSurface(
      (*this), *theTrackingGeometry, theField);
}
...
CandidatePtrTransientTrack::stateAtBeamLine() const {
  // see ThreadSafe statement above about the order of operator= and store
