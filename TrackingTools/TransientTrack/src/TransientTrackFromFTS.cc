...
TransientTrackFromFTS::TransientTrackFromFTS()
    : hasTime(false), timeExt_(0.), dtErrorExt_(0.), theField(nullptr),
      initialTSOSAvailable(false), initialTSCPAvailable(false),
...
TransientTrackFromFTS::TransientTrackFromFTS(
    ...
    : initialFTS(fts), hasTime(false), timeExt_(0.), dtErrorExt_(0.),
      theField(&(initialFTS.parameters().magneticField())),
      initialTSOSAvailable(false), initialTSCPAvailable(false),
      trackAvailable(false), blStateAvailable(false), theTrackingGeometry(tg) {}
...
TransientTrackFromFTS::TransientTrackFromFTS(const TransientTrackFromFTS &tt)
    : initialFTS(tt.initialFreeState()), hasTime(tt.hasTime),
...
void TransientTrackFromFTS::setTrackingGeometry(
    const edm::ESHandle<GlobalTrackingGeometry> &tg) {
...
TransientTrackFromFTS::outermostMeasurementState() const {
  throw cms::Exception("LogicError")
      << "TransientTrack built from a FreeTrajectoryState "
         "(TransientTrackFromFTS) can not have an outermostMeasurementState";
}

TrajectoryStateOnSurface
TransientTrackFromFTS::innermostMeasurementState() const {
  throw cms::Exception("LogicError")
      << "TransientTrack built from a FreeTrajectoryState "
         "(TransientTrackFromFTS) can not have an innermostMeasurementState";
}
...
TransientTrackFromFTS::stateAtBeamLine() const {
  if
