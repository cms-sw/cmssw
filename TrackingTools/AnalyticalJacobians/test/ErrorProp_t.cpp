...
int main(int argc, char **argv) {
...

  {
    ...
    for (int i = 0; i < 10; ++i) {
      ...
      HelixForwardPlaneCrossing::DirectionType p(
          lplane.toLocal(tpg.momentum()));
      ...
      GlobalTrajectoryParameters tpg2(
          lplane.toGlobal(x), (p.mag() / dir.mag()) * lplane.toGlobal(dir),
          tpg.charge(), &m);
      ...
      delta.computeInfinitesimalJacobian(tpg, tpg2.position(), tpg2.momentum(),
                                         h, s);
    ...
    full.computeFullJacobian(tpg0, tpg.position(), tpg.momentum(), h,
                             totalStep);
    ...
    for (unsigned int i = 0; i < 5; ++i) {
      for (unsigned int j = 0; j < 5; ++j) {
        //       div(i,j) = fabs(full.jacobian()(i,j))>1.e-20 ?
        //       fullJacobian(i,j)/full.jacobian()(i,j)-1 : 0;
    ...
    for (unsigned int i = 0; i < 5; ++i) {
      for (unsigned int j = 0; j < 5; ++j) {
        //       div(i,j) = fabs(full.jacobian()(i,j))>1.e-20 ?
        //       deltaJacobian(i,j)/full.jacobian()(i,j)-1 : 0;
  ...
  //
  // plane at end of propagation (equivalent to Curvilinear definition up to
  ...
  LocalTrajectoryParameters newlpg(newplane.toLocal(tpg.position()),
                                   newplane.toLocal(tpg.momentum()), 1.);
  JacobianCurvilinearToLocal jcl(newplane, newlpg, m);
  //   AlgebraicMatrix55 jacobianL2L =
  //   jcl.jacobian()*full.jacobian()*jlc.jacobian();
  AlgebraicMatrix55 jacobianL2L =
      jcl.jacobian() * deltaJacobian * jlc.jacobian();
  //
  // redo propagation to target plane (should give identical position ...)
  //
  HelixArbitraryPlaneCrossing aprop(
      HelixForwardPlaneCrossing::PositionType(tpg0.position()),
      HelixForwardPlaneCrossing::DirectionType(tpg0.momentum()), curv);
  ...
  GlobalTrajectoryParameters tpg0mod(tpg0.position(),
                                     tpg0.momentum().unit() / fabs(qpmod),
                                     qpmod > 0 ? 1. : -1., &m);
  HelixArbitraryPlaneCrossing aprop2(
      HelixForwardPlaneCrossing::PositionType(tpg0mod.position()),
  ...
  //
  std::cout << newDx.x() - dpPred[3] << " " << newDx.y() - dpPred[4]
            << std::endl;
  std::cout << newDp.x() - dpPred[1] << " " << newDp.y() - dpPred[2]
            << std::endl;
