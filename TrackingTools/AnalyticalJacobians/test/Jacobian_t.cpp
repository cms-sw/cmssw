...
int main() {
  ...
  if (rot.xx() != Rsub(0, 0) || rot.xy() != Rsub(1, 0) ||
      rot.xz() != Rsub(2, 0) || rot.yx() != Rsub(0, 1) ||
      rot.yy() != Rsub(1, 1) || rot.yz() != Rsub(2, 1) ||
      rot.zx() != Rsub(0, 2) || rot.zy() != Rsub(1, 2) ||
      rot.zz() != Rsub(2, 2))
  ...
  GlobalTrajectoryParameters gp(plane.toGlobal(tp.position()),
                                plane.toGlobal(tp.momentum()), tp.charge(), &m);
...

  {
    ...
    edm::HRTimeType s = edm::hrRealTime();
    st();
    JacobianLocalToCurvilinear __attribute__((aligned(16)))
    jl2c(plane, tp, gp, m);
    en();
    edm::HRTimeType e = edm::hrRealTime();
    std::cout << e - s << std::endl;
...

  {
    ...
    edm::HRTimeType s = edm::hrRealTime();
    st();
    JacobianCurvilinearToLocal __attribute__((aligned(16)))
    jl2c(plane, tp, gp, m);
    en();
    edm::HRTimeType e = edm::hrRealTime();
    std::cout << e - s << std::endl;
