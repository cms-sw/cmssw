...
void AnalyticalCurvilinearJacobian::computeFullJacobian(
    const GlobalTrajectoryParameters &globalParameters, const GlobalPoint &x,
    const GlobalVector &p, const GlobalVector &h, const double &s) {
  ...
  // Martijn: field is now given as parameter.. GlobalVector h  =
  // globalParameters.magneticFieldInInverseGeV(xStart);
  ...
  theJacobian(2, 2) =
      cost * (u11 * u21 + u12 * u22) + sint * (hu1 * u21 + hu2 * u22) +
  ...
  const double limit =
      5.; // valid for propagations with effectively float precision
  ...
  theJacobian(3, 1) =
      (sint * (v11 * u21 + v12 * u22) + omcost * (hv1 * u21 + hv2 * u22) +
       tmsint * (hn1 * u21 + hn2 * u22) * (hn1 * v11 + hn2 * v12 + hn3 * v13)) /
      q;

  theJacobian(3, 2) =
      (sint * (u11 * u21 + u12 * u22) + omcost * (hu1 * u21 + hu2 * u22) +
  ...
  theJacobian(4, 1) = (sint * (v11 * v21 + v12 * v22 + v13 * v23) +
                       ...
                       tmsint * (hn1 * v21 + hn2 * v22 + hn3 * v23) *
                           (hn1 * v11 + hn2 * v12 + hn3 * v13)) /
                      q;

  theJacobian(4, 2) =
      (sint * (u11 * v21 + u12 * v22) +
...
void AnalyticalCurvilinearJacobian::computeInfinitesimalJacobian(
    const GlobalTrajectoryParameters &globalParameters, const GlobalPoint &,
    const GlobalVector &p, const GlobalVector &h, const double &s) {
  /*
   * origin  TRPROP
   *
...
void AnalyticalCurvilinearJacobian::computeStraightLineJacobian(
    const GlobalTrajectoryParameters &globalParameters, const GlobalPoint &,
