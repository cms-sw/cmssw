...
namespace multiTrajectoryStateMode {
  ...
  bool momentumFromModeCartesian(TrajectoryStateOnSurface const& tsos, GlobalVector& momentum) {
    ...
    for (const auto& component : components) {
      ...
      pxStates.emplace_back(mom.x(), cov(3, 3), component.weight());
      pyStates.emplace_back(mom.y(), cov(4, 4), component.weight());
      pzStates.emplace_back(mom.z(), cov(5, 5), component.weight());
    }
    //
  ...
  bool positionFromModeCartesian(TrajectoryStateOnSurface const& tsos, GlobalPoint& position) {
    ...
    for (const auto& component : components) {
      ...
      AlgebraicSymMatrix66 cov(component.cartesianError().matrix());
      xStates.emplace_back(pos.x(), cov(0, 0), component.weight());
      yStates.emplace_back(pos.y(), cov(1, 1), component.weight());
      zStates.emplace_back(pos.z(), cov(2, 2), component.weight());
    }
    //
    // transformation in 1D multi-states and creation of utility classes
    //
  ...
  bool momentumFromModePPhiEta(TrajectoryStateOnSurface const& tsos, GlobalVector& momentum) {
    ...
    for (const auto& component : components) {
      ...
      pStates.emplace_back(1 / op, covPPhiEta(0, 0), component.weight());
      phiStates.emplace_back(phi, covPPhiEta(1, 1), component.weight());
      etaStates.emplace_back(eta, covPPhiEta(2, 2), component.weight());
    }
    //
    // transformation in 1D multi-states and creation of utility classes
    //
