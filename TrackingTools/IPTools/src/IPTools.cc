...
absoluteImpactParameter3D(const reco::TransientTrack &transientTrack,
  ...
  return absoluteImpactParameter(
      extrapolator.extrapolate(transientTrack.impactPointState(),
                               RecoVertex::convertPos(vertex.position())),
      vertex, dist);
}
std::pair<bool, Measurement1D>
...
signedTransverseImpactParameter(const TransientTrack &track,
  ...
  TrajectoryStateOnSurface closestOnTransversePlaneState =
      extrapolator.extrapolate(track.impactPointState(),
                               RecoVertex::convertPos(vertex.position()));

  // Compute absolute value
  VertexDistanceXY dist;
  pair<bool, Measurement1D> result =
      absoluteImpactParameter(closestOnTransversePlaneState, vertex, dist);
  if (!result.first)
    return result;

  // Compute Sign
  GlobalPoint impactPoint = closestOnTransversePlaneState.globalPosition();
  GlobalVector IPVec(impactPoint.x() - vertex.x(), impactPoint.y() - vertex.y(),
                     0.);
...
pair<bool, Measurement1D> signedImpactParameter3D(const TransientTrack &track,
  ...
  TrajectoryStateOnSurface closestIn3DSpaceState = extrapolator.extrapolate(
      track.impactPointState(), RecoVertex::convertPos(vertex.position()));

  // Compute absolute value
  VertexDistance3D dist;
  pair<bool, Measurement1D> result =
      absoluteImpactParameter(closestIn3DSpaceState, vertex, dist);
  if (!result.first)
    return result;
...
pair<bool, Measurement1D>
signedDecayLength3D(const TrajectoryStateOnSurface &closestToJetState,
  ...
  double decayLen =
      jetDirection.dot(closestToJetState.globalPosition() - vertexPosition);
  ...
  // TODO: FIXME: the extrapolation uncertainty is very relevant here should be
  // taken into account!!
  double trackError2 =
      ROOT::Math::Similarity(jj, closestToJetState.cartesianError().matrix());
  double vertexError2 = ROOT::Math::Similarity(j, vertex.covariance());

  double decayLenError = sqrt(trackError2 + vertexError2);

  return pair<bool, Measurement1D>(true,
                                   Measurement1D(decayLen, decayLenError));
}

pair<bool, Measurement1D> linearizedSignedImpactParameter3D(
    const TrajectoryStateOnSurface &closestToJetState,
    const GlobalVector &direction, const Vertex &vertex) {
  // Check if extrapolation has been successfull
  if (!closestToJetState.isValid()) {
    return pair<bool, Measurement1D>(false, Measurement1D(0., 0.));
  ...
  GlobalPoint vertexPosition(vertex.x(), vertex.y(), vertex.z());
  GlobalVector impactParameter =
      linearImpactParameter(closestToJetState, vertexPosition);
  GlobalVector jetDir = direction.unit();
  GlobalVector flightDistance(closestToJetState.globalPosition() -
                              vertexPosition);
  double theDistanceAlongJetAxis = jetDir.dot(flightDistance);
  double signedIP =
      impactParameter.mag() *
      ((theDistanceAlongJetAxis != 0)
           ? theDistanceAlongJetAxis / abs(theDistanceAlongJetAxis)
           : 1.);
  ...
  deriv[3] = -(momentumDir.dot(flightDistance) * ipDirection.x()) /
             momentumAtClosestPoint.mag();
  deriv[4] = -(momentumDir.dot(flightDistance) * ipDirection.y()) /
             momentumAtClosestPoint.mag();
  deriv[5] = -(momentumDir.dot(flightDistance) * ipDirection.z()) /
             momentumAtClosestPoint.mag();

  double trackError2 = ROOT::Math::Similarity(
      deriv, closestToJetState.cartesianError().matrix());
...
closestApproachToJet(const TrajectoryStateOnSurface &state,
                     const Vertex &vertex, const GlobalVector &direction,
...
/**
 * Compute the impact parameter of a track, linearized from the given state,
 * with respect to a given point
 */
GlobalVector linearImpactParameter(const TrajectoryStateOnSurface &state,
