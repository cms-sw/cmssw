...
public:
  ...
  GlobalTrajectoryParameters()
      : theField(nullptr), theX(), theP(), theCharge(0) {
  ...
  GlobalTrajectoryParameters(const GlobalPoint &aX, const GlobalVector &aP,
      ...
      : theField(fieldProvider), theX(aX), theP(aP),
        cachedMagneticField(fieldValue), theCharge(aCharge) {}
  ...
  /** Transverse curvature kappa (which is the inverse radius of curvature in
   * the transverse plane) in cm^{-1}. Sign convention is such that positive
   * kappa means counterclockwise rotation of the track with respect to the
   * global z-axis.
   */

  float transverseCurvature() const {
    return -2.99792458e-3f * signedInverseTransverseMomentum() *
           cachedMagneticField.z();
  }
  ...
  AlgebraicVector6 vector() const {
    return AlgebraicVector6(theX.x(), theX.y(), theX.z(), theP.x(), theP.y(),
                            theP.z());
  }
