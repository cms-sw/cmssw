...
/** Class providing access to the covariance matrix of a set of relevant
 * parameters of a trajectory in a local, Cartesian frame. The errors provided
 * are: <BR> <BR>
 *
 *  sigma^2(q/p) : charge (plus or minus one) divided by magnitude of momentum
 * <BR> sigma^2(dxdz) : direction tangent in local xz-plane <BR> sigma^2(dydz) :
 * direction tangent in local yz-plane <BR> sigma^2(x) : local x-coordinate <BR>
...
public:
  ...
  LocalTrajectoryError(InvalidError)
      : theCovarianceMatrix(ROOT::Math::SMatrixNoInit()) {
  ...
  bool posDef() const {
    return (theCovarianceMatrix(0, 0) >= 0) &&
           (theCovarianceMatrix(1, 1) >= 0) &&
           (theCovarianceMatrix(2, 2) >= 0) &&
           (theCovarianceMatrix(3, 3) >= 0) && (theCovarianceMatrix(4, 4) >= 0);
  }

  /** Constructing class from a full covariance matrix. The sequence of the
   * parameters is the same as the one described above.
   */
  ...
  /** Constructing class from standard deviations of the individual parameters,
   * making the covariance matrix diagonal. The sequence of the input parameters
   * is sigma(x), sigma(y), sigma(dxdz), sigma(dydz), sigma(q/p), but the
   * resulting covariance matrix has the same structure as the one described
   * above.
   */
  ...
  /** Enables the multiplication of the covariance matrix with the scalar
   * "factor".
   */
  ...
  /** Returns the two-by-two submatrix of the covariance matrix which yields the
   * local position errors as well as the correlation between them.
   */
