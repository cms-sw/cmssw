...
secondOrderAccurate(const GlobalPoint &startingPos,
  ...
  LocalPoint thePos(x * cosPhi - y * sinPhi, x * sinPhi + y * cosPhi, 0);
...
void crossing1() {
  ...
  Surface::RotationType rot(0.995292, 0.0969201, 0.000255868, 8.57131e-06,
                            0.00255196, -0.999997, -0.0969205, 0.995289,
                            0.00253912);
  ...
  HelixBarrelPlaneCrossingByCircle precise(startingPos, startingDir, rho,
                                           alongMomentum);
  bool cross;
  double s;
  std::tie(cross, s) = precise.pathLength(plane);

  HelixBarrelPlaneCrossing2OrderLocal crossing(startingPos, startingDir, rho,
                                               plane);

  std::cout << plane.toLocal(GlobalPoint(precise.position(s))) << " "
            << plane.toLocal(GlobalVector(precise.direction(s))) << std::endl;
  std::cout << HelixBarrelPlaneCrossing2OrderLocal::positionOnly(
                   startingPos, startingDir, rho, plane)
            << ' ';
  std::cout << crossing.position() << ' ' << crossing.direction() << std::endl;

  LocalPoint thePos;
  LocalVector theDir;
  std::tie(thePos, theDir) =
      secondOrderAccurate(startingPos, startingDir, rho, plane);
...
void crossing2() {
  ...
  Surface::RotationType rot(0.995041, 0.0994701, 0.000124443, 0.000108324,
                            -0.00233467, 0.999997, 0.0994701, -0.995038,
                            -0.00233387);
  std::cout << rot << std::endl;
  ...
  std::cout << s << ' ' << precise.position(s) << ' ' << precise.direction(s)
            << std::endl;
  std::cout << plane.toLocal(GlobalPoint(precise.position(s))) << " "
            << plane.toLocal(GlobalVector(precise.direction(s))) << std::endl;

  {
    ...
    OptimalHelixPlaneCrossing ocrossing(
        plane, HelixPlaneCrossing::PositionType(startingPos),
        HelixPlaneCrossing::DirectionType(startingDir), rho, alongMomentum);
    ...
    if (!cross)
      ...
      std::cout << s << ' ' << crossing.position(s) << ' '
                << crossing.direction(s) << std::endl;
      std::cout << plane.toLocal(GlobalPoint(crossing.position(s))) << " "
                << plane.toLocal(GlobalVector(crossing.direction(s)))
                << std::endl;
    }
  }
...
void crossing3() {
  ...
  Surface::RotationType rot(-0.808996, 0.587813, 1.16466e-16, 0.587813,
                            0.808996, -3.78444e-17, -1.16466e-16, 3.78444e-17,
                            -1);
  ...
  if (!cross)
    ...
    std::cout << crossing.position(s) << ' ' << crossing.direction(s)
              << std::endl;
    std::cout << plane.toLocal(GlobalPoint(crossing.position(s))) << " "
              << plane.toLocal(GlobalVector(crossing.direction(s)))
              << std::endl;
