...
public:
  AnalyticalPropagator(const MagneticField *field,
      ...
      : Propagator(dir), theMaxDPhi2(maxDPhi * maxDPhi), theMaxDBzRatio(0.5),
        theField(field), isOldPropagationType(isOld) {}
...
public:
   ...
   * NB: this propagator assumes constant, non-zero magnetic field parallel to
   *the z-axis!
   **/
...
private:
  ...
  /// parameter propagation to cylinder (returns position, momentum and path
  /// length)
