...
public:
  ...
  HelixArbitraryPlaneCrossing2Order(
      ...
      const PropagationDirection propDir = alongMomentum)
      : theX0(x0), theY0(y0), theZ0(z0), theCosPhi0(cosPhi0),
        theSinPhi0(sinPhi0), theCosTheta(cosTheta), theSinThetaI(1. / sinTheta),
  ...
  inline double smallestPathLength(const double firstPathLength,
    ...
    return fabs(firstPathLength) < fabs(secondPathLength) ? firstPathLength
                                                          : secondPathLength;
  }
...
private:
  ...
  solutionByDirection(const double dS1, const double dS2) const dso_internal;
