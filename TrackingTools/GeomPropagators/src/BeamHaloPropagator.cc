...
void BeamHaloPropagator::directionCheck(PropagationDirection dir) {
  ...
  if (getEndCapTkPropagator()->propagationDirection() != dir &&
    ...
    edm::LogError("BeamHaloPropagator")
        << "composite propagator set with inconsistent direction components\n"
        << "EndCap propagator is: "
        << getEndCapTkPropagator()->propagationDirection()
        << "\n to be set on: " << dir;
  ...
  if (getCrossTkPropagator()->propagationDirection() != dir &&
    ...
    edm::LogError("BeamHaloPropagator")
        << "composite propagator set with inconsistent direction components\n"
        << "Cross propagator is: "
        << getCrossTkPropagator()->propagationDirection()
        << "\n to be set on: " << dir;
...
BeamHaloPropagator::BeamHaloPropagator(const Propagator *aEndCapTkProp,
    ...
    : Propagator(dir), theEndCapTkProp(aEndCapTkProp->clone()),
      theCrossTkProp(aCrossTkProp->clone()), theField(field) {
  directionCheck(dir);
}
...
BeamHaloPropagator::BeamHaloPropagator(const BeamHaloPropagator &aProp)
    : Propagator(aProp.propagationDirection()), theEndCapTkProp(nullptr),
      theCrossTkProp(nullptr) {
...
bool BeamHaloPropagator::crossingTk(const FreeTrajectoryState &fts,
  ...
  LogDebug("BeamHaloPropagator")
      << "going from: " << fts.position() << " to: " << plane.position() << "\n"
      << "and hence "
      << ((fts.position().z() * plane.position().z() < 0) ? "crossing"
                                                          : "not crossing");
