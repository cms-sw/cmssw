...
TrajectoryStateOnSurface AnalyticalTrajectoryExtrapolatorToLine::extrapolate(
    const FreeTrajectoryState &fts, const Line &line) const {
...
AnalyticalTrajectoryExtrapolatorToLine::extrapolateSingleState(
  ...
  //   detailedDetTimer("AnalyticalTrajectoryExtrapolatorToLine"); TimeMe
  //   t(timer,false);
  //
  ...
  //
    ...
    IterativeHelixExtrapolatorToLine extrapolator(helixPos, helixDir, rho,
                                                  anyDirection);
  ...
  //
  GlobalTrajectoryParameters gtp(x, p, fts.charge(),
  ...
  if (fts.hasError()) {
    ...
    AnalyticalCurvilinearJacobian analyticalJacobian(
        fts.parameters(), gtp.position(), gtp.momentum(), s);
...
bool AnalyticalTrajectoryExtrapolatorToLine::propagateWithHelix(
    const IterativeHelixExtrapolatorToLine &extrapolator, const Line &line,
