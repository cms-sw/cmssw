...
AnalyticalPropagator::propagateWithPath(const FreeTrajectoryState &fts,
  ...
  if
    UNLIKELY(std::abs(gtp.transverseCurvature() - rho) >
             theMaxDBzRatio * std::abs(rho))
  return TsosWP(TrajectoryStateOnSurface(), 0.);
  //
...
AnalyticalPropagator::propagateWithPath(const FreeTrajectoryState &fts,
  ...
  bool parametersOK =
      this->propagateParametersOnCylinder(fts, cylinder, x, p, s);
  // check status and deltaPhi limit
  float dphi2 = s * rho;
  dphi2 = dphi2 * dphi2 * fts.momentum().perp2();
  if
  ...
  return TsosWP(TrajectoryStateOnSurface(), 0.);
  //
  // create result TSOS on TangentPlane (local parameters & errors are better
  // defined)
  //
...
AnalyticalPropagator::propagatedStateWithPath(
    const FreeTrajectoryState &fts, const Surface &surface,
    const GlobalTrajectoryParameters &gtp, const double &s) const {
  //
  // for forward propagation: state is before surface,
  // for backward propagation: state is after surface
  //
  SurfaceSide side =
      PropagationDirectionFromPath()(s, propagationDirection()) == alongMomentum
          ? beforeSurface
          : afterSurface;
  //
  //
  ...
  if (fts.hasError()) {
    ...
    // CurvilinearTrajectoryError cte(ROOT::Math::Similarity(jacobian,
...
bool AnalyticalPropagator::propagateParametersOnCylinder(
    const FreeTrajectoryState &fts, const Cylinder &cylinder, GlobalPoint &x,
    GlobalVector &p, double &s) const {

  GlobalPoint const &sp = cylinder.position();
  if
    UNLIKELY(sp.x() != 0. || sp.y() != 0.) {
  ...
  //
  HelixBarrelCylinderCrossing cylinderCrossing(
      fts.position(), fts.momentum(), rho, propagationDirection(), cylinder);
  if
...
bool AnalyticalPropagator::propagateParametersOnPlane(
    const FreeTrajectoryState &fts, const Plane &plane, GlobalPoint &x,
    GlobalVector &p, double &s) const {
  ...
  if
    LIKELY(isOldPropagationType) {
      OptimalHelixPlaneCrossing planeCrossing(plane, helixPos, helixDir, rho,
                                              propagationDirection());
      return propagateWithHelixCrossing(*planeCrossing, plane,
                                        fts.momentum().mag(), x, p, s);
    }

  //--- Alternative implementation to be used for the propagation of the
  // parameters  of looping
  //    particles that cross twice the (infinite) surface of the plane. It is
  //    not trivial to determine which of the two intersections has to be
  //    returned.

  //---- FIXME: WHAT FOLLOWS HAS TO BE REWRITTEN IN A CLEANER (AND
  // CPU-OPTIMIZED) WAY ---------
  LogDebug("AnalyticalPropagator")
  ...
  OptimalHelixPlaneCrossing planeCrossing1(plane, helixPos1, helixDir1, rho1,
                                           propagationDirection());
  ...
  bool check1 = propagateWithHelixCrossing(*planeCrossing1, plane,
                                           fts.momentum().mag(), gp1, gm1, s1);
  double dphi1 = fabs(fts.momentum().phi() - gm1.phi());
  LogDebug("AnalyticalPropagator")
      << "check1, s1, dphi, gp1: " << check1 << " , " << s1 << " , " << dphi1
      << " , " << gp1 << "\n";

  // move forward a bit to avoid that the propagator doesn't propagate because
  // the state is already on surface. we want to go to the other point of
  // intersection between the helix and the plane
  xGen = (*planeCrossing1).position(s1 + tolerance);
  pGen = (*planeCrossing1).direction(s1 + tolerance);

  /*
    if(!check1 || s1>170 ){
    //PropagationDirection newDir = (propagationDirection() == alongMomentum) ?
    oppositeToMomentum : alongMomentum; PropagationDirection newDir =
    anyDirection; HelixArbitraryPlaneCrossing
    planeCrossing1B(helixPos1,helixDir1,rho1,newDir); check1 =
    propagateWithHelixCrossing(planeCrossing1B,plane,fts.momentum().mag(),gp1,gm1,s1);
  ...
  OptimalHelixPlaneCrossing planeCrossing2(plane, helixPos2, helixDir2, rho2,
                                           propagationDirection());

  bool check2 = propagateWithHelixCrossing(*planeCrossing2, plane, gm2.mag(),
                                           gp2, gm2, s2);
  ...
  if (!check1) {
    edm::LogError("AnalyticalPropagator")
  ...
  LogDebug("AnalyticalPropagator")
      << "check2, s2, gp2: " << check2 << " , " << s2 << " , " << gp2 << "\n";

  double dist1 = (plane.position() - gp1).perp();
  double dist2 = (plane.position() - gp2).perp();

  LogDebug("AnalyticalPropagator")
      << "propDir, dist1, dist2: " << propagationDirection() << " , " << dist1
      << " , " << dist2 << "\n";

  // If there are two solutions, the one which is the closest to the module's
  // center is chosen
  if (dist1 < 2 * dist2) {
    x = gp1;
    p = gm1;
...
bool AnalyticalPropagator::propagateWithLineCrossing(const GlobalPoint &x0,
  ...
  // in the constructor).
  //
  StraightLineBarrelCylinderCrossing cylCrossing(x0, p0,
                                                 propagationDirection());
  //
  // get solution
  //
...
bool AnalyticalPropagator::propagateWithHelixCrossing(
    HelixPlaneCrossing &planeCrossing, const Plane &plane, const float pmag,
