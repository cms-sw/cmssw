...
HelixArbitraryPlaneCrossing2Order::HelixArbitraryPlaneCrossing2Order(
  ...
  theCosTheta = pz * pI;
  theSinThetaI =
      p2 * pI * ptI; //  (1/(pt/p)) = p/pt = p*ptI and p = p2/p = p2*pI
}
...
HelixArbitraryPlaneCrossing2Order::pathLength(const Plane &plane) {
  ...
  double ceq2 =
      nPx * theCosPhi0 + nPy * theSinPhi0 + nPz * theCosTheta * theSinThetaI;
  double ceq3 = cP;
  //
...
HelixArbitraryPlaneCrossing2Order::positionInDouble(double s) const {
  ...
  return PositionTypeDouble(
      theX0 + (theCosPhi0 - (st * 0.5 * theRho) * theSinPhi0) * st,
...
HelixArbitraryPlaneCrossing2Order::directionInDouble(double s) const {
  ...
  return DirectionTypeDouble(
      theCosPhi0 - (theSinPhi0 + 0.5 * dph * theCosPhi0) * dph,
