...
SmartPropagator::SmartPropagator(const Propagator *aTkProp,
    ...
    : Propagator(dir), theTkProp(aTkProp->clone()),
      theGenProp(aGenProp->clone()), theField(field) {
...
SmartPropagator::SmartPropagator(const Propagator &aTkProp,
                                 ...
                                 const MagneticField *field,
                                 PropagationDirection dir, float epsilon)
    : Propagator(dir), theTkProp(aTkProp.clone()), theGenProp(aGenProp.clone()),
      theField(field) {
...
SmartPropagator::SmartPropagator(const SmartPropagator &aProp)
    : Propagator(aProp.propagationDirection()), theTkProp(nullptr),
      theGenProp(nullptr) {
  if (aProp.theTkProp)
...
void SmartPropagator::initTkVolume(float epsilon) {
  ...
  theTkVolume = Cylinder::build(
      radius, pos, rot, new SimpleCylinderBounds(r_in, r_out, z_min, z_max));
}
...
bool SmartPropagator::insideTkVol(const FreeTrajectoryState &fts) const {
  ...
  return ((gp.perp() <= TrackerBounds::radius() + 10.) &&
          (fabs(gp.z()) <= TrackerBounds::halfLength() + 10.));
}
