...
IterativeHelixExtrapolatorToLine::IterativeHelixExtrapolatorToLine(
    ...
    const float curvature, const PropagationDirection propDir)
    : theX0(point.x()), theY0(point.y()), theZ0(point.z()), theRho(curvature),
      theQuadraticSolutionFromStart(point, direction, curvature, propDir),
      thePropDir(propDir), theCachedS(0), theCachedDPhi(0.), theCachedSDPhi(0.),
      theCachedCDPhi(1.) {
  //
...
IterativeHelixExtrapolatorToLine::pathLength(const GlobalPoint &point) const {
 ...
 */
std::pair<bool, double>
IterativeHelixExtrapolatorToLine::pathLength(const Line &line) const {
...
IterativeHelixExtrapolatorToLine::genericPathLength(const T &object) const {
  ...
  //
  // Prepare internal value of the propagation direction and position /
  ...
  while (true) {
    //
    // return empty solution vector if no convergence after maxIterations
    // iterations
    //
    if (--iteration == 0) {
    ...
    if (first) {
      ...
      HelixExtrapolatorToLine2Order linearCrossing(
          xnew.x(), xnew.y(), xnew.z(), pnew.x(), pnew.y(), theCosTheta,
          theSinTheta, theRho, anyDirection);
    ...
    PropagationDirection newDir =
        dSTotal >= 0 ? alongMomentum : oppositeToMomentum;
...
IterativeHelixExtrapolatorToLine::positionInDouble(double s) const {
  ...
  if (fabs(theCachedDPhi) > 1.e-4) {
    ...
    return PositionTypeDouble(theX0 + (-theSinPhi0 * (1. - theCachedCDPhi) +
                                       theCosPhi0 * theCachedSDPhi) /
                                          theRho,
                              theY0 + (theCosPhi0 * (1. - theCachedCDPhi) +
                                       theSinPhi0 * theCachedSDPhi) /
                                          theRho,
  ...
  //      return
  //      PositionTypeDouble(theX0+(-theSinPhi0*theCachedSDPhi*theCachedSDPhi/(1.+theCachedCDPhi)+
...
IterativeHelixExtrapolatorToLine::directionInDouble(double s) const {
  ...
  if (fabs(theCachedDPhi) > 1.e-4) {
    // full helix formula
    return DirectionTypeDouble(
        theCosPhi0 * theCachedCDPhi - theSinPhi0 * theCachedSDPhi,
