...
StraightLinePropagator::propagateWithPath(const FreeTrajectoryState &fts,
  ...
  if (fts.hasError()) {
    ...
    return std::make_pair(
        TSOS(LocalTrajectoryParameters(x, p, fts.charge()), plane, theField),
        s);
  }
}
...
StraightLinePropagator::propagateWithPath(const FreeTrajectoryState &fts,
  ...
  if (fts.hasError()) {
    ...
    // return state without errors
    return std::make_pair(
        TSOS(GlobalTrajectoryParameters(x, p, fts.charge(), theField),
             cylinder),
        s);
  }
}

TrajectoryStateOnSurface StraightLinePropagator::propagatedState(
    const FTS &fts, const Surface &surface, const AlgebraicMatrix55 &jacobian,
    const LocalPoint &x, const LocalVector &p) const {
  if (fts.hasError()) {
    // propagate error
    ...
    // return state without errors
    return TSOS(LocalTrajectoryParameters(x, p, fts.charge()), surface,
                theField);
  }
}
...
TrajectoryStateOnSurface StraightLinePropagator::propagatedState(
    const FTS &fts, const Surface &surface, const AlgebraicMatrix55 &jacobian,
    const GlobalPoint &x, const GlobalVector &p) const {
  ...
  if (fts.hasError()) {
    ...
    // return state without errors
    return TSOS(GlobalTrajectoryParameters(x, p, fts.charge(), theField),
                surface);
  }
}
...
bool StraightLinePropagator::propagateParametersOnCylinder(
    const FTS &fts, const Cylinder &cylinder, GlobalPoint &x, GlobalVector &p,
    double &s) const {
...
bool StraightLinePropagator::propagateParametersOnPlane(const FTS &fts,
  ...
  x = LocalPoint(x.x() + (p.x() / p.z()) * s, x.y() + (p.y() / p.z()) * s,
                 x.z() + s);
