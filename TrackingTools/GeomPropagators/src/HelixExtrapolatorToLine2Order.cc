...
HelixExtrapolatorToLine2Order::pathLength(const Line &line) const {
  ...
  DirectionTypeDouble helix1p(theDirection -
                              lineDirection * theDirection.dot(lineDirection));
  DirectionTypeDouble helix2p(helix2 -
                              lineDirection * helix2.dot(lineDirection));
  //
  // coefficients of 3rd order equation
  //
  double ceq[4];
...
int HelixExtrapolatorToLine2Order::solve3rdOrder(const double ceq[],
  ...
  if (fabs(ceq[3]) > FLT_MIN) {
    ...
    double r = (2 * ceq[2] * ceq[2] * ceq[2] - 9 * ceq[3] * ceq[2] * ceq[1] +
                27 * ceq[3] * ceq[3] * ceq[0]) /
...
int HelixExtrapolatorToLine2Order::solve2ndOrder(const double coeff[],
                                                 double solutions[]) const {
  //
...
HelixExtrapolatorToLine2Order::positionInDouble(double s) const {
  ...
  return PositionTypeDouble(
      thePosition.x() +
          (theDirection.x() - st * 0.5 * theRho * theDirection.y()) * st,
      thePosition.y() +
          (theDirection.y() + st * 0.5 * theRho * theDirection.x()) * st,
...
HelixExtrapolatorToLine2Order::directionInDouble(double s) const {
  ...
  return DirectionTypeDouble(
      theDirection.x() -
          (theDirection.y() + 0.5 * theDirection.x() * dph) * dph,
      theDirection.y() +
          (theDirection.x() - 0.5 * theDirection.y() * dph) * dph,
      theDirection.z());
}
