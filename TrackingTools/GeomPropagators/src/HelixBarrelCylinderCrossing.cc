...
HelixBarrelCylinderCrossing::HelixBarrelCylinderCrossing(
    const GlobalPoint &startingPos, const GlobalVector &startingDir, double rho,
    PropagationDirection propDir, const Cylinder &cyl, Solution sol) {
  ...
  thePos = GlobalPoint(startingPos.x() + theD.x(), startingPos.y() + theD.y(),
  ...
  thePos1 = GlobalPoint(startingPos.x() + d1.x(), startingPos.y() + d1.y(),
  ...
  thePos2 = GlobalPoint(startingPos.x() + d2.x(), startingPos.y() + d2.y(),
                        startingPos.z() + theS2 * cosTheta);
}

std::pair<HelixBarrelCylinderCrossing::Vector, int>
HelixBarrelCylinderCrossing::chooseSolution(const Vector &d1, const Vector &d2,
