...
HelixArbitraryPlaneCrossing::pathLength(const Plane &plane) {
  ...
  float safeMaxDist =
      (theMaxDistToPlane > maxNumDz ? theMaxDistToPlane : maxNumDz);
  //
  ...
  while (notAtSurface(plane, xnew, safeMaxDist)) {
    ...
    if
      UNLIKELY(--iteration == 0) {
        LogDebug("HelixArbitraryPlaneCrossing")
...
//   Iteration control: continue if distance to plane > theMaxDistToPlane.
//   Includes protection for numerical precision (Surfaces work with single
//   precision).
bool HelixArbitraryPlaneCrossing::notAtSurface(const Plane &plane,
                                               const PositionTypeDouble &point,
                                               const float maxDist) const {
  float dz =
      plane.localZ(Surface::GlobalPoint(point.x(), point.y(), point.z()));
