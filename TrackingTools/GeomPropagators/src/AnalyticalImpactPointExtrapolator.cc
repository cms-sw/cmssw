...
AnalyticalImpactPointExtrapolator::AnalyticalImpactPointExtrapolator(
    const Propagator &propagator, const MagneticField *field)
...
AnalyticalImpactPointExtrapolator::extrapolate(const FreeTrajectoryState &fts,
  ...
  //   detailedDetTimer("AnalyticalImpactPointExtrapolator"); TimeMe
  //   t(timer,false);

  return extrapolateSingleState(fts, vtx);
}

TrajectoryStateOnSurface AnalyticalImpactPointExtrapolator::extrapolate(
    const TrajectoryStateOnSurface tsos, const GlobalPoint &vtx) const {
  if (tsos.isValid())
...
AnalyticalImpactPointExtrapolator::extrapolateFullState(
    const TrajectoryStateOnSurface tsos, const GlobalPoint &vertex) const {
  //
...
AnalyticalImpactPointExtrapolator::extrapolateSingleState(
    const FreeTrajectoryState &fts, const GlobalPoint &vertex) const {
  //
  ...
  if (fts.hasError()) {
    ...
    CurvilinearTrajectoryError cte(ROOT::Math::Similarity(
        analyticalJacobian.jacobian(), fts.curvilinearError().matrix()));
...
bool AnalyticalImpactPointExtrapolator::propagateWithHelix(
    const IterativeHelixExtrapolatorToLine &extrapolator,
    const GlobalPoint &vertex, GlobalPoint &x, GlobalVector &p,
    double &s) const {
  //
