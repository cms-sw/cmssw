...
void TrackDetectorAssociator::fillEcal(const edm::Event& iEvent,
  ...
  for (auto crossedEcalId : crossedEcalIds) {
    auto ebHit = (*EBRecHits).find(crossedEcalId);
    auto eeHit = (*EERecHits).find(crossedEcalId);
...
void TrackDetectorAssociator::fillCaloTowers(const edm::Event& iEvent,
  ...
  for (auto crossedCaloTowerId : crossedCaloTowerIds) {
    auto tower = (*caloTowers).find(crossedCaloTowerId);
...
void TrackDetectorAssociator::fillHcal(const edm::Event& iEvent,
  ...
  for (auto crossedId : crossedIds) {
    auto hit = (*collection).find(crossedId);
