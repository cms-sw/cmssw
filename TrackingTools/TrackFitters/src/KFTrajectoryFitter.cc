						...
						const TSOS& firstPredTsos) const 
{
  ...
  if (firsthit.isValid()){
    ...
    LogDebug("TrackFitters")
      ...
      <<" currTsos !"<<"\n"
      <<currTsos<<"\n";
    LogDebug("TrackFitters") <<"  GOING TO examine hit "<<(firsthit).geographicalId().rawId()<<"\n";
    if ((firsthit).geographicalId().subdetId() == StripSubdetector::TIB ) {
      LogDebug("TrackFitters") <<" I am TIB "<<TIBDetId((firsthit).geographicalId()).layer()<<"\n";
    }else if ((firsthit).geographicalId().subdetId() == StripSubdetector::TOB ) { 
      LogDebug("TrackFitters") <<" I am TOB "<<TOBDetId((firsthit).geographicalId()).layer()<<"\n";
    }else if ((firsthit).geographicalId().subdetId() == StripSubdetector::TEC ) { 
      LogDebug("TrackFitters") <<" I am TEC "<<TECDetId((firsthit).geographicalId()).wheel()<<"\n";
    }else if ((firsthit).geographicalId().subdetId() == StripSubdetector::TID ) { 
      LogDebug("TrackFitters") <<" I am TID "<<TIDDetId((firsthit).geographicalId()).wheel()<<"\n";
    }else{
      LogDebug("TrackFitters") <<" I am Pixel "<<"\n";
    }
  }
  ...
  for(RecHitContainer::const_iterator ihit = hits.begin() + 1; 
	...
	<<(**ihit).det()->surface().rotation()<<"\n";
      LogDebug("TrackFitters") <<" GOING TO examine hit "<<(**ihit).geographicalId().rawId()<<"\n";
      if ((**ihit).geographicalId().subdetId() == StripSubdetector::TIB ) {
	LogDebug("TrackFitters") <<" I am TIB "<<TIBDetId((**ihit).geographicalId()).layer()<<"\n";
      }else if ((**ihit).geographicalId().subdetId() == StripSubdetector::TOB ) { 
	LogDebug("TrackFitters") <<" I am TOB "<<TOBDetId((**ihit).geographicalId()).layer()<<"\n";
      }else if ((**ihit).geographicalId().subdetId() == StripSubdetector::TEC ) { 
	LogDebug("TrackFitters") <<" I am TEC "<<TECDetId((**ihit).geographicalId()).wheel()<<"\n";
      }else if ((**ihit).geographicalId().subdetId() == StripSubdetector::TID ) { 
	LogDebug("TrackFitters") <<" I am TID "<<TIDDetId((**ihit).geographicalId()).wheel()<<"\n";
      }else{
	LogDebug("TrackFitters") <<" I am Pixel "<<"\n";
      }
    }
    ...
    if((**ihit).isValid()) {
      //update
      LogDebug("TrackFitters") <<"THE HIT IS VALID: updating predTsos"<<"\n";
