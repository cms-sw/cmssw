...
void KFSplittingFitter::sorter(const RecHitContainer& hits, PropagationDirection dir, RecHitContainer& result) const {
  ...
  for (auto ihit = hits.begin(); ihit != hits.end(); ihit++) {
    if (!(**ihit).isValid())
      ...
      for (auto srt = sorted.begin(); srt != sorted.end(); srt++) {
...
Trajectory KFSplittingFitter::fitOne(const Trajectory& aTraj, fitType type) const {
  ...
  TSOS firstTsos = TrajectoryStateWithArbitraryError()(firstTM.predictedState());
...
Trajectory KFSplittingFitter::fitOne(const TrajectorySeed& aSeed, const RecHitContainer& hits, fitType type) const {
  RecHitContainer result;
