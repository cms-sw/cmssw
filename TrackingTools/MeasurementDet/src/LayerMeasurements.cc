...
inline void addInvalidMeas(std::vector<TrajectoryMeasurement> &result,
                           ...
                           const GeomDet *det, const DetLayer &layer) {
  result.emplace_back(
      ts,
      std::make_shared<InvalidTrackingRecHit>(*det, TrackingRecHit::missing),
      0.F, &layer);
}

/** The std::vector<DetWithState> passed to this method should not be empty.
 *  In case of no compatible dets the result should be either an empty container
 * if the det is itself incompatible, or a container with one invalid
 * measurement on the det surface. The method does not have enough information
 * to do this efficiently, so it should be done by the caller, or an exception
 * will be thrown (DetLogicError).
 */
inline std::vector<TrajectoryMeasurement>
get(const MeasurementDetSystem *theDetSystem,
    const MeasurementTrackerEvent *theData, const DetLayer &layer,
    std::vector<DetWithState> const &compatDets,
    const TrajectoryStateOnSurface &ts, const Propagator &prop,
  ...
  for (auto const &ds : compatDets) {
    MeasurementDetWithData mdet =
        theDetSystem->idToDet(ds.first->geographicalId(), *theData);
    if
      UNLIKELY(mdet.isNull()) {
    ...
    if (mdet.measurements(ds.second, est, tmps))
      for (std::size_t i = 0; i != tmps.size(); ++i)
        result.emplace_back(ds.second, std::move(tmps.hits[i]),
                            tmps.distances[i], &layer);
    tmps.clear();
  }
  // WARNING: we might end up with more than one invalid hit of type 'inactive'
  // in result to be fixed in order to avoid usless double traj candidates.
  ...
  if (!result.empty()) {
    ...
    addInvalidMeas(result, result.front().predictedState(),
...
vector<TrajectoryMeasurement> LayerMeasurements::measurements(
    const DetLayer &layer, const TrajectoryStateOnSurface &startingState,
    const Propagator &prop, const MeasurementEstimator &est) const {

  typedef DetLayer::DetWithState DetWithState;

  vector<DetWithState> const &compatDets =
      layer.compatibleDets(startingState, prop, est);

  if (!compatDets.empty())
    return get(theDetSystem, theData, layer, compatDets, startingState, prop,
               est);
  ...
  if (compat.first) {
    result.push_back(
        TrajectoryMeasurement(compat.second,
                              std::make_shared<InvalidTrackingRecHitNoDet>(
                                  layer.surface(), TrackingRecHit::inactive),
                              0.F, &layer));
...
vector<TrajectoryMeasurementGroup> LayerMeasurements::groupedMeasurements(
    const DetLayer &layer, const TrajectoryStateOnSurface &startingState,
    const Propagator &prop, const MeasurementEstimator &est) const {
  vector<TrajectoryMeasurementGroup> result;

  vector<DetGroup> &&groups =
  ...
  for (auto &grp : groups) {
    ...
    for (auto const &det : grp) {
      MeasurementDetWithData mdet =
          theDetSystem->idToDet(det.det()->geographicalId(), *theData);
      if (mdet.isNull()) {
        throw MeasurementDetException("MeasurementDet not found");
      }
      if (mdet.measurements(det.trajectoryState(), est, tmps))
        for (std::size_t i = 0; i != tmps.size(); ++i)
          tmpVec.emplace_back(det.trajectoryState(), std::move(tmps.hits[i]),
                              tmps.distances[i], &layer);
      tmps.clear();
    }

    // sort the final result
    LogDebug("LayerMeasurements")
        << "Sorting " << tmpVec.size() << " measurements in this grp.";
  ...
  // if the result is empty check if the layer is compatible (for invalid
  // measurement)
  if (result.empty()) {
    pair<bool, TrajectoryStateOnSurface> compat =
        layer.compatible(startingState, prop, est);
...
void LayerMeasurements::addInvalidMeas(vector<TrajectoryMeasurement> &measVec,
  ...
  if (!measVec.empty()) {
    ...
    auto toll =
        measVec.front().recHitR().det()->surface().bounds().significanceInside(
            ts.localPosition(), ts.localError().positionError());
    measVec.emplace_back(
        measVec.front().predictedState(),
        std::make_shared<InvalidTrackingRecHit>(
            *measVec.front().recHitR().det(), TrackingRecHit::missing),
        toll, &layer);
  } else if (!group.empty()) {
    // invalid state on first compatible Det
    auto const &ts = group.front().trajectoryState();
    auto toll = group.front().det()->surface().bounds().significanceInside(
        ts.localPosition(), ts.localError().positionError());
    measVec.emplace_back(group.front().trajectoryState(),
                         std::make_shared<InvalidTrackingRecHit>(
                             *group.front().det(), TrackingRecHit::missing),
                         toll, &layer);
  }
}
