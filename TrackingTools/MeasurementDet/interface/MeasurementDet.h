...
public:
  ...
  MeasurementDet(const GeomDet *gdet)
      : theGeomDet(gdet), theMissingHit(std::make_shared<InvalidTrackingRecHit>(
                              fastGeomDet(), TrackingRecHit::missing)),
        theInactiveHit(std::make_shared<InvalidTrackingRecHit>(
            fastGeomDet(), TrackingRecHit::inactive)) {}

  virtual ~MeasurementDet() = default;
  virtual RecHitContainer recHits(const TrajectoryStateOnSurface &,
                                  const MeasurementTrackerEvent &) const = 0;
   ...
   *  Det is already available. The first TrajectoryStateOnSurface is on the
   * surface of this Det, and the second TrajectoryStateOnSurface is not used,
   * as the propagator... The stateOnThisDet should the result of <BR>
   *  prop.propagate( startingState, this->surface())
   */
  std::vector<TrajectoryMeasurement>
  fastMeasurements(const TrajectoryStateOnSurface &stateOnThisDet,
                   const TrajectoryStateOnSurface &, const Propagator &,
    ...
    for (std::size_t i = 0; i != tmps.size(); ++i) {
      auto j = index[i];
      result.emplace_back(stateOnThisDet, std::move(tmps.hits[j]),
                          tmps.distances[j]);
    }
    return result;
  }
  ...
  virtual bool hasBadComponents(const TrajectoryStateOnSurface &tsos,
                                const MeasurementTrackerEvent &data) const = 0;
