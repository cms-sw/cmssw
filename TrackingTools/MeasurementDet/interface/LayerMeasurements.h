...
public:
  ...
  // dummy default constructor (obviously you can't use any object created this
  // way), but it can be needed in some cases
  LayerMeasurements() : theDetSystem(nullptr), theData(nullptr) {}

  // the constructor that most of the people should be using
  LayerMeasurements(const MeasurementDetSystem &detSystem,
                    const MeasurementTrackerEvent &data)
      : theDetSystem(&detSystem), theData(&data) {}

  // return just valid hits, no sorting (for seeding mostly)
  bool recHits(SimpleHitContainer &result, const DetLayer &layer,
  ...
  std::vector<TrajectoryMeasurementGroup> groupedMeasurements(
  ...
  void addInvalidMeas(std::vector<TrajectoryMeasurement> &measVec,
                      const DetGroup &group, const DetLayer &layer) const;
