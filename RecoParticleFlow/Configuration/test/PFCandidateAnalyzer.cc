...
void PFCandidateAnalyzer::analyze(const Event& iEvent, const EventSetup& iSetup) {
  ...
  for (unsigned i = 0; i < pfCandidates->size(); i++) {
    ...
    if (verbose_) {
      cout << cand << endl;
      if (printBlocks_)
        printElementsInBlocks(cand);
    }
  }

  LogDebug("PFCandidateAnalyzer") << "STOP event: " << iEvent.id().event() << " in run " << iEvent.id().run() << endl;
}

void PFCandidateAnalyzer::printElementsInBlocks(const PFCandidate& cand, ostream& out) const {
  if (!out)
    return;
