...
#define BOOK2D(name, title, nbinsx, lowx, highx, nbinsy, lowy, highy)          \
  h##name =                                                                    \
      dbe_ ? dbe_->book2D(#name, title, nbinsx, lowx, highx, nbinsy, lowy,     \
                          highy)                                               \
                 ->getTH2F()                                                   \
...
/*#define SET2AXES(name,xtitle,ytitle) \
  hE##name->GetXaxis()->SetTitle(xtitle);
  hE##name->GetYaxis()->SetTitle(ytitle);
  hB##name->GetXaxis()->SetTitle(xtitle); hB##name->GetYaxis()->SetTitle(ytitle)
*/
...
void PFJetBenchmark::setup(string Filename, bool debug, bool plotAgainstReco,
                           bool onlyTwoJets, double deltaRMax,
                           string benchmarkLabel_, double recPt, double maxEta,
                           DQMStore *dbe_store) {
  debug_ = debug;
  ...
  dbe_ = dbe_store;
  // print parameters
  cout << "PFJetBenchmark Setup parameters "
          "=============================================="
       << endl;
  ...
  BOOK2D(RNONLvsEta, "R_{HCAL+ECAL - Hcal Only} vs #eta", 200, -5., 5., 200, -1,
         1);
  ...
  DBOOK2D(RPtvsPt, #DeltaP_{T} / P_{T} vs P_{T}, 250, 0, 500, 200, -1,
          1); // used to be 50 bin for each in x-direction
  ...
  DBOOK2D(RNEUTvsP, #DeltaE / E(ECAL + HCAL) vs P, 250, 0, 1000, 150, -1.5,
          1.5);
  DBOOK2D(RNONLvsP, #DeltaE / E(ECAL + HCAL - only) vs P, 250, 0, 1000, 150,
          -1.5, 1.5);
...
void PFJetBenchmark::process(const reco::PFJetCollection &pfJets,
  ...
  for (unsigned i = 0; i < pfJets.size(); i++) {
    ...
    if (deltaR < deltaRMax_ || (abs(rec_eta) > 2.5 && deltaR < 0.2) ||
      ...
      // closest in delta-R) get the quantities to place on the denominator
      // and/or divide by
      double pt_denom;
      ...
      gettrue(truth, true_ChargedHadEnergy, true_NeutralHadEnergy,
      ...
      if (true_ChargedHadEnergy > cut1) {
        resChargedHadEnergy = (rec_ChargedHadEnergy - true_ChargedHadEnergy) /
                              true_ChargedHadEnergy;
        plot2 = true;
      }
      if (true_NeutralHadEnergy > cut2) {
        resNeutralHadEnergy = (rec_NeutralHadEnergy - true_NeutralHadEnergy) /
      ...
      if (true_NeutralEmEnergy > cut4) {
        resNeutralEmEnergy =
            (rec_NeutralEmEnergy - true_NeutralEmEnergy) / true_NeutralEmEnergy;
        plot4 = true;
      }
      if (true_NeutralEnergy > cut5) {
        resNeutralEnergy =
            (rec_NeutralEnergy - true_NeutralEnergy) / true_NeutralEnergy;
        plot5 = true;
      }
      ...
      if ((resPt > 0.2 && true_pt > 100.) || (resPt < -0.5 && true_pt > 100.)) {
        ...
        //       ( truth->eta()<-3.0 && rec_eta-truth->eta() > 0.1 ) ))) {
        std::cout << "Entry " << entry_ << " resPt = " << resPt
                  << " resCharged  " << resChargedHadEnergy
                  << " resNeutralHad  " << resNeutralHadEnergy
                  << " resNeutralEm  " << resNeutralEmEnergy << " pT (T/R) "
                  << true_pt << "/" << rec_pt << " Eta (T/R) " << truth->eta()
                  << "/" << rec_eta << " Phi (T/R) " << truth->phi() << "/"
                  << rec_phi << std::endl;
        ...
        for (unsigned j = 0; j < pfJets.size(); j++) {
          ...
          if (j != i && algo_->deltaR(&pfj, &pfo) < dRo &&
              pfo.pt() > 0.25 * pfj.pt()) {
        ...
        for (unsigned j = 0; j < genJets.size(); j++) {
          ...
          if (gjo != truth && algo_->deltaR(truth, gjo) < dRgo &&
              gjo->pt() > 0.25 * truth->pt()) {
        ...
        if (dRo < 0.8 && dRgo < 0.8 &&
            algo_->deltaR(genoj, pfoj) < 2. * deltaRMax_)
          std::cout << "Excess probably due to overlapping jets (DR = "
                    << algo_->deltaR(genoj, pfoj) << "),"
      ...
      if (debug_) {
        cout << i << "  =========PFJet Pt " << rec_pt << " eta " << rec_eta
             << " phi " << rec_phi << " Charged Had Energy "
             << rec_ChargedHadEnergy << " Neutral Had Energy "
             << rec_NeutralHadEnergy << " Neutral elm Energy "
             << rec_NeutralEmEnergy << endl;
        cout << " matching Gen Jet Pt " << true_pt << " eta " << truth->eta()
             << " phi " << truth->phi() << " Charged Had Energy "
             << true_ChargedHadEnergy << " Neutral Had Energy "
             << true_NeutralHadEnergy << " Neutral elm Energy "
             << true_NeutralEmEnergy << endl;
        printPFJet(&pfj);
        //      cout<<pfj.print()<<endl;
        printGenJet(truth);
        ...
        cout << "==============deltaR " << deltaR << "  resPt " << resPt
             ...
             << " resNeutralEmEnergy " << resNeutralEmEnergy << endl;
      }
...
void PFJetBenchmark::printPFJet(const reco::PFJet *pfj) {
  ...
  cout << "PFJet  p/px/py/pz/pt: " << pfj->p() << "/" << pfj->px() << "/"
       ...
       << "      charged/neutral hadrons energy: " << pfj->chargedHadronEnergy()
       << '/' << pfj->neutralHadronEnergy() << endl
       << "      charged/neutral em energy: " << pfj->chargedEmEnergy() << '/'
       << pfj->neutralEmEnergy() << endl
       << "      charged muon energy: " << pfj->chargedMuEnergy() << '/' << endl
       << "      charged/neutral multiplicity: " << pfj->chargedMultiplicity()
       << '/' << pfj->neutralMultiplicity() << endl;
