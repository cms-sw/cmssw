...
void PFMETBenchmark::setup(string Filename, bool debug, bool plotAgainstReco,
                           string benchmarkLabel_, DQMStore *dbe_store) {
  debug_ = debug;
  ...
  dbe_ = dbe_store;
  // print parameters
  // cout<< "PFMETBenchmark Setup parameters
  // =============================================="<<endl;
  ...
  profileSETvsSETresp = new TProfile("#DeltaPFSET / trueSET vs trueSET", "",
                                     200, 0.0, 1000.0, -1.0, 1.0);
  profileMETvsMETresp = new TProfile("#DeltaPFMET / trueMET vs trueMET", "", 50,
                                     0.0, 200.0, -1.0, 1.0);
  ...
  BOOK2D(CaloSETvsDeltaCaloMET, "Calorimeter", 200, 0.0, 1000.0, 400, -200.0,
         200.0);
  BOOK2D(CaloSETvsDeltaCaloSET, "Calorimeter", 200, 0.0, 1000.0, 400, -200.0,
         200.0);
  profileCaloSETvsCaloSETresp = new TProfile(
      "#DeltaCaloSET / trueSET vs trueSET", "", 200, 0.0, 1000.0, -1.0, 1.0);
  profileCaloMETvsCaloMETresp = new TProfile(
      "#DeltaCaloMET / trueMET vs trueMET", "", 200, 0.0, 200.0, -1.0, 1.0);

  BOOK2D(DeltaPFMETvstrueMET, "Particle Flow", 500, 0.0, 1000.0, 400, -200.0,
         200.0);
  BOOK2D(DeltaCaloMETvstrueMET, "Calorimeter", 500, 0.0, 1000.0, 400, -200.0,
         200.0);
  BOOK2D(DeltaPFPhivstrueMET, "Particle Flow", 500, 0.0, 1000.0, 400, -3.2,
         3.2);
  BOOK2D(DeltaCaloPhivstrueMET, "Calorimeter", 500, 0.0, 1000.0, 400, -3.2,
         3.2);
  ...
  BOOK2D(DeltaCaloMEXvstrueSET, "Calorimeter", 200, 0.0, 1000.0, 400, -200.0,
         200.0);
  BOOK2D(DeltaPFMEXvstrueSET, "Particle Flow", 200, 0.0, 1000.0, 400, -200.0,
         200.0);
  ...
  BOOK2D(TCSETvsDeltaTCMET, "Track Corrected", 200, 0.0, 1000.0, 400, -200.0,
         200.0);
  BOOK2D(TCSETvsDeltaTCSET, "Track Corrected", 200, 0.0, 1000.0, 400, -200.0,
         200.0);
  profileTCSETvsTCSETresp = new TProfile("#DeltaTCSET / trueSET vs trueSET", "",
                                         200, 0.0, 1000.0, -1.0, 1.0);
  profileTCMETvsTCMETresp = new TProfile("#DeltaTCMET / trueMET vs trueMET", "",
                                         200, 0.0, 200.0, -1.0, 1.0);

  BOOK1D(TCMET, "Track Corrected", 500, 0, 1000);
  BOOK2D(DeltaTCMETvstrueMET, "Track Corrected", 500, 0.0, 1000.0, 400, -200.0,
         200.0);
  BOOK2D(DeltaTCPhivstrueMET, "Track Corrected", 500, 0.0, 1000.0, 400, -3.2,
         3.2);

  BOOK2D(DeltaTCMEXvstrueSET, "Track Corrected", 200, 0.0, 1000.0, 400, -200.0,
         200.0);
...
// void PFMETBenchmark::process(const reco::PFMETCollection& pfMets, const
// reco::GenMETCollection& genMets) {
void PFMETBenchmark::process(const reco::PFMETCollection &pfMets,
  ...
  if (true_met > 5.0)
    profileCaloMETvsCaloMETresp->Fill(true_met,
                                      (calo_met - true_met) / true_met);
...
void PFMETBenchmark::calculateQuantities(
    const reco::PFMETCollection &pfMets,
    const reco::GenParticleCollection &genParticleList,
    const reco::CaloMETCollection &caloMets,
    const reco::METCollection &tcMets) {
  ...
  //  for( genParticle = genParticleList.begin(); genParticle !=
  //  genParticleList.end(); genParticle++ )
  for (unsigned i = 0; i < genParticleList.size(); i++) {
    if (genParticleList[i].status() == 1 &&
        fabs(genParticleList[i].eta()) < 5.0) {
...
void PFMETBenchmark::calculateQuantities(
    const reco::PFMETCollection &pfMets,
    const reco::GenParticleCollection &genParticleList,
    const reco::CaloMETCollection &caloMets, const reco::METCollection &tcMets,
  ...
  for (unsigned int caloJetc = 0; caloJetc < caloJets.size(); ++caloJetc) {
    // std::cout << "caloJets[" << caloJetc << "].pt() = " <<
    // caloJets[caloJetc].pt() << std::endl; std::cout << "caloJets[" <<
    // caloJetc
    // << "].phi() = " << caloJets[caloJetc].phi() << std::endl; std::cout <<
    // "caloJets[" << caloJetc << "].eta() = " << caloJets[caloJetc].eta() <<
    // std::endl;
    //}
    for (unsigned int corr_caloJetc = 0; corr_caloJetc < corr_caloJets.size();
         ++corr_caloJetc) {
      // std::cout << "corr_caloJets[" << corr_caloJetc << "].pt() = " <<
      // corr_caloJets[corr_caloJetc].pt() << std::endl; std::cout <<
      // "corr_caloJets[" << corr_caloJetc << "].phi() = " <<
      // corr_caloJets[corr_caloJetc].phi() << std::endl; std::cout <<
      // "corr_caloJets[" << corr_caloJetc << "].eta() = " <<
      // corr_caloJets[corr_caloJetc].eta() << std::endl;
      //}
      Float_t DeltaPhi =
          corr_caloJets[corr_caloJetc].phi() - caloJets[caloJetc].phi();
      Float_t DeltaEta =
          corr_caloJets[corr_caloJetc].eta() - caloJets[caloJetc].eta();
      Float_t DeltaR2 = DeltaPhi * DeltaPhi + DeltaEta * DeltaEta;
      if (DeltaR2 < 0.0001 && caloJets[caloJetc].pt() > 20.0) {
        caloJetCorPX +=
            (corr_caloJets[corr_caloJetc].px() - caloJets[caloJetc].px());
        caloJetCorPY +=
            (corr_caloJets[corr_caloJetc].py() - caloJets[caloJetc].py());
      }
    }
  }
  double corr_calomet =
      sqrt((cm.px() - caloJetCorPX) * (cm.px() - caloJetCorPX) +
           (cm.py() - caloJetCorPY) * (cm.py() - caloJetCorPY));
...
void PFMETBenchmark::analyse() {
  ...
  // fit gaussian to Delta MET corresponding to different slices in MET, store
  // fit values (mean,sigma) in histos
  ////FitSlicesInY(hSETvsDeltaMET, hmeanPF, hrmsPF, false, 1); //set option flag
  /// for RMS or gaussian /FitSlicesInY(hSETvsDeltaMET, hmeanPF, hsigmaPF, true,
  /// 1); //set option flag for RMS or gaussian
  ...
  /*
  TCanvas* canvas_MetResVsRecoSet = new TCanvas("MetResVsRecoSet", "MET Sigma vs
  Reco SET", 500,500); hsigmaPF->SetStats(0); func1->SetLineColor(1);
  ...
  */
  ...
  /*
  TCanvas* canvas_SetRespVsTrueSet = new TCanvas("SetRespVsTrueSet", "SET
  Response vs True SET", 500,500); profileSETvsSETresp->SetStats(0);
  ...
  */
  ...
  /*
  TCanvas* canvas_MetRespVsTrueMet = new TCanvas("MetRespVsTrueMet", "MET
  Response vs True MET", 500,500); profileMETvsMETresp->SetStats(0);
...
// void PFMETBenchmark::FitSlicesInY(TH2F* h, TH1F* mean, TH1F* sigma, bool
// doGausFit, int type )
//{
...
//      if (f1 == 0) f1 = new TF1("gaus","gaus", fYaxis->GetXmin(),
//      fYaxis->GetXmax()); else         f1->SetRange( fYaxis->GetXmin(),
//      fYaxis->GetXmax());
//    }
...
//      if (bins->fN == 0)
//	hlist[ipar] = new TH1F(name,title, nbins, fXaxis->GetXmin(),
// fXaxis->GetXmax());
//      else
...
//  delete gDirectory->FindObject(name);
//  TH1F *hchi2 = new TH1F(name,"chisquare", nbins, fXaxis->GetXmin(),
//  fXaxis->GetXmax()); hchi2->GetXaxis()->SetTitle(fXaxis->GetTitle());
//
...
//    {
//      TH1F *hpy = (TH1F*) h->ProjectionY("_temp", (Int_t) bin, (Int_t) (bin +
//      ngroup - 1), "e"); if(hpy == 0) continue; nentries = Int_t(
//      hpy->GetEntries() ); if(nentries == 0 ) {delete hpy; continue;}
//      f1->SetParameters(parsave);
//      hpy->Fit( f1, opt.Data() );
//      Int_t npfits = f1->GetNumberFitPoints();
//      //cout << "bin = " << bin << "; Npfits = " << npfits << "; npar = " <<
//      npar << endl; if( npfits > npar )
//	{
//	  Int_t biny = bin + (Int_t)ngroup/2;
//	  for( ipar=0; ipar < npar; ipar++ )
//	    {
//	      if( doGausFit ) hlist[ipar]->Fill( fXaxis->GetBinCenter(biny),
// f1->GetParameter(ipar) ); 	      else            hlist[ipar]->Fill(
// fXaxis->GetBinCenter(biny), hpy->GetRMS() );
//	      //cout << "bin[" << bin << "]: RMS = " << hpy->GetRMS() << ";
// sigma = " << f1->GetParameter(ipar) << endl; hlist[ipar]->SetBinError( biny,
// f1->GetParError(ipar) );
//	    }
//	  hchi2->Fill( fXaxis->GetBinCenter(biny),
