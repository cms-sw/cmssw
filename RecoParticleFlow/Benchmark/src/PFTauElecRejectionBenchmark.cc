...
#define BOOK2D(name, title, nbinsx, lowx, highx, nbinsy, lowy, highy)          \
  h##name =                                                                    \
      db_ ? db_->book2D(#name, title, nbinsx, lowx, highx, nbinsy, lowy,       \
                        highy)                                                 \
                ->getTH2F()                                                    \
...
void PFTauElecRejectionBenchmark::write() {
  ...
  if (!outputFile_.empty()) {
    ...
    // use bare Root if no DQM (FWLite applications)
      ...
      cout << "Benchmark output written to file " << outputFile_.c_str()
           << endl;
      file_->Close();
    }
  } else
    cout << "No output file specified (" << outputFile_
         << "). Results will not be saved!" << endl;
}

void PFTauElecRejectionBenchmark::setup(
    string Filename, string benchmarkLabel, double maxDeltaR, double minRecoPt,
    double maxRecoAbsEta, double minMCPt, double maxMCAbsEta,
    string sGenMatchObjectLabel, bool applyEcalCrackCut, DQMStore *db_store) {
  ...
  // print parameters
  cout << "PFTauElecRejectionBenchmark Setup parameters "
          "=============================================="
       << endl;
  ...
  if (db_) {
    ...
    cout << "Info: DQM is not available to provide data storage service. Using "
            "TFile to save histograms. "
         << endl;
  }
  ...
  BOOK1D(pfcand_deltaEta_weightE, "PFCand cluster dEta, energy weighted", 100,
         0., 0.8);
  ...
  BOOK1D(pfcand_deltaPhiOverQ_weightE, "PFCand cluster dEta/q, energy weighted",
         100, -0.8, 0.8);
  SETAXES(pfcand_deltaPhiOverQ_weightE, "PFCand cluster #Delta(#phi)/q",
          "Entries");
  ...
  BOOK2D(EmfracvsEoP_preid0, "emfrac vs. E/p (preid=0)", 100, 0., 2., 100, 0.,
         1.01);
  SETAXES(EmfracvsEoP_preid0, "E/p", "em fraction");

  BOOK2D(EmfracvsEoP_preid1, "emfrac vs. E/p (preid=0)", 100, 0., 2., 100, 0.,
         1.01);
  SETAXES(EmfracvsEoP_preid1, "E/p", "em fraction");
}

void PFTauElecRejectionBenchmark::process(
  ...
  for (p = generated_event->particles_begin();
       p != generated_event->particles_end(); p++) {
    if (std::abs((*p)->pdg_id()) == 15 && (*p)->status() == 2) {
      bool lept_decay = false;
      TLorentzVector tau((*p)->momentum().px(), (*p)->momentum().py(),
                         (*p)->momentum().pz(), (*p)->momentum().e());
      HepMC::GenVertex::particle_iterator z =
          (*p)->end_vertex()->particles_begin(HepMC::descendants);
      for (; z != (*p)->end_vertex()->particles_end(HepMC::descendants); z++) {
        if (std::abs((*z)->pdg_id()) == 11 || std::abs((*z)->pdg_id()) == 13)
          lept_decay = true;
        if (std::abs((*z)->pdg_id()) == 16)
          taunet.SetPxPyPzE((*z)->momentum().px(), (*z)->momentum().py(),
                            (*z)->momentum().pz(), (*z)->momentum().e());
      }
      if (lept_decay == false) {
      ...
      TLorentzVector elec((*p)->momentum().px(), (*p)->momentum().py(),
                          (*p)->momentum().pz(), (*p)->momentum().e());
  ...
  for (PFTauCollection::size_type iPFTau = 0; iPFTau < pfTaus->size();
       iPFTau++) {
    PFTauRef thePFTau(pfTaus, iPFTau);
    if ((*pfTauIsoDiscr)[thePFTau] == 1) {
      if ((*thePFTau).et() > minRecoPt_ &&
          std::abs((*thePFTau).eta()) < maxRecoAbsEta_) {
        ...
        if (pflch.isNonnull()) {
          ...
          if (myleadTk.isNonnull()) {
            if (applyEcalCrackCut_ &&
                isInEcalCrack(std::abs((double)myleadTkEcalPos.eta()))) {
              continue; // do nothing
            } else {

              // Match with gen object
              for (unsigned int i = 0; i < _GenObjects.size(); i++) {
                if (_GenObjects[i].Et() >= minMCPt_ &&
                    std::abs(_GenObjects[i].Eta()) < maxMCAbsEta_) {
                  TLorentzVector pftau((*thePFTau).px(), (*thePFTau).py(),
                                       (*thePFTau).pz(), (*thePFTau).energy());
                  ...
                  if (GenDeltaR < maxDeltaR_) {
                    ...
                    hHoPvsEoP->Fill((*thePFTau).ecalStripSumEOverPLead(),
                                    (*thePFTau).hcal3x3OverPLead());
                    hEmfracvsEoP->Fill((*thePFTau).emFraction(),
                                       (*thePFTau).hcal3x3OverPLead());
                    ...
                    if ((*thePFTau).electronPreIDDecision() == 1) {
                      ...
                      hHoPvsEoP_preid1->Fill(
                          (*thePFTau).ecalStripSumEOverPLead(),
                          (*thePFTau).hcal3x3OverPLead());
                      hEmfracvsEoP_preid1->Fill((*thePFTau).emFraction(),
                                                (*thePFTau).hcal3x3OverPLead());
                    } else {
                      hEoverP_preid0->Fill(
                          (*thePFTau).ecalStripSumEOverPLead());
                      ...
                      hHoPvsEoP_preid0->Fill(
                          (*thePFTau).ecalStripSumEOverPLead(),
                          (*thePFTau).hcal3x3OverPLead());
                      hEmfracvsEoP_preid0->Fill((*thePFTau).emFraction(),
                                                (*thePFTau).hcal3x3OverPLead());
                    }
                  }
                }
                ...
                std::vector<CandidatePtr> myPFCands =
                    (*thePFTau).pfTauTagInfoRef()->PFCands();
                for (int i = 0; i < (int)myPFCands.size(); i++) {
                  ...
                  if (std::abs(pfCand->pdgId()) == 211 ||
                      std::abs(pfCand->pdgId()) ==
                          11) // if charged hadron or electron
                    candPos = pfCand->positionAtECALEntrance();
                  else
                    candPos = math::XYZPointF(pfCand->px(), pfCand->py(),
                                              pfCand->pz());

                  // double deltaR   =
                  // ROOT::Math::VectorUtil::DeltaR(myleadTkEcalPos,candPos);
                  double deltaPhi = ROOT::Math::VectorUtil::DeltaPhi(
                      myleadTkEcalPos, candPos);
                  double deltaEta =
                      std::abs(myleadTkEcalPos.eta() - myPFCands[i]->eta());
                  double deltaPhiOverQ = deltaPhi / (double)myleadTk->charge();

                  hpfcand_deltaEta->Fill(deltaEta);
                  hpfcand_deltaEta_weightE->Fill(deltaEta *
                                                 pfCand->ecalEnergy());
                  hpfcand_deltaPhiOverQ->Fill(deltaPhiOverQ);
                  hpfcand_deltaPhiOverQ_weightE->Fill(deltaPhiOverQ *
                                                      pfCand->ecalEnergy());
                }
              }
            }
...
bool PFTauElecRejectionBenchmark::isInEcalCrack(double eta) const {
  return (eta < 0.018 || (eta > 0.423 && eta < 0.461) ||
          (eta > 0.770 && eta < 0.806) || (eta > 1.127 && eta < 1.163) ||
          (eta > 1.460 && eta < 1.558));
}
