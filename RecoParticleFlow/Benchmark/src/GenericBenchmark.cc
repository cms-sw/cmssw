...
#define BOOK2D(name, title, nbinsx, lowx, highx, nbinsy, lowy, highy)          \
  h##name =                                                                    \
      DQM ? DQM->book2D(#name, title, nbinsx, lowx, highx, nbinsy, lowy,       \
                        highy)                                                 \
                ->getTH2F()                                                    \
...
#define ET                                                                     \
  (PlotAgainstReco_) ? "reconstructed E_{T} [GeV]" : "generated E_{T} [GeV]"
#define ETA (PlotAgainstReco_) ? "reconstructed #eta" : "generated #eta"
#define PHI                                                                    \
  (PlotAgainstReco_) ? "reconstructed #phi (rad)" : "generated #phi (rad)"
...
void GenericBenchmark::setup(DQMStore *DQM, bool PlotAgainstReco_,
                             float minDeltaEt, float maxDeltaEt,
                             float minDeltaPhi, float maxDeltaPhi,
                             bool doMetPlots) {
  ...
  BOOK2D(DeltaEtvsEt, "#DeltaE_{T} vs E_{T}", nbinsEt, minEt, maxEt, nbinsEt,
         minDeltaEt, maxDeltaEt);
  BOOK2D(DeltaEtOverEtvsEt, "#DeltaE_{T}/E_{T} vsE_{T}", nbinsEt, minEt, maxEt,
         nbinsEt, -1, 1);
  BOOK2D(DeltaEtvsEta, "#DeltaE_{T} vs #eta", nbinsEta, minEta, maxEta, nbinsEt,
         minDeltaEt, maxDeltaEt);
  BOOK2D(DeltaEtOverEtvsEta, "#DeltaE_{T}/E_{T} vs #eta", nbinsEta, minEta,
         maxEta, 100, -1, 1);
  BOOK2D(DeltaEtvsPhi, "#DeltaE_{T} vs #phi", 200, -M_PI, M_PI, nbinsEt,
         minDeltaEt, maxDeltaEt);
  BOOK2D(DeltaEtOverEtvsPhi, "#DeltaE_{T}/E_{T} vs #Phi", 200, -M_PI, M_PI, 100,
         -1, 1);
  BOOK2D(DeltaEtvsDeltaR, "#DeltaE_{T} vs #DeltaR", 100, 0, 1, nbinsEt,
         minDeltaEt, maxDeltaEt);
  BOOK2D(DeltaEtOverEtvsDeltaR, "#DeltaE_{T}/E_{T} vs #DeltaR", 100, 0, 1, 100,
         -1, 1);

  // delta eta quantities
  BOOK1D(DeltaEta, "#Delta#eta", nbinsDeltaEta, minDeltaEta, maxDeltaEta);
  BOOK2D(DeltaEtavsEt, "#Delta#eta vs E_{T}", nbinsEt, minEt, maxEt,
         nbinsDeltaEta, minDeltaEta, maxDeltaEta);
  BOOK2D(DeltaEtavsEta, "#Delta#eta vs #eta", nbinsEta, minEta, maxEta,
         nbinsDeltaEta, minDeltaEta, maxDeltaEta);

  // delta phi quantities
  BOOK1D(DeltaPhi, "#Delta#phi", nbinsDeltaPhi, minDeltaPhi, maxDeltaPhi);
  BOOK2D(DeltaPhivsEt, "#Delta#phi vs E_{T}", nbinsEt, minEt, maxEt,
         nbinsDeltaPhi, minDeltaPhi, maxDeltaPhi);
  BOOK2D(DeltaPhivsEta, "#Delta#phi vs #eta", nbinsEta, minEta, maxEta,
         nbinsDeltaPhi, minDeltaPhi, maxDeltaPhi);
  ...
  BOOK2D(EtRecvsEt, "Rec E_{T} vs E_{T}", nbinsEt, minEt, maxEt, nbinsEt, minEt,
         maxEt);
  BOOK2D(EtRecOverTrueEtvsTrueEt, "Rec E_{T} / E_{T} vs E_{T}", nbinsEt, minEt,
         maxEt, 1000, 0., 100.);
  ...
  BOOK2D(EtvsEtaGen, "generated E_{T} vs generated #eta", 100, -5, 5, 200, 0,
         200);
  BOOK2D(EtvsPhiGen, "generated E_{T} vs generated #phi", 100, -3.5, 3.5, 200,
         0, 200);
  ...
  if (doMetPlots) {
    ...
    BOOK2D(DeltaSetvsSet, "#DeltaSEt vs trueSEt", 3000, 0., 3000., 1000, -1000.,
           1000.);
    BOOK2D(DeltaMexvsSet, "#DeltaMEX vs trueSEt", 3000, 0., 3000., 1000, -400.,
           400.);
    BOOK2D(DeltaSetOverSetvsSet, "#DeltaSetOverSet vs trueSet", 3000, 0., 3000.,
           1000, -1., 1.);
    BOOK2D(RecSetvsTrueSet, "Set vs trueSet", 3000, 0., 3000., 3000, 0., 3000.);
    BOOK2D(RecSetOverTrueSetvsTrueSet, "Set/trueSet vs trueSet", 3000, 0.,
           3000., 500, 0., 2.);
    BOOK2D(TrueMexvsTrueSet, "trueMex vs trueSet", 3000, 0., 3000., nbinsEt,
           -maxEt, maxEt);
  }
...
bool GenericBenchmark::accepted(const reco::Candidate *particle, double ptCut,
                                double minEtaCut, double maxEtaCut) const {
...
void GenericBenchmark::fillHistos(const reco::Candidate *genParticle,
                                  ...
                                  double deltaR_cut, bool plotAgainstReco) {
  ...
  if (doMetPlots_) {
    ...
    if (met1 != nullptr && met2 != nullptr) {
      ...
      hDeltaMexvsSet->Fill((*met1).sumEt(),
                           recParticle->px() - genParticle->px());
      hDeltaMexvsSet->Fill((*met1).sumEt(),
                           recParticle->py() - genParticle->py());
      if ((*met1).sumEt() > 0.01)
        hDeltaSetOverSetvsSet->Fill((*met1).sumEt(),
                                    ((*met2).sumEt() - (*met1).sumEt()) /
                                        (*met1).sumEt());
      hRecSetvsTrueSet->Fill((*met1).sumEt(), (*met2).sumEt());
      hRecSetOverTrueSetvsTrueSet->Fill((*met1).sumEt(),
                                        (*met2).sumEt() / ((*met1).sumEt()));
