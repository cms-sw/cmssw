...
// COLIN: this class REALLY needs to be cleaned up and rationalized, on the
// model of PFCandidateBenchmark.
...
public:
  ...
  void setup(DQMStore *DQM = nullptr, bool PlotAgainstReco_ = true,
             float minDeltaEt = -100., float maxDeltaEt = 50.,
             float minDeltaPhi = -0.5, float maxDeltaPhi = 0.5,
             bool doMetPlots = false);

  template <typename C>
  void fill(const C *RecoCollection, const C *GenCollection,
            bool startFromGen = false, bool PlotAgainstReco = true,
            bool onlyTwoJets = false, double recPt_cut = -1.,
            double minEta_cut = -1., double maxEta_cut = -1.,
            double deltaR_cut = -1.);
...
private:
  bool accepted(const reco::Candidate *particle, double ptCut, double minEtaCut,
                double maxEtaCut) const;

  void fillHistos(const reco::Candidate *genParticle,
                  const reco::Candidate *recParticle, double deltaR_cut,
...
template <typename C>
void GenericBenchmark::fill(const C *RecoCollection, const C *GenCollection,
                            bool startFromGen, bool PlotAgainstReco,
                            bool onlyTwoJets, double recPt_cut,
                            double minEta_cut, double maxEta_cut,
                            double deltaR_cut) {

  // if (doMetPlots_)
  //{
  //  const reco::MET* met1=static_cast<const
  //  reco::MET*>(&((*RecoCollection)[0])); if (met1!=NULL) std::cout << "FL:
  //  met1.sumEt() = " << (*met1).sumEt() << std::endl;
  //}
  ...
  if (!startFromGen) {
    ...
    for (unsigned int i = 0; i < RecoCollection->size(); i++) {
      ...
      // generate histograms comparing the reco and truth candidate (truth =
      // closest in delta-R)
      ...
      const reco::Candidate *gen_particle =
          algo_->matchByDeltaR(particle, GenCollection);
  ...
  for (unsigned int i = 0; i < GenCollection->size(); i++) {
    ...
    const reco::Candidate *rec_particle =
        algo_->matchByDeltaR(gen_particle, RecoCollection);
    nGen++;
    if (!rec_particle)
      continue; // no match
    ...
    if (gen_particle->et() != 0.0)
      hEtRecOverTrueEtvsTrueEt->Fill(gen_particle->et(),
                                     rec_particle->et() / gen_particle->et());
