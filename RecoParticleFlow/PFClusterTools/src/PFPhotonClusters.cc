...
PFPhotonClusters::PFPhotonClusters(PFClusterRef PFClusterRef) : PFClusterRef_(PFClusterRef) {
  if (PFClusterRef_->layer() == PFLayer::ECAL_BARREL)
    isEB_ = true;
  else
    isEB_ = false;
  SetSeed();
...
void PFPhotonClusters::SetSeed() {
  ...
  DetId idseed;
  const auto& PFRecHits = PFClusterRef_->recHitFractions();
...
void PFPhotonClusters::PFCrystalCoor() {
  if (PFClusterRef_->layer() == PFLayer::ECAL_BARREL) {  //is Barrel
    ...
    float X0 = 0.89;
    float T0 = 1.2;
    if (std::abs(PFClusterRef_->eta()) > 1.653)
      T0 = 3.1;
