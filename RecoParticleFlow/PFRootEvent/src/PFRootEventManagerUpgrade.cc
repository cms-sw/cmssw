...
PFRootEventManagerUpgrade::PFRootEventManagerUpgrade() {}



PFRootEventManagerUpgrade::PFRootEventManagerUpgrade(const char* file)
  : 
  iEvent_(0),
  options_(0),
  ev_(0),
  ...
  calibFile_(0)
{
  ...
  h_deltaETvisible_MCPF_  
    = new TH1F("h_deltaETvisible_MCPF" ,"Jet Et difference ParticleFlow-MC"
               ,1000,-50.,50.);
  h_time_vs_tdc_
    = new TH2F("h_time_vs_tdc", "Pulse shape time versus tdc"
               ,256, -64.5, 63.5, 256, -64.5, 63.5);
  h_time_vs_tdc5_
    = new TH2F("h_time_vs_tdc5", "Pulse shape time versus tdc (energy>5 GeV)"
               ,256, -64.5, 63.5, 256, -64.5, 63.5);
  h_time_vs_tdc10_
    = new TH2F("h_time_vs_tdc10", "Pulse shape time versus tdc (energy>10 GeV)"
               ,256, -64.5, 63.5, 256, -64.5, 63.5);
...
void PFRootEventManagerUpgrade::initializeEventInformation() {
...
void PFRootEventManagerUpgrade::reset() { 
...
void PFRootEventManagerUpgrade::readOptions(const char* file, 
                                     bool refresh, 
                                     bool reconnect) {
                                     
  readSpecificOptions(file);
  
  cout<<"calling PFRootEventManagerUpgrade::readOptions"<<endl;
  ...
  debug_hiPU_ = false; 
  options_->GetOpt("hiPU", "debug", debug_hiPU_);
...
// PFMET benchmark options and output jetfile to be open before input file!!!--
  ...
  if (doPFMETBenchmark_) {
    ...
    if (trueMetSpecificIdCut.size()!=trueMetSpecificEtaCut.size()) std::cout << "Warning: PFRootEventManagerUpgrade: trueMetSpecificIdCut.size()!=trueMetSpecificEtaCut.size()" << std::endl;
...
//-----------------------------------------------
  ...
  if( !filterTaus_.empty() &&
      filterTaus_.size() != 2 ) {
    cerr<<"PFRootEventManagerUpgrade::ReadOptions, bad filter/taus option."<<endl
        <<"please use : "<<endl
...
void PFRootEventManagerUpgrade::connect( const char* infilename ) {
...
//CGT
  std::string rechitsHCALUpgradetagname;
  options_->GetOpt("root","rechits_HCALUpgrade_inputTag", rechitsHCALUpgradetagname);
  rechitsHCALUpgradeTag_ = edm::InputTag(rechitsHCALUpgradetagname);

  std::string digiHCALUpgradeDigitagname;
  options_->GetOpt("root","digi_HCALUpgradeDigi_inputTag", digiHCALUpgradeDigitagname);
  digiHCALUpgradeDigiTag_ = edm::InputTag(digiHCALUpgradeDigitagname);
//CGT
...
void PFRootEventManagerUpgrade::write() {
...
int PFRootEventManagerUpgrade::eventToEntry(int run, int lumi, int event) const {
...
bool PFRootEventManagerUpgrade::processEvent(int run, int lumi, int event) {
...
bool PFRootEventManagerUpgrade::processEntry(int entry) {
...
bool PFRootEventManagerUpgrade::eventAccepted() const {
  // return highPtJet(10); 
...
bool PFRootEventManagerUpgrade::highPtJet(double ptMin) const {
...
bool PFRootEventManagerUpgrade::highPtPFCandidate( double ptMin, 
...
bool PFRootEventManagerUpgrade::readFromSimulation(int entry) {
  ...
  if ( foundstdTracks ) { 
    ...
    cerr<<"PFRootEventManagerUpgrade::ProcessEntry : stdTracks Collection not found : "
  ...
  if ( foundMCTruth ) { 
    ...
    // cerr<<"PFRootEventManagerUpgrade::ProcessEntry : MCTruth Collection not found : "
  ...
  if ( foundTP ) { 
    ...
    //cerr<<"PFRootEventManagerUpgrade::ProcessEntry : trueParticles Collection not found : "
  ...
  if ( foundECAL ) { 
    ...
    cerr<<"PFRootEventManagerUpgrade::ProcessEntry : rechitsECAL Collection not found : "
  ...
  if ( foundHCAL ) { 
    ...
    cerr<<"PFRootEventManagerUpgrade::ProcessEntry : rechitsHCAL Collection not found : "
        <<entry << " " << rechitsHCALTag_<<endl;
  }

//CGT
  bool foundHCALUpgrade = iEvent.getByLabel(rechitsHCALUpgradeTag_,rechitsHCALUpgradeHandle_);
  if ( foundHCALUpgrade ) { 
    rechitsHCALUpgrade_ = *rechitsHCALUpgradeHandle_;
    cout << "Found " << rechitsHCALUpgrade_.size() << " HCAL Upgrade rechits" << endl;
  } else { 
    cerr<<"PFRootEventManagerUpgrade::ProcessEntry : rechitsHCALUpgrade Collection not found : "
        <<entry << " " << rechitsHCALUpgradeTag_<<endl;
  }

  bool foundHCALUpgradeDigi = iEvent.getByLabel(digiHCALUpgradeDigiTag_,digiHCALUpgradeDigiHandle_);
  if ( foundHCALUpgradeDigi ) { 
    digiHCALUpgradeDigi_ = *digiHCALUpgradeDigiHandle_;
    cout << "Found " << digiHCALUpgradeDigi_.size() << " HCAL Upgrade digi" << endl;
  } else { 
    cerr<<"PFRootEventManagerUpgrade::ProcessEntry : digiHCAL Upgrade digi Collection not found : "
        <<entry << " " << digiHCALUpgradeDigiTag_<<endl;
  }
//CGT
  ...
  if ( foundHFEM ) { 
    ...
    cerr<<"PFRootEventManagerUpgrade::ProcessEntry : rechitsHFEM Collection not found : "
  ...
  if ( foundHFHAD ) { 
    ...
    cerr<<"PFRootEventManagerUpgrade::ProcessEntry : rechitsHFHAD Collection not found : "
  ...
  for ( unsigned i=0; i<rechitsCLEANEDTags_.size(); ++i ) { 
    ...
    if ( foundCLEANED ) { 
      ...
      cerr<<"PFRootEventManagerUpgrade::ProcessEntry : rechitsCLEANED Collection not found : "
  ...
  if ( foundPS ) { 
    ...
    cerr<<"PFRootEventManagerUpgrade::ProcessEntry : rechitsPS Collection not found : "
  ...
  if ( foundCT ) { 
    ...
    cerr<<"PFRootEventManagerUpgrade::ProcessEntry : caloTowers Collection not found : "
  ...
  if ( foundPV ) { 
    ...
    cerr<<"PFRootEventManagerUpgrade::ProcessEntry : primaryVertices Collection not found : "
  ...
  if ( foundPFV ) { 
    ...
    cerr<<"PFRootEventManagerUpgrade::ProcessEntry : pfNuclearTrackerVertex Collection not found : "
  ...
  if ( foundrecTracks ) { 
    ...
    cerr<<"PFRootEventManagerUpgrade::ProcessEntry : recTracks Collection not found : "
  ...
  if ( founddisplacedRecTracks ) { 
    ...
    cerr<<"PFRootEventManagerUpgrade::ProcessEntry : displacedRecTracks Collection not found : "
  ...
  if ( foundgsfrecTracks ) { 
    ...
    cerr<<"PFRootEventManagerUpgrade::ProcessEntry : gsfrecTracks Collection not found : "
  ...
  if ( foundconvBremGsfrecTracks ) { 
    ...
    cerr<<"PFRootEventManagerUpgrade::ProcessEntry : convBremGsfrecTracks Collection not found : "
  ...
  if ( foundmuons ) { 
    ...
    */
    ...
    cerr<<"PFRootEventManagerUpgrade::ProcessEntry : muons Collection not found : "
  ...
  if ( foundconversion ) { 
    ...
    cerr<<"PFRootEventManagerUpgrade::ProcessEntry : conversion Collection not found : "
  ...
  if ( foundv0 ) { 
    ...
    cerr<<"PFRootEventManagerUpgrade::ProcessEntry : v0 Collection not found : "
        <<entry << " " << v0Tag_<<endl;
  }
  ...
  if(useEGPhotons_) {
    ...
    if ( foundPhotons) {
      ...
      cerr <<"PFRootEventManagerUpgrade::ProcessEntry : photon collection not found : " 
  ...
  if ( foundgenJets ) { 
    ...
    //cerr<<"PFRootEventManagerUpgrade::ProcessEntry : genJets Collection not found : "
  ...
  if ( foundgenParticlesforJets ) { 
    ...
    //cerr<<"PFRootEventManagerUpgrade::ProcessEntry : genParticlesforJets Collection not found : "
  ...
  if ( foundgenParticlesforMET ) { 
    ...
    //cerr<<"PFRootEventManagerUpgrade::ProcessEntry : genParticlesforMET Collection not found : "
  ...
  if ( foundcaloJets ) { 
    ...
    cerr<<"PFRootEventManagerUpgrade::ProcessEntry : caloJets Collection not found : "
  ...
  if ( foundcorrcaloJets ) { 
    ...
    //cerr<<"PFRootEventManagerUpgrade::ProcessEntry : corrcaloJets Collection not found : "
  ...
  if ( foundpfJets ) { 
    ...
    cerr<<"PFRootEventManagerUpgrade::ProcessEntry : PFJets Collection not found : "
  ...
  if ( foundpfCands ) { 
    ...
    cerr<<"PFRootEventManagerUpgrade::ProcessEntry : PFCandidate Collection not found : "
  ...
  if ( foundpfMets ) { 
    ...
    cerr<<"PFRootEventManagerUpgrade::ProcessEntry : PFMets Collection not found : "
  ...
  if(rechitsHCAL_.size() && rechitsHCALUpgrade_.size() && digiHCALUpgradeDigi_.size()){
//    RemakeRecHits( rechitsHCAL_, rechitsHCALUpgrade_, digiHCALUpgradeDigi_, rechitsHCALnew_ );
//    rechitsHCAL_ = rechitsHCALnew_;
  }	
...
bool PFRootEventManagerUpgrade::isHadronicTau() const {
...
bool PFRootEventManagerUpgrade::countChargedAndPhotons() const {
...
int PFRootEventManagerUpgrade::chargeValue(const int& Id) const {
...
void 
PFRootEventManagerUpgrade::PreprocessRecTracks(reco::PFRecTrackCollection& recTracks) {  
...
void 
PFRootEventManagerUpgrade::PreprocessRecTracks(reco::GsfPFRecTrackCollection& recTracks) {  
  for( unsigned i=0; i<recTracks.size(); ++i ) {     
    recTracks[i].calculatePositionREP();
...
void 
PFRootEventManagerUpgrade::RemakeRecHits(reco::PFRecHitCollection& rechits, 
                                  HcalUpgradeRecHitCollection& rechitsupgrade,
                                  HcalUpgradeDigiCollection& digiupgrade,
                                  reco::PFRecHitCollection& rechitsnew ){

  map<unsigned, unsigned> rechitdetId2index;
//  map<unsigned, double> rechitdetId2time;
//  map<unsigned, uint16_t> digidetId2tdc;

  for(unsigned i=0; i<rechits.size(); i++) {
    rechitdetId2index.insert( make_pair(rechits[i].detId(), i) );
  }

  for(unsigned i=0; i<rechitsupgrade.size(); i++) {
    const map<unsigned, unsigned>::const_iterator& it = rechitdetId2index.find(rechitsupgrade[i].id());

    if(it != rechitdetId2index.end() ) {
      double pulsetime = rechits[it->second].time();
      double tdctime = rechitsupgrade[i].time();
      double newenergy = rechitsupgrade[i].energy();
      rechits[it->second].setRescale(tdctime);
      if(newenergy>1.0) {
        h_time_vs_tdc_->Fill(tdctime, pulsetime);
      }
      if(newenergy>5.0) {
        h_time_vs_tdc5_->Fill(tdctime, pulsetime);
      }
      if(newenergy>10.0) {
        h_time_vs_tdc10_->Fill(tdctime, pulsetime);
      }
      rechits[it->second].setEnergy(newenergy);
    }
  }

/*
  for(unsigned i=0; i<digiupgrade.size(); i++) {
    const map<unsigned, unsigned>::const_iterator& it = rechitdetId2index.find(digiupgrade[i].id());

    if(it != rechitdetId2index.end() ) {
//      double pulsetime = rechits[it->second].time();
//      uint16_t tdc3 = digiupgrade[i].tdc(3);
      double hitenergy = rechits[it->second].energy();
//      if(hitenergy>5.0 && digiupgrade[i].id().subdet()==1) {
      if(hitenergy>5.0 && digiupgrade[i].id().subdet()==1) {
        cout << " Upgrade digi " << i <<endl;
        cout << "0 " << digiupgrade[i].tdc(0) <<endl;
        cout << "1 " << digiupgrade[i].tdc(1) <<endl;
        cout << "2 " << digiupgrade[i].tdc(2) <<endl;
        cout << "3 " << digiupgrade[i].tdc(3) <<endl;
        cout << "4 " << digiupgrade[i].tdc(4) <<endl;
        cout << "5 " << digiupgrade[i].tdc(5) <<endl;
        cout << "6 " << digiupgrade[i].tdc(6) <<endl;
        cout << "7 " << digiupgrade[i].tdc(7) <<endl;
        cout << "8 " << digiupgrade[i].tdc(8) <<endl;
        cout << "9 " << digiupgrade[i].tdc(9) <<endl;
      }
    }
//    uint16_t tdc3 = digiupgrade[i].tdc(3);
//    digidetId2tdc.insert( make_pair(digiupgrade[i].id(), tdc) );
  }
*/
...
void 
PFRootEventManagerUpgrade::PreprocessRecHits(reco::PFRecHitCollection& rechits, 
                                      bool findNeighbours) {
  ...
  /// NICHOLAS JIN
  // FILTER OUT LOW ENERGIES
  /*
  for (unsigned i = 0; i < rechits.size(); i++) {
    if (rechits[i].energy() < 10) {
      rechits[i].zeroEnergy();
    }
  }
  */

  /// NICHOLAS JIN
  // FILTER OUT OUTLIER TIMES

  for (unsigned i = 0; i < rechits.size(); i++) {
    if (rechits[i].layer() == PFLayer::HCAL_BARREL1 || rechits[i].layer() == PFLayer::HCAL_ENDCAP ) {	
      if ((rechits[i].time() < -35.0) ||(rechits[i].time() > -10.0) ) {
//        rechits[i].setEnergy(0.0);
      }
    }
  }
...
void PFRootEventManagerUpgrade::setRecHitNeigbours
( reco::PFRecHit& rh, 
...
void PFRootEventManagerUpgrade::clustering() {
  ...
  cout << " number of HCAL clusters: " << clustersHCAL_->size() <<endl;
  superClusterAlgoHCAL_.doClustering(*clustersHCAL_,*clustersHCAL_);
  clustersHCAL_ = superClusterAlgoHCAL_.clusters();
  cout << " number of HCAL superclusters: " << clustersHCAL_->size() <<endl;
/*
  for (unsigned short ic=0; ic<clustersHCAL_->size();++ic) {
    if( (*clustersHCAL_)[ic].energy()>5.0) {
      cout << (*clustersHCAL_)[ic] <<endl;
    }
  }
*/
...
void 
PFRootEventManagerUpgrade::fillOutEventWithClusters(const reco::PFClusterCollection& 
                                             clusters) {
...
void 
PFRootEventManagerUpgrade::fillOutEventWithSimParticles(const reco::PFSimParticleCollection& trueParticles ) {
...
void 
PFRootEventManagerUpgrade::fillOutEventWithPFCandidates(const reco::PFCandidateCollection& pfCandidates ) {
...
void 
PFRootEventManagerUpgrade::fillOutEventWithCaloTowers( const CaloTowerCollection& cts ){
...
void 
PFRootEventManagerUpgrade::fillOutEventWithBlocks( const reco::PFBlockCollection& 
                                            blocks ) {
...
void PFRootEventManagerUpgrade::particleFlow() {
  ...
  if( debug_) {
    cout<<"PFRootEventManagerUpgrade::particleFlow start"<<endl;
  ...
  if( debug_hiPU_) cout<<pfBlockAlgo_<<endl;
  ...
  if( debug_hiPU_) cout<< pfAlgo_<<endl;
  ...
  if( debug_) cout<<"PFRootEventManagerUpgrade::particleFlow stop"<<endl;
}

void PFRootEventManagerUpgrade::pfCandCompare(int entry) {
...
void PFRootEventManagerUpgrade::reconstructGenJets() {
...
void PFRootEventManagerUpgrade::reconstructCaloJets() {
...
void PFRootEventManagerUpgrade::reconstructPFJets() {
...
void 
PFRootEventManagerUpgrade::reconstructFWLiteJets(const reco::CandidatePtrVector& Candidates, vector<ProtoJet>& output ) {
...
double 
PFRootEventManagerUpgrade::tauBenchmark( const reco::PFCandidateCollection& candidates) {
...
//   }//loop
  ...
  if (fastsim_){
    ...
    if(!tauFound || tooManyTaus ) {
      // cerr<<"PFRootEventManagerUpgrade::tauBenchmark : not a single tau event"<<endl;
      return -9999;
    }
...
void PFRootEventManagerUpgrade::lookForGenParticle(unsigned barcode) {
...
string PFRootEventManagerUpgrade::expand(const string& oldString) const {
...
void 
PFRootEventManagerUpgrade::printMCCalib(ofstream& out) const {
...
void  PFRootEventManagerUpgrade::print(ostream& out,int maxNLines ) const {
...
void
PFRootEventManagerUpgrade::printGenParticles(std::ostream& out,
                                      int maxNLines) const {
...
void PFRootEventManagerUpgrade::printRecHits(const reco::PFRecHitCollection& rechits, const PFClusterAlgo& clusterAlgo, ostream& out) const{
...
void  PFRootEventManagerUpgrade::printRecHit(const reco::PFRecHit& rh,
				      unsigned index,  
...
void PFRootEventManagerUpgrade::printClusters(const reco::PFClusterCollection& clusters,
                                       ostream& out ) const {  
  for(unsigned i=0; i<clusters.size(); i++) {
...
void  PFRootEventManagerUpgrade::printCluster(const reco::PFCluster& cluster,
                                       ostream& out ) const {
...
bool PFRootEventManagerUpgrade::trackInsideGCut( const reco::PFTrack& track ) const {
...
void  
PFRootEventManagerUpgrade::fillRecHitMask( vector<bool>& mask, 
...
void  
PFRootEventManagerUpgrade::fillClusterMask(vector<bool>& mask, 
...
void  
PFRootEventManagerUpgrade::fillTrackMask(vector<bool>& mask, 
...
void  
PFRootEventManagerUpgrade::fillPhotonMask(vector<bool>& mask, 
...
PFRootEventManagerUpgrade::closestParticle( reco::PFTrajectoryPoint::LayerType layer, 
  ...
  if( trueParticles_.empty() ) {
    string err  = "PFRootEventManagerUpgrade::closestParticle : ";
...
//-----------------------------------------------------------
void 
PFRootEventManagerUpgrade::readCMSSWJets() {
...
std::string PFRootEventManagerUpgrade::getGenParticleName(int partId, std::string &latexString) const
{
  std::string  name;
  switch(partId) {
...
void PFRootEventManagerUpgrade::mcTruthMatching( std::ostream& out,
...
edm::InputTag 
PFRootEventManagerUpgrade::stringToTag(const std::vector< std::string >& tagname) { 
