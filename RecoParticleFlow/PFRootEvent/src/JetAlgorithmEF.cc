#include "../interface/JetAlgorithmEF.h"
#include <iostream>
#include <set>
...
ostream& operator<<(ostream& out, const JetAlgorithmEF::Jet& jet) {
  if(!out) return out;
...
const vector< JetAlgorithmEF::Jet >&
JetAlgorithmEF::FindJets( const vector<TLorentzVector>* vecs) {
...
double JetAlgorithmEF::DeltaR(double eta1, double phi1, double eta2, double phi2) {
...
//   }
  ...
  vector< JetAlgorithmEF::Jet >  tmp = fJets;

  map< double, JetAlgorithmEF::Jet, greater<double> > etjets;
  // typedef map< double, JetAlgorithmEF::Jet, greater<double> >::iterator IJ;
  
  vector< JetAlgorithmEF::Jet >  tmp2;
  fJets.clear();
...
void JetAlgorithmEF::MergeJets(map< double, JetAlgorithmEF::Jet, greater<double> >& etjets) {
