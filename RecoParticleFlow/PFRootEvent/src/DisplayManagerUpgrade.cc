...
DisplayManagerUpgrade::DisplayManagerUpgrade(PFRootEventManagerUpgrade *em,
                               const char* optfile ) : 
  em_(em), 
  options_(0),
  maxERecHitEcal_(-1),
...
DisplayManagerUpgrade::~DisplayManagerUpgrade()
{
  reset();
} 

//________________________________________________________
void DisplayManagerUpgrade::readOptions( const char* optfile ) {
  ...
  if(viewSizeEtaPhi_.size() != 2) {
    cerr<<"PFRootEventManagerUpgrade::ReadOptions, bad display/viewsize_etaphi tag...using 700/350"
        <<endl;
    viewSizeEtaPhi_.clear();
  ...
  if(viewSize_.size() != 2) {
    cerr<<"PFRootEventManagerUpgrade::ReadOptions, bad display/viewsize_xy tag...using 700/350"
        <<endl;
    viewSize_.clear();
  ...
  if(clusterAttributes_.size() != 4) {
    cerr<<"PFRootEventManagerUpgrade::::ReadOptions, bad display/cluster_attributes tag...using 20 10 2 5"
        <<endl;
    clusterAttributes_.clear();
  ...
  if(trackAttributes_.size() != 4) {
    cerr<<"PFRootEventManagerUpgrade::::ReadOptions, bad display/track_attributes tag...using 103 1 8 8"
        <<endl;
    trackAttributes_.clear();
  ...
  if(gsfAttributes_.size() != 4) {
    cerr<<"PFRootEventManagerUpgrade::::ReadOptions, bad display/gsf_attributes tag...using 105 1 8 8"
	<<endl;
    gsfAttributes_.clear();
  ...
  if(bremAttributes_.size() != 4) {
    cerr<<"PFRootEventManagerUpgrade::::ReadOptions, bad display/gsf_attributes tag...using 106 1 8 8"
	<<endl;
    bremAttributes_.clear();
  ...
  if(simPartAttributes.size() != 3) {
    cerr<<"PFRootEventManagerUpgrade::::ReadOptions, bad display/simPart_attributes tag...using 103 1 8 8"
        <<endl;
...
//________________________________________________________
void DisplayManagerUpgrade::createCanvas()
{
...
void DisplayManagerUpgrade::createGCluster(const reco::PFCluster& cluster, 
                                    int ident, 
                                    double phi0)
{
    ...
    case XY:
      {
        ...
        graphicMap_.insert(pair<int,GPFBaseUpgrade *> (ident, gc));
      }
      break;
    case RZ:
      {
...
void DisplayManagerUpgrade::createGPart( const reco::PFSimParticle &ptc,
                                  ...
                                  int markerIndex)
{
  ...
  for (int viewType=0;viewType<4;++viewType) {
    ...
    //if( !inside ) return;
    //int color = 4;
    GPFSimParticleUpgrade *gc   = new GPFSimParticleUpgrade(this,
                                              viewType, ident,
                                              &ptc,
    ...
    graphicMap_.insert(pair<int,GPFBaseUpgrade *>      (ident, gc));                                 
    //graphicMap_.insert(pair<int,GPFBaseUpgrade *> (ident,new GPFSimParticleUpgrade(this,viewType,ident,&ptc,xPos.size(),&xPos[0],&yPos[0],pt,simPartPattern_[indexMarker], "pl")));
  }
}
//____________________________________________________________________
void DisplayManagerUpgrade::createGRecHit(reco::PFRecHit& rh,int ident, double maxe, double phi0, int color)
{
  ...
  default:
    {
      cerr<<"DisplayManagerUpgrade::createGRecHit : manage other layers."
  ...
  //cout << " Test Upgrade HCAL view DEPTH: " << ((HcalDetId)rh.detId()).depth() <<endl;
  ...
  for(int viewType=0;viewType<4;++viewType) {
    ...
    for ( unsigned jc=0; jc<4; ++jc ) { 
      ...
      //cout << " Test Upgrade HCAL view2 " << rh.detId() <<endl;
                        
      // WORKSPACE NICHOLAS JIN

      //cout << " Print ieta " << ((rh.detId()&0x2000)?(1):(-1)) * ((rh.detId()>>7)&0x3) <<endl;
      //cout << " Print cell iphi " << (rh.detId()&0x7F) <<endl;
      //cout << " Print tower depth " << ((rh.detId()>>14)&0x1F) <<endl;

      //cout << " Print ieta " << ((HcalDetId)rh.detId()).ieta() << endl;
      //cout << " Print cell iphi " << ((HcalDetId)rh.detId()).iphi() << endl;
      //cout << " Print tower depth " << ((HcalDetId)rh.detId()).depth() << endl;
	...
	//NICHOLAS JIN
	/*etaprop2[jc] = centreXYZrot.Eta() + .015 + centertocornerep.X()*ampl;
	  phiprop2[jc] = centreXYZrot.Phi() + .015 + centertocornerep.Y()*ampl;
	  
	  etaprop3[jc] = centreXYZrot.Eta() + .015 + centertocornerep.X()*ampl;
	  phiprop3[jc] = centreXYZrot.Phi() + .015 + centertocornerep.Y()*ampl;
	  
	  etaprop4[jc] = centreXYZrot.Eta() + .0225 + centertocornerep.X()*ampl;
	  phiprop4[jc] = centreXYZrot.Phi() + .0225 + centertocornerep.Y()*ampl;
	  
	  etaprop5[jc] = centreXYZrot.Eta() + .03 + centertocornerep.X()*ampl;
	  phiprop5[jc] = centreXYZrot.Phi() + .03 + centertocornerep.Y()*ampl;*/
      }
    }//loop on jc 
    ...
    //NICHOLAS JIN
    //HcalHitRelabeller rl = relabel(rh.detId());
    
    //int newcolor = TColor::GetColor(255, 255, 255);
	...
	if(layer == PFLayer::ECAL_BARREL || 
	 layer == PFLayer::HCAL_BARREL1 || 
	   layer == PFLayer::HCAL_BARREL2) {
	  graphicMap_.insert(pair<int,GPFBaseUpgrade *> (ident,new GPFRecHitUpgrade(this, viewType,ident,&rh,npoints,x,y,color,"f")));
	}
	else {
	  graphicMap_.insert(pair<int,GPFBaseUpgrade *> (ident,new GPFRecHitUpgrade(this, viewType,ident,&rh,npoints,x,y,color,"l")));
	  if( ampl>0 ) { // not for preshower
	    xprop[4]=xprop[0];
	    yprop[4]=yprop[0]; // closing the polycell    
	   graphicMap_.insert(pair<int,GPFBaseUpgrade *> (ident,new GPFRecHitUpgrade(this, viewType,ident,&rh,npoints,xprop,yprop,color,"f")));
	  }
	}
      } 
      break;
    ...
    case RZ:
      graphicMap_.insert(pair<int,GPFBaseUpgrade *> (ident,new GPFRecHitUpgrade(this, viewType,ident,&rh,npoints,z,r,color,"f")));
      break;
      
    case EPE:
      {
        graphicMap_.insert(pair<int,GPFBaseUpgrade *> (ident,new GPFRecHitUpgrade(this, viewType,ident,&rh,npoints,eta,phi,color,"l")));
                                
        if( ampl>0 ) { // not for preshower
          etaprop[4]=etaprop[0];
          phiprop[4]=phiprop[0]; // closing the polycell    
          graphicMap_.insert(pair<int,GPFBaseUpgrade *> (ident,new GPFRecHitUpgrade(this, viewType,ident,&rh,npoints,etaprop,phiprop,color,"f")));
        }
      }  
      break;
    case EPH:
	...
	//Draws the outline of the box.
	  ...
	  etaprop[4]=etaprop[0]; // closing the polycell
	  phiprop[4]=phiprop[0];

	  Int_t newcolor = 0;

	  switch ((rh.detId()>>14)&0x1F) {
	  case 1:
	    newcolor = TColor::GetColor(255, 0, 0);
	    break;

	  case 2:
	    newcolor = TColor::GetColor(255, 102, 0);
	    break;

	  case 3:
	    newcolor = TColor::GetColor(0, 255, 0);
	    break;

	  case 4:
	    newcolor = TColor::GetColor(0, 0, 255);
	    break;

	  case 5:
	    newcolor = TColor::GetColor(255, 0, 255);
	    break;
	  ...
	  for (int i = 0; i < 5; i++) {
	    etaprop[i]+= 0.01 * (((rh.detId()>>14)&0x1F) - 1);
	    phiprop[i]+= 0.01 * (((rh.detId()>>14)&0x1F) - 1);
	  }

	  graphicMap_.insert(pair<int,GPFBaseUpgrade *> (ident,new GPFRecHitUpgrade(this, viewType,ident,&rh,npoints,etaprop,phiprop,
								      newcolor,"f")));
	  graphicMap_.insert(pair<int,GPFBaseUpgrade *> (ident,new GPFRecHitUpgrade(this, viewType,ident,&rh,npoints,etaprop,phiprop,
...
void DisplayManagerUpgrade::createGTrack( reco::PFRecTrack &tr,
                                   const std::vector<reco::PFTrajectoryPoint>& points, 
                                   int ident,double pt,double phi0, double sign, bool displayInitial,
                                   int linestyle,int kfgsfbrem) 
{
  ...
  for (int viewType=0;viewType<4;++viewType) {
    ...
    //fill map with graphic objects
      GPFTrackUpgrade *gt=0;
      if(kfgsfbrem==0) {
	gt = new  GPFTrackUpgrade(this,
			   viewType,ident,
			   &tr,
	...
	gt = new  GPFTrackUpgrade(this,
      ...
      graphicMap_.insert(pair<int,GPFBaseUpgrade *> (ident, gt));
  }   
}
//NICHOLAS JIN_____________________________________________________________________________
void DisplayManagerUpgrade::createGCandidate(reco::PFCandidate& rh, int ident,
		      double phi0) {
  ...
  /*
    ...
    default:
    {
    cerr<<"DisplayManagerUpgrade::createGCandidate : manage other layers."
    <<" GCandidate notcreated."<<endl;
    return;
    }
    }
    if( rh.energy() < thresh ) return;
  ...
  for(int viewType=0;viewType<4;++viewType) {
    ...
    double etapos = rh.positionAtECALEntrance().Eta();
    double phipos = rh.positionAtECALEntrance().Phi();
    ...
    */
    ...
    etaprop[0] =   0;          phiprop[0] =   r * c;
    etaprop[1] = - r * B;      phiprop[1] =   r * C;
    etaprop[2] = - r * D * c ; phiprop[2] =   r * A * c;
    etaprop[3] = - r * D;      phiprop[3] = - r * A;
    etaprop[4] = - r * B * c;  phiprop[4] = - r * C * c;
    etaprop[5] =   0;          phiprop[5] = - r;
    etaprop[6] =   r * B * c;  phiprop[6] = - r * C * c;
    etaprop[7] =   r * D;      phiprop[7] = - r * A;
    etaprop[8] =   r * D * c;  phiprop[8] =   r * A * c;
    etaprop[9] =   r * B;      phiprop[9] =   r * C;
    etaprop[10] = etaprop[0];  phiprop[10] = phiprop[0];
    ...
    */
    ...
    case  XY:
	  ...
	  layer == PFLayer::HCAL_BARREL1 || 
	  layer == PFLayer::HCAL_BARREL2) {
          graphicMap_.insert(pair<int,GPFBaseUpgrade *> (ident,new GPFCandidateUpgrade(this, viewType,ident,&rh,npoints,x,y,color,"f")));
	  
	  } else {
          graphicMap_.insert(pair<int,GPFBaseUpgrade *> (ident,new GPFCandidateUpgrade(this, viewType,ident,&rh,npoints,x,y,color,"l")));
          if( ampl>0 ) { // not for preshower
	  xprop[4]=xprop[0];
	  yprop[4]=yprop[0]; // closing the polycell    
	*/
	//graphicMap_.insert(pair<int,GPFBaseUpgrade *> (ident,new GPFCandidateUpgrade(this, viewType,ident,&rh,npoints,xprop,yprop,color,"f")));
      }
      break;
      
    case RZ:
      //graphicMap_.insert(pair<int,GPFBase *> (ident,new GPFCandidate(this, viewType,ident,&rh,npoints,z,r,color,"f")));
      break;
      
    case EPE:
      {
	/*
	  graphicMap_.insert(pair<int,GPFBaseUpgrade *> (ident,new GPFCandidateUpgrade(this, viewType,ident,&rh,npoints,eta,phi,color,"l")));
...
//_________________________________________________________________________________________
void DisplayManagerUpgrade::displayEvent(int run, int lumi, int event) {
  reset();
...
//________________________________________________________
void DisplayManagerUpgrade::display(int ientry)
{
  if (ientry<0 || ientry>maxEvents_) {
    std::cerr<<"DisplayManagerUpgrade::no event matching criteria"<<std::endl;
    return;
  }  
  reset();
...
void DisplayManagerUpgrade::displayAll(bool noRedraw)
{
  if (!isGraphicLoaded_) {
  ...
  std::multimap<int,GPFBaseUpgrade *>::iterator p;
  ...
  for (p=graphicMap_.begin();p!=graphicMap_.end();p++) {
    ...
    case GENPARTICLEID:
      ...
      break;           
    default : std::cout<<"DisplayManagerUpgrade::displayAll()-- unknown object "<<std::endl;               
    }  //switch end
  }   //for end
...
void DisplayManagerUpgrade::drawWithNewGraphicAttributes()
{
  std::multimap<int,GPFBaseUpgrade *>::iterator p;
...
//___________________________________________________________________________________
void DisplayManagerUpgrade::displayCanvas()
{
  double zLow = -500.;
  double zUp  = +500.;
  double rUp  = +300.;
...
//________________________________________________________________________________
void DisplayManagerUpgrade::displayNext()
{
  int eventNumber_=em_->eventNumber();
  display(++eventNumber_);
}
//_________________________________________________________________________________
void DisplayManagerUpgrade::displayNextInteresting(int ientry)
{
  bool ok=false;
  ...
  if (ok) {
    ...
    std::cerr<<"DisplayManagerUpgrade::dislayNextInteresting : no event matching criteria"<<std::endl;
}
//_________________________________________________________________________________
void DisplayManagerUpgrade::displayPrevious()
{
  int eventNumber_=em_->eventNumber();
  display(--eventNumber_);
}
//______________________________________________________________________________
void DisplayManagerUpgrade::rubOutGPFBlock()
{
...
void DisplayManagerUpgrade::displayPFBlock(int blockNb) 
{
  rubOutGPFBlock();
  selectedGObj_.clear();
  ...
  if (p !=blockIdentsMap_.end()) {
    ...
    cout<<"DisplayManagerUpgrade::displayPFBlock :not found"<<endl;    
}  
//_______________________________________________________________________________
void DisplayManagerUpgrade::drawGObject(int ident,int color,bool toInitial) 
{
  typedef std::multimap<int,GPFBaseUpgrade *>::const_iterator iter;
  iter p;
...
//______________________________________________________________________________
void DisplayManagerUpgrade::enableDrawPFBlock(bool state)
{
  drawPFBlocks_=state;
}  
//______________________________________________________________________________
void DisplayManagerUpgrade::enableDrawBrem(bool state)
{
  drawBrems_=state;
}  
//_______________________________________________________________________________
void DisplayManagerUpgrade::findAndDraw(int ident) 
{
        
  int type=ident >> shiftId_;            // defined in DisplayCommon.h
  int color=1;
  if (type>15) {
    std ::cout<<"DisplayManagerUpgrade::findAndDraw :object Type unknown"<<std::endl;
    return;
  }  
  if (drawPFBlocks_==0  ||
...
//___________________________________________________________________________________
void DisplayManagerUpgrade::findBlock(int ident) 
{
  int blockNb=-1;
  int elemNb=-1;
  multimap<int, pair <int,int > >::const_iterator p;
...
bool DisplayManagerUpgrade::findBadBremsId(int ident)
{
...
void DisplayManagerUpgrade::updateDisplay() {
...
//_________________________________________________________________________
double DisplayManagerUpgrade::getMaxE(int layer) const
{
  ...
  case PFLayer::PS2:
    ...
    break;
  default:
    cerr<<"DisplayManagerUpgrade::getMaxE : manage other layers"<<endl;
    return maxe;
  }
...
double DisplayManagerUpgrade::getMaxEEcal() {
...
//_______________________________________________________________________________
double DisplayManagerUpgrade::getMaxEHcal() {
...
void DisplayManagerUpgrade::loadGGenParticles()
{
...
//NICHOLAS JIN_______________________________________________________________
void DisplayManagerUpgrade::loadGCandidates() {
  ...
  cout << "Number of GCandidates: " << em_->pfCandCMSSW_.size() << endl;

  for(unsigned i=0; i<em_->pfCandCMSSW_.size(); i++) { 
    //int rhcolor = color;
    int recHitId=i;
    createGCandidate(em_->pfCandCMSSW_[i],recHitId,phi0);
    //cout << em_->pfCandCMSSW_[i].positionAtECALEntrance().Eta() << endl;
  }
...
//____________________________________________________________________________
void DisplayManagerUpgrade::createGGenParticle(HepMC::GenParticle* p)
{
...
//                            p->production_vertex()->position().z());
  ...
  if ( mother ) {
    ...
    for (int view = 2; view< NViews; view++) {
      GPFGenParticleUpgrade* gp   = new GPFGenParticleUpgrade(this,             
                                                view, genPartId,
                                                x, y,              //double *, double *
                                                e,pt,barcode,barcodeMother,
                                                genPartPattern_, 
                                                name,latexStringName);
      graphicMap_.insert(pair<int,GPFBaseUpgrade *>    (genPartId, gp));
    }
  }
  else {     //no Mother    
    for (int view = 2; view< NViews; view++) {
...
//____________________________________________________________________________  
void DisplayManagerUpgrade::loadGClusters()
{
  double phi0=0;
...
void DisplayManagerUpgrade::retrieveBadBrems()
{
...
//_____________________________________________________________________
void DisplayManagerUpgrade::loadGPFBlocks()
{
  int size = em_->pfBlocks_->size();
  for (int ibl=0;ibl<size;ibl++) {
...
void DisplayManagerUpgrade::loadGraphicObjects()
{
  loadGClusters();
  loadGRecHits();
  loadGRecTracks();
...
//________________________________________________________
void DisplayManagerUpgrade::loadGRecHits()
{
  double phi0=0;
  ...
  for(unsigned i=0; i<em_->rechitsECAL_.size(); i++) { 
    ...
    if( unsigned col = em_->clusterAlgoECAL_.color(i) ) {
      ...
      default:
        cerr<<"DisplayManagerUpgrade::loadGRecHits: unknown color"<<endl;
      }
    }
...
void DisplayManagerUpgrade::loadGRecTracks()
{
  double phi0=0;
...
//________________________________________________________________________
void DisplayManagerUpgrade::loadGGsfRecTracks()
{
  double phi0=0;
...
void DisplayManagerUpgrade::loadGSimParticles()
{
  double phi0=0;
...
void DisplayManagerUpgrade::lookForMaxRecHit(bool ecal)
{
...
//________________________________________________________________________________
void DisplayManagerUpgrade::lookForGenParticle(unsigned barcode) {
...
//_______________________________________________________________________
void DisplayManagerUpgrade::printDisplay(const char* sdirectory ) const
{
...
//_____________________________________________________________________________
void DisplayManagerUpgrade::reset()
{
  maxERecHitEcal_=-1;
  maxERecHitHcal_=-1;
  isGraphicLoaded_= false;
...
//_______________________________________________________________________________
void DisplayManagerUpgrade::unZoom()
{
...
//_______________________________________________________________________________
void DisplayManagerUpgrade::setNewAttrToSimParticles()
{
...
//_______________________________________________________________________________
void DisplayManagerUpgrade::printGenParticleInfo(std::string name,int barcode,int barcodeMother) 
{ 
  const HepMC::GenEvent* myGenEvent = em_->MCTruth_.GetEvent();
