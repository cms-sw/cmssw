#ifndef PF_DisplayManagerUpgrade_h
#define PF_DisplayManagerUpgrade_h
...
class GPFRecHitUpgrade;
class GPFClusterUpgrade;
class GPFTrackUpgrade;
class GPFSimParticleUpgrade;
class GPFBaseUpgrade;
class GPFGenParticleUpgrade;
...
class DisplayManagerUpgrade {
 ...
 public:
  DisplayManagerUpgrade( PFRootEventManagerUpgrade *em, 
                  const char* optfile );
  virtual ~DisplayManagerUpgrade();
  ...
  std::multimap<int,GPFBaseUpgrade *>  graphicMap_;
 ...
 private:
  ...
  /// NICHOLAS JIN
  void createGCandidate(reco::PFCandidate& candidate, int ident,
			double phi0 = 0.);
