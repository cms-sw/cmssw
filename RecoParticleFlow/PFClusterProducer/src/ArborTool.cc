...
float DisSeedSurface( TVector3 SeedPos )	//ECAL, HCAL, EndCapRing...
{
	...
	if( fabs(SeedPos[2]) > ECALHalfZ + 80 )		//EcalEndcap hit start from 2350 + 100 = 2450
	{
		/*
		if( SeedPos.Perp() < ECALRadius + 400  )
		{
			DisSS = fabs(SeedPos[2]) - ECALHalfZ - 103;           //Depth...	First layer: 2453.5
		}
		else
		{
			DisToR = SeedPos.Perp() - ECALRadius - 100; 			//Should Use Direct Distance, is it??
			DisToZ = fabs(SeedPos[2]) - ECALHalfZ - 103;
			
			DisSS = max(DisToR, DisToZ);
		}
		*/
