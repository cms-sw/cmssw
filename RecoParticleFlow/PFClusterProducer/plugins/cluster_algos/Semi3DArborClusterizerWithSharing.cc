#include "Semi3DArborClusterizerWithSharing.h"
...
void Semi3DArborClusterizerWithSharing::
buildClusters(const reco::PFClusterCollection& input,
	      const std::vector<bool>& seedable,
	      reco::PFClusterCollection& output) {
  reco::PFClusterCollection clustersInTopo, linkedSeeds;
  for( const auto& topocluster : input ) {
    clustersInTopo.clear(); linkedSeeds.clear();
...
growPFClusters(const reco::PFCluster& topo,
  ...
  if( iter >= _maxIterations ) {
    LOGDRESSED("Semi3DArborClusterizerWithSharing:growAndStabilizePFClusters")
