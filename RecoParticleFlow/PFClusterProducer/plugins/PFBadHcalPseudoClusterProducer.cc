...
void PFBadHcalPseudoClusterProducer::init(const EventSetup& iSetup)
{
    ...
    for (const DetId & i : chanquality.getAllChannels()) {
        ...
        if (status & statusMask) {
            ...
            good[std::make_pair(id.ieta(), id.iphi())]++;
        }
    }
    ...
    for (const auto & rec : bads) {
        ...
        bool barrel = std::abs(ieta) <= 16; // 16 is the largest ieta value for which the first depths are in the barrel.
        HcalSubdetector  subdet = barrel ? HcalBarrel : HcalEndcap;
        HcalDetId id(subdet, ieta, iphi, 1);
        bool isBad = (nbad > 0 && nbad >= ngood);
        if (debug_) std::cout << "At ieta " << id.ieta() << ", iphi " << id.iphi() << " I have " << nbad << " bad depths, " << ngood << " good depths. " << (isBad ? " MARK BAD": " ignore") << std::endl;
        if (!isBad) continue;
