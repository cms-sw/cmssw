...
goodPtResolution( const reco::TrackRef& trackref) const {
  ...
  const unsigned int NHit = 
    trackref->getHitPattern().trackerLayersWithMeasurement(reco::HitPattern::TRACK_HITS);
  const unsigned int NLostHit = 
    trackref->getHitPattern().trackerLayersWithoutMeasurement(reco::HitPattern::TRACK_HITS);
