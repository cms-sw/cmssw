...
PFBlockProducerNew::PFBlockProducerNew(const edm::ParameterSet& iConfig) {
  ...
  if (useNuclear_){
    if (nuclearInteractionsPurity > 3 || nuclearInteractionsPurity < 1)  {
      ...
      edm::LogInfo("PFBlockProducerNew")  << "NI purity not properly implemented. Set it to the strongest level " << nuclearInteractionsPurity << endl;
    }
...
PFBlockProducerNew::produce(Event& iEvent, 
  ...
  // LogDebug("PFBlockProducerNew")<<"get reco tracks"<<endl;
