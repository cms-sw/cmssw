...
template< template<typename> class T >
void
PFBlockAlgoNew::setInput(const T<reco::PFRecTrackCollection>&    trackh,
		      const T<reco::GsfPFRecTrackCollection>&    gsftrackh, 
		      const T<reco::GsfPFRecTrackCollection>&    convbremgsftrackh,
		      const T<reco::MuonCollection>&    muonh,
		      const T<reco::PFDisplacedTrackerVertexCollection>&  nuclearh,
  ...
  if(gsftrackh.isValid() ) {
	...
	if(seedRef.isAvailable() && seedRef->isEcalDriven()) {
	  ...
	  if(scRef.isNonnull())   {	     
		...
		elements_.emplace_back(sce);
	      }
	    else // it is already present, update the PFBE
	      {
		PFBlockElementSCEqual myEqual(scRef);
		ElementList::iterator itcheck=find_if(elements_.begin(),elements_.end(),myEqual);
		if(itcheck!=elements_.end())
		  {
		    reco::PFBlockElementSuperCluster* thePFBE=dynamic_cast<reco::PFBlockElementSuperCluster*>(itcheck->get());
...
// 	}
  ...
  if(convh.isValid() ) {
	...
	elements_.emplace_back( trkFromConversionElement );
  ...
  if(v0.isValid() ) {
	...
	if (bNew) {
	  ...
	  elements_.emplace_back( trkFromV0Element );
	}
  ...
  if(nuclearh.isValid()) {
	...
	for(unsigned iTk=0;iTk < trackSize; ++iTk) {
	  ...
	  for(IE iel = elements_.begin(); iel != elements_.end(); ++iel){
	    reco::TrackBaseRef elemTrackBaseRef((*iel)->trackRef());
	    if (newTrackBaseRef == elemTrackBaseRef){
	      trkFromDisplacedVertexElement = iel->get();
	      bNew = false;
	      continue;
	    }
	  }
	  ...
	  if (bNew) { 
	    ...
	    elements_.emplace_back( trkFromDisplacedVertexElement );
	  }
  ...
  if(trackh.isValid() ) {
	...
	if(convBremPFRecTracks.size() > 0.) {
      ...
      elements_.emplace_back( primaryElement );
    }
