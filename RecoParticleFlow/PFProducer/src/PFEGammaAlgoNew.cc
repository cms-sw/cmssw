...
fillPFCandidates(const std::list<PFEGammaAlgoNew::ProtoEGObject>& ROs,
  ...
  for( const auto& RO : ROs ) {    
    ...
    // with the erefined SC in hand we build a naive candidate p4 
...
// both places...
reco::SuperCluster PFEGammaAlgoNew::
buildRefinedSuperCluster(const PFEGammaAlgoNew::ProtoEGObject& RO) {  
