  ...
  if (bCorrect_){
	...
	if (debug_) 
	  cout << "" << endl << "Nuclear Interaction w no Primary Candidate " << ce1 
	       ...
	       << " Nmissing hits = " << pfCand->at(ce1).trackRef()->getHitPattern().numberOfHits(reco::HitPattern::MISSING_OUTER_HITS) << endl;
...
PFCandConnector::analyseNuclearWPrim(std::auto_ptr<PFCandidateCollection>& pfCand, unsigned int ce1) {
  ...
  for( unsigned int ce2=0; ce2 < pfCand->size(); ++ce2) {
	...
	if (debug_) cout << "\t here is a Secondary Candidate " << ce2  
			 ...
			 << " Nmissing hits = " << pfCand->at(ce2).trackRef()->getHitPattern().numberOfHits(reco::HitPattern::MISSING_OUTER_HITS) << endl;
	...
	int nMissOuterHits = pfCand->at(ce2).trackRef()->getHitPattern().numberOfHits(reco::HitPattern::MISSING_OUTER_HITS);
...
PFCandConnector::analyseNuclearWSec(std::auto_ptr<PFCandidateCollection>& pfCand, unsigned int ce1){
  ...
  int nMissOuterHits = pfCand->at(ce1).trackRef()->getHitPattern().numberOfHits(reco::HitPattern::MISSING_OUTER_HITS);
