  ...
  for( unsigned int ce2=0; ce2 < pfCand->size(); ++ce2) {
	...
	for (auto & elementsInBlock : elementsInBlocks){
	  ...
	  for (auto & elementsAlreadyInBlock : elementsAlreadyInBlocks){
	    if (elementsAlreadyInBlock.second == elementsInBlock.second) isAlreadyHere = true;
	  }
	  if (!isAlreadyHere) pfCand->at(ce1).addElementInBlock( elementsInBlock.first,  elementsInBlock.second);
	}
