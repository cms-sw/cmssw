  ...
  for(unsigned int ipft=0;ipft<PfRTkColl.size();ipft++){
	...
	for (unsigned int i1=0;i1<Idd[i].size();i1++){
	  for (unsigned int i2=0;i2<Idd[i+1].size();i2++){
		...
		bool newTrip=true;
		for (auto & iv : tripl){
		  if((iv[0]==tmp[0])&&(iv[1]==tmp[1])&&(iv[2]==tmp[2])) newTrip=false;
	...
	for (const auto & it2 : DetSet2){
	  ...
	  for (const auto & it3 : DetSet3){
		...
		PTrajectoryStateOnDet const & PTraj= 
		  trajectoryStateTransform::persistentState(updatedState,i[2]);
...
ConvBremSeedProducer::initializeLayerMap()
{
  ...
  for (auto posForwardLayer : posForwardLayers) {
    LogDebug("FastTracker") << "Z pos "
			    << (*posForwardLayer).surface().position().z()
			    << " radii " 
