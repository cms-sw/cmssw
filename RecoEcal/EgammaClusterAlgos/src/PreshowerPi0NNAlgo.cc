...
// Aris 10/7/2006
// ---------------
//void PreshowerPi0NNAlgo::findPreshVector(ESDetId strip,  RecHitsMap *rechits_map,
//                                                     CaloSubdetectorTopology *topology_p, std::vector<float>& vout_stripE)
std::vector<float> PreshowerPi0NNAlgo::findPreshVector(ESDetId strip,  RecHitsMap *rechits_map,
...
bool PreshowerPi0NNAlgo::goodPi0Strip(RecHitsMap::iterator candidate_it, ESDetId lastID)
{
...
void PreshowerPi0NNAlgo::findPi0Road(ESDetId strip, EcalPreshowerNavigator theESNav, 
...
//===================================================================
// PreshowerPi0NNAlgo::readWeightFile(...), a method that reads the weigths of the NN 
// INPUT: Weights_file
// OUTPUT: I_H_Weight, H_Thresh, H_O_Weight, O_Thresh arrays
//===================================================================
void PreshowerPi0NNAlgo::readWeightFile(char *Weights_file){
   FILE *weights;
...
//=====================================================================================
// PreshowerPi0NNAlgo::getNNoutput(float *input), a method that calculated the NN output 
// INPUT: input[25] -> the 25 input to the NN variables
// OUTPUT : nnout -> the NN output
//=====================================================================================
float PreshowerPi0NNAlgo::getNNoutput(float *input)
{
...
float PreshowerPi0NNAlgo::Activation_fun(float SUM){
        return( 1.0 / ( 1.0 + exp(-2.0*SUM) ) );
}
//=====================================================================================
// PreshowerPi0NNAlgo::calculateNNInputVariables(...), a method that calculates the 25 input variables 
// INPUTS: 
...
// f25 -> E25/Esc
//=====================================================================================
void PreshowerPi0NNAlgo::calculateNNInputVariables(std::vector<float>& vph1, std::vector<float>& vph2, 
...
float PreshowerPi0NNAlgo::GetNNOutput(float Et_SE, float *nn_invar_presh) 
{
    float nnout = -10;
...
//     std::string LocalRT = "/afs/cern.ch/user/a/akyriaki/scratch0";
     ...
     std::string nn_paterns_file  = "/src/RecoEcal/EgammaClusterProducers/data/nn_weights_files/";

// Choose the correct file according to the cluster's energy
     if(Et_SE<25.0) {nn_paterns_file = LocalRT + nn_paterns_file + "wght_endcap_et20.wts"; }
     else if(Et_SE>=25.0 && Et_SE<35.0) {nn_paterns_file = LocalRT + nn_paterns_file + "wght_endcap_et30.wts"; }            
     else if(Et_SE>=35.0 && Et_SE<45.0) {nn_paterns_file = LocalRT + nn_paterns_file + "wght_endcap_et40.wts"; } 
     else if(Et_SE>=45.0 && Et_SE<55.0) {nn_paterns_file = LocalRT + nn_paterns_file + "wght_endcap_et50.wts"; }
     else {nn_paterns_file = LocalRT + nn_paterns_file + "wght_endcap_et60.wts"; }
