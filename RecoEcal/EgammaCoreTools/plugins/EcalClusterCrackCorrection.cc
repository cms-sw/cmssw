...
public:
  EcalClusterCrackCorrection(const edm::ParameterSet &, edm::ConsumesCollector iC)
      : paramsToken_(iC.esConsumes()), geometryToken_(iC.esConsumes()) {}
...
private:
  edm::ESGetToken<EcalClusterCrackCorrParameters, EcalClusterCrackCorrParametersRcd> paramsToken_;
...
void EcalClusterCrackCorrection::init(const edm::EventSetup &es) {
  ...
  geometry_ = &es.getData(geometryToken_);
}
...
float EcalClusterCrackCorrection::getValue(const reco::CaloCluster &seedbclus) const {
  ...
  const CaloSubdetectorGeometry *geom = geometry_->getSubdetectorGeometry(DetId::Ecal, EcalBarrel);  //EcalBarrel = 1
