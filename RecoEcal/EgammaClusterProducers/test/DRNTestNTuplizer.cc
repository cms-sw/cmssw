...
#define initFloat \
  { initSingleFloat, initSingleFloat, initSingleFloat }
#define initInt \
  { initSingleInt, initSingleInt, initSingleInt }
#define initIntCharge \
  { initSingleIntCharge, initSingleIntCharge, initSingleIntCharge }
#define PDGID 11
...
private:
  ...
  // electron variables
  UInt_t _eleID[NELE] =
      initInt;  ///< bit mask for _eleID: 1=fiducial, 2=loose, 6=medium, 14=tight, 16=WP90PU, 48=WP80PU, 112=WP70PU, 128=loose25nsRun2, 384=medium25nsRun2, 896=tight25nsRun2, 1024=loose50nsRun2, 3072=medium50nsRun2, 7168=tight50nsRun2. Selection from https://twiki.cern.ch/twiki/bin/viewauth/CMS/EgammaCutBasedIdentification#Electron_ID_Working_Points
  Short_t _chargeEle[NELE] =
      initIntCharge;  ///< -100: no electron, 0: SC or photon, -1 or +1:electron or muon //Char_t is interpreted as char and not as integer
  ...
  Float_t _energy_ECAL_ele[NELE] =
      initFloat;  ///< ele-tuned regression energy: mustache for rereco and correctedEcalEnergy for official reco
...
void DRNTestNTuplizer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  using namespace edm;
  using namespace std;
  ...
  for (size_t i = 0; i < electrons->size(); i++) {
    ...
    if (!(ele1->ecalDrivenSeed()))
      continue;
    if (ele1->parentSuperCluster().isNull())
      continue;
    if (ele1->pt() < 15.)
      continue;
    if (!(*loose_id_decisions)[ele1])
      continue;
    for (size_t j = i + 1; j < electrons->size() && doFill == false; j++) {
      const auto ele2 = electrons->ptrAt(j);
      if (!(ele2->ecalDrivenSeed()))
        continue;
      if (ele2->parentSuperCluster().isNull())
        continue;
      if (ele2->pt() < 15.)
        continue;
      ...
      double t2q = t2 * t2;
      double angle =
          1 - ((1 - t1q) * (1 - t2q) + 4 * t1 * t2 * cos(ele1->phi() - ele2->phi())) / ((1 + t1q) * (1 + t2q));
  ...
  if (doFill)
    _tree->Fill();  // Write out the events
...
void DRNTestNTuplizer::beginJob() {
...
void DRNTestNTuplizer::endJob() {}

//Clear tree vectors each time analyze method is called

// ------------ method fills 'descriptions' with the allowed parameters for the module  ------------
void DRNTestNTuplizer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
void DRNTestNTuplizer::TreeSetSingleElectronVar(const reco::GsfElectron& electron, int index) {
  ...
  if (electron.ecalDrivenSeed()) {
    if (electron.trackerDrivenSeed())
      _recoFlagsEle[index] = 3;
    else
      _recoFlagsEle[index] = 2;
  } else
    _recoFlagsEle[index] = 1;
...
void DRNTestNTuplizer::InitNewTree() {
  ...
  _tree->Branch("energy_ECAL_ele",
                _energy_ECAL_ele,
                "energy_ECAL_ele[3]/F");  ///< correctedEcalEnergy from MINIAOD or mustache regression if rereco
...
void DRNTestNTuplizer::TreeSetPileupVar(void) {
  ...
  if (_primaryVertexHandle->size() > 0) {
    for (reco::VertexCollection::const_iterator v = _primaryVertexHandle->begin(); v != _primaryVertexHandle->end();
         ++v) {
      _nPV++;
    }
  }
