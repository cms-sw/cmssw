...
PreshowerPi0NNProducer::PreshowerPi0NNProducer(const edm::ParameterSet& ps) {
  ...
  // output collection
  new_ClusterPi0DiscriminatorCollection_  = ps.getParameter<std::string>("PreshClusterPi0DiscriminatorCollection");
  
  std::string debugString = ps.getParameter<std::string>("debugLevel");
  
  if      (debugString == "DEBUG")   debugL_pi0 = PreshowerPi0NNAlgo::pDEBUG;
  else if (debugString == "INFO")    debugL_pi0 = PreshowerPi0NNAlgo::pINFO;
  else                               debugL_pi0 = PreshowerPi0NNAlgo::pERROR;

  presh_pi0_algo = new PreshowerPi0NNAlgo(preshStripECut, preshNst, debugL_pi0); // Aris 20/7/2006 
  
  produces< reco::ClusterPi0DiscriminatorCollection >(new_ClusterPi0DiscriminatorCollection_);
...
PreshowerPi0NNProducer::~PreshowerPi0NNProducer() {
   delete presh_pi0_algo; 
}


void PreshowerPi0NNProducer::produce(edm::Event& evt, const edm::EventSetup& es) {

  if ( debugL_pi0 <= PreshowerPi0NNAlgo::pINFO ) std::cout << "\n .......  Event # " << nEvt_+1 
...
//  CaloSubdetectorTopology *& topology_p = topology;
  ...
  if ( debugL_pi0 == PreshowerPi0NNAlgo::pDEBUG ) std::cout << "PreshowerPi0NNProducerInfo: ### Total # of preshower RecHits: " 
  ...
  if ( debugL_pi0 <= PreshowerPi0NNAlgo::pINFO ) std::cout 
                                << "PreshowerPi0NNProducerInfo: ### Preshower RecHits_map of size " 
...
//  evt.getByLabel("ecalRecHit", "EcalRecHitsEE", rhcHandle);
  ...
  if (!(rhcHandle.isValid()))
    {
      std::cout << "Pi0rejection : could not get a handle on the EcalRecHitCollection!" << std::endl;
    }        
 ...
 // fetch the product (pSuperClusters)
  ...
  if ( debugL_pi0 <= PreshowerPi0NNAlgo::pINFO ) std::cout <<"### Total # Endcap Superclusters: " << SClusts->size() << std::endl;
  if ( debugL_pi0 == PreshowerPi0NNAlgo::pDEBUG ) std::cout << " Making a cycle over Superclusters ..." << std::endl; 
  ...
  for ( it_super=SClusts->begin();  it_super!=SClusts->end(); it_super++ ) { 
      ...
      if ( debugL_pi0 <= PreshowerPi0NNAlgo::pINFO ) std::cout << " superE = " << it_super->energy() 
       ...
       if(fabs(it_super->eta()) > 1.5) { 
          ...
          std::cout << "SC centroind = " << pointSC << std::endl;
  ...
  // Parameters for the position calculation:
          ...
          std::cout << "BC energy_max = " <<  SC_seed_energy << std::endl;
          std::cout << "ClusterShape  E1_max = " <<   SC_seed_Shape_E1 << std::endl;
          std::cout << "ClusterShape  E3x3_max = " <<   SC_seed_Shape_E3x3 <<  std::endl;
          std::cout << "ClusterShape  E5x5_max = " <<   SC_seed_Shape_E5x5 << std::endl;
...
//          presh_pi0_algo->findPreshVector(stripY, &rechits_map, topology_p, vout_stripE2);
          ...
          std::cout << "SC Et = " << SE_et << std::endl;
          nnoutput = presh_pi0_algo->GetNNOutput(SE_et, nn_invar_presh);
       }
       std::cout << "PreshowerPi0NNProducer: SC NNOUTPUT = " <<  nnoutput << " and SC contains " 
   ...
   evt.put(Pi0DiscCollection, new_ClusterPi0DiscriminatorCollection_);
   if ( debugL_pi0 <= PreshowerPi0NNAlgo::pINFO ) std::cout << "Corrected SClusters added to the event" << std::endl;
