...
void CalibratedElectronProducer::produce( edm::Event & event, const edm::EventSetup & setup )
 {
  ...
  for ( unsigned iele = 0; iele < nElectrons ; ++iele) {
    ...
    SimpleElectron mySimpleElectron(run, elClass, r9, correctedEcalEnergy, correctedEcalEnergyError, trackMomentum, trackMomentumError, regressionEnergy, regressionEnergyError, combinedMomentum, combinedMomentumError, ele.superCluster()->eta(), ele.isEB(), isMC, ele.ecalDriven(), ele.trackerDrivenSeed());
