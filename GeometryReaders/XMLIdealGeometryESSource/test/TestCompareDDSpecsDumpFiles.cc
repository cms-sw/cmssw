...
class TestCompareDDSpecsDumpFiles : public edm::one::EDAnalyzer<> {
public:
...
private:
  ...
  std::string fillAndSort(const tokenizer::iterator& start,
...
std::string TestCompareDDSpecsDumpFiles::clean(const std::string& in) {
  std::string str(in);
  boost::trim(str);
  size_t found1;
  found1 = str.find('/');
...
std::string TestCompareDDSpecsDumpFiles::preFill(const tokenizer::iterator& it, std::list<std::string>& list) {
...
std::string TestCompareDDSpecsDumpFiles::merge(const std::list<std::string>& list) {
  std::string str("");
...
std::string TestCompareDDSpecsDumpFiles::fillAndSort(const tokenizer::iterator& start,
                                                     const tokenizer::iterator& end,
                                                     std::list<std::string>& list) {
...
void TestCompareDDSpecsDumpFiles::analyze(const edm::Event&, const edm::EventSetup&) {
  std::string l1, l2;
  ...
  while (!f1_.eof() && !f2_.eof()) {
    ...
    edm::LogInfo("TestCompareDDSpecsDumpFiles")
        << "#" << ++line << " Comparing " << firstStr1 << " " << firstStr1.size() << " with " << firstStr2 << " "
        << firstStr2.size() << " : ";
    ...
    if (l1 != l2) {
      ...
      if (sl1 != sl2) {
        edm::LogError("TestCompareDDSpecsDumpFiles") << "#" << line << " Lines don't match.\n"
                                                     << "[" << l1 << "]\n"
                                                     << "[" << l2 << "]\n";
