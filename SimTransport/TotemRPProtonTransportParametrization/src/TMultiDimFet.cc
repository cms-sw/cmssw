...
const TMultiDimFet &TMultiDimFet::operator=(const TMultiDimFet &in) {
  ...
  fMaxFunctionsTimesNVariables =
      in.fMaxFunctionsTimesNVariables; // fMaxFunctionsTimesNVariables
  fPowers = in.fPowers;
...
TMultiDimFet::TMultiDimFet(Int_t dimension, EMDFPolyType type, Option_t *option)
    : TNamed("multidimfit", "Multi-dimensional fit object"),
      fQuantity(dimension), fSqError(dimension), fVariables(dimension * 100),
      fMeanVariables(dimension), fMaxVariables(dimension),
...
void TMultiDimFet::AddTestRow(const Double_t *x, Double_t D, Double_t E) {
  ...
  for (i = 0; i < fNVariables; i++) {
    ...
    if (x[i] > fMaxVariables(i))
      Warning("AddTestRow", "variable %d (row: %d) too large: %f > %f", i,
              fTestSampleSize, x[i], fMaxVariables(i));
    if (x[i] < fMinVariables(i))
      Warning("AddTestRow", "variable %d (row: %d) too small: %f < %f", i,
              fTestSampleSize, x[i], fMinVariables(i));
  }
}
...
void TMultiDimFet::ZeroDoubiousCoefficients(double error) {
  ...
  for (cmt::iterator it = m.begin(); it != m.end() && del_error_abs < error;
       ++it) {
...
void TMultiDimFet::Fit(Option_t *option)
{
   ...
   //
   // Also, refer to
   // Begin_Html<a href="#TMultiDimFet:description">class
description</a>End_Html
...
Double_t TMultiDimFet::MakeChi2(const Double_t *coeff) {
  ...
  for (i = 0; i < fTestSampleSize; i++) {
    ...
    fChi2 += 1. / TMath::Max(fTestSqError(i), 1e-20) * (fTestQuantity(i) - f) *
             (fTestQuantity(i) - f);
  }
...
void TMultiDimFet::MakeCoefficients() {
    ...
    for (row = col - 1; row > -1; row--) {
      ...
      for (i = row; i <= col; i++)
        fOrthCurvatureMatrix(row, col) -=
            fOrthCurvatureMatrix(i, row) * fOrthCurvatureMatrix(i, col);
    }
  }
  ...
  if (TESTBIT(fHistogramMask, HIST_RD) || TESTBIT(fHistogramMask, HIST_RTRAI) ||
      TESTBIT(fHistogramMask, HIST_RX)) {
    for (i = 0; i < fSampleSize; i++) {
      if (TESTBIT(fHistogramMask, HIST_RD))
        ((TH2D *)fHistograms->FindObject("res_d"))
            ->Fill(fQuantity(i), fResiduals(i));
...
void TMultiDimFet::MakeCorrelation() {
  ...
  Double_t d2 = 0;
  Double_t ddotXi =
      0; // G.Q. needs to be reinitialized in the loop over i fNVariables
  Double_t xiNorm =
      0; // G.Q. needs to be reinitialized in the loop over i fNVariables
  Double_t xidotXj =
      0; // G.Q. needs to be reinitialized in the loop over j fNVariables
  Double_t xjNorm =
      0; // G.Q. needs to be reinitialized in the loop over j fNVariables
...
Double_t TMultiDimFet::MakeGramSchmidt(Int_t function) {
  ...
  for (j = 0; j < fSampleSize; j++) {
    ...
    fOrthFunctionNorms(fNCoefficients) +=
        fOrthFunctions(fNCoefficients, j) * fOrthFunctions(fNCoefficients, j);

    // calculate D dot w_fNCoefficients in A
    fOrthCoefficients(fNCoefficients) +=
        fQuantity(j) * fOrthFunctions(fNCoefficients, j);
  }
...
void TMultiDimFet::MakeHistograms(Option_t *option) {
    ...
    for (i = 0; i < fNVariables; i++)
      if (!fHistograms->FindObject(Form("x_%d_orig", i)))
        fHistograms->Add(new TH1D(Form("x_%d_orig", i),
                                  Form("Original variable # %d", i), 100,
                                  fMinVariables(i), fMaxVariables(i)));
  }
  ...
  if (opt.Contains("d") || opt.Contains("a")) {
    ...
    if (!fHistograms->FindObject("d_orig"))
      fHistograms->Add(new TH1D("d_orig", "Original Quantity", 100,
                                fMinQuantity, fMaxQuantity));
  }
    ...
    for (i = 0; i < fNVariables; i++)
      if (!fHistograms->FindObject(Form("x_%d_norm", i)))
        fHistograms->Add(new TH1D(Form("x_%d_norm", i),
                                  Form("Normalized variable # %d", i), 100, -1,
                                  1));
  }
  ...
  if (opt.Contains("s") || opt.Contains("a")) {
    ...
    if (!fHistograms->FindObject("d_shifted"))
      fHistograms->Add(new TH1D("d_shifted", "Shifted Quantity", 100,
    ...
    for (i = 0; i < fNVariables; i++)
      if (!fHistograms->FindObject(Form("res_x_%d", i)))
        fHistograms->Add(new TH2D(Form("res_x_%d", i),
                                  Form("Computed residual versus x_%d", i), 100,
                                  -1, 1, 35, fMinQuantity - fMeanQuantity,
                                  fMaxQuantity - fMeanQuantity));
  }
  ...
  if (opt.Contains("r2") || opt.Contains("a")) {
    ...
    if (!fHistograms->FindObject("res_d"))
      fHistograms->Add(new TH2D(
          "res_d", "Computed residuals vs Quantity", 100,
          fMinQuantity - fMeanQuantity, fMaxQuantity - fMeanQuantity, 35,
          fMinQuantity - fMeanQuantity, fMaxQuantity - fMeanQuantity));
  }
  ...
  if (opt.Contains("r3") || opt.Contains("a")) {
    ...
    if (!fHistograms->FindObject("res_train"))
      fHistograms->Add(
          new TH1D("res_train", "Computed residuals over training sample", 100,
                   fMinQuantity - fMeanQuantity, fMaxQuantity - fMeanQuantity));
  }
  if (opt.Contains("r4") || opt.Contains("a")) {
    ...
    if (!fHistograms->FindObject("res_test"))
      fHistograms->Add(
          new TH1D("res_test", "Distribution of residuals from test", 100,
                   fMinQuantity - fMeanQuantity, fMaxQuantity - fMeanQuantity));
  }
}
...
void TMultiDimFet::MakeParameterization() {
  ...
  while (kTRUE) {
    ...
    if (fIsVerbose) {
      edm::LogVerbatim("TMultiDimFet")
          << std::setw(5) << fNCoefficients << " " << std::setw(10)
          << std::setprecision(4) << squareResidual << " " << std::setw(10)
          << std::setprecision(4) << dResidur << " " << std::setw(7)
          << std::setprecision(3) << fMaxAngle << " " << std::setw(7)
          << std::setprecision(3) << s << " " << std::setw(5) << i << " "
          << std::setw(10) << std::setprecision(4)
          << fOrthCoefficients(fNCoefficients - 1) << " " << std::setw(10)
          << std::setprecision(4) << fOrthFunctionNorms(fNCoefficients - 1)
          << " " << std::flush;
      for (j = 0; j < fNVariables; j++)
        edm::LogInfo("TMultiDimFet")
    ...
    Double_t err =
        TMath::Sqrt(TMath::Max(1e-20, squareResidual) / fSumSqAvgQuantity);
...
//____________________________________________________________________
void TMultiDimFet::MakeRealCode(const char *filename, const char *classname,
                                Option_t *) {
  // PRIVATE METHOD:
  ...
  if (fIsVerbose)
    edm::LogInfo("TMultiDimFet")
        << "Writing on file \"" << filename << "\" ... " << std::flush;
  //
  ...
  outFile << cv_qual << "int    " << prefix
          << "gNVariables    = " << fNVariables << ";"
          << "\n";
  outFile << cv_qual << "int    " << prefix
          << "gNCoefficients = " << fNCoefficients << ";"
          << "\n";
  outFile << cv_qual << "double " << prefix
          << "gDMean         = " << fMeanQuantity << ";"
          << "\n";
  ...
  outFile << "// Assignment to coefficients vector."
          << "\n";
  outFile << cv_qual << "double " << prefix << "gCoefficient[] = {"
          << std::flush;
  for (i = 0; i < fNCoefficients; i++)
    outFile << (i != 0 ? "," : "") << "\n"
  ...
  outFile << "// "
          << "\n"
          << "// The " << (isMethod ? "method " : "function ") << "  double "
          << prefix << "MDF(double *x)"
          << "\n"
          << "// "
          << "\n";
  outFile << "double " << prefix << "MDF(double *x) {"
          ...
          << "\n"
          << "      int power = " << prefix << "gPower[" << prefix
          << "gNVariables * i + j]; "
          << "\n"
          << "      double p1 = 1, p2 = 0, p3 = 0, r = 0;"
          << "\n"
          << "      double v =  1 + 2. / (" << prefix << "gXMax[j] - " << prefix
...
void TMultiDimFet::Print(Option_t *option) const {
  ...
  if (opt.Contains("s")) {
    ...
    edm::LogInfo("TMultiDimFet")
        << "\n"
        << " Max:   " << std::setw(10) << std::setprecision(7) << fMaxQuantity
        << std::flush;
    for (i = 0; i < fNVariables; i++)
      edm::LogInfo("TMultiDimFet")
          << " " << std::setw(10) << std::setprecision(4) << fMaxVariables(i)
          << std::flush;
    edm::LogInfo("TMultiDimFet")
        << "\n"
        << " Min:   " << std::setw(10) << std::setprecision(7) << fMinQuantity
        << std::flush;
    for (i = 0; i < fNVariables; i++)
      edm::LogInfo("TMultiDimFet")
          << " " << std::setw(10) << std::setprecision(4) << fMinVariables(i)
          << std::flush;
    edm::LogInfo("TMultiDimFet")
        << "\n"
        << " Mean:  " << std::setw(10) << std::setprecision(7) << fMeanQuantity
        << std::flush;
    for (i = 0; i < fNVariables; i++)
      edm::LogInfo("TMultiDimFet")
          << " " << std::setw(10) << std::setprecision(4) << fMeanVariables(i)
          << std::flush;
    edm::LogInfo("TMultiDimFet")
        << "\n"
  ...
  if (opt.Contains("c")) {
    ...
    for (i = 0; i < fNCoefficients; i++) {
      edm::LogInfo("TMultiDimFet")
          << " " << std::setw(3) << i << "  " << std::setw(12)
          << fCoefficients(i) << "  " << std::setw(12) << fCoefficientsRMS(i)
          << "  " << std::flush;
      for (j = 0; j < fNVariables; j++)
        edm::LogInfo("TMultiDimFet")
            << " " << std::setw(3)
...
void TMultiDimFet::PrintPolynomialsSpecial(Option_t *option) const {
  ...
  if (opt.Contains("m")) {
    ...
    for (i = 0; i < fNVariables; i++)
      edm::LogInfo("TMultiDimFet")
          << "\tdouble y" << i << "\t=1+2*(x" << i << "-" << fMaxVariables(i)
          << ")/(" << fMaxVariables(i) << "-" << fMinVariables(i) << ");"
          << "\n";
    edm::LogInfo("TMultiDimFet") << "\n"
                                 << "  f[";
