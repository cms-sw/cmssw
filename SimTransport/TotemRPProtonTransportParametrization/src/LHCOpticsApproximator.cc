...
LHCOpticsApproximator::LHCOpticsApproximator(
    ...
    TMultiDimFet::EMDFPolyType polynom_type, std::string beam_direction,
  ...
  nominal_beam_energy_ =
      TMath::Sqrt(nominal_beam_momentum_ * nominal_beam_momentum_ +
...
double LHCOpticsApproximator::ParameterOutOfRangePenalty(
    double in[], bool invert_beam_coord_sytems) const {
  ...
  for (int i = 0; i < 5; i++) {
    if (in_corrected[i] < (*min_var)(i)) {
      double dist = TMath::Abs(((*min_var)(i)-in_corrected[i]) /
                               ((*max_var)(i) - (*min_var)(i)));
      res += 8 * (TMath::Exp(dist) - 1.0);
      in_corrected[i] = (*min_var)(i);
    } else if (in_corrected[i] > (*max_var)(i)) {
      double dist = TMath::Abs((in_corrected[i] - (*max_var)(i)) /
                               ((*max_var)(i) - (*min_var)(i)));
      res += 8 * (TMath::Exp(dist) - 1.0);
...
bool LHCOpticsApproximator::Transport_m_GeV(
    double in_pos[3], double in_momentum[3], double out_pos[3],
    double out_momentum[3], bool check_apertures, double z2_z1_dist) const {
  double in[5];
  double out[5];
  double part_mom = 0.0;
  for (int i = 0; i < 3; i++)
  ...
  out_momentum[2] = TMath::Sqrt(part_out_total_mom * part_out_total_mom -
                                out_momentum[0] * out_momentum[0] -
...
LHCOpticsApproximator::LHCOpticsApproximator(const LHCOpticsApproximator &org)
    : TNamed(org), x_parametrisation(org.x_parametrisation),
...
const LHCOpticsApproximator &LHCOpticsApproximator::
operator=(const LHCOpticsApproximator &org) {
  if (this != &org) {
...
void LHCOpticsApproximator::Train(TTree *inp_tree, std::string data_prefix,
                                  polynomials_selection mode, int max_degree_x,
  ...
  InitializeApproximators(mode, max_degree_x, max_degree_tx, max_degree_y,
                          max_degree_ty, common_terms);
  ...
  edm::LogInfo("LHCOpticsApproximator")
      << "Optical functions parametrizations from " << s_begin_ << " to "
      << s_end_ << "\n";
  PrintInputRange();
  for (int i = 0; i < 4; i++) {
    ...
    edm::LogInfo("LHCOpticsApproximator")
        << "Out variable " << coord_names[i] << " polynomial"
        << "\n";
...
void LHCOpticsApproximator::InitializeApproximators(
    polynomials_selection mode, int max_degree_x, int max_degree_tx,
    int max_degree_y, int max_degree_ty, bool common_terms) {
  SetDefaultAproximatorSettings(x_parametrisation, VariableType::X,
                                max_degree_x);
  SetDefaultAproximatorSettings(theta_x_parametrisation, VariableType::THETA_X,
                                max_degree_tx);
  SetDefaultAproximatorSettings(y_parametrisation, VariableType::Y,
                                max_degree_y);
  SetDefaultAproximatorSettings(theta_y_parametrisation, VariableType::THETA_Y,
                                max_degree_ty);

  if (mode == PREDEFINED) {
    SetTermsManually(x_parametrisation, VariableType::X, max_degree_x,
                     common_terms);
    SetTermsManually(theta_x_parametrisation, VariableType::THETA_X,
                     max_degree_tx, common_terms);
    SetTermsManually(y_parametrisation, VariableType::Y, max_degree_y,
                     common_terms);
    SetTermsManually(theta_y_parametrisation, VariableType::THETA_Y,
                     max_degree_ty, common_terms);
  }
}

void LHCOpticsApproximator::SetDefaultAproximatorSettings(
    TMultiDimFet &approximator, VariableType var_type, int max_degree) {
...
void LHCOpticsApproximator::SetTermsManually(TMultiDimFet &approximator,
  ...
  // push common terms
  if (common_terms) {
    term_literals.push_back(1), term_literals.push_back(1),
        term_literals.push_back(0), term_literals.push_back(0),
        term_literals.push_back(0);
    term_literals.push_back(1), term_literals.push_back(0),
        term_literals.push_back(1), term_literals.push_back(0),
        term_literals.push_back(0);
    term_literals.push_back(1), term_literals.push_back(0),
        term_literals.push_back(0), term_literals.push_back(1),
        term_literals.push_back(0);
    term_literals.push_back(0), term_literals.push_back(1),
        term_literals.push_back(1), term_literals.push_back(0),
        term_literals.push_back(0);
    ...
    term_literals.push_back(1), term_literals.push_back(2),
        term_literals.push_back(0), term_literals.push_back(0),
        term_literals.push_back(0);
    term_literals.push_back(1), term_literals.push_back(0),
        term_literals.push_back(2), term_literals.push_back(0),
        term_literals.push_back(0);
    term_literals.push_back(1), term_literals.push_back(0),
        term_literals.push_back(0), term_literals.push_back(2),
        term_literals.push_back(0);
    term_literals.push_back(2), term_literals.push_back(1),
        term_literals.push_back(0), term_literals.push_back(0),
        term_literals.push_back(0);
    term_literals.push_back(0), term_literals.push_back(1),
        term_literals.push_back(2), term_literals.push_back(0),
...
void LHCOpticsApproximator::Test(TTree *inp_tree, TFile *f_out,
  ...
  WriteHistograms(err_hists, err_inp_cor_hists, err_out_cor_hists, f_out,
                  base_out_dir);
...
void LHCOpticsApproximator::AllocateErrorHists(TH1D *err_hists[4]) {
  ...
  for (int i = 0; i < 4; ++i) {
    err_hists[i] = new TH1D(error_labels[i].c_str(), error_labels[i].c_str(),
                            100, -0.0000000001, 0.0000000001);
...
void LHCOpticsApproximator::TestAperture(
    TTree *inp_tree, TTree *out_tree) // x, theta_x, y, theta_y, ksi,
...
void LHCOpticsApproximator::AllocateErrorInputCorHists(
    TH2D *err_inp_cor_hists[4][5]) {
  std::vector<std::string> error_labels;
  ...
  for (int eri = 0; eri < 4; ++eri) {
    for (int dati = 0; dati < 5; ++dati) {
      ...
      err_inp_cor_hists[eri][dati] =
          new TH2D(name.c_str(), title.c_str(), 100, -0.0000000001,
                   0.0000000001, 100, -0.0000000001, 0.0000000001);
...
void LHCOpticsApproximator::AllocateErrorOutputCorHists(
    TH2D *err_out_cor_hists[4][5]) {
  std::vector<std::string> error_labels;
  std::vector<std::string> data_labels;
...
void LHCOpticsApproximator::FillErrorDataCorHistograms(
    double errors[4], double var[5], TH2D *err_cor_hists[4][5]) {
  for (int eri = 0; eri < 4; ++eri) {
    for (int dati = 0; dati < 5; ++dati) {
...
void LHCOpticsApproximator::DeleteErrorCorHistograms(
...
void LHCOpticsApproximator::PrintInputRange() {
  ...
  for (int i = 0; i < 5; i++) {
    edm::LogInfo("LHCOpticsApproximator")
        << (*min_var)(i) << " < " << coord_names[i] << " < " << (*max_var)(i)
        << "\n";
  }
  edm::LogInfo("LHCOpticsApproximator") << "\n";
}

bool LHCOpticsApproximator::CheckInputRange(
    const double *in,
    bool invert_beam_coord_sytems) const // x, thx, y, thy, ksi
{
  ...
  for (int i = 0; i < 5; i++) {
    res = res && in_corrected[i] >= (*min_var)(i) &&
          in_corrected[i] <= (*max_var)(i);
  }
  return res;
}
...
void LHCOpticsApproximator::AddRectEllipseAperture(
    const LHCOpticsApproximator &in, double rect_x, double rect_y,
    double r_el_x, double r_el_y) {
  apertures_.push_back(LHCApertureApproximator(
      in, rect_x, rect_y, r_el_x, r_el_y,
...
LHCApertureApproximator::LHCApertureApproximator(
    const LHCOpticsApproximator &in, double rect_x, double rect_y,
    double r_el_x, double r_el_y, ApertureType type)
...
bool LHCApertureApproximator::CheckAperture(
    const double *in,
    bool invert_beam_coord_sytems) const // x, thx. y, thy, ksi
{
  double out[5];
  ...
  if (ap_type_ == ApertureType::RECTELLIPSE) {
    result = result && out[0] < rect_x_ && out[0] > -rect_x_ &&
             out[2] < rect_y_ && out[2] > -rect_y_ &&
             (out[0] * out[0] / (r_el_x_ * r_el_x_) +
                  out[2] * out[2] / (r_el_y_ * r_el_y_) <
              1);
  }
  return result;
}
...
void LHCOpticsApproximator::PrintOpticalFunctions() {
  edm::LogInfo("LHCOpticsApproximator") << std::endl
                                        ...
                                        << "\n";
  for (int i = 0; i < 4; i++) {
    PrintCoordinateOpticalFunctions(*out_polynomials[i], coord_names[i],
                                    coord_names);
  }
}

void LHCOpticsApproximator::PrintCoordinateOpticalFunctions(
    TMultiDimFet &parametrization, const std::string &coord_name,
  ...
  for (int i = 0; i < 5; i++) {
    edm::LogInfo("LHCOpticsApproximator")
        << " + " << d_out_d_in[i] << "*" << input_vars[i];
  }
  edm::LogInfo("LHCOpticsApproximator") << "\n";
}

void LHCOpticsApproximator::GetLinearApproximation(double atPoint[], double &Cx,
                                                   double &Lx, double &vx,
                                                   double &Cy, double &Ly,
                                                   double &vy, double &D,
                                                   double ep) {
...
void LHCOpticsApproximator::GetLineariasedTransportMatrixX(
    double mad_init_x, double mad_init_thx, double mad_init_y,
    double mad_init_thy, double mad_init_xi, TMatrixD &transp_matrix,
    double d_mad_x, double d_mad_thx) {
  ...
  transp_matrix(1, 0) =
      (thx2_dx - thx1) / (d_mad_x * MADX_momentum_correction_factor);
  transp_matrix(0, 1) =
      MADX_momentum_correction_factor * (x2_dthx - x1) / d_mad_thx;
...
void LHCOpticsApproximator::GetLineariasedTransportMatrixY(
  ...
  transp_matrix(1, 0) =
      (thy2_dy - thy1) / (d_mad_y * MADX_momentum_correction_factor);
  transp_matrix(0, 1) =
      MADX_momentum_correction_factor * (y2_dthy - y1) / d_mad_thy;
  transp_matrix(1, 1) = (thy2_dthy - thy1) / d_mad_thy;
}

// MADX convention used only for input
double LHCOpticsApproximator::GetDx(double mad_init_x, double mad_init_thx,
...
// MADX convention used only for input
// angular dispersion
double LHCOpticsApproximator::GetDxds(double mad_init_x, double mad_init_thx,
                                      double mad_init_y, double mad_init_thy,
                                      double mad_init_xi, double d_mad_xi) {
  double MADX_momentum_correction_factor = 1.0 + mad_init_xi;
  double in[5];
