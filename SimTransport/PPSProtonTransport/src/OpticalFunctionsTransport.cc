...
// Choose the optical function corresponding to the first station ono each side (it is in lhc ref. frame)
  ...
  for (const auto& ofp:(*opticalFunctions_)) {
      if (ofp.second.getScoringPlaneZ()<0) {
         if (optFunctionId45_==0) optFunctionId45_=ofp.first;
         if (opticalFunctions_->at(optFunctionId45_).getScoringPlaneZ()<ofp.second.getScoringPlaneZ()) optFunctionId45_=ofp.first;
      }
      if (ofp.second.getScoringPlaneZ()>0) {
         if (optFunctionId56_==0) optFunctionId56_= ofp.first;
         if (opticalFunctions_->at(optFunctionId56_).getScoringPlaneZ()>ofp.second.getScoringPlaneZ()) optFunctionId56_=ofp.first;
      }
  }
//
...
bool OpticalFunctionsTransport::transportProton(const HepMC::GenParticle* in_trk) {
  ...
  if (useEmpiricalApertures_) {
    const auto& xangle = (lhcInfo_->crossingAngle()>1.0)?lhcInfo_->crossingAngle()*urad:lhcInfo_->crossingAngle();
