...
Hector::Hector(const edm::ParameterSet & param, 
  ...
  if(m_FP420Transport && lengthfp420>0. ) {
    ...
    if ( m_verbosity ) edm::LogVerbatim("HectorSetup") << "Hector: WARNING: lengthfp420=  " << lengthfp420;
  }
  ...
  if (m_ZDCTransport && lengthzdc>0. && lengthd1>0.) {
    ...
    if ( m_verbosity ) edm::LogVerbatim("HectorSetup") << "Hector: WARNING: lengthzdc=  " << lengthzdc << "lengthd1=  " << lengthd1;
  }
    ...
    if ( (*eventParticle)->status() == 1 ) {
      if ( abs( (*eventParticle)->momentum().eta())>etacut){
        ...
        if ( m_beamPart.find(line) == m_beamPart.end() ) {
          ...
          if(m_verbosity) { 
            edm::LogVerbatim("HectorEventProcessing") << "Hector:add: barcode = " << line 
                                              << " status = " << g->status() 
                                              << " PDG Id = " << g->pdg_id() 
                                              << " mass = " << mass 
                                              << " pz = " << pz 
...
void Hector::filterFP420(TRandom3* rootEngine){
  ...
  if ( !m_beamPart.empty() && lengthfp420>0. ) {
    ...
    for (it = m_beamPart.begin(); it != m_beamPart.end(); ++it ) {
      ...
      if(m_verbosity) {
        edm::LogVerbatim("HectorEventProcessing") << "Hector:filterFP420: barcode = " << line;
      }
      if ( (*m_isCharged.find( line )).second ) {
        ...
        if ( direction == 1 && m_beamlineFP4201 != 0 ) {
          ...
          if(m_verbosity) edm::LogVerbatim("HectorEventProcessing") << "Hector:filterFP420: barcode = " << line << " positive is_stop=  "<< is_stop;
        }
        else if ( direction == -1 && m_beamlineFP4202 != 0 ){
          part->computePath( m_beamlineFP4202 );
          is_stop = part->stopped( m_beamlineFP4202 );
          if(m_verbosity) edm::LogVerbatim("HectorEventProcessing") << "Hector:filterFP420: barcode = " << line << " negative is_stop=  "<< is_stop;
        }
        else {
          is_stop = true;
          if(m_verbosity) edm::LogVerbatim("HectorEventProcessing") << "Hector:filterFP420: barcode = " << line << " 0        is_stop=  "<< is_stop;
        }
        
        //propagating
        m_isStoppedfp420[line] = is_stop;
        if(m_verbosity) edm::LogVerbatim("HectorEventProcessing") << "Hector:filterFP420: barcode = " << line << " isStopped=" << (*m_isStoppedfp420.find(line)).second;
        ...
        if (!is_stop) {
          ...
          x1_420 = part->getX();
          y1_420 = part->getY();
          if(m_verbosity) edm::LogVerbatim("HectorEventProcessing") << "Hector:filterFP420: barcode = " << line << " x=  "<< x1_420 <<" y= " << y1_420;
...
void Hector::filterZDC(TRandom3* rootEngine){
  ...
  if ( !m_beamPart.empty() && lengthzdc>0. ) {
    ...
    for (it = m_beamPart.begin(); it != m_beamPart.end(); ++it ) {
      ...
      part = (*it).second;
      if(m_verbosity) {
	edm::LogVerbatim("HectorEventProcessing") << "Hector:filterZDC: barcode = " << line << " charge  = " << (*m_isCharged.find(line)).second;
	if (m_FP420Transport) edm::LogVerbatim("HectorEventProcessing") << " isStoppedFP420 =" << (*m_isStoppedfp420.find(line)).second;
      }
      //      if ( ((*m_isStoppedfp420.find(line)).second) && ((*m_isCharged.find(line)).second) ) {
      if ( ((*m_isCharged.find(line)).second) ) {
	
	if(m_verbosity) edm::LogVerbatim("HectorEventProcessing") << "Hector:filterZDC: barcode = " << line << " propagated ";
	
        direction = (*m_direct.find( line )).second;
        if(m_verbosity) edm::LogVerbatim("HectorEventProcessing") << "Hector:filterZDC: barcode = " << line << " direction = " << direction;
        if (m_smearAng) {
          if ( m_sigmaSTX>0. && m_sigmaSTY>0.) {
            // the beam transverse direction is centered on (TXforPosition, TYforPosition) at IP
            part->smearAng(m_sigmaSTX,m_sigmaSTY,rootEngine);
        ...
        if ( direction == 1 && m_beamlineZDC1 != 0 ){
          ...
          if(m_verbosity) edm::LogVerbatim("HectorEventProcessing") << "Hector:filterZDC: barcode " << line << " positive is_stop_zdc=  "<< is_stop_zdc;
        } else if ( direction == -1 && m_beamlineZDC2 != 0 ){
          part->computePath( m_beamlineZDC2 );
          is_stop_zdc = part->stopped( m_beamlineZDC2 );
          m_isStoppedzdc[line] = is_stop_zdc;
          if(m_verbosity) edm::LogVerbatim("HectorEventProcessing") << "Hector:filterZDC: barcode " << line << " negative is_stop_zdc=  "<< is_stop_zdc;
        } else {
          m_isStoppedzdc[line] = true;
          if(m_verbosity) edm::LogVerbatim("HectorEventProcessing") << "Hector:filterZDC: barcode " << line << " 0         is_stop_zdc=  "<< is_stop_zdc;
        }
      }
...
void Hector::filterD1(TRandom3* rootEngine){
  ...
  if ( !m_beamPart.empty() && lengthd1>0.) {
    ...
    for (it = m_beamPart.begin(); it != m_beamPart.end(); ++it ) {
      line = (*it).first;
      part = (*it).second;
      if(m_verbosity) edm::LogVerbatim("HectorEventProcessing") << "Hector:filterD1: barcode = " << line << " isStoppedZDC =" << (*m_isStoppedzdc.find(line)).second;
      if ( ((*m_isStoppedzdc.find(line)).second) || !((*m_isCharged.find( line )).second) ) {
	
        if(m_verbosity) edm::LogVerbatim("HectorEventProcessing") << "Hector:filterD1: barcode = " << line << " propagated ";
	
        direction = (*m_direct.find( line )).second;
        if(m_verbosity) edm::LogVerbatim("HectorEventProcessing") << "Hector:filterD1:direction=" << direction;
        if (m_smearAng) {
          if ( m_sigmaSTX>0. && m_sigmaSTY>0.) {
            // the beam transverse direction is centered on (TXforPosition, TYforPosition) at IP
            part->smearAng(m_sigmaSTX,m_sigmaSTY,rootEngine);
        ...
        if ( direction == 1 && m_beamlineD11 != 0 ) {
          ...
          if(m_verbosity) edm::LogVerbatim("HectorEventProcessing") << "Hector:filterD1 barcode " << line << " positive is_stop_d1 =  "<< is_stop_d1;
        } else  if ( direction == -1 && m_beamlineD12 != 0 ){
          part->computePath( m_beamlineD12 );
          is_stop_d1 = part->stopped( m_beamlineD12 );
          m_isStoppedd1[line] = is_stop_d1;
          if(m_verbosity) edm::LogVerbatim("HectorEventProcessing") << "Hector:filterD1 barcode " << line << " negative is_stop_d1 =  "<< is_stop_d1;
        } else {
          is_stop_d1 = true;
          m_isStoppedd1[line] = is_stop_d1;
          if(m_verbosity) edm::LogVerbatim("HectorEventProcessing") << "Hector:filterD1 barcode " << line << " 0        is_stop_d1 =  "<< is_stop_d1;
        }
        //propagating
        if (!is_stop_d1 ) {
        ...
        if(m_verbosity) edm::LogVerbatim("HectorEventProcessing") << "Hector:filterD1: barcode = " << line << " isStopped=" << (*m_isStoppedd1.find(line)).second;
      }
...
HepMC::GenEvent * Hector::addPartToHepMC( HepMC::GenEvent * evt ){
  ...
  for (it = m_beamPart.begin(); it != m_beamPart.end(); ++it ) {
    ...
    if(m_verbosity) {
      edm::LogVerbatim("HectorEventProcessing") << "Hector:addPartToHepMC: barcode = " << line << "\n"
    ...
    if (!((*m_isStoppedfp420.find(line)).second) || (!((*m_isStoppedd1.find(line)).second) && ((*m_isStoppedzdc.find(line)).second))){
      ...
      if ( gpart ) {
        ...
        if(ddd != 0.) {
          if(m_verbosity) {
            edm::LogVerbatim("HectorEventProcessing") <<"Hector:: x= "<< (*(m_xAtTrPoint.find(line))).second*0.001<< "\n"
          ...
          if (m_verbosity) edm::LogVerbatim("HectorEventProcessing") << "Hector:addPartToHepMC: LHCTransportLink " << theLink;
          theCorrespondenceMap.push_back(theLink);

          if(m_verbosity) edm::LogVerbatim("HectorEventProcessing") << "Hector::TRANSPORTED pz= " << gpart->momentum().pz()  
      ...
      if ( gpart ) {
        ...
        //        evt->add_vertex( vert );
        if(m_verbosity) edm::LogVerbatim("HectorEventProcessing") << "Hector::NON-transp. pz= " << gpart->momentum().pz()  
                                                               << " eta= "<< gpart->momentum().eta()  
                                                               << " status= "<< gpart->status();
      }
    }
