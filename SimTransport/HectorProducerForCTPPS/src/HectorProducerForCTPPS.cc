...
// Hector headers
#include "SimTransport/HectorProducerForCTPPS/interface/HectorProducerForCTPPS.h"
...
HectorProducerForCTPPS::HectorProducerForCTPPS(edm::ParameterSet const & parameters):eventsAnalysed(0){
    ...
    hector_ctpps = new HectorForCTPPS(parameters, m_verbosity,m_CTPPSTransport);

    edm::Service<edm::RandomNumberGenerator> rng;
    if ( ! rng.isAvailable() ) {
        throw cms::Exception("Configuration")
            << "LHCTransport (HectorProducerForCTPPS) requires the RandomNumberGeneratorService\n"
            "which is not present in the configuration file.  You must add the service\n"
            "in the configuration file or remove the modules that require it.";
    }
}

HectorProducerForCTPPS::~HectorProducerForCTPPS(){}


void HectorProducerForCTPPS::beginRun(const edm::Run & r,const edm::EventSetup& c) {}

void HectorProducerForCTPPS::endRun(const edm::Run & r,const edm::EventSetup& c) {}

void HectorProducerForCTPPS::produce(edm::Event & iEvent, const edm::EventSetup & es){
    ...
    if ( engine->name() != "TRandom3" ) {
        throw cms::Exception("Configuration")
            << "The TRandom3 engine type must be used with HectorProducerForCTPPS, Random Number Generator Service not correctly configured!";
    }
...
DEFINE_FWK_MODULE (HectorProducerForCTPPS);
