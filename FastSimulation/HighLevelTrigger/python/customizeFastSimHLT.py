...
def _as_set(values):
    ...
    if isinstance(values, str):
        return set(value.strip() for value in values.split(",") if value.strip())
    return set(values)


def _option(settings, name, default=None):
    return settings.get(name, default)


def _modify_if_present(process, module_name, modifier, verbose=False):
    ...
    try:
        ...
        _log(verbose, f"FastSim HLT: skipped {module_name} ({exc})")
...
class _CustomizerProfiler(object):
    ...
    def mark(self, label):
        ...
        now = time.monotonic()
        print(f"FastSim HLT profile: {label:<42} step={now - self._previous:8.3f}s total={now - self._start:8.3f}s")
        self._previous = now
...
def _is_cross_object_pf_path(base):
    ...
    if re.fullmatch(r"HLT_DoubleMu3_(?:DCA|DZ)_PFMET\d+_PFMHT\d+", base):
        return True
    if re.fullmatch(rf"HLT_Ele\d+.*_(?:{pf_object})(?:_PFMET\d+)?", base):
        return True
    if re.fullmatch(rf"HLT_(?:IsoMu|Mu)\d+(?:eta[\w]+)?.*_(?:{pf_object})(?:_PFMET\d+)?", base):
        return True
...
def _format_path_list(paths, limit=20):
    ...
    return f"{', '.join(paths[:limit])}, ... ({len(paths)} total)"
...
def _input_tag_from_text(text, fallback="hltVerticesPF", verbose=False):
    parts = [part.strip() for part in str(text).split(":")]
    if parts and parts[0] and len(parts) <= 3:
        return cms.InputTag(*parts)

    _log(verbose, f"FastSim HLT: unsupported InputTag text '{text}'; using {fallback}")
...
def _reroute_muon_iso_track_sources(process, track_source):
    ...
    for module_name, module in process.producers_().items():
        ...
        changed.append(f"{module_name}:{old_source}->{track_source}")
...
def _set_muon_iso_track_selection_quality(process, quality):
    ...
    for module_name, module in process.producers_().items():
        ...
        changed.append(f"{module_name}:{old_quality}->{quality}")
...
def _install_pnet_btag_vertex_filter_for_fastsim(
    ...
    for path_name, path in process.paths_().items():
        ...
        try:
            ...
            _log(verbose, f"FastSim HLT: could not install PNet b-tag vertex chain in {path_name}: {exc}")
...
def _replace_l3_oi_seed_with_fast_tsg(process, verbose=False):
    if not hasattr(process, "hltIterL3OISeedsFromL2Muons"):
        _log(verbose, "FastSim HLT: hltIterL3OISeedsFromL2Muons not present; cannot install FastTSGFromL2Muon")
        return
...
def _replace_l3_oi_track_candidates_with_fastsim(process, verbose=False):
    if not hasattr(process, "hltIterL3OITrackCandidates"):
        _log(verbose, "FastSim HLT: hltIterL3OITrackCandidates not present; cannot install FastSim TrackCandidateProducer")
        return
...
def _install_fast_hlt_pixel_tracks(process, target_paths=(), scheduled_only=False, verbose=False):
    if not hasattr(process, "hltPixelTracksInRegionIter0L3Muon"):
        _log(verbose, "FastSim HLT: hltPixelTracksInRegionIter0L3Muon not present; cannot install FastSim HLT pixel tracks")
        return []
    ...
    if missing:
        _log(verbose, f"FastSim HLT: missing FastSim HLT pixel-track modules: {', '.join(missing)}")
        return []
    ...
    for path_name, path in process.paths_().items():
        ...
        try:
            ...
            _log(verbose, f"FastSim HLT: could not splice FastSim hltPixelTracks sequence into {path_name}: {exc}")

    if changed_paths:
        _log(
            verbose,
            f"FastSim HLT: installed FastSim hltPixelTracks sequence in paths: {_format_path_list(sorted(changed_paths))}",
        )
...
def _replace_egamma_gsf_pixel_seeds_with_fastsim(process, verbose=False):
    from FastSimulation.Tracking.TrajectorySeedProducer_cfi import (
        trajectorySeedProducer as _fastsim_trajectory_seed_producer,
    )
    ...
    for seed_module_name, region_module_name, layer_module_name, hit_pair_module_name in replacements:
        ...
        if missing:
            _log(
                verbose,
                f"FastSim HLT: skipped EGamma GSF seed replacement for {seed_module_name}; missing {', '.join(missing)}",
            )
            continue
        ...
        if not layer_list:
            _log(
                verbose,
                f"FastSim HLT: skipped EGamma GSF seed replacement for {seed_module_name}; {layer_module_name} has no layerList",
            )
            continue
    ...
    if installed_modules:
        _log(
            verbose,
            "FastSim HLT: replaced EGamma GSF pixel seed modules with "
            f"FastSim TrajectorySeedProducer: {_format_path_list(sorted(installed_modules))}",
        )

    return installed_modules


def _replace_egamma_gsf_track_candidates_with_fastsim(process, verbose=False):
    ...
    if installed_modules:
        _log(
            verbose,
            "FastSim HLT: replaced EGamma GSF track-candidate modules with "
            f"FastSim TrackCandidateProducer: {_format_path_list(sorted(installed_modules))}",
        )
...
_GRUN_REVIEW_DEFAULT_OPTIONS = {
    "enable_review_default": True,
    "enable_muon_prototype": True,
    "enable_muon_iso_prototype": True,
    "enable_muon_pixel_tracks_prototype": True,
    "enable_object_prototypes": True,
    "enable_egamma_gsf_prototype": True,
    "enable_egamma_mkfit_iso_prototype": True,
    "enable_pfmet_mkfit_prototype": True,
    "enable_high_pfmet_prototype": True,
    "enable_pfjet_pfht_mkfit_prototype": True,
    "enable_ak8_softdrop_prototype": True,
    "enable_cross_object_pf_mkfit_prototype": True,
    "enable_pnet_vertex_prototype": False,
    "enable_pnet_btag_vertex_prototype": False,
    "enable_jet_met_btag_l1_prototypes": True,
    "enable_egamma_tau_l1_prototypes": True,
    "pnet_vertex_source": "hltVerticesPF:WithBS",
    "pnet_vertex_cut": "!isFake",
    "pnet_vertex_track_source": "generalTracksBeforeMixing",
    "pnet_btag_vertex_source": "offlineSlimmedPrimaryVertices",
    "pnet_btag_vertex_cut": "!isFake && ndof >= 4",
    "pnet_btag_vertex_include_tau_cross_paths": False,
    "object_l1_save_tags": False,
    "relax_l3_muon_id": False,
    "use_fast_l3_muon_seeds": True,
    "use_fast_l3_muon_track_candidates": True,
    "muon_iso_track_source": "",
    "muon_iso_track_quality": "loose",
    "keep_debug_products": False,
    "profile_customizer": False,
    "use_combined_pruner": True,
    "verbose": False,
}


def _customizeFastSimHLT(process, settings=None):
    """Apply Python-configured compatibility fixes for modern HLT menus in FastSim."""
    settings = dict(settings or {})
    verbose = bool(_option(settings, "verbose", False))
    profiler = _CustomizerProfiler(bool(_option(settings, "profile_customizer", False)))
    keep_paths = _as_set(_option(settings, "keep_paths", ()))
    only_paths = _as_set(_option(settings, "only_paths", ()))
    keep_l1_seed_modules = _as_set(_option(settings, "keep_l1_seed_modules", ()))
    keep_l1_seed_families = _as_set(_option(settings, "keep_l1_seed_families", ()))
    enable_review_default = bool(_option(settings, "enable_review_default", False))
    enable_muon_prototype = bool(_option(settings, "enable_muon_prototype", enable_review_default))
    enable_muon_iso_prototype = bool(_option(settings, "enable_muon_iso_prototype", enable_review_default))
    enable_muon_pixel_tracks_prototype = bool(
        _option(settings, "enable_muon_pixel_tracks_prototype", enable_muon_iso_prototype)
    )
    enable_object_prototypes = bool(_option(settings, "enable_object_prototypes", enable_review_default))
    enable_egamma_gsf_prototype = bool(_option(settings, "enable_egamma_gsf_prototype", enable_review_default))
    enable_egamma_mkfit_iso_prototype = bool(
        _option(settings, "enable_egamma_mkfit_iso_prototype", enable_review_default)
    )
    enable_pfmet_mkfit_prototype = bool(_option(settings, "enable_pfmet_mkfit_prototype", enable_review_default))
    enable_high_pfmet_prototype = bool(_option(settings, "enable_high_pfmet_prototype", enable_review_default))
    enable_pfjet_pfht_mkfit_prototype = bool(
        _option(settings, "enable_pfjet_pfht_mkfit_prototype", enable_review_default)
    )
    enable_ak8_softdrop_prototype = bool(_option(settings, "enable_ak8_softdrop_prototype", enable_review_default))
    enable_cross_object_pf_mkfit_prototype = bool(
        _option(settings, "enable_cross_object_pf_mkfit_prototype", enable_review_default)
    )
    enable_pnet_vertex_prototype = bool(_option(settings, "enable_pnet_vertex_prototype", False))
    enable_pnet_btag_vertex_prototype = bool(_option(settings, "enable_pnet_btag_vertex_prototype", False))
    enable_jet_met_btag_l1_prototypes = bool(_option(
        settings,
        "enable_jet_met_btag_l1_prototypes",
    ...
    enable_egamma_tau_l1_prototypes = bool(
        _option(settings, "enable_egamma_tau_l1_prototypes", enable_review_default)
    )
    pnet_vertex_source = str(_option(settings, "pnet_vertex_source", "hltVerticesPF:WithBS")).strip() or "hltVerticesPF:WithBS"
    pnet_vertex_cut = str(_option(settings, "pnet_vertex_cut", "!isFake")).strip() or "!isFake"
    pnet_vertex_track_source = str(_option(settings, "pnet_vertex_track_source", "generalTracksBeforeMixing")).strip()
    pnet_btag_vertex_source = (
        str(_option(settings, "pnet_btag_vertex_source", "offlineSlimmedPrimaryVertices")).strip()
        or "offlineSlimmedPrimaryVertices"
    )
    pnet_btag_vertex_cut = str(_option(settings, "pnet_btag_vertex_cut", "!isFake && ndof >= 4")).strip() or "!isFake && ndof >= 4"
    pnet_btag_vertex_include_tau_cross_paths = bool(
        _option(settings, "pnet_btag_vertex_include_tau_cross_paths", False)
    )
    object_l1_save_tags = bool(_option(settings, "object_l1_save_tags", False))
    relax_l3_muon_id = bool(_option(settings, "relax_l3_muon_id", False))
    use_fast_l3_muon_seeds = bool(_option(settings, "use_fast_l3_muon_seeds", enable_muon_prototype))
    use_fast_l3_muon_track_candidates = bool(
        _option(settings, "use_fast_l3_muon_track_candidates", enable_muon_prototype)
    )
    muon_iso_track_source = str(_option(settings, "muon_iso_track_source", "")).strip()
    muon_iso_track_quality = str(_option(settings, "muon_iso_track_quality", "")).strip()
    if enable_muon_iso_prototype and not muon_iso_track_quality:
        muon_iso_track_quality = "loose"
    keep_debug_products = bool(_option(settings, "keep_debug_products", False))
    use_combined_pruner = bool(_option(settings, "use_combined_pruner", enable_review_default))
    ...
    if enable_muon_prototype:
        ...
        _log(verbose, "FastSim HLT: enabling muon prototype mode")
    if enable_object_prototypes:
        ...
        _log(verbose, "FastSim HLT: enabling curated object L1 seed prototype mode")
    if enable_jet_met_btag_l1_prototypes:
        ...
        _log(verbose, "FastSim HLT: enabling active jet/MET/b-tag L1 seed prototype mode")
    if enable_egamma_tau_l1_prototypes:
        ...
        _log(verbose, "FastSim HLT: enabling active EGamma/tau L1 seed prototype mode")
    ...
    if enable_pfmet_mkfit_prototype:
        ...
        _log(verbose, "FastSim HLT: enabling PF-MET/PFMHT mkFit prototype mode")
    ...
    if enable_high_pfmet_prototype:
        ...
        keep_l1_seed_modules.update(_DEFAULT_OBJECT_L1_SEED_MODULES)
        _log(verbose, "FastSim HLT: enabling high-PFMET prototype mode")
    ...
    if enable_pfjet_pfht_mkfit_prototype:
        ...
        keep_l1_seed_modules.update(_DEFAULT_OBJECT_L1_SEED_MODULES)
        _log(verbose, "FastSim HLT: enabling PFJet/PFHT mkFit prototype mode")
    ...
    if enable_ak8_softdrop_prototype:
        ...
        keep_l1_seed_modules.update(_DEFAULT_OBJECT_L1_SEED_MODULES)
        _log(verbose, "FastSim HLT: enabling AK8 soft-drop prototype mode")
    ...
    if enable_cross_object_pf_mkfit_prototype:
        ...
        _log(verbose, "FastSim HLT: enabling cross-object PF mkFit prototype mode")
    ...
    if enable_egamma_mkfit_iso_prototype:
        ...
        keep_l1_seed_modules.update(_DEFAULT_EGAMMA_TAU_L1_SEED_MODULES)
        _log(verbose, "FastSim HLT: enabling WPTight/TightIso EGamma mkFit-isolation prototype mode")
    ...
    if enable_pnet_vertex_prototype or enable_pnet_btag_vertex_prototype:
        ...
        _log(verbose, "FastSim HLT: enabling curated PNet b-tag/tau prototype paths")
    ...
    if only_paths:
        ...
        _log(verbose, f"FastSim HLT: running only requested HLT menu paths: {_format_path_list(sorted(only_paths))}")
    if keep_paths:
        _log(verbose, f"FastSim HLT: keeping requested paths during pruning: {_format_path_list(sorted(keep_paths))}")
    if enable_egamma_mkfit_iso_prototype:
        _log(
            verbose,
            "FastSim HLT: keeping WPTight/TightIso EGamma mkFit-isolation "
            f"prototype paths: {_format_path_list(egamma_mkfit_iso_prototype_paths)}",
        )
    if keep_l1_seed_modules:
        _log(verbose, f"FastSim HLT: preserving requested L1 seed expressions: {_format_path_list(sorted(keep_l1_seed_modules))}")
    if keep_l1_seed_families:
        _log(
            verbose,
            "FastSim HLT: preserving L1 seed expressions by path family "
            f"{_format_path_list(sorted(keep_l1_seed_families))}: "
            f"{_format_path_list(sorted(family_l1_seed_modules))}",
        )
    if relax_l3_muon_id:
        _log(verbose, "FastSim HLT: relaxing L3 muon trigger ID filters")
    if use_fast_l3_muon_seeds:
        _log(verbose, "FastSim HLT: replacing hltIterL3OISeedsFromL2Muons with FastTSGFromL2Muon")
    if use_fast_l3_muon_track_candidates:
        _log(verbose, "FastSim HLT: replacing hltIterL3OITrackCandidates with FastSim TrackCandidateProducer")
    if enable_muon_iso_prototype:
        _log(verbose, "FastSim HLT: enabling L3 muon track-isolation prototype mode")
    if enable_muon_pixel_tracks_prototype:
        _log(verbose, "FastSim HLT: enabling FastSim HLT pixel-track prototype mode")
    if muon_iso_track_quality:
        _log(verbose, f"FastSim HLT: setting L3 muon isolation track-selection quality to {muon_iso_track_quality}")
    if enable_egamma_gsf_prototype:
        _log(verbose, "FastSim HLT: enabling EGamma GSF-track prototype mode")
    if enable_pfmet_mkfit_prototype:
        _log(verbose, f"FastSim HLT: keeping PF-MET/PFMHT prototype paths: {_format_path_list(pfmet_mkfit_prototype_paths)}")
    if enable_high_pfmet_prototype:
        _log(verbose, f"FastSim HLT: keeping high-PFMET prototype paths: {_format_path_list(high_pfmet_prototype_paths)}")
    if enable_pfjet_pfht_mkfit_prototype:
        _log(verbose, f"FastSim HLT: keeping PFJet/PFHT prototype paths: {_format_path_list(pfjet_pfht_mkfit_prototype_paths)}")
    if enable_ak8_softdrop_prototype:
        _log(verbose, f"FastSim HLT: keeping AK8 soft-drop prototype paths: {_format_path_list(ak8_softdrop_prototype_paths)}")
    if enable_cross_object_pf_mkfit_prototype:
        _log(verbose, f"FastSim HLT: keeping cross-object PF prototype paths: {_format_path_list(cross_object_pf_mkfit_prototype_paths)}")
    if pnet_btag_tau_prototype_paths:
        _log(verbose, f"FastSim HLT: keeping curated PNet b-tag/tau prototype paths: {_format_path_list(pnet_btag_tau_prototype_paths)}")
    if enable_pnet_vertex_prototype:
        _log(
            verbose,
            "FastSim HLT: enabling PNet vertex compatibility prototype mode "
            f"(trackSource={pnet_vertex_track_source or '<unchanged>'}, source={pnet_vertex_source}, cut={pnet_vertex_cut})",
        )
    if enable_pnet_btag_vertex_prototype:
        _log(
            verbose,
            "FastSim HLT: enabling PNet b-tag path-local vertex prototype mode "
            f"(source={pnet_btag_vertex_source}, cut={pnet_btag_vertex_cut}, "
            f"includeTauCrossPaths={pnet_btag_vertex_include_tau_cross_paths})",
        )
    if keep_debug_products:
        _log(verbose, "FastSim HLT: keeping diagnostic HLT products in PoolOutputModules")
    if use_combined_pruner:
        _log(verbose, "FastSim HLT: using combined scheduled-path pruner")
    if enable_review_default:
        _log(verbose, "FastSim HLT: enabling review default path set")
    ...
    _modify_if_present(process, "hltL3TrajSeedOIHit", modify_hltL3TrajSeedOIHit, verbose)
    _modify_if_present(process, "hltL3NoFiltersNoVtxTrajSeedOIHit", modify_hltL3TrajSeedOIHit, verbose)
    _modify_if_present(process, "hltL3TrajSeedIOHit", modify_hltL3TrajSeedIOHit, verbose)
    _modify_if_present(process, "hltL3NoFiltersNoVtxTrajSeedIOHit", modify_hltL3TrajSeedIOHit, verbose)
    profiler.mark("apply L3 muon hit customizations")

    if use_fast_l3_muon_seeds:
        _replace_l3_oi_seed_with_fast_tsg(process, verbose)
        _keep_fast_l3_muon_seed_debug_products(process)
    if use_fast_l3_muon_track_candidates:
        _replace_l3_oi_track_candidates_with_fastsim(process, verbose)
        _keep_fast_l3_muon_seed_debug_products(process)
    if enable_egamma_gsf_prototype:
        egamma_gsf_fastsim_seed_modules = _replace_egamma_gsf_pixel_seeds_with_fastsim(process, verbose)
        egamma_gsf_fastsim_track_candidate_modules = _replace_egamma_gsf_track_candidates_with_fastsim(process, verbose)
    else:
    ...
    if enable_pfmet_mkfit_prototype or enable_high_pfmet_prototype or enable_cross_object_pf_mkfit_prototype:
        ...
        if pfmet_mht_rerouted_modules:
            _log(
                verbose,
                "FastSim HLT: rerouted PF-MHT jet inputs away from FastSim-hostile "
                f"PFJet ID collections: {_format_path_list(pfmet_mht_rerouted_modules)}",
            )
    else:
    ...
    if enable_pfjet_pfht_mkfit_prototype or enable_ak8_softdrop_prototype or enable_cross_object_pf_mkfit_prototype:
        ...
        if pfjet_id_filter_rerouted_modules:
            _log(
                verbose,
                "FastSim HLT: rerouted PFJet ID filter inputs to corrected PF jets: "
                f"{_format_path_list(pfjet_id_filter_rerouted_modules)}",
            )
    else:
    ...
    if muon_iso_track_rerouted_modules:
        _log(verbose, f"FastSim HLT: rerouted muon track-isolation inputs: {_format_path_list(muon_iso_track_rerouted_modules)}")
    ...
    if muon_iso_track_quality_modules:
        _log(
            verbose,
            "FastSim HLT: changed L3 muon isolation track-selection quality: "
            f"{_format_path_list(muon_iso_track_quality_modules)}",
        )
    if enable_pnet_vertex_prototype:
        ...
        if pnet_vertex_relaxed_modules:
            _log(verbose, f"FastSim HLT: relaxed PNet vertex filters for FastSim: {_format_path_list(pnet_vertex_relaxed_modules)}")
    else:
    ...
    if enable_pnet_btag_vertex_prototype:
        ...
        if pnet_btag_vertex_paths:
            _log(
                verbose,
                "FastSim HLT: installed path-local PNet b-tag vertex chain in paths: "
                f"{_format_path_list(pnet_btag_vertex_paths)}",
            )
    else:
        pnet_btag_vertex_paths = []
        pnet_btag_vertex_modules = []
    ...
    if only_paths:
        ...
        if removed_unrequested:
            _log(verbose, f"FastSim HLT: removed unrequested HLT menu paths: {_format_path_list(removed_unrequested)}")
    else:
        removed_unrequested = []
    ...
    if enable_review_default:
        ...
        if removed_review_default_paths:
            _log(
                verbose,
                "FastSim HLT: review default pruned unsupported/experimental paths: "
                f"{_format_path_list(removed_review_default_paths)}",
            )
    else:
    ...
    if removed_paths:
        _log(verbose, f"FastSim HLT: removed unsupported MC paths: {_format_path_list(removed_paths)}")
    _set_audit_vstring(process, "removedMCPaths", removed_paths)
    if removed_mkfit_paths:
        _log(verbose, f"FastSim HLT: removed paths using unsupported mkFit seeds: {_format_path_list(removed_mkfit_paths)}")
    _set_audit_vstring(process, "removedMkFitStringPaths", removed_mkfit_paths)
    if removed_mkfit_module_paths:
        _log(
            verbose,
            "FastSim HLT: removed paths containing unsupported mkFit seed modules: "
            f"{_format_path_list(removed_mkfit_module_paths)}",
        )
    _set_audit_vstring(process, "removedMkFitModulePaths", removed_mkfit_module_paths)
    if removed_any_mkfit_paths:
        _log(
            verbose,
            "FastSim HLT: removed paths containing unsupported MkFit modules: "
            f"{_format_path_list(removed_any_mkfit_paths)}",
        )
    _set_audit_vstring(process, "removedMkFitAnyPaths", removed_any_mkfit_paths)
    if removed_tracking_paths:
        _log(
            verbose,
            "FastSim HLT: removed paths using unsupported tracking sequences: "
            f"{_format_path_list(removed_tracking_paths)}",
        )
...
def customizeFastSimHLT(process):
    """Apply baseline FastSim HLT customizations with Python defaults."""
...
def customizeFastSimHLTGRunReviewDefault(process):
    ...
    return _customizeFastSimHLT(process, _GRUN_REVIEW_DEFAULT_OPTIONS)
