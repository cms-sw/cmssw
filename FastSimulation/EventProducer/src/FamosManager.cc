...
FamosManager::FamosManager(edm::ParameterSet const& p)
  ...
  mySimEvent = new FSimEvent(p.getParameter<edm::ParameterSet>("ParticleFilter"));
...
FamosManager::~FamosManager() {
  if (mySimEvent)
    delete mySimEvent;
  if (myTrajectoryManager)
    delete myTrajectoryManager;
  if (myCalorimetry)
    delete myCalorimetry;
}

void FamosManager::setupGeometryAndField(edm::Run const& run, const edm::EventSetup& es) {
  ...
  if (m_pUseMagneticField) {
    ...
    myTrajectoryManager->initializeRecoGeometry(
        &(*theGeomSearchTracker), &(*theTrackerInteractionGeometry), &(*theMagneticFieldMap));
  } else {
    myTrajectoryManager->initializeRecoGeometry(&(*theGeomSearchTracker), &(*theTrackerInteractionGeometry), nullptr);
    bField000 = 4.0;
  }
...
void FamosManager::reconstruct(const HepMC::GenEvent* evt,
  ...
  if (myCalorimetry)
    myCalorimetry->reconstruct(random);

  edm::LogInfo("FamosManager") << " saved : Event  " << iEvent << " of weight " << mySimEvent->weight() << " with "
                               << mySimEvent->nTracks() << " tracks and " << mySimEvent->nVertices()
                               << " vertices, generated by " << mySimEvent->nGenParts() << " particles " << std::endl;
}
