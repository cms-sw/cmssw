...
castorDir=/castor/cern.ch/user/a/azzi/CMSSW310pre9PR/
 ...
 for ((energy=0; energy<=7; energy++));
  ...
  do
    ...
    echo "JOB "$njobs" : "$job", Energy = "$energy", "$nevt" events"
      name="SinglePionPR_E"$energy"_"${job}
      filename="fevt_"${name}".root"
      logname="log_"${name}".txt"
      echo $name
      
      seed1=$(( ($job+1) + 10985*($energy+1) ))
      sed -e "s/==seed1==/${seed1}/" -e "s/==MINPT==/${ptmin}/" -e "s/==MAXPT==/${ptmax}/" -e "s/==NUMEVT==/${numevt}/" SinglePionFastSimPR_cfg.py > tmp_cfg
