...
testGeneralTracks::produce(edm::Event& iEvent, const edm::EventSetup& iSetup )
{
  ...
  for ( unsigned ievt=0; ievt<2; ++ievt ) {
    ...
    for(;itk0!=itk0_e;++itk0){
        ...
        LayersvsEta[ievt]->Fill(etaGen,itk0->getHitPattern().trackerLayersWithMeasurement(reco::HitPattern::TRACK_HITS), 1.);
        LayersvsP[ievt]->Fill(pGen,itk0->getHitPattern().trackerLayersWithMeasurement(reco::HitPattern::TRACK_HITS), 1.);
    }
