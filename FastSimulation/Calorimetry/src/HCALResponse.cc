...
double HCALResponse::interHD(int mip, double e, int ie, int ieta, int det) {
 ...
 mean =random->poissonShoot((int (PoissonShootNoNegative(pars[0],pars[1]))+(int (PoissonShootNoNegative(pars[2],pars[3])))/4+random->flatShoot()/4) *6)/(0.3755*6);
  }
...
double HCALResponse::PoissonShootNoNegative(double e, double sigma) {
  double out = -1;
    while (out < 0.) {
