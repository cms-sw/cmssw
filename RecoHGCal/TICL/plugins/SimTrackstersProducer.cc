...
private:
  ...
  struct InputHolder {
    ...
    edm::Handle<reco::SimToRecoCollection> TPtoGsfTrackMapHandle;  // Map SimTrack -> GsfTrack
...
void SimTrackstersProducer::produceOne(edm::Event& evt,
  ...
  auto simTrackToGsfTrack = [&](UniqueSimTrackId simTkId) -> std::vector<int> {
    ...
    if (!holder.gsf_tracks_h.isValid() || !holder.TPtoGsfTrackMapHandle.isValid())
      return gsfTrackIdx;
    auto ipos = holder.simTrackToTPMap.mapping.find(simTkId);
    if (ipos != holder.simTrackToTPMap.mapping.end()) {
      auto jpos = holder.TPtoGsfTrackMapHandle->find((ipos->second));
      if (jpos != holder.TPtoGsfTrackMapHandle->end()) {
        auto& associatedGsfTracks = jpos->val;
        if (!associatedGsfTracks.empty()) {
          int gsfIndex = associatedGsfTracks[0].first.key();
          // Validate the index is within bounds
          if (gsfIndex >= 0 && gsfIndex < (int)holder.gsf_tracks_h->size()) {
            gsfTrackIdx.push_back(gsfIndex);
          } else {
            edm::LogWarning("SimTrackstersProducer")
                << "Invalid GSF track index: " << gsfIndex << " (size: " << holder.gsf_tracks_h->size() << ")";
          }
        }
      }
      ...
      if (!tp.decayVertices().empty()) {
        const auto& iTV = tp.decayVertices()[0];
        for (auto iTP = iTV->daughterTracks_begin(); iTP != iTV->daughterTracks_end(); ++iTP) {
          auto kpos = holder.TPtoGsfTrackMapHandle->find((*iTP));
          if (kpos != holder.TPtoGsfTrackMapHandle->end()) {
  ...
  for (unsigned int simTs_i = 0; simTs_i < simTracksters.size(); ++simTs_i) {
    ...
    if (not bestAssociatedGsfTracks.empty()) {
      for (auto const gsfIndex : bestAssociatedGsfTracks)
        simTrackster.addGSFTrackIdx(gsfIndex);
    }
  }
