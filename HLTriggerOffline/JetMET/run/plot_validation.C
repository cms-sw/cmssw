void plot_validation() {
  ...
  if (dojet)
    TString trigname = "SingleJet50";
  if (!dojet)
    TString trigname = "SingleMET35";
  ...
  TString file1 =
      Form("/uscms_data/d1/cammin/CMSSW/TriggerPerformance/CMSSW_2_2_3/src/run/relval%s_%s/%s%s_output.root",
           shortrelease1.Data(),
           setup.Data(),
           sample.Data(),
           ff1.Data());
  ...
  if (dojet)
    hframe->GetXaxis()->SetTitle("Leading GenJet p_{T} (GeV)");
  if (!dojet)
    hframe->GetXaxis()->SetTitle("GenMET (GeV)");
  ...
  if (compare_releases) {
    c->Print(Form("plots/RelVal_%s_%s_%s--%s_%s.eps",
                  sample.Data(),
                  fastfull.Data(),
                  release1.Data(),
                  release2.Data(),
                  trigname.Data()));
    c->Print(Form("plots/RelVal_%s_%s_%s--%s_%s.png",
                  sample.Data(),
                  fastfull.Data(),
                  release1.Data(),
                  release2.Data(),
