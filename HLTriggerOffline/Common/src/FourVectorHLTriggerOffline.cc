...
std::vector< std::pair<int,int> > FourVectorHLTriggerOffline::ParseTriggerType(const std::string& pathname)
{
  ...
  if (!boost::regex_match(pathname, matches, boost::regex("^HLT\\w*")))
    {
      return objecttype;
    }
  ...
  for (std::vector< std::string >::iterator word = pathwordlist.begin(); word != pathwordlist.end(); word++)
    {
      ...
      boost::smatch matches;
      if (boost::regex_match(*word, matches, boost::regex("\\w*(BTag)\\w*")))
	{
	  objecttype.push_back(std::pair<int,int>(trigger::TriggerBJet,1));
	}
      if (boost::regex_match(*word, matches, boost::regex("\\w*(IsoTrack)\\w*")))
	{
	  objecttype.push_back(std::pair<int,int>(trigger::TriggerTrack,1));
	}
      if (boost::regex_match(*word, matches, boost::regex("\\w*(Tau)\\w*")))
	{
	  objecttype.push_back(std::pair<int,int>(trigger::TriggerTau,1));
	}
      if (boost::regex_match(*word, matches, boost::regex("\\w*(Photon)\\w*")))
	{
	  objecttype.push_back(std::pair<int,int>(trigger::TriggerPhoton,1));
	}
      if (boost::regex_match(*word, matches, boost::regex("\\w*(Ele)\\w*")))
	{
	  objecttype.push_back(std::pair<int,int>(trigger::TriggerElectron,1));
	}
      if (boost::regex_match(*word, matches, boost::regex("\\w*(L1\\w*EG)\\w*")))
	{
	  objecttype.push_back(std::pair<int,int>(trigger::TriggerElectron,1));
	}
      if (boost::regex_match(*word, matches, boost::regex("\\w*(Mu)\\w*")))
	{
	  objecttype.push_back(std::pair<int,int>(trigger::TriggerMuon,1));
	}
      if (boost::regex_match(*word, matches, boost::regex("\\w*(Jet)\\w*")))
	{
	  objecttype.push_back(std::pair<int,int>(trigger::TriggerJet,1));
	}
      if (boost::regex_match(*word, matches, boost::regex("\\w*(HT)\\w*")))
	{
	  objecttype.push_back(std::pair<int,int>(trigger::TriggerTET,1));
	}
      if (boost::regex_match(*word, matches, boost::regex("\\w*(SumET)\\w*")))
	{
	  objecttype.push_back(std::pair<int,int>(trigger::TriggerTET,1));
	}
      if (boost::regex_match(*word, matches, boost::regex("\\w*(MET)\\w*")))
	{
	  objecttype.push_back(std::pair<int,int>(trigger::TriggerMET,1));
	}
    }  
