...
void HLTExoticaSubAnalysis::beginRun(const edm::Run & iRun, const edm::EventSetup & iSetup)
{
	...
	{
		edm::LogError("ExoticaValidations") << "HLTExoticaSubAnalysis::beginRun: "
	...
	for(size_t i = 0; i < _hltPathsToCheck.size(); ++i)
	{
		...
		if( ! found )
		{
			edm::LogWarning("ExoticaValidations") << "HLTExoticaSubAnalysis::beginRun, In "
			...
			iPath != _hltPaths.end(); ++iPath) 
	{
		...
		HLTExoticaPlotter analyzer(_pset, shortpath,objsNeedHLT, _dbe);
		_analyzers.push_back(analyzer);
    	}

      	// Call the beginRun (which books all the path dependent histograms)
      	for(std::vector<HLTExoticaPlotter>::iterator it = _analyzers.begin(); 
...
void HLTExoticaSubAnalysis::analyze(const edm::Event & iEvent, const edm::EventSetup & iSetup, EVTColContainer * cols)
{
			...
			it != sourceMatchMap.end(); ++it)
	{
		...
		for(std::vector<HLTExoticaPlotter>::iterator an = _analyzers.begin();
...
void HLTExoticaSubAnalysis::bookHist(const std::string & source, 
