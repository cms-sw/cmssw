...
void GetRates() {
  /* Function: GetRates()
     Produces a table with the rates for the samples being studied.  The table is written to the file indicated by myfile.  
     The variables used here are the ones found to be optimal in the new optimization.
  */
  ...
  Double_t cutEt = 17.; // 19.
  Double_t cutHoE = 0.07; // 0.06
  Double_t cutEpMatch = 0.9; // 0.45
  Double_t cutItrack = 0.15; // 0.45
  /* ********************************************* */
  ...
  Int_t nCandsCut = 1; // Change the number of candidates that are required to pass cuts (should be 2 for double paths)
 
  ofstream myfile;
  myfile.open("RatesTableNewVars.txt"); // Output file name
  ...
  */
  ...
  cutText += ".Itrack < ";
  cutText += cutItrack;
  cutText += ") >= "; // Change to >= 2 for double cuts
  cutText += nCandsCut;
