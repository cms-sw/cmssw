  ...
  */                     
  ...
  for (pathNum = 0; pathNum < paths.size(); pathNum++) {  
    ...
    while (stepSizes[0] > stepThresh) {
      isBetter = false;
      cout<<cutEt<<", "<<cutHoE<<", "<<cutEpMatchBarrel<<". "<<cutEpMatchEndcap<<", "<<cutItrack<<endl;
      for (direction = 0; direction < 243; direction++) {
	rate = 0.;
	...
	cout<<thisCutEt<<", "<<thisCutHoE<<", "<<thisCutEpMatchBarrel<<", "<<thisCutEpMatchEndcap<<", "<<thisCutItrack<<endl;
	for (sampleNum = 0; sampleNum < samples.size(); sampleNum++) {
	  cutText = "Sum$(";
	  cutText += paths[pathNum].name;
	  cutText += ".l1Match && ";
	...
	if ((bestDir / 3) % 3 == 0) cutHoE = cutHoE + stepSizes[1];
	else if ((bestDir / 3) % 3 == 1) cutHoE = cutHoE;
	else cutHoE = cutHoE - stepSizes[1];
	if ((bestDir / 9) % 3 == 0) cutEpMatchBarrel = cutEpMatchBarrel + stepSizes[2];
	else if ((bestDir / 9) % 3 == 1) cutEpMatchBarrel = cutEpMatchBarrel;
	else cutEpMatchBarrel = cutEpMatchBarrel - stepSizes[2];
	if ((bestDir / 27) % 3 == 0) cutEpMatchEndcap = cutEpMatchEndcap + stepSizes[3];
	else if ((bestDir / 27) % 3 == 1) cutEpMatchEndcap = cutEpMatchEndcap;
	else cutEpMatchEndcap = cutEpMatchEndcap - stepSizes[3];
    ...
    myfile<<"HoE = "<<cutHoE<<endl;
    myfile<<"EpMatchBarrel = "<<cutEpMatchBarrel<<endl;
    myfile<<"EpMatchEndcap = "<<cutEpMatchEndcap<<endl;
