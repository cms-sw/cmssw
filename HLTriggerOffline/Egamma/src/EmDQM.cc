...
EmDQM::EmDQM(const edm::ParameterSet &pset_) : pset(pset_) {
  ...
  minEtForEtaEffPlot_ =
      pset.getUntrackedParameter<unsigned int>("minEtForEtaEffPlot", 15);
  ...
  noIsolationPlots_ =
      pset.getUntrackedParameter<bool>("noIsolationPlots", true);
  ...
  // consumes
  genParticles_token =
      consumes<edm::View<reco::Candidate>>(edm::InputTag("genParticles"));
  triggerObject_token = consumes<trigger::TriggerEventWithRefs>(triggerObject_);
  hltResults_token = consumes<edm::TriggerResults>(
      edm::InputTag("TriggerResults", "", triggerObject_.process()));
  if (autoConfMode_) {
    gencutColl_fidWenu_token =
        mayConsume<edm::View<reco::Candidate>>(edm::InputTag("fiducialWenu"));
    gencutColl_fidZee_token =
        mayConsume<edm::View<reco::Candidate>>(edm::InputTag("fiducialZee"));
    gencutColl_fidTripleEle_token = mayConsume<edm::View<reco::Candidate>>(
        edm::InputTag("fiducialTripleEle"));
    gencutColl_fidGammaJet_token = mayConsume<edm::View<reco::Candidate>>(
        edm::InputTag("fiducialGammaJet"));
    gencutColl_fidDiGamma_token = mayConsume<edm::View<reco::Candidate>>(
        edm::InputTag("fiducialDiGamma"));
  } else {
    gencutColl_manualConf_token = consumes<edm::View<reco::Candidate>>(
        pset.getParameter<edm::InputTag>("cutcollection"));
  }
}
...
void EmDQM::dqmBeginRun(edm::Run const &iRun, edm::EventSetup const &iSetup) {
  ...
  if (hltConfig_.init(iRun, iSetup, triggerObject_.process(), changed)) {
    ...
    if (autoConfMode_) {
      ...
      for (unsigned int j = 0; j < egammaPaths.size(); j++) {
        ...
        for (unsigned int i = 0; i < egammaPaths.at(j).size(); i++) {
          ...
          paramSet.addUntrackedParameter("pathIndex",
                                         hltConfig_.triggerIndex(pathName));
          paramSet.addParameter("@module_label",
                                hltConfig_.removeVersion(pathName) + "_DQM");
          ...
          if (ptMax < (1.2 * genEtMin)) {
            paramSet.addUntrackedParameter<double>(
                "PtMax", (10 * ceil(0.12 * genEtMin)));
            paramSet.addUntrackedParameter<double>(
                "PtMin", (10 * ceil(0.12 * genEtMin) - ptMax + ptMin));
          } else {
          ...
          case TYPE_SINGLE_ELE:
            ...
            paramSet.addParameter<edm::InputTag>("cutcollection",
                                                 edm::InputTag("fiducialWenu"));
          ...
          case TYPE_DOUBLE_ELE:
            ...
            paramSet.addParameter<int>("pdgGen", 11);
            paramSet.addParameter<edm::InputTag>("cutcollection",
                                                 edm::InputTag("fiducialZee"));
            paramSet.addParameter<int>("cutnum", 2);
            break;
          case TYPE_TRIPLE_ELE:
            paramSet.addParameter<unsigned>("reqNum", 3);
            paramSet.addParameter<int>("pdgGen", 11);
            paramSet.addParameter<edm::InputTag>(
                "cutcollection", edm::InputTag("fiducialTripleEle"));
          ...
          case TYPE_SINGLE_PHOTON:
            ...
            paramSet.addParameter<edm::InputTag>(
                "cutcollection", edm::InputTag("fiducialGammaJet"));
            paramSet.addParameter<int>("cutnum", 1);
            break;
          case TYPE_DOUBLE_PHOTON:
            paramSet.addParameter<unsigned>("reqNum", 2);
            paramSet.addParameter<int>("pdgGen", 22);
            paramSet.addParameter<edm::InputTag>(
                "cutcollection", edm::InputTag("fiducialDiGamma"));
            paramSet.addParameter<int>("cutnum", 2);
          }
          //--------------------
          ...
          for (std::vector<std::string>::iterator filter =
                   filterModules.begin();
               filter != filterModules.end(); ++filter) {
            std::string moduleType =
                hltConfig_.modulePSet(*filter).getParameter<std::string>(
                    "@module_type");
            moduleLabel =
                hltConfig_.modulePSet(*filter).getParameter<std::string>(
                    "@module_label");
            ...
            if (moduleType == "Pythia6GeneratorFilter" ||
                ...
                moduleType == "HLTPrescaler" || moduleType == "HLTBool")
              continue;
            ...
            if (moduleType == "HLTLevel1GTSeed") {
              ...
              filterPSet =
                  makePSetForL1SeedToSuperClusterMatchFilter(moduleLabel);
              ...
              filterPSet = makePSetForEgammaDoubleEtDeltaPhiFilter(moduleLabel);
                ...
                moduleType == "HLTGlobalSumsMET" ||
                moduleType == "HLTHtMhtFilter" ||
                moduleType == "HLTMhtFilter" || moduleType == "HLTJetTag" ||
                moduleType == "HLT1CaloJet" || moduleType == "HLT1CaloMET" ||
                moduleType == "HLT1CaloBJet" || moduleType == "HLT1Tau" ||
                moduleType == "HLT1PFTau" || moduleType == "PFTauSelector" ||
                ...
                moduleType ==
                    "HLTEgammaTriggerFilterObjectWrapper" // 'fake' filter
                || moduleType ==
                       "HLTEgammaDoubleLegCombFilter" // filter does not put
                                                      // anything in
                ...
                moduleType == "HLT1Photon" || moduleType == "HLTRFilter" ||
            ...
            if (!filterPSet.empty()) {
              if (!hltConfig_.modulePSet(moduleLabel).exists("saveTags")) {
                ...
                if (moduleLabel.find("Unseeded") != std::string::npos &&
                  ...
                  filterVPSet.back().addParameter<int>("theHLTOutputTypes",
              ...
              if (filterPSet.getParameter<int>("ncandcut") < 0) {
                edm::LogPrint("EmDQM")
                    ...
                    << " found. Set to " << paramSet.getParameter<int>("cutnum")
                    << ", determined from path name.";
                filterPSet.addParameter<int>(
                    "ncandcut", paramSet.getParameter<int>("cutnum"));
              } else if (filterPSet.getParameter<int>("ncandcut") >
                         paramSet.getParameter<int>("cutnum")) {
                edm::LogInfo("EmDQM")
                    << "Changed required number of candidates from "
                    << paramSet.getParameter<int>("cutnum") << " to "
                    << filterPSet.getParameter<int>("ncandcut")
                    << " for filter " << moduleLabel;
                paramSet.addParameter<int>(
                    "cutnum", filterPSet.getParameter<int>("ncandcut"));
                paramSet.addParameter<unsigned>(
                    "reqNum",
                    (unsigned)filterPSet.getParameter<int>("ncandcut"));
              }
          ...
          if (!filterPSet.empty()) {
            std::string lastModuleName =
                filterPSet.getParameter<edm::InputTag>("HLTCollectionLabels")
                    .label();
            if (!hltConfig_.modulePSet(lastModuleName).exists("saveTags")) {
              ...
              if ((j == TYPE_SINGLE_PHOTON || j == TYPE_DOUBLE_PHOTON) &&
                  pathName.rfind("Ele") == std::string::npos) {
                filterVPSet.back().addParameter<int>("theHLTOutputTypes",
                                                     trigger::TriggerPhoton);
              }
            }
            paramSet.addParameter<std::vector<edm::ParameterSet>>("filters",
                                                                  filterVPSet);
          } else {
            if (verbosity_ >= OUTPUT_ALL)
              edm::LogPrint("EmDQM")
    ...
    ////////////////////////////////////////////////////////////
    for (std::vector<edm::ParameterSet>::iterator psetIt = paramSets.begin();
         psetIt != paramSets.end(); ++psetIt) {
...
void EmDQM::bookHistograms(DQMStore::IBooker &iBooker, edm::Run const &iRun,
  ...
  for (std::vector<edm::ParameterSet>::iterator psetIt = paramSets.begin();
    ...
    if (!mcMatchedOnly_) {
      ...
      total = iBooker.book1D(histName.c_str(), histTitle.c_str(),
    ...
    totalmatch = iBooker.book1D(histName.c_str(), histTitle.c_str(),
                                numOfHLTCollectionLabels + 2, 0,
                                numOfHLTCollectionLabels + 2);
    ...
    etgen = iBooker.book1D(histName.c_str(), histTitle.c_str(), nbins_,
                           plotPtMin, plotPtMax);
    histName = "gen_eta";
    histTitle = "#eta of " + pdgIdString + "s ";
    etagen = iBooker.book1D(histName.c_str(), histTitle.c_str(), nbins_,
                            -etaMax_, etaMax_);
    histName = "gen_phi";
    histTitle = "#phi of " + pdgIdString + "s ";
    if (!noPhiPlots_)
      phigen = iBooker.book1D(histName.c_str(), histTitle.c_str(), nbins_,
                              -phiMax_, phiMax_);
    histName = "gen_etaphi";
    histTitle = "#eta-#phi of " + pdgIdString + "s ";
    etaphigen =
        iBooker.book2D(histName.c_str(), histTitle.c_str(), nbins2D_ - 2,
                       -eta2DMax_, eta2DMax_, nbins2D_, -phi2DMax_, phi2DMax_);
    ////////////////////////////////////////////////////////////
    ...
    for (unsigned int i = 0; i < numOfHLTCollectionLabels; i++) {
      if (!mcMatchedOnly_) {
        ...
        tmphisto = iBooker.book1D(histName.c_str(), histTitle.c_str(), nbins_,
                                  plotPtMin, plotPtMax);
        ...
        tmphisto = iBooker.book2D(histName.c_str(), histTitle.c_str(),
                                  nbins2D_ - 2, -eta2DMax_, eta2DMax_, nbins2D_,
                                  -phi2DMax_, phi2DMax_);
...
bool EmDQM::checkGeneratedParticlesRequirement(const edm::Event &event) {
  ...
  for (edm::View<reco::Candidate>::const_iterator currentGenParticle =
           genParticles->begin();
       currentGenParticle != genParticles->end(); currentGenParticle++) {
    ...
    if (!(abs((*currentGenParticle).pdgId()) == pdgGen &&
          (*currentGenParticle).status() == 1 &&
  ...
  std::sort(allSortedGenParticles.begin(), allSortedGenParticles.end(),
            pTGenComparator_);
  ...
  for (unsigned int i = 0; i < gencut_; i++) {
    bool inECALgap = fabs(allSortedGenParticles[i].eta()) > 1.4442 &&
                     fabs(allSortedGenParticles[i].eta()) < 1.556;
...
bool EmDQM::checkRecoParticlesRequirement(const edm::Event &event) {
  ...
  for (edm::View<reco::Candidate>::const_iterator currentReferenceParticle =
           referenceParticles->begin();
...
void EmDQM::analyze(const edm::Event &event, const edm::EventSetup &setup) {
  ...
  unsigned int vPos = 0;
  for (std::vector<edm::ParameterSet>::iterator psetIt = paramSets.begin();
       psetIt != paramSets.end(); ++psetIt, ++vPos) {
    SetVarsFromPSet(psetIt);
    ...
    for (edm::View<reco::Candidate>::const_iterator genpart =
             cutCounter->begin();
         genpart != cutCounter->end(); genpart++) {
      reco::Particle tmpcand(genpart->charge(), genpart->p4(),
                             genpart->vertex(), genpart->pdgId(),
                             genpart->status());
      if (tmpcand.et() >= plotEtMin) {
    ...
    for (unsigned int i = 0; i < gencut_; i++) {
      etgens.at(vPos)->Fill(
          sortedGen[i].et()); // validity has been implicitily checked by the
    ...
    if (gencut_ >= reqNum)
      totalmatchs.at(vPos)->Fill(
          numOfHLTCollectionLabels +
          1.5); // this isn't really needed anymore keep for backward comp.

    bool accepted =
        true; // flags that the event has been accepted by all filters before
    ...
    for (unsigned int n = 0; n < numOfHLTCollectionLabels; n++) {
      ...
      sortedGenForFilter.erase(sortedGenForFilter.begin() + nCandCuts.at(n),
                               sortedGenForFilter.end());

      // Fill only if this filter was run.
      if (pathIndex != 0 &&
          hltConfig_.moduleIndex(pathIndex, theHLTCollectionLabels[n].label()) >
              hltResults->index(pathIndex))
        break;
      ...
      case trigger::TriggerL1NoIsoEG: // Non-isolated Level 1
        histoFillerL1NonIso->fillHistos(triggerObj, event, vPos, n,
                                        sortedGenForFilter, accepted);
        break;
      case trigger::TriggerL1IsoEG: // Isolated Level 1
        histoFillerL1Iso->fillHistos(triggerObj, event, vPos, n,
                                     sortedGenForFilter, accepted);
        break;
      case trigger::TriggerPhoton: // Photon
        histoFillerPho->fillHistos(triggerObj, event, vPos, n,
                                   sortedGenForFilter, accepted);
        break;
      case trigger::TriggerElectron: // Electron
        histoFillerEle->fillHistos(triggerObj, event, vPos, n,
                                   sortedGenForFilter, accepted);
        break;
      case trigger::TriggerCluster: // TriggerCluster
        histoFillerClu->fillHistos(triggerObj, event, vPos, n,
                                   sortedGenForFilter, accepted);
        break;
      default:
...
////////////////////////////////////////////////////////////////////////////////
template <class T>
void HistoFiller<T>::fillHistos(
    edm::Handle<trigger::TriggerEventWithRefs> &triggerObj,
    const edm::Event &iEvent, unsigned int vPos, unsigned int n,
    std::vector<reco::Particle> &sortedGen, bool &accepted) {
  ...
  if ((triggerObj->filterIndex(dqm->theHLTCollectionLabels[n]) >=
    ...
    dqm->hltCollectionLabelsMissed.insert(
  ...
  if (!dqm->mcMatchedOnly_) {
    ...
    for (unsigned int i = 0; i < dqm->nCandCuts.at(n); i++) {
      ...
      if (closestEcalCandIndex >= 0) {
        dqm->histEtOfHltObjMatchToGens.at(vPos).at(n)->Fill(
            recoecalcands[closestEcalCandIndex]->et());
        dqm->histEtaOfHltObjMatchToGens.at(vPos).at(n)->Fill(
            recoecalcands[closestEcalCandIndex]->eta());
        if (!dqm->noPhiPlots_)
          dqm->histPhiOfHltObjMatchToGens.at(vPos).at(n)->Fill(
              recoecalcands[closestEcalCandIndex]->phi());
        dqm->histEtaPhiOfHltObjMatchToGens.at(vPos).at(n)->Fill(
            recoecalcands[closestEcalCandIndex]->eta(),
            recoecalcands[closestEcalCandIndex]->phi());
    ...
    for (unsigned int i = 0; i < recoecalcands.size(); i++) {
      ...
      dqm->etaphihists.at(vPos).at(n)->Fill(recoecalcands[i]->eta(),
                                            recoecalcands[i]->phi());
    }
  }
  ...
  for (unsigned int i = 0; i < dqm->nCandCuts.at(n); ++i) {
    ...
    for (unsigned int trigOb = 0; trigOb < recoecalcands.size(); ++trigOb) {
      ...
      if (closestDr >
          mindist) { // it's not really a "match" if it's that far away
        // closest = -1;
      } else {
        matchedMcParts++;
        matchThis = true;
    ...
    if (sortedGen[i].et() > dqm->minEtForEtaEffPlot_) {
      ...
      dqm->etaphihistmatchs.at(vPos).at(n)->Fill(sortedGen[i].eta(),
                                                 sortedGen[i].phi());
    }
  }
  ...
  for (unsigned int i = 0; i < hltConfig_.size(); i++) {
    ...
    if (int(path.find("HLT_")) == 0) { // Path should start with 'HLT_'
      ...
      int doublePhotonCount = countSubstring(path, "DoublePhoton") +
                              countSubstring(path, "Diphoton");

      int totEleCount = 2 * tripleEleCount + doubleEleCount + eleCount +
                        scCount + 2 * doubleSCCount;
...
std::vector<std::string> EmDQM::getFilterModules(const std::string &path) {
  ...
  for (unsigned int i = 0; i < hltConfig_.size(path); i++) {
    ...
    if (moduleEDMType == "EDFilter" ||
        moduleType.find("Filter") !=
            std::string::npos) { // older samples may not have EDMType data
                                 // included
...
EmDQM::makePSetForL1SeedFilter(const std::string &moduleName) {
  ...
  retPSet.addParameter<std::vector<double>>("PlotBounds",
                                            std::vector<double>(2, 0.0));
  // retPSet.addParameter<edm::InputTag>("HLTCollectionLabels",
  // edm::InputTag(moduleName, "", processName_));
  retPSet.addParameter<edm::InputTag>(
      "HLTCollectionLabels",
      edm::InputTag(moduleName, "", triggerObject_.process()));
  retPSet.addParameter<std::vector<edm::InputTag>>(
      "IsoCollections", std::vector<edm::InputTag>(1, std::string("none")));
  ...
  int orCount =
      countSubstring(moduleName, "OR") + countSubstring(moduleName, "Or");
  ...
  if (orCount > 0 && candCount > 0) {
    if (egCount % (orCount + 1) == 0 && dEgCount % (orCount + 1) == 0 &&
        tEgCount % (orCount + 1) == 0)
      candCount /= (orCount + 1);
...
edm::ParameterSet EmDQM::makePSetForL1SeedToSuperClusterMatchFilter(
  ...
  retPSet.addParameter<int>(
      "ncandcut",
      hltConfig_.modulePSet(moduleName).getParameter<int>("ncandcut"));
...
EmDQM::makePSetForEgammaGenericFilter(const std::string &moduleName) {
  ...
  edm::InputTag varTag =
      hltConfig_.modulePSet(moduleName).getParameter<edm::InputTag>("varTag");
  ...
  if (inputType == "EgammaHLTR9Producer" ||               // R9 shape
      ...
      inputType ==
          "EgammaHLTHcalIsolationProducersRegional" || // HCAL isolation and HE
      inputType ==
          "EgammaHLTGsfTrackVarProducer" || // GSF track deta and dphi filter
      inputType ==
          "EgammaHLTBcHcalIsolationProducersRegional" || // HCAL isolation and
                                                         // HE
      inputType ==
          "EgammaHLTEcalPFClusterIsolationProducer" || // ECAL PF isolation
                                                       // filter
      inputType ==
          "EgammaHLTHcalPFClusterIsolationProducer" || // HCAL PF isolation
                                                       // filter
      inputType ==
          "EgammaHLTElectronTrackIsolationProducers" // Track isolation filter
  ) {
    retPSet.addParameter<std::vector<double>>("PlotBounds",
                                              std::vector<double>(2, 0.0));
    // retPSet.addParameter<edm::InputTag>("HLTCollectionLabels",
  ...
  if (verbosity_ >= OUTPUT_ERRORS)
    edm::LogError("EmDQM")
        ...
        << "' should do: uses a collection produced by a module of C++ type '"
        << inputType << "'.";
...
EmDQM::makePSetForEgammaGenericQuadraticFilter(const std::string &moduleName) {
  ...
  if (inputType == "EgammaHLTR9Producer" ||               // R9 shape
      ...
      inputType ==
          "EgammaHLTHcalPFClusterIsolationProducer" || // HCAL PF isolation
                                                       // filter
      inputType ==
          "EgammaHLTPhotonTrackIsolationProducersRegional" // Photon track
                                                           // isolation
  ) {
    retPSet.addParameter<std::vector<double>>("PlotBounds",
                                              std::vector<double>(2, 0.0));
  ...
  if (verbosity_ >= OUTPUT_ERRORS)
    edm::LogError("EmDQM")
        << "Can't determine what the HLTEgammaGenericQuadraticFilter '"
        << moduleName
        << "' should do: uses a collection produced by a module of C++ type '"
        << inputType << "'.";
  return edm::ParameterSet();
...
EmDQM::makePSetForElectronGenericFilter(const std::string &moduleName) {
  ...
  if (inputType ==
          "EgammaHLTElectronDetaDphiProducer" || // deta and dphi filter
...
void EmDQM::SetVarsFromPSet(std::vector<edm::ParameterSet>::iterator psetIt) {
  dirname_ = "HLT/HLTEgammaValidation/" +
             psetIt->getParameter<std::string>("@module_label");
