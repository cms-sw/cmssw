...
EmDQMReco::EmDQMReco(const edm::ParameterSet& pset)
{
  ...
  for(auto & filter : filters)
  {
    ...
    theHLTCollectionHumanNames.push_back(filter.getUntrackedParameter<std::string>("HLTCollectionHumanName",theHLTCollectionLabels[i].label()));
	...
	histoFillerL1NonIso->isoNameTokens_.push_back(consumes<edm::AssociationMap<edm::OneToValue<l1extra::L1EmParticleCollection , float>>>(i));
	break;
      case trigger::TriggerL1IsoEG: // Isolated Level 1
	histoFillerL1Iso->isoNameTokens_.push_back(consumes<edm::AssociationMap<edm::OneToValue<l1extra::L1EmParticleCollection , float>>>(i));
	break;
      case trigger::TriggerPhoton: // Photon 
	histoFillerPho->isoNameTokens_.push_back(consumes<edm::AssociationMap<edm::OneToValue<reco::RecoEcalCandidateCollection , float>>>(i));
	break;
      case trigger::TriggerElectron: // Electron 
	histoFillerEle->isoNameTokens_.push_back(consumes<edm::AssociationMap<edm::OneToValue<reco::ElectronCollection , float>>>(i));
	break;
      case trigger::TriggerCluster: // TriggerCluster
	histoFillerClu->isoNameTokens_.push_back(consumes<edm::AssociationMap<edm::OneToValue<reco::RecoEcalCandidateCollection , float>>>(i));
	break;
      default: 
