...
void L25TauAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup){
  ...
  if(thePFTaus.isValid()){
	...
	pfTauMuonDisc = disc[thisTauRef];
      ...
      const PFCandidatePtr& thePFTauLeadTrack = thePFTaus->at(pfTauIt).leadPFChargedHadrCand(); 
      ...
      const vector<PFCandidatePtr>& theSignalCands = thePFTaus->at(pfTauIt).signalPFChargedHadrCands();
      for(vector<PFCandidatePtr>::const_iterator vIt = theSignalCands.begin(); vIt != theSignalCands.end(); ++vIt){
	...
	if(pftauSignalTrkDeltaR > _signalCone) pfTauInBounds = false;
      ...
      const vector<PFCandidatePtr>& theIsoCands = thePFTaus->at(pfTauIt).isolationPFChargedHadrCands();
      pfTauNTrkIso = theIsoCands.size();
      float PFTauLeadIsoPt = 0.;
      Track thePFTauLeadIsoTrk;      
      for(vector<PFCandidatePtr>::const_iterator vIt = theIsoCands.begin(); vIt != theIsoCands.end(); ++vIt){
...
void L25TauAnalyzer::printInfo(const reco::PFTau& thePFTau, const reco::IsolatedTauTagInfo& theTauTagInfo){
  ...
  for(vector<PFCandidatePtr>::const_iterator isoIt = thePFTau.isolationPFChargedHadrCands().begin(); 
