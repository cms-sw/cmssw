...
// 
/**\class HLTTauL25Validation HLTTauL25Validation.cc RecoTauTag/ConeIsolation/test/HLTTauL25Validation.cc
...
HLTTauL25Validation::HLTTauL25Validation(const edm::ParameterSet& iConfig)
{
  ...
  genJetSrc_ = iConfig.getParameter<InputTag>("GenLevelProd");
...
void HLTTauL25Validation::beginJob()
{ 
...
void HLTTauL25Validation::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  using namespace edm;
  using namespace reco;
  numEvents_++;
  ...
  if(dataType_ == "TAU"){
    TauJetsMC=getVectorOfVisibleTauJets(iEvent);
...
void HLTTauL25Validation::endJob(){
...
std::vector<TLorentzVector> HLTTauL25Validation::getVectorOfVisibleTauJets(const edm::Event& iEvent)
{
  typedef math::XYZTLorentzVectorD   LV;
  typedef std::vector<LV>            LVColl;

  edm::Handle<LVColl> McInfo;
  iEvent.getByLabel(genJetSrc_,McInfo);
  ...
  //  if(McInfo.isValid()) {
    ...
    for(size_t i=0;i<tmpvec.size();i++){
      ...
      energyTauMC_->Fill(tmpvec[i].E());
      nMCTaus_ptTauJet_->Fill(tmpvec[i].Perp());  // Fill the histogram with the Pt, Eta, Energy of the Tau Jet at Generator level
      nMCTaus_etaTauJet_->Fill(tmpvec[i].Eta()); 
      nMCTaus_phiTauJet_->Fill(tmpvec[i].Phi()*180./TMath::Pi());
      nMCTaus_energyTauJet_->Fill(tmpvec[i].E());
    }
    //  }
  cout <<"TMP VEC SIZE "<<tmpvec.size();
     return tmpvec;
}     
...
std::vector<TLorentzVector> HLTTauL25Validation::getVectorOfGenJets(Handle< GenJetCollection >& genJets ) {
int jjj=0;
