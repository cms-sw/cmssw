...
//
/**\class Run3ScoutingVertexToRecoVertexProducer Run3ScoutingVertexToRecoVertexProducer.cc HLTriggerOffline/Scouting/plugins/Run3ScoutingVertexToRecoVertexProducer.cc

 Description: Converts Run3ScoutingVertex to reco::Vertex

 Implementation:
     Uses the existing dqm::makeRecoVertex helper function
*/
//
...
void Run3ScoutingVertexToRecoVertexProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (const auto& sVtx : *scoutingVerticesH) {
    ...
    reco::Vertex recoVtx = dqm::makeRecoVertex(sVtx);
