...
PatElectronTagProbeAnalyzer::~PatElectronTagProbeAnalyzer() = default;

void PatElectronTagProbeAnalyzer::dqmAnalyze(
  ...
  for (size_t i = 0; i < patEls->size(); ++i) {
    ...
    ROOT::Math::PtEtaPhiMVector tag_pat_el(pat_el->pt(), pat_el->eta(),
                                           pat_el->phi(), pat_el->mass());
    for (size_t j = 0; j < patEls->size(); ++j) {
      ...
      ROOT::Math::PtEtaPhiMVector probe_pat_el(
          pat_el_second->pt(), pat_el_second->eta(), pat_el_second->phi(),
          pat_el_second->mass());
      float invMass = (tag_pat_el + probe_pat_el).mass();
      if ((TandP_Z_minMass < invMass) && (invMass < TandP_Z_maxMass)) {
        fillHistograms_resonance(histos.resonanceZ_patElectron, *pat_el_second,
                                 invMass);
        fillHistograms_resonance(histos.resonanceAll_patElectron,
                                 *pat_el_second, invMass);
      }
      if ((TandP_jpsi_minMass < invMass) && (invMass < TandP_jpsi_maxMass)) {
        fillHistograms_resonance(histos.resonanceJ_patElectron, *pat_el_second,
      ...
      if ((TandP_ups_minMass < invMass) && (invMass < TandP_ups_maxMass)) {
        fillHistograms_resonance(
            histos.resonanceY_patElectron, *pat_el_second,
      ...
      if (patElectron_passHLT(*pat_el_second, "hltSingleEG30CaloIdLHEFilter",
                              legObjects[1])) {
        if ((TandP_Z_minMass < invMass) && (invMass < TandP_Z_maxMass)) {
          fillHistograms_resonance(
      ...
      if (fire_doubleEG_DST) {
        if ((TandP_Z_minMass < invMass) && (invMass < TandP_Z_maxMass)) {
          fillHistograms_resonance(
              histos.resonanceZ_patElectron_passDoubleEG_DST, *pat_el_second,
              invMass);
          fillHistograms_resonance(
              histos.resonanceAll_patElectron_passDoubleEG_DST, *pat_el_second,
              invMass);
        }
        if ((TandP_jpsi_minMass < invMass) && (invMass < TandP_jpsi_maxMass)) {
          fillHistograms_resonance(
              histos.resonanceJ_patElectron_passDoubleEG_DST, *pat_el_second,
              invMass);  // J/Psi mass: 3.3 +/- 0.2 GeV
          fillHistograms_resonance(
              histos.resonanceAll_patElectron_passDoubleEG_DST, *pat_el_second,
              invMass);
        ...
        if ((TandP_ups_minMass < invMass) && (invMass < TandP_ups_maxMass)) {
          fillHistograms_resonance(
              histos.resonanceY_patElectron_passDoubleEG_DST, *pat_el_second,
              invMass);  // Y mass: 9.8 +/- 0.4 GeV & 10.6 +/- 1 GeV
          fillHistograms_resonance(
              histos.resonanceAll_patElectron_passDoubleEG_DST, *pat_el_second,
              invMass);
      ...
      if (patElectron_passHLT(*pat_el_second,
    ...
    for (const auto& sct_el_second : *sctEls) {
      ...
      ROOT::Math::PtEtaPhiMVector probe_sct_el(
          computePtFromEnergyMassEta(sctEl0.energy(), 0.0005,
                                     sct_el_second.trketaMode()[gsfTrackIndex]),
          sct_el_second.trketaMode()[gsfTrackIndex],
          sct_el_second.trkphiMode()[gsfTrackIndex], 0.0005);
      ...
      if ((TandP_Z_minMass < invMass) && (invMass < TandP_Z_maxMass)) {
        fillHistograms_resonance_sct(histos.resonanceZ_sctElectron,
                                     sct_el_second, invMass);
        fillHistograms_resonance_sct(histos.resonanceAll_sctElectron,
                                     sct_el_second, invMass);
      }
      if ((TandP_jpsi_minMass < invMass) && (invMass < TandP_jpsi_maxMass)) {
      ...
      if ((TandP_ups_minMass < invMass) && (invMass < TandP_ups_maxMass)) {
        fillHistograms_resonance_sct(
            histos.resonanceY_sctElectron, sct_el_second,
            invMass);  // Y mass: 9.8 +/- 0.4 GeV & 10.6 +/- 1 GeV
        fillHistograms_resonance_sct(histos.resonanceAll_sctElectron,
                                     sct_el_second, invMass);
      }
      ...
      if (fire_singlePhoton_DST) {
        if ((TandP_Z_minMass < invMass) && (invMass < TandP_Z_maxMass)) {
          fillHistograms_resonance_sct(
              histos.resonanceZ_sctElectron_passSinglePhoton_DST, sct_el_second,
              invMass);
          fillHistograms_resonance_sct(
        ...
        if ((TandP_jpsi_minMass < invMass) && (invMass < TandP_jpsi_maxMass)) {
          fillHistograms_resonance_sct(
              histos.resonanceJ_sctElectron_passSinglePhoton_DST, sct_el_second,
              invMass);  // J/Psi mass: 3.3 +/- 0.2 GeV
          fillHistograms_resonance_sct(
              histos.resonanceAll_sctElectron_passSinglePhoton_DST,
              sct_el_second, invMass);
      ...
      if (fire_doubleEG_DST) {
        if ((TandP_Z_minMass < invMass) && (invMass < TandP_Z_maxMass)) {
          fillHistograms_resonance_sct(
              histos.resonanceZ_sctElectron_passDoubleEG_DST, sct_el_second,
              invMass);
          fillHistograms_resonance_sct(
              histos.resonanceAll_sctElectron_passDoubleEG_DST, sct_el_second,
              invMass);
        }
        if ((TandP_jpsi_minMass < invMass) && (invMass < TandP_jpsi_maxMass)) {
          fillHistograms_resonance_sct(
              histos.resonanceJ_sctElectron_passDoubleEG_DST, sct_el_second,
              invMass);  // J/Psi mass: 3.3 +/- 0.2 GeV
          fillHistograms_resonance_sct(
              histos.resonanceAll_sctElectron_passDoubleEG_DST, sct_el_second,
              invMass);
        ...
        if ((TandP_ups_minMass < invMass) && (invMass < TandP_ups_maxMass)) {
          fillHistograms_resonance_sct(
              histos.resonanceY_sctElectron_passDoubleEG_DST, sct_el_second,
              invMass);  // Y mass: 9.8 +/- 0.4 GeV & 10.6 +/- 1 GeV
          fillHistograms_resonance_sct(
              histos.resonanceAll_sctElectron_passDoubleEG_DST, sct_el_second,
              invMass);
...
bool PatElectronTagProbeAnalyzer::scoutingElectronID(
  ...
  if (isEB) {
    ...
    if (std::abs(el.dEtaIn()) > 0.008) return false;
    if (std::abs(el.dPhiIn()) > 0.06) return false;
    ...
    if (el.hOverE() > 0.2) return false;
    if (std::abs(el.dEtaIn()) > 0.012) return false;
    if (std::abs(el.dPhiIn()) > 0.06) return false;
...
void PatElectronTagProbeAnalyzer::fillHistograms_resonance(
    const kProbeKinematicHistos histos, const pat::Electron el,
  ...
  if (el.isEB()) {
    ...
    histos.hMissingHits_Barrel->Fill(
        el.gsfTrack()->hitPattern().numberOfLostHits(
...
void PatElectronTagProbeAnalyzer::fillHistograms_resonance_sct(
    const kProbeKinematicHistos histos, const Run3ScoutingElectron el,
    const float inv_mass) const {
  histos.hEta->Fill(el.eta());
  histos.hPhi->Fill(el.phi());
  histos.hInvMass->Fill(inv_mass);
...
void PatElectronTagProbeAnalyzer::bookHistograms(
    DQMStore::IBooker& ibook, edm::Run const& run,
    edm::EventSetup const& iSetup, kTagProbeHistos& histos) const {
  ibook.setCurrentFolder(outputInternalPath_);

  bookHistograms_resonance(ibook, run, iSetup, histos.resonanceZ_patElectron,
                           "resonanceZ_Tag_pat_Probe_patElectron");
  bookHistograms_resonance(ibook, run, iSetup, histos.resonanceJ_patElectron,
                           "resonanceJ_Tag_pat_Probe_patElectron");
  bookHistograms_resonance(ibook, run, iSetup, histos.resonanceY_patElectron,
                           "resonanceY_Tag_pat_Probe_patElectron");
  bookHistograms_resonance(ibook, run, iSetup, histos.resonanceAll_patElectron,
                           "resonanceAll_Tag_pat_Probe_patElectron");

  bookHistograms_resonance(ibook, run, iSetup, histos.resonanceZ_sctElectron,
                           "resonanceZ_Tag_pat_Probe_sctElectron");
  bookHistograms_resonance(ibook, run, iSetup, histos.resonanceJ_sctElectron,
                           "resonanceJ_Tag_pat_Probe_sctElectron");
  bookHistograms_resonance(ibook, run, iSetup, histos.resonanceY_sctElectron,
                           "resonanceY_Tag_pat_Probe_sctElectron");
  bookHistograms_resonance(ibook, run, iSetup, histos.resonanceAll_sctElectron,
                           "resonanceAll_Tag_pat_Probe_sctElectron");

  bookHistograms_resonance(
      ibook, run, iSetup, histos.resonanceZ_patElectron_passSinglePhoton_DST,
      "resonanceZ_Tag_pat_Probe_patElectron_passSinglePhoton_DST");
  bookHistograms_resonance(
      ibook, run, iSetup, histos.resonanceJ_patElectron_passSinglePhoton_DST,
      "resonanceJ_Tag_pat_Probe_patElectron_passSinglePhoton_DST");
  bookHistograms_resonance(
      ibook, run, iSetup, histos.resonanceY_patElectron_passSinglePhoton_DST,
      "resonanceY_Tag_pat_Probe_patElectron_passSinglePhoton_DST");
  bookHistograms_resonance(
      ibook, run, iSetup, histos.resonanceAll_patElectron_passSinglePhoton_DST,
      "resonanceAll_Tag_pat_Probe_patElectron_passSinglePhoton_DST");

  bookHistograms_resonance(
      ibook, run, iSetup, histos.resonanceZ_sctElectron_passSinglePhoton_DST,
      "resonanceZ_Tag_pat_Probe_sctElectron_passSinglePhoton_DST");
  bookHistograms_resonance(
      ibook, run, iSetup, histos.resonanceJ_sctElectron_passSinglePhoton_DST,
      "resonanceJ_Tag_pat_Probe_sctElectron_passSinglePhoton_DST");
  bookHistograms_resonance(
      ibook, run, iSetup, histos.resonanceY_sctElectron_passSinglePhoton_DST,
      "resonanceY_Tag_pat_Probe_sctElectron_passSinglePhoton_DST");
  bookHistograms_resonance(
      ibook, run, iSetup, histos.resonanceAll_sctElectron_passSinglePhoton_DST,
  ...
  bookHistograms_resonance(
      ibook, run, iSetup, histos.resonanceJ_patElectron_passDoubleEG_DST,
      "resonanceJ_Tag_pat_Probe_patElectron_passDoubleEG_DST");
  bookHistograms_resonance(
      ibook, run, iSetup, histos.resonanceY_patElectron_passDoubleEG_DST,
      "resonanceY_Tag_pat_Probe_patElectron_passDoubleEG_DST");
  bookHistograms_resonance(
      ibook, run, iSetup, histos.resonanceAll_patElectron_passDoubleEG_DST,
      "resonanceAll_Tag_pat_Probe_patElectron_passDoubleEG_DST");

  bookHistograms_resonance(
      ibook, run, iSetup, histos.resonanceZ_sctElectron_passDoubleEG_DST,
      "resonanceZ_Tag_pat_Probe_sctElectron_passDoubleEG_DST");
  bookHistograms_resonance(
      ibook, run, iSetup, histos.resonanceJ_sctElectron_passDoubleEG_DST,
      "resonanceJ_Tag_pat_Probe_sctElectron_passDoubleEG_DST");
  bookHistograms_resonance(
      ibook, run, iSetup, histos.resonanceY_sctElectron_passDoubleEG_DST,
      "resonanceY_Tag_pat_Probe_sctElectron_passDoubleEG_DST");
  bookHistograms_resonance(
      ibook, run, iSetup, histos.resonanceAll_sctElectron_passDoubleEG_DST,
...
void PatElectronTagProbeAnalyzer::bookHistograms_resonance(
    DQMStore::IBooker& ibook, edm::Run const& run,
    edm::EventSetup const& iSetup, kProbeKinematicHistos& histos,
    const std::string& name) const {
  ibook.setCurrentFolder(outputInternalPath_);
  histos.hPt_Barrel =
      ibook.book1D(name + "_Pt_Barrel", name + "_Pt_Barrel", 40, 0, 200);
  histos.hPt_Endcap =
      ibook.book1D(name + "_Pt_Endcap", name + "_Pt_Endcap", 40, 0, 200);

  histos.hEta = ibook.book1D(name + "_Eta", name + "_Eta", 20, -5.0, 5.0);
  histos.hPhi = ibook.book1D(name + "_Phi", name + "_Phi", 10, -3.3, 3.3);

  histos.hHoverE_Barrel = ibook.book1D(name + "_HoverE_Barrel",
                                       name + "_HoverE_Barrel", 200, 0, 0.5);
  histos.hHoverE_Endcap = ibook.book1D(name + "_HoverE_Endcap",
                                       name + "_HoverE_Endcap", 200, 0, 0.5);

  histos.hOoEMOoP_Barrel = ibook.book1D(
      name + "_OoEMOoP_Barrel", name + "_OoEMOoP_Barrel", 250, -0.2, 0.2);
  histos.hOoEMOoP_Endcap = ibook.book1D(
      name + "_OoEMOoP_Endcap", name + "_OoEMOoP_Endcap", 250, -0.2, 0.2);

  histos.hdPhiIn_Barrel = ibook.book1D(name + "_dPhiIn_Barrel",
                                       name + "_dPhiIn_Barrel", 100, 0, 0.1);
  histos.hdPhiIn_Endcap = ibook.book1D(name + "_dPhiIn_Endcap",
                                       name + "_dPhiIn_Endcap", 100, 0, 0.1);

  histos.hdEtaIn_Barrel = ibook.book1D(name + "_dEtaIn_Barrel",
                                       name + "_dEtaIn_Barrel", 100, 0, 0.1);
  histos.hdEtaIn_Endcap = ibook.book1D(name + "_dEtaIn_Endcap",
  ...
  histos.hMissingHits_Barrel =
      ibook.book1D(name + "_MissingHits_Barrel", name + "_MissingHits_Barrel",
                   21, -0.5, 20.5);
  histos.hMissingHits_Endcap =
      ibook.book1D(name + "_MissingHits_Endcap", name + "_MissingHits_Endcap",
                   21, -0.5, 20.5);

  histos.hTrackfbrem_Barrel = ibook.book1D(
      name + "_Trackfbrem_Barrel", name + "_Trackfbrem_Barrel", 100, 0, 1.0);
  histos.hTrackfbrem_Endcap = ibook.book1D(
      name + "_Trackfbrem_Endcap", name + "_Trackfbrem_Endcap", 100, 0, 1.0);

  histos.hTrack_pt_Barrel = ibook.book1D(
      name + "_Track_pt_Barrel", name + "_Track_pt_Barrel", 200, 0, 100.0);
  histos.hTrack_pt_Endcap = ibook.book1D(
      name + "_Track_pt_Endcap", name + "_Track_pt_Endcap", 200, 0, 100.0);

  histos.hTrack_pMode_Barrel =
      ibook.book1D(name + "_Track_pMode_Barrel", name + "_Track_pMode_Barrel",
                   50, -0.5, 49.5);
  histos.hTrack_pMode_Endcap =
      ibook.book1D(name + "_Track_pMode_Endcap", name + "_Track_pMode_Endcap",
                   50, -0.5, 49.5);
  ...
  histos.hPtvsInvMass_Barrel =
      ibook.book2D(name + "_PtvsInvMass_Barrel", name + "_PtvsInvMass_Barrel",
                   5, 0, 100, 10, 80, 100);
  histos.hPtvsInvMass_Endcap =
      ibook.book2D(name + "_PtvsInvMass_Endcap", name + "_PtvsInvMass_Endcap",
                   5, 0, 100, 10, 80, 100);

  histos.hEtavsInvMass =
      ibook.book2D(name + "_EtavsInvMass", name + "_EtavsInvMass", 10, -2.5,
                   2.5, 10, TandP_Z_minMass, TandP_Z_maxMass);

  histos.hInvMass = ibook.book1D(name + "_Invariant_Mass",
...
double PatElectronTagProbeAnalyzer::computePtFromEnergyMassEta(
    double energy, double mass, double eta) const {
  double transverseEnergy = std::sqrt(energy * energy - mass * mass);
  return transverseEnergy / std::cosh(eta);
}

bool PatElectronTagProbeAnalyzer::scoutingElectronGsfTrackID(
    const Run3ScoutingElectron el, size_t trackIdx) const {
  if (trackIdx > el.trkpt().size())
    edm::LogError("ScoutingMonitoring")
  ...
  math::PtEtaPhiMLorentzVector particleTrk(el.trkpt()[trackIdx],
                                           el.trketa()[trackIdx],
                                           el.trkphi()[trackIdx], 0.0005);
...
bool PatElectronTagProbeAnalyzer::scoutingElectronGsfTrackIdx(
    const Run3ScoutingElectron el, size_t& trackIdx) const {
  ...
  for (size_t i = 0; i < el.trkpt().size(); ++i) {
    if (scoutingElectronGsfTrackID(el, i)) {
      if (!foundGoodGsfTrkIdx) {
        ...
        double relPtDiffOld =
            std::abs(el.trkpt()[trackIdx] - el.pt()) / el.pt();
...
bool PatElectronTagProbeAnalyzer::hasPatternInHLTPath(
...
bool PatElectronTagProbeAnalyzer::scoutingElectron_passHLT(
  ...
  for (unsigned int i = 0; i < legObjects.size(); i++) {
    float delR2 =
        deltaR2(legObjects.at(i).eta(), legObjects.at(i).phi(), el_eta, el_phi);
    if (delR2 < 0.01) {
      foundTheLeg = true;
      break;
    }
...
bool PatElectronTagProbeAnalyzer::patElectron_passHLT(
    const pat::Electron el, TString filter,
    trigger::TriggerObjectCollection legObjects) const {
  bool foundTheLeg = false;
  for (unsigned int i = 0; i < legObjects.size(); i++) {
    float delR2 = deltaR2(legObjects.at(i).eta(), legObjects.at(i).phi(),
                          el.eta(), el.phi());
    if (delR2 < 0.01) {
      foundTheLeg = true;
      break;
