...
void  ScoutingMuonTagProbeAnalyzer::bookHistograms_resonance(DQMStore::IBooker& ibook,
     ...
     histos.hInvMass = ibook.book1D(name + "_sctMuon_Invariant_Mass",name + "_sctMuon_Invariant_Mass;Muon Inv mass (GeV); Muons", 100, 0, 5);
     histos.hNormChisq = ibook.book1D(name + "_Probe_sctMuon_NormChisq",name + "_Probe_sctMuon_NormChisq; Muon normChi2; Muons", 60, 0, 5.0);
     histos.hTrk_dxy = ibook.book1D(name + "_Probe_sctMuon_Trk_dxy",name + "_Probe_sctMuon_Trk_dxy; Track dxy; Muons", 60, 0, 5.0);
     histos.hTrk_dz = ibook.book1D(name + "_Probe_sctMuon_Trk_dz",name + "_Probe_sctMuon_Trk_dz; Track dz; Muons", 60, 0, 20.0);
     histos.htype = ibook.book1D(name + "_Probe_sctMuon_type",name + "_Probe_sctMuon_type;Muon type; Muons", 15, 0, 15);
     histos.hcharge = ibook.book1D(name + "_Probe_sctMuon_charge",name + "_Probe_sctMuon_charge; Muon charge; Muons", 3, -1, 2);
     histos.hecalIso = ibook.book1D(name + "_Probe_sctMuon_ecalIso",name + "_Probe_sctMuon_ecalIso; Muon ecalIso; Muons", 60, 0.0, 20.0);
     ...
     histos.hnValidStandAloneMuonHits = ibook.book1D(name + "_Probe_sctMuon_nValidStandAloneMuonHits",name + "_Probe_sctMuon_nValidStandAloneMuonHits;nValidStandAloneMuonHits; Muons", 25, 0, 25);
     histos.hnValidRecoMuonHits = ibook.book1D(name + "_Probe_sctMuon_nValidRecoMuonHits",name + "_Probe_sctMuon_nValidRecoMuonHits;nValidRecoMuonHits; Muons", 25, 0, 25);
     histos.hnRecoMuonChambers = ibook.book1D(name + "_Probe_sctMuon_nRecoMuonChambers",name + "_Probe_sctMuon_nRecoMuonChambers; nRecoMuonChambers; Muons", 10, 0, 10.0);
     histos.hnStandAloneMuonMatchedStations = ibook.book1D(name + "_Probe_sctMuon_nStandAloneMuonMatchedStations",name + "_Probe_sctMuon_nStandAloneMuonMatchedStations; nStandAloneMuonMatchedStations; Muons", 5, 0, 5.0);
     histos.hnRecoMuonChambersCSCorDT = ibook.book1D(name + "_Probe_sctMuon_nRecoMuonChambersCSCorDT",name + "_Probe_sctMuon_nRecoMuonChambersCSCorDT;nRecoMuonChambersCSCorDT; Muons", 10, 0, 10.0);
     histos.hnRecoMuonMatches = ibook.book1D(name + "_Probe_sctMuon_nRecoMuonMatches",name + "_Probe_sctMuon_nRecoMuonMatches; nRecoMuonMatches; Muons", 6, 0, 6);
     histos.hnRecoMuonMatchedStations = ibook.book1D(name + "_Probe_sctMuon_nRecoMuonMatchedStations",name + "_Probe_sctMuon_nRecoMuonMatchedStations; nRecoMuonMatchedStations; Muons", 5, 0.0, 5.0);
     histos.hnRecoMuonExpectedMatchedStations = ibook.book1D(name + "_sctMuon_nRecoMuonExpectedMatchedStations",name + "_sctMuon_nRecoMuonExpectedMatchedStations;nRecoMuonExpectedMatchedStations; Muons", 6, 0, 6.0);
     histos.hnValidPixelHits = ibook.book1D(name + "_Probe_sctMuon_nValidPixelHits",name + "_Probe_sctMuon_nValidPixelHits;nValidPixelHits; Muons", 14, 0, 14.0);
     histos.hnValidStripHits = ibook.book1D(name + "_Probe_sctMuon_nValidStripHits",name + "_Probe_sctMuon_nValidStripHits; nValidStripHits; Muons", 25, 0, 25);
     histos.htrk_chi2 = ibook.book1D(name + "_Probe_sctMuon_trk_chi2",name + "_Probe_sctMuon_trk_chi2; trk_chi2; Muons", 60, 0.0, 20);
     histos.htrk_ndof = ibook.book1D(name + "_sctMuon_trk_ndof",name + "_sctMuon_trk_ndof;trk_ndof; Muons", 100, 0, 50);
     histos.htrk_lambda = ibook.book1D(name + "_Probe_sctMuon_trk_lambda",name + "_Probe_sctMuon_trk_lambda; trk_lambda; Muons", 60, -5, 5);
     histos.htrk_pt = ibook.book1D(name + "_Probe_sctMuon_trk_pt",name + "_Probe_sctMuon_trk_pt; trk_pt; Muons", 60, 0, 50.0);
     histos.htrk_eta = ibook.book1D(name + "_Probe_sctMuon_trk_eta",name + "_Probe_sctMuon_trk_eta;trk_eta; Muons", 60, -3.3, 3.3);
     histos.htrk_dxyError = ibook.book1D(name + "_Probe_sctMuon_trk_dxyError",name + "_Probe_sctMuon_trk_dxyError; trk_dxyError; Muons", 60, 0.0, 0.8);
     histos.htrk_dzError = ibook.book1D(name + "_Probe_sctMuon_trk_dzError",name + "_Probe_sctMuon_trk_dzError; trk_dzError; Muons", 60, 0.0, 2);
     histos.htrk_qoverpError = ibook.book1D(name + "_sctMuon_trk_qoverpError",name + "_sctMuon_trk_qoverpError;trk_qoverpError; Muons", 100, 0, 0.05);
     histos.htrk_lambdaError = ibook.book1D(name + "_Probe_sctMuon_trk_lambdaError",name + "_Probe_sctMuon_trk_lambdaError; trk_lambdaError; Muons", 60, 0, 0.2);
     histos.htrk_phiError = ibook.book1D(name + "_Probe_sctMuon_trk_phiError",name + "_Probe_sctMuon_trk_phiError; trk_phiError; Muons", 60, 0, 0.2);
     histos.htrk_dsz = ibook.book1D(name + "_Probe_sctMuon_trk_dsz",name + "_Probe_sctMuon_trk_dsz; trk_dsz; Muons", 60, -20.0, 20.0);
     histos.htrk_dszError = ibook.book1D(name + "_Probe_sctMuon_trk_dszError",name + "_Probe_sctMuon_trk_dszError; trk_dszError; Muons", 60, 0, 0.8);
     histos.htrk_vx = ibook.book1D(name + "_Probe_sctMuon_trk_vx",name + "_Probe_sctMuon_trk_vx; trk_vx; Muons", 60, -1, 1);
     histos.htrk_vy = ibook.book1D(name + "_Probe_sctMuon_trk_vy",name + "_Probe_sctMuon_trk_vy; trk_vy; Muons", 60, -1, 1);
     histos.htrk_vz = ibook.book1D(name + "_Probe_sctMuon_trk_vz",name + "_Probe_sctMuon_trk_vz; trk_vz; Muons", 60, -20, 20);
     histos.hnPixel = ibook.book1D(name + "_Probe_sctMuon_nPixel",name + "_Probe_sctMuon_nPixel; n Pixel Layers; Muons", 6, 0, 6);
     histos.hnTracker = ibook.book1D(name + "_Probe_sctMuon_nTracker",name + "_Probe_sctMuon_nTracker; n Tracker Layers; Muons", 14, 0, 25);
     histos.htrk_qoverp = ibook.book1D(name + "_Probe_sctMuon_trk_qoverp",name + "_Probe_sctMuon_trk_qoverp; n Track q/p; Muons", 40, -1, 1);
     ...
     histos.htracksSize = ibook.book1D(name + "_Vertex_tracksSize",name + "_Vertex_tracksSize; vertex tracksSize; Muons", 60, 0, 10);
