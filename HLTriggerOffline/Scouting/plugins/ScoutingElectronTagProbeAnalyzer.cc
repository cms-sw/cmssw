...
void ScoutingElectronTagProbeAnalyzer::dqmAnalyze(
  ...
  std::vector<std::string> filterToMatch = {
      "hltDoubleEG11CaloIdLHEFilter", "hltEG30EBTightIDTightIsoTrackIsoFilter"};
  size_t numberOfFilters = filterToMatch.size();
  trigger::TriggerObjectCollection* legObjects =
      new trigger::TriggerObjectCollection[numberOfFilters];
    ...
    for (const auto& sct_el_second : *sctEls) {
      ...
      if ((9.0 < invMass) && (invMass < 12.6)) {
        fillHistograms_resonance(
            histos.resonanceY, sct_el_second, invMass,
...
bool ScoutingElectronTagProbeAnalyzer::scoutingElectron_passHLT(
  ...
  for (unsigned int i = 0; i < legObjects.size(); i++) {
    float delR = deltaR(legObjects.at(i).eta(), legObjects.at(i).phi(),
                        el.eta(), el.phi());
    if (delR < 0.1) {
      foundTheLeg = true;
      break;
...
bool ScoutingElectronTagProbeAnalyzer::scoutingElectronID(
    const Run3ScoutingElectron el) const {
...
void ScoutingElectronTagProbeAnalyzer::fillHistograms_resonance(
  ...
  if (fabs(el.eta()) < 1.5) {
    ...
    if (scoutingElectron_passHLT(el, "hltDoubleEG16EG12CaloIdLHEFilter",
                                 legObjectsCollection[0]))
      histos.hPt_Barrel_passDSTdoubleEG->Fill(el.pt());
    if (scoutingElectron_passHLT(el, "hltSingleEG30CaloIdLHEFilter",
                                 legObjectsCollection[1]))
...
void ScoutingElectronTagProbeAnalyzer::bookHistograms(
    DQMStore::IBooker& ibook, edm::Run const& run,
    edm::EventSetup const& iSetup, kSctTagProbeHistos& histos) const {
  ...
  bookHistograms_resonance(ibook, run, iSetup, histos.resonanceAll,
                           "resonanceAll");
}

void ScoutingElectronTagProbeAnalyzer::bookHistograms_resonance(
    DQMStore::IBooker& ibook, edm::Run const& run,
    edm::EventSetup const& iSetup, kSctProbeKinematicHistos& histos,
  ...
  histos.hEta = ibook.book1D(name + "_Probe_sctElectron_Eta",
                             name + "_Probe_sctElectron_Eta", 100, -5.0, 5.0);
  histos.hPhi = ibook.book1D(name + "_Probe_sctElectron_Phi",
...
void ScoutingElectronTagProbeAnalyzer::fillDescriptions(
  ...
  desc.add<edm::InputTag>("TriggerResultTag",
