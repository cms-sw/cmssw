...
bool ScoutingEGammaCollectionMonitoring::scoutingElectronGsfTrackID(const Run3ScoutingElectron el, size_t trackIdx) const{

  if(trackIdx > el.trkpt().size()) edm::LogError("ScoutingEGammaCollectionMonitoring") << "Invalid track index for electron: Exceeds the number of tracks";
...
bool ScoutingEGammaCollectionMonitoring::scoutingElectronGsfTrackIdx(const Run3ScoutingElectron el, size_t &trackIdx) const{
...
bool ScoutingEGammaCollectionMonitoring::hasPatternInHLTPath(const edm::TriggerNames& triggerNames, const std::string& pattern) const{
