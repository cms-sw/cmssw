...
void ScoutingEGammaCollectionMonitoring::dqmAnalyze(edm::Event const& iEvent,
  ...
  if (tight_sctElectron_index.size() == 2) {
    ...
    math::PtEtaPhiMLorentzVector sctElCombined0(
        getPtFromEnergyMassEta(sctEl0.energy(), 0.0005, sctEls->at(tight_sctElectron_index[0]).trketa()[gsfTrkIdx0]),
    ...
    math::PtEtaPhiMLorentzVector sctElCombined1(
        getPtFromEnergyMassEta(sctEl1.energy(), 0.0005, sctEls->at(tight_sctElectron_index[1]).trketa()[gsfTrkIdx1]),
    ...
    if (fabs(sctEls->at(tight_sctElectron_index[0]).eta()) < 1.479 &&
        fabs(sctEls->at(tight_sctElectron_index[1]).eta()) < 1.479) {
      histos.sctElectron.h1InvMassIDEBEB->Fill(invMass);
    } else if (fabs(sctEls->at(tight_sctElectron_index[0]).eta()) < 1.479 &&
               fabs(sctEls->at(tight_sctElectron_index[1]).eta()) > 1.479) {
