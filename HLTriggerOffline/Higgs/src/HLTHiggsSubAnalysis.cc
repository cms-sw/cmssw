...
void HLTHiggsSubAnalysis::bookHistograms(DQMStore::IBooker &ibooker)
{
    ...
    for(const auto & source : sources)
    {
        ...
        if( (! _useNminOneCuts) || source == "rec" ) _elements[nameVtxPlot] = ibooker.book1D(nameVtxPlot.c_str(), titlePu.c_str(), nBinsPu, minPu, maxPu);
        ...
        for (size_t j = 0 ; j < _hltPathsToCheck.size() ; j++){
            ...
            if( (! _useNminOneCuts) || source == "rec" ) _elements[nameVtxPlot+"_"+shortpath] = ibooker.book1D(nameVtxPlot+"_"+shortpath, titlePassingPu.c_str(), nBinsPu, minPu, maxPu);
...
/*      if( it->first == EVTColContainer::MUON || it->first == EVTColContainer::TRACK )
    ...
    for(auto & it : sourceMatchMap)
    {
        ...
        for(auto & _analyzer : _analyzers)
        {
            ...
            if( _useNminOneCuts ) {
                _analyzer.analyze(ispassTrigger,source,it.second, nMinOne, dEtaqq, mqq, dPhibb, CSV1, CSV2, CSV3, passAllCuts);
            }
            else {
            ...
            for (const auto & itePath : _hltPathsToCheck){
                refOfThePath++;
                if (TString(hltPath).Contains(itePath)) break;
            }
...
//      EVTColContainer::TRACK,  // Note is tracker muon
    ...
    for(unsigned int objtriggername : objtriggernames)
    {
        ...
        {
            if( (objtriggername == EVTColContainer::PFJET && TString(hltPath).Contains("WHbbBoost") ) ||   // fix for HLT_Ele27_WPLoose_Gsf_WHbbBoost_v
        ...
        if( ( objtriggername == EVTColContainer::CALOMET && (TString(hltPath).Contains("PFMET") || TString(hltPath).Contains("MHT") ) ) || // fix for PFMET
        (objtriggername == EVTColContainer::PFJET && TString(hltPath).Contains("JetIdCleaned") && ! TString(hltPath).Contains(TRegexp("Jet[^I]"))) || // fix for Htaunu
    ...
    if( _NminOneCuts[8] ) {
        ...
        for(const auto & matche : matches)
        {
            if( matche.objType == EVTColContainer::PFMET ) {
                if( matche.pt > _NminOneCuts[8] ) jetCutResult["PFMET"] = true;
                break;
            }
        }
    }    
