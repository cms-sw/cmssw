...
int closestJet(const RefToBase<reco::Jet> jet,
  ...
  for (unsigned int i = 0; i < association.size(); ++i) {
    double d = ROOT::Math::VectorUtil::DeltaR(jet->momentum(),
                                              association[i].first->momentum());
    if (d < distance) {
      distance = d;
      closest = i;
    }
...
HLTBTagPerformanceAnalyzer::HLTBTagPerformanceAnalyzer(
  ...
  hlTriggerResults_ = consumes<edm::TriggerResults>(
      iConfig.getParameter<InputTag>("TriggerResults"));
  JetTagCollection_ = edm::vector_transform(
      iConfig.getParameter<std::vector<edm::InputTag>>("JetTag"),
      [this](edm::InputTag const &tag) {
        return mayConsume<reco::JetTagCollection>(tag);
      });
  ...
  shallowTagInfosTokenPf_ = consumes<std::vector<reco::ShallowTagInfo>>(
      edm::InputTag("hltDeepCombinedSecondaryVertexBJetTagsInfos"));
  m_mcPartons = consumes<JetFlavourMatchingCollection>(
      iConfig.getParameter<InputTag>("mcPartons"));
  hltPathNames_ =
  ...
  for (unsigned int i = 0; i < m_mcLabels.size(); ++i)
    m_mcFlavours.push_back(
        mc.getParameter<std::vector<unsigned int>>(m_mcLabels[i]));
...
void HLTBTagPerformanceAnalyzer::dqmBeginRun(const edm::Run &iRun,
  ...
  std::set_union(std::begin(keepSet), std::end(keepSet), std::begin(keepSetVtx),
                 std::end(keepSetVtx),
                 std::inserter(keepSet, std::begin(keepSet)));
...
void HLTBTagPerformanceAnalyzer::analyze(const edm::Event &iEvent,
                                         const edm::EventSetup &iSetup) {
  ...
  // get partons
  if (m_mcMatching && !m_mcPartons_Label.empty() &&
      m_mcPartons_Label != "NULL") {
  ...
  for (unsigned int ind = 0; ind < hltPathNames_.size(); ind++) {
    ...
    if (!JetTagCollection_Label[ind].empty() &&
        JetTagCollection_Label[ind] != "NULL") {
    ...
    // fill JetTag map
    if (BtagOK)
      for (auto iter = JetTagHandler->begin(); iter != JetTagHandler->end();
           iter++) {
        JetTag.insert(JetTagMap::value_type(iter->first, iter->second));
      }
    else {
      edm::LogInfo("NoCollection")
          << "Collection " << JetTagCollection_Label[ind] << " ==> not found";
      return;
    }
    // fill Inputs for All
      ...
      for (auto &info : *(shallowTagInfosPf)) {
        ...
        for (auto entry = vars.begin(); entry != vars.end(); ++entry) {
          if (keepSet.find(TaggingVariableTokens[entry->first]) !=
            ...
            try {
              H1_.at(ind)[TaggingVariableTokens[entry->first]]->Fill(
                  std::fmax(0.0, entry->second));
    ...
    for (auto &BtagJT : JetTag) {
      ...
      inmodule[HEP17] = (BtagJT.first->phi() >= -0.87) &&
                        (BtagJT.first->phi() < -0.52) &&
                        (BtagJT.first->eta() > 1.3);
      inmodule[HEP18] = (BtagJT.first->phi() >= -0.52) &&
                        (BtagJT.first->phi() < -0.17) &&
                        (BtagJT.first->eta() > 1.3);
      inmodule[HEM17] = (BtagJT.first->phi() >= -0.87) &&
                        (BtagJT.first->phi() < -0.52) &&
                        (BtagJT.first->eta() < -1.3);

      // fill 1D btag plot for 'all'
      H1_.at(ind)[JetTagCollection_Label[ind]]->Fill(
          std::fmax(0.0, BtagJT.second));
      for (auto i : HCALSpecialsNames) {
        if (inmodule[i.first])
          H1mod_.at(ind)[JetTagCollection_Label[ind]][i.first]->Fill(
              std::fmax(0.0, BtagJT.second));
      }
      if (MCOK) {
        int m = closestJet(BtagJT.first, *h_mcPartons, m_mcRadius);
        unsigned int flavour =
            (m != -1) ? abs((*h_mcPartons)[m].second.getFlavour()) : 0;
        for (unsigned int i = 0; i < m_mcLabels.size(); ++i) {
          std::string flavour_str = m_mcLabels[i];
          flavours_t flav_collection = m_mcFlavours[i];
          auto it = std::find(flav_collection.begin(), flav_collection.end(),
                              flavour);
          if (it == flav_collection.end())
            continue;
          std::string label = JetTagCollection_Label[ind] + "__";
          label += flavour_str;
          H1_.at(ind)[label]->Fill(
              std::fmax(0.0, BtagJT.second)); // fill 1D btag plot for 'b,c,uds'
          for (auto j : HCALSpecialsNames) {
            if (inmodule[j.first])
              H1mod_.at(ind)[label][j.first]->Fill(std::fmax(
                  0.0, BtagJT.second)); // fill 1D btag plot for 'b,c,uds' in
                                        // modules (HEP17 etc.)
          }
          ...
          label += "_disc_pT";
          H2_.at(ind)[label]->Fill(
              std::fmax(0.0, BtagJT.second),
              BtagJT.first->pt()); // fill 2D btag, jetPt plot for 'b,c,uds'
          for (auto j : HCALSpecialsNames) {
            if (inmodule[j.first])
              H2mod_.at(ind)[label][j.first]->Fill(
                  std::fmax(0.0, BtagJT.second), BtagJT.first->pt());
          }
          labelEta += "_disc_eta";
          H2Eta_.at(ind)[labelEta]->Fill(
              std::fmax(0.0, BtagJT.second),
              BtagJT.first->eta()); // fill 2D btag, jetEta plot for 'b,c,uds'
          labelPhi += "_disc_phi";
          H2Phi_.at(ind)[labelPhi]->Fill(
              std::fmax(0.0, BtagJT.second),
              BtagJT.first->phi()); // fill 2D btag, jetPhi plot for 'b,c,uds'
          labelEtaPhi += "_eta_phi";
          H2EtaPhi_.at(ind)[labelEtaPhi]->Fill(
              BtagJT.first->eta(),
              BtagJT.first->phi()); // fill 2D btag, jetPhi plot for 'b,c,uds'
  ...
  for (unsigned int ind = 0; ind < hltPathNames_.size(); ind++) {
    ...
    if (!JetTagCollection_Label[ind].empty() &&
      ...
      H1_.back()[JetTagCollection_Label[ind]] = ibooker.book1D(
          JetTagCollection_Label[ind] + "_all",
          JetTagCollection_Label[ind] + "_all", btagBins, btagL, btagU);
      H1_.back()[JetTagCollection_Label[ind]]->setAxisTitle(
      ...
      for (int i = 0; i < 100; i++) {
        if (keepSetJet.find(TaggingVariableTokens[i]) !=
            keepSetJet.end()) { // if input name in defined set
      ...
      for (int i = 0; i < 100; i++) {
        if (keepSetTrack.find(TaggingVariableTokens[i]) !=
            keepSetTrack.end()) { // if input name in defined set
          std::string inpt = TaggingVariableTokens[i];
          H1_.back()[inpt] = ibooker.book1D(inpt, inpt, 105, -5, 100.);
          H1_.back()[inpt]->setAxisTitle(inpt, 1);
        } else
      ...
      for (int i = 0; i < 100; i++) {
        if (keepSetVtx.find(TaggingVariableTokens[i]) !=
            keepSetVtx.end()) { // if input name in defined set
          std::string inpt = TaggingVariableTokens[i];
          H1_.back()[inpt] = ibooker.book1D(inpt, inpt, 105, -5, 100.);
          H1_.back()[inpt]->setAxisTitle(inpt, 1);
        } else
    ...
    for (unsigned int i = 0; i < m_mcLabels.size(); ++i) {
      ...
      if (!JetTagCollection_Label[ind].empty() &&
        ...
        H1_.back()[label] = ibooker.book1D(
            label,
            Form("%s %s", JetTagCollection_Label[ind].c_str(), flavour.c_str()),
            btagBins, btagL, btagU);
        H1_.back()[label]->setAxisTitle("disc", 1);
        for (auto j : HCALSpecialsNames) {
          ibooker.setCurrentFolder(dqmFolder + "/" + j.second);
          H1mod_.back()[label][j.first] =
              ibooker.book1D(label,
                             Form("%s %s", JetTagCollection_Label[ind].c_str(),
                                  flavour.c_str()),
                             btagBins, btagL, btagU);
        ...
        H2_.back()[label] = ibooker.book2D(label, label, btagBins, btagL, btagU,
                                           nBinsPt, pTmin, pTMax);
        H2_.back()[label]->setAxisTitle("pT", 2);
        H2_.back()[label]->setAxisTitle("disc", 1);
        for (auto j : HCALSpecialsNames) {
          ibooker.setCurrentFolder(dqmFolder + "/" + j.second);
          H2mod_.back()[label][j.first] = ibooker.book2D(
              label, label, btagBins, btagL, btagU, nBinsPt, pTmin, pTMax);
          H2mod_.back()[label][j.first]->setAxisTitle("pT", 2);
          H2mod_.back()[label][j.first]->setAxisTitle("disc", 1);
        }
        ibooker.setCurrentFolder(dqmFolder);
        H2Eta_.back()[labelEta] =
            ibooker.book2D(labelEta, labelEta, btagBins, btagL, btagU, nBinsEta,
                           etamin, etaMax);
        H2Eta_.back()[labelEta]->setAxisTitle("eta", 2);
        H2Eta_.back()[labelEta]->setAxisTitle("disc", 1);
        H2Phi_.back()[labelPhi] =
            ibooker.book2D(labelPhi, labelPhi, btagBins, btagL, btagU, nBinsPhi,
                           phimin, phiMax);
        H2Phi_.back()[labelPhi]->setAxisTitle("phi", 2);
        H2Phi_.back()[labelPhi]->setAxisTitle("disc", 1);
        H2EtaPhi_.back()[labelEtaPhi] =
            ibooker.book2D(labelEtaPhi, labelEtaPhi, nBinsEta, etamin, etaMax,
                           nBinsPhi, phimin, phiMax);
        H2EtaPhi_.back()[labelEtaPhi]->setAxisTitle("phi", 2);
        H2EtaPhi_.back()[labelEtaPhi]->setAxisTitle("eta", 1);
        H2EtaPhi_threshold_.back()[labelEtaPhi_threshold] =
            ibooker.book2D(labelEtaPhi_threshold, labelEtaPhi_threshold,
                           nBinsEta, etamin, etaMax, nBinsPhi, phimin, phiMax);
        H2EtaPhi_threshold_.back()[labelEtaPhi_threshold]->setAxisTitle("phi",
                                                                        2);
        H2EtaPhi_threshold_.back()[labelEtaPhi_threshold]->setAxisTitle("eta",
                                                                        1);
      }
    } /// for mc.size()
