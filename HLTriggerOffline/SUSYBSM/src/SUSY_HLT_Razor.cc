...
SUSY_HLT_Razor::SUSY_HLT_Razor(const edm::ParameterSet &ps) {
  ...
  theMETCollection_ = consumes<edm::View<reco::MET>>(
      ps.getParameter<edm::InputTag>("METCollection"));
  theHemispheres_ = consumes<std::vector<math::XYZTLorentzVector>>(
      ps.getParameter<edm::InputTag>("hemispheres"));
  ...
  theJetCollection_ = consumes<edm::View<reco::Jet>>(
      ps.getParameter<edm::InputTag>("jetCollection"));
}
...
void SUSY_HLT_Razor::fillDescriptions(
    edm::ConfigurationDescriptions &descriptions) {
  edm::ParameterSetDescription desc;
  desc.add<edm::InputTag>("jetCollection", edm::InputTag("ak4PFJetsCHS"));
  desc.add<edm::InputTag>(
          "CaloFilter", edm::InputTag("hltRsqMR200Rsq0p01MR100Calo", "", "HLT"))
      ->setComment(
  ...
  desc.add<std::string>("TriggerPath", "HLT_RsqMR300_Rsq0p09_MR200_v")
      ->setComment("trigger path name");
  desc.add<edm::InputTag>("TriggerFilter",
                          edm::InputTag("hltRsqMR300Rsq0p09MR200", "", "HLT"))
...
void SUSY_HLT_Razor::analyze(edm::Event const &e,
  ...
  if (!(filterIndex >= triggerSummary->sizeFilters())) {
    ...
    for (size_t j = 0; j < keys.size(); ++j) {
      ...
      if (foundObject.id() ==
          0) { // the MET object containing MR and Rsq will show up with ID = 0
        onlineMR =
            foundObject
                .px(); // razor variables stored in dummy reco::MET objects
  ...
  for (unsigned int i = 0; i < jetCollection->size(); i++) {
    if (std::abs(jetCollection->at(i).eta()) < 3.0 &&
        jetCollection->at(i).pt() >= 40.0) {
  ...
  if (hasFired) {
    ...
    if (hemispheres->empty()) { // the Hemisphere Maker will produce an empty
                                ...
                                // jets in the
      edm::LogError("SUSY_HLT_Razor")
          << "Cannot calculate M_R and R^2 because there are too many jets! "
    ...
    if (!hemispheres->empty() && hemispheres->size() != 2 &&
        hemispheres->size() != 5 && hemispheres->size() != 10) {
      edm::LogError("SUSY_HLT_Razor")
...
void SUSY_HLT_Razor::bookHistos(DQMStore::IBooker &ibooker_) {
  ...
  h_mrRsq = ibooker_.book2D("mrRsq", "R^{2} vs M_{R}; GeV; ", 100, 0.0, 4000.0,
                            100, 0.0, 1.5);

  h_mr_tight =
      ibooker_.book1D("mr_tight", "M_{R} (R^{2} > 0.25) ; GeV", 100, 0.0, 4000);
  h_rsq_tight =
      ibooker_.book1D("rsq_tight", "R^{2} (M_{R} > 400) ; ", 100, 0.0, 1.5);

  h_rsq_loose =
      ibooker_.book1D("rsq_loose", "R^{2} (M_{R} > 0) ; ", 100, 0.0, 1.5);

  h_ht = ibooker_.book1D("ht", "HT; GeV; ", 100, 0.0, 4000.0);
  h_met = ibooker_.book1D("met", "MET; GeV; ", 100, 0.0, 1000);
  h_htMet = ibooker_.book2D("htMet", "MET vs HT; GeV; ", 100, 0.0, 4000.0, 100,
                            0.0, 1000);

  h_online_mr_vs_mr =
      ibooker_.book2D("online_mr_vs_mr",
                      "Online M_{R} vs  Offline M_{R} (events passing "
                      "trigger); Offline M_{R} (GeV); Online M_{R} (GeV); ",
                      100, 0.0, 4000.0, 100, 0.0, 4000.0);
  h_calo_mr_vs_mr =
      ibooker_.book2D("calo_mr_vs_mr",
                      "Calo M_{R} vs  Offline M_{R} (events passing trigger); "
                      "Offline M_{R} (GeV); Calo M_{R} (GeV); ",
                      100, 0.0, 4000.0, 100, 0.0, 4000.0);
  h_online_rsq_vs_rsq =
      ibooker_.book2D("online_rsq_vs_rsq",
                      "Online R^{2} vs Offline R^{2} (events passing trigger); "
                      "Offline R^{2}; Online R^{2}; ",
                      100, 0.0, 1.5, 100, 0.0, 1.5);
  h_calo_rsq_vs_rsq =
      ibooker_.book2D("calo_rsq_vs_rsq",
...
double
SUSY_HLT_Razor::CalcR(double MR, TLorentzVector ja, TLorentzVector jb,
                      edm::Handle<edm::View<reco::MET>> inputMet,
