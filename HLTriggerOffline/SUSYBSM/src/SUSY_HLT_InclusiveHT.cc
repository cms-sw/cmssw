...
SUSY_HLT_InclusiveHT::SUSY_HLT_InclusiveHT(const edm::ParameterSet &ps) {
  edm::LogInfo("SUSY_HLT_InclusiveHT")
...
void SUSY_HLT_InclusiveHT::analyze(edm::Event const &e,
  ...
  if (!hltresults.isValid()) {
    edm::LogError("SUSY_HLT_InclusiveHT")
...
void SUSY_HLT_InclusiveHT::bookHistos(DQMStore::IBooker &ibooker_) {
  ...
  h_caloJetPt =
      ibooker_.book1D("caloJetPt", "CaloJet P_{T}; GeV", 20, 0.0, 500.0);
  ...
  h_triggerMetPt =
      ibooker_.book1D("triggerMetPt", "Trigger Met Pt; GeV", 20, 0.0, 500.0);
  h_triggerMetPhi =
      ibooker_.book1D("triggerMetPhi", "Trigger Met Phi", 20, -3.5, 3.5);
  h_triggerHT =
      ibooker_.book1D("triggerHT", "Trigger HT; GeV", 30, 0.0, 1500.0);
