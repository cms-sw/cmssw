...
void HltSusyExoPostProcessor::dqmEndJob(DQMStore::IBooker& iBooker_, DQMStore::IGetter& iGetter_) {
  ...
  int L1bins[7]; for(int & L1bin : L1bins) L1bin=0;
  int Hltbins[8]; for(int & Hltbin : Hltbins) Hltbin=0;
  ...
  int L1bincounter[8]; for(int & i : L1bincounter) i=0;
  int Hltbincounter[8]; for(int & i : Hltbincounter) i=0;
  ...
  for(auto & mc_dir : mc_dirs) {
    ...
    MonitorElement* hL1EffAfterMcCuts  = bookEffMEProfileFromTH1(iGetter_.get(subDir_ + mcSelBitsDir + "/" + mc_dir + "/L1Paths_" + mc_dir)->getTH1F(), (std::string) set_name_L1, iBooker_);
    hL1EffAfterMcCuts->setTitle("L1 Efficiencies for " + mc_dir + " selection");
    MonitorElement* hHltEffAfterMcCuts = bookEffMEProfileFromTH1(iGetter_.get(subDir_ + mcSelBitsDir + "/" + mc_dir + "/HltPaths_" + mc_dir)->getTH1F(), (std::string) set_name_Hlt, iBooker_);
    ...
    if(mcFlag)
      nTotalAfterMcCuts = iGetter_.get(subDir_ + mcSelBitsDir + "/" + mc_dir + "/L1Bits_" + mc_dir)->getBinContent(nL1bins);
    else
      nTotalAfterMcCuts = iGetter_.get(subDir_ + mcSelBitsDir + "/" + mc_dir + "/HltBits_" + mc_dir)->getBinContent(reference_bin);
    ...
    //fill the eff histo
    int L1bincounter_mc[8]; for(int & j : L1bincounter_mc) j=0;
    int Hltbincounter_mc[8]; for(int & j : Hltbincounter_mc) j=0;
    ...
    for(unsigned int j=0; j<L1placement.size(); j++) {
      value = nTotalAfterMcCuts ? (float)iGetter_.get(subDir_ + mcSelBitsDir + "/" + mc_dir + "/L1Paths_" + mc_dir)->getBinContent(j+1) / nTotalAfterMcCuts : 0;
    ...
    if(nL1bins!=int(L1placement.size())) {
      value = nTotalAfterMcCuts ? (float)iGetter_.get(subDir_ + mcSelBitsDir + "/" + mc_dir + "/L1Paths_" + mc_dir)->getBinContent(nL1bins) / nTotalAfterMcCuts : 0;
      error = nTotalAfterMcCuts ? sqrt(value*(1-value)/nTotalAfterMcCuts) : 0;
    ...
    for(unsigned int j=0; j<Hltplacement.size(); j++) {
      value = nTotalAfterMcCuts ? (float)iGetter_.get(subDir_ + mcSelBitsDir + "/" + mc_dir + "/HltPaths_" + mc_dir)->getBinContent(j+1) / nTotalAfterMcCuts : 0;
      error = nTotalAfterMcCuts ? sqrt(value*(1-value)/nTotalAfterMcCuts) : 0;
    ...
    if(nHltbins!=int(Hltplacement.size())) {
      value = nTotalAfterMcCuts ? (float)iGetter_.get(subDir_ + mcSelBitsDir + "/" + mc_dir + "/HltPaths_" + mc_dir)->getBinContent(nHltbins) / nTotalAfterMcCuts : 0;
      error = nTotalAfterMcCuts ? sqrt(value*(1-value)/nTotalAfterMcCuts) : 0;
  ...
  for(auto & reco_dir : reco_dirs) {
    ...
    MonitorElement* hL1EffAfterRecoCuts  = bookEffMEProfileFromTH1(iGetter_.get(subDir_ + recoSelBitsDir + "/" + reco_dir + "/L1Paths_" + reco_dir)->getTH1F(), (std::string) set_name_L1, iBooker_);
    hL1EffAfterRecoCuts->setTitle("L1 Efficiencies for " + reco_dir + " selection");
    MonitorElement* hHltEffAfterRecoCuts = bookEffMEProfileFromTH1(iGetter_.get(subDir_ + recoSelBitsDir + "/" + reco_dir + "/HltPaths_" + reco_dir)->getTH1F(), (std::string) set_name_Hlt, iBooker_);
    ...
    if(mcFlag)
      nTotalAfterRecoCuts = iGetter_.get(subDir_ + recoSelBitsDir + "/" + reco_dir + "/L1Bits_" + reco_dir)->getBinContent(nL1bins);
    else
      nTotalAfterRecoCuts = iGetter_.get(subDir_ + recoSelBitsDir + "/" + reco_dir + "/HltBits_" + reco_dir)->getBinContent(reference_bin);
    ...
    //fill the eff histo
    int L1bincounter_reco[8]; for(int & j : L1bincounter_reco) j=0;
    int Hltbincounter_reco[8]; for(int & j : Hltbincounter_reco) j=0;
    TProfile* hL1_reco = (TProfile*)hL1EffAfterRecoCuts->getTProfile();
    TProfile* hHlt_reco = (TProfile*)hHltEffAfterRecoCuts->getTProfile();
    for(unsigned int j=0; j<L1placement.size(); j++) {
      value = nTotalAfterRecoCuts ? (float)iGetter_.get(subDir_ + recoSelBitsDir + "/" + reco_dir + "/L1Paths_" + reco_dir)->getBinContent(j+1) / nTotalAfterRecoCuts : 0;
    ...
    if(nL1bins!=int(L1placement.size())) {
      value = nTotalAfterRecoCuts ? (float)iGetter_.get(subDir_ + recoSelBitsDir + "/" + reco_dir + "/L1Paths_" + reco_dir)->getBinContent(nL1bins) / nTotalAfterRecoCuts : 0;
      error = nTotalAfterRecoCuts ? sqrt(value*(1-value)/nTotalAfterRecoCuts) : 0;
    ...
    for(unsigned int j=0; j<Hltplacement.size(); j++) {
      value = nTotalAfterRecoCuts ? (float)iGetter_.get(subDir_ + recoSelBitsDir + "/" + reco_dir + "/HltPaths_" + reco_dir)->getBinContent(j+1) / nTotalAfterRecoCuts : 0;
      error = nTotalAfterRecoCuts ? sqrt(value*(1-value)/nTotalAfterRecoCuts) : 0;
    ...
    if(nHltbins!=int(Hltplacement.size())) {
      value = nTotalAfterRecoCuts ? (float)iGetter_.get(subDir_ + recoSelBitsDir + "/" + reco_dir + "/HltPaths_" + reco_dir)->getBinContent(nHltbins) / nTotalAfterRecoCuts : 0;
      error = nTotalAfterRecoCuts ? sqrt(value*(1-value)/nTotalAfterRecoCuts) : 0;
