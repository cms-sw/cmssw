...
void SUSY_HLT_DoubleEle_Hadronic::dqmBeginRun(edm::Run const &run, edm::EventSetup const &e)
{
    ...
    if(!pathFound) {
        edm::LogError ("SUSY_HLT_DoubleEle_Hadronic") << "Path not found" << "\n";
        return;
    }
...
void SUSY_HLT_DoubleEle_Hadronic::analyze(edm::Event const& e, edm::EventSetup const& eSetup){
    ...
    for( unsigned int hltIndex=0; hltIndex<numTriggers; ++hltIndex ){
        ...
        if (trigNames.triggerName(hltIndex)==triggerPathAuxiliaryForElectron_ && hltresults->wasrun(hltIndex) && hltresults->accept(hltIndex)) hasFiredAuxiliaryForElectronLeg = true;
...
{
    edm::LogInfo("SUSY_HLT_DoubleEle_Hadronic") << "SUSY_HLT_DoubleEle_Hadronic::endLuminosityBlock" << std::endl;
}
