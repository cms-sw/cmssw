...
SUSY_HLT_DoubleEle_Hadronic::SUSY_HLT_DoubleEle_Hadronic(
  ...
  edm::LogInfo("SUSY_HLT_DoubleEle_Hadronic")
      << "Constructor SUSY_HLT_DoubleEle_Hadronic::SUSY_HLT_DoubleEle_Hadronic "
      << std::endl;
  ...
  triggerPathAuxiliaryForElectron_ =
      ps.getParameter<std::string>("TriggerPathAuxiliaryForElectron");
...
SUSY_HLT_DoubleEle_Hadronic::~SUSY_HLT_DoubleEle_Hadronic() {
  edm::LogInfo("SUSY_HLT_DoubleEle_Hadronic")
      << "Destructor SUSY_HLT_DoubleEle_Hadronic::~SUSY_HLT_DoubleEle_Hadronic "
      << std::endl;
}
...
void SUSY_HLT_DoubleEle_Hadronic::dqmBeginRun(edm::Run const &run,
  ...
  if (!fHltConfig.init(run, e, HLTProcess_, changed)) {
    edm::LogError("SUSY_HLT_DoubleEle_Hadronic")
...
void SUSY_HLT_DoubleEle_Hadronic::analyze(edm::Event const &e,
  ...
  if (!(filterIndex >= triggerSummary->sizeFilters())) {
    ...
    if (ptElectron.size() >= 2) {
      math::PtEtaPhiMLorentzVectorD *ele1 = new math::PtEtaPhiMLorentzVectorD(
          ptElectron[0], etaElectron[0], phiElectron[0], 0.0005);
      math::PtEtaPhiMLorentzVectorD *ele2 = new math::PtEtaPhiMLorentzVectorD(
          ptElectron[1], etaElectron[1], phiElectron[1], 0.0005);
      (*ele1) += (*ele2);
  ...
  for (unsigned int hltIndex = 0; hltIndex < numTriggers; ++hltIndex) {
    ...
    if (trigNames.triggerName(hltIndex).find(
            triggerPathAuxiliaryForElectron_) != std::string::npos &&
        hltresults->wasrun(hltIndex) && hltresults->accept(hltIndex))
    ...
    if (hasFiredAuxiliaryForElectronLeg && ElectronCollection->size() > 1) {
      if (hasFired && indexOfMatchedElectron[1] >= 0) { // fill trailing leg
        h_EleTurnOn_num->Fill(
            ElectronCollection->at(indexOfMatchedElectron[1]).pt());
      }
...
void SUSY_HLT_DoubleEle_Hadronic::bookHistos(DQMStore::IBooker &ibooker_) {
  ...
  h_triggerDoubleEleMass = ibooker_.book1D(
      "triggerDoubleEleMass", "Trigger DoubleElectron Mass", 202, -2, 200);
