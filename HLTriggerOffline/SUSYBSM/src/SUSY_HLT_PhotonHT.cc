...
void SUSY_HLT_PhotonHT::bookHistos(DQMStore::IBooker &ibooker_) {
  ...
  // offline quantities
  h_photonPt = ibooker_.book1D(
      "photonPt", "Photon transverse momentum; p_{T} (GeV)", 20, 0, 500);
  h_ht = ibooker_.book1D("ht", "Hadronic activity;H_{T} (GeV)", 20, 0, 2000);
  h_htTurnOn_num = ibooker_.book1D("pfHtTurnOn_num", "PF HT Turn On Numerator",
                                   20, 300, 800);
  h_htTurnOn_den = ibooker_.book1D("pfHtTurnOn_den",
                                   "PF HT Turn On Denominator", 20, 300, 800);
  h_photonTurnOn_num = ibooker_.book1D("photonTurnOn_num",
                                       "Photon Turn On Numerator", 20, 70, 130);
  h_photonTurnOn_den = ibooker_.book1D(
      "photonTurnOn_den", "Photon Turn On Denominator", 20, 70, 130);
