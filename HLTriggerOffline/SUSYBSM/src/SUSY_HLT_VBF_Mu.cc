...
SUSY_HLT_VBF_Mu::SUSY_HLT_VBF_Mu(const edm::ParameterSet &ps) {
  ...
  theCaloMETCollection_ = consumes<reco::CaloMETCollection>(
      ps.getParameter<edm::InputTag>("caloMETCollection"));
  ...
  triggerCaloMETFilter_ =
      ps.getParameter<edm::InputTag>("TriggerFilterCaloMET");
...
void SUSY_HLT_VBF_Mu::analyze(edm::Event const &e,
  ...
  size_t filterIndexCaloMET =
      triggerSummary->filterIndex(triggerCaloMETFilter_);
  ...
  for (reco::MuonCollection::const_iterator muon = MuonCollection->begin();
    ...
    for (size_t off_i = 0; off_i < ptMuon.size(); ++off_i) {
      if (reco::deltaR(muon->eta(), muon->phi(), etaMuon[off_i],
                       phiMuon[off_i]) < 0.5) {
  ...
  for (reco::PFJetCollection::const_iterator i_pfjet = pfJetCollection->begin();
  ...
  for (size_t i = 0; i < jetCol; i++) {
    ...
    if (pfJetCollection->at(i).pt() > ptThrJetTrig_ &&
        fabs(pfJetCollection->at(i).eta()) < etaThrJetTrig_) {

      for (size_t j = i; j < jetCol; j++) {

        if (i < j) {

          if (pfJetCollection->at(j).pt() > ptThrJetTrig_ &&
              fabs(pfJetCollection->at(j).eta()) < etaThrJetTrig_) {

            double tmpdetaOff = fabs(pfJetCollection->at(i).eta() -
                                     pfJetCollection->at(j).eta());
            double tmpoppositeOff =
                pfJetCollection->at(i).eta() * pfJetCollection->at(j).eta();
            if (tmpdetaOff > deltaetaVBFJets && tmpoppositeOff < 0) {
              TLorentzVector j1Off(
                  pfJetCollection->at(i).px(), pfJetCollection->at(i).py(),
                  pfJetCollection->at(i).pz(), pfJetCollection->at(i).energy());
              TLorentzVector j2Off(
                  pfJetCollection->at(j).px(), pfJetCollection->at(j).py(),
                  pfJetCollection->at(j).pz(), pfJetCollection->at(j).energy());
  ...
  if (indexOfMatchedMuon > -1 && (dijetOff > mjjOnlinethreshold) &&
      (pfMETCollection->begin()->et() > pfmetOnlinethreshold) &&
      (pfHT > htOnlinethreshold)) {
...
void SUSY_HLT_VBF_Mu::bookHistos(DQMStore::IBooker &ibooker_) {
  ...
  h_triggerCaloMet = ibooker_.book1D("h_triggerCaloMet",
                                     "Trigger Calo MET; GeV", 20, 0.0, 500.0);

  h_ht = ibooker_.book1D("h_ht", "h_ht", 30, 0.0, 1500.0);

  h_triggerMet =
      ibooker_.book1D("triggerMet", "Trigger MET; GeV", 20, 0.0, 500.0);
  h_triggerMetPhi =
  ...
  h_den_muonpt =
      ibooker_.book1D("h_den_muonpt", "h_den_muonpt", 50, 0.0, 500.0);
  h_num_muonpt =
      ibooker_.book1D("h_num_muonpt", "h_num_muonpt", 50, 0.0, 500.0);

  h_den_muoneta =
      ibooker_.book1D("h_den_muoneta", "h_den_muoneta", 20, -3.0, 3.0);
  h_num_muoneta =
      ibooker_.book1D("h_num_muoneta", "h_num_muoneta", 20, -3.0, 3.0);
