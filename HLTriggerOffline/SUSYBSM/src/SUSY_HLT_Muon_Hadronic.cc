...
SUSY_HLT_Muon_Hadronic::SUSY_HLT_Muon_Hadronic(const edm::ParameterSet &ps) {
  ...
  edm::LogInfo("SUSY_HLT_Muon_Hadronic")
      << "Constructor SUSY_HLT_Muon_Hadronic::SUSY_HLT_Muon_Hadronic "
      << std::endl;
  ...
  triggerPathAuxiliaryForMuon_ =
      ps.getParameter<std::string>("TriggerPathAuxiliaryForMuon");
...
SUSY_HLT_Muon_Hadronic::~SUSY_HLT_Muon_Hadronic() {
  edm::LogInfo("SUSY_HLT_Muon_Hadronic")
      << "Destructor SUSY_HLT_Muon_Hadronic::~SUSY_HLT_Muon_Hadronic "
      << std::endl;
}
...
void SUSY_HLT_Muon_Hadronic::dqmBeginRun(edm::Run const &run,
  ...
  if (!fHltConfig.init(run, e, HLTProcess_, changed)) {
    edm::LogError("SUSY_HLT_Muon_Hadronic")
...
void SUSY_HLT_Muon_Hadronic::analyze(edm::Event const &e,
  ...
  for (unsigned int hltIndex = 0; hltIndex < numTriggers; ++hltIndex) {
    if (trigNames.triggerName(hltIndex).find(triggerPath_) !=
      ...
      hasFired = true;
    if (trigNames.triggerName(hltIndex).find(triggerPathAuxiliaryForMuon_) !=
            std::string::npos &&
        hltresults->wasrun(hltIndex) && hltresults->accept(hltIndex))
  ...
  if (hasFiredAuxiliaryForMuonLeg || hasFiredAuxiliaryForHadronicLeg ||
    ...
    */
    ...
    for (std::vector<Lepton>::iterator offline_muon = offlineMuons.begin();
         ...
         offline_muon++) {
      for (std::vector<Lepton>::iterator online_muon = onlineMuons.begin();
           online_muon != onlineMuons.end(); online_muon++) {
        if (sqrt((offline_muon->phi - online_muon->phi) *
                     (offline_muon->phi - online_muon->phi) +
                 (offline_muon->eta - online_muon->eta) *
                     (offline_muon->eta - online_muon->eta)) < 0.5) {
    ...
    if ((hasFiredAuxiliaryForMuonLeg || !e.isRealData()) &&
        !offlineMuons.empty() && pfMETCollection->begin()->et() > METOffline_ &&
        pfHT > HTOffline_) {
    ...
    if ((hasFiredAuxiliaryForHadronicLeg || !e.isRealData()) &&
        indexOfMatchedMuon >= 0 &&
        offlineMuons.at(indexOfMatchedMuon).pt > ptMuonOffline_ &&
        pfHT > HTOffline_) {
      if (hasFired) {
    ...
    if ((hasFiredAuxiliaryForHadronicLeg || !e.isRealData()) &&
        indexOfMatchedMuon >= 0 &&
        offlineMuons.at(indexOfMatchedMuon).pt > ptMuonOffline_ &&
        pfMETCollection->begin()->et() > METOffline_) {
      if (hasFired) {
...
void SUSY_HLT_Muon_Hadronic::bookHistos(DQMStore::IBooker &ibooker_) {
  ...
  // online quantities
  h_triggerMuPt =
      ibooker_.book1D("triggerMuPt", "Trigger Muon Pt; GeV", 20, 0.0, 500.0);
  h_triggerMuEta =
  ...
  h_pfMetTurnOn_num = ibooker_.book1D(
      "pfMetTurnOn_num", "PF MET Turn On Numerator", 20, 0.0, 500.0);
  h_pfMetTurnOn_den = ibooker_.book1D(
      "pfMetTurnOn_den", "PF MET Turn OnDenominator", 20, 0.0, 500.0);
