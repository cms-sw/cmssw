...
SUSY_HLT_MuEle_Hadronic::SUSY_HLT_MuEle_Hadronic(const edm::ParameterSet &ps) {
  edm::LogInfo("SUSY_HLT_MuEle_Hadronic")
      << "Constructor SUSY_HLT_MuEle_Hadronic::SUSY_HLT_MuEle_Hadronic "
      << std::endl;
  ...
  triggerPathAuxiliaryForMuEle_ =
      ps.getParameter<std::string>("TriggerPathAuxiliaryForMuEle");
...
SUSY_HLT_MuEle_Hadronic::~SUSY_HLT_MuEle_Hadronic() {
  edm::LogInfo("SUSY_HLT_MuEle_Hadronic")
      << "Destructor SUSY_HLT_MuEle_Hadronic::~SUSY_HLT_MuEle_Hadronic "
      << std::endl;
}
...
void SUSY_HLT_MuEle_Hadronic::dqmBeginRun(edm::Run const &run,
  ...
  if (!fHltConfig.init(run, e, HLTProcess_, changed)) {
    edm::LogError("SUSY_HLT_MuEle_Hadronic")
...
void SUSY_HLT_MuEle_Hadronic::analyze(edm::Event const &e,
  ...
  // get online objects
  std::vector<float> ptMuon, etaMuon, phiMuon, ptElectron, etaElectron,
      phiElectron;
      ...
      if (foundObject.id() == trigger::TriggerElectron) { // It's an electron
        ...
        for (unsigned int x = 0; x < ptElectron.size(); x++) {
          if (fabs(ptElectron[x] - foundObject.pt()) < 0.01 ||
              fabs(etaElectron[x] - foundObject.eta()) < 0.001 ||
  ...
  if (!ptMuon.empty() && !ptElectron.empty()) {
    math::PtEtaPhiMLorentzVectorD *mu1 = new math::PtEtaPhiMLorentzVectorD(
        ptMuon[0], etaMuon[0], phiMuon[0], 0.106);
    math::PtEtaPhiMLorentzVectorD *ele2 = new math::PtEtaPhiMLorentzVectorD(
        ptElectron[0], etaElectron[0], phiElectron[0], 0.0005);
    (*mu1) += (*ele2);
  ...
  for (unsigned int hltIndex = 0; hltIndex < numTriggers; ++hltIndex) {
    if (trigNames.triggerName(hltIndex).find(triggerPath_) !=
      ...
      hasFired = true;
    if (trigNames.triggerName(hltIndex).find(triggerPathAuxiliaryForMuEle_) !=
            std::string::npos &&
        hltresults->wasrun(hltIndex) && hltresults->accept(hltIndex))
  ...
  if (hasFiredAuxiliaryForMuEleLeg || hasFiredAuxiliaryForHadronicLeg) {
    ...
    for (reco::MuonCollection::const_iterator muon = MuonCollection->begin();
         (muon != MuonCollection->end() && matchedCounter < 1); ++muon) {
      for (size_t off_i = 0; off_i < ptMuon.size(); ++off_i) {
        if (sqrt((muon->phi() - phiMuon[off_i]) *
    ...
    for (reco::GsfElectronCollection::const_iterator electron =
      ...
      for (size_t off_i = 0; off_i < ptElectron.size(); ++off_i) {
        if (sqrt((electron->phi() - phiElectron[off_i]) *
                     (electron->phi() - phiElectron[off_i]) +
    ...
    if (hasFiredAuxiliaryForMuEleLeg && !MuonCollection->empty() &&
        !ElectronCollection->empty()) {
      if (hasFired && indexOfMatchedMuEle[1] >= 0 &&
          indexOfMatchedMuEle[0] >= 0) { // check that ele is there too
        h_EleTurnOn_num->Fill(
            ElectronCollection->at(indexOfMatchedMuEle[1]).pt());
...
void SUSY_HLT_MuEle_Hadronic::bookHistos(DQMStore::IBooker &ibooker_) {
  ...
  // online quantities
  h_triggerMuPt =
      ibooker_.book1D("triggerMuPt", "Trigger Muon Pt; GeV", 50, 0.0, 500.0);
  h_triggerMuEta =
      ibooker_.book1D("triggerMuEta", "Trigger Muon Eta", 20, -3.0, 3.0);
  h_triggerMuPhi =
      ibooker_.book1D("triggerMuPhi", "Trigger Muon Phi", 20, -3.5, 3.5);
  ...
  h_triggerMuEleMass =
      ibooker_.book1D("triggerMuEleMass", "Trigger MuEle Mass", 202, -2, 200);

  // num and den hists to be divided in harvesting step to make turn on curves
  h_pfHTTurnOn_num = ibooker_.book1D(
      "pfHTTurnOn_num", "PF HT Turn On Numerator", 30, 0.0, 1500.0);
  h_pfHTTurnOn_den = ibooker_.book1D(
      "pfHTTurnOn_den", "PF HT Turn On Denominator", 30, 0.0, 1500.0);
  h_MuTurnOn_num =
      ibooker_.book1D("MuTurnOn_num", "Muon Turn On Numerator", 30, 0.0, 150);
  h_MuTurnOn_den = ibooker_.book1D("MuTurnOn_den", "Muon Turn On Denominator",
                                   30, 0.0, 150.0);
  h_EleTurnOn_num = ibooker_.book1D("EleTurnOn_num",
                                    "Electron Turn On Numerator", 30, 0.0, 150);
  h_EleTurnOn_den = ibooker_.book1D(
      "EleTurnOn_den", "Electron Turn On Denominator", 30, 0.0, 150.0);
