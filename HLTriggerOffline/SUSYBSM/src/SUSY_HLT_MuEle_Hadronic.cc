...
void SUSY_HLT_MuEle_Hadronic::analyze(edm::Event const& e, edm::EventSetup const& eSetup){
    ...
    for( unsigned int hltIndex=0; hltIndex<numTriggers; ++hltIndex ){
        ...
        if (trigNames.triggerName(hltIndex)==triggerPathAuxiliaryForMuEle_ && hltresults->wasrun(hltIndex) && hltresults->accept(hltIndex)) hasFiredAuxiliaryForMuEleLeg = true;
