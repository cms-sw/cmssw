...
void SUSY_HLT_InclusiveCaloHT::analyze(edm::Event const& e, edm::EventSetup const& eSetup){
  ...
  if(hasFiredAuxiliaryForHadronicLeg || !e.isRealData()) {
    ...
    if(hasFired){
      ...
      h_caloMetTurnOn_num -> Fill(CaloMETCollection->begin()->et());
      h_caloHTTurnOn_num -> Fill(caloHT);
    }
    //fill denominator histograms for all events, used for turn on curves
    h_caloMetTurnOn_den -> Fill(CaloMETCollection->begin()->et());
    h_caloHTTurnOn_den -> Fill(caloHT);
  }
}
...
void SUSY_HLT_InclusiveCaloHT::bookHistos(DQMStore::IBooker & ibooker_)
{
  ...
  h_caloMetTurnOn_num = ibooker_.book1D("caloMetTurnOn_num", "Calo MET Turn On Numerator", 20, 0.0, 500.0 );
  h_caloMetTurnOn_den = ibooker_.book1D("caloMetTurnOn_den", "Calo MET Turn OnDenominator", 20, 0.0, 500.0 );
  h_caloHTTurnOn_num = ibooker_.book1D("caloHTTurnOn_num", "Calo HT Turn On Numerator", 30, 0.0, 1500.0 );
  h_caloHTTurnOn_den = ibooker_.book1D("caloHTTurnOn_den", "Calo HT Turn On Denominator", 30, 0.0, 1500.0 );
