...
SUSY_HLT_DoubleMuon_Hadronic::SUSY_HLT_DoubleMuon_Hadronic(
  ...
  edm::LogInfo("SUSY_HLT_DoubleMuon_Hadronic")
      << "Constructor "
         "SUSY_HLT_DoubleMuon_Hadronic::SUSY_HLT_DoubleMuon_Hadronic "
      << std::endl;
...
void SUSY_HLT_DoubleMuon_Hadronic::dqmBeginRun(edm::Run const &run,
  ...
  if (!fHltConfig.init(run, e, HLTProcess_, changed)) {
    edm::LogError("SUSY_HLT_DoubleMuon_Hadronic")
...
void SUSY_HLT_DoubleMuon_Hadronic::analyze(edm::Event const &e,
  ...
  if (!(filterIndex >= triggerSummary->sizeFilters())) {
    ...
    if (ptMuon.size() >= 2) {
      ...
      math::PtEtaPhiMLorentzVectorD *mu2 = new math::PtEtaPhiMLorentzVectorD(
          ptMuon[1], etaMuon[1], phiMuon[1], 0.106);
      (*mu1) += (*mu2);
  ...
  if (hasFiredAuxiliaryForMuonLeg || hasFiredAuxiliaryForHadronicLeg) {
    ...
    for (reco::MuonCollection::const_iterator muon = MuonCollection->begin();
         (muon != MuonCollection->end() && matchedCounter < 2); ++muon) {
...
void SUSY_HLT_DoubleMuon_Hadronic::endRun(edm::Run const &run,
                                          edm::EventSetup const &eSetup) {
  edm::LogInfo("SUSY_HLT_DoubleMuon_Hadronic")
...
void SUSY_HLT_DoubleMuon_Hadronic::bookHistos(DQMStore::IBooker &ibooker_) {
  ...
  h_triggerDoubleMuMass = ibooker_.book1D(
      "triggerDoubleMuMass", "Trigger DoubleMuon Mass", 202, -2, 200);
