...
void SUSY_HLT_PhotonCaloHT::analyze(edm::Event const& e, edm::EventSetup const& eSetup){
  ...
  if ( !CaloMETCollection.isValid() ){
    edm::LogError ("SUSY_HLT_PhotonCaloHT") << "invalid met collection" << "\n";
    return;
  }
  //-------------------------------
  //--- Photon
  ...
  if(hasFiredAuxiliaryForHadronicLeg || !e.isRealData()) {
   ...
   float recoHt = CaloMETCollection->size() ? CaloMETCollection->begin()->et() : 0;

    if(hasFired){
      if( photonCollection->size() && recoHt >= htThrOffline_ ) h_photonTurnOn_num -> Fill( recoPhotonPt );
      if( CaloMETCollection->size() && recoPhotonPt >= ptThrOffline_ ) h_htTurnOn_num -> Fill( recoHt );
    }
    if( photonCollection->size() && recoHt >= htThrOffline_ ) h_photonTurnOn_den -> Fill( recoPhotonPt );
    if( CaloMETCollection->size() && recoPhotonPt >= ptThrOffline_ ) h_htTurnOn_den -> Fill( recoPhotonPt );
  }
