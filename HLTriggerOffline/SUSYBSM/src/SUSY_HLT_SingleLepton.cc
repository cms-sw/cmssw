...
bool IsGood(const reco::GsfElectron &el, const reco::Vertex::Point &pv_position,
  ...
  if (el.ecalEnergy() > 0.0 && std::isfinite(el.ecalEnergy())) {
    ooemoop =
        fabs(1.0 / el.ecalEnergy() - el.eSuperClusterOverP() / el.ecalEnergy());
  }
  const auto &iso = el.pfIsolationVariables();
  const float absiso = iso.sumChargedHadronPt +
                       std::max(0.0, iso.sumNeutralHadronEt + iso.sumPhotonEt -
                                         0.5 * iso.sumPUPt);
  ...
  if (convs.isValid()) {
    try {
      pass_conversion =
          !ConversionTools::hasMatchedConversion(el, *convs, bs_position);
    } catch (...) {
      edm::LogError("SUSY_HLT_SingleLepton")
...
SUSY_HLT_SingleLepton::SUSY_HLT_SingleLepton(const edm::ParameterSet &ps)
    : theElectronTag_(ps.getParameter<edm::InputTag>("electronCollection")),
      theElectronCollection_(
          consumes<reco::GsfElectronCollection>(theElectronTag_)),
      ...
      theVertexCollection_(
          consumes<reco::VertexCollection>(theVertexCollectionTag_)),
      theConversionCollectionTag_(
          ps.getParameter<edm::InputTag>("conversionCollection")),
      theConversionCollection_(
          consumes<reco::ConversionCollection>(theConversionCollectionTag_)),
      ...
      theHLTJetCollection_(
          consumes<reco::CaloJetCollection>(theHLTJetCollectionTag_)),
      theHLTJetTagCollectionTag_(ps.getParameter<edm::InputTag>("hltJetTags")),
      theHLTJetTagCollection_(
          consumes<reco::JetTagCollection>(theHLTJetTagCollectionTag_)),
      ...
      triggerPathAuxiliary_(
          ps.getParameter<std::string>("triggerPathAuxiliary")),
      triggerPathLeptonAuxiliary_(
          ps.getParameter<std::string>("triggerPathLeptonAuxiliary")),
      ...
      h_triggerLepPt_(nullptr), h_triggerLepEta_(nullptr),
      h_triggerLepPhi_(nullptr), h_HT_(nullptr), h_MET_(nullptr),
      h_maxCSV_(nullptr), h_leptonTurnOn_num_(nullptr),
      h_leptonTurnOn_den_(nullptr), h_pfHTTurnOn_num_(nullptr),
      h_pfHTTurnOn_den_(nullptr), h_pfMetTurnOn_num_(nullptr),
      h_pfMetTurnOn_den_(nullptr), h_CSVTurnOn_num_(nullptr),
      h_CSVTurnOn_den_(nullptr), h_btagTurnOn_num_(nullptr),
...
void SUSY_HLT_SingleLepton::dqmBeginRun(const edm::Run &run,
  ...
  if (!pathFound) {
    LogDebug("SUSY_HLT_SingleLepton")
        << "Path not found: " << triggerPath_ << '\n';
    return;
  }
...
void SUSY_HLT_SingleLepton::bookHistograms(DQMStore::IBooker &ibooker,
  ...
  // online quantities
  h_triggerLepPt_ = ibooker.book1D(
      "triggerLepPt", (";" + Lepton + " p_{T} [GeV];").c_str(), 20, 0.0, 500.0);
  h_triggerLepEta_ = ibooker.book1D(
      "triggerLepEta", (";" + Lepton + " #eta;").c_str(), 20, -3.0, 3.0);
  h_triggerLepPhi_ = ibooker.book1D(
      "triggerLepPhi", (";" + Lepton + " #phi;").c_str(), 20, -3.5, 3.5);
  ...
  if (!theHLTJetCollectionTag_.label().empty() &&
      !theHLTJetTagCollectionTag_.label().empty()) {
  ...
  h_leptonTurnOn_num_ = ibooker.book1D(
      "leptonTurnOn_num",
      ("Numerator;Offline " + lepton + " p_{T} [GeV];").c_str(), 30, 0.0, 150);
  h_leptonTurnOn_den_ = ibooker.book1D(
      "leptonTurnOn_den",
      ("Denominator;Offline " + lepton + " p_{T} [GeV];").c_str(), 30, 0.0,
      150.0);
  h_pfHTTurnOn_num_ = ibooker.book1D(
      "pfHTTurnOn_num", "Numerator;Offline H_{T} [GeV];", 30, 0.0, 1500.0);
  h_pfHTTurnOn_den_ = ibooker.book1D(
      "pfHTTurnOn_den", "Denominator;Offline H_{T} [GeV];", 30, 0.0, 1500.0);

  if (!theHLTMETTag_.label().empty()) {
    h_pfMetTurnOn_num_ = ibooker.book1D(
        "pfMetTurnOn_num", "Numerator;Offline MET [GeV];", 20, 0.0, 500.0);
    h_pfMetTurnOn_den_ = ibooker.book1D(
        "pfMetTurnOn_den", "Denominator;Offline MET [GeV];", 20, 0.0, 500.0);
  }

  if (!theHLTJetCollectionTag_.label().empty() &&
      !theHLTJetTagCollectionTag_.label().empty()) {
    h_CSVTurnOn_num_ =
        ibooker.book1D("CSVTurnOn_num",
                       "Numerator;Offline Max CSV Discriminant;", 20, 0.0, 1.0);
    h_CSVTurnOn_den_ = ibooker.book1D(
        "CSVTurnOn_den", "Denominator;Offline Max CSV Discriminant;", 20, 0.0,
        1.0);

    h_btagTurnOn_num_ = ibooker.book1D(
        "btagTurnOn_num", "Numerator;Offline CSV Requirement;", 13, -0.5, 12.5);
    h_btagTurnOn_den_ =
        ibooker.book1D("btagTurnOn_den",
                       "Denominator;Offline CSV Requirements;", 13, -0.5, 12.5);
...
void SUSY_HLT_SingleLepton::analyze(const edm::Event &e,
  ...
  if (!theConversionCollectionTag_.label().empty()) {
    ...
    if (!ConversionCollection.isValid()) {
      edm::LogWarning("SUSY_HLT_SingleLepton")
          << "Invalid ConversionCollection: "
          << theConversionCollectionTag_.label() << '\n';
    }
  }
  ...
  if (triggerSummary.isValid()) {
    ...
    if (!(filterIndex >= triggerSummary->sizeFilters())) {
      size_t ilep = 0,
             num_keys = triggerSummary->filterKeys(filterIndex).size();
  ...
  const float hlt_ht =
      ((HLTHT.isValid() && !HLTHT->empty()) ? HLTHT->front().sumEt() : -1.0);
  if (h_HT_)
    h_HT_->Fill(hlt_ht);
  const float hlt_met =
      ((HLTMET.isValid() && !HLTMET->empty()) ? HLTMET->front().pt() : -1.0);
  if (h_MET_)
    h_MET_->Fill(hlt_met);
  ...
  if (VertexCollection.isValid() &&
      !VertexCollection->empty()) { // for quality checks
    // Try to find a reco electron
    if (ElectronCollection.isValid() && ConversionCollection.isValid() &&
        BeamSpot.isValid()) {
  ...
  const bool lep_plateau =
      lep_max_pt > lep_pt_threshold_ || lep_pt_threshold_ < 0.0;
  ...
  if (hasFiredLeptonAuxiliary || triggerPathLeptonAuxiliary_.empty() ||
      !e.isRealData()) {
    // Fill histograms using highest pt reco lepton
    if (ht_plateau && met_plateau && csv_plateau &&
        (pfMET > metCut_ || metCut_ < 0.0) && (pfHT > htCut_ || htCut_ < 0.0)) {
