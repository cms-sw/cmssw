...
SUSY_HLT_MuonFakes::SUSY_HLT_MuonFakes(const edm::ParameterSet &ps) {
  edm::LogInfo("SUSY_HLT_MuonFakes")
...
void SUSY_HLT_MuonFakes::dqmBeginRun(edm::Run const &run,
  ...
  if (!fHltConfig.init(run, e, HLTProcess_, changed)) {
    edm::LogError("SUSY_HLT_MuonFakes")
...
void SUSY_HLT_MuonFakes::bookHistos(DQMStore::IBooker &ibooker_) {
  ...
  // online quantities
  h_triggerMuPt =
      ibooker_.book1D("triggerMuPt", "Trigger Mu Pt; GeV", 40, 0.0, 80.0);
  h_triggerMuEta =
      ibooker_.book1D("triggerMuEta", "Trigger Mu Eta", 20, -2.5, 2.5);
  h_triggerMuPhi =
      ibooker_.book1D("triggerMuPhi", "Trigger Mu Phi", 20, -3.5, 3.5);
