...
SUSY_HLT_DiJet_MET::SUSY_HLT_DiJet_MET(const edm::ParameterSet &ps) {
  edm::LogInfo("SUSY_HLT_DiJet_MET")
...
void SUSY_HLT_DiJet_MET::dqmBeginRun(edm::Run const &run,
  ...
  if (!fHltConfig.init(run, e, HLTProcess_, changed)) {
    edm::LogError("SUSY_HLT_DiJet_MET")
...
void SUSY_HLT_DiJet_MET::analyze(edm::Event const &e,
  ...
  if (!(jetFilterIndex >= triggerSummary->sizeFilters())) {
    const trigger::Keys &keys_jetfilter =
        triggerSummary->filterKeys(jetFilterIndex);
    for (size_t j = 0; j < keys_jetfilter.size(); ++j) {
      ...
      if (foundObject.pt() > ptThrJetTrig_ &&
          fabs(foundObject.eta()) < etaThrJetTrig_) {
  ...
  if (hasFiredAuxiliaryForHadronicLeg) {
    ...
    for (reco::PFJetCollection::const_iterator i_pfjet =
      ...
      if (i_pfjet->pt() > ptThrJet_ && fabs(i_pfjet->eta()) < etaThrJet_) {
        ...
        for (unsigned int itrigjet = 0; itrigjet < ptJet.size(); ++itrigjet) {
          if (itrigjet < 2 &&
              (sqrt((i_pfjet->phi() - phiJet.at(itrigjet)) *
                        (i_pfjet->phi() - phiJet.at(itrigjet)) +
                    (i_pfjet->eta() - etaJet.at(itrigjet)) *
                        (i_pfjet->eta() - etaJet.at(itrigjet))) < 0.4)) {
            nMatch++;
            if (nMatch == 1)
            ...
            if (hasFired) {
              ...
              if (offlineCentralJets == 2 &&
                  pfMETCollection->begin()->et() > metCut_)
    ...
    if (hasFired) {
      ...
      h_caloMetvsPFMet->Fill(pfMETCollection->begin()->et(),
                             caloMETCollection->begin()->et());

      if (jet1Index > -1 && jet2Index > -1) {
        h_pfJet1Jet2DPhi->Fill(
            fabs(reco::deltaPhi(pfJetCollection->at(jet1Index).phi(),
                                pfJetCollection->at(jet2Index).phi())));
      }
    }
...
void SUSY_HLT_DiJet_MET::bookHistos(DQMStore::IBooker &ibooker_) {
  ...
  h_pfJetPt = ibooker_.book1D(
      "pfJetPt", "PF Jet p_{T} (trigger-matched jets, |#eta| < 2.4); GeV", 20,
      0.0, 500.0);
  h_pfJetEta = ibooker_.book1D(
      "pfJetEta", "PF Jet #eta (trigger-matched jets, |#eta| < 2.4)", 20, -3.0,
      3.0);
  h_pfJetPhi = ibooker_.book1D(
      "pfJetPhi", "PF Jet #phi (trigger-matched jets, |#eta| < 2.4)", 20, -3.5,
      3.5);
  h_pfJet1Jet2DPhi = ibooker_.book1D(
      "pfJet1Jet2DPhi", "|#Delta#phi| between two leading trigger-matched jets",
      20, 0.0, 3.5);
  h_caloMetvsPFMet =
      ibooker_.book2D("caloMetvsPFMet", "Calo MET vs PF MET; GeV; GeV", 25, 0.0,
                      500.0, 25, 0.0, 500.0);
  ...
  h_triggerJetPt =
      ibooker_.book1D("triggerJetPt", "Trigger Jet p_{T}; GeV", 20, 0.0, 500.0);
  h_triggerJetEta =
  ...
  h_pfMetTurnOn_num =
      ibooker_.book1D("pfMetTurnOn_num", "PF MET", 20, 0.0, 500.0);
  h_pfMetTurnOn_den = ibooker_.book1D(
      "pfMetTurnOn_den", "PF MET Turn On Denominator", 20, 0.0, 500.0);
  h_pfJet2PtTurnOn_num = ibooker_.book1D(
      "pfJet2PtTurnOn_num", "PF Jet2 Pt (NCentralPFJets >= 2, PFMET > 250)", 20,
      0.0, 500.0);
  h_pfJet2PtTurnOn_den = ibooker_.book1D(
      "pfJet2PtTurnOn_den", "PF Jet2 Pt Turn On Denominator", 20, 0.0, 500.0);
