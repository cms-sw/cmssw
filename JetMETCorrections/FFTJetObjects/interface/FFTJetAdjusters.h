...
struct FFTSimpleScalingAdjuster : public AbsFFTJetAdjuster<MyJet, Adjustable> {
  ...
  void adjust(const MyJet& /* jet */,
              const Adjustable& in,
              const double* factors,
              const unsigned lenFactors,
              Adjustable* out) const override {
    if (lenFactors != 1U)
      throw cms::Exception("FFTJetBadConfig") << "In FFTSimpleScalingAdjuster::adjust: wrong number of "
...
struct FFTUncertaintyAdjuster : public AbsFFTJetAdjuster<MyJet, Adjustable> {
  ...
  void adjust(const MyJet& /* jet */,
              ...
              Adjustable* out) const override {
    if (lenFactors != 1U)
      throw cms::Exception("FFTJetBadConfig") << "In FFTUncertaintyAdjuster::adjust: wrong number of "
                                              << "scales (expected 1, got " << lenFactors << ")\n";
    assert(factors);
    assert(out);
    *out = in;
...
template <class MyJet, class Adjustable>
struct FFTScalingAdjusterWithUncertainty : public AbsFFTJetAdjuster<MyJet, Adjustable> {
  ...
  void adjust(const MyJet& /* jet */,
              ...
              Adjustable* out) const override {
    if (lenFactors != 2U)
      throw cms::Exception("FFTJetBadConfig") << "In FFTScalingAdjusterWithUncertainty::adjust: wrong "
