...
public:
  ...
  inline FFTJetCorrector(std::shared_ptr<const AbsAdjuster> adjuster,
      ...
      : adjuster_(adjuster), scalers_(scalers), level_(i_level), app_(a) {}

  inline void correct(const Jet& jet, const bool isMC, const Adjustable& in, Adjustable* out) const {
    if ((isMC && app_ == FFTJetCorrectorApp::DATA_ONLY) || (!isMC && app_ == FFTJetCorrectorApp::MC_ONLY))
