    ...
    for (jet = jets->begin(); jet != jets->end(); jet++) {
      ...
      for (unsigned i = 0; i < mCorrectors.size(); ++i) {
        if (!(mCorrectors[i]->vectorialCorrection())) {
          ...
          if (mVerbose)
            std::cout << "JetCorrectionProducer::produce-> Corrector # " << i << ", correction factor: " << scale
                      << std::endl;
          ...
          double scale = mCorrectors[i]->correction(*referenceJet, jetRef, fEvent, fSetup, corrected);
          if (mVerbose)
            std::cout << "JetCorrectionProducer::produce-> Corrector # " << i << ", correction factor: " << scale
                      << std::endl;
