...
namespace {
  ...
  public:
    ...
    double correction(const reco::Jet& fJet, const edm::RefToBase<reco::Jet>& fJetRef) const override;
  ...
  double ChainedJetCorrectorImpl::correction(const reco::Jet& fJet, const edm::RefToBase<reco::Jet>& fJetRef) const {
    std::unique_ptr<reco::Jet> jet(dynamic_cast<reco::Jet*>(fJet.clone()));
    double result = 1;
    for (auto cor : jetCorrectors_) {
...
void ChainedJetCorrectorProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  using namespace edm;
  ...
  std::unique_ptr<reco::JetCorrector> pCorr(new reco::JetCorrector(
      std::unique_ptr<reco::JetCorrectorImpl>(new ChainedJetCorrectorImpl(std::move(correctors)))));
