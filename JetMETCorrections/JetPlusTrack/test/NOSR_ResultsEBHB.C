...
void Draw()
{
   ...
   hprH5->SetMaximum(0.6);
   hprH5->SetMinimum(-0.1);
   hprH5->Draw();
   ...
   TLatex *t = new TLatex();
   t->SetTextSize(0.042);
   t->DrawLatex(1.5,0.50,"CMSSW_1_6_12, |#eta|< 1.3");
   t->DrawLatex(1.5,0.45,"no ZSP in HCAL");

   c2->SaveAs("HBmatrix.gif");
   ...
   TLatex *t = new TLatex();
   t->SetTextSize(0.042);
   t->DrawLatex(-1.8,0.14,"CMSSW_1_6_12");
   t->DrawLatex(-1.8,0.12,"no ZSP in HCAL");
   t->DrawLatex(-1.8,0.10,"|#eta|< 1.3");
   t->DrawLatex(-1.8,0.08,"1 < p_{T}^{trk} < 2 GeV");
   c3->SaveAs("hHB_1_2GeV.gif");
