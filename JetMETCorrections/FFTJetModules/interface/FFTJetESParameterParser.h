...
std::shared_ptr<AbsFFTJetScaleCalculator<Jet, Adjustable> > parseFFTJetScaleCalculator(const edm::ParameterSet& ps,
  ...
  if (!mapper_type.compare("FFTEtaLogPtConeRadiusMapper"))
    ...
    const edm::ParameterSet& subclass = ps.getParameter<edm::ParameterSet>("Subclass");
    AbsFFTSpecificScaleCalculator* p = parseFFTSpecificScaleCalculator(subclass, f->description());
    return Result(new FFTSpecificScaleCalculator<Jet, Adjustable>(f, p));
  }

  else
    throw cms::Exception("FFTJetBadConfig")
        << "In parseFFTJetScaleCalculator: unknown mapper type \"" << mapper_type << '"' << std::endl;
}
...
Corrector parseFFTJetCorrector(const edm::ParameterSet& ps, gs::StringArchive& ar, const bool verbose) {
  ...
  // "scalers" is a VPSet
  const std::vector<edm::ParameterSet>& scalers = ps.getParameter<std::vector<edm::ParameterSet> >("scalers");
