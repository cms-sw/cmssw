...
inline bool CommandLine::getValue<bool>(const std::string& name) {
  ...
  if (it != _options.end()) {
    ...
    if (val_as_string == "true")
      return true;
    if (val_as_string == "false")
      return false;
    int val_as_int;
    std::stringstream ss;
    ss << val_as_string;
      ...
      do {
        ...
        if (tmp.length() == length) {
          std::cout << std::setiosflags(std::ios::left) << std::setw(22) << it->first
                    ...
                    << std::setw(35) << tmp.substr(0, pos) << std::resetiosflags(std::ios::right) << std::endl;
        } else {
          std::cout << std::setiosflags(std::ios::right) << std::setw(60) << tmp.substr(0, pos)
                    << std::resetiosflags(std::ios::right) << std::endl;
        }
        tmp.erase(0, pos + 1);
      } while (pos != std::string::npos);
    } else {
      std::cout << std::setiosflags(std::ios::left) << std::setw(22) << it->first << std::resetiosflags(std::ios::left)
                << std::setw(3) << "=" << std::setiosflags(std::ios::right) << std::setw(35) << it->second.first
                << std::resetiosflags(std::ios::right) << std::endl;
    }
  }
  std::cout << "------------------------------------------------------------" << std::endl;
  ...
  while (!ss.eof()) {
    if (token == "=") {
      if (!key.empty() && !value.empty())
        _options[key] = std::make_pair(value, false);
      key = last_token;
      last_token = "";
      value = "";
