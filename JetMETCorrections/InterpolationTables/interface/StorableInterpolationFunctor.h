...
namespace npstat {
  ...
  public:
    ...
    template <class Num2>
    inline StorableInterpolationFunctor(const LinInterpolatedTableND<Num2, Axis>& table)
        : StorableMultivariateFunctor(), table_(table) {}

    template <class Num2>
    inline StorableInterpolationFunctor(const LinInterpolatedTableND<Num2, Axis>& table, const std::string& descr)
        : StorableMultivariateFunctor(descr), table_(table) {}
    //@}

    /** Converting copy constructor */
    template <class Num2, class Conv2>
    inline StorableInterpolationFunctor(const StorableInterpolationFunctor<Num2, Axis, Conv2>& tab)
    ...
    /**
        ...
        */
    inline StorableInterpolationFunctor(const std::vector<Axis>& axes,
    ...
    inline StorableInterpolationFunctor(const Axis& xAxis, bool leftX, bool rightX, const char* functionLabel = nullptr)
        : StorableMultivariateFunctor(), table_(xAxis, leftX, rightX, functionLabel) {}

    inline StorableInterpolationFunctor(const Axis& xAxis,
                                        bool leftX,
                                        bool rightX,
                                        const Axis& yAxis,
                                        bool leftY,
    ...
    inline StorableInterpolationFunctor(const Axis& xAxis,
        ...
        : StorableMultivariateFunctor(),
          table_(xAxis, leftX, rightX, yAxis, leftY, rightY, zAxis, leftZ, rightZ, functionLabel) {}
    ...
    inline StorableInterpolationFunctor(const Axis& xAxis,
        ...
        : StorableMultivariateFunctor(),
          table_(
              xAxis, leftX, rightX, yAxis, leftY, rightY, zAxis, leftZ, rightZ, tAxis, leftT, rightT, functionLabel) {}
    ...
    double operator()(const double* point, unsigned dim) const override { return conv_(table_(point, dim)); }
...
namespace npstat {
  ...
  const char* StorableInterpolationFunctor<Numeric, Axis, Converter>::classname() {
    static const std::string myClass(gs::template_class_name<Numeric, Axis>("npstat::StorableInterpolationFunctor"));
    return myClass.c_str();
  }

  template <typename Numeric, class Axis, class Converter>
  bool StorableInterpolationFunctor<Numeric, Axis, Converter>::write(std::ostream& of) const {
    gs::write_pod(of, this->description());
    return table_.classId().write(of) && table_.write(of);
  }

  template <typename Numeric, class Axis, class Converter>
  StorableInterpolationFunctor<Numeric, Axis, Converter>* StorableInterpolationFunctor<Numeric, Axis, Converter>::read(
      const gs::ClassId& id, std::istream& in) {
    static const gs::ClassId current(gs::ClassId::makeId<StorableInterpolationFunctor<Numeric, Axis> >());
