...
namespace npstat {
  ...
  struct ComplexComparesAbs<std::complex<T> > {
    inline static bool less(const std::complex<T>& l, const std::complex<T>& r) { return std::abs(l) < std::abs(r); }

    inline static bool more(const std::complex<T>& l, const std::complex<T>& r) { return std::abs(l) > std::abs(l); }
  };
}  // namespace npstat
