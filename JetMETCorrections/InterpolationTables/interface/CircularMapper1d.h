...
namespace npstat {
  ...
  public:
    ...
    inline CircularMapper1d(const double ca, const double cb, const double cperiod)
        : a_(ca), b_(cb), period_(std::abs(cperiod)) {
      check();
    }

    inline CircularMapper1d(const LinearMapper1d& mapper, const double cperiod)
        : a_(mapper.a()), b_(mapper.b()), period_(std::abs(cperiod)) {
      check();
    }
    ...
    inline LinearMapper1d linearMapper() const { return LinearMapper1d(a_, b_); }
