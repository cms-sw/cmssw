...
namespace npstat {
  UniformAxis::UniformAxis(const unsigned nCoords, const double min, const double max, const char* label)
  ...
  double UniformAxis::coordinate(const unsigned i) const {
    if (i >= npt_)
      throw npstat::NpstatOutOfRange("In npstat::UniformAxis::coordinate: index out of range");
    if (i == npt_ - 1U)
      return max_;
    else
      return min_ + bw_ * i;
  ...
  bool UniformAxis::operator==(const UniformAxis& r) const {
    return min_ == r.min_ && max_ == r.max_ && label_ == r.label_ && npt_ == r.npt_;
  }
