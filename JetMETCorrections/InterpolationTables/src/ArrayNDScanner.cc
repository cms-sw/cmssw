...
namespace npstat {
  void ArrayNDScanner::initialize(const unsigned* shape, const unsigned lenShape) {
    // Check argument validity
    if (lenShape == 0U || lenShape >= CHAR_BIT * sizeof(unsigned long))
      throw npstat::NpstatInvalidArgument("In npstat::ArrayNDScanner::initialize: invalid scan shape");
    assert(shape);
  ...
  void ArrayNDScanner::getIndex(unsigned* ix, const unsigned indexBufferLen) const {
    if (indexBufferLen < dim_)
      throw npstat::NpstatInvalidArgument(
          "In npstat::ArrayNDScanner::getIndex: "
          "insufficient length of the output buffer");
    if (state_ >= maxState_)
      throw npstat::NpstatRuntimeError("In npstat::ArrayNDScanner::getIndex: invalid scanner state");
    assert(ix);
