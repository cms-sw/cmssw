...
L6SLBCorrectorImplMaker::L6SLBCorrectorImplMaker(edm::ParameterSet const& fConfig, edm::ConsumesCollector fCollector)
    : JetCorrectorImplMakerBase(fConfig),
      ...
      muonToken_(fCollector.consumes<std::vector<reco::SoftLeptonTagInfo>>(
          fConfig.getParameter<edm::InputTag>("srcBTagInfoMuon"))),
...
std::unique_ptr<reco::JetCorrectorImpl> L6SLBCorrectorImplMaker::make(edm::Event const& fEvent,
  ...
  auto calculator = getCalculator(fSetup, [](std::string const&) {});
  return std::unique_ptr<reco::JetCorrectorImpl>(new L6SLBCorrectorImpl(calculator, muonProd, elecProd, addMuonToJet_));
}
...
//______________________________________________________________________________
double L6SLBCorrectorImpl::correction(const reco::Jet& fJet, const edm::RefToBase<reco::Jet>& refToRawJet) const {
  ...
  const reco::SoftLeptonTagInfo& sltMuon = (*bTagInfoMuon_)[getBTagInfoIndex(refToRawJet, *bTagInfoMuon_)];
  if (sltMuon.leptons() > 0) {
    ...
    const reco::SoftLeptonTagInfo& sltElec = (*bTagInfoElec_)[getBTagInfoIndex(refToRawJet, *bTagInfoElec_)];
