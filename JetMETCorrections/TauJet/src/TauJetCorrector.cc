...
double TauJetCorrector::ParametrizationTauJet::value(double et, double eta) const {
  ...
  switch (type) {
    ...
    default:
      edm::LogError("TauJetCorrector: Error: unknown parametrization type ")
          << type << " in TauJetCorrector. No correction applied" << endl;
...
void TauJetCorrector::setParameters(std::string aCalibrationType, int itype) {
  ...
  if ((!pset.valid()) && (aCalibrationType != "no")) {
    edm::LogError("TauJetCorrector:Jet Corrections not found ")
        << aCalibrationType
  ...
  for (int ieta = 0; ieta < pset.neta(); ieta++) {
    ...
    if (pset.type(ieta) == -1) {
      etaboundx[mtype - 1] = pset.eta(ieta);
    }
  }
  ...
  for (int ieta = 0; ieta < pset.neta(); ieta++) {
    ...
    if (pset.type(ieta) == -1)
      continue;
    if (ieta < iq + 1) {
      parametrization[pset.eta(ieta)] =
          new ParametrizationTauJet(pset.type(ieta), (*pq.find(ieta)).second, (*etaboundx.find(0)).second);
    ...
    if (ieta > iq && ieta < iq + ig + 2) {
      mynum = ieta - iq - 1;
      parametrization[pset.eta(ieta)] =
          new ParametrizationTauJet(pset.type(ieta), (*pg.find(mynum)).second, (*etaboundx.find(1)).second);
    ...
    if (ieta > iq + ig + 1) {
      ...
      parametrization[pset.eta(ieta)] =
          new ParametrizationTauJet(pset.type(ieta), (*pqcd.find(mynum)).second, (*etaboundx.find(2)).second);
...
double TauJetCorrector::correction(const reco::Jet& fJet) const { return correction(fJet.p4()); }
