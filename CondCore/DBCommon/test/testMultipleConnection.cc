...
int main(){
  ...
  conHandler.registerConnection("mysqlite1","sqlite_file:mydata.db",0);
  conHandler.registerConnection("mysqlite2","sqlite_file:miodati.db",0);
  session->open();
  ...
  cond::TypedRef<testCondObj> myref(myconnection->poolTransaction(),myobj);
  ...
  cond::CoralTransaction& coralTransaction2=myconnection2->coralTransaction();
  coralTransaction2.start(false);
