...
class connection():
	...
	def __init__(self, connection_data):
		...
		"""if type(connection_data) == str and connection_data[0:9] == "oracle://":
			db_name = connection_data[10:].split("/")[0]
			schema = connection_data[10:].split("/")[1]
			connection_data = {}
	...
	def setup(self):
		...
		tmp_models_dict = {}
		for key in self.models.__dict__:
			try:
				if self.models.__dict__[key].__class__ == sqlalchemy.ext.declarative.api.DeclarativeMeta\
				   and str(self.models.__dict__[key].__name__) != "Base":

					self.models.__dict__[key].__table__.schema = self.schema

					self.models.__dict__[key].session = self.session
					self.models.__dict__[key].authentication = self.netrc_authenticators
					self.models.__dict__[key].secrets = self.secrets
					tmp_models_dict[key.lower()] = self.models.__dict__[key]
