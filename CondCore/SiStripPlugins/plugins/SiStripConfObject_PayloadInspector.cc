...
namespace {
  ...
  public:
    SiStripConfObjectTest() : cond::payloadInspector::Histogram1D<SiStripConfObject>("SiStrip Configuration Object test",
  ...
  public:
    ...
    bool fill( const std::vector<std::tuple<cond::Time_t,cond::Hash> >& iovs ) override{
      ...
      auto h_Config = std::unique_ptr<TH1F>(new TH1F("ConfigParamter",";;configuration value",configsize_,0.,configsize_));
	...
	latex.DrawLatex(0.1,0.92,"Parameter");     latex.DrawLatex(0.6,0.92,"Value");
	...
	for( ; it != payload->parameters.end(); ++it ) {
	  ...
	  latex.DrawLatex(0.1,y_loc,(it->first).c_str());     latex.DrawLatex(0.6,y_loc,(it->second).c_str());
