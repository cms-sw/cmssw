 ...
 ******************************************/
  class EcalClusterLocalContCorrParametersPlot
      : public cond::payloadInspector::PlotImage<EcalClusterLocalContCorrParameters> {
  public:
    EcalClusterLocalContCorrParametersPlot()
        : cond::payloadInspector::PlotImage<EcalClusterLocalContCorrParameters>(
              "Ecal Cluster Local Cont Corr Parameters - map ") {
      setSingleIov(true);
    }

    bool fill(const std::vector<std::tuple<cond::Time_t, cond::Hash> >& iovs) override {
      auto iov = iovs.front();  //get reference to 1st element in the vector iovs
      std::shared_ptr<EcalClusterLocalContCorrParameters> payload =
      ...
      if (payload.get()) {  //payload is an iov retrieved from payload using hash.
        ...
        fillFunctionParamsValues(
            align, (*payload).params(), "Ecal Cluster Local Cont Corr Parameters", gridRows, NbColumns);
