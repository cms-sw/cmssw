...
namespace {
/*****************************************
 2d plot of Ecal Cluster Local Cont Corr Parameters of 1 IOV
 ******************************************/
class EcalClusterLocalContCorrParametersPlot: public cond::payloadInspector::PlotImage<EcalClusterLocalContCorrParameters> {
public:
  EcalClusterLocalContCorrParametersPlot() :
      cond::payloadInspector::PlotImage<EcalClusterLocalContCorrParameters>("Ecal Cluster Local Cont Corr Parameters - map ") {
    setSingleIov(true);
  }
  ...
  bool fill(const std::vector<std::tuple<cond::Time_t, cond::Hash> >& iovs)override {
    ...
    std::shared_ptr < EcalClusterLocalContCorrParameters > payload = fetchPayload(std::get < 1 > (iov)); //std::get<1>(iov) refers to the Hash in the tuple iov
    ...
    if (payload.get()) { //payload is an iov retrieved from payload using hash.
      ...
      fillFunctionParamsValues(align, (*payload).params(), "Ecal Cluster Local Cont Corr Parameters",
        gridRows, NbColumns);
    ...
    t1.SetTextColor(2);
    t1.DrawLatex(0.5, 0.96,Form("Ecal Cluster Local Cont Corr Parameters, IOV %i", run));
