	...
	public:
		...
		bool fill(const std::vector<std::tuple<cond::Time_t, cond::Hash> >& iovs) override {
			...
			if(payload.get()){
			  ...
			  align =new TH2F("TPGPhysicsConstant","mapKey   EtSat         ttf_threshold_Low         ttf_threshold_High         FG_lowThreshold         FG_highThreshold         FG_lowRatio         FG_highRatio",
			    	8, 0, 8, NbRows, 0, NbRows);

			  double row = NbRows - 0.5;
				for (std::map<uint32_t, EcalTPGPhysicsConst::Item>::const_iterator it = map.begin();
				 it != map.end();it++) {
...
PAYLOAD_INSPECTOR_MODULE(EcalTPGPhysicsConst) {
  PAYLOAD_INSPECTOR_CLASS(EcalTPGPhysicsConstPlot);
}