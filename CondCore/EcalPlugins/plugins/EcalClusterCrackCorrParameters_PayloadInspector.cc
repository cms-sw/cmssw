...
namespace {
/*****************************************
 2d plot of Ecal Cluster Crack Corr Parameters of 1 IOV
 ******************************************/
class EcalClusterCrackCorrParametersPlot: public cond::payloadInspector::PlotImage<EcalClusterCrackCorrParameters> {
public:
  EcalClusterCrackCorrParametersPlot() :
      cond::payloadInspector::PlotImage<EcalClusterCrackCorrParameters>("Ecal Cluster Crack Corr Parameters - map ") {
    setSingleIov(true);
  }
  ...
  bool fill(const std::vector<std::tuple<cond::Time_t, cond::Hash> >& iovs)override {
    ...
    std::shared_ptr < EcalClusterCrackCorrParameters > payload = fetchPayload(std::get < 1 > (iov)); //std::get<1>(iov) refers to the Hash in the tuple iov
    ...
    if (payload.get()) { //payload is an iov retrieved from payload using hash.
      ...
      fillFunctionParamsValues(align, (*payload).params(), "Ecal Cluster Crack Corr Parameters",
        gridRows, NbColumns);
    ...
    t1.SetTextSize(0.05);
    t1.SetTextColor(2);
    t1.DrawLatex(0.5, 0.96,Form("Ecal Cluster Crack Corr Parameters, IOV %i", run));
...
PAYLOAD_INSPECTOR_MODULE(EcalClusterCrackCorrParameters) {
  PAYLOAD_INSPECTOR_CLASS(EcalClusterCrackCorrParametersPlot);
}
