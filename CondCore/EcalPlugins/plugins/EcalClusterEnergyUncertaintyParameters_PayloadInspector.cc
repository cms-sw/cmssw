 ...
 ******************************************/
  class EcalClusterEnergyUncertaintyParametersPlot
      : public cond::payloadInspector::PlotImage<EcalClusterEnergyUncertaintyParameters> {
  public:
    EcalClusterEnergyUncertaintyParametersPlot()
        : cond::payloadInspector::PlotImage<EcalClusterEnergyUncertaintyParameters>(
              "Ecal Cluster Energy Uncertainty Parameters - map ") {
      setSingleIov(true);
    }

    bool fill(const std::vector<std::tuple<cond::Time_t, cond::Hash> >& iovs) override {
      auto iov = iovs.front();  //get reference to 1st element in the vector iovs
      std::shared_ptr<EcalClusterEnergyUncertaintyParameters> payload =
      ...
      if (payload.get()) {  //payload is an iov retrieved from payload using hash.
        ...
        fillFunctionParamsValues(
            align, (*payload).params(), "Ecal Cluster Energy Uncertainty Parameters", gridRows, NbColumns);
