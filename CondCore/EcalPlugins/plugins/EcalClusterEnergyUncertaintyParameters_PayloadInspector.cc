...
namespace {
/*****************************************
 2d plot of Ecal Cluster Energy Uncertainty Parameters of 1 IOV
 ******************************************/
class EcalClusterEnergyUncertaintyParametersPlot: public cond::payloadInspector::PlotImage<EcalClusterEnergyUncertaintyParameters> {
public:
  EcalClusterEnergyUncertaintyParametersPlot() :
      cond::payloadInspector::PlotImage<EcalClusterEnergyUncertaintyParameters>("Ecal Cluster Energy Uncertainty Parameters - map ") {
    setSingleIov(true);
  }

  bool fill(const std::vector<std::tuple<cond::Time_t, cond::Hash> >& iovs)override {
    auto iov = iovs.front(); //get reference to 1st element in the vector iovs
    std::shared_ptr < EcalClusterEnergyUncertaintyParameters > payload = fetchPayload(std::get < 1 > (iov)); //std::get<1>(iov) refers to the Hash in the tuple iov
    ...
    if (payload.get()) { //payload is an iov retrieved from payload using hash.
      ...
      fillFunctionParamsValues(align, (*payload).params(), "Ecal Cluster Energy Uncertainty Parameters",
        gridRows, NbColumns);
    ...
    t1.SetTextSize(0.04);
    t1.SetTextColor(2);
    t1.DrawLatex(0.5, 0.96,Form("Ecal Cluster Energy Uncertainty Parameters, IOV %i", run));
