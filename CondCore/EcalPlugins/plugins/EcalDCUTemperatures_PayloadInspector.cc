 ...
 *******************************************************/
  ...
  public:
    EcalDCUTemperaturesPlot() : cond::payloadInspector::PlotImage<EcalDCUTemperatures>("Ecal DCU Temperatures - map ") {
      setSingleIov(true);
    }
    ...
    bool fill(const std::vector<std::tuple<cond::Time_t, cond::Hash> >& iovs) override {
      ...
      if (payload.get()) {
        ...
        for (EcalDCUTemperatures::EcalDCUTemperatureMap::const_iterator it = map.begin(); it != map.end(); it++) {
...
PAYLOAD_INSPECTOR_MODULE(EcalDCUTemperatures) { PAYLOAD_INSPECTOR_CLASS(EcalDCUTemperaturesPlot); }