...
public:
  ...
  bool fill(const std::vector<std::tuple<cond::Time_t, cond::Hash> >& iovs)override {
    ...
    std::shared_ptr < EcalDAQTowerStatus > payload = fetchPayload(std::get < 1 > (iov)); //std::get<1>(iov) refers to the Hash in the tuple iov
    ...
    if (payload.get()) { //payload is an iov retrieved from payload using hash.
      ...
      align =new TH2F("Ecal DAQ Tower Status Summary","EB/EE      bit 0 -> towers excluded from the DAQ",NbColumns, 0, NbColumns, NbRows, 0, NbRows);
...
PAYLOAD_INSPECTOR_MODULE( EcalDAQTowerStatus){
  PAYLOAD_INSPECTOR_CLASS( EcalDAQTowerStatusSummaryPlot);
}
