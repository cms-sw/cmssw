...
#include "CondTools/Ecal/interface/EcalHFNoiseXMLTranslator.h"
...
namespace cond {
	...
	namespace ecalnoise {
		enum Quantity { rms_x12=1, rms_x6=2, rms_x1=3 };
		enum How { singleChannel, bySuperModule, all};

		float average(EcalHFNoise const & peds, Quantity q) {
			return std::accumulate(
				boost::make_transform_iterator(peds.barrelItems().begin(),bind(&EcalHFChannelNoise::mean,_1,q)),
				boost::make_transform_iterator(peds.barrelItems().end(),bind(&EcalHFChannelNoise::mean,_1,q)),
				0.)/float(peds.barrelItems().size());
		}

		void extractAverage(EcalHFNoise const & peds, Quantity q, std::vector<int> const &,  std::vector<float> & result) {
			result.resize(1);
			result[0] = average(peds,q);
		}

		void extractSuperModules(EcalHFNoise const & peds, Quantity q, std::vector<int> const & which,  std::vector<float> & result) {
			// bho...
		}

		void extractSingleChannel(EcalHFNoise const & peds, Quantity q, std::vector<int> const & which,  std::vector<float> & result) {
		...
		typedef boost::function<void(EcalHFNoise const & peds, Quantity q, std::vector<int> const & which,  std::vector<float> & result)> PedExtractor;
	}

	template<>
	struct ExtractWhat<EcalHFNoise> {
	...
	template<>
	class ValueExtractor<EcalHFNoise>: public  BaseValueExtractor<EcalHFNoise> {
	public:
	...
	template<>
	std::string
		PayLoadInspector<EcalHFNoise>::dump() const {
			std::stringstream ss;
			EcalCondHeader header;
			ss<<EcalHFNoiseXMLTranslator::dumpXML(header,object());
			return ss.str();
	}  // dump


	class EcalHFNoiseHelper: public EcalPyWrapperHelper<EcalPedestal>{
	public:
		EcalHFNoiseHelper():EcalPyWrapperHelper<EcalObject>(3){}
	protected:
		...
		type_vValues getValues( const std::vector<EcalPedestal> & vItems)
		{
			...
			vValues[0].first = "rms_x12";
			vValues[1].first = "rms_x6";
			vValues[2].first = "rms_x1";
			vValues[0].second = .0;
			vValues[1].second = .0;
			vValues[2].second = .0;
			...
			for(std::vector<EcalPedestal>::const_iterator iItems = vItems.begin(); iItems != vItems.end(); ++iItems){
				vValues[0].second += iItems->rms(1);
				vValues[1].second += iItems->rms(2);
				vValues[2].second += iItems->rms(3);
	...
	template<>
	std::string PayLoadInspector<EcalHFNoise>::summary() const {
		std::stringstream ss;
		EcalHFNoiseHelper helper;
	...
	template<>
	std::string PayLoadInspector<EcalHFNoise>::plot(std::string const & filename,
		std::string const &, 
...
namespace condPython {
	template<>
	void defineWhat<EcalHFNoise>() {
		using namespace boost::python;
		enum_<cond::ecalped::Quantity>("Quantity")
			.value("mean_x12",cond::ecalped::rms_x12)
			.value("mean_x6",  cond::ecalped::rms_x6)
			.value("mean_x3", cond::ecalped::rms_x1)
			;
		...
		typedef cond::ExtractWhat<EcalHFNoise> What;
...
PYTHON_WRAPPER(EcalHFNoise,EcalHFNoise);
