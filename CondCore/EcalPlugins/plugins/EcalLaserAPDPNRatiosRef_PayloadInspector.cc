...
namespace {
  ...
  *******************************************************/
  ...
  public:
    EcalLaserAPDPNRatiosRefEBMap() : cond::payloadInspector::Histogram2D<EcalLaserAPDPNRatiosRef>("ECAL Barrel Laser APDPN Ratios Ref- map ",
  ...
  *******************************************************/
  ...
  public:
    EcalLaserAPDPNRatiosRefEEMap() : cond::payloadInspector::Histogram2D<EcalLaserAPDPNRatiosRef>( "ECAL Endcap Laser APDPN Ratios Ref- map ",
  ...
  *************************************************/
  ...
  public:
    EcalLaserAPDPNRatiosRefPlot() : cond::payloadInspector::PlotImage<EcalLaserAPDPNRatiosRef>("Ecal Laser APDPN Ratios Ref - map ") {
      setSingleIov( true );
    }

    bool fill( const std::vector<std::tuple<cond::Time_t,cond::Hash> >& iovs ) override{
      TH2F* barrel = new TH2F("EB", "Laser APDPN Ratios Ref EB", MAX_IPHI, 0, MAX_IPHI, 2 * MAX_IETA, -MAX_IETA, MAX_IETA);
      TH2F* endc_p = new TH2F("EE+", "Laser APDPN Ratios Ref EE+", IX_MAX, IX_MIN, IX_MAX + 1, IY_MAX, IY_MIN, IY_MAX + 1);
      TH2F* endc_m = new TH2F("EE-", "Laser APDPN Ratios Ref EE-", IX_MAX, IX_MIN, IX_MAX + 1, IY_MAX, IY_MIN, IY_MAX + 1);
  ...
  *****************************************************************/
  ...
  public:
    EcalLaserAPDPNRatiosRefDiff() : cond::payloadInspector::PlotImage<EcalLaserAPDPNRatiosRef>("Ecal Laser APDPN Ratios Ref difference ") {
      setSingleIov(false);
    }
...
class EcalLaserAPDPNRatiosRefSummaryPlot: public cond::payloadInspector::PlotImage<EcalLaserAPDPNRatiosRef>{
  ...
  bool fill(const std::vector<std::tuple<cond::Time_t, cond::Hash> >& iovs)override {
    ...
    if(payload.get()){
      ...
      fillTableWithSummary(align, "Ecal Laser APDPN Ratios Ref", mean_x_EB, rms_EB, num_x_EB, mean_x_EE, rms_EE, num_x_EE);
