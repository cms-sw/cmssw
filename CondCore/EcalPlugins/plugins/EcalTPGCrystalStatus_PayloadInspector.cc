...
public:
  ...
  bool fill(const std::vector<std::tuple<cond::Time_t, cond::Hash> >& iovs)override {
    ...
    std::shared_ptr < EcalTPGCrystalStatus > payload = fetchPayload(std::get < 1 > (iov)); //std::get<1>(iov) refers to the Hash in the tuple iov
    ...
    if (payload.get()) { //payload is an iov retrieved from payload using hash.
      ...
      align =new TH2F("Ecal TPGCrystal Status Error Summary","EB/EE-/EE+            ErrorCount            Total Number",NbColumns, 0, NbColumns, NbRows, 0, NbRows);
      ...
      getBarrelErrorSummary(payload->barrelItems(),ebErrorCount);
      getEndCapErrorSummary(payload->endcapItems(),ee1ErrorCount,ee2ErrorCount,ee1Total,ee2Total);
  ...
  void getBarrelErrorSummary(std::vector<EcalTPGCrystalStatusCode> vItems,long unsigned int & errorCount){
    unsigned int stcode = 0;
    
    for(std::vector<EcalTPGCrystalStatusCode>::const_iterator iItems = vItems.begin(); iItems != vItems.end(); ++iItems){
  ...
  void getEndCapErrorSummary(std::vector<EcalTPGCrystalStatusCode> vItems,long unsigned int & errorCountEE1,
