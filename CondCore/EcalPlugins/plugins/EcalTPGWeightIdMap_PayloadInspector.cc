 ...
 ******************************************/
  ...
  public:
    EcalTPGWeightIdMapPlot() : cond::payloadInspector::PlotImage<EcalTPGWeightIdMap>("Ecal TPG Weight Id Map - map ") {
      setSingleIov(true);
    }
    ...
    bool fill(const std::vector<std::tuple<cond::Time_t, cond::Hash> >& iovs) override {
      ...
      if (payload.get()) {
        ...
        align = new TH2F("Ecal TPG Weight Id Map",
                         "MapKey          w0                     w1                     w2                       w3    "
                         "                   w4",
                         6,
                         0,
...
PAYLOAD_INSPECTOR_MODULE(EcalTPGWeightIdMap) { PAYLOAD_INSPECTOR_CLASS(EcalTPGWeightIdMapPlot); }
