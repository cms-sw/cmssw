...
//#include "CondTools/Ecal/interface/EcalTPGPedestalsXMLTranslator.h"
...
namespace cond {
  ...
  template<>
  class ValueExtractor<EcalTPGPedestals>: public  BaseValueExtractor<EcalTPGPedestals> {
  public:
  ...
  private:
  	...
  	class EcalTPGPedestalsHelper: public EcalPyWrapperHelper<EcalTPGPedestal>{
	public:
		EcalTPGPedestalsHelper():EcalPyWrapperHelper<EcalObject>(3){}
	protected:
  ...
  template<>
  std::string PayLoadInspector<EcalTPGPedestals>::summary() const {
	std::stringstream ss;
	...
	template<>
	std::string PayLoadInspector<EcalTPGPedestals>::plot(std::string const & filename,
		std::string const &, 
...
PYTHON_WRAPPER(EcalTPGPedestals,EcalTPGPedestals);
