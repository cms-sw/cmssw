 ...
 ******************************************/
  class EcalClusterEnergyCorrectionObjectSpecificParametersPlot
      : public cond::payloadInspector::PlotImage<EcalClusterEnergyCorrectionObjectSpecificParameters> {
  public:
    EcalClusterEnergyCorrectionObjectSpecificParametersPlot()
        : cond::payloadInspector::PlotImage<EcalClusterEnergyCorrectionObjectSpecificParameters>(
              "Ecal Cluster Energy Correction Object Specific Parameters - map ") {
      setSingleIov(true);
    }

    bool fill(const std::vector<std::tuple<cond::Time_t, cond::Hash> >& iovs) override {
      auto iov = iovs.front();  //get reference to 1st element in the vector iovs
      std::shared_ptr<EcalClusterEnergyCorrectionObjectSpecificParameters> payload =
      ...
      if (payload.get()) {  //payload is an iov retrieved from payload using hash.
        ...
        fillFunctionParamsValues(align,
                                 (*payload).params(),
                                 "Ecal Cluster Energy Correction Object Specific Parameters",
                                 gridRows,
                                 NbColumns);
