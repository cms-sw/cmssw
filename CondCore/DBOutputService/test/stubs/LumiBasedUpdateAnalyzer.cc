...
LumiBasedUpdateAnalyzer::~LumiBasedUpdateAnalyzer() { std::cout << "LumiBasedUpdateAnalyzer::~LumiBasedUpdateAnalyzer" << std::endl; }
void LumiBasedUpdateAnalyzer::analyze(const edm::Event& evt, const edm::EventSetup& evtSetup) {
  ...
  std::cout <<"## last lumi: "<<lastLumi<<" run: "<<cond::time::unpack(lastLumi).first<<" lumiid:"<<lumiId<<std::endl;
