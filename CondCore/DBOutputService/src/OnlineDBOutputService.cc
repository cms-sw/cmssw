...
cond::service::OnlineDBOutputService::OnlineDBOutputService(const edm::ParameterSet& iConfig, edm::ActivityRegistry& iAR)
...
cond::Time_t cond::service::OnlineDBOutputService::getLastLumiProcessed(){
  ...
  if( !m_lastLumiUrl.size() == 0 ) {
    ...
    if( m_lastLumiFile.size() == 0 ){
      ...
      edm::LogInfo( MSGSOURCE ) << "Last lumi: "<<lastLumiProcessed<<" Current run: "<<upkTime.first<<" lumi id:"<<upkTime.second;
    }
  }
