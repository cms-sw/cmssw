  ...
  //m_logheaders()
{
  ...
  m_session.configuration().setParameters( connectionPset );
  //m_session.open( m_connectionString );  
  m_session.open( iConfig.getParameter<std::string>("connect") );  
...
cond::service::PoolDBOutputService::initDB( bool forReading )
{
  ...
  cond::db::TransactionScope scope( m_session.transaction() );
  try{ 
    if(!forReading) {
...
cond::service::PoolDBOutputService::createNewIOV( const Hash& firstPayloadId,
  ...
  if(!myrecord.m_isNewTag) {
    throw cond::db::Exception(myrecord.m_tag + " is not a new tag from PoolDBOutputService::createNewIOV",
  ...
  try{
    ...
    cond::db::IOVEditor editor = m_session.createIov( payloadType, myrecord.m_tag, myrecord.m_timetype, cond::OFFLINE ); 
    ...
    //}
    throw cond::db::Exception(std::string(er.what()) + " from PoolDBOutputService::createNewIOV ",
			      "PoolDBOutputService::createNewIOV");
  }
  scope.close();
}
...
cond::service::PoolDBOutputService::appendSinceTime( const Hash& payloadId,
  ...
  Record& myrecord=this->lookUpRecord(recordName);
  if( myrecord.m_isNewTag ) {
    throw cond::db::Exception(std::string("Cannot append to non-existing tag ") + myrecord.m_tag,
  ...
  try{
    cond::db::IOVEditor editor = m_session.editIov( myrecord.m_tag ); 
    ...
    //}
    throw cond::db::Exception(std::string(er.what()),
			      "PoolDBOutputService::appendSinceTime");
  }
  scope.close();
}
...
cond::service::PoolDBOutputService::closeIOV(Time_t lastTill, const std::string& recordName, 
  ...
  if( myrecord.m_isNewTag ) {
    throw cond::db::Exception(std::string("Cannot close non-existing tag ") + myrecord.m_tag,
...
/**
// Probably still required. Needs to implement a TagInfo_t in cond.
void 
