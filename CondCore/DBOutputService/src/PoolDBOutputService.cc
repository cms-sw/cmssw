...
void cond::service::PoolDBOutputService::fillRecord(edm::ParameterSet& pset) {
  Record thisrecord;

  thisrecord.m_idName = pset.getParameter<std::string>("record");
  thisrecord.m_tag = pset.getParameter<std::string>("tag");

  thisrecord.m_closeIOV = pset.getUntrackedParameter<bool>("closeIOV", m_closeIOV);

  thisrecord.m_timetype =
      cond::time::timeTypeFromName(pset.getUntrackedParameter<std::string>("timetype", m_timetypestr));
...
void cond::service::PoolDBOutputService::initDB(bool) {
...
void cond::service::PoolDBOutputService::postEndJob() {
  if (m_dbstarted) {
...
void cond::service::PoolDBOutputService::appendSinceTime(const std::string& payloadId,
                                                         cond::Time_t time,
                                                         const std::string& recordName,
                                                         bool withlogging) {
  std::lock_guard<std::recursive_mutex> lock(m_mutex);
...
void cond::service::PoolDBOutputService::closeIOV(Time_t lastTill, const std::string& recordName, bool withlogging) {
  std::lock_guard<std::recursive_mutex> lock(m_mutex);
...
// Still required.
void cond::service::PoolDBOutputService::tagInfo(const std::string& recordName, cond::TagInfo_t& result) {
  //
  std::lock_guard<std::recursive_mutex> lock(m_mutex);
