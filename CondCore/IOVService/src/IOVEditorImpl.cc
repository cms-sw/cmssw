...
namespace cond {
  ...
  IOVEditorImpl::bulkAppend(std::vector< std::pair<cond::Time_t,std::string> >& values){
    ...
    cond::Time_t   lastTime = values.back().second;
   ...
   if(LastTime>=lastTill() ||
      ( !m_iov->iov.empty() && firstTime<=m_iov->iov.back().second) 
  ...
  IOVEditorImpl::bulkAppend(std::vector< cond::IOVElement >& values){
   ...
   if(LastTime>=lastTill() ||
      ( !m_iov->iov.empty() && firstTime<=m_iov->iov.back().sinceTime()) 
       )    throw cond::Exception("cond::IOVEditorImpl::bulkInsert IOV not in range");
  ...
  IOVEditorImpl::append( cond::Time_t sinceTime,
   ...
   if(!validTime(sinceTime))
    ...
    if(  m_iov->iov.size()>1 ){
      //range check in case 
      cond::Time_t lastValidSince=(++m_iov->iov.rbegin())->sinceTime();
      //std::cout<<"lastValidTill "<<lastValidTill<<std::endl;
      if( sinceTime<= lastValidTill){
 ...
 IOVEditorImpl::freeInsert( cond::Time_t sinceTime ,
   ...
   if (p!=m_iov->iov.end() &&  (*(p-1)).sinceTime()==sinceTime)
  ...
  IOVEditorImpl::deleteEntries(bool withPayload){
    ...
    if(withPayload){
      std::string tokenStr;
      IOVSequence::iterator payloadIt;
      IOVSequence::iterator payloadItEnd=m_iov->iov.end();
