...
namespace {
  ...
  ******************************************/
  class HcalGainRatiosPlot : public cond::payloadInspector::PlotImage<HcalGains> {
  public:
    HcalGainRatiosPlot() : cond::payloadInspector::PlotImage<HcalGains>("HCAL Gain Ratios - map ") {
      setSingleIov( true );
    }
	...
	// TODO: fix DetIdLime
	int DetIdLim = 10000
	for(int cellid = 0; cellid < DetIdLim; ++cellid) {    // loop on EB cells
	  HcalCalibDetId DetIdMap = new HcalCalibDetId(cellid);  
          uint32_t rawid = DetIdMap->id_
	  Double_t phi = (Double_t)DetIdMap.iphi() - 0.5;
	  Double_t eta = (Double_t)DetIdMap.ieta();
...
PAYLOAD_INSPECTOR_MODULE(HcalGains){
  PAYLOAD_INSPECTOR_CLASS(HcalGainRatiosPlot);
