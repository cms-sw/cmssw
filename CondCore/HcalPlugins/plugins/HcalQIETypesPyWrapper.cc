...
namespace cond {
	template<>
	class ValueExtractor<HcalQIETypes>: public  BaseValueExtractor<HcalQIETypes> {
	public:
		typedef HcalQIETypes Class;
	...
	class HcalQIETypesDataRepr: public ADataRepr
	{
	public:
		HcalQIETypesDataRepr(unsigned int total, HcalQIETypes::tAllContWithNames const & allCont)
			:ADataRepr(total), allContainers(allCont){}



	protected:
		HcalQIETypes::tAllContWithNames allContainers;

		void doFillIn(std::vector<TH2F> &graphData) override{
			//ITERATORS AND VALUES:
			HcalQIETypes::tAllContWithNames::const_iterator iter;
			std::vector<HcalQIEType>::const_iterator contIter;
			float value = 0.0;
	...
	template<>
	std::string PayLoadInspector<HcalQIETypes>::summary() const {
		std::stringstream ss;
		unsigned int totalValues = 1;
		// get all containers with names
		HcalQIETypes::tAllContWithNames allContainers = object().getAllContainers();
	...
	template<>
	std::string PayLoadInspector<HcalQIETypes>::plot(std::string const & filename,
		std::string const &,
		...
		std::vector<float> const& ) const 
	{
		...
		HcalQIETypesDataRepr datarepr(numOfValues, object().getAllContainers());

		datarepr.nr = 0;
		datarepr.id = 0;
		datarepr.rootname.str("_HcalQIETyperootvalue_");
		datarepr.plotname.str("HcalQIEType ");
...
PYTHON_WRAPPER(HcalQIETypes,HcalQIETypes);
