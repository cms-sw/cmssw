...
namespace cond {
	template<>
	class ValueExtractor<HcalQIEDataExtended>: public  BaseValueExtractor<HcalQIEDataExtended> {
	public:
		typedef HcalQIEDataExtended Class;
	...
	class HcalQIEDataExtendedDataRepr: public ADataRepr
	{
	protected:
		HcalQIEDataExtended::tAllContWithNames allContainers;
	public:
		unsigned int fCapId;
		unsigned int fRange;
		bool slopeOrOffset;

		HcalQIEDataExtendedDataRepr(unsigned int total, HcalQIEDataExtended::tAllContWithNames const & allCont)
	...
	protected:
		void doFillIn(std::vector<TH2F> &graphData) override{
			...
			HcalQIEDataExtended::tAllContWithNames::const_iterator iter;
			std::vector<HcalQIECoderExtended>::const_iterator contIter;
			float value = 0.0;
	...
	std::string QIEDataExtendedCounter(const int nr, unsigned int &formated_nr, int base = 4){
		int numer = nr;
		int tens = 0, ones = 0;
		if (numer >= 16){
			numer = numer -16;
	...
	template<>
	std::string PayLoadInspector<HcalQIEDataExtended>::summary() const {
		std::stringstream ss;
		// get all containers with names
		HcalQIEDataExtended::tAllContWithNames allContainers = object().getAllContainers();
	...
	template<>
	std::string PayLoadInspector<HcalQIEDataExtended>::plot(std::string const & filename,
		std::string const &,
		...
		std::vector<float> const& ) const 
	{
		...
		HcalQIEDataExtendedDataRepr datarepr(numOfValues, object().getAllContainers());
		...
		for (unsigned int fCapId = 0; fCapId < 4; ++fCapId){
			for (unsigned int fRange = 0; fRange < 4; ++fRange){
				...
				QIEDataExtendedCounter(datarepr.id, datarepr.nr);
...
PYTHON_WRAPPER(HcalQIEDataExtended,HcalQIEDataExtended);
