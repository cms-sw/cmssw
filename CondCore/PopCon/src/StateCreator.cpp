...
popcon::StateCreator::StateCreator(const std::string& connectionString, 
  ...
  if( loc == std::string::npos ) {
    m_sqlite = false;
    conHandler.registerConnection("statedb",m_connect,0);
    conHandler.registerConnection("offlinedb",m_offline,0);
...
void popcon::StateCreator::storeStatusData()
{
  ...
  if (m_sqlite)
    return;
  cond::CoralTransaction& coraldb=conHandler.getConnection("statedb")->coralTransaction();
  coraldb.start(false);
...
void popcon::StateCreator::generateStatusData()
{
  ...
  cond::CoralTransaction& status_db=conHandler.getConnection("statedb")->coralTransaction();
...
{
  if (m_debug)
    std::cerr<< "State creator: " << " get pool data from "<<m_offline<<"\n";	
  cond::CoralTransaction& offline_db=conHandler.getConnection("offlinedb")->coralTransaction();
