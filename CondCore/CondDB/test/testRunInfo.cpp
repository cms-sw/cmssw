...
int run( const std::string& connectionString ){
  try{
    ...
    if( it != reader.end() ){
      auto rdata = *it;
      std::cout <<"For target=1015 Found run="<<rdata.run<<" start="<<rdata.start<<" end="<<rdata.end<<std::endl;
    } else std::cout <<" Can't find run 1015 in the selected range"<<std::endl;
    it = reader.find( 1030 );
    if( it != reader.end() ){
      auto rdata = *it;
      std::cout <<"For target=1030 Found run="<<rdata.run<<" start="<<rdata.start<<" end="<<rdata.end<<std::endl;
    } else std::cout <<" Can't find run 1030 in the selected range"<<std::endl;
    it = reader.find( 1035 );
    if( it != reader.end() ){
      auto rdata = *it;
      std::cout <<"For target=1035 Found run="<<rdata.run<<" start="<<rdata.start<<" end="<<rdata.end<<std::endl;
