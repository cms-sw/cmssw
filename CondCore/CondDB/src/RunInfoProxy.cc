...
namespace cond {
  ...
  namespace persistency {
    ...
    struct IOVRunComp {
      ...
      bool operator()( const std::tuple<cond::Time_t,boost::posix_time::ptime,boost::posix_time::ptime>& x, const cond::Time_t& y ){ return ( y > std::get<0>(x) ); }
    ...
    struct IOVTimeComp {
      ...
      bool operator()( const std::tuple<cond::Time_t,boost::posix_time::ptime,boost::posix_time::ptime>& x, const boost::posix_time::ptime& y ){ return ( y > std::get<2>(x) ); }
    ...
    cond::RunInfo_t RunInfoProxy::get( Time_t target ) const{
      ...
      if( it == Iterator() ) throwException("No data has been found.","RunInfoProxy::get(Time_t)" );
      if( it == end() ) throwException("The target run has not been found in the selected run range.","RunInfoProxy::get(Time_t)" );
      return *it;
    }
    ...
    cond::RunInfo_t RunInfoProxy::get( const boost::posix_time::ptime&  target ) const {
      Iterator it = find( target );
      if( it == Iterator() ) throwException("No data has been found.","RunInfoProxy::get(const boost::posix_time::ptime&)" );
      if( it == end() ) throwException("The target time has not been found in the selected time range.","RunInfoProxy::get(const boost::posix_time::ptime&)" );
      return *it;
    }
