...
namespace new_impl {
  ...
  public:
    ...
    // GT data
    std::string name;
    conddb::Time_t validity = conddb::time::MAX;
...
void GTEditor::load( const std::string& gtName ){
  ...
  if( !conddb::GLOBAL_TAG::select( gtName, m_data->validity, m_data->description, m_data->release, m_data->snapshotTime, *m_session ) ){
    conddb::throwException( "Global Tag \""+gtName+"\" has not been found in the database.","GTEditor::load");
  }
  m_data->name = gtName;
  m_data->exists = true;
  m_data->change = false;
...
conddb::Time_t GTEditor::validity() const {
  return m_data.get()? m_data->validity : conddb::time::MIN;
}
    
void GTEditor::setValidity( conddb::Time_t validity ){
  if( m_data.get() ) {
...
bool GTEditor::flush( const boost::posix_time::ptime& operationTime ){
  ...
  if( m_data->change ){
    if( m_data->description.empty() ) conddb::throwException( "A non-empty Description string is mandatory.","GTEditor::flush" );
    if( m_data->release.empty() ) conddb::throwException( "A non-empty Release string is mandatory.","GTEditor::flush" );
    if( !m_data->exists ){
      conddb::GLOBAL_TAG::insert( m_data->name, m_data->validity, m_data->description, m_data->release, 
			  m_data->snapshotTime, operationTime, *m_session );
      ret = true;
      m_data->exists = true;
    } else {
      conddb::GLOBAL_TAG::update( m_data->name,  m_data->validity, m_data->description, m_data->release,
			  m_data->snapshotTime, operationTime, *m_session );
      ret = true;
    }
    m_data->change = false;  
  ...
  if( m_data->tagListBuffer.size() ) {
    ...
    // insert the new iovs
    conddb::GLOBAL_TAG_MAP::insert( m_data->name, m_data->tagListBuffer, *m_session );
