...
namespace conddb {
  ...
  table( GLOBAL_TAG ) {
    ...
    bool select( const std::string& name, conddb::Time_t& validity, boost::posix_time::ptime& snapshotTime, SessionImpl& session );
    bool select( const std::string& name, conddb::Time_t& validity, std::string& description, 
		 std::string& release, boost::posix_time::ptime& snapshotTime, SessionImpl& session );
    void insert( const std::string& name, conddb::Time_t validity, std::string& description, const std::string& release, 
		 const boost::posix_time::ptime& snapshotTime, const boost::posix_time::ptime& insertionTime, SessionImpl& session );
    void update( const std::string& name, conddb::Time_t validity, std::string& description, const std::string& release, 
		 const boost::posix_time::ptime& snapshotTime, const boost::posix_time::ptime& insertionTime, SessionImpl& session );
  }
