...
class Server:
	...
	def ShowTable(self, curs, label):
		res1 = results_db.GetWebStatus(curs[0], label)
		res2, stCount = results_db.GetWebResultsList(curs[0], label)
		#print stCount
		Code ="""
	...
	def index(self, **args):
		...
		it = 0
		for row in results_db.GetWebLabels():
		...
		if(selectRel == True):
			if 'arch' in cherrypy.session.keys() and 'release' in cherrypy.session.keys():
				DBdata = results_db.GetWebReleasesHeaders(cherrypy.session['label'], cherrypy.session['release'], cherrypy.session['arch'])
			elif 'release' in cherrypy.session.keys():
				DBdata = results_db.GetWebReleasesHeaders(cherrypy.session['label'], cherrypy.session['release'])
			elif 'arch' in cherrypy.session.keys():
				DBdata = results_db.GetWebReleasesHeaders(cherrypy.session['label'], "", cherrypy.session['arch'])
			counter =0
			for data in DBdata:
				counter += 1
				if stCount != 0:
			...
			DBdata = results_db.GetWebStatusHeaders(cherrypy.session['label'])
			for data in DBdata:
				if stCount != 0:
					htmlCode += self.ShowTable(data, cherrypy.session['label'])
					stCount = stCount -1
	...
	def showLogs(self, **args):
		...
		logInfo += results_db.GetWebReadLogStatus(label, runID)
		logInfo += '\n</pre></body></html>'
		return logInfo
