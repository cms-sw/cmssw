...
CREATE TABLE RRES_AUX_1 AS SELECT ROWNUM RID, RUNID, LABEL FROM RRES_AUX_0;
CREATE TABLE RUN_RESULT_TMP AS SELECT T0.ID, T0.RUNID, T0.RDATE, T0.LABEL, T0.T_RELEASE, T0.T_ARCH, T0.R_RELEASE, T0.R_ARCH, T0.LOG, T1.RID  FROM TEST_STATUS T0, RRES_AUX_1 T1 WHERE T0.RUNID=T1.RUNID AND T0.LABEL=T1.LABEL;
CREATE TABLE RUN_RESULT AS SELECT ROWNUM E_ID, ID, RUNID, RDATE, LABEL, T_RELEASE, T_ARCH, R_RELEASE, R_ARCH, LOG, RID  FROM  RUN_RESULT_TMP;
