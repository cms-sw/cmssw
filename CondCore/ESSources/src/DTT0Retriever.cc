...
// $Id: TrackerPedestalsRetriever.cc,v 1.1 2005/07/27 19:48:22 xiezhen Exp $
//
...
DTT0Retriever::DTT0Retriever( const edm::ParameterSet&  pset)
{         
...
//
const DTT0*
DTT0Retriever::produce( const DTT0Rcd& )
{
  std::cout<<"DTT0Retriever::produce"<<std::endl;
  try{
    peds_ = pool::Ref<DTT0>(svc_.get(),pedCid_);
    *peds_;
  }catch(seal::Exception& e){
    std::cout << e.what() << std::endl;
  } catch ( std::exception& e ) {
...
void
DTT0Retriever::setIntervalFor( const EventSetupRecordKey&,
			       const edm::Timestamp& iTime, 
			       edm::ValidityInterval& oValidity)
{
  std::cout<<"DTT0Retriever::setIntervalFor "<< iTime.value()<<std::endl;
  ...
  std::cout<<"end DTT0Retriever::setIntervalFor"<<std::endl;
}
