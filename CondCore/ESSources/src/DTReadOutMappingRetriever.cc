...
// member functions
//
const DTReadOutMapping*
DTReadOutMappingRetriever::produce( const DTReadOutMappingRcd& )
{
  std::cout<<"DTReadOutMappingRetriever::produce"<<std::endl;
  try{
    peds_ = pool::Ref<DTReadOutMapping>(svc_.get(),pedCid_);
    *peds_;
  }catch(seal::Exception& e){
    std::cout << e.what() << std::endl;
  } catch ( std::exception& e ) {
