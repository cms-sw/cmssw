...
SiStripReadOutCablingRetriever::SiStripReadOutCablingRetriever( const edm::ParameterSet&  pset)
{         
  //  std::cout<<"PedestalRetriever::PedestalRetriever"<<std::endl;
  //Tell Producer what we produce
  setWhatProduced(this);
  //Tell Finder what records we find
  findingRecord<SiStripReadOutCablingRcd>();
...
SiStripReadOutCablingRetriever::~SiStripReadOutCablingRetriever()
{
  std::cout<<"SiStripReadOutCablingRetriever::~SiStripReadOutCablingRetriever"<<std::endl;
...
// member functions
//
const SiStripReadOutCabling*
SiStripReadOutCablingRetriever::produce( const SiStripReadOutCablingRcd& )
{
  std::cout<<"SiStripReadOutCablingRetriever::produce"<<std::endl;
  try{
    peds_ = pool::Ref<SiStripReadOutCabling>(svc_.get(),pedCid_);
    *peds_;
  }catch(seal::Exception& e){
    std::cout << e.what() << std::endl;
  } catch ( std::exception& e ) {
...
void
SiStripReadOutCablingRetriever::setIntervalFor( const EventSetupRecordKey&,
						const edm::Timestamp& iTime, 
						edm::ValidityInterval& oValidity)
{
  std::cout<<"SiStripReadOutCablingRetriever::setIntervalFor "<< iTime.value()<<std::endl;
  ...
  std::cout<<"end SiStripReadOutCablingRetriever::setIntervalFor"<<std::endl;
}
