...
GenParticleParentage::GenParticleParentage(const reco::GenParticleRef& m) : _match(m) {
...
void GenParticleParentage::getParentageRecursive(const reco::GenParticleRef& p, int daughterId) {
  ...
  if (p->numberOfMothers() == 0)
    return;  // no mothers
  if (p->pt() < 0.1)
    return;  // reached initial state quarks/proton
...
bool GenParticleParentage::hasAsParent(const reco::GenParticleRef& d, const reco::GenParticleRef& pc) const {
