...
void HLTMuTree::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  using namespace edm;
  using namespace std;
  ...
  if (doGen) {
    ...
    if (genColl.isValid()) {
      ...
      for (unsigned i = 0; i < genColl->size(); ++i) {
        pat::PackedGenParticle const& genPtl = genColl->at(i);
  ...
  if (doReco) {
    ...
    for (unsigned int i = 0; i < muons->size(); i++) {
      ...
      if (muCand->isStandAloneMuon() && muCand->outerTrack().isNonnull()) {
        if (muCand->standAloneMuon().get()->hitPattern().numberOfValidMuonHits() > 0 &&
            fabs(muCand->standAloneMuon()->eta()) < 2.4) {
          ...
          for (unsigned int j = i + 1; j < muons->size(); j++) {
            ...
            if (muCand2->globalTrack().isNonnull() && muCand2->innerTrack().isNonnull()) {
              if (muCand2->isGlobalMuon() && muCand2->isTrackerMuon() && fabs(muCand2->combinedMuon()->eta()) < 2.4) {
                ...
                t_tks.push_back(
                    theTTBuilder->build(*muCand2->track()));  // otherwise the vertex will have transient refs inside.
