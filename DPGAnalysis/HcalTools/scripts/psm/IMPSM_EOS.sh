...
HistoDir='/store/group/dpg_hcal/comm_hcal/www/HcalRemoteMonitoring/IMPSM/histos'
...
#    ./../../macros/psm/RemoteMonitoringALCARECOPSM.cc.exe Global_$runnumber.root Global_$runnumber.root Global 2>&1 | tee -a ${logFile}
    ./../../macros/psm/RemoteMonitoringIMPSM.cc.exe Global_$runnumber.root Global_$runnumber.root 2>&1 | tee -a ${logFile}

    if [ ! $? -eq 0 ] ; then
	echo "IMPSM processing failed"
	exit 2
    fi
...
##    fi
    ...
    local_WebDir=dir-IMPSM-GLOBAL_${runnumber}
    rm -rf ${local_WebDir}
...
#---------------111
# first variant:
    mkdir $WebDir/IMPSM/GLOBAL_$runnumber
    cp *.png $WebDir/IMPSM/GLOBAL_$runnumber/.
    cp *.html $WebDir/IMPSM/GLOBAL_$runnumber/.
...
#scp -r ${local_WebDir} $WebDir/IMPSM/GLOBAL_$runnumber
...
#    files=`cd ${local_WebDir}; ls`
    ###echo "IMPSM files=${files}"
#    if [ ${debug} -eq 0 ] ; then
#	eos mkdir -p $WebDir/IMPSM/GLOBAL_$runnumber
#	if [ ! $? -eq 0 ] ; then
#	    echo "IMPSM eos mkdir failed"
#	    exit 2
#	fi
#	for f in ${files} ; do
#	    echo "eoscp ${local_WebDir}/${f} $WebDir/IMPSM/GLOBAL_$runnumber/${f}"
#	    eos cp ${local_WebDir}/${f} $WebDir/IMPSM/GLOBAL_$runnumber/${f}
#	    if [ ! $? -eq 0 ] ; then
#		echo "IMPSM eoscp failed for ${f}"
####		exit 2
#	    fi
#	done
#    else
