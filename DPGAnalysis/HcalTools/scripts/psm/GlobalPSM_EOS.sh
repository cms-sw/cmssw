...
HistoDir='/store/group/dpg_hcal/comm_hcal/www/HcalRemoteMonitoring/GlobalPSM/histos'
...
#    ./../../macros/psm/RemoteMonitoringPSM.cc.exe Global_$runnumber.root 2>&1 | tee -a ${logFile}
    ./../../macros/psm/RemoteMonitoringPSM.cc.exe Global_$runnumber.root Global_$runnumber.root Global 2>&1 | tee -a ${logFile}
...
##    fi
    ...
    local_WebDir=dir-GlobalPSM-GLOBAL_${runnumber}
    rm -rf ${local_WebDir}
...
#---------------111
# first variant:
    mkdir $WebDir/GlobalPSM/GLOBAL_$runnumber
    cp *.png $WebDir/GlobalPSM/GLOBAL_$runnumber/.
    cp *.html $WebDir/GlobalPSM/GLOBAL_$runnumber/.
...
#scp -r ${local_WebDir} $WebDir/GlobalPSM/GLOBAL_$runnumber
...
#    files=`cd ${local_WebDir}; ls`
    ###echo "GlobalPSM files=${files}"
#    if [ ${debug} -eq 0 ] ; then
#	eos mkdir -p $WebDir/GlobalPSM/GLOBAL_$runnumber
#	if [ ! $? -eq 0 ] ; then
#	    echo "GlobalPSM eos mkdir failed"
#	    exit 2
#	fi
#	for f in ${files} ; do
#	    echo "eoscp ${local_WebDir}/${f} $WebDir/GlobalPSM/GLOBAL_$runnumber/${f}"
#	    eos cp ${local_WebDir}/${f} $WebDir/GlobalPSM/GLOBAL_$runnumber/${f}
#	    if [ ! $? -eq 0 ] ; then
#		echo "GlobalPSM eoscp failed for ${f}"
####		exit 2
#	    fi
#	done
#    else
