...
bool orderLuminosityBlockRange (edm::LuminosityBlockRange u, edm::LuminosityBlockRange v) { return ( u.startRun() < v.startRun() ); }
...
PickEvents::PickEvents(const edm::ParameterSet& iConfig)
{
  ...
  luminositySectionsBlockRanges_ = iConfig.getUntrackedParameter< std::vector<edm::LuminosityBlockRange> >("LuminositySectionsBlockRange");
...
PickEvents::filter(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   ...
   if( isRunLsBased_ ) {
	 ...
	 if (         luminositySectionsBlockRange.endRun() < kRun   ) continue;
