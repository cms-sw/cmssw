...
EventWithHistory::EventWithHistory(const std::vector<edm::EventAuxiliary>& veaux)
  ...
  for (auto eaux = veaux.rbegin(); eaux != veaux.rend(); eaux++) {
...
EventWithHistory::EventWithHistory(const edm::Event& event,
  ...
  for (auto l1abc = l1abcc.begin(); l1abc != l1abcc.end(); ++l1abc) {
	...
	*/
        ...
        if (l1abc->eventType() != 0) {
          ...
          for (auto debu = l1abcc.begin(); debu != l1abcc.end(); ++debu) {
...
int EventWithHistory::add(const EventWithHistory& he, const int idepth) {
  ...
  for (auto ev = he._prevse.begin(); ev != he._prevse.end(); ev++) {
    if (!add(*ev, idepth))
      return 0;
  }
  return 1;
