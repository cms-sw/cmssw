...
RunHistogramManager::RunHistogramManager(edm::ConsumesCollector & iC, const bool fillHistograms):
  _fillHistograms(fillHistograms), _histograms(), _cirbToken(iC.mayConsume<edm::ConditionsInRunBlock>(edm::InputTag("conditionsInEdm"))) { }
...
void  RunHistogramManager::beginRun(const edm::Run& iRun, TFileDirectory& subdir) {
  ...
  if(!_fillHistograms) {
    ...
    iRun.getByToken(_cirbToken,cirb);
