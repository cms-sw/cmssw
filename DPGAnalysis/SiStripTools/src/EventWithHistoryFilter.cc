...
const bool EventWithHistoryFilter::is_selected(const EventWithHistory& he, const edm::EventSetup& iSetup, const std::vector<int>& _apvphases) const {
  ...
  if(!isAPVModeNotNeeded()) {
    ...
    bool modeok = false;
    for(int m_apvmode : m_apvmodes) {
      modeok = modeok || (apvmode == m_apvmode);
    }
  ...
  for(int apvphase : apvphases) {
    phaseselected = phaseselected || isInRange(he.absoluteBXinCycle(apvphase)%70,m_bxcyclerange,apvphase>=0);
  }
  selected = selected && phaseselected;

  phaseselected = isCutInactive(m_bxcyclerangelat);
  for(int apvphase : apvphases) {
    phaseselected = phaseselected || isInRange((he.absoluteBXinCycle(apvphase)-latency)%70,m_bxcyclerangelat,
  ...
  for(int apvphase : apvphases) {
    phaseselected = phaseselected || isInRange(he.deltaBXinCycle(apvphase),m_dbxcyclerange,he.depth()!=0 && apvphase>=0);
  }
  selected = selected && phaseselected;

  phaseselected = isCutInactive(m_dbxcyclerangelat);
  for(int apvphase : apvphases) {
    phaseselected = phaseselected || isInRange(he.deltaBXinCycle(apvphase)-latency,m_dbxcyclerangelat,
