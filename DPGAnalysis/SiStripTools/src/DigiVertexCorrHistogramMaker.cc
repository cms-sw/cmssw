...
void DigiVertexCorrHistogramMaker::book(const std::string dirname, edm::ConsumesCollector& iC) {
  ...
  for (std::map<unsigned int, std::string>::const_iterator lab = m_labels.begin(); lab != m_labels.end(); ++lab) {
    ...
    if (m_subdirs[i]) {
      ...
      m_nmultvsnvtx[i] = m_subdirs[i]->make<TH2F>(name,
                                                  title,
                                                  m_maxnvtx,
                                                  -0.5,
                                                  m_maxnvtx - 0.5,
                                                  m_nbins,
                                                  0.,
                                                  (1 + m_binmax[i] / (m_scalefact * m_nbins)) * m_nbins);
      m_nmultvsnvtx[i]->GetXaxis()->SetTitle("Number of Vertices");
      m_nmultvsnvtx[i]->GetYaxis()->SetTitle("Number of Hits");
      ...
      m_nmultvsnvtxprof[i]->GetXaxis()->SetTitle("Number of Vertices");
      m_nmultvsnvtxprof[i]->GetYaxis()->SetTitle("Number of Hits");
      ...
      if (m_runHisto) {
        ...
        m_nmultvsnvtxvsbxprofrun[i] =
            m_fhm[i]->makeTProfile2D(name, title, 3564, -0.5, 3563.5, m_maxnvtx, -0.5, m_maxnvtx - 0.5);
      }
    }
  }
}
...
void DigiVertexCorrHistogramMaker::beginRun(const edm::Run& iRun) {
  ...
  for (std::map<unsigned int, std::string>::const_iterator lab = m_labels.begin(); lab != m_labels.end(); ++lab) {
    ...
    if (m_runHisto) {
      (*m_nmultvsnvtxvsbxprofrun[i])->GetXaxis()->SetTitle("BX");
      (*m_nmultvsnvtxvsbxprofrun[i])->GetYaxis()->SetTitle("Nvertices");
    }
  }
}
