  ...
  m_subdets(),m_subdetsels()
{
  ...
  for(auto & wantedsubd : wantedsubds) {
    ...
    m_subdetsels[wantedsubd.getParameter<unsigned int>("detSelection")] =
      DetIdSelector(wantedsubd.getUntrackedParameter<std::vector<std::string> >("selection",std::vector<std::string>()));
  }
}
