...
L1ABCDebugger::L1ABCDebugger(const edm::ParameterSet& iConfig)
    : m_l1abccollectionToken(
          consumes<L1AcceptBunchCrossingCollection>(iConfig.getParameter<edm::InputTag>("l1ABCCollection"))),
  ...
  m_hoffsets = m_rhm.makeTH2F(
      "offsets", "Orbit vs BX offsets between SCAL and Event", 2 * 3564 + 1, -3564.5, 3564.5, 201, -100.5, 100.5);
  m_horboffvsorb =
      m_rhm.makeTProfile("orboffvsorb", "SCAL Orbit offset vs orbit number", m_LSfrac * m_maxLS, 0, m_maxLS * 262144);
  m_hbxoffvsorb =
      m_rhm.makeTProfile("bxoffvsorb", "SCAL BX offset vs orbit number", m_LSfrac * m_maxLS, 0, m_maxLS * 262144);
}
...
void L1ABCDebugger::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  using namespace edm;
...
void L1ABCDebugger::beginRun(const edm::Run& iRun, const edm::EventSetup& iSetup) {
  m_rhm.beginRun(iRun);

  if (m_hoffsets && *m_hoffsets) {
    (*m_hoffsets)->GetXaxis()->SetTitle("#Delta BX (SCAL-Event)");
    (*m_hoffsets)->GetYaxis()->SetTitle("#Delta orbit (SCAL-Event)");
  }
  if (m_horboffvsorb && *m_horboffvsorb) {
    (*m_horboffvsorb)->GetXaxis()->SetTitle("Orbit");
    (*m_horboffvsorb)->GetYaxis()->SetTitle("#Delta orbit (SCAL-Event)");
    (*m_horboffvsorb)->SetCanExtend(TH1::kXaxis);
  }
  if (m_hbxoffvsorb && *m_hbxoffvsorb) {
    (*m_hbxoffvsorb)->GetXaxis()->SetTitle("Orbit");
    (*m_hbxoffvsorb)->GetYaxis()->SetTitle("#Delta BX (SCAL-Event)");
