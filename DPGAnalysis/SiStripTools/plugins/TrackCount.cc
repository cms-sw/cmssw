...

{
  ...
  m_ntrk->GetXaxis()->SetTitle("Tracks");
  m_ntrk->GetYaxis()->SetTitle("Events");
  m_ntrkvslumi = tfserv->make<TProfile>("ntrkvslumi", "Number of Tracks vs Luminosity", 250, 0., 10.);
  m_ntrkvslumi->GetXaxis()->SetTitle("BX lumi [10^{30}cm^{-2}s^{-1}]");
  m_ntrkvslumi->GetYaxis()->SetTitle("Ntracks");
  m_ntrkvslumi2D = tfserv->make<TH2D>("ntrkvslumi2D", "Number of Tracks vs Luminosity", 80, 0., 10., 125, -0.5, 2499.5);
  m_ntrkvslumi2D->GetXaxis()->SetTitle("BX lumi [10^{30}cm^{-2}s^{-1}]");
  m_ntrkvslumi2D->GetYaxis()->SetTitle("Ntracks");

  m_nhptrk = tfserv->make<TH1F>("nhptrk", "Number of High Purity Tracks", 2500, -0.5, 2499.5);
  m_nhptrk->GetXaxis()->SetTitle("Tracks");
  m_nhptrk->GetYaxis()->SetTitle("Events");
  m_hhpfrac = tfserv->make<TH1F>("hhpfrac", "Fraction of High Purity Tracks", 51, 0., 1.02);
  m_hhpfrac->GetXaxis()->SetTitle("hp/all");
  m_hhpfrac->GetYaxis()->SetTitle("Events");
  m_hsqsumptsq = tfserv->make<TH1F>("hsqsumptsq", "Sqrt(Sum pt**2)", 1000, 0., 200.);
  m_hsqsumptsq->GetXaxis()->SetTitle("#sqrt(#Sigma pt^2) (GeV)");
  m_hsqsumptsq->GetYaxis()->SetTitle("Events");

  m_hphi = tfserv->make<TH1F>("phi", "Track azimuth", 40, -M_PI, M_PI);
  m_hphi->GetXaxis()->SetTitle("#phi (rad)");
  m_hphi->GetYaxis()->SetTitle("Tracks");
  m_heta = tfserv->make<TH1F>("eta", "Track pseudorapidity", netabin1d, etamin, etamax);
  m_heta->GetXaxis()->SetTitle("#eta");
  m_heta->GetYaxis()->SetTitle("Tracks");
  m_hcos = tfserv->make<TH1F>("cos", "Track polar angle", 50, -1., 1.);
  m_hcos->GetXaxis()->SetTitle("cos(#theta)");
  m_hcos->GetYaxis()->SetTitle("Tracks");
  m_hpt = tfserv->make<TH1F>("pt", "Track pt", m_nptbin, m_ptmin, m_ptmax);
  m_hpt->GetXaxis()->SetTitle("pt (GeV)");
  m_hpt->GetYaxis()->SetTitle("Tracks");

  m_chisq = tfserv->make<TH1F>("chisq", "Track Chi2", nchisqbin1d, 0., 100.);
  m_chisq->GetXaxis()->SetTitle("chi2");
  m_chisq->GetYaxis()->SetTitle("Tracks");
  m_chisqnorm = tfserv->make<TH1F>("chisqnorm", "Track normalized Chi2", nchisqbin1d, 0., 10.);
  m_chisqnorm->GetXaxis()->SetTitle("normalized chi2");
  m_chisqnorm->GetYaxis()->SetTitle("Tracks");
  m_ndof = tfserv->make<TH1F>("ndof", "Track ndof", nndofbin1d, 0., 100.);
  m_ndof->GetXaxis()->SetTitle("ndof");
  m_ndof->GetYaxis()->SetTitle("Tracks");
  if (m_2dhistos) {
    m_chisqvseta =
        tfserv->make<TH2F>("chisqvseta", "Track Chi2 vs #eta", netabin2d, etamin, etamax, nchisqbin2d, 0., 100.);
    m_chisqvseta->GetXaxis()->SetTitle("#eta");
    m_chisqvseta->GetYaxis()->SetTitle("#chi2");
    m_chisqnormvseta = tfserv->make<TH2F>(
        "chisqnormvseta", "Track normalized Chi2 vs #eta", netabin2d, etamin, etamax, nchisqbin2d, 0., 10.);
    m_chisqnormvseta->GetXaxis()->SetTitle("#eta");
    m_chisqnormvseta->GetYaxis()->SetTitle("normalized #chi2");
    m_ndofvseta =
        tfserv->make<TH2F>("ndofvseta", "Track ndof vs #eta", netabin2d, etamin, etamax, nndofbin2d, 0., 100.);
    m_ndofvseta->GetXaxis()->SetTitle("#eta");
    m_ndofvseta->GetYaxis()->SetTitle("ndof");
  }

  m_hptphieta =
      tfserv->make<TProfile2D>("ptphivseta", "Average pt vs #phi vs #eta", netabin2d, etamin, etamax, 40, -M_PI, M_PI);
  m_hptphieta->GetXaxis()->SetTitle("#eta");
  m_hptphieta->GetYaxis()->SetTitle("#phi");

  m_hnlosthits = tfserv->make<TH1D>("nlosthits", "Number of Lost Hits", 10, -0.5, 9.5);
  m_hnlosthits->GetXaxis()->SetTitle("Nlost");
  m_hnlosthits->GetYaxis()->SetTitle("Tracks");

  m_hnrhits = tfserv->make<TH1D>("nrhits", "Number of Valid Hits", 55, -0.5, 54.5);
  m_hnrhits->GetXaxis()->SetTitle("Nvalid");
  m_hnrhits->GetYaxis()->SetTitle("Tracks");
  m_hnpixelrhits = tfserv->make<TH1D>("npixelrhits", "Number of Valid Pixel Hits", 20, -0.5, 19.5);
  m_hnpixelrhits->GetXaxis()->SetTitle("Nvalid");
  m_hnpixelrhits->GetYaxis()->SetTitle("Tracks");
  m_hnstriprhits = tfserv->make<TH1D>("nstriprhits", "Number of Valid Strip Hits", 45, -0.5, 44.5);
  m_hnstriprhits->GetXaxis()->SetTitle("Nvalid");
  m_hnstriprhits->GetYaxis()->SetTitle("Tracks");

  m_hnlostlayers = tfserv->make<TH1D>("nlostlayers", "Number of Layers w/o measurement", 10, -0.5, 9.5);
  m_hnlostlayers->GetXaxis()->SetTitle("Nlostlay");
  m_hnlostlayers->GetYaxis()->SetTitle("Tracks");

  m_hnlayers = tfserv->make<TH1D>("nlayers", "Number of Layers", 20, -0.5, 19.5);
  m_hnlayers->GetXaxis()->SetTitle("Nlayers");
  m_hnlayers->GetYaxis()->SetTitle("Tracks");
  m_hnpixellayers = tfserv->make<TH1D>("npixellayers", "Number of Pixel Layers", 10, -0.5, 9.5);
  m_hnpixellayers->GetXaxis()->SetTitle("Nlayers");
  m_hnpixellayers->GetYaxis()->SetTitle("Tracks");
  m_hnstriplayers = tfserv->make<TH1D>("nstriplayers", "Number of Strip Layers", 20, -0.5, 19.5);
  m_hnstriplayers->GetXaxis()->SetTitle("Nlayers");
  m_hnstriplayers->GetYaxis()->SetTitle("Tracks");

  m_hnhitphieta = tfserv->make<TProfile2D>(
      "nhitphivseta", "N valid hits vs #phi vs #eta", netabin2d, etamin, etamax, 40, -M_PI, M_PI);
  m_hnhitphieta->GetXaxis()->SetTitle("#eta");
  m_hnhitphieta->GetYaxis()->SetTitle("#phi");
  m_hnlayerphieta = tfserv->make<TProfile2D>(
      "nlayerphivseta", "N layers vs #phi vs #eta", netabin2d, etamin, etamax, 40, -M_PI, M_PI);
  ...
  m_hphieta->GetXaxis()->SetTitle("#eta");
  m_hphieta->GetYaxis()->SetTitle("#phi");

  if (m_runHisto) {
    m_ntrkvsorbrun =
        m_rhm.makeTProfile("ntrkvsorbrun", "Number of tracks vs orbit number", m_maxLS * m_LSfrac, 0, m_maxLS * 262144);
  }
}
...
void TrackCount::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  using namespace edm;
  edm::Service<TFileService> tfserv;
  ...
  if (m_runHisto) {
    if (m_ntrkvsorbrun && *m_ntrkvsorbrun)
      (*m_ntrkvsorbrun)->Fill(iEvent.orbitNumber(), tracks->size());
  }
...
void TrackCount::beginRun(const edm::Run& iRun, const edm::EventSetup&) {
  m_rhm.beginRun(iRun);

  if (m_runHisto) {
    (*m_ntrkvsorbrun)->GetXaxis()->SetTitle("time [orbit#]");
    (*m_ntrkvsorbrun)->GetYaxis()->SetTitle("Ntracks");
